...
'If isEmpty(m_wxPayKindId) Then
	...
	Sub onReportInit(rpt, lt)
		...
		app.addDefaultCss
		app.addCssPath  "../../inc/themes/icon.css"
		Dim rs,CategoryList,StatusList,sTypeList,batchOptBtns
		rpt.title = "商品列表"
		Dim Referrer,groupid,productid
		Referrer = Request("Referrer")
		groupid = Request("groupid")
		productid = app.getint("productid")
		If app.power.existsPower(109,10) And Referrer <> "setWxShopHome" Then
			rpt.canexcel = True
		else
			rpt.canexcel = False
		end if
		If app.power.existsPower(109,7) And Referrer <> "setWxShopHome" Then
			rpt.canPrint = True
		else
			rpt.canPrint = False
		end if
		If app.power.existsPower(109,33) And Referrer <> "setWxShopHome" Then
			batchOptBtns = "批量下架,"
			batchOptBtns = batchOptBtns & "批量上架"
		end if
		rpt.batbuttons = batchOptBtns
		Set rs = cn.Execute("SELECT ord,sort1 FROM sortonehy WHERE isStop = 0 AND gate2 = 80 ORDER BY gate1 DESC")
		While rs.Eof = False
			CategoryList = CategoryList & rs("sort1") & Chr(1) & rs("ord")
			rs.movenext
			If Not rs.Eof Then
				CategoryList = CategoryList & Chr(2)
			end if
		wend
		rs.close
		set rs = nothing
		...
		statusDefV = -1
'Dim statusDefV
		If Referrer = "setWxShopHome" Then
			statusDefV = 0
		end if
		StatusList = StatusList & "上架" & Chr(1) & 1 & Chr(2)
		StatusList = StatusList & "下架" & Chr(1) & 0 & Chr(2)
		StatusList = StatusList & "定时上架" & Chr(1) & 2
		sTypeList = sTypeList & "商品名称" & Chr(1) & 1 & Chr(2)
		sTypeList = sTypeList & "商品编号" & Chr(1) & 2
		rpt.AddField2 "所属分类：","select","category",CategoryList,"","","全部=-1"
		sTypeList = sTypeList & "商品编号" & Chr(1) & 2
		rpt.AddField2 "状态：","select","status",StatusList,"","","全部=-1"
		sTypeList = sTypeList & "商品编号" & Chr(1) & 2
		rpt.AddField2 "条件：","select","sType",sTypeList,"","","==不限=="
		rpt.AddField "","text","keywords",,"",""
		rpt.addsort "添加时间", "1"
		rpt.addsort "可售数量", "2"
		rpt.addsort "重要指数", "3"
		If app.power.existsPower(106,13) And ZBruntime.MC(63000) And Referrer <> "setWxShopHome" Then
			rpt.beforeBtnsHTML = "<input type='button' value='添加二维码' class='oldbutton3' onclick=""selectGoodsToCode2();""  />"
		end if
		rpt.showtoppagesize = False
		rpt.PageSize = 10
		Dim settingHelper : Set settingHelper = GetSettingHelper(cn)
		Dim validTime : validTime = settingHelper.shop.preOrderValidTime
		rpt.sql = "EXEC [ERP_GoodsList] "&Info.User&",&ReportSort,@category,@status,@sType,@keywords,"& validTime &","& statusDefV &"," & productid & ",&pagesize,&pageindex,&excelmode"
		rpt.BillListUIModel = True
	end sub
	Sub App_doBatHandle(cmd, checkValues, batResult)
		Select Case cmd
		Case "批量下架"
		Call batchHandle(checkValues,batResult,0)
		Case "批量上架"
		Call batchHandle(checkValues,batResult,1)
		End Select
	end sub
	Sub batchHandle(checkValues,batResult,sType)
		If sType = 0 Then
			cn.Execute("UPDATE Shop_Goods SET onSale = 0,onSaleAfter = GETDATE() WHERE id IN ("& checkValues &")")
		ElseIf sType = 1 Then
			cn.Execute("UPDATE Shop_Goods SET onSale = 1 WHERE id IN ("& checkValues &")")
		end if
	end sub
	Sub app_oncreate(lvw)
		Dim col,batchOptBtns,beginCol,totalCol,i
		Dim Referrer,groupid
		Referrer = Request.form("rpt_f_Referrer")
		groupid = Request.form("rpt_f_groupid")
		If Referrer = "setWxShopHome" Then
			lvw.checkbox = False
		else
			lvw.checkbox = True
		...
		col.FormatText = "code:GetGoodsPic(@cells[""id""])"
		Set col = lvw.headers("单位")
		col.width = 80
		Set col = lvw.headers("商品属性")
		col.FormatText = "code:GetGoodsAttr(@cells[""id""])"
		col.width = 250
		Set col = lvw.headers("市场价")
		col.formattext = "￥@value"
		col.dbtype = "salesprice"
		col.canSum = false
		For i = 10 To lvw.headers.count
			Set col = lvw.headers(i)
			col.canSum = False
		next
		...
		col.width = 250
		If Referrer = "setWxShopHome" Then
			col.formattext = "code:GetWxShopHomeOptionCol(@cells[""id""],"""& groupid &""")"
		else
			col.formattext = "code:GetOptionCol(@cells[""id""],@cells[""creator""])"
		end if
		beginCol = 13
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		next
		Set col = lvw.headers("商品属性")
		col.Formula = "GetGoodsAttr(@cells[""ID""])"
		Set col = lvw.headers("关联产品")
		col.Formula = "PowerCol(@value,@cells[""proID""],@cells[""proCreator""],21)"
		For i = 14 To totalCol
			Set col = lvw.headers(i)
			col.display = "none"
		next
	end sub
	Function moneyFormat(moneyV)
		moneyFormat = "￥"&FormatNumber(moneyV,info.MoneyNumber,-1)
'Function moneyFormat(moneyV)
	end function
...
'If qx = 1 Then
	...
	Function GetOptionCol(gID,AddUser)
		Dim str,curID
		curID = app.base64.pwurl(gID)
		str = "<div class='option-col'>"
'curID = app.base64.pwurl(gID)
		If app.power.existsPowerIntro(109,14,AddUser) Then
			str = str&"<input type='button' value='详情' class='oldbutton2' onclick=""javascript:window.open('content.asp?ID="& curID &"','goodsContent','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" />&nbsp;"
'If app.power.existsPowerIntro(109,14,AddUser) Then  '
		end if
		If app.power.existsPowerIntro(109,2,AddUser) Then
			str = str&"<input type='button' value='修改' class='oldbutton2' onclick=""javascript:window.open('add.asp?Referrer=List&goodsID="& curID &"','goodsEdit','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" />&nbsp;"
'If app.power.existsPowerIntro(109,2,AddUser) Then   '
		end if
		If app.power.existsPowerIntro(109,31,AddUser) Then
			str = str&"<input type='button' value='追加' class='oldbutton2' onclick=""addGoodsNum("& gID &");"" />&nbsp;"
		end if
		str = str&"</div>"
		GetOptionCol = str
	end function
	Function GetWxShopHomeOptionCol(gID,groupid)
		Dim str,curID,rs,isUsing
		curID = app.base64.pwurl(gID)
		str = "<div class='option-col'>"
		curID = app.base64.pwurl(gID)
		Set rs = cn.Execute("SELECT 1 FROM Shop_HomeGroupItems WHERE groupid = "& groupid &" AND source = "& gID &" ")
		If Not rs.Eof Then
			isUsing = True
		else
			isUsing = False
		...
		If isUsing = False Then
			str = str & "<a href='javascript:;' class='select-goods-btn' gid='"& gID &"' groupid='"& groupid &"'>点击选择此商品</a>"
'If isUsing = False Then
		end if
		str = str&"</div>"
		GetWxShopHomeOptionCol = str
	end function
...
'Function FormatNum(num)
	...
	Function GetGoodsAttr(gID)
		Dim rs,sql,temp
		sql = "SELECT b.title FROM Shop_GoodsAttrValue a " &_
		"LEFT JOIN Shop_GoodsAttr b ON a.degreeID = b.id " &_
		"WHERE goodsID = "& gID &" " &_
	...
	Function GetGoodsPic(gID)
		Dim rs,sql,temp
		sql = "SELECT TOP 1 fpath FROM sys_upload_res WHERE id1 = "& gID &" AND id2 = 1 AND source = 'goodsPic' "
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
	...
	Sub App_saveGoodsNum
		Dim goodsID,goodsNum
		goodsID = Request("goodsID")
		goodsNum = Request("goodsNum")
		If goodsNum <> "" And IsNumeric(goodsNum) Then
			conn.CursorLocation = 3
			conn.begintrans
			cn.Execute("INSERT INTO Shop_StorageAppendLog (goodsId,num1,creator,createtime) VALUES ("& goodsID &","& goodsNum &","& Info.User &",GETDATE())")
			If Err.number <> 0 Then
				conn.rollbacktrans
	...
	Sub App_addNumPage
		Dim goodsName,goodsAttr,goodsNum,goodsUnit
		goodsName = Request("goodsName")
		goodsAttr = Request("goodsAttr")
		goodsNum = Request("goodsNum")
		goodsUnit = Request("goodsUnit")
		If goodsNum&"" = "" Then goodsNum = 0
		Response.write "" & vbcrlf & "<style>" & vbcrlf & ".edit-page {" & vbcrlf & "  padding: 20px 10px;" & vbcrlf & "}" & vbcrlf & ".edit-page input.f-text {" & vbcrlf & "   width: 150px;" & vbcrlf & "   height: 16px;" & vbcrlf & "   line-height: 16px;" & vbcrlf & "}" & vbcrlf & ".edit-page input.sort {" & vbcrlf & "      width: 50px;" & vbcrlf & "}" & vbcrlf & "#content {" & vbcrlf & "        width: 100%;" & vbcrlf & "    border: 1px solid #c0ccdd;" & vbcrlf & "      border-collapse: collapse;" & vbcrlf & "      background-image: none;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "#content th, #content td {" & vbcrlf & "      border: 1px solid #c0ccdd;" & vbcrlf & "       padding: 5px;" & vbcrlf & "   height: 24px;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "#content th{" & vbcrlf & "      width: 25%;" & vbcrlf & "     text-align: right;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & ".cur-num {" & vbcrlf & "   margin-bottom: 10px;" & vbcrlf & "}" & vbcrlf & ".description{" & vbcrlf & "       margin: 20px 0 10px 0;" & vbcrlf & "  text-align: center;" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "<div class=""edit-page"">" & vbcrlf & "       <div class=""cur-num"">当前可售数量："
'If goodsNum&"" = "" Then goodsNum = 0
		Response.write goodsNum
		Response.write "&nbsp;"
		Response.write goodsUnit
		Response.write "</div>" & vbcrlf & "       <form id=""editForm"" name=""editForm"" method=""post"" action=""?"" onkeydown=""if(event.keyCode==13)return false;"">" & vbcrlf & "              <table id=""content"">" & vbcrlf & "                      <tbody>" & vbcrlf & "                         <tr>" & vbcrlf & "                                    <th>商品名称：</th>" & vbcrlf & "                                     <td>"
		Response.write goodsName
		Response.write "</td>" & vbcrlf & "                                </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <th>商品属性：</th>" & vbcrlf & "                                     <td>"
		Response.write goodsAttr
		Response.write "</td>" & vbcrlf & "                                </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <th>追加数量：</th>" & vbcrlf & "                                     <td><input type=""text"" size=""12"" id=""goodsNum"" name=""goodsNum"" onkeyup=""value=value.replace(/[^-?\d\.]/g,'');checkDot('goodsNum','"
		Response.write goodsAttr
		Response.write info.floatnumber
		...
		Response.write info.floatnumber
		Response.write "')"" dataType=""number"" min=""-"
		Response.write info.floatnumber
		Response.write Abs(Replace(goodsNum,",",""))
		Response.write """ max=""100000000"" msg=""必填"" value="""">" & vbcrlf & "                                  <span class=""red"">*</span>" & vbcrlf & "                                </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </tbody>" & vbcrlf & "                </table>                " & vbcrlf & "        </form>" & vbcrlf & " <div class=""description"">说明：追加数量可以是正数、也可以是负数。负数不能大于可售数量</div>" & vbcrlf & "</div>" & vbcrlf & ""
	end sub
