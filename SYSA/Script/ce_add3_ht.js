...
function callServer(m,name1) {
  ...
  var url = "cu2.asp?name=" + escape(u_name)+"&ord="+escape(m)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
...
function getBirthYear() {
    ...
    if (xmlHttp.readyState == 4) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            if (window.birYear == json[i].yValue) { objBirthYear.options[i].selected = "selected"; }
        };
        getBirthMonth();
    }
}
function getBirthMonth() {
    ...
    if (xmlHttp.readyState == 4) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            if (window.birMonth == json[i].mValue) { objBirthMonth.options[i].selected = "selected"; }
        };
        getBirthDay();
    }
}
function getBirthDay() {
    ...
    if (xmlHttp.readyState == 4) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            if (window.birMonth == json[i].dValue) { objBirthDay.options[i].selected = "selected"; }
        };
    }
}
