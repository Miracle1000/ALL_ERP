...
startDate=s_tdDay1
endDate=s_TdEndDay
if datediff("d",date(),endDate)>0 then endDate=date()
...
rssort.close
set percentage1=conn.execute("select num1 from setjm3 where ord=20171221")
if not percentage1.eof then
percentage=percentage1(0)
else
percentage=2
end if
percentage1.close
baseSQL="select PAGE_COUNT_NUM from (select PAGE_TOP_NUM * from (select talNum as 总人数_DOSUM,leaveNum as 离职人数_DOSUM,case talNum when 0 then 0 else (cast(leaveNum as float)/cast(talNum as float))*100 end as 离职率  from (select (select count(*) from hr_leave  where del=0 and status=3 and datediff(d,'"&startDate&"',indate)>=0 and datediff(d,'"&endDate&"',indate)<=0 ) leaveNum,(select count(*) from gate a inner join hr_person b on a.ord = b.userid and b.del=0 where a.del=1 or a.del=3 or(a.ord  in(select creator from hr_leave  where del=0 and status=3 and datediff(d,'"&startDate&"',indate)>=0 and datediff(d,'"&endDate&"',indate)<=0 ))) talNum) tempLeave) aaaavv PAGE_ORDER_STR) bbb  where 1=1"
...
case "离职率"
Response.write "<span class='proport' title='"&FormatNumber(trim(rsobj("离职率")),percentage,-1,0,0)&"+_0%'><span style='width:"&trim(rsobj("离职率"))&"%;' class='ratio'><span class='rationText'>"&FormatNumber(trim(rsobj("离职率")),percentage,-1,0,0)&"%</span></span></span>"
case "离职率"
case else
...
sub ShowSearchDiv()
Response.write "" & vbcrlf & "<form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""width:510px;height:370px;padding:5px;background: #fafafa;top:0px;""  closed=""true"" >" & vbcrlf & "     <div region=""center"" border=""false"" style=""padding:10px;background:#fff;border:1px solid #ccc;width:100%;height:290px"">" & vbcrlf & "          <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <style>.detailTable {table-layout:auto!important;}</style>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">完成比例：</div></td>" & vbcrlf & "          <td><select name=""hr_resume_d1"">" & vbcrlf & "              <option value=""1"">等于</option>" & vbcrlf & "              <option value=""2"">大于</option>" & vbcrlf & "              <option value=""3"">大于等于</option>" & vbcrlf & "              <option value=""4"">小于</option>" & vbcrlf & "              <option value=""5"">小于等于</option>" & vbcrlf & "              <option value=""6"">不等于</option>" & vbcrlf & "            </select>" & vbcrlf & "            <input name=""hr_resume_d2"" onKeyUp=""value=value.replace(/[^\d]/g,'')"" type=""text"" size=""15""></td>" & vbcrlf & "        </tr>" & vbcrlf & "         </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <input type=""hidden"" name=""jtdate"" value="""
'sub ShowSearchDiv()
...
Response.write ">每页显示200条</option>" & vbcrlf & "</select>" & vbcrlf & "<script language=""JavaScript"" type=""text/javascript"">" & vbcrlf & "    <!--" & vbcrlf & "    function HrformAction(val)" & vbcrlf & "      {" & vbcrlf & "       gotourl(""jtdate=""+val+""-1"");" & vbcrlf & "        }" & vbcrlf & "       -->" & vbcrlf & "</script>" & vbcrlf & "<script language=""JavaScript"" src=""js/DatePicker.js?ver="
Response.write Application("sys.info.jsver")
Response.write """ type=""text/javascript""></script>" & vbcrlf & "<input type=""button"" class=""anybutton"" value=""选择月份"" align=""absMiddle"" onClick=""setmonth(this)""   border=""0""  id=""daysOfMonth1Pos"" name=""daysOfMonth1Pos""   />" & vbcrlf & "当前:"
Response.write tdyear
Response.write "年"
Response.write tdmonth
Response.write "月份&nbsp;&nbsp;" & vbcrlf & "<input type=""hidden"" name=""px"" value="""
Response.write px
Response.write """>" & vbcrlf & "<input type=""hidden"" name=""sflg"" value=""1""/>" & vbcrlf & "<!--<input type=""submit"" value=""查看"" class=""anybutton""/>-->"
Response.write px
if open_80_10<>0 then
