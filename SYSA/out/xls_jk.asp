...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		...
		for i=1 to num_dot_xs
			...
			j=19
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select id,FName,TName,OptionID from ERP_CustomFields where TName=1001 and IsUsing=1 and CanExport=1 order by FOrder asc "
			rs1.open sql1,conn,1,1
			if rs1.eof=False Then
				do until rs1.eof
			...
			if rs1.eof=False Then
				do until rs1.eof
					xlWorksheet.Cells(1,j).Value = rs1("FName")
			...
			if open_6_10=3 then
				list1=""
			elseif open_6_10=1 then
				list1=" and a.sorce2 in ("&intro_6_10&") "
			else
				list1=" and a.sorce2=0 "
			end if
			...
			W1=tmparr(0)
			W2=tmparr(1)
			if W4<>"" and not isnull(W4) Then Str_Result=Str_Result+" and a.sorce2 in ("&W3&")  "
			W2=tmparr(1)
			if title<>"" Then Str_Result=Str_Result&" and  a.title like '%"&title&"%' "
			if intro<>"" Then Str_Result=Str_Result+"and  a.note like '%"&intro&"%' "
			if title<>"" Then Str_Result=Str_Result&" and  a.title like '%"&title&"%' "
			if money1<>"" then
				if isnumeric(money1)=false then
					...
					Str_Result=Str_Result+"and  isnull(a.allmoney,0) >="&Replace(money1,",","")&" "
					'call db_close : Response.end
				end if
			end if
			if money12<>"" then
				if isnumeric(money12)=false then
					...
					Str_Result=Str_Result+"and  isnull(a.allmoney,0) <="&Replace(money12,",","")&" "
					'call db_close : Response.end
				end if
			end if
			if money2<>"" then
			...
			if listbz<>"" Then Str_Result=Str_Result+" and a.bz="&listbz&" "
			'call db_close : Response.end
			if typ="" or isnull(typ) Then typ="0"
			if typ="3333333" or typ="5555555" then
				new_list="<span class='alt_tx'></span>"
				timelg=session("timezbintel2007")
				if typ="3333333" and timelg<>"" Then noalt1=" and a.date7>'"&timelg&"'"
				Str_Result="and (a.gate_sp="&session("personzbintel2007")&" and a.sp_id>0 and a.del=1 and a.spstate in (2,5) "&Str_Result&") "&noalt1&""
			end if
			if typ="0" then
...
'if trim(perid)<>"" then
				...
				elseif A="sqje" then
					if isnumeric(D)=false then
						Response.write("<script>alert('请输入合法金额');history.back();</script>")
						call db_close : Response.end
					else
...
'if trim(fhid)<>"" then
			...
			if m1<>"" Then Str_Result=Str_Result+" and  datejk>='"&m1&" 00:00:00' "
'if trim(fhid)<>"" then
			if m2<>"" Then Str_Result=Str_Result+" and  datejk<='"&m2&" 23:59:59' "
'if trim(fhid)<>"" then
			...
			elseif px=9 then
				pxResult=" order by a.money1 desc,a.date7 desc"
			elseif px=10 then
				pxResult=" order by a.money1 asc,a.date7 desc"
			elseif px=11 then
			...
			i=1
			dim sum_allmoney,sum_spmoney,sum_paymoney,sum_dmoney , sum_unpaymoney
			sum_allmoney=0
			sum_spmoney=0
			sum_paymoney=0
			sum_dmoney=0
			sum_unpaymoney=0
			sqls="SELECT id,title,bh,name,datejk,bzname,allmoney,spmoney, Returnmoney , dkmoney , "&_
			"  (case when isnull(dateout,'') ='' then 0 else spmoney-Returnmoney-dkmoney end ) as unpaymoney, "&_
			"note,spstatus,czStatus , czname ,czbank , dateout , addname ,date7 FROM ("&_
			"  SELECT a.*,b.name, "&_
			"  ISNULL(c.returnMoney,0) returnMoney,ISNULL(d.dkmoney,0) dkmoney,(case when isnull(k.id,0)>0 then isnull(a.spmoney,0)-ISNULL(c.returnMoney,0)-ISNULL(d.dkmoney,0) else 0 end) as NoreturnMoney,isnull(bz.sort1,'人民币') bzname , "&_
			"  s.sort1 as czbank,k.date1 as dateout,g.name as czname , "&_
			"  (case a.spstate when 2 then '审批中' when 3 then '审批未通过' when 5 then '待审批' else '审批通过' end ) as spstatus,  " &_
			"  (case isnull(k.id,0) when 0 then '未出账' else '已出账' end ) as czStatus,B1.name as addname "&_
			"  From Payjk As A  "&_
			"  Inner Join Gate As B On B.Ord = A.Sorce2 "&_
			"  inner join gate as B1 on B1.ord = a.addcate "&_
			"  left join sortbz bz on bz.id=a.bz  "&_
			"  Left Join  "&_
			"          (  "&_
			"          Select Jkid,Returnmoney  "&_
			"          From ( "&_
			"                  Select Isnull(Sum(P.Money1),0) Returnmoney,Jkid  "&_
			"                  From Pay P "&_
			"                  left Join Bank B On B.Gl = P.Ord And B.Sort = 4 And B.Del = 1  "&_
			"                  Where P.Complete in (7,8,11) And P.Del=1 Group By Jkid  "&_
			"                  ) X  "&_
			"          ) C On C.Jkid = A.Id "&_
			"  Left Join ( Select Isnull(Sum(Dkmoney),0) As Dkmoney,Jkid From Dk Where Del=1 Group By Jkid ) D On D.Jkid=A.Id  "&_
			"  left join Bank k on k.sort=50 and k.gl = a.id and k.del=1 "&_
			"  left join sortbank s on s.id = k.bank  "&_
			"  left join gate g on g.ord=k.cateid  "&_
			") A "&_
			"Where A.Id>0 "&Str_result&" "&Pxresult&" "
			set rss = server.CreateObject("adodb.recordset")
			rss.open sqls,conn,1,1
			C1=rss.recordcount
			xlApplication.Init "费用借款_"&session("name2006chen")&".xls",C1
			if rss.eof=False Then
				do until rss.eof
					title=rss("title")
					bh=rss("bh")
					...
					datejk=rss("datejk")
					bzname=rss("bzname")
					allmoney=zbcdbl(rss("allmoney"))
					if allmoney="" or isnull(allmoney) then allmoney=0
					spmoney=zbcdbl(rss("spmoney"))
					if spmoney="" or isnull(spmoney) then spmoney=0
					paymoney=zbcdbl(rss("Returnmoney"))
					if paymoney="" or isnull(paymoney) Then  paymoney=0
					dmoney  =zbcdbl( rss("dkmoney"))
					if dmoney="" or isnull(dmoney) Then  dmoney=0
					unpaymoney = rss("unpaymoney")
					if unpaymoney="" or isnull(unpaymoney) Then  unpaymoney=0
					note=rss("note")
					if note&""="" or isnull(note) Then note=""
					statxt=rss("spstatus")
					czStatus = rss("czStatus")
					czCatename = rss("czname")&""
					czbank = rss("czbank")
					dateout = rss("dateout")
					member1=rss("addname")
					date7=rss("date7")
					...
					date7=rss("date7")
					xlWorksheet.Cells(1+i,4).Value = datejk
					date7=rss("date7")
					...
					rszdy.open "select id,tname,fname from ERP_CustomFields where tname=1001 and isusing=1 and canexport=1 and del=1 order by forder asc",conn,3,1
					do while not rszdy.eof
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select a.id,a.tname,b.fvalue from ERP_CustomFields a inner join  ERP_CustomValues b on a.id=b.FieldsID inner join payjk c on b.orderid=c.id where a.tname=1001 and a.canexport=1 and a.del=1 and c.id="&rss("id")&" and a.id="&rszdy("id")&" order by a.FOrder asc "
						rs1.open sql1,conn,1,1
						if not rs1.eof then
							zdy=rs1("fvalue")
						else
						...
						rs1.close
						set rs1=nothing
						xlWorksheet.Cells(1+i,j).Value =replace(RemoveHTML(zdy),"&nbsp;"," ")
						set rs1=nothing
					...
					rszdy.close
					set rszdy=nothing
					sum_allmoney=sum_allmoney+cdbl(allmoney)
					set rszdy=nothing
					sum_spmoney=sum_spmoney+cdbl(spmoney)
					set rszdy=nothing
					sum_paymoney=sum_paymoney+cdbl(paymoney)
					set rszdy=nothing
					sum_dmoney=sum_dmoney+cdbl(dmoney)
					set rszdy=nothing
					sum_unpaymoney = sum_unpaymoney + CDbl(unpaymoney)
					set rszdy=nothing
					...
					rss.movenext
				loop
				xlWorksheet.Cells(1+i,1).Value = "合计："
'loop
				xlWorksheet.Cells(1+i,1).font.Size=10
'loop
...
'loop
				xlWorksheet.Cells(1+i,1).HorizontalAlignment=1
'loop
				xlWorksheet.Cells(1+i,6).Value = Formatnumber(sum_allmoney,num_dot_xs,-1)
'loop
				xlWorksheet.Cells(1+i,6).font.Size=10
'loop
				xlWorksheet.Cells(1+i,6).Font.Bold = True
'loop
				xlWorksheet.Cells(1+i,6).HorizontalAlignment=1
'loop
				xlWorksheet.Cells(1+i,7).Value = Formatnumber(sum_spmoney,num_dot_xs,-1)
'loop
				xlWorksheet.Cells(1+i,7).font.Size=10
'loop
				xlWorksheet.Cells(1+i,7).Font.Bold = True
'loop
				xlWorksheet.Cells(1+i,7).HorizontalAlignment=1
'loop
				xlWorksheet.Cells(1+i,8).Value = Formatnumber(sum_paymoney,num_dot_xs,-1)
'loop
				xlWorksheet.Cells(1+i,8).font.Size=10
'loop
...
'loop
				xlWorksheet.Cells(1+i,8).HorizontalAlignment=1
'loop
				xlWorksheet.Cells(1+i,9).Value = Formatnumber(sum_dmoney,num_dot_xs,-1)
'loop
				xlWorksheet.Cells(1+i,9).font.Size=10
'loop
...
'loop
				xlWorksheet.Cells(1+i,9).HorizontalAlignment=1
'loop
				xlWorksheet.Cells(1+i,10).Value = Formatnumber(sum_unpaymoney,num_dot_xs,-1)
'loop
				xlWorksheet.Cells(1+i,10).font.Size=10
'loop
...
'loop
			...
			rss.close
			set rss=nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>费用导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!费用导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rss=nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("费用借款_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的费用借款</u></strong></font></a></p>" & vbcrlf & ""
