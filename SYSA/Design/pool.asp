...
'If ubound(v)>2 Then maxvalue = v(3)
		...
		Sub CreateKhqy
			...
			Response.write "];" & vbcrlf & "   if(data.length>0) {" & vbcrlf & "             var datastr = "",,"" + data.join("","") + "",""" & vbcrlf & "             var boxs = document.getElementsByName(""A2"");" & vbcrlf & "              for (var i = 0 ; i < boxs.length ;  i ++)" & vbcrlf & "               {" & vbcrlf & "                       var box = boxs[i];" & vbcrlf & "                      if(datatr.indexOf("","" + box.value + "","")>0)" & vbcrlf & "                   {" & vbcrlf & "                               box.click();" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "</script>" & vbcrlf & "      "
			Response.write request("A2")
		end sub
		Sub showtelCls
...
'If sdk.power.existsPowerIntro(5029,14, designer) Then
	...
	Sub onReportInit(rpt, lt)
		...
		rpt.addsort "设计编号","bh"
		rpt.addsort "领用情况","designstatus"
		rpt.addsort "领用人","designer"
		rpt.addsort "领用日期","date3"
		rpt.addsort "申请人","apply"
		rpt.addsort "指派人","appoint"
		rpt.addsort "放弃人","abandon"
		rpt.addsort "放弃日期","abandontime"
		Dim designstatus : designstatus = Replace(app.glAttribute("MI_DESIGN_STATUS_2016051602")," ","")
		Dim dstr , gstr : dstr = "" : gstr = ""
		If instr(","& designstatus &",",",0,")>0 Then
			designstatus = "0"
			dstr = "未领用"&Chr(1)&"1"&Chr(2)&"申请中"&Chr(1)&"2"&Chr(2)&"已领用"&Chr(1)&"3"&Chr(2)&"待设计"&Chr(1)&"4"&Chr(2)&"设计中"&Chr(1)&"5"&Chr(2)&"设计完毕"&Chr(1)&"6"&Chr(2)&"待审批"&Chr(1)&"7"&Chr(2)&"审批通过"&Chr(1)&"8"&Chr(2)&"审批未通过"&Chr(1)&"9"
			gstr = "1=未领用;2=申请中;3=已领用;4=待设计;5=设计中;6=设计完毕;7=待审批;8=审批通过;9=审批未通过"
		else
			If instr(","& designstatus &",",",1,")>0 Then
				dstr = dstr & "未领用"&Chr(1)&"1"
				gstr = gstr & "1=未领用"
			end if
			If instr(","& designstatus &",",",2,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "申请中"&Chr(1)&"2"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "2=申请中"
			end if
			If instr(","& designstatus &",",",3,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "已领用"&Chr(1)&"3"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "3=已领用"
			end if
			If instr(","& designstatus &",",",4,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "待设计"&Chr(1)&"4"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "4=待设计"
			end if
			If instr(","& designstatus &",",",5,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "设计中"&Chr(1)&"5"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "5=设计中"
			end if
			If instr(","& designstatus &",",",6,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "设计完毕"&Chr(1)&"6"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "6=设计完毕"
			end if
			If instr(","& designstatus &",",",7,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "待审批"&Chr(1)&"7"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "7=待审批"
			end if
			If instr(","& designstatus &",",",8,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "审批通过"&Chr(1)&"8"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "8=审批通过"
			end if
			If instr(","& designstatus &",",",9,")>0 Then
				dstr = dstr & app.iif(len(dstr)>0 ,Chr(2) , "")& "审批未通过"&Chr(1)&"9"
				gstr = gstr & app.iif(len(gstr)>0 ,";" , "")& "9=审批未通过"
			end if
		end if
		rpt.AddField2 "","select","designstatus", dstr , "", "","设计状态=0"
		rpt.AddField2 "","select","B", "设计编号"&Chr(1)&"bh"&Chr(2)&"领用人"&Chr(1)&"designer"&Chr(2)&"指派人"&Chr(1)&"appoint"&Chr(2)&"申请人"&Chr(1)&"apply", "", "","设计主题=#title"
		rpt.addfield "","text","C", "", "", ""
		rpt.aSearch.addfield "设计状态","checks","dstatus","options:"& gstr
		rpt.aSearch.addfield "领用人", "gates", "designer", ""
		rpt.aSearch.addfield "领用日期","dates","date3",""
		rpt.aSearch.addfield "指派人", "gates", "appoint", ""
		rpt.aSearch.addfield "放弃人", "gates", "abandon", ""
		rpt.aSearch.addfield "放弃日期","dates","abandontime",""
		...
		app.addScriptPath app.virPath & "skin/default/js/Dn_list.js"
		rpt.pagesize = 10
		...
		If app.power.existsPower(5029,6) Then
			If Len(strHandle)>0 Then strHandle = strHandle &","
			strHandle = strHandle & "批量指派,批量收回,批量同意,批量否决"
		end if
		If Len(strHandle)>0 Then rpt.setBatHandle "ord", strHandle '"批量删除,批量指派,批量收回,批量审批"
		rpt.Settings.clear
		rpt.canExcel = app.power.existsPower(5029,10)
		rpt.canPrint = app.power.existsPower(5029,7)
		Dim sql : sql = " exec [erp_design_pool] "& Info.User &" ,'"& designstatus &"',@designstatus ,@B ,@C ,@dstatus,@designer,@date3_0 ,@date3_1,@appoint,@abandon,@abandontime_0 ,@abandontime_1,@title,@bh,@zdy5029_1,@zdy5029_2,@zdy5029_3,@zdy5029_4,@zdy5029_5,@zdy5029_6,&SearchMode ,&excelmode,&pagesize,&pageindex, &ReportSort"
		rpt.sql = sql
	end sub
...
'Case "批量删除","删除" :
		...
		Set rs =cn.execute("select id , (case when ("& Len(pwr_del) &"=0 or CHARINDEX(','+CAST(designer  as varchar(12))+',',',"& pwr_del &",')>0) then 'url' else 'none' end) as url  from design where id in ("& data &")")
'Dim pwr_del : pwr_del = app.power.getPowerIntro(5029,3)
		While rs.eof =False
			If rs("url") = "url" Then
		...
		Case "放弃" :
		Dim reson : reson = app.getText("reson")
		cn.execute("update design set designstatus=1,designer = 0,date3 = null , abandon='"& Info.user &"' ,abandontime=getdate(),abandonintro='"& Replace(reson,"'","''") & "'  where id in ("& data &")")
		Case "批量收回","收回" :
		haszp = app.power.existsPower(5029,6)
		Set rs =cn.execute("select id , designstatus,designer from design where id in ("& data &")")
		While rs.eof =False
			If haszp And rs("designstatus")>=3 And rs("designstatus")<7 And rs("designer")<>Info.User Then
				cn.execute("update design set designstatus=1,abandon=designer, designer=0,date3=null,abandontime=getdate(),abandonintro='主管收回',appoint=0, date4=null where id= "& rs("id"))
			else
				result.AddResult " 不能收回", rs("id"), "red"
			end if
			rs.movenext
		wend
		rs.close
		Case "批量同意","批量否决" :
		haszp = app.power.existsPower(5029,6)
		Set rs =cn.execute("select id , designstatus from design where id in ("& data &")")
		While rs.eof =False
			If haszp And rs("designstatus")=2 Then
				cn.execute("update design set designstatus="&app.iif(cmd="批量同意",3,1)&",designer="& app.iif(cmd="批量同意" , "apply,date3=getdate()" , "0") &", apply=0,applytime=null where id= "& rs("id"))
			else
				result.AddResult " 不能"&app.iif(cmd="批量同意","同意","否决"), rs("id"), "red"
			end if
			rs.movenext
		wend
		rs.close
	...
	Sub App_doApply
		Dim id : id = app.getInt("ord")
		cn.execute("update design set designstatus=2,apply="& Info.User &", applytime=getdate(),abandonintro='',abandontime=null,abandon=0 where id="& id )
	end sub
	Sub App_doDesignerSp
		Dim id,stype
		id = app.getInt("ord")
		stype = app.getInt("stype")
		cn.execute("update design set designstatus="& app.iif(stype=1 , 3 , 1) &",designer="& app.iif(stype=1 , "apply,date3=getdate()" , "0") &",apply=0, applytime=null where id="& id )
	end sub
	Sub app_onCreateList(lvw,rpd)
		...
		lvw.cansort=False
		lvw.checkvalue = "id"
		lvw.addJoinFields "abandonintro,abandonname,abandontime"
		...
		End With
		With lvw.headers("applyname")
		.title = "申请人"
		.width = "80"
		End With
		With lvw.headers("appointname")
		.title = "指派人"
		.width = "80"
		End With
		...
		.width = "90"
		End With
		With lvw.headers("abandonintro")
		.title = "放弃原因"
		.width = "90"
		End With
		With lvw.headers("abandonname")
		.title = "放弃人"
		.width = "90"
		End With
		With lvw.headers("abandontime")
		.title = "放弃日期"
		.width = "90"
		End With
		With lvw.headers("url")
		.width = "160"
		.formatText = "code:buttonhtml(@value,rs(""id"") ,rs(""zp"") ,rs(""del"") , rs(""designstatus"") ,rs(""haszp"") ,rs(""designer""), rs(""creator""))"
		End With
		lvw.headers("del").display="none"
		lvw.headers("zp").display="none"
		lvw.headers("haszp").display="none"
...
'If (url="url"  Or creator=Info.user) And ord>0 Then
	...
	Function buttonhtml(value,ord ,zp ,del, designstatus ,haszp ,designer ,creator)
		Dim s : s = ""
		If (value="url" Or creator=Info.user) And ord>0 Then
...
'If (value="url" Or creator=Info.user) And ord>0 Then
		...
		If designstatus=1 And app.power.existsPower(5029,17) Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesigner("& ord &")"">申请</button>&nbsp;"
		end if
		If (haszp=1 Or haszp = 3) And designstatus>=3 And designstatus<7 And designer<>Info.User Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesignBack("& ord &")"">收回</button>&nbsp;"
		ElseIf designstatus>=3 And designstatus<7 And designer =Info.User  Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setAbandon("& ord &")"">放弃</button>&nbsp;"
		ElseIf (haszp=1 Or haszp = 3) And designstatus = 2 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesignerSp("& ord &",1)"">同意</button>&nbsp;"
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesignerSp("& ord &",0)"">否决</button>&nbsp;"
		end if
		if zp="url" And designstatus=1 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesignAppoint("& ord &")"">指派</button>&nbsp;"
		end if
		if del="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:delDesign(" & ord & ",'"& app.base64.pwurl(ord) &"')"">删除</button> "
		end if
		buttonhtml = s
	end function
	...
	sub App_ListviewZdySet(lvw)
		...
		lvw.InsertZdyMap "DesignBH", "designstatus", "设计状态", 80
		lvw.InsertZdyMap "designstatus", "designerName", "设计人员", 80
		lvw.InsertZdyMap "designerName", "date3", "领用日期", 80
		lvw.InsertZdyMap "date3", "applyname", "申请人", 80
		lvw.InsertZdyMap "applyname", "appointname", "指派人", 80
		lvw.InsertZdyMap "appointname", "plandate1", "计划开始日期", 100
		lvw.InsertZdyMap "plandate1", "plandate2", "计划完成日期", 100
		lvw.InsertZdyMap "plandate2", "abandonname", "放弃人", 80
		lvw.InsertZdyMap "abandonname", "abandonintro", "放弃原因", 80
		lvw.InsertZdyMap "abandonintro", "abandontime", "放弃日期", 100
		lvw.InsertZdyMap "abandontime", "d_zdy1", "", 100
