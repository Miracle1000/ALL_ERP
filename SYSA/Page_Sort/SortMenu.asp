...
'If Not rs.Eof Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".STYLE1 {color: #909090}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF""  height=""1600"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "         <td class=""place"">"
	Response.write OrderName
	Response.write PageTitle
	Response.write "设置</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> " & vbcrlf & " <tr border=1 valign=""center"" class=""top"">" & vbcrlf & " <td>" & vbcrlf & "   <div align=""left""><a href=""javascript:void(0)"" onClick=""javascript:window.open('add_sort.asp?TableID="
	Response.write TableID
	Response.write "&TypeID="
	Response.write TypeID
	Response.write "&Action=add','newwinadd','width=' + 600 + ',height=' + 300 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
	'Response.write TypeID
	Response.write AddTitle
	Response.write PageTitle
	Response.write "</a></div></td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "<div style=""padding-left:10px;"">" & vbcrlf & ""
'Response.write PageTitle
	select case action
	case "del"
	delmenu(Ord)
	Response.redirect("SortMenu.asp?TableID="& TableID &"&TypeID="&TypeID&"")
	case else
	...
	function menu(Ord)
		...
		sql="select *,isnull((select count(*) from Page_sort where Sortid=mm.Ord and  TableID="& TableID &" and typeID="& typeID &" and del=1),0) as ChildCount from Page_sort as mm where sortid="&Ord&" and  TableID="& TableID &" and typeID="& typeID &" and del=1 order by px desc"
		RS8.open sql,conn,1,1
		If Not RS8.eof  Then
			StrReturn="<table border='0' width='100%' cellpadding='0' cellspacing='0' bgcolor='#FFFFFF' >"
			i=1
			while not RS8.eof
				ChildCount=RS8("ChildCount")
				if ChildCount=0 then
					if i=RS8.recordcount then
						menutype="file1"
					else
						menutype="file"
					end if
					...
					if i=RS8.recordcount then
						menutype="menu3"
						listtype="list1"
						onmouseup="change1('a"&RS8("Ord")&"','b"&RS8("Ord")&"');"
					else
						menutype="menu1"
						listtype="list"
						onmouseup="change2('a"&RS8("Ord")&"','b"&RS8("Ord")&"');"
					end if
					menuname=RS8("title")
				end if
				If CanChild=False Then
					StrChild=""
				else
					StrChild="<a href=""javascript:void(0)"" onClick=""javascript:window.open('add_Sort.asp?TableID="&TableID&"&TypeID="&TypeID&"&action=add&Ord="&RS8("Ord")&"','newwinadd','width=' + 600 + ',height=' + 300 + ',fullscreen =no,scrollbaRS8=1,toolbar=0,resizable=1,left=100,top=100')""><font  color=""#909090"">添加 |</font></a> "
				end if
				StrReturn=StrReturn&"<tr><td id=""b"&RS8("Ord")&""" class="""&menutype&""" onMouseUp="""&onmouseup&""">"&menuname&"&nbsp;&nbsp;&nbsp;"& StrChild &"<a href=""javascript:void(0)"" onClick=""javascript:window.open('add_sort.asp?TableID="& TableID &"&TypeID="&TypeID&"&action=edit&Ord="&RS8("Ord")&"','newwincorrect','width=' + 600 + ',height=' + 300 + ',fullscreen =no,scrollbaRS8=1,toolbar=0,resizable=1,left=100,top=100')""><font color=""#909090"">修改 |</font></a> "
				If rs8("flag") >= 1000000 Then
					StrReturn=StrReturn&"<del title='微信默认分类，不允许删除'>删除</del>"
				else
					StrReturn=StrReturn&"<a href=""SortMenu.asp?TableID="& TableID &"&TypeID="&TypeID&"&action=del&Ord="&RS8("Ord")&""" onclick=""return confirm('建议不要轻易删除设置好的参数，以免影响到其他关联的数据。确认删除?')""><font  color=""#909090"">删除</font></a>"
				end if
				StrReturn=StrReturn&"</td></tr>"
				if ChildCount>0 then
					StrReturn=StrReturn&"<tr id=""a"&RS8("Ord")&"""><td class="""&listtype&""">"&menu(RS8("Ord"))&"</td></tr>"
				end if
				RS8.movenext
				i=i+1
'RS8.movenext
	...
	function delmenu(id)
		ChildNum=conn.execute("Select count(*) from Page_sort where SortID="&id&" and del=1")(0)
		If ChildNum>0 then
			Response.write(Historyback("有子分类不允许删除！"))
			call db_close : Response.end
		else
			conn.execute("update  Page_sort set del=2 where Ord="&id)
		end if
	end function
	Response.write "" & vbcrlf & "</div>" & vbcrlf & "               </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30"" ><div align=""center"">&nbsp;</div></td>" & vbcrlf &"    </tr>" & vbcrlf & "</table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
%>
