...
if(v=="MSIE5.0"){return 5;}
	...
	{	if(navigator.userAgent.toLowerCase().match(/rv:([\d.]+)\) like gecko/)){ return 11;   }
	    return 100;
	}}
...
if(!top||isNaN(top)){
	...
	var allh = document.documentElement.offsetHeight || 1200;
	top = (parseInt((allh - height) / 2) + (tv > 0 ? tv : document.body.scrollTop));
}
...
div.innerHTML = "<div class='sys_dlg_" + id + " resetBgE3E7F0' style='position:absolute;top:10px;left:10px;height:" + (height - 20) + "px;width:" + (width - 20) + "px;'>" + (app.IeVer < 7 ? "<iframe class='sys_dlgframe'></iframe><iframe class='sys_dlgframe2'></iframe>" : "") + "<table class='sys_dtab1 resetBgE3E7F0' cellspacing=0 cellpadding=0><tr><td class='sys_dtl' onclick='alert(this.offsetWidth)'></td><td class='sys_dtc'></td><td class='sys_dtr'></td></tr></table>"
+ "<table class='sys_dtab2' cellspacing=0 cellpadding=0 bgcolor='" + bgcolor + "'><tr><td class='sys_dtooll resetBgE3E7F0'></td><td class='sys_dtoolc resetBgE3E7F0'>"
...
+"<td class='sys_dtit_bar'><img "+(style%3==0?"style='display:none'":"")+" title='关闭' onclick='app.closeWindow(\""+id+"\")' src='"+win.sysskin+"/images/dlgIco/close.png' onmouseout='app.imagechange(this,\"close2.png\",\"close.png\")' onmouseover='app.imagechange(this,\"close.png\",\"close2.png\")'></td></tr></table>"
+ "</td><td class='sys_dtoolr resetBgE3E7F0'></td></tr></table>"
+ "<table class='sys_dtab1 resetBgE3E7F0' cellspacing=0 cellpadding=0 bgcolor='" + bgcolor + "' style='height:" + (height - 8 - 29 - 10 - 20) + "px'>"
+"<tr><td class='sys_dcl'></td><td class='sys_dcc'><div id='sys_comm_dlg_body_"+id+"' style='height:"+(height-8-29-10-20-2)+"px;width:100%;padding:0 10px;overflow:auto;"+(position?"position:"+position+";":"") +"'></div></td><td class='sys_dcr'></td></tr></table>"
+ "<table class='sys_dtab3' cellspacing=0 cellpadding=0 bgcolor='" + bgcolor + "'><tr><td class='sys_dbl1 resetBgE3E7F0'></td><td class='sys_dbc1 resetBgE3E7F0'></td><td class='sys_dbr1 resetBgE3E7F0'></td></tr></table>"
...
app.playMedia=function(wUrl){if(app.IeVer<=11)
...
var div=app.createWindow("appmsg","<span style='font-size:12px;position:relative;left:-20px'>"+title+"</span>","s.gif","","",w,h,0,(disshadow==1)?false:true,"#f5f9fc");
...
if(!app.Alert){
	...
	window.alert=function(msg,body)
	{
		msg=msg+"";
...
app.beginMoveWindow=function(ev ,scobj,id)
{
	...
	if(div)
	{
		...
		{
			app.currWindow.document.captureEvents(Event.MOUSEMOVE);
			div.mv_x=div.offsetLeft;
			div.mv_y=div.offsetTop;
			div.preX=null;
			div.dtTop=null;
			var currwin = app.currWindow;
...
return data;}
base.defUrl=function(){return ("/"+window.location.pathname).replace("//","/")}
...
base.addParam=function(name,value){
	...
	base.sendText=base.sendText+"&"+escape(name)+"="+base.UrlEncode(value).replace(/\+/g,"%2B");
}
...
if(!nitem){nitem=nobj.parentNode;
root=nitem;}
...
{var v=bgElement.getAttribute("datajosn");
if(!v||v.length==0){return;}
try{var obj=eval(v);bgElement.setAttribute("datajosn","")}
catch (e)
{alert("树的子级缓存数据损坏，请确认页面加载是否完整");return;}
var html=new Array();
for(var i=0;i<obj.length;i++)
...
else{html[html.length]="<div class='tvw_n_spc' style='width:"+item.idw+"px'></div>";}}
html[html.length]="<div class='tvw_n_st' style='width:5px'>&nbsp;</div>"
...
if(item.hasnext==0)
{css=item.nodes==0?"ty_1_e2":"ty_1_e"+item.expand;}
else
{css=item.nodes==0?"ty_2_e2":"ty_2_e"+item.expand;}
if(item.firstnode==1)
{css=css+" fnode";}
...
{if(item.ico2.length==0){item.ico2=item.ico;}
...
if(item.text.lengt==0){item.text="&nbsp;";}
...
return obj;}
window.onmousedownHandle=function(){if(parent!=window)
{if($DC.all){$PDC.fireEvent("onmousedown");}
else{var evt=parent.document.createEvent("MouseEvents");
...
if ($ID("TopSearchSelectList") && $ID("TopSearchSelectList").offsetHeight) { $ID("TopSearchSelectList").style.display = 'none'; $("#topSearchForm").removeClass("searched") }
if ($ID("topShowList")) { $DC.body.removeChild($ID("topShowList")); $("#topdiv div.link-right a").removeClass("actived") }
}
...
function __stabClick(id,srcTag,index){
	...
	{ 
		if(cache!=1||!html||html.length==0){
			...
			$ap("id",id);
			$ap("index",index);
			$ap("text",srcTag.innerText||srcTag.textContent);
			$ap("key",srcTag.getAttribute("key"))
	...
	$div.empty().html(""
		...
		+"<TBODY>"
			...
			+"<TR>"
				...
				+"<TD style='border:1px solid #c0ccdd;background-color:white;padding:12px;color:#2f496e;background-color:#fff;' valign='top' id='lxls_by'>"
			...
			$(".ewebeditorImg img,.ewebeditorImg_plan img").each(function (index, element) {
				...
				if(parentsVal == null){
					...
					if(w > defWidth || h > defHeight){
						...
						$(this).click(function () {
							window.open( window.virpath  + 'inc/img.asp?url='+url)
						});
					}
				}
...
app.showHelp = function(ord) {
	var url =  window.location.href.toLowerCase().split("/sysn/")[0] + "/SYSN/help/html/help_usefunction.html?one=" + ord[0] + "&two=" + ord[1] + "&return=" + ord[2] + "&ord=" + ord[3];
...
app.drawVMLCone = function(id, data) {
	function DrawVMLConeRect(context, x1, y1, x2, y2, color){
		...
		context.beginPath();
		context.moveTo(x1+dtx,y1+20);
		context.lineTo(x2-dtx,y1+20);
		context.lineTo(x2-dtx2,y2+20);
		context.lineTo(x1+dtx2,y2+20);
		context.lineTo(x1+dtx,y1+20);
		context.closePath();
...
app.PopMessage = function(message, t){
	var win = window.top
	var iw = 300,  ih = 120;
	if(win.location.href.toLowerCase().indexOf("/home.ashx")==-1 && win.opener && win.opener.app && win.opener.app.PopMessage) { 
		win.opener.app.PopMessage(message)
		return; 
	}
	if(win!=window &&  win.app && win.app.PopMessage) { 
		win.app.PopMessage(message)
		return; 
	}
	setTimeout(function(){
		div = document.getElementById("app_sys_PopMessage");
		if(!div) {
			div = document.createElement("div");
			div.id = "app_sys_PopMessage";
			div.style.cssText =  "width:" + (iw + 10) + "px;height:" + (ih+10) + "px;z-index:100000;display:none;position:fixed;_position:absolute;right:10px;top:" + ($(win).height()-ih-25) + "px;overflow:hidden";
			div.innerHTML = "<div id='app_sys_PopMessage_body' style='background-color:white;box-shadow: 0px 0px 10px #333;position:absolute;left:5px;width:" + iw + "px;height:" + ih + "px;top:" + (ih+10) + "px;border:1px solid #fff'>"
								+ "<div style='background-color:#eeeef9;width:100%;border-bottom:1px solid #ccccd9;display:block'><div style=';font-size:13px;color:#000;padding:5px'>消息提示</div></div>"
								+"<div id='app_sys_PopMessage_content' style='padding:10px;font-size:13px;color:#000'><div></div>"
			document.body.appendChild(div);
		}
		div.style.top = ($(win).height()-ih-15) + "px";
		div.style.display = "block";
		var dbody = div.children[0];
		document.getElementById("app_sys_PopMessage_content").innerHTML =  message;
		$(dbody).css({ opacity: 0});
		$(dbody).stop().animate({top: "5px", opacity: 1}, 800);
		if(app.PopMessageTimerH2 && app.PopMessageTimerH2>0) { window.clearTimeout(app.PopMessageTimerH2); }
		app.PopMessageTimerH2 =  window.setTimeout(function() { 
			$(dbody).stop().animate({top: "" + (ih+10) + "px",  opacity: 0}, 1000, function(){
				div.style.display = "none";
			});	
			app.PopMessageTimerH2 = 0;
		}, 4000);
	},200);
}

app.ApplyUserZoom = function () {}
String.prototype.Right = function (charcount) {
    return this.substr(this.length - charcount, charcount);
};
app.SerializeArraySign = Array.toString();
app.SerializeDateSign = Date.toString();
/**
 * @description 对象序列化
 * @method app.GetJSON
 * @constructor 
 ...
 */
app.Serialize = function (obj, deepn, deepi) {
    ...
    switch (typeof (obj)) {  //typeof速度快
        ...
        case "object":
            ...
            if (oc == app.SerializeArraySign) {
                var str = [];
                for (var i = 0; i < obj.length; i++) {
                    str[i] = app.Serialize(obj[i], (deepn + ",[]" + i), deepi + 1);
                }
                return "[" + str.join(",") + "]";
            }
            if (oc == app.SerializeDateSign) {
                return "\"" + obj.toString() + "\"";
            }
            //object
            var str = [];
            for (var o in obj) {
                ...
                var v = app.Serialize(obj[o], (deepn + "," + o), deepi + 1);
                str.push("\"" + o + "\":" + v);
            }
            return "{" + str.join(",") + "}";
    }
...
app.OpenUrl = function (url, name, attrs, longattrs) {
    ...
    if (url && url.toLowerCase().indexOf("/sysn/") == 0) {
        url = (window.SysConfig.VirPath + url).replace("//", "/"); //防止绝对路径问题
    }
    if (url && url.indexOf("/SYSN/") > 0) {
        url = (window.SysConfig.VirPath + "SYSN/" + url.split("/SYSN/")[1]).replace("//", "/"); //防止绝对路径问题
    }
    ...
    if (url && url.indexOf("/SYSA/") > 0) {
        url = (window.SysConfig.VirPath + "SYSA/" + url.split("/SYSA/")[1]).replace("//", "/"); //防止绝对路径问题
    }
    ...
    if (app.isObject(attrs)) {
        ...
        if (attrs) {
            ...
            var w = "1200", h = "700";
            if (url.indexOf("winwidth=") > 0) { w = url.split("winwidth=")[1].split("&")[0]; }
            if (url.indexOf("winheight=") > 0) { h = url.split("winheight=")[1].split("&")[0]; }
            attrs = "width=" + w + "px,height=" + h + "px,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=" + (90 + Math.random() * 20) + "px,top=" + (60 + Math.random() * 20) + "px";
        }
    }
    if (window.SysConfig.IsWorkSiteModel == true) {
        if (url.indexOf("/SYSN") >= 0) {
            url = "/SYSN" + url.split("/SYSN")[1];
        }
    }
    var win = window.open(url, name, attrs);
    setTimeout(function () { if (win) win.focus() }, 150);
    if (window.event) { app.stopDomEvent(window.event); }
    return win;
}
//ajax请求
...
app.isnull = function (o, defv) { return (o == null || o == undefined) ? defv : o; }
app.ajax = new Object();
app.ajax.eventmodulesObject = { msgid: "", url: "", params: [], sysmsgid: "" };
app.ajax.regEvent = function (msgid) {
    app.ajax.eventmodulesObject.msgid = msgid;
    app.ajax.eventmodulesObject.sysmsgid = "";
};
app.ajax.regSysEvent = function (msgid) {
    app.ajax.eventmodulesObject.msgid = "";
    app.ajax.eventmodulesObject.sysmsgid = msgid;
};
app.ajax.setUrl = function (url) {
    app.ajax.eventmodulesObject.url = url;
}
app.ajax.deleteSysMsgKey = function (url) {
    return app.ReplaceUrl(url, "__sys_msgid");
}
/**
 * @description 用于提交数据的键值对
 * @method app.ajax.addParam
 * @constructor
 * @param param {String} key值
 * @param value {String} value值
 * @return null
 */
app.ajax.addParam = function (param, value) {
    app.ajax.eventmodulesObject.params.push([param, value]);
};
/**
 * @description 服务器上传数据
 * @method app.ajax.send
 * @constructor
 * @param callback {Function} 回调函数
 * @param proccallback {Function} 回调函数
 * @param failcallback {Function} 回调函数
 * @example app.ajax.send()
 * @return null
 */
app.ajax.send = function (callback, proccallback, failcallback, shadobj) {
    if (callback && callback.tagName && callback.getAttribute) {
        var pcall = app.ajax.sendforlink(callback);
        if (pcall.stoped) { return; }
        callback = pcall.callbackfun;
    }
    app.ajax.lastsendProcTmp = function (proxysenddata) {
        if (proxysenddata) {
            app.ajax.eventmodulesObject = proxysenddata;
        }
        app.ajax.lastsendpostdatas = {};
        $.extend(true, app.ajax.lastsendpostdatas, app.ajax.eventmodulesObject);
        if (proccallback && callback) { //创建带进度反馈的ajax提交，模拟的
            app.ajax.sendAsProcForm(callback, proccallback, failcallback);
            app.ajax.eventmodulesObject = { sysmsgid: "", msgid: "", url: "", params: [] };
            return;
        }
        var mobj = app.ajax.eventmodulesObject;
        var xhttp = (window.XMLHttpRequest ? new window.XMLHttpRequest() : new window.ActiveXObject("msxml2.xmlhttp"));
        mobj.sysmsgid = mobj.sysmsgid || (window.RuntimeInfo || {}).SystemMessageKey || "";
        var url = mobj.url || window.location.href;
        if (mobj.sysmsgid) {
            url = app.ajax.deleteSysMsgKey(url);
        }
        if (mobj.msgid != "") {
            app.ajax.addParam("__msgid", mobj.msgid);
            try {
                var btn = window.event.srcElement;
                if (btn.tagName == "BUTTON" || (btn.tagName == "INPUT" && btn.type == "button")) {
                    var cmdbuttontxt = btn.value || btn.innerHTML || "";
                    if (cmdbuttontxt != "") {
                        app.ajax.addParam("__syscurr_cmd_text", cmdbuttontxt);
                    }
                    if ($ID("comm_itembarText")) {
                        app.ajax.addParam("__syscurr_page_title", $ID("comm_itembarText").innerText);
                    }
                }
            } catch (bex) { }
        }
        var asyn = (callback ? true : false);
        var domobj = (window.event && window.event.srcElement && window.event.srcElement.getAttribute) ? window.event.srcElement : null;
        var asynsign = (domobj && domobj.getAttribute("disajaxasyn") == "1") ? false : true;
        xhttp.open("POST", url, asyn && asynsign);
        app.ajax.addParam("__ajaxsign", "1");
        xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        if (asyn == true) {
            xhttp.onreadystatechange = app.ajax.statuschangeEvent(callback, xhttp);
        };
        var hssysmsgkey = false;
        for (var i = 0; i < mobj.params.length; i++) {
            if (mobj.params[i][0] == "__sys_msgid") { hssysmsgkey = true; break; }
        }
        if (mobj.sysmsgid != "" && hssysmsgkey == false) {
            app.ajax.addParam("__sys_msgid", mobj.sysmsgid);
        };
        if (app.ajax.encodeSign == 1) {
            app.ajax.encodeSign = 0;
            app.ajax.encodeProc();
        }
        for (var i = 0; i < mobj.params.length; i++) {
            var values = mobj.params[i];
            mobj.params[i] = (values[0] + "=" + encodeURIComponent(values[1]).replace(/\+/g, "%2b"));
        };
        if (asyn == true) {
            var waitico = app.CloneObject(app.ajax.showWaitIco || {});
            app.ajax.createDefAjaxProcDiv(waitico);
            app.ajax.procTimeouthwnd = setTimeout(app.ajax.closeDefAjaxProcDiv, 120000);
        };
        app.ajax.showWaitIco = null;
        xhttp.send(mobj.params.join("&"));
        app.ajax.eventmodulesObject = { msgid: "", url: "", params: [] };
        if (asyn == false) {
            if (xhttp.responseText.indexOf("系统错误") > -1) {
                document.write(xhttp.responseText);
                return "";
            }
            return app.ajax.PreScript(xhttp.responseText);
        }
    }
    return app.ajax.lastsendProcTmp(null);
};

app.ajax.statuschangeEvent = function (callback, http) {
    return function () {
        if (http.readyState == 4) {
            app.ajax.closeDefAjaxProcDiv();  //PS:目前为遮盖层,后期待优化样式
            callback(app.ajax.PreScript(http.responseText));
        }
    }
};

app.ajax.PreScript = function (txt) {
    var a = txt.replace(/\<\/ajaxscript\>/g, "<ajaxscript>").split("<ajaxscript>");
    for (var i = 1; i < a.length; i = i + 2) {
        var v = eval(a[i]);
        a[i] = a[i].indexOf("breakcallbacksign='") > 0 ? v : "";  //breakcallbacksign是系统中断标识, 如果存在此信息, eval过程中会重新请求,并返回新的执行结果
    }
    var result = a.join("");
    if (result.indexOf("\"SystemErrorSign\":true") > 0) {
        try {
            var obj = eval("(" + result + ")");
            if (obj.SystemErrorSign == true) {
                setTimeout(function () {
                    try { document.body.innerHTML = ""; } catch (ex) { }
                    window.PageInitParams[0] = obj;
                    window.createPage();
                }, 500);
                return "";
            }
        } catch (ex) {
            if (txt.indexOf("<!Doctype html>") >= 0) {
                var div = app.createWindow("a", "信息提示", { width: 860, height: 500, closeButton: 1, maxButton: 1 });
                div.innerHTML = "<iframe id='errorbox' style='width:100%;height:99%' scrolling=0 frameborder=0></iframe>";
                $ID("errorbox").contentWindow.document.write(txt);
            } else {
                alert(ex.message)
            }
        }
    }
    if (app.ajax.onResultHandled) { app.ajax.onResultHandled(result); }
    return result;
};
/**
 * @description 克隆对象
 * @method app.CloneObject
 * @constructor 
 * @param obj {Object} 需要复制的信息集合
 * @param deep {Number|String} 是否深度拷贝:2=深度拷贝  1=浅拷贝,对象引用  deep=0浅拷贝
 * @param filter {String} 复制时需要过滤的对象名称,过滤的属性,多个之间用逗号隔开["pararentNode,node"]
 * @return {Object} 返回复制的信息集合
 */
app.CloneObject = function (obj, deep, filter) {
    deep = deep == undefined ? 2 : deep;  //是否深度拷贝： deep=2 深度拷贝  deep=1 浅拷贝, 对象引用，  deep=0 浅拷贝，对象为null 不引用， 
    if (obj == undefined) { return undefined; }
    if (obj == null) { return null; }
    var onlyml = (filter || "").indexOf("only:") == 0;
    var filterlist = (filter || "").replace("only:", "").split(",");  //要过滤的属性， 多个之间用逗号隔开
    var newobj = null;
    var CloneItem = function (nobj) {
        var iobj = null;
        if (nobj == undefined) { return undefined; }
        if (nobj == null) { return null; }
        var objstr = nobj.constructor.toString();
        if (objstr == Array.toString()) {
            switch (deep) {
                case 2: iobj = []; for (var i = 0; i < nobj.length; i++) { iobj[i] = app.CloneObject(nobj[i], deep, filter); } break;
                case 1: iobj = nobj; break;
                case 0: iobj = null; break;
            };
            return iobj
        }
        if (objstr == Object.toString() || objstr.toLowerCase().indexOf("object") >= 0) {
            switch (deep) {
                case 2: iobj = {}; for (var n in nobj) { if ((filterlist.indexOf(n) >= 0) == onlyml) { iobj[n] = app.CloneObject(nobj[n], deep, filter); } }; break;
                case 1: iobj = nobj; break;
                case 0: iobj = null; break;
            };
            return iobj;
    ...
    var rootobjstr = obj.constructor.toString();
    if (rootobjstr == Array.toString()) {
        newobj = [];
        for (var i = 0; i < obj.length; i++)
        { newobj[i] = CloneItem(obj[i]); }
        return newobj;
    }

    if (rootobjstr == Object.toString() || rootobjstr.toLowerCase().indexOf("object") >= 0) {
        newobj = {};
        for (var n in obj) {
            if ((filterlist.indexOf(n) >= 0) == onlyml) {
                newobj[n] = CloneItem(obj[n]);
            }
        }
        return newobj;
    }
...
app.ajax.createDefAjaxProcDiv = function (waitico) {
    var div = $ID("DefAjaxProcDiv_div");
    if (!div) {
        div = document.createElement("div");
        div.id = "DefAjaxProcDiv_div";
        var height = document.documentElement.clientHeight || document.body.clientHeight;
        div.style.cssText = "width:100%;height:" + height + "px;position:fixed;top:0;left:0;z-index:100000;";
        document.body.appendChild(div);
    }
    if (waitico && waitico.delay) {
        setTimeout(function () {
            var ico = (window.SysConfig.VirPath + "sysn/skin/default/img/bigloading.gif");
            var div = $ID("DefAjaxProcDiv_div");
            if (!div) { return; }
            div.style.backgroundColor = "rgba(30,30,30,0.2)";
            div.innerHTML = "<table style='width:260px;position:absolute;top:45%;left:50%;margin-left:-130px;background-color:white;border:6px solid #fff'><tr><td width='40px'>&nbsp;<img src='" + ico + "' style='height:16px'></td><td>正在加载数据，请稍后...</td></tr></table>"
        }, waitico.delay);
    }
}

app.ajax.closeDefAjaxProcDiv = function () {
    var div = $ID("DefAjaxProcDiv_div");
    if (div) { $(div).remove(); }
    if (app.ajax.procTimeouthwnd > 0) {
        clearTimeout(app.ajax.procTimeouthwnd);
        app.ajax.procTimeouthwnd = 0;
    }
}
app.GetLongAttrUrl = function (url, longattrs) {
    ...
    if (pobj.length > 0) {
        app.ajax.regSysEvent("sdk_sys_RegLongUrlParams");
        app.ajax.addParam("Sign", (window.location.href.split("?")[0] + name).Right(50));
        app.ajax.addParam("Data", app.GetJSON(pobj));
        var r = app.ajax.send();
