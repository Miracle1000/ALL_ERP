	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				kharea=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=7")(0)
				khfenl=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=4")(0)
				khgenj=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=5")(0)
				khhangy=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=8")(0)
				khlaiy=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=6")(0)
...
'elseif open_6_11=1 then
				...
				Dim formType : formType = request("formType")
				if m1="" And Len(formType&"")=0 then
					m1=cdate(year(date)&"-"&month(date)&"-1")
'if m1="" And Len(formType&"")=0 then
				end if
				if m2="" And Len(formType&"")=0 then
					m2=dateadd("m",1,m1)-1
'if m2="" And Len(formType&"")=0 then
				end if
				if m1<>"" then
...
'if B="khmc" then
		elseif B="khid" then
			str_Result=str_Result+" and khid like '%"& C &"%' "
'elseif B="khid" then
		elseif B="xsry" then
...
'elseif B="xsry" then
	...
	if E<>"" then Str_Result=Str_Result+" and  telsort in  ("&E&") "
'elseif B="xsry" then
	if F<>"" then Str_Result=Str_Result+" and  telsort1 in  ("&F&") "
'elseif B="xsry" then
...
'if bz<>"0" and bz<>"" then
	...
	if khmc<>"" then
		str_Result=str_Result+" and name like '%"&khmc&"%' "
'if khmc<>"" then
	end if
	if khid<>"" then
		str_Result=str_Result+" and khid like '%"&khid&"%' "
'if khid<>"" then
	end if
	if khly<>"" then
		str_Result=str_Result+" and ly="&khly&" "
'if khly<>"" then
	end if
	...
	money3_all=0
	n=0
	sql="select * from (select tel.ord,tel.sort as telsort,tel.sort1 as telsort1,tel.area,tel.trade,tel.ly,tel.cateid,tel.name,tel.khid,paytype.id,paytype.sort1,paytype.sort2,sortbz.id as sortbzid,sortbz.sort1 as bzname,sortbz.intro,"&_
	"(select sort1 from sortonehy where ord=paytype.sort2) as sortname,"&_
	"(select isnull(sum(money1),0) from paysqlist where del=1 and company=tel.ord and sort1=paytype.id and fid in (select id from paysq where isnull(bz,14)=sortbz.id "&str_Result11&")) as money1,"&_
	"(select isnull(sum(money1),0) from pay where del=1 and company=tel.ord and sort=paytype.id and fid in (select id from f_pay where isnull(bz,14)=sortbz.id "&str_Result22&")) as money2,"&_
	"(select isnull(sum(money1),0) from paybxlist where del=1 and payid in (select ord from pay where del=1 "&Str_Result44&" and company=tel.ord and sort=paytype.id) and bxid in (select id from paybx where isnull(bz,14)=sortbz.id "&str_Result33&")) as money3"&_
	"   from tel,paytype,sortbz where tel.del=1 and tel.sort3=1 and (tel.ord in (select company from paysqlist where del=1) or tel.ord in (select company from pay where del=1)) and (sortbz.id in (select bz from paysq where del=1) or sortbz.id in (select bz from f_pay where del=1) or sortbz.id in (select bz from paybx where del=1)) ) as fy4 "&str_Result&" and (money1>0 or money2>0 or money3>0) "&px_Result&""
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			if rs("telsort")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1 from sort4 where ord="&rs("telsort")&""
				rs7.open sql7,conn,1,1
				dim sort67
				if rs7.eof then
					sort67=""
			...
			if rs("ly")<>0 then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1 from sortonehy where ord="&rs("ly")&""
				rs7.open sql7,conn,1,1
				dim ly
				if rs7.eof then
					ly=""
			...
			ly=""
			xlWorksheet.Cells(1+i,4).Value = area
			ly=""
			...
			ly=""
			xlWorksheet.Cells(1+i,7).Value = ly
			ly=""
			xlWorksheet.Cells(1+i,7).font.Size=10
			ly=""
			xlWorksheet.Cells(1+i,8).Value = rs("bzname")
			ly=""
			xlWorksheet.Cells(1+i,8).font.Size=10
			ly=""
			xlWorksheet.Cells(1+i,9).Value = Formatnumber(money1,num_dot_xs,-1)
			ly=""
			xlWorksheet.Cells(1+i,9).font.Size=10
			ly=""
			xlWorksheet.Cells(1+i,10).Value = Formatnumber(money2,num_dot_xs,-1)
			ly=""
			xlWorksheet.Cells(1+i,10).font.Size=10
			ly=""
			xlWorksheet.Cells(1+i,11).Value = Formatnumber(money3,num_dot_xs,-1)
			ly=""
...
'loop
		xlWorksheet.Range((xlWorksheet.Cells(1+i,1)),(xlWorksheet.Cells(1+i,8))).Merge
'loop
		xlWorksheet.Cells(1+i,9).Value = Formatnumber(summoney1,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,9).font.Size=10
'loop
		xlWorksheet.Cells(1+i,10).Value = Formatnumber(summoney2,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,10).font.Size=10
'loop
		xlWorksheet.Cells(1+i,11).Value = Formatnumber(summoney3,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,11).font.Size=10
'loop
	end if
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("客户费用明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的客户费用明细表</u></strong></font></a></p>"
