...
'function CurrCostDate(cn)
	...
	if ord="" or isnull(ord) then
		...
		no_del = request("no_del")
	end if
	if ord="" or isnull(ord) then
		Response.write "<script>alert('请选择要删除的数据');history.back();</script>"
		call db_close : Response.end()
	end if
	if r=1 then
		zdy3=2
	...
	u_ord = Split(ord,",")
	canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & ord &"',105, 14, '' ").eof
	if canDelete = false then
		Response.write "<script>alert('您选择的数据存在对应的凭证,无法删除');history.back();</script>"
		call db_close : Response.end()
	end if
	if IsHasNotionalPooling(conn, 49002 , ord) then
		Response.write "<script>alert('您选择了已归集的计提折旧，无法删除!');history.back();</script>"
		call db_close : Response.end()
	end if
	If Len(no_del&"") > 0 Then
		set rs7=server.CreateObject("adodb.recordset")
		sql="select id,D_del from  O_assDeprect where D_del<>3 and id in("&ord&")"
		rs7.open sql,conn,1,1
		if rs7.RecordCount>0 then
			do while not rs7.eof
				if rs7("D_del")=1 then
					sqltext="update O_assDeprect set D_del=3,D_zdy3=D_del,D_delcateid="&session("personzbintel2007")&",D_deltime='"&now&"' where id ="&rs7("id")&" and id not in ("&no_del&")"
					conn.execute(sqltext)
					conn.execute("update a set a.ass_jttime = isnull(c.ass_jttime,a.ass_jttime_bak) from O_asset a inner join O_assDeprect b on b.D_assID = a.ass_id and a.ass_del = 1 and b.id ="&rs7("id")&" left join (select max(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del<>3 and id<>"&rs7(id)&" group by d_assid) c on c.D_assID = a.ass_id where b.id not in ("&no_del&")")
				else
					sqltext="update O_assDeprect set D_del=3,D_zdy3=D_del,D_delcateid="&session("personzbintel2007")&",D_deltime='"&now&"' where id ="&rs7("id")&" and id not in ("&no_del&")"
					conn.execute(sqltext)
					conn.execute("update a set a.ass_ycycle = (case when a.ass_ycycle - 1 < 0 then 0 else a.ass_ycycle - 1 end),a.ass_money2 = (case when a.ass_money2 - b.d_money < 0 then 0 else a.ass_money2 - b.d_money end),a.ass_jttime = isnull(c.ass_jttime,a.ass_jttime_bak) from O_asset a inner join O_assDeprect b on b.D_assID = a.ass_id and a.ass_del = 1 and b.id ="&rs7("id")&" left join (select max(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del<>3 and id<>"&rs7("id")&" group by d_assid) c on c.D_assID = a.ass_id where b.id not in ("&no_del&")")
				end if
				rs7.movenext
			loop
		end if
		...
		if rs7.RecordCount>0 then
			do while not rs7.eof
				if(rs7("D_del")=1)then
				sqltext="update O_assDeprect set D_del=3,D_zdy3=D_del,D_delcateid="&session("personzbintel2007")&",D_deltime='"&now&"' where id ="&rs7("id")
				conn.execute(sqltext)
				conn.execute("update a set a.ass_jttime = isnull(c.ass_jttime,a.ass_jttime_bak) from O_asset a inner join O_assDeprect b on b.D_assID = a.ass_id and a.ass_del = 1 and b.id ="&rs7("id")&" left join (select max(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del<>3 group by d_assid) c on c.D_assID = a.ass_id")
			else
				sqltext="update O_assDeprect set D_del=3,D_zdy3=D_del,D_delcateid="&session("personzbintel2007")&",D_deltime='"&now&"' where id ="&rs7("id")
				conn.execute(sqltext)
				conn.execute("update a set a.ass_ycycle = (case when a.ass_ycycle - 1 < 0 then 0 else a.ass_ycycle - 1 end),a.ass_money2 = (case when a.ass_money2 - b.d_money < 0 then 0 else a.ass_money2 - b.d_money end),a.ass_jttime = isnull(c.ass_jttime,a.ass_jttime_bak) from O_asset a inner join O_assDeprect b on b.D_assID = a.ass_id and a.ass_del = 1 and b.id ="&rs7("id")&" left join (select max(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del<>3 group by d_assid) c on c.D_assID = a.ass_id")
			end if
			rs7.movenext
		loop
	end if
