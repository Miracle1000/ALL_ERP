...
'Private Sub Class_Initialize
		...
		Sub page_load
			...
			Dim m3 : m3=request.querystring("indate_ret1")
			Dim m4 : m4=request.querystring("indate_ret2")
			Dim B : B=request.querystring("B")
			Dim C : C=request.querystring("C")
			Dim D : D=request.querystring("D")
			Dim d1 : d1=request.querystring("d1")
			Dim px : px=request.querystring("px")
			Dim typ : typ=request.querystring("typ")
			Dim title : title=request.querystring("title")
			Dim intro : intro=request.querystring("intro")
			Dim money3 : money3=request.querystring("money1")
			Dim money12 : money12=request.querystring("money12")
			Dim czzt : czzt=request.querystring("czzt")
			Dim e : e=request("e")
			Dim e1 : e1=request("e1")
			Dim tjr : tjr=request("tjr")
			Dim jkr : jkr=request("jkr")
			Dim listbz : listbz=request("listbz")
...
'if intro<>"" then
			...
			Dim Str : Str = "having 1=1 "
			if money3<>"" then
				if isnumeric(money3)=false then
					app.alert("请输入合法金额")
					Response.end
				else
					str=Str & " and sum(c.money1)>="&money3&" "
				end if
			end if
			if money12<>"" then
				if isnumeric(money12)=false then
					...
					str=Str & " and sum(c.money1)<="&money12&" "
				end if
			end if
			if czzt&""<>"" then
...
'if m4<>"" then
			...
			Dim page_count,currpage,pxResult
			...
			elseif px=5 then
				pxResult=" order by a.bxdate desc"
			elseif px=6 then
				pxResult=" order by a.bxdate asc"
			elseif px=3 then
			...
			Dim sql
			sql="exec erp_xls_bx '"& Replace(Str_Result&"","'","''") &"','"& Replace(pxResult&"","'","''") &"','"& Replace(Str&"","'","''") &"'"
			Dim Fields
			Fields = "报销编号,205;报销标题,205;审批状态,87;出账状态,87;出账账户,87;出账日期,87;出账人员,80;报销概要,245;币种,101;报销总额,165;审批金额,165;添加日期,165;报销日期,165;报销人员,80;添加人员,80;费用分类,87;报销分类,165;报销金额,165;使用日期,165;报销说明,245;明细添加人员,90;idx;id;px"
			Dim adapter
			Dim sqls,sheetSetting,i,item
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "费用报销"
			adapter.canSplitFormula_And = "canSplit(rs)"
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = SQL
			sheetSetting.title = "费用报销"
