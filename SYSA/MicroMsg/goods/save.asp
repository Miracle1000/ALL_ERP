...
'If Not rs.Eof Then
	...
	Function UnIncFields()
		UnIncFields = " '0' "
		If ZBruntime.MC(1002) = False Then
			UnIncFields = UnIncFields & " , '主供应商' "
		end if
		If ZBruntime.MC(17000) = False Then
			UnIncFields = UnIncFields & " , '库存上限' , '库存下限' , '提前失效期' , '预计销售周期' "
		end if
		If ZBruntime.MC(17009) = False Then
			UnIncFields = UnIncFields & " , '养护周期' "
		end if
		If ZBruntime.MC(18000) = False Then
			UnIncFields = UnIncFields & " , '生产周期' "
		end if
	end function
	Function UnIncType()
		UnIncType = " '0' "
		If ZBruntime.MC(13000) = False Then
			UnIncType = UnIncType & " , '产品自定义' "
		end if
		If ZBruntime.MC(39004) = False Then
			UnIncType = UnIncType & " , '员工档案自定义' "
		end if
		If ZBruntime.MC(76010) = False Then
			UnIncType = UnIncType & " , '商品自定义' "
		end if
		If ZBruntime.MC(17002) = False Then
			UnIncType = UnIncType & " , '入库明细自定义' "
		end if
	end function
...
'Dim rs,sql,temp,attrNum
	...
	goodsBH = Request("goodsBH")
	goodsAD = Request("goodsAD")
	goodsSort = Request("goodsSort")
	...
	goodsParameter = Request("goodsParameter")
	goodsBZSH = Request("goodsBZSH")
	attrID = Request("attrID")
	primaryImg = Request("primaryImg")
	onSale = Request("onSale")
	putTime = Request("putTime")
	fileID = Request("fileID")
	autoCode2 = Request("autoCode2")
	If onSale = 2 Then
		If isdate(putTime) = False Then
			Response.write "<script>alert('请选择正确的定时上架时间！');window.history.back();</script>"
			conn.close
			Response.end
		end if
		If DateDiff("s",putTime,Now()) > 0  Then
			Response.write "<script>alert('定时上架时间不能小于当前时间！');window.history.back();</script>"
			conn.close
			Response.end
		end if
	end if
	Call strCheckBH("bh","Shop_Goods",bh,"商品")
	Call ValidField(goodsName,"商品名称")
	attrID = Replace(attrID,", ",",")
	fileID = Replace(fileID,", ",",")
	conn.CursorLocation = 3
	conn.begintrans
	Dim ntime : ntime = now()
	Set rs = server.CreateObject("adodb.recordset")
	sql = "SELECT * FROM Shop_Goods WHERE id = "& goodsID &" "
	...
	rs("name") = goodsName
	rs("adWord") = goodsAD
	rs("sortonehy") = goodsCategory
	rs("sort") = goodsSort
	rs("unit") = goodsUnit
	rs("price1") = goodsPrice
	rs("intro3") = goodsDescription
	rs("intro2") = goodsParameter
	rs("intro1") = goodsBZSH
	rs("onSale") = onSale
	If putTime <> "" Then
		rs("onSaleAfter") = putTime
	end if
	rs("del") = 1
	rs.update
	rs.close
	set rs = nothing
	If CLng(goodsID) = -111 Then
		rs("del") = 1
		goodsID = conn.execute("select  id from Shop_Goods where creator=" & curUser & " and createtime='" & ntime & "'")(0).value
	end if
	If goodsNum <> "" And IsNumeric(goodsNum) Then
		conn.Execute("INSERT INTO Shop_StorageAppendLog (goodsId,num1,creator,createtime) VALUES ("& goodsID &","& goodsNum &","& curUser &",'"& ntime &"')")
	end if
	attrID = Split(attrID,",")
	For i = 0 To Ubound(attrID)
		degreeID = Request("degreeID_"& attrID(i) &"")
		degreeVal = Request("degree_"& attrID(i) &"")
		If degreeID <> "" And IsNumeric(degreeID) And degreeID > 0 Then
			If conn.Execute("SELECT 1 FROM Shop_GoodsAttrValue WHERE attrID = "& attrID(i) &" AND goodsID = "& goodsID &" AND proID = "& proID &" ").Eof = True Then
				conn.Execute("INSERT INTO Shop_GoodsAttrValue (attrID,degreeID,attrVal,goodsID,proID) VALUES ("& attrID(i) &","& degreeID &",'"& degreeVal &"',"& goodsID &","& proID &")")
			else
				conn.Execute("UPDATE Shop_GoodsAttrValue SET degreeID = "& degreeID &", attrVal = '"& degreeVal &"' WHERE attrID = "& attrID(i) &" AND goodsID = "& goodsID &" AND proID = "& proID &" ")
			end if
		end if
	next
	fileID = Split(fileID,",")
	For i = 0 To Ubound(fileID)
		conn.Execute("UPDATE sys_upload_res SET ID3 = "& i + 1 &" WHERE ID = "& fileID(i) &"")
'For i = 0 To Ubound(fileID)
	next
	If primaryImg <> "" And IsNumeric(primaryImg) Then
		conn.Execute("UPDATE sys_upload_res SET ID2 = 0 WHERE [source] = 'goodsPic' AND ID1 = "& goodsID &" ")
		conn.Execute("UPDATE sys_upload_res SET ID2 = 1 WHERE ID = "& primaryImg &" ")
	end if
	If autoCode2 = 1 And ZBruntime.MC(63000) Then
		conn.Execute "EXEC C2_AddAuto '"& goodsID &"','商品自定义',"& session("personzbintel2007") &",'" & Replace(UnIncFields(),"'","") & "'"
	end if
	If Err.number <> 0 Then
		conn.rollbacktrans
	...
	If Referrer = "List" Then
		Response.write("<script>parent.opener.lvw_refresh('mlistvw');parent.close();</script>")
	ElseIf Referrer = "content" Then
		Response.write("<script>parent.opener.location.reload();parent.close();</script>")
	ElseIf pageType = "保存" Then
		Response.write "<script>window.parent.location.href='list.asp';</script>"
	ElseIf pageType = "增加" Then
		Response.write "<script>window.location.href='add.asp';$('body').scrollTop(0);</script>"
	end if
	Call db_close : Response.end
