...
'Private Sub Class_Initialize
		...
		Dim gateord22,Servicfl_list,Str_Result
		Dim area_list,area_list2,area_list3
		Sub page_load
			Set conn = cn
			Dim lay
			lay=trim(request("lay"))
			Dim open_42_10,intro_42_10,open_1_1,intro_1_1, open_42_1, intro_42_1,open_26_1,intro_26_1
			Call fillPowerInfo(42,10,open_42_10,intro_42_10)
			Call fillPowerInfo(1,1,open_1_1,intro_1_1)
			Call fillPowerInfo(26,1,open_26_1,intro_26_1)
			Call fillPowerInfo(42,1,open_42_1,intro_42_1)
			Dim list,list1,Str_Result2,sql,rs,aryReturn2
			if open_42_10=3 then
				list=""
				list1=""
			elseif open_42_10=1 then
				list=" and ( " & vbcrlf &_
				"a.addcate in ("&intro_42_10&") " & vbcrlf &_
				"or (a.catelead in ("&intro_42_10&") and a.catelead>0) " & vbcrlf &_
				"or a.cate1 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate2 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate3 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate4 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate5 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate6 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate7 in ("&intro_42_10&") " & vbcrlf &_
				"or a.cate8 in ("&intro_42_10&") " & vbcrlf &_
				"or a.member1 in ("&intro_42_10&") " & vbcrlf &_
				"or CHARINDEX(','+CAST("&info.user&" AS VARCHAR)+',' , ','+Replace(a.share, ' ', '')+',') > 0 " & vbcrlf &_
				"or a.member1 in ("&intro_42_10&") " & vbcrlf &_
				"or dbo.erp_HasIntersection(REPLACE(a.share,' ',''),replace('"&intro_42_10&"',' ',''))>0 " & vbcrlf &_
				") "
				list1=" and a.addcate in ("&intro_42_10&") "
			else
				list=" and 1=2 "
				list1=" and 1=2 "
			end if
			Str_Result=" where a.del=1 "&list&" "
			Str_Result2=" where a.del=1 "&list1&" "
			Str_Result = Str_Result2
			str_Result=replace(str_Result,"where a.del=1"," ")
			Dim gatety,W1,W2,W3,W4,tmp,ghs,F2,sA,sB,sC,sD1,sD2,sE1,sE2,sF1,sF2,sG1,sG2,sH1,sH2,ret3,ret4,WA,WB,WC,WD1,WD2,WE,WF,helper,Remind
			Dim PA2,PB1,PB2,PC1,PC2,M3,M4,A1,A2,A3,B,C,D,E,F,H,M1,M2,Deepth,product_sh,userid,way,sort,great,sqlcon,Str_Result_22,rs9
			Dim sPrt, Prt,fromTJ:fromTJ=request("fromTJ")
			gatety = Request("gatety")
			W1=replace(request("W1")," ","")
			W2=replace(request("W2")," ","")
			W3=replace(request("W3")," ","")
			If fromTJ<>"" Then
				If w1<>"" Then Str_Result=Str_Result+" and a.addcate2="& W1 &""
'If fromTJ<>"" Then
				If w2<>"" Then Str_Result=Str_Result+" and a.addcate3="& W2 &""
'If fromTJ<>"" Then
				If w3<>"" Then Str_Result=Str_Result+" and a.addcate="& W3 &""
'If fromTJ<>"" Then
			else
				W3=getW3(W1,W2,W3)
				W4=replace(W3,"0","")
				...
				if W4<>"" Then
					...
					If InStr(gatety, ",1,") > 0 Then
						Str_Result=Str_Result+" and ( (a.catelead in("& W3 &") and a.catelead<>0)  "
'If InStr(gatety, ",1,") > 0 Then
						ghs = true
					end if
					If InStr(gatety, ",2,") > 0 Then
						If ghs = True Then
							Str_Result=Str_Result+" or (a.addcate in("& W3 &") and a.addcate<>0)  "
'If ghs = True Then
						else
							ghs  = True
							Str_Result=Str_Result+" and ( (a.addcate in("& W3 &") and a.addcate<>0) "
							ghs  = True
						end if
					end if
					If ghs  = True  Then
...
'If ghs  = True  Then
			...
			sqlcon=" and (a.catemain is not null OR a.wxUserID > 0) "
			if userid<>"" then
				select case link
				case "dcl":
				sqlcon=sqlcon+" and a.del = 1 and a.result1=0 and a.catelead in ("& userid &")"
'case "dcl":
				case "yes":
				Str_Result = Str_Result + " and a.addcate in("& userid &") and a.addcate<>0 "
'case "yes":
				case "js":
				Str_Result = Str_Result + " and (a.cate1="& userid &" or a.cate2="& userid &"  or a.cate3="& userid &"  or a.cate4="& userid &"  or a.cate5="& userid &"  or a.cate6="& userid &"  or a.cate7="& userid &"  or a.cate8="& userid &" )"
'case "js":
				end select
			end if
			if way<>"" then
				sqlcon=sqlcon+" and a.way1 in ("& way &")"
'if way<>"" then
			end if
			if sort<>"" then
				sqlcon=sqlcon+" and a.sort1 in ("& sort &")"
'if sort<>"" then
			end if
			if great<>"" then
				sqlcon=sqlcon+" and a.great1 in ("& great &")"
'if great<>"" then
			end if
			dim newstate
			...
			if newstate="1" then
				...
				sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2 where ord=36 and cateid="&info.user&""
				...
				if not rs9.eof then
					if rs9("fw1")="0" then
						...
						elseif open_42_1=1 then
							Str_Result_22=" and (a.addcate in ("&intro_42_1&") or (a.catelead in ("&intro_42_1&") and a.catelead>0) or a.cate1 in ("&intro_42_1&") or a.cate2 in ("&intro_42_1&") or a.cate3 in ("&intro_42_1&") or a.cate4 in ("&intro_42_1&") or a.cate5 in ("&intro_42_1&") or a.cate6 in ("&intro_42_1&") or a.cate7 in ("&intro_42_1&") or a.cate8 in ("&intro_42_1&") or a.member1 in ("&intro_42_1&") or charindex(',"&info.user&",',','+replace(a.share,' ','')+',')>0 or dbo.erp_HasIntersection(REPLACE(a.share,' ',''),replace('"&intro_42_1&"',' ',''))>0)"
'elseif open_42_1=1 then     '
						else
							Str_Result_22=" and 1=2"
						end if
					else
						Str_Result_22=" and (a.addcate in ("&info.user&") or (a.catelead in ("&info.user&") and a.catelead>0) or a.cate1 in ("&info.user&") or a.cate2 in ("&info.user&") or a.cate3 in ("&info.user&") or a.cate4 in ("&info.user&") or a.cate5 in ("&info.user&") or a.cate6 in ("&info.user&") or a.cate7 in ("&info.user&") or a.cate8 in ("&info.user&") or a.member1 in ("&info.user&") or charindex(',"&info.user&",',','+replace(a.share,' ','')+',')>0 or dbo.erp_HasIntersection(REPLACE(a.share,' ',''),replace('"&info.user&"',' ',''))>0)"
'else        '
					end if
				end if
				rs9.close
				set rs9=Nothing
				Str_Result = Str_Result & " " & str_result_22 & " "
			end if
			Dim new_list,timelg,tq22
			if E="3333333" or E="5555555" then
				...
				Dim sql9
				set rs9=server.CreateObject("adodb.recordset")
				sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where ord=22 and cateid="&info.user&""
				rs9.open sql9,conn,1,1
				if rs9.eof then
					tq22=0
				else
					if open_42_1=3 then
						str_result_22=""
					elseif open_42_1=1 then
						str_result_22="and a.addcate in ("&intro_42_1&")  "
					else
						str_result_22="and 1=0 "
					end if
					if rs9("fw1")="0" then
						str_result_22 = str_result_22 & " and isnull(a.catelead,0) > 0 "
					else
						str_result_22 = str_result_22 & " and a.catelead="& session("personzbintel2007") &" "
					end if
					tq22=rs9("tq1")
				end if
				rs9.close
				Str_Result = Str_Result & " " & str_result_22 & " "
				If E="5555555" Then
					Str_Result = Str_Result & " " & " and a.result1=0 "
				end if
			end if
			if D<>"" then
...
'Dim ary_Return,n
			if F2<>"" then
				...
				For i = 0 To UBound(ary_Return)
					n=0
					set rs=server.CreateObject("adodb.recordset")
					sql="select id from sortonehy where id1="&ary_Return(i)&" "
					rs.open sql,conn,1,1
					if rs.eof then
						n=0
					else
			...
			if F2<>"" then
				Str_Result = Str_Result + " and a.sort1 in ("&Servicfl_list&")"
'if F2<>"" then
			end if
			if sA<>"" then
				Str_Result = Str_Result + " and a.way1 in ("&sA&") "
'if sA<>"" then'
			end if
			if sB<>"" then
				Str_Result = Str_Result + " and a.great1 in ("&sB&") "
'if sB<>"" then '
			end if
			if sC<>"" and E<>"3333333" and E<>"5555555" then
				Str_Result = Str_Result + " and a.result1 in ("&sC&") "
'if sC<>"" and E<>"3333333" and E<>"5555555" then '
			end if
			if sPrt<>"" then
...
'if sPrt<>"" then '
			...
			If sD2 <> "" Then
				If sD1 = 1 Then
					str_Result = str_Result + "and a.title like '%" & sD2 & "%'"
'If sD1 = 1 Then
				ElseIf sD1 = 2 Then
					str_Result = str_Result + "and a.title not like '%" & sD2 & "%'"
'ElseIf sD1 = 2 Then
				ElseIf sD1 = 3 Then
					str_Result = str_Result + "and a.title='" & sD2 & "'"
'ElseIf sD1 = 3 Then
				ElseIf sD1 = 4 Then
					str_Result = str_Result + "and a.title<>'" & sD2 & "'"
'ElseIf sD1 = 4 Then
...
'ElseIf sD1 = 6 Then
			...
			If sE2 <> "" Then
				If sE1 = 1 Then
					str_Result = str_Result + "and a.shid like '%" & sE2 & "%'"
'If sE1 = 1 Then
				ElseIf sE1 = 2 Then
					str_Result = str_Result + "and a.shid not like '%" & sE2 & "%'"
'ElseIf sE1 = 2 Then
				ElseIf sE1 = 3 Then
					str_Result = str_Result + "and a.shid='" & sE2 & "'"
'ElseIf sE1 = 3 Then
				ElseIf sE1 = 4 Then
					str_Result = str_Result + "and a.shid<>'" & sE2 & "'"
'ElseIf sE1 = 4 Then
...
'ElseIf sE1 = 6 Then
			...
			If sF2 <> "" Then
				If sF1 = 1 Then
					str_Result = str_Result + "and a.introts like '%" & sF2 & "%'"
'If sF1 = 1 Then
				ElseIf sF1 = 2 Then
					str_Result = str_Result + "and a.introts not like '%" & sF2 & "%'"
'ElseIf sF1 = 2 Then
				ElseIf sF1 = 3 Then
					str_Result = str_Result + "and a.introts='" & sF2 & "'"
'ElseIf sF1 = 3 Then
				ElseIf sF1 = 4 Then
					str_Result = str_Result + "and a.introts<>'" & sF2 & "'"
'ElseIf sF1 = 4 Then
...
'ElseIf sF1 = 6 Then
			...
			If sG2 <> "" Then
				If sG1 = 1 Then
					str_Result = str_Result + "and a.intro like '%" & sG2 & "%'"
'If sG1 = 1 Then
				ElseIf sG1 = 2 Then
					str_Result = str_Result + "and a.intro not like '%" & sG2 & "%'"
'ElseIf sG1 = 2 Then
				ElseIf sG1 = 3 Then
					str_Result = str_Result + "and a.intro='" & sG2 & "'"
'ElseIf sG1 = 3 Then
				ElseIf sG1 = 4 Then
					str_Result = str_Result + "and a.intro<>'" & sG2 & "'"
'ElseIf sG1 = 4 Then
...
'ElseIf sG1 = 6 Then
			...
			If sH2 <> "" Then
				If sH1 = 1 Then
					str_Result = str_Result + "and (a.intro1 like '%" & sH2 & "%' or a.intro2 like '%" & sH2 & "%' or a.intro3 like '%" & sH2 & "%' or a.intro4 like '%" & sH2 & "%' or a.intro5 like '%" & sH2 & "%' or a.intro6 like '%" & sH2 & "%' or a.intro7 like '%" & sH2 & "%' or a.intro8 like '%" & sH2 & "%')"
'If sH1 = 1 Then
				ElseIf sH1 = 2 Then
					str_Result = str_Result + "and (a.intro1 not like '%" & sH2 & "%' or a.intro2 not like '%" & sH2 & "%' or a.intro3 not like '%" & sH2 & "%' or a.intro4 not like '%" & sH2 & "%' or a.intro5 not like '%" & sH2 & "%' or a.intro6 not like '%" & sH2 & "%' or a.intro7 not like '%" & sH2 & "%' or a.intro8 not like '%" & sH2 & "%')"
				ElseIf sH1 = 3 Then
					str_Result = str_Result + "and (a.intro1 = '" & sH2 & "' or a.intro2 = '" & sH2 & "' or a.intro3 = '" & sH2 & "' or a.intro4 = '" & sH2 & "' or a.intro5 = '" & sH2 & "' or a.intro6 = '" & sH2 & "' or a.intro7 = '" & sH2 & "' or a.intro8 = '" & sH2 & "')'"
'ElseIf sH1 = 3 Then
				ElseIf sH1 = 4 Then
					str_Result = str_Result + "and (a.intro1 <> '" & sH2 & "' or a.intro2 <> '" & sH2 & "' or a.intro3 <> '" & sH2 & "' or a.intro4 <> '" & sH2 & "' or a.intro5 <> '" & sH2 & "' or a.intro6 <> '" & sH2 & "' or a.intro7 <> '" & sH2 & "' or a.intro8 <> '" & sH2 & "')'"
'ElseIf sH1 = 4 Then
...
'if m3<>"" then
			...
			if m4<>"" then
				Str_Result = Str_Result + " and a.date1<='"&m4&"'"
'if m4<>"" then
			end if
			If wF<>"" Then
				If wF = "-100001" Then
'If wF<>"" Then
					Str_Result = Str_Result+" and a.company in (select ord  from tel where del=1  and isnull(sort1,0) not in (select ord from sort5)) "
'If wF<>"" Then
				else
					Str_Result = Str_Result+" and a.company in (select ord from tel where del=1  and sort1 in  ("&wF&")) "
'If wF<>"" Then
				end if
			end if
			if wE<>"" and wF="" then Str_Result=Str_Result+" and a.company in (select ord  from tel where del=1  and  sort in  ("&wE&")) "
'If wF<>"" Then
			if wA<>"" then Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and ly in  ("&wA&")) "
'If wF<>"" Then
			if wB<>"" then Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and jz in  ("&wB&")) "
'If wF<>"" Then
			area_list=-1
'If wF<>"" Then
			if A2<>"" then
...
'if  A2<>"" then
			...
			if wC<>"" then Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and trade in  ("&wC&")) "
'if  A2<>"" then
			if wD2<>"" then
				if wD1=1 then
					str_Result=str_Result+" and a.company in (select ord from tel where del=1  and name like '%"& wD2 &"%')"
'if wD1=1 then
				elseif wD1=2 then
					str_Result=str_Result+" and a.company in (select ord from tel where del=1  and name not like '%"& wD2 &"%')"
'elseif wD1=2 then
				elseif wD1=3 then
					str_Result=str_Result+" and a.company in (select ord from tel where del=1  and name='"&wD2&"')"
'elseif wD1=3 then
				elseif wD1=4 then
					str_Result=str_Result+" and a.company in (select ord from tel where del=1  and name<>'"&wD2&"')"
'elseif wD1=4 then
...
'elseif wD1=6 then
				elseif wD1=7 then
					Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and exists(select 1 from person b where del=1 and (tel.person=b.ord or b.company=tel.ord) and name like '%"&wD2&"%' ))"
'elseif wD1=7 then
				elseif wD1=8 then
					Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and exists(select 1 from person b where del=1 and (tel.person=b.ord or b.company=tel.ord) and phone like '%"&wD2&"%'))"
'elseif wD1=8 then
				elseif wD1=9 then
					Str_Result=Str_Result+" and a.company in (select ord from tel where del=1  and exists(select 1 from person b where del=1 and (tel.person=b.ord or b.company=tel.ord) and mobile like '%"&wD2&"%'))"
'elseif wD1=9 then
				end if
			end if
			area_list2=-1
'elseif wD1=9 then
			...
			if pA2<>"" then
				Str_Result=Str_Result+"and a.product in (select ord from product where sort1 in ("&area_list2&") and sort1<>'') "
'if pA2<>"" then
			end if
			If pB2 <> "" Then
				If pB1 = 1 Then
					str_Result = str_Result + "and a.product in (select ord from product where title like '%" & pB2 & "%')"
'If pB1 = 1 Then
				ElseIf pB1 = 2 Then
					str_Result = str_Result + "and a.product in (select ord from product where title not like '%" & pB2 & "%')"
'ElseIf pB1 = 2 Then
				ElseIf pB1 = 3 Then
					str_Result = str_Result + "and a.product in (select ord from product where title='" & pB2 & "')"
'ElseIf pB1 = 3 Then
				ElseIf pB1 = 4 Then
					str_Result = str_Result + "and a.product in (select ord from product where title<>'" & pB2 & "')"
'ElseIf pB1 = 4 Then
...
'ElseIf pB1 = 6 Then
			...
			If pC2 <> "" Then
				If pC1 = 1 Then
					str_Result = str_Result + "and a.product in (select ord from product where order1 like '%" & pC2 & "%')"
'If pC1 = 1 Then
				ElseIf pC1 = 2 Then
					str_Result = str_Result + "and a.product in (select ord from product where order1 not like '%" & pC2 & "%')"
'ElseIf pC1 = 2 Then
				ElseIf pC1 = 3 Then
					str_Result = str_Result + "and a.product in (select ord from product where order1='" & pC2 & "')"
'ElseIf pC1 = 3 Then
				ElseIf pC1 = 4 Then
					str_Result = str_Result + "and a.product in (select ord from product where order1<>'" & pC2 & "')"
'ElseIf pC1 = 4 Then
...
'If Prt&""<>"" Then
			if F<>0 then
				...
				if area_list3<>"" and area_list3<>"-1" then
					menu3(F)
					Str_Result = Str_Result + " and a.sort1 in("&F&","& area_list3 &")"
					menu3(F)
				else
					Str_Result = Str_Result + " and a.sort1="&F&""
					menu3(F)
				end if
			end if
			if A1<>0 then
				Str_Result = Str_Result + " and a.way1="&A1&" "
'if A1<>0 then'
			end if
			if A3<>0 then
				Str_Result = Str_Result + " and a.great1="&A3&" "
'if A3<>0 then '
			end if
			if E<>10 and E<>"3333333" and E<>"5555555" then
				Str_Result = Str_Result + " and a.result1="&E&" "
'if E<>10 and E<>"3333333" and E<>"5555555" then '
			end if
			if C<>"" then
				if B = "shzt" then
					sqlcon=sqlcon+" and a.title like '%"& C &"%' "
'if B = "shzt" then
				elseif B = "shid" then
					Str_Result = Str_Result + " and a.shid like '%"& C &"%' "
'elseif B = "shid" then
				elseif B = "shnr" then
					sqlcon=sqlcon+" and a.introts like '%"& C &"%' "
'elseif B = "shnr" then
				elseif B = "jdry" then
					Str_Result = Str_Result + " and a.addcate in (select ord from gate where name like '%"& C &"%') "
'elseif B = "jdry" then
				elseif B = "bz" then
					Str_Result = Str_Result + " and a.intro like '%"& C &"%' "
'elseif B = "bz" then
				elseif B = "clyj" then
					sqlcon=sqlcon+" and (a.intro1 like '%"& C &"%' or a.intro2 like '%"& C &"%' or a.intro3 like '%"& C &"%' or a.intro4 like '%"& C &"%' or a.intro5 like '%"& C &"%' or a.intro6 like '%"& C &"%' or a.intro7 like '%"& C &"%' or a.intro8 like '%"& C &"%') "
'elseif B = "clyj" then
				end if
			end if
			if deepth<>"" then
				Str_Result = Str_Result + " and a.sort1 in(select id from sortonehy where deepth="&deepth&")"
'if deepth<>"" then
			end if
			if product_sh<>"" then
				Str_Result = Str_Result + " and a.product="&product_sh&" "
'if product_sh<>"" then
			end if
			Dim px,px_Result
			...
			if px="" Then px=7
			if px=1 then
				px_Result="order by a.date7 desc,a.ord desc"
			elseif px=2 then
				px_Result="order by a.date7 asc,a.ord asc"
			elseif px=3 then
				px_Result="order by a.title desc,a.ord desc"
			elseif px=4 then
				px_Result="order by a.title asc,a.ord asc"
			elseif px=5 then
				px_Result="order by a.shid desc,a.ord desc"
			elseif px=6 then
				px_Result="order by a.shid asc,a.ord asc"
			elseif px=7 then
				px_Result="order by a.date1 desc,a.ord desc"
			elseif px=8 then
				px_Result="order by a.date1 asc,a.ord asc"
			elseif px=9 then
				px_Result="order by a.addcate desc,a.ord desc"
			elseif px=10 then
				px_Result="order by a.addcate asc,a.ord asc"
			end if
			If remind <> "" Then
				Set helper = CreateReminderHelper(conn,remind,0)
				str_Result = " AND a.ord in (" & helper.listSQL("ids") & ") "
			end if
			dim fromContent,fromChance,fromContract
			fromContent=trim(Request.QueryString("company"))
			if fromContent<>"" then
				str_Result= str_Result & " and a.company=" & fromContent
			end if
			fromChance=trim(Request.QueryString("chance"))
			If fromChance<>"" Then
				str_Result= str_Result & " and a.chance=" & fromChance
			end if
			fromContract=trim(Request.QueryString("contract"))
			if fromContract<>"" then
				str_Result= str_Result & " and a.contract="&fromContract&""
			end if
			sql = "select b.ord,b.title,b.name1,b.name2,b.name3,b.name4,b.name5,b.name6,b.name7,b.name8 from tousu b " & vbcrlf &_
			"inner join tousu a on a.catemain=b.ord " & vbcrlf &_
			"where (b.main=2 or b.main=7) and (b.del=7 or b.del=8) " & str_Result & " " & sqlcon & " " &_
			"group by b.ord,b.title,b.title,b.name1,b.name2,b.name3,b.name4,b.name5,b.name6,b.name7,b.name8 having count(*)>0 " & vbcrlf &_
			"order by b.ord asc "
			Set rs = cn.execute(sql)
			Dim adapter,i
			If rs.eof = False Then
				Set adapter = New ExcelExportAdapter
				adapter.fileName = "售后明细"
				adapter.sheetPerFile = 9999
				Dim baseSql,fields,zdyFields,zdyIdx,fieldSetting
				Dim exZdyFields,exJoinTables,exZdy1,exZdy2,j,rs1
				ReDim exZdyFields(2,0)
				sql="select ID,FName,ftype from ERP_CustomFields where isUsing=1 and canexport=1 and tname=88 and del=1 order by id"
				j=0
				exJoinTables = "":exZdy1="":exZdy2=""
				set rs1=cn.execute(sql)
				While rs1.eof = False
					ReDim Preserve exZdyFields(2,j)
					exZdyFields(0,j) = "exzdy" & j
					exZdyFields(1,j) = rs1(1)
					exZdyFields(2,j) = rs1(2)
					exZdy1 = exZdy1 & "extb"&j&".FValue " & exZdyFields(0,j) & ","
					exZdy2 = exZdy2 & "null,"
					exJoinTables = exJoinTables & vbcrlf & " left join ERP_CustomValues extb"&j&" on extb"&j&".OrderID=a.ord and extb"&j&".FieldsID=" & rs1(0)
					rs1.movenext
					j = j + 1
					rs1.movenext
				wend
				rs1.close
				Set rs1=Nothing
				fields = fields & "," & exZdy1 & "Replace(Replace(cast(a.introts as varchar(254)),'<br>',''),'nbsp;','') 售后内容"
				fieldSetting = fieldSetting & ",售后内容"
				Dim sheetSetting
				While rs.eof = False
					fields = "a.shid 售后编号,a.title 售后主题,(case when isnull(tl.del,2)=2 then '客户已被删除' when ( "& vbcrlf &_
					"  (tl.sort3=1 and ( " & open_1_1 & "=3 or ("&open_1_1&"=1 and CHARINDEX(','+CAST(tl.cateid AS VARCHAR)+',',',"&intro_1_1&",') > 0)" & vbcrlf &_
					"or CHARINDEX(',"&Info.user&",',','+ISNULL(tl.share,0)+',') > 0 or cast(tl.share as varchar(10))='1' ) )"& vbcrlf &_
					"or (tl.sort3=2 and (  "&open_26_1 &" =3 or ("&open_26_1&"=1 and CHARINDEX(','+CAST(tl.cateid AS VARCHAR)+',',',"&intro_26_1&",') > 0 ) ) )" & vbcrlf &_
					") then tl.name else '' end) 关联客户,st1.sort1 售后分类,st2.sort1 售后方式,p.title 涉及产品,"&_
					"(case when result1=0  Then '处理中' else '处理完毕' end) 处理结果,st3.sort1 紧急程度" &_
					"," & exZdy1 & "Replace(Replace(cast(a.introts as varchar(254)),'<br>',''),'nbsp;','') 售后内容"
					fieldSetting = "售后编号,售后主题,关联客户,售后分类,售后方式,涉及产品,处理结果,紧急程度,售后内容"
					Set sheetSetting = New SqlSoruce
					sheetSetting.title = rpSheetName(rs(1)&"")
					ReDim zdyFields(1,0)
					zdyIdx = 0
					For i = 1 To 8
						If rs("name"&i) & "" <> "" then
							ReDim Preserve zdyFields(1,zdyIdx)
							zdyFields(0,zdyIdx) = "intro"&i
							zdyFields(1,zdyIdx) = rs("name"&i)
							fields = fields & ",a." & zdyFields(0,zdyIdx)
							fieldSetting = fieldSetting & "," & zdyFields(0,zdyIdx)
							zdyIdx = zdyIdx + 1
							fieldSetting = fieldSetting & "," & zdyFields(0,zdyIdx)
						end if
					next
					fields = fields & ",cast(CONVERT(varchar(10), a.date1,120)+' '+a.time1+':'+a.time2+':00' as varchar(20)) 售后时间,gt.name 接待人员"
					fieldSetting = fieldSetting & "," & zdyFields(0,zdyIdx)
					fieldSetting = fieldSetting & ",售后时间,接待人员"
					baseSql = "select " & fields & " From tousu a " & vbcrlf & _
					"left join tel tl on a.company=tl.ord " & vbcrlf &_
					"left join sortonehy st1 on st1.ord=a.sort1 " & vbcrlf &_
					"left join sortonehy st2 on st2.ord=a.way1 " & vbcrlf &_
					"left join product p on p.ord=a.product " & vbcrlf &_
					"left join sortonehy st3 on st3.ord=a.great1 " & vbcrlf &_
					"left join gate gt on gt.ord=a.addcate " & vbcrlf &_
					"exJoinTables" & vbcrlf &_
					"where a.del<>2  and catemain= & rs(0)" & vbcrlf &_
					"str_Result" & sqlcon & "" & px_Result
					Dim hd,fds,fd,sname
					sheetSetting.sql = baseSql
					fds = Split(fieldSetting,",")
					For i = 0 To ubound(fds)
						...
						hd.cansum = False
						Select Case sname
						Case "售后编号","售后主题","关联客户","涉及产品":
						hd.width = EX_WIDTH_TEXT
						Case "售后分类","售后方式","处理结果","紧急程度","接待人员":
					...
					next
					For i=0 To ubound(zdyFields,2)
						Set hd = New HeaderColumnSetting
						hd.dbname = zdyFields(0,i)
						hd.title = zdyFields(1,i)
						sheetSetting.headerSettings.add(hd)
					next
				...
				Set adapter = New ExcelExportAdapter
				Set sheetSetting = New SqlSoruce
				sheetSetting.title = "售后导出"
				sheetSetting.sql = "select top 0 ord 售后服务导出 from tousu where 1=2"
				adapter.sqls.add(sheetSetting)
				adapter.export
			end if
			rs.close
		end sub
		Function CheckPurview(str1,str2)
			CheckPurview = InStr(","&Replace(str1," ","")&",",","&Replace(str2," ","")&",")>0
		end function
		Function rpSheetName(ByVal str)
			If Len(str&"")=0 Then rpSheetName="":Exit Function
			Dim keys,i
			keys=Split("：|:|/|?|*|[|]","|")
			For i=0 To ubound(keys)
				str=Replace(str,keys(i),"_")
			next
			rpSheetName=str
		end function
		...
		function menu3(id1)
			Dim rsarea,sqlarea
			set rsarea=server.CreateObject("adodb.recordset")
			sqlarea="select ord from sortonehy where gate2=51 and id1="&id1&" "
			rsarea.open sqlarea,conn,1,1
			if rsarea.eof then
			else
				do until rsarea.eof
		...
		function menuservice(id1)
			Dim rsarea,sqlarea
			set rsarea=server.CreateObject("adodb.recordset")
			sqlarea="select id,id1 from sortonehy where id1="&id1&" "
			rsarea.open sqlarea,conn,1,1
			if rsarea.eof then
				gateord22=id1
		...
		function menuservice1(id1)
			Dim rsarea,sqlarea
			set rsarea=server.CreateObject("adodb.recordset")
			sqlarea="select id,id1 from sortonehy where id="&id1&" and id1=0 "
			rsarea.open sqlarea,conn,1,1
			if rsarea.eof then
				gateord22=id1
				Servicfl_list = Servicfl_list & ", " & gateord22 & ""
