...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	j=2
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from sortwages order by gate1 desc"
	rs7.open sql7,conn,1,1
	num1=rs7.recordCount
	if not rs7.eof then
		do while not rs7.eof
			xlApplication.ActiveSheet.Columns(j).ColumnWidth=12
	...
	xlApplication.ActiveSheet.Columns(j).ColumnWidth=12
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+1).ColumnWidth=12
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+1).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+2).ColumnWidth=12
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+2).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+3).ColumnWidth=12
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j+3).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	set rs1=server.CreateObject("adodb.recordset")
	rs1.open "select title,bz from wages where id="&id&"",conn,1,1
	if not rs1.eof then
		title=rs1("title")
		bz=rs1("bz")
	end if
	rs1.close
	set rs1=Nothing
	If bz<>"" Or Not IsNull(bz) then
		set rs1=server.CreateObject("adodb.recordset")
		rs1.open "select intro from sortbz where id="&bz&"",conn,1,1
		if not rs1.eof then
			sortbz=rs1("intro")
		else
			sortbz=""
	...
	jf=0
	set rs1=server.CreateObject("adodb.recordset")
	rs1.open "select intro from setopen  where sort1=18 ",conn,1,1
	if not rs1.eof then
		jf=rs1("intro")
	end if
	rs1.close
	If jf=0 Then
		rs1.open "select id from wages_jj where wages2="&id&" ",conn,1,1
		if rs1.eof then
			jf=0
		else
			jf=1
	...
	num1=0
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select sort1 from sortwages order by gate1 desc"
	rs7.open sql7,conn,1,1
	num1=rs7.recordCount
	if ZBRuntime.MC(26002) then
		num1=num1+6
...
'if ZBRuntime.MC(26002) then
	...
	xlWorksheet.Cells(1,1).colspan = num1
	xlWorksheet.Cells(1,1).Value= title
	xlWorksheet.Cells(1,1).font.Size=16
	xlWorksheet.Cells(1,1).font.bold=true
	xlWorksheet.Cells(2,1).Value = "员工姓名"
	xlWorksheet.Cells(2,1).font.Size=10
	xlWorksheet.Cells(2,1).font.bold=true
	j=2
	if not rs7.eof then
		do while not rs7.eof
			xlWorksheet.Cells(2,j).Value = rs7("sort1")
			xlWorksheet.Cells(2,j).font.Size=10
			xlWorksheet.Cells(2,j).font.bold=true
			rs7.movenext
			j=j+1
			rs7.movenext
		loop
	...
	rs7.close
	set rs7=nothing
	if ZBRuntime.MC(26002) and jf=1 then
		xlWorksheet.Cells(2,j).Value = "计件工资"
		xlWorksheet.Cells(2,j).font.Size=10
		xlWorksheet.Cells(2,j).font.bold=true
		j=j+1
		xlWorksheet.Cells(2,j).font.bold=true
	end if
	xlWorksheet.Cells(2,j).Value = "子女教育"
	xlWorksheet.Cells(2,j).font.Size=10
	xlWorksheet.Cells(2,j).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+1).Value = "继续教育(学历)"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+1).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+1).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+1).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+2).Value = "继续教育(技能)"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+2).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+2).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+2).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).Value = "大病医疗"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+3).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+4).Value = "住房贷款"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+4).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+4).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+4).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+5).Value = "住房租金"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+5).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+5).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+5).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+6).Value = "赡养老人"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+6).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+6).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+6).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+7).Value = "应发工资"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+7).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+7).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+7).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+8).Value = "应扣工资"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+8).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+8).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+j+8).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+9).Value = "实发工资"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+9).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+9).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j+9).NumberFormatLocal = "#,##0.00_ "
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+10).Value = "备注"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+10).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+10).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	Response.Flush
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,name from gate where ord="&ord&" order by cateid asc"
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "工资条_"&session("name2006chen")&".xls",C1
	i=1
	do until rs.eof
		xlWorksheet.Cells(2+i,1).Value = rs("name")
		i=1
		xlWorksheet.Cells(2+i,1).font.Size=10
		i=1
		xlWorksheet.Cells(2+i,1).HorizontalAlignment=1
		i=1
		set rs2=server.CreateObject("adodb.recordset")
		sql2="select * from sortwages order by gate1 desc"
		rs2.open sql2,conn,1,1
		j=2
		do until rs2.eof
			set rs88=server.CreateObject("adodb.recordset")
			sql88="select money1,intro from wageslist Where wages="&id&" and cateid="&rs("ord")&"  and sort1="&rs2("id")&""
			rs88.open sql88,conn,1,1
			if rs88.eof then
				money1=0
				intro=""
			else
				money1=zbcdbl(rs88("money1"))
				intro=rs88("intro")
			end if
			rs88.close
			set rs88=nothing
			if rs2("intro")=1 then
				money2=cdbl(money2)+cdbl(money1)
'if rs2("intro")=1 then
			elseif rs2("intro")=-1 then
'if rs2("intro")=1 then
				money3=cdbl(money3)+cdbl(money1)
'if rs2("intro")=1 then
			end if
			xlWorksheet.Cells(2+i,j).Value = sortbz&Formatnumber(money1,num_dot_xs,-1)
'if rs2("intro")=1 then
...
'if rs2("intro")=1 then
		...
		rs2.close
		set rs2=nothing
		money4=cdbl(money2)-cdbl(money3)
		set rs2=nothing
		set rs99=server.CreateObject("adodb.recordset")
		sql99="select isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-1),0)ChildrenseDucation,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-2),0)ContinuingEducationxl,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-3),0)ContinuingEducationjn,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-4),0)medical,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-5),0)Housingloans,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-6),0)payment,isnull(dbo.HrWageList("&id&","&rs("ord")&" ,-7),0)SupportOldPeople from wages Where id="&id&" "
		set rs99=server.CreateObject("adodb.recordset")
		rs99.open sql99,conn,1,1
		if rs99.eof then
			ChildrenseDucation=0
			ContinuingEducationxl=0
			ContinuingEducationjn=0
			medical=0
			Housingloans=0
			payment=0
			SupportOldPeople=0
		else
			ChildrenseDucation=rs99("ChildrenseDucation")
			ContinuingEducationxl=rs99("ContinuingEducationxl")
			ContinuingEducationjn=rs99("ContinuingEducationjn")
			medical=rs99("medical")
			Housingloans=rs99("Housingloans")
			payment=rs99("payment")
			SupportOldPeople=rs99("SupportOldPeople")
		end if
		if ZBRuntime.MC(26002) and jf=1 then
			set rs8=server.CreateObject("adodb.recordset")
			sql8="select isnull(sum(money1),0) as money1 from wageslist_jj where wages2="&id&" and cateid="&rs("ord")&" "
			rs8.open sql8,conn,1,1
			if rs8.eof then
				money_jj=0
			else
				money_jj=zbcdbl(rs8("money1"))
			end if
			rs8.close
			set rs8=nothing
			money2=cdbl(money2)+cdbl(money_jj)
			set rs8=nothing
			money4=cdbl(money4)+cdbl(money_jj)
			set rs8=nothing
			xlWorksheet.Cells(2+i,j).value = sortbz&Formatnumber(money_jj,num_dot_xs,-1)
			set rs8=nothing
			xlWorksheet.Cells(2+i,j).font.Size=10
			set rs8=nothing
			j=j+1
		...
		xlWorksheet.Cells(2+i,j).Value = sortbz&Formatnumber(ChildrenseDucation,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+1).Value = sortbz&Formatnumber(ContinuingEducationxl,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+1).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+2).Value = sortbz&Formatnumber(ContinuingEducationjn,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+2).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+3).Value = sortbz&Formatnumber(medical,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+3).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+4).Value = sortbz&Formatnumber(Housingloans,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+4).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+5).Value = sortbz&Formatnumber(payment,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+5).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+6).Value = sortbz&Formatnumber(SupportOldPeople,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+6).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+7).Value = sortbz&Formatnumber(money2,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+7).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+8).Value = sortbz&Formatnumber(money3,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+8).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+9).Value = sortbz&Formatnumber(money4,num_dot_xs,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+9).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+10).Value = intro
		set rs8=nothing
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>工资条导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!工资条导出成功,共有"&(i-1)&"条工资记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("工资条_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的工资条</u></strong></font></a></p>" & vbcrlf & ""
