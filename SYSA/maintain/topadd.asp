...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write """>" & vbcrlf & "<script language=""javascript"" src=""../sortcp/function.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<SCRIPT language=JavaScript1.2>" & vbcrlf & "// 一个简单的测试是否IE浏览器的表达式" & vbcrlf & "isIE = (document.all ? true : false);" & vbcrlf & "" & vbcrlf & "// 得到IE中各元素真正的位移量，即使这个元素在一个表格中" & vbcrlf & "function getIEPosX(elt) { return getIEPos(elt,""Left""); }" & vbcrlf & "function getIEPosY(elt) { return getIEPos(elt,""Top""); }" & vbcrlf & "function getIEPos(elt,which) {" & vbcrlf & " iPos = 0" & vbcrlf & " while (elt!=null) {" & vbcrlf & "  iPos += elt[""offset"" + which]" & vbcrlf & "  elt = elt.offsetParent" & vbcrlf & " }" & vbcrlf & " return iPos" & vbcrlf & "}" & vbcrlf & "function ask() { " & vbcrlf & "document.all.date.action = ""savelistadd13.asp""; " & vbcrlf & "} " & vbcrlf & "// -->" & vbcrlf & "</SCRIPT>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "        /**BUG.6814.ZYF 解决easyui和jQuery树样式冲突问题**/" & vbcrlf & "     .tree-folder-open{background:;}" & vbcrlf &"  #cp_search{width:221px!important;}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body onLoad=""txmFocus();"" onclick=""TexTxmFocus(event);"">" & vbcrlf & ""
	dim MODULES
	MODULES=session("zbintel2010ms")
	session("num_click2009")=0
	dim ids,top,f, act
	top=deurl(request("top"))
	f=request("f") : act=request("act")
	If act&""="" Then act = ""
	if f="101" then
		sql="Delete  mxpx Where cateid="&session("personzbintel2007")&" and sort1=79 "
		conn.Execute(sql)
	end if
...
'ElseIf treeType = "TC" Then
	...
	Call ShowLeftTree
	Response.write "" & vbcrlf & "      </td>" & vbcrlf & "    <td valign=""top"">" & vbcrlf & "        <form name=""date"" id=""demo"" class=""pagefrm"" method=""post"" action=""savelistadd.asp"" onsubmit=""return Validator.Validate(this,2)"" >" & vbcrlf & " <input type=""hidden"" name=""act"" value="""
	Response.write act
	Response.write """>" & vbcrlf & " <table width="""
	Response.write num1_kd+120
'Response.write """>" & vbcrlf & " <table width="""
	Response.write """ border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "        <td class=""place"">养护产品清单</td>" & vbcrlf & "            <td> </td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & ""
	Dim rs_zdy, len_rszdy
	Set rs = conn.execute("select sorce, name, title, name, kd from zdymx where sort1=9001 and set_open=1 order by gate1 asc")
	If rs.eof = False Then
		rs_zdy = rs.GetRows()
	end if
	rs.close
	...
	pitr = 0
	Set rs = conn.execute("select count(sorce) from zdymx where sort1=9001 and set_open=1 and sorce in(10,12)")
	pitr = rs(0)
	rs.close
	set rs = nothing
	If pitr>0 Then
		Response.write "" & vbcrlf & "<style> " & vbcrlf & "  tr.top_tr td{padding-top:0;padding-bottom:0}" & vbcrlf & "  #productlistHead td{height:40px!important;line-height:40px!important;padding-top:0;padding-bottom:0}" & vbcrlf & "  #trpx0 td{height:40px;}" & vbcrlf & "</style>" & vbcrlf & "<table width=""100%""border=""0"" class=""bgfff""  cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;table-layout:fixed; border:#CCC 0px solid;border-bottom:none; "">" & vbcrlf & "        <tr class=""top_tr"">" & vbcrlf & "       "
'If pitr>0 Then
		cols = 0 : pi1 = ""
...
'If pi = "" Then
			...
			If rs_zdy(0,i) = 10 Then
				If pi1="" Then
					If cols>1 Then Response.write "<td colspan="""&cols&""" align='right'>批量录入：</td>"
					pi1 = "1"
				end if
				Response.write "" & vbcrlf & "              <td align=""center"" width="""
				Response.write kd
				Response.write """><input name=""yhCate_pi"" id=""yhCate_pi"" type=""text"" style=""height: 19px; solid;font-size: 9pt; width: 98%;"" size=""10"" readonly onclick=""chkYHForm('pi')""><input name=""RemindPerson_pi"" id=""RemindPerson_pi"" type=""hidden""></td>" & vbcrlf & "     "
				'Response.write kd
			ElseIf rs_zdy(0,i) = 12 Then
				If pi1="" Then
					If cols>1 Then Response.write "<td colspan="""&cols&""" align='right'>批量录入：</td>"
					pi1 = "1"
				end if
				Response.write "" & vbcrlf & "              <td align=""center"" width="""
				Response.write kd
				Response.write """><select name=""status_pi"" id=""status_pi"" onChange=""batSetStatus(this.value)""><option value=""""></option><option value=""1"">合格</option><option value=""0"">不合格</option></select></td>" & vbcrlf & " "
			ElseIf rs_zdy(0,i) <> 10 And rs_zdy(0,i) <> 12 And pi1 = "1" Then
				Response.write "<td width="""&kd&""">&nbsp;</td>"
			end if
		next
		Response.write "" & vbcrlf & "      </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0""  background=""../images/m_table_top.jpg""  cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;table-layout:fixed"">" & vbcrlf & "  <tr  id='productlistHead' style=""height:30px;"">" & vbcrlf & ""
	'Response.write "" & vbcrlf & "      </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	for i=0 to len_rszdy
		zdyName = rs_zdy(1,i)
		zdyTitle = rs_zdy(2,i)
		kd = rs_zdy(4,i)
		Response.write "" & vbcrlf & "<td  dbname='"
		Response.write zdyName
		Response.write "' width="""
		Response.write kd
		Response.write """ align=""center"" style=""border:#CCC 1px solid;""><strong>"
		Response.write zdyTitle
		Response.write "</strong></td>" & vbcrlf & ""
	next
	Response.write "" & vbcrlf & "     </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	sql = "select a.id, a.ord cpord, c.title, c.order1, c.type1, c.unitjb, k.unit, a.ku kuid, a.num1, k.ph,case when isnull(s2.id,0)=0 then k.xlh else nl.serinum end xlh, k.datesc, k.dateyx, isnull(c.MaintainNum,0) MaintainNum, c.MaintainUnit,a.share,a.status,a.result,a.intro,"&_
	" n.zdy1,n.zdy2,n.zdy3,n.zdy4,n.zdy5,n.zdy6,isnull(s2.serialid,0) serialid "&_
	" from maintainlist a "&_
	" left join product c on a.ord=c.ord and c.del=1 "&_
	" left join ku k on k.ord=c.ord and a.ku=k.id and k.num2>0 "&_
	" left join kuinlist n on k.kuinlist=n.id and n.del=1 "&_
	" left join S2_SerialNumberRelation s2 on s2.Billtype = 61001 and s2.billid=n.kuin and s2.Listid = n.id and s2.del=1 and isnull(s2.[HandleStatus],0)=0 and s2.serialID= a.serialID "&_
	" left join M2_SerialNumberList nl on nl.id = s2.serialID "&_
	" where a.maintain="& top &" "
	If act = "edit" Then
		sql = sql &" and a.del = 1"
	else
		sql = sql &" and a.del = 7"
	...
	If rss.eof = True Then
		Response.write "" & vbcrlf & "    <span class=""bgfff"" id=""trpx0"">" & vbcrlf & "    <table width=""100%"" border=""0""  cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"" class=""bgfff"">" & vbcrlf & "    <tr >" & vbcrlf & "       <td height=""40"" style=""border:#CCC 1px solid;border-top:none;"">无产品明细！</td>" & vbcrlf & "         </tr>" & vbcrlf & "    </table>" & vbcrlf & "    </span>" & vbcrlf & "    "
'If rss.eof = True Then
		Dim qxOpen
		sdk.setup.getpowerattr 21,14,qxOpen, qxIntro
		i=1
	ElseIf rss.eof = False Then
		i = 0
		While rss.eof =False
			i = i + 1
'While rss.eof =False
			cpord = rss("cpord") : k=rss("title") : order1=rss("order1") : type1=rss("type1") : unitall=rss("unit") : unitjb=rss("unitjb")
			num1 = CDbl(rss("num1")) : ph = rss("ph") : xlh = rss("xlh") : datesc = rss("datesc") : dateyx = rss("dateyx")
			serialid = rss("serialid")
			kuid = rss("kuid") : yhlid = rss("id") : share = rss("share")
			status = rss("status") : result = rss("result") : intro = rss("intro")
			zdy1 = rss("zdy1") : zdy2 = rss("zdy2") : zdy3 = rss("zdy3") : zdy4 = rss("zdy4") : zdy5 = rss("zdy5") : zdy6 = rss("zdy6")
			MaintainNum = CDbl(rss("MaintainNum")) : MaintainUnit = rss("MaintainUnit")
			if isnull(unitall) or unitall="" then
				unitall=0
			end if
			if unitjb="" or isnull(unitjb) then
				unitjb=0
			end if
			unit=unitall
			if unit&""="0" then unit=unitjb
			If share&""<>"" Then
				share = Replace(share,","," ")
				share = Replace(Trim(share)," ",",")
			end if
			MaintainCyc = ""
			If MaintainNum>0 Then
				MaintainCyc = Formatnumber(MaintainNum,num1_dot,-1,0,0)
'If MaintainNum>0 Then
				Select Case MaintainUnit&""
				Case "1" : MaintainCyc = MaintainCyc &"小时"
				Case "2" : MaintainCyc = MaintainCyc &"天"
				Case "3" : MaintainCyc = MaintainCyc &"周"
				Case "4" : MaintainCyc = MaintainCyc &"月"
				Case "5" : MaintainCyc = MaintainCyc &"年"
				End Select
			end if
			...
			Response.write "" & vbcrlf & "             <span class=""bgfff"" id=""trpx"
			Response.write i-1
			Response.write "" & vbcrlf & "             <span class=""bgfff"" id=""trpx"
			Response.write """>" & vbcrlf & "                <table class=""bgfff"" width=""100%"" border=""0"" id=""tpx"
			Response.write i-1
			Response.write """ cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;table-layout:fixed;""><tr onmouseout=this.style.backgroundColor="""" onmouseover=this.style.backgroundColor=""ecf5ff""> " & vbcrlf & ""
			for y=0 to len_rszdy
				sorce=rs_zdy(0,y)
				zdyName = rs_zdy(1,y)
				zdyTitle = rs_zdy(2,y)
				kd = rs_zdy(4,y)
				if sorce=1 then
					...
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;"">" & vbcrlf & "           "
					Response.write kd
					If qxOpen > 0 Then
					...
					Response.write "&nbsp;"
					Response.write k
					Response.write "</a> <a href=""javascript:void(0)"" onclick=""del('trpx"
					Response.write i-1
					Response.write "</a> <a href=""javascript:void(0)"" onclick=""del('trpx"
					Response.write "',"
					Response.write id
				...
				elseif sorce=2 then
					Response.write "" & vbcrlf & "         <td class=""name"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;"">"
					Response.write kd
					Response.write order1
					Response.write "<input type=""hidden"" name=""yhlid_"
					Response.write id
					Response.write """ id=""yhlid_"
					Response.write id
					Response.write """ value="""
					Response.write yhlid
					Response.write """>" & vbcrlf & "             <input type=""hidden"" name=""kuid_"
					Response.write id
					Response.write """ id=""kuid_"
					Response.write id
					Response.write """ value="""
					Response.write kuid
					Response.write """>" & vbcrlf & "             <input type=""hidden"" name=""serialid_"
					Response.write id
					Response.write """ id=""serialid_"
					Response.write id
					Response.write """ value="""
				...
				elseif sorce=3 then
					...
					Response.write "</td>" & vbcrlf & ""
				elseif sorce=4 then
					Response.write "" & vbcrlf & "              <td class=""name"" align=""center"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;"">"
					Response.write kd
				...
				elseif sorce=5 then
					Response.write "" & vbcrlf & "     <td  class=""name"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;""><div align=""center""><input Name=""num1_"
					Response.write kd
					Response.write id
					Response.write """ maxlength=""20"" id=""num"
					Response.write id
					Response.write """ if num1=0 then onfocus=if(value==defaultValue){value='';this.style.color='#000'}  onBlur=if(!value){value=defaultValue;this.style.color='#000'} value="""
					Response.write Formatnumber(num1,num1_dot,true,0,0)
					Response.write """ onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('num"
					Response.write id
					Response.write "','"
					Response.write num1_dot
					Response.write "');"" type=""text""   style=""height: 19px; solid;font-size: 9pt;"" size=""5"" dataType=""Limit"" min=""1"" max=""100""  msg=""不能为空""> <img src='../images/116.png'  border=0 style='cursor:hand' onmouseover=""getCPYHRec("
					Response.write num1_dot
					Response.write cpord
					Response.write "); "" onmouseout=""showDIV(0,'1','yhRecs')""></div></td>" & vbcrlf & ""
				elseif sorce=6 then
					Response.write "" & vbcrlf & "     <td width="""
					Response.write kd
					Response.write """ class=""name"" align=""center"" style=""border:#CCC 1px solid;border-top:none;"">"
					Response.write kd
					Response.write ph
					Response.write "</td>" & vbcrlf & ""
				elseif sorce=7 then
				...
				elseif sorce=10 Then
					userStr = ""
					If share&""<>"" Then
						Set rs1 = conn.execute("select name from gate where del=1 and ord in("& share &")")
						While rs1.eof = False
							userStr = userStr & rs1("name")
							rs1.movenext
							If rs1.eof = False Then userStr = userStr & " "
						wend
						rs1.close
						Set rs1 = Nothing
					end if
					Response.write "" & vbcrlf & "     <td align=""center"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;""><input name=""yhCate_"
					Response.write kd
					Response.write id
					Response.write """ id=""yhCate_"
					Response.write id
					Response.write """ value="""
					Response.write userStr
					Response.write """ type=""text"" style=""height: 19px; solid;font-size: 9pt; width: 98%;"" size=""10"" readonly onclick=""chkYHForm("
'Response.write userStr
					Response.write id
					Response.write ")""><input name=""RemindPerson_"
					Response.write id
					Response.write """ id=""RemindPerson_"
					Response.write id
					Response.write """ value="""
					Response.write share
					Response.write """ type=""hidden"" dataType=""Limit"" min=""1"" msg=""请选择养护人！""></td>" & vbcrlf & ""
				elseif sorce=11 then
					Response.write "" & vbcrlf & "     <td align=""center"" class=""name"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;"">"
					Response.write kd
					Response.write MaintainCyc
					Response.write "</td>" & vbcrlf & ""
				elseif sorce=12 then
					Response.write "" & vbcrlf & "     <td align=""center"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;""><select name=""status_"
					Response.write kd
					Response.write id
					Response.write """ id=""status_"
					Response.write id
					Response.write """><option value=""1"""
					If status&""="1" Then Response.write " selected"
					Response.write ">合格</option><option value=""0"""
					If status&""="0" Then Response.write " selected"
					Response.write ">不合格</option></select></td>" & vbcrlf & ""
				elseif sorce=13 then
					Response.write "" & vbcrlf & "     <td align=""center"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;""><textarea rows=""1"" id=""result_"
'Response.write kd
					Response.write id
					Response.write """ name=""result_"
					Response.write id
					Response.write """ style=""overflow-y:hidden;word-break:break-all;width:"
'Response.write id
					Response.write kd-5
'Response.write id
					Response.write "px"" onFocus=""this.style.posHeight=this.scrollHeight"" onpropertychange=""this.style.posHeight=this.scrollHeight""  dataType=""Limit"" min=""0"" max=""200"" msg=""不要超过200个字"">"
					Response.write result
					Response.write "</textarea></td>" & vbcrlf & ""
				elseif sorce=14 then
					Response.write "" & vbcrlf & "     <td align=""center"" width="""
					Response.write kd
					Response.write """ style=""border:#CCC 1px solid;border-top:none;""><textarea rows=""1"" id=""intro_"
'Response.write kd
					Response.write id
...
'Response.write id
					Response.write kd-5
'Response.write id
					Response.write "px"" onFocus=""this.style.posHeight=this.scrollHeight"" onpropertychange=""this.style.posHeight=this.scrollHeight""  dataType=""Limit"" min=""0"" max=""500"" msg=""不要超过500个字"">"
...
'Response.write kd
				...
				elseif sorce=19 then
					...
					Response.write kd
					If zdy5&""<>"" Then
						Set rs1 = conn.execute("select sort1 from sortonehy where id="& zdy5)
				...
				elseif sorce=20 then
					...
					Response.write kd
					If zdy6&""<>"" Then
						Set rs1 = conn.execute("select sort1 from sortonehy where id="& zdy6)
						If rs1.eof = False Then
							Response.write rs1("sort1")
						end if
						rs1.close
			...
			next
			Response.write "" & vbcrlf & "             </tr></table></span>" & vbcrlf & ""
			rss.movenext
		wend
	end if
	rss.close
	...
	for j=i to num_cpmx_yl
		list_ys=list_ys+"<span class='bgfff' id='trpx"&j&"'></span>"
'for j=i to num_cpmx_yl
	next
	Response.write(""&list_ys&"")
	Response.write "" & vbcrlf & "  </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page bgfff"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""50"" colspan=""2"">&nbsp;&nbsp;<span id=""all_num""><input name=""636r"" type=""hidden"" size=""6"" value="""
	Response.write i+1
	...
	Response.write top
	Response.write """></td></tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "<div style=""position:absolute; display:none; width:320px;"" id=""yhRecs""></div>" & vbcrlf & "" & vbcrlf & ""
	action1="项目产品清单"
	call close_list(2)
	Response.write " " & vbcrlf & "" & vbcrlf & "</td>" & vbcrlf & " </tr> " & vbcrlf & "</table></form> " & vbcrlf & "</td>" & vbcrlf & "</tr></table>" & vbcrlf & "<div id=""w2"" class=""easyui-window"" title=""人员选择"" style=""width:584px;height:515px;padding:5px;background: #fafafa; display:none;""  closed=""true""  collapsible=""false"" minimizable=""false"">" & vbcrlf & "    <div region=""center"" id=""TXUser"" border=""false"" style=""width:100%; height:420px; overflow:auto; margin-top:2px;scrollbar-highlight-color:#fff; scrollbar-face-color:#f0f0ff; scrollbar-arrow-color:#c0c0e8; scrollbar-shadow-color:#d0d0e8; scrollbar-darkshadow-color:#fff; scrollbar-base-color:#ffffff; scrollbar-track-color:#fff;""></div>" & vbcrlf & "    <div region=""south"" border=""false"" style=""text-align:center;height:25px;line-height:25px; margin-top:5px;"">" & vbcrlf & "        <input type=""hidden"" name=""itemID""id=""itemID"" value="""">" & vbcrlf & "          <input type=""button"" class=""anybutton"" value=""全选"" onClick=""selectAll()"">&nbsp;" & vbcrlf & "                <input type=""button"" class=""anybutton"" value=""反选"" onClick=""selectFan()"">&nbsp;" & vbcrlf & "                <input type=""button"" class=""anybutton"" value=""确定""onClick=""setYHUser()""> "& vbcrlf & "    </div> "& vbcrlf &" </div> "& vbcrlf & vbcrlf &" </body>" & vbcrlf & "<script> "& vbcrlf & "function updateTxm(x1) { "& vbcrlf &  "     if (xmlHttp.readyState < 4) { "& vbcrlf &  "  //      cp_search.innerHTML=""loading...""; "& vbcrlf &"  } "& vbcrlf &"        if (xmlHttp.readyState == 4) {" & vbcrlf & "             var response = xmlHttp.responseText;" & vbcrlf & "            // alert(response);" & vbcrlf & "             response=response.split(""</noscript>"");" & vbcrlf & "           //alert(response[1]);" & vbcrlf & "           if (response[1]!=""""){" & vbcrlf & "                     callServer4(response[1],"""",x1);" & vbcrlf & "            }else{" & vbcrlf & "                  alert(""产品不存在"");" & vbcrlf & "              }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function nodeClick(e,node){" & vbcrlf & "   callServer4($(node).attr('nid'),$(node).attr('attr_2'),'"
	Response.write top
	Response.write "');" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function ajaxSubmit_page(sort1,pagenum,callBack,isadsearch){" & vbcrlf & "     $('#productTree').hide();" & vbcrlf & "       $('#cp_search').show();" & vbcrlf & " //获取用户输入" & vbcrlf & "  var B=document.forms[0].B.value;" & vbcrlf & "        var C=(document.forms[0].C.value==$(""#txtKeywords"").get(0).defaultValue?"""":document.forms[0].C.value);" & vbcrlf & "   var top=document.forms[0].top.value;" & vbcrlf & "    var url = ""search_cp.asp?P=""+pagenum+""&B=""+escape(B)+""&C=""+escape(C) +""&top=""+escape(top) +""&cstore="
	'Response.write top
	Response.write abs(ShowOnlyCanStoreProduct)
	Response.write "&sort1=""+escape(sort1) + ""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "     if(parent.searchModel && parent.searchModel==1){" & vbcrlf & "                //处理高级搜索" & vbcrlf & "          var ifcobj=document.getElementById(""adsIF"").contentWindow.document;" & vbcrlf &"         var sobj=ifcobj.getElementsByTagName(""input"");" & vbcrlf & "            var txValue="""";" & vbcrlf & "           for(var i=0;i<sobj.length;i++){" & vbcrlf & "                 if(sobj[i].getAttribute(""sk"")&&sobj[i].type=='text'&&sobj[i].value!=''){" & vbcrlf & "                          txValue+=(txValue==""""?"""":""&"")+sobj[i].getAttribute(""sk"")+""=""+escape(sobj[i].value);" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "               sobj=ifcobj.getElementsByTagName(""select"");" & vbcrlf & "               for(var i=0;i<sobj.length;i++){" & vbcrlf & "                 if(sobj[i].getAttribute(""sk"")&&sobj[i].value!=''){" & vbcrlf & "                                txValue+=(txValue==""""?"""":""&"")+sobj[i].getAttribute(""sk"")+""=""+escape(sobj[i].value);" & vbcrlf & "                   }" & vbcrlf & "               }" & vbcrlf & "               sobj=ifcobj.getElementsByName(""A2"");" & vbcrlf & "              var tmp="""";" & vbcrlf & "               for(var i=0;i<sobj.length;i++){" & vbcrlf & "                 if(sobj[i].checked){" & vbcrlf & "                            tmp+=(tmp==""""?"""":"","")+escape(sobj[i].value);" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               txValue+=(tmp==""""?"""":(txValue==""""?"""":""&"")+""A2=""+tmp)" & vbcrlf & "                url=""search_cp.asp?ads=1""+(txValue==""""?"""":""&"")+txValue+""&P=""+pagenum+""&top=""+escape(top) +""&cstore="
	Response.write abs(ShowOnlyCanStoreProduct)
	Response.write "&sort1=""+escape(sort1) + ""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "     }" & vbcrlf & "       xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "                updatePage_cp(callBack);" & vbcrlf & "        };" & vbcrlf & "    xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function updatePage_cp(callBack) {" & vbcrlf & "        if (xmlHttp.readyState <script 4) {" & vbcrlf & "           cp_search.innerHTML=""loading..."";" & vbcrlf & " }" & vbcrlf & "       if (xmlHttp.readyState == 4) {" & vbcrlf & "          var response = xmlHttp.responseText;" & vbcrlf & "              cp_search.innerHTML=response;" & vbcrlf & "           if(callBack){" & vbcrlf & "                   callBack.call(this.arguments);" & vbcrlf & "          }" & vbcrlf & "               xmlHttp.abort();" & vbcrlf & "        }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function ajaxSubmit(sort1,callBack){" & vbcrlf & "  //获取用户输入" & vbcrlf & "        var B=document.forms[0].B.value;" & vbcrlf & "        var C=document.forms[0].C.value;" & vbcrlf & "        var top=document.forms[0].top.value;" & vbcrlf & "    var url = ""search_cp.asp?B=""+escape(B)+""&C=""+escape(C) +""&top=""+escape(top) +""&sort1=""+escape(sort1) + ""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100)" & vbcrlf & "        ;" & vbcrlf & "       xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "                updatePage_cp(callBack);" & vbcrlf & "        };" & vbcrlf & "      xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</html>" & vbcrlf & ""
%>
