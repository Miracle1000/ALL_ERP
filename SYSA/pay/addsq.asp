...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	ord=request("ord")
	if ord="" then
		conn.execute "delete from paysq where del=7 and addcateid=" & session("personzbintel2007")
		conn.execute "delete from paysqlist where del=7 and addcateid=" & session("personzbintel2007")
		conn.execute "insert into paysq(addcateid,adddate,del) values("&session("personzbintel2007")&",'"&now&"',7)"
		ord = GetIdentity("paysq","id","addcateid","")
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "window.paysqOrd = """
	Response.write ord
	Response.write """;" & vbcrlf & "</script>" & vbcrlf & "<script src= ""../Script/py_addsq.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=javascript></script>" & vbcrlf & "<script type=""text/javascript"" src=""../inc/jquery.easyui.min.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".as {width:60px;" & vbcrlf & "height:20px;}" & vbcrlf & "tr{height:27px;}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body onunload=""if(window.opener){window.opener.location.reload();}"" oncontextmenu=self.event.returnValue=false  onMouseOver=""window.status='none';return true;""  scroll=AUTO>" & vbcrlf & ""
...
'If Len(exists_Pics) = 0 Then
	...
	function ClearLostedFiles(modultype)
		if modultype="contract" or modultype="caigou" then
			if modultype="contract" then
				field1="addcate"
				field2="addcate"
				field3="addcate"
			else
			...
			sql="select intro from "&modultype&" where "&field1&"="&session("personzbintel2007")&" and del=7"
			set rsdel=conn.execute(sql)
			while not rsdel.eof
				intro=rsdel(0)
				picUrl = getPicUrl(intro)
				if picUrl<>"" then
					picUrlArray1 = split(picUrl,",")
					for x = 0 to ubound(picUrlArray1)
						DeleteFile("../edit/upimages/"&picUrlArray1(x))
					next
				end if
				rsdel.movenext
			wend
			rsdel.close
			sql="select intro2 from "&modultype&"bz where "&field2&"="&session("personzbintel2007")&" and del=7"
			set rsdel=conn.execute(sql)
			while not rsdel.eof
				intro2=rsdel(0)
			...
			wend
			rsdel.close
			sql="select intro from "&modultype&"list where "&field3&"="&session("personzbintel2007")&" and del=7"
			set rsdel=conn.execute(sql)
			while not rsdel.eof
				intro=rsdel(0)
				picUrl = getPicUrl(intro)
	...
	Response.write "" & vbcrlf & "<form method=""POST"" action=""savesq.asp?ord="
	Response.write ord
	Response.write """ id=""demo"" onsubmit=""return Validator.Validate(this,2)&DelUnusedFilesBeforeSubmit();"" name=""date"" target=""savedata"">" & vbcrlf & "<table width=""100%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td class=""place"">申请添加</td>" & vbcrlf & "        <td>&nbsp;</td>" & vbcrlf & "        <td align=""right""><div align=""right"">" & vbcrlf & "" & vbcrlf & "        </div>" & vbcrlf & "        </a></div></td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "  </table>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "   <tr class=""top resetGroupTableBg"">" & vbcrlf & "                <td colspan=""3"">基本信息</td>" & vbcrlf & "             <td> <div align=""right"">" & vbcrlf & "                    <input name=""Submit"" type=""submit"" class=""anniu"" value=""保存"" onClick=""return ask();"">" & vbcrlf & "                        <input name=""Submit4"" type=""reset"" class=""anniu"" value=""重置"">" & vbcrlf & "              </div></td>" & vbcrlf & "     </tr>" & vbcrlf & "               <tr>" & vbcrlf & "              <td width=""15%"" height=""35""><div align=""right"">申请主题：</div></td>" & vbcrlf & "                  <td width=""35%"" ><div align=""left"">" & vbcrlf & "              <input name=""title""  type=""text"" id=""title"" dataType=""Limit"" min=""1"" max=""200"" msg=""请输入1至200个字符"">" & vbcrlf & "            <font id=""tit"" style=""color:red"">*</font></div></td>" & vbcrlf & "                      <td width=""15%""><div align=""right"">" & vbcrlf & "                   <div align=""right"">申请总额：</div>" & vbcrlf & "                 </div></td>" & vbcrlf & "                     <td width=""35%""><div align=""left""> <input name=""allpay"" type=""text"" id=""allpay"" size=""15"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('allpay','"
	Response.write num_dot_xs
	Response.write "')""></div></td>" & vbcrlf & "            </tr>" & vbcrlf & "             <tr>" & vbcrlf & "              <td width=""15%"" height=""35""><div align=""right"">申请日期：</div></td>" & vbcrlf & "                  <td width=""35%"" style=""padding-top:0!important;padding-bottom:0!important""><div align=""left"">" & vbcrlf & "             <TABLE border=0 align=""left"" >" & vbcrlf & "                  <TBODY>" & vbcrlf & "                    <TR>" & vbcrlf & "                      <TD><INPUT name=ret readonly onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" value="""
	Response.write date
	Response.write """ size=10  dataType=""Limit"" min=""1"" max=""200"" msg=""请选择日期"">" & vbcrlf & "                          <input type=""button"" style=""margin-bottom:-0.5px"" class=""anybutton"" value=""选择日期"" align=""absMiddle""  border=""0""  id=""daysOfMonthPos"" name=""daysOfMonthPos""  onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" />" & vbcrlf & "                          <span class=""red"">*</span>" & vbcrlf & "                          <DIV id=daysOfMonth style=""POSITION: absolute""></DIV></TD>" & vbcrlf & "                    </TR>" & vbcrlf & "                  </TBODY>" & vbcrlf& "                </TABLE>" & vbcrlf & "                </div></td>" & vbcrlf & "                 <td width=""15%""><div align=""right"">总张数：</div></td>" & vbcrlf & "              <td width=""35%""><div align=""left""><span class=""gray"">" & vbcrlf & "            <input name=""allnum"" style=""color:#999999"" type=""text"" id=""allnum"" value=""0"" size=""4"" maxlength=""3"" readonly>" & vbcrlf & "            </span></div></td>" & vbcrlf & "                </tr>" & vbcrlf & "           "
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	set rs8=nothing
	Response.write "" & vbcrlf & "               <tr>" & vbcrlf & "          <td height=""35""><div align=""right"">是否借款：</div></td>" & vbcrlf & "            <td><div align=""left"">是" & vbcrlf & "            <input type=""radio"" name=""rejk"" value=""1"">" & vbcrlf & "            否" & vbcrlf & "            <input name=""rejk"" type=""radio"" value=""0"" checked>" & vbcrlf & "            </div></td>" & vbcrlf & "                 <td><div align=""right"">"
	if bz<>0 then
		Response.write "币种："
	end if
	...
	Response.write "" & vbcrlf & "                 </div></td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "          <td height=""35""><div align=""right"">申请概要：</div></td>" & vbcrlf & "            <td><div align=""left""><input name=""intro"" type=""text"" id=""intro"" size=""30"" maxlength=""200""></td>" & vbcrlf & "<td><div align=""right"">天数：</div></td>" & vbcrlf & "              <td><div align=""left"">" & vbcrlf & "              <input name=""days"" type=""text"" id=""days"" onpropertychange=""formatData(this,'int');"" onKeyUp=""checkDot('days','0')"" size=""4"" maxlength=""4"">" & vbcrlf & "          </div></td>"& vbcrlf & "                </tr>" & vbcrlf & "           <tr class=""top"">" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
	...
	Response.write "" & vbcrlf & "                <td width=""15%"" colspan=""4""><div align=""left""><font class=""left""><a href=""###"" style=""text-decoration:none"" onClick=""" & vbcrlf & "                                                         if(document.getElementById('tdr').style.display=='none')" & vbcrlf & "                                                                {document.getElementById('tdr').style.display='';" & vbcrlf & "                                                                document.getElementById('nodsr').src='../images/12.gif'}" & vbcrlf & "                                                                else" & vbcrlf & "                                                            {" & vbcrlf & "                                                               document.getElementById('tdr').style.display='none';" & vbcrlf & "                                                            document.getElementById('nodsr').src='../images/11.gif';" & vbcrlf & "                        }""><img border=""0"" id=""nodsr"" src=""../images/11.gif"">&nbsp;<strong>申请人员</strong></a></font>&nbsp;&nbsp;<font id=""sqr"">" & vbcrlf & "                                                           </font>&nbsp;<font id=""ry"" color=""red"">*</font></div></td>" & vbcrlf & "           </tr>" & vbcrlf & "            <tr id=""tdr"" style=""display:none"">" & vbcrlf & "              <td width=""15%"" colspan=""4"">" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                      <tr>" & vbcrlf & "                <td colspan=""4"">" & vbcrlf & "                           "
	basesql="select ord, orgsid from gate where 1=1 "&str_w3
	Response.write CBaseUserTreeHtmlRadioCE(basesql,"", "","","member2",  "", "", "", gateord,"Tiggerfirst:if(node.value){chanceR(node.text)}")
	Response.write "</td>" & vbcrlf & "                          </table>" & vbcrlf & "                      </td>" & vbcrlf & "            </tr>" & vbcrlf & "          <tr>" & vbcrlf & "              <td  colspan=""4"" style=""padding-left:0px;"">" & vbcrlf & "                       <table width=""100%"" border=""0"" cellpadding=""5"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" class=""defaultSort"">" & vbcrlf & "               "
	CurrPage=clng(Request("CurrPage"))
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from sortonehy where gate2=41 and del=1 and isnull(id1,0)=0 order by gate1 desc"
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		Response.write "<tr><td align='center'>还没有费用分类！请先设置费用分类！</td></tr>"
	else
		i=0
		h=0
		do until rs.eof
			Response.write "" & vbcrlf & "                        <tr valign=""center"" class=""top resetGroupTableBg"">" & vbcrlf & "                                <td height=""30"" align=""left"" valign=""middle"" style=""line-height:64px;""><a href=""###"" style=""text-decoration:none"" onClick=""" & vbcrlf & "                                                             if(document.getElementById('td"
			h=0
			...
			Response.write "</strong></a></td>" & vbcrlf & "                </tr>" & vbcrlf & "                         <tr valign=""center"" id=""td"
			Response.write rs("ord")
			Response.write """ style=""display:none;"">" & vbcrlf & "                 <td colspan=""2"" style=""padding-left:30px;"">" & vbcrlf & "                                    "
			Response.write rs("ord")
			dim m
			m=0
			set rs1=server.CreateObject("adodb.recordset")
			...
			if not rs1.eof then
				do until rs1.eof
					...
					loop
					set rszdy=nothing
					Response.write "" & vbcrlf & "                     <table width=""100%"" border=""1"" cellpadding=""0"" cellspacing=""1"" bgcolor=""#C0CCDD"" style=""border-color:#CCC"">" & vbcrlf & "                                                 <tr class=""resetGroupTableBg tableHeadBg"" style=""cursor:pointer;"" onClick=""downtype('"
					set rszdy=nothing
					Response.write rs1("id")
					Response.write "','"
					Response.write zdarr
					Response.write "')"">" & vbcrlf & "                                                        <td height=""25"" width=""8%"" align=""left""><div align=""left"">&nbsp;<strong>"
					Response.write rs1("sort1")
					Response.write "</strong>&nbsp;<img class=""resetElementHidden"" style=""border:0px;"" src=""../../SYSN/skin/default/img/arrow-down.png"" alt=""添加费用记录"" width=""9"" height=""6""><img class=""resetElementShowNoAlign"" style=""border:0px;display:none;"" src=""../skin/default/images/MoZihometop/content/r_down2.png"" alt=""添加费用记录"" width=""9"" height=""6""></div></td>" & vbcrlf & "                                                                <td height=""25"" colspan="""
					Response.write rn1+1
					Response.write """></td>" & vbcrlf & "                                                   </tr>" & vbcrlf & "                                                   <tr style=""display:none"" id=""tid"
					Response.write rs1("id")
	...
	Response.write "" & vbcrlf & "             </table>                  </td>" & vbcrlf & "           </tr>" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""10%"" align=""right"" height=""27"">备注：</td>" & vbcrlf & "                 <td colspan=""6"">" & vbcrlf & "                  <span class=""gray"">" & vbcrlf & "                               <textarea name=""remark"" style=""display:none"" cols=""1"" rows=""1"">"
	Response.write remark
	Response.write "</textarea>" & vbcrlf & "                          <iframe id=""eWebEditor1"" src=""../edit/ewebeditor.asp?id=remark&style=news"" frameborder=""0"" scrolling=""no"" width=""100%"" height=""300"" marginwidth=""1"" marginheight=""1"" name=""remark"" class=""g-textarea""></iframe>" & vbcrlf & "                     </span>" & vbcrlf & "</td>" & vbcrlf & "           </tr>" & vbcrlf & "" & vbcrlf & "      <tr>" & vbcrlf & "        <td height=""50"" colspan=""6"">" & vbcrlf & "           <div align=""center"">" & vbcrlf & "                <input name=""Submit"" type=""submit"" class=""anniu"" value=""保存"" onClick=""return ask();"">" & vbcrlf & "                    <input name=""Submit4"" type=""reset"" class=""anniu"" value=""重置"">" & vbcrlf & "             </div>                  </td>" & vbcrlf & "        </tr>" & vbcrlf & "    </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=800></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "   </td>" & vbcrlf & "    "
	action1="申请添加"
	call close_list(1)
	Response.write "" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "<div style=""position:absolute;display:none;width:500px;height:320px;border:0px;padding:1px;z-index:9999;""id=""dhtml""></div>" & vbcrlf & "<iframe id=""savedata"" name=""savedata"" style=""display:none;height:0px;width:0px""></frame>" & vbcrlf & "<script language='javascript'>" & vbcrlf & "var slbl=document.getElementById(""member2_"
