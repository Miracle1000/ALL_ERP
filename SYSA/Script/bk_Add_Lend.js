...
function selectBook(x,sel,c,page)
{
	...
	var pid="";
	$("input[name='Ld_bkid']").each(function(){		
		if ($(this).val()!=""){
			pid += (pid==""?"":",");
			pid += $(this).val();
		}
...
function setOpenr(id,bk_name,bk_bh,set_name,bk_returnnum,bk_auther,bk_pubtime,bk_publishing,bk_pubnum,bk_pagenum,bk_pagesize,bk_printtime,bk_format,bk_paper,bk_printnum,bk_isbn,bk_binding,bk_price,bk_num,bk_allmoney,bk_postion,name,bk_addtime,pk)
{
	...
	var ele=document.getElementsByName('Ld_time'+pk)[0];
	var v=ele.value;
	time1=new Date(v.replace(/-/g,"/"));
	var y=time1.getFullYear();
	var m=time1.getMonth()+1;
	var d=time1.getDate();
	ele.value=y+"-"+(m<10?("0"+m):m)+"-"+(d<10?("0"+d):d);
	var newtime=computDate(y,m+1,d-1);
	document.getElementsByName('Ld_rettime'+pk)[0].value=newtime;
	selectBook(b,'','',1)
	$('#ddd').dialog('open');
}
function computDate(y,m,d){
   if(m>12){m=1;y=y+1}
   var daies=[31,y%4?28:29,31,30,31,30,31,31,30,31,30,31];
   if(d>daies[m-1]){d=daies[m-1];}
   if(d<1){d=1;m=m-1}
   m=m<10?("0"+m):m;
   d=d<10?("0"+d):d;
   return y+"-"+m+"-"+d
}
function extdiv(obj,args)
{
	...
	var ord=document.getElementById("Ld_bkid"+args).value;
	sdivobj.style.display="none";
	sdivobj.style.left=tleft+"px";
	sdivobj.style.top=ttop+theight+"px";
	document.getElementById("ssIFForm").action="../book/ShowMessage.asp?ord="+ord;
