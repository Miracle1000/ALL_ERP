...
function KeyNameFun(lvw, rowindex, cellindex) {
    var cpindex = -1;
    var nindex = -1;
    for (var k = 0; k < lvw.headers.length; k++) {
        if (lvw.headers[k].dbname == "KeyMean") { cpindex = k; }
    ...
    var dateindex = -1;
    for (var l = 0; l < lvw.rows.length; l++) {
        if (lvw.rows[l][nindex] == "日期编码：" || lvw.rows[l][nindex] == "日期编码") {
            dateindex = l;
        }
        if (lvw.rows[l][nindex].indexOf("：") < 0) {
            var name = lvw.rows[l][nindex];
            name = name + "：";
            lvw.rows[l][nindex] = name;
        }
    }

    var isReamerZ = 2, isReamerF = 2;
    for (var j = 0; j < lvw.rows.length; j++) {
        if (j > 0 && j < dateindex) {
            lvw.rows[j][nindex] = '固定标识' + isReamerZ + "：";
            isReamerZ += 1;
        }
        if (j > dateindex + 1) {
            lvw.rows[j][nindex] = '顺序编码' + isReamerF + "：";
            isReamerF += 1;
        }
    }

    return lvw.rows[rowindex][nindex];
...
$(document).ready(function () {
    if (app.IeVer == "8") {
        var elements = $("#lvw_dbtable_RuleLvw col[dbname*='@indexcol']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }
        var elements = $("#lvw_dbtable_RuleLvw col[dbname*='KeyMean']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }

        var elements = $("#lvw_dbtable_RuleLvw col[dbname*='NO']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }
    }
    if (app.IeVer == "9" || app.IeVer == "8") {
        var elements = $("#lvw_dbtable_RuleLvw td[dbname*='@indexcol']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }
        var elements = $("#lvw_dbtable_RuleLvw td[dbname*='KeyMean']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }

        var elements = $("#lvw_dbtable_RuleLvw td[dbname*='NO']");
        for (var i = 0; i < elements.length; i++) {
            var e = elements[i];
            $(e).css("display", "none");
        }
    ...
    var btn = $ID("CreateBtn").parentNode.style.textAlign = "center";
    window.__lvw_addEvent(function () {
        if (app.IeVer == "8") {
            var elements = $("#lvw_dbtable_RuleLvw col[dbname*='@indexcol']");
            for (var i = 0; i < elements.length; i++) {
                var e = elements[i];
