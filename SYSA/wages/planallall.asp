...
'dim sqlstr
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  "
	if open_10_8=0 then
...
'if m1="" then
	...
	if m2="" then
		m2=year(date())&"-"&month(date())&"-1"
'if m2="" then
	end if
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "            <td class=""place"">员工工资查询</td>" & vbcrlf & "             <form method=""get"" action=""planallall.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "             <td align=""left"" class=""top_btns"">选择日期：" & vbcrlf & ""
	Response.write "<script language=""JavaScript"" src=""../hrm/js/DatePicker.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript""></script>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "        <!--" & vbcrlf & "    function HrformAction(val)" & vbcrlf & "      {" & vbcrlf & "        return true;" & vbcrlf & "    //gotourl(""jtdate=""+val+""-1"");" & vbcrlf & "     }" & vbcrlf & "       -->" & vbcrlf & "</script>"& vbcrlf
...
'& vbcrlf
	if m1="" then
		m1=year(currdate)&"-"&month(currdate)&"-1"
'if m1="" then
	else
		if InstrRev(m1,"-")<7 then
'if m1="" then
			m1=m1&"-1"
'if m1="" then
		end if
	...
	if m2="" then
		m2 = dateadd("d",-1,dateadd("m",1,CDate(year(currdate)&"-"&month(currdate)&"-1")))
'if m2="" then
		m2 = Replace(m2 & "", "/" , "-")
'if m2="" then
	else
		if InstrRev(m2,"-")<7 Then
'if m2="" then
			m2=dateadd("d",-1,dateadd("m",1,CDate(m2 & "-1")))
'if m2="" then
		end if
	end if
	if CDate(m1)>CDate(m2) then m1=m2
	m1=Replace(m1 & "", "/", "-")
'if CDate(m1)>CDate(m2) then m1=m2
	m2=Replace(m2 & "", "/", "-")
'if CDate(m1)>CDate(m2) then m1=m2
	m1_s = year(CDate(m1)) & "-" & Right("0" & month(CDate(m1)),2)
'if CDate(m1)>CDate(m2) then m1=m2
	m2_s = year(CDate(m2)) & "-" & Right("0" & month(CDate(m2)),2)
'if CDate(m1)>CDate(m2) then m1=m2
	Response.write "" & vbcrlf & "自:<input type=""text"" readonly=""true"" value="""
	Response.write m1_s
	Response.write """ align=""absMiddle"" size=""9"" onClick=""setmonth(this)""  border=""0""  id=""ret"" name=""ret""/>" & vbcrlf & "至:<input type=""text"" readonly=""true"" value="""
	Response.write m2_s
	Response.write """ align=""absMiddle"" size=""9"" onClick=""setmonth(this)""  border=""0""  id=""ret2"" name=""ret2""/>" & vbcrlf & ""
	
	Response.write "" & vbcrlf & "                <input type=""submit"" name=""Submit422"" value=""查询""  class=""anybutton2""/>" & vbcrlf & "                              <input type=""hidden"" name=""W1"" value="""
	Response.write W1
	Response.write """>" & vbcrlf & "                         <input type=""hidden"" name=""W2"" value="""
	Response.write W2
	Response.write """>" & vbcrlf & "                         <input type=""hidden"" name=""W3"" value="""
	Response.write W3
	Response.write """>" & vbcrlf & "                         <input type=""hidden"" name=""CurrPage"" value="""
	Response.write Request("CurrPage")
	Response.write """>" & vbcrlf & "                         <input type=""hidden"" name=""person_type"" value="""
	Response.write person_type
	Response.write """>" & vbcrlf & "             </td>" & vbcrlf & "             </form>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "     <tr class=""top resetGroupTableBg"">" & vbcrlf & "          <td colspan=""2"" align=""center""><div align=""center""><strong>员工编号</strong></div></td>" & vbcrlf & "       <td align=""center""><div align=""center"">姓名</div></td>" & vbcrlf & "    <td align=""center""><div align=""center""><strong>应发总额</strong></div></td>" & vbcrlf & "         <td width=""13%"" height=""27"" align=""center""><div align=""center"">应扣总额</div></td>" & vbcrlf & "      <td width=""15%"" ><div align=""center""><strong>实发总额</strong></div></td>" & vbcrlf &         "<td width=""15%"" ><div align=""center"">所在部门</div></td>" & vbcrlf &     "<td width=""15%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf &        "</tr>" & vbcrlf
	person_Result="where 1=1 "&list&""
	person_Result1=""
	if W3<>"" then
		person_Result=person_Result+" and  ord in ("&W3&")"
'if W3<>"" then
	end if
	if W2<>"" then
		person_Result1=person_Result1+" and  cateid3 in ("&W2&")"
'if W2<>"" then
	end if
	if W1<>"" then
		person_Result1=person_Result1+" and  cateid2 in ("&W1&")"
'if W1<>"" then
	end if
	Str_Result2=""
	if m1<>"" then
...
'if m1<>"" then
	...
	if m2<>"" then
		Str_Result2=Str_Result2+"and date2<='"&m2&"' "
'if m2<>"" then
	end if
	person_type=request("person_type")
	if ZBRuntime.MC(39000) Then
		If person_type="" Then person_type="0"
		If InStr(person_type,"0")=0 Then  person_Result1=person_Result1 &" and cateid in (select userid from hr_person where nowstatus in (2,4) )"
		If InStr(person_type,"2")=0 Then  person_Result1=person_Result1 &" and cateid not in (select userid from hr_person where nowstatus=2 )"
		If InStr(person_type,"4")=0 Then  person_Result1=person_Result1 &" and cateid not in (select userid from hr_person where nowstatus=4 )"
	else
		If person_type="" Then person_type="1"
		If InStr(person_type,"1")=0 Then  person_Result1=person_Result1 &" and cateid in (select ord from gate where del in (2,5) )"
		If InStr(person_type,"3")=0 Then  person_Result1=person_Result1 &" and cateid not in (select ord from gate where del=2 )"
		If InStr(person_type,"5")=0 Then  person_Result1=person_Result1 &" and cateid not in (select ord from gate where del=5 )"
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select ord,name,sorce,sorce2,ygid from gate_person WITH(NOLOCK) "&person_Result&" and (ord in (select distinct cateid from wageslist WITH(NOLOCK) where del=1 and wages in (select id from wages WITH(NOLOCK) where del=1 and complete1=1 and isnull(salaryClass,0)=0 "&Str_Result2&") "&person_Result1 &" ) or ord in (select distinct cateid from wageslist_jj WITH(NOLOCK) where del=1 and wages2 in (select id from wages WITH(NOLOCK) where del=1 and complete1=1 and isnull(salaryClass,0)=0 "&Str_Result2&") "&person_Result1 &" ) ) order by sorce asc,sorce2 asc ,cateid asc"
	rs1.open sql1,conn,1,1
	if rs1.RecordCount<=0 then
		Response.write "<tr><td  colspan='9'>没有信息!</td></tr>"
	else
		n=0
		rs1.PageSize=15
		...
		do until rs1.eof
			...
			money_jj=0
			set rs=server.CreateObject("adodb.recordset")
			sql="select money1,sort1,wages,isNull(sortName,'删除') as sortName,isNull(updown,0) as updown from wageslist where del=1 and cateid="&rs1("ord")&" and wages in (select id from wages where complete1=1 and del=1 and isnull(salaryClass,0)=0 "&Str_Result2&") "&person_Result1 &" and money1<>0 order by date7 desc"
			rs.open sql,conn,1,1
			if rs.eof then
				money1=0
				money1=0
				...
				do until rs.eof
					...
					wagesid =rs("wages")
					if rs("updown")=1 then
						money1=cdbl(money1)+cdbl(rs("money1"))*cdbl(gethl(rs("wages"),"wages"))
'if rs("updown")=1 then
					elseif rs("updown")=-1 then
'if rs("updown")=1 then
						money2=cdbl(money2)+cdbl(rs("money1"))*cdbl(gethl(rs("wages"),"wages"))
'if rs("updown")=1 then
					end if
					rs.movenext
				loop
			...
			rs.close
			sql="select id from wages where complete1=1 and del=1 and isnull(salaryClass,0)=0 "&Str_Result2&""
			rs.open sql,conn,1,1
			money_jj=0
			do until rs.eof
				wagesid =rs("id")
				If jjgz=0 Then
					...
					sql="select id from wages_jj where wages2="&wagesid&" "
				...
				if ZBRuntime.MC(26002) and jjgz=1 Then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select  isnull(sum(money1),0) as money1  from wageslist_jj where wages2="&wagesid&"  and cateid="&rs1("ord")&" "
					rs7.open sql7,conn,1,1
					If Not rs7.eof Then
						money_jj=money_jj+zbcdbl(rs7("money1"))
'If Not rs7.eof Then
					end if
					rs7.close
					set rs7=Nothing
			...
			rs.close
			set rs=Nothing
			money1=cdbl(money1)+cdbl(money_jj)
			set rs=Nothing
			money3=cdbl(money1)-cdbl(money2)
			set rs=Nothing
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td colspan=""2"" align=""center"" class=""name""><span class=""name"">"
			Response.write rs1("ygid")
			Response.write "" & vbcrlf & "       </span></td>" & vbcrlf & "                <td width=""15%"" align=""center"" class=""name"">"
			Response.write rs1("name")
			Response.write "</td>" & vbcrlf & "                <td width=""15%"" align=""center"" class=""name""><div align=""right"">"
			Response.write Formatnumber(money1,num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "                <td height=""27"" class=""name""  align=""center""><div align=""right"">" & vbcrlf & "                  <div align=""right"">"
			Response.write Formatnumber(money2,num_dot_xs,-1)
			Response.write "</div>" & vbcrlf & "                </div></td>" & vbcrlf & "                <td  class=""name""><div align=""center"">" & vbcrlf & "                  <div align=""right""><font class=""red"">"
			Response.write Formatnumber(money3,num_dot_xs,-1)
			Response.write "</font></div>" & vbcrlf & "" & vbcrlf & "                </div></td>" & vbcrlf & "        "
			sorce1=""
			sorce2=""
			if rs1("sorce")<>0 and rs1("sorce")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1 from gate1 where ord="&rs1("sorce")&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
					sorce1=""
				else
			...
			if rs1("sorce2")<>0 and rs1("sorce2")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort2 from gate2 where ord="&rs1("sorce2")&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
					sorce2=""
				else
			...
			Response.write sorce1
			Response.write "&nbsp;"
			Response.write sorce2
			Response.write "</div></td>" & vbcrlf & "        <td><div align=""center"">" & vbcrlf & "          "
			if open_10_14=3 or CheckPurview(intro_10_14,trim(cateid))=True then
				Response.write "<input type=""button"" name=""Submit32"" value=""明细查询""  onClick=""javascript:window.open('planall2.asp?ord="
				...
				Response.write "&w3="
				Response.write w3
				Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""  class=""anybutton"">"
				Response.write w3
			end if
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "                 <input id=""curp1"" name=""currpage""  type=text  onkeyup=""value=value.replace(/[^\d]/g,'')""  size=3  maxlength=8/>&nbsp;" & vbcrlf & "                        <input type=""button"" name=""Submit422"" value=""跳转"" onClick=""window.location.href='planallall.asp?currPage='+document.getElementById('curp1').value+'&a="
		Response.write rs1.pagesize
		Response.write a
		Response.write "&b="
		Response.write b
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='planallall.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&person_type="
			Response.write person_type
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='planallall.asp?currPage="
		...
		if currpage=rs1.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='planallall.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&person_type="
			Response.write person_type
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='planallall.asp?currPage="
			Response.write  rs1.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
