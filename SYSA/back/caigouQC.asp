...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "form{padding:0px; margin:0px;}" & vbcrlf & "</style>" & vbcrlf & "<script language='javascript' src='AutoHiddeFunBtn.js?ver="
	Response.write Application("sys.info.jsver")
...
'select case B
	...
	Response.write "" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "      <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td class=""place"">质检回收站</td>" & vbcrlf & "        <td>&nbsp;</td>" & vbcrlf & "        <td align=""right"" class='ser_btn2'>" & vbcrlf & "        <form method=""get"" action=""caigouQC.asp"">" & vbcrlf &"        <select name=""B"">" & vbcrlf & "          <option value=""zjzt"" "
	if B="zjzt" then
	...
	Response.write ">质检编号</option>" & vbcrlf & "          <option value=""cpmc"" "
	if B="cpmc" then
		Response.write "selected"
	end if
	...
	Response.write C
	Response.write """/>" & vbcrlf & "        <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "        <input type=""button"" name=""Submitdel"" class=""anybutton"" style=""margin-left:20px;"" value=""全部清空"" onClick=""if(confirm('确认要全部清空吗？')){window.location.href='delall.asp?ord=caigouQC&url=caigouQC.asp&list1=caigouQClist&B="
	Response.write B
	Response.write "&C="
	Response.write C
	Response.write "'}""/>" & vbcrlf & "        </form></td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "    </table>" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                       <tr class=""top"">" & vbcrlf & "                    <td width=""5%"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "                    <td width=""5%"" align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "                        <td width=""35%"" height=""27""><div align=""center"">质检主题</div></td>" & vbcrlf & "                        <td width=""12%""><div align=""center""><strong>质检人员</strong></div></td>" & vbcrlf & "                    <td width=""12%""><div align=""center""><strong>删除人</strong></div> </td>" & vbcrlf & "                     <td width=""15%""><div align=""center"">删除日期</div></td>" & vbcrlf & "                    <td width=""16%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "              </tr>" & vbcrlf & ""
	Set rssort=conn.execute("select intro from setopen  where sort1=49")
	If rssort.eof Then
		sort49=1
	else
		sort49=rssort("intro")
	...
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from caigouQC where del=2 "& sql2 &" order by deldate desc"
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "<form name=""form1"" method=""post"" action=""deletecgqcall.asp?ord="
		...
		do until rs.eof
			...
			Response.write """></span></td>" & vbcrlf & "    <td align=""center"" height=""24"">"
			Response.write "</td>" & vbcrlf & "    <td height=""27"" class=""name"" >&nbsp;<a href=""#"" onclick=""javascript:window.open('../caigouQC/content.asp?ord="
			...
			if rs("Inspector")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select name from gate where ord="&rs("Inspector")&""
			...
			ishuifu=false
			If sort49<>0 Then
				sql1 =  "select top 1 1 from caigouQC a " & vbcrlf & _
				"inner join caigoulist b on a.id = " & id & " and a.caigoulist = b.id and b.num1 >= a.recnum  " & vbcrlf & _
				"where not exists( " & vbcrlf & _
				"  select  1 from caigouQC c  " & vbcrlf & _
				"  where c.caigoulist = b.id and  " & vbcrlf & _
				"  c.id <> a.id and c.del =1) " & vbcrlf & _
				"and not exists(select 1 from kuinlist where sort=1 and del=1 and caigoulist=b.id)"
				ishuifu = conn.execute(sql1).eof
			end if
			If ishuifu Then
				isfh= " disabled"
			else
			...
			If IsBillRecovery(5,id) = true And isfh="" Then isfh=" disabled"
			Response.write "" & vbcrlf & "     <input type=""button"" name=""Submit3c""  class=""anybutton""  value=""恢复""  "
			Response.write isfh
			Response.write " onClick=""if(confirm('确认恢复？')){window.location.href='setcgqc.asp?ord="
			Response.write id
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>&nbsp;&nbsp;" & vbcrlf & " <input type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletecgqc.asp?ord="
			Response.write id
			Response.write "&CurrPage="
			Response.write CurrPage
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "    <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='caigouQC.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='caigouQC.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "   <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='caigouQC.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='caigouQC.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "<script type=""text/Javascript"">window.currask2Url=""setcgqcall.asp?currPage="
		Response.write currPage
