...
'Dim sql
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      margin-top: 0px;" & vbcrlf & "        background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "form{padding:0; margin:0;}" & vbcrlf & "#content .t-center td{text-align:center; height:36px;}" & vbcrlf & ".dateBox{display:inline-block;}" & vbcrlf & ".dateBox input{text-align:center;}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf& ""
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=45 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_45_1 = 0
		intro_45_1 = 0
	else
		open_45_1    = rs1("qx_open")
		intro_45_1   = rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=46 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_46_11 = 0
		intro_46_11 = 0
	else
		open_46_11   = rs1("qx_open")
		intro_46_11  = rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=45 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_45_11 = 0
		intro_45_11 = 0
	else
		open_45_11   = rs1("qx_open")
		intro_45_11  = rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	dbDate       = Request("dbDate")
	ybDate1      = Request("ybDate1")
	ybDate2      = Request("ybDate2")
	wxDate1      = Request("wxDate1")
	wxDate2      = Request("wxDate2")
	dim StartDate,EndDate
	Set rs = conn.Execute("select convert(varchar(10),dateadd(dd,1-day(getdate()),getdate()),120) as StartDate , convert(varchar(10),dateadd(dd,-day(getdate()),dateadd(mm,1,getdate())) ,120) as EndDate ")
'dim StartDate,EndDate
	if rs.eof=false then
	...
	rs.close
	If dbDate <> "" Then
		dbWhere1 = " AND CONVERT(VARCHAR(10),a.date1,120) <= '"&dbDate&"' "
		dbWhere2 = " AND CONVERT(VARCHAR(10),AddTime,120) <= '"&dbDate&"' "
	else
		dbDate        = FormatDate(Date(),2)
		dbWhere1 = " AND CONVERT(VARCHAR(10),a.date1,120) <= '"&dbDate&"' "
		dbWhere2 = " AND CONVERT(VARCHAR(10),AddTime,120) <= '"&dbDate&"' "
	end if
	If ybDate1 = "" And ybDate2 = "" Then
		ybDate1 = StartDate
		ybDate2      = EndDate
	end if
	If wxDate1 = "" And wxDate2 = "" Then
		wxDate1 = StartDate
		wxDate2      = EndDate
	end if
	If ybDate1 <> "" And ybDate2 <> "" Then
		ybWhere1 = " AND datediff(dd,a.date1, '"&ybDate1&"' )<=0 and datediff(dd,a.date1, '"&ybDate2&"' )>=0 "
		ybWhere2 = " AND datediff(dd,AddTime, '"&ybDate1&"' )<=0 and datediff(dd,AddTime, '"&ybDate2&"' )>=0 "
	end if
	If wxDate1 <> "" And wxDate2 <> "" Then
		wxWhere1 = " AND CONVERT(VARCHAR(10),date1,120) < '"&wxDate1&"' "
		wxWhere2 = " AND CONVERT(VARCHAR(10),a.date1,120) BETWEEN '"&wxDate1&"' AND '"&wxDate2&"' "
	end if
	sql  =       "SELECT ISNULL(SUM(b.num1) - SUM(b.num2),0) dpgNum FROM repair_sl a " &_
	"INNER JOIN repair_sl_list b ON a.id = b.repair_sl "  &_
	"WHERE a.del = 1 "&dbWhere1&" " &_
	"AND ("&open_45_1&" = 3 OR("&open_45_1&" = 1 AND CHARINDEX(','+CAST(a.addcate AS VARCHAR(12))+',' , ',"&intro_45_1&",') > 0 ))  "
'WHERE a.del = 1 &dbWhere1&  &_
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		dpgNum = CDbl(rs("dpgNum"))
	else
		dpgNum = 0
	end if
	rs.close
	set rs = nothing
	sql =       "SELECT ISNULL(SUM(NUM),0) dwxNum FROM RepairOrder WHERE Del = 1 AND Status < 2 "&dbWhere2&" " &_
	"AND ("&open_46_1&" = 3 OR("&open_46_1&" = 1 AND CHARINDEX(','+CAST(AddUser AS VARCHAR(12))+',' , ',"&intro_46_1&",') > 0 ))  "
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		dwxNum = CDbl(rs("dwxNum"))
	else
		dwxNum = 0
	end if
	rs.close
	set rs = nothing
	sql =       "SELECT ISNULL(SUM(b.num2),0) pgwbNum FROM repair_sl a " &_
	"INNER JOIN repair_sl_list b ON a.id = b.repair_sl AND a.del = 1 AND b.del = 1 " &_
	"WHERE b.num2 > 0 "&ybWhere1&" " &_
	"AND ("&open_45_1&" = 3 OR("&open_45_1&" = 1 AND CHARINDEX(','+CAST(a.addcate AS VARCHAR(12))+',' , ',"&intro_45_1&",') > 0 ))  "
'WHERE b.num2 > 0 &ybWhere1&  &_
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		pgwbNum = CDbl(rs("pgwbNum"))
	else
		pgwbNum = 0
	end if
	rs.close
	set rs = nothing
	sql =       "SELECT ISNULL(SUM(NUM),0) wxwbNum FROM RepairOrder WHERE Del = 1 AND Status = 2 "&ybWhere2&" " &_
	"AND ("&open_46_1&" = 3 OR ("&open_46_1&" = 1 AND CHARINDEX(','+CAST(AddUser AS VARCHAR(12))+',' , ',"&intro_46_1&",') > 0 ))  "
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		wxwbNum = CDbl(rs("wxwbNum"))
	else
		wxwbNum = 0
	end if
	rs.close
	set rs = nothing
	sql =       "SELECT SUM(ISNULL(rkNum - ckNum,0)) qckcNum FROM " &_
	"("  &_
	"  SELECT SUM(num1) rkNum,0 ckNum FROM repair_kulist WHERE del = 1 AND sort1 = 1 "&wxWhere1&" " &_
	"  UNION ALL " &_
	"  SELECT 0 rkNum,SUM(num1) ckNum FROM repair_kulist WHERE del = 1 AND sort1 = 2 "&wxWhere1&" " &_
	") a "
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		qckcNum = CDbl(rs("qckcNum"))
	else
		qckcNum = 0
	end if
	rs.close
	set rs = nothing
	sql =       "SELECT ISNULL(SUM(a.num1),0) rkNum FROM repair_kulist a " &_
	"INNER JOIN Repair_sl b ON b.id = a.repair_sl AND b.del =1 " &_
	"WHERE a.del = 1 AND a.sort1 = 1 "&wxWhere2&" "
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		rkNum = CDbl(rs("rkNum"))
	else
	...
	rs.close
	set rs = nothing
	sql =       "SELECT ISNULL(SUM(a.num1),0) ckNum FROM repair_kulist a " &_
	"INNER JOIN RepairOrder b ON b.ID = a.RepairOrder AND b.del =1 " &_
	"WHERE a.del = 1 AND a.sort1 = 2 "&wxWhere2&" "
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		ckNum = CDbl(rs("ckNum"))
	else
	...
	rs.close
	set rs = nothing
	qmkcNum     = qckcNum+rkNum-ckNum
	ckNum = 0
	Response.write "" & vbcrlf & "" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "    <tr>" & vbcrlf & "        <td class=""place"">维修工作台</td>" & vbcrlf & "        <td>&nbsp;</td>" & vbcrlf & "        <td align=""right"">&nbsp;</td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf &"    <tr class=""top"">" & vbcrlf & "               <td colspan=""2"" width=""50%"">待办工作" & vbcrlf & "        <span class=""dateBox"" style=""margin-left:180px;"">" & vbcrlf & "        <form name=""form1"" method=""post"" action=""?"">" & vbcrlf & "            <input readonly name=""dbDate"" id=""dbDate"" size=""10"" value="""
	Response.write dbDate
	Response.write """  onMouseUp=""datedlg.show();"" minDate="""
	Response.write FormatDate(Date(),2)
	Response.write """ dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "            <input type=""submit"" value=""检索"" class=""anybutton"" id=""search-bnt1"">" & vbcrlf & "        </form>" & vbcrlf & "        </span>" & vbcrlf & "        </td>" & vbcrlf & "        <td colspan=""2"" width=""50%"">已办工作" & vbcrlf & "        <span class=""dateBox"" style=""margin-left:130px;"">" & vbcrlf & "        <form name=""form2"" method=""post"" action=""?"">" & vbcrlf & "            从" & vbcrlf & "            <input readonly name=""ybDate1"" size=""10"" value="""
	'Response.write FormatDate(Date(),2)
	Response.write ybDate1
	Response.write """  onMouseUp=""datedlg.show();""dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "            到" & vbcrlf & "            <input readonly name=""ybDate2"" size=""10"" value="""
	Response.write ybDate2
	Response.write """  onMouseUp=""datedlg.show();""dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "            <input type=""submit"" value=""检索"" class=""anybutton"">" & vbcrlf & "        </form>" & vbcrlf & "        </span>" & vbcrlf & "        </td>" & vbcrlf & "   </tr>" & vbcrlf & "    <tr class=""t-center"">" & vbcrlf & "     <td>待派工</td>" & vbcrlf & "         <td>待维修</td>" & vbcrlf & "         <td>已派工</td>" & vbcrlf & "         <td>维修完毕</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr class=""t-center"">" & vbcrlf & "       <td>"
	'Response.write ybDate2
	Response.write FormatNumber(dpgNum,numScale,-1)
	'Response.write ybDate2
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(dwxNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(pgwbNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(wxwbNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr class=""top"">" & vbcrlf & "             <td colspan=""4"">维修库存" & vbcrlf & "        <span class=""dateBox"" style=""margin-left:435px;"">" & vbcrlf & "        <form name=""form3"" method=""post"" action=""?"">" & vbcrlf & "            从" & vbcrlf & "    <input readonly name=""wxDate1"" size=""10"" value="""
	Response.write wxDate1
	Response.write """  onMouseUp=""datedlg.show();""dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "            到" & vbcrlf & "            <input readonly name=""wxDate2"" size=""10"" value="""
	Response.write wxDate2
	Response.write """  onMouseUp=""datedlg.show();""dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "            <input type=""submit"" value=""检索"" class=""anybutton"">" & vbcrlf & "        </form>" & vbcrlf & "        </span>" & vbcrlf & "        " & vbcrlf & "        </td>" & vbcrlf & "   </tr>"& vbcrlf & "    <tr class=""t-center"">" & vbcrlf & "     <td width=""25%"">期初数量</td>" & vbcrlf & "     <td width=""25%"">入库数量</td>" & vbcrlf & "     <td width=""25%"">出库数量</td>" & vbcrlf & "     <td width=""25%"">期末数量</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr class=""t-center"">" & vbcrlf & "        <td>"
	Response.write FormatNumber(qckcNum,numScale,-1)
'& "        <td>"
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(rkNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(ckNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "        <td>"
	Response.write FormatNumber(qmkcNum,numScale,-1)
	'Response.write "</td>" & vbcrlf & "        <td>"
	Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>"
