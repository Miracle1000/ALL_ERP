...
'sqltext = sqltext & " select " & ProductAttrValue & " as Num1 ,'" & attrs & AttrID &"' as Attrs "
		...
		Dim Str_Result
		Sub page_load
			Dim sqlcon, px_Result
			Set conn = cn
			Dim SH : SH = request("SH")
			Str_Result = app.base64.safeDeCode(request.form("Str_Result_v"))
			sqlcon = app.base64.safeDeCode(request.form("persons_result_v"))
			px_Result = app.base64.SafeDeCode(request.form("px_v"))
			Dim intro_5_10 : intro_5_10 = app.power.GetPowerIntro(5,10)
			If SH = "2" Then
				Str_Result = Str_Result & app.iif(Len(intro_5_10)=0,""," and cateid in (" & intro_5_10 & ")")
			end if
			Dim baseSql
			...
			If app.power.existsPower(5,21) = 0 Then
...
'If rs("set_open") <> 1 Or rs("dc") <> 1 Then
			...
			rs.close
			set rs=nothing
			fields=fields&",合同概要,洽谈进展,下级审批人,销售人员,创建人,创建时间"
...
'if ZBRuntime.MC(12006) = False then
			...
			Set rs=Nothing
			fields = fields &",深度"
			Dim mxzdyFields(),sorce , set_open
			dim newZdy : set newZdy = cn.execute(NewMxZDYSql(11001 , 1 , 0,"") )
			j = 0
			ReDim mxzdyFields(1,0)
			While newZdy.eof = False
				tname = newZdy("tname")
				if instr(newZdy("fieldname").value, "InheritId_id")=0 then
					ReDim Preserve mxzdyFields(1,j)
					mxzdyFields(0,j) = tname
					mxzdyFields(1,j) = newZdy("title")
					fields=fields&"," & mxzdyFields(0,j)
					set_open = newZdy("IsUsed")
					If set_open <> "1" Then
...
'If set_open <> "1" Then
					...
					if tname = "unit" then
						fields=fields&",单位属性"
						fields=fields&",产品属性1,产品属性2"
					elseif tname = "num1" Then
			...
			wend
			newZdy.close
			Dim mxExtendzdyFields(),extendfields,extendfieldstmp,extendJoinTables,temptable
			sql ="      select  isnull(b.title, t.title) as title,cast(isnull(b.id,0) as varchar(10))+','+cast(isnull(b.InheritId,t.id) as varchar(10)) as id , "&_
			"'id_' + cast(isnull(b.InheritId,t.id) as varchar(12)) as InheritId     "&_
			"from ( select Title,  'InheritId_id_' + cast(id as varchar(12)) as dbname,  id, (showindex + 21) as showindex,  isUsed as defIsUsed,Candc               "&_
			"from sys_sdk_BillFieldInfo           "&_
			"      where ListType=0 and BillType =16001 and BillType>0    "&_
			"  ) t   "&_
			"  left join sys_sdk_BillFieldInfo b on t.dbname=b.dbname and b.BillType =  11001  and b.ListType=1 AND b.InheritModel IN (0,1) AND b.IsParentInheritMX=0           "&_
			"  where (isnull(b.IsUsed, t.defIsUsed) =  1 )                                                                                                                                                             "&_
			"  order by isnull(b.showindex, t.ShowIndex) "
			Set rs = cn.execute(sql)
			j = 0
			ReDim mxExtendzdyFields(1,0)
			While rs.eof = False
				ReDim Preserve mxExtendzdyFields(1,j)
				mxExtendzdyFields(0,j) = rs("InheritId")
				mxExtendzdyFields(1,j) = rs("title")
				fields=fields&"," & rs("InheritId")
				temptable="extend"&j
				extendfieldstmp=extendfieldstmp+","&temptable+ ".Value as "&rs("InheritId")
				'temptable="extend"&j
				extendfields=extendfields+ ","&" null as "&rs("InheritId")
				'temptable="extend"&j
				extendJoinTables=extendJoinTables&" left join #extendzdy "&temptable&" on "&temptable&".BillId=tc.ord and "&temptable&".ListID=cl.id and "&temptable&".FieldId "&" in ("&rs("id")&")"
				rs.movenext
				j = j + 1
			wend
			if ZBRuntime.MC(17000) = False Then
...
'if ZBRuntime.MC(17000) = False Then '
			...
			if isOpenMoreUnitAttr=false then
				CantExport_Fields(cfidx) = "单位属性" : cfidx=cfidx+1
'if isOpenMoreUnitAttr=false then
			end if
			dim extIntro : extIntro = "1"
			if isOpenProductAttr=false Then
				CantExport_Fields(cfidx) = "产品属性1" : cfidx=cfidx+1
'if isOpenProductAttr=false Then '
				CantExport_Fields(cfidx) = "产品属性2" : cfidx=cfidx+1
'if isOpenProductAttr=false Then '
				extIntro = "0"
			end if
...
'if ZBRuntime.MC(8000) = False Then '
			...
			if app.power.existsPower(5,21) = 0 Then
				Dim tmpArr
				tmpArr = Split("未税单价,折扣,折后单价,含税单价,含税折后单价,税额,金额,税后总价,明细优惠,优惠后单价,优惠后总价",",")
				For j=0 To ubound(tmpArr)
					CantExport_Fields(cfidx) = fDBName(tmpArr(j)) : cfidx=cfidx+1
'For j=0 To ubound(tmpArr)
				next
			end if
			if app.power.existsPower(5,24)  = 0 Then
				tmpArr = Split("建议总价,建议进价",",")
				For j=0 To ubound(tmpArr)
					CantExport_Fields(cfidx) = fDBName(tmpArr(j)) : cfidx=cfidx+1
'For j=0 To ubound(tmpArr)
				next
			end if
			baseSql = " exec erp_xls_ht '"& replace(Str_Result & sqlcon,"'","''") &"','"& replace(px_Result,"'","''") &"','"& replace(exZdy1,"'","''") &"','"& replace(exZdy2,"'","''") &"','"& replace(exJoinTables,"'","''") &"','"& replace(fields,"'","''")  &"','"& replace(extendfields,"'","''")&"','"& replace(extendfieldstmp,"'","''")&"','"& replace(extendJoinTables,"'","''")&"','"& extIntro &"'"
			Response.write baseSql
			Dim adapter
			...
			For i = 0 To ubound(fds)
				...
				Select Case sname
				Case "合同总额","合同优惠金额","优惠后总额","到账金额","折扣","优惠后总价","税额","金额","税后总价","明细优惠","运杂费","建议总价":
				hd.dbtype = "money"
				...
				hd.ignoreNonnumeric = True
				hd.excelAlign = "right"
				Case "折后单价","未税单价","含税折后单价","含税单价","优惠后单价":
				...
				hd.width = 60
				hd.excelAlign = "center"
				If sname="税率" Then hd.Formula = "myFormatNumber(@value,"&info.MoneyNumber&",""%"")"
			...
			For i=0 To ubound(mxzdyFields,2)
				...
				sheetSetting.headerSettings.add(hd)
			next
			For i=0 To ubound(mxExtendzdyFields,2)
				Set hd = New HeaderColumnSetting
				hd.dbname = mxExtendzdyFields(0,i)
				hd.title = mxExtendzdyFields(1,i)
				sheetSetting.headerSettings.add(hd)
			next
			Set hd = New HeaderColumnSetting
			hd.dbname = "深度"
		...
		Function MoreUnitAttr(v)
			If v&""<>"" Then
				MoreUnitAttr = LoadMoreUnit(4,Replace(v,"&#125;","}"), 0 , 0 ,info.floatNumber)
			else
				MoreUnitAttr = ""
			end if
		end function
...
'If v&""<>"" Then
		...
		Function fShowName(fname)
			...
			Case "title":fShowName="产品名称"
			Case "order1":fShowName="产品编号"
			Case "type1":fShowName="产品型号"
			Case "unit":fShowName="产品单位"
			Case "num1":fShowName="购买数量"
			Case "price1":fShowName="未税单价"
			Case "discount":fShowName="折扣"
			Case "priceAfterDiscount":fShowName="折后单价"
			Case "invoiceType":fShowName="票据类型"
			Case "taxRate":fShowName="税率"
			Case "priceAfterTaxPre":fShowName="含税折后单价"
			Case "taxValue":fShowName="税额"
			Case "moneyAfterConcessions":fShowName="金额"
			Case "moneyAfterTax":fShowName="税后总价"
			Case "concessions":fShowName="明细优惠"
			Case "priceIncludeTax":fShowName="含税单价"
			case "priceAfterTax":fShowName="优惠后单价"
			Case "money1":fShowName="优惠后总价"
			Case "date2":fShowName="交货日期"
			Case "intro":fShowName="备注"
			...
			Case "pricejy":fShowName="建议进价"
			Case "tpricejy":fShowName="建议总价"
		...
		Function fDBName(fname)
			Select Case fname
			Case "深度":fDBName="P0"
			Case "产品名称":fDBName="title"
			Case "产品编号":fDBName="order1"
			Case "产品型号":fDBName="type1"
			Case "产品单位":fDBName="unit"
			Case "购买数量":fDBName="num1"
			Case "未税单价":fDBName="price1"
			Case "优惠后总价":fDBName="money1"
			Case "折扣":fDBName="discount"
			Case "折后单价":fDBName="priceAfterDiscount"
			Case "票据类型":fDBName="invoiceType"
			Case "税率":fDBName="taxRate"
			Case "含税折后单价":fDBName="priceAfterTaxPre"
			Case "税额":fDBName="taxValue"
			Case "金额":fDBName="moneyAfterConcessions"
			Case "税后总价":fDBName="moneyAfterTax"
			Case "明细优惠":fDBName="concessions"
			Case "含税单价":fDBName="priceIncludeTax"
			case "优惠后单价":fDBName="priceAfterTax"
			Case "交货日期":fDBName="date2"
			Case "备注":fDBName="intro"
			...
			Case "建议进价":fDBName="pricejy"
			Case "建议总价":fDBName="tpricejy"
