...
'if m2="" and clrtype<>"1" then
	...
	if A2<>"0" then
		str_Result11=str_Result11+" and a.bz="&A2&" "
'if A2<>"0" then
	end if
	if m1<>"" then
		str_Result11=str_Result11+" and a.date1>='"&m1&"' "
'if m1<>"" then
	end if
	if m2<>"" then
		str_Result11=str_Result11+" and a.date1<='"&m2&"' "
'if m2<>"" then
	end if
	if clrtype <> "1" then
		clrtype=1
	...
	if C<>"" then
		if B="khmc" then
			str_Result11=str_Result11+" and a.company in (select ord from tel where name like '%"& C &"%')"
'if B="khmc" then
		elseif B="khid" then
			str_Result11=str_Result11+" and a.company in (select ord from tel where khid like '%"& C &"%')"
'elseif B="khid" then
		elseif B="thdmc" then
			str_Result11=str_Result11+" and a.company in (select company from caigouth where title like '%"& C &"%')"
'elseif B="thdmc" then
		elseif B="thid" then
			str_Result11=str_Result11+" and a.company in (select company from caigouth where cgthid like '%"& C &"%')"
'elseif B="thid" then
		elseif B="thry" then
			str_Result11=str_Result11+" and a.company in (select company from caigouth where cateid in (select ord from gate where name like '%"& C &"%'))"
'elseif B="thry" then
		end if
	end if
	...
	if px=1 then
		px_Result="order by name desc,a.company desc"
	elseif px=2 then
		px_Result="order by name asc,a.company asc"
	elseif px=3 then
		px_Result="order by money1 desc,a.company desc"
	elseif px=4 then
		px_Result="order by money1 asc,a.company asc"
	elseif px=5 then
		px_Result="order by money2 desc,a.company desc"
	elseif px=6 then
		px_Result="order by money2 asc,a.company asc"
	elseif px=7 then
		px_Result="order by money3 desc,a.company desc"
	elseif px=8 then
		px_Result="order by money3 asc,a.company asc"
	elseif px=9 then
		px_Result="order by cateidname desc,a.company desc"
	elseif px=10 then
		px_Result="order by cateidname asc,a.company asc"
	end if
	...
	elseif open_76_1=1 then
		str_Result11=str_Result11&" and a.cateid in ("&intro_76_1&")"
	else
		str_Result11=str_Result11&" and 1=2 "
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/m4_planall2_hz.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "     <form action=""planall2_hz.asp?clrtype="
	Response.write clrtype
	Response.write "&px="
	Response.write px
	Response.write "&page_count="
	Response.write page_count
	Response.write """ method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "               <tr>" & vbcrlf & "      <td class=""place"">采购退款汇总表</td>" & vbcrlf & "      <td>&nbsp;<a href=""#"" onClick=""Myopen_px(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "      <td align=""right""><select name=""select2"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "      <option>-请选择-</option>" & vbcrlf & "      <option value=""page_count=10"" "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "    </select></td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td  background=""../images/112.gif"" class=""top_btns""  height=""30"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;""><input name=""D"" type=""radio"" value=""1"" checked>" & vbcrlf & "汇总模式" & vbcrlf & "  <input type=""radio"" name=""D"" value=""2"" onClick=""go('planall2_mx.asp?clrtype="
	Response.write clrtype
	Response.write "&A="
	Response.write A
	Response.write "&A2="
	...
	Response.write "&px="
	Response.write px
	Response.write "');"">" & vbcrlf & "明细模式" & vbcrlf & ""
	...
	Response.write ">供应商编号</option>" & vbcrlf & "  <option value=""thdmc"" "
	if B="thdmc" then
		Response.write "selected"
	end if
	...
	if open_76_10=1 or open_76_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if (confirm('确认导出为EXCEL文档？')) { exportExcel({ from: 'form_with_page_action', page:'../out/xls_cgtkhz.asp?clrtype="
		Response.write clrtype
		Response.write "&A="
		Response.write A
		Response.write "&A2="
	...
	Response.write "</span></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "</form>" & vbcrlf & "      <tr class=""top resetGroupTableBg"">" & vbcrlf & "          <td height=""27""><div align=""center"">供应商名称</div></td>" & vbcrlf & "    <td width=""15%""><div align=""center"">应退总额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">实退总额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">剩余金额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">退货人员</div></td>" & vbcrlf & "        </tr>" & vbcrlf & ""
	...
	money3_all=0
	n=0
	sql="select a.company,a.bz,"&_
	"(select name from tel WITH(NOLOCK) where del=1 and ord=a.company) as name, "&_
	"(select intro from sortbz WITH(NOLOCK) where id=a.bz) as sortbz, "&_
	"isnull(sum(a.money1),0) as money1,"&_
	"isnull(sum(case a.complete when 2 then a.money1 else 0 end), 0) as money2,"&_
	"isnull(sum(a.money1),0) -isnull(sum(case a.complete when 2 then a.money1 else 0 end),0) as money3, "&_
	"isnull(sum(case a.complete when 2 then a.money1 else 0 end), 0) as money2,"&_
	"(select name from payout3 py3 inner join gate g on py3.cateid=g.ord and g.del=1 where py3.ord=max(a.ord)) as cateidname, "&_
	"max(a.ord) as maxord "&_
	"from payout3 as a WITH(NOLOCK) left join caigouth as b WITH(NOLOCK) on a.caigouth=b.ord where a.del=1 "&Str_Result11&" group by a.company,a.bz "&px_Result&""
	set rs1=server.CreateObject("adodb.recordset")
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			if cateid="" or cateid=0 then cateid=-1
			delname="已彻底删除"
			if rs("money1")&""="" then
				money1=0
			else
			...
			if delname="已彻底删除" then
				...
				if sort3=2 and (open_26_14=3 or CheckPurview(intro_26_1,trim(cateid))=True) then
					...
					Response.write delname
					Response.write "</span></a></td>" & vbcrlf & "                     "
				elseif sort3=1 and ((open_1_14=3 or CheckPurview(intro_1_1,trim(cateid))=True) or (InStr(1,","&share&"," , ","& sdk.user &",",1) Or share = "1" )) then
			...
			maxord=  clng("0" &  rs("maxord").value)
			if maxord>0 then
				set rs1 = conn.execute("select b.name from payout3 a inner join gate b on a.ord=" & maxord  & " and a.cateid=b.ord")
				if rs1.eof = false then
					Response.write rs1("name").value
				else
					Response.write "&nbsp;"
				end if
				rs1.close
		...
		loop
		Response.write "     " & vbcrlf & "    <tr height=""27"" onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "       <td class=""name""><div align=""right"">本页合计：</div></td>" & vbcrlf & "   <td class=""name""><div align=""right"">"
		...
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write "</div></td>" & vbcrlf & "    <td class=""red""><div align=""right"">&nbsp;</div></td>" & vbcrlf & "        </tr>" & vbcrlf & " <tr height=""27"" onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "   <td class=""name""><div align=""right"">所有合计：</div></td>" & vbcrlf & "   <td class=""name""><div align=""right"">"
	...
	action1="采购退款汇总表"
	call close_list(1)
	Response.write "   " & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<script src= ""../Script/m4_planall2_hz_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:400;display:none;"">" & vbcrlf & "<table width=""150"" height="""
	if A2<>"0" then
		Response.write "300"
	else
		Response.write "150"
	end if
	Response.write """  border=""0"" cellpadding=""2"" cellspacing=""2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""100"">" & vbcrlf & "        <table width=""150"" height=""100"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按供应商名称排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按供应商名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & ""
	if A2<>"0" then
