...
'dim sqlstr
	...
	dim w,a ,b,c,d,e,f,sort1,sort2,order,m1,m2,ctord
	ctord=deurl(request("ctord"))
	m1=request("ret")
	m2=request("ret2")
	A=request("A")
	A2=request("A2")
...
'If formTj = "1" Then
		If W3&""<>"" And W3&""<>"0" Then Str_Result=Str_Result+ " and cateid in ("& W3 &")  "
'If formTj = "1" Then
	else
		W3=getW3(W1,W2,W3)
	...
	if ctord&""<>"" then
		str_Result=str_Result+"and contractth in ("&ctord&") "
'if ctord&""<>"" then
	end if
	if C<>"" then
		if B="khmc" then
			str_Result=str_Result+"and contractth in (select ord from contractth where company in (select ord from tel where name like '%"& C &"%')) "
'if B="khmc" then
		elseif B="htzt" then
			str_Result=str_Result+"and contractth in (select ord from contractth where  title like '%"& C &"%') "
'elseif B="htzt" then
		elseif B="htid" then
			str_Result=str_Result+"and  BH like '%"& C &"%' "
'elseif B="htid" then
		end if
	end if
	...
	if request("khmc")<>"" then
		str_Result=str_Result+"and contractth in (select ord from contractth where company in (select ord from tel where name like '%"& request("khmc") &"%')) "
'if request("khmc")<>"" then
	end if
	if request("khbh")<>"" then
		str_Result=str_Result+"and contractth in (select ord from contractth where company in (select ord from tel where khid like '%"& request("khbh") &"%')) "
'if request("khbh")<>"" then
	end if
	if request("htbh")<>"" then
		str_Result=str_Result+"and BH like '%"& request("htbh") &"%' "
'if request("htbh")<>"" then
	end if
	If request("htid")<>"" Then
		str_Result=str_Result+"and BH  like '%"& request("htid") &"%'  "
'If request("htid")<>"" Then
	end if
	If request("htord")<>"" Then
		str_Result=str_Result+"and contractth in (select ct.ord from contractth ct inner join contractthlist ctl on ct.ord=ctl.Caigou where ctl.contract>0 and ctl.contract="&request("htord")&" group by ct.ord) "
'If request("htord")<>"" Then
	end if
	if request("hkzt")<>"" and request("hkzt")<>"10" then
		if request("hkzt")="1" then
			Str_Result=Str_Result+"and  complete=1 "
'if request("hkzt")="1" then
		elseif request("hkzt")="2" then
			Str_Result=Str_Result+"and  complete=2 "
'elseif request("hkzt")="2" then
		end if
	end if
...
'if  Request("duemoney1")<>"" and Request("duemoney2")<>"" then
	...
	if  Request("duepaydate1")<>"" and Request("duepaydate2")<>"" then
		...
		str_Result=str_Result+" and date1  between '"&Request("duepaydate1")&"' and '"&Request("duepaydate2")&"'"
		m2=Request("duepaydate2")
	end if
...
'if cint(request("paytype"))>0 then
	...
	if Request("paydate1")<>"" and Request("paydate2")<>"" then
		str_Result=str_Result+" and date2  between '"&Request("paydate1")&"' and '"&Request("paydate2")&"'"
'if Request("paydate1")<>"" and Request("paydate2")<>"" then
	end if
	if Request("invdate1")<>"" and Request("invdate2")<>"" then
		str_Result=str_Result+" and date2  between '"&Request("invdate1")&"' and '"&Request("invdate2")&"'"
'if Request("invdate1")<>"" and Request("invdate2")<>"" then
	end if
...
'if request("intro")<>"" then
	...
	if request("bz")<>"" then
		if cint(request("bz"))>0 then
			str_Result=str_Result+" and contractth in (select ord from contractth where bz = "&cint(request("bz"))&")"
'if cint(request("bz"))>0 then
		end if
	end if
	str_Result=str_Result+" and ("&open_9_1&"=3 or ("&open_9_1&"=1 and  charindex(','+cast(Cateid as varchar(10))+',',',"& intro_9_1 &",')>0))"
'if cint(request("bz"))>0 then
...
'If khbianh="" Then khbianh="客户编号"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "    #kh{height:22px!important;line-height:22px!important}" & vbcrlf & "   #w.easyui-window.panel-body.panel-body-noborder.window-body{height:500px!important}" & vbcrlf & "     .IE5 .top_btns input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "" & vbcrlf & "</head>" & vbcrlf & "<body  "
	'Response.write Application("sys.info.jsver")
	if open_9_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"" >" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "      <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "   <form method=""get"" action=""planall2.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "              <tr>" & vbcrlf & "        <td class=""place"">退款列表"
	if A="1" then
		Response.write "&gt;&gt;应退账款"
	elseif A="2" then
		Response.write "&gt;&gt;已退账款"
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "          </select>" & vbcrlf & "        </div></td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "    </table>" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td  class='top_btns resetHeadBg' height=""50"" align=""right"" style=""background:rgb(244, 250, 254)"">"
	'Response.write "selected"
	...
	Response.write "" & vbcrlf & "                     <select name=""A"">" & vbcrlf & "                   <option value=""1"" "
	if A=1 then
		Response.write "selected"
	end if
	Response.write ">未退款</option>" & vbcrlf & "                       <option value=""2"" "
	if A=2 then
		Response.write "selected"
	end if
	Response.write ">已退款</option>" & vbcrlf & "                       <option value=""10"" "
	if A=10 then
		Response.write "selected"
	end if
	Response.write ">退款状态</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <select name=""B"">" & vbcrlf & "                   <option value=""khmc"" "
	if B="khmc" then
		Response.write "selected"
	end if
	Response.write ">"
	Response.write khname
	Response.write "</option>" & vbcrlf & "                      <option value=""htzt"" "
	if B="htzt" then
		Response.write "selected"
	end if
	Response.write ">退货主题</option>" & vbcrlf & "                     <option value=""htid"" "
	if B="htid" then
		Response.write "selected"
	end if
	Response.write ">退款编号</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "                       <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                 <input type=""button"" name=""openbn"" id=""openbn"" value=""高级检索"" class=""anybutton"" onclick=""$('#w').show().window('open');""/>" & vbcrlf & "                        <span id=s1>"
	if open_9_10=1 or open_9_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""EXCEL导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_tk.asp'})}"" class=""anybutton""/>" & vbcrlf & "                  "
	end if
	...
	Response.write "</span></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      </form>" & vbcrlf & " <tr class=""top"">" & vbcrlf & "    <td width=""40"" height=""30"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "         <td><div align=""center""><strong>"
	Response.write khname
	Response.write "</strong></div></td>" & vbcrlf & "   <td width=""12%""><div align=""center""><strong>退款编号</strong></div></td>" & vbcrlf & "    <td width=""10%"" height=""27"" ><div align=""center""><strong>金额</strong></div></td>" & vbcrlf & "     <td width=""10%""><div align=""center"">应退日期</div></td>" & vbcrlf & "          <td width=""10%""><div align=""center"">退款日期</div></td>" & vbcrlf & "     <td width=""7%""><div align=""center""><strong>退款账号</strong></div></td>" & vbcrlf & "     <td width=""7%""><div align=""center""><strong>退款状态</strong></div></td>" & vbcrlf & "     <td width=""7%""><div align=""center""><strong>退货人员</strong></div></td>" & vbcrlf & "         <td width=""12%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "        </tr>" & vbcrlf & ""
	dim n
	n=0
	total_money=0
	all_total_money=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select isnull(sum(money1),0) as money1 from payout2 "&Str_Result
	rs.open sql,conn
	if not rs.eof then
		all_total_money=cdbl(rs(0).value)
	end if
	rs.close
	dim isOpen : isOpen = IsOpenVoucherCForXTK
	dim all_total_money_hl,sql_hl,rs_hl
	all_total_money_hl=0
	set rs_hl=server.CreateObject("adodb.recordset")
	sql_hl="select isnull(sum(money1*hl),0) from("&_
	"select isnull(money1,0) as money1,"&_
	"isnull((select isnull(bz,0) from contractth WITH(NOLOCK) where ord = payout2.contractth),0) as bz,"&_
	"isnull((select top 1 hl from hl WITH(NOLOCK) where date1=(select date3 from contractth WITH(NOLOCK) where ord = payout2.contractth) and bz=(select bz from contractth WITH(NOLOCK) where ord = payout2.contractth)),1) as hl"&_
	" from payout2 WITH(NOLOCK) " & Str_Result & ") as a"
	rs_hl.Open sql_hl,conn
	if not rs_hl.eof then
		all_total_money_hl=cdbl(rs_hl(0).Value)
	end if
	set rs_hl=nothing
	sql="select * from payout2  "&Str_Result&" "&px_Result&""
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""deleteall2.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """>" & vbcrlf & ""
		do until rs.eof
			CompleteType= rs("CompleteType")
			contractthord= rs("contractth")
			if contractthord&""="" Then contractthord = 0
			company=0
			cateid_tel=0
			share=""
			sort3 = 1
			...
			sql7="select ord,name,isnull(cateid,0) as cateid, ISNULL(share,'-222') share ,sort3, del from tel where ord in (select company from contractth where ord="& contractthord &") and del=1"
			set rs7=server.CreateObject("adodb.recordset")
			...
			intro=rs("intro")
			SureListId=rs("SureListId")
			if SureListId&""="" then
				SureListId=0
			end if
			if rs("contractth")<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select ord,title,cateid,contract,thType from contractth where ord="&rs("contractth")&" and del=1"
				rs1.open sql1,conn,1,1
				if rs1.eof then
					contractth=0
					contractthname="关联退货单已被删除"
					cateid1=0
				else
					contractth=rs1("ord")
					contractthname=rs1("title")
					cateid1=rs1("cateid")
					contractord=rs1("contract")
				end if
				rs1.close
				set rs1=nothing
...
'if cateid1="" or cateid1=0 then
			...
			Isdelete=true
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select top 1 1 from PayBackSureList where ID="&SureListId&" "
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				Isdelete=false
			end if
			rs1.close
			set rs1=nothing
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td align=""center"" height=""30""><span class=""red"">" & vbcrlf & "       "
			if open_9_3=3 or CheckPurview(intro_9_3,trim(cateid))=True then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>" & vbcrlf & "          "
			end if
			Response.write "</span></td>" & vbcrlf & "      <td><div align=""left"">" & vbcrlf & "               "
			If sort3="2" Then
				If del=1 Then
					If open_26_1 = 3 Or (open_26_1 = 1 And CheckPurview(intro_26_1,trim(cateid_tel))=True)  Then
						if open_26_14=3 or CheckPurview(intro_26_14,trim(cateid_tel))=True then
							Response.write "" & vbcrlf & "                                             <a href=""#"" onclick=""javascript:window.open('../work2/content.asp?ord="
							Response.write pwurl(company)
							Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">"
							Response.write pwurl(company)
					...
					Response.write "<span class='red'>&nbsp;供应商已被删除</span>"
				end if
			else
				If del=1 Then
					If open_1_1 = 3 Or (open_1_1 = 1 And CheckPurview(intro_1_1,trim(cateid_tel))=True) Or  InStr(1,","&share&",", ","&sdk.info.user&",",1) > 0 Or share = "1"  Then
						if open_1_14=3 or CheckPurview(intro_1_14,trim(cateid_tel))=True then
			...
			Response.write "" & vbcrlf & "       </div><br />" & vbcrlf & "        "
			if open_41_1=3 or (open_41_1 = 1 And CheckPurview(intro_41_1,trim(cateid1))=True) then
				Response.write "" & vbcrlf & "          <div align=""left"">" & vbcrlf & "          "
				if open_41_14=3 or (open_41_14 = 1 And CheckPurview(intro_41_14,trim(cateid1))=True) then
					Response.write "" & vbcrlf & "          <a href=""#"" onclick=""javascript:window.open('../contractth/content.asp?ord="
					Response.write pwurl(contractth)
					Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >" & vbcrlf & "          【退货】"
					Response.write pwurl(contractth)
					Response.write contractthname
				else
					Response.write "" & vbcrlf & "" & vbcrlf & "         【退货】"
					Response.write contractthname
				end if
			...
			Response.write "</a></div></td>" & vbcrlf & "" & vbcrlf & "      <td><div align=""right"">"
			Response.write  getbzflag(rs("contractth"),"contractthid")
			...
			if rs("complete")="1" then
				Response.write "未退款"
				if open_9_13=3 or (open_9_13=1 And CheckPurview(intro_9_13,trim(cateid))=True) then
					Response.write "<img src=""../images/jiantou.gif""><a href=""#"" onclick=""javascript:window.open('../money3/addht2.asp?ord="
					Response.write pwurl(rs("ord"))
					Response.write "','plancor5','width=' + 700 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100');return false;"">退款</a>"
					Response.write pwurl(rs("ord"))
				end if
			...
			BillID=rs("ord")
			dim canUpdate: canUpdate = true
			if isOpen then canUpdate = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & BillID &"',8, 6, '' ").eof
			Response.write "" & vbcrlf & "        <td><div align=""center"">"
			Response.write cateidname
			Response.write "</div></td>" & vbcrlf & "     <td class=""func""><div align=""center"">" & vbcrlf & "        "
			if open_9_14=3 or (open_9_14 = 1 And CheckPurview(intro_9_14,trim(cateid))=True) then
				...
				Response.write pwurl(rs("ord"))
				Response.write "', 'new432win', 'width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=50,top=50')""/> "
				Response.write pwurl(rs("ord"))
			end if
			if rs("complete")<>"2" and (open_9_2=3 or (open_9_2=1 And CheckPurview(intro_9_2,trim(cateid))=True) ) and canUpdate Then
				Response.write "<input type=""button"" name=""Submit"" value=""修改"" onClick=""javascript: window.open('../money3/correct.asp?ord="
				Response.write pwurl(rs("contractth"))
				Response.write "&rd="
				Response.write pwurl(rs("ord"))
				Response.write "', 'new433win', 'width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=300,top=150') ""/>"
				Response.write pwurl(rs("ord"))
			end if
			if IsDeletePayout2(rs("ord")) and (completeType<>2 or (completeType=2 and IsDeletePayout2Bybankin2(rs("ord")))) and (open_9_3=3 or (open_9_3=1 And CheckPurview(intro_9_3,trim(cateid))=True) ) and canUpdate Then
				Response.write " <input type=""button"" name=""Submit"" value=""删除"" onClick=""if (confirm('确认删除？')) { window.location.href = '../money3/deletepay2.asp?"
				Response.write sdk.ReturnURL("Ismode=1&ord=" & rs("ord") &"")
				Response.write "' }""/>"
			end if
			Response.write "" & vbcrlf & "        " & vbcrlf & "        </div>" & vbcrlf & "" & vbcrlf & "         </td>" & vbcrlf & "  </tr>" & vbcrlf & ""
			n=n+1
			rs.movenext
			if rs.eof or n>=rs.pagesize then exit do
		loop
		m=n
		Response.write "" & vbcrlf & "      <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td height=""30"" colspan=""3"" align=""right"">本页合计：</td>" & vbcrlf & "     <td><div align=""right"" class=""red"">"
		Response.write Formatnumber(total_money,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "     <td colspan=""6"">&nbsp;</td>" & vbcrlf & "       </tr>" & vbcrlf & " <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td height=""30"" colspan=""3"" align=""right"">全部合计：</td>" & vbcrlf & "     <td><div align=""right"" class=""red"">"
		Response.write Formatnumber(all_total_money,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "      <td><div align=""right"" class=""red"">￥"
		Response.write Formatnumber(all_total_money_hl,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "      <td><div align=""right"" class=""red"">￥"
		...
		if open_9_3=1 or open_9_3=3 then
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test(form)" & vbcrlf & "{" & vbcrlf & "    var tip = true;" & vbcrlf & "    for (var i = 0; i < form.elements.length; i++) {" & vbcrlf & "        var e = form.elements[i];" & vbcrlf & "        if (e.name == 'selectid' && e.checked) {" & vbcrlf & "            tip = false;" & vbcrlf & "            break;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "    if (tip) {" & vbcrlf & "        alert('您没有选择任何单据，请选择后再删除！');" & vbcrlf & "        return false;" & vbcrlf & "    }" & vbcrlf & "    else if (!confirme != 'chkall')" & vbcrlf & "               e.checked = form.chkall.checked;" & vbcrlf & "        }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""138"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "    <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td width=""100%"" height=""10""><img src=""../image/pixel.gif"" width=""1"" height=""1""></td>" & vbcrlf & "      </tr>" & vbcrlf & "             <tr>" & vbcrlf & "           <td height=""10"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen_px(divID){" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "      }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=0;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:350;display:none;"">" & vbcrlf & "<table width=""150"" height=""250""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按应退日期排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按应退日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按退款日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按退款日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按退款金额排序(升)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl款状态排序(升)</font></a> </td>""" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""高级检索""  style=""width:580px;height:500px;padding:5px;background: #fafafa;top:60px;display:none""  closed=""true"" >" & vbcrlf & "  <form method=""get"" action=""planall2.asp"" id=""date1"" onSubmit=""return Validator.Validate(this,2)"" name=""date33"">" & vbcrlf & "                   <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:530px"">" & vbcrlf & "                        <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                        <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                  <td width=""24%"" class=""func""><div align=""right"">"
	Response.write khname
	Response.write "</div></td>" & vbcrlf & "                  <td width=""76%""><label><input name=""khmc"" type=""text"" id=""khmc""></label></td>" & vbcrlf & "              </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "   <td width=""24%"" class=""func""><div align=""right"">"
	Response.write khbianh
	Response.write "</div></td>" & vbcrlf & "                  <td width=""76%""><label><input name=""khbh"" type=""text"" id=""khbh""></label></td>" & vbcrlf & "              </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "   <td width=""24%"" class=""func""><div align=""right"">退货主题</div></td>" & vbcrlf & "                 <td><input name=""contractname"" type=""text"" id=""contractname""><input name=""formTj"" type=""hidden"" value="""
	Response.write formTj
	Response.write """></td>" & vbcrlf & "               </tr>" & vbcrlf & "                          <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""24%"" class=""func""><div align=""right"">退款编号</div></td>" & vbcrlf & "                 <td width=""76%""><label><input name=""htbh"" type=""text"" id=""htbh""></label></td>" & vbcrlf & "              </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">退货人员</div></td>" & vbcrlf & "                 <td>"
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	Response.write "</td>" & vbcrlf & "               </tr>" & vbcrlf & "                        <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                  <td width=""24%"" class=""func""><div align=""right"">退款状态</div></td>" & vbcrlf & "                  <td width=""76%""><label>" & vbcrlf & "                               <select name=""hkzt"">" & vbcrlf & "                                      <option value=""10"" selected>退款状态</option>" & vbcrlf & "                                     <option value=""1"">未退款</option>" & vbcrlf & "                                         <option value=""2"">已退款</option>" & vbcrlf & "                                 </select>" & vbcrlf & "                               </label></td>" & vbcrlf & "               </tr>" & vbcrlf & "                         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                  <td width=""24%"" class=""func""><div align=""right"">退款方式</div></td>" & vbcrlf & "                  <td width=""76%""><label>" & vbcrlf & "                                 <select name=""skfs"">" & vbcrlf & "                                      <option value=""0"" selected>退款方式</option>" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
	...
	bzrs.close
	set bzrs = nothing
	Response.write "" & vbcrlf & "                 </select>" & vbcrlf & "                 </select></td>" & vbcrlf & "               </tr>" & vbcrlf & "                           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">金额</div></td>" & vbcrlf & "                 <td><input name=""duemoney1"" type=""text""  class=""easyui-numberbox"" id=""duemoney1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" onpropertychange=""formatData(this,'money');""  precision="""
	set bzrs = nothing
	Response.write num_dot_xs
	Response.write """   value=""0"">" & vbcrlf & "                   -" & vbcrlf & "                 <input name=""duemoney2"" type=""text"" id=""duemoney2"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision="""
	Response.write num_dot_xs
	Response.write num_dot_xs
	Response.write """ onpropertychange=""formatData(this,'money');""  value=""999999999""></td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">备注</div></td>" & vbcrlf & "                 <td><label>" & vbcrlf & "                   <input name=""intro"" type=""text"" id=""intro"">" & vbcrlf & "                 </label></td>" & vbcrlf & "               </tr>" & vbcrlf & "              </table>" & vbcrlf & "                     </div>" & vbcrlf & "                  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "                           <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                         <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                   </div>" & vbcrlf & "  </form>" & vbcrlf & "</div>" & vbcrlf & ""
	action1="退款列表"
	call close_list(1)
