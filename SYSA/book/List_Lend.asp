...
'Private Sub Class_Initialize
	...
	if open_103_1=3 then
	elseif open_103_1=1 then
		Str_Result=Str_Result&" and addcateid in ("&intro_103_1&")"
	else
		Str_Result=Str_Result&" and addcateid=0"
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and addcateid in ("&W3&") "
	end if
	if D1<>"" then
		Str_Result=Str_Result&" and bk_type in ("&D1&")"
	end if
	if D2<>"" then
		Str_Result=Str_Result&" and bk_returnnum in ("&D2&")"
	end if
	if D3<>"" then
		Str_Result=Str_Result&" and bk_name like '%"&ml.De(D3)&"%'"
	end if
	if D4<>"" then
		Str_Result=Str_Result&" and bk_auther like '%"&ml.De(D4)&"%'"
	end if
	if D5<>"" then
		Str_Result=Str_Result&" and bk_publishing like '%"&ml.De(D5)&"%'"
	end if
	if D6<>"" then
		Str_Result=Str_Result&" and bk_pubnum like '%"&ml.De(D6)&"%'"
	end if
	if D7<>"" then
		Str_Result=Str_Result&" and bk_pagenum >="&ml.isNum(D7)&""
	end if
	if D71<>"" then
		Str_Result=Str_Result&" and bk_pagenum <="&ml.isNum(D71)&""
	end if
	if D8<>"" then
		Str_Result=Str_Result&" and bk_pagesize >="&ml.isNum(D8)&""
	end if
	if D81<>"" then
		Str_Result=Str_Result&" and bk_pagesize >="&ml.isNum(D81)&""
	end if
	if D9<>"" then
		Str_Result=Str_Result&" and bk_format like '%"&ml.De(D9)&"%'"
	end if
	if D10<>"" then
		aa=" 1=2 "
		if instr(D10,3)>0 then
			aa=aa&" or Ld_num=(select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id) "
		end if
		if instr(D10,2)>0 then
			aa=aa&" or (Ld_num>(select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id) and ((select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id))!=0) "
		end if
		if instr(D10,1)>0 then
			aa=aa&" or ((select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id)=0) "
		end if
		Str_Result=Str_Result&" and ("&aa&") "
	end if
	if D11<>"" then
		Str_Result=Str_Result&" and bk_price >="&ml.isNum(D11)&""
	end if
	if D12<>"" then
		Str_Result=Str_Result&" and bk_price <="&ml.isNum(D12)&""
	end if
	if D13<>"" then
		Str_Result=Str_Result&" and bk_paper like '%"&ml.De(D13)&"%'"
	end if
	if D14<>"" then
		Str_Result=Str_Result&" and bk_num >="&ml.isNum(D14)&""
	end if
	if D15<>"" then
		Str_Result=Str_Result&" and bk_num <="&ml.isNum(D15)&""
	end if
	if D16<>"" then
		Str_Result=Str_Result&" and bk_ISBN like '%"&ml.De(D16)&"%'"
	end if
	if D17<>"" then
		Str_Result=Str_Result&" and bk_bh like '%"&ml.De(D17)&"%'"
	end if
	if D18<>"" then
		Str_Result=Str_Result&" and bk_printnum like '%"&ml.De(D18)&"%'"
	end if
	if D19<>"" then
		Str_Result=Str_Result&" and bk_binding like '%"&ml.De(D19)&"%'"
	end if
	if D20<>"" then
		Str_Result=Str_Result&" and Ld_num >="&ml.isNum(D20)&""
	end if
	if D201<>"" then
		Str_Result=Str_Result&" and Ld_num <="&ml.isNum(D201)&""
	end if
	if D21<>"" then
		Str_Result=Str_Result&" and bk_postion like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and Ld_note like '%"&ml.De(D22)&"%'"
	end if
	if M<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&" 00:00:00',Ld_time)>=0 "
'if M<>"" then
	end if
	if K<>"" then
		Str_Result=Str_Result+" and  datediff(s,Ld_time,'"&K&" 23:59:59')>=0 "
'if K<>"" then
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',Ld_rettime)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,Ld_rettime,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if L3<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L3&" 00:00:00',addtime)>=0 "
'if L3<>"" then
	end if
	if L4<>"" then
		Str_Result=Str_Result+" and  datediff(s,addtime,'"&L4&" 23:59:59')>=0 "
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="5"
	if px="1" then
		Ptr_Result=" order by Ld_time desc"
	elseif px="2" then
		Ptr_Result=" order by Ld_time asc"
	elseif px="3" then
		Ptr_Result=" order by Ld_rettime desc"
	elseif px="4" then
		Ptr_Result=" order by Ld_rettime asc"
	elseif px="5" then
		Ptr_Result=" order by addtime desc"
	elseif px="6" then
		Ptr_Result=" order by addtime asc"
	end if
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',addtime)>=0 "
'if ret<>"" then
	end if
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,addtime,'"&ret2&" 23:59:59')>=0 "
'if ret2<>"" then
	end if
	if C<>"" then
		if B="name" then
			Str_Result=Str_Result&" and bk_name like '%"&ml.De(C)&"%'"
		elseif B="auther" then
			Str_Result=Str_Result&" and bk_auther like '%"&ml.De(C)&"%'"
		elseif B="pub" then
			Str_Result=Str_Result&" and bk_publishing like '%"&ml.De(C)&"%'"
		elseif B="bh" then
			Str_Result=Str_Result&" and bk_bh like '%"&ml.De(C)&"%'"
		elseif B="isbn" then
			Str_Result=Str_Result&" and bk_isbn like '%"&ml.De(C)&"%'"
		elseif B="post" then
		end if
	end if''
	If remind <> "" Then
		Set helper = CreateReminderHelper(conn,remind,0)
		Str_Result = Str_Result &" and d.id in (" & helper.listSQL("ids") & ")"
	end if
	...
	if tx=1 then
		title="归还提醒列表"
		Str_Result=Str_Result&" and Ld_rettime<=dateadd(d,3,getdate()) AND Ld_state=1 "
	else
		title="借阅列表"
	end if
	newstate=request("newstate")
	If newstate=1 Then
		Set rs=conn.execute("select fw1,tq1 from setjm2 where ord = 51 and cateid = "&session("personzbintel2007"))
		If Not rs.eof Then
			Str_Result=Str_Result&" and ("&rs("fw1")&"=0 or addcateid="&session("personzbintel2007")&") and ld_rettime<=dateadd(d,"&rs("tq1")&",getdate()) and a.ld_num>(select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id)"
		else
			Str_Result=Str_Result&" and ld_rettime<=dateadd(d,0,getdate()) and a.ld_num>(select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id)"
		end if
		rs.close
	end if
	propm=request("propm")
	If propm=1 Then
		Set rs=conn.execute("select fw1,tq1 from setjm where ord = 51 and intro=1 and cateid = "&session("personzbintel2007"))
		If Not rs.eof Then
			Str_Result=Str_Result&" and ("&rs("fw1")&"=0 or addcateid="&session("personzbintel2007")&") and ld_rettime<=dateadd(d,"&rs("tq1")&",getdate()) and a.ld_num>(select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid=a.id)"
		end if
		rs.close
	...
	if lie_2="" or isnull(lie_2) then lie_2=2
	sql7="select * from O_Lendbookmx "
	set rs7=conn.execute(sql7)
	if not rs7.eof then
		do while not rs7.eof
			sql8="select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid="&rs7("id")
			...
			set rs8=nothing
			if rs7("Ld_num")=Ret_num then
				if rs7("Ld_state")=1 then
					sqltxt="update O_Lendbookmx set Ld_state=2 where id="&rs7("id")
					conn.execute(sqltxt)
				end if
			end if
			rs7.movenext
	...
	set rs7=nothing
	if Ret_num="" or isnull(Ret_num) then Ret_num=0
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_Lend.asp?ret="
	Response.write ret
	Response.write "&ret2="
	Response.write ret2
	Response.write "&B="
	...
	Response.write "&C="
	Response.write C
	Response.write """ method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">"
	Response.write title
	Response.write "</td>" & vbcrlf & "                        <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr>" & vbcrlf & "              <td colspan=""9"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding:0!important"">" & vbcrlf & "                <span id=""kh"">自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" name=""ret"" readonly value="""
	Response.write ret
	Response.write """/><DIV id=daysOfMonth style=""POSITION: absolute""></DIV>&nbsp;至：<input id=""daysOfMonth2Pos"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9"" name=""ret2"" readonly value="""
	Response.write ret2
	Response.write """/><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;<select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                             <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""name"" "
	Response.write ret2
	if B="name" then Response.write "selected"
	Response.write ">书名</option>" & vbcrlf & "                               <option value=""auther"" "
	if B="auther" then Response.write "selected"
	Response.write ">作者</option>" & vbcrlf & "                               <option value=""pub"" "
	if B="pub" then Response.write "selected"
	Response.write ">出版社</option>" & vbcrlf & "                             <option value=""bh"" "
	if B="bh" then Response.write "selected"
	Response.write ">编号</option>" & vbcrlf & "                               <option value=""isbn"" "
	if B="isbn" then Response.write "selected"
	Response.write ">ISBN</option>" & vbcrlf & "                       </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	Response.write """ size=""10""><input name=""tx"" type=""hidden"" value="""
	Response.write tx
	Response.write """><input name=""page_count"" type=""hidden"" value="""
	Response.write page_count
	Response.write """><input name=""px"" type=""hidden"" value="""
	Response.write px
	Response.write """><input name=""lie_1"" type=""hidden"" value="""
	Response.write lie_1
	Response.write """><input name=""lie_2"" type=""hidden"" value="""
	Response.write lie_2
	Response.write """>" & vbcrlf & "                        <input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索"">" & vbcrlf & "                   "
	if open_103_10=3 or (open_103_10=1 and CheckPurview(intro_103_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_Lend.asp?px="
		Response.write px
		Response.write "&A="
		...
		Response.write "&D71="
		Response.write ml.Dep(D71)
		Response.write "&D81="
		Response.write ml.Dep(D81)
		Response.write "&D201="
		Response.write ml.Dep(D201)
		Response.write "&D9="
	...
	if open_103_7=3 or CheckPurview(intro_103_7,trim(session("personzbintel2007")))=True then
	...
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "         </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">书名</div></td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">编号</div></td>" & vbcrlf & "            <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">ISBN</div></td>" & vbcrlf & "                   <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">作者</div></td>" & vbcrlf & "             <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">分类</div></td>" & vbcrlf & "             <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "            <select name=""select76"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                              <option value="""">请选择</option>" & vbcrlf & "                          <option value=""lie_1=1"" "
	if lie_1=1 then
		Response.write "selected"
	end if
	Response.write ">归还状态</option>" & vbcrlf & "                           <option value=""lie_1=2"" "
	if lie_1=2 then
		Response.write "selected"
	end if
	Response.write ">借阅人员</option>" & vbcrlf & "                   </select></div></td>" & vbcrlf & "               <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "              <select name=""select77"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf &               "               <option value="""">请选择</option> "& vbcrlf &                "           <option value=""lie_2=1"" "
	Response.write "selected"
	if lie_2=1 then
		Response.write "selected"
	end if
	Response.write ">归还数量</option>" & vbcrlf & "                           <option value=""lie_2=2"" "
	if lie_2=2 then
		Response.write "selected"
	end if
	Response.write ">借阅数量</option>" & vbcrlf & "                           <option value=""lie_2=3"" "
	if lie_2=3 then
		Response.write "selected"
	end if
	Response.write ">借阅时间</option>" & vbcrlf & "                           <option value=""lie_2=4"" "
	if lie_2=4 then
		Response.write "selected"
	end if
	Response.write ">归还时间</option>" & vbcrlf & "                   </select>" & vbcrlf & "                    </div></td>" & vbcrlf & "                   <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf &           "             <option>-请选择-</option>" & vbcrlf &           "             <option value=""page_count=10"" "
	Response.write "selected"
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.id,a.Ld_fid,a.Ld_bkid,isnull(a.Ld_num,0) as Ld_num,a.Ld_time,a.Ld_rettime,a.Ld_note,a.Ld_state,a.ModifyStamp,b.set_name,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing,d.id as fid,d.addcateid,d.addtime from O_Lendbookmx a "&_
	"inner join O_Lendbook d on a.Ld_fid=d.id "&_
	"inner join O_regbook c on a.Ld_bkid=c.id "&_
	"inner join O_bookset b on c.bk_type=b.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=9><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		...
		do while not rs7.eof
			...
			if open_103_3=3 or CheckPurview(intro_103_3,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                       <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                 "
			else
				Response.write "" & vbcrlf & "                       <input name=""selid"" type=""checkbox"" id=""selid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "            "
			end if
			Response.write "" & vbcrlf & "                 </div></td>" & vbcrlf & "             <td width=""11%"" height=""30"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "              <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_Lend.asp?ord="
				Response.write rs7("Ld_fid")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("Ld_fid")
				Response.write rs7("bk_name")
				Response.write "</a>" & vbcrlf & "          "
			else
				Response.write rs7("bk_name")
			end if
			Response.write "</td>" & vbcrlf & "            <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("bk_bh")
			Response.write "</div></td>" & vbcrlf & "              <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("bk_isbn")
			Response.write "</div></td>" & vbcrlf & "              <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("bk_auther")
			Response.write "</div></td>" & vbcrlf & "              <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><span class=""func"">" & vbcrlf & "                     <div align=""center"">"
			Response.write rs7("set_name")
			Response.write "</div>" & vbcrlf & "                   </span></div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			if lie_1=1 then
				sql8="select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid="&rs7("id")
				set rs8=conn.execute(sql8)
				if not rs8.eof then
				...
				if rs7("Ld_num")=Ret_num then
					Response.write "归还完毕"
				elseif rs7("Ld_num")>Ret_num and Ret_num<>0 then
					Response.write "归还部分"
				elseif Ret_num=0 then
					Response.write "未归还"
				end if
			elseif lie_1=2 then
				sql8="select * from gate where ord="&ml.IsNum(addcateid)
				set rs8=conn.execute(sql8)
				if not rs8.eof then
			...
			if lie_2=1 then
				...
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					Response.write FormatNumber(rs8("Ret_num"),num1_dot,-1)
'if not rs8.eof then
				else
					Response.write FormatNumber("0",num1_dot,-1)
'if not rs8.eof then
				end if
				set rs8=nothing
			elseif lie_2=2 then
				Response.write FormatNumber(rs7("Ld_num"),num1_dot,-1)
'elseif lie_2=2 then
			elseif lie_2=3 then
				ldr=rs7("Ld_time")
				if ldr<>"" and not isnull(ldr) then
					ldr=year(ldr)&"-"&month(ldr)&"-"&day(ldr)
'if ldr<>"" and not isnull(ldr) then
				end if
				Response.write ldr
			elseif lie_2=4 then
...
'if ldr<>"" and not isnull(ldr) then
			...
			Response.write "</div></td>" & vbcrlf & "             <td width=""17%"" height=""30"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_Lend.asp?ord="
				Response.write rs7("Ld_fid")
			...
			sql8="select * from O_RetBookmx where Ret_bkid="&rs7("id")
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Ld_state=1
			else
			...
			set rs8=nothing
			if (open_103_2=3 or CheckPurview(intro_103_2,trim(addcateid))=True) and Ld_state=2 then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_Lend.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
			...
			if open_103_132=1 and rs7("Ld_state")=1 then
				Response.write "" & vbcrlf & "                       <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""window.location=('Add_Return.asp?ord="
				Response.write rs7("id")
				Response.write "')"" value=""归还""/>" & vbcrlf & "                    "
			end if
			if (open_103_3=3 or CheckPurview(intro_103_3,trim(addcateid))=True) and Ld_state=2 then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_Lend.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30"">全选<input name=""chkall"" type=""checkbox""id=""chkall"" value=""chkall"" onClick=""mm(this.form)""></td>" & vbcrlf & "         <td><input name=""submit6"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit6""  style=""cursor:hand""  value=""批量删除"" onClick=""testall()""/>" & vbcrlf & "            "
	if open_103_132=1 then
		Response.write "" & vbcrlf & "             <input name=""submit61"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit61""  style=""cursor:hand""  value=""批量归还"" onClick=""testall1()""/>" & vbcrlf & "             "
	end if
	Response.write "</td>" & vbcrlf & "          </form>" & vbcrlf & "       <form name=""oneself"" action=""List_Lend.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&lie_1="
	...
	Response.write "条信息/页" & vbcrlf & "            <input name=""CurrPage"" type=""text"" size=""4"" onChange=""this.value=this.value.replace(/[^\d]/g,'')""/>" & vbcrlf & "             <input name=""btntz"" type=""submit""  class=""anybutton2"" id=""btntz"" value=""跳转"" />" & vbcrlf & "          "
	if currpage=1 then
	...
	Response.write "" & vbcrlf & "             </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:300px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按借阅时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按借阅时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按归还时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按归还时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按添加时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	dim actinon1
	action1=title
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_Lend.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&lie_1="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "   document.getElementById('data').action=""Del_Lend.asp?px="
	Response.write px
	Response.write "&lie_1="
	Response.write (lie_1)
	Response.write "&lie_2="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "        document.getElementById('data').method=""post""" & vbcrlf & "     document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & "         document.getElementById('data').action=""Add_Return.asp"";" & vbcrlf & "  document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量" & vbcrlf & "var e = form.elements[i];" & vbcrlf & "if (e.name != 'chkall')" & vbcrlf & "e.checked = form.chkall.checked;" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
