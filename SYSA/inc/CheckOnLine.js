...
function getSession(tout) {
    ...
    var url = "../getsession.asp?ac=g&stamp=" + (new Date()).getTime() + "&date1=" + Math.round(Math.random() * 100);
...
function LoginDialog(r)
{
	...
	{
		//出错意味着被弹出窗体程序禁止
		var rtnvalue = window.showModalDialog("../../SYSN/view/init/relogin.ashx?sver=" + window.syssoftversion + "&dlg=" + t.getTime() + "&res=" + r + "&unique=" + UserUniqueID + "&sid=" + UserUniqueSID, window, "status:no;help:no;scroll:no;dialogWidth:600px;dialogHeight:428px");
    }
	catch(e) {
...
function showreloginDivWindow(r) {
	...
	if(!div) {
		...
		document.body.appendChild(div);
		div.style.cssText = "display:none;width:600px;height:428px;position:absolute;z-index:100000;border:1px solid #333;top:160px";
	}
	div.style.left = parseInt((document.body.offsetWidth - 600) / 2) + "px";
	div.style.display = "block";
	div.innerHTML = "<iframe scrolling=no frameborder=0 style='height:100%;width:100%'  src='../../SYSN/view/init/relogin.ashx?sver=" + window.syssoftversion + "&dlg=" + t.getTime() + "&res=" + r + "&unique=" + UserUniqueID + "&sid=" + UserUniqueSID + "'></iframe>"
}
