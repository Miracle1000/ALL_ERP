...
'if strW3<>"" Then
	...
	if W4<>""  then
		...
		sql1="select ord,name,sorce,sorce2,cateid  from gate  where ord in ("&W3&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
			member2=""
		else
			do until rs1.eof
				if (CheckPurview(W1,trim(rs1("sorce")))=True and  CheckPurview(W2,trim(rs1("sorce2")))=True) or rs1("cateid")=1 Then
					list=list&" "&rs1("name")
				else
					W3=replace(W3,rs1("ord")&",","")
				end if
				W3=replace(W3,",0,",",")
				rs1.movenext
			loop
	...
	if member1="1" then
		sqltext="Update mobanMessage set shareinfo='0',share="&member1&" where ord="&ord&""
		conn.execute(sqltext)
	else
		sqltext="Update mobanMessage set shareinfo='"&W3&"',share="&member1&" where ord="&ord&""
		conn.execute(sqltext)
	end if
	conn.close
	set conn = nothing
	Response.write "" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "if (window.opener)" & vbcrlf & "{" & vbcrlf & "       window.opener.location.reload();" & vbcrlf & "        window.close();" & vbcrlf & "}" & vbcrlf & "else " & vbcrlf & "{" & vbcrlf & "  window.location.href=""listMoban.asp """ & vbcrlf & "}" & vbcrlf & "</script>"
