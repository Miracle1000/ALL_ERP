...
'if StoreCapacity>0 then
	...
	Sub Bill_OnPageInit
		Response.write "<style>body{height:auto;position:initial;}#comm_itembarbg{height:43px!important;}#setAutoComplete{padding:1px}</style>"
	end sub
	Sub Bill_OnInit(ByVal bill, ByVal t)
		Dim title,act : act = app.getText("act")
		title = app.iif(bill.id=0,"工序汇报","工序汇报修改")
		If act = "change" Then  title = "工序汇报变更"
		Bill.maxspan = 2
		bill.edit=True
		bill.loadJs =True
		bill.loadEasyUI = True
		Dim rs,bh ,ord , sql ,pgdid , proid ,wpname ,workcenter ,prenum ,cateid ,num1 ,needQA,creator, creatorname ,isupdate
		pgdid = app.getText("pgdid")
		proid = app.getText("proid")
		cateid = app.getText("cateid")
		If len(pgdid&"")>0 Then pgdid = app.base64.deurl(pgdid)
		If len(proid&"")>0 Then proid = app.base64.deurl(proid)
		If len(cateid&"")>0 Then cateid = app.base64.deurl(cateid)
		num1 = 1
		needQA = 1
		sql = ""
		creator = 0
		If bill.id = 0 Then
			isupdate= False
			cn.execute("delete from M_ProcedureProgres where del=7 and creator="& Info.User )
			bh = ""
			set rs = cn.execute("EXEC erp_getdjbh 9031,"& Info.User )
			bh=rs(0).value
			rs.close
			set rs=nothing
			If bh = "error" Then bh = ""
			sql="Insert Into M_ProcedureProgres(PrefixCode,creator,inDate,bh,del) values('PP','" & Info.User & "','" & now() & "','"  & bh & "','"& 7 & "')"
			cn.execute(sql)
			ord =CDbl(app.GetIdentity("M_ProcedureProgres","id","creator",""))
			bill.setBillId ord
			bill.title = title
			creatorname = Info.username
			If Len(proid)>0 Then
				sql = "select d.wpname, d.wcenter  , (case when isnull(b.qnum,0)>=a.NumMake then a.NumMake else sum(case when isnull(r.result,0) in (0,1) then isnull(r.num1,0) else 0 end) end) as prenum ,isnull(w.ProgresNum,1) as cnum ,isnull(w.result,0) as needQA "&_
				"   from M_WorkAssigns a "&_
				"   inner join M_WFP_Assigns w on w.WFID = a.WProID  "&_
				"   inner join M_WorkingProcedures d on d.id=w.wpid "&_
				"   left join M_ProcedureProgres r on r.M_WorkAssigns = a.ID and w.id = r.[Procedure] "&_
				"   left join (  " &_
				"           select m.WAID , sum(NumQualified) as qnum  " &_
				"           from M_MaterialProgres m   " &_
				"           inner join M_MaterialProgresDetail t on t.MPID = m.id and m.del=0 and t.del=0 " &_
				"           group by m.WAID  " &_
				"   ) b on b.WAID = a.id "&_
				"   where a.id ="& pgdid &" and w.id =" & proid & " " &_
				"   group by w.id, d.wpname,d.wcenter ,a.NumMake ,b.qnum ,w.ProgresNum ,w.result "
				Set rs = cn.execute(sql)
				if rs.eof = False Then
					wpname =  rs.fields(0).value
					workcenter = rs.fields(1).value
					prenum = rs.fields(2).value
					num1 = rs.fields(3).value
					needQA = rs.fields(4).value
				end if
				rs.close
			end if
		else
			isupdate= True
			sql = "select l.*, d.wcenter as workcenter , (case when isnull(b.qnum,0)>=a.NumMake then a.NumMake else r.num1 end) as prenum,isnull(w.result,0) as needQA "&_
			"   from M_ProcedureProgres l "&_
			"   left join M_WorkAssigns a  on l.M_WorkAssigns = a.ID "&_
			"   left join M_WFP_Assigns w on w.id = l.[Procedure] "&_
			"   left join M_WorkingProcedures d on d.id=w.wpid "&_
			"   left join ( "&_
			"           select M_WorkAssigns ,[Procedure] ,sum(num1) as num1 from M_ProcedureProgres where del=0 and result in (0,1) group by M_WorkAssigns ,[Procedure]  "&_
			"   ) r on r.M_WorkAssigns = l.M_WorkAssigns and r.[Procedure] = l.[Procedure] "&_
			"   left join (  " &_
			"           select m.WAID , sum(NumQualified) as qnum  " &_
			"           from M_MaterialProgres m   " &_
			"           inner join M_MaterialProgresDetail t on t.MPID = m.id and m.del=0 and t.del=0  " &_
			...
			bill.title = title
			If Bill.exists Then
				bh = bill.Data.fields("bh").value
				creator = bill.Data.fields("creator").value
				needQA = bill.Data.fields("needQA").value
			end if
			creatorname = getGateName(creator)
		end if
		bill.ahonegp = False
		Bill.groups(0).title="工序汇报编号：" & bh &" &nbsp;&nbsp; 操作员：" & Info.username
		Bill.fields.add1("汇报编号","bh","hidden","varchar",True,false,True,False).defvalue = bh
		bill.canscan = True
		With Bill.fields.add1("派工单","M_WorkAssigns","text","int",True,true,True,true)
		.colspan = 2
		.defvalue = pgdid
		.source = "autocomplete:M_WorkAssigns"
		End With
		With Bill.fields.add1("工序","Procedure","text","int",True,True,True,true)
		.colspan = 2
		.defvalue = proid
		.source = "autocomplete:Procedure"
		End With
		With Bill.fields.add1("汇报主题","title","text","varchar",True,True,false,False)
		.colspan = 2
		.defvalue = wpname
		End With
		With Bill.fields.add1("工作中心","workcenter","text","varchar",True,false,false,True)
		.colspan = 2
		.defvalue =workcenter
		.source = "autocomplete:M_WorkingCenters"
		End With
		With Bill.fields.add1("生产人员","cateid","gate","int",True,True,false,True)
		.source="produce"
		.colspan = 2
		.defvalue = cateid
		End With
		With Bill.fields.add1("标识/序列号", "codeProduct", "text", "varchar",  true, true, true , False)
		.colspan = 2
		.maxlimit = 200
		End With
		With Bill.fields.add1("此前完工量", "prenum", "number", "number",  true, false, false , False)
		.colspan = 2
		.defvalue = prenum
		End With
		With Bill.fields.add1("本次完工量", "num1", "number", "number",  true, true, true , False)
		.colspan = 2
		.defvalue = num1
		End With
		bill.fields.addHidden "needQA", "int",  needQA
		With Bill.fields.add1("质检结果", "result", "radio", "int",  true, true, True , False)
		.source = "options:通过=1;返工=2;作废=3"
		.defvalue = 1
		.colspan = 2
		.hidden = app.iif(needQA=1,false,true)
		End With
		...
		If isupdate =False Then
			bill.Buttons.GetItemByDbName("insert").topvisible = true
			bill.Buttons.GetItemByDbName("insert").bottomvisible = True
		end if
	end sub
	Function getGateName(ord)
		Dim rs , s :s = ""
		Set rs = cn.execute("select name from gate where ord in (" & ord & ")")
		While rs.eof = False
	...
	Function App_setAutoComplete(ByRef field ,ByVal stype , ByVal value , ByRef url , ByRef mtext)
		Dim rs
		mtext = ""
		If value&""="" Then value = 0
		Select Case stype
		Case "gate" :
		mtext = getGateName(value)
		Case "M_WorkingCenters" :
		If Len(field.value&"")=0 Then value = field.defvalue
		If value&""="" Then value = 0
		Set rs = cn.execute("select wcname from M_WorkingCenters where id =" & value & "")
		if rs.eof = False Then
			mtext = mtext & rs.fields(0).value & " "
		end if
		rs.close
		Case "M_WorkAssigns" :
		If Len(field.value&"")=0 Then value = field.defvalue
		If value&""="" Then value = 0
		Set rs = cn.execute("select title from M_WorkAssigns where id =" & value & "")
		if rs.eof = False Then
			mtext = mtext & rs.fields(0).value & " "
		end if
		rs.close
		If field.edit=True Then
			field.linktype = "M_WorkAssigns"
		end if
		Case "Procedure" :
		If Len(field.value&"")=0 Then value = field.defvalue
		If value&""="" Then value = 0
		Set rs = cn.execute("select d.wpname from M_WFP_Assigns w inner join M_WorkingProcedures d on d.id=w.wpid where w.id =" & value & "")
		if rs.eof = False Then
			mtext = mtext & rs.fields(0).value & " "
		end if
		rs.close
		If field.edit=True Then
			field.linktype = "Procedure"
			url = "{@M_WorkAssigns}"
		end if
		End Select
	end function
	Function app_AjaxWindow_setAutoComplete(byref ajaxpage, ByVal datatype, ByVal fid , ByVal fv , ByRef html)
		Dim keycode : keycode = app.getText("keycode")
		Dim lvw : Set lvw = New listview
		lvw.border = 0
		lvw.PageButtonAlign = "right"
		lvw.oldPageSizeUI = True
		...
		lvw.pagesize = 500
		lvw.colresize = False
		ajaxpage.backgroundColor = "white"
		Dim almsg
		Select Case datatype
		Case "M_WorkAssigns" :
		ajaxpage.width = 900
		ajaxpage.height = 420
		lvw.pagesize = 10
		lvw.id = "M_WorkAssigns"
		lvw.PageBar = true
		lvw.sql = "select top 500 * from dbo.erp_list_pgd_fun("& Info.user &",2 , '"& now() &"') order by 派工时间 desc"
		lvw.headers("ID").display="none"
		With lvw.headers.insert("选择", "",2)
		.formattext="code:checkPGD(rs(""id""),"""& fid &""" , rs(""派工主题"") )"
		.align="left"
		End With
		lvw.headers("派工单号").dbtype="str"
		lvw.headers("派工主题").dbtype="str"
		lvw.headers("产品编号").dbtype="str"
		With lvw.headers("产品名称")
		.formattext="code:toHtml(@value)"
		.dbtype="str"
		End With
		lvw.headers("型号").dbtype="str"
		lvw.headers("单位").width= 60
		With lvw.headers("数量")
		.width= 100
		.dbtype = "number"
		End With
		lvw.headers("状态").width= 100
		lvw.headers("#hide_Status_WA").display="none"
		lvw.headers("#hide_Cateid_WA").display="none"
		lvw.headers("派工人员").display="none"
		lvw.headers("派工时间").display="none"
		lvw.headers("添加人").display="none"
		lvw.headers("添加时间").display="none"
		If lvw.recordcount >= 500 Then
			almsg = "<span style='position:relative;top:-22px;color:red'>&nbsp;提示：只显示最近500条派工记录</span>"
'If lvw.recordcount >= 500 Then
		end if
		Case "Procedure" :
		Dim M_WorkAssigns : M_WorkAssigns = app.getint("M_WorkAssigns")
		ajaxpage.width = 600
		ajaxpage.height = 400
		lvw.sql = "exec [erp_childlist_pgd_ProcedureProgres] "& Info.user &","& M_WorkAssigns &",'" & keycode &"'"
		lvw.headers("id").display="none"
		lvw.headers("加工次序").width="80"
		With lvw.headers.insert("选择", "",2)
		.formattext="code:checkGX(rs(""id""),"""& fid &""" , rs(""工序名称"") ,rs(""cid""),rs(""#hide_allnum"") , rs(""派工数量"") ,rs(""cnum""),rs(""result""))"
		.align="left"
		End With
		With lvw.headers("派工数量")
		.width="80"
		.dbtype="number"
		End With
		With lvw.headers("完工量")
		.title = "已汇报数量"
		.width="80"
		.dbtype="number"
		.formattext = "code:FormatNumber(rs(""#hide_allnum""), Info.FloatNumber ,-1,0,0)"
		.dbtype="number"
		End With
		With lvw.headers("完工率")
		.title = "汇报百分比"
		.width="80"
		.dbtype="number"
		.formattext="code:showPercent(rs(""#hide_allnum"").value ,rs(""派工数量"").value)"
		End With
		lvw.headers("详情").display="none"
		lvw.headers("返工量").display="none"
		lvw.headers("返工率").display="none"
		lvw.headers("报废量").display="none"
		lvw.headers("报废率").display="none"
		lvw.headers("cid").display="none"
		lvw.headers("cnum").display="none"
		lvw.headers("result").display="none"
		lvw.headers("#hide_allnum").display="none"
		End Select
		html =lvw.HTML & almsg
	end function
	Function lvw_onCallback(ByRef lvw)
		If lvw.id = "M_WorkAssigns" Then
			With lvw.headers.insert("选择", "",2)
			.formattext="code:checkPGD(rs(""id""),""M_WorkAssigns"" , rs(""派工主题"") )"
			.align="left"
			End With
		end if
	end function
	Sub app_onScanComplete
		on error resume next
		Dim rs
		Dim data : data = app.getText("data")
		Dim json : json = "{msg:'false'}"
		If InStr(data,":")>0 Or InStr(data,"：")>0 Then
			data = Replace(data,"：",":")
			json = "{msg:'true'"
			Dim datatype : datatype = Split(data,":")(0)
			Dim dataContent :dataContent = Split(data,":")(1)
			json = json & ",datatype:'"& datatype &"',dataContent:'"& dataContent &"' "
			Select Case datatype
			Case "SCPG":
			Set rs = cn.execute("select id, title from M_WorkAssigns where id="& dataContent )
			If rs.eof=False Then
				json = json & ",id:'"& rs("id").value &"'"
				json = json & ",fid:'M_WorkAssigns'"
				json = json & ",title:'"& rs("title").value &"'"
			end if
			rs.close
			Case "SCGX":
			Dim cid : cid = app.getInt("cid")
			Set rs = server.CreateObject("adodb.recordset")
			rs.open "exec [erp_childlist_pgd_ProcedureProgres] "& Info.user &","& cid &",'" & dataContent &"'" , cn,1,1
			If rs.recordcount>1 Then
				json = json & ",errmessage:'此派工单下工艺流程工序重复,请手动选择'"
			elseIf rs.eof=False Then
				json = json & ",errmessage:''"
				json = json & ",id:'"& rs("id").value &"'"
				json = json & ",fid:'Procedure'"
				json = json & ",title:'"& rs("工序名称").value &"'"
				json = json & ",wcname:'"& rs("cid").value &"'"
				json = json & ",prenum:'"& rs("完工量").value &"'"
				json = json & ",cnum:'"& rs("cnum").value &"'"
				json = json & ",needQA:'"& Abs(rs("result").value) &"'"
			else
				json = json & ",errmessage:'ERR'"
			end if
			rs.close
			Case "ZJJG":
			Select Case dataContent
			Case "1": json = json & ",r:'1'"
			Case "2": json = json & ",r:'2'"
			Case "3": json = json & ",r:'3'"
			Case Else
			json = json & ",result:'0'"
			End Select
			Case "YGDA" , "YGTX" :
			If InStr(dataContent,"QrUrl=V")>0 Then
				Dim typeid
				Set rs = cn.execute("select top 1 sourceID,ctype from C2_CodeItems where ctype in (2,3) and id="& Replace(dataContent,"QrUrl=V" ,"" )&"")
				If rs.eof=False Then
					dataContent = rs(0).value
					typeid = rs(1).value
				else
					dataContent = "0"
					typeid = 0
				...
				rs.close
				If typeid = 2 Then
					Set rs = cn.execute("select top 1 userid from hr_person where  id="& dataContent &"")
					If rs.eof=False Then
						dataContent = rs(0).value
					else
						dataContent = "0"
			...
			Set rs = cn.execute("select ord, name from GATE where ord="& dataContent )
			If rs.eof=False Then
				json = json & ",errmessage:''"
				json = json & ",id:'"& rs("ord").value &"'"
				json = json & ",name:'"& rs("name").value &"'"
			else
				json = json & ",errmessage:'不是有效的生产人员(条码/二维码),请手动选择'"
			end if
			rs.close
			Case else:
			json = "{msg:'false'"
		...
		If Err.number<>0 Then json = "{msg:'false'}"
		Response.write json
	end sub
	Function showPercent(v , NumMake)
		v = CDbl(v)/CDbl(NumMake)
		v = v * 100
		showPercent = FormatNumber(v, Info.FloatNumber ,-1,0,0) &"%"
		v = v * 100
	end function
	Function checkGX(id , fid ,title ,wcname , prenum ,NumMake , cnum ,needQA)
		If CDbl(prenum)<CDbl(NumMake) Then
			checkGX = "<center><a href='javascript:void(0)' onclick='checkGx("& id &" , """& fid &""","""& title &""","""& wcname &""","""& prenum &""" ,"""& cnum &""","""& abs(needQA) &""")'>点击选择该工序</a></center>"
		end if
	end function
	Function checkPGD(id , fid ,title )
		title = app.htmltotext(title)
		checkPGD = "<center><a href='javascript:void(0)' onclick='checkPgd("& id &" ,"""& fid &""","""& title &""")'>点击选择</a></center>"
	end function
	Function toHtml(title)
		If Len(title)>0 Then
			toHtml = app.htmltotext(title)
		else
			toHtml = ""
		end if
	end function
	Function Bill_OnSave(ByVal bill)
		Dim Rs ,sql ,bh , M_WorkAssigns , Procedure ,title ,cateid ,codeProduct ,num1 ,result ,intro , sys_ad_model ,act
		bh = app.getText("bh")
		M_WorkAssigns = app.getInt("M_WorkAssigns")
		Procedure = app.getInt("Procedure")
		If M_WorkAssigns = 0 or Procedure=0 Then
			bill.showSaveResultEx "保存失败，请选择汇报派工单以及工序！",false,"none",""
			Exit Function
		end if
		title = app.getText("title")
		cateid = app.getInt("cateid")
		codeProduct = app.getText("codeProduct")
		num1 = app.getText("num1")
		If num1&""="" Or Not isnumeric(num1) Then
			bill.showSaveResultEx "保存失败，请录入本次完工量！",false,"none",""
			Exit Function
		end if
		act = app.getText("act")
		If act = "change" Then Call CopyBillBeforChange(cn , -11 , bill.id , "工序汇报变更" , info.user)
		act = app.getText("act")
		result = app.getint("result")
		intro = app.getText("intro")
		Dim creator : creator= 0
		sql = "select * from M_ProcedureProgres where id= " & bill.id
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.open sql,cn,1,3
		If rs.eof=false Then
			rs("PrefixCode")="PP"
			rs("M_WorkAssigns")=M_WorkAssigns
			rs("Procedure")= Procedure
			rs("bh")= bh
			rs("title")= title
			rs("codeProduct")= codeProduct
			rs("cateid")= cateid
			rs("num1")= num1
			rs("result")= result
			rs("intro")= intro
			rs("del")=0
			rs.update
			creator = rs("creator").value
		else
			bill.showSaveResultEx "保存失败，请重新录入！" & Err.description,false,"self","add.asp"
			Exit Function
		end if
		rs.close
		sys_ad_model = app.getInt("sys_ad_model")
		If sys_ad_model = 1 Then
			bill.showSaveResultEx "",false,"self","add.asp?act="&act&"&pgdid="&app.base64.pwurl(M_WorkAssigns)&"&proid="& app.base64.pwurl(Procedure) &"&cateid="& app.base64.pwurl(cateid)
		else
			If act = "content" Or  act = "change" Then
				Response.write "<script>window.alert('保存成功');try{top.opener.RefreshWindow();}catch(e){top.opener.location.reload();};"
				Response.write "top.close();"
				Response.write "</script>"
			ElseIf act="list" Then
				bill.ReportRefresh "保存成功"
			else
				bill.showSaveResultEx "保存成功",false,"self","list.asp"
			end if
		end if
		Bill_OnSave = True
	end function
