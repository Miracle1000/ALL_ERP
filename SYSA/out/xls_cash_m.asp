...
'If not isNumeric(B) Then
	...
	if m1="" then
		m1=getQuarterFrom(date())
	end if
	if m2="" then
		m2=getQuarterEnd(date())
	end if
	m=datediff("m",m1,m2)+1
	m2=getQuarterEnd(date())
	if B>0 then
...
'if B>0 then
	...
	if m>12 then
		Response.write "<script>alert('显示数据最多不能超过12个月');window.location.href='cash_m.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	...
	if intCols1=0 or intCols2=0 then
		Response.write "<script>alert('收入项与支出项至少各保留一项');window.location.href='../tongji/cash_m.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlWorksheet.WriteHtmlCell "<center><b>日期</b></center>" , "rowspan=2 width=200 "
...
'do while not rs.eof
				...
				For k=0 to m-1
					tempSort1 = ""
					date1=dateadd("m",k,m1)
					if k>0 then
						date1=year(date1)&"-"&month(date1)&"-1"
'if k>0 then
					end if
					date3=year(date1)&"-"&month(date1)&"-1"
'if k>0 then
					date2=dateadd("m",1,date3)-1
'if k>0 then
					if date2>cDate(m2) then date2=m2
					str2=year(date1)&"年"&month(date1)&"月"
					hl=1
					sort1 =  rs("sort1")
					...
					xlWorksheet.WriteHtmlCell "<center>"& str2 &"</center>" , ""
					...
					"          inner join sortbank b on a.bank=b.id and a.sort>0 and b.bz="&rs("id") &"  and a.date1>='"&date1&"' and a.date1<='"&date2&"' "&_
					...
					do while not rs1.eof
						if rs1("sorce")=1 then
							...
							allIncomeTotal=allIncomeTotal+money1
							money1=zbcdbl(rs1("money1"))
						elseif rs1("sorce")=0 then
							money1=-zbcdbl(rs1("money2"))
'elseif rs1("sorce")=0 then
							allExpensesTotal=allExpensesTotal+abs(money1)
'elseif rs1("sorce")=0 then
						end if
...
'For Each key In colsTotal
			...
			doSave("现金流量每月统计")
				...
				Response.write """><font class=""red""><strong><u>下载导出的现金流量每月统计</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
