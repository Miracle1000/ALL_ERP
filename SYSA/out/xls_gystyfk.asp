...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from tel where ord=bankout3.company) as companyname,"&_
			"(select name from gate where ord=bankout3.cateid) as cateidname"&_
			" from bankout3 "&Str_Result&" "&px_Result&" "
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "供应商退预付款列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>供应商退预付款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!供应商退预付款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("供应商退预付款列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的供应商退预付款列表</u></strong></font></a></p>"
