...
'bigsystemtype = ".mozi"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=3 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_3_11=0
		intro_3_11=0
	else
		open_3_11=rs1("qx_open")
		intro_3_11=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	if open_3_11=3 then
		list_tj=""
	elseif open_3_11=1 then
		list_tj="and cateid in ("&intro_3_11&")"
	else
		list_tj="and cateid=0"
	end if
	dim rs,sql,Str_Result,Str_Result2,catesafe,sorce_user,sorce_user2
	Str_Result="where ((del=1 "&list_tj&")  or  ((share like '%"&session("personzbintel2007")&"%' or share='1') and del=1) or (addcate="&session("personzbintel2007")&" and del=1) or (cateorder1="&session("personzbintel2007")&" and del=1))"
	Str_Result2="and ((del=1 "&list_tj&")  or  ((','+replace(share,' ','')+',' like '%,"&session("personzbintel2007")&",%' or share='1') and del=1) )"
	Str_power=""
	Str_power22=""
	Str_power33=""
	sorce=0
	...
	if open_3_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_3_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'if E<>"" then
	...
	if F<>"" then
		Str_Result=Str_Result+"and  complete2='"&F&"'"
'if F<>"" then
		Str_Result2=Str_Result2+"and  complete2='"&F&"'"
'if F<>"" then
	end if
	dim sort,sort1
	if E<>"" then
	...
	if F<>"" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select sort1 from sortjh2 where ord="&F&""
		rs.open sql,conn,1,1
		sort1=rs("sort1")
		sort1=sort1&">>"
		rs.close
	...
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td cla	ss=""place"">项目预计金额统计</td>" & vbcrlf & "                       " & vbcrlf & "            <td  align=""left""><strong>"
	Response.write sort
	Response.write sort1
	Response.write "人员分布"
	Response.write m1name
	Response.write m2name
	Response.write "" & vbcrlf & "    </strong>" & vbcrlf & "                 </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "<form method=""POST"" action=""jh3_m.asp?width_tj="
	...
	if E<>"" then
		...
		set rss=conn.execute("select * from sortonehy  where gate2=21 and  ord<>"&rs7("id")&" order by gate1 desc")
		while not rss.eof
			sele=""
		...
		rs7.close
		set rs7=nothing
		Response.write "" & vbcrlf & "<option value="""" >项目状态</option>" & vbcrlf & ""
	else
		'Response.write "" & vbcrlf & "<option value="""" >项目状态</option>" & vbcrlf & ""
		set rss=conn.execute("select * from sortonehy where gate2=21 order by gate1 desc")
		while not rss.eof
			sele=""
			if request("sort")&""=rss("id")&"" then
				sele="selected"
	...
	if F<>"" then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select id,sort1 from sortjh2 where  ord="&F&""
		rs7.open sql7,conn,1,1
		Response.write " " & vbcrlf & "<option value="""
		Response.write rs7("id")
		Response.write """ >"
		Response.write rs7("sort1")
		Response.write "</option> " & vbcrlf & ""
		set rss=conn.execute("select id ,sort1 from sortjh2 where ord<>"&rs7("id")&"  order by gate1 desc")
		while not rss.eof
			sele=""
		...
		rs7.close
		set rs7=nothing
		Response.write "" & vbcrlf & "<option value="""" >项目阶段</option>" & vbcrlf & ""
	else
		'Response.write "" & vbcrlf & "<option value="""" >项目阶段</option>" & vbcrlf & ""
		set rss=conn.execute("select id ,sort1 from sortjh2  order by gate1 desc")
		while not rss.eof
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "           <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""window.location.href='jh3_m.asp?type_tj=2&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""window.location.href='jh3_m.asp?type_tj=3&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""jh3_m.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""jh3_m.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""jh3_m.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""jh3_m.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""jh3_m.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	if open_3_10=1 or open_3_10=3  then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='jh3_m.asp?type_tj=4&sort="
		Response.write E
		Response.write "&sort1="
		Response.write F
		Response.write "&ret="
	...
	if counts<=0 then
		...
		do until rs1.eof
			set rs=server.CreateObject("adodb.recordset")
			sql="select isnull(sum(money3),0) as money1 from chance where cateid="&rs1("ord")&"  "&Str_Result2&" "
			rs.open sql,conn,1,1
			...
			set rs=nothing
			title=rs1("name")
			lburl="../chance/result.asp?W3="&rs1("ord")&"&E="&E&"&F="&F&"&ret="&m1&"&ret2="&m2&""
			total1(j,1)=numcg1
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			total2(j,1)=numcg1
			total2(0,1)="#FF0000,1.5,1,2,项目分布"
			total2(j,2)=lburl
			total3(j,0)=title
			total3(j,1)=numcg1
			total3(j,2)=lburl
