...
'Set rs = conn.execute("SELECT top 1 1 FROM (SELECT id,CAST(short_str AS INT) cateid FROM dbo.split('"& intro_33_3 &"',',')) a INNER JOIN (select distinct s.cateid from send s inner join kuout k on k.ord=s.kuout and s.del=1 and k.sort1 in(1,4) and k.order1 in("& htords &")) b on a.cateid=b.cateid AND a.cateid>0")
	...
	Response.write "" & vbcrlf & "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>合同利润明细表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">合同利润明细表导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出合同利润明细表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=000000>" & vbcrlf &"<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Dim formulaIdx : formulaIdx = GetSetopenValue(2018030701, 2)
	dim MODULES
	MODULES=session("zbintel2010ms")
	set rs=server.CreateObject("adodb.recordset")
	set rs1=server.CreateObject("adodb.recordset")
	strSql="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=5 and sort2=11"
	rs.open strSql,conn
	if rs.eof then
		open_5_11=0
		intro_5_11=0
	else
		open_5_11=rs("qx_open")
		intro_5_11=rs("qx_intro")
	end if
	rs.close
	if open_5_11=3 then
		Str_Result=" where 1=1"
	elseif open_5_11=1 then
		Str_Result=" where ord in("&intro_5_11&") and ord>0"
	else
		Str_Result = " where 1=0 "
		open_5_11=" where 1=2"
	end if
	strSql="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=5 and sort2=17"
	rs.open strSql,conn
	if rs.eof then
		open_5_17=0
		intro_5_17=0
	else
		open_5_17=rs("qx_open")
		intro_5_17=rs("qx_intro")
	end if
	rs.close
	if open_5_10=3 then
		Str_Result = Str_Result & " and 1=1 "
	elseif open_5_10=1 then
		Str_Result = Str_Result & " and ord in ("&intro_5_10&") and ord>0 "
	else
	...
	Dim datetype : datetype = request("datetype")
	If datetype = "" Then datetype = "contract"
	...
	CurrPage=int(getInt(request("CurrPage")))
	if CurrPage=0 then
		CurrPage=1
	end if
	Dim paybackCondition : paybackCondition = ""
	If datetype = "payback" Then
		if m1<>"" And isDate(m1) Then
			paybackCondition = paybackCondition & " and cc.date5>='"&m1&"' "
		end if
		if m2<>"" And isDate(m2) Then
			paybackCondition = paybackCondition & " and cc.date5<='"&cdate(m2)&"' "
		end if
	else
		if m1<>"" And isDate(m1) Then
		...
		if m2<>"" And isDate(m2) Then
			Str_Result=Str_Result & " and date3<='"&cdate(m2)&"' "
		end if
	end if
	B=Conver(request("B"))
	C=Conver(request("C"))
	Dim htbh , htzt , khmc , gates
	htbh = request("asft_htbh")
	htzt = request("asft_htzt")
	khmc = request("asft_khmc")
	fhjz = request("asearch_f_fhjz")
	khqy = Replace(request("A2")," ","")
	khhy = request("asft_khhy")
	if htbh<>"" then
		Str_Result=Str_Result+" and contractNo like '%"& htbh &"%'"
'if htbh<>"" then
	end if
	if htzt<>"" then
		Str_Result=Str_Result+" and contractTitle like '%"& htzt &"%'"
'if htzt<>"" then
	end if
	if khmc<>"" then
		Str_Result=Str_Result+" and companyName like '%"& khmc &"%'"
'if khmc<>"" then
	end if
	If Len(khqy) > 0 Then
		khqy = " and b.area in (" & khqy & ")"
	end if
	If Len(khhy) > 0 Then
		khhy = " and b.trade in (" & khhy & ")"
	end if
	if C<>"" then
		if B = "xsry" then
...
'if B = "xsry" then
		elseif B = "khmc" then
			Str_Result=Str_Result+" and companyName like '%"& C &"%'"
'elseif B = "khmc" then
		elseif B = "khbh" then
			Str_Result=Str_Result+" and companyNo like '%"& C &"%'"
'elseif B = "khbh" then
		elseif B = "htmc" then
			Str_Result=Str_Result+" and contractTitle like '%"& C &"%'"
'elseif B = "htmc" then
		elseif B = "htbh" then
			Str_Result=Str_Result+" and contractNo like '%"& C &"%'"
'elseif B = "htbh" then
		end if
	end if
	If Len(fhjz&"")>0 Then
...
'elseif fhjz="7" then
		elseif fhjz = "100" then
			Str_Result = Str_Result + " and (zt1=1 or zt1=2)"
'elseif fhjz = "100" then
		elseif fhjz = "101" then
...
'elseif fhjz = "101" then
	...
	mlFiledStr = ""
	Select Case formulaIdx&""
	Case "1" : mlFiledStr = " (contractTotal-taxTotal+thTaxTotal-productCost-payTotal-thTotal+thCost) "
'Select Case formulaIdx&""
	Case "2" : mlFiledStr = " (contractTotal-productCost-payTotal-thTotal+thCost) "
'Select Case formulaIdx&""
	Case "3" : mlFiledStr = " (paybackTotal-taxTotal+thTaxTotal-productCost-payTotal-thTotal+thCost) "
'Select Case formulaIdx&""
	Case "4" : mlFiledStr = " (paybackTotal-productCost-payTotal-thTotal+thCost) "
'Select Case formulaIdx&""
	End Select
	if request("total1")<>"" then
		total1=getInt(request("total1"))
		Str_Result=Str_Result&" and " & mlFiledStr & ">="&total1
	end if
	if request("total2")<>"" then
		total2=getInt(request("total2"))
		Str_Result=Str_Result&" and " & mlFiledStr & "<="&total2
	end if
	pageOrder=int(getInt(pageOrder))
	...
	case 3
	Str_Order = " order by " & mlFiledStr & " desc"
	case 4
	Str_Order = " order by " & mlFiledStr & " asc"
	case 5
	Str_Order = " order by contractTotal desc"
	case 6
	Str_Order = " order by contractTotal asc"
	case 7
	Str_Order = " order by contractTitle desc"
	case 8
	Str_Order = " order by contractTitle asc"
	case 9
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xApp.init Me, conn
			Set xsheet = xApp.sheets.add("合同利润明细表")
			HeaderStr=HeaderStr & "客户名称"
			HeaderStr=HeaderStr & ",客户编号"
			HeaderStr=HeaderStr & ",合同名称"
			HeaderStr=HeaderStr & ",合同编号"
			If model Then
				HeaderStr=HeaderStr & ",发货进展"
				HeaderStr=HeaderStr & ",利润总额"
				HeaderStr=HeaderStr & ",利润率"
				HeaderStr=HeaderStr & ",购买总额"
				if showSkMoney=1 then HeaderStr=HeaderStr & ",收款总额"
				HeaderStr=HeaderStr & ",税额"
				HeaderStr=HeaderStr & ",产品成本"
				If showFyMoney = 1 Then HeaderStr=HeaderStr & ",费用总额"
				if showThMoney = 1 Then
					HeaderStr=HeaderStr & ",退货总额"
					HeaderStr=HeaderStr & ",退货成本"
				end if
				if open_5_24 = 1 Then
					HeaderStr=HeaderStr & ",销售毛利（进价）"
					HeaderStr=HeaderStr & ",毛利率（进价）"
					HeaderStr=HeaderStr & ",建议进价成本"
				end if
				HeaderStr=HeaderStr & ",销售人员"
				HeaderStr=HeaderStr & ",所在部门"
			else
				HeaderStr=HeaderStr & ",利润总额"
				HeaderStr=HeaderStr & ",利润率"
				HeaderStr=HeaderStr & ",购买总额"
			...
			sumProfitTotal=0
			sumProductCost=0
			sumProductCostJinjia=0
			sumPayTotal=0
			sumThTotal=0
			sumThCost=0
			gates = ""
			if(request("W3"))<>"" then
			gatesop = Replace(request("as_gs_ty") & "", " ","")
			gates =  request("W3")
			If InStr(gatesOP,"1") > 0 And InStr(gatesOP,"2") > 0  then
				gates = " and ((a.cateid in (" & gates & ") and a.cateid > 0 ) or (a.addcate in (" & gates & ") and a.addcate > 0 ) )"
			Elseif InStr(gatesOP,"1") = 0 And InStr(gatesOP,"2") > 0  Then
				gates = " and (a.addcate in (" & gates & ") and a.addcate > 0 )"
			Elseif InStr(gatesOP,"1") > 0 And InStr(gatesOP,"2") = 0  Then
				gates = " and (a.cateid in (" & gates & ") and a.cateid > 0 )"
			else
				gates = ""
			end if
		end if
		strTable=" from(" & vbcrlf &_
		"select a.ord contractId,a.title contractTitle,a.htid contractNo,a.date3" & vbcrlf &_
		",b.ord companyId,b.name companyName,b.khid companyNo,c.name,c.ord,d.sort1 departName" & _
		",contractTotal=(case "& showHtMoney &" when 1 then (select isnull(sum(isnull(money2,money1)),0) from contract where ord=a.ord) else 0 end)" & _
		",paybackTotal=(case "& showSkMoney &" when 1  then  (" & vbcrlf &_
		"select isnull(sum(cc.money1),0)*isnull(e.hl,1) from payback cc where cc.contract=a.ord and cc.complete=3 and cc.del=1 " & paybackCondition & vbcrlf &_
		") else 0 end) " & vbcrlf & _
		",thTotal=(case "& showThMoney &" when 1  then isnull((" & vbcrlf &_
		"select SUM(thl.money1*dbo.gethl(thl.bz,thl.date1,getdate())) from contractth th INNER JOIN contractthlist thl ON thl.caigou=th.ord AND th.del=1 and thl.del=1 and thl.contract=a.ord group by thl.contract " & vbcrlf &_
		"),0) else 0 end) " & vbcrlf & _
		",sthTotal=(case "& showThMoney &" when 1  then isnull((" & vbcrlf &_
		"isnull((select SUM(thl.money1*dbo.gethl(thl.bz,thl.date1,getdate())) from contractth th INNER JOIN contractthlist thl ON thl.caigou=th.ord AND th.del=1 and thl.del=1 and thl.contract=a.ord group by thl.contract),0) - "&_
		",sthTotal=(case "& showThMoney &" when 1  then isnull((" & vbcrlf &_
		"isnull((select isnull(sum(thd.money2*dbo.gethl(th.bz,th.date3,getdate())),0) from contractthListDetail thd "&_
		"inner join contractth th on thd.contractth = th.ord and th.del=1 and th.sp=0 "&_
		"and thd.thtype='GOODS' and thd.contract=a.ord),0) " & vbcrlf &_
		"),0) else 0 end) " & vbcrlf & _
		",thCost=isnull((case "& showThMoney &" when 1  then (" & vbcrlf &_
		"(case a.paybacktype when 1 then  " & vbcrlf &_
		"  (select isnull(sum(c.price1*d.num1),0) from kuoutlist2 c,contractthlist d,contractth e where e.del=1 and d.caigou=e.ord and c.del=1 and d.del=1 and c.id=d.kuoutlist2 and d.contract=a.ord) " & vbcrlf &_
		"else   " & vbcrlf &_
		"  (select SUM(ISNULL((CASE ISNULL(c.kuinlist,0) WHEN 0 THEN c.price1*d.num1 ELSE isnull(convert(decimal(30,8),ROUND(REPLACE(r.price1,',',''),8)),0)*d.num1 END),c.price1*d.num1)) " & vbcrlf &_
		"  FROM kuoutlist2 c INNER JOIN contractthlist d ON c.id=d.kuoutlist2 and c.del=1 and d.del=1 " & vbcrlf &_
		"  INNER JOIN contractth e ON e.del=1 and e.ord=d.caigou and d.contract=a.ord " & vbcrlf &_
		"  LEFT JOIN kuinlist r ON r.id=c.kuinlist AND r.del=1) " & vbcrlf &_
		"end) " & vbcrlf &_
		") else 0 end),0)" & vbcrlf & _
		",productCost=(case "& showCB &" when 1  then (" & vbcrlf &_
		"SELECT ISNULL((CASE ISNULL(paybacktype,0) WHEN 0 THEN ( " & vbcrlf &_
		"          select isnull(sum(d.FinaMoney),0) as money1 from kuoutlist2 d " & vbcrlf &_
		"          where d.sort1=1 and d.contract=a.ord and d.del=1)  " & vbcrlf &_
		"  WHEN 1 THEN (" & vbcrlf &_
		"          select isnull(sum(k.FinaMoney),0) money1 " & vbcrlf &_
		"          from kuoutlist2 k " & vbcrlf &_
		"          where (k.sort1=1 or k.sort1=4) and k.contract=a.ord and k.del=1)" & vbcrlf &_
		"  ELSE 0 END),0) money1 " & vbcrlf &_
		"FROM contract WHERE ord=a.ord" & vbcrlf &_
		") else 0 end) " & vbcrlf & _
		",productCostJinjia=isnull((select sum(tpricejy) tpricejy from contractlist WITH(NOLOCK) where del=1 and contract=a.ord),0)" & vbcrlf &_
		",payTotal=(case "& showFyMoney &" when 1  then (" & vbcrlf &_
		"select isnull(sum(p.money1*isnull(e2.hl,1)),0) from pay p left join hl e2 on e2.date1=p.date1 and e2.bz=(select bz from f_pay where id = p.fid) where p.del=1 and p.complete=3 and p.contract=a.ord " & vbcrlf &_
		") else 0 end)  " & vbcrlf & _
		"taxValueSql(""salesProfit_2"", "")" & vbcrlf &_
		"thTaxValueSql(""salesProfit_2"", "")" & vbcrlf &_
		",a.zt1,a.zt2," & vbcrlf &_
		"num1=(" & vbcrlf &_
		"select isnull(sum(num1),0) from contractlist where contract=a.ord and del=1" & vbcrlf &_
		")," & vbcrlf &_
		"num2=(" & vbcrlf &_
		"select isnull(sum(num2),0) from contractlist  where contract=a.ord and del=1" & vbcrlf &_
		")" & _
		" from contract a WITH(NOLOCK)" & vbcrlf & _
		" left join tel b WITH(NOLOCK) on a.company=b.ord " & vbcrlf & _
		" left join gate c WITH(NOLOCK) on a.cateid=c.ord " & vbcrlf &  _
		" left join gate1 d WITH(NOLOCK) on c.sorce=d.ord " & vbcrlf & _
		" left join hl e on e.date1=a.date3 and e.bz = a.bz " & vbcrlf & _
		" where a.del=1 " &gates& khqy & khhy & ") as t1 " & Str_Result
		strSql="select distinct contractId,contractTitle,contractNo,companyId,companyNo,companyName,name,ord,departName,contractTotal,paybackTotal,thTotal,sthTotal,thCost,productCost,productCostJinjia,payTotal,taxTotal,thTaxTotal,zt1,zt2,num1,num2  "&strTable & Str_Order
		...
		do while not rs.eof
			i=i+1
'do while not rs.eof
			contractTotal=cdbl(formatNumber(rs("contractTotal"),num_dot_xs,-1))
'do while not rs.eof
			paybackTotal=cdbl(formatNumber(rs("paybackTotal"),num_dot_xs,-1))
'do while not rs.eof
			productCost=cdbl(formatNumber(rs("productCost"),num_dot_xs,-1))
'do while not rs.eof
			productCostJinjia=cdbl(formatNumber(rs("productCostJinjia"),num_dot_xs,-1))
'do while not rs.eof
			payTotal=cdbl(formatNumber(rs("payTotal"),num_dot_xs,-1))
'do while not rs.eof
			taxTotal=cdbl(formatNumber(rs("taxTotal"),num_dot_xs,-1))
'do while not rs.eof
			if showThMoney = 1 Then
				thTotal=cdbl(formatNumber(rs("thTotal"),num_dot_xs,-1))
'if showThMoney = 1 Then
				thCost=cdbl(formatNumber(rs("thCost"),num_dot_xs,-1))
'if showThMoney = 1 Then
				sthTotal=cdbl(formatNumber(rs("sthTotal"),num_dot_xs,-1))
'if showThMoney = 1 Then
				thTaxTotal=cdbl(formatNumber(rs("thTaxTotal"),num_dot_xs,-1))
'if showThMoney = 1 Then
			else
				thTotal = CDbl(0)
				thCost = CDbl(0)
			...
			profitTotal = getHtMlMoney2(formulaIdx, contractTotal, paybackTotal, taxTotal, thTaxTotal, productCost, payTotal, thTotal, sthTotal, thCost)
			profitTotalJinjia = getHtMlMoney2(formulaIdx, contractTotal, paybackTotal, taxTotal, thTaxTotal, productCostJinjia, payTotal, thTotal, sthTotal, thCost)
			profitRate=getHtMlRate3(formulaIdx, profitTotal, contractTotal, paybackTotal, thTotal, sthTotal)
			profitRateJinjia=getHtMlRate3(formulaIdx, profitTotalJinjia, contractTotal, paybackTotal, thTotal, sthTotal)
			showTaxValue = tdTaxValueHtml("salesProfit_2",1,formulaIdx,taxTotal,thTaxTotal)
			if showTaxValue&""= "" then showTaxValue = 0 else showTaxValue = cdbl(showTaxValue)
			tdShowThMoney = tdThMoneyHtml("salesProfit_2",1,formulaIdx, thTotal, sthTotal)
			if tdShowThMoney&"" = "" then tdShowThMoney = 0 else tdShowThMoney = cdbl(tdShowThMoney)
			if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				pageProductCost=pageProductCost+productCost
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				pageProfitTotal=pageProfitTotal+profitTotal
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				pageThCost=pageThCost+thCost
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			end if
			pageProductCostJinjia=pageProductCostJinjia+productCostJinjia
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pageProfitTotalJinjia=pageProfitTotalJinjia+profitTotalJinjia
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pageContractTotal=pageContractTotal+contractTotal
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pagePayBackTotal=pagePayBackTotal+paybackTotal
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pagePayTotal=pagePayTotal+payTotal
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pageTaxTotal = pageTaxTotal + showTaxValue
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			pageThTotal=pageThTotal+tdShowThMoney
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			zt1=rs("zt1")
			zt2=rs("zt2")
			num_1=cdbl(formatNumber(zbcdbl(rs("num1")),num1_dot,-1))
			zt2=rs("zt2")
			num_2=cdbl(formatNumber(zbcdbl(rs("num2")),num1_dot,-1))
			zt2=rs("zt2")
			if num_1>num_2 then
				zt3=1
			elseif num_1=num_2 and num_2>0 then
				zt3=2
			else
				zt3=0
			end if
			...
			If model Then
				...
				if zt2="0" then
					fhjzstr=fhjzstr& "未发货"
				elseif zt2="1" then
					fhjzstr=fhjzstr& "部分发货"
				elseif zt2="2" then
					fhjzstr=fhjzstr& "发货完毕"
				end if
			end if
			xsheet.writestr rs("companyName")
			xsheet.writestr rs("companyNo")
			xsheet.writestr rs("contractTitle")
			xsheet.writestr rs("contractNo")
			If model Then
				xsheet.writestr fhjzstr
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writeMoney Formatnumber(profitTotal,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writestr Formatnumber(profitRate,num_dot_xs,-1)&"%"
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				else
					xsheet.writespace(1)
					xsheet.writespace(1)
				end if
				xsheet.writeMoney Formatnumber(contractTotal,num_dot_xs,-1)
				xsheet.writespace(1)
				if showSkMoney=1 then xsheet.writeMoney Formatnumber(paybackTotal,num_dot_xs,-1)
				xsheet.writespace(1)
				xsheet.writeMoney showTaxValue
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writeMoney Formatnumber(productCost,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				else
					xsheet.writespace(1)
				end if
				if showFyMoney = 1 Then xsheet.writeMoney Formatnumber(payTotal,num_dot_xs,-1)
				xsheet.writespace(1)
				if showThMoney = 1 Then
					xsheet.writeMoney tdThMoneyHtml("salesProfit_2",1,formulaIdx,thTotal,sthTotal)
					if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
						xsheet.writeMoney Formatnumber(thCost,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					else
						xsheet.writespace(1)
					end if
				...
				if open_5_24 = 1 Then
					xsheet.writeMoney Formatnumber(profitTotalJinjia,num_dot_xs,-1)
'if open_5_24 = 1 Then
					xsheet.writestr Formatnumber(profitRateJinjia,num_dot_xs,-1)&"%"
'if open_5_24 = 1 Then
					xsheet.writeMoney Formatnumber(productCostJinjia,num_dot_xs,-1)
'if open_5_24 = 1 Then
				end if
				xsheet.writestr rs("name")
				xsheet.writestr rs("departName")
			else
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writeMoney Formatnumber(profitTotal,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writeMoney Formatnumber(profitRate,num_dot_xs,-1)&"%"
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				else
					xsheet.writespace(1)
					xsheet.writespace(1)
				...
				if showSkMoney=1 then xsheet.writeMoney Formatnumber(paybackTotal,num_dot_xs,-1)
				xsheet.writespace(1)
				xsheet.writeMoney Formatnumber(showTaxValue,num_dot_xs,-1)
				xsheet.writespace(1)
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xsheet.writeMoney Formatnumber(productCost,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
...
'if open_5_24 = 1 Then
			...
			j=j+1
			xsheet.writestr rs("departName")
			currProcV = Clng(j/intCount*100)
			Call ClientClosedExit
			If PreProcV < currProcV Then
				Response.write "<script>exportProcBar.showExcelProgress(" & currProcV & "," & intCount & "," & j & ")</script>"
				PreProcV = currProcV
				Response.Flush
			end if
			rs.movenext
			...
			If i > 40000 Then
				If pageCount = 1 then
					xsheet.title = "合同利润明细表(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = "合同利润明细表(" & pageCount & "页)"
				Set xsheet = xApp.sheets.add("合同利润明细表(" & pageCount & "页)")
				i = 0
				xsheet.showheader headerStr
				xsheet.movenext
			end if
		...
		If model Then
			xsheet.writespace(4)
			xsheet.WriteCellB "合计"
			if open_5_17=3 or open_5_17=1 then
				xsheet.writeMoney Formatnumber(pageProfitTotal,num_dot_xs,-1)
'if open_5_17=3 or open_5_17=1 then
			else
				xsheet.writespace(1)
			end if
			xsheet.writespace(1)
			xsheet.writeMoney Formatnumber(pageContractTotal,num_dot_xs,-1)
			xsheet.writespace(1)
			if showSkMoney=1 then xsheet.writeMoney Formatnumber(pagePaybackTotal,num_dot_xs,-1)
			xsheet.writespace(1)
			xsheet.writeMoney Formatnumber(pageTaxTotal,num_dot_xs,-1)
			xsheet.writespace(1)
			if open_5_17=3 or open_5_17=1 then
				xsheet.writeMoney Formatnumber(pageproductCost,num_dot_xs,-1)
'if open_5_17=3 or open_5_17=1 then
			else
				xsheet.writespace(1)
			end if
			xsheet.writeMoney Formatnumber(pagepayTotal,num_dot_xs,-1)
			xsheet.writespace(1)
			if showThMoney = 1 Then
				xsheet.writeMoney Formatnumber(pagethTotal,num_dot_xs,-1)
'if showThMoney = 1 Then
				if open_5_17=3 or open_5_17=1 then
					xsheet.writeMoney Formatnumber(pagethCost,num_dot_xs,-1)
'if open_5_17=3 or open_5_17=1 then
				else
					xsheet.writespace(1)
				end if
			...
			if open_5_24 = 1 Then
				xsheet.writeMoney Formatnumber(pageProfitTotalJinjia,num_dot_xs,-1)
'if open_5_24 = 1 Then
				xsheet.writespace(1)
				xsheet.writeMoney Formatnumber(pageProductCostJinjia,num_dot_xs,-1)
				xsheet.writespace(1)
			end if
		else
			xsheet.writespace(3)
...
'if open_5_17=3 or open_5_17=1 then
			...
			if showFyMoney = 1 Then xsheet.writeMoney Formatnumber(pagepayTotal,num_dot_xs,-1)
			xsheet.writespace(1)
			if showThMoney = 1 Then
				xsheet.writeMoney Formatnumber(pagethTotal,num_dot_xs,-1)
'if showThMoney = 1 Then
...
'if open_5_24 = 1 Then
		...
		tfile=Server.MapPath(xApp.cFileName("合同利润明细表_"&session("name2006chen")) & ".xls")
		xApp.save tfile
		xApp.dispose
		tfile = xApp.HexEncode(tfile)
		Set xApp = Nothing
		action1="合同利润明细表导出"
		Response.write "<script>exportProcBar.showExcelProgress(100," & intCount & "," & intCount & ")</script>"
		Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'合同利润明细表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
		call close_list(1)
		Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
		Response.write tfile
		Response.write """><font class=""red""><strong><u>下载导出的合同利润明细表</u></strong></font></a></p>"
