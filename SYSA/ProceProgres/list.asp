...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		...
		If Len(pgdid)>0 Then
			pgdid = app.base64.deurl(pgdid)
		else
			pgdid = 0
		end if
		ppid = app.getText("ppid")
		If Len(ppid)>0 Then
			ppid = app.base64.deurl(ppid)
		else
			ppid = 0
		end if
		rpt.title = "工序汇报列表"
		rpt.addsort "工序汇报编号","bh"
		rpt.addsort "汇报主题","title"
		rpt.addsort "对应派工单","M_WorkAssigns"
		rpt.addsort "汇报人","creator"
		rpt.addsort "汇报时间","indate"
		rpt.AddField2 "","select","B", "汇报主题"&Chr(1)&"title"&Chr(2)&"对应派工单"&Chr(1)&"M_WorkAssigns"&Chr(2)&"汇报人"&Chr(1)&"creator", "", "","汇报编号=#bh"
		rpt.addfield "","text","C", "", "", ""
		rpt.aSearch.addfield "工序汇报编号","text","bh",""
		rpt.aSearch.addfield "汇报主题","text","title",""
		rpt.aSearch.addfield "质检结果","checks","result","sql:select '通过' sort1,1 id union all select '返工' sort1,2 id union all select '作废' sort1,3 id union all select '无需质检' sort1,0 id "
		rpt.aSearch.addfield "汇报人", "gates", "creator", ""
		rpt.aSearch.addfield "汇报日期","dates","indate",""
		...
		If app.power.existsPower(5031,3) Then strHandle = "批量删除"
		If Len(strHandle)>0 Then rpt.setBatHandle "id", strHandle '"批量删除,批量共享"
		rpt.Settings.clear
		rpt.canExcel = app.power.existsPower(5031,10)
		rpt.canPrint = app.power.existsPower(5031,7)
		Dim sql : sql = "exec [erp_proceprogres_List] "& Info.User &","& pgdid &","& ppid &",@B ,@C ,@bh,@title,@result,@creator,@indate_0 ,@indate_1, &SearchMode ,&excelmode,&pagesize ,&pageindex, &ReportSort"
		rpt.sql = sql
	end sub
	Sub App_doBatHandle(cmd, data, result)
		Dim rs
		If cmd = "批量删除" Or  cmd = "删除" Then
			Dim pwr_del : pwr_del = app.power.getPowerIntro(5031,3)
			If Len(data&"")=0 Then data = 0
			Set rs =cn.execute("select id , (case when ("& Len(pwr_del) &"=0 or CHARINDEX(','+CAST(creator  as varchar(12))+',',',"& pwr_del &",')>0) then 'url' else 'none' end) as url  from M_ProcedureProgres where id in ("& data &")")
			If Len(data&"")=0 Then data = 0
			While rs.eof =False
				If rs("url") = "url" Then
					cn.execute("update M_ProcedureProgres set del = 1 where id = "& rs("id"))
				else
	...
	Sub app_onCreateList(lvw,rpd)
		...
		.title = "汇报主题"
		.formatText = "code:titlehtml(@value ,rs(""id""), rs(""url""))"
		.dbtype="str"
		.width = "100"
		End With
		With lvw.headers("WAtitle")
		.title = "对应派工单"
		.formatText = "code:linkWorkAssigns(@value ,rs(""M_WorkAssigns""), rs(""pgd_url""))"
		.dbtype="str"
		.width = "100"
		End With
		...
		.width = "150"
		.formatText = "code:buttonhtml(@value,rs(""id""),rs(""up""),rs(""del"") )"
		End With
		lvw.headers("up").display="none"
		lvw.headers("del").display="none"
		lvw.headers("pgd_url").display="none"
		lvw.headers("M_WorkAssigns").display="none"
	end sub
	Sub App_OnListviewExcel(lvw)
		...
		.width = "100"
		End With
		With lvw.headers("WCName")
		.title = "工作中心"
		.dbtype="str"
		.width = "100"
		End With
		With lvw.headers("WPName")
		.title = "工序名称"
		.dbtype="str"
		.width = "100"
		End With
	...
	Function titlehtml(value ,ord, url )
		Dim s : s = value
		If url="url" And ord>0 Then
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')"">"& value &"</a>"
'If url="url" And ord>0 Then
		end if
		titlehtml = s
	end function
	Function linkWorkAssigns(value ,ord, url)
		Dim s : s = value
		If url="url" And ord>0 Then
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('../manufacture/inc/Readbill.asp?orderid=8&ID=" & ord & "&SplogId=0&vTime=1465983844002','newwin','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">"& value &"</a>"
'If url="url" And ord>0 Then
		end if
		linkWorkAssigns = s
	end function
	Function buttonhtml(value,ord ,up,del)
		Dim s : s = ""
		If value="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')"">详情</button>&nbsp;"
'If value="url" And ord>0 Then
		end if
		if up="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('add.asp?ord=" & app.base64.pwurl(ord) & "&act=list','newwin','width=' + 900 + ',height=' + 520 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">修改</button>&nbsp;"
'if up="url" And ord>0 Then
		end if
		if del="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:delpropro(" & ord & ")"">删除</button> "
		end if
		buttonhtml = s
	end function
%>
