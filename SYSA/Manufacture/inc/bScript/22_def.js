...
function bill_onLoad(){
	...
	var d1 = document.getElementsByName("MT6")[0]
	var d2 = document.getElementsByName("MT7")[0]
	
	if(window.ActiveXObject){
		d1.attachEvent("onpropertychange",UpdateFcDateList);
		d2.attachEvent("onpropertychange",UpdateFcDateList);
	} else {
		$(d1).change(UpdateFcDateList);
		$(d2).change(UpdateFcDateList);
	}

	if(document.getElementById("MT10_MFRadio0")){ 

		document.getElementById("MT10_MFRadio0").onmouseup = function(){
			document.getElementsByName("MT11")[0].value = ""
			document.getElementsByName("MT11")[0].title = ""
			window.UpdateFcDateList();
		}

		document.getElementById("MT10_MFRadio1").style.display = "none"
		document.getElementById("MT10_MFRadio1").parentElement.nextSibling.style.display = "none"
		document.getElementById("MT10_MFRadio1").onmouseup = function(){
			document.getElementsByName("MT11")[0].value = ""
			document.getElementsByName("MT11")[0].title = ""
			window.UpdateFcDateList();
		}

		document.getElementById("MT10_MFRadio2").style.display = "none"
		document.getElementById("MT10_MFRadio2").parentElement.nextSibling.style.display = "none"
		document.getElementById("MT10_MFRadio2").onmouseup = function(){
			document.getElementsByName("MT11")[0].value = ""
			document.getElementsByName("MT11")[0].title = ""
			window.UpdateFcDateList();
		}

		document.getElementById("MT10_MFRadio3").style.display = "none"
		document.getElementById("MT10_MFRadio3").parentElement.nextSibling.style.display = "none"
		document.getElementById("MT10_MFRadio3").onmouseup = function(){
			document.getElementsByName("MT11")[0].value = ""
			document.getElementsByName("MT11")[0].title = ""
			window.UpdateFcDateList();
		}
...
function UpdateFcDateList(){
	...
	{
		var dates = d2.getTime()-d1.getTime();
		var times = Math.floor(dates/(1000*60*60*24)); //1095 3年
		if (times>1095){
			p.innerHTML = "<span>起始日期到截止日期不能超过3年。</span>";
			p.style.color="red";
			return;
		}	
	}
	...
	{
		var m1 = (i==d1.getFullYear() ? d1.getMonth() : 0);
		var m2 = (i==d2.getFullYear() ? d2.getMonth() : 11);
		for (var ii=m1 ;ii<= m2 ; ii ++ )
		{
			if (iii==0)
			{
				html = html + "<tr>"
...
function selectDayItem(td){
	...
	td.setAttribute("wtype",window.currwtypebutton.getAttribute("wtype"));
		...
		if(dat.getTime()<d1.getTime() || dat.getTime() > d2.getTime()){
			...
			for (var i=0;i<window.wdlistArray.length ;i++ )
			{
				...
				if(dat.getTime()>=d01.getTime() && dat.getTime() <=d02.getTime()){
					wIndex = parseInt(window.wdlistArray[i][0]);
					i = window.wdlistArray.length;
				}
			}
			html  = html + "<td wkdat=1 title='" + works[wIndex][0] + "' cY=" + year + " cM=" + month + " cD=" + d + "  wtype=" + wIndex + " class=fcdlcell style='background-Color:" + works[wIndex][1] + ";" +  (dlistReadMode ? "" : "cursor:url(../../images/smico/wCursor.cur)") + ",default' onmouseover = 'if(window.currwcursorselect==1){selectDayItem(this)}' onmousedown='selectDayItem(this)'>" + getonedayHTML(year,month+1,d,0,wIndex) + "</td>"
		}
...
function updatelvwArrayByDateList(){
	...
	{
		var td = cells[i]
		if(td.getAttribute("wkdat")=="1" && td.getAttribute("wtype")!=0){
			var tdcy = td.getAttribute("cY");
			var tdcm = td.getAttribute("cM");
			var tdcd = td.getAttribute("cD");
			var d1 = new Date(tdcy,tdcm,tdcd)
			if(Date.DateDiff("d",d0,d1)!=1 || cW !=td.getAttribute("wtype"))
			{
				cW = td.getAttribute("wtype");
				ii = ii + 1
				window.wdlistArray[ii] = new Array("","0", cW , tdcy + "-" + (tdcm*1+1) + "-" + tdcd, tdcy + "-" + (tdcm*1+1) + "-" + tdcd , cells[i].remark ? cells[i].remark : "")
			}
			else{
				window.wdlistArray[ii][4] = tdcy + "-" + (tdcm*1+1) + "-" + tdcd
			}
			d0 = d1
