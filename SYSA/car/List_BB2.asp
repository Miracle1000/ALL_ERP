...
'if CurrPage="" then CurrPage=1
	...
	NumDigits=0
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=102 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_102_1=rssort(0)
		intro_102_1=rssort(1)
	else
		open_102_1=0
		intro_102_1=""
	end if
	if intro_102_1="" then intro_102_1="0"
	if open_102_1="0" then
		strCate=strCate&" and 1=2"
	elseif open_102_1="1" then
		strCate=strCate&" and addcateid in ("&intro_102_1&")"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=102 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_102_14=rssort(0)
		intro_102_14=replace(rssort(1)," ","")
	else
		open_102_14=0
		intro_102_14=""
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=102 and sort2=10 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_102_10=rssort(0)
	else
		open_102_10=0
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=102 and sort2=7 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_102_7=rssort(0)
	else
		open_102_7=0
	...
	rssort.close
	set rssort=nothing
	baseSQL="select PAGE_COUNT_NUM from ( SELECT PAGE_TOP_NUM 车牌号,主键_ID,操作类型,操作时间,操作人员,操作人员_ID,添加人员_ID,车辆类型,车辆名称,车辆型号,发动机号,底盘号,车辆_ID,记录_ID FROM ("&_
	"select a.carcode as 车牌号,a.free1 as 车辆_ID,a.free2 as 记录_ID,a.id as 主键_ID,a.cztype as 操作类型,a.cztime as 操作时间,b.name as 操作人员,a.czname as 操作人员_ID,a.addcateid as 添加人员_ID,"&_
	"a.cartype as 车辆类型,a.carname as 车辆名称,a.carxh as 车辆型号,a.carfdjh as 发动机号,a.cardph as 底盘号"&_
	" from O_carlog a"&_
	" LEFT JOIN gate b ON a.czname=b.ord where 1=1 "&strCate&" "&_
...
'if sflg="" then
	...
	for each reqPara in request.querystring
		if instr(1,reqPara,"car_")>0 then
	...
	if C<>"" then
		if B=1 then
			strCondition=strCondition&" and 车牌号 like '%"&ml.De(C)&"%'"
		elseif B=2 then
			strCondition=strCondition&" and 车辆名称 like '%"&ml.De(C)&"%'"
		elseif B=3 then
			strCondition=strCondition&" and 车辆型号 like '%"&ml.De(C)&"%'"
		elseif B=4 then
			strCondition=strCondition&" and 发动机号 like '%"&ml.De(C)&"%'"
		elseif B=5 then
			strCondition=strCondition&" and 底盘号 like '%"&ml.De(C)&"%'"
		end if
	end if
	if strW3<>"" then strCondition=strCondition&" and 操作人员_ID in ("&strW3&")"
	if car_carcode<>"" then strCondition=strCondition&" and 车牌号 like '%"&ml.De(car_carcode)&"%'"
	if car_name<>"" then strCondition=strCondition&" and 车辆名称 like '%"&ml.De(car_name)&"%'"
	if car_xh<>"" then strCondition=strCondition&" and 车辆型号 like '%"&ml.De(car_xh)&"%'"
	if car_dph<>"" then strCondition=strCondition&" and 底盘号 like '%"&ml.De(car_dph)&"%'"
	if car_fdjh<>"" then strCondition=strCondition&" and 发动机号 like '%"&ml.De(car_fdjh)&"%'"
	arrtype=split(car_type,",")
	for i=0 to Ubound(arrtype)
		if arrlist="" then
			arrlist="'"&replace(arrtype(i)," ","")&"'"
		else
			arrlist=arrlist&",'"&replace(arrtype(i)," ","")&"'"
		end if
	next
	if car_type<>"" then strCondition=strCondition&" and 车辆类型 in ("&arrlist&")"
	if car_type1<>"" then strCondition=strCondition&" and 操作类型 in ("&car_type1&")"
	dim gather
	set gather=new GatherList
	with gather
	.digits=NumDigits
	.title="车辆日志"
	.FieldsSettingIndex=5004
	...
	function CustomShowFields(idx,rsobj)
		...
		case "操作类型"
		if rsobj("操作类型").value=1 then
			valuetest="车辆申请"
		elseif rsobj("操作类型").value=2 then
			valuetest="车辆派车"
		elseif rsobj("操作类型").value=3 then
			valuetest="车辆返还"
		elseif rsobj("操作类型").value=4 then
			valuetest="车辆保险"
		elseif rsobj("操作类型").value=5 then
			valuetest="车辆维护"
		end if
		Response.write valuetest
		case "车牌号"
		ord=rsobj("车辆_ID")
		if open_102_14=3 or (open_102_14=1 and IsInList(intro_102_14,rsobj("添加人员_ID"),",")) then
			Response.write "<a href=""../car/Detail_carData.asp?ord="&ord&""" target=""_blank"" title=""查看车辆详情"">"&rsobj("车牌号")&"</a>"
		else
			Response.write rsobj("车牌号")
		end if
		case "关联单据"
		ord=rsobj("关联单据")
		...
		if rsobj("单据类型").value="用品入库" or  rsobj("单据类型").value="入库删除" or rsobj("单据类型").value="入库修改" then
			...
			set rs=conn.execute(sql)
			if not rs.eof then
				if open_102_14=3 or (open_102_14=1 and IsInList(intro_102_14,rsobj("操作人员_ID"),",")) then
...
'if CanFormatNumber(rsobj(idx).name) then
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<style>" & vbcrlf & ".IE5 .ser_top  input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""display:none;width:700px;height:550px;padding:5px;background: #fafafa;top:0px;""  closed=""true"" >" & vbcrlf & "<form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "<div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc;width:98%;min-height:300"">" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "             <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"" width=""80"">操作人员</td>" & vbcrlf & "                  <td align=""left"" colspan=""3"">" & vbcrlf & "                            "
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
		...
		Response.write "                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">车牌号</td>" & vbcrlf & "                     <td align=""left""><input name=""car_carcode"" type=""text"" onFocus=""this.select();"" value="""
		Response.write car_carcode
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">车辆名称</td>" & vbcrlf & "             <td width=""177"" align=""left""><input name=""car_name"" type=""text"" onFocus=""this.select();"" value="""
		Response.write car_name
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "              </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">车辆型号</td>" & vbcrlf & "                   <td align=""left""><input name=""car_xh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write car_xh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">底盘号</td>" & vbcrlf & "                     <td align=""left""><input name=""car_dph"" type=""text"" class=""searchInput"" id=""car_dph"" style=""width:110px"" onFocus=""this.select();"" value="""
		Response.write car_dph
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">操作时间</td>" & vbcrlf & "                   <td align=""left"">" & vbcrlf & "                 <input type=""text"" name=""ret3"" id=""ret3"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"">&nbsp;-&nbsp;<input type=""text"" name=""ret4"" id=""ret4"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""></td>" & vbcrlf & "                        <td align=""right"">发动机号</td>" & vbcrlf & "                   <td align=""left""><input name=""car_fdjh"" type=""text"" class=""searchInput"" id=""car_xlh"" style=""width:110px"" onFocus=""this.select();"" value="""
		Response.write car_fdjh
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">车辆类型</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       "
		sql7="select * from O_carSet where setType=2 order by setsort desc"
		set rs7=conn.execute(sql7)
		if not rs7.eof then
			do while not rs7.eof
				Response.write "" & vbcrlf & "                       <input name=""car_type"" type=""checkbox"" value="""
		...
		set rs7=nothing
		Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">操作类型</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       <input name=""car_type1""type=""checkbox"" value=""1"">车辆申请" & vbcrlf & "                     <input name=""car_type1"" type=""checkbox"" value=""2"">车辆派车" & vbcrlf & "                    <input name=""car_type1"" type=""checkbox"" value=""3"">车辆返还" & vbcrlf & "                    <input name=""car_type1"" type=""checkbox"" value=""4"">车辆保险" & vbcrlf & "                    <input name=""car_type1"" type=""checkbox"" value=""5"">车辆维护</td>" & vbcrlf & "              </tr>" & vbcrlf & "           </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:20px; padding:8px"">" & vbcrlf & "         <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "   <input type=""hidden"" name=""page_count"" value="""
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """>" & vbcrlf & "                <a class=""easyui-linkbutton"" icon=""icon-search"" href=""javascript:void(0)"" onClick=""document.getElementById('searchform').submit();return false;"">检索</a>" & vbcrlf & "               <a class=""easyui-linkbutton"" icon=""icon-cancel"" href=""javascript:void(0)"" onClick=""$('#dd').window('close');"">取消</a>" & vbcrlf & "                <a class=""easyui-linkbutton"" icon=""icon-undo"" href=""javascript:void(0)"" onClick=""resetForm();"">清空</a>" & vbcrlf & " </div>" & vbcrlf & "</form>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & ""
		Response.write px
	end sub
...
'sub CurstomButtons()
		...
		Response.write ">车牌号</option>" & vbcrlf & "<option value=""2"" "
		if B=2 then
			Response.write "selected"
		end if
		Response.write ">车辆名称</option>" & vbcrlf & "<option value=""3"" "
		if B=3 then
			Response.write "selected"
		end if
		Response.write ">车辆型号</option>" & vbcrlf & "<option value=""4"" "
		if B=4 then
			Response.write "selected"
		end if
		Response.write ">发动机号</option>" & vbcrlf & "<option value=""5"" "
		if B=5 then
			Response.write "selected"
		end if
		Response.write ">底盘号</option>" & vbcrlf & "</select>" & vbcrlf & "<input type=""text"" name=""C"" value="""
		Response.write C
