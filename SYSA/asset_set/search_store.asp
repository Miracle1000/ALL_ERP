...
'If sc>0 Then v = v & ";"
    ...
    sql="select isnull(max(depth),0) from O_assSetType"
    set rs=conn.execute(sql)
    Depth=rs(0)
    rs.close
    set rs=nothing
    for i=0 to Depth
        sql="select id,sort1,gate1,parentid,depth,storecode,isLeef,isnull((select count(*) from O_assSettype where depth=a.depth and parentid=a.parentid and id>a.id),0) as f from O_assSettype a where Depth="&i&" "&_
        "and (RootID in (select RootID from O_assSettype where sort1 like '%"&kv&"%') or RootID in (select b.RootID from O_assSetname a inner join O_assSettype b on a.sort=b.id and a.sort1 like '%"&kv&"%')"&_
        ") order by gate1 asc"
        set rs=conn.execute(sql)
        j=1
        while not rs.eof
