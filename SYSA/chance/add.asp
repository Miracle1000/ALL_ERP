...
'getExtendedValue=""
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select ts,jz,bt from celue where sort1=3 "
	rs.open sql,conn,1,1
	if rs.eof then
		ts_kh=""
		jz_kh=""
		...
		ts_kh=replaceSpace(rs("ts"))
		jz_kh=replaceSpace(rs("jz"))
		bt_kh=replaceSpace(rs("bt"))
	end if
	rs.close
	set rs=Nothing
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<link href=""../inc/jquery.autocomplete.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script defer=true>" & vbcrlf & "function frameResize(){" & vbcrlf & "  try{" & vbcrlf & "            var tb3 = I3.document.body.getElementsByTagName(""table"")[0];" & vbcrlf & "              var tb31 = I31.document.body.getElementsByTagName(""table"")[0];" & vbcrlf & "     document.getElementById(""mxlist"").style.height=$(tb3).height()+10+""px"";" & vbcrlf & "     document.getElementById(""mxlist1"").style.height=$(tb31).height()+""px"";" & vbcrlf & "      }catch(e){}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script type=""text/javascript"" src=""../inc/jquery-1.4.2.min.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/javascript"" src=""../inc/jquery.autocomplete.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/javascript"" src=""chanceproc.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFiles.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "$(document).ready(function(){" & vbcrlf & "      "
	if  CheckPurview(ts_kh,"7")=True then
		Response.write " " & vbcrlf & "     $(""#title"").autocomplete(""getChanceJson.asp"",{" & vbcrlf & "              delay:10,              " & vbcrlf & "         minChars:0,              " & vbcrlf & "               matchSubset:0,              " & vbcrlf & "            matchContains:1,              " & vbcrlf & "          cacheLength:10             " & vbcrlf & "     });" & vbcrlf & "  $(""#title"").result(function(event,data,formatted) {" & vbcrlf & "               if (data&&("
		if  CheckPurview(ts_kh,"1")=True then Response.write "true" else Response.write "false"
		Response.write "==true)){" & vbcrlf & "                     $(""#queryresult"").html(""项目已存在"");" & vbcrlf & "                       $(""#flag"").val(""1"");" & vbcrlf & "                }" & vbcrlf & "       });" & vbcrlf & "     "
	end if
	Response.write " " & vbcrlf & "});" & vbcrlf & "var checkTS1 = "
	if  CheckPurview(ts_kh,"1")=True then Response.write "true" else Response.write "false"
	Response.write ";" & vbcrlf & "var checkTS2 = "
	if  CheckPurview(ts_kh,"2")=True then Response.write "true" else Response.write "false"
	Response.write ";" & vbcrlf & "" & vbcrlf & "window.canSubmit = 1;" & vbcrlf & "//判断录入必填项：项目策略录入策略3" & vbcrlf & "function ask(ord) { " & vbcrlf & " var errors=0;" & vbcrlf & "   "
	if  CheckPurview(bt_kh,"1")=True then
		Response.write "" & vbcrlf & "              if($(""#title"").val()==""""){$(""#title_tip"").text(""必填"");errors++;}" & vbcrlf & "       "
'if  CheckPurview(bt_kh,"1")=True then
	end if
	if  CheckPurview(bt_kh,"2")=True then
		Response.write "" & vbcrlf & "              if($("").val()==""""){$("").text(""必填"");errors++;}" & vbcrlf & " "
'if  CheckPurview(bt_kh,"2")=True then
	end if
	if  CheckPurview(bt_kh,"3")=True then
		Response.write "" & vbcrlf & "              try{" & vbcrlf & "            var iframedoc=document.getElementById('areaid').contentWindow.document;//iframe框架" & vbcrlf & "             var areatxt=iframedoc.getElementById('areatxt').value;//地区值" & vbcrlf & "          if(areatxt==""""){$(""#areaids"").text(""* 必填"");errors++;}" & vbcrlf & "               }catch(e){}"& vbcrlf &""
	end if
	if  CheckPurview(bt_kh,"4")=True then
		Response.write "if($(""#trade"").val()==""""){$(""#trade_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"4")=True then
	end if
	if  CheckPurview(bt_kh,"5")=True then
		Response.write "if($(""#complete1"").val()==""""){$(""#complete1_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"5")=True then
	end if
	if  CheckPurview(bt_kh,"6")=True then
		Response.write "if($(""#complete2"").val()==""""){$(""#complete2_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"6")=True then
	end if
	if  CheckPurview(bt_kh,"7")=True then
		Response.write "if($(""#sorce"").val()==""""){$(""#sorce_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"7")=True then
	end if
	if  CheckPurview(bt_kh,"8")=True then
		Response.write "if($(""#pay1"").val()==""""){$(""#pay1_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"8")=True then
	end if
	if  CheckPurview(bt_kh,"9")=True then
		Response.write "if($(""#daysOfMonth2Pos"").val()==""""){$(""#daysOfMonth2Pos_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"9")=True then
	end if
	if  CheckPurview(bt_kh,"10")=True then
		Response.write "if($(""#zdy1"").val()==""""){$(""#zdy1_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"10")=True then
	end if
	if  CheckPurview(bt_kh,"11")=True then
		Response.write "if($(""#zdy2"").val()==""""){$(""#zdy2_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"11")=True then
	end if
	if  CheckPurview(bt_kh,"12")=True then
		Response.write "if($(""#zdy3"").val()==""""){$(""#zdy3_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"12")=True then
	end if
	if  CheckPurview(bt_kh,"13")=True then
		Response.write "if($(""#zdy4"").val()==""""){$(""#zdy4_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"13")=True then
	end if
	if  CheckPurview(bt_kh,"14")=True then
		Response.write "if($(""#zdy5"").val()==""""){$(""#zdy5_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"14")=True then
	end if
	if  CheckPurview(bt_kh,"15")=True then
		Response.write "if($(""#zdy6"").val()==""""){$(""#zdy6_tip"").text(""必填"");errors++;}"
'if  CheckPurview(bt_kh,"15")=True then
	end if
	Response.write "" & vbcrlf & "      var bt_kh = "
	Response.write iif(CheckPurview(bt_kh,"16")=True,1,0)
	Response.write ";     " & vbcrlf & "        var khiframe = document.getElementById('mxlist1').contentWindow.document;" & vbcrlf & "       var khDataFrame = document.getElementById('mxlist1').contentWindow;" & vbcrlf & "     try{" & vbcrlf & "            var lvw = khDataFrame.window[""lvw_JsonData_companylist""];" & vbcrlf & "         if(lvw.rows.length == 0 ||(lvw.rows.length>0 && lvw.rows[0][0] == khDataFrame.window.ListView.NewRowSignKey)){" & vbcrlf & "                  if(bt_kh == 1){" & vbcrlf & "                         alert(""请添加关联客户及联系人！"");" & vbcrlf & "                                errors++;" & vbcrlf & "                       }" & vbcrlf & "               }else{" & vbcrlf & "                  khiframe.getElementById('bill.dosave_btn').click();" & vbcrlf & "             }" & vbcrlf & "       }catch(e){}" & vbcrlf & "     if(window.canSubmit ==0){errors++;}" & vbcrlf & "     if (errors==0){return true} " & vbcrlf & "    return false;" & vbcrlf & "} " & vbcrlf & "</script>" & vbcrlf & "<script src= ""../Script/ce_add.js?ver="
'Response.write iif(CheckPurview(bt_kh,"16")=True,1,0)
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	chance_1_19=openPower(1,19)
	session("companyzbintel")=""
	areamenu_list=session("areamenu_list")
	ty=request("ty")
	conn.execute "delete chanceProcNodesBak where chance in (select ord from chance where cateid="&session("personzbintel2007")&" and del=7)"
	conn.execute "delete ChanceNextNodes where chance in (select ord from chance where cateid="&session("personzbintel2007")&" and del=7)"
	conn.execute "delete ChanceProcRunLogs where chance in (select ord from chance where cateid="&session("personzbintel2007")&" and del=7)"
	sql="Delete chance where cateid="&session("personzbintel2007")&" and del=7"
	conn.Execute(sql)
	sql="Delete chancelist where  addcate="&session("personzbintel2007")&" and del=7"
	conn.Execute(sql)
	sql="Delete tel where del=7 and cateid="&session("personzbintel2007")&""
	conn.Execute(sql)
	set rs88 = conn.execute("EXEC erp_getdjbh 3,"&session("personzbintel2007"))
	khid=rs88(0).value
	rs88.close
	set rs88=nothing
	sqlStr="Insert Into chance(cateid,date7,xmid,del) values('"
	...
	Conn.execute(sqlStr)
	dim rd
	rd = GetIdentity("chance","ord","cateid","")
	session("chance_idzbintel")=rd
	dim bz
	set rs=server.CreateObject("adodb.recordset")
	...
	sql5="select area from arealist where cateid="&session("personzbintel2007")&""
	...
	rs5.close
	set rs5=nothing
	Response.write "" & vbcrlf & "<table width=""100%"" border     =""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""center"" id='mainbody'>" & vbcrlf & "    <form method=""POST"" action=""save.asp?ord="
	Response.write rd
	Response.write "&ty="
	Response.write ty
	Response.write """ autocomplete=""off"" id=""demo"" onsubmit=""return Validator.Validate(this,2)&&DelUnusedFilesBeforeSubmit()&&checkFlag()&&checkValue()&checkxmid()&&ask("
	Response.write rd
	Response.write ")&check_all_zdy()"" name=""date"" target='submitfrm'>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"" class=""resetTableBg"">" & vbcrlf & "                <tr>" & vbcrlf & "            <td class=""place place42"">项目添加</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">" & vbcrlf & "                 <iframe name='submitfrm' style='width:1px;height:1px;position:absolute;top:-100px' ></iframe>" & vbcrlf & "                   &nbsp;<input type=""submit"" name=""Submit42"" value=""保存"" onclick=""return ask("
	Response.write rd
	Response.write rd
	Response.write ");"" class=""page""/>" & vbcrlf & "                  <input type=""button""  value=""重填"" class=""page"" name=""B2"" onclick='RreshElement(""mainbody"")'>" & vbcrlf & "                     "
	dim xlsdrHTML
	dim xlsdrHTML2
	if not conn.execute("select * from power where sort1=3 and sort2=9 and qx_open=1 and ord=" & session("personzbintel2007")).eof then
		Response.write "" & vbcrlf & "<style>" & vbcrlf & "        div.DisDivBgCss {" & vbcrlf & "               display:block;" & vbcrlf & "          position:fixed;" & vbcrlf & "         _position:absolute;" & vbcrlf & "             width:100%;" & vbcrlf & "             height:2000px;" & vbcrlf & "          z-index:100;" & vbcrlf & "            top:0px;" & vbcrlf & "                left:0px;" & vbcrlf & "          filter:Alpha(opacity=40);" & vbcrlf & "               -moz-opacity:0.4;  " & vbcrlf & "             -khtml-opacity: 0.4;  " & vbcrlf & "          opacity: 0.4; " & vbcrlf & "          /*background-color:#CAD0E0;" & vbcrlf & "             background-image:url(../images/login_sample_bg.jpg);" & vbcrlf & "            background-repeat:repeat-x;*/" & vbcrlf & "          /*IE以下模式*/" & vbcrlf & "          " & vbcrlf & "        }" & vbcrlf & "       span.spitem{" & vbcrlf & "            cursor:pointer;" & vbcrlf & " }" & vbcrlf & "       table.divForm{" & vbcrlf & "          border-top:1px solid #CCC;" & vbcrlf & "              border-left:1px solid #CCC;" & vbcrlf & "             border-bottom:1px solid #CCC;" & vbcrlf & "           border-right:1px solid #CCC;" & vbcrlf & "      }" & vbcrlf & "       div.divdlgb{" & vbcrlf & "            width:100%;" & vbcrlf & "             height:100%;" & vbcrlf & "    }" & vbcrlf & "       div.divdlgBody{" & vbcrlf & "         width:100%;" & vbcrlf & "             height:100%;" & vbcrlf & "            border:1px solid #CCC;" & vbcrlf & "          background-color:white;"& vbcrlf &"               padding:500px; "& vbcrlf &"           overflow:auto;" & vbcrlf &  vbcrlf &           "overflow-x:auto;" & vbcrlf &"                 overflow-y:hidden; "& vbcrlf &"               scrollbar-3dlight-color:#ffffFF;" & vbcrlf & "                scrollbar-highlight-color:#ffffFF; "& vbcrlf &"               scrollbar-face-color:#eeeeFF; "& vbcrlf &"            scrollbar-arrow-color:#6666ff;" & vbcrlf & "         scrollbar-shadow-color:#ffffFF;" & vbcrlf & "         scrollbar-darkshadow-color:#FFFFFF;" & vbcrlf & "             scrollbar-base-color:#FFFFFF;" & vbcrlf & "           scrollbar-track-color:#FFFFFF;" & vbcrlf & "  }" & vbcrlf & "       div.dvt_closebar{" & vbcrlf & "       width:16px;" & vbcrlf & "       height:16px;" & vbcrlf & "    overflow:hidden;" & vbcrlf & "        background-image:url(../images/smico/panel_tools.gif);" & vbcrlf & "  background-position:-16px 0px;" & vbcrlf & "  float:right;" & vbcrlf & "    margin-right:2px;" & vbcrlf & "}" & vbcrlf & "      div.dvt_maxbar{" & vbcrlf & "         width:16px;" & vbcrlf & "           height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools.gif);" & vbcrlf & "          background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            margin-right:2px;" & vbcrlf & "               background-repeat:no-repeat;" & vbcrlf & "            background-position:0px -16px;" & vbcrlf &"          overflow:hidden;" & vbcrlf & "        }" & vbcrlf & "" & vbcrlf & "       div.dvt_minbar{" & vbcrlf & "         width:16px;" & vbcrlf & "             height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools.gif);" & vbcrlf & "          background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            overflow:hidden;" & vbcrlf & "            margin-right:2px;" & vbcrlf & "       }" & vbcrlf & "" & vbcrlf & "       div.dvt_maxedbar{" & vbcrlf & "               width:16px;" & vbcrlf & "             height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools.gif);" & vbcrlf & "          background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            background-position:-16px -16px;" & vbcrlf & "                overflow:hidden;" & vbcrlf & "                margin-right:2px;" & vbcrlf & "       }" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "       div.dvt_closebar_out{" & vbcrlf & "           width:16px;" & vbcrlf & "             height:16px;" & vbcrlf & "            overflow:hidden;" & vbcrlf & "                background-image:url(../images/smico/panel_tools2.gif);" & vbcrlf & "         background-position:-16px 0px;" & vbcrlf & "          float:right;" & vbcrlf & "            margin-right:2px;" & vbcrlf & "       }" & vbcrlf & "       div.dvt_maxbar_out{" & vbcrlf & "             width:16px;" & vbcrlf & "             height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools2.gif);" & vbcrlf & "          background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            margin-right:2px;" & vbcrlf & "               background-position:0px -16px;" & vbcrlf & "          overflow:hidden;" & vbcrlf & "        }" & vbcrlf & "" & vbcrlf & "       div.dvt_minbar_out{" & vbcrlf & "             width:16px;" & vbcrlf & "            height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools2.gif);" & vbcrlf & "         background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            background-repeat:no-repeat;" & vbcrlf & "            overflow:hidden;" & vbcrlf & "                margin-right:2px;" & vbcrlf & " }" & vbcrlf & "" & vbcrlf & "       div.dvt_maxedbar_out{" & vbcrlf & "           width:16px;" & vbcrlf & "             height:16px;" & vbcrlf & "            background-image:url(../images/smico/panel_tools2.gif);" & vbcrlf & "         background-repeat:no-repeat;" & vbcrlf & "            float:right;" & vbcrlf & "            background-position:-16px -16px;" & vbcrlf & "            overflow:hidden;" & vbcrlf & "                margin-right:2px;" & vbcrlf & "       }" & vbcrlf & "       " & vbcrlf & "        button.wavbutton {" & vbcrlf & "              border:1px solid #9FA3BC;" & vbcrlf & "               font-family:arial;" & vbcrlf & "              font-size:12px;" & vbcrlf & "         background-color:white;" & vbcrlf & "         height:20px;" & vbcrlf & "          filter:wave(strength=0,freq=1,lightstrength=6,phase=0);" & vbcrlf & "         color:#000;" & vbcrlf & "             cursor:default;" & vbcrlf & " }" & vbcrlf & "       th{" & vbcrlf & "             font-size:12px;" & vbcrlf & " }" & vbcrlf & "</style>" & vbcrlf & "<script src='../Manufacture/inc/base.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write "'></script>" & vbcrlf & "<script src= ""../Script/ce_xlsImport.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & ""
		if instr(1,request.ServerVariables("url"),"content.asp",1)>0 then
			xlsdrHTML = "&nbsp;<a href='javascript:void(0)' onClick='clistdr(" & ord & ",1)' title='点击导入明细'>导入明细</a>"
			xlsdrHTML2 = "&nbsp;<a href='javascript:void(0)' onClick='clistdr(" & ord & ",1)' title='点击导入明细'><font class='blue2'>导入明细</font></a>"
		else
			Response.write "<input type=""button"" value=""导入"" title='点击导入产品明细' class=""page"" onclick='clistdr(" & rd & ",0)'>"
		end if
	end if
	Response.write "" & vbcrlf & "                     </td>                   " & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> " & vbcrlf & "  <input type=""hidden"" id=""flag"" name=""flag"" value=""0"">" & vbcrlf & "   <input type=""hidden"" id=""flag_xmid"" name=""flag_xmid"" value=""0"">" & vbcrlf & "         <input type=""hidden"" id=""flag_zdy1"" name=""flag_zdy1"" value=""0"">" & vbcrlf & "         <input type=""hidden"" id=""flag_zdy2"" name=""flag_zdy2"" value=""0"">" & vbcrlf &"   <input type=""hidden"" id=""flag_zdy3"" name=""flag_zdy3"" value=""0"">" & vbcrlf &  "        <input type=""hidden"" id=""flag_zdy4"" name=""flag_zdy4"" value=""0"">" & vbcrlf &"           <tr>" & vbcrlf & "        <td ><div align=""right"">项目主题：</div></td>" & vbcrlf &"         <td  class=""gray""><div align=""left"">" & vbcrlf &"           <input name=""title"" type=""text"" size=""20"" id=""title"" dataType=""Limit"" min=""1"" max=""100"" msg=""项目主题必须在1到100个字之间"" onKeyUp=""checkValue()"">" & vbcrlf &""
	if  CheckPurview(bt_kh,"1")=True then
		Response.write " <span class=""red"" id=""title_tip"">*</span>"
	end if
	Response.write "<span id=""queryresult"" class=""red""></span></div></td>" & vbcrlf & "        <td ><div align=""right"">项目编号：</div></td>" & vbcrlf & "        <td width=""23%""  class=""gray""><div align=""left"">" & vbcrlf & "          <input name=""xmid"" id=""xmid"" type=""text"" size=""15"" value="""
	Response.write khid
	Response.write """ " & vbcrlf & "                                dataType=""Limit"" "
	if CheckPurview(bt_kh,"2")=True then
		Response.write "min=""1"" max=""50"""
	end if
	Response.write "  msg=""长度必须在1至50个字之间"" onKeyUp=""checkxmid()"" >" & vbcrlf & "            "
	if  CheckPurview(bt_kh,"2")=True then
		Response.write " <span class=""red"" id=""xmid_tip"">*</span>"
	end if
	Response.write "<span id=""xmid_queryresult"" class=""red""></span></div></td>" & vbcrlf & "           <td width=""10%""><div align=""right"">发现日期：</div></td>" & vbcrlf & "            <td width=""23%"" class=""gray""><INPUT name=""ret"" readonly id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')""value="
	Response.write date
	Response.write """ size=10  dataType=""Date"" min=""1"" format=""ymd""  msg=""日期格式不正确"">" & vbcrlf & "          <DIV id=daysOfMonth style=""POSITION: absolute""></DIV>" & vbcrlf & "          <span class=""gray""><span class=""red"">*</span></td>" & vbcrlf & "         </tr>" & vbcrlf & "       <tr>" & vbcrlf & "    " & vbcrlf & "        <td ><div align=""right"">地　　区：</div></td>" & vbcrlf & "        <td  class=""gray""><IFRAME name=""I2"" id=""areaid"" style=""float:left"" SRC=""../event/area.asp?ID=company"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""135px"" HEIGHT=""27"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf &""
	if  CheckPurview(bt_kh,"3")=True then
		Response.write "<span class=""red"" id=""areaids"">*</span>"
	end if
	Response.write "" & vbcrlf & "             </td>" & vbcrlf & "        <td><div align=""right"">主负责人：</div></td>" & vbcrlf & "        <td><select name=""cateid"" dataType=""Limit"" min=""1"" max=""50""  msg=""不能为空"">" & vbcrlf & "                       "
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select sort1,qx_open,w1,w2,w3 from power2  where cateid="&session("personzbintel2007")&" and sort1=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1_1=0
	else
	...
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select ord,name from gate where  del=1  "&str_w3&"  order by cateid asc"
	rs7.open sql7,conn,1,1
	do until rs7.eof
		Response.write "" & vbcrlf & "          <option value="""
		Response.write rs7("ord")
		Response.write """ "
		if rs7("ord")=session("personzbintel2007") then
	...
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "        </select></td>" & vbcrlf & "                 <td><div align=""right"">币　　种：</div></td>" & vbcrlf & "            "
	if bz="1" then
		str=""
	else
		str=" where id=14 "
	end if
	Response.write "" & vbcrlf & "                  <td class=""gray""><select name=""bz"" class=""gray"" dataType=""Limit"" min=""1"" max=""50""  msg=""请选择币种"">" & vbcrlf & "        "
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,sort1 from sortbz "&str&" order by gate1 desc"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		Response.write "" & vbcrlf & "               <option>还没有设置币种，不能添加项目</option>" & vbcrlf & "               "
	else
		do until rs1.eof
			Response.write "" & vbcrlf & "               <option value="""
	...
	Response.write "" & vbcrlf & "             </select></td>" & vbcrlf & "       </tr>" & vbcrlf & "              <tr>" & vbcrlf & "              <td><div align=""right"">项目总额：</div></td><td><input type=""text"" name=""premoney"" id=""premoney"" value=""0"" readonly size=""10"" onkeyup=""value=value.replace(/[^\d\.]/g,'')""   dataType=""LimitB"" min=""0"" max=""15""  msg=""金额长度必须在15位之内""></td>" & vbcrlf & "               <td><div align=""right"">优惠方式：</div></td>" & vbcrlf & "              <td>" & vbcrlf & "                  <input type=""radio"" name=""yhtype"" value=""0"" id=""yh1"" checked onclick=""changeYHType(0);"">优惠金额" & vbcrlf & "                  <input type=""radio"" name=""yhtype"" value=""1"" onclick=""changeYHType(1);"">折扣&nbsp;" & vbcrlf & "                     <input type=""text"" name=""yhvalue"" id=""yhvalue"" size=""10"" onfocus=""this.select();"" " & vbcrlf & "                                onkeyup=""yhvalueKeyUp(this);"" dataType=""LimitB"" min=""0"">&nbsp;" & vbcrlf & "                        <span id=""__ErrorMessagePanel"" style=""color: red;""></span>" & vbcrlf & "                       </td>" & vbcrlf & "             <td><div align=""right"">优惠后总额：</div></td><td><input type=""hidden"" name=""Inverse"" id=""Inverse"" value=""0"" ><input type=""text"" name=""moneyall"" id=""moneyall"" size=""10"" onkeyup=""value=value.replace(/[^\d\.]/g,'');checkDot('moneyall','"
	Response.write num_dot_xs
	Response.write "');Calculation(2);"" onfocus=""this.select();""  dataType=""LimitB"" min=""0"" max=""15""  msg=""金额长度必须在15位之内"">&nbsp;<span id=""ts1"" class=""red""></span></td>" & vbcrlf & "            </tr>" & vbcrlf & "           <tr>" & vbcrlf & "              <td ><div align=""right"">项目分类：</div></td>" & vbcrlf & "             <td ><select name=""trade"" id=""trade"">" & vbcrlf & "            "
	dim trade,trade2
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from sortonehy where gate2=24 order by gate1 desc "
	rs.open sql,conn,1,1
	do until rs.eof
		trade=rs("ord")
		trade2=rs("sort1")
	...
	Response.write "" & vbcrlf & "          </select>" & vbcrlf & "            "
	if  CheckPurview(bt_kh,"4")=True then
		Response.write " <span class=""red"" id=""trade_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "               </td>" & vbcrlf & "           "
	If ZBRuntime.MC(203101) then
		Response.write "" & vbcrlf & "               <td ><div align=""right"">项目流程：</div></td>" & vbcrlf & "             <td ><select name=""ProcId"" id=""ProcId"" onchange='onProcChange(this,0)'>" & vbcrlf & "                   "
		set rs=server.CreateObject("adodb.recordset")
		sql="select id, name from ProcModels where Flag=1 order by type desc,gate1 desc,id desc"
		Response.write "<option value='0'>不需要流程</option>"
		rs.open sql,conn,1,1
		do until rs.eof
			Response.write "" & vbcrlf & "                                     <option value="""
	...
	Response.write "" & vbcrlf & "               <td ><div align=""right"">项目阶段：</div></td>" & vbcrlf & "             <td ><select name=""complete2"" id=""complete2"">" & vbcrlf & "            "
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from sortjh2 where id1=0 and Flag = 1 order by gate1 desc"
	rs.open sql,conn,1,1
	do until rs.eof
		Response.write "" & vbcrlf & "            <option value="""
...
'Response.write "</option>" & vbcrlf & "            "
	...
	Response.write "" & vbcrlf & "          </select>" & vbcrlf & "            "
	if  CheckPurview(bt_kh,"6")=True then
		Response.write " <span class=""red"" id=""complete2_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "               </td>" & vbcrlf & "           "
	If Not ZBRuntime.MC(203101) Then
		Response.write "<td>&nbsp;</td><td><input type='hidden' name='ProcId' id='ProcId'  value='0'></td>"
	end if
	Response.write "" & vbcrlf & "             </tr>" & vbcrlf & "       <tr>" & vbcrlf & "              <td ><div align=""right"">项目来源：</div></td>" & vbcrlf & "             <td width=""23%"" ><select name=""sorce"" id=""sorce"">" & vbcrlf & "                                       "
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from sortonehy where gate2=23 order by gate1 desc"
	rs.open sql,conn,1,1
	do until rs.eof
		Response.write "" & vbcrlf & "                                     <option value="""
		Response.write rs("ord")
	...
	Response.write "" & vbcrlf & "          </select>" & vbcrlf & "            "
	if  CheckPurview(bt_kh,"7")=True then
		Response.write " <span class=""red"" id=""sorce_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "               </td>" & vbcrlf & "           <td ><div align=""right"">项目状态：</div></td>" & vbcrlf & "             <td ><select name=""complete1"" id=""complete1"">" & vbcrlf & "                     "
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from sortonehy where gate2=21 order by gate1 desc"
	rs.open sql,conn,1,1
	do until rs.eof
		Response.write "" & vbcrlf & "                                     <option value="""
		Response.write rs("ord")
	...
	Response.write "" & vbcrlf & "          </select>" & vbcrlf & "            "
	if  CheckPurview(bt_kh,"5")=True then
		Response.write " <span class=""red"" id=""complete1_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "               </td>" & vbcrlf & "       <td width=""11%"" ><div align=""right"">预计费用：</div></td>" & vbcrlf & "           <td ><span class=""gray"">" & vbcrlf & "            <input name=""pay1"" id=""pay1"" type=""text"" size=""10"" onkeyup=""value=value.replace(/[^\d\.]/g,'');checkDot('pay1','"
	Response.write num_dot_xs
	Response.write "')""   dataType=""LimitB"" min=""0"" max=""15""  msg=""金额长度必须在15位之内""></span>" & vbcrlf & "                    元 "
	if  CheckPurview(bt_kh,"8")=True then
		Response.write " <span class=""red"" id=""pay1_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "               </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "        <td ><div align=""right"">预计日期：</div></td>" & vbcrlf & "             <td ><input name=""ret2"" size=""10"" readonly id=""daysOfMonth2Pos"" onMouseUp=""toggleDatePicker('daysOfMonth2','date.ret2')"" dataType=""Date"" format=""ymd""  msg=""日期格式不正确"">" & vbcrlf & "            <DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>" & vbcrlf & "                      "
	if  CheckPurview(bt_kh,"9")=True then
		Response.write " <span class=""red"" id=""daysOfMonth2Pos_tip"">*</span>"
	end if
	Response.write "" & vbcrlf & "             </td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>" & vbcrlf & "           </tr>" & vbcrlf & "       "
	Dim zdyid,zdyid2
	zdyid="3,4,5,6,7"
	zdyid2="10,11,12,13,14,15"
	arr_zdyid = split(zdyid,",")
	arr_zdyid2 = split(zdyid2,",")
	dn = 0
	set rs=server.CreateObject("adodb.recordset")
	sql="select id,title,name,sort,gl from zdy where sort1=3 and set_open=1 order by gate1 asc "
	rs.open sql,conn,1,1
	num1=rs.RecordCount
	i_jm=0
	j_jm=1
...
'Response.write "colspan="""
			...
			if rs("sort")=2 then
				...
				Response.write """ id="""
				Response.write zdyname
				Response.write """ type=""text"" size=""15"" dataType=""Limit"" max=""50"" msg=""长度不能超过50个字"" "
				if  CheckPurview(ts_kh,arr_zdyid(dn)&"")=True then Response.write"onKeyUp=""check_zdy('"&zdyname&"')"""
				Response.write ">" & vbcrlf & "                    "
				if  CheckPurview(bt_kh,arr_zdyid2(dn)&"")=True then
					Response.write " <span class=""red"" id="""
					Response.write zdyname
					Response.write "_tip"" >*</span>"
				end if
				Response.write "" & vbcrlf & "                     <span id="""
				Response.write zdyname
				Response.write "_queryresult"" class=""red""></span>" & vbcrlf & "            <span class=""inhibit red""></span>" & vbcrlf & "                        "
...
'Response.write "</td>" & vbcrlf & "                "
	...
	rs.close
	set rs=nothing
	If ZBRuntime.MC(203103) Then Response.write getExtended(3,0)
	Response.write "" & vbcrlf & "               <tr>" & vbcrlf & "               <td ><div align=""right"">关联客户/联系人：</div></td>" & vbcrlf & "            "
	if chance_1_19=0 then
		Response.write "" & vbcrlf & "                  <td  colspan=""5"" class=""gray""><input type=""hidden"" name=""ptord"" id=""ptord""><IFRAME name=""I31"" SRC=""../../SYSN/view/sales/project/ProjectCompany.ashx?xmord="
		Response.write pwurl(rd)
		Response.write """ id=""mxlist1"" width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>" & vbcrlf & "              "
	else
		Response.write "" & vbcrlf & "            <td  colspan=""5"" class=""gray""><input type=""hidden"" name=""ptord"" id=""ptord""><IFRAME name=""I31"" SRC="""" id=""mxlist1"" width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>"& vbcrlf &""
	end if
	Response.write "" & vbcrlf & "           </tr>" & vbcrlf & "               <!--" & vbcrlf & "            <tr>" & vbcrlf & "               <td ><div align=""right"">关联客户/联系人：</div></td>" & vbcrlf & "              <td  colspan=""5"" class=""gray""><input type=""hidden"" name=""ptord"" id=""ptord""><IFRAME name=""I31"" SRC=""../event/xmkh.asp?ID=company"" id=""mxlist1"" width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>" & vbcrlf & "        </tr>" & vbcrlf & "               -->" & vbcrlf & "              <tr>" & vbcrlf & "              <td ><div align=""right"">产品明细：</div></td>" & vbcrlf & "                 <td  colspan=""5"" class=""gray"">" & vbcrlf & "                          <IFRAME name=""I3"" SRC=""../event/xmmx.asp?ID=company"" id=""mxlist"" width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>" & vbcrlf & "            </tr>" & vbcrlf & "        <tr>" & vbcrlf & "        <td width=""10%"" >" & vbcrlf & "          <div align=""right"">项目概要：</div></td>" & vbcrlf & "        <td  colspan=""5"" class=""gray""><textarea name=""intro3"" style=""display:none"" cols=""1"" rows=""1"">"
	if intro3<>"" then Response.write intro3
	Response.write "</textarea>" & vbcrlf & "                    <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro3&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME>" & vbcrlf & "          <span class=""gray""></span></td>" & vbcrlf &      " </tr> "& vbcrlf &    " <tr id=""ProcNodeArea"" style='display:none'><td><div align=""right"">阶段工作：</div></td><td id="
	If ZBRuntime.MC(203103) Then Call getExtended1(3,0)
	Response.write "" & vbcrlf & "      <tr>" & vbcrlf & "        <td  colspan=""6"" class=""gray"">" & vbcrlf & "            <div align=""center"">" & vbcrlf & "                <input type=""submit"" name=""Submit42"" value=""保存"" onclick=""return ask("
	Response.write rd
	Response.write ");""  class=""page""/>　　　　　　" & vbcrlf & "                <input type=""button"" value=""重填"" class=""page"" name=""B2""  onclick='RreshElement(""mainbody"")'>" & vbcrlf & "            </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "    </table>" & vbcrlf & "  </form> " & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""3130"" ><div align=""center""></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "     " & vbcrlf & "  </td> " & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & ""
	action1="项目添加"
	call close_list(1)
