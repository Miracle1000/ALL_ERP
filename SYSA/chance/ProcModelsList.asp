...
'Dim r
	...
	Class AppInfo
		...
		public function DiscountNumber
			if len( mDiscountNumber) = 0 then
				dim rs
				set rs = cn.execute("select num1 from setjm3 where ord=2014053102")
				if rs.eof = false then
					mDiscountNumber = rs.fields(0).value
				else
					mDiscountNumber = 2
				end if
				rs.close
			end if
			DiscountNumber = mDiscountNumber
		end function
		public function HlNumber
			if len( mHlNumber) = 0 then
				dim rs
				set rs = cn.execute("select num1 from setjm3  where ord=87")
				if rs.eof = false then
					mHlNumber = rs.fields(0).value
				end if
				rs.close
			else
				mHlNumber = 0
		...
		If Application("_sys_info_version") = "" Then
			Application("_sys_info_version") = GetItem("version")
		end if
		version = Application("_sys_info_version")
		End Property
		Private Function GetItem(ByVal keytxt)
			on error resume next
			Dim cn, rs, result
			Set cn = server.CreateObject("adodb.connection")
			cn.open "Driver={SQLite3 ODBC Driver};Database=" & server.mappath("../update/db.asp") & ""
			Set rs = cn.execute("select value from SysInfo where key='" & keytxt & "'")
			If rs.eof = False Then
		...
		fullCompanyName="北京智邦国际软件技术有限公司"
		End Property
		Public Property Get user
		...
		If Len(is_supperadmin) = 0 Then
			If Me.isAdmin  Then
				Set rs = cn.execute("select qx_open from power where sort1=66 and sort2=12 and ord=" & Me.User & " and qx_open=1")
		...
		username = musername
		End Property
		public property get propmTimer
		dim rs , v
		set rs = cn.execute("select num1 from setjm3 where ord=4")
		if rs.eof then
			v = 30000
			cn.execute "Insert Into setjm3(ord,num1) values(4," & v & ")"
		else
			v = rs("num1").value
		end if
		rs.close
		...
		DebugMode = True
		End Property
		public property get SoftType
		dim M , i
		M = session("zbintel2010ms")
		for i = 1 to 12
			if ZBRuntime.MC(cstr(99000+i))  = true then
'for i = 1 to 12
				SoftType = i
				exit property
			end if
		...
		public property get UniqueName
		Dim fso , fobj, Strs , i
		UniqueName = ZBRuntime.sUid
		session("UniqueName") = UniqueName
		end Property
		public function CheckPower2010(v1,v2)
			CheckPower2010 = ZBRuntime.MC(v2)
		end function
		Public Function IP
			dim dulip,proip
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/ce_ProcModelsList.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	isSupperAdminor = info.isSupperAdmin
	set info = nothing
	cn.close
	set db = nothing
	set cn = nothing
	Response.write "" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "    <table width=""100%""border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">项目流程模板设置</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right""><input type=""button"" name=""Submit32""value=""添加流程模板""  class=""anybutton"" onClick=""javascript:window.open('ProcModelsAdd.asp?act=add','newwin','width=' + 800 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=10,left=50,top=50')""/>&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & ""
	CurrPage=cint(Request("CurrPage"))
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.Flag,a.id,a.name,gate1,type,a.addcate,b.name username,c.dp from ProcModels a left join gate b on a.addcate=b.ord left join (select distinct chancePMid as dp from ProcModelsNodes) c on a.id=c.dp order by type desc, gate1 desc, id desc"
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <div align=""center"">" & vbcrlf & "  <center>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> " & vbcrlf & " <tr border=1 valign=""center"" class=""top""> " & vbcrlf & "    <td width=""35%"" height=""27"" align=""center"" valign=""middle"" bgcolor=""#F8F8F8""><div align=""center""><font style=""font-size: 9pt"">模板名称</font></div></td>" & vbcrlf & "    <td width=""10%"" align=""center"" valign=""middle"" bgcolor=""#F8F8F8""><div align=""center"">启用状态</div></td>" & vbcrlf & "    <td width=""20%"" align=""center"" valign=""middle"" bgcolor=""#F8F8F8""><div align=""center""><font style=""font-size: 9pt"">添加人员</font></div></td>" & vbcrlf & "    <td width=""25%"" height=""32"" align=""center"" valign=""middle"" bgcolor=""#F8F8F8""><div align=""center""><font style=""font-size: 9pt"">操作</font></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "  "
		do until rs.eof
			Response.write " " & vbcrlf & "  <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"" > " & vbcrlf & "    <td height=""25"" align=""left"" valign=""middle""  class=""name"" align=""center""><div align=""left"">"
			Response.write rs("name")
			if rs("type") = 1 Then
				Response.write " <font color='red'>(主模板)</font>"
			end if
			Response.write "</div></td>" & vbcrlf & "    <td width=""176"" height=""25"" align=""center"" valign=""middle""  class=""name"">"
			IF rs("Flag") = 0 Then
				Response.write "<span style='color: red;'>停用</span>"
			ElseIF rs("Flag") = 1 Then
				Response.write "<span style='color: #009933;'>启用</span>"
			end if
			Response.write "</td>" & vbcrlf & " <td height=""25"" align=""center"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs("username")
			Response.write "</div></td>" & vbcrlf & "    <td height=""25"" align=""center"" valign=""middle""  class=""func""><input type=""button"" name=""Submit3c"" value=""预览""  onClick=""openlook('"
			Response.write rs("id")
			Response.write "','"
			Response.write rs("dp")
			Response.write "')""/> <input type=""button"" name=""Submit3c"" value=""详情""  onClick=""openupdate('"
			Response.write rs("id")
			Response.write "')""/> <input type=""button"" name=""Submit3c"" value=""复制""  onClick=""openuCopy('"
			Response.write rs("id")
			Response.write "')""/>" & vbcrlf & "    <input type=""button"" name=""Submitdel"" value="""
			IF rs("Flag") = 1 Then
				Response.write "停用"
				rsFlag = 0
			...
			Response.write """ onClick=""window.location.href='SetPMFlag.asp?ord="
			Response.write rs("id")
			...
			Response.write "'""/> "
			If rs("addcate") = session("personzbintel2007") Or isSupperAdminor Then
				Response.write "<input type=""button"" name=""Submitdel"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='PMLDelete.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='ProcModelsList.asp?currPage="
			Response.write  1
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='ProcModelsList.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='ProcModelsList.asp?currPage="
			Response.write  currpage + 1
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='ProcModelsList.asp?currPage="
			Response.write  rs.PageCount
			Response.write "'"" class=""page""/>" & vbcrlf & "          "
		end if
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script src= ""../Script/ce_ProcModelsList_2.js?ver="
		Response.write Application("sys.info.jsver")
