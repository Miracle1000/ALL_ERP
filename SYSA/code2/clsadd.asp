...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub  Bill_OnPageInit
		Response.write "" & vbcrlf & "<style>.detailTableList > tbody > tr{border-top:1px solid #ccc;}</style>" & vbcrlf & "<script> " & vbcrlf & "     document.onkeydown = function () {" & vbcrlf & "        if (window.event && window.event.keyCode == 13) {" & vbcrlf & "            window.event.returnValue = false;" & vbcrlf & "        }" & vbcrlf & "     }" & vbcrlf & "</script>" & vbcrlf & ""
	end sub
	Sub Bill_OnInit(bill, t)
		...
		bill.title = "设置二维码分类"
		bill.sql = "select * from C2_CodeTypes where id=" & bill.id
		With bill.fields.add("二维码分类" , "title")
		.notnull = true
		.maxlimit = 20
		End With
		With bill.fields.add("重要指数" , "gate1")
		.uitype = "select"
		.source = "snumber:1,30"
		.defvalue = 1
		End With
		With bill.fields.add("是否启用" , "stop")
		.uitype = "radio"
		.source = "options:启用=0,停用=1"
		.defvalue = 0
		End With
		With bill.fields.add("编码方式" , "entype")
		.uitype = "radio"
		.source = "options:文本=0, 网址=1"
		.defvalue = 0
		End With
		bill.fields.addListView "", "@lst"
	end sub
	sub bill_onListCreate(byref field, byref lvw)
		lvw.editkey = "lstid"
		lvw.sql = "select id as lstid, uName, uType ,[stop] as lststop, isShow, gate1 as lstgate1, zdys from C2_CodeTypeFields where cTypeId=" & field.parentgroup.bill.id & " and cTypeId <> 0 order by gate1 desc, id"
		lvw.edit = True
		With  lvw.headers("uName")
		.title = "字段名称"
		.uitype = "text"
		.notnull = True
		.maxsize = 20
		.width = "20%"
		End with
		With lvw.headers("uType")
		.title = "字段类型"
		.uitype = "select"
		.source  = "options:单行文本(短)=0,单行文本(长)=1,多行文本=2,网址=3,图片（普通）=4,图片（人员）=9,数字=5,是/否=6,日期=7,自定义列表=8"
		.width = "15%"
		.unit = "code:czdyEditUnit(rs(""zdys"").value, rs(""uType"").value)"
		.js = "onchange='showZdyDlg(this)'"
		End With
		lvw.headers("zdys").display = "none"
		With lvw.headers("lststop")
		.title = "是否启用"
		.uitype = "select"
		.source = "options:是=0,否=1"
		.width = "15%"
		.editlock = "code: rowindex>1"
		End With
		With lvw.headers("isShow")
		.title = "是否含字段名"
		.uitype = "select"
		.source = "options:是=0,否=1"
		.width = "15%"
		End With
		With lvw.headers("lstgate1")
		.title = "排列顺序"
		.uitype = "select"
		.source = "snumber:50"
		.width = "15%"
		.editlock = "code: rowindex>1"
		End With
		lvw.edit.candelexpress = "rowindex = 1"
		lvw.edit.canistexpress = "rowindex = 1"
		lvw.edit.rowmove = false
	end sub
	function czdyEditUnit(ByVal zdydata, ByVal utype)
		czdyEditUnit = "<span>" & app.iif(utype & ""="8", "&nbsp;<a href='javascript:void(0)' onclick='showZdyEdit(this)'>编辑</a>", "") &  "</span><textarea style='border:0px;display:none;height:1px;width:1px' type='text' name='zdys' onchange='zdyDataChange(this)'>" & zdydata & "</textarea>"
	end function
	function Bill_OnSave(ByVal bill)
		Dim rs, bid
		Set rs = cn.execute("select 1 from C2_CodeTypes where title='" & Replace(app.getText("title"),"'","''") & "' and id <> " & bill.id)
		If rs.eof = False Then
			Response.write "<script>alert(""分类名称不允许重复！"")</script>"
			rs.close
			Bill_OnSave = false
			Exit function
		end if
		...
		bid = bill.ID
		Set rs = server.CreateObject("adodb.recordset")
		rs.open "select * from C2_CodeTypes where id=" & bill.ID, cn, 1, 3
		If rs.eof  Then
			rs.addnew
			rs("del").value = 1
		...
		If instr(app.getText("title"), "系统生成") > 0 Then
			Response.write "<script>alert(""分类名称不允许出现“系统生成”"")</script>"
			Exit function
		end if
		rs("title").value = app.gettext("title")
		rs("stop").value = app.getint("stop")
		rs("gate1").value = app.getint("gate1")
		rs("entype").value = app.getint("entype")
		rs.update
		bid = rs("id").value
		rs.close
		Dim lists, i, kid
		Set lists = request.form("lstid")
		Dim newids
		newids = lists & ""
		If Len(newids) > 0 Then
			cn.execute "delete C2_CodeTypeFields where cTypeId = " &  bill.id & " and id not in (" & newids & ") "
		end if
		For i = 1 To lists.count
			kid = lists(i)
			Set rs = server.CreateObject("adodb.recordset")
			If Len(kid) = 0 Then kid = "0"
			rs.open "select * from C2_CodeTypeFields where id=" & kid, cn, 1, 3
			If rs.eof Then
				rs.addnew
				rs("cTypeId")=bid
			end if
			rs("uName").value = request.form("uName")(i)
			rs("uType").value = request.form("uType")(i)
			rs("stop").value = request.form("lststop")(i)
			rs("isShow").value = request.form("isShow")(i)
			rs("zdys").value = request.form("zdys")(i)
			rs("gate1").value = app.iif(i=1, 100, request.form("lstgate1")(i))
			'rs("uName").value = request.form("uName")(i)
			rs.update
			set rs = nothing
		next
		...
		If app.getInt("sys_ad_model") =  1 Then
			Response.write "<script>alert('" & msg & "');if(top.opener.DoRefresh){top.opener.DoRefresh()};top.location.href= 'clsadd.asp';</script>"
		else
			bill.showSaveResult  msg
		end if
	end function
	sub bill_AjaxWindow_zdydlg(byval win)
		win.canCollapse =  False
		win.width = 700
		win.height = 460
		win.title = "设置"
		win.backgroundColor = "white"
		Dim lvw, data, id, sql
		data = app.getText("text")
		id = app.getText("id")
		Set lvw = New listview
		lvw.id = "zdy_" & id
		lvw.tagData = id
		lvw.sql = "declare @v varchar(1); set @v=char(7); select short_str as 字段内容, 0 as 重要指数, id as 操作 from dbo.split('" & Replace(data, "'","''") & "',@v) t where short_str<>''"
		lvw.headers(1).formattext = "code:itemText(rs(0).value,0)"
		lvw.headers(2).formattext = "code:itemText(rs(0).value,1)"
		lvw.headers(3).formattext = "code:handleBtn(""" & id & """, @value)"
		lvw.checkbox = False
		lvw.indexbox = False
		lvw.oldPageSizeUI = True
		lvw.PageButtonAlign = "right"
		lvw.addlink = ""
		lvw.border = 0
		Response.write "<div style='padding:3px'>"
		Response.write lvw.html
		Response.write "</div>"
	end sub
	Sub App_lvw_onnullData(l)
		If InStr(l.id, "zdy") >0 Then
			Dim tag
			tag = l.tagData
			l.addhtml "<center>没有信息 <img src='../images/jiantou.gif'><a href='javascript:void(0)'  onclick='window.open(""clszdy.asp?view=add&txtid=" & tag & """, """", ""width=500px,height=300,left=300px,top=100px,resizable=1"")'>添加</a></center>"
		else
		end if
	end sub
	function handleBtn(id, rowindex)
		handleBtn = "<button class='oldbutton2' onclick='window.open(""clszdy.asp?view=add&txtid=" & id & """, """", ""width=500px,height=300,left=300px,top=100px,resizable=1"")'>增加</button>&nbsp;" &_
		"<button class='oldbutton2' onclick='openZdyUpdateDlg(""" & id & """," & rowindex & ")'>修改</button>&nbsp;" &_
		"<button class='oldbutton2' onclick='dodelzdyItem(""" & id & """," & rowindex & ")'>删除</button>"
	end function
	Function itemText(txt, index)
		Dim ds : ds = Split(txt & " ",Chr(6))
		If index > ubound(ds) Then index = ubound(ds)
		itemText = ds(index)
	End  Function
