...
'elseif S1=6 then
	...
	C1=rs.recordcount
	xlApplication.Init "接收记录_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof
		ord=rs("ord")
		...
		if strRecvClass="B" then
			strRecvClassName="合同"
		else
			strRecvClassName=""
		end if
		if strRecvNumber<>"" then
			if strRecvClass="B" then
		...
		xlWorksheet.Cells(1+i,1).Value = ","&mobile
		eventName=rs1(0)
		...
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,3).Value = strRecvClassName
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,3).font.Size=10
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,4).Value = eventName
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,4).font.Size=10
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,5).Value = recvtime
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,5).font.Size=10
		eventName=rs1(0)
		xlWorksheet.Cells(1+i,6).Value = addtime
		eventName=rs1(0)
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>接收记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!接收记录导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("接收记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的接收记录</u></strong></font></a></p> " & vbcrlf & ""
