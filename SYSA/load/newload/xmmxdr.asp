...
'elseif instr(1,t, "varchar",1) > 0 then           '
		...
		Response.write "" & vbcrlf & "<style>" & vbcrlf & "        html,body{padding:0px;background:#FFF!important;}" & vbcrlf & "       #billtopbardiv,#billtitle{height:42px!important;line-height:30px;}" & vbcrlf & "      #billtitle{padding-left:24px;}" & vbcrlf & "  #bodyDiv{background:#FFF;}" & vbcrlf & "      #billtitle:before{display:none;}" & vbcrlf & "</style>" & vbcrlf & ""
		sub Page_loadConfig(uploadatt)
			...
			.fileName = "项目明细数据"
			.filters =  "xls|xlsx"
			.smpFilePath =  "../../in/xiangmumx.xls"
			.helpFilePath =  "../../in/xiangmumx.doc"
		...
		Sub Page_OnFileSave(uploader)
			...
			dim dbname
			Randomize
			dbname =  uploader.GetNewTempName()
			Call app.RegTempTable(dbname)
			If uploader.InsertTableByExcel(dbname, "") Then
				uploader.AddReport True
				call App_lvw_InExcel(uploader , dbname)
			end if
		end sub
		sub App_lvw_InExcel(uploader ,db)
			cn.CursorLocation=3
			dim fields , n1 , n2 , topid  , fid, rs, zdys , SalesPrice_dot_num
			dim zdy5tit, zdy6tit, zdynames, zdyfields, i , zdycount
			redim zdynames(0)
			redim zdyfields(0)
			i = 0
			set rs=cn.execute("select num1 from setjm3  where ord=2019042802")
			if Not rs.eof Then
				SalesPrice_dot_num=rs("num1")
			else
				SalesPrice_dot_num=2
			end if
			rs.close
			set rs = cn.execute("select title,name from zdymx where sort1=3 and set_open=1 and name like 'zdy%'")
			while rs.eof = false
			...
			rs.close
			zdycount = i
			if not uploader.CheckFields("产品名称,产品编号,产品型号,单位,数量,单价,建议进价,到货日期,备注" & zdys,db) then
				exit sub
			end if
			...
			cn.execute "create table #k_all (序号 int  IDENTITY(1,1) not null,内容 varchar(300), 说明 varchar(300))"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单位错误' from " & db & " where isnumeric(isnull(单位,0))=1"
			cn.execute "delete from " & db & " where isnumeric(isnull(单位,0))=1" , n
			cn.execute "update " & db & " set 单位 = u.ord from sortonehy u where u.gate2=61 and u.sort1 = 单位"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单位错误' from " & db & " where isnumeric(单位)=0"
			cn.execute "delete from " & db & " where isnumeric(单位)=0" , n1
			...
			cn.execute "update " & db & " set 产品名称=ord, 产品型号='#s_xx_tok' from product x where x.title = isnull(产品名称,'') and x.order1 = isnull(产品编号,'') and x.type1=isnull(产品型号,'') and x.del=1"
			...
			cn.execute "delete from " & db & " where isnull(产品型号,'') <> '#s_xx_tok'" , n1
			cn.execute "update " & db & " set 产品型号='dyfwxx' from product x where x.ord=产品名称 and x.user_list is not null and replace(replace(replace(x.user_list,' ',''),',',''),'0','')<>'' and charindex(',"& app.info.user &",',','+replace(x.user_list,' ','')+',')=0 "
			cn.execute "delete from " & db & " where isnull(产品型号,'') <> '#s_xx_tok'" , n1
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '产品资料不在调用范围内' from " & db & "  where isnull(产品型号,'') ='dyfwxx' "
			cn.execute "delete from " & db & " where isnull(产品型号,'') ='dyfwxx' " , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测产品调用范围','" & app.iif(n1> 0 ,"共<b style=""color:red"">" & n1 & "</b>条记录产品资料不在调用范围内。","") & "')"
			cn.execute "update " & db & " set 产品型号='ddddxx' from product x where x.ord=产品名称 and (单位=unitjb or ','+replace(unit,' ','')+',' like '%,' + 单位 + ',%')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单位不匹配' from " & db & " where isnull(产品型号,'') <> 'ddddxx'"
			cn.execute "delete from " & db & " where isnull(产品型号,'') <> 'ddddxx'" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测单位匹配性','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录单位不匹配。","") & "')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '数量格式录入错误' from " & db & " where isnumeric(isnull(数量,'c'))=0"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '数量不能小于等于零' from " & db & " where isnumeric(isnull(数量,'c'))=1 and cast(replace(数量,',','') as float)<=0"
			cn.execute "delete from " & db & "  where isnumeric(isnull(数量,'c'))=0 or (isnumeric(isnull(数量,'c'))=1 and cast(replace(数量,',','') as float)<=0)" , n1
			cn.execute "insert into #k_all (内容,说明) values ('数量检测','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录数量不正确。","") & "')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单价小数位数错误' from " & db & " where  isnumeric(isnull(单价,'c'))=1 and len(REPLACE(replace((case CHARINDEX('.',replace(单价,',','')-floor(replace(单价,',',''))) when 0 then -1 else replace(单价,',','')-floor(replace(单价,',','')) end),'0.',''),'-1',''))>"&SalesPrice_dot_num
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单价格式录入错误' from " & db & " where isnumeric(isnull(replace(单价,',',''),'c'))=0"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单价不能小于零' from " & db & " where  isnumeric(isnull(单价,'c'))=1 and cast(replace(单价,',','') as float)<0 "
			cn.execute "delete from " & db & "  where isnumeric(isnull(replace(单价,',',''),'c'))=0 or (isnumeric(isnull(单价,'c'))=1 and cast(replace(单价,',','') as float)<0 ) or len(REPLACE(replace((case CHARINDEX('.',replace(单价,',','')-floor(replace(单价,',',''))) when 0 then -1 else replace(单价,',','')-floor(replace(单价,',','')) end),'0.',''),'-1',''))>"&SalesPrice_dot_num, n1
			cn.execute "insert into #k_all (内容,说明) values ('单价检测','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录单价不正确。","") & "')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '建议进价格式录入错误' from " & db & " where isnumeric(isnull(建议进价,'c'))=0"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '建议进价不能小于零' from " & db & " where isnumeric(isnull(建议进价,'c'))=1 and cast(replace(建议进价,',','') as float)<0"
			cn.execute "delete from " & db & "  where isnumeric(isnull(建议进价,'c'))=0 or (isnumeric(isnull(建议进价,'c'))=1  and cast(replace(建议进价,',','') as float)<0)" , n1
			cn.execute "insert into #k_all (内容,说明) values ('建议进价检测','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录建议进价不正确。","") & "')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '交货日期不正确' from " & db & " where isdate(isnull(到货日期,'c'))=0 and len(isnull(到货日期,'')) >0"
			cn.execute "delete from " & db & "  where isdate(isnull(到货日期,'c'))=0 and len(isnull(到货日期,'')) >0" , n1
			cn.execute "insert into #k_all (内容,说明) values ('到货日期','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录到货日期不正确。","") & "')"
			...
			topid = request.querystring("ord_attr")
			ty = request.querystring("ty_attr")
			if isnumeric(ty) = false or len(ty)=0 then ty = 0
			cn.execute "update #k_fail set 失败原因 = 失败原因 + '(' + 产品信息 + ')' from #pname where up_index=行号"
'if isnumeric(ty) = false or len(ty)=0 then ty = 0
			dim t
			t = now
			if isnumeric(topid) = false or len(topid) =  0 then
				Response.write "<script type='text/javascript'>alert('无法获取单据ID')</script>"
			else
				sql = "select isnull(num1,500) as num1,(select count(1) from " & db & ") + (select count(1) from chancelist where chance=" & topid & ") as num2 from setjm3 where ord=3"
				Response.write "<script type='text/javascript'>alert('无法获取单据ID')</script>"
				set rs = cn.execute(sql)
				toomore = true
				if rs.eof = false then
					if rs.fields(0).value < rs.fields(1).value then
						cn.execute "insert into #k_fail (行号,失败原因) values (NULL, '导入数据后明细行数为" & rs.fields(1).value & ",超出了系统设置的明细最多" & rs.fields(0).value & "行限制') "
						cn.execute "insert into #k_all (内容,说明) values ('行数限制检测','<span style=""color:red"">行数太多，导入终止。</span>')"
						toomore = false
					else
						cn.execute "insert into #k_all (内容,说明) values ('行数限制检测','正常')"
					end if
				end if
				rs.close
				if toomore = true then
					if zdycount > 0 then
						sql = "insert into chancelist (ord,price1,num1,money2,addcate,date2,chance,date1,date7,del,intro,bz,unit,pricejy,tpricejy," & join(zdyfields,",") & ")" & vbcrlf & _
						"                ""select 产品名称,isNull(convert(float,replace(单价,',','')),0) as 单价,cast(replace(数量,',','') as float) as 数量,isNull(cast(replace(单价,',','') as float),0)*cast(replace(数量,',','') as float),"" & app.Info.user & "",到货日期,"" & topid & "",NULL,getdate(),"" & app.iif(ty=0,7,1) & "",备注,0,单位,isNull(convert(float,replace(建议进价,',','')),0) as 建议进价,isNull(cast(replace(建议进价,',','') as float),0)*cast(replace(数量,',','') as float),"& join(zdynames,",") &  " from " & db
					else
						sql = "insert into chancelist (ord,price1,num1,money2,addcate,date2,chance,date1,date7,del,intro,bz,unit,pricejy,tpricejy)" & vbcrlf & _
						"                ""select 产品名称,isNull(convert(float,replace(单价,',','')),0) as 单价,cast(replace(数量,',','') as float) as 数量,isNull(cast(replace(单价,',','') as float),0)*cast(replace(数量,',','') as float),"" & app.Info.user & "",到货日期,"" & topid & "",NULL,getdate(),"" & app.iif(ty=0,7,1) & "",备注,0,单位,isNull(convert(float,replace(建议进价,',','')),0) as 建议进价,isNull(cast(replace(建议进价,',','') as float),0)*cast(replace(数量,',','') as float) from " & db
					end if
					cn.BeginTrans
					cn.execute sql
					if ty = 1 then
						sql =   "update chance set premoney= isnull(je,0), money1 = (case isnull(yhtype,0) when 0 then isnull(je,0)-isnull(yhmoney,0) else isnull(je,0)*isnull(zk,10)/10 end) , money2= (case isnull(yhtype,0) when 0 then isnull(je,0)-isnull(yhmoney,0) else isnull(je,0)*isnull(zk,10)/10 end) from (select isnull(sum(money2),0) as je from chancelist where chance=" & topid & ") tt where ord=" & topid & " "
						cn.execute sql
					else
						session("chancemoney_all") = cn.execute("select isnull(sum(money2),0) as je from chancelist where chance=" & topid).fields(0).value
						Dim mymoney
						mymoney=FormatNumber(session("chancemoney_all"),app.info.MoneyNumber,-1,0,0)
'Dim mymoney
					end if
					cn.CommitTrans
					Response.write "<script type='text/javascript'>var frm = window.parent.parent.document.getElementById('mxlist');var premoney = window.parent.parent.document.getElementById('premoney');var yh1 = window.parent.parent.document.getElementById('yh1');var yhvalue = window.parent.parent.document.getElementById('yhvalue');var moneyall = window.parent.parent.document.getElementById('moneyall');if(frm){frm.contentWindow.location.reload();premoney.value='"&mymoney&"';moneyall.value='"&mymoney&"';if(yh1.checked==false){yhvalue.value=10;}else{yhvalue.value=0;}}else{window.parent.parent.RreshElement('productlist,moneylabel1,moneylabel2,moneylabel3,moneylabel4')}</script>"
				end if
			end if
			uploader.AddReport(true)
		end sub
