...
function ask1()
{
	...
	if (bh!=""){
		...
		var url="../asset/Search_All.asp?tplink=5&id="+bhid+"&bh="+bh+"&s="+new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
		xmlHttp.open("GET", url, false);
		xmlHttp.onreadystatechange = function(){
			if (xmlHttp.readyState == 4){
...
function setjcz()
{
	...
	if(m=="" || isNaN(m))
	{m=0;}
	document.getElementById('ass_jcz').value = FormatNumber(parseFloat(y)*parseFloat(m),window.sysConfig.moneynumber);
}
