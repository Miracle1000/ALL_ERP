...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		rpt.sql = "select " &_
		" b.ord, b.title,g.name addName,g1.name delCate, convert(varchar(10),b.delDate,121) delDate, b.cateid addcate, b.ord id " &_
		" from (select * from kuzz where del=2 and isnull(isnew,0) = 1) b " &_
		" left join gate g on g.ord = b.cateid " &_
		" left join gate g1 on g1.ord = b.delcate " &_
	...
	Function doDelete(checkvalues, batResult)
		cn.execute("delete from kuzz where ord in (" & checkvalues & ")")
	end function
	Function doHF(checkvalues, batResult)
		cn.execute "update kuzz set del = 1,delcate = null,delDate = null where ord in (" & checkvalues & ")"
	end function
	Sub App_DoDel
		dim id
		id = request.Form("id")
	...
	Sub App_DoDelAll
		cn.execute("delete from kuzz where del = 2 and isnull(isnew,0) = 1")
		Response.write("true")
	end sub
	...
	Function TitleCell(byval v, ByVal rs)
		if app.power.existsPowerIntro(34, 14, rs("addcate")) Then
			TitleCell = "<a href='javascript:void(0)' onclick=""window.open('../store/contentzz.asp?ord=" & app.base64.pwurl(rs("ord")) & "&isnew=1','bomadd_view001','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">" & v & "</a>"
'if app.power.existsPowerIntro(34, 14, rs("addcate")) Then
		else
			TitleCell = v
		end if
