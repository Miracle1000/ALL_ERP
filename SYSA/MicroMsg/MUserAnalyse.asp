...
'bigsystemtype = ".mozi"
	...
	if open_7_11=3 then
		list_tj=""
	elseif open_7_11=1 then
		list_tj=" and cateid in ("&intro_7_11&") and cateid<>0 "
	else
	...
	if open_7_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_7_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
				If Len(rs1("sorce")&"")>0 then
					sorce=sorce&","&rs1("sorce")
				end if
				If Len(rs1("sorce2")&"") > 0 then
...
'dim sqlstr
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "body {" & vbcrlf & "  margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "#echarts{margin-top:115px;}" & vbcrlf & "</style>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0" & vbcrlf & "     eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf & "  if (restore) selObj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body >" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dim st,m1,m1name,m2,m2name,tpx,tpName,unit,unitName
	Dim hzb,zzb
	m1=request("ret")
	m2=request("ret2")
	tpx=request("tpx")
	...
	End Select
	If m1="" or m2="" Then
		Set rs = conn.execute("" & vbcrlf &_
		"SET nocount on " & vbcrlf &_
		"SET DATEFIRST 1 " & vbcrlf &_
		"select " &_
		"convert(varchar(10),dateadd(d,1-DATEPART(Weekday,getdate()),getdate()),121)," &_
		"convert(varchar(10),dateadd(d,7-DATEPART(Weekday,getdate()),getdate()),121)")
'select  &_
		m1 = rs(0)
		m2 = rs(1)
	end if
	m = conn.execute("" & vbcrlf &_
	"SET DATEFIRST 1 " & vbcrlf &_
	"select datediff(" & unit & ",'1970-01-01','" & m2 & "') - datediff(" & unit & ",'1970-01-01','" & m1 & "') + 1")(0)
'SET DATEFIRST 1  & vbcrlf &_
	m1name = "  起始：" & m1
	m2name = "  截止：" & m2
	...
	For i=0 To m - 1
'Dim dateStr : dateStr = ""
		dateStr = dateStr & iif(i=0,"",",") & i
	next
	if width_tj="" Then width_tj=550
	dim numcg1,zs1
	tdw = m1
	counts = m
	If counts< 0 Then counts= 1
	ReDim total1(counts,9)
	ReDim total2(counts,9)
	ReDim total3(counts,9)
	cellcountOne = 3
	zzb = tpName
	lburl = ""
	numcg1 = 0
	zs1 = 0
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "            <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                 <tr>" & vbcrlf & "                            <td class=""place"">走势统计分析</td>" & vbcrlf & "                              <td align=""left""><strong>"
	Response.write tpName
	Response.write unitName
	Response.write "走势</strong></td>" & vbcrlf & "                            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                      </tr>" & vbcrlf & "           </table>" & vbcrlf & "                <form method=""get"" style=""margin:0"" action=""?width_tj="
	Response.write width_tj
	Response.write """ id=""demo"" name=""date"" " & vbcrlf & "                       onSubmit=""return Validator.Validate(this,2)"">" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                      <tr class=""top"">" & vbcrlf & "                          <td>选择统计条件：" & vbcrlf & "      <select name=""unit"" style=""max-width:80px;"">" & vbcrlf & "                                                <option value=""dy"" "
	'Response.write width_tj
	Response.write iif(unit="dy"," selected","")
	Response.write ">每日走势</option>" & vbcrlf & "                                            <option value=""ww"" "
	Response.write iif(unit="ww"," selected","")
	Response.write ">每周走势</option>" & vbcrlf & "                                            <option value=""m"" "
	Response.write iif(unit="m"," selected","")
	Response.write ">每月走势</option>" & vbcrlf & "                                            <option value=""yyyy"" "
	Response.write iif(unit="yyyy"," selected","")
	Response.write ">每年走势</option>" & vbcrlf & "                                    </select>" & vbcrlf & "                                       "
	...
	Response.write "" & vbcrlf & "                                      <input type=""hidden"" name=""tpx"" value="""
	Response.write tpx
	Response.write """/>" & vbcrlf & "                                        <input type=""hidden"" name=""convert"" value="""
	Response.write convert
	Response.write """/>" & vbcrlf & "                                        <input type=""submit"" name=""Submit422"" value=""统计""  class=""anybutton2""/>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "           </table>" & vbcrlf & "                </form>" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "<tr> "& vbcrlf &                 "           <td height=""30""> "& vbcrlf &                            "       <div style=""float:right;display:inline-block""> "& vbcrlf &                    "                 <input type=""button"" name=""pillar"" value=""柱状图分析"" onClick=""gotourl('type_tj=1')"" class=""anybutton2""> "& vbcrlf &                       "            <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""gotourlolor:#2F496E;border:0px solid #FFFFFF;overflow:hidden"">" & vbcrlf & "                                           <option>-请选择-</option>" & vbcrlf & "                                               <option value=""550"" "
	'Response.write convert
	...
	if sdk.power.existsPower(108,7) Then
		Response.write "" & vbcrlf & "                                     <input type=""button"" name=""Submit43"" value=""打印""  onClick=""window.print()"" class=""anybutton2"">" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "                             &nbsp;" & vbcrlf & ""
	If sdk.power.existsPower(108,10) Then
		Response.write "" & vbcrlf & "                                     <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='sk_ys6.asp?type_tj=4&W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
	...
	'Response.write "" & vbcrlf & "                                     </div>" & vbcrlf & "                                  <div style=""float:left;display:inline-block"">" & vbcrlf & "                                             <span style=""margin-left:10px;height:15px;width:15px;background-color:#00ff00;border:1px solid;"">&nbsp;</span>" & vbcrlf & "                                            <span style=""padding-left:3px;"">净增数量</span>" & vbcrlf & "                                           <span style=""margin-left:10px;height:15px;width:15px;background-color:#ff0000;border:1px solid;"">&nbsp;</span>" & vbcrlf & "                                            <span style=""padding-left:3px;"">转客户数量</span>" & vbcrlf & "                                         <span style=""margin-left:10px;height:15px;width:15px;background-color:#ff9900;border:1px solid;"">&nbsp;</span>" & vbcrlf & "                                          <span style=""padding-left:3px;"">成功客户数量</span>" & vbcrlf & "                                       </div>" & vbcrlf & "                          </td>" & vbcrlf & "                   </tr>" & vbcrlf & "           </table>" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" id=""user2"">" & vbcrlf & "        <tr bgcolor=""#FFFFFF"">" & vbcrlf & "                            <td align=""left"">" & vbcrlf & "                                 <div align=""left"">" & vbcrlf & "                                                <span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt;""><B>正在处理数据...</B></span>" & vbcrlf & "                                 </div>" & vbcrlf & "                          </td>" & vbcrlf & "                   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "                            <td colspan=""2"" class=""tablebody1"">" & vbcrlf & "                                 <table width=""600"" border=""0"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                                               <tr bgcolor=""#FFFFFF"">  " & vbcrlf & "                                                  <td bgcolor=ffffff height=9>" & vbcrlf & "                                                            <imgsrc=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage2"" name=""CountImage2"" align=""absmiddle"">" & vbcrlf & "                                                        </td>" & vbcrlf & "</tr>" & vbcrlf & "                                   </table>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   <tr bgcolor=""#FFFFFF"">" & vbcrlf & "                            <td>" & vbcrlf & "                                    <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span>" & vbcrlf & "                                     <span style=""font-size:9pt; color:#008040"">%</span>" & vbcrlf & "                          </td>" & vbcrlf & "                   </tr>" & vbcrlf & "           </table>" & vbcrlf & "        </div>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "                <tr>" & vbcrlf & "                    <td align=""center"">" & vbcrlf & ""
	Dim qxlist : qxlist = sdk.power.getPowerIntro(108,11)
	Dim powerCondition
	If qxlist = "" Then
		powerCondition = ""
	else
	...
	sql = "" & vbcrlf &_
	"select isnull(a1.cnt,0) cntNew,isnull(a2.cnt,0) cntConvert,isnull(a3.cnt,0) cntSuccess," & vbcrlf &_
	"t.d,dateadd(" & unit & ",t.d,'" & m1 & "') dt " & vbcrlf &_
	"from (" & vbcrlf &_
	"select cast(short_str as int) d from dbo.split('" & dateStr & "',',')" & vbcrlf &_
	") t " & vbcrlf &_
	"left join (" & vbcrlf &_
	"select count(*) cnt,datediff(" & unit & ",createtime,'" & m1 & "') tt " & vbcrlf &_
	"from MMsg_User " & vbcrlf &_
	"where datediff(" & unit & ",createtime,'" & m1 & "') <= 0 and datediff(" & unit & ",createtime,'" & m2 & "') >= 0 " & vbcrlf &_
	"group by datediff( "& unit & ",createtime,' "& m1 &_
	"')) a1 on a1.tt = t.d " & vbcrlf &_
	"left join (" & vbcrlf &_
	"select count(*) cnt,datediff(" & unit & ",bindTime,'" & m1 & "') tt " & vbcrlf &_
	"from MMsg_User " & vbcrlf &_
	"where datediff(" & unit & ",bindTime,'" & m1 & "') <= 0 and datediff(" & unit & ",bindTime,'" & m2 & "') >= 0 " & vbcrlf &_
	"and isnull(person,0) > 0 " & powerCondition & " " & vbcrlf &_
	"group by datediff(" & unit & ",bindTime,'" & m1 & "') " & vbcrlf &_
	") a2 on a2.tt = t.d" & vbcrlf &_
	"left join (" & vbcrlf &_
	"select count(*) cnt,datediff(" & unit & ",ct.date7,'" & m1 & "') tt from (" & vbcrlf &_
	"select a.id,min(ct.date7) date7 " & vbcrlf &_
	"from MMsg_User a" & vbcrlf &_
	"inner join person b on a.person = b.ord " & vbcrlf &_
	"inner join tel c on b.company = c.ord " & vbcrlf &_
	"inner join contract ct on ct.company=c.ord " & vbcrlf &_
	"where ct.del=1 and isnull(ct.cateid_sp,0) = 0 " & powerCondition & " " & vbcrlf &_
	"and datediff(" & unit & ",ct.date7,'" & m1 & "') <= 0 " & vbcrlf &_
	"and datediff(" & unit & ",ct.date7,'" & m2 & "') >= 0 " & vbcrlf &_
	"and c.ord not in (" & vbcrlf &_
	"select top 1 1 from contract where company=c.ord and del=1 and isnull(cateid_sp,0) = 0 " & vbcrlf &_
	"and datediff(" & unit & ",ct.date7,'" & m1 & "') > 0 " & vbcrlf &_
	")" & vbcrlf &_
	"group by a.id " & vbcrlf &_
	") ct " & vbcrlf &_
	"group by datediff(" & unit & ",ct.date7,'" & m1 & "') " & vbcrlf &_
	") a3 on a3.tt = t.d" & vbcrlf &_
	"order by t.d "
    'response.write sql : response.end
	total2(0,1)="#00ff00,1.5,1,2," & unitName & "走势"
	total2(0,2)="#ff0000,1.5,1,2," & unitName & "走势"
	total2(0,3)="#ff9900,1.5,1,2," & unitName & "走势"
	set rs = conn.execute(sql)
	While rs.eof = False
		numcg1 = rs(0)
		numcg2 = rs(1)
		...
		j=rs("d") + 1
		numcg3 = rs(2)
		title = dateFormat(rs("dt"),unit)
		lburl="../MicroMsg/MUserList.asp?fromTj=1&qc=" & (j-1) & "&dtUnit=" & unit & "&m1=" & Split(m1," ")(0) & "&m2=" & Split(m2," ")(0) & "&tpx="
		'title = dateFormat(rs("dt"),unit)
		total1(j,0)=lburl & "new"
		total1(j,1)=numcg1
		total1(j,2)=title
		total1(j,3)=lburl & "convert"
		total1(j,4)=numcg2
		total1(j,5)=title
		total1(j,6)=lburl & "success"
		total1(j,7)=numcg3
		total1(j,8)=title
		total2(j,0)=title
		total2(j,1)=numcg1
		total2(j,2)=lburl & "new"
		total2(j,3)=title
		total2(j,4)=numcg2
		total2(j,5)=lburl & "convert"
		total2(j,6)=title
		total2(j,7)=numcg3
		total2(j,8)=lburl & "success"
		total3(j,0)=title
		total3(j,1)=numcg1
		total3(j,2)=lburl & "new"
		total3(j,3)=title
		total3(j,4)=numcg2
		total3(j,5)=lburl & "convert"
		total3(j,6)=title
		total3(j,7)=numcg3
		total3(j,8)=lburl & "success"
		zs1 = zs1 + numcg1 + numcg2 + numcg3
		'total3(j,8)=lburl & "success"
	...
	wend
	zzb="数量"
	action1= tpName & unitName & "走势"
	Function dateFormat(dt,tpx)
		Select Case tpx:
		Case "dy":
		dateFormat =  year(dt) & "-" & month(dt) & "-" & day(dt)
'Case "dy":
		Case "ww":
		dateFormat =  "第" & datepart("ww",dt) & "周"
		Case "m":
		dateFormat =  year(dt) & "年" & month(dt) & "月"
		Case "yyyy":
		dateFormat =  year(dt) & "年"
		Case Else :
		dateFormat =  year(dt) & "-" & month(dt) & "-" & day(dt)
'Case Else :
		End Select
