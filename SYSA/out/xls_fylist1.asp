...
'if strW3<>"" Then
		...
		Sub page_load
			Set conn = cn
			Dim open_6_10,intro_6_10
			Call fillPowerInfo(6,10,open_6_10,intro_6_10)
			Dim list1
			if open_6_10=3 then
				list1=" "
			elseif open_6_10=1 then
				list1=" and c.cateid in ("&intro_6_10&")"
			else
				list1=" and c.cateid=0"
			end if
			Str_Result=" "&list1&" "
			Dim m1,m2,m3,m4,B,C,px,typ,title,intro,money3,money12,e,f,tjr,jkr,listbz,payid,stel,scontract,sperson,sricheng
			Dim sshouhou,schance,scaigou,sfahuo,swork,W1,W2,W3,W4,strtmp,tmparr,str,pxResult,SQL
			if request("ret")<>"" then
				m1=request("ret")
				m2=request("ret2")
			else
			...
			if money3<>"" then
				if isnumeric(money3)=false then
					Response.write("<script>alert('请输入合法金额');</script>")
					call db_close : Response.end
				else
					Str_Result = Str_Result & " and b.money1 >="&money3
				end if
			end if
			if money12<>"" then
				if isnumeric(money12)=false then
					...
					Str_Result = Str_Result & " and b.money1 <="&money12
				end if
			end if
...
'if request("ret_sp3")<>"" and request("ret_sp4")<>"" then
			...
			if m1<>"" then
				Str_Result=Str_Result & " and a.bxdate>='"&m1&"' "
			end if
			if m2<>"" then
				Str_Result=Str_Result & " and a.bxdate<='"&m2&"' "
			end if
			if m3<>"" then
			...
			if px="" Then px=5
			if px=1 then
				pxResult=" order by a.title desc,a.indate desc"
			elseif px=2 then
				pxResult=" order by a.title asc,a.indate asc"
			elseif px=5 then
				pxResult=" order by a.bxdate desc,a.indate desc"
			elseif px=6 then
				pxResult=" order by a.bxdate asc,a.indate asc"
			elseif px=3 then
				pxResult=" order by a.indate desc"
			elseif px=4 then
				pxResult=" order by a.indate asc"
			end if
			SQL = "select a.id bxid,b.id bxlistid,c.ord payid,a.bh 报销编号,a.title 报销标题,"&vbcrlf&_
			"case when a.complete=0 then '待审批' "&vbcrlf&_
			"when a.complete=2 then '审批未通过' "&vbcrlf&_
			"when a.complete=3 then '审批通过' "&vbcrlf&_
			"else '审批中' end 报销状态,"&vbcrlf&_
			"b.intro 报销备注,isnull(f.sort1,'人民币') 币种,"&vbcrlf&_
			"isnull(c.money1,0) 报销金额,"&vbcrlf&_
			"(case when a.complete in(0,2) then 0 else isnull(b.money1,0) end) 审批金额,"&vbcrlf&_
			"a.indate 添加日期,a.bxdate 报销日期,"&vbcrlf&_
			"d.name 报销人员,isnull(h.sort1,'')+'→'+isnull(g.sort1,'')费用分类,fy.title 关联使用单 " &vbcrlf&_
			"from paybx a "&vbcrlf&_
			"inner join paybxlist b on a.id=b.bxid "&vbcrlf&_
			"left join pay c on c.ord=b.payid "&vbcrlf&_
			"left join f_pay fy on fy.id=c.fid "&vbcrlf&_
			"left join gate d on c.cateid=d.ord "&vbcrlf&_
			"left join gate e on a.addcate=e.ord "&vbcrlf&_
			"left join sortbz f on f.id=a.bz "&vbcrlf&_
			"left join paytype g on g.id=b.sort "&vbcrlf&_
			"left join sortonehy h on h.ord=g.sort2 "&vbcrlf&_
			"where a.del=1 and b.del=1 " & Str_Result & pxResult
			Dim Fields
			Fields = "报销编号,205;报销标题,205;报销状态,87;报销备注,245;币种,101;报销金额,165;审批金额,165;添加日期,165;报销日期,165;报销人员,80;费用分类,165;关联使用单,205;bxid;bxlistid;payid"
			Dim adapter
			Dim sqls,sheetSetting,i,item
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "费用报销明细"
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = SQL
			sheetSetting.title = "费用报销明细"
			Dim hd,arrFields,arrField
			arrFields = Split(Fields,";")
			For i = 0 To ubound(arrFields)
				arrField = Split(arrFields(i),",")
				Set hd = New HeaderColumnSetting
				hd.dbname = arrField(0)
				hd.cansum = False
				...
				hd.dbtype = "str"
				hd.excelAlign = "left"
				Case "bxid","bxlistid","payid":
				hd.display = "none"
				Case "报销备注":
				hd.dbtype = "str"
				hd.ignoreHTMLTag = True
				hd.excelAlign = "left"
				Case Else:
				hd.dbtype = "str"
		...
		Function myFormatNumber(bz,v,n)
			If v<>"" Then
				myFormatNumber = bz & "&nbsp;" & FormatNumber(v,n,-1,0,-1)
'If v<>"" Then
			else
				myFormatNumber = ""
			end if
