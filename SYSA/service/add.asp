...
'end if
	...
	sub ConflictProcHandle
		If isAjaxRequest Then ret
		Err.clear
		on error resume next
		if len(request.form & "") > 0 Then
			If Err.number = 0 Then ret
		end if
		If Err.number <> 0 Then
			On Error GoTo 0
		...
		For i= 0 To ubound(exiturl)
			If InStr(1, url, exiturl(i), 1)>0 Then ret
		next
		on error resume next
		Dim cftManger: Set cftManger = Nothing
		Set cftManger = server.createobject(ZBRLibDLLNameSN & ".ConflictManger")
		If cftManger Is Nothing Then Err.clear: ret
...
'If Not rs.Eof Then
	...
	If msgid&""<>"" Then
		...
		'Response.clear
		Select Case msgid&""
		Case "serviceTemplateChange" : Call serviceTemplateChange()
		End Select
		conn.close
		Set conn = Nothing
		Response.end
	...
	Response.write "</title>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "#content tr.top td {" & vbcrlf & "     background-image: url(../images/m_table_top.jpg);" & vbcrlf & "       background-repeat: repeat-x;" & vbcrlf & "    text-align:left;" & vbcrlf & "        line-height:20px;" & vbcrlf & "       font-weight: bold;" & vbcrlf& "        color: #2F496E;" & vbcrlf & "}" & vbcrlf & "td{" & vbcrlf & "     line-height:20px;" & vbcrlf & "}" & vbcrlf & "iframe{" & vbcrlf & "       width:95%;" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/se_add.js?ver=3""  language=""JavaScript"" type=""text/JavaScript""></script>" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFiles.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../publiccla/ajax.js?ver="
	Response.write Application("sys.info.jsver")
	...
	If addtype & "" = "tel" Then
		...
		rs.open sql,conn,1,1
		if rs.eof then
			Response.write"<script language=javascript>alert('此客户已被删除！');history.back()</script>"
			Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
			call db_close : Response.end
		else
			name=rs("name")&"<input name='company' value='"&ord&"' type='hidden'>" : session("companyzbintel") = ord
		end if
		rs.close
		set rs=nothing
	end if
	If addtype & "" = "contract" Then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select company,title from contract where ord="&ord&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			Response.write"<script language=javascript>alert('此合同已被删除！');history.back()</script>"
			Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
			call db_close : Response.end
		else
			company=rs7("company") : session("contractzbintel")=ord : contractTitle = rs7("title")
		end if
		rs7.close
		set rs7=Nothing
		If company & "" = "" Or isnumeric(company & "") = False Then
			company = 0
		end if
		set rs=server.CreateObject("adodb.recordset")
		sql="select name from tel where ord="&company&""
		rs.open sql,conn,1,1
		if rs.eof then
			Response.write"<script language=javascript>alert('此客户已被删除！');history.back()</script>"
			Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
			...
			name=rs("name")&"<input name='company' value='"&company&"' type='hidden'>"
			session("companyzbintel") = company
		end if
		rs.close
		set rs=nothing
	end if
	If addtype & "" = "chance" Then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select company from chance where ord="&ord&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			Response.write"<script language=javascript>alert('此项目已被删除！');history.back()</script>"
			Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
			call db_close : Response.end
		else
			company=rs7("company")
		...
		If company & "" = "" Or isnumeric(company & "") = False Then
			...
			if right(company&"",1)&"" = "," then
				company = company &"0"
			end if
		end if
		set rs=server.CreateObject("adodb.recordset")
		sql="select name,ord from tel where ord in ("&company&")"
		rs.open sql,conn,1,1
		if rs.eof then
			Response.write"<script language=javascript>alert('此客户已被删除！');history.back()</script>"
			Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
			...
			name = "<select name=""company"" onchange=""companychange(this.value)"" dataType=""Limit"" min=""1"" max=""100"" msg=""请选择客户"">" &_
			"<option value=""""></option>"
			While rs.eof =False
				name = name & "<option value=" & rs("ord") & ">" & rs("name") & "</option>"
				rs.movenext
			wend
			name = name & "</select>"
		end if
	...
	If addtype & "" = "edit" Then
		set rs=server.CreateObject("adodb.recordset")
		sql="select *,ISNULL(share,-1) nullSare  from tousu Where ord="&ord&" "
		'set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if rs.eof then
		...
		ord=rs("ord")
		if isnull(rs("way1")) or rs("way1")="" then
			way1=0
		else
			way1=rs("way1")
		end if
		title=trim(rs("title"))
		shid=rs("shid")
		if Isnull(rs("share")) or rs("share")="" then
			share=0
		else
			share=rs("share")
		end if
		...
		id = rs("moduleID")
		session("companyzbintel")=company
		session("contractzbintel")=rs("contract")
		nullSare = rs("nullSare")
		rs.close
		set rs = nothing
		If company & "" = "" Or isnumeric(company & "") = False Then
			company = 0
		...
		if rs.eof then
			...
			name=rs("name") : session("companyzbintel") = company
		end if
		rs.close
		set rs=Nothing
		If person & "" = "" Or isnumeric(person & "") = False Then
			person = 0
		end if
		set rs=server.CreateObject("adodb.recordset")
		sql="select name from person where ord="&person&""
		rs.open sql,conn,1,1
	...
	ty=request("ty")
	uid = session("personzbintel2007")
	sql="Delete tousu where addcate="& uid &" and del=7"
	conn.Execute(sql)
	If addtype & "" <> "edit" Then
		set rs88 = conn.execute("EXEC erp_getdjbh 42,"&session("personzbintel2007"))
		khid=rs88(0).value
		set rs88=nothing
		sqlStr="Insert Into tousu(addcate,date7,shid,del) values('"
		sqlStr=sqlStr & uid & "','"
		sqlStr=sqlStr & date & "','"
		sqlStr=sqlStr & khid & "','"
		sqlStr=sqlStr & 7 & "')"
		Conn.execute(sqlStr)
		shord = GetIdentity("tousu","ord","addcate","")
	else
		khid = shid
		shord = ord
	end if
	...
	curmain=request("curmain")
	if curmain="" then curmain=7
	S_result="and del=7"
	if id<>"" then
		S_result=S_result + "and ord="&id&""
'if id<>"" then
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select *  from tousu Where main="&main&"  "&S_result&""
	rs.open sql,conn,1,1
	if not rs.eof then
		id=rs("ord")
		name1=conver(rs("name1"))
		name2=conver(rs("name2"))
		name3=conver(rs("name3"))
		name4=conver(rs("name4"))
		name5=conver(rs("name5"))
		name6=conver(rs("name6"))
		name7=conver(rs("name7"))
		name8=conver(rs("name8"))
		date11=rs("date11")
		date12=rs("date12")
		date13=rs("date13")
		date14=rs("date14")
		...
		If id<>"" Then
			conn.execute("update tousu set main = 7,moduleID = (select top 1 ord from tousu where main = 7 and del = 7) where ord = " &ord)
			call db_close : Response.redirect("?ord="&pwurl(ord)&"&addtype=edit") :Response.end
		else
			Response.write "<script language='javascript'>alert('没有设置售后流程主模板！请联系管理员设置！');location.href='event.asp';</script>"
			call db_close : Response.end
		end if
	end if
	rs.close
	set rs=Nothing
	session("promenu_list2_sh")=""
	session("areamenu_list")=""
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "      <tr>" & vbcrlf & "            <td width=""100%"" valign=""top"">" & vbcrlf & "              <form method=""POST"" action=""save.asp?ty="
	Response.write ty
	Response.write "&refreshParent="
	Response.write request("refreshParent")
	Response.write """ id=""demo"" onsubmit=""return Validator.Validate(this,2) && DelUnusedFilesBeforeSubmit()"" name=""date"">" & vbcrlf & "                   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                         <tr>" & vbcrlf & "                                    <td class=""place place42"">"
	If addtype & "" <> "edit" Then Response.write("售后添加") Else Response.write("售后修改")
	Response.write "</td>" & vbcrlf & "                                        <td>&nbsp;</td>" & vbcrlf & "                                 <td align=""right"">&nbsp;</td>" & vbcrlf & "                                     <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""31"" /></td>" & vbcrlf & "                                </tr>" & vbcrlf & "                 </table>" & vbcrlf & "                        <table width=""100%"" id=""content"" bgColor=""#c0ccdd"" border=""0"" cellSpacing=""1"" cellPadding=""4"" sizset=""1"" sizcache05812039326002498=""1"">" & vbcrlf & "                          <tr  class=""top noNeedLineIcon"">" & vbcrlf & "                                  <td   colspan=""6"" align=""left"">" & vbcrlf & "                                             <div align=""right"">" & vbcrlf & "                                                       <input type=""submit"" name=""Submit42"" value=""保存""  class=""page""/>" & vbcrlf & "                                                       <input type=""reset"" value=""重填"" class=""page"" name=""B2"">" & vbcrlf & "                                                </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">关联客户：</div></td>" & vbcrlf & "                                       <td width=""20%"">" & vbcrlf & "                                          <span id=""khmc"">" & vbcrlf & "                                                  "
	Select Case addtype
	Case "tel","contract","chance"
	Response.write(name)
	Case "","edit"
	Response.write "" & vbcrlf & "                                                     <input name=""companyName"" value="""
	Response.write name
	Response.write """ type=""text"" "
	If Len(name&"") = 0 Then
		Response.write "style=""color: #999999;text-align:center;"""
'If Len(name&"") = 0 Then
	end if
	Response.write " size=""20"" onClick=""javascript:window.open('../search/result4.asp','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;"" readonly dataType=""Limit"" min=""1"" max=""50"" msg=""请选择客户"">" & vbcrlf & "                                                     <inputname=""company"" value="""
	Response.write company
	Response.write """ type=""hidden"">" & vbcrlf & "                                                    "
	End Select
	Response.write "" & vbcrlf & "                                                     <span class=""red"">*</span>" & vbcrlf & "                                                </span>" & vbcrlf & "                                 </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">售后编号：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <input name=""shid"" type=""text"" size=""15"" value="""
	Response.write khid
	Response.write """ class='jquery-auto-bh' autobh-options='cfgId:42,recId:"
	'Response.write khid
	Response.write shord
	Response.write ",autoCreate:false'>" & vbcrlf & "                                  </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">"
	'Response.write shord
	If addtype & "" <> "edit" Then Response.write("售后模板：") Else Response.write("&nbsp;")
	Response.write "</div></td>" & vbcrlf & "                                  <td width=""20%"">" & vbcrlf & "                                          <select name=""main"" onChange=""serviceTemplateChange()"" style=""width:100%;"
	If addtype & "" = "edit" Then Response.write("display:none")
	Response.write """>" & vbcrlf & "                                                        <option value="""">选择售后模板</option>" & vbcrlf & "                                                    "
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select ord,main,title from tousu where (main=2 or main=7) and del=7 order by main asc"
	rs7.open sql7,conn,1,1
	do until rs7.eof
		if clng(id)=rs7("ord") then
			curmain=rs7("main")
		end if
		Response.write "" & vbcrlf & "                                                     <option value="""
		Response.write rs7("main")
		Response.write """ curid="""
		Response.write rs7("ord")
		Response.write """ "
		if rs7("ord")=clng(id) then
			Response.write "selected"
		end if
	...
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               <input type=""hidden"" name=""curmain"" value="""
	Response.write curmain
	Response.write """ />" & vbcrlf & "                                              <input type=""hidden"" name=""curid"" value="""
	Response.write id
	Response.write """ />" & vbcrlf & "                                              <input type=""hidden"" name=""id"" value="""
	Response.write id
	Response.write """ />" & vbcrlf & "                                              <input type=""hidden"" name=""companyord"" id=""companyord"" value=""0"">" & vbcrlf & "                                               <input type=""hidden"" name=""shord"" value="""
	Response.write shord
	Response.write """>" & vbcrlf & "                                                <input type=""hidden"" name=""ord"" value="""
	Response.write ord
	Response.write """>" & vbcrlf & "                                                <input type=""hidden"" name=""addtype"" value="""
	Response.write addtype
	Response.write """>" & vbcrlf & "                                        </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">售后人员：</div></td>" & vbcrlf & "                                 <td colspan=""5"" style='padding:2px!important;'>" & vbcrlf & "                                           <table width=""98%""  border=""0"" align=""center""  bgcolor=""#FFFFFF"" > "& vbcrlf &"                                                      <tr> "& vbcrlf &"                                                             <td height=""37"" colspan=""3"" class=""gray""  style"
	If share & "" = "" Then
		share = 0
	end if
	check1 = ""
	...
	If nullSare&"" = "-1" Or nullSare&"" = "" Then
		check1 = "checked" : check2 = ""
		sort_display = ""
		check1 = "checked" : check2 = ""
	end if
	CurrBookID = ord
	Response.write "" & vbcrlf & "                                                                             <tr>" & vbcrlf & "                                                                                    <td colspan=""4""><input name=""member1"" id=""secwk"" type=""radio"" value=""1"" onClick=""document.getElementById('sort_member').style.display=''""; "
	Response.write check1
	Response.write ">" & vbcrlf & "                                                                                            <label for=""secwk"" style=""cursor:pointer""><strong>共享给以下用户</strong>（在需要共享用户前面的方框打勾）<span id=""lbmsg"" style=""color:red""></span></label>" & vbcrlf & "                                                                                     </td>" & vbcrlf & "                                                                           </tr>" & vbcrlf & "                                                                                   <tr id=""sort_member"" style="""
	Response.write sort_display
	Response.write """>" & vbcrlf & "                                                                                        <td colspan=""4"" height=""16"">"
	Response.write "<script type=""text/javascript"">" & vbcrlf & "function GE(fobj){return document.getElementById(fobj);}" & vbcrlf & "" & vbcrlf & "function showBumen(chkzu,ti,gate,id1)" & vbcrlf & "{" & vbcrlf & "    GE('Wt_'+ti+'_'+gate+'_'+id1).style.display=(chkzu.checked==true?'':'none');" & vbcrlf & "    GE('s_'+ti+'_'+gate+'_'+id1).style.display=(chkzu.checked==true?'':'none');" & vbcrlf & "}" & vbcrlf & "</script>"
	ReDim d_at(54)
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	sorceStr2 = " "
	if CurrBookID<>"" then
		SeaStr = " and ord in (select ord from power where sort1=42 and sort2=13 and del=1 and (qx_open=1 or qx_open=3))"
	else
		SeaStr = " and ord<>"& session("personzbintel2007") &" and ord in (select ord from power where sort1=42 and sort2=13 and del=1 and (qx_open=1 or qx_open=3))"
	end if
	set rs2 = conn.execute("select sorce,sorce2 from gate where cateid>1"&SeaStr)
	while not rs2.eof
		sorce = rs2("sorce")
		sorce2 = rs2("sorce2")
		if instr(sorceStr," "& sorce &" ")<1 then
			sorceStr = sorceStr & sorce &" "
		end if
		if instr(sorceStr2," "& sorce2 &" ")<1 then
			sorceStr2 = sorceStr2 & sorce2 &" "
		end if
		rs2.movenext
	wend
	rs2.close
	set rs2 = nothing
	if share<>"" Then
		If Left(share,1) = "," Then share = "0"&share
		set rs2 = conn.execute("select sorce,sorce2 from gate where ord in("& share &")")
		while not rs2.eof
			Correct_W2 = Correct_W2 & rs2("sorce") &" "
			Correct_W3 = Correct_W3 & rs2("sorce2") &" "
			rs2.movenext
		wend
		rs2.close
		set rs2 = nothing
		if trim(Correct_W3) <>"" then
			Correct_W2 = replace(trim(Correct_W2)," ",",")
			Correct_W3 = replace(trim(Correct_W3)," ",",")
		end if
	end if
	set rs8=server.CreateObject("adodb.recordset")
	sql="select ord,orgsid from gate where del=1 "& SeaStr
	Response.write CBaseUserTreeHtml(sql,"", "","","W3", "", "" , "", share)
	Response.write "</td>" & vbcrlf & "                                                                                </tr>" & vbcrlf & "                                                                           <tr>" & vbcrlf & "                                                                                    <td colspan=""4"" height=""16"">" & vbcrlf & "                                                                                                <input type=""radio"" name=""member1"" id=""allwk"" value=""0""  onclick=""document.getElementById('sort_member').style.display='none';document.getElementById('lbmsg').innerHTML='';"" "
	Response.write check2
	Response.write ">" & vbcrlf & "                                                                                            <label for=""allwk"" style=""cursor:pointer""><strong>共享给所有用户</strong></label>" & vbcrlf & "                                                                                   </td>" & vbcrlf & "                                                                           </tr>  " & vbcrlf & "                                                                 </table>" & vbcrlf & "                                                                </td>" & vbcrlf & "                                                   <tr>" & vbcrlf & "                                            </table>" & vbcrlf & "                        </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">售后主题：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <input name=""title"" type=""text"" dataType=""Limit"" min=""1"" max=""50"" value="""
	'Response.write check2
	Response.write title
	Response.write """ msg=""主题必须在1到50个字之间"">" & vbcrlf & "                                            <span class=""red""> *</span>" & vbcrlf & "                                       </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">售后分类：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          "
	'Response.write title
	set rs7=server.CreateObject("adodb.recordset")
	if sort1="" or isnull(sort1) then sort1=0
	sql7="select * from sortonehy where ord="&sort1&""
	rs7.open sql7,conn,1,1
	dim sort1,sort1name
	if rs7.eof then
		sort1=0
		...
		sort1=rs7("ord")
		sort1name=rs7("sort1")
		session("promenu_list2_sh")=rs7("ord")
		session("areamenu_list")=""
	end if
	rs7.close
	set  rs7=nothing
	Response.write "" & vbcrlf & "                                             <IFRAME name=""I2"" SRC=""../sort3/correct_Service.asp?ID=company&sort1="
	Response.write sort1
	Response.write """ FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""23"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "                                  </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">售后方式</div></td>" & vbcrlf & "                                   <td width=""20%"">" & vbcrlf & "                                          <select name=""way1"" class=""gray"">" & vbcrlf & "                                                 "
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord, sort1 from sortonehy where gate2=55 order by gate1 desc"
	rs.open sql,conn,1,1
	do until rs.eof
		Response.write "" & vbcrlf & "                                                     <option value="""
		Response.write rs("ord")
		Response.write """ "
		If way1&"" = rs("ord")&"" Then Response.write(" selected")
		Response.write ">"
		Response.write rs("sort1")
		Response.write "</option>" & vbcrlf & "                                                    "
		i=i+1
	...
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                       </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">关联联系人：</div></td>" & vbcrlf & "                                       <td width=""20%"">" & vbcrlf & "                                          <input name=""person"" id=""telmc"" type=""text"" size=""15"" onClick=""javascript:this.blur();window.open('../search2/result2ht.asp','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;"" readonly dataType=""Limit"" value="""
	set rs=nothing
	Response.write personname
	Response.write """ min=""0"" max=""50""  msg=""长度必须在1至50个字之间""><input type=""hidden"" name=""personOrd"" id=""personOrd"" value="""
	Response.write person
	Response.write """ />" & vbcrlf & "                                      </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">固定电话：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <input name=""phone"" value="""
	'Response.write person
	Response.write phone
	Response.write """ id=""phone"" type=""text"" size=""15"" dataType=""PhoneNull"" min=""0"" max=""50""  msg=""格式不正确"">" & vbcrlf & "                                     </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">手 机：</div></td>" & vbcrlf & "                                    <td width=""20%"">" & vbcrlf & "                                          <input name=""mobile"" value="""
	Response.write mobile
	Response.write """ id=""mobile"" type=""text"" size=""15"" dataType=""MobileNull"" min=""0"" max=""50""  msg=""格式不正确"">" & vbcrlf & "                                   </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">售后日期：</div></td>" & vbcrlf & "                                 "
	'Response.write mobile
	If date1 & "" = "" Then date1 = Date()
	Response.write "" & vbcrlf & "                                     <td width=""20%"">" & vbcrlf & "                                          <INPUT name=ret onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" value="""
	Response.write date1
	Response.write """ size=10  dataType=""Date"" format=""ymd""  msg=""日期格式不正确"">" & vbcrlf & "                                          <input type=""button"" class=""anybutton"" value=""选择日期"" align=""absMiddle""  border=""0""  id=""daysOfMonthPos2"" name=""daysOfMonthPos""  onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" />" & vbcrlf &"                                         <DIV id=daysOfMonth style=""POSITION: absolute""></DIV>" & vbcrlf & "                                             <span class=""red"">*</span>" & vbcrlf & "                                        </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">具体时间：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <select name=""time1"" class=""gray"">" & vbcrlf & "                                                 "
	If time1 & "" = "" Or isnumeric(time1 & "") = False Then
		time1 = 0
	end if
	For i = 0 To 23
		Selected = ""
		If CInt(time1) = i Then
			Selected = " selected"
		end if
		h = Right("00" & i,2)
		Response.write("<option value='" & h & "'" & Selected & ">" & h & "</option>")
	next
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               时" & vbcrlf & "                                              <select name=""time2"" class=""gray"">" & vbcrlf & "                                                  "
	If time2 & "" = "" Or isnumeric(time2 & "") = False Then
		time2 = 0
	end if
	For i = 0 To 11
		Selected = ""
		If CInt(time2) = (i*5) Then
			Selected = " selected"
		end if
		h = Right("00" & (i*5),2)
		Response.write("<option value='" & h & "'" & Selected & ">" & h & "</option>")
	next
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               分" & vbcrlf & "                                      </td>" & vbcrlf & "                                   <td width=""10%""><div style=""text-align:right"">紧急程度：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <select name=""great1"" class=""gray"">" & vbcrlf & "                                         "
	Response.write("<option value='" & h & "'" & Selected & ">" & h & "</option>")
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from sortonehy where gate2=52 order by gate1 desc"
	rs.open sql,conn,1,1
	do until rs.eof
		Response.write "" & vbcrlf & "                                             <option value="""
		Response.write rs("ord")
		Response.write """ "
		If great1&"" = rs("ord")&"" Then Response.write(" selected")
		Response.write ">"
		Response.write rs("sort1")
		Response.write "</option>" & vbcrlf & "                                            "
		i=i+1
...
'If addtype & "" <> "edit" Then
	...
	If tbNum > 0 Then
		...
		if ZBRuntime.MC(7000) then
			Response.write "" & vbcrlf & "                                      <td width=""10%""><div style=""text-align:right"">关联合同：<input type=""hidden"" class=""initCompany_css"" value=""""></div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          "
'if ZBRuntime.MC(7000) then
			If addtype&"" <> "contract" Then
				Response.write "" & vbcrlf & "                                              <IFRAME id=""link_contract"" name=""I2"" SRC=""../event/contract.asp"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""220"" HEIGHT=""23"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "                                                "
			else
		...
		If addtype & "" <> "edit" Then
			Response.write "" & vbcrlf & "                                      <td width=""10%""><div style=""text-align:right"">涉及产品：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <IFRAME name=""I2"" SRC=""../event/product.asp?ID=product&uitype=short"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""350"" HEIGHT=""23"" marginwidth=""1""style=""width:100%"" marginheight=""1""></IFRAME> "& vbcrlf &"                                  </td> "& vbcrlf &"    "
		end if
		If addtype & "" <> "edit" Then
			Response.write "" & vbcrlf & "                                      <td width=""10%""><div style=""text-align:right"">指派售后：</div></td>" & vbcrlf & "                                 <td width=""20%"">" & vbcrlf & "                                          <div id=""leadDiv"" style=""display:"
'If addtype & "" <> "edit" Then
			Response.write iif(i>0,"","none")
			Response.write ";"">" & vbcrlf & "                                                "
			Response.write "" & vbcrlf & "                                              <select name=""lead"" class=""gray"" dataType=""Limit"" min="""
			Response.write iif(i>0,"1","0")
			Response.write """ max=""60"" msg=""请选择下一阶段的售后人员"">" & vbcrlf & "                                                     <option></option>" & vbcrlf & "                                                       "
			Call getGateLTable("select ord,orgsid from gate where del=1 and ord in (select ord from power where sort1=42 and sort2=13 and (qx_open=1 or qx_open=3))")
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select ord, name from #gate where orgstype=1"
			rs1.open sql1,conn,1,1
			if rs1.eof then
				Response.write "" & vbcrlf & "                                                      <option>无售后人员</option>" & vbcrlf & "                                                     "
			else
				do until rs1.eof
					Response.write "" & vbcrlf & "                                                      <option value="""
		...
		if ZBRuntime.MC(7000) = False then
			Response.write "" & vbcrlf & "                                      <td width=""10%""><div style=""text-align:right"">&nbsp;</div></td>" & vbcrlf & "                                     <td width=""20%"">&nbsp;</td>" & vbcrlf & "                                       "
'if ZBRuntime.MC(7000) = False then
		end if
		If addtype & "" = "edit" Then
			Response.write "" & vbcrlf & "                                      <td width=""10%""><div style=""text-align:right"">&nbsp;</div></td>" & vbcrlf & "                                     <td width=""20%"">&nbsp;</td>" & vbcrlf & "                                       <td width=""10%""><div style=""text-align:right"">&nbsp;</div></td>" & vbcrlf & "                                     <td width=""20%"">&nbsp;</td>" & vbcrlf & "                                       "
'If addtype & "" = "edit" Then
		end if
		Response.write "" & vbcrlf & "                              </tr>" & vbcrlf & "                           "
	end if
	Response.write getExtended(88,shord)
	Response.write "" & vbcrlf & "                              <tr>" & vbcrlf & "                                    <td width=""10%""><div style=""text-align:right"">售后内容：</div></td>" & vbcrlf & "                                 <td colspan=""5"">" & vbcrlf & "                                          <textarea name=""introts"" style=""width:95%"" rows=""4"" dataType=""Limit"" min=""0"" max=""2000""  msg=""必须在2000个字之内"">"
	'Response.write getExtended(88,shord)
	Response.write HTMLDecode(introts)
	Response.write "</textarea>" & vbcrlf & "                                   </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   <tbody id=""serviceTemplate"">" & vbcrlf & "                      "
	shTemplateStr = getServiceTemplate(id, addtype)
	arr_shtemp = split(shTemplateStr&"", chr(1)&chr(2)&chr(1))
	if ubound(arr_shtemp)>0 then
		Response.write arr_shtemp(1)
	end if
	Response.write "" & vbcrlf & "                      </tbody>" & vbcrlf & "                                <tr>" & vbcrlf & "                                    <td >" & vbcrlf & "                                           <div style=""text-align:right"">备注：</div>" & vbcrlf & "                                        </td>" & vbcrlf & "                                   "
	Response.write arr_shtemp(1)
...
'If Len(exists_Pics) = 0 Then
	...
	Response.write "" & vbcrlf & "                                      <td colspan=""5"" class=""gray"">" & vbcrlf & "                                               <textarea name=""content1"" style=""display:none"" cols=""1"" rows=""1"">"
	if content<>"" then Response.write content
	Response.write "</textarea>" & vbcrlf & "                                           <IFRAME ID=""eWebEditor9"" SRC=""../edit/ewebeditor.asp?id=content1&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""95%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg9""></IFRAME>" & vbcrlf & "                                    </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                              "
	Response.write getExtended1(88,shord)
	Response.write "" & vbcrlf & "                              <tr>" & vbcrlf & "                                    <td height=""37"" colspan=""6"" class=""gray"">" & vbcrlf & "                                             <div align=""center"">" & vbcrlf & "                                                      <input type=""submit"" name=""Submit42"" value=""保存""  class=""page""/>" & vbcrlf & "                                                       <input type=""reset"" value=""重填"" class=""page"" name=""B2"">" & vbcrlf & "                                              </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </form>" & vbcrlf & "         </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class=""page"">" & vbcrlf & "                 <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td height=""580"" ><div align=""center""></div></td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Sub serviceTemplateChange()
		Dim IntroName, IntroDate, intro, i, rs
		Dim date11, date12, date13, date14, date15, date16, date17, date18
		Dim name1, name2, name3, name4, name5, name6, name7, name8
		refreshParent = request("refreshParent")
		companyord = request("companyord")
		id = request("id")
		ord = request("ord")
		main = request("main")
		addtype = request("addtype")
		Response.write getServiceTemplate(id, addtype)
	end sub
	Function getServiceTemplate(id, addtype)
		Dim IntroName, IntroDate, intro, i, rs, retStr
		Dim date11, date12, date13, date14, date15, date16, date17, date18
		Dim name1, name2, name3, name4, name5, name6, name7, name8
		If id&"" = "" Then id = 0
		retStr = GetLeftNodeNum(id) &""& chr(1)&chr(2)&chr(1)
		Set rs = conn.execute("select date11,date12,date13,date14,date15,date16,date17,date18,name1,name2,name3,name4,name5,name6,name7,name8 from tousu where ord="&id&"")
		If rs.eof = False Then
			date11=rs("date11")
			date12=rs("date12")
			date13=rs("date13")
		...
		If addtype & "" <> "edit" Then
			for i = 1 to 8
				execute("IntroName = name" & i & " : IntroDate = date1" & i)
				If  IntroName<>"" and  IntroDate<=date and IntroDate>dateadd("yyyy",-20,date) Then
					execute("IntroName = name" & i & " : IntroDate = date1" & i)
					retStr = retStr &"<tr>"
					retStr = retStr &" <td><div align='right'>"& IntroName &"</div></td>"
					retStr = retStr &"     <td colspan='5' class='gray'><textarea name='intro"& i &"' cols='60' rows='4' style='display:none' dataType='Limit' min='1'  msg='必填'></textarea><IFRAME ID='eWebEditor"& i &"' SRC='../edit/ewebeditor.asp?id=intro"& i &"&style=news' FRAMEBORDER='0' SCROLLING='no' WIDTH='95%' HEIGHT='300' marginwidth='1' marginheight='1' name='wfasdg1'></IFRAME><span class='red'> *</span></td>"
					retStr = retStr &"</tr>"
				end if
			next
		else
			For i = 1 To 8
				execute("IntroName = name" & i & " : Intro = intro" & i & " : cateOrd = cate" & i)
				If  IntroName <> "" and  Intro <> "" Then
					retStr = retStr &"<tr>"
					retStr = retStr &" <td><div align='right'>"& IntroName &"</div></td>"
					retStr = retStr &" <td colspan='5' class='gray ewebeditorImg'>"
					if cateOrd=uid  then
						retStr = retStr &"     <textarea name='intro"& i &"' cols='60' rows='4' style='display:none' dataType='Limit' min='1'  msg='必填'>"& Intro &"</textarea><IFRAME ID='eWebEditor"& i &"' SRC='../edit/ewebeditor.asp?id=intro"& i &"&style=news' FRAMEBORDER='0' SCROLLING='no' WIDTH='95%' HEIGHT='300' marginwidth='1' marginheight='1' name='wfasdg1'></IFRAME>"
					else
						retStr = retStr &Intro
					end if
					retStr = retStr &"<span class='red'> *</span></td>"
					retStr = retStr &"</tr>"
				end if
			next
		end if
		getServiceTemplate = retStr
	end function
	Function GetLeftNodeNum(id)
		Dim rs, sql, i
		Dim date11, date12, date13, date14, date15, date16, date17, date18
		Dim name1, name2, name3, name4, name5, name6, name7, name8
		If id&"" = "" Then id = 0
		i = 0
		set rs=server.CreateObject("adodb.recordset")
		sql="select date11,date12,date13,date14,date15,date16,date17,date18,name1,name2,name3,name4,name5,name6,name7,name8 from tousu where ord="&id&""
		rs.open sql,conn,1,1
		if rs.eof = false then
			date11=rs("date11")
			date12=rs("date12")
		...
		rs.close
		set rs=nothing
		if name1<>"" and  date11<dateadd("yyyy",-20,date) then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name2<>"" and  date12<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name3<>"" and  date13<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name4<>"" and  date14<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name5<>"" and  date15<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name6<>"" and  date16<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name7<>"" and  date17<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		elseif name8<>"" and  date18<dateadd("yyyy",-20,date)  then
'set rs=nothing
			i=i+1
'set rs=nothing
		end if
