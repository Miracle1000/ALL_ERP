...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,23).Value = "图书备注"
...
'if ret2<>"" then
	...
	i=1
	sql="select a.*,b.set_name from O_regbook a left join O_bookset b on a.bk_type=b.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "图书列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			bk_pubtime=rs("bk_pubtime")
			bk_printtime=rs("bk_printtime")
			...
			set rs8=nothing
			xlWorksheet.Cells(1+i,11).Value =bk_printtime
			set rs8=nothing
			...
			set rs8=nothing
			xlWorksheet.Cells(1+i,17).Value =Formatnumber(ml.isNum(zbcdbl(rs("bk_price"))),CommPrice_dot_num,-1)
			set rs8=nothing
			xlWorksheet.Cells(1+i,17).font.Size=10
			set rs8=nothing
			xlWorksheet.Cells(1+i,17).HorizontalAlignment=1
			set rs8=nothing
			xlWorksheet.Cells(1+i,18).Value =rs("bk_allnum")
			set rs8=nothing
			...
			set rs8=nothing
			xlWorksheet.Cells(1+i,23).Value =CutStrX(rs("bk_note"),1)
			set rs8=nothing
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>图书列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!图书列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("图书列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的图书列表</u></strong></font></a></p>"
