...
'Dim s, c, i , rnds, item
	...
	Dim p_2_2,p_2_3
	Sub initPower
		If Len(p_2_1) > 0 Then Exit sub
		q_1_1 =  " " & app.power.GetPowerIntro(1,1)
		q_1_14 = " " & app.power.GetPowerIntro(1,14)
		p_2_1 = " " & app.power.GetPowerIntro(2,1)
		p_2_14 = " " & app.power.GetPowerIntro(2,14)
		p_2_5= " " &  app.power.GetPowerIntro(2,5)
		p_2_2= " " & app.power.GetPowerIntro(2,2)
		p_2_3= " " & app.power.GetPowerIntro(2,3)
	end sub
	Sub onReportInit(rpt,state)
		Dim metype,tord,tdate,ecateid,cateid,stype,scontent,open,t1,t2
		t1=request("t1")
		If Not isdate(t1) Then t1=""
		t2=request("t2")
		...
		rpt.PageButtonAlign = "right"
		Dim datetype,searchtype
		searchtype=request("searchtype")
		If isnumeric(searchtype)=False Or Len(searchtype)=0 Then searchtype=0
		tdate=Split(t1&","&t2, ",")
		scontent=request("scontent")
		rpt.title="联系间隔分析"
		rpt.addfield "","html","","<style><!--input.anybutton{margin:0 5px;}--></style>","",""
		rpt.title="联系间隔分析"
		app.addScriptPath app.virpath & "inc/popup_layer.js"
		rpt.addfield "上次联系时间：","dates","date1","","",tdate
		rpt.addfield "","select","searchtype","姓名" & Chr(1) & "1" & Chr(2) & "拼音码" & Chr(1) & "8" & Chr(2) & "来源" & Chr(1) & "2" & Chr(2) & "电话" & Chr(1) & "3"  & Chr(2) & "手机" & Chr(1) & "4" & Chr(2) & "手机2" & Chr(1) & "5" & Chr(2) & "邮件" & Chr(1) & "6" & Chr(2) & "QQ" & Chr(1) & "7","",searchtype
		rpt.addfield "","text", "keytxt" , "", "", scontent
		Dim rs, intgate1
		Dim arrShow()
		Dim arrName()
		Set rs=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name, (case when show>0 then 1 else 0 end) as show,gate1 from setfields WITH(NOLOCK) order by gate1 asc ")
		While Not rs.eof
			intgate1=rs("gate1")
		...
		rs.close
		set rs = nothing
		datetype = cn.execute("select isnull(MIN(gate2),3) from sort11")(0).value
		rpt.aSearch.AddField2 "人员选择", "gateoption", "gates","","销售人员"& Chr(2) &"添加人员"& Chr(2) &"销售人员和添加人员"
		rpt.aSearch.addField arrName(4), "telcls", "khSort",""
		rpt.aSearch.addField arrName(7), "khqy", "khArea",""
		If arrShow(8) = 1 Then
			rpt.aSearch.addField arrName(8), "checks", "khTrade","select ord as value,sort1 as name from sortonehy WITH(NOLOCK) where gate2='11' order by gate1 desc"
		end if
		If arrShow(17) = 1 Then
			rpt.aSearch.addField arrName(17), "selectys", "lxrName",""
		end if
		rpt.aSearch.addField arrName(1), "selectys", "khName",""
		If arrShow(27) = 1 Then
			rpt.aSearch.addField arrName(27), "selectys", "lxrbm",""
		end if
		If arrShow(28) = 1 Then
			rpt.aSearch.addField arrName(28), "selectys", "lxrjob",""
		end if
		If arrShow(20) = 1 Then
			rpt.aSearch.addField arrName(20), "selectys", "lxrMobile",""
			rpt.aSearch.addField arrName(20)&"2", "selectys", "lxrMobile2",""
		end if
		If arrShow(19) = 1 Then
			rpt.aSearch.addField arrName(19), "selectys", "lxrPhone",""
		end if
		If arrShow(21) = 1 Then
			rpt.aSearch.addField arrName(21), "selectys", "lxrfax",""
		end if
		If arrShow(23) = 1 Then
			rpt.aSearch.addField arrName(23), "selectys", "lxrqq",""
		end if
		If arrShow(24) = 1 Then
			rpt.aSearch.addField arrName(24), "selectys", "lxrmsn",""
		end if
		If arrShow(22) = 1 Then
			rpt.aSearch.addField arrName(22), "selectys", "lxrEmail",""
		end if
		rpt.aSearch.addField "联系人地址", "selectys", "lxrAddress",""
		rpt.aSearch.addField "联系人爱好", "selectys", "lxrjoy",""
		rpt.aSearch.addField "联系人拼音码", "selectys", "lxrpym",""
		rpt.aSearch.addField "联系人备注", "selectys", "lxrIntro",""
		rpt.aSearch.addField "添加日期", "dates", "lxrdate7",""
		rpt.aSearch.addField "dateTypeV", "hidden", "dateTypeV", datetype
		If arrShow(30) = 1 Then
			rpt.aSearch.addField arrName(30), "checks", "lxrSex","select '男' as value, '男' as name union all select '女' as value, '女' as name"
		end if
		If arrShow(32) = 1 Then
			rpt.aSearch.addField arrName(32), "ints", "lxrage",""
		end if
		rpt.onlyadSearchModel =True
		rpt.adSearchAutoHide = True
		rpt.sql="exec erp_report_personintelval "& Info.User &", &pagesize, &pageindex,&sortSql,@date1_0,@date1_1,@searchtype,@keytxt,@dateTypeV, @gates_0, @gates_1, @khSort_0, @khSort_1, @khArea, @khTrade, @lxrName_0, @lxrName_1, @khName_0, @khName_1, @lxrbm_0, @lxrbm_1, @lxrjob_0, @lxrjob_1, @lxrMobile_0, @lxrMobile_1, @lxrMobile2_0, @lxrMobile2_1, @lxrPhone_0, @lxrPhone_1, @lxrfax_0, @lxrfax_1, @lxrqq_0, @lxrqq_1, @lxrmsn_0, @lxrmsn_1, @lxrEmail_0, @lxrEmail_1, @lxrAddress_0, @lxrAddress_1, @lxrjoy_0, @lxrjoy_1, @lxrpym_0, @lxrpym_1, @lxrIntro_0, @lxrIntro_1, @lxrdate7_0, @lxrdate7_1, @lxrSex, @lxrage_0, @lxrage_1"
		rpt.canexcel =  false
		rpt.canprint =  false
		rpt.firstSpace = 0
		If app.power.existsPower(1,12) then
			rpt.buttonsHTML ="<button class='anybutton'  onclick=""javascript:window.open('../person/setdatetype.asp','newwins','width=' + 800 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=200,top=1')"">间隔时间定义</button>"
'If app.power.existsPower(1,12) then
		end if
		rpt.headHTML = "<div id='zdiv' style='background-color:#e1ecfe;line-height:25px;'><form method='post' style='display:inline' id='personIntelval'>" & showintervalbtn(datetype) & "</form></div><style>th.lvwheader{height:32px} td.lvw_cell{height:32px}</style>"
'If app.power.existsPower(1,12) then
	end sub
	Sub OnReportBottom
		Response.write "" & vbcrlf & "<div style='position:relative;top:-34px;width:200px'>" & vbcrlf & "        <table style='table-layout:auto;width:auto'>" & vbcrlf & "    <tr>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "         <td><input type='checkbox' id='ckall' onclick='selectall(this);'> </td>" & vbcrlf & "         <td><label for='ckall'>全选</label></td>" & vbcrlf & "           <td width='20'>&nbsp;</td>" & vbcrlf & "              <td>" & vbcrlf & "                            <input type='hidden' name='selectid' id='selectid'>" & vbcrlf & "                     "
		If  app.power.existsPower(2,3) then
			Response.write "" & vbcrlf & "                     <button class='oldbutton3' onclick=""if(confirm('确认批量删除？')){dosubmit();}"">批量删除</button>" & vbcrlf & "                 "
		end if
		Response.write "" & vbcrlf & "             </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "</div>" & vbcrlf & "<script src= ""../Script/pn_telday.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """ ></script>" & vbcrlf & ""
	end sub
	Function showintervalbtn(datetype)
		Dim rs, html
		Set rs=cn.execute("select c.sort1 as title,c.gate2 as datetype from sort11 c order by c.gate2 asc")
		While rs.eof = False
			html = html & "<input name='dfbtns' type='button' value='" & rs("title").value & "' tag=" & rs("datetype").value
			If datetype & "" =  rs("datetype").value & "" Then
				html = html & " style='font-weight:bold;' "
'If datetype & "" =  rs("datetype").value & "" Then
			else
				html = html & " style='' "
			end if
			html = html & " class='anybutton' onclick='dfbtnsClick(this)'>"
			rs.movenext
		wend
		rs.close
		showintervalbtn="<span style='padding-right:50px;'>" & html &"</span>"
		html = html & " class='anybutton' onclick='dfbtnsClick(this)'>"
	end function
	Sub app_oncreate(lvw)
		...
		col.Formattext = "code:needcheckbox(@cells[1],@cells[11],@value)"
		col.width=30
		Set col = lvw.headers(2)
		col.width=180
		col.setLink2 "姓名","选择", "cateid",  2, -10,"share"
		col.width=180
		Set col = lvw.headers(3)
		col.width=230
		col.dbtype = "str"
		col.Formula = "GetFormaterTel(@cells[3],@cells[10],@cells[11])"
		Set col = lvw.headers(4)
		col.width=230
		col.dbtype = "str"
		col.setLink2 "来源","khord", "cateid",  1, -9,"share"
		col.dbtype = "str"
		Set col = lvw.headers(5)
		col.width=195
		col.dbtype = "str"
		col.Formula = "Getreplystr(@cells[1],@cells[11],@value,@me)"
		...
		col.Formula = "GetOpstr(@cells[1],@cells[11])"
		col.width=160
		lvw.headers("khord").display = "none"
		lvw.headers("cateid").display = "none"
		lvw.headers("inx").display = "none"
		lvw.headers("share").display = "none"
		lvw.cansort = True
		lvw.headers("电话").cansort = False
		lvw.headers("洽谈进展").cansort = false
	end sub
	Function GetOpstr(pord,cateid)
		Dim v
		If Len(cateid&"")=0 or cateid&""="0" Then cateid="-1"
'Dim v
		If isEmpty(p_2_14) Then p_2_14 = app.power.GetPowerIntro(2,14)
		If isEmpty(p_2_2) Then p_2_2 = app.power.GetPowerIntro(2,2)
		If isEmpty(p_2_3) Then p_2_3 = app.power.GetPowerIntro(2,3)
		v = ""
		If Len(LTrim(p_2_14))=0 Or InStr("," & LTrim(p_2_14) & ",", "," & cateid & ",") > 0 Then
			v=v +"<button class=""oldbutton2"" onClick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(pord) & "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>详情</button>&nbsp;"
'If Len(LTrim(p_2_14))=0 Or InStr("," & LTrim(p_2_14) & ",", "," & cateid & ",") > 0 Then
		end if
		If Len(LTrim(p_2_2))=0 Or InStr("," & LTrim(p_2_2) & ",", "," & cateid & ",") > 0 Then
			v=v +"<button class=""oldbutton2"" onClick=""javascript:window.open('correct.asp?ord=" & app.base64.pwurl(pord) & "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>修改</button>&nbsp;"
'If Len(LTrim(p_2_2))=0 Or InStr("," & LTrim(p_2_2) & ",", "," & cateid & ",") > 0 Then
		end if
		If Len(LTrim(p_2_3))=0 Or InStr("," & LTrim(p_2_3) & ",", "," & cateid & ",") > 0 Then
			v=v +"<button class=""oldbutton2"" onClick=""if(confirm('确认删除？')){ajax.regEvent('persondelete');ajax.addParam('pord','"&pord&"');ajax.exec();}"" />删除</button>"
'If Len(LTrim(p_2_3))=0 Or InStr("," & LTrim(p_2_3) & ",", "," & cateid & ",") > 0 Then
		end if
		GetOpstr=v
	end function
	Sub App_persondelete()
		Dim pord,sqltext
		Dim cateid,rs,v
		pord=app.getText("pord")
		v=""
		If isEmpty(p_2_3) Then p_2_3 = app.power.GetPowerIntro(2,3)
		Set rs = cn.execute("select name,cateid,ord from person where ord in(" & pord &")" )
		Do While Not rs.eof
			cateid =  rs.fields(1).value
			If Len(cateid&"")=0 or cateid&""="0" Then cateid="-1"
			cateid =  rs.fields(1).value
			If Len(LTrim(p_2_3))=0 Or InStr("," & LTrim(p_2_3) & ",", "," & cateid & ",") > 0 Then
				If v="" Then
		...
		Set rs =  Nothing
		If Len(v&"")=0 Then v=0
		sqltext="Update person set del=2 ,delcate="&session("personzbintel2007")&",deldate='"&date&"' where ord in(" & v & ")"
		cn.Execute(sqltext)
		Response.write "lvw_refresh('mlistvw');"
	end sub
	Function GetFormaterTel(mobileStr,khord,cateid)
		Dim tempStr,v,i,mobileNum,tofrom
		Dim rs,sql,open_67_13,open_67_19
		set rs=server.CreateObject("adodb.recordset")
		sql="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=13"
		rs.open sql,conn,1,1
		if rs.eof then
			open_67_13=0
		else
			open_67_13=rs("qx_open")
		end if
		rs.close
		set rs=Nothing
		set rs=server.CreateObject("adodb.recordset")
		sql="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=19"
		rs.open sql,conn,1,1
		if rs.eof then
			open_67_19=0
		else
			open_67_19=rs("qx_open")
		end if
		rs.close
		set rs=nothing
		tempStr = ""
		...
		For i = 0 To ubound(v)
			If Len(v(i))> 0 Then
				If tempStr = "" Then
					tempStr = GetPhoneNumber(v(i), cateid)
				else
					tempStr = tempStr &"<br>"& GetPhoneNumber(v(i), cateid)
				end if
				if ZBRuntime.MC(32000) Then
					tempStr = tempStr & " <img src='../images/"
					If isMobile(v(i)) then
						tempStr = tempStr & "sj.gif"
					else
						tempStr = tempStr & "15.gif"
					end if
					tempStr = tempStr & "' onClick=""getCall('../china/bh.asp?ord=-"&khord&"&ordtype=1&phone="&v(i)&"')"" style='cursor:hand' border='0' align='absbottom'>"
					tempStr = tempStr & "15.gif"
				end if
				If ZBRuntime.MC(100000) And open_67_13=1 and open_67_19<>1 and isMobile(v(i)) Then
					If IsPhonePower(cateid) Then
						mobileNum = trim(v(i))
						tofrom = "normal"
					else
						mobileNum = HexEncode(trim(v(i)))
						tofrom = "encrypt"
					end if
					tempStr = tempStr & "&nbsp;<img src='../images/message.gif'  onClick=""javascript:window.open('../message/topadd.asp?from="&tofrom&"&phone="&mobileNum&"','newsSendSMS','width=' + 900 + ',height=' + 800 + ',fullscreen =no,scrollbars=0,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=200,top=100')""  style='cursor:hand' border='0' alt='发送短信' align='absbottom'>"
				end if
			end if
		next
		GetFormaterTel = tempStr
	end function
	Function getreplystr(pord,khcateid,gjIntro, lvw)
		Dim cateid,tempStr,v
		tempStr = ""
		If isEmpty(p_2_5) Then
			p_2_5 = app.power.GetPowerIntro(2,5)
		end if
		If lTrim(p_2_5) = "0" Then Exit function
		If Len(khcateid&"")=0 or khcateid&""="0" Then khcateid="-1"
'If lTrim(p_2_5) = "0" Then Exit function
		If Len(LTrim(p_2_5))=0 Or InStr("," & LTrim(p_2_5) & ",", "," & khcateid & ",") > 0 Then
			tempStr = gjIntro
			If lvw.excelmode = False Then
				tempStr =  tempStr & " <img class=""resetElementHidden"" src=""../images/jiantou.gif""><img class=""resetElementShow"" style=""display:none;"" src=""../skin/default/images/MoZihometop/content/lvw_addrow_btn.png""><a href=""#"" onClick=""javascript:window.open('reply.asp?ord=" & app.base64.pwurl(pord) & "','plancor5','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;""><font class=""blue2"">添加</font></a>"
'If lvw.excelmode = False Then
			end if
		else
			tempStr = ""
	...
	Function get1person(v,v1)
		If app.power.existsPower(2,14) Then
			get1person="<a href=''>"&v1&"</a>"
		else
			getpersons=v1
		end if
	end function
	Function needcheckbox(ord,cateid,v)
		If Len(cateid&"")=0 or cateid&""="0" Then cateid="-1"
'Function needcheckbox(ord,cateid,v)
		If isEmpty(p_2_3) Then
			p_2_3 = app.power.GetPowerIntro(2,3)
		end if
		If Len(LTrim(p_2_3))=0 Or InStr("," & LTrim(p_2_3) & ",", "," & cateid & ",") > 0 Then
			needcheckbox = "<input type='checkbox' name='ord' value='" & v & "'>"
		end if
	end function
