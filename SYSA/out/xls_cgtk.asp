...
xlApplication.ActiveSheet.Columns(2).ColumnWidth=25
...
dim headercols : headercols = split("供应商名称,关联单据,退款计划编号,金额,应退日期,退款日期,退款帐号,退款方式,退款状态,备注,退货人员",",")
if(hasCG or hasWW) then
headercols = split("供应商名称,关联单据,退货来源,退款计划编号,金额,应退日期,退款日期,退款帐号,退款方式,退款状态,备注,退货人员",",")
xlApplication.ActiveSheet.Columns(1).ColumnWidth=20
xlApplication.ActiveSheet.Columns(1).HorizontalAlignment=3
xlApplication.ActiveSheet.Columns(2).ColumnWidth=25
xlApplication.ActiveSheet.Columns(2).HorizontalAlignment=3
...
for i = 1 to ubound(headercols) + 1
xlApplication.ActiveSheet.Columns(11).HorizontalAlignment=3
xlWorksheet.Cells(1,i).Value = headercols(i-1)
xlApplication.ActiveSheet.Columns(11).HorizontalAlignment=3
...
Response.Flush
dim w,a ,b,c,d,e,f,sort1,sort2,order,m1,m2,thord,caigouid
m1=request("ret")
m2=request("ret2")
A=request("A")
A2=request("A2")
...
list=" and cateid in ("&intro_76_10&") "
else
list=" and cateid=0 "
end if
Str_Result = " where del=1 "&list&" "
...
set rs=server.CreateObject("adodb.recordset")
sql="select a.*,b.bankname from v_payout3 a " & _
"left join (" & vbcrlf &_
...
C1=rs.recordcount
xlApplication.Init "采购退款列表_"&session("name2006chen")&".xls",C1
dim i
i=1
...
if rs("company")<>"" then
set rs1=server.CreateObject("adodb.recordset")
sql1="select ord,name,cateid, share, Sort3 from tel where  ord="&rs("company")&" and del=1"
rs1.open sql1,conn,1,1
if rs1.eof then
if open_26_1 =  0 then
companyname=""
...
if (rs1("Sort3") & "") = "2" then
if open_26_1 = 3 or  (open_26_1=1 and instr( "," & replace(intro_26_1," ","") & ",", "," & rs1("cateid") & ",")>0  )  then
companyname=rs1("name")
else
companyname = ""
end if
else
if open_1_1 = 3 or  (open_1_1=1 and instr( "," & replace(intro_1_1," ","") & ",", "," & rs1("cateid") & ",")>0  )  or rs1("Share")="1" or  instr( "," & replace(rs1("Share")&""," ","") & ",", "," & session("personzbintel2007") & ",")>0    then
companyname=rs1("name")
else
companyname = ""
end if
...
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,7).Value = date2
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,7).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,8).Value = bankname
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,8).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,9).Value = pay
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,9).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,10).Value = complete
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,10).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,10).HorizontalAlignment=3
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,11).Value = intro
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,11).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,11).HorizontalAlignment=1
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,12).Value = cateidname
'if(hasCG or hasWW) then
...
xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,3).Value = bh
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,3).font.Size=10
'if(hasCG or hasWW) then
xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
...
rs.close
set rs=Nothing
Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>采购退款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!采购退款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
set rs=Nothing
Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
Set fs = CreateObject("Scripting.FileSystemObject")
tfile=Server.MapPath("采购退款列表_"&session("name2006chen")&".xls")
...
Response.write """><font class=""red""><strong><u>下载导出的采购退款列表</u></strong></font></a></p>"
