...
'dim sqlstr
	...
	if m1="" and Ismode=""   then
		m1=cdate(year(date)&"-"&month(date)&"-1")
'if m1="" and Ismode=""   then
	end if
	if m2=""  and Ismode=""  then
		m2=dateadd("m",1,m1)-1
'if m2=""  and Ismode=""  then
	end if
	...
	if A2<>"0" then
		str_Result11=str_Result11+" and b.bz="&A2&" "
'if A2<>"0" then
	end if
	if C<>"" then
		if B="khmc" then
			str_Result11=str_Result11+" and b.company in (select ord from tel where name like '%"& C &"%')"
'if B="khmc" then
		elseif B="khid" then
			str_Result11=str_Result11+" and b.company in (select ord from tel where khid like '%"& C &"%')"
'elseif B="khid" then
		elseif B="thdmc" then
			str_Result11=str_Result11+" and b.company in (select company from contractth where title like '%"& C &"%')"
'elseif B="thdmc" then
		elseif B="thid" then
			str_Result11=str_Result11+" and b.company in (select company from contractth where thid like '%"& C &"%')"
'elseif B="thid" then
		elseif B="thry" then
			str_Result11=str_Result11+" and b.company in (select company from contractth where cateid in (select ord from gate where name like '%"& C &"%'))"
'elseif B="thry" then
		end if
	end if
	...
	if px=1 then
		px_Result="order by name desc,b.company desc"
	elseif px=2 then
		px_Result="order by name asc,b.company asc"
	elseif px=3 then
		px_Result="order by money1 desc,b.company desc"
	elseif px=4 then
		px_Result="order by money1 asc,b.company asc"
	elseif px=5 then
		px_Result="order by money2 desc,b.company desc"
	elseif px=6 then
		px_Result="order by money2 asc,b.company asc"
	elseif px=7 then
		px_Result="order by money3 desc,b.company desc"
	elseif px=8 then
		px_Result="order by money3 asc,b.company asc"
	elseif px=9 then
		px_Result="order by cateidname desc,b.company desc"
	elseif px=10 then
		px_Result="order by cateidname asc,b.company asc"
	end if
	...
	if open_9_1=3 then
		str_Result11=str_Result11&""
	elseif open_9_1=1 then
		str_Result11=str_Result11&" and a.cateid in ("&intro_9_1&")"
	else
		str_Result11=str_Result11&" and a.cateid=0"
	end if
	if m1&""<>"" then
		str_Result11=str_Result11&" and DATEDIFF(DD,'"&m1&"', a.date1)>=0  "
	end if
	if m2&""<>"" then
		str_Result11=str_Result11&" and DATEDIFF(DD,a.date1,'"&m2&"')>=0  "
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "" & vbcrlf & "function callServer2() {" & vbcrlf & "  var url = ""liebiao_tj.asp?timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "  updatePage2();" & vbcrlf & "  };" & vbcrlf & "  xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "function updatePage2() {" & vbcrlf & "var test7=""ht1""" & vbcrlf & "  if (xmlHttp.readyState < 4) {" & vbcrlf & "  ht1.innerHTML=""loading..."";" & vbcrlf & "  }" & vbcrlf & "  if (xmlHttp.readyState == 4) {" & vbcrlf & "    var response = xmlHttp.responseText;" & vbcrlf & "       ht1.innerHTML=response;" & vbcrlf & " xmlHttp.abort();" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "function go(loc) {" & vbcrlf & "window.location.href= loc;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "   <form action=""planall_hk.asp?px="
	Response.write px
	Response.write "&page_count="
	Response.write page_count
	Response.write """ method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "                <tr>" & vbcrlf & "      <td class=""place"">" & vbcrlf & "    退款汇总表</td>" & vbcrlf & "      <td>&nbsp;<a href=""#"" onClick=""Myopen_px(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "      <td align=""right""><select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "      <option>-请选择-</option>" & vbcrlf & "      <option value=""page_count=10"" "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "    </select>" & vbcrlf & "    <input name=""Ismode"" type=""hidden"" size=""6"" value=""1"">" & vbcrlf & "      </td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "  </table> " & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class='top_btns resetHeadBg resetBorderColor'   height=""50"" align=""center"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;background:rgb(244, 250, 254)"">"
	'Response.write "selected"
	...
	Response.write "'>"
	Response.write "" & vbcrlf & "     <select name=""A2"">" & vbcrlf & "        <option value=""0"" >币种</option>" & vbcrlf & ""
	...
	Response.write "</option>" & vbcrlf & "  <option value=""thdmc"" "
	if B="thdmc" then
		Response.write "selected"
	end if
	...
	Response.write ">退货人员</option>" & vbcrlf & "</select>" & vbcrlf & "<input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/><input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/><span id=s1>"
	if open_9_10=1 or open_9_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""EXCEL导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_tkhz.asp'})}"" class=""anybutton""/>"
	end if
	...
	Response.write "</span></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "</form>" & vbcrlf & "      <tr class=""top"">" & vbcrlf & "    <td height=""27""><div align=""center"">"
	Response.write khname
	Response.write "</div></td>" & vbcrlf & "    <td width=""15%""><div align=""center"">应退总额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">实退总额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">剩余金额</div></td>" & vbcrlf & "     <td width=""15%""><div align=""center"">退货人员</div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	dim summoney1,summoney2,summoney3,money1_all,money2_all,money3_all
	summoney1=0
	summoney2=0
	summoney3=0
	money1_all=0
	...
	money3_all=0
	n=0
	sql="select b.company,b.bz,"&_
	"(select name from tel WITH(NOLOCK) where sort3=1 and del=1 and ord=b.company) as name,"&_
	"(select intro from sortbz WITH(NOLOCK) where id=b.bz) as sortbz,"&_
	"sum(isnull(a.money1,0)) as money1,"&_
	"sum((case when a.complete = 2 then isnull(a.money1,0) else 0 end)) as money2,"&_
	"sum((case when a.complete = 1 then isnull(a.money1,0) else 0 end)) as money3,"&_
	"(select name from gate WITH(NOLOCK) where del=1 and ord in (select max(cateid) from contractth WITH(NOLOCK) where company=b.company and bz=b.bz)) as cateidname "&_
	" from payout2 as a WITH(NOLOCK) "&_
	"left join contractth as b WITH(NOLOCK) on a.contractth=b.ord "&_
	"where a.del=1 "&Str_Result11&"  "&_
	"group by b.company,b.bz "&px_Result&""
	set rs1=server.CreateObject("adodb.recordset")
	rs1.open sql,conn,1,1
	if not rs1.eof then
		do while not rs1.eof
			money1=zbcdbl(rs1("money1"))
			if rs1("money1")="" or IsNull(zbcdbl(rs1("money1"))) then
				money1=0
			else
				money1=cdbl(rs1("money1"))
			end if
			money2=zbcdbl(rs1("money2"))
			if rs1("money2")="" or IsNull(zbcdbl(rs1("money2"))) then
				money2=0
			else
				money2=cdbl(rs1("money2"))
			end if
			money3=zbcdbl(rs1("money3"))
			if rs1("money3")="" or IsNull(zbcdbl(rs1("money3"))) then
				money3=0
			else
				money3=cdbl(rs1("money3"))
			end if
			money1_all=money1_all+money1
			money3=cdbl(rs1("money3"))
			money2_all=money2_all+money2
			money3=cdbl(rs1("money3"))
			money3_all=money3_all+money3
			money3=cdbl(rs1("money3"))
			rs1.movenext
		loop
	end if
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			sql7="select ord,name,isnull(cateid,0) as cateid, ISNULL(share,'-222') share ,sort3, del from tel where ord="&company&" and del=1"
			set rs7=server.CreateObject("adodb.recordset")
			...
			money3=cdbl(rs("money3"))
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td>"
			If sort3="2" Then
				If del=1 Then
			...
			Response.write "" & vbcrlf & "       </td> " & vbcrlf & "      <td align=""right"">"
			Response.write rs("sortbz")
		...
		loop
		Response.write "     " & vbcrlf & "    <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "         <td class=""name""><div align=""right"">本页合计：</div></td>" & vbcrlf & "   <td class=""name""><div align=""right"">"
		Response.write Formatnumber(summoney1,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write Formatnumber(summoney2,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write Formatnumber(summoney3,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write "</div></td>" & vbcrlf & "    <td class=""red""><div align=""right"">&nbsp;</div></td>" & vbcrlf & "        </tr>" & vbcrlf & " <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td class=""name""><div align=""right"">所有合计：</div></td>" & vbcrlf & "     <td class=""name""><div align=""right"">"
		...
		Response.write "</div></td>" & vbcrlf & "    <td class=""name""><div align=""right"">"
		Response.write "</div></td>" & vbcrlf & "    <td class=""red""><div align=""right"">&nbsp;</div></td>" & vbcrlf & "        </tr> " & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "    </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "  <tr>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "         <td>&nbsp;</td>" & vbcrlf & "       </form> " & vbcrlf & "  <td width=""79%""><div align=""right"">" & vbcrlf & "   <span class=""black"">"
		...
		Response.write "条信息/页</span>&nbsp;&nbsp;" & vbcrlf & "  <input name=""currpage""  id=""currpage"" type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "    <input type=""button"" name=""Submit422"" value=""跳转""  onclick=""gotourl('currPage='+document.getElementById('currpage').value);"" class=""anybutton2""/>" & vbcrlf & "    "
		if currpage=1 then
	...
	action1="退款汇总表"
	call close_list(1)
	Response.write "   " & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen_px(divID){" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "              divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=0;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:400;display:none;"">" & vbcrlf & "<table width=""150"" height="""
	if A2<>"0" then
		Response.write "300"
	else
		Response.write "150"
	end if
	Response.write """  border=""0"" cellpadding=""2"" cellspacing=""2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按"
	Response.write khname
	...
	Response.write khname
	Response.write "排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & ""
	if A2<>"0" then
		Response.write "" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按应退总额排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按应退总额排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按实退总额排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按实退总额排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按剩余金额排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按剩余金额排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=9')""><font color=""#2F496E"">按退货人员排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"">&nbsp;<a href=""###"" onclick=""gotourl('px=10')""><font color=""#2F496E"">按退货人员排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "</body>" & vbcrlf & "</html>"
