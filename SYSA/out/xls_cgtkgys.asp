...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(2).NumberFormatLocal = "#,##0." & String(num1_dot,"0") & "_ "
			...
			xlApplication.ActiveSheet.Columns(4).NumberFormatLocal = "#,##0_ "
...
'elseif B="khid" then
				elseif B="thdmc" then
					str_Result11=str_Result11+" and b.company in (select company from caigouth where title like '%"& C &"%')"
'elseif B="thdmc" then
				elseif B="thid" then
					str_Result11=str_Result11+" and b.company in (select company from caigouth where cgthid like '%"& C &"%')"
'elseif B="thid" then
				elseif B="thry" then
					str_Result11=str_Result11+" and b.company in (select company from caigouth where cateid in (select ord from gate where name like '%"& C &"%'))"
'elseif B="thry" then
				end if
			end if
			...
			elseif px=3 then
				px_Result="order by num1 desc,b.company desc"
			elseif px=4 then
				px_Result="order by num1 asc,b.company asc"
			elseif px=5 then
				px_Result="order by money1 desc,b.company desc"
			elseif px=6 then
				px_Result="order by money1 asc,b.company asc"
			elseif px=7 then
				px_Result="order by num2 desc,b.company desc"
			elseif px=8 then
				px_Result="order by num2 asc,b.company asc"
			elseif px=9 then
				px_Result="order by money2 desc,b.company desc"
			elseif px=10 then
				px_Result="order by money2 asc,b.company asc"
			elseif px=11 then
				px_Result="order by num3 desc,b.company desc"
			elseif px=12 then
				px_Result="order by num3 asc,b.company asc"
			end if
			sumnum1=0
			summoney1=0
			sumnum2=0
			...
			"(select isnull(sum(num1),0) as num1 from caigouthlist where company=b.company and del=1) as num1,"&_
			"(select isnull(sum(money1),0) as money1 from caigouth where company=b.company and bz=b.bz and del=1) as money1,"&_
			"(select isnull(count(*),0) as num2 from caigouth where company=b.company and bz=b.bz and del=1) as num2,"&_
			"(select isnull(sum(money1),0) as money2 from caigou where company=b.company and bz=b.bz and del=1) as money2,"&_
			"(select isnull(count(*),0) as num3 from caigou where company=b.company and bz=b.bz and del=1) as num3"&_
			" from payout3 as a left join caigouth as b on a.caigouth=b.ord where a.del=1 "&Str_Result11&" and a.date1>='"&m1&"' and a.date1<='"&m2&"' group by b.company,b.bz "&px_Result&""
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "供应商退款排行_"&session("name2006chen")&".xls",C1
			dim i
			i=1
...
'if cateid="" or cateid=0 then
					...
					if rs("num1")="" or IsNull(zbcdbl(rs("num1"))) then
						num1=0
					else
						num1=zbcdbl(rs("num1"))
					end if
					if rs("num2")="" or IsNull(zbcdbl(rs("num2"))) then
						num2=0
					else
						num2=zbcdbl(rs("num2"))
					end if
					if rs("num3")="" or IsNull(zbcdbl(rs("num3"))) then
						num3=0
					else
						num3=zbcdbl(rs("num3"))
					end if
					...
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,2).Value = num1
					num3=zbcdbl(rs("num3"))
					...
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,4).Value = num2
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,4).font.Size=10
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,5).Value = Formatnumber(money2,num_dot_xs,-1)
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,5).font.Size=10
					num3=zbcdbl(rs("num3"))
					xlWorksheet.Cells(1+i,6).Value = num3
					num3=zbcdbl(rs("num3"))
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>供应商退款排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!供应商退款排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("供应商退款排行_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的供应商退款排行</u></strong></font></a></p>"
