...
'Public Function AddFormValue(name,  value)
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		rpt.title = "组装清单列表"
		rpt.canexcel = app.power.existsPower(56,10)
		rpt.canprint = app.power.existsPower(56,7)
		rpt.BillListUIModel = True
		rpt.pagesize = 10
		Dim html, showtype, checked, h1, val1, i
		showtype = app.getint("showtype")
		h1 = Split("首页,待审批,通过,未通过",",")
		html = ""
		For i = 0 To ubound(h1)
			...
			html = html & "<input type='radio' " & checked & " value='" & i & "' name='showtype' onclick='window.location.href=""?showtype=" & i & """' />" & h1(i) & "&nbsp;"
		next
		rpt.beforeBtnsHTML = html
		rpt.addfield2 "","select", "datetype" , "作废日期" & Chr(1) & "1", "", "0", "添加日期=0"
		rpt.addfield "自：", "dates", "date1", "", "", Split(",", ",")
		Dim s1, v1, opts1
		s1 = Split("父件编号,父件型号,子件名称,子件编号,子件型号",",")
		v1 = Split("1,2,3,4,5",",")
		opts1 = ""
		For i = 0 To ubound(s1)
			...
			opts1 = opts1 & s1(i) & Chr(1) & v1(i)
		next
		rpt.addfield2 "","select", "protype" , opts1, "", "", "父件名称=0"
		rpt.addfield "", "text", "protext", "", "", ""
		s1 = Split("组装清单编号,版本号,添加人",",")
		v1 = Split("1,2,3,",",")
		opts1 = ""
		For i = 0 To ubound(s1)
			If Len(opts1) > 0 Then
			...
			opts1 = opts1 & s1(i) & Chr(1) & v1(i)
		next
		rpt.addfield2 "","select", "keytype" , opts1, "", "", "组装清单名称=0"
		rpt.addfield "", "text", "keytext", "", "", ""
		rpt.sql = "exec BOMStructure_Info " & Info.User & ",'" & showtype & "',@datetype,@date1_0,@date1_1,@protype,@protext,@keytype,@keytext,&pagesize,&pageindex,&excelmode,&sortsql"
		dim bats
		if app.power.existsPower(56,3) then
			bats = "批量删除"
		end if
		rpt.setBatHandle "ord", bats
	end sub
	Sub app_oncreate(byval lvw)
		lvw.headers("ord").title = "选择"
		lvw.headers("ord").width = 5
		lvw.headers("ord").display = "none"
		lvw.headers("title").title = "组装清单名称"
		lvw.headers("title").formattext = "code: TitleCell(@value,rs)"
		lvw.headers("title").width = 150
		lvw.headers("title").dbtype        = "str"
		lvw.headers("BBH").title   = "版本号"
		lvw.headers("BBH").width   = 150
		lvw.headers("BH").title            = "组装清单编号"
		lvw.headers("BH").width            = 150
		lvw.headers("sp1").title   = "状态"
		lvw.headers("sp1").width   = 70
		lvw.headers("addDate").title = "添加日期"
		lvw.headers("addDate").width = 70
		lvw.headers("sxDate").title = "生效日期"
		lvw.headers("sxDate").width        = 70
		lvw.headers("zfDate").title = "作废日期"
		lvw.headers("zfDate").width        = 70
		lvw.headers("addCate").title = "添加人"
		lvw.headers("addCate").width = 70
		lvw.headers("id").title            = "操作"
		lvw.headers("id").width            = 150
		lvw.headers("id").formattext = "code: HandleCell(@value,rs)"
		lvw.headers("sp").display  = "none"
		lvw.headers("cateid_sp").display = "none"
		lvw.headers("cateid").display = "none"
		lvw.headers("tid").display = "none"
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		.dbtype = "str"
		End With
		With lvw.headers("sp1")
		.title      = "状态"
		.dbtype = "str"
		.excelAlign = "center"
		.width      = "80"
		End With
		With lvw.headers("addDate")
		.title = "添加日期"
		.Width = "80"
		End With
		With lvw.headers("sxDate")
		.title = "生效日期"
		.Width = "80"
		End With
		With lvw.headers("zfDate")
		.title = "作废日期"
		.Width = "80"
		End With
		With lvw.headers("addCate")
		.title = "添加人"
		.Width = "60"
		.dbtype = "str"
		.excelAlign = "center"
		...
		lvw.headers("id").title           = "操作"
		Dim rs, i
		Set rs = cn.execute("select title,name,set_open,dc from zdy where sort1 = 8040")
		If rs.eof  Then
			For i = 1 To 6
				lvw.headers("zdy" & i).display = "none"
			next
		end if
		While rs.eof = False
		...
		rs.close
		set rs = nothing
		Set rs = cn.execute("select title,name,set_open,dc from zdy where sort1 = 21")
		While rs.eof = False
			With lvw.headers("p" & rs("name"))
			.title      = rs("title")
			.dbtype = "str"
			If rs("set_open") <> "1" or rs("dc") <> "1" Then
				.display = "none"
		...
		rs.close
		set rs = nothing
		With lvw.headers("nodes")
		.title      = "节点"
		.width      = "80"
		.dbtype = "str"
		.excelAlign = "center"
		End With
		With lvw.headers("sProType")
		.title      = "类型"
		.width      = "50"
		.dbtype = "str"
		.excelAlign = "center"
		...
		.excelAlign = "center"
		End With
		With lvw.headers("pName")
		.title      = "产品名称"
		.dbtype = "str"
		.excelAlign = "left"
		End With
		lvw.headers("pBH").title  = "产品编号"
		lvw.headers("pXH").title  = "产品型号"
		With lvw.headers("pUnit")
		.title      = "单位"
		.width      = "50"
		.dbtype = "str"
		.excelAlign = "center"
		...
		End With
		With lvw.headers("pNum")
		.title      = "数量"
		.dbtype = "number"
		.ignoreNonnumeric = True
		.width      = "80"
		End With
		With lvw.headers("canCK")
		.title      = "产品属性"
		.width      = "60"
		.align      = "center"
		.dbtype = "str"
		.excelAlign = "center"
		End With
		With lvw.headers("pType")
		.title      = "结构类型"
		.dbtype = "str"
		End With
		With lvw.headers("pCode")
		.title      = "结构编码"
		.dbtype = "str"
		End With
		lvw.headers("sp").display = "none"
		...
		lvw.headers("cateid").display = "none"
		lvw.headers("tid").display = "none"
		lvw.headers("isinfo").display = "none"
		lvw.headers("isparent").display = "none"
	end sub
	Function HandleCell(byval id, ByVal rs)
		Dim buttonSP, buttonContent, buttonEdit, buttonDel, Creator
		Creator = rs("cateid")
		if app.power.existsPowerIntro(56, 16, Creator) And rs("cateid_sp") = Info.User then
			buttonSP = "<button class=""oldbutton2"" onclick=""javascript:window.open('../inc/CommSPSet.asp?ord=" & app.base64.pwurl(id) & "&sort1=8040&lvw=mlistvw','new6win','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">审批</button>&nbsp;"
'if app.power.existsPowerIntro(56, 16, Creator) And rs("cateid_sp") = Info.User then
		end if
		if app.power.existsPowerIntro(56, 14, Creator) then
			buttonContent = "<button class='oldbutton2' onclick=""window.open('add.asp?add=1&edit=1&bomord=" & app.base64.pwurl(id) & "','bomadd_view001','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">详情</button>&nbsp;"
'if app.power.existsPowerIntro(56, 14, Creator) then
		end if
		if app.power.existsPowerIntro(56, 2, Creator) then
			buttonEdit = "<button class='oldbutton2' onclick=""window.open('add.asp?add=1&bomord=" & app.base64.pwurl(id) & "','bomadd_edit001','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">修改</button>&nbsp;"
'if app.power.existsPowerIntro(56, 2, Creator) then
		end if
		if app.power.existsPowerIntro(56, 3, Creator) then
			buttonDel = "<button class='oldbutton2' onclick=""ListAction.DoDel('" & id & "')"">删除</button>"
		end if
		HandleCell =  buttonSP & buttonContent & buttonEdit & buttonDel
	end function
	Function TitleCell(byval v, ByVal rs)
		if app.power.existsPowerIntro(56, 14, rs("cateid")) Then
			TitleCell = "<a href='javascript:void(0)' onclick=""window.open('add.asp?add=1&edit=1&bomord=" & app.base64.pwurl(rs("ord")) & "','bomadd_view001','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">" & v & "</a>"
'if app.power.existsPowerIntro(56, 14, rs("cateid")) Then
		else
			TitleCell = v
		end if
	...
	sub lvw_onCreateCheckBox(lvw, rs, ckhtml)
		if not app.power.existsPowerIntro(56,3, rs("cateid").value) then
			ckhtml = "&nbsp;"
		end if
	end sub
	...
	Sub doDelete(values, batResult)
		cn.execute "update BOM_Structure_list set del = 2,deldate = getDate(),delcate = " & Info.User & " where BOMord in (" & values & ");update BOM_Structure_Info set del = 2,deldate = getDate(),delcate = " & Info.User & " where ord in (" & values & ")"
	end sub
	Sub App_DoDel
		dim id
		id = request.Form("id")
