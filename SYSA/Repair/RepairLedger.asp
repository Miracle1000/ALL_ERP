...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		...
		app.addDefaultCss
		Dim rs,defDate,StatusList,jjList,ProcessList,sTypeList, khord, slord
		rpt.title = "维修库存台账"
		...
		If app.power.existsPower(45,7) Then
			rpt.canPrint = True
		else
			rpt.canPrint = False
		end if
		...
		sTypeList = sTypeList & "关联客户" & Chr(1) & 1 & Chr(2)
		sTypeList = sTypeList & "关联联系人" & Chr(1) & 2 & Chr(2)
		sTypeList = sTypeList & "产品名称" & Chr(1) & 3 & Chr(2)
		sTypeList = sTypeList & "产品编号" & Chr(1) & 4 & Chr(2)
		sTypeList = sTypeList & "产品型号" & Chr(1) & 5 & Chr(2)
		sTypeList = sTypeList & "受理单主题" & Chr(1) & 6 & Chr(2)
		sTypeList = sTypeList & "受理单编号" & Chr(1) & 7
		rpt.AddField2 "","select","sType",sTypeList,"","","null"
		rpt.AddField "","text","keywords",,"",""
		rpt.PageSize = 10
		rpt.sql = "EXEC Erp_Report_RepairLadger "&Info.User&",&ReportSort,@date1_0,@date1_1,@sType,@keywords,&excelmode"
	...
	Sub app_oncreate(lvw)
		...
		col.setlink "关联受理单","slID", "slAddUser" ,  45, -12
		Set col = lvw.headers(1)
		col.width = 250
		Set col = lvw.headers(3)
		col.setlink "产品名称","porID", "proAddUser" ,  21, -6
		Set col = lvw.headers(3)
		col.width = 250
		Set col = lvw.headers(11)
		col.setlink "联系人","personID", "personCateID" ,  2, -10
		Set col = lvw.headers(11)
		Set col = lvw.headers(12)
		col.setlink2 "关联客户", "customID" , "customCateID", 1, -9, ""
		Set col = lvw.headers(12)
		col.width = 250
		beginCol = 13
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		next
		For i = 13 To totalCol
			Set col = lvw.headers(i)
			col.display = "none"
		next
	end sub
	Function PowerCol(text,ID,User,qx)
		Dim str,share,IsShare
		IsShare = False
		If qx = 1 Then
			share = cn.Execute("SELECT TOP 1 ISNULL(share,0) share FROM tel WHERE ord = "&ID&" ")(0)
			If InStr(1,","&share&",", ","&info.user&",",1) > 0 Then
				IsShare = True
			end if
		end if
