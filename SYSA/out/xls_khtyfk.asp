...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(5).ColumnWidth=22
			...
			xlApplication.ActiveSheet.Columns(7).ColumnWidth=16
			...
			If bankin2&""<>"" And bankin2<>"0" Then  Str_Result = Str_Result &" and bankin2="& bankin2 &" "
...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from tel where ord=bankin3.company) as companyname,"&_
			"(select name from gate where ord=bankin3.cateid) as cateidname"&_
			" from bankin3 "&Str_Result&" "&px_Result&" "
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "客户退预收款列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				...
				sortbz=""
				sortbz=getbzflag(rs("bank"),"bankid")
				...
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,5).Value = intro2
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,5).font.Size=10
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,6).Value = cateidname
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,6).font.Size=10
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,7).Value = date7
				money1=sortbz&money1
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户退预收款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户退预收款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("客户退预收款列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的客户退预收款列表</u></strong></font></a></p>"
