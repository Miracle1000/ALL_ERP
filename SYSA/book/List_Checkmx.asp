...
'if isnull(values) then IsTime = null : exit function
	...
	if open_103_1=3 then
	elseif open_103_1=1 then
		Str_Result=Str_Result&" and chk_addcateid in ("&intro_103_1&")"
	else
		Str_Result=Str_Result&" and chk_addcateid=0"
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and chk_addcateid in ("&W3&") "
	end if
	if D1<>"" then
	...
	if D20<>"" then
		Str_Result=Str_Result&" and chk_pdnum >="&ml.isNum(D20)&""
	end if
	if D201<>"" then
		Str_Result=Str_Result&" and chk_pdnum <="&ml.isNum(D201)&""
	end if
	if D21<>"" then
		Str_Result=Str_Result&" and bk_postion like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and chk_note1 like '%"&ml.De(D22)&"%'"
	end if
	if M<>"" then
		Str_Result=Str_Result+" and  chk_yknum >="&ml.isNum(M)&""
'if M<>"" then
	end if
	if K<>"" then
		Str_Result=Str_Result+" and  chk_yknum <="&ml.isNum(K)&""
'if K<>"" then
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',chk_time)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,chk_time,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if L3<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L3&" 00:00:00',chk_addtime)>=0 "
'if L3<>"" then
	end if
	if L4<>"" then
		Str_Result=Str_Result+" and  datediff(s,chk_addtime,'"&L4&" 23:59:59')>=0 "
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	if px="1" then
		Ptr_Result=" order by chk_title desc"
	elseif px="2" then
		Ptr_Result=" order by chk_title asc"
	elseif px="3" then
		Ptr_Result=" order by chk_time desc"
	elseif px="4" then
		Ptr_Result=" order by chk_time asc"
	elseif px="5" then
		Ptr_Result=" order by chk_addtime desc"
	elseif px="6" then
		Ptr_Result=" order by chk_addtime asc"
	end if
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',chk_time)>=0 "
'if ret<>"" then
	end if
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,chk_time,'"&ret2&" 23:59:59')>=0 "
'if ret2<>"" then
	end if
	if C<>"" then
		if B="name" then
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_Checkmx.asp?ret="
	Response.write ret
	Response.write "&ret2="
	Response.write ret2
	Response.write "&B="
	...
	Response.write "&C="
	Response.write C
	Response.write """ method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">盘点明细表</td>" & vbcrlf & "                     <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> "    & vbcrlf &            "     <tr>" & vbcrlf &          "     <td colspan=""10"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding:0!important""> "& vbcrlf & "              <span id=""kh""><input type=""radio"" onClick=""Goto(1)"">盘点列表<input type=""radio"" onClick=""Goto(2)"" checked=""checked"">盘点明细表&nbsp;自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" name=""ret"" readonly=""true"" value="""
	Response.write ret
	...
	if open_103_10=3 or (open_103_10=1 and CheckPurview(intro_103_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_CheckmxBook.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "         </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""14%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">书名</div></td>" & vbcrlf & "                  <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">编号</div></td>" & vbcrlf & "                  <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">ISBN</div></td>" & vbcrlf & "            <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">作者</div></td>" & vbcrlf & "                 <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">出版社</div></td>" & vbcrlf & "                  <td width=""5%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">账面数量</div></td>" & vbcrlf & "                 <td width=""5%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">实际数量</div></td>" & vbcrlf & "             <td width=""5%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盈亏数量</div></td>" & vbcrlf & "                 <td width=""20%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">备注</div></td>" & vbcrlf & "              <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                      <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.id,a.Chk_fid,a.Chk_bkid,isnull(a.Chk_sjnum,0) as Chk_sjnum,isnull(a.Chk_pdnum,0) as Chk_pdnum,isnull(a.Chk_yknum,0) as Chk_yknum,a.Chk_note1,b.chk_addcateid,b.chk_addtime,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing from O_ChkBookmx a "&_
	"left join O_ChkBook b on a.Chk_fid=b.id "&_
	"left join O_RegBook c on a.Chk_bkid=c.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			addcateid=rs7("chk_addcateid")
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "              <td width=""14%"" height=""30"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_Check.asp?ord="
				Response.write rs7("chk_fid")
			...
			Response.write "</td>" & vbcrlf & "                   <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			...
			Response.write "</div></td>" & vbcrlf & "             <td width=""5%"" align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("chk_sjnum"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td width=""5%"" align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("chk_pdnum"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td width=""5%"" align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("chk_yknum"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("chk_note1")
			Response.write "</div></td>" & vbcrlf & "             <td width=""12%"" height=""30"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_Check.asp?ord="
				Response.write rs7("chk_fid")
	...
	Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30""></td>" & vbcrlf & "            <td></td>" & vbcrlf &"   </form>" & vbcrlf & "       <form name=""oneself"" action=""List_Checkmx.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "              " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按盘点主题排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "      <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按盘点主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按盘点时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按盘点时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按添加时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	dim actinon1
	action1="盘点明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "  divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_Checkmx.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action=""Del_Checkmx.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & "         document.getElementById('data').action=""Del_Checkmx.asp"";" & vbcrlf & " document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function Goto(args)" & vbcrlf & "{" & vbcrlf & "  if(args==1)" & vbcrlf & "     {" & vbcrlf & "               window.location=""List_check.asp"";" & vbcrlf & " }" & vbcrlf & "       else" & vbcrlf & "    {" & vbcrlf & "               window.location=""List_checkmx.asp"";" & vbcrlf & "} "& vbcrlf & "}" & vbcrlf &" </script> "& vbcrlf &" </body> "& vbcrlf &" </html>" & vbcrlf
