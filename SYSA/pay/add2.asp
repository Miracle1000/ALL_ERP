...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	Response.write Application("sys.info.jsver")
	Response.write """ language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function frameResize(){" & vbcrlf & "    var tb3 = I3.document.body.getElementsByTagName(""table"")[0];" & vbcrlf & "      document.getElementById(""sylist"").style.height=$(tb3).height()+0+""px"";" & vbcrlf & "}" & vbcrlf & "function chanceR(name)" & vbcrlf & "{" & vbcrlf & "     document.getElementById('sqr').innerHTML=name;" & vbcrlf & "  hm=1" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".as {width:60px;" & vbcrlf & "height:20px;}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	Response.write Application("sys.info.jsver")
	Response.expires=-1
	Response.write Application("sys.info.jsver")
	Response.AddHeader"pragma","no-cache"
	...
	up=request("up")
	ly=request("ly")
	If request.QueryString("qttype")&""<>"" Then
		qttype=request.QueryString("qttype")
	end if
	If request.QueryString("qtord")&""<>"" Then
		qtord=deurl(request.QueryString("qtord"))
	end if
	if request("payid")="" then
		conn.execute "delete from f_pay where del=7 and addcate=" & session("personzbintel2007")
		conn.execute "delete from pay where del=7 and addcate=" & session("personzbintel2007")
		conn.execute "insert into f_pay(addcate,del) values("&session("personzbintel2007")&",7)"
		payid=GetIdentity("f_pay","id","addcate","")
	else
		payid=getInt(request("payid"))
		ypayid = payid
		conn.Execute "UPDATE pay SET del3 = 0 WHERE del3 = 1 AND fid="& payid &" "
		If up&"" = "1" Then
			Set rs = conn.execute("select id from f_pay where del=7 and yid="& ypayid &" and addcate="& session("personzbintel2007"))
			If rs.eof Then
				conn.execute "insert into f_pay([title],[datejk],[sorce],[sorce1],[sorce2],[bz],[bx],[sp],[cateid_sp],[addcate],[sqid],[intro],[money1],[num],del,yid,qttype) select [title],[datejk],[sorce],[sorce1],[sorce2],[bz],[bx],[sp],[cateid_sp],"&session("personzbintel2007")&" As [addcate],[sqid],[intro],[money1],[num],7,"& ypayid &",qttype from f_pay where id="& ypayid
				payid=GetIdentity("f_pay","id","addcate","")
			else
				payid = rs("id")
			end if
			conn.execute "delete from pay where del=7 and addcate=" & session("personzbintel2007") &" and fid="& payid
			conn.execute "insert into pay ([money1],[money2],[money3],[money4],[money5],[money6],[sort],[intro],[intro2],[intro4],[intro5],[intro6],[complete],[plan1],[lead],[lead4],[lead5],[lead6],[addcate],[cateid],[cateid2],[cateid3],[company],[person],[option1],[event1],[chance],[contract],[contractth],[date1],[date2],[date4],[date5],[date6],[del],[delcate],[deldate],[cateid_sp],[sp],[del2],[del3],[alt],[caigou],[caigouth],[payid],[title],[jkid],[shouhou],[richeng],[fahuo],[iwork],[num],[bx],[fid],[jid],[startime],[staraddr],[endtime],[endaddr],[lc],[bus],[retime],[startime1],[endtime1],[city],[hotel],[ggtime],[ggcate],[ggintro],[gglw],[xytime],[yt],[bz],[ManuOrder],[office],[repair],[insure],[book],scdd,zdww,gxww,scsb,yord,Sccj) select [money1],[money2],[money3],[money4],[money5],[money6],[sort],[intro],[intro2],[intro4],[intro5],[intro6],[complete],[plan1],[lead],[lead4],[lead5],[lead6]," & session("personzbintel2007") &",[cateid],[cateid2],[cateid3],[company],[person],[option1],[event1],[chance],[contract],[contractth],[date1],[date2],[date4],[date5],[date6],7 as del,[delcate],[deldate],[cateid_sp],[sp],[del2],[del3],[alt],[caigou],[caigouth],[payid],[title],[jkid],[shouhou],[richeng],[fahuo],[iwork],[num],[bx],"& payid &" [fid],[jid],[startime],[staraddr],[endtime],[endaddr],[lc],[bus],[retime],[startime1],[endtime1],[city],[hotel],[ggtime],[ggcate],[ggintro],[gglw],[xytime],[yt],[bz],[ManuOrder],[office],[repair],[insure],[book],scdd,zdww,gxww,scsb,ord,Sccj from pay where fid="& ypayid & ""
		end if
	end if
	sqls="select * from f_pay where id="&payid
	...
	If qttype&"" = "" Then
		If rss("qttype")&""<>"" Then qttype = rss("qttype")
		If qttype&""<>"" Then
			Set rs2 = conn.execute("select top 1 scdd, zdww, gxww, scsb from pay where fid="& payid &"")
			If rs2.eof = False Then
				Select Case qttype&""
				Case "scdd" : qtord = rs2("scdd")
				Case "zdww" : qtord = rs2("zdww")
				Case "gxww" : qtord = rs2("gxww")
				Case "scsb" : qtord = rs2("scsb")
				End Select
			end if
		end if
	end if
	if request("payid")="" then
		orid=session("personzbintel2007")
	else
		orid=rss("sorce2")
		note=rss("intro")
	end if
	selmoney=getrsval("select sum(money1) as money1 from pay where del in(1,7) and fid="&payid)
	selnum=getrsval("select sum(num) as num from pay where del in(1,7) and fid="&payid)
	datejk=rss("datejk")
	if datejk="" or isnull(datejk) then
		datejk=date
	end if
	Response.write "<form method=""POST"" action=""save2.asp"" id=""demo"" onsubmit=""return Validator.Validate(this,2);"" name=""date"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td valign=""top"">" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""place place42"">费用使用"
	if up=1 then
		Response.write "修改"
	else
		Response.write "添加"
	end if
	Response.write "</td>" & vbcrlf & "      <td>&nbsp;</td>" & vbcrlf & "      <td align=""right""></td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table> " & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "                  <td colspan=""3"">基本信息</td>" & vbcrlf & "             <td><div align=""right"">" & vbcrlf & "           <input type=""hidden"" name=""qttype"" value="""
	Response.write qttype
	Response.write """>" & vbcrlf & "           "
	if ly<>"1" then
		Response.write "" & vbcrlf & "                  <input name=""Submit221"" type=""submit"" class=""anybutton"" value=""保存并报销"" onClick=""return CheckDetail()&&ask1();"">" & vbcrlf & "                   "
	end if
	Response.write "" & vbcrlf & "                 <input name=""Submit21"" type=""submit"" class=""anniu"" value=""保存"" onClick=""return CheckDetail()&&ask();"">" & vbcrlf & "                   <input name=""Submit41"" type=""reset"" class=""anniu"" value=""重置"">" & vbcrlf & "                 &nbsp;</div></td>" & vbcrlf & "       </tr>        " & vbcrlf & "                <tr>" & vbcrlf & "              <td width=""15%""><div align=""right"">使用主题：</div></td>" & vbcrlf & "            <td width=""35%"" ><div align=""left"">" & vbcrlf & "                   <input name=""title""  type=""text"" id=""title"" maxlength='200' msg=""友情提示：标题录入不能超过200个字符。"" value="""
	Response.write rss("title")
	Response.write """>" & vbcrlf & "                  <font id=""tit"" style=""color:red"">*</font></div></td>" & vbcrlf & "                      <td width=""15%""><div align=""right"">" & vbcrlf & "                   <div align=""right"">使用总额：</div>" & vbcrlf & "                     </div></td>" & vbcrlf & "                     <td width=""35%""><div align=""left""> <input name=""allpay"" style=""color:#999999"" type=""text"" id=""allpay"" value="""
	Response.write selmoney
	Response.write """ onKeyUp=""value=value.replace(/[^ \d\.]/g,'');checkDot('allpay','"
	Response.write num_dot_xs
	Response.write "')""  size=""15"" readonly >" & vbcrlf & "                   <input name=""yallpay"" type=""hidden"" value="""
	Response.write selmoney
	Response.write """>" & vbcrlf & "                        </div></td>" & vbcrlf & "             </tr>" & vbcrlf & "             <tr>" & vbcrlf & "              <td width=""15%""><div align=""right"">使用日期：</div></td>" & vbcrlf & "                    <td width=""35%"" style=""padding-top:0!important;padding-bottom:0!important""><div align=""left"">" & vbcrlf & "          <TABLE border=0 align=""left"" >" & vbcrlf & "            <TBODY>" & vbcrlf & "              <TR>" & vbcrlf & "                <TD>" & vbcrlf & "                                                 <INPUT name=""ret"" readonly onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" value="""
	Response.write datejk
	Response.write """ size=""10"" >" & vbcrlf & "                <input type=""button"" style=""margin-bottom:-0.5px""  class=""anybutton"" value=""选择日期"" align=""absMiddle""  border=""0""  id=""daysOfMonthPos"" name=""daysOfMonthPos""  onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" />" & vbcrlf & "   <span class=""red"">*</span>" & vbcrlf & "                <DIV id=daysOfMonth style=""POSITION: absolute""></DIV></TD>" & vbcrlf & "              </TR>" & vbcrlf & "            </TBODY>" & vbcrlf & "          </TABLE>" & vbcrlf & "              </div></td>" & vbcrlf & "                 <td width=""15%""><div align=""right"">总张数：</div></td>" & vbcrlf & "                 <td width=""35%""><div align=""left""><span class=""gray"">" & vbcrlf & "          <input name=""allnum"" style=""color:#999999"" type=""text"" id=""allnum"" value="
	Response.write selnum
	Response.write """ size=""4"" maxlength=""3"" readonly>" & vbcrlf & "          <input name=""yallnum"" type=""hidden"" value="""
	Response.write selnum
	Response.write """ >" & vbcrlf & "          </span></div></td>" & vbcrlf & "             </tr>" & vbcrlf & "           "
	...
	Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "                   <td width=""15%""><div align=""right"">关联申请单：</div></td>" & vbcrlf & "                  "
	sqtitle=setname("paysq","id",rss("sqid"),"title")
	Response.write "" & vbcrlf & "                     <td><input type='text' size='20' name='sqlist' id='sqlist' readonly value="""
	Response.write sqtitle
	Response.write """ "
	If InStr(",scdd,zdww,gxww,scsb,chargeshare,", ","& LCase(qttype&"") &",")>0 Then Response.write "disabled style='background-color:#e5e5e5; border:solid #bcbcbc 1px;' "
	Response.write """ "
	Response.write " onClick=""javascript:window.open('../search/resultpay10.asp?intro_cateid="
	Response.write w3_list
	Response.write "','eventcom1','width=' + 900 + ',height=' + 550 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=200,top=200')""/><font style=""color:red"" id=""sql""></font>                   " & vbcrlf & "                        <input name=""paysqid"" id=""paysqid"" type=""hidden"" value="""
	Response.write w3_list
	Response.write rss("sqid")
	Response.write """></td>" & vbcrlf & "                   <td><div align=""right"">" & vbcrlf & ""
	if up=1 then
		Response.write "币种："
	else
		if bz<>0 then
			Response.write "币种：" & vbcrlf & ""
		end if
	end if
	Response.write "</div></td>" & vbcrlf & "                  <td><div align=""left"">" & vbcrlf & ""
	if up=1 then
		bz=rss("bz")
		if bz="" or isnull(bz) or bz=0 then bz=14
		sqls="select sort1 from sortbz where id="&bz
		set rss=conn.execute(sqls)
		Response.write rss("sort1") &"<input type='hidden' name='bz' id='bz' value="& bz &">"
	else
		if bz<>0 then
			Response.write "" & vbcrlf & "                     <select name=""bz"" id=""bz"">" & vbcrlf & ""
	...
	Response.write "" & vbcrlf & "                     </div>" & vbcrlf & "                  </td>" & vbcrlf & "         </tr>               " & vbcrlf & "               <tr>            " & vbcrlf & "                  <td width=""15%"" colspan=""4""><div align=""left"" style=""padding-left:32px;""><font class=""left""><a href=""###"" style=""text-decoration:none"" onClick=""" & vbcrlf & "                        if(document.getElementById('tdr').style.display=='none')" & vbcrlf & "                                                                {document.getElementById('tdr').style.display='';" & vbcrlf & "                                                               document.getElementById('nodsr').src='../images/12.gif'}" & vbcrlf & "                                                                else" & vbcrlf & "                                                            {" & vbcrlf & "                                                               document.getElementById('tdr').style.display='none';" & vbcrlf & "                                                           document.getElementById('nodsr').src='../images/11.gif';" & vbcrlf & "                                                                }""><img border=""0"" id=""nodsr"" src=""../images/11.gif"">&nbsp;<strong>使用人员</strong></a></font>&nbsp;&nbsp;<font id=""sqr"">" & vbcrlf & "                                                           </font>&nbsp;<fontid=""ry"" color=""red"">*</font></div></td>" & vbcrlf & "                 </tr>" & vbcrlf & "            <tr id=""tdr"" style=""display:none"">" & vbcrlf & "                   <td colspan=""4"">" & vbcrlf & "                  <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "           <tr>" & vbcrlf & "                <td colspan=""4""> " & vbcrlf & "                          "
	basesql="select ord, orgsid from gate where 1=1 "&str_w3
	Response.write CBaseUserTreeHtmlRadioCE(basesql,"", "","","member2",  "", "", "", orid,"Tiggerfirst:if(node.value){chanceR(node.text)}")
	Response.write "</td>" & vbcrlf & "                        </tr>" & vbcrlf & "      </table>" & vbcrlf & "       </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "              <td  colspan=""4"">" & vbcrlf & "                 <IFRAME name=""I3"" SRC=""symx.asp?payid="
	Response.write cstr(payid)
	Response.write "&qttype="
	Response.write qttype
	Response.write "&qtord="
	Response.write qtord
	Response.write """ id=""sylist"" width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>" & vbcrlf & "                </tr>" & vbcrlf & "            <tr>" & vbcrlf & "        <td><div align=""right"">使用概要：</div></td>" & vbcrlf & "        <td colspan=""5"" height=""37""><textarea name=""note"" style=""display:none"" cols=""1"" rows=""1"">"
	if note<>"" then Response.write note
	Response.write "</textarea>" & vbcrlf & " <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=note&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME></td>" & vbcrlf & "             </tr> " & vbcrlf & "      <tr>" & vbcrlf &"        <td height=""50"" colspan=""6"">" & vbcrlf & "           <div align=""center"">" & vbcrlf & "              "
	if ly<>"1" then
		Response.write "" & vbcrlf & "                 <input name=""Submit22"" type=""submit"" class=""anybutton"" value=""保存并报销"" onClick=""return CheckDetail()&&ask1();"">" & vbcrlf & "            "
	end if
	Response.write "" & vbcrlf & "                 <input name=""Submit2"" type=""submit"" class=""anniu"" value=""保存"" onClick=""return CheckDetail()&&ask();"">" & vbcrlf & "                    <input name=""Submit4"" type=""reset"" class=""anniu"" value=""重置"">" & vbcrlf & "                </div>                  </td>" & vbcrlf & "        </tr> " & vbcrlf & " </table>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "    <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr height=""200"">" & vbcrlf & "    <td ></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & " </td>" & vbcrlf & ""
	if up=1 then
		action1="费用使用修改"
	else
		action1="费用使用添加"
	...
	call close_list(1)
	Response.write "" & vbcrlf & "  </tr>" & vbcrlf & " </table>" & vbcrlf & "</form> " & vbcrlf & "<div style=""position:absolute;display:none;width:500px;height:320px;border:0px;padding:1px;z-index:9999;""id=""dhtml""></div>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "varhm=0;" & vbcrlf & "function ask()" & vbcrlf & "{" & vbcrlf & "       if(document.getElementById('title').value=="""")" & vbcrlf & "    {" & vbcrlf & "               document.getElementById('tit').innerHTML=""*必填""" & vbcrlf & "          return false;" & vbcrlf & "    }" & vbcrlf & "      var membool=false" & vbcrlf & "       var memarr=document.getElementsByName('member2');" & vbcrlf & "        for (i=0;i<memarr.length;i++)" & vbcrlf & "   {" & vbcrlf & "               if(memarr[i].checked)" & vbcrlf & "           {" & vbcrlf & "                       membool=true" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "       if(membool==false)" & vbcrlf & "      {" & vbcrlf & "               document.getElementById('ry').innerHTML=""*必填""" & vbcrlf & "              return false;" & vbcrlf & "   }" & vbcrlf & "       document.date.action=""save2.asp?ord="
	Response.write payid
	Response.write "&yord="
	Response.write ypayid
	Response.write "&ly="
	Response.write ly
	Response.write "&up="
	Response.write up
	Response.write "&qtord="
	Response.write qtord
	Response.write """;" & vbcrlf & "        return true;" & vbcrlf & "}" & vbcrlf & "function ask1()" & vbcrlf & "{" & vbcrlf & "   if(document.getElementById('title').value=="""")" & vbcrlf & "    {" & vbcrlf & "               document.getElementById('tit').innerHTML=""必填""" & vbcrlf & "           return false;" & vbcrlf & "    }" & vbcrlf & "      var membool=false" & vbcrlf & "   var memarr=document.getElementsByName('member2');" & vbcrlf & "       for (i=0;i<memarr.length;i++)" & vbcrlf & "   {" & vbcrlf & "               if(memarr[i].checked)" & vbcrlf & "           {" & vbcrlf & "                       membool=true" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "       if(membool==false||document.getElementById(""sqr"").innerHTML=="""")" & vbcrlf & "    {" & vbcrlf & "               document.getElementById('ry').innerHTML=""*必填""" & vbcrlf & "           return false;" & vbcrlf & "   }" & vbcrlf & "       document.date.action=""save2.asp?bx=1&ord="
	Response.write payid
	Response.write "&yord="
	Response.write ypayid
	Response.write "&ly="
	...
	Response.write "&qtord="
	Response.write qtord
	Response.write """;" & vbcrlf & "        return true;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function CheckDetail()" & vbcrlf & "{" & vbcrlf & "    var dobj=document.getElementById(""sylist"").contentWindow.document.getElementsByTagName(""table"");" & vbcrlf & "    if(dobj[0].rows.length<3)" & vbcrlf & "       {" & vbcrlf & "alert('没有添加使用明细！');" & vbcrlf & "            return false;" & vbcrlf & "   }" & vbcrlf & "       return true;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div style=""position: absolute; display:none;width: 500px;height:320px;border: 0px;padding: 1px;z-index: 100;"" id=""dhtml""></div>" & vbcrlf & "<script language='javascript'>" & vbcrlf & "var slbl=document.getElementById(""member2_"
	Response.write session("personzbintel2007")
	Response.write """);" & vbcrlf & "if(slbl) slbl.fireEvent(""onclick"");" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
