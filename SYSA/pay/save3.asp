...
'set f=fso.getfile(filepath)
	...
	jkid=Request.form("sid")
	date1=trim(Request.form("ret"))
	paymoney=getInt(Trim(request("paymoney")))
	date1=cdate(date1)
	intro=trim(request.form("note"))
	complete=11
	addcate=session("personzbintel2007")
	del=1
	date6=now
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from payjk where id="&jkid&" "
	rs.open sql,conn,1,1
	sorce_user2=rs("sorce")
	sorce_user3=rs("sorce1")
	sorce_user=rs("sorce2")
	bz=rs("bz")
	rs.close
	set rs=nothing
	If Len(sorce_user2&"") = 0 Then sorce_user2 = 0
	If Len(sorce_user3&"") = 0 Then sorce_user3 = 0
	spmoney=setname("payjk","id",jkid,"spmoney")
	jkmoney=getrsval("select sum(money1) as money1 from pay where del=1 and complete<>12 and jkid="&jkid)
	dkmoney=getrsval("select sum(dkmoney) as money1 from dk where del=1 and jkid="&jkid)
	if jkmoney="" or isnull(jkmoney) then jkmoney=0
	if spmoney="" or isnull(spmoney) then spmoney=0
	if dkmoney="" or isnull(dkmoney) then dkmoney=0
	yemoney=zbcdbl(spmoney)-zbcdbl(jkmoney)-zbcdbl(dkmoney)
	if dkmoney="" or isnull(dkmoney) then dkmoney=0
	if paymoney&""<>"" And Not IsNumeric(paymoney&"") then
		Response.write("<script>alert('请输入正确的金额');history.back();</script>")
		call db_close : Response.end
	end if
	If CDbl(Formatnumber(yemoney,num_dot_xs,-1,-1,0))<CDbl(Formatnumber(paymoney,num_dot_xs,-1,-1,0)) then
		call db_close : Response.end
		Response.write "<script>alert('返还金额不能大于借款使用金额');history.back();</script>"
		call db_close : Response.end
	elseif cdbl(yemoney)=cdbl(paymoney) then
	end if
	sqlStr="Insert Into pay(title,money1,money2,intro,complete,addcate,cateid,cateid2,cateid3,date1,jkid,date6,del,bz) values('"
	sqlStr=sqlStr & title & "',"
	sqlStr=sqlStr & paymoney & ","
	sqlStr=sqlStr & paymoney & ",'"
	sqlStr=sqlStr & intro & "',"
	sqlStr=sqlStr & complete & ","
	sqlStr=sqlStr & addcate & ","
	sqlStr=sqlStr & sorce_user & ","
	sqlStr=sqlStr & sorce_user2 & ","
	sqlStr=sqlStr & sorce_user3 & ",'"
	sqlStr=sqlStr & date1 & "',"
	sqlStr=sqlStr & jkid & ",'"
	sqlStr=sqlStr & date6 & "',"
	sqlStr=sqlStr & del & ","
	sqlStr=sqlStr & bz & ")"
	Conn.execute(sqlStr)
	ord = GetIdentity("pay","ord","addcate","")
	Set commSP = New CommSPHandle
	Call commSP.init("payfh",ord)
	sp_id = request("sp")
	If sp_id &"" = "" Then sp_id = 0 Else sp_id = CLng(sp_id)
	cateid_sp = request("cateid_sp")
	If cateid_sp &"" = "" Then cateid_sp = 0 Else cateid_sp = CLng(cateid_sp)
	Call commSP.saveBill2(sp_id, cateid_sp, 0, paymoney)
	if conn.execute("select 1 from pay where ord="&ord&" and complete=8 ").eof=false then
		Response.redirect "fh1.asp?ord="&ord
	else
		Response.redirect "fhlist.asp"
	end if
	conn.close
	set conn = Nothing
