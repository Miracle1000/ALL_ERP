...
'While rs.eof = False
	...
	Sub Page_Load
		...
		dim pagesizes ,i, sizepageHTML ,pagesize
		pagesize=10
...
'for i = 0 to ubound(pagesizes)
		...
		next
		Response.write(app.DefTopBarHTML(app.virPath, "", "预算使用明细表", "<select name='sizepage' id='sizepage' onchange='doSearch(0)' style='margin-top:5px;margin-right:5px' >" & sizepageHTML & "</select><input type='hidden' id='px' value='4' /><a class='tableTitleLinks sortRule' href='javascript:void();' onclick='showmenu(this);return false' style='position: absolute; top: 20px; left: 260px;'>排序规则<img class='resetElementHidden' src='../images/i10.gif' width='9' height='5' border='0'><img class='resetElementShowNoAlign' style='display:none;' src='../skin/default/images/MoZihometop/content/r_down2.png' width='9' height='5' border='0'></a>")) & vbCrLf
		Dim rs
		Response.write "" & vbcrlf & "    <script type=""text/javascript"">" & vbcrlf & "    function LoadUrl(str) {" & vbcrlf & "        location.href = str;" & vbcrlf & "    }" & vbcrlf & "    function DelSend(ids) {" & vbcrlf & "        $.get('?__msgId=doDel', {" & vbcrlf & "            id: ids" & vbcrlf & "      }," & vbcrlf & "        function(data) {" & vbcrlf & "            var msg = parseInt(data);" & vbcrlf & "            if (msg == 1) {" & vbcrlf & "                lvw_refresh('ids1');" & vbcrlf & "            }" & vbcrlf & "        })" & vbcrlf & "    }" & vbcrlf & "    function doSearch(pIndex) {"& vbcrlf & "        ajax.regEvent(""dosearch"")" & vbcrlf & "                ajax.addParam('ysid',$('#ysid').val());" & vbcrlf & "        ajax.addParam('px', $('#px').val());" & vbcrlf & "        ajax.addParam('indate1', $('#indate1').val());" & vbcrlf & "        ajax.addParam('indate2', $('#indate2').val());" &vbcrlf & "         ajax.addParam('status', $('#status').val());" & vbcrlf & "            ajax.addParam('bz', $('#bz').val());" & vbcrlf & "            ajax.addParam('tbi', $('#tbi').val());" & vbcrlf & "        ajax.addParam('key', $('#key').val());" & vbcrlf & "            ajax.addParam(""Psize"",$('#sizepage').val());" & vbcrlf & "      //pIndex==1表示要带上当前的翻页条件" & vbcrlf & "             ajax.addParam(""pageindex"",pIndex==1?document.getElementById(""lvw_pindex_ids1"").value:1);" & vbcrlf & "        ajax.send(function(r){" & vbcrlf & "                      document.getElementById(""lvwbody"").innerHTML = r;" & vbcrlf & "                 MergeCells();" & vbcrlf & "           });" & vbcrlf & "    }" & vbcrlf & "        function doExcel(){var lvw = new Listview('ids1');lvw.cexcel();}" & vbcrlf & "        window.onlistviewRefresh = function(){" & vbcrlf & "          document.getElementById(""toolbar1"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "            MergeCells();" &vbcrlf & " }" & vbcrlf & "       document.body.onload = function(){" & vbcrlf & "              document.getElementById(""toolbar1"").style.width=document.body.scrollWidth+""px"";//document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "             document.getElementById(""lvwbody"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "              MergeCells();" & vbcrlf & "           window.onresize=function() {" & vbcrlf & "                    document.getElementById(""toolbar1"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "                     document.getElementById(""lvwbody"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       //合并合计列" & vbcrlf & "    function MergeCells()" & vbcrlf & "   {" & vbcrlf & "               var tb= document.getElementById(""lvw_dbtable_ids1"");" & vbcrlf & "              if(tb.rows.length>2)" & vbcrlf & "            {" & vbcrlf & "                       var tr1=tb.rows[tb.rows.length-1];" & vbcrlf & "                      if(tr1.cells[0].colSpan==1){" & vbcrlf & "                            tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.cells[0].colSpan = 5;" & vbcrlf & "                               tr1.cells[0].style.cssText= ""text-align:right;"";" & vbcrlf & "                          tr1.cells[0].innerHTML=tr1.cells[0].innerHTML + ""：&nbsp;&nbsp;""" & vbcrlf & "                 }" & vbcrlf & "                       var tr2=tb.rows[tb.rows.length-2];" & vbcrlf & "                      if(tr2.cells[0].colSpan==1){" & vbcrlf & "                            tr2.deleteCell(1);" & vbcrlf & "                              tr2.deleteCell(1);" & vbcrlf & "                              tr2.deleteCell(1);" & vbcrlf & "                              tr2.deleteCell(1);" & vbcrlf & "                                tr2.cells[0].colSpan = 5;" & vbcrlf & "                               tr2.cells[0].style.cssText= ""text-align:right;"";" & vbcrlf & "                          tr2.cells[0].innerHTML=tr2.cells[0].innerHTML + ""：&nbsp;&nbsp;""" & vbcrlf & "                  }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       function showmenu(tag) {" &vbcrlf & "         if (document.getElementById('asasd'))" & vbcrlf & "           {" & vbcrlf & "                       document.getElementById(""asasd"").style.display='';" & vbcrlf & "                }" & vbcrlf & "               else " & vbcrlf & "           {       " & vbcrlf & "                        var obj=document.createElement(""div"");" & vbcrlf & "                    obj.id=""asasd"";" & vbcrlf & "                   obj.style.cssText=""Z-INDEX:20000; POSITION: absolute; HEIGHT: 150px; TOP: 20px; LEFT: 270px"";" & vbcrlf & "                  var html;" & vbcrlf & "                       html="""";" & vbcrlf & "                  html=html+""<TABLE border=0 cellSpacing=-2 cellPadding=-2 width=150 height=150>"";" & vbcrlf & "                  html=html+""<TBODY><TR><TD height=139>"";" & vbcrlf & "                      html=html+""<TABLE border=0 width=150 bgColor=#ecf5ff height=115><TBODY>"";" & vbcrlf & "                 html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(1);doSearch();\""><FONT color=#2f496e>按照人员排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                 html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(2);doSearch();\""><FONT color=#2f496e>按照人员排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                 html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(3);doSearch();\""><FONT color=#2f496e>按照报销日期排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(4);doSearch();\""><FONT color=#2f496e>按照报销日期排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                     html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(5);doSearch();\""><FONT color=#2f496e>按照报销金额排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                   html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(6);doSearch();\""><FONT color=#2f496e>按照报销金额排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                       html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(7);doSearch();\""><FONT color=#2f496e>按照报销状态排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                   html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(8);doSearch();\""><FONT color=#2f496e>按照报销状态排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""</TBODY></TABLE>"";" & vbcrlf & "                        html=html+""</TD></TR></TBODY>"";" & vbcrlf & "                   html=html+""</TABLE>"";" & vbcrlf & "                     obj.innerHTML=html;" & vbcrlf & "                     document.body.appendChild(obj);" & vbcrlf & "                 //人员、时间、金额、类型、状态字段" & vbcrlf & "              }" & vbcrlf & "    }" & vbcrlf & "    </script>" &vbcrlf & " <div style='background-image:url(../images/112.gif);width:100%;height: 55px;' id='toolbar1' align=""right"">" & vbcrlf & "          <table style='width:auto;table-layout:auto;border-collapse:separate' cellspacing='3' id=""kh"">" & vbcrlf & "      <tr>" & vbcrlf & "    <td align=""right"" style='border-top:0px;border-bottom:0px;border-left:0px'>报销日期" & vbcrlf & "    从 <input type=""text"" id=""indate1"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" /> " & vbcrlf & "    至 <input type=""text"" id=""indate2"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" />　" & vbcrlf & "     <select name=""status"" id=""status"">" & vbcrlf & "  <option value=""10000"">审批状态</option>" & vbcrlf & "   <option value=""0"">未审批</option>" & vbcrlf & " <option value=""1"">审批中</option>" & vbcrlf & " <option value=""3"">审批通过</option>" & vbcrlf & "       </select>" & vbcrlf & "       <select name=""bz"" id=""bz"">" & vbcrlf & "                <option value="""">币种</option>" & vbcrlf & "            "
		Dim bzsql,setbz
		setbz=0
		...
		loop
		set rs=nothing
		Response.write "" & vbcrlf & "      </select>" & vbcrlf & "       <select name=""tbi"" id=""tbi"">" & vbcrlf & "        <option value=""0"">报销主题</option>" & vbcrlf & "       <!-- <option value=""1"">报销金额</option> -->" & vbcrlf & "      <option value=""2"">报销人员</option>" & vbcrlf & "       </select>" & vbcrlf & "       <input type=""text""  id=""key""maxlength=""20"" /> "& vbcrlf &   " <input type=""hidden"" id=""ysid"" name=""ysid"" value="
		Response.write app.GetText("ysid")
		Response.write """>" & vbcrlf & "    <input type=""button"" value=""检索"" class=""oldbutton"" onclick=""doSearch();"" /> " & vbcrlf & "    "
		if app.power.existsPower(6,10) then
			Response.write "<input type=""button"" value=""导出"" class=""oldbutton"" id=""dcdb"" onclick='doExcel()'/>&nbsp;"
		end if
		if app.power.existsPower(6,7) then
		...
		Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "        </div>" & vbcrlf & "  <style>" & vbcrlf & "         .lvw_pagebar" & vbcrlf & "            {" & vbcrlf & "                       background:#fff;" & vbcrlf & "                }" & vbcrlf & "               .lvwpagesizearea {" & vbcrlf & "                      display:none;" & vbcrlf & "           }" &vbcrlf & "         td.lvw_index {background-image:url()}" & vbcrlf & "   </style>" & vbcrlf & "        <div style=""overflow:hidden;min-width:900px;box-sizing:border-box;"" id='lvwbody'>" & vbcrlf & " "
		App_doSearch
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		...
		For i=1 To 8
			If i=5 Then
				...
				lvw.headers(i).canSum =True
			else
				lvw.headers(i).dbtype ="str"
				lvw.headers(i).canSum =False
			end if
		next
		For i=9 To 12
	...
	Function App_doSearch
		Dim dtype, indate1,indate2,status,bz,lead,tbi, Keystr ,Psize,pageindex ,px ,ysid
		If Len(app.GetText("ysid"))>0 Then
			ysid      = app.base64.deurl(app.GetText("ysid"))
		else
			ysid          = 0
		end if
		px            = app.getInt("px")
		...
		lvw.sql = "exec erp_budget_paymx " & Info.User & ",&excelmode,"& tbi &",'" & Keystr & "','"& indate1 &"','" & indate2 &"',"& px &","& status &","& bz &","& ysid &",&pagesize, "&pageindex
     'response.end
		...
		lvw.headers(5).dbtype = "money"
		lvw.headers(5).formatText = "code:FormatHTML(@value)"
		lvw.headers(6).width = 80
		lvw.headers(6).canSum =False
		lvw.headers(7).width = 140
		lvw.headers(7).dbtype = "str"
		lvw.headers(7).formattext = "code:BudgetLinkHTML(1,@cells[10],""@cells[8]"",@cells[12])"
		lvw.headers(7).canSum =False
		lvw.headers(8).width = 140
		lvw.headers(8).dbtype = "str"
		lvw.headers(8).formattext = "code:BudgetLinkHTML(2,@cells[11],""@cells[9]"",@cells[13])"
	...
	Function FormatHTML(v)
		FormatHTML=FormatNumber(v&"",Info.MoneyNumber,-1)
'Function FormatHTML(v)
	end function
	Function BudgetLinkHTML(sType,bgId, bgtit, isqx)
		Dim surl
		If sType=1 Then
			surl="../pay/paybxcontent.asp?paybxid=" & bgId
		else
			surl="../budget/content.asp?ord=" & app.base64.pwurl(bgId)
		end if
		If Len(bgtit)>0 Then
			If isqx=1 Then
				BudgetLinkHTML = "<a href='" & surl & "' target='_blank'>" & bgtit & "</a>"
			else
