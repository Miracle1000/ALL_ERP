...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>发送短信</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	...
	openSignature=trim(request.Form("openSignature"))
	if openSignature="" then
		Response.write"<script language=javascript>alert('请选择是否开启短信签名！'); history.back();</script>"
		call db_close : Response.end
	end if
	if openSignature=1 and signature="" then
		Response.write"<script language=javascript>alert('请填写签名内容！'); history.back();</script>"
		call db_close : Response.end
	end if
	set rs=server.CreateObject("adodb.recordset")
	...
	if rs.eof then
		conn.execute "insert into setMessage(lastCon,openLastCon) values('"&signature&"',"&openSignature&")"
		Response.write"<script language=javascript>alert('设置成功！'); window.location.href('edit.asp');</script>"
		call db_close : Response.end
	else
		conn.execute "update setMessage set lastCon='"&signature&"',openLastCon="&openSignature&""
		Response.write"<script language=javascript>alert('设置成功！'); window.location.href='edit.asp';</script>"
		call db_close : Response.end
	end if
