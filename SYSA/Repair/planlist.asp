...
'items = rpt.Settings.item(i)
	...
	Dim num1_dot, num_dot_xs
	Sub onReportInit(rpt, lt)
		Call app.addDefaultScript()
		Dim rs, dat
		Dim jinjiStr, jiedaiStr, width2, sTypeList
		dat = Split(year(now) & "-" & month(now) & "-1," & date, ",")
'Dim jinjiStr, jiedaiStr, width2, sTypeList
		jinjiStr = "" : jiedaiStr = ""
		set rs = cn.execute("Select ord,sort1 from sortonehy WITH(NOLOCK) where gate2=46 order by gate1 desc")
		while rs.eof = False
			jinjiStr = jinjiStr & rs("sort1") & Chr(1) & rs("ord")
			rs.movenext
			If rs.eof = False Then
				jinjiStr = jinjiStr & Chr(2)
			end if
		wend
		rs.close
		set rs = nothing
		set rs = cn.execute("Select ord,sort1 from sortonehy WITH(NOLOCK) where gate2=45 order by gate1 desc")
		while rs.eof = False
			jiedaiStr = jiedaiStr & rs("sort1") & Chr(1) & rs("ord")
			rs.movenext
			If rs.eof = False Then
				jiedaiStr = jiedaiStr & Chr(2)
			end if
		wend
		rs.close
		set rs = nothing
		rpt.title = "维修受理单明细表"
		rpt.pagesize = 10
		rpt.addfield "受理时间：","dates","date1","","", dat
		rpt.addfield "&nbsp;","select","jinji", jinjiStr,"",""
		rpt.SetNullMsg 1,"紧急程度"
		rpt.addfield "&nbsp;","select","jiedai", jiedaiStr,"",""
		rpt.SetNullMsg 2,"接待方式"
		rpt.addfield "&nbsp;","select","zt1", "未派工" & Chr(1) & "1" & Chr(2) & "部分派工" & Chr(1) & "2"& Chr(2) & "派工完毕" & Chr(1) & "3","",""
		rpt.SetNullMsg 3,"派工状态"
		rpt.addfield "&nbsp;","select","zt2", "未维修" & Chr(1) & "1" & Chr(2) & "维修中" & Chr(1) & "2"& Chr(2) & "维修完毕" & Chr(1) & "3","",""
		rpt.SetNullMsg 4,"维修状态"
		rpt.addfield "&nbsp;","select","zt3", "保修期内" & Chr(1) & "2" & Chr(2) & "保修期外" & Chr(1) & "1"& Chr(2) & "其他" & Chr(1) & "3","",""
		rpt.SetNullMsg 5,"保修情况"
		sTypeList = sTypeList & "受理单编号" & Chr(1) & "2"& Chr(2)
		sTypeList = sTypeList & "产品名称" & Chr(1) & "20"& Chr(2)
		sTypeList = sTypeList & "产品编号" & Chr(1) & "21"& Chr(2)
		sTypeList = sTypeList & "产品型号" & Chr(1) & "22"& Chr(2)
		sTypeList = sTypeList & "故障描述" & Chr(1) & "23"& Chr(2)
		sTypeList = sTypeList & "受理概要" & Chr(1) & "3"& Chr(2)
		sTypeList = sTypeList & "关联客户" & Chr(1) & "4"& Chr(2)
		sTypeList = sTypeList & "联系人" & Chr(1) & "5"
		If ZBRuntime.MC(17002) then
			sTypeList = sTypeList & Chr(2) & "批号" & Chr(1) & "30"
			sTypeList = sTypeList & Chr(2) & "序列号" & Chr(1) & "31"
		end if
		rpt.addfield "&nbsp;","select","searchType",sTypeList ,"",""
		rpt.SetNullMsg 6,"受理单主题"
		rpt.addfield "","text","searckKey", "", "", ""
		rpt.headHTML = "<style>#lvw_dbtable_mlistvw tr td table{width:100%} .slJian table td img{width:50px;} .slJian table td.td1{width:35%; text-align:right;vertical-align:top; padding-top:1px;} .slJian table td.td2{width:65%; text-align:left;}</style>"
		rpt.addfield "","text","searckKey", "", "", ""
		If app.power.existsPower(45,7) = True Then
			rpt.canprint = True
		else
			rpt.canprint = False
		end if
		If app.power.existsPower(45,10) Then
			rpt.canexcel = True
		else
			rpt.canexcel = False
		end if
		...
		if app.power.existsPower(45,3)= true Then strHandle ="批量派工"
		If Len(strHandle)>0 Then rpt.setBatHandle "ord", strHandle '"批量删除,批量派工"
		rpt.sql = "exec erp_report_wxslmx "& Info.User &", &excelmode,&pagesize, &pageindex,&reportsort,&sortSql,@date1_0, @date1_1, @jinji, @jiedai, @zt1, @zt2, @zt3, @searchType, @searckKey, @proUnitID, @zt"
	end sub
	Dim intro_45_2, intro_45_3, intro_45_6, intro_45_8, intro_45_1, intro_45_14, intro_1_1, intro_1_14,  intro_2_1,  intro_2_14,  intro_21_1,  open_21_14
	Sub app_oncreate(lvw)
		...
		col.title = "选择"
		col.width = 30
		col.formattext = "code:GetCheckboxHtml(@value,@cells[1],@cells[10],@cells[34])"
		col.canSum = False
		Set col = lvw.headers(3)
		col.width = 150
		col.dbtype = "str"
		col.formattext = "code:GetSLTitleHtml(@cells[1],""@value"",@cells[34],""@cells[35]"")"
		col.canSum = False
		Set col = lvw.headers(4)
		col.width = 100
		col.dbtype = "str"
		col.formattext = "code:GetSLidHtml(""@value"",@cells[34],""@cells[35]"")"
		col.canSum = False
		...
		col.width = 120
		col.dbtype = "str"
		col.formattext = "code:GetProductHtml(""@value"",@cells[36],@cells[37])"
		col.canSum = False
		...
		col.title = "派工状态"
		col.width = 80
		col.formattext = "code:GetPGStatusHtml(@value)"
		col.canSum = False
		...
		col.width = 70
		col.Formula = "FormatNumber(@value,"& num1_dot &",-1)"
		col.width = 70
		col.dbtype = "number"
		...
		col.width = 90
		col.Formula = "FormatNumber(@value,"& num_dot_xs &",-1)"
		col.width = 90
		col.dbtype = "money"
		...
		col.dbtype = "str"
		col.canSum = False
		col.formattext = "code:JiejianToHtml(@cells[2],1)"
		...
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(25)
		col.width = 64
		col.canSum = False
		Set col = lvw.headers(26)
		col.width = 80
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(27)
		col.width = 80
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(28)
		col.width = 64
		col.canSum = False
		Set col = lvw.headers(29)
		col.width = 64
		col.canSum = False
		Set col = lvw.headers(30)
		col.width = 120
		col.dbtype = "str"
		col.formattext = "code:GetCompanyHtml(""@value"",@cells[38],@cells[39])"
		col.canSum = False
		Set col = lvw.headers(31)
		col.width = 90
		col.dbtype = "str"
		col.formattext = "code:GetPersonHtml(""@value"",@cells[40],@cells[41])"
		col.canSum = False
		Set col = lvw.headers(32)
		col.width = 120
		col.dbtype = "str"
		col.setLink2 "关联单据","htord", "htCateid",  5, -1, "htShare"
		col.dbtype = "str"
		col.canSum = False
		...
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(37)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(38)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(39)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(40)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(41)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(42)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(43)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(44)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(45)
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(46)
		col.width = 0
		col.display = "none"
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		...
		lvw.headers(7).canSum = False
		lvw.headers(8).dbtype = "str"
		lvw.headers(8).canSum = False
		lvw.headers(9).title = "派工状态"
		lvw.headers(9).Formula = "GetPGStatusHtml(@value)"
		lvw.headers(10).dbtype = "str"
		lvw.headers(10).canSum = False
		lvw.headers(11).dbtype = "number"
		lvw.headers(12).dbtype = "number"
		lvw.headers(13).dbtype = "number"
		lvw.headers(14).dbtype = "money"
		lvw.headers(15).dbtype = "str"
		lvw.headers(15).canSum = False
		lvw.headers(16).dbtype = "str"
		lvw.headers(16).canSum = False
		lvw.headers(19).Formula = "introToHTML(@value)"
		lvw.headers(19).dbtype = "str"
		lvw.headers(19).canSum = False
		lvw.headers(20).Formula = "JiejianToHtml(@cells[1],0)"
		lvw.headers(23).dbtype = "str"
		lvw.headers(23).canSum = False
		lvw.headers(23).Formula = "introToHTML(@value)"
		lvw.headers(25).dbtype = "str"
		lvw.headers(25).canSum = False
		lvw.headers(26).dbtype = "str"
		lvw.headers(26).canSum = False
		lvw.headers(29).dbtype = "str"
		lvw.headers(29).canSum = False
		lvw.headers(30).dbtype = "str"
		lvw.headers(30).canSum = False
		lvw.headers(31).dbtype = "str"
		lvw.headers(31).canSum = False
		lvw.headers(32).display = "none"
	end sub
	Function GetCheckboxHtml(listid,slord,pgStatus,addcate)
		Dim tempStr : tempStr = ""
		If pgStatus < 2 Then
			If isEmpty(intro_45_6) Then
				intro_45_6 = app.power.GetPowerIntro(45,6)
			end if
			if intro_45_6="" or instr(","& intro_45_6 &"," , ","& addcate &",")>0 Then
				tempStr = "<input type=checkbox class='lvcbox' name='sys_lvw_ckbox' value='" & listid & "'><input type='hidden' id='slord_"& listid &"' name='slord' value='" & slord & "'>"
			end if
		end if
		GetCheckboxHtml = tempStr
	end function
	Function GetSLTitleHtml(slord,slTitle,addcate,titleColor)
		If slord&""<>"" And addcate&""<>"" Then
			If isEmpty(intro_45_14) Then
				intro_45_14 = app.power.GetPowerIntro(45,14)
			end if
			Dim titleStr, tipStr, rs
			titleStr = ""
			if slTitle&"" = "" then slTitle=""
			tipStr = "<span id='tip_"& slord &"' class=red></span>"
			if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
				titleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(slord) & "','newwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"""
'if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
				If titleColor&""<>"" Then
					titleStr = titleStr &" style='color:"& titleColor &";'"
				end if
				titleStr = titleStr &">"& slTitle &"</a>"
			else
				If titleColor&""<>"" Then
					titleStr = "<font color='"& titleColor &"'>"& slTitle &"</font>"
				else
					titleStr = slTitle
				end if
			end if
			titleStr = titleStr & tipStr
			GetSLTitleHtml = titleStr
		end if
	end function
	Function GetSLidHtml(slTitle,addcate,titleColor)
		If addcate&""<>"" Then
			Dim titleStr, rs
			titleStr = ""
			if slTitle&"" = "" then slTitle=""
			...
			GetSLidHtml = titleStr
		end if
	end function
	Function GetProductHtml(title1,cpord,addcate)
		If title1&""<>"" And cpord&""<>"" Then
			Dim titleStr
			titleStr = ""
			If isEmpty(open_21_14) Then
				open_21_14 = app.power.existsPower(21,14)
			end if
			if open_21_14 then
				titleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../product/content.asp?ord=" & app.base64.pwurl(cpord) & "','newTelwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title1 &"</a>"
'if open_21_14 then
			else
				titleStr = title1
			end if
		ElseIf cpord&""="0" Then
			titleStr = "<font color='#ff0000'>产品已被删除</font>"
		end if
		GetProductHtml = titleStr
	end function
	Function GetCompanyHtml(title1,khord,khCateid)
		If khord&""<>"" And khord&""<>"0" Then
			Dim titleStr
			titleStr = ""
			If isEmpty(intro_1_14) Then
				intro_1_14 = app.power.GetPowerIntro(1,14)
			end if
			if intro_1_14="" or (intro_1_1&""<>"0" And instr(","& intro_1_14 &"," , ","& khCateid &",")>0) then
				titleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../work/content.asp?ord=" & app.base64.pwurl(khord) & "','newTelwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title1 &"</a>"
			else
				titleStr = title1
			end if
		ElseIf khord&""="0" Then
			titleStr = "<font color='#ff0000'>客户已被删除</font>"
		end if
		GetCompanyHtml = titleStr
	end function
	Function GetPersonHtml(title1,lxrOrd,lxrCateid)
		Dim titleStr
		titleStr = ""
		If lxrOrd&""<>"" And lxrOrd&""<>"0" Then
			If isEmpty(intro_2_14) Then
				intro_2_14 = app.power.GetPowerIntro(2,14)
			end if
			if intro_2_14="" or instr(","& intro_2_14 &"," , ","& lxrCateid &",")>0 then
				titleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../person/content.asp?ord=" & app.base64.pwurl(lxrOrd) & "','newPerwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title1 &"</a>"
'if intro_2_14="" or instr(","& intro_2_14 &"," , ","& lxrCateid &",")>0 then
			else
				titleStr = title1
			end if
		ElseIf lxrOrd="0" Then
			titleStr = "<font color='#ff0000'>联系人已被删除</font>"
		end if
		GetPersonHtml = titleStr
	end function
	Function GetPGStatusHtml(pgStatus)
		Dim tempStr
		tempStr = ""
		Select Case pgStatus
		Case 0 : tempStr = "未派工"
		Case 1 : tempStr = "部分派工"
		Case 2 : tempStr = "派工完毕"
		End Select
		GetPGStatusHtml = tempStr
	end function
	function GetSLActHTML(slord, addcate)
		dim actStr
		actStr = ""
		if slord&""<>"" then
			If isEmpty(intro_45_2) Then
				intro_45_2 = app.power.GetPowerIntro(45,2)
			end if
			If isEmpty(intro_45_3) Then
				intro_45_3 = app.power.GetPowerIntro(45,3)
			end if
			If isEmpty(intro_45_14) Then
				intro_45_14 = app.power.GetPowerIntro(45,14)
			end if
			actStr = actStr &""
			if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
				actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(slord) & "&ly=list','new1win','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">详情</button>"
'if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
			end if
			If intro_45_2="" or instr(","& intro_45_2 &"," , ","& addcate &",")>0 then
				if actStr&""<>"" then actStr = actStr & "&nbsp;"
				actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:window.open('correctsl.asp?ord=" & app.base64.pwurl(slord) & "','new2win','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">修改</button>"
				if actStr&""<>"" then actStr = actStr & "&nbsp;"
			end if
			If intro_45_3="" or instr(","& intro_45_3 &"," , ","& addcate &",")>0 then
				if actStr&""<>"" then actStr = actStr & "&nbsp;"
				actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:delSLOrders("& slord &",'list')"">删除</button>"
			end if
			actStr = actStr &""
		end if
		GetSLActHTML = actStr
	...
	Sub App_delSLOrders
		if app.power.existsPower(45,3)= false Then
			Response.write "-1|"
'if app.power.existsPower(45,3)= false Then
			Response.end
		end if
		Dim ord, rs, addcate, noDel, noDel1, noDel2, noDelStr, wxNum, id1
		ord = request("ord")
		if ord&""="" then
			ord = "0"
		else
...
'if right(ord,1)="," then
		...
		noDel = ""  : noDel1 = "": noDel2 = "" : noDelStr = ""
		If ord&"" = "" Or ord="0" Then
			Response.write "0|"
			Response.end
		else
			Set rs = cn.execute("select id, addcate from repair_sl where id in("& ord &") and del=1")
			If rs.eof = True Then
				Response.write "3|"
				Response.end
			ElseIf rs.eof = False Then
				...
				While rs.eof = False
					addcate = rs("addcate") : id1 = rs("id")
					If intro_45_3="" or instr(","& intro_45_3 &"," , ","& addcate &",")>0 Then
						wxNum = cn.execute("select count(id) from RepairOrder where del<>7 and repair_sl="& id1)(0)
						If CLng(wxNum)>0 Then
							noDel = noDel & id1 &" "
							noDel2 = noDel2 &"-"&  id1 &" "
							noDel = noDel & id1 &" "
							End  If
				...
				if trim(noDel1 & noDel2)<>"" then
					noDelStr = replace(trim(noDel1 & noDel2)," ",",")
				end if
				if noDel = "0" Then
					cn.execute("update repair_sl set del=2,delcate="& info.User &",deldate=getdate() where id in("& ord &")")
					cn.execute("update repair_sl_list set del=2 where repair_sl in("& ord &")")
					cn.execute("update repair_sl_jian set del=2 where repair_sl in("& ord &")")
					cn.execute("update repair_kulist set del=2 where repair_sl in("& ord &")")
					Response.write("1|")
					Response.end
				else
					cn.execute("update repair_sl set del=2,delcate="& info.User &",deldate=getdate() where id in("& ord &") and id not in("& noDel &")")
					cn.execute("update repair_sl_list set del=2 where repair_sl in("& ord &") and repair_sl not in("& noDel &")")
					cn.execute("update repair_sl_jian set del=2 where repair_sl in("& ord &") and repair_sl not in("& noDel &")")
					cn.execute("update repair_kulist set del=2 where repair_sl in("& ord &") and repair_sl not in("& noDel &")")
					Response.write("2|"& noDelStr)
				end if
			end if
		end sub
		function noImgHTML(str)
			dim re
			Set re=new RegExp
			re.IgnoreCase =True
			re.Global=True
	...
	Function getKzzdyFileds(sort1)
		Dim zdyStr, FName, rs
		zdyStr = "" : FName = ""
		If sort1&"" <> "" Then
			Set rs = cn.execute("select FName from ERP_CustomFields WITH(NOLOCK) where TName='"& sort1 &"' and IsUsing=1 and CanExport=1 and del=1 order by FOrder asc")
			While rs.eof = False
				FName = rs("FName")
				If FName&""<>"" Then
					zdyStr = zdyStr &","& FName
		...
		getKzzdyFileds = zdyStr
	end function
	Function JiejianToHtml(mxid,showlink)
		Dim rs, jiejianStr, JFtype, JFTitle, jianID, arr_img, JFIntro
		jiejianStr = "" : JFtype = "" : JFTitle = ""
		If mxid&""<>"" Then
			Set rs = cn.execute("select a.id,a.title1,a.title2,b.Ftype,a.intro from repair_sl_jian a WITH(NOLOCK) inner join ERP_CustomFields b WITH(NOLOCK) on a.sortid2=b.ID and b.del=1 where a.repair_sl_list="& mxid &" order by a.date7 desc")
			If rs.eof = False Then
				jiejianStr = jiejianStr &"<table border=0 cellpadding=0 cellspacing=0>"
				While rs.eof = False
					JFtype = rs("Ftype") : JFTitle = rs("title1") : JFIntro = rs("intro") : jianID = rs("id")
					If JFIntro&""<>"" Then
						JFIntro = Replace(JFIntro,Chr(13),"<br>")
						arr_img = split(JFIntro,"<img",-1,1)
						JFIntro = Replace(JFIntro,Chr(13),"<br>")
						if ubound(arr_img)>0 Then
							If showlink = 1 Then
								JFIntro = "<div onClick=""window.open('info.asp?ord="& app.base64.pwurl(jianID) &"&sort1=3&sort2=intro','neww6768999in','width=' + 1600 + ',height=' + 800 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=0,top=150');return false;"" onMouseOver=""window.status='none';return true;"" title='放大查看'>" & JFIntro &"</div>"
							else
								JFIntro = noImgHTML(JFIntro)
							end if
						end if
					end if
					jiejianStr = jiejianStr &"<tr>"
					jiejianStr = jiejianStr &"<td class='td1'>"& rs("title2") &": </td>"
					jiejianStr = jiejianStr &"<td class='td2'>"& JFIntro &"</td>"
					jiejianStr = jiejianStr &"</tr>"
					rs.movenext
				wend
				jiejianStr = jiejianStr & "</table>"
			end if
			rs.close
			set rs = nothing
		end if
		JiejianToHtml = "<div class='slJian'>"& jiejianStr &"</div>"
	end function
	...
	Sub App_chkPaigong
		if app.power.existsPower(45,6)= false Then
			Response.write "-1"
'if app.power.existsPower(45,6)= false Then
			Response.end
		end if
...
'if right(id,1)="," then
		...
		If id&"" = "" Or id="0" Then
			...
			Set rs = cn.execute("select top 1 id from repair_sl_list WITH(NOLOCK) where id in("& id &") and del=1")
			If rs.eof = True Then
				Response.write "3"
				Response.end
			ElseIf rs.eof = False Then
