...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "<style>#comm_itembarbg{height:64px!important;}html{background:#EFEFEF;}</style>" & vbcrlf & ""
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		Dim title, gtitle, options, i, ismain, pord, ord
		ismain = app.getint("ismain")
		pord = app.base64.deurl(app.gettext("pord"))
		ord = app.base64.deurl(app.gettext("ord"))
		If isnumeric(pord & "") = False Then
			pord = "0"
		end if
		If isnumeric(ord & "") = False Then
			ord = "0"
		end if
		If ismain = "1" Then
			title = "设置结构类型"
		...
		bill.ahonegp = False
		bill.maxspan = 1
		Dim pTitle, cTitle, rs, gate1
		pTitle = ""
		gate1 = "1"
		If CLng(pord) > 0 Then
			Set rs = cn.execute("select * from Bom_Code where ord = " & pord)
			If rs.bof = False And rs.eof = False Then
				pTitle = Trim(rs("title"))
				gate1 = Trim(rs("gate1"))
			end if
			rs.close
			set rs = nothing
		...
		cTitle = ""
		If CLng(ord) > 0 Then
			Set rs = cn.execute("select * from Bom_Code where ord = " & ord)
			If rs.bof = False And rs.eof = False Then
				cTitle = Trim(rs("title"))
				gate1 = Trim(rs("gate1"))
			end if
			rs.close
			set rs = nothing
		end if
		bill.Groups(0).title = gtitle
		If (ismain = "1" And pord = "0") Or (ismain = "0" And ord = "0") Then
			Call bill.buttons.Insert("增加","Bom_code_add","bill.doSaveAdd(this)",True, True,1)
		end if
		If ismain = "0" Then
			With Bill.fields.add("类型名称", "title")
			.value = pTitle
			.colspan = 3
			.uitype = "html"
			.notnull = True
		...
		If ismain = "0" Then
			With Bill.fields.add("结构编码", "title")
			.defvalue = cTitle
			.value = cTitle
			.colspan = 3
			.uitype = "text"
		...
		For i = 1 To 40
			...
			options = options & i
		next
		With Bill.fields.add("重要程度", "gate1")
		.defvalue = gate1
		.value = gate1
		.colspan = 3
		.uitype = "select"
		.source = options
		End With
		With Bill.fields.add("结构类型ord", "pord")
		.defvalue = pord
		.value = pord
		.colspan = 3
		.uitype = "hidden"
		End With
		With Bill.fields.add("是否是结构类型", "ismain")
		.value = ismain
		.colspan = 3
		.uitype = "hidden"
		End With
	...
	Function Bill_OnSave(ByVal bill)
		Dim title, ismain, pord, ord, gate1, aTitle, rs, continue_add
		title = Replace(Trim(app.gettext("title")),"'","''")
		ismain = app.getint("ismain")
		pord = app.gettext("pord")
		ord = app.gettext("ord")
		If isnumeric(pord & "") = False Then
		...
		gate1 = app.getint("gate1")
		continue_add = app.getint("sys_ad_model")
		If ismain & "" = "1" Then
			If CLng(pord) = 0 Then
				Set rs = cn.execute("select * from bom_code where ismain = 1 and title = '" & title & "'")
				If Not rs.eof Then
					bill.alert("结构类型已存在！")
					Bill_OnSave = True
					Response.end()
				...
				set rs = nothing
				aTitle = "结构类型添加"
				cn.execute("insert into Bom_Code (title,ismain,gate1,addCate,addDate) values ('"& title &"'," & ismain & "," & gate1 & "," & Info.User & ",getDate())")
			Else
				Set rs = cn.execute("select * from bom_code where ismain = 1 and title = '" & title & "' and ord <> " & pord)
				If Not rs.eof Then
					bill.alert("结构类型已存在！")
					Bill_OnSave = True
					Response.end()
				...
				set rs = nothing
				aTitle = "结构类型修改"
				cn.execute("update Bom_Code set title = '" & title & "', ismain =" & ismain & ", gate1=" & gate1 & " where ord = " & pord)
			end if
		else
			If CLng(ord) = 0 Then
				Set rs = cn.execute("select * from bom_code where ismain = 0 and title = '" & title & "'")
				If Not rs.eof Then
					bill.alert("结构编码已存在！")
					Bill_OnSave = True
					Response.end()
				...
				set rs = nothing
				aTitle = "结构编码添加"
				cn.execute("insert into Bom_Code (title,ismain,gate1,addCate,addDate,p_ord) values ('"& title &"'," & ismain & "," & gate1 & "," & Info.User & ",getDate()," & pord & ")")
			Else
				Set rs = cn.execute("select * from bom_code where ismain = 0 and title = '" & title & "' and ord <> " & ord)
				If Not rs.eof Then
					bill.alert("结构编码已存在！")
					Bill_OnSave = True
					Response.end()
				...
				set rs = nothing
				aTitle = "结构编码修改"
				cn.execute("update Bom_Code set title = '" & title & "', ismain =" & ismain & ", gate1=" & gate1 & " where ord = " & ord)
			end if
		end if
		If continue_add = "0" Then
			Call bill.showSaveResult(aTitle & "成功！")
		else
			Call bill.showSaveResultEx(aTitle & "成功！",False,"opener","")
			Response.write("<script type='text/javascript'>parent.window.location.href='?ismain=" & ismain & "&pord=" & app.base64.pwurl(pord) & "&ord=" & app.base64.pwurl(ord) & "'</script>")
		end if
		Bill_OnSave = true
	end function
	Sub App_dodel
		Dim ord:ord = app.getint("ord")
		cn.execute("delete from Bom_Code where ord = " & ord &" or p_ord = " &ord)
		Response.redirect "set_Code.asp"
	end sub
