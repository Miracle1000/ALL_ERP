...
'if W1<>"" then
	...
	if  A2<>"" then
		if Str_Result="" then
			Str_Result="where area in ("&A2&")"
		else
			Str_Result=Str_Result+"and  area in ("&A2&")"
			Str_Result="where area in ("&A2&")"
		end if
	end if
	if D<>"" then
...
'if D<>"" then
	...
	if A1<>"" then
		Str_Result=Str_Result+"and  jz in  ("&A1&")"
'if A1<>"" then
	end if
	if A3<>"" then
		Str_Result=Str_Result+"and  ly in  ("&A3&")"
'if A3<>"" then
	end if
	if E<>"" then
		Str_Result=Str_Result+"and  sort in  ("&E&")"
'if E<>"" then
	end if
	if F<>"" then
...
'elseif G1=6 then
	...
	if H2<>"" then
		if H1=1 then
			str_Result=str_Result+"and fax like '%"& H2 &"%'"
'if H1=1 then
		elseif H1=2 then
			str_Result=str_Result+"and fax not like '%"& H2 &"%'"
'elseif H1=2 then
		elseif H1=3 then
			str_Result=str_Result+"and fax='"& H2 &"'"
'elseif H1=3 then
		elseif H1=4 then
			str_Result=str_Result+"and fax<>'"& H2 &"'"
'elseif H1=4 then
...
'elseif zdy4_1=6 then
	...
	elseif px=3 then
		px_Result="order by name desc"
	elseif px=4 then
		px_Result="order by name asc"
	elseif px=5 then
		px_Result="order by khid desc"
	elseif px=6 then
		px_Result="order by khid asc"
	elseif px=7 then
		px_Result="order by jz desc"
	elseif px=8 then
		px_Result="order by jz asc"
	elseif px=9 then
		px_Result="order by datealt desc"
	elseif px=10 then
		px_Result="order by datealt asc"
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from tel  "&Str_Result&"  "&px_Result&" "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i
	i=1
	do until rs.eof
		dim name,khid,sort1,sort2,jz,ly,area,trade,preson,sex,part1,job,phone,fax,mobile,email,url,address,zip,product,c2,c3,c4,intro,intro2,cateid,date7
		name=rs("name")
		khid=rs("khid")
		cateid=rs("cateid")
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select isnull(sum(jf),0) as sum_jf from teljf where company="&rs("ord")&""
		...
		set rs7=nothing
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select isnull(sum(num1),0) as sum_jf2 from teljf2 where company="&rs("ord")&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			sum_jf2=0
		else
		...
		date7=rs("date1")
		xlWorksheet.Cells(1+i,1).Value = name
		date7=rs("date1")
		...
		date7=rs("date1")
		xlWorksheet.Cells(1+i,3).Value = sum_jf
		date7=rs("date1")
		xlWorksheet.Cells(1+i,3).font.Size=10
		date7=rs("date1")
		xlWorksheet.Cells(1+i,4).Value = rs("jf")
		date7=rs("date1")
		xlWorksheet.Cells(1+i,4).font.Size=10
		date7=rs("date1")
		xlWorksheet.Cells(1+i,5).Value = sum_jf2
		date7=rs("date1")
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户积分资料导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户积分资料导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("客户积分_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的客户积分资料</u></strong></font></a></p> " & vbcrlf & ""
