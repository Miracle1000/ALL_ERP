...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select * from sortMessage order by order1 desc"
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do while not rs1.eof
			Response.write("<a href='###'  onclick='getMobanList("&rs1("ord")&")' style='color=#217DDA'>【"&rs1("sortName")&"】</a> ")
			rs1.movenext
		loop
		call db_close : Response.end
	else
		Response.write("暂无模板!请先<a href='correctSort.asp' target='_blank' style='color:#ff0000' title='点击添加'>添加模板</a>分类")
		call db_close : Response.end
	end if
	rs1.close
	set rs1=nothing
