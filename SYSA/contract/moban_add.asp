...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "span.tdSpan input{margin-bottom:-3px;}" & vbcrlf & "#content2 td{text-align:left;}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body onunload=""if(window.opener) window.opener.location.reload();"">" & vbcrlf & ""
	dim sort,main,id
	sort=request("sort")
	main=request("main")
	mbTitle = ""
	...
	Case 3 : mbTitle = "采购"
	Case 4 : mbTitle = "发货"
	Case 18001 : mbTitle = "售后"
	Case 6 : mbTitle = "入库"
	Case 61 : mbTitle = "入库条码(简)"
	Case 62 : mbTitle = "入库条码(详)"
	Case 7 : mbTitle = "信封"
	Case 8 : mbTitle = "采购退货"
	Case 16 : mbTitle = "开票"
	Case 17, 43003 : sort = 43003 : mbTitle = "回款"
	Case 44009 : mbTitle = "付款计划"
	Case 18 : mbTitle = "付款"
	Case 19 : mbTitle = "费用使用"
	Case 20 : mbTitle = "费用返还"
	Case 21 : mbTitle = "费用报销"
	Case 22 : mbTitle = "费用借款"
	Case 23 : mbTitle = "费用申请"
	Case 26 : mbTitle = "固定资产卡片"
	Case 28 : mbTitle = "质检"
	Case 9 : mbTitle = "项目"
	Case 10 : mbTitle = "出库"
	Case 11 : mbTitle = "二维码"
	...
	Case 45 : mbTitle = "受理单"
	Case 46 : mbTitle = "维修单"
	Case 101: mbTitle = "车辆申请"
	Case 102: mbTitle = "询价"
	Case 126: mbTitle = "供应商"
	Case 2003: mbTitle = "产品"
	End Select
	set rs=server.CreateObject("adodb.recordset")
	sql="select A1,countnum from contract_out Where main="&main&" and sort="&sort&" and del=1"
	rs.open sql,conn,1,1
	if rs.eof then
		countnum=0
	else
...
'for i = 2 to 6
		...
		if isnull(countnum) then countnum=0
	end if
	rs.close
	set rs=nothing
	action1="模板添加"
	...
	Response.write mbTitle
	Response.write "模板添加</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right""><input type=""button"" name=""Submit422"" value=""模板说明""  onclick=""window.location.href='../contract/打印模板参数对应表.doc'"" class=""anniu""/></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table> " & vbcrlf & "<form method=""POST"" action=""moban_save.asp?sort="
	Response.write sort
	Response.write """ id=""demo"" onsubmit=""return Validator.Validate(this,2)&&DelUnusedFilesBeforeSubmit()"" name=""date"" style=""margin:0"" target=""savedata"">" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content2"">" & vbcrlf & "     <tr class=""top noBefore"">" & vbcrlf & "       <td width=""55%"" height=""58px""><span class=""name""><strong >模板名称：</strong>" & vbcrlf & "             "
	If sort = 46 Then
		Response.write "" & vbcrlf & "           <input name=""title"" type=""text""  size=""30"" dataType=""Limit"" min=""1"" max=""200""  msg=""长度在1至200个字之间"">" & vbcrlf & "           "
	else
		Response.write "" & vbcrlf & "           <input name=""title"" type=""text""  size=""30"" dataType=""Limit"" min=""1"" max=""50""  msg=""长度在1至50个字之间"">" & vbcrlf & "           "
	end if
	Response.write "" & vbcrlf & "           <span class=""red"">*           </span></span></td>" & vbcrlf & "    <td width=""33%"" height=""37"" class=""name"">" & vbcrlf & "    "
	If sort <> 46 Then
	...
	Response.write "</td>" & vbcrlf & "           <td width=""45%"">&nbsp;<select name=""main"">" & vbcrlf & "           <option value=""7"">主模板</option>" & vbcrlf & "           <option value=""2"">副模板</option>" & vbcrlf & "         </select></td>" & vbcrlf & "     </tr>" & vbcrlf & "  <tr>" & vbcrlf & "            <td valign=""middle"" style='height:40px;'>纸张类型：<select name=""pageType"" id=""pageType"" onchange=""changePageType()"" style=""margin-bottom:-3px;"">" & vbcrlf & "                      <option value="""">请选择</option>" & vbcrlf & "                  <option value=""A4"" "
	'Response.write """>"
	...
	Response.write ">自定义</option>" & vbcrlf & "                </select>" & vbcrlf & "               <span style=""margin-left:20px;"" class=""tdSpan"">纸张大小： " & vbcrlf & "                       宽：<input type=""text"" name=""pageWidth"" id=""pageWidth"" size=""4"" value="""
	if pageType&""="zdy" then Response.write "selected pageWidth='"& pageWidth &"' pageHeight='"& pageHeight &"'"
	Response.write pageWidth
	Response.write """ onpropertychange=""formatData(this,'number2');"" onKeyUp=""checkDot('pageWidth','2');changePageSize();"">厘米</span>" & vbcrlf & "                    <span style=""margin-left:8px;"" class=""tdSpan"">高：<input type=""text"" name=""pageHeight"" id=""pageHeight"" size=""4"" value="""
	Response.write pageWidth
	Response.write pageHeight
	Response.write """ onpropertychange=""formatData(this,'number2');"" onKeyUp=""checkDot('pageHeight','2');changePageSize();"">厘米</span>" & vbcrlf & "           </td>" & vbcrlf & "           <td colspan=""2""  valign=""middle"" style='height:40px;'>" & vbcrlf & "                      页边距：<span class=""tdSpan"">上：<input type=""text"" name=""topMargin"" id=""topMargin"" size=""4"" value="""
	Response.write topMargin
	Response.write """ onpropertychange=""formatData(this,'number2');"" onKeyUp=""checkDot('topMargin','2');changePageSize();"">厘米</span>" & vbcrlf & "                    <span style=""margin-left:8px;"" class=""tdSpan"">下：<input type=""text"" name=""bottomMargin"" id=""bottomMargin"" size=""4"" value="""
	Response.write topMargin
	Response.write bottomMargin
	Response.write """ onpropertychange=""formatData(this,'number2');"" onKeyUp=""checkDot('bottomMargin','2');changePageSize();"">厘米</span>" & vbcrlf & "                 <span style=""margin-left:8px;"" class=""tdSpan"">左：<input type=""text"" name=""leftMargin"" id=""leftMargin"" size=""4"" value="""
	Response.write bottomMargin
	Response.write leftMargin
	Response.write """ onpropertychange=""formatData(this,'number2');"" onKeyUp=""checkDot('leftMargin','2');changePageSize();"">厘米</span>" & vbcrlf & "                   <span style=""margin-left:8px;"" class=""tdSpan"">右：<input type=""text"" name=""rightMargin"" id=""rightMargin"" size=""4"" value="""
	Response.write leftMargin
	...
	if sort=22 Or sort=1 or sort=7 or sort=9 Or sort=16 or sort=45 or sort=126 then
		Response.write "" & vbcrlf & "      <tr><td height=""30"" colspan=""3""><strong>以下为自定义字段模板内容</strong></td></tr>" & vbcrlf & "         <tr>" & vbcrlf & "           <td colspan=""3"">" & vbcrlf & "                   "
		If sort=22 Then
		...
		Response.write "" & vbcrlf & "             </td>" & vbcrlf & "    </tr>" & vbcrlf & "   "
	ElseIf sort=28 Then
		Response.write "" & vbcrlf & "      <tr><td height=""30"" colspan=""3""><strong>以下为自定义字段模板内容</strong></td></tr>" & vbcrlf & "         <tr>" & vbcrlf & "           <td colspan=""3"">基本信息自定义字段：<br>" & vbcrlf & "                   "
		Response.write dyExtended(28,3)
		Response.write "" & vbcrlf & "                      <br><br>明细质检方案自定义字段：<br>" & vbcrlf & "                    "
	...
	call close_list(1)
	Response.write "" & vbcrlf & "      <tr>" & vbcrlf & "        <td colspan=""3""  valign=""middle"" style='height:40px;'>" & vbcrlf & "            <strong>以下为模板内容，直接修改然后点击“保存”即可</strong></td>" & vbcrlf & "        </tr>" & vbcrlf & "              <tr>" & vbcrlf & "        <td height=""30"" colspan=""3"">" & vbcrlf & "                    <div align=""center"">" & vbcrlf & "                        <div id=""mbPage"" style=""margin:0 auto;border:solid #555 1px; background:#efefef; width:"
	...
	Response.write "</textarea>" & vbcrlf & "                          <IFRAME ID=""eWebEditor1"" SRC=""../moban/ewebeditor.asp?id=content&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""100%"" marginwidth=""0"" marginheight=""0"" name=""wfasdg""></IFRAME>" & vbcrlf & "                           </div>" & vbcrlf & "                  </div>" & vbcrlf & "                      </div>" & vbcrlf & "            </td>" & vbcrlf & "        </tr> " & vbcrlf & "      <tr>" & vbcrlf & "        <td height=""60"" colspan=""3"" class=""gray"">" & vbcrlf & "            <div align=""center"">" & vbcrlf & "                <input type=""submit"" name=""Submit42""  autocomplete=""off"" value=""保存""  class=""page""/>　　　　　　          " & vbcrlf & "            <input type=""reset"" value=""重填""  autocomplete=""off"" class=""page"" name=""B2""></div></td>" & vbcrlf & "        </tr>" & vbcrlf & "    </table>" & vbcrlf & "       </form> " & vbcrlf & "      </td>" & vbcrlf & "  </tr>" & vbcrlf & " <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "<iframe id=""savedata"" name=""savedata"" style=""display:none;height:0px;width:0px""></frame>" & vbcrlf & "</body>" & vbcrlf & "</html>"
