...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	if open_67_10=3 then
		list=""
	elseif open_67_10=1 then
		list="and addcate in ("&intro_67_10&")"
	else
		list="and addcate=0"
	end if
...
'if m1<>"" then
	...
	if m2<>"" then
		Str_Result=Str_Result+"and addtime<='"&cdate(m2)&"' "
'if m2<>"" then
	end if
...
'If A3 = "0" Then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from logMessage "&str_Result&" order by addtime desc "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i
	i=1
	xlApplication.Init "短信统计汇总表_"&session("name2006chen")&".xls",C1
	do until rs.eof
		mobile=rs("phone")
		content=rs("content")
		stact=rs("stact1")
		...
		if person<>"" then
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select ord, name from person  where ord="&person&" "
			rs1.open sql1,conn,1,1
			if rs1.eof  then
				personName=""
				personID=""
		...
		if rs("addcate")<>"" then
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select name from gate where ord="&rs("addcate")&""
			rs1.open sql1,conn,1,1
			dim cateid
			if rs1.eof then
				cateid=""
		...
		if stact<>"" then
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select errMgs from errMessage  where errNum='"&stact&"'"
			rs1.open sql1,conn,1,1
			if rs1.eof  then
				stact=""
			else
		...
		set rs1=nothing
		xlWorksheet.Cells(1+i,2).Value = personName
		set rs1=nothing
		xlWorksheet.Cells(1+i,2).font.Size=10
		set rs1=nothing
		xlWorksheet.Cells(1+i,3).Value = content
		set rs1=nothing
		xlWorksheet.Cells(1+i,3).font.Size=10
		set rs1=nothing
		xlWorksheet.Cells(1+i,4).Value = stact
		set rs1=nothing
		xlWorksheet.Cells(1+i,4).font.Size=10
		set rs1=nothing
		xlWorksheet.Cells(1+i,5).Value = addtime
		set rs1=nothing
		xlWorksheet.Cells(1+i,5).font.Size=10
		set rs1=nothing
		xlWorksheet.Cells(1+i,6).Value = cateid
		set rs1=nothing
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>短信统计汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!短信统计汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("短信统计汇总表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的短信统计汇总表</u></strong></font></a></p> " & vbcrlf & ""
