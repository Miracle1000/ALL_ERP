...
'if isnull(values) then IsTime = null : exit function
	...
	elseif open_103_1=1 then
		Str_Result=" where 1=1 and chk_addcateid in ("&intro_103_1&")"
	else
		Str_Result=" where 1=1 and chk_addcateid=0"
	end if
	...
	if D2<>"" then
		Str_Result=Str_Result&" and bk_Checknum in ("&D2&")"
	end if
	if D3<>"" then
	...
	if M<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',Ld_time)>=0 "
'if M<>"" then
	end if
	if K<>"" then
		Str_Result=Str_Result+" and  datediff(s,Ld_time,'"&ret2&" 23:59:59')>=0 "
'if K<>"" then
	end if
	if L1<>"" then
...
'if ret<>"" then
	...
	if C<>"" then
		if B="tl" then
			Str_Result=Str_Result&" and Chk_title like '%"&ml.De(C)&"%'"
		elseif B="nt" then
			Str_Result=Str_Result&" and Chk_note like '%"&ml.De(C)&"%'"
		end if
	end if''
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_Check.asp?ret="
	Response.write ret
	Response.write "&ret2="
	Response.write ret2
	Response.write "&B="
	...
	Response.write "&C="
	Response.write C
	Response.write """ method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">盘点列表</td>" & vbcrlf & "                       <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">    "   & vbcrlf &           "      <tr>" & vbcrlf &          "     <td colspan=""6"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding:0!important"">" & "                <span id=""kh""><input type=""radio"" onClick=""Goto(1)"" checked=""checked"">盘点列表<input type=""radio"" onClick=""Goto(2)"">盘点明细表&nbsp;自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" name=""ret"" readonly value="""
	Response.write ret
	...
	Response.write ret2
	Response.write """/><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;<select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                             <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""tl"" "
	Response.write ret2
	if B="tl" then Response.write "selected"
	Response.write ">盘点主题</option>" & vbcrlf & "                           <option value=""nt"" "
	if B="nt" then Response.write "selected"
	Response.write ">盘点备注</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	...
	if open_103_10=3 or (open_103_10=1 and CheckPurview(intro_103_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_CheckBook.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""></a></span></td>" & vbcrlf & "              </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "            <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                <td width=""20%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点主题</div></td>" & vbcrlf & "                <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点时间</div></td>" & vbcrlf &             "     <td width=""16%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点人员</div></td> "& vbcrlf &              "   <td width=""24%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点备注</div></td>" & vbcrlf & "              <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                      <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.name from O_ChkBook a left join gate b on a.Chk_cateid=b.ord "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=6><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		...
		do while not rs7.eof
			...
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""20%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				...
				'Response.write rs7("id")
				Response.write rs7("Chk_title")
				Response.write "</a>" & vbcrlf & "         "
			else
				Response.write rs7("Chk_title")
			end if
			Response.write "</td>" & vbcrlf & "                   <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("Chk_time")
			Response.write "</div></td>" & vbcrlf & "             <td width=""16%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("name")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name ewebeditorImg""><div align=""center"">"
			Response.write rs7("Chk_note")
			...
			if (open_103_2=3 or CheckPurview(intro_103_2,trim(addcateid))=True) then
				if rs7("Chk_del")=2 then
					Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.location=('Update_Check1.asp?ord="
					Response.write rs7("id")
					Response.write "')"" value=""盘点""/>" & vbcrlf & "                  "
				end if
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_Check.asp?ord="
				Response.write rs7("id")
			...
			if open_103_3=3 or CheckPurview(intro_103_3,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_Check.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	action1="盘点列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_Check.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action=""Del_Check.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').method=""post""" & vbcrlf & "     document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & "         document.getElementById('data').action=""Del_Check.asp"";" & vbcrlf & "   document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall')"  & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf &" }" & vbcrlf &" }" & vbcrlf & "function Goto(args)" & vbcrlf &" {" & vbcrlf &"    if(args==1)" & vbcrlf &  "    {" & vbcrlf &           "     window.location=""List_check.asp""; "& vbcrlf & "} "& vbcrlf &    "    else "& vbcrlf &  "   {" & vbcrlf & "           window.location=""List_checkmx.asp"";" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
