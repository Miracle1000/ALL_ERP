...
'if m2<>"" then
	...
	If propm="1" Then
		tq72 = 0
		Set rs = conn.execute("select isnull(tq1,0) tq1 from setjm where ord = 72")
		If rs.eof = False Then
			tq72 = rs("tq1")
		end if
		rs.close
	...
	If open_94_10=1 Then
		Str_Result = Str_Result + " and b.cateid in ("& intro_94_10 &")"
'If open_94_10=1 Then
	ElseIf open_94_10=0 Then
...
'ElseIf open_94_10=0 Then
	...
	if px=1 then
		px_Result="order by b.indate desc,b.id asc"
	elseif px=2 then
		px_Result="order by b.indate asc,b.id asc"
	elseif px=3 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select b.title,b.company,t.cateid as khcateid,b.cateid,b.id,t.name,convert(varchar(20),b.ybackTime,120) ybackTime,convert(varchar(20),b.servertime,120) servertime,f.title as procs,l.title as projd,(case when b.isback=0 then '未回访' else '已回访' end) as status,r.sort1 as types,(select top 1 name from gate where ord=b.cateid) as username,e.name as personname,b.manyi from Sale_CallBack b left join tel t on t.ord=b.company  left join hf_config f on f.id=b.procsid left join hf_configlist l on l.id=b.projdid left join  sortonehy r on r.gate2=56 and r.ord=b.typeid left join person e on e.ord=b.person_kh where b.del=1 " & Str_Result &" "& px_Result&" "
	rs.open sql,conn,3,1
	dim i
	i=1
	C1=rs.recordcount
	xlApplication.Init "客户回访记录_"&session("name2006chen")&".xls",C1
	If Not rs.eof Then
		do until rs.eof
			manyi=Rs("manyi")
			If manyi&""="" Then
				...
				manyi=Replace(Formatnumber(manyi&"",num1_dot,-1),",","")&"%"
				manyi=""
			end if
			...
			If servertime&""="" Then
				...
				servertime=CStr(servertime)
			end if
			ybackTime=Rs("ybackTime")
			If ybackTime&""="" Then
				ybackTime=""
			else
				ybackTime=CStr(ybackTime)
			end if
			...
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,3).Value = ybackTime
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,3).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,4).Value = servertime
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,4).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,5).Value = Rs("procs")
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,5).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,6).Value = Rs("projd")
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,6).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,7).Value = Rs("status")
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,7).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,8).Value = Rs("types")
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,8).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,9).Value = manyi
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,9).font.Size=10
			'ybackTime=CStr(ybackTime)
			xlWorksheet.Cells(1+i,10).Value = Rs("username")
			'ybackTime=CStr(ybackTime)
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户回访记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户回访记录导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("客户回访记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的客户回访记录</u></strong></font></a></p>"
