...
'Public Function AddFormValue(name,  value)
	...
	dotstr_num=""
		for i=1 to num1_dot
			...
			xApp.init Me, conn
			Set xsheet = xApp.sheets.add("调拨单")
			headerStr = "调拨主题,调拨编号,调拨数量,调拨状态,申请人,申请日期,审批人,审批日期,出货库管,出货日期,进货库管,进货日期"
			set rs = conn.execute("select title,name,sorce  from zdymx where sort1=1003 and set_open=1 order by gate1 asc")
			If Not rs.eof Then
				i=9
				While Not rs.eof
					sorce=rs("sorce")
					if sorce = 5 Then
						headerStr = headerStr & "," & rs("title") & ",审批数量,出库数量,出货仓库,进货仓库,批号,序列号,生产日期,有效日期,包装,件数"
					else
						headerStr = headerStr & "," & rs("title")
						if sorce = 4 and isOpenProductAttr then
...
'if m2<>"" then
			...
			elseif px=13 then
				px_Result="order by datein desc,ord desc"
			elseif px=14 then
				px_Result="order by datein,ord asc"
			end if
			set rs1=server.CreateObject("adodb.recordset")
			...
			rs1.close
			set rs1=nothing
			if open_36_10=3 then
				list=""
			elseif open_36_10=1 then
				list="and cateid in ("&intro_36_10&")"
			else
				list="and cateid=0"
			end if
...
'Dim rsmxcount2, rsmxname2, rsmxsort2
			...
			rsmx.open "select name,sorce from zdymx where sort1=1003 and set_open=1 order by gate1 asc" , conn, 3, 1
...
'While rsmx.eof = False
			...
			rsmx.close
			set rs=server.CreateObject("adodb.recordset")
			sql="select ord,title,dbbh,catein,cateid,complete1,catein,(select isnull(sum(num1),0) from kumovelist where kumove=a.ord) as num2," &_
			"(select top 1 name from gate where ord=a.cateout) as cateidname," & _
			"(select top 1 name from gate where ord=a.catein) as cateinname," & _
			"(select top 1 name from gate where ord=a.cateid) as catenamesq," & _
			"(select top 1 name from gate where ord=a.kg) as catenamesp," & _
			"CONVERT(VARCHAR(20),date3,111) date3," & _
			"CONVERT(VARCHAR(20),date5,111) date5," & _
			"CONVERT(VARCHAR(20),datein,120) datein," & _
			"CONVERT(VARCHAR(20),dateout,120) dateout" & _
			" from kumove a "&Str_Result&" "&px_Result&""
			...
			do until rs.eof
				...
				elseif complete=3 then
					zt="已调入，调拨完毕"
				elseif complete=4 then
					zt="审批已通过"
				elseif complete=5 then
					zt="已调出，在途中"
				end if
				datesq=rs("date3")
				date3=rs("date3")
				...
				cateinname=rs("cateinname")&""
				catenamesq=rs("catenamesq")&""
				catenamesp=rs("catenamesp")&""
				datesp=rs("date5")
				dateout=rs("dateout")
				xsheet.writestr title
				xsheet.writestr  ckid
				xsheet.writenumber  num2
				xsheet.writecell zt
				xsheet.writestr catenamesq
				xsheet.writecell datesq
				xsheet.writestr catenamesp
				xsheet.writecell datesp
				xsheet.writestr cateidname
				xsheet.writecell dateout
				xsheet.writestr cateinname
				xsheet.writecell datein
				i = 0
				sql1 = "select a.id,c.ph,case when isnull(sr.id,0)=0 then c.xlh else nl.serinum end xlh,a.datesc,a.dateyx,a.js,a.price1,a.zdy1,a.zdy2,a.zdy3,a.zdy4,"&_
				" a.num1,a.num2 ,a.del,"&_
				" (case isnull(a.num2,-1) when -1 then 1 when -2 then 2 else 0 end) as nums,case when isnull(sr.id,0)=0 then c.num1 else 1 end as bnum1," & _
				" a.num1,a.num2 ,a.del,"&_
				...
				"(select top 1 sort1 from sortck where ord=a.ck1) as ck1name," &_
				"(select top 1 sort1 from sortck where ord=a.ck2) as ck2name," &_
				"(select top 1 sort1 from sortonehy  where ord=a.zdy5) as zdy5name," &_
				"(select top 1 sort1 from sortonehy  where ord=a.zdy6) as zdy6name," &_
				"isnull(b.title,'产品已被删除') as ptitle, b.order1,b.type1  , s1.title ProductAttr1Name , s2.title ProductAttr2Name "&_
				" from (select * from kumovelist where kumove="&id&") a "&_
				" left join product b on a.ord=b.ord "&_
				" left join kuoutlist2 c on c.sort1=7 and c.JoinDBListID=a.id and  c.del=1 "&_
				"  left join S2_SerialNumberRelation sr with(nolock)  on sr.Billtype = 62001 and sr.listid = c.id "&_
				"  left join M2_SerialNumberList nl with(nolock) on nl.id = sr.serialID "&_
				" left join Shop_GoodsAttr s1 on s1.id = isnull(c.ProductAttr1,a.ProductAttr1) "&_
				" left join Shop_GoodsAttr s2 on s2.id = isnull(c.ProductAttr2,a.ProductAttr2) "&_
				" order by a.id asc"
				...
				if rs1.eof then
					...
					do until rs1.eof
						...
						ck2name=rs1("ck2name")
						del=rs1("del")
						ProductAttr1Name = rs1("ProductAttr1Name")
						ProductAttr2Name = rs1("ProductAttr2Name")
						nums=rs1("nums")&""
						If nums="1" Then
							num2=num1
							If del="3" Then
						...
						i=i+1
						xsheet.movenext
						xsheet.writespace 11
						xsheet.writecell "产品明细"
						For iii = 0 To rsmxcount2
							sorce=rsmxsort2(iii)
							Select Case sorce
							...
							if isOpenProductAttr then
								xsheet.writecell ProductAttr1Name
								xsheet.writecell ProductAttr2Name
							end if
							Case 5 :
							xsheet.writenumber num2
							xsheet.writenumber num1
							xsheet.writenumber bnum1
							xsheet.writecell ck1name
							xsheet.writecell ck2name
							xsheet.writestr ph
							xsheet.writestr xlh
							...
							xsheet.writecell js
							Case 6 : xsheet.writecell intro
							Case 7 : xsheet.writecell zdy1
							Case 8 : xsheet.writecell zdy2
							Case 9 : xsheet.writecell zdy3
							Case 10 : xsheet.writecell zdy4
							Case 11 :
							xsheet.writecell rs1("zdy5name")
							ii=ii+1
							xsheet.writecell rs1("zdy5name")
							Case 12 :
							xsheet.writecell rs1("zdy6name")
							ii=ii+1
							xsheet.writecell rs1("zdy6name")
							End Select
						next
						...
						If rowIndex > 40000 Then
							If pageCount = 1 then
								xsheet.title = "调拨单(" & pageCount & "页)"
							end if
							pageCount = pageCount + 1
							xsheet.title = "调拨单(" & pageCount & "页)"
							Set xsheet = xApp.sheets.add("调拨单(" & pageCount & "页)")
							rowIndex = 0
							xsheet.showheader headerStr
							xsheet.movenext
						end if
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>调拨单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!调拨单导出成功,共有"&(j-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			tfile=Server.MapPath(xApp.cFileName("调拨单") & ".xls")
			xApp.save tfile
			xApp.dispose
			tfile = xApp.HexEncode(tfile)
			Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
			Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'调拨单_"&session("name2006chen")&".xls',fileCnt:1})</script>"
			Set xApp = Nothing
			action1="调拨单导出"
			call close_list(1)
			Response.write "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的调拨单</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
