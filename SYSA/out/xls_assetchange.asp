...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_105_10=1 then
		Str_Result=Str_Result&" and H_addcateid in ("&intro_105_10&")"
	else
		Str_Result=Str_Result&" and H_addcateid=0"
	end if
...
'if Exit Sub2<>"" then
	...
	i=1
	k=0
	sql="select a.*,b.ass_name,c.* from O_assChange a left join O_asset b on a.H_assID=b.ass_id left join O_assChangeWay c on a.H_type=c.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "变动列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			H_cateid_sp=ml.IsNum(rs("H_cateid_sp"))
			sql8="select * from gate where ord="&H_cateid_sp
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				H_cateid_sp=rs8("name")
			else
				H_cateid_sp=""
			end if
			rs8.close
			set rs8=nothing
			sql7="select * from O_assSetFont where id="&ml.IsNum(rs("W_pz"))
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>变动列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!变动列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("变动列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的变动列表</u></strong></font></a></p>"
