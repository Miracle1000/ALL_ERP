...
function chtotals(id,types){
    ...
    for (var i = 1; i < 1000; i++) {
        ...
        if (document.getElementById("moneyall_" + i)) {
			...
			{
			    document.getElementById("moneysum_all").value = parseFloat(allmoneys + parseFloat(document.getElementById("moneyall_" + i).value)).toFixed(window.sysConfig.moneynumber);
			}
        }
    }
    xmlHttp.abort();
