...
'If mode = "select" Then
	...
	Sub onReportInit(rpt, lt)
		Dim dat, rs, zdy,SC_level,i ,company0rd,contractOrd , isinvoiced ,dateStr , invoicedate1,invoicedate2
		rpt.buttonsHTML = rpt.buttonsHTML & "<link href='../inc/showLoading.css' rel='stylesheet' type='text/css'/>" & vbcrlf &_
		"<link href='../skin/default/css/wxchat.css' rel='stylesheet' type='text/css'/>" & vbcrlf &_
		"<script src='../inc/jquery.showLoading.min.js'></script>" & vbcrlf &_
		"<script src='../inc/AjaxLoading.js'></script>" & vbcrlf &_
		"<script src='../skin/default/js/Mg_MUserList.js'></script>" & vbcrlf &_
		"<script src='../skin/default/js/wxchat.js'></script>" & vbcrlf &_
		"<style>.lvw_cell table{width:100%}#lvw_ac_ptb_mlistvw{background:none!important}.lvwframe2{width:100%}</style>"
		khname=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=1 ")(0)
		fw1 = 0
		Set rs= conn.execute("select fw1 from setjm where intro='1' and ord=157 and cateid = "&info.user)
		If rs.eof=False Then
			fw1 = rs(0).value
		end if
		rs.close
		...
		Dim helper,remindSql
		If remind > 0 Then
			Set helper = CreateReminderHelper(cn,remind,0)
			remindSql = Replace(helper.listSql("ids_withOutOrderBy"),"'","''")
		end if
		rpt.title = "微信用户列表"
		...
		on error resume next
		Dim helperMsg : Set helperMsg = CreateHelper(cn,1 , "muserlist")
		dim str
		if err.number=0 then
			str=helperMsg.getAuthorizeUser
		else
			isError = true
			str = err.Description
		end if
		on error goto 0
		rpt.addfield "","radio","datetype","select '关注时间',0 union all select '添加时间',1" ,"", "1"
		rpt.addfield "从&nbsp;","dates","date1","","", Split( dateadd("yyyy",-1,date)& "," & date,",")
		rpt.addfield "","radio","datetype","select '关注时间',0 union all select '添加时间',1" ,"", "1"
		Set rs = cn.execute("select name,id from MMsg_Group order by id")
		Dim options
		If rs.eof Then
			options = ""
		else
			options = rs.getString(,,Chr(1),Chr(2),"")
			options = Left(options,Len(options)-1)
			options = rs.getString(,,Chr(1),Chr(2),"")
		end if
		rpt.addfield "","select","groupid",options,"",""
		rpt.SetNullMsg 2,"所属小组"
		rpt.addfield "昵称：","text","nickname","","",""
		rpt.aSearch.addField "领用人员", "gates","ad_cateid",""
		rpt.aSearch.addField "昵称","selectys","ad_nickname",""
		rpt.aSearch.addField "性别","checks", "ad_sex","select 0 value,'未知' name union all select 1,'男' as name union all select 2, '女' "
		rpt.aSearch.addField "所属小组","checks","ad_groupid","select id value,name from MMsg_Group order by id"
		rpt.aSearch.addField "状态","checks","ad_status","select 1 value,'未转' name union all select 2,'已转' as name union all select 3, '废止' "
		rpt.aSearch.addField "添加时间","datetime","ad_createtime",""
		rpt.aSearch.addField "最新关注时间","datetime","ad_subscribe_time",""
		rpt.aSearch.addField "关注取消时间","datetime","ad_unsubscribe_time",""
		rpt.aSearch.addField khname,"selectys","ad_khname",""
		rpt.aSearch.addField "最新跟进","selectys","ad_reply",""
		rpt.sql = " exec erp_MMsgUser_List "& Info.User&",@datetype,@date1_0,@date1_1,@groupid,@nickname,@ad_cateid,@ad_nickname_0,@ad_nickname_1,@ad_sex," &_
		"@ad_groupid,@ad_status,@ad_createtime_0,@ad_createtime_1,@ad_subscribe_time_0,@ad_subscribe_time_1,@ad_unsubscribe_time_0,@ad_unsubscribe_time_1,"&_
		"@ad_khname_0,@ad_khname_1,@ad_reply_0,@ad_reply_1," & ord & ",'" & remindSql & "'," &_
		"@tp,@dt,@dtBegin,@dtEnd,@st,@tpx,@qc,@dtUnit,@m1,@m2,@fromTj," & _
		"&SearchMode,&excelmode,&ReportSort,&pagesize,&pageindex "
		rpt.addsort "添加时间","1"
		rpt.addsort "关注时间","2"
		rpt.addsort "关注取消时间","3"
		rpt.canexcel = app.power.existsPower(108,10)
		rpt.canprint = app.power.existsPower(108,7)
		rpt.cancolset = true
		rpt.onlyadSearchModel = True
		if isError=false then
			rpt.beforeBtnsHTML = "<div id='helpStr' style='display:none;'>"&str&"</div>&nbsp;<button class='oldbutton3' onclick='javascript:showHelpExplan()'>温馨提示</button>&nbsp;<button class='oldbutton3' onclick='javascript:loadFans();'>加载粉丝</button>"
		else
			rpt.beforeBtnsHTML = "<span style='color:red'>" &str &"</span>"
		end if
	end sub
	dim open_7_2, intro_7_2, open_7_3, intro_7_3, open_7_14,intro_7_1, intro_7_14, open_7_16, intro_7_16, open_7_20, intro_7_20,open_7_22, intro_7_22
	dim power_7_3, power_7_6,intro_1_1,intro_1_14,power_7_21,intro_5_1,intro_5_14,intro_7_21,fw1
...
'Dim remind : remind = app.getInt("remind")
		...
		lvw.CurrSum=False
		lvw.allSum=False
		Dim hiddenCols : hiddenCols = Split("stat,person,cateid,telcateid,subscribeStat,sendOrReceive,canReply,msgAccId,msgUserId,msgCreateTime,MsgType,Content,PicUrl,MediaId,MediaPath,Format,Recognition,ThumbMediaId,Location_X,Location_Y,Scale,Label,Title,Description,msgUrl,tMsgId,msgCateid",",")
		For i = 0 To ubound(hiddenCols)
			With lvw.headers(hiddenCols(i))
			.display = "none"
			End With
		next
		With lvw.headers("id")
		.title = "选择"
		.width = "30"
		.formula = "showCheckbox(@value,@cells[""cateid""])"
		End With
		With lvw.headers("头像")
		.title = "头像"
		.width = "50"
		.formula = "showHeadImg(@value)"
		End With
		With lvw.headers("昵称")
		.width = "80"
		End With
		With lvw.headers("最新跟进")
		.width = "270"
		.align = "left"
		.formula = "showReply(@cells[""最新跟进""],@cells[""sendOrReceive""],@cells[""msgAccId""],@cells[""msgUserId""],@cells[""msgCreateTime""],@cells[""MsgType""],@cells[""Content""],@cells[""PicUrl""],@cells[""MediaId""],@cells[""MediaPath""],@cells[""Format""],@cells[""Recognition""],@cells[""ThumbMediaId""],@cells[""Location_X""],@cells[""Location_Y""],@cells[""Scale""],@cells[""Label""],@cells[""Title""],@cells[""Description""],@cells[""msgUrl""],@cells[""tMsgId""],@cells[""msgCateid""],false)"
		End With
		With lvw.headers("最新关注时间")
		.width = "110"
		End With
		With lvw.headers("所属分组")
		.width = "80"
		End With
		With lvw.headers("状态")
		.width = "50"
		End With
		With lvw.headers("领用人员")
		.width = "50"
		End With
		With lvw.headers("操作")
		.formula = "getActionButtons(@cells[""id""],@cells[""stat""],@cells[""person""],@cells[""cateid""],@cells[""subscribeStat""],@cells[""canReply""],"&remind&")"
		.width = "180"
		End With
		dim btmStr
		If app.power.existsPower(108,6) Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='setAppoint();'>批量指派</button>"
		btmStr = btmStr& "<a>"
		lvw.addlink = "</a><input type='checkbox' onclick='$("".lvcbox"").attr(""checked"",this.checked);'>全选"& btmStr
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		Dim h, i, col, Czplist
		Dim hiddenCols : hiddenCols = Split("id,stat,person,cateid,telcateid,sendOrReceive,msgAccId,canReply,msgUserId,msgCreateTime,MsgType,Content,PicUrl,MediaId,MediaPath,Format,Recognition,ThumbMediaId,Location_X,Location_Y,Scale,Label,Title,Description,msgUrl,tMsgId,msgCateid",",")
		For i = 0 To ubound(hiddenCols)
			With lvw.headers(hiddenCols(i))
			.display = "none"
			End With
	...
	Function showCheckbox(id,cateid)
		If app.power.existsPowerIntro(108,6,cateid) Then
			showCheckbox = "<input name='sys_lvw_ckbox' class='lvcbox' id='mlistvw_ckv_" & id & "' type='checkbox' value='" & id & "'/>"
		end if
	end function
	Function showHeadImg(headPicPath)
		If Len(headPicPath)>0 Then
			showHeadImg = "<img src='" & headPicPath & "' border='0' style='cursor:hand;width:50px;height:50px;margin:5px;' onclick='showPic(this);'/>"
		else
			showHeadImg = "<img src='../hrm/img/noneperson.jpg' border='0' style='width:50px;height:50px;margin:5px'/>"
		end if
	end function
	Function showReply(msgid,sendOrReceive,msgAccId,msgUserId,msgCreateTime,MsgType,Content,PicUrl,MediaId,MediaPath,Format,Recognition,ThumbMediaId,_
		Location_X,Location_Y,Scale,Label,Title,Description,msgUrl,tMsgId,msgCateid,isExcelMode)
		If msgid = 0 Then
			showReply = ""
			Exit Function
		end if
		Dim act : act = Split(",接收,发送",",")(sendOrReceive) & "于:" & FromUnixTime(msgCreateTime)
		Call HandleErrorStr(Content&"")
		Select Case LCase(msgType)
		Case "text":
		showReply = "<span style='width:100%;display:block;text-align:left;'>" & replaceFaces(Replace(Content,Chr(10),"<br>")) & "</span><span style='width:100%;display:block;text-align:right;clear:both'>--&nbsp;" & act & "</span>"
		Case "text":
		Case "image":
		If isExcelMode Then
			showReply = "[图片信息] " & act
		else
			showReply = "<img src='" & PicUrl & "' style='cursor:hand;height:90px;float:left' onclick='showPic(this);' title='点击查看原图'/><span style='width:100%;display:block;text-align:right;clear:both'>--&nbsp;" & act & "</span>"
			showReply = "[图片信息] " & act
		end if
		Case "audio","voice":
		If isExcelMode Then
			...
			showReply = "<a href='javascript:void(0);' file='" & HexEncode(server.mappath(MediaPath)) & "' style='cursor:hand;float:left' onclick='downloadFile(this);' title='点击下载该音频文件'>[语音信息]</a>" & _
			"<span style='width:100%;display:block;text-align:right;clear:both'>--&nbsp;" & act & "</span>"
		end if
		Case "video","shortvideo":
		If isExcelMode Then
			showReply = "[视频信息] " & act
		else
			showReply = "<img src='" & ThumbMediaId & "' file='" & HexEncode(server.mappath(MediaPath)) & "' style='cursor:hand;height:90px;float:left' onclick='downloadFile(this);' title='点击下载该视频文件'/>" & _
			"<span style='width:100%;display:block;text-align:right;clear:both'>--&nbsp;" & act & "</span>"
		end if
		Case "location":
		If isExcelMode Then
			showReply = "[位置信息] " & Label & "<br/>" & act
		else
			showReply = "<img src='http://st.map.qq.com/api?size=600*300&center="&Location_Y&","&Location_X&"&zoom="&Scale&"&markers="&Location_Y&","&Location_X&"' onclick='showPic(this);' title='" & Label & "[点击放大]' style='cursor:hand;height:90px;float:left'><span style='width:100%;display:block;text-align:right;clear:both'>--&nbsp;" & act & "</span>"
			showReply = "[位置信息] " & Label & "<br/>" & act
		end if
		Case Else
		showReply = ""
	...
	Function getActionButtons(id,stat,person,cateid,subscribeStat,canReply,remind)
		Dim btnStr : btnStr = ""
		If remind > 0 Then
			btnStr = "<a href='javascript:void(0);' onclick='sendCancelRequest(0," & remind & ",0," & id & ")' ><img src='../images/alt3.gif' alt='取消提醒' border='0'></a>"
		end if
		If app.power.existsPowerIntro(108,1,cateid) Then
			btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" onclick=""javascript:window.open('MUserContent.asp?ord=" & app.base64.pwurl(id) & "','newwin','width=' + 860 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">详情</button>"
'If app.power.existsPowerIntro(108,1,cateid) Then
		end if
		If app.power.existsPowerIntro(108,6,cateid) Then
			btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" uid='" & id & "' cateid='" & cateid & "' onclick=""javascript:setAppoint(this);"">指派</button>"
		end if
		If app.power.existsPowerIntro(108,5,cateid) Or (fw1 = 2 And app.power.existsPowerIntro(108,5,cateid)) Then
			If subscribeStat <> 1 Then
				btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" onclick=""javascript:app.Alert('用户已取消关注，无法使用微信联系该用户！')"">跟进</button>"
			ElseIf canReply = 1 Then
				btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" uid='" & id &"' onclick=""javascript:wxchat.openDlg("& id &")"">跟进</button>"
			else
			end if
		end if
		If CLng(cateid) = app.Info.User And stat <> 2 And app.power.existsPower(1,13) Then
			btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton5"" uid='" & id & "' binded='" & app.iif(CLng(person)>0,1,0) & "' onclick=""javascript:showMenu(this);"">转客户</button>"
		end if
		If CLng(cateid) = app.Info.User And CLng(person) = 0 Then
			If stat=2 Then
				btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" onclick=""javascript:resumeUser(" & id & ")"">启用</button>"
			else
				btnStr = btnStr & "&nbsp;" & "<button class=""oldbutton2"" onclick=""javascript:cancelUser(" & id & ")"">作废</button>"
			end if
		end if
		getActionButtons = btnStr
	end function
	Sub App_loadFans
		server.scripttimeout=99999999
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Call helper.loadFans("")
		Response.write "{success:true,msg:'加载成功'}"
		app.Log.remark = "加载粉丝"
	end sub
	Sub App_changeStat
		Response.clear
		Dim stat : stat = app.getInt("stat")
		If stat > 2 Or stat < 1 Then stat = 1
		Dim id : id = app.getInt("id")
		Dim reason : reason = app.getText("reason")
		Dim rs : Set rs=conn.execute("select * from MMsg_User where id=" & id)
		If rs.eof Then
			Response.write "{success:false,msg:'用户不存在或者已经被删除'}"
		else
			If CLng(rs("stat")) <> stat Then
				If CLng("0" & rs("person")) <> 0 And stat = 2 Then
					Response.write "{success:false,msg:'该用户已转客户，不可以作废'}"
				ElseIf Not app.power.existsPowerIntro(108,1,rs("cateid")) Then
					Response.write "{success:false,msg:'没有权限对此用户进行操作'}"
				else
					conn.execute "update MMsg_User set stat=" & stat & ",reason='" & reason & "' where id="  & id
		...
		app.Log.remark = "微信用户" & app.iif(stat = 2,"作废","启用")
	end sub
	Sub App_bindMicroMsgUser
		Response.clear
		Dim uid : uid = app.getInt("uid")
		Dim pid : pid = app.getInt("pid")
		Dim rs,cateid,person,stat,msg
		Set rs = cn.execute("select isnull(cateid,0) cateid,isnull(person,0) person,stat from MMsg_User where id=" & uid)
		If rs.eof Then
			msg = "{success:false,msg:'用户不存在或者已被删除！'}"
		else
			cateid = CLng(rs("cateid"))
			person = CLng(rs("person"))
			stat = CLng(rs("stat"))
			If stat = 2 Then
				msg = "{success:false,msg:'用户不存在或者已被删除！'}"
			ElseIf cateid <> app.Info.User Then
				msg = "{success:false,msg:'只能操作自己领用的微信用户！'}"
			ElseIf pid > 0 Then
				cn.execute "update MMsg_User set person=null,bindTime=null where person=" & pid & " " & vbcrlf &_
				"update MMsg_User set person=" & pid & ",bindTime=getdate() where id=" & uid & " " & vbcrlf &_
				"update person set weixinAcc=(select top 1 nickname from MMsg_User where id=" & uid & ") where ord=" & pid
				msg = "{success:true,msg:'操作已成功'}"
			end if
		end if
		app.Log.remark = "微信用户转已存在客户"
		rs.close
		Set rs=Nothing
		Response.write msg
	end sub
	Sub App_saveAppoint
		...
		Dim uid,cid
		uid = app.getText("uid")
		cid = app.getInt("cateid")
		If cid=0 Then
			Response.write "{success:false,msg:'请选择用户！'}"
			Response.end
		end if
		If Len(uid) = 0 Then
			Response.write "{success:false,msg:'请选择微信用户！'}"
			Response.end
		end if
		Dim qxintro : qxintro = app.power.getPowerIntro(108,6)
		Dim condition
		If qxintro <> "" Then condition =  " and cateid in (" & qxintro & ")"
		Dim sql
		sql = "update MMsg_User set cateid=" & cid & ",leader=" & app.Info.User & " where id in (" & uid & ")" & condition
		cn.execute sql
		app.Log.remark = "微信用户指派"
		Response.write "{success:true,msg:'操作已成功！'}"
	end sub
	Sub App_postTextMsg
		Response.clear
		Dim msg : msg = request("msg")
		Dim uid : uid = request("uid")
		If Len(msg) = 0 Then
			Response.write "{success:false,msg:'发送的消息内容不能为空！'}"
			Response.end
		ElseIf Len(msg) > 600 Then
			Response.write "{success:false,msg:'发送的消息内容超长！'}"
			Response.end
		end if
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Dim result : result = helper.PostMsg(uid,msg)
		If Left(result,1) = "0" Then
			Response.write "{success:false,msg:'" & Mid(result,2) & "'}"
		else
			Dim rs,sql,userAvatar
			sql = "SELECT photos FROM hr_person WHERE userid = "& sdk.Info.User &" "
			Set rs = cn.Execute(sql)
			If Not rs.Eof Then
				userAvatar = "../hrm/load/" & rs(0)
			end if
			rs.close
			set rs = nothing
			Response.write "{success:true,msg:'发送成功！',mgID:"& result &",userAvatar:"""& userAvatar &"""}"
		end if
		app.Log.remark = "向用户发送微信消息"
		Set helper = Nothing
	end sub
	Sub App_GetChatAvatar
		Response.Clear
		Dim uid,rs,sql,avatar,nickName,result
		uid = Request("uid")
		sql = "SELECT TOP 1 headimgPath,nickName FROM MMsg_User WHERE id = "& uid &" "
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
			result = "{avatar:"""& rs("headimgPath") &""",nickName:"""& rs("nickName") &"""}"
		else
			result = ""
		end if
		rs.close
		set rs = nothing
		Response.write result
		app.Log.remark = "获取聊天窗口的头像和昵称"
	end sub
	Sub App_GetRecentlyMsg
		Response.Clear
		Dim uid : uid = Request("uid")
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Dim result : result = helper.GetRecentlyMsg(uid)
		Response.write result
		app.Log.remark = "获取当天的收发消息记录"
		Set helper = Nothing
	end sub
	Sub App_GetMoreMsg
		Response.Clear
		...
		mgID = Request("mgID")
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Dim result : result = helper.GetMoreMsg(uid,mgID)
		Response.write result
		app.Log.remark = "获取更多消息记录"
		Set helper = Nothing
	end sub
	Sub App_GetHisMsg
		Response.Clear
		Dim uid,pageindex,pagesize,sDate
		uid = CLng(Request("uid"))
		pageindex = CLng(Request("pageindex"))
		pagesize = CLng(Request("pagesize"))
		sDate = Request("sDate")
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Dim result : result = helper.GetHisMsg(uid,pageindex,pagesize,sDate)
		Response.write result
		app.Log.remark = "获取历史消息记录"
		Set helper = Nothing
	end sub
	Sub App_GetCurMsg
		Response.Clear
		Dim uid : uid = Request("uid")
		Dim helper : Set helper = CreateMicroMsgHelper(cn,1)
		Dim result : result = helper.GetCurMsg(uid)
		Response.write result
		app.Log.remark = "获取实时消息记录"
		Set helper = Nothing
	end sub
	Sub App_updateMsgTimeFlag
		Dim mgID,flag
		mgID = Request("mgID")
		flag = Request("flag")
		cn.Execute("UPDATE MMsg_Message SET timeFlag = "& flag &" WHERE timeFlag < 1 AND id = "& mgID &" ")
	end sub
	Sub App_showUserList
		dim rs, rs1, rs2, rs8, rs9, sql, sql1, sql2, sql8, sql9, open_1_1, w1_list, w2_list, w3_list, str_w1, str_w2, str_w3, selectedid, i, i3, i4
...
'set rs1=nothing
		...
		Response.write "" & vbcrlf & "<table style=""width:475px"" border=""1"" borderColor=""#ccc"" cellpadding=""0"" cellspacing=""0"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "    <tr>" & vbcrlf & "            <td colspan=""4"" style=""background:;background-color:#fff;word-wrap:normal;word-break:keep-all;height:35px"">"& vbcrlf
		set rs=server.CreateObject("adodb.recordset")
		...
		if rs8.RecordCount<=0 then
			...
			do until rs8.eof
				...
				if rs.RecordCount<=0 then
					...
					do until rs.eof
						Response.write "" & vbcrlf & "      <tr>" & vbcrlf & "            <td width=""14%""><div align=""center""></div></td>" & vbcrlf & "             <td colspan=""3"" style=""word-wrap:normal;word-break:keep-all;height:35px"">" & vbcrlf & "                   <input name=""member"" id=""member_"
'if rs.RecordCount<=0 then
						Response.write rs("ord")
						Response.write """ type=""radio"" " & vbcrlf & "                              value="""
						Response.write rs("ord")
				...
				if rs9.RecordCount<=0 then
					...
					do until rs9.eof
						...
						if rs1.RecordCount<=0 then
							...
							do until rs1.eof
								Response.write "" & vbcrlf & "      <tr>" & vbcrlf & "            <td colspan=""2"">&nbsp;</td>" & vbcrlf & "               <td colspan=""2"" style=""word-wrap:normal;word-break:keep-all;height:35px"">" & vbcrlf & "                   <input name=""member"" id=""member_"
'do until rs1.eof
								Response.write rs1("ord")
								Response.write """ type=""radio"" " & vbcrlf & "                              value="""
								Response.write rs1("ord")
