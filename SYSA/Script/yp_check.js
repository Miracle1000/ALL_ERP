...
function searchMe(a,b)
{
	...
	var url = "ajax_yp3.asp?a=" +escape(u_name)+"&b=" + escape(u_bh)+"&c=" + escape(u_xh)+"&d=" + escape(u_ph)+"&e=" + escape(u_xlh)+"&t="+u_type+"&sid=" +sid+"&cp="+b+"&stamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
	xmlHttp.open("GET",url,false)
	xmlHttp.send(null);	
...
function addMx(id,prod_name,prod_bh,prod_xh,sort1,prod_num,sto_name,prod_ph,prod_xlh,prod_sctime,prod_ystime)
{
	...
	tr.appendChild(td5);
	td5.align="center";
	var delHref = "<div align=\"center\"><input name=\"che_zmnum\" type=\"text\" id=\"che_zmnum" + a + "\" onKeyUp=\"value=value.replace(/[^\\d*|\\.]/g,'')\" size=\"8\" style=\"width:60px;\"  onpropertychange=\"formatData(this,'number');\" value=\"" + parseFloat(prod_num) + "\" readonly=\"true\"></div>";
	td5.innerHTML=delHref;
	var td6 = document.createElement('td');
	tr.appendChild(td6);
	td6.align="center";
	delHref = "<div align=\"center\"><input name=\"che_sjnum\" type=\"text\" id=\"che_sjnum" + a + "\" onKeyUp=\"value=value.replace(/[^\\d*|\\.]/g,'')\" size=\"8\" style=\"width:60px;\"  onpropertychange=\"formatData(this,'number');set1(" + a + ")\"></div>"
	td6.innerHTML=delHref;
	var td4 = document.createElement('td');
	tr.appendChild(td4);
	td4.align="center";
	var delHref = "<div align=\"center\"><input name=\"che_yknum\" type=\"text\" id=\"che_yknum" + a + "\" onKeyUp=\"value=value.replace(/[^\\d*|\\.]/g,'')\" size=\"8\" style=\"width:60px;\"  onpropertychange=\"formatData(this,'number');\" readonly=\"true\"></div>"
...
function ask1(val)
{
	che_title=document.getElementById("che_title");
	prod_id=document.getElementsByName("prod_id");
	if (che_title.value=="")
	{
		alert('盘点标题不能为空！')
		return false;
	...
	var sjnum=document.getElementsByName("che_sjnum");
	for(i=0;i<sjnum.length;i++)
	{
		if(sjnum[i].value=="")
		{
			alert('实际数量不能为零')
	...
	document.all.data.action="save_check.asp?tp="+val;
	document.all.data.submit();
}
