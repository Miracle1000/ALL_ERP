...
'If Not rs.Eof Then
	...
	Ord=deUrl(SaveRequestUrl("Ord"))
	OrdList=SaveRequestUrl("OrdList")
	Used=SaveRequestForm("Used")
	If Used="" Then Used=True
	j=0
	k=0
	l=0
	m=0
	...
	x=0
	c=0
	Title=SaveRequestForm("Title")
	SQLTemplate=SaveRequestForm("SQLTemplate")
	EffectiveTime=SaveRequestForm("EffectiveTime")
	For Each RegPara in Request.Form
		If Instr(RegPara,"config_IsBase")>0 Then
			if j=0 then
				config_IsBase=SaveRequestForm(RegPara)
			else
				config_IsBase=config_IsBase&","&SaveRequestForm(RegPara)
			end if
			j=j+1
			config_IsBase=config_IsBase&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_datenum")>0 Then
			if k=0 then
				config_datenum=SaveRequestForm(RegPara)
			else
				config_datenum=config_datenum&","&SaveRequestForm(RegPara)
			end if
			k=k+1
			config_datenum=config_datenum&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_unit")>0 Then
			if l=0 then
				config_unit=SaveRequestForm(RegPara)
			else
				config_unit=config_unit&","&SaveRequestForm(RegPara)
			end if
			l=l+1
			config_unit=config_unit&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_QuestionsTemplate")>0 Then
			if m=0 then
				config_QuestionsTemplate=SaveRequestForm(RegPara)
			else
				config_QuestionsTemplate=config_QuestionsTemplate&","&SaveRequestForm(RegPara)
			end if
			m=m+1
			config_QuestionsTemplate=config_QuestionsTemplate&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_ListID")>0 Then
			if n=0 then
				config_ListID=SaveRequestForm(RegPara)
			else
				config_ListID=config_ListID&","&SaveRequestForm(RegPara)
			end if
			n=n+1
			config_ListID=config_ListID&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_user_list")>0 Then
			if x=0 then
				config_userlist=SaveRequestForm(RegPara)
			else
				config_userlist=configuser_list&"§"&SaveRequestForm(RegPara)
			end if
			x=x+1
			config_userlist=configuser_list&"§"&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"config_jdTitle")>0 Then
			if c=0 then
				config_jdTitle=SaveRequestForm(RegPara)
			else
				config_jdTitle=config_jdTitle&","&SaveRequestForm(RegPara)
			end if
			c=c+1
			config_jdTitle=config_jdTitle&","&SaveRequestForm(RegPara)
		end if
	next
	If action="save" Then
		config_userlist=Replace(config_userlist,vbcrlf,"")
		If config_userlist&""="" Then
			Call jsAlert("请选择回访人员！")
			call db_close : Response.end
		ElseIf Replace(config_userlist&"",",","")="" Then
			Call jsAlert("请选择回访人员！")
			call db_close : Response.end
		end if
		Set Rs=server.CreateObject("adodb.recordset")
		SQL="Select Top 1 * from hf_config Where title='"&Title&"'"
		Rs.Open Sql,conn,1,3
		If Rs.Eof Then
			Rs.AddNew
			Rs("Title")=Title
			Rs("SQLTemplate")=SQLTemplate
			Rs("EffectiveTime")=EffectiveTime
			Rs("Used")=Used
			Rs("InDate")=now
			Rs("Creator")=session("personzbintel2007")
			Rs("del")=1
			rs.update
		else
			Call jsAlert("流程名称重复！")
		...
		rs.close
		Set Rs=Nothing
		MainID = GetIdentity("hf_config","id","creator","")
		If config_IsBase<>"" Then
			Arr_IsBase=Split(config_IsBase,",")
			Arr_datenum=Split(config_datenum,",")
			Arr_unit=Split(config_unit,",")
			Arr_QuestionsTemplate=Split(config_QuestionsTemplate,",")
			Arr_ListID=Split(config_ListID,",")
			Arr_userlist=Split(config_userlist,",")
			config_jdTitle=Split(config_jdTitle,",")
			For i=0 To Ubound(Arr_IsBase)
				Set Rs=server.CreateObject("adodb.recordset")
				SQL="Select Top 1 * from hf_configList Where MainID="&MainID&" And  id="&Arr_ListID(i)&""
				Rs.Open Sql,conn,1,3
				If Rs.Eof Then
					Rs.AddNew
					Rs("Title")=trim(config_jdTitle(i))
					Rs("user_list")=replace(replace(Arr_userlist(i),"|",",")," ","")
					Rs("IsBase")=trim(Arr_IsBase(i))
					Rs("MainID")=trim(MainID)
					Rs("datenum")=trim(Arr_datenum(i))
					Rs("unit")=trim(Arr_unit(i))
					Rs("Template")=trim(Arr_QuestionsTemplate(i))
					Rs("InDate")=now
					Rs("Creator")=session("personzbintel2007")
					Rs("del")=1
					rs.update
		...
		Response.write "<script language=''javascript>if(window.parent.opener){window.parent.opener.location.href='HfConfigList.asp';window.parent.close();}else{window.parent.location.href='HfConfigList.asp';}</script>"
		call db_close : Response.end
	ElseIf Action="edit" And IsNum(Ord) Then
		ItemList=""
		Base_user_list_Name=""
		Set Rs=server.CreateObject("adodb.recordset")
		SQL="Select Top 1 * from hf_config Where id="&Ord&""
		rs.open sql,conn,1,1
		If Not Rs.Eof Then
			configTitle=Rs("Title")
			SQLTemplate=Rs("SQLTemplate")
			Used=Rs("Used")
			EffectiveTime=Rs("EffectiveTime")
		end if
		rs.close
		Set Rs=Nothing
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select a.* from hf_configList a  where a.MainID="&Ord&" and a.del=1"
		rs.open sql,conn,1,1
		If Not Rs.Eof Then
			Do While Not Rs.Eof
				unit=Rs("unit")
				If Rs("IsBase") Then
					Base_Title=Rs("Title")
					Base_ListID=Rs("id")
					Base_datenum=Rs("datenum")
					Base_unit=Rs("unit")
					Base_Template=Rs("Template")
					Base_user_list=replace(Rs("user_list"),"|",",")
					If Base_user_list<>"" then
						Set Rs8=server.CreateObject("adodb.recordset")
						Sql="select a.name from gate a  where a.ord in("&Base_user_list&") and a.del=1"
						Rs8.Open Sql,conn,1,1
						If Not Rs8.Eof Then
							Do While Not Rs8.Eof
								Base_user_list_Name=Base_user_list_Name&Rs8("name")&" "
								Rs8.MoveNext
							Loop
						end if
						Rs8.Close
					...
					TempList=TempList&"<tr class='config_edit_"&Rs("id")&"'><td><div align='center'><input name='config_jdTitle' type='text' value='"&rs("title")&"' id='config_jdTitle' size='16'  dataType='Limit' min='1' max='100' ><span class='red'>*</span></div></td>"&_
					"                                                    ""<td align='center'><div align='center'><input type='hidden' value='""&Rs(""id"")&""' name='config_ListID'><input type='hidden' value='0' name='config_IsBase'><input name='config_datenum' type='text' value='""&Rs(""datenum"")&""'  onKeyUp=""""value=value.replace(/[^\d]/g,'')"""" size='8' dataType='Limit'""min='1' max='50'  msg='必填'>&nbsp;&nbsp;"&_
					"<select name='config_unit' >"&_
					"<option value='1' "
					If unit=1 Then TempList=TempList&"selected"
					TempList=TempList&">年</option>"&_
					"<option value='2' "
					If unit=2 Then TempList=TempList&"selected"
					TempList=TempList&">季</option>"&_
					"<option value='3' "
					If unit=3 Then TempList=TempList&"selected"
					TempList=TempList&">月</option>"&_
					"<option value='4' "
					If unit=4 Then TempList=TempList&"selected"
					TempList=TempList&">周</option>"&_
					"<option value='5' "
					If unit=5 Then TempList=TempList&"selected"
					TempList=TempList&">日</option>"&_
					"</select><span class='red'>*</span></div></td>"&_
					"<td align='center'>"&_
					"<select name='config_QuestionsTemplate' dataType='Limit' min='1' max='50'  msg='必选'>"&_
					"<option value='' >请选择</option>"
					Set Rs8=server.CreateObject("adodb.recordset")
					SQL="Select * from hf_Template where del=1 And Used=1 order by id desc"
					Rs8.Open SQL,Conn,1,1
					If Not Rs8.Eof Then
						Do While Not Rs8.Eof
							TempList=TempList&"<option value='"&Rs8("id")&"' "
							If Rs8("id")=Rs("Template") Then TempList=TempList&"selected"
							TempList=TempList&">"&Rs8("Title")&"</option>"
							Rs8.MoveNext
						Loop
					end if
					Rs8.Close
					Set Rs8=Nothing
					If replace(Rs("user_list"),"|",",")<>"" then
						Set Rs8=server.CreateObject("adodb.recordset")
						Sql="select a.name from gate a  where a.ord in("&replace(Rs("user_list"),"|",",")&") and a.del=1"
						Rs8.Open Sql,conn,1,1
						If Not Rs8.Eof Then
							Do While Not Rs8.Eof
								W3Name=Rs8("name")&" "
					...
					TempList=TempList&"</select><span class='red'>*</span></td>"&_
					"<td align='center' class='gray'><input type='hidden' name='config_user_list' id='config_user_listname"&Rs("id")&"_hiden'   value='"&replace(Rs("user_list"),"|",",")&"'>"&_
					"<input name='user_listname' type='text' id='config_user_listname"&Rs("id")&"'   readonly='readonly' onclick=""showGatePersonDiv(this.name,this.id,'','Search_GateCheck.asp','','');"" value='"&W3Name&"' size='16' dataType='Limit' min='1' max='50'  msg='必选'><span class='red'>*</span></td>"&_
					"<td align='center'><div align='center'><input name='button' class='anybutton2' type='button' value='删除' onClick='delClassItem(this,"&rs("id")&")'></div></td>"
		...
		rs.close
		Set Rs=Nothing
	ElseIf Action="update" And IsNum(Ord) Then
		If conn.execute("select 1 from hf_config where id<>"&Ord&" and Title = '"&Title&"'").eof = False Then
			Call jsAlert("流程名称重复！")
			call db_close : Response.end
		end if
		Set Rs=server.CreateObject("adodb.recordset")
		...
		If config_IsBase<>"" Then
			...
			For i=0 To Ubound(Arr_IsBase)
				...
				Rs("Title")=trim(config_jdTitle(i))
				Rs("user_list")=trim(replace(replace(Arr_userlist(i),"|",",")," ",""))
				Rs("IsBase")=trim(Arr_IsBase(i))
				Rs("MainID")=MainID
				Rs("datenum")=trim(Arr_datenum(i))
				Rs("unit")=trim(Arr_unit(i))
		...
		Response.write "<script language=''javascript>alert('修改成功！');if(window.parent.opener){window.parent.opener.location.href='HfConfigList.asp?CurrPage="&CurrPage&"';window.parent.close();}else{window.parent.location.href='HfConfigList.asp?CurrPage="&CurrPage&"';}</script>"
		call db_close : Response.end
	ElseIf Action="del"  And IsNum(Ord) Then
		Conn.execute("Delete hf_config Where id="&Ord&"")
		Conn.execute("Delete hf_configList Where MainID="&Ord&"")
		CurrPage=request("CurrPage")
		Call jsLocat("删除成功！","HfConfigList.asp?CurrPage="&CurrPage)
		call db_close : Response.end
	ElseIf Action="dellist"  And OrdList<>"" Then
		CurrPage = request("CurrPage")
		Conn.execute("Delete hf_config Where id in("&OrdList&")")
		Conn.execute("Delete hf_configList Where MainID in("&OrdList&")")
		Call jsLocat("批量删除成功！","HfConfigList.asp?CurrPage="&CurrPage)
		call db_close : Response.end
	end if
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "var xmlHttpRequest;" & vbcrlf & "function createXMLHttpRequest(){" & vbcrlf & "try" & vbcrlf & "{  xmlHttpRequest=new XMLHttpRequest();}" & vbcrlf &" catch (e)" & vbcrlf & "       {" & vbcrlf & "               try" & vbcrlf & "             {  xmlHttpRequest=new ActiveXObject(""Msxml2.XMLHTTP"");}" & vbcrlf & "           catch (e)" & vbcrlf & "               {" & vbcrlf & "                       try" & vbcrlf & "                             { xmlHttpRequest=new ActiveXObject(""Microsoft.XMLHTTP"");}" & vbcrlf & "                 catch (e)" & vbcrlf & "       {alert(""您的浏览器不支持AJAX！"");return false;  }" & vbcrlf & "                 }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "function TrToggle(ClassName)" & vbcrlf & "{" & vbcrlf & "  $("".""+ClassName+"""").toggle();" & vbcrlf & "}" & vbcrlf & "function delClassItem(cid,ord)" & vbcrlf & "{" & vbcrlf & "if(ord!=0&&ord!="""")" & vbcrlf & "{" & vbcrlf & "     createXMLHttpRequest();" & vbcrlf & " xmlHttpRequest.open(""POST"",""Ajax_DelConfigList.asp"",true);" & vbcrlf & "  xmlHttpRequest.setRequestHeader(""Content-Type"",""application/x-www-form-urlencoded"");" & vbcrlf & "        xmlHttpRequest.onreadystatechange = DelResponse;" & vbcrlf & "      xmlHttpRequest.send(""ord=""+""""+ord+"""");" & vbcrlf & "}" & vbcrlf & "       $("".""+cid.parentElement.parentElement.parentElement.className).remove();" & vbcrlf & "" & vbcrlf & "          try" & vbcrlf & "     { parent.frameResize();}" & vbcrlf & "        catch(e){}" & vbcrlf & "}" & vbcrlf & "//处理返回信息函数" & vbcrlf & "function DelResponse(){" & vbcrlf & "           if(xmlHttpRequest.readyState == 4){" & vbcrlf & "                 if(xmlHttpRequest.status == 200){" & vbcrlf & "                   var tempStr = """";" & vbcrlf & "                 if (!xmlHttpRequest.responseText.split(""</noscript>"")[1]) {" & vbcrlf & "                         tempStr = xmlHttpRequest.responseText;" & vbcrlf & "                    }" & vbcrlf & "                       else {" & vbcrlf & "                      tempStr = xmlHttpRequest.responseText.split(""</noscript>"")[1];" & vbcrlf & "                        }" & vbcrlf & "                               alert(tempStr);" & vbcrlf & "                         try" & vbcrlf & "                             { parent.frameResize();}" & vbcrlf & "                            catch(e){}" & vbcrlf & "                              }else{" & vbcrlf & "                                   // window.alert(""请求页面异常"");" & vbcrlf & "                         }" & vbcrlf & "               }" & vbcrlf & "}" & vbcrlf & "function addNextStage()" & vbcrlf & "{" & vbcrlf & "      var date=new Date();" & vbcrlf & "    var trID=date.getHours()+""-""+date.getMinutes()+""-""+date.getSeconds()+""-""+date.getMilliseconds();" & vbcrlf & "     var tempStr=""<tr class='""+trID+""'><td><div align='center'><input name='config_jdTitle' type='text' value='' id='config_jdTitle' size='16' dataType='Limit' min='1' max='100'  msg='必填'><span class='red'>*</span></div></td>""" & vbcrlf & "                                                       +""<td class='gray' align='center'><input type='hidden' value='0' name='config_ListID'><input type='hidden' value='0' name='config_IsBase'><input name='config_datenum' type='text'  onKeyUp=\""value=value.replace(/[^\\d]/g,'')\"" size='8' dataType='Limit' min='1' max='50'  msg='必填'>&nbsp;&nbsp;""" & vbcrlf & "                                                    +""<select name='config_unit' >""" & vbcrlf & "                                                   +""<option value='1' selected>年</option>""" & vbcrlf & "                                                 +""<option value='2'>季</option>""" & vbcrlf & "                                                  +""<option value='3'>月</option>""" & vbcrlf & "                                                  +""<option value='4'>周</option>"""& vbcrlf & "                                                      +""<option value='5'>日</option>""" & vbcrlf & "                                                  +""</select><span class='red'>*</span></td>""" & vbcrlf & "                                                       +""<td><div align='center'>""" & vbcrlf & "                                                       +""<select name='config_QuestionsTemplate' dataType='Limit' min='1' max='50'  msg='必选'>""" & vbcrlf & "                                                 +""<option value='' selected>请选择</option>""" & vbcrlf & "                                                  "
	'Response.write Application("sys.info.jsver")
	Set Rs=server.CreateObject("adodb.recordset")
	SQL="Select * from hf_Template where del=1 And Used=1 order by id desc"
	rs.open sql,conn,1,1
...
'Do While Not Rs.Eof
	...
	rs.close
	Set Rs=Nothing
	Response.write "" & vbcrlf & "                                             +""</select><span class='red'>*</span></div></td>""" & vbcrlf & "                                         +""<td align='center' class='gray'><input type='hidden' name='config_user_list' id='config_user_listname""+trID+""_hiden'   value=''>""" & vbcrlf & "                                         +""<input name='user_listname' type='text' id='config_user_listname""+trID+""'   readonly='readonly' onclick=\""showGatePersonDiv(this.name,this.id,'','Search_GateCheck.asp','','');\"" value='' size='16' dataType='Limit' min='1' max='50'  msg='必选'><span class='red'>*</span></td>""" & vbcrlf & "                                               +""<td><div align='center'><input name='button'class='anybutton2' type='button' value='删除' onClick='delClassItem(this,0)'></div></td></tr>"";" & vbcrlf & "                                         $(""#StageList"").append(tempStr);" & vbcrlf & "                                          try" & vbcrlf & "                                     { parent.frameResize();}" & vbcrlf & "                                        catch(e){}" & vbcrlf & "}" & vbcrlf & "function showGatePersonDiv(InputName,InputId,defaultval,strUrl,width,height)" & vbcrlf & "{" & vbcrlf & "  if(strUrl.indexOf(""?"")>=0)" & vbcrlf & "        {" & vbcrlf & "               strUrl=strUrl+""&InputName=""+InputName+""&InputId=""+InputId+""&W3=""+defaultval;" & vbcrlf & "          }" & vbcrlf & "       else" & vbcrlf & "    {" & vbcrlf & "               strUrl=strUrl+""?InputName=""+InputName+""&InputId=""+InputId+""&W3=""+defaultval;" & vbcrlf & "         }" & vbcrlf & "       var w = 960 , h = 640 ;" & vbcrlf & " window.open( strUrl ,'newwin','width=' + w + ',height=' + h + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');" & vbcrlf & "}" & vbcrlf & "function GetUserVal(inputId,val,username)" & vbcrlf & "{" & vbcrlf & "      $(""#""+inputId+""_hiden"").val(val);" & vbcrlf & "   $(""#""+inputId+"""").val(username);" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	Set Rs=Nothing
	If Base_ListID="" or isnull(Base_ListID)  then Base_ListID=0
	If EffectiveTime="" or isnull(EffectiveTime) then EffectiveTime=date()
	Response.write "" & vbcrlf & "<form method=""post"" name=""demo""  action=""?action=save"" id=""demo"" onsubmit=""return Validator.Validate(this,2)"" target=""saveFrom"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">回访流程"
	If action = "edit" Then
		Response.write "修改"
	else
		Response.write "添加"
	end if
	Response.write "</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>    " & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""8"" ><div align=""left""><img class=""resetElementHidden"" src=""../images/jiantou.gif"" /><img class='resetElementShow' style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png' /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_1')"">基本信息设置</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""12%""><div align=""right"">流程名称：</div></td>" & vbcrlf &          "  <td width=""25%"" class=""gray""><input type=""text"" name=""title"" size=""25"" value="""
	Response.write configTitle
	Response.write """ dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td width=""15%""><div align=""right"">触发点：</div></td>" & vbcrlf & "            <td width=""16%"" class=""gray""><select name=""SQLTemplate"" dataType=""Limit"" min=""1"" max=""50""  msg=""必选""> "& vbcrlf &                " <option value="""
	if SQLTemplate="" or isnull(SQLTemplate) Then
		Response.write "selected"
	end if
	Response.write ">请选择</option>" & vbcrlf & "                "
	Set Rs=server.CreateObject("adodb.recordset")
	SQL="Select * from hf_ywlmSetings where del=1 order by id asc"
	rs.open sql,conn,1,1
	If Not Rs.Eof Then
		Do While Not Rs.Eof
			If InStr(Rs("Title"),"合同") > 0 Then
				isVisiable = zbruntime.MC(7000)
			end if
			If InStr(Rs("Title"),"出库") > 0 Then
				isVisiable = zbruntime.MC(17003)
			end if
			If InStr(Rs("Title"),"发货") > 0 Then
				isVisiable = zbruntime.MC(17008)
			end if
			If InStr(Rs("Title"),"收款") > 0 Then
				isVisiable = (zbruntime.MC(23000) And zbruntime.MC(23001))
			end if
			If InStr(Rs("Title"),"项目") > 0 Then
				isVisiable = zbruntime.MC(3000)
			end if
			If InStr(Rs("Title"),"售后服务") > 0 Then
				isVisiable = zbruntime.MC(9000)
			end if
			If InStr(Rs("Title"),"售后维修") > 0 Then
				isVisiable = zbruntime.MC(71000)
			end if
			If isVisiable Then
				Response.write "" & vbcrlf & "                <option value="""
				Response.write Rs("id")
				Response.write """ "
				if SQLTemplate=Rs("id") Then
					Response.write "selected"
				end if
				Response.write ">"
				Response.write Rs("Title")
	...
	rs.close
	Set Rs=Nothing
	Response.write "" & vbcrlf & "              </select>" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td><div align=""right"">是否开启：</div></td>" & vbcrlf & "            <td ><input name=""Used"" type=""radio"" value=""1"" "
	If Used=true then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "              启用" & vbcrlf & "              <input type=""radio"" name=""Used"" value=""0"" "
	If Used=false then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "              暂停</td>" & vbcrlf & "          </tr>" & vbcrlf & "<tr>" & vbcrlf & "<td>" & vbcrlf & "<div align=""right"">生效日期：</div>" & vbcrlf & "</td>" & vbcrlf & "<td colspan=""5"">" & vbcrlf & "<div align=""left"">" & vbcrlf & "  <input type=""text"" name=""EffectiveTime""  value="""
	Response.write EffectiveTime
	Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly  size=""15"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填""></div>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""8"" ><div align=""left""><imgclass=""resetElementHidden"" src=""../images/jiantou.gif"" /><img class='resetElementShow' style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png' /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_2')"">回访阶段设置</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""StageList"">" & vbcrlf & "                <tr class=""top"">" & vbcrlf & "                  <td width=""22%""><div align=""center"">回访阶段</div></td>" & vbcrlf &                  " <td width=""21%""><div align=""center"">回访间隔</div></td>" & vbcrlf &                 "  <td width=""18%""><div align=""center"">回访模板</div></td>" & vbcrlf &                   "<td width=""22%""><div align=""center"">回访人员</div></td>" & vbcrlf & "                  <td width=""17%""><div align=""center"">操作</div></td>" & vbcrlf & "                </tr>" & vbcrlf & " <tr>" & vbcrlf & "            <td align=""center""><input name=""config_jdTitle"" type=""text"" value="""
	Response.write Base_Title
	Response.write """ id=""config_jdTitle"" size=""16"" dataType=""Limit""  min='1' max='100'  msg='必填'><span class='red'>*</span></td>" & vbcrlf & "            <td class=""gray"" align=""center""><input type=""hidden"" value=""1"" name=""config_IsBase"">" & vbcrlf & "              <input type=""hidden"" value="""
	Response.write Base_ListID
	Response.write """ name=""config_ListID"">" & vbcrlf & "              <input name=""config_datenum"" type=""text"" onKeyUp=""value=value.replace(/[^\d]/g,'')""  value="""
	Response.write Base_datenum
	Response.write """ size=""8"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">&nbsp;&nbsp;<select name=""config_unit"" >" & vbcrlf & "                <option value=""1"" "
	if Base_unit=1 then
		Response.write "selected"
	end if
	Response.write ">年</option>" & vbcrlf & "                <option value=""2"" "
	if Base_unit=2 then
		Response.write "selected"
	end if
	Response.write ">季</option>" & vbcrlf & "                <option value=""3"" "
	if Base_unit=3 then
		Response.write "selected"
	end if
	Response.write ">月</option>" & vbcrlf & "                <option value=""4"" "
	if Base_unit=4 then
		Response.write "selected"
	end if
	Response.write ">周</option>" & vbcrlf & "                <option value=""5"" "
	if Base_unit=5 then
		Response.write "selected"
	end if
	Response.write ">日</option>" & vbcrlf & "              </select><span class=""red"">*</span></td>" & vbcrlf & "            <td align=""center""><span class=""gray"">" & vbcrlf & "              <select name=""config_QuestionsTemplate"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">" & vbcrlf & "         <option value="""" >请选择</option>" & vbcrlf & "                "
	Set Rs=server.CreateObject("adodb.recordset")
	SQL="Select * from hf_Template where del=1 And Used=1 order by id desc"
	rs.open sql,conn,1,1
	If Not Rs.Eof Then
	...
	rs.close
	Set Rs=Nothing
	Response.write "" & vbcrlf & "              </select><span class=""red"">*</span></span></td>" & vbcrlf & "            <td align=""center"" class=""gray""><input type=""hidden"" name=""config_user_list"" id=""config_user_listname01_hiden""   value="""
	Response.write Base_user_list
	Response.write """>" & vbcrlf & "              <input name=""user_listname"" type=""text"" id=""config_user_listname01""   readonly=""readonly"" onclick=""showGatePersonDiv(this.name,this.id,'"
	Response.write Base_user_list
	Response.write "','Search_GateCheck.asp','','');"" value="""
	Response.write Base_user_list_Name
	Response.write """ size=""16"" dataType='Limit' min='1' max='50'  msg='必选'><span class='red'>*</span></td>" & vbcrlf & "<td align=""center"" class=""gray""><div align='center'><input name='button' class='anybutton2' type='button' value='删除'  disabled=""disabled""></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          "
	if action="edit" then Response.write TempList
	Response.write "" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "" & vbcrlf & "          <tr >" & vbcrlf & "            <td  colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addNextStage()"" type=""button"" id=""addList"" value=""添加下一阶段""></td>" & vbcrlf & " </tr>" & vbcrlf & "              </table>" & vbcrlf & "        </td>" & vbcrlf & "    </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "          <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "             <tr>" & vbcrlf & "                  <td  colspan=""6"" class=""gray""><p align=""center"">" & vbcrlf &  "       "
	if action="edit" then
		Response.write "" & vbcrlf & "                         <input name=""button22"" class=""anybutton2"" onClick=""javascript:document.all.demo.action='?action=update&ord="
		Response.write pwurl(ord)
		Response.write "&CurrPage="
		Response.write request("CurrPage")
		Response.write "';""  value="" 保存 "" type=""submit"">" & vbcrlf & "                        "
	else
		Response.write "" & vbcrlf & "                         <input name=""button23"" class=""anybutton2"" value="" 保存 ""  onClick=""javascript:document.all.demo.action='?action=save';"" type=""submit"">" & vbcrlf & "                            "
	end if
	Response.write "" & vbcrlf & "                         <input type=""reset"" value="" 重填 "" class=""page"" name=""B2"">" & vbcrlf & "              </td>" & vbcrlf & "             </tr>" & vbcrlf & "          </table>" & vbcrlf & "" & vbcrlf & "   </td></tr>" & vbcrlf & "    " & vbcrlf & "</td></tr>" & vbcrlf & "</table>" & vbcrlf & "</form> " & vbcrlf & "<iframe name=""saveFrom"" id=""saveFrom"" style=""width:0;height:0""></iframe>" & vbcrlf & "</body>" & vbcrlf & "</html>"
