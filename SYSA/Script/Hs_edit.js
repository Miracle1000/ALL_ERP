...
function getcount(uid){
	...
	for (var i = 0 ; i < boxs.length ; i ++ )	{
		...
		if ((item.type=="text" || item.type=="hidden") && item.id.indexOf("intro_")<0)	{		
			if(item.id.indexOf("YesTax")<0 && item.id.indexOf("NzjTax")<0 && item.id.indexOf("NzjMoney")<0){
				...
				var v3 = v1*v2;				
				if (v5==1) { CutAll += v3; } else { AddAll += v3; }
				thisAll+= v3;
			}else{
	...
	if(TaxID!="" || TaxID2!=""){
		var arr_res = ""; 
		$.get("../hrm/GetTax.asp",
		{ action: "get", ToalMoney: thisAll, ord: $("#hr_salaryClass").attr("ord"), ismode: 1, qy: qy, ToalMoney2: moneyNzj, uid: ord, startDate: window.s_tdDay1, endDate: window.s_TdEndDay, num: Math.random(), ChildrenseDucation: $("#ChildrenseDucation" + ord).val(), ContinuingEducationxl: $("#ContinuingEducationxl" + ord).val(), ContinuingEducationjn: $("#ContinuingEducationjn" + ord).val(), medical: $("#medical" + ord).val(), Housingloans: $("#Housingloans" + ord).val(), payment: $("#payment" + ord).val(), SupportOldPeople: $("#SupportOldPeople" + ord).val() },
		function(data,html){ 
			arr_res = data.split("|");
			thisTax=parseFloat(FormatNumber(parseFloat(arr_res[0]),window.sysConfig.moneynumber));
			thisTax2=parseFloat(FormatNumber(parseFloat(arr_res[1]),window.sysConfig.moneynumber));	
			if(TaxID!=""){
				$("#"+TaxID+"").val(FormatNumber(thisTax,window.sysConfig.moneynumber));
			}
			if(TaxID2!=""){
				$("#"+TaxID2+"").val(FormatNumber(thisTax2,window.sysConfig.moneynumber));
			}
