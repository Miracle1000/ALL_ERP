...
'Public Function AddFormValue(name,  value)
	...
	Sub Bill_OnPageInit
		...
		Response.write Application("sys.info.jsver")
		Response.write """></script>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "//入库详情" & vbcrlf & "function ShowAllXlh(rowIndex){" & vbcrlf & "    var xlh = $("""").html();" & vbcrlf & "      var title = $(""#title""+rowIndex).html();" & vbcrlf & "  var order1 = $(""#order""+rowIndex).html();" & vbcrlf & "  var type1 = $(""#type""+rowIndex).html();" & vbcrlf & "   var wtitle = ""  【产品名称：""+ title + (order1 && order1.length>0?""   产品编号：""+order1 :"""") + (type1 && type1.length>0 ?""   产品型号：""+type1:"""") +""】"";" & vbcrlf & "  var xlhs = xlh.split(""\1"");//序列号个数" & vbcrlf & "   var html = ""<div id='target' class='div_li' style='border:1px solid #ccc;width:98%;height:98%;float:left;margin:5px;overflow:auto;cursor:default;padding-bottom:3px;'>""" & vbcrlf & "    var xlh_lis = new Array();" & vbcrlf & "      for (var i=0;i< xlhs.length ;i++ )" & vbcrlf & "      {       " & vbcrlf & "                xlh_lis[i] = ""<li style='width:100px'>""+xlhs[i]+""</li>"";" & vbcrlf & "     }" & vbcrlf & "       html = html + xlh_lis.join("""") + ""</div>"";" & vbcrlf & "  $('#w').html(html).window({" & vbcrlf & "             title:'序列号管理'+wtitle ," & vbcrlf & "             width:670," & vbcrlf & "              height:420," & vbcrlf & "             top:150+document.body.scrollTop + document.documentElement.scrollTop," & vbcrlf & "                closeable:true," & vbcrlf & "         collapsible:false," & vbcrlf & "              minimizable:false," & vbcrlf & "              maximizable:false," & vbcrlf & "              resizable:true" & vbcrlf & "  }).window('open');" & vbcrlf & "}" & vbcrlf & "//入库申请" & vbcrlf & "function checkXLH(){" & vbcrlf & "    var w = document.getElementById(""i"").value;" & vbcrlf & "       var canSubmit = true ;" & vbcrlf & "  for(var i=1; i<w; i++)" & vbcrlf & "  {" & vbcrlf & "               var xlh = document.getElementById(""xlh""+i);" & vbcrlf & "               if(xlh==null) { continue; }  //xlh为null，说明此行可能被删除了。" & vbcrlf & "                var xlhs = xlh.value;" & vbcrlf & "            if(xlhs.length>0){      " & vbcrlf & "                        var int_num = 0" & vbcrlf & "                 var num = document.getElementById(""num"" + xlh.name.replace(""xlh_"","""")).value;" & vbcrlf & "                 if (num.length==0){" & vbcrlf & "                             num = 0;" & vbcrlf & "                        }else{" & vbcrlf & "                          int_num=parseInt(num);" & vbcrlf & "                            if(int_num<parseFloat(num)){int_num = int_num + 1;}" & vbcrlf & "                     }" & vbcrlf & "                       var xlhs_num = xlhs.split(""\1"").length;//序列号个数" & vbcrlf & "                       //上级要求 序列号可以小于申请入库数量" & vbcrlf & "                   if (int_num<xlhs_num){" & vbcrlf & "                          document.getElementById(""xlhts""+i).innerHTML=""序列号个数[""+ xlhs_num +""]不能大于申请入库数量"";" & vbcrlf & "                            canSubmit = false;" & vbcrlf & "                      }else{" & vbcrlf & "                          document.getElementById(""xlhts""+i).innerHTML="""";" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       resizeDiv();" & vbcrlf & "    return canSubmit;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setContent(rowIndex){ "& vbcrlf &       " if (window.event.keyCode==13){" & vbcrlf &         "  addxlhSQ(rowIndex , true);" & vbcrlf &     "  } "& vbcrlf &" }" & vbcrlf & vbcrlf &" function addxlhSQ(rowIndex ,isKeyDown){" & vbcrlf &     "   var obj = document.getElementById(""xlh""+rowIndex); "&vbcrlf & "  var curr = document.getElementById(""scanxlh""+rowIndex);" & vbcrlf & "   var v = curr.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "        var oldxlh = obj.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "    var xlhs = oldxlh.split(""\1"");//序列号个数" & vbcrlf & "        if (isKeyDown ==false && v.length>0)" & vbcrlf & "  {" & vbcrlf & "               for(var i=0;i<xlhs.length;i++){" & vbcrlf & "                 if (v==xlhs[i]){return;}" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "       var int_num = 0" & vbcrlf & " var num = document.getElementById(""num"" + obj.name.replace(""xlh_"","""")).value;" & vbcrlf & " if (num.length==0){num = 0;}else{int_num=parseInt(num);}" & vbcrlf & "      if (xlhs.length>=int_num && oldxlh.length>0){" & vbcrlf & "           curr.setAttribute(""canevent"" ,""1"");" & vbcrlf & "         curr.value=xlhs[xlhs.length-1];" & vbcrlf & "         document.getElementById(""xlhimg""+rowIndex).src = ""../images/percent100.png"";" & vbcrlf & " }else{" & vbcrlf & "          curr.setAttribute(""canevent"",""0"");" & vbcrlf & "          if (v.length>0)" & vbcrlf & "         {" & vbcrlf & "                       var imgid = ""00"";" & vbcrlf & "                 if (xlhs.length>=int_num){imgid = ""100"";}" & vbcrlf & "                 else if(int_num!=0){" & vbcrlf & "                            if (oldxlh.length==0)" & vbcrlf & "                              {" & vbcrlf & "                                       imgid = (parseInt(1/num * 10) >0 ? parseInt(1/num * 10) + ""0"":""10"");" & vbcrlf & "                                }else{" & vbcrlf & "                                  imgid = (parseInt((xlhs.length+1)/num * 10) >0 ? parseInt((xlhs.length+1)/num * 10) + ""0"":""10"");" & vbcrlf & "                            }" & vbcrlf & "                       }" & vbcrlf & "                       document.getElementById(""xlhimg""+rowIndex).src = ""../images/percent""+ imgid +"".png"";" & vbcrlf & "                        obj.value = oldxlh.length>0? oldxlh + ""\1"" + v : v;" & vbcrlf & "                       if (imgid==""100""){curr.setAttribute(""canevent"",""1"");}" & vbcrlf & "         }" & vbcrlf & "       }" & vbcrlf & "       if(isKeyDown){curr.select();}" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setBlurContent(rowIndex){" & vbcrlf & "      addxlhSQ(rowIndex , false);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "var setObjSQ = null;" & vbcrlf & "function SetAllXlhSQ(rowIndex){" & vbcrlf & " setObjSQ = document.getElementById(""xlh""+rowIndex);" & vbcrlf & "      var xlh = setObjSQ.value;" & vbcrlf & "       var title =document.getElementById(""rkmx_title_td""+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "      var order1 =document.getElementById(""rkmx_order1_td""+rowIndex)? document.getElementById(""rkmx_title_td""+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'') : """";" & vbcrlf & "      var type1 = document.getElementById(""rkmx_type1_td""+rowIndex)? document.getElementById(""rkmx_type1_td""+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'') : """";" & vbcrlf & " var wtitle = ""  【产品名称：""+ title + (order1.length>0?""      产品编号：""+order1 :"""") + (type1.length>0 ?""   产品型号：""+type1:"""") +""】"";" & vbcrlf & "       var xlhs = xlh.split(""\1"");//序列号个数" & vbcrlf & "   var html = ""<table style='width:100%;height:100%;'><tr><td style='width:23%;'>""" & vbcrlf & "   html += ""<div style='height:90%;margin:2px;'><textarea id='lrxlh' style='height:100%;border:1px solid #ccc;overflow:auto;cursor:default'></textarea></div>"";" & vbcrlf & "       html += ""<div style='width:100%;text-align:right;margin-right:2px;margin-top:3px;'>"";" & vbcrlf & "     html += "" <input type='button' onclick='addxlh()' class='page' value='加入'>"";" & vbcrlf & "    html += "" <input type='button' onclick='deleteaddxlh()' class='page' value='重置'>"";" & vbcrlf & " html += ""</div>""  " & vbcrlf & "        html += ""</td><td style='width:77%;'>"";" & vbcrlf & "   html += ""<div id='target' class='div_li' style='height:89%;margin:3px;margin-left:0px;border:1px solid #ccc;overflow:auto;cursor:default;padding-bottom:3px;' >""" & vbcrlf & "      var xlh_lis = new Array();" & vbcrlf & "      var num = document.getElementById(""num"" + setObjSQ.name.replace(""xlh_"","""")).value ;" & vbcrlf & "   var xlhnum = 0" & vbcrlf & "  if (xlh.length>0)" & vbcrlf & "       {" & vbcrlf & "               for (var i=0;i< xlhs.length ;i++ )" & vbcrlf & "            {       " & vbcrlf & "                        xlh_lis[i] = ""<li style='width:100px;position:relative' onmouseover='showdel(this)' onmouseout='hiddendel(this)'><img title='删除' style='width:10px;display:none;right:1px;top:1px;color:red;position:absolute' onclick='deleteli(this)' src='../images/delete.jpg'/>""+xlhs[i]+""</li>"";" & vbcrlf & "             }" & vbcrlf & "               xlhnum = xlhs.length;" & vbcrlf & "   }" & vbcrlf & "       html += xlh_lis.join("""");" & vbcrlf & " html += ""</div>"";" & vbcrlf & " html += ""<div style='width:100%;text-align:right;margin-right:10px;margin-top:5px;'>"";" & vbcrlf & "    html += "" 入库申请数量：<span class='gray'>"" + num + ""</span> "";" & vbcrlf & "   html += "" 当前序列号数量：<span id='xlhnum' class='gray'>""+ xlhnum +""</span>&nbsp;&nbsp;<input type='button' onclick='deleteallxlh()' class='page' value='清空'>"";" & vbcrlf & "  html += ""</div>"";" & vbcrlf & " html += ""</td></tr></table>"";" & vbcrlf & "" & vbcrlf & " $('#w').html(html).window({" & vbcrlf & "             title:'序列号管理'+wtitle ," & vbcrlf & "             width:730," & vbcrlf & "              height:420," & vbcrlf & "             top:150+document.body.scrollTop + document.documentElement.scrollTop," & vbcrlf & "           closeable:true," & vbcrlf & "         collapsible:false," & vbcrlf & "              minimizable:false," & vbcrlf & "              maximizable:false," & vbcrlf & "              modal:true," & vbcrlf & "             resizable:true," & vbcrlf & "         onClose:function(){" & vbcrlf & "                             var x1 = new Array();" & vbcrlf & "                           var i = 0;" & vbcrlf & "                              $(""#target"").find(""li"").each(function(){" & vbcrlf & "                                 x1[i] = $(this).text();" & vbcrlf & "                                 i++;" & vbcrlf & "                            });" & vbcrlf & "                             if (i>0)"&_
		vbcrlf & "                                {" & vbcrlf & "                                       setObjSQ.value = x1.join(""\1"");" & vbcrlf & "                                   var imgid = ""00"";" & vbcrlf & "                                 var num = document.getElementById(""num"" + setObjSQ.name.replace(""xlh_"","""")).value;" & vbcrlf & "                                    if (num.length==0){num = 0;}" & vbcrlf & "                                    if (i>=parseInt(num)){imgid = ""100"";}" & vbcrlf & "                                     else if(parseInt(num)!=0){" & vbcrlf & "                                              imgid = (parseInt(i/num * 10)>0 ? parseInt(i/num * 10) + ""0"":""10"");" & vbcrlf & "                                 }" & vbcrlf & "document.getElementById(""xlhimg""+setObjSQ.id.replace(""xlh"","""")).src = ""../images/percent""+ imgid +"".png"";" & vbcrlf & "                                 document.getElementById(""scanxlh""+setObjSQ.id.replace(""xlh"","""")).value = x1[x1.length-1];" & vbcrlf & "                             }else{" & vbcrlf & "                                  setObjSQ.value = """";"& vbcrlf & "                                       document.getElementById(""scanxlh""+setObjSQ.id.replace(""xlh"","""")).value = """";" & vbcrlf & "                                    document.getElementById(""xlhimg""+setObjSQ.id.replace(""xlh"","""")).src = ""../images/percent100.png"";" & vbcrlf & "                               }" & vbcrlf & "               }" & vbcrlf & "       }).window('open');" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "//入库审批" & vbcrlf & "function checkAllXLH(extIndex){" & vbcrlf & " var canSubmit = true ;" & vbcrlf & "  for (var i = 0 ; i<lv.Rows.length ;  i++)" & vbcrlf & "       {" & vbcrlf & "               var v = lv.Rows[i].Cells[12+extIndex].value;" & vbcrlf & "            if (v.length>0)" & vbcrlf & "         {       " & vbcrlf & "                        var num = lv.Rows[i].Cells[8+extIndex].value;" & vbcrlf & "                   var int_num = 0;" & vbcrlf & "                        if (num.length==0){" & vbcrlf & "                             num = 0;" & vbcrlf & "                        }else{" & vbcrlf & "                          int_num=parseInt(num);" & vbcrlf & "                          if(int_num<parseFloat(num)){int_num = int_num + 1;}" & vbcrlf & "                  }" & vbcrlf & "                       var xlhs_num = v.split(""\1"").length;" & vbcrlf & "                      //序列号个数" & vbcrlf & "                    //上级要求 序列号可以小于申请入库数量" & vbcrlf & "                   if (int_num<xlhs_num)" & vbcrlf & "                   {" & vbcrlf & "                               lv.Rows[i].Cells[12+extIndex].isError = true;" & vbcrlf & "                           if (canSubmit==true)" & vbcrlf & "{             "           & vbcrlf &                                "         app.Alert(""第""+(i+1)+""行[""+lv.Headers[12].innerHTML+""]列数据不合法：序列号个数[""+ xlhs_num +""]不能大于入库数量"");" & vbcrlf &                              "          lv.startIdx=(i>(lv.Rows.length-lv.RowsPerPage))?(lv.Rows.length>lv.RowsPerPage?lv.Rows.length-lv.RowsPerPage:0):i;" & vbcrlf &                                      " if(lv.Rows[i].Cells[12+extIndex].obj)" & vbcrlf & "                                     {" & vbcrlf & "                                               var tr=lv.Rows[i].Cells[12+extIndex].obj.parentElement;" & vbcrlf & "                                         //tr.fireEvent(""onclick"");" & vbcrlf & "                                                $(tr).trigger(""click"");" & vbcrlf & "                                           lv.EditRow=tr.rowIndex;" & vbcrlf & "                                         lv.RefreshRow(lv.EditRow);" & vbcrlf & "                                     }" & vbcrlf & "                               }" & vbcrlf & "                               canSubmit = false;" & vbcrlf & "                      }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       lv.RefreshContent();" & vbcrlf & "    return canSubmit;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "var setObjZZ = null;" & vbcrlf & "function SetAllXlhZZ(rowIndex, xlhExt, xlhNameExt,cpTitleExt,order1Ext,type1Ext,numExt){" & vbcrlf & "  setObjZZ = document.getElementById(xlhExt+rowIndex);" & vbcrlf & "    var xlh = setObjZZ.value;" & vbcrlf & "       var title = """";" & vbcrlf & "   if(document.getElementById(cpTitleExt+rowIndex)){" & vbcrlf & "               title = document.getElementById(cpTitleExt+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "       }else{" & vbcrlf & "          document.getElementById(cpTitleExt).innerText.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "     }" & vbcrlf & "       var order1 =document.getElementById(order1Ext+rowIndex)? document.getElementById(order1Ext+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'') : """";" & vbcrlf & "     var type1 = document.getElementById(type1Ext+rowIndex)? document.getElementById(type1Ext+rowIndex).innerText.replace(/(^\s*)|(\s*$)/g,'') : """";" & vbcrlf & "   var wtitle = ""  【产品名称：""+ title + (order1.length>0?""      产品编号：""+order1 :"""") + (type1.length>0 ?""   产品型号：""+type1:"""") +""】""; "& vbcrlf & "  var xlhs = xlh.split(""\1"");//序列号个数"& vbcrlf &    "var html = ""<table stylestyle='height:305px;border:1px solid #ccc;overflow:auto;cursor:default'></textarea></div>"";" & vbcrlf & "      html += ""<div style='width:100%;text-align:right;margin-right:2px;margin-top:3px;'>"";" & vbcrlf & "     html += "" <input type='button' onclick='addxlh()' class='zb-button' value='加入'>"";" & vbcrlf & "     html += "" <input type='button' onclick='deleteaddxlh()' class='zb-button' value='重置'>"";" & vbcrlf & " html += ""</div>""  " & vbcrlf & "        html += ""</td><td style='width:77%;' valign='top'>"";" & vbcrlf & "      html += ""<div id='target' class='div_li' style='height:305px;margin:3px;margin-left:0px;border:1px solid #ccc;overflow:auto;cursor:default;padding-bottom:3px;' >""" & vbcrlf & "    var xlh_lis = new Array();" & vbcrlf & "      var num = document.getElementById(numExt + setObjZZ.name.replace(xlhNameExt,"""")).value ;" & vbcrlf & "  var xlhnum = 0" & vbcrlf & "  if (xlh.length>0)" & vbcrlf & "{" & vbcrlf & "               for (var i=0;i< xlhs.length ;i++ )" & vbcrlf & "              {       " & vbcrlf & "                        xlh_lis[i] = ""<li style='width:100px;position:relative' onmouseover='showdel(this)' onmouseout='hiddendel(this)'><img title='删除' style='width:10px;display:none;right:1px;top:1px;color:red;position:absolute' onclick='deleteli(this)' src='../images/delete.jpg'/>""+xlhs[i]+""</li>"";" & vbcrlf & "          }" & vbcrlf & "               xlhnum = xlhs.length;" & vbcrlf & "   }" & vbcrlf & "       html += xlh_lis.join("""");" & vbcrlf & " html += ""</div>"";" & vbcrlf & " html += ""<div style='width:100%;text-align:right;margin-right:10px;margin-top:5px;'>"";" & vbcrlf & " html += "" 入库申请数量：<span class='gray'>"" + num + ""</span> "";" & vbcrlf & "    html += "" 当前序列号数量：<span id='xlhnum' class='gray'>""+ xlhnum +""</span>&nbsp;&nbsp;<input type='button' onclick='deleteallxlh()' class='zb-button' value='清空'>"";" & vbcrlf & "     html += ""</div>"";" & vbcrlf & "       html += ""</td></tr></table>"";" & vbcrlf & "" & vbcrlf & "     $('#w').html(html).window({" & vbcrlf & "             title:'序列号管理'+wtitle ," & vbcrlf & "             width:730," & vbcrlf & "              height:420," & vbcrlf & "             top:150+document.body.scrollTop + document.documentElement.scrollTop," & vbcrlf & "           closeable:true," & vbcrlf & "                collapsible:false," & vbcrlf & "              minimizable:false," & vbcrlf & "              maximizable:false," & vbcrlf & "              modal:true," & vbcrlf & "             resizable:true," & vbcrlf & "         onClose:function(){" & vbcrlf & "                             var x1 = new Array();" & vbcrlf & "                           var i = 0;" & vbcrlf & "                              $(""#target"").find(""li"").each(function(){" & vbcrlf & "                                  x1[i] = $(this).text();" & vbcrlf & "                                 i++;" & vbcrlf & "                            });" & vbcrlf & "                             if (i>0)" & vbcrlf & "                                {" & vbcrlf & "                                       setObjZZ.value = x1.join(""\1"");" & vbcrlf & "                                   var imgid = ""00"";" & vbcrlf & "                                 var num = document.getElementById(numExt + setObjZZ.name.replace(xlhNameExt,"""")).value;" & vbcrlf & "                                       if (num.length==0){num = 0;}" & vbcrlf & "                                    if (i>=parseInt(num)){imgid = ""100"";}" & vbcrlf & "                                     else if(parseInt(num)!=0){" & vbcrlf & "                                              imgid = (parseInt(i/num * 10)>0 ? parseInt(i/num * 10) + ""0"":""10""); "& vbcrlf &                                "    } "& vbcrlf &                                  "      document.getElementById(""xlhimg""+setObjZZ.id.replace(xlhExt,"""")).src = ""../images/percent""+ imgid +"".png""; "& vbcrlf &                        "               document.getElementById(""scanxlh""+setObjZZ.id.replace(xlhExt,"""")).value = x1[x1.length-1];" & vbcrlf &                        "   }else{ "& vbcrlf & "                                 setObjZZ.value = """";" & vbcrlf & "                                      document.getElementById(""scanxlh""+setObjZZ.id.replace(xlhExt,"""")).value = """";" & vbcrlf & "                                 document.getElementById(""xlhimg""+setObjZZ.id.replace(xlhExt,"""")).src = ""../images/percent100.png"";" & vbcrlf & "                            }" & vbcrlf & "               }"& vbcrlf & "       }).window('open');" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setContentZZ(rowIndex, xlhExt, xlhNameExt,cpTitleExt,order1Ext,type1Ext,numExt){"&_
		vbcrlf & "    if (window.event.keyCode==13){" & vbcrlf & "          addxlhSQZZ(rowIndex , true, xlhExt, xlhNameExt,cpTitleExt,order1Ext,type1Ext,numExt);" & vbcrlf & "           return false;" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setBlurContentZZ(rowIndex, xlhExt, xlhNameExt,numExt){" & vbcrlf & "       addxlhSQZZ(rowIndex , false, xlhExt, xlhNameExt,numExt);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function addxlhSQZZ(rowIndex ,isKeyDown, xlhExt, xlhNameExt,numExt){" & vbcrlf & "  var obj = document.getElementById(xlhExt+rowIndex);" & vbcrlf & "     var curr = document.getElementById(""scanxlh""+rowIndex);" & vbcrlf & "   var v = curr.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "        var oldxlh = obj.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "  var xlhs = oldxlh.split(""\1"");//序列号个数" & vbcrlf & "        if (isKeyDown ==false && v.length>0)" & vbcrlf & "    {" & vbcrlf & "               for(var i=0;i<xlhs.length;i++){" & vbcrlf & "                 if (v==xlhs[i]){return;}" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "       var int_num = 0" & vbcrlf & " var num = document.getElementById(numExt + obj.name.replace(xlhNameExt,"""")).value;" & vbcrlf & "       if (num.length==0){num = 0;}else{int_num=parseInt(num);}" & vbcrlf & "        if (xlhs.length>=int_num && oldxlh.length>0){" & vbcrlf & "           curr.setAttribute(""canevent"" ,""1"");" & vbcrlf & "         curr.value=xlhs[xlhs.length-1];" & vbcrlf & "              document.getElementById(""xlhimg""+rowIndex).src = ""../images/percent100.png"";" & vbcrlf & "        }else{" & vbcrlf & "          curr.setAttribute(""canevent"",""0"");" & vbcrlf & "          if (v.length>0)" & vbcrlf & "         {" & vbcrlf & "                       var imgid = ""00"";" & vbcrlf & "                 if (xlhs.length>=int_num){imgid = ""100"";}" & vbcrlf & "                   else if(int_num!=0){" & vbcrlf & "                            if (oldxlh.length==0)" & vbcrlf & "                           {" & vbcrlf & "                                       imgid = (parseInt(1/num * 10) >0 ? parseInt(1/num * 10) + ""0"":""10"");" & vbcrlf & "                                }else{" & vbcrlf & "                                  imgid = (parseInt((xlhs.length+1)/num * 10) >0 ? parseInt((xlhs.length+1)/num * 10) + ""0"":""10"");" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf & "                       document.getElementById(""xlhimg""+rowIndex).src = ""../images/percent""+ imgid +"".png"";" & vbcrlf & "                  obj.value = oldxlh.length>0? oldxlh + ""\1"" + v : v;" & vbcrlf & "                       if (imgid==""100""){curr.setAttribute(""canevent"",""1"");}" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "       if(isKeyDown){curr.select();}" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "var setObj = null;" & vbcrlf & "var scanObjTD= null;" & vbcrlf & "function SetAllXlh(obj,extIndex){" & vbcrlf & "   scanObjTD = obj.parentElement;" & vbcrlf & "  setObj = obj.parentElement.obj;" & vbcrlf & " var rowidx=obj.parentElement.parentElement.rowIndex;" & vbcrlf & "    var xlh = setObj.value;" & vbcrlf & " var title = setObj.getParent().Cells[2].obj.innerText.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "     var order1 =setObj.getParent().Cells[4].text;" & vbcrlf & " var type1 = setObj.getParent().Cells[5].text;" & vbcrlf & "   var wtitle = ""  【产品名称：""+ title + (order1.length>0?""      产品编号：""+order1 :"""") + (type1.length>0 ?""   产品型号：""+type1:"""") +""】"";" & vbcrlf & "  var xlhs = xlh.split(""\1"");//序列号个数" & vbcrlf& "        var html = ""<table style='width:100%;height:100%;'><tr><td style='width:23%;'>""" & vbcrlf & "   html += ""<div style='height:82%;margin:3px;*margin-top:2px'><textarea id='lrxlh' style='height:100%;border:1px solid #ccc;overflow:auto;cursor:default'></textarea></div>"";" & vbcrlf & "       html += ""<div style='width:100%;text-align:right;margin-right:2px;margin-right:3px;'>"";" & vbcrlf & "     html += "" <input type='button' onclick='addxlh()' class='page' value='加入'>"";" & vbcrlf & "    html += "" <input type='button' onclick='deleteaddxlh()' class='page' value='重置'>"";" & vbcrlf & "      html += ""</div>""        " & vbcrlf & "        html += ""</td><td style='width:77%;'>"";" & vbcrlf & "   html += ""<div id='target' class='div_li' style='height:82%;margin:3px;margin-left:0px;border:1px solid #ccc;overflow:auto;cursor:default;padding-bottom:3px;*padding-bottom:0' >""" & vbcrlf & " var num = setObj.getParent().Cells[8+extIndex].value;" & vbcrlf & "       var xlhnum = 0" & vbcrlf & "  var xlh_lis = new Array();" & vbcrlf & "      if (xlh.length>0)" & vbcrlf & "       {" & vbcrlf & "               for (var i=0;i< xlhs.length ;i++ )" & vbcrlf & "              {       " & vbcrlf & "                        xlh_lis[i] = ""<li style='width:100px;position:relative' onmouseover='showdel(this)' onmouseout='hiddendel(this)'><img title='删除' style='width:10px;right:1px;top:1px;color:red;display:none;position:absolute' onclick='deleteli(this)' src='../images/delete.jpg'/>""+xlhs[i]+""</li>"";" & vbcrlf & "              }" & vbcrlf & "               xlhnum = xlhs.length;" & vbcrlf & "   }" & vbcrlf & "       html += xlh_lis.join("""");" & vbcrlf & "      html += ""</div>"";" & vbcrlf & " html += ""<div style='width:100%;text-align:right;margin-right:10px;margin-top:5px;'>"";" & vbcrlf & "    html += "" 入库申请数量：<span class='gray'>"" + num + ""</span> "";" & vbcrlf & "    html += "" 当前序列号数量：<span id='xlhnum' class='gray'>""+xlhnum +""</span>&nbsp;&nbsp;<input type='button' onclick='deleteallxlh()' class='page' value='清空'>"";" & vbcrlf & "        html += ""</div>"";" & vbcrlf & " html += ""</td></tr></table>"";" & vbcrlf & "     $('#w').html(html).window({" & vbcrlf & "             title:'序列号管理'+wtitle ," & vbcrlf & "             width:730," & vbcrlf & "         height:420," & vbcrlf & "             top:150+document.body.scrollTop + document.documentElement.scrollTop," & vbcrlf & "           closeable:true," & vbcrlf & "         collapsible:false," & vbcrlf & "              minimizable:false," & vbcrlf & "              maximizable:false," & vbcrlf & "              modal:true," & vbcrlf & "             resizable:true," & vbcrlf & "              onClose:function(){" & vbcrlf & "                             var x1 = new Array();" & vbcrlf & "                           var i = 0;" & vbcrlf & "                              $(""#target"").find(""li"").each(function(){" & vbcrlf & "                                    x1[i] = $(this).text();" & vbcrlf & "                                 i++;" & vbcrlf & "                            });" & vbcrlf & "                             if (i>0)" & vbcrlf & "                                {" & vbcrlf & "setObj.value = x1.join(""\1""); "& vbcrlf &                                     " var imgid = ""00"";" & vbcrlf &                                 " var num = setObj.getParent().Cells[8+extIndex].value;" & vbcrlf &                                  "  if (num.length==0){num = 0;} "& vbcrlf &                               "      if (i>=parseInt(num)){imgid = ""100"";} "& vbcrlf &                                     " else if(parseInt(num)!=0){" & vbcrlf & "                                         imgid = (parseInt(i/num * 10)>0 ? parseInt(i/num * 10) + ""0"":""10"");" & vbcrlf & "                                 }" & vbcrlf & "                                       $(scanObjTD).find(""img"")[0].src = ""../images/percent""+ imgid +"".png"";" & vbcrlf & "                                 $(scanObjTD).find(""input"")[0].value = x1[x1.length-1];" & vbcrlf & "                            }else{" & vbcrlf & "                                    setObj.value = """";" & vbcrlf & "                                        $(scanObjTD).find(""input"")[0].value = """";" & vbcrlf & "                                   $(scanObjTD).find(""img"")[0].src = ""../images/percent100.png"";" & vbcrlf & "                               }" & vbcrlf & "               }" & vbcrlf & "       }).window('open');" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setXlh(curr,extIndex){" & vbcrlf & "      if (window.event.keyCode==13){" & vbcrlf & "          setxlhSP(curr ,true,extIndex);" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setxlhSP(curr ,isKeyDown,extIndex){" & vbcrlf & "  var obj = curr.parentElement.obj;" & vbcrlf & "       var v = curr.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & " var oldxlh = obj.value.replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "    var xlhs = oldxlh.split(""\1"");//序列号个数" & vbcrlf & "        if (isKeyDown ==false && v.length>0)" & vbcrlf & "    {" & vbcrlf & "               for(var i=0;i<xlhs.length;i++){" & vbcrlf & "                 if (v==xlhs[i]){return;}" & vbcrlf & "         }" & vbcrlf & "       }" & vbcrlf & "       var cfxlh = checkXlhExists(v);" & vbcrlf & "  if(cfxlh != """"){" & vbcrlf & "          app.Alert(""序列号【""+ v +""】存在重复！"");" & vbcrlf & "           return;" & vbcrlf & " }" & vbcrlf & "       var int_num = 0" & vbcrlf & " var num = obj.getParent().Cells[8+extIndex].value;//数量" & vbcrlf & "       if (num.length==0){num = 0;}else{int_num=parseInt(num);}" & vbcrlf & "        if (xlhs.length>=int_num && oldxlh.length>0){" & vbcrlf & "           curr.setAttribute(""canevent"" ,""1"");" & vbcrlf & "         curr.value=xlhs[xlhs.length-1];" & vbcrlf & "         $(curr.parentElement).find(""img"")[0].src = ""../images/percent100.png"";" & vbcrlf & "      }else{" & vbcrlf & "          curr.setAttribute(""canevent"",""0"");" & vbcrlf & "          if (v.length>0)" & vbcrlf & "         {" & vbcrlf & "                       var imgid = ""00"";" & vbcrlf & "                 if (xlhs.length>=int_num){imgid = ""100"";}" & vbcrlf & "                 else if(int_num!=0){" & vbcrlf & "                               if (oldxlh.length==0)" & vbcrlf & "                           {" & vbcrlf & "                                       imgid = (parseInt(1/num * 10) >0 ? parseInt(1/num * 10) + ""0"":""10"");" & vbcrlf & "                                }else{" & vbcrlf & "                                  imgid = (parseInt((xlhs.length+1)/num * 10) >0 ? parseInt((xlhs.length+1)/num * 10) + ""0"":""10"");" & vbcrlf & "                    }" & vbcrlf & "                       }" & vbcrlf & "                       $(curr.parentElement).find(""img"")[0].src = ""../images/percent""+ imgid +"".png"";" & vbcrlf & "                        obj.value = oldxlh.length>0? oldxlh + ""\1"" + v : v;" & vbcrlf & "                       if (imgid==""100""){curr.setAttribute(""canevent"",""1"");}" & vbcrlf & "         }" & vbcrlf & "       }"& vbcrlf &     " if(isKeyDown){curr.select();} "& vbcrlf &" } "& vbcrlf & vbcrlf & "function setBlurXlh(curr,extIndex){ "& vbcrlf &       " setxlhSP(curr ,false , extIndex); "& vbcrlf &" } "& vbcrlf & vbcrlf &"function scanEvent(){ "& vbcrlf &"  if (window.event.keyCode==13){addxlh();}"&_
		"}" & vbcrlf & "" & vbcrlf & "function addxlh(){" & vbcrlf & "      var v = $(""#lrxlh"").val();" & vbcrlf & "        if (v.length>0)" & vbcrlf & " {       " & vbcrlf & "                var xlhStr1 = """";" & vbcrlf & "         var xlhExists = 0;" & vbcrlf & "              var cfxlh = """";" & vbcrlf & "           var arrli = v.split(/\n/g);" & vbcrlf & "     var i=0;" & vbcrlf & "                for(i=0;i<arrli.length ;i++){" & vbcrlf & "                   var v1 = arrli[i].replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "                 if(v1 != """"){" & vbcrlf & "                             if((""\2""+xlhStr1+""\2"").indexOf(""\2""+v1+""\2"")==-1){" & vbcrlf & "                                      xlhStr1 += (xlhStr1==""""?"""":""\2"") + v1;" & vbcrlf & "                                }else{                          " & vbcrlf & "                                        xlhExists = 1;" & vbcrlf & "                                  break;" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               if(xlhExists == 1){" & vbcrlf & "                     app.Alert(""序列号【""+ v1 +""】存在重复！"");" & vbcrlf & "                  return;" & vbcrlf & "         }" & vbcrlf & "               var targetXlh = """";" & vbcrlf & "          $(""#target"").children().each(function(){" & vbcrlf & "                  targetXlh += (targetXlh==""""?"""":""\2"") + $(this).text();" & vbcrlf & "                });" & vbcrlf & "             for(i=0;i<arrli.length ;i++){" & vbcrlf & "                   var v1 = arrli[i].replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "                 if(v1 != """"){" & vbcrlf& "                               if((""\2""+targetXlh+""\2"").indexOf(""\2""+v1+""\2"")>-1){" & vbcrlf & "                                     xlhExists = 1;" & vbcrlf & "                                  break;" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               if(xlhExists == 1){" & vbcrlf & "                     app.Alert(""序列号【""+ v1 +""】存在重复！"");" & vbcrlf & "                  return;" & vbcrlf & "              }" & vbcrlf & "               cfxlh = checkXlhExists(xlhStr1);" & vbcrlf & "                if(cfxlh != """"){" & vbcrlf & "                  app.Alert(""序列号【""+ cfxlh +""】存在重复！"");" & vbcrlf & "                       return;" & vbcrlf & "         }" & vbcrlf & "" & vbcrlf & "               for (i=0;i<arrli.length ;i++ )" & vbcrlf & "          {" & vbcrlf & "                       var v1 = arrli[i].replace(/(^\s*)|(\s*$)/g,'');" & vbcrlf & "                        if (v1.length>0)" & vbcrlf & "                        {" & vbcrlf & "                               $(""<li style='width:100px;position:relative' onmouseover='showdel(this)' onmouseout='hiddendel(this)'><img style='width:10px;right:1px;top:1px;color:red;display:none;position:absolute' onclick='deleteli(this)' title='删除' src='../images/delete.jpg'/>""+v1+""</li>"").appendTo($(""#target""));" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               $(""#lrxlh"").val("""");" & vbcrlf & "                $("""").text($(""#target"").children().size());" & vbcrlf & "  }" & vbcrlf & "       $(""#lrxlh"")[0].focus();" & vbcrlf& "}" & vbcrlf & "" & vbcrlf & "function checkXlhExists(xlhStr){" & vbcrlf & "      var ret = """";" & vbcrlf & "     var xlhExists = 0;" & vbcrlf & "      $.ajax({" & vbcrlf & "                url:'../store/CommonReturn.asp'," & vbcrlf & "                data:{act:'CheckXlhExists',xlhStr:xlhStr}," & vbcrlf & "              async:false," & vbcrlf & "    type:'post'," & vbcrlf & "            success:function(r){" & vbcrlf & "                    var arr_ret = r.split(""\2"");" & vbcrlf & "                      if(arr_ret[0]==""1""){" & vbcrlf & "                              ret = arr_ret[1];" & vbcrlf & "                               xlhExists = 1;" & vbcrlf & "                          return false;" & vbcrlf & "                   }" & vbcrlf & "               }" & vbcrlf & "       });     " & vbcrlf & "return ret; "& vbcrlf & "}" & vbcrlf & vbcrlf &" function deleteaddxlh(){" & vbcrlf &   "  $(""#lrxlh"").val("""");" & vbcrlf &      "   $(""#lrxlh"")[0].focus();" & vbcrlf & "}" & vbcrlf & vbcrlf &" function deleteallxlh(){" & vbcrlf & "  if(confirm(""确认清空？"")){" & vbcrlf &                 "$(""#target li"").remove();" & vbcrlf & "             $("""").text($(""#target"").children().size());  " & vbcrlf & "        }" & vbcrlf & "       $(""#lrxlh"")[0].focus();" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function showdel(obj){" & vbcrlf & "    var $close =$(obj).find(""img"");" & vbcrlf & "   $close.css({display:""block""});" & vbcrlf & "}" & vbcrlf & "function hiddendel(obj){" & vbcrlf & "  var $close =$(obj).find(""img"");" & vbcrlf & "   $close.css({display:""none""});" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function deleteli(obj){" & vbcrlf & "     $(obj).parent().remove();" & vbcrlf & "       $("""").text($(""#target"").children().size());" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "<div id=""w""></div>"
		
		Response.write "" & vbcrlf & "      <style>" & vbcrlf & " #test-window-asdas{height:370px!important;overflow-y:hidden}" & vbcrlf & "    html{padding: 10px 10px 0;background: #efefef;box-sizing:border-box;}" & vbcrlf & "   body{background: #ffffff;}" & vbcrlf & "      </style>" & vbcrlf & "    <script>" & vbcrlf & "        window.isFixAssRat="
		Response.write isFixAssRat
		Response.write "" & vbcrlf & "              window.OnDoSaveHack.push(function(){" & vbcrlf & "                    $ID(""mainform"").action = ""../../SYSN/view/store/kuAssemble/packaging.save.ashx"";" & vbcrlf & "            });" & vbcrlf & "    </script>" & vbcrlf & "        "
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		isOpenAssUnit = (GetAssistUnitTactics()&""="1")
		isOpenProductAttrV=isOpenProductAttr
		If initType = "0" Then
			cn.execute(" delete from Bom_Trees where tType=1 and del <> 1 and addcate = " & Info.User & ";" &_
			"delete from Bom_snapshot_Info where del <> 1 and addcate = " & Info.User & ";" &_
			"delete from Bom_snapshot_List where del <> 1 and addcate = " & Info.User & ";")
		end if
		app.addScriptPath("../store/storeselbox.js")
		app.addScriptPath("../store/sczz.js")
		app.addScriptPath("../inc/function.js")
		app.addScriptPath("../Script/store_function.js")
		app.addDefaultScript()
		bill.loadeasyui = True
		bill.title = "组装添加"
		Dim rs, BH
		With Bill.fields.add("组装标题", "title")
		.notnull = True
		.maxlimit = 100
		End With
		set rs = cn.execute("EXEC erp_getdjbh 8041," & Info.User & " ")
		bh=rs(0).value
		set rs=Nothing
		With Bill.fields.add("组装编号", "BH")
		.notnull = True
		.value = BH
		.defvalue = BH
		.maxlimit = 50
		End With
		Dim zzdate : zzdate = now
		If cn.execute("select 1 from M2_CostComputation where  complete1 >=1 and datediff(mm,date1,'"& now &"')=0 ").eof=False Then
			zzdate = cn.execute("select CONVERT(varchar(7), dateadd(mm,1,getdate()) , 120) + '-1'")(0)
		end if
		With Bill.fields.add("组装日期", "addDate")
		.notnull = True
		.value =zzdate
		.defvalue = zzdate
		.uitype = "datetime"
		End With
		With Bill.fields.add("当前树ORD", "currTree")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("当前树type", "treetype")
		.uitype = "hidden"
		.value = 1
		.defvalue = 1
		End With
		With Bill.fields.add("子件ORD", "s_proord")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件单位", "s_unit")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("产品属性1", "s_ProductAttr1")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("产品属性2", "s_ProductAttr2")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件数量", "s_num1")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件单价", "s_price1")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件仓库", "s_ck1ord")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件出库方式", "s_ckfs2")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件指定出库值", "s_ckfs3")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("辅助数量", "s_Assinum")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件包装", "s_bz1")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件件数", "s_js1")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件备注", "s_intro")
		.uitype = "hidden"
		.value = 0
		.defvalue = 0
		End With
		With Bill.fields.add("子件自定义1", "s_zdy1")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件自定义2", "s_zdy2")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件自定义3", "s_zdy3")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件自定义4", "s_zdy4")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件自定义5", "s_zdy5")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		With Bill.fields.add("子件自定义6", "s_zdy6")
		.uitype = "hidden"
		.value = ""
		.defvalue = ""
		End With
		...
		v="money"
		if(app.power.existsPowerIntro(23,3,Info.User)=false) then
		v="hidden"
	end if
	Set rs = cn.execute("select * from zdymx where sort1 = 8040 /*and set_open = 1*/ order by gate1")
	While rs.eof = False
		If rs("name") = "title" Or rs("name") = "ck1" Then
			With Bill.fields.add(rs("title") & "_ord", "p_" & rs("name") & "_h")
			.uitype = "hidden"
			End With
		end if
		With Bill.fields.add(rs("title"), "p_" & rs("name"))
		Select Case rs("name")
		Case "title":
		.onlyread = True
		.notnull = True
		...
		.maxlimit = "200"
		Case "order1","type1":
		.value = "<span id='p_" & rs("name") & "_0'></span>"
		.uitype = "html"
		Case "unitall":
		.uitype = "select"
		.source = "options:请选择单位=NULL"
		.notnull = True
		Case "num1":
		.uitype = "number"
		.value = 1
		...
		.onlyread = True
		.notnull = True
		.js = " onclick='showStoreDlg2(this.id)'"
		Case "ph1":
		.maxlimit = "50"
		Case "xlh1":
		.uitype = "html"
		...
		Case "bz1":
		.uitype = "select"
		.source = "sql:select sort1,ord from sortonehy where gate2 = 82 order by gate1 desc"
		Case "js1":
		.uitype = "number"
		Case "intro":
		.maxlimit = 200
		Case "zdy1","zdy2","zdy3","zdy4":
		.maxlimit = 50
		Case "zdy5":
		.uitype = "select"
		.source = "sql:select sort1,ord from sortonehy where gate2 = 2101 order by gate1 desc"
		Case "zdy6":
		.uitype = "select"
		.source = "sql:select sort1,ord from sortonehy where gate2 = 2102 order by gate1 desc"
		End Select
		If rs("name") = "num1" and isOpenAssUnit Then
			With Bill.fields.add("辅助单位", "p_AssistUnit")
			.uitype = "select"
			.source = "options: =NULL"
			End With
			onlyRead=false
			With Bill.fields.add("辅助数量", "p_AssistNum")
			.uitype = "number"
			.minValue = "0.0001"
			.maxValue = "1000000000000"
			End With
		end if
		If rs("name") = "unitall" Then
			With Bill.fields.add("产品属性1", "p_ProductAttr1")
			.uitype = "html"
			.value = "<span id='p_ProductAttr1_0'></span>"
			if isOpenProductAttrV = false then
				.uitype = "hidden"
			end if
			End With
			With Bill.fields.add("产品属性2", "p_ProductAttr2")
			.uitype = "html"
			.value = "<span id='p_ProductAttr2_0'></span>"
			if isOpenProductAttrV = false then
				.uitype = "hidden"
			end if
			End With
			With Bill.fields.add("p_ProductAttr1_ord", "p_ProductAttr1_h")
			.uitype = "hidden"
			End With
			With Bill.fields.add("p_ProductAttr2_ord", "p_ProductAttr2_h")
			.uitype = "hidden"
			End With
		end if
		If rs("set_open") = "0" Then
			.uitype = "hidden"
		end if
		End With
		rs.movenext
	...
	rs.close
	set rs = nothing
	Call Bill.AddCurrGroup("子件明细 （温馨提示：点击‘仓库’列显示‘批量录入仓库’操作）", "s_ExecBody")
	Dim f
	Set f = Bill.fields.addListView("","clist")
	Call Bill.AddCurrGroup("组装备注", "r_ExecBody")
	With Bill.fields.add("", "remark")
	.colspan = 3
	.value = ""
	.defvalue = ""
	.uitype = "editor"
	...
	Sub XlhHtmlSub(ByVal field)
		Response.write "<div class='sub-field' id=""p_xlh1_div""><input type='text' isfield=""1"" canevent='0' id='scanxlh0' onkeydown=""setContentZZ(0,'p_xlh1_','p_xlh1','p_num1_0')"" onblur=""setBlurContentZZ(0,'p_xlh1_','p_xlh1','p_num1_0')"" onfocus='this.select()'> <img id='xlhimg0' src='../images/percent100.png' title='序列号管理' style='width:12px;height:18px;cursor:hand;vertical-align:top' onclick=""SetAllXlhZZ(0,'p_xlh1_','p_xlh1','p_title_div','p_order1_','p_type1_','p_num1_0')""><span id='xlhts0' style='color:red'></span><input name='p_xlh1' id='p_xlh1_0' type='text' size=""0"" style=""width:0px;visibility:hidden;"" value=""""><input class='notnull' style='display:none' title='必填' type='button' value='*'></div>"
	end sub
	...
	lvw.allsum = true
	lvw.indexbox = True
	lvw.id = "packing_proList"
	lvw.jsoneditmodel = True
	lvw.edit.canadd = False
	lvw.edit.candel = False
	lvw.colresize = True
	lvw.headers("@@序号").width = 65
	Dim rs, sql, i
	sql = "exec bom_packaging_Prolist " & Info.User & ",1"
	i = 0
	Set rs = cn.execute("select * from zdymx where sort1 = 8041 /*and set_open = 1*/ order by gate1")
	lvw.sql = sql
	While rs.eof = False
		With lvw.headers(rs("name"))
		.title = rs("title")':Response.write(rs("title") & "<br />")
		.uitype = "text"
		.width = (rs("kd") * 1)
		Select Case rs("name")
		Case "title","order1","type1":
		.uitype = "html"
		.canBatchInput = 1
		If rs("name") = "title" Then
			.formula = "getTitleLink(rs)"
		end if
		Case "unitall":
		.uitype = "html"
		.align = "center"
		...
		.onlyread = True
		.canBatchInput = 0
		.source = "script:showStoreDlg1(this.parentElement.children[0].id)"
		Case "kcxx1":
		.uitype = "html"
	...
	.display = "none"
	End With
	With lvw.headers("ck1Ord")
	.uitype = "text"
	.display = "none"
	End With
	With lvw.headers("snOrd")
	.display = "none"
	End With
	With lvw.headers("num2")
	.display = "none"
	End With
	With lvw.headers("total2")
	.display = "none"
	End With
	With lvw.headers("price2")
	.display = "none"
	End With
	With lvw.headers("ProductAttr1")
	.display = "none"
	End With
	With lvw.headers("ProductAttr2")
	.display = "none"
	.canBatchInput=1
	End With
	With lvw.headers("ckfs2")
	.title = "出库方式值"
	.uitype = "text"
	.display = "none"
	End With
	With lvw.headers("ckfs3")
	.title = "指定出库值"
	.uitype = "text"
	.display = "none"
	End With
	With lvw.headers("Assinum")
	.display = "none"
	End With
	With lvw.headers("ckfs4")
	.title = "指定出库总的指定数量"
	.uitype = "text"
	.display = "none"
	End With
	...
	Sub lvw_OnLoadUserConfig(ByVal lvw, ByRef colmaps)
		Dim i, rs
		i = 1
		Set rs = cn.execute("select * from zdymx where sort1 = 8041 /*and set_open = 1*/ order by gate1")
		While rs.eof = False
...
'While rs.eof = False
	...
	Function Bill_OnSave(ByVal bill)
		Dim title, BH, addDate, remark, currTree, treetype, s_proord, s_unit, s_num1, s_price1, s_ck1ord, s_ckfs2, s_ckfs3, s_bz1, s_js1, s_intro, s_zdy1, s_zdy2, s_zdy3, s_zdy4, s_zdy5, s_zdy6, isDel,s_Assinum
		Dim rs, rs1, rs2, ckbh, title1, kuout, conn
		set conn = cn
		title               = Replace(app.gettext("title"),"'","''")
		BH                  = app.gettext("BH")
		addDate             = Replace(app.gettext("addDate"),"'","''")
		remark              = Replace(app.gettext("remark"),"'","''")
		If Len(addDate)>0 and conn.execute("select 1 from M2_CostComputation where  complete1 >=1 and datediff(mm,date1,'"& addDate &"')=0 ").eof=false Then
			Call bill.alert("组装日期对应的成本核算月已核算，请重新选择日期！")
			Exit Function
		end if
		Set rs = cn.execute("select * from kuzz where zzid = '" & BH & "'")
		If rs.eof = False Then
			Call bill.alert("组装编号已存在！")
			Exit Function
		end if
		rs.close
		set rs = nothing
		Dim sql
		sql = "insert into kuzz (title, complete1, zzid, intro, cateid, cateid2, cateid3, date3, date7, del,zztype, isnew) " &_
		"select '" & title & "', 3, '" & BH & "', '" & remark & "', ord, sorce, sorce2, '" & addDate & "', GETDATE(), 1, 0, 1 from gate where ord = " & Info.User
		cn.execute(sql)
		Dim Ord : Ord = app.GetIdentity("kuzz","ord","cateid","")
		Set rs = conn.execute("EXEC erp_getdjbh 32," & Info.User)
		If rs.eof = False Then
			ckbh = rs(0).value
		else
			ckbh = ""
		...
		Set rs=Nothing
		title1 = "组装出库：" & title
		sql = "Insert Into kuout(title,ckbh,ck,kg,date3,complete1,fh1,order1,cateid,cateid2,cateid3,date7,date5,cateout,sort1,del) " &_
		" select '" & title1 & "', '" & ckbh & "', 0, ord, '" & addDate & "', 3, 0, " & ord & ", ord, sorce, sorce2," &_
		" GETDATE(), '" & addDate & "', ord, 9, 1 from gate where ord = " & Info.User
		cn.execute(sql)
		kuout = app.GetIdentity("kuout","ord","cateid","")
		currTree     = app.gettext("currTree")
		treetype     = app.gettext("treetype")
		sql = "update Bom_Trees set del = 1,topid = " & ord & " where ord = " & currTree & ";" &_
		"  update Bom_snapshot_Info set del = 1 where treeord = " & currTree & ";" &_
		"  update Bom_snapshot_List set del = 1 where treeord = " & currTree & ";" &_
		"  delete from Bom_Trees where tType=1 and del <> 1 and addcate = " & Info.User & ";" &_
		"  delete from Bom_snapshot_Info where del <> 1 and addcate = " & Info.User & ";" &_
		"  delete from Bom_snapshot_List where del <> 1 and addcate = " & Info.User & ";"
		cn.execute(sql)
		s_proord     = app.gettext("s_proord")
		s_unit               = app.gettext("s_unit")
		s_num1               = app.gettext("s_num1")
		s_price1     = app.gettext("s_price1")
		s_ck1ord     = app.gettext("s_ck1ord")
		s_ckfs2              = app.gettext("s_ckfs2")
		s_ckfs3              = app.gettext("s_ckfs3")
		s_Assinum   = app.gettext("s_Assinum")
		s_bz1                = app.gettext("s_bz1")
		s_js1                = app.gettext("s_js1")
		s_intro              = Replace(app.gettext("s_intro"),"'","''")
		s_zdy1               = Replace(app.gettext("s_zdy1"),"'","''")
		s_zdy2               = Replace(app.gettext("s_zdy2"),"'","''")
		s_zdy3               = Replace(app.gettext("s_zdy3"),"'","''")
		s_zdy4               = Replace(app.gettext("s_zdy4"),"'","''")
		s_zdy5               = app.gettext("s_zdy5")
		s_zdy6               = app.gettext("s_zdy6")
		s_ProductAttr1       = app.gettext("s_ProductAttr1")
		s_ProductAttr2       = app.gettext("s_ProductAttr2")
		If Len(s_intro) = 0 Then s_intro = " "
		If Len(s_zdy1) = 0 Then s_zdy1 = " "
		If Len(s_zdy2) = 0 Then s_zdy2 = " "
		If Len(s_zdy3) = 0 Then s_zdy3 = " "
		If Len(s_zdy4) = 0 Then s_zdy4 = " "
		If Len(s_zdy5) = 0 Then s_zdy5 = " "
		If Len(s_zdy6) = 0 Then s_zdy6 = " "
		If Len(s_bz1) = 0 Then s_bz1 = " "
		If Len(s_js1) = 0 Then s_js1 = " "
		If Len(s_intro) = 0 Then s_intro = " "
		s_proord     = Split(s_proord,Chr(1))
		s_unit               = Split(s_unit,Chr(1))
		s_num1               = Split(s_num1,Chr(1))
		s_price1     = Split(s_price1,Chr(1))
		s_ck1ord     = Split(s_ck1ord,Chr(1))
		s_ckfs2              = Split(s_ckfs2,Chr(1))
		s_ckfs3              = Split(s_ckfs3,Chr(1))
		s_Assinum   = Split(s_Assinum,Chr(1))
		s_bz1                = Split(s_bz1,Chr(1))
		s_js1                = Split(s_js1,Chr(1))
		s_intro              = Split(s_intro,Chr(1))
		s_zdy1               = Split(s_zdy1,Chr(1))
		s_zdy2               = Split(s_zdy2,Chr(1))
		s_zdy3               = Split(s_zdy3,Chr(1))
		s_zdy4               = Split(s_zdy4,Chr(1))
		s_zdy5               = Split(s_zdy5,Chr(1))
		s_zdy6               = Split(s_zdy6,Chr(1))
		s_ProductAttr1       = Split(s_ProductAttr1,Chr(1))
		s_ProductAttr2       = Split(s_ProductAttr2,Chr(1))
		Dim i, ii, kuinlist, num1, s_ckfs3_i, kuid,s_Assinum_i , rid
		For i = 0 To ubound(s_proord)
			ProductAttr1Value="0"
			ProductAttr2Value="0"
			if UBound(s_ProductAttr1)>=0 then
				ProductAttr1Value=s_ProductAttr1(i)
			end if
			if UBound(s_ProductAttr2)>=0 then
				ProductAttr2Value=s_ProductAttr2(i)
			end if
			If s_ckfs2(i) & "" = "1" Then
				sql = "insert into kuoutlist2 (ord, zzyckData, price1, num1, ku, caigoulist, kuinlist, gys, num2, num3, kuout, ck, contract, area, trade, cateid, cateid2, cateid3, date7,date1, del, addcate, bz, js, unit, intro, money1, company, contractlist, kuoutlist, mxid, sort1, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6,ProductAttr1,ProductAttr2) " &_
				" values (" &_
				"'" & s_proord(i) & "', NULL, " & s_price1(i) & ", " & s_num1(i) & ", 0, 0, NULL, 0, 0, 0, " &_
				" '" & kuout & "', '" & s_ck1ord(i) & "', '" & ord & "', -222, 0, " & Info.User & ", 0, 0, " &_
				"'" & s_proord(i) & "', NULL, " & s_price1(i) & ", " & s_num1(i) & ", 0, 0, NULL, 0, 0, 0, " &_
				" getDate(),getDate(),1, " & Info.User & ", '" & s_bz1(i) & "', '" & s_js1(i) & "', '" & s_unit(i) & "', " &_
				" '" & Replace(s_intro(i),"'","''") & "', " & (s_price1(i) * s_num1(i)) & ", 0, 0, 0, 0, 9, " &_
				" '" & Replace(s_zdy1(i),"'","''") & "', " &_
				" '" & Replace(s_zdy2(i),"'","''") & "', " &_
				" '" & Replace(s_zdy3(i),"'","''") & "', " &_
				" '" & Replace(s_zdy4(i),"'","''") & "', " &_
				" '" & Replace(s_zdy5(i),"'","''") & "', " &_
				" '" & Replace(s_zdy6(i),"'","''") & "', " &_
				" '" & Replace(ProductAttr1Value,"'","''") & "', " &_
				" '" & Replace(ProductAttr2Value,"'","''") & "'" &_
				")"
				cn.execute(sql)
			Else
				s_Assinum_i= Split(s_Assinum(i),"|")
				s_ckfs3_i = Split(s_ckfs3(i),"|")
				For ii = 0 To ubound(s_ckfs3_i)
					kuinlist = Split(s_ckfs3_i(ii),"=")(0)
					num1 = Split(s_ckfs3_i(ii),"=")(1)
					if num1&""="" then  num1=0
					rid = Split(s_ckfs3_i(ii),"=")(4)
					if rid&""="" then rid = 0
					fznum=Split(s_Assinum_i(ii),"=")(1)
					fzunit=Split(s_Assinum_i(ii),"=")(2)
					if fznum&""="" then
						fznum=0
					end if
					if fzunit&""="" then fzunit=0
					If num1 * 1 > 0 Then
						sql = "insert into kuoutlist2 (ord,AssistUnit,AssistNum, zzyckData, price1, num1, ku, caigoulist, kuinlist, gys, num2, num3, kuout, ck, contract, area, trade, cateid, cateid2, cateid3, date7, date1, del, addcate, bz, js, unit, intro, money1, company, contractlist, kuoutlist, mxid, sort1, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6, datesc, dateyx, ph, xlh,ProductAttr1,ProductAttr2,ProductAttrBatchId) " &_
						" select top 1 " &_
						"'" & s_proord(i) & "',"&fzunit&","&fznum&", '" & s_ckfs3_i(ii) & "|', isnull(ku.price1,0) price1, " & num1 & ", " &_
						" id, caigoulist, kuinlist, gys, 0, 0, " &_
						" '" & kuout & "', '" & s_ck1ord(i) & "', '" & ord & "', -222, 0, " & Info.User & ", 0, 0, " &_
						" id, caigoulist, kuinlist, gys, 0, 0, " &_
						" getDate(), getDate(),1, " & Info.User & ", '" & s_bz1(i) & "', '" & s_js1(i) & "', '" & s_unit(i) & "', " &_
						" '" & Replace(s_intro(i),"'","''") & "', case when ku.num1 = 0 then 0 else (cast(isnull(ku.money1,0) * " & num1 & " as decimal(25,12))/ku.num1) end, 0, 0, 0, 0, 9, " &_
						" '" & Replace(s_zdy1(i),"'","''") & "', " &_
						" '" & Replace(s_zdy2(i),"'","''") & "', " &_
						" '" & Replace(s_zdy3(i),"'","''") & "', " &_
						" '" & Replace(s_zdy4(i),"'","''") & "', " &_
						" '" & Replace(s_zdy5(i),"'","''") & "', " &_
						" '" & Replace(s_zdy6(i),"'","''") & "', ku.datesc, ku.dateyx, ku.ph, ku.xlh,ku.ProductAttr1,ku.ProductAttr2, "& rid &" ProductAttrBatchId " &_
						" from ku where id = " & kuinlist
						cn.execute(sql)
					end if
				next
			end if
		next
		sqltext=" insert into S2_SerialNumberRelation(SerialID ,BillType , BillID , BillListType  , ListID , Del) "&_
		"   select s2.SerialID, 62001 BillType, "& kuout &" BillID,1 BillListType, k2.id ListID, 1 Del  "&_
		"   from kuoutlist2 k2 "&_
		"   inner join S2_SerialNumberRelation s2  with(nolock) on s2.id=k2.ProductAttrBatchId "&_
		"   inner join M2_SerialNumberList nl with(nolock) on nl.id = s2.serialID "&_
		"   where k2.kuout = "& kuout
		conn.execute(sqltext)
		conn.execute("update kuoutlist2 set ProductAttrBatchId=0 where kuout = "& kuout)
		...
		errords = ""
		Set rs = conn.execute("exec [erp_store_KuoutProc] " & session("personzbintel2007") & ",'" & kuout & "',1 ")
		If rs.eof = False Then
			while rs.eof = False
				msg = rs("msg")
				errords = errords & rs("id")
				rs.movenext
			wend
		end if
		rs.close
		Set rs=Nothing
		If msg&""<>"" Then
			Call bill.alert("出库过程产生错误:" & msg)
			Bill_OnSave = False
			Exit Function
		end if
		conn.execute("exec [erp_comm_store_ChangeLog] 200,'" & kuout & "'," & session("personzbintel2007") & ",'" & getIP() & "'")
		conn.execute("select id as kuoutlist2, ck, ku as ID ,num1 as num2, isnull(datesc,'')  as datesc ,isnull(datesc,'') Daterk,isnull(dateyx,'')  as dateyx into #ku_kuout from kuoutlist2 where kuout in ( "& kuout &")")
		...
		set rs = conn.execute(sql)
		If rs.eof = False Then
			Call bill.alert( rs("sort1") & " 【"& rs("title") &"】库存不足")
			Bill_OnSave = False
			Exit Function
		end if
		rs.close
		...
		"from kuoutlist a  "&_
		"   inner join ( "&_
		"       select sum(num1) num1,kuoutlist from kuoutlist2 where kuout="& kuout &" group by kuoutlist  "&_
		...
		sumMoney = conn.execute("select isnull(sum(b.price1*a.num2),0) sumMoney  from ku  b inner join (select sum(num2) num2 ,ID from #ku_kuout group by ID ) a on a.ID=b.id ")(0)
		sumMoney = cdbl(sumMoney)
		conn.execute("drop table #ku_kuout")
		isOpenAssUnit = (GetAssistUnitTactics()&""="1")
		Dim p_title_h, p_ck1_h, p_unitall, p_num1, p_price1,p_AssistNum,p_AssistUnit
		Dim p_ph1, p_xlh1, p_scrq1, p_yxrq1, p_bz1, p_js1, p_intro, p_zdy1, p_zdy2, p_zdy3, p_zdy4, p_zdy5, p_zdy6
		p_title_h   = app.gettext("p_title_h")
		p_ck1_h             = app.gettext("p_ck1_h")
		p_unitall   = app.gettext("p_unitall")
		p_num1              = app.gettext("p_num1")
		p_price1    = app.gettext("p_price1")
		p_AssistNum=null
		p_AssistUnit=null
		if isOpenAssUnit then
			p_AssistNum = app.gettext("p_AssistNum")
			p_AssistUnit= app.gettext("p_AssistUnit")
			if isOpenAssUnit then
				if Len(p_AssistNum)>0 then
					if cdbl(p_AssistNum)=0 then  alterText="辅助数量不能为0"
				end if
				if p_AssistUnit&""="" or  p_AssistUnit&""="0" then
					if Len(p_AssistNum)>0 then alterText="请填写辅助单位"
				else
					if Len(p_AssistNum)=0 then alterText="请填写辅助数量"
				end if
				if p_unitall=p_AssistUnit  and p_AssistUnit&""<>"" then
					alterText="单位与辅助单位不能相同"
				end if
				if Len(alterText)>0 then
					Call bill.alert(alterText)
					Exit Function
				end if
			end if
		end if
		if p_AssistNum&""="" then p_AssistNum=null
		if p_AssistUnit&""="" then p_AssistUnit=null
		p_ph1               = app.gettext("p_ph1") & ""
		p_xlh1              = app.gettext("p_xlh1") & ""
		p_scrq1             = app.gettext("p_scrq1") & ""
		p_yxrq1             = app.gettext("p_yxrq1") & ""
		p_bz1               = app.gettext("p_bz1") & ""
		p_js1               = app.gettext("p_js1") & ""
		p_intro             = app.gettext("p_intro") & ""
		p_zdy1              = app.gettext("p_zdy1") & ""
		p_zdy2              = app.gettext("p_zdy2") & ""
		p_zdy3              = app.gettext("p_zdy3") & ""
		p_zdy4              = app.gettext("p_zdy3") & ""
		p_zdy5              = app.gettext("p_zdy5") & ""
		p_zdy6              = app.gettext("p_zdy6") & ""
		p_ProductAttr1      = app.gettext("p_ProductAttr1_h") & ""
		p_ProductAttr2      = app.gettext("p_ProductAttr2_h") & ""
		p_ph1               = replace(p_ph1,"'","''")
		p_xlh1              = replace(p_xlh1,"'","''")
		p_intro             = replace(p_intro,"'","''")
		p_zdy1              = replace(p_zdy1,"'","''")
		p_zdy2              = replace(p_zdy2,"'","''")
		p_zdy3              = replace(p_zdy3,"'","''")
		p_zdy4              = replace(p_zdy3,"'","''")
		dim PhXlhManage, phManage, xlhManage, arrxlh, j, xlh2, NeedToSplit
		PhXlhManage = GetProductPhXlhManage(p_title_h, p_unitall)
		phManage = PhXlhManage(0)
		xlhManage = PhXlhManage(1)
		If phManage = 1 and trim(p_ph1&"") = "" Then
			Call bill.alert("批号必须在1到50字之间")
			Bill_OnSave = False
			Exit Function
		end if
		If xlhManage = 1 and trim(p_xlh1&"") = "" Then
			Call bill.alert("序列号必须在1到50字之间")
			Bill_OnSave = False
			Exit Function
		end if
		Dim rkbh
		set rs = cn.execute("EXEC erp_getdjbh_Base 61001,"& Info.User &",'kuin' , 'rkbh' , 'date7','' ")
		If rs.eof = False Then
			rkbh = rs(0).value
		else
			rkbh = ""
		...
		set rs=Nothing
		title1 = "组装入库：" & title
		sql = "Insert Into kuin(title,rkbh,ck,kg,date3,complete1,caigou,sort,cateid,cateid2,cateid3,date7,date5,catein,sort1,del) " &_
		" select '" & title1 & "','" & rkbh & "','0',ord,'" & addDate & "','3','" & ord & "','9',ord,sorce,sorce2,getDate(),'" & addDate & "',ord,9,1 from gate where ord = " & Info.User
		cn.execute(sql)
		Dim kuin
		kuin = app.GetIdentity("kuin","ord","cateid","")
		sql = "select top 0 * from kuinlist"
		Set rs= server.CreateObject("adodb.recordset")
		rs.open sql,cn,3,2
		rs.addnew
		rs("ord") = p_title_h
		rs("price1") = (sumMoney/p_num1)
		rs("price2") = p_price1
		rs("num1") = p_num1
		rs("num2") = p_num1
		rs("kuin") = kuin
		...
		rs("caigou") = ord
		rs("sort") = 9
		If Len(addDate) = 0 Then addDate = date()
		rs("dateadd") = addDate
		rs("date7") = now()
		rs("del") = 1
		rs("addcate") = Info.User
		If Len(p_bz1 & "") = 0 Then p_bz1 = "0" End If
		rs("bz") = p_bz1 & ""
		rs("js") = p_js1 & ""
		rs("unit") = p_unitall
		rs("intro") = p_intro & ""
		...
		rs("zdy3") = p_zdy3 & ""
		rs("zdy4") = p_zdy4 & ""
		If Len(p_zdy5) = 0 Then
			rs("zdy5") = null
		else
			rs("zdy5") = p_zdy5 & ""
		end if
		If Len(p_zdy6) = 0 Then
			rs("zdy6") = null
		else
			rs("zdy6") = p_zdy6 & ""
		end if
		...
		rs("money1") = sumMoney
		rs("money2") = p_price1 * p_num1
		rs("company") = 0
		rs("caigoulist") = -222
		rs("company") = 0
		rs("sort1") = 9
		if isOpenAssUnit then
			rs("AssistUnit") = p_AssistUnit
			rs("AssistNum") = p_AssistNum
		end if
		if isOpenProductAttr then
			if p_ProductAttr1&""="" then p_ProductAttr1=0
			if p_ProductAttr2&""="" then p_ProductAttr2=0
			rs("ProductAttr1") = p_ProductAttr1
			rs("ProductAttr2") = p_ProductAttr2
		end if
		rs.update
		rs.close
		set rs  =  Nothing
		kuinlist = app.GetIdentity("kuinlist","id","addcate","")
		NeedToSplit = false
		arrxlh = split(p_xlh1&"",chr(1))
		if trim(p_xlh1&"")<>"" then
			NeedToSplit = true
		end if
		dim recID, oldrecID, newnum
		recID = kuinlist : oldrecID = recID
		if NeedToSplit then
			For ii=0 To ubound(arrxlh)
				If NeedToSplit = True And ubound(arrxlh)>=0 Then
					newnum = 1
					If ii=0 Then
						conn.execute("update kuinlist set num1=1,num2=1,xlh='"& arrxlh(ii) &"', money1=cast([price1] as money), money2=cast([price2] as money),AssistNum =round(AssistNum / num1 * 1,"& app.info.FloatNumber &")  where id = "& recID)
					else
						sql = "INSERT INTO [kuinlist]([ord],[price1],[price2],[basePrice],[num1],[num2],[kuin],[ku],[caigou],[sort],[dateadd],[date7],[del],[addcate],[bz],[js],[unit],[intro],"&_
						"[ph],[xlh],[datesc],[dateyx],[money1],[money2],baseMoney,[company],[caigoulist],[sort1],[mxpx],[zdy1],[zdy2],[zdy3],[zdy4],[zdy5],[zdy6],[QTLID],[BFID],[date2],[MOrderID],"&_
						"[JoinDBListID],[kuoutlist2],[caigoulist2],[StoreCode],[CaigouQCList],[CaigouQC],M2_QTLID,M2_BFID,M2_OrderID,kuinlist,AssistUnit,AssistNum,ProductAttr1,ProductAttr2,ProductAttrBatchId) "&_
						"select [ord],[price1],[price2],price1 as [basePrice],1 as num1,1 as num2,[kuin],isnull([ku],0) [ku],[caigou],[sort],[dateadd],[date7],[del],[addcate],[bz],[js],[unit],[intro],"&_
						"[ph],'"& arrxlh(ii) &"',[datesc],[dateyx],cast([price1] as money) as money1,cast([price2] as money) as money2,cast([price1] as money) as baseMoney,[company],[caigoulist],[sort1],[mxpx],[zdy1],[zdy2],[zdy3],[zdy4],[zdy5],[zdy6],[QTLID],[BFID],[date2],[MOrderID],"&_
						"[JoinDBListID],[kuoutlist2],[caigoulist2],[StoreCode],[CaigouQCList],[CaigouQC],M2_QTLID,M2_BFID,M2_OrderID,"& oldrecID &" as kuinlist,AssistUnit,AssistNum,ProductAttr1,ProductAttr2,ProductAttrBatchId "&_
						"from kuinlist where id = "& recID
						conn.execute(sql)
						recID = conn.execute("select max(id) from kuinlist where cast(xlh as varchar(8000))='"& arrxlh(ii) &"'")(0)
					end if
				else
					newnum = cdbl(p_num1)
				end if
				if CheckKuXlhExists(arrxlh(ii), 1) then
					Call bill.alert("序列号【" & trim(arrxlh(ii)&"") & "】已存在，请重新录入")
					Bill_OnSave = False
					Exit Function
				end if
				Call saveKuList(arrxlh(ii),109, p_title_h, recID, p_ck1_h, newnum, sumMoney/newnum, p_unitall, p_ph1, p_scrq1, p_yxrq1, p_bz1, p_js1, p_intro, addDate)
			next
		end if
		If NeedToSplit = True and cdbl(p_num1)>ubound(arrxlh)+1 And ubound(arrxlh)>=0 Then
			newnum = cdbl(p_num1) - cdbl(ubound(arrxlh)+1)
			sql = "INSERT INTO [kuinlist]([ord],[price1],[price2],[basePrice],[num1],[num2],[kuin],[ku],[caigou],[sort],[dateadd],[date7],[del],[addcate],[bz],[js],[unit],[intro],"&_
			"[ph],[xlh],[datesc],[dateyx],[money1],[money2],baseMoney,[company],[caigoulist],[sort1],[mxpx],[zdy1],[zdy2],[zdy3],[zdy4],[zdy5],[zdy6],[QTLID],[BFID],[date2],[MOrderID],"&_
			"[JoinDBListID],[kuoutlist2],[caigoulist2],[StoreCode],[CaigouQCList],[CaigouQC],[commUnitAttr],M2_QTLID,M2_BFID,M2_OrderID,kuinlist,AssistUnit,AssistNum,ProductAttr1,ProductAttr2,ProductAttrBatchId) "&_
			"select [ord],[price1],[price2],price1 as [basePrice],"& newnum &" as num1,"& newnum &" as num2,[kuin],isnull([ku],0) [ku],[caigou],[sort],[dateadd],[date7],[del],[addcate],[bz],[js],[unit],[intro],"&_
			"[ph],'',[datesc],[dateyx],cast(("& newnum &"*cast([price1] as money)) as money) as money1,"&_
			"cast(("& newnum &"*cast([price2] as money)) as money) as money2,"&_
			"cast(("& newnum &"*cast([price1] as money)) as money) as baseMoney,"&_
			"[company],[caigoulist],[sort1],[mxpx],[zdy1],[zdy2],[zdy3],[zdy4],[zdy5],[zdy6],[QTLID],[BFID],[date2],[MOrderID],"&_
			"[JoinDBListID],[kuoutlist2],[caigoulist2],[StoreCode],[CaigouQCList],[CaigouQC],'"& Replace(commUnitAttr,"'","''") &"',M2_QTLID,M2_BFID,M2_OrderID,"& oldrecID &" as kuinlist,AssistUnit,round(AssistNum* "&newnum&","&app.info.FloatNumber&")"&_
			",ProductAttr1,ProductAttr2,ProductAttrBatchId from kuinlist where id = "& recID
			conn.execute(sql)
			recID = conn.execute("select max(id) from kuinlist where cast(xlh as varchar(8000))='' ")(0)
			Call saveKuList("",109, p_title_h, recID, p_ck1_h, newnum, sumMoney/newnum, p_unitall, p_ph1, p_scrq1, p_yxrq1, p_bz1, p_js1, p_intro, addDate)
		end if
		if NeedToSplit = false then
			Call saveKuList("",109, p_title_h, recID, p_ck1_h, p_num1, sumMoney/p_num1, p_unitall, p_ph1, p_scrq1, p_yxrq1, p_bz1, p_js1, p_intro, addDate)
		end if
		if isOpenAssUnit then
			if p_title_h&""<>"" and p_AssistUnit&""<>"" then Call SetHistoryAssistUnit(p_title_h,p_AssistUnit)
		end if
		sql="update k2 set k2.JoinDBListID=k.id,k2.area=0 " &_
		" from kuoutlist2 k2 " &_
		" inner join kuinlist k on  k.caigoulist=k2.area and k.caigoulist=-222 and k2.area=-222 where k2.kuout="&kuout&" and k.kuin="&kuin&" and k2.sort1=9 "
		conn.Execute(sql)
		sql="update k set k.caigoulist=0 " &_
		" from kuinlist k " &_
		" where k.kuin="&kuin&" and k.sort1=9 "
		conn.Execute(sql)
		if conn.execute("select 1 from kuinlist kl inner join ku k on k.kuinlist = kl.id where kl.kuin=" & kuin &" and len(k.xlh)>0 ").eof=false then
			conn.execute("exec [erp_store_KuXlhConvertToRelation] " & kuin)
		end if
		dim creatorurl : creatorurl = sdk.getvirpath() & "../SYSN/view/AspVirBill/AutoHandlerASPBill.ashx?billtype=65001&fromid="& kuoutkuins & ord &"&actionname=Kuzz_InventoryCost_Add&t=" & cdbl(now)
		Response.write "<script language='javascript'>"
		Response.write("var xhttp=new XMLHttpRequest(); xhttp.open('GET','" &creatorurl & "',false);xhttp.send();")
		Response.write "</script>"
		Call bill.showSaveResult2("组装单保存成功！","../store/planallzz.asp?isnew=1")
		Bill_OnSave = True
	end function
	Sub saveKuList(xlh, Optype, cpord, recID, ck, newnum, p_price1, unit, p_ph1, p_scrq1, p_yxrq1, p_bz1, p_js1, p_intro, addDate)
		dim logrecid, money_chg, ckid,conn
		set conn = cn
		isOpenAssUnit = (GetAssistUnitTactics()&""="1")
		money_chg=p_price1*newnum
		Call WriteStoreChangeLog(Optype,"kuinlist",cpord,recID,ck,1,newnum,money_chg,0)
		logrecid = app.GetIdentity("Store_ChangeLog","LogID","OperatorID","")
		sql7 = "select * from ku where 1=0"
		Set Rs7= server.CreateObject("adodb.recordset")
		...
		Rs7("num2")=newnum
		Rs7("price1")=p_price1
		Rs7("money1")=money_chg
		Rs7("ph")=p_ph1
		If Len(Trim(xlh & "")) > 50 then
			Rs7("xlh")=Left(Trim(xlh&""),50)
		else
			Rs7("xlh")=Trim(xlh&"")
		end if
		if p_scrq1&""<>"" then Rs7("datesc")=p_scrq1
		if p_yxrq1&""<>"" then Rs7("dateyx")=p_yxrq1
		Rs7("bz")=p_bz1
		If isnumeric(p_js1) = False Then
			p_js1 = "0"
		end if
		Rs7("js")=p_js1
		Rs7("intro")=p_intro
		...
		Rs7("daterk")=addDate
		if isOpenAssUnit then
			set rs8 = conn.execute("select AssistNum,AssistUnit from kuinlist where id= "&recID)
			Rs7("AssistNum")=rs8("AssistNum")
			Rs7("AssistUnit")=rs8("AssistUnit")
		end if
		if isOpenProductAttr  then
			set rs8 = conn.execute("select ProductAttr1,ProductAttr2,ProductAttrBatchId from kuinlist where id= "&recID)
			Rs7("ProductAttr1")=rs8("ProductAttr1")
			Rs7("ProductAttr2")=rs8("ProductAttr2")
			Rs7("ProductAttrBatchId")=rs8("ProductAttrBatchId")
		end if
		Rs7.update
		ckid = Rs7("id").value
		Rs7.close
		set Rs7 = nothing
		conn.execute "update Store_ChangeLog set KuId=" & ckid & " where logid=" & logrecid
	end sub
	Sub App_getBomInfo
		isOpenAssUnit = (GetAssistUnitTactics()&""="1")
		Dim rs, proord, treeord, sql, conn
		set conn = cn
		Set rs = cn.execute("select top 1 * from Bom_Trees where del = 7 and addCate = " & Info.User & " order by ord desc")
		If rs.eof = False Then
			proord = rs("proord")
			treeord = rs("ord")
		else
		...
		rs.close
		set rs = nothing
		Dim title, order1, type1, unitall, unitText, price1, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6,AssistUnitAll,AssistUnitTextAll
		sql = "select " &_
		"   p.title,proOrd, " &_
		"   p.order1,p.type1,x.ProductAttr1,x.ProductAttr2, " &_
		"   x.unit unitall, x.unitText, " &_
		"   p.zdy1, p.zdy2, p.zdy3, p.zdy4, p.zdy5, p.zdy6" &_
		"   from " &_
		"   (select top 1 " &_
		"           proOrd,unit,unitText,ProductAttr1,ProductAttr2 " &_
		"           from Bom_snapshot_List where addCate = " & Info.User & " and isMain = 1 and treeOrd = " & treeord & " order by treeCode " &_
		"   ) x " &_
		"   left join (select * from product where ord = " & proord & ") p on p.ord = x.proOrd "
		Set rs = cn.execute(sql)
		Set rs1 = cn.execute("select b.sort1, b.ord from product a inner join sortonehy b on a.ord=" & proord &" and charindex(','+ cast(b.ord as varchar(12)) + ',',','+a.unit+',')>0 and b.ord<>"&rs("unitall"))
		Set rs = cn.execute(sql)
		i = 0
		While rs1.eof = False
			AssistUnitAll = AssistUnitAll & "_" & rs1("ord")
			AssistUnitTextAll =AssistUnitTextAll &  "___" & rs1("sort1")
			i = i + 1
			AssistUnitTextAll =AssistUnitTextAll &  "___" & rs1("sort1")
			rs1.movenext
		wend
		Set rs1 = Nothing
		set rs1 = GetProductAttrOption(proord,1)
		While rs1.eof = False
			ProductAttr1Ids = ProductAttr1Ids & "_" & rs1("id")
			ProductAttr1Text = ProductAttr1Text &  "___" & rs1("title")
			rs1.movenext
		wend
		Set rs1 = Nothing
		set rs1 = GetProductAttrOption(proord,0)
		While rs1.eof = False
			ProductAttr2Ids = ProductAttr2Ids & "_" & rs1("id")
			ProductAttr2Text = ProductAttr2Text &  "___" & rs1("title")
			rs1.movenext
		wend
		Set rs1 = Nothing
		If rs.eof = False Then
			...
			unitall = rs("unitall")
			unitText = Replace(rs("unitText")& "","'","\'")
			zdy1 = Replace(rs("zdy1") & "","'","\'")
			zdy2 = Replace(rs("zdy2") & "","'","\'")
			zdy3 = Replace(rs("zdy3") & "","'","\'")
			zdy4 = Replace(rs("zdy4") & "","'","\'")
			zdy5 = Replace(rs("zdy5") & "","'","\'")
			zdy6 = Replace(rs("zdy6") & "","'","\'")
			ProductAttr1ChooseName =""
			ProductAttr2ChooseName =""
			ProductAttr1ChooseId =""
			ProductAttr2ChooseId =""
			if rs("ProductAttr1")&""<>"" then
				ProductAttr1ChooseId = rs("ProductAttr1")
				ProductAttr1ChooseName=GetProductAttrNameById(rs("ProductAttr1")&"")
			end if
			if rs("ProductAttr2")&""<>"" then
				ProductAttr2ChooseId = rs("ProductAttr2")
				ProductAttr2ChooseName=GetProductAttrNameById(rs("ProductAttr2")&"")
			end if
		else
			Response.write("false")
			Exit Sub
		...
		rs.close
		set rs = nothing
		dim PhXlhManage, phManage, xlhManage, cpyxqHours, arr_unit
		arr_unit = split(unitall&"",",")
		PhXlhManage = GetProductPhXlhManage(proord, arr_unit(0))
		phManage = PhXlhManage(0)
		xlhManage = PhXlhManage(1)
		cpyxqHours = PhXlhManage(2)
		Set rs = cn.execute("exec bom_packaging_Prolist " & Info.User & ",1")
		Set rs = rs.nextrecordset
		If rs.eof = False Then
			price1 = rs(0)
		...
		set rs = nothing
		if isOpenAssUnit then
			AssistUnitChose=GetHistoryAssistUnit(proOrd)
			if AssistUnitChose&""<>"" then
				AssistNum = ConvertUnitData(proOrd,unitall,AssistUnitChose,1)
				AssistNum =formatnumber(AssistNum, Info.floatnumber ,-1,0,0)
				AssistNum = ConvertUnitData(proOrd,unitall,AssistUnitChose,1)
			end if
		end if
		if isOpenProductAttr then
			ProductAttr1NameT="产品属性1"
			ProductAttr2NameT="产品属性2"
			set rs7 = ProductAttrsByOrd(proOrd)
			if rs7.eof =false then
				do until rs7.eof
					isTiled=rs7("isTiled")
					if isTiled=1 then
						ProductAttr1NameT = rs7("title")
					end if
					if isTiled=0 then
						ProductAttr2NameT=rs7("title")
					end if
					rs7.movenext
				loop
			end if
			ProductAttrNames = ProductAttr1NameT&  "___" & ProductAttr2NameT
		end if
		Dim data
		data = "true:"
		data = data & "{"
		data = data & "'title':'" & title & "',"
		data = data & "'proOrd':'" & proOrd & "',"
		data = data & "'order1':'" & order1 & "',"
		data = data & "'type1':'" & type1 & "',"
		data = data & "'unitall':'" & unitall & "',"
		data = data & "'unitText':'" & unitText & "',"
		data = data & "'ProductAttr1Ids':'" & ProductAttr1Ids & "',"
		data = data & "'ProductAttr1Text':'" & ProductAttr1Text & "',"
		data = data & "'ProductAttr1ChooseName':'" & ProductAttr1ChooseName & "',"
		data = data & "'ProductAttr1ChooseId':'" & ProductAttr1ChooseId & "',"
		data = data & "'ProductAttr2Ids':'" & ProductAttr2Ids & "',"
		data = data & "'ProductAttr2Text':'" & ProductAttr2Text & "',"
		data = data & "'ProductAttr2ChooseName':'" & ProductAttr2ChooseName & "',"
		data = data & "'ProductAttr2ChooseId':'" &  ProductAttr2ChooseId & "',"
		data = data & "'ProductAttrNames':'" & ProductAttrNames & "',"
		data = data & "'AssistUnit':'" & AssistUnitAll & "',"
		data = data & "'AssistUnitText':'" & AssistUnitTextAll & "',"
		data = data & "'AssistUnitChose':'" & AssistUnitChose & "',"
		data = data & "'AssistNum':'" & AssistNum & "',"
		data = data & "'price1':'" & price1 & "',"
		data = data & "'treeord':'" & treeord & "',"
		data = data & "'phManage':'" & phManage & "',"
		data = data & "'xlhManage':'" & xlhManage & "',"
		data = data & "'cpyxqHours':'" & cpyxqHours & "',"
		data = data & "'zdy1':'" & zdy1 & "',"
		data = data & "'zdy2':'" & zdy2 & "',"
		data = data & "'zdy3':'" & zdy3 & "',"
		data = data & "'zdy4':'" & zdy4 & "',"
	...
	Function getKCInfo(ByVal rs)
		Dim rs1, i
		Set rs1 = cn.execute("exec bom_packaging_kcInfo " & Info.User & "," & rs("proord") & "," & rs("unit") & ",0,'" & rs("ProductAttr1") &"','"&rs("ProductAttr2")&"'")
		If rs1.eof = False Then
			getKCInfo = "<div style='color:#cc3333;text-align:left'>"
'If rs1.eof = False Then
			i = 0
			While rs1.eof = False
				If i > 0 Then
					getKCInfo = getKCInfo & "<br />"
				end if
				getKCInfo = getKCInfo & rs1("fullpath") & " " & rs1("uName") & " " & formatnumber(rs1("num1").value&"", Info.floatnumber ,-1,0,0)
				getKCInfo = getKCInfo & "<br />"
				i = i + 1
				getKCInfo = getKCInfo & "<br />"
				rs1.movenext
			wend
			getKCInfo = getKCInfo & "</div>"
		else
			getKCInfo = "<div>没有库存！</div>"
		end if
		rs1.close
		Set rs1 = Nothing
	end function
	Function getTitleLink(ByVal rs)
		If app.power.existsPower(21,14) Then
			getTitleLink = "<a onclick=""javascript:window.open('../product/content.asp?ord=" & app.base64.pwurl(rs("ProOrd")) & "','newwin211','width='+1000+',height='+600+',toolbar=0,scrollbars=1,resizable=1,left=100,top=100');return false;"" href='javascript:void(0);return false;'>" & rs("title") & "</a>"
'If app.power.existsPower(21,14) Then
		else
			getTitleLink = rs("title")
		end if
	end function
	Function getKCFS(ByVal rs, ByVal lvwid)
		Dim rowindex : rowindex = rs("snord")
		getKCFS = "<input id='packing_proList_jec_" & rowindex & "_13_0' value='1'" &_
		" onclick=""changeCkfs('" & lvwid & "',13,this)""" &_
		" checked type='radio' name='packing_proList_jec_" & rowindex & "_13'>" &_
		"<label for='packing_proList_jec_" & rowindex & "_13_0'>随机</label>" &_
		"<input id='packing_proList_jec_" & rowindex & "_13_1' value='2'" &_
		" onclick=""changeCkfs('" & lvwid & "',13,this)""" &_
		" type='radio' name='packing_proList_jec_" & rowindex & "_13'>" &_
		"<label for='packing_proList_jec_" & rowindex & "_13_1'>指定</label>"
	end function
	Sub app_getKCInfo
		Dim proord, unit, ckord, rs
		proord = request.form("proord")
		unit = request.form("unit")
		ckord = request.form("ckord")
		ProductAttr1 = request.form("ProductAttr1")
		ProductAttr2 = request.form("ProductAttr2")
		if ProductAttr1&""="" or  ProductAttr1&""="undefined" then ProductAttr1 = 0
		if ProductAttr2&""="" or  ProductAttr2&""="undefined" then ProductAttr2 = 0
		Set rs = cn.execute("exec bom_packaging_kcInfo " & Info.User & "," & proord & "," & unit & "," & ckord & ",'"& ProductAttr1 &"','"& ProductAttr2&"'" )
		If rs.eof = False Then
			Response.write("true:" & rs("num1") & ":<div style='color:#cc3333;text-align:left;font-weight:bold'>" & Replace(rs("fullpath"),"'","\'") & " " & Replace(rs("uName"),"'","\'") & " " & formatnumber(rs("num1").value&"", Info.floatnumber ,-1,0,0)  & "</div>")
'If rs.eof = False Then
		else
			Response.write("false:<div>没有库存！</div>")
		end if
		rs.close
		set rs = nothing
	end sub
	Sub app_getBatchKCInfo
		Dim dataText,arr,dataArr,ckord,proord,unit,sqlPart,sql,resultText
		dataText = app.getText("dataText")
		If Len(dataText & "") = 0 Then
			Response.write("false")
			Exit Sub
		end if
		arr = Split(dataText,"|")
		For i = 0 To ubound(arr)
			dataArr = Split(arr(i),"_")
			ckord = dataArr(0) & ""
			proord = dataArr(1) & ""
			unit = dataArr(2) & ""
			ProductAttr1 = dataArr(3) & ""
			ProductAttr2 = dataArr(4) & ""
			if ProductAttr1&""="" then ProductAttr1=0
			if ProductAttr2&""="" then ProductAttr2=0
			If ckord ="" or ckord="" or unit="" then
			else
				sqlPart=" select "&i&" rowindex, "&ckord&" ck, "&proord&" ord, "&unit&" unit, 0 kuinlist,"&ProductAttr1&" ProductAttr1,"&ProductAttr2&" ProductAttr2"
				if i=0 then
					sql=sqlPart
				else
					sql=sql &" union "& sqlPart
				end if
			end if
		next
		cn.execute("SELECT * INTO #datalist FROM("&sql&") t")
		cn.execute("select ord as ck into #sortck from sortck where del=1 and (cast(intro as varchar(8000))='0' or charindex('," & Info.User & ",',','+cast(intro as varchar(8000))+',')>0) order by gate1 desc,ord desc")
		cn.execute("SELECT * INTO #datalist FROM("&sql&") t")
		cmdText = "set nocount on;"&_
		" select k.id into #cklist"&_
		" from (select distinct ord, ck,unit,ProductAttr1,ProductAttr2 from #datalist) d "&_
		" inner join ku k on d.ord = k.ord and isnull(k.num2,0)>0 and (d.ck = k.ck or d.ck=0) and (isnull(d.ProductAttr1,0)=isnull(k.ProductAttr1,0) or isnull(d.ProductAttr1,0)=0) and (isnull(d.ProductAttr2,0)=isnull(k.ProductAttr2,0) or isnull(d.ProductAttr2,0)=0 ) "&_
		" inner join #sortck s0 on s0.ck = k.ck"&_
		" left join setjm3 on setjm3.ord=5430 and setjm3.cateid="& Info.User&_
		" where isnull(setjm3.num1,0)=0 or "&_
		" k.ck in (select b.id from jiage a inner join sortck b on a.MainStore=b.id and a.product=d.ord and b.del=1 and a.bm=0 and a.unit=d.unit and "&_
		"(cast(b.intro as varchar(8000))='0' or charindex(',' + CAST(" & Info.User & " as varchar(10)) + ',',','+cast(b.intro as varchar(8000))+',')>0)"&_
		"  UNION ALL "&_
		" SELECT b.id      FROM ProductStoreBinding a inner join sortck b on a.StoreID=b.id and a.ProductID=d.ord and a.unit=d.unit left join sortonehy c on c.ord=63) "&_
		" select d.rowindex , isnull(y.ck,0) ck ,"&_
		"       y.unit as mUnit , mh.sort1 as munitName , isnull(y.num2,0) as mNum2 ,"&_
		"       d.unit,  h.sort1 as UnitName, cast(y.defNum2 as decimal(25,12)) /isnull(cast(a.bl as decimal(25,12)),1) as num1,"&_
		"       replace(replace(s0.FullPath,CHAR(1) + s0.sort1,' ' + s0.sort1),CHAR(1),'-') FullPath,"&_
		"       isnull(s0.sort1,'') CKName "&_
		" from #datalist d "&_
		" left join erp_comm_unitRelation a on a.ord = d.ord and a.unit = d.unit"&_
		" left join ("&_
		"     select dl.kuinlist ,s.ord , s.ck , s.unit ,sum(s.num2) as num2 , sum(cast(s.num2 as decimal(25,12))*cast(u.bl as decimal(25,12))) as defNum2"&_
		"     from ku s"&_
		"     inner join #cklist c on c.id=s.id "&_
		"     inner join (select distinct kuinlist from #datalist) dl on dl.kuinlist = 0 or dl.kuinlist = s.kuinlist"&_
		"     inner join erp_comm_unitRelation u on u.ord = s.ord and u.unit= s.unit"&_
		"     group by s.ord ,s.ck ,s.unit, dl.kuinlist"&_
		" ) y on d.ord = y.ord and d.unit = y.unit and y.kuinlist = d.kuinlist"&_
		" left join sortck s0 on s0.ord = y.ck"&_
		" left join sortck1 s1 on s1.del=1 and s1.id = s0.[sort]"&_
		" left join sortonehy h on h.ord = d.unit"&_
		" left join sortonehy mh on mh.ord = y.unit"&_
		" order by d.rowindex, isnull(s0.gate1,0) desc ,d.ck asc, (case when y.unit = d.unit then 0 else 1 end) asc"&_
		" drop table #cklist;"&_
		" set nocount off;"
		...
		While rs.eof = False
			If rs("mNum2")&""=0 Then
				prefix="false:"
				if i>0 then prefix="|false:"
				resultText=resultText & prefix & "<div>没有库存！</div>"
			else
				prefix="true:"
				if i>0 then prefix="|true:"
				resultText=resultText & prefix & rs("mNum2") &"_"& rs("ck") &"_"& rs("CKName") &_
				":<div style='color:#cc3333;text-align:left;font-weight:bold'>" & Replace(rs("FullPath"),"'","\'") & " " & Replace(rs("munitName"),"'","\'") & " " & formatnumber(rs("mNum2").value&"", Info.floatnumber ,-1,0,0)  & "</div>"
'resultText=resultText & prefix & rs("mNum2") &"_"& rs("ck") &"_"& rs("CKName") &_
			end if
			i = i + 1
'resultText=resultText & prefix & rs("mNum2") &"_"& rs("ck") &"_"& rs("CKName") &_
	...
	Sub app_getAvgPrice
		Dim data, arr, num1, i, sumNum1, sumMoney1, rs, avgPrice1
		data = app.getText("data")
		sumNum1 = 0
		sumMoney1 = 0
		If Len(data & "") = 0 Then
			Response.write("false")
		...
		For i = 0 To ubound(arr)
			...
			if isnumeric(unit & "") = False Then
				unit = 0
			end if
			if isnumeric(fznum & "") = False Then
				fznum = 0
			end if
			If CDbl(num1) > 0 Then
				Set rs = cn.execute("select price1,unit from ku where id = " & Split(arr(i),"=")(0))
				If rs.eof = False Then
					sumNum1 = sumNum1 + CDbl(num1)
'If rs.eof = False Then
					if(CDbl(rs(1))=cdbl(unit))then
					sumMoney1 = sumMoney1 + (CDbl(num1) * CDbl(rs(0)))
'if(CDbl(rs(1))=cdbl(unit))then
				else
					sumMoney1 = sumMoney1 + (CDbl(fznum) * CDbl(rs(0)))
'if(CDbl(rs(1))=cdbl(unit))then
				end if
			end if
			rs.close
	...
	If sumNum1 = 0 Then
		...
		avgPrice1 = sumMoney1 / sumNum1
	end if
	Response.write("true:" & avgPrice1)
	end sub
	...
	Function WriteStoreChangeLog(iOpType,sTableName,iProductId,iMXID,iStoreId,iInOrOut,iNumChg,iMoneyChg,iKuId)
		...
		sql="select * from " & sTableName & " where id=" & iMXID
		set funcrs=conn.execute(sql)
		insUnit=funcrs("unit")
		...
		sql="select isnull(sum(case when num1>0 then cast(isnull(money1,0)*isnull(num2,0) as decimal(25,12))/isnull(num1,1) else 0 end),0) from ku where ord="&iProductId&" and unit='"&insUnit&"' and ck="&iStoreId
...
'elseif iInOrOut=2 then
	...
	Sub app_getNum
		...
		if ProductID&""<>"" and OldUnit&""<>"" and NewUnit&""<>"" and Num<>"" and OldUnit&""<>"NULL" then resultNum= ConvertUnitData(ProductID,OldUnit,NewUnit,Num)
