...
'Private Sub Class_Initialize
		...
		End Class
		Sub page_load
			Dim open_25_10, intro_25_10
			Dim i, rs, sql, Str_Result, searchtype, list, px, tt, px_Result, Export_Fields ,showPrice
			tt = "预购单"
			showPrice = app.power.existsPower(25,21)
			Call fillPowerInfo(25,10,open_25_10,intro_25_10)
			if open_25_10=3 then
				list=""
			elseif open_25_10=1 then
				list="and (a.cateid in ("&intro_25_10&") or a.share='1' or charindex(',"& session("personzbintel2007") &",',','+isnull(a.share,'0')+',')>0 )"
'elseif open_25_10=1 then
			else
				list="and a.cateid=0"
			end if
			Str_Result=" and a.del=1 "&list&""
			dim searchmode , W1,W2,W3,A1,A2,B,C,D,E,F,H,sort1,sort2,pord,HBZT, m1, m2, sp, caigouid, zt1 , YGFL,XJZT  ,strW1,strW2,strW3 , strW4 , X , cgzt ,tmp
			Dim glbill,title,ygid,cp_title , cp_order1 , cp_type1 ,intro ,ygmoney1 , ygmoney2 , ret3,ret4,ret5,ret6 ,kz_search_sql ,SPZT,s
			searchmode = request("searchmode")
			If searchmode&"" ="" Then searchmode = 0
			B=request("B")
			SPZT = request("SPZT")
			HBZT=request("HBZT")
			YGFL = request("YGFL")
			XJZT = request("XJZT")
			If Len(XJZT&"")=0 Then XJZT = -1
			XJZT = request("XJZT")
			C=Request("C")
			m1=request("ret")
			m2=request("ret2")
			caigouid=request("caigouid")
			sp=request("T")
			If sp&""="" Then sp = 0
			...
			X = Replace(request("X")," ","")
			S = Replace(request("S")," ","")
			cgzt = Replace(request("cgzt")," ","")
			glbill = request("glbill")
			title = request("title")
			ygid = request("ygid")
			cp_title = request("cp_title")
			cp_order1 = request("cp_order1")
			cp_type1 = request("cp_type1")
			intro = request("intro")
			ygmoney1 = request("ygmoney1")
			ygmoney2 = request("ygmoney2")
			ret3= request("ret3")
			ret4= request("ret4")
			ret5= request("ret5")
			ret6= request("ret6")
			Dim helper,remind : remind = request("remind")
			If remind & "" = "" Then  Str_Result=Str_Result & " AND ((" & sp &" = 0 AND ISNULL(a.status,0)=0) OR (" & sp &"=1 AND a.status IN (1,2)) OR (" & sp &"=2 AND a.status=-1) ) "
'Dim helper,remind : remind = request("remind")
			dim link ,uid
			link = request("link")
			zt1=request("zt1")
			uid=replace(request("uid")," ","")
			pord = app.base64.deurl(request.QueryString("pro_ord"))
			If Len(pord&"")>0 Then Str_Result=Str_Result & " AND b.ord = "& pord &" "
			If Len(caigouid&"")>0 Then  Str_Result=Str_Result & " and b.id in (SELECT mx.fromid caigoulist_yg FROM caigoulist cl WITH(NOLOCK)  INNER JOIN caigoulist_mx mx WITH(NOLOCK) ON mx.fromType=5 AND cl.id=mx.caigoulist and cl.caigou="&caigouid&")"
			if uid&""="" Or InStr(uid&"","%") > 0 then uid=0
			if link = "yes" then
				if zt1 ="02" Then Str_Result = Str_Result & " and (a.zt1 = 0 or a.zt1 = 2)"
				if uid<>"" And uid<>0 Then Str_Result = Str_Result & " and a.cateid=" & uid
			end if
			If remind & "" <> "" Then
				Set helper = CreateReminderHelper(cn,remind,0)
				Str_Result = Str_Result & " and a.id in (" & helper.listSQL("ids_withoutOrderBy") & ")"
			end if
			dim fromContent,fromChance, fromContract, fromXunjia
			fromChance=trim(Request.QueryString("chance"))
			if fromChance<>"" Then fromContent=" and a.chance="&fromChance&""
			fromContract=trim(Request.QueryString("contract"))
			if fromContract<>"" Then fromContent=" and a.contract="&fromContract&""
			fromXunjia=trim(Request.QueryString("xunjia"))
			if fromXunjia<>"" Then fromContent=" and a.xunjia="&fromXunjia&""
			If request("propm")=1 Then
				If app.power.existsPower(22,13) Then
					Set rs=cn.execute("select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where intro=1  and cateid="& Info.User &" and ord=55")
					If rs.eof = False Then
						If rs("fw1")="1" Then Str_Result= Str_Result & " and a.cateid="& Info.User &" "
						Str_Result= Str_Result & " and a.del=1 and a.zt1<>1 and charindex(',"& Info.User &",',','+isnull(a.alt,'0')+',')=0 "
						If rs("fw1")="1" Then Str_Result= Str_Result & " and a.cateid="& Info.User &" "
					end if
					rs.close
					set rs = nothing
			...
			If request("newstate")=1 Then
				Set rs=cn.execute("select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where cateid="& Info.User &" and ord=85")
				If rs.eof = False Then
					If rs("fw1")="1" Then Str_Result= Str_Result & " and a.cateid="& Info.User &" "
				end if
				rs.close
			...
			If searchmode = 0 Then
				If SPZT <>"" Then Str_Result= Str_Result & " and isnull(a.status,0)="& SPZT &" "
				If XJZT <>"" And XJZT<>"-1" Then Str_Result= Str_Result & " AND (CASE WHEN  isnull(a.xunjia,0)>0 THEN 3  WHEN ISNULL(a.needxj,0)=1 THEN  "&_
				"(CASE WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0) = 0 THEN 1 "&_
				"  WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0)>0 And ISNULL(xj2.num2,0)<ISNULL(xj2.num1,0) THEN 2  "&_
				"   WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0)>=ISNULL(xj2.num1,0) THEN 3    "&_
				"   ELSE 0 END) "&_
				" ELSE 0 end)= "& XJZT &"  "
				if HBZT<>"" And IsNumeric(HBZT) Then Str_Result= Str_Result & " and a.zt1="& HBZT &" "
				If YGFL <>"" Then Str_Result= Str_Result & " and a.sort1="& YGFL &" "
				if B = "ygzt" then
					Str_Result=Str_Result & " and isnull(a.title,'') like '%"& C &"%'"
				elseif B = "ygbh" then
					Str_Result=Str_Result & " and isnull(a.ygid,'') like '%"& C &"%'"
				elseif B = "yggy" then
					Str_Result=Str_Result & " and isnull(a.intro1,'') like '%"& C &"%'"
				elseif B = "ygry" then
					Str_Result=Str_Result & " and g.name like '%"& C &"%'"
				elseif B = "spry" then
					Str_Result=Str_Result & " and g1.name like '%"& C &"%'"
				elseif B = "gys" then
					Str_Result=Str_Result & " and exists(select top 1 b.id from caigoulist_yg b inner join tel c on b.gys=c.ord where a.id=b.caigou and c.name like '%"& C &"%')"
				elseif B = "yg_zt" Then
					Select Case C
					Case "未采购"
					Str_Result=Str_Result & " and a.zt1 = 0"
					Case "部分采购"
					Str_Result=Str_Result & " and a.zt1 = 2"
					Case "采购完毕"
					Str_Result=Str_Result & " and a.zt1 = 1"
					End Select
				ElseIf  B = "cpmc" Then
					Str_Result=Str_Result & " and exists(select top 1  b.id from caigoulist_yg b inner join product p on b.ord=p.ord where a.id=b.caigou and p.title like '%"& C &"%')"
				ElseIf  B = "cpbh" Then
					Str_Result=Str_Result & " and exists(select top 1 b.id from caigoulist_yg b inner join product p on b.ord=p.ord where a.id=b.caigou and p.order1 like '%"& C &"%')"
				ElseIf  B = "cpxh" Then
					Str_Result=Str_Result & " and exists(select top 1 b.id from caigoulist_yg b inner join product p on b.ord=p.ord where a.id=b.caigou and p.type1 like '%"& C &"%')"
				end if
				if m1<>"" then Str_Result=Str_Result & " and datediff(day,'"&m1&" 00:00:00',isnull(a.date1,0))>=0"
				if m2<>"" then Str_Result=Str_Result & " and datediff(day,'"&m2&" 23:59:59',isnull(a.date1,0))<=0"
			else
				If len(w3)>0 Then Str_Result=Str_Result & " AND CHARINDEX(','+CAST(a.cateid AS VARCHAR(10))+',',',"& w3 &",')>0 "
'if m2<>"" then Str_Result=Str_Result & " and datediff(day,'"&m2&" 23:59:59',isnull(a.date1,0))<=0"
				If len(F)>0 Then Str_Result=Str_Result & " AND CHARINDEX(','+CAST(ISNULL(a.sort1,0) AS VARCHAR(10))+',',',"& F &",')>0 "
'if m2<>"" then Str_Result=Str_Result & " and datediff(day,'"&m2&" 23:59:59',isnull(a.date1,0))<=0"
				If len(X)>0 Then Str_Result=Str_Result & " AND             CHARINDEX(','+CAST(ISNULL(  "&_
				"(CASE WHEN  isnull(a.xunjia,0)>0 THEN 3   "&_
				"   WHEN ISNULL(a.needxj,0)=1 THEN   "&_
				"  (CASE WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0) = 0 THEN 1   "&_
				"   WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0)>0 And ISNULL(xj2.num2,0)<ISNULL(xj2.num1,0) THEN 2  "&_
				"   WHEN ISNULL(xj2.num1,0)>0 And ISNULL(xj2.num2,0)>=ISNULL(xj2.num1,0) THEN 3  "&_
				"   ELSE 0 END)  "&_
				"   ELSE 0 end),0) AS VARCHAR(10))+',',',"& X &",')>0  "
				If len(S)>0 Then Str_Result=Str_Result & " AND CHARINDEX(','+CAST(ISNULL(a.status,0) AS VARCHAR(10))+',',',""& S &"",')>0 "
				If len(cgzt)>0 Then  Str_Result=Str_Result & " AND CHARINDEX(','+CAST(ISNULL(a.zt1,0) AS VARCHAR(10))+',',',""& cgzt &"",')>0 "
				If Len(glbill)>0 Then
					Str_Result=Str_Result & " and ((a.fromtype = 1 and a.chance in (select ord from chance where title like '%"& glbill &"%')) "&_
					" or (a.fromtype = 2 and a.contract in (select ord from contract where title like '%"& glbill &"%')) "&_
					" or (a.fromtype = 3 and a.xunjia in (select id from xunjia where title like '%"& glbill &"%')) "&_
					" or (a.fromtype = 4 and a.MOrderID in (select id from M_ManuOrders where title like '%"& glbill &"%')) )"
				end if
				If len(title)>0 Then  Str_Result=Str_Result & " AND a.title LIKE '%"& title & "%' "
				If len(ygid)>0 Then  Str_Result=Str_Result & " AND a.ygid LIKE '%"& ygid & "%' "
				If len(cp_title)>0 Then  Str_Result=Str_Result & " AND cp.title LIKE '%"& cp_title & "%' "
				If len(cp_order1)>0 Then  Str_Result=Str_Result & " AND cp.order1 LIKE '%"& cp_order1 & "%' "
				If len(cp_type1)>0 Then  Str_Result=Str_Result & " AND cp.type1 LIKE '%"& cp_type1 & "%' "
				If len(intro)>0 Then  Str_Result=Str_Result & " AND a.intro1 LIKE '%"& intro & "%' "
				If len(ygmoney1)>0 Then  Str_Result=Str_Result & " AND a.money1>="& ygmoney1 &" "
				If len(ygmoney2)>0 Then  Str_Result=Str_Result & " AND a.money1<="& ygmoney2 &" "
				If len(ret3)>0 Then  Str_Result=Str_Result & " and DATEDIFF(S,'"& ret3 &"',a.date1)>=0 "
				If len(ret4)>0 Then  Str_Result=Str_Result & " and DATEDIFF(S,'"& ret4 &"',a.date1)<=0 "
				If len(ret5)>0 Then  Str_Result=Str_Result & " and DATEDIFF(S,'"& ret5 &"',a.date7)>=0 "
				If len(ret6)>0 Then  Str_Result=Str_Result & " and DATEDIFF(S,'"& ret6 &"',a.date7)<=0 "
				kz_search_sql = searchExtended_Simple(25," a.id ")
				If Len(kz_search_sql)>0 Then  Str_Result=Str_Result & kz_search_sql
			end if
			If request("px")&"" <> "" Then px=request("px")
			if px="" then px=1 Else px = CInt(px)
			if px=1 then
				px_Result="order by a.date7 desc,a.id DESC,b.id asc "
			elseif px=2 then
				px_Result="order by a.date7 asc,a.id DESC,b.id asc "
			elseif px=3 then
				px_Result="order by a.title desc,a.id DESC,b.id asc "
			elseif px=4 then
				px_Result="order by a.title asc,a.id DESC,b.id asc "
			elseif px=5 then
				px_Result="order by a.ygid desc,a.id DESC,b.id asc "
			elseif px=6 then
				px_Result="order by a.ygid asc,a.id DESC,b.id asc "
			elseif px=7 then
				px_Result="order by a.date1 desc,a.id DESC,b.id asc "
			elseif px=8 then
				px_Result="order by a.date1 asc,a.id DESC,b.id asc "
			elseif px=9 then
				px_Result="order by 预购金额 desc,a.id DESC,b.id asc "
			elseif px=10 then
				px_Result="order by 预购金额 asc,a.id DESC,b.id asc "
			elseif px=11 then
				px_Result="order by 负责人 desc,a.id DESC,b.id asc "
			elseif px=12 then
				px_Result="order by 负责人 asc,a.id DESC,b.id asc "
			end if
			Dim zdySQL : zdySQL = ""
			Dim j ,exZdyFields(),exJoinTables,exZdy1 , fields
			ReDim exZdyFields(1,0)
			sql="select ID,FName from ERP_CustomFields where isUsing=1 and canexport=1 and tname=25 and del=1 order by id"
			j=0
			exJoinTables = "":exZdy1=""
			set rs=cn.execute(sql)
			While rs.eof = False And ZBRuntime.MC(207102)
				ReDim Preserve exZdyFields(1,j)
				exZdyFields(0,j) = "exzdy" & j
				exZdyFields(1,j) = rs(1)
				exZdy1 = exZdy1 & ",extb"&j&".FValue " & exZdyFields(0,j) & " "
				exJoinTables = exJoinTables & vbcrlf & " left join ERP_CustomValues extb"&j&" on extb"&j&".OrderID=a.id and extb"&j&".FieldsID=" & rs(0)
				fields = fields & "," & exZdyFields(0,j)
				rs.movenext
				j = j + 1
				fields = fields & "," & exZdyFields(0,j)
			wend
			rs.close
			Set rs=Nothing
			zdySQL = zdySQL & exZdy1
			Dim cptitle, cporder1, cptype1, cpunitall, cpnum1, cpprice1, cpmoney1, cpdate2, cpintro
			Dim cpzdy1, cpzdy2, cpzdy3, cpzdy4, cpzdy5, cpzdy6
			Dim rs_zdy, len_rszdy
			Set rs = cn.execute("select name, title, set_open from zdymx where sort1=25 order by gate1 asc")
			If rs.eof = False Then
				rs_zdy = rs.GetRows()
			else
				zdySQL = zdySQL & ", "&_
				"cp.title 产品名称, cp.order1 编号, cp.type1 型号, s.sort1 单位,  isnull(b.num1,0) 数量, isnull(b.price1,0) 单价, isnull(b.money1,0) 总价, " &_
				"CONVERT(varchar(100),b.date2,23) 到货日期, b.intro1 备注, " &_
				"b.zdy1, b.zdy2, b.zdy3, b.zdy4, s5.sort1 zdy5, s6.sort1 zdy6 "
			end if
			rs.close
			set rs = nothing
			...
			for i=0 to len_rszdy
				If rs_zdy(2,i) = 0 Then
					execute("cp"& rs_zdy(0,i) &"=False")
				else
					execute("cp"& rs_zdy(0,i) &"="""& rs_zdy(1,i) &"""")
				end if
				Select Case rs_zdy(0,i)
				Case "title" : zdySQL = zdySQL &",cp.title 产品名称 "
				Case "order1" : zdySQL = zdySQL &",cp.order1 编号 "
				Case "type1" : zdySQL = zdySQL &",cp.type1 型号 "
				Case "unitall" : zdySQL = zdySQL &",s.sort1 单位 "
				Case "num1" : zdySQL = zdySQL &",isnull(b.num1,0) 数量 "
				Case "price1" : zdySQL = zdySQL &",isnull(b.price1,0) 单价 "
				Case "money1" : zdySQL = zdySQL &",isnull(b.money1,0) 总价 "
				Case "date2" : zdySQL = zdySQL &",CONVERT(varchar(100),b.date2,23) 到货日期 "
				Case "intro" : zdySQL = zdySQL &",b.intro1 备注 "
				Case "zdy1" : zdySQL = zdySQL &",b.zdy1 "
				Case "zdy2" : zdySQL = zdySQL &",b.zdy2 "
				Case "zdy3" : zdySQL = zdySQL &",b.zdy3 "
				Case "zdy4" : zdySQL = zdySQL &",b.zdy4 "
				Case "zdy5" : zdySQL = zdySQL &",s5.sort1 zdy5 "
				Case "zdy6" : zdySQL = zdySQL &",s6.sort1 zdy6 "
				End Select
			next
			sql="select a.title 预购主题, a.ygid 预购编号, CONVERT(varchar(100),a.date1,23) 预购日期, a.cateid cgCateid,g.name 负责人,st.sort1 as 预购分类,  " &_
			"          sz.sort1 as 币种,(case isnull(a.needxj,0) when 0 then '否' else '是' end) as 是否需要询价,ISNULL(a.money1,0) 预购金额,"&_
			"          case isnull(a.status,0) when 0 then '审批通过' when 1 then '待审批' when 2 then '审批中' when -1 then '审批不通过' end as 审批状态 , " &_
			"          g1.name as 当前审批人,  " &_
			"        (case when isnull(a.needxj,0)=1 then  " &_
			"          (case when isnull(a.xunjia,0)>0 then '询价完毕'  " &_
			"          else  " &_
			"                  (case when ISNULL(xj2.num1,0)=0 then '无需询价' when  ISNULL(xj2.num2,0)=0  " &_
			"                          then '待询价' when  ISNULL(xj2.num2,0)<ISNULL(xj2.num1,0) then '部分询价' " &_
			"                          else '询价完毕' end) " &_
			"          end) " &_
			"        else '无需询价'  " &_
			"        end) as 询价状态,"&_
			"          (case isnull(a.zt1,0) when 0 then '未采购' when 1 then '采购完毕' when 2 then '部分采购' end) 采购状态, " &_
			"          a.intro1 预购概要,"&_
			"          case a.fromtype when 0 then '直接添加' when 1 then '项目' when 2 then '合同' when 3 then '询价' when 4 then '生产订单' " &_
			"                                          when 7 then '整单委外' when 8 then '生产派工' when 9 then '生产返工' when 10 then '工序委外' when 11 then '生产订单' when 12 then '物料分析' end as 预购来源, "&_
			"          case a.fromtype when 1 then ce.title when 2 then ht.title when 3 then xa.title when 4 then ms.title when 7 then mr.title when 8 then mns.title when 9 then mns.title when 10 then mr.title when 12 then mis.title else '' end  来源单据, "&_
			"          case a.fromtype when 1 then ceg.name when 2 then htg.name when 3 then xag.name when 4 then msg.name " &_
			"       when 7 then g2.name "&_
			"       when 8 then g3.name "&_
			"       when 9 then g3.name "&_
			"       when 10 then g2.name "&_
			"       when 12 then g5.name "&_
			" else '' end  业务人员 "& zdySQL &_
			"          from caigou_yg a " &_
			"          LEFT JOIN ("&_
			"                          SELECT   "&_
			"                                  sum(CASE WHEN ISNULL(z.caigoulist,0)=0 THEN 1 "&_
			"                                  when ISNULL(z.caigoulist,0)>0 AND ISNULL(z.djzt,0)=0 THEN 1 "&_
			"                                  ELSE 0  END) AS unDJRows, sum(CASE WHEN ISNULL(z.caigoulist,0)>0 THEN 1 ELSE 0 END) AS isxjnum , y.caigou "&_
			"                          FROM caigoulist_yg y "&_
			"                          left JOIN xunjialist z ON z.caigoulist = y.id "&_
			"                          GROUP BY  y.caigou "&_
			"                  ) xj ON xj.caigou = a.id "&_
			"        LEFT JOIN (  "&_
			"            select a.caigou, COUNT(a.id) num1, SUM(b.num2) num2  "&_
			"            from caigoulist_yg  a "&_
			"            left join (select caigoulist_yg,1 num2 from xunjialist where isnull(caigoulist_yg,0)>0 and del=1  group by caigoulist_yg) b on a.id=b.caigoulist_yg "&_
			"            where a.del=1 "&_
			"            group by a.caigou "&_
			"        ) xj2 ON xj2.caigou = a.id         "&_
			"       left join power p1 on p1.ord="&session("personzbintel2007")&" and  p1.sort1=(case a.fromtype when 1 then 3 when 2 then 5 when 3 then 24 when 4 then 51  when 7 then 5025 when 8 then 54 when 9 then 62 when 10  then 5026 when 11 then  51 when 12 then 63 end) and p1.sort2=1  " &_
			"          left join caigoulist_yg b on b.caigou = a.id and b.del=1 " &_
			"          left join gate g on a.cateid = g.ord  " &_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          left join chance ce on ce.ord =a.chance and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ce.cateid AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))   " &_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          Left Join gate ceg on ceg.ord= ce.cateid and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ce.cateid AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0)) "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          left join contract ht on a.contract = ht.ord and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ht.cateid AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))  " &_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          Left Join gate htg on htg.ord= ht.cateid and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ht.cateid AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))  "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          left join xunjia xa on xa.id = a.xunjia and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(xa.cateid  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))   " &_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join M2_OutOrder mr on mr.ID=a.M2_OutID  and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mr.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))   "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join gate g2 on g2.ord=mr.creator and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mr.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))  "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join M2_WorkAssigns mns on mns.ID=a.M2_AssignID and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mns.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))  "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join gate g3 on g3.ord=mns.creator and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mns.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0)) "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join M2_MaterialAnalysis mis on mis.id=a.m2_AnalysisID and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mis.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0)) "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"       left join gate g5 on g5.ord=mis.creator and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(mis.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0)) "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          Left Join gate xag on xag.ord= xa.cateid and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(xa.cateid  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))    "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          left join M2_ManuOrders ms on ms.id =case when isnull(a.MOrderID,0)>0 then a.MOrderID else a.m2_orderid end and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ms.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))    " &_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          Left Join gate msg on msg.ord= ms.creator  and (p1.qx_open=3 or (p1.qx_open=1 and CHARINDEX(','+CAST(ms.creator  AS VARCHAR(20))+',',','+CAST(p1.qx_intro AS nvarchar(max))+',') > 0))  "&_
			"          left join gate g1 on g1.ord=a.cateid_sp "&_
			"          left join sortonehy st on st.ord = a.sort1  " &_
			"          left join sortbz sz on sz.id = a.bz  " &_
			"          left join product cp on b.ord = cp.ord and cp.del=1  " &_
			"          left join sortonehy s on b.unit = s.ord  " &_
			"          left join sortonehy s5 on b.zdy5 = s5.ord  " &_
			"          left join sortonehy s6 on b.zdy6 = s6.ord  " &_
			"       left join (  " &_
			"          select distinct a.caigoulist_yg from caigoulist a inner join caigou b on a.caigou=b.ord and b.isstop = 0 and b.del=1 and a.del=1  " &_
			"          )x on x.caigoulist_yg=b.id  " &_
			"          "& exJoinTables &" " &_
			"          where 1=1  "&Str_Result & fromContent &" "& px_Result
			Dim adapter
			...
			adapter.lvw.distinctSpaceCol = "预购主题,预购编号,预购日期,负责人,预购分类,币种,是否需要询价,预购金额,审批状态,当前审批人,询价状态,采购状态,预购概要,预购来源,来源单据,业务人员"& fields
			Dim hd
			...
			hd.dbname = "预购金额"
			hd.dbtype = "money"
			If showPrice=False Then hd.display="none"
			sheetSetting.headerSettings.add(hd)
			Set hd = New HeaderColumnSetting
			hd.dbname = "审批状态"
			hd.excelAlign = "center"
			...
			hd.width = EX_WIDTH_PERSON
			sheetSetting.headerSettings.add(hd)
			For i=0 To ubound(exZdyFields,2)
				Set hd = New HeaderColumnSetting
				hd.dbname = exZdyFields(0,i)
				hd.title = exZdyFields(1,i)
				hd.dbtype = "str"
				sheetSetting.headerSettings.add(hd)
			next
			Set hd = New HeaderColumnSetting
			hd.dbname = "产品名称"
			If cptitle = False Then
				hd.display = "none"
			else
				hd.title = cptitle
				hd.dbtype = "str"
			...
			Set hd = New HeaderColumnSetting
			hd.dbname = "单价"
			If cpprice1 = False Or showPrice=False Then
				hd.display = "none"
			else
				hd.title = cpprice1
				hd.dbtype = "storeprice"
				hd.canSum = False
				hd.width = EX_WIDTH_SELECT
			end if
			sheetSetting.headerSettings.add(hd)
			Set hd = New HeaderColumnSetting
			hd.dbname = "总价"
			If cpmoney1 = False Or showPrice=False  Then
				hd.display = "none"
			else
				hd.title = cpmoney1
				hd.dbtype = "money"
			...
			Set hd = New HeaderColumnSetting
			hd.dbname = "备注"
			If cpintro = False Then
				hd.display = "none"
			else
				hd.title = cpintro
				hd.dbtype = "str"
				hd.width = EX_WIDTH_TEXT
			end if
			sheetSetting.headerSettings.add(hd)
			Set hd = New HeaderColumnSetting
			hd.dbname = "zdy1"
			If cpzdy1 = False Then
				hd.display = "none"
			else
				hd.title = cpzdy1
				hd.dbtype = "str"
