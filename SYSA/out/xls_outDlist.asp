...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_101_10=1 then
		list=" and get_addcateid in ("&intro_101_10&")"
	else
	...
	if open_101_1=3 then
	elseif open_101_1=1 then
		Str_Result=Str_Result&" and get_addcateid in ("&intro_101_1&")"
	else
		Str_Result=Str_Result&" and get_addcateid=0"
	end if
	px=request("px")
...
'if Exit Sub2 <>"" and not isnull(ret2) then
	...
	j=1
	k=1
	sql="select a.*,b.reg_fid,b.prod_num,b.prod_ph,b.prod_xlh,b.prod_sctime,b.prod_ystime,c.id as cid, "&_
	...
	"isnull(h.fh_num,0) as fh_num, "&_
	"case when f.pro_type = 2 then (isnull(a.get_num,0)-isnull(h.fh_num,0)) else 0 end as dfh_num "&_
	...
	C1=rs.recordcount
	xlApplication.Init "领用明细表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			xlWorksheet.Cells(1+i,1).Value =rs("get_title")
'if not rs.eof then
			xlWorksheet.Cells(1+i,1).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,2).Value =rs("get_bh")
'if not rs.eof then
			xlWorksheet.Cells(1+i,2).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).Value =rs("cateid_Name")
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).Value = rs("prod_name")
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).Value = rs("prod_bh")
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).Value = rs("prod_xh")
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,7).Value = rs("sort1")
'if not rs.eof then
			xlWorksheet.Cells(1+i,7).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,8).Value =Formatnumber(zbcdbl(rs("get_num")),num1_dot,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,8).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,9).Value =Formatnumber(rs("fh_num"),num1_dot,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,9).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,10).Value =Formatnumber(rs("dfh_num"),num1_dot,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,10).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,11).Value =rs("sto_name")
'if not rs.eof then
			xlWorksheet.Cells(1+i,11).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,12).Value =Formatnumber(zbcdbl(rs("prod_num")),num1_dot,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,12).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,13).Value =rs("pro_name")
'if not rs.eof then
			xlWorksheet.Cells(1+i,13).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,14).Value =rs("type_attribute")
'if not rs.eof then
			xlWorksheet.Cells(1+i,14).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,15).Value =rs("prod_ph")
'if not rs.eof then
			xlWorksheet.Cells(1+i,15).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,16).Value =rs("prod_xlh")
'if not rs.eof then
			xlWorksheet.Cells(1+i,16).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,17).Value =rs("prod_sctime")
'if not rs.eof then
			xlWorksheet.Cells(1+i,17).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,18).Value =rs("prod_ystime")
'if not rs.eof then
			xlWorksheet.Cells(1+i,18).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,19).Value =rs("get_intro")
'if not rs.eof then
...
'if not rs.eof then
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>领用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!领用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("领用明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的领用明细表</u></strong></font></a></p>"
