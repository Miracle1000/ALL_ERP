...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	loadNum2=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from gate  where del=1 "
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			if phone<>"" then
				...
				sql2="select * from MessagePerson where del=1 and clictPhone='"&phone&"'"
				...
				if  rs2.eof then
					...
					conn.execute"insert into MessagePerson (clictName,clictPhone,clictSex,clictJob,complete,addtime,del,addcate,sort) values('"&rs("name")&"','"&phone&"','"&rs("sex")&"','"&rs("title")&"',1,'"&now()&"',1,"&session("personzbintel2007")&","&classType&")"
					loadNum1=loadNum1+1
				else
					loadNum2=loadNum2+1
				end if
	...
	rs.close
	set rs=nothing
	Response.write"<script language=javascript>alert('导入成功！共导入了"&loadNum1&"条,重复"&loadNum2&"条.');window.parent.location.href='../message/alliframe.asp';</script>"
	call db_close : Response.end
	conn.close
	set conn=nothing
