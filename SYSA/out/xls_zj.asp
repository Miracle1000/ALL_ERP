...
'Public Function AddFormValue(name,  value)
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>导出质检单</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "</HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class=""place"">质检单导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf &" <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出质检单,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf
	Response.write Application("sys.info.jsver")
	if open_28_10=3 then
		list=""
	elseif open_28_10=1 then
		list=" and addcate in ("&intro_28_10&") and addcate<>0 "
	else
		list=" and 1=2 "
	end if
	Str_Result=" where del=1 "&list&" "
	str_Result=replace(str_Result,"where del=1"," ")
	Server.ScriptTimeOut=100000000
	H=request("H")
	ret=request("ret")
	ret2=Request("ret2")
...
'if Exit Sub8&"" <>"" then
	...
	If Z1&""<>"" Then
		Z1 = Replace(Z1&""," ","")
		Str_Result = Str_Result + " and a.id in(select qcord from dbo.FUN_CaigouQCRknum(0,2,'0') t where cgord=a.caigou and qcord=a.id and charindex(','+(case when okNum=0 or yrkNum=0 then '1' when yrkNum>0 and okNum>yrkNum then '2' when yrkNum>0 and okNum=yrkNum then '3' else '0' end)+',',',"& Z1 &",')>0)"
	end if
	if ry&"" <>"" then
		ryFiled = "addcate"
		if ry="0" then
...
'if W4<>"" then
	...
	if D&"" <>"" then
		if instr(D,",")>0 then
			Str_Result = Str_Result + " and  a.id in(select caigouQC from caigouQClist where QCRank in("& D &") and del=1) "
'if instr(D,",")>0 then
		else
			Str_Result = Str_Result + " and  a.id in(select caigouQC from caigouQClist where QCRank="& D &" and del=1) "
'if instr(D,",")>0 then
		end if
	end if
	if F2<>"" then
...
'select case B
	...
	xApp.init Me, conn
	HeaderStr =  "质检主题,质检编号,质检方案,质检状态"
	if ShowRKInfo() then HeaderStr = HeaderStr & ",入库状态"
	HeaderStr = HeaderStr & ",申请人员,申请日期,添加时间,质检人员,质检时间,审批人员,审批时间"
	Dim rsmxcount, rsmxname, rsmxsort
	rsmxcount = -1
'Dim rsmxcount, rsmxname, rsmxsort
	ReDim rsmxname(0)
	ReDim rsmxsort(0)
	zdy = ""
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,fname from ERP_CustomFields where isUsing=1 and canexport=1 and tname=28 and del=1 order by FOrder asc "
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
			zdy = zdy&",(select fvalue from ERP_CustomValues where FieldsID="&rs1("id")&" and OrderID=a.id) as ["&rs1("fname")&"]"
			wirteFilds = wirteFilds&zdy
			HeaderStr = HeaderStr  & "," & rs1("fname")
			rsmxcount = rsmxcount + 1
			HeaderStr = HeaderStr  & "," & rs1("fname")
			ReDim Preserve rsmxname(rsmxcount)
			ReDim Preserve rsmxsort(rsmxcount)
			rsmxname(rsmxcount) = rs1("fname")
			rs1.movenext
		loop
	end if
	rs1.close
	set rs1=nothing
	HeaderStr = HeaderStr & ",概要,关联采购,关联供应商"
	Dim rszjmxcount, rszjmxsort
	rszjmxcount = -1
'Dim rszjmxcount, rszjmxsort
	ReDim rszjmxsort(0)
	set rs=server.CreateObject("adodb.recordset")
	sql="select title,sorce,name,kd from zdymx where sort1=28 and set_open=1 order by gate1 asc "
	rs.open sql,conn,1,1
	if not rs.eof then
		rs_zdymx = rs.GetRows()
	end if
	rs.close
	set rs = nothing
	if isArray(rs_zdymx) then
		len_zdymx = ubound(rs_zdymx,2)
	else
		len_zdymx = -1
		len_zdymx = ubound(rs_zdymx,2)
	end if
	showFileds = ""
	for i=0 to len_zdymx
		zdytitle = rs_zdymx(0,i)
		sorce = rs_zdymx(1,i)
		zdyname = rs_zdymx(2,i)
		showFileds = showFileds     & zdyname &","
		rszjmxcount = rszjmxcount + 1
		showFileds = showFileds     & zdyname &","
		ReDim Preserve rszjmxsort(rszjmxcount)
		rszjmxsort(rszjmxcount) = sorce
		select case sorce
		case 1 : HeaderStr = HeaderStr  & ","& zdytitle
		case 2
		zdytitle = "产品编号"
		HeaderStr = HeaderStr  & ","& zdytitle
		case 3
		zdytitle = "产品型号"
		HeaderStr = HeaderStr  & ","& zdytitle
		case 4 :
		...
		if isOpenProductAttr then
			HeaderStr = HeaderStr  & ",产品属性1"
			HeaderStr = HeaderStr  & ",产品属性2"
		end if
		case 5, 6
		if open_28_21<>0 then
			HeaderStr = HeaderStr  & ","& zdytitle
		end if
		case 7
		HeaderStr = HeaderStr  & ","& zdytitle
		HeaderStr = HeaderStr  & ",质检数量,合格数量,不合格数量,合格率,质检结果,质检类型,质检等级"
		case 8, 9, 10, 11, 12, 13, 14, 15 : HeaderStr = HeaderStr  & ","& zdytitle
		end select
	next
	HeaderStr = HeaderStr & ",审批结果,审批意见"
	sql2 = ""
	...
	wirteFild=" title as 质检主题,isnull(b.sort1,'') as 质检方案,qcid as 质检编号,"&_
	"(case Complete when 0 then '待质检' when 1 then '质检中' when 2 then '待审批' when 3 then (case sign(cateid_sp) when 1 then '审批完毕' else '质检完毕' end) end) as 质检状态,"&_
	"case when p.okNum=0 or p.yrkNum=0 then '未入库' when p.yrkNum>0 and p.okNum>p.yrkNum then '部分入库' when p.yrkNum>0 and p.okNum=p.yrkNum then '入库完毕' end as 入库状态,"&_
	"(select top 1 title from caigou where ord=a.caigou) as 关联采购,"&_
	"(select top 1 name from tel where sort3=2 and ord=a.company) as 关联供应商,"&_
	"dbo.TrimHTML(intro) as 概要"& zdy &","&_
	"(select name from gate where ord=a.Inspector) as 质检人员,"&_
	"QCTime as 质检时间,"&_
	"(select top 1 name from gate where ord=a.addcate) as 申请人员,"&_
	"date1 as 申请日期,"&_
	"date7 as 添加时间,"&_
	"(select top 1 name from gate where ord=a.cateid_sp) as 审批人员,"&_
	"date2 as 审批时间,"&_
	"(select sort1 from sortonehy where ord=isnull(a.unit,0)) as 单位,datedh as 到货日期,"&_
	"intro2 as 备注,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6,productid,a.id,caigoulist "& sql2
	strComplainType = ""
	strComplainType2 = ""
	allNum = 0
	sql1 = "select count(id) from caigouQC a where del=1 " &str_Result&sqlcon
	set rs1 = conn.execute(sql1)
	if not rs1.eof then
		if rs1(0)>0 then allNum = rs1(0) else allNum = 0
	end if
	rs1.close
	set rs1 = nothing
	jn = 0
	...
	xsheet.movenext
	Response.Flush
	dim i, PreProcV : i=0 : j=1 : PreProcV=-1 : currProcV = 0
	Response.Flush
	Dim complete,rowIndex,pageCount
	pageCount = 1
	rowIndex = 0
	set rs=server.CreateObject("adodb.recordset")
	sql=" set nocount on; select * into #temp from dbo.FUN_CaigouQCRknum(0,2,'0');set nocount off;select "& wirteFild &",addcate,isnull(recnum,0) as recnum,isnull(NumQC,0) as NumQC,isnull(a.OKNum,0) as OKNum,isnull(FailNum,0) as FailNum,(case when isnull(NumQC,0)>0 then isnull(a.OKNum,0)/isnull(NumQC,0)*100 else 0 end) as POP,isnull(QCResult,0) as Result,isnull(QCType,0) as QCType,'' as QCRank,isnull(SpResult,0) as SpResult,isnull(SpRemark,'') as SpRemark from caigouQC as a left join #temp p on p.qcord=a.id  left join sortonehy b on isnull(a.QC_id,0) = b.id where a.del = 1 "& str_Result&sqlcon&px_Result&";drop table  #temp "
	rs.open sql,conn,3,1
	C1=rs.recordcount
	If rs.eof Then
		xsheet.writehtml "<td style='text-align:center;height:30;line-height:30;' colspan='" & (ubound(Split(HeaderStr,","))+1) & "'>没有信息</td>"
'If rs.eof Then
	else
		For j = 1 To C1
			productid = rs("productid")
			id = rs("id") : caigoulist = rs("caigoulist")
			xsheet.writecell rs("质检主题")
			xsheet.writecell rs("质检编号")
			xsheet.writecell rs("质检方案")
			xsheet.writecell rs("质检状态")
			if ShowRKInfo() then
				xsheet.writecell rs("入库状态")
			end if
			xsheet.writestr rs("申请人员")
			xsheet.writecell rs("申请日期")
			xsheet.writecell rs("添加时间")
			xsheet.writestr rs("质检人员")
			xsheet.writecell rs("质检时间")
			xsheet.writestr rs("审批人员")
			xsheet.writecell rs("审批时间")
			For i = 0 To rsmxcount
				zdyCon = rs(rsmxname(i))
				if zdyCon&""<>"" then
					if instr(Lcase(zdyCon),"<br>")>0 or instr(Lcase(zdyCon),"<br/>")>0 or instr(Lcase(zdyCon),"<br />")>0 or instr(Lcase(zdyCon),"<p>")>0 then
						zdyCon = filterhtml(zdyCon)
						if left(zdyCon,4)="<br>" then
							zdyCon = mid(zdyCon,5,len(zdyCon))
						end if
					elseif instr(zdyCon,chr(13))>0 then
						zdyCon = replace(zdyCon,chr(13),"<br/>")
						zdyCon = replace(zdyCon,chr(10),"")
						zdyCon = replace(zdyCon,chr(32),"&nbsp;")
					end if
				end if
				xsheet.writecell zdyCon
			next
			qcintro = rs("概要")
			if qcintro&""<>"" then
				qcintro = filterhtml(qcintro)
				if left(qcintro,4)="<br>" then
					qcintro = mid(qcintro,5,len(qcintro))
				end if
			end if
			xsheet.writecell qcintro
			if open_22_1=3 or CheckPurview(intro_22_1,trim(rs("addcate")))=True then
				xsheet.writecell rs("关联采购")
			else
				xsheet.writecell ""
			end if
			if open_26_1=3 or CheckPurview(intro_26_1,trim(rs("addcate")))=True then
				xsheet.writecell rs("关联供应商")
			else
				xsheet.writecell ""
			end if
			zdy5 = ""
			if instr(showFileds,"zdy5")>0 then
				if rs("zdy5")&"" <>"" then
					set rs3 = conn.execute("select top 1 sort1 from sortonehy where ord="& rs("zdy5"))
					if not rs3.eof then
						zdy5 = rs3("sort1")
					end if
					rs3.close
			...
			zdy6 = ""
			if instr(showFileds,"zdy6")>0 then
				if rs("zdy6")&"" <>"" then
					set rs3 = conn.execute("select top 1 sort1 from sortonehy where ord="& rs("zdy6"))
					if not rs3.eof then
						zdy6 = rs3("sort1")
					end if
					rs3.close
			...
			price1="" :money1=""
			if caigoulist&""<>"" then
				set rs2 = conn.execute("select top 1  isnull(priceAfterDiscountTax,0) price1,money1,ProductAttr1,ProductAttr2 from caigoulist where id="& caigoulist &"")
				if not rs2.eof then
					price1 = CDbl(rs2("price1"))
					money1 = rs2("money1")
					ProductAttr1 = rs2("ProductAttr1")
					ProductAttr2 = rs2("ProductAttr2")
				end if
				rs2.close
				set rs2 = nothing
			end if
			cptitle="" : order1="" : type1=""
			set rs3 = conn.execute("select top 1 title,order1,type1 from product where ord="& productid)
			if not rs3.eof then
				cptitle=rs3("title") : order1=rs3("order1") : type1=rs3("type1")
			end if
			rs3.close
			set rs3 = nothing
			For iii=0 To rszjmxcount
				mx_sorce = rszjmxsort(iii)
				select case mx_sorce
				case 1 : xsheet.writestr cptitle
				case 2 : xsheet.writestr order1
				case 3 : xsheet.writestr type1
				case 4 :
				xsheet.writestr rs("单位")
				if isOpenProductAttr then
					xsheet.writestr GetProductAttrNameById(ProductAttr1)
					xsheet.writestr GetProductAttrNameById(ProductAttr2)
				end if
				case 5
				if open_28_21<>0 then
					xsheet.writestr price1
				end if
				case 6
				if open_28_21<>0 then
					xsheet.writestr money1
				end if
				case 7
				xsheet.writenumber rs("recnum")
				Result = rs("Result") : SpResult = rs("SpResult")
				if Result = 0 and SpResult = 0 then
					NumQC = "" : OKNum = "" : FailNum = "" : POP = "" : QCResult = "" : QCType = "" : QCRank = ""
				else
					NumQC = formatNum(cdbl(rs("NumQC"))) : OKNum = formatNum(cdbl(rs("OKNum")))
					FailNum = formatNum(cdbl(rs("FailNum"))) : POP = formatNum(cdbl(rs("POP"))) &"%"
					if Result = 1 then
						QCResult = "合格"
					...
					if rs("QCType") = 1 then
						QCType = "抽检"
					elseif rs("QCType") = 0 then
						QCType = "全检"
					end if
					if rs("QCRank")&""<>"" then
						QCRank = ""
						set rs3 = conn.execute("select top 1 sort1 from sortonehy where ord="& rs2("QCRank"))
						if not rs3.eof then
							QCRank = rs3("sort1")
						end if
						rs3.close
				...
				xsheet.writenumber NumQC
				xsheet.writenumber OKNum
				xsheet.writenumber FailNum
				xsheet.writecell POP
				xsheet.writestr QCResult
				xsheet.writestr QCType
				xsheet.writestr QCRank
				case 8:xsheet.writestr rs("到货日期")
				case 9:xsheet.writestr rs("备注")
				case 10:xsheet.writestr rs("zdy1")
				case 11:xsheet.writestr rs("zdy2")
				case 12:xsheet.writestr rs("zdy3")
				case 13:xsheet.writestr rs("zdy4")
				case 14:
				zdy5 = ""
				if rs("zdy5")&"" <>"" then
					set rs3 = conn.execute("select top 1 sort1 from sortonehy where ord="& rs("zdy5"))
				...
				xsheet.writestr zdy6
				end select
			next
			Select Case rs("SpResult")
			Case 0:
			SpResult = ""
			Case 1:
			SpResult = "紧急放行"
			...
			SpResult = ""
			End Select
			xsheet.writestr SpResult
			xsheet.writestr rs("SpRemark")                    '审批意见
			jn = jn +1
			xsheet.writestr rs("SpRemark")                    '审批意见
			currProcV = Clng(jn/allNum*100)
			If PreProcV < currProcV Then
				Response.write "<script>exportProcBar.showExcelProgress(" & currProcV & "," & allNum & "," & jn & ")</script>"
				PreProcV = currProcV
				Response.Flush
			end if
			rs.movenext
			...
			Response.Flush
			xsheet.movenext
			If rowIndex >= 10000 Then
				If pageCount = 1 then
					xsheet.title = mainTitle &"(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = mainTitle &"(" & pageCount & "页)"
				Set xsheet = xApp.sheets.add(mainTitle &"(" & pageCount & "页)")
				rowIndex = 0
				xsheet.showheader HeaderStr1
				xsheet.movenext
			end if
		next
	end if
	rs.close
	set rs=Nothing
	function formatNum(numf)
		if numf&""<>"" then
			if numf>1 then
				formatNum = round(numf,num_dot_xs)
			elseif numf>0 and numf<1 then
				numf2 = cstr(round(numf,num_dot_xs))
				if left(numf2,1)="." then
					formatNum = "0"& round(numf,num_dot_xs)
	...
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>质检单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!质检单导出成功,共有"& allNum &"条记录!</B>"";</script>"
	tfile=Server.MapPath(xApp.cFileName("质检单_"&session("name2006chen")) & ".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Response.write "<script>exportProcBar.showExcelProgress(100," & allNum & "," & allNum & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'质检单_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	Set xApp = Nothing
	action1="质检导出"
	call close_list(1)
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的质检单</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
