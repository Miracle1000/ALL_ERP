		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					next
					Dim numall,moneyall,fznumall
					Set xApp = server.createobject( ZBRLibDLLNameSN & ".HtmlExcelApplication")
					xApp.init Me, conn
					Set xsheet = xApp.sheets.add("出库汇总表")
					HeaderStr = "产品名称,产品编号,产品型号,产品单位,出库数量"
					if fzunit=1 then
						HeaderStr = HeaderStr & ",辅助单位,辅助数量"
					end if
					...
					elseif open_32_11=1 then
						list1="and c.cateid in ("&intro_32_11&") "
					else
						list1="and c.cateid = 0 "
					end if
...
'If Request("appDate2") = "" Then
	...
	If searchType = "1" Then
		If A2 <> "" Then
			Str_Result = Str_Result & " AND b.sort1 IN ("&A2&") "
		end if
		If product_title <> "" Then
			Str_Result = Str_Result & " AND b.title LIKE '%"&product_title&"%' "
		end if
		If product_order <> "" Then
			Str_Result = Str_Result & " AND b.order1 LIKE '%"&product_order&"%' "
		end if
		If product_type <> "" Then
			Str_Result = Str_Result & " AND b.type1 LIKE '%"&product_type&"%' "
		end if
		If product_PH <> "" Then
...
'if khjz&""<>"" then
		...
		If kuout_title <> "" Then
			Str_Result = Str_Result & " AND c.title LIKE '%"&kuout_title&"%' "
		end if
		If kuout_SN <> "" Then
			Str_Result = Str_Result & " AND c.ckbh LIKE '%"&kuout_SN&"%' "
		end if
		If kuout_intro <> "" Then
			Str_Result = Str_Result & " AND c.intro LIKE '%"&kuout_intro&"%' "
		end if
		If kuout_type <> "" Then
			Str_Result = Str_Result & " AND c.sort1 IN ("&kuout_type&") "
		end if
		If W3 <> "" Then
		...
		If appDate1 <> "" And appDate2 <> "" Then
			Str_Result = Str_Result & " AND CONVERT(VARCHAR(10),c.date5,120) BETWEEN '"&appDate1&"' AND '"&appDate2&"' "
		ElseIf appDate1 <> "" And appDate2 = "" Then
			Str_Result = Str_Result & " AND CONVERT(VARCHAR(10),c.date5,120) >= '"&appDate1&"' "
		ElseIf appDate1 = "" And appDate2 <> "" Then
			Str_Result = Str_Result & " AND CONVERT(VARCHAR(10),c.date,120) <= '"&appDate1&"' "
		end if
	end if
	a=request("a")
	B=request("B")
...
'elseif B="ckbz" then
		...
		if D<>"" and D<>"33" then
			Str_Result=Str_Result+" and  c.sort1="&D&""
'if D<>"" and D<>"33" then
		end if
		if khjz&""<>"" then
...
'if khjz&""<>"" then
		...
		if not ZBRuntime.MC(7000) then
			Str_Result=Str_Result+" and  c.sort1<>1 "
'if not ZBRuntime.MC(7000) then
		end if
		if not ZBRuntime.MC(16000) then
			Str_Result=Str_Result+" and  c.sort1<>2 "
'if not ZBRuntime.MC(16000) then
		end if
		if not ZBRuntime.MC(17006) then
			Str_Result=Str_Result+" and  c.sort1<>6 "
'if not ZBRuntime.MC(17006) then
		end if
		if not ZBRuntime.MC(17004) then
			Str_Result=Str_Result+" and  c.sort1<>7 "
'if not ZBRuntime.MC(17004) then
		end if
		if not ZBRuntime.MC(17005) then
			Str_Result=Str_Result+" and  c.sort1<>8 "
'if not ZBRuntime.MC(17005) then
		end if
		if not ZBRuntime.MC(17007) then
			Str_Result=Str_Result+" and  c.sort1<>9 "
'if not ZBRuntime.MC(17007) then
		end if
		if m1<>"" then
			Str_Result=Str_Result+" and  c.date5>='"&m1&"'"
'if m1<>"" then
		end if
		if m2<>"" Then
			if instr(m2,":") = 0 then
			...
			Str_Result=Str_Result+" and  c.date5<='"&m2&"'"
			m2 = m2 & " 23:59:59"
		end if
	end if
	pageCount = 1
	i=1: currProcV =0 : PreProcV = -1
	pageCount = 1
	sql1 = "select a.ord,(select top 1 sort1 from sortonehy where ord=a.unit) as unitname ,"&_
	"  isnull(sum(case a.num1 when null then 0 else a.num1 end),0) as num2,"&_
	"   isnull(sum(round( a.FinaMoney,"&num_dot_xs&") ),0) as money1 ,  " &_
	"  isnull(b.title,'产品已被删除') as ptitle, b.order1,b.type1,b.pricemode,sum(isnull(a.AssistNum,0)) AssistNum,s.sort1 AssistUnitname  "&_
	"  from kuoutlist2 a "&_
	"  left join sortonehy s on s.ord = a.AssistUnit  "&_
	"  inner join kuout c on c.ord = a.kuout and (c.del=1 or c.del=99) "&_
	...
	"  left join contract ht WITH(NOLOCK) on c.sort1 in(1,4) and c.order1=ht.ord and ht.del in(1,3) and isnull(ht.sp,0)>=0 "&_
	"  left join tel kh WITH(NOLOCK) on ht.company=kh.ord "& Str_Result&_
	"  group by a.ord,a.unit,s.sort1,b.title,b.order1,b.type1,b.pricemode,isnull(AssistUnit,0) "&Str_having&" order by a.ord,a.unit,isnull(AssistUnit,0) asc "
	set rs1 = server.CreateObject("adodb.recordset")
	rs1.open sql1,conn,1,1
	C1=rs1.recordcount
	if not rs1.eof then
		do until rs1.eof
			...
			if fzunit=1  then
				xsheet.writemoney AssistUnitname
				xsheet.writemoney AssistNum
			end if
			numall=numall+CDbl(num2)
			xsheet.writemoney AssistNum
			fznumall=fznumall+CDbl(AssistNum)
			xsheet.writemoney AssistNum
			moneyall=moneyall+CDbl(money1)
			xsheet.writemoney AssistNum
			currProcV = Clng(i*1.0/C1*100)
			...
			If rowIndex > 40000 Then
				If pageCount = 1 then
					xsheet.title = "出库汇总表(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = "出库汇总表(" & pageCount & "页)"
				Set xsheet = xapp.sheets.add("出库汇总表(" & pageCount & "页)")
				rowIndex = 0
				xsheet.showHeader HeaderStr
				xsheet.movenext
			end if
	...
	xsheet.writespace 3
	xsheet.writecell "总计："
	xsheet.writenumber Formatnumber(numall,num1_dot,-1)
	xsheet.writecell "总计："
	if fzunit=1 then
		xsheet.writespace 1
		xsheet.writenumber Formatnumber(fznumall,num1_dot,-1)
		xsheet.writespace 1
	end if
	Response.write "<script>CountImage.style.width=""100%"";CountTXT.innerHTML=""<font color=red><b>出库汇总导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!出库汇总导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	xsheet.writespace 1
	tfile=Server.MapPath(xApp.cFileName("出库汇总表") & ".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'出库汇总表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="出库汇总导出"
	call close_list(1)
	Response.write " " & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的出库汇总</u></strong></font></a></p> " & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
