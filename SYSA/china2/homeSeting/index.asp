...
Response.write "" & vbcrlf & "      <button onclick='expPanel(""2"");' style=""display:none""  name='funbutton' class='button fun' id='fbutton2'>顶部导航</button>" & vbcrlf & "  <button onclick='expPanel(""3"");'  name='funbutton' class='button fun' id='fbutton3'>工具栏</button>" & vbcrlf & "       <button onclick='expPanel(""4"");'  name='funbutton' class='button fun' id='fbutton4'>快速检索</button>" & vbcrlf & " <button onclick='expPanel(""5"");'  name='funbutton' class='button fun' id='fbutton5'>首页报表</button>" & vbcrlf & "     <button onclick='expPanel(""6"");'  name='funbutton' class='button fun' id='fbutton6'>显示内容</button>" & vbcrlf & "        "
Response.write "</div>"
...
fp = server.mappath("../../skin/" & info.skin & "/images/toolbar")
end if
set fs = fso.GetFolder(fp).Files
for each f in  fs
ic = lcase(f.name)
if instr(ic,"gif") > 0 then
imi = imi + 1
'if instr(ic,"gif") > 0 then
if ic = ico and ismyuploadico = false then
Response.write "<option value=""" & ic & """ selected>图标" & imi & "</option>"
else
Response.write "<option value=""" & ic & """>图标" & imi & "</option>"
end if
end if
next
if ismyuploadico = false then
Response.write "<option value='0'>自定义图标</option>"
else
Response.write "<option value='0' selected>自定义图标</option>"
end if
if abs(err.number) > 0 then
Response.write "<option>无图标</option>"
end if
on error goto 0
Response.write "" & vbcrlf & "                             </select>" & vbcrlf & "                       </td>" & vbcrlf & "           </tr>" & vbcrlf & "        <tr id=""uploadIcon"" style=""display:none;"">" & vbcrlf & "            <td style='width:65px;'>上传图标：" & vbcrlf & "                               <input type=""hidden"" id='imgbindata' class='hidden' value='"
if ismyuploadico  then  Response.write imgbindata
Response.write "'>" & vbcrlf & "                           <input type=""hidden"" id='imgfilename' class='hidden' value='"
if ismyuploadico  then  Response.write ico
Response.write "'>" & vbcrlf & "                <input type=""hidden"" id='imgIEPath' class='hidden' value=''>" & vbcrlf & "            </td>" & vbcrlf & "                        <td colspan=""3"">" & vbcrlf & "                          <form action=""../../../SYSN/view/comm/uploaderReturnBase64.ashx?returnmode=iframe"" target=""uploadicofrm""  id=""uploadicoform"" method=""post""   enctype=""multipart/form-data"">" & vbcrlf & "                <div class=""userDefineIcon"">" & vbcrlf & "                    上传图标" & vbcrlf & "                    <input id=""upLoaderBtn""  name=""icofile1"" type=""file"" onchange=""uploadPicInfo(this)""/>" & vbcrlf& "                </div>" & vbcrlf & "                <span id=""fileName""></span>" & vbcrlf & "                                </form>" & vbcrlf & "                <a class=""delIcon disShow"" onclick=""delIcon(this)"">删除</a>" & vbcrlf & "                          <iframe name=""uploadicofrm"" style=""width:1px;height:1px;overflow:hidden"" frameborder=""0""></iframe>" & vbcrlf & "                 </td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "        <div style='height:16px;overflow:hidden'></div>" & vbcrlf & " <table style='width:100%'>" & vbcrlf & "    <tr id=""warmTips"" style=""display:none;"">" & vbcrlf & "        <td>" & vbcrlf & "<div id=""notice"" style=""padding-left:18px;"">" & vbcrlf & "                <p>1.支持上传的图标格式为jpg,gif,png;</p>" & vbcrlf & "                <p>2.上传图标大小需小于2KB，且小于20*20;</p>" & vbcrlf & "            </div>" & vbcrlf & "        </td>" & vbcrlf & "    </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td align='center'>" & vbcrlf & "                  <button class='button' onclick='if(addnewTopBarItem("
Response.write Id
Response.write ")==true){app.closeWindow(""newcItemdlg"")}'>保存</button>&nbsp;&nbsp;<button onclick='app.closeWindow(""newcItemdlg"")' class='button'>取消</button>" & vbcrlf & "                 "
if xg = 1 and id>1000 then
Response.write "" & vbcrlf & "                     &nbsp;&nbsp;<button onclick='if(delTopBarItem("
Response.write Id
Response.write ")==true){app.closeWindow(""newcItemdlg"")}'class='button'>删除</button>" & vbcrlf & "                  "
end if
Response.write "" & vbcrlf & "             </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & ""
end sub
sub App_showtoolico
dim id, userid, rs, bindata, imgname
userid =  info.user
id = abs(request.QueryString("id"))
set rs = cn.execute("select imgbindata, img from home_toolbar_user where id=" & id  & " and uid=" & userid)
if rs.eof= false then
bindata = rs("imgbindata").value
imgname = split(rs("img").value, ".")
end if
rs.close
set rs = nothing
Dim xml : Set xml=Server.CreateObject("MSXML2.DOMDocument")
xml.resolveExternals=False
xml.loadxml("<data>" & bindata & "</data>")
xml.documentElement.setAttribute "xmlns:dt","urn:schemas-microsoft-com:datatypes"
'xml.loadxml("<data>" & bindata & "</data>")
xml.documentElement.dataType = "bin.base64"
response.Clear
response.ContentType = "image/" & imgname(ubound(imgname))
response.BinaryWrite  xml.documentElement.nodeTypedValue
set xml = nothing
end sub
sub App_addnewTopBarItem
dim tit, ico , stoped, url , sort , umsg , rs2
dim id, rs, maxord
id = abs(request.form("id"))
tit = request.form("title")
...
stoped = abs(request.form("stoped"))
url = request.form("url")
umsg = request.form("umsg")
sort = abs(request.form("sort"))
imgbindata = request.form("imgbindata")
imgfilename = request.form("imgfilename") & ""
set rs = server.CreateObject("adodb.recordset")
maxord = cn.execute("select isnull(max(ID),0)+1 from home_toolbar_user where uid=" & info.user).fields(0).value
'set rs = server.CreateObject("adodb.recordset")
if maxord < 10001 then maxord= 10001
if id = 0 then id = maxord
rs.open "select ID,title,sortnum,img,url,msgnumurl,target,uid,hide,qxlb,qxlblist, imgbindata from home_toolbar_user where uid=" & info.user & " and id=" & id , cn , 1, 3
if  rs.eof  then
rs.addnew
rs.fields("ID").value = clng(ID)
rs.fields("qxlb").value = 0
rs.fields("qxlblist").value = 0
rs.fields("target").value = 2
rs.fields("url").value = ""
rs.fields("msgnumurl").value = ""
if id < 10001 then
set rs2 = cn.execute("select qxlb, qxlblist,url, msgnumurl from home_toolbar_comm where id =" & id)
if rs2.eof = false then
rs.fields("qxlb").value = rs2.fields("qxlb").value
rs.fields("qxlblist").value = rs2.fields("qxlblist").value
rs.fields("url").value = rs2.fields("url").value
rs.fields("msgnumurl").value = rs2.fields("msgnumurl").value
end if
rs2.close
end if
end if
...
if umsg <> "sys_def" then
rs.fields("msgnumurl").value = umsg
end if
rs.fields("title").value = tit
rs.fields("sortnum").value = sort
rs.fields("hide").value = stoped
rs.fields("uid").value = info.user
if ico = "0" then
rs.fields("imgbindata").value =imgbindata
rs.fields("img").value =imgfilename
else
rs.fields("img").value = ico
rs.fields("imgbindata").value = ""
end if
rs.update
rs.close
Response.write "1"
...
sub App_SearchSet
Response.write "<div id='searchtree' style='position:relative;overflow:auto'>"
dim tvw , rs , nd , n
set tvw = new treeview
tvw.id = "searchlist"
tvw.indent = 25
...
nd.text = "检索栏目"
nd.ico = "../../skin/" & info.skin & "/images/ico_menu_03.gif"
nd.canSelect = false
set rs = cn.execute("select x.cls,x.ID,isnull(z.stoped,0) as hide from home_search_config_def x left join home_search_config_us z on x.id= z.id and z.uid=" & info.user & " where x.usign='" & Info.UniqueName & "' and (x.qxlb = 0 or exists(select 1 from power y where abs(qx_open-2)=1 and ord=" & info.user & " and x.qxlb=y.sort1 and x.qxlblist=y.sort2) or exists(select 1 from power y where  ord=" & info.user & ")) order by isnull(z.sort,x.id)")
while rs.eof = false
set n = nd.nodes.add
if rs.fields("hide").value = 1 then
n.color = "#666"
end if
n.text = rs.fields("cls").value
n.value = rs.fields("ID").value
rs.movenext
wend
rs.close
Response.write tvw.html
Response.write "</div><div id='SearchItemPanel'></div>"
Response.write "<div id='searchbottom'></div>"
end sub
...
sub App_GetSearchItem
dim id , sort, rs
id =  app.getint("clsid")
set rs = cn.execute("select x.id, x.fields, z.stopfields, x.cls,x.ID,isnull(z.stoped,0) as hide,  isnull(z.sort,x.id) as sort from home_search_config_def x left join home_search_config_us z on x.id= z.id and z.uid=" & info.user & " where x.id=" & id & " and x.usign='" & Info.UniqueName & "'")
if rs.eof then
Response.write "<span style='color:red;position:relative;top:20px;left:20px'>提示：检索栏目无效！</span>"
rs.close
exit sub
end if
Response.write "" & vbcrlf & "              <table style='position:relative;top:10px;left:10px;width:100%'>" & vbcrlf & "         <tr>" & vbcrlf & "                    <td height='26' align='right'>栏目排序：</td>" & vbcrlf & "                   <td colspan='5'>" & vbcrlf & "                                <select onchange='UpdateSearchConfig("
Response.write id
Response.write ",""sort"",this.value)'>" & vbcrlf & "                           "
sort = rs.fields("sort").value
for i = 1 to 20
if sort = i then
Response.write "<option value='" & i & "' selected>" & i & "</option>"
else
...
next
Response.write "" & vbcrlf & "                              </select>" & vbcrlf & "                       </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td height='26' align='right'>启用状态：</td>" & vbcrlf & "                   <td colspan='5'>" & vbcrlf & "                                <input type='radio' name='sr_qy' onclick='UpdateSearchConfig("
Response.write id
Response.write ",""stoped"",this.checked?0:1)' class='radio' id='sr_qy1' "
if rs.fields("hide").value = 0 then Response.write "checked"
Response.write "><label for='sr_qy1'>启用</label> &nbsp;" & vbcrlf & "                              <input type='radio' name='sr_qy' onclick='UpdateSearchConfig("
Response.write id
Response.write ",""stoped"",this.checked?1:0)' class='radio' id='sr_qy2' "
if rs.fields("hide").value = 1 then Response.write "checked"
Response.write "><label for='sr_qy2'>停用</label>" & vbcrlf & "                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                            <th class='lvwheader'>检索字段</th>" & vbcrlf & "                             <th class='lvwheader'>启用状态</th>" & vbcrlf & "                             <th class='lvwheader'>检索字段</th>" & vbcrlf & "                             <th class='lvwheader'>启用状态</th>" & vbcrlf & "                          <th class='lvwheader'>检索字段</th>" & vbcrlf & "                             <th class='lvwheader'>启用状态</th>" & vbcrlf & "                     </tr>" & vbcrlf & "                   "
dim allFields , disFields , i , x, stopfields, nallFields
nallFields=  split(rearrName(rs.fields("fields").value),"|")
allfields =  split(rs.fields("fields").value,"|")
stopfields =  "|" & rs.fields("stopfields").value & "|"
...
Response.write "<td class='lvw_cell disqy2'>" & nallFields(i) & "</td>"
else
Response.write "<td class='lvw_cell disqy2'>" & allFields(i) & "</td>"
end if
if instr(stopfields,"|" & allfields(i) & "|" ) > 0 then
Response.write "<td class='lvw_cell disqy' align='center'><input type=checkbox onclick='src_fieldsClick(this);UpdateSearchConfig(" & id &",""cfs"",""" & allfields(i)  & "|"" + (this.checked?1:0))'></td>"
'if instr(stopfields,"|" & allfields(i) & "|" ) > 0 then
else
Response.write "<td class='lvw_cell' align='center'><input type=checkbox checked onclick='src_fieldsClick(this);UpdateSearchConfig(" & id &",""cfs"",""" & allfields(i)  & "|"" + (this.checked?1:0))'></td>"
'if instr(stopfields,"|" & allfields(i) & "|" ) > 0 then
end if
next
i = i - 1
'if instr(stopfields,"|" & allfields(i) & "|" ) > 0 then
if i mod 3<>2 then
for x = (i mod 3) + 1 to 2
'if i mod 3<>2 then
Response.write "<td class='lvw_cell'></td><td class='lvw_cell'></td>"
next
Response.write "</tr>"
end if
Response.write "" & vbcrlf & "              </table><br>" & vbcrlf & "            <!--&nbsp;&nbsp;<button class='button' style='width:60px;' onclick='app.msgbox(""保存完成"")'>保存</button>-->" & vbcrlf & "              &nbsp;&nbsp;提示您：对检索进行设置操作时，系统会自动保存操作结果。" & vbcrlf & "              "
Response.write "</tr>"
rs.close
end sub
sub App_UpdateSearchConfig
dim id , cmd , value , fattr , v
dim rs
id = abs(request.form("id"))
cmd = request.form("cmd")
value = request.form("value")
set rs = server.CreateObject("adodb.recordset")
rs.open "select * from home_search_config_us where uid = " & info.user & " and  id=" & id , cn , 1 , 3
if rs.eof then
rs.addnew
rs.fields("id").value = id
rs.fields("uid").value = info.user
rs.fields("stoped").value = 0
rs.fields("sort").value = id
end if
select case lcase(cmd)
case "sort":
rs.fields("sort").value = abs(value)
case "stoped"
rs.fields("stoped").value = abs(value)
case "cfs"
fattr = split(value,"|")
v = "|" & rs.fields("stopfields").value & "|"
if abs(fattr(1)) = 1 then
v = replace(v,"|" & fattr(0) & "|","|")
else
if instr(v,"|" & fattr(0) & "|") = 0 then
v = v & fattr(0)
end if
end if
v = replace("###" & replace(replace(v,"||","|"),"||","|") & "###","###|","")
v = replace(replace(v, "|###",""),"###","")
rs.fields("stopfields").value = v
end select
rs.update
rs.close
Response.write 1
end sub
sub App_setTopMenuUsered
dim uState , id , rs, rs2
uState = app.getint("uState")
id = app.getint("id")
set rs = server.CreateObject("adodb.recordset")
rs.open "select * from  home_topmenu_item_us where ID=" & id & " and uid=" & info.user, cn, 1, 3
if rs.eof then
rs.addnew
set rs2 = cn.execute("select * from home_topmenu_item_def where id=" & id & " and usign='" & Info.UniqueName & "'")
if rs2.eof = false then
rs.fields("ID").value = rs2.fields("ID").value
rs.fields("title").value = rs2.fields("title").value
rs.fields("sort").value = rs2.fields("sort").value
rs.fields("cls").value = rs2.fields("cls").value
rs.fields("url").value = rs2.fields("url").value
rs.fields("uid").value = info.user
rs.fields("sysId").value = rs2.fields("ID").value
rs.fields("otype").value = rs2.fields("otype").value
else
Response.write "菜单句柄ID{"& id & "}无效"
exit sub
end if
rs2.close
end if
rs.fields("stop").value = abs(1-uState)
'rs2.close
rs.update
rs.close
Response.write 1
...
Sub App_SmpCardSet
Response.write "<div style='height:2px;overflow:hidden'></div>"
Dim lvw
Set lvw = New listview
lvw.id = "cardsdata"
lvw.checkbox =  False
...
lvw.pagesize = 70
lvw.cansort = false
lvw.sql = "exec home_marcard_cybg_config '" & app.power.models & "'," & Info.user
lvw.headers(1).display = "none"
lvw.headers(2).formatText = "@value&nbsp;<a href='javascript:void(0)' class='fun' title='点击修改报表的所属栏目' onclick='UpdateRptCls(@cells[1])'>修改</a>"
Response.write lvw.html
Response.write "</div>&nbsp;&nbsp;提示您：进行设置操作时，系统会自动保存操作结果。"
end sub
Sub App_RgtCardSet
Response.write "<div style='height:2px;overflow:hidden'></div>"
Dim lvw
...
lvw.cansort = False
cn.cursorlocation = 3
Dim sql,rsRemind,remind,disableID : disableID = "0"
sql = "select a.*,isnull(b.num1,4) num1,isnull(b.intro,'0') opened,isnull(b.gate1,1) gate1,b.tq1,b.fw1,0 subCfgId " & vbcrlf &_
"from reminderConfigs a " & vbcrlf &_
"inner join home_maincards_def h on a.setjmId = h.setJM " & vbcrlf &_
"left join ( " & vbcrlf &_
"select ord,intro,fw1,tq1,gate1,num1 from setjm where cateid=63 and subCfgId=0 " & vbcrlf &_
") b on a.setjmId=b.ord "
Set rsRemind = cn.execute(sql)
While rsRemind.eof = False
Set remind = CreateReminderHelperByRs(cn,rsRemind)
If remind.hasModule <> True Then disableID = disableID & "," & rsRemind("setjmId")
rsRemind.movenext
wend
lvw.sql = "exec home_marcard_cytx_config '" & disableID & "'," & app.Info.user
lvw.headers("hid").display = "none"
lvw.headers("fw1").display = "none"
if(Application("sys.info.systemtype")=3) then
lvw.headers("栏目").display = "none"
end if
lvw.headers("显示范围").formula = "fwShow(@cells[""hid""],@cells[""fw1""],@value)"
lvw.pagesize = 70
Response.write lvw.html
Response.write "</div>&nbsp;&nbsp;提示您：进行设置操作时，系统会自动保存操作结果。"
end sub
Function fwShow(hid,fw1,v)
If v = "" Then
fwShow = ""
Exit Function
end if
Dim arrFw,i,arrFwGroup,radio,fwName,fwValue
arrFwGroup = Split(v,",")
For i=0 To ubound(arrFwGroup)
arrFw = Split(arrFwGroup(i),":")
fwName = arrFw(0)
fwValue = arrFw(1)
radio = radio & _
"<input onclick='updateCardfw(" & hid & "," & fwValue & ")' type=radio " & _
"app.iif(fwValue = fw1&"""",""checked"","""") & "" name='bbstfw"" & hid & ""' id='bbstfw"" & hid & ""_"" & fwValue & ""'>"" & fwName"
next
fwShow = radio
end function
Sub App_updateCard
...
Set rs = server.CreateObject("adodb.recordset")
id = app.getInt("id")
rs.open "select * from home_maincards_us where uid=" & Info.User & " and id=" & id , cn , 1, 3
If rs.eof Then
Set rs1 = cn.execute("select ID,cardClass,title,sql,sql2,colspan,maxspan,sort,qxlb,qxlblist,model,mustadmin,canadd,canset,canmore,canclose,visible,monthjs,defjs,gjjs,attrs,setJM,fw,defRows,canqt from home_maincards_def where id=" & id)
If rs1.eof = False Then
rs.addnew
...
Response.write "不识别数据ID" & id
Exit sub
end if
rs1.close
end if
Select Case LCase(app.gettext("cmdtype"))
Case "visible":
rs.fields("visible").value = app.getint("value")
Set rs1 = server.CreateObject("adodb.recordset")
rs1.open "select * from setjm where cateid = " & Info.user & " and ord=" & rs.fields("setjm").value , cn , 1, 3
If  rs1.eof Then
rs1.addnew
rs1.fields("ord").value = rs.fields("setjm").value
rs1.fields("cateid").value = Info.User
rs1.fields("fw1").value = 0
rs1.fields("num1").value = 4
end if
rs1.fields("intro").value = app.getint("value")
rs1.update
rs1.close
Case "sort"
rs.fields("sort").value = app.getint("value")
Set rs1 = server.CreateObject("adodb.recordset")
rs1.open "select * from setjm where cateid = " & Info.user & " and ord=" & rs.fields("setjm").value , cn , 1, 3
If  rs1.eof Then
rs1.addnew
...
rs1.fields("gate1").value = app.getint("value")
rs1.update
rs1.close
Case "tsfw"
Set rs1 = server.CreateObject("adodb.recordset")
...
rs1.fields("tq1").value = app.getint("value")
rs1.update
rs1.close
Case "maxrows"
Set rs1 = server.CreateObject("adodb.recordset")
...
rs1.fields("cateid").value = Info.User
rs1.fields("fw1").value = 0
rs1.fields("tq1").value = 10
end if
rs1.fields("num1").value = app.getint("value")
rs1.update
rs1.close
rs.fields("defrows").value = app.getint("value")
Case "cardpower"
Set rs1 = server.CreateObject("adodb.recordset")
rs1.open "select * from setjm where cateid = " & Info.user & " and ord=" & rs.fields("setjm").value , cn , 1, 3
If  rs1.eof Then
...
rs1.fields("fw1").value = app.getint("value")
rs1.update
rs1.close
Case "":
End Select
...
Sub App_enableTopBarItem
dim id, rs, maxord , v, rs2
id = app.getint("id")
v = app.getint("v")
set rs = server.CreateObject("adodb.recordset")
maxord = cn.execute("select isnull(max(ID),0)+1 from home_toolbar_user where uid=" & info.user).fields(0).value
...
if maxord < 10001 then maxord= 10001
if id = 0 then id = maxord
rs.open "select ID,title,sortnum,img,url,msgnumurl,target,uid,hide,qxlb,qxlblist,sortnum from home_toolbar_user where uid=" & info.user & " and id=" & id , cn , 1, 3
if  rs.eof  Then
rs.addnew
rs.fields("ID").value = clng(ID)
rs.fields("qxlb").value = 0
...
rs.fields("msgnumurl").value = ""
if id < 10001 then
set rs2 = cn.execute("select title, qxlb, qxlblist,url, msgnumurl, sortnum, img from home_toolbar_comm where id =" & id)
if rs2.eof = false then
rs.fields("qxlb").value = rs2.fields("qxlb").value
rs.fields("qxlblist").value = rs2.fields("qxlblist").value
rs.fields("url").value = rs2.fields("url").value
rs.fields("msgnumurl").value = rs2.fields("msgnumurl").value
rs.fields("sortnum").value = rs2.fields("sortnum").value
rs.fields("uid").value = Info.User
rs.fields("title").value = rs2.fields("title").value
rs.fields("img").value = rs2.fields("img").value
end if
rs2.close
end if
end if
rs.fields("hide").value = v
rs.update
rs.close
Response.write "1"
end sub
...
dim sql
sql = "" &_
"create proc [dbo].[#home_getCardsData](@ty int , @uid int , @tag varchar(100), @models varchar(5000)) " & vbcrlf & _
"as " & vbcrlf & _
"begin " & vbcrlf & _
"set nocount on " & vbcrlf & _
"declare @mi int, @mx int " & vbcrlf & _
"--获取首页卡片数据 " & vbcrlf & _
"declare @mi int, @mx int " & vbcrlf & _
"if @ty = 1  --获取大类 " & vbcrlf & _
"declare @mi int, @mx int " & vbcrlf & _
"begin " & vbcrlf & _
"set @mi = 0 " & vbcrlf & _
"set @mx = 100000 " & vbcrlf & _
"select cardclass from ( " & vbcrlf & _
"select cardclass, min(id) as id from ( " & vbcrlf & _
"select cardclass,qxlb,qxlblist,id from home_maincards_def a " & vbcrlf & _
"where visible=1 and id >@mi and id <@mx " & vbcrlf & _
"and not exists(select 1 from home_maincards_us b where uid=@uid and b.id= a.id) and dbo.evalModel(@models,model)=1 " & vbcrlf & _
"and dbo.evalPower(@uid,isnull(powers,''))=1 " & vbcrlf & _
"union all " & vbcrlf & _
"select cardclass,qxlb,qxlblist,id from home_maincards_us a " & vbcrlf & _
"where dbo.evalModel(@models,isnull((select top 1 model from home_maincards_def x where x.id = a.Id),''))=1 " & vbcrlf & _
"and dbo.evalPower(@uid,isnull((select top 1 powers from home_maincards_def  x where x.id = a.Id),''))=1 " & vbcrlf & _
"and uid = uid and isnull(nClsPosId,id) >@mi and isnull(nClsPosId,id) <@mx and  visible = 1 " & vbcrlf & _
") x " & vbcrlf & _
"where qxlb=0 " & vbcrlf & _
"or exists(" & vbcrlf & _
"select * from power y where ord=@uid and (case sort2 when 19 then (case qx_open when 1 then 0 else 1 end) else abs(2-qx_open) end) = 1 and y.sort1= x.qxlb and y.sort2= x.qxlblist" & vbcrlf & _
"or exists(" & vbcrlf & _
") " & vbcrlf & _
"group by cardclass " & vbcrlf & _
") ss order by id " & vbcrlf & _
"end " & vbcrlf & _
"if @ty = 2  --获取首页右侧导航卡片 " & vbcrlf & _
"end " & vbcrlf & _
"begin " & vbcrlf & _
"set @mi = 10000 " & vbcrlf & _
"set  @mx = 20000 " & vbcrlf & _
"select x.ID,x.title title,x.maxspan,x.colspan,x.sort,x.mustadmin,x.canadd,x.canmore,x.canclose,x.attrs,x.addurl,setjm from ( " & vbcrlf & _
"select a.ID,a.title,a.qxlb,a.qxlblist,a.maxspan,a.colspan,a.sort,a.mustadmin,a.canadd,a.canmore,a.canclose," & vbcrlf &_
"attrs,addurl,setjm " & vbcrlf &_
"from home_maincards_def a " & vbcrlf &_
"inner join setjm b on a.setjm = b.ord and b.cateid=@uid and b.intro='1'" & vbcrlf &_
"where a.id >@mi and a.id <@mx and cardclass = tag " & vbcrlf & _
"and not exists(select 1 from home_maincards_us b where uid=@uid and b.id= a.id)  " & vbcrlf & _
"union all " & vbcrlf & _
"select a.ID,a.title,a.qxlb,a.qxlblist,a.maxspan,a.colspan,a.sort,a.mustadmin,a.canadd,a.canmore,a.canclose," & vbcrlf &_
"a.attrs,addurl,a.setjm " & vbcrlf &_
"from home_maincards_us a " & vbcrlf &_
"inner join home_maincards_def y on y.id=a.id " & vbcrlf &_
"inner join setjm b on a.setjm = b.ord and b.cateid=@uid and b.intro='1'" & vbcrlf &_
"where a.uid = uid and isnull(a.nClsPosId,a.id) >@mi and isnull(a.nClsPosId,a.id) <@mx and a.cardclass = tag " & vbcrlf & _
") x " & vbcrlf &_
"order by x.sort,x.id " & vbcrlf & _
"end " & vbcrlf & _
"if @ty = 3  --获取首页中部导航卡片 " & vbcrlf & _
"end " & vbcrlf & _
"begin " & vbcrlf & _
"set @mi = 0 " & vbcrlf & _
"set @mx = 10000 " & vbcrlf & _
"select ID,title,colspan,sort,mustadmin,canadd,canmore,canclose,monthjs,defjs,gjjs,attrs,addurl,setjm from ( " & vbcrlf & _
"select a.ID,a.title,a.qxlb,a.qxlblist,a.maxspan,a.colspan,a.sort,a.mustadmin,a.canadd," & vbcrlf &_
"a.canmore,a.canclose,a.monthjs,a.defjs,a.gjjs,a.attrs,a.addurl,a.setjm " & vbcrlf & _
"from home_maincards_def a " & vbcrlf & _
"where a.visible=1 and a.id >@mi and a.id <@mx and a.cardclass = tag " & vbcrlf & _
"and not exists(select 1 from home_maincards_us b where uid=@uid and b.id= a.id) " & vbcrlf & _
"and dbo.evalModel(@models, model)=1 " & vbcrlf & _
"and dbo.evalPower(@uid, isnull(powers,''))=1  " & vbcrlf & _
"union all " & vbcrlf & _
"select a.ID,a.title,a.qxlb,a.qxlblist,a.maxspan,a.colspan,a.sort,a.mustadmin,a.canadd," & vbcrlf &_
"a.canmore,a.canclose,a.monthjs,a.defjs,a.gjjs,a.attrs,b.addurl,b.setjm " & vbcrlf & _
"from home_maincards_us a " & vbcrlf & _
"inner join home_maincards_def b on a.id=b.id " & vbcrlf &_
"where dbo.evalModel(@models,isnull((select top 1 model from home_maincards_def x where x.id = a.Id),''))=1 " & vbcrlf & _
"and dbo.evalPower(@uid,isnull((select top 1 powers from home_maincards_def x where x.id = a.Id),''))=1 " & vbcrlf & _
"and a.uid = uid and isnull(a.nClsPosId,a.id) >@mi and isnull(a.nClsPosId,a.id) <@mx " & vbcrlf &_
"and a.cardclass =@tag and a.visible = 1 " & vbcrlf & _
") x " & vbcrlf & _
"where qxlb=0 or exists(" & vbcrlf & _
"select * from power y where ord=@uid and (case sort2 when 19 then (case qx_open when 1 then 0 else 1 end) else abs(2-qx_open) end) = 1 and y.sort1= x.qxlb and y.sort2= x.qxlblist" & vbcrlf & _
"where qxlb=0 or exists(" & vbcrlf & _
") " & vbcrlf & _
"order by sort , id " & vbcrlf & _
"end " & vbcrlf & _
"if @ty = 4 --获取2级页面右侧栏目 此处 @tag=栏目名称 " & vbcrlf & _
"end " & vbcrlf & _
"begin " & vbcrlf & _
"select @tag as title " & vbcrlf & _
"end " & vbcrlf & _
...
sub App_UpdateRptClsPage
dim id, rs , currcls , title
id = app.getInt("id")
set rs = cn.execute("select cardClass,title from home_maincards_us where uid =" & info.user  & " and id=" & id)
if rs.eof then
set rs = cn.execute("select cardClass,title from home_maincards_def where id=" & id)
end if
if rs.eof = false then
currcls = rs.fields(0).value
title = rs.fields(1).value
else
Response.write "要配置的报表不存在"
rs.close
exit sub
end if
rs.close
Response.write "" & vbcrlf & "        <div style='height:5px;overflow:hidden'></div>" & vbcrlf & "        <table align=center style='width:90%;color:#000;height:50px;'>" & vbcrlf & "            <tr><td style='width:40%' align='right'>报表名称：</td><td>&nbsp;"
Response.write title
Response.write "</td></tr>" & vbcrlf & "            <tr><td  align='right'>所在栏目：</td><td>&nbsp;<select id='rptclsListbox'>" & vbcrlf & "             "
Dim canshow
call createProc
set rs = cn.execute("exec #home_getCardsData 1," & info.user & ",'','" & app.power.models & "'")
while rs.eof = False
canshow = false
If rs.fields(0).value="驾驶舱" And (app.power.existsModel(6000) Or app.power.existsModel(7000) or app.power.existsModel(15000) Or app.power.existsModel(27000)) Then
canshow=True
end if
If rs.fields(0).value="办公管控" And (app.power.existsModel(31000) Or app.power.existsModel(28000) or app.power.existsModel(51000) Or app.power.existsModel(29000)) Then
canshow=True
end if
If rs.fields(0).value="财务管控" And (app.power.existsModel(36000) Or app.power.existsModel(27000) Or app.power.existsModel(19000) or app.power.existsModel(24000) or app.power.existsModel(23000)) Then
canshow=True
end if
If rs.fields(0).value="库存管控" And (app.power.existsModel(17000) Or app.power.existsModel(15000) or app.power.existsModel(16000) or app.power.existsModel(34000)) Then
canshow=True
end if
If rs.fields(0).value="人资管控" And (app.power.existsModel(39000) Or app.power.existsModel(39002) or app.power.existsModel(39004)) Then
canshow=True
end if
If rs.fields(0).value="销售管控" And (app.power.existsModel(33000) Or app.power.existsModel(6000) or app.power.existsModel(7000)) Then
canshow=True
end if
If rs.fields(0).value="生产管控" And app.power.existsModel(18000) Then
canshow=True
end if
If canshow then
if rs.fields(0).value <> currcls then
Response.write "<option value='" & rs.fields(0).value & "'>" & rs.fields(0).value & "</option>"
else
Response.write "<option value='" & rs.fields(0).value & "' selected>" & rs.fields(0).value & "</option>"
end if
end if
rs.movenext
wend
rs.close
Response.write "" & vbcrlf & "            </select>" & vbcrlf & "            </td></tr>" & vbcrlf & "        </table>" & vbcrlf & "        <table align='center'>" & vbcrlf & "            <tr><td align='center' style='height:30px' valign='bottom'>" & vbcrlf & "            <button style='width:50px;height:22px'class='button' onclick='saveRptClsUpdate("
Response.write id
Response.write ",document.getElementById(""rptclsListbox"").value)'>确定</button>&nbsp;&nbsp;" & vbcrlf & "            <button style='width:50px;height:22px' class='button' onclick='app.closeWindow(""setrptCls"")'>取消</button>" & vbcrlf & "            </td></tr>" & vbcrlf & "        </table>" & vbcrlf & "    "
end sub
sub App_UpdateRptClsSave
Dim id , rs, rs1, i, cls
Set rs = server.CreateObject("adodb.recordset")
id = app.getInt("id")
cls = app.getText("cls")
...
rs("nclsPosId").value = rs.fields("ID").value
rs("cardClass").value = cls
rs.update
rs.close
Response.write "ok"
end sub
