...
'if qjret2<>"" then
	i=1
	k=0
	sql="SELECT a.D_assID,SUM(D_money) as allmoney,b.ass_name,b.ass_cycle,b.ass_ycycle,b.ass_addcateid,b.ass_money,c.sort1 FROM "&_
	...
	"GROUP BY D_assID,b.ass_name,b.ass_cycle,b.ass_ycycle,ass_addcateid,b.ass_money,c.sort1 "
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "已提折旧明细_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
...
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).Value =rs("ass_cycle")
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money"))),num_dot_xs,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).Value =Formatnumber(ml.IsNum(zbcdbl(rs("allmoney"))),num_dot_xs,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).Value =rs("ass_ycycle")
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).HorizontalAlignment=1
'if not rs.eof then
			sql8="select a.*,convert(varchar(7),a.ass_jttime,120) as jttime,b.name from O_assDeprect a left join gate b on a.D_cateid=b.ord where D_del=2 and D_assID="&rs("D_assID")&" "& Str_Result2 &" order by D_num asc"
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				do while not rs8.eof
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,5).Value =Formatnumber(ml.IsNum(zbcdbl(rs8("D_money"))),num_dot_xs,-1)
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,5).font.Size=10
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,5).HorizontalAlignment=1
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,6).Value =rs8("D_num")
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,6).font.Size=10
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,6).HorizontalAlignment=1
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,7).Value =rs8("jttime")
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,7).font.Size=10
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,7).HorizontalAlignment=1
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,8).Value =D_time
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,8).font.Size=10
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,8).HorizontalAlignment=1
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,9).Value =rs8("name")
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,9).font.Size=10
					D_time=rs8("D_time")
					xlWorksheet.Cells(2+i,9).HorizontalAlignment=1
					D_time=rs8("D_time")
					i=i+1
					D_time=rs8("D_time")
					rs8.movenext
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>已提折旧明细导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!已提折旧明细导出成功,共有"&(k)&"条记录"&(i-k-1)&"条明细!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("已提折旧明细_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的已提折旧明细</u></strong></font></a></p>"
