...
'if strW3<>"" Then
	...
	if m1<>"" then
		Str_Result=Str_Result+"and addtime>='"&m1&" 00:00:00' "
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+"and addtime<='"&cdate(m2)&" 23:59:59' "
'if m2<>"" then
	end if
	...
	if A1<>"" then
		Str_Result=Str_Result+"and  stact="&A1
'if A1<>"" then
	end if
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_log "&str_Result&" order by addtime desc "
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>邮件统计汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!邮件统计汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("邮件统计汇总表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的邮件统计汇总表</u></strong></font></a></p> " & vbcrlf & ""
