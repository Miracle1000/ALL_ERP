...
'if strW3<>"" Then
		...
		Sub page_load
			Dim Ptr_Result,Str_Result
			Set conn = cn
			Dim px : px=request("px")
			Str_Result=""''
			Ptr_Result=""
			...
			Dim ret : ret=request("ret")
			Dim ret2 : ret2 = request("ret2")
			Dim content : content = request("content")
			Dim conkey : conkey=request("conkey")
			if ret<>"" Then Str_Result=Str_Result&" and datediff(dd,'"& ret &"',date7)>=0" 'Str_Result=Str_Result&" and year(date7)>="&year(ret)&" and month(date7)>="&month(ret)&" and day(date7)>="&day(ret)
			if ret2<>"" Then Str_Result=Str_Result&" and datediff(dd,date7,'"& ret2 &"')>=0"       'Str_Result=Str_Result&" and year(date7)<="&year(ret2)&" and month(date7)<="&month(ret2)&" and day(date7)<="&day(ret2)
			if conkey<>"" then
			...
			Dim names : names=request("name")
			if names<>"" Then Str_Result=Str_Result&" and name like '%"&names&"%'"
			Dim email : email=request("email")
			if email<>"" Then Str_Result=Str_Result&" and email like '%"&email&"%'"
			Dim sex : sex=request("sex")
			if sex<>"" Then Str_Result=Str_Result&" and sex ='"&sex&"'"
			Dim qq : qq=request("qq")
			if qq<>"" Then Str_Result=Str_Result&" and qq like '%"&qq&"%'"
			Dim sorts : sorts=request("sort")
			if sorts<>"" Then Str_Result=Str_Result&" and sort in ("&sorts&")"
			Dim msn : msn=request("msn")
			if msn<>"" Then Str_Result=Str_Result&" and msn like '%"&msn&"%'"
			Dim phone1 : phone1=request("phone1")
			if phone1<>"" Then Str_Result=Str_Result&" and phone1 like '%"&phone1&"%'"
			Dim ret5 : ret5=request("ret5")
			if ret5<>"" Then Str_Result=Str_Result&" and month(birthday) >="&month(ret5)&" and day(birthday) >="&day(ret5)
			Dim ret6 : ret6=request("ret6")
			if ret6<>"" Then Str_Result=Str_Result&" and month(birthday) <="&month(ret6)&" and day(birthday) <="&day(ret6)
			Dim phone2 : phone2=request("phone2")
			if phone2<>"" Then Str_Result=Str_Result&" and phone2 like '%"&phone2&"%'"
			Dim age1 : age1=request("age1")
			if age1<>"" Then Str_Result=Str_Result&" and age>="&age1
			Dim age2 : age2=request("age2")
			if age2<>"" Then Str_Result=Str_Result&" and age2<="&age2
			Dim fax : fax=request("fax")
			if fax<>"" Then Str_Result=Str_Result&" and fax like '%"&fax&"%'"
			Dim familyadd : familyadd=request("familyadd")
			if familyadd<>"" Then Str_Result=Str_Result&" and familyadd like '%"&familyadd&"%'"
			Dim mobile1 : mobile1=request("mobile1")
			if mobile1<>"" Then Str_Result=Str_Result&" and mobile1 like '%"&mobile1&"%'"
			Dim comadd1 : comadd1=request("comadd1")
			if comadd1<>"" Then Str_Result=Str_Result&" and comadd1 like '%"&comadd1&"%'"
			Dim comadd : comadd=request("comadd")
			if comadd<>"" Then Str_Result=Str_Result&" and comadd like '%"&comadd&"%'"
			Dim xl : xl=request("xl")
			if xl<>"" Then Str_Result=Str_Result&" and xl like '%"&xl&"%'"
			Dim zy : zy=request("zy")
			if zy<>"" Then Str_Result=Str_Result&" and zy like '%"&zy&"%'"
			Dim xw : xw=request("xw")
			if xw<>"" Then Str_Result=Str_Result&" and xw like '%"&xw&"%'"
			Dim shool : shool=request("shool")
			if shool<>"" Then Str_Result=Str_Result&" and shool like '%"&shool&"%'"
			Dim mz : mz=request("mz")
			if mz<>"" Then Str_Result=Str_Result&" and mz like '%"&mz&"%'"
			Dim jg : jg=request("jg")
			if jg<>"" Then Str_Result=Str_Result&" and jg like '%"&jg&"%'"
			Dim smoke : smoke=request("smoke")
			if smoke<>"" Then Str_Result=Str_Result&" and smoke='"&smoke&"'"
			Dim wine : wine=request("wine")
			if wine<>"" Then Str_Result=Str_Result&" and wine='"&wine&"'"
			Dim tea : tea=request("tea")
			if tea<>"" Then Str_Result=Str_Result&" and tea='"&tea&"'"
			Dim food : food=request("food")
			if food<>"" Then Str_Result=Str_Result&" and food like '%"&food&"%'"
			Dim drive : drive=request("drive")
			if drive<>"" Then Str_Result=Str_Result&" and drive='"&drive&"'"
			Dim car : car=request("car")
			if car<>"" Then Str_Result=Str_Result&" and car='"&car&"'"
			Dim note : note=request("note")
			if note<>"" Then Str_Result=Str_Result&" and note like '%"&note&"%'"
			Dim ret3 : ret3=request("ret3")
			Dim time1 : time1=request("time1")
			Dim time2 : time2=request("time2")
			if ret3<>"" Then Str_Result=Str_Result&" and date7>='"&ret3 &" " & time1 &":"&time2&":00'"
			Dim ret4 : ret4=request("ret4")
			Dim time3 : time3=request("time3")
			Dim time4 : time4=request("time4")
			if ret4<>"" Then Str_Result=Str_Result&" and date7<'"&ret4 &" " & time3 &":"&time4&":00'"
			Dim share : share=request("share")
			if share<>"" then
				if share="1" then
			...
			Dim Fileds,Export_Fileds,SQL
			Fileds="[name] as [姓名],"&_
			"(case when sort='' then '' else (select name from txltype where ord="&session("personzbintel2007")&" and id=sort) end) as [分类],"&_
			"sex [性别],phone1 [家庭电话],phone2 [办公电话],fax [传真],mobile1 [移动电话],mobile2 [手机],"&_
			"email [邮箱],[qq],[msn],birthday [生日],age [年龄],familyadd [家庭住址],postcode [邮编],company [所在单位],comtel [单位电话],"&_
			"comfax [单位传真] , comadd [单位地址],positions [担当职务],comadd1 [公司地址],zip [单位邮编],xl [学历],xw [学位],zy [专业],"&_
			"shool [毕业院校],jg [籍贯],mz [民族],high [身高],weigh [体重],tx [体型],xm [鞋码],smoke [是否吸烟],smotype [喜欢烟牌],"&_
			"wine [是否饮酒],winetype [喜欢酒类],winesign [喜欢酒牌],winel [酒量],tea [是否喝茶],teatype [喜欢茶类],food [饮食忌口],"&_
			"health [疾病情况],drive [驾照],car [私车],cartype [私车车型],carcolor [私车颜色],cardrive [私车牌照],tz [明显特征],tc [爱好特长],note [备注]"
			Export_Fileds="[姓名],[分类],[性别],[家庭电话],[办公电话],[传真],[移动电话],[手机],[邮箱],[qq],[msn],[生日],[年龄],[家庭住址],[邮编],[所在单位],[单位电话],[单位传真],[单位地址],[担当职务],[公司地址],[单位邮编],[学历],[学位],[专业],[毕业院校],[籍贯],[民族],[身高],[体重],[体型],[鞋码],[是否吸烟],[喜欢烟牌],[是否饮酒],[喜欢酒类],[喜欢酒牌],[酒量],[是否喝茶],[喜欢茶类],[饮食忌口],[疾病情况],[驾照],[私车],[私车车型],[私车颜色],[私车牌照],[明显特征],[爱好特长],[备注]"
			Dim rs1,sql1
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select name,title,sort from zdy where sort1=94 and set_open=1 and dc=1 order by gate1 asc "
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				do until rs1.eof
					if rs1("sort")=1 then
						Fileds=Fileds&","&"(case when isnull("&rs1("name")&",'')='' then '' else (select sort1 from sortonehy where ord="&rs1("name")&") end) as ["&rs1("title")&"]"
					else
						Fileds=Fileds&","&rs1("name")&" as ["&rs1("title")&"]"
					end if
					Export_Fileds = Export_Fileds & ",[" & rs1("title") & "]"
					rs1.movenext
				loop
			end if
			rs1.close
			set rs1=nothing
			Dim baseSql
			baseSql="select " & Fileds & " " &_
			"from tongxunlu  where del=1 " &_
			"Str_Result"&""& vbcrlf &_
			"and (addcateid="&session(personzbintel2007)&" or share like '%"&session(personzbintel2007) &"%' or share='0')" &_
			"Ptr_Result"
			Dim adapter
			Dim sheetSetting
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "通讯录资料"
			Dim hd,fds,i,fd,sname
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = baseSql
			sheetSetting.title = "通讯录资料"
			fds = Split(Export_Fileds,",")
