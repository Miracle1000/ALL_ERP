...
'if request("khbh")<>"" then
			...
			if request("contractname")<>"" then
				str_Result=str_Result+"and contractth in (select ord from contractth where title like '%"& request("contractname") &"%') "
'if request("contractname")<>"" then
			end if
...
'if cint(request("bz"))>0 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select a.*,b.bankname from payout2 a " & vbcrlf &_
			"left join (" & vbcrlf &_
			"select id bankid,sort1 bankname from sortbank where del=1 and (person like '"&session("personzbintel2007")&",%' or person like '%,"&session("personzbintel2007")&",%' or person like '%, "&session("personzbintel2007")&",%'  or person like '%,"&session("personzbintel2007")&"' or person like '%, "&session("personzbintel2007")&"'  or person like '"&session("personzbintel2007")&"' or person like '0') " & _
			") b on a.bank = b.bankid " &_
			" " & Str_Result & " " & px_Result
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "退款列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0  then
				...
				do until rs.eof
					...
					if rs("contractth")<>"" then
						...
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select title,thType,cateid from contractth where ord="&rs("contractth")&" and del=1"
						rs1.open sql1,conn,1,1
						if rs1.eof then
							contractname="退货单已被删除"
							thType = ""
						else
							cateid=rs1("cateid")
							If open_41_1 = 3 Or (open_41_1 = 1 And CheckPurview(intro_41_1,trim(cateid))=True) then
					...
					if rs("pay")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select * from sortonehy where ord="&rs("pay")&""
						rs7.open sql7,conn,1,1
						if rs7.eof then
							pay=""
						else
					...
					date1=rs("date1")
					date2=rs("date2")
					sortbz=getbzflag(rs("contractth"),"contractthid")
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>退款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!退款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("退款列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的退款列表</u></strong></font></a></p>"
