...
window.getIntoChooseProcPage = function (el) {
    ...
    $ID("childrefreshEventbox").value = 1;
    bill.triggerFieldEvent($ID("childrefreshEventbox"), "change");
    el.setAttribute("url", info.hosturl + "/mobilephone/salesManage/product/billlist.asp?fromtype=contractbill&Ismode=1&company=" + company + "&invoiceType=" + invoiceType);
    ui.CZSMLPage(el);
}
...
window.procSumvForSpecPage = function (dbname, sumv) {   
    ...
    var zk = $("#c_zk").val() || 0;
    if (yhval == 0) {
        sumv = bill.FormatNumber(sumv * 1, __currwin.zsml.header.moneybit)
    } else {
        sumv = bill.FormatNumber(sumv * 1, __currwin.zsml.header.moneybit)
    }
    return sumv;
...
window.HandleFieldFormul = function (currDBName, mBit) {    
    switch (currDBName) {
        case "PriceIncludeTax":/*正式报价产品明细-含税单价赋值*/
            ...
            var priceAfterTax = $("#PriceIncludeTax_" + rowinx).val() * 1 * $("#discount_" + rowinx).val() * 1;
            priceAfterTax = bill.FormatNumber(priceAfterTax, pricestore);
            $('span[showdbname="priceAfterTax_' + rowinx + '"]').html(priceAfterTax);
            break;
        case "c_yhmoney":               
            var money1sumv = $("#money1sumv").text();
            var c_yhmoney = $("#c_yhmoney").val();
            $("#yhmoney").val(bill.FormatNumber(c_yhmoney * 1, mBit));
            $("#xs_yhsum").html(bill.FormatNumber(money1sumv * 1, mBit));
            bill.triggerFieldEvent($ID("YhfreshEventbox"), "change");
            break;
        case "c_zk":
            var money1sumv = $("#money1sumv").text();
            var c_zk = $("#c_zk").val();
...
window.HandleSaleType = function () {
    ...
    if (yhval != window.ytypeValue) {
        ...
        if (window.ytypeValue == 1) {
            ...
            var money1sumv = $("#money1sumv").text();
            var zk = $("#c_zk").val();
            $("#xs_yhsum").html(bill.FormatNumber((money1sumv * 1) + "", __currwin.zsml.header.moneybit))
        } else {
...
window.HandlePlanTypeCallBack = function () {
    var btval = $("input[dbname='plan']").val();
    if (btval == 6) {
        $("tr[for='CKAccModel']").css("display", "");
    } else {
        $("tr[for='CKAccModel']").css("display", "none");
    }    
}
...
window.updateSumForSpcFun = function (lvw, rowindex, dbi, v) {
    ...
    for (var i = 0; i < hds.length; i++) {
        if (hds[i].dbname == "MoneyAfterTax") { midx = i; }
    ...
    if (hds[dbi].dbname =="PriceIncludeTax") {
        ...
        lvw.rows[rowindex][midx] = bill.FormatNumber(v * 1 * (lvw.rows[rowindex][nidx] * 1) * (lvw.rows[rowindex][disdx] * 1), __currwin.zsml.header.moneybit);
    }
    if (hds[dbi].dbname == "num1") {
        v = bill.FormatNumber(v, __currwin.zsml.header.numberbit);
        lvw.rows[rowindex][nidx] = v;
        lvw.rows[rowindex][midx] = bill.FormatNumber(v * 1 * (lvw.rows[rowindex][pidx] * 1) * (lvw.rows[rowindex][disdx] * 1), __currwin.zsml.header.numberbit);
    }
}
...
window.clearLvwForCaigou = function () {
    var dbsign = $("#MobListView_contractlist").attr("dbsign");
...
window.createBtnForSpecGroup = function (btn) {
    return "<div class='bill_txt' onclick='window.clearBtn(this);' action='SysBillCallBack' url='ClearAllHtMxList'  target='" + (btn.target || "") + "' >" + btn.title + "</div>"
}

window.clearBtn = function (el) {
    var ev = window.event;
    ev.stopPropagation();
    ui.confirm("您确定要清空合同明细?", function (e) {
        if (e.index == 1) {
            var parms = new Object();
            ui.CZSMLPage(el);
        }
...
window.onBeforePageInit = function () {
    ...
    scanfbtn.formathtml = "<div style='width:100%;height:36px;border-bottom:1px solid #bbb;background:#EFEFEF;display:none'><span id='money1sumv' style='height:36px;line-height:36px;color:#666;float:right;margin-right:10px;'></span><span style='height:36px;line-height:36px;color:#666;float:right;'>合同金额：</span></div>"
...
app.addMessageEvent("refresh_address", function (data, closeWinhwnd) {
    if (!data || closeWinhwnd.indexOf("_mobilechoseaddress") == -1) {
        return;
    }
    $ID("addressRefreshEventbox").value = data;
    bill.triggerFieldEvent($ID("addressRefreshEventbox"), "change");
});