...
{
	var sid=setID();
	document.getElementById('selectiddiv').innerHTML="";
	...
	var url = "ajax_yp1.asp?a=" + escape(u_name)+"&b=" + escape(u_bh)+"&c=" + escape(u_xh)+"&d=" + escape(u_ph)+"&e=" + escape(u_xlh)+"&t="+u_type+"&sid=" +sid+"&cp="+b+"&stamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
	xmlHttp.open("GET",url,false)
	xmlHttp.send(null);
...
function addMx(id,prod_name,prod_bh,prod_xh,sort1,prod_num,sto_name,sto_id,pro_name,prod_ph,prod_xlh,prod_sctime,prod_ystime,prod_1)
{
	...
	tr.appendChild(td5);
	td5.align="center";
	var delHref = "<input name=\"prod_num\" value=\""+prod_1+"\" type=\"text\" id=\"prod_num\" onKeyUp=\"value=value.replace(/[^\\d*|\\.]/g,'')\" onpropertychange=\"formatData(this,'number');\" style=\"width:100px;\"><span class=\"red\" name=\"numtext\"></span>";
	...
	tr.appendChild(td4);
	td4.align="center";
	var delHref = "<div align=\"center\"><input name=\"prod_num1\" type=\"hidden\" value=\"" + prod_num + "\" id=\"prod_num1\" onpropertychange=\"formatData(this,'number');\">" + prod_num + "</div>"
	...
	tr.appendChild(td12);
	td12.align="center";
	var delHref ="<INPUT type=\"text\" maxlength=\"200\" name=\"get_note1\" size=\"15\" style=\"width:140px;\">"
...
function ask1()
{
	...
	var get_type=document.getElementById('get_type').value;
	if (get_type==2)
	{
		if(document.getElementById('get_storecateid').value=="")
		{
			alert('库管人员不能为空')
			return false;
		}
	...
	var num1=document.getElementsByName('prod_num1');
	var sto_id=document.getElementsByName('sto_id');
	if(num.length==0)
	{
		alert('领用明细不能为空')
		return false;
	...
	{
	try{
			if ((parseFloat(num[i].value.replace(/,/g,""))>parseFloat(num1[i].value.replace(/,/g,"")))||parseFloat(num[i].value.replace(/,/g,""))==0||num[i].value.replace(/,/g,"")=="")
			{
				//alert(num[i].value+"-"+num1[i].value)
				alert('领用数量不能大于库存数量或者数量为零')
				return false;
				break;
			}
			if (sto_id[i].value=="")
...
function setType(val)
{
	if(val==1)
	{
		document.getElementById("get_storecateid").style.display='none'
		document.getElementById("type1").style.display='none'
	}
	else if(val==2)
	{
		document.getElementById("get_storecateid").style.display=''
		document.getElementById("type1").style.display=''
	}
}
function check_person(){
	$('#d_person').dialog('open');
	$('#d_person').dialog('move',{left:100,top:150});
}
function chanceR(name,ord){
	$("#get_lycateName").val(name);
	//document.getElementById("").value=name;
	$("#get_lycateid").val(ord);
	$('#d_person').window('close');
}
