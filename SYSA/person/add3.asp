...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write """;" & vbcrlf & "</script>" & vbcrlf & "<script src= ""../Script/pn_add3.js?ver="
	Response.write Application("sys.info.jsver")
	...
	ty = request("ty")
	gs_kh=""
	set rscl=conn.execute("select ts from celue  where  sort1=11 ")
	if Not rscl.eof Then gs_kh=rscl("ts")
	rscl.close
	Response.write "" & vbcrlf & "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "<tr>" & vbcrlf & "   <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0""cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "              <form method=""POST"" action=""save3.asp?ord="
	Response.write Split(ord,",")(0)
	Response.write "&h="
	Response.write h
	Response.write """id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" target=""saveform"">" & vbcrlf & "                     <tr><td class=""place"">联系人添加</td><td>&nbsp;</td><td align=""right"">&nbsp;<input type=""submit"" name=""Submit42"" value=""保存""  class=""page""/>&nbsp;&nbsp;&nbsp;&nbsp; <input type=""reset"" value=""重填"" class=""page"" name=""B2"">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td></tr>" & vbcrlf & "              </table>" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">    " & vbcrlf & "                <col width=""8%"" />" & vbcrlf & "                <col width=""25%""  />" & vbcrlf & "              <col width=""8%""  />" & vbcrlf & "               <col width=""25%"" />" & vbcrlf & "               <col width=""8%""  />" & vbcrlf & "               <col width=""25%"" />" & vbcrlf & "               <tr class=""top""><td height=""27"" colspan=""6"" ><div align=""left""><strong>基本情况</strong></div></td></tr>" & vbcrlf & "               "
	h_sql=""
	If h=2 Then
		h_sql= " or gate1=1 "
	else
		Response.write "<input type='hidden' id='name_id' name='name_id' value="&ord&">"
	end if
	Response.write "<input type='hidden' name='ty' value='"& ty &"'>"
	Set rs=conn.execute("select gate1,point,enter,sort2,name,oldname,fieldName,Required,format,type,(case gate1 when 13 then 33 else order1 end ) as order1 from setfields where sort=1 and show>0 and ((sort2=2 and gate1<>26) or gate1=13 "& h_sql &" ) order by order1 asc")
	If Not rs.eof Then
		n=0
		While Not rs.eof
			gate1=rs("gate1")
			...
			size=25
			max=100
			msg=name&"不能少于"&max&"个字"
			If Required=1 Then  msg=name&"必须在1至"&max&"个字之间"
			Case "person_name"
			size=25
			max=100
			msg=name&"不能少于"&max&"个字"
			...
			If fieldName="name" Then  strHTML=strHTML&  " colspan=3 "
			If fieldName="person_name" Then  strHTML=strHTML&  " colspan=3 "
			strHTML=strHTML&">"
			Select Case intType
			Case 1:
			strHTML=strHTML&" <select name='"&fieldName&"' "
			strHTML = strHTML& " dataType='"&dataType&"' "
			If Required=1 Then strHTML = strHTML& " min='"&min&"' "
			strHTML = strHTML& " max='"&max&"' msg='"&msg&"' "
			strHTML = strHTML& ">  "
			If fieldName="trade" Then
				set rss=conn.execute("select ord,sort1 from sortonehy where gate2=11 order by gate1 desc")
				while not rss.eof
			...
			ElseIf fieldName="role" Then
				set rss=conn.execute("select ord,sort1 from sort9 order by gate2 desc")
			...
			If fieldName="year1" Then
				...
				If fieldName="name" Then
					telname=""
					if ord<>"" then
						set rst=conn.execute("select name from tel where ord in ("&ord&")")
						if Not rst.eof Then telname=replace(rst("name").value,"""","&#34;")
						rst.close
					end if
					strHTML = strHTML& " value='"&telname&"' readonly='true' "
					n=n+1
					strHTML = strHTML& " value='"&telname&"' readonly='true' "
				ElseIf fieldName="address" Then
					strHTML = strHTML& " value='"&tel_address&"' "
				...
				ElseIf fieldName="person_name" Then
					strHTML = strHTML& " onKeyUp=""ff4(this.value,document.getElementById('pym'))"" style='width:180px' "
					n=n+1
					strHTML = strHTML& " onKeyUp=""ff4(this.value,document.getElementById('pym'))"" style='width:180px' "
				ElseIf fieldName="qq" Then
					strHTML = strHTML& " onKeyUp=""value=value.replace(/[^\d]/g,'')"" "
				end if
				If point=1 Or enter=1 Then strHTML = strHTML& " onChange=""callServer("&gate1&",this.value);"" onfocus=""callServer("&gate1&",this.value);"" "
				strHTML = strHTML& " size='"&size&"' "
				strHTML = strHTML& " dataType='"&dataType&"' "
				If Required=1 Then strHTML = strHTML& " min='"&min&"' "
				strHTML = strHTML& " max='"&max&"' msg='"&msg&"' "
				strHTML = strHTML& ">"
				If fieldName="name" Then  strHTML = strHTML& "<input name='"&fieldName&"_id' id='"&fieldName&"_id' type='hidden' value='"&ord&"'><a href='../search/result8.asp?page=person&ty="& ty &"'><span class='style1'>查找对应客户</span></a>"
				If fieldName="person_name" Then strHTML = strHTML& "&nbsp;&nbsp;拼音码：<input name='pym' type='text' id='pym' readonly='true' size='22' style='width:180px'>"
			end if
			Case 3 :
			strHTML = strHTML& "<input type='radio' name='"&fieldName&"' value='男'>男&nbsp;<input type='radio' name='"&fieldName&"' value='女'>女&nbsp;"
			Case 5 :
			strHTML=strHTML& " <input type='text' name='"&fieldName&"' id='"&fieldName&"' "
			If fieldName="age" Then strHTML = strHTML& " maxlength='3' "
			strHTML = strHTML& " onkeyup=""value=value.replace(/[^\d\.]/g,'')"" "
			strHTML = strHTML& " size='"&size&"' "
			strHTML = strHTML& " dataType='"&dataType&"' "
			If Required=1 Then strHTML = strHTML& " min='1' "
			strHTML = strHTML& " max='"&max&"' msg='"&msg&"' "
			strHTML = strHTML& ">"
			...
			If point=1 Then strHTML=strHTML& "<span class='red' name='ts' id='ts_"&gate1&"'></span>"
			If enter=1 Then strHTML=strHTML& "<input type='hidden' name='jzlr' value='"&gate1&"'>"
			Response.write strHTML &"</td>"
			If fieldName="year1" Then Response.write "<script language='javascript' defer='defer'>getBirthYear();</script>"
			n=n+1
			If fieldName="year1" Then Response.write "<script language='javascript' defer='defer'>getBirthYear();</script>"
			If n=2 Then
				Response.write "<td rowspan=3><div align=right>照　　片：</div></td>"
				Response.write "<td rowspan=3><img id='imageField' src='../person/img/noneperson.jpg' width='90' height='100' alt='头像'><input type='hidden' id='imgurl1' name='imgurl1' value=''></td>"
				n=n+1
			end if
			If n=5 Or n=8 Then n=n+1
			If n=11 Then
				Response.write "<td><div align='right'>上　　传：</div></td>"
				Response.write "<td class='gray'><div align='left'><input type='button' name='openbn' id='openbn' value='上传头像'  class='anybutton' onclick=""$('#w').window('open');""/></div></td>"
				n=n+1
			end if
			If fieldName="mobile" Then
				check=""
				If point=1 Or enter=1 Then
					check=" onChange=""callServer("&gate1+1&",this.value);"" onfocus=""callServer("&gate1+1&",this.value);"" "
'If point=1 Or enter=1 Then
				end if
				If n Mod 3=0 Then Response.write "</tr><tr>"
				btStr = iif(strformat = "1","MobileNull","Limit")
				Response.write "<td><div align='right'>"&name&"2：</div></td><td class='gray'>"
				Response.write "<input type='text' name='"&fieldName&"2' id='"&fieldName&"2' "&check&" size='"&size&"' dataType='" & btstr & "' max='"&max&"' msg='"&msg&"' >"
				If point=1 Then Response.write "<span class='red' name='ts' id='ts_"&gate1+1&"'></span>"
				If enter=1 Then Response.write "<input type='hidden' name='jzlr' value='"&gate1&"'>"
				If  open_67_13=1  and open_67_19<>1 then Response.write "&nbsp;<img src='../images/message.gif' onClick=""sendSms("&fieldName&"2.value);function sendSms(phone){if (phone.length==11){window.open('../message/topadd.asp?phone='+phone+'','newsSendSMS','width=900,height=800,fullscreen=no,scrollbars=0,toolbar=0,status=no,resizable=1,location=no,menubar=no,menubar=no,left=20,top=100');} else{ alert('请填写正确的号码！');}}""  style='cursor:hand' border='0' alt='发送短信' align='absbottom'>"
				Response.write "</td>"
				n=n+1
				Response.write "</td>"
			end if
...
'If n>11 Then
					...
					If n Mod 3=1 Then Response.write "<td colspan='2'></td>"
					If n Mod 3>0 Then n=n+3-(n Mod 3)
					If n Mod 3=1 Then Response.write "<td colspan='2'></td>"
					If n=3 Then Response.write "</tr><tr><td colspan=4>&nbsp;</td></tr><tr><td colspan=4>&nbsp;</td></tr><tr><td colspan=4>&nbsp;</td>"
					If n=6 Then Response.write "</tr><tr><td colspan=4>&nbsp;</td></tr><tr><td colspan=4>&nbsp;</td>"
					If n=9 Then Response.write "</tr><tr><td colspan=4>&nbsp;</td>"
					Response.write "<td><div align='right'>上　　传：</div></td>"
					Response.write "<td class='gray'><div align='left'><input type='button' name='openbn' id='openbn' value='上传头像'  class='anybutton' onclick=""$('#w').window('open');""/></div></td>"
				end if
				n=0
	...
	rs.close
	Response.write "" & vbcrlf & "<input name=""source"" type=""hidden"" size=""15"" max=""50"" value=""2"">" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "<tr class=""top""><td height=""27"" colspan=""6"" ><div align=""left""><strong>详细信息</strong></div></td></tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">学　　历：</div></td>" & vbcrlf & "<td height=""33"" class=""gray""><input name=""xl"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td height=""33""><div align=""right"">学　　位：</div></td>" & vbcrlf & "<td height=""33"" class=""gray""><input name=""xw"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">专　　业：</div></td>" & vbcrlf & "<td class=""gray""><input name=""zy"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">毕业院校：</div></td>" & vbcrlf & "<td height=""33"" class=""gray""><input name=""yx"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">民　　族：</div></td>" & vbcrlf & "<td class=""gray""><input name=""mz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align='right'>家庭地址：</div></td><td class='gray'><div align='left'><input name='address' type='text' size='32' dataType='Limit' max='100' msg='地址太长了，最多100个字'></div></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">身　　高：</div></td>" & vbcrlf & "<td class=""gray""><input name=""sg"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">体　　重：</div></td>" & vbcrlf & "<td class=""gray""><input name=""tz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">体　　型：</div></td>" & vbcrlf & "<td class=""gray""><input name=""tx"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">鞋　　码：</div></td>" & vbcrlf & "<td class=""gray""><input name=""xm"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">是否吸烟：</div></td>" & vbcrlf & "<td class=""gray""><input type=""radio"" name=""xy"" value=""吸烟"">" & vbcrlf & "吸烟" & vbcrlf & "<input type=""radio"" name=""xy"" value=""不吸烟"">" & vbcrlf & "不吸烟 </td>" & vbcrlf & "<td><div align=""right"">喜欢烟牌：</div></td>" & vbcrlf & "<td class=""gray""><input name=""xyname"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">是否饮酒：</div></td>" & vbcrlf & "<td class=""gray"">" & vbcrlf & "<div align=""left"">" & vbcrlf & "<input type=""radio"" name=""yj"" value=""饮酒"">" & vbcrlf & "饮酒" & vbcrlf & "<input type=""radio"" name=""yj"" value=""不饮酒"">" & vbcrlf & "不饮酒</div></td>" & vbcrlf & "<td><div align=""right"">喜欢酒类：</div></td>" & vbcrlf & "<td class=""gray""><div align=""left"">" & vbcrlf & "<input name=""yjsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "</div></td>" & vbcrlf & "<td><div align=""right"">喜欢酒牌：</div></td>" & vbcrlf & "<td class=""gray""><input name=""yjname"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "酒量：<input name=""yjsize"" type=""text"" size=""5"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">是否喝茶：</div></td>" & vbcrlf & "<td class=""gray""><input type=""radio"" name=""hc"" value=""喝茶"">" & vbcrlf & "喝茶" & vbcrlf & "<input type=""radio"" name=""hc"" value=""不喝茶"">" & vbcrlf & "不喝茶</td>" & vbcrlf & "<td><div align=""right"">喜欢茶类：</div></td>" & vbcrlf & "<td class=""gray""><input name=""hcsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">饮食忌口：</div></td>" & vbcrlf & "<td class=""gray""><input name=""jk"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">疾病情况：</div></td>" & vbcrlf & "<td class=""gray""><input name=""jb"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "<td><div align=""right"">驾　　照：</div></td>" & vbcrlf & "<td class=""gray""><div align=""left"">" & vbcrlf & "<input type=""radio"" name=""jz"" value=""有"">" & vbcrlf & "有" & vbcrlf & "<input type=""radio"" name=""jz"" value=""没有"">" & vbcrlf & "没有</div></td>" & vbcrlf & "<td><div align=""right"">私　　车：</div></td>" & vbcrlf & "<td class=""gray""><input type=""radio"" name=""sc"" value=""有"">" & vbcrlf & "有" & vbcrlf & "<input type=""radio"" name=""sc"" value=""无"">" & vbcrlf & "无" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">私车车型：</div></td>" & vbcrlf & "<td class=""gray""><div align=""left"">" & vbcrlf & "<input name=""scsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "</div></td>" & vbcrlf & "<td><div align=""right"">私车颜色：</div></td>" & vbcrlf & "<td class=""gray""><div align=""left"">" & vbcrlf & "<input name=""scys"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "</div></td>" & vbcrlf & "<td><div align=""right"">私车牌照：</div></td>" & vbcrlf & "<td class=""gray""><input name=""scpz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">明显特征：</div></td>" & vbcrlf & "<td colspan=""5"" class=""gray""><input name=""tezheng"" type=""text"" size=""60"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">爱好特长：</div></td>" & vbcrlf & "<td colspan=""5"" class=""gray"">" & vbcrlf & "<div align=""left"">" & vbcrlf & "<textarea name=""joy"" cols=""80"" rows=""4"" dataType=""Limit"" max=""3000""  msg=""最多3000个字""></textarea>" & vbcrlf & "</div></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td><div align=""right"">备　　注：</div></td>" & vbcrlf & "<td colspan=""5"" class=""gray"">" & vbcrlf & "<div align=""left"">" & vbcrlf & "<textarea name=""intro"" cols=""80"" rows=""4"" dataType=""Limit"" max=""5000""  msg=""最多5000个字"" style=""display:none""></textarea>" & vbcrlf & "<IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME>" & vbcrlf & "</div></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td  colspan=""6"" class=""gray""><p align=""center""><input type=""submit"" name=""Submit42"" value=""保存"" class=""page""/>" & vbcrlf & "&nbsp;&nbsp;&nbsp;&nbsp;<input type=""reset"" value=""重填"" class=""page"" name=""B2""></td>" & vbcrlf & "</tr>" & vbcrlf & "</form>" & vbcrlf & "</table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td  class=""page"">" & vbcrlf & "<table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "<tr>" & vbcrlf & "<td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "<IFRAME style=""display:none"" name=""I5"" id=""I5"" FRAMEBORDER=""0"" SCROLLING=""no"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "</table>" & vbcrlf & ""
	action1="联系人添加"
	call close_list(1)
	Response.write "" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""上传头像""  style=""width:500px;height:230px;padding:5px;background: #fafafa;top:60px;left:450px;""  closed=""true"" >" & vbcrlf & "  <div region=""center"" border=""false"" style=""padding:10px;background:#fff;border:0px solid #ccc; width:450px"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <form action=""saveimg.asp"" method=""post"" enctype=""multipart/form-data"" name=""updata"" id=""updata"" target=""I5"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td width=""28%"" height=""30"" valign=""middle""><div align=""right"">路径：</div></td>" & vbcrlf & "          <td width=""72%"" align=""left"" valign=""middle""><div align=""left"">" & vbcrlf & "              <input name=""imgurl"" class=""anybutton"" type=""file"" id=""imgurl"" style=""background-color:#FFFFFF;"">" & vbcrlf & "              <input name=""t""  id=""t"" type=""hidden"" value=""2"">" & vbcrlf & "          </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "                              <tr><td colspan=""2"" align=""center"">" & vbcrlf & "                友情提示：请将上传文件大小控制在200KB以内。<br/>" & vbcrlf & "                最佳尺寸：90*100(px)  &nbsp;格式要求：JPG/GIF/PNG </td></tr>" & vbcrlf & "      </form>" & vbcrlf & "    </table>" & vbcrlf & "  </div>" & vbcrlf & "  <br />" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:center;height:30px;line-height:30px;"">" & vbcrlf & "    <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('updata').submit()"">确认</a>" & vbcrlf & "    <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a> </div>" & vbcrlf & "</div>" & vbcrlf & "<iframe id=""saveform"" name=""saveform"" style=""width:0px;height:1px;display:none""></iframe>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
