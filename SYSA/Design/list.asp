...
'If sdk.power.existsPowerIntro(5029,14, designer) Then
	...
	Sub onReportInit(rpt, lt)
		Dim title,remindSql,remind , designtype , fromid
		...
		If remind > 0 Then
			...
			remindSql = Replace(helper.listSQL("ids"),"'","''")
			title = helper.name
		end if
		rpt.title = title
		rpt.addsort "设计主题","title"
		rpt.addsort "设计编号","bh"
		rpt.addsort "设计分类","sort1"
		rpt.addsort "设计等级","level"
		rpt.addsort "设计人员","designer"
		rpt.addsort "领用日期","date3"
		rpt.addsort "添加人员","creator"
		rpt.addsort "添加日期","indate"
		rpt.AddField2 "","select","designstatus", "未领用"&Chr(1)&"1"&Chr(2)&"申请中"&Chr(1)&"2"&Chr(2)&"已领用"&Chr(1)&"3"&Chr(2)&"待设计"&Chr(1)&"4"&Chr(2)&"设计中"&Chr(1)&"5"&Chr(2)&"设计完毕"&Chr(1)&"6"&Chr(2)&"待审批"&Chr(1)&"7"&Chr(2)&"审批通过"&Chr(1)&"8"&Chr(2)&"审批未通过"&Chr(1)&"9"&Chr(2)&"作废"&Chr(1)&"10", "", "","设计状态=0"
		rpt.AddField2 "","sortonehy","sort1", "5029", "", app.getint("sort1"),"设计分类=0"
		rpt.AddField2 "","select","B", "设计编号"&Chr(1)&"bh"&Chr(2)&"设计人员"&Chr(1)&"designer"&Chr(2)&"添加人员"&Chr(1)&"creator", "", "","设计主题=#title"
		rpt.addfield "","text","C", "", "", ""
		rpt.aSearch.addfield "设计主题","text","title",""
		rpt.aSearch.addfield "设计编号","text","bh",""
		rpt.aSearch.addfield "设计分类","checks","sort","sql:select sort1,cast(id as varchar) id from sortonehy where gate2=5029 order by gate1 desc"
		rpt.aSearch.addfield "设计等级","checks","level","sql:select sort1,cast(id as varchar) id from sortonehy where gate2=5030 order by gate1 desc"
		rpt.aSearch.addfield "设计状态","checks","dstatus","options:1=未领用;2=申请中;3=已领用;4=待设计;5=设计中;6=设计完毕;7=待审批;8=审批通过;9=审批未通过;10=作废"
		rpt.aSearch.addfield "设计人员", "gates", "designer", ""
		rpt.aSearch.addfield "领用日期","dates","date3",""
		rpt.aSearch.addfield "添加人员", "gates", "creator", ""
		rpt.aSearch.addfield "添加日期","dates","indate",""
		rpt.asearch.addZdyFields 5029
		app.addScriptPath app.virPath & "inc/CommSPAjax.js"
		Call app.addDefaultScript() '加载本文件关联JS ="../skin/default/js/[my_pagename].js
		rpt.pagesize = 10
		rpt.onlyadSearchModel =True
		rpt.BillListUIModel = True
		Dim strHandle : strHandle = ""
		If app.power.existsPower(5029,3) Then strHandle = "批量删除"
		If app.power.existsPower(5029,6) Then strHandle = strHandle & app.iif(Len(strHandle)>0 , ",","") & "批量指派"
		If app.power.existsPower(5029,4) Then strHandle = strHandle & app.iif(Len(strHandle)>0 , ",","") & "批量共享"
		If Len(strHandle)>0 Then rpt.setBatHandle "ord", strHandle '"批量删除,批量共享"
		rpt.Settings.clear
		rpt.canExcel = app.power.existsPower(5029,10)
		rpt.canPrint = app.power.existsPower(5029,7)
		Dim sql : sql = "exec [erp_design_List] "& Info.User &",0,"& designtype &","& fromid &",@designstatus ,@sort1 ,@B ,@C ,@title,@bh,@sort,@level,@dstatus,@designer,@date3_0 ,@date3_1,@creator,@indate_0 ,@indate_1,@zdy5029_1,@zdy5029_2,@zdy5029_3,@zdy5029_4,@zdy5029_5,@zdy5029_6,'"& remindSql &"' , &SearchMode ,&excelmode,&pagesize ,&pageindex, &ReportSort"
		rpt.sql = sql
	end sub
	Sub App_doBatHandle(cmd, data, result)
		Dim rs
		If cmd = "批量删除" Or  cmd = "删除" Then
			If cmd = "删除" Then Call CopyBillBeforChange(cn , -31 , data , "设计删除", info.user)
'If cmd = "批量删除" Or  cmd = "删除" Then
			Dim pwr_del : pwr_del = app.power.getPowerIntro(5029,3)
			If Len(data&"")=0 Then data = 0
			Set rs =cn.execute("select x.id , (case when isnull(y.id,0)>0 then 'none' else (case when ("& Len(pwr_del) &"=0 or CHARINDEX(','+CAST(x.designer  as varchar(12))+',',',"& pwr_del &",')>0) then 'url' else 'none' end) end) as url  from design x left join (select distinct b.id from designlist a innerjoin Design b on a.Design = b.id inner join dbo.M2_BOM c on c.fromtype=3 AND a.id = c.fromID and c.status<>0 and c.del=1) y on y.id = x.id where x.id in ("& data &")")
			While rs.eof =False
				If rs("url") = "url" Then
					cn.execute("update design set del = 2, delcate="& Info.user &",deltime=getdate() where id = "& rs("id"))
				else
					result.AddResult " 不能删除", rs("id"), "red"
				end if
				rs.movenext
			wend
			rs.close
			cn.execute("update designlist set del = 2 where design in (select id from design where del=2)")
		end if
	end sub
	Sub App_doSpHandle
		Dim sp_cateid , sp_level_id ,id
		id = app.getInt("id")
		sp_cateid = app.getint("@__sp_cateid")
		sp_level_id = app.getint("@__sp_level_id")
		Dim commSP : Set commSP = New CommSPHandle
		Call commSP.initById(id , 5029)
		Call commSP.saveBillBySdk(sp_level_id, sp_cateid)
	end sub
	Sub app_onCreateList(lvw,rpd)
		Dim remind : remind = rpd.getint("remind")
		lvw.checkbox = True
		lvw.colresize = False
		lvw.cansort=False
		lvw.checkvalue = "id"
		lvw.addJoinFields "date3,indate,addname"
		lvw.excelcallbackproc = "App_OnListviewExcel"
		lvw.checkboxwidth=6
		lvw.headers("id").display= "none"
		With lvw.headers("title")
		.title = "设计主题"
		.formatText = "code:titlehtml(@value ,rs(""id""), rs(""url"") , rs(""creator""))"
		.dbtype="str"
		.width = "100"
		End With
		With lvw.headers("DesignBH")
		.title = "设计编号"
		.dbtype="str"
		.width = "100"
		End With
		With lvw.headers("sort1name")
		.title = "设计分类"
		.width = "80"
		End With
		With lvw.headers("levelname")
		.title = "设计等级"
		.width = "80"
		End With
		With lvw.headers("designstatus")
		.title = "设计状态"
		.width = "80"
		.formatText = "code:showDesignStatus(@value)"
		End With
		With lvw.headers("designerName")
		.title = "设计人员"
		.width = "80"
		End With
		With lvw.headers("progress")
		.title = "设计进展"
		.formatText = "code:replyhtml(@value ,rs(""id"") ,rs(""designer""))"
		.dbtype= "str"
		End With
		With lvw.headers("indate")
		.title = "添加日期"
		.width = "110"
		End With
		With lvw.headers("date3")
		.title = "领用日期"
		.width = "110"
		End With
		With lvw.headers("addname")
		.title = "添加人员"
		.width = "110"
		End With
		With lvw.headers("url")
		.width = "170"
		.formatText = "code:buttonhtml(@value,rs(""id"") ,rs(""gx"") ,rs(""del"")  ,rs(""up""),rs(""zf""), rs(""designstatus"") , rs(""designer"") ,rs(""sp"") , rs(""cateid_sp""), rs(""sort1"") ,"& remind &")"
		End With
		lvw.headers("designer").display="none"
		lvw.headers("gx").display="none"
		lvw.headers("del").display="none"
		lvw.headers("up").display="none"
		lvw.headers("zf").display="none"
		lvw.headers("sp").display="none"
		lvw.headers("cateid_sp").display="none"
		lvw.headers("sort1").display="none"
		lvw.headers("creator").display="none"
	end sub
	Dim zbintel_design_zp_nolist
	Function titlehtml(value ,ord, url, creator )
		Dim s : s = value
		If (url="url"  Or creator=Info.user) And ord>0 Then
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">"& value &"</a>"
'If (url="url"  Or creator=Info.user) And ord>0 Then
		end if
		If len(zbintel_design_zp_nolist)=0 Then zbintel_design_zp_nolist = session("zbintel_design_zp_nolist") : session("zbintel_design_zp_nolist")=""
		If InStr(","& zbintel_design_zp_nolist &"," ,","& ord &",")>0 Then s = s &"&nbsp;<font style='color:red'>不允许指派</font>"
		titlehtml = s
	end function
	Function buttonhtml(value,ord ,gx ,del ,up, zf, designstatus , designer ,sp , cateid_sp , sptype, remind)
		Dim s,rid : s = ""
		If remind>0 Then
			set helper =  CreateReminderHelper(cn,remind,0)
			rid =  helper.getRemindIdByOID(ord)
			if helper.canCancelOrder(rid) Then s = s & "<img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0"" style=""cursor:pointer;margin-right:5px;"" rid=""" & rid & """ class=""cancelBtn"" onclick=""sendCancelRequest('"& rid &"',"& remind &")"">&nbsp;"
			rid =  helper.getRemindIdByOID(ord)
		end if
		If value="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">详情</button>&nbsp;"
'If value="url" And ord>0 Then
		end if
		If gx="url" And ord>0 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:setDesignShare("& ord &")"">共享</button>&nbsp;"
		end if
		If designstatus=6 And designer = Info.User Then
			s = s & "<button class=""oldbutton2"" onclick=""setSpProc("& ord &" , "& sptype &" , "& designer &" , 1)"">提交</button>&nbsp;"
		ElseIf designstatus=7 And sp="url" And cateid_sp =Info.User  Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('../inc/CommSPSet.asp?ord=" & app.base64.pwurl(ord) & "&sort1=5029&lvw=mlistvw','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">审批</button>&nbsp;"
'ElseIf designstatus=7 And sp="url" And cateid_sp =Info.User  Then
		end if
		if up="url" And ord>0 And designstatus<7 Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('add.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 900 + ',height=' + 520 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">修改</button>&nbsp;"
'if up="url" And ord>0 And designstatus<7 Then
		end if
		if del="url" And ord>0 Then
			Dim canAddNotice : canAddNotice = 0
			if ZBRuntime.MC(77000) And app.power.existsPowerIntro(5032 , 13, Designer) Then canAddNotice = 1
			s = s & "<button class=""oldbutton2"" onclick=""javascript:delDesign(" & ord & ",'"& app.base64.pwurl(ord) &"' , "& canAddNotice &")"">删除</button>&nbsp;"
		end if
		if zf="url" And ord>0 And designstatus<>10 Then
		end if
		buttonhtml = s
	end function
	Sub App_PreListviewExcel(lvw)
		lvw.mxzdyId=5029
	end sub
	sub App_ListviewZdySet(lvw)
		lvw.InsertZdyMap "", "btitle", "设计主题", 100
		lvw.InsertZdyMap "btitle", "DesignBH", "设计编号", 100
		lvw.InsertZdyMap "DesignBH", "sort1name", "设计分类", 80
		lvw.InsertZdyMap "sort1name", "levelname", "设计等级", 80
		lvw.InsertZdyMap "levelname", "designstatus", "设计状态", 80
		lvw.InsertZdyMap "designstatus", "designerName", "设计人员", 80
		lvw.InsertZdyMap "designerName", "date3", "领用日期", 80
		lvw.InsertZdyMap "date3", "plandate1", "计划开始日期", 100
		lvw.InsertZdyMap "plandate1", "plandate2", "计划完成日期", 100
		lvw.InsertZdyMap "plandate2", "d_zdy1", "", 100
		lvw.InsertZdyMap "d_zdy1", "d_zdy2", "", 100
		lvw.InsertZdyMap "d_zdy2", "d_zdy3", "", 100
		lvw.InsertZdyMap "d_zdy3", "d_zdy4", "", 100
		lvw.InsertZdyMap "d_zdy4", "d_zdy5", "", 100
		lvw.InsertZdyMap "d_zdy5", "d_zdy6", "", 100
		lvw.InsertZdyMap "unit", "date1", "设计开始日期", 100
		lvw.InsertZdyMap "date1", "date2", "设计完成日期", 100
	end sub
	Sub App_OnListviewExcel(lvw)
		lvw.headers("designstatus").formula = "showDesignStatus(@value)"
		lvw.headers("billtitle").title = "关联单据"
		Call setZdyCol(lvw ,5029 ,"d_")
	end sub
	Function replyhtml(reply_jz ,ord ,designer)
		Dim s :s = ""
		If ord>0 And app.power.existsPowerIntro(5029,15, Designer) Then
			Dim rs, date7, intro,uploadfile : intro=""
			set rs = cn.execute("select top 1 intro,date7,isnull(sort98,0) as sort98s,uploadfile from reply where sort1=5029 and del=1 and ord2="& ord &" order by date7 desc")
			if not rs.eof then
				intro=rs("intro")
				date7=rs("date7")
				uploadfile=rs("uploadfile")
				If Len(uploadfile&"")>0 Then uploadfile = Getuploadfile(uploadfile)
			end if
			rs.close
			if intro<>"" Or uploadfile&""<>"" then
				s = "<a href='#' onClick=""javascript:window.open('all.asp?ord="&app.base64.pwurl(ord)&"&sortreply="&app.base64.pwurl(5029)&"','newwinsd3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;"" title='点击查看全部洽谈进展' >"&intro&"</a>"&uploadfile &" <font class='red'>("&date7&")</font> "
			end if
			if reply_jz="url" Then
				s = s & " <img class='resetElementHidden' src='../images/jiantou.gif'><img class='resetElementShow' style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png'><a href='#' onClick=""javascript:window.open('reply.asp?action=listAdd&ord="&app.base64.pwurl(ord)&"&sortreply="&app.base64.pwurl(5029)&"','plancor50','width=' + 990 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;""><font class='blue2'>添加</font></a>"
'if reply_jz="url" Then
			end if
		end if
		replyhtml = s
