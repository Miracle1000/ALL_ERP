...
'If m1 = "" Then
	...
	If CDate(m2)< CDate(m1) Then
		m1 = m2
	end if
	If B<>"0" Then
		Str_Result = Str_Result + " bz="&B&" and "
'If B<>"0" Then
	end if
	conn.cursorlocation = 3
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""place"">银行账户汇总</td>" & vbcrlf & "              <form method=""POST"" action=""planall.asp?px="
	Response.write px
	Response.write """ id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "      <td align=""left"" class='top_btns'>" & vbcrlf & "                      自：<INPUT name=""ret"" size=""9"" readonly id=""daysOfMonthPos"" onMouseUp=""toggleDatePicker('daysOfMonth','date.ret')"" value="""
	Response.write m1
	Response.write """><DIV id=""daysOfMonth"" style=""POSITION: absolute""></DIV>" & vbcrlf & "                      &nbsp;至：" & vbcrlf & "                      <INPUT name=""ret2"" size=""9"" readonly id=""daysOfMonth2Pos"" onMouseUp=""toggleDatePicker('daysOfMonth2','date.ret2')""  value="""
	Response.write m2
	Response.write """>&nbsp;" & vbcrlf & "                    <DIV id=""daysOfMonth2"" " & vbcrlf & "style=""POSITION: absolute""></DIV>" & vbcrlf & "<SCRIPT language=JavaScript1.2>" & vbcrlf & "function Cancel() {hideElement(""daysOfMonth"");}" & vbcrlf & "</SCRIPT>" & vbcrlf & "" & vbcrlf & "<SCRIPT language=JavaScript1.2>" &vbcrlf & "<!--" & vbcrlf & "hideElement('daysOfMonth');" & vbcrlf & "hideElement('daysOfMonth2');" & vbcrlf & "//-->" & vbcrlf & "</SCRIPT>" & vbcrlf & ""
	Response.write m2
	if 0=1 then
		Response.write "" & vbcrlf & "<select name=""B"">" & vbcrlf & "       <option value=""0"">币种</option>" & vbcrlf & ""
		Set rs88 = server.CreateObject("adodb.recordset")
		rs88.Open "select * from sortbz where id in (select bz from sortbank where del=1)", conn, 1, 1
		If Not rs88.EOF Then
			Do While Not rs88.EOF
				Response.write "" & vbcrlf & "      <option value="""
				Response.write rs88("id")
				Response.write """ "
				if rs88("id")=cint(B) then
	...
	Response.write "" & vbcrlf & "<input type=""submit"" name=""Submit422"" value=""查看""  class=""anybutton whiteBtn""/>"
	if open_11_10=1 or open_11_10=3 Then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""汇总导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_bank.asp?W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
	...
	Response.write "" & vbcrlf & "</td>" & vbcrlf & "                        </form>" & vbcrlf & "      <td align=""right""></td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "     </table> " & vbcrlf & ""
	Dim bzarray
	bzarray = getrsarray("select distinct bz from sortbank where "&Str_Result&" (','+cast(person as varchar(max))+',' like '%, "&session("personzbintel2007")&",%'  or  ','+cast(person as varchar(max))+',' like '%,"&session("personzbintel2007")&",%') ")
'Dim bzarray
	If IsArray(bzarray) Then
		For i = 0 To UBound(bzarray, 2)
			Call banklist(bzarray(0, i))
		next
	else
		Response.write "<center>没有相关信息!</center>"
	end if
	Response.write "" & vbcrlf & "      <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;</p></div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>                    " & vbcrlf & "  <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "    <tr> " & vbcrlf & "      <td height=""10"" colspan=""2""><img src=""../image/pixel.gif"" width=""1"" height=""1""></td>" & vbcrlf & "    </tr> " & vbcrlf & "        <tr>" & vbcrlf & "      <td width=""16%"" height=""10""><div align=""right""></div></td>" & vbcrlf & "      <td width=""84%"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "     <tr>" & vbcrlf & "      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "     <tr>" & vbcrlf & "      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "  </table>" & vbcrlf & "     </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Dim actinon1
	action1 = "银行账户汇总"
	Call close_list(1)
	Response.write "" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	Sub banklist(Byval bzstr)
		Dim banklistarray, i, bzcn
		Dim money1all, money2all, money3all, money4all
		money1all = 0
		money2all = 0
		money3all = 0
		money4all = 0
		If bzstr = -1 Then
			money4all = 0
			banklistarray = getrsarray("select sortbank.id,sortbank.sort1,sortbank.intro,sortbank.bz ,sortbz.sort1  as sortbzstr from sortbank left join  sortbz on sortbz.id = sortbank.bz  where sortbank.del =1 and (','+cast(person as varchar(max))+',' like '%, "&session("personzbintel2007")&",%'  or  ','+cast(person as varchar(max))+',' like '%,"&session("personzbintel2007")&",%') order by gate1 desc")
			money4all = 0
		else
			banklistarray = getrsarray("select sortbank.id,sortbank.sort1,sortbank.intro,sortbank.bz ,sortbz.sort1  as sortbzstr from sortbank left join  sortbz on sortbz.id = sortbank.bz  where sortbank.del =1 and (','+cast(person as varchar(max))+',' like '%, "&session("personzbintel2007")&",%'  or  ','+cast(person as varchar(max))+',' like '%,"&session("personzbintel2007")&",%') and bz='"&bzstr&"' order by sortbank.gate1 desc")
			money4all = 0
			bzcn = getrsval("select top 1 sort1  from sortbz where id="&bzstr)
		end if
		If Not IsArray(banklistarray) Then
			Exit Sub
		end if
		Response.write "" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              "
		if bzstr<>-1 then
			Response.write "" & vbcrlf & "             <tr class="""">" & vbcrlf & "             <td colspan=""9"" style=""border:0px!important"">" & vbcrlf & "                       <div align=""left""><strong>"
			Response.write  bzcn
			Response.write "</strong></div>" & vbcrlf & "              </td>" & vbcrlf & "           </tr>" & vbcrlf & "           "
		end if
		Response.write "" & vbcrlf & "             <tr class=""top"">" & vbcrlf & "          <td align=""center""><div align=""center""><strong>账户名称</strong></div></td>" & vbcrlf & "         <td width=""17%"" height=""27"" align=""center""><div align=""center""><strong>银行账号</strong></div></td>" & vbcrlf & "             "
		if bzstr <> -1 then
			Response.write "" & vbcrlf & "             <td width=""8%"" align=""center""><div align=""center""><strong>币种</strong></div></td>" & vbcrlf & "            "
		end if
		Response.write "" & vbcrlf & "             <td width=""12%"" ><div align=""center""><strong>期初</strong></div></td>" & vbcrlf & "               <td ><div align=""center""><strong>本期收入</strong></div></td>" & vbcrlf & "             <td ><div align=""center""><strong>本期支出</strong></div></td>" & vbcrlf & "        <td><div align=""center""><strong>信用额度</strong></div></td>" & vbcrlf & "          <td ><div align=""center""><strong>余额</strong></div></td>" & vbcrlf & "         <td width=""12%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "               </tr>" & vbcrlf & "           "
		For i = 0 To UBound(banklistarray, 2)
			Dim money1, money2, money3
			money1 = getrsval("select (sum(round(money1,"&num_dot_xs&"))- sum(round(money2,"&num_dot_xs&"))) as money1 from bank where del=1 and bank="&banklistarray(0, i)&" and date1<'"&m1&"' group by bank ")
'Dim money1, money2, money3
			Set moneyrs = conn.execute("select sum(round(money1,"&num_dot_xs&")) as money2,sum(round(money2,"&num_dot_xs&")) as money3 from bank where del=1 and bank="&banklistarray(0, i)&" and date1>='"&m1&"' and date1<='"&m2&"' group by bank ")
			If isnul(money1) Then money1 = 0
			If Not moneyrs.EOF Then
				money2 =zbcdbl( moneyrs("money2"))
				money3 =zbcdbl( moneyrs("money3"))
			else
				money2 = 0
				money3 = 0
			end if
			...
			If bzstr <> -1 Then
'Dim hl
				hl = getrsval("select hl from hl where date1='"&Date&"' and bz="&bzstr)
				If isnul(hl) Then hl = 1
			else
				hl = 1
			end if
			dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			money4 = CDbl(Formatnumber(money1,num_dot_xs,-1,-1,0)) + CDbl(Formatnumber(money2,num_dot_xs,-1,-1,0)) - CDbl(Formatnumber(money3,num_dot_xs,-1,-1,0))
			'dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			money1all = money1all + cdbl(money1)
			'dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			money2all = money2all + cdbl(money2)
			'dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			money3all = money3all + cdbl(money3)
			'dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			money4all = money4all + cdbl(money4)
			'dblMinus=CDbl(getCanMinus(banklistarray(0, i)))
			Response.write "" & vbcrlf & "                     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   <td width=""15%"" align=""center"" class=""name"">"
			Response.write banklistarray(1,i)
			Response.write "</td>" & vbcrlf & "                        <td height=""27""  align=""center"" class=""name"">"
			Response.write banklistarray(2,i)
			Response.write "</td>" & vbcrlf & "                        "
			if bzstr <> -1 then
				'Response.write "</td>" & vbcrlf & "                        "
				Response.write "" & vbcrlf & "                     <td height=""27""  align=""center"" class=""name"">"
				Response.write banklistarray(4,i)
				Response.write "</td>" & vbcrlf & "                        "
			end if
			Response.write "" & vbcrlf & "                     <td  class=""name""><div align=""right"">"
			Response.write Formatnumber(money1,num_dot_xs,-1)
			'Response.write "" & vbcrlf & "                     <td  class=""name""><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "                  <td width=""12%"" class=""name"" ><div align=""right"">"
			Response.write Formatnumber(money2,num_dot_xs,-1)
			'Response.write "</div></td>" & vbcrlf & "                  <td width=""12%"" class=""name"" ><div align=""right"">"
			Response.write Formatnumber(money3,num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "                  <td width=""12%"" ><div align=""right"">"
			Response.write FormatNumber(dblMinus,num_dot_xs,-1)
			'Response.write "</div></td>" & vbcrlf & "                  <td width=""12%"" ><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "            <td width=""12%"" ><div align=""right"">"
			Response.write Formatnumber(money4,num_dot_xs,-1)
			'Response.write "</div></td>" & vbcrlf & "            <td width=""12%"" ><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "                  <td> " & vbcrlf & "                    <div align=""center""><input type=""button"" name=""Submit3"" value=""账目明细""  onClick=""javascript:window.open('list.asp?id="
			Response.write banklistarray(0,i)
			Response.write "&ret="
			Response.write m1
			Response.write "&ret2="
			Response.write m2
			Response.write "','newwin','width=' + 1024 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" class=""anybutton""/> " & vbcrlf & "                        </div>" & vbcrlf & "                  </td>" & vbcrlf & "                   </tr>   " & vbcrlf & "                "
			'Response.write m2
		next
		Response.write "" & vbcrlf & "                     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   "
		if bzstr <> -1 then
			Response.write "" & vbcrlf & "                     <td height=""27"" colspan=""3"" align=""right"" class=""name"">共计：</td>" & vbcrlf & "                      "
		else
			Response.write "" & vbcrlf & "                     <td height=""27"" colspan=""2"" align=""right"" class=""name"">共计：</td>" & vbcrlf & "                      "
		end if
		Response.write "" & vbcrlf & "                     <td  class=""name""><div align=""right""><font class=""red"">"
		Response.write Formatnumber(money1all,num_dot_xs,-1)
		Response.write "</font></div></td>" & vbcrlf & "                   <td class=""name"" ><div align=""right""><font class=""red"">"
		Response.write Formatnumber(money2all,num_dot_xs,-1)
		Response.write "</font></div></td>" & vbcrlf & "            <td class=""name"" ><div align=""right""><font class=""red"">"
		Response.write Formatnumber(money3all,num_dot_xs,-1)
		Response.write "</font></div></td>" & vbcrlf & "            <td class=""name""><div align=""right"">&nbsp;</div></td>" & vbcrlf & "                      <td ><div align=""right""><font class=""red"">"
		Response.write Formatnumber(money4all,num_dot_xs,-1)
		Response.write "</font></div></td>" & vbcrlf & "                   <td>&nbsp;</td>" & vbcrlf & "                 </tr>" & vbcrlf & "                   </table>" & vbcrlf & ""
	end sub
%>
