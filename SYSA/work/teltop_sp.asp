...
'Private Sub Class_Initialize
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "#kh .anybutton2{margin-bottom:0}" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function do_search(){" & vbcrlf & "   var val=document.getElementById(""sear"").value;" & vbcrlf & "    if(document.getElementById(""tit_1"").value==""F2""){window.location=""?remind="
	Response.write request("remind")
	Response.write "&F1=1&F2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""S2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&S1=1&S2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""Y2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&Y1=1&Y2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""G2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&G1=1&G2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""P2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&P1=1&P2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""J2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&J1=1&J2=""+val+""&sear=""+val;};" & vbcrlf & "       if(document.getElementById(""tit_1"").value==""T2""){window.location=""?remind="
	Response.write request("remind")
	Response.write request("remind")
	Response.write "&T1=1&T2=""+val+""&sear=""+val;};" & vbcrlf & "}" & vbcrlf & "function callServer2() {" & vbcrlf & "  var url = ""leibiao_js.asp?TName=1&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100)+""&E="
	Response.write request("remind")
	...
	Str_Result=" and ("&open_1_1&"=3 or ("&open_1_1&"=1 and cateadd in ("&intro_1_1&")) )"
	if B<>"" or (open_1_6=3 or open_1_6=1) or (F1<>"" and F2<>"") or (sort46=1 or sort46=2) Then Str_Result=Str_Result& " and del=1 and  sort3=1 "
	If Abandon<>"" Then Str_Result=Str_Result+" and  sortfq in ("&Replace(Abandon," ","")&") "
	Response.expires=-1
	Response.AddHeader"pragma","no-cache"
	Response.AddHeader"cache-control","no-store"
	if G<>"" Then Str_Result=Str_Result+" and  order1 in ("&G&") "
	W3=getW3(W1,W2,W3)
...
'case 1:
		case 2:
		Str_Result=Str_Result+"and  cateid_sp in  ("&W3&")"
'case 2:
		case 3:
		Str_Result=Str_Result+"and  (cateid_sp in  ("&W3&") and cateadd in ("&W3&"))"
'case 3:
		end select
	end if
	If H="31" Then
		...
		If isdate(v2)=True then Str_Result=Str_Result+" and datediff(d,date1,'" & v2 & "')>=0 and  cateadd in  ("&W31&")"
		'If Len(w31)=0 Then w31=0
	end if
	function menuarea(id1)
		set rsarea=server.CreateObject("adodb.recordset")
		sqlarea="select id,id1 from menuarea where id1="&id1&" "
		rsarea.open sqlarea,conn,3,1
		if rsarea.eof then
			gateord22=id1
	...
	Function menuarear_top(id,list)
		set rsarea=server.CreateObject("adodb.recordset")
		sqlarea="select id,id1 from menuarea where id="&id&" "
		rsarea.open sqlarea,conn,3,1
		if Not rsarea.eof then
			If rsarea("id1")=0 Then
				menuarear_top=0
			else
				If InStr(","&list&",",","&rsarea("id1")&",")>0 Then
					menuarear_top=menuarear_top(rsarea("id1"),list)
				else
					menuarear_top=1
				end if
			end if
	...
	if A2<>"" then
		...
		For i = 0 To UBound(aryReturn2)
			n=0
			n=menuarear_top(aryReturn2(i),A2)
			If n=0 Then
				set rs=conn.execute("select id from menuarea where id1="&aryReturn2(i)&" ")
				if rs.eof then
					n=0
				else
					do until rs.eof
				...
				rs.close
				if n=0 Then menuarea(aryReturn2(i))
			end if
		next
	end if
	if A2<>"" Then Str_Result=Str_Result+"and  area in ("&area_list&")"
	'if n=0 Then menuarea(aryReturn2(i))
...
'Select Case intType
	...
	ElseIf px=16 Then
		px_Result1="order by cateid_sp_px desc,date1 desc"
		px_Result2="order by cateid_sp_px asc,date1 asc"
		px_Result="order by cateidSpPx desc,tel.date1 desc,tel.ord asc"
	elseif px=17 then
		px_Result1="order by cateid_sp_px asc,date1 asc"
		px_Result2="order by cateid_sp_px desc,date1 desc"
		px_Result="order by cateidSpPx asc,tel.date1 asc,tel.ord asc"
	end if
	...
	if lie_5="" or isnull(lie_5) then lie_5=1
	Select Case lie_5
	...
	Response.write request("sear")
	Response.write """>" & vbcrlf & "                <input type=""button"" name=""submit2"" value=""检索"" class=""anybutton2"" onClick=""do_search()"" >" & vbcrlf & "               "
	if open_1_10=1 or open_1_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出资料"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({page:'../out/xls.asp?actdc=2',postParams:[{n:'px_v',v:document.getElementById('px_1').value},{n:'Str_Result_v',v:document.getElementById('Str_Result').value}]})}""  class=""anybutton2""/>"
	end if
	Response.write "" & vbcrlf & "             <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div></td><td width=""3""><img class=""resetElementHidden"" src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & "     <span id=""ht1""></span>" & vbcrlf & "    </form>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "   <tr class=""top"">" & vbcrlf & "    <td align=""center"" width=""5%""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "    <td width=""20%"" height=""27"" align=""center""><div align=""center""><strong>"
	Response.write arrName(1)
	Response.write "</strong></div></td>" & vbcrlf & "   <td width=""6%""><div align=""center""><strong>审核情况</strong></div></td>" & vbcrlf & "     <td width=""7%""><div align=""center""><strong>领用人</strong></div></td>" & vbcrlf & "       <td width=""7%""><div align=""center""><strong>审核人</strong></div></td>" & vbcrlf &"   <td width=""9%""><div align=""center""><strong>审核日期</strong></div></td>" & vbcrlf & "     <td width=""12%""><div align=""center""><strong>"
	Response.write arrName(4)
	Response.write "</strong></div></td>" & vbcrlf & "   <td width=""7%""><div align=""center""><strong>"
	Response.write arrName(5)
	Response.write "</strong></div></td>" & vbcrlf & "   <td width=""10%""><div align=""center""><strong>添加时间</strong></div></td>" & vbcrlf & "    <td width=""16%""><div align=""center"">" & vbcrlf & "          <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid #FFFFFF; overflow:hidden"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""lie_5=1"" "
	Response.write arrName(5)
	if lie_5=1 then
		Response.write "selected"
	end if
	...
	sqlcount="select count(*) from tel where sort3=1 and del<>2 and (isnull(sp,0)>0 or isnull(sp,0)=-1 "&newWhere&") "&Str_Result&" "
...
'if currpage=pagecount then
	...
	"from (" &_
	"select top "&page_count*CurrPage&" * ,(case isnull(cateid_sp,0) when 0 then isnull(intro_sp_cateid,0) else isnull(cateid_sp,0) end ) as cateid_sp_px " &_
	"from tel " &_
	"where sort3=1 and del<>2 and (isnull(sp,0)>0 or isnull(sp,0)=-1 "&newWhere&") "& Str_Result &_
	" "&px_Result1 &",ord asc " &_
	") e " &_
	" "&px_Result2&",ord desc " &_
	") a " &_
	" "&px_Result1&",ord asc "
	rs.open sql,conn,3,1
	if rs.eof then
		Response.write "<table style='width:100%'><tr><td  align='center' class='name'>没有信息!</td></tr></table>"
	else
		i=0
		...
		do until rs.eof
			...
			if cateid<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select num_6,num_7 from gate where ord="&cateid&" "
				rs1.open sql1,conn,1,1
				if not rs1.eof then
					time1=rs1("num_6")
					time2=rs1("num_7")
			...
			if open_1_3=3 or (open_1_3=1 and CheckPurview(intro_1_3,trim(cateid))=True) or open_1_6=3 or (open_1_6=1 and (CheckPurview(intro_1_6,trim(cateid))=True or CheckPurview(intro_1_6,trim(cateid4))=True Or CheckPurview(intro_1_6,trim(cateadd))=True)) Then
				Response.write "" & vbcrlf & "                               <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>" & vbcrlf & "                  "
			end if
			Response.write "" & vbcrlf & "               </td>" & vbcrlf & "           <td height=""27"" align=""center""><div align=""left"">" & vbcrlf & "                     "
			if open_1_14=3 Or (open_1_14=1 and CheckPurview(intro_1_14,trim(cateadd))=True  And cateadd<>0) then
				Response.write "" & vbcrlf & "                               <a href=""javascript:void(0)"" onclick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newttwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,resizable=1,toolbar=0,left=100,top=100');return false;"">" & vbcrlf & "                             "
				Response.write pwurl(rs("ord"))
			...
			if rs("status_sp")="1" then
				Response.write "" & vbcrlf & "                      <font class=""red"">待审核</font>" & vbcrlf & "           "
			elseif rs("status_sp")="2" then
				Response.write "" & vbcrlf & "                      <font class=""red"" status=2 >待审核</font>" & vbcrlf & "         "
			elseif rs("status_sp")="3" then
				Response.write "" & vbcrlf & "                      <font class=""name"">审核未通过</font>" & vbcrlf & "              "
			else
				Response.write "" & vbcrlf & "                      审核通过" & vbcrlf & "                "
			end if
			...
			Case "1" :  cateidname=""
			Case "2" :
			set rs7=conn.execute("select * from gate where ord='"&rs("cateid")&"'")
			...
			If rs("status_sp")=3 Then cateid_sp=rs("intro_sp_cateid")
			if cateid_sp<>"" then
				set rs7=conn.execute("select name from gate where ord="&cateid_sp&"")
				if not rs7.eof then spName=rs7("name")
				rs7.close
			end if
			Response.write "" & vbcrlf & "              <td><div align=""center"">"
			Response.write spName
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			Response.write rs("date_sp")
			...
			if open_1_14=3 or (open_1_14=1 and CheckPurview(intro_1_14,trim(cateadd))=True) then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('content.asp?issp=1&ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')""/>" & vbcrlf & "                    "
				Response.write pwurl(rs("ord"))
			end if
			if rs("cateid_sp")=session("personzbintel2007") And (open_1_6=3 or (open_1_6=1 and (CheckPurview(intro_1_6,trim(cateid4))=True or CheckPurview(intro_1_6,trim(cateid4))=True Or CheckPurview(intro_1_6,trim(cateadd))=True))) Then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3"" value=""审核""  onClick=""javascript:window.open('shenhe_set.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')""/>" & vbcrlf & "                    "
				Response.write pwurl(rs("ord"))
			end if
			if rs("cateid_sp")&""="" Or rs("cateid_sp")&""="0" then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3"" value=""提交""  onClick=""javascript:window.open('shenhe.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "&sort3=3','newwin','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')""/>" & vbcrlf & "                    "
				Response.write pwurl(rs("ord"))
			end if
			if open_1_2=3 or (open_1_2=1 and CheckPurview(intro_1_2,trim(cateadd))=True) then
			...
			if open_1_3=3 or CheckPurview(intro_1_3,trim(cateadd))=True then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit"" value=""删除"" onClick=""window.location.href='delkhconfirm.asp?"
				Response.write sdk.ReturnURL("pag=3&ord=" & pwurl(rs("ord")) &"")
				Response.write "';""/>" & vbcrlf & "                     "
			end if
		...
		if open_1_6=1 or open_1_6=3 then
			Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit427"" value=""批量审核""  onclick=""ask();"" class=""anybutton2""/>" & vbcrlf & "     "
		end if
		...
		Response.write "/"
		Response.write PageCount
		Response.write "页</span>&nbsp;" & vbcrlf & "     <input  size=3  type=text id=""currpage""   name=""currPage""   onpropertychange=""formatData(this,'int');"" maxlength=8>&nbsp;<input type=""button"" name=""Submit422"" value=""跳转"" onClick=""gotourl('currPage='+document.getElementById('currpage').value);""  class=""anybutton2""/>" & vbcrlf & "      "
		if currpage=1 then
		...
		Response.write sdk.ReturnURL("pag=3")
		Response.write """;" & vbcrlf & "        document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function ask()" & vbcrlf & "{" & vbcrlf & "    document.all.form1.action = ""shenhe_set.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """;" & vbcrlf & "        document.all.form1.target = ""_blank"";" & vbcrlf & "     document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "{" & vbcrlf & "var e = form.elements[i];" & vbcrlf & "if (e.name != 'chkall')" & vbcrlf & "e.checked = form.chkall.checked;" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	end if
	Set BC= server.createobject(ZBRLibDLLNameSN & ".Base64Class")
	Response.write "" & vbcrlf & "<input type='hidden' name=""Str_Result"" id='Str_Result' value='"
	Response.write BC.SafeEnCode(" where sort3=1 and del<>2 and (isnull(sp,0)>0 or isnull(sp,0)=-1 "&newWhere&") " & Str_Result)
	'Response.write "" & vbcrlf & "<input type='hidden' name=""Str_Result"" id='Str_Result' value='"
	...
	'Response.write arrName(3)
	Response.write "排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=16');""><font color=""#2F496E"">按审核人排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & "</tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & ""
	if remind<>"" then
		Response.write "" & vbcrlf & "jQuery(function(){" & vbcrlf & "   BindCancelEvents({cfgId:"
		Response.write remind
		Response.write "});" & vbcrlf & "});" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "<script src= ""../Script/wk_teltop_sp.js?ver="
	Response.write Application("sys.info.jsver")
