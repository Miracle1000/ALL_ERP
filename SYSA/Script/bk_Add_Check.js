function ask1(tp)
{
	...
	document.getElementById('demo').action="Save_Check.asp?tp="+tp;
...
function allsum()
{
	...
	for(i=0 ;i<Chk_sjnum.length; i++)
	{
        ...
        Chk_yknum[i].value=parseFloat(pdnum)-parseFloat(Chk_sjnum[i].value);
		temp=temp+parseFloat(Chk_sjnum[i].value);
		temp1=temp1+parseFloat(pdnum);
		temp2=temp2+(parseFloat(pdnum)-parseFloat(Chk_sjnum[i].value));
	}
