...
'If Right(s,1) = "," Then
	...
	Dim leftlist
	Sub page_load
		Dim productmenu, clo, C
		productmenu=request("productmenu")
		session("productmenu")=productmenu
		clo=request("clo")
		If clo <> "" Then
			user_list = "0"
			allcanname="全部"
			...
			if productmenu<>"" then
				...
				sqlm="select case when user_list1 is null or user_list1='' then '0' else user_list1 end from menu where id="&productmenu&" "
			...
			Response.write "" & vbcrlf & "        <script language='javascript'>" & vbcrlf & "            try { window.opener.location.reload(); } catch (e) { };" & vbcrlf & "            try {" & vbcrlf & "                var allcansee = """
			Response.write allcansee
			Response.write """;" & vbcrlf & "                var ords = """
			Response.write user_list
			Response.write """;" & vbcrlf & "                var showText = """
			Response.write allcanname
			Response.write """;" & vbcrlf & "                var dates = { allcansee: allcansee, ords: ords, showText: showText }" & vbcrlf & "                window.opener.parent.User_ListChooses(dates)" & vbcrlf & "            } catch (e) { };" & vbcrlf & "            window.open('', '_self');" & vbcrlf & "            window.close();" & vbcrlf & "        </script>" & vbcrlf & "        "
			Exit sub
		end if
		Response.write app.defheadhtml("../","")
		Dim rs5, sql5
		set rs5=server.CreateObject("adodb.recordset")
		sql5="select num1 from setjm3 where ord=7"
		rs5.open sql5,conn,1,1
		if rs5.eof then
			leftlist = "1"
		else
			leftlist =zbcdbl( rs5("num1"))
		end if
		rs5.close
		set rs5=Nothing
		Response.write "" & vbcrlf & "<script language=""javascript"" src=""../sortcp/function.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """></script>" & vbcrlf & "<script src= ""../Script/pt_result3.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """ ></script>" & vbcrlf & "<style>" & vbcrlf & "        html{ overflow:hidden; scrollbar-3dlight-color:#d0d0e8;scrollbar-highlight-color:#fff;scrollbar-face-color:#f0f0ff;scrollbar-arrow-color:#c0c0e8;scrollbar-shadow-color:#d0d0e8;scrollbar-darkshadow-color:#fff;scrollbar-base-color:#ffffff;scrollbar-track-color:#fff;}" & vbcrlf & " a {color:#2f496e}" & vbcrlf & "       td.menutab {background:transparent url(../images/lmeuntab_bg.gif) no-repeat center center ;padding: 3px 0 0 20px;height:23px;font-weight:bold;}" & vbcrlf & " td.menutab_hover {background-image: url(../images/lmeuntab_hbg.gif);background-repeat: no-repeat;padding: 3px 0 0 20px;height:23px;font-weight:bold;}" & vbcrlf & "   td.menutab_hover a{color:#000000;}" & vbcrlf & "      .tvw_txt {color:#2f496e;}" & vbcrlf & "       #txtKeywords" & vbcrlf & "    {width:100px;border:1px solid #999;height:16px;_height:18px;line-height:16px;font-size:12px;color:#999999;text-align:center;}" & vbcrlf & "</style>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false  onMouseOver=""window.status='none';return true;"" onload=""resize()"" >" & vbcrlf & "      <div class=""resetTableBg"" style='background:white url(../images/topb1g.gif) repeat-x;padding:3px0px 0px 0px;height:24px;overflow:hidden'>" & vbcrlf & "           <table style='table-layout:fixed;width:100%;'>" & vbcrlf & "                  <tr>" & vbcrlf & "                            <td class=""leftPageBgPd"" style='text-align:right;white-space:nowrap;'>分类检索&nbsp;<input id='txtKeywords' type='text' style='width:80px;' onblur=""keyTextBlur(this)"" onfocus=""keyTextFocus(this)""  onkeyup=""keyTextKeyUp(this)"" maxlength=100 onkeydown='if(window.event.keyCode==13){doSearch();event.cancelBubble=true;return false;}'>&nbsp;<button class='zb-button' onclick='doSearch()'>检索</button>" & vbcrlf & "                               </td>" & vbcrlf & "                   </tr>" & vbcrlf & "</table>" & vbcrlf &         "</div>" & vbcrlf &   "<div style='height:4px;overflow:hidden;margin-top:-1px;background-color:white'>&nbsp;</div>" & vbcrlf &     "<div class=""resetBgColor"" style='background:white url(""../images/imunetab_bg_2.gif"") repeat;height:25px;'>" & vbcrlf &    "<div class=""resetBgColor"" style='background:white url(""../images/lmeuntab_bgl.gif"") no-repeat;height:25px;width:300px;line-height:25px;font-weight:bold;'>" & vbcrlf & "            <a class=""tableTitleLinks"" style='margin-left:13px;'>产品分类</a>" & vbcrlf & "         <a class=""tableTitleLinks"" style='margin-left:5px;' href='javascript:void(0)' onclick='showAllStore()'>显示分类</a>" & vbcrlf & "          <a class=""tableTitleLinks"" style='margin-left:5px;' href='javascript:void(0)' onclick='expTree(this)'>"
		Response.write Application("sys.info.jsver")
		if leftlist = "1" then
			Response.write "全部收缩"
		elseif leftlist="0" then
			Response.write "全部展开"
		end if
		Response.write "</a>" & vbcrlf & "  </div>" & vbcrlf & "    </div>" & vbcrlf & "        <div id=""allMenu"" style=""_width:198px;overflow:auto;margin-right:0px;margin-left:3px;_margin-left:0px;position:relative"">" & vbcrlf & "   "
		Response.write "全部展开"
		Call showMenuCls(0, Nothing)
		Response.write "" & vbcrlf & "      </div><script> window.onresize = resize; </script>" & vbcrlf & "</body></html>" & vbcrlf & ""
	end sub
	Sub Menutree_ctree
		Call showMenuCls(0, nothing)
	end sub
	Sub showMenuCls(ByVal pid, ByRef pnd)
		Dim maxWidth, rs, keytxt, tvw, explan
		Set rs = cn.execute("exec erp_selbox_getMenuMaxWidth")
		If rs.eof =False Then
			maxWidth = rs(0)
		end if
		rs.close
		...
		Set tvw = New treeview
		tvw.id = "Menutree"
		tvw.pagesize = 999999999
		tvw.pagedataemodel = "all"
		If maxWidth>150 Then tvw.stylecss="width:"&maxWidth&"px"
		tvw.defExplan = explan
		Call tvw.addAllNodes(tvw.nodes, "exec erp_selbox_createMenuNode " & Info.User & ",'" & Replace(keytxt,"'","''") & "',@parentid,@pagesize,@pageindex,''", explan, 1, pid)
