...
'Public Function AddFormValue(name,  value)
	...
	Dim oldProductUnit : oldProductUnit = ""
	Dim intro_1_14 , intro_1_1 , intro_26_1 , intro_26_14
	sub onReportInit(byval rpt, byval  state)
		Dim ord : ord = app.base64.deurl(app.gettext("ord"))
		If Len(ord)=0 Then ord = 0
		If ord=0 Then ord=app.getint("pdid")
		Dim unit : unit =app.getint("unit")
		Dim showtype : showtype = app.getint("showtype")
		If showtype = 0 Then  showtype = 1
		rpt.title = "产品库存列表"
		Dim beforeBtnsHTML
		if app.getInt("searchType")=1 then
			beforeBtnsHTML =  "<script>window.onReportPageLoad = function(){window.__isadsearchmodel=4; ReportSubmit();}</script>"
		end if
		beforeBtnsHTML = beforeBtnsHTML & "<style>#lvw_ac_ptb_mlistvw{background:none!important}#fsitem_ckcls{width:120px;};#fsitem_ckitem{width:140px;}.lvw_tablebg{overflow:visible;}#PageSizeBox{top:6px;height:21px;}#btnsdiv {padding-top:4px!important;padding-bottom:5px!important;}#btnsdiv .oldbutton{margin-bottom:-1px;}</style><input onclick='window.location.href=""?showtype=1""' type='radio' " & app.iif(showtype=1,"checked","") & " name='ms1'>汇总模式&nbsp;<input type='radio' name='ms1' " & app.iif(showtype=2,"checked","")  & " onclick='window.location.href=""?showtype=2""' >明细模式&nbsp;"
		if showtype=2 and app.power.existsModel(17001) and sdk.Info.isSupperAdmin then
			Set rs= cn.execute("select count(1) from ku with(nolock) where ku.xlh is not null and ku.xlh!='' and ku.num2>0 and ku.num2!=1")
			if not rs(0)&"" ="0" then
				clickHtml="addrModify("&rs(0)&")"
				Set rs1= cn.execute("select 1 from home_usConfig  with(nolock) where name='REMOVE_SERNUM_DIALOG_FLAG' and uid="&Info.User)
				if rs1.eof=false then clickHtml="window.open('../../sysn/view/statistics/store/SerialNumberRemove.ashx')"
				beforeBtnsHTML = beforeBtnsHTML & "&nbsp;<button class='oldbutton' id='removeBtn' onclick="""&clickHtml&""">序列号迁移</button>"
			end if
		end if
		If app.power.existsPower(23,9) Then
			beforeBtnsHTML = beforeBtnsHTML & "&nbsp;<button class='oldbutton' type='button' onclick=""window.location.href='../../SYSN/view/store/kuin/KuinImport.ashx?importType=2'"">导入</button>"
		end if
		Dim i ,zdyfields, field , zdySearchType : zdySearchType = ""
		Set zdyfields = GetZdyFields(21)
		For i = 0 To zdyfields.count-1
			Set zdyfields = GetZdyFields(21)
			Set field = zdyfields.item(i)
			If field.search = True And field.extra=0 Then
				zdySearchType = zdySearchType & Chr(2) & field.name & Chr(1) & field.dbname
			end if
		next
		Dim sflg ,defselect ,defvalue
		defselect = "name"
		defvalue = ""
		sflg = app.getInt("sflg")
		If sflg=1 Then
			If app.getText("product_title")<>"" Then
				defselect = "name"
				defvalue = app.getText("product_title")
			ElseIf app.getText("product_order")<>"" Then
				defselect = "bh"
				defvalue = app.getText("product_order")
			ElseIf app.getText("product_type")<>"" Then
				defselect = "xh"
				defvalue = app.getText("product_type")
			ElseIf app.getText("product_ck")<>"" Then
				remindIds = "productck="& Replace(app.getText("product_ck")," ","")
			end if
		end if
		app.addDefaultScript
		rpt.beforeBtnsHTML = beforeBtnsHTML
		rpt.addfield2 "","select", "ckcls" , getckclslist(state),"updateckitem" , "", "选择仓库分类=0"
		rpt.addfield2 "","select", "ckitem" , getcklist(state, 0),"" , "", "选择仓库=0"
		rpt.addfield2 "","select", "searchType" , "产品编号" & Chr(1) & "bh" & Chr(2) & "产品型号" & Chr(1) & "xh" &Chr(2) &"条形码" &Chr(1)&"txm"& app.iif(showtype=2,Chr(2) &"批号" &Chr(1)&"ph","") & zdySearchType,"","","产品名称=#" & defselect
		rpt.addfield "","text", "searchKey" , "", "", defvalue
		rpt.aSearch.AddField "产品分类", "cpfl", "cpfl", ""
		rpt.aSearch.AddField "产品名称", "text", "name", ""
		rpt.aSearch.AddField "产品编号", "text", "bh", ""
		rpt.aSearch.AddField "产品型号", "text", "xh", ""
		if showtype = 1 and isOpenProductAttr then
			rpt.aSearch.AddField "产品属性1", "text", "proAttr1", ""
			rpt.aSearch.AddField "产品属性2", "text", "proAttr2", ""
		end if
		rpt.aSearch.AddField "产品说明", "text", "intro1", ""
		rpt.aSearch.AddField "产品参数", "text", "intro2", ""
		rpt.aSearch.AddField "图片与附件", "text", "intro3", ""
		If showtype = 2 Then
			rpt.aSearch.AddField "备注", "text", "intro_cp", ""
		end if
		rpt.aSearch.addZdyFields 21
		If showtype = 2 Then
			rpt.aSearch.AddField "生产日期", "dates", "scdate", ""
			rpt.aSearch.AddField "有效日期", "dates", "yxdate", ""
			rpt.aSearch.AddField "添加日期", "dates", "adddate", ""
			rpt.aSearch.AddField "批号", "text", "ph", ""
			rpt.aSearch.AddField "序列号", "text", "xlh", ""
			if isOpenProductAttr then
				rpt.aSearch.AddField "产品属性1", "text", "proAttr1", ""
				rpt.aSearch.AddField "产品属性2", "text", "proAttr2", ""
			end if
			rpt.aSearch.AddField "件数", "numbers", "js", ""
			rpt.aSearch.AddField "包装", "sortonehy", "bz", "82"
		end if
		rpt.aSearch.AddField "产品单位", "sortonehy", "allunit", "61"
		rpt.aSearch.AddField "添加人员", "gates", "addcate", ""
		rpt.aSearch.AddField "仓库", "ckcls", "cks", ""
		rpt.aSearch.AddField "库存数量", "numbers", "kcnum", ""
		If showtype = 1 And 1=2 Then
			rpt.aSearch.AddField "预定库存", "numbers", "ydnum", ""
			rpt.aSearch.AddField "在途库存", "numbers", "ztnum", ""
		end if
		rpt.aSearch.AddField "冻结库存", "numbers", "djnum", ""
		If showtype = 1 And 1=2 Then rpt.aSearch.AddField "可用库存", "numbers", "kynum", ""
		If showtype = 2 Then
			If ZBRuntime.MC(1001) or ZBRuntime.MC(1002) Then
				rpt.aSearch.AddField "关联供应商", "text", "gysname", ""
			end if
			dim batItem
			batItem = ""
			If ZBRuntime.MC(17009) And app.power.existsPower(79,13) Then batItem = "养护"
			if app.power.existsPower(23,21) then batItem = batItem & app.iif(batItem&""="","",",") & "批量冻结,批量解冻"
			rpt.setBatHandle "id", batItem
		end if
		rpt.cancolset = True
		rpt.Settings.add  "增强设置","showExtraConfigPage"
		rpt.BillListUIModel = True
		rpt.showtoppagesize = True
		rpt.onlyadSearchModel =True
		rpt.canexcel = app.power.existsPower(23,10)
		rpt.canprint = app.power.existsPower(23,7)
		rpt.SortUiType = 1
		rpt.addSortByCol
		rpt.addSort "产品添加时间","addtime"
		Dim remind : remind = app.Gettext("remind")
		Dim remindIds : remindIds = ""
		If remind <> "" Then
			Dim helper :Set helper = CreateReminderHelper(cn,remind,0)
			remindIds =  helper.listSql("ids_withoutOrderBy")
		end if
		Dim sql : sql = ""
		Dim startDate,endDate,productOrd,jieZhiShiJian,danWei,gongYingShang,cangKu
		startDate=trim(request("searchdate1"))
		endDate=trim(request("searchdate2"))
		productOrd=app.base64.deurl(request("productOrd"))
		jieZhiShiJian=trim(request("jieZhiShiJian"))
		cangKu=trim(request("ck"))
		gongYingShang=trim(app.getText("gys"))
		danWei=trim(request("dw"))
		sql = "set nocount on;"
		sql = sql & "select CAST('"&startDate&"' as nvarchar(20)) startDate,CAST('"&endDate&"' as nvarchar(20)) endDate,CAST('"&productOrd&"' as nvarchar(20)) productOrd,CAST('"&jieZhiShiJian&"' as nvarchar(20)) jieZhiShiJian,CAST('"&cangKu&"' as nvarchar(20)) cangKu,CAST('"&gongYingShang&"' as nvarchar(200)) gongYingShang,CAST('"&danWei&"' as nvarchar(20)) danWei into #KuAge;"
		sql = sql & "exec [erp_store_kulist_ck] "& Info.User &",'"& Replace(remindIds,"'","''") &"',"& showtype &","& ord &","& unit &" ,@ckcls,@ckitem ,@searchType ,@searchKey,@cpfl,@name ,@bh ,@xh ,@intro1 ,@intro2 ,@intro3 ,@zdy21_1,@zdy21_2,@zdy21_3,@zdy21_4,@zdy21_5,@zdy21_6 ,@scdate_0,@scdate_1,@yxdate_0,@yxdate_1,@adddate_0,@adddate_1,@ph,@xlh,@js_0,@js_1,@bz,@allunit ,@addcate ,@cks ,@kcnum_0 ,@kcnum_1 ,@ydnum_0, @ydnum_1 ,@ztnum_0 ,@ztnum_1 ,@djnum_0,@djnum_1,@kynum_0,@kynum_1,@kcmoney_0,@kcmoney_1,@gysname,@intro_cp,@proAttr1,@proAttr2,&SearchMode,&excelmode,&pagesize,&pageindex,&ReportSort,&ScanText,&headerconfig;"
		sql = sql & "drop table #KuAge; "
		sql = sql & "set nocount off;"
		rpt.sql = sql
	end sub
	Sub app_showExtraConfigPage
		Dim html
		Dim sort1 ,sort2,sort3
		sort1 = app.Attributes("storelist_sort1")
		If Len(sort1&"")=0 Then sort1 = 0
		sort2 = app.Attributes("storelist_sort2")
		If Len(sort2&"")=0 Then sort2 = 0
		sort3 = app.Attributes("storelist_sort3")
		If Len(sort3&"")=0 Then sort3 = 1
		html = "<div style='display:block;height:368px;overflow:auto;overflow-x:hidden;border-top:0px;border:1px solid #c0ccdd;margin-top:5px;'>"  &_
		"<table style='position:static;text-align:center;background:'> " &_
		"<col style='width:190px;background:'><col style='width:70%;background:'>  "&_
		"<tr>"  &_
		"<td class='lvw_cell' style='border-left:0px;border-top:0px;text-align:right'>零库存产品是否显示：</td>" &_
		"<tr>"  &_
		"             ""<td class='lvw_cell' style='border-left:0px;border-right:0px;border-top:0px;text-align:left'><input type='radio' value='1' id='sort1' name='sort1' ""& app.iif(sort1=1 , ""checked"","""") &"">显示&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' value='0' name='sort1' "& app.iif(sort1=0 , "checked","") &">不显示</td>"  &_
		"</tr>" &_
		"<tr>"  &_
		"<td class='lvw_cell' style='border-left:0px;text-align:right'>产品信息是否完整显示：</td>" &_
		"<tr>"  &_
		"             ""<td class='lvw_cell' style='border-left:0px;border-right:0px;text-align:left'><input type='radio' value='1' id='sort2' name='sort2' ""& app.iif(sort2=1 , ""checked"","""") &"">是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' value='0' name='sort2' "& app.iif(sort2=0 , "checked","") &">否</td>"  &_
		"</tr>" &_
		"<tr>"  &_
		"<td class='lvw_cell' style='border-left:0px;text-align:right'>仓库信息是否全路径显示：</td>" &_
		"<tr>"  &_
		"<td class='lvw_cell' style='border-left:0px;border-right:0px;text-align:left'><input type='radio' value='1' id='sort3' name='sort3' "& app.iif(sort3=1 , "checked","") &">显示全路径&nbsp;<input type='radio' value='0' name='sort3' "& app.iif(sort3=0 , "checked","") &">只显示仓库名称</td>"  &_
		"<tr>"  &_
		"</tr>" &_
		"</table>"  &_
		"</div>"
	...
	Sub App_PreReportExtraConfig(lvw)
		Dim sort1 , sort2 , sort3
		sort1 = app.getint("sort1")
		sort2 = app.getint("sort2")
		sort3 = app.getint("sort3")
		app.Attributes("storelist_sort1") = sort1
		app.Attributes("storelist_sort2") = sort2
		app.Attributes("storelist_sort3") = sort3
	end sub
	Sub App_AftReportExtraConfig(lvw)
		Dim sort2 : sort2 = app.getint("sort2")
		lvw.distinctSpaceCol = "产品名称,产品编号,产品型号,单位"& app.iif(isOpenProductAttr,"",app.iif(Len(lvw.HeaderConfigKey)>0,"",",预定数量,在途数量"))& app.iif(sort2=0 ,"|产品名称,产品编号,产品型号" ,";产品名称,产品编号,产品型号,单位")
	end sub
	sub app_onCreateList(lvw, rpt)
		cpimg="none"
		If GetCpimg()=1 Then cpimg=""
		Dim h , showtype : showtype = rpt.getint("showtype")
		Dim remind : remind = app.getInt("remind")
		If showtype = 0 Then  showtype = 1
		Dim rs ,unck_dj
		unck_dj=0
		Set rs= cn.execute("select intro from setopen  with(nolock) where sort1=109")
		If rs.eof= False Then unck_dj=rs("intro")
		rs.close
		set rs=Nothing
		If unck_dj&"" = "" Or isnumeric(unck_dj&"")=False Then unck_dj=0
		Dim sort2 : sort2 = app.Attributes("storelist_sort2")
		If Len(sort2&"")=0 Then sort2 = 0
		lvw.distinctSpaceCol = "产品名称,产品编号,产品型号,单位"& app.iif(isOpenProductAttr,"",app.iif(showtype=2,"",",预定数量,在途数量")) & app.iif(sort2=0 ,"|产品名称,产品编号,产品型号" ,";产品名称,产品编号,产品型号,单位")
		lvw.allsum = True
		lvw.HeaderPageSizeUI = False
		lvw.cansort = False
		If showtype = 2 Then
			lvw.checkbox = True
			lvw.HeaderConfigKey = "storelistmx"
		end if
		lvw.headers("id").display = "none"
		Set h = lvw.headers("产品ID")
		h.display = "none"
		Set h = lvw.headers("产品图片")
		h.display=cpimg
		h.dbtype="str"
		h.width = 150
		h.formula = "GetProductPic(rs(""产品ID"").value)"
		h.cansum = false
		Set h = lvw.headers("产品名称")
		h.dbtype="str"
		h.formula = "titleLinkHTML(rs(""id"").value,rs(""产品ID"").value, @value , rs(""del"").value , 1)"
		h.width = 150
		h.cansum = false
		Set h = lvw.headers("产品编号")
		h.width = 140
		h.dbtype="str"
		h.formula = "titleLinkHTML(rs(""id"").value,rs(""产品ID"").value, @value , rs(""del"").value , 2)"
		h.cansum = False
		Set h = lvw.headers("产品型号")
		h.width = 120
		h.dbtype="str"
		h.formula = "titleLinkHTML(rs(""id"").value,rs(""产品ID"").value, @value , rs(""del"").value , 2)"
		h.cansum = false
		Set h = lvw.headers("单位")
		h.width = 60
		h.cansum = false
		Set h = lvw.headers("产品属性1")
		h.cansum = false
		if isOpenProductAttr=false then
			h.display = "none"
		end if
		Set h = lvw.headers("产品属性2")
		h.cansum = false
		if isOpenProductAttr=false then
			h.display = "none"
		end if
		Set h = lvw.headers("库存数量")
		h.width = 120
		h.formattext = "code:setColorByNumber(@value , ""kc"")"
		h.ignoreNonnumeric = True
		h.dbtype="number"
		Set h = lvw.headers("辅助数量")
		h.width = 120
		h.align = "right"
		h.formattext="code:setColorByNumber(@value , ""fz"")"
		h.unit = "@辅助单位"
		h.ignoreNonnumeric = True
		h.dbtype="number"
		h.cansum = true
		if cn.execute("select 1 from home_usConfig  with(nolock) where name='AssistUnitTactics' and nvalue=1 ").eof=true then h.display="none"
		Set h = lvw.headers("辅助单位")
		h.display="none"
		If showtype = 1 Then
			Set h = lvw.headers("车间剩余数量")
			h.dbtype = "number"
			h.width = 120
			h.formattext = "code:setColorByNumber(@value , ""kc"")"
			h.dbtype="number"
			If not app.power.existsModel(18620) Then h.display="none"
			Set h = lvw.headers("预定数量")
			h.dbtype = "number"
			h.width = 120
			h.formattext = "code:showMXListPageLink(@value, rs(""产品ID"").value ,3 , rs(""unit"") ,0,rs(""详情""))"
			h.ignoreNonnumeric = True
			h.dbtype="number"
			h.cansort = False
			h.cansum = false
			Set h = lvw.headers("在途数量")
			h.dbtype = "number"
			h.width = 120
			h.formattext = "code:showMXListPageLink(@value, rs(""产品ID"").value ,4, rs(""unit"") ,0,rs(""详情""))"
			h.ignoreNonnumeric = True
			h.cansort = False
			h.cansum = false
		end if
		Set h = lvw.headers("冻结数量")
		h.width = 120
		h.formattext = "code:setColorByNumber(@value , ""dj"")"
		If showtype = 1 Then
			Set h = lvw.headers("可用数量")
			h.dbtype = "number"
			h.ignoreNonnumeric = True
			h.width = 120
			h.formula = "GetCanSaleNum(rs(""产品ID""),rs(""unit""),rs(""辅助单位""),rs(""产品属性1""),rs(""产品属性2""),rs(""库存数量"") , rs(""预定数量"") , rs(""在途数量"") , rs(""冻结数量"") ,"& unck_dj &",0)"
			h.cansort = False
			h.cansum = false
			Set h = lvw.headers("车间剩余成本")
			h.dbtype = "money"
			h.ignoreNonnumeric = True
			h.width = 120
			h.cansum = True
			If not app.power.existsModel(18620) OR 1=1 Then h.display="none"
			If not app.power.existsPower(23,2) OR 1=1 Then h.display="none"
		end if
		Set h = lvw.headers("平均成本")
		h.dbtype = "storeprice"
		h.width = 120
		h.cansum = False
		If app.power.existsPower(23,2)=False OR 1=1 Then h.display = "none"
		h.formattext = "code:setColorByNumber(@value , ""pj"")"
		Set h = lvw.headers("库存成本")
		h.dbtype = "money"
		h.width = 120
		If app.power.existsPower(23,2)=False OR 1=1 Then h.display = "none"
		h.formattext = "code:setColorByNumber(@value , ""cb"")"
		Set h = lvw.headers("批号")
		h.cansum = False
		Set h = lvw.headers("序列号")
		h.cansum = False
		Set h = lvw.headers("包装")
		h.cansum = False
		Dim sort :sort=21
		Dim pername : pername ="p_"
		Dim i ,zdyfields ,field
		Set zdyfields = GetZdyFields(sort)
		If zdyfields.count=0 Then
			For i = 1 To 6
				Set h = lvw.headers.getItemByDbname(pername &"zdy" & i)
			...
			For i = 0 To zdyfields.count-1
				...
				If Not h Is Nothing then
					If field.show = True Then
						h.title = field.name
						h.width = 100
						If showtype = 1 Then h.formula = "titleLinkHTML(rs(""id"").value,rs(""产品ID"").value, @value , rs(""del"").value , 2)"
						h.cansum=False
						If field.extra > 0 Then h.width = 80
					else
						h.width = 0
		...
		Set h = lvw.headers("仓库")
		h.width = 180
		h.dbtype = "str"
		h.cansum = False
		Set h = lvw.headers("备注")
		h.width = 180
		h.dbtype = "str"
		h.cansum = False
		Set h = lvw.headers("库龄")
		h.cansum = False
		If app.power.existsPower(23,11)=False Then h.display = "none"
		If showtype = 1 Then
			Set h = lvw.headers("详情")
			h.formattext = "code:showMXListPageLink(@value, rs(""产品ID"").value ,1, rs(""unit"") ,0,@value)"
			h.cansort = 0
			lvw.headers("unit").display="none"
		else
			Set h = lvw.headers("关联供应商")
			h.width = 180
			h.dbtype = "str"
			h.formattext = "code:showCompanyPageLink(@value,rs(""gys""), rs(""cateid"") ,rs(""share"") ,rs(""sort3""))"
			If ZBRuntime.MC(1001) or ZBRuntime.MC(1002) Then
			else
				h.display="none"
			end if
			lvw.headers("RelationID").display = "none"
			Dim rs3
			Set rs3=cn.execute("select top 1 id as maxid from [erp_sys_LvwConfig] where lvwid='"& lvw.Md5Key16 &"' and uid="&session("personzbintel2007")&" order by id desc")
			If rs3.eof= True Then
				Dim rs4,maxid
				cn.execute("insert into erp_sys_LvwConfig (uid,lvwid,width,pagesize) values ("&session("personzbintel2007")&",'"& lvw.Md5Key16 &"',null,null)")
				rs4=cn.execute("select max(id) as maxid from erp_sys_LvwConfig  with(nolock) where lvwid='"& lvw.Md5Key16 &"' and uid="&session("personzbintel2007"))
				maxid=rs4("maxid")
				cn.execute("insert into erp_sys_LvwcolConfig (cfgid,dbname,dbindex,newdbindex,colindex,title,sort,width,visible) values ("&maxid&",'入库确认时间',null,null,null,'',null,null,0)")
			else
				maxid=rs3("maxid")
				cn.execute("if not exists(select 1 from erp_sys_LvwcolConfig where cfgid="& maxid &") insert into erp_sys_LvwcolConfig (cfgid,dbname,dbindex,newdbindex,colindex,title,sort,width,visible) values ("&maxid&",'入库确认时间',null,null,null,'',null,null,0)")
				Set h = lvw.headers("入库确认时间")
			end if
			Set h = lvw.headers("操作")
			h.formattext = "code:showMXListPageLink(@value, rs(""库存数量"") ,2 ,0 , "& remind &",@value)"
			h.cansort = 0
			If app.power.existsPower(23,21)=False Then h.display = "none"
			lvw.headers("gys").display="none"
			lvw.headers("cateid").display="none"
			lvw.headers("share").display="none"
			lvw.headers("sort3").display="none"
		end if
		lvw.headers("kuinlist").display="none"
		lvw.headers("del").display="none"
		lvw.excelcallbackproc = "App_OnListviewExcel"
	end sub
	Sub App_OnListviewExcel(lvw)
		Dim h , showtype : showtype = app.getint("showtype")
		If showtype = 0 Then  showtype = 1
		Dim rs ,unck_dj
		unck_dj=0
		Set rs= cn.execute("select intro from setopen  with(nolock) where sort1=109")
		...
		h.ignoreNonnumeric = True
		h.width = 120
		Set assunit = lvw.headers("辅助单位")
		Set h = lvw.headers("辅助数量")
		h.dbtype = "number"
		h.width = 120
		h.ignoreNonnumeric = True
		if cn.execute("select 1 from home_usConfig  with(nolock) where name='AssistUnitTactics' and nvalue=1 ").eof=true then
			h.display="none"
			assunit.display="none"
		else
			dim rsAss
			Set rsAss=cn.execute("select visible,newdbindex from [erp_sys_LvwConfig] b  with(nolock) inner join [erp_sys_LvwColConfig] a  with(nolock) on a.cfgid=b.id and b.lvwid='"& lvw.Md5Key16 &"' and b.uid="&session("personzbintel2007")&" and a.dbname='辅助数量'")
			if rsAss.eof=false then
				assVisible=rsAss("visible")&""
				if assVisible="" then assVisible="False"
				assSort=rsAss("newdbindex")&""
				if assSort="" then assSort=0
				assSort=cdbl(assSort)+1
'if assSort="" then assSort=0
				dim rsAssUnit
				Set rsAssUnit=cn.execute("select visible from [erp_sys_LvwConfig] b  with(nolock) inner join [erp_sys_LvwColConfig] a  with(nolock) on a.cfgid=b.id and b.lvwid='"& lvw.Md5Key16 &"' and b.uid="&session("personzbintel2007")&" and a.dbname='辅助单位'")
				if rsAssUnit.eof=True then
					maxid=0
					Set rs3=cn.execute("select visible from [erp_sys_LvwConfig] b  with(nolock) inner join [erp_sys_LvwColConfig] a  with(nolock) on a.cfgid=b.id and b.lvwid='"& lvw.Md5Key16 &"' and b.uid="&session("personzbintel2007"))
					If rs3.eof= True Then
						cn.execute("insert into erp_sys_LvwConfig (uid,lvwid,width,pagesize) values ("&session("personzbintel2007")&",'"& lvw.Md5Key16 &"',null,null)")
						rs4=cn.execute("select max(id) as maxid from erp_sys_LvwConfig  with(nolock) where lvwid='"& lvw.Md5Key16 &"' and uid="&session("personzbintel2007"))
						maxid=rs4("maxid")
					...
					if assVisible="False" then
						visibleBool=0
					else
						visibleBool=1
					end if
					cn.execute("insert into erp_sys_LvwcolConfig (cfgid,dbname,dbindex,newdbindex,colindex,title,sort,width,visible) values ("&maxid&",'辅助单位',null,"&assSort&",null,'',1000,null,"&visibleBool&")")
				end if
				if assVisible="False" then
					cn.execute("update b set b.visible=0 from erp_sys_LvwcolConfig b inner join [erp_sys_LvwConfig] a on b.cfgid=a.id and a.lvwid='"& lvw.Md5Key16 &"' and a.uid="&session("personzbintel2007")&" and b.dbname='辅助单位'")
				else
					cn.execute("update b set b.visible=1,b.newdbindex="&assSort&" from erp_sys_LvwcolConfig b inner join [erp_sys_LvwConfig] a on b.cfgid=a.id and a.lvwid='"& lvw.Md5Key16 &"' and a.uid="&session("personzbintel2007")&" and b.dbname='辅助单位'")
				end if
			end if
		end if
		If showtype = 1 Then
		...
		If showtype = 1 Then
			...
			h.width = 120
			h.ignoreNonnumeric = True
			h.formula = "GetCanSaleNum(rs(""产品ID""),rs(""unit""),rs(""辅助单位""),rs(""产品属性1""),rs(""产品属性2""),rs(""库存数量"") , rs(""预定数量"") , rs(""在途数量"") , rs(""冻结数量"") ,"& unck_dj &",1)"
			Set h = lvw.headers("车间剩余成本")
			h.dbtype = "money"
			h.ignoreNonnumeric = True
			h.width = 120
		...
		If showtype = 1 Then
			...
			h.dbtype = "str"
			h.cansum = False
			h.formula = "showCompanyPageLink(@value,rs(""gys""), rs(""cateid"") ,rs(""share"") ,rs(""sort3""))"
			If ZBRuntime.MC(1001) or ZBRuntime.MC(1002) Then
			else
				h.display="none"
			end if
	...
	Sub App_lvw_OnSumProcInit(lvw, header, i, ByRef dosum)
	...
	Function App_lvw_OnSumProc(lvw, dbname, isAllSum, html)
		Select Case dbname
		Case "库存数量", "冻结数量", "可用数量" ,"辅助数量" :
		If Len(html)>0 And isnumeric(html)  Then
			html =formatnumber(html, Info.floatnumber,-1,0)
'If Len(html)>0 And isnumeric(html)  Then
		end if
		End Select
	end function
	Function titleLinkHTML(id, ProductId, v , del ,stype)
		Dim s :s = ""
		Select Case stype
		Case 1 :
		If del = "2" Then
			s = v &"<span style='color:red'>(已删除)</span>"
		else
			If Len(productpower)=0 Then productpower = app.power.existsPower(21,14)
			If app.power.existsPower(21,1) Then
				If productpower = True Then
					s = "<a target=_blank href='" & app.virpath & "product/content.asp?ord=" & app.base64.pwurl(ProductId) & "'>" &  v & "</a>"
				else
					s = v
				end if
			end if
		end if
		s = s &" <span class='red' id='tip_"& id &"'></span>"
		Case Else
		If del<>2 Then s = v
		End Select
		titleLinkHTML = s
	end function
	Sub lvw_onCreateCheckBox(lvw, rs, html)
		If CDbl(rs("库存数量").value)<=0 Then html = ""
		dim rid: rid = Split(rs("操作").value ,"|")(2)
		html = Replace(html&"","name='sys_lvw_ckbox'","name='sys_lvw_ckbox' rid=" & rid,1,-1,1)
'dim rid: rid = Split(rs("操作").value ,"|")(2)
		if html&""<>"" and Split(rs("操作").value ,"|")(0)="1" then
			html = Replace(html&"","name='sys_lvw_ckbox'","name='sys_lvw_ckbox' canyh=0",1,-1,1)
'if html&""<>"" and Split(rs("操作").value ,"|")(0)="1" then
		end if
	end sub
	Function GetCanSaleNum(ord,unit,AssistUnit,ProductAttr1,ProductAttr2 ,num2 , ydnum , ztnum , locknum1 , unck_dj, isExcel)
		Dim v : v = 0
		if cn.execute("select 1 from home_usConfig  with(nolock) where name='AssistUnitTactics' and nvalue=1 ").eof=false then
			If oldProductUnit = ord&unit&ProductAttr1&ProductAttr2&AssistUnit Then ydnum = 0 : ztnum = 0
			oldProductUnit = ord&unit&ProductAttr1&ProductAttr2&AssistUnit
		else
			If oldProductUnit = ord&unit&ProductAttr1&ProductAttr2 Then ydnum = 0 : ztnum = 0
			oldProductUnit = ord&unit&ProductAttr1&ProductAttr2
		end if
		v = CDbl(num2) - CDbl(ydnum) + CDbl(ztnum) - CDbl(locknum1) * CDbl(unck_dj)
		oldProductUnit = ord&unit&ProductAttr1&ProductAttr2
		If isExcel = 0 Then
			v= app.iif(CDbl(v)=0 ,"<span style='color:#d0d0d1;'>"& FormatNumber(v,Info.FloatNumber,-1,0,-1) & "<span>" , FormatNumber(v,Info.FloatNumber,-1,0,-1))
'If isExcel = 0 Then
		else
			v = FormatNumber(v,Info.FloatNumber,-1,0,-1)
'If isExcel = 0 Then
		end if
		GetCanSaleNum = v
	end function
	Function setColorByNumber(num , datatype)
		Dim s : s= ""
		If num&""="" Then num = 0
		Select Case datatype
		Case "dj"
		s = app.iif(CDbl(num)>0 ,"<span style='color:red'>"& FormatNumber(num,Info.FloatNumber,-1,0,-1) & "<span>" , "<span style='color:#d0d0d1;'>"&FormatNumber(num,Info.FloatNumber,-1,0,-1)& "<span>" )
'Case "dj"
		Case "kc"
		s = app.iif(CDbl(num)=0 ,"<span style='color:#d0d0d1;'>"& FormatNumber(num,Info.FloatNumber,-1,0,-1) & "<span>" , FormatNumber(num,Info.FloatNumber,-1,0,-1))
'Case "kc"
		Case "pj"
		s = app.iif(CDbl(num)=0 ,"<span style='color:#d0d0d1;'>"& FormatNumber(num,Info.StorePriceDotNum,-1,0,-1) & "<span>" , FormatNumber(num,Info.StorePriceDotNum,-1,0,-1))
'Case "pj"
		Case "cb"
		s = app.iif(CDbl(num)=0 ,"<span style='color:#d0d0d1;'>"& FormatNumber(num,Info.moneyNumber,-1,0,-1) & "<span>" , FormatNumber(num,Info.moneyNumber,-1,0,-1))
'Case "cb"
		Case "fz"
		s = app.iif(CDbl(num)>0 ,FormatNumber(num,Info.FloatNumber,-1,0,-1),"")
'Case "fz"
		End Select
		setColorByNumber = s
	end function
	Function SerialNumber(kuinlist , xlh)
		dim kuin : kuin = sdk.getSQLValue("select kuin from kuinlist  with(nolock) where id=" &kuinlist , 0 )
		SerialNumber = ShowSerialNumber(cn ,kuin, kuinlist , "../../" , true,xlh)
	end function
	Function showMXListPageLink(v1, cpid ,stype ,unit ,remind,xq)
		Dim html :html = ""
		Dim u : u = Split(v1 & "","|")
		If stype = 1 Then
			html = "<a href=""javascript:;""  onclick=""javascript:window.open('content_kc.asp?ord=" & app.base64.pwurl(cpid) & "&unit=" & u(0) & "&ck=" & u(1) & "&assistunit="& u(2) &"&ProductAttr1Search="& u(3) &"&ProductAttr2Search="& u(4) &"')"">查看详情</a>"
		ElseIf stype = 2 Then
			If CDbl(cpid)>0 Then
				Dim bvalue : bvalue = "冻结"
				If u(0)="1" Then bvalue = "解冻"
				html = "<button class=""oldbutton2"" onclick=""lockKuCP("& u(1) &","& u(0) &","& u(2) &")"">"& bvalue &"</button>&nbsp;"
			end if
		ElseIf stype = 3 Then
			Dim u3 : u3 = Split(xq & "","|")
			html = app.iif(CDbl(v1)=0 ,"<span style='color:#d0d0d1;'>"& v1 & "<span>" , "<a href='../contract/content_yd.asp?ord=" & app.base64.pwurl(cpid) & "&unit=" & unit & "&proAttr1=" & u3(3) & "&proAttr2=" & u3(4) & "' target='_blank'>"& v1 &"</a>")
		ElseIf stype = 4 Then
			Dim u4 : u4 = Split(xq & "","|")
			html = app.iif(CDbl(v1)=0 ,"<span style='color:#d0d0d1;'>"& v1 & "<span>" , "<a href='../caigou/content_zt.asp?ord=" & app.base64.pwurl(cpid) & "&unit=" & unit & "&proAttr1=" & u4(3) & "&proAttr2=" & u4(4) & "' target='_blank'>"& v1 &"</a>")
		end if
		Dim btnStr : btnStr= ""
		If remind>0 Then  btnStr = "<a href='javascript:void(0);' onclick='sendCancelRequest(0," & remind & ",0," & u(1) & ")' ><img src='../images/alt3.gif' alt='取消提醒' border='0'></a>&nbsp;"
		showMXListPageLink =btnStr & html
	end function
	Function showCompanyPageLink(title,ord, cateid ,share ,sort3)
		If isEmpty(intro_1_14) Then  intro_1_14 = app.power.GetPowerIntro(1,14)
		If isEmpty(intro_1_1) Then   intro_1_1 = app.power.GetPowerIntro(1,1)
		If isEmpty(intro_26_1) Then  intro_26_1 = app.power.GetPowerIntro(26,1)
		If isEmpty(intro_26_14) Then intro_26_14 = app.power.GetPowerIntro(26,14)
		dim TitleStr : TitleStr = ""
		If sort3="2" Then
			if intro_26_1="" or (instr(","& intro_26_1 &"," , ","& cateid &",")>0 And cateid<>0)  then
				if intro_26_14="" or (instr(","& intro_26_14 &"," , ","& cateid &",")>0 And cateid<>0)  then
					TitleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../work2/content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title &"</a>"
'if intro_26_14="" or (instr(","& intro_26_14 &"," , ","& cateid &",")>0 And cateid<>0)  then
				else
					TitleStr = title
				end if
			...
			if intro_1_1="" or (instr(","& intro_1_1 &"," , ","& cateid &",")>0 And cateid<>0) Or InStr(1,","&share&",", ","&Info.user&",",1) > 0 Or share = "1" then
				if intro_1_14="" or (instr(","& intro_1_14 &"," , ","& cateid &",")>0 And cateid<>0)  then
					TitleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../work/content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title &"</a>"
'if intro_1_14="" or (instr(","& intro_1_14 &"," , ","& cateid &",")>0 And cateid<>0)  then
				else
					TitleStr = title
				end if
		...
		showCompanyPageLink = TitleStr
	end function
	Function getckclslist(state)
		Dim rs, r, i
		If state = 1 Then
			i = 0
			ReDim r(0)
			Set rs = cn.execute("select distinct a.sort1,a.id, a.gate1 from sortck1 a  with(nolock) inner join sortck b  with(nolock) on a.id = b.sort and a.del=1 and b.del=1 and (cast(b.intro as varchar(10)) = '0' or charindex('," & Info.user & ",',','+cast(b.intro as varchar(7900))+',') > 0)  order by a.gate1 desc")
			While rs.eof = False
				ReDim Preserve r(i)
				r(i) = rs.fields(0).value & Chr(1) & rs.fields(1).value
				i = i + 1
'r(i) = rs.fields(0).value & Chr(1) & rs.fields(1).value
			...
			wend
			rs.close
			getckclslist = Join(r, Chr(2))
		end if
	end function
	Function getcklist(state, v)
		Dim rs, r, i
		If state = 1 Then
			i = 0
			ReDim r(0)
			Set rs = cn.execute("select b.sort1, b.ord from sortck b  with(nolock) where b.sort=" & v & " and b.del=1 and (cast(b.intro as varchar(10)) = '0' or charindex('," & Info.user & ",',','+cast(b.intro as varchar(7900))+',') > 0)  order by b.gate1 desc")
			ReDim r(0)
			While rs.eof = False
				ReDim Preserve r(i)
				r(i) = rs.fields(0).value & Chr(1) & rs.fields(1).value
...
'r(i) = rs.fields(0).value & Chr(1) & rs.fields(1).value
			...
			wend
			rs.close
			getcklist = Join(r, Chr(2))
		end if
	end function
	Sub App_ReportJoinField(rpt, name, value)
		Select Case name
		Case "updateckitem"
		rpt.ajaxUpdateField2 "", "select", "ckitem", getcklist(1, value),"","","选择仓库=0"
		End select
	end sub
	Sub app_lockKuCP
		Dim rs ,idx, status, rid, toStatus , locknum ,num2_sql ,unck_dj,lockIntro , relation_sql
		idx = app.getText("idx")
		status = app.getint("status")
		rid = app.getText("rid")
		if rid&""="" then rid = 0
		If idx&"" = "" Then idx = 0
		lockIntro = ""
		num2_sql = ""
		relation_sql = ""
		dim arr_idx ,arr_rid, i, ids1 , ids2 , rids
		arr_idx = split(idx,",")
		arr_rid = split(rid,",")
		ids1 = ""
		ids2 = ""
		...
		for i=0 to ubound(arr_idx)
			if  arr_rid(i)<>"0" and arr_rid(i)<>"" then
				if len(ids2)>0 then ids2 = ids2 &","
				ids2 = ids2 & arr_idx(i)
				if len(rids)>0 then rids = rids &","
				rids = rids & arr_rid(i)
			else
				if len(ids1)>0 then ids1 = ids1 &","
				ids1 = ids1 & arr_idx(i)
			end if
		next
		If status = 0 Then
			toStatus = 1 : lockIntro = "手动冻结"
			unck_dj = 0
			Set rs=cn.execute("select intro from setopen  with(nolock) where sort1=109")
			If rs.eof = False Then
				unck_dj = rs("intro")
			...
			If unck_dj&"" = "" Or isnumeric(unck_dj&"")=False Then unck_dj=0
			dim rdel : rdel = 1
			If unck_dj = 1 Then  rdel = 3
			if rids<>"" then
				relation_sql = " update S2_SerialNumberRelation set del="& rdel &", [HandleStatus]=1 where  id in (" & rids &") and del<>2 "
			end if
			if ids1<>"" then
				If unck_dj = 1 Then
					num2_sql = "0"
					locknum= "(case when isnull(locknum,0)>num2 then isnull(locknum,0) else num2 end)"
				Else
					num2_sql = "(case when isnull(locknum,0)>num2 then isnull(locknum,0) else num2 end) "
					locknum = "0"
				end if
			end if
		ElseIf status = 1 Then
			toStatus = 0 : lockIntro = "手动解冻"
			if rids<>"" then
				relation_sql = " update S2_SerialNumberRelation set del=1, [HandleStatus]=0 where  id in (" & rids &") and del<>2 "
			end if
			if ids1<>"" then
				locknum = "0"
				num2_sql = "(case when isnull(locknum,0)>num2 then isnull(locknum,0) else num2 end) "
		...
		cn.CursorLocation = 3
		cn.begintrans
		if len(relation_sql)>0 then
			cn.execute(relation_sql)
			num2_sql = " select k.id, sum(case when s2.del=1 then 1 else 0 end) num2, sum(case when s2.del=3 then 1 else 0 end) locknum, sum(case when isnull(s2.[HandleStatus],0)=1 then 1 else 0 end) lockcnt  " &_
			"from ku k "&_
			"inner join S2_SerialNumberRelation s2 on s2.Billtype=61001 and s2.listid = k.kuinlist  "&_
			"where k.id in ("& ids2 &") "&_
			"group by k.id "
			num2_sql = "update k set k.num2 = isnull(u.num2,0) , k.locknum=isnull(u.locknum,0), k.locked= (case when u.lockcnt>0 then 1 else 0 end),k.lockCate="& Info.User &",k.lockDate=getdate(),k.lockIntro='"& lockIntro &"'  "&_
			" from ku k "&_
			" inner join ("& num2_sql &") u on u.id = k.id "
			cn.execute(num2_sql)
		end if
		if ids1<>"" then
			cn.execute("update ku set locked = "& toStatus &", locknum="&locknum&" ,num2 = "&num2_sql &",lockCate="& Info.User &",lockDate=getdate(),lockIntro='"& lockIntro &"' where id in ("& ids1 &")  ")
		end if
		cn.CommitTrans
	end sub
	Sub app_removeDialogFlag
		cn.execute("insert into home_usConfig(name,nvalue,tvalue,uid) values('REMOVE_SERNUM_DIALOG_FLAG',null,1,"&Info.User&")")
	end sub
