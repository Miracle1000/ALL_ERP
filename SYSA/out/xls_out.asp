...
'Private Sub Class_Initialize
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>领用列表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">领用列表导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf &"</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出领用列表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Chtml="<td style='font-weight:bold'>领用主题</td>" & vbcrlf &_
	"<td style='font-weight:bold'>领用编号</td>" & vbcrlf &_
	"<td style='font-weight:bold'>领用人员</td>" & vbcrlf &_
	"<td style='font-weight:bold'>领用时间</td>" & vbcrlf &_
	"<td style='font-weight:bold'>领用分类</td>" & vbcrlf &_
	"<td style='font-weight:bold'>领用备注</td>" & vbcrlf &_
	"<td style='font-weight:bold'>名称</td>" & vbcrlf &_
	"<td style='font-weight:bold'>编号</td>" & vbcrlf &_
	"<td style='font-weight:bold'>型号</td>" & vbcrlf &_
	"<td style='font-weight:bold'>单位</td>" & vbcrlf &_
	"<td style='font-weight:bold'>数量</td>" & vbcrlf &_
	"<td style='font-weight:bold'>出库状态</td>" & vbcrlf &_
	"<td style='font-weight:bold'>出库仓库</td>" & vbcrlf &_
	"<td style='font-weight:bold'>库存信息</td>" & vbcrlf &_
	"<td style='font-weight:bold'>分类</td>" & vbcrlf &_
	"<td style='font-weight:bold'>批号</td>" & vbcrlf &_
	"<td style='font-weight:bold'>序列号</td>" & vbcrlf &_
	"<td style='font-weight:bold'>生产日期</td>" & vbcrlf &_
	"<td style='font-weight:bold'>有效日期</td>" & vbcrlf &_
	"<td style='font-weight:bold'>明细备注</td></tr>"
	Set xApp = server.createobject(ZBRLibDLLNameSN & ".HtmlExcelApplication")
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("领用列表")
	response.Flush()
	...
	sql="select * from O_productOut "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	...
	if not rs.eof then
		do until rs.eof
			...
			if not rs8.eof then
				get_lycateid=rs8("name")
			end if
			rs8.close
			set rs8=nothing
			get_time=rs("get_time")
			if rs("get_type")=1 then
				get_type="用品领用"
			elseif rs("get_type")=2 then
				get_type="用品申请"
			end if
			If rs("get_store")=1 Then
				get_store="已出库"
			ElseIf rs("get_store")=2 Then
				get_store="未出库"
			elseif rs("get_store")=3 then
				get_store="已否决"
			end if
			get_note=rs("get_note")
			on error resume next
			set xlsdoc=server.createobject("htmlfile")
			xlsdoc.open()
			set div = xlsdoc.createElement("div")
			div.innerHTML = get_note
			Set imgs = div.getElementsByTagName("img")
			imgstr=""
			If imgs.length > 0 Then
				...
				For i = 0 To imgs.length-1
					...
					w = img.style.width
					h = img.style.height
					If Len(w) > 0 Then img.width = Replace(w,"px","")
					If Len(h) > 0 Then img.height = Replace(h,"px","")
					If InStr(img.src,"about:blank")>0 Then
						img.src = xApp.images.add(Replace(img.src,"about:blank", GetHttpType() &  "://"&Request.ServerVariables("SERVER_NAME")))
					else
						img.src = xApp.images.add(Replace(img.src,"about:",GetHttpType() &  "://"&Request.ServerVariables("SERVER_NAME")))
					end if
				next
			end if
			On Error GoTo 0
			get_note=Replace(Replace(LCase(div.innerHTML),"src=","src="),"<img border=0 ","<img ")
			Set div = Nothing
			xlsdoc.close
			Set xlsdoc = Nothing
			rowHTML="<tr><td>"&get_title&"</td>" & vbcrlf &_
			"<td>"&get_bh&"</td>" & vbcrlf &_
			"<td>"&get_lycateid&"</td>" & vbcrlf &_
			"<td>"&get_time&"</td>" & vbcrlf &_
			"<td>"&get_type&"</td>" & vbcrlf &_
			"<td>"&get_note&"</td>" & vbcrlf &_
			"<td></td>" & vbcrlf &_
			"<td></td>" & vbcrlf &_
			"<td></td>" & vbcrlf &_
			"<td></td>" & vbcrlf &_
...
'<td></td></tr>
			...
			sql8="select a.*,b.prod_ph,b.prod_xlh,b.prod_sctime,b.prod_ystime,b.prod_num,c.pro_name,d.sort1,f.prod_name,f.prod_bh,f.prod_xh,k.sto_name from O_proOutList a "&_
			" inner join O_kuinlist b on a.prod_id=b.id "&_
			" left join O_product f on b.prod_id=f.id "&_
			" left join O_OfficeType c on f.prod_type=c.id "&_
			" left join sortonehy d on b.prod_unit=d.id "&_
			" left join O_store k on a.sto_id=k.id where a.get_fid="&rs("id")
			...
			if not rs8.eof then
				do while not rs8.eof
					...
					get_num=zbcdbl(rs8("get_num"))
					sto_name=rs8("sto_name")
					pro_name=rs8("pro_name")
					get_intro=rs8("get_intro")
					prod_ph=rs8("prod_ph")
					...
					"<td></td>" & vbcrlf &_
					"<td></td>" & vbcrlf &_
					"<td>"&prod_name&"</td>" & vbcrlf &_
					"<td>"&prod_bh&"</td>" & vbcrlf &_
					"<td>"&prod_xh&"</td>" & vbcrlf &_
					"<td>"&sort1&"</td>" & vbcrlf &_
					"<td>"&get_num&"</td>" & vbcrlf &_
					"<td>"&get_store&"</td>" & vbcrlf &_
					"<td>"&sto_name&"</td>" & vbcrlf &_
					"<td>"&prod_num&"</td>" & vbcrlf &_
					"<td>"&pro_name&"</td>" & vbcrlf &_
					"<td>"&prod_ph&"</td>" & vbcrlf &_
					"<td>"&prod_xlh&"</td>" & vbcrlf &_
					"<td>"&prod_sctime&"</td>" & vbcrlf &_
					"<td>"&prod_ystime&"</td>" & vbcrlf &_
					"<td>"&get_intro&"</td></tr>"
					rowIndex=rowIndex+1
'<td>&get_intro&</td></tr>
					xsheet.writeHTML rowHTML
			...
			If rowIndex > 40000 Then
				...
				If pageCount = 1 then
					xsheet.title = "领用列表(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = "领用列表(" & pageCount & "页)"
				Set xsheet = xApp.sheets.add("领用列表(" & pageCount & "页)")
				rowIndex = 0
				xsheet.writeHTML Chtml
			end if
			rs.movenext
	...
	set rs=Nothing
	xsheet.writeHTML "</table>"
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>领用列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!领用列表导出成功,共有"&(n)&"个领用单!</B>"";</script>"
	tfile=Server.MapPath("领用列表_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'领用列表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="领用列表导出"
	call close_list(1)
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的领用列表</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
