...
'If Not rs.Eof Then
	...
	Server.ScriptTimeOut=300000000
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>询价资料导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "<td class=""place"">询价资料导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%""border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出询价资料,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"" id=""CountTXT_width"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write Application("sys.info.jsver")
	curCate = session("personzbintel2007")
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&curCate&" and sort1=26 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_26_1=0
		intro_26_1=0
	...
	rs1.close
	sql1="select isnull(qx_open,0) qx_open,qx_intro from power where ord="& curCate &" and sort1=24 and sort2=21"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_24_21=0
	else
	...
	Set rs1 = Nothing
	Chtml=  ""&_
	"<td height='26' width='200' ><div align='center' style='font-weight:bold'>询价主题</div></td>"&_
	"<td width='8%'><div align='center' style='font-weight:bold'>询价编号</div></td>"&_
	"<td width='8%'><div align='center' style='font-weight:bold'>询价日期</div></td>"&_
	"<td width='8%'><div align='center' style='font-weight:bold'>询价状态</div></td>"
	if ZBRuntime.MC(4000) then
		Chtml=Chtml&"<td width='150'><div align='center' style='font-weight:bold'>关联报价</div></td>"
'if ZBRuntime.MC(4000) then
	end if
	Chtml = Chtml & "<td width='8%'><div align='center' style='font-weight:bold'>询价人员</div></td>"&_
	"<td width='8%'><div align='center' style='font-weight:bold'>定价人员</div></td>"&_
	"<td width='150'><div align='center' style='font-weight:bold'>询价概要</div></td>"
'if ZBRuntime.MC(4000) then
	Dim xjZdy, len_xjZdy,rs_xjmx,len_rsxjmx,xjZdyFields
	xjZdyFields = ""
	bjmxZdyFields = ""
	Set rs1 = conn.execute("select name,title,sort from zdy where sort1=24 and set_open=1 and dc=1 order by gate1 asc " )
	If rs1.eof = False Then
		xjZdy = rs1.GetRows()
	end if
	rs1.close
	Set rs1 = Nothing
	if isArray(xjZdy) then
		len_xjZdy = ubound(xjZdy,2)
	else
		len_xjZdy = -1
		'len_xjZdy = ubound(xjZdy,2)
	end if
	for i=0 to len_xjZdy
		Chtml = Chtml & "<td width='8%'><div align='center' style='font-weight:bold'>"& xjZdy(1,i) &"</div></td>"
'for i=0 to len_xjZdy
		If xjZdy(2,i)&"" = 1 Then
			xjZdyFields = xjZdyFields & ",isnull((select top 1 sort1 from sortonehy where ord=a." & xjZdy(0,i) & "),'') as " & xjZdy(0,i)
		else
			xjZdyFields = xjZdyFields & ",a." & xjZdy(0,i)
		end if
	next
	set rs = conn.execute("select name,title,sorce from zdymx where sort1=24 and set_open=1 order by gate1 asc")
	if not rs.eof then
		rs_xjmx = rs.GetRows()
	end if
	rs.close
	...
	For i = 0 To len_rsxjmx
		sorce = rs_xjmx(2,i)
		if Not((sorce=7 Or (sorce>=10 And sorce<=16)) and open_24_21=0) then
			Chtml = Chtml & "<td width='8%'><div align='center'>"& rs_xjmx(1,i) &"</div></td>"
		end if
	next
	Set xApp = server.createobject(ZBRLibDLLNameSN & ".HtmlExcelApplication")
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("询价资料")
	response.Flush()
	xsheet.writeHTML Chtml
	xsheet.movenext
	if open_24_10=3 then
		list=""
	elseif open_24_10=1 then
		list="and a.cateid in ("&intro_24_10&")"
	else
		list="and a.cateid=0"
	end if
	Str_Result="where a.del=1 "&list&""
	...
	End Select
	set rs=server.CreateObject("adodb.recordset")
	sql = "select a.id,a.title,a.xjid, "& vbCrLf &_
	...
	"                  c.name cateName, d.name djname, isnull(b.title,'') bjTitle, a.remark "&xjZdyFields& vbCrLf &_
	...
	priceMxSql = "select distinct pl.id as id,pl.id as id1,0 as id2,p.order1,p.title,p.type1,pl.zdy1,pl.zdy2,pl.zdy3,pl.zdy4,isnull(z5.sort1,'') zdy5,isnull(z6.sort1,'') zdy6,"&_
	"isnull(pl.price1,0) price1,pl.num1,pl.date2 date1,pl.intro,isnull(u1.sort1,'') unitall,'' as company, " &_
	"0 taxRate,'' invoiceType,1 discount, isnull(pl.price1,0) priceAfterDiscount, isnull(pl.price1,0) priceIncludeTax, "&_
	"isnull(pl.price1,0) priceAfterTax, isnull(pl.price1,0)*pl.num1 money1, 0 taxValue, isnull(pl.price1,0)*pl.num1 moneyAfterTax,0 cateid , cast('' as varchar(max)) share "&_
	" from pricelist pl "&_
	"inner join product p on pl.ord=p.ord "&_
	"left join sortonehy u1 on u1.ord=pl.unit "&_
	"left join sortonehy z5 on z5.ord=pl.zdy5 "&_
	"left join sortonehy z6 on z6.ord=pl.zdy6 "&_
	"where pl.del=3 and pl.price={price}" &_
	" union all "&_
	"select pl.id as id,pricelist as id1,pl.id as id2,p.order1,p.title,p.type1,pl.zdy1,pl.zdy2,pl.zdy3,pl.zdy4,isnull(z5.sort1,'') zdy5,isnull(z6.sort1,'') zdy6,"&_
	"isnull(pl.price1,0) price1,pl.num1,pl.date2 date1,pl.intro,isnull(u1.sort1,'') unitall,isnull(t.name,'') company,"&_
	"isnull(pl.taxRate,0) taxRate,isnull(kp.sort1,'') invoiceType,isnull(pl.discount,0) discount, "&_
	"isnull(pl.priceAfterDiscount,0) priceAfterDiscount, isnull(pl.priceIncludeTax,0) priceIncludeTax, " &_
	"isnull(pl.priceAfterTax,0) priceAfterTax, isnull(pl.money1,0) money1, isnull(pl.taxValue,0) taxValue,  " &_
	"isnull(pl.moneyAfterTax,0) moneyAfterTax,ISNULL(t.cateid,0) cateid,ISNULL(t.share,-222) share " &_
	"from xunjialist pl "&_
	"inner join product p on pl.ord=p.ord "&_
	"left join tel t on t.ord=pl.gys "&_
	"left join sortonehy u1 on u1.ord=pl.unit "&_
	"left join sortonehy z5 on z5.ord=pl.zdy5 "&_
	"left join sortonehy z6 on z6.ord=pl.zdy6 "&_
	"left join sortonehy kp on kp.id=pl.invoiceType and kp.gate2=34 and kp.isStop=0 and isnull(kp.id1,0)<>-65535 "&_
	"where pl.del=1 and pl.xunjia={xunjia} " &_
	" order by id1 asc,id2 asc"
	xjmxSql = "select p.order1,p.title,p.type1,pl.zdy1,pl.zdy2,pl.zdy3,pl.zdy4,isnull(z5.sort1,'') zdy5,isnull(z6.sort1,'') zdy6,"&_
	"isnull(pl.price1,0) price1,pl.num1,pl.date2 date1,pl.intro,isnull(u1.sort1,'') unitall,isnull(t.name,'') company,"&_
	"isnull(pl.taxRate,0) taxRate,isnull(kp.sort1,'') invoiceType,isnull(pl.discount,0) discount, "&_
	"isnull(pl.priceAfterDiscount,0) priceAfterDiscount, isnull(pl.priceIncludeTax,0) priceIncludeTax, " &_
	"isnull(pl.priceAfterTax,0) priceAfterTax, isnull(pl.money1,0) money1, isnull(pl.taxValue,0) taxValue,  " &_
	...
	"left join sortonehy z5 on z5.ord=pl.zdy5 "&_
	"left join sortonehy z6 on z6.ord=pl.zdy6 "&_
	"left join sortonehy kp on kp.id=pl.invoiceType and kp.gate2=34 and isnull(kp.id1,0)<>-65535 "&_
	"where pl.del=1 and pl.xunjia={xunjia} AND isnull(pl.pricelist,0)>=0" &_
	" order by pl.id "
	...
	if C1<=0 then
		...
		pageCount=1
		While rs.eof = False
			id=rs("id") : title=rs("title") : order1=rs("xjid") : date1=rs("date1") : complete=rs("complete")
			cateName=rs("cateName") : djname=rs("djname") : status=rs("status")
			bjTitle=rs("bjTitle") : price=zbcdbl(rs("price"))
			remark = RemoveHTML(HTMLDecode(rs("remark")))
			Select Case status
			Case 0 : trade5 = "暂存"
			...
			End Select
			rowHTML  =  ""&_
			"<td align='left'>"&title&"</td>"&_
			"<td align='left'>"&order1&"</td>"&_
			"<td align='center'>"&date1&"</td>"&_
			"<td align='center'>"&trade5&"</td>"
			colspan = 4
			if ZBRuntime.MC(4000) then
				rowHTML=rowHTML&"<td align='left'>"&bjTitle&"</td>"
				colspan = colspan + 1
				'rowHTML=rowHTML&"<td align='left'>"&bjTitle&"</td>"
			end if
			rowHTML=rowHTML&"<td align='center'>"&cateName&"</td>"&_
			"<td align='center'>"&djname&"</td>"&_
			"<td align='left'>"&remark&"</td>"
			colspan = colspan + 2
'<td align='left'>&remark&</td>
			for i=0 to len_xjZdy
				If xjZdy(0,i)="zdy5" Or xjZdy(0,i)="zdy6" Then
					rowHTML=rowHTML&"<td align='center'>"&rs(xjZdy(0,i))&"</td>"
				else
					rowHTML=rowHTML&"<td align='left'>"&rs(xjZdy(0,i))&"</td>"
				end if
				colspan = colspan + 1
				'rowHTML=rowHTML&"<td align='left'>"&rs(xjZdy(0,i))&"</td>"
			next
			if price > 0 then
				strSql=Replace(Replace(priceMxSql,"{price}",price),"{xunjia}",id)
			else
				strSql=Replace(xjmxSql,"{xunjia}",id)
			end if
			set rs7=server.CreateObject("adodb.recordset")
			rs7.open strSql,conn
			rowHTML=rowHTML&"</tr><tr>"
			While rs7.eof = False
				For ii = 1 To colspan+1
'While rs7.eof = False
					rowHTML=rowHTML&"<td></td>"
				next
				price1 = CDbl(rs7("price1")) : num1 = CDbl(rs7("num1"))
				unit = rs7("unitall")
				telCateid=rs7("cateid")
				telShare=rs7("share")
				If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(telCateid))=True) Or (telShare="1" Or InStr(1,","&telShare&",",","&session("personzbintel2007")&",",1) > 0) Then
					companyName = rs7("company")
				else
					companyName = ""
				end if
				For i = 0 To len_rsxjmx
					Select Case rs_xjmx(0,i)
					Case "company":
					rowHTML=rowHTML&"<td align='center'>"&companyName&"</td>"
					Case "price1":
					If open_24_21=1 Then rowHTML=rowHTML&"<td align='right' class='storeprice'>"& FormatNumber(price1,StorePrice_dot_num,-1) &"</td>"
'Case "price1":
					Case "priceAfterDiscount","priceIncludeTax","priceAfterTax":
					If open_24_21=1 Then rowHTML=rowHTML&"<td align='right' class='storeprice'>"& FormatNumber(rs7(rs_xjmx(0,i)&"")&"",StorePrice_dot_num,-1) &"</td>"
'Case "priceAfterDiscount","priceIncludeTax","priceAfterTax":
					Case "money1","taxValue","moneyAfterTax":
					If open_24_21=1 Then rowHTML=rowHTML&"<td align='right' class='money'>"& FormatNumber(rs7(rs_xjmx(0,i)&"")&"",num_dot_xs,-1) &"</td>"
'Case "money1","taxValue","moneyAfterTax":
					Case "taxRate"
					rowHTML=rowHTML&"<td align='center' class='str'>"& FormatNumber(rs7(rs_xjmx(0,i)&"")&"",num_dot_xs,-1) &"%</td>"
'Case "taxRate"
					Case "discount"
					If open_24_21=1 Then rowHTML=rowHTML&"<td align='center'>"&FormatNumber(rs7(rs_xjmx(0,i)&"")&"",DISCOUNT_DOT_NUM,-1)&"</td>"
'Case "discount"
					Case "num1":
					rowHTML=rowHTML&"<td align='center'>"&FormatNumber(num1,num1_dot,-1)&"</td>"
'Case "num1":
					Case "unitall":
					rowHTML=rowHTML&"<td align='center'>"&unit&"</td>"
					Case "zdy5","zdy6"
					rowHTML=rowHTML&"<td align='center'>"&rs7(rs_xjmx(0,i)&"")&"</td>"
					Case Else
					rowHTML=rowHTML&"<td align='left'>"&rs7(rs_xjmx(0,i)&"")&"</td>"
					End Select
				next
				rs7.movenext
			...
			If rowIndex > 40000 Then
				If pageCount = 1 then
					xsheet.title = "询价资料(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = "询价资料(" & pageCount & "页)"
				Set xsheet = xApp.sheets.add("询价资料(" & pageCount & "页)")
				rowIndex = 0
				xsheet.writeHTML Chtml
				xsheet.movenext
			end if
	...
	rs.close
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>询价资料导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!询价资料导出成功,共有"&(n)&"条记录!</B>"";</script>"
	tfile=Server.MapPath("询价资料_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'询价资料_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	Set xApp = Nothing
	action1="询价资料"
	call close_list(1)
	Response.write "" & vbcrlf & "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的询价资料</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & "" & vbcrlf & ""
