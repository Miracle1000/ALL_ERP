...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,3).Value = "跟进次数"
	...
	xlWorksheet.Cells(1,5).Value = "购买次数"
	...
	elseif open_1_10=1 then
		list="and x.cateid in ("&intro_1_10&")"
	else
		list="and cateid=0"
	end if
	Str_Result="and x.del=1 and x.sort3=1 "&list&""
	set rs=server.CreateObject("adodb.recordset")
	sql = "" & vbcrlf &_
	"select a.ord,c.khid,c.name,c.cateid,a.num1,isnull(b.pay,0) pay,isnull(b.numht,0) numht,g.name cateidname from (" & vbcrlf &_
	"select x.ord,count(*) num1 from tel x " & vbcrlf &_
	"inner join reply y on x.ord=y.ord and y.del=1 " & Str_Result & " " & vbcrlf & _
	"group by x.ord " & vbcrlf &_
	") a " & vbcrlf &_
	"left join (" & vbcrlf &_
	"select company,isnull(sum(money1),0) as pay,count(ord) as numht from contract where del=1 group by company " & vbcrlf &_
	") b on a.ord=b.company " & vbcrlf &_
	"inner join tel c on c.ord=a.ord " & vbcrlf &_
	"left join gate g on g.ord=c.cateid order by a.num1 desc "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "客户跟进排行_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof
		k=rs("name")
		...
		cateid=rs("cateidname")
		xlWorksheet.Cells(1+i,3).Value = num1
		cateid=rs("cateidname")
		...
		cateid=rs("cateidname")
		xlWorksheet.Cells(1+i,5).Value =numht
		cateid=rs("cateidname")
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户跟进排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户跟进排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("客户跟进排行_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的客户跟进排行</u></strong></font></a></p> " & vbcrlf & ""
