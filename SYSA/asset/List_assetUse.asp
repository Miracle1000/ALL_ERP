...
'function IsTime(values)
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and ass_addcateid in ("&W3&") "
	end if
	...
	if D2<>"" then
		Str_Result=Str_Result&" and ass_cycle in ("&D2&")"
	end if
	if D3<>"" then
	...
	if D10<>"" then
		Str_Result=Str_Result&" and ass_method in ("&D10&")"
	end if
	D11=ChangeDate(D11,1)
	if D11<>"" then
		Str_Result=Str_Result+" and  datediff(m,'"&D11&"',ass_using)>=0 "
'if D11<>"" then
	end if
	D12=ChangeDate(D12,2)
	if D12<>"" then
		Str_Result=Str_Result+" and  datediff(m,ass_using,'"&D12&"')>=0 "
'if D12<>"" then
	end if
	if D13<>"" then
	...
	if D18<>"" then
		Str_Result=Str_Result&" and ass_ycycle  in ("&D18&")"
	end if
	if D19<>"" then
	...
	if D21<>"" then
		Str_Result=Str_Result&" and ass_pj like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and ass_note like '%"&ml.De(D22)&"%'"
	end if
	M=ChangeDate(M,1)
	if M<>"" then
		Str_Result=Str_Result+" and ass_lytime>='"&M&"' "
'if M<>"" then
	end if
	K=ChangeDate(K,2)
	if K<>"" then
		Str_Result=Str_Result+" and '"&K&"'>=ass_lytime "
'if K<>"" then
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" then
		Str_Result=Str_Result+" and  ass_time>='"&L1&"' "
'if L1<>"" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" then
		Str_Result=Str_Result+" and  '"&L2&"'>=ass_time "
'if L2<>"" then
	end if
	L3=ChangeDate(L3,1)
	if L3<>"" then
		Str_Result=Str_Result+" and  ass_addtime>='"&L3&"' "
'if L3<>"" then
	end if
	L4=ChangeDate(L4,2)
	if L4<>"" then
		Str_Result=Str_Result+" and  '"&L4&"'>=ass_addtime"
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	if px="1" then
		Ptr_Result=" order by ass_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by ass_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by ass_time desc"
	elseif px="4" then
		Ptr_Result=" order by ass_time asc"
	elseif px="5" then
		Ptr_Result=" order by ass_lytime desc"
	elseif px="6" then
		Ptr_Result=" order by ass_lytime asc"
	end if
	ret=ChangeDate(ret,1)
	if ret<>"" then
		Str_Result=Str_Result+" and  ass_addtime>='"&ret&"' "
'if ret<>"" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" then
		Str_Result=Str_Result+" and  '"&ret2&"'>=ass_addtime"
'if ret2<>"" then
	end if
	alt=request("alt")
	if alt="1" then
		Str_Result=Str_Result+" and ass_id in(select ass_id from O_asset2 where ass_complete in(1,2) and ass_cateid_sp="&session("personzbintel2007")&") "
'if alt="1" then
	end if
	if C<>"" then
		if B="name" then
	...
	Response.write px
	Response.write """><input name=""alt"" type=""hidden"" value="""
	Response.write alt
	Response.write """>" & vbcrlf & "                        <input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索"">" & vbcrlf & "                   "
	if open_105_9=1  then
		Response.write "" & vbcrlf & "                     <input name=""submit1"" type=""button""  class=""anybutton2"" value=""导入"" onClick=""window.location.href='../load/UpLoad_assetUse.asp';""/>" & vbcrlf & "                      "
	end if
	if open_105_10=3 or open_105_10=1 then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_assetUse.asp?px="
		Response.write px
		Response.write "&alt="
		Response.write alt
	...
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产名称</div></td>" & vbcrlf & "                  <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产编号</div></td>" & vbcrlf & "                <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产型号</div></td>" & vbcrlf & "            <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产分类</div></td>" & vbcrlf & "                <td width=""13%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">存放地址</div></td>" & vbcrlf & "                <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产状态</div></td>" & vbcrlf & "                <td width=""18%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                          <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.sort1,c.C_name,d.sort1 as state from O_asset a left join O_assSetname b on a.ass_type=b.id left join O_assCardType c on a.ass_cartype=c.id left join O_assStatename d on a.ass_state=d.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=8><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		...
		do while not rs7.eof
			...
			if open_105_3=3 or CheckPurview(intro_105_3,trim(addcateid))=True then
				...
				Response.write rs7("ass_id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""15%"" height=""30"" align=""left"" valign=""middle"">" & vbcrlf & "            "
			if open_105_14=3 or CheckPurview(intro_105_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_assetUse.asp?ord="
				Response.write rs7("ass_id")
			...
			Response.write "</td>" & vbcrlf & "                   <td width=""14%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("ass_bh")
			Response.write "</div></td>" & vbcrlf & "             <td width=""14%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("ass_xh")
			Response.write "</div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("sort1")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("ass_position")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("state")
			Response.write "</div></td>" & vbcrlf & "             <td width=""18%"" height=""30"" align=""left"" valign=""middle""><div align=""center"">" & vbcrlf & "             "
			manbool=true
			sqltext="select * from O_asset2 where ass_complete in(1,2) and ass_id="&rs7("ass_id")
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				manbool=false
				ass_cateid_sp=rstxt("ass_cateid_sp")
				ass_complete=rstxt("ass_complete")
			else
				ass_cateid_sp=0
				ass_complete=0
			end if
			set rstxt=nothing
			if (open_105_16=3 or CheckPurview(intro_105_16,trim(addcateid))=True) and (ass_cateid_sp=session("personzbintel2007")) and (ass_complete="1" or ass_complete="2") then
				Response.write "" & vbcrlf & "                <input name=""Submit33"" type=""button"" class=""anybutton2""  onClick=""javascript:window.location=('Set_sp.asp?ord="
				Response.write pwurl(rs7("ass_id"))
				Response.write "')"" value=""审批""/>" & vbcrlf & "             "
			end if
			if open_105_14=3 or CheckPurview(intro_105_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""anybutton2"" id=""submit3"" onClick=""javascript:window.open('Detail_assetUse.asp?ord="
				Response.write rs7("ass_id")
			...
			if (open_105_2=3 or CheckPurview(intro_105_2,trim(addcateid))=True) and manbool then
				Response.write "" & vbcrlf & "                     <input name=""button4"" type=""button""  class=""anybutton2"" id=""submit4"" onClick=""javascript: window.open('Update_assetUse.asp?ord="
				Response.write rs7("ass_id")
			...
			delargs=true
			sql8="select * from O_assRepair where R_assID="&ml.IsNum(rs7("ass_id"))
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				delargs=false
			end if
			set rs8=nothing
			sql8="select * from O_assChange where H_assID="&ml.IsNum(rs7("ass_id"))
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				delargs=false
			end if
			set rs8=nothing
			sql8="select * from O_assDeprect where D_assID="&ml.IsNum(rs7("ass_id"))
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				delargs=false
			end if
			rs8.close
			set rs8=nothing
			if (open_105_3=3 or CheckPurview(intro_105_3,trim(addcateid))=True) and delargs then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""anybutton2"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_assetUse.asp?ord="
				Response.write rs7("ass_id")
				Response.write "&px="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按启用日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按启用日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按领用时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按领用时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="登记列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;z-index:2001;display:none;padding:2px 2px 2px 2px;background-color:lightblue"">" & vbcrlf & "<iframe src="""" name=""adsIF"" id=""adsIF"" width=""195px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<form target=""ssIF"" action=""../asset/Search_All_Search.asp"" id=""ssIFForm"" style=""display:none"" method=""post"">" & vbcrlf & "</form>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function adSearch(obj,sel_type)" & vbcrlf & "{" & vbcrlf & "   " & vbcrlf & "        var tleft=obj.offsetLeft;" & vbcrlf & "     var ttop=obj.offsetTop;" & vbcrlf & " var theight=obj.clientHeight;" & vbcrlf & "   while (obj = obj.offsetParent){ttop+=obj.offsetTop; tleft+=obj.offsetLeft;}" & vbcrlf & "     var sdivobj=document.getElementById(""adsDiv"");" & vbcrlf & "    sdivobj.style.display=""none"";" & vbcrlf &" sdivobj.style.left=tleft+""px"";" & vbcrlf & "    sdivobj.style.top=ttop+theight+""px"";" & vbcrlf & "      document.getElementById(""ssIFForm"").action=""../asset/Search_All_Search.asp?sel_type=""+sel_type;" & vbcrlf & "     document.getElementById(""ssIFForm"").target=""adsIF"";" & vbcrlf & " document.getElementById(""ssIFForm"").submit();" & vbcrlf & "      sdivobj.style.display=""inline"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "  }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_assetUse.asp?px="
	Response.write px
	Response.write "&alt="
	Response.write alt
	Response.write "&A="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action=""Del_assetUse.asp?px="
	Response.write px
	Response.write "&alt="
	Response.write alt
	Response.write "&A="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').method=""post"";" & vbcrlf & "    document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & " document.getElementById('data').action=""Add_lend.asp"";" & vbcrlf & "    document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function testall2()" & vbcrlf & "{" & vbcrlf & "      document.getElementById('data').action=""Add_Check.asp"";" & vbcrlf & "   document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
