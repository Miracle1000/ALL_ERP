...
'elseif open_6_11=1 then
					str_Result44=str_Result44+ " and c.cateid in ("&intro_6_11&") "
'elseif open_6_11=1 then
				else
...
'elseif open_6_11=1 then
					str_Result44=str_Result44+ " and c.cateid =0 "
'elseif open_6_11=1 then
				end if
...
'if khid<>"" then
				if E<>"" then Str_Result=Str_Result+" and  ly in  ("&E&") "
'if khid<>"" then
				area_list=-1
'if khid<>"" then
				if A2<>"" then
	...
	n=0
	sql="select * from("&_
	"select tel.ord,tel.name,tel.area,tel.khid,tel.cateid,tel.trade,tel.ly,"&_
	"bz.id as sortbzid,bz.sort1 as sortbzname, "&_
	"pt.id,pt.sort1,pt.sort2,"&_
	"(select sort1 from sortonehy where ord=pt.sort2) as sortname,"&_
	"m1 as money1,m2 as money2,m3 as money3,tel.share " & _
	"from ( " & vbcrlf & _
	"--先查数据 " & vbcrlf & _
	"from ( " & vbcrlf & _
	"  Select sum(money1) As m1, sum(money2) As m2, sum(money3) As m3,ord,bz,ptype From (" & vbcrlf & _
	"          select a.money1 as money1, 0 as money2, 0 as money3,b.ord, ISNULL(sq.bz,14) as bz, a.sort1 as ptype from paysqlist a WITH(NOLOCK) " & vbcrlf & _
	"          inner join paysq sq WITH(NOLOCK) on a.fid=a.id " & str_Result11 &  vbcrlf & _
	"          inner join tel b WITH(NOLOCK) On b.sort3=2 and a.del=1 and a.iwork=b.ord" & vbcrlf & _
	"          union all" & vbcrlf & _
	"          select 0,a.money1,0,b.ord,ISNULL(f.bz,14) as bz, a.sort as ptype from pay a WITH(NOLOCK) " & vbcrlf & _
	"          inner join f_pay f WITH(NOLOCK) on f.id=a.fid " & vbcrlf & _
	"          inner join tel b WITH(NOLOCK) on b.sort3=2 And a.del=1 and a.complete=3 and a.iwork=b.ord "& vbcrlf &_
	"          and exists (select 1 from f_pay x WITH(NOLOCK) where a.fid=x.id " & str_Result22 & ")" & vbcrlf & _
	"          union all" & vbcrlf & _
	"          select 0,0,a.money1, b.ord,isnull(c.bz,14), a.sort "&vbcrlf &_
	"          from paybxlist a WITH(NOLOCK)" & vbcrlf & _
	"          inner join paybx c WITH(NOLOCK) on c.id=a.bxid " & str_Result33 & vbcrlf & _
	"          inner join pay n WITH(NOLOCK) on a.del=1 and n.del=1 and a.payid = n.ord " & str_Result44 & vbcrlf & _
	"          inner join tel b WITH(NOLOCK) on b.sort3=2 And n.iwork = b.ord" & vbcrlf & _
	"  ) mmm " & vbcrlf &_
	"  Group By ord, bz, ptype " & vbcrlf & _
	") x" & vbcrlf & _
	"inner Join tel WITH(NOLOCK) on  tel.ord=x.ord and (x.m1>0 or x.m2>0 or x.m3>0) " & _
	"inner join paytype pt WITH(NOLOCK) on pt.id=x.ptype " & _
	"inner Join sortbz bz WITH(NOLOCK) on bz.id=x.bz " & _
	") as fy6 "& Str_Result &" "
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql & px_Result ,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "供应商费用明细表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			if rs("ly")<>0 and rs("ly")<>"" and (not isnull(rs("ly"))) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			area=""
			xlWorksheet.Cells(1+i,4).Value = rs("khid")
			area=""
			...
			area=""
			xlWorksheet.Cells(1+i,6).Value = ly
			area=""
			xlWorksheet.Cells(1+i,6).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,7).Value = trade
			area=""
			xlWorksheet.Cells(1+i,7).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,8).Value = area
			area=""
			xlWorksheet.Cells(1+i,8).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,9).Value = rs("sortbzname")
			area=""
			xlWorksheet.Cells(1+i,9).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,10).Value = Formatnumber(money1,num_dot_xs,-1)
			area=""
			xlWorksheet.Cells(1+i,10).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,11).Value = Formatnumber(money2,num_dot_xs,-1)
			area=""
			xlWorksheet.Cells(1+i,11).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,12).Value = Formatnumber(money3,num_dot_xs,-1)
			area=""
...
'loop
		xlWorksheet.Cells(1+i,1).colspan = 9
'loop
...
'loop
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>供应商费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!供应商费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("供应商费用明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的供应商费用明细表</u></strong></font></a></p>"
