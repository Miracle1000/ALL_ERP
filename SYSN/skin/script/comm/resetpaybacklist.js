$(function () {
    ...
    __lvw_je_addNew = function (id, disrefresh) {
        if (id == "contractlist" && $("#Company_tit").val() == "" && (Bill.GetField("Company").value == undefined || Bill.GetField("Company").value == "0")) {
            alert("请先选择客户");
            return;
        }
    ...
    ListView.nulldataRowClickEvent = function (lvwid, DBName, cellIndex, type) {
        if (lvwid == "contractlist" && $("#Company_tit").val() == "" && (Bill.GetField("Company").value == undefined || Bill.GetField("Company").value == "0")) {
            alert("请先选择供应商");
            return;
        }
