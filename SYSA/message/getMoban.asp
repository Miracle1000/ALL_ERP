...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	if open_67_13<>1 then
		Response.write"<script language=javascript>alert('您还没有短信权限!');window.opener=null;window.open('','_self');window.close();</script>"
		call db_close : Response.end
	end if
	if open_67_1=3 then
		Str_Result=""
...
'elseif open_67_1=1 then
	...
	if sortid<>"" and sortid<>"undefined" then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select * from mobanMessage  where del=1 and sort="&sortid&" "&Str_Result&" order by ord desc"
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			rs1.pagesize=5
			...
			for j=1 to rs1.pagesize
				...
				content=rs1("content")
				if len(content)>25 then
					content=left(content,25)&"..."
				end if
				Response.write("<li><a href='###' onclick=""getMobanCon('"&myReplace(rs1("content"))&"');getWordsLength();"" style='color:#ff0000'>"&i&"、"&content&"</a> </li>")
				rs1.movenext
				i=i+1
				rs1.movenext
			next
			...
			if page=1 then
				...
				Response.write("<a href='javascript:FF("&(page+1)&","&sortid&")'>"&("下一页&nbsp;")&"</a>")
				Response.write(("首页 上一页&nbsp;"))
				Response.write("<a href='javascript:FF("&rs1.pagecount&","&sortid&")'>"&("末页")&"</a>")
			elseif page=rs1.pagecount then
				Response.write("<a href='javascript:FF(1,"&sortid&")'>"&("首页&nbsp;")&"</a>")
				Response.write("<a href='javascript:FF("&(page-1)&","&sortid&")'>"&("上一页&nbsp;")&"</a>")
				Response.write("<a href='javascript:FF(1,"&sortid&")'>"&("首页&nbsp;")&"</a>")
			...
			Response.write("<font color=#ff0000>请先添加短信模板!</font>")
			call db_close : Response.end
		end if
		rs1.close
		set rs1=nothing
		...
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select top 6 * from mobanMessage  where del=1 "&Str_Result&" order by ord desc"
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			i=1
			Response.write("<ul>")
			do while not rs1.eof
				content=rs1("content")
				content1=rs1("content")
				if len(content)>25 then
					content=left(content,25)&"..."
				end if
				Response.write("<li><a href='###' onclick=""getMobanCon('"&trim(myReplace(rs1("content")))&"');getWordsLength();"" style='color:#ff0000'>"&i&"、"&content&"</a> </li>")
				rs1.movenext
				i=i+1
				rs1.movenext
			loop
