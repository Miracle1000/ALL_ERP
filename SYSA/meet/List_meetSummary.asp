...
'Private Sub Class_Initialize
	...
	if open_104_1=3 then
		list=""
	elseif open_104_1=1 then
		list=" and sum_addcateid in ("&intro_104_1&")"
	else
		list=" and sum_addcateid=0"
	end if
	Str_Result=" where 1=1 "&list&""
	ret          =request("ret")
	ret2 =request("ret2")
	...
	if M="1" Then remind = 44
	If remind & "" <> "" Then
		Set helper = CreateReminderHelper(conn,44,0)
		remindWhere = " AND a.id IN (" & helper.listSQL("ids") & ") "
	end if
	if M="3333333" or M="5555555" then
		new_list="<span class='alt_tx'></span>"
		timelg=session("timezbintel2007")
		if M="3333333" and timelg<>"" then
		end if
		Str_Result_44=list &" and send_cateid="&session("personzbintel2007")
	end if
	...
	if ret<>"" and ret<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',a.sum_addtime)>=0 "
'if ret<>"" and ret<>"点击选择时间" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" and ret2<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,a.sum_addtime,'"&ret2&"')>=0 "
'if ret2<>"" and ret2<>"点击选择时间" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	if C<>"" then
		...
		elseif B="mc" then
			Str_Result=Str_Result&" and sum_content  like '%"&ml.De(C)&"%'"
		end if
	end if''
	...
	Response.write ">纪要内容</option>" & vbcrlf & "                   </select><input type=""text"" name=""C"" size=""12"" id=""C"" value="""
	Response.write C
	...
	if open_104_10<>0 then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_meetSummary.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     </span></td>" & vbcrlf & "            </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">会议主题</div></td>" & vbcrlf & "             <td width=""30%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">纪要内容</div></td>" & vbcrlf & "                <td width=""27%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">会议时间</div></td>" & vbcrlf & "                  <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">记录员</div></td>" & vbcrlf & "                   <td width=""16%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                      <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	set rs7=server.CreateObject("adodb.recordset")
	if M="3333333" or M="5555555" then
		sql7="select a.*,b.id as sid,b.send_cateid,b.send_content,c.use_title,a.sum_addtime,b.id as use_id,c.use_meetingcycle as use_meetingcycle,c.use_startime,c.use_endtime,c.use_state from O_MeetingSummary a INNER join O_SummarySend b on a.id=b.send_meetingid INNER join O_MeetingUse c on c.id=a.sum_metId where 1=1 "&Str_Result_44&" and send_type=1 and send_issucceed=1 "&noalt1&"  order by a.sum_addtime desc "
	else
		sql7="select a.*,b.use_title,b.id as use_id,b.use_meetingcycle,b.use_startime,b.use_endtime,b.use_state from O_MeetingSummary a left join O_MeetingUse b on a.sum_metId=b.id "&Str_Result&" "& remindWhere &" "&Ptr_Result
	end if
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			sum_addcateid=rs7("sum_addcateid")
			...
			if open_104_3=3 or CheckPurview(intro_104_3,trim(sum_addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""left"">" & vbcrlf & "               "
			if open_104_14=3 or CheckPurview(intro_104_14,trim(sum_addcateid))=True then
			...
			Response.write "</a>"
			if timelg<>"" and datediff("s",timelg,rs7("sum_addtime"))>0 then Response.write new_list
			Response.write "</div></td>" & vbcrlf & "               <td width=""30%"" align=""left"" class=""name""><div align=""left"">"
			Response.write rs7("sum_content")
			Response.write "</div></td>" & vbcrlf & "               <td width=""27%"" align=""left"" class=""name""><div align=""center""> "
			if rs7("use_meetingcycle")=1 then
				Response.write rs7("use_startime")&"—"&rs7("use_endtime")
			else
				sql8="select lst_starttime,lst_endtime from O_MeetingUseList where lst_fid="&ml.IsNum(rs7("use_id"))
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					do while not rs8.eof
						Response.write rs8("lst_starttime")&"—"&rs8("lst_endtime")&"<br />"
						rs8.movenext
					loop
				end if
				set rs8=nothing
			...
			sql8="select name from gate where ord="&ml.IsNum(rs7("sum_sendcateid"))
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Response.write rs8("name")
			end if
			set rs8=nothing
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "              <td height=""30"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                   "
			if M="1" Or M="3333333" or M="5555555" then
				Response.write "" & vbcrlf & "                          <a href=""setalt.asp?ord="
				Response.write rs7("id") &"&tk=1"
				Response.write """ onclick=""return confirm('确认取消提醒？')"" ><img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0""></a>" & vbcrlf & "                  "
			end if
			if open_104_14=3 or CheckPurview(intro_104_14,trim(sum_addcateid))=True then
				Response.write "" & vbcrlf & "                   <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_meetSummary.asp?ord="
				Response.write rs7("id")
			...
			if ((open_104_2=3 or CheckPurview(intro_104_2,trim(sum_addcateid))=True)) and rs7("use_state")=2 then
				Response.write "" & vbcrlf & "                       <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_meetSummary.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
			...
			if open_104_3=3 or CheckPurview(intro_104_3,trim(sum_addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_meetSummary.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30"">全选<input name=""chkall"" type=""checkbox"" id=""chkall"" value=""chkall"" onClick=""mm(document.getElementById('data'))""></td>" & vbcrlf & "            <td>" & vbcrlf & "            "
	...
	Response.write "</td>" & vbcrlf & "          </form>" & vbcrlf & "       <form name=""oneself"" action=""List_meetSummary.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按申请时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按申请时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按申请主题排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按申请主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="纪要列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & "  window.location=""List_meetSummary.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "       document.getElementById('data').action=""Del_meetSummary.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & " document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>"
