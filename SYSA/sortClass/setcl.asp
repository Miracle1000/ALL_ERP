...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "       <style>.edit-body .sub-thead td.fcell{border-top:#c0ccdd 1px solid}#comm_itembarbg{height:42px!important}" & vbcrlf & "div.bottomdiv{height:40px;}</style>" & vbcrlf & "    "
'Sub Bill_OnPageInit
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		Dim mButtons,rs, sort , typeTitle ,secTitle , sort1,f
		...
		bill.title = typeTitle&"资质策略设置"
		bill.groups(0).title = typeTitle&"资质策略控制"
		Bill.fields.addHtml "", "Ahtml", 3, 1
		With Bill.AddCurrGroup("资质审批控制", "divBody1")
		.visible = (getValue(sort1)<>0)
		.fields.addHtml "", "Bhtml", 3, 1
		End with
		with Bill.AddCurrGroup(typeTitle&secTitle&"策略控制", "divBody2")
		.visible = (getValue(sort1)<>0)
		.fields.addHtml "", "Chtml", 3, 1
		End With
		Set f = Bill.fields.add("", "sort")
		f.uitype ="hidden"
		f.value = sort
	end sub
	Sub AHtml(ByVal field)
		Dim rs, PreNodes,sort,sort1
		sort = request("sort")
		sort1 = 101
		If sort & "" = "1" Then
			sort1 = 101
		...
		PreNodes = "<input type='radio' name='openQualifications' value='0' onclick=""bill.groupvisible('divBody1',false);bill.groupvisible('divBody2',false)"" "
		If getValue(sort1) =0 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes & ">不启用资质&nbsp;&nbsp;<input type='radio' name='openQualifications' value='1' onclick=""bill.groupvisible('divBody1',true);bill.groupvisible('divBody2',true)""  "
		If getValue(sort1) =1 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes & ">录入时选择是否需要资质&nbsp;&nbsp;<input type='radio' name='openQualifications' value='2' onclick=""bill.groupvisible('divBody1',true);bill.groupvisible('divBody2',true)"" "
		If getValue(sort1) =2 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes & ">一直启用资质"
		Response.write "<div class='sub-field'>" & PreNodes & "</div>"
		PreNodes = PreNodes & ">一直启用资质"
	end sub
	Sub Bhtml(Byval field)
		...
		PreNodes = "<input type='checkbox' name='SpQualifications' value='1' "
		If getValue(sort1)=1 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes &">任何资质都需要审批"
		Response.write "<div class='sub-field'>" & PreNodes & "</div>"
		PreNodes = PreNodes &">任何资质都需要审批"
	end sub
	Sub Chtml(Byval field)
		Dim rs, PreNodes,sort,sort1,title1,sort2,title2
		sort = request("sort")
		sort1 = 103
		sort2 = 104
		If sort & "" = "1" Then
			...
			sort1 =107
			title1 = "资质审批未通过不允许签订合同"
			sort2 = 108
			title2 = "资质过期不允许签订合同"
		end if
		PreNodes = "<input type='checkbox' name='UnQualifications' value='1'"
		If getValue(sort1) = 1 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes & ">"&title1&" &nbsp;&nbsp;<input type='checkbox' name='GqQualifications' value='1'"
		If getValue(sort2) = 1 Then PreNodes = PreNodes & " checked "
		PreNodes = PreNodes & ">"&title2&""
		Response.write "<div class='sub-field'>" & PreNodes & "</div>"
		PreNodes = PreNodes & ">"&title2&""
	end sub
	Function getValue(sort)
		Dim rs,nValue
		nValue = 0
		Set rs=cn.execute("select intro from setopen where sort1="&sort)
		If rs.eof = False Then
			nValue = rs("intro")
		end if
		rs.close
	...
	Function setValue(sort ,nValue)
		Dim rs
		Set rs = server.CreateObject("adodb.recordset")
		rs.open "select * from setopen where sort1="& sort, cn, 1, 3
		If rs.eof = False Then
			rs("intro").value = nValue
			rs.update
		else
			rs.addnew
			rs("intro").value = nValue
	...
	function Bill_OnSave(ByVal bill)
		Dim openQualifications, SpQualifications, UnQualifications, GqQualifications,sort
		openQualifications = app.getint("openQualifications")
		SpQualifications = app.getint("SpQualifications")
		UnQualifications = app.getint("UnQualifications")
		GqQualifications = app.getint("GqQualifications")
		sort = app.getint("sort")
		If sort & "" = "1" Then
			Call setValue(101 ,openQualifications)
			Call setValue(102 ,SpQualifications)
			Call setValue(103 ,UnQualifications)
			Call setValue(104 ,GqQualifications)
		else
			Call setValue(105 ,openQualifications)
			Call setValue(106 ,SpQualifications)
			Call setValue(107 ,UnQualifications)
			Call setValue(108 ,GqQualifications)
		end if
		bill.alert "保存成功。"
		Bill_OnSave = true
	end function
