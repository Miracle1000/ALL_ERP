...
'If InStr(1,","&mustcon&",",",97,",1)>0 Then
		...
		Sub GetDefFieldName(ByRef arrShow, ByRef arrName,ByRef arrRequired)
			Set rs=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name, (case when show>0 then 1 else 0 end) as show,gate1,required from setfields where gate1 in (1,3,17,19,20) order by gate1 asc ")
			ReDim arrShow(0)
			ReDim arrName(0)
			ReDim arrRequired(0)
			While Not rs.eof
		...
		sub Page_loadConfig(uploadatt)
			...
			.fileName = "洽谈进展资料"
			.filters = "xls|xlsx"
			.smpFilePath = replace("../../in/example_reply.asp","_0","")
			.helpFilePath = "../../in/caption_reply.doc"
		...
		Sub Page_OnFileSave(uploader)
			If uploader.InsertTableByExcel("#replydrList", "") Then
		...
		Sub Page_OnCreateTempTable(uploader)
			...
			If arrShow(19)=1 Then strcols = strcols & "|" & arrName(19)& "=phone=0=tel"
			If arrShow(20)=1 Then strcols = strcols & "|" & arrName(20)& "=mobile=0=tel"
			strcols = strcols & "|跟进方式=sort98=1=reply|洽谈进展=intro=0=reply|跟进人员=cateid=1=tel|添加时间=date7=0=reply"
			uploader.baseCols = strcols
			uploader.defdbtable = "reply"
		...
		sub Page_LinkFieldHand(tmp , ywname , dbname , gl , UserDefCol, dbtable)
			...
			Case "跟进人员" : cn.execute "update " & tmp & " set 跟进人员 = cast(a.ord as varchar(10)) from gate a where a.name=跟进人员"
			End select
		end sub
		function Page_InsertDataBase(dbname , uploader)
			...
			cn.execute "  update " & dbname & " set ["&arrName(1)&"]=(select top 1 cast(["&arrName(1)&"] as nvarchar(100)) from " & dbname & " c where datalength(isnull(rtrim(cast(c.["&arrName(1)&"] as nvarchar(100))),'')) > 0 and c.up_index<" & dbname & ".up_index order by c.up_index desc) where datalength(isnull(rtrim(cast(["&arrName(1)&"] as nvarchar(100))),'')) = 0"
			cn.execute "  update " & dbname & " set ["&arrName(3)&"]=(select top 1 cast(b.["&arrName(3)&"] as nvarchar(50)) from " & dbname & " b where datalength(isnull(rtrim(cast(b.["&arrName(3)&"] as nvarchar(50))),'')) > 0 and b.up_index<" & dbname & ".up_index order by b.up_index desc) where datalength(isnull(rtrim(cast(["&arrName(3)&"] as nvarchar(50))),'')) = 0"
			uploader.ShowProc "写入客户资料：删除空数据         进度：" , 100
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'空数据("&arrName(1)&"或"&arrName(3)&"不能为空)' from " & dbname & " where  len(isnull(rtrim(cast(["&arrName(1)&"] as nvarchar(100))),'')) = 0 "
			cn.execute "delete from " & dbname & " where len(isnull(rtrim(cast(["&arrName(1)&"] as nvarchar(100))),'')) = 0 " , n
			cn.execute "insert into #k_all  (内容)  values ('从导入文件删除" & n & "条空数据')"
			cn.execute "update " & dbname  & " set " & arrName(3) & "='' where " & arrName(3) & " is NULL"
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'洽谈进展不可为空' from " & dbname & " where  len(isnull(rtrim(cast(洽谈进展 as varchar(4000))),'')) = 0"
			cn.execute "delete from " & dbname & " where len(isnull(rtrim(cast(洽谈进展 as varchar(4000))),'')) = 0" , n
			cn.execute "insert into #k_all  (内容)  values ('从导入文件删除" & n & "条洽谈进展空数据')"
			uploader.ShowProc "写入客户资料：删除无效客户               进度：" ,200
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'第' + cast(up_index as varchar(20)) + '行客户不存在' from " & dbname & " a where  (select count(1) from tel where name=a.["&arrName(1)&"] and isnull(khid,'')=a.["&arrName(3)&"])=0"
			uploader.ShowProc "写入客户资料：删除无效客户               进度：" ,200
			cn.execute "delete " & dbname & " where (select count(1) from tel where name=["&arrName(1)&"] and isnull(khid,'')=["&arrName(3)&"])=0" , n
			cn.execute "insert into #k_all  (内容,说明)  values ('从导入文件删除" & n & "条客户不存在','"&arrName(1)&"或"&arrName(3)&"必须一致')"
			uploader.ShowProc "写入客户资料：删除无效跟进方式           进度：" ,250
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'跟进方式是必填字段,不能为空,无法导入。' from " & dbname & " where len(isnull(cast(跟进方式 as varchar(500)),''))=0 "
			cn.execute "delete from " & dbname & " where len(isnull(cast(跟进方式 as varchar(500)),''))=0 ", n
			cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & n & "条跟进方式为空或无效数据,请参见跟进方式设置')"
			uploader.ShowProc "写入客户资料：删除无效跟进方式          进度：" ,250
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'跟进方式不匹配,无法导入。' from " & dbname & " where isnull(cast(跟进方式 as varchar(500)),'') not in (select cast(ord as varchar) from sortonehy where gate2=98)"
			cn.execute "delete from " & dbname & " where isnull(cast(跟进方式 as varchar(500)),'') not in (select cast(ord as varchar) from sortonehy where gate2=98)", n
			cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & n & "条跟进方式不匹配,请参见跟进方式设置')"
			Dim countR
			For countR=0 To ubound(arrRequired)
				If arrShow(countR)>0 And arrRequired(countR)>0 And countR<>11 And countR<>18 And countR<>25 Then
					If countR>=17 And countR<=32 Then
						cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(countR)&" 是必填字段,不能为空,无法导入。' from " & dbname & " where len(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 "
						cn.execute "delete from " & dbname & " where len(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 ", i  :  n = n + i
						cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & i & "条"&arrName(countR)&" 为空数据,请参见字段策略设置')"
					else
						cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(countR)&" 是必填字段,不能为空,无法导入。' from " & dbname & " where len(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 and len(isnull(["&arrName(1)&"],'')) > 0"
						cn.execute "delete from " & dbname & " where len(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 and len(isnull(["&arrName(1)&"],'')) > 0", i  :  n = n + i
						cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & i & "条"&arrName(countR)&" 为空数据,请参见字段策略设置')"
					end if
				end if
			next
			uploader.ShowProc "写入客户资料：删除无效关联空数据                进度：" ,300
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'第' + cast(up_index as varchar(20)) + '行跟进人员不存在' from " & dbname & " a where  isnull(跟进人员,0) not in (select cast(ord as varchar) from gate)"
			uploader.ShowProc "写入客户资料：删除无效关联空数据                进度：" ,300
			cn.execute "delete " & dbname & " where isnull(跟进人员,0) not in (select cast(ord as varchar) from gate)" , n1
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'第' + cast(up_index as varchar(20)) + '行日期格式不正确' from " & dbname & " a where  isdate(cast(a.添加时间 as varchar(50)))=0 "
			cn.execute "delete " & dbname & " where isnull(跟进人员,0) not in (select cast(ord as varchar) from gate)" , n1
			cn.execute "delete " & dbname & " where  isdate(cast(添加时间 as varchar(50)))=0" , n2
			cn.execute "insert into #k_all (内容,说明) values ('从导入文件删除" & n1 & "条跟进人员不存在记录','" & n2 & "条日期格式不正确记录')"
			maxord=cn.execute("select isnull((select max(id) from reply),0)")(0).value
			uploader.ShowProc "写入客户资料：写入客户洽谈进展          进度：" ,400
			cn.execute "insert into reply (ord,ord2,sort1,gj1,gj2,name2,sort98,intro,name,cateid,date7,cateid2,cateid3,del)" & vbcrlf & _
			"                       ""select a.ord,a.ord,1,a.sort,a.sort1,isnull((select top 1 name from person where name=b.[""&arrName(17)&""] and company=a.ord),''),b.跟进方式,cast(b.洽谈进展 as nvarchar(4000)),(select top 1 name from gate where ord=b.跟进人员),b.跟进人员,cast(b.添加时间 as varchar(50)),-1,0,1 from tel a, "" & dbname & "" b where a.[name]=b.["&arrName(1)&"] and isnull(a.[khid],'')=isnull(b.["&arrName(3)&"],'')" ,n
			cn.execute "update a set a.datelast = convert(varchar(20),b.添加时间,120) from tel a " & vbcrlf & _
			" inner join (" & vbcrlf & _
			"  select "&arrName(1)&", "&arrName(3)&",max(添加时间) as 添加时间 from " & dbname & " group by "&arrName(1)&", "&arrName(3)&" " & vbcrlf & _
			" ) b on a.[name]=b.["&arrName(1)&"] and isnull(a.[khid],'')=isnull(b.["&arrName(3)&"],'') " & vbcrlf & _
			" where a.del<>2 "
...
'"select sorce as bm, sorce2 as xz , ord  from gate "& vbcrlf & _
			cn.execute "insert into #k_all  (内容) values ('共写入" & n & "条客户洽谈进展记录')"
			uploader.ShowProc "写入联系人资料：写入联系人数据          进度：" ,500
			sql = "select up_index,["&arrName(3)&"],["&arrName(1)&"],["&arrName(17)&"],"
			If arrShow(19)=1 Then sql = sql & "["&arrName(19)&"],"
			If arrShow(20)=1 Then sql = sql & "["&arrName(20)&"],"
			sql = sql & "跟进方式,洽谈进展,跟进人员,添加时间,0 as pord,0 as cateid2,0 as cateid3 into #lxrReply  from " & dbname &" c where len(isnull(rtrim(c.["&arrName(17)&"]),'')) > 0"
			cn.execute sql
			sql = " update #lxrReply set pord=isnull((select top 1 isnull(ord,0) from person c where company=(select top 1 ord from tel where [name]=#lxrReply.["&arrName(1)&"] and isnull([khid],'')=isnull(#lxrReply.["&arrName(3)&"],'')) and name=#lxrReply.["&arrName(17)&"] "
			If arrShow(19)=1 Then sql = sql & " and isnull(phone,'')=isnull(#lxrReply.["&arrName(19)&"],'') "
			If arrShow(20)=1 Then sql = sql & " and isnull(mobile,'')=isnull(#lxrReply.["&arrName(20)&"],'')"
			sql = sql & "),0)"
			cn.execute sql
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'第' + cast(up_index as varchar(20)) + '行联系人洽谈进展导入失败' from #lxrReply b where isnull(pord,0)=0"
			cn.execute sql
			cn.execute "delete #lxrReply where isnull(pord,0)=0" , n1
			If arrShow(19)=1 And arrShow(20)=1 Then cn.execute "insert into #k_all (内容,说明) values ('从导入文件删除" & n1 & "条联系人不匹配','请检查"&arrName(19)&"与"&arrName(20)&"是否一致')"
			uploader.ShowProc "写入客户资料：写入联系人洽谈进展                进度：" ,600
			cn.execute "insert into reply (ord,ord2,sort1,gj1,gj2,name2,sort98,intro,name,cateid,date7,cateid2,cateid3,del)" & vbcrlf & _
			"select a.ord,b.pord,8,a.sort,a.sort1,b.["&arrName(17)&"],b.跟进方式,cast(b.洽谈进展 as nvarchar(4000)),(select top 1 name from gate where ord=b.跟进人员),b.跟进人员,cast(b.添加时间 as varchar(50)),-1,0,1 from tel a,  #lxrReply b where a.[name]=b.["&arrName(1)&"] and isnull(a.[khid],'')=b.["&arrName(3)&"]" ,n
			cn.execute  "update reply set cateid2 = bm , cateid3 = xz from (" & vbcrlf & _
			"  select sorce as bm, sorce2 as xz , ord  from gate" & vbcrlf & _
			") a where a.ord = cateid and cateid2=-1 "
'select sorce as bm, sorce2 as xz , ord  from gate & vbcrlf & _
			cn.execute "insert into #k_all  (内容) values ('共写入" & n & "条联系人洽谈进展')"
			Dim ordarr()
			Dim sortarr()
			Dim sort1arr()
			Dim replyarr()
			i=0
			Dim crss
			Set crs=cn.execute("select distinct(a.ord) as ord,max(a.sort) as sort,max(a.sort1) as sort1 from tel a, " & dbname & " b where a.[name]=b.["&arrName(1)&"] and isnull(a.[khid],'')=isnull(b.["&arrName(3)&"],'') group by a.ord")
			If crs.eof=False then
				Do While Not crs.eof
					ReDim Preserve ordarr(i)
					ReDim Preserve sortarr(i)
					ReDim Preserve sort1arr(i)
					ReDim Preserve replyarr(i)
					ordarr(i)=getInt(crs("ord"))
					sortarr(i)=getInt(crs("sort"))
					sort1arr(i)=getInt(crs("sort1"))
			...
			For vi=0 To i-1
				...
				If Len(sort1arr(vi))>0 And isnumeric(sort1arr(vi)) Then
					call autoSkipSort(ordarr(vi),sortarr(vi),sort1arr(vi),13,replyarr(vi),True,"洽谈进展导入")
				end if
			next
