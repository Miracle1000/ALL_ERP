...
'sub index()
		...
		clo=request("clo")
		if clo<>"" then
			Response.write "" & vbcrlf & "<script language='javascript'>window.opener.location.reload();window.opener=null;window.open('','_self');window.close();</script>" & vbcrlf & ""
		end if
		rd=request("rd")
		if rd<>"" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select *  from email_Person_class Where id="&rd&" "
		...
		Response.write """  onkeyup=""value=value.replace(/\D/g,'')""  size=""10"">" & vbcrlf & "              （排序框里只能输入数字，排序时将按照数字从大到小排序）</td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""15%"" align=""right"" height=""33"">使用范围：</td>" & vbcrlf & "            <td width=""85%""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                <tr class=""gray"">" & vbcrlf & "                  <td style=""padding:5px""><input type=""radio"" name=""allcansee"" value=""1"" checked=""true"" id=""rbtn1"" onclick=""document.getElementById('tb1').style.display='none';"" />" & vbcrlf & "                    <label for=""rbtn1"">允许所有用户使用</label>" & vbcrlf & "                    <input type=""radio"" name=""allcansee"" value=""0"" id=""rbtn2"" onclick=""document.getElementById('tb1').style.display='';"" />" & vbcrlf & "                    <label for=""rbtn2"">选择允许使用用户</label>" & vbcrlf & "                    <span id=""ulist1"" class=""red""></span> </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr style=""display:none"" id=""tb1"">" & vbcrlf & "                  <td style=""padding:5px"">"
		sort_zjjg=2
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	sub addto()
		...
		if id1="" then id1=0
		set rs=conn.execute("select title,depth from email_Person_class where id="&id1)
		if rs.eof then
			gate2=0
		else
			gate2=rs("depth")
...
'if gate2="" then gate2=0
		...
		W3=getW3(W1,W2,W3)
		W3=getLimitedW3(W3,2,2,0,session("personzbintel2007"))
		If W3&""="" Then W3 = 0
		if menuname = "" then
			Response.write"<script language=javascript>alert('请填写分类！');history.back()</script>"
			call db_close : Response.end
		end if
		sql = "select id from email_Person_class where title='"&menuname&"' and parent="&id1&""
		Set Rs = server.CreateObject("adodb.recordset")
		...
		rs.close
		set rs=nothing
		sqlStr="Insert Into email_Person_class(parent,title,sort,depth,User_List) values('"
		sqlStr=sqlStr & id1 & "','"
