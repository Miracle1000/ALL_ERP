...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "    <style>" & vbcrlf & "        body{margin:0;}" & vbcrlf & "    </style>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF""  height=""1600""> "& vbcrlf & " <tr>" & vbcrlf &   "  <td width=""100%"" valign=""top"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> "& vbcrlf &     "     <tr> "& vbcrlf &         "    <td class=""place"">联系人分组设置</td>" & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> " & vbcrlf & " <tr border=1 valign=""center"" class=""top"">" & vbcrlf & " <td>" & vbcrlf & "   <div align=""left""><a href=""javascript:void(0)"" onClick=""javascript:window.open('add_cp.asp','newwinadd','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">添加根目录</a></div></td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	...
	editmenu
	case "open"
	openmenu request.QueryString("id")
	Response.redirect("menu.asp")
	case "close"
	closemenu request.QueryString("id")
	Response.redirect("menu.asp")
	case else
	menu 0
	end select
	...
	set conn=nothing
	function menu(id)
		if open_77_12=3  then
			powersql=""
		else
			powersql=" and (CHARINDEX(',"&session("personzbintel2007")&",',','+replace(user_list,' ','')+',')>0 or user_list='0,0' or user_list='0' or user_list is null) "
			powersql=""
		end if
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from email_Person_class where parent="&id&" "&powersql&" order by sort desc,id asc"
		rs.open sql,conn,1,1
		Response.write("<table border='0' width='100%' cellpadding='0' cellspacing='0' bgcolor='#FFFFFF' >")
		i=1
		while not rs.eof
			...
			Response.write "" & vbcrlf & "<tr><td id=""b"
			Response.write  rs("id")
			Response.write """ class="""
			Response.write  menutype
			Response.write """ onMouseUp="""
			Response.write  onmouseup
			Response.write """>"
			Response.write  menuname
			Response.write " &nbsp;&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""javascript:window.open('add_cp.asp?id="
			Response.write  rs("id")
			Response.write "&gate2="
			Response.write  rs("depth")
			Response.write "','newwinadd','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""><font  color=""#909090"">添加 |</font></a> <a href=""javascript:void(0)"" onClick=""javascript:window.open('correct_cp.asp?rd="
			Response.write  rs("depth")
			Response.write  rs("id")
			Response.write "','newwincorrect','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""><font color=""#909090"">修改</font></a> "
			Response.write  rs("id")
			if session("top1zbintel2007")= "1" then
				Response.write "| <a href=""menu.asp?action=del&id="
				Response.write  rs("id")
				Response.write """ onclick=""return confirm('建议不要轻易删除设置好的参数，以免影响到其他关联的数据。确认删除?')""><font  color=""#909090"">删除</font></a>"
			end if
			if 1=1  then
				if rs("del")=2 then
					Response.write " |<a href=""menu.asp?action=open&id="
					Response.write  rs("id")
					Response.write """ onclick=""return confirm('确认开启?')""><font  color=""#ff0000""> 开启</font></a> "
				end if
				if isnull(rs("del")) or rs("del")<>2 then
					Response.write "|<a href=""menu.asp?action=close&id="
					Response.write  rs("id")
					Response.write """ onclick=""return confirm('确认关闭?')""><font  color=""#909090""> 关闭</font></a>"
				end if
			end if
			Response.write "" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & ""
			if ChildCount>0 then
				Response.write "" & vbcrlf & "<tr id=""a"
				Response.write  rs("id")
				Response.write """><td class="""
				Response.write  listtype
				Response.write """>"
				menu(rs("id"))
	...
	function delmenu(id)
		set rs=conn.execute("select id from email_Person_class where parent="&id)
		if not rs.eof then
			Response.write("<script>alert('有下级分类，不可删除！');history.back()</script>")
			call db_close : Response.end()
		end if
		sql7="select * from email_Person where sort="&id
		set rs7=conn.Execute(sql7)
		if not rs7.eof then
			Response.write("<script>alert('此联系人组分类已经在联系人组中使用，不可删除！');history.back()</script>")
			call db_close : Response.end()
		else
			conn.execute("delete from email_Person_class where id="&id)
		end if
		set rs7=nothing
	end function
	function closemenu(id)
		conn.execute("update  email_Person_class set del=2 where id="&id)
		conn.execute("update  email_Person_class set del=2 where parent="&id)
	end function
	function openmenu(id)
		conn.execute("update  email_Person_class set del=1 where id="&id)
		conn.execute("update  email_Person_class set del=1 where parent="&id)
	end function
	function savemenu()
		action=request.Form("action")
		id=request.Form("id")
		id1=session("smsPsmenu_list")
		if id1="" then
			id1=0
		end if
		...
		select case action
		case "edit"
		ChildCount=conn.execute("select count(*) from email_Person_class where parent="&id)(0)
		if ChildCount>0 then url=""
		conn.execute("update email_Person_class set parent="&id1&",title='"&menuname&"',sort="&gate1&" where id="&id)
		case "add"
		conn.execute("insert into email_Person_class (parent,title,sort) values ("&id1&",'"&menuname&"',"&gate1&")")
		end select
	end function
	function addmenu()
	...
	function menuform(id,id1,action)
		if action="edit" then
			set rs=conn.execute("select * from email_Person_class where id="&id)
			menuname=rs("title")
			gate1=rs("sort")
			rs.close
			set rs=nothing
		end if
		session("smsPsmenu_list")=id1
		Response.write "" & vbcrlf & "<form name=""date""  id=""demo""  method=""post"" onsubmit=""return Validator.Validate(this,2)"" action=""menu.asp?action=save"">" & vbcrlf & "  <table width=""100%""  border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td width=""7%"" align=""right"" height=""33"">上级分类：</td>" & vbcrlf & "      <td width=""80%""><IFRAME name=""I2"" SRC=""correct.asp?ID=company"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""95%"" HEIGHT=""27"" marginwidth=""1"" marginheight=""1""></IFRAME></</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "<td width=""7%"" align=""right"" height=""33"">名称：</td>" & vbcrlf & "      <td width=""80%""><input name=""menuname"" type=""text"" id=""menuname"" value="""
		Response.write  menuname
		Response.write """ size=""20"" dataType=""Limit"" min=""1"" max=""50""  msg=""长度必须在1至50个字之间"" ></</td>" & vbcrlf & "    </tr>" & vbcrlf & "      <tr>" & vbcrlf & "      <td width=""7%"" align=""right"" height=""33"">排序：</td>" & vbcrlf & "         <td width=""80%""><input name=""gate1"" type=""text"" id=""gate1"" value="""
