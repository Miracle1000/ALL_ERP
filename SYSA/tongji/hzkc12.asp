...
'items = rpt.Settings.item(i)
	...
	Dim cplistPower, cpInfoPower, jhlistPower, jhInfoPower
	Function onReportInit(rpt, state)
		...
		rpt.addfield "借货人：","text", "jhr" ,jhr ,"" , ""
		rpt.addfield "","select", "cp" , "产品名称" & Chr(1) & "0" & Chr(2) & "产品编号" & Chr(1) & "1" & Chr(2) & "产品型号" & Chr(1) & "2","", ""
		rpt.addfield "","text", "C" , C, "", ""
		rpt.sql = "exec erp_report_jhmxbz "& session("personzbintel2007") &",@date1_0, @date1_1,@jhr,@cp,@C, &pagesize, &pageindex"
		rpt.canexcel =  app.power.existsPower(37,10)
		rpt.canprint =  app.power.existsPower(37,7)
		If state = 1 Then
			If sort<>"" Then
				rpt.headHTML = "<script defer='defer' language='javascript'>window.onjoinFieldUpdate=function(){document.getElementById('fsitem_ckitem').value='"&sort1&"';"&_
	...
	Function app_oncreate(lvw)
		...
		col.formattext = "code:ProductLinkHTML2(@cells[28],@cells[2],""@value"")"
		col.width = 100
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(4)
		col.formattext = "code:ProductBH(@cells[28],""@value"")"
		col.width = 120
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(5)
		col.formattext = "code:ProductXH(@cells[28],""@value"")"
		col.width = 120
		col.dbtype = "str"
		col.canSum = False
		setProductZdyCol2 lvw
		...
		col.formattext = "code:gljhLinkHTML(@cells[28],@cells[26],""@value"")"
		col.width = 100
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
	...
	Public Function ProductLinkHTML2(cateid, ProductId, ProductName)
		cplistPower = getqxIntro(cateid,21,1)
		If cplistPower = True Then
			cpInfoPower = getqxIntro(cateid,21,14)
		else
			cpInfoPower = False
		end if
		If cplistPower = True Then
			If cpInfoPower = True Then
				ProductLinkHTML2 = "<a target=_blank href='../product/content.asp?ord=" & app.base64.pwurl(ProductId) & "'>" &  ProductName & "</a>"
			else
	...
	Public Function getqxIntro(pord,sort1,sort2)
		Dim rsf, qx_open, qx_intro, qlsort, qxtmp
		Set rsf = cn.execute("select QL.sort,P.qx_open,P.qx_intro from qxlb Q,qxlblist QL,power P where QL.sort1 = Q.sort1 and P.sort1 = QL.sort1 and P.sort2=QL.sort2 and P.ord="& session("personzbintel2007") &" and QL.sort1="& sort1 &" and QL.sort2="& sort2)
		If Not rsf.eof Then
			qlsort = rsf(0) : qx_open = rsf(1) : qx_intro = rsf(2)
			If qlsort = 1 Then
				If qx_open = 1 Then
					qxtmp = True
			...
			ElseIf qlsort = 3 Then
				If qx_open = 3 Then
					qxtmp = True
				ElseIf qx_open = 1 Then
					If qx_intro<>"" Then
						If Instr(","& qx_intro &",",","& pord &",")>0 Then
							qxtmp = True
						else
							qxtmp = False
						end if
	...
	Function gljhLinkHTML(cateid,jhord,jhtitle)
		jhlistPower = getqxIntro(cateid,37,1)
		If jhlistPower = True Then
			jhInfoPower = getqxIntro(cateid,37,14)
		else
			jhInfoPower = False
		end if
		If jhlistPower = True Then
			If jhInfoPower = True Then
				gljhLinkHTML = "<a target=_blank href='" & app.virpath & "store/contentjh.asp?ord=" & app.base64.pwurl(jhord) & "'>" &  jhtitle & "</a>"
			else
				gljhLinkHTML = jhtitle
			end if
		else
			gljhLinkHTML=""
		end if
	end function
	Public Function setProductZdyCol2(lvw)
		Dim rsf, rsf2, k, h, db, len_rsf, arr_rsf, lvalue
		Set rsf = cn.execute("select title,name,sort,set_open,gl from zdy where sort1=21 order by gate1")
		If Not rsf.eof Then
			arr_rsf = rsf.GetRows()
		end if
		rsf.close
	...
	Function getckKu(KLku)
		Dim rsf
		If KLku<>"" then KLku = replaceTip(KLku) else KLku = ""
		If KLku = "" Then
			getckKu = ""
		else
			Set rsf = cn.execute("select K1.sort1+'-'+K.sort1 kname from sortck K left join sortck1 K1 on K.sort=K1.id where K.id="& KLku)
			getckKu = ""
			If Not rsf.eof Then
				If rsf(0)<>"" Then getckKu = rsf(0) Else getckKu = ""
			else
				getckKu = ""
			end if
			rsf.close
