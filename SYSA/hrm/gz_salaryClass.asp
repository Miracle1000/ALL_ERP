...
'if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
	...
	Response.write Application("sys.info.jsver")
	'Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "<!--" & vbcrlf & "" & vbcrlf & "    $(document).ready(function () {" & vbcrlf & "        $(""#title"").blur(function () { if ($(""#title"").val() =="""") { alert("""
	Response.write inName1
	Response.write "不能为空""); }; });" & vbcrlf & "" & vbcrlf & "    });" & vbcrlf & "    function openDisplay(id) {" & vbcrlf & "        if (id == 1) {" & vbcrlf & "            $("".ksSet"").show();" & vbcrlf & "        }" & vbcrlf & "        else {" & vbcrlf & "            $("".ksSet"").hide();" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "    }" & vbcrlf & "-->" & vbcrlf & "</script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".open{ display:inline}" & vbcrlf & ".close{ display:none}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body   "
	Response.write inName1
	if open_10_8=0 then
	...
	if open_10_13=1 then
		str_w3=" and userid in ("&intro_10_13&")"
	elseif open_10_13=3 then
		str_w3=""
	else
		str_w3="and 1=0"
	end if
	if action="update" then
		...
		salaryLi=replace(trim(SaveRequestForm("salaryLi"))," ","")
		allcansee=trim(SaveRequestForm("allcansee"))
		if title="" then
			Response.write "<script>alert('"&inName1&"不能为空');</script>"
			call db_close : Response.end()
		end if
		if salaryLi="" then
			Response.write "<script>alert('"&inName2&"不能为空');</script>"
			call db_close : Response.end()
		end if
		if allcansee=""  then
			Response.write "<script>alert('"&inName3&"不能为空');</script>"
			call db_close : Response.end()
		end if
		Set rs = conn.execute("select count(1) from sortwages where id in("& salaryLi &") and salaryClass like '%{个人所得税}%' ")
		If rs.eof = False Then
			If rs(0)>1 Then
				Response.write "<script>alert('同一账套中只允许出现一个 个人所得税项目');</script>"
				call db_close : Response.end()
			end if
		end if
		rs.close
		set rs = nothing
		Set rs = conn.execute("select count(1) from sortwages where id in("& salaryLi &") and salaryClass like '%{年终奖}%'")
		If rs.eof = False Then
			If rs(0)>1 Then
				Response.write "<script>alert('同一账套中只允许出现一个年终奖项目');</script>"
				call db_close : Response.end()
			end if
		end if
		rs.close
		set rs = nothing
		Set rs = conn.execute("select count(1) from sortwages where id in("& salaryLi &") and salaryClass like '%{年终奖所得税}%' ")
		If rs.eof = False Then
			If rs(0)>1 Then
				Response.write "<script>alert('同一账套中只允许出现一个年终奖所得税项目');</script>"
				call db_close : Response.end()
			end if
		end if
		rs.close
		...
		If allcansee=0 Then
			...
			If w2="" Then w2=0
			If w3="" Then w3=0
		ElseIf allcansee=1 Then
			set rs=conn.execute("select userid from hr_person where del=0 " & str_w3)
			while not rs.eof
				w3=w3&","&rs(0)
				rs.movenext
			wend
			rs.close
			If w3<>"" Then w3=Right(w3,Len(w3)-1)
			'w3=w3&","&rs(0)
		end if
		...
		if Dtype="add" then
			...
			call add_logs(1)
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from hr_gongziClass  where title='"&title&"'  and del=0"
			rs.open sql,conn,1,1
			if rs.eof then
				call SalaryCheckDate("hr_gongziClass",allcansee,W3,0)
				conn.execute "insert into hr_gongziClass (title,gongzi,isall,user_list,creator,inDate,del) values('"&title&"','"&salaryLi&"',"&allcansee&",'"&W3&"',"&session("personzbintel2007")&",'"&now()&"',0)"
				hr_gongziClass_id = GetIdentity("hr_gongziClass","id","creator","")
				conn.execute("update gate set salaryClass='"&hr_gongziClass_id&"' where ord in (" &W3&")")
				conn.execute("update hr_person set salaryClass='"&hr_gongziClass_id&"' where userid in (" &W3&")")
				call jsLocat2("添加成功","gz_salaryClass.asp")
			else
				call jsAlert(title&"已存在")
			end if
			rs.close
		elseif Dtype="save" and isnumeric(ord) then
			action1="工资账套修改"
			sql="select id from wages where salaryClass="&ord&" and complete1=0 and del=1"
			set Crs=conn.execute(sql)
			if not Crs.eof Then
				call jsLocat2("该工资账套有未发放工资单,暂不允许修改!","gz_salaryClass.asp")
			end if
			Crs.close
			set Crs=nothing
			call add_logs(1)
			call SalaryCheckDate("hr_gongziClass",allcansee,W3,ord)
			conn.execute "update hr_gongziClass set title='"&title&"',gongzi='"&salaryLi&"',isall="&allcansee&",user_list='"&W3&"',editTime='"&now()&"' where id="&ord&" and del=0"
			conn.execute("update gate set salaryClass=0 where salaryClass="&ord&" and ord not in (" &W3&")")
			conn.execute("update gate set salaryClass='"&ord&"' where ord in (" &W3&")")
			conn.execute("update hr_person set salaryClass=0 where salaryClass="&ord&" and userid not in (" &W3&")")
			conn.execute("update hr_person set salaryClass='"&ord&"' where userid in (" &W3&")")
			call jsLocat2("修改成功","gz_salaryClass.asp")
		end if
	elseif action="edit" and isnumeric(ord) then
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from hr_gongziClass  where id="&ord&" and del=0"
		rs.open sql,conn,1,1
		if not rs.eof then
			title=rs("title")
			gongziList=rs("gongzi")
			allcansee=rs("isall")
	...
	elseif action="del" and isnumeric(ord) then
		action1="工资账套删除"
		sql="select id from wages where salaryClass="&ord&" and complete1=0"
		set Crs=conn.execute(sql)
		if not Crs.eof Then
			call jsLocat("该工资账套有未发放工资单,暂不允许删除!","gz_salaryClass.asp")
		end if
		Crs.close
		set Crs=nothing
		call add_logs(2)
		conn.execute "update hr_gongziClass set del=3 where id="&ord&""
		call jsLocat("删除成功","gz_salaryClass.asp")
	end if
	sub SalaryCheckDate(table,allcansee,W3,id)
		hasNumPerson=""
		ThisHad=false
		if allcansee=1 then
			hasNum=conn.execute("select count(*) from "&table&" where del=0 and id<>"&id&"")(0)
			...
			if W3<>"" then
				...
				for j=0 to ubound(hasNumStr)
					if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
						hasNum=conn.execute("select count(*) from "&table&" where del=0 and (isall=1 or (isall=0 and charindex(','+cast("& hasNumStr(j) &" as varchar)+',',','+cast(user_list as varchar(8000))+',')>0)) and id<>"&id&"")(0)
'if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
						if hasNum>0 then
							ThisHad=true
							asfsfsdfsdfwer="("&getGateName(hasNumStr(j))&")"
						end if
					end if
				next
			end if
		...
		if thisHad then
			call jsAlert("同一人员"&asfsfsdfsdfwer&"不能出现在多个账套中！")
			call db_close : Response.end
		end if
	end sub
	if s_needWorkClass="" or isnull(s_needWorkClass) then s_needWorkClass=0
	Response.write "" & vbcrlf & "<iframe style='height:1px;width:1px;position:absolute;top:-100px' name='resultframe'></iframe>" & vbcrlf & "<form method=""post"" name=""demo""  target='resultframe'onsubmit=""return Validator.Validate(this,2)""  action=""gz_salaryClass.asp?action=update"" id=""demo"" >" & vbcrlf& "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place""><span class=""name"">工资账套设置</span></td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3""height=""32"" /></td>" & vbcrlf &  "       </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf
	if open_10_13=3 or (open_10_13=1 or CheckPurview(intro_10_1,trim(creator))=True)  or ((open_10_2=3 or CheckPurview(intro_10_2,trim(creator))=True) and ord<>"") then
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">工资账套设置</td>" & vbcrlf & "        </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName1
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input name=""title""  type=""text"" id=""title""  value="""
		Response.write title
		Response.write """ size=""23"" maxlength=""50""    dataType=""Limit"" min=""1"" max=""50""  msg=""1~50个字符"">" & vbcrlf & "              <span class=""red""> *</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName2
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name"">"
		set rsT=server.CreateObject("adodb.recordset")
		sqlT="select id, sort1 from sortwages order by gate1 desc ,sort1 asc "
		rsT.open sqlT,conn,1,1
		if not rsT.eof then
			do while not rsT.eof
				T_title=rsT("sort1")
				T_ord=rsT("id")
				Response.write "" & vbcrlf & "              <input name=""salaryLi"" type=""checkbox"" id=""salaryLi"" value="""
				Response.write T_ord
				Response.write """ "
				if instr(","&gongziList&",",","&T_ord&",")>0 then
			...
			Response.write "" & vbcrlf & "                                                     您还没有设置班次<a href=""workTimeSet.asp"" style=""color:#FF0000"">点击这里</a>设置" & vbcrlf & "                                                    "
		end if
		rsT.close
		set rsT=nothing
		...
		Response.write "  id=""rbtn1"" onclick=""selectAllcansee()""  min=""1""  dataType=""Limit"" msg=""必填""  />" & vbcrlf & "        <label for=""rbtn1"">所有用户调用</label>" & vbcrlf & "        <input type=""radio"" name=""allcansee"" value=""0"" id=""rbtn2"" "
		if allcansee="0" then
			Response.write " checked=""true"""
		end if
		Response.write "   onclick=""selectAllcansee()"" />" & vbcrlf & "        <label for=""rbtn2"">选择调用用户</label>" & vbcrlf & "        <span id=""ulist1"" class=""red""> *&nbsp;</span>" & vbcrlf & "                <script language=""javascript"">" & vbcrlf & "             function selectAllcansee(){" & vbcrlf & "                            if($(""#rbtn1"").attr(""checked"")){" & vbcrlf & "                                 $(""#tb1"").hide();" & vbcrlf & "                         }else if($(""#rbtn2"").attr(""checked"")){" & vbcrlf & "                                      $(""#tb1"").show();" & vbcrlf & "                                 getmoreperson();" & vbcrlf & "                                }" & vbcrlf & "                        }" & vbcrlf & "" & vbcrlf & "              function getstr(obj){" & vbcrlf & "                   //obj为对象checkbox数组;" & vbcrlf & "                       var ztstr =new Array();" & vbcrlf & "                 var ii=0;" & vbcrlf & "                       for(var i=0 ;i<obj.length;i++){" & vbcrlf & "                         if(obj[i].checked){" & vbcrlf & "                                     ztstr[ii]=obj[i].value;" & vbcrlf & "                                 ii=ii+1;" & vbcrlf & "                                 }" & vbcrlf & "                      }" & vbcrlf & "                       return ztstr.join("","");" & vbcrlf & "             }" & vbcrlf & "               function getmoreperson(){" & vbcrlf & "                       var nowstatuslist=getstr(document.getElementsByName(""ygzt""));" & vbcrlf & "                     var user_w1=getstr(document.getElementsByName(""w1""));" & vbcrlf & "                     var user_w2=getstr(document.getElementsByName(""w2""));" & vbcrlf & "                    var user_liststr="""
		Response.write user_list
		Response.write """;" & vbcrlf & "                        var user_list=getstr(document.getElementsByName(""w3""));" & vbcrlf & "                   if (user_list.length>0)" & vbcrlf & "                 {" & vbcrlf & "                               if (user_liststr.length>0)" & vbcrlf & "                              {" & vbcrlf & "                                       user_list=user_list+"",""+user_liststr;" & vbcrlf & "                             }" & vbcrlf & "                       }"& vbcrlf & "                      else" & vbcrlf & "                    {" & vbcrlf & "                               user_list=user_liststr;" & vbcrlf & "                 }" & vbcrlf & "                       $.post(""share_person_type.asp"",{nowstatus:nowstatuslist,user_w1:user_w1,user_w2:user_w2,user_list:user_list},function(result){" & vbcrlf & "                            document.getElementById(""moreperson"").innerHTML=result;" & vbcrlf & "                   })" & vbcrlf & "              }" & vbcrlf & "               </script>" & vbcrlf & "               "
		nowstatuslist=0
		...
		next
		Response.write "</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name"" valign=""top""><div id=""moreperson"" ></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">&nbsp;</td>" & vbcrlf &"            <td width=""84%""  height=""26""  class=""name"">"
		if action="edit" then
			Response.write "" & vbcrlf & "              <input name=""button22"" class=""anybutton2"" onClick=""javascript:document.all.demo.action='gz_salaryClass.asp?action=update&type=save&ord="
			Response.write pwurl(ord)
			Response.write "'""  value=""保  存"" type=""submit"">" & vbcrlf & "              "
		else
			Response.write "" & vbcrlf & "              <input name=""button23"" class=""anybutton2"" value=""保  存""  onClick=""javascript:document.all.demo.action='gz_salaryClass.asp?action=update&type=add'"" type=""submit"">" & vbcrlf & "              "
		end if
		Response.write "" & vbcrlf & "              &nbsp;&nbsp;</td>" & vbcrlf & "          </tr>        " & vbcrlf & ""
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from hr_gongziClass  where del=0"
	rs.open sql,conn,1,1
	if not rs.eof then
		k=1
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">工资账套列表</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name""><table width=""100%"" border=""0"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#C0CCDD"" >" & vbcrlf & "              <tr class=""top"">" & vbcrlf & "                <td width=""15%"" align=""center""><div align=""center"">序号</div></td>" & vbcrlf & "                <td width=""21%"" align=""center""><div align=""center"">"
		Response.write inName1
		Response.write "</div></td>" & vbcrlf & "                <td width=""20%"" align=""center""><div align=""center"">"
		Response.write inName2
		Response.write "</div></td>" & vbcrlf & "                <td width=""24%"" align=""center""><div align=""center"">"
		Response.write inName3
		Response.write "</div></td>" & vbcrlf & "                <td width=""20%"" align=""center""><div align=""center"">操作</div></td>" & vbcrlf & "              </tr>" & vbcrlf & "              "
		do while not rs.eof
			P_title=rs("title")
			P_gongzi=rs("gongzi")
			P_isall=rs("isall")
			P_user_list=rs("user_list")
			if P_gongzi<>"" and isnull(P_gongzi)=false and replace(replace(P_gongzi,",","")," ","")<>"" then
				set rs_dw=server.CreateObject("adodb.recordset")
				sql_dw="select sort1 from sortwages  where id in("&P_gongzi&")"
				rs_dw.open sql_dw,conn,1,1
				gongziStr=""
				if not rs_dw.eof then
					do while not rs_dw.eof
						gongziStr=gongziStr&"<span style='padding:4px'>"&rs_dw("sort1")&"</span>"
						rs_dw.movenext
					loop
				else
					gongziStr="无"
			...
			if P_isall=1 then
				...
				if P_user_list<>"" and isnull(P_user_list)=false and replace(replace(P_user_list,",","")," ","")<>"" then
					...
					sql_pl="select name from gate_person  where ord in("&P_user_list&")"
					rs_pl.open sql_pl,conn,1,1
					if not rs_pl.eof then
						user_listStr=""
						do while not rs_pl.eof
							user_listStr=user_listStr&"<span style='padding:4px'>"&rs_pl("name")&"</span>"
							rs_pl.movenext
						loop
					else
						user_listStr="无"
			...
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write gongziStr
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write user_listStr
			Response.write "</td>" & vbcrlf & "                <td align=""center"" class=""func"">&nbsp;" & vbcrlf & "                 "
			if open_10_2=3 or CheckPurview(intro_10_2,trim(rs("creator")))=True then
				Response.write " <input name=""button3"" class=""anybutton2"" "
				Response.write disabled
				Response.write " value=""修改"" onClick=""javascript:window.location.href='gz_salaryClass.asp?action=edit&ord="
				Response.write pwurl(ord)
				Response.write "'"" type=""button"">" & vbcrlf & "                 "
			end if
			if open_10_3=3 or CheckPurview(intro_10_3,trim(rs("creator")))=True then
				Response.write " <input name=""button32"" "
				Response.write disabled
				Response.write " class=""anybutton2"" value=""删除"" onClick=""if(confirm('确认删除？')){javascript:window.location.href='gz_salaryClass.asp?action=del&ord="
				Response.write pwurl(ord)
				Response.write "'}"" type=""button"">" & vbcrlf & ""
			end if
	...
	rs.close
	Response.write "" & vbcrlf & "      </table></form></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""1130"" ><div align=""center""></div></td>" & vbcrlf & "      </tr>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	if allcansee="1" or allcansee="" Then
	else
