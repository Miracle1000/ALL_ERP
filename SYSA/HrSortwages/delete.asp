...
'If Not rs.Eof Then
	...
	if CurrBookID<>"" and isnumeric(CurrBookID) then
		set rs=server.CreateObject("adodb.recordset")
		sql="select title from hr_gongziClass where del=0 and charindex(','+cast("&CurrBookID&" as varchar)+',',','+cast(gongzi as varchar)+',')>0 "
		set rs=server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		if not rs.eof then
			call jsBack("该工资项目在工资账套("&rs("title")&")中有关联数据，不允许删除！")
		else
			sql="Delete  sortwages where id="&CurrBookID&" "
			conn.Execute(sql)
		end if
		rs.close
		set rs=nothing
