...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_102_10=1 then
		list=" and car_addcateid in ("&intro_102_10&")"
	else
		list=" and car_addcateid=0"
	end if
	Str_Result=" where car_del=1 and a.del=1 "&list&""
	Ptr_Result=""
	if open_102_1=3 then
	elseif open_102_1=1 then
		Str_Result=Str_Result&" and car_addcateid in ("&intro_102_1&")"
	else
		Str_Result=Str_Result&" and car_addcateid=0"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
...
'if Exit Sub2<>"" then
	...
	i=1
	sql="select a.*,b.drivername,b.phone,b.mobile,b.mobile2,b.addr from O_carData a left join O_carDriver b on a.car_driver=b.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	...
	if not rs.eof then
		do until rs.eof
			...
			sql7="select * from O_carSet where id="&car_type
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				car_type=replace(rs7("setName"),"-_^_-","'")
'if not rs7.eof then
			end if
			set rs7=nothing
			car_state=rs("car_state")
			if car_state="1" then
				car_state="正常"
...
'if not rs7.eof then
			...
			if not rs1.eof then
				do until rs1.eof
					if rs1("sort")=1 and rs("car_"&rs1("name")&"")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select sort1 from sortonehy where ord="&rs("car_"&rs1("name")&"")&""
