...
'if strW3<>"" Then
	...
	response.Clear()
	dim cid,sel2,sid,fromname,sqlk,tj,strshow,i,qx_open,str3,heigt,fromtype
	tj=trim(unescape(request.QueryString("tj")))
	strshow=trim(unescape(request.QueryString("strshow")))
	vch=trim(unescape(request.QueryString("vch")))
	ch=trim(unescape(request.QueryString("ch")))
	fromtype=trim(unescape(request.QueryString("fromtype")))
	sub close_conn
		conn.close
		set conn = nothing
	end sub
	...
	w3S = 0
	Set Rs = conn.execute("select w1,w2,w3,qx_open from power2 where cateid="&session("peRsonzbintel2007")&" and sort1=2")
	If Not Rs.eof then
		w1S=Rs("w1")
		w2S=Rs("w2")
		w3S=Rs("w3")
	...
	If w3S&""="" Then w3S = 0
	IF qx_open = 3 Then
		str3  = "" 'and ord <>'"&session("peRsonzbintel2007")&"'"
	else
		str3  = "and ord in ("&w3S&") "' and ord <>'"&session("peRsonzbintel2007")&"'"
	end if
	IF tj = "bm" and strshow <> "0" Then sqlk = " and sorce='"&strshow&"'"
	IF tj = "xz" Then sqlk = " and sorce2='"&strshow&"'"
	IF tj = "gr" Then sqlk = " and ord='"&strshow&"'"
	IF tj = "upid" Then
		if fromtype<>"" then
			Set Rs = conn.execute("select sortonehyId as id,isnull(share1,'') as share1,isnull(share2,'') as share2 from DocSortsAndSharingRelation where sortonehyId='"&strshow&"'")
		else
			Set Rs = conn.execute("select id,isnull(share1,'') as share1,isnull(share2,'') as share2,addcate from document where id='"&strshow&"'")
		end if
		If Not Rs.eof then
			id=Rs("id")
			share1=Rs("share1")
	...
	IF strshow = "" Then sqlk = ""
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord, name from gate where del=1 "&sqlk&str3&" order by cateid asc,del asc,ord asc,name asc"
	rs.open sql,conn,1,1
	Response.write "" & vbcrlf & "<div style=""90%; background:#FFFFFF; border:solid 0px; color:#C0CCDD; margin-left:20px;"">" & vbcrlf & "<table width=""95%"" border=""1"" style=""margin-top:0px;""  bordercolor=""#E2E2E1"">" & vbcrlf & "  <tr class=""top"">" & vbcrlf & "    <td class=""resetGroupTableBg"" width=""91"" style=""height:27px;text-align:center;background-color: #EFF6FF;""><strong>人员</strong></td>" & vbcrlf &     "<td class=""resetGroupTableBg"" width=""55"" style=""text-align:center;background-color: #EFF6FF;""><INPUT name=""chkAll"" id=""chkAll"" title=""全选"" onClick=""ChkAllClickkAll')"" type=""checkbox"" /><strong>浏览</strong></td>" & vbcrlf & "    <td class=""resetGroupTableBg"" width=""55"" style=""text-align:center;background-color: #EFF6FF;""><INPUT name=""vchkAll"" id=""vchkAll"" title=""全选"" onClick=""ChkAllClick('vchkSon','vchkAll')"" type=""checkbox"" /><strong>下载</strong></td>" & vbcrlf & "  </tr>" & vbcrlf & "" & vbcrlf & ""
	if not rs.eof then
		do while not rs.eof
			ckd=""
			ckd2=""
			...
			IF vch <> "" or ch <> "" Then
				chIU = Split(ch, ",")
				For k = 0 To UBound(chIU)
					If int(chIU(k)) = int(rs("ord")) Then
						ckd = "checked"
						exit for
					end if
				next
				vchIU2 = Split(vch, ",")
				For k = 0 To UBound(vchIU2)
					If int(vchIU2(k)) = int(rs("ord")) Then
						ckd2 = "checked"
						exit for
					end if
				next
			end if
			Response.write "  <tr height=""25"">" & vbcrlf & "    <td align=""center""  width=""91"">"
			Response.write rs("name")
			Response.write "</td>" & vbcrlf & "    <td align=""center"" width=""55"">" & vbcrlf & "        <INPUT name=""chkSon"" id=""chkSon"" type=""checkbox""  value='"
			Response.write rs("ord")
			Response.write "' "
			Response.write ckd
			Response.write "/>" & vbcrlf & "    </td>" & vbcrlf & "    <td align=""center""  width=""55"">" & vbcrlf & "           <INPUT name=""vchkSon"" id=""vchkSon"" type=""checkbox""  value='"
			Response.write rs("ord")
			Response.write "' "
			Response.write ckd2
		...
		call close_rs
		call close_conn
		Response.write "NU001"
		Response.end
	end if
	Response.write "" & vbcrlf & "</table>" & vbcrlf & "</div>"
