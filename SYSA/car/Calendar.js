//<input name="txt" value="点击选择时间" size="19" style="color:#CCCCCC;" readonly="true" onclick="setday(this)">
var bMoveable=true;
var strFrame;        
...
strFrame+='.setcolor{color:CCCCCC;}';
strFrame+='</style>';
strFrame+='<scr' + 'ipt>';
strFrame+='var datelayerx,datelayery,bDrag;';
strFrame+='function onmousemove()';
...
var doc = document.getElementById('endDateLayer').contentWindow.document || window.frames['endDateLayer'].document;
...
var outDate="";
var bUseTime=false;
var odatelayer=doc.all;
var odatelayer=doc.all;

bImgSwitch();
odatelayer.bUseTimeLayer.innerHTML=bImg;
function setday(tt,obj,timebool)
{
...
function WriteDateTo(yy,mm,n,hh,mi,se)
{
    if (bUseTime)
    {
        outObject.value= yy + "-" + format(mm) + "-" + format(n) + " " + format(hh) + ":" + format(mi) + ":" + format(se);
		outObject.style.color="black"
    }
    else
    {
