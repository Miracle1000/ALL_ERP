...
'if strW3<>"" Then
	...
	if typeid=1 then
		...
		if selectid="" then
			Response.write"<script language=javascript>alert('您没有选择任何联系人，请选择后再删除！');history.back()</script>"
			call db_close : Response.end
		else
	...
	if open_67_3= 3 then
		Str_Result2=""
	elseif open_67_3= 1 then
		Str_Result2=" and (addcate in("&intro_67_3&")) "
	else
		Str_Result2=" and 1=2"
	end if
	G1=trim(request("G1"))
	...
	A1=trim(request("A1")) : A3=trim(request("A3"))
	T1=trim(request("T1"))
	T2=trim(request("T2"))
	C1=trim(request("C1"))
	C2=trim(request("C2"))
...
'elseif G1=6 then
	...
	sql="update  logMessage set del=2 where person=0   "&Str_Result2&" "&str_sql_checkid&""
	conn.Execute(sql)
	action1="短信发送记录删除"
	call close_list(2)
	Response.redirect "logList.asp?page_count="&page_count&"&currPage="& currpage&"&A2="&A2&"&A1="&A1&"&A3="&A3&"&person="&personid&"&company="&companyid&"&S1="&S1&"&S2="&S2&"&G1="&G1&"&G2="&G2&"&D="&D&"&q="&q&"&F1="&F1&"&F2="&F2&"&E="&E&"&F="&F&"&C1="&C1&"&C2="&C2&"&W1="&W1&"&W2="&W2&"&W3="&W3&"&P1="&P1&"&P2="&P2&"&J1="&J1&"&J2="&J2&"&ret="&m1&"&ret2="&m2&""
