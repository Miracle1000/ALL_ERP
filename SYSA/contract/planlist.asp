...
'Private Sub Class_Initialize
	...
	set rs1 = conn.execute("select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=5 and sort2=21")
	if rs1.eof then
		open_5_21=0
	else
	...
	if newstate="1" then
		...
		if rs9.eof then
			...
			if open_5_1=3 then
				Str_Result_21=""
			elseif open_5_1=1 then
				Str_Result_21=" and cateid<>0 and (cateid in ("&intro_5_1&") or (charindex(',"&session("personzbintel2007")&",',','+replace(cast(share as varchar(8000)),' ','')+',')>0 or share='1'))"
'elseif open_5_1=1 then
			else
				Str_Result_21="and 1=0"
			end if
			if rs9("fw1")="1" then
				Str_Result_21=" and (cateid="&session("personzbintel2007")&" or (charindex(',"&session("personzbintel2007")&",',','+replace(cast(share as varchar(8000)),' ','')+',')>0 or share='1'))"
'if rs9("fw1")="1" then
			end if
			tq21=rs9("tq1")
		end if
		rs9.close
		set rs9=nothing
		Str_Result=Str_Result&" and date2<='"&dateadd("d",tq21,date)&"'"
	end if
	b=request("b")
	...
	if b="3333333" or b="5555555" then
		...
		If b="3333333" Then
			Str_Result=Str_Result&" and date2<='"&dateadd("d",tq21,date)&"' and alt2 is NULL "&noalt1&""
		end if
	end if
...
'If InStr(gatety, ",2,") > 0 Then
			If ghs = True Then
				Str_Result=Str_Result+" or (addcate in("& W3 &") and addcate<>0)  "
'If ghs = True Then
			else
				ghs  = True
				Str_Result=Str_Result+" and ( (addcate in("& W3 &") and addcate<>0) "
				ghs  = True
			end if
		end if
		If ghs  = True  Then
...
'end if
	...
	if HtType<>"" then
		select case HtType
		case 0
		Str_Result=Str_Result+" and (paybacktype=0 or paybacktype is null) AND ISNULL(sort1,0) = 0"
'case 0   '
		titlename="销售"
		case 2
		Str_Result=Str_Result+" and paybacktype=1 AND ISNULL(sort1,0) = 0"
'case 2   '
		titlename="直接出库"
		case 3
...
'case 3      '
		titlename="销售开单"
		case 4
		Str_Result=Str_Result+" AND repairOrderId is not null "
'case 4      '
		titlename="维修"
		Case 6
		Str_Result=Str_Result+" AND exists(select 1 from contractlist where contract = t.ord and isnull(treeord,0)>0)  "
'Case 6
		titlename="组装合同"
		Case 5
...
'if bz<>0 then
	...
	if MinContractMoney<>"" then
		Str_Result=Str_Result+"and money1>="&MinContractMoney&" "
'if MinContractMoney<>"" then
	end if
	if MaxContractMoney<>"" then
		Str_Result=Str_Result+"and money1<="&MaxContractMoney&" "
'if MaxContractMoney<>"" then
	end if
	if m1<>"" then
...
'if m3<>"" then
	...
	if m4<>"" then
		Str_Result=Str_Result+"and date2<='"&m4&"' "
'if m4<>"" then
	end if
	if ZT<>"" then
		if zt="1" then
...
'elseif zt = "101" then
	...
	Call searchExtended(5,"t.ord")
	If Not (b="3333333" or b="5555555" or newstate="1") then
		Str_Result = Str_Result + " AND DATEADD(D,-"&interval&",date2) <= GETDATE() "
'If Not (b="3333333" or b="5555555" or newstate="1") then
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & ".orderLinke{" & vbcrlf & "   color:#2F496E" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "div#kh {height:23px;line-height:23px;}" & vbcrlf & "#kh .anybutton{*height:14px;*line-height:13px;*margin-bottom:0;*padding-top:2px}" & vbcrlf & "#ht1 #content{margin-bottom:-1px}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/ct_planlist.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body onMouseOver=""window.status='none';return true;""" & vbcrlf & ""
	if open_5_8=0 Then
		Response.write "" & vbcrlf & "      onmouseup=document.selection.empty() " & vbcrlf & "   oncontextmenu=""return false"" " & vbcrlf & "     onselectstart=""return false"" " & vbcrlf & "     ondragstart=""return false"" " & vbcrlf & "       onbeforecopy=""return false"" " & vbcrlf & "      oncopy=document.selection.empty()" & vbcrlf & ""
	end if
	Response.write " " & vbcrlf & "     >" & vbcrlf & "       <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""100%"" valign=""top"">" & vbcrlf & "                              <form action=""planlist.asp?HtType="
	Response.write HtType
	Response.write """ method=""get"" id=""demo"" " & vbcrlf & "                                      onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "                            <INPUT TYPE=""hidden"" NAME=""H"" VALUE="""
	Response.write H
	...
	Response.write """>" & vbcrlf & "                         <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td class=""place"">" & vbcrlf & ""
	if H="1" and sp1="" Then
		Response.write "合同检索"
	elseif sp1="1" Then
		Response.write "审批中"
	elseif sp1="2" Then
		Response.write "未通过"
	elseif SH="1" Then
		Response.write "我共享给他人"
	elseif SH="2" Then
		Response.write "他人共享给我"
	else
		Response.write titlename
	end if
	Response.write "" & vbcrlf & "                                                      即将到期合同" & vbcrlf & "                                            </td>" & vbcrlf & "                                           <td>&nbsp;" & vbcrlf & "                                                      <a href=""javascript:void(0)"" onClick=""Myopen(User);return false;"">" & vbcrlf & "                                                          排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0"">" & vbcrlf & "                                                   </a>"& vbcrlf & "                                               </td>" & vbcrlf & "                                           <td align=""right"">" & vbcrlf & "                                                        <div id=""kh"">" & vbcrlf & ""
	if ZBRuntime.MC(17003) and ZBRuntime.MC(17000) Then
...
'if ZBRuntime.MC(17003) and ZBRuntime.MC(17000) Then
	...
	if open_5_10=1 or open_5_10=3 Then
		Response.write "" & vbcrlf & "                                                             <input type=""button"" name=""Submitdel2"" value=""导出"" " & vbcrlf & "                                                                  onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xlsht2.asp'})}"" " & vbcrlf & "                                                                      class=""anybutton""/>" & vbcrlf & ""
	end if
	if open_5_7=1 or open_5_7=3 Then
		Response.write "" & vbcrlf & "                                                             <input type=""button"" name=""print"" " & vbcrlf & "                                                                  onclick=""javascript:kh.style.display='none';window.print();return  false;"" " & vbcrlf & "                                                                       value=""打印"" class=""anybutton""/>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "                                                             <a href=""javascript:void(0)"" class=""AfterQuickSearch"" style='*display:inline-block;*padding-bottom:3px;' " & vbcrlf & "                                                                   onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>" & vbcrlf & "                                                      </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                                      </tr>" & vbcrlf & "                                </table>" & vbcrlf & "                                <span id=""ht1""></span>" & vbcrlf & "                            </form>" & vbcrlf & "                         <form name=""form1"" method=""post"" style=""margin:0"" >" & vbcrlf & "                           <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & ""
	...
	Response.write "" & vbcrlf & "                                     <tr class=""top"">" & vbcrlf & "                                          <td align=""center"" width=""5%""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "                                            <td width=""12%"" ><div align=""center"">终止日期</div></td>" & vbcrlf & "                                            <td width=""23%"" height=""27"" align=""center""><div align=""center"">"
	Response.write khname
	Response.write "</div></td>" & vbcrlf & "                                          <td width=""10%"" ><div align=""center"">"
	Response.write khbianh
	Response.write "</div></td>" & vbcrlf & "                                          <td width=""35%"" align=""center""><div align=""center""><strong>关联合同</strong></div></td>" & vbcrlf & "                                               <td width=""10%"" ><div align=""center"">负责人</div></td>" & vbcrlf & "                                              <td width=""10%"" ><div align=""center"">" & vbcrlf & "                                                       <select name=""select5"" onChange=""if(this.selectedIndex!=0){gotourl(this.value);}"" " & vbcrlf & "                                                         style=""HEIGHT:20px;font-size:12px;font-weight:bold;color:#2F496E;border:0px solid #FFFFFF; overflow:hidden"">" & vbcrlf & "                                                              <option>-请选择-</option>" & vbcrlf & "                                                               <option value=""page_count=10"" "
	Response.write khbianh
	...
	"select a.*" & vbcrlf &_
	",num1=(select isnull(sum(num1),0)  from  contractlist  where contract=a.ord and del=1) " & vbcrlf &_
	",num2=(select isnull(sum(num2),0)  from  contractlist  where contract=a.ord and del=1) " & vbcrlf &_
	"from contract a " & vbcrlf &_
	") as t  "&Str_Result&" "&Str_Result_21&" "&fromContent&" "&querystr&" "&px_Result&""
	rs.open sql,conn,3,1
	if rs.RecordCount<=0 then
		Response.write "</table><table><tr><td>没有信息!</td></tr></table>"
	else
		n=0
...
'elseif cstr(cateid)="0" then
			...
			sql1 = "select name,khid from tel where ord="&company&""
			set rs1 = server.CreateObject("adodb.recordset")
			rs1.open sql1,conn,1,1
			if rs1.eof then
				companyname=""
			...
			rs1.close
			set rs1=nothing
			Response.write "" & vbcrlf & "                                     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                                           <td align=""center"">" & vbcrlf & "                                                       <span class=""red"">" & vbcrlf & ""
			canCancelOrder = helper.canCancelOrder(rId) And helper.isOpened
			if  open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True Then
				If canCancelOrder Then
					Response.write "" & vbcrlf & "                                                             <input name=""selectid"" type=""checkbox"" class=""reminderIds"" rid="""
					Response.write rId
					Response.write """ value="""
			...
			Response.write "" & vbcrlf & "                                                     </span>" & vbcrlf & "                                         </td>" & vbcrlf & "                                           <td align=""center""  class=""name"" style=""height:33px;text-valign:middle"">"
			Response.write """>" & vbcrlf & ""
			Response.write date2
			Response.write "</td>" & vbcrlf & "                                                <td align=""center"">" & vbcrlf & "                                                       <div align=""left"">" & vbcrlf & ""
			if open_1_14=3 or CheckPurview(intro_1_14,trim(cateid))=True Then
				Response.write "" & vbcrlf & "                                                             <a href=""javascript:void(0)"" " & vbcrlf & "                                                                     onclick=""javascript:window.open('../work/content.asp?ord="
				Response.write pwurl(company)
				Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" " & vbcrlf & "                                                                        title="""
				Response.write pwurl(company)
				Response.write rs("title")
				Response.write """>" & vbcrlf & ""
			end if
			Response.write""&companyname&""
			Response.write "" & vbcrlf & "                                                             </a>" & vbcrlf & "                                                    </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td align=""right"" class=""name""><div align=""center"">"
			Response.write khid
			Response.write "</div></td>" & vbcrlf & "                                          <td align=""left"">" & vbcrlf & ""
			if  open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True Then
				Response.write "" & vbcrlf & "                                                     <a href=""javascript:void(0)"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord="
			...
			Response.write "" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td class=""name"" >" & vbcrlf & "                                                        <div align=""center"">" & vbcrlf & ""
			catename=""
			if cateid<>"" then
				set rs7=server.CreateObject("adodb.recordset")
			...
			if  open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True Then
				If canCancelOrder Then
					Response.write "" & vbcrlf & "                                                             &nbsp;<input type=""button"" rid="""
					Response.write rId
					Response.write """ name=""Submitdel"" value=""取消提醒"" class=""anybutton cancelBtn""/>" & vbcrlf & ""
				end if
			end if
		...
		loop
		m=n
		Response.write "" & vbcrlf & "                             </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td  class=""page"">" & vbcrlf & "                                <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td width=""7%"" height=""30"">" & vbcrlf & "                                                 <divalign=""center"">" & vbcrlf & "                                                              全选<input type=""checkbox"" name=""checkbox2"" id=""checkbox2"" value=""Check All"" onClick=""mm(this.form)"">" & vbcrlf & "                                                     </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td width=""22%"" >" & vbcrlf & "                                                 <input type=""button"" id=""batchCancel"" value=""批量取消"" class=""anybutton2""/> "& vbcrlf &"                                               </td> "& vbcrlf &"                                            <td width=""71%"">" & vbcrlf &"                                                   <div align=""right""> "& vbcrlf &"                                                                共"
		...
		Response.write "页&nbsp;" & vbcrlf & "                                                             <input name=""currpage"" id=""currpage"" type=text onkeyup=""value=value.replace(/[^\d]/g,'')"" size=""3"">&nbsp;" & vbcrlf & "                                                               <input type=""button"" name=""Submit422"" value=""跳转"" " & vbcrlf & "                                                                   onclick=""gotourl('currPage='+document.getElementById('currpage').value);""  class=""anybutton2""/>" & vbcrlf & ""
		if currpage=1 Then
		...
		Response.write "" & vbcrlf & "                                                     </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "   </table>" & vbcrlf & "        </form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	action1="合同列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script src= ""../Script/ct_planlist_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:400px;left:0;top:0;height:400;display:none;"">" & vbcrlf & "<table width=""150"" height=""300""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "<tr>" & vbcrlf & "<td height=""139"">" & vbcrlf & "<table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" style=""color:#2F496E"">" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1');"" class=""orderLinke"">按添加时间排序(降)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2');"" class=""orderLinke"">按添加时间排序(升)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3');"" class=""orderLinke"">按合同主题排序(降)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4');"" class=""orderLinke"">按合同主题排序(升)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5');"" class=""orderLinke"">按合同编号排序(降)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6');"" class=""orderLinke"">按合同编号排序(升)</a></td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7');"" class=""orderLinke"">按签订日期排序(降)</a> </td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8');"" class=""orderLinke"">按签订日期排序(升)</a> </td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=9');"" class=""orderLinke"">按截止日期排序(降)</a> </td></tr>" & vbcrlf & "<tr valign=""middle""><td height=""24"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=10');"" class=""orderLinke"">按截止日期排序(升)</a> </td></tr>" & vbcrlf & "</table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<scriptsrc= ""../Script/ct_planlist_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
