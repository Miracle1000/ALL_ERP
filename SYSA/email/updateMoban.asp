...
'if strW3<>"" Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "        background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dim  ord,sortName,order1
	ord=deurl(request("ord"))
	title=trim(request("title"))
	content=trim(request("messageContent"))
	sortid=trim(request("sortid"))
	member1=request("member1")
	If member1 &"" = "" Then member1 = 0
	member2=request("member2")
	sharecontact=request("sharecontact")
	if not IsNumeric(sharecontact) or sharecontact="" Then sharecontact=0
	...
	W4=replace(W3,"0","")
	W4=replace(W4,",","")
	If member1 = "0" Then W3 = ""
	if title="" then
		Response.write"<script language=javascript>alert('标题不能为空！'); history.back();</script>"
		call db_close : Response.end
	end if
	dim checkcont
	checkcont = replace(content,"<P>","")
	checkcont = replace(checkcont,"</p>","")
	checkcont = replace(checkcont,"</p>","")
	checkcont = replace(checkcont,"</P>","")
	checkcont = replace(checkcont,"&nbsp;","")
	if checkcont="" then
	...
	if ord="" or ord=0 then
		if request("sortid")<>"" and request("messageContent")<>"" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from email_maoban where del=1 and title='"&title&"' "
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "insert into email_maoban(title,content,sort,addtime,addcate,del,shareinfo,share) values('"&title&"','"&(content)&"',"&sortid&",'"&now()&"',"&session("personzbintel2007")&",1,'"&W3&"',"& member1 &")"
				Response.write"<script language=javascript>alert('添加成功！'); window.opener.location.reload(); window.opener=null;window.open('','_self');window.close();</script>"
				call db_close : Response.end
			else
				Response.write"<script language=javascript>alert('该模板已存在！'); history.back();</script>"
				call db_close : Response.end
			end if
		else
			Response.write"<script language=javascript>alert('内容或分类不能为空！'); history.back();</script>"
			call db_close : Response.end
		end if
	else
		addcate=""
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select addcate from email_maoban where del=1 and ord="&ord&" "
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			addcate=rs1("addcate")
		end if
		...
		set rs1=nothing
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from email_maoban where del=1 and title='"&title&"'  and ord<>"&ord&" "
		rs.open sql,conn,1,1
		if  rs.eof then
			if addcate<>session("personzbintel2007") and session("top1zbintel2007")<>"1" then
				sqltext="Update email_maoban set title='"&title&"',shareinfo='"&W3&"',share="& member1 &", content=' "& content &" ',sort="&sortid&",editTime='"&now()&"',editcate="&session("personzbintel2007")&" where del=1 and  ord="&ord&""
			else
				sqltext="Update email_maoban set title='"&title&"',shareinfo='"&W3&"',share="& member1 &", content=' "& content &" ',sort="&sortid&",editTime='"&now()&"',editcate="&session("personzbintel2007")&" where del=1 and  ord="&ord&""
			end if
			conn.execute(sqltext)
			Response.write"<script language=javascript>alert('修改成功！');window.opener.location.reload();  window.opener=null;window.open('','_self');window.close();</script>"
			call db_close : Response.end
		else
			Response.write"<script language=javascript>alert('该模板已存在！'); history.back();</script>"
			call db_close : Response.end
