...
'if ZBRuntime.MC(35000) = False  And ZBRuntime.MC(18100)=false Then
	...
	Sub MessagePost(msgid)
		Select Case msgid
		Case "" : Call page_Load
		Case "onsave" : Call app_onsave
		End select
	end sub
	Sub page_load
		Dim v, n1, n2
		Response.write app.deftopbarhtml("../","","可用库存设置","")
		v = app.glAttribute("AvailStorckType")
		If Len(v) = 0 Then v = 110
		v =  clng(v)
		n1 =  int(v /100)
		n2 = int(int(v Mod 100) / 10)
		Dim scPower , cgPower, htPower, rkPower, ckPower
		scPower = app.power.existsModel(18000) And app.power.existsModel(35000)
		cgPower = app.power.existsModel(15000)
		htPower = app.power.existsModel(6000) Or app.power.existsModel(7000)
		rkPower = app.power.existsModel(17002)
		ckPower = app.power.existsModel(17003)
		Response.write "" & vbcrlf & "      <div style='border:1px solid #d0d0df;width:auto;margin:5px;padding:40px;text-align:center'>" & vbcrlf & "     可用库存计算公式 = 库存数量 + <select id='v1'>" & vbcrlf & "  <option value='0'>==在途数量==</option>" & vbcrlf & " "
'ckPower = app.power.existsModel(17003)
		If rkPower Then
			Response.write "<option value='100'"
			If n1 = 1 Then Response.write " selected"
			Response.write ">库管未确认入库</option> "
		end if
		If cgPower Then Response.write "<option value='200'" & app.iif(n1 = 2 , " selected" , "") & ">采购未入库数量</option>"
		If scPower and Application("sys.info.systemtype")<>3  Then Response.write "<option value='300'" & app.iif(n1 = 3 , " selected" , "") & ">生产在制数量(老版)</option>"
		If scPower And cgPower and Application("sys.info.systemtype")<>3  Then Response.write "<option value='400'" & app.iif(n1 = 4 , " selected" , "") & ">(采购未入库数量 + 生产在制数量(老版))</option>"
		Response.write "" & vbcrlf & "      </select> - <select id='v2'>" & vbcrlf & "    <option value='0'>==预定数量==</option>" & vbcrlf & " "
		If ckPower Then
			Response.write "<option value='10'"
			If n2 = 1 Then Response.write " selected"
			Response.write "> 库管未确认出库</option> "
		end if
		If htPower Then Response.write "<option value='20'" & app.iif(n2 = 2 , " selected" , "") & ">合同未出库数量</option>"
		If scPower and Application("sys.info.systemtype")<>3 Then Response.write "<option value='30'" & app.iif(n2 = 3 , " selected" , "") & ">生产预定数量(老版)</option>"
		If scPower And htPower and Application("sys.info.systemtype")<>3  Then Response.write "<option value='40'" & app.iif(n2 = 4 , " selected" , "") & ">(合同未出库数量 + 生产订单预定数量(老版))</option>"
		Response.write "" & vbcrlf & "      </select> &nbsp; &nbsp;<button class='oldbutton' onclick='savedata()'>保存</button>" & vbcrlf & "     </div>" & vbcrlf & "  <div class='bottomdiv'>" & vbcrlf & "        "
		if(Application("sys.info.systemtype")=3) Then
		Response.write " " & vbcrlf & "             <br><br>" & vbcrlf & "                &nbsp;&nbsp;&nbsp;&nbsp;<b>注意：</b><br>" & vbcrlf & "               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1、库存数量=产品现有库存；" & vbcrlf & "              <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、库管未确认入库=所有审批通过、无需审批的入库申请单中产品数量的总和；" & vbcrlf & "                <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3、采购未入库数量=采购单中未入库产品数量总和；" & vbcrlf & "          <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4、库管未确认出库=所有审批通过、无需审批的出库申请单中产品数量的总和；" & vbcrlf & "               <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5、合同未出库数量=合同单中未出库产品数量总和；" & vbcrlf & "          <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;备注：以上描述的都是正常状态下单据（审批通过未删除）。" & vbcrlf & "         "
	else
		Response.write "" & vbcrlf & "        <br><br>" & vbcrlf & "              &nbsp;&nbsp;&nbsp;&nbsp;<b>注意：</b><br>" & vbcrlf & "               &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1、库存数量=产品现有库存；" & vbcrlf & "              <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、库管未确认入库=所有审批通过、无需审批的入库申请单中产品数量的总和；" & vbcrlf & "               <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3、采购未入库数量=采购单中未入库产品数量总和；" & vbcrlf & "          <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;4、生产在制数量=生产订单（已审批）的核定数量 - 对应成品或半成品入库数量的和(老版)。" & vbcrlf & "               <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;5、库管未确认出库=所有审批通过、无需审批的出库申请单中产品数量的总和；" & vbcrlf & "          <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;6、合同未出库数量=合同单中未出库产品数量总和；" &vbcrlf & "         <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;7、生产预定数量= 原材料的需求数量（根据bom计算） - 对应领料出库的数量和（不算补料）(老版)。" & vbcrlf & "             <br>" & vbcrlf & "            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;备注：以上描述的都是正常状态下单据（审批通过未删除）。" & vbcrlf & "         "
	end if
	Response.write "" & vbcrlf & "      </div>" & vbcrlf & "  <script>" & vbcrlf & "                function savedata() {" & vbcrlf & "                   var vv = document.getElementById(""v1"").value*1+document.getElementById(""v2"").value*1;" & vbcrlf & "                       ajax.regEvent(""onsave"");" & vbcrlf & "                  ajax.addParam(""v"", vv);" & vbcrlf & "                   ajax.exec();" & vbcrlf & "              }" & vbcrlf & "       </script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & " "
	end sub
	Sub app_onsave
		app.glAttribute("AvailStorckType") = app.getint("v")
		Response.write  "app.Alert('保存成功');"
	end sub
