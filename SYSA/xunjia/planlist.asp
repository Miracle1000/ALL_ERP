...
'items = rpt.Settings.item(i)
	...
	Response.write "" & vbcrlf & "<style>" & vbcrlf & "        #PageSizeBox{" & vbcrlf & "           margin-top:12px;" & vbcrlf & "        }" & vbcrlf & "</style>" & vbcrlf & ""
	sub onReportInit(rpt, state)
		app.adddefaultCss
		Dim dat, rs, cpord
		dat = Split(",",",")
		rpt.title = "询价明细表"
		rpt.headHTML = "<script type=""text/JavaScript"" src=""../skin/default/js/ti_ygmxlist.js""></script>"
		rpt.pagesize = 10
		rpt.addfield "询价日期：","dates","date1","","", dat
		rpt.addfield2 "","select", "XJZT" , "暂存" & Chr(1) & "0" & Chr(2) & "询价中，待定价" & Chr(1) & "1" & Chr(2) & "询价中，部分定价" & Chr(1) & "2" & Chr(2) & "询价完毕" & Chr(1) & "3" & Chr(2) & "已终止" & Chr(1) & "4" ,"", "", "#询价状态=-1"
		'rpt.addfield "询价日期：","dates","date1","","", dat
		rpt.addfield2 "","select", "B" , "询价编号" & Chr(1) & "xjbh" & Chr(2) & "供应商" & Chr(1) & "gys" & Chr(2) & "产品名称" & Chr(1) & "cpname" & Chr(2) & "产品编号" & Chr(1) & "cporder1" & Chr(2) & "产品型号" & Chr(1) & "cptype1","", "", "询价主题="
		rpt.addfield "","text", "C" , "", "", ""
		rpt.aSearch.addField "人员选择", "gates", "cateids",""
		rpt.aSearch.addField "询价状态", "checks", "adStatus","select '暂存' name,0 value union all "&_
		"select '询价中，待定价',1 union all " &_
		"select '询价中，部分定价',2 union all " &_
		"select '询价完毕',3 union all " &_
		"select '已终止',4 "
		rpt.aSearch.addField "询价主题", "text", "xjTitle",""
		rpt.aSearch.addField "询价编号", "text", "xjid",""
		rpt.aSearch.addField "供应商", "text", "gys",""
		rpt.aSearch.addField "产品名称", "text", "proName",""
		rpt.aSearch.addField "产品编号", "text", "proOrder",""
		rpt.aSearch.addField "产品型号", "text", "proType",""
		rpt.aSearch.addField "备注", "text", "mxintro",""
		rpt.aSearch.addField "产品数量", "numbers", "xjNum",""
		rpt.aSearch.addField "交货日期", "dates", "jhdate",""
		rpt.aSearch.addField "询价日期", "dates", "xjdate",""
		rpt.onlyadSearchModel =True
		rpt.addsort "询价主题", "xjTitle"
		rpt.addsort "询价编号", "xjid"
		rpt.addsort "询价日期", "xjdate"
		rpt.addsort "询价状态", "xjStatus"
		rpt.addsort "询价人员", "xjCate"
		cpord = app.gettext("product")
		if cpord&"" <> "" then
			cpord = app.base64.deurl(cpord)
		end if
		if cpord&"" = "" then cpord = 0
		rpt.sql = "exec [erp_xunjia_mxlist] "& Info.user &",@date1_0, @date1_1,@XJZT,@B,@C ,@cateids,@adStatus,@xjTitle,@xjid,@gys,@proName,@proOrder,@proType,@mxintro,@mxzdy1,@mxzdy2,@mxzdy3,@mxzdy4,@mxzdy5,@mxzdy6,@xjNum_0,@xjNum_1,@jhdate_0,@jhdate_1,@xjdate_0,@xjdate_1, &SearchMode, &pagesize, &pageindex, &excelmode, &reportsort, "& cpord
		rpt.canexcel =  app.power.existsPower(24,10)
		rpt.canprint =  app.power.existsPower(24,7)
	end sub
	Sub app_onlistInit(lvw)
		lvw.mxzdyId=24
	end sub
	sub App_ListviewZdySet(lvw)
		lvw.InsertZdyMap "", "xjTitle", "询价主题", 140
		lvw.InsertZdyMap "xjTitle", "xjid", "询价编号", 100
		lvw.InsertZdyMap "xjid", "cateName", "询价人员", 60
		lvw.InsertZdyMap "cateName", "xjDate", "询价日期", 80
		lvw.InsertZdyMap "xjDate", "status", "询价状态", 80
		lvw.InsertZdyMap "unitall", "gysname", "供应商", 100
	end sub
	sub app_oncreate(lvw)
		...
		open_24_21 = app.power.existsPower(24,21)
		lvw.headers("id").display="none"
		With lvw.headers("xjTitle")
		.dbtype="str"
		.formattext = "code:xunjiaLinkHTML(""@value"" , rs(""xjord"") , rs(""url""), rs(""price""))"
		End With
		With lvw.headers("xjid")
		.dbtype="str"
		End With
		With lvw.headers("cateName")
		.dbtype="str"
		.align2="center"
		End With
		With lvw.headers("xjDate")
		.dbtype="str"
		.align2="center"
		End With
		...
		.dbtype="str"
		.formattext = "code:ProductLinkHTML2(rs(""addcate"") , rs(""pord""),""@value"" ,rs(""pdel""))"
		End With
		With lvw.headers("order1")
		.dbtype="str"
		.formattext = "code:ProductBH(rs(""addcate"") , ""@value"")"
		End With
		With lvw.headers("type1")
		.dbtype="str"
		.formattext = "code:ProductBH(rs(""addcate"") , ""@value"")"
		End With
		With lvw.headers("gysname")
		.width=120
		.dbtype="str"
		.formattext = "code:GysLinkHTML(rs(""gyscateid"") , rs(""gysord"") , ""@value"" ,rs(""gysdel""),rs(""toUse""))"
		End With
		With lvw.headers("num1")
		.dbtype="number"
		End With
		With lvw.headers("price1")
		.dbtype="storeprice"
		.bz="bzcode"
		If open_24_21=false Then .display ="none"
		End With
		With lvw.headers("taxRate")
		.width=60
		.formattext = "code:formatRate(""@value"")"
		End With
		With lvw.headers("priceAfterTax")
		.dbtype="storeprice"
		.bz="bzcode"
		If open_24_21=false Then .display ="none"
		End With
		With lvw.headers("discount")
		.width=60
		.dbtype="zk"
		End With
		...
		If open_24_21=false Then .display ="none"
		End With
		With lvw.headers("taxValue")
		.dbtype="money"
		.bz="bzcode"
		If open_24_21=false Then .display ="none"
		End With
		With lvw.headers("moneyAfterTax")
		.dbtype="money"
		.bz="bzcode"
		If open_24_21=false Then .display ="none"
		End With
		With lvw.headers("date2")
		.dbtype="str"
		End With
		...
		.dbtype="str"
		End With
		lvw.headers("url").display="none"
		lvw.headers("xjord").display="none"
		lvw.headers("price").display="none"
		lvw.headers("toUse").display="none"
		lvw.headers("bzcode").display="none"
		lvw.headers("pord").display="none"
		lvw.headers("addcate").display="none"
		lvw.headers("gysord").display="none"
		lvw.headers("gyscateid").display="none"
		lvw.headers("pdel").display="none"
		lvw.headers("gysdel").display="none"
	end sub
	Function xunjiaLinkHTML(title , ord,url, price)
		If url="_url" then
			If price > 0 Then
				xunjiaLinkHTML = "<a href=""javascript:void(0);"" onclick=""javascript:window.open('../xunjia/content.asp?ord=" & app.base64.pwurl(ord) & "','ygcon','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  title & "</a>"
'If price > 0 Then
			else
				xunjiaLinkHTML = "<a href=""javascript:void(0);"" onclick=""javascript:window.open('../xunjia/content2.asp?ord=" & app.base64.pwurl(ord) & "','ygcon','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  title & "</a>"
'If price > 0 Then
			end if
		else
			xunjiaLinkHTML = title
		end if
	end function
	Dim cplistPower ,cpInfoPower , gyslistPower ,gysInfoPower
	Function ProductLinkHTML2(cateid,ProductId, ProductName ,del)
		If ProductId&"" = "" Or ProductId&""="0" Then
			ProductLinkHTML2 =" <span style='color:red'>已彻底删除</span>"
		else
			If isEmpty(cplistPower) Then cplistPower = app.power.GetPowerIntro(21,1)
			if cplistPower="" or instr(","& cplistPower &"," , ","& cateid &",")>0 then
				If isEmpty(cpInfoPower) Then
					cpInfoPower = powerIntro(21,14)
				end if
				if cpInfoPower="" And del=1 then
					ProductLinkHTML2 = "<a href=""javascript:;"" onclick=""javascript:window.open('../product/content.asp?ord=" & app.base64.pwurl(ProductId) & "','productcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  ProductName& "</a>"
				else
					ProductLinkHTML2 = ProductName & app.iif(del&""="2" ," <span style='color:red'>(已删除)</span>","")
				end if
			else
				ProductLinkHTML2=""
			end if
	...
	Function ProductBH(cateid, ProductStr)
	...
	Function GysLinkHTML(cateid,gysord,gystitle, del, toUse)
		Dim useStr : useStr = ""
		If toUse&""="1" Then
			useStr = " <span style='color:#cc3333'>选用</span>"
		ElseIf toUse&""="2" Then
			useStr = " <span style='color:#cc3333'>备用</span>"
		end if
		If del=0 And gysord>0 Then
			GysLinkHTML = "<span style='color:red'>已彻底删除</span> "& useStr
			Exit Function
		end if
		...
		if gyslistPower="" or instr(","& gyslistPower &"," , ","& cateid &",")>0 then
			...
			if (gysInfoPower="" or instr(","& gysInfoPower &"," , ","& cateid &",")>0) And del=1 then
				GysLinkHTML = "<a href=""javascript:;"" onclick=""javascript:window.open('../work2/content.asp?ord=" & app.base64.pwurl(gysord) & "','qccon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  gystitle & "</a>"& useStr
'if (gysInfoPower="" or instr(","& gysInfoPower &"," , ","& cateid &",")>0) And del=1 then
			else
				GysLinkHTML = gystitle & app.iif(del="2" ," <span style='color:red'>(已删除)</span>","")& useStr
			end if
		else
			GysLinkHTML=""& useStr
		end if
	...
	Function formatRate(rate)
		formatRate = FormatNumber(rate&"",Info.moneyNumber,-1)&"%"
'Function formatRate(rate)
	end function
	Function powerIntro(sort1,sort2)
	...
	Function formatNum(numf,dot_num)
		if numf&""<>"" then
			if numf>1 then
	...
	Sub App_OnListviewExcel(lvw)
		...
		open_24_21 = app.power.existsPower(24,21)
		lvw.headers("id").display="none"
		With lvw.headers("btitle")
		.title="预购主题"
		.dbtype="str"
		.formula = "yugouLinkHTML(@value , @cells[""ygord""] , @cells[""url""])"
		End With
		With lvw.headers("ygid")
		.title="预购编号"
		.dbtype="str"
		End With
		With lvw.headers("xjzt")
		.title="询价状态"
		.formula = "xunjiazt(@value)"
		End With
		With lvw.headers("zt1")
		.title="采购状态"
		.formula = "caigouzt(@value)"
		End With
		With lvw.headers("title")
		.dbtype="str"
		.formula = "ProductLinkHTML2(@cells[""addcate""] , @cells[""pord""],@value ,@cells[""pdel""])"
		End With
		With lvw.headers("order1")
		.width=100
		.dbtype="str"
		.formula = "ProductBH(@cells[""addcate""] , @value)"
		End With
		With lvw.headers("type1")
		.width=100
		.dbtype="str"
		...
		.width=120
		.dbtype="str"
		.formula = "GysLinkHTML(@cells[""gyscateid""] , @cells[""gysord""] , @value , @cells[""gysdel""],@cells[""toUse""])"
		End With
		With lvw.headers("num1")
		.width=100
		.dbtype="number"
