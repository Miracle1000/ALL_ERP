...
'If Right(s,1) = "," Then
	...
	Dim defexpandvalue
	Function getDefExpand()
		If Len(defexpandvalue & "") = 0 Then
			Dim rs
			Set rs = cn.execute("select count(1) from sortck with(nolock) where del=1 and (intro like '0' or ',' + cast(intro as varchar(8000)) + ',' like '%," & Info.user & ",%')")
'Dim rs
			defexpandvalue = rs.fields(0).value  < 20
			rs.close
		end if
		getDefExpand = defexpandvalue
	end function
	...
	Sub Page_Load
		Dim sql, rs, ckTagname, tt, ckSortTagName, pid, unit
		Dim setmodel, clskeyword
		setmodel = app.getint("setmodel")
		...
		ckTagname="仓库"
		ckSortTagName="仓库分类"
		set rs=cn.execute("select intro from setjm3 with(nolock) where ord=5431")
		if Not rs.eof then
			ckTagname=rs.fields(0).value
		end if
		rs.close
		set rs=cn.execute("select intro from setjm3 with(nolock) where ord=5432")
		if Not rs.eof then
			ckSortTagName=rs.fields(0).value
		end if
		rs.close
		tt=ckTagname &"选择"
		Response.write "" & vbcrlf & "<style>" & vbcrlf & "       html{scrollbar-3dlight-color:#d0d0e8;scrollbar-highlight-color:#fff;scrollbar-face-color:#f0f0ff;scrollbar-arrow-color:#c0c0e8;scrollbar-shadow-color:#d0d0e8;scrollbar-darkshadow-color:#fff;scrollbar-base-color:#ffffff;scrollbar-track-color:#fff;}" & vbcrlf & " a {color:#000}" & vbcrlf & "  td.menutab {padding: 3px 0 0 6px;height:34px;font-weight:bold; border-top:1px solid #ccc;border-bottom:1px solid #ccc;}" & vbcrlf & " td.menutab_hover {padding: 3px 0 0 20px;height:23px;font-weight:bold;}" & vbcrlf & "  td.menutab_hover a{color:#000000;}" & vbcrlf & ".tvw_txt {color:#000;}" & vbcrlf & "  #txtKeywords" & vbcrlf & "    {width:100px;border:1px solid #999;height:24px;_height:24px;line-height:24px;font-size:12px;color:#999999;text-align:center;border:1px solid #CCC}" & vbcrlf & "</style>" & vbcrlf & "<body onclick=""try{parent.document.getElementById('adsIF').style.height=document.body.offsetHeight+'px';}catch(e){}"" style='overflow:hidden'>" & vbcrlf & ""
		tt=ckTagname &"选择"
		If setmodel = 0 and request.querystring("notitle")<>"1" then
			Response.write "" & vbcrlf & "<table class=""resetHeadBg""  width='100%' border='0' cellspacing='0' cellpadding='0' height='27' background='../images/m_table_top2.jpg'>" & vbcrlf & "        <tr>" & vbcrlf & "    <td class=""reseetTextColor"" style='padding-left:10px;font-weight:bold;color:#000'>"
'If setmodel = 0 and request.querystring("notitle")<>"1" then
			Response.write tt
			Response.write "</td>" & vbcrlf & " <td align='right' style=""padding-right:6px""><input type=""button"" class=""oldbutton"" onclick=""parent.adClose();if(parent.showBG){parent.showBG(0);}"" value=""关闭""></td>" & vbcrlf & "     </tr>" & vbcrlf & "</table>" & vbcrlf & ""
			Response.write tt
		end if
		Response.write "" & vbcrlf & "<table width='100%' border='0' cellpadding='0' cellspacing='0'>" & vbcrlf & "<tr>" & vbcrlf & "<td valign=""center"" id='leftmenuall'>" & vbcrlf & "        <table width=""100%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0""> " & vbcrlf & "           <tr style='"
		Response.write app.iif(setmodel = 0,"","display:none'")
		Response.write "' >" & vbcrlf & "                   <td height=""27"" background=""../images/topb1g.gif"">" & vbcrlf & "                          <table border=""0"" cellpadding=""0"" cellspacing=""0"" width=""100%"">" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td width=""100%"" height=""27"" align=""left"" style=""padding-left:5px"" valign=""middle"">" & vbcrlf & "                                                      "
		Response.write ckTagname
		Response.write "检索:<input name=""C"" value="""
		Dim tmv : tmv = Replace(app.gettext("keytext"),"""","")
		If tmv = "" Then tmv = "输入按回车检索"
		Response.write tmv
		Response.write """ id=""txtKeywords"" type=""text"" onblur=""keyTextBlur(this)"" onfocus=""keyTextFocus(this)""  onkeyup=""keyTextKeyUp(this)"" maxlength=100 onkeydown='if(window.event.keyCode==13){doSearch();event.cancelBubble=true;return false;}'/>" & vbcrlf & "                                              </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                         </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td class=""menutab"">" & vbcrlf & "                              <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td width=""30%""  height=""27""><a href=""#"" onclick=""return false;"" class=""m_l"">"
		Response.write ckSortTagname
		Response.write "</a></td>" & vbcrlf & "                                             <td width=""70%"" align=""right"">" & vbcrlf & "                                                      <a href=""javascript:void(0);"" onclick='showAllStore();"
		Response.write app.iif(setmodel=1,"clearTopKeybox()","")
		Response.write "'>所有"
		Response.write ckTagname
		Response.write "</a>" & vbcrlf & "                                                  <a href=""javascript:void(0);"" onclick='expTree(this)'>全部"
		Response.write app.iif(getDefExpand(), "收缩", "展开")
		Response.write "</a>&nbsp;" & vbcrlf & "                                            </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <div id=""allStore"" style=""_width:198px;height:"
		Response.write app.iif(setmodel=1,520,400)
		Response.write "px;overflow:auto;margin-right:2px;margin-left:3px;_margin-left:0px;position:relative"">" & vbcrlf & "     "
		Response.write app.iif(setmodel=1,520,400)
		Call showStoreCls(0, Nothing)
		Response.write "" & vbcrlf & "      </div>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "<input type='hidden' id='pid' value='"
		Response.write app.getint("pid")
		Response.write "'>" & vbcrlf & "<input type='hidden' id='unit' value='"
		Response.write app.getint("unit")
		Response.write "'>" & vbcrlf & "<input type='hidden' id='setmodel' value='"
		Response.write app.getint("setmodel")
		Response.write "'> <!-- setmodel=1返回无权限的仓库  -->" & vbcrlf & "<input type='hidden' id='fixName' value='"
		Response.write app.getint("setmodel")
		Response.write app.gettext("robj")
		Response.write "'> <!-- 当编辑某一行的仓库时候，该值表示行的标识  -->" & vbcrlf & "<input type='hidden' id='currvalue' value='"
		Response.write app.gettext("robj")
		Response.write app.getint("sid")
		Response.write "'>    <!-- 表示当前选中值 sid命名继承以前代码中的 -->" & vbcrlf & "<input type='hidden' id='extvalue' value='"
		Response.write app.getint("sid")
		Response.write app.gettext("ext")
		Response.write "'>    <!-- 需排除的值 -->"
		Response.write app.gettext("ext")
		If app.getint("oldmodel") = 1 then
			Response.write "" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & " try{parent.document.getElementById('adsIF').style.height='485px';}catch(e){}" & vbcrlf & "</script>" & vbcrlf & ""
		end if
	...
	Sub showStoreCls(ByVal pid, ByRef pnd)
		Dim tvw , doup, setmodel, extValue, explan ,rs
		Dim keytxt, productid, unit, i, item
		extvalue = app.gettext("ext")
		If Len(extvalue) = 0 Then extvalue = app.gettext("extvalue")
		explan = (app.gettext("explan") <> "0")
		If Len(app.gettext("explan")) = 0 Then
			explan = getDefExpand()
		end if
		Dim maxWidth
		Set rs = cn.execute("select  max(widthNum) from (select len(sort1)*12+(depth+2)*18+10 as widthNum from sortck1 with(nolock) union all select len(ck.sort1)*12+(depth+3)*18+10 as widthNum from sortck ck with(nolock) inner join sortck1 ck1 with(nolock) on ck.sort = ck1.id) a")
'Dim maxWidth
		If rs.eof =False Then
			maxWidth = rs(0)
		end if
		...
		productid = app.getint("pid")
		unit = app.getint("unit")
		Set tvw = New treeview
		tvw.id = "Storetree"
		tvw.pagesize = 15
		tvw.pagedataemodel = "all"
		If maxWidth>150 Then tvw.stylecss="width:"&maxWidth&"px"
		tvw.defExplan = explan
		Dim exitcklist
		If Len (extValue) > 0 Then
			extValue = Split(extValue,";")
			For i = 0 To ubound(extValue)
				item = Split(extvalue(i),",")
				If isnumeric(item(0)) Then
					If Len(exitcklist) > 0 Then
						exitcklist = exitcklist & ("," & item(0))
					else
						exitcklist = item(0)
					end if
				end if
		...
		Call tvw.addAllNodes(tvw.nodes, "exec erp_selbox_createStoreNode " & Info.User & "," & productid & "," & unit & ",'" & Replace(keytxt,"'","''") & "',@parentid,@pagesize,@pageindex," & setmodel & ",'" & exitcklist & "'", explan, 1, pid)
		Response.write tvw.HTML
	end sub
	Sub App_sys_onAddAllNode(rs, nodes, node)
		Dim setmodel, ico
		setmodel = app.getint("setmodel")
		ico = rs.fields("ico").value
		If setmodel = 1 Then
			If ico = 1 Then
				node.value = rs.fields("n_value").value & ""
				node.canselect = true
			else
				node.value = ""
