...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	rs1.close
	set rs1=nothing
	if open_101_10=3 then
		list=""
	elseif open_101_10=1 then
		list="and reg_addcateid in ("&intro_101_10&")"
	else
	...
	if open_101_1=3 then
	elseif open_101_1=1 then
		Str_Result=Str_Result&" and reg_addcateid in ("&intro_101_1&")"
	else
		Str_Result=Str_Result&" and reg_addcateid=0"
	end if
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by reg_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by reg_addtime asc"
	elseif px="3" then
	...
	j=1
	k=1
	sql="select * from O_kuin "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "用品入库列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			reg_name=rs("reg_name")
			reg_time=rs("reg_time")
			reg_note=rs("reg_note")
			reg_userid=rs("reg_userid")
			reg_addtime=rs("reg_addtime")
			reg_addcateid=rs("reg_addcateid")
			if reg_userid="" or isnull(reg_userid) then reg_userid=0
			sql7="select * from gate where ord="&reg_userid
			...
			rs7.close
			set rs7=nothing
			sql7="select * from gate where ord="&rs("reg_addcateid")
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				reg_addcateid=rs7("name")
			end if
			rs7.close
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).Value =reg_name
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).Value =reg_userid
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).Value = reg_time
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).Value = reg_addcateid
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).Value = reg_addtime
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,6).Value = reg_note
			set rs7=nothing
			xlWorksheet.Cells(1+i,6).font.Size=10
			set rs7=nothing
			sql8="select a.*,b.reg_name,b.reg_addcateid,b.reg_userid,b.reg_time,c.pro_name,d.sort1,e.name,f.prod_name,f.prod_bh,f.prod_xh,k.sto_name from O_kuinList a "&_
			...
			" left join gate e on b.reg_userid=e.ord where a.reg_fid="&rs("id")
			set rs8=server.CreateObject("adodb.recordset")
			rs8.open sql8,conn,1,1
			if not rs8.eof then
				do while not rs8.eof
					prod_name=rs8("prod_name")
					prod_bh=rs8("prod_bh")
					prod_xh=rs8("prod_xh")
					sort1=rs8("sort1")
					prod_num=zbcdbl(rs8("prod_allnum"))
					prod_price=zbcdbl(rs8("prod_price"))
					prod_allmoney=zbcdbl(rs8("prod_allmoney"))
					if prod_price="" or isnull(prod_price) then prod_price=0
					if prod_allmoney="" or isnull(prod_allmoney) then prod_allmoney=0
					sto_name=rs8("sto_name")
					reg_gztime=rs8("reg_gztime")
					prod_gys=rs8("prod_gys")
					prod_ph=rs8("prod_ph")
					prod_xlh=rs8("prod_xlh")
					prod_sctime=rs8("prod_sctime")
					prod_ystime=rs8("prod_ystime")
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,1).Value = "【用品明细】"
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,2).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,7).Value = prod_name
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,7).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,8).Value =prod_bh
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,8).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,9).Value = prod_xh
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,9).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,10).Value =sort1
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,10).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,11).Value =prod_num
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,11).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,12).Value =Formatnumber(prod_price,CommPrice_dot_num,-1)
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,12).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,13).Value =Formatnumber(prod_allmoney,num_dot_xs,-1)
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,13).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,14).Value =sto_name
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,14).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,15).Value =reg_gztime
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,15).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,16).Value =prod_gys
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,16).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,17).Value =prod_ph
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,17).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,18).Value =prod_xlh
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,18).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,19).Value =prod_sctime
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,19).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,20).Value =prod_ystime
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,20).font.Size=10
					reg_intro=rs8("reg_intro")
					xlWorksheet.Cells(2+i,21).Value =reg_intro
					reg_intro=rs8("reg_intro")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>入库列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!用品入库列表导出成功,共有"&(i-1)&"条记录!"&(j-1)&"条记录"&(k-1)&"条明细!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("用品入库列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的入库列表</u></strong></font></a></p>"
