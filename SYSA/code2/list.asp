...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(byval rpt, byval  state)
		rpt.title = "二维码列表"
		Dim clsDefV,category
		clsDefV = 0
		category = Request.QueryString("category")
		If category <> "" Then clsDefV = category
		Dim ShowTitle : ShowTitle = " where isnull(fromSys,0) <> 2 or (fromSys = 2 and title not in (" & UnIncType() & ")) "
		Call rpt.AddField2("选择分类：", "select","cls",  rpt.getselDataBySql("select title + (case stop when 0 then '' else ' (已停用)' end), id from C2_CodeTypes " & ShowTitle & " order by stop,gate1 desc"), "", ""& clsDefV &"", "=所有分类=" )
		Call rpt.AddField2("条件：","select", "wtype", rpt.getselDataBytxt("主题=1,添加人=2"), "", "", "null")
		Call rpt.AddField("","text", "wtext", "", "", "")
		Call app.addDefaultScript()
		rpt.BillListUIModel = True
		Dim handleStr : handleStr = ""
		If app.power.existsPower(106,3) Then  handleStr= "批量删除"
		Dim canPrint : canPrint = False
		If app.power.existsPower(106,7) Then
			canPrint = True
			If len(handleStr)>0 Then handleStr = handleStr & ","
			handleStr = handleStr &"批量打印"
		end if
		rpt.canExcel = app.power.existsPower(106,10)
		rpt.canPrint = canPrint
		rpt.batbuttons = handleStr
		rpt.sql = "exec erp_qrcode_list " & Info.User & ", @cls, @wtype, @wtext, &excelmode,'" & Replace(UnIncType(),"'","") & "'  , &SearchMode ,&excelmode,&pagesize ,&pageindex, &ReportSort"
	end sub
	sub app_oncreate(byval lvw)
		lvw.checkbox = True
		lvw.checkvalue = "操作"
		lvw.headers("序号").Formula="rowindex"
		lvw.headers("序号").width = 60
		lvw.headers("操作").formattext = "code:handleHtml(@value)"
		lvw.headers("addcate").display = "none"
	end sub
	Sub lvw_onCreateCheckBox(lvw, rs, html)
		If Not app.power.existsPowerIntro(106,3,rs("addcate")) Then html = ""
	end sub
	Function handleHtml(data)
		...
		id = data(0)
		addcate=data(1)
		If app.power.existsPowerIntro(106,14,addcate) then
			html = "<button class='oldbutton2' onclick='app.PageOpen(""add.asp?ord=" & app.base64.pwurl(id) & "&onlyread=1"",1100,640)'>详情</button>&nbsp;"
		end if
		If app.power.existsPowerIntro(106,2,addcate) then
			html = html & "<button onclick='app.PageOpen(""add.asp?ord="& app.base64.pwurl(id) & """,1100,640)' class='oldbutton2'>修改</button>&nbsp;"
		end if
		If app.power.existsPowerIntro(106,3,addcate) then
			html = html & "<button class='oldbutton2' onclick='if(window.confirm(""确定要删除吗？"")){ajax.regEvent(""delItem"");ajax.addParam(""id""," & id & ");ajax.exec()}'>删除</button>"
		end if
		handleHtml = html
	end function
	Sub App_delItem
		...
		Response.write "window.DoRefresh()"
	end sub
	Sub App_doBatHandle(cmd, checkvalues, batResult)
		Select Case cmd
		Case "批量删除":
		Call doBatDelete(checkvalues, batResult)
		End select
	end sub
	Sub doBatDelete(ByVal checkvalues, ByVal batResult)
		Dim s, i, c, nitems
		s = Split(checkvalues,",")
		c = (ubound(s)+1)/2-1
		's = Split(checkvalues,",")
		...
		For i = 0 To c
			nitems(i) = s(i*2)
		next
		checkvalues = Join(nitems, ",")
		If Len(checkvalues) > 0 then
			cn.execute "update C2_CodeItems set del=2, delcate=" & Info.user & ", deltime=getdate() where id in (" & checkvalues & ")"
		end if
	end sub
