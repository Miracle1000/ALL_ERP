...
'if strW3<>"" Then
	...
	elseif px=7 then
		px_Result1=" order by a.reason desc,id desc"
		px_Result2=" order by a.reason asc,id asc"
	elseif px=8 then
		px_Result1=" order by a.reason asc,id asc"
		px_Result2=" order by a.reason desc,id desc"
	end if
	Dim ord,conditions,contexts,time1,time2,cateid
	ord=request("ord")
	...
	If Len(cateid)>0 Then
		states= states & " and precateid in("&cateid&")"
	end if
	sort=getint(request("sort"))
	sort1=getint(request("sort1"))
	If sort>0 Then
		states= states & " and sort="&sort
		If sort1>0 Then
			states= states & " and sort1="&sort1
		end if
	end if
	reasonchildren=request("reasonchildren")
	If isnumeric(reasonchildren) And reasonchildren<>"" Then
		states= states & " and reason=4 and isnull(reasonchildren,0)="&reasonchildren
	end if
	...
	If Len(contexts&"")>0 then
		...
		Case 1:
		Str_Result8=Str_Result8&" and cateid in(select ord from gate where charindex('" & contexts & "',name)>0) "
		Case 2:
	...
	If sflg="1" Then
		...
		reasonc=request("reasonc")
		If Len(reasonc&"")>0 Then Str_Result8=Str_Result8 & " and reasonchildren in (" & reasonc & ") "
		W1=request("W1")
		W2=request("W2")
		W3=request("W3")
		W3=getW3(W1,W2,W3)
		...
		If Len(w4&"")>0 Then
			Str_Result8=Str_Result8 & " and cateid in(" & w4 & ")"
		end if
	end if
	set rs1=server.CreateObject("adodb.recordset")
...
'ElseIf open_1_10=1 Then
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("销售人员变更表")
	If ord>0 then
		xsheet.showHeader "操作时间,变更前人员,变更后人员,变更前阶段,变更后阶段,类型,备注,操作人,领用时间,添加时间"
	else
		xsheet.showHeader "操作时间,变更前人员,变更后人员,变更前阶段,变更后阶段,类型,备注,操作人,关联客户,领用时间,添加时间"
	end if
	xsheet.movenext
	i=0
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select tord as tord_ID,cateid as cateid_ID,date7 as 操作时间,isnull((select name from gate where ord=a.precateid),'') as 变更前人员, " & _
	"  isnull((select name from gate where ord=a.newcateid),'') as 变更后人员," & _
	"  isnull((select sort1 from sort4 where ord=a.sort),'')+'/'+isnull((select sort2 from sort5 where ord=a.sort1),'') as 变更前阶段," & _
	"  isnull((select sort1 from sort4 where ord=a.sort),'')+'/'+isnull((select sort2 from sort5 where ord=a.sort1),'') as 变更后阶段," & _
	"  isnull(reason,0) as 类型," & _
	"  isnull(intro,'') as 备注," & _
	"  isnull((select name from gate where ord=a.cateid),'') as 操作人," & _
	"tname" &date2& " as 领用时间," & _
	"isnull((select date1 from tel where ord=a.tord),'') as 添加时间" & _
	"   from tel_sales_change_log a where 1=1 " & strCondition2 & px_Result1
	rs1.open sql1,conn,1,1
	C1 = rs1.recordCount
	if rs1.eof=True Then
		...
		Do While Not rs1.eof
			...
			xsheet.writehtml "<td width='100' align='center'>"&rs1("变更前人员")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&rs1("变更后人员")&"</td>"
			xsheet.writehtml "<td width='150' align='center'>"&rs1("变更前阶段")&"</td>"
			xsheet.writehtml "<td width='150' align='center'>"&rs1("变更后阶段")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&sal_reason(rs1("类型"))&"</td>"
			xsheet.writehtml "<td width='150' align='center'>"&rs1("备注")&"</td>"
			If Len(rs1("操作人").value&"")=0 And rs1("cateid_ID").value>=0 Then
				If rs1("cateid_ID").value>0 Then
					xsheet.writehtml "<td width='100' align='center'>用户已删除</td>"
				ElseIf rs1("cateid_ID").value=0 then
	...
	rs1.close
	set rs1=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>销售人员变更表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!销售人员变更表"&i&"条导出成功!</B>"";</script>"
	tfile=Server.MapPath("销售人员变更表_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.Dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'销售人员变更表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="销售人员变更表导出"
	call close_list(1)
	Function sal_reason(idx)
		Dim v
		Select Case idx
		Case 1 : v="添加"
		Case 2 : v="导入"
	...
	Response.write """><font class=""red""><strong><u>下载导出的销售人员变更表</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
