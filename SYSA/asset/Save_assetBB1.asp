...
'if strW3<>"" Then
	...
	tp=request("tp")
	wr_assID=request("wr_assID")
	wr_working=request("wr_working")
	wr_assIDarr=split(wr_assID,", ")
	wr_workingarr=split(wr_working,", ")
	for i=0 to ubound(wr_assIDarr)
		if ml.IsNum(wr_workingarr(i))<>0 and ml.IsNum(wr_assIDarr(i))<>0 then
			sql="select * from O_assWork where 1=2"
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,3
			rs.addnew
			rs("wr_assID")=ml.IsNum(wr_assIDarr(i))
			rs("wr_working")=ml.IsNum(wr_workingarr(i))
			sql7="select ass_ycycle from O_asset where ass_id="&ml.IsNum(wr_assIDarr(i))
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				zdy3=ml.IsNum(rs7("ass_ycycle"))
			end if
			rs7.close
			set rs7=nothing
			rs("zdy3")=ml.IsNum(zdy3)+1
			set rs7=nothing
			rs("zdy2")=1
			rs("zdy1")=session("personzbintel2007")
			rs("zdy5")=now
			rs.update
			rs.close
			set rs=nothing
	...
	next
	Response.write "<script>window.location='List_assetBB1.asp';</script>"
	call db_close : Response.end()
