...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if px="1" then
		Ptr_Result=" order by id desc"
	elseif px="2" then
		Ptr_Result=" order by id asc"
	elseif px="3" then
		Ptr_Result=" order by reg_time desc"
	elseif px="4" then
		Ptr_Result=" order by reg_time asc"
	elseif px="5" then
		Ptr_Result=" order by (select name from gate where ord = reg_userid) desc"
	elseif px="6" then
		Ptr_Result=" order by (select name from gate where ord = reg_userid) asc"
	end if
	ret          =request("ret")
	ret2 =request("ret2")
	A    =request("A")
	...
	C    =request("C")
	if ret<>"" then
		Str_Result=Str_Result&" and datediff(dd,'"& ret &"',reg_time)>=0"
	end if
	if ret2<>"" then
		Str_Result=Str_Result&" and datediff(dd,reg_time,'"& ret2 &"')>=0"
	end if
	if A<>"" then
	end if
	if C<>"" then
		if B="djzt" then
			Str_Result=Str_Result&" and reg_name like '%"&C&"%'"
		elseif B="ypmc" then
			Str_Result=Str_Result&" and reg_userid  in (select ord from gate where name like '%"&C&"%')"
		elseif B="djry" then
			Str_Result=Str_Result&" and reg_note  like '%"&C&"%'"
		end if
	end if''
	...
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "<form name=""data"" id=""data"" action=""kuinList.asp"" method=""get"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">入库列表</td>" & vbcrlf & "                      <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"" class=""top_btns""><span id=""kh"">自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" style=""width:80px"" name=""ret"" value="""
	Response.write ret
	Response.write """/><DIV id=daysOfMonth style=""POSITION: absolute""></DIV> 至：<input id=""daysOfMonth2Pos"" style=""width:80px"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9"" name=""ret2"" value="""
	Response.write ret2
	Response.write """/><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;<select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                              <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""djzt"" "
	Response.write ret2
	if B="djzt" then
		Response.write "selected"
	end if
	Response.write ">入库主题</option>" & vbcrlf & "                            <option value=""ypmc"" "
	if B="ypmc" then
		Response.write "selected"
	end if
	Response.write ">入库人员</option>" & vbcrlf & "                            <option value=""djry"" "
	if B="djry" then
		Response.write "selected"
	end if
	Response.write ">入库备注</option>" & vbcrlf & "                    </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" size=""10"" value="""
	Response.write C
	Response.write """  style='height:20px;line-height:20px;'><input type=""hidden"" name=""page_count"" size=""10"" value="""
	Response.write C
	Response.write page_count
	Response.write """><input type=""hidden"" name=""px"" size=""10"" value="""
	Response.write px
	...
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "<input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_kuin.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
	...
	Response.write "" & vbcrlf & "                       <a href=""javascript:;"" onClick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""></a></span></td>" & vbcrlf & "           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">      " & vbcrlf & "                 <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "               <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库主题</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库人员</div></td>" & vbcrlf & "                 <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库时间</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">添加人员</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">添加时间</div></td>" & vbcrlf & "                   <td align=""left"" valign=""middle""  class=""name""><div align=""center"">入库备注</div></td>" & vbcrlf & "                  <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                        <option>-请选择-</option>" & vbcrlf & "                   <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from O_kuin "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			del_bool=true
			sqltext="select * from O_proOutList where prod_id in (select id from O_kuinList where reg_fid="&rs7("id")&") "
			...
			rstxt.close
			set rstxt=nothing
			sqltext="select * from O_kuCheList where prod_id in (select id from O_kuinList where reg_fid="&rs7("id")&") "
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				del_bool=false
			end if
			...
			Response.write "</td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                     "
			...
			rs8.close
			set rs8=nothing
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("reg_time")
			Response.write "</div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""func"">" & vbcrlf & "               <div align=""center"">" & vbcrlf & "                "
			reg_addcateid=rs7("reg_addcateid")
			if reg_addcateid="" or isnull(reg_addcateid) then reg_addcateid=0
			sql8="select  * from gate where ord="&reg_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Response.write rs8("name")
			end if
			...
			set rs8=nothing
			Response.write "" & vbcrlf & "            </div></td>" & vbcrlf & "                 <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("reg_addtime")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""func ewebeditorImg"">"
			Response.write rs7("reg_note")
			Response.write "</td>" & vbcrlf & "                   <td width=""17%"" height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('kuinDetail.asp?ord="
				Response.write rs7("id")
			...
			ifbool=false
			sqltext="select * from O_proOutList where prod_id in (select id from O_kuinList where reg_fid="&ord&")"
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				ifbool=true
			end if
			rstxt.close
			set rstxt=nothing
			sqltext="select * from O_kuCheList where prod_id in (select id from O_kuinList where reg_fid="&ord&")"
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				ifbool=true
			end if
			rstxt.close
			set rstxt=nothing
			sqltext="select * from O_proRetList where prod_id in (select id from O_proOutList where prod_id in (select id from O_kuinList where reg_fid="&ord&"))"
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				ifbool=true
			end if
			rstxt.close
			set rstxt=nothing
			if (open_101_2=3 or CheckPurview(intro_101_2,trim(addcateid))=True) and ifbool=false then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('kuinUpdate.asp?ord="
				Response.write rs7("id")
			...
			if (open_101_2=3 or CheckPurview(intro_101_2,trim(addcateid))=True) and ifbool=true then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('kuinUpdate1.asp?ord="
				Response.write rs7("id")
				Response.write "','neww35win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" value=""修改""/>" & vbcrlf & "                     "
				Response.write rs7("id")
			end if
			if (open_101_3=3 or CheckPurview(intro_101_3,trim(addcateid))=True) and del_bool then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('kuindel.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td></form>" & vbcrlf & "      </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:300; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""192"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><ahref=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按入库时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按入库时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按入库人员排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按入库人员排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="入库列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=0;" & vbcrlf & "}" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " document.getElementById('data').action=""kuinList.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
	...
	Response.write "&ret2="
	Response.write ret2
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "      document.getElementById('data').action=""kuindel.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
