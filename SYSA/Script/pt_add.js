...
function callServer_ts(m,name1) {
  ...
  w2=document.all[w2];
  if ((u_name == null) || (u_name == "")) return;
  var url = "cu_ts.asp?name=" + UrlEncode(u_name) + "&ord=" + escape(m) + "&timestamp=" + new Date().getTime() + "&date7=" + Math.round(Math.random() * 100);
  xmlHttp.open("GET", url, false);
  xmlHttp.onreadystatechange = function(){
  updatePage_ts(w2,m);
  };
...
//				if(document.getElementById("tb1").children[0].nodeName=="INPUT"){
//					document.getElementById("tb1").innerHTML = r;
//				}
