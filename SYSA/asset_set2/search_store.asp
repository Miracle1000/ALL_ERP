...
'If sc>0 Then v = v & ";"
    ...
    set rs=nothing
    for i=0 to Depth
        sql="select id,sort1,gate1,parentid,depth,storecode,isLeef,isnull((select count(*) from O_assAddresstype where depth=a.depth and parentid=a.parentid and id>a.id),0) as f from O_assAddresstype a where Depth="&i&" "&_
        "and (RootID in (select RootID from O_assAddresstype where sort1 like '%"&kv&"%') or RootID in (select b.RootID from O_assAddressname a inner join O_assAddresstype b on a.sort=b.id and a.sort1 like '%"&kv&"%')"&_
        ") order by a.gate1 desc"
        set rs=conn.execute(sql)
        j=1
        while not rs.eof
            ...
            if i=0 then
                ...
                if rs("isLeef").value=true then
                    sql="select * from O_assAddressname where sort="&rs("id")&" order by gate1 desc"
