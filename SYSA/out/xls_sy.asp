...
'If Not rs.Eof Then
	...
	elseif open_6_10=1 then
		Str_Result=" and sorce2 in ("&intro_6_10&")"
	else
		Str_Result="and sorce2=0"
	end if
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result+" and sorce2 in ("&W3&")  "
'if W4<>"" and not isnull(W4) then
	end if
...
'if intro<>"" then
	...
	if money3<>"" then
		if isnumeric(money3)=false then
			...
			Str_Result=Str_Result+"and  money1 >="&money3&""
			call db_close : Response.end
		end if
	end if
	if money12<>"" then
...
'if B="1" then
	elseif B="2" then
		...
		if trim(perid)<>"" then
			Str_Result=Str_Result+" and sorce2 in ("&perid&") "
'if trim(perid)<>"" then
		end if
	elseif B="3" then
		perid=selname("gate","name",C,"ord")
...
'if trim(perid)<>"" then
	...
	elseif B="4" then
		Str_Result=Str_Result+" and intro like ''%"&B&"%'' "
'elseif B="4" then
	end if
	if m1<>"" then
		Str_Result=Str_Result+" and  datejk>=''"&m1&"'' "
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+" and  datejk<=''"&m2&"'' "
'if m2<>"" then
	end if
	dim chance,contract,fromContract,fromChance
	chance=trim(Request.QueryString("chance"))
	contract=trim(Request.QueryString("contract"))
	if chance<>"" then
		Str_Result = Str_Result & " and id in (select fid from pay where chance=" & chance&")"
	end if
	if contract<>"" then
		Str_Result = Str_Result & " and id in (select fid from pay where contract=" & contract&")"
	end if
	...
	elseif px=3 then
		pxResult=" order by date7 desc"
	elseif px=4 then
		pxResult=" order by date7 asc"
	elseif px=5 then
		pxResult=" order by datejk desc"
	elseif px=6 then
		pxResult=" order by datejk asc"
	end if
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("费用使用列表")
	headerStr="使用主题,币种,使用总额,总张数,使用概要,使用人员,使用日期,添加人员,添加时间,费用分类,编号,张数,备注,出发时间,出发地点,到达时间,到达地点,里程,交通工具,返回时间,开始日期,结束日期,所在城市,宾馆名称,公关日期,公关对象,公关事由,赠送礼品,需要时间,用途,关联客户,关联联系人,关联项目,关联合同,关联销售退货,关联售后,关联日程,关联发货"
	if ZBRuntime.MC(15000) then
		headerStr = headerStr & ",关联采购"
	end if
	if ZBRuntime.MC(16000) then
		headerStr = headerStr & ",关联采购退货"
	end if
	...
	if ZBRuntime.MC(18000) And scVer&""="1" then
		headerStr = headerStr & ",关联生产订单"
	end if
	...
	If 1=0 Then
		if ZBRuntime.MC(18700) then
			headerStr = headerStr & ",关联整单委外"
		end if
		if ZBRuntime.MC(18610) then
			headerStr = headerStr & ",关联工序委外"
		end if
		if ZBRuntime.MC(18900) then
			headerStr = headerStr & ",关联生产设备"
		end if
	end if
	Response.Flush
	xsheet.showHeader headerStr
	xsheet.movenext
	Dim spFields : spFields = ",cateid,Creator,reg_addcateid,iss_addcateid,rep_addcateid,bk_addcateid,"
	i=1
	k=1
	sql = " exec erp_xls_fysy '"&Str_Result&"','"&pxResult&"'"
	...
	if rss.eof Then
		...
		do until rss.eof
			If rss("ord") = 0 Then
				...
				bzname = rss("bzname")
				member2 = rss("member2")
				money1 =zbcdbl( rss("money2"))
				if money1="" or isnull(money1) then money1=0
				num =zbcdbl( rss("num1"))
				note1 = rss("note1")
				addmember = rss("addmember")
				xsheet.writestr title
				xsheet.writecell bzname
				xsheet.writemoney Formatnumber(money1,num_dot_xs,-1)
				xsheet.writecell bzname
				xsheet.writenumber num
				xsheet.writecell RegReplace(note1,"<[^>]+>","")
				xsheet.writenumber num
				xsheet.writecell member2
				xsheet.writecell date1
				xsheet.writecell addmember
			...
			If rss("ord") > 0 Then
				...
				vperson=rss("vperson")
				vchance=rss("vchance")
				vcontract=rss("vcontract")
				vcontractth=rss("vcontractth")
				vshouhou=rss("vshouhou")
				vricheng=rss("vricheng")
				vfahuo=rss("vfahuo")
				vcaigou=rss("vcaigou")
				vcaigouth=rss("vcaigouth")
				vwork=rss("vwork")
				...
				ggtime=rss("ggtime")
				xsheet.writecell ""
				xsheet.writecell "【使用明细】"
				xsheet.writemoney Formatnumber(money2,num_dot_xs,-1)
				xsheet.writecell "【使用明细】"
				xsheet.writecell ""
				xsheet.writecell ""
				xsheet.writecell rss("member2_1")
				xsheet.writecell date1
				xsheet.writecell rss("addmember")
				xsheet.writecell rss("date7")
				xsheet.writestr fytype
				xsheet.writecell payid
				xsheet.writenumber rss("num1")
				xsheet.writestr note1
				xsheet.writestr startime
				xsheet.writestr rss("staraddr")
				xsheet.writestr endtime
				xsheet.writestr rss("endaddr")
				xsheet.writestr rss("lc")
				xsheet.writestr rss("bus")
				xsheet.writestr retime
				xsheet.writestr startime1
				xsheet.writestr endtime1
				xsheet.writestr rss("city")
				xsheet.writestr rss("hotel")
				xsheet.writestr ggtime
				xsheet.writestr ggcate
				xsheet.writestr rss("ggintro")
				xsheet.writestr rss("gglw")
				xsheet.writestr rss("xytime")
				xsheet.writestr rss("yt")
				...
				if ZBRuntime.MC(18000) And scVer&""="1" then
					xsheet.writestr vManuOrder
				end if
				if ZBRuntime.MC(18500) And scVer&""="0" then
					xsheet.writestr vscdd
				end if
				If 1=0 Then
					if ZBRuntime.MC(18600) then
						xsheet.writestr vzdww
					end if
					if ZBRuntime.MC(18800) then
						xsheet.writestr vgxww
					end if
					if ZBRuntime.MC(18610) then
						xsheet.writestr vscsb
					end if
				end if
			end if
			xsheet.movenext
	...
	tfile=Server.MapPath("费用使用_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'费用使用_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	Set xApp = Nothing
	action1="费用使用导出"
	call close_list(1)
