...
'dim sqlstr
	...
	Function getFormulaMoney(formula, adviseSale, minSale, adviseBuy, maxBuy, gjprice)
		Dim newMoney, f
		If Len(formula)>0 Then
			f= Replace(formula,"{建议售价}",adviseSale)
			f= Replace(f,"{最低售价}",minSale)
			f= Replace(f,"{建议进价}",adviseBuy)
			f= Replace(f,"{最高进价}",maxBuy)
			f= Replace(f,"{客户跟进程度价格}",gjprice)
			on error resume next
			newMoney = eval(f)
			If Err.number<>0 Then newMoney = 0
			On Error GoTo 0
		else
			newMoney = 0
		end if
		getFormulaMoney = CDbl(newMoney)
	end function
	Function getProductTcMoney(pord,unit, bm, cb, baseMoney, basePrice, baseNum, num_tc, tcsort1,tcsort2 ,all_sort_tc1, all_sort_tc2 , contractOrd, myconn)
		Dim clrs, sql , cp_ord , sort1 , sort2 , tc_money ,tcmoney1 ,tcmoney2, tcnum
		If tcsort1&"" = "0" Or tcsort1&""="" Then
			sort1 = all_sort_tc1
			sort2 = all_sort_tc2
			cp_ord = 0
		else
		...
		if cp_ord &"" = "" then  cp_ord=0
		If sort2&"" = "" then  sort2=0
		If sort1=3 or sort1=7 Then baseMoney = CDbl(baseMoney) - CDbl(cb)
'If sort2&"" = "" then  sort2=0
		If sort2 = 3 Then
			tc_money = CDbl(baseMoney) * CDbl(num_tc)/100
		else
			If sort2 = 4 Then
				Dim gjsort1
				gjsort1=0
				Set contractrs = myconn.execute("select isnull(t.sort1,0) as sort1 from contract a inner join tel t on t.ord=a.company inner join sort5 s on s.ord=t.sort1 and s.time1=1 where a.ord="&contractOrd)
				If contractrs.eof = False Then
					gjsort1 = contractrs(0)
				end if
				contractrs.close
				If gjsort1>0 Then gjSQL= " and sort="&gjsort1&" "
				Set jgrs=myconn.execute("select price2jy as adviseSale,price2 as minSale,price1jy as adviseBuy,price1 as maxBuy,price3 as gjprice from jiage where isnull(bm,0) = "&bm&" and product="&pord &" and unit = "&unit & gjSQL)
				If jgrs.eof = False Then
					adviseSale = jgrs("adviseSale")
					minSale = jgrs("minSale")
					adviseBuy = jgrs("adviseBuy")
					maxBuy = jgrs("maxBuy")
					If gjsort1>0 Then
						gjprice = jgrs("gjprice")
					else
						gjprice = 0
					end if
				else
			...
			sql = "select isnull(num1,0) as num1,isnull(money1,0) as money1, isnull(money2,0) as money2,isnull(tc_formula1,'') as  tc_formula1 ,isnull(tc_formula2,'') as tc_formula2 from tcbl where isnull(ord,0) = "&cp_ord &" and sort1="&sort2 &" order by gate1 asc "
			Set clrs = myconn.execute(sql)
			if clrs.eof then
				sql = "select isnull(num1,0) as num1,isnull(money1,0) as money1, isnull(money2,0) as money2,isnull(tc_formula1,'') as  tc_formula1 ,isnull(tc_formula2,'') as tc_formula2 from tcbl where ord is null and sort1="&sort2 &" order by gate1 asc "
			end if
			clrs.close
			set clrs = nothing
			Set clrs = myconn.execute(sql)
			If clrs.eof =False Then
				If sort2 =1 Then
					tc_money = CDbl(baseMoney) * CDbl(clrs("num1"))/100.0
				else
					Do While clrs.eof = False
						tcnum  = cdbl(clrs("num1"))
						If sort2 = 2 Then
							Cmoney=CDbl(baseMoney)
							tcmoney1 = CDbl(clrs("money1"))
							tcmoney2 = CDbl(clrs("money2"))
							baseNum = 1
						ElseIf sort2 = 4 Then
							Cmoney = CDbl(basePrice)
							tcmoney1 = getFormulaMoney(clrs("tc_formula1"), adviseSale, minSale, adviseBuy, maxBuy, gjprice)
							tcmoney2 = getFormulaMoney(clrs("tc_formula2"), adviseSale, minSale, adviseBuy, maxBuy, gjprice)
						end if
						If tcmoney1>=Cmoney Then
							tc_money = tc_money + CDbl(0)
'If tcmoney1>=Cmoney Then
							Exit Do
						ElseIf tcmoney2>=Cmoney then
							tc_money = tc_money + CDbl(baseNum) * (Cmoney -tcmoney1) * tcnum/100.0
'ElseIf tcmoney2>=Cmoney then
							Exit Do
						else
							tc_money = tc_money + CDbl(baseNum) * (tcmoney2-tcmoney1) * tcnum/100.0
							Exit Do
						end if
						clrs.movenext
					Loop
		...
		getProductTcMoney = tc_money
	end function
	Function getContractTcMoney(contractOrd , bm ,isOpenJg, sort_tc1, sort_tc2, myconn)
		Dim xsmoney , fymoney,Sql, lrSql,listrs ,TcMoney, taxValue, num2018030701,payMoney, Money1Jinjia, num_tc
		Dim showHtMoney, showCB, showFyMoney, showThMoney, showSkMoney
		TcMoney= 0
		If isOpenJg = "0" Then bm = 0
		showHtMoney = 0
		showCB = 0
		showFyMoney = 0
		showThMoney = 0
		...
		if sort_tc2&""="1" then
			num_tc = sdk.getSqlValue("SELECT isnull((SELECT TOP 1 num1 FROM tcbl WHERE ord IS NULL AND sort1=1),0)", 0)
		end if
		If num_tc&"" = "" Then num_tc = 0
		mlFormula=myconn.execute("select isnull(intro,0) from setopen where sort1='2018030701'")(0)
		if cdbl(mlFormula)=1 or cdbl(mlFormula)=3 then
			mlFormula=1
		else
			mlFormula=0
		end if
		xsmoney=myconn.execute("select isnull(sum(a.money1),0) from contractlist a inner join contract b on a.contract = b.ord and b.del=1 where a.contract="&contractOrd)(0)
		fymoney=myconn.execute("select isnull(sum(money1*dbo.GetHl(bz,date1,getdate())),0) from pay where contract="&contractOrd&" and del=1 and complete=3")(0)
		lrSql=""
		If cdbl(xsmoney)= 0 Then
			lrSql = "0"
		else
'lrSql = "(case when isnull(t2.tcsort1,"& sort_tc1 &") in (1,5) then isnull(t1.price1*(isnull(t1.num1,0)-isnull(t3.num1,0))*dbo.gethl(ct.bz,ct.date1,getdate()),0)/"&xsmoney&" else (isnull(t1.money1*dbo.GetHl(ct.bz,ct.date1,getdate()),0)-isnull(yh.yhlistmoney*dbo.gethl(ct.bz,ct.date1,getdate()),0))/"&xsmoney&" end)"
			lrSql = "t1.money1/"&xsmoney&""
		end if
		Sql = "select t2.ord,t1.unit ,(case isnull(t2.tcsort1,"& sort_tc1 &") when 1 then  "&_
		"          (isnull(t1.PriceAfterTax,t1.price1)*isnull(t1.num1,0)-ISNULL(t1.concessions,0))*dbo.gethl(ct.bz,ct.date1,getdate())-isnull(yh.yhlistmoney*dbo.gethl(ct.bz,ct.date1,getdate()),0)  "&_
		"when 5 then  "&_
		"    ""              ((isnull(t1.PriceAfterTax,t1.price1)-(case isnull(t1.num1,0) when 0 then 0 else cast(ISNULL(t1.concessions,0)*(case when isnull(t1.num1,0)=0 then 0 else 1/cast(t1.num1 as DECIMAL(25,12)) end ) as decimal(25,12)) end))*dbo.gethl(ct.bz,ct.date1,getdate())-isnull((case isnull(t1.num1,0) when 0 then 0 else cast(yh.yhlistmoney*(case when isnull(t1.num1,0)=0 then 0 else 1/cast(t1.num1 as DECIMAL(25,12)) end) as decimal(25,12)) end)*dbo.gethl(ct.bz,ct.date1,getdate()),0))*t1.num1 "&_
		"  else isnull(t1.Money1*dbo.gethl(ct.bz,ct.date1,getdate()),0)-isnull(yh.yhlistmoney*dbo.gethl(ct.bz,ct.date1,getdate()),0)  "&_
		"  end) as Money1 "&_
		",isnull(t1.PriceAfterTax*dbo.gethl(ct.bz,ct.date1,getdate()),0) as Price1 "&_
		",(isnull(t1.num1,0)-isnull(t3.thnum1,0)) as num1 "&_
		",isnull(t1.PriceAfterTax*dbo.gethl(ct.bz,ct.date1,getdate()),0) as Price1 "&_
		", (CASE ISNULL(t2.tcsort2,"& sort_tc2 &") WHEN 1 THEN ISNULL(tc.num1,"& num_tc &") WHEN 3 THEN ISNULL(t2.num_tc,0) ELSE 0 END) as num_tc,"&_
		"isnull(ct.Money2,0) as money2,"&_
		"isnull(ct.yhMoney,0) as yhmoney,"&_
		"isnull(t1.taxValue,0) as taxValue,"&_
		"isnull((case "& showSkMoney &" when 1 then (case ct.paybackMode when 1 then "&_
		"          sk2.money1 * (case when isnull(ctl2.money1,0)=0 then 0 else t1.money1/ctl2.money1 end) "&_
		"  when 2 then sk.money1 else 0 end) else 0 end),0) as payMoney, "&_
		" isnull(t2.tcsort1,0) as tcsort1, isnull(t2.tcsort2,0) as tcsort2,"&_
		" ((case when isnull(t2.tcsort1 ,"& sort_tc1 &")=7 then t1.pricejy * (isnull(t1.num1,0)-isnull(t3.thnum1,0)) + (case when isnull(ic.adTax,0)=1 and "&mlFormula&"=1 then t1.taxValue - isnull(t3.thtax,0) else 0 end) "&_
		" isnull(t2.tcsort1,0) as tcsort1, isnull(t2.tcsort2,0) as tcsort2,"&_
		"           when isnull(t2.tcsort1 ,"& sort_tc1 &")=3 then isnull(htcb,0)+(case when isnull(ic.adTax,0)=1 and "&mlFormula&"=1 then t1.taxValue - isnull(t3.thtax,0) else 0 end) - isnull(thcb,0) "&_
		" isnull(t2.tcsort1,0) as tcsort1, isnull(t2.tcsort2,0) as tcsort2,"&_
		"           else isnull(htcb,0) -isnull(thcb,0) end) "&_
		" isnull(t2.tcsort1,0) as tcsort1, isnull(t2.tcsort2,0) as tcsort2,"&_
		"           +isnull(thmoney,0) + ("&lrSql&")*"&fymoney&" ) as cb " &_
		" isnull(t2.tcsort1,0) as tcsort1, isnull(t2.tcsort2,0) as tcsort2,"&_
		" from contractlist t1 " &_
		" inner join product t2 on t1.ord=t2.ord " &_
		" inner join contract ct on ct.ord=t1.contract " &_
		" LEFT JOIN tcbl tc ON tc.ord=t2.ord AND tc.sort1=1 " &_
		" left join invoiceConfig ic on t1.invoiceType=ic.typeid "&_
		" left join " &_
		" ( " &_
		"  select a.id, " &_
		"          (case "& showCB &" when 1  then sum(b.money1) else 0 end) as htcb, " &_
		"          (case "& showThMoney &" when 1  then sum(thmoney) else 0 end) as thmoney, " &_
		"          (case "& showThMoney &" when 1  then sum(thcb) else 0 end) as thcb, "&_
		"          (case "& showThMoney &" when 1  then sum(thtax) else 0 end) as thtax, "&_
		"          sum(c.num1) thnum1 " &_
		"  from contractlist a " &_
		"  left join ( "&_
		"          select kol.contractlist,sum(kol.money1) as money1,sum(kol.num1) as num1 "&_
		"          from kuoutlist2 kol " &_
		"          where kol.del=1 and kol.contract="&contractOrd&" " &_
		"          group by contractlist "&_
		"   ) b on a.del=1 and a.id=b.contractlist " &_
		"  left join " &_
		"  ( " &_
		"          select " &_
		"          sum(x.money1*dbo.GetHl(x.bz,x.date1,getdate())) as thmoney, " &_
		"          sum(x.num1*y.price1/dbo.product_bl(x.ord,x.unit,isnull(y.unit,x.unit))) as thcb , x.contractlist,sum(x.num1) num1,sum(x.TaxValue) thtax " &_
		"          from contractthlist x " &_
		"          left join kuoutlist2 y " &_
		"          on y.del=1 and y.contract="&contractOrd &" and isnull(x.kuoutlist2,0) = isnull(y.id,0) " &_
		"       where x.del=1 and x.contract="&contractOrd&" "&_
		"          group by x.contractlist " &_
		"  ) c  on c.contractlist = a.id " &_
		"  group by a.id " &_
		" ) t3  on t1.del=1 and t1.id = t3.id " &_
		" left join ( " &_
		"  select (case when isnull(b.snum,0)=0 then 0 else b.yhmoney * a.num1/b.snum end) yhlistmoney,a.id from contractlist a " &_
		"  inner join ( " &_
		"          select c2.snum,c1.yhmoney,c1.ord from contract c1 " &_
		"          inner join ( " &_
		"                  select sum(num1) snum, contract from contractlist where del=1 group by contract " &_
		"          ) c2 on c1.ord = c2.contract and c1.del=1 " &_
		"  ) b on a.contract = b.ord and a.contract = "& contractOrd &" " &_
		") yh on yh.id = t1.id " &_
		"left join ("&_
		"  select a.contract,b.contractlist,SUM(b.money1-ISNULL(c.money1,0)*(case when isnull(d.money1,0)=0 then 0 else b.money1/d.money1 end)) money1  " &_
		"left join ("&_
		"  FROM payback a   " &_
		"  inner join paybacklist b on b.payback=a.ord and a.del=1 and a.complete=3 and a.contract = "& contractOrd &" and b.product>0  " &_
		"  inner join ( " &_
		"          SELECT payback,SUM(money1) money1 FROM paybacklist WHERE del=1 AND product>0 GROUP BY payback " &_
		"  ) d on d.payback = a.ord " &_
		"  LEFT JOIN paybacklist c on c.payback=a.ord and c.contractlist=0 and c.product=0 " &_
		"  GROUP by a.contract,b.contractlist " &_
		") sk on sk.contract=t1.contract and sk.contractlist=t1.id and ct.paybackMode=2 "&_
		"left join (select contract,sum(money1) money1 from payback where del=1 and complete=3 group by contract) sk2 on sk2.contract=ct.ord and ct.paybackMode=1 "&_
		"left join (select contract,sum(money1) money1 from contractlist where del=1 and ord>0 and contract="& contractOrd &" group by contract) ctl2 on ctl2.contract=ct.ord and ct.paybackMode=1"&_
		"where t1.contract="&contractOrd &" "
		Set listrs = myconn.execute(sql)
		While listrs.eof= False
			pord  = listrs("Ord")
			unit =listrs("unit")
			num_tc = listrs("num_tc")
			tcsort1 = listrs("tcsort1")
			tcsort2 = listrs("tcsort2")
			baseMoney = listrs("Money1")
			basePrice = listrs("Price1")
			baseNum = listrs("num1")
			cb = listrs("cb")
			taxValue = listrs("taxValue")
			payMoney = listrs("payMoney")
			if instr(",3,4,",","& num2018030701 &",")>0 and tcsort1&""="3" then baseMoney = payMoney
			if cb&"" = "" then cb = 0 else cb = cdbl(cb)
			if taxValue&"" = "" then taxValue = 0 else taxValue = cdbl(taxValue)
			TcMoney  = TcMoney + getProductTcMoney(pord,unit, bm, cb, baseMoney, basePrice, baseNum, num_tc, tcsort1,tcsort2 ,sort_tc1, sort_tc2 ,contractOrd, myconn)
'if taxValue&"" = "" then taxValue = 0 else taxValue = cdbl(taxValue)
			listrs.movenext
		wend
		listrs.close
		getContractTcMoney = TcMoney
	end function
	Function canTc(contractOrd, myconn)
		Dim canF , ffCl , xsGoal
		Dim tcRs, rsss, contractRs ,cateid , xsmoney ,htmoney ,hkmoney, thmoney
		canF = True
		If contractOrd&"" = "" Then contractOrd = 0
		Set tcRs = myconn.execute("select isnull(intro,1) as intro from setopen where sort1 = 111")
		If tcRs.eof = False Then
			ffCl = tcRs("intro")
		else
			ffCl = 1
		end if
		...
		cateid=0
		htmoney = 0
		Set contractRs = myconn.execute("select isnull(cateid,0) as cateid, year(date3) as contractY, month(date3) as contractM,isnull(money1,0) as htmoney from contract where ord="&contractOrd)
		If contractRs.eof= False Then
			cateid = contractRs("cateid")
			contractY = contractRs("contractY")
			contractM = contractRs("contractM")
			htmoney = CDbl(contractRs("htmoney"))
		end if
		contractRs.close
		Select Case ffCl
		Case 2 :
		If htmoney>0 Then
			tcbl = myconn.execute("select isnull(max(num1),0) as num1 from tcbl where sort1 = 10")(0)
			hkmoney =  myconn.execute("select isnull(sum(money1),0) as num1 from payback where complete=3 and del=1 and contract = "&contractOrd)(0)
			thmoney = 0
			Set rsss = myconn.execute("select isnull(sum(d.money2),0) as thmoney from contractthListDetail d inner join contractlist l  on l.id=d.contractlist inner join contractthlist tl on tl.id=d.contractthlist inner join contractth ct on ct.ord=tl.caigou and ct.del=1 and ct.sp=0 where d.del=1 and d.thtype = 'GOODS' and  l.contract="& contractOrd)
			If rsss.eof = False Then
				thmoney=rsss("thmoney")
			end if
			rsss.close
			set rsss = nothing
			If thmoney&"" = "" Then thmoney = 0
			htmoney = CDbl(htmoney)-CDbl(thmoney)
'If thmoney&"" = "" Then thmoney = 0
			if htmoney >0 then
				If (cdbl(hkmoney) / cdbl(htmoney)) < CDbl(tcbl)/100 Then canF=False
			end if
		end if
		Case 3 :
		If cateid>0 Then
			Set tcRs = myconn.execute("select m"&contractM&" as xsGoal from sys_bill_TarSet Where preid="&cateid&" And years="&contractY&" and sellType=0")
			If tcRs.eof = False Then
				xsGoal = tcRs("xsGoal")
				If xsGoal&""="" Then xsGoal = 0
				xsmoney = myconn.execute("select isnull(sum(money1),0) as xsmoney from contract where isnull(cateid,0)="&cateid&" and del=1 and isnull(sp,0)=0 and datediff(m,'"&contractY&"-"&contractM&"-01',date3)=0 ")(0)
'If xsGoal&""="" Then xsGoal = 0
				If CDbl(xsmoney)<CDbl(xsGoal) Then canF=False
			end if
			tcRs.close
		end if
		End Select
	...
	Function GetSetopenValue(keysign,  nullvalue)
		...
		ret = ""
		if keysign&""<>"" then
			set rs = conn.execute("select intro from setopen where sort1="& keysign)
			If rs.eof = false then
				ret = rs("intro")
			end if
		end if
	...
	Function UnContractTCMoney(conn , PayBackOrd)
		dim sqltext
		if len(PayBackOrd&"")>0 then
			sqltext = "Update c set c.money_tc2= (case when c.money_tc2-isnull(p.money_tc,0)<0 then 0 else c.money_tc2-isnull(p.money_tc,0) end) "&_
			"from contract c  "&_
			"  inner join (select contract , sum(money_tc) as money_tc from payback where ord in (" & PayBackOrd &") and tc=1 group by contract) p on c.ord =p.contract "
			conn.execute(sqltext)
			conn.execute("Update payback set tc = 0 , money_tc = 0,tc_person = 0 , tc_date = null  where ord in (" & PayBackOrd &") and tc=1  ")
			call UnContractTCStatus(conn , PayBackOrd)
		end if
	end function
	Function UnContractTCStatus(conn , PayBackOrd)
		if PayBackOrd &""="" then PayBackOrd = 0
		dim sqltext
		sqltext = "Update c set c.tc = case  "&_
		"   when isnull(c.money_tc1,0) = isnull(c.money_tc2,0) and isnull(c.money_tc1,0)>0 and isnull(c.money_tc2,0)>0 then 3 "&_
		"   when isnull(c.money_tc2,0) = 0 then 1 "&_
		"   when isnull(c.money_tc1,0) > isnull(c.money_tc2,0) then 2 "&_
		"   else 4 end " &_
		" from contract c" &_
		" where  c.ord in (select distinct contract from payback where ord in (" & PayBackOrd &") ) "
		conn.Execute(sqltext)
	end function
...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	Dim helper : Set helper = CreateReminderHelper(conn,11,0)
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=44 and sort2=3"
	rs1.open sql1,conn,3,1
	if rs1.eof then
		open_44_3=0
		intro_44_3=0
	else
		open_44_3=rs1("qx_open")
		intro_44_3=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=1701 and sort2=1"
	rs1.open sql1,conn,3,1
	if rs1.eof then
		open_1701_1=0
		intro_1701_1=0
	else
		open_1701_1=rs1("qx_open")
		intro_1701_1=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=1701 and sort2=14"
	rs1.open sql1,conn,3,1
	if rs1.eof then
		open_1701_14=0
		intro_1701_14=0
	else
		open_1701_14=rs1("qx_open")
		intro_1701_14=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	dim w,a ,b,c,d,e,f,sort1,sort2,order,m1,m2,bz,skfs_type
	From=request("From")
	str_Result = ""
	hastk = request("hastk")
...
'if m1&""="" and m2&""=""  then
	...
	if From<>"1" and request("A")&""<>"3" and request("A")&""<>"7" then
		if datetype="ysdate" then
			if request("A")&""="8" then
				if m1<>"" then str_Result = str_Result + "and  c.date3>='"&m1&"' "
'if request("A")&""="8" then
				if m2<>"" then str_Result = str_Result + "and  c.date3<='"&m2&" 23:59:59' "
'if request("A")&""="8" then
			else
...
'case "11" :
	...
	if A="3333333" or A="5555555" or newstate="1" then
		...
		If A="5555555" Then Str_Result = " and c.complete='1' and c.ord in (" & helper.listSql("ids") & ")"
	end if
	...
	if IsOther="1" THEN
		strOrgids = request("Orgids")
		str_Result = str_Result + " and g1.orgsid in ("& strOrgids &") "
		'strOrgids = request("Orgids")
	else
...
'if A = "8" then
				...
				str_Result = str_Result + " and c.cateid in ("& W3 &") "
'if A = "8" then
			end if
		end if
	end if
...
'select case B
		case "khid" : str_Result = str_Result + "and t.khid like '%"& C &"%'"
'select case B
...
'select case B
		case "dzry" : str_Result = str_Result + "and bc.Creator IN (SELECT ORD FROM gate WHERE name Like '%"& C &"%') "
'select case B
		case "xsry" : str_Result = str_Result + "and c.cateid IN (SELECT ORD FROM gate WHERE name LIKE '%"& C &"%') "
'select case B
...
'If CStr(skfs_type)="999" Then
	...
	ret = Request("ret")
	ret2 = Request("ret2")
	if ret<>"" and datetype="dzdate" then str_Result = str_Result + " and c.date5>='"&Request("ret")&" 00:00:00'"
	'ret2 = Request("ret2")
	if ret2<>"" and datetype="dzdate" then str_Result = str_Result + " and c.date5<='"&Request("ret2")&" 23:59:59'"
	'ret2 = Request("ret2")
...
'if link="yes" then
	...
	UUrl=ReturnUrl()
	Cshklist=request("Cshklist")
	UUrl=replace(UUrl,"&Cshklist="&Cshklist&"","",1,-1,1)
	'Cshklist=request("Cshklist")
	UUrl=replace(UUrl,"Cshklist="&Cshklist&"","",1,-1,1)
	'Cshklist=request("Cshklist")
	Ctclist=request("Ctclist")
	UUrl=replace(UUrl,"&Ctclist="&Ctclist&"","",1,-1,1)
	'Ctclist=request("Ctclist")
	UUrl=replace(UUrl,"Ctclist="&Ctclist&"","",1,-1,1)
	'Ctclist=request("Ctclist")
	Czplist=request("Czplist")
	UUrl=replace(UUrl,"&Czplist="&Czplist&"","",1,-1,1)
	'Czplist=request("Czplist")
	UUrl=replace(UUrl,"Czplist="&Czplist&"","",1,-1,1)
	'Czplist=request("Czplist")
	Untclist=request("Untclist")
	UUrl=replace(UUrl,"&Untclist="&Untclist&"","",1,-1,1)
	'Untclist=request("Untclist")
	UUrl=replace(UUrl,"Untclist="&Untclist&"","",1,-1,1)
	'Untclist=request("Untclist")
	...
	khid=conn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=3 ")(0)
	If khid="" Then khid="客户编号"
...
'If khid="" Then khid="客户编号"
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "    var remind = """
	Response.write remind
	Response.write """" & vbcrlf & "    if(remind!=""""){" & vbcrlf & "        jQuery(function(){" & vbcrlf & "            BindCancelEvents({ cfgId: remind});" & vbcrlf & "        });" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "       function test()" & vbcrlf & " {" & vbcrlf & "               if ($(""input[name='selectid']:checked"").size()==0){" & vbcrlf & "                 alert(""您没有选择任何收款记录，请重新选择！"");" & vbcrlf & "                    return;" & vbcrlf & "         }" & vbcrlf & "               if(confirm('确认删除吗？')) {" & vbcrlf & "                   document.all.form1.action = ""deleteall2.asp?"
	Response.write UUrl
	Response.write """;" & vbcrlf & "                        document.all.form1.submit();" & vbcrlf & "            };" & vbcrlf & "      }" & vbcrlf & "       function ask() {" & vbcrlf & "                document.all.form1.action = ""tcall.asp?"
	Response.write UUrl
	Response.write """;" & vbcrlf & "                document.all.form1.submit();" & vbcrlf & "    }" & vbcrlf & "       function ask4(){" & vbcrlf & "                if(!confirm('确认取消吗？')) return false;" & vbcrlf & "              document.all.form1.action = ""tc_contract_cancel.asp?"
	Response.write UUrl
	Response.write """;" & vbcrlf & "                document.all.form1.submit();" & vbcrlf & "    } " & vbcrlf & "      function ask2() {" & vbcrlf & "               var tmp=[];" & vbcrlf & "             $("":checked[name='selectid']"").each(function(i){" & vbcrlf & "                  tmp[i]=$(this).val();" & vbcrlf & "           })" & vbcrlf & "              if (tmp.length==0)" & vbcrlf & "      {" & vbcrlf & "                       alert(""您没有选择任何信息！"");" & vbcrlf & "            }" & vbcrlf & "               else" & vbcrlf & "            {" & vbcrlf & "                       var strSelect= tmp.join(',');" & vbcrlf & "                   window.open('../../SYSN/view/finan/payback/PayBackSure.ashx?paybackIds=' + strSelect, 'plancor6', 'width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=120,top=100');" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "       function ask5() {" & vbcrlf & "           var tmp = [];" & vbcrlf & "           $("":checked[name='selectid']"").each(function (i) {" & vbcrlf & "            tmp[i] = $(this).val();" & vbcrlf & "     })"& vbcrlf & "          if (tmp.length == 0) {" & vbcrlf & "              alert(""您没有选择任何信息！"");" & vbcrlf & "        }" & vbcrlf & "       else {" & vbcrlf & "              var strSelect = tmp.join(',');" & vbcrlf & "          window.open('../../SYSN/view/finan/payback/PayBackSureBatch.ashx?paybackIds=' + strSelect, 'plancor6', 'width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=120,top=100');" & vbcrlf & "           }" & vbcrlf & "   }" & vbcrlf & "       function ask3() {" & vbcrlf & "               document.all.form1.action = ""orderallhy.asp?"
	Response.write UUrl
	Response.write UUrl
	Response.write """;" & vbcrlf & "                document.all.form1.submit();" & vbcrlf & "    }" & vbcrlf & "       function mm(form)" & vbcrlf & "       {" & vbcrlf & "               for(var i=0;i<form.elements.length;i++)" & vbcrlf & "         {" & vbcrlf & "                       var e = form.elements[i];" & vbcrlf & "                       if (e.name != 'chkall')" & vbcrlf & "                 e.checked = form.chkall.checked;" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "" & vbcrlf & "    function Myopen_px(divID){" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "                  divID.style.display=""""" & vbcrlf & "            }" &vbcrlf & "            divID.style.left=300;" & vbcrlf & "           divID.style.top=0;" & vbcrlf & "    }" & vbcrlf & "    function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "        if(divID.style.display==""""){" & vbcrlf & "            divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "  divID.style.display=""""" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function checkAll2(str){" & vbcrlf & "        var a=document.getElementById(""u""+str).getElementsByTagName(""input"");" & vbcrlf & "       var b=document.getElementById(""e""+str);" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "                 a[i].checked=b.checked;" & vbcrlf & "         }" & vbcrlf & "    }" & vbcrlf & "    function CheckhtCreatType(values)" & vbcrlf & "    {" & vbcrlf & "      document.location.href=values + ""?""" & vbcrlf & "    }" & vbcrlf & "    function callServer2() {" & vbcrlf & "       document.getElementById('kh').style.display='none';" & vbcrlf & "     document.getElementById('ht1').style.display='';" & vbcrlf & "        document.getElementById('ht1').style.position='relative'; " & vbcrlf & "      document.getElementById('ht1').style.zIndex=1;" & vbcrlf & "          var url = ""liebiao_PaybackList.asp?timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100)+""&E="" + window.request_E + ""&F="" + window.request_F + ""&H2="" + window.request_H2;" & vbcrlf & "         xmlHttp.open(""GET"", url, false);" & vbcrlf & "          xmlHttp.onreadystatechange = function(){" & vbcrlf & "                 updatePage2();" & vbcrlf & "          };" & vbcrlf & "      xmlHttp.send(null);" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function updatePage2() {" & vbcrlf & "            var test7=""ht1""" & vbcrlf & "           if (xmlHttp.readyState < 4) {" & vbcrlf & "                   ht1.innerHTML=""loading..."";" & vbcrlf & "            }" & vbcrlf & "       if (xmlHttp.readyState == 4) {" & vbcrlf & "                  var response = xmlHttp.responseText;" & vbcrlf & "                    ht1.innerHTML=response;" & vbcrlf & "                 xmlHttp.abort();" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "</script>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "#content{word-wrap: break-word;table-layout:auto;}" & vbcrlf & "#content td{word-wrap : break-word;}" & vbcrlf & "#w.easyui-window.panel-body.panel-body-noborder.window-body{height:500px!important}" & vbcrlf & "select{vertical-align:middle;}" & vbcrlf & "input[name=""C""],input[name=""ret""],input[name=""ret2""]{vertical-align:middle;height:20px;box-sizing:border-box;line-height:20px;}" & vbcrlf & "input#hastk{vertical-align:middle;}" & vbcrlf & "table[background=""../images/m_mpbg.gif""] .advanSearch{background:#EFEFEF}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  "
	if open_7_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "           <tr>" & vbcrlf & "      <td class=""place"">收款计划列表"
	if A="1" then
		Response.write "&gt;&gt;应收账款"
	elseif A="2" then
		Response.write "&gt;&gt;底单到账"
	elseif A="3" then
		Response.write "&gt;&gt;已收款"
	elseif A="7" And ZBRuntime.MC(11000) then
		Response.write "&gt;&gt;待发提成"
	elseif A="8" And ZBRuntime.MC(11000) then
		Response.write "&gt;&gt;已发提成"
	end if
	Response.write "</td>" & vbcrlf & "          <td align=""left""><pre style='display:inline'>&nbsp;<a class='px_btn sortRule' href=""#"" onClick=""Myopen_px(User);return false;"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></pre></td>" & vbcrlf & "        <td width=""66%""><div align=""right""> "& vbcrlf &    "           "
	If (open_7_13=1 Or open_7_13=3) And (open_7_20 = 3 Or open_7_20=1)  then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""openbn"" id=""openbn"" value=""期初收款""  class=""anybutton"" onClick=""javascript:window.open('../money/begin_balance.asp','new_balancewin','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=50,top=50')"" />"
	end if
	if open_7_10=1 or open_7_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_hk.asp?"
		Response.write UUrl
	...
	Response.write "" & vbcrlf & "        <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" >" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""page_count=10"" "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "        </select>" & vbcrlf & "      </td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "         <td  class=""resetHeadBg"" colspan=""3"" class=""ser_btn2"" height=""30"" align=""right""> "& vbcrlf &        "     <div id=""kh"" styleen"" name=""px"" value="""
	Response.write px
	Response.write """>" & vbcrlf & "            <INPUT TYPE=""hidden"" NAME=""page_count"" VALUE="""
	Response.write page_count
	Response.write """>" & vbcrlf & "            "
	If ZBRuntime.MC(25000) Then
		Response.write " <INPUT name=hastk id=hastk value=1 type=""checkbox"" "
		if hastk="1" then
			Response.write "checked"
		end if
		Response.write "><label for=""hastk"">含退款</label>&nbsp;"
	end if
	Response.write "" & vbcrlf & "            <select name=""datetype"" id=""datetype"">" & vbcrlf & "                <option value=""ysdate"" "
	if datetype="" or datetype="ysdate" then
		Response.write "selected"
	end if
	Response.write ">应收日期</option>" & vbcrlf & "                <option value=""jhdate"" "
	if datetype="jhdate" then
		Response.write "selected"
	end if
	Response.write ">计划日期</option>" & vbcrlf & "                <option value=""dzdate"" "
	if datetype="dzdate" then
		Response.write "selected"
	end if
	Response.write ">到账日期</option>" & vbcrlf & "            </select>" & vbcrlf & "                <INPUT readonly=""true"" name=ret size=9  id=daysOfMonthPos  onmousedown=""datedlg.show()"" value="""
	Response.write m1
	Response.write """> - <INPUT name=ret2 readonly=""true"" size=9  id=daysOfMonth2Pos onmousedown=""datedlg.show()"" value="""
	Response.write m1
	Response.write m2
	Response.write """>" & vbcrlf & "            <select name=""P1"">" & vbcrlf & "                    "
	if ZBRuntime.MC(19000) then
		Response.write "<option value=""1"" "
		if P1=1 then
			Response.write "selected"
		end if
		Response.write ">直接收款</option>"
	end if
	Response.write "" & vbcrlf & "                     <option value=""7"" "
	if P1=7 then
		Response.write "selected"
	end if
	Response.write ">收款抹零</option>" & vbcrlf & "                   "
	if ZBRuntime.MC(25000) and ZBRuntime.MC(25001) then
		Response.write "<option value=""8"" "
		if P1=8 then
			Response.write "selected"
		end if
		Response.write ">应退抵应收</option>"
	end if
	if ZBRuntime.MC(24000) and ZBRuntime.MC(24001) then
		Response.write "<option value=""4"" "
		if instr(P1,4)>0 or instr(P1,9)>0 then
			Response.write "selected"
		end if
		Response.write ">应付抵应收</option>"
	end if
	...
	if ZBRuntime.MC(20001) then
		...
		Response.write ">承兑汇票</option>"
	end if
	Response.write "" & vbcrlf & "                <option value=""10"" "
	if P1=10 then
		Response.write "selected"
	end if
	Response.write ">收款方式</option>" & vbcrlf & "               </select>" & vbcrlf & "               <select name=""A"">" & vbcrlf & "                     <option value=""1"" "
	if A=1 then
		Response.write "selected"
	end if
	Response.write ">未收款</option>" & vbcrlf & "                     <option value=""3"" "
	if A=3 then
		Response.write "selected"
	end if
	Response.write ">已收款</option>" & vbcrlf & "                <option value=""2"" "
	if A=2 then
		Response.write "selected"
	end if
	Response.write ">底单到账</option>" & vbcrlf & "                   "
	If ZBRuntime.MC(11000) Then
		Response.write "" & vbcrlf & "                         <option value=""7"" "
		if A=7 then
			Response.write "selected"
		end if
		Response.write ">待发提成</option>" & vbcrlf & "                       <option value=""8"" "
		if A=8 then
			Response.write "selected"
		end if
		Response.write ">已发提成</option>" & vbcrlf & "                   "
	end if
	Response.write "" & vbcrlf & "                     <option value=""10"" "
	if A=10 then
		Response.write "selected"
	end if
	Response.write ">收款状态</option>" & vbcrlf & "               </select>" & vbcrlf & "               <select name=""B"">" & vbcrlf & "                     "
	If ZBRuntime.MC(1000) Then
		Response.write "" & vbcrlf & "                    <option value=""khmc"" "
		if B="khmc" then
			Response.write "selected"
		end if
		Response.write ">"
	...
	If ZBRuntime.MC(7000) Then
		Response.write "" & vbcrlf & "                     <option value=""htzt"" "
		if B="htzt" then
			Response.write "selected"
		end if
		...
		Response.write ">合同编号</option>" & vbcrlf & "                "
	end if
	If ZBRuntime.MC(23003) Then
		Response.write "" & vbcrlf & "                <option value=""dzzt"" "
		if B="dzzt" then
			Response.write "selected"
		end if
		Response.write ">对账主题</option>" & vbcrlf & "                <option value=""dzry"" "
		if B="dzry" then
			Response.write "selected"
		end if
		Response.write ">对账人员</option>" & vbcrlf & "                "
	end if
	Response.write "" & vbcrlf & "                     <option value=""xsry"" "
	if B="xsry" then
		Response.write "selected"
	end if
	Response.write ">销售人员</option>" & vbcrlf & "                   <option value=""tjry"" "
	if B="tjry" then
		Response.write "selected"
	end if
	Response.write ">添加人员</option>" & vbcrlf & "               </select> <input name=""C"" type=""text"" size=""15""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "                   <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "            </form>" & vbcrlf & "            <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onClick=""callServer2()"" style='display:inline-block;*padding-bottom:3px;width:66px;'><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>&nbsp;" & vbcrlf & "        </div>" & vbcrlf & "        <form action=""planall2.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "            <input type=""hidden"" name=""px"" value="""
	Response.write px
	Response.write """>" & vbcrlf & "            <INPUT TYPE=""hidden"" NAME=""page_count"" VALUE="""
	Response.write page_count
	Response.write """>" & vbcrlf & "            <div id=""ht1"" style=""border-top:1px solid #ccc;display:none""></div>" & vbcrlf & "         </form>" & vbcrlf & "             </td>" & vbcrlf & "   </tr>" & vbcrlf & "</table>" & vbcrlf & "<form name=""form1"" method=""post"" action=""deleteall2.asp?"
	Response.write page_count
	Response.write UUrl
	Response.write """>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-top:0px"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "   <td width=""4%"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "<td width=""15%""><div align=""center""><strong>"
	Response.write khname
	Response.write "</strong></div></td>" & vbcrlf & " <td width=""10%"" height=""27"" ><div align=""center""><strong>应收金额</strong></div></td>" & vbcrlf & " <td width=""10%"" ><div align=""center"">日期</div></td>          " & vbcrlf & "      <td width=""8%""><div align=""center"">收款方式</div></td>" & vbcrlf & "      <td width=""10%""><div align=""center""><strong>收款账户</strong></div></td>" & vbcrlf &    " "
	if ((A=7 or A=8) or (hkzt="4" or hkzt="5")) and ZBRuntime.MC(11000) then
		Response.write "<td width=""6%"" ><div align=""center""><strong>已发提成</strong></div></td>"
	end if
	Response.write "" & vbcrlf & "     <td width=""10%""><div align=""center""><strong>收款计划状态</strong></div></td>" & vbcrlf & "        <td width=""10%""><div align=""center""><strong>人员</strong></div></td>" & vbcrlf & "        <td width=""12%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "</tr>" & vbcrlf & ""
	dim n
	n=0
	all_total_money=0
	all_total_tc=0
	...
	"    t.name as companyname,t.cateid as khcateid ,ISNULL(t.share,'-222') as khshare,t.sort3, bc.title as bcTitle ,bc.MyPerson ,"& vbcrlf &_
	"    identity(int,1,1) as rowindex,t.del as telDel into #temp "&_
	"from ( "&_
	"    select p.ord,p.title,p.intro, p.company, p.contract , p.BCheckId , p.bz,  p.money1,isnull(p.money_tc,0) money_tc, p.date4,p.date1, p.DATE2,p.date5, p.complete , p.CompleteType, "&_
	"      p.bank , isnull(p.tc,0) tc, p.cateid ,p.addcate ,case when isnull(p.complete,0)<3 and isnull(p.OutSureId,0) = 0 and r.Status in (-1, 1)  and (isnull(r.wxUserId,0)=0 or isnull(r.payStatus,0) = 2 or isnull(r.payKind,2) = 2) then 1 else 0 end canPayback ,"&_
	"      r.cpCostChanged , ISNULL(r.share,'-222') as htshare, r.cateid as htcateid ,r.title as httitle,r.del as htDel,p.planhkid,p.pay,p.date7 , p.paybackType , r.date3 as date3 , p.tik ,  "&_
	"      r.htid , r.zdy1,r.zdy2,r.zdy3,r.zdy4,r.zdy5,r.zdy6 ,r.fqhk , r.complete1 ,r.sort , r.area ,p.sureID,p.outsureId "&_
	"   from payback p WITH(NOLOCK)  "&_
	"   left join contract r WITH(NOLOCK) on r.ord= p.contract "&_
	"   where p.del=1 "& Str_Result4 &_
	"   union all "&_
	"   select -p.ord, '' title, p.intro, r.company, -contractth , 0 BCheckId, p.PayBz  ,p.money1 ,0 as money_tc, null date4 , p.date1 ,null DATE2, p.date2 date5 , p.complete , p.CompleteType, "&_
	"      p.bank , 0 tc, p.cateid, p.addcate ,case when isnull(p.Complete,0) = 1 then 1 else 0 end canPayback ,"&_
	...
	"      r.thid htid, r.zdy1,r.zdy2,r.zdy3,r.zdy4,r.zdy5,r.zdy6 , r.BKPayModel as fqhk , 0 complete1 , 0 sort , r.area ,p.sureID,0 AS outsureId "&_
	"   from payout2 p "&_
	...
	"left join gate g2 WITH(NOLOCK) on g2.ord=c.addcate  "&_
	"left join hl h WITH(NOLOCK) on datediff(d,h.date1,c.date3)=0 and h.bz=c.bz "&_
	"left join sortbz b WITH(NOLOCK) on b.id=c.bz "&_
	"where 1=1 " & str_Result & px_Result1 &" "
	conn.execute sql
	if request("debug")=1 then Response.write sql
	if request("debug")="1" then Response.write replace(sql,"into #temp","")
	sql_money="select isnull(sum((case when ord<0 then -1 else 1 end)*money1),0) as money1,isnull(sum(money_tc),0) as money_tc from #temp"
'if request("debug")="1" then Response.write replace(sql,"into #temp","")
	set rs=conn.execute(sql_money)
	if not rs.eof then
		all_total_money=cdbl(rs(0).value)
		all_total_tc=cdbl(rs(1).value)
	end if
	rs.close
	sql8="select isnull(sum((case when ord<0 then -1 else 1 end)*money1*isnull(hl,1)),0) from #temp "
	'all_total_tc=cdbl(rs(1).value)
	set rs8=conn.execute(sql8)
	if not rs8.eof Then alltotal_money_hl=cdbl(rs8(0).value)
	rs8.close
	sqlcount="select count(1) from #temp "
...
'if currpage=pagecount then
	...
	dim isOpen : isOpen = IsOpenVoucherCForSKInvoice
	dim isTkOpen : isTkOpen = IsOpenVoucherCForXTK
	sql="select * from ("&_
	"  select top "&page_count&" * from #temp where rowindex in ("&_
	"          select top "&intcount&" rowindex from #temp where rowindex in ("&_
	"                  select top "&page_count*CurrPage&" rowindex from #temp order by rowindex asc) "&_
	"          order by rowindex desc) "&_
	"  order by rowindex asc "&_
	") a "
	rs.open sql,conn,3,1
	if rs.eof=true then
		...
		while rs.eof=false
			BillID = rs("ord")
			if BillID>0 then
				if len(hkids)>0 then hkids = hkids & ","
				hkids = hkids & BillID
			else
				if len(tkids)>0 then tkids = tkids & ","
				tkids = tkids & abs(BillID)
			end if
			rs.movenext
		wend
		if len(hkids)>0 then
			set hklist = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & hkids &"',1, 6, '6003,6006,6009' ")
		end if
		if len(tkids)>0 then
			set tklist = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & tkids &"',8, 6, '' ")
		end if
		rs.movefirst
		Dim rid
		do until rs.eof
			bcTitle= rs("bcTitle")
			BillID = rs("ord")
			If remind <> "" and BillID>0 Then rid = helper.getRemindIdByOID(BillID)
			canPayback = rs("canPayback")
			titleTxt= rs("title")
			intro=rs("intro")
			cateid=rs("cateid")
			...
			if billid>0 then
				total_money=zbcdbl(total_money)+zbcdbl(rs("money1"))
'if billid>0 then
				total_money_hl=zbcdbl(total_money_hl)+zbcdbl(rs("money1"))*zbcdbl(rs("hl"))
'if billid>0 then
			else
				total_money=zbcdbl(total_money)-zbcdbl(rs("money1"))
'if billid>0 then
			...
			total_tc=zbcdbl(total_tc)+zbcdbl(rs("money_tc"))
			paybackType=rs("paybackType")
			bzintro = rs("bzintro")
			company=rs("company")
			companyname=rs("companyname")
			cateid_kh= rs("khcateid")
			khshare = rs("khshare")
			htshare = rs("htshare")
			cID = rs("contract")
			cateid_ht = rs("htcateid")
			httitle = rs("httitle")
			paytypename = rs("paytypename")
			bankname = rs("bankname")
			complete = rs("complete")
			cpCostChanged = rs("cpCostChanged")
			htDel = rs("htDel")
			telDel =rs("telDel").value
			Response.write "" & vbcrlf & "                      <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   <td height=""30"" align=""center"">" & vbcrlf & "                             <span class=""red"">"
			if open_7_3=3 or open_7_3=1 Or open_7_20=3 or open_7_20=1 or open_44_3=1 or open_44_3=3 or  open_7_6=1 or open_7_6=3 then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write BillID
				Response.write """>"
			end if
			Response.write "</span>" & vbcrlf & "                       </td>" & vbcrlf & "                   <td>" & vbcrlf & "                            <div align=""left"">" & vbcrlf & "                                "
			dim writetext : writetext = companyname
			if billID<0 then writetext = "<font color=red>"&companyname&"</font>"
			if ZBRuntime.MC(1000) or ZBRuntime.MC(1002) Then
				dim IsCompanyNoDel : IsCompanyNoDel = true
				if company<>"" And Len(companyname&"")>0 Then
					if  (A="3333333" or A="7777777") and timelg<>"" and datediff("s",timelg,rs("date7"))>0 Then companyname=companyname&new_list
					If rs("sort3").value="2" Then
						if ZBRuntime.MC(1002) and (open_26_1=3 or (CheckPurview(intro_26_1,trim(cateid_kh))=True  And cateid_kh<>0) ) then
							if open_26_14=3 or (CheckPurview(intro_26_14,trim(cateid_kh))=True  And cateid_kh<>0) Then
								Response.write "<a href=""#"" onclick=""javascript:window.open('../work2/content.asp?ord="
						...
						IsShare = False
						If InStr(1,","&khshare&",", ","&session("personzbintel2007")&",",1) > 0 Or khshare = "1" Then IsShare = True
						if ZBRuntime.MC(1000) and  (open_1_1=3 or (CheckPurview(intro_1_1,trim(cateid_kh))=True  And cateid_kh<>0) Or IsShare) then
							if open_1_14=3 or (CheckPurview(intro_1_14,trim(cateid_kh))=True  And cateid_kh<>0) Then
								Response.write "<a href=""#"" onclick=""javascript:window.open('../work/content.asp?ord="
								Response.write pwurl(company)
								Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >"
								Response.write pwurl(company)
								Response.write writetext
					...
					if telDel=2 then Response.write "<font color=red>(已删除)</font>"
				else
					IsCompanyNoDel = false
					Response.write "<font color=red>客户/供应商已彻底删除</font>"
				end if
			end if
			If cID<>"" and Len(httitle&"")>0 Then
				if BillID>0 then
					IsShare = False
					If InStr(1,","&htshare&",", ","&session("personzbintel2007")&",",1) > 0 Or htshare = "1" Then IsShare = True
					if ZBRuntime.MC(7000) and (open_5_1=3 or (CheckPurview(intro_5_1,trim(cateid))=True And cateid<>0) Or IsShare) then
						if open_5_14=3 or (open_5_14=1 and CheckPurview(intro_5_14,trim(cateid))=True) Then
							if htDel=2 then
								Response.write " <br>【合同】<font color=""red"">合同已删除</font>"
							else
								Response.write "<br><a href=""###"" onClick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord="
								Response.write pwurl(rs("contract"))
								Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >【合同】"
								Response.write pwurl(rs("contract"))
								Response.write httitle
								Response.write "</a>"
							end if
						else
							Response.write "<br>"
							if htDel=2 then
								Response.write "【合同】<font color=""red"">合同已删除</font>"
							else
								Response.write "【合同】"
								Response.write httitle
							end if
					...
					if open_41_1=3 or (CheckPurview(intro_41_1,trim(cateid))=True And cateid<>0) then
						if open_41_14=3 or (open_41_14=1 and CheckPurview(intro_41_14,trim(cateid))=True) Then
							if htDel=2 then
								Response.write " <br>【退货】退货已删除"
							else
								Response.write "<br><a href=""###"" onClick=""javascript:window.open('../contractth/content.asp?ord="
								Response.write pwurl(abs(rs("contract").value))
								Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" ><font color=red>【退货】"
								Response.write pwurl(abs(rs("contract").value))
								Response.write httitle
			...
			BCheckId = rs("BCheckId").value
			if BCheckId&""<>"" and BCheckId<>"0" and ZBRuntime.MC(23003) then
				MyPerson = rs("MyPerson").value
				if MyPerson&""="" then MyPerson = 0
				if open_1701_1=3 or (CheckPurview(intro_1701_1,trim(MyPerson))=True And MyPerson<>0) then
					if ((open_1701_14=3 or (open_1701_14=1 and CheckPurview(intro_1701_14,trim(MyPerson))=True)) and len(bcTitle&"")>0) Then
						Response.write "<br><a href=""###"" onClick=""javascript:window.open('../../SYSN/view/finan/Checks/Bill.ashx?ord="
						Response.write pwurl(BCheckId)
						Response.write "&view=details','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >【对账单】"
						Response.write pwurl(BCheckId)
						Response.write bcTitle
						Response.write "</a>"
					else
						Response.write "<br>【对账单】"
						Response.write bcTitle
					end if
				end if
			end if
			If Cshklist<>"" And InStr(","&Replace(Cshklist," ",",")&",",","&BillID&",")>0 Then Response.write "&nbsp;<font color=red>不允许删除</font>"
			If Ctclist<>"" And InStr(","&Replace(Ctclist," ",",")&",",","&BillID&",")>0 Then Response.write "&nbsp;<font color=red>不允许提成</font>"
			If Czplist<>"" And InStr(","&Replace(Czplist," ",",")&",",","&BillID&",")>0 Then Response.write "&nbsp;<font color=red>不允许指派</font>"
			If Untclist<>"" And InStr(","&Replace(Untclist," ",",")&",",","&BillID&",")>0 Then Response.write "&nbsp;<font color=red>不允许取消提成</font>"
			Response.write "" & vbcrlf & "                             </div>" & vbcrlf & "                  </td>" & vbcrlf & "                   <td><div align=""right"" "
			if billid<0 then
				Response.write " style=""color:Red"""
			end if
			Response.write ">"
			Response.write bzintro & Formatnumber(zbcdbl(rs("money1").value),num_dot_xs,-1)
			Response.write ">"
			Response.write "</div></td>" & vbcrlf & "                  <td><div align=""left"" style=""padding-left:10%"">" & vbcrlf & "                "
			Response.write ">"
			if billID>0 then
				Response.write "计划："
				Response.write rs("date4")
				Response.write "<br>应收："
				Response.write rs("date1")
				Response.write "<br>实收："
				Response.write rs("date5")
			else
				Response.write "<font color=""red"">应退："
				Response.write rs("date1")
				Response.write "<br>实退："
				Response.write rs("date5")
				Response.write "</font>"
			end if
			...
			if billID>0 then
				if complete = 3 then
					...
					case "1" : writetext = "直接收款"
					case "2" : writetext = "预收款抵扣"
					case "3" : writetext = "承兑汇票"
					case "4","9" : writetext = "应付抵应收"
					case "7" : writetext = "收款抹零"
					case "8" : writetext = "应退抵应收"
					end select
					if completeType="9" then
						fkstr = "(付款申请)"
						outsureId = RS("outsureId").VALUE
						if outsureId&""<>"" and outsureId&""<>"0" then
							if conn.execute("select 1 from payoutsure where id =" & outsureId &" and isnull(status,-1)=0").eof=false then
'if outsureId&""<>"" and outsureId&""<>"0" then
								fkstr= "(付款否决)"
							elseif conn.execute("select 1 from payoutsure where id =" & outsureId &" and [Complete]=3 ").eof=false then
								fkstr= ""
							end if
						end if
					end if
					thmoney = sdk.GetSqlValue("select isnull(sum(money1),0) thmoney from paybacklist where payback=" & BillID &" and contractlist<0" ,0 )
					if cdbl(thmoney)>0 then
						fkstr = "<font color='red' title='退货金额"& Formatnumber(thmoney,num_dot_xs,-1) & "'>(有退货)</font>"
'if cdbl(thmoney)>0 then
					end if
				end if
			else
				select case completeType
				case "1" : writetext = "直接退款"
				case "2" : writetext = "退款转预收款"
				case "8" : writetext = "应退抵应收"
				end select
				writetext = "<font color=red>" & writetext & "</font>"
			end if
			Response.write writetext
			...
			if ((A=7 or A=8) or (hkzt="4" or hkzt="5")) And ZBRuntime.MC(11000) then
				...
				Response.write Formatnumber(zbcdbl(rs("money_tc")),num_dot_xs,-1)
				Response.write "<td><div align=""right"">"
			...
			if BillID>0 then
				if complete="1" then
					Response.write "" & vbcrlf & "                                         未收款" & vbcrlf & "                                          "
					if (open_7_20=3 or (open_7_20=1 and CheckPurview(intro_7_20,trim(cateid))=True  And cateid<>0)) And canPayback="1" And IsCompanyNoDel = true then
						Response.write "" & vbcrlf & "                                                 <img src=""../images/jiantou.gif""><a href=""#"" onclick=""javascript:window.open('../../SYSN/view/finan/payback/PayBackSure.ashx?paybackId="
						Response.write pwurl(BillID)
						Response.write "','plancor6','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=120,top=100');return false;"">收款</a>" & vbcrlf & "                                      "
						Response.write pwurl(BillID)
					end if
				elseif complete="2" then
					Response.write "" & vbcrlf & "                                         底单到账<img src=""../images/jiantou.gif"">" & vbcrlf & "                                         "
					if (open_7_20=3 or (open_7_20=1 and CheckPurview(intro_7_20,trim(cateid))=True  And cateid<>0)) And canPayback="1" And IsCompanyNoDel = true then
						Response.write "" & vbcrlf & "                                         <a href=""#"" onclick=""javascript:window.open('../../SYSN/view/finan/payback/PayBackSure.ashx?paybackId="
						Response.write pwurl(BillID)
						Response.write "','plancor6','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=120,top=100');return false;""><font class=""name"">收款</font></a>" & vbcrlf & "                                      "
						Response.write pwurl(BillID)
					end if
				elseif complete="3" then
					Response.write "" & vbcrlf & "                                         已收款"
					Response.write fkstr
				end if
				If ZBRuntime.MC(11000) Then
					if rs("tc")="1"  Then
						Response.write "已提成"
						if (open_44_3=3 or (open_44_3=1 and CheckPurview(intro_44_3,trim(cateid))=True  And cateid<>0) )  and rs("tc")=1 and complete="3" then
							Response.write "" & vbcrlf & "                                                         <img src=""../images/jiantou.gif""><a href=""tc_contract_cancel.asp?"
							Response.write sdk.ReturnURL("fromType=payback&ord=" & BillID &"")
							Response.write """ onclick=""return confirm('确认取消吗？')""><font class=""name"">取消提成</font></a>" & vbcrlf & "                                                     "
						end if
					else
						if (open_44_3=3 or (open_44_3=1 and CheckPurview(intro_44_3,trim(cateid))=True  And cateid<>0) )  and rs("tc")=0 and complete="3" And canTc(rs("contract"), conn) Then
							If cpCostChanged Then
								tcClick = "alert('产品成本发生变化，建议重新核算提成');return false;"
							else
								tcClick = "return confirm('确认提成？')"
							end if
							Response.write "" & vbcrlf & "                                                         <img src=""../images/jiantou.gif""><a href=""tc_contract.asp?"
							Response.write sdk.ReturnURL("fromType=payback&ord=" & BillID &"")
							Response.write """ onclick="""
							Response.write tcClick
							Response.write """><font class=""name"">提成</font></a>" & vbcrlf & "                                                        "
						end if
					end if
				end if
				if intro<>"" then
				...
				Response.write "" & vbcrlf & "                    <font color=""red"">" & vbcrlf & "                    "
				if complete="1" then
					Response.write "" & vbcrlf & "                                         未退款" & vbcrlf & "                                          "
					if open_9_13=3 or (open_9_13=1 and CheckPurview(intro_9_13,trim(cateid))=True  And cateid<>0) then
						Response.write "" & vbcrlf & "                                                 <img src=""../images/jiantou.gif""><a href=""#"" onclick=""javascript:window.open('../money3/addht2.asp?ord="
						Response.write pwurl(abs(BillID))
						Response.write "','plancor6','width=' + 950 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=280,top=150');return false;""><font color=""red"">退款</font></a>" & vbcrlf & "                                        "
						Response.write pwurl(abs(BillID))
					end if
				else
					Response.write "" & vbcrlf & "                                         已退款" & vbcrlf & "                                  "
				end if
				Response.write "" & vbcrlf & "                    </font>" & vbcrlf & "                "
			end if
			Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "                   <td><div align=""left"" style=""padding-left:10%;"
			Response.write "" & vbcrlf & "                    </font>" & vbcrlf & "                "
			if billID<0 then
			...
			if BillID>0 then
				If remind <> "" Then
					if helper.canCancelOrder(rid) then
						Response.write "<img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0"" style=""cursor:hand"" rid="""
						Response.write rid
						Response.write """ class=""cancelBtn"">"
					end if
				end if
				if open_7_14=3 or (open_7_14=1 and CheckPurview(intro_7_14,trim(cateid))=True  And cateid<>0) then
					Response.write "" & vbcrlf & "                                 <input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript: window.open('../../SYSN/view/finan/payback/PayBack.ashx?view=details&ord="
					Response.write pwurl(billID)
					Response.write "', 'new432win', 'width=' + 1300 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=50,top=50')"" class=""anybutton2""/> " & vbcrlf & "                        "
					Response.write pwurl(billID)
				end if
				canUpdate=true
				hklist.filter="id="& BillID
				if isOpen then canUpdate = hklist.eof
				if (open_7_2=3 or (open_7_2=1 And CheckPurview(intro_7_2,trim(cateid))=True  And cateid<>0)) And canPayback="1" and canUpdate then
					If complete<>"3" Then
						Response.write "" & vbcrlf & "                                         <input type=""button"" name=""Submit3"" value=""修改""  onClick=""javascript: window.open('addht.asp?paybackid="
						Response.write pwurl(BillID)
						Response.write "', 'plancor1', 'width=' + 900 + ',height=' + 450 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=350,top=180'); return false;"" title=""点击可修改此收款计划"" class=""anybutton2"">" & vbcrlf & "                                    "
						Response.write pwurl(BillID)
					else
						Response.write "" & vbcrlf & "                        <input type=""button"" name=""Submit3"" value=""修改""  onClick=""javascript: window.open('correct_intro.asp?ord="
						Response.write pwurl(BillID)
						Response.write "', 'plancor2', 'width=' + 900 + ',height=' + 450 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=350,top=180'); return false;"" title=""点击可修改此收款计划备注"" class=""anybutton2"">" & vbcrlf & "                                        "
						Response.write pwurl(BillID)
					end if
				end if
				if complete<>"3" and (open_7_3=3 or (open_7_3=1 and CheckPurview(intro_7_3,trim(cateid))=True  And cateid<>0)) And canPayback="1" and canUpdate then
					Response.write "" & vbcrlf & "                                 <input type=""button"" name=""Submit"" value=""删除"" onClick=""if (confirm('确认删除？')) { window.location.href = '../contract/Deletepay.asp?ord="
					Response.write BillID
					Response.write "&"
					Response.write ForwardParams("ord,SSS,CurrPage1",3)
					Response.write "&SSS&"
					Response.write Replace(Request.QueryString,"CurrPage","CurrPage1")
					Response.write "'; }"" class=""anybutton2""/>" & vbcrlf & "                      "
				end if
			else
				if open_9_14=3 or (open_9_14=1 and CheckPurview(intro_9_14,trim(cateid))=True  And cateid<>0) then
					Response.write "<input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript: window.open('../../SYSN/view/finan/payback/Payout2Detail.ashx?view=details&ord="
					Response.write pwurl(abs(billID))
					Response.write "', 'new432win', 'width=' + 1300 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=50,top=50')"" class=""anybutton2""/> " & vbcrlf & "                        "
					Response.write pwurl(abs(billID))
				end if
				canUptk = true
				tklist.filter="id="& abs(BillID)
				if isTkOpen then  canUptk =tklist.eof
				if canUptk and complete<>"2" and (open_9_2=3 or (CheckPurview(intro_9_2,trim(cateid))=True and open_9_2=1) ) Then
					Response.write "<input type=""button"" name=""Submit"" value=""修改"" onClick=""javascript: window.open('../money3/correct.asp?ord="
					Response.write pwurl(rs("contract"))
					Response.write "&rd="
					Response.write pwurl(abs(BillID))
					Response.write "', 'new433win', 'width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=300,top=150') ""/>"
					Response.write pwurl(abs(BillID))
				end if
				if  canUptk and  IsDeletePayout2(abs(BillID))   and (open_9_3=3 or (open_9_3=1 and CheckPurview(intro_9_3,trim(cateid))=True  And cateid<>0))  and ((completeType=2 and IsDeletePayout2Bybankin2(abs(rs("ord")))) or completeType <>2) then
					Response.write "<input type=""button"" name=""Submit"" value=""删除"" onClick=""if (confirm('确认删除？')) { window.location.href = '../money3/deletepay2.asp?"
					Response.write sdk.ReturnURL("Ismode=2&ord=" & abs(BillID) &"")
		...
		loop
		m=n
		bzname = sdk.getSqlValue("select intro from sortbz where id = 14","RMB")
		Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   <td height=""30"" colspan=""2"" align=""right"">本页合计：</td>" & vbcrlf & "                     "
		if ((A=7 or A=8) or (hkzt="4" or hkzt="5")) Then
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
			Response.write Formatnumber(total_money,num_dot_xs,-1)
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
			Response.write "</div></td>" & vbcrlf & "                          <td><div align=""left"" class=""red"">&nbsp;"
			Response.write bzname
			Response.write Formatnumber(total_money_hl,num_dot_xs,-1)
			Response.write bzname
			Response.write "</div></td>" & vbcrlf & "                          <td><div align=""right"">"
			Response.write Formatnumber(total_tc,num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "                          <td><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "                          <td colspan=""9"">&nbsp;</td>" & vbcrlf & "                       "
		else
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
			Response.write Formatnumber(total_money,num_dot_xs,-1)
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
			Response.write "</div></td>" & vbcrlf & "                          <td colspan=""11""><div align=""left"" class=""red"">&nbsp;"
			Response.write bzname
			Response.write Formatnumber(total_money_hl,num_dot_xs,-1)
			Response.write bzname
			Response.write "</div></td>" & vbcrlf & "                  "
		end if
		Response.write "" & vbcrlf & "             </tr>" & vbcrlf & "           <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   <td height=""30"" colspan=""2"" align=""right"">全部合计：</td>" & vbcrlf & "                     "
		if ((A=7 or A=8) or (hkzt="4" or hkzt="5")) then
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
			...
			Response.write bzname
			Response.write "</div></td>" & vbcrlf & "                          <td><div align=""right"">"
			Response.write Formatnumber(all_total_tc,num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "                          <td><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "                          <td colspan=""9"">&nbsp;</td>" & vbcrlf & "                       "
		else
			Response.write "" & vbcrlf & "                             <td><div align=""right"" class=""red"">"
		...
		Response.write "" & vbcrlf & "             </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td  class=""page"">" & vbcrlf & "                "
		Response.write "" & vbcrlf & "             <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""8%"" height=""25""><div align=""center"">全选&nbsp;<input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" /></div></td>" & vbcrlf & "                     <td width=""40%"">&nbsp;" & vbcrlf & "                        "
		if open_7_3=1 or open_7_3=3 then
			Response.write "<input type=""button"" onClick=""test();"" name=""Submit2"" value=""批量删除"" class=""anybutton""/>"
		end if
		if open_7_20=1 or open_7_20=3 then
			Response.write "" & vbcrlf & "                    <input type=""button"" onClick=""ask2();"" name=""Submit2"" value=""合并收款"" class=""anybutton""/>" & vbcrlf & "                    <input type=""button"" onClick=""ask5();"" name=""Submit2"" value=""批量收款"" class=""anybutton""/>"
		end if
		if (open_44_3=1 or open_44_3=3 ) And ZBRuntime.MC(11000) then
			Response.write "" & vbcrlf & "                    <input type=""button"" value=""提成发放""  onclick=""ask();"" class=""anybutton""/>" & vbcrlf & "                    <input type=""button"" value=""批量取消提成""  onclick=""ask4();"" class=""anybutton""/>" & vbcrlf & "                "
		end if
		if open_7_6=1 or open_7_6=3 then
			Response.write "" & vbcrlf & "                    <input type=""button"" onClick=""ask3();"" name=""Submit2"" value=""批量指派"" class=""anybutton""/>"
		end if
		Response.write "" & vbcrlf & "                 <input type=""hidden"" name=""backMode"" value=""1"" class=""anybutton2""/>" & vbcrlf & "                 </td>" & vbcrlf & "                   <td width=""52%""><div align=""right"">" & vbcrlf & "                 <span class=""black"">"
		Response.write recCount
		Response.write "个 | "
		...
		Response.write "页 | &nbsp;"
		Response.write page_count
		Response.write "条信息/页</span>&nbsp;&nbsp;" & vbcrlf & "                 <input name=""currpage"" id=""currpage""  type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "                   <input type=""button"" value=""跳转"" class=""anybutton2"" onclick=""gotourl('currPage='+document.getElementById('currpage').value);""/>" & vbcrlf & "                       "
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit4"" value=""首页"" class=""page"" onClick=""gotourl('currPage=1');"" />" & vbcrlf & "                 <input type=""button"" name=""Submit4"" value=""上一页"" class=""page"" onClick=""gotourl('currPage="
		...
		Response.write "" & vbcrlf & "                     </div></td>" & vbcrlf & "             </tr>" & vbcrlf & "           <tr><td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;</p></div></td></tr>" & vbcrlf & "              </table>" & vbcrlf & "                "
	end if
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "     </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150px;display:none;"">" & vbcrlf & "<table width=""150"" height=""300""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按应收日期排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按应收日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按计划日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按计划日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=5');""><font color=""#2F496E"">按应收金额排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按应收金额排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=11');""><font color=""#2F496E"">按销售人员排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=12');""><font color=""#2F496E"">按销售人员排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "       <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=13');""><font color=""#2F496E"">按客户名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourleight:120px;border:0px;padding:1px;z-index:9999;""id=""dhtml"">" & vbcrlf & "</div>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	set rs=nothing
	action1="收款列表"
	call close_list(1)
