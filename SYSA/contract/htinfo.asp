...
'If ii<Len(formula) then
	...
	Sub PageLoad()
		Dim strFilePath
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery-1.4.2.min.js""></script>"
'Dim strFilePath
		Response.write(app.DefTopBarHTML("../", strFilePath, "目标设置", "")) & vbCrLf
		Response.write("<script type=""text/javascript"">") & vbCrLf
		Response.write("function doSet(id,ids,sellId) {") & vbCrLf
		Response.write("    window.open('?__msgId=ShowInfo&years='+id+'&types='+ids+'&sellId='+sellId+'','','width=1024,height=620,fullscreen=no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100');") & vbCrLf
		Response.write("function doSet(id,ids,sellId) {") & vbCrLf
		Response.write("}") & vbCrLf
		Response.write("function DelContent(id,sellId) {" & vbCrLf)
		Response.write("    $.get('?__msgId=doDel', {" & vbCrLf)
		Response.write("        years: id," & vbCrLf)
		Response.write("        sellType: sellId" & vbCrLf)
		Response.write("    }," & vbCrLf)
		Response.write("    function(data) {" & vbCrLf)
		Response.write("        if (data == 1) {" & vbCrLf)
		...
		Response.write("</script>") & vbCrLf
		Dim oSQL, strHtml
		oSQL = "Select distinct years As '时间',Case When sellType = 0 Then '销售额' When sellType=1 Then '回款额' When sellType=2 Then '洽谈进展' Else '跟进客户数' End As '项目',sellType,'1' As '操作'"
		oSQL = oSQL & "from sys_bill_TarSet "
		oSQL = oSQL & " Order By years "
		Dim lvw
		...
		lvw.addlink = ""
		lvw.sql = oSQL
		strHtml = "<input type=""button"" value="" 详 情 "" class=""oldbutton"" onclick=""doSet(@cells[2],1,@cells[4]);"" /> "
		strHtml = strHtml & "<input type=""button"" value="" 修 改 "" class=""oldbutton"" onclick=""doSet(@cells[2],0,@cells[4]);"" /> "
		strHtml = strHtml & "<input type=""button"" value="" 删 除 "" class=""oldbutton"" onclick=""if(confirm('确认删除？')){DelContent(@cells[2],@cells[4]);}"" />"
		lvw.headers(3).display = "none"
		lvw.headers(4).formatText = strHtml
		Response.write("<table id=""content"" style='border-bottom:0px;'>") & vbCrLf
		lvw.headers(4).formatText = strHtml
		Response.write("<tr>") & vbCrLf
		Response.write("<td align=""right"" class='resetHeadBg' style='border-bottom:0px;background:#f4fafe;height:55px!important;'><input type=""button"" value=""目标设置"" class=""oldbutton3"" onclick=""doSet(0,0,0);"" />&nbsp;&nbsp;</td>") & vbCrLf
		Response.write("<tr>") & vbCrLf
		Response.write("</tr>") & vbCrLf
		Response.write("</table>") & vbCrLf
		Response.write("<div style=""overflow:hidden"">"&lvw.html&"</div>") & vbCrLf
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub doDel()
		Dim years, oSQL, sellType
		years = ChkNumeric(app.GetInt("years"))
		sellType = ChkNumeric(app.GetInt("sellType"))
		oSQL = "Delete From sys_bill_TarSet Where years = "&years&" And sellType = " & sellType & " "
		Cn.Execute(oSQL)
		Response.write(1)
		call db_close : Response.end()
	end sub
	Sub ShowInfo()
		Dim strFilePath, types, BtnType, years, sellType, h , i
		types = ChkNumeric(app.GetInt("types"))
		years = ChkNumeric(app.GetInt("years"))
		If years = 0 Then
			years = Year(Now())
		end if
		sellType = ChkNumeric(app.GetInt("sellId"))
		If isEmpty(num_dot_xs) Then
		...
		Response.write(app.DefTopBarHTML("../", strFilePath, "目标设置", "")) & vbCrLf
		Response.write("<script type=""text/javascript"">" & vbCrLf)
		Response.write("var types = "&types&";" & vbCrLf)
		Response.write("var IsDoSearch = 0;" & vbCrLf)
		Response.write("function doSet(id,ids,sellId) {") & vbCrLf
		Response.write("    window.open('?__msgId=ShowInfo&years='+id+'&types='+ids+'&sellId='+sellId+'','','width=1024,height=620,fullscreen=no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100');") & vbCrLf
		Response.write("function doSet(id,ids,sellId) {") & vbCrLf
		Response.write("}") & vbCrLf
		Response.write("function doSearch() {" & vbCrLf)
		Response.write("    if (IsDoSearch == 1) {" & vbCrLf)
		Response.write("        var lvw = new Listview('ids1');" & vbCrLf)
		Response.write("        lvw.beginCallBack('mysort');" & vbCrLf)
		Response.write("        lvw.addParam('W1', parseInt($('#W1').val()));" & vbCrLf)
		Response.write("        lvw.addParam('W2', parseInt($('#W2').val()));" & vbCrLf)
		Response.write("        lvw.addParam('W3', parseInt($('#W3').val()));" & vbCrLf)
		Response.write("        lvw.addParam('years', parseInt($('#years').val()));" & vbCrLf)
		Response.write("        lvw.addParam('sellType', parseInt($('#sellType').val()));" & vbCrLf)
		Response.write("        lvw.addParam('types', types);" & vbCrLf)
		Response.write("        lvw.exec();" & vbCrLf)
		Response.write("    }" & vbCrLf)
		Response.write("}" & vbCrLf)
		Response.write("function fh(obj) {obj.select()}" & vbCrLf)
		Response.write "function tarStat(mtarid){" & vbCrLf
		Response.write "     var arr_mtar = mtarid.split(""_"");" & vbCrLf
		Response.write "     var monthid = arr_mtar[0];" & vbCrLf
		Response.write "     var gateid = arr_mtar[1];" & vbCrLf
		Response.write "     var allMonthMoney = 0;" & vbCrLf
		Response.write "     var allGateMoney = 0;" & vbCrLf
		Response.write "     var dot_num = "& num_dot_xs&";" & vbCrLf
		Response.write "     var curSum, allSum, allSum2;" & vbCrLf
		Response.write "     curSum = Number($(""#curSum_""+monthid).val());" & vbCrLf
		Response.write "     var curSum, allSum, allSum2;" & vbCrLf
		Response.write "     allSum = Number($(""#allSum_""+monthid).val());" & vbCrLf
		Response.write "     var curSum, allSum, allSum2;" & vbCrLf
		Response.write "     $(""input[name='""+monthid+""']"").each(function(){" & vbCrLf
		Response.write "     var curSum, allSum, allSum2;" & vbCrLf
		Response.write "        allMonthMoney += Number($(this).val());                        " & vbCrLf
		Response.write "     var curSum, allSum, allSum2;" & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "     $(""#MStat_""+monthid+""_0"").html(formatNumDot(allMonthMoney,dot_num)); " & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "     allSum2 = (allSum - curSum) + allMonthMoney;" & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "     $(""#MStat_""+monthid+""_1"").html(formatNumDot(allSum2,dot_num));" & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "             $(""input[id$='_""+gateid+""']"").each(function(){" & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "        allGateMoney += Number($(this).val());                        " & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "     $(""#stat""+gateid).html(formatNumDot(allGateMoney,dot_num));" & vbCrLf
		Response.write "     }); " & vbCrLf
		Response.write "}" & vbCrLf
		Response.write("</script>" & vbCrLf)
		Dim oSQL, strHtml
		oSQL = "Select '&nbsp;' + IsNull(b.sort1,'')+'－'+IsNull(c.sort2,'') As '所在部门',IsNull(a.name,'') As '人员',"
'Dim oSQL, strHtml
		oSQL = oSQL & "IsNull(d.m1,0) As '1月份',IsNull(d.m2,0) As '2月份',IsNull(d.m3,0) As '3月份',"
		oSQL = oSQL & "IsNull(d.m4,0) As '4月份',IsNull(d.m5,0) As '5月份',IsNull(d.m6,0) As '6月份',"
		oSQL = oSQL & "IsNull(d.m7,0) As '7月份',IsNull(d.m8,0) As '8月份',IsNull(d.m9,0) As '9月份',"
		oSQL = oSQL & "IsNull(d.m10,0) As '10月份',IsNull(d.m11,0) As '11月份',IsNull(d.m12,0) As '12月份',a.ord, '&nbsp;' 合计 "
		oSQL = oSQL & " from gate a left join gate1 b on  a.sorce = b.ord "
		oSQL = oSQL & " left join gate2 c on  a.sorce2 = c.ord "
		oSQL = oSQL & " left join sys_bill_TarSet d On a.ord = d.preid "
		If years > 0 Then
			oSQL = oSQL & " And d.years = "&years&" "
		end if
		oSQL = oSQL & " And d.sellType = "&sellType&" "
		Dim lvw , rs , pws
		set rs = cn.execute("select (case qx_open when 3 then '' when 0 then '0' else w3 end) as r from power2 where cateid="&session("personzbintel2007")&" and sort1=2")
		if rs.eof = false then
			pws = "@" & replace(rs.fields(0).value & ""," ","") & "@"
			pws = replace(pws,"@,","")
			pws = replace(pws,",@","")
			pws = replace(pws,"@","")
		else
		...
		rs.close
		if len(pws) > 0 then
			oSQL = oSQL & " where a.ord in (" & pws & ")"
			oSQL = oSQL & " and a.del=1 "
		else
			oSQL = oSQL & " where a.del=1 "
		end if
		oSQL = oSQL & "  order by isnull(b.gate1,0), isnull(b.ord,0), isnull(c.gate2,0) , isnull(c.ord,0), a.ord"
		Set lvw = New listview
		lvw.checkbox = False
		lvw.indexbox = False
		lvw.currsum = true
		...
		h.width = 150
		h.align = "left"
		lvw.headers(3).formatText = "code:getColHtml(@cells[15],""m1"",@value,"& types &","& years &")"
		lvw.headers(4).formatText = "code:getColHtml(@cells[15],""m2"",@value,"& types &","& years &")"
		lvw.headers(5).formatText = "code:getColHtml(@cells[15],""m3"",@value,"& types &","& years &")"
		lvw.headers(6).formatText = "code:getColHtml(@cells[15],""m4"",@value,"& types &","& years &")"
		lvw.headers(7).formatText = "code:getColHtml(@cells[15],""m5"",@value,"& types &","& years &")"
		lvw.headers(8).formatText = "code:getColHtml(@cells[15],""m6"",@value,"& types &","& years &")"
		lvw.headers(9).formatText = "code:getColHtml(@cells[15],""m7"",@value,"& types &","& years &")"
		lvw.headers(10).formatText = "code:getColHtml(@cells[15],""m8"",@value,"& types &","& years &")"
		lvw.headers(11).formatText = "code:getColHtml(@cells[15],""m9"",@value,"& types &","& years &")"
		lvw.headers(12).formatText = "code:getColHtml(@cells[15],""m10"",@value,"& types &","& years &")"
		lvw.headers(13).formatText = "code:getColHtml(@cells[15],""m11"",@value,"& types &","& years &")"
		lvw.headers(14).formatText = "code:getColHtml(@cells[15],""m12"",@value,"& types &","& years &")"
		lvw.headers(1).canSum = False
		lvw.headers(2).canSum = False
		for i = 2 to 15
			lvw.headers(i).width = 70
			If i>2 And i<15 Then
				lvw.headers(i).dbtype = "money"
			end if
		next
		lvw.headers(15).display = "none"
		lvw.headers(16).formattext = "code:getTrSum(@cells[15],@cells[3],@cells[4],@cells[5],@cells[6],@cells[7],@cells[8],@cells[9],@cells[10],@cells[11],@cells[12],@cells[13],@cells[14])"
		lvw.headers(16).width = 90
		lvw.headers(16).align = "right"
		If types = 0 Then
			Response.write("<form action=""?__msgId=doSave"" method=""post"">") & vbCrLf
			BtnType = "<input type=""submit"" value=""保存"" class=""button"" />"
		else
			BtnType = "<input type=""button"" value=""修改"" class=""button"" onclick=""doSet("&years&",0,0);"" />"
		end if
		Response.write("<table id=""content"">") & vbCrLf
		Response.write("<tr>") & vbCrLf
		Response.write("<td align=""right"" class='resetHeadBg' colspan=""14"" style='height:55px!important;background:#f4fafe;'>" & BtnType & "&nbsp;</td>") & vbCrLf
		Response.write("</tr>") & vbCrLf
		Response.write("<tr>") & vbCrLf
		Response.write("<td align=""center"" colspan=""14"" style='border-bottom:0px'>") & vbCrLf
		Response.write("<tr>") & vbCrLf
		Response.write(SearchList(years, sellType)) & vbCrLf
		...
		Response.write("<script type=""text/javascript"">") & vbCrLf
		Response.write("window.onload = function(){IsDoSearch = 1;}") & vbCrLf
		Response.write("</script>") & vbCrLf
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub App_lvw_onSumProc(lvw, db,  mtype, html)
		...
		If db <> "@label" Then
			mtar = Formatnumber(html,num_dot_xs,true,0,0)
			If mtype = 0 Then
				html = "<div id=""MStat_m"& Replace(db,"月份","") &"_0"" style='word-wrap:break-word; word-break:normal; text-align:right'>" & mtar & "</div><input type='hidden' name='curSum_m"& Replace(db,"月份","") &"' id='curSum_m"& Replace(db,"月份","") &"' value='"& mtar &"'>"
'If mtype = 0 Then
			ElseIf mtype = 1 Then
				html = "<div id=""MStat_m"& Replace(db,"月份","") &"_1"" style='word-wrap:break-word; word-break:normal; text-align:right'>" & mtar & "</div><input type='hidden' name='allSum_m"& Replace(db,"月份","") &"' id='allSum_m"& Replace(db,"月份","") &"' value='"& mtar &"'>"
'ElseIf mtype = 1 Then
			end if
		end if
	end sub
	Function getColHtml(mxid,colName,colValue,revID, revID2)
		Dim tempStr, types, years, colMonth
		tempStr = ""
		If isEmpty(num_dot_xs) Then
			num_dot_xs = info.MoneyNumber
		end if
		types = revID : years = revID2 : colMonth = CInt(Replace(colName,"m",""))
		colValue = Formatnumber(colValue,num_dot_xs,true,0,0)
		If colName = "m1" Then
			tempStr = app.IIf(types = 0, "<input onpropertychange='formatData(this,""money"");tarStat(this.id);' onfocus='fh(this)' type=""text"" maxlength=10 name="""& colName &""" id="""& colName &"_"& mxid &""" value="""& colValue &""" size=""8"" class=""textbox"""&app.IIf(LoadTime(years, colMonth), " readonly=""readonly"" style=""background:#F0F0F0""", "")&" /><input type=""hidden"" name=""preid"" value="""& mxid &""" />", ""& colValue &"")
		else
			tempStr = app.IIf(types = 0, "<input onpropertychange='formatData(this,""money"");tarStat(this.id);' onfocus='fh(this)' type=""text"" maxlength=10 name="""& colName &""" id="""& colName &"_"& mxid &""" value="""& colValue &""" size=""8"" class=""textbox"""&app.IIf(LoadTime(years, colMonth), " readonly=""readonly"" style=""background:#F0F0F0""", "")&" />", ""& colValue &"")
		end if
		getColHtml = tempStr
	end function
	Sub lvw_defcallback(cmd, lvw)
		Dim W1, W2, W3, years, sellType, types
		W1 = ChkNumeric(Request("W1"))
		W2 = ChkNumeric(Request("W2"))
		W3 = ChkNumeric(Request("W3"))
		years = ChkNumeric(Request("years"))
		sellType = ChkNumeric(Request("sellType"))
		types = ChkNumeric(Request("types"))
		Select Case cmd
		Case "mysort"
		Call TempOrder(W1, W2, W3, years, sellType, types, lvw)
		End Select
	end sub
	Sub TempOrder(W1, W2, W3, years, sellType, types, lvw)
		Dim oSQL, strHtml, strWhere, strSQL , i, h
		strWhere = " Where 1 = 1 "
		oSQL = "Select '&nbsp;' + IsNull(b.sort1,'')+'－'+IsNull(c.sort2,'') As '所在部门',IsNull(a.name,'') As '人员',"
		strWhere = " Where 1 = 1 "
		oSQL = oSQL & "IsNull(d.m1,0) As '1月份',IsNull(d.m2,0) As '2月份',IsNull(d.m3,0) As '3月份',"
		...
		oSQL = oSQL & " And d.sellType = "& sellType &" "
		If W1 > 0 Then
			strWhere = strWhere & " And b.ord = "& W1 &" "
		end if
		If W2 > 0 Then
			strWhere = strWhere & " And c.ord = "& W2 &" "
		end if
		If W3 > 0 Then
			strWhere = strWhere & " And a.ord = "& W3 &" "
		end if
		Dim  rs , pws
		set rs = cn.execute("select (case qx_open when 3 then '' when 0 then '0' else w3 end) as r from power2 where cateid="&session("personzbintel2007")&" and sort1=2")
		if rs.eof = false then
		...
		rs.close
		if len(pws) > 0 then
			strWhere = strWhere & " and a.ord in (" & pws & ")"
		end if
		strWhere=strWhere&" and a.del=1 "
		strSQL = oSQL & strWhere
		strSQL = strSQL & "  order by isnull(b.gate1,0), isnull(b.ord,0), isnull(c.gate2,0) , isnull(c.ord,0), a.ord"
		lvw.sql = strSQL
		set h = lvw.headers(1)
		h.width = 150
		h.align = "left"
	...
	Sub doSave()
		Dim m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12, preid, years, sellType
		m1 = app.GetText("m1")
		m2 = app.GetText("m2")
		m3 = app.GetText("m3")
		m4 = app.GetText("m4")
		...
		m11 = app.GetText("m11")
		m12 = app.GetText("m12")
		preid = app.GetText("preid")
		years = app.GetInt("years")
		sellType = app.GetInt("sellType")
		If preid <> "" Then
			Dim arrPreid, arrM1, arrM2, arrM3, arrM4, arrM5, arrM6, arrM7, arrM8, arrM9, arrM10, arrM11, arrM12
			Dim oSQL, oRs, i, depId, groupId, preName
			Dim SQL, Rs
			arrPreid = Split(preid, ",")
			arrM1 = Split(m1, ",")
			arrM2 = Split(m2, ",")
			arrM3 = Split(m3, ",")
			arrM4 = Split(m4, ",")
			...
			For i = 0 To UBound(arrPreid)
				oSQL = "Select * From sys_bill_TarSet Where preid = " & arrPreid(i) & " And years = "&years&" And sellType = "&sellType&" "
				...
				depId = 0
				groupId = 0
				SQL = "Select a.name,b.ord,c.ord,a.ord "
				SQL = SQL & " from gate a inner join gate1 b on a.sorce = b.ord inner join gate2 c on a.sorce2 = c.ord left join  "
				SQL = SQL & " sys_bill_TarSet d On a.ord = d.preid Where a.del=1 and a.ord = " & arrPreid(i) & " "
				Set Rs = server.CreateObject("adodb.recordset")
				Rs.Open SQL, cn, 1, 1
				If Not Rs.EOF Then
					preName = Rs(0)
				...
				oRs("depid") = depId
				oRs("groupid") = groupId
				oRs("preid") = arrPreid(i)
				oRs("prename") = preName
				oRs("sellType") = sellType
				If Len(arrM1(i)) >0 Then
					oRs("m1") = app.IIf(IsNumeric(arrM1(i)), round(arrM1(i),info. MoneyNumber), 0)
				end if
				If Len(arrM2(i)) >0 Then
					oRs("m2") = app.IIf(IsNumeric(arrM2(i)), round(arrM2(i),info. MoneyNumber), 0)
				end if
				If Len(arrM3(i)) >0 Then
					oRs("m3") = app.IIf(IsNumeric(arrM3(i)), round(arrM3(i),info. MoneyNumber), 0)
				end if
				If Len(arrM4(i)) >0 Then
					oRs("m4") = app.IIf(IsNumeric(arrM4(i)), round(arrM4(i),info. MoneyNumber), 0)
				end if
				If Len(arrM5(i)) >0 Then
					oRs("m5") = app.IIf(IsNumeric(arrM5(i)), round(arrM5(i),info. MoneyNumber), 0)
				end if
				If Len(arrM6(i)) >0 Then
					oRs("m6") = app.IIf(IsNumeric(arrM6(i)), round(arrM6(i),info. MoneyNumber), 0)
				end if
				If Len(arrM7(i)) >0 Then
					oRs("m7") = app.IIf(IsNumeric(arrM7(i)), round(arrM7(i),info. MoneyNumber), 0)
				end if
				If Len(arrM8(i)) >0 Then
					oRs("m8") = app.IIf(IsNumeric(arrM8(i)), round(arrM8(i),info. MoneyNumber), 0)
				end if
				If Len(arrM9(i)) >0 Then
					oRs("m9") = app.IIf(IsNumeric(arrM9(i)), round(arrM9(i),info. MoneyNumber), 0)
				end if
				If Len(arrM10(i)) >0 Then
					oRs("m10") = app.IIf(IsNumeric(arrM10(i)), round(arrM10(i),info. MoneyNumber), 0)
				end if
				If Len(arrM11(i)) >0 Then
					oRs("m11") = app.IIf(IsNumeric(arrM11(i)), round(arrM11(i),info. MoneyNumber), 0)
				end if
				If Len(arrM12(i)) >0 Then
					oRs("m12") = app.IIf(IsNumeric(arrM12(i)), round(arrM12(i),info. MoneyNumber), 0)
				end if
				oRs("uptime") = Now()
				oRs.Update
				oRs.Close
		...
		Response.write("window.open('','_self');") & vbCrLf
		Response.write("window.close();") & vbCrLf
		Response.write("</script>") & vbCrLf
	end sub
	Function SearchList(YearList, sellType)
		Dim strTemp, i
		strTemp = SelPreList()
		strTemp = strTemp & "<select name=""years"" id=""years"" onchange=""doSearch();"" style='margin-right:5px;'>"
		strTemp = SelPreList()
		For i = 2010 To Year(Now()) + 10
			strTemp = SelPreList()
			strTemp = strTemp & "<option value="""&i&""""&app.IIf(Trim(YearList) = Trim(i), " selected=""selected""", "")&">"&i&"</option>"
		next
		strTemp = strTemp & "</select>"
		strTemp = strTemp & "<select name=""sellType"" id=""sellType"" onchange=""doSearch();"">"
		strTemp = strTemp & "<option value=""0"""&app.IIf(sellType = 0, " selected=""selected""", "")&">销售额</option>"
		strTemp = strTemp & "<option value=""1"""&app.IIf(sellType = 1, " selected=""selected""", "")&">回款额</option>"
		strTemp = strTemp & "<option value=""2"""&app.IIf(sellType = 2, " selected=""selected""", "")&">洽谈进展</option>"
		strTemp = strTemp & "<option value=""3"""&app.IIf(sellType = 3, " selected=""selected""", "")&">跟进客户数</option>"
		strTemp = strTemp & "</select>"
		SearchList = strTemp
	end function
	Function SelPreList()
		Dim strHtml, oSQL, oRs
		Dim  rs , pws,strWhere1
		set rs = cn.execute("select (case qx_open when 3 then '' when 0 then '0' else w3 end) as r from power2 where cateid="&session("personzbintel2007")&" and sort1=2")
		if rs.eof = false then
			pws = "@" & replace(rs.fields(0).value & ""," ","") & "@"
			pws = replace(pws,"@,","")
		...
		rs.close
		if len(pws) > 0 then
			strWhere1 = strWhere1 & " and gate.ord in (" & pws & ")"
		end if
		Dim lustr, Str_power3
		oSQL = "select ord,name,sorce,sorce2 from gate where ord>0 and del=1 "&strWhere1&" order by cateid"
		Set oRs = Cn.Execute(oSQL)
		lustr = ""
		Do While Not oRs.EOF
			If lustr = "" Then
				lustr = "['"&oRs(0)&"','"&oRs(1)&"','"&oRs(2)&"','"&oRs(3)&"']"
			else
				lustr = lustr&","&"['"&oRs(0)&"','"&oRs(1)&"','"&oRs(2)&"','"&oRs(3)&"']"
			end if
			oRs.MoveNext
		Loop
		oRs.Close
		Set oRs = Nothing
		strWhere1=""
		set rs = cn.execute("select (case qx_open when 3 then '' when 0 then '0' else w1 end) as r from power2 where cateid="&session("personzbintel2007")&" and sort1=2")
		if rs.eof = false then
			pws = "@" & replace(rs.fields(0).value & ""," ","") & "@"
			pws = replace(pws,"@,","")
			pws = replace(pws,",@","")
		...
		rs.close
		if len(pws) > 0 then
			strWhere1 = strWhere1 & " and gate1.ord in (" & pws & ")"
		end if
		Dim gt1str
		oSQL = "select ord,sort1 from gate1 Where ord>0 "&strWhere1
		Set oRs = Cn.Execute(oSQL)
		gt1str = ""
		Do While Not oRs.EOF
			If gt1str = "" Then
		...
		Dim gt2str
		strWhere1=""
		set rs = cn.execute("select (case qx_open when 3 then '' when 0 then '0' else w2 end) as r from power2 where cateid="&session("personzbintel2007")&" and sort1=2")
		if rs.eof = false then
			pws = "@" & replace(rs.fields(0).value & ""," ","") & "@"
			pws = replace(pws,"@,","")
			pws = replace(pws,",@","")
		...
		set rs=nothing
		if len(pws) > 0 then
			strWhere1 = strWhere1 & " and gate2.ord in (" & pws & ")"
		end if
		oSQL = "select ord,sort2,sort1 from gate2 where ord>0 "
		Set oRs = Cn.Execute(oSQL)
		gt2str = ""
		Do While Not oRs.EOF
			If gt2str = "" Then
				gt2str = "['"&oRs(0)&"','"&oRs(1)&"','"&oRs(2)&"']"
			else
				gt2str = gt2str&",['"&oRs(0)&"','"&oRs(1)&"','"&oRs(2)&"']"
			end if
			oRs.MoveNext
		Loop
		oRs.Close
		Set oRs = Nothing
		strHtml = ""
		strHtml = strHtml & "<select name=""W1"" id=""W1"" onchange=""chgOthers(1);doSearch();"">" & vbCrLf
		strHtml = strHtml & "  <option value=""0"">所属部门</option>" & vbCrLf
		strHtml = strHtml & "</select>" & vbCrLf
		strHtml = strHtml & "<select name=""W2"" id=""W2"" onchange=""chgOthers(2);doSearch();"">" & vbCrLf
		strHtml = strHtml & "  <option value=""0"">所属小组</option>" & vbCrLf
		strHtml = strHtml & "</select>" & vbCrLf
		strHtml = strHtml & "<select name=""W3"" id=""W3"" onchange=""chgOthers(3);doSearch();"">" & vbCrLf
		strHtml = strHtml & "  <option value=""0"">所属人员</option>" & vbCrLf
		strHtml = strHtml & "</select>" & vbCrLf
		strHtml = strHtml & SelPreListJs(lustr, gt1str, gt2str)
		SelPreList = strHtml
	end function
	Function SelPreListJs(str1, str2, str3)
		Dim strJs
		strJs = ""
		strJs = strJs & "<script type=""text/javascript"">" & vbCrLf
		strJs = strJs & "var ListUsers=["&str1&"];" & vbCrLf
		strJs = strJs & "var ListGate1=["&str2&"];" & vbCrLf
		strJs = strJs & "var ListGate2=["&str3&"];" & vbCrLf
		strJs = strJs & "for(var i=0;i<ListGate1.length;i++){document.getElementById(""W1"").options.add(new Option(ListGate1[i][1],ListGate1[i][0]));}" & vbCrLf
		strJs = strJs & "var ListGate2=["&str3&"];" & vbCrLf
		strJs = strJs & "for(var i=0;i<ListUsers.length;i++){document.getElementById(""W3"").options.add(new Option(ListUsers[i][1],ListUsers[i][0]));}" & vbCrLf
		strJs = strJs & "var ListGate2=["&str3&"];" & vbCrLf
		strJs = strJs & "function chgOthers(f) {" & vbCrLf
		strJs = strJs & "    var sobj1 = document.getElementById(""W1"");" & vbCrLf
		strJs = strJs & "    var sobj2 = document.getElementById(""W2"");" & vbCrLf
		strJs = strJs & "    var sobj3 = document.getElementById(""W3"");" & vbCrLf
		strJs = strJs & "    if (f == 1) {" & vbCrLf
		strJs = strJs & "        RemoveAll(sobj2);" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "        for (var i = 0; i < ListGate2.length; i++) {" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "            if (ListGate2[i][2] == sobj1.value) OptionAdd(sobj2, ListGate2[i][1], ListGate2[i][0])" & vbCrLf
		strJs = strJs & "        }" & vbCrLf
		strJs = strJs & "        RemoveAll(sobj3);" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "        for (var i = 0; i < ListUsers.length; i++) {" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "            if (ListUsers[i][2] == sobj1.value || sobj1.value == '') OptionAdd(sobj3, ListUsers[i][1], ListUsers[i][0])" & vbCrLf
		strJs = strJs & "        }" & vbCrLf
		strJs = strJs & "    } else if (f == 2) {" & vbCrLf
		strJs = strJs & "        RemoveAll(sobj3);" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "        for (var i = 0; i < ListUsers.length; i++) {" & vbCrLf
		strJs = strJs & "        OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "            if (ListUsers[i][2] == sobj1.value && (sobj2.value == '' || ListUsers[i][3] == sobj2.value)) OptionAdd(sobj3, ListUsers[i][1], ListUsers[i][0])" & vbCrLf
		strJs = strJs & "        }" & vbCrLf
		strJs = strJs & "    } else if (f == 3) {" & vbCrLf
		strJs = strJs & "        var tmp;" & vbCrLf
		strJs = strJs & "        if (sobj3.value == 0) {" & vbCrLf
		strJs = strJs & "            RemoveAll(sobj1);" & vbCrLf
		strJs = strJs & "            RemoveAll(sobj2);" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj1, ""所属部门"", """");" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "            for (var i = 0; i < ListGate1.length; i++) {" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "                document.getElementById(""W1"").options.add(new Option(ListGate1[i][1], ListGate1[i][0]))" & vbCrLf
		strJs = strJs & "            }" & vbCrLf
		strJs = strJs & "            RemoveAll(sobj3);" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "            for (var i = 0; i < ListUsers.length; i++) {" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj3, ""所属人员"", """");" & vbCrLf
		strJs = strJs & "                document.getElementById(""W3"").options.add(new Option(ListUsers[i][1], ListUsers[i][0]))" & vbCrLf
		strJs = strJs & "            }" & vbCrLf
		strJs = strJs & "        } else {" & vbCrLf
		strJs = strJs & "            for (var tmp = 0; tmp < ListUsers.length; tmp++) {" & vbCrLf
		strJs = strJs & "        } else {" & vbCrLf
		strJs = strJs & "                if (ListUsers[tmp][0] == sobj3.value) break" & vbCrLf
		strJs = strJs & "            }" & vbCrLf
		strJs = strJs & "            RemoveAll(sobj2);" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "            for (var i = 0; i < ListGate2.length; i++) {" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "                if (ListGate2[i][0] == ListUsers[tmp][3]) {" & vbCrLf
		strJs = strJs & "                    OptionAdd(sobj2, ListGate2[i][1], ListGate2[i][0]);" & vbCrLf
		strJs = strJs & "                }" & vbCrLf
		strJs = strJs & "            }" & vbCrLf
		strJs = strJs & "            if (sobj2.length == 0) OptionAdd(sobj2, ""所属小组"", """");" & vbCrLf
		strJs = strJs & "            sobj2.value = ListUsers[tmp][3] == '0' ? '': ListUsers[tmp][3];" & vbCrLf
		strJs = strJs & "            RemoveAll(sobj1);" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj1, ""所属部门"", """");" & vbCrLf
		strJs = strJs & "            for (var i = 0; i < ListGate1.length; i++) {" & vbCrLf
		strJs = strJs & "            OptionAdd(sobj1, ""所属部门"", """");" & vbCrLf
		strJs = strJs & "                if (ListGate1[i][0] == ListUsers[tmp][2]) {" & vbCrLf
		strJs = strJs & "                    OptionAdd(sobj1, ListGate1[i][1], ListGate1[i][0]);" & vbCrLf
		strJs = strJs & "                }" & vbCrLf
		strJs = strJs & "            }" & vbCrLf
		strJs = strJs & "            if (sobj1.length == 0) OptionAdd(sobj1, ""所属部门"", """");" & vbCrLf
		strJs = strJs & "            sobj1.value = ListUsers[tmp][2] == '0' ? '': ListUsers[tmp][2]" & vbCrLf
		strJs = strJs & "        }" & vbCrLf
		strJs = strJs & "    }" & vbCrLf
		strJs = strJs & "}" & vbCrLf
		strJs = strJs & "function RemoveAll(obj){while(obj.options[0]){obj.options.remove(0);}}" & vbCrLf
		strJs = strJs & "function OptionAdd(obj,skey,svalue){obj.options.add(new Option(skey,svalue));}" & vbCrLf
	...
	Function LoadTime(Str, str2)
		If (Str = "" Or Str2 = "") Then
			Exit Function
		else
			Dim strTime, strTimeTemp, strReturn
			strTime = CDate(Str & "-" & str2 & "-01")
'Dim strTime, strTimeTemp, strReturn
			If Month(Date) = 1 then
				strTimeTemp = CDate((Year(date)-1) & "-12-01")
'If Month(Date) = 1 then
			else
				strTimeTemp = CDate(Year(date) & "-" & (Month(Date) - 1) & "-01")
'If Month(Date) = 1 then
			end if
			If strTime < strTimeTemp Then
				strReturn = True
			else
				strReturn = False
			end if
	...
	Function getTrSum(gateid,m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, m11, m12)
		Dim trSum
		If isEmpty(num_dot_xs) Then
			num_dot_xs = info.MoneyNumber
		end if
		trSum = m1 + m2 + m3 + m4 + m5 + m6 + m7 + m8 + m9 + m10 + m11 + m12
		num_dot_xs = info.MoneyNumber
		trSum = Formatnumber(trSum,num_dot_xs,true,0,0)
		getTrSum = "<span id=""stat"&gateid&""">"& trSum &"</div>"
	end function
