...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "<style>#reason_0{width:440px;}html{padding:0 10px;background:#EFEFEF}</style>"
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		Dim mButtons,rs, sort , typeTitle ,secTitle ,f
		Dim title, bh, date1, reason, status, intro, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6
		Dim sqlStr, yhord
		yhord = app.base64.deurl(request("ord"))
		If yhord&""<>"" Then
			Set rs = cn.execute("select * from maintain where ord="& yhord)
			If rs.eof = True Then
		...
		While rs.eof = False
			...
			Case "zdy5" : fValue = zdy5
			Case "zdy6" : fValue = zdy6
			End Select
			If rs("sort") = 2 Then
				f.uitype = "text"
				f.value = fValue
	...
	function Bill_OnSave(ByVal bill)
		...
		rs.close
		set rs = nothing
		If sp>0 And cateid_sp>0 Then
			Set commSP = New CommSPHandle
			Call commSP.init("maintain",yhord)
			Call commSP.saveBill2(sp, cateid_sp, 0, 0)
			Set commSP = nothing
		end if
		Bill_OnSave = True
		Response.write "" & vbcrlf & "<script>" & vbcrlf & "try{" & vbcrlf & " var openHref = window.parent.opener.location.href;      " & vbcrlf & "        if(openHref.indexOf(""/maintain/planall.asp"")>0){" & vbcrlf & "          window.parent.opener.lvw_refresh(""mlistvw"")" & vbcrlf & "       }else if(openHref.indexOf(""/maintain/content.asp"")>0){" & vbcrlf & "         var openHref2 = window.parent.opener.opener.location.href;              " & vbcrlf & "                if(openHref2.indexOf(""/maintain/planall.asp"")>0){" & vbcrlf & "                 " & vbcrlf & "                        window.parent.opener.opener.lvw_refresh(""mlistvw"")" & vbcrlf & "                }" & vbcrlf & "               window.parent.opener.location.reload();" & vbcrlf & "  }" & vbcrlf & "}catch(e){}" & vbcrlf & "window.parent.opener=null;window.parent.open('','_self');window.parent.close();" & vbcrlf & "</script>" & vbcrlf & ""
	end function
%>
