...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,11).Value = "添加时间"
	...
	elseif open_102_10=1 then
		list=" and iss_addcateid in ("&intro_102_10&")"
	else
	...
	if open_102_1=3 then
	elseif open_102_1=1 then
		Str_Result=Str_Result&" and iss_addcateid in ("&intro_102_1&")"
	else
		Str_Result=Str_Result&" and iss_addcateid=0"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
...
'if ret2<>"" and ret2<>"点击选择时间" then
	...
	k=0
	i=1
	sql="select a.*,b.car_code from O_insure a"&_
	" left join O_carData b on a.iss_carid=b.car_id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "车辆保险列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			iss_cateid=rs("iss_cateid")
			...
			rs8.close
			set rs8=nothing
			iss_type=rs("iss_type")
			if iss_type="" or isnull(iss_type) then iss_type=0
			sql8="select * from O_carSet where setType=3 and id="&iss_type
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				iss_type=rs8("setName")
			else
				iss_type=""
			end if
			rs8.close
			set rs8=nothing
			if rs("iss_warn")=1 then
				iss_warn="是"
			elseif rs("iss_warn")=2 then
				iss_warn="否"
			end if
			iss_addcateid=rs("iss_addcateid")
			if iss_addcateid="" or isnull(iss_addcateid) then iss_addcateid=0
			sql8="select * from gate where ord="&iss_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				iss_addcateid=rs8("name")
			else
				iss_addcateid=""
			end if
			rs8.close
			set rs8=nothing
			money1=zbcdbl(rs("iss_money"))
			if money1="" or isnull(money1) then money1=0
			iss_startime=rs("iss_startime")
			iss_endtime=rs("iss_endtime")
			iss_addtime=rs("iss_addtime")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>车辆保险列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!车辆保险列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("车辆保险列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的车辆保险列表</u></strong></font></a></p>"
