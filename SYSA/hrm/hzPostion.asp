...
'if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
	...
	elseif open_85_11="1" then
		SQL_Tj=" and b.creator in ("&intro_85_11&")"
	end if
	baseSQL="select PAGE_COUNT_NUM from (select PAGE_TOP_NUM * from (select 序号_ID,关联招聘计划,职位,招聘人数_DOSUM,到岗人数_DOSUM,case 招聘人数_DOSUM when 0 then 0  else isnull((cast(到岗人数_DOSUM as float)/cast(招聘人数_DOSUM as float))*100,0) end as 完成比例 from (select b.id as 序号_ID,b.title as 关联招聘计划 ,dbo.hrGetPostion_sorceName(c.id) as 职位 ,isnull(a.num,0) as 招聘人数_DOSUM,isnull(a.hadnum,0) as  到岗人数_DOSUM from (select planID,postion,dbo.hrSumPostionNum(planID,postion) as num,dbo.hrSumPostionHadNum(planID,postion) as hadnum from hr_plan_list group by planID,postion) a left join hr_ret_plan b on a.planID=b.id left join hr_pub_postion c on c.id=a.postion left join gate1 d on c.sorce=d.ord left join gate2  e on c.sorce2=e.ord   where b.status=3 "&SQL_Tj&" and b.del=0) tmp) aaaavv PAGE_ORDER_STR) bbb  where 1=1"
...
'elseif hr_resume_a1=6 then
	...
	if hr_resume_e2<>"" then
		if hr_resume_e1=1 then
			strCondition=strCondition+" and 职位 like '%"& hr_resume_e2 &"%'"
'if hr_resume_e1=1 then
		elseif hr_resume_e1=2 then
			strCondition=strCondition+" and 职位 not like '%"& hr_resume_e2 &"%'"
'elseif hr_resume_e1=2 then
		elseif hr_resume_e1=3 then
			strCondition=strCondition+" and 职位='"&hr_resume_e2&"'"
'elseif hr_resume_e1=3 then
		elseif hr_resume_e1=4 then
			strCondition=strCondition+" and 职位<>'"&hr_resume_e2&"'"
'elseif hr_resume_e1=4 then
		elseif hr_resume_e1=5 then
			strCondition=strCondition+" and 职位 like '"& hr_resume_e2 &"%'"
'elseif hr_resume_e1=5 then
		elseif hr_resume_e1=6 then
			strCondition=strCondition+" and 职位 like '%"& hr_resume_e2 &"'"
'elseif hr_resume_e1=6 then
		end if
	end if
...
'elseif hr_resume_d1=6 then
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""width:510px;height:370px;padding:5px;background: #fafafa;top:0px;""  closed=""true"" >" & vbcrlf & "<form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "     <div region=""center"" border=""false"" style=""padding:2%;background:#fff;border:1px solid #ccc;width:96%;height:275px"">" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">关联招聘计划：</div></td>" & vbcrlf & "          <td><select name=""hr_resume_a1"">" & vbcrlf & "                                                   <option value=""1"">包含</option>" & vbcrlf & "                                                   <option value=""2"">不包含</option>" & vbcrlf & "                                                         <option value=""3"">等于</option>" & vbcrlf & "                                                  <option value=""4"">不等于</option>" & vbcrlf & "                                                         <option value=""5"">以..开始</option>" & vbcrlf & "                                                       <option value=""6"">以..结束</option>" & vbcrlf & "            </select>" & vbcrlf & "            <input name=""hr_resume_a2""  type=""text"" size=""15""></td>" & vbcrlf & "        </tr>" & vbcrlf & "            <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">职位：</div></td>" & vbcrlf & "          <td><select name=""hr_resume_e1"">" & vbcrlf & "                                                        <option value=""1"">包含</option>" & vbcrlf &  "                                                     <option value=""2"">不包含</option>" & vbcrlf &  "                                                        <option value=""3"">等于</option>" & vbcrlf &  "                                                  <option value=""4"">不等于</option> "& vbcrlf &  "                                                        <option value=""5"">以..开始</option>" & vbcrlf & "                                                       <option value=""6"">以..结束</option>" & "            </select>" & vbcrlf & "            <input name=""hr_resume_e2""  type=""text"" size=""15""></td>" & vbcrlf & "        </tr>" & vbcrlf & "                <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">招聘人数：</div></td>" & vbcrlf & "          <td><select name=""hr_resume_b1"">" & vbcrlf & "              <option value=""1"">等于</option>" & vbcrlf & "              <option value=""2"">大于</option>" & vbcrlf & "              <option value=""3"">大于等于</option>" & vbcrlf & "              <option value=""4"">小于</option>" & vbcrlf & "              <option value=""5"">小于等于</option>" & vbcrlf & "              <option value=""6"">不等于</option>" & vbcrlf &"             </select>" & vbcrlf &  "           <input name=""hr_resume_b2"" onKeyUp=""value=value.replace(/[^\d]/g,'')"" type=""text"" onpropertychange=me_c1"">" & vbcrlf & "              <option value=""1"">等于</option>" & vbcrlf & "              <option value=""2"">大于</option>" & vbcrlf & "              <option value=""3"">大于等于</option>" & vbcrlf & "              <option value=""4"">小于</option>" & vbcrlf & "              <option value=""5"">小于等于</option>" & vbcrlf & "              <option value=""6"">不等于</option>" & vbcrlf & "            </select>" & vbcrlf & "            <input name=""hr_resume_c2"" onKeyUp=""value=value.replace(/[^\d]/g,'')"" onpropertychange=""formatData(this,'int');"" type=""text"" size=""15""></td>" & vbcrlf & "        </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">完成比例：</div></td>" & vbcrlf & "          <td><select name=""hr_resume_d1"">" & vbcrlf & "              <option value=""1"">等于</option>"& vbcrlf & "              <option value=""2"">大于</option>" & vbcrlf & "              <option value=""3"">大于等于</option>" & vbcrlf &  "             <option value=""4"">小于</option>" & vbcrlf &  "             <option value=""5"">小于等于</option>" & vbcrlf &"               <option value=""6"">不等于</option>"& vbcrlf &  "           </select>" & vbcrlf & "            <input name=""hr_resume_d2"" onKeyUp=""value=value.replace(/[^\d]/g<div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "           <input type=""hidden"" name=""page_count"" value="""
'sub ShowSearchDiv()
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
