...
'bigsystemtype = ".mozi"
	...
	Class oExecel_J
		...
		public sub  class_initialize
			...
			excelApp.WorkBooks.Open(server.MapPath("../out/Null.xls"))
			set excelBook = excelApp.ActiveWorkBook
		end sub
		Private Sub Class_Terminate
			Set excelBook=Nothing
			Set excelSheet=Nothing
			IF Lcase(TypeName(ExcelApp))<>"nothing" Then ExcelApp.Quit
			set ExcelApp=Nothing
		end sub
		Public Function getVersion()
			If sVersion="" Then
				getVersion=excelApp.Application.Version
			else
				getVersion=sVersion
			end if
		end function
		Public Function DelSheet(SheetName)
			excelBook.Sheets(SheetName).Delete
		end function
		Public Function DelRow(RowName,sheetNameT)
			Set excelSheet = excelBook.Sheets(sheetNameT)
			excelSheet.Range(RowName).Delete
			Set excelSheet=Nothing
		end function
		...
		Public Function createSheet()
			...
			for i=0 to ubound(split(sheetName,","))
				If split(sheetName,",")(i)<>"" Then
					excelBook.Sheets.Add(,,,-4167).Name=rpSheetName(split(sheetName,",")(i))
'If split(sheetName,",")(i)<>"" Then
					If existsFun("execelFields") Then
						Set excelSheet = excelBook.Sheets(rpSheetName(split(sheetName,",")(i)))
						execelFields excelSheet
						Set excelSheet=Nothing
					end if
				end if
			next
		...
		Public Function saveExecel()
			If Not existsFun("execelFields") Then Response.write "意外的调用了组件，需要函数execelFields！":Exit Function
			If sheetNum="" Then sheetNum=1
			If sheetNum<1 Then sheetNum=1
			Set excelSheet = excelBook.Sheets(sheetNum)
			saveExecel=execelFields(excelSheet)
			Set excelSheet = Nothing
			excelBook.SaveAs server.MapPath(ExecelName)
		end function
		Public Function CopyTable()
			If sheetSql<>"" Then
				...
				ExcelApp.Quit
				set ExcelApp=Nothing
				If sheetName="" Then sheetName="Sheet"&sheetNum
				conn.execute("insert into OpenDataSource('Microsoft.Jet.OLEDB.4.0','Data Source="&server.MapPath(ExecelName)&";User ID=;Password=;Extended properties=Excel 8.0')..."&sheetName&"$  "&sheetSql)
				If   ERR.Number<>0 Then
					err.clear()
					CopyTable=False
				else
		...
		Public Function setExecelAttribute()
		end function
		Public Function existsFun(Name)
			on error resume next
			Execute Name&"=1"
			ExistsFun = (Err.Number = 501 Or Err.Number = 450)
			On Error GoTo 0
		end function
	End Class
...
'if strW3<>"" Then
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>退货明细导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "<td class=""place"">退货明细导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf &  "   <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table> "  & vbcrlf & " <table width=""100%""border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出退货明细,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write Application("sys.info.jsver")
	Server.ScriptTimeOut=100000000
	dim MODULES
	MODULES=session("zbintel2010ms")
	if Not ZBRuntime.MC(13000) and Not ZBRuntime.MC(8000) and Not ZBRuntime.MC(17000) then
		conn.close()
		conn=Nothing
		call db_close : Response.end()
	end if
	Dim open_1_41,intro_1_41,open_14_41,intro_14_41,strWhere,strWhere1
	Dim LookQx,lookKhQx
	open_4_41=0
	intro_1_41=0
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.open sql1,conn,1,1
	if Not rs1.eof then
		open_1_41=rs1("qx_open")
		intro_1_41=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	set rs1=nothing
	Function checkPower_(o,s,c)
		If o=1 Then
			checkPower_=checkpower2010(s,c)
		elseIf o=3 Then
			checkPower_=true
		else
			checkPower_=false
	...
	If open_41_10=1 Then
		strWhere=strWhere&" and contractth.cateid in("&intro_41_10&") and contractth.cateid<>0"
	ElseIf open_41_10=3 Then
	else
		strWhere=" and 1=2 "
	end if
	LookQx=" (select count(*) from power where (ord="&session("personzbintel2007")&" and sort1=41 and sort2=14) and ((qx_open=3) or (qx_open=1 and (qx_intro like '%,'+cast(contractth.cateid as varchar(20))+',%' or qx_intro like '%'+cast(contractth.cateid as varchar(20))+',%' or qx_intro like '%,'+cast(contractth.cateid as varchar(20))+'%')))) look,"
	strWhere=" and 1=2 "
	lookKhQx=" (select count(*) from power where (ord="&session("personzbintel2007")&" and sort1=1 and sort2=14) and ((qx_open=3) or (qx_open=1 and (qx_intro like '%,'+cast(contractth.cateid as varchar(20))+',%' or qx_intro like '%'+cast(contractth.cateid as varchar(20))+',%' or qx_intro like '%,'+cast(contractth.cateid as varchar(20))+'%')))) look1,"
	strWhere=" and 1=2 "
	Dim page_count
	page_count=request("page_count")
	If Not isNumeric(page_count) Then page_count=20
	If page_count<=0 Then page_count=20
	page_count=cint(page_count)
	Dim m1,m2
	Dim unitList,retrunGoodsPra,retrunGoodsValue,productcaigoulistord,pord
	m1=request("date1")
	m2=request("date2")
	unitList=Request("unitList")
	retrunGoodsPra=Request("retrunGoodsPra")
	retrunGoodsValue=Request("retrunGoodsValue")
	productcaigoulistord=Request("ord")
	pord=request("pord")
	If productcaigoulistord="" or pord="" Then call db_close : Response.end()
	If Not IsNumeric(unitList) Then unitList=0
	If Not IsNumeric(retrunGoodsPra) Then retrunGoodsPra=0
	If Not IsNumeric(productcaigoulistord) Then productcaigoulistord=0
	If unitList<>0 Then strWhere=strWhere& " and (kuoutlist2.unit='"&unitList&"' or (kuoutlist2.unit like '%,"&unitList&",%' or kuoutlist2.unit like '%"&unitList&",%' or kuoutlist2.unit like '%,"&unitList&"%'))"
	If retrunGoodsPra=3 Then strWhere=strWhere& " and kuoutlist2.ph like '%"&retrunGoodsValue&"%'"
	If retrunGoodsPra=4 Then strWhere=strWhere& " and kuoutlist2.xlh like '%"&retrunGoodsValue&"%'"
	If retrunGoodsPra=1 Then strWhere=strWhere& " and tel.name like '%"&retrunGoodsValue&"%'"
	If retrunGoodsPra=2 Then strWhere=strWhere& " and contractth.title like '%"&retrunGoodsValue&"%'"
	If isDate(m1) Then strWhere=strWhere& " and contractth.date3>='"&m1&"'"
	If isDate(m2) Then strWhere=strWhere& " and contractth.date3<='"&m2&"'"
	If productcaigoulistord<>0 Then strWhere=strWhere& " and contractth.ord in ("&productcaigoulistord&") "
	Function cordCount(table_)
		cordCount=Conn.Execute("select count(*) from ("&table_&") c")(0)
	end function
	Function strReturnValue(sql)
		Dim objsRs
		set objsRs=Conn.execute(sql)
		intObjCont=Conn.execute("select count(*) from ("&sql&") t")(0)
		If Not objsRs.eof Then
			strReturnValue=objsRs.getString(,,"<row>","<line>","")
		else
			strReturnValue=null
		end if
		objsRs.close
	...
	Function dblSum(sql,F)
		Dim T
		T=0
		T=conn.execute("select sum("&F&") From ("&sql&") s")(0)
		If isNull(T)=True Then T=0
		dblSum=Formatnumber(T,num_dot_xs,-1)
'If isNull(T)=True Then T=0
	end function
	Dim strOrd,intCount,D1,D2,dotstr,dotstrs
	strOrd=Request("xlsord")
	If strOrd="" Then
		conn.close
		Set Conn=Nothing
		call db_close : Response.end
		...
		for i=1 to num_dot_xs
			...
			dotstrs=""
				for i=1 to num1_dot
					...
					xlWorksheet.WriteHtmlCell "<center><b>退货明细</b></center>" , "colspan=15 width=1260"
					xlWorksheet.movenext
					Dim tempArr
					If open_5_21=1 and open_23_3=1 Then
						tempArr = Array("产品名称","关联销售退货", "退货日期", "客户名称", "单位", "币种", "退货数量", "退货单价", "退货总价", "成本单价", "成本总价", "批号", "序列号", "生产日期", "有效日期")
						For i = 0 To UBound(tempArr)
							xlWorksheet.WriteHtmlCell "<center>"& tempArr(i) &"</center>" , ""
						next
					ElseIf open_5_21=1 and open_23_3<>1 Then
						tempArr = Array("产品名称","关联销售退货", "退货日期", "客户名称", "单位", "币种", "退货数量", "退货单价", "退货总价", "批号", "序列号", "生产日期", "有效日期")
						For i = 0 To UBound(tempArr)
							xlWorksheet.WriteHtmlCell "<center>"& tempArr(i) &"</center>" , ""
						next
					ElseIf open_5_21<>1 and open_23_3=1 Then
						tempArr = Array("产品名称","关联销售退货", "退货日期", "客户名称", "单位", "币种", "退货数量", "成本单价", "成本总价", "批号", "序列号", "生产日期", "有效日期")
						For i = 0 To UBound(tempArr)
							xlWorksheet.WriteHtmlCell "<center>"& tempArr(i) &"</center>" , ""
						next
					ElseIf open_5_21=0 and open_23_3=0 Then
						tempArr = Array("产品名称","关联销售退货", "退货日期", "客户名称", "单位", "币种", "退货数量", "批号", "序列号", "生产日期", "有效日期")
						For i = 0 To UBound(tempArr)
							xlWorksheet.WriteHtmlCell "<center>"& tempArr(i) &"</center>" , ""
						next
					end if
					xlWorksheet.movenext
					strListSql="select isNull((select Top 1 hl from hl where bz=sortbz.id and date1=contractth.date3),1) hl,(select title from product where ord=contractthlist.ord) ptitle, kuoutlist2.price1 costPrice,kuoutlist2.id kid,kuoutlist2.money1 costAllPrice,kuoutlist2.ph,kuoutlist2.xlh ,"&LookQx&lookKhQx&"contractth.ord id,contractth.title,contractth.date3,contractth.cateid,contractthlist.ord,contractthlist.datesc,contractthlist.dateyx,contractthlist.company,contractthlist.num1,isNull(contractthlist.unit,0) unit,contractthlist.money1,contractthlist.price1,contractth.money2,contractthlist.date7,contractthlist.kuoutlist2,tel.name,tel.cateid as cateid_kh, tel.sort3,ISNULL(tel.share,'-222') share,sortbz.sort1,ku.money1 money1All,isnull(ku.Num3,0) kuNum,ISNULL(contract.bz,14) contractBz,contract.date3 contractTime "&_
					"xlWorksheet.movenext"&_
					"from contractth "&_
					" inner join contractthlist on contractthlist.caigou=contractth.ord and contractthlist.del=1 and contractthlist.ord="&pord&"  "&_
					" left join tel on tel.ord=contractthlist.company  and tel.del=1  "&_
					" inner join sortbz on sortbz.id=contractth.bz  "&_
					" left join kuoutlist2 on kuoutlist2.id=contractthlist.kuoutlist2 "&_
					" left join ku on ku.id=kuoutlist2.ku  "&_
					" left join contract on contract.ord=contractthlist.contract  "&_
					" where  1=1 "& strWhere
					intCount=Conn.execute("select count(*) from ("&strListSql&") c")(0)
					Dim pageCount,pageIndex,pageSize
					pageSize=page_count
					pageIndex=request("page")
					if pageIndex="" Then pageIndex=1
					if pageIndex<1 Then pageIndex=1
					pageIndex=cint(pageIndex)
					Set objRs=Conn.execute(strListSql&" order by contractth.date7 desc")
					Dim i,C1
					C1=objRs.recordcount
					i=2
					dblRetrunGoodsNum=0
					dblRetrunGoodsPrice=0
					dblCostPriceAll1=0
					T=0
					Dim title,date3,name,sort1,unit,num1,price1,money1,costPrice,costAllPrice,ph,xlh
					Do While Not objRs.Eof
						contracthL=1
						Set objRs1=Conn.execute("select isNull(hl,1) from hl where bz="&objRs("contractBz").value&" and date1='"&objRs("contractTime").value&"'")
						If Not objRs1.Eof Then contracthL=objRs1(0)
						objRs1.close
						Set objRs1=Nothing
						i=i+1
						Set objRs1=Nothing
						D1=objRs("datesc")
						D2=objRs("dateyx")
						If D1<>"" Then D1=cstr(D1)
						If D2<>"" Then D2=cstr(D2)
						unit = ""
						Set objRs1=Conn.execute("select sort1 from sortonehy where id = "&objRs("unit").value&" ")
						If Not objRs1.Eof Then unit=objRs1(0)
						objRs1.close
						title=objRs("title")
						date3=cstr(objRs("date3"))
						bz=objRs("sort1")
						num1=zbcdbl(objRs("num1"))
						price1=zbcdbl(objRs("price1"))
						money1=zbcdbl(objRs("money1"))
						costPrice=zbcdbl(objRs("costPrice"))
						costAllPrice=objRs("costAllPrice")
						xlh=objRs("xlh")
						ph=objRs("ph")
						Product=objRs("ptitle").value
						If objRs("kuNum").value&""="0" Then
							constPrice = 0
						else
							constPrice=Formatnumber(cdbl(objRs("money1All").value&"")/cdbl(objRs("kuNum").value&""),num_dot_xs,-1)
							constPrice = 0
							costAllPrice=Formatnumber((cdbl(objRs("money1All").value&"")/cdbl(objRs("kuNum").value&""))*(zbcdbl(objRs("Num1").value)&""),num_dot_xs,-1)
							constPrice = 0
						end if
						dblCostPriceAll=dblCostPriceAll+costAllPrice
						constPrice = 0
						dblCostPriceAll1=dblCostPriceAll1+(constPrice*(zbcdbl(objRs("Num1").value)&""))
						constPrice = 0
						sort3 =objRs("sort3")
						name=""
						If sort3&"" = "2" Then
							If open_26_1=3 Or checkPower_(open_26_1,intro_26_1,objRs("cateid_kh")) Then
								name=objRs("name")
							end if
						else
							If open_1_1=3 Or checkPower_(open_1_1,intro_1_1,objRs("cateid_kh"))  Or InStr(1,","&objRs("share")&",", ","& sdk.info.user &",",1) > 0 Or objRs("share")&"" = "1" Then
								name=objRs("name")
							end if
						end if
						If open_5_21=1 and open_23_3=1 Then
							tempArr = Array(Product,title,date3,name,unit,bz,num1,price1,money1,constPrice/contracthL ,(constPrice*(zbcdbl(objRs("Num1").value)&""))/contracthL,ph,xlh,D1,D2)
							For x = 0 To UBound(tempArr)
								xlWorksheet.WriteHtmlCell "<center>"& tempArr(x) &"</center>" , ""
							next
						ElseIf open_5_21=1 and open_23_3<>1 Then
							tempArr = Array(Product,title,date3,name,unit,bz,num1,price1,money1,ph,xlh,D1,D2)
							For x = 0 To UBound(tempArr)
								xlWorksheet.WriteHtmlCell "<center>"& tempArr(x) &"</center>" , ""
							next
						ElseIf open_5_21<>1 and open_23_3=1 Then
							tempArr = Array(Product,title,date3,name,unit,bz,num1,constPrice,costAllPrice,ph,xlh,D1,D2)
							For x = 0 To UBound(tempArr)
								xlWorksheet.WriteHtmlCell "<center>"& tempArr(x) &"</center>" , ""
							next
						ElseIf open_5_21=0 and open_23_3=0 Then
							tempArr = Array(Product,title,date3,name,unit,bz,num1,ph,xlh,D1,D2)
							For x = 0 To UBound(tempArr)
								xlWorksheet.WriteHtmlCell "<center>"& tempArr(x) &"</center>" , ""
							next
						end if
						...
						If PreProcV < currProcV Then
							Response.write "<script>window.exportProcBar.showExcelProgress(" & currProcV & "," & C1 & "," & i & ")</script>"
							PreProcV = currProcV
							Response.Flush
						end if
						xlWorksheet.movenext
						dblRetrunGoodsNum=dblRetrunGoodsNum+Formatnumber(zbcdbl(objRs("num1").value),num_dot_xs,-1)
						xlWorksheet.movenext
						dblRetrunGoodsPrice=dblRetrunGoodsPrice+Formatnumber((zbcdbl(objRs("money1").value)&"")*(zbcdbl(objRs("hl").value)&""),num_dot_xs,-1)
						xlWorksheet.movenext
						D1=""
						D2=""
						objRs.MoveNext
					...
					set objRs=Nothing
					Dim AllNum
					AllNum=dblSum(strListSql,"num1")
					xlWorksheet.writehtmlcell "<center><b>本页合计</b></center>", ""
					xlWorksheet.WriteNumber dblRetrunGoodsNum
					If open_5_21=1 Then
						xlWorksheet.writemoney dblRetrunGoodsPrice
						xlWorksheet.writemoney dblSum(strListSql,"hl*money1")
					end if
					xlWorksheet.movenext
					xlWorksheet.writehtmlcell "<center><b>所有合计</b></center>", ""
					xlWorksheet.WriteNumber AllNum
					If open_23_3=1 Then
						xlWorksheet.writemoney dblCostPriceAll1
						xlWorksheet.writemoney dblSum(strListSql,"num1*(money1All/kuNum)")
					end if
					title = "退货明细"
					...
					If Abs(Err.number) > 0 then
						...
						Response.write "" & vbcrlf & "             <script>window.exportProcBar.addFileLink({fileUrl:"""
					...
					Set xlsapp = Nothing
					Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & "<!-- <p align=""center""><a href=""downfile.asp?fileSpec="
					Set xlsapp = Nothing
					Response.write server.MapPath("/out/退货明细_"&session("name2006chen")&".xls")
