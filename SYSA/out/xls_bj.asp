...
'Private Sub Class_Initialize
		...
		Dim area_list,aryReturn2
		Sub page_load
			Set conn = cn
			Dim open_4_21,intro_4_21,open_4_22,intro_4_22,open_4_10,intro_4_10
			Call fillPowerInfo(4,21,open_4_21,intro_4_21)
			Call fillPowerInfo(4,22,open_4_22,intro_4_22)
			Call fillPowerInfo(4,10,open_4_10,intro_4_10)
			Dim list,Str_Result,kh_Str_Result,px_Result,rs
			if open_4_10=3 then
				list=""
			elseif open_4_10=1 then
				list="and cateid in ("&intro_4_10&") and cateid>0"
			else
				list="and cateid=0"
			end if
			Str_Result="where (del=1 or del=3) "&list&""
			Dim A2,D,E,F,b,m1,m2,B1,B2,C1,C2,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6
			A2=request("A2")
			D=request("D")
			E=request("E")
			F=Request("F")
			...
			zdy5=request("zdy5")
			zdy6=request("zdy6")
			bjmoney1 = Request("bjmoney1")
			bjmoney2 = Request("bjmoney2")
			retbj = Request("retbj")
			retbj2 = Request("retbj2")
			Dim new_list
			if b="3333333" or b="5555555" Then
				new_list="<span class='alt_tx'></span>"
				if b="3333333" and session("timezbintel2007")<>"" Then noalt1=" and date7>'"&session("timezbintel2007")&"'"
				Str_Result=" where (del = 1 or del = 3 ) and ((cateid_sp=" & session("personzbintel2007")&_
				" and complete=3) or (cateid="&session("personzbintel2007")&" and alt=1)) "&noalt1&" "
				Str_Result=Str_Result&" and ("&open_4_10&"=3 or ("&open_4_10&"=1 AND CHARINDEX(','+CAST(cateid as varchar(50))+',' , ','+'"&intro_4_10&"'+',')>0)) "
'and complete=3) or (cateid=&session(personzbintel2007)& and alt=1)) &noalt1& 
			end if
			...
			if bjmoney1&""<>"" then
				if isnumeric(bjmoney1) then
					Str_Result = Str_Result & " and money1>="& bjmoney1 &" "
				end if
			end if
			if bjmoney2&""<>"" then
				if isnumeric(bjmoney2) then
					Str_Result = Str_Result & " and money1<="& bjmoney2 &" "
				end if
			end if
			if retbj&""<>"" then
				if IsDate(retbj) then
					Str_Result = Str_Result & " and datediff(dd,'"& retbj &"',date1)>=0 "
				end if
			end if
			if retbj2&""<>"" then
				if IsDate(retbj2) then
					Str_Result = Str_Result & " and datediff(dd,date1,'"& retbj2 &"')>=0 "
				end if
			end if
			dim newstate
			...
			if newstate="1" then
				set rs=server.CreateObject("adodb.recordset")
				sql="select ord,intro,num1,tq1,fw1 from setjm2  where ord=34 and cateid="&session("personzbintel2007")&""
				rs.open sql,conn,1,1
				if not rs.eof then
					if rs("fw1")="0" then
						if open_4_1=3 then
							kh_Str_Result = kh_Str_Result & ""
						elseif open_4_1=1 then
							kh_Str_Result = kh_Str_Result & "and cateid in ("&intro_4_1&")"
						else
							kh_Str_Result = kh_Str_Result & "and cateid=0"
						end if
					else
						kh_Str_Result = kh_Str_Result & "and cateid="&session("personzbintel2007")&""
					end if
				end if
				rs.close
				set rs=nothing
			end if
			Dim W1,W2,W3,W4,tmp,W3_T
			W1=replace(request("W1")," ","")
			W2=replace(request("W2")," ","")
			W3=replace(request("W3")," ","")
			W3_T=replace(request("W3_T")," ","")
			on error resume next
			If request("fromTJ") = "1" Then
				If w1<>"" Then kh_Str_Result=kh_Str_Result+" and cateid2="& W1 &""
'If request("fromTJ") = "1" Then
				If w2<>"" Then kh_Str_Result=kh_Str_Result+" and cateid3="& W2 &""
'If request("fromTJ") = "1" Then
				If w3<>"" Then kh_Str_Result=kh_Str_Result+" and cateid="& W3 &""
'If request("fromTJ") = "1" Then
			else
				if W1="" then W1=0
				if W2="" then W2=0
				...
				if W4<>"" Then
					...
					W2=tmp(1)
					If W3_T = "0" Then
						kh_Str_Result=kh_Str_Result+" and cateid in("& W3 &") "
'If W3_T = "0" Then
					ElseIf W3_T = "1" Then
						kh_Str_Result=kh_Str_Result+" and addcate in("& W3 &") "
'ElseIf W3_T = "1" Then
					end if
				end if
			end if
...
'Dim n
			...
			Dim khid,A1,px
			khid=request("khid")
			If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if A2<>"" Then kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and area in ("&area_list&")) "
'If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if D<>"" Then kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  trade in  ("&D&")) "
'If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if A1<>"" Then kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  jz in  ("&A1&")) "
'If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if E<>"" Then kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  sort in  ("&E&")) "
'If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if F<>"" Then kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  sort1 in  ("&F&")) "
'If khid<>"" Then kh_Str_Result = kh_Str_Result & " and company="& khid &" "
			if C1<>"" then
				if B1="mc" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  name like '%"&C1&"%') "
'if B1="mc" then
				elseif B1="pym" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  pym like '%"&C1&"%') "
'elseif B1="pym" then
				elseif B1="dh" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  phone like '%"&C1&"%') "
'elseif B1="dh" then
				elseif B1="cz" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  fax like '%"&C1&"%') "
'elseif B1="cz" then
				elseif B1="wz" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  url like '%"&C1&"%') "
'elseif B1="wz" then
				elseif B1="dz" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  address like '%"&C1&"%') "
'elseif B1="dz" then
				elseif B1="yb" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  zip like '%"&C1&"%') "
'elseif B1="yb" then
				elseif B1="bz" then
					kh_Str_Result=kh_Str_Result+" and company in (select ord from tel where del=1  and  intro like '%"&C1&"%') "
'elseif B1="bz" then
				end if
			end if
			if b<>"" and b<>"7" Then Str_Result=Str_Result+"and  complete in ("&b&")"
'elseif B1="bz" then
...
'if B2="bjzt" then
				elseif B2="bjbh" then
					Str_Result=Str_Result+"and  order1 like '%"&C2&"%'"
'elseif B2="bjbh" then
				elseif B2="bjbz" then
...
'elseif B2="zdy4" then
			...
			cfidx=0
			fields = "报价主题,报价编号,报价日期,币种,报价金额,报价状态,报价人,添加人,关联客户,对方代表,手机,办公电话"
			Dim zdyFields(),j
			ReDim zdyFields(1,0)
			set rs=server.CreateObject("adodb.recordset")
			rs.open "select id,title,name,sort,gl,set_open,dc from zdy where sort1=4 order by gate1 asc ",conn,1,1
			j=0
			do while rs.eof=False
...
'If rs("set_open") <> 1 Or rs("dc") <> 1 Then
			...
			set rs=Nothing
			if open_4_21 <> 1 Then
				CantExport_Fields(cfidx) = "报价金额" : cfidx = cfidx + 1
'if open_4_21 <> 1 Then
				CantExport_Fields(cfidx) = fDBName("单价") : cfidx = cfidx + 1
'if open_4_21 <> 1 Then
				CantExport_Fields(cfidx) = fDBName("总价") : cfidx = cfidx + 1
'if open_4_21 <> 1 Then
			end if
			if open_4_22 <> 1 Then
				CantExport_Fields(cfidx) = fDBName("建议进价") : cfidx = cfidx + 1
'if open_4_22 <> 1 Then
				CantExport_Fields(cfidx) = fDBName("建议总价") : cfidx = cfidx + 1
'if open_4_22 <> 1 Then
			end if
			fields=fields&",P0"
			Dim mxzdyFields(),sorce
			sql = "SELECT name,title,sorce,set_open FROM zdymx WHERE sort1 = 4 ORDER BY gate1"
			Set rs = cn.execute(sql)
			j = 0
...
'If rs("set_open") <> 1 Then
			wend
			rs.close
			baseSql = " exec erp_xls_bj '"& replace(Str_Result,"'","''") &"','"& replace(px_Result,"'","''") &"','"& replace(kh_Str_Result,"'","''") &"','"& app.iif(ZBRuntime.MC(5000),"待询价","") &"','"& app.iif(ZBRuntime.MC(5000),"询价中","") &"','"& replace(fields,"'","''")        &"'," & Info.User
			Dim adapter
			Dim sqls,sheetSetting
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "报价资料"
			adapter.canSplitFormula_And = "canSplit(rs)"
			Dim hd,fds,i,fd,sname
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = baseSql
			sheetSetting.title = "报价资料"
			fds = Split(fields,",")
			For i = 0 To ubound(fds)
				...
				hd.width = 60
				hd.excelAlign = "center"
				Case "报价主题","报价编号","关联客户","zdy1","zdy2","zdy3","zdy4","产品名称","产品编号","产品型号","明细自定义1","明细自定义2","明细自定义3","明细自定义4":
				hd.width = EX_WIDTH_TEXT
				Case "报价日期","到货日期":
				hd.width = EX_WIDTH_DATE
				Case "报价人","添加人","对方代表":
				hd.width = EX_WIDTH_PERSON
				Case "报价状态","zdy5","zdy6","单位","明细自定义5","明细自定义6":
				hd.width = EX_WIDTH_SELECT
				Case Else :
				hd.dbtype = "str"
				hd.excelAlign = "left"
				End Select
				If InStr(sname,"单价")>0 Or sname="建议进价" Then hd.canSum = False
