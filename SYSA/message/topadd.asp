...
'c=Mid(str,i,1)
	...
	function menu(id,opentype)
		...
		sort=request("sort")
		group                = Request("group")
		group                = searchCID(group,"MessagePersonClass")
...
'If position <> "" Then
		...
		If mobile <> "" Then
			str_Result = str_Result + " AND clictPhone LIKE '%"&mobile&"%' "
'If mobile <> "" Then
		end if
		MaxRecSize=40
		RecPageSize=25
		if opentype="1" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from MessagePersonClass where (del is null or del<>2) and parent="&id&" and (CHARINDEX(',"&session("personzbintel2007")&",',','+replace(user_list,' ','')+',')>0 or user_list='0,0' or user_list='0' or user_list is null) order by sort desc,id asc"
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			Response.write("<table border='0' cellspacing='0' cellpadding='0'>"&chr(13))
			i=1
			while not rs.eof
				ChildCount=conn.execute("select count(*) from MessagePersonClass where parent="&rs("id"))(0)
				if ChildCount=0 then
					...
					set rs7=server.CreateObject("adodb.recordset")
					RecCount=conn.execute("select count(*) from MessagePerson where sort="&rs("id")&Str_Result)(0)
					sql7="select *,(CASE WHEN company > 0 THEN ISNULL((SELECT TOP 1 cateid FROM tel WHERE del = 1 AND ord = MessagePerson.company),0)   ELSE -1 END) customCateID from MessagePerson where sort="&rs("id")& " " & Str_Result
					RecCount=conn.execute("select count(*) from MessagePerson where sort="&rs("id")&Str_Result)(0)
					rs7.open sql7,conn,1,1
					...
					while not rs7.eof and k<RecPageSize
						customCateID = rs7("customCateID")
						title_1=trim(rs7("clictName"))
						if len(title_1)>3 then title_1=left(title_1,3)
						mobile=trim(rs7("clictPhone"))
						mobile1=""
						if isMobile(mobile) then
							If customCateID >=0 Then
								If IsPhonePower(customCateID) Then
									Char = ""
									phoneNum = GetPhoneNumber(mobile, customCateID)
								else
									Char = title_1
									phoneNum = "***********"
								end if
							...
							mobile1="<a href='###' title='"&trim(rs7("clictName"))&":"&phoneNum&"' onClick=""changeMobile('"&rs7("ord")&"','"&trim(rs7("clictPhone"))&"',true,'"&Char&"')"">"
							mobile2=phoneNum&"</a><br/>"&chr(13)
						else
							mobile1="<a href='###' title='号码格式不正确,点击修改' onClick=""changeMobile('"&rs7("ord")&"','"&mobile&"',true,'')"">"
							mobile2="(号码有误)</a><br/>"&chr(13)
						end if
...
'if PageFlag=true then
					...
					if PageFlag=true and recCount>0  then
						...
						Response.write "<input id='txtGoToPage_"&rs("id")&"' value='"&P&"' style='width:30px;height:18px' type=text style='width:30px'>"&chr(13)
...
'if C<>"" and C<>"检索关键字" then
'end if
			elseif B="mobile" then
'if C<>"" and C<>"检索关键字" then
				str_Result=str_Result+" and clictPhone like '%"& C &"%'"
'if C<>"" and C<>"检索关键字" then
'end if
			end if
			if P="" then P=1
			...
			set rs=server.CreateObject("adodb.recordset")
			sql7="select *,(CASE WHEN company > 0 THEN ISNULL((SELECT TOP 1 cateid FROM tel WHERE del = 1 AND ord = MessagePerson.company),0)  ELSE -1 END) customCateID from MessagePerson where 1=1 " &strsort&str_Result
			set rs=server.CreateObject("adodb.recordset")
			...
			while not rs.eof and k<RecPageSize
				customCateID = rs("customCateID")
				title_1=rs("clictName")
				mobile=trim(rs("clictPhone"))
				mobile1=""
				if isMobile(mobile) then
					If customCateID >=0 Then
						If IsPhonePower(customCateID) Then
					...
					mobile1="<a href='###' onClick=""changeMobile('"&rs("ord")&"','"&trim(rs("clictPhone"))&"',true,'"&Char&"')"">"
					mobile2=phoneNum&"</a><br/>"&chr(13)
				else
					mobile1=""
					mobile2="<br/>"&chr(13)
...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>短信发送</title>" & vbcrlf & "<script src= ""../Script/me_topadd.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "</head>" & vbcrlf & ""
	from = Request("from")
	phone=trim(request("phone"))
	content=trim(replace(replace(replace(replace(replace(request("content"),"<","&lt;"),">","&gt;"),"&","&amp;"),"'","&apos;"),"""","&quot;"))
	...
	if phone<>"" then
		if len(phone)>11 then
			if RegTest(phone,"^\,((13[0-9]|14[0-9]|15[^4]|16[^4]|17[0-9]|18[0-9]|19[0-9])\d{8}((\,(13[0-9]|14[0-9]|15[^4]|16[^4]|17[0-9]|18[0-9]|19[0-9])\d{8})*))$")=true then
'if len(phone)>11 then
				phone=mid(""&phone&"",2)
			end if
		elseif len(phone)=11 then
			if RegTest(phone,"^(13[0-9]|14[0-9]|15[^4]|16[^4]|17[0-9]|18[0-9]|19[0-9])\d{8}$")=false then
'elseif len(phone)=11 then
				Response.write "<script language='javascript'> alert('号码格式不符！只能发送给手机号码'); window.opener=null;window.open('','_self');window.close();</script> "
				call db_close : Response.end
			end if
		else
			Response.write "<script language='javascript'> alert('号码格式不符！只能发送给手机号码'); window.opener=null;window.open('','_self');window.close();</script> "
	...
	if open_67_13<>1 then
		Response.write"<script language=javascript>alert('没有权限 ！'); history.back()</script>"
		call db_close : Response.end
	end if
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""1"">" & vbcrlf & "  <tr>" & vbcrlf & "  <td width=""195px"" valign=""top"" id='rightmenuall'>"
	Response.write "<script src= ""../Script/me_rightadd.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "button.wavbutton {" & vbcrlf & "     font-family:arial;" & vbcrlf & "      font-size:14px;" & vbcrlf & " background-color:white;" & vbcrlf & " filter:wave(strength=0,freq=1,lightstrength=6,phase=0);" & vbcrlf & " cursor:default;" & vbcrlf & "       padding:0;" & vbcrlf & "      border:1px solid #ccc;" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;zindex:2000;display:none"">" & vbcrlf & "<iframe src=""MessageADSearch.asp"" id=""adsIF"" width=""400px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "     <table border=""0"" cellpadding=""0"" width=""195""  cellspacing=""0"" id=""placeInfo"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td  id=""placeInfo"" class=""leftPageBg tableTitleLinks"">&nbsp;短信地址库</td>" & vbcrlf & "      </tr>" & vbcrlf & "</table>" & vbcrlf & "        <table height=""100%"" width=""193""  border=""0"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td valign=""center"" background=""../images/m_leftbg.gif""><table width=""193"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" height=""100px"">" & vbcrlf & "                                  <tr>" & vbcrlf & "                    <td class=""resetGroupTableBg"" height=""27"" background=""../images/topb1g.gif"">" & vbcrlf & "                    <div align=""center"">" & vbcrlf & "                                  <form name=""form"" method=""post"">" & vbcrlf & "                                            <select name=""B"" id=""B""  onchange=""ajaxSubmit_page(0,1,3)"" style=""width:55px;"">" & vbcrlf & "                                                 <option value=""name"" "
	if B="name" then
	...
	Response.write ">职位</option>" & vbcrlf & "                                                        <option value=""mobile"" "
	if B="mobile" then
		Response.write "selected=""selected"""
	end if
	Response.write ">手机号码</option>" & vbcrlf & "                                            </select>   " & vbcrlf & "                                            <input name=""C"" id=""C8"" type=""text"" size=""10"" value=""检索关键字"" style=""width:70px; solid;font-size: 9pt;color: #999999;text-align:center;"" onFocus=""if(value==defaultValue){value='';this.style.color='#000'}""  onkeyup=""ajaxSubmit_page(0,1,3)"" onKeyPress=""if(event.keyCode==13) {ajaxSubmit_page(0,1,3);return false;}"" />" & vbcrlf & "                                                <input name=""top"" type=""hidden"" size=""2""  value="""
	Response.write top
	Response.write """>" & vbcrlf & "                                 </form>" & vbcrlf & "                    <button class='wavbutton'style='width:44px;' onclick=""Left_adSearch(this);"" >高级</button>" & vbcrlf & "                    </div>" & vbcrlf & "                    </td>" & vbcrlf & "          </tr>" & vbcrlf & "" & vbcrlf & "     <tr>" & vbcrlf & "              <td valign=""center"" class=""menutab"" style=""padding:0px;""><table class=""resetGroupTableBg"" style=""background-image:url(../images/smico/lmeuntab_bg_c.gif);height:26px"" width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "       <tr>" & vbcrlf & "                    <td width=""68px""  height=""22px"" class=""name"" style=""padding-left:2px""><b>联系人列表</b></td>" & vbcrlf & "                                      <td class=""name"" align=""right"">" & vbcrlf & "                                             <a href=""###"" onclick=""ajaxSubmit_page(0,1,1);"">显示分类</a>" & vbcrlf & "                                                "
	Response.write top
	if open_67_12=1 or  open_67_12=3  then
		Response.write "<a href=""../sortsms/add_cp.asp"" target=""_blank"">添加分组</a>"
	end if
	Response.write "" & vbcrlf & "                                      </td>" & vbcrlf & "                  </tr>" & vbcrlf & "              </table></td>" & vbcrlf & "            </tr>" & vbcrlf & "                          " & vbcrlf & "            <tr>" & vbcrlf & "              <td valign=""top"">" & vbcrlf & "                         <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""3"" id=""leftmenu1"" style=""table-layout:fixed"">" & vbcrlf & "                               <tr>" & vbcrlf & "                            <td>" & vbcrlf & "                            <span id=""cp_search"">" & vbcrlf & "                             "
	Response.write "<a href=""../sortsms/add_cp.asp"" target=""_blank"">添加分组</a>"
	menu 0,1
	Response.write "" & vbcrlf & "                              </span>" & vbcrlf & "                         </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                          </td>" & vbcrlf & "            </tr>" & vbcrlf & "          </table></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><img src=""../images/m_leftb.gif"" width=""195"" height=""3"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	
	Response.write "</td>" & vbcrlf & "    <td valign=""top"">" & vbcrlf & "      <iframe name=""cFF2"" id=""cFF2"" width=""100%"" onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" style=""height:750px"" >" & vbcrlf & "    浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</iframe>" & vbcrlf & "    <form action=""../message/sendMessage.asp"" method =""post"" id=""sendSm"" target=""cFF2"">" & vbcrlf & "    <input type=""hidden"" name=""qf"" value="""
	Response.write qf
	Response.write """ />" & vbcrlf & "     <input type=""hidden"" name=""phone"" value="""
	Response.write phone
	Response.write """ />" & vbcrlf & "     <input type=""hidden"" name=""from"" value="""
	Response.write from
	Response.write """ />" & vbcrlf & "      <input type=""hidden"" name=""content"" value="""
	Response.write content
	Response.write """ />" & vbcrlf & "      <input type=""hidden"" name=""smsApproval"" value="""
	Response.write smsApproval
	Response.write """ />" & vbcrlf & "    </form>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "    document.getElementById(""sendSm"").submit();" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & ""
