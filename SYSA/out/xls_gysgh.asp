...
'bigsystemtype = ".mozi"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(9).NumberFormatLocal = "0."&dotstr&"% "
			...
			zdy5=request("zdy5")
			zdy6=request("zdy6")
			Str_Result2=Str_Result2 + "and x.del=1 and x.sort3=2"
			zdy6=request("zdy6")
			ksjs=request("B")
			ksjs2=request("C")
			m1=request("ret")
...
'if T2<>"" then
			...
			if m1<>"" then
				Str_Result2=Str_Result2+"and  y.date3>='"&m1&"'"
'if m1<>"" then
...
'if m1<>"" then
			...
			if m2<>"" then
				Str_Result2=Str_Result2+"and  y.date3<='"&cdate(m2)+1&"'"
'if m2<>"" then
				Str_Result_fy=Str_Result_fy+"and  date1<='"&m2&"'"
'if m2<>"" then
				Str_Result_cp=Str_Result_cp+"and  date1<='"&m2&"'"
'if m2<>"" then
			end if
			set rs=server.CreateObject("adodb.recordset")
			sql="select x.ord,x.name,x.khid,x.cateid,isnull(sum(y.money2),0) as money1,count(y.ord) as numht  from tel as x  inner join caigou as y on x.ord=y.company and y.del=1 "&Str_Result2&" group by x.ord,x.name,x.khid,x.cateid order by money1 desc"
			...
			do until rs.eof
				...
				money_cpall=0
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select isnull(sum(money1),0) as money1 from kuoutlist2 where gys="&ord&" and del=1 "&Str_Result_cp&""
				rs7.open sql7,conn,1,1
				money_xs=zbcdbl(rs7("money1"))
				rs7.close
				set rs7=nothing
				sql1 = "select num1,ku from kuoutlist2 where gys="&ord&" and del=1 "&Str_Result_cp&""
				set rs1 = server.CreateObject("adodb.recordset")
				rs1.open sql1,conn,1,1
				if rs1.eof then
				else
					do until rs1.eof
						if rs1("ku")<>"" then
							sql7 = "select num1,money1 from ku where id="&rs1("ku")&""
							set rs7 = server.CreateObject("adodb.recordset")
							rs7.open sql7,conn,1,1
							if rs7.eof then
							else
								money_cpall=money_cpall+zbcdbl(rs7("money1"))*cdbl(rs1("num1"))/cdbl(rs7("num1"))
'if rs7.eof then
							end if
							rs7.close
							set rs7=nothing
				...
				set rs1=nothing
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select isnull(sum(money1),0) as money1 from ku where gys="&ord&"  "&Str_Result_rk&""
				rs7.open sql7,conn,1,1
				money_kc=zbcdbl(rs7("money1"))-money_cpall
				sql7="select isnull(sum(money1),0) as money1 from ku where gys="&ord&"  "&Str_Result_rk&""
				rs7.close
				set rs7=nothing
				money_mlall=cdbl(money_xs)-cdbl(money_cpall)-cdbl(pay)
				set rs7=nothing
				money_ll=0
				if money1<>0 then
					money_ll=money_mlall*100/money1
				end if
				...
				money_ll=money_mlall*100/money1
				xlWorksheet.Cells(1+i,5).Value =Formatnumber(money_xs,num_dot_xs,-1)
				money_ll=money_mlall*100/money1
				...
				if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,7).Value =Formatnumber(money_cpall,num_dot_xs,-1)
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,7).font.Size=10
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,8).Value =Formatnumber(money_mlall,num_dot_xs,-1)
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,8).font.Size=10
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,9).Value =Formatnumber(money_ll,num_dot_xs,-1)&"%"
'if open_5_17=1 or open_5_17=3 then
...
'if open_5_17=1 or open_5_17=3 then
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>供应商供货总额排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!供应商供货总额排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("供应商供货总额排行_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的供应商供货总额排行</u></strong></font></a></p> " & vbcrlf & ""
