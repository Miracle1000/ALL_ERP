...
'if strW3<>"" Then
	...
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_1=0
		intro_67_1=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_7=0
		intro_67_7=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_8=0
		intro_67_8=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_10=0
		intro_67_10=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_11=0
		intro_67_11=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_3=0
		intro_67_3=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=12"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_12=0
		intro_67_12=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=13"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_13=0
		intro_67_13=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=17"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_17=0
		intro_67_17=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=20"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_20=0
		intro_67_20=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=15"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_15=0
		intro_67_15=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=67 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_67_19=0
		intro_67_19=0
	...
	elseif open_67_1=1 then
		list_tj="and addcate in ("&intro_67_1&")"
	else
	...
	if open_67_1="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_67_1&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'If Not rs.Eof Then
	...
	If A1=-100 Then A1="-100,-1,-3,-4,-5,-8,-13,-17,-201,-202,-203,-19"
	companyid = ""
	A3 = Trim(request("A3"))
	If A3&"" <> "" Then
'If A3 = "0" Then
		Str_Result=Str_Result+" and stact1 is not null  and  stact1=0"
'If A3 = "0" Then
		Str_Result2=Str_Result2+" and stact1 is not null and  stact1=0"
'If A3 = "0" Then
	else
		Str_Result=Str_Result+" and stact1 is not null  and  stact1<>0"
'If A3 = "0" Then
...
'end if
	open_2_14 = openPower(2, 14)
	intro_2_14 = introPower(2, 14)
	W1 = Replace(Trim(request("W1")), " ", "")
	W2 = Replace(Trim(request("W2")), " ", "")
	W3 = Replace(Trim(request("W3")), " ", "")
	W1 = Replace(request("W1"), " ", "")
	W2 = Replace(request("W2"), " ", "")
	W3 = Replace(request("W3"), " ", "")
	If request("tj") & ""="1" Or request("FromTJ") & ""="1" Then
		If w1<>"" Then Str_Result=Str_Result+" and cateid2="& W1 &"" : Str_Result2 = Str_Result2 + " and cateid2="& W1 &""
'If request("tj") & ""="1" Or request("FromTJ") & ""="1" Then
		If w2<>"" Then Str_Result=Str_Result+" and cateid3="& W2 &"" : Str_Result2 = Str_Result2 + " and cateid3="& W2 &""
'If request("tj") & ""="1" Or request("FromTJ") & ""="1" Then
		If w3<>"" Then Str_Result=Str_Result+" and cateid="& W3 &"" : Str_Result2 = Str_Result2 + " and cateid="& W3 &""
'If request("tj") & ""="1" Or request("FromTJ") & ""="1" Then
	else
		If W1 = "" Then W1 = 0
		If W2 = "" Then W2 = 0
...
'If m2<>"" Then
	...
	If A1<>"" Then
		Str_Result = Str_Result + " and stact1 is not null  and  stact1 in("&A1&")"
'If A1<>"" Then
		Str_Result2 = Str_Result2 + " and stact1 is not null and  stact1 in("&A1&")"
'If A1<>"" Then
	end if
	If C2<>"" Then
		If C1 = 1 Then
...
'ElseIf C1 = 2 Then
		ElseIf C1 = 3 Then
			str_Result = str_Result + " and content='"&C2&"'"
'ElseIf C1 = 3 Then
			str_Result2 = str_Result2 + " and content='"&C2&"'"
'ElseIf C1 = 3 Then
		ElseIf C1 = 4 Then
			str_Result = str_Result + " and content<>'"&C2&"'"
'ElseIf C1 = 4 Then
...
'ElseIf C1 = 6 Then
	...
	If S2<>"" And S1<>"" Then
		If S1 = "1" Then
			str_Result = str_Result + " and phone like '%"& S2 &"%'"
'If S1 = "1" Then
			str_Result2 = str_Result2 + " and phone like '%"& S2 &"%'"
'If S1 = "1" Then
		ElseIf S1 = "2" Then
			str_Result = str_Result + " and phone not like '%"& S2 &"%'"
'ElseIf S1 = "2" Then
			str_Result2 = str_Result2 + " and phone not like '%"& S2 &"%'"
'ElseIf S1 = "2" Then
		ElseIf S1 = "3" Then
			str_Result = str_Result + " and phone='"&S2&"'"
'ElseIf S1 = "3" Then
			str_Result2 = str_Result2 + " and phone='"&S2&"'"
'ElseIf S1 = "3" Then
		ElseIf S1 = "4" Then
			str_Result = str_Result + " and phone<>'"&S2&"'"
'ElseIf S1 = "4" Then
...
'ElseIf S1 = "6" Then
	...
	If G2<>"" Then
		If G1 = 1 Then
			str_Result2 = str_Result2 + "and person in (select ord from person where name like '%"& G2 &"%')"
'If G1 = 1 Then
		ElseIf G1 = 2 Then
			str_Result2 = str_Result2 + "and person in (select ord from person where name not like '%"& G2 &"%')"
'ElseIf G1 = 2 Then
		ElseIf G1 = 3 Then
			str_Result2 = str_Result2 + "and person in (select ord from person where name='"&G2&"')"
'ElseIf G1 = 3 Then
		ElseIf G1 = 4 Then
			str_Result2 = str_Result2 + "and person in (select ord from person where name<>'"&G2&"')"
'ElseIf G1 = 4 Then
...
'ElseIf G1 = 6 Then
	...
	ElseIf open_67_1 = 1 Then
		Str_Result2 = Str_Result2&" and (addcate in("&intro_67_1&")) "
	else
	...
	If companyid<>"" Then
		Str_Result2 = Str_Result2&" and person in(select ord from person where company="& companyid &")"
	end if
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<form name=""date"" id=""demo"" method=""post"" action=""logList.asp?page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currPage
	Response.write """>" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "           <tr>" & vbcrlf & "            <td class=""place"">短信发送记录</td><td>&nbsp;</td>" & vbcrlf & "            <td>" & vbcrlf & "                        <div align=""right"" id=""kh"" style='height:22px;line-height:22px;'>手机号码：<input name=""S3""  value=""1"" id=""Kjs1"" type=""hidden"" size=""15"" />" & vbcrlf & "                      <input  name=""S4"" id=""Kjs1"" type=""text"" size=""15"" />" & vbcrlf & "                    <input type=""submit"" name=""Submit45"" value=""检索"" onClick=""this.form.action = 'logList.asp?person="
	...
	if open_67_10=1 or open_67_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""记录导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_SMS.asp?tj="
		Response.write request("tj")
	...
	if open_67_3=3 or open_67_3=1  then
		Response.write "" & vbcrlf & "                     <input name=submit22332 type=""button"" onClick=""if(confirm('确认清除全部？')){window.location.href='delallLog.asp?tj="
		Response.write request("tj")
		Response.write "&FromTJ="
		Response.write request("FromTJ")
		Response.write "&complete=1&page_count="
		Response.write page_count
		Response.write "&currPage="
		Response.write  currpage
		Response.write "&A2="
	...
	if open_67_7=1 or open_67_7=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit43"" value=""打印""  onClick=""window.print();return  false;"" class=""anybutton2""/>" & vbcrlf & "                   "
	end if
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').disabled='disabled';document.getElementById('kh').style.display='none';document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';document.getElementById('ht1').disabled='';""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>" & vbcrlf & "                       </div></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>"& vbcrlf & "              </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <span id=""ht1""></span>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "       <tr class=""top"">" & vbcrlf & "   <td align=""center""><div align=""center"">选择</div></td>" & vbcrlf & "    <td width=""8%"" height=""27"" ><div align=""center"">号码</div></td>" & vbcrlf & "       <td width=""8%"" ><div align=""center"">联系人</div></td>" & vbcrlf & "       <td width=""25%"" ><div align=""center"">内容</div></td>" & vbcrlf & "        <td width=""6%"" ><div align=""center"">状态</div></td>" & vbcrlf & "        <td ><div align=""center"">时间</div></td>" & vbcrlf & "          <td ><div align=""center"">发送人</div></td>" & vbcrlf & "        <td ><div align=""center"">号码数量</div></td>" & vbcrlf & "      <td ><div align=""center"">字数</div></td>" & vbcrlf & "          <td ><div align=""center"">条数</div></td>" & vbcrlf & "     <td ><div align=""center"">           <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){window.open(this.value,'_self');}this.selectedIndex=0;"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	Response.write ">每页显示10条</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	Response.write ">每页显示20条</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	Response.write ">每页显示30条</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	Response.write ">每页显示50条</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	Response.write ">每页显示100条</option>" & vbcrlf & "          <option value=""logList.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	...
	toalNUm = 0
	Set rs = server.CreateObject("adodb.recordset")
	sql = "select * from logMessage  where del=1  "&Str_Result2&" order by ord desc "
	rs.Open sql, conn, 1, 1
	If rs.RecordCount<= 0 Then
		Response.write "<tr><td  align='center' colspan='11'  class='gray' height='200'>没有日志记录！</td></tr>"
	else
		i = 1
		...
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=2 and sort2=16"
		rs1.open sql1,conn,1,1
		if rs1.eof then
			open_2_16=0
			intro_2_16=0
		else
			open_2_16=rs1("qx_open")
			intro_2_16=rs1("qx_intro")
		end if
		rs1.close
		set rs1=Nothing
		do until rs.eof
			ord = rs("ord")
			mobile = rs("phone")
			mobile1 = Trim(rs("phone"))
			If InStr(mobile1, ",")>0 Then
				If Left(mobile1, 1) = "," Then mobile1 = Right(mobile1, Len(mobile1) -1)
'If InStr(mobile1, ",")>0 Then
				If Right(mobile1, 1) = "," Then mobile1 = Left(mobile1, Len(mobile1) -1)
'If InStr(mobile1, ",")>0 Then
'If InStr(mobile1, ",")>0 Then
				mobileNum = UBound(Split(mobile1, ",")) + 1
'If InStr(mobile1, ",")>0 Then
			else
				mobileNum = 1
			end if
...
'end if
			phones=rs("phone")
			if len(phones)>239 then
				phones=left(phones,239) & "..."
			end if
			If Len(mobile)>11 Then
				mobile = Left(mobile, 11)&"..."
			end if
			content = rs("content")
			If content<>"" Then
				contentNum = Len(content)
			else
				contentNum = 0
			end if
			stact = rs("stact1")
			...
			FDnum=0
			moneyNum=0
			If Len(rs("zunchcontent")&"")=0 Then
				If Len(content) Mod 64 = 0 Then
					SMSsendNum = Int(Len(content) / 64)
				else
					SMSsendNum = Int(Len(content) / 64) + 1
					SMSsendNum = Int(Len(content) / 64)
				end if
				moneyNum = SMSsendNum * mobileNum
			else
				zunchcontent=rs("zunchcontent")
				Arrzunch=Split(zunchcontent,",")
				FDnum=contentNum
				For iii=0 To ubound(Arrzunch)
					If Trim(Arrzunch(iii))<>"" Then
						Ccontent="尊敬的"&Arrzunch(iii)&"："&content
						If FDnum<Len(Ccontent) Then
							FDnum=Len(Ccontent)
						end if
						If Len(Ccontent) Mod 64 = 0 Then
							SMSsendNum=Int(Len(Ccontent) / 64)
						else
							SMSsendNum=Int(Len(Ccontent) / 64) + 1
							SMSsendNum=Int(Len(Ccontent) / 64)
						end if
						moneyNum=moneyNum+SMSsendNum
						SMSsendNum=Int(Len(Ccontent) / 64)
					else
						If Len(content) Mod 64 = 0 Then
							moneyNum = moneyNum+Int(Len(content) / 64)
'If Len(content) Mod 64 = 0 Then
						else
							moneyNum = moneyNum+Int(Len(content) / 64) + 1
'If Len(content) Mod 64 = 0 Then
						end if
					end if
				next
			end if
			If Len(rs("sendmoney")&"")<>0 Then moneyNum=zbcdbl(rs("sendmoney"))*10
			If stact <> "0" Then
				moneyNum = 0
			end if
			toalMoney = toalMoney + moneyNum
			moneyNum = 0
			If FDnum<>0  Then contentNum=contentNum&"~"&FDnum
			toalNUm = toalNUm + mobileNum
'If FDnum<>0  Then contentNum=contentNum&"~"&FDnum
			If person<>"" Then
				Set rs1 = server.CreateObject("adodb.recordset")
				sql1 = "select * from person  where ord="&person&" "
				rs1.Open sql1, conn, 1, 1
				If rs1.EOF Then
					personName = ""
					personRs1 = "0"
			...
			If stact<>"" Then
				Set rs1 = server.CreateObject("adodb.recordset")
				sql1 = "select * from errMessage  where errNum='"&stact&"'"
				rs1.Open sql1, conn, 1, 1
				If rs1.EOF Then
					stact = ""
				else
			...
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "            <td width=""6%"" align=""center"">" & vbcrlf & "                             "
			If mobile<>"" Then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>"
			end if
			Response.write "</td>" & vbcrlf & "                <td height=""28"" title="""
			Response.write phones
			Response.write """> <div align=""center"">"
			Response.write mobile
			Response.write "</div> </td>" & vbcrlf & "                <td height=""28""><div align=""center"">"
			if open_2_14=3 or CheckPurview(intro_2_14,trim(session("personzbintel2007")))=True then
				Response.write "<a href=""#"" onclick=""javascript:window.open('../person/content.asp?ord="
				Response.write pwurl(personRs1)
				Response.write "','personCon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此联系人详情"">"
				Response.write pwurl(personRs1)
			end if
			Response.write personName
			Response.write "" & vbcrlf & "                </a></div></td>" & vbcrlf & "                <td><div align=""left"" class=""cateLine"">"
			Response.write content
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">"
			Response.write stact
			Response.write "</div></td>" & vbcrlf & "          <td width=""12%""><div align=""center"">"
			Response.write addtime
			Response.write "</div></td>" & vbcrlf & "          <td width=""8%"" ><div align=""center"">"
			Response.write getGateName(rs("addcate"))
			Response.write "</div></td>" & vbcrlf & "          <td width=""6%"" ><div align=""center"">"
			Response.write mobileNum
			Response.write "</div></td>" & vbcrlf & "          <td width=""5%"" ><div align=""center"">"
			Response.write contentNum
			Response.write "</div></td>" & vbcrlf & "          <td width=""4%"" ><div align=""center"">"
			Response.write moneyNum
			Response.write "</div></td>" & vbcrlf & "          <td width=""12%"" class=""gray""><div align=""center"">"
			if open_67_13=1 and open_67_19<>1  then
				Response.write "" & vbcrlf & "" & vbcrlf & "<input  name=submit2 type=""button"" class=""anybutton"" onClick=""javascript:window.open('','newsSendSMS','width=' + 900 + ',height=' + 800 + ',fullscreen =no,scrollbars=0,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=200,top=100');document.getElementById('phone').value='"
				Response.write Replace(mobile1,vbcrlf,"")
				Response.write "';document.getElementById('sendform').submit();""   value=""单发"">"
			end if
			if open_67_3=3 or CheckPurview(intro_67_3,trim(addcate))=True then
				Response.write "<input  name=submit2 type=""button"" class=""anybutton""   onClick=""if(!confirm('确认删除吗？')){return false;}else{window.location.href='delLog.asp?tj="
				Response.write request("tj")
				Response.write "&FromTJ="
				Response.write request("FromTJ")
				Response.write "&ord="
			...
			rs.movenext
		Loop
		Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "            <td width=""6%"" align=""center"">&nbsp;</td>" & vbcrlf & "                <td height=""28"" >&nbsp;</td>" & vbcrlf & "                <td height=""28"">&nbsp;</td>" & vbcrlf & "                <td>&nbsp;</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td width=""12%"">&nbsp;</td>" & vbcrlf & "" & vbcrlf & "          <td width=""8%"" ><div align=""center"">&nbsp;合计:</div></td>" & vbcrlf & "          <td width=""6%""><div align=""center"">&nbsp;"
		Response.write toalNUm
		Response.write "</div></td>" & vbcrlf & "          <td width=""5%""><div align=""center""></div></td>" & vbcrlf & "          <td width=""4%"" ><div align=""center"">"
		Response.write toalMoney
		Response.write "</div></td>" & vbcrlf & "          <td width=""12%"" ></td>" & vbcrlf & "    </tr>" & vbcrlf & "      </table>" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "      <tr>" & vbcrlf & "      <td  class=""page""><table width=""100%"" border=""0"" align=""center"">" & vbcrlf & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""6%"" height=""30px;"" valign=""top""><div align=""center"">全选" & vbcrlf & "            <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)""/>" & vbcrlf & " </div></td>" & vbcrlf & "    <td width=""18%"" valign=""top"">"
		if open_67_13=1 and open_67_19<>1  then
			Response.write "<input  name=""submit22"" type=""submit"" class=""anybutton"" style=""cursor:hand""   onClick=""submit3();"" value=""群发所选"">"
		end if
		if open_67_3=3 or open_67_3=1  then
			Response.write "<input  name=submit2233 type=""submit"" onClick=""if(confirm('确认清除所选？')){submit1();}"" style=""cursor:pointer"" class=""anybutton""  value=""清除所选"">"
		end if
		Response.write "</td>" & vbcrlf & "    <td width=""76%"" valign=""top""><div align=""right"">" & vbcrlf & "            <span class=""black"">"
		...
		Response.write "条信息/页</span>&nbsp;<input  id=""currpage"" name=""currpage""  type=text   onkeyup=""value=value.replace(/[^\d\.]/g,'')""  size=3  >" & vbcrlf & "     &nbsp;<input type=""button"" name=""Submit422"" value=""跳转""  class=""anybutton2"" onClick=""window.location.href='logList.asp?tj="
		Response.write request("tj")
		Response.write "&FromTJ="
		Response.write request("FromTJ")
		Response.write "&page_count="
		Response.write page_count
		Response.write "&currPage='+document.getElementById('currpage').value+'&A2="
		Response.write page_count
		Response.write A2
		Response.write "&A1="
		Response.write A1
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "                  <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='logList.asp?tj="
			Response.write request("tj")
			Response.write "&FromTJ="
			Response.write request("FromTJ")
			Response.write "&page_count="
			...
			Response.write "&ret2="
			Response.write m2
			Response.write "'""/>" & vbcrlf & "                  <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='logList.asp?tj="
			Response.write request("tj")
			Response.write "&FromTJ="
			Response.write request("FromTJ")
			Response.write "&page_count="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "                  <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='logList.asp?tj="
			Response.write request("tj")
			Response.write "&FromTJ="
			Response.write request("FromTJ")
			Response.write "&page_count="
			...
			Response.write "&ret2="
			Response.write m2
			Response.write "'"" class=""page""/>" & vbcrlf & "                  <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='logList.asp?tj="
			Response.write request("tj")
			Response.write "&FromTJ="
			Response.write request("FromTJ")
			Response.write "&page_count="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & " </td>" & vbcrlf & "    </tr>        " & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "<form target=""newsSendSMS"" id=""sendform"" method=""post"" name=""sendform"" action =""../message/topadd.asp""><input type=""hidden"" name=""phone"" id=""phone""></form>" & vbcrlf & " "
	end if
	rs.close
	set rs = nothing
	action1 = "短信发送记录"
	Call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & " function test(){" & vbcrlf & "                if(!confirm('确认删除吗？')) return false;" & vbcrlf & "      }" & vbcrlf & "       function mm(form){ ///定义函数checkall,参数为form" & vbcrlf & "               for(var i=0;i<form.elements.length;i++){" & vbcrlf & "                        ///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "                     ///把表单里的内容依依付给e这个变量" & vbcrlf & "                      var e = form.elements[i];" & vbcrlf & "                       if (e.name != 'chkall'){e.checked = form.chkall.checked;}" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       function submit1(){" & vbcrlf & "             document.getElementById(""demo"").action = ""delallLog.asp?tj="
	Response.write request("tj")
	Response.write "&FromTJ="
	Response.write request("FromTJ")
	Response.write "&complete=1&type=1&page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currpage
	Response.write "&A2="
	...
	Response.write "&ret2="
	Response.write m2
	Response.write """;" & vbcrlf & "        }" & vbcrlf & "       function submit3(){" & vbcrlf & "             document.getElementById(""demo"").action = ""sendAllLog.asp?type=1&currPage="
	Response.write currPage
	Response.write "&page_count="
	Response.write page_count
