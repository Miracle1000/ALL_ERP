...
'[CATECONDITION] [CANCELCONDITION] [ORDERBY]
			...
			cateCondition = cateCondition & " AND ((ret_bcateid = "& uid &" AND ret_state = 1 ) OR (ret_state > 1 AND ret_addcateid = "& uid &" )) "
			sql = "SELECT COUNT(*) REMIND_CNT FROM reminderQueue a  with(nolock) " & vbcrlf &_
			" [CANCELJOINTABLE] " & _
...
'Private Sub Class_Initialize
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {     margin-top: 0px;}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src=""../inc/loadingBar.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/yp_returnList.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if open_101_1=3 then
		list=""
	elseif open_101_1=1 then
		list=" and ret_addcateid in ("&intro_101_1&")"
	else
		list=" and ret_addcateid=0"
	end if
	Str_Result=" where ret_del=1 "&list&""
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by ret_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by ret_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by ret_time desc"
	elseif px="4" then
		Ptr_Result=" order by ret_time asc"
	elseif px="5" then
		Ptr_Result=" order by ret_btime desc"
	elseif px="6" then
		Ptr_Result=" order by ret_btime asc"
	end if
	ret          =request("ret")
	ret2 =request("ret2")
	A    =request("A")
	...
	If Len(D&"")=0 Then D=0
	If D<>0 Then Str_Result=Str_Result&" and ret_state="&D&" "
	if ret<>"" then
		Str_Result=Str_Result&" and datediff(dd,'"& ret &"',ret_time)>=0"
	end if
	if ret2<>"" then
		Str_Result=Str_Result&" and datediff(dd,ret_time,'"& ret2 &"')>=0"
	end if
	if A<>"" then
	end if
	if C<>"" then
		if B="ffzt" then
			Str_Result=Str_Result&" and ret_title like '%"&C&"%'"
		elseif B="ffbh" then
			Str_Result=Str_Result&" and ret_cateid in (select ord from gate where name like '%"&C&"%')"
		elseif B="ffry" then
			Str_Result=Str_Result&" and ret_bcateid in (select ord from gate where name like '%"&C&"%')"
		elseif B="ffbz" then
			Str_Result=Str_Result&" and ret_note like '%"&C&"%'"
		end if
	end if''
	dim newstate
	newstate=trim(request("newstate"))
	if newstate="1" then
		Str_Result=Str_Result&" and ret_state = 1 and ret_del = 1  and ret_bcateid ="&session("personzbintel2007")
	end if
	If remind <> "" Then
	...
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "<form name=""data"" id=""data"" action=""returnList.asp"" method=""get"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">返还列表</td>" & vbcrlf & "                   <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"" class=""top_btns""><span id=""kh"">自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" style=""width:80px;"" name=""ret"" value="""
	Response.write ret
	Response.write """/><DIV id=daysOfMonth style=""POSITION: absolute""></DIV> 至：<input id=""daysOfMonth2Pos"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9"" style=""width:80px;"" name=""ret2"" value="""
	Response.write ret2
	Response.write """/><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;" & vbcrlf & "                              <select name=""D"" id=""D""style=""color:#000000"" onchange=""document.getElementById('data').submit();"">" & vbcrlf & "                                      <option value=""0"" "
	if D="0" then
		Response.write "selected"
	end if
	Response.write ">返还状态</option>" & vbcrlf & "                                   <option value=""1"" "
	if D="1" then
		Response.write "selected"
	end if
	Response.write ">未返还</option>" & vbcrlf & "                                     <option value=""2"" "
	if D="2" then
		Response.write "selected"
	end if
	Response.write ">已返还</option>" & vbcrlf & "                                     <option value=""3"" "
	if D="3" then
		Response.write "selected"
	end if
	Response.write ">已否决</option>" & vbcrlf & "                             </select>" & vbcrlf & "                       &nbsp;<select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                          <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""ffzt"" "
	Response.write "selected"
	if B="ffzt" then
		Response.write "selected"
	end if
	Response.write ">返还主题</option>" & vbcrlf & "                           <option value=""ffbh"" "
	if B="ffbh" then
		Response.write "selected"
	end if
	Response.write ">返还人员</option>" & vbcrlf & "                           <option value=""ffry"" "
	if B="ffry" then
		Response.write "selected"
	end if
	Response.write ">确认人员</option>" & vbcrlf & "                           <option value=""ffbz"" "
	if B="ffbz" then
		Response.write "selected"
	end if
	Response.write ">返还备注</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" size=""10"" value="""
	Response.write C
	Response.write """>" & vbcrlf & "                        <input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索""><input name=""page_count"" type=""hidden"" value="""
	...
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "<input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_return.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
	...
	Response.write "" & vbcrlf & "                       <a href=""javascript:;"" onClick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""></a></span></td>" & vbcrlf & "           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "               <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                 <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">返还主题</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">返还时间</div></td>" & vbcrlf & "                  <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">返还人员</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">库管人员</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">返还状态</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""center"">返还备注</div></td>" & vbcrlf & "                  <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                          <option>-请选择-</option>" & vbcrlf & "                    <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from O_proReturn "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			addcateid=rs7("ret_addcateid")
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "              <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "                  "
			if open_101_3=3 or CheckPurview(intro_101_3,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""17%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                   "
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('returnDetail.asp?ord="
				Response.write rs7("id")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("id")
				Response.write rs7("ret_title")
				Response.write "</a>" & vbcrlf & "         "
			else
				Response.write rs7("ret_title")
			end if
			Response.write "</td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("ret_time")
			Response.write "</div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                     <div align=""center"">" & vbcrlf & "               "
			ret_cateid=rs7("ret_cateid")
			if ret_cateid="" or isnull(ret_cateid) then ret_cateid=0
			sql8="select  * from gate where ord="&ret_cateid
			...
			rs8.close
			set rs8=nothing
			Response.write "" & vbcrlf & "                    </div>" & vbcrlf & "              </div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			ret_bcateid=rs7("ret_bcateid")
			if ret_bcateid="" or isnull(ret_bcateid) then ret_bcateid=0
			sql8="select  * from gate where ord="&ret_bcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Response.write rs8("name")
			end if
			rs8.close
			set rs8=nothing
			Response.write "</div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""func"">" & vbcrlf & "               <div align=""center"">"
			if rs7("ret_state")=2 then
				Response.write "已返还"
			elseif rs7("ret_state")=1 then
				Response.write "未返还"
			elseif rs7("ret_state")=3 then
				Response.write "已否决"
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""func ewebeditorImg"">"
			Response.write rs7("ret_note")
			Response.write "</td>" & vbcrlf & "                   <td width=""17%"" height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & ""
			If remind <> "" Then
				rid = helper.getRemindIdByOID(rs7("id"))
			...
			if rs7("ret_bcateid")=session("personzbintel2007") and rs7("ret_state")=1 and open_101_17=1 then
				Response.write "" & vbcrlf & "                 <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('ret_submit.asp?ord="
				Response.write rs7("id")
				Response.write "','neww35win','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=300,top=300')"" value=""确认""/>" & vbcrlf & "               "
				Response.write rs7("id")
			end if
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('returnDetail.asp?ord="
				Response.write rs7("id")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" value=""详情""/>" & vbcrlf & "              "
				Response.write rs7("id")
			end if
			if (open_101_2=3 or CheckPurview(intro_101_2,trim(addcateid))=True) and rs7("ret_state")=1  then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('returnUpdate.asp?ord="
				Response.write rs7("id")
			...
			if (open_101_2=3 or CheckPurview(intro_101_2,trim(addcateid))=True) and rs7("ret_state")<>1  then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('returnUpdate1.asp?ord="
				Response.write rs7("id")
				Response.write "','neww35win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""修改""/>" & vbcrlf & "                     "
				Response.write rs7("id")
			end if
			if open_101_3=3 or CheckPurview(intro_101_3,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('returndel.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
	...
	Response.write "" & vbcrlf & "             </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:300; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按返还时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按返还时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5');""><font color=""#2F496E"">按确认时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>"& vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按确认时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "   </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="返还列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function share(sid)" & vbcrlf & "{" & vbcrlf & "document.getElementById('data').action=""share.asp?sid=""+sid+""&tp=1&currPage="
	'call close_list(1)
	Response.write currpage
	Response.write "&A="
	Response.write A
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function shareall()" & vbcrlf & "{" & vbcrlf & "document.getElementById('data').action=""share.asp?tp=2&currPage="
	Response.write currpage
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function test(sid)" & vbcrlf & "{" & vbcrlf & " if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " return false;" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "     document.getElementById('data').action=""delete.asp?sid=""+sid+""&tp=1&currPage="
	Response.write remind
	Response.write currpage
	Response.write "&A="
	Response.write A
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " document.getElementById('data').action=""returnList.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "      document.getElementById('data').action=""returndel.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & ""
	if remind<>"" then
