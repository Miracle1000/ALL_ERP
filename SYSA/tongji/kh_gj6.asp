...
'if sort_lm<>"0" then
	...
	if m1="" then
		m1=cdate(year(date)&"-1"&"-1")
'if m1="" then
	end if
	if m2="" then
		m2=cdate(year(date)&"-12"&"-31")
'if m2="" then
	end if
	tdw=cdate(year(m1)&"-"&month(m1)&"-1")
'if m2="" then
...
'if ZBRuntime.MC(12000) and ZBRuntime.MC(9004) then
	...
	numcg1=0
	zs1=0
	Response.write " " & vbcrlf & "  <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">客户跟进统计</td>" & vbcrlf & "                        " & vbcrlf & "            <td  align=""left""><strong>"
	...
	Response.write "客户跟进每月走势"
	Response.write m1name
	Response.write m2name
	Response.write sw
	Response.write "</strong>" & vbcrlf & "                    </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style='table-layout:fixed'>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "<form method=""POST"" action=""kh_gj6.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "<td class=""top_btns"">选择统计条件：" & vbcrlf & "  <select name=""sort_lm"">"& vbcrlf &   "  <option value=""0"">跟进分类</option>" & vbcrlf & "       "
	if ZBRuntime.MC(12001) and ZBRuntime.MC(1000) then
	...
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "          <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""window.location.href='kh_gj6.asp?type_tj=2&W1="
	Response.write W1
	Response.write "&W2="
	Response.write W2
	Response.write "&W3="
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""window.location.href='kh_gj6.asp?type_tj=3&W1="
	Response.write W1
	Response.write "&W2="
	Response.write W2
	Response.write "&W3="
	...
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""kh_gj6.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""kh_gj6.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""kh_gj6.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""kh_gj6.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""kh_gj6.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	if open_1_10=1 or open_1_10=3  then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='kh_gj6.asp?type_tj=4&W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
	...
	for j=1 to m
		if j=1 then
			date1=m1
			if m=1 then
				date2=dateadd("d",1,cdate(m2))
			else
				date2=year(dateadd("m",1,m1))&"-"&month(dateadd("m",1,m1))&"-1"
				date2=dateadd("d",1,cdate(m2))
			end if
		elseif j<=m then
			date1=year(dateadd("m",j-1,m1))&"-"&month(dateadd("m",j-1,m1))&"-1"
'elseif j<=m then
			if j<m then
				date2=dateadd("m",1,cdate(date1))
			else
				date2=dateadd("d",1,cdate(m2))
			end if
		end if
...
'if sort_lm=1 then
			sql="select ord from reply a inner join (select ord as khord, isnull(replyShare,0) replyShare from tel where del=1) b on b.khord=a.ord and date7>='"&cdate(date1)&"' and date7<'"&cdate(date2)&"'  "&Module_Result&" "&powerWhere&""
		else
			...
			sql="select ord from reply a inner join (select ord as khord, isnull(replyShare,0) replyShare from tel where del=1) b on b.khord=a.ord and date7>='"&cdate(date1)&"' and date7<'"&cdate(date2)&"'  "&Module_Result&"  "&Str_Result2&" "&powerWhere&""
		end if
		rs.open sql,conn,1,1
		numcg1=rs.recordCount
		rs.close
		set rs=nothing
		title=year1&"年"&month1&"月"
		lburl="../work/genjin.asp?W1="&W1&"&W2="&W2&"&W3="&W3&"&sort="&E&"&sort1="&F&"&khjz="&khjz&"&khhy="&khhy&"&khly="&khly&"&ret="&date1&"&ret2="&cdate(date2)-1&"&time1="&time1&"&sort_lm="&sort_lm&""
		title=year1&"年"&month1&"月"
		total1(j,1)=numcg1
		total1(j,2)=title
		total1(j,0)=lburl
