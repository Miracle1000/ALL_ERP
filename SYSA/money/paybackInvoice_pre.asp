...
'items = rpt.Settings.item(i)
	...
	Sub App_doDel
		Dim ord : ord=request("id")
		cn.execute("update paybackinvoice set del=2 where id="&ord)
		Response.write "1"
		call db_close : Response.end()
	end sub
	Sub App_doDelAll
		...
		If Len(ordlist)>0 Then
			pwdel = app.power.GetPowerIntro(7,3)
			If cn.execute("select count(1) from paybackinvoice where isinvoiced=0 and id in ( "& ordlist &" ) and ( '" & pwdel & "'='' or charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')>0 )")(0)>0 Then
'pwdel = app.power.GetPowerIntro(7,3)
				oSQL = "update paybackinvoice set del=2 Where isinvoiced=0 and id in ( "& ordlist &" ) and ( '" & pwdel & "'='' or charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')>0 )"
'pwdel = app.power.GetPowerIntro(7,3)
				cn.Execute(oSQL)
				Response.write(1)
			else
	...
	Dim Czplist ,khname, khid
	Sub onReportInit(rpt, lt)
		Dim dat, rs, zdy,SC_level,i
		...
		dat= split( ""& "," & "" , ",")
		if app.GetText("ret")="" then    dat = split( dateadd("d",1,dateadd("yyyy",-1,date))& "," & date , ",")
		dat= split( ""& "," & "" , ",")
		companyid=app.base64.deurl(app.gettext("companyid"))
		if companyid&""="" then  companyid=0
		rpt.title = "待建立开票计划列表"
		rpt.pagesize = 10
		rpt.firstSpace = 200
		rpt.addfield "&nbsp;","select","dateType", "合同签订日期" & Chr(1) & "2" ,"",""
		rpt.SetNullMsg 0,"合同添加日期"
		rpt.addfield "","dates","date1","","", dat
		rpt.addfield "&nbsp;","select","invoicestatus", "未生成" & Chr(1) & "1" & Chr(2) & "部分生成" & Chr(1) & "2" ,"",""
		rpt.SetNullMsg 2,"开票计划进展"
		rpt.addfield "&nbsp;","select","kptype",   "明细开票" & Chr(1) & "2"& Chr(2) & "汇总开票" & Chr(1) & "1" ,"",""
		rpt.SetNullMsg 3,"开票类型"
		rpt.addfield "&nbsp;","select","searchType", khid & Chr(1) & "2"& Chr(2)&"单据主题" & Chr(1) & "3"& Chr(2)&"单据编号" & Chr(1) & "4","",""
		rpt.SetNullMsg 4,khname
		...
		rpt.aSearch.addField "开票类型","checks", "g_kptype","select 1 as value, '汇总开票' as name union all select 2, '明细开票' "
		rpt.aSearch.addField "开票计划进展","checks", "zt","select 1 as value, '未生成' as name union all select 2, '部分生成' "
		Dim bzsql,setbz
		...
		rpt.aSearch.addfield "已计划总额","moneys","money2",""
		rpt.aSearch.addfield "待计划总额","moneys","money3",""
		rpt.addsort "添加日期", "date7"
		rpt.addsort "签订日期", "date3"
		rpt.addsort "开票总额", "money1"
		rpt.addsort "客户名称", "name"
		rpt.sql = "exec erp_paybackInvoice_pre "& Info.User&",@dateType , @date1_0, @date1_1,@invoicestatus,@kptype,@searchType, @searckKey,@name_0,@name_1,@khid_0,@khid_1,@title_0,@title_1,@bh_0,@bh_1,@g_xsry,@g_kptype,@zt,@bz,@money1_0,@money1_1,@money2_0,@money2_1,@money3_0,@money3_1,&SearchMode,&excelmode,&ReportSort,"&companyid&""
		rpt.canexcel = app.power.existsPower(7001,10)
		rpt.canprint = app.power.existsPower(7001,7)
		rpt.onlyadSearchModel = True
		if app.power.existsPower(7001,13) then rpt.setBatHandle "id", "批量生成开票计划"
	end sub
	dim intro_1_1,intro_1_14,intro_26_1,intro_26_14
	dim intro_5_1,intro_5_14
	dim intro_41_1, intro_41_14
	sub app_oncreate(lvw)
		...
		col.width = 100
		col.align="left"
		col.formattext = "<div style='margin-left:10px'>添加:@cells[""date7""]<br>签订:@cells[""date3""]</div>"
		col.align="left"
		col.canSum=false
		...
		col.width = 130
		col.align = "left"
		col.formattext = "code:GetTitleHtml(rs(""company""),""@value"",rs(""cateid_kh""),rs(""share""),rs(""sort3""),rs(""tdel""))"
		col.canSum=false
		Set col = lvw.headers("title")
		col.title="单据主题"
		col.width = 180
		col.align="left"
		col.formattext = "code:GetNameHtml(rs(""fromtype""),rs(""ord""),""@value"",rs(""cateid""))"
		col.canSum=false
		Set col = lvw.headers("date3")
		col.width = 0
		col.display = "none"
		...
		col.formattext = "code:showCurrMoney(""@value"",""@cells[""moneycolor""]"")"
		col.canSum=true
		Set col = lvw.headers("invoicemode")
		col.title="开票类型"
		col.width = 70
		col.formattext = "code:GetKpType(""@value"")"
		col.canSum=false
		Set col = lvw.headers("invoicePro")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("invoiceProStatus")
		col.title="开票进展"
		col.width = 100
		col.formattext = "code:GetStatus(rs(""fromtype""),rs(""ord""),""@value"",rs(""cateid""))"
		col.canSum=false
...
'function showCurrMoney(money1,color)
	...
	Sub App_OnListviewExcel(lvw)
		...
		col.width = 130
		col.align = "left"
		col.formula = "GetTitleHtml(rs(""company""),@value,rs(""cateid_kh""),rs(""share""),rs(""sort3""),rs(""tdel""))"
		Set col =lvw.headers("date7")
		col.title ="添加日期"
		col.width = 100
		col.dbtype="str"
	...
	Function GetNameHtml(fromType, fromid, name , cateid)
		Dim namestr ,rs
		namestr = name
		If fromType ="CONTRACT" Then
			If isEmpty(intro_5_14) Then  intro_5_14 = app.power.GetPowerIntro(5,14)
			if Len(name)>0 then
				if intro_5_14="" or (instr(","& intro_5_14 &"," , ","& cateid &",")>0 And cateid<>"0")  then
		...
		ElseIf fromType ="ContractTH" Then
			...
			if intro_41_1="" or (instr(","& intro_41_1 &"," , ","& cateid &",")>0 And cateid<>"0") then
				if intro_41_14="" or (instr(","& intro_41_14 &"," , ","& cateid &",")>0 And cateid<>"0")  then
					namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../contractth/content.asp?ord=" & app.base64.pwurl(abs(fromid)) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
'if intro_41_14="" or (instr(","& intro_41_14 &"," , ","& cateid &",")>0 And cateid<>"0")  then
				else
					namestr = name
				end if
	...
	function GetTitleHtml(ord,title,cateid_kh ,share , sort3 , tdel)
...
'if intro_1_14="" or (instr(","& intro_1_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
			...
			if tdel=2 then TitleStr = TitleStr & "<span style='color:red'>(已删除)</span>"
		else
			TitleStr = "客户已彻底删除"
		end if
		GetTitleHtml = TitleStr
	end function
	function GetStatus(fromtype, ord, statusStr,cateid)
		If isEmpty(intro_7001_13) Then       intro_7001_13 = app.power.GetPowerIntro(7001,13)
		If intro_7001_13="" Or (instr(","& intro_7001_13 &"," , ","& cateid &",")>0 And cateid<>0) Then
			statusStr = statusStr & "<img src='../images/jiantou.gif'><a href='javascript:void(0);' onclick=""javascript:window.open('../../SYSN/view/finan/InvoiceManage/InvoicePlan/InvoicePlansAdd.ashx?fromtype="& fromtype &"&fromid=" & app.base64.pwurl(abs(ord)) & "','newwin','width=' + 960 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">生成开票计划</a>"
'If intro_7001_13="" Or (instr(","& intro_7001_13 &"," , ","& cateid &",")>0 And cateid<>0) Then
		end if
		GetStatus = statusStr
	end function
	Function GetKpType(kptype)
		If kptype = "1" Then
			GetKpType = "汇总开票"
