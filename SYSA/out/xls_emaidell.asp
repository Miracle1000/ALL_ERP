...
'If Not rs.Eof Then
	...
	xlWorksheet.Cells(1,1).Value = "发送邮箱"
...
'elseif G1=6 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_recv_list  where 1=1 "&Str_Result2&" order by ord desc "
	...
	do until rs.eof
		...
		xlWorksheet.Cells(1+i,1).Value = sendMail
		'cateid=""
		...
		'cateid=""
		xlWorksheet.Cells(1+i,3).Value = recvmail
		'cateid=""
		xlWorksheet.Cells(1+i,3).font.Size=10
		'cateid=""
		xlWorksheet.Cells(1+i,4).Value = csEmail
		'cateid=""
		...
		'cateid=""
		xlWorksheet.Cells(1+i,6).Value = sendtime
		'cateid=""
		xlWorksheet.Cells(1+i,6).font.Size=10
		'cateid=""
		xlWorksheet.Cells(1+i,7).Value = addtime
		'cateid=""
		xlWorksheet.Cells(1+i,7).font.Size=10
		'cateid=""
		xlWorksheet.Cells(1+i,8).Value = cateid
		'cateid=""
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>接收删除记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!接收删除记录导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	'set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("接收删除记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的接收删除记录</u></strong></font></a></p> " & vbcrlf & ""
