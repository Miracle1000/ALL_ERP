...
'if strW3<>"" Then
	...
	rs1.close
	set rs1=nothing
	if open_67_10=3 then
		list_tj=""
	elseif open_67_10=1 then
		list_tj="and addcate in ("&intro_67_10&")"
	else
	...
	sorce2=0
	sorce3=0
	if open_67_10="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_67_10&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'elseif G1=6 then
	...
	if px=1 then
		px_Result="order by addtime desc"
	elseif px=2 then
		px_Result="order by addtime asc"
	elseif px=3 then
		px_Result="order by phone desc"
	elseif px=4 then
		px_Result="order by phone asc"
	elseif px=5 then
		px_Result="order by stact1 desc"
	elseif px=6 then
		px_Result="order by stact1 asc"
	elseif px=7 then
		px_Result="order by content desc"
	elseif px=8 then
		px_Result="order by content asc"
	elseif px=9 then
		px_Result="order by person desc"
	elseif px=10 then
		px_Result="order by person asc"
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from logMessage  where del=1  "&Str_Result2&" order by ord desc "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "短信记录_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	x=0
	do until rs.eof  Or response.IsClientConnected = False
		...
		if rs("person")<>"" then
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select * from person where ord="&rs("person")&""
			rs1.open sql1,conn,1,1
			dim person
			if rs1.eof then
				person=""
		...
		if len(k)>32760 then
			maxlen=len(k)+1
'if len(k)>32760 then
			maxlenc=maxlen \ 32760
			if (maxlen mod 32760)<>0 then maxlenc=maxlenc+1
			maxlenc=maxlen \ 32760
			a=i
			for idx=0 to maxlenc-1
				a=i
				xlWorksheet.Cells(1+i+idx,1).NumberFormatLocal = "@"
				a=i
				if (idx+1)*32760>maxlen then
					a=i
					xlWorksheet.Cells(1+i+idx,1).Value =  mid(k,idx*32760+1,maxlen-idx*32760-1)
					a=i
					xlWorksheet.Cells(1+i+idx,1).font.Size=10
					a=i
					xlWorksheet.Cells(1+i+idx,1).HorizontalAlignment=1
					a=i
				else
					xlWorksheet.Cells(1+i+idx,1).Value =  mid(k,idx*32760+1,32759)
					a=i
					xlWorksheet.Cells(1+i+idx,1).font.Size=10
					a=i
					xlWorksheet.Cells(1+i+idx,1).HorizontalAlignment=1
				...
				if idx=0 then
					xlWorksheet.Cells(1+i+idx,2).Value =content
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,2).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,3).Value = stact
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,3).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,4).Value = adddate
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,4).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,5).Value = companyName
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,5).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,6).Value = person
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,6).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,7).Value = cateid
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,7).font.Size=10
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,8).Value = maxlen/12
'if idx=0 then
					xlWorksheet.Cells(1+i+idx,8).font.Size=10
'if idx=0 then
				end if
			next
			i=i+idx-1
...
'if idx=0 then
			xlWorksheet.Cells(1+i,2).Value =content
'if idx=0 then
			xlWorksheet.Cells(1+i,2).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,3).Value = stact
'if idx=0 then
			xlWorksheet.Cells(1+i,3).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,4).Value = adddate
'if idx=0 then
			xlWorksheet.Cells(1+i,4).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,5).Value = companyName
'if idx=0 then
			xlWorksheet.Cells(1+i,5).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,6).Value = person
'if idx=0 then
			xlWorksheet.Cells(1+i,6).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,7).Value = cateid
'if idx=0 then
			xlWorksheet.Cells(1+i,7).font.Size=10
'if idx=0 then
			xlWorksheet.Cells(1+i,8).Value = (len(k)+1) \ 12
'if idx=0 then
...
'if idx=0 then
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>短信记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!短信记录导出成功,共有"&(i-1-x)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("短信记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的短信记录</u></strong></font></a></p> " & vbcrlf & ""
