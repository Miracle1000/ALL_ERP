...
'function CurrCostDate(cn)
	...
	if open_6_1=3 then
		Str_Result=""
	elseif open_6_1=1 then
		Str_Result=" and c.cateid in ("&intro_6_1&")"
	else
		Str_Result="and c.cateid=0"
	end if
	sflg=request.form("sflg")
	if sflg=1 then
		if request("ret")<>"" then
			m1=request("ret")
			m2=request("ret2")
		...
		m3=request.form("indate_ret1")
		m4=request.form("indate_ret2")
		B=request.form("B")
		C=request.form("C")
		D=request.form("D")
		d1=request.form("d1")
		...
		money3=request.form("money1")
		money12=request.form("money12")
		czzt=request.form("czzt")
		e=request("e")
		e1=request("e1")
		tjr=request("tjr")
		jkr=request("jkr")
		...
		m3=request.querystring("indate_ret1")
		m4=request.querystring("indate_ret2")
		...
		intro=request.querystring("intro")
		money3=request.querystring("money1")
		money12=request.querystring("money12")
		czzt=request.querystring("czzt")
		e=request("e")
		e1=request("e1")
		tjr=request("tjr")
		jkr=request("jkr")
	...
	if B="3333333" or B="5555555" then
		new_list="<span class='alt_tx'></span>"
		timelg=session("timezbintel2007")
		if B="3333333" and timelg<>"" then
			noalt1=" and a.indate>'"&timelg&"'"
		end if
	end if
	if listbz<>"" then
		listbz=cdbl(listbz)
	...
	if e<>"" then
		Str_Result=Str_Result+" and a.cateid_sp="&e&" and a.complete in (0,1) "
'if e<>"" then
	end if
	if e1<>"" then
		Str_Result=Str_Result+" and a.complete in ("&e1&") "
'if e1<>"" then
	end if
	if czzt&""<>"" then
		If str&""="" Then
			str=str+" having (case when a.complete=3 and (f.gl>0 or ISNULL(g.dkmoney,0)>=isnull(sum(b.money1),0)) then 1 else 0 end) in ("&czzt&") "
'If str&""="" Then
		else
			str=str+" and (case when a.complete=3 and (f.gl>0 or ISNULL(g.dkmoney,0)>=isnull(sum(b.money1),0)) then 1 else 0 end) in ("&czzt&") "
'If str&""="" Then
		end if
	end if
	if title<>"" then
	...
	if bh<>"" then
		Str_Result=Str_Result&" and  a.bh like '%"&bh&"%' "
	end if
	if intro<>"" then
		Str_Result=Str_Result+"and  a.intro like '%"&intro&"%' "
'if intro<>"" then
	end if
	if money3<>"" then
		if isnumeric(money3)=false then
			Response.write("<script>alert('请输入合法金额');window.location=('paybx.asp');</script>")
			call db_close : Response.end
		else
			if str="" then
				str=str+" having sum(isnull(c.money1,0))>="&money3&" "
'if str="" then
			else
				str=str+" and sum(isnull(c.money1,0))>="&money3&" "
'if str="" then
			end if
		end if
	end if
	if money12<>"" then
		if isnumeric(money12)=false then
			...
			if str="" then
				str=str+" having sum(isnull(c.money1,0))<="&money12&" "
'if str="" then
			else
				str=str+" and sum(isnull(c.money1,0))<="&money12&" "
'if str="" then
			end if
		end if
	end if
	if request("ret_sp1")<>"" and request("ret_sp2")<>"" then
		Str_Result=Str_Result+" and a.id in(select ord from (SELECT MAX(date1) AS date1,ord FROM sp_intro WITH(NOLOCK) WHERE sort1=4 GROUP BY ord) a where date1>='"&cdate(request("ret_sp1"))&" 00:00:00' and date1<='"&cdate(request("ret_sp2"))&" 23:59:59') "
'if request("ret_sp1")<>"" and request("ret_sp2")<>"" then
	Elseif request("ret_sp1")<>"" And request("ret_sp2")="" Then
		Str_Result=Str_Result+" and a.id in(select ord from (SELECT MAX(date1) AS date1,ord FROM sp_intro WITH(NOLOCK) WHERE sort1=4 GROUP BY ord) a where date1>='"&cdate(request("ret_sp1"))&" 00:00:00') "
'Elseif request("ret_sp1")<>"" And request("ret_sp2")="" Then
	ElseIf request("ret_sp1")="" And request("ret_sp2")<>"" Then
		Str_Result=Str_Result+" and a.id in(select ord from (SELECT MAX(date1) AS date1,ord FROM sp_intro WITH(NOLOCK) WHERE sort1=4 GROUP BY ord) a where date1<='"&cdate(request("ret_sp2"))&" 23:59:59') "
'ElseIf request("ret_sp1")="" And request("ret_sp2")<>"" Then
	end if
	if request("ret_sp3")<>"" and request("ret_sp4")<>"" then
		Str_Result=Str_Result+" and a.id in(select gl from bank WITH(NOLOCK) where sort=40 and date1>='"&request("ret_sp3")&" 00:00:00' and date1<='"&cdate(request("ret_sp4"))&" 23:59:59' ) "
'if request("ret_sp3")<>"" and request("ret_sp4")<>"" then
	Elseif request("ret_sp3")<>"" And request("ret_sp4")="" then
		Str_Result=Str_Result+" and a.id in(select gl from bank WITH(NOLOCK) where sort=40 and date1>='"&request("ret_sp3")&" 00:00:00') "
'Elseif request("ret_sp3")<>"" And request("ret_sp4")="" then
	ElseIf request("ret_sp3")="" And request("ret_sp4")<>"" then
		Str_Result=Str_Result+" and a.id in(select gl from bank WITH(NOLOCK) where sort=40 and date1<='"&cdate(request("ret_sp4"))&" 23:59:59') "
'ElseIf request("ret_sp3")="" And request("ret_sp4")<>"" then
	end if
	if tjr<>"" then
		Str_Result=Str_Result+"and e.name like '%"&tjr&"%'"
'if tjr<>"" then
	end if
	if jkr<>"" then
		Str_Result=Str_Result+"and d.name like '%"&jkr&"%'"
'if jkr<>"" then
	end if
	...
	if typ="1" then
		Str_Result=Str_Result+"and a.complete=3 "
'if typ="1" then
	elseif typ="2" then
		Str_Result=Str_Result+"and a.complete=0 "
'elseif typ="2" then
	elseif typ="3" then
		Str_Result=Str_Result+"and a.complete=2 "
'elseif typ="3" then
	elseif typ="4" then
		Str_Result=Str_Result+"and a.complete=1 "
'elseif typ="4" then
	end if
...
'if W4<>"" and not isnull(W4) then
	...
	if C<>"" and (not isnull(C)) then
		if B="1" then
			Str_Result=Str_Result+" and a.title like '%"&C&"%'"
'if B="1" then
		elseif B="2" then
			Str_Result=Str_Result+"and d.name like '%"&C&"%'"
'elseif B="2" then
		elseif B="3" then
			Str_Result=Str_Result+"and e.name like '%"&C&"%'"
'elseif B="3" then
		elseif B="4" then
			if isnumeric(C)=false then
				...
				if str="" then
					str=str+" having sum(isnull(c.money1,0))="&C&" "
'if str="" then
				else
					str=str+" and sum(isnull(c.money1,0))="&C&" "
'if str="" then
				end if
			end if
		elseif B="8" then
			Str_Result=Str_Result+"and a.bh like '%"&C&"%'"
'elseif B="8" then
		elseif B="10" then
			Str_Result=Str_Result+"and h.name like '%"&C&"%' and a.sp_id>0"
'elseif B="10" then
		end if
	end if
	if m1<>"" then
		Str_Result=Str_Result+" and bxdate>='"&m1&"' "
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+" and bxdate<='"&m2&"' "
'if m2<>"" then
	end if
	if m3<>"" then
		Str_Result=Str_Result+" and datediff(s,a.indate,'"&m3&" 00:00:00')<=0"
'if m3<>"" then
	end if
	if m4<>"" then
		Str_Result=Str_Result+" and datediff(s,a.indate,'"&m4&" 23:59:59')>=0"
'if m4<>"" then
	end if
	If isnumeric(D&"") Then
		Str_Result=Str_Result & " and a.bxtype=" & D & " "
	end if
	If d1&""<>"" Then
		Str_Result=Str_Result & " and a.bxtype in (" & d1 & ") "
	end if
	If remind <> "" Then
	...
	if px=1 then
		pxResult=" order by a.title desc"
	elseif px=2 then
		pxResult=" order by a.title asc"
	elseif px=5 then
		pxResult=" order by a.bxdate desc,a.indate desc"
	elseif px=6 then
		pxResult=" order by a.bxdate asc,a.indate desc"
	elseif px=3 then
		pxResult=" order by a.indate desc"
	elseif px=4 then
		pxResult=" order by a.indate asc"
	end if
	conn.execute "delete from paybx where del=7 and addcate=" & session("personzbintel2007")
	conn.execute "update pay set bx=0 where ord in (select payid from paybxlist where del=7 and (addcate=" & session("personzbintel2007") & " or datediff(hh,indate,getdate())>=12)) and ord not in (select payid from paybxlist where del=1 and (addcate=" & session("personzbintel2007") & " or datediff(hh,indate,getdate())>=12)) "
	conn.execute "delete from paybxlist where del=7 and (addcate=" & session("personzbintel2007") & " or datediff(hh,indate,getdate())>=12)"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "form {" & vbcrlf & "      margin: 0px;" & vbcrlf & "    padding: 0px;" & vbcrlf & "}" & vbcrlf & ".ser_btn input[type='text']{height:20px;line-height:20px;}" & vbcrlf & ".ser_btn select{position:relative;top:1px;*top:0}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  "
	if open_6_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "       <tr>" & vbcrlf & "            <td width=""100%"" valign=""top"">" & vbcrlf & "              <form action=""paybx.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "            <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> " & vbcrlf & "                        <tr>" & vbcrlf & "                            <td class=""place"">费用报销列表</td>" & vbcrlf & "                               <td align=""left""><div align=""left""><A class='px_btn' onclick=""Myopen(User);return false;"" href=""javascript:void(0)"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></div></td>" & vbcrlf & "                             <td align=""right"">" & vbcrlf & "                                  <div align=""right"">                               " & vbcrlf & "                          <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "                              <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	if open_6_10=1 or open_6_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_fylist.asp'});}"" class=""anybutton2""/>" & vbcrlf & "                 "
	end if
	if open_6_7=1 or open_6_7=3 then
		Response.write "" & vbcrlf & "                     <input type=""button""  name=""print"" onclick=""javascript:window.print();return  false;"" value=""打印""  class=""anybutton2""/>" & vbcrlf & "                  "
	end if
	Response.write "" & vbcrlf & "                   </div></td><td align=""right""><img src=""../images/m_mpr.gif"" width=""3"" height=""32""/></td>" & vbcrlf & "      </tr>                      " & vbcrlf & "    </table> " & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "           <tr class=""table_btn_tr""><td colspan=""13"" class='ser_btn' style='height:50px;line-height:50px;padding-top:0;padding-bottom:0;border:0!important' align=""right"">" & vbcrlf & "                       <input type=""radio"" name=""rad1"" checked=""checked"">列表<input type=""radio"" name=""rad1"" onClick=""window.location.href='paybxdet.asp?"
	Response.write sdk.ReturnURL("sflg=")
	Response.write "';"">明细" & vbcrlf & "                  "
	...
	Response.write "'>"
	Response.write "                                     " & vbcrlf & "                        <select name=""czzt"">" & vbcrlf & "                      <option value="""">出账状态</option>" & vbcrlf & "                        <option value=""0"""
	If czzt&"" = "0" Then Response.write " selected"
	Response.write ">未出账</option>" & vbcrlf & "                     <option value=""1"""
	If czzt&"" = "1" Then Response.write " selected"
	Response.write ">已出账</option>" & vbcrlf & "                     </select>" & vbcrlf & "                       <select name=""D"" id=""D"">" & vbcrlf & "                            <option>选择报销分类</option>" & vbcrlf & "                           "
	Dim sql_bxtype, rs_bxtype
	sql_bxtype = "select ord,sort1 from sortonehy where gate2='10001' order by gate1 desc"
	Set rs_bxtype = conn.execute (sql_bxtype)
	While rs_bxtype.eof = False
		Response.write "" & vbcrlf & "                             <option value="""
		Response.write rs_bxtype("ord")
		Response.write """"
		If CStr(Trim(D)) = CStr(rs_bxtype("ord")) Then Response.write(" selected")
		Response.write ">"
		Response.write rs_bxtype("sort1")
		Response.write "</option>" & vbcrlf & "                            "
		rs_bxtype.movenext
	wend
	rs_bxtype.close
	Set rs_bxtype = Nothing
	Response.write "" & vbcrlf & "                     </select>" & vbcrlf & "                       <select name=""B"" id=""B"">" & vbcrlf & "                    <option value=""1"" "
	if B="1" then
		Response.write "selected"
	end if
	Response.write ">报销主题</option>" & vbcrlf & "                   <option value=""2"" "
	if B="2" then
		Response.write "selected"
	end if
	Response.write ">报销人员</option>" & vbcrlf & "                   <option value=""3"" "
	if B="3" then
		Response.write "selected"
	end if
	Response.write ">添加人员</option>   " & vbcrlf & "                        <option value=""10"" "
	if B="10" then
		Response.write "selected"
	end if
	Response.write ">当前审批人员</option>" & vbcrlf & "                       <option value=""8"" "
	if B="8" then
		Response.write "selected"
	end if
	Response.write ">报销编号</option>   " & vbcrlf & "                        </select>" & vbcrlf & "                       <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "                       <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "                   <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton2""/>" & vbcrlf & "                        <input type=""button"" name=""openbn"" id=""openbn"" value=""高级检索""  class=""anybutton2"" onclick=""$('#w').show();$('#w').window('open');$('#hideDiv').show();""/>                       " & vbcrlf & "                        </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   </form>" & vbcrlf & "         <tr class=""top"">" & vbcrlf & "            <td width=""4%"" align=""center""><div align=""center"">选择</div></td>" & vbcrlf & "             <td width=""10%""><div align=""center"">报销主题</div></td>" & vbcrlf & "                  <td width=""8%""><div align=""center"">报销编号</div></td>" & vbcrlf & "          <td width=""9%""><div align=""center"">报销总额</div></td>" & vbcrlf & "                  <td width=""9%""><div align=""center"">审批总额</div></td>" & vbcrlf & "              <td width=""5%""><div align=""center"">审批状态</div></td>" & vbcrlf & "            <td width=""5%""><div align=""center"">出账状态</div></td>" & vbcrlf & "              <td width=""6%""><div align=""center"">报销人员</div></td>" & vbcrlf & "              <td width=""6%""><div align=""center"">添加人员</div></td>" & vbcrlf & "              <td width=""6%""><div align=""center"">审批人员</div></td>" & vbcrlf & "                 <td width=""7%""><div align=""center"">报销日期</div></td>" & vbcrlf & "              <td width=""12%""><div align=""center"">添加时间</div></td>                     " & vbcrlf & "                <td width=""12%""><div align=""center"">操作</div></td>" & vbcrlf & "               </tr>" & vbcrlf & "           "
	dim n
	n=0
	almoney=0
	asmoney=0
	if len(str) = 0 then
		str = " having sum(isnull(c.money1,0))>=0 and sum(isnull(c.money1,0))<999999999 "
	end if
	conn.cursorlocation = 3
	set rs=server.CreateObject("adodb.recordset")
	sql = "select isnull(sum(isnull(smoney,0)),0) asmoney,isnull(sum(isnull(summoney,0)),0) almoney from (" & vbcrlf &_
	"  select a.id,c.cateid as payerID,a.addcate as adderID," & vbcrlf &_
	"  sum(case when a.complete=0 or a.complete=2 then 0 else isnull(b.money1,0) end) as summoney,sum(c.num) as sumnum,sum(isnull(c.money1,0)) as smoney " & vbcrlf &_
	"  from paybx a WITH(NOLOCK) " & vbcrlf &_
	"  inner join paybxlist b WITH(NOLOCK) on a.id=b.bxid " & vbcrlf &_
	"  left join pay c WITH(NOLOCK) on c.ord=b.payid " & vbcrlf &_
	"  left join gate d WITH(NOLOCK) on c.cateid=d.ord " & vbcrlf &_
	"  left join gate e WITH(NOLOCK) on a.addcate=e.ord " & vbcrlf &_
	"  left join gate h WITH(NOLOCK) on a.cateid_sp=h.ord " & vbcrlf &_
	"  left join (select distinct gl from bank where sort=40 and del=1) f on f.gl=a.id and a.complete=3 "&_
	"  left join (select bxid,isnull(sum(dkmoney),99999999) dkmoney from dk group by bxid) g on g.bxid=a.id and a.complete=3 "&_
	"  where a.del=1 and b.del=1 "&Str_Result&" " &_
	"  group by a.id,c.cateid,a.addcate,a.complete,f.gl,g.dkmoney " & vbcrlf &_
	") tt " & vbcrlf &_
	"inner join paybx a on a.id=tt.id " & vbcrlf &_
	"left join gate d WITH(NOLOCK) on tt.payerID=d.ord " & vbcrlf &_
	"left join gate e WITH(NOLOCK) on tt.adderID=e.ord " & vbcrlf
	rs.open sql,conn,3,1
	if rs.RecordCount<=0 then
	else
		asmoney=CDbl(rs("asmoney"))
		almoney=cdbl(rs("almoney"))
	end if
	set rs=nothing
	set rs=server.CreateObject("adodb.recordset")
	if B="3333333" or B="5555555" then
		if open_6_1=3 then
			Str_Result_41=""
		elseif open_6_1=1 then
			Str_Result_41=" and c.cateid in ("&intro_6_1&")"
		else
			Str_Result_41=" and c.cateid=0"
		end if
		sql="select distinct a.id,a.bh,a.indate,a.complete,a.title,a.bz,a.bxdate,a.cateid_sp,c.cateid as paycate,d.name as payer,e.name as adder,sum(case when a.complete=0 or a.complete=2 then 0 else isnull(b.money1,0) end) as summoney,sum(c.num) as sumnum,sum(isnull(c.money1,0)) as smoney,h.name as spCate,(case when a.complete=3 and (f.gl>0 or ISNULL(g.dkmoney,0)>=isnull(sum(b.money1),0)) then 1 else 0 end) ckStatus,case when exists(select top 1 1 from  PrinterInfo a1 where a1.sort = 21  and a1.formid = a.id) then '<span style=''color:#009900''>[已打印]</span>' else '<span  style=''color:#FF0000''>[未打印]</span>' end  as printStatus "&_
		"from paybx a WITH(NOLOCK) " & vbcrlf &_
		"left join paybxlist b WITH(NOLOCK) on b.bxid=a.id " & vbcrlf &_
		"left join pay c WITH(NOLOCK) on c.ord=b.payid " & vbcrlf &_
		"left join gate d WITH(NOLOCK) on c.cateid=d.ord " & vbcrlf &_
		"left join gate e WITH(NOLOCK) on a.addcate=e.ord " & vbcrlf &_
		"left join gate h WITH(NOLOCK) on a.cateid_sp=h.ord " & vbcrlf &_
		"left join (select distinct gl from bank WITH(NOLOCK) where sort=40 and del=1) f on f.gl=a.id and a.complete=3 "&_
		"left join (select bxid,isnull(sum(dkmoney),99999999) dkmoney from dk WITH(NOLOCK) group by bxid) g on g.bxid=a.id and a.complete=3 "&_
		"where (a.cateid_sp="&session("personzbintel2007")&" and a.sp_id>0 and a.del=1 and a.complete in (0,1) "&Str_Result_41&") "&noalt4&" " & vbcrlf &_
		"group by a.id,a.bh,a.indate,a.complete,a.title,a.bz,a.bxdate,a.cateid_sp,c.cateid,d.name,e.name,h.name,f.gl,g.dkmoney " & vbcrlf &_
		"order by a.indate desc"
	else
		sql="select tt.*,a.bh,a.indate,a.complete,a.title,a.bz,a.bxdate,a.cateid_sp,payerID as paycate,d.name as payer,e.name as adder from (" & vbcrlf &_
		"select a.id,c.cateid as payerID,a.addcate as adderID,h.name as spCate,(case when a.complete=3 and (f.gl>0 or ISNULL(g.dkmoney,0)>=isnull(sum(b.money1),0)) then 1 else 0 end) ckStatus,case when exists(select top 1 1 from  PrinterInfo a1 where a1.sort = 21  and a1.formid = a.id) then '<span style=''color:#009900''>[已打印]</span>' else '<span  style=''color:#FF0000''>[未打印]</span>' end  as printStatus," & vbcrlf &_
		"sum(case when a.complete=0 or a.complete=2 then 0 else isnull(b.money1,0) end) as summoney,sum(c.num) as sumnum,sum(isnull(c.money1,0)) as smoney " & vbcrlf &_
		"from paybx a WITH(NOLOCK) " & vbcrlf &_
		"inner join paybxlist b WITH(NOLOCK) on a.id=b.bxid " & vbcrlf &_
		"left join pay c WITH(NOLOCK) on c.ord=b.payid " & vbcrlf &_
		...
		"left join (select bxid,isnull(sum(dkmoney),99999999) dkmoney from dk WITH(NOLOCK) group by bxid) g on g.bxid=a.id and a.complete=3 "&_
		"where a.del=1 and b.del=1 "&Str_Result&" " &_
		"group by a.id,c.cateid,a.addcate,h.name,a.complete,f.gl,g.dkmoney " & vbcrlf &_
		") tt " & vbcrlf &_
		"inner join paybx a WITH(NOLOCK) on a.id=tt.id " & vbcrlf &_
		"left join gate d WITH(NOLOCK) on tt.payerID=d.ord " & vbcrlf &_
		"left join gate e WITH(NOLOCK) on tt.adderID=e.ord " & vbcrlf &_
		pxResult
	end if
	rs.open sql,conn,3,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""deletebxall.asp?"
		Response.write sdk.ReturnURL("sflg=")
		Response.write """ >" & vbcrlf & "         "
		smoney=0
		amoney=0
		do until rs.eof
			...
			spCate = rs("spCate")
			money1=rs("summoney")
			money2=zbcdbl(rs("smoney"))
			if money1="" or isnull(money1) then money1=0
			amoney=cdbl(money1)+cdbl(amoney)
'if money1="" or isnull(money1) then money1=0
			smoney=cdbl(money2)+cdbl(smoney)
'if money1="" or isnull(money1) then money1=0
			num1=rs("sumnum")
			cateid=rs("paycate")
			...
			if cateid="" or isnull(cateid) then cateid=0
			if  timelg<>"" and datediff("s",timelg,rs("indate"))>0 then
				k=k&new_list
			end if
			...
			if open_6_3=3 or (open_6_3=1 And CheckPurview(intro_6_3,trim(cateid))=True And cateid<>0) Or open_6_20=3 or (open_6_20=1 and CheckPurview(intro_6_20,trim(cateid))=True And cateid<>0) then
				Response.write " " & vbcrlf & "                     <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("id")
				Response.write """>" & vbcrlf & "                 "
			end if
			Response.write "</span></div>         </td>           " & vbcrlf & "                <td><div align=""left"">" & vbcrlf & ""
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                      <a href=""###"" onClick=""javascript:window.open('paybxcontent.asp?paybxid="
				Response.write rs("id")
				Response.write "','neww3win','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs("id")
				Response.write k
			...
			Response.write "<span style=""color:red""></span></div></td>" & vbcrlf & "              <td><div align=""center"">"
			Response.write bh
			...
			Response.write trade8
			Response.write "&nbsp;"
			Response.write iif(complete=0 or complete=2, Formatnumber(0,num_dot_xs,-1), Formatnumber(money1,num_dot_xs,-1))
			Response.write "&nbsp;"
			...
			Select Case rs("ckStatus")
			Case 0 : Response.write "未出账"
			Case 1 : Response.write "已出账"
			End Select
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			Response.write payer
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			Response.write adder
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			If complete <> "2" And  complete <> "3" Then Response.write spCate
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			Response.write rs("bxdate")
			Response.write "</div></td>" & vbcrlf & "           <td><div align=""center"">"
			Response.write rs("indate")
			...
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "        <input name=""Submit53"" type=""button"" class=""anybutton2""  onClick=""javascript:window.open('paybxcontent.asp?paybxid="
				Response.write rs("id")
				Response.write "','neww3win','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""详情""/>" & vbcrlf & "        "
				Response.write rs("id")
			end if
			if (rs("complete")=0) and (open_6_17=3 or CheckPurview(intro_6_17,trim(cateid))=True) then
				Response.write "" & vbcrlf & "        <input name=""Submit53"" type=""button"" class=""anybutton2""  onClick=""javascript:window.location='paybxcorrect.asp?"
				Response.write sdk.ReturnURL("paybxid="& rs("id"))
				Response.write "'"" value=""修改""/>" & vbcrlf & "        "
			end if
			if (rs("complete")=0 or rs("complete")=1) and rs("cateid_sp")&""=session("personzbintel2007")&"" and (open_6_16=3 or (open_6_16=1 and CheckPurview(intro_6_16,cstr(cateid)))) then
				Response.write "" & vbcrlf & "        <input name=""Submit"" type=""button"" class=""anybutton2"" onClick=""window.location.href='setbx.asp?"
				Response.write sdk.ReturnURL("paybxid="& rs("id"))
				Response.write "'"" value=""审批""/>" & vbcrlf & "        "
			end if
			if rs("complete")=3 and rs("ckStatus")=0 and (open_6_20=3 or CheckPurview(intro_6_20,trim(cateid))=True) then
				Response.write "" & vbcrlf & "        <input name=""Submit"" type=""button"" class=""anybutton2"" onClick=""window.location.href='paybxout2.asp?"
				Response.write sdk.ReturnURL("ord="& pwurl(rs("id")))
				Response.write "'"" value=""出账""/>" & vbcrlf & "        "
			end if
			if (open_6_3=3 or CheckPurview(intro_6_3,trim(cateid))=True) And IsCostAnalysis(conn , 41003, rs("id") , 0 ,rs("bxdate")) =False And IsHasOrderCostsNotionalPooling(conn,rs("id"))=False then
				Response.write "" & vbcrlf & "                 <input name=""Submit"" type=""button"" class=""anybutton2"" onClick=""if(confirm('确认删除？')){window.location.href='Deletebx.asp?"
				Response.write sdk.ReturnURL("ord="& rs("id"))
				Response.write "&bxdate="
				Response.write rs("bxdate")
		...
		loop
		Response.write "" & vbcrlf & "   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "       <td height=""27"" colspan=""3""><div align=""right"">本页合计：</div></td>" & vbcrlf & "          <td height=""27"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		Response.write Formatnumber(smoney,num_dot_xs,-1)
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "      <td height=""27"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		Response.write Formatnumber(amoney,num_dot_xs,-1)
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "    <td height=""27"" colspan=""8"">&nbsp;</td>" & vbcrlf & "  </tr>" & vbcrlf & "   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "      <td height=""27"" colspan=""3""><div align=""right"">共合计：</div></td>" & vbcrlf & "     <td height=""27"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		Response.write Formatnumber(asmoney,num_dot_xs,-1)
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "      <td height=""27"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
		...
		Response.write Formatnumber(almoney,num_dot_xs,-1)
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "    <td height=""27"" colspan=""8"">&nbsp;</td>" & vbcrlf & "  </tr>" & vbcrlf & "      </table>" & vbcrlf & "    </td>" & vbcrlf & "       </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""chkall"" id=""chkall"" value=""Check All"" onClick=""mm()"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >" & vbcrlf & "       "
		if open_6_3=1 or open_6_3=3 then
			Response.write "&nbsp;<input type=""submit"" onClick=""return test();"" name=""Submit2"" value=""批量删除"" class=""anybutton2""/>"
		end if
		if open_6_20=1 or open_6_20=3 then
			Response.write " &nbsp;<input type=""button"" name=""Submit422"" value=""批量出账""  onclick=""ask();"" class=""anybutton2""/>"
		end if
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:152;display:none;"">" & vbcrlf & "<table width=""150"" height=""160""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""152"">" & vbcrlf & "        <table width=""150"" height=""152"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按报销主题排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl""" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=5');""><font color=""#2F496E"">按报销日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按报销日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<div id=""w"" title=""高级检索""  style=""width:490px;height:500px;padding:5px;background: #fafafa;top:60px;display:none;""  class=""easyui-window"" closed=""true"" >" & vbcrlf & "<form action=""paybx.asp"" id=""date1"" onSubmit=""return Validator.Validate(this,2)"" name=""date22"">" & vbcrlf & "<div id=""hideDiv"" style=""border-style: none; border-width: 0px; border-color: inherit; margin: 0px; display:none; padding:0px;"">" & vbcrlf & "       <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:440px"">" & vbcrlf & "           <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "           <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "   </div>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "           <td width=""20%"" class=""func""><div align=""right"">报销编号</div></td>" & vbcrlf & "           <td width=""80%""><input name=""bh"" type=""text"" id=""bh"">" & vbcrlf & "       <input type=""hidden"" name=""sflg"" value=""1""></td>" & vbcrlf & "      </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "           <td width=""20%"" class=""func""><div align=""right"">报销主题</div></td>" & vbcrlf & "           <td width=""80%""><input name=""title"" type=""text"" id=""title"">" & vbcrlf & "            <input type=""hidden"" name=""sflg"" value=""1""></td>" & vbcrlf & "      </tr>" & vbcrlf & "  <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "         <td width=""20%""><div align=""right"">报销概要</div></td>" & vbcrlf & "      <td width=""80%""><input name=""intro"" type=""text"" id=""intro""></td>" & vbcrlf & "      </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "         <td width=""20%""><div align=""right"">报销金额</div></td>" & vbcrlf & "    <td width=""80%""><input name=""money1"" type=""text""  class=""easyui-numberbox"" id=""money1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision=""2""  value=""0"">" & vbcrlf & "       -" & vbcrlf & "     <input name=""money12"" type=""text"" id=""money12"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision=""2""  value=""999999999""></td>" & vbcrlf & "       </tr>                     " & vbcrlf & "      <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "<td width=""20%""><div align=""right"">报销日期</div></td>" & vbcrlf & "      <td width=""80%"">" & vbcrlf & "                       <INPUT name=""rett"" size=""9"" readonly id=""duepaydate1divPos"" onmouseup=toggleDatePicker(""duepaydate1div"",""date.rett"") value="
	Response.write m1
	...
	Response.write m1
	Response.write m2
	Response.write """>" & vbcrlf & "                        <DIV id=""duepaydate2div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate2div""></DIV>                           </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "         <td width=""20%""><div align=""right"">添加日期</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & "                       <INPUT name=""indate_ret1"" size=""9"" readonly id=""paybxindate1Pos"" onmouseup=toggleDatePicker(""paybxindate1"",""date.indate_ret1"") value="""
	Response.write m3
	Response.write """>" & vbcrlf & "                        <DIV id=""paybxindate1"" style=""POSITION: absolute;z-index:10"" name=""paybxindate1""></DIV>-" & vbcrlf & "                      <INPUT name=""indate_ret2"" size=""9"" readonly id=""paybxindate2Pos"" onmouseup=toggleDatePicker(""paybxindate2"",""date.indate_ret2"") value="""
	Response.write m3
	Response.write m4
	Response.write """>" & vbcrlf & "                        <DIV id=""paybxindate2"" style=""POSITION: absolute;z-index:10"" name =""paybxindate2""></DIV>                               </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">审批日期</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & "                        <INPUT name=""ret_sp1"" size=""9"" readonly id=""ret_sp1divPos"" onmouseup=toggleDatePicker(""ret_sp1div"",""date.ret_sp1"")>" & vbcrlf & "                       <DIV id=""ret_sp1div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp1div""></DIV>-" & vbcrlf & "                    <INPUT name=""ret_sp2"" size=""9"" readonly id=""ret_sp2divPos"" onmouseup=toggleDatePicker(""ret_sp2div"",""date.ret_sp2"")>" & vbcrlf & "                       <DIV id=""ret_sp2div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp2div""></DIV>                           </td>" & vbcrlf & "</tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">出账日期</div></td>" & vbcrlf &      "<td width=""80%"">" & vbcrlf & "                    <INPUT name=""ret_sp3"" size=""9"" readonly id=""ret_sp3divPos"" onmouseup=toggleDatePicker(""ret_sp3div"",""date.ret_sp3"")>" & vbcrlf & "                       <DIV id=""ret_sp3div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp3div""></DIV>-" & vbcrlf & "                 <INPUT name=""ret_sp4"" size=""9"" readonly id=""ret_sp4divPos"" onmouseup=toggleDatePicker(""ret_sp4div"",""date.ret_sp4"")>" & vbcrlf & "                    <DIV id=""ret_sp4div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp4div""></DIV>                           </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <tdwidth=""20%""><div align=""right"">出账状态</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & "                  <label><INPUT type=""checkbox"" name=""czzt"" value=""0"">未出账</label>" & vbcrlf & "                    <label><INPUT type=""checkbox"" name=""czzt"" value=""1"">已出账</label></td>" & vbcrlf & "   </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">报销分类</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & "            "
	sql_bxtype = "select ord,sort1 from sortonehy where gate2='10001' order by gate1 desc"
	Set rs_bxtype = conn.execute (sql_bxtype)
	While rs_bxtype.eof = False
		Response.write "" & vbcrlf & "             <input name=""d1"" type=""checkbox"" id=""e1""  value="""
		Response.write rs_bxtype("ord")
		Response.write """>"
		Response.write rs_bxtype("sort1")
		Response.write "&nbsp;" & vbcrlf & "               "
	...
	rs_bxtype.close
	Set rs_bxtype = Nothing
	Response.write "" & vbcrlf & "      </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">审批状态</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & "             <input name=""e1"" type=""checkbox"" id=""e1""  value=""3"">审批通过&nbsp;" & vbcrlf & "     <input name=""e1"" type=""checkbox"" id=""e1""  value=""1"">审批中&nbsp;" & vbcrlf & "     <input name=""e1"" type=""checkbox"" id=""e1""  value=""0"">待审批&nbsp;" & vbcrlf & "      <input name=""e1"" type=""checkbox"" id=""e1"" value=""2"">审批未通过 " & vbcrlf & "                 </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">币种</div></td>" & vbcrlf & "     <td width=""80%"">" & vbcrlf & ""
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	set rs8=nothing
	...
	set rs8=nothing
	Response.write "" & vbcrlf & "     </select></td>" & vbcrlf & "  </tr>" & vbcrlf & "   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "     <td width=""20%""><div align=""right"">报销人员</div></td>" & vbcrlf & "     <td width=""80%"">"
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	Response.write "</td>" & vbcrlf & "        </tr>                    " & vbcrlf & "</table>" & vbcrlf & "       </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""padding-top:5px;text-align:right;height:27px;line-height:27px;"">" & vbcrlf & "            <a class=""easyui-linkbutton""  href=""javascript:void(0);"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "              <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "   </div>" & vbcrlf & "  </div>" & vbcrlf & "</form>" & vbcrlf & "  </div>" & vbcrlf & ""
	action1="费用报销列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function Myopen(divID){" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "    divID.style.top=20;" & vbcrlf & "}" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "} " & vbcrlf & "function ask() {" & vbcrlf & "      document.all.form1.action = ""paybxout_hb.asp?"
	Response.write sdk.ReturnURL("sflg=")
	Response.write """;" & vbcrlf & "        document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "   var a = document.getElementsByTagName(""input"");" & vbcrlf & "   var b=document.getElementById(""chkall"");" & vbcrlf & "   if(b.checked==true)" & vbcrlf & "       {" & vbcrlf & "for (var i=0; i<a.length; i++)" & vbcrlf & "          {" & vbcrlf & "               if (a[i].type == ""checkbox"")" & vbcrlf & "                      { a[i].checked = true;}" & vbcrlf & "         }" & vbcrlf & "   }" & vbcrlf &    "else" & vbcrlf & "   {" & vbcrlf & "                for (var i=0; i<a.length; i++)" & vbcrlf & "          {" & vbcrlf & "if (a[i].type == ""checkbox"")" & vbcrlf & "                      {a[i].checked = false;}" & vbcrlf & "         }" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & ""
	if remind<>"" then
