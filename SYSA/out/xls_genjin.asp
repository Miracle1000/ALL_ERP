...
'If request("H")="30" Then
	...
	If searchType = "1" Then
		...
		If F2 <> "" Then
			Select Case F1
			Case 1
			tel_Result = tel_Result & " AND name LIKE '%"& F2 &"%' "
			Case 2
			tel_Result = tel_Result & " AND name NOT LIKE '%"& F2 &"%' "
			Case 3
			tel_Result = tel_Result & " AND name = '"& F2 &"' "
			Case 4
			tel_Result = tel_Result & " AND name <> '"& F2 &"' "
			Case 5
			tel_Result = tel_Result & " AND name LIKE '"& F2 &"%' "
		...
		If sort4 <> "" Then
			tel_Result = tel_Result & " AND sort IN ("& sort4 &") "
		end if
		If sort5 <> "" Then
			tel_Result = tel_Result & " AND sort1 IN ("& sort5 &") "
		end if
		...
		If A2 <> "" Then
			tel_Result = tel_Result & " AND area IN ("& A2 &") "
		end if
	end if
	...
	if sort_lm<>"0" AND sort_lm<>"" And sort_lm<>"100000" then
...
'if F<>"" then
	...
	if newstate="1" then
		tel_Result=tel_Result&" and  sort3=1 "
		Str_Result2=Str_Result2&"  and id1 is null AND ISNULL(alt,0) = 0 "
		set rs9=server.CreateObject("adodb.recordset")
		sql9="select ord,intro,num1,tq1,fw1 from setjm2  where ord=32  and cateid="&session("personzbintel2007")&""
		rs9.open sql9,conn,1,1
		if rs9.eof then
		else
			if rs9("fw1")="1" then
				Str_Result2=Str_Result2&"and cateid="&session("personzbintel2007")&""
			end if
		end if
		rs9.close
		set rs9=nothing
	...
	If remind <> "" Then
		...
		Str_Result2 = Str_Result2 & " and id in (" & helper.listSQL("ids") & ") "
	end if
	...
	jz=trim(request("jz"))
	if link="yes" then
		tel_Result=tel_Result &" and ord in (select isnull(ord,0) from tel where sort3=1 and del=1)"
		if area<>"" then
			tel_Result=tel_Result &" and area in ("& area &")"
		end if
		if ly<>"" then
			tel_Result=tel_Result &" and ly in ("& ly &")"
		end if
		if trade<>"" then
			tel_Result=tel_Result &" and trade in ("& trade &")"
		end if
		if jz<>"" then
			tel_Result=tel_Result &" and jz in ("& jz &")"
		end if
		if C<>"" and C<>"客户名称" then
			if B="name" then
				tel_Result=tel_Result &" and name like '%"& C &"%'"
			elseif B="pym" then
				tel_Result=tel_Result &" and pym like '%"& C &"%'"
			elseif B="phone" then
				tel_Result=tel_Result &" and phone like '%"& C &"%'"
			elseif B="product" then
				tel_Result=tel_Result &" and product like '%"& C &"%'"
			elseif B="zdy1" then
				tel_Result=tel_Result &" and zdy1 like '%"& C &"%'"
			elseif B="zdy2" then
				tel_Result=tel_Result &" and zdy2 like '%"& C &"%'"
			elseif B="zdy3" then
				tel_Result=tel_Result &" and zdy3 like '%"& C &"%'"
			elseif B="zdy4" then
				tel_Result=tel_Result &" and zdy4 like '%"& C &"%'"
			elseif B="fax" then
				tel_Result=tel_Result &" and fax like '%"& C &"%'"
			elseif B="address" then
				tel_Result=tel_Result &" and address like '%"& C &"%'"
			elseif B="intro" then
				tel_Result=tel_Result &" and intro like '%"& C &"%'"
			elseif B="zip" then
				tel_Result=tel_Result &" and zip like '%"& C &"%'"
			end if
		end if
		If gjfs <> "" Then
			tel_Result=tel_Result+"AND  sort98 IN ("& gjfs &")"
'If gjfs <> "" Then '
		end if
		If sort_lm <> "" And sort_lm <> "0" Then
			tel_Result=tel_Result+"and  sort1 IN ("&sort_lm&") "
'If sort_lm <> "" And sort_lm <> "0" Then
		end if
		If keywords <> "" Then
			If sType = "1" Then
				tel_Result=tel_Result+" AND name LIKE '%"& keywords &"%' "
'If sType = "1" Then
			ElseIf sType = "2" Then
...
'If sType = "1" Then
	...
	if normalCate&""="1" then Str_Result2 = Str_Result2 & " AND c.cateidDel=1 "
	...
	if W1<>"" And personType = "" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select ord,sort1 from  gate1 where ord in ("&W1&")"
	...
	if W2<>"" And personType = "" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select ord,sort2 from  gate2 where ord in ("&W2&")"
		rs.open sql,conn,1,1
		If rs.eof = False Then
			W2name=rs("sort2")
			W2name=W2name&">>"
	...
	if W3<>"" And personType = "" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select ord,name from  gate where ord in ("&W3&")"
		rs.open sql,conn,1,1
		If rs.eof = False Then
			W3name=rs("name")
			W3name=W3name&">>"
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("洽谈进展")
	xsheet.showHeader "客户名称,联系时间,跟进方式,洽谈进展"
	xsheet.movenext
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=1 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1_7=0
	else
	...
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=1 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1_10=0
	else
	...
	sql="select * from ("&_
	"select r.*,"&_
	"(case when sort1=1 "& str11 &" then (select top 1 qx_open from power where sort1=1 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=2 "& str12 &" then (select top 1 qx_open from power where sort1=3 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=3 "& str13 &" then (select top 1 qx_open from power where sort1=4 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=4 "& str14 &" then (select top 1 qx_open from power where sort1=5 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=5 "& str15 &" then (select top 1 qx_open from power where sort1=22 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=6 "& str16 &" then (select top 1 qx_open from power where sort1=42 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=7 "& str17 &" then (select top 1 qx_open from power where sort1=41 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=8 "& str18 &" then (select top 1 qx_open from power where sort1=2 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=75 "& str19 &" then (select top 1 qx_open from power where sort1=75 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=102001 "& str20 &" then (select top 1 qx_open from power where sort1=95 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=102002 "& str21 &" then (select top 1 qx_open from power where sort1=96 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"else 0 "&_
	"end) "&_
	"as qxopen, "&_
	"(case when sort1=1 "& str11 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=1 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=2 "& str12 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=3 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=3 "& str13 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=4 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=4 "& str14 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=5 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=5 "& str15 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=22 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=6 "& str16 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=42 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=7 "& str17 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=41 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=8 "& str18 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=2 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=75 "& str19 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=75 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=102001 "& str20 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=95 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"when sort1=102002 "& str21 &" then (select top 1 cast(qx_intro as nvarchar(3000)) from power where sort1=96 and sort2=5 and ord="&session("personzbintel2007")&") "&_
	"else '0' "&_
	"end) "&_
	"as qxintro,isnull(sort98,0) as sort98s ,g.del as cateidDel  from reply r "&_
	" left join dbo.gate g on r.cateid=g.ord "&_
	") as c "&_
	" where del=1 "&Str_Result2&" and ord in(select ord from tel "&tel_Result&") and (qxopen=3 or (qxopen=1 and  charindex(','+cast(cateid as nvarchar(10))+',',','+qxintro+',')>0)) "&gj_Result&" order by date7 desc"
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 Then
		xsheet.writecell "没有信息"
		xsheet.writestr ""
		xsheet.writestr ""
		xsheet.writestr ""
		xsheet.writestr ""
		...
		do While not rs.eof
			...
			if rs("ord")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select name,isnull(cateid,0) as cateid,isnull(share,0) as share from tel where ord="&rs("ord")&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
					catename="此客户已被删除"
					cateid_kh=0
			...
			xsheet.writehtml "<td width='200' align='left'>"&catename&"</td>"
			v=""
			if sortname<>1 Then
				if biao<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select "&clsname&" from "&biao&" where ord="&ord2&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						v= "已被删除!"
					else
						v= "关联"&biao_name&"："&rs7(""&clsname&"")&""
					end if
					rs7.close
					set rs7=nothing
				end if
			end if
			xsheet.writehtml "<td width='200' align='center'>"&rs("date7")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&getsort98(rs("sort98s"))&"</td>"
			v="" : s=""
			allowExt="bmp,jpeg,png,gif,jpg"
			v=rs("uploadfile")
			If Len(v)=0 Or isnumeric(v)=False Then v=0
			Set rs7=conn.execute("select * from reply_file_Access where ord in(" & sdk.FormatNumList(v) & ")")
			Do While Not rs7.eof
				v=Split(rs7("Access_url"),".")(ubound(Split(rs7("Access_url"),".")))
				If InStr(1,allowExt,v,1)>0 Then
					s=s&"<img src='" &  xApp.images.add( Replace( rs7("Access_url"),"..", GetHttpType() & "://"&Request.ServerVariables("SERVER_NAME") & ":" & Request.ServerVariables("SERVER_PORT") ) ) & "'>"
				else
					s=s&rs7("oldname") &"&nbsp;"
				end if
				rs7.movenext
			...
			v=rs("intro")&s
			v=v&"(跟进："&rs("name")&"&nbsp;&nbsp;"
			if rs("name2")<>"" then v=v&"对方联系人："&rs("name2")&"&nbsp;&nbsp;"
			v=v&rs("date7")&")"
			...
			if rs7.eof then
				...
				do While not rs7.eof
					v=v&"点评："&rs7("intro")&"(点评人："& rs7("name")&"&nbsp;&nbsp;"&rs7("date7")&")"
					rs7.movenext
				Loop
			end if
			rs7.close
			xsheet.writehtml "<td width='800' align='left'>"&v&"</td>"
			If Not isnumeric(C1) Then C1 = 0
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户跟进导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户跟进"&i&"条导出成功!</B>"";</script>"
	tfile=Server.MapPath("客户跟进情况_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.Dispose
	...
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'客户跟进情况_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="客户跟进情况导出"
	call close_list(1)
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的客户跟进</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
