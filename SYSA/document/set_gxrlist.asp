...
'if ZBRuntime.MC(35000) = False  And ZBRuntime.MC(18100)=false Then
	...
	Dim xmlPath,rs1,sql1,skd,commSP,nextSpId,nextGates,ordtype,i,arr_gates1,arr_gates2,bakll,editord,ch,vch
	...
	Sub Page_Load
		...
		ord = app.base64.deurl(app.getText("id"))
		headtitle="文档添加"
		status="Add"
		editord= ord
		session("ch")=request("ch")
		session("vch")=request("vch")
		app.addDefaultScript
		Response.write app.DefTopBarHTML(app.virPath, "", "", "")
		Response.write "" & vbcrlf & "<style type=""text/css"">" & vbcrlf & ".label{" & vbcrlf & "  border-top:0px;" & vbcrlf & " text-align:right;               " & vbcrlf & "        padding-right:6px;" & vbcrlf & "}" & vbcrlf & ".labe2{" & vbcrlf & "      border-top:0px;" & vbcrlf & " padding-top:6px;        " & vbcrlf & "        padding-left:6px;" & vbcrlf & "}" & vbcrlf & "form{margin:0; padding:0;}" & vbcrlf & "html {overflow-x: hidden; overflow-y: scroll; } " & vbcrlf & "body.defcomm{background-repeat:repeat-x;_overflow:visible;background-image:url("""")}" & vbcrlf & "#comm_itembarbg {display:none;}" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" src=""AjaxSubmit.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """ type=""text/javascript""></script>" & vbcrlf & "<script type=""text/javascript"" language=""javascript"">" & vbcrlf & "function logout()  " & vbcrlf & " {" & vbcrlf & "     parent.window.opener = null;" & vbcrlf & "     parent.window.open("""", ""_self"");" & vbcrlf & "     parent.window.close();" & vbcrlf & " }" & vbcrlf & "" & vbcrlf & "function CloseWind(){ " & vbcrlf & "var strq=document.getElementsByName(""chkSon"");" & vbcrlf & "var objarrayq=strq.length;" & vbcrlf & "var q="""";" & vbcrlf & "for (i=0;i<objarrayq;i++)" & vbcrlf & "{" & vbcrlf & "  if(strq[i].checked == true)" & vbcrlf & "  {" & vbcrlf & "   q+=strq[i].value+"","";" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "var strm=document.getElementsByName(""vchkSon"");" & vbcrlf & "var objarraym=strm.length;" & vbcrlf & "var m="""";" & vbcrlf & "for (i=0;i<objarraym;i++)" & vbcrlf & "{" & vbcrlf & "  if(strm[i].checked == true)" & vbcrlf & "  {" & vbcrlf & "   m+=strm[i].value+"","";" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "q = q.substr(0,q.length-1);" & vbcrlf & "m = m.substr(0,m.length-1);" & vbcrlf & "if (q == """"){" & vbcrlf & "     q=0;" & vbcrlf & "}" & vbcrlf & "if (m == """"){" & vbcrlf & "        m=0;" & vbcrlf & "}" & vbcrlf & "//抛出主页面数据" & vbcrlf & "opener.setValue(q,m); " & vbcrlf & "window.close(); " & vbcrlf & "} " & vbcrlf & "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "// --列头全选框被单击---" & vbcrlf & "function ChkAllClick(sonName, cbAllId){" & vbcrlf & "      var arrSon = document.getElementsByName(sonName);" & vbcrlf & " var cbAll = document.getElementById(cbAllId);" & vbcrlf & " var tempState=cbAll.checked;" & vbcrlf & " for(i=0;i<arrSon.length;i++) {" & vbcrlf & "  if(arrSon[i].checked!=tempState)" & vbcrlf & "                   arrSon[i].click();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "window.resetbtn=function(){" & vbcrlf & "    document.location.href=""set_gxrlist.asp?id="
		Response.write app.base64.pwurl(ord)
		Response.write "&fromtype="
		Response.write fromtype
		Response.write """;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "        " & vbcrlf & ""
		IF editord = "" Then
			Response.write "" & vbcrlf & "      FnUserOrdByFromtype('','','"
			Response.write session("ch")
			Response.write "','"
			Response.write session("vch")
			Response.write "','"
			Response.write fromtype
			Response.write "');" & vbcrlf & ""
		else
			Response.write "" & vbcrlf & "      FnUserOrdByFromtype('"
			Response.write editord
			Response.write "','upid','"
			Response.write session("ch")
			Response.write "','"
			Response.write session("vch")
		...
		Response.write "" & vbcrlf & "mysel2(""ss"",""0"");" & vbcrlf & "</script>" & vbcrlf & "<div style=""margin-left:20px;margin-top:5px;""><p style=""text-align:left; height:30px; ""><span class=""reseetTextColor"" style=""font-family:Arial;font-size:13px;font-weight:bold;font-style:normal;text-decoration:none;color:#5B7CAE;"">共享给以下用户</span><span class=""reseetTextColor"" style=""font-family:Arial;font-size:13px;font-weight:normal;font-style:normal;text-decoration:none;color:#5B7CAE;"">（在需要共享的权限下的方框打勾）</span></p>" & vbcrlf & "</div>" & vbcrlf & "<form action=""?__msgid=dosave&id="
		Response.write "');" & vbcrlf & ""
		Response.write app.base64.pwurl(ord)
		Response.write "&fromtype="
		Response.write fromtype
		Response.write """ method=""POST"" name=""demo""id=""demo"">" & vbcrlf & "     <div style=""width:100%; margin-bottom:3px; margin-left:20px;"">" & vbcrlf & "            <div id=""con1""><select name=""sel1"" id=""sel1"" style=""width:150px;"" onChange=""mysel(this.value);FnUserOrd(this.value,'bm','"
		Response.write fromtype
		Response.write session("ch")
		Response.write "','"
		Response.write session("vch")
		Response.write "');mysel2($BI('sel2').value,this.value)"">" & vbcrlf & "               <option value=""0"">所属部门</option>" & vbcrlf & "               "
		dim str,skd1,skd2
		skd = 0
		skd1 = 0
		skd2 = 0
		set rs88 = cn.execute("select w1,qx_open,w2 from power2 where cateid="&info.user&" and sort1=2 ")
		...
		rs88.close
		set rs88=Nothing
		If Len(skd & "")=0 Then skd = "0"
		If Len(skd2 & "")=0 Then skd2 = "0"
		IF skd1 = 3 Then
			str = ""
		else
			If Len(skd & "")=0 Then
		...
		Set rs=cn.execute("select sort1,ord from gate1 where 1=1"&str)
		...
		loop
		set rs=nothing
		Response.write "" & vbcrlf & "            </select>&nbsp;" & vbcrlf & "            <span id=""span1""><select name=""sel2"" id=""sel2"" style=""width:150px;"" onChange=""mysel2(this.value,$BI('sel1').value);FnUserOrd(this.value,'xz','"
		Response.write session("ch")
		Response.write "','"
		Response.write session("vch")
		Response.write "');"">" & vbcrlf & "            <option value='ss'>所属小组</option>" & vbcrlf & "               "
		IF skd1 = 3 Then
			str = ""
		else
			str = " and ord in ("&skd2&")"
		end if
		Set rs=cn.execute("select sort2,ord from gate2 where 1=1"&str)
		do while rs.eof = False
			Response.write "" & vbcrlf & "                                             <option value="""
			Response.write rs("ord")
			Response.write """>"
		...
		loop
		set rs=nothing
		Response.write "            " & vbcrlf & "            </select></span></div> " & vbcrlf & "        </div>      " & vbcrlf & "        <div id=""ShowSubjectKJ""><!--显示分享人框--></div>" & vbcrlf & "</form>" & vbcrlf & ""
		set rs=nothing
	end sub
	Sub app_dosave
		Dim share1,share2,id
		share1 = app.getText("chkSon")
		share2 = app.getText("vchkSon")
		id = app.base64.deurl(app.getText("id"))
		fromtype=app.getText("fromtype")&""
		If share1 <> "" Then share1 = Replace(share1,", ",",")
		If share2 <> "" Then share2 = Replace(share2,", ",",")
		if fromtype<>"" then
			cn.Execute("if exists( select 1 from DocSortsAndSharingRelation where sortonehyId = "& id &" ) begin UPDATE DocSortsAndSharingRelation SET postView = '', postDown = '',share1='"& share1 &"',share2='"& share2 &"' WHERE sortonehyId = "& id &" end else begin  insert into DocSortsAndSharingRelation(sortonehyId,share1,share2,updatetime)values ("& id &",'"& share1 &"','"& share2 &"',getdate()) end ")
		else
			cn.Execute("UPDATE document SET postView = '', postDown = '',share1='"& share1 &"',share2='"& share2 &"' WHERE id = "& id &" ")
		end if
		Response.write "<script>parent.window.opener.getSharePostList("& id &",0,'"& fromtype &"');parent.window.close();</script>"
		Response.end
	end sub
