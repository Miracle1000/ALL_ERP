...
'if isnull(values) then IsTime = null : exit function
	...
	if lie_2="" or isnull(lie_2) then lie_2=3
	A    =request("A")
	B    =request("B")
	C    =request("C")
	M    =request("M")
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and bk_addcateid in ("&W3&") "
	end if
	if D1<>"" then
	...
	if D10<>"" then
		...
		if instr(D10,1)>0 then
			aa=aa&" or bk_allnum<=bk_num "
		end if
		if instr(D10,2)>0 then
			aa=aa&" or (bk_num<bk_allnum and bk_num!=0) "
		end if
		if instr(D10,3)>0 then
			aa=aa&" or (bk_num=0 and bk_allnum!=0) "
		end if
	...
	if D20<>"" then
		Str_Result=Str_Result&" and bk_allmoney >="&ml.isNum(D20)&""
	end if
	if D201<>"" then
		Str_Result=Str_Result&" and bk_allmoney <="&ml.isNum(D201)&""
	end if
	if D21<>"" then
		Str_Result=Str_Result&" and bk_postion like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and bk_note like '%"&ml.De(D22)&"%'"
	end if
	if M<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&" 00:00:00',bk_pubtime)>=0 "
'if M<>"" then
	end if
	if K<>"" then
		Str_Result=Str_Result+" and  datediff(s,bk_pubtime,'"&K&" 23:59:59')>=0 "
'if K<>"" then
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',bk_printtime)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,bk_printtime,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if L3<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L3&" 00:00:00',bk_addtime)>=0 "
'if L3<>"" then
	end if
	if L4<>"" then
		Str_Result=Str_Result+" and  datediff(s,bk_addtime,'"&L4&" 23:59:59')>=0 "
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	if px="1" then
		Ptr_Result=" order by bk_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by bk_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by bk_pubtime desc"
	elseif px="4" then
		Ptr_Result=" order by bk_pubtime asc"
	elseif px="5" then
		Ptr_Result=" order by bk_printtime desc"
	elseif px="6" then
		Ptr_Result=" order by bk_printtime asc"
	end if
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',bk_addtime)>=0 "
'if ret<>"" then
	end if
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,bk_addtime,'"&ret2&" 23:59:59')>=0 "
'if ret2<>"" then
	end if
	if C<>"" then
		if B="name" then
		...
		elseif B="post" then
			Str_Result=Str_Result&" and bk_postion like '%"&ml.De(C)&"%'"
		end if
	end if''
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_bookData.asp?ret="
	Response.write ret
	Response.write "&ret2="
	Response.write ret2
	Response.write "&B="
	...
	Response.write "&C="
	Response.write C
	Response.write """ method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">图书列表</td>" & vbcrlf & "                       <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">   "    & vbcrlf &            "     <tr> "& vbcrlf &            "   <td colspan=""10"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding:0!important"">" & "               <span id=""kh"">自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" name=""ret"" readonly=""true"" value="""
	Response.write ret
	Response.write """/><DIV id=daysOfMonth style=""POSITION: absolute""></DIV>&nbsp;至：<input id=""daysOfMonth2Pos"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9"" name=""ret2"" readonly=""true"" value="""
	Response.write ret2
	...
	Response.write ">ISBN</option>" & vbcrlf & "                               <option value=""post"" "
	if B="post" then Response.write "selected"
	Response.write ">存放地点</option>" & vbcrlf & "                   </select>&nbsp;<input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	...
	if open_103_9=1  then
		Response.write "" & vbcrlf & "                     <input name=""submit1"" type=""button""  class=""anybutton2"" value=""导入"" onClick=""window.location.href='../load/UpLoad_bookData.asp';""/>" & vbcrlf & "                      "
	end if
	if open_103_10=3 or (open_103_10=1 and CheckPurview(intro_103_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_bookData.asp?px="
		Response.write px
		...
		Response.write "&C="
		Response.write Server.URLEncode(ml.Dep(C)&"")
		Response.write "&L3="
	...
	if open_103_13=1 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit43"" value=""图书添加""  onClick=""window.location='Add_bookData.asp'"" class=""anybutton2""/>" & vbcrlf & "                  "
	end if
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">书名</div></td>" & vbcrlf & "              <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">分类</div></td>" & vbcrlf & "             <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">作者</div></td>" & vbcrlf & "                <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "                     <select name=""select1"" id=""select1"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid #FFFFFF; overflow:hidden"" >" & vbcrlf & "                          <option>--请选择--</option>" & vbcrlf & "               <option value=""lie_1=1"" "
	if lie_1="1" then Response.write "selected"
	Response.write ">出版时间</option>" & vbcrlf & "               <option value=""lie_1=2"" "
	if lie_1="2" then Response.write "selected"
	Response.write ">版次</option>" & vbcrlf & "               <option value=""lie_1=3"" "
	if lie_1="3" then Response.write "selected"
	Response.write ">印刷时间</option>" & vbcrlf & "                      <option value=""lie_1=4"" "
	if lie_1="4" then Response.write "selected"
	Response.write ">印次</option>" & vbcrlf & "             </select>" & vbcrlf & "            </div></td>" & vbcrlf & "             <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "              <select name=""select12"" id=""select12"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                           <option>--请选择--</option>" & vbcrlf & "               <option value=""lie_2=3"" "
'if lie_1="4" then Response.write "selected"
	if lie_2="3" then Response.write "selected"
	Response.write ">ISBN</option>" & vbcrlf & "               <option value=""lie_2=1"" "
	if lie_2="1" then Response.write "selected"
	Response.write ">页数</option>" & vbcrlf & "               <option value=""lie_2=2"" "
	if lie_2="2" then Response.write "selected"
	Response.write ">字数</option>" & vbcrlf & "                           <option value=""lie_2=4"" "
	if lie_2="4" then Response.write "selected"
	Response.write ">存放位置</option>" & vbcrlf & "             </select>" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">总数量</div></td>" & vbcrlf & "                   <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">可借阅数量</div></td>" & vbcrlf & "                <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">借阅状态</div></td>" & vbcrlf & "                 <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                   <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
'if lie_2="4" then Response.write "selected"
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.set_name from O_regbook a left join O_bookset b on a.bk_type=b.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""10%"" height=""30"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_bookData.asp?ord="
				Response.write rs7("id")
			...
			Response.write "</td>" & vbcrlf & "                   <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "             "
			Response.write rs7("set_name")
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "                <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("bk_auther")
			Response.write "</div></td>" & vbcrlf & "             <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			if lie_1="1" then
				Response.write rs7("bk_pubtime")
			elseif lie_1="2" then
				Response.write rs7("bk_pubnum")
			elseif lie_1="3" then
				Response.write rs7("bk_printtime")
			elseif lie_1="4" then
				Response.write rs7("bk_printnum")
			end if
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			if lie_2="1" then
				Response.write FormatNumber(rs7("bk_pagenum"),num1_dot,-1)
'if lie_2="1" then
			elseif lie_2="2" then
				Response.write FormatNumber(rs7("bk_pagesize"),num1_dot,-1)
'elseif lie_2="2" then
			elseif lie_2="3" then
				Response.write rs7("bk_isbn")
			elseif lie_2="4" then
				Response.write rs7("bk_postion")
			end if
			Response.write "</div></td>" & vbcrlf & "               <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("bk_allnum"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "               <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("bk_num"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "               <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			if rs7("bk_allnum")<=rs7("bk_num") then
				Response.write "未借阅"
			elseif rs7("bk_num")<rs7("bk_allnum") and rs7("bk_num")<>0 then
				Response.write "部分借阅"
			elseif rs7("bk_num")=0 then
				Response.write "全部借阅"
			end if
			Response.write "</div></td>" & vbcrlf & "              <td width=""17%"" height=""30"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                   <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_bookData.asp?ord="
				Response.write rs7("id")
			...
			manbool=true
			sqltext="select top 1 Ld_bkid from O_LendBookmx where Ld_bkid="&rs7("id")
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				manbool=false
			end if
			set rstxt=nothing
			sqltext="select top 1 Chk_bkid from O_ChkBookmx where Chk_bkid="&rs7("id")
			set rstxt=conn.execute(sqltext)
			if not rstxt.eof then
				manbool=false
			end if
			set rstxt=nothing
			if (open_103_2=3 or CheckPurview(intro_103_2,trim(addcateid))=True) and manbool then
				Response.write "" & vbcrlf & "                       <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_bookData.asp?ord="
				Response.write rs7("id")
			...
			if open_103_131=1 and rs7("bk_num")>0 then
				Response.write "" & vbcrlf & "                        <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""window.location=('Add_Lend.asp?ord="
				Response.write rs7("id")
				Response.write "')"" value=""借阅""/>" & vbcrlf & "                     "
			end if
			if open_103_133=1 then
				Response.write "" & vbcrlf & "                        <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""window.location=('Add_Check.asp?ord="
				Response.write rs7("id")
				Response.write "')"" value=""盘点""/>" & vbcrlf & "                   "
			end if
			if (open_103_3=3 or CheckPurview(intro_103_3,trim(addcateid))=True) and manbool then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_bookData.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
	...
	if open_103_131=1 then
		Response.write "" & vbcrlf & "             <input name=""submit61"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit61""  style=""cursor:hand""  value=""批量借阅"" onClick=""testall1()""/>" & vbcrlf & "             "
	end if
	if open_103_133=1 then
		Response.write "" & vbcrlf & "             <input name=""submit62"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit62""  style=""cursor:hand""  value=""批量盘点"" onClick=""testall2()""/>" & vbcrlf & "             "
	end if
	Response.write "" & vbcrlf & "             </td>" & vbcrlf & "     </form>" & vbcrlf & "       <form name=""oneself"" action=""List_bookData.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按出版时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按出版时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按印刷时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按印刷时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	dim actinon1
	action1="图书列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_bookData.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action='Del_bookData.asp?px="
	Response.write ml.Dep(px)
	Response.write "&A="
	Response.write ml.Dep(A)
	Response.write "&B="
