...
'If Not rs.Eof Then
	...
	if action="backAll" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select userID,id,isnull(interviewID,0) interviewID from hr_person  where del<>0 order by id desc"
		rs.open sql,conn,1,1
		errNum=0
		sussNum=0
		if not rs.eof then
			do while not rs.eof
				if conn.execute("select count(id) from hr_person where userID="&rs("userID")&" and del=0")(0)>0 then
					errNum=errNum+1
'if conn.execute("select count(id) from hr_person where userID="&rs("userID")&" and del=0")(0)>0 then
				ElseIf conn.execute("select count(id) from hr_person where interviewID="&rs("interviewID")&" and interviewID>0 and del=0")(0)>0 Then
					errNum=errNum+1
				else
					sussNum=sussNum+1
					conn.execute("update hr_person set del=0 where id="&rs("id")&" and del<>0")
				end if
				rs.movenext
			loop
		end if
		...
		set rs=nothing
		if errNum>0 then
			errStr="失败"&errNum&"条，原因：档案已存在，不能恢复!"
		else
			errStr=""
		end if
		action1="员工档案恢复"
		call close_list(2)
		call jsLocat("恢复成功"&sussNum&"条!"&errStr&"","personFile.asp")
	elseif action="backOne" then
		errNum=0
		ord=deurl(trim(request("ord")))
		set rs=server.CreateObject("adodb.recordset")
		sql="select userID,id,isnull(interviewID,0) interviewID from hr_person  where del<>0 and id="&ord&" order by id desc"
		rs.open sql,conn,1,1
		if not rs.eof then
			if conn.execute("select count(id) from hr_person where userID="&rs("userID")&" and del=0")(0)>0 then
				errNum=errNum+1
'if conn.execute("select count(id) from hr_person where userID="&rs("userID")&" and del=0")(0)>0 then
			ElseIf conn.execute("select count(id) from hr_person where interviewID="&rs("interviewID")&" and interviewID>0 and del=0")(0)>0 Then
				...
				conn.execute("update hr_person set del=0 where del<>0 and id="&ord&"")
			end if
		else
			errNum=errNum+1
			conn.execute("update hr_person set del=0 where del<>0 and id="&ord&"")
		...
		set rs=nothing
		if errNum>0 then
			errStr="恢复失败，原因：档案已存在，不能恢复!"
		else
			errStr="恢复成功！"
		end if
		action1="员工档案恢复"
		call close_list(2)
		call jsLocat(""&errStr&"","personFile.asp")
	elseif action="backSome" then
		selectid=trim(request("selectid"))
		if selectid<>"" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select userID,id,isnull(interviewID,0) interviewID from hr_person  where del<>0 and id in("&selectid&") order by id desc"
			rs.open sql,conn,1,1
			errNum=0
			sussNum=0
			if not rs.eof then
...
'if conn.execute("select count(id) from hr_person where userID="&rs("userID")&" and del=0")(0)>0 then
			...
			call jsLocat("无选择人员！","personFile.asp")
		end if
	elseif action="delAll" Then
		conn.execute("create table #RelaList(id int,voucher int,account int)")
		Set rs = conn.execute("exec [erp_rela_finance_GetAssistIDs] "& session("personzbintel2007") &", 1,'4', 'select userid from hr_person  where del<>0' ,'"& Application("__saas__company") &"'")
		While rs.eof=False
			conn.execute("insert into #RelaList(id)values(" & rs("id").value & ") ")
			rs.movenext
		wend
		rs.close
		conn.execute("delete hr_person  where del<>0 and userid not in (select id from #RelaList)")
		action1="员工档案删除"
		call close_list(2)
		call jsLocat("删除成功！","personFile.asp")
	elseif action="delSome" then
		selectid=trim(request("selectid"))
		if selectid<>"" Then
			If conn.execute("exec [erp_rela_finance_GetAssistIDs] "& session("personzbintel2007") &", 1,'4', 'select userid from hr_person  where del<>0 and id in (" & selectid &")','"& Application("__saas__company") &"'").eof=False Then
				call close_list(2)
				call jsLocat("您选择了已被总账引用的单据，请重新选择!","personFile.asp")
			else
				conn.execute("delete hr_person  where del<>0 and id in("&selectid&")")
				action1="员工档案删除"
				call close_list(2)
				call jsLocat("删除成功!","personFile.asp")
			end if
	...
	elseif action="delOne" then
		ord=deurl(trim(request("ord")))
		If conn.execute("exec [erp_rela_finance_GetAssistIDs] "& session("personzbintel2007") &", 1,'4', 'select userid from hr_person  where del<>0 and id in (" & ord &") ','"& Application("__saas__company") &"'").eof=false Then
			call close_list(2)
			call jsLocat("已被总账引用，无法删除!","personFile.asp")
		end if
		if ord<>"" then
			conn.execute("delete hr_person  where del<>0 and id="&ord&"")
			action1="员工档案删除"
			call close_list(2)
			call jsLocat("删除成功!","personFile.asp")
		else
	...
	Response.redirect "personFile.asp"
