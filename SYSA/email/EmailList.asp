...
'if strW3<>"" Then
	...
	dim strSearchKeyName,strSearchKeyValue
	dim strSearchTag
	dim strSearchSQL
	dim strOrdList
	strSearchKeyName=trim(Request("SearchKeyName"))
	strSearchKeyValue=trim(Request("SearchKeyValue"))
	strOrdList=""
	if strSearchKeyName<>"" and strSearchKeyValue<>"" then
		strSearchSQL=" and " & strSearchKeyName & " like '%" & strSearchKeyValue & "%' "
		strSearchTag="<span style=""color:#b31ac4"">" & strSearchKeyValue & "</span>"
		Session("SSExportOrdList")=""
	end if
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_Person where del=1 " & strSearchSQL & Str_Result&" and (sort is null or sort=0 or sort in(select id from email_Person_class where (CHARINDEX(',"&session("personzbintel2007")&",',','+user_list+',')>0 or user_list='0,0' or user_list='0' or isnull(user_list,'')='') and (del=1 or del is null))) order by sort,ord desc "
	...
	if rs.eof=false then
		...
		if Request("page_count")="" or Request("page_count")=0 then
			...
			page_count=cdbl(Request("page_count"))
		end if
	...
	Response.write "" & vbcrlf & "<form name=""date"" id=""date"" method=""post"" action=""EmailList.asp"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"" >" & vbcrlf & "    <tr>" & vbcrlf & "      <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">邮件地址" & vbcrlf & "            <input name=""takeName"" type=""hidden"" value="""
	Response.write strid
	Response.write """ id=""takeName""></td>" & vbcrlf & "          <td colspan=""1""></td>" & vbcrlf & "          <td width=""2"" height=""32""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td  background=""../images/112.gif""  height=""30"" align=""right"" colspan=""4""><divalign=""right"" id=""kh"">" & vbcrlf & "          <select name=""SearchKeyName"" id=""SearchKeyName"" >检索:" & vbcrlf & "            <option value="""">全部</option>" & vbcrlf & "            <option value=""clictName"" "
	If strSearchKeyName = "clictName" Then
		Response.write " selected"
	end if
	Response.write ">姓名</option>" & vbcrlf & "            <option value=""clictEmail"" "
	If strSearchKeyName = "clictEmail" Then
		Response.write " selected"
	end if
	Response.write ">邮件</option>" & vbcrlf & "            <option value=""clictJob"" "
	If strSearchKeyName = "clictJob" Then
		Response.write " selected"
	end if
	Response.write ">职位</option>" & vbcrlf & "          </select>" & vbcrlf & "          <input type=""text"" name=""SearchKeyValue"" id=""SearchKeyValue""  value="""
	Response.write strSearchKeyValue
	Response.write """/>&nbsp;<input type=""button"" name=""btnSearch"" value=""检索"" class=""anybutton2"" style=""cursor:hand"" onclick=""submitSearch();"" />&nbsp;&nbsp;&nbsp;" & vbcrlf & "        <select name=""select"" class=""anybutton""  onChange=""window.location.href=this.value"">" & vbcrlf & "          <option selected>导入方式</option>" & vbcrlf & "          <option value=""../email_sort/loadEmailList.asp?upLoadUrl=../email/searchGate.asp"">员工通讯录</option>" & vbcrlf & "          <option value=""../email_sort/loadEmailList.asp?upLoadUrl=../email/searchTXL.asp"">常用通讯录</option>" & vbcrlf & "          <option value=""../email_sort/loadEmailList.asp?upLoadUrl=../load/UpLoad_Email.asp"">文本导入</option>" & vbcrlf & "          <option value=""../email_sort/loadEmailList.asp?upLoadUrl=../email/searchPerson.asp"">联系人</option>" & vbcrlf & "        </select>&nbsp;<input  name=""submit2242"" type=""submit"" class=""anybutton2"" style=""cursor:hand""   onClick=""submit5();"" value=""全部指定"">"
	if open_77_13=1  and open_77_19<>1 then
		Response.write "<input  name=""submit222"" IsShowProcessBar=""True"" type=""button"" class=""anybutton"" style=""cursor:hand""  onClick=""window.location.href='sendPersonAll.asp?takeName="
		Response.write strid
		Response.write "&page_count="
		Response.write page_count
		Response.write "&currPage="
		Response.write  currpage
		Response.write "&sendType=1'""  value=""群发所有"">"
	end if
	if open_77_10=1 or open_77_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""记录导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_emailAddress.asp?page_count="
		Response.write page_count
		Response.write "&currPage="
		Response.write  currpage
	...
	if open_77_3=3 or open_77_3=1  then
		Response.write "<input  name=submit2233 type=""button"" onClick=""if(!confirm('确认全部清除删除吗？将清除通讯录导航中所有邮箱！')){return false;} else{window.location.href='delAllPerson.asp?complete=1&takeName="
		Response.write strid
		Response.write "&page_count="
		Response.write page_count
		Response.write "&currPage="
	...
	Response.write "</div></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr height=""30"" class=""top"">" & vbcrlf & "        <td align=""center""><div align=""center"">选择 </div></td>" & vbcrlf & "        <td width=""12%"" ><div align=""center"">邮件地址</div></td>" & vbcrlf & "        <td height=""27"" ><div align=""center"">姓名</div></td>" & vbcrlf & "        <td ><div align=""center"">性别</div></td>" & vbcrlf & "        <td ><div align=""center"">分类</div></td>" & vbcrlf & "        <td ><div align=""center"">职位</div></td>" & vbcrlf & "        <td width=""11%"" ><div align=""center"">来源</div></td>" & vbcrlf & "        <td ><div align=""center"">销售人员</div></td>" & vbcrlf & "        <td ><div align=""center"">添加人员</div></td>" & vbcrlf & "        <td ><div align=""center"" >" & vbcrlf & "        <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf &"          <option>-请选择-</option>" & vbcrlf & "          <option value=""page_count=10"" "
	...
	if rs.RecordCount<=0 then
		Response.write "<tr><td  align='center' colspan='10'  class='gray' height='200'>地址库里没有邮箱，如要群发请先添加邮箱!</td></tr>"
	else
		if Session("SSExportOrdList")="" then
			do until rs.eof
				strOrdList=strOrdList & "," & rs("ord")
				rs.movenext
			loop
			if strOrdList<>"" Then strOrdList=right(strOrdList,len(strOrdList)-1)
'loop
			Session("SSExportOrdList")=strOrdList
			rs.MoveFirst
		end if
		do until rs.eof
			...
			if sortid<>"" then
				...
				sql2="select * from email_Person_class  where id="&sortid&" "
				rs2.open sql2,conn,1,1
				if not rs2.eof  then
					sortTitle=rs2("title")
					set rs3=server.CreateObject("adodb.recordset")
					sql3="select count(*) yeco from email_Person_class  where parent="&sortid&" "
			...
			if LCase(strSearchKeyName)="clictemail" then Response.write Replace(LCase(clictEmail),LCase(strSearchKeyValue),strSearchTag) else Response.write clictEmail end if
			Response.write "</div></td>" & vbcrlf & "            <td width=""12%"" height=""28""><div align=""center"">" & vbcrlf & "                "
			if isnull(person) or person="" then
				...
				Response.write "','emailres','width=' + 1000 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return true;"" "
				'Response.write pwurl(person)
				if len(name)>4 then
			...
			if LCase(strSearchKeyName)="clictname" then Response.write Replace(LCase(name),LCase(strSearchKeyValue),strSearchTag) else Response.write name end if
			Response.write "</a></div></td>" & vbcrlf & "            <td width=""6%"" height=""28""><div align=""center"">"
			Response.write sex
			...
			if LCase(strSearchKeyName)="clictjob" then Response.write Replace(LCase(Job),LCase(strSearchKeyValue),strSearchTag) else Response.write Job end if
			Response.write "</div></td>" & vbcrlf & "" & vbcrlf & "            <td>" & vbcrlf & "              <div align=""left"">" & vbcrlf & "              "
			if rs("cateid")<>"" then
				set rsSellerName=server.CreateObject("adodb.recordset")
				sqlGate="select * from gate where ord="&rs("cateid")&""
				rsSellerName.open sqlGate,conn,1,1
				dim cateid
				if rsSellerName.EOF then
					cateid="-"
'if rsSellerName.EOF then
				else
					cateid=rsSellerName("name")
					seller=rsSellerName("ord")
				end if
				rsSellerName.close
				set rsSellerName=nothing
			end if
			if sort3=1 then
				if name2="无关联客户" then
					Response.write "" & vbcrlf & "                    无关联客户" & vbcrlf & "                "
				else
					sqlQx_intro="select qx_open,qx_intro FROM power where sort1=1 and sort2=14 and ord="&session("personzbintel2007") &""
					flag=IsPermission(sqlQx_intro,seller)
					if flag then
				...
				if name2="无关联供应商" then
					Response.write "" & vbcrlf & "                无关联供应商" & vbcrlf & "                "
				else
					sqlQx_intro2="select qx_open,qx_intro FROM power where sort1=26 and sort2=14 and ord="&session("personzbintel2007") &""
					flag2=IsPermission(sqlQx_intro2,seller)
					if flag2 then
			...
			Response.write "" & vbcrlf & "              </div></td>" & vbcrlf & "" & vbcrlf & ""
			if addcate<>"" then
				set rsAdder=server.CreateObject("adodb.recordset")
				sqlAdder="select * from gate where ord="&addcate&""
				rsAdder.open sqlAdder,conn,1,1
				if rsAdder.EOF then
					addcate="-"
'if rsAdder.EOF then
				else
					addcate=rsAdder("name")
				end if
				rsAdder.close
				set rsAdder=nothing
			end if
			Response.write "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "            <td width=""9%"" ><div align=""center"">"
			Response.write cateid
			Response.write "</div></td>" & vbcrlf & "            <td width=""9%"" ><div align=""center"">"
			Response.write addcate
			Response.write "</div></td>" & vbcrlf & "            <td width=""18%"" class=""func""><div align=""center"">"
			if open_77_13=1 and open_77_19<>1  then
				Response.write "<input  name=submit2 type=""button""   onClick=""javascript:window.open('../email/index.asp?email="
				Response.write trim(clictEmail)
				Response.write "','newsSendemail','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=yes,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=200,top=100')""    value=""单发"">"
				Response.write trim(clictEmail)
			end if
			Response.write "" & vbcrlf & "                                             <input type=""button"" name=""Submit3c"" value=""修改""  onClick=""javascript:window.open('editPerson.asp?ord="
			...
			if open_77_3=3 or CheckPurview(intro_77_3,trim(addcate))=True then
				Response.write "" & vbcrlf & "                                             <input  name=submit2 type=""button""   onClick=""if(!confirm('确认删除吗？')){return false;} else{window.location.href='delPerson.asp?ord="
				Response.write pwurl(ord)
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "page_count="
			...
			rs.movenext
		loop
		Response.write "" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td height=""100"" align=""left"" valign=""center"" class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "      <tr>" & vbcrlf & "      <td width=""8%"" height=""30""><input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "      <label>全选</label></td>" & vbcrlf & "    <td width=""30%"" align=""left"">"
		if open_77_13=1 and open_77_19<>1  then
			Response.write "" & vbcrlf & "      <input  name=""submit22"" type=""submit"" class=""anybutton"" style=""cursor:hand""   onClick=""submit3();"" value=""群发所选"">" & vbcrlf & "      "
		end if
		if open_77_3=3 or open_77_3=1  then
			Response.write "" & vbcrlf & "      <input  name=submit22332 type=""submit"" onClick=""submit1();"" style=""cursor:hand"" class=""anybutton""  value=""批量删除"">" & vbcrlf & "      "
		end if
		...
		Response.write "" & vbcrlf & "      </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td  colspan=""7"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf& ""
	end if
	rs.close
	set rs=nothing
	action1="邮件地址"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "    function test() {" & vbcrlf & "        if (!confirm('确认删除吗？')) return false;" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function mm(form) {" & vbcrlf & "        for (var i = 0; i < form.elements.length; i++) {" & vbcrlf & "    var e = form.elements[i];" & vbcrlf & "            if (e.name != 'chkall')" & vbcrlf & "                e.checked = form.chkall.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "</script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "<!--" & vbcrlf & "    function submit1() {" & vbcrlf & "        document.all.date.action = ""delAllPerson.asp?complete=1&type=1&takeName="
	Response.write strid
	Response.write "&page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currpage
	Response.write """;" & vbcrlf & "    }" & vbcrlf & "    function submit3() {" & vbcrlf & "        document.all.date.action = ""sendPersonAll.asp?type=1&qf=1&sendType=0&currPage="
	Response.write currpage
	Response.write """;" & vbcrlf & "    }" & vbcrlf & "    function submit4() {" & vbcrlf & "        document.all.date.action = ""SetPersonType.asp?type=1&takeName="
	Response.write strid
	Response.write "&page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currpage
	Response.write """;" & vbcrlf & "    }" & vbcrlf & "    function submit5() {" & vbcrlf & "        document.all.date.action = ""SetPersonType.asp?type=2&takeName="
	Response.write strid
	Response.write "&page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currpage
	Response.write """;" & vbcrlf & "    }" & vbcrlf & "    function shareAll() {" & vbcrlf & "        document.all.date.action = ""share_person_EmailList.asp?type=1&takeName="
	Response.write strid
	Response.write "&page_count="
	Response.write page_count
	Response.write "&currPage="
	Response.write  currpage
	Response.write """;" & vbcrlf & "    }" & vbcrlf & "    function submitSearch() {" & vbcrlf & "        var objSearchKeyValue = document.getElementById(""SearchKeyValue"");" & vbcrlf & "        var strSearchKeyName = document.getElementById(""SearchKeyName"").value;" & vbcrlf & "        var strSearchKeyValue =document.getElementById(""SearchKeyValue"").value;" & vbcrlf & "        strSearchKeyValue = strSearchKeyValue.replace(""%"", """");" & vbcrlf & "        strSearchKeyValue = strSearchKeyValue.replace(""'"", """");" & vbcrlf & "        strSearchKeyValue.replace(/\s/g, """");" & vbcrlf & "             /*" & vbcrlf & "        if ((strSearchKeyValue.length == 0) && (strSearchKeyName.length != 0)) { " & vbcrlf & "                     alert(""请输入检索关键字!\n\n注意要与检索项对应!""); " & vbcrlf & "                       objSearchKeyValue.focus(); " & vbcrlf & "                     objSearchKeyValue.select();" & vbcrlf & "                     return false; " & vbcrlf & "          };*/" & vbcrlf & "       window.location.href = ""EmailList.asp?time="
	Response.write now()
	Response.write "&SearchKeyName="" + strSearchKeyName + ""&SearchKeyValue="" + strSearchKeyValue;" & vbcrlf & "    }" & vbcrlf & "-->" & vbcrlf & "</script>" & vbcrlf & ""
	'Response.write now()
