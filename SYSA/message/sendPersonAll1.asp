...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>短信群发</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Server.ScriptTimeOut=100000000
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script language=""javascript"" src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript""></script>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "var getx;" & vbcrlf & "function sendSubmit(phone,person,openSir)" & vbcrlf & "{" & vbcrlf & "getMessage(""../message/sendMessage.asp?phone=""+phone+""&person=""+person+""&openSir=""+openSir+"""",600,400)" & vbcrlf & "}" & vbcrlf & "function testfun(x1)" & vbcrlf & "{" & vbcrlf & "getx=x1" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "<script src=""../inc/loadingBar.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body >" & vbcrlf & "<table width=""100%""  border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <form id=""date"" name=""date"" method=""post"" action=""topadd.asp?qf=1"" target=""_parent"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">群发短信</td>" & vbcrlf & "            <td></td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td align=""center""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" >" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""19%"" align=""right"" valign=""middle"">状态：</td>" & vbcrlf & "                  <td width=""81%"" align=""left"">"
	...
	q=request.QueryString("complete")
	...
	if typeid=1 then
		...
		if selectid="" then
			Response.write"<script language=javascript>alert('您没有选择任何联系人，请选择后再重试！');history.back()</script>"
			call db_close : Response.end
		else
			str_sql_checkid=" and ord in ("&selectid&") "
		end if
	end if
	...
	elseif open_67_1= 1 then
		Str_Result=" and (cateid in("&intro_67_1&") or addcate in("&intro_67_1&")) "
	else
		Str_Result=" and 1=2"
	end if
	if strid<>"" then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from MessagePerson where 1=1 "&Str_Result&" "&str_sql_checkid&" order by ord desc"
	...
	strPerson=""
	do until rs.eof
		personMobile=trim(rs("clictPhone"))
		if personMobile<>"" then
			if RegTest(personMobile,"^(13[0-9]|14[0-9]|15[^4]|17[0-9]|18[0-9])\d{8}$")=false then
'if personMobile<>"" then
				Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的手机号码("&personMobile&")格式不符，不能发送</font><br/>")
				errNum=errNum+1
				Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的手机号码("&personMobile&")格式不符，不能发送</font><br/>")
			else
				strPhone=strPhone&","&personMobile
			end if
		else
			if RegTest(personPhone,"^(13[0-9]|14[0-9]|15[^4]|17[0-9]|18[0-9])\d{8}$")=false then
				strPhone=strPhone&","&personMobile
				Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的手机号码为空，不能发送</font><br/>")
				errNum=errNum+1
				Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的手机号码为空，不能发送</font><br/>")
			else
				Response.write ("第【"&i&"】条:“"&personName&"”的手机为空，但电话栏目中有手机号码，短信将发送到"&personPhone&"<br/>")
				strPhone=strPhone&","&personPhone
			end if
		end if
		rs.movenext
		i=i+1
		strPhone=strPhone&","&personPhone
	loop
	Response.write ("<br/><font  color=#ff0000>合计【"&C1&"】条：其中【"&errNum&"】条号码有误，此次将发送【"&C1-errNum&"】条</font>")
'loop
	rs.close
	set rs=Nothing
	conn.close
	set conn=nothing
	Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "              <!--  <tr>" & vbcrlf & "                  <td align=""right"">启用尊称：</td>" & vbcrlf & "                  <td align=""left""><input name=""personPre"" type=""radio"" id=""personPre"" onClick=""testfun(0)"" value=""0"" checked=""checked"" />" & vbcrlf & "                    不启用" & vbcrlf & "                    <input name=""personPre"" type=""radio"" id=""personPre"" onClick=""testfun(1)""  value=""1"" />" & vbcrlf & "                    按性别" & vbcrlf & "                    <input name=""personPre"" type=""radio"" id=""personPre"" onClick=""testfun(2)""  value=""2"" />" & vbcrlf & "                    按职位" & vbcrlf & "                    <input name=""personPre"" type=""radio"" id=""personPre"" onClick=""testfun(3)""  value=""3"" />" & vbcrlf & "                    按姓名" & vbcrlf & "                    <input name=""personPre"" type=""radio"" id=""radio"" onClick=""testfun(4)""  value=""4"" />" & vbcrlf & "按姓名+职位</td>" & vbcrlf & "                </tr>-->" & vbcrlf & "     <!--           <tr>" & vbcrlf & "                  <td align=""right"">选择通道：</td>" & vbcrlf & "                  <td align=""left""><input name=""sendType"" type=""radio"" id=""personPre"" value=""1"" checked=""checked"" />" & vbcrlf & "                    祝福、通知通道(1)" & vbcrlf & "                    <input name=""sendType"" type=""radio"" id=""personPre""  value=""2"" />" & vbcrlf & "                    营销通道(2)" & vbcrlf & "              <input name=""sendType"" type=""radio"" id=""personPre""   value=""3"" />" & vbcrlf & "                    备用通道(3)<span class=""redFont"">*为了不影响效率，如营销类群发请选择通道2</span></td>" & vbcrlf & "                </tr>-->" & vbcrlf & "                <tr>" & vbcrlf & "                  <script type=""text/javascript"" language=""javascript"">" & vbcrlf & "                                                 <!--" & vbcrlf & "//                                                  function subSend(getx)" & vbcrlf & "//                                                        {" & vbcrlf & "//                                                             if (getx=2)" & vbcrlf & "//                                                           {" & vbcrlf & "//                                                             sendSubmit("""
	set conn=nothing
	Response.write strPhone
	Response.write ""","""
	Response.write strPerson2
	Response.write """,2);" & vbcrlf & "//                                                                    }" & vbcrlf & "//                                                             else if(getx=1)" & vbcrlf & "//                                                               {" & vbcrlf & "//                                                             sendSubmit("""
	Response.write strPhone
	Response.write ""","""
	Response.write strPerson1
	Response.write """,1);" & vbcrlf & "//                                                                            }" & vbcrlf & "//                                                             else" & vbcrlf & "//                                                          {" & vbcrlf & "//                                                             sendSubmit("""
	Response.write strPhone
	Response.write ""","""",0);" & vbcrlf & "//                                                                          }" & vbcrlf & "//                                             }" & vbcrlf & "                                                       -->" & vbcrlf & "                                                     </script>" & vbcrlf & "                  <td align=""right""></td>" & vbcrlf & "                  <td align=""left""><input type=""hidden"" value="""
	Response.write strPhone
	Response.write strPhone
	Response.write """ name=""phone"" />" & vbcrlf & "        <!--            <input type=""hidden"" value="""
	Response.write strPhone
	Response.write strPerson
	Response.write """ id=""person3"" name=""person3"" />" & vbcrlf & "                    <input type=""hidden"" value="""
	Response.write strPerson1
	Response.write """ id=""person1"" name=""person1"" />" & vbcrlf & "                    <input type=""hidden"" value="""
	Response.write strPerson2
	Response.write """ id=""person2""  name=""person2"" />" & vbcrlf & "                                                                              <input type=""hidden"" value="""
	Response.write strPerson3
	Response.write """ id=""person4"" name=""person4"" />-->" & vbcrlf & "                    <input type=""submit"" "
	Response.write strPerson3
	if (C1-errNum)>0 then
		Response.write strPerson3
		Response.write "IsShowProcessBar=""True"""
	end if
	...
	Response.write " class=""anybutton"" id=""button""  value=""提交到发送队列"" name=""B8"" ></td>" & vbcrlf & "                </tr>" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "          <tr>" & vbcrlf & "            <td height=""30"" >&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </form>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>"
