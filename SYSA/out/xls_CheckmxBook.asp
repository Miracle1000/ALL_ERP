...
'if L4<>"" then
	...
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',che_time)>=0 "
'if ret<>"" then
	end if
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,che_time,'"&ret2&" 23:59:59')>=0 "
'if ret2<>"" then
	end if
	if C<>"" then
		if B="name" then
	...
	i=1
	sql="select a.*,b.chk_addcateid,b.chk_addtime,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing from O_ChkBookmx a "&_
	"left join O_ChkBook b on a.Chk_fid=b.id "&_
	"left join O_RegBook c on a.Chk_bkid=c.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "盘点明细表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
...
'if not rs.eof then
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>盘点明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!盘点明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("盘点明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的盘点明细表</u></strong></font></a></p>"
