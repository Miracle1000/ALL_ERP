...
'if Exit Sub2<>"" then
	...
	i=1
	sql="select a.id as Ret_id,a.Ret_fid,a.Ret_bkid,a.Ret_num,a.Ret_time,a.Ret_isBreak,a.Ret_note,b.addcateid,b.addtime, "&_
	"e.set_name,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing,d.Ld_time,d.Ld_num,d.Ld_rettime,f.name from O_RetBookmx a "&_
	"left join O_RetBook b on a.Ret_fid=b.id "&_
	"left join O_Lendbookmx d on a.Ret_bkid=d.id "&_
	"left join gate f on b.addcateid=f.ord "&_
	"left join O_RegBook c on d.Ld_bkid=c.id "&_
	"left join O_bookset e on c.bk_type=e.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "归还列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			Ret_time=rs("Ret_time")
			addtime=rs("addtime")
			if rs("Ret_isBreak")=1 then
				Ret_isBreak="否"
			else
				Ret_isBreak="是"
			end if
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>归还列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!归还列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("归还列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的归还列表</u></strong></font></a></p>"
