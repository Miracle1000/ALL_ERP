...
'If Not rs.Eof Then
	...
	Dim mbID2
	mbID2 = mbID
	IF act="openuCopy" Then
		mbID2 = 0
	end if
	If conn.execute("select top 1 id from ProcModels where name='"&title&"' and id <>'"&mbID2&"'").eof=False Then
		Response.write("<script>alert('模板名称不能重复');</script>")
		conn.close
		Response.end()
	end if
	If conn.execute("select top 1 id from ProcModels where type='1' and id <>'"&mbID2&"'").eof=False and type1=1 Then
		Response.write("<script>alert('主模板已存在');</script>")
		conn.close
		Response.end()
	end if
	conn.CursorLocation = 3
	...
	IF act = "add" Then
		Set rs9 = server.CreateObject("adodb.recordset")
		sql9 = "select * from ProcModels where name='"&title&"'"
		...
		If Not rs9.EOF Then
			...
			rs9("Flag") = Flag
			rs9("gate1") = gate1
			rs9("addcate") = session("personzbintel2007")
			rs9("date7") = now
			rs9.update
			mobanid=rs9("id")
		end if
		...
		For k = 0 To UBound(jhIU)
			jdtype=Request.form("jdtype"&jhIU(k))
			execorder=CLng("0" & Request.form("execorder"&jhIU(k)))
			...
			intro=Request.form("intro"&jhIU(k))
			mustat=Request.form("mustat"&jhIU(k))
			allOKModel=Request.form("allOKModel"&jhIU(k))
			...
			splinktype=Request.form("splinktype"&jhIU(k))
			nextid=Request.form("nextid"&jhIU(k))
			sort1=Request.form("sort1"&jhIU(k))
			id1=Request.form("id1"&jhIU(k))
			IF jdtype <> "" Then
				...
				If Not rs1.EOF Then
					...
					rs1("actors") = actors
					rs1("intro") = intro
					rs1("mustat") = CLng("0" & mustat)
					rs1("allOKModel") = CLng("0" & allOKModel)
					rs1("commFields") = commFields
					rs1("linkFields") = linkFields
					rs1("zdyFields") = zdyFields
					rs1("splinktype") = CLng("0" & splinktype)
					rs1.update
					PMID=rs1("id")
				end if
				rs1.Close
		...
		next
		conn.execute("update b set b.nextid=id from ProcNextNodes b inner join ProcModelsNodes a on b.nextid=a.sortid and a.chancePMid='"&mobanid&"'")
	end if
	IF act="update" Then
		mobanid=mbID
		IF jh="" Then
		...
		deletd="delete from ProcModelsNodes where sortid not in ("&jh&") and chancePMid='"&mobanid&"'"& Chr(13) & Chr(10)
		conn.execute(deletd&"update ProcModels set name='"&title&"',type='"&type1&"',Flag='"&Flag&"',gate1='"&gate1&"' where  id='"&mobanid&"'")
		For k = 0 To UBound(jhIU)
			b=""
			t=""
			jdtype=Request.form("jdtype"&jhIU(k))
			execorder=Request.form("execorder"&jhIU(k))
			timeproject=Request.form("timeproject"&jhIU(k))
			budgetmoney=Request.form("budgetmoney"&jhIU(k))
			executors=Request.form("executors"&jhIU(k))
			actors=Request.form("actors"&jhIU(k))
			...
			sort1=Request.form("sort1"&jhIU(k))
			id1=Request.form("id1"&jhIU(k))
			ids=Request.form("ids"&jhIU(k))
			IF timeproject <> "" Then
				t="'"&timeproject&"',"
				timeproject = "timeproject='"&timeproject&"',"
			else
				t=0&","
			end if
			IF budgetmoney <> "" Then
				b=budgetmoney&","
				budgetmoney = "budgetmoney="&budgetmoney&","
			else
				b=0&","
				budgetmoney="budgetmoney=0,"
			end if
			If conn.execute("select id  from ProcModelsNodes where sortid='"&jhIU(k)&"' and chancePMid=" & mobanid).eof=False Then
				IF jdtype <> "" Then
					sqls="update ProcModelsNodes set name='"&sort1&"', chancePMid='"&mobanid&"', sortid='"&jhIU(k)&"', sortid1='"&id1&"', jdtype='"&jdtype&"', execorder='"&execorder&"', "&timeproject&budgetmoney&" executors='"&executors&"', actors='"&actors&"', intro='"&intro&"', mustat='"&mustat&"', allOKModel='"&allOKModel&"', commFields='"&commFields&"', linkFields='"&linkFields&"', zdyFields='"&zdyFields&"', splinktype='"&splinktype&"' where sortid='"&jhIU(k)&"' and chancePMid=" & mobanid
					conn.execute(sqls)
				end if
			else
				IF jdtype <> "" Then
					sqlins="INSERT INTO [ProcModelsNodes]([name], [chancePMid], [sortid], [sortid1], [addcate], [date7], [jdtype], [execorder], [timeproject], [budgetmoney], [executors], [actors], [intro], [mustat], [allOKModel], [commFields], [linkFields], [zdyFields], [splinktype])VALUES('"&sort1&"', '"&mobanid&"', '"&jhIU(k)&"', '"&id1&"', '"&session("personzbintel2007")&"', getdate(), '"&jdtype&"', '"&execorder&"', "&t&b&" '"&executors&"', '"&actors&"','"&intro&"', '"&mustat&"', '"&allOKModel&"', '"&commFields&"', '"&linkFields&"','"&zdyFields&"', '"&splinktype&"')"& Chr(13) & Chr(10)
					conn.execute(sqlins)
					Set Rss = conn.Execute("SELECT top 1 id FROM ProcModelsNodes order by id desc")
					ids = Rss("id")
					Rss.close
				end if
			end if
			conn.execute("delete from ProcNextNodes where nodeid='"&ids&"'")
			IF jdtype <> "" and nextid <> "" Then
				ntsIU = Split(nextid, ",")
				For s = 0 To UBound(ntsIU)
					Set rs2 = server.CreateObject("adodb.recordset")
					sql2 = "select * from ProcNextNodes where nodeid='-320'"
	...
	if err.number<>0 then
		conn.rollbacktrans
		Response.write("<script>alert('未知错误，请返回重试');</script>")
		Response.end()
	else
		conn.CommitTrans
	end if
	call db_close
	If request("addType") = 2 Then
		Response.write("<script>alert('操作成功!');top.location.href='ProcModelsAdd.asp?act=add'</script>")
	else
		Response.write("<script>alert('操作成功!');top.close();</script>")
	end if
	Response.end()
