...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">联系人分组设置</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> " & vbcrlf & " <tr border=1 valign=""center"" class=""top"" style=""line-height:64px"">" & vbcrlf & " <td>" & vbcrlf & "   <div align=""left""><a href=""javascript:void(0)"" onClick=""javascript:window.open('add_cp.asp','newwinadd','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">添加根目录</a></div></td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	...
	function menu(id)
		...
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from MessagePersonClass where parent="&id&" "&powersql&" order by sort desc,id asc"
		...
		while not rs.eof
			...
			Response.write "','newwinadd','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""><font  color=""#909090"">添加 </font></a>" & vbcrlf & "    "
			Response.write  rs("depth")
			if session("top1zbintel2007")="1" or addcate=session("personzbintel2007") or addcate="" then
				Response.write " " & vbcrlf & "     | <a href=""javascript:void(0)"" onClick=""javascript:window.open('correct_cp.asp?rd="
				Response.write  rs("id")
				Response.write "','newwincorrect','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""><font color=""#909090"">修改</font></a>" & vbcrlf & "   | <a href=""menu.asp?action=del&id="
				Response.write  rs("id")
				Response.write  rs("id")
				Response.write """ onclick=""return confirm('建议不要轻易删除设置好的参数，以免影响到其他关联的数据。确认删除?')""><font  color=""#909090"">删除</font></a>" & vbcrlf & "" & vbcrlf & " "
			end if
			if rs("del")=2 then
				Response.write " " & vbcrlf & "      |<a href=""menu.asp?action=open&id="
				Response.write  rs("id")
				Response.write """ onclick=""return confirm('确认开启?')""><font  color=""#ff0000""> 开启</font></a> " & vbcrlf & "        "
			end if
			if isnull(rs("del")) or rs("del")<>2 then
				Response.write "" & vbcrlf & "       |<a href=""menu.asp?action=close&id="
				Response.write  rs("id")
				Response.write """ onclick=""return confirm('确认关闭?')""><font  color=""#909090""> 关闭</font></a>" & vbcrlf & "        "
			end if
	...
	function delmenu(id)
		set rs=conn.execute("select id from MessagePersonClass where parent="&id)
		if not rs.eof then
			Response.write("<script>alert('有下级分类，不可删除！');history.back()</script>")
			call db_close : Response.end()
		end if
		sql7="select * from MessagePerson where sort="&id
		set rs7=conn.Execute(sql7)
		if not rs7.eof then
			Response.write("<script>alert('此联系人组分类已经在联系人组中使用，不可删除！');history.back()</script>")
			call db_close : Response.end()
		else
			conn.execute("delete from MessagePersonClass where id="&id)
		end if
		set rs7=nothing
	end function
	function closemenu(id)
		conn.execute("update  MessagePersonClass set del=2 where id="&id)
	end function
	function openmenu(id)
		conn.execute("update  MessagePersonClass set del=1 where id="&id)
	end function
	function savemenu()
		...
		select case action
		case "edit"
		ChildCount=conn.execute("select count(*) from MessagePersonClass where parent="&id)(0)
		if ChildCount>0 then url=""
		conn.execute("update MessagePersonClass set parent="&id1&",title='"&menuname&"',sort="&gate1&" where id="&id)
		case "add"
		conn.execute("insert into MessagePersonClass (parent,title,sort) values ("&id1&",'"&menuname&"',"&gate1&")")
		end select
	end function
	function addmenu()
	...
	function menuform(id,id1,action)
		if action="edit" then
			set rs=conn.execute("select * from MessagePersonClass where id="&id)
			menuname=rs("title")
			gate1=rs("sort")
			rs.close
			set rs=nothing
