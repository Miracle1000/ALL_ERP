...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	j=23
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,name,sort,gl from zdy where sort1=803 and set_open=1 and dc=1 order by gate1 asc "
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
...
'do until rs1.eof
	...
	rs1.close
	set rs1=nothing
	xlWorksheet.Cells(1,j+1).Value = "车牌号"
	set rs1=nothing
	xlWorksheet.Cells(1,j+1).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+1).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+2).Value = "司机"
	set rs1=nothing
	xlWorksheet.Cells(1,j+2).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+2).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+3).Value = "用车性质"
	set rs1=nothing
	xlWorksheet.Cells(1,j+3).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+3).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+4).Value = "派车时间"
	set rs1=nothing
	xlWorksheet.Cells(1,j+4).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+4).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+5).Value = "开始时间"
	set rs1=nothing
	xlWorksheet.Cells(1,j+5).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+5).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+6).Value = "结束时间"
	set rs1=nothing
	xlWorksheet.Cells(1,j+6).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+6).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+7).Value = "目的地"
	set rs1=nothing
	xlWorksheet.Cells(1,j+7).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+7).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+8).Value = "里程"
	set rs1=nothing
	xlWorksheet.Cells(1,j+8).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+8).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+9).Value = "事由"
	set rs1=nothing
	xlWorksheet.Cells(1,j+9).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+9).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+10).Value = "明细备注"
	set rs1=nothing
	xlWorksheet.Cells(1,j+10).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+10).font.bold=true
	set rs1=nothing
	xlWorksheet.Cells(1,j+11).Value = "返还状态"
	set rs1=nothing
	xlWorksheet.Cells(1,j+11).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(1,j+11).font.bold=true
	set rs1=nothing
	...
	set rs1=nothing
	Str_Result=""
	if open_102_10=3 then
		list=""
	elseif open_102_10=1 then
		list=" and use_cateid in ("&intro_102_10&")"
	else
	...
	if open_102_1=3 then
	elseif open_102_1=1 then
		Str_Result=Str_Result&" and use_cateid in ("&intro_102_1&")"
	else
	...
	Dim remind : remind = request("remind")
	If remind <> "" Then
		Dim helper : Set helper = CreateReminderHelper(conn,remind,0)
		Str_Result = Replace(Str_Result,"del=1","1=1")
		Str_Result= Str_Result & " and use_id in (" & helper.listSQL("ids_withoutOrderBy") & ")"
	end if
...
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	i=1
	k=0
	sql="select * from O_carUse "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	...
	if not rs.eof then
		do until rs.eof
			...
			use_cateid_spid=rs("use_cateid_sp")
			if use_cateid_spid="" or isnull(use_cateid_spid) then use_cateid_spid=0
			sql8="select * from gate where ord="&use_cateid_spid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
			...
			rs8.close
			set rs8=nothing
			use_addcateid=rs("use_addcateid")
			...
			j=7
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select id,name,sort,gl from zdy where sort1=804 and set_open=1 and dc=1 order by gate1 asc "
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				do until rs1.eof
					j=j+1
'do until rs1.eof
					if rs1("sort")=1 and rs("use_"&rs1("name")&"")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select sort1 from sortonehy where ord="&rs("use_"&rs1("name")&"")&""
			...
			rs1.close
			set rs1=nothing
			sql8="select a.*,d.car_code,e.drivername from O_carUsemx a"&_
			" left join O_carData d on a.use_carid=d.car_id "&_
			" left join O_carDriver e on a.use_driver=e.id where a.use_fid="&rs("use_id")
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				do while not rs8.eof
					if rs8("use_catetype")=1 then
						use_catetype="公用"
					else
						use_catetype="私用"
					end if
					use_pctime=rs8("use_pctime")
					use_startime=rs8("use_startime")
					use_endtime=rs8("use_endtime")
					if rs8("use_complete")=1 then
						mx_complete="未返还"
					else
						mx_complete="已返还"
					end if
					...
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+1).Value =rs8("car_Code")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+1).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+1).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+2).Value =rs8("drivername")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+2).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+2).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+3).Value =use_catetype
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+3).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+3).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+4).Value =use_pctime
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+4).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+4).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+5).Value =use_startime
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+5).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+5).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+6).Value =use_endtime
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+6).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+6).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+7).Value =rs8("use_mdd")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+7).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+7).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+8).Value =rs8("use_lc")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+8).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+8).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+9).Value =rs8("use_thing")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+9).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+9).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+10).Value =rs8("use_notes")
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+10).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+10).HorizontalAlignment=1
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+11).Value =mx_complete
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+11).font.Size=10
					mx_complete="已返还"
					xlWorksheet.Cells(2+i,j+11).HorizontalAlignment=1
					mx_complete="已返还"
					i=i+1
					mx_complete="已返还"
					rs8.movenext
