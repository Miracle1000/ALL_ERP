...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,1).Value = "通话时间"
	xlWorksheet.Cells(1,1).font.Size=10
	xlWorksheet.Cells(1,1).font.bold=true
	xlWorksheet.Cells(1,2).Value = "通话号码"
...
'if m1<>"" then
	...
	if m2<>"" then
		Str_Result=Str_Result+"and adddate<='"&cdate(m2)&"' "
'if m2<>"" then
	end if
...
'if A1<>"" then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from call "&str_Result&" order by adddate desc "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "电话统计汇总表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof
		k=rs("phone")
		...
		xlWorksheet.Cells(1+i,1).Value = adddate
		cateid=""
		...
		cateid=""
		xlWorksheet.Cells(1+i,4).Value = company
		cateid=""
		xlWorksheet.Cells(1+i,4).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,5).Value = person
		cateid=""
		xlWorksheet.Cells(1+i,5).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,6).Value = talktime
		cateid=""
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>电话统计汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!电话统计汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("电话统计汇总表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的电话统计汇总表</u></strong></font></a></p> " & vbcrlf & ""
