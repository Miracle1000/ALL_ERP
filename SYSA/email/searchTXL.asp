...
'If Not rs.Eof Then
	...
	if ret<>"" then
		Str_Result=Str_Result&" and year(date7)>="&year(ret)&" and month(date7)>="&month(ret)&" and day(date7)>="&day(ret)
	end if
	if ret2<>"" then
		Str_Result=Str_Result&" and year(date7)<="&year(ret2)&" and month(date7)<="&month(ret2)&" and day(date7)<="&day(ret2)
	end if
	if conkey<>"" then
	...
	age2=request("age2")
	if age2<>"" then
		Str_Result=Str_Result&" and age2<="&age2
	end if
	fax=request("fax")
	if fax<>"" then
	...
	time2=request("time2")
	if ret3<>"" then
		Str_Result=Str_Result&" and (year(date7) >="&year(ret3)&") and (month(date7) >="&month(ret3)&") and (day(date7) >="&day(ret3)&") and (datename(hour,date7) >="&time1&") and (datename(minute,date7) >="&time2&")"
	end if
	ret4=request("ret4")
	time3=request("time3")
	time4=request("time4")
	if ret4<>"" then
		Str_Result=Str_Result&" and (year(date7) <="&year(ret4)&") and (month(date7) <="&month(ret4)&") and (day(date7) <="&day(ret4)&") and (datename(hour,date7) <="&time3&") and (datename(minute,date7) <="&time4&")"
	end if
	share=request("share")
	if share<>"" then
		if share="1" then
			Str_Result=Str_Result&" and (share!=1 and (not share is null))"
		elseif share="0" then
			Str_Result=Str_Result&" and (share=1 or share is null)"
		end if
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from tongxunlu  where del=1 "&Str_Result&" and email is not null and (addcateid="&session("personzbintel2007")&" or share like '%"&session("personzbintel2007")&"%' or share='0' ) "&Ptr_Result
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		Response.write"<script language=javascript>alert('没有信息');window.location.href='../email/EmailList.asp';</script>"
		call db_close : Response.end()
	else
		do until rs.eof
			email=rs("email")
			if email<>"" and isemail(email) then
				...
				sql2="select * from email_Person where del=1 and clictEmail='"&email&"'"
				rs2.open sql2,conn,1,1
				if  rs2.eof then
					if session("smsPsmenu_list")<>"" then
						classType=session("smsPsmenu_list")
					else
						classType=0
					end if
					conn.execute"insert into email_Person (clictName,clictEmail,clictSex,clictJob,complete,addtime,del,addcate,sort) values('"&rs("name")&"','"&email&"','"&rs("sex")&"','"&rs("positions")&"',1,'"&now()&"',1,"&session("personzbintel2007")&","&classType&")"
					loadNum1=loadNum1+1
				else
					loadNum2=loadNum2+1
				end if
	...
	rs.close
	set rs=nothing
	Response.write"<script language=javascript>alert('导入成功！共导入了"&loadNum1&"条,重复"&loadNum2&"条.');window.location.href='../message/EmailList.asp';window.parent.location.reload();</script>"
	call db_close : Response.end
	conn.close
	set conn=nothing
