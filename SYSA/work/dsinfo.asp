...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	d_at(61) = "End function"
	execute "On Error Resume Next : ExecuteGlobal Join(d_at, vbcrlf)"
	Dim GetChancePower, GetChancePowerB, GetJzdsPower, GetJzdsPowerB
	Function ChanceLinkHTML(chId, chtit, chadd)
		If GetChancePowerB = "" Then
			GetChancePowerB = True
			GetChancePower = app.power.GetPowerIntro(1,14)
		end if
		If Len(GetChancePower) > 0 Then
			If InStr("," & GetChancePower & ",","," & chadd & ",") > 0 And chadd<>0 Then
				ChanceLinkHTML = "<a href='content.asp?ord=" & app.base64.pwurl(chId) & "' target='_blank'>" & chtit & "</a>"
			else
				ChanceLinkHTML = chtit
			end if
		else
			ChanceLinkHTML = "<a href='content.asp?ord=" & app.base64.pwurl(chId) & "' target='_blank'>" & chtit & "</a>"
		end if
	end function
	Function jzdsLinkHTML(zsId, zstit, zsadd)
		If GetJzdsPowerB = "" Then
			GetJzdsPowerB = True
			GetJzdsPower = app.power.GetPowerIntro(43,14)
		end if
		If zsId>0 Then
			If Len(GetJzdsPower) > 0 Then
				If InStr("," & GetJzdsPower & ",","," & zsadd & ",") > 0  Then
					JzdsLinkHTML = "<a href='../work/contentds.asp?ord=" & app.base64.pwurl(zsId) & "' target='_blank'>" & zstit & "</a>"
				else
					JzdsLinkHTML = zstit
				end if
			else
	...
	Function workButtonHTML(candel, canupdate, canxq, id)
		Dim html
		html = "<input type=""button"" value=""详情"" class=""oldbutton2"" onclick=""javascript:window.open('?__msgId=ShowInfo&id=" & id & "');""/> "
		If canupdate = 1 Then
			html = html & "<input type=""button"" value=""修改"" class=""oldbutton2"" onclick=""javascript:window.open('?__msgId=Edit&id=" & id & "');"" /> "
		end if
		If candel = 1 Then
			html = html & "<input type=""button"" value=""删除"" class=""oldbutton2"" onclick=""if(confirm('确认删除？')){DelSend(" & id & ");}"" />"
		end if
		workButtonHTML = html
	end function
	...
	Sub PageLoad()
		...
		Response.write(app.DefTopBarHTML("../", strFilePath, "添加关联对手", "<div style='padding-top:5px'><input type='submit' onclick='document.getElementById(""sb1"").click()' value='保存' class='oldbutton' />&nbsp;&nbsp;<input onclick='document.getElementById(""sb2"").click()' type='reset' value='重填' class='oldbutton' />&nbsp;&nbsp;&nbsp;</div>"))
		id = ChkNumEric(app.base64.deurl(request("ord")))
		Response.write "" & vbcrlf & "<script src= ""../Script/wk_dsinfo.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<style>" & vbcrlf & "   #content .label{" & vbcrlf & "                width:140px;" & vbcrlf & "            text-align:right;" & vbcrlf & "               padding-right:8px;" & vbcrlf & "      }" & vbcrlf & "</style>" & vbcrlf & "<form method=""post"" name=""myform"" action=""?__msgId=doSave"" onsubmit=""return CheckForm()&&Validator.Validate(this,2)"">" & vbcrlf & "  <input type=""hidden"" name=""itemId"" id=""itemId"" value="""
		Response.write id
		Response.write """ />" & vbcrlf & "  <input type=""hidden"" name=""userId"" id=""userId"" value=""0"" />" & vbcrlf & "  <table id=""content"" style='border-top:1px solid #ccc'>  <!-- 客户详情-关联对手-去掉border -->" & vbcrlf & "    <tr height=""30px"">" & vbcrlf & "      <td class='label'style='border-top:0px'>关联对手：</td>" & vbcrlf & "      <td align=""left"" style='border-top:0px;padding-left:5px;'><input type=""text"" style=""width:280px;color:#bbb"" readonly=""readonly"" value=""选择关联竞争对手"" name=""dsinfo"" id=""dsinfo"" onclick=""javascript:window.open('?__msgId=Search','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;"" />" & vbcrlf & "        <span id=""msgInfo"" style='color:red'>*</span></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr height=""30px"">" & vbcrlf & "      <td class='label'>对手报价：</td>" & vbcrlf & "      <td align=""left""  style='padding-left:5px;'><input type=""text"" name=""dsMoney"" id=""dsMoney"" maxlength=""15"" value=""0"" onpropertychange='formatData(this,""money"")'/>" & vbcrlf & "        (元) <span id=""msgprice"" style='color:red'>*</span></td>" & vbcrlf & "    </tr>" & vbcrlf & "     <tr>" & vbcrlf & "      <td class='label'>竞争产品：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""jzcp"" id=""jzcp"" min=""0"" max=""200"" datatype=""Limit"" msg=""必须少于200个字符""></textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手优势：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""dsYs"" id=""dsYs"" min=""0"" max=""2000"" datatype=""Limit"" msg=""必须少于2000个字符""></textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手劣势：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""dsLs"" id=""dsLs"" min=""0"" max=""2000"" datatype=""Limit"" msg=""必须少于2000个字符""></textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td align=""right"">备注：</td>" & vbcrlf & "      <td align=""left""><textarea class=""textbox"" style=""display:none;"" name=""intro"" id=""intro""></textarea>" & vbcrlf & "        <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro&style=news&isNew=0"" FRAMEBORDER=""0""SCROLLING=""no"" WIDTH=""98%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME></td> "& vbcrlf &"     </tr> "& vbcrlf &"     <tr> "& vbcrlf &"       <td colspan=""2"" align=""center""><input type=""submit"" value=""保存"" id='sb1' class=""button"" />&nbsp;&nbsp; "& vbcrlf & "        <input type=""reset"" value=""重填"" class=""button"" id='sb2' /></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & "</form>" & vbcrlf & ""
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub doSave()
		Dim itemId, userId, dsMoney, jzcp, dsYs, dsLs, intro, SQL, Rs, zdyCount, arrSp, BillID, strValue, i, oSQL, oRs
		itemId = ChkNumEric(app.GetInt("itemId"))
		userId = ChkNumEric(app.GetInt("userId"))
		dsMoney = app.GetFloat("dsMoney")
		jzcp = ChkQueryStr(app.GetText("jzcp"))
		dsYs = ChkQueryStr(app.GetText("dsYs"))
		dsLs = ChkQueryStr(app.GetText("dsLs"))
		intro = app.GetText("intro")
		SQL = "Select * From tel_zjds where 1=0 "
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.Open SQL, cn, 1, 3
		Rs.AddNew
		Rs("addUser") = info.user
		Rs("itemId") = itemId
		Rs("userId") = userId
		Rs("dsMoney") = dsMoney
		Rs("jzcp") = jzcp
	...
	sub SearchByKey
		dim Key
		Dim oSQL, lvw, powers , rs
		key = replace(request.form("key"),"'","")
		set rs = cn.execute("select case qx_open when 3 then '' else qx_intro end as p from power where sort1=43 and sort2=1 and ord=" & info.user)
		if rs.eof then
			powers = " and 1=0"
		else
			powers = rs.fields(0).value
			if len(powers) > 0 then
				powers = " and charindex(',' + cast(a.cateid as varchar(12)) + ',','," & replace(powers," ","") & ",')>0"
'if len(powers) > 0 then
			end if
		end if
		rs.close
		oSQL = "Select id As '编号',name As '选择',name As '对手名称', (select top 1 name from gate g where g.ord=a.cateid) as 添加人员 "
		oSQL = oSQL & "from jzds a where del=1 " & powers & " and name like '%" & key & "%'  order by id desc "
		...
		lvw.headers(2).formatText = "<a href=""javascript:void(0);"" onclick=""doSub('@value','@cells[1]')"">点击选择此对手</a>"
		lvw.headers(2).width = "200"
		lvw.headers(3).width = "300"
		lvw.headers(4).width = "200"
		Response.write(lvw.html)
	end sub
	Sub Search()
		Dim strFilePath
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery.js""></script>"
		Response.write(app.DefTopBarHTML("../", strFilePath, "选择关联对手", "")) & vbCrLf
		Response.write("<style>")
		Response.write("     .listview table")
		Response.write("     {")
		Response.write("             border-top:0px solid #000;")
		Response.write("     {")
		Response.write("     }")
		Response.write("     .listview th")
		Response.write("     {")
		Response.write("             border-top:0px solid #000;height:22px;")
		Response.write("     {")
		Response.write("     }")
		Response.write("</style>")
		Response.write("<script type=""text/javascript"">") & vbCrLf
		Response.write("window.onlistviewRefresh=function() {") & vbCrLf
		Response.write("    document.getElementById('lvw_dbtable_ids').style.borderTop='0px solid red';") & vbCrLf
		Response.write("}") & vbCrLf
		Response.write("window.onload=function() {") & vbCrLf
		Response.write("    document.getElementById('lvw_dbtable_ids').style.borderTop='0px solid red';") & vbCrLf
		Response.write("}") & vbCrLf
		Response.write("function doSub(str, strId) {") & vbCrLf
		Response.write("    window.opener.document.getElementById('dsinfo').value = str;") & vbCrLf
		Response.write("    window.opener.document.getElementById('dsinfo').style.color=""#000"";") & vbCrLf
		Response.write("    window.opener.document.getElementById('userId').value = strId;") & vbCrLf
		Response.write("    window.close();") & vbCrLf
		Response.write("}") & vbCrLf
		Response.write("function dosearch(key){") & vbcrlf
		Response.write("    ajax.regEvent(""SearchByKey"");") & vbCrLf
		Response.write("    ajax.addParam(""key"",key);") & vbCrLf
		Response.write("    document.getElementById(""listbody"").innerHTML = ajax.send();") & vbCrLf
		Response.write("    window.onlistviewRefresh();") & vbCrLf
		Response.write("}") & vbcrlf
		Response.write("</script>" & vbCrLf) & vbCrLf
		Response.write("<div style='position:absolute;right:20px;top:3px;'>对手名称：<input type=text onkeypress='if(window.event.keyCode==13){dosearch(this.value)}'></div><div style='overflow:hidden;border-top:1px solid #cdcde0;' id='listbody'>")
		Response.write("</script>" & vbCrLf) & vbCrLf
		Dim oSQL, lvw, powers , rs
		set rs = cn.execute("select case qx_open when 3 then '' else qx_intro end as p from power where sort1=43 and sort2=1 and ord=" & info.user)
		if rs.eof then
...
'if len(powers) > 0 then
		...
		rs.close
		oSQL = "Select id As '编号',name As '选择',name As '对手名称', (select top 1 name from gate g where g.ord=a.cateid) as 添加人员 "
		oSQL = oSQL & "from jzds a where del=1 " & powers & " order by id desc "
		Set lvw = New listview
		lvw.checkbox = False
		lvw.indexbox = False
		lvw.id = "ids"
		...
		Response.write(lvw.html)
		Response.write("</div>")
		Response.write "</div><div class='bottomdiv'></div>"
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub ShowInfo()
		Dim strFilePath
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery.js""></script>"
		Dim id, oSQL, oRs, dsName, dsMoney, dsYs, dsLs, intro, userId, addUser, addTime, jzcp, addUserid,dscateid
		id = ChkNumEric(request("id"))
		oSQL = "Select Top 1 (Select Top 1 name From jzds where jzds.id = tel_zjds.userId and jzds.del=1) As dsName,addTime,dsMoney,dsYs,"
		oSQL = oSQL & "dsLs,intro,userId,addTime,"
		oSQL = oSQL & "(Select Top 1 name From gate Where ord = tel_zjds.addUser) As AddUserName,jzcp,isnull(addUser,0) addUser,(Select Top 1 cateid From jzds where jzds.id = tel_zjds.userId and jzds.del=1) As cateid From tel_zjds Where id = "&id&" "
		...
		If (oRs.EOF And oRs.BOF) Then
			...
			jzcp=oRs("jzcp")
			dsName = oRs("dsName")
			dsMoney =zbcdbl( oRs("dsMoney"))
			dsYs = oRs("dsYs")
			dsLs = oRs("dsLs")
			intro = oRs("intro")
			userId = oRs("userId")
			addUserid=oRs("addUser")
			addUser = oRs("AddUserName")
			addTime = oRs("addTime")
			dscateid= oRs("cateid")
		end if
		oRs.Close
		Set oRs = Nothing
		dim qx_print, hspower, html
		qx_print = app.power.GetPowerIntro(1,7)
		hspower = True
		html = "<div id='ccc' style='padding-top:5px'>"
		hspower = True
		If Info.isSupperAdmin Or addUserid=Info.User Then
			html = html & "<input type=""button"" value="" 修改 "" class=""oldbutton"" onclick=""window.location.href = '?__msgId=Edit&id=" & id & "';""/>"
			html = html & "&nbsp;&nbsp;<input type=""button"" value=""删除"" class=""oldbutton"" onclick=""if(confirm('确认删除？')){AjaxSend();}""/>"
		end if
		if len(qx_print) >  0 then
			if cn.execute("select  1 where " & addUserid & " in (" & qx_print & ")").eof then
				hspower = false
			end if
		end if
		if hspower then
			html = html & "&nbsp;&nbsp;<input type=""button"" value=""打印"" class=""oldbutton"" onclick='document.getElementById(""ccc"").style.display=""none"";window.print();document.getElementById(""ccc"").style.display=""block""'/>&nbsp;"
		end if
		html = html & "&nbsp;</div>"
		Response.write(app.DefTopBarHTML("../", strFilePath, "关联对手详情", html))
		Response.write "" & vbcrlf & "<style>" & vbcrlf & "      #comm_itembarbg{height:43px!important;}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/wk_dsinfo_1.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<input type=""hidden"" id=""userid"" name=""userid"" value="""
		Response.write id
		Response.write """ />" & vbcrlf & "<table id=""content"">" & vbcrlf & "  <tr class=""top""><td align=""left"" width=""10%"" colspan=""4"">&nbsp;<strong class=""blue2"">基本信息</strong></td></tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"" width=""10%"">关联对手：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""40%"">" & vbcrlf & "  "
		If dsName&""<>"" Then
			If app.power.GetPowerIntro(43,1)<>"0" Then
				If Len(app.power.GetPowerIntro(43,1))=0 Or InStr("," & app.power.GetPowerIntro(43,1) & ",","," & dscateid & ",") > 0 Then
					If Len(app.power.GetPowerIntro(43,14))>0 Then
						If InStr("," & app.power.GetPowerIntro(43,14) & ",","," & dscateid & ",") > 0  Then
							Response.write "" & vbcrlf & "                                             <span style=""margin-left:10px;""><a href=""javascript:void(0);"" onclick=""javascript:window.open('../work/contentds.asp?ord="
'If InStr("," & app.power.GetPowerIntro(43,14) & ",","," & dscateid & ",") > 0  Then
							Response.write app.base64.pwurl(userId)
							Response.write "','newwinds','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100');return false;"">"
							Response.write app.base64.pwurl(userId)
							Response.write dsName
							Response.write "</a></span>" & vbcrlf & "                                  "
						else
							Response.write "" & vbcrlf & "                                             <span style=""margin-left:10px;""><font color=""#5b7cae"">"
							Response.write "</a></span>" & vbcrlf & "                                  "
							Response.write dsName
							Response.write "</font></span>" & vbcrlf & "                                       "
						end if
					else
						Response.write "" & vbcrlf & "                                     <span style=""margin-left:10px;""><a href=""javascript:void(0);"" onclick=""javascript:window.open('../work/contentds.asp?ord="
						Response.write "</font></span>" & vbcrlf & "                                       "
			...
			Response.write "" & vbcrlf & "             <span class=""red"" style=""margin-left:10px;"">该对手已被删除</span>" & vbcrlf & "   "
			Response.write "</a></span>" & vbcrlf & "                                  "
		end if
		Response.write "" & vbcrlf & "     </td>" & vbcrlf & "    <td align=""right"" width=""10%"">对手报价：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""40%""><span style=""margin-left:10px;"">"
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write Formatnumber(dsMoney,Info.moneynumber,-1)
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write "　(元)</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"" width=""10%"">添加人员：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""60%""><span style=""margin-left:10px;"">"
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write addUser
		Response.write "</span></td>" & vbcrlf & "    <td align=""right"" width=""10%"">添加时间：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""20%""><span style=""margin-left:10px;"">"
		Response.write addUser
		Response.write addTime
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">竞争产品：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"">"
		Response.write addTime
		Response.write HtmlBr(jzcp)
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">对手优势：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"">"
		Response.write HtmlBr(jzcp)
		Response.write HtmlBr(dsYs)
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">对手劣势：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"">"
		Response.write HtmlBr(dsYs)
		Response.write HtmlBr(dsLs)
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">备注：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"" class=""ewebeditorImg"">"
		Response.write HtmlBr(dsLs)
		Response.write intro
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div class='bottomdiv' style='border-top:0px;text-align:center'></div>" & vbcrlf & ""
		Response.write intro
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub Edit()
		Dim strFilePath,id,oSQL,oRs
		Dim dsName,userId,dsMoney,dsYs,dsLs,intro,jzcp
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery.js""></script>"
		Response.write(app.DefTopBarHTML("../", strFilePath, "修改关联对手", "<div style='padding-top:5px'><input type='submit' onclick='document.getElementById(""sb1"").click()' value='保存' class='oldbutton' />&nbsp;&nbsp;<input onclick='document.getElementById(""sb2"").click()' type='reset' value='重填' class='oldbutton' />&nbsp;&nbsp;&nbsp;</div>"))
		id = ChkNumEric(request("id"))
		oSQL = "Select Top 1 (Select Top 1 name From jzds where jzds.id = tel_zjds.userId) As dsName,userId,dsMoney,dsYs,dsLs,intro,jzcp From tel_zjds Where id = "&id&" "
		Set oRs = server.CreateObject("adodb.recordset")
		oRs.Open oSQL, cn, 1, 1
		If (oRs.EOF And oRs.BOF) Then
		else
		...
		oRs.Close
		Set oRs = Nothing
		Response.write "" & vbcrlf & "<script src= ""../Script/wk_dsinfo_2.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<style>" & vbcrlf & "  #content .label{" & vbcrlf & "                width:140px;" & vbcrlf & "            text-align:right;" & vbcrlf & "               padding-right:8px;" & vbcrlf & "      }" & vbcrlf & "</style>" & vbcrlf & "<form method=""post"" name=""myform"" action=""?__msgId=dsoEdit"" onsubmit=""return AjaxSend()&&Validator.Validate(this,2)"">" & vbcrlf & "  <input type=""hidden"" name=""id"" id=""id"" value="""
		Response.write id
		Response.write """ />" & vbcrlf & "  <input type=""hidden"" name=""userId"" id=""userId"" value="""
		Response.write userId
		Response.write """ />" & vbcrlf & "  <table id=""content"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label' style='border-top:0px'>关联对手：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px;border-top:0px'><input type=""text""  style=""width:280px;"" readonly=""readonly"" value="""
		Response.write userId
		Response.write dsName
		Response.write """ name=""dsinfo"" id=""dsinfo"" onclick=""javascript:window.open('?__msgId=Search','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;"" />" & vbcrlf & "        <span id=""msgInfo"" style='color:red'>*</span></td>"& vbcrlf &"     </tr> "& vbcrlf &"     <tr> "& vbcrlf &"       <td class"
		Response.write dsMoney
		Response.write """  onpropertychange='formatData(this,""money"")'/>" & vbcrlf & "        (元)</td>" & vbcrlf & "    </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class='label'>竞争产品：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""jzcp"" id=""jzcp"" min=""0"" max=""200"" datatype=""Limit"" msg=""必须少于200个字符"">"
		Response.write jzcp
		Response.write "</textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "     <td class='label'>对手优势：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px'><textarea style=""width:400px;height:50px;"" name=""dsYs"" id=""dsYs"" min=""0"" max=""2000"" datatype=""Limit"" msg=""必须少于2000个字符"">"
		Response.write dsYs
		Response.write "</textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手劣势：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px'><textarea style=""width:400px;height:50px;"" name=""dsLs"" id=""dsLs"" min=""0"" max=""2000"" datatype=""Limit"" msg=""必须少于2000个字符"">"
		Response.write dsLs
		Response.write "</textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td align=""right"">备注：</td>" & vbcrlf & "      <td align=""left""><textarea class=""textbox"" style=""display:none;"" name=""intro"" id=""intro"">"
		Response.write Server.HTMLEncode(intro & "")
		Response.write "</textarea>" & vbcrlf & "        <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro&style=news&isNew=0"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""98%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td colspan=""2"" align=""center""><input type=""submit"" value=""保存"" class=""button"" id='sb1'/>&nbsp;&nbsp;&nbsp;" & vbcrlf & "        <input type=""reset"" value=""重填"" class=""button"" id='sb2'/></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & "</form>" & vbcrlf & ""
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub doEdit()
		Dim id, userId, dsMoney, dsYs, dsLs, intro, SQL, Rs, strId, strValue, arrSp, i, oSQL, oRs, DataTextType, arrDataType, jzcp
		id = app.GetInt("id")
		userId = app.GetInt("userId")
		dsMoney = app.getfloat("dsMoney")
		jzcp = app.GetText("jzcp")
		dsYs = app.GetText("dsYs")
		dsLs = app.GetText("dsLs")
		intro = app.GetText("intro")
		SQL = "Select * From tel_zjds Where id = "& id &" "
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.Open SQL, cn, 1, 3
		Rs("userId") = userId
		Rs("dsMoney") = dsMoney
		...
		set rs = nothing
		Response.write("<script type=""text/javascript"">")
		Response.write("try{window.opener.doSearch(1);}catch(e){}")
		Response.write("window.close();")
		Response.write("</script>")
		call db_close : Response.end()
	end sub
	Sub doDel()
		...
		oSQL = "Delete From tel_zjds Where id = "& id &" "
		cn.Execute(oSQL)
		Response.write(1)
		call db_close : Response.end()
	end sub
	Sub List()
		Dim oSQL, strHtml,ItemsId, i,khmc
		ItemsId = app.getint("ItemsId")
		Response.write(app.DefTopBarHTML("../", "", "关联对手列表", "<span id='tmv'></span>")) & vbCrLf
		khmc=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=1")(0)
		khmc="关联客户"
		Response.write "" & vbcrlf & "    <script type=""text/javascript"">" & vbcrlf & "    function LoadUrl(str) {" & vbcrlf & "        location.href = str;" & vbcrlf & "    }" & vbcrlf & "    function showmenu(tag) {" & vbcrlf & "            if (document.getElementById('asasd'))" & vbcrlf & "           {" & vbcrlf & "                       document.getElementById(""asasd"").style.display='';" & vbcrlf & "                }" & vbcrlf & "               else " & vbcrlf & "           {       " & vbcrlf & "                        var obj=document.createElement(""div"");" & vbcrlf & "                    obj.id=""asasd"";" & vbcrlf & "                   obj.style.cssText=""Z-INDEX:20000; POSITION: absolute; HEIGHT: 270px; TOP:48px; LEFT: 186px;padding-top:2px 0px;background:#FFF"";" & vbcrlf & "                        var html;" & vbcrlf & "                       html="""";" & vbcrlf & "                  html=html+""<TABLE border=0 cellSpacing=-2 cellPadding=-2 width=162 height=300>"";" & vbcrlf & "                  html=html+""<TBODY><TR><TD height=139>"";" & vbcrlf & "                   html=html+""<TABLE border=0 width=162 class='resetTableBgColor' bgColor=#ecf5ff height=115><TBODY>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(10);doSearch();\""><FONT color=#2f496e>按照"
		'khmc="关联客户"
		Response.write khmc
		Response.write "排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                   html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(9);doSearch();\""><FONT color=#2f496e>按照"
		'Response.write khmc
		Response.write khmc
		Response.write "排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                   html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(2);doSearch();\""><FONT color=#2f496e>按照对手名称排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(1);doSearch();\""><FONT color=#2f496e>按照对手名称排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(4);doSearch();\""><FONT color=#2f496e>按照对手报价排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                  html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(3);doSearch();\""><FONT color=#2f496e>按照对手报价排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(6);doSearch();\""><FONT color=#2f496e>按照添加时间排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(5);doSearch();\""><FONT color=#2f496e>按照添加时间排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                   html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(8);doSearch();\""><FONT color=#2f496e>按照添加人员排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=24 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(7);doSearch();\""><FONT color=#2f496e>按照添加人员排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                  html=html+""</TBODY></TABLE>"";" & vbcrlf & "                     html=html+""</TD></TR></TBODY>"";" & vbcrlf & "                 html=html+""</TABLE>"";" & vbcrlf & "                     obj.innerHTML=html;" & vbcrlf & "                     document.body.appendChild(obj);" & vbcrlf & "         }" & vbcrlf & "    }" & vbcrlf & "    function DelSend(ids) {" & vbcrlf & "        $.get('?__msgId=doDel', {" & vbcrlf & "            id: ids" & vbcrlf & "        }," & vbcrlf & "        function(data) {" & vbcrlf & "            var msg = parseInt(data);" & vbcrlf & "            if (msg == 1) {" & vbcrlf & "                lvw_refresh('ids1');" & vbcrlf & "            }" & vbcrlf & "        })" & vbcrlf & "    }" & vbcrlf & "    function doSearch(pIndex) {" & vbcrlf & "        ajax.regEvent(""dosearch"")" & vbcrlf & "        ajax.addParam('px', $('#px').val());" & vbcrlf & "        ajax.addParam('stratTime', $('#stratTime').val());" & vbcrlf & "        ajax.addParam('endTime', $('#endTime').val());" & vbcrlf & "        ajax.addParam('stype', $('#stype').val());" & vbcrlf & "        ajax.addParam('key', $('#key').val());" & vbcrlf & "          ajax.addParam(""ItemsId"","
		Response.write ItemsId
		Response.write ");" & vbcrlf & "           ajax.addParam(""Psize"",$('#lvw_pgsize_selids1').val());" & vbcrlf & "            //pIndex==1表示要带上当前的翻页条件" & vbcrlf & "             ajax.addParam(""pageindex"",pIndex==1?document.getElementById(""lvw_pindex_ids1"").value:1);" & vbcrlf & "        ajax.send(function(r){" & vbcrlf & "                      document.getElementById(""lvwbody"").innerHTML = r;" & vbcrlf & "         });" & vbcrlf & "    }" & vbcrlf & "        function doExcel(){var lvw = new Listview('ids1');lvw.cexcel();}" & vbcrlf & "        window.onlistviewRefresh = function(){" & vbcrlf & "          document.getElementById(""toolbar1"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width; "& vbcrlf &"                 document.getElementById(""searchitemspanel"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width; "& vbcrlf &"     } "& vbcrlf &"        document.body.onload = function(){ "& vbcrlf &"               document.getElementById(""toolbar1"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "          document.getElementById(""searchitemspanel"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "    }" & vbcrlf & "       function searchClick() {" & vbcrlf & "                var W1="""";" & vbcrlf & "                var sW1=document.getElementsByName(""W1"");" & vbcrlf & "            for(var i=0;i<sW1.length;i++){" & vbcrlf & "                   if(sW1[i].checked){" & vbcrlf & "                            W1=W1+sW1[i].value+"","";" & vbcrlf & "                    }" & vbcrlf & "                 }" & vbcrlf & "            if (W1.length>0)" & vbcrlf & "                {" & vbcrlf & "                       W1=W1.substring(0,W1.length-1);" & vbcrlf& "               }" & vbcrlf & "               var W2="""";" & vbcrlf & "                var sW2=document.getElementsByName(""W2"");" & vbcrlf & "         for(var i=0;i<sW2.length;i++){" & vbcrlf & "                   if(sW2[i].checked){" & vbcrlf & "                            W2=W2+sW2[i].value+"","";" & vbcrlf & "                    }" & vbcrlf & "                 }" & vbcrlf & "            if (W2.length>0)" & vbcrlf & "              {" & vbcrlf & "                       W2=W2.substring(0,W2.length-1);" & vbcrlf & "         }" & vbcrlf & "               var W3="""";" & vbcrlf & "                var sW3=document.getElementsByName(""W3"");" & vbcrlf & "         for(var i=0;i<sW3.length;i++){" & vbcrlf & "                   if(sW3[i].checked){" & vbcrlf & "                            W3=W3+sW3[i].value+"","";" & vbcrlf & "}" & vbcrlf & "                 }" & vbcrlf & "            if (W3.length>0)" & vbcrlf & "                {" & vbcrlf & "                       W3=W3.substring(0,W3.length-1);" & vbcrlf & "         }" & vbcrlf & "           ajax.regEvent(""dosearch"")" & vbcrlf & "        ajax.addParam('px', $('#px').val());" & vbcrlf & "         ajax.addParam('W1', W1);" & vbcrlf & "   ajax.addParam('W2', W2);" & vbcrlf & "        ajax.addParam('W3', W3);" & vbcrlf & "               ajax.addParam('khmc', $('#khmc').val());" & vbcrlf & "        ajax.addParam('dsmc', $('#dsmc').val());" & vbcrlf & "        ajax.addParam('jzcp', $('#jzcp').val());" & vbcrlf & "                ajax.addParam('dsys', $('#dsys').val());" & vbcrlf & "        ajax.addParam('dsls', $('#dsls').val());" & vbcrlf & "        ajax.addParam('beiz', $('#beiz').val());" & vbcrlf & "         ajax.addParam('dsbj0', $('#dsbj_0').val());" & vbcrlf & "        ajax.addParam('dsbj1', $('#dsbj_1').val());" & vbcrlf & "        ajax.addParam('stratTime', $('#addtime_0').val());" & vbcrlf & "        ajax.addParam('endTime', $('#addtime_1').val());" & vbcrlf & "             ajax.addParam(""ItemsId"","
		Response.write ItemsId
		Response.write ");" & vbcrlf & "           ajax.addParam(""Psize"",$('#lvw_pgsize_selids1').val());" & vbcrlf & "            ajax.addParam(""isgj"",1);" & vbcrlf & "        ajax.send(function(r){" & vbcrlf & "                    document.getElementById(""lvwbody"").innerHTML = r;" & vbcrlf & "         });" & vbcrlf & "     }" & vbcrlf & "       " & vbcrlf & "        function resetClick(){" & vbcrlf & "               var inputs =  document.getElementsByTagName(""input"")" & vbcrlf & "              for (var i = 0 ; i < inputs.length ; i ++ )" & vbcrlf & "             {" & vbcrlf & "                       var b = inputs[i]" & vbcrlf & "                       if(b.type==""text"") { b.value = b.defaultValue; }" & vbcrlf & "                  if(b.type==""checkbox"") { b.checked = false; }" & vbcrlf & "          }" & vbcrlf & "       }" & vbcrlf & "    </script>" & vbcrlf & "  <style>#searchitemspanel {margin-top:-1px} td{height:27px;} html{padding:0px 10px;background:#EFEFEF;}</style>" & vbcrlf & "    <input type=""hidden"" id=""px"" value=""0"" />" & vbcrlf & "    <a class=""tableTitleLinks sortRule"" href=""javascript:void();"" onclick=""showmenu(this);return false"" style=""position: absolute; top: 20px; left: 234px;"">排序规则 <img class=""resetElementHidden"" src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""><img class=""resetElementShowNoAlign""style=""display:none;"" src=""../skin/default/images/MoZihometop/content/r_down2.png"" width=""9"" height=""5"" border=""0""></a> "& vbcrlf &"     <table id=""content"" style='background-image:url();border:0px;background-color:transparent;margin-top:2px;display:none;'> "& vbcrlf &"     <tr>" & vbcrlf & "            <td align=""right"" style='border-top:0px;border-bottom:0px;border-left:0px'></td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "     <div style='background-image:url(../images/112.gif);width:100%;' id='toolbar1' align=""right"">" & vbcrlf & "       <table style='width:auto;table-layout:auto;border-collapse:separate;margin-right:120px' cellspacing='3' id=""kh"">" & vbcrlf & "       <tr>" & vbcrlf & "    <td align=""right"" style='border-top:0px;border-bottom:0px;border-left:0px'> 添加时间：<input type=""text"" id=""stratTime"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" /> - <input type=""text"" id=""endTime"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" />　" & vbcrlf & "    <select id=""stype"">　" & vbcrlf & "      <option value=""0"" selected=""selected"">对手名称</option>" & vbcrlf & "      <!-- <option value=""1"">关联客户</option> --> " & vbcrlf & "          <option value=""6"">竞争产品</option>" & vbcrlf & "       <option value=""3"">对手优势</option>" & vbcrlf & "      <option value=""4"">对手劣势</option>" & vbcrlf & "        <option value=""2"">对手报价</option>" & vbcrlf & "      <!-- <option value=""5"">备注</option> -->" & vbcrlf & "    </select>" & vbcrlf & "    <input type=""text""  id=""key"" maxlength=""20"" /> " & vbcrlf & "    <input type=""button"" value="" 检索 "" class=""oldbutton"" onclick=""doSearch();"" /> " & vbcrlf & "    "
		if app.power.existsPower(1,10) then
			Response.write "<input type=""button"" value="" 导出 "" class=""oldbutton"" id=""dcdb"" onclick='doExcel()'/>&nbsp;"
		end if
		if app.power.existsPower(1,7) then
			Response.write "<input type=""button"" value="" 打印 "" class=""oldbutton"" id=""dydb"" onclick=""$('#dcdb').hide();$('#dydb').hide();window.print();$('#dcdb').show();$('#dydb').show();"" />&nbsp;"
		end if
		Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "       <span id=""ht1""></span>" & vbcrlf & "    </div>" & vbcrlf & "  "
		Dim aSearch
		Set aSearch = New AdvanceSearchClass
		aSearch.adSearchAutoHide = true
		aSearch.AddField "添加人员", "gates", "addcate", ""
		aSearch.AddField "关联客户", "text", "khmc", ""
		aSearch.AddField "对手名称", "text", "dsmc", ""
		aSearch.AddField "竞争产品", "text", "jzcp", ""
		aSearch.AddField "对手优势", "text", "dsys", ""
		aSearch.AddField "对手劣势", "text", "dsls", ""
		aSearch.AddField "备注", "text", "beiz", ""
		aSearch.AddField "对手报价", "moneys", "dsbj", ""
		aSearch.AddField "添加时间", "dates", "addtime", ""
		Call aSearch.showButton
		Response.write "" & vbcrlf & "     <script type=""text/javascript"">" & vbcrlf & "   document.getElementById(""content"").swapNode(document.getElementById(""tmv""));" & vbcrlf & "        </script>" & vbcrlf & "       <style>" & vbcrlf & "         .lvw_pagebar" & vbcrlf & "            {" & vbcrlf & "                       background-repeat:repeat-x;" & vbcrlf & "             height:40px;" & vbcrlf & "            }" & vbcrlf & "       </style>" & vbcrlf & "        <div style=""overflow:hidden;min-width:900px;border-top:0px solid #cdcde0"" id='lvwbody'>" & vbcrlf & "   "
		'Call aSearch.showButton
		Dim lvw
		Set lvw = New listview
		...
		lvw.sql = "exec erp_tel_gldsList " & Info.User & "," & ItemsId & ",&excelmode,0,'','','',0,'','','','','','','','',''"
		lvw.headers(1).display = "none"
		lvw.headers(2).formattext = "code:ChanceLinkHTML(@cells[15],""@cells[2]"",@cells[14])"
		lvw.headers(3).formattext = "code: jzdsLinkHTML(@cells[17],""@cells[3]"",@cells[16])"
		lvw.headers(10).formatText = "code:  workButtonHTML(@cells[11],@cells[13],@cells[12],@cells[1])"
		lvw.headers(2).dbtype = "str"
		lvw.headers(2).width = 80
		lvw.headers(3).dbtype = "str"
		lvw.headers(3).width = 80
		lvw.headers(4).dbtype = "money"
		lvw.headers(4).width = 40
		lvw.headers(5).dbtype = "str"
		lvw.headers(5).width = 80
		lvw.headers(6).dbtype = "str"
		lvw.headers(6).width = 80
		lvw.headers(7).dbtype = "str"
		lvw.headers(7).width = 80
		lvw.headers(8).width = 60
		lvw.headers(9).width = 60
		lvw.headers(10).width = 110
		For i=11 To 17
	...
	Function HtmlBr(Str)
		If (Str = "" Or IsNull(Str)) Then
			...
			strTemp = Replace(strTemp, Chr(10) & Chr(13), "<br/>")
			HtmlBr = strTemp
		end if
	end function
	Sub SearchResult
		Dim k, d1, d2, ItemsId, i
		k = ChkQueryStr(app.GetText("key"))
		d1 = app.GetText("stratTime")
		d2 = app.GetText("endTime")
		ItemsId = app.getInt("ItemsId")
		Dim aSearch,isgj,gates,khmc,dsmc,jzcp,dsys,dsls,beiz,dsbj0,dsbj1
		isgj=app.GetText("isgj")
		If isgj="1" Then k=""
		Set aSearch = New AdvanceSearchClass
		gates=aSearch.GetGates(1)
		khmc=app.GetText("khmc")
		dsmc=app.GetText("dsmc")
		jzcp=app.GetText("jzcp")
		dsys=app.GetText("dsys")
		dsls=app.GetText("dsls")
		beiz=app.GetText("beiz")
		dsbj0=app.GetText("dsbj0")
		dsbj1=app.GetText("dsbj1")
		Dim lvw
		Set lvw = New listview
		lvw.PageButtonAlign = "right"
		lvw.oldPageSizeUI = True
		...
		lvw.addlink = ""
		lvw.border = 0
		lvw.pageindex = app.getint("pageindex")
		lvw.cansort = False
		lvw.sql = "exec erp_tel_gldsList " & Info.User & "," & ItemsId & ",&excelmode," & app.GetInt("stype") & ",'" & k & "','" & d1 & "','" & d2 & "'," & app.GetInt("px") & ",'" & gates & "','" & khmc & "','" & dsmc & "','" & jzcp & "','" & dsys & "','" & dsls & "','" & beiz & "','" & dsbj0 & "','" & dsbj1 &"'"
		lvw.headers(1).display = "none"
		lvw.headers(2).formattext = "code:ChanceLinkHTML(@cells[15],""@cells[2]"",@cells[14])"
		lvw.headers(3).formattext = "code:  jzdsLinkHTML(@cells[17],""@cells[3]"",@cells[16])"
		lvw.headers(10).formatText = "code:workButtonHTML(@cells[11],@cells[13],@cells[12],@cells[1])"
		...
		For i=11 To 17
			lvw.headers(i).display = "none"
		next
		lvw.pagesize = app.getint("Psize")
		Response.write lvw.html
	end sub
	Function ZdyFun(strType, strName, strNum, strValue)
		Dim strTemp, oRs, oSQL
		If (strType = "" Or IsNull(strType) Or Not IsNumeric(strType)) Then
			Exit Function
		else
			Select Case strType
			Case 1
			strTemp = "<input maxlength=200 type=""text"" name="""& strName &""" id="""& strName &""" style=""width:400px;"" value="""& strValue &""" />"
			Case 2
			strTemp = "<textarea  maxlength=200 style=""width:400px;height:50px;"" name="""& strName &""" id="""& strName &""">"& strValue &"</textarea>"
			Case 3
			strTemp = "<input maxlength=200 type=""text"" name="""& strName &""" id="""& strName &""" onclick=""datedlg.show()"" value="""& strValue &""" size=""10"" readonly=""readonly"" />"
			Case 4
			strTemp = "<input maxlength=200 type=""text"" name="""& strName &""" id="""& strName &""" size=""20"" value="""& strValue &""" onkeyup=""value=value.replace(/[^\d]/g,'')"" />"
			Case 5
			strTemp = "<textarea  maxlength=200 style=""width:400px;height:50px;"" name="""& strName &""" id="""& strName &""">"& strValue &"</textarea>"
			Case 6
			strTemp = "<input type=""radio"" name="""& strName &""" value=""1"""&app.IIf(Trim(strValue) = "1", " checked=""checked""", "")&" />是　<input type=""radio"" name="""& strName &""" value=""0"""&app.IIf((Trim(strValue) = "0" Or strValue = ""), " checked=""checked""", "")&" />否"
			Case 7
			oSQL = "SELECT id,CFID,CValue  FROM ERP_CustomOptions WHERE CFID = "&strNum&" ORDER BY id desc"
			Set oRs = server.CreateObject("adodb.recordset")
			oRs.Open oSQL, Cn, 1, 1
			If Not oRs.EOF Then
				strTemp = "<select name="""& strName &""">"
				Do While Not oRs.EOF
					strTemp = strTemp & "  <option value="""&oRs(2)&""""&app.IIf(Trim(strValue) = Trim(oRs(2)), " selected=""selected""", "")&">"&oRs(2)&"</option>"
					oRs.MoveNext
				Loop
	...
	Function ShowMsgBox(Str, strType)
		If strType = 6 Then
	...
	Function ChkQueryStr(ByVal Str)
		on error resume next
		If IsNull(Str) Then
			ChkQueryStr = ""
			Exit Function
