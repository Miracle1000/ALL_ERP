...
'dim i
		...
		Dim Str_Result
		Dim intro_1_1, intro_2_1, intro_2_6
		Sub page_load
			Set conn = cn
			Dim open_1_10, intro_1_10
			Dim i, rs, sql, searchtype, C, list, px, tt, px_Result, Export_Fields
			tt = "客户积分"
			Call fillPowerInfo(1,10,open_1_10,intro_1_10)
			if open_1_10=3 then
				list=""
			elseif open_1_10=1 then
				list="and a.cateid in ("&intro_1_10&")"
			else
				list="and a.cateid=0"
			end if
			Str_Result="where a.del=1 and isnull(a.sp,0)=0 and a.sort3=1 "&list&""
			...
			If searchtype<>"" And C<>"" Then
				...
				ElseIf searchtype="2" Then
					Str_Result = Str_Result & " and a.khid like '%"&C&"%'"
				...
				ElseIf searchtype="7" Then
					Str_Result = Str_Result & " and p.name like '%"&C&"%' "
				ElseIf searchtype="8" Then
					Str_Result = Str_Result & " and p.phone like '%"&C&"%' "
				ElseIf searchtype="9" Then
					Str_Result = Str_Result & " and p.mobile like '%"&C&"%' "
				end if
			else
				searchtype="0"
			end if
			...
			px = CInt(px)
			if px=1 then
				px_Result="order by 客户编号 desc, 剩余积分 desc, 积分总数 desc,消费积分 desc,a.ord desc "
			elseif px=2 then
				px_Result="order by 客户编号, 剩余积分,积分总数 ,消费积分 ,a.ord  "
			elseif px=3 then
				px_Result="order by 客户名称 desc, 剩余积分 desc,积分总数 desc,消费积分 desc,a.ord desc "
			elseif px=4 then
				px_Result="order by 客户名称 , 剩余积分,积分总数 ,消费积分 ,a.ord  "
			elseif px=5 then
				px_Result="order by 积分总数 desc, 剩余积分 desc,消费积分 desc,a.ord desc "
			elseif px=6 then
				px_Result="order by 积分总数 , 剩余积分 ,消费积分 ,a.ord "
			elseif px=7 then
				px_Result="order by 剩余积分 desc,积分总数 desc,消费积分 desc,a.ord desc "
			elseif px=8 then
				px_Result="order by 剩余积分 ,积分总数 ,消费积分 ,a.ord "
			elseif px=9 then
				px_Result="order by 消费积分 desc, 剩余积分 desc,积分总数 desc,a.ord desc "
			elseif px=10 then
				px_Result="order by 消费积分 , 剩余积分,积分总数 ,a.ord  "
			end if
			sql="select isnull(a.cateid,0) khcateid,isnull(p.cateid,0) xlrAddcate, " &_
			"   a.name as 客户名称,a.khid as 客户编号, p.name 联系人姓名," &_
			"   p.phone 联系人电话, p.mobile 联系人电手机,g.name as 销售人员, " &_
			"   (select isnull(sum(jf),0) as sum_jf from teljf where company=a.ord) as 积分总数, a.jf 剩余积分, " &_
			"   (select isnull(sum(num1),0) as sum_jf2 from teljf2 where company=a.ord) as 消费积分, a.date1 添加时间 " &_
			"   from tel a left join person p on a.person=p.ord and p.del=1 " &_
			"   left join gate g on a.cateid=g.ord  "&Str_Result &" "& px_Result
			Export_Fields = "khcateid,xlrAddcate,客户名称,客户编号,联系人姓名,联系人电话,联系人电手机,积分总数,剩余积分,消费积分,销售人员,添加时间"
			Dim adapter
			...
			For i = 0 To ubound(arrFields)
				...
				Select Case field
				Case "客户名称"
				hd.Formula = "getColHtml(@cells[1],""客户名称"",@value,0)"
				hd.dbtype = "str"
				hd.excelAlign = "left"
				hd.width = EX_WIDTH_DATE
				Case "联系人姓名"
				hd.Formula = "getColHtml(@cells[1],""联系人姓名"",@value,0)"
				hd.dbtype = "str"
				hd.excelAlign = "left"
				hd.width = EX_WIDTH_MAIL
				Case "联系人电话", "联系人电手机"
				hd.Formula = "getColHtml(@cells[1],"""& field &""",@value,0)"
				hd.dbtype = "str"
				hd.excelAlign = "left"
				hd.width = EX_WIDTH_TEL
				Case "客户编号"
				hd.dbtype = "str"
		...
		Function getColHtml(mxid,colName,colValue,revID)
			Dim tempStr, khcateid, lxrAddcate
			tempStr = ""
			Select Case colName
			Case "客户名称"
			khcateid = mxid
			If isEmpty(intro_1_1) Then
				intro_1_1 = app.power.GetPowerIntro(1,1)
			end if
			if intro_1_1="" or (instr(","& intro_1_1 &"," , ","& khcateid &",")>0 And khcateid<>0) then
				tempStr = colValue
			end if
			Case "联系人姓名"
			lxrAddcate = mxid
			If isEmpty(intro_2_1) Then
				intro_2_1 = app.power.GetPowerIntro(2,1)
			end if
			if intro_2_1="" or (instr(","& intro_2_1 &"," , ","& lxrAddcate &",")>0 And lxrAddcate<>0) then
				tempStr = colValue
			end if
			Case "联系人电话", "联系人电手机"
			lxrAddcate = mxid
			If lxrAddcate &"" = "" Then lxrAddcate = 0
			If colValue&""<>"" Then
				If isEmpty(intro_2_6) Then
					intro_2_6 = app.power.GetPowerIntro(2,6)
				end if
				if intro_2_6="" or (instr(","& intro_2_6 &"," , ","& lxrAddcate &",")>0 And lxrAddcate<>0) then
					tempStr = colValue
				else
					tempStr = GetPhoneNumber(colValue)
				end if
			end if
			End Select
			getColHtml = tempStr
		...
		Function GetPhoneNumber(phoneNum)
			Dim HiddenNumber, numLength,i, num()
			...
			If numLength > 0 Then
				...
				For i = 1 To numLength
					num(i) = "*"
				next
				HiddenNumber = Join(num,"")
			else
				HiddenNumber = ""
			end if
			GetPhoneNumber = HiddenNumber
		end function
