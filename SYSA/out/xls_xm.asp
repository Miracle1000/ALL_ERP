...
'Private Sub Class_Initialize
		...
		Dim Str_Result,area_list,aryReturn2
		Dim allPersons ,open_1_1,intro_1_1,open_26_1,intro_26_1
		open_1_1 = "-1"
'Dim allPersons ,open_1_1,intro_1_1,open_26_1,intro_26_1
		Sub page_load
			Set conn = cn
			Dim A1,A2,D,E,F,F1,F2,H,m1,m2,khmc,khdh,khcz,khdz,m3,m4,m5,m6,B1,B2,C1,C2,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6,khid,remind,needProc,px,px_Result
			A1=request("A1")
			A2=request("A2")
			D=request("D")
			E=request("E")
			...
			If needProc&""= "" Then needProc = ""
			Dim list,spState,rs
			Dim open_3_10,intro_3_10
			Call fillPowerInfo(3,10,open_3_10,intro_3_10)
			Call fillPowerInfo(1,1,open_1_1,intro_1_1)
			Call fillPowerInfo(26,1,open_26_1,intro_26_1)
			if open_3_10=3 then
				list=""
			elseif open_3_10=1 then
				list="and cateid in ("&intro_3_10&") "
			else
				list="and cateid=0 "
			end if
			spState = request("spState")
			If Len(spState & "") = 0 Or isnumeric(spState) = False Then spState = 0
			Dim helper
			If remind <> "" Then
				Set helper = CreateReminderHelper(cn,remind,0)
				Str_Result = " where a.ord in (" & helper.listSQL("ids") & ")"
			ElseIf khid & "" <> "" Then
				Str_Result="where del<>2 and del<>7"
			else
				Select Case spState
				Case 0 : Str_Result="where del=1 "&list&""
				Case 1 : Str_Result="where (del=3 and sp<>-1) "&list&""
				Case 0 : Str_Result="where del=1 "&list&""
				Case 2 : Str_Result="where (del=3 and sp=-1) "&list&""
				Case 0 : Str_Result="where del=1 "&list&""
				End select
			end if
			Dim W1,W2,W3,W4
			...
			If Not noWhere Then
				...
				Str_Result=Str_Result &" and cateid in("& Rsw &") "
			End If
			area_list=-1
'End If       '
			Dim n
			if A2<>"" then
				...
				For i = 0 To UBound(aryReturn2)
					...
					if rs.eof then
						...
						do until rs.eof
							if CheckPurview(A2,trim(rs("id")))=True  Then n=1
							if n=1 then exit do
							rs.movenext
						loop
					end if
			...
			'Str_Result = Str_Result & " and a.ord in("& getRelevanceOrds("chance", datatype, fromid) &") "
			if E<>"" Then Str_Result=Str_Result+"and complete1 in ("&E&")"
			'Str_Result = Str_Result & " and a.ord in("& getRelevanceOrds("chance", datatype, fromid) &") "
			if F<>"" Then Str_Result=Str_Result+"and complete2 in ("&F&")"
			'Str_Result = Str_Result & " and a.ord in("& getRelevanceOrds("chance", datatype, fromid) &") "
			if A1<>"" Then Str_Result=Str_Result+"and sorce in ("&A1&")"
			'Str_Result = Str_Result & " and a.ord in("& getRelevanceOrds("chance", datatype, fromid) &") "
			if m3<>"" Then Str_Result=Str_Result &" and date1>='"&m3&"'"
			if m4<>"" Then Str_Result=Str_Result &" and date1<='"&cdate(m4)&" 23:59:59'"
			if m5<>"" Then Str_Result=Str_Result &" and date2>='"&m5&"'"
			if m6<>"" Then Str_Result=Str_Result &" and date2<='"&cdate(m6)&" 23:59:59'"
			If Not noWhere Then
				if m1<>"" then
					Str_Result=Str_Result &" and date1>='"&m1&"'"
				end if
				if m2<>"" then
					Str_Result=Str_Result &" and date1<='"&cdate(m2)&" 23:59:59'"
				end if
			else
				if m1<>"" then
			...
			If needProc&""="0" Then
				Str_Result=Str_Result &" and  isnull(procid,0) = 0"
			ElseIf needProc&""="1" Then
				Str_Result=Str_Result &" and  isnull(procid,0) > 0"
			end if
			If request("propm")&""="2" Then
				Set rs=conn.execute("select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where intro=1  and cateid="&session("personzbintel2007")&" and ord=64")
				If Not rs.eof Then
					Str_Result= Str_Result & " and cateid_sp = "&session("personzbintel2007")&""
				else
					Str_Result= Str_Result & " and 1=0 "
				end if
				rs.close
			ElseIf request("propm")&""="3" Then
				Set rs=conn.execute("select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where intro=1  and cateid="&session("personzbintel2007")&" and ord=63")
				If Not rs.eof Then
					If rs("fw1")="1" Then
...
'If rs("fw1")="1" Then
			...
			If request("newstate")&""="1" Then
				set rs=server.CreateObject("adodb.recordset")
				sql="select ord,intro,num1,tq1,fw1 from setjm2  where ord=33 and cateid="&session("personzbintel2007")&""
				rs.open sql,conn,1,1
				if not rs.eof then
					If rs("fw1")="1" Then
						Str_Result= Str_Result & " and (cateid="&session("personzbintel2007")&" or (share='1' or charindex(',"&session("personzbintel2007")&",',','+isnull(share,'0')+',')>0))"
...
'If rs("fw1")="1" Then
				...
				rs.close
				set rs=Nothing
			ElseIf request("newstate")&""="3" Then
				Set rs=conn.execute("select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where intro=1  and cateid="&session("personzbintel2007")&" and ord=89")
				If Not rs.eof Then
					If intro_3_16&""<>"" Then intro_3_16 = Replace(intro_3_16," ","")
					Str_Result= Str_Result & " and cateid_sp = "&session("personzbintel2007")&" and sp<>0 and sp<>-1 and ("& open_3_16 &"=3 or ("& open_3_16 &"=1 and charindex(','+cast(cateid as varchar(10))+',',',"& intro_3_16 &",')>0)) "
'If intro_3_16&""<>"" Then intro_3_16 = Replace(intro_3_16," ","")
				else
					Str_Result= Str_Result & " and 1=0 "
				end if
			...
			Dim strsql_kh, khnum
			strsql_kh ="" : khnum = 0
			If khmc<>"" Then kh_js=kh_js &" and name like '%"&khmc&"%'"
			If khdh<>"" Then kh_js=kh_js &" and phone like '%"&khdh&"%'"
			If khcz<>"" Then kh_js=kh_js &" and fax like '%"&khcz&"%'"
			If khdz<>"" Then kh_js=kh_js &" and address like '%"&khdz&"%'"
			if kh_js<>"" Then
				Set rs=conn.execute("select ord from tel where del=1 and sort3=1 "&kh_js&"")
				If rs.eof Then
					...
					While Not rs.eof
						...
						strsql_kh=strsql_kh & " charindex(',"&rs("ord")&",',','+replace(company,' ','')+',')>0"
'If khnum<>0 Then strsql_kh=strsql_kh&" or "
						khnum=khnum+1
'If khnum<>0 Then strsql_kh=strsql_kh&" or "
						rs.movenext
...
'elseif B2="zdy4" then
				elseif B2="khmc" then
					Str_Result=Str_Result &" and a.ord in (select y.ord from tel x inner join chance y on x.del=1 and y.del=1 and x.sort3=1 and x.name like '%sdeee%' and charindex(','+cast(x.ord as nvarchar(4000))+',',','+y.company+',')>0)"
'elseif B2="khmc" then
				end if
			end if
			Call searchExtended(3,"a.ord")
			If khid&""<>"" Then
				Str_Result=Str_Result &" and charindex('," & khid & ",',','+replace(company,' ','')+',')>0"
'If khid&""<>"" Then
			end if
			...
			elseif px=13 then
				px_Result="order by (select gate1 from sortjh2 where ord=a.complete2) desc,date7 desc"
			elseif px=14 then
				px_Result="order by (select gate1 from sortjh2 where ord=a.complete2) asc,date7 desc"
			elseif px=15 then
				px_Result="order by (select top 1 name from gate where ord=a.cateid) desc,date7 desc"
			elseif px=16 then
				px_Result="order by (select top 1 name from gate where ord=a.cateid) asc,date7 desc"
			elseif px=17 then
				px_Result="order by (select top 1 date7 from reply where ord2=a.ord and sort1=2 order by date7 desc) desc,date7 desc"
			elseif px=18 then
				px_Result="order by (select top 1 date7 from reply where ord2=a.ord and sort1=2 order by date7 desc) asc,date7 desc"
			elseif px=19 then
			...
			if H="7" then
				str_Result=str_Result& " and isnull(share,'0')<>'0' and share<>'' and share<>'0' and share<>'-1' and cateid ="&session("personzbintel2007")&" and del=1"
'if H="7" then
			elseif H="8" Then
				If request("remind") <> "" Then
					Set helper = CreateReminderHelper(conn,54,0)
					str_Result=str_Result& " and a.ord in (" & helper.listSQL("ids") & ")"
				else
					if cstr(trim(w3))="0" Or Len(w3)=0 then w3=session("personzbintel2007")
					str_Result=str_Result& " and  (','+share+',' like '%"&","&w3&","&"%' or share='1') and cateid <>"&session("personzbintel2007")&" and del=1"
'if cstr(trim(w3))="0" Or Len(w3)=0 then w3=session("personzbintel2007")
				end if
			elseif H="81" then
				str_Result= " where cateid<>"&w3&" and (','+share+',' like '%"&","&w3&","&"%' or share='1') and del=1"
'elseif H="81" then
			end if
			Dim baseSql
			Dim open_3_21
			open_3_21 = (conn.execute("select 1 from power where ord = "& session("personzbintel2007") & " and sort1 = 3 and sort2= 21 and qx_open = 1 ").eof = False )
			Dim open_3_22
			open_3_22 = (conn.execute("select 1 from power where ord = "& session("personzbintel2007") & " and sort1 = 3 and sort2= 22 and qx_open = 1 ").eof = False )
			Dim Export_Fields,CantExport_Fields(255),cfidx,fields,sql
			cfidx=0
			fields = "项目主题,项目编号,客户名称,币种,"& app.iif(open_3_21 = True ,"项目总额,项目优惠金额,优惠后总额,","") & "预计费用,"&_
			"发现日期,预计日期,项目阶段,项目流程,项目状态,项目来源,项目分类,项目地区,项目概要,"&_
			"洽谈进展,添加人,主负责人,协作人,创建时间"
			Dim zdyFields(),j
			ReDim zdyFields(1,0)
			set rs=server.CreateObject("adodb.recordset")
			rs.open "select id,title,name,sort,gl,set_open,dc from zdy where sort1=3 order by gate1 asc ",conn,1,1
			j=0
			do while rs.eof=False
				ReDim Preserve zdyFields(1,j)
				zdyFields(0,j) = rs("name")
				zdyFields(1,j) = rs("title")
				fields=fields&"," & zdyFields(0,j)
				If rs("set_open") <> 1 Or rs("dc") <> 1 Then
					CantExport_Fields(cfidx) = zdyFields(0,j) : cfidx=cfidx+1
'If rs("set_open") <> 1 Or rs("dc") <> 1 Then
				end if
				rs.movenext
				j=j+1
'If rs("set_open") <> 1 Or rs("dc") <> 1 Then
			...
			rs.close
			set rs=Nothing
			if ZBRuntime.MC(12003) <> True Then
				CantExport_Fields(cfidx) = "洽谈进展" : cfidx = cfidx + 1
'if ZBRuntime.MC(12003) <> True Then
			end if
			Dim exZdyFields(),exJoinTables,exZdy1,exZdy2
			ReDim exZdyFields(1,0)
			sql="select ID,FName from ERP_CustomFields where isUsing=1 and canexport=1 and tname=3 and del=1 order by id"
			j=0
			...
			While rs.eof = False And ZBRuntime.MC(207102)
				...
				exZdy2 = exZdy2 & "null,"
				exJoinTables = exJoinTables & vbcrlf & " left join ERP_CustomValues extb"&j&" on extb"&j&".OrderID=a.ord and extb"&j&".FieldsID=" & rs(0)
			...
			rs.close
			Set rs=Nothing
			Dim mxzdyFields(),sorce
			sql = "SELECT name,title,sorce,set_open FROM zdymx WHERE sort1 = 3 "& app.iif(open_3_21 = False ," and sorce not in (5,7) ","") & app.iif(open_3_22 = False ," and sorce not in (16,17) ","") &" ORDER BY gate1"
			Set rs = cn.execute(sql)
			j = 0
			ReDim mxzdyFields(1,0)
			While rs.eof = False
				sorce = rs("sorce")
				ReDim Preserve mxzdyFields(1,j)
				mxzdyFields(0,j) = "P"&sorce
				mxzdyFields(1,j) = rs("title")
				fields=fields&"," & mxzdyFields(0,j)
				If rs("set_open") <> 1 Then
					CantExport_Fields(cfidx) = mxzdyFields(0,j) : cfidx=cfidx+1
'If rs("set_open") <> 1 Then
				end if
				rs.movenext
				j = j + 1
'If rs("set_open") <> 1 Then
			wend
			baseSql = " exec erp_xls_xm '"& replace(Str_Result,"'","''") &"','"& replace(px_Result,"'","''") &"','"& replace(exZdy1,"'","''") &"','"& replace(exZdy2,"'","''") &"','"& replace(exJoinTables,"'","''") &"','"& replace(fields,"'","''")       &"' "
			Dim adapter
			Dim sqls,sheetSetting
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "项目资料"
			adapter.canSplitFormula_And = "canSplit(rs)"
			Dim hd,fds,i,fd,sname
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = baseSql
			sheetSetting.title = "项目资料"
			fds = Split(fields,",")
			For i = 0 To ubound(fds)
				Set hd = New HeaderColumnSetting
				hd.dbname = fds(i)
				hd.dbtype = "str"
				sname = fShowName(fds(i))
				Select Case sname
				Case "项目总额","项目优惠金额","优惠后总额","预计费用","总价","建议总价":
				hd.dbtype = "money"
				hd.width = EX_WIDTH_DIGIT
				hd.ignoreNonnumeric = True
				hd.excelAlign = "right"
				case "单价"
				hd.dbtype = "salesprice"
				hd.width = EX_WIDTH_DIGIT
				hd.ignoreNonnumeric = True
				hd.excelAlign = "right"
				case "建议进价"
				...
				hd.width = 60
				hd.excelAlign = "center"
				Case "项目主题","项目编号","客户名称","zdy1","zdy2","zdy3","zdy4","产品名称","产品编号","产品型号","明细自定义1","明细自定义2","明细自定义3","明细自定义4":
				hd.width = EX_WIDTH_TEXT
				If sname = "客户名称" Then
					hd.Formula = "getTelNames(cn,@value)"
				end if
				Case "发现日期","预计日期","到货日期":
				...
				Case "协作人":
				hd.width = EX_WIDTH_PERSON
				hd.Formula = "getSharePersons(cn,@value)"
				Case "项目阶段","项目状态","项目来源","项目分类","项目地区","zdy5","zdy6","单位","明细自定义5","明细自定义6":
				...
				hd.excelAlign = "left"
				End Select
				If InStr(sname,"单价")>0 Then hd.canSum = False
				If InStr(sname,"建议进价")>0 Then hd.canSum = False
			...
			For i=0 To ubound(zdyFields,2)
				...
				If zdyFields(1,i) = "" Then
					zdyFields(1,i) = "未命名" & noNameIdx
			...
			For i=0 To ubound(exZdyFields,2)
				Set hd = New HeaderColumnSetting
				If Not IsEmpty(exZdyFields(0,i)) Then
					hd.dbname = exZdyFields(0,i)
					If exZdyFields(1,i) = "" Then
						exZdyFields(1,i) = "未命名" & noNameIdx
						noNameIdx=noNameIdx+1
						exZdyFields(1,i) = "未命名" & noNameIdx
					end if
			...
			next
			For i=0 To ubound(mxzdyFields,2)
				Set hd = New HeaderColumnSetting
				hd.dbname = mxzdyFields(0,i)
				If mxzdyFields(1,i) = "" Then
					mxzdyFields(1,i) = "未命名" & noNameIdx
					noNameIdx=noNameIdx+1
					mxzdyFields(1,i) = "未命名" & noNameIdx
				end if
				hd.title = mxzdyFields(1,i)
				sheetSetting.headerSettings.add(hd)
			next
		...
		Function canSplit(rs)
			canSplit = instr(rs("项目编号")&"","【产品明细】")<=0
		end function
		Function getSharePersons(ByVal cn,persons)
			getSharePersons = ""
			persons = Replace(persons," ","")
			If persons = "" Then Exit Function
			If isEmpty(allPersons) Then
				Dim rs
				Set rs=server.CreateObject("adodb.recordset")
				rs.open "select ord,name from gate",cn,1,1
				If rs.eof = False Then allPersons = rs.getRows()
				rs.close
				Set rs=Nothing
			end if
			If isEmpty(allPersons) = False Then
				Dim i
				For i=0 To ubound(allPersons,2)
					If InStr(","&persons&",","," & allPersons(0,i) & ",") > 0 Then
						If getSharePersons = "" Then
							getSharePersons = allPersons(1,i)
						else
							getSharePersons = getSharePersons & " " & allPersons(1,i)
						end if
					end if
				next
			end if
		end function
		Function getTelNames(ByVal cn,company)
			getTelNames = company
			If len(company&"") = 0 Then Exit Function
			if left(company&"",1)="," then company = "0"& company
			if right(company&"",1)="," then company = company&"0"
			Dim rs,sql
			sql = "select (case when isnull(tl.del,2)=2 then '客户已被删除' when ( "& vbcrlf &_
			...
			") then tl.name else '' end) as name from tel tl where ord in ("&sdk.FormatNumList(company)&")"
			Set rs = cn.execute(sql)
			If rs.eof = False Then
				getTelNames = ""
				While rs.eof=False
					If Len(rs("name").value)>0 And Len(getTelNames)>0 Then getTelNames = getTelNames &"|"
					getTelNames = getTelNames & rs("name").value
					rs.movenext
				wend
				If Right(getTelNames,1) = "|" Then getTelNames = Left(getTelNames,Len(getTelNames)-1)
				getTelNames = getTelNames & rs("name").value
			else
				getTelNames = "客户不存在或者已被删除"
			end if
			rs.close
			Set rs=Nothing
		end function
		Function fShowName(fname)
			Select Case fname
			Case "P1":fShowName="产品名称"
			Case "P2":fShowName="产品编号"
			Case "P3":fShowName="产品型号"
			Case "P4":fShowName="单位"
			Case "P5":fShowName="单价"
			Case "P6":fShowName="数量"
			Case "P7":fShowName="总价"
			Case "P8":fShowName="到货日期"
			Case "P9":fShowName="备注"
			Case "P10":fShowName="明细自定义1"
			Case "P11":fShowName="明细自定义2"
			Case "P12":fShowName="明细自定义3"
			Case "P13":fShowName="明细自定义4"
			Case "P14":fShowName="明细自定义5"
			Case "P15":fShowName="明细自定义6"
			Case "P16":fShowName="建议进价"
			Case "P17":fShowName="建议总价"
			Case Else : fShowName = fname
			End Select
		end function
		Function fDBName(fname)
			Select Case fname
			Case "产品名称":fDBName="P1"
			Case "产品编号":fDBName="P2"
			Case "产品型号":fDBName="P3"
			Case "单位":fDBName="P4"
			Case "单价":fDBName="P5"
			Case "数量":fDBName="P6"
			Case "总价":fDBName="P7"
			Case "到货日期":fDBName="P8"
			Case "备注":fDBName="P9"
			Case "明细自定义1":fDBName="P10"
			Case "明细自定义2":fDBName="P11"
			Case "明细自定义3":fDBName="P12"
			Case "明细自定义4":fDBName="P13"
			Case "明细自定义5":fDBName="P14"
			Case "明细自定义6":fDBName="P15"
			Case "建议进价":fDBName="P16"
			Case "建议总价":fDBName="P17"
			Case Else : fDBName = fname
			End Select
		end function
