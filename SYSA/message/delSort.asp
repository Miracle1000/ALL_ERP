...
'If Not rs.Eof Then
	...
	if ord="" or ord=0 then
		...
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from sortMessage where ord="&ord&" and ord not in (select sort from mobanMessage where del=1)"
		rs.open sql,conn,1,1
		if not rs.eof then
			sqltext="delete from sortMessage  where  ord="&ord&" and ord not in (select sort from mobanMessage where del=1)"
			conn.execute(sqltext)
			Response.write"<script language=javascript>alert('删除成功！');window.location.href='editSort.asp?currpage="&currpage&"';</script>"
			call db_close : Response.end
		else
			Response.write"<script language=javascript>alert('该分类下有数据！'); history.back();</script>"
			call db_close : Response.end
		end if
		rs.close
		set rs=nothing
