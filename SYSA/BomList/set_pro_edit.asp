...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "<style>#comm_itembarbg{height:43px!important;}</style>" & vbcrlf & ""
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		...
		If edit = 0 Then
			...
			bill.sql = "select * from bom_proName where ord = " & bill.id
		end if
		bill.Groups(0).title = title
		Dim pTitle, options, i
		pTitle = ""
		With Bill.fields.add("虚拟名称", "name")
		.colspan = 3
		.uitype = "text"
		.notnull = True
		.maxlimit = 25
	...
	Function Bill_OnSave(ByVal bill)
		Dim ord, name, gate1,continue_add, rs, sql, aTitle
		ord = bill.id
		name = app.gettext("name")
		gate1 = app.getint("gate1")
		continue_add = app.getint("sys_ad_model")
		If ord > 0 Then
			aTitle = "虚拟名称修改"
			Set rs = cn.execute("select * from Bom_ProName where name = '" & Replace(name,"'","''") & "' and ord <> " & ord)
			If rs.eof = False Then
				bill.alert("虚拟名称已存在！")
				Bill_OnSave = False
				Response.end()
			...
			aTitle = "虚拟名称添加"
			Set rs = cn.execute("select * from Bom_ProName where name = '" & Replace(name,"'","''") & "'")
			If rs.eof = False Then
				bill.alert("虚拟名称已存在！")
				Bill_OnSave = False
				Response.end()
		...
		Set rs = server.CreateObject("adodb.recordset")
		sql = "select * from Bom_ProName where ord = " & ord
		rs.open sql,cn,3,3
		If rs.eof = True Then
			rs.addnew
		end if
		rs("name").value = name
		rs("gate1").value = gate1
		...
		If continue_add = "0" Then
			...
			Response.write("<script type='text/javascript'>parent.window.location.href='?'</script>")
		end if
		Bill_OnSave = true
	end function
