...
'if m2="" then
	...
	p1=dateadd("d",-(weekday(m1)-1),m1)
'if m2="" then
	p2=dateadd("d",7-weekday(m2),m2)
'if m2="" then
	x=datediff("d",p1,p2)+1
'if m2="" then
	m = x \ 7
	if B>0 then
...
'if B>0 then
	...
	if m>30 then
		Response.write "<script>alert('显示数据最多不能超过30天');window.location.href='../tongji/cash_w.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	...
	if intCols1=0 or intCols2=0 then
		Response.write "<script>alert('收入项与支出项至少各保留一项');window.location.href='../tongji/cash_w.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
...
'do while not rs.eof
				...
				For k=1 to m
					if k=1 then
						d=weekday(m1)
						date1=m1
						date2=dateadd("d",7-d,date1)
						date1=m1
					else
						date1=dateadd("d",1,date2)
						date2=dateadd("ww",1,date1)-1
						date1=dateadd("d",1,date2)
					end if
...
'For Each key In colsTotal
			...
			doSave("现金流量每周统计")
				...
				Response.write """><font class=""red""><strong><u>下载导出的现金流量每周统计</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
