...
'Private Sub Class_Initialize
	...
	Randomize
	if request("sorttype") = "0" then
		Response.write"<script language=javascript>alert('请选择分类！');history.back()</script>"
		call db_close : Response.end
	end if
...
'if html="1" then
	...
	sql="select qx_open,w3 from power2 where sort1=2 and cateid="& session("personzbintel2007")
	set rs=conn.execute(sql)
	if not rs.eof then
		qx_open=rs("qx_open")
		pw3=rs("w3")
	...
	if member1="0" then
		...
		if W3&""="0" or W3&""="" then W3 = session("personzbintel2007")
	end if
	if request("edit")="" then
		sql = "select  * from learnhd where 1=0"
		Set rs = server.CreateObject("adodb.recordset")
		rs.Open sql,conn,3,2
		rs.addnew
		rs("date7")=now()
	else
		sql = "select * from learnhd where id="&request("id")
		Set rs = server.CreateObject("adodb.recordset")
	...
	rs.update
	rs.close
	ord = GetIdentity("learnhd","ord","cateid","")
	Dim helper
	...
	conn.close
	set conn=nothing
	Response.redirect "edit.asp?sort="&sort1
