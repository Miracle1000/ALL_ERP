...
'Dim s, c, i , rnds, item
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css""> " & vbcrlf & "<style>" & vbcrlf & ".IE5 .page input.anybutton2," & vbcrlf & ".IE5 .top_btns input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src=""../inc/popup_layer.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/ti_kh_jf1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ ></script>" & vbcrlf & "</head>" & vbcrlf & "<body bgcolor=""#ebebeb"" oncontextmenu=self.event.returnValue=false  onMouseOver=""window.status='none';return true;"" >" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=1 and sort2=15"
	rs1.open sql1,conn,3,1
	if rs1.eof then
		open_1_15=0
		intro_1_15=0
	else
		open_1_15=rs1("qx_open")
		intro_1_15=rs1("qx_intro")
	end if
	rs1.close
	...
	rs1.close
	Dim qxintro_2_1, qxintro_2_14, qxintro_2_6
	Dim open_67_13, open_67_19
	sql1="select isnull(qx_open,0) qx_open,replace(isnull(cast(qx_intro as varchar(4000)),'0'),' ','') qx_intro, isnull(sort1,0) sort1, isnull(sort2,0) sort2 from power  where ord="&session("personzbintel2007")&" and sort1 in(2,67) and sort2 in(1,6,13,14,19)"
	rs1.open sql1,conn,1,1
	While rs1.eof = False
		qxsort1 = rs1("sort1") : qxsort2 = rs1("sort2") : qx_open = rs1("qx_open") : qx_intro=rs1("qx_intro")
		Select Case qxsort1
		Case 2
		Select Case qxsort2
		Case 1
		open_2_1 = qx_open : intro_2_1=qx_intro
		Case 6
		open_2_6 = qx_open : intro_2_6=qx_intro
		Case 14
		open_2_14 = qx_open : intro_2_14=qx_intro
		End Select
		Case 67
		Select Case qxsort2
		Case 13 : open_67_13 = qx_open
		Case 19 : open_67_19 = qx_open
		End Select
		End Select
		rs1.movenext
	wend
	...
	if open_1_15=3 then
		list=""
	elseif open_1_15=1 then
		list="and a.cateid in ("&intro_1_15&")"
	else
		list="and a.cateid=0"
	end if
	dim rs,sql,Str_Result,Str_Result2
	Str_Result="where del=1 and sort3=1 "&list&" "
	Str_Result2="and a.sort3=1 "&list&" "
	searchtype=request("searchtype")
	C=request("C")
	If searchtype<>"" And C<>"" Then
		If searchtype="0" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.name)>0 "
		ElseIf searchtype="1" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.pym)>0 "
		ElseIf searchtype="2" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.khid)>0 "
		ElseIf searchtype="3" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.phone)>0 "
		ElseIf searchtype="4" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.fax)>0 "
		ElseIf searchtype="5" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.address)>0 "
		ElseIf searchtype="6" Then
			Str_Result2 = Str_Result2 & " and charindex('"&C&"',a.intro)>0 "
		ElseIf searchtype="7" Then
			Str_Result2 = Str_Result2 & " and a.person in(select ord from person where del=1 and name like '%"&C&"%') "
		ElseIf searchtype="8" Then
			Str_Result2 = Str_Result2 & " and a.person in(select ord from person where del=1 and phone like '%"&C&"%') "
		ElseIf searchtype="9" Then
			Str_Result2 = Str_Result2 & " and a.person in(select ord from person where del=1 and mobile like '%"&C&"%') "
		end if
	else
		searchtype="0"
	end if
	...
	if px=1 then
		px_Result="order by a.khid desc, a.jf desc, a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
		px_Result2 = "order by a.khid , a.jf , a.sum_jf ,a.sum_jf2 ,a.ord "
	elseif px=2 then
		px_Result="order by a.khid, a.jf,a.sum_jf ,a.sum_jf2 ,a.ord  "
		px_Result2 = "order by a.khid desc, a.jf desc, a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
	elseif px=3 then
		px_Result="order by a.name desc, a.jf desc,a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
		px_Result2 = "order by a.name , a.jf,a.sum_jf ,a.sum_jf2 ,a.ord  "
	elseif px=4 then
		px_Result="order by a.name , a.jf,a.sum_jf ,a.sum_jf2 ,a.ord  "
		px_Result2 = "order by a.name desc, a.jf desc,a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
	elseif px=5 then
		px_Result="order by a.sum_jf desc, a.jf desc,a.sum_jf2 desc,a.ord desc "
		px_Result2 = "order by a.sum_jf , a.jf ,a.sum_jf2 ,a.ord "
	elseif px=6 then
		px_Result="order by a.sum_jf , a.jf ,a.sum_jf2 ,a.ord "
		px_Result2 ="order by a.sum_jf desc, a.jf desc,a.sum_jf2 desc,a.ord desc "
	elseif px=7 then
		px_Result="order by a.jf desc,a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
		px_Result2="order by a.jf ,a.sum_jf ,a.sum_jf2 ,a.ord "
	elseif px=8 then
		px_Result="order by a.jf ,a.sum_jf ,a.sum_jf2 ,a.ord "
		px_Result2="order by a.jf desc,a.sum_jf desc,a.sum_jf2 desc,a.ord desc "
	elseif px=9 then
		px_Result="order by a.sum_jf2 desc, a.jf desc,a.sum_jf desc,a.ord desc "
		px_Result2="order by a.sum_jf2 , a.jf,a.sum_jf ,a.ord  "
	elseif px=10 then
		px_Result="order by a.sum_jf2 , a.jf,a.sum_jf ,a.ord  "
		px_Result2="order by a.sum_jf2 desc, a.jf desc,a.sum_jf desc,a.ord desc "
	end if
	...
	rCount=conn.execute("select count(*) from tel a where a.del=1 and isnull(a.sp,0)=0  " &Str_Result2)(0)
	if rCount=0 then
		PageCount=1
		bookNum=PageCount
	else
		if rCount mod page_count=0 then
			PageCount=rCount\page_count
		else
			PageCount=rCount\page_count+1
			PageCount=rCount\page_count
		end if
		bookNum=PageCount
	end if
	...
	rs.close
	UURL=ReturnUrl()
	Response.write "" & vbcrlf & "             <table class=""resetTableBg"" width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""208"" class=""place"">客户积分排行" & vbcrlf & "            </td>" & vbcrlf & "         <td>&nbsp;<a href=""#"" class=""tableTitleLinks"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a>" & vbcrlf & "<script src= ""../Script/ti_kh_jf1_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:160px; height:400;display:none;"">" & vbcrlf & "<table width=""150"" height=""260""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">"& vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按客户编号排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按客户编号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按客户名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按客户名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><fontcolor=""#2F496E"">按积分总数排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按积分总数排序(升)</font></a> </td>" & vbcrlf & "     </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按剩余积分排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按剩余积分排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=9')""><font color=""#2F496E"">按消费积分排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=10')""><font color=""#2F496E"">按消费积分排序(升)</font></a> </td>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div></td>" & vbcrlf & "            <td><div align=""right"">" & vbcrlf & "              "
	if open_1_10=1 or open_1_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""EXCEL导出"" onClick=""if(confirm('确定要导出吗？')){exportExcel({debug:false,page:'../out/xls_jf2.asp'});}"" class=""anybutton2""/>"
	end if
	if open_1_7=3 or  CheckPurview(intro_1_7,trim(cateid))=True then
		Response.write "<input type=""button"" name=""Submit43"" value=""打印""  onClick=""window.print()""  class=""anybutton""/>"
	end if
	Response.write "" & vbcrlf & "            </div></td>" & vbcrlf & "            <td width=""3""><img class=""resetElementHidden"" src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>  " & vbcrlf & "" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "   <form method=""get"" action=""kh_jf1.asp"" name=""form1"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""resetHeadBg resetBorderColor"" background=""../images/112.gif"" height=""30"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "                 <input type=""hidden"" name=""px"" value="""
	Response.write px
	Response.write """>" & vbcrlf & "                                <select name=""searchtype"">" & vbcrlf & "                                <option value="""" "
	if searchtype="" then
		Response.write "selected"
	end if
	Response.write ">检索条件</option>" & vbcrlf & "                           <option value=""0"" "
	if searchtype="0" then
		Response.write "selected"
	end if
	Response.write ">"
	Response.write arrName(1)
	Response.write "</option>" & vbcrlf & "                            <option value=""1"" "
	...
	Response.write ">联系人姓名</option>" & vbcrlf & "                         "
	If arrShow(19)=1 Then
		Response.write "" & vbcrlf & "                                     <option value=""8"" "
		if searchtype="8" then
			Response.write "selected"
		end if
		Response.write ">联系人电话</option>" & vbcrlf & "                         "
	end if
	If arrShow(20)=1 Then
		Response.write "" & vbcrlf & "                                     <option value=""9"" "
		if searchtype="9" then
			Response.write "selected"
		end if
		Response.write ">联系人手机</option>" & vbcrlf & "                         "
	end if
	...
	Response.write C
	Response.write """/>" & vbcrlf & "        <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                " & vbcrlf & "          </td>" & vbcrlf & "        </tr>" & vbcrlf & "             </form>" & vbcrlf & "      </table>" & vbcrlf & "" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "       <tr class=""top"">" & vbcrlf & "   <td width=""6%""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "         <td width=""6%""><div align=""center"">排名</div></td>" & vbcrlf & "   <td align=""center"" width=""11%""><div align=""center"">"
	Response.write arrName(3)
	Response.write "</div></td>" & vbcrlf & "    <td width=""22%"" align=""center"">" & vbcrlf & "          <div align=""center"">" & vbcrlf & "            "
	Response.write arrName(1)
	Response.write "" & vbcrlf & "          </div></td>" & vbcrlf & "          <td align=""center"" width=""17%""><div align=""center"">主联系人</div></td>" & vbcrlf & "        <td width=""14%"">" & vbcrlf & "            <div align=""center"">积分总数</div></td>" & vbcrlf & "         <td width=""10%"">" & vbcrlf & "            <div align=""center"">剩余积分        </div></td> " & vbcrlf & "        <td width=""13%""><div align=""center"">消费积分</div></td>" & vbcrlf & "     <td width=""22%"" >" & vbcrlf & "           <div align=""center"">" & vbcrlf & "        <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){window.open(this.value,'_self');}this.selectedIndex=0;"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "              <option value=""kh_jf1.asp?page_count=10&CurrPage="
	'Response.write arrName(1)
	...
	Response.write ">每页显示10条</option>" & vbcrlf & "              <option value=""kh_jf1.asp?page_count=20&CurrPage="
	Response.write CurrPage
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">每页显示20条</option>" & vbcrlf & "                     <option value=""kh_jf1.asp?page_count=30&CurrPage="
	Response.write CurrPage
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">每页显示30条</option>" & vbcrlf & "                     <option value=""kh_jf1.asp?page_count=50&CurrPage="
	Response.write CurrPage
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">每页显示50条</option>" & vbcrlf & "                      <option value=""kh_jf1.asp?page_count=100&CurrPage="
	Response.write CurrPage
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">每页显示100条</option>" & vbcrlf & "                     <option value=""kh_jf1.asp?page_count=200&CurrPage="
	Response.write CurrPage
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	dim khid,ord,cateid,sum_jf,sum_jf2
	if currpage=pagecount then
		intcount=rCount-(pagecount-1)*page_count
'if currpage=pagecount then
	else
		intcount=page_count
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql = " select * from (select top "& intcount &" * from (select top " & (CurrPage * page_count) & " a.ord,a.khid,a.name,a.cateid,a.jf,isnull(sum(b.jf),0) as sum_jf,isnull(sum(c.num1),0) as sum_jf2,isnull(a.person,0) person from tel a WITH(NOLOCK) left join (select x.company,SUM(x.jf) jf from teljf x WITH(NOLOCK) group by x.company ) b on a.ord=b.company left join (select x.company,SUM(num1) num1 from teljf2 x WITH(NOLOCK) group by x.company) c on a.ord=c.company where a.del=1 and isnull(a.sp,0)=0 "&str_Result2 &" group by a.ord,a.khid,a.name,a.cateid,a.jf,a.person "& Replace(px_Result,"a.sum_","sum_") &")  d "& Replace(px_Result2,"a.","d.") &" ) y "& Replace(px_Result,"a.","y.") &""
	rs.open sql,conn,3,1
	if rs.eof then
		...
		do until rs.eof
			...
			Response.write ord
			Response.write """></td>" & vbcrlf & "     <td class=""name"" align=""center"">"
			Response.write page_count*(currpage-1)+i
			Response.write """></td>" & vbcrlf & "     <td class=""name"" align=""center"">"
			Response.write "</td>" & vbcrlf & "          <td align=""center"">"
			Response.write khid
			Response.write "</td> " & vbcrlf & "                <td>" & vbcrlf & "                           "
			if open_1_1=3 or  CheckPurview(intro_1_1,trim(cateid))=True  then
				if open_1_14=3 or (open_1_14=1 and CheckPurview(intro_1_14,trim(cateid))=True  And cateid<>0) then
					...
					Response.write "&pn=2','newwin','width=' + 980 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,status=no,resizable=1,location=no,menubar=no,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此客户详情"">"
					Response.write pwurl(ord)
					Response.write k
			...
			If person > 0 Then
				personName = "" : lxrCateid = 0 : phone2 = "" : mobile2 = "" : lxrPhone = "" : lxrmobile = ""
				Set rs2 = conn.execute("select name,cateid, phone, mobile from person where del=1 and ord="& person)
				If rs2.eof = False Then
					personName = rs2("name"): lxrCateid = rs2("cateid") : phone2 = rs2("phone") : mobile2 = rs2("mobile")
				end if
				rs2.close
				Set rs2 = Nothing
				if open_2_1=3 or  (CheckPurview(intro_2_1,trim(cateid))=True And cateid<>0)  Then
					if open_2_14=3 or (open_2_14=1 and CheckPurview(intro_2_14,trim(cateid))=True  And cateid<>0) Then
						Response.write "" & vbcrlf & "                                             <a href=""#"" onclick=""javascript:window.open('../person/content.asp?ord="
						Response.write pwurl(person)
						Response.write "&pn=2','newlxrwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此联系人详情"">"
						Response.write pwurl(person)
					...
					Response.write "<br>"
					If phone2&""<>"" Then
						lxrPhone = GetPhoneNumber(phone2, cateid)
						If lxrPhone&""<>"" Then
							Response.write lxrPhone & "<br>"
						end if
					end if
					If mobile2&""<>"" Then
						lxrmobile = GetPhoneNumber(mobile2, cateid)
						If lxrmobile&""<>"" Then
							Response.write lxrmobile
						end if
					end if
					If phone2&""<>"" Then
						if ZBRuntime.MC(32000) and tp=1 Then
							Response.write " <img src='../images/15.gif' onClick=""getCall('../china/bh.asp?ord="&ord&"&ordtype=0&phone="& phone2 &"')"" style='cursor:hand' border='0' align='absbottom'>"
						end if
					end if
					if ZBRuntime.MC(32000) And  mobile2<>"" and tp=1 Then Response.write " <img src='../images/sj.gif' onClick=""getCall('../china/bh.asp?ord="&ord&"&ordtype=1&phone="& mobile2 &"')"" style='cursor:hand' border='0' align='absbottom'>"
					if open_67_13=1 and open_67_19<>1   and isMobile(trim(mobile2)) Then
						If IsPhonePower(cateid) Then
							mobileNum = trim(mobile2)
							from = "normal"
						else
							mobileNum = HexEncode(trim(mobile2))
							from = "encrypt"
						end if
			...
			Response.write "</td>" & vbcrlf & "                <td  class=""name"" align=""center""><div align=""center""><a href=""#"" onclick=""javascript:window.open('list_jf.asp?id="
			Response.write pwurl(ord)
			Response.write "&ret="
			Response.write m1
			Response.write "&ret2="
			...
			'Response.write m2
			Response.write sum_jf
			Response.write "</a></div></td>" & vbcrlf & "                <td class=""red tableLinks""><div align=""center"">"
			Response.write rs("jf")
			Response.write "</div></td>" & vbcrlf & "                <td class=""name""><div align=""center""><a href=""#"" onclick=""javascript:window.open('list_jf2.asp?id="
			Response.write pwurl(ord)
			Response.write "&ret="
			Response.write m1
			Response.write "&ret2="
			...
			Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">"
			'Response.write m2
			Response.write sum_jf2
			Response.write "</a></div></td> " & vbcrlf & "                <td class=""func"">                 " & vbcrlf & "                  <div align=""center"">"
			if open_1_14=3 or (open_1_14=1 And CheckPurview(intro_1_14,trim(cateid))=True  And cateid<>0 )  then
				Response.write "<input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('../work/content.asp?ord="
				Response.write pwurl(ord)
				Response.write "&pn=2','newwin','width=' + 980 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')""/> "
				'Response.write pwurl(ord)
			end if
			if open_1_15=3 or  CheckPurview(intro_1_15,trim(cateid))=True  then
				Response.write "" & vbcrlf & " <input type=""button"" name=""Submit3c"" value=""清零""  onClick=""javascript:window.open('set_jf.asp?ord="
				Response.write pwurl(ord)
				Response.write "','newwincor','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & ""
				'Response.write pwurl(ord)
			end if
			...
			rs.movenext
		loop
		Response.write " " & vbcrlf & "      </table>" & vbcrlf & "</td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "      <td  class=""page"">" & vbcrlf & "     <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""6%"" height=""30""><div align=""center"">全选" &vbcrlf & "            <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onClick=""SelectAll()"" />" & vbcrlf & "          </div></td>" & vbcrlf & "    <td  width=""10%"">" & vbcrlf & "                 <input type=""button"" onClick=""if(confirm('您确定要执行批量删除积分吗?'))batdel();"" name=""Submit2"" value=""批量清零""  class=""anybutton2""/>" & vbcrlf & "                        </td>" & vbcrlf & "    <td  width=""26%"">  </td>" & vbcrlf & "             <form   name=""form2""   method=""post""   action=""kh_jf1.asp?W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""anybutton2"" onClick=""window.location.href='kh_jf1.asp?currPage="
			Response.write  1
			Response.write "&W1="
			Response.write W1
			Response.write "&W2="
			...
			Response.write "&ret2="
			Response.write m2
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上页""  onClick=""window.location.href='kh_jf1.asp?currPage="
		...
		if currpage=pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下页""  onClick=""window.location.href='kh_jf1.asp?currPage="
			Response.write  currpage + 1
			Response.write "&W1="
			Response.write W1
			Response.write "&W2="
			...
			Response.write "&ret2="
			Response.write m2
			Response.write "'"" class=""anybutton2""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='kh_jf1.asp?currPage="
			Response.write  PageCount
			Response.write "&W1="
			Response.write W1
			Response.write "&W2="
		...
		Response.write "" & vbcrlf & "    </div></td></form>" & vbcrlf & "  </tr>" & vbcrlf & "<tr>" & vbcrlf & "    <td height=""338"" colspan=""3"">&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "<iframe src="""" id=""main"" name=""main"" style='height:1px;width:1px'></iframe>" & vbcrlf & " "
	end if
	rs.close
	set rs=nothing
	action1="客户积分排行"
