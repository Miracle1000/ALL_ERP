...
'Set rs = conn.execute("SELECT top 1 1 FROM (SELECT id,CAST(short_str AS INT) cateid FROM dbo.split('"& intro_33_3 &"',',')) a INNER JOIN (select distinct s.cateid from send s inner join kuout k on k.ord=s.kuout and s.del=1 and k.sort1 in(1,4) and k.order1 in("& htords &")) b on a.cateid=b.cateid AND a.cateid>0")
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write "' defer='defer'></script>" & vbcrlf & "</head>" & vbcrlf & "<body onresize=""try{fixPositions()}catch(e1){}"" oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">合同回收站</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=contract&url=contract.asp&list1=contractlist'}""/></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf &        "   </tr> "& vbcrlf &      " </table>"  & vbcrlf &     "   <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-left:20px"">" & vbcrlf &     "  <tr class=""top"">" & vbcrlf &   "  <td align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "       <td align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "   <td height=""27"" ><div align=""center""><strong>合同</strong></div></td>" & vbcrlf & "       <td ><div align=""center""><strong>负责人</strong></div></td>" & vbcrlf & "     <td width=""14%"" ><div align=""center""><strong>删除人</strong></div>            <div align=""center""></div></td>" & vbcrlf & "         <td ><div align=""center""><strong>删除日期</strong></div></td>" & vbcrlf & "     <td width=""20%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "      </tr>" & vbcrlf & ""
	noDel = Request("noDel")&""
	if noDel&""<>"" then noDel = replace(noDel&""," ","")
	dim n, delDisableStr,restDisable
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from contract WITH(NOLOCK) where del=2 order by deldate desc"
	...
	if rs.RecordCount<=0 then
		...
		If CurrPage&"" = "" Then CurrPage = 0 Else CurrPage = CDBL(CurrPage)
		...
		Response.write "" & vbcrlf & "       <form name=""form1"" method=""post"" action=""deletehtall.asp?ord="
		...
		do until rs.eof
			...
			Response.write "</td>" & vbcrlf & "      <td width=""24%"" height=""27"" class=""name"" >&nbsp;<a href=""#"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?ord="
			Response.write pwurl(htord)
			Response.write "&view=details','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;"" title="""
			Response.write pwurl(htord)
			Response.write rs("title")
			Response.write """>" & vbcrlf & "        "
			Response.write""&k&""
			Response.write "</a><span id=""ht"
			Response.write htord
			Response.write """ class=""red"" style=""padding-left:5px;"">"
			Response.write htord
			if instr(","& noDel &",", ","& htord &",")>0 then Response.write "此合同不允许删除"
			...
			set rs7=server.CreateObject("adodb.recordset")
			ExistSql="select top 1 1 from contractlist where contract<>"&htord&" and pricelist>0 and isnull(del,1)<>2 and  pricelist in ( "&_
			"select pricelist from contractlist  "&_
			"where  contract="&htord&" and del=2 "&_
			") "
			rs7.open ExistSql,conn,1,1
			If not rs7.eof then
				restDisable=" disabled"
			end if
			rs7.close
			set rs7=Nothing
			Response.write "" & vbcrlf & "        <td  class=""name""><div align=""center"">"
			Response.write delcate
			Response.write "</div></td> " & vbcrlf & "         <td width=""13%"" ><div align=""center""><font class=""gray"">"
			Response.write rs("deldate")
			Response.write "</font></div></td>" & vbcrlf & "             <td> " & vbcrlf & "                      <div align=""center"">" & vbcrlf & "                        <input type=""button"" name=""Submit3c""  class=""anybutton"" "
			Response.write restDisable
			Response.write "  value=""恢复""  onClick=""recovery("
			Response.write htord
			Response.write ")""/>" & vbcrlf & "                   <input type=""button"" name=""Submitdel"" class=""anybutton"" "
			Response.write delDisableStr
			Response.write " value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deleteht.asp?ord="
			Response.write htord
			Response.write "&CurrPage="
			Response.write CurrPage
		...
		loop
		m=n
		Response.write "       " & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "          </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr class=""bgfff"">" & vbcrlf & "    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""checkbox2"" value=""Check All"" onClick=""mm()"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >" & vbcrlf & "           <input type=""submit"" name=""Submit426"" value=""批量删除""   onClick=""return test();""  class=""anybutton2"">" &vbcrlf & "        <input type=""button"" name=""Submit426"" value=""批量恢复""  onclick=""ask2();"" class=""anybutton2""/>" & vbcrlf & "    </td>" & vbcrlf & "    <td width=""69%""><div align=""right"">" & vbcrlf & "    "
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='contract.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='contract.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='contract.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='contract.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""sethtall.asp?currPage="
		Response.write currPage
		Response.write """; window.currpage="""
		Response.write currPage
		Response.write """ ;</script>" & vbcrlf & "<script src='../script/bk_contract.js?ver="
		Response.write Application("sys.info.jsver")
