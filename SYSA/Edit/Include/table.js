...
window.selectedTD
window.selectedTR
window.selectedTBODY
window.selectedTable
...
function isTableSelected() {
	var oSession = GetCurrSession();
	if (oSession.type == "Control" || !window.ActiveXObject) {
		var oControlRange = oSession.createRange();
		if(!oControlRange[0] && oControlRange && oControlRange.parentElement && oControlRange.parentElement()) {
			var obj = oControlRange.parentElement();
			if(!window.ActiveXObject) {
				if(obj){
					if(obj.tagName!="TD") {
						var obj = obj.parentElement;
						if(obj && obj.tagName=="TD") {
							selectedTable = obj.parentNode.parentNode.parentNode;
							return true;
						}
					}
					else{
		...
		if (oControlRange && oControlRange[0] && oControlRange[0].tagName.toUpperCase() == "TABLE") {
			selectedTable = oControlRange[0];
			return true;
		}	
	}
} 
...
function isCursorInTableCell() {
	var oSession = GetCurrSession();
	if (oSession.type != "Control" || !window.ActiveXObject) {
		try{ var elem = oSession.createRange().parentElement();}catch(e){}
		if( !window.ActiveXObject) {
	
			var win = document.getElementById("eWebEditor").contentWindow;
			if (win.event) {
				obj = win.event.srcElement;
			}
		...
		if(elem && elem.parentElement && elem.parentElement.tagName=="TD") {
			elem =  elem.parentElement;
		}
		if (elem && elem.tagName && elem.tagName=="TD") {
			selectedTD = elem;
			selectedTR = selectedTD.parentElement;
			selectedTBODY =  selectedTR.parentElement;
			selectedTable = selectedTBODY.parentElement;
