...
'Dim s, c, i , rnds, item
	...
	Response.write ">" & vbcrlf & "<style>" & vbcrlf & "body{" & vbcrlf & " scrollbar-highlight-color:#fff;" & vbcrlf & " scrollbar-face-color:#f0f0ff;" & vbcrlf & "   scrollbar-arrow-color:#c0c0e8;" & vbcrlf & "  scrollbar-shadow-color:#d0d0e8;" & vbcrlf & " scrollbar-darkshadow-color:#fff;" & vbcrlf & "        scrollbar-base-color:#ffffff;" & vbcrlf & "  scrollbar-track-color:#fff;" & vbcrlf & "}" & vbcrlf & "#ht1{margin-top:-1px}" & vbcrlf & "div#kh{height:50px!important;line-height:50px!important;}" & vbcrlf & ".IE5 div#kh{padding-top:18px}" & vbcrlf & "</style>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power WITH(NOLOCK) where ord="&session("personzbintel2007")&" and sort1=26 and sort2=14"
	...
	if open_2_1=3 then
		list="1=1"
	elseif open_2_1=1 then
		list="tel.cateid in  ("&iif(intro_2_1&""="","0",intro_2_1)&")"
	else
		list="tel.cateid=-1"
		list="tel.cateid in  ("&iif(intro_2_1&""="","0",intro_2_1)&")"
	end if
	Str_Result=" and a.del=1"
	Dim MODULES
	MODULES=session("zbintel2010ms")
	open_2_18=openPower(2,18)
	dim q,m1,m2,F1,F2,C1,C2,E,F,W1,W2,W3,D,P1,P2,J1,J2,Mob1,Mob2,T1,T2,Fax1,Fax2,QQ1,QQ2,MSN1,MSN2,Email1,Email2,Bir,Add1,Add2,Joy1,Joy2,Pym1,Pym2,Intro1,Intro2
	Dim WW1,WW2,WW3
	...
	Pym1=request("Pym1")
	Pym2=request("Pym2")
	Intro1=request("Intro1")
	Intro2=request("Intro2")
	m1=request("ret")
	m2=request("ret2")
	Bir=request("Bir")
	D=request("D")
	...
	WW2=request("WW2")
	WW3=request("WW3")
	if ly="" or isnull(ly) Then ly=0
	if s3="" Then s3=0
	if q="" Then q=1
	A2=request("A2")
	B=request("B")
	sex=request("sex")
	age_0=request("age_0") : age_1=request("age_1")
	uiHide = request("uitype") = "micromsg.binduser"
	...
	If s3="1" Or s3="2" Then
		Str_Result=Str_Result&" and (/*dis.p.out.b*/(tel.sharecontact=1 and (tel.share = '1' or charindex(',"&session("personzbintel2007")&",',','+cast(tel.share as varchar(8000))+',')>0)) or /*dis.p.out.e*/ (" & vbcrlf & "/*p-2-a.cateid-s*/" & vbcrlf & list & vbcrlf & "/*pe*/)) "
'If s3="1" Or s3="2" Then
	else
		if list<>"" Then
			Str_Result=Str_Result& " and (/*dis.p.out.b*/(tel.sharecontact=1 and (tel.share = '1' or charindex(',"&session("personzbintel2007")&",',','+cast(tel.share as varchar(8000))+',')>0)) or /*dis.p.out.e*/ (" & vbcrlf & "/*p-2-a.cateid-s*/" & vbcrlf & list & vbcrlf & "/*pe*/))"
'if list<>"" Then
	...
	if s3<>"0" and s3<>"" Then Str_Result=Str_Result+" and  a.sort3 ="&s3&" "
'if list<>"" Then
	If WW1<>"" And isnumeric(WW1) Then Str_Result=Str_Result+" and  a.cateid2 ="&WW1&" "
'if list<>"" Then
	If WW2<>"" And isnumeric(WW2) Then Str_Result=Str_Result+" and  a.cateid3 ="&WW2&" "
'if list<>"" Then
	If WW3<>"" And isnumeric(WW3) Then Str_Result=Str_Result+" and  a.cateid ="&WW3&" "
'if list<>"" Then
	if s3=2 And ly<>"0" Then Str_Result=Str_Result+" and  ((tel.ly="&ly&" ) or (isnull(tel.ord,0)=0)) and tel.del=1"
'if list<>"" Then
	if B="" then B=1
	if W4<>"" then
		select case B
		case 1:
		Str_Result=Str_Result+" and tel.cateid in ("&W3&") and tel.cateid>0 "
'case 1:
		case 2:
		Str_Result=Str_Result+" and a.cateid in ("&W3&") and a.cateid>0 "
'case 2:
		case 3:
		Str_Result=Str_Result+" and (tel.cateid in ("&W3&") and a.cateid in ("&W3&") and tel.cateid>0 and a.cateid>0 ) "
'case 3:
		end select
	end if
	Function selectArea(id)
		Dim rs,strid
		strid=conn.execute("select dbo.GetMenuArea('" & id & "', 'menuarea') as r")(0).value
		If Len(strid)=0 Then strid = -1
		strid=conn.execute("select dbo.GetMenuArea('" & id & "', 'menuarea') as r")(0).value
		selectArea=strid
	end function
	if A2<>"" then Str_Result=Str_Result+" and  a.area in ("&selectArea(Replace(A2," ",""))&") and tel.del=1 "
	'selectArea=strid
	if D<>"" Then Str_Result=Str_Result+" and  a.trade in  ("&D&") and tel.del=1 "
	'selectArea=strid
	if sex<>"" Then Str_Result=Str_Result+" and  a.sex in('"& Replace(Replace(sex," ",""),",","','")&"') "
	'selectArea=strid
	If age_0&""<>"" And IsNumeric(age_0&"") Then Str_Result=Str_Result+" and  cast(a.age as int) >="& age_0
	'selectArea=strid
	If age_1&""<>"" And IsNumeric(age_1&"") Then Str_Result=Str_Result+" and  cast(a.age as int) <="& age_1
	'selectArea=strid
...
'Select case strField
			Case "a.joy" ,"a.intro" :
			str_Result=str_Result+" and  "& strField &" like '"&strValue&"'"
'Case "a.joy" ,"a.intro" :
'Case Else :
			str_Result=str_Result+" and  "& strField &"='"&strValue&"'"
'Case Else :
'End Select
...
'End Select
	...
	If gjjs = "0" Then
		...
		Mob4 = ""
		T2 = ""
		ksjsv_1= request("ksjsv_1")
		If Len(ksjsv_1)>0 Then
			ksjs_1 = request("ksjs_1")
			If ksjs_1 ="lxrxm" Then
				F2 = ksjsv_1
				F1 = 1
				strSelecT_lxrxm = " selected "
			ElseIf ksjs_1 ="wx" Then
				WX2 = ksjsv_1
				WX1 = 1
				strSelecT_wx  = " selected "
			else
				C2 = ksjsv_1
				C1 = 1
				strSelecT_khmc = " selected "
			end if
		end if
		ksjsv_2 = request("ksjsv_2")
		If Len(ksjsv_2)>0 Then
			ksjs_2 = request("ksjs_2")
			If ksjs_2 ="mobile" Then
				Mob2 = ksjsv_2
				Mob1 = 1
				strSelecT_mobile = " selected "
			ElseIf ksjs_2 ="mobile2" Then
				Mob4 = ksjsv_2
				Mob3 = 1
				strSelecT_mobile2 = " selected "
			else
				T2 = ksjsv_2
				T1 = 1
				strSelecT_phone = " selected "
			end if
		end if
	else
		Call search_scope("a.part1", P2, P1)
		Call search_scope("a.job", J2, J1)
		Call search_scope("a.fax", Fax2, Fax1)
		Call search_scope("a.qq", QQ2, QQ1)
		Call search_scope("a.msn", MSN2, MSN1)
		Call search_scope("a.email", Email2, Email1)
		Call search_scope("a.address", Add2, Add1)
		Call search_scope("a.joy", Joy2, Joy1)
		Call search_scope("a.pym", Pym2, Pym1)
		Call search_scope("a.intro", Intro2, Intro1)
		if E<>"" Then str_Result=str_Result+" and  tel.sort in  ("&E&") and tel.del=1"
		Call search_scope("a.intro", Intro2, Intro1)
		if F<>"" Then str_Result=str_Result+" and  tel.sort1 in  ("&F&")"
		Call search_scope("a.intro", Intro2, Intro1)
		if Bir<>"" Then str_Result=str_Result+" and a.year1='"&Bir&"'"
		Call search_scope("a.intro", Intro2, Intro1)
		if m1<>"" Then Str_Result=Str_Result+" and  a.date7>='"&m1&"'"
		Call search_scope("a.intro", Intro2, Intro1)
		if m2<>"" Then Str_Result=Str_Result+" and  a.date7<'"&dateadd("d",1,CDate(m2))&"'"
		Call search_scope("a.intro", Intro2, Intro1)
	end if
	Call search_scope("a.name", F2, F1)
	Call search_scope("a.mobile", Mob2, Mob1)
	Call search_scope("a.mobile2", Mob4, Mob3)
	Call search_scope("a.phone", T2, T1)
	Call search_scope("a.weixinAcc", WX2, WX1)
	if C2<>"" then
		sql="select ord from tel WITH(NOLOCK) where del=1 "
		Select Case C1
		Case 1 : sql=sql+" and name like '%"& C2 &"%'"
'Select Case C1
		Case 2 : sql=sql+" and name not like '%"& C2 &"%'"
'Select Case C1
		Case 3 : sql=sql+" and name='"&C2&"'"
'Select Case C1
		Case 4 : sql=sql+" and name<>'"&C2&"'"
'Select Case C1
		Case 5 : sql=sql+" and name like '"& C2 &"%'"
'Select Case C1
		Case 6 : sql=sql+" and name like '%"& C2 &"'"
'Select Case C1
		End Select
		str_Result=str_Result+" and a.company in("&sql&") and tel.del=1"
'End Select
	end if
	sType = Request("sType")
	If sType = "top" Then
		...
		If C <> "" Then
			C = Trim(C)
			If B = "xm" Then
				Str_Result = Str_Result + " and  a.name like '%" & C & "%' "
'If B = "xm" Then
			ElseIf B = "pym" Then
				Str_Result = Str_Result + " and  a.pym like '%" & C & "%' "
'ElseIf B = "pym" Then
			ElseIf B = "xb" Then
				Str_Result = Str_Result + " and  a.sex like '%" & C & "%' "
'ElseIf B = "xb" Then
			ElseIf B = "dh" Then
				Str_Result = Str_Result + " and  a.phone like '%" & C & "%' "
'ElseIf B = "dh" Then
			ElseIf B = "sj" Then
				Str_Result = Str_Result + " and a.mobile like '%" & C & "%' "
'ElseIf B = "sj" Then
			ElseIf B = "sj2" Then
				Str_Result = Str_Result + " and a.mobile2 like '%" & C & "%' "
'ElseIf B = "sj2" Then
			ElseIf B = "yx" Or B = "yj" Then
				Str_Result = Str_Result + " and  a.email like '%" & C & "%' "
'ElseIf B = "yx" Or B = "yj" Then
			ElseIf B = "QQ" Then
				Str_Result = Str_Result + " and  a.QQ like '%" & C & "%' "
'ElseIf B = "QQ" Then
			ElseIf B = "msn" Then
				Str_Result = Str_Result + " and  a.MSN like '%" & C & "%' "
'ElseIf B = "msn" Then
			ElseIf B = "dz" Then
				Str_Result = Str_Result + " and  a.address like '%" & C & "%' "
'ElseIf B = "dz" Then
			ElseIf B = "sr" Then
				Str_Result = Str_Result + " and  a.year1 like '%" & C & "%' "
'ElseIf B = "sr" Then
			ElseIf B = "ah" Then
				Str_Result = Str_Result + " and  a.joy like '%" & C & "%' "
'ElseIf B = "ah" Then
			ElseIf B = "bz" Then
				Str_Result = Str_Result + " and  a.intro like '%" & C & "%' "
'ElseIf B = "bz" Then
			ElseIf B = "lxrcz" Then
				Str_Result = Str_Result + " and  a.fax like '%" & C & "%' "
'ElseIf B = "lxrcz" Then
			ElseIf B = "lxrdh" Then
				Str_Result = Str_Result + " and  a.phone2 like '%" & C & "%' "
'ElseIf B = "lxrdh" Then
			ElseIf B = "wx" Then
				Str_Result = Str_Result + " and  a.weixinAcc like '%" & C & "%' "
'ElseIf B = "wx" Then
			end if
		end if
	end if
	...
	if px="" then px=9
	if px=1 then
		px_Result1="order by a.name desc,a.ord asc"
		px_Result2="order by a.name asc,a.ord desc"
	elseif px=2 then
		px_Result1="order by a.name asc,a.ord asc"
		px_Result2="order by a.name desc,a.ord desc"
	elseif px=3 then
		px_Result1="order by tel.name desc,a.ord asc"
		px_Result2="order by tel.name asc,a.ord desc"
	elseif px=4 then
		px_Result1="order by tel.name asc,a.ord asc"
		px_Result2="order by tel.name desc,a.ord desc"
	elseif px=9 then
		px_Result1="order by a.date7 desc,a.name desc"
		px_Result2="order by a.date7 asc,a.name asc"
	elseif px=10 then
		px_Result1="order by a.date7 asc,a.name asc"
		px_Result2="order by a.date7 desc,a.name desc"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where sort1=2 and sort2=13 and ord="&session("personzbintel2007"))
	...
	set rssort=conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where sort1=26 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_26_1=rssort(0)
		intro_26_1=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where sort1=26 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_26_14=rssort(0)
	...
	fieldName4 = "手机"
	if s3<>"2" Then
		fieldName1=arrName(17)
		fieldName2=arrName(19)
		fieldName3 =arrName(1)
		fieldName4 = arrName(20)
		fieldName5 = arrName(100)
	end if
	Response.write "" & vbcrlf & "<form action=""telall.asp?px="
	Response.write px
	Response.write "&page_count="
	Response.write page_count
	Response.write "&H=1"" method=""get"" id=""demo"" onsubmit=""$('#kh').css('display')=='none'?$('#gjjs').val(1):$('#gjjs').val(0);"" name=""date"" style=""margin:0"">" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"" onkeydown=""if(event.keyCode==13){$('#kh').css('display')=='none'?$('#gjjs').val(1):$('#gjjs').val(0);$('#demo')[0].submit();}"">" & vbcrlf & "    <input type=""hidden"" id=""s3"" value="""
	Response.write s3
	Response.write """ name=""s3"">" & vbcrlf & "        <input type='hidden' name='uitype' value='"
	Response.write request("uitype")
	Response.write "'/>" & vbcrlf & "   <tr>" & vbcrlf & "      <td class=""place"">"
	if q=7 then
		Response.write "好友通讯录</font>"
	elseif q=8 then
		Response.write "所有联系人"
	elseif q=1 then
		if s3=2 then
			Response.write "供应商"
		else
	...
	Response.write "</td>" & vbcrlf & "      <td>&nbsp;<a href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "      <td align=""right"" width=""60%"" height=""32"">" & vbcrlf & "      <div align=""right"">" & vbcrlf & ""
	If s3="2" And Not uiHide then
		if open_2_13=3 or open_2_13=1 Then
			Response.write "" & vbcrlf & "        <input type=""button"" name=""Submitdel2"" value=""添加联系人"" onClick=""window.location.href='add4.asp?ty=0'"" class=""anybutton""/> " & vbcrlf & ""
		end if
	end if
	if open_2_9=1 and s3=1  And Not uiHide then
		Response.write "" & vbcrlf & "        <input type=""button"" name=""Submitdel2"" value=""联系人导入"" onClick=""window.location.href='../load/UpLoad_lxr.asp?bgColor=F1F3F5&amp;Wid=0'"" class=""anybutton""/> " & vbcrlf & ""
	end if
	if (open_2_10=1 or open_2_10=3) And Not uiHide Then
		Response.write "" & vbcrlf & "        <input type=""button"" name=""Submitdel2"" value=""联系人导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_lxr.asp',postParams:[{n:'px_v',v:document.getElementById('px_1').value},{n:'Str_Result_v',v:document.getElementById('ReportOrds_1').value}]});}"" class=""anybutton2""/>" & vbcrlf & ""
	end if
	If s3=1 And Not uiHide then
		Response.write "" & vbcrlf & "             <input type=""button"" name=""Submitdel2"" value=""信封打印"" onClick=""$('#mailprint')[0].submit();"" class=""anybutton""/>" & vbcrlf & ""
	end if
	if (open_2_7=1 or open_2_7=3) And Not uiHide Then
	...
	Response.write "" & vbcrlf & "        " & vbcrlf & "      </div>" & vbcrlf & "      </td>" & vbcrlf & "    </tr>" & vbcrlf & "     <tr><td colspan=4  style=""border-right:0px solid #c0ccdd""><div class=""resetHeadBg"" align=""right"" id=""kh"" style='height:55px;line-heihgt:55px;' >" & vbcrlf & "        <input type=""hidden"" name=""gjjs"" id=""gjjs"" value=""0"">" & vbcrlf & " <select name=""ksjs_1"">" & vbcrlf & "                    "
	Response.write "<option value='lxrxm' " & strSelecT_lxrxm & ">"&fieldName1&"</option>"
	If arrShow(100)=1 And s3<>2 Then Response.write "<option value='wx' "& strSelecT_wx &">"&fieldName5&"</option>"
	Response.write "<option value='khmc' "& strSelecT_khmc &">"&fieldName3&"</option>"
	Response.write "" & vbcrlf & "     </select>&nbsp;<input type=""text"" value="""
	Response.write ksjsv_1
	Response.write """ name=""ksjsv_1"" id=""ksjsv_1"" size=""20"">" & vbcrlf & "        "
	If arrShow(19)=1 Or arrShow(20)=1 or s3 =2 Then
		Response.write "" & vbcrlf & "     <select name=""ksjs_2"">" & vbcrlf & "                    "
		If arrShow(20)=1 or s3 =2 Then Response.write "<option value='mobile' " & strSelecT_mobile & ">"&fieldName4&"</option>"
		If arrShow(20)=1 or s3 =2 Then Response.write "<option value='mobile2' " & strSelecT_mobile2 & ">"&fieldName4&"2</option>"
		If arrShow(19)=1 or s3 =2 Then Response.write "<option value='phone' "& strSelecT_phone &">"&fieldName2&"</option>"
		Response.write "" & vbcrlf & "     </select>&nbsp;<input type=""text"" value="""
		Response.write ksjsv_2
		Response.write """ name=""ksjsv_2"" id=""ksjsv_2"" size=""20"">" & vbcrlf & "        "
	end if
	Response.write "" & vbcrlf & "     <input type=""submit"" value=""检索"" class=""anybutton"" >" & vbcrlf & " <a class=""AfterQuickSearch"" style='display:inline-block;width:108px;padding:0;margin:0;height:20px;line-height:20px;*padding-top:2px' href=""javascript:void(0)"" onClick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='block';if(window.parent.frameResize){window.parent.frameResize();};return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"" style=""background:#EFEFEF"">高级检索</font></u></a>&nbsp;</div></td></tr>" & vbcrlf & "  </table>" & vbcrlf & "  <div id=""ht1"" style=""padding:20px;display:none""></div>" & vbcrlf & "</form>" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-left:20px"">" & vbcrlf & "     <form name=""form1"" method=""post"" action=""deleteall.asp?CurrPage="
	...
	Response.write "&B="
	Response.write ly
	Response.write "&age_0="
	Response.write age_0
	Response.write "&age_1="
	Response.write age_1
	Response.write """>" & vbcrlf & " <tr class=""top"">" & vbcrlf & "    <td width=""5%"" align=""center"" "
	Response.write iif(uiHide,"style='display:none'","")
	Response.write "><div align=""center""><strong>选择</strong></div></td>    " & vbcrlf & "         <td width=""8%"" height=""27"" ><div align=""center""><strong>"
	Response.write fieldName1
	Response.write "</strong></div></td>" & vbcrlf & "    <td width=""8%""><div align=""center""><strong>来源</strong></div></td>" & vbcrlf & "         "
	If arrShow(19)=1 or s3 =2 Then
		Response.write "<td width=""10%"" ><div align=""center""><strong>办公电话</strong></div></td>"
	end if
	If arrShow(20)=1 or s3 =2 Then
		Response.write "<td width=""10%"" ><div align=""center""><strong>手机号码</strong></div></td>"
	end if
	Response.write "" & vbcrlf & "        <td width=""8%"" ><div align=""center""><strong>"
	if s3=2 then
		Response.write "采购"
	else
		Response.write "销售"
	end if
	Response.write "人员</strong></div></td>" & vbcrlf & "        <td width=""8%"" ><div align=""center""><strong>添加人</strong></div></td>" & vbcrlf & "      <td width=""8%"" ><div align=""center""><strong>添加时间</strong></div></td>" & vbcrlf & "    <td width=""20%"" ><div align=""center"">" & vbcrlf & "         <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid #FFFFFF;overflow:hidden"">" & vbcrlf & "      <option>-请选择-</option>" & vbcrlf & "      <option value=""page_count=10"" "
	...
	if q=8 then
		Str_Result = Str_Result &" and (a.sort<>7 or (a.sort=7 and a.sort1 is not null))"
	elseif q=7 then
		Str_Result = Str_Result &" and a.sort=7 and a.sort1 is null"
	elseif q=1 then
		Str_Result = Str_Result &" and (isnull(a.sort,0)<>7 or (isnull(a.sort,0)=7 and a.sort1 is not null))"
	end if
	sqlcount="select count(*) from person a WITH(NOLOCK) left join tel WITH(NOLOCK) on a.company=tel.ord where 1=1 "&Str_Result
...
'if currpage=pagecount then
	...
	sql="select * from ( "&_
	"  select top "& page_count &" person.*,isnull(tel.cateid,0) as addcate "&_
	"  from person "&_
	"  left join tel WITH(NOLOCK) on person.company=tel.ord "&_
	"  inner join ( "&_
	"          select top "& intcount &" person.ord "&_
	"          from person WITH(NOLOCK) "&_
	"          left join tel on person.company=tel.ord "&_
	"          inner join ( "&_
	"                  select top "& page_count*CurrPage &" a.ord "&_
	"                  from person a "&_
	"                  left join tel on a.company=tel.ord "&_
	"                  where a.del=1 "& replace(Str_Result,"'","'") &" "& px_Result1 &_
	"          ) x on x.ord = person.ord "& Replace(px_Result2,"a.","person.") &_
	"  ) y on y.ord = person.ord "& Replace(px_Result1,"a.","person.") &_
	" ) aa "
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,3,1
	if rs.eof then
		...
		i=0
		BookNum=recCount
		dim cateid,cateidname, lxr_addcate, lxr_addname,addtime
		do until rs.eof = True Or Response.IsClientConnected = False
			dim name2, itemdel
			tel_ord=rs("ord")
			addtime=rs("date7")
			itemdel = false
			if rs("company")<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select top 1 ord,sort1,name,del,share from tel WITH(NOLOCK) where ord="&rs("company")&" "
				rs1.open sql1,conn,3,1
				if  rs("sort")="7" and isnull(rs("sort1")) then
					name2="备注：私人好友"
				else
					if rs1.eof  then
						name2="此联系人对应公司已被删除"
						...
						name2=rs1("name")
						share=rs1("share")
						itemdel = (rs1("del").value = 2)
						If itemdel = True Then
							name2 = "<span style='color:#888'>" & name2 & "</span>&nbsp;<span style='color:red'>(已删)</span>"
						end if
					end if
				end if
				rs1.close
			...
			cateid = "" : lxr_addcate = ""
			cateid=rs("addcate") : lxr_addcate=rs("cateid")
			cateidname="" : lxr_addname = ""
			if cateid&"" <>" " Or lxr_addcate&"" <> "" then
				If cateid&"" = "" Then cateid = 0
				If lxr_addcate&"" = "" Then lxr_addcate = 0
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select cateidname = (select top 1 name from gate WITH(NOLOCK) where ord="&cateid&"), addname = (select top 1 name from gate where ord="& lxr_addcate &")"
				rs7.open sql7,conn,3,1
				if rs7.eof = False then
					cateidname=rs7("cateidname")&""
					lxr_addname=rs7("addname")&""
				end if
				rs7.close
				set rs7=nothing
			end if
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td width=""6%"" align=""center"" "
			Response.write iif(uiHide,"style='display:none'","")
			Response.write ">"
			if open_2_3=3 or (open_2_3=1 and CheckPurview(intro_2_3,trim(cateid))=True) then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>"
			end if
			Response.write "</td>" & vbcrlf & "    <td height=""27"" class=""name""><div align=""center"">"
			if open_2_14=3 or (open_2_14=1 and CheckPurview(intro_2_14,trim(cateid))=True) then
				Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('content"
				if rs("sort3")="2" Then Response.write "y"
				Response.write ".asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 980 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此联系人详情"">"
				Response.write pwurl(rs("ord"))
			end if
			Response.write rs("name")
			Response.write "" & vbcrlf & "    </a></div></td>" & vbcrlf & "    <td width=""27%""><div align=""left"">&nbsp;" & vbcrlf & ""
			if rs("sort")="7" and isnull(rs("sort1")) then
				...
				if rs("sort3")=1 then
					if (open_1_1=3 or (open_1_1=1 and CheckPurview(intro_1_1,trim(cateid))=True)) or share="1" or instr(","&share&",",","&session("personzbintel2007")&",")>0 then
						if (open_1_14=3 or (open_1_14=1 and CheckPurview(intro_1_14,trim(cateid))=True)) And itemdel=False then
							Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" onClick=""javascript:window.open('../work/content.asp?ord="
							Response.write pwurl(rs("company"))
							Response.write "','newwin','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此客户详情"">"
							Response.write pwurl(rs("company"))
							Response.write name2
					...
					if open_26_1=3 or (open_26_1=1 and CheckPurview(intro_26_1,trim(cateid))=True)  or share="1" or instr(","&share&",",","&session("personzbintel2007")&",")>0  then
						if (open_26_14=3 or (open_26_14=1 and CheckPurview(intro_26_14,trim(cateid))=True)) And itemdel=False then
							Response.write "" & vbcrlf & "                <a href=""javascript:void(0)"" onClick=""javascript:window.open('../work2/content.asp?ord="
							Response.write pwurl(rs("company"))
							Response.write "','newwin','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此供应商详情"">"
							Response.write pwurl(rs("company"))
							Response.write name2
							Response.write "" & vbcrlf & "                             </a>" & vbcrlf & "                    "
						else
			...
			If arrShow(19)=1 or s3 =2 Then
				...
				Response.write GetPhoneNumber(rs("phone"), cateid)
				if ZBRuntime.MC(32000) then
					if rs("phone")<>"" and tp=1 then
						Response.write "" & vbcrlf & "<a href=""javascript:void(0);"" onclick=""getCall('../china/bh.asp?ord=-"
'if rs("phone")<>"" and tp=1 then
						Response.write (tel_ord)
						Response.write "&ordtype=1&phone="
						Response.write rs("phone")
						Response.write "');return false;"" title=呼出><img src=""../images/15.gif"" border=""0"" align=""absbottom""></a>" & vbcrlf & ""
					end if
				end if
				Response.write "" & vbcrlf & "" & vbcrlf & "                             </div></td>" & vbcrlf & "                             "
			end if
			If arrShow(20)=1 or s3 =2 Then
				Response.write "" & vbcrlf & "                     <td class=""name""><div align=""center"">" & vbcrlf & "                       "
				Response.write GetPhoneNumber(rs("mobile"), cateid)
				if ZBRuntime.MC(32000) and isMobile(trim(rs("mobile"))) And open_74_12 = 1 Then Response.write " <img src='../images/sj.gif' onClick=""getCall('../china/bh.asp?ord=-"&tel_ord&"&ordtype=1&phone="&rs("mobile")&"')"" style='cursor:hand' border='0' align='absbottom'>"
				Response.write GetPhoneNumber(rs("mobile"), cateid)
				If ZBRuntime.MC(100000) And open_67_13=1 and open_67_19<>1   and isMobile(trim(rs("mobile"))) Then
					If IsPhonePower(cateid) Then
						mobileNum = trim(rs("mobile"))
						from = "normal"
					else
						mobileNum = HexEncode(trim(rs("mobile")))
						from = "encrypt"
					end if
					Response.write "&nbsp;<img src='../images/message.gif'  onClick=""javascript:window.open('../message/topadd.asp?from="&from&"&phone="&mobileNum&"','newsSendSMS','width=' + 900 + ',height=' + 800 + ',fullscreen =no,scrollbars=0,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=200,top=100')""  style='cursor:hand' border='0' alt='发送短信' align='absbottom'>"
				end if
			...
			Response.write lxr_addname
			Response.write "</div></td>" & vbcrlf & "                  <td class=""name""><div align=""center"">"
			Response.write addtime
			Response.write "</div></td>" & vbcrlf & "                  <td class=""func"">" & vbcrlf & "                 <div align=""center"">" & vbcrlf & ""
			If uiHide = True Then
				Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" onclick='parent.bindMicroMsgUser("
				Response.write rs("ord")
				Response.write ");' >点击关联此联系人</a>" & vbcrlf & ""
			else
				if open_2_14=3 or (open_2_14=1 and CheckPurview(intro_2_14,trim(cateid))=True) Then
					Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('"
					if s3="2" then Response.write("contenty.asp?ord="& pwurl(rs("ord")) &"") else Response.write("content.asp?ord="& pwurl(rs("ord")) &"")
					Response.write "','newwin','width=' + 980 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & ""
				end if
				if open_2_2=3 or (open_2_2=1 and CheckPurview(intro_2_2,trim(cateid))=True) Then
					Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit3c"" value=""修改""  onClick=""javascript:window.open('"
					if s3="2" then Response.write("correcty.asp?cc=2&ord="& pwurl(rs("ord"))&"") else Response.write("correct.asp?cc=2&ord="& pwurl(rs("ord"))&"")
					Response.write "','newwincor','width=' + 980 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & ""
				end if
				if open_2_3=3 or (open_2_3=1 and CheckPurview(intro_2_3,trim(cateid))=True) Then
					Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete.asp?ord="
					Response.write rs("ord")
					Response.write "&CurrPage="
					Response.write CurrPage
					Response.write "&A2="
			...
			rs.movenext
		loop
		Response.write "" & vbcrlf & "      </table>" & vbcrlf & "</td>" & vbcrlf & "    </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30"">" & vbcrlf & ""
		if (open_2_3=3 or open_2_3=1) And Not uiHide Then
			Response.write "" & vbcrlf & "             <div align=""center"">全选" & vbcrlf & "                  <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "           </div>" & vbcrlf & ""
		end if
		Response.write "" & vbcrlf & "     </td>" & vbcrlf & "    <td >" & vbcrlf & ""
		if (open_2_3=3 or open_2_3=1) And Not uiHide Then
			Response.write "" & vbcrlf & "                     <input class=""resetElementHidden"" type=image src=""../images/m_an_del.gif""   onClick=""return test();""  name=submit>" & vbcrlf & "        <input class=""resetElementShow anybutton"" type=submit value=""批量删除"" style=""display:none;"" onClick=""return test();"" name=submit>"& vbcrlf & ""
		end if
		Response.write "" & vbcrlf & "     </td>" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "     <span class=""black"">"
		...
		Response.write "页 | &nbsp;"
		Response.write page_count
		Response.write "条信息/页</span>&nbsp;&nbsp;" & vbcrlf & "    <input id=""curp1"" name=""curp1""  type=text   onpropertychange=""formatData(this,'int');"" maxlength=8 onkeyup=""checkDot('curp1',0,8)""  size=3 value='"
		Response.write currPage
		Response.write "' />" & vbcrlf & "    <input type=""button"" name=""Submit422"" value=""跳转"" onclick=""gotourl('currPage='+document.getElementById('curp1').value);""  class=""anybutton2""/>" & vbcrlf & "    "
		Response.write currPage
		if currpage=1 then
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "  for (var i=0;i<form.elements.length;i++)" & vbcrlf & "  {" & vbcrlf & "               var e = form.elements[i];" & vbcrlf & "               if (e.name != 'chkall')" & vbcrlf & "         e.checked = form.chkall.checked;" & vbcrlf & "        }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	action1="联系人列表"
	call close_list(1)
	Response.write "  </form>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:160;display:none;"">" & vbcrlf & "<table width=""150"" height=""160""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf& "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "<tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按姓名排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "<tr valign=""middle"">" & vbcrlf & "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按姓名排序(升)</font></a> </td>" & vbcrlf & "   </tr>" & vbcrlf & "<tr  valign=""middle"">" & vbcrlf & "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按来源排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr valign=""middle"">" & vbcrlf & "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按来源排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr valign=""middle"">" & vbcrlf & "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=9');""><font color=""#2F496E"">按添加时间排序(降)</font></a> </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=10');""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf &     "</td>" & vbcrlf &   "</tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "</div>" & vbcrlf & vbcrlf & "<script language=""javascript"">"  & vbcrlf & "function Myopen(divID){" & vbcrlf &"if(divID.style.display==""""){" & vbcrlf & "divID.style.display=""none""" & vbcrlf &         "}else{" & vbcrlf & "divID.style.display=""""" & vbcrlf &     "}" & vbcrlf &        "divID.style.left=300;" & vbcrlf &    "divID.style.top=0;" & vbcrlf & "}" & vbcrlf & vbcrlf & "function checkAll2(str){" & vbcrlf & "        var a=document.getElementById(""u""+str).getElementsByTagName(""input"");" & vbcrlf & "       var b=document.getElementById(""e""+str);" & vbcrlf & "   for(var i=0;i<a.length;i++){" & vbcrlf & "            a[i].checked=b.checked;" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function fixChk2(str){" & vbcrlf & " var a=document.getElementById(""u""+str).getElementsByTagName(""input"");" & vbcrlf & "       var b=document.getElementById(""e""+str);" & vbcrlf & "   for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                        b.checked=false;" & vbcrlf & "                        return ;" & vbcrlf & "                }" & vbcrlf & "}" & vbcrlf & "b.checked=true;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & ""
	Set BC= server.createobject(ZBRLibDLLNameSN & ".Base64Class")
	Response.write "" & vbcrlf & "<form target='_blank' action='moban_dy.asp' id='mailprint' method='post'>" & vbcrlf & "<input type='hidden' name=""sort"" value=7>" & vbcrlf & "<input type='hidden' name=""main"" value=1>" & vbcrlf & "<input type='hidden' name=""ReportOrds"" id='ReportOrds_1' value='"
	Response.write BC.SafeEnCode(Str_Result)
	Response.write "'>" & vbcrlf & "<input type='hidden' name=""px"" id='px_1' value="""
	Response.write BC.SafeEnCode(px_Result1)
