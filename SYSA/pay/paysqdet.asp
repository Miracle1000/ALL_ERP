...
'If Not rs.Eof Then
	...
	if open_6_1=3 then
		Str_Result=""
	elseif open_6_1=1 then
		Str_Result=" and a.cateid in ("&intro_6_1&")"
	else
		Str_Result="and a.cateid=0"
	end if
	if request("ret")<>"" then
		m1=request("ret")
		m2=request("ret2")
	...
	px=request("px")
	stel=request("stel")
	scontractth=request("scontractth")
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result+" and a.cateid in ("&W3&")  "
'if W4<>"" and not isnull(W4) then
	end if
	if stel<>"" then
		Str_Result=Str_Result&" and b.company in (select ord from tel where name like '%"&stel&"%') "
	end if
	if sperson<>"" then
		Str_Result=Str_Result&" and b.person in (select ord from person where name like '%"&sperson&"%') "
	end if
	if scontract<>"" then
		Str_Result=Str_Result&" and b.contract in (select ord from contract where title like '%"&scontract&"%') "
	end if
	if scontractth<>"" then
		Str_Result=Str_Result&" and b.contractth in (select ord from contractth where title like '%"&scontractth&"%') "
	end if
	if sshouhou<>"" then
		Str_Result=Str_Result&" and b.shouhou in (select ord from tousu where title like '%"&sshouhou&"%') "
	end if
	if scaigou<>"" then
		Str_Result=Str_Result&" and b.caigou in (select ord from caigou where title like '%"&scaigou&"%') "
	end if
	if scaigouth<>"" then
		Str_Result=Str_Result&" and b.caigouth in (select ord from caigouth where title like '%"&scaigouth&"%') "
	end if
	if swork<>"" then
		Str_Result=Str_Result&" and b.iwork in (select ord from tel where name like '%"&swork&"%') "
	end if
	if schance<>"" then
		Str_Result=Str_Result&" and b.chance in (select ord from chance where title like '%"&schance&"%') "
	end if
	if sfahuo<>"" then
		Str_Result=Str_Result&" and b.fahuo in (select ord from send where title like '%"&sfahuo&"%') "
	end if
	if sricheng<>"" then
		Str_Result=Str_Result&" and b.richeng in (select ord from plan1 where intro like '%"&sricheng&"%') "
	end if
	if sjk<>"" then
		Str_Result=Str_Result&" and b.jkid in (select id from payjk where title like '%"&sjk&"%') "
	end if
	if f<>"" then
		Str_Result=Str_Result+" and b.sort1 in("&f&") "
'if f<>"" then
	end if
	if payid<>"" then
		Str_Result=Str_Result+" and b.bh like '%"&payid&"%' "
'if payid<>"" then
	end if
	if listbz<>"" then
		listbz=cdbl(listbz)
...
'if e<>"" then
	...
	if intro<>"" then
		Str_Result=Str_Result+"and  b.note like '%"&intro&"%' "
'if intro<>"" then
	end if
	if remark<>"" then
		Str_Result=Str_Result+"and  a.remark like '%"& remark &"%' "
'if remark<>"" then
	end if
	if money1<>"" then
		if isnumeric(money1)=false then
			Response.write("<script>alert('请输入合法金额');window.location=('paysqdet.asp');</script>")
			call db_close : Response.end
		else
			Str_Result=Str_Result+"and money1 >="&money1&" "
			call db_close : Response.end
		end if
	end if
	if money12<>"" then
		if isnumeric(money12)=false then
			...
			Str_Result=Str_Result+"and  money1 <="&money12&" "
			call db_close : Response.end
		end if
	end if
	if T="1" then
		Str_Result=Str_Result+" and a.complete in ("&T&") "
'if T="1" then
	end if
	if T="2" then'
	Str_Result=Str_Result+" and a.complete in ("&T&") "
...
'if B="1" then
	elseif B="2" then
		...
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("gate","name",C,"ord")
		if trim(perid)<>"" then
			Str_Result=Str_Result+"and a.cateid in ("&perid&") "
'if trim(perid)<>"" then
		end if
	elseif B="3" then
		perid=selname("gate","name",C,"ord")
		...
		perid=selname("gate","name",C,"ord")
		if trim(perid)<>"" then
			Str_Result=Str_Result+"and a.addcateid in ("&perid&") "
'if trim(perid)<>"" then
		end if
	elseif B="20" then
		Str_Result=Str_Result+"and a.remark LIKE '%"& C &"%' "
'elseif B="20" then
	elseif B="4" then
		Str_Result=Str_Result+" and  b.bh like '%"&C&"%'"
'elseif B="4" then
	elseif B="6" then
		perid=selname("tel","name",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("tel","name",C,"ord")
		Str_Result=Str_Result+" and  b.company in ("&perid&") "
		perid=selname("tel","name",C,"ord")
	elseif B="7" then
		perid=selname("person","name",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("person","name",C,"ord")
		Str_Result=Str_Result+" and  b.person in ("&perid&") "
		perid=selname("person","name",C,"ord")
	elseif B="8" then
		perid=selname("contract","title",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("contract","title",C,"ord")
		Str_Result=Str_Result+" and  b.contract in ("&perid&") "
		perid=selname("contract","title",C,"ord")
	elseif B="9" then
		perid=selname("tousu","title",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("tousu","title",C,"ord")
		Str_Result=Str_Result+" and  b.shouhou in ("&perid&") "
		perid=selname("tousu","title",C,"ord")
	elseif B="10" then
		perid=selname("caigou","title",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("caigou","title",C,"ord")
		Str_Result=Str_Result+" and  b.caigou in ("&perid&") "
		perid=selname("caigou","title",C,"ord")
	elseif B="11" then
		perid=selname("tel","name",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("tel","name",C,"ord")
		Str_Result=Str_Result+" and  b.iwork in ("&perid&") "
		perid=selname("tel","name",C,"ord")
	elseif B="12" then
		perid=selname("chance","title",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("chance","title",C,"ord")
		Str_Result=Str_Result+" and  b.chance in ("&perid&") "
		perid=selname("chance","title",C,"ord")
	elseif B="13" then
		perid=selname("send","title",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("send","title",C,"ord")
		Str_Result=Str_Result+" and  b.fahuo in ("&perid&") "
		perid=selname("send","title",C,"ord")
	elseif B="5" then
		Str_Result=Str_Result+" and  b.note like '%"&C&"%'"
'elseif B="5" then
	elseif B="14" then
		perid=selname("plan1","intro",C,"ord")
		if trim(perid)="" or isnull(perid) then   perid=-1
		perid=selname("plan1","intro",C,"ord")
		Str_Result=Str_Result+" and  b.richeng in ("&perid&") "
		perid=selname("plan1","intro",C,"ord")
	end if
	if m1<>"" then
...
'if m2<>"" then
	...
	if not isNumeric(currpage) or currpage<=0 then
		currpage=1
	end if
	...
	elseif px=3 then
		pxResult=" order by a.adddate desc"
	elseif px=4 then
		pxResult=" order by a.adddate asc"
	elseif px=5 then
		pxResult=" order by a.sp_date desc"
	elseif px=6 then
		pxResult=" order by a.sp_date asc"
	end if
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">     " & vbcrlf & "            <form action=""paysqdet.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" >" & vbcrlf & "                <tr>" & vbcrlf & "         <td class=""place"">申请"
	if x="1" then
		Response.write "统计"
	else
		Response.write "明细表"
	end if
	Response.write "</td>" & vbcrlf & "         <td><div align=""left""><A class='px_btn' onclick=""Myopen(User);return false;"" href=""javascript:void(0)"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></div></td>" & vbcrlf & "        <td align=""right""><div id=""kh""style='height:20px;line-height:20px'>" & vbcrlf & "          <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "                      <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	Response.write "明细表"
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "              </select></div></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""6""cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "  <tr><td colspan=""10"" class='ser_btn' style='height:50px;line-height:50px;padding-top:0;padding-bottom:0;'  align=""right""><div align=""right"" >"
	Response.write "selected"
	if x="1" then
	else
		Response.write "<input type=""radio"" name=""rad1"" onClick=""javascript:window.location.href='paysq.asp?"
		Response.write sdk.ReturnURL("")
		Response.write "'"">" & vbcrlf & "       列表" & vbcrlf & "          <input name=""rad1"" type=""radio"" onClick="""" checked=""CHECKED"">" & vbcrlf & "     明细&nbsp;&nbsp;" & vbcrlf & "        "
	...
	Response.write ">申请主题</option>" & vbcrlf & "                  <option value=""2"" "
	if B="2" then
		Response.write "selected"
	end if
	Response.write ">申请人员</option>" & vbcrlf & "                  <option value=""3"" "
	if B="3" then
		Response.write "selected"
	end if
	Response.write ">添加人员</option>" & vbcrlf & "                      "
	if x<>"1" then
		Response.write "" & vbcrlf & "                        <option value=""4"" "
		if B="4" then
			Response.write "selected"
		end if
		Response.write ">申请编号</option>" & vbcrlf & "                      <option value=""5"" "
		if B="5" then
			Response.write "selected"
		end if
		Response.write ">申请说明</option>" & vbcrlf & "                      <option value=""20"" "
		if B="20" then
			Response.write "selected"
		end if
		Response.write ">申请备注</option>" & vbcrlf & "                      <option value=""6"" "
		if B="6" then
			Response.write "selected"
		end if
		Response.write ">关联客户</option>" & vbcrlf & "                      <option value=""7"" "
		if B="7" then
			Response.write "selected"
		end if
		Response.write ">关联联系人</option>" & vbcrlf & "                    <option value=""8"" "
		if B="8" then
			Response.write "selected"
		end if
		Response.write ">关联合同</option>" & vbcrlf & "                      <option value=""9"" "
		if B="9" then
			Response.write "selected"
		end if
		Response.write ">关联售后</option>" & vbcrlf & "                      <option value=""10"" "
		if B="10" then
			Response.write "selected"
		end if
		Response.write ">关联采购</option>" & vbcrlf & "                      <option value=""11"" "
		if B="11" then
			Response.write "selected"
		end if
		Response.write ">关联供应商</option>" & vbcrlf & "                    <option value=""12"" "
		if B="12" then
			Response.write "selected"
		end if
		Response.write ">关联项目</option>" & vbcrlf & "                      <option value=""13"" "
		if B="13" then
			Response.write "selected"
		end if
		Response.write ">关联发货</option>" & vbcrlf & "                      <option value=""14"" "
		if B="14" then
			Response.write "selected"
		end if
		Response.write ">关联日程</option>" & vbcrlf & "                      "
	end if
	Response.write "" & vbcrlf & "               </select>" & vbcrlf & "                  <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "                    <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton2""/>"
	if x<>"1" then
		Response.write "" & vbcrlf & "                      <input type=""button"" name=""openbn"" id=""openbn"" value=""高级检索""  class=""anybutton2"" onclick=""$('#w').show();$('#w').window('open');$('#hideDiv').show();""/>" & vbcrlf & "                     "
		if open_6_10=1 or open_6_10=3 then
			Response.write "" & vbcrlf & "                    <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_sqdet.asp'})}"" class=""anybutton2""/>" & vbcrlf & "                    "
		end if
	end if
	...
	Response.write "     " & vbcrlf & "                    </div></td></tr>" & vbcrlf & "            </form>" & vbcrlf & " <tr class=""top"">" & vbcrlf & "    <td width=""8%""  ><div align=""center"">申请编号</div></td>" & vbcrlf & "    <td width=""14%"" ><div align=""center"" >申请主题</div></td>" & vbcrlf & "   <td width=""10%"" ><div align=""center"" >费用分类</div></td>" & vbcrlf & "    <td width=""12%"" ><div align=""center"" >申请金额</div></td>" & vbcrlf & "   <td width=""8%"" ><div align=""center"" >审批状态</div></td>" & vbcrlf & "    <td width=""12%"" ><div align=""center"" >申请日期</div></td>" & vbcrlf & "   <td width=""8%"" ><div align=""center"">申请人员</div></td>" & vbcrlf & "       <td width=""10%"" ><div align=""center"">分类明细</div></td>" & vbcrlf & "    <td width=""10%"" ><div align=""center"">关联数据</div></td>" & vbcrlf & "    <td width=""8%"" ><div align=""center"">备注</div></td>" & vbcrlf & "       </tr>" & vbcrlf & "           "
	dim n
	n=0
	summoney1=0
	money1_all=0
	sql="select isnull(sum(cast( (isnull(h.hl,1)*isnull(b.money1,0)) as decimal(32,8))),0) as summoney1 "&_
	"  from paysqlist as b WITH(NOLOCK) "&_
	"  left join paysq as a WITH(NOLOCK) on b.fid=a.id "&_
	"  left join hl h WITH(NOLOCK) on h.bz=isnull(a.bz,14) and datediff(d,h.date1,getdate())=0 " &_
	"  where a.del=1 and (b.del is null or b.del=1) "& Str_Result
	money1_all=conn.execute(sql)(0)
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.title,a.sp_date,a.cateid,a.complete,a.bz,b.fid,b.bh,b.money1,b.id,b.note, isnull(h.hl,1)*isnull(b.money1,0) as rmbmoney1,g.name as cateidname,isnull(s.intro,'RMB') as trade8,p.sort1 as ftype,y.sort1 as fftype "&_
	"  from paysqlist as b WITH(NOLOCK) "&_
	"  left join paysq as a WITH(NOLOCK) on b.fid=a.id "&_
	"  left join hl h WITH(NOLOCK) on h.bz=isnull(a.bz,14) and datediff(d,h.date1,getdate())=0 " &_
	"  left join gate g WITH(NOLOCK) on g.ord = a.cateid " &_
	"  left join sortbz s on s.id = a.bz "&_
	"  left join paytype p on p.id = b.sort1 "&_
	"  left join sortonehy y on y.ord = p.sort2 "&_
	"  where a.del=1 and (b.del is null or b.del=1) "& Str_Result &" "& pxResult
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "             <form name=""form1"" method=""post"" action=""deleteall.asp?CurrPage="
		...
		Response.write "&stel="
		Response.write stel
		Response.write "&scontract="
		Response.write scontract
		Response.write "&scontractth="
		Response.write scontractth
		Response.write "&sperson="
		Response.write sperson
		Response.write "&sricheng="
		Response.write sricheng
		Response.write "&sshouhou="
		Response.write sshouhou
		Response.write "&schance="
		Response.write schance
		Response.write "&scaigou="
		Response.write scaigou
		Response.write "&scaigouth="
		Response.write scaigouth
		Response.write "&sfahuo="
		Response.write sfahuo
		Response.write "&swork="
		Response.write swork
		Response.write "&sjk="
		Response.write sjk
		Response.write "&ret="
		...
		do until rs.eof
			...
			summoney1=cdbl(summoney1)+cdbl(numcg2)
'if numcg2="" or isnull(numcg2) then numcg2=0
			k=rs("title")
			catename=rs("cateidname")
			fid=rs("fid")
			if fid="" or isnull(fid) then fid=0
			trade8=rs("trade8")
			ftype=rs("ftype")
			fftype=rs("fftype")
			cateid=rs("cateid")
			Response.write "" & vbcrlf & "              <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "           <td width=""8%"" align=""center""><div align=""center"">"
			Response.write rs("bh")
			Response.write "</div></td>" & vbcrlf & "           <td width=""14%"" ><div align=""left"">"
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                              <a href=""javascript:void(0);"" onClick=""javascript:window.open('paysqdetail.asp?ord="
				Response.write fid
			...
			Response.write "" & vbcrlf & "              </div></td>" & vbcrlf & "        <td width=""10%""><div align=""center"">"
			if ftype<>"" then Response.write fftype&"→"&ftype
			Response.write "</div></td>" & vbcrlf & "        <td width=""10%"" class=""func""><div align=""right"">"
			...
			if rs("complete")=0 then
				Response.write "待审批"
			elseif rs("complete")=3 then
				Response.write "审批未通过"
			elseif rs("complete")=1 then
				Response.write "审批通过"
			elseif rs("complete")=2 then
				Response.write "审批中"
			end if
			Response.write "</div></td>" & vbcrlf & "           <td width=""10%""><div align=""center"">"
			Response.write rs("sp_date")
			Response.write "</div></td>" & vbcrlf & "           <td width=""8%""><div align=""center"">"
			Response.write catename
			Response.write "</div></td>" & vbcrlf & "           <td width=""10%""><div align=""center"">" & vbcrlf & "                <a href=""javascript:void(0)"" onMouseOut=""hidelabel()"" onMouseOver=""xmldata2('"
			Response.write rs("id")
			Response.write "')"">查看</a>" & vbcrlf & "               </div></td>" & vbcrlf & "             <td width=""10%""><div align=""center""><a href=""javascript:void(0)"" onMouseOut=""hidelabel()"" onMouseOver=""xmldata1('"
			Response.write rs("id")
			Response.write "')"">查看</a></div></td>            " & vbcrlf & "            <td width=""10%""><div align=""center"">"
		...
		loop
		Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "            <td width=""30%"" height=""25"" colspan=""3""><div align=""right"">本页合计：</div></td>" & vbcrlf & "                <td width=""10%"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		...
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "           <td colspan=""6""></td>" & vbcrlf & "             </tr>" & vbcrlf & "           <tr>" & vbcrlf & "            <td width=""30%"" height=""25"" colspan=""3""><div align=""right"">所有合计：</div></td>" & vbcrlf & "                <td width=""10%"" class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		...
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "           <td colspan=""6""></td>" & vbcrlf & "             </tr>" & vbcrlf & "             </table>" & vbcrlf & "    </td>" & vbcrlf & "       </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" id=""content5"">                " & vbcrlf & "                <tr>" & vbcrlf & "            <td colspan=""3""><div align=""right"">" & vbcrlf & "         "
		...
		Response.write "" & vbcrlf & "             </div>" & vbcrlf & "          </td>" & vbcrlf & "     </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "     </td>" & vbcrlf & "  </tr>" & vbcrlf & "" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:152;display:none;"">" & vbcrlf & "<table width=""150"" height=""160""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>"& vbcrlf &"     <td height=""152"">" & vbcrlf & "        <table width=""150"" height=""152"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl='#2F496E'>按申请主题排序(降)</font></a></td>""" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按申请主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "    <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按申请日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按申请日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "<div id=""w"" title=""高级检索""  style=""width:720px;height:520px;padding:5px;background: #fafafa;top:60px;display:none;""  class=""easyui-window"" closed=""true"" >" & vbcrlf & "<form method=""get"" action=""paysqdet.asp"" id=""date1"" onSubmit=""return Validator.Validate(this,2)"" name=""date22"" style=""margin:0px"">" & vbcrlf & "<div id=""hideDiv"" style=""border-style: none; border-width: 0px; border-color: inherit; margin: 0px; display:none; padding:0px;"">" & vbcrlf & "  <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:670px"">" & vbcrlf & "           <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "                         <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                         <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                   </div>" & vbcrlf & "                  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td width=""15%"" class=""func""><div align=""right"">申请主题</div></td>" & vbcrlf & "          <td width=""35%""><label>" & vbcrlf & "                  <input name=""title"" type=""text"" id=""title"">" & vbcrlf & "          </label>                </td>" & vbcrlf & "                  <td width=""15%""><div align=""right"">明细编号</div></td>" & vbcrlf & "                  <td width=""35%""><input name=""payid"" type=""text"" id=""payid""></td>" & vbcrlf & "           </tr>" & vbcrlf & "                     <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td width=""15%""><div align=""right"">明细备注</div></td>" & vbcrlf & "                      <td width=""35%""><input name=""intro"" type=""text"" id=""intro""></td>" & vbcrlf & "                <td width=""15%""><div align=""right"">申请金额</div></td>" & vbcrlf & "                <td width=""35%""><input name=""money1"" type=""text""  class=""easyui-numberbox"" id=""money1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision="""
	Response.write num_dot_xs
	Response.write """  onKeyUp=""value=value.replace(/[^ \d\.]/g,'');checkDot('money1','"
	Response.write num_dot_xs
	Response.write "')"" value=""0"">" & vbcrlf & "                  -" & vbcrlf & "                  <input name=""money12"" type=""text"" id=""money12"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision="""
	Response.write num_dot_xs
	Response.write num_dot_xs
	Response.write """  onKeyUp=""value=value.replace(/[^ \d\.]/g,'');checkDot('money12','"
	Response.write num_dot_xs
	Response.write "')"" value=""999999999""></td>" & vbcrlf & "                   </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""15%""><div align=""right"">费用分类</div></td>" & vbcrlf & "               <td colspan=""3"">"
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select * from paytype where del=1 order by gate2 desc"
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
	...
	set rs1=nothing
	Response.write "</td>" & vbcrlf & "               </tr>" & vbcrlf & "                       <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td><div align=""right"">审批状态</div></td>" & vbcrlf & "                        <td colspan=""3""> " & vbcrlf & "                           <input name=""e"" type=""checkbox"" id=""e""  value=""1"">审批通过&nbsp;" & vbcrlf & "                                 <input name=""e"" type=""checkbox"" id=""e""  value=""2"">审批中&nbsp;" & vbcrlf & "                                  <input name=""e"" type=""checkbox"" id=""e""  value=""0"">待审批&nbsp;" & vbcrlf & "                                  <input name=""e"" type=""checkbox"" id=""e""  value=""3"">审批未通过" & vbcrlf & "                               </td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""15%""><div align=""right"">申请时间</div></td>" & vbcrlf & "  <td width=""35%""><INPUT name=""rett"" size=""9"" readonly=""true"" id=""duepaydate1divPos"" onmouseup=toggleDatePicker(""duepaydate1div"",""date.rett"") value="""
	Response.write m1
	...
	Response.write m2
	Response.write """>" & vbcrlf & "                     <DIV id=""duepaydate2div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate2div""></DIV></td>" & vbcrlf & "                 <td width=""15%""><div align=""right"">币种</div></td>" & vbcrlf & "                 <td width=""35%""><select name=""listbz"">" & vbcrlf
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	set rs8=nothing
	...
	set rs8=nothing
	Response.write "" & vbcrlf & "                 </select></td>" & vbcrlf & "               </tr>" & vbcrlf & "                     <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td width=""15%""><div align=""right"">申请人员</div></td>" & vbcrlf & "                      <td colspan=""3"">"
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	if ZBRuntime.MC(1001) then
		if g mod 2=1 then
			...
			Response.write "<td width=""15%""><div align=""right"">关联客户</div></td>"
			Response.write "<td width=""35%""><input name=""stel"" type=""text"" id=""stel""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联客户</div></td>"
			Response.write "<td width=""35%""><input name=""stel"" type=""text"" id=""stel""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(2000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联联系人</div></td>"
			Response.write "<td width=""35%""><input name=""sperson"" type=""text"" id=""sperson""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联联系人</div></td>"
			Response.write "<td width=""35%""><input name=""sperson"" type=""text"" id=""sperson""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(3000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联项目</div></td>"
			Response.write "<td width=""35%""><input name=""schance"" type=""text"" id=""schance""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联项目</div></td>"
			Response.write "<td width=""35%""><input name=""schance"" type=""text"" id=""schance""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(7000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联合同</div></td>"
			Response.write "<td width=""35%""><input name=""scontract"" type=""text"" id=""scontract""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联合同</div></td>"
			Response.write "<td width=""35%""><input name=""scontract"" type=""text"" id=""scontract""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(9000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联售后</div></td>"
			Response.write "<td width=""35%""><input name=""sshouhou"" type=""text"" id=""sshouhou""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联售后</div></td>"
			Response.write "<td width=""35%""><input name=""sshouhou"" type=""text"" id=""sshouhou""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(31000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联日程</div></td>"
			Response.write "<td width=""35%""><input name=""sricheng"" type=""text"" id=""sricheng""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联日程</div></td>"
			Response.write "<td width=""35%""><input name=""sricheng"" type=""text"" id=""sricheng""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(17008) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联发货</div></td>"
			Response.write "<td width=""35%""><input name=""sfahuo"" type=""text"" id=""sfahuo""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联发货</div></td>"
			Response.write "<td width=""35%""><input name=""sfahuo"" type=""text"" id=""sfahuo""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(15000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联采购</div></td>"
			Response.write "<td width=""35%""><input name=""scaigou"" type=""text"" id=""scaigou""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联采购</div></td>"
			Response.write "<td width=""35%""><input name=""scaigou"" type=""text"" id=""scaigou""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(1002) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联供应商</div></td>"
			Response.write "<td width=""35%""><input name=""swork"" type=""text"" id=""swork""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联供应商</div></td>"
			Response.write "<td width=""35%""><input name=""swork"" type=""text"" id=""swork""></td>"
			Response.write "</tr>"
	...
	if g mod 2=1 then
		Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
		Response.write "<td width=""15%""><div align=""right"">关联借款单</div></td>"
		Response.write "<td width=""35%""><input name=""sjk"" type=""text"" id=""sjk""></td>"
		g=g+1
		Response.write "<td width=""35%""><input name=""sjk"" type=""text"" id=""sjk""></td>"
	else
		Response.write "<td width=""15%""><div align=""right"">关联借款单</div></td>"
	...
	if ZBRuntime.MC(16000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联采购退货</div></td>"
			Response.write "<td width=""35%""><input name=""scaigouth"" type=""text"" id=""scaigouth""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联采购退货</div></td>"
			Response.write "<td width=""35%""><input name=""scaigouth"" type=""text"" id=""scaigouth""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(8000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td width=""15%""><div align=""right"">关联销售退货</div></td>"
			Response.write "<td width=""35%""><input name=""scontractth"" type=""text"" id=""scontractth""></td>"
		else
			Response.write "<td width=""15%""><div align=""right"">关联销售退货</div></td>"
			Response.write "<td width=""35%""><input name=""scontractth"" type=""text"" id=""scontractth""></td>"
			Response.write "</tr>"
	...
	if g mod 2=1 then
		Response.write "<td width=""15%""><div align=""right""></div></td>"
		Response.write "<td width=""35%""></td>"
		Response.write "</tr>"
	end if
	Response.write "                        " & vbcrlf & "                        " & vbcrlf & "              </table>" & vbcrlf & "       </div>" & vbcrlf & "                  <div region=""south"" border=""false"" style=""padding:5px;text-align:right;height:27px;line-height:27px;"">" & vbcrlf & "                                <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                           <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                   </div>" & vbcrlf & "  </div>" & vbcrlf & "</form>" & vbcrlf & "  </div>" & vbcrlf & "" & vbcrlf & "<div style=""position: absolute; display:none;width: 500px;border: 0px;padding: 1px;z-index: 100;"" id=""dhtml""></div>" & vbcrlf & "<script src= ""../Script/py_paysqdet.js?ver="
	Response.write "</tr>"
	Response.write Application("sys.info.jsver")
