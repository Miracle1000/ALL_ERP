...
'if strW3<>"" Then
	...
	if typeid=1 then
		...
		if selectid="" then
			Response.write"<script language=javascript>alert('您没有选择任何记录，请选择后再删除！');history.back()</script>"
			call db_close : Response.end
		else
	...
	if open_77_3= 3 then
		Str_Result2=""
	elseif open_77_3= 1 then
		Str_Result2=" and (addcate in("&intro_77_3&")) "
	else
		Str_Result2=" and 1=2"
	end if
	G1=trim(request("G1"))
	...
	if trim(request("person"))<>"" then
		personid=deurl(trim(request("person")))
	else
	end if
	if trim(request("company"))<>"" then
		companyid=deurl(trim(request("company")))
	end if
...
'if m2<>"" then
	...
	if A1<>"" then
		Str_Result=Str_Result+" and  stact in('"&A1&"')"
'if A1<>"" then
		Str_Result2=Str_Result2+" and  stact in('"&A1&"')"
'if A1<>"" then
	end if
	if C2<>"" then
		if C1=1 then
...
'elseif G1=6 then
	...
	if companyid<>"" then
		Str_Result2=Str_Result2&" and person in(select ord from person where company="& companyid &" And Del=1)"
	end if
	DelAllAttach("select Access_url from email_recv_Access where del=1 and mailType=1 and email_ord in(select ord from email_log  where person=0 and isAccess is not null and isAccess=1 "&Str_Result2&" "&str_sql_checkid&")")
	conn.Execute("delete  email_recv_Access where del=1 and mailType=1 and email_ord in(select ord from email_log  where person=0 and isAccess is not null and isAccess=1 "&Str_Result2&" "&str_sql_checkid&")")
	sql="delete email_log  where person=0   "&Str_Result2&" "&str_sql_checkid&""
	conn.Execute(sql)
	havePersonNum=int(conn.execute ("select count(*) from email_log where person<>0 "&Str_Result2&" "&str_sql_checkid&"")(0))
	action1="邮件发送记录删除"
	call close_list(2)
	if havePersonNum>0 then
		call jsLocat("有"&int(havePersonNum)&"条记录有关联客户联系人，不允许删除","logList.asp?page_count="&page_count&"&currPage="& currpage&"&A2="&A2&"&A1="&A1&"&person="&personid&"&company="&companyid&"&S1="&S1&"&S2="&S2&"&T1="&T1&"&T2="&T2&"&G1="&G1&"&G2="&G2&"&D="&D&"&q="&q&"&F1="&F1&"&F2="&F2&"&E="&E&"&F="&F&"&C1="&C1&"&C2="&C2&"&W1="&W1&"&W2="&W2&"&W3="&W3&"&P1="&P1&"&P2="&P2&"&J1="&J1&"&J2="&J2&"&ret="&m1&"&ret2="&m2&"")
	else
		Response.redirect "logList.asp?page_count="&page_count&"&currPage="& currpage&"&A2="&A2&"&A1="&A1&"&person="&personid&"&company="&companyid&"&S1="&S1&"&S2="&S2&"&T1="&T1&"&T2="&T2&"&G1="&G1&"&G2="&G2&"&D="&D&"&q="&q&"&F1="&F1&"&F2="&F2&"&E="&E&"&F="&F&"&C1="&C1&"&C2="&C2&"&W1="&W1&"&W2="&W2&"&W3="&W3&"&P1="&P1&"&P2="&P2&"&J1="&J1&"&J2="&J2&"&ret="&m1&"&ret2="&m2&""
	end if
