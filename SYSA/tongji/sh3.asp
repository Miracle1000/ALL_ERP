...
'bigsystemtype = ".mozi"
	...
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=42 and sort2=11"
	...
	elseif open_42_11=1 then
		list_tj="and addcate in ("&intro_42_11&")"
	else
	...
	sorce2=0
	sorce3=0
	if open_42_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_42_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
	...
	Response.write "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "" & vbcrlf & "function callServer2() {" & vbcrlf & "  var url = ""../sort3/liebiao_Service.asp?timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "  updatePage2();" & vbcrlf & "  };" & vbcrlf & "  xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "function updatePage2() {" & vbcrlf & "var test7=""ht1""" & vbcrlf & "  if (xmlHttp.readyState < 4) {" & vbcrlf & " ht1.innerHTML=""loading..."";" & vbcrlf & "  }" & vbcrlf & "  if (xmlHttp.readyState == 4) {" & vbcrlf & "    var response = xmlHttp.responseText;" & vbcrlf & "      ht1.innerHTML=response;" & vbcrlf & " xmlHttp.abort();" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & ""
	...
	'Response.write Application("sys.info.jsver")
	dim st,m1,m1name,m2,m2name,E,F,F2
	m1=request("ret")
	m2=request("ret2")
	E=request("sort")
	F=request("sort1")
...
'if E<>"" then
		Str_Result2=Str_Result2+"and  result1='"&E&"'"
'if E<>"" then
	end if
	if F<>"" then
...
'if F<>"" then
	...
	if  F2<>"" then
		Str_Result2=Str_Result2+"and  sort1 in ("&Servicfl_list&")"
'if  F2<>"" then
	end if
	dim sort,sort1
	if E="0" then
	...
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">售后数量统计</td>" & vbcrlf & "                   " & vbcrlf & "            <td  align=""left""><strong>"
	Response.write sort
	Response.write sort1
	Response.write "人员对比"
	Response.write m1name
	Response.write m2name
	Response.write "" & vbcrlf & "    </strong>" & vbcrlf & "                 </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"" id=""zc"">" & vbcrlf & "<form method=""POST"" action=""sh3.asp?width_tj="
	...
	Response.write "<select name=""sort"">" & vbcrlf & "<option value="""" >处理结果</option>" & vbcrlf & "<option value=""0"" "
	if E="0" then
	...
	Response.write ">处理完毕</option>" & vbcrlf & "            </select>" & vbcrlf & "<!--select name=""sort1""  datatype=""Limit"" >" & vbcrlf & ""
	Response.write "selected"
	if F<>"" then
		...
		set rss=conn.execute("select id ,sort1 from sortonehy where  gate2=51 and ord<>"&rs7("id")&"  order by gate1 desc")
		while not rss.eof
			sele=""
			if int(sorce)=rss("id") then
				sele="selected"
			end if
		...
		rs7.close
		set rs7=nothing
		Response.write "" & vbcrlf & "<option value="""" >售后分类</option>" & vbcrlf & ""
	else
		Response.write "" & vbcrlf & "<option value="""" >售后分类</option>" & vbcrlf & ""
		set rss=conn.execute("select id ,sort1 from sortonehy where gate2=51  order by gate1 desc")
		while not rss.eof
	...
	Response.write "" & vbcrlf & "" & vbcrlf & "            </select-->" & vbcrlf & "            " & vbcrlf & ""
	set rss=nothing
	...
	Response.write "" & vbcrlf & "<input type=""submit"" name=""Submit422"" value=""统计""  class=""anybutton2""/>" & vbcrlf & "<!-- <a href=""javascript:void(0)"" onClick=""callServer2();document.getElementById('zc').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""red advanSearch"">高级检索</font></u></a> -->" & vbcrlf & "</td>" & vbcrlf & "  </tr></table>" & vbcrlf & "  <span id=""ht1""></span> </form>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr> "& vbcrlf &   "  <td height=""30""  align=""right"">" & vbcrlf &  "    <input type=""button"" name=""pillar"" value=""柱状图分析"" onClick=""window.location.href"
	Response.write F2
	Response.write "&type_tj=1&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "          <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""window.location.href='sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj=2&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""window.location.href='sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj=3&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj="
	Response.write type_tj
	Response.write "&sort="
	...
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj="
	Response.write type_tj
	Response.write "&sort="
	...
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj="
	Response.write type_tj
	Response.write "&sort="
	...
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj="
	Response.write type_tj
	Response.write "&sort="
	...
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""sh3.asp?F2="
	Response.write F2
	Response.write "&type_tj="
	Response.write type_tj
	Response.write "&sort="
	...
	if open_42_7=1 or open_42_7=3  then
		Response.write "<input type=""button"" name=""Submit43"" value=""打印""  onClick=""window.print()"" class=""anybutton2"">"
	end if
	Response.write "&nbsp;"
	if open_42_10=1 or open_42_10=3  then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='sh3.asp?F2="
		Response.write F2
		Response.write "&type_tj=4&sort="
		Response.write E
		Response.write "&sort1="
		Response.write F
		Response.write "&ret="
	...
	if counts<=0 then
		...
		do until rs1.eof
			set rs=server.CreateObject("adodb.recordset")
			sql="select ord  from  tousu  where addcate="&rs1("ord")&"  "&Str_Result2&" "
			rs.open sql,conn,1,1
			numcg1=rs.recordCount
			rs.close
			set rs=nothing
			title=rs1("name")
			lburl="../service/event.asp?fromTJ=1&W3="&rs1("ord")&"&E="&E&"&F="&F&"&ret="&m1&"&ret2="&m2&"&F2="&F2&"&lay=1"
			total1(j,1)=numcg1
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			total2(j,1)=numcg1
			total2(0,1)="#FF0000,1.5,1,2,售后分布"
			total2(j,2)=lburl
			total3(j,0)=title
			total3(j,1)=numcg1
			'total3(j,2)=lburl
