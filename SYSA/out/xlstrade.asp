...
'Str_Result=" where del=1 and sort3=1   "&str_temp_where&""
	...
	xlWorksheet.Cells(1,1).Value = "代码"
	xlWorksheet.Cells(1,2).Value = "客户行业"
	xlWorksheet.Range("A2:F2").Borders.LineStyle=1
	Response.Flush
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from sortonehy where gate2=11 "
	...
	do until rs.eof
		...
		ord=rs("ord")
		sort1=rs("sort1")
		xlWorksheet.Cells(1+i,1).Value = ord
		sort1=rs("sort1")
		xlWorksheet.Cells(1+i,2).Value = sort1
		sort1=rs("sort1")
		Call ClientClosedExit
		Response.write "<script>CountImage.width=" & Fix((i/C1) * 710) & ";" & VbCrLf
		Response.write "CountTXT.innerHTML=""共有<font color=red><b>"&C1&"</b></font>条数据!导出进度:<font color=red><b>" & CInt(FormatNumber(i/C1*100,4,-1)) & "</b></font>"";" & VbCrLf
		Response.write "<script>CountImage.width=" & Fix((i/C1) * 710) & ";" & VbCrLf
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户行业代码导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("trade.xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的客户行业代码对应表</u></strong></font></a></p> " & vbcrlf & ""
