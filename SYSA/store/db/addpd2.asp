...
'if StoreCapacity>0 then
		...
		sub page_load
			Response.write "" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & " function fMove(obj){ " & vbcrlf & "           if(obj.offsetWidth<197){" & vbcrlf & "                        obj.style.width = 197;" & vbcrlf & "          }" & vbcrlf & "       }" & vbcrlf & "       try{window.parent.document.getElementsByName(""I1"")[0].style.height = ""1100px"";}catch(e){}" & vbcrlf & "</script>" & vbcrlf & "<style type=""text/css"">html{overflow:auto;padding:0;}</style>" & vbcrlf & "<frameset cols='197,*' border=""10"" frameSpacing=10 frameborder=1 style='border:4px solid white' onload=''>" & vbcrlf & "                <frame src='addpd.asp?__msgId=ckcptreelist&musthasnum=2' frameborder=0 bordercolor=""#fefeff"" style='min-width:197px' onresizex='return fMove(this)'></iframe>" & vbcrlf & "               <frame src='?__msgId=addpdPage&"
'sub page_load
			Response.write request.querystring
			Response.write "'  frameborder=0></iframe>" & vbcrlf & "</frameset>" & vbcrlf & "" & vbcrlf & ""
		end sub
		Sub App_addpdPage
			dim sql , rs  ,  top , bs
			...
			if not rs.eof then
				...
				pddate = now
				If cn.execute("select 1 from inventoryCost WHERE datediff(mm,date1,'"& date &"')=0 and complete1 >= 1").eof=False Then
				...
				sql="Delete kupd where cateid="&session("personzbintel2007")&" and del=7"
				cn.Execute(sql)
				sql="Delete kupdlist where  addcate="&session("personzbintel2007")&" and del=7"
'cn.Execute(sql)
				set rs88 = cn.execute("EXEC erp_getdjbh 35,"&session("personzbintel2007"))
				pdno=rs88(0).value
				rs88.close
				set rs88=nothing
				sqlStr="Insert Into kupd(cateid,date7,del,pdbh) values('"
				sqlStr=sqlStr & session("personzbintel2007") & "',getDate(),7,'" & pdno & "')"
				cn.execute(sqlStr)
				dim rd
				set rsTmp=cn.execute("SELECT SCOPE_IDENTITY()")
				rd=rsTmp(0)
				rsTmp.close
				set rsTmp=nothing
				session("storepd_idzbintel")=rd
				top = rd
			end if
			rs.close
			session("zbintel_jqpdresubmit")=now()
			Response.write app.headhtml
			Response.write "<style>html{padding:0px}</style>"
			Response.write "" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "function TexTxmFocus(event){" & vbcrlf & "     event = event? event: window.event" & vbcrlf & "      if(!event) return;" & vbcrlf & "      var obj = event.srcElement ? event.srcElement:event.target; " & vbcrlf & "    if(!obj) return ;" & vbcrlf & "       if(obj.name==undefined){" & vbcrlf & "              var eo = null;" & vbcrlf & "          try{" & vbcrlf & "                    eo = document.getElementsByName(""txm"")[0];" & vbcrlf & "                        eo.focus();" & vbcrlf & "             }catch(e1){" & vbcrlf & "                     try{" & vbcrlf & "                            eo = parent.document.getElementsByName(""txm"")[0];" & vbcrlf & "                         eo.focus();" & vbcrlf & "                     }catch(e1){}" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "//判断是不是辅助数量触发的格式" & vbcrlf & "function checkDBNname(colname,InputDBName)" & vbcrlf & "{" & vbcrlf & "    if(colname==InputDBName && InputDBName==""实盘辅助数量"") return false;" & vbcrlf & "    return true;"& vbcrlf & "}" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "function txmAjaxSubmit(obj){" & vbcrlf & "  var TxmID=obj.value;" & vbcrlf & "    if (TxmID.length ==0){return;}" & vbcrlf & "  if (TxmID.indexOf(""："")>=0)" & vbcrlf & "       {" & vbcrlf & "               //多行文本内容，二维码文本编码" & vbcrlf & "          if( TxmID.indexOf(""流水号："")!=0) {return;}" & vbcrlf & " }" & vbcrlf & "       else if (TxmID.toLowerCase().indexOf(""view.asp?v"")>0)" & vbcrlf & "     {" & vbcrlf & "               //网址信息，可能是二维码URL编码" & vbcrlf & "         TxmID = ""QrUrl="" + TxmID.split(""view.asp?"")[1];" & vbcrlf & "     }" & vbcrlf & "       RegEventOnScanf(TxmID);" & vbcrlf & " obj.value = """";" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<body style='margin-top:0px;padding-top:0px;' onclick='TexTxmFocus(event);'>" & vbcrlf & "<style>" & vbcrlf & "       html {overflow:auto;}" & vbcrlf & "   textarea{ border:1px solid #ccc; }" & vbcrlf & "</style>" & vbcrlf & "      <script language=javascript src='../../Manufacture/inc/bill.js?ver="
			...
			Response.write top
			Response.write "' id='pdID'>" & vbcrlf & "  <table class=""resetBorderColor"" style='table-layout:fixed;border:1px solid #A7BBD7;border-top:0px;width:100%'>" & vbcrlf & "    <tr>" & vbcrlf & "            <td class='billfieldleft resetBorderColor' style='width:70px;border-left:1px solid #ccc'><pre style='display:inline'>  盘点主题： </pre></td>" & vbcrlf & "         <td class='billfieldright'>" & vbcrlf & "                     <table>" & vbcrlf & "                 <tr>" & vbcrlf & "                            <td><input type=text class=text style='height:17px' id='MT1' value='"
			Response.write tit
			...
			Response.write "'></td><td><button class=InselButton   value='0' onclick='Bill.showDateTimeDlg()'><img src='../../images/datePicker.gif'></button></td><td>&nbsp;<span class=c_red>*</span></td><td  id='MT3_msg'></td></tr></table>" & vbcrlf & "         </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class='billfieldleft'><pre style='display:inline'>  盘点备注： </td>" & vbcrlf & "          <td class='billfieldright' colspan=5>" & vbcrlf & "                   <textarea name=""intro""  type=""text"" id='MT4' class='text' rows=""4"" onpropertychange=""this.style.posHeight=this.scrollHeight>64?this.scrollHeight:64;"" style=""width:80%;height:64px;vertical-align:middle;margin:4px 0;"">"
			'Response.write pddate
			...
			ConversionUnitTactics=GetConversionUnitTactics()
			flag=1
			if ConversionUnitTactics=1 then flag=0
			fzunitShow=fzunit=1
			...
			lvw.canadd = False
			lvw.candr = true
			lvw.sql =   "exec erp_childlist_ck_jqpd " & top
...
'attrInx = 2
			lvw.cols.items(10+attrInx).edit = 0
'attrInx = 2
...
'attrInx = 2
			lvw.cols.items(12+attrInx).edit = flag
'attrInx = 2
			lvw.cols.items(12+attrInx).save = 1
'attrInx = 2
			lvw.cols.items(12+attrInx).BgColor = "【盈亏辅助数量】==0?'':(【盈亏辅助数量】>0?'#ff9933':'#cc3333')"
'attrInx = 2
			lvw.cols.items(12+attrInx).disztlr = 1
'attrInx = 2
			lvw.cols.items(12+attrInx).htmlvisible = fzunitShow
'attrInx = 2
			lvw.cols.items(13+attrInx).edit = 0
'attrInx = 2
			lvw.cols.items(13+attrInx).save = 1
'attrInx = 2
			lvw.cols.items(13+attrInx).htmlvisible = fzunitShow
'attrInx = 2
			lvw.cols.items(14+attrInx).edit = 1
'attrInx = 2
			lvw.cols.items(14+attrInx).save = 1
'attrInx = 2
			lvw.cols.items(14+attrInx).dtype = "storeprice"
'attrInx = 2
			lvw.cols.items(14+attrInx).lockformat = "【IsNoKu】==0 || 【盈亏数量】<=0"
'attrInx = 2
			lvw.cols.items(15+attrInx).edit = 0
'attrInx = 2
...
'attrInx = 2
			If cn.execute("select top 1 1 from power where ord=" & app.Info.User & " and sort1=35 and sort2=21 and qx_open=1").eof Then
				lvw.cols.items(14+attrInx).htmlvisible = false
				lvw.cols.items(15+attrInx).htmlvisible = false
			end if
			lvw.cols.items(16+attrInx).edit = 1
			lvw.cols.items(16+attrInx).save = 0
			lvw.cols.items(16+attrInx).selid = 1064
			lvw.cols.items(16+attrInx).lockformat = "【isNoKu】==0"
			lvw.cols.items(16+attrInx).disztlr = 0
			Set rsl = cn.Execute("Select num1 from setjm3 Where ord=5430")
			If rsl.eof = False Then
				If rsl("num1") = 1 Then
					lvw.cols.items(16+attrInx).disztlr = 1
'If rsl("num1") = 1 Then
				else
					lvw.cols.items(16+attrInx).disztlr = 0
'If rsl("num1") = 1 Then
...
'If rsl("num1") = 1 Then
			...
			'rsl.close
			lvw.cols.items(17+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(17+attrInx).save = 1
			'rsl.close
			lvw.cols.items(17+attrInx).resize = 0
			'rsl.close
			lvw.cols.items(18+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(18+attrInx).save = 1
			'rsl.close
			lvw.cols.items(18+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(19+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(19+attrInx).save = 1
			'rsl.close
			lvw.cols.items(19+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(20+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(20+attrInx).save = 1
			'rsl.close
			lvw.cols.items(20+attrInx).selid = 10002
			'rsl.close
			lvw.cols.items(20+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(21+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(21+attrInx).save = 1
			'rsl.close
			lvw.cols.items(21+attrInx).selid = 10002
			'rsl.close
			lvw.cols.items(21+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(22+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(22+attrInx).save = 1
			'rsl.close
			lvw.cols.items(22+attrInx).lockformat = "【isNoKu】==0"
			'rsl.close
			lvw.cols.items(22+attrInx).selid = 60
			'rsl.close
			lvw.cols.items(23+attrInx).edit = 1
			'rsl.close
			lvw.cols.items(23+attrInx).save = 1
			'rsl.close
			lvw.cols.items(24+attrInx).htmlvisible  =  false
			'rsl.close
			lvw.cols.items(24+attrInx).save = 1
			'rsl.close
			lvw.cols.items(24+attrInx).resize = 0
			'rsl.close
			lvw.cols.items(25+attrInx).save = 1
			'rsl.close
			lvw.cols.items(25+attrInx).edit = 0:
			'rsl.close
			lvw.cols.items(25+attrInx).htmlvisible = False
			'rsl.close
			lvw.cols.items(26+attrInx).save = 1
			'rsl.close
			lvw.cols.items(26+attrInx).edit = 0:
			'rsl.close
			lvw.cols.items(26+attrInx).htmlvisible = False
			'rsl.close
			lvw.cols.items(27+attrInx).save = 1
			'rsl.close
			lvw.cols.items(27+attrInx).edit = 0:
			'rsl.close
			lvw.cols.items(27+attrInx).htmlvisible = False
			'rsl.close
			lvw.cols.items(28+attrInx).save = 1
			'rsl.close
			lvw.cols.items(28+attrInx).edit = 0:
			'rsl.close
			lvw.cols.items(28+attrInx).htmlvisible = False
			'rsl.close
			lvw.formula = "【实盘数量】=【实盘数量】;【盈亏数量】=【实盘数量】-【账面数量】;【盈亏金额】=【盈亏数量】*【单价】;【单位ID】=【单位】;【单价】=【单价】;【实盘辅助数量】=【初始库存】==0?0:(【实盘数量】*【辅助数量】/【初始库存】);【盈亏辅助数量】=【实盘辅助数量】-【账面辅助数量】;":
			'rsl.close
			Response.write lvw.innerHTML
			Response.write "<br>" & vbcrlf & "                 </div>" & vbcrlf & "          </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <table style='table-layout:fixed;width:100%'>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class=""resetTransparent"" style='background-image:url(../../images/m_table_b.jpg);padding-top:0px;height:40px;line-height:40px;'>" & vbcrlf & "                  <table align=center style='width:300px;text-align:center;margin:0 auto;'>" & vbcrlf & "                       <tr>" & vbcrlf & "                            <td><button class=""button save-btn"" onclick='savepd(1)'>暂存盘点</button></td>" & vbcrlf & "                            <td><button class=""button save-btn"" onclick='savepd(3)'>结束盘点</button></td>" & vbcrlf & "                               <!-- <td><button class=button onclick='savepd(4)'>打印明细</button></td> -->" & vbcrlf & "                            <td><button class=button onclick='window.location.href=window.location.href'>重新填写</button></td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & " </tr>" & vbcrlf & "   </table>" & vbcrlf & "    <div style=""color:red"">温馨提示：盘亏出库，盘点的账面数量按同一个产品、单位和仓库自动求和，所以汇总盘亏出库成本无需手动录入，按照先进先出法扣减匹配库存和成本；<br>" & vbcrlf & "          &nbsp&nbsp&nbsp&nbsp&nbsp盘盈入库，以手动录入的单价作为盘点入库的成本单价；</div>" & vbcrlf & "        <br><br><br><br>" & vbcrlf & "        <script type=""text/javascript"">" & vbcrlf & "             document.body.style.paddingTop=""0px"";" & vbcrlf & "             function RegEventOnScanf(key){" & vbcrlf & "                  ajax.regEvent()" & vbcrlf & "                 ajax.regEvent(""addChildByXLH"");" & vbcrlf & "                   ajax.addParam(""xlh"",key);" & vbcrlf & "                 ajax.addParam(""hasKuArray"",GetHasKuIdArray())" & vbcrlf & "                        ajax.send(getnewlist);" & vbcrlf & "                  ajax.showprocc();" & vbcrlf & "               }" & vbcrlf & "       </script>" & vbcrlf & "       <script language=javascript src='../../Manufacture/inc/dateCalender.js?ver="
			Response.write Application("sys.info.jsver")
			Response.write "'></script><!-- 日期选择组件 -->" & vbcrlf & "     <script language=javascript>" & vbcrlf & "            ListViewInit();" & vbcrlf & "         function printmxlist(){" & vbcrlf & "                 " & vbcrlf & "                }" & vbcrlf & "               function savepd(sType){" & vbcrlf & "                 ajax.regEvent("""")" & vbcrlf & "                 for(var i=1;i<5;i++){" & vbcrlf & "                             if(i<4){document.getElementById(""MT"" + i + ""_msg"").innerHTML = """"}" & vbcrlf & "                            ajax.addParam(""MT"" + i,document.getElementById(""MT"" + i).value)" & vbcrlf & "                     }" & vbcrlf & "                       ajax.addParam(""tempsave"",sType)" & vbcrlf & "                   ajax.addParam(""dat"",lvw.GetSaveDetailData(document.getElementById('listview_pdlist')));" & vbcrlf & "                   ajax.addParam(""ID"","
			Response.write top
			Response.write ");" & vbcrlf & "            if (!window.ajaxbaseurl) { window.ajaxbaseurl = ajax.url; }" & vbcrlf & "            ajax.url = window.ajaxbaseurl.split(""/SYSA/"")[0] + ""/SYSN/view/store/kupd/addpd2.save.ashx"";" & vbcrlf & "                        r = ajax.send();" & vbcrlf & "                        if(r.indexOf(""Microsoft VBScript"") > 0){" & vbcrlf & "                             var div = document.createElement(""div"");" & vbcrlf & "                          div.lang = ""zh-cn""" & vbcrlf & "                                div.innerHTML = r;" & vbcrlf & "                              alert(div.innerText);" & vbcrlf & "                           return false;" & vbcrlf & "                   }" & vbcrlf & "                       if(r.indexOf(""msg="") >= 0){" & vbcrlf & "                               alert(r.replace(""msg="",""""));" & vbcrlf & "                           return false;" & vbcrlf & "                   }" & vbcrlf & "                       if(r.indexOf(""<"" + ""script"" + "">"") >= 0){" & vbcrlf & "                             window.parent.location.href=""../planall4.asp"";" & vbcrlf & "                    }" & vbcrlf & "                       else { try { eval(r) } catch (e) { alert(r) } }" & vbcrlf & "ajax.url = window.ajaxbaseurl;" & vbcrlf & "          }" & vbcrlf & "               window.top.TreeDataSelect = function(dat){" & vbcrlf & "                      var dats = dat.join(""$"")" & vbcrlf & "                  ajax.regEvent(""addChild"");" & vbcrlf & "                        ajax.addParam(""datArray"",dats);" & vbcrlf & "                   ajax.addParam(""hasKuArray"",GetHasKuIdArray())" & vbcrlf & "                 ajax.send(getnewlist);" & vbcrlf & "                  ajax.showprocc();" & vbcrlf & "               }" & vbcrlf & "               " & vbcrlf & "                window.lvwCellValue = function(v){" & vbcrlf & "                      v = v.split(lvw.sBoxSpr)" & vbcrlf & "                        if(v.length==2){" & vbcrlf & "                                return v[1].length>0 ? v[1] : v[0]" & vbcrlf & "                   }" & vbcrlf & "                       else{" & vbcrlf & "                           return v[0];" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "               function GetHasKuIdArray(){" & vbcrlf & "                     var r = """"" & vbcrlf & "                        var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    for(var i=0;i<div.hdataArray.length;i++){" & vbcrlf & "                     var row = div.hdataArray[i]" & vbcrlf & "                     var indexclo=6;//增加了字段导致列下标发生了变化" & vbcrlf & "                         r = r + ""|"" + lvwCellValue(row[3]) + "","" + lvwCellValue(row[7]) + "","" +lvwCellValue(row[21+indexclo]) + "","" +lvwCellValue(row[22+indexclo]) //IsNoku" &vbcrlf & "                        }" & vbcrlf & "                       return r" & vbcrlf & "                } " & vbcrlf & "              function GetHasKuIdArray1(){" & vbcrlf & "                    var r = """"" & vbcrlf & "                        var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    for(var i=0;i<div.hdataArray.length;i++){" & vbcrlf & "                           var row = div.hdataArray[i]" & vbcrlf & "                          var indexclo=6;//增加了字段导致列下标发生了变化" & vbcrlf & "                         r = r + ""|"" + lvwCellValue(row[3]) + "","" + lvwCellValue(row[7]) + "","" + lvwCellValue(row[8])+ "","" + lvwCellValue(row[9])+ "","" + lvwCellValue(row[14+indexclo]) + "","" +lvwCellValue(row[22+indexclo]) + "","" + lvwCellValue(row[9+indexclo]) + "","" + lvwCellValue(row[20+indexclo]) + "","" +lvwCellValue(row[21+indexclo]) + "","" +lvwCellValue(row[15+indexclo]) + "","" +lvwCellValue(row[16+indexclo]) + "","" +lvwCellValue(row[17+indexclo]) + "","" +lvwCellValue(row[18+indexclo]) + "","" +lvwCellValue(row[19+indexclo]) //IsNoku" & vbcrlf & "                  }" & vbcrlf & "                       return r" & vbcrlf & "                } " & vbcrlf & "" & vbcrlf & "              window.isRepeat = function(div,cells){ //判断是否充分" & vbcrlf & "                   " & vbcrlf & "                        for(var i=0;i<div.hdataArray.length;i++){" & vbcrlf & "                               var row = div.hdataArray[i]" & vbcrlf & "                             var ord = row[3] //产品编号" & vbcrlf & "                               var unt = row[7] //单位" & vbcrlf & "                         var ku  = row[16] //所在仓库" & vbcrlf & "                            var ok1 = (cells[14] == lvwCellValue(ku));" & vbcrlf & "                              var ok2 = (cells[1] == lvwCellValue(ord));" & vbcrlf & "                              var ok3 = (cells[7] == lvwCellValue(unt));" & vbcrlf & "                              //window.top.document.title = (ok1 + ' - ' + ok2 + ' - ' + ok3 + ' - ' + ok4 + ' - ' + ok5) +  "" ["" + cells[10] + ""] - ["" + cells[11] + ""] - ["" +  ph + ""]""" & vbcrlf & "                             if(ok1 == true && ok2 ==true && ok3 ==true){" & vbcrlf & "                                    return i" & vbcrlf & "                                }" & vbcrlf & "                       }" & vbcrlf & "       return -1;" & vbcrlf & "              }" & vbcrlf & "               window.getnewlist = function(r){" & vbcrlf & "                        var rowIndex = 0" & vbcrlf & "                        var hsCount = 0 , addCount = 0" & vbcrlf & "                  var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    var rows = r.split(""$="")" & vbcrlf & "                  for(var i = 0 ; i < rows.length ; i++){" & vbcrlf & "                           if(rows[i].length>1){" & vbcrlf & "                                   var cells = rows[i].split(""|-"")" & vbcrlf & "                                   addCount = addCount + 1" & vbcrlf & "                                 div.hdataArray[div.hdataArray.length] = (""+%;$++%;$+"" + cells.join(""+%;$+"")).split(""+%;$+"");  " & vbcrlf & "                                }" & vbcrlf & "                 }" & vbcrlf & "                       var PageStartIndex = div.hdataArray.length - div.PageSize + 1;" & vbcrlf & "                  PageStartIndex = PageStartIndex > 0 ? PageStartIndex : 1;" & vbcrlf & "                       div.setAttribute(""PageStartIndex"",PageStartIndex);" & vbcrlf & "                        var PageEndIndex = div.hdataArray.length;" & vbcrlf & "                   div.setAttribute(""PageEndIndex"",PageEndIndex)" & vbcrlf & "                     lvw.UpdateScrollBar(div);" & vbcrlf & "                       lvw.Refresh(div)" & vbcrlf & "                        if(hsCount > 0){" & vbcrlf & "                                alert(""有"" + hsCount + ""条记录已经存在，没有重复添加。"")" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "               window.getnewlist1 = function(r){" & vbcrlf & "                  var rowIndex = 0" & vbcrlf & "                        var hsCount = 0 , addCount = 0" & vbcrlf & "                  var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    div.hdataArray = [];" & vbcrlf & "                    var rows = r.split(""$="")" & vbcrlf & "                  for(var i = 0 ; i < rows.length ; i++){" & vbcrlf & "                         if(rows[i].length>1){" & vbcrlf & "                                   var cells = rows[i].split(""|-"")" & vbcrlf & "                                   addCount = addCount + 1" & vbcrlf & "                                 div.hdataArray[div.hdataArray.length] = (""+%;$++%;$+"" + cells.join(""+%;$+"")).split(""+%;$+"");" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf & "                       div.PageStartIndex = div.hdataArray.length - div.PageSize + 1" & vbcrlf & "                   div.PageStartIndex = div.PageStartIndex > 0 ? div.PageStartIndex : 1" & vbcrlf & "                    div.PageEndIndex = div.hdataArray.length" & vbcrlf & "                        lvw.UpdateScrollBar(div);" & vbcrlf & "                       lvw.Refresh(div)" & vbcrlf & "                    if(hsCount > 0){" & vbcrlf & "                                alert(""有"" + hsCount + ""条记录已经存在，没有重复添加。"")" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "               document.body.onload = function(){" & vbcrlf & "                      var buttons = document.getElementById(""listtoolbar_pdlist"").getElementsByTagName(""button"")" & vbcrlf & "                       for(var i = 0 ; i < buttons.length ; i ++){" & vbcrlf & "                             if(buttons[i].title==""导出表格(Excel)"" || buttons[i].title==""统计图示""){" & vbcrlf & "                                    buttons[i].style.display = ""none""" & vbcrlf & "                         }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "               window.onExcelDrSetTag = function() {" & vbcrlf & "                 return GetHasKuIdArray1();" & vbcrlf & "              }" & vbcrlf & "               //document.getElementById(""MT4"").fireEvent(""onpropertychange"")" & vbcrlf & "              $(document.getElementById(""MT4"")).trigger(""change"");" & vbcrlf & "    </script>" & vbcrlf & "</body>" & vbcrlf & ""
		end sub
		Function getFieldName(findex)
			...
			attrindex = 2
			Select Case findex
			Case 2+attrindex: getFieldName = "账面数量"
'Select Case findex
			Case 3+attrindex: getFieldName = "实盘数量"
'Select Case findex
			Case 4+attrindex: getFieldName = "盈亏数量"
'Select Case findex
			Case 5+attrindex+(cint(clos)-cint(afterclos)): getFieldName = "单价"
'Select Case findex
			Case 6+attrindex+(cint(clos)-cint(afterclos)): getFieldName = "盈亏金额"
'Select Case findex
			Case 7+attrindex+(cint(clos)-cint(afterclos)): getFieldName = "仓库名称"
'Select Case findex
			Case Else
...
'if len(MF(I))>1000 then
			...
			cn.execute "select 0 id,cast(0 as decimal(25,12)) num1 into #tempCheckNum"&vbCrLf&_
			"CREATE NONCLUSTERED INDEX [_dta_index_tempCheckNum_11_1678717938__K1_K2_K3_K5_K6_K4] ON [dbo].[#tempCheckNum]"&_
			"([id] ASC,[num1] ASC)WITH (SORT_IN_TEMPDB = OFF, DROP_EXISTING = OFF, ONLINE = OFF) ON [PRIMARY]"
			dim CheckSql:CheckSql="insert into #tempCheckNum(id,num1)values"
			dim ids:ids="0"
			dim cks:cks="0"
			for I = 0 to ubound(rows)
				rows(i) = split(rows(i),"#oc")
				if IsNumeric(rows(i)(0)) = false or len(rows(i)(0)&"") = 0 then exit for
				if ubound(rows(i)) <> cint(15)+cint(9) then
'if IsNumeric(rows(i)(0)) = false or len(rows(i)(0)&"") = 0 then exit for
					app.alert "明细资料超出预期。" & ubound(rows(i))
					f = true
					exit sub
				end if
				if IsNumeric(rows(i)(7+cint(clos)-cint(afterclos))) then
					exit sub
					if cdbl(rows(i)(7+cint(clos)-cint(afterclos)))>0 and instr(","&cks&",",","&rows(i)(7+cint(clos)-cint(afterclos))&",")=0 then
						exit sub
						cks =cks & "," & rows(i)(7+cint(clos)-cint(afterclos))
						exit sub
					end if
				end if
				if cdbl(rows(i)(14+(cint(clos)-cint(afterclos))))>0 and instr(","&ids&",",","&rows(i)(14+(cint(clos)-cint(afterclos)))&",")=0 then
					exit sub
					ids =ids & "," & rows(i)(14+(cint(clos)-cint(afterclos)))
					exit sub
				end if
				if i>0 and i Mod 1000=0 then
					cn.execute Left(CheckSql, Len(CheckSql) - 1)
'if i>0 and i Mod 1000=0 then
					CheckSql="insert into #tempCheckNum(id,num1)values"
				end if
				CheckSql=CheckSql&vbCrLf&"("&rows(i)(14+(cint(clos)-cint(afterclos)))&","&rows(i)(4)&"),"
'CheckSql="insert into #tempCheckNum(id,num1)values"
				for II = 0 to ubound(rows(i)) - (cint(2)+cint(afterclos))
'CheckSql="insert into #tempCheckNum(id,num1)values"
...
'If cdbl(rows(i)(5)) < 0  Then
					...
					if rows(i)(6)&""="" or not isnumeric(rows(i)(6)) then rows(i)(6)=0  end if
					if rows(i)(7)&""="" or not isnumeric(rows(i)(7)) then rows(i)(7)=0  end if
					if rows(i)(8)&""="" or not isnumeric(rows(i)(8)) then rows(i)(8)=0  end if
					if rows(i)(9)&""="" or not isnumeric(rows(i)(9)) then rows(i)(9)=0  end if
					if rows(i)(10)&""="" or not isnumeric(rows(i)(10)) then rows(i)(10)=0  end if
					if rows(i)(9)<0 then
						app.alert "第" & (I+1) & "行，实盘辅助数量不可以为负数!"
'if rows(i)(9)<0 then
						call db_close : Response.end
					end if
					if rows(i)(7)=0 and rows(i)(9)>0 then
						app.alert "第" & (I+1) & "行，辅助单位不可以为空!"
'if rows(i)(7)=0 and rows(i)(9)>0 then
						call db_close : Response.end
					end if
					If len(rows(i)(5+(cint(clos)-cint(afterclos)))&"")= 0 or isnumeric(rows(i)(5+(cint(clos)-cint(afterclos)))&"")=0 Then
						call db_close : Response.end
						app.alert "第" & (I+1) & "行，单价不能为空或非数值型的内容!"
'call db_close : Response.end
'call db_close : Response.end
					end if
					If cdbl(rows(i)(5+(cint(clos)-cint(afterclos)))&"")< 0  Then
						call db_close : Response.end
						app.alert "第" & (I+1) & "行，单价不能为负数!"
'call db_close : Response.end
'call db_close : Response.end
					end if
					If Len(rows(i)(7+(cint(clos)-cint(afterclos)))&"") = 0 or rows(i)(7+(cint(clos)-cint(afterclos)))="0" Then
						call db_close : Response.end
						app.alert "第" & (I+1) & "行，仓库不能为空!"
'call db_close : Response.end
'call db_close : Response.end
					end if
					if  ((cdbl(rows(i)(10))>0 and cdbl(rows(i)(6)))<0 or (cdbl(rows(i)(10))<0 and cdbl(rows(i)(6)))>0  or (cdbl(rows(i)(10))=0 and cdbl(rows(i)(6))<>0)  or (cdbl(rows(i)(6))=0 and cdbl(rows(i)(10))<>0)) and fzunit=1 and cdbl(rows(i)(7))>0  then
						app.alert "第" & (I+1) & "行，盈亏辅助数量与盈亏数量变化不一致!"
						call db_close : Response.end
					end if
					if (ii>3 and ii < 8+(cint(clos)-cint(afterclos)))  or ii = 12+(cint(clos)-cint(afterclos))  then
						call db_close : Response.end
						if not isnumeric(rows(i)(II)) then
							if ii = 12+(cint(clos)-cint(afterclos)) then
'if not isnumeric(rows(i)(II)) then
								rows(i)(II) = "$Data-NULL"
'if not isnumeric(rows(i)(II)) then
							else
								app.alert "第" & (I+1) & "行，"&getFieldName(ii)&"不正确。"
'if not isnumeric(rows(i)(II)) then
								f = true
								exit sub
							end if
						else
							If ii <> 12+(cint(clos)-cint(afterclos)) And Len(Split(rows(i)(II)&" ",".")(0))-1>12+(cint(clos)-cint(afterclos)) Then
								exit sub
								app.alert "第" & (I+1) & "行，"&getFieldName(ii)&"不正确。"
								exit sub
								f = true
					...
					if ii = 11+(cint(clos)-cint(afterclos)) or ii = 10+(cint(clos)-cint(afterclos)) then
						exit sub
						if not isdate(rows(i)(ii)) then
							rows(i)(ii) = "$Data-NULL"  'app.alert "第" & (I+1) & "行【C:" & ii & "=" & rows(i)(II)  & "】，日期格式不正确。
'if not isdate(rows(i)(ii)) then
						end if
					end if
				next
				If Len(Trim(rows(i)(15+(cint(clos)-cint(afterclos)))&""))=0 Then
'if not isdate(rows(i)(ii)) then
					app.alert  "第" & (I+1) & "行， 第"& (15+(cint(clos)-cint(afterclos))) &"列，数据不能为空。"
'if not isdate(rows(i)(ii)) then
					f = True
				ElseIf Not isnumeric(rows(i)(15+(cint(clos)-cint(afterclos)))) Or InStr(rows(i)(15+(cint(clos)-cint(afterclos))),".")>0 Then
					f = True
					app.alert  "第" & (I+1) & "行， 第"& (15+(cint(clos)-cint(afterclos))) &"列，数据类型必须为整型。"
					f = True
					f = True
				end if
				If Len(rows(i)(13+(cint(clos)-cint(afterclos)))) > 200 Then
					f = True
...
'ckid=ckrs("id")&""
			...
			checknumds.open "select a.id,isnull(num2,0)+(case when num2=0 then isnull(locknum,0) else 0 end) allnum from ku a inner join #tempCheckNum b on a.id in ("&ids&") and a.id=b.id and b.num1<>isnull(num2,0)+(case when num2=0 then isnull(locknum,0) else 0 end)", cn , 1, 1
'set checknumds = server.CreateObject("adodb.recordset")
			if not checknumds.eof then
				allnum=checknumds("allnum")
				idTempCheck=checknumds("id")
			else
				allnum=0
				idTempCheck=0
			end if
			for i = 0 to ubound(rows)
				if Instr(pCheck, rows(i)(7+cint(clos)-cint(afterclos))&"")=0 then
'for i = 0 to ubound(rows)
...
'for i = 0 to ubound(rows)
				...
				if cdbl(ordTempCheck)>0 and rows(i)(14+(cint(clos)-cint(afterclos)))=idTempCheck&"" then
					exit sub
					If FormatNumber(CDbl(allnum),app.Info.FloatNumber,-1,0,0) <> FormatNumber(CDbl(rows(i)(4)),app.Info.FloatNumber,-1,0,0) Then
						exit sub
						app.alert alerttxt & vbcrlf & "第" & (i+1) & "行，仓库数量"& FormatNumber(CDbl(allnum),app.Info.FloatNumber,-1,0,0) &"与账面数量"&  FormatNumber(CDbl(rows(i)(4)),app.Info.FloatNumber,-1,0,0) &"不符。"
						exit sub
						f = True
						checknumds.close
			...
			Set checknumds = nothing
			if f then exit sub
			dim PhXlhManage, phManage, xlhManage
			set conn = cn
			cn.BeginTrans
			set rs = server.CreateObject("adodb.recordset")
			...
			if bid <> 0 then
				ID = bid
				cn.execute "delete  from kupdlist where pd=" & bid & " and sort1 = 2"
			end if
			rs.open "select top 0 * from kupdlist",cn,1,3
			for I = 0 to ubound(rows)
				cells = rows(i)
				rs.addnew
				if request.form("tempsave") = "3" Then
					PhXlhManage = GetProductPhXlhManage(cells(0), cells(1))
					phManage = PhXlhManage(0)
					xlhManage = PhXlhManage(1)
					if phManage=1 and (cells(8+(cint(clos)-cint(afterclos)))="$Data-NULL" or cells(8+(cint(clos)-cint(afterclos)))="") then
						xlhManage = PhXlhManage(1)
						cn.rollbacktrans
						app.alert "第" & (I+1) & "行，批号必填"
						cn.rollbacktrans
						Exit Sub
					end if
					if xlhManage=1 and (cells(9+(cint(clos)-cint(afterclos)))="$Data-NULL" or cells(9+(cint(clos)-cint(afterclos)))="") then
						Exit Sub
						cn.rollbacktrans
						app.alert "第" & (I+1) & "行，序列号必填"
						cn.rollbacktrans
						Exit Sub
					end if
				end if
				ProductAttr1 = cells(2)
				if ProductAttr1&""="" then ProductAttr1 = 0
				ProductAttr2 = cells(3)
				if ProductAttr2&""="" then ProductAttr2 = 0
				rs.fields("ord").value = cells(0)
				rs.fields("unit").value = cells(1)
				rs.fields("ProductAttr1").value = ProductAttr1
				rs.fields("ProductAttr2").value = ProductAttr2
				rs.fields("num1").value = cells(4)
				rs.fields("num2").value = cells(5)
				rs.fields("num3").value = cells(6)
				rs.fields("intro").value = cells(5+(cint(clos)-cint(afterclos)))
'rs.fields("num3").value = cells(6)
				rs.fields("money1").value = cells(6+(cint(clos)-cint(afterclos)))
'rs.fields("num3").value = cells(6)
				rs.fields("AssistNum").value = cells(9)
				if cells(24)&""=""then cells(24)=0
				rs.fields("AssistUnit").value = cells(24)
				rs.fields("ku").value = cells(7+(cint(clos)-cint(afterclos)))
'rs.fields("AssistUnit").value = cells(24)
				if cells(8+(cint(clos)-cint(afterclos))) <> "$Data-NULL" then  rs.fields("ph").value = cells(8+(cint(clos)-cint(afterclos)))
'rs.fields("AssistUnit").value = cells(24)
				if cells(9+(cint(clos)-cint(afterclos))) <> "$Data-NULL" then  rs.fields("xlh").value = cells(9+(cint(clos)-cint(afterclos)))
'rs.fields("AssistUnit").value = cells(24)
				if cells(10+(cint(clos)-cint(afterclos))) <> "$Data-NULL" then
'rs.fields("AssistUnit").value = cells(24)
					if isdate(cells(10+(cint(clos)-cint(afterclos))))=false then
						rs.fields("AssistUnit").value = cells(24)
						cn.rollbacktrans
						app.alert "第" & (I+1) & "行，生产日期不是正确的日期格式"
						cn.rollbacktrans
						Exit Sub
					else
						rs.fields("datesc").value = cells(10+(cint(clos)-cint(afterclos)))
						Exit Sub
					end if
				end if
				if cells(11+(cint(clos)-cint(afterclos))) <> "$Data-NULL" then
'Exit Sub
					if isdate(cells(11+(cint(clos)-cint(afterclos))))=false then
'Exit Sub
						cn.rollbacktrans
						app.alert "第" & (I+1) & "行，有效期不是正确的日期格式"
						cn.rollbacktrans
						Exit Sub
					else
						rs.fields("dateyx").value = cells(11+(cint(clos)-cint(afterclos)))
						Exit Sub
					end if
				end if
				if cells(12+(cint(clos)-cint(afterclos))) <> "$Data-NULL" then  rs.fields("bz").value = cells(12+(cint(clos)-cint(afterclos)))
'Exit Sub
				rs.fields("price1").value = cells(13+(cint(clos)-cint(afterclos)))
'Exit Sub
				rs.fields("ckid").value = cells(14+(cint(clos)-cint(afterclos)))
'Exit Sub
				rs.fields("sort1").value = 2
				...
				rs.fields("IsNoKu").value = cells(15+(cint(clos)-cint(afterclos)))
'rs.fields("pd").value = ID
				rs.update
			next
			rs.close
			...
			if request.form("tempsave") = "3" Then
				if session("zbintel_jqpdresubmit")="" then
				...
				dim r : r=  AutoKuinKuout(cn, 63001 ,ID , app.info.user, 0) 'Response.redirect "../savelist6_1.asp?top="& ID &"&sort3=2"
				if len(r)>0 then
					cn.rollbacktrans
					app.alert r
					exit sub
				...
				Response.write "window.parent.AutoHandleToNet(63001, '"& kuoutkuins & ID &"', 'KuPd_InventoryCost_AddPd2');"
			end if
			cn.CommitTrans
			session("zbintel_jqpdresubmit")=""
			Response.write "window.parent.location.href='../planall4.asp';"
		end sub
		Sub App_addChildByXLH
			dim rs , xlh , productOrd , scanSQL
			xlh = request.form("xlh")
			productOrd=0
			If InStr(xlh, "流水号：")=1 Then
				If InStr(Replace(xlh, "流水号：", ""), "B")=1 Then
					productOrd=mid(Replace(xlh, "流水号：", ""),7,len(Replace(xlh, "流水号：", ""))-6)
'If InStr(Replace(xlh, "流水号：", ""), "B")=1 Then
				else
					If isnumeric(Replace(xlh, "流水号：", "")) = true Then
						productOrd = CLng("0" & Replace(xlh, "流水号：", ""))
					end if
				end if
				scanSQL = " j.product= " & productOrd & " "
			ElseIf InStr(xlh, "QrUrl=V") = 1 Then
				xlh = Replace(xlh, "QrUrl=V", "")
				Set rs = cn.execute("select sourceID from C2_CodeItems  where id=" & clng("0" & xlh))
				If rs.eof = False Then
					productOrd = rs(0).value
				end if
				rs.close
				scanSQL = " j.product= " &  CLng("0" & productOrd) & " "
			else
				scanSQL = " (a.xlh='" & replace(xlh,"'","") & "' or j.txm = '" & replace(xlh,"'","") &"' )  "
			end if
			dat = request.form("hasKuArray")
			dats = split(dat,"|")
			cn.execute("select 10000 as kuid into #ttt2;delete from #ttt2")
			set rs = server.CreateObject("adodb.recordset")
			rs.open "select kuid from #ttt2" , cn , 1 ,3
			for i = 1 to ubound(dats)
				if len(dats(i)) > 0 then
					if len(dats(i))>1 Then
						rs.addnew
						cells = split(dats(i),",")
						If isnumeric(cells(2)) = False Or Len( cells(2)) = 0 then
							rs.fields(0).value = 0
						else
							rs.fields(0).value = cells(2)
						end if
						rs.update
					else
						app.alert "验证数据有误。请重试操作\n\n第" & I & "行(" & dats(i) & ")"
						exit sub
					end if
				end if
			next
			...
			"  c.sort1+ '^tag~' + cast(j.unit as varchar(12)) as 单位,  " & vbcrlf &_
			"  b.type1 as 型号,  " & vbcrlf &_
			"  j.unit as 单位ID," & vbcrlf &_
			...
			"  j.unit as 单位ID," & vbcrlf &_
			"  cast(num2 as varchar(50)) as 账面数量,  " & vbcrlf &_
			"  cast(num2 as varchar(50)) as 实盘数量,  " & vbcrlf &_
			"  cast(0 as varchar(50)) as 盈亏数量 ," & vbcrlf &_
			"  isnull(cc.sort1,'')+ '^tag~' + cast(isnull(a.AssistUnit,0) as varchar(12)) as 辅助单位,  " & vbcrlf &_
			"  cast(0 as varchar(50)) as 盈亏数量 ," & vbcrlf &_
			"  cast(0 as varchar(50)) as 账面辅助数量 ," & vbcrlf &_
			"  cast(0 as varchar(50)) as 实盘辅助数量 ," & vbcrlf &_
			"  cast(0 as varchar(50)) as 盈亏辅助数量 ," & vbcrlf &_
			"  isnull((select top 1 y.price1 from ku y inner join kuinlist zz on y.kuinlist=zz.id and zz.sort1=1 where y.ord=b.ord  and y.unit=a.unit and y.ck=a.ck order by y.id desc),dbo.erp_getProductPrice(b.ord,isnull(a.unit,b.unitjb),63))  as 单价,  " & vbcrlf &_
			"  cast(0 as varchar(50)) as 盈亏金额,  " & vbcrlf &_
			"  d.sort1 as 仓库名称,  " & vbcrlf &_
			"  a.ck as 仓库ID," & vbcrlf &_
			"  a.ph as 批号," & vbcrlf &_
			"  a.xlh as 序列号," & vbcrlf &_
			"  a.datesc as 生产日期," & vbcrlf &_
			"  a.dateyx as 有效期," & vbcrlf &_
			"  isnull((select top 1 sort1 from sortonehy where gate2=82 and ord=a.bz),'') + '^tag~' + cast(a.bz as varchar(10)) as 包装," & vbcrlf &_
			"  a.dateyx as 有效期," & vbcrlf &_
			"  '' as 备注,a.id as kuid,  " & vbcrlf &_
			"  0 as isNoKu,a.AssistNum as 辅助数量,a.num3 as 初始库存 " & vbcrlf &_
			"from jiage j "&_
			"inner join sortonehy c on c.gate2=61 and c.ord=j.unit and j.bm=0 " &_
			"inner join product b on j.product = b.ord " &_
			"inner join ku a on a.unit = j.unit and a.ord= j.product and (a.num2+isnull(locknum,0))<>0 "&_
			"inner join product b on j.product = b.ord " &_
			"left join sortonehy cc on cc.gate2=61 and cc.ord=isnull(a.AssistUnit,0) " &_
			"left join sortck d on d.ord=a.ck  and (cast(d.intro as varchar(20))='0' or  charindex('," & app.info.user & ",',',' + replace(cast(d.intro as varchar(4000)),' ','') + ',') > 0) " & vbcrlf &_
			"left join sortonehy cc on cc.gate2=61 and cc.ord=isnull(a.AssistUnit,0) " &_
			" left join Shop_GoodsAttr s1 on s1.id = a.ProductAttr1  "& vbcrlf &_
			" left join Shop_GoodsAttr s2 on s2.id = a.ProductAttr2  "& vbcrlf &_
			"where "& scanSQL &" and a.id not in (select kuid from #ttt2) "
			set rs = cn.execute(sql)
			while not rs.eof
				for i = 0 to rs.fields.count-1
'while not rs.eof
					sValue=rs.fields(i).value
					If (i>=8 and i<=10) or (i>=12 and i<=14) Then
...
'ElseIf i=15 or i=16 Then
		...
		sub App_addChild
			...
			cn.execute("select 10000 as cpID , 10000 as ckID,100 as IsNoKu into #ttt;delete from #ttt;select 100 as ord, 100 as unit, 10000 as kuid,100 as IsNoKu into #ttt2;delete from #ttt2")
			cn.cursorLocation = 3
			set rs = server.CreateObject("adodb.recordset")
			dats = split(dat,"$")
			...
			dat = request.form("hasKuArray")
			dats = split(dat,"|")
			rs.open "select ord, unit,kuid, isNoKu from #ttt2" , cn , 1 ,3
			for i = 1 to ubound(dats)
				if len(dats(i)) > 0 then
					if len(dats(i))>1 Then
						rs.addnew
			...
			If Not rs_s.eof Then
				sql2 = "   isnull(d.sort1,(select sort1 from sortck where id = (select top 1 MainStore from jiage where product = b.ord and unit = b.unitjb))) as 仓库名称,  "
				sql2 = sql2&"      isnull(a.ck,(select id from sortck where id = (select top 1 MainStore from jiage where product = b.ord and unit = b.unitjb))) as 仓库ID,"
			else
			...
			"   isnull(s2.title,'') + '^tag~' + cast(isnull(a.ProductAttr2,0) as varchar(12)) 产品属性2, " & vbcrlf &_
			"  isnull(a.unit,b.unitjb) as 单位ID," & vbcrlf &_
			"  (case when isnull(sr.id,0)=0 then isnull((a.num2+isnull(locknum,0)),0) else 1 end) as 账面数量,  " & vbcrlf &_
			"  isnull(a.unit,b.unitjb) as 单位ID," & vbcrlf &_
			"  (case when isnull(sr.id,0)=0 then isnull((a.num2+isnull(locknum,0)),0) else 1 end) as 实盘数量,  " & vbcrlf &_
			"  isnull(a.unit,b.unitjb) as 单位ID," & vbcrlf &_
			"  0 as 盈亏数量 ," & vbcrlf &_
			"  (select sort1 from sortonehy c where gate2=61 and c.ord=isnull(a.AssistUnit,0)) + '^tag~' + cast(isnull(a.unit,b.unitjb) as varchar(12)) as 辅助单位,  " & vbcrlf &_
			"  0 as 盈亏数量 ," & vbcrlf &_
			"  (case when isnull(sr.id,0)=0 then isnull((a.num2+isnull(locknum,0)),0) else 1 end) * a.AssistNum/a.num3 as 账面辅助数量 ," & vbcrlf &_
			"  0 as 盈亏数量 ," & vbcrlf &_
			"  (case when isnull(sr.id,0)=0 then isnull((a.num2+isnull(locknum,0)),0) else 1 end) * a.AssistNum/a.num3 as 实盘辅助数量 ," & vbcrlf &_
			"  0 as 盈亏数量 ," & vbcrlf &_
			"  0 as 盈亏辅助数量 ," & vbcrlf &_
			"  isnull((select top 1 y.price1 from ku y inner join kuinlist zz on y.kuinlist=zz.id and zz.sort1=1 where y.ord=b.ord  and y.unit=a.unit and y.ck=a.ck order by y.id desc),dbo.erp_getProductPrice(b.ord,isnull(a.unit,b.unitjb),63))  as 单价,  " & vbcrlf &_
			"  0 as 盈亏金额,  " & vbcrlf &_
			"   "&sql2&" " & vbcrlf &_
			"  a.ph as 批号," & vbcrlf &_
			"  (case when isnull(sr.id,0)=0 then a.xlh else nl.serinum end) as 序列号," & vbcrlf &_
			"  a.datesc as 生产日期," & vbcrlf &_
			"  a.dateyx as 有效期," & vbcrlf &_
			"  isnull((select top 1 sort1 from sortonehy where gate2=82 and ord=a.bz),'') + '^tag~' + cast(a.bz as varchar(10)) as 包装," & vbcrlf &_
			"  a.dateyx as 有效期," & vbcrlf &_
			"  '' as 备注, isnull(a.id,0) as kuid,  " & vbcrlf &_
			"  (case when sign(isnull(a.ck,0))<>1 then isnull(e.isNoku,0) else 1 end) as isNoku, " & vbcrlf &_
			"   a.AssistNum as 辅助数量,a.num3 as 初始库存,a.AssistUnit AssistUnit "  & vbcrlf &_
			" from #ttt e "&_
			" inner join product b on e.cpId=b.ord " & vbcrlf &_
			" left join ku a on a.ord = b.ord and e.ckid = a.ck and (a.num2+isnull(locknum,0))<>0 " & vbcrlf &_
			" inner join product b on e.cpId=b.ord " & vbcrlf &_
			" left join S2_SerialNumberRelation sr with(nolock)  on sr.Billtype = 61001 and sr.listid = a.kuinlist and sr.del<>2 " & vbcrlf &_
			" left join M2_SerialNumberList nl with(nolock) on nl.id = sr.serialID " & vbcrlf &_
			" left join sortck d on d.ord=a.ck " & vbcrlf &_
			" left join Shop_GoodsAttr s1 on s1.id = a.ProductAttr1  "& vbcrlf &_
			" left join Shop_GoodsAttr s2 on s2.id = a.ProductAttr2  "& vbcrlf &_
			" where not exists(select * from #ttt2 x where x.ord = isnull(b.ord,0) and x.unit = isnull(a.unit,b.unitjb) and x.kuid=isnull(a.id,0)) " & vbcrlf &_
			" order by b.date7 desc, isnull(a.unit,b.unitjb) asc,daterk asc"
			set rs = cn.execute(sql)
			Dim sValue
			while not rs.eof
				for i = 0 to rs.fields.count-1
'while not rs.eof
					sValue=rs.fields(i).value
					If i=8 Or i=9 Or i=10 or i=12 or i=13 or i=14 Then
						if sValue&""=""then
							sValue=0
						end if
						sValue = FormatNumber(sValue,app.Info.FloatNumber,-1,0,0)
		...
		sub App_ListDrConfig(drdat)
			dim mFilePath
			mFilePath = "范例1=../../in/example_jqpddr1.xls|范例2=../../in/example_jqpddr2.xls"
			if isOpenProductAttr then mFilePath = "范例1=../../in/example_jqpddr3.xls|范例2=../../in/example_jqpddr2.xls"
			with drdat
			.title = "精确盘点导入"
			.fileName = "精确盘点数据"
			.filters = "xls|xlsx"
			.smpFilePath = mFilePath
			.helpFilePath = "../../in/jqpddr.doc"
...
'drmodel = 1
			...
			If drmodel = 1 Then
				...
				db  = uploader.dbname
				if not uploader.CheckFields("产品名称,产品编号,产品型号,单位,"&mFields&"批号,序列号,生产日期,有效日期,实盘数量,仓库名称,备注") then
					exit sub
				end if
...
'cn.execute "delete from " & db & " where datalength(rtrim(isnull(cast(产品名称 as nvarchar(500)),''))) = 0" , n
				...
				f_Num = f_Num + n2
'cn.execute "delete from " & db & " where datalength(isnull(产品名称,'')) > 1000" , n2
				cn.execute "update " & db & " set 产品名称=cast(ord as nvarchar(50)), 产品型号='#s_xx_tok' "&_
				"   from product x "&_
				"   where x.del=1 and x.title = isnull(cast(产品名称 as nvarchar(500)),'') and x.order1 = isnull(cast(产品编号 as nvarchar(500)),'') and x.type1=isnull(cast(产品型号 as nvarchar(500)),'')"
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '此产品不存在' from " & db & " where isnull(cast(产品型号 as nvarchar(500)),'') <> '#s_xx_tok'"
				cn.execute "delete from " & db & " where isnull(cast(产品型号 as nvarchar(50)),'') <> '#s_xx_tok'" , n1
				f_Num = f_Num + n1
'cn.execute "delete from " & db & " where isnull(cast(产品型号 as nvarchar(50)),'') <> '#s_xx_tok'" , n1
...
'cn.execute "delete from " & db & " where cast(cast(实盘数量 as nvarchar(15)) as float) < 0" , n1
				...
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '批号长度必须在1-50字之间' from " & db & " where datalength(isnull(批号,''))>100"
				cn.execute "delete from " & db & " where datalength(isnull(批号,''))>100" , n
				f_Num = f_Num + n
'cn.execute "delete from " & db & " where datalength(isnull(批号,''))>100" , n
				cn.execute "insert into #k_all (内容,说明) values ('检测批号合法性','" & app.iif((n)> 0 ,"共<b style=""color:red"">" & (n) & "</b>条记录批号不正确。","") & "')"
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '生产日期格式不正确' from " & db & " where isdate(生产日期)=0 and isnull(生产日期,'')<>''"
				cn.execute "delete from " & db & "  where  isdate(生产日期)=0 and isnull(生产日期,'')<>''" , n
				f_Num = f_Num + n
'cn.execute "delete from " & db & "  where  isdate(生产日期)=0 and isnull(生产日期,'')<>''" , n
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '有效日期格式不正确' from " & db & " where isdate(有效日期)=0 and isnull(有效日期,'')<>''"
				cn.execute "delete from " & db & "  where  isdate(有效日期)=0 and isnull(有效日期,'')<>''" , n1
				f_Num = f_Num + n1
'cn.execute "delete from " & db & "  where  isdate(有效日期)=0 and isnull(有效日期,'')<>''" , n1
				cn.execute "insert into #k_all (内容,说明) values ('检测日期数据合法性','" & app.iif((n + n1)> 0 ,"共<b style=""color:red"">" & (n+n1) & "</b>条记录日期不正确。","") & "')"
'cn.execute "delete from " & db & "  where  isdate(有效日期)=0 and isnull(有效日期,'')<>''" , n1
...
'if isOpenProductAttr then
				...
				"and cast(x.单位 as nvarchar(500)) = cast(y.单位 as nvarchar(500))" & "attrText" &_
				"                       and isnull(cast(x.序列号 as nvarchar(50)),'') = isnull(cast(y.序列号 as nvarchar(50)),'')  "&_
				"                       and isnull(cast(x.批号 as nvarchar(50)),'') = isnull(cast(y.批号 as nvarchar(50)),'')  "&_
				"                       and isnull(cast(x.生产日期 as nvarchar(500)),'') = isnull(cast(y.生产日期 as nvarchar(500)),'') "&_
				"                       and isnull(cast(x.有效日期 as nvarchar(500)),'') = isnull(cast(y.有效日期 as nvarchar(500)),'') and x.up_index<>y.up_index "&_
				"where  y.up_index>0 "
				...
				f_Num = f_Num + n
				cn.execute "delete from " & db & " where up_index in (select distinct up_index from #overlist)" , n
				cn.execute "insert into #k_all (内容,说明) values ('检测产品是否重复','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录产品重复","") & "')"
				cn.execute("drop table #overlist")
				cn.execute("select top 0 10000 as ord, 10000 as unit , 10000 as ProductAttr1, 10000 as ProductAttr2,10000 as ku, 1 as isNoku, cast(100000 as numeric(20,"&num_dot_xs&")) as num, cast('' as nvarchar(500)) as remark,10000 as kuid,cast('' as nvarchar(50)) as ph ,cast('' as nvarchar(50)) as xlh ,cast('' as nvarchar(50)) as scrq,cast('' as nvarchar(50)) as yxrq,cast('' as nvarchar(50)) as bz ,10000 as px1 into #ttt2")
				cn.cursorLocation = 3
				set rs = server.CreateObject("adodb.recordset")
...
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(6+ attrInx).value = cells(6+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(7+ attrInx).value = cells(7+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(8+ attrInx).value = cells(8+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(9+ attrInx).value = cells(9+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(10+ attrInx).value = cells(10+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(11+ attrInx).value = cells(11+ attrInx)
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.fields(12+ attrInx).value = i+1
'rs.fields(3).value = app.iif(Len(cells(3))=0, 0, cells(3))
						rs.update
					end if
				next
				rs.close
				cn.execute "select up_index into #tlist  "&_
				"from " & db & " a  "&_
				"inner join #ttt2 b on cast(a.产品名称 as nvarchar(50)) = cast(b.ord as nvarchar(50))  "&_
				"                       and cast(a.仓库名称 as nvarchar(50))=cast(b.ku as nvarchar(50))  "&_
				"                       and cast(a.单位 as nvarchar(50))=cast(b.unit as nvarchar(50))  "& attrText2 &_
				"                       and isnull(cast(a.序列号 as nvarchar(50)),'')=isnull(cast(b.xlh as nvarchar(50)),'')   "&_
				"                       and isnull(cast(a.批号 as nvarchar(50)),'')=isnull(cast(b.ph as nvarchar(50)),'')   "&_
				"                       and isnull(cast(a.生产日期 as datetime),'')=isnull(cast(b.scrq as datetime),'')   "&_
				"                       and isnull(cast(a.有效日期 as datetime),'')=isnull(cast(b.yxrq as datetime),'') "
				cn.execute "insert into #k_fail (行号,失败原因) "&_
				"select up_index, '此产品库存已存在，不可重复导入'  from #tlist "
				cn.execute "delete from " & db & " where up_index in (select up_index from #tlist )" , n
				f_Num = f_Num + n
				cn.execute "delete from " & db & " where up_index in (select up_index from #tlist )" , n
				cn.execute "insert into #k_all (内容,说明) values ('检测重复导入','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录产品重复导入","") & "')"
				cn.execute("drop table #tlist ")
...
'cn.execute("drop table #tlist ")
				...
				cn.execute "insert into #ttt2 (ord,unit,ProductAttr1,ProductAttr2,ku,isNoku,num,remark,px2,kuid,ph,xlh,yxrq,scrq,px1) "&_
				"   select cast(e.产品名称 as nvarchar(50)),cast(e.单位 as nvarchar(50)),cast(e.产品属性1 as nvarchar(50)),cast(e.产品属性2 as nvarchar(50)),cast(e.仓库名称 as nvarchar(50)),case when a.ord is null then 1 else 0 end,"&_
				"       cast(e.实盘数量 as nvarchar(15)),isnull(cast(e.备注 as nvarchar(500)),''),up_index,0,cast(e.批号 as nvarchar(50)),cast(e.序列号 as nvarchar(50)),e.有效日期,e.生产日期,0 "&_
				"   from "&db&" e "&_
				"   left join ku a on a.unit > 0 and (a.num2+isnull(locknum,0))<>0 "&_
				"   from "&db&" e "&_
				"       and cast(a.ord as nvarchar(50))=cast(e.产品名称 as nvarchar(50))"&_
				"       and cast(e.仓库名称 as nvarchar(50))=cast(a.ck as nvarchar(50)) "&_
				"       and cast(a.unit as nvarchar(50))=cast(e.单位 as nvarchar(50)) "& attrText3  &_
				"       and cast(a.ph as nvarchar(50))=cast(e.批号 as nvarchar(50)) "&_
				"       and cast(a.xlh as nvarchar(50))=cast(e.序列号 as nvarchar(50)) "&_
				"       and a.datesc=e.生产日期 and a.dateyx=e.有效日期 "&_
				"   group by cast(e.单位 as nvarchar(50)),cast(e.实盘数量 as nvarchar(15)),cast(e.仓库名称 as nvarchar(50)),cast(e.产品名称 as nvarchar(50)),a.ord,isnull(cast(e.备注 as nvarchar(500)),''),up_index,"&_
				"       cast(e.批号 as nvarchar(50)),cast(e.序列号 as nvarchar(50)),e.有效日期,e.生产日期"
				Set rs_s = server.CreateObject("adodb.recordset")
				sql_s = "Select num1 from setjm3 Where ord=5430 and num1=1"
				rs_s.open sql_s,cn,1,1
				If Not rs_s.eof Then
					sql2 = "    isnull(d.sort1,(select sort1 from sortck where id = (select top 1 MainStore from jiage where product = b.ord and unit = b.unitjb))) as 仓库名称,  "
					sql2 = sql2&"       isnull(a.ck,(select id from sortck where id = (select top 1 MainStore from jiage where product = b.ord and unit = b.unitjb))) as 仓库ID,"
					sql3 = " ,仓库名称 = isnull((select sort1 from sortck where ord = ku and ord in(select top 1 MainStore from jiage where product = 产品ID and unit = 单位ID ) or ord in(select top 1 StoreID from ProductStoreBinding where ProductID = 产品ID and Unit = 单位ID )),'') "
				else
					sql2 = "    d.sort1 as 仓库名称,  "
					sql2 = sql2&"       a.ck as 仓库ID,"
					sql3 = " ,仓库名称 = isnull((select sort1 from sortck where ord = ku),'') "
				end if
				rs_s.close
				sql =        "select  " & vbcrlf &_
				"   b.title as 产品名称,  " & vbcrlf &_
				...
				"   isnull(a.unit,e.unit) as 单位ID," & vbcrlf &_
				"   isnull((a.num2+isnull(locknum,0)),0) as 账面数量,  " & vbcrlf &_
				"   isnull(a.unit,e.unit) as 单位ID," & vbcrlf &_
				"   e.num as 实盘数量,  " & vbcrlf &_
				"   cast(0 as decimal(24,8)) as 盈亏数量 ," & vbcrlf &_
				"   isnull(a.price1,0) as 单价,  " & vbcrlf &_
				"   cast(0 as decimal(38,8)) as 盈亏金额,  " & vbcrlf &_
				"   "&sql2&" " & vbcrlf &_
				"  e.ph as 批号," & vbcrlf &_
				"  e.xlh as 序列号," & vbcrlf &_
				"  e.scrq as 生产日期," & vbcrlf &_
				"  e.yxrq as 有效期," & vbcrlf &_
				"  isnull((select top 1 sort1 from sortonehy where gate2=82 and ord=e.bz),'') + '^tag~' + cast(e.bz as varchar(10)) as 包装," & vbcrlf &_
				"  e.yxrq as 有效期," & vbcrlf &_
				"  e.remark as 备注, isnull(a.id,0) as kuid,  " & vbcrlf &_
				"  (case when sign(isnull(a.ck,0))<>1 then isnull(e.isNoku,0) else 1 end) as isNoku,e.ku,e.px2,e.px1 " & vbcrlf &_
				"  into #ttt3 " & vbcrlf &_
				"from #ttt2 e inner join product b on e.ord=b.ord and b.del=1 " & vbcrlf &_
				" left join ku a on a.ord = b.ord and e.ku = a.ck and (a.num2+isnull(locknum,0))<>0 and isnull(a.xlh,'') = isnull(e.xlh,'') "& attrText4 &_
				"from #ttt2 e inner join product b on e.ord=b.ord and b.del=1 " & vbcrlf &_
				"                and isnull(a.ph,'') = isnull(e.ph,'') and isnull(a.datesc,'') = isnull(e.scrq,'') and isnull(a.dateyx,'') = isnull(e.yxrq,'') " & vbcrlf &_
				" left join sortck d on d.ord=a.ck "&_
				" left join Shop_GoodsAttr s1 on s1.id = e.ProductAttr1  "&_
				" left join Shop_GoodsAttr s2 on s2.id = e.ProductAttr2  "
				cn.execute(sql)
				...
				Set rs = cn.execute ("select 产品名称,产品ID,产品编号,型号,单位,单位ID,产品属性1,产品属性2,账面数量,实盘数量,盈亏数量,单价,盈亏金额,仓库名称,仓库ID,批号,序列号,生产日期,有效期,包装,备注,kuid,isNoku from #ttt3 order by px2,px1")
				while not rs.eof
					for i = 0 to rs.fields.count-1
'while not rs.eof
						sValue=rs.fields(i).value
...
'ElseIf i=11 Or i=12 Then
				...
				wend
				rs.close
				Response.write "" & vbcrlf & "        <script>" & vbcrlf & "        //alert("""
				Response.write sValue1
				Response.write """);" & vbcrlf & "        try{" & vbcrlf & "        parent.parent.window.getnewlist1("""
			...
			If drmodel = 2 Then
				...
				set uploader = lvw.getuploader()
				db  = uploader.dbname
				if not uploader.CheckFields("序列号,实盘数量,仓库名称,备注") then
					exit sub
				end if
				call uploader.RegRptItem("#k_all","导入报告")
				call uploader.RegRptItem("#k_fail","未导入报告")
				...
				cn.execute "create table #k_fail (行号 int, 失败原因 varchar(300))"
				cn.execute "create table #k_all (序号 int  IDENTITY(1,1) not null,内容 varchar(300), 说明 varchar(300))"
				cn.execute "select up_index, ('序列号:<b style=""color:#000088"">' + isnull(序列号,'') + '</b>') as 产品信息 into #pname from " & db,a_Num
				cn.execute "create table #k_all (序号 int  IDENTITY(1,1) not null,内容 varchar(300), 说明 varchar(300))"
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '序列号不可为空' from " & db & " where len(rtrim(isnull(序列号,''))) = 0"
				cn.execute "delete from " & db & " where len(rtrim(isnull(序列号,''))) = 0" , n
				f_Num = f_Num + n
'cn.execute "delete from " & db & " where len(rtrim(isnull(序列号,''))) = 0" , n
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '序列号超长，请修改' from " & db & " where len(rtrim(isnull(序列号,''))) > 50"
				cn.execute "delete from " & db & " where  len(rtrim(isnull(序列号,''))) > 50" , n1
				f_Num = f_Num + n1
'cn.execute "delete from " & db & " where  len(rtrim(isnull(序列号,''))) > 50" , n1
				cn.execute "insert into #k_all (内容,说明) values ('检测序列号长度','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & n & "</b>条记录序列号为空，<b style=""color:red"">" & n1 & "</b>条记录序列号长度超过50。","") & "')"
'cn.execute "delete from " & db & " where  len(rtrim(isnull(序列号,''))) > 50" , n1
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '此产品有多条，请核查' from " & db & " x where (select count(*) from ku where xlh = 序列号) >1 "
				cn.execute "delete from " & db & "  where (select count(*) from ku where xlh = 序列号) >1" , n
				f_Num = f_Num + n
'cn.execute "delete from " & db & "  where (select count(*) from ku where xlh = 序列号) >1" , n
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '此产品不存在' from " & db & " x  where (select count(*) from ku where xlh = 序列号) =0 "
				cn.execute "delete from " & db & "  where (select count(*) from ku where xlh = 序列号) = 0" , n1
				f_Num = f_Num + n1
'cn.execute "delete from " & db & "  where (select count(*) from ku where xlh = 序列号) = 0" , n1
				cn.execute "insert into #k_all (内容,说明) values ('检测序列号正确性','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & n1 & "</b>条记录对应产品不存在，<b style=""color:red"">" & n & "</b>条记录对应产品有多条。","") & "')"
'cn.execute "delete from " & db & "  where (select count(*) from ku where xlh = 序列号) = 0" , n1
				cn.execute "alter table "&db&" add 产品名称 nvarchar(500),产品编号 nvarchar(500),产品型号 nvarchar(500),单位 nvarchar(500),批号 nvarchar(50),生产日期 nvarchar(50),有效日期 nvarchar(50)"
				cn.execute "update " & db & " set 产品名称=ord, 产品型号='',产品编号='',单位=unit from ku where xlh=序列号"
				cn.execute  "declare @I int" & vbcrlf &_
				"set @I = 1 " & vbcrlf &_
				"select ord ,sort1 ,sort , cast(sort1 as varchar(7000)) as ckname,del,case when (charindex( ',"&app.Info.user&",',','+replace(cast(d.intro as varchar(4000)),' ','')+',')>0 or replace(cast(d.intro as varchar(10)),' ','')='0') then 1 else 0 end as qx into  #tmp from sortck d " & vbcrlf &_
				"set @I = 1 " & vbcrlf &_
...
'cn.execute "delete from " & db & " where  datalength(备注)>1000" , n
				cn.execute "insert into #k_all (内容,说明) values ('检测备注长度','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录备注超过500字","") & "')"
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '产品重复' from ( select cast(产品名称 as nvarchar(500)) 产品名称,cast(产品编号 as nvarchar(500)) 产品编号,cast(产品型号 as nvarchar(500)) 产品型号,cast(仓库名称 as nvarchar(500)) 仓库名称,cast(单位 as nvarchar(500)) 单位,cast(序列号 as nvarchar(50)) 序列号,cast(批号 as nvarchar(50)) 批号,cast(生产日期 as nvarchar(500)) 生产日期,cast(有效日期 as nvarchar(500)) 有效日期 from "&db&" group by cast(产品名称 as nvarchar(500)),cast(产品编号 as nvarchar(500)),cast(产品型号 as nvarchar(500)),cast(仓库名称 as nvarchar(500)),cast(单位 as nvarchar(500)),cast(序列号 as nvarchar(50)),cast(批号 as nvarchar(50)),cast(生产日期 as nvarchar(500)),cast(有效日期 as nvarchar(500)) having COUNT(up_index) > 1) x inner join "&db&" y on cast(x.产品名称 as nvarchar(500)) = cast(y.产品名称 as nvarchar(500)) and isnull(cast(x.产品编号 as nvarchar(500)),'') = isnull(cast(y.产品编号 as nvarchar(500)),'') and isnull(cast(x.产品型号 as nvarchar(500)),'') = isnull(cast(y.产品型号 as nvarchar(500)),'') and isnull(cast(x.仓库名称 as nvarchar(500)),'') = isnull(cast(y.仓库名称 as nvarchar(500)),'') and cast(x.单位 as nvarchar(500)) = cast(y.单位 as nvarchar(500)) and isnull(cast(x.序列号 as nvarchar(50)),'') = isnull(cast(y.序列号 as nvarchar(50)),'') and isnull(cast(x.批号 as nvarchar(50)),'') = isnull(cast(y.批号 as nvarchar(50)),'') and isnull(cast(x.生产日期 as nvarchar(500)),'') = isnull(cast(y.生产日期 as nvarchar(500)),'') and isnull(cast(x.有效日期 as nvarchar(500)),'') = isnull(cast(y.有效日期 as nvarchar(500)),'') "
				cn.execute "delete from " & db & " where up_index in (select up_index from ( select cast(产品名称 as nvarchar(500)) 产品名称,cast(产品编号 as nvarchar(500)) 产品编号,cast(产品型号 as nvarchar(500)) 产品型号,cast(仓库名称 as nvarchar(500)) 仓库名称,cast(单位 as nvarchar(500)) 单位,cast(序列号 as nvarchar(50)) 序列号,cast(批号 as nvarchar(50)) 批号,cast(生产日期 as nvarchar(500)) 生产日期,cast(有效日期 as nvarchar(500)) 有效日期 from "&db&" group by cast(产品名称 as nvarchar(500)),cast(产品编号 as nvarchar(500)),cast(产品型号 as nvarchar(500)),cast(仓库名称 as nvarchar(500)),cast(单位 as nvarchar(500)),cast(序列号 as nvarchar(50)),cast(批号 as nvarchar(50)),cast(生产日期 as nvarchar(500)),cast(有效日期 as nvarchar(500)) having COUNT(up_index) > 1) x inner join "&db&" y on cast(x.产品名称 as nvarchar(500)) = cast(y.产品名称 as nvarchar(500)) and isnull(cast(x.产品编号 as nvarchar(500)),'') = isnull(cast(y.产品编号 as nvarchar(500)),'') and isnull(cast(x.产品型号 as nvarchar(500)),'') = isnull(cast(y.产品型号 as nvarchar(500)),'') and isnull(cast(x.仓库名称 as nvarchar(500)),'') = isnull(cast(y.仓库名称 as nvarchar(500)),'') and cast(x.单位 as nvarchar(500)) = cast(y.单位 as nvarchar(500)) and isnull(cast(x.序列号 as nvarchar(50)),'') = isnull(cast(y.序列号 as nvarchar(50)),'') and isnull(cast(x.批号 as nvarchar(50)),'') = isnull(cast(y.批号 as nvarchar(50)),'') and isnull(cast(x.生产日期 as nvarchar(500)),'') = isnull(cast(y.生产日期 as nvarchar(500)),'') and isnull(cast(x.有效日期 as nvarchar(500)),'') = isnull(cast(y.有效日期 as nvarchar(500)),''))" , n
				f_Num = f_Num + n
				cn.execute "insert into #k_all (内容,说明) values ('检测产品是否重复','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录产品重复","") & "')"
				cn.cursorLocation = 3
				cn.execute("select top 0 10000 as ord, 10000 as unit , 10000 as ku, 1 as isNoku, cast(100000 as numeric(20,"&num_dot_xs&")) as num, cast('' as nvarchar(500)) as remark,10000 as kuid,cast('' as nvarchar(50)) as ph ,cast('' as nvarchar(50)) as xlh ,cast('' as nvarchar(50)) as scrq,cast('' as nvarchar(50)) as yxrq,cast('' as nvarchar(50)) as bz ,10000 as px1 into #ttt2")
				set rs = server.CreateObject("adodb.recordset")
				dat = request.form("tagData")
				dats = split(dat,"|")
				rs.open "select * from #ttt2" , cn , 1 ,3
				for i = 0 to ubound(dats)
					if len(dats(i)) > 1 then
						...
						rs.fields(6).value = cells(6)
						rs.fields(7).value = cells(7)
						rs.fields(8).value = cells(8)
						rs.fields(9).value = cells(9)
						rs.fields(10).value = cells(10)
						rs.fields(11).value = cells(11)
						rs.fields(12).value = i+1
'rs.fields(11).value = cells(11)
						rs.update
					end if
				next
				rs.close
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '此产品库存已存在，不可重复导入' from " & db & " a inner join #ttt2 b on cast(a.产品名称 as nvarchar(50)) = cast(b.ord as nvarchar(50)) and cast(a.仓库名称 as nvarchar(50))=cast(b.ku as nvarchar(50)) and cast(a.单位 as nvarchar(50))=cast(b.unit as nvarchar(50)) and isnull(cast(a.序列号 as nvarchar(50)),'')=isnull(cast(b.xlh as nvarchar(50)),'') and isnull(cast(a.批号 as nvarchar(50)),'')=isnull(cast(b.ph as nvarchar(50)),'') and isnull(cast(a.生产日期 as datetime),'')=isnull(cast(b.scrq as datetime),'') and isnull(cast(a.有效日期 as datetime),'')=isnull(cast(b.yxrq as datetime),'') "
				cn.execute "delete from " & db & " where up_index in (select up_index from " & db & " a inner join #ttt2 b on cast(a.产品名称 as nvarchar(50)) = cast(b.ord as nvarchar(50)) and cast(a.仓库名称 as nvarchar(50))=cast(b.ku as nvarchar(50)) and cast(a.单位 as nvarchar(50))=cast(b.unit as nvarchar(50)) and isnull(cast(a.序列号 as nvarchar(50)),'')=isnull(cast(b.xlh as nvarchar(50)),'') and isnull(cast(a.批号 as nvarchar(50)),'')=isnull(cast(b.ph as nvarchar(50)),'') and isnull(cast(a.生产日期 as datetime),'')=isnull(cast(b.scrq as datetime),'') and isnull(cast(a.有效日期 as datetime),'')=isnull(cast(b.yxrq as datetime),''))" , n
				f_Num = f_Num + n
',''))" , n
				cn.execute "insert into #k_all (内容,说明) values ('检测重复导入','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录产品重复导入","") & "')"
				cn.execute "update #k_fail set 失败原因 = 失败原因 + '(' + 产品信息 + ')' from #pname where up_index=行号"
				...
				cn.execute "insert into #k_all (内容,说明) values ('导入结果','共<b style=""color:red"">" & s_Num & "</b>条记录产品导入成功，共<b style=""color:red"">" & f_Num & "</b>条记录产品导入失败')"
				cn.execute "alter table #ttt2 add px2 int"
				cn.execute "insert into #ttt2 (ord,unit,ku,isNoku,num,remark,px2,kuid,ph,xlh,yxrq,scrq,px1) select cast(e.产品名称 as nvarchar(50)),cast(e.单位 as nvarchar(50)),cast(e.仓库名称 as nvarchar(50)),case when a.ord is null then 1 else 0 end,cast(e.实盘数量 as nvarchar(15)),isnull(cast(e.备注 as nvarchar(500)),''),up_index,0,cast(e.批号 as nvarchar(50)),cast(e.序列号 as nvarchar(50)),e.有效日期,e.生产日期,0 from "&db&" e left join ku a on a.unit > 0 and (a.num2+isnull(locknum,0))<>0 and cast(a.ord as nvarchar(50))=cast(e.产品名称 as nvarchar(50)) and cast(e.仓库名称 as nvarchar(50))=cast(a.ck as nvarchar(50)) and cast(a.unit as nvarchar(50))=cast(e.单位 as nvarchar(50)) and cast(a.ph as nvarchar(50))=cast(e.批号 as nvarchar(50)) and cast(a.xlh as nvarchar(50))=cast(e.序列号 as nvarchar(50)) and a.datesc=e.生产日期 and a.dateyx=e.有效日期 group by cast(e.单位 as nvarchar(50)),cast(e.实盘数量 as nvarchar(15)),cast(e.仓库名称 as nvarchar(50)),cast(e.产品名称 as nvarchar(50)),a.ord,isnull(cast(e.备注 as nvarchar(500)),''),up_index,cast(e.批号 as nvarchar(50)),cast(e.序列号 as nvarchar(50)),e.有效日期,e.生产日期"
				Set rs_s = server.CreateObject("adodb.recordset")
				sql_s = "Select num1 from setjm3 Where ord=5430 and num1=1"
				rs_s.open sql_s,cn,1,1
				If Not rs_s.eof Then
				...
				"   "&sql2&" " & vbcrlf &_
				"  e.ph as 批号," & vcbrlf &_
				"  e.xlh as 序列号," & vcbrlf &_
				"  e.scrq as 生产日期," & vcbrlf &_
				"  e.yxrq as 有效期," & vcbrlf &_
				"  isnull((select top 1 sort1 from sortonehy where gate2=82 and ord=e.bz),'') + '^tag~' + cast(e.bz as varchar(10)) as 包装," & vcbrlf &_
				"  e.yxrq as 有效期," & vcbrlf &_
				"  e.remark as 备注, isnull(a.id,0) as kuid,  " & vbcrlf &_
				"  (case when sign(isnull(a.ck,0))<>1 then isnull(e.isNoku,0) else 1 end) as isNoku,e.ku,e.px2,e.px1 " & vbcrlf &_
				"  into #ttt3 " & vbcrlf &_
				"from #ttt2 e inner join product b on e.ord=b.ord and b.del=1 " & vbcrlf &_
				" left join ku a on a.ord = b.ord and e.ku = a.ck and (a.num2+isnull(locknum,0))<>0 and a.xlh = e.xlh  " & vbcrlf &_
				"from #ttt2 e inner join product b on e.ord=b.ord and b.del=1 " & vbcrlf &_
				" left join sortck d on d.ord=a.ck " &_
				" left join Shop_GoodsAttr s1 on s1.id = a.ProductAttr1  "&_
				" left join Shop_GoodsAttr s2 on s2.id = a.ProductAttr2  "
				cn.execute(sql)
				cn.execute  "update #ttt3 set 盈亏数量 = cast(实盘数量 as decimal(24,8))-账面数量,盈亏金额=(cast(实盘数量 as decimal(24,8))-账面数量)*单价"
				cn.execute(sql)
				cn.execute " update #ttt3 set 仓库ID=ku"&sql3&" where isNoku = 1"
