...
'Response.write "</div></form>"
	...
	Sub onReportInit(byval rpt, byval  state)
		rpt.title = "知识库列表"
		app.addScriptPath("inc/SaleKnowledge.js")
		dim Table, tiaojian, RowsList, RowsExcel, tiaojian_List, tiaojian_Excel, tiaojian_aSearch, rs
		rpt.adSearchAutoHide = True
		rpt.cancopy = app.power.existsPower(97,8)
		Dim sortID, sortID1
		sortID = app.Getint("sortID")
		If sortID > 0 Then
			sortID1 = GetSub(sortID)
		end if
		rpt.addfield "自：", "dates", "date3", "", "", Split("," & date, ",")
		rpt.addfield2 "","select", "modeID" , rpt.getselDataBySql("select sort1, ord from sortonehy where gate2 = 8004 order by gate1 desc,ord asc") ,"", "","知识库级别"
		dim keytype : keytype = "编号" & Chr(1) & "1"
		keytype = keytype & Chr(2) & "知识库内容" & Chr(1) & "2"
		keytype = keytype & Chr(2) & "解决方法" & Chr(1) & "3"
		keytype = keytype & Chr(2) & "关联客户" & Chr(1) & "4"
		keytype = keytype & Chr(2) & "关联售后" & Chr(1) & "5"
		rpt.addfield2 "","select", "keytype" , keytype, "", "", "标题=0"
		rpt.addfield "", "text", "keytxt", "", "", ""
		If sortID > 0 Then
			tiaojian = tiaojian & " and charindex(',' + cast(k.sortID as nvarchar) + ',',',"&sortID1&",') > 0 "
'If sortID > 0 Then
		end if
		If sortID = -2 Then
'If sortID > 0 Then
			tiaojian = tiaojian & " and k.sortID = 0 "
		end if
		tiaojian = tiaojian & " and (LEN(ISNULL(@date3_0,'')) = 0 or (LEN(@date3_0) > 0 and DATEDIFF(D,k.indate,@date3_0) <= 0)) "
		tiaojian = tiaojian & " and (LEN(ISNULL(@date3_1,'')) = 0 or (LEN(@date3_1) > 0 and DATEDIFF(D,k.indate,@date3_1) >= 0)) "
		tiaojian = tiaojian & " and (LEN(@modeID)=0 or (len(@modeID) > 0 and k.modeID = modeID))"
		tiaojian = tiaojian & " and (LEN(@keytype)=0 or (len(@keytype) > 0 and (case @keytype"_
		&" when '0' then k.title"_
		&" when '1' then k.bh"_
		&" when '2' then k.content"_
		&" when '3' then k.solution"_
		&" when '4' then t.name"_
		&" when '5' then ts.title"_
		&" end like '%' + @keytxt + '%'))) "
		rpt.aSearch.addField "添加人员", "gates", "AddCate",""
		rpt.aSearch.addfield "标题","text","title",""
		rpt.aSearch.addfield "编号","text","bh",""
		rpt.aSearch.addfield2 "知识库级别","checks", "modeID1" , "select sort1, ord from sortonehy where gate2 = 8004 order by gate1 desc,ord asc" ,""
		rpt.aSearch.addfield "关键字","text","keyWord",""
		rpt.aSearch.addfield "网址","text","url1",""
		set rs = cn.execute ("select title,name,sort,gl from zdy where sort1 = 8004 and set_open = 1 and js = 1 order by gate1")
		while rs.eof = false
			select case rs("sort")
			case "2"
			rpt.aSearch.addfield rs("title"),"text",rs("name"),""
			case "1"
			rpt.aSearch.addfield2 rs("title"),"checks", rs("name") , "select sort1,ord from sortonehy where gate2 = " & rs("gl") & " and del = 1" ,""
			end select
			rs.movenext
		wend
		rs.close
		set rs = nothing
		rpt.aSearch.addfield "添加时间","dates","date1",""
		rpt.aSearch.addfield "更新时间","dates","date2",""
		If sortID > 0 Then
			tiaojian_aSearch = tiaojian_aSearch & " and charindex(',' + cast(k.sortID as nvarchar) + ',',',"&sortID1&",') > 0 "
'If sortID > 0 Then
		end if
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@AddCate) = 0 or (LEN(@AddCate) > 0 and charindex(',' + cast(k.creator as nvarchar) + ',',',' + @AddCate + ',') > 0)) "
'If sortID > 0 Then
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@title) = 0 or (LEN(@title) > 0 and k.title like '%' + @title + '%')) "
'If sortID > 0 Then
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@bh) = 0 or (LEN(@bh) > 0 and k.bh like '%' + @bh + '%')) "
'If sortID > 0 Then
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@modeID1) = 0 or (LEN(@modeID1) > 0 and k.modeID in (@modeID1))) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@keyWord) = 0 or (LEN(@keyWord) > 0 and k.keyWord like '%' + @keyWord + '%')) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@url1) = 0 or (LEN(@url1) > 0 and k.url1 like '%' + @url1 + '%')) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(ISNULL(@date1_0,'')) = 0 or (LEN(@date1_0) > 0 and DATEDIFF(D,k.indate,@date1_0) <= 0)) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(ISNULL(@date1_1,'')) = 0 or (LEN(@date1_1) > 0 and DATEDIFF(D,k.indate,@date1_1) >= 0)) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(ISNULL(@date2_0,'')) = 0 or (LEN(@date2_0) > 0 and DATEDIFF(D,k.EditDate,@date2_0) <= 0)) "
		tiaojian_aSearch = tiaojian_aSearch & " and (LEN(ISNULL(@date2_1,'')) = 0 or (LEN(@date2_1) > 0 and DATEDIFF(D,k.EditDate,@date2_1) >= 0)) "
		set rs = cn.execute ("select title,name,sort,gl from zdy where sort1 = 8004 and set_open = 1 and js = 1 order by gate1")
		while rs.eof = false
			select case rs("sort")
			case "2"
			tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@" & rs("name") & ") = 0 or (LEN(@" & rs("name") & ") > 0 and k." & rs("name") & " like '%' + @" & rs("name") & " + '%')) "
'case "2"
			case "1"
			tiaojian_aSearch = tiaojian_aSearch & " and (LEN(@" & rs("name") & ") = 0 or (LEN(@" & rs("name") & ") > 0 and k." & rs("name") & " in (@" & rs("name") & ") )) "
			end select
			rs.movenext
		wend
		rs.close
		set rs = nothing
		dim PW_97_1
		Set rs = cn.execute("select qx_open from power where ord = " & info.user & " and sort1 = 97 and sort2 = 1")
		If rs.eof Then
			PW_97_1 = 0
		else
			PW_97_1 = rs(0)
		...
		dim info_97_1 : info_97_1 = app.power.getPowerIntro(97,1)
		tiaojian_List = tiaojian_List & " and ("&_
		"('" & PW_97_1 & "' = '3' or ('" & PW_97_1 & "' = '1' and CHARINDEX(',' + CAST(k.creator as varchar(100)) + ',','," & info_97_1 & ",') > 0))"&_
		" or CHARINDEX('," & info.user & ",',',' + k.user_list + ',') > 0 or k.user_list = '1' "&_
		") "
		dim PW_97_10
		Set rs = cn.execute("select qx_open from power where ord = " & info.user & " and sort1 = 97 and sort2 = 10")
		If rs.eof Then
			PW_97_10 = 0
		else
			PW_97_10 = rs(0)
		...
		rs.close : Set rs=Nothing
		dim info_97_10 : info_97_10 = app.power.getPowerIntro(97,10)
		tiaojian_Excel = tiaojian_Excel & " and ("&_
		"('" & PW_97_10 & "' = '3' or ('" & PW_97_10 & "' = '1' and CHARINDEX(',' + CAST(k.creator as varchar(100)) + ',','," & info_97_10 & ",') > 0))"&_
		" or CHARINDEX('," & info.user & ",',',' + k.user_list + ',') > 0 or k.user_list = '1' "&_
		") "
		RowsList = " k.id as ord, k.title, k.bh, so.sort1 as ModeTitle, ps.ord as SortTitle, k.Company, k.ServiceID, convert(varchar(20),k.indate,120) as indate, convert(varchar(20),k.EditDate,120) as EditDate, c.name as AddName, e.name as EditName, k.id, k.creator "
		RowsExcel = " k.title as 标题, k.bh as 编号, so.sort1 as 知识库级别, ps.ord as 知识库分类, k.KeyWord as 关键词, k.content as 知识库内容, k.solution as 解决方法, replace(cast(k.remark as nvarchar(1000)),'<IMG','<IMG1') as 备注, "
		set rs = cn.execute ("select title,name,sort,gl from zdy where sort1 = 8004 and set_open = 1 and dc = 1 order by gate1")
		while rs.eof = false
			select case rs("sort")
			case "2"
			RowsExcel = RowsExcel & " k." & rs("name") & " as [" & rs("name") & "],"
			case "1"
			RowsExcel = RowsExcel & " " & rs("name") & ".sort1 as [" & rs("name") & "],"
			end select
			rs.movenext
		wend
		rs.close
		set rs = nothing
		RowsExcel = RowsExcel &"t.name as 关联客户, ts.title as 关联售后, c.name as 添加人员, k.indate as 添加时间, e.name as 更新人员, k.editDate as 更新时间"
		Table = " sale_knowledge k "&_
		" left join Page_sort ps on ps.Ord = k.sortID "&_
		" left join sortonehy so on so.ord = k.modeID "&_
		" left join gate c on c.ord = k.creator "&_
		" left join gate e on e.ord = k.EditCate "&_
		" left join tel t on t.ord = k.Company "&_
		" left join tousu ts on ts.ord = k.ServiceID "&_
		" left join sortonehy zdy5 on zdy5.ord = k.zdy5 "&_
		" left join sortonehy zdy6 on zdy6.ord = k.zdy6 "
		rpt.sql = "if &excelmode = 1 begin "&_
		"if &SearchMode = 1 begin "&_
		"select " & RowsExcel & " from " & Table & " where k.del = 1 " & tiaojian_aSearch & tiaojian_Excel & " order by k.id desc "&_
		"end else begin "&_
		"select " & RowsExcel & " from " & Table & " where k.del = 1 " & tiaojian & tiaojian_Excel & " order by k.id desc "&_
		"end "&_
		"end else begin "&_
		"if &SearchMode = 1 begin "&_
		"select " & RowsList & " from " & Table & " where k.del = 1 " & tiaojian_aSearch & tiaojian_List & " order by k.id desc  "&_
		"end else begin "&_
		"select " & RowsList & " from " & Table & " where k.del = 1 " & tiaojian & tiaojian_List & " order by k.id desc end "&_
		"end "
		dim bats
		if app.power.existsPower(97,3) then
			bats = "批量删除"
		end if
		rpt.setBatHandle "id", bats
		if app.power.existsPower(97,13) then
			rpt.beforeBtnsHTML = "<button class='oldbutton' onclick=""javascript:window.open('add.asp','addnewwin22','width=' + 1024 + ',height=' + 650 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=50')"" type='button'>添加</button>"
'if app.power.existsPower(97,13) then
		end if
		rpt.canexcel = app.power.existsPower(97,10)
		rpt.canprint = app.power.existsPower(97,7)
		rpt.BillListUIModel = true
	end sub
	Sub app_oncreate(byval lvw)
		lvw.headers(1).title = "选择"
		lvw.headers(2).title = "标题"
		lvw.headers(3).title = "编号"
		lvw.headers(4).title = "知识库级别"
		lvw.headers(5).title = "知识库分类"
		lvw.headers(6).title = "关联客户"
		lvw.headers(7).title = "关联售后"
		lvw.headers(8).title = "发布时间"
		lvw.headers(9).title = "更新时间"
		lvw.headers(10).title = "添加人员"
		lvw.headers(11).title = "更新人员"
		lvw.headers(12).title = "操作"
		lvw.headers(8).width = 230
		lvw.headers(9).width = 230
		lvw.headers("12").width = 230
		lvw.headers(2).dbtype = "str"
		lvw.headers(1).display = "none"
		lvw.headers(13).display = "none"
		lvw.headers(2).formattext = "code: TitleCell(@value,rs(""ord""), rs(""creator""))"
		lvw.headers(5).formattext = "code: SortCell(@value)"
		lvw.headers(6).formattext = "code: CompanyCell(@value)"
		lvw.headers(7).formattext = "code: ServiceCell(@value)"
		lvw.headers(12).formattext = "code: CHandleCell(@value, rs(""creator""))"
		lvw.excelcallbackproc= "OnExcelDC"
		lvw.addJoinFields lvw.headers(8).dbname & ";" &  lvw.headers(9).dbname
		lvw.addJoinFields lvw.headers(10).dbname & ";" &  lvw.headers(11).dbname
	end sub
	sub OnExcelDC(byval lvw)
		lvw.headers(4).Formula = "SortCell(@value)"
		Dim rs
		set rs = cn.execute ("select title,name,sort,gl from zdy where sort1 = 8004 and set_open = 1 and dc = 1 order by gate1")
		while rs.eof = false
			lvw.headers(rs("name")).title = rs("title")
			rs.movenext
		wend
		rs.close
		set rs = nothing
	end sub
	sub lvw_onCreateCheckBox(lvw, rs, ckhtml)
		if not app.power.existsPowerIntro(97,3, rs("creator").value) then
			ckhtml = "&nbsp;"
		end if
	end sub
	Sub App_doBatHandle(cmd, checkvalues, batResult)
		Select Case cmd
		Case "批量删除":
		Call doDelete(checkvalues, batResult)
		End select
	end sub
	Sub doDelete(values, batResult)
		cn.execute "delete from sale_knowledge where id in (" & values & ")"
	end sub
	Sub App_DoDel
		dim id
		id = request.Form("id")
		cn.execute "delete from sale_knowledge where id in (" & id & ")"
		Response.write("true")
	end sub
	Function SortCell(byval sortID)
		dim rs, v, title
		title = ""
		v = sortID
		if len(v) <> 0 and isnull(v) = false then
			dim i : i = 1
			while v <> 0
				set rs = cn.execute ("select title,isnull(sortID,0) as sortID from Page_sort where ord = " & v)
				If  rs.eof = false then
					if i = 1 then
						title = rs("title")
					else
		...
		SortCell =app.HtmlConvert(title)
	end function
	Function CompanyCell(byval Companyid)
		dim rs, v, title
		title = ""
		v = Companyid
		if len(v) <> 0 or isnull(v) = false then
			dim PW_1_1 : PW_1_1 = app.power.existsPower(1,1)
			dim info_1_1 : info_1_1 = app.power.getPowerIntro(1,1)
			dim tiaojian : tiaojian = "  "
			tiaojian = tiaojian & " and ("&_
			"('" & lcase(PW_1_1) & "' = 'true' and (len('" & info_1_1 & "') = 0 or CHARINDEX(',' + CAST(cateadd as varchar(100)) + ',','," & info_1_1 & ",') > 0))"&_
			" or CHARINDEX('," & info.user & ",',',' + share + ',') > 0 or share = '1' "&_
			") "
			set rs = cn.execute ("select name from tel where ord = " & v & tiaojian)
			if  rs.eof = false then
				title = rs("name")
			end if
			rs.close
			set rs = nothing
			dim PW_1_14 : PW_1_14 = app.power.existsPower(1,14)
			dim info_1_14 : info_1_14 = app.power.getPowerIntro(1,14)
			if isnull(info_1_14) then
				info_1_14 = ""
			end if
			if PW_1_14 and (len(info_1_14) = 0 or InStr("," & info_1_14 & ",","," & info.user & ",") > 0) then
				CompanyCell = ("<a href=""javascript:void(0)"" onclick=""javascript:window.open('"&app.virpath&"work/content.asp?ord=" & app.base64.pwurl(v) & "&pn=1&','newwin11','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')"">" & app.HtmlConvert(title) & "</a>")
			else
				CompanyCell = (app.HtmlConvert(title))
			end if
		end if
	end function
	Function ServiceCell(byval ServiceID)
		dim rs, v, title, Creator
		title = ""
		Creator = 0
		v = ServiceID
		if len(v) <> 0 or isnull(v) = false then
			dim tiaojian : tiaojian = "  "
			dim PW_42_1 : PW_42_1 = app.power.existsPower(42,1)
			dim info_42_1 : info_42_1 = app.power.getPowerIntro(42,1)
			tiaojian = tiaojian & " and ("&_
			"('" & lcase(PW_42_1) & "' = 'true' and (len('" & info_42_1 & "') = 0 or CHARINDEX(',' + CAST(addcate as varchar(100)) + ',','," & info_42_1 & ",') > 0))"&_
			") "
			set rs = cn.execute ("select title,isnull(addcate,0) as addcate from tousu where ord = " & v & tiaojian)
			if  rs.eof = false then
				title = rs("title")
				Creator = rs("addcate")
			end if
			rs.close
			set rs = nothing
			if app.power.existsPowerIntro(42, 14, Creator) then
				ServiceCell = ("<a href=""javascript:void(0)"" title=""查看此售后详情"" onclick=""javascript:window.open('"&app.virpath&"service/content.asp?ord=" & app.base64.pwurl(v) & "','newwin2','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150')"">" & app.HtmlConvert(title) & "</a>")
			else
				ServiceCell = (app.HtmlConvert(title))
			end if
		end if
	end function
	Function CHandleCell(byval id, byval cateid)
		dim buttonShow, buttonEdit, buttonDel
		if app.power.existsPowerIntro(97,14,cateid) then
			buttonShow = "<button class='oldbutton2' onclick=""window.open('content.asp?ord=" & app.base64.pwurl(id) & "','PrinterRecordInfo1','width=' + 980 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">详情</button>"
'if app.power.existsPowerIntro(97,14,cateid) then
		end if
		if app.power.existsPowerIntro(97,2,cateid) then
			buttonEdit = "<button class='oldbutton2' onclick=""window.open('correct.asp?ord=" & app.base64.pwurl(id) & "','PrinterRecordInfo1','width=' + 980 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">修改</button>"
'if app.power.existsPowerIntro(97,2,cateid) then
		end if
		if app.power.existsPowerIntro(97,3,cateid) then
			buttonDel = "<button class='oldbutton2' onclick=""ListAction.DoDel('" & id & "')"">删除</button>"
		end if
		CHandleCell = buttonShow & "&nbsp;" & buttonEdit & "&nbsp;" & buttonDel
	end function
	Function TitleCell(byval title, byval id, byval cateid)
		if app.power.existsPowerIntro(97,14,cateid) then
			TitleCell = "<a href=""javascript:void(0)"" onclick=""window.open('content.asp?ord=" & app.base64.pwurl(id) & "','PrinterRecordInfo1','width=' + 980 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">" & title & "</button>"
'if app.power.existsPowerIntro(97,14,cateid) then
		else
			TitleCell = title
		end if
	...
	Function GetSub(id)
		Dim sql,rs
		sql = "IF EXISTS  (SELECT * FROM tempdb.dbo.sysobjects WHERE name = '#tmp') "&_
		"  DROP TABLE [#tmp] "&_
		"  CREATE TABLE [#tmp]( "&_
		"          [id] int NULL, "&_
		"          [id1] int NULL, "&_
		...
		"declare  @I int "&_
		"set @I = 0 "&_
		"insert into #tmp(id, id1) select cast(ord as int) as id, sortid from Page_sort where ord = " & id & " and del = 1 "&_
		"while exists( "&_
		"  select 1 from Page_sort a inner join #tmp b  "&_
		"  on a.del = 1 and a.sortid=b.id and a.ord not in (select id from #tmp) "&_
		")  and @I < 100 "&_
		"begin "&_
		"  insert into #tmp(id, id1) select cast(a.ord as int), a.sortid from Page_sort a inner join #tmp b on a.del = 1 and a.sortid=b.id "&_
		"  and a.ord not in (select id from #tmp) "&_
		"  set @I = I + 1 "&_
		"end "&_
