...
'items = rpt.Settings.item(i)
	...
	Function FormatNum(num,dot)
		If num&"" = "Null" Then
			FormatNum = ""
		else
			If num&"" = "" Then num = 0
			FormatNum = FormatNumber(num,dot,-1)
'If num&"" = "" Then num = 0
		end if
	end function
	Function getOtherName(urlpath, lvwid)
		Dim rs, other, i
		Set rs=cn.execute("select attrv from erp_sys_listviewConfig where path='"& urlpath &"' and lvwid='"& lvwid &"' and attrn='anotherName' and len(attrv)>0 order by id ")
		i=0
		ReDim other(0)
		While Not rs.eof
			ReDim Preserve other(i+1)
'While Not rs.eof
			other(i)=rs(0).value
			rs.movenext
			i=i+1
		...
		set rs = nothing
		If i=0 Then
			other=Split("A|B|C|D|E|F|G|N","|")
		end if
		getOtherName = other
	end function
	Function getItemFormula(urlpath, lvwid)
		Dim rs, other, formula, formula_head, i, ii
		other = getOtherName(urlpath, lvwid)
		Set rs=cn.execute("select attrv from erp_sys_listviewConfig where path='"& urlpath &"' and lvwid='"& lvwid &"' and attrn='formula' and len(attrv)>0 order by id ")
		i=0
		ReDim formula(ubound(other)),formula_head(ubound(other))
		While Not rs.eof
			formula(i)=rs(0)
			formula_head(i)=rs(0)
			i=i+1
		...
		If i=0 Then
			other=Split("A|B|C|D|E|F|G|N","|")
			formula=Split("A|B|C|A/((B+C)/2)|N/D|F|E>F|N","|")
'other=Split("A|B|C|D|E|F|G|N","|")
			formula_head=Split("A|B|C|A/((B+C)/2)|N/D|F|E>F|N","|")
'other=Split("A|B|C|D|E|F|G|N","|")
		end if
		getItemFormula = formula
	end function
	Function getCPZdy()
		Dim rs, rs_zdy, len_rszdy
		Set rs=cn.execute("select set_open, title from zdy where sort1=21 order by gate1 asc, id asc")
		if not rs.eof then
			rs_zdy = rs.GetRows()
		end if
		rs.close
	...
	Function DeleteKuout(ckord, updateHtStatus)
		dim rs, rs2, ret, date1, costType
		dim numchg, moneyback, ku, n1, ContractID
		If ckord&"" = "" Then ckord = 0
		If updateHtStatus&"" = "" Then updateHtStatus = 0
		Set rs = conn.execute("select top 1 a.kuout from kuoutlist2 a inner join kuhclist b on a.ku=b.kuid and b.del=1 where a.kuout="& ckord &"")
		If rs.eof = False Then
			DeleteKuout = "2|出库单有回冲记录的单据不可删除!"
			Exit Function
		end if
		rs.close
		set rs = nothing
		Set rs = conn.execute("select top 1 a.kuout from kuoutlist2 a inner join kumovelist b on ((isnull(a.joindblistid,0)=0 and a.contractlist=b.id) or a.joindblistid=b.id) and b.del=1 and a.sort1=7 inner join kumove c on c.ord=b.kumove and c.complete1=3 where a.kuout="& ckord &"")
		If rs.eof = False Then
			DeleteKuout = "2|调入的调拨出库不可删除!"
			Exit Function
		end if
		rs.close
		set rs = nothing
		Set rs = conn.execute("SELECT TOP 1 1 FROM kuoutlist2 a WITH(NOLOCK) INNER JOIN contractthlist b WITH(NOLOCK) ON b.kuoutlist2=a.id AND b.del=1 INNER JOIN contractth c WITH(NOLOCK) ON c.ord=b.caigou AND c.del IN(1,3) AND ISNULL(c.sp,0)<>-1 AND a.del=1 AND a.kuout="& ckord &"")
		Exit Function
		If rs.eof = False Then
			DeleteKuout = "2|有产品已退货，不可以删除出库单!"
			Exit Function
		end if
		rs.close
		set rs = nothing
		Set rs = conn.execute("select k.date5 as date1 from kuout k where ord="& ckord &" and k.complete1 = 3 ")
		If rs.eof = False Then
			date1 = rs("date1")
			costType = 0
			if date1&""<>"" then
				if sdk.getSqlValue("select top 1 1 from inventoryCost WHERE left(convert(varchar,date1,21),7)=(select left(convert(varchar, k.date5,21),7) from kuout k where ord="& ckord &" ) and complete1 >= 1",0) > 0 then
					DeleteKuout = "2|出库单对应的成本核算月已核算，不可以删除!"
					Exit Function
				end if
				Set rs2 = conn.execute("select 1 from M2_CostComputation where (costType = " & costType & " or 0=" & costType & ") and complete1 >= 1 and datediff(mm,date1,'" & date1 & "')=0")
				If rs2.eof = False Then
					DeleteKuout = "2|出库单对应的生产成本已核算，不可以删除!"
					Exit Function
				end if
				rs2.close
				Set rs2 = Nothing
		...
		rs.close
		set rs = nothing
		Set rs = conn.execute("select order1 from kuout where ord="& ckord)
		If rs.eof = False Then
			ContractID = rs("order1")
		end if
		rs.close
		set rs = nothing
		If ContractID&"" = "" Then ContractID = 0
		conn.execute("Update kuout set del=2 ,delcate='" & session("personzbintel2007") & "',deldate=getdate() where ord=" & ckord & "")
		conn.execute("Update kuoutlist set del=2 where kuout=" & ckord & "")
		conn.execute("Update Kuoutlist2 set del=2 where kuout=" & ckord & "")
		Set rs = conn.execute("select * from kuoutlist2 where kuout="&ckord&"")
		While rs.eof = False
			if rs("sort1")=1 then
				if rs("num1")<>"" then
					numchg=cdbl(rs("num1"))
				...
				if rs("money1")<>"" then
					moneyback=cdbl(rs("money1"))
				else
					moneyback=0
				end if
			elseif rs("sort1")=7 or rs("sort1")=4 or rs("sort1")=8 or rs("sort1")=6 or rs("sort1")=3 Or rs("sort1")=2 Or rs("sort1")=5 or rs("sort1")=12 then
				if rs("num1")<>"" then
					numchg=cdbl(rs("num1"))
				else
					numchg=0
			...
			elseif rs("sort1")=9 then
				numchg=cdbl(rs("num1"))
				If Len(rs("ku")&"") = 0 Then
					moneyback=0
				else
					moneyback=cdbl(conn.execute("select money1/num1 from ku where id="&rs("ku"))(0))
				end if
				moneyback=moneyback*numchg
			else
				numchg=cdbl(rs("num1"))
				moneyback=cdbl(rs("money1"))*cdbl(rs("num2"))/ cdbl(rs("num1"))
			end if
			Call WriteStoreChangeLog(302,"kuoutlist2",rs("ord"),rs("id"),rs("ck"),1,numchg,moneyback,rs("ku"))
			if rs("ku")<>"" Then
				ku = rs("ku").value  & ""  :  If Len( ku) = 0 Then  ku = 0
				n1 = rs("num1").value  & ""  :  If Len(n1) = 0 Then  n1 = 0
				sqltext="Update ku set num2=num2+"&  n1 &" where id="& ku
'n1 = rs("num1").value  & ""  :  If Len(n1) = 0 Then  n1 = 0
		...
		rs.close
		set rs = nothing
		If ContractID> 0 and updateHtStatus&""="1" Then
			conn.execute "exec [erp_contract_UpdateStatus] " & ContractID
		end if
		DeleteKuout = "1"
	end function
	function KuoutFromBillRelation(conn, SortType , BillID , lbopen , lbintro , xqopen ,xqintro, tag)
		dim brs, gltitle , qxcate,lbqx , xqqx, xqurl , sqltext , v ,splitstr , s , appxqurl
		v= ""
		s = ""
		gltitle = ""
		lbqx = false
		...
		splitstr = "|@@@@@@@@|"
		dim billHtml()
		redim Preserve billHtml(1)
		billHtml(0) = 0
		select case SortType
		case 1,4:
		v = "合同"
		sqltext="select ord, title ,cateid , htid from contract  with(nolock) Where ord in ("&BillID&")  order by ord "
		xqurl = "../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord=@ord"
		appxqurl = "mobilephone/salesmanage/contract/BillService.asp?ord=@ord"
		case 2:
		v = "采购退货"
		sqltext="select ord, title ,cateid , cgthid from caigouth  with(nolock) Where ord in ("&BillID&")  order by ord "
		xqurl = "../../SYSN/view/store/caigouth/PurchaseReturn.ashx?ord=@ord&view=details"
		case 3 :
		v = "领料单"
		sqltext="select id, title ,creator, MOBH from M2_MaterialOrders  with(nolock) Where id in ("&BillID&")  order by id "
		xqurl = "../../sysn/view/produceV2/Material/OrdersAdd.ashx?ord=@ord&view=details"
		case 6 :
		v = "借货单"
		sqltext="select ord, title ,cateid,jhbh from kujh  with(nolock) Where ord in ("&BillID&")  order by ord"
		xqurl = "../store/contentjh.asp?ord=@ord&view=details"
		case 7 :
		v = "调拨单"
		sqltext="select ord, title ,cateid,dbbh from kumove  with(nolock) Where ord in ("&BillID&")  order by ord"
		xqurl = "../store/contentdb.asp?ord=@ord&view=details"
		case 8 :
		v = "盘点单"
		sqltext="select ord, title ,cateid,pdbh from kupd  with(nolock) Where ord in ("&BillID&")  order by ord"
		xqurl = "../../SYSA/store/contentpd2.asp?ord=@ord"
		appxqurl = "../SYSN/view/store/pandian/InventoryDetail.ashx?ord=@ord&view=details"
		case 9 :
		v = "组装单"
		sqltext="select ord, title ,cateid,zzid from kuzz  with(nolock) Where ord in ("&BillID&")  order by ord "
		xqurl = "../store/contentzz.asp?ord=@ord"
		case 12:
		v = "补料单"
		sqltext="select id, title ,creator, MOBH from M2_MaterialOrders  with(nolock) Where id in ("&BillID&") order by id "
		xqurl = "../../sysn/view/produceV2/Material/SupplementsAdd.ashx?ord=@ord&view=details"
		case else
		end select
		dim title , bh: title = ""
		bh = ""
		if len(sqltext)>0 then
			set brs =conn.execute(sqltext)
			if brs.eof=false then
				while brs.eof=false
					BillID = brs(0).value
					redim Preserve billHtml(BillID)
					title = brs(1).value
					qxcate = brs(2).value
					bh = brs(3).value
					if tag=2 or tag = 3 then
						lbqx = app.power.existsPowerIntro(lbopen,lbintro,qxcate)
						xqqx = app.power.existsPowerIntro(xqopen,xqintro,qxcate)
					else
						lbqx = (lbopen=3 or (lbopen=1 and CheckPurview(lbintro,trim(qxcate))=True))
						xqqx = (xqopen=3 or (xqopen=1 and CheckPurview(xqintro,trim(qxcate))=True))
					end if
					s = ""
					if lbqx=false then
						title = ""
					...
					s = title & splitstr & bh
					if lbqx and xqqx then
						if tag=2 or tag = 3 then
							s = s & splitstr & replace(appxqurl ,"@ord" , BillID )
						else
							s = s & splitstr & replace(xqurl ,"@ord" , pwurl(BillID) )
						end if
					end if
					billHtml(BillID) = s
					brs.movenext
				...
				if lbqx then s = title
			end if
			brs.close
		end if
		if len(s) >0 then v = v & splitstr & s
		if tag=0 or tag=2 then
			KuoutFromBillRelation = split(v ,splitstr)
		else
			KuoutFromBillRelation = billHtml
		end if
	end function
	function returnLinkArray(s)
		splitstr = "|@@@@@@@@|"
		dim av : av = split(s , splitstr)
		returnLinkArray = av
	end function
	function KuoutFromBillRelationMainHTML(glattr)
		if ubound(glattr)=0 then
			Response.write "" & vbcrlf & "        <tr>" & vbcrlf & "        <td height=""27""><div align=""right"">关联"
			Response.write glattr(0)
			Response.write "：</div></td>" & vbcrlf & "        <td height=""27"" colspan=""4""><span class=""red"">&nbsp;关联"
			Response.write glattr(0)
			Response.write "已被删除，请查证。</span><div align=""center""></div></td>" & vbcrlf & "        <td width=""20%"" height=""27"">&nbsp;</td>" & vbcrlf & "        </tr>" & vbcrlf & "        "
		else
			Response.write "" & vbcrlf & "        <tr>" & vbcrlf & "        <td height=""27""><div align=""right"">关联"
			Response.write glattr(0)
			Response.write "：</div></td>" & vbcrlf & "        <td height=""27"" colspan=""4""><span class=""gray"">&nbsp;"
			Response.write glattr(1)
			Response.write "</span></td>" & vbcrlf & "        <td height=""27""><div align=""right"">" & vbcrlf & "            "
			if ubound(glattr)=>3 then
				Response.write "" & vbcrlf & "                <a href=""javascript:;"" title=""查看此"
				Response.write glattr(0)
				Response.write "详情""  onclick=""javascript:window.open('"
				Response.write glattr(3)
				Response.write "','newwin2','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""><font class=""red"">查看此"
				Response.write glattr(3)
				Response.write glattr(0)
				Response.write "详情</font></a>" & vbcrlf & "            "
			end if
			Response.write "</div></td>" & vbcrlf & "        </tr>" & vbcrlf & "        "
		end if
	end function
	function ShowSerialNumber(cn, Billtype , Billid)
		dim xlhHtml()
		if Billtype= 62001 then
			msql ="select kl.id,kl.kuout, isnull( x.SeriNum, kl.xlh) xlh , isnull(x.cnt,0) cnt   "&_
			" from kuoutlist2 kl "&_
			" left join ( " &_
			"   select b.listid, ml.SeriNum , a.cnt "&_
			"   from ("&_
			"       select listid , min(id) id , count(1) cnt from S2_SerialNumberRelation where billtype=62001 and abs(billid)="& Billid &" group by listid "&_
			"   ) a "&_
			"   inner join S2_SerialNumberRelation b on b.id = a.id "&_
			"   inner join M2_SerialNumberList ml on ml.id = b.SerialID  "&_
			" ) x on abs(x.listid) = kl.id  " &_
			" where kl.kuout=" & Billid &" order by kl.id "
			set rs = cn.execute(msql)
			if rs.eof=false then
				while rs.eof=false
					kuoutlistID = rs("id").value&""
					cnt = rs("cnt").value
					divhtml = ""
					if cnt>1 then
						divhtml = "<img style='cursor:pointer;width:10px;height:20px' src='../../SYSN/skin/default/img/percent100.png' onclick='ShowSerialNumber(this,62001 , "& Billid &","& kuoutlistID &" )'><div id='w"&kuoutlistID&"'></div>"
					end if
					redim Preserve xlhHtml(kuoutlistID)
					xlhHtml(kuoutlistID) = rs("xlh").value&" " & divhtml
					rs.movenext
				wend
			end if
			rs.close
		end if
		ShowSerialNumber = xlhHtml
	end function
	
	Dim intro_21_1, open_21_14, open_23_2, num1_dot, num_dot_xs
	Sub onReportInit(rpt, lt)
		app.adddefaultScript
		app.addDefaultCss
		Dim rs,defDate,sTypeList, curDate, date3, topUrl
		rpt.title = "产品周转率明细表"
		If Info.issupperadmin Then rpt.Settings.formulaset = true
		rpt.cancolset = True
		rpt.canexcel = app.power.existsPower(23,10)
		rpt.canPrint = app.power.existsPower(23,7)
		topUrl = replace(replace(request.ServerVariables("url"),"/",""),".asp","")
		curDate = Date
		date3 = dateadd("m",-3,curDate)
		curDate = Date
		defDate = Split(date3 & "," & curDate, ",")
		sTypeList = sTypeList & "产品名称" & Chr(1) & 1 & Chr(2)
		sTypeList = sTypeList & "产品编号" & Chr(1) & 2 & Chr(2)
		sTypeList = sTypeList & "产品型号" & Chr(1) & 3
		rpt.AddField "时间：","dates","date1","","",defDate
		rpt.AddField2 "","select","searchType",sTypeList,"","","null"
		rpt.AddField "","text","searckKey",,"",""
		rpt.PageSize = 10
		rpt.sql = "exec erp_report_zzmx "& Info.User &", '"& topUrl &"', &pagesize, &pageindex,@date1_0, @date1_1, @searchType, @searckKey,'',&headerconfig"
		rpt.BillListUIModel = True
		rpt.showtoppagesize = True
		rpt.addgroup "呆滞库存TOP10", "exec erp_report_zzmx "& Info.User &", '"& topUrl &"', 10000000,1,@date1_0, @date1_1, @searchType, @searckKey,'BAD5','' ", "count",  "row",  "产品名称"
		rpt.addgroup "周转最快TOP10", "exec erp_report_zzmx "& Info.User &", '"& topUrl &"', 10000000,1,@date1_0, @date1_1, @searchType, @searckKey,'TOP5','' ", "count",  "row",  "产品名称"
	end sub
	Sub app_oncreate(lvw)
		Dim col,batchDel,beginCol,totalCol,i, other, formula, topUrl, lenf, curDate, date3, rs_zdy, len_rszdy
		curDate = Date
		date3 = dateadd("d",-91,curDate)
		curDate = Date
		topUrl = replace(replace(request.ServerVariables("url"),"/",""),".asp","")
		...
		Dim lvkey : lvkey = lvw.GetSboxHeaderConfigMd5
		other = getOtherName(topUrl, lvkey)
		formula = getItemFormula(topUrl, lvkey)
		lvw.CanSort = True
		lvw.indexbox = False
		...
		lvw.isshow_ymc=True
		lvw.isshow_xmc=True
		lvw.headNameJoin = False
		if app.power.existsPower(23,12) then
			lvw.isshow_anotherName=True
			lvw.isshow_formula=True
		end if
		lvw.FaStr="N="&datediff("d",date3,curDate)
		lvw.isshow_visible=false
		lvw.ServerConfig = true
		lvw.headers(1).display = "none"
		lvw.headers(2).display = "none"
		Set col = lvw.headers(3)
		col.width = 150
		col.dbtype = "str"
		col.setLink "产品名称","ord", "addcate",  21, -6
		col.dbtype = "str"
		lvw.headers(4).width = 120
		lvw.headers(5).width = 120
		lvw.headers(6).width = 65
		lvw.headers(7).width = 65
		open_23_2 = app.power.existsPower(23,2)
		lenf = ubound(other)
		For i = 0 To lenf - 2
			lenf = ubound(other)
			Set col = lvw.headers(i+8)
			lenf = ubound(other)
			col.evalName = other(i)
			col.evalcode = formula(i)
			If i < 3 Then
				If open_23_2 = True Then
					col.dbtype = "money"
					col.Formula = "FormatNum(@value,"& num_dot_xs &")"
				else
					col.formattext = " "
				end if
			else
				col.dbtype = "number"
				col.Formula = "FormatNum(@value,"& num1_dot &")"
			end if
			If i < lenf - 2 Then
				col.Formula = "FormatNum(@value,"& num1_dot &")"
				col.width = 140
		...
		next
		lvw.headers(lenf+7).width = 120
		col.width = 90
		Set col = lvw.headers(lenf+8)
		col.width = 90
		col.width = 80
		col.evalName = other(lenf-1)
		col.width = 80
		col.evalcode = formula(lenf-1)
		col.width = 80
		lvw.headers(lenf+9).evalName = other(lenf)
		col.width = 80
		lvw.headers(lenf+9).display = "none"
		col.width = 80
		For i = 0 To len_rszdy
			If rs_zdy(0,i) = 0 Then
				lvw.headers(17+i).display = "none"
'If rs_zdy(0,i) = 0 Then
			else
				lvw.headers(17+i).title = rs_zdy(1,i)
'If rs_zdy(0,i) = 0 Then
			end if
		next
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		Dim i, rs_zdy, len_rszdy, topUrl, other, formula, lenf, col
		rs_zdy = getCPZdy()
		if isArray(rs_zdy) Then
			len_rszdy = ubound(rs_zdy,2)
		else
		...
		For i = 0 To lenf - 2
			...
			If i < 3 Then
				If open_23_2 = True Then
					...
					col.Formula = "FormatNum(""Null"","& num_dot_xs &")"
				end if
			else
				col.dbtype = "number"
				col.Formula = "FormatNum(@value,"& num1_dot &")"
