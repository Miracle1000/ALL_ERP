...
'End  If
			...
			if ret<>"" then
				Str_Result=Str_Result+" and  datediff(s,'"&ret&"',a.time_login)>=0 "
'if ret<>"" then
			else
				ret="点击选择时间"
			end if
			...
			if ret2<>"" then
				Str_Result=Str_Result+" and  datediff(s,a.time_login,'"&ret2&"')>=0 "
'if ret2<>"" then
			else
				ret2="点击选择时间"
			end if
			...
			if tname=1 then
				sql="SELECT a.username,case when a.wxUserId is not null then m.nickname else a.name end  name,a.page1,a.time_login,a.action1,a.type_login,a.type_sys,a.type_brower,a.ip,a.keyValue,a.wxUserId " & vbcrlf &_
				"FROM action_list a " & vbcrlf &_
				"left join gate b on a.username=b.ord " & vbcrlf &_
				"left join MMsg_User m on m.id=a.wxUserId "&Str_Result
				set rs=server.CreateObject("adodb.recordset")
				rs.open sql,conn,1,1
				if not rs.eof then
					sql7="select max(a.time_login) as time_login FROM action_list a left join gate b on a.username=b.ord "&Str_Result
					set rs7=conn.execute(sql7)
					if not rs7.eof then
						endtime=rs7("time_login")
					end if
					rs7.close
					set rs7=nothing
					sql7="select min(a.time_login) as time_login FROM action_list a left join gate b on a.username=b.ord "&Str_Result
					set rs7=conn.execute(sql7)
					if not rs7.eof then
						startime=rs7("time_login")
					end if
					rs7.close
					set rs7=nothing
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select * from action_list2 where 1=2 "
					rs7.open sql7,conn,3,2
					if rs7.eof then
						rs7.addnew
						rs7("cateid")=session("personzbintel2007")
						rs7("catetime")=now
						rs7("startime")=startime
						rs7("endtime")=endtime
						rs7.update
					end if
					rs7.close
					set rs7=nothing
					stamptime=datediff("s","1970-1-1 08:00:00",now)
					set rs7=nothing
					sql7="SELECT IDENT_CURRENT('action_list2')"
					set rs7=conn.execute(sql7)
					sid=rs7(0)
					rs7.close
					bakname="ActionFile_"&sid&"_"&stamptime&""
					sqltext="update action_list2 set bakname='"&bakname&"' where id="&sid
					conn.execute(sqltext)
					filepath=server.MapPath("../ActionLog")
					set MyFileObject=server.CreateObject("Scripting.FileSystemObject")
					if Not MyFileObject.FolderExists(filepath) then
						MyFileObject.CreateFolder(filepath)
					end if
					set MyFileObject=nothing
					rs.save(filepath&"\"&bakname&".dat")
				end if
				rs.close
				set rs=nothing
				sql="delete from action_list where id in (select a.id from action_list a left join gate b on a.username=b.ord "&Str_Result&")"
				conn.execute(sql)
			end if
			Response.redirect "action_list.asp?tname="&tname&"&A="&A&"&B="&B&"&C="&C&"&D="&D&"&ret="&ret&"&ret2="&ret2&"&L1="&L1&"&L2="&L2
			call db_close : Response.end()
