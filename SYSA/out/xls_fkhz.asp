	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			elseif open_8_10=1 then
				list_tj=" and b.cateid in ("&intro_8_10&")"
			else
				list_tj=" and 1=2 "
			end if
			str_Result11 = " and a.del=1 "&list_tj&""
			...
			xlApplication.ActiveSheet.Columns(2).ColumnWidth=8
...
'elseif B="khid" then
				elseif B="htzt" then
					str_Result11=str_Result11+" and b.company in (select company from contract where title like '%"& C &"%')"
'elseif B="htzt" then
				elseif B="htid" then
					str_Result11=str_Result11+" and b.company in (select company from contract where htid like '%"& C &"%')"
'elseif B="htid" then
				elseif B="xsmc" then
					str_Result11=str_Result11+" and b.company in (select a.ord from tel a,gate b where a.cateid=b.ord and b.name like'%"&C&"%')"
'elseif B="xsmc" then
				end if
			end if
			...
			if px=1 then
				px_Result="order by name desc,company desc"
			elseif px=2 then
				px_Result="order by name asc,company asc"
			elseif px=3 then
				px_Result="order by money1 desc,company desc"
			elseif px=4 then
				px_Result="order by money1 asc,company asc"
			elseif px=5 then
				px_Result="order by money2 desc,company desc"
			elseif px=6 then
				px_Result="order by money2 asc,company asc"
			elseif px=7 then
				px_Result="order by money3 desc,company desc"
			elseif px=8 then
				px_Result="order by money3 asc,company asc"
			elseif px=9 then
				px_Result="order by money4 desc,company desc"
			elseif px=10 then
				px_Result="order by money4 asc,company asc"
			elseif px=11 then
				px_Result="order by (isnull(money3,0) - isnull(money5,0)) desc,company desc"
'elseif px=11 then
			elseif px=12 then
				px_Result="order by (isnull(money3,0) - isnull(money5,0)) asc,company asc"
'elseif px=12 then
			elseif px=13 then
				px_Result="order by money6 desc,company desc"
			elseif px=14 then
				px_Result="order by money6 asc,company asc"
			end if
			dim summoney1,summoney2,summoney3,summoney4,summoney5,summoney6,money1_all,money2_all,money3_all,money4_all,money5_all,money6_all
			summoney1=0
			summoney2=0
			summoney3=0
			summoney4=0
			...
			summoney6=0
			n=0
			sql="select b.company,b.bz," & vbcrlf &_
			"(select name from tel where sort3=2 and del=1 and ord=b.company) as name," & vbcrlf &_
			"(select sort1 from sortbz where id=b.bz) as sortbz," & vbcrlf &_
			"isnull(sum(a.money1),0) as money1," & vbcrlf &_
			"isnull("& vbcrlf & _
			"("& vbcrlf & _
			"select isnull(sum(c.money1),0) as money1 from payout as c WITH(NOLOCK) "& vbcrlf & _
			"left join PayCaigouProxy as d WITH(NOLOCK) on c.cls=d.cls and c.contract=d.ord "& vbcrlf & _
			"where c.del=1 and c.date2>='"&m1&"' and c.date2<='"&m2&" 23:59:59' and d.company=b.company and d.bz=b.bz and c.complete=3 "& vbcrlf & _
			"group by d.company,d.bz"& vbcrlf & _
			")"& vbcrlf & _
			",0) as money2,"& vbcrlf & _
			"isnull(( "& vbcrlf & _
			"select isnull(sum(money1),0) from ("& vbcrlf & _
			"select company,bz,(case fromtype when 'PREOUT' then isnull(money_left,money1) else money1 end) money1  "& vbcrlf & _
			"from payoutInvoice "& vbcrlf & _
			"where del=1 and isInvoiced in (0,1,2) and date1>='"&m1&"' and date1<='"&m2&" 23:59:59'  "& vbcrlf &_
			") t "& vbcrlf & _
			"where t.company=b.company and t.bz=b.bz "& vbcrlf & _
			"  ),0) as money3,"& vbcrlf & _
			"isnull("& vbcrlf & _
			"("& vbcrlf & _
			"select isnull(sum(c.money1),0) as money1 from payout as c WITH(NOLOCK) "& vbcrlf & _
			"left join PayCaigouProxy as d WITH(NOLOCK) on c.cls=d.cls and c.contract=d.ord "& vbcrlf & _
			"where c.del=1 "&Replace(Replace(Str_Result11,"b.","d."),"and a.del=1","")&" and c.date1>='"&m1&"' and c.date1<='"&m2&" 23:59:59' and d.company=b.company and d.bz=b.bz and c.complete=1 "& vbcrlf & _
			"group by d.company,d.bz"& vbcrlf & _
			")"& vbcrlf & _
			",0) as money4,"& vbcrlf & _
			"isnull("& vbcrlf & _
			"("& vbcrlf & _
			"select isnull(sum(money1),0) from ("& vbcrlf & _
			"select company,bz,(case fromtype when 'PREOUT' then isnull(money_left,money1) else money1 end) money1  "& vbcrlf & _
			"from payoutInvoice "& vbcrlf & _
			"where del=1 and isInvoiced in (1,2) and invoiceDate>='"&m1&"' and invoiceDate<='"&m2&" 23:59:59'  "& vbcrlf &_
			") t "& vbcrlf & _
			"where company=b.company and t.bz=b.bz "& vbcrlf & _
			")"& vbcrlf & _
			",0) As money5,"& vbcrlf & _
			" isnull((select isnull(sum(money1),0) as money1 " & vbcrlf &_
			" from ( " & vbcrlf &_
			"  select k.bz,sum(o.money1) as money1 from bankout2 o inner join sortbank k on k.id=o.bank and o.del=1 and isnull(o.status_sp,1) in (-1,1) and isnull(o.bank,0)>0 " & vbcrlf &_
			" from ( " & vbcrlf &_
			" where o.company =b.company and k.bz =b.bz  group by k.bz " & vbcrlf &_
			" union all " & vbcrlf &_
			" select c.bz,sum(o.money1) from bankout2 o " & vbcrlf &_
			" inner join payout p on p.ord=isnull(o.payout,0) and o.del=1 and isnull(o.status_sp,0)=0 and o.bank=0 " & vbcrlf &_
			" inner join PayCaigouProxy c on c.ord = p.contract and c.cls = isnull(p.cls,0) " & vbcrlf &_
			" where o.company =b.company and c.bz =b.bz group by c.bz " & vbcrlf &_
			" union all " & vbcrlf &_
			" select c.bz,sum(o.money1) from bankout2 o " & vbcrlf &_
			" inner join payout3 p on p.ord=isnull(o.payout3,0) and o.del=1 and isnull(o.status_sp,0)=0 and o.bank=0 " & vbcrlf &_
			" inner join caigouth c on c.ord = p.caigouth " & vbcrlf &_
			" where o.company =b.company  and c.bz =b.bz group by c.bz " & vbcrlf &_
			" union all " & vbcrlf &_
			" select bz, -isnull(sum(money1),0) from bankout4 where del=1 and company=b.company and bz=b.bz and money1>0 group by bz " & vbcrlf &_
			" union all " & vbcrlf &_
			" union all " & vbcrlf &_
			" select k.bz,-isnull(sum(ts.money1),0) from bankout3 ts inner join sortbank k on k.id =ts.bank where ts.del=1 and ts.company=b.company and k.bz=b.bz group by k.bz" & vbcrlf &_
			" union all " & vbcrlf &_
			" ) l),0) as money6 " & vbcrlf &_
			"from payout as a WITH(NOLOCK) "& vbcrlf & _
			"left join PayCaigouProxy as b WITH(NOLOCK) on a.cls=b.cls and a.contract=b.ord "& vbcrlf & _
			"left join hl as h WITH(NOLOCK) on a.date4=h.date1 and b.bz=h.bz "& vbcrlf & _
			"where a.del=1 "&Str_Result11&" and a.date1>='"&m1&"' and a.date1<='"&m2&" 23:59:59' "& vbcrlf & _
			"group by b.company,b.bz "
			sql = "select * from (" & sql &" ) k " & px_Result
			money1=0
			money2=0
			money3=0
			money4=0
			...
			C1=rs.recordcount
			xlApplication.Init "付款汇总表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0 then
				...
				do until rs.eof
					if rs("company")<>"" then
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select ord,name,cateid from tel where ord="&rs("company")&" and del=1"
						rs1.open sql1,conn,1,1
						if rs1.eof then
							company=0
							companyname="供应商已被删除"
...
'if cateid_kh=0 or cateid_kh="" then
					...
					money4=zbcdbl(rs("money4"))
					if rs("money4")&""="" then
						money4=0
					else
						money4=CDbl(rs("money4"))
					end if
					money5=zbcdbl(rs("money5"))
					if rs("money5")&""="" then
						money5=0
					else
						money5= money3 - CDbl(rs("money5"))
						money5=0
					end if
					money6=zbcdbl(rs("money6"))
					if rs("money6")&""="" then
						money6=0
					else
						money6=CDbl(rs("money6"))
					end if
					sortbz=rs("sortbz")
					set rs88=server.CreateObject("adodb.recordset")
					rs88.open "select name from gate where ord in (select cateid from tel where ord="&company&")",conn,1,1
					if not rs88.eof then
						cateidname=rs88("name")
					else
						cateiname=""
					end if
					rs88.close
					...
					companyname=""
					xlWorksheet.Cells(1+i,2).NumberFormatLocal = "@"
					companyname=""
					xlWorksheet.Cells(1+i,2).Value = sortbz
					companyname=""
					...
					companyname=""
					xlWorksheet.Cells(1+i,4).Value = Formatnumber(money2,num_dot_xs,-1)
					companyname=""
					xlWorksheet.Cells(1+i,4).font.Size=10
					companyname=""
					xlWorksheet.Cells(1+i,5).Value = Formatnumber(money3,num_dot_xs,-1)
					companyname=""
					xlWorksheet.Cells(1+i,5).font.Size=10
					companyname=""
					xlWorksheet.Cells(1+i,6).Value = Formatnumber(money4,num_dot_xs,-1)
					companyname=""
					xlWorksheet.Cells(1+i,6).font.Size=10
					companyname=""
					xlWorksheet.Cells(1+i,7).Value = Formatnumber(money5,num_dot_xs,-1)
					companyname=""
					xlWorksheet.Cells(1+i,7).font.Size=10
					companyname=""
					xlWorksheet.Cells(1+i,8).Value = Formatnumber(money6,num_dot_xs,-1)
					companyname=""
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>付款汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!付款汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("付款汇总表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的付款汇总表</u></strong></font></a></p>"
