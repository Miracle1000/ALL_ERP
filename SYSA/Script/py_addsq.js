...
function ask()
{
	...
	if (!Validator.Validate(obj, 2) || !DelUnusedFilesBeforeSubmit()) return false;
...
//    }
	for (var i = 0; i < str.length; i++) {
		...
		if (temp == "Money" && str[i].value.length == 0)
		{
			...
			var remark ="";
			var obj = document.getElementsByName("note"+name)[0];
			if (obj){remark = obj.value;}
			var zs ="";
			var obj = document.getElementsByName("num"+name)[0]
			if (obj){zs = obj.value;}
    ...
    document.getElementById("eWebEditor1").contentWindow.syncText();
...
function addNameTR(sid,tid)
{
	...
	tr.appendChild(td0);
	td0.align="center";
	var delHref = "<A title=\"点击添加费用明细\" onclick=\"addNameTR('"+sid+"','"+tid+"')\" href=\"javascript:void(0);\"><img style=\"border:0px;\" src=\"../images/add.gif\" width=\"10\" height=\"10\" alt=\"点击添加费用明细\"></A>&nbsp;&nbsp;<a href='javascript:void(0);' onclick=\"delTR('"+tr.id+"');\"><img style=\"border:1px;\" src=\"../images/del2.gif\" alt=\"点击删除费用明细\" width=\"8\" height=\"7\"></a>";
	...
	tr.appendChild(td11);
	td11.align="center";
	td11.innerHTML="<INPUT name='retime_"+sid+"_"+eindex+"' onmouseup=\"toggleDatePicker('daysOfMonth3"+eindex+"','date.retime_"+sid+"_"+eindex+"')\" value=\""+window.nowDate+"\" readonly size=9 id=\"daysOfMonthPos3"+eindex+"\"><DIV id='daysOfMonth3"+eindex+"' style=\"POSITION: absolute\"></DIV>"
	}
	...
	tr.appendChild(td16);
	td16.align="center";
	td16.innerHTML="<INPUT name='ggdate_"+sid+"_"+eindex+"' onmouseup=\"toggleDatePicker('daysOfMonth6"+eindex+"','date.ggdate_"+sid+"_"+eindex+"')\" value=\""+window.nowDate+"\" readonly size=9 id=\"daysOfMonthPos6"+eindex+"\"><DIV id='daysOfMonth6"+eindex+"' style=\"POSITION: absolute\"></DIV>"
	}
	if(zdyarr[i]=="ggdx")
	{
	...
	tr.appendChild(td20);
	td20.align="center";
	td20.innerHTML="<INPUT name='xytime_"+sid+"_"+eindex+"' onmouseup=\"toggleDatePicker('daysOfMonth7"+eindex+"','date.xytime_"+sid+"_"+eindex+"')\" value=\""+window.nowDate+"\" readonly size=9 id=\"daysOfMonthPos7"+eindex+"\"><DIV id='daysOfMonth7"+eindex+"' style=\"POSITION: absolute\"></DIV>"
	}
	if(zdyarr[i]=="yt")
	{
...
function xmldata1(ord)
{
	...
	xmlHttp.onreadystatechange=function()
	{
		...
		if(xmlHttp.readyState==4)
		{
			...
			//alert(event.clientY);//鼠标的高度
			//alert(document.body.offsetHeight); 页面高度
			if(document.body.offsetHeight-event.clientY>460)
			{
				top = top;
			}
			else
			{
				top = top-(460-(document.body.offsetHeight-event.clientY))-20;
			}
...
function updatepage1(k,sort1)
{
	...
	if(xmlHttp.readyState==4)
	{
		...
		document.getElementById(k).value=ajaxtxt;
