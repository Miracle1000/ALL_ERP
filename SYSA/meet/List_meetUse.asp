...
'Private Sub Class_Initialize
	...
	if D1<>"" then
		Str_Result=Str_Result&" and use_meeting in ("&D1&")"
	end if
	if D2<>"" then
		Str_Result=Str_Result&" and use_complete in ("&D2&")"
	end if
	if D3<>"" then
		Str_Result=Str_Result&" and use_property in ("&D3&")"
	end if
	if D4<>"" then
		Str_Result=Str_Result&" and use_title like '%"&ml.De(D4)&"%'"
	end if
	if D6<>"" then
		Str_Result=Str_Result&" and use_meetingcycle in ("&D6&")"
	end if
	if D8<>"" then
		Str_Result=Str_Result&" and use_intro in ("&D8&")"
	end if
	if D13<>"" then
		Str_Result=Str_Result&" and use_sms in ("&D13&")"
	end if
	if D16<>"" then
		Str_Result=Str_Result&" and use_content like '%"&ml.De(D16)&"%'"
	end if
	if D17<>"" then
		Str_Result=Str_Result&" and use_important in("&D17&")"
	end if
	...
	if M<>"" and M<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&"',use_time)>=0 "
'if M<>"" and M<>"点击选择时间" then
	end if
	K=ChangeDate(K,2)
	if K<>"" and K<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,use_time,'"&K&"')>=0 "
'if K<>"" and K<>"点击选择时间" then
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" and L1<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&"',use_addtime)>=0 "
'if L1<>"" and L1<>"点击选择时间" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" and L2<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,use_addtime,'"&L2&"')>=0 "
'if L2<>"" and L2<>"点击选择时间" then
	end if
...
'if ret2<>"" and ret2<>"点击选择时间" then
	...
	elseif px="3" then
		Ptr_Result=" order by use_time desc"
	elseif px="4" then
		Ptr_Result=" order by use_time asc"
	elseif px="5" then
		Ptr_Result=" order by use_title desc"
	elseif px="6" then
		Ptr_Result=" order by use_title asc"
	end if
	if C<>"" then
		if B="chp" then
			Str_Result=Str_Result&" and use_title like '%"&ml.De(C)&"%'"
		elseif B="mc" then
			Str_Result=Str_Result&" and use_content like '%"&ml.De(C)&"%'"
		end if
	end if''
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_meetUse.asp"" method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "           <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">申请列表</td>" & vbcrlf & "                        <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">      " & vbcrlf & "                <tr>" & vbcrlf & "              <td colspan=""9"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0pxsolid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "         <span id=""kh"">" & vbcrlf & "            自：<input type=""text"" name=""ret"" value="""
	'call ml.UpdateMeeting
	Response.write ret
	Response.write """ id=""ret"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" readonly>&nbsp;至：<input type=""text"" name=""ret2"" value="""
	Response.write ret2
	Response.write """ id=""ret2"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" readonly>" & vbcrlf & "             <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                               <option value=""chp"" "
	if B="chp" then Response.write "selected"
	Response.write ">会议主题</option>" & vbcrlf & "                           <option value=""mc"" "
	if B="mc" then Response.write "selected"
	Response.write ">会议内容</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	Response.write """ size=""12""><input name=""page_count"" type=""hidden"" value="""
	...
	if open_104_10<>0 then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_meetUse.asp?px="
		Response.write px
		Response.write "&A="
	...
	if open_104_7=3 or CheckPurview(intro_104_7,trim(session("personzbintel2007")))=True then
	...
	if open_104_13<>0 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit4q3"" value=""会议室申请""  onClick=""window.location='Add_meetUse.asp';""   class=""anybutton2""/>" & vbcrlf & "                     "
	end if
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0);"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span>                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""5%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">会议主题</div></td>" & vbcrlf &"                  <td width=""13%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">会议室</div></td>" & vbcrlf & "                  <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">会议性质</div></td>" & vbcrlf & "                 <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">重要程度</div></td>" & vbcrlf & "                 <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">申请人员</div></td>" & vbcrlf & "              <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">审批状态</div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">审批人员</div></td>" & vbcrlf & "                <td width=""20%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                       <option>-请选择-</option>" & vbcrlf & "                    <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.met_name from O_MeetingUse a left join O_Meeting b on a.use_meeting=b.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			if open_104_3=3 or CheckPurview(intro_104_3,trim(use_cateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""left"">" & vbcrlf & "             "
			if open_104_14=3 or CheckPurview(intro_104_14,trim(use_cateid))=True then
				Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" onClick=""javascript:window.open('Detail_meetUse.asp?ord="
				Response.write rs7("id")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">" & vbcrlf & "                  "
				Response.write rs7("id")
			end if
			Response.write rs7("use_title")
			Response.write "</a></div></td>" & vbcrlf & "                 <td width=""13%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("met_name")
			Response.write "</div></td>" & vbcrlf & "             <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "            "
			use_property=ml.IsNum(rs7("use_property"))
			sql8="select set_name from O_MeetingSet where set_type=1 and id="&use_property
			...
			use_important=ml.IsNum(rs7("use_important"))
			sql8="select set_name from O_MeetingSet where set_type=2 and id="&use_important
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Response.write rs8("set_name")
			end if
			...
			use_addcateid=ml.IsNum(rs7("use_cateid"))
			...
			set rs8=nothing
			Response.write "" & vbcrlf & "                 </div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">" & vbcrlf & "           "
			...
			Response.write "" & vbcrlf & "           </div></td>" & vbcrlf & "                   <td align=""left"" valign=""middle"" class=""func""><div align=""center"">" & vbcrlf & "               "
			use_cateid_sp=ml.IsNum(rs7("use_cateid_sp"))
			sql8="select * from gate where ord="&use_cateid_sp
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				Response.write rs8("name")
			end if
			set rs8=nothing
			Response.write "" & vbcrlf & "                 </div></td>" & vbcrlf & "             <td width=""22%"" height=""30"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if (open_104_14=3 or CheckPurview(intro_104_14,trim(use_cateid))=True) and (rs7("use_cateid_sp")=session("personzbintel2007")) and (rs7("use_complete")="1" or rs7("use_complete")="2") then
			...
			sql8="select * from O_MeetingSummary where sum_metId="&rs7("id")
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				summary=false
			else
			...
			if summary and rs7("use_jyy")=session("personzbintel2007") and rs7("use_complete")="3" and rs7("use_state")=2 then
				Response.write "" & vbcrlf & "                   <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Add_meetSummary.asp?ord="
				Response.write rs7("id")
				Response.write "','neww35win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" value=""纪要""/>" & vbcrlf & "               "
				Response.write rs7("id")
			end if
			if open_104_14=3 or CheckPurview(intro_104_14,trim(use_cateid))=True then
				Response.write "" & vbcrlf & "                   <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_meetUse.asp?ord="
				Response.write rs7("id")
			...
			if ((open_104_2=3 or CheckPurview(intro_104_2,trim(use_cateid))=True)) and rs7("use_complete")="1" then
				Response.write "" & vbcrlf & "                       <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_meetUse.asp?ord="
				Response.write rs7("id")
			...
			if open_104_3=3 or CheckPurview(intro_104_3,trim(use_cateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_meetUse.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30"">全选<input name=""chkall"" type=""checkbox""id=""chkall"" value=""chkall"" onClick=""mm(this.form)""></td>" & vbcrlf & "         <td>" & vbcrlf & "            "
	if open_104_3="3" or (open_104_3="1" and intro_104_3<>"0") then
		Response.write "" & vbcrlf & "             <input name=""submit6"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit6""  style=""cursor:hand""  value=""批量删除"" onClick=""testall()""/>" & vbcrlf & "                "
	end if
	Response.write "" & vbcrlf & "             </td>" & vbcrlf & "     </form>" & vbcrlf & "       <form name=""oneself"" action=""List_meetUse.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "条信息/页            " & vbcrlf & "                <input name=""CurrPage"" type=""text"" size=""4"" maxlength=""6"" onkeyup=""this.value=this.value.replace(/[^\d]/g,'')""/>" & vbcrlf & "          <input name=""btntz"" type=""submit""  class=""anybutton2"" id=""btntz"" value=""跳转"" />" & vbcrlf & "          "
	if currpage=1 then
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按申请时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按申请时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按会议主题排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按会议主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="申请列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & "  window.location=""List_meetUse.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&remind="
	Response.write remind
	Response.write """;" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "       document.getElementById('data').action=""Del_meetUse.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
