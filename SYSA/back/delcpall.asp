...
'If Len(exists_Pics) = 0 Then
	...
	wend
	rs.close
	sql="select a.ord,a.intro3 into #CPRelaList from product a left join dbo.Check_ProductAll(0) b on a.ord = b.ord where a.del=2 and b.ord is null and a.ord not in (select id from #RelaList)"
	conn.execute(sql)
	conn.execute("delete from product where ord in (select ord from #CPRelaList)")
	conn.execute "delete ERP_CustomValues where id in (select b.id from ERP_CustomFields  a " _
	& " inner join ERP_CustomValues b on a.tname=21 and a.id=b.fieldsid " _
	& " inner join #CPRelaList c on c.ord= b.orderid " _
	& ") "
	conn.execute "delete jiage where product in (select ord from #CPRelaList) "
	intro_old = sdk.getsqldata("select intro3 from #CPRelaList",",")
	...
	dim actinon1
	action1="产品清空"
	if conn.execute("select 1 from product a left join dbo.Check_ProductAll(0) b on a.ord = b.ord where a.del=2 and b.ord is not null and a.ord not in (select id from #RelaList)").eof=false then
		alerttxt=ReturnMsg(idtest)
		Response.write "<script>alert('此产品正在被使用，请把其关联单据彻底删除后，再进行产品的彻底删除！');window.location='product.asp?currpage="&currpage&"';</script>"
		call close_list(2)
		call db_close : Response.end()
	end if
	call close_list(2)
	function ReturnMsg(ByVal Text)
		dim Rtext
		Rtext=""
		if instr(Text,",1,")>0 then
			Rtext=Rtext&"项目 "
		...
		elseif instr(Text,",4,")>0 then
			Rtext=Rtext&"销售退货 "
		elseif instr(Text,",5,")>0 then
			Rtext=Rtext&"预购 "
		elseif instr(Text,",6,")>0 then
			Rtext=Rtext&"询价 "
		elseif instr(Text,",7,")>0 then
			Rtext=Rtext&"采购 "
		elseif instr(Text,",8,")>0 then
			Rtext=Rtext&"采购退货 "
		elseif instr(Text,",9,")>0 then
			Rtext=Rtext&"入库 "
		elseif instr(Text,",10,")>0 then
			Rtext=Rtext&"出库 "
		elseif instr(Text,",11,")>0 then
			Rtext=Rtext&"调拨 "
		elseif instr(Text,",12,")>0 then
			Rtext=Rtext&"盘点 "
		elseif instr(Text,",13,")>0 then
			Rtext=Rtext&"借货 "
		elseif instr(Text,",14,")>0 then
			Rtext=Rtext&"组装/销售开单/发货 "
		elseif instr(Text,",15,")>0 then
			Rtext=Rtext&"组装清单 "
		elseif instr(Text,",16,")>0 then
			Rtext=Rtext&"发货 "
		elseif instr(Text,",17,")>0 then
			Rtext=Rtext&"预测单 "
		elseif instr(Text,",18,")>0 then
			Rtext=Rtext&"生产计划 "
		elseif instr(Text,",19,")>0 then
			Rtext=Rtext&"生产订单 "
		elseif instr(Text,",20,")>0 then
			Rtext=Rtext&"生产计划产品清单 "
		elseif instr(Text,",21,")>0 then
			Rtext=Rtext&"下达 "
		elseif instr(Text,",22,")>0 then
			Rtext=Rtext&"派工 "
		elseif instr(Text,",23,")>0 then
			Rtext=Rtext&"领料\补料\退料\废料 "
		elseif instr(Text,",24,")>0 then
			Rtext=Rtext&"物料调拨 "
		elseif instr(Text,",26,")>0 or instr(Text,",25,")>0 then
			Rtext=Rtext&"委外明细 "
		elseif instr(Text,",27,")>0 then
			Rtext=Rtext&"物料清单 "
		elseif instr(Text,",57,")>0 then
			Rtext=Rtext&"物料清单组件 "
		elseif instr(Text,",28,")>0 then
			Rtext=Rtext&"适用产品 "
		elseif instr(Text,",29,")>0 then
			Rtext=Rtext&"工价清单 "
		elseif instr(Text,",30,")>0 then
			Rtext=Rtext&"物料清单（增强） "
		end if
		ReturnMsg=Rtext
	end function
	Response.redirect "product.asp?currpage="&currpage&""
