...
'Set lvw = Nothing
	...
	function menu(sql,id,parentid,defvalue,objname, nodes)
		Dim rs , nd
		Set rs = cn.execute(Replace(Replace(sql,"@id",id),"@parentid",parentid))
		while not rs.eof
			Set nd = nodes.add()
			nd.Text = rs("title").value
			nd.value = rs("ord").value
			nd.ico = "@img/ico16/fd_open.gif"
			nd.ico2 = "@img/ico16/fd_close.gif"
			nd.canSelect = false
			Call menu(sql,-1 , nd.value ,defvalue,objname, nd.nodes)
'nd.canSelect = false
			If nd.nodes.count = 0 Then
				nd.ico = "@img/ico16/item.gif"
				nd.canSelect = true
	...
	Sub page_load
		Dim tvw
		Response.write app.DefTopBarHTML("../../" , "", "选择", "")
		Dim selId,selType,defvalue,objname,sql,rs
		selType=request("selType")
		selType = 0
		defvalue=request("defvalue")
		objname=request("objname")'返回值 对象 name=objname 为隐藏值  "s_" & objname & "_txt" 对象为text值
		sql = "select * from Page_sort where TableID = 4 and TypeID = 1 and del = 1 and SortID = @parentid order by px desc"
		If selType="0" Then
			Set tvw = New TreeView
			Call menu(sql,-1,0,CStr(defvalue), objname, tvw.nodes)
'Set tvw = New TreeView
			Response.write tvw.HTML
		end if
		Response.write "" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "tvw.onitemclick = function(a){" & vbcrlf & "    if (window.opener)" & vbcrlf & "      {" & vbcrlf & "               var objname = """
		Response.write objname
		Response.write """;" & vbcrlf & "         window.opener.document.getElementsByName(objname)[0].value=a.value;" & vbcrlf & "             window.opener.document.getElementById(""s_""+objname+""_txt"").value=a.text;" & vbcrlf & "    }" & vbcrlf & "       window.close();" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "$(document).ready(function(e) {" & vbcrlf & " $(""body"").css({""min-width"":""2000px""});" & vbcrlf & "        $(""html"").css({""overflow-x"":""hidden""});" & vbcrlf & "});" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
		'Response.write objname
	end sub
%>
