...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		Dim sql
		rpt.title= "进度产量登记条码"
		sql ="select 1 as 序号 , '派工产品' as 工价编号 ,'产量登记' as 工序 , '1' as 一维条形码 , '1' as 二维条形码 "
		rpt.sql = sql
		rpt.canExcel = False
		rpt.canPrint = False
	...
	Sub app_oncreate(lvw)
		lvw.cansort= False
		lvw.PageBar = False
		lvw.headers("一维条形码").formatText = "code:showImageCode(1 , @value , rs(""工序""))"
		lvw.headers("二维条形码").formatText = "code:showImageCode(2 , @value , rs(""工序""))"
	end sub
	Function showImageCode(stype ,v , code)
		Dim s ,imgurl
		If stype=2 Then
			imgurl = "../code2/view.asp?sn=view&ct=46&data=CLDJ:"& v &"&width=120&errorh=3"
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('../code2/viewImage.asp?title="& server.urlencode(code) &"&imgurl=" & server.urlencode(imgurl) & "','imgurl_2','width=' + 320 + ',height=' + 320 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')""><img width='30' title='产量登记二维码' src='"& imgurl &"' style='padding-top:10px;cursor:pointer'></a>"
			imgurl = "../code2/view.asp?sn=view&ct=46&data=CLDJ:"& v &"&width=120&errorh=3"
		else
			imgurl = "../code2/viewCode.asp?codeType=128&data=CLDJ:"& v &"&height=60"
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('../code2/viewImage.asp?codeType=128&title="& server.urlencode(code) &"&imgurl=" & server.urlencode(imgurl) & "&t="&now()&"','imgurl_1','width=' + 320 + ',height=' + 320 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')""><img width='100' height='30' title='产量登记一维码' src='"& imgurl &"' style='padding-top:10px;cursor:pointer'></a>"
			imgurl = "../code2/viewCode.asp?codeType=128&data=CLDJ:"& v &"&height=60"
		end if
		showImageCode = s
	end function
