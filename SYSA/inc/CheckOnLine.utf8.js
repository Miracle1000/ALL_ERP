...
else if (window.attachEvent) {window.attachEvent('onbeforeunload', BeforeUnload);}
else{window.onunload = BeforeUnload;}
...
function WinManager()
{
	...
	{
		for (var i = 0; i < me.Windows.length; i++) {
			try { me.Windows[i].obj.DisableWindow(); }
			catch (e1) { }
		}
	};
	...
	me.EnableWindow = function() 
	{
		for(var i=0;i<me.Windows.length;i++)
		{
			try { me.Windows[i].obj.EnableWindow(); }
			catch (e1) { }
		}
	};
	...
	{
		for (var i = 0; i < me.Windows.length; i++) {
			try { if (me.Windows[i].obj.document.body && me.Windows[i].obj != window) { return true; } }
			catch (e1) { continue; }
		}
		return false;
	};
	...
	me.Recycle = function()
	{
		...
		while (i < me.Windows.length) {
			try { if (me.Windows[i].obj.document.body) { i++; continue; } }
			catch (e1) { me.Windows.splice(i, 1); }
		}
	}
	return me;
}
...
window.getSessionHttpErrorTick = 0;
function getSession(tout) {
    ...
    xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4)
		{
			...
			{
				if (window.getSessionHttpErrorTick < 3)
				{
					window.getSessionHttpErrorTick++;
					return;
				}
			}
			window.getSessionHttpErrorTick = 0;
			...
			if(response.indexOf("{result:")>=0) 
			{
				...
				if(res.result == "0") {
					...
					if (res.result == "1") {
						...
						if (res.userid > 0 && (res.userid + "") != top.UserUniqueID && top.UserUniqueID) {
...
function showreloginDivWindow(r) {
	...
	div.style.display = "block";
	div.innerHTML = "<div class='reloginBox'><div class='logout-link' style='float:right;padding-right:5px'><a href='login.ashx'>直接退出系统</a></div>"
					+"<div class='logout-desc' style='font-weight:bold;color:#000'>重新登录</div></div>"
					+"<iframe class='relogin-frame' scrolling=no frameborder=0 style='border-radius:2px;border:1px solid #909aaa;display:block;width:600px;height:428px;margin:0px auto' src='" + window.virpath + "SYSN/view/init/relogin.ashx?sver=" + window.SysConfig.ProductVersion + "&dlg=" + t.getTime() + "&res=" + r + "&unique=" + UserUniqueID + "&sid=" + UserUniqueSID + "'></iframe>"
}
