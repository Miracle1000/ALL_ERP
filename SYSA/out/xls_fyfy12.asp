	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				xlWorksheet.Cells(1,3).Value = "发货名称"
				xlWorksheet.Cells(1,3).font.Size=10
				xlWorksheet.Cells(1,3).font.bold=true
				xlWorksheet.Cells(1,4).Value = "发货编号"
				xlWorksheet.Cells(1,4).font.Size=10
				xlWorksheet.Cells(1,4).font.bold=true
				xlWorksheet.Cells(1,5).Value = "发货方式"
				xlWorksheet.Cells(1,5).font.Size=10
				xlWorksheet.Cells(1,5).font.bold=true
				i=0
				if ZBRuntime.MC(1001) then
					xlWorksheet.Cells(1,6).Value = "客户名称"
					xlWorksheet.Cells(1,6).font.Size=10
					xlWorksheet.Cells(1,6).font.bold=true
					i=i+1
					xlWorksheet.Cells(1,6).font.bold=true
				end if
				xlWorksheet.Cells(1,6+i).Value = "发货人员"
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,6+i).font.Size=10
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,6+i).font.bold=true
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,7+i).Value = "币种"
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,7+i).font.Size=10
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,7+i).font.bold=true
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,8+i).Value = "费用申请"
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,8+i).font.Size=10
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,8+i).font.bold=true
				xlWorksheet.Cells(1,6).font.bold=true
				xlApplication.ActiveSheet.Columns(8+i).NumberFormatLocal = "#,##0"&dotstr&"_ "
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,9+i).Value = "费用使用"
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,9+i).font.Size=10
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,9+i).font.bold=true
				xlWorksheet.Cells(1,6).font.bold=true
				xlApplication.ActiveSheet.Columns(9+i).NumberFormatLocal = "#,##0"&dotstr&"_ "
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,10+i).Value = "费用报销"
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,10+i).font.Size=10
				xlWorksheet.Cells(1,6).font.bold=true
				xlWorksheet.Cells(1,10+i).font.bold=true
				xlWorksheet.Cells(1,6).font.bold=true
				xlApplication.ActiveSheet.Columns(10+i).NumberFormatLocal = "#,##0"&dotstr&"_ "
				xlWorksheet.Cells(1,6).font.bold=true
...
'if B="shmc" then
					elseif B="code" then
						str_Result=str_Result+" and code like '%"& C &"%' "
'elseif B="code" then
					elseif B="khmc" then
...
'if khmc<>"" then
				...
				if code<>"" then
					str_Result=str_Result+" and code like '%"&code&"%' "
'if code<>"" then
				end if
				if shmc<>"" then
...
'if shmc<>"" then
				...
				if sort<>"" then
					str_Result=str_Result+" and sort ="&sort&" "
'if sort<>"" then
				end if
				...
				n=0
				sql="select * from("&_
				"select send.ord,send.title,send.sort,send.code,send.company,send.cateid,sortbz.id as sortbzid,sortbz.sort1 as sortbzname, "&_
				"paytype.id,paytype.sort1,paytype.sort2,"&_
				"(select sort1 from sortonehy where ord=paytype.sort2) as sortname,"&_
				"(select isnull(sum(money1),0) from paysqlist where del=1 and fahuo=send.ord and sort1=paytype.id and fid in  "&_
				"(select id from paysq where bz=sortbz.id "&str_Result11&" ) ) as money1,"&_
				"(select isnull(sum(money1),0) from pay where del=1 and complete=3 and sort=paytype.id and  fahuo=send.ord and fid in "&_
				...
				"(select ord from pay where fahuo=send.ord "&str_Result44&") and bxid in (select id from paybx where bz=sortbz.id "&str_Result33&" ) ) as money3 "&_
				"from send,paytype,sortbz "&_
				"where send.del=1 and "&_
				"("&_
				"send.ord in (select fahuo from paysqlist where del=1) or send.ord in (select fahuo from pay where del=1 and complete=3)"&_
				")"&_
				"and "&_
				"("&_
				...
				rs.open sql,conn,1,1
				C1=rs.recordcount
				xlApplication.Init "发货费用明细表_"&session("name2006chen")&".xls",C1
				dim i
				i=1
				if rs.RecordCount<=0 then
					...
					do until rs.eof
						...
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select * from tel where ord="&company&""
						rs7.open sql7,conn,1,1
						if rs7.eof then
							companyname=""
						else
						...
						set rs7=nothing
						xlWorksheet.Cells(1+i,4).Value = rs("code")
						set rs7=nothing
						xlWorksheet.Cells(1+i,4).font.Size=10
						set rs7=nothing
						xlWorksheet.Cells(1+i,5).Value = so
						set rs7=nothing
						xlWorksheet.Cells(1+i,5).font.Size=10
						set rs7=nothing
						j=0
						if ZBRuntime.MC(1001) then
							xlWorksheet.Cells(1+i,6).Value = companyname
'if ZBRuntime.MC(1001) then
...
'if ZBRuntime.MC(1001) then
						...
						xlWorksheet.Cells(1+i,6+j).Value = per
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,6+j).font.Size=10
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,7+j).Value = rs("sortbzname")
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,7+j).font.Size=10
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,8+j).Value = Formatnumber(money1,num_dot_xs,-1)
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,8+j).font.Size=10
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,9+j).Value = Formatnumber(money2,num_dot_xs,-1)
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,9+j).font.Size=10
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,10+j).Value = Formatnumber(money3,num_dot_xs,-1)
'if ZBRuntime.MC(1001) then
						xlWorksheet.Cells(1+i,10+j).font.Size=10
'if ZBRuntime.MC(1001) then
...
'Loop
					xlWorksheet.Cells(1+i,1).colspan = 8
'Loop
...
'Loop
				...
				rs.close
				set rs=Nothing
				Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>发货费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!发货费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
				set rs=Nothing
				Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
				Set fs = CreateObject("Scripting.FileSystemObject")
				tfile=Server.MapPath("发货费用明细表_"&session("name2006chen")&".xls")
				...
				Response.write """><font class=""red""><strong><u>下载导出的发货费用明细表</u></strong></font></a></p>" & vbcrlf & ""
