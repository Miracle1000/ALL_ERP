...
'Private Sub Class_Initialize
	...
	Sub App_doDel
		...
		cn.execute("update sortFieldsContent set del=2 where id="&ord)
		Response.write "1"
		call db_close : Response.end()
	end sub
	Sub App_doDelAll
		Dim ordlist, oSQL, pwdel , rs , Scplist, qx_sort1
		ordlist = app.GetText("ordlist")
		qx_sort1 = app.GetInt("qx_sort1")
		If Len(ordlist)>0 Then
			pwdel = app.power.GetPowerIntro(qx_sort1,3)
			If cn.execute("select count(1) from sortFieldsContent where sort=1 and del=1 and id in ( "& ordlist &" ) and ord in (select ord from tel where del=1 and (  '" & pwdel & "'='' or charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')>0 ))")(0)>0 Then
'pwdel = app.power.GetPowerIntro(qx_sort1,3)
				Set rs = cn.execute("select id from sortFieldsContent where sort=1 and del=1 and id in ( "& ordlist &" ) and ord in (select ord from tel where del=1 and ('" & pwdel & "'<>'' and ((charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')=0 and isnull(cateid,0)<>0) or isnull(cateid,0)=0) ) )")
				While rs.eof = False
				...
				oSQL = "update sortFieldsContent set del=2 Where sort=1 and del=1 and id in ( "& ordlist &" ) "
				If Len(Scplist)>0 Then oSQL = oSQL & " and id not in ("&Scplist &")"
				cn.Execute(oSQL)
				Response.write(1)
			else
				Response.write(0)
	...
	Sub onReportInit(rpt, lt)
		Dim dat, rs ,sort, title,qx_sort1
		sort = request("sort")
		If sort&""="" Then sort = 1
		If sort = 1 Or sort=3 Then
		...
		Call app.addDefaultScript() '加载本文件关联JS ="../skin/default/js/qs_planall.js
		dat = Split(",", ",")    'year(now) & "-" & month(now) & "-1," & date
'Call app.addDefaultScript() '加载本文件关联JS ="../skin/default/js/qs_planall.js
		rpt.title = title&"资质列表"
		rpt.showtoppagesize = false
		rpt.pagesize = 10
		rpt.firstSpace = 200
		rpt.addfield "日期：","dates","date2","","", dat
		rpt.addfield "&nbsp;","select","searchType", "资质类型" & Chr(1) & "2","",""
		rpt.SetNullMsg 1,khname
		rpt.addfield "","text","searckKey", "", "", ""
		rpt.addfield "","hidden","sort","","",sort
		dim remind : remind = app.getInt("remind")
		if remind <> 0 then
			dim insertSql,helper
			Set helper = CreateReminderHelper(cn,remind,0)
			insertSql =  " insert into #id select * from (" & helper.listSQL("ids") & ") a "
		end if
		rpt.sql = " set nocount on /*必须静态游标*/"&_
		" create table #id(id int) "&_
		insertSql&_
		"exec erp_Qualifications_list "& Info.User&",@sort, @date2_0, @date2_1, @searchType, @searckKey, &SearchMode,&excelmode,&ReportSort, &pagesize, &pageindex,@remind"&_
		" drop table #id "&_
		" set nocount off "
		rpt.addsort "添加时间", "date1"
		rpt.addsort khname, "name"
		rpt.addsort "资质类型", "sortid"
		rpt.addsort "添加人", "cateadd"
		rpt.canexcel = app.power.existsPower(qx_sort1,10)
		rpt.canprint = app.power.existsPower(qx_sort1,7)
		sort = request("sort")
		if sort=1 then
			name="客户"
		else
		...
		rpt.bottomhtml = "<p style='color:red;margin-left:10px'>温馨提示</p>"
	end sub
	dim power_3,intro_3,intro_2,intro_14
	Sub App_OnListviewExcel(ByRef lvw)
		...
		col.width = 160
		col.align = "left"
		lvw.headers(6).Formula = "GetPerson(@value)"
	...
	sub app_oncreate(lvw)
		Dim h, i, col,sort,title,qx_sort1
		sort = app.GetInt("rpt_f_sort")
		If sort = 1 Or sort=3 Then
			qx_sort1=1
		else
			qx_sort1= 26
		...
		col.width = 130
		col.align = "left"
		lvw.headers(2).formattext = "code:GetTitleHtml(@cells[2],""@value"",@cells[9],@cells[12],"&qx_sort1&")"
		Set col = lvw.headers(3)
		col.width = 120
		Set col = lvw.headers(4)
		col.width = 120
		...
		col.width = 160
		col.align = "left"
		lvw.headers(6).formattext = "code:GetPerson(""@value"")"
		Set col = lvw.headers(7)
		col.width = 80
		Set col = lvw.headers(8)
		col.width = 150
		col.formattext = "code:OperateButtons(@cells[2],@cells[9],@cells[12],@cells[13],"&qx_sort1&",""@cells[14]"",""@cells[10]"",@cells[15],"&app.getInt("remind")&",@cells[10],@cells[11])"
		col.canSum=False
		Set col = lvw.headers(9)
		col.width = 0
		col.display = "none"
		...
		dim btmStr
		btmStr = ""
		If Len(power_3) = 0 Then power_3 = app.power.existsPower(qx_sort1,3)
		if power_3 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='OperateDeleteAll("&qx_sort1&")'>批量删除</button>"
		btmStr = btmStr& "<a>"
		lvw.addlink = "</a><input type='checkbox' onclick='selectall(this)'>全选"& btmStr
	end sub
	function GetTitleHtml(zzid,title,ord,cateid_kh,qx_sort1)
		If isEmpty(intro_14) Then    intro_14 = app.power.GetPowerIntro(qx_sort1,14)
		dim TitleStr : TitleStr = ""
		Dim frmContent : frmContent = "work"
		If qx_sort1 = 26 Then frmContent = "work2"
		if intro_14="" or (instr(","& intro_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
			TitleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../"&frmContent&"/content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& title &"</a>"
'if intro_14="" or (instr(","& intro_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
		else
			TitleStr = title
		end if
		...
		If Len(Scplist)>0 Then
			If InStr(","&Scplist&",",","& zzid &",") Then TitleStr = TitleStr & "&nbsp;<span style='color:red'>不允许删除</span>"
		end if
		GetTitleHtml = TitleStr
	end function
	function GetPerson(personStr)
		Dim personName,person_rs
		personName=""
		If Len(personstr)>0 Then
			Set person_rs=cn.execute("select name from gate where del=1 and ord in ("&personstr&")")
	...
	function OperateButtons(id,tel_ord,cateid,sortid,qx_sort1,alt,spStatus,status_tx,remindID,status_sp_qualifications,cateid_sp_qualifications)
		sort = request("sort")
		dim actStr,rid,helper
		If remindID > 0 Then
			Set helper = CreateReminderHelper(cn,remindID,0)
			rid = helper.getRemindIdByOID(id)
		end if
		If isEmpty(intro_14) Then intro_14 = app.power.GetPowerIntro(qx_sort1,14)
		If isEmpty(intro_26_16) Then intro_26_16 = app.power.GetPowerIntro(qx_sort1,16)
		If isEmpty(intro_1_16) Then intro_1_16 = app.power.GetPowerIntro(qx_sort1,16)
		actStr = actStr &"<div style='word-wrap: break-word; word-break: normal;width:100%;'>"
'If isEmpty(intro_1_16) Then intro_1_16 = app.power.GetPowerIntro(qx_sort1,16)
		If remindID > 0 Then
			If remindID = 149 Or remindID = 148 Then
				If spStatus=4 Or spStatus=0 Then
					actStr = actStr &"<a href='javascript:;' onclick='sendCancelRequest("&rid&","&remindID&")' ><img src='../images/alt3.gif' alt='取消提醒' border='0'></a>&nbsp;"
				end if
			else
				actStr = actStr &"<a href='javascript:;' onclick='sendCancelRequest("&rid&","&remindID&")' ><img src='../images/alt3.gif' alt='取消提醒' border='0'></a>&nbsp;"
			end if
		end if
		if intro_14="" or (instr(","& intro_14 &"," , ","& cateid &",")>0 And cateid<>"0" ) then
			actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(id) & "','newwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">详情</button>"
'if intro_14="" or (instr(","& intro_14 &"," , ","& cateid &",")>0 And cateid<>"0" ) then
		end if
		If sort= 2 then
			If status_sp_qualifications>0 Then
				if (intro_26_16="" or (instr(","& intro_26_16 &"," , ","& cateid &",")>0 And cateid<>"0" ))  And cateid_sp_qualifications=session("personzbintel2007") then
					actStr = actStr & "<button class='oldbutton2 buttons' onclick=""javascript:window.open('../inc/CommSPSet.asp?ord="&app.base64.pwurl(tel_ord)&"&sort1=93','newwin5','width=' + 800 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">审批</button>"
				end if
			end if
		else
			If status_sp_qualifications>0 Then
				if (intro_1_16="" or (instr(","& intro_1_16 &"," , ","& cateid &",")>0 And cateid<>"0" ))  And cateid_sp_qualifications=session("personzbintel2007") then
					actStr = actStr & "<button class='oldbutton2 buttons' onclick=""javascript:window.open('../inc/CommSPSet.asp?ord="&app.base64.pwurl(tel_ord)&"&sort1=92','newwin5','width=' + 800 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">审批</button>"
				end if
			end if
		end if
		If isEmpty(intro_2) Then intro_2 = app.power.GetPowerIntro(qx_sort1,2)
		if (intro_2="" or (instr(","& intro_2 &"," , ","& cateid &",")>0 and cateid<>"0")) then
			actStr = actStr & "<button class='oldbutton2 buttons' onclick=""javascript:window.open('edit_qualifications.asp?ord="&app.base64.pwurl(tel_ord)&"&sort1="&sortid&"','xg');"">修改</button>"
		end if
		If isEmpty(intro_3) Then intro_3 = app.power.GetPowerIntro(qx_sort1,3)
		if intro_3="" or (instr(","& intro_3 &"," , ","& cateid &",")>0 and cateid<>"0") Then
			actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:OperateDelete("& id &")"">删除</button>"
		end if
		actStr = actStr &"</div>"
