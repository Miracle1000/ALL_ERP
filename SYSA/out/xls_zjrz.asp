...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlWorksheet.Cells(1,6).Value = "入账概要"
...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from gate where ord=bankin.cateid) as cateidname" &_
			",bank=(select top 1 sort1 from sortbank where id=bankin.ck and ','+replace(convert(char,person),' ','')+',' like '%,"&session("personzbintel2007")&",%')" & _
			"from bankin "&Str_Result&"" &px_Result&""
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "直接入账单_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				...
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,6).Value = Replace(Replace(Replace(Replace(intro,"<br>",vbcrlf),"</P>",vbcrlf),"<P>",vbcrlf),"&nbsp;"," ")
				money1=sortbz&money1
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>直接入账单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!直接入账单导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("直接入账单_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的直接入账单</u></strong></font></a></p>"
