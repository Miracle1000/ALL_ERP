...
'Private Sub Class_Initialize
	...
	Dim helper,remind
	dim areamenu_list,areamenu2
	Sub onReportInit(rpt, lt)
		areamenu_list=request("areamenu_list")
		areamenu2=request("areamenu2")
		IF areamenu_list <> "" Then
			areamenu2 =""
		end if
		IF areamenu2 <> "" Then
			areamenu_list= ""
		...
		Call app.addDefaultScript()
		Set rs=cn.execute("select id,Sort1,gate1 from sortonehy where gate2='10' and (id='"&areamenu_list&"' or id='"&areamenu2&"')")
		IF not rs.eof Then
			rpt.title = rs("Sort1")
		else
			rpt.title = "所有文档列表"
		...
		rpt.pagesize = 10
		rpt.firstSpace = 180
		rpt.addfield "时间：","dates","date1","","", dat
		rpt.addfield "&nbsp;","select","zt1", "未审批" & Chr(1) & "2"& Chr(2) & "审批中" & Chr(1) & "3"& Chr(2) &"审批通过" & Chr(1) & "1" & Chr(2) & "审批未通过" & Chr(1) & "-1"& Chr(2) &  "部分归档" & Chr(1) & "4"& Chr(2) & "已归档" & Chr(1) & "5","",""
		rpt.addfield "时间：","dates","date1","","", dat
		rpt.SetNullMsg 1,"状态"
		...
		do while rs.eof = False
			if i=1 then
				SC_level=rs("Sort1")& Chr(1)&rs("id")
			else
				SC_level=SC_level&Chr(2)&rs("Sort1")& Chr(1)&rs("id")
			end if
			i=i+1
			SC_level=SC_level&Chr(2)&rs("Sort1")& Chr(1)&rs("id")
			rs.movenext
		loop
		set rs=nothing
		rpt.addfield "&nbsp;","select","c_level",SC_level,"",""
		rpt.SetNullMsg 2,"机密级别"
		rpt.addfield "&nbsp;","select","searchType", "标题" & Chr(1) & "1","",""
		rpt.SetNullMsg 3,"添加人"
		rpt.addfield "","text","searckKey", "", "", ""
		rpt.aSearch.addField "添加人员", "gates", "g_tjry",""
		rpt.aSearch.addField "状态","checks", "zt","select 2 as value, '未审批' as name union all select 3, '审批中'  union all select 1, '审批通过'  union all select -1, '审批未通过'  union all select 101, '部分归档'  union all select 102, '已归档' "
		rpt.aSearch.addField "添加人员", "gates", "g_tjry",""
		rpt.aSearch.addField "机密级别","checks", "c_","select id as value,Sort1 as name,gate1 from sortonehy where gate2='79' order by gate1 desc"
		rpt.aSearch.addField "标题","selectys", "sTitle",""
		rpt.aSearch.addField "文档编号","selectys", "swdid",""
		rpt.aSearch.addField "文件名称","selectys", "swdname",""
		rpt.aSearch.addField "","hidden", "remind",remind
		remind = app.getInt("remind")
		Dim remindSQL : remindSQL = ""
		if remind <> 0 then
			dim insertSql
			Set helper = CreateReminderHelper(cn,remind,0)
			remindSQL = Replace(helper.listSQL("ids"),"'","''")
		end if
		rpt.sql = " exec erp_editReport_document @g_tjry, @date1_0, @date1_1, @zt,@zt1, @c_ ,@c_level, @searchType, @searckKey, @sTitle_0,@sTitle_1,@swdid_0,@swdid_1,@swdname_0,@swdname_1,"& remind &",'"&areamenu_list&"','"&areamenu2&"'," & Info.User & ",'"& remindSQL &"',&SearchMode,&pagesize,&pageindex,'',"& remind &",'' "
		rpt.canexcel = false
		rpt.canprint = false
		rpt.onlyadSearchModel = True
		rpt.BillListUIModel = true
		rpt.beforeBtnsHTML = "<style>html{overflow-x:visible}</style>"
		rpt.BillListUIModel = true
		if app.power.existsPower2(78,13) then
			rpt.beforeBtnsHTML  = rpt.beforeBtnsHTML &"<button class='oldbutton3' onclick=""javascript:window.open('add.asp?bakll=ok','new2win','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">文档添加</button>"
'if app.power.existsPower2(78,13) then
		end if
	end sub
	dim open_78_2, intro_78_2, open_78_3, intro_78_3, open_78_14, intro_78_14, open_78_16, intro_78_16, open_78_20, intro_78_20
	dim power_78_3, power_78_20
	sub app_oncreate(lvw)
		...
		col.title = "序号"
		col.width = 40
		col.formattext = "code:i"
		Set col = lvw.headers(2)
		col.width = 150
		col.align="left"
		lvw.headers(2).formattext = "code:GetWDTitleHtml(@cells[16],""@value"",@cells[12])"
		...
		col.width = 200
		col.title = "操作"
		col.formattext = "code:WDActHTML(@cells[16],@cells[12],@cells[13],@cells[14],@cells[15])"
		...
		dim btmStr
		btmStr = ""
		power_78_3 = app.power.existsPower(78,3)
		power_78_20 = app.power.existsPower(78,20)
		if power_78_3 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='batDel()'>批量删除</button>"
		if power_78_20 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='batArch()'>批量归档</button>"
		btmStr = btmStr& "<a>"
		lvw.addlink = "</a><input type='checkbox' onclick='selectall(this)'>全选"& btmStr
	end sub
	function GetWDTitleHtml(wdord, wdTitle, addcate)
		If isEmpty(intro_78_14) Then
			intro_78_14 = app.power.GetPowerIntro(78,14)
		end if
		dim wdTitleStr , wdTitle2 , tipStr
		wdTitleStr = ""
		if wdTitle&"" = "" then wdTitle=""
		tipStr = "<span id='wd_tip_"& wdord &"' class=red></span>"
		if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
			wdTitleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(wdord) & "','newwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& wdTitle &"</a>"
'if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
		else
			wdTitleStr = wdTitle
		end if
		wdTitleStr = wdTitleStr & tipStr
		GetWDTitleHtml = wdTitleStr
	end function
...
'If rst.eof = true Then Exit For
	...
	function WDActHTML(wdord, addcate, cateid_sp, spFlag, wdArch)
		dim actStr
		if wdord&""<>"" Then
			If isEmpty(intro_78_2) Then intro_78_2 = app.power.GetPowerIntro(78,2)
			If isEmpty(intro_78_3) Then intro_78_3 = app.power.GetPowerIntro(78,3)
			If isEmpty(intro_78_14) Then intro_78_14 = app.power.GetPowerIntro(78,14)
			If isEmpty(intro_78_16) Then intro_78_16 = app.power.GetPowerIntro(78,16)
			If isEmpty(intro_78_20) Then intro_78_20 = app.power.GetPowerIntro(78,20)
			actStr = actStr &"<div style='word-wrap: break-word; word-break: normal;width:100%;'>"
'If isEmpty(intro_78_20) Then intro_78_20 = app.power.GetPowerIntro(78,20)
			remind = app.getInt("remind")
			if remind=0 then remind=203
			Dim rid
			If remind > 0 Then
				If isEmpty(helper) Then Set helper = CreateReminderHelper(cn,remind,0)
				rid = helper.getRemindIdByOID(wdord)
				if helper.canCancelOrder(rid) Then
					actStr = actStr & "<img src='../images/alt3.gif' alt='取消提醒' border='0' style='cursor:hand' onclick='sendCancelRequest(" & rid & "," & remind & ",0)'/>"
				end if
			end if
			if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
				actStr = actStr & "<input type=""button"" class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(wdord) & "','newwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""详情"">"
'if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
			end if
			if (intro_78_16="" or instr(","& intro_78_16 &"," , ","& addcate &",")>0) and cateid_sp=session("personzbintel2007") then
				if actStr&""<>"" then actStr = actStr & " "
				actStr = actStr & "<input type=""button"" class=""oldbutton2"" onclick=""javascript:window.open('set.asp?ord=" & app.base64.pwurl(wdord) & "','neww3win','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""审批"">"
				if actStr&""<>"" then actStr = actStr & " "
			end if
			If (wdArch=0 or wdArch=1) And ( ((spFlag=1 Or spFlag=-1 ) And (intro_78_2="" or instr(","& intro_78_2 &"," , ","& addcate &",")>0) ) Or (spFlag=2 And addcate = Info.user) ) then
				if actStr&""<>"" then actStr = actStr & " "
				if actStr&""<>"" then actStr = actStr & " "
				actStr = actStr & "<input type=""button"" class=""oldbutton2"" onclick=""javascript:window.open('correct.asp?ord=" & app.base64.pwurl(wdord) & "','new2win','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""修改"">"
				if actStr&""<>"" then actStr = actStr & " "
			end if
			if (intro_78_20="" or instr(","& intro_78_20 &"," , ","& addcate &",")>0) and spFlag=1 then
				if actStr&""<>"" then actStr = actStr & " "
				if (wdArch=1 or wdArch=0) then
					actStr = actStr & "<input type=""button"" class=""oldbutton2"" onclick=""javascript:toArchive('document',"& wdord &",1)"" value=""归档"">"
				elseif wdArch=2 then
					actStr = actStr & "<input type=""button"" class=""oldbutton3"" onclick=""javascript:toArchive('document',"& wdord &",0)"" value=""取消归档"">"
				end if
			end if
			if (intro_78_3="" or instr(","& intro_78_3 &"," , ","& addcate &",")>0) and wdArch=0 then
				if actStr&""<>"" then actStr = actStr & " "
				actStr = actStr & "<input type=""button"" class=""oldbutton2"" onclick=""javascript:delWD("& wdord &")"" value=""删除"">"
			end if
			actStr = actStr &"</div>"
		end if
		WDActHTML = actStr
