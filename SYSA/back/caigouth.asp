...
'bigsystemtype = ".mozi"
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write "' defer='defer'></script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf &   "        <tr>" & vbcrlf &           "  <td class=""place"">采购退货回收站</td> "& vbcrlf &      "       <td>&nbsp;</td> "& vbcrlf &    "         <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=caigouth&url=caigouth.asp&list1=caigouthlist'}""/></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "      </table> " & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "    <td align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf &"<td align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td> "& vbcrlf & "   <td height=""27"" ><div align=""center""><strong>退货主题</strong></div></td> "& vbcrlf &  "  <td ><div align=""center""><strong>负责人</strong></div></td> "& vbcrlf &    "    <td width=""14%"" ><div align=""center""><strong>删除人</strong></div>          <div align=""center""></div></td>" & vbcrlf & "         <td ><div align=""center"">删除日期</div></td>" & vbcrlf & "      <td width=""20%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	dim n,rs77,rs88
	dim k,ord,id,kuinlist,ifHasTrue,qcId
	ifHasTrue=0
	n=0
	qcId=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from caigouth where del=2 order by deldate desc"
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "       <form name=""form1"" method=""post"" action=""deletecgthall.asp?ord="
		...
		do until rs.eof
			id=rs("ord")
			k=rs("title")
			If len(rs("QcId"))>0 Then qcId=rs("QcId")
			if len(rs("QcId"))<=0 then qcId=rs("QcId") end if
			set rs77=conn.execute("select kuinlist from caigouthlist where caigouth="&id)
			do while not rs77.eof
				kuinlist=rs77("kuinlist")
				if ifHasTrue=0 then
					set rs88=conn.execute("select count(1) as hascount  from caigouthlist where kuinlist="&kuinlist&" and kuinlist!=0 and del=1")
					if rs88("hascount")>0 then
						ifHasTrue=1
					end if
					set rs88=conn.execute("select count(1) as qcCount from caigouth where FromModel=2 and QcId="&qcId&" and del=1 ")
					if rs88("qcCount")>0 then
						ifHasTrue=1
					end if
					rs88.close
					set rs88=nothing
			...
			Response.write "</td>" & vbcrlf & "      <td width=""24%"" height=""27"" class=""name"" ><a href=""javascript:;"" onclick=""javascript:window.open('../../SYSN/view/store/caigouth/PurchaseReturn.ashx?ord="
			...
			rs7.close
			set rs7=nothing
			Response.write "" & vbcrlf & "        <td width=""13%"" class=""name""><div align=""center"">"
			Response.write delcate
			Response.write "</div></td> " & vbcrlf & "         <td width=""13%"" class=""name""><div align=""center"">"
			Response.write rs("deldate")
			Response.write "</div></td>" & vbcrlf & "             <td> " & vbcrlf & "                             <div align=""center"">" & vbcrlf & "                        <input type=""button"" "
			if ifHasTrue=1 Or IsBillRecovery(4,rs("ord")) Or IsBillRecovery(6,rs("ord")) then Response.write "disabled"
			Response.write " name=""Submit3c""  class=""anybutton""  value=""恢复""  onClick=""if(confirm('确认恢复？')){window.location.href='setcgth.asp?ord="
			Response.write rs("ord")
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>" & vbcrlf & "                   <input type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletecgth.asp?ord="
			Response.write rs("ord")
			Response.write "&CurrPage="
			Response.write CurrPage
...
'& "        <input type=""button"" name=""Submit426"" value=""批量恢复""  onclick=""ask2();"" class=""anybutton2""/>" & vbcrlf & "            </td>" & vbcrlf & "    <td width=""69%""><div align=""right"">" & vbcrlf & "    "
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='caigouth.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='caigouth.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='caigouth.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='caigouth.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""setcgthall.asp?currPage="
		Response.write currPage
