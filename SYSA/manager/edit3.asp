...
'If Not rs.Eof Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {      margin-top: 0px;}" & vbcrlf & "form{margin:0px; padding:0px;}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf & "<!--" & vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0" & vbcrlf & "eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf & "if (restore) selObj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "function BoxCheck(e) {" & vbcrlf & "    $('.Del_Item_List').each(function (index, domEle) {" & vbcrlf & "                if (e.checked) {" & vbcrlf & "                        domEle.checked = true;" & vbcrlf & "        }" & vbcrlf & "         else {" & vbcrlf & "                  domEle.checked = false;" & vbcrlf & "        }" & vbcrlf & "    });" & vbcrlf & "}" & vbcrlf & "function GetSelectid(){" & vbcrlf& "        var tb = document.getElementById(""content"");" & vbcrlf & "      var SelectIds = $(""[name='selectid']"");" & vbcrlf & "   var ids = [];" & vbcrlf & "   SelectIds.each(function(){" & vbcrlf & "              if($(this).attr(""checked"")){" & vbcrlf & "                      ids.push($(this).val())" & vbcrlf & "         }" & vbcrlf & "       });" &vbcrlf & " if (ids.length == 0)" & vbcrlf & "    {" & vbcrlf & "               ids.push(0)" & vbcrlf & "     }" & vbcrlf & "       var url = ""../code2/inc/getCode2.asp?c2type=3&selectid=""+ids.join("","")" & vbcrlf & "      window.location.href = url;" & vbcrlf & "}" & vbcrlf & "function PrintAll(isSum){" & vbcrlf & "   var selectid = document.getElementsByName(""selectid"");" & vbcrlf & "  var ids = """";" & vbcrlf & "     for(var i = 0; i < selectid.length; i++){" & vbcrlf & "               if(selectid[i].checked){" & vbcrlf & "                        ids = ids + "","" + selectid[i].value;" & vbcrlf & "              }" & vbcrlf & "       }" & vbcrlf & "       ids = ids.replace("","",""""); "& vbcrlf & "   if(ids.length == 0){" & vbcrlf &  "           alert(""您没有选择任何信息，请选择后再打印！""); "& vbcrlf &   "          return false; "& vbcrlf & "   } "& vbcrlf &   "     ids = ids.split("",""); "& vbcrlf & "     if (ids.length > 50){alert(""选择的单据数量不要超过50个！"");return false;} "& vbcrlf &"  window.OpenNoUrlrinterResolve.asp?formid=' + ids + '&sort=2002&isSum='+isSum,'newwin77','width=' + 850 + ',height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=' + (screen.availWidth-850)/2  + ',top=0')" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "window.OpenNoUrl = function(url, name, attr) {" & vbcrlf & "     //通过代理的方式，屏蔽url" & vbcrlf & "       var urls = window.location.href.split(""/"");" & vbcrlf & "       urls[urls.length-1] = url;" & vbcrlf & "      window.currOpenNoUrl= urls.join(""/"");" & vbcrlf & "     window.open(  window.sysCurrPath + ""inc/datawin.asp"", name, attr);" & vbcrlf & "}"& vbcrlf & "" & vbcrlf & "//-->" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	open_67_13 = 0 : open_67_19 = 1 : open_77_13 = 0 : open_77_19 = 1 : open_44_9 = 0 : intro_44_9 = "0"
	qxsort1 = 0 : qxsort2 = 0
	Set rs = conn.execute("select sort1,sort2,qx_open,isnull(qx_intro,'0') qx_intro from power  where ord="&session("personzbintel2007")&" and sort1 in(44,67,77) and sort2 in(9,13,19) order by sort1,sort2")
	While rs.eof = False
		qxsort1 = rs("sort1") : qxsort2 = rs("sort2")
		If qxsort1&"" = "" Then qxsort1 = 0 Else qxsort1 = CInt(qxsort1)
		If qxsort2&"" = "" Then qxsort2 = 0 Else qxsort2 = CInt(qxsort2)
		Select Case qxsort1
		Case 67, 77
		If qxsort2 = 13 Or qxsort2 = 19 Then
			execute "open_"& qxsort1 &"_"& qxsort2 &"="& rs("qx_open")
		end if
		Case 44
		If qxsort2 = 9 Then
			execute "open_"& qxsort1 &"_"& qxsort2 &"="& rs("qx_open") &" : intro_"& qxsort1 &"_"& qxsort2 &"="""& rs("qx_intro")&""""
		end if
		End Select
		rs.movenext
	wend
	...
	if open_44_9=3 then
		list=""
	elseif open_44_9=1 then
		list="and ord in ("&intro_44_9&") "
	else
		list="and ord=0"
	end if
	Str_Result="where del=1 "&list&""
	E=request("E") : B=request("B") : C=Trim(request("C"))
	If E&"" = "" Then E = ""
	If E <> "" Then Str_Result = Str_Result &" and cateid="& E
	If B&"" = "" Then B = ""
	If C&"" = "" Then C = ""
	If C <> "" Then
		Str_Result = Str_Result &" and "& B &" like '%"& C &"%'"
	end if
	qx=request("qx")
	if qx="" then
		qx=1
	end if
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" >" & vbcrlf & "<tr>" & vbcrlf & "<td width=""100%"" valign=""top"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class=""place"">通讯录</td>" & vbcrlf & "     <td>&nbsp;</td>" & vbcrlf & " <td align=""right"">&nbsp;              </td>" & vbcrlf & "       <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "   </table> " & vbcrlf & "<formmethod=""get"" action=""edit3.asp"" id=""demo"" name=""date"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"" class=""zdy"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td background=""../images/112.gif"" height=""30"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;""> "& vbcrlf &     "       <select name=""E""> "& vbcrlf &  "          <option value="""">所有级别</option> "& vbcrlf & "                <option value=""1"""
	qx=1
	if E="1" then
		Response.write "selected"
	end if
	Response.write ">总经理级</option>" & vbcrlf & "              <option value=""2"" "
	if E="2" then
		Response.write "selected"
	end if
	Response.write ">经理级</option>" & vbcrlf & "                <option value=""3"" "
	if E="3" then
		Response.write "selected"
	end if
	Response.write ">主管级</option>" & vbcrlf & "                <option value=""4"" "
	if E="4" then
		Response.write "selected"
	end if
	Response.write ">代表级</option>" & vbcrlf & "              </select>" & vbcrlf & "               <select name=""B"">" & vbcrlf & "           <option value=""name"" "
	if B="name" then
		Response.write "selected"
	end if
	Response.write ">姓名</option>" & vbcrlf & "                  <option value=""phone1"" "
	if B="phone1" then
		Response.write "selected"
	end if
	Response.write ">办公电话</option>" & vbcrlf & "              <option value=""phone2"" "
	if B="phone2" then
		Response.write "selected"
	end if
	Response.write ">家庭电话</option>" & vbcrlf & "              <option value=""mobile"" "
	if B="mobile" then
		Response.write "selected"
	end if
	Response.write ">手机</option>" & vbcrlf & "                  <option value=""email"" "
	if B="email" then
		Response.write "selected"
	end if
	Response.write ">电子邮件</option>" & vbcrlf & "              <option value=""title"" "
	if B="title" then
		Response.write "selected"
	end if
	Response.write ">职位名称</option>" & vbcrlf & "           </select>" & vbcrlf & "               <input name=""C"" type=""text"" size=""15""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "               <input type=""submit"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                "
	...
	rs1.close
	set rs1=nothing
	if open_106_13=1 And ZBruntime.MC(63000) then
		Response.write "" & vbcrlf & "             <input type=""button"" name=""Submitdel2"" value=""添加二维码"" onClick=""GetSelectid()"" class=""anybutton2""/>" & vbcrlf & "            "
	end if
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	if open_44_7=1 then
		Response.write "" & vbcrlf & "             <input type=""button"" name=""Submitdel21"" value=""批量打印"" onClick=""PrintAll(0)"" class=""anybutton2""/>" & vbcrlf & "               <input type=""button"" name=""Submitdel2"" value=""设置打印模板"" onClick=""window.location.href='../Manufacture/PrinterList.asp?sort=2002'"" class=""anybutton2""/>" & vbcrlf & "           "
	end if
	Response.write "" & vbcrlf & "             </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "</form>" & vbcrlf & "<div align=""center"">" & vbcrlf & "<center >" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-top:-1px""> " &vbcrlf & "        <tr  valign=""center"" class=""top"">" & vbcrlf & "           <td width=""8%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>选择<input type=""checkbox"" onclick=""BoxCheck(this)"" style=""cursor: auto;"" name=""Del_Batch"" id=""Del_Batch"" value=""1""/></strong></div></td>" & vbcrlf & "             <td width=""14%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>用户级别</strong></div></td>" & vbcrlf & "             <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>职位名称</strong></div></td>" & vbcrlf & "         <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>姓名</strong></div></td>" & vbcrlf & "           <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>办公电话</strong></div></td>" & vbcrlf & "               <td width=""13%"" align=""left"" valign=""middle""  class=""name""><strong><div align=""center""><strong>家庭电话</strong></div></td>" & vbcrlf & "            <td width=""13%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>手机</strong></div></td>" & vbcrlf & "         <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><strong>电子邮件</strong></div></td>" & vbcrlf & "  </tr>" & vbcrlf & ""
	If Request("CurrPage")&"" = "" Then
		CurrPage = 0
	else
		If isnumeric(Request("CurrPage")) Then CurrPage=CLng(Request("CurrPage")) Else CurrPage = 0
	end if
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select ord,cateid, title, name, phone1, phone2, mobile, email from gate "& Str_Result &" order by sorce asc,sorce2 asc,cateid asc"
	...
	if rs7.RecordCount<=0 then
		...
		do until rs7.eof
			mobile = rs7("mobile") : email = rs7("email")
			Response.write " " & vbcrlf & "    <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "           <td height=""25"" align=""center"" valign=""middle""  class=""name""><input class=""Del_Item_List"" type=""checkbox"" name=""selectid"" value="""
			Response.write rs7("ord")
			Response.write """ /></td>" & vbcrlf & "         <td height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & ""
			Select Case rs7("cateid")
			Case 1 : Response.write "<span class='red'>总经理级</span>"
			Case 2 : Response.write "<span class='home style3'>经理级</span>"
			Case 3 : Response.write "<span class='blue2'>主管级</span>"
			Case 4 : Response.write "代表级"
			End Select
			Response.write "" & vbcrlf & "             </td>" & vbcrlf & "           <td align=""left"" valign=""middle""  class=""name"">"
			Response.write rs7("title")
			Response.write "</td>" & vbcrlf & "                <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("name")
			Response.write "</div></td>" & vbcrlf & "          <td align=""left"" valign=""middle""  class=""name""><div align=""center""><span class=""gray"">"
			Response.write rs7("phone1")
			Response.write "</span></div></td>" & vbcrlf & "           <td align=""left"" valign=""middle""  class=""name""><div align=""center""><span class=""gray"">"
			Response.write rs7("phone2")
			Response.write "</span></div></td>" & vbcrlf & "           <td height=""25"" align=""left"" valign=""middle""  class=""func"">" & vbcrlf & "                     <div align=""center""><span class=""gray"">"
			Response.write rs7("mobile")
			Response.write "</span>" & vbcrlf & ""
			if open_67_13=1  and open_67_19<>1  and isMobile(trim(rs7("mobile"))) Then
				Response.write "" & vbcrlf & "             <img src=""../images/message.gif"" onClick=""javascript:window.open('../message/topadd.asp?phone="
				Response.write trim(rs7("mobile"))
			...
			Response.write "" & vbcrlf & "                     </div>" & vbcrlf & "          </td>" & vbcrlf & "           <td height=""25"" align=""left"" valign=""middle""  class=""func"">" & vbcrlf & "                     <div align=""center""><span class=""gray"">"
			Response.write email
			Response.write "</span>" & vbcrlf & ""
			if open_77_13=1 and open_77_19<>1 and isemail(email) Then
				Response.write "" & vbcrlf & "                     <img src=""../images/email.gif"" onClick=""javascript:window.open('../email/index.asp?email="
				Response.write trim(email)
		...
		Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "       </tr>" & vbcrlf & " </table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "</table>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " " & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "   var a = document.getElementsByTagName(""input"");" & vbcrlf & "   if(a[0].checked==true){ "& vbcrlf & "   for (var i=0; i<a.length; i++) "& vbcrlf & "      if (a[i].type == ""checkbox"") a[i].checked = false; "& vbcrlf &"    } "& vbcrlf &"    else "& vbcrlf &"    { "& vbcrlf &  "  for (var i=0; i<a.length; i++)" & vbcrlf &   "    if (a[i].type== ""checkbox"") a[i].checked = true;" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	
' End --- Proc_54_12_16661970
