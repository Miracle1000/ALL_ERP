...
'bigsystemtype = ".mozi"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			Response.Flush
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select qx_open from power  where ord="&session("personzbintel2007")&" and sort1=23 and sort2=2"
			rs1.open sql1,conn,1,1
			if rs1.eof then
				open_23_2=0
			else
			...
			set rs1=nothing
			dim Str_Result,a
			Str_Result="where ck in (select ord from sortck where del=1 and (intro like '"&session("personzbintel2007")&",%' or intro like '%,"&session("personzbintel2007")&",%' or intro like '%, "&session("personzbintel2007")&",%' or intro like '%, "&session("personzbintel2007")&"'  or intro like '%,"&session(personzbintel2007)&"' or intro like '&session(personzbintel2007)&' or intro like '0')) and num2<>0"
			sort=request("sort")
			sort1=request("sort1")
			B=request("B")
			C=request("C")
			if sort<>"" and sort<>"0" then
				Str_Result=Str_Result+"and  ck in (select ord from sortck where del=1 and sort="&sort&")"
'if sort<>"" and sort<>"0" then
			end if
			if sort1<>"" and sort1<>"0" then
				Str_Result=Str_Result+"and  ck="&sort1&""
'if sort1<>"" and sort1<>"0" then
			end if
			if C<>"" then
				if B="cpmc" then
					Str_Result=Str_Result+"and  ord in (select ord from product where title like '%"&C&"%')"
'if B="cpmc" then
				elseif B="cpbh" then
					Str_Result=Str_Result+"and  ord in (select ord from product where order1 like '%"&C&"%')"
'elseif B="cpbh" then
				elseif B="cpxh" then
					Str_Result=Str_Result+"and  ord in (select ord from product where type1 like '%"&C&"%')"
'elseif B="cpxh" then
				elseif B="zgys" then
					Str_Result=Str_Result+"and ord in (select ord from product where company in (select ord from tel where name like '%"&C&"%')) "
'elseif B="zgys" then
				end if
			end if
			if sort<>"" then
			...
			if px=1 then
				px_Result="order by e.title desc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=2 then
				px_Result="order by e.title asc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=3 then
				px_Result="order by e.order1 desc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=4 then
				px_Result="order by e.order1 asc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=5 then
				px_Result="order by e.type1 desc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=6 then
				px_Result="order by e.type1 asc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=7 then
				px_Result="order by d.num2 desc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=8 then
				px_Result="order by d.num2 asc,d.ord desc,d.unit desc,d.ck desc"
			elseif px=9 then
				px_Result="order by d.datesc desc,d.ord desc,d.unit desc"
			elseif px=10 then
				px_Result="order by d.datesc asc,d.ord desc,d.unit desc"
			elseif px=11 then
				px_Result="order by d.dateyx desc,d.ord desc,d.unit desc"
			elseif px=12 then
				px_Result="order by d.dateyx asc,d.ord desc,d.unit desc"
			elseif px=13 then
				px_Result="order by d.ph desc,d.ord desc,d.unit desc"
			elseif px=14 then
				px_Result="order by d.ph asc,d.ord desc,d.unit desc"
			elseif px=15 then
				px_Result="order by d.gys desc,d.ord desc,d.unit desc"
			elseif px=16 then
				px_Result="order by d.gys asc,d.ord desc,d.unit desc"
			end if
			set rs=server.CreateObject("adodb.recordset")
			sql="select d.*,e.title,e.order1,e.type1,e.company from (select ord,unit,ck,num1,num2,money1,ph,xlh,datesc,dateyx,bz,gys from ku  "&Str_Result&" ) d inner join product e on e.ord=d.ord "&px_Result&""
			...
			do until rs.eof
				...
				if num1<=0 then
					...
					money1=Formatnumber(money1*num2/num1,num_dot_xs,-1)
					money1=0
				end if
				ph=rs("ph")
				xlh=rs("xlh")
				...
				set rs7=nothing
				xlWorksheet.Cells(1+i,4).Value =ckname
				set rs7=nothing
				...
				set rs7=nothing
				xlWorksheet.Cells(1+i,6).Value = num2
				set rs7=nothing
				xlWorksheet.Cells(1+i,6).font.Size=10
				set rs7=nothing
				xlWorksheet.Cells(1+i,7).Value = price1
				set rs7=nothing
				xlWorksheet.Cells(1+i,7).font.Size=10
				set rs7=nothing
				If open_23_2=1 then
					xlWorksheet.Cells(1+i,8).Value =  money1
'If open_23_2=1 then
				else
					xlWorksheet.Cells(1+i,8).Value =  ""
'If open_23_2=1 then
				end if
				xlWorksheet.Cells(1+i,8).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,9).Value = ph
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,9).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,10).Value = xlh
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,10).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,11).Value = datesc
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,11).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,12).Value = dateyx
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,12).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,13).Value = bzname
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,13).font.Size=10
'If open_23_2=1 then
				xlWorksheet.Cells(1+i,14).Value = gysname
'If open_23_2=1 then
...
'If open_23_2=1 then
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>库存明细导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!库存明细导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("库存明细_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的库存明细</u></strong></font></a></p>" & vbcrlf & ""
