...
'If abs(Err.number) > 0 Then
	...
	relFileName=Server.MapPath("HtmlWord\" & title & "_" & session("name2006chen") & ".doc")
	...
	docObj.open()
		...
		If imgs.length > 0 Then
			...
			For i = 0 To imgs.length-1
				...
				'img.src = xDoc.images.add(img.src)
				src = img.src
				src = Replace(src,"about:blank","***",1,-1,1)
				'src = img.src
				src = Replace(src,"about:","***",1,-1,1)
				'src = img.src
				src = Replace(src,"***","",1,-1,1)
				'src = img.src
				html1 = "<v:shape type=""#_x0000_t75"" style='width:" & w & ";height:" & h & ";visibility:visible;mso-wrap-style:square'>"
				'src = img.src
				html1 = html1 & "<v:imagedata src='" & src & "'/>"
				html1 = html1 & "</v:shape>"
