...
'sqltext = sqltext & " select " & ProductAttrValue & " as Num1 ,'" & attrs & AttrID &"' as Attrs "
	...
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=28 and sort2=12"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_28_12=0
	else
		open_28_12=rs1("qx_open")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	set rs1=nothing
	if open_21_21<>0 then
		strDisplay21_21=""
	else
		strDisplay21_21="display:none;"
	end if
	if open_21_22<>0 then
		strDisplay21_22=""
	else
		strDisplay21_22="display:none;"
	end if
	sql="Delete  product where del=7 and  addcate="&session("personzbintel2007")&" "
	conn.Execute(sql)
	Session("unitallList")=""
	unitallList = Session("unitallList")
	if unitallList<>"" then
	else
		unitallList=0
	end if
	khid= ""
	session("companyzbintelgys") = ""
	set rs=server.CreateObject("adodb.recordset")
	...
	rs.close
	set rs=nothing
	phXlhManageShow = ZBRuntime.MC(34000) and ZBRuntime.MC(17002) and ZBRuntime.MC(213102)
	productmenu=session("productmenu")
	user_list = "0"
	allcanname="全部"
	if productmenu<>"" then
		set rsm=server.CreateObject("adodb.recordset")
		sqlm="select isnull(user_list1,'') from menu where id="&productmenu&" "
		...
		rsm.close
		Set rsm = Nothing
		if isnull(user_list) Then
			allcansee=1
		elseif replace(replace(user_list,",",""),"0","")="" then
			allcansee=1
		else
			allcansee=0
		end if
		if allcansee=1 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select (stuff((select ' ' + name from gate where CHARINDEX(',' + CAST(ord as varchar(12)) + ',', ',"&user_list&",' )>0 for xml path('')),1,1,'')) name"
'set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			if not rs.eof then
				allcanname=rs("name")
	...
	Response.write "" & vbcrlf & "<html>" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Language"" content=""zh-cn"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>"
	'allcanname=rs("name")
	Response.write title_xtjm
	Response.write "</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "#suggest font{cursor:pointer}" & vbcrlf & "#hasbor tr.top td{border:1px solid #ccc!important}" & vbcrlf & "input#showText{padding-right:18px;box-sizing:border-box;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}" & vbcrlf & ".autocompleteico{z-index:10;height:15px;width:18px;border:0;cursor:pointer;margin-left:-20px;margin-top:7px;float:left;background:#FFFFFF url(../../SYSN/skin/default/img/auto_search.png) no-repeat center center;}" & vbcrlf & ".bill_help_expaln_text{background-color:#b2dbfd;width:auto;position:absolute;margin-left:20px;margin-top:-2px;padding:4px;font-weight:normal !important;}" & vbcrlf & ".bill_help_expaln_text3{background-color:#b2dbfd;width:400px;position:absolute;margin-left:20px;margin-top:-2px;padding:4px;font-weight:normal !important;}" & vbcrlf & ".bill_help_expaln_top{height:17px;padding:4px 3px 0 0;float:right;}" & vbcrlf & ".bill_help_expaln_close{display: block;float: right;margin-right: 3px;width: 16px;height:16px;cursor: pointer;font-size: 14px;background-position: -48px 32px;text-decoration: none;position:absolute;top:6px;right:2px;}" & vbcrlf & "" & vbcrlf & ".bill_help_expaln_text1{width:600px;position:absolute;left:245px;padding:15px;line-height:29px;z-index: 1}" & vbcrlf & "    #TBSr_tb1_1 td, #TBSr_tb1_2 td,      #TBSr_tb1_1 tr.tabtr, #TBSr_tb1_2 tr.tabtr  {" & vbcrlf & "        background-color:transparent;" & vbcrlf & "        background-image:none;" & vbcrlf & "    border-width:0px}" & vbcrlf & "</style>" & vbcrlf & "<script type=""text/javascript"" src=""suggest.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/javascript"" src=""../inc/comm.pym.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"" src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src= ""../Script/pt_add.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "<script src= ""../Script/pt_unitHandle.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & ""
	if ZBRuntime.MC(18000) then
		Response.write "" & vbcrlf & "<script src= ""../Script/pt_add_1.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFiles.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../Scripts/CBillExtSNCoder.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src= ""../Script/s3_set_khcl.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "<!--<script type=""text/JavaScript"" src=""../skin/default/js/comm.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """></script>-->" & vbcrlf & "<script>" & vbcrlf & "    function $ID(id){return document.getElementById(id);}" & vbcrlf & "    if(!window.app) { window.app = {};}" & vbcrlf & "    app.swpCss=function(obj){obj.className=obj.className.indexOf(""_over"")>0?obj.className.replace(""_over"",""""):obj.className+""_over"";}" & vbcrlf & "    app.stopDomEvent = function (e) { e = e || window.event; e.stopPropagation ? e.stopPropagation() : window.event.cancelBubble = true; return false; }" & vbcrlf & "  function beforeCheckSubmit(){" & vbcrlf & "           var cpyxqNum = jQuery(""#cpyxqNum"").val();" & vbcrlf & "              if(cpyxqNum!=""""){" & vbcrlf & "                 jQuery(""#cpyxqNum"").attr(""min"",""1"");" & vbcrlf & "          }else{" & vbcrlf & "                  jQuery(""#cpyxqNum"").removeAttr(""min"");" & vbcrlf & "              }" & vbcrlf & "               return true;" & vbcrlf & "    }" & vbcrlf & "       window.BillExtSN.Reg(16001, ""order1"");" & vbcrlf & "   var  cfun  =  window.__ChangeMenuArea;" & vbcrlf & "    window.__ChangeMenuArea = function(){" & vbcrlf & "        if(cfun) { cfun(); }" & vbcrlf & "        var doc =document.getElementById(""menuclsframe"").contentWindow.document;" & vbcrlf & "        var alinks =doc.getElementsByTagName(""a"");" & vbcrlf & "        var sort1value = 0;" & vbcrlf & "        if(alinks.length>0) {" & vbcrlf & "            sort1value = alinks[0].outerHTML.split(""productmenu="")[1].split(""'"")[0];" & vbcrlf & "        } else {" & vbcrlf & "            sort1value = 0;" & vbcrlf & "        }" & vbcrlf & "        document.getElementById(""sort1"").value=(sort1value);" & vbcrlf & "        window.BillExtSN.Refresh();" & vbcrlf & "    }" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "      <table border=""0"" width=""100%"" cellspacing=""0"" cellpadding=""0"" bgcolor=""#FFFFFF"" >" & vbcrlf & "              <tr>" & vbcrlf & "                    <td valign=""top"">" & vbcrlf & "                         <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td class=""place"">产品添加</td>" & vbcrlf & "                                           <td>&nbsp;</td>" & vbcrlf & "                                         <td align=""right"">&nbsp;</td>" & vbcrlf & "                                          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                                      </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                          <input type=""hidden"" name=""flag1"" id=""flag1"" value=""0"">" & vbcrlf & "                         <input type=""hidden"" name=""flag2"" id=""flag2"" value=""0"">" & vbcrlf & "                          <input type=""hidden"" name=""flag3"" id=""flag3"" value=""0"">" & vbcrlf & "                               <form method=""POST"" action=""save.asp?top="
	Response.write deurl(request("top"))
	Response.write "&fromautocomplete="
	Response.write request.querystring("fromautocomplete")
	Response.write """ target=""dosave""  onsubmit=""return beforeCheckSubmit() && Validator.Validate(this,2) && CheckPageFieldValue()""  name=""date"" id=""productForm"" style=""margin:0"">" & vbcrlf & "                             <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">"& vbcrlf &                             "<col width=""12%""/>" & vbcrlf &                                 "<col width=""21%""/>" & vbcrlf &                                 "<col width=""12%""/>" & vbcrlf &                                 "<col width=""21%""/>" & vbcrlf &                                 "<col width=""12%""/>" & vbcrlf &                                 "<col width=""21%""/>" & vbcrlf &                                         "<tr class=""top accordion"">" & vbcrlf &                                                 "<td colspan=""6"" class=""accordion-bar-bg"" style=""padding-top:0px"">" & vbcrlf & "                                                      <div  class=""accordion-bar-tit"" style=""padding-top:4px;"">" & vbcrlf & "                                                           基本信息<span class=""accordion-arrow-down""></span>" & vbcrlf & "                                                        </div>" & vbcrlf & "                                                  <div class=""accordion-bar-btns"">"& vbcrlf &                                                             "<input type=""submit"" name=""Submit422222"" value=""保存""  class=""page""/>" & vbcrlf &                                                            "<input type=""submit"" name=""Submit424222"" value=""增加"" onClick=""ask2();""  class=""page""/>" & vbcrlf &                                                            "<input type=""reset"" value=""重填""  class=""page"" name=""B2222"">&nbsp;</div>" & vbcrlf &                                              "</td>" & vbcrlf &                                    "</tr>" & vbcrlf &                                    "<tr>" & vbcrlf &                                             "<td><div align=""right"">产品名称：</div></td>" & vbcrlf &                                               "<td colspan=""3"">" & vbcrlf &                                                   "<TEXTAREA  name=""title"" style=""float:left;OVERFLOW-Y: hidden; WIDTH: 150px; HEIGHT: 22px;box-sizing:border-box; WORD-BREAK: break-all"" oninput=""textareaHandle(event)"" "
	Response.write request.querystring("fromautocomplete")
	if CheckPurview(ts_kh,"1")=True then
		Response.write " onChange=""callServer_ts(1,'keyword');"" "
	end if
	Response.write " dataType=""Limit""  "
	if CheckPurview(bt_kh,"1")=True then
		Response.write " min=""1"" "
	end if
	Response.write " maxlength=""500"" msg=""不允许为空"" id=""keyword"" onKeyUp=""keyupdeal(event);ff();keyUpChangeSize(this,event)"" onClick=""keyupdeal(event);ff();"" autocomplete=""off"">"
	Response.write title
	Response.write "</TEXTAREA>" & vbcrlf & "                                                  "
	if CheckPurview(bt_kh,"1")=True then
		Response.write "&nbsp;<span class=""red"">*</span>"
	end if
	Response.write "&nbsp;" & vbcrlf & "" & vbcrlf & "                                                       拼音码：<input id=pym type='text' size=15 name=pym max=""100"" msg=""拼音码长度必须在1至100个字符之间"" dataType=""Limit"" value="""
	Response.write pym
	Response.write """><span id=""test1"" class=""red""></span>" & vbcrlf & "                                                        <div ID=""floater1"" style=""position: absolute; left: 113px; top: 121px; width: 293px;"">" & vbcrlf & "                                                              <div id=""suggest""></div>" & vbcrlf & "                                                  </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td><div align=""right"">产品编号：</div></td>" & vbcrlf & "                                                <td>" & vbcrlf & "                                                    <div align=""left"">" & vbcrlf & "                                                        <input type=""text"" size=""20"" name=""order1"" Id=""order1"" "
	if CheckPurview(ts_kh,"2")=True then
		Response.write " onChange=""callServer_ts(2,'order1');"" "
	end if
	Response.write " dataType=""Limit"" "
	if CheckPurview(bt_kh,"2")=True then
		Response.write " min=""1"" "
	end if
	Response.write " maxlength=""500"" msg=""不允许为空"" value="""
	Response.write khid
	Response.write """>" & vbcrlf & "                                                        "
	if CheckPurview(bt_kh,"2")=True then
		Response.write "<span class=""red"">*</span>"
	end if
	Response.write "" & vbcrlf & "                                                     <span id=""test2"" class=""red""></span>" & vbcrlf & "                                                        </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td  align=""center""><div align=""right"">产品分类：</div></td>" & vbcrlf & "                                                <td ><input  name=""sort1"" id=""sort1"" type=""hidden"">" & vbcrlf & "                                                        <IFRAME id='menuclsframe' name=""I2"" SRC=""../event/product2.asp?ID=company"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""95%"" HEIGHT=""24"" style=""padding-top:3px;box-sizing:border-box;"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "                                               </td>" & vbcrlf & "                        <td ><div align=""right"">调用范围：</div></td>" & vbcrlf & "                                            <td>" & vbcrlf & "                            <input type=""hidden""  name=""allcansee"" id=""allcansee"" value="""
	Response.write allcansee
	Response.write """/>" & vbcrlf & "                            <input type=""hidden""  name=""member2"" value="""
	Response.write user_list
	Response.write """ id=""member2"" />" & vbcrlf & "                                                   <input type=""text""  size=""20"" readonly dataType=""Limit"" min=""1"" msg=""请选择调用范围"" value="""
	Response.write allcanname
	Response.write """ id=""showText"" style=""float:left;"">" & vbcrlf & "                            <div title=""点击打开选择页面"" onclick=""openurl()"" class=""autocompleteico ""></div>" & vbcrlf & "                            <span class=""red"" style=""margin-left:6px;"">*</span>" & vbcrlf & "                                                </td>" & vbcrlf& "                                               <td ><div align=""right"">产品型号：</div></td>" & vbcrlf & "                                             <td  >" & vbcrlf & "                                                  <textarea name=""type1"" id=""type1"" style=""float:left;OVERFLOW-Y: hidden; WIDTH: 150px; HEIGHT: 22px;box-sizing:border-box; WORD-BREAK: break-all"" oninput=""textareaHandle(event)"" onKeyUp=""keyupdeal(event);ff()"" onClick=""keyupdeal(event);ff();""" & vbcrlf & "                                "
	if CheckPurview(ts_kh,"3")=True then
		Response.write " onChange=""callServer_ts(3,'type1');"""
	end if
	Response.write " dataType=""Limit"" " & vbcrlf & "                                "
	if CheckPurview(bt_kh,"3")=True then
		Response.write " min=""1"" "
	end if
	Response.write " msg=""不允许为空"" maxlength=""500"">"
	Response.write type1
	Response.write "</textarea>" & vbcrlf & "                                                  "
	if CheckPurview(bt_kh,"3")=True then
		Response.write "<span class=""red"">*</span>"
	end if
	Response.write "<span id=""test3"" class=""red""></span>" & vbcrlf & "                                             </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "           " & vbcrlf & "                <!--    扩展自定义-->"
	Response.write "<span class=""red"">*</span>"
	If isUsingExtend(21) Then
		Response.write ShowExtendedByKZZDY(16001,0,3,1,1,false , bt_kh , ts_kh , 21 , false , 0)
		Response.write ShowExtendedByProductGroup(16001,0,3,1,1,false , bt_kh , ts_kh , 21 , false , productzdysort)
	end if
	If ZBRuntime.MC(34000)  or ZBRuntime.MC(18000) or ZBRuntime.MC(35000) Then
		Response.write "" & vbcrlf & "                                     <tr class=""top accordion"" style=""border-bottom:0px"">" & vbcrlf & "                                            <td colspan=""6"" class=""accordion-bar-bg"" style=""border-bottom:0px"">" & vbcrlf & "                                                  <style> " & vbcrlf & "                                    #tabtool { width:100%;table-layout:fixed;border-collapse:collapse;margin-top:8px}" & vbcrlf & "                                                            #tabtool tr.tabtool {" & vbcrlf & "                                                            background:none !important;" & vbcrlf & "                                       }" & vbcrlf & "                                                          #tabtool tr.tabtool td {" & vbcrlf & "                                                      border-top:0px;" & vbcrlf & "                                       border-left:0px;" & vbcrlf & "                                       border-right:0px;" & vbcrlf & "                                       height:30px;" & vbcrlf & "                                                                          padding-top:0px;" & vbcrlf & "                                                          }"& vbcrlf & "                                      #tabtool tr.tabtool td.tabitem {                                       " & vbcrlf & "                                       text-align:center;" & vbcrlf & "                                                             }" & vbcrlf & "                                     #tabtool tr.tabtool td.tabsel {" & vbcrlf & "                                        background-color:white;" & vbcrlf & "                                       border-bottom:0px solid #c0ccdc !important;" & vbcrlf & "                                                       }" & vbcrlf & "                                             </style>" & vbcrlf & "                            <script>" & vbcrlf & "                                function setTabitem(i){" & vbcrlf & "                                    var curTR = jQuery(""#tabtool"");" & vbcrlf & "                                    if(i==0){                                        " & vbcrlf & "                                      if(jQuery(curTR).find("".accordion-arrow-down"").hasClass(""accordion-arrow-up"")==true){" & vbcrlf & "                                            jQuery(curTR).find("".accordion-arrow-down"").click();" & vbcrlf & "                                        }" & vbcrlf & "                            $ID(""tabitem0"").className = ""tabitem tabsel""" & vbcrlf & "                                        $ID(""tabitem1"").className = ""tabitem""" & vbcrlf & "                                        $ID(""canOutStore"").value = 1" & vbcrlf & "                                        if( $ID(""cpjs"")){" & vbcrlf & "                                            $ID(""cpjs"").style.display = ""block"";   " & vbcrlf & "                                            $ID(""bill_help_expaln3"").style.display = ""block"";   " & vbcrlf & "                                        }  " & vbcrlf & "  $ID(""trcontent"").style.display = """";" & vbcrlf & "                                        $ID(""tablecontent"").style.display = """";" & vbcrlf & "                                        " & vbcrlf & "                                        var j=0;" & vbcrlf & "                                        var a=document.getElementsByName(""Roles"");" & vbcrlf & "                                        for(var i=0;i<a.length;i++){" & vbcrlf & "                                            if(a[i].checked==true)" & vbcrlf & "                                   {" & vbcrlf & "                                                    j++;" & vbcrlf & "                                                }" & vbcrlf & "                                        }      " & vbcrlf & "                                        if(j==0)" & vbcrlf & "                {" & vbcrlf & "                                            if( $ID(""sc"")){" & vbcrlf & "                                                $ID(""sc"").style.display = 'none';" & vbcrlf & "                                            }" & vbcrlf & "                              if( $ID(""sc2"")){" & vbcrlf & "                                                $ID(""sc2"").style.display = 'none';" & vbcrlf & "                                            }" & vbcrlf & "                                        }     " & vbcrlf & "                                    }"& vbcrlf &                                     "else{" & vbcrlf &                                         "if( $ID(""cpjs"")){" & vbcrlf &                                             "$ID(""cpjs"").style.display = ""none"";"  & vbcrlf &                                             "$ID(""bill_help_expaln3"").style.display = ""none"";     " & vbcrlf & "                                        }    " & vbcrlf & "                                        $ID(""tabitem0"").className = ""tabitem""" & vbcrlf & "                                        $ID(""tabitem1"").className = ""tabitem tabsel""   " & vbcrlf & "                                        $ID(""canOutStore"").value = 0        " & vbcrlf & "                                        $ID(""trcontent"").style.display = ""none"";" & vbcrlf & "                                        $ID(""tablecontent"").style.display = ""none"";       " & vbcrlf & "                                    }" & vbcrlf & "                                }" & vbcrlf & "" & vbcrlf & "                            </script>" & vbcrlf & "                            <table id=""tabtool"">" & vbcrlf & "                                <colgroup>" & vbcrlf &"                                    <col width=""120""/>" & vbcrlf & "                                    <col width=""8""/>" & vbcrlf & "                                    <col width=""70""/>" & vbcrlf & "                                    <col width=""70""/>" & vbcrlf & "                      <col width=""60""/>" & vbcrlf & "                                    <col width=""280""/>" & vbcrlf & "                                </colgroup>" & vbcrlf & "                                <tr class=""tabtool"">" & vbcrlf & "                                     <td><div  class=""accordion-bar-tit"">角色信息<span class=""accordion-arrow-down""></span></div></td>" & vbcrlf & "                                     <td></td>" & vbcrlf & "                                     <td class=""tabitem tabsel stclass"" id=""tabitem0"" onclick=""app.stopDomEvent(event)"" onmousedown=""setTabitem(0)"">实体</td>" & vbcrlf & "                                     <td class=""tabitem""  onclick=""app.stopDomEvent(event)"" id=""tabitem1"" onmousedown=""setTabitem(1)"">非实体</td>" & vbcrlf & "                                     <td class=""help_td"">" & vbcrlf & "                                    <span class=""help_explan_ico"" onclick=""showHelpExplan(1)"" ></span>" & vbcrlf & "                                          <div id=""bill_help_expaln_text"" class=""bill_help_expaln_text"" style=""display:none;font-weight:100"">" & vbcrlf & "                                          注：非实体产品不允许出入库" & vbcrlf & "                                          <a title=""关闭"" href=""javascript:;""  onclick=""closediv(1)"" class=""bill_help_expaln_close resetElementHidden"">×</a>" & vbcrlf & "                                          <a title=""关闭"" href=""javascript:;""  onclick=""closediv(1)""class=""bill_help_expaln_close resetElementShow"" style=""display:none;""></a>                                          " & vbcrlf & "                                          </div>" & vbcrlf & "                                          <input type=""hidden""  name=""canOutStore"" id=""canOutStore"" value="" />" & vbcrlf &                                      "</td>" & vbcrlf &                                      "<td >" & vbcrlf &                               ""
		If ZBRuntime.MC(18000) or  ZBRuntime.MC(35000) Then
			Response.write "" & vbcrlf & "                                        <div id=""cpjs"" style=""margin-bottom:6px;"">" & vbcrlf & "                                        "
'If ZBRuntime.MC(18000) or  ZBRuntime.MC(35000) Then
			If ZBRuntime.MC(18600) Then
				Response.write "" & vbcrlf & "                                          <input type=""checkbox"" name=""Roles"" onclick=""checktype(1, this)"" id=""Role1""  value=""1"">" & vbcrlf & "                                          <label onclick=""app.stopDomEvent(event)"" for=""Role1"">自制件</label>" & vbcrlf & "   "
			end if
			If ZBRuntime.MC(18700) Then
				Response.write "" & vbcrlf & "                                                       <input type=""checkbox"" name=""Roles"" onclick=""checktype(2, this)"" id=""Role2""  value=""2"">" & vbcrlf & "                                          <label onclick=""app.stopDomEvent(event)"" for=""Role2"">委外件</label>" & vbcrlf & "               "
			end if
			Response.write "" & vbcrlf & "                                                       <input  type=""checkbox"" name=""Roles"" onclick=""checktype(3, this)"" id=""Role3""  value=""3"">" & vbcrlf & "                                          <label onclick=""app.stopDomEvent(event)"" for=""Role3"">外购件</label>" & vbcrlf & "                                                              <span class=""red"">*</span><span id=""test4"" class=""red""></span>" & vbcrlf & "                                        </div>" & vbcrlf & "                                        "
		end if
		Response.write "" & vbcrlf & "                                    </td>" & vbcrlf & "                                    <td class=""help_td"">" & vbcrlf & "                                        "
		If ZBRuntime.MC(18000) or  ZBRuntime.MC(35000) Then
			Response.write "" & vbcrlf & "                                          <span class=""help_explan_ico"" id=""bill_help_expaln3"" onclick=""showHelpExplan(3)"" ></span>" & vbcrlf & "                                          <div id=""bill_help_expaln_text3"" class=""bill_help_expaln_text3"" style=""display:none;font-weight:100"">" & vbcrlf & "                                            注：" & vbcrlf & "                                            1、自制件、委外件不支持移动加权平均法的计价方式<br />" & vbcrlf & "                                            2、当上次完成存货核算后且无新出入库单生成时，可以修改计价方式" & vbcrlf & "                             <a title=""关闭"" href=""javascript:;""  onclick=""closediv(3)"" class=""bill_help_expaln_close resetElementHidden"">×</a>" & vbcrlf & "                                          <a title=""关闭"" href=""javascript:;""  onclick=""closediv(3)"" class=""bill_help_expaln_close resetElementShow"" style=""display:none;""></a>" & vbcrlf &                                           "</div>" & vbcrlf &     ""
		end if
		Response.write "" & vbcrlf & "                                     </td>" & vbcrlf & "                                    <td>&nbsp;</td>" & vbcrlf & "                                </tr>" & vbcrlf & "                            </table>" & vbcrlf & "                         " & vbcrlf & "                   " & vbcrlf & "                                          " & vbcrlf & "                                         </td>                      " & vbcrlf & "                                     </tr>" & vbcrlf & "" & vbcrlf & "        "
	end if
	Response.write "" & vbcrlf & "                             <tr id=""trcontent"">" & vbcrlf & "                        <td colspan=""6"" style=""border-top:0px"">" & vbcrlf & "                            <table class=""reseetTextColor"" id=""tablecontent"" style=""width:100%; table-layout:fixed; border:0px;color: #5B7CAE;margin-top:1px;border-collapse:collapse""  cellspacing=""0"" border=""0"">" & vbcrlf & "                                <colgroup>" & vbcrlf & "                                    <col width=""5%""/>" & vbcrlf & "                                    <col width=""11%""/>" & vbcrlf & "                       <col width=""22%""/>" & vbcrlf & "                                    <col width=""11%""/>" & vbcrlf & "                                    <col width=""22%""/>" & vbcrlf & "                                    <col width=""11%""/>" & vbcrlf & "                                    <col width=""22%""/>" & vbcrlf & "                                </colgroup" & vbcrlf & "            "
	If ZBRuntime.MC(7000) and ZBRuntime.MC(17003) Then
		Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                    <td style=""width:5%""><div align=""center"">销售</div></td>" & vbcrlf & "                    <td><div align=""right"">销售出库超量上限：</div>" & vbcrlf & "                                        </td>" & vbcrlf & "                    <td colspan =""5""><input name=""KuoutLimitExcess"" id=""KuoutLimitExcess"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('KuoutLimitExcess','2')"" onpropertychange=formatData(this,""number"")  dataType=""Range"" value=""0.00"" min=""0.00"" max=""9999.99""  msg=""超出范围(0.00-9999.99)。""> %" & vbcrlf & "</td>" & vbcrlf & "                </tr>" & vbcrlf & "            "
	end if
	If ZBRuntime.MC(34000) Then
		If ZBRuntime.MC(1002) or  ZBRuntime.MC(15000) or  ZBRuntime.MC(215102)  or  (ZBRuntime.MC(17002) and ZBRuntime.MC(15000))Then
			...
			If ZBRuntime.MC(1002) Then
				Response.write "" & vbcrlf & "                        <td><div align=""right"">主供应商：</div></td>" & vbcrlf & "                        <td><IFRAME name=""I5"" SRC=""../event/company_gys.asp?ID=company"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""200"" HEIGHT=""27"" marginwidth=""1"" marginheight=""1"" style=""padding-top:4px;box-sizing:border-box;""></IFRAME>" & vbcrlf & "                                 </td>   " & vbcrlf & "                         "
'If ZBRuntime.MC(1002) Then
			else
				Response.write "" & vbcrlf & "                        <td></td><td></td>   " & vbcrlf & "                        "
			end if
			If ZBRuntime.MC(15000) Then
				Response.write "" & vbcrlf & "                        <td><div align=""right"">采购提前期：</div></td>" & vbcrlf & "                        <td><input name=""PurchaleadDays"" id=""PurchaleadDays"" title=""采购提前期指的是外购件角色的产品从签订采购合同到实际入库的时间。在执行计划时，系统会根据产品的建议到货日期、提前期， 推算出此产品应当进行采购的日期。"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('PurchaleadDays','0')"" onpropertychange=formatData(this,""int"") value=""0"" maxlength=""4"" > 天" & vbcrlf & "                        </td>" & vbcrlf & "                        "
			else
				Response.write "" & vbcrlf & "                        <td></td><td></td>   " & vbcrlf & "                        "
			end if
			If ZBRuntime.MC(215102) Then
				Response.write "" & vbcrlf & "                                             <td height=""35"" align=""center""><div align=""right"">质检方案：</div></td>" & vbcrlf & "                                                       <td>" & vbcrlf & "                                    <select name=""QC_id"" id=""QC_id"">" & vbcrlf & "                                                        <option value=""0"">      </option>" & vbcrlf & "                     "
				set rs=server.CreateObject("adodb.recordset")
				sql="select ord,sort1 from sortonehy where del=1 and gate2=3001 and isNull(isStop,0)=0 order by gate1 desc"
				rs.open sql,conn,1,1
				do until rs.eof
					Response.write "" & vbcrlf & "                                        <option value="""
			...
			Response.write "" & vbcrlf & "                    </tr>          " & vbcrlf & "                              <tr>"
			If ZBRuntime.MC(15000) Then
				Response.write "" & vbcrlf & "                                 <td><div align=""right"">预计销售周期：</div></td> " & vbcrlf & "                                         <td><input name=""period"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');"" onpropertychange=formatData(this,""number2"")  maxlength=""4""  value="""
				Response.write FormatNumber(3.5,num1_dot,-1,0,0)
				Response.write """> 月" & vbcrlf & "                                         </td>" & vbcrlf & "                        "
			else
				Response.write "" & vbcrlf & "                        <td></td><td></td>   " & vbcrlf & "                        "
			end if
			If ZBRuntime.MC(15000) Then
				Response.write "" & vbcrlf & "                        <td><div align=""right"">最小采购量：</div>" & vbcrlf & "                                          </td>" & vbcrlf & "                        <td><input name=""LimitBuyNum"" id=""LimitBuyNum"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('LimitBuyNum','2')"" onpropertychange=formatData(this,""number"")  value=""0.00"">" & vbcrlf & "                                            </td>" & vbcrlf & "                        "
			else
				Response.write "" & vbcrlf & "                        <td></td><td></td>" & vbcrlf & "                        "
			end if
			If ZBRuntime.MC(15000) and  ZBRuntime.MC(17002) Then
				Response.write "" & vbcrlf & "                        <td><div align=""right"">采购入库超量上限：</div>" & vbcrlf & "                                            </td>" & vbcrlf & "                        <td><input name=""LimitExcess"" id=""LimitExcess"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('LimitExcess','2')""onpropertychange=formatData(this,""number"")  dataType=""Range"" value=""0.00"" min=""0.00"" max=""9999.99""  msg=""超出范围(0.00-9999.99)。""> %" & vbcrlf &                                             "</td>" & vbcrlf &      ""
'If ZBRuntime.MC(15000) and  ZBRuntime.MC(17002) Then
			else
				Response.write "" & vbcrlf & "                        <td></td><td></td>   " & vbcrlf & "                        "
			end if
		...
		If ZBRuntime.MC(17000) or ZBRuntime.MC(17009) or ZBRuntime.MC(17001) or ZBRuntime.MC(17002)  or ZBRuntime.MC(17002) Then
			If ZBRuntime.MC(17000) Then
				Response.write "" & vbcrlf & "                                     <tr>" & vbcrlf & "                            <td rowspan=""3""><div align=""center"">库存</div></td>" & vbcrlf & "                          <td style=""width:50%""><div align=""right"">库存下限：</div></td>" & vbcrlf & "                                                 <td width='21%'><input name=""aleat1"" id=""aleat1""type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('aleat1','"
				Response.write num1_dot
				Response.write "')""   onpropertychange=formatData(this,""number"") dataType=""Limit"" value="""
				Response.write FormatNumber(0,num1_dot,-1,0,0)
				Response.write "')""   onpropertychange=formatData(this,""number"") dataType=""Limit"" value="""
				Response.write """ min=""0"" max=""10""  msg=""长度必须在10位之内""  value="""
				Response.write aleat1
				Response.write """></td>                               " & vbcrlf & "                                                <td width='12%'><div align=""right"">安全库存：</div></td>" & vbcrlf & "                                                  <td width='21%'><input name=""SafeNum"" id=""SafeNum"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('SafeNum','"
				Response.write num1_dot
				Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
				Response.write FormatNumber(0,num1_dot,-1,0,0)
				Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
				Response.write """ min=""0"" max=""10""  msg=""长度必须在10位之内""></td>      " & vbcrlf & "                            <td width='12%'><div align=""right"">库存上限：</div></td>" & vbcrlf & "                                                  <td width='21%'><input name=""aleat2"" id=""aleat2"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('aleat2','"
				Response.write num1_dot
				Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
				Response.write FormatNumber(0,num1_dot,-1,0,0)
				Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
				Response.write """ min=""0"" max=""10""  msg=""长度必须在10位之内""  value="""
				Response.write aleat2
				Response.write """></td>   " & vbcrlf & "                                        </tr>" & vbcrlf & "                    "
			else
				Response.write "" & vbcrlf & "                    <tr>" & vbcrlf & "                        <td rowspan=""3""></td>" & vbcrlf & "                      <td style=""width:50%""></td>" & vbcrlf & "                                           <td width='21%'></td>                               " & vbcrlf & "                                            <td width='12%'></td>" & vbcrlf & "                                           <td width='21%'></td>"& vbcrlf &                                             "<td width"
			end if
			Response.write "     " & vbcrlf & "" & vbcrlf & "" & vbcrlf & "                                        <tr>" & vbcrlf & "                    "
			If ZBRuntime.MC(17000) Then
				Response.write "" & vbcrlf & "                        <td align=""center""><div align=""right"">产品有效期：</div></td>" & vbcrlf & "                                            <td>" & vbcrlf & "                                                    <input name=""cpyxqNum"" id=""cpyxqNum"" type=""text"" size=""15"" onKeyUp=""checkDot('cpyxqNum','1')"" onpropertychange=formatData(this,""int"")  maxlength=""4"" dataType=""Range"" msg='不能小于1'>" & vbcrlf & "                                                              <select name=""cpyxqUnit"" id=""cpyxqUnit"">" & vbcrlf & "                                                                    <option value=""2"" selected>天</option>" & vbcrlf & "                                                                    <option value=""3"">周</option>" & vbcrlf & "                                                                     <option value=""4"">月</option>" & vbcrlf & "                                                                     <option value=""5"">年</option>" & vbcrlf & "                                                          </select>" & vbcrlf & "                                               </td>" & vbcrlf & "                                           <td><div align=""right"">失效提前期：</div></td>" & vbcrlf & "                                            <td>" & vbcrlf & "                                                    <input name=""RemindNum"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');"" onpropertychange=formatData(this,""number2"")  maxlength=""4"">" & vbcrlf & "                                                     <select name=""RemindUnit"" id=""RemindUnit"">" & vbcrlf & "                                                          <option value=""1"">小时</option>" & vbcrlf & "                                                           <option value=""2"" selected>天</option>" & vbcrlf & "                                                            <option value=""3"">周</option>" & vbcrlf &"                                                         <option value=""4"">月</option>" & vbcrlf & "                                                             <option value=""5"">年</option>" & vbcrlf & "                                                     </select>" & vbcrlf & "                                               </td>" & vbcrlf & "                    "
			else
				Response.write "" & vbcrlf & "                        <td></td>" & vbcrlf & "                        <td></td>" & vbcrlf & "                        <td ></td>" & vbcrlf & "                        <td></td>" & vbcrlf & "                    "
			end if
			If ZBRuntime.MC(17009) Then
				Response.write "" & vbcrlf & "                        <td height=""35"" align=""center""><div align=""right"">养护周期：</div></td>" & vbcrlf & "                        <td>" & vbcrlf & "                                <input name=""MaintainNum"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');"" onpropertychange=formatData(this,""number2"")  maxlength=""4"">" & vbcrlf & "                                                      <select name=""MaintainUnit"" id=""MaintainUnit"">" & vbcrlf & "                                                              <option value=""1"">小时</option>" & vbcrlf & "                                                           <option value=""2"" selected>天</option>" & vbcrlf & "                                                            <option value=""3"">周</option>"& vbcrlf & "                                                              <option value=""4"">月</option>" & vbcrlf & "                                                             <option value=""5"">年</option>" & vbcrlf & "                                                     </select>" & vbcrlf & "                        </td>" & vbcrlf & "                    "
			else
				Response.write "     " & vbcrlf & "                        <td height=""35"" align=""center""></td>" & vbcrlf & "                        <td></td>" & vbcrlf & "                    "
			end if
			Response.write "     " & vbcrlf & "                                        </tr>" & vbcrlf & "" & vbcrlf & "                               <tr>" & vbcrlf & "                    "
			If ZBRuntime.MC(17001) AND  ZBRuntime.MC(17003) Then
				Response.write "" & vbcrlf & "                                 <td style=""width:50%""><div align=""right"">计价方式：</div></td>" & vbcrlf & "                                          <td>" & vbcrlf & "                                                <select name=""priceMode"" id=""priceMode"">" & vbcrlf & "                                                    <option value=0 >先进先出法</option>" & vbcrlf & "                            <option value=3>移动加权平均法</option>" & vbcrlf & "                                               <option value=1 >个别计价法</option>" & vbcrlf & "                                                    <option value=2 >全月平均法</option>" & vbcrlf & "                                                    </select>" & vbcrlf & "                                           </td>" & vbcrlf & "                    "
			else
				Response.write "     " & vbcrlf & "                        <td style=""width:50%""></td>" & vbcrlf & "                        <td></td>" & vbcrlf & "                    "
			end if
			if phXlhManageShow then
				If ZBRuntime.MC(34000) AND  ZBRuntime.MC(17002) Then
					Response.write "                                              " & vbcrlf & "                            <td height=""35"" align=""center""><div align=""right"">批号必填：</div></td>" & vbcrlf & "                            <td>" & vbcrlf & "                              <label><input type=""radio"" name=""phManage""value=""1"" "
					Response.write iif(phManage&""="1","checked","")
					Response.write ">是</label>" & vbcrlf & "                              <label><input type=""radio"" name=""phManage"" value=""0"" "
					Response.write iif(phManage&""="" or phManage&""="0","checked","")
					Response.write " style=""margin-left:8px;"">否</label>" & vbcrlf & "                            </td>" & vbcrlf & "                            <td></td>      <td></td>" & vbcrlf & "                            "
					Response.write iif(phManage&""="" or phManage&""="0","checked","")
				else
					Response.write "" & vbcrlf & "                            <td height=""35"" align=""center""></td>" & vbcrlf & "                            <td>                        " & vbcrlf & "                            </td>" & vbcrlf & "                            <td colspan=""2""></td>" & vbcrlf & "                "
				end if
			else
				Response.write "" & vbcrlf & "                        <td height=""35"" align=""center""></td>" & vbcrlf & "                        <td>                        " & vbcrlf & "                        </td>" & vbcrlf & "                        <td colspan=""2""></td>" & vbcrlf & "                        "
			end if
	...
	If ZBRuntime.MC(18400) and  (ZBRuntime.MC(18000) or ZBRuntime.MC(35000))  Then
		Response.write "" & vbcrlf & "" & vbcrlf & "                                 <tr id=""sc"" style=""display:none"">" & vbcrlf & "                         <td style=""width:50%"" "
		if ZBRuntime.MC(18000) then
			Response.write " rowspan=""2"""
		end if
		Response.write "><div align=""center"">生产</div></td>" & vbcrlf & "                     "
		If ZBRuntime.MC(18400)  Then
			Response.write "" & vbcrlf & "                         <td style=""width:50%""><div align=""right"">固定制造提前：</div></td>" & vbcrlf & "                                              <td><input name=""ProduceleadDays"" id=""ProduceleadDays"" title=""固定提前期指的是不论批量大小,都以一定时间为提前期；会根据自制件或者委外件的完工日期推算其直接子级产品的完工日期。"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('ProduceleadDays','0')"" onpropertychange=formatData(this,""int"")  maxlength=""4""> 天</td>" & vbcrlf & "                                                   " & vbcrlf & "                                                <td><div align=""right"">变动制造：</div></td>" & vbcrlf & "                                              <td>提前期 <input style=""width:50px;"" name=""extleadDays""id=""extleadDays"" title=""变动提前期指的是提前时间的长短随着每批加工量大小而变动，需要结合“变动提前期批量”计算自制件或者委外件产品的提前期。提前期 = 固定提前期+（需求数量/变动提前期批量）取整*变动提前。"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g批量 """ & vbcrlf & "" & vbcrlf & "                        <input  style=""width:50px;"" name=""extleadNum"" id=""extleadNum"" title=""变动提前期批量指的是和“变动提前期”结合使用计算自制件或者委外件产品提前期的一个参数，其单位是产品的基本单位。提前期 = 固定提前期+（需求数量/变动提前期批量）取整*变动提前。"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('extleadNum','"
			Response.write num1_dot
			Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
			Response.write FormatNumber(0,num1_dot,-1,0,0)
			Response.write "')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" value="""
			Response.write """ min=""0"" max=""10""  msg=""长度必须在10位之内""> " & vbcrlf & "" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td><div align=""right"">损耗率：</div></td>" & vbcrlf & "                                                <td colspan="""
			Response.write spanNum
			Response.write """><input name=""WastAge"" id=""WastAge"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('WastAge','"
			Response.write percentWithDot
			Response.write "')"" onpropertychange=formatData(this,""float"")  dataType=""Range"" value="""
			Response.write Formatnumber(CDbl(WastAge),percentWithDot,-1,0,0)
			Response.write "')"" onpropertychange=formatData(this,""float"")  dataType=""Range"" value="""
			Response.write """ min=""0.00"" max=""99.99""  msg=""超出范围(0.00-99.99)。""> %</td>" & vbcrlf & "" & vbcrlf & "                    "
			Response.write "')"" onpropertychange=formatData(this,""float"")  dataType=""Range"" value="""
		else
		...
		If ZBRuntime.MC(18000) Then
			Response.write "" & vbcrlf & "                                     <tr id=""sc2"" style=""display:none"">" & vbcrlf & "                        <td align=""center""><div align=""right"">最小生产量：</div></td>" & vbcrlf & "                                         <td><input name=""LimitProduceNum"" id=""LimitProduceNum"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'');checkDot('LimitProduceNum','2')"" onpropertychange=formatData(this,""number"")  value=""0.00""></td>" & vbcrlf & "                                            <td></td>" & vbcrlf & "                                               <td></td>" & vbcrlf & "                                               <td></td>" & vbcrlf & "                                               <td></td>" & vbcrlf & "                                       </tr>" & vbcrlf & "                    "
		end if
	end if
	Response.write "" & vbcrlf & "" & vbcrlf & "                    </table>" & vbcrlf & "                    </td>" & vbcrlf & "                    </tr>" & vbcrlf & "                "
	If ZBRuntime.MC(23004) Then
		Response.write "" & vbcrlf & "                <tr class=""top accordion"">" & vbcrlf & "                                     <td colspan=""6"" class=""accordion-bar-bg"">" & vbcrlf & "                                           <div  class=""accordion-bar-tit"">" & vbcrlf & "                                                  开票信息<span class=""accordion-arrow-down""></span>" & vbcrlf & "                                                </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td>" & vbcrlf & "                                                    <div align=""right"">产品开票名称：</div>" & vbcrlf & "                                           </td>" & vbcrlf & "                        <td>" & vbcrlf & "                            <input type=""text"" name=""InvoiceTitle"" Id=""InvoiceTitle"" value="""
		Response.write InvoiceTitle
		Response.write """ max=""92"" msg=""产品开票名称长度必须在0至92个字符之间"" dataType=""Limit"" style=""float:left;width:90%;"">" & vbcrlf & "                            <span id=""test19"" class=""red""></span>" & vbcrlf & "                        </td>" & vbcrlf & "                        <td><div align=""right"">享受税收优惠政策：</div></td>" & vbcrlf & "                                           <td>" & vbcrlf & "                                                    <input type=""radio"" name=""TaxPreference"" id=""TaxPreference1"" value=""1""><label for=""includeTax1"">是</label>" & vbcrlf & "                                                        <input type=""radio"" name=""TaxPreference"" id=""TaxPreference2"" checked value=""0""><label for=""includeTax2"">否</label>" & vbcrlf & "                                            </td>" & vbcrlf & "                                           <td><div align=""right"" id=""TaxPreferenceType0"" style=""display:none"">税收优惠政策类型：</div></td>" & vbcrlf & "                                             <td><div id=""TaxPreferenceType1"" style=""display:none"">" & vbcrlf & "                                <select name=""TaxPreferenceType"" id=""TaxPreferenceType2"" style=""float:left"">" & vbcrlf & "                                                                    <option value=""2"" selected>免税</option>" & vbcrlf & "                                                                  <option value=""3"">不征税</option>" & vbcrlf & "                                                                 <option value=""4"">普通零税率</option>" & vbcrlf & "                                                     </select>" & vbcrlf & "          <span class=""help_explan_ico"" onclick=""showHelpExplan(4)"" ></span>" & vbcrlf & "                                <div id=""bill_help_expaln_text4"" class=""bill_help_expaln_text"" style=""display:none;font-weight:100"">" & vbcrlf & "                                注：<br>销售产品的税率为“0.00%”<br>按照优惠政策类型开具发票" & vbcrlf & "                                <a title=""关闭"" href=""javascript:;""  onclick=""closediv(4)"" class=""bill_help_expaln_close resetElementHidden"">×</a>" & vbcrlf & "                                <a title=""关闭"" href=""javascript:;""  onclick=""closediv(4)""class=""bill_help_expaln_close resetElementShow"" style=""display:none;""></a> "                                          & vbcrlf &                                 "</div>" & vbcrlf &                             "</div>" & vbcrlf &                                          "</td>" & vbcrlf &                                    "</tr>" & vbcrlf &                                    "<tr>" & vbcrlf & "                                           <td>" & vbcrlf & "                                                    <div align=""right"">税收分类编码：</div>" & vbcrlf & "                                           </td>" & vbcrlf & "                        <td>" & vbcrlf & "                            <input type=""hidden"" name=""TaxClassifyMergeCoding"" id=""TaxClassifyMergeCoding"" value="""
		Response.write TaxClassify
		Response.write """/>" & vbcrlf & "                                                       <input type=""text"" readonly dataType=""Limit"" min=""1"" msg=""请选择产品税收分类编码！"" value="""
		Response.write TaxClassifyMergeCoding
		Response.write """ id=""TaxPreferenceTypeMergeCoding"" style=""float:left;width:90%;"">" & vbcrlf & "                            <div title=""点击打开选择页面"" onclick=""openTaxPreferenceType()"" class=""autocompleteico""></div>" & vbcrlf & "                            <span class=""red"" style=""margin-left:6px;"">*</span>" & vbcrlf & "                            <span id=""test20"" class=""red""></span>" & vbcrlf & "                        </td>" & vbcrlf & "                        <td><div align=""right"">税收分类名称：</div></td>" & vbcrlf & "                                            <td><div id=""TaxPreferenceTypeName""></div></td>" & vbcrlf & "           <td><div align=""right"">简称：</div></td>" & vbcrlf & "                                          <td><div id=""TaxPreferenceTypeJName""></div>" & vbcrlf & "                                               </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "              "
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select intro from setopen  where sort1=12 "
	rs1.open sql1,conn,1,1
	if rs1.eof then
		sort2=1
	else
	...
	If ZBRuntime.MC(19000) Then
		Response.write "" & vbcrlf & "                                     " & vbcrlf & "                                        <tr>" & vbcrlf & "                                            <td>" & vbcrlf & "                                                    <div align=""right"">是否含税：</div>" & vbcrlf & "                                               </td>" & vbcrlf & "                                           <td>" & vbcrlf & "                                                    <input type=""radio"" name=""includeTax"" id=""includeTax1"" value=""1""><label for=""includeTax1"">含税</label>" & vbcrlf & "                                                       <input type=""radio"" name=""includeTax"" id=""includeTax2"" checked value=""0""><label for=""includeTax2"">不含税</label>" & vbcrlf & "                                          </td>" & vbcrlf & "                                           <td><div align=""right"">可开具票据类型：</div></td>" & vbcrlf & "                                                <td>" & vbcrlf & "                                                    "
		Set rsConfig = conn.execute("select id,sort1 from sortonehy where gate2 = 34 AND isStop = 0 and isnull(id1,0)<>-65535 order by gate1 desc")
		While rsConfig.eof = False
			Response.write "" & vbcrlf & "                                                             <input type=""checkbox"" id=""invoiceConfig_"
			Response.write rsConfig("id")
			Response.write """ name=""invoiceTypes"" value="""
			Response.write rsConfig("id")
			Response.write """ checked><label for=""invoiceConfig_"
			Response.write rsConfig("id")
			Response.write """>"
		...
		Response.write "" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td ><div align=""right"">"
		if sort2="3" then
			Response.write "提成比例："
		end if
		Response.write "</div></td>" & vbcrlf & "                                        <td >"
		if sort2="3" then
			Response.write "<input name=""num_tc"" type=""text"" size=""15"" onKeyUp=""value=value.replace(/[^\d\.]/g,'')"" onpropertychange=formatData(this,""number"")  dataType=""Limit"" min=""0"" max=""10""  msg=""长度必须在10位之内""  value="""
			Response.write num_tc
			Response.write """> %"
		end if
		Response.write "</td>" & vbcrlf & "                                  </tr>" & vbcrlf & "                                   "
	end if
	session("num_click2009cp")=1
	Session("unitallList")=0
	sql="Delete  mxpxcp Where cateid="&session("personzbintel2007")&" "
	conn.Execute(sql)
	unitgp = 0
	Response.write "" & vbcrlf & "                              <tr>" & vbcrlf & "                                    <td ><div align=""right"">基本单位组：</div></td>" & vbcrlf & "                                   <td colspan=5>" & vbcrlf & "                                          <span id=""trpx_unit1"" style=""float:left"">" & vbcrlf & "                                                   <select name=""unitgp"" id=""unitgp"" onChange=""ChangeUnitGroup('add')"" dataType=""Limit""min=""1"" max=""100"" msg=""请选择基本单位组"">" & vbcrlf &                                                        "<option value=""></option>" & vbcrlf &                                           ""
	set rs2=server.CreateObject("adodb.recordset")
	sql2="select id,name from erp_comm_UnitGroup where isNull(Stoped,0)=0 and exists(select 1 from ErpUnits a where isnull(a.stoped,0)=0 and a.unitgp=erp_comm_UnitGroup.id) order by sort1 desc"
	rs2.open sql2,conn,3,1
	do until rs2.eof
		Response.write "" & vbcrlf & "                                                                      <option value="""
	...
	rs2.close
	set rs2=nothing
	Response.write "" & vbcrlf & "                                                      </select>                                               " & vbcrlf & "                                                </span>" & vbcrlf & "                        <div id=""bill_help_expaln_text1"" class=""bill_help_expaln_text1"" style=""display:none;"">温馨提示：<br />1、换算比例：基本单位同一行的换算比例始终显示为1；<br />   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;其他单位同一行的换算比例可以编辑；<br />" & vbcrlf & "                            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;单位比例的换算是以1为基准进行换算；<br />" & vbcrlf & "                            例如：<br />" & vbcrlf & "                            数量组     包       1.00<br />" & vbcrlf & "                            数量组     箱       10.00<br />" & vbcrlf & "                            数量组     车       50.00<br />" & vbcrlf & "                            此设置代表：1箱=10包     1车=50包    1车=5箱<br />" & vbcrlf & "                            2、分组属性、属性项：<br />" & vbcrlf & "                            产品所选单位分组为体积组或面积组时，支持选择分组属性，并且可自定义设置属性项，如果属性项值大于0，则此项为固定项，不允许编辑；如果属性项值为0，则此项为变动项，支持手动编辑。<br />" & vbcrlf & "                            注：当前分类已应用产品属性时，不支持设置产品单位属性。<br />" & vbcrlf & "                            3、产品以采购主单位作为存货核算单位，当上次完成存货核算后且无新出入库单生成时进行核算单位调整。<a title=""关闭"" href=""javascript:;""   onclick=""closediv(2)"" class=""bill_help_expaln_close1 resetElementHidden"" style=""position:absolute;top:-2px;right:5px;font-size:14"">×</a><a title=""关闭"" href=""javascript:;""   onclick=""closediv(2)"" class=""bill_help_expaln_close1 resetElementShow"" style=""position:absolute;top:-2px;right:5px;font-size:14px;display:none;""></a></div>" &vbcrlf & "                          <span style=""color:red;float:left;margin-top:3px;margin-left:5px;"">*</span>" & vbcrlf & "                         <span class=""help_explan_ico1"" style=""float:left"" onclick=""showHelpExplan(2)"" ></span>                                              " & vbcrlf & "                                       </td>                                           " & vbcrlf & "                                        </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td colspan=""6"" valign=""top"">" & vbcrlf & "                                                       <div style=""overflow-x:auto;height:auto;"" id=""celue_div"">" & vbcrlf & "                                                           <div>" & vbcrlf & "                                                                   "
	set rs2=nothing
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select sort2 from sort5 where time1=1  order by sort1 asc,gate2 desc"
	rs7.open sql7,conn,1,1
...
'if proStore=1 then
	...
	If isOpenMoreUnitAttr Then colLen = colLen + 2
'if proStore=1 then
	isOpenCGMainUnit = OpenCGMainUnit() and ZBRuntime.MC(15000)
	if isOpenCGMainUnit then colLen = colLen + 1
	isOpenCGMainUnit = OpenCGMainUnit() and ZBRuntime.MC(15000)
	Response.write "" & vbcrlf & "                                                                      <table width="""
	Response.write colLen*80+260
	Response.write """ border=""0"" cellpadding=""3"" style=""table-layout:fixed;word-break:break-all;word-wrap:break-word;border-collapse:collapse;""  id=""hasbor"">" & vbcrlf & "                                                                          <tr class=""top"">" & vbcrlf & "                                                                                  <td width=""70""><div align=""center"">单位分组</div></td>" & vbcrlf & "                                                                                      <td width=""70""><div align=""center"">单位<input type=""hidden"" id=""isOpenCGMainUnit"" value="""
	Response.write iif(isOpenCGMainUnit,1,0)
	Response.write """></div></td>" & vbcrlf & "                                                                                      <td width=""100""><div align=""center"">换算比例</div></td>" & vbcrlf & "                                                                                     "
	If isOpenCGMainUnit Then
		Response.write "" & vbcrlf & "                                                                                              <td width=""70""><div align=""center"">采购主单位</div></td>" & vbcrlf & "                                                                                    "
	end if
	If phXlhManageShow Then
		Response.write "" & vbcrlf & "                                                                                      <td width=""70""><div align=""center"">序列号管理</div></td>" & vbcrlf & "                                                                                    "
	end if
	If isOpenMoreUnitAttr Then
		Response.write "" & vbcrlf & "                                                                                              <td width=""60""><div align=""center"">分组属性</div></td>" & vbcrlf & "                                                                                              <td width=""120""><div align=""center"">属性项</div></td>" & vbcrlf & "                                                                                       "
	end if
	Response.write "" & vbcrlf & "                                                                                      <td width=""70""><div align=""center"">部门</div></td>" & vbcrlf & "                                                                                  "
	if proStore=1 then
		Response.write "" & vbcrlf & "                                                                                              <td width=""70""><div align=""center"">主仓库</div></td>" & vbcrlf & "                                                                                                <td width=""70""><div align=""center"">仓库容量</div></td>" & vbcrlf & "                                                                                              <td width=""50""><div align=""center"">辅仓库</div></td>" & vbcrlf & "                                                                                        "
	end if
	Response.write "" & vbcrlf & "                                                                                      <td width=""70"" style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center"">条形码</div></td>" & vbcrlf & "                                                                                  "
	if ZBRuntime.MC(15000) then
		Response.write "" & vbcrlf & "                                                                                              <td width=""70"" style='"
		Response.write strDisplay21_21
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center"">建议进价</div></td>" & vbcrlf & "                                                                                          <td width=""70"" style='"
		Response.write strDisplay21_21
		Response.write strDisplay21_21
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center"">最高进价</div></td>" & vbcrlf & "                                                                                  "
		Response.write strDisplay21_21
	end if
	if ZBRuntime.MC(7000) then
		Response.write "" & vbcrlf & "                                                                                      <td width=""70"" style='"
		Response.write strDisplay21_22
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center"">建议售价</div></td>" & vbcrlf & "                                                                                  <td width=""70"" style='"
		Response.write strDisplay21_22
		Response.write strDisplay21_22
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center"">最低售价</div></td>" & vbcrlf & "                                                                                  "
		Response.write strDisplay21_22
	end if
	do until rs7.eof
		Response.write "" & vbcrlf & "                                                                                              <td width=""70"" style='"
		Response.write strDisplay21_22
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;word-break:break-all;word-wrap:break-word;border-collapse:collapse;'><div align=""center"">"
		Response.write strDisplay21_22
		Response.write rs7("sort2")
		Response.write "价格</div></td>" & vbcrlf & "                                                                                               "
		rs7.movenext
	loop
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "                                                                              </tr>" & vbcrlf & "                                                                   </table>" & vbcrlf & "                                                                        <span id=""trpx0""></span>" & vbcrlf & "                                                                  "
	list=""
	for i=1 to 100
		list=list+"<span id='trpx"&i&"'></span>"
'for i=1 to 100
	next
	Response.write(""&list&"")
	Response.write "" & vbcrlf & "                                                                      <table width="""
	Response.write colLen*80+260
	Response.write """ border=""0"" cellpadding=""3"" style=""table-layout:fixed;word-break:break-all;word-wrap:break-word;border-collapse:collapse;margin-top:-1px;""  id=""hasbor"">" & vbcrlf & "                                                                         <tr>" & vbcrlf & "                                                                                    <td width=""70""><div align=""left""><a href=""javascript:void(0);"" id=""newInsert"" onclick=""AddProductPriceRule(jQuery('#unitDiv_0_'+jQuery('#baseUnitInput').val()).find('option:selected').text(),'add')"">点击新增</a></div></td>" & vbcrlf & "                                                                                     <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                      <td width=""100""><div align=""center""></div></td>"& vbcrlf &                                            ""
	If isOpenCGMainUnit Then
		Response.write "" & vbcrlf & "                                                                                             <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                      "
	end if
	If phXlhManageShow Then
		Response.write "" & vbcrlf & "                                                                                             <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                      "
	end if
	If isOpenMoreUnitAttr Then
		Response.write "" & vbcrlf & "                                                                                             <td width=""60""><div align=""center""></div></td>" & vbcrlf & "                                                                                              <td width=""120""><div align=""center""></div></td>" & vbcrlf & "                                                                                     "
	end if
	Response.write "                                                                                     " & vbcrlf & "                                                                                        <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                      "
	if proStore=1 then
		Response.write "" & vbcrlf & "                                                                                             <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                              <td width=""70""><div align=""center""></div></td>" & vbcrlf & "                                                                                              <td width=""50""><div align=""center""></div></td>" & vbcrlf & "                                                                                      "
	end if
	Response.write "" & vbcrlf & "                                                                                     <td width=""70"" style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center""></div></td>" & vbcrlf & "                                                                                        "
	if ZBRuntime.MC(15000) then
		Response.write "" & vbcrlf & "                                                                                     <td width=""70"" style='"
		Response.write strDisplay21_21
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center""></div></td>" & vbcrlf & "                                                                                 <td width=""70"" style='"
		Response.write strDisplay21_21
		Response.write strDisplay21_21
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align=""center""></div></td>" & vbcrlf & "                                                                                 "
		Response.write strDisplay21_21
	end if
	if ZBRuntime.MC(7000) then
		Response.write "" & vbcrlf & "                                                                                     <td width=""70"" style='"
	...
	For i=0 To num1-1
		...
		Response.write "" & vbcrlf & "                                                                                             <td width=""70"" style='"
		Response.write strDisplay21_22
		Response.write "border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;word-break:break-all;word-wrap:break-word;border-collapse:collapse;'><div align=""center""></div></td>" & vbcrlf & "                                                                                              "
		Response.write strDisplay21_22
	next
	Response.write "" & vbcrlf & "                                                                             </tr>" & vbcrlf & "                                                                   </table>" & vbcrlf & "                                                                </div>" & vbcrlf & "                                                  </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "" & vbcrlf & "                                   <tr class=""top accordion"">" & vbcrlf & "                                                <td colspan=""6"" class=""accordion-bar-bg"">" & vbcrlf & "                                                 <div  class=""accordion-bar-tit"">图片信息<span class=""accordion-arrow-down""></span></div>" & vbcrlf & "                                            </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td width=""10%"" align=""right"" height=""27"">产品图片：</td>" & vbcrlf & "                                             <td colspan=""6"">" & vbcrlf & "                                                    <div class=""multimage-gallery"">" & vbcrlf & "                                                           <input type=""hidden"" id=""primaryImg"" name=""primaryImg"" value="""
	Response.write strDisplay21_22
	Response.write primaryImg
	Response.write """ >" & vbcrlf & "                                                               <ul id=""productPic"">" & vbcrlf & "                                                              "
	For i = 1 To 6
...
'For i = 1 To 6
	...
	next
	Response.write "" & vbcrlf & "                                                             </ul>" & vbcrlf & "                                                           <div style=""clear:both;""></div>" & vbcrlf & "                                                   <table class=""resetBgColor"" id=""atttb"" border=""0"" cellspacing=""1"" cellpadding=""0"" bgcolor=""#C0CCDD"" style=""display:none;"">" & vbcrlf & "" & vbcrlf & "                                                    </table>                                                " & vbcrlf & "" & vbcrlf & "                                                   </div>" & vbcrlf & "" & vbcrlf & "                                                  <span style=""cursor:pointer"" onClick=""showUploadForm(this);""><div style=""vertical-align:middle;float:left""><img src='../images/smico/3.gif'/></div><div id=""addImgBtn"" style="""">添加图片</div></span>" & vbcrlf & "                                             </td>" & vbcrlf & "                                       </tr>" & vbcrlf & "" & vbcrlf & "                                   <tr class=""top accordion"">" & vbcrlf & "                                                <td colspan=""6"" class=""accordion-bar-bg"">" & vbcrlf & "                                                   <div  class=""accordion-bar-tit"">" & vbcrlf & "                                                          概要信息<span class=""accordion-arrow-down""></span>" & vbcrlf & "                                                        </div>"& vbcrlf &                                             "</td>" & vbcrlf &                                    "</tr>" & vbcrlf & vbcrlf &                                    "<tr>" & vbcrlf &                                             "<td><div align=""right"">产品说明：</div></td>" & vbcrlf &                                               "<td colspan=""5"">" & vbcrlf &                                                   "<span class=""gray"">" & vbcrlf &                                                                "<textarea name=""intro1"" cols=""80"" rows=""6"" dataType=""Limit"" min=""0"" max=""3000""  msg=""产品说明必须在3000个字之内"">"
	Response.write intro1
	Response.write "</textarea>" & vbcrlf & "                                                  </span>" & vbcrlf & "                                         </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                    <td><div align=""right"">产品参数：</div></td>" & vbcrlf & "                                      <td colspan=""5"">" & vbcrlf & "                                          <span class=""gray"">" & vbcrlf & "                                                       <textarea name=""intro2""cols=""80"" rows=""6"" dataType=""Limit"" min=""0"" max=""3000""  msg=""产品参数必须在3000个字之内"">"
	Response.write intro2
	Response.write "</textarea>" & vbcrlf & "                                          </span>" & vbcrlf & "                                         </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td>" & vbcrlf & "                                                    <div align=""right"">图片与附件：</div>" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td colspan=""5"">" & vbcrlf & "                                                  <span class=""gray"">"& vbcrlf & "                                                              <textarea name=""intro3"" style=""display:none"" cols=""1"" rows=""1"">"
	if intro3<>"" then Response.write intro3
	Response.write "</textarea>" & vbcrlf & "                                                    <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro3&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME>" & vbcrlf & "                                                    </span>" & vbcrlf & "                                         </td>"& vbcrlf & "                                      </tr>" & vbcrlf & "                                   "
	Response.write ShowExtendedByKZZDY(16001,0,1,1,5, true , bt_kh , ts_kh , 21,false, 0)
	Response.write "" & vbcrlf & "                                     <tr class=""btns-bar"">" & vbcrlf & "                                             <td colspan=""6"">" & vbcrlf & "                                                  <div align=""center"">" & vbcrlf & "                                                              <input type=""submit"" name=""Submit42222"" value=""保存""  class=""page""/>" & vbcrlf & "                                                            <input type=""submit"" name=""Submit42422"" value=""增加"" onClick=""ask2();""  class=""page""/>" & vbcrlf & "                                                                <input type=""reset"" value=""重填""  class=""page"" name=""B222"">" & vbcrlf & "                                                     </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr>   " & vbcrlf & "                                </table></form>" & vbcrlf & "                 </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                   <td  class=""page"">" & vbcrlf & "                                <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                                 <tr>" & vbcrlf & "                                            <td height=""2230"" ><div align=""center""></div></td>" & vbcrlf & "                                  </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" &vbcrlf & "        </table>" & vbcrlf & "<script src= ""../Script/pt_add_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""adsDiv"" style=""border: 1px solid rgb(0, 0, 0); position:absolute;z-index:2001;display:none;padding:2px 2px 2px 2px;background-color:white;width:200px"">" & vbcrlf & "<iframe src="""" name=""adsIF"" id=""adsIF"" width=""100%""  frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<div id=""ssDiv"" style=""border: 1px solid rgb(0, 0, 0); position:absolute;z-index:2002;display:none;padding:2px 2px 2px 2px;background-color:white"">" & vbcrlf & "<iframe src="""" name=""ssIF"" id=""ssIF"" width=""500px"" height='99%' frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<div id=""bgDiv"" style=""display:none;position:absolute;top:0%;left:0%;width:100%;height:100%;background-color:#B9C5DD;z-index:2000;-moz-opacity:0.7;opacity:.70;filter:alpha(opacity=50);""></div>" & vbcrlf & "<form target=""ssIF"" action=""../Product/SlaveStoreList.asp"" id=""ssIFForm"" style=""display:none"" method=""post"">" & vbcrlf & "     <input type=""hidden"" id=""mainfrm"" name=""sid"" value="""">" & vbcrlf & "  <input type=""hidden"" id=""robjfrm"" name=""robj"" value="""">" & vbcrlf & "       <textarea id=""extfrm"" style=""display:none"" name=""ext""></textarea>" & vbcrlf & "</form>" & vbcrlf & "<iframe src="""" name=""dosave"" id=""dosave"" width=""0"" height='0' frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & ""
	Response.write "<!--上传文件模块开始-->"
	xmlPath = "../edit/upimages/product/" & Timer & ".xml"
	If proID = "" Then proID = tempProID
	...
	Response.write xmlPath
	Response.write "');//启动进度条" & vbcrlf & "  return true;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "//启动进度条" & vbcrlf & "function startProgress(xmlPath)" & vbcrlf & "{" & vbcrlf & "  displayProgress();" & vbcrlf & "  setProgressDivPos();" & vbcrlf & "  setTimeout(""DisplayProgressBar('"" + xmlPath + ""')"",500);"& vbcrlf & "}" & vbcrlf & vbcrlf & "function DisplayProgressBar(xmlPath)" & vbcrlf & "{" & vbcrlf & "       var xmlDoc;" & vbcrlf & "     if (window.ActiveXObject) {" & vbcrlf & "             xmlDoc = new ActiveXObject(xmlPath);" & vbcrlf & "           if (xmlDoc.parseError.errorCode!=0)" & vbcrlf & "             {" & vbcrlf & "                       //var error = xmlDoc.parseError;" & vbcrlf & "                        //alert(error.reason)" & vbcrlf & "                   setTimeout(""DisplayProgressBar('"" + xmlPath + ""')"",1000);" & vbcrlf & "                   return;" & vbcrlf & "         }" & vbcrlf & "               try" & vbcrlf & "          {" & vbcrlf & "                       var root = xmlDoc.documentElement;   //根节点" & vbcrlf & "                   var totalbytes = root.childNodes[0].text;" & vbcrlf & "                       var uploadbytes = root.childNodes[1].text;" & vbcrlf & "                      var percent = root.childNodes[2].text;" & vbcrlf & "                  ProgressPercent.innerHTML = percent + ""%"";" & vbcrlf & "                     ProgressBar.style.width = percent + ""%"";" & vbcrlf & "                  uploadSize.innerHTML = uploadbytes;" & vbcrlf & "                     uploadSpeed.innerHTML = root.childNodes[3].text;" & vbcrlf & "                        totalTime.innerHTML = root.childNodes[4].text;" & vbcrlf & "                  leftTime.innerHTML = root.childNodes[5].text;" & vbcrlf & "                        if (percent<100)" & vbcrlf & "                        {" & vbcrlf & "                               setTimeout(""DisplayProgressBar('"" + xmlPath + ""')"",1000);" & vbcrlf & "                   }" & vbcrlf & "               }" & vbcrlf & "               catch(e)" & vbcrlf & "                {" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "       else if (isFirefox=navigator.userAgent.indexOf(""Firefox"")>0) { //火狐浏览器" & vbcrlf & "     //else if (document.implementation && document.implementation.createDocument) {//这里主要是对谷歌浏览器进行处理" & vbcrlf & "         xmlDoc = document.implementation.createDocument('', '', null);" & vbcrlf & "          xmlDoc.async = false;" & vbcrlf & "           xmlDoc.load(xmlPath);" & vbcrlf & "           try" & vbcrlf & "             {" & vbcrlf & "                       var root = xmlDoc.documentElement;   //根节点" & vbcrlf & "                   var totalbytes =  xmlDoc.childNodes[0].childNodes[0].textContent;" & vbcrlf & "                       var uploadbytes =  xmlDoc.childNodes[0].childNodes[1].textContent;" & vbcrlf & "                      var percent =  xmlDoc.childNodes[0].childNodes[2].textContent;" & vbcrlf & "                    ProgressPercent.innerHTML = percent + ""%"";" & vbcrlf & "                        ProgressBar.style.width = percent + ""%"";" & vbcrlf & "                  uploadSize.innerHTML = uploadbytes;" & vbcrlf & "                     " & vbcrlf & "                        uploadSpeed.innerHTML = xmlDoc.childNodes[0].childNodes[3].textContent;" & vbcrlf & "                   totalTime.innerHTML = xmlDoc.childNodes[0].childNodes[4].textContent;" & vbcrlf & "                   leftTime.innerHTML = xmlDoc.childNodes[0].childNodes[5].textContent;" & vbcrlf & "                    if (percent<100)" & vbcrlf & "                        {" & vbcrlf & "                               setTimeout(""DisplayProgressBar('"" + xmlPath + ""')"",1000);" & vbcrlf & "                      }" & vbcrlf & "               }" & vbcrlf & "               catch(e)" & vbcrlf & "                {" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "       }" & vbcrlf & "       else{ //谷歌浏览器" & vbcrlf & "        var xmlhttp = new XMLHttpRequest();" & vbcrlf & "           xmlhttp.open(""GET"",xmlPath,false);" & vbcrlf & "                xmlhttp.send(null);" & vbcrlf & "            if(xmlhttp.readyState == 4){" & vbcrlf & "                    xmlDoc = xmlhttp.responseXML;" & vbcrlf & "           }" & vbcrlf & "               try" & vbcrlf & "             {" & vbcrlf & "                       var root = xmlDoc.documentElement;   //根节点" & vbcrlf & "                   var totalbytes = root.childNodes[0].textContent;" & vbcrlf & "                        var uploadbytes = root.childNodes[1].textContent;" & vbcrlf & "                   var percent = root.childNodes[2].textContent;" & vbcrlf & "                   ProgressPercent.innerHTML = percent + ""%"";" & vbcrlf & "                        ProgressBar.style.width = percent + ""%"";" & vbcrlf & "                  uploadSize.innerHTML = uploadbytes;" & vbcrlf & "                     uploadSpeed.innerHTML = root.childNodes[3].textContent;" & vbcrlf & "                     totalTime.innerHTML = root.childNodes[4].textContent;" & vbcrlf & "                   leftTime.innerHTML = root.childNodes[5].textContent;" & vbcrlf & "                    if (percent<100)" & vbcrlf & "                        {" & vbcrlf & "                               setTimeout(""DisplayProgressBar('"" + xmlPath + ""')"",1000);" & vbcrlf & "                        }" & vbcrlf & "               }" & vbcrlf & "               catch(e)" & vbcrlf & "                {" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function displayProgress()" & vbcrlf & "{" & vbcrlf & "  var objProgress = document.getElementById(""Progress"");" & vbcrlf & "  objProgress.style.display = """";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function closeProgress()" & vbcrlf & "{" & vbcrlf & "  var objProgress = document.getElementById(""Progress"");" & vbcrlf & "  objProgress.style.display = ""none"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setProgressDivPos()" & vbcrlf & "{" & vbcrlf & "   var $ = jQuery;" & vbcrlf & " var objProgress = document.getElementById(""Progress"");" & vbcrlf & "    var y, scrollTop, h;" & vbcrlf & "    scrollTop = document.documentElement.scrollTop || document.body.scrollTop;" & vbcrlf & "      h = document.documentElement.clientHeight || document.body.clientHeight;" & vbcrlf & " y = h / 2 - objProgress.offsetHeight / 2 + scrollTop;" & vbcrlf & "   objProgress.style.top = y + ""px"";" & vbcrlf & " objProgress.style.left = (document.documentElement.scrollLeft || document.body.scrollLeft) + (document.documentElement.clientWidth - document.getElementById(""Progress"").offsetWidth) / 2 + ""px"";" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""Progress"" style=""display:none;"" class=""progress"">" & vbcrlf & "    <div class=""bar"">" & vbcrlf & "        <div id=""ProgressPercent"" class=""ProgressPercent"">0%</div>" & vbcrlf & "        <div id=""ProgressBar"" class=""ProgressBar""></div>" & vbcrlf & "    </div>" & vbcrlf & "    <table border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "        <tr>" & vbcrlf & "            <td>已经上传</td>" & vbcrlf & "            <td>:</td>" & vbcrlf & "          <td id=""uploadSize""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "            <td>上传速度</td>" & vbcrlf & "            <td>:</td>" & vbcrlf & "            <td id=""uploadSpeed"">&nbsp;</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "            <td>共需时间</td>" & vbcrlf & "            <td>:</td>" & vbcrlf & "            <td id=""totalTime"">&nbsp;</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "            <td>剩余时间</td>" & vbcrlf & "            <td>:</td>" & vbcrlf & "            <td id=""leftTime"">&nbsp;</td>" & vbcrlf &"        </tr>" & vbcrlf & "    </table>" & vbcrlf & "</div>" & vbcrlf & "<div id=""bg""></div>" & vbcrlf & "<!--上传模块结束-->" & vbcrlf & ""
	'Response.write xmlPath
