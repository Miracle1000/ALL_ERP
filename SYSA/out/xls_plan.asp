...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	Function RemoveHTML(strHTML)
		...
		strHTML=replace(strHTML,"<p>",chr(13))
		strHTML=replace(strHTML,"</p>",chr(13))
		strHTML=replace(strHTML,"<BR>",chr(13))
		strHTML=replace(strHTML,"<br />",chr(13))
		strHTML=replace(strHTML,"<br/>",chr(13))
		strHTML=replace(strHTML,"<TD>","  ")
		strHTML=replace(strHTML,"</TD>"," ")
		strHTML=replace(strHTML,"</TR>",chr(13))
		strHTML=replace(strHTML,"<TABLE>",chr(13))
		strHTML=replace(strHTML,"</TABLE>",chr(13))
	...
	if newstate="1" then
		...
		sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where ord=1 and cateid="&session("personzbintel2007")&""
		rs9.open sql9,conn,1,1
		if rs9.eof then
			tq1=0
		else
			Str_Result_alt="and a.cateid="&session("personzbintel2007")&""
			tq1=rs9("tq1")
		end if
		rs9.close
		set rs9=nothing
		Str_Result=Str_Result+" and a.complete='1' and a.option1<>'1' and (a.startdate1<'"&dateadd("d",tq1,date)&"' or (a.startdate1='"&dateadd("d",tq1,date)&"' and (a.time1<'"&hour(time)&"' or (a.time1='"&hour(time)&"'and a.time2<'"&minute(time)&"')))) "&noalt1&" "&Str_Result_alt&""
		set rs9=nothing
	end if
	If remind <> "" Then
		Set helper = CreateReminderHelper(conn,57,0)
		Str_Result = " and a.ord in (" & helper.listSQL("ids") & ")"
	end if
	if s="3333333" or s="5555555" then
		...
		set rs9=server.CreateObject("adodb.recordset")
		sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where ord=1 and cateid="&session("personzbintel2007")&""
		rs9.open sql9,conn,1,1
		if rs9.eof then
			tq1=0
		else
			if rs9("fw1")="0" then
				if open_71_1=3 then
					Str_Result_alt=""
				elseif open_71_1=1 then
					Str_Result_alt="and a.cateid in ("&intro_71_1&")"
				else
					Str_Result_alt="and a.cateid=0"
				end if
			else
				Str_Result_alt="and a.cateid="&session("personzbintel2007")&""
			end if
	...
	if s="1" then
		Str_Result=Str_Result+"and a.complete='1' and (DateDiff(d,a.startdate1,'"&date&"')<0  or (DateDiff(d,a.startdate1,'"&date&"')=0  and (cast (a.time1 as int)>"&hour(time)&" or (cast(a.time1 as int)="&hour(time)&" and cast(a.time2 as int)>="&minute(time)&")))) "
'if s="1" then
	end if
	if s="5" then
		Str_Result=Str_Result+"and a.complete='1' and (DateDiff(d,a.startdate1,'"&date&"')>0  or (DateDiff(d,a.startdate1,'"&date&"')=0  and (cast (a.time1 as int)<'"&hour(time)&"' or (cast (a.time1 as int)="&hour(time)&" and cast(a.time2 as int)<"&minute(time)&")))) "
...
'if s="2" then
	...
	if s="3" then
		Str_Result=Str_Result+"and a.order1<>'' and a.cateid="&session("personzbintel2007")&" "
'if s="3" then
		pname="指派人"
	end if
	if s="4" then
		Str_Result=Str_Result+"and a.order1='"&session("personzbintel2007")&"'"
'if s="4" then
	end if
	if C="1" then
		Str_Result=Str_Result+"and  a.complete='1' and (a.startdate1>'"&date&"' or (a.startdate1='"&date&"' and (a.time1>'"&hour(time)&"' or (a.time1='"&hour(time)&"'and a.time2>='"&minute(time)&"'))))"
'if C="1" then
	elseif C="2" then
		Str_Result=Str_Result+"and  a.complete ='2'"
'elseif C="2" then
...
'elseif C="3" then
	...
	If qtjz <> "" Then
		Str_Result=Str_Result &" AND a.intro2 LIKE '%"& qtjz &"%' "
	end if
	If rczj <> "" Then
		Str_Result=Str_Result &" AND EXISTS (SELECT * FROM plan2 WHERE date1 = DATEADD(D,1,a.date1) AND type = 11 AND cateid = a.cateid AND intro LIKE '%"& rczj &"%') "
	end if
	if D <> "" then
		Str_Result=Str_Result+"and  a.intro like '%"& D &"%'"
'if D <> "" then
	end if
	time1=request("time1")
	time2=request("time2")
	time3=request("time11")
	time4=request("time22")
	If time1="" Then time1="23"
	If time2="" Then time2="59"
	If time3="" Then time3="00"
	If time4="" Then time4="00"
	if ret <> "" then
		Str_Result=Str_Result+" and  DATEDIFF(MI,'"&ret& " " & time3 &":"&time4&":00',dateadd(MI,CAST(isnull(a.starttime2,0) as int),dateadd(HH,cast(isnull(a.starttime1,0) as int),a.startdate1))) >= 0 "
'if Exit Sub <> "" then
	end if
	if ret2 <> "" then
		Str_Result=Str_Result+" and  DATEDIFF(MI,'"&ret2& " " & time1 &":"&time2&":00',dateadd(MI,CAST(isnull(a.starttime2,0) as int),dateadd(HH,cast(isnull(a.starttime1,0) as int),a.startdate1))) <= 0 "
'if Exit Sub2 <> "" then
	end if
	if open_71_10=1 then
		Str_Result=Str_Result+"and  a.cateid in("&intro_71_10&")"
'if open_71_10=1 then
	elseif open_71_10=3 then
	else
...
'if W4<>"" Then
	...
	if w<>"" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from gate where ord="&w&" "
	...
	elseif px=3 then
		pxResult="  order by a.startdate1 desc,a.time1 desc,a.time2 desc"
	elseif px=4 then
		pxResult="  order by a.startdate1 asc,a.time1 asc,a.time2 asc"
	elseif px=7 then
	...
	xlWorksheet.Cells(1,1).Value = "开始时间"
	xlWorksheet.Cells(1,1).font.Size=10
	xlWorksheet.Cells(1,1).font.bold=True
	xlWorksheet.Cells(1,2).Value = "截止时间"
	xlWorksheet.Cells(1,2).font.Size=10
	xlWorksheet.Cells(1,2).font.bold=true
	xlWorksheet.Cells(1,3).Value = "日程内容"
	xlWorksheet.Cells(1,3).font.Size=10
	xlWorksheet.Cells(1,3).font.bold=true
	xlWorksheet.Cells(1,4).Value = "洽谈进展"
	xlWorksheet.Cells(1,4).font.Size=10
	xlWorksheet.Cells(1,4).font.bold=true
	xlWorksheet.Cells(1,5).Value = "完成状态"
	xlWorksheet.Cells(1,5).font.Size=10
	xlWorksheet.Cells(1,5).font.bold=true
	xlWorksheet.Cells(1,6).Value = pname
	xlWorksheet.Cells(1,6).font.Size=10
	xlWorksheet.Cells(1,6).font.bold=true
	xlWorksheet.Cells(1,7).Value = "当天总结"
	xlWorksheet.Cells(1,7).font.Size=10
	xlWorksheet.Cells(1,7).font.bold=true
	Set rs = server.CreateObject("adodb.recordset")
	sql = "SELECT charName FROM sp_schedule ORDER BY intLevel, id"
	rs.open sql,conn,1,1
	If Not rs.Eof Then
		dim colNum
		colNum = 8
		Do While rs.Eof = False
			charName = rs("charName")
			xlWorksheet.Cells(1,colNum).NumberFormatLocal = "@"
			xlWorksheet.Cells(1,colNum).font.Size=10
			xlWorksheet.Cells(1,colNum).font.bold=true
			xlApplication.ActiveSheet.Columns(colNum).ColumnWidth=10
			xlApplication.ActiveSheet.Columns(colNum).HorizontalAlignment=3
			xlWorksheet.Cells(1,colNum).Value = charName
			colNum = colNum + 1
			xlWorksheet.Cells(1,colNum).Value = charName
			rs.movenext
		Loop
	...
	Response.Flush
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.*,b.name from plan1 as a,gate as b where a.cateid=b.ord "&Str_Result&" "&pxResult
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "日程_"&session("name2006chen")&".xls",C1
	i=0
	do while not rs.eof
		if rs("complete")="1" and (cdate(rs("startdate1"))>date or (cdate(rs("startdate1"))=date and (clng(rs("time1"))>hour(time) or (clng(rs("time1"))=hour(time) and clng(rs("time2"))>=minute(time))))) then
			complete="待完成"
		elseif rs("complete")="1" and (cdate(rs("startdate1"))<date or (cdate(rs("startdate1"))=date and (clng(rs("time1"))<hour(time) or (clng(rs("time1"))=hour(time) and clng(rs("time2"))<minute(time))))) then
			complete="已延误"
		elseif rs("complete")="2" then
			complete="已完成"
		elseif rs("complete")="7" then
			complete="周计划"
		end if
		if s=3 Then
			set rs8=conn.execute("select isnull(name,'') from gate where ord="&rs("order1")&" ")
		else
			set rs8=conn.execute("select isnull(name,'') from gate where ord="&rs("cateid")&" ")
		end if
		if not rs8.eof then
			pname=rs8(0).value
		else
		...
		If Len(time2)=1 Then time1="0"&time2
		If  rs("startdate1")="" Or isnull(rs("startdate1")) Then
			date2=""
		else
			...
			date2=formatdatetime(rs("startdate1"),0) & " " & starttime1&":"&starttime2
		end if
		cateid      = rs("cateid")
		sql1 = "SELECT intro,date7 FROM plan2 WHERE date1 = '"&DateAdd("d",1,rs("startdate1"))&"' AND TYPE = 11 AND cateid = "&cateid&" "
		...
		rs1.Close
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,1).Value = date2
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,1).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,1).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,1).NumberFormatLocal = "yyyy-m-d hh:mm"
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,2).Value = formatdatetime(rs("startdate1"),0) & " " & time1&":"&time2
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,2).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,2).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,2).NumberFormatLocal = "yyyy-m-d hh:mm"
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,3).Value = RemoveHTML(rs("intro"))
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,3).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,3).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,4).Value = RemoveHTML(rs("intro2"))
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,4).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,4).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,5).Value = complete
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,5).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,5).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,6).Value = pname
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,6).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,6).HorizontalAlignment=3
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,7).Value = RemoveHTML(zjIntro)
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,7).font.Size=10
		Set rs1 = Nothing
		xlWorksheet.Cells(2+i,7).HorizontalAlignment=3
		Set rs1 = Nothing
		sql2 =      "SELECT isnull(a.intro,'') as intro, a.date7 FROM sp_schedule b " &_
		"left JOIN plan2 a ON b.intLevel = a.lcb " &_
		"and a.type = 17 AND a.date1 = '"&DateAdd("d",1,rs("startdate1"))&"' AND a.cateid = "&cateid&" " &_
		"ORDER BY b.intLevel, b.id "
		Set rs2 = conn.Execute(sql2)
		If Not rs2.Eof Then
			colNum = 8
			Do While rs2.Eof = False
				pdIntro = rs2("intro")
				xlWorksheet.Cells(2+i,colNum).Value = RemoveHTML(pdIntro)
				pdIntro = rs2("intro")
				xlWorksheet.Cells(2+i,colNum).font.Size=10
				pdIntro = rs2("intro")
				xlWorksheet.Cells(2+i,colNum).HorizontalAlignment=3
				pdIntro = rs2("intro")
				colNum = colNum + 1
				pdIntro = rs2("intro")
				rs2.MoveNext
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>日程导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!日程导出成功,共有"&(i)&"条记录!</B>"";</script>"
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("日程_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的日程</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
