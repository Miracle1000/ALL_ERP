	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				Str_Result=""
				if open_6_11=3 then
'elseif open_6_11=1 then
					str_Result=str_Result+ " and e.cateid in ("&intro_6_11&") "
'elseif open_6_11=1 then
				else
					str_Result=str_Result+ " and e.cateid =0 "
'elseif open_6_11=1 then
				end if
				Str_Result=" where 1=1 "&list1&""
				A3=request("A3")
				B=request("B")
				C=request("C")
				m1=request("ret")
				...
				duemoney3=request("duemoney3")
				duemoney4=request("duemoney4")
				duemoney5=request("duemoney5")
				duemoney6=request("duemoney6")
				if duepaydate1<>"" and duepaydate2<>"" then
					m1=duepaydate1
					m2=duepaydate2
				end if
				if m1="" then
...
'if m2="" then
				...
				if m1<>"" then
					str_Result=str_Result+ " and e.date1>='"&m1&"' "
'if m1<>"" then
				end if
				if m2<>"" then
					str_Result=str_Result+ " and e.date1<='"&m2&"' "
'if m2<>"" then
				end if
				if A3="" or IsNull(A3) then
					A3=0
				end if
				if bz="" or IsNull(bz) then
					bz=0
				end if
				if A3<>"0" then
					str_Result=str_Result+" and s.id="&A3&" "
'if A3<>"0" then
				end if
				...
				W4=replace(W3,"0","")
				W4=replace(W4,",","")
				if W4<>"" then    str_Result=str_Result+"and addcate in ("&W3&")"
				W4=replace(W4,",","")
				if C<>"" then
					if B="shmc" then
...
'elseif B="shid" then
					elseif B="cpmc" then
						str_Result=str_Result+" and product in (select ord from product where title like '%"& C &"%')"
'elseif B="cpmc" then
					elseif B="khmc" then
...
'elseif B="zfzr" then
				...
				if FK<>"" then Str_Result=Str_Result+" and  sort2 in  ("&FK&") "
'elseif B="zfzr" then
				if FK1>"" then Str_Result=Str_Result+" and  p.id in  ("&FK1&") "
'elseif B="zfzr" then
				if duemoney1<>"" and duemoney2<>"" then
					str_Result=str_Result+ " and money1 between "&duemoney1&" and "&duemoney2&" "
'if duemoney1<>"" and duemoney2<>"" then
				end if
				if duemoney3<>"" and duemoney4<>"" then
					str_Result=str_Result+ " and money2 between "&duemoney3&" and "&duemoney4&" "
'if duemoney3<>"" and duemoney4<>"" then
				end if
				if duemoney5<>"" and duemoney6<>"" then
					str_Result=str_Result+ " and money3 between "&duemoney5&" and "&duemoney6&" "
'if duemoney5<>"" and duemoney6<>"" then
				end if
				if bz<>"0" and bz<>"" then
					str_Result=str_Result+" and s.id="&bz&" "
'if bz<>"0" and bz<>"" then
				end if
				if khmc<>"" then
					str_Result=str_Result+" and company in(select ord from tel where name like '%"&khmc&"%') "
'if khmc<>"" then
				end if
				if shid<>"" then
					str_Result=str_Result+" and shid like '%"&shid&"%' "
'if shid<>"" then
				end if
				if shmc<>"" then
					str_Result=str_Result+" and title like '%"&shmc&"%' "
'if shmc<>"" then
				end if
				If cpmc<>"" Then
					str_Result=str_Result+" and product in (select ord from product where title like '%"& cpmc &"%')"
'If cpmc<>"" Then
				end if
				if D<>"" then Str_Result=Str_Result+" and  sort in  ("&D&") "
'If cpmc<>"" Then
				if E<>"" then Str_Result=Str_Result+" and  way1 in  ("&E&") "
'If cpmc<>"" Then
				area_list=-1
'If cpmc<>"" Then
				if A2<>"" then
	...
	if A2<>"" then Str_Result=Str_Result+" and company in (select ord from tel where area in ("&area_list&")) "
	set rsarea=nothing
	...
	elseif px=3 then
		px_Result="order by sort1 desc,ord desc"
	elseif px=4 then
		px_Result="order by sort1 asc,ord asc"
	elseif px=5 then
		px_Result="order by money1 desc,ord desc"
	elseif px=6 then
		px_Result="order by money1 asc,ord asc"
	elseif px=7 then
		px_Result="order by money2 desc,ord desc"
	elseif px=8 then
		px_Result="order by money2 asc,ord asc"
	elseif px=9 then
		px_Result="order by money3 desc,ord desc"
	elseif px=10 then
		px_Result="order by money3 asc,ord asc"
	end if
	Set xApp = server.createobject(ZBRLibDLLNameSN & ".HtmlExcelApplication")
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("售后费用明细表")
	Response.Flush
	zdyStr_Result=""
	strZdySQL=""
	strZdy=""
	...
	strJs2=""
	Dim ArryExport()
	Set zdyrs=conn.execute("Select id,FName,FType,MustFillin,canExport from ERP_CustomFields where TName=88 and IsUsing=1 and del=1 order by FOrder asc")
	While Not zdyrs.eof
		ReDim Preserve ArryExport(intZ)
		strZdySQL =strZdySQL & ",(select top 1 FValue from ERP_CustomValues where FieldsID="&zdyrs("id")&" and OrderID=t.ord)  as zdy"&intZ &" "
		intExport=zdyrs("canExport")
		ArryExport(intZ)=intExport
		If intExport Then strZdy=strZdy & "<td><div align='center'>"& zdyrs("FName") &"</div></td>"
		strHTML=strHTML & "<tr onMouseOut=this.style.backgroundColor='' onMouseOver=this.style.backgroundColor='efefef'><td><div align='right'>"& zdyrs("FName") &"</div></td><td>"
		intFType=zdyrs("FType")
		Select Case intFType
		Case 1:
		strJs=request("zdy"&intZ)
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" like '%"& strJs &"%' "
		strHTML=strHTML & "<input name='zdy"&intZ&"' type='text' size='15' id='zdy"&intZ&"' value='"&strJs&"' dataType='Limit' max='500'  msg='长度不能超过500个字'>"
		Case 2:
		strJs=request("zdy"&intZ)
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" like '%"& strJs &"%' "
		strHTML=strHTML & "<input name='zdy"&intZ&"' type='text' size='15' id='zdy"&intZ&"' value='"&strJs&"' dataType='Limit' max='500'  msg='长度不能超过500个字'>"
		Case 3:
		strJs=request("zdy"&intZ&"data1")
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" >= '"& strJs &"'"
		strJs2=request("zdy"&intZ&"data2")
		If strJs2<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" <= '"& strJs2 &"'"
		strHTML=strHTML & "<INPUT name='zdy"&intZ&"data1' readonly=true value='"&strJs&"' size=9  id=duepaydate1divPos onmouseup=toggleDatePicker('duepaydate1div"&intZ&"','date.zdy"&intZ&"data1')><DIV id='duepaydate1div"&intZ&"' style='POSITION: absolute;z-index:10' name ='duepaydate1div"&intZ&"'></DIV> -  <INPUT name='zdy"&intZ&"data2' readonly=true value='"&strJs2&"' size=9  id=duepaydate1divPos onmouseup=toggleDatePicker('duepaydate2div"&intZ&"','date.zdy"&intZ&"data2')><DIV id='duepaydate2div"&intZ&"' style='POSITION: absolute;z-index:10' name ='duepaydate2div"&intZ&"'></DIV>"
		If strJs2<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" <= '"& strJs2 &"'"
		Case 4:
		strJs=request("zdy"&intZ&"num1")
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" >= '"& strJs &"'"
		strJs2=request("zdy"&intZ&"num2")
		If strJs2<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" <= '"& strJs2 &"'"
		strHTML=strHTML & "<input name='zdy"&intZ&"num1' type='text' size='15' id='zdy"&intZ&"num1' value='' onkeyup=value=value.replace(/[^\d\.]/g,'') dataType='Limit' max='500'  msg='长度不能超过500个字'>  -  <input name='zdy"&intZ&"num2' type='text' size='15' id='zdy"&intZ&"num2' value='' onkeyup=value=value.replace(/[^\d\.]/g,'') dataType='Limit' max='500'  msg='长度不能超过500个字'>"
		Case 5:
		strJs=request("zdy"&intZ)
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" like '%"& strJs &"%' "
		strHTML=strHTML & "<input name='zdy"&intZ&"' type='text' size='15' id='zdy"&intZ&"' value='' dataType='Limit' max='500'  msg='长度不能超过500个字'>"
		Case 6:
		strJs=request("zdy"&intZ)
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&" ='"& strJs &"' "
		strHTML=strHTML & "<select name='zdy"&intZ&"' id='zdy"&intZ&"' dataType='Limit'><option value='' selected></option><option value='是'>是</option><option value='否'>否</option></select>"
		Case Else
		strJs=request("zdy"&intZ)
		If strJs<>"" Then zdyStr_Result=zdyStr_Result &" and zdy"&intZ&"='"& strJs &"' "
		strHTML=strHTML & "<select name='zdy"&intZ&"' id='zdy"&intZ&"' dataType='Limit'><option value='' selected></option>"
		sql7="select CValue from ERP_CustomOptions where CFID="&zdyrs("id")&" order by id asc "
		Set rs7=conn.execute(sql7)
		do until rs7.eof
			strHTML=strHTML & "<option value='"&rs7("CValue")&"'>"&rs7("CValue")&"</option>"
			rs7.movenext
		loop
		rs7.close
	...
	wend
	zdyrs.close
	Chtml="<table width='2000' border='0' cellpadding='6' cellspacing='1' id='content'><tr class='top'><td height='26' ><div align='center'>第一级分类</div></td><td><div align='center'>第二级分类</div></td><td><div align='center'>分类明细</div></td><td height='26' ><div align='center'>产品名称</div></td><td height='26' ><div align='center'>售后名称</div></td><td height='26' ><div align='center'>售后编号</div></td><td height='26' ><div align='center'>售后分类</div></td><td height='26' ><div align='center'>售后方式</div></td>"& strZdy &"<td height='26' ><div align='center'>主负责人</div></td><td height='26' ><div align='center'>币种</div></td><td height='26' ><div align='center'>费用申请</div></td><td height='26' ><div align='center'>费用使用</div></td><td height='26' ><div align='center'>费用报销</div></td></tr>"
	xsheet.writeHTML Chtml
	dim summoney1,summoney2,summoney3,summoney4,money1_all,money2_all,money3_all,money4_all
	summoney1=0
	summoney2=0
	summoney3=0
	money1_all=0
	...
	n=0
	sql="select * from ("&_
	"select t.ord,t.title,t.shid,t.company,t.addcate,t.sort1 as sort,t.way1, t.product ,(select title from product where del=1 and ord=isnull(t.product,0)) as productName,"&_
	"s.id as sortbzid,s.sort1 as sortbzname, "&_
	"p.id,p.sort1,p.sort2,(select sort1 from sortonehy where ord=p.sort2) as sortname, "&_
	"sum(isnull(e.money1,0)) as money1, "&_
	"sum(isnull(e.money2,0)) as money2, "&_
	"sum(isnull(e.money3,0)) as money3 "&_
	"strZdySQL" &_
	"from "&_
	"( "&_
	"select q.shouhou,q.sort1 as sort,q.money1,0 as money2,0 as money3,y.bz,y.sp_date as date1,y.cateid from paysqlist q inner join paysq y on y.id=q.fid where q.del=1 "&_
	"union all "&_
	"select a.shouhou,a.sort,0 as money1 ,a.money1 as money2,0 as money3,f.bz,f.datejk as date1,f.sorce2 as cateid from pay a inner join f_pay f on f.id=a.fid where a.del=1 and a.complete=3 "&_
	"union all "&_
	"select c.shouhou,b.sort,0 as money1,0 as money2,b.money1 as money3,d.bz,d.bxdate as date1,c.cateid from paybxlist b inner join pay c on c.ord=b.payid inner join  paybx d on d.id=b.bxid where b.del=1 "&_
	") e "&_
	"inner join tousu  t on t.ord=e.shouhou "&_
	"inner join paytype p on p.id=e.sort "&_
	"inner join sortbz s on s.id=e.bz "&_
	" "&Str_Result&" "&_
	"group by t.ord,t.title,t.shid,t.company,t.addcate,t.sort1,t.way1,t.product,s.id ,s.sort1,p.id,p.sort1,p.sort2 "&_
	"having sum(isnull(e.money1,0))>0 or sum(isnull(e.money2,0))>0 or sum(isnull(e.money3,0))>0 " &_
	") x where 1=1 "& zdyStr_Result &" " & px_Result &""
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	if rs.RecordCount<=0 then
		xsheet.writeHTML  "<table><tr><td>没有信息!</td></tr><tr><td height='1300'></td></tr></table>"
	else
		n=0
		rowIndex=0
		do until rs.eof
			...
			money3=zbcdbl(rs("money3"))
			if rs("money3")="" or IsNull(zbcdbl(rs("money3"))) then
				money3=0
			else
				money3=zbcdbl(rs("money3"))
			end if
			if rs("addcate")<>"" and rs("addcate")<>0 and (not isnull(rs("addcate"))) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			sort=rs("sort")
			if sort="" or isnull(sort) then sort=0
			set rs7=server.CreateObject("adodb.recordset")
			sql7="select * from sortonehy where ord="&sort&""
			rs7.open sql7,conn,1,1
			if rs7.eof then
				so=""
			else
			...
			if rs("way1")<>0 then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1  from sortonehy Where ord="&rs("way1")&" "
				rs7.open sql7,conn,1,1
				if not rs7.eof then
					companyname=rs7("sort1")
				else
					companyname=""
				end if
			end if
			...
			companyname=""
			xsheet.writeHTML  "<tr><td align='center'>"&rs("sortname")&"</td><td align='center' class='red'>"&rs("sort1")&"</td>"
			xsheet.writeHTML  "<td>"
			strHTML_mx=""
			Set rss= conn.execute("select * from pay where shouhou="& rs("ord") &" and sort="&rs("id")&"  order by sort asc")
			If Not rss.eof Then
				While Not rss.eof
					payid=rss("payid")
					num=zbcdbl(rss("num"))
					...
					intro=rss("intro")
					startime=rss("startime")
					endtime=rss("endtime")
					staraddr=rss("staraddr")
					endaddr=rss("endaddr")
					lc=rss("lc")
					bus=rss("bus")
					retime=rss("retime")
					startime1=rss("startime1")
					endtime1=rss("endtime1")
					city=rss("city")
					hotel=rss("hotel")
					ggtime=rss("ggtime")
					ggcate=rss("ggcate")
					...
					yt=rss("yt")
					set rs1=server.CreateObject("adodb.recordset")
					sql="select * from zdymx where sort1=(select sid from paytype where id="&rs("id")&") and (sort=1 or (sort=2 and set_open=1)) and (name not in ('tel','person','contract','chance','caigou','richeng','shouhou','fahuo','iwork','jkid','scdd','zdww','gxww','scsb')) order by gate1 asc"
					...
					If Not rs1.eof Then
						do while not rs1.eof
							...
							Select Case name
							Case "bh" :
							If payid&""<>"" Then strHTML_mx=strHTML_mx & rs1("title") &"： "& payid &"<br>"
							Case "num" :
							If num&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& num &"<br>"
							Case "money1" :
							If money1&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& Formatnumber(money1,num_dot_xs,-1) &"<br>"
'Case "money1" :
							Case "intro" :
							If intro&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& intro &"<br>"
							Case "startime" :
							If startime&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& startime &"<br>"
							Case "endtime" :
							If endtime&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& endtime &"<br>"
							Case "smdd" :
							If staraddr&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& staraddr &"<br>"
							Case "mdd" :
							If endaddr&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& endaddr &"<br>"
							Case "lic" :
							If lc&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& lc &"<br>"
							Case "jtgj" :
							If bus&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& bus &"<br>"
							Case "retime" :
							If retime&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& retime &"<br>"
							Case "startime1" :
							If startime1&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& startime1 &"<br>"
							Case "endtime1" :
							If endtime1&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& endtime1 &"<br>"
							Case "city" :
							If city&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& city &"<br>"
							Case "hatol" :
							If hotel&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& hotel &"<br>"
							Case "ggdate" :
							If ggtime&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& ggtime &"<br>"
							Case "ggdx" :
							If ggcate&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& ggcate &"<br>"
							Case "ggsy" :
							If ggintro&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& ggintro &"<br>"
							Case "lw" :
							If gglw&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& gglw &"<br>"
							Case "xytime" :
							If xytime&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& xytime &"<br>"
							Case "yt" :
							If yt&""<>"" Then strHTML_mx=strHTML_mx &  rs1("title") &"： "& yt &"<br>"
							End Select
							rs1.movenext
						Loop
					end if
					...
					Set rs1=Nothing
					rss.movenext
					If Not rss.eof Then strHTML_mx=strHTML_mx & "<br>"
				wend
			end if
			rss.close
			xsheet.writeHTML  strHTML_mx&"</td>"
			xsheet.writeHTML  "<td align='center'>"
			If (open_21_1=1 Or open_21_1=3) And rs("productName")&""<>"" Then xsheet.writeHTML  rs("productName")
			xsheet.writeHTML  "</td><td align='center' class='red'>"&rs("title")&"</td><td align='center'>"&rs("shid")&"</td><td align='center' class='red'>"&so&"</td><td align='center'>"&companyname&"</td>"
			m=0
			For k=0 To intZ-1
				m=0
				If ArryExport(k) Then
					xsheet.writeHTML "<td align='center' class='name'>"& rs("zdy"&k) &"</td>"
					m=m+1
				end if
			next
			xsheet.writeHTML  "<td align='center' class='red'>"&sort67&"</td><td align='center'>"&rs("sortbzname")&"</td><td align='center' class='red'>"&Formatnumber(money1,num_dot_xs,-1)&"</td><td align='center'>"&Formatnumber(money2,num_dot_xs,-1)&"</td><td align='center' width='5%'>"&Formatnumber(money3,num_dot_xs,-1)&"</td></tr>"
			...
			If rowIndex > 40000 Then
				xsheet.writeHTML  "<tr><td colspan='"& 10+m &"'><div align='right'>合计：</div></td><td  align='center'>"&Formatnumber(summoney1,num_dot_xs,-1)&"</td><td  align='center'>"&Formatnumber(summoney2,num_dot_xs,-1)&"</td><td  align='center'>"&Formatnumber(summoney3,num_dot_xs,-1)&"</td></tr>"
'If rowIndex > 40000 Then
				summoney1=0
				summoney2=0
				summoney3=0
				...
				If pageCount = 1 then
					xsheet.title = "售后费用明细表(" & pageCount & "页)"
				end if
				pageCount = pageCount + 1
				xsheet.title = "售后费用明细表(" & pageCount & "页)"
				Set xsheet = xApp.sheets.add("售后费用明细表(" & pageCount & "页)")
				rowIndex = 0
				xsheet.writeHTML Chtml
			end if
			rs.movenext
...
'Loop
	...
	set rs=Nothing
	xsheet.writeHTML  "</table>"
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>售后费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!售后费用明细表导出成功,共有"&(n)&"条记录!</B>"";</script>"
	tfile=Server.MapPath("售后费用明细表_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'售后费用明细表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	Set xApp = Nothing
	action1="售后费用明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的售后费用明细表</u></strong></font></a></p>"
