...
'bigsystemtype = ".mozi"
	...
	dim rs,sql,Str_Result,Str_Result2,catesafe,sorce_user,sorce_user2,Str_power,Str_power2,Str_power22,Str_power3
	...
	set rs=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from power1 where ord=13 "
	rs.open sql,conn,1,1
	dim power1
	power1=rs("gate1")
	rs.close
	set rs=nothing
	if power1="0" then
		if session("see1zbintel2007")="1" then
			if catesafe="1"  then
				Str_Result=Str_Result
				Str_Result2=Str_Result2
			elseif catesafe="2"  then
				Str_Result=Str_Result+"and cateid2="&sorce_user&""
'elseif catesafe="2"  then
				Str_Result2=Str_Result2+"and cateid2="&sorce_user&""
'elseif catesafe="2"  then
				Str_power="where ord="&sorce_user&" "
			elseif catesafe="3"  then
				Str_Result=Str_Result+"and cateid3="&sorce_user2&""
'elseif catesafe="3"  then
				Str_Result2=Str_Result2+"and cateid3="&sorce_user2&""
'elseif catesafe="3"  then
				Str_power=" where ord="&sorce_user&""
				Str_power2="ord="&sorce_user2&" and "
				Str_power22="where ord="&sorce_user2&""
			else
				Str_Result=Str_Result+"and cateid ="&session("personzbintel2007")&""
				Str_power22="where ord="&sorce_user2&""
				...
				Str_power2="ord="&sorce_user2&" and "
				Str_power22="where ord="&sorce_user2&""
				Str_power3="ord="&session("personzbintel2007")&" and "
			end if
		else
			Str_Result=Str_Result+"and cateid ="&session("personzbintel2007")&""
			Str_power3="ord="&session("personzbintel2007")&" and "
	...
	elseif power1="1" then
		if session("see1zbintel2007")="1" then
			...
			elseif catesafe="2"  then
				Str_power="where ord="&sorce_user&" "
			elseif catesafe="3" or catesafe="4"   then
				Str_Result=Str_Result+"and cateid3="&sorce_user2&""
			elseif catesafe="3" or catesafe="4"   then
				Str_Result2=Str_Result2+"and cateid3="&sorce_user2&""
			elseif catesafe="3" or catesafe="4"   then
			...
			Str_Result=Str_Result+"and ((cateid3="&sorce_user2&" and cateid3<>0) or cateid ="&session("personzbintel2007")&" )"
			Str_power22="where ord="&sorce_user2&""
			Str_Result2=Str_Result2+"and ((cateid3="&sorce_user2&" and cateid3<>0) or cateid ="&session("personzbintel2007")&" )"
			Str_power22="where ord="&sorce_user2&""
			Str_power=" where ord="&sorce_user&""
			Str_power2="ord="&sorce_user2&" and "
			Str_power22="where ord="&sorce_user2&""
		end if
	elseif power1="2" then
		if session("see1zbintel2007")="1" then
			if catesafe="1"  then
				Str_Result=Str_Result
				Str_Result2=Str_Result2
			elseif catesafe="2" or  catesafe="3" or catesafe="4"  then
				Str_Result=Str_Result+"and cateid2="&sorce_user&""
'elseif catesafe="2" or  catesafe="3" or catesafe="4"  then
				Str_Result2=Str_Result2+"and cateid2="&sorce_user&""
'elseif catesafe="2" or  catesafe="3" or catesafe="4"  then
			...
			Str_Result=Str_Result+"and ((cateid2="&sorce_user&" and cateid2<>0) or cateid ="&session("personzbintel2007")&" )"
			Str_power="where ord="&sorce_user&" "
			Str_Result2=Str_Result2+"and ((cateid2="&sorce_user&" and cateid2<>0) or cateid ="&session("personzbintel2007")&" )"
			Str_power="where ord="&sorce_user&" "
			Str_power="where ord="&sorce_user&" "
		end if
...
'Private Sub Class_Initialize
	...
	if m1<>"" then
		Str_Result1 = " and date2>='"&m1&"' "
	end if
	if m2<>"" then
		Str_Result1 = Str_Result1 & " and date2<='"&m2&"' "
	end if
	If searchtype<>"" And C<>"" Then
		If searchtype="0" Then
			Str_Result1 = Str_Result1 & " and caigou in(select ord from caigou where charindex('"&C&"',title)>0) "
		ElseIf searchtype="1" Then
			Str_Result1 = Str_Result1 & " and ord in(select ord from product where charindex('"&C&"',title)>0) "
		ElseIf searchtype="2" Then
			Str_Result1 = Str_Result1 & " and cateid in(select ord from gate where charindex('"&C&"',name)>0) "
		end if
	else
		searchtype="0"
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".ie5 input.anybutton.aa{margin-bottom:-0.5px;height:19px;line-height:16px;}" & vbcrlf & "</style>" & vbcrlf & "" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<form method=""get"" action=""planlist.asp"" name=""form2"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf &       "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf &               "<tr>" & vbcrlf &             "<td class=""place"">采购到货提醒</td>" & vbcrlf &             "<td>&nbsp;</td>" & vbcrlf &             "<tdalign=""right""><div align=""right"">" & vbcrlf & "" & vbcrlf & "            <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "       <option value=""page_count=10"" "
	'Response.write Application("sys.info.jsver")
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "           </select>" & vbcrlf & "                   " & vbcrlf & "                        " & vbcrlf & "                        </div>" & vbcrlf & "             </tdalign=>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td background=""../images/112.gif"" height=""30"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "          " & vbcrlf & "                                "
	'Response.write "selected"
	...
	Response.write ">关联采购</option>" & vbcrlf & "                              <option value=""1"" "
	...
	Response.write ">负责人</option>" & vbcrlf & "                              </select>" & vbcrlf & "                               <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "        <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton aa""/>" & vbcrlf & "              " & vbcrlf & "          </td>" & vbcrlf & "        </tr>" & vbcrlf & "             </form>" & vbcrlf & "      </table>" & vbcrlf & "    " & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "  <form method=""post"" action=""setalt2.asp?CurrPage="
	...
	Response.write "&ret2="
	Response.write m2
	Response.write "&types=batch"" name=""form1"">" & vbcrlf & " <tr class=""top"">" & vbcrlf & "    <td align=""center"" width=""5%""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "    <td width=""12%"" height=""24"" align=""center""><div align=""center""><strong>到货日期</strong></div></td>" & vbcrlf & "       <td height=""27"" ><div align=""center"">产品名称</div></td>" & vbcrlf & "    <td width=""11%"" ><div align=""center""><strong>数量</strong></div></td>" & vbcrlf & "       <td width=""28%"" ><div align=""center"">关联采购</div></td>" & vbcrlf & "    <td ><div align=""center""><strong>负责人</strong></div></td>" & vbcrlf & "     <td width=""13%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	if open_22_1 = 3 then
		sql="where del =1 and alt=1 " & Str_Result1 & " "
	else
		if intro_22_1&"" = "" then
			intro_22_1=0
		end if
		if open_22_1=1 then
			sql="where del=1 and  alt=1 and cateid in ("&intro_22_1&") " & Str_Result1 & " "
		else
			sql="where 1=0"
		end if
	end if
	Dim helper
	Set helper = CreateReminderHelper(conn,9,0)
	conn.execute("select * into #ReminderTemp from (" & helper.listSQL("ids") & ") t ")
	if b="3333333" or b="5555555" or b="cylm" then
		...
		if newstate="1" then
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where ord=9"
		else
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where ord=9"
		end if
		sql9=sql9 & " and cateid=" & session("personzbintel2007")&""
			...
			if rs9("fw1")="0" then
				if open_22_1=3 then
					if link="yes" and uid<>"" then
						Str_Result_9 = " and cateid="&uid
						str= " and b.cateid="&uid
					else
						Str_Result_9 = ""
					end if
				elseif open_22_1=1 then
					if link="yes" and uid<>"" then
						Str_Result_9 = " and cateid in ("&uid&")"
					else
						Str_Result_9 = " and cateid in ("&intro_22_1&")"
					end if
				else
					Str_Result_9 = " and cateid=0"
				end if
			else
				if link="yes" and uid<>"" then
					Str_Result_9 = " and cateid="&uid&" "
				else
					Str_Result_9 = " and cateid="&session("personzbintel2007")&""
				end if
			end if
			tq9=rs9("tq1")
		end if
		...
		set rs9=nothing
		If b="5555555" Then
			sql=" where "&Str_Result_9&" and alt=1 and del=1 and id in (select id from #ReminderTemp)" &noalt1&" " & Str_Result1 & " "
		else
			sql=" where alt=1 and date2<='"&dateadd("d",tq9,date)&"' "&Str_Result_9&"  and del=1 "&noalt1&" " & Str_Result1 & " "
		end if
	end if
	sql = " where id in (select id from #ReminderTemp)" & Str_Result1
	dim actinon1
	action1="采购到货提醒"
	dim n
	n=0
	sqlcount="select count(*) from caigoulist b "&sql &str
...
'if currpage=pagecount then
	...
	pStart=page_count*(CurrPage-1)
	intcount=page_count
	pEnd=page_count*CurrPage
	conn.execute("select cast(id as int)id,IDENTITY(int,1,1) inx into #temp from caigoulist  where id in (select id from #ReminderTemp) order by date2 desc")
	sql="select top "&page_count&" a.*,b.cateid as fzrID from caigoulist a inner join caigou b on b.ord=a.caigou "&_
	"   INNER JOIN #temp TP ON TP.id=a.id and  inx >"&pStart&" and inx <= "&pEnd
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Dim rId
		do until rs.eof
			dim k,ord,id,zt1,p_cateid,c_cateid
			...
			id=rs("ord")
			set rs7=server.CreateObject("adodb.recordset")
			sql7="select title,isnull(addcate,-1) as cateids from product where ord="&rs("ord")&""
'set rs7=server.CreateObject("adodb.recordset")
			rs7.open sql7,conn,1,1
			if rs7.eof then
				k10=""
				...
				k10=rs7("title")
				p_cateid=rs7("cateids")
			end if
			rs7.close
			set rs7=nothing
			set rs7=server.CreateObject("adodb.recordset")
			sql7="select title,isnull(cateid,-1) as cateids from caigou where ord="&rs("caigou")&""
'set rs7=server.CreateObject("adodb.recordset")
			rs7.open sql7,conn,1,1
			if rs7.eof then
				caigou=""
				...
				caigou=rs7("title")
				c_cateid=rs7("cateids")
			end if
			rs7.close
			set rs7=nothing
			if  b="3333333" and timelg<>"" and datediff("s",timelg,rs("date7"))>0 then
				k10=k10&new_list
			end if
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "           <td align=""center"">" & vbcrlf & "                       <span class=""red"">" & vbcrlf & "                                <input name=""selectid"" type=""checkbox"" class=""reminderIds"" rid="""
			Response.write rId
			Response.write """ id=""selectid"" value="""
			Response.write rs("id")
			Response.write """>" & vbcrlf & "                        </span>" & vbcrlf & "         </td>" & vbcrlf & "     <td height=""24"" align=""center"">"
			Response.write rs("date2")
			Response.write "</td> " & vbcrlf & "             <td width=""25%"" height=""27"" >" & vbcrlf & "           "
			If open_21_1=3 Or (open_22_1=1 And CheckPower2010(intro_21_1,p_cateid)=true) then
				If open_21_14=1 then
					Response.write "" & vbcrlf & "                                     <a href=""#"" onclick=""javascript:window.open('../product/content.asp?ord="
					Response.write pwurl(rs("ord"))
					Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & vbcrlf & "                                        "
					Response.write pwurl(rs("ord"))
					Response.write""&k10&""
			...
			Response.write "</div>" & vbcrlf & "                <div align=""center"">" & vbcrlf & "                 </div></td>" & vbcrlf & "        <td >" & vbcrlf & "                    "
			If open_22_14=3 Or (open_22_14=1 And CheckPower2010(intro_22_14,c_cateid)=true) then
				Response.write "" & vbcrlf & "                                     <a href=""#"" onclick=""javascript:window.open('../../SYSN/view/store/caigou/caigoudetails.ashx?view=details&ord="
				Response.write pwurl(rs("caigou"))
			...
			set rs7=server.CreateObject("adodb.recordset")
			sql7="select name from gate where ord="&rs("fzrID")&""
			rs7.open sql7,conn,1,1
			dim cateid
			...
			Response.write "</div></td>" & vbcrlf & "             <td> " & vbcrlf & "                             <div align=""center""> " & vbcrlf & "                                   &nbsp;<input type=""button"" rid="""
			Response.write rId
			Response.write """ name=""Submitdel"" value=""取消提醒"" class=""anybutton cancelBtn""/>" & vbcrlf & "                 </div>" & vbcrlf & "                   </td>" & vbcrlf & "        </tr>" & vbcrlf & "    "
			n=n+1
			rs.movenext
		loop
		m=n
		Response.write "       " & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "          </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "  <td width=""7%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""checkbox2"" id=""checkbox2"" value=""Check All"" onClick=""mm(this.form)"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td width=""22%"" ><input type=""submit"" onClick=""return test();"" name=""Submit2"" value=""批量取消""  class=""anybutton2""/>" & vbcrlf & "       </td>" & vbcrlf & "    <td width=""71%""><div align=""right"">" & vbcrlf & "    "
		Response.write recCount
		Response.write "个 | "
		Response.write currpage
		Response.write "/"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='planlist.asp?currPage=1&page_count="
			Response.write page_count
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write newstate
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='planlist.asp?currPage="
		...
		if currpage=PageCount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='planlist.asp?currPage="
			Response.write  currpage + 1
			Response.write "&page_count="
			Response.write page_count
			Response.write "&a="
			...
			Response.write "&newstate="
			Response.write newstate
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='planlist.asp?currPage="
			Response.write  PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script src= ""../Script/cu_planlist.js?ver="
		Response.write Application("sys.info.jsver")
	...
	set rs=nothing
	call close_list(1)
	Response.write "   " & vbcrlf & "      <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "              <tr> " & vbcrlf & "                <td height=""10"" colspan=""2""></td>" & vbcrlf & "              </tr> " & vbcrlf & "                   <tr>" & vbcrlf & "                      <td width=""16%"" height=""10""><div align=""right""></div></td>" & vbcrlf & "              <td width=""84%"">&nbsp;</td>" & vbcrlf & "       </tr>" & vbcrlf & "                         <tr>" & vbcrlf & "                      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "     </tr>" & vbcrlf & "                         <tr>" & vbcrlf & "                      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script>" & vbcrlf & "jQuery(function(){" & vbcrlf & "  BindCancelEvents({cfgId:9});" & vbcrlf & "});" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
