...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>直接出账单导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf &"    <td class=""place"">直接出账单导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%""border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出直接出账单,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" id=""CountTXT_tb"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write Application("sys.info.jsver")
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(4).ColumnWidth=22
			...
			if D<>"0" then
				Str_Result=Str_Result+" and ck in (select id from sortbank where del=1 and bz in (select id from sortbz where id="&D&")) "
'if D<>"0" then
			end if
			if C<>"" then
				if B="rzzt" then
...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from gate where ord=bankout.cateid) as cateidname" & _
			",bank=(select top 1 sort1 from sortbank where id=bankout.ck and ','+replace(convert(char,person),' ','')+',' like '%,"&session("personzbintel2007")&",%')" & _
			"from bankout "&Str_Result&"" &px_Result&""
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "直接出账单_"&session("name2006chen"),C1
			dim i
			i=1
			do until rs.eof
				...
				if ck<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select intro from sortbz where id in (select bz from sortbank where id="&ck&") "
					rs7.open sql7,conn,1,1
					if rs7.eof then
						sortbz=""
					else
				...
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,4).Value = bank
				money1=sortbz&money1
				...
				money1=sortbz&money1
				xlWorksheet.Cells(1+i,6).Value = replacehtml(intro)
				money1=sortbz&money1
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=document.all.CountTXT_tb.offsetWidth;CountTXT.innerHTML=""<font color=red><b>直接出账单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!直接出账单导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("直接出账单_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的直接出账单</u></strong></font></a></p>"
