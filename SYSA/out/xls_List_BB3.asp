...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	"(SELECT d_assid,d_money from O_assdeprect WHERE D_del=2) a "&_
	...
	C1=rs.recordcount
	xlApplication.Init "已提折旧汇总_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
...
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).Value =rs("ass_ycycle")
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money"))),num_dot_xs,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).Value =Formatnumber(ml.IsNum(zbcdbl(rs("allmoney"))),num_dot_xs,-1)
'if not rs.eof then
...
'if not rs.eof then
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>已提折旧汇总导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!已提折旧汇总导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("已提折旧汇总_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的已提折旧汇总</u></strong></font></a></p>"
