...
if px="" or instr(1,px,"-",1)<=0 then px="7-0"
'if CurrPage="" then CurrPage=1
...
if newstate="1" then
set rs=server.CreateObject("adodb.recordset")
sql="select isnull(intro,1) as intro,isnull(fw1,1) as fw1,isnull(tq1,0) as tq1 from setjm2  where cateid="&session("personzbintel2007")&" and ord=49"
...
set rs1=server.CreateObject("adodb.recordset")
sql1="select isnull(intro,0) as intro,isnull(fw1,1) as fw1,isnull(tq1,0) as tq1 from setjm  where cateid="&session("personzbintel2007")&" and ord=49"
rs1.open sql1,conn,1,1
if not rs1.eof then
Isopen=rs1("intro")'
fw1=rs1("fw1")'
...
If tq1="" or Isnull(tq1) Then tq1=0
SQL_SetJm=" "
If newstate="1" Or request("propm")=1 Then
If Isopen=1 Then
If fw1=1  Then
SQL_SetJm="And 序号_ID="&session("personzbintel2007")&" and datediff(d,'"&now&"',本次体检日期)<="&tq1&""
else
SQL_SetJm=" and datediff(d,'"&now&"',本次体检日期)<="&tq1&""
end if
end if
end if
If open_82_1=1 Then
SQL_SetJm = SQL_SetJm & "And 序号_ID in("&intro_82_1&") "
ElseIf open_82_1=0 Then
SQL_SetJm=" and 1=2"
end if
baseSQL="select PAGE_COUNT_NUM from (select PAGE_TOP_NUM * from ("&_
"select *  from (select a.personID as 序号_ID,"&_
"   b.username as 员工姓名,"&_
"   b.userbh as 员工编号,"&_
"   dbo.hrGetPostion_sorceName(b.PostionID) as 职位,"&_
"a.lastdate as 上次体检日期,"&_
"   cast(a.zhouqi as varchar(50))+(case a.unit when 1 then '年' "&_
"   when 2 then '季' "&_
"   when 3 then '月' "&_
"   when 4 then '周' "&_
"   when 5 then '日'"&_
"   else null end) as 周期,"&_
"   (case a.unit when 1 then dateadd(yyyy,a.zhouqi,a.lastdate) "&_
"   when 2 then dateadd(qq,a.zhouqi,a.lastdate) "&_
"   when 3 then dateadd(m,a.zhouqi,a.lastdate) "&_
"   when 4 then dateadd(ww,a.zhouqi,a.lastdate) "&_
"   when 5 then dateadd(d,a.zhouqi,a.lastdate)"&_
"   else null end) as 本次体检日期,cast(a.personID as int) as REC_ID "&_
"    from hr_person_health a inner join hr_person b on b.userID=a.personID where  b.del=0 and b.nowstatus not in (3,4) and a.lastdate is not null and a.zhouqi is not null "
If newstate="1" Or request("propm")=1 Then
baseSQL=baseSQL&" and isnull(a.alt,1)<>2"
else
Set helper = CreateReminderHelper(conn,49,0)
baseSQL=baseSQL&" AND a.id IN (" & helper.listSQL("ids_withoutOrderBy") & ")"
end if
baseSQL=baseSQL&") c where  1=1 "& SQL_SetJm&""&_
") aaaavv PAGE_ORDER_STR) bbb  where 1=1"
...
.digits=NumDigits '
.title="待体检档案列表" '
.FieldsSettingIndex=80012 '
.PageSize=clng(page_count) '
...
sub CurstomButtons()
Response.write "" & vbcrlf & "<style>" & vbcrlf & ".IE5 .ser_top input.anybutton {height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<select name=""page_count"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl('page_count='+this.value);}"">" & vbcrlf & "<option>-请选择-</option>" & vbcrlf & "<option value=""10"" "
'sub CurstomButtons()
...
Response.write px
Response.write """>" & vbcrlf & "<input type=""hidden"" name=""sflg"" value=""1""/>" & vbcrlf & "<!--<input type=""submit"" value=""查看"" class=""anybutton""/>-->" & vbcrlf & "<!--<input type=""button"" class=""anybutton"" value=""高级检索"" onClick=""open1();""/>-->"
'Response.write px
if open_82_10<>0 then
