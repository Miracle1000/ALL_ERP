...
'bigsystemtype = ".mozi"
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write "' defer='defer'></script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<form name=""form1"" method=""post"" action=""../car/Del_carData.asp?realDel=1&CurrPage="
	Response.write CurrPage
	Response.write """>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "            <td width=""100%"" valign=""top"">" & vbcrlf & "                      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "<tr>" & vbcrlf &                                  "   <td class=""place"">车辆档案回收站</td>" & vbcrlf &                            "          <td>&nbsp;</td> "& vbcrlf &                          "        <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='../car/Del_carData.asp?ord=-634&realDel=1&CurrPage="
	Response.write CurrPage
	Response.write CurrPage
	Response.write "'}""/></td>" & vbcrlf & "                                 <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                              </tr>" & vbcrlf & "                   </table> " & vbcrlf & "                       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf& "                                <tr class=""top"">" & vbcrlf & "                                  <td align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "                                     <td align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "                                   <td height=""27"" ><div align=""center""><strong>车牌号</strong></div></td>" & vbcrlf & "                                      <td ><div align=""center""><strong>车辆名称</strong></div></td>" & vbcrlf & "                                     <td ><div align=""center""><strong>添加人</strong></div></td>" & vbcrlf & "                                       <td width=""14%"" ><div align=""center""><strong>删除人</strong></div></td>" & vbcrlf & "                                     <td ><div align=""center"">删除时间</div></td>" & vbcrlf & "                                 <td width=""20%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "                               </tr>" & vbcrlf & ""
	dim n
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.*,isnull(g1.name,'') addName,isnull(g2.name,'') delName,car_addcateid, " & vbcrlf &_
	"case when not exists (select top 1 1 from O_carUsemx where use_carid=a.car_id) " & vbcrlf &_
	"and not exists (select top 1 1 from O_insure where iss_carid=a.car_id) " & vbcrlf &_
	"and not exists (select top 1 1 from O_repair where rep_carid=a.car_id)" & vbcrlf &_
	"then 1 else 0 end candel, " & vbcrlf &_
	"case when exists(" & vbcrlf &_
	"select top 1 1 from O_carData b where b.del=1 and b.car_id<>a.car_id " & vbcrlf &_
	"and (b.car_code=a.car_code or (len(isnull(a.car_fdjh,''))>0 and b.car_fdjh=a.car_fdjh))" & vbcrlf &_
	") then 0 else 1 end canResume " & vbcrlf &_
	"from O_carData a "&_
	"left join gate g1 on g1.ord=a.car_addcateid " & vbcrlf &_
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			id=rs("car_id")
			car_addcateid = rs("car_addcateid")
			...
			Response.write id
			Response.write """>" & vbcrlf & "                                         </span>" & vbcrlf & "                                 </td> " & vbcrlf & "                                  <td width=""8%"" align=""center"" height=""24"">"
			Response.write Rs.recordcount-Rs.pagesize*(currpage-1)-n
			Response.write "</td>" & vbcrlf & "                                 <td width=""15%"" height=""27"" class=""name"" >" & vbcrlf & "                                            <div align=""center"">" & vbcrlf & ""
			if open_102_14=3 or CheckPurview(intro_102_14,trim(car_addcateid))=True Then
				Response.write "" & vbcrlf & "                                                      <a href=""javascript:void(0)"" onclick=""window.open('../car/Detail_carData.asp?ord="
				Response.write id
				Response.write "','carInsure','width=900,height=500,fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;"" title="""
				Response.write rs("car_code")
				Response.write """>"
				Response.write rs("car_code")
			...
			Response.write "" & vbcrlf & "                                              </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                                   <td width=""15%"" class=""name""><div align=""center"">"
			Response.write rs("car_Name")
			Response.write "</div></td> " & vbcrlf & "                                  <td width=""15%"" class=""name""><div align=""center"">"
			...
			Response.write " class=""anybutton"" value=""恢复"" onClick=""if(confirm('确认恢复？')){window.location.href='setCarData.asp?ord="
			Response.write id
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>" & vbcrlf & "                                                      <input type=""button"" name=""Submitdel"" "
			Response.write iif(rs("canDel") = 1,""," disabled")
			Response.write " class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='../car/Del_carData.asp?realDel=1&ord="
			Response.write id
			Response.write "&CurrPage="
			Response.write CurrPage
		...
		Response.write "" & vbcrlf & "                                             </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <script type=""text/Javascript"">window.currask2Url=""setCarData.asp?currPage="
		Response.write currPage
