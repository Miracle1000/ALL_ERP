...
'[CATECONDITION] [CANCELCONDITION] [ORDERBY]
			...
			" WHERE a.reminderConfig=" & configId & " [CATECONDITION] [CANCELCONDITION] [ORDERBY]"
			fields = "ISNULL(b.ord,0) [id],isnull(b.title,'【已删除数据】') title,isnull(convert(varchar(19),b.date7,21),'----') dt,"&_
			"datediff(s,'"&actDate&"',a.inDate) newTag"
			orderBy = "ORDER BY a.id DESC"
			Case 209:
...
'Private Sub Class_Initialize
	...
	dim intro,num1,gate,date1,time1,order1
	H=trim(Request("H"))
	intro=trim(Request.form("intro3"))
	date1=trim(Request("date1"))
	spid=getInt(request("spid"))
	cateid=getInt(request("cateid"))
	lcb=getInt(request("lcb"))
	divID=trim(Request("divID"))
	if time1="" then
		time1=hour(now())
	end if
	if time2="" then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from gate where ord="&cateid&" "
	...
	If rs.eof = False then
		...
		Response.write"<script language=javascript>alert('提交的数据有误！（附加数据：" & request("cateid") & "）');history.back()</script>"
		rs.close
		call db_close : Response.end
	end if
	rs.close
	sql="select * from gate where ord="&spid&" "
	rs.open sql,conn,1,1
	if not rs.eof then
		spname=rs("name")
	end if
	...
	set rs=nothing
	if intro = "" then
		Response.write"<script language=javascript>alert('请填写"&planContent&"！');history.back()</script>"
		call db_close : Response.end
	end if
	if message<> "" then
		call error(""&message&"")
	else
		sql = "select * from plan2 where 1=0"
		Set Rs = server.CreateObject("adodb.recordset")
		...
		set rs = nothing
		Dim helper
		Set helper = CreateReminderHelper(conn,58,0)
		helper.appendRemind(ord)
		conn.close
		set conn = nothing
		if request("divID")&"" = "" Then
			Response.write "" & vbcrlf & "     <script>" & vbcrlf & "        if (window.opener){" & vbcrlf & "             if(window.opener.ajaxRefreshPage){" & vbcrlf & "                      window.opener.ajaxRefreshPage();" & vbcrlf & "                }else if(window.opener.document.forms[0]!=null){" & vbcrlf & "                        window.opener.document.forms[0].submit();" & vbcrlf & "               }else{" & vbcrlf & "                      window.opener.location.reload();" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "       </script>" & vbcrlf & "       "
		else
			Response.write "" & vbcrlf & "     <script>" & vbcrlf & "        if (window.opener){" & vbcrlf & "             if(window.opener.lvw_refresh){" & vbcrlf & "          window.opener.lvw_refresh("""
			Response.write request("divID")
