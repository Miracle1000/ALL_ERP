...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write "' defer='defer'></script>" & vbcrlf & "</head>" & vbcrlf & "<body  oncontextmenu=self.event.returnValue=false >" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf &         "  <tr> "& vbcrlf &        "     <td class=""place"">费用借款回收站</td> "& vbcrlf &        "     <td>&nbsp;</td> "& vbcrlf &          "   <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delpay.asp?ord=payjk&url=pay2.asp&bank=50'}""/></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr  class=""top"">" & vbcrlf & "        <td align=""center"" ><div align=""center"">选择</div></td>" & vbcrlf & "        <td height=""26"" align=""center"" ><div align=""center""><strong>标题</strong></div></td>" & vbcrlf & "        <td height=""26""  ><div align=""center""><strong>金额</strong></div></td>" & vbcrlf & "        <td width=""12%"" ><div align=""center"" ><strong>借款人</strong></div><div align=""center""></div></td>" & vbcrlf & "        "
	if c="" then
	end if
	if c="" then
		Response.write "" & vbcrlf & "        <td width=""12%"" ><div align=""center""><strong>删除人</strong></div></td>" & vbcrlf & "        "
	end if
	Response.write "" & vbcrlf & "        <td ><div align=""center""><strong>删除日期</strong></div></td>" & vbcrlf & "        <td width=""15%"" ><div align=""center"" ><strong>操作</strong></div></td>" & vbcrlf & "      </tr>" & vbcrlf & "      "
	dim n , canDelete
	n=0
	canDelete = true
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from payjk where del=2 order by deldate desc"
	...
	if rs.RecordCount<=0 then
		...
		if Request("CurrPage")&"" = "" then CurrPage=1 else CurrPage=cdbl(Request("CurrPage"))
		if CurrPage<=0 then
			CurrPage=1
		end if
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""deletepay2all.asp?id="
		...
		do until rs.eof
			...
			Response.write "" & vbcrlf & "              <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "           <td width=""7%"" align=""center""><span class=""red"">" & vbcrlf & "              <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
			Response.write rs("id")
			Response.write """>" & vbcrlf & "         </span></td>" & vbcrlf & "            <td align=""center"" height=""26"">" & vbcrlf & "             "
			...
			Response.write "                </td>" & vbcrlf & "         <td width=""10%"" height=""26"" >" & vbcrlf & "               <div align=""center"">"
			Response.write Formatnumber(spmoney,num_dot_xs,-1)
			Response.write " </div></td>" & vbcrlf & "" & vbcrlf & "          "
			dim trade5
			if rs("sorce2")<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select * from gate where ord="&rs("sorce2")&" "
				rs1.open sql1,conn,1,1
				if rs1.eof then
				else
					trade5=rs1("name")
			...
			Response.write "" & vbcrlf & "              <td width=""12%"" class=""name"" ><div align=""center"">"
			Response.write trade5
			Response.write "</div></td>" & vbcrlf & "           "
			dim trade6
			if rs("delcate")<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select * from gate where ord="&rs("delcate")&" "
				rs1.open sql1,conn,1,1
				if rs1.eof then
				else
					trade6=rs1("name")
			...
			canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs("id") &"',50, 8, '8009' ").eof
			Response.write "" & vbcrlf & "             <td width=""12%"" class=""name"" ><div align=""center"">"
			Response.write trade6
			Response.write "</div></td>" & vbcrlf & "          <td width=""15%""  class=""name""><div align=""center"">"
			Response.write rs("deldate")
			Response.write "</div></td>" & vbcrlf & "          <td width=""15%""><div align=""center"">" & vbcrlf & "                <input type=""button"" name=""Submit3c""  class=""anybutton""  value=""恢复""  onClick=""if(confirm('确认恢复？')){window.location.href='setpay2.asp?ord="
			Response.write rs("id")
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>" & vbcrlf & "             "
			if canDelete then
				Response.write "<input   type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletepay2.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
			end if
			Response.write "" & vbcrlf & "" & vbcrlf & "             </div></td>" & vbcrlf & "             </tr>" & vbcrlf & "        "
			n=n+1
			rs.movenext
			if rs.eof or n>=rs.PageSize then exit do
		loop
		Response.write "" & vbcrlf & "    </table></td>" & vbcrlf & "</tr>" & vbcrlf & "         <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""checkbox2"" value=""Check All"" onClick=""mm()"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >" & vbcrlf & "    <input type=""submit"" name=""Submit426"" value=""批量删除""   onClick=""return test();""  class=""anybutton2"">" & vbcrlf & "    <input type=""button"" name=""Submit426"" value=""批量恢复""  onclick=""ask2();"" class=""anybutton2""/>" & vbcrlf & "            </td>" & vbcrlf & "    <td width=""69%""><div align=""right"">" & vbcrlf & "    "
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='pay2.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='pay2.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='pay2.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='pay2.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""setpay2all.asp?currPage="
		Response.write currPage
	...
	action1="费用借款回收站"
	call close_list(1)
	Response.write " " & vbcrlf & "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
