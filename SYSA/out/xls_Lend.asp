...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	if request("tx")=1 then
		tit="归还提醒"
	else
		tit="借阅"
	end if
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>"
	Response.write tit
	Response.write "列表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">"
	Response.write tit
	Response.write "列表导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出借阅列表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write tit
	set ml=new myClass
	...
	xlWorksheet.Cells(1,4).Value = "ISBN"
	...
	elseif open_103_10=1 then
		list=" and addcateid in ("&intro_103_10&")"
	else
		list=" and addcateid=0"
	end if
	Str_Result=" where 1=1 "&list&""
	Ptr_Result=""
	if open_103_1=3 then
...
'if ret2<>"" then
	...
	i=1
	sql="select a.*,b.set_name,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing,d.id as fid,d.addcateid,d.addtime,e.name from O_Lendbookmx a "&_
	"left join O_Lendbook d on a.Ld_fid=d.id "&_
	"left join gate e on d.addcateid=e.ord "&_
	"left join O_regbook c on a.Ld_bkid=c.id "&_
	"left join O_bookset b on c.bk_type=b.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "借阅列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			Ld_time=rs("Ld_time")
			if Ld_time<>"" and not isnull(Ld_time) then
				Ld_time=year(Ld_time)&"-"&month(Ld_time)&"-"&day(Ld_time)
'if Ld_time<>"" and not isnull(Ld_time) then
			end if
			Ld_rettime=rs("Ld_rettime")
			if Ld_rettime<>"" and not isnull(Ld_rettime) then
				Ld_rettime=year(Ld_rettime)&"-"&month(Ld_rettime)&"-"&day(Ld_rettime)
'if Ld_rettime<>"" and not isnull(Ld_rettime) then
			end if
			addtime=rs("addtime")
			sql8="select isnull(sum(Ret_num),0) as Ret_num from O_RetBookmx where Ret_bkid="&rs("id")
			...
			set rs8=nothing
			if rs("Ld_num")=Ret_num then
				ret_state="归还完毕"
			elseif rs("Ld_num")>Ret_num and Ret_num<>0 then
				ret_state="归还部分"
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>"&tit&"列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!"&tit&"列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath(""&tit&"列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的"
	Response.write tit
	Response.write "列表</u></strong></font></a></p>"
