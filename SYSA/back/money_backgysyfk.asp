...
'If typ=2 Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""setbackgysyfkall.asp?currPage="
	Response.write currPage
	Response.write """; </script>" & vbcrlf & "<script src='../script/bk_comm01.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write "'></script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "    <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "         <tr>" & vbcrlf & "              <td class=""place"">供应商退预付款回收站</td>" & vbcrlf & "       <td align=""left"">&nbsp;</td>" & vbcrlf & "                                <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=bankout3&url=money_backgysyfk.asp&bank=12'}""/></td>" & vbcrlf & "           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "            </tr>" & vbcrlf & "               </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                    <tr class=""top"">" & vbcrlf & "                    <td width=""5%"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "                    <td width=""5%"" align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td> "& vbcrlf &        "              <td width=""20%""><div align=""center"">供应商名称</div></td> "& vbcrlf &            "        <td width=""10%""><div align=""center""><strong>退款日期</strong></div></td> "& vbcrlf &           "          <td width=""10%""><div align=""center""><strong>退款金额</strong></div></td>" & vbcrlf & "                         <td width=""10%""><div align=""center""><strong>负责人</strong></div></td>" & vbcrlf & "                      <td width=""10%""><div align=""center""><strong>删除人</strong></div></td>" & vbcrlf & "                      <td width=""15%""><div align=""center""><strong>删除日期</strong></div></td>" & vbcrlf & "                      <td width=""15%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "                      </tr>" & vbcrlf & ""
	dim n,canDelete
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from bankout3  where del=2 order by deldate desc "
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "       <form name=""form1"" method=""post"" action=""deletebackgysyfkall.asp?ord="
		...
		do until rs.eof
			id=rs("id")
			company=rs("company")
			bankout2 = rs("bankout2")
			If bankout2&""="" Then bankout2 = 0
			cateid=rs("cateid")
			money1=zbcdbl(rs("money1"))
			set rs7=server.CreateObject("adodb.recordset")
			sql7="select name,sort3 from tel where ord="&company&""
			...
			rs7.close
			set rs7=Nothing
			if not isNumeric(money1) then money1=0
			if conn.execute("select 1 from bankout2 where id="& bankout2 &" and company="& company &" and del=1 and isnull(money_left,0)>="& money1 ).eof = true then
				str="disabled"
			else
				str=""
			end if
			...
			if open_8_3=3 or (open_8_3=1 and CheckPurview(intro_8_3,trim(cateid))=True) then
				Response.write "<input name=""selectid"" "
				Response.write str
			...
			Response.write "</div></td>" & vbcrlf & ""
			delcate=rs("delcate")
			if delcate="" or Isnull("delcate") Then delcate=0
			if delcate<>0 then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select * from gate where ord="&delcate&""
				rs7.open sql7,conn,1,1
				dim delcatename
				if rs7.eof then
					delcatename=""
				else
					delcatename=rs7("name")
				end if
				rs7.close
				set rs7=nothing
			else
			...
			canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs("id") &"',12, 10, '' ").eof
			Response.write "" & vbcrlf & "             <td><div align=""center"">"
			Response.write delcatename
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">"
			Response.write rs("deldate")
			Response.write "</div></td>" & vbcrlf & "          <td ><div align=""center"">" & vbcrlf & "    <input type=""button"" name=""Submit3c""  class=""anybutton""  value=""恢复""  "
			Response.write str
			Response.write "  onClick=""if(confirm('确认恢复？')){window.location.href='setbackgysyfk.asp?ord="
			Response.write rs("id")
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>" & vbcrlf & "       "
			if canDelete then
				Response.write "<input  type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletebackgysyfk.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
		...
		loop
		m=n
		Response.write "" & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "         </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                <tr>" & vbcrlf & "    <td width=""5%"" height=""30""><div align=""center"">全选<input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" /></div></td>" & vbcrlf & "    <td><input type=""submit"" name=""Submit426"" value=""批量删除""   onClick=""return test();""  class=""anybutton2"">&nbsp;<input type=""button"" name=""Submit427"" value=""批量恢复""  onclick=""ask2();"" class=""anybutton2""/></td>" & vbcrlf & "     </form>" & vbcrlf & " <form name=""form2"" action=""money_backgysyfk.asp?A="
		Response.write A
		Response.write "&B="
		Response.write B
		Response.write "&C="
		...
		Response.write "&px="
		Response.write px
		Response.write """ method=""post"">" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "    "
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "    <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='money_backgysyfk.asp?currPage="
			Response.write  1
			Response.write "&A="
			Response.write A
			Response.write "&B="
			...
			Response.write "&page_count="
			Response.write page_count
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='money_backgysyfk.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "     <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='money_backgysyfk.asp?currPage="
			Response.write  currpage + 1
			Response.write "&A="
			Response.write A
			Response.write "&B="
			...
			Response.write "&page_count="
			Response.write page_count
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='money_backgysyfk.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&A="
			Response.write A
			Response.write "&B="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "      </form>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " " & vbcrlf & "function mm(form) " & vbcrlf & "{" & vbcrlf & "  for (var i=0;i<form.elements.length;i++)" & vbcrlf & "        {" & vbcrlf & "       var e = form.elements[i]; " & vbcrlf & "      if (e.name != 'chkall') " & vbcrlf & "        e.checked = form.chkall.checked; " & vbcrlf & "       } " & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>                    " & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	dim actinon1
