...
'Private Sub Class_Initialize
	...
	elseif open_1_10=1 then
		list=" and c.cateid in ("&intro_1_10&") "
	else
		list=" and c.cateid=0 "
	end if
	if sp1<>"" then
		if sp1="1" then
			Str_Result=" where c.del=3 and c.sp>0 "
		else
			Str_Result=" where c.del=3 and c.sp=-1 "
			Str_Result=" where c.del=3 and c.sp>0 "
		end if
	end if
	If remind <> "" Then
		Set helper = CreateReminderHelper(conn,remind,0)
		Str_Result= "where (c.del = 1 OR c.del = 3) and c.ord in (" & helper.listSQL("ids") & ")"
	end if
	if E&""="" Then E=10
	if F&""="" Then F=0
	if E="3333333" or E="5555555" then
		...
		if E="3333333" and timelg<>"" then
			noalt1=" and c.date7>'"&timelg&"'"
		end if
		Str_Result= " where ((c.cateid_sp="&session("personzbintel2007")&"  and c.del=3 and c.sp>0) or (c.cateid="&session("personzbintel2007")&" and c.alt=1)) " &noalt1&""
	end if
	Str_Result=Str_Result+""&list&""
	If z & "" <> "" Then
		Select Case z
		Case "zdy1","zdy2","zdy3","zdy4"
		Str_Result = Str_Result + " and c." & z & " like '%"& t &"%'"
'Case "zdy1","zdy2","zdy3","zdy4"
		Case "zdy5","zdy6"
		Str_Result = Str_Result + " and c." & z & " in (select ord from sortonehy where gate2=2201 AND sort1 like '%"& t &"%')"
'Case "zdy5","zdy6"
		End Select
	end if
	area_list=-1
'End Select
	...
	if  A2<>"" then
		Str_Result=Str_Result+"and  c.area in ("&area_list&")"
'if  A2<>"" then
	end if
	D=request("D")
	if D<>"" then
		Str_Result=Str_Result+"and  c.trade in ("&D&") "
'if D<>"" then
	end if
	if E<>"10" And E <>"3333333" And E <>"5555555" then
		Str_Result=Str_Result+"and  c.zt1="&E&""
'if E<>"10" And E <>"3333333" And E <>"5555555" then
	end if
	if F<>"0" then
		Str_Result=Str_Result+"and  c.sort='"&F&"'"
'if F<>"0" then
	end if
	If C&""<>"" Then
		if B = "cgzt" then
...
'if B = "cgzt" then
		elseif B = "cgbh" then
			Str_Result=Str_Result+"and  c.cgid like '%"& C &"%'"
'elseif B = "cgbh" then
		elseif B = "cggy" then
			Str_Result=Str_Result+"and  c.intro like '%"& C &"%'"
'elseif B = "cggy" then
		elseif B = "cgry" then
			Str_Result=Str_Result+"and  c.cateid in (select ord from gate where name like '%"& C &"%')"
'elseif B = "cgry" then
		elseif B = "gys" then
			Str_Result=Str_Result+"and  c.company in (select ord from tel where name like '%"& C &"%')"
'elseif B = "gys" then
		end if
	end if
	if m1<>"" then
		Str_Result=Str_Result+"and CONVERT(VARCHAR(10),c.date3,120) >= '"&m1&"' "
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+"and CONVERT(VARCHAR(10),c.date3,120) <= '"&m2&"' "
'if m2<>"" then
	end if
	...
	elseif px=5 then
		px_Result="order by c.cgid desc"
	elseif px=6 then
		px_Result="order by c.cgid asc"
	elseif px=7 then
		px_Result="order by c.date3 desc"
	elseif px=8 then
		px_Result="order by c.date3 asc"
	elseif px=9 then
		px_Result="order by c.money1 desc"
	elseif px=10 then
		px_Result="order by c.money1 asc"
	elseif px=11 then
		px_Result=" order by (select top 1 date7 from reply where ord2=c.ord and sort1=5 order by date7 desc) desc"
	elseif px=12 then
		px_Result="order by (select top 1 date7 from reply where ord2=c.ord and sort1=5 order by date7 desc) asc"
	end if
	Chtml=""&_
	"<td style='font-weight:bold'>采购主题</td>"&_
	"<td style='font-weight:bold'>采购编号</td>"&_
	"<td style='font-weight:bold'>采购日期</td>"&_
	"<td style='font-weight:bold'>采购分类</td>"&_
	"<td style='font-weight:bold'>入库状态</td>"&_
	"<td style='font-weight:bold'>所属地区</td>"&_
	"<td style='font-weight:bold'>所属行业</td>"&_
	"<td style='font-weight:bold'>采购人员</td>"&_
	"<td style='font-weight:bold'>创建人</td>"&_
	"<td style='font-weight:bold'>创建时间</td>"&_
	"<td style='font-weight:bold'>我方代表</td>"&_
	"<td style='font-weight:bold'>对方代表</td>"&_
	"<td style='font-weight:bold'>供应商名称</td>"&_
	"<td style='font-weight:bold'>币种</td>"&_
	"<td style='font-weight:bold'>采购金额</td>"&_
	"<td style='font-weight:bold'>付款金额</td>"&_
	"<td style='font-weight:bold'>退款金额</td>"&_
	"<td style='font-weight:bold'>费用合计</td>"
	set rs1=conn.execute("select * from zdy where sort1=22 and set_open=1 and dc=1 order by gate1 asc ")
	do until rs1.eof
		Chtml=Chtml&"<td style='font-weight:bold'>"&rs1("title")&"</td>"
'do until rs1.eof
		m=m+1
'do until rs1.eof
		rs1.movenext
	...
	if ZBRuntime.MC(215101) then
		...
		rs_1.open "select FName,id from ERP_CustomFields where tName = 22 and isUsing=1 and CanExport = 1",conn,3,1
		redim arr_CF(rs_1.recordcount)
		idx_i=0
		do until rs_1.eof
			Chtml=Chtml&"<td style='font-weight:bold'>"&rs_1("FName")&"</td>"
'do until rs_1.eof
			arr_CF(idx_i)=rs_1("id")
			idx_i=idx_i+1
			arr_CF(idx_i)=rs_1("id")
			rs_1.movenext
		loop
	...
	m=m+idx_i
	rs_1.close
	if ZBRuntime.MC(12010) Then
		Chtml=Chtml&"<td style='font-weight:bold'>洽谈进展</td>"
'if ZBRuntime.MC(12010) Then
		m=m+1
'if ZBRuntime.MC(12010) Then
	end if
	Chtml=Chtml&"<td style='font-weight:bold'>采购概要</td>"
'if ZBRuntime.MC(12010) Then
	m=m+1
'if ZBRuntime.MC(12010) Then
	set rs1=conn.execute("select id,title,sort,kd,set_open,gate1,sorce from zdymx where sort1=22 and set_open=1 order by gate1 asc")
	do until rs1.eof
		Chtml=Chtml&"<td style='font-weight:bold'>"&rs1("title")&"</td>"
'do until rs1.eof
		if rs1("sorce")=5 Then
			Chtml=Chtml&"<td style='font-weight:bold'>折扣</td>"
'if rs1("sorce")=5 Then
		elseif rs1("sorce")=6 Then
			Chtml=Chtml&"<td style='font-weight:bold'>申请</td><td>入库</td>"
'elseif rs1("sorce")=6 Then
		end if
		rs1.movenext
	loop
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("采购单")
	response.Flush()
	xsheet.writeHTML Chtml
	set rs=server.CreateObject("adodb.recordset")
	sql="select c.ord,c.title,c.cgid,c.money1,"&_
	"CONVERT(VARCHAR(20),c.date3,120) date3,"&_
	"m1.menuname area,st1.sort1 trade,person1 person_us,p2.name person_otherside,isnull(tk.money1,0) moneytk,"&_
	"isnull((select sum(isnull(money1,0)) as sumfy from pay where caigou=c.ord and del=1 and complete=3),0) moneyfy,"&_
	"c.zt1,c.intro,c.date7,s.sort1 as sortbz,t.name as name1,y.sort1,g.name as cateid,"&_
	"c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,"&_
	"isnull(c.company,0) company "&_
	"from caigou c "&_
	"left join ("&_
	"select isnull(sum(a.money1),0) as money1,b.caigou from payout3 a "&_
	"inner Join caigouth b On a.caigouth=b.ord "&_
	"Where a.del=1 And a.complete=2 "&_
	"Group By b.caigou" &_
	") tk on tk.caigou = c.ord " &_
	"left join menuarea m1 on m1.id = c.area " &_
	"left join sortonehy st1 on st1.ord = c.trade " &_
	"left join sortbz s on s.id=c.bz "&_
	"left join tel t on t.ord=c.company "&_
	"left join person p2 on p2.ord=c.person2 "&_
	"left join sortonehy y on y.ord=c.sort "&_
	"left join gate g on g.ord=c.cateid "&Str_Result&" "&px_Result&" "
	...
	do until rs.eof
		...
		If gysRs.eof = False Then
			If gysRs("del")=2 Then
				name1 ="<font color='red'>供应商已删除</font>"
			else
		...
		Set rss=conn.execute("select isnull(sum(money1),0) as money2  from payout where contract="& rs("ord") &" and complete=3 and del=1")
		If Not rss.eof Then
			money2=zbcdbl(rss("money2"))
		else
			money2=0
		end if
		rss.close
...
'If money2&""="" Then money2=0
		moneytk=Formatnumber(moneytk,num_dot_xs,-1)
'If money2&""="" Then money2=0
		moneyfy=Formatnumber(moneyfy,num_dot_xs,-1)
'If money2&""="" Then money2=0
		date3=rs("date3")
		sort1=rs("sort1")
		if rs("zt1")="0" then
		...
		intro2=""
		set rs1=conn.execute("select top 1 intro,date7 from reply  where ord2="&rs("ord")&" and sort1=5 and del=1 order by date7 asc")
		do until rs1.eof
		...
		"<td>"& moneytk & "</td>"&_
		"<td>"& moneyfy & "</td>"
		set rs88=conn.execute("select id,name,sort,gl from zdy where sort1=22 and set_open=1 and dc=1 order by gate1 asc ")
		do until rs88.eof
			if rs88("sort")=1 then
				zdy=rs(""&rs88("name")&"")
				if IsNull(zdy) or zdy="" then
					...
					set rs7=conn.execute("select ord,sort1 from sortonehy where ord="&zdy&"")
					if rs7.eof then
						zdyname=""
					else
						zdyname=rs7("sort1")
				...
				rowHTML=rowHTML&"<td>"&zdyname&"</td>"
			else
				rowHTML=rowHTML&"<td>"& rs(""&rs88("name")&"") &"</td>"
			end if
			rs88.movenext
		loop
		rs88.close
		if ZBRuntime.MC(215101) then
			...
			for i=0 to ubound(arr_CF)-1
				set rs_2=server.CreateObject("adodb.recordset")
				rs_2.open "select isNull(FValue,'') from ERP_CustomValues where fieldsID=" & arr_CF(i) & " and OrderID=" & rs("ord"),conn,3,1
				if not rs_2.EOF then
					rowHTML=rowHTML&"<td>" & rs_2(0) & "</td>"
				else
		...
		if ZBRuntime.MC(12010) Then rowHTML=rowHTML&"<td>"& intro2 &"</td>"
		rowHTML=rowHTML&"<td>"& intro&"</td>"
		rowHTML=rowHTML&"</tr>"
		rowIndex=rowIndex+1
		rowHTML=rowHTML&"</tr>"
		xsheet.writeHTML rowHTML
		set rs1 =conn.execute("select c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,c.num1,c.num2,c.num3,c.date2,c.intro,c.price1,c.money1,p.title as productname,p.order1 as productorder1,p.type1 as producttype1,s.sort1 as unitname from caigoulist c left join product p on p.ord=c.ord and p.del=1 left join sortonehy s on s.ord=c.unit where caigou="&rs("ord")&" order by c.date7 asc,c.id asc")
		do until rs1.eof
			product=rs1("productname")
			order1=rs1("productorder1")
			type1=rs1("producttype1")
			...
			zk=0
			if price1<>0 and cdbl(num1)<>0 then
				zk=money1/price1/cdbl(num1)
			else
				zk=1
			end if
			rowHTML="<tr><td></td><td>产品明细</td>"
			For k=3 To m
				rowHTML=rowHTML&"<td></td>"
			next
			set rs88=conn.execute("select * from zdymx where sort1=22 and set_open=1 order by gate1 asc ")
			do until rs88.eof
				sorce=rs88("sorce")
				Select Case sorce
				Case 1 :
				rowHTML=rowHTML&"<td>"&product&"</td>"
				Case 2 :
				rowHTML=rowHTML&"<td>"&order1&"</td>"
				Case 3 :
				rowHTML=rowHTML&"<td>"&type1&"</td>"
				Case 4 :
				rowHTML=rowHTML&"<td>"&unitname&"</td>"
				Case 5 :
				if open_22_21<>0 then
					rowHTML=rowHTML&"<td>"& Formatnumber(price1,num_dot_xs,-1)&"</td><td>"& zk &"</td>"
'if open_22_21<>0 then
				else
					rowHTML=rowHTML&"<td></td><td></td>"
				end if
				Case 6 :
				rowHTML=rowHTML&"<td>"& num1&"</td><td>"& num2 &"</td><td>"& num3 &"</td>"
				Case 7 :
				if open_22_21<>0 then
					rowHTML=rowHTML&"<td>"& Formatnumber(money1,num_dot_xs,-1)&"</td>"
'if open_22_21<>0 then
				else
					rowHTML=rowHTML&"<td></td>"
				end if
				Case 8 :
				rowHTML=rowHTML&"<td>"&date2&"</td>"
				Case 9 :
				rowHTML=rowHTML&"<td>"&intro1&"</td>"
				Case Else :
				if trim(rs88("name"))="zdy5" or trim(rs88("name"))="zdy6" then
					zdy=rs1(""&rs88("name")&"")
					if zdy="" or isnull(zdy) or not isNumeric(zdy) then zdy=0
					set rs7=conn.execute("select * from sortonehy where ord="&zdy&" ")
					if not rs7.eof then
						zdyname=rs7("sort1")
					else
						zdyname=""
		...
		If rowIndex > 40000 Then
			...
			If pageCount = 1 then
				xsheet.title = "采购单(" & pageCount & "页)"
			end if
			pageCount = pageCount + 1
			xsheet.title = "采购单(" & pageCount & "页)"
			Set xsheet = xApp.sheets.add("采购单(" & pageCount & "页)")
			rowIndex = 0
			xsheet.writeHTML Chtml
		end if
		rs.movenext
	...
	set rs=Nothing
	xsheet.writeHTML "</table>"
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>采购单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!采购单导出成功,共有"&(n)&"个采购单!</B>"";</script>"
	tfile=Server.MapPath("采购单_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'采购单_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="采购单导出"
	call close_list(1)
	...
	Response.write """><font class=""red""><strong><u>下载导出的采购单</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
