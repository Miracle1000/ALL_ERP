...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "       <style>" & vbcrlf & "         body{height:auto;position:initial;}" & vbcrlf & "             #gxmx_cel td {color:#5b7cae;}" & vbcrlf & "           #comm_itembarbg{height:43px!important;}" & vbcrlf & " </style>" & vbcrlf & "        "
	end sub
	Sub Bill_OnInit(ByVal bill, ByVal t)
		...
		bill.loadVml = True
		bill.title = "工序进度看板"
		Bill.groups(0).title="派工汇总"
		Bill.groups(0).barHTML = getGroupBarHtml(1)
		bill.groups(0).Buttons.add "检索", "search", "bntClick(this,1)", true, True
		Bill.fields.addListView("", "pghz").ImageVMLGraphic = True
		bill.AddCurrGroup "工序明细","gxmxlist"
		Bill.groups(1).barHTML = getGroupBarHtml(2)
		bill.groups(1).Buttons.add "检索", "search", "bntClick(this,2)", true, True
		Bill.fields.addListView "", "gxmx"
	...
	sub bill_onListCreate(byref field, byref lvw)
		...
		Case "pghz" :
		date2 = Date
		date1 = dateadd("ww" , -1 ,date)
'date2 = Date
		lvw.id = "zttj"
		Call lvw_CreateList("zttj" , lvw , date1 , date2 , 0 ,  "" , "" )
		Case "gxmx" :
		date2 = Date
		date1 = dateadd("ww" , -1 ,date)
'date2 = Date
		lvw.pagesize = 10
		lvw.oldPageSizeUI = False
		lvw.PageBar = True
		lvw.id = "tjlist"
		Call lvw_CreateList("tjlist" , lvw , date1 , date2 , 0 ,  "" , "" )
		End Select
	end sub
	Sub bill_OnVmlCreate(ByVal cmd , ByRef vml)
		vml.title = "派工进度"
		vml.filterText = " n<>'所有派工单' "
		Dim i , urls
		For i = 1 To ubound(vml.urls)
			vml.urls(i-1) = vml.urls(i)
'For i = 1 To ubound(vml.urls)
		next
	end sub
	Sub lvw_defCallBack(ByVal cmd,ByVal lvw)
		Dim date1 ,date2 , gx , B , C
		date1 = app.getText("date1")
		date2 = app.getText("date2")
		Select Case cmd
		Case "zttj":
		lvw.id = "zttj"
		Call lvw_CreateList("zttj" , lvw , date1 , date2 , gx ,  B , C )
		Case "tjlist":
		lvw.id = "tjlist"
		gx = app.getint("gx")
		B = app.getText("B")
		C = app.getText("C")
		Call lvw_CreateList("tjlist" , lvw , date1 , date2 , gx ,  B , C )
		End Select
	end sub
	Dim open_21_14
	Sub lvw_CreateList(cmd , lvw , date1 ,date2 , gx , B, C)
		Select Case cmd
		Case "zttj":
		Dim curl : curl ="../manufacture/inc/Billlist.asp?orderid=8&t1="& date1 &"&t2="& date2 &""
		lvw.sql = "exec erp_pgd_zttj "& info.user &" ,'"& date1 &"' ,'"& date2 &"' "
		lvw.headers("所有派工单").formatText="<a href='"& curl &"' target='_blank'>@value</a>"
		With lvw.headers("未开始")
		.formatText="<a href='"& curl &"&status=1' target='_blank'>@value</a>"
		.url = curl &"&status=1"
		End With
		With lvw.headers("滞期未开始")
		.formatText="<a href='"& curl &"&status=2' target='_blank'>@value</a>"
		.url = curl &"&status=2"
		End With
		With lvw.headers("生产中")
		.formatText="<a href='"& curl &"&status=3' target='_blank'>@value</a>"
		.url = curl &"&status=3"
		End With
		With lvw.headers("超期未完成")
		.formatText="<a href='"& curl &"&status=4' target='_blank'>@value</a>"
		.url = curl &"&status=4"
		End With
		With lvw.headers("提前完成")
		.formatText="<a href='"& curl &"&status=5' target='_blank'>@value</a>"
		.url = curl &"&status=5"
		End With
		With lvw.headers("按期完成")
		.formatText="<a href='"& curl &"&status=6' target='_blank'>@value</a>"
		.url = curl &"&status=6"
		End With
		With lvw.headers("超期完成")
		.formatText="<a href='"& curl &"&status=7' target='_blank'>@value</a>"
		.url = curl &"&status=7"
		End With
		Case "tjlist":
		If isEmpty(open_21_14) Then  open_21_14 = app.power.existsPower(21,14)
		lvw.sql = "exec erp_proceprogres_tjlist "& info.user &" ,'"& date1 &"' ,'"& date2 &"',"& gx &",'"& B &"','"& C &"',0 ,&excelmode,&pagesize ,&pageindex, &sortSql "
		lvw.headers("tord").display="none"
		lvw.headers("tCateid").display="none"
		lvw.headers("mid").display="none"
		lvw.headers("mCreator").display="none"
		lvw.headers("aid").display="none"
		lvw.headers("aCreator").display="none"
		lvw.headers("eord").display="none"
		With lvw.headers("name")
		.title = "客户名称"
		.dbtype="str"
		.setLink "name","tOrd", "tCateid",  1, -9
'.dbtype="str"
		End With
		With lvw.headers("MOBH")
		.title = "订单编号"
		.setLink "MOBH","mid", "mCreator",  51, 2
		End With
		With lvw.headers("WABH")
		.title = "派工单编号"
		.setLink "WABH","aid", "aCreator",  54, 8
		End With
		With lvw.headers("ptitle")
		.title = "产品名称"
		.dbtype="str"
		If open_21_14 Then .formatText="code:getProductUrl(@value , rs(""eord""))"
		End With
		With lvw.headers("DateWA")
		.title = "派工日期"
		.width = 120
		End With
		With lvw.headers("DateEnd")
		.title = "预计完成日期"
		.width = 120
		End With
		With lvw.headers("zt")
		...
		.title = "工序名称"
		End With
		With lvw.headers("NumMake")
		.title = "派工数量"
		.dbtype = "number"
		.width = 100
		End With
		With lvw.headers("tnum")
		.title = "工序汇报量_通过"
		.width = 100
		End With
		With lvw.headers("fnum")
		.title = "工序汇报量_返工"
		.width = 100
		End With
		With lvw.headers("bnum")
		.title = "工序汇报量_作废"
		.width = 100
		End With
		With lvw.headers("wgl")
		.title = "汇报完工率"
		.formatText = "code:showPercent(@value , rs(""NumMake"").value)"
		End With
		With lvw.headers("DateEnd")
		.title = "预计完成日期"
		End With
	...
	Function getProductUrl(v ,ord)
		Dim s : s = "<a href='../product/content.asp?ord="& app.base64.pwurl(ord) &"' target='_blank'>"& v &"</a>"
		getProductUrl = s
	end function
...
'v = CDbl(v) * 100
	...
	Function getGroupBarHtml( stype)
		Dim rs ,sql, html
		Select Case stype
		Case 1 :
		html = "派工日期 <input type='text' id='date1_v_0' onmousedown='datedlg.show()' readonly size='10' maxlength=10 value='"& dateadd("ww" , -1 ,date) &"'>"&_
		"至：<input type='text' id='date1_v_1' onmousedown='datedlg.show()' readonly size='10' maxlength=10 value='"& date &"'>"
		Case 2 :
		html = "派工日期 <input type='text' id='date2_v_0' onmousedown='datedlg.show()' readonly size='10' maxlength=10 value='"& dateadd("ww" , -1 ,date) &"'>"&_
		"至：<input type='text' id='date2_v_1' onmousedown='datedlg.show()' readonly size='10' maxlength=10 value='"& date &"'>" &_
		"<select id='gx' name='gx' style='max-width:500px'><option value=0>工序</option>"
		sql = "select r.WPname , r.ID from M_WorkingProcedures r where r.del=0 order by r.id desc  "
		Set rs = cn.execute(sql)
		While rs.eof=False
			html = html & "<option value= "& rs("id").value &">"& rs("WPname") &"</option>"
			rs.movenext
		wend
		rs.close
		html = html & "</select> " &_
		"<select name='B' id='B'><option value='khmc'>客户名称</option><option value='scdd'>订单编号</option><option value='pgd'>派工单编号</option><option value='cpmc'>产品名称</option></select> <input type='text' size=15 name='C' id='C'> "
		End Select
		getGroupBarHtml = html
	end function
%>
