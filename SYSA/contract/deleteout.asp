...
'If Len(exists_Pics) = 0 Then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select a1 from contract_out Where id="&request("id")&" "
	...
	rs.close
	set rs=nothing
	picUrl = getPicUrlForMoban(intro_old)
	if picUrl <> "" then
		picUrlArray = split(picUrl,",")
		for x = 0 to ubound(picUrlArray)
			DeleteFile("../moban/upimages/"&picUrlArray(x))
		next
	end if
	dim W1,W2,W3,A1,A2,B,C,D,E,F,H,m1,m2,bh,lie_1,lie_2,lie_3,page_count,px
	sort=request("sort")
	main=request("main")
	CurrBookID=request("id")
	isnew = request("isnew")
	sql="Delete contract_out where id="&CurrBookID&" "
	conn.Execute(sql)
	action1="打印设置删除"
	call close_list(2)
	Response.redirect "planall_out.asp?sort="&sort&"&main="&main&"&isnew="& isnew &""
