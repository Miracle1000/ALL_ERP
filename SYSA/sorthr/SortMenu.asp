...
'If Not rs.Eof Then
	...
	If OrderID = 1021 Then
		conn.execute "update hr_ret_type set orderid=1021, typeid=2,cnid=id,sortid=0,indate='" & now & "',creator=" & session("personzbintel2007") & ",px=0 where orderid is null"
		tname = "hr_ret_type"
	end if
	...
	function menu(cnid)
		...
		sql="select *,isnull((select count(*) from " & tname & " where Sortid=mm.cnid and  orderid="& OrderID &" and typeID="& typeID &" and del=0),0) as ChildCount from " & tname & " as mm where sortid="&cnid&" and  orderid="& OrderID &" and typeID="& typeID &" and del=0 order by px desc"
			...
			while not RS8.eof
				...
				if ChildCount=0 then
					...
					if i=RS8.recordcount then
						menutype="menu3"
						listtype="list1"
						onmouseup="change1('a"&RS8("cnid")&"','b"&RS8("cnid")&"');"
					else
						menutype="menu1"
						listtype="list"
						onmouseup="change2('a"&RS8("cnid")&"','b"&RS8("cnid")&"');"
					end if
					menuname=RS8("title")
				end if
				If CanChild=False Then
					...
					If OrderID= 1021 Then
						...
						StrChild="<a href=""javascript:void(0)"" onClick=""javascript:window.open('add_Sort.asp?OrderID="&OrderID&"&TypeID="&TypeID&"&action=add&cnid="&RS8("cnid")&"','newwinadd','width=' + 600 + ',height=' + 300 + ',fullscreen =no,scrollbaRS8=1,toolbar=0,resizable=1,left=100,top=100')""><font  color=""#909090"">添加 |</font></a> "
					end if
				end if
				If ChildCount=0  Then
					StrDel="href=""SortMenu.asp?OrderID="& OrderID &"&TypeID="&TypeID&"&action=del&cnid="&RS8("cnid")&""" onclick=""return confirm('建议不要轻易删除设置好的参数，以免影响到其他关联的数据。确认删除?')"" "
				else
					StrDel="href=""###"" readonly=""readonly"" onclick=""alert('请先删除子分类！');"" "
				end if
				StrReturn=StrReturn&"<tr><td id=""b"&RS8("cnid")&""" class="""&menutype&""" onMouseUp="""&onmouseup&""">"&menuname&"&nbsp;&nbsp;&nbsp;"& StrChild &"<a href=""javascript:void(0)"" onClick=""javascript:window.open('add_sort.asp?orderID="& orderID &"&TypeID="&TypeID&"&action=edit&cnid="&RS8("cnid")&"','newwincorrect','width=' + 600 + ',height=' + 300 + ',fullscreen =no,scrollbaRS8=1,toolbar=0,resizable=1,left=100,top=100')""><font color=""#909090"">修改 |</font></a> <a  "&StrDel&" ><font  color=""#909090"">删除</font></a></td></tr>"
				StrDel="href=""###"" readonly=""readonly"" onclick=""alert('请先删除子分类！');"" "
				if ChildCount>0 then
					StrReturn=StrReturn&"<tr id=""a"&RS8("cnid")&"""><td class="""&listtype&""">"&menu(RS8("cnid"))&"</td></tr>"
				end if
				RS8.movenext
				i=i+1
				'RS8.movenext
	...
	function delmenu(id)
		conn.execute("update  " & tname & " set del=2 where cnid="&id)
	end function
