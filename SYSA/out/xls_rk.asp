...
'bigsystemtype = ".mozi"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=31 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_31_8=0
		intro_31_8=0
	else
		open_31_8=rs1("qx_open")
		intro_31_8=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	if open_31_1=3 then
		list=""
	elseif open_31_1=1 then
		list="and cateid in ("&intro_31_1&")"
	else
		list="and cateid=0"
	end if
...
') c on a.fromType=c.fromType and a.id=c.fromid &_
	...
	Dim cellCountBeforeDetail : cellCountBeforeDetail = 0
	dim isShowFyFt
	isShowFyFt = showFyFt()
	Sub createXlsHead
		Dim rs1
		xsheet.writecellB "入库编号"
		xsheet.writecellB "入库主题"
		xsheet.writecellB "入库数量"
		xsheet.writecellB "入库类别"
		xsheet.writecellB "入库状态"
		xsheet.writecellB "库管"
		xsheet.writecellB "申请日期"
		xsheet.writecellB "申请人员"
		xsheet.writecellB "审批日期"
		xsheet.writecellB "审批人员"
		cellCountBeforeDetail = cellCountBeforeDetail + 10
		xsheet.writecellB "审批人员"
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select title from zdy where sort1=31 and set_open=1 and dc=1 order by gate1 asc "
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			do until rs1.eof
				xsheet.writecellB rs1("title").value
				cellCountBeforeDetail = cellCountBeforeDetail + 1
				xsheet.writecellB rs1("title").value
				rs1.movenext
			loop
		end if
		rs1.close
		set rs1=Nothing
		xsheet.writecellB "审批意见"
		xsheet.writecellB "关联采购"
		xsheet.writecellB "供应商名称"
		xsheet.writecellB ""
		cellCountBeforeDetail = cellCountBeforeDetail + 3
		xsheet.writecellB ""
		n=m
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select id,title,sort,kd,set_open,gate1,sorce from zdymx where sort1=31 and set_open=1 order by gate1 asc,id asc"
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			do until rs1.eof
				Select Case  rs1("sorce").value
				Case 5
				If open_23_3<>0  Then
					xsheet.writecellB rs1("title").value
				end if
				...
				If open_23_3<>0 Then
					xsheet.writecellB  rs1("title").value
					if isShowFyFt then
						xsheet.writecellB  "费用均摊金额"
					end if
					xsheet.writecellB  "成本单价"
					xsheet.writecellB  "成本总价"
				end if
				xsheet.writecellB "批号"
				xsheet.writecellB "序列号"
				xsheet.writecellB "生产日期"
				xsheet.writecellB "有效日期"
				Case 15
				xsheet.writecellB rs1("title").value
				xsheet.writecellB "包装"
				xsheet.writecellB "件数"
				Case Else
				xsheet.writecellB rs1("title").value
				End select
				rs1.movenext
		...
		rs1.close
		set rs1=nothing
		xsheet.writecellB "仓库"
	end sub
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>入库单导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">入库单导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出入库单,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<tablewidth=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" &vbcrlf & "  "
	dim MODULES
	...
	dotstr_num=""
		for i=1 to num1_dot
			...
			xlsApp.init Me, conn
			Set xsheet = xlsApp.sheets.add("入库列表")
			Call createXlsHead
			xsheet.movenext
			Response.Flush
			...
			if newstate="1" Then
				Str_Result= Str_Result + " and ((kg="&session("personzbintel2007")&" and complete1=1 and ("& open_31_16 &"=3 or ("& open_31_16 &"=1 and charindex(','+cast(cateid as varchar(10))+',',',"& intro_31_16 &",')>0))) or (complete1=3 and alt is null and cateid="&session("personzbintel2007")&"))"
'if newstate="1" Then
			end if
			if a="" then
				a=7
...
'if not ZBRuntime.MC(15000) then
			...
			if not ZBRuntime.MC(8000) then
				Str_Result=Str_Result+" and  sort1<>2 "
'if not ZBRuntime.MC(8000) then
			end if
			if not ZBRuntime.MC(17006) then
				Str_Result=Str_Result+" and  sort1<>6 "
'if not ZBRuntime.MC(17006) then
			end if
...
'if not ZBRuntime.MC(17004) then
			...
			if not ZBRuntime.MC(17005) then
				Str_Result=Str_Result+" and  sort1<>8 "
'if not ZBRuntime.MC(17005) then
			end if
			if not ZBRuntime.MC(17007) then
				Str_Result=Str_Result+" and  sort1<>9 "
'if not ZBRuntime.MC(17007) then
			end if
			if not ZBRuntime.MC(18000) then
				Str_Result=Str_Result+" and  sort1<>3 and sort1<>13 and sort1<>5 "
'if not ZBRuntime.MC(18000) then
			end if
			if C<>"" then
				if B="ckzt" then
...
'if B="ckzt" then
				elseif B="ckbh" then
					Str_Result=Str_Result+" and rkbh like '%"&C&"%'"
'elseif B="ckbh" then
				elseif B="sqry" then
...
'elseif B="spry" then
				elseif B="glht" then
					Str_Result=Str_Result+" and sort1=1 and caigou in (select ord from caigou where title like '%"&C&"%') "
'elseif B="glht" then
				elseif B="glkh" then
					Str_Result=Str_Result+" and sort1=1 and caigou in (select ord from caigou where company in (select ord from tel where name like '%"&C&"%')) "
'elseif B="glkh" then
				elseif B="zdy1" then
					Str_Result = Str_Result + " and  zdy1 like '%"&C&"%'"
'elseif B="zdy1" then
				elseif B="zdy2" then
					Str_Result = Str_Result + " and  zdy2 like '%"&C&"%'"
'elseif B="zdy2" then
				elseif B="zdy3" then
					Str_Result = Str_Result + " and  zdy3 like '%"&C&"%'"
'elseif B="zdy3" then
				elseif B="zdy4" then
					Str_Result = Str_Result + " and  zdy4 like '%"&C&"%'"
'elseif B="zdy4" then
				elseif B="zdy5" then
					Str_Result = Str_Result + " and  zdy5 in (select ord from sortonehy where gate2=4001 and sort1 like '%"&C&"%')"
'elseif B="zdy5" then
				elseif B="zdy6" then
					Str_Result = Str_Result + " and  zdy6 in (select ord from sortonehy where gate2=4002 and sort1 like '%"&C&"%')"
'elseif B="zdy6" then
				ElseIf B="cpmc" Then
					conn.execute("select a.kuin into #temp from kuinlist a inner join product b on a.ord = b.ord and b.del=1 and a.del=1 where b.title like '%"&C&"%' group by a.kuin")
					Str_Result = Str_Result + " and  ord in (select kuin from #temp)"
				end if
			end if
			if m1<>"" then
...
'if m2<>"" then
			...
			cgid=request("cgid")
			if cgid<>"" then
				Str_Result=Str_Result&" and caigou=" & clng(cgid)
			end if
			If remind <> "" Then
				Set helper = CreateReminderHelper(conn,remind,0)
				Str_Result=Str_Result &" and ord in (" & helper.listSQL("ids") & ")"
			end if
			ddlistid = request("ddlistid")&""
			If Len(ddlistid)>0 Then
				ddlistid = deurl(ddlistid)
				Str_Result = Str_Result & " and ord in ( "        &_
				"select kl.kuin  "        &_
				"from M2_WorkAssigns WA  "        &_
				"inner join M2_QualityTestingLists QTL on QTL.bid = WA.ID   "     &_
				"inner join M2_QualityTestings QT on QT.id = QTL.QTID and QT.poType in (3,4)  "   &_
				"inner join kuinlist kl on QTL.ID = kl.M2_QTLID  or QTL.ID = isnull(kl.M2_BFID,0)  "      &_
				"where isnull(WA.ddlistid,0) = "  & ddlistid &  "  "    &_
				"union all  "     &_
				"select kl.kuin  "        &_
				"from M2_OutOrderlists ool  "     &_
				"inner join M2_QualityTestingLists QTL on QTL.bid = ool.ID    "   &_
				"inner join M2_QualityTestings QT on QT.id = QTL.QTID and QT.poType in (1,2)  "   &_
				"inner join kuinlist kl on QTL.ID = kl.M2_QTLID  "        &_
				"where isnull(ool.molist,0) = "& ddlistid &"  " &_
				")"
			end if
...
'elseif px=2 then
			elseif px=3 then
				px_Result="order by rkbh desc,datediff(mi,'1970-01-01 00:00:00',date7) desc"
'elseif px=3 then
			elseif px=4 then
				px_Result="order by rkbh asc,datediff(mi,'1970-01-01 00:00:00',date7) asc"
'elseif px=4 then
			elseif px=7 then
				px_Result="order by complete1 desc,datediff(mi,'1970-01-01 00:00:00',date7) desc"
'elseif px=7 then
			elseif px=8 then
				px_Result="order by complete1 asc,datediff(mi,'1970-01-01 00:00:00',date7) asc"
'elseif px=8 then
			elseif px=9 then
...
'elseif px=10 then
			elseif px=11 then
				px_Result="order by date5 desc,datediff(mi,'1970-01-01 00:00:00',date7) desc"
'elseif px=11 then
			elseif px=12 then
				px_Result="order by date5 asc,datediff(mi,'1970-01-01 00:00:00',date7) asc"
'elseif px=12 then
			end if
			On Error GoTo 0
			set rs1=server.CreateObject("adodb.recordset")
			...
			rs1.close
			set rs1=nothing
			if open_31_10=3 then
				list=""
			elseif open_31_10=1 then
				list="and cateid in ("&intro_31_10&")"
			else
				list="and cateid=0"
			end if
			Str_Result=Str_Result+""&list&""
			list="and cateid=0"
			Dim rsmx, rsmxcount, rsmxname, rsmxsorce
			rsmxcount = -1
'Dim rsmx, rsmxcount, rsmxname, rsmxsorce
			ReDim rsmxname(0)
			ReDim rsmxsorce(0)
			set rsmx=server.CreateObject("adodb.recordset")
			rsmx.open "select name,sorce from zdymx where sort1=31 and set_open=1 order by gate1 asc,id asc" , conn, 3, 1
			While rsmx.eof = False
				rsmxcount = rsmxcount + 1
'While rsmx.eof = False
				ReDim Preserve rsmxname(rsmxcount)
				ReDim Preserve rsmxsorce(rsmxcount)
				rsmxname(rsmxcount) = rsmx(0).value
				rsmxsorce(rsmxcount) = rsmx(1).value
				rsmx.movenext
			wend
			rsmx.close
			Dim rsmxcount2, rsmxname2, rsmxsorce2, rsmxid2, rowIndex, pageCount
			rsmxcount2 = -1
'Dim rsmxcount2, rsmxname2, rsmxsorce2, rsmxid2, rowIndex, pageCount
			pageCount = 1
			rowIndex = 0
			...
			ReDim rsmxid2(0)
			set rsmx=server.CreateObject("adodb.recordset")
			rsmx.open "select id, name,sort,gl from zdy where sort1=31 and set_open=1 and dc=1 order by gate1 asc" , conn, 3, 1
			While rsmx.eof = False
				rsmxcount2 = rsmxcount2 + 1
'While rsmx.eof = False
				ReDim Preserve rsmxid2(rsmxcount2)
				ReDim Preserve rsmxname2(rsmxcount2)
				ReDim Preserve rsmxsort2(rsmxcount2)
				rsmxid2(rsmxcount2) = rsmx(0).value
				rsmxname2(rsmxcount2) = rsmx(1).value
				rsmxsort2(rsmxcount2) = rsmx(2).value
				rsmx.movenext
			wend
			rsmx.close
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select top 1 name from tel x inner join caigou y  on x.sort3=2 and  x.ord=y.company and y.ord=kuin.caigou) as telname, " & _
			"(select top 1 name from gate y where y.ord=kuin.cateid) as cateidname,"&_
			"(select top 1 name from gate y where y.ord=kuin.kg) as kgname,"&_
			"(case when sort1=1 then (select top 1 title from caigou where ord=kuin.caigou) else '' end) as caigouTitle,"&_
			"CONVERT(VARCHAR(20),date3,120) date30,CONVERT(VARCHAR(20),date5,120) date50, " & _
			"(select isnull(sum(num1),0) from kuinlist x where x.kuin=kuin.ord) as num2 from kuin  "&Str_Result&" "&px_Result&""
			rs.open sql,conn,3,1
			C1=rs.recordcount
			dim i , inx ,SortTypeSource, arrtypes
			SortTypeSource = KuinSortTypeSource
			arrtypes = Split(SortTypeSource,";")
			i=1
			j=1
			Dim PreProcV, currProcV
			PreProcV = -1
'Dim PreProcV, currProcV
			...
			do until rs.eof
				...
				sort=rs("sort1")
				rklb=""
				For inx = 0 To ubound(arrtypes)
					if sort&""=Split(arrtypes(inx),"=")(1)&"" Then  rklb = Split(arrtypes(inx),"=")(0) :Exit For
				next
				...
				if cateout<>"" then
					...
					if rs7.eof then
						...
						cateoutname=rs7("name")
					end if
					rs7.close
					set rs7=nothing
				end if
				xsheet.writestr rkid
				xsheet.writestr title
				xsheet.writenumber num2
				xsheet.writestr rklb
				xsheet.writestr zt
				xsheet.writestr kgname
				xsheet.writecell date3
				xsheet.writestr cateidname
				xsheet.writecell date5
				xsheet.writestr cateoutname
				For m = 0 To rsmxcount2
					if rsmxsort2(m)=1 then
						zdy=rs(rsmxname2(m))
						...
						xsheet.writestr zdyname
					else
						xsheet.writestr  rs(rsmxname2(m)).value
					end if
				next
				xsheet.writestr rs("intro2")
				xsheet.writestr rs("caigouTitle")
				xsheet.writestr name1
				i = 0
				sql1 = "select x.*,isnull(kilf.money,0) ftmoney," & _
				"(case when (case when x.sort1 in (1,2) then x.num2 else x.num1 end)=0 then 0 "&_
				"  else isnull(x.money2,0)/(case when x.sort1 in (1,2) then x.num2 else x.num1 end)*b.num3 "&_
				"end) as ymoney2,"&_
				"isnull(x.price1,0) as cbprice1,"&_
				"isnull(x.Money1,0)  as cbmoney1,"&_
				"(select top 1 sort1 from sortonehy a where a.ord=x.bz) as bzname," & _
				"(select top 1 sort1 from sortonehy a where a.ord=x.unit) as unitname," & _
				"(select top 1 sort1 from sortck a where a.ord=x.ku) as ckname," & _
				"isnull(y.title,'产品已被删除') as p_title, y.order1 as p_order1, y.type1 as p_type1," & _
				"(select top 1 sort1 from sortonehy a where a.ord=x.zdy5) as zdy5name," & _
				"(select top 1 sort1 from sortonehy a where a.ord=x.zdy6) as zdy6name " & _
				"from kuinlist x "&_
				"inner join ku b on x.id=b.kuinlist "&_
				"left join product y on x.ord=y.ord  " & _
				"left join caigou_CostSharing_RKList kilf on kilf.kuinlist=x.id and kilf.del=1 "&_
				"where x.kuin="&id&" order by x.id asc"
...
'do until rs1.eof
						...
						If isnull(price1)  Or price1="" Then price1=0
						price1=replace(price1,",","")
						price2=zbcdbl(rs1("price2"))
						If isnull(price2)  Or price2="" Then price2=0
						...
						type1=rs1("p_type1")
						ftmoney=rs1("ftmoney")
						cbprice1=rs1("cbprice1")
						cbmoney1=rs1("cbmoney1")
						sortname = rs1("bzname").value & ""
						ckname = rs1("ckname").value & ""
						if ftmoney&"" = "" then ftmoney = 0 else ftmoney = cdbl(ftmoney)
						if cbprice1&"" = "" then cbprice1 = 0 else cbprice1 = cdbl(cbprice1)
						if cbmoney1&"" = "" then cbmoney1 = 0 else cbmoney1 = cdbl(cbmoney1)
						If i=1 Then
							xsheet.movenext
						end if
						For iiii = 0 To cellCountBeforeDetail - 1
							xsheet.movenext
							xsheet.writecell ""
						next
						xsheet.writecell "产品明细"
						For iii=0 To rsmxcount
							mx_name = rsmxname(iii)
							mx_sorce = rsmxsorce(iii)
							Select Case mx_sorce
							Case 5:
							If open_23_3 <> 0 then
								xsheet.writemoney price2
							end if
							Case 7
							If open_23_3<>0 Then
								If isnull(money2)  Or money2="" Then money2=0
								money2=replace(money2,",","")
								xsheet.writemoney money2
								if isShowFyFt then
									xsheet.writemoney ftmoney
								end if
								xsheet.writemoney cbprice1
								xsheet.writemoney cbmoney1
							end if
							xsheet.writestr ph
							xsheet.writestr xlh
							...
							Case else
							If mx_sorce>=10 and mx_sorce<=15 Then
								if trim(mx_name)="zdy5" Then
									zdyname = rs1("zdy5name")
								ElseIf  trim(mx_name)="zdy6" then
									zdyname = rs1("zdy6name")
								else
									zdyname=rs1(mx_name)
								end if
								xsheet.writestr zdyname
						...
						If rowIndex > 40000 Then
							If pageCount = 1 then
								xsheet.title = "入库单(" & pageCount & "页)"
							end if
							pageCount = pageCount + 1
							xsheet.title = "入库单(" & pageCount & "页)"
							Set xsheet = xlsapp.sheets.add("入库单(" & pageCount & "页)")
							rowIndex = 0
							Call createXlsHead
							xsheet.movenext
						end if
				...
				rs1.close
				set rs1=nothing
				currProcV = Clng(FormatNumber(j/C1*100,4,-1))
				set rs1=nothing
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>入库单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!入库单导出成功,共有"&(j-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			tfile=Server.MapPath(xlsApp.cFileName("入库单") & ".xls")
			xlsApp.save tfile
			xlsApp.dispose
			tfile = xlsApp.HexEncode(tfile)
			Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
			Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'入库单_"&session("name2006chen")&".xls',fileCnt:1})</script>"
			Set xlsApp = Nothing
			action1="入库单导出"
			call close_list(1)
			Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的入库单</u></strong></font></a></p>" & vbcrlf & ""
