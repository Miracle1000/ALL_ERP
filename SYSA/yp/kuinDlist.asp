...
'if strW3<>"" Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {     margin-top: 0px;}" & vbcrlf & "-->" & vbcrlf & "#kh{height:20px!important;line-height:20px!important;}" & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "#ht1 table{margin-bottom:-1px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src=""../inc/loadingBar.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<link rel=""stylesheet"" type=""text/css"" href=""../inc/themes/default/easyui.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src= ""../Script/yp_kuinDlist.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if open_101_1=3 then
		list=" and 1=1 "
	elseif open_101_1=1 then
		list=" and reg_addcateid in ("&intro_101_1&")"
	else
		list=" and reg_addcateid=0"
	end if
	Str_Result=" where reg_del=1 "&list&""
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by reg_gztime desc"
	elseif px="2" then
		Ptr_Result=" order by reg_gztime asc"
	elseif px="3" then
		Ptr_Result=" order by prod_sctime desc"
	elseif px="4" then
		Ptr_Result=" order by prod_sctime asc"
	elseif px="5" then
		Ptr_Result=" order by prod_ystime desc"
	elseif px="6" then
		Ptr_Result=" order by prod_ystime asc"
	end if
	ret          =request("ret")
	ret2 =request("ret2")
	A    =request("A")
	...
	W2=tmparr(1)
	if A<>"" then
		Str_Result=Str_Result&" and a.prod_unit in ("&A&") "
	end if
	if B<>"" then
		Str_Result=Str_Result&" and a.sto_id in ("&B&") "
	end if
	if C<>"" then
		Str_Result=Str_Result&" and a.prod_gys like '%"&C&"%' "
	end if
	if D<>"" then
		Str_Result=Str_Result&" and reg_name like '%"&D&"%' "
	end if
	if E<>"" then
		Str_Result=Str_Result&" and f.prod_name like '%"&E&"%' "
	end if
	if F<>"" then
		Str_Result=Str_Result&" and f.prod_bh like '%"&F&"%' "
	end if
	if G<>"" then
		Str_Result=Str_Result&" and f.prod_xh like '%"&G&"%' "
	end if
	if H<>"" then
		Str_Result=Str_Result&" and a.prod_ph like '%"&H&"%' "
	end if
	if I<>"" then
		Str_Result=Str_Result&" and a.prod_xlh like '%"&I&"%' "
	end if
	if J<>"" then
		Str_Result=Str_Result&" and a.reg_intro like '%"&J&"%' "
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&"',a.prod_ystime)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,a.prod_ystime,'"&L2&"')>=0 "
'if L2<>"" then
	end if
	if ret <>"" and not isnull(ret) then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',a.reg_gztime)>=0 "
'if ret <>"" and not isnull(ret) then
	end if
	if ret2 <>"" and not isnull(ret2) then
		Str_Result=Str_Result+" and  datediff(s,a.reg_gztime,'"&ret2&" 23:59:59')>=0 "
'if ret2 <>"" and not isnull(ret2) then
	end if
	if M <>"" and not isnull(M) then
		Str_Result=Str_Result+" and  datediff(s,'"&M&" 00:00:00',a.prod_sctime)>=0 "
'if M <>"" and not isnull(M) then
	end if
	if K <>"" and not isnull(K) then
		Str_Result=Str_Result+" and  datediff(s,a.prod_sctime,'"&K&" 23:59:59')>=0 "
'if K <>"" and not isnull(K) then
	end if
	if N <>"" and not isnull(N) then
		Str_Result=Str_Result+" and  f.prod_type in("&N&")"
'if N <>"" and not isnull(N) then
	end if
	if S <>"" and not isnull(S) then
		Str_Result=Str_Result+" and  c.pro_type in("&S&")"
'if S <>"" and not isnull(S) then
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and  b.reg_userid in ("&W3&") "
	end if
	...
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <form name=""data"" id=""data"" action=""kuinDList.asp"" method=""get"">" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">入库明细表</td>" & vbcrlf & "                       <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"" class=""top_btns""><div id=""kh"">" & vbcrlf & "                       "
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "<input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_kuinDlist.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&S="
	...
	Response.write "" & vbcrlf & "                     <input name=""page_count"" type=""hidden"" value="""
	Response.write page_count
	Response.write """><input name=""px"" type=""hidden"" value="""
	Response.write px
	Response.write """><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                         <option>-请选择-</option>" & vbcrlf & "                    <option value=""page_count=10"" "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "                </select><a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "        <span id=""ht1""></span>" & vbcrlf & "  </form>"& vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">      " & vbcrlf & "                 <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""9%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库主题</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库人员</div></td>" & vbcrlf & "                 <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库时间</div></td>" & vbcrlf & "                 <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">名称</div></td>" & vbcrlf & "             <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">编号</div></td>" & vbcrlf & "             <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">型号</div></td>" & vbcrlf & "           <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">单位</div></td>" & vbcrlf & "             <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">数量</div></td>" & vbcrlf & "             <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">单价</div></td>" & vbcrlf & "              <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">总价</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">入库仓库</div></td>" & vbcrlf & "            <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">购置时间</div></td>" & vbcrlf & "         <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">供应商</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""name""><div align=""center"">操作</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "           "
	currpage=Request("currpage")
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,f.id as aid,b.reg_name,b.reg_addcateid,b.reg_userid,b.reg_time,c.pro_name,d.sort1,e.name,f.prod_name,f.prod_bh,f.prod_xh,k.sto_name from O_kuinList a "&_
	" inner join O_kuin b on a.reg_fid=b.id "&_
	" left join O_product f on a.prod_id=f.id "&_
	" left join O_OfficeType c on f.prod_type=c.id "&_
	" left join sortonehy d on a.prod_unit=d.id "&_
	" left join O_store k on a.sto_id=k.id "&_
	" left join gate e on b.reg_userid=e.ord "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=14><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		allnum=0
		allprice=0
		...
		do while not rs7.eof
			addcateid=rs7("reg_addcateid")
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "              <td width=""9%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "            "
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('kuinDetail.asp?ord="
				Response.write rs7("reg_fid")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("reg_fid")
				Response.write rs7("reg_name")
				Response.write "</a>" & vbcrlf & "         "
			else
				Response.write rs7("reg_name")
			end if
			Response.write "</td>" & vbcrlf & "                   <td width=""100"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                    "
			reg_userid=rs7("reg_userid")
			if reg_userid="" or isnull(reg_userid) then reg_userid=0
			sql8="select  * from gate where ord="&reg_userid
			...
			rs8.close
			set rs8=nothing
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "                <td width=""8%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("reg_time")
			...
			if open_101_16=3 or CheckPurview(intro_101_16,trim(addcateid))=True then
				...
				Response.write rs7("aid")
				Response.write "','neww35win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"">" & vbcrlf & "                  "
				'Response.write rs7("aid")
			end if
			Response.write rs7("prod_name")
			Response.write "</a></div></td>" & vbcrlf & "                 <td width=""8%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("prod_bh")
			Response.write "</div></td>" & vbcrlf & "             <td width=""8%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("prod_xh")
			Response.write "</div></td>" & vbcrlf & "         <td width=""6%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("sort1")
			Response.write "</div></td>" & vbcrlf & "         <td width=""6%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write FormatNumber(zbcdbl(rs7("prod_allnum")),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "         <td width=""6%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs7("prod_price")),CommPrice_dot_num,-1)
			Response.write "</div></td>" & vbcrlf & "         <td width=""6%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs7("prod_allmoney")),num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "         <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("sto_name")
			Response.write "</div></td>" & vbcrlf & "         <td width=""8%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("reg_gztime")
			Response.write "</div></td>" & vbcrlf & "         <td width=""8%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("prod_gys")
			Response.write "<input name=""a1"" id=""a1_"
			Response.write rs7("id")
			Response.write """ type=""hidden"" value="""
			Response.write rs7("prod_ph")
			Response.write """><input name=""a2"" id=""a2_"
			Response.write rs7("id")
			Response.write """ type=""hidden"" value="""
			Response.write rs7("prod_xlh")
			Response.write """><input name=""a3"" id=""a3_"
			Response.write rs7("id")
			Response.write """ type=""hidden"" value="""
			Response.write rs7("prod_sctime")
			Response.write """><input name=""a4"" id=""a4_"
			Response.write rs7("id")
			Response.write """ type=""hidden"" value="""
			Response.write rs7("prod_ystime")
			Response.write """></div></td>" & vbcrlf & "            <td align=""left"" valign=""middle""  class=""func""><div align=""center""><a href=""###"" onClick=""openDiv('"
			Response.write rs7("id")
			Response.write "')"">更多</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "             "
			num1=zbcdbl(rs7("prod_allnum"))
			price1=zbcdbl(rs7("prod_price"))
			money1=zbcdbl(rs7("prod_allmoney"))
			if num1="" or isnull(num1) then num1=0
			if price1="" or isnull(price1) then price1=0
			if money1="" or isnull(money1) then money1=0
			allnum=cdbl(allnum)+cdbl(num1)
'if money1="" or isnull(money1) then money1=0
			allprice=cdbl(allprice)+cdbl(price1)
'if money1="" or isnull(money1) then money1=0
			allmoney=cdbl(allmoney)+cdbl(money1)
'if money1="" or isnull(money1) then money1=0
			o=o+1
'if money1="" or isnull(money1) then money1=0
	...
	Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "              <td height=""25"" colspan=""7"" align=""left"" valign=""middle""  class=""name""><div align=""right"">合计：</div></td>" & vbcrlf & "                 <td width=""80"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write FormatNumber(allnum,num1_dot,-1)
	Response.write "</div></td>" & vbcrlf & "         <td width=""80"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write Formatnumber(allprice,CommPrice_dot_num,-1)
	Response.write "</div></td>" & vbcrlf & "         <td width=""80"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write Formatnumber(allmoney,num_dot_xs,-1)
	Response.write "</div></td>" & vbcrlf & "         <td colspan=""4"" align=""left"" valign=""middle""  class=""name"">&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "    </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%""border=""0"" align=""left"">" & vbcrlf & "         <tr>" & vbcrlf & "          <td width=""60"" height=""30""></td>" & vbcrlf & "            <td></td>         " & vbcrlf & "      <form name=""oneself"" action=""kuinDList.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&S="
	...
	Response.write "条信息/页            " & vbcrlf & "                <input name=""CurrPage"" id=""CurrPage"" type=""text"" size=""4"" maxlength=""8"" onpropertychange=""formatData(this,'int');"" onKeyUp=""checkDot('CurrPage','0')"" />" & vbcrlf & "              <input name=""btntz"" type=""submit""  class=""anybutton2"" id=""btntz"" value=""跳转"" />"& vbcrlf & "              "
	if currpage=1 then
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td></form>" & vbcrlf & "      </tr>" & vbcrlf & "  <tr><td height=""2000""></td></tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:300; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""200""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""192"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按购置时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按购置时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按生产时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按生产时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按有效时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按有效时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "   </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="入库明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "" & vbcrlf & "<div id=""ccc"" class=""easyui-dialog"" icon=""icon-search"" title=""更多信息""  style=""width:510px;height:240px;padding:5px;background:#fafafa;top:0px"" closed=""true"" >" & vbcrlf & " <div region=""center"" border=""false"" style=""box-sizing:border-box;padding:10px;background:#fff;border:1px solid #ccc;width:100%"">" & vbcrlf & "               <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""  style=""box-sizing:border-box;word-break:break-all;word-wrap:break-word;""> " & vbcrlf & "                     <tr>" & vbcrlf & "                            <td height=""27""><div align=""center""><strong>生产日期</strong></div></td>" & vbcrlf & "                          <td height=""27""><div align=""center""><strong>有效日期</strong></div></td>" & vbcrlf & "                           <td height=""27""><div align=""center""><strong>批号</strong></div></td>" & vbcrlf & "                               <td height=""27""><div align=""center""><strong>序列号</strong></div></td>                             " & vbcrlf & "                        </tr>" & vbcrlf & "                   <tr>" & vbcrlf & "                            <td height=""27""><div align=""center""><span id=""date2""></span></div></td>" & vbcrlf & "                               <td height=""27""><div align=""center""><span id=""date3""></span></div></td>" & vbcrlf & "                               <td height=""27""><div align=""center""><span id=""txt_ph""></span></div></td>" & vbcrlf & "                               <td height=""27""><div align=""center""><span id=""txt_xlh""></span></div></td>" & vbcrlf & "                     </tr>" & vbcrlf & "           </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "              <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "           <input type=""hidden"" name=""page_count"" value="""
	'call close_list(1)
	Response.write page_count
	Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
	Response.write px
	Response.write """><br />" & vbcrlf & "          <a class=""easyui-linkbutton"" icon=""icon-cancel"" href=""javascript:void(0)"" onClick=""$('#ccc').window('close');"">关闭</a>" & vbcrlf & " </div>" & vbcrlf & "</div>" & vbcrlf & "<!--<input type=button value=""test"" onclick=""testf();"">" & vbcrlf & "<script>" & vbcrlf & "function testf()" & vbcrlf & "{" & vbcrlf & "    var a=document.getElementsByName(""A"");" & vbcrlf & "    for(var i=0;i<a.length;i++)" & vbcrlf & "     {" & vbcrlf & "               //alert(""a""+i+"":tag=""+a[i].tagName+"",a.type=""+a[i].type+"",name=""+a[i].name+"",id=""+a[i].id);" & vbcrlf & "               if(a[i].type==""hidden"")alert(a[i].parentElement.parentElement.parentElement.outerHTML);" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "--></body>" & vbcrlf & "</html>" & vbcrlf & ""
	Response.write px
