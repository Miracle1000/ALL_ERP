...
Dim allowExt
allowExt="|doc|docx|ppt|pptx|xls|xlsx|avi|bmp|jpeg|png|rmvb|gif|jpg|txt|pdm|zip|7z|rar|iso|apk|pdf|dwg|tif|mp3|mp4|tiff|wps|"
Dim MaxUploadSize
MaxUploadSize=52428800
sub getUploadFileList(cn,OrderID,mailType,edit)
sql="select * from email_recv_Access where mailType="&mailType&" and del=1 and email_ord="&OrderID&""
...
if CheckLocalFileExist(FileName) then
...
about=""
workid = 0
if action="resume" and isnumeric(interviewID) and interviewID<>"" then '
set rs=server.CreateObject("adodb.recordset")
sql="select top 1 * from hr_Resume where id in(select isnull(resumeID,0) resumeID from hr_interview where id="&interviewID&" and not exists(select top 1 interviewID from hr_person where del=0 and interviewID>0 and interviewID="&interviewID&") and del=0) and isnull(del,0)=0 order by id desc"
rs.open sql,conn,1,1
if not rs.eof then
newPersonName=rs("userName")
resumeID=rs("id")
cardId=rs("cardID")
Account=rs("Account")
sex=rs("sex")
if instr(sex,"女")>0 then
sex=0
else
sex=1
end if
...
If resumeID = 0 Then
set rs=server.CreateObject("adodb.recordset")
sql="select * from hr_interview where id="&interviewID&" and not exists(select top 1 interviewID from hr_person where del=0 and interviewID>0 and interviewID="&interviewID&") and del=0"
rs.open sql,conn,1,1
if not rs.eof then
newPersonName = rs("userName")
resumeID = -1
...
workid = 0
gateid=0
Elseif action="gate" and isnumeric(gateid) and gateid<>"" Then   '
interviewID=0
Set Rs=server.CreateObject("adodb.recordset")
Sql="select ord,ygid,cardid,sex,jg,mz,date1,date2,date3,phone1,phone2,mobile,fax,email,address,alt,datealt,tc,ah,intro,jjgz,salaryClass,workPosition from Gate where   ord="&gateid&" "
rs.open sql,conn,1,1
If not rs.eof then
gateid=Rs("ord")
bh=Rs("ygid")
...
mz=Rs("mz")
birthday=Rs("date1")
contractStart=Rs("date2")
contractEnd=Rs("date3")
OffTel=Rs("phone1")
homeTel=Rs("phone2")
mobile=Rs("mobile")
fax=Rs("fax")
email=Rs("email")
homeAddress=Rs("address")
aContract=Rs("alt")
aDay=Rs("date3")-Rs("datealt")
aContract=Rs("alt")
about=Rs("tc")
intro=Rs("intro")
jjSalarry=Rs("jjgz")
if isnumeric(aDay)=false  then aDay=0
salaryClass=rs("salaryClass")
...
'Response.write Application("sys.info.jsver")
Response.write Application("sys.info.jsver")
Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script type=""text/javascript"" src=""../inc/jquery.easyui.min.js?ver="
Response.write Application("sys.info.jsver")
Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFiles.js?ver="
Response.write Application("sys.info.jsver")
Response.write """></script>" & vbcrlf & ""
...
set rs1=nothing
if open_1_1=1 Then
If Len(w3_list&"") = 0 Then w3_list = "0"
...
Response.write " " & vbcrlf & "" & vbcrlf & "" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "    window.yearBirth = """
Response.write year(birthday)
Response.write """;" & vbcrlf & "    window.monthBirth = """
Response.write month(birthday)
Response.write """;" & vbcrlf & "    window.dayBirth = """
Response.write day(birthday)
Response.write """;" & vbcrlf & "" & vbcrlf & "    // 验证邮箱格式，并追加提示信息以及样式" & vbcrlf & "    // 解决[BUG:53654] 临后Bug：档案导入提示电子邮件格式有误 添加" & vbcrlf & "    function checkEmail () {" & vbcrlf & "        var email = $(""#email"").val();" & vbcrlf & "        if (email != """")" & vbcrlf & "        {" & vbcrlf & "       var regEmail = /\w+[@]{1}\w+[.]\w+/;" & vbcrlf & "            if (!regEmail.test(email)) {" & vbcrlf & "                $(""#msgEmail"").html(""邮箱格式不正确"");" & vbcrlf & "                $(""#msgEmail"").css(""color"", ""#ff0705"");" & vbcrlf & "            }" & vbcrlf & "            else {" & vbcrlf & "                $(""#msgEmail"").html("""");" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        else" & vbcrlf & "        {" & vbcrlf & "            $(""#msgEmail"").html("""");" & vbcrlf & "        }" & vbcrlf & "    };" & vbcrlf & "" & vbcrlf & "    " & vbcrlf & "" & vbcrlf & "        function addWorkList()" & vbcrlf & "        {" & vbcrlf & "            var date=new Date();" & vbcrlf & "            var trID=date.getHours()+""-""+date.getMinutes()+""-""+date.getSeconds();" & vbcrlf & "            var tempStr=""<tr class='""+trID+"" top'><td colspan='6' >&nbsp;</td></tr><tr class='""+trID+""'><td ><div align='right'>起始日期：</div></td>""" & vbcrlf & "                                    +""<td class='gray'><input name='workStartTime""+trID+""' type='text' Class='DatePick'  onclick='datedlg.show();' readonly='readonly' size='15' dataType='Limit' min='0' max='50'  >""" & vbcrlf & "                                    +""</td>""" & vbcrlf & "                                    +""<td><div align='right'>结束日期：</div></td>""" & vbcrlf & "                                    +""<td class='gray'><input name='workEndTime""+trID+""' type='text' Class='DatePick'  onclick='datedlg.show();' readonly='readonly'  size='15' dataType='Limit' min='0' max='50'  >""" & vbcrlf & "                                    +""<div id=e_daysOfMonth""+trID+"" style='position: absolute'></div></td>""" & vbcrlf & "                                    +""<td ><div align='right'>证 明 人：</div></td>""" & vbcrlf & "                                    +""<td  ><input name='workZhengMing' type='text'  size='15' dataType='Limit' min='0' max='50'  >&nbsp;&nbsp;<span ><img src='../images/smico/icon_del.gif' width='19' height='17' alt='删除' onClick='delClassItem(this)'></span>""" & vbcrlf & "  +""</td>""" & vbcrlf & "                                    +""</tr>""" & vbcrlf & "                                    +""<tr class='""+trID+""'>""" & vbcrlf & "                                    +""<td><div align='right'>公司名称：</div></td>""" & vbcrlf & "          +""<td><input name='workCompany' type='text'  size='20' dataType='Limit' min='0' max='50'  ></td>""" & vbcrlf & "                                    +""<td><div align='right'>担任职务：</div></td>""" & vbcrlf & "                                    +""<td class='gray'><input name='workPosition' type='text'  size='15'  dataType='Limit' max='50'  ></td>""" & vbcrlf & "                                    +""<td><div align='right'>电　　话：</div></td>""" & vbcrlf & "                                    +""<td class='gray'><input name='workTel' type='text' id='workTel""+trID+""'  size='15'  dataType='Limit' max='50'  >""" & vbcrlf & "                                    +"""
'Response.write day(birthday)
if tp=1 then
Response.write """" & vbcrlf & "                                    +""<img src='../images/15.gif' border='0' align='absbottom' style=' cursor:pointer' onClick='phoneCall(document.getElementById(\""workTel\"").value)'>""" & vbcrlf & "                                    +"""
'if tp=1 then
end if
Response.write "</td>""" & vbcrlf & "                                    +""</tr>""" & vbcrlf & "                                    +""<tr class='""+trID+""'>""" & vbcrlf & "                                    +""<td><div align='right'>工作内容：</div></td>""" & vbcrlf & "                                    +""<td colspan='5' class='gray'><textarea name='workContent'  cols='80' rows='4'  dataType='Limit' max='4000'  msg='最多4000个字'></textarea></td>""" & vbcrlf & "                                    +""</tr>""" & vbcrlf & "                                    +""<tr class='""+trID+""'>""" & vbcrlf & "         +""<td><div align='right'>工作业绩：</div></td>""" & vbcrlf & "                                    +""<td colspan='5' class='gray'><textarea name='workJiXiao'  cols='80' rows='4'  dataType='Limit' max='4000'  msg='最多4000个字'></textarea></td>""" & vbcrlf & "                      +""</tr>""" & vbcrlf & "                                    +""<tr class='""+trID+""'>""" & vbcrlf & "                                    +""<td><div align='right'>离职原因：</div></td>""" & vbcrlf & "                                    +""<td colspan='5' class='gray'><div align='left'>""" & vbcrlf & "                                    +""<textarea name='workLever' cols='80' rows='4'   dataType='Limit' max='4000'  msg='最多4000个字'></textarea>""" & vbcrlf & "                                    +""</div></td></tr>"";" & vbcrlf & "" & vbcrlf & "            $(""#WorkList"").append(tempStr);" & vbcrlf & "            try" & vbcrlf & "            { parent.frameResize();}" & vbcrlf & "            catch(e){}" & vbcrlf & "        }" & vbcrlf & "        window.OnFieldAutoCompleteCallBack = function(data) {" & vbcrlf & "            document.getElementById(""orgspathtxt"").innerHTML = data.tag.fullpath;" & vbcrlf & "            document.getElementById(""orgsid"").value = data.value;" & vbcrlf & "        }" & vbcrlf & "" & vbcrlf & "        marryChange = function () {" & vbcrlf & "            var obj = $(""[name='marry']:checked"");" & vbcrlf & "            if (obj.val() == ""1"") {" & vbcrlf & "                $(""#WeddingDaySpan"").css(""display"", """");" & vbcrlf & "                $(""#WeddingDaySpan span"").show();" & vbcrlf & "                //$(""#WeddingDay"").attr(""min"", ""1"");" & vbcrlf & "                return;" & vbcrlf & "            }" & vbcrlf & "            $(""#WeddingDaySpan"").css(""display"", ""none"");" & vbcrlf &  "           $(""#WeddingDaySpan span"").hide();" & vbcrlf & "            $(""#WeddingDay"").attr(""min"", ""0"");" & vbcrlf &  "           $(""#WeddingDay"").val(""""); "& vbcrlf & "        } "& vbcrlf & vbcrlf &  "       // 控制提示信息是否显示" & vbcrlf & "        function isShow()" & vbcrlf & "        {" & vbcrlf & "            var doc = document.getElementById(""isShowDiv"");" & vbcrlf & "            if (doc.style.display == ""inline"")" & vbcrlf & "            {" & vbcrlf & "                doc.style.display = ""none""" & vbcrlf & "}" & vbcrlf & "            else {" & vbcrlf & "                doc.style.display = ""inline""" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "<script type=""text/javascript"" src= ""../Script/hm_personAdd.js?ver="
Response.write Application("sys.info.jsver")
Response.write """  language=javascript></script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {   margin-top: 0px;}" & vbcrlf & ".style1 {color: #0066FF}" & vbcrlf & "-->" & vbcrlf & "#content #atttb tr.top td{border:1px solid #c0ccdd!important}" & vbcrlf & "#atttb{border-collapse:collapse}" & vbcrlf & "input[name='edu_StartTime'],input[name='edu_EndTime']{" & vbcrlf & "    width:126px;" & vbcrlf & "}" & vbcrlf & "input#addList {" & vbcrlf & "    margin-left: 28px;" & vbcrlf & "}" & vbcrlf & ".DatePick{" & vbcrlf & "    background-position:108px center!important;" & vbcrlf &"    width:126px;" & vbcrlf & "}" & vbcrlf & "#w{" & vbcrlf & "    height:182px!important;" & vbcrlf & "}" & vbcrlf & ".home tr.top td{font-weight:400!important;text-align:left!important;}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body "
'Response.write Application("sys.info.jsver")
if open_82_8=0 then
...
Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place""><p>员工档案添加</p></td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right""><input type=""submit""  class=""anybutton2"" name=""Submit4222""  onClick=""javascript:window.open('person_load.asp','newload','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=100,top=100')"" value=""账号转档案"" /></td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "     <form action=""person_save.asp?action=add&interviewID="
Response.write interviewID
Response.write """ method=""post"" name=""demo"" id=""demo"" onsubmit=""return Validator.Validate(this,2)&&__CheckDate()&&checkSC()"" style=""margin:0"" target=""I4"">" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "    <tr class=""top"">" & vbcrlf & "            <td height=""27""  ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_1')"">基本情况</a></div></td>" & vbcrlf & "            <td height=""27"" colspan=""5"" align=""right"" ><div align=""right"">" & vbcrlf &  "               <input type=""hidden"" name=""tbinterviewID"" value="
Response.write interviewID
Response.write """>" & vbcrlf & "                <input type=""submit"" onclick=""checkEmail()"" name=""Submit422"" value=""保存""  class=""page""/>" & vbcrlf & "                <input type=""reset"" value=""重填"" class=""page"" name=""B22"">" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""11%""><div align=""right"">姓名：</div></td>" & vbcrlf & "            <td colspan=""3""><div align=""left"" class=""gray"">" & vbcrlf & "                <select name=""gateid"" id=""gateid"" datatype=""Limit"" onChange=""getNewPerson(this.value)"" min=""1"" msg=""必选"">" & vbcrlf & "                  <OPTION  disabled=""disabled"" "
if isnumeric(resumeID)=false or  resumeID=0 then
Response.write "selected=""selected"""
end if
Response.write " value="""">请选择人员</OPTION>" & vbcrlf & "                  <OPTION "
if isnumeric(resumeID) and resumeID<>0 then
Response.write "selected=""selected"""
end if
Response.write " value=""0"">新建人员</OPTION>" & vbcrlf & "                  "
set rs=server.CreateObject("adodb.recordset")
sql="select ord,name from gate  where del=1 "& str_w3 &" and ord not in(select userID from hr_person where del=0) "&list_gate&""
rs.open sql,conn,1,1
if not rs.eof then
do while not rs.eof
...
Response.write """ "
if int(gateid)=rs("ord") Then
newPersonName = rs("name")
...
rs.close
set rs=nothing
Response.write "" & vbcrlf & "                </select>" & vbcrlf & "                            <span id='newpersondiv' "
If interviewID =0 then
Response.write "style='display:none'"
end if
Response.write ">" & vbcrlf & "                             <input name=""PersonName"" min=""1"" type=""text"" value="""
Response.write newPersonName
Response.write """  id=""PersonName"" size=""12"" datatype=""Limit"" max=""50"" msg=""长度必须在1到50个字符之间"">" & vbcrlf & "                <span class='red'>*</span>&nbsp;" & vbcrlf & "                             <input id='orgsid' name='orgsid' type='hidden' value=''>" & vbcrlf & "                                <span id='orgspathtxt'></span>&nbsp;" & vbcrlf & "                    <a href='javascript:void(0)' onclick='window.open(""../../SYSN/view/magr/OrganizList.ashx?linkbar=1"",""personorgsdlg"",""width=450px,height=500px,top=20px,left=100px,scrollbars=1,resizable=1"")'>选择部门</a>" & vbcrlf & "                            &nbsp;<input type='checkbox' value='1' name='partadmin' id='partadmin' "
Response.write partadmin
Response.write " style='position:relative;top:2px'><label for='partadmin'>部门管理员</label>" & vbcrlf & "                </span> " & vbcrlf & "                <img src=""../../SYSN/skin/default/img/help.png"" style=""margin-left:10px; height:19px;width:19px;vertical-align: middle;"" onclick=""isShow()""/>" & vbcrlf & "                <div id=""isShowDiv"" style=""display:none;font-size:15px;"">下拉框中姓名是有账号无档案人员，选择对应姓名即可添加档案</div>" & vbcrlf & "                </div>" & vbcrlf & "            </td>" & vbcrlf & "            <td width=""11%"" height=""100"" rowspan=""3""><div align=""right"">照片：</div></td>" & vbcrlf & "            <td width=""20%"" height=""100"" rowspan=""3""><img id=""imageField"" src="""
if len(trim(picname))>1 then Response.write "../person/load/"& picname else Response.write "../person/img/noneperson.jpg" end if
Response.write """ width=""90"" height=""100"" alt=""头像""><input type=""hidden"" id=""imgurl1"" name=""imgurl1"" value=""""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""12%""><div align=""right"">曾用名：</div></td>" & vbcrlf & "            <tdwidth=""22%"" class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""zName"" type=""text""  id=""zName"" size=""15"" datatype=""Limit"" max=""50"" msg=""0~50个字符"">" & vbcrlf & "              </div></td>" & vbcrlf & "            <td width=""11%""><div align=""right"">英文名：</div></td>" & vbcrlf & "            <td width=""24%"" class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""eName"" type=""text""  id=""eName"" size=""15"" dataType=""Limit"" max=""50"" msg=""0~50个字符"">" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""12%""><div align=""right"">员工编号：</div></td>" & vbcrlf & "            <td  class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""bh"" type=""text"" id=""bh"" size=""15""  dataType=""Limit"" value="""
Response.write bh
Response.write """ max=""50"" min=""1"" msg=""必填""  >" & vbcrlf & "                <span class=""red"">*</span></div></td>" & vbcrlf & "            <td ><div align=""right"">身份证号：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""idCard"" type=""text"" id=""idCard"" value="""
Response.write cardId
Response.write """ size=""25"" dataType=""Limit"" max=""50"" msg=""0~50个字符"" ></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">户口所在地：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "        <input name=""fkAddress"" type=""text"" id=""fkAddress"" value="""
Response.write Account
Response.write """ size=""25"" dataType=""Limit"" max=""50"" msg=""0~50个字符"" >" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">档案所在地：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""daAddress"" type=""text"" id=""daAddress"" size=""25"" dataType=""Limit"" max=""50"" msg=""0~50个字符"" >" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">上传：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left""><input type=""button"" name=""openbn"" id=""openbn"" value=""上传头像""  class=""anybutton"" onclick=""$('#w').window('open');""/></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">性别：</div></td>" & vbcrlf & "            <td><div align=""left"" class=""gray"">" & vbcrlf & "                <input name=""sex"" type=""radio"" value=""1"" "
if sex=1 then
Response.write " checked"
end if
Response.write ">" & vbcrlf & "                男" & vbcrlf & "                <input type=""radio"" name=""sex"" value=""0"" "
if sex=0 then
Response.write " checked"
end if
Response.write ">" & vbcrlf & "                女</div></td>" & vbcrlf & "            <td><div align=""right"">籍贯：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""jgAddress"" id=""jgAddress"" type=""text"" value="""
Response.write address
Response.write """ size=""25"" dataType=""Limit"" max=""50""  msg=""0~50个字符"">" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">民族：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""nation"" type=""text"" id=""nation"" size=""15""  value="""
Response.write mz
Response.write """ dataType=""Limit"" max=""50"" msg=""0~50个字符"" >" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">婚否：</div></td>" & vbcrlf & "            <td class=""gray"">" & vbcrlf & "         <input name=""marry"" type=""radio"" onclick=""marryChange()"" value=""0"" "
if Maryy=0 then
Response.write "checked"
end if
Response.write ">未婚" & vbcrlf & "              <input type=""radio"" name=""marry"" onclick=""marryChange()"" value=""1"" "
if Maryy=1 then
Response.write "checked"
end if
Response.write ">已婚" & vbcrlf & "              <span id=""WeddingDaySpan"" style=""display:none;"">" & vbcrlf & "                  登记: <input name=""WeddingDay"" type=""text"" id=""WeddingDay"" value="""
Response.write WeddingDay
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" max=""100"" min=""0"">" & vbcrlf & "                  <DIV id=daysOfMonth10 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </span>" & vbcrlf & "            </td>" & vbcrlf &"            <td><div align=""right"">出生日期：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <select name=""birthdayType"" id=""birthdayType"" onChange=""getBirthYear()"">" & vbcrlf & "                                <option value=""0"">类型</option>" & vbcrlf & "       <option value=""1"">公历</option>" & vbcrlf & "                  <option value=""2"">农历</option>" & vbcrlf & "                </select>&nbsp;<select name=""birthYear"" id=""birthYear"" onChange=""getBirthMonth()"">" & vbcrlf & "                  <option value="""">年</option>" & vbcrlf & "  </select>&nbsp;<select name=""birthMonth"" id=""birthMonth"" onChange=""getBirthDay()"">" & vbcrlf & "                  <option value="""">月</option>" & vbcrlf & "                </select>&nbsp;<select name=""birthDay"" id=""birthDay"">" & vbcrlf & "                  <option value="""">日</option>" & vbcrlf & "                </select>" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">政治面貌：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""polit"" type=""text"" id=""polit"" size=""15"" dataType=""Limit"" max=""50""   msg=""0~50个字符"">" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">办公电话：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf &   "              <input name=""offTel"" type=""text"" id=""offTel"" size=""15"" value="""
Response.write offTel
Response.write """ dataType=""Limit"" max=""50""  msg=""0~50个字符"">" & vbcrlf & "                          "
if  tp=1 then
Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(offTel.value)"">" & vbcrlf & ""
end if
Response.write "</div></td>" & vbcrlf & "            <td><div align=""right"">家庭电话：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""homeTel"" type=""text"" id=""homeTel""  size=""15"" value="""
Response.write hometel
Response.write """ datatype=""Limit"" max=""50""  msg=""0~50个字符"">" & vbcrlf & ""
if  tp=1 then
Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(homeTel.value)"">" & vbcrlf & ""
end if
Response.write "" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">传真：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""fax"" type=""text""  id=""fax"" size=""15"" value="""
Response.write fax
Response.write """ dataType=""Limit"" max=""50"" msg=""0~50个字符"">" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">手机1：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""mobile"" type=""text"" id=""mobile"" size=""15""  value="""
Response.write mobile
Response.write """ onKeyUp=""value=value.replace(/[^\d]/g,'')"" dataType=""Limit"" max=""50""  msg=""0~50个字符"">" & vbcrlf & "                 "
if  tp=1 then
Response.write "" & vbcrlf & "<img src=""../images/sj.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(mobile.value)"">" & vbcrlf & ""
end if
if open_67_13=1  and open_67_19<>1  then
Response.write "" & vbcrlf & "                <img src=""../images/message.gif"" onClick=""sendSms(mobile.value);function sendSms(phone){if (phone.length==11){" & vbcrlf & "window.open('../message/topadd.asp?phone='+phone+'','newsSendSMS','width=900,height=800,fullscreen=no,scrollbars=0,toolbar=0,status=no,resizable=1,location=no,menubar=no,menubar=no,left=20,top=100');} else{ alert('请填写正确的号码！');}}""  style=""cursor:pointer"" border=""0"" alt=""发送短信"" align=""absbottom"">" & vbcrlf & "                "
end if
Response.write "" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">电子邮件：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""email"" type=""text"" value="""
Response.write email
Response.write """ id=""email"" size=""15"" dataType=""Limit"" max=""50"">" & vbcrlf & "              "
if open_77_13=1  and open_77_19<>1  then
Response.write "" & vbcrlf & "              <img src=""../images/email.gif"" onClick=""sendEmail(email.value);function sendEmail(email){if (email.indexOf('@')>0){" & vbcrlf & "window.open('../email/index.asp?email='+email+'','newsSendEmail','width=900,height=800,fullscreen=no,scrollbars=0,toolbar=0,status=no,resizable=1,location=no,menubar=no,menubar=no,left=20,top=100');} else{ alert('请输入正确的邮箱！');}}""  style=""cursor:pointer"" border=""0"" alt=""发送邮件"" align=""absbottom"">" & vbcrlf & "              "
end if
Response.write "" & vbcrlf & "                <span id=""msgEmail""></span>" & vbcrlf & "            </td>" & vbcrlf & "            <td><div align=""right"">邮编：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""zipCode"" type=""text"" value="""
Response.write zipcode
Response.write """  id=""zipCode"" size=""15"" dataType=""Limit"" max=""50"" msg=""0~50个字符""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">手机2：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""mobile2"" type=""text"" id=""mobile2"" size=""15"" dataType=""Limit"" onKeyUp=""value=value.replace(/[^\d]/g,'')"" max=""50""  msg=""0~50个字符"">" & vbcrlf & "                "
if  tp=1 then
Response.write "" & vbcrlf & "<img src=""../images/sj.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(mobile2.value)"">" & vbcrlf & ""
end if
if open_67_13=1  and open_67_19<>1  then
Response.write "" & vbcrlf & "                <img src=""../images/message.gif"" onClick=""sendSms(mobile2.value);function sendSms(phone){if (phone.length==11){" & vbcrlf & "window.open('../message/topadd.asp?phone='+phone+'','newsSendSMS','width=900,height=800,fullscreen=no,scrollbars=0,toolbar=0,status=no,resizable=1,location=no,menubar=no,menubar=no,left=20,top=100');} else{ alert('请填写正确的号码！');}}""  style=""cursor:pointer"" border=""0"" alt=""发送短信"" align=""absbottom"">" & vbcrlf & "                "
end if
Response.write "" & vbcrlf & "              </div></td>" & vbcrlf & "            <td><div align=""right"">家庭住址：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""homeAddress"" type=""text"" id=""homeAddress"" value="""
Response.write homeAddress
Response.write """ size=""25"" dataType=""Limit"" max=""50"" msg=""0~50个字符"" >" & vbcrlf & "              </div></td>" & vbcrlf & "            <td ><div align=""right"">通讯地址：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""cmAddress"" type=""text"" id=""cmAddress"" value="""
Response.write nowAddress
Response.write """ size=""25"" dataType=""Limit"" max=""50""  msg=""0~50个字符""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td ><div align=""right"">基本工资：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""baseSalarry"" type=""text"" id=""baseSalarry""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('baseSalarry','"
Response.write num_dot_xs
Response.write "')"" size=""15"" dataType=""Limit"" max=""50""  min=""1"" msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td><div align=""right"">入职日期：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""Entrydate"" type=""text"" id=""Entrydate"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" max=""100"" min=""1"" msg=""必填"" >" & vbcrlf & "                <span class=""red"">*</span>" & vbcrlf & "                <DIV id=daysOfMonth3 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </div></td>" & vbcrlf & "                      <td><div align=""right"">合同起始日期：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""contractStart"" type=""text"" id=""contractStart"" value="""
Response.write contractStart
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" max=""100"" min=""1"" msg=""必填"" >" & vbcrlf & "                <span class=""red"">*</span>" & vbcrlf & "                <DIV id=daysOfMonth1 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">试用期工资：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""proSalarry"" type=""text"" id=""proSalarry"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('proSalarry','"
Response.write num_dot_xs
Response.write "')"" size=""15"" dataType=""Limit"" max=""50"" min=""1"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td><div align=""right"">转正日期：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""pubReguldate"" type=""text"" id=""pubReguldate"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" max=""100""  min=""1"" msg=""必填"">" & vbcrlf & "                <span class=""red"">*</span>" & vbcrlf & "                <DIV id=daysOfMonth9 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </div></td>" & vbcrlf & "                      <td><div align=""right"">合同终止日期：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""contractEnd"" type=""text""  value="""
Response.write contractEnd
Response.write """ id=""contractEnd"" size=""15"" dataType=""Limit"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" max=""100""  min=""1"" msg=""必填"">" & vbcrlf & "                <span class=""red"">*</span>" & vbcrlf & "                <DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td><div align=""right"">工资卡开户行：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""depositBank"" type=""text"" id=""depositBank"" value="" size=""25"" dataType=""Limit"" max=""100""  msg=""0~100个字符""></td>" & vbcrlf &"                         <td><div align=""right"">工资卡卡号：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""cardnumBank"" type=""text"" id=""cardnumBank"" value="""" size=""25"" dataType=""Limit"" max=""100""  msg=""0~100个字符""></td>" & vbcrlf & "            <td><div align=""right"">合同提醒：</div></td>" & vbcrlf & "            <td class=""gray""><div align=""left"">" & vbcrlf & "                <input name=""aContract"" type=""radio""  value=""0"" "
if aContract="" or aContract="0" then
Response.write " checked=""checked"""
end if
Response.write " />" & vbcrlf & "                关闭" & vbcrlf & "                <input type=""radio"" name=""aContract"" value=""1"" "
if aContract="1" then
Response.write " checked=""checked"""
end if
Response.write "/>" & vbcrlf & "                开启</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "               "
If ZBRuntime.MC(39002) Or ZBRuntime.MC(18000) Then
Response.write "" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "                    "
Dim tds : tds=0
if ZBRuntime.MC(39002) Then
tds=tds+2
'if ZBRuntime.MC(39002) Then
Response.write "" & vbcrlf & "                             <td><div align=""right"">工资账套：</div></td>" & vbcrlf & "                              <td class=""gray"" "
If Not ZBRuntime.MC(39001) Then
Response.write "colspan=""3"""
end if
Response.write ">" & vbcrlf & "                        "
isupdate=1
Set SCrs=conn.execute("select id,title from hr_gongziClass where del=0 and charindex(','+cast("&gateid&" as varchar(10))+',',','+cast(user_list as varchar(8000))+',')>0")
isupdate=1
If Not SCrs.eof Then
isupdate=0
Response.write "" & vbcrlf & "                                     <input name=""salaryClass_old"" type=""hidden"" id=""salaryClass_old"" value="""
Response.write SCrs("id")
Response.write """>" & vbcrlf & "                                        <input name=""salaryClass"" type=""hidden"" id=""salaryClass"" value="""
Response.write SCrs("id")
Response.write """>"
...
Response.write """>" & vbcrlf & "                                        <select id=""salaryClass"" name=""salaryClass"">" & vbcrlf & "                                        <option value="""">请选择</option>" & vbcrlf & "                                  "
Set SCrs=conn.execute("select id,title,user_list from hr_gongziClass where del=0")
while Not SCrs.eof
Response.write "" & vbcrlf & "                                             <option value="""
Response.write SCrs("id")
Response.write """ "
If salaryClass=SCrs("id") Then
Response.write "selected"
end if
Response.write ">"
Response.write SCrs("title")
...
Response.write "" & vbcrlf & "                             <td><div align=""right"">考勤分组：</div></td>" & vbcrlf & "                <td class=""gray"">" & vbcrlf & "                    <select id=""PersonGroup"" name=""PersonGroup"">" & vbcrlf & "                        <option value =""0"">请选择考勤分组</option>" & vbcrlf & "               "
Set rsTemp=conn.execute("select top 1 id from HrKQ_PersonGroup where RangeType = 0 or dbo.existsPower2(" & gateid & ",ISNULL([Range],''),',') = 1")
If Not rsTemp.eof then
PersonGroup = rsTemp(0)
end if
rsTemp.close
Set rsTemp = Nothing
...
SCrs.movenext
wend
Response.write "" & vbcrlf & "                        SCrs.close" & vbcrlf & "                    </select>" & vbcrlf & "                </td>" & vbcrlf & "            "
end if
if ZBRuntime.MC(18000) or ZBRuntime.MC(218910) or ZBRuntime.MC(26002) Then
tds=tds+1
'if ZBRuntime.MC(18000) or ZBRuntime.MC(218910) or ZBRuntime.MC(26002) Then
Response.write "" & vbcrlf & "                             <td><div align=""right"">计件工资：</div></td>" & vbcrlf & "                              <td><input name=""jjSalarry"" type=""radio"" value=""1"" "
If jjSalarry=1 Then
Response.write "checked"
end if
Response.write ">" & vbcrlf & "                            计算" & vbcrlf & "                              <input name=""jjSalarry"" type=""radio"" value=""0"" "
If jjSalarry<>1 Then
Response.write "checked"
end if
Response.write ">" & vbcrlf & "                            不计算</td>" & vbcrlf & "                             "
end if
If tds<3 Then Response.write "<td colspan="& (3-tds)*2 &">&nbsp;</td>"
Response.write ">" & vbcrlf & "                            不计算</td>" & vbcrlf & "                             "
Response.write "" & vbcrlf & "          </tr>" & vbcrlf & "                "
end if
Response.write "" & vbcrlf & "               <tr class=""tr_1"">" & vbcrlf & "                       <td><div align=""right"">员工状态：</div></td>" & vbcrlf & "                      <td class=""gray""><div align=""left"">" & vbcrlf & "                 <select name=""nowStatus"" min=""1"" msg=""必选""  datatype=""Limit"">" & vbcrlf & "                  <option value="""" "
if nowStatus="" then
Response.write "selected"
end if
Response.write ">请选择</option>" & vbcrlf & "                     "
for i=1 to 7
Response.write "" & vbcrlf & "                     <option value="""
Response.write i
Response.write """  "
...
Response.write ">"
Response.write getPersonStatus(i)
Response.write "</option>" & vbcrlf & "                    "
next
Response.write "" & vbcrlf & "                     </select>" & vbcrlf & "                       <span class=""red"">*</span></div></td>" & vbcrlf & "                     <td><div align=""right"">职位：</div></td>" & vbcrlf & "                  <td class=""gray"">" & vbcrlf & "                         <input name=""aDay"" type=""text"" style=""display:none"" readonly=""readonly"" value="""
Response.write aDay
Response.write """ id=""aDay"" size=""15"" dataType=""Limit"" max=""100""  >" & vbcrlf & "                               <span id='positionList'>" & vbcrlf & "                                        <select name=""PostionID"" min=""1"" msg=""必选""  datatype=""Limit"" style=""width:70%"">" & vbcrlf & "                                  <option value="""" selected>请选择</option>" & vbcrlf & "                                 "
set rs8=server.CreateObject("adodb.recordset")
sql8="select id,dbo.hrGetPostion_SorceName(id) as Name from hr_pub_postion  where del=0 order by sorce asc,sorce2 asc"
...
Rs8.close
Set Rs8=Nothing
Response.write "" & vbcrlf & "                                       </select>" & vbcrlf & "                                       <span class=""red"">*</span>" & vbcrlf & "                                </span>" & vbcrlf & "                         "
If open_82_12&"" = "1" Then
Response.write "<span><a href=""javascript:;"" onclick=""javascript:window.open('../manufacture/inc/Bill.asp?orderid=1020','newPosition','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=200')"">设置</a></span>"
'If open_82_12&"" = "1" Then
end if
Response.write "" & vbcrlf & "                      </td>" & vbcrlf & "                   <td><div align=""right"">岗位：</div></td>" & vbcrlf & "                  <td class=""gray"">" & vbcrlf & "                         <select name=""workid"" min=""1"" msg=""必选""  datatype=""Limit"">" & vbcrlf & "                                     <option value="""" selected>请选择</option>" & vbcrlf & "                                 "
set rs8=server.CreateObject("adodb.recordset")
sql8="select * from sortonehy where gate2=1080 and del=1 order by gate1 desc ,ord "
rs8.open sql8,conn,1,1
If not rs8.eof then
Do while not Rs8.eof
Response.write "<option value="""
...
Rs8.close
Set Rs8=Nothing
Response.write "" & vbcrlf & "                              </select>" & vbcrlf & "                               <span class=""red"">*</span>" & vbcrlf & "                                "
If session("top1zbintel2007")="1" And sdk.Power.existsPower(66,12) Then
Response.write "<span><a href=""javascript:;"" onclick=""javascript:window.open('../sort3/edit.asp?ord=1080','newPosition','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=200')"">设置</a></span>"
'If session("top1zbintel2007")="1" And sdk.Power.existsPower(66,12) Then
end if
Response.write "" & vbcrlf & "                      </td>" & vbcrlf & "             </tr>" & vbcrlf & "          <tr>" & vbcrlf & "              <td><div align=""right"">首次工作日期：</div></td>" & vbcrlf & "              <td class=""gray"" colspan=""5"">" & vbcrlf & "                  <input name=""DateForStartedWork"" type=""text"" id=""DateForStartedWork"" value="""
Response.write DateForStartedWork
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"">" & vbcrlf & "                  <DIV id=daysOfMonth11 style=""POSITION: absolute""></DIV>" & vbcrlf & "              </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & " <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_2')"">社会保险基数设置</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "            <td ><div align=""right"">养老保险：</div></td>" & vbcrlf &  "           <td class=""gray""><input name=""Pension"" type=""text"" id=""Pension"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('Pension','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" datatype=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">医疗保险：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Health"" type=""text"" id=""Health""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('Health','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">失业保险：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Unployment"" type=""text"" id=""Unployment""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('Unployment','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "            <td ><div align=""right"">工伤保险：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Injury"" type=""text"" id=""Injury""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('Injury','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">生育保险：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Maternity"" type=""text"" id=""Maternity""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('Maternity','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">住房公积金：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Housing"" type=""text"" id=""Housing""  onpropertychange=""formatData(this"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "         <tr class=""tr_2"">" & vbcrlf & "            <td ><div align=""right"">子女教育：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""ChildrenseDucation"" type=""text"" id=""ChildrenseDucation""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('ChildrenseDucation','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">继续教育(学历)：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""ContinuingEducationxl"" type=""text"" id=""ContinuingEducationxl""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('ContinuingEducationxl','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">继续教育(技能)：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""ContinuingEducationjn"" type=""text"" id=""ContinuingEducationjn""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('ContinuingEducationjn','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "            <td ><div align=""right"">大病医疗：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""medical"" type=""text"" id=""medical""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('medical','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">住房贷款：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""Housingloans"" type=""text"" id=""Housingloans""  onpropertychange=""formatData(this,'money');""onKeyUp=""checkDot('Housingloans','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right"">住房租金：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""payment"" type=""text"" id=""payment""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('payment','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "            <td ><div align=""right"">赡养老人：</div></td>" & vbcrlf & "            <td class=""gray""><input name=""SupportOldPeople"" type=""text"" id=""SupportOldPeople""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('SupportOldPeople','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "               <td ><div align=""right"">婴幼儿照护：</div></td>" & vbcrlf & "           <td class=""gray""><input name=""InfantCare"" type=""text"" id=""InfantCare""  onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('InfantCare','"
Response.write num_dot_xs
Response.write "')"" onfocus=""this.select()"" value=""0"" size=""15"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"" >" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td ><div align=""right""></div></td>" & vbcrlf & "            <td class=""gray""></td>" & vbcrlf & "</tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_3')"">教育信息</a> </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "<tr class=""tr_3"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""eduList"">" & vbcrlf & "                "
if isnumeric(resumeID) and resumeID<>0 then
set rs8=server.CreateObject("adodb.recordset")
sql8="select * from hr_resume_edu  where Resume="&resumeID&""
rs8.open sql8,conn,1,1
if not rs8.eof then
do while not rs8.eof
Response.write "" & vbcrlf & "                                                              <tr class=""person_edu"
Response.write rs8("id")
Response.write """>" & vbcrlf & "                  <td width=""11%""><div align=""right"">起始日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""edu_StartTime"
Response.write rs8("id")
Response.write """ type=""text"" value="""
Response.write rs8("startdate")
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    <div id=daysOfMonth11"
Response.write rs8("id")
Response.write " style=""position: absolute""></div></td>" & vbcrlf & "                  <td ><div align=""right"">结束日期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""edu_EndTime"
Response.write rs8("id")
Response.write """ type=""text""  value="""
Response.write rs8("enddate")
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" >" & vbcrlf & "                    <div id=daysOfMonth12"
Response.write rs8("id")
Response.write " style=""position: absolute""></div></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">毕业院校：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""edu_college"" type=""text""  size=""15"" id=""edu_college"" value="""
Response.write rs8("school")
Response.write """  dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" > </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_edu"
Response.write rs8("id")
Response.write """>" & vbcrlf & "                  <td width=""11%""><div align=""right"">学历：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""edu_Name"" type=""text"" id=""edu_Name"" value="""
Response.write rs8("Education")
Response.write """ size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td ><div align=""right"">专业：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""edu_prof"" type=""text"" id=""edu_prof"" value="""
Response.write rs8("Professional")
Response.write """ size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""edu_remark"" type=""text""  size=""15"" id=""edu_remark"" value=""""  dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" >" & vbcrlf &  "                   &nbsp;&nbsp;<span ><img src"
rs8.movenext
loop
else
Response.write "" & vbcrlf & "                                                             <tr class=""person_edu"">" & vbcrlf & "                  <td width=""11%""><div align=""right"">起始日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""edu_StartTime"" type=""text""   Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">结束日期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""edu_EndTime"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">毕业院校：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""edu_college"" type=""text""size=""15"" id=""edu_college""   dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" > </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_edu"">" & vbcrlf & "                  <td width=""11%""><div align=""right"">学历：</div></td>" & vbcrlf & "<td width=""21%"" class=""gray""><input name=""edu_Name"" type=""text"" id=""edu_Name""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td ><div align=""right"">专业：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""edu_prof"" type=""text"" id=""edu_prof""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""edu_remark"" type=""text""  size=""15"" id=""edu_remark"" value=""""  dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" >" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
end if
rs8.close
set rs8=nothing
else
...
Response.write "" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_3"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addEduList()"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "      </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_4')"">语言能力</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_4"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""languaList"">" & vbcrlf & "                "
if isnumeric(resumeID) and resumeID<>0 then
set rs8=server.CreateObject("adodb.recordset")
sql8="select * from hr_resume_Language  where Resume="&resumeID&" order by id asc"
rs8.open sql8,conn,1,1
if not rs8.eof then
do while not rs8.eof
Response.write "" & vbcrlf & "                <tr class=""person_language"
Response.write rs8("id")
Response.write """>" & vbcrlf & "                  <td width=""11%""><div align=""right"">语言种类：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""lang_name"" type=""text"" value="""
Response.write rs8("typeID")
Response.write """  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">语言水平：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""lang_lv"" type=""text"" value="""
Response.write rs8("Proficiency")
Response.write """  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">获得证书：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""lang_zhengshu"" type=""text""  dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf &  "                  </td>" & vbcrlf &"                 </tr>" & vbcrlf & "              <tr class=""person_language"
Response.write rs8("id")
Response.write """>" & vbcrlf & "                  <td width=""11%""><div align=""right"">颁发时间：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""lang_bfdate"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td ><div align=""right"">颁发机构：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""lang_jigou"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""lang_content"" type=""text"" value="""
Response.write rs8("content")
Response.write """  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    &nbsp;&nbsp;<span ><img src='../images/smico/icon_del.gif' width='19' height='17' alt='删除' onClick='if(confirm(""确认删除？"")){delClassItem(this)}'></span> </td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
rs8.movenext
loop
else
Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">语言种类：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""lang_name"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">语言水平：</div></td>" & vbcrlf &  "                 <td width=""27%"" class=""gray""><input name=""lang_lv"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">获得证书：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""lang_zhengshu"" type=""text""  dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "               <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">颁发时间：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""lang_bfdate"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit""min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td ><div align=""right"">颁发机构：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""lang_jigou"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>"& vbcrlf & "                  <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""lang_content"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & " </tr>" & vbcrlf & "                "
end if
rs8.close
set rs8=nothing
else
...
Response.write "" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_4"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addLanguaList()"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "   </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_5')"">工作经历</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "       <tr class=""tr_5"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""WorkList"">" & vbcrlf & "                "
if isnumeric(resumeID) and resumeID<>0 then
set rs1=server.CreateObject("adodb.recordset")
sql="select * from hr_resume_Work_exp  where Resume="&resumeID&" "
rs1.open sql,conn,1,1
if not rs1.eof then
do while not rs1.eof
workStartTime=rs1("startDate")
workEndTime=rs1("enddate")
workCompany=rs1("companyName")
workPosition=rs1("Position")
workContent=rs1("jobDes")
Response.write "" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td height=""27"" colspan=""6"" >&nbsp;</td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td width=""11%""><div align=""right"">起始日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""workStartTime_"
Response.write rs1("id")
Response.write """ type=""text""  value="""
Response.write workStartTime
Response.write """ readonly=""readonly"" Class=""DatePick""  onclick=""datedlg.show();"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">结束日期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""workEndTime_"
Response.write rs1("id")
Response.write """ type=""text"" value="""
Response.write workEndTime
Response.write """ Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">证明人：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""workZhengMing"" type=""text"" value="""
Response.write workZhengMing
Response.write """  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    &nbsp;&nbsp;<span ><img src='../images/smico/icon_del.gif' width='19' height='17' alt='删除' onClick='if(confirm(""确认删除？"")){delClassItem(this)}'></span> </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td><div align=""right"">公司名称：</div></td>" & vbcrlf & "                  <td><input name=""workCompany"" type=""text"" id=""workCompany"" value="""
Response.write workCompany
Response.write """ size=""20"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">担任职务：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workPosition"" type=""text"" value="""
Response.write workPosition
Response.write """ id=""workPosition"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">电话：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workTel"" type=""text"" value="""
Response.write workTel
Response.write """ id=""workTel"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    "
if tp=1 then
Response.write "" & vbcrlf & "                    <img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(document.getElementById('workTel').value)"">" & vbcrlf & "                    "
end if
Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td><div align=""right"">工作内容：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workContent""   cols=""80"" rows=""4"" id=""workContent"" dataType=""Limit"" max=""4000""  msg=""最多4000个字"">"
Response.write workContent
Response.write "</textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td><div align=""right"">工作业绩：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workJiXiao""  cols=""80"" rows=""4"" id=""workJiXiao"" dataType=""Limit"" max=""4000""  msg=""最多4000个字"">"
Response.write workJiXiao
Response.write "</textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_work"
Response.write rs1("id")
Response.write """>" & vbcrlf & "                  <td><div align=""right"">离职原因：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workLever"" cols=""80"" rows=""4""   dataType=""Limit"" max=""4000""  msg=""最多4000个字"">"
Response.write workLever
Response.write "</textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
rs1.movenext
loop
else
Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">起始日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""workStartTime"" type=""text""   Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">结束日期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""workEndTime"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">证明人：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""workZhengMing"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf &   "                </td>" & vbcrlf & "                </tr>" & vbcrlf &  "               <tr> "& vbcrlf &  "                 <td><div align=""right"">公司名称：</div></td>" & vbcrlf & "                  <td><input name=""workCompanytype=""text"" id=""workCompany""  size=""20"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">担任职务：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workPosition"" type=""text""  id=""workPosition"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">电话：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workTel"" type=""text""  id=""workTel"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    "
if tp=1 then
Response.write "" & vbcrlf & "                    <img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(document.getElementById('workTel').value)"">" & vbcrlf & "                    "
end if
Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">工作内容：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workContent""   cols=""80"" rows=""4"" id=""workContent"" dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">工作业绩：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workJiXiao""  cols=""80"" rows=""4"" id=""workJiXiao"" dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">离职原因：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workLever"" cols=""80"" rows=""4""   dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
end if
rs1.close
set rs1=nothing
else
Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">起始日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""workStartTime"" type=""text"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">结束日期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""workEndTime"" type=""text"" Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50"" msg=""0~50字符"" ></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">证 明 人：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""workZhengMing"" type=""text""  size=""15""dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">公司名称：</div></td>" & vbcrlf & "                  <td><input name=""workCompany"" type=""text"" id=""workCompany"" size=""20"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">担任职务：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workPosition"" type=""text"" id=""workPosition"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td><div align=""right"">电话：</div></td>" & vbcrlf & "                  <td class=""gray""><input name=""workTel"" type=""text"" id=""workTel"" size=""15""  dataType=""Limit"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    "
if tp=1 then
Response.write "" & vbcrlf & "                    <img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:pointer"" onClick=""phoneCall(document.getElementById('workTel').value)"">" & vbcrlf & "                    "
end if
Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">工作内容：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workContent""  cols=""80"" rows=""4"" id=""workContent"" dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">工作业绩：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""workJiXiao""  cols=""80"" rows=""4"" id=""workJiXiao"" dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td><div align=""right"">离职原因：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><div align=""left"">" & vbcrlf & "                      <textarea name=""workLever"" cols=""80"" rows=""4""   dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea>" & vbcrlf & "                    </div></td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
end if
Response.write "" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_5"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" onClick=""addWorkList()"" class=""addBtn"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "     </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_6')"">社会关系</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "         <tr class=""tr_6"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""relationList"">" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">姓名：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""relat_name"" type=""text"" id=""relat_name""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">出生日期：</div></td>" & vbcrlf & "     <td width=""27%"" class=""gray""><input name=""relat_birth"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">与本人关系：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""relat_ship"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">工作单位：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""relat_work"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">联系方式：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""relat_tel"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">政治面貌：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""relat_polit"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_6"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addRelatList()"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "        <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_7')"">资格证书</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_7"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""CertList"">" & vbcrlf & "                <tr class=""person_Cert"">" & vbcrlf & "                  <td width=""11%""><div align=""right"">资格证书：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""cert_title"" type=""text"" id=""cert_title""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">颁发时间：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""cert_hasDate"" type=""text""  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">颁发机构：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""cert_agency"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr class=""person_Cert"">" & vbcrlf & "                  <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td colspan=""5"" class=""gray""><textarea name=""cert_remark"" cols=""80"" rows=""6""  id=""cert""  dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea></td>" & vbcrlf & "                </tr>" & vbcrlf & "</table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_7"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addCertList()"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_8')"">健康信息</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_8"">" & vbcrlf & "            <td colspan=""6""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" id=""HealthList"">" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">健康状况：</div></td>" & vbcrlf & "      <td width=""21%"" class=""gray""><input name=""health_State"" type=""text""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">有无传染病：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""health_isDisease"" type=""radio"" value=""0"" checked>" & vbcrlf & "                    无" & vbcrlf & "                    <input type=""radio"" name=""health_isDisease"" value=""1"">" & vbcrlf &  "                   有</td>" & vbcrlf & "                  <td width=""11%""><div align=""right"">有无大病史：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""health_serious"" type=""radio"" value=""0"" checked>" & vbcrlf & "                    无" & vbcrlf & "                    <input type=""radio"" name=""health_serious"" value=""1"">" & vbcrlf & "             有 </td>" & vbcrlf & "                </tr>" & vbcrlf & "                <tr>" & vbcrlf & "                  <td width=""11%""><div align=""right"">上次体检日期：</div></td>" & vbcrlf & "                  <td width=""21%"" class=""gray""><input name=""health_lastdate"" type=""text"" Class=""DatePick""onclick=""datedlg.show();"" readonly=""readonly""   size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符""></td>" & vbcrlf & "                  <td ><div align=""right"">体检周期：</div></td>" & vbcrlf & "                  <td width=""27%"" class=""gray""><input name=""health_cycle"" type=""text"" onKeyUp=""value=value.replace(/[^\d]/g,'')""  size=""8"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                    <select name=""health_unit"" >" & vbcrlf & "                      <option value=""1"" selected>年</option>" & vbcrlf & "                      <option value=""2"">季</option>" & vbcrlf & "                      <option value=""3"">月</option>" & vbcrlf & "                      <option value=""4"">周</option>" & vbcrlf & "                      <option value=""5"">日</option>" & vbcrlf & "                    </select></td>" & vbcrlf & "            <td width=""11%""><div align=""right"">备注：</div></td>" & vbcrlf & "                  <td  class=""gray""><input name=""health_content"" type=""text""  size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""0~50字符"">" & vbcrlf & "                  </td>" & vbcrlf & "                </tr>" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_8"">" & vbcrlf & "            <td colspan=""6""><input name=""addList"" class=""addBtn"" onClick=""addHealthList()"" type=""button"" id=""addList"" value=""继续添加""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_9')"">其他信息</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_9"">" & vbcrlf & "            <td ><div align=""right"">特长：</div></td>" & vbcrlf & "            <td colspan=""5"" class=""gray""><div align=""left"">" & vbcrlf & "                <textarea name=""specialty"" cols=""80"" rows=""6""  id=""specialty""  dataType=""Limit"" max=""4000""  msg=""最多4000个字"">"
Response.write about
Response.write "</textarea>" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_9"">" & vbcrlf & "            <td ><div align=""right"">爱好：</div></td>" & vbcrlf & "            <td colspan=""5"" class=""gray""><div align=""left"">" & vbcrlf & "             <textarea name=""interest"" cols=""80"" rows=""6""  id=""interest""  dataType=""Limit"" max=""4000""  msg=""最多4000个字""></textarea>" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_9"">" & vbcrlf & "            <td ><div align=""right"">备注：</div></td>" & vbcrlf & "            <td colspan=""5"" class=""gray""><div align=""left"">" & vbcrlf & "                <textarea name=""content"" cols=""80"" rows=""6"" dataType=""Limit"" max=""4000""  msg=""最多4000个字"">"
Response.write intro
Response.write "</textarea>" & vbcrlf & "              </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_10')"">附件信息</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_10"">" & vbcrlf & "            <td ><div align=""right"">附件列表：</div></td>" & vbcrlf & "            <td colspan=""5"" class=""gray""><div align=""left"">" & vbcrlf & "                <table id=""atttb"" class=""fileAccess"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "                  "
if isnumeric(draft) and draft<>"" and isAccess=1 then
call getUploadFileList(conn,draft,82)
end if
Response.write "" & vbcrlf & "                </table>" & vbcrlf & "              </div>" & vbcrlf & "              <span style=""cursor:pointer"" onClick=""showUploadForm(this);""><img src='../images/smico/3.gif'/>添加文件</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "        <td  colspan=""6"" class=""gray""><p align=""center"">" & vbcrlf & "                <input type=""submit"" onclick=""checkEmail()"" name=""Submit42"" value=""保存""  class=""page""/>" & vbcrlf & "                <input type=""reset"" value=""重填"" class=""page"" name=""B2"">" & vbcrlf & "          </td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "          </form>" & vbcrlf & "         </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <IFRAME style=""display:none"" name=""I4"" id=""I4"" FRAMEBORDER=""0"" SCROLLING=""no"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "  <IFRAME style=""display:none"" name=""I5"" id=""I5"" FRAMEBORDER=""0"" SCROLLING=""no"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "</table>" & vbcrlf & "<!--8888888888888888888888888888888888888888888888-->" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""上传头像""  style=""width:500px;height:218px;padding:5px;background: #fafafa;top:60px;left:450px;""  closed=""true"" >" & vbcrlf & "  <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:0px solid #ccc; width:450px;"">" & vbcrlf & "     <form action=""saveimg.asp"" method=""post"" enctype=""multipart/form-data"" name=""updata"" id=""updata"" target=""I5"">" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td width=""28%"" height=""45"" valign=""middle""><div align=""right"">路径：</div></td>" & vbcrlf &  "         <td width=""72%"" align=""left"" valign=""middle""><div align=""left"">" & vbcrlf & "              <input name=""imgurl"" class=""anybutton"" type=""file"" id=""imgurl"" style=""background-color:#FFFFFF;"">" & vbcrlf & "              <input name=""t""  id=""t"" type=""hidden"" value=""2"">" & vbcrlf &"           </div></td> & vbcrlf &         </tr> & vbcrlf &                          <tr><td colspan=""2"" align=""center""> "& vbcrlf &  "               友情提示：请将上传文件大小控制在200KB以内。<br /> "& vbcrlf & "                最佳尺寸：90*100(px)  &nbsp;格式要求：JPG/GIF/PNG </td></tr>       "& vbcrlf & "    </table>" & vbcrlf & "    </form>" & vbcrlf & "  </div>" & vbcrlf & "  <br />" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:center;height:30px;line-height:30px;"">" & vbcrlf & "    <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('updata').submit()"">确认</a>" & vbcrlf & "    <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a> </div>" & vbcrlf & "</div>" & vbcrlf & "<!--888888888888888888888888888888888888888888888-->" & vbcrlf & "</body>" & vbcrlf & "</html>"
'call getUploadFileList(conn,draft,82)
action1="员工档案添加"
call close_list(1)
