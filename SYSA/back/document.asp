...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(rpt, lt)
		app.addScriptPath "../skin/default/js/back.js"
		rpt.title = "文档回收站"
		rpt.pagesize = 15
		rpt.nosearchBar=true
		rpt.sql = " select d.id ,d.title,d.wdid, isnull(g.name,'已删除') as addname,isnull(g1.name,'已删除') as delname,deldate,d.addcate,(select count(1) from document where wdid=d.wdid and del=1) as reCount from document d left join gate g on d.addcate=g.ord left join gate g1 on g1.ord=d.delcate where  d.del=2order by d.deldate desc "
		rpt.canexcel = false
		rpt.canprint = false
		rpt.BillListUIModel = true
		rpt.beforeBtnsHTML  = "<button class='oldbutton3' onclick=""javascript:deleteall(2,0)"">全部清空</button>"
	end sub
	Dim intro_78_14
	sub app_oncreate(lvw)
		...
		col.title = "标题"
		col.align="left"
		lvw.headers(2).formattext = "code:GetWDTitleHtml(rs(""id""),""@value"",rs(""addcate""))"
		...
		col.width = 150
		col.title = "操作"
		col.formattext = "code:WDActHTML(rs(""id""),rs(""reCount""))"
		Set col = lvw.headers(8)
		col.width = 0
		col.display = "none"
		dim btmStr
		btmStr = ""
		btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='deleteall(0,0)'>批量删除</button>"
		btmStr = btmStr& "   &nbsp;<button class='oldbutton3' onclick='resetall(0,0)'>批量恢复</button>"
		btmStr = btmStr& "<a>"
	...
	function GetWDTitleHtml(wdord, wdTitle, addcate)
		...
		if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
			wdTitleStr = "<a href=""javascript:;"" onclick=""javascript:window.open('../document/content.asp?__fReclst=1&ord=" & app.base64.pwurl(wdord) & "','newwin','width=' + 910 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& wdTitle &"</a>"
'if intro_78_14="" or instr(","& intro_78_14 &"," , ","& addcate &",")>0 then
		else
			wdTitleStr = wdTitle
		end if
	...
	function WDActHTML(wdord,reCount)
		dim actStr ,disabled
		if wdord&""<>"" Then
			If reCount>0 Then disabled = " disabled "
			actStr = actStr &"<div style='word-wrap: break-word; word-break: normal;width:100%;'>"
'If reCount>0 Then disabled = " disabled "
			actStr = actStr & "<button class=""anybutton"" onclick=""javascript:resetall(1,"& wdord &")"" "& disabled &">恢复</button>"
			if actStr&""<>"" then actStr = actStr & "&nbsp;"
			actStr = actStr & "<button class=""anybutton"" onclick=""javascript:deleteall(1,"& wdord &")"">彻底删除</button>"
			actStr = actStr &"</div>"
		end if
		WDActHTML = actStr
	end function
	Sub App_reSet
		Dim rs,typ , ordlist ,upcount
		typ = app.getInt("typ")
		ordlist = app.getText("ordlist")
		upcount = 0
		If Len(ordlist)=0 Then Response.write "0" : Exit Sub
		Set rs = cn.execute("select id,wdid from document where del = 2 and id in ("& ordlist &") order by deldate desc ")
		While rs.eof = False
			If cn.execute("select count(1) from document where wdid='"& rs("wdid") &"' and del=1")(0)=0 Then
				cn.execute("update documentlist set del=1 where del=2 and document=" &  rs("id"))
				cn.execute("update document set del=1 where del=2 and id=" &  rs("id") )
				upcount = upcount + 1
'cn.execute("update document set del=1 where del=2 and id=" &  rs("id") )
			end if
			rs.movenext
	...
	Sub App_doDelete
		...
		ordlist = app.getText("ordlist")
		If typ = 2 Then
			Set rs = cn.execute ("select wdurl from documentlist where del=2")
			If Not rs.eof Then
				while not rs.eof
					Call deletfile(rs(0))
					rs.movenext
			...
			rs.close
			set rs = nothing
			cn.execute("delete from documentlist where del=2 ")
			cn.execute("delete from document where del=2 ")
		else
			If Len(ordlist)=0 Then Response.write "0" : Exit Sub
			Set rs = cn.execute ("select wdurl from documentlist where del=2 and document in ("& ordlist &")")
			If Not rs.eof Then
				while not rs.eof
					Call deletfile(rs(0))
					rs.movenext
			...
			rs.close
			set rs = nothing
			cn.execute("delete from documentlist where del=2 and document in ("& ordlist &")")
			cn.execute("delete from document where del=2 and id in ("& ordlist &") ")
		end if
		Response.write "1"
	end sub
	Sub deletfile(filepath)
		if sdk.file.ExistsFile(filepath) Then
			sdk.file.DeleteFile filepath
		end if
	end sub
