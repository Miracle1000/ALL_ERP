...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, state)
		Dim cindex
		Dim redateB,redateE,CSstatus,cs
		redateB = request("redateB")
		redateE = request("redateE")
		CSstatus = request("CSstatus")
		cs = request("cs")
		rpt.beforeBtnsHTML = "<style> "& vbcrlf &_
		" #commfieldsBox #sfields_wstatus,#commfieldsBox #sfields_ktype{*margin-top:2px} "& vbcrlf &_
		"rpt.beforeBtnsHTML = ""<style> """& vbcrlf &_
		"#commfieldsBox select{*height:20px;} " & vbcrlf &_
		" #commfieldsBox .oldbutton{*margin-top:1px} "& vbcrlf &_
		" #commfieldsBox select{*height:20px;}  "& vbcrlf &_
		" </style>"
		app.adddefaultScript
		rpt.title = "阶段工作列表"
		rpt.addfield "计划开始时间：","datetimes","de1","","", Split(",",",")
		rpt.addfield2 "","select", "wstatus" , Rpt.getselDataByTxt("未启动=0;未执行=1;执行中=9;执行完毕=2;终止=7;待审核=53;审核通过=54;审核未通过=55;延期=60"), "" , "", "状态=-1"
		'rpt.addfield "计划开始时间：","datetimes","de1","","", Split(",",",")
		rpt.addfield2 "","select", "ktype" , Rpt.getselDataByTxt("项目编号=1;执行人=2;协作人=3"), "" , "", "项目主题=0"
		rpt.addfield "","text", "ktxt" , "", "", ""
		rpt.aSearch.addfield "执行人", "gates", "g1",""
		rpt.aSearch.addfield "协作人", "gates", "g2",""
		rpt.aSearch.addfield "项目阶段", "treechecks", "sortjh2","select id, id1, sort1 as name from sortjh2 where id1=@parentid"
		rpt.aSearch.addfield "执行状态", "checks", "wstatus","options:0=未启动;1=未执行;9=执行中;2=执行完毕;7=终止;53=待审核;54=审核通过;55=审核未通过;60=延期"
		rpt.aSearch.addfield "项目分类", "sortonehy", "chcls","24"
		rpt.aSearch.addfield "项目状态", "sortonehy", "chstate","21"
		rpt.aSearch.addfield "项目来源", "sortonehy", "chly","23"
		rpt.aSearch.addfield "项目区域", "khqy", "chqy",""
		rpt.aSearch.addfield "项目主题", "selectys", "chtitle", ""
		rpt.aSearch.addfield "项目编号", "selectys", "chbh", ""
		rpt.aSearch.addfield "计划开始时间", "dates", "d3", ""
		rpt.aSearch.addfield "计划截止时间", "dates", "d4", ""
		rpt.aSearch.addfield "实际开始时间", "dates", "d5", ""
		rpt.aSearch.addfield "实际截止时间", "dates", "d6", ""
		rpt.sql = "set nocount on;select top 0 cast(0 as money) as id into #existssqltable; "&_
		"create index fnis_t01 on [#existssqltable] ( id desc ); "&_
		"insert into #existssqltable(id) select -1 as r;"&_
		"exec erp_list_chanceProcItemList " & Info.user & " , 0, 0, @de1_0, @de1_1,  '', '', @wstatus, @ktype, @ktxt, @g1, @g2," &_
		"@sortjh2, @chcls, @chstate, @chly, @chqy, @chtitle_0, @chtitle_1, @chbh_0, @chbh_1, @d3_0, @d3_1, @d4_0, @d4_1, @d5_0, @d5_1, @d6_0, @d6_1, &ReportSort, " & _
		"&SearchMode, &excelmode,'"&redateB&"','"&redateE&"','"&CSstatus&"','"&cs&"';"&_
		"drop table #existssqltable;set nocount off;"
		rpt.canexcel =  app.power.existsPower(3,10)
		rpt.canprint =  app.power.existsPower(3,7)
		rpt.BillListUIModel = true
		rpt.onlyadSearchModel = True
		rpt.addsort "项目阶段", "1"
		rpt.addsort "执行人", "2"
		rpt.addsort "计划开始时间", "3"
		rpt.addsort "计划截止时间", "4"
		rpt.addsort "实际开始时间", "5"
		rpt.addsort "实际截止时间", "6"
		Dim bats
		If app.power.existsPower(3,3) Then
			bats = "批量删除"
		end if
		If app.power.existsPower(3,27) Then
			If Len(bats) > 0 Then bats = bats & ","
			bats = bats & "批量变更"
		end if
		If Len(bats) > 0 Then
			rpt.setBatHandle "logid", bats
		end if
	end sub
	Sub App_doBatHandle(cmd, checkvalues, batResult)
		...
		Case "批量变更"
		Call doBatChance(checkvalues, batResult)
		End select
	end sub
	Sub doBatChance(checkvalues, batResult)
		session("ch_p_checkvalues") = checkvalues
		Response.write "window.open(""chanceNdBatChange.asp?t=" & Replace(CDbl(now) & "",".","") & """,'Ismode');"
	end sub
	Sub doDelete(values, batResult)
		Dim powers, sql, rs
		powers = app.power.GetPowerIntro(3,3)
		sql = "delete  ChanceProcRunLogs_bak where logid in (" & values & ")"
		If Len(powers) > 0 Then
			sql = sql & " and MainExecutor in (" & powers & ")"
		end if
		cn.execute sql
		sql = "delete  ChanceProcRunLogs where id in (" & values & ")"
		If Len(powers) > 0 Then
			sql = sql & " and MainExecutor in (" & powers & ")"
		end if
		cn.execute sql
		Set rs = cn.execute("select id from ChanceProcRunLogs WITH(NOLOCK) where id in(" & values & ")")
		While rs.eof = false
			batResult.addResult "不允许删除", rs(0).value, "red"
			rs.movenext
		wend
		rs.close
		set rs = nothing
	...
	Sub app_oncreate(ByVal lvw)
		...
		lvw.checkbox = true
		lvw.cansort = False
		lvw.headers(2).setLink2 "", "xmid", "xmcateid", 3, -7, "chanceshare"
		lvw.cansort = False
		lvw.headers(6).Formula = "getProcBarHTML(@value , me)"
		lvw.headers(8).width = "200"
		lvw.headers(8).formattext = "code:cHandleHtml(@cells[12])"
		lvw.headers(9).display  = "none"
		lvw.headers(10).display  = "none"
		lvw.headers(11).display  = "none"
		lvw.headers(12).display  = "none"
		lvw.headers(13).display  = "none"
		lvw.headers(14).display  = "none"
		lvw.headers("项目阶段").dbtype="str"
		lvw.headers("项目阶段").align2="center"
		lvw.headers("项目阶段").excelAlign = "center"
		lvw.headers("项目名称").dbtype="str"
		lvw.headers("执行人").dbtype="str"
		lvw.headers("执行人").align2="center"
		lvw.headers("执行人").excelAlign = "center"
		lvw.headers("计划开始时间").dbtype="str"
		lvw.headers("计划开始时间").align2="center"
		lvw.headers("计划开始时间").excelAlign = "center"
		lvw.headers("计划截至时间").dbtype="str"
		lvw.headers("计划截至时间").align2="center"
		lvw.headers("计划截至时间").excelAlign = "center"
		lvw.headers("计划截至时间").Title = "计划截止时间"
		lvw.headers("执行状态").dbtype="str"
		lvw.headers("执行状态").align2="center"
		lvw.headers("执行状态").excelAlign = "center"
		lvw.headers("chanceshare").display  = "none"
		lvw.headers("jdtype").display  = "none"
	end sub
	Function getProcBarHtml(ByVal value , ByVal lvw)
		If isnumeric(value) And Len(value & "") > 0 Then
			If lvw.excelmode Then
				getProcBarHtml = app.format(value,"0.00") & "%"
			else
				getProcBarHtml = "<div align='center' title='阶段进度：" & app.format(value,"0.00") & "%' style='width:100px;border:1px solid #ccc;padding:1px;background-color:white;height:14px;margin:0 auto'><div style='float:left;width:" & clng(value*1) & "px;overflow:hidden;background-color:#00a2e7;height:14px;'>&nbsp;</div></div>"
			end if
		end if
	end function
	Function cHandleHtml(ByVal logId)
		Dim rs, sql, chance, status, nodeid
		Dim ParentOK
		Dim OneParentOK
		Dim ExistChild
		Dim MainExecutor
		...
		Dim allOKModel
		Dim Htmls
		Set rs = cn.execute("select x.chance ,x.status, x.MainExecutor, x.Executors, y.jdtype, y.allOKModel,y.nodeid from ChanceProcRunLogs x WITH(NOLOCK) inner join chanceProcNodesBak y WITH(NOLOCK) on x.ProcNodesBak=y.id and x.chance=y.chance where x.id=" & logId)
		If rs.eof = true Then
			cHandleHtml = ""
			rs.close
			Exit function
		else
			nodeid = rs("nodeid").value
			chance = rs("chance").value
			status = rs("status").value
			MainExecutor = rs("MainExecutor").value
			Executors = rs("Executors").value
			jdtype = rs("jdtype").value
			allOKModel = (rs("allOKModel").value = 1)
		end if
		sql =   "set nocount on;" & vbcrlf & _
		"create table #t(nodeid int,nustat int, name nvarchar(100), deep int, haslog int, logstate int) " & vbcrlf & _
		"insert into #t(nodeid, nustat, name, deep, haslog, logstate) " & vbcrlf & _
		"exec [erp_chance_getParentCanRebackNodes_ForLogExit] " & chance & ", " & nodeid & ",0 " & vbcrlf & _
		"select   " & vbcrlf & _
		"isnull(min(case when  isnull(z.status,0) in (2,5,7,8) then 1  else 0 end),1) as n,  " & vbcrlf & _
		"isnull(max(case when  isnull(z.status,0) in (2,5,7,8) then 1  else 0 end),1) as n1 " & vbcrlf & _
		"from #t x  " & vbcrlf & _
		"inner join chanceProcNodesBak y on y.chance = " & chance & " and x.nodeid=y.nodeid and (x.nustat=1 or x.haslog = 1)  " & vbcrlf & _
		"left join ChanceProcRunLogs z on y.id= z.ProcNodesBak and z.[Status] not in (4,6)" & vbcrlf & _
		...
		rs.close
		sql = "exec erp_chance_ExistsNextItemLog " & chance & "," & logId & ",0"
		Set rs = cn.execute(sql)
		If rs.eof = True Then
			ExistChild = false
		else
		...
		rs.close
		isCurrHandle =  MainExecutor*1 = Info.User*1
		Select Case status
		Case 0
		Htmls = "<button class=oldbutton2 onclick='ReadLogPage(""" & app.base64.pwurl(logid) & """)'>详情</button>"
		If isCurrHandle Then
			If ParentOK Or (allOKModel=False And OneParentOK = true) Then
				If jdtype = 1 then
					Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='InitRunLog(" & logid & ")'>启动</button>"
				else
					Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='doSpLog(" & logid & ")'>审核</button>"
				end if
			end if
		end if
		If app.power.existsPowerIntro(3,27, MainExecutor) then
			Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='ChangeRunLog(""" & app.base64.pwurl(logid) & """," & Abs(ParentOK) & ")'>变更</button>"
		end if
		Case 1
		Htmls = "<button class=oldbutton2 onclick='ReadLogPage(""" & app.base64.pwurl(logid) & """)'>详情</button>"
		If isCurrHandle Or InStr("," & Executors & ",", "," & Info.User & ",") > 0 Then
			Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='doRunLog(""" & app.base64.pwurl(logid) & """)'>执行</button>"
		end if
		If app.power.existsPowerIntro(3,27, MainExecutor) then
			Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='ChangeRunLog(""" & app.base64.pwurl(logid) & """," & Abs(ParentOK) & ")'>变更</button>"
		end if
		...
		Case 7
		Htmls = "<button class=oldbutton2 onclick='ReadLogPage(""" & app.base64.pwurl(logid) & """)'>详情</button>"
		If ParentOK And ExistChild = False And isCurrHandle Then
			Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='InitRunLog(""" & (logid) & """)'>启动</button>"
		end if
		Case Else
		Htmls = "<button class=oldbutton2 onclick='ReadLogPage(""" & app.base64.pwurl(logid) & """)'>详情</button>"
		End select
		If app.power.existsPowerIntro(3,3,MainExecutor) Then
			Htmls = Htmls & "&nbsp;<button class=oldbutton2 onclick='DelRunLog(""" & (logid) & """)'>删除</button>"
		end if
		cHandleHtml = Htmls
	end function
	Sub App_InitLog
		cn.execute "update ChanceProcRunLogs set status=1,execTime = GETDATE() where id=" & app.getint("logid")
	end sub
	Sub App_DelLog
		Dim rs, logid
		cn.execute "delete ChanceProcRunLogs where id=" & app.getint("logid")
		cn.execute "delete ChanceProcRunLogs_bak where logid=" & app.getint("logid")
	end sub
	Sub App_lvw_onnulldata (lvw)
		lvw.addhtml "<div align='center'>没有信息！</div>"
	end sub
