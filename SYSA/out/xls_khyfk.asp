...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(8).ColumnWidth=22
			xlApplication.ActiveSheet.Columns(8).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(9).ColumnWidth=16
			...
			xlWorksheet.Range("A2:G2").Borders.LineStyle=1
			Response.Flush
			A=request("A")
			B=request("B")
			C=request("C")
			...
			ElseIf  open_7_10 =1 Then
				Str_Result = Str_Result & " and cateid in ("& intro_7_10 &") and cateid>0 "
			else
...
'if m2<>"" then
			...
			If intro_7_16&""<>"" Then intro_7_16 = Replace(intro_7_16," ","")
			propm = request("propm")
			If propm="1" Then
				if open_7_16=3 then
					Str_Result_65=Str_Result_65&" "
				elseif open_7_16=1 then
					Str_Result_65=Str_Result_65& " and cateid in ("&intro_7_16&") and cateid>0 "
				else
					Str_Result_65=Str_Result_65& " and 1 = 0 "
				end if
				Str_Result=Str_Result+" and ((status_sp in(2,3) and cateid_sp="&session("personzbintel2007")&" "& Str_Result_65 &") or (ISNULL(status_sp,0)=0 and cateid="&session("personzbintel2007")&" and isnull(alt,0)=0))"
				Str_Result_65=Str_Result_65& " and 1 = 0 "
			end if
			...
			If status_sp<>"" And status_sp<>10 Then
				Str_Result=Str_Result+"and  isnull(status_sp,0)="&status_sp&""
'If status_sp<>"" And status_sp<>10 Then
			end if
			If remind <> "" Then
...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from tel where ord=bankin2.company) as companyname,"&_
			"(select name from gate where ord=bankin2.cateid) as cateidname"&_
			" from bankin2 "&Str_Result&" "&px_Result&" "
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "客户预收款列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				dim title,money1,date3,date7,cateid,intro,cateidname,ck,sortbz
				companyname = ""
				company=rs("company")
				if company<>"" then
					cateid_kh = 0
					...
					sql7="select name ,isnull(cateid,0) as cateid , sort3,ISNULL(share,'-222') share , del  from tel where ord="&company&""
					set rs7=server.CreateObject("adodb.recordset")
					...
					If sort3_kh ="2" Then
						...
						If del = 1 Then
							...
							companyname = " 关联客户已被删除"
						end if
					end if
				end if
				money1=Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
				companyname = " 关联客户已被删除"
				date3=rs("date3")
				date7=rs("date7")
				cateid=rs("cateid")
				intro=rs("intro")&""
				bank=rs("bank")
				If rs("status_sp")=0 Or rs("status_sp")&""="" Then
					spstatus="审批通过"
				...
				if bank<>0 then
					...
					rs88.open "select sort1 from sortbank where id="&bank&" and (person like '"&session("personzbintel2007")&",%' or person like '%,"&session("personzbintel2007")&",%' or person like '%, "&session("personzbintel2007")&",%'  or person like '%,"&session("personzbintel2007")&"' or person like '%, "&session("personzbintel2007")&"'  or person like '"&session("personzbintel2007")&"' or person like '0')",conn,1,1
					if rs88.eof then
						bankname=""
					else
						bankname=rs88("sort1")
				...
				sortbz=""
				Set rs7=conn.execute("select top 1 intro from sortbz where id="&rs("bz"))
				If rs7.eof=False  Then
					sortbz = rs7("intro")
				end if
				rs7.close
				set rs7=nothing
				money1=sortbz&money1
				InvoiceMode = rs("InvoiceMode")
				If InvoiceMode&""="" Then InvoiceMode=0
				InvoiceModeName=""
				Select Case InvoiceMode
				Case 0: InvoiceModeName = "销售时开票"
				Case 1: InvoiceModeName = "不开票"
				Case 2: InvoiceModeName = "预收款开票"
				End Select
...
'End Select
				xlWorksheet.Cells(1+i,2).Value = date3
'End Select
				xlWorksheet.Cells(1+i,2).font.Size=10
'End Select
				xlWorksheet.Cells(1+i,3).Value = money1
'End Select
				xlWorksheet.Cells(1+i,3).font.Size=10
'End Select
				xlWorksheet.Cells(1+i,4).Value = bankname
'End Select
...
'End Select
				xlWorksheet.Cells(1+i,6).Value = InvoiceModeName
'End Select
				xlWorksheet.Cells(1+i,6).font.Size=10
'End Select
				xlWorksheet.Cells(1+i,7).Value = spstatus
'End Select
				xlWorksheet.Cells(1+i,7).font.Size=10
'End Select
				xlWorksheet.Cells(1+i,8).Value = Replace(Replace(Replace(Replace(intro,"<br>",vbcrlf),"</P>",vbcrlf),"<P>",vbcrlf),"&nbsp;"," ")
'End Select
				xlWorksheet.Cells(1+i,8).font.Size=10
'End Select
				xlWorksheet.Cells(1+i,9).Value = date7
'End Select
...
'End Select
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户预收款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户预收款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("客户预收款列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的客户预收款列表</u></strong></font></a></p>"
