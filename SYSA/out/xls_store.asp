...
'Private Sub Class_Initialize
		...
		Sub page_load
			Set conn = cn
			dim currpage,page_count,px,showtype,MODULES,rssort,NumDigits,MoneyDigits
			dim sql,rszdy,sflg,m1,m2,A2,strW1,strW2,strW3
			dim strW4,reqPara,pdid,YD_NUM,ZT_NUM,tmppx
			Dim remind,helper
			remind = request("remind")
			pdid=request("pdid")
			px=request("px")
			if px="" or instr(1,px,"-",1)<=0 then
				px=request("px")
				px="1-1_7-1_8-0"
				px=request("px")
			else
				tmppx=strSubtraction(strSubtraction(strSubtraction(px,"1-1","_"),"7-1","_"),"8-0","_")
				px=request("px")
				if tmppx="" then
					px="1-1_7-1_8-0"
'if tmppx="" then
				else
					px=tmppx&"_1-1_7-1_8-0"
'if tmppx="" then
				end if
			end if
			showtype=request("showtype")
			if showtype="" then showtype="1"
			Dim unck_dj,rs
			unck_dj=0
			Set rs= conn.execute("select intro from setopen where sort1=109")
			If rs.eof= False Then
				unck_dj=rs("intro")
			end if
			rs.close
			...
			rs.close
			set rs=nothing
			Dim open_2_1,intro_2_1,open_2_14,intro_2_14,open_21_1,intro_21_1,open_21_14,intro_21_14
			Dim open_23_2,intro_23_2,open_23_21,intro_23_21,open_26_1,intro_26_1,open_26_14,intro_26_14
			Call fillPowerInfo(2,1,open_2_1,intro_2_1)
			Call fillPowerInfo(2,14,open_2_14,intro_2_14)
			Call fillPowerInfo(21,1,open_21_1,intro_21_1)
			Call fillPowerInfo(21,14,open_21_14,intro_21_14)
			Call fillPowerInfo(23,2,open_23_2,intro_23_2)
			Call fillPowerInfo(23,21,open_23_21,intro_23_21)
			Call fillPowerInfo(26,1,open_26_1,intro_26_1)
			Call fillPowerInfo(26,14,open_26_14,intro_26_14)
			Dim CantExport_Fields,cfidx
			cfidx=0
			ReDim CantExport_Fields(100)
			sql="SELECT title,name,set_open,dc FROM zdy WHERE sort1=21 order by gate1"
			set rszdy=conn.execute(sql)
			Dim idx
			Dim zdyfields(),zdyShowName(),zdyRealName()
			ReDim zdyfields(0)
			ReDim zdyShowName(0)
			ReDim zdyRealName(0)
			idx = 0
			while rszdy.eof=False
				ReDim Preserve zdyShowName(idx)
				ReDim Preserve zdyRealName(idx)
				zdyShowName(idx)=rszdy(0)
				zdyRealName(idx)= "自定义字段"&replace(rszdy(1),"zdy","")
				ReDim Preserve zdyfields(idx)
				if cint(replace(rszdy(1),"zdy",""))>4 then
					zdyfields(idx) = "isnull((select top 1 sort1 from sortonehy where ord=bb."&rszdy(1)&"),'') " & zdyRealName(idx)
				else
					zdyfields(idx) = "bb." & rszdy(1) & " " & zdyRealName(idx)
				end if
				If rszdy(2) <> 1 Or rszdy(3) <> 1 Then
					CantExport_Fields(cfidx) = zdyRealName(idx) : cfidx=cfidx+1
'If rszdy(2) <> 1 Or rszdy(3) <> 1 Then
				end if
				rszdy.movenext
				idx = idx + 1
			...
			Dim tmp
			m1=request.querystring("product_date1")
			m2=request.querystring("product_date2")
			A2=replace(request.querystring("a2")," ","")
			Dim rsf
			If A2 <> "" Then
				Set rsf = conn.Execute("SELECT khqy = dbo.GetMenuArea('"& A2 &"','menu')")
				If Not rsf.eof Then
					A2 = rsf(0)
				end if
				rsf.Close
			...
			Dim strCondition : strCondition=""
			Dim baseCondition : baseCondition=""
			dim product_content,product_param,product_intro,product_title,product_order,product_type,product_Unit,product_CK
			dim product_ProCompany,product_PH,product_XLH,product_dateSC1,product_dateSC2,product_dateYX1,product_dateYX2
			dim product_BZ,product_JS1,product_JS2,product_NumKuYD1,product_NumKuYD2,product_NumKuKY1,product_NumKuKY2
			dim product_NumKu1,product_NumKu2,product_CostsKu1,product_CostsKu2
			dim sqlzdy,zdynum,StrCost,baseSQL,cbSQL ,zdySQL ,iwidth,sqlbz,rsbz,i,product_ty,product_BB
			Dim JumpNum : JumpNum = 0
			for each reqPara in request.querystring
				if instr(1,reqPara,"product_")>0 then
					execute(reqPara&"=request.querystring("""&reqPara&""")&""""")
				end if
			next
			if m1<>"" then baseCondition=baseCondition&" and datediff(d,'"&m1&"',bb.date7)>=0"
			if m2<>"" then baseCondition=baseCondition&" and datediff(d,'"&m2&"',bb.date7)<=0"
			if a2<>"" then baseCondition=baseCondition&" and bb.sort1 in ("&a2&")"
			if strW3<>"" then baseCondition=baseCondition&" and bb.addcate in ("&strw3&")"
			if product_content<>"" then baseCondition=baseCondition&" and bb.intro1 like '%"&product_content&"%'"
			if product_param<>"" then baseCondition=baseCondition&" and bb.intro2 like '%"&product_param&"%'"
			if product_intro<>"" then baseCondition=baseCondition&" and bb.intro3 like '%"&product_intro&"%'"
			baseCondition=baseCondition&" and bb.canoutstore=1 "
			If product_BB="" Then product_BB="1"
			if product_ty<>"" then
				if product_BB="1" then
					strCondition = strCondition & " and 产品名称 like '%"&Trim(product_ty)&"%'"
				elseif product_BB="2" then
					strCondition = strCondition & " and 产品编号 like '%"&Trim(product_ty)&"%'"
				elseif product_BB="3" then
					strCondition = strCondition & " and 产品型号 like '%"&Trim(product_ty)&"%'"
				end if
			elseif pdid<>"" And isnumeric(pdid) Then
				strCondition=strCondition&" and 产品ID in ("&pdid&")"
			end if
			Dim propm, newstate, rs1
			propm = request("propm")
			newstate = request("newstate")
			If propm&"" = "1" And showtype&""="2" Then
				If remind <> "" Then
					Set helper = CreateReminderHelper(conn,52,0)
					baseCondition = baseCondition & " and aa.id in (" & helper.listSql("ids_withoutOrderBy") & ")"
				end if
				If px = "1-1_7-1_8-1" Then
					baseCondition = baseCondition & " and aa.id in (" & helper.listSql("ids_withoutOrderBy") & ")"
					px="13-1"
					baseCondition = baseCondition & " and aa.id in (" & helper.listSql("ids_withoutOrderBy") & ")"
				end if
			end if
			If newstate&"" = "1" And showtype="2" Then
				Set rs1=conn.execute("select id,ord,intro,num1,isnull(tq1,0) tq1,fw1,gate1 from setjm2  where intro=1  and cateid="&info.user&" and ord=52")
				If rs1.eof = False Then
					baseCondition=baseCondition&" and ("&open_21_1&"=3 or ("&open_21_1&"=1 and charindex(',' + cast(bb.addcate as nvarchar(10)) + ',',',' + '"&intro_21_1&"' + ',') > 0)) and bb.RemindNum>0 and charindex(',"& info.user &",',','+isnull(cast(bb.RemindPerson as varchar(4000)),'')+',')>0 and dbo.dateDiffByDay(aa.dateyx,bb.RemindUnit,bb.RemindNum,"& rs1("tq1") &",getdate())>=0 and isnull(aa.status,0)=0 and charindex(',"& info.user &",',','+isnull(aa.unRemind,'')+',')=0 and ("&rs1("fw1")&"=0 or bb.addcate="&info.user&") "
'If rs1.eof = False Then
				else
					baseCondition=baseCondition&" and 1=0 "
				end if
				rs1.close
				Set rs1 = Nothing
				If px = "1-1_7-1_8-1" Then
			...
			If propm&"" = "2" And showtype&""="2" Then
				If remind <> "" Then
			...
			If newstate&"" = "2" And showtype&""="2" Then
				Set rs1=conn.execute("select id,ord,intro,num1,isnull(tq1,0) tq1,fw1,gate1 from setjm2  where intro=1  and cateid="&info.user&" and ord=93")
				If rs1.eof = False Then
					baseCondition=baseCondition&" and isnull(aa.locked,0)=0 and LEN(aa.dateyx)>0 and ISNULL(bb.RemindNum,0)>0 and LEN(cast(bb.RemindPerson as varchar(10)))>0      and CHARINDEX(',"&info.user&",',','+cast(bb.RemindPerson as varchar(4000))+',')>0     and len(ISNULL(m.date1,aa.datesc))>0 and dbo.dateDiffByDay(ISNULL(m.date1,ISNULL(aa.datesc,'1970-1-1')),bb.MaintainUnit,bb.MaintainNum,3,GETDATE())>=0 and charindex(',"& info.user &",',','+isnull(isnull(m.alt2,aa.alt2),'')+',')=0  "
'If rs1.eof = False Then
				else
					baseCondition=baseCondition&" and 1=0 "
				end if
			...
			if link = "yes" then
				if request("ck")<>"" then
					product_CK=request("ck")
				end if
				if request("unit")<>"" then
					product_Unit=request("unit")
				end if
				if ord<>"" then
					strCondition = strCondition & " and 产品ID in ("&ord&")"
				end if
			end if
			If request("qrcode") <> "" Then
				baseCondition = baseCondition & " and bb.ord = " & CLng("0" & request("qrcode")) & " "
			end if
			if product_title<>"" then strCondition = strCondition & " and 产品名称 like '%"&Trim(product_title)&"%'"
			if product_order<>"" then strCondition = strCondition & " and 产品编号 like '%"&Trim(product_order)&"%'"
			if product_type<>"" then strCondition = strCondition & " and 产品型号 like '%"&Trim(product_type)&"%'"
			if product_Unit<>"" then strCondition = strCondition & " and 单位ID in ("& product_Unit &")"
			if product_CK<>"" then strCondition = strCondition & " and 仓库ID in ("& product_CK &")"
			if showtype="2" then
				if product_ProCompany<>"" then strCondition = strCondition & " and 关联供应商 like '%"&product_ProCompany&"%'"
				if product_PH<>"" then strCondition = strCondition & " and 批号 like '%"&product_PH&"%'"
				if product_XLH<>"" then strCondition = strCondition & " and 序列号 like '%"&product_XLH&"%'"
				if product_dateSC1<>"" then strCondition = strCondition & " and datediff(d,'"&product_dateSC1&"',生产日期)>=0"
				if product_dateSC2<>"" then strCondition = strCondition & " and datediff(d,'"&product_dateSC2&"',生产日期)<=0"
				if product_dateYX1<>"" then strCondition = strCondition & " and datediff(d,'"&product_dateYX1&"',有效日期)>=0"
				if product_dateYX2<>"" then strCondition = strCondition & " and datediff(d,'"&product_dateYX2&"',有效日期)<=0"
				if product_BZ<>"" then strCondition = strCondition & " and 包装ID in ("& product_BZ &")"
				if product_JS1<>"" then strCondition = strCondition & " and 件数 >= " & cdbl(product_JS1)
				if product_JS2<>"" then strCondition = strCondition & " and 件数 <= " & cdbl(product_JS2)
			else
				if product_NumKuYD1<>"" then strCondition = strCondition & " and 预定数量 >= " & cdbl(product_NumKuYD1)
				if product_NumKuYD2<>"" then strCondition = strCondition & " and 预定数量 <= " & cdbl(product_NumKuYD2)
				if product_NumKuKY1<>"" then strCondition = strCondition & " and ISNULL(库存数量-预定数量+在途数量,0) >= " & cdbl(product_NumKuKY1)
				if product_NumKuYD2<>"" then strCondition = strCondition & " and 预定数量 <= " & cdbl(product_NumKuYD2)
				if product_NumKuKY2<>"" then strCondition = strCondition & " and ISNULL(库存数量-预定数量+在途数量,0) <= " & cdbl(product_NumKuKY2)
				if product_NumKuYD2<>"" then strCondition = strCondition & " and 预定数量 <= " & cdbl(product_NumKuYD2)
			end if
			if product_NumKu1<>"" then strCondition = strCondition & " and 库存数量 >= " & cdbl(product_NumKu1)
			if product_NumKu2<>"" then strCondition = strCondition & " and 库存数量 <= " & cdbl(product_NumKu2)
			if open_23_2=1 then
				if product_CostsKu1<>"" then strCondition = strCondition & " and 库存成本 >= " & cdbl(product_CostsKu1)
				if product_CostsKu2<>"" then strCondition = strCondition & " and 库存成本 <= " & cdbl(product_CostsKu2)
			end if
			sqlzdy="SELECT name FROM zdy WHERE sort1=21 AND set_open=1 and js=1 order by gate1"
			set rszdy=conn.execute(sqlzdy)
			while rszdy.eof=false
				zdynum=replace(rszdy(0),"zdy","")
				execute("if product_zdy"&zdynum&"<>"""" then strCondition = strCondition & "" and 自定义字段"&zdynum&" like '%""&product_zdy"&zdynum&"&""%'""")
				rszdy.movenext
			wend
			rszdy.close
			set rszdy=nothing
			Dim StrStore,getOrdSql,Export_Fields,customFields
			customFields = Join(zdyRealName,",")
			zdySQL = app.iif(Len(customFields)>0,"," & Join(zdyFields,","),"")
			if showtype="1" Then
				Export_Fields = "产品名称,产品ID,主键ID,产品编号,产品型号,delID,单位,单位ID,仓库ID,库存数量,预定数量,在途数量,冻结数量,可用数量,库存成本,仓库"& app.iif(Len(customFields)>0,","& customFields,"") &",详情"
				if open_23_2=1 Then
					cbSQL =  " OR bbb.库存成本<>0 "
				else
					CantExport_Fields(cfidx) = "库存成本" : cfidx=cfidx+1 : JumpNum = JumpNum + 1
					cbSQL =  " OR bbb.库存成本<>0 "
					Export_Fields = Replace(Export_Fields,"库存成本,","")
				end if
				CantExport_Fields(cfidx) = "产品ID" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
				CantExport_Fields(cfidx) = "主键ID" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
				CantExport_Fields(cfidx) = "delID" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
				CantExport_Fields(cfidx) = "单位ID" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
				CantExport_Fields(cfidx) = "仓库ID" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
				CantExport_Fields(cfidx) = "详情" : cfidx=cfidx+1
				Export_Fields = Replace(Export_Fields,"库存成本,","")
			else
				Export_Fields = "产品名称,产品ID,主键ID,产品编号,产品型号,delID,单位,单位ID,仓库ID,库存数量,冻结数量,最大容量,剩余容量,库存成本,批号,序列号,生产日期,有效日期,包装ID,包装,件数,仓库" & app.iif(Len(customFields)>0,"," & customFields,"") &",供应商ID,供应商采购人ID,关联供应商,操作"
				CantExport_Fields(cfidx) = "产品ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "主键ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "delID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "单位ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "仓库ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "包装ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "供应商ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "供应商采购人ID" : cfidx=cfidx+1
				CantExport_Fields(cfidx) = "操作" : cfidx=cfidx+1
				if open_23_2<>1 then
					CantExport_Fields(cfidx) = "库存成本" : cfidx=cfidx+1 : JumpNum = JumpNum + 1
'if open_23_2<>1 then
					Export_Fields = Replace(Export_Fields,"库存成本,","")
				...
				if proStore<>1 then
					CantExport_Fields(cfidx) = "最大容量" : cfidx=cfidx+1
'if proStore<>1 then
					CantExport_Fields(cfidx) = "剩余容量" : cfidx=cfidx+1 : JumpNum = JumpNum + 2
'if proStore<>1 then
					Export_Fields = Replace(Export_Fields,"最大容量,剩余容量,","")
				end if
				If open_23_21 <> 1 Then
					CantExport_Fields(cfidx) = "操作" : cfidx=cfidx+1
'If open_23_21 <> 1 Then
				end if
				If (propm&""="2" Or newstate&"" = "2") And showtype&""="2" Then
					cbSQL =  " left join (select m2.ku,m2.ord,m3.date1,m2.alt2 from maintain m1 inner join (select maintain,ord,ku,MAX(alt2) alt2 from maintainlist where del=1 group by maintain,ord,ku) m2 on m2.maintain=m1.ord " &_
					"                  inner join ( " &_
					"                          select m2.ord, m2.ku, max(m1.date1) date1 " &_
					"                          from maintain m1 inner join maintainlist m2 on m2.maintain=m1.ord and m2.del=1 " &_
					"                          inner join product p on p.ord=m2.ord and p.del=1  " &_
					"                          and ISNULL(p.RemindNum,0)>0 and LEN(cast(p.RemindPerson as varchar(10)))>0 " &_
					"                          where m1.del=1 and isnull(m1.status,0)=0 " &_
					"                          and CHARINDEX(',"& info.user &",',','+cast(p.RemindPerson as varchar(4000))+',')>0 " &_
					"                          where m1.del=1 and isnull(m1.status,0)=0 " &_
					"                          group by m2.ord,m2.ku    " &_
					"                  )m3 on m2.ord=m3.ord and m2.ku=m3.ku     " &_
					"where m1.del=1 and isnull(m1.status,0)=0 and m1.date1=m3.date1) m on m.ku=aa.id and bb.ord=m.ord "
				end if
			end if
			If px="15-1_2-2" Then
'where m1.del=1 and isnull(m1.status,0)=0 and m1.date1=m3.date1) m on m.ku=aa.id and bb.ord=m.ord 
...
'where m1.del=1 and isnull(m1.status,0)=0 and m1.date1=m3.date1) m on m.ku=aa.id and bb.ord=m.ord 
			...
			Dim settingIdx,tt,fieldShowSql,fieldOrderSql
			sql = "select * from setjm3 where ord={1} and cateid=" & info.user
			if showtype & "" = "1" Then
				tt = "产品库存列表(汇总模式)"
				settingIdx=2301
			else
			...
			fieldShowSql = Replace(sql,"{1}",settingIdx)
			fieldOrderSql = Replace(sql,"{1}",settingIdx + 10000)
			fieldShowSql = Replace(sql,"{1}",settingIdx)
			Set rs = cn.execute(fieldOrderSql)
			Dim arrExportFields,exoprtShowFields,userShowFields,maxIdx
			arrExportFields = Split(Export_Fields,",")
			exoprtShowFields = ""
			maxIdx = ubound(arrExportFields)
			Dim strOrder,arrpx,arrsort,stype
...
'For i = 0 To UBound(arrpx)
				...
				If strOrder = "" Then
					strOrder = "["&arrExportFields(CInt(arrsort(0)))&"]" & stype
				else
					strOrder = strOrder&",["&arrExportFields(CInt(arrsort(0))) &"]"& stype
				end if
			next
			Dim topSQL
			If rs.eof Then
				For i = 0 To maxIdx
					exoprtShowFields = exoprtShowFields & app.iif(exoprtShowFields="","",",") & arrExportFields(i)
				next
				topSQL = ""
				strOrder = ""
			else
				userShowFields = Split(rs("intro"),",")
				For i = 0 To ubound(userShowFields)
					If CInt(userShowFields(i)) <= ubound(arrExportFields) Then
						exoprtShowFields = exoprtShowFields & app.iif(exoprtShowFields="","",",") & arrExportFields(CInt(userShowFields(i)))
					end if
				next
				topSQL = "top 100 percent"
				strOrder = " order by " & strOrder
			end if
			rs.close
			Set rs=cn.execute(fieldShowSql)
			Dim ttt
			If rs.eof = False Then
				userShowFields = Split(rs("intro"),",")
				For i= 0 To ubound(arrExportFields)
					If InStr(","&rs("intro")&",",","&i&",")<=0 Then
						CantExport_Fields(cfidx) = arrExportFields(i) : cfidx=cfidx+1
'If InStr(","&rs("intro")&",",","&i&",")<=0 Then
						ttt = ttt & "," & arrExportFields(i)
					end if
				next
				app.addlog fieldShowSql
				app.addlog ttt
				app.addlog Join(arrExportFields,",")
			end if
			rs.close
			Dim adapter
			...
			adapter.fileName = tt
			if showtype="1" Then
				adapter.lvw.distinctSpaceCol = "产品ID,产品名称,产品编号,产品型号|产品ID,产品名称,产品编号,产品型号,单位;预定数量,在途数量|产品ID,产品名称,产品编号,产品型号,单位,仓库ID"
			else
				adapter.lvw.distinctSpaceCol = "产品ID,产品名称,产品编号,产品型号|产品ID,产品名称,产品编号,产品型号,单位|产品ID,产品名称,产品编号,产品型号,单位,仓库ID"
			end if
			basesql = " exec erp_xls_store "& info.user &", '"& showtype &"','"& Replace(cbSQL,"'","''") &"', '"& replace(baseCondition,"'","''") &"','"& replace(topSQL,"'","''") &"','"& replace(strOrder,"'","''") &"','"& replace(zdySQL,"'","''") &"','"& replace(exoprtShowFields,"'","''") &"' ,'" &replace(strCondition,"'","''")  & "' "
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = basesql
			sheetSetting.title = tt
			For i = 0 To cfidx
				If isEmpty(CantExport_Fields(i)) Then Exit For
				Set hd = New HeaderColumnSetting
				hd.dbname = CantExport_Fields(i)
				hd.display = "none"
				sheetSetting.headerSettings.add(hd)
			next
			Dim hd,arrFields,field
			arrFields = Split(exoprtShowFields,",")
			For i = 0 To ubound(arrFields)
				field = arrFields(i)
				Set hd = New HeaderColumnSetting
				hd.dbname = field
				Select Case field
				Case "库存数量","预定数量","在途数量","可用数量","冻结数量":
				hd.dbtype = "number"
				hd.excelAlign = "right"
				hd.width = 165
				hd.canSum = True
				Select Case field
				Case "可用数量":
				hd.Formula = "calcAvailableNum(rs,isReatCol,"&unck_dj&")"
				hd.tryCurrSumWhenRepeat = True
				Case "库存数量","冻结数量","在途数量":
				hd.ignoreNonnumeric = True
				hd.tryCurrSumWhenRepeat = True
				Case Else
				hd.tryCurrSumWhenRepeat = False
				End Select
				Case "件数":
				hd.width = 165
				hd.dbtype = "number"
				...
				Case "最大容量","剩余容量":
				hd.width = 165
				hd.Formula = "getStoreCapacityShowStr(rs,@value,info)"
				hd.dbtype = "str"
				hd.excelAlign = "left"
				Case "自定义字段1","自定义字段2","自定义字段3","自定义字段4":
				hd.title = getZdyShowName(field,zdyRealName,zdyShowName)
				hd.width = 205
				Case "自定义字段5","自定义字段6":
				hd.title = getZdyShowName(field,zdyRealName,zdyShowName)
				hd.width = 87
		...
		Function calcAvailableNum(rs,isReatCol,unck_dj)
			If unck_dj = 1 Then
				calcAvailableNum = cdbl(rs("库存数量"))-cdbl(app.iif(isReatCol(1)=true,0,cdbl(rs("预定数量"))))+cdbl(app.iif(isReatCol(1)=true,0,cdbl(rs("在途数量"))))-cdbl(rs("冻结数量"))
'If unck_dj = 1 Then
			else
				calcAvailableNum = cdbl(rs("库存数量"))-cdbl(app.iif(isReatCol(1)=true,0,cdbl(rs("预定数量"))))+cdbl(app.iif(isReatCol(1)=true,0,cdbl(rs("在途数量"))))
'If unck_dj = 1 Then
			end if
		end function
		Function getZdyShowName(n,arrReal,arrShow)
			Dim i
			For i = 0 To ubound(arrReal)
				If arrReal(i) = n Then
					getZdyShowName = arrShow(i)
					Exit Function
				end if
			next
			getZdyShowName = ""
		end function
		Function getStoreCapacityShowStr(rs,v,info)
			If rs("最大容量")&"" = "" Then
				getStoreCapacityShowStr = "------"
'If rs("最大容量")&"" = "" Then
			else
				getStoreCapacityShowStr = FormatNumber(v,info.FloatNumber,-1,0,-1)
			end if
		end function
