...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "#content tr.top td{padding:0 4px!important;background:#f4fafe;line-height:22px;}" & vbcrlf & ".IE5 .top_btns input.anybutton2 {height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & ".top_btns  select{height:20px!important;line-height:20px;}" & vbcrlf & ".IE5 .top_btns  select{height:22px!important;line-height:22px;}" & vbcrlf & "</style>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf & "<!--" & vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0" & vbcrlf & "eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf & "if (restore) selObj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "//-->" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body onMouseOver=""window.status='none';return true;""  scroll=AUTO>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dim MODULES
	MODULES=session("zbintel2010ms")
	dim st,W1,W2,W3,m1,m2,E,F,time1
	W1=request("W1")
	W2=request("W2")
	W3=request("W3")
	if W3<>"" then
	...
	if sort_lm<>"0" then
		Str_Result=Str_Result+"and  sort1="&sort_lm&""
'if sort_lm<>"0" then
		Str_Result2=Str_Result2+"and  sort1="&sort_lm&""
'if sort_lm<>"0" then
	end if
	if sort_lm="1" then
		sort_lmname="客户跟进"
...
'elseif time1=15 then
	...
	if E<>"" then
		Str_Result=Str_Result+"and  gj1="&E&""
'if E<>"" then
		Str_Result2=Str_Result2+"and  gj1="&E&""
'if E<>"" then
	end if
	if F<>"" then
		Str_Result=Str_Result+"and  gj2="&F&""
'if F<>"" then
		Str_Result2=Str_Result2+"and  gj2="&F&""
'if F<>"" then
	end if
	If khjz <> "" Then
		Str_Result2 = Str_Result2 & " AND ord IN (SELECT ord FROM tel WHERE jz = "& khjz &") "
	end if
	If khhy <> "" Then
		Str_Result2 = Str_Result2 & " AND ord IN (SELECT ord FROM tel WHERE trade = "& khhy &") "
	end if
	If khly <> "" Then
		Str_Result2 = Str_Result2 & " AND ord IN (SELECT ord FROM tel WHERE ly = "& khly &") "
	end if
	if W1<>"" then
		set rs=server.CreateObject("adodb.recordset")
	...
	Module_Result=""
	if ZBRuntime.MC(12001) and ZBRuntime.MC(1000) then
	else
		Module_Result=Module_Result+" and  sort1<>1 "
'if ZBRuntime.MC(12001) and ZBRuntime.MC(1000) then
	end if
	if ZBRuntime.MC(12003) and ZBRuntime.MC(3000) then
	else
		Module_Result=Module_Result+" and  sort1<>2 "
'if ZBRuntime.MC(12003) and ZBRuntime.MC(3000) then
	end if
	if ZBRuntime.MC(12004) and ZBRuntime.MC(4000) then
	else
		Module_Result=Module_Result+" and  sort1<>3 "
'if ZBRuntime.MC(12004) and ZBRuntime.MC(4000) then
	end if
	if ZBRuntime.MC(12006) and ZBRuntime.MC(7000) then
	else
		Module_Result=Module_Result+" and  sort1<>4 "
'if ZBRuntime.MC(12006) and ZBRuntime.MC(7000) then
	end if
	if ZBRuntime.MC(12010) and ZBRuntime.MC(15000) then
	else
		Module_Result=Module_Result+" and  sort1<>5 "
'if ZBRuntime.MC(12010) and ZBRuntime.MC(15000) then
	end if
	if ZBRuntime.MC(12007) and ZBRuntime.MC(9000) then
	else
		Module_Result=Module_Result+" and  sort1<>6 "
'if ZBRuntime.MC(12007) and ZBRuntime.MC(9000) then
	end if
	if ZBRuntime.MC(12008) and ZBRuntime.MC(8000) then
	else
		Module_Result=Module_Result+" and  sort1<>7 "
'if ZBRuntime.MC(12008) and ZBRuntime.MC(8000) then
	end if
	if ZBRuntime.MC(12002) and ZBRuntime.MC(2000) then
	else
		Module_Result=Module_Result+" and  sort1<>8 "
'if ZBRuntime.MC(12002) and ZBRuntime.MC(2000) then
	end if
	if ZBRuntime.MC(12000) and ZBRuntime.MC(9003) then
	else
		Module_Result=Module_Result+" and  sort1<>102001 "
'if ZBRuntime.MC(12000) and ZBRuntime.MC(9003) then
	end if
	if ZBRuntime.MC(12000) and ZBRuntime.MC(9004) then
	else
		Module_Result=Module_Result+" and  sort1<>102002 "
'if ZBRuntime.MC(12000) and ZBRuntime.MC(9004) then
	end if
	Response.write " " & vbcrlf & "<div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "<table class=""resetTableBg"" width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class=""place"">客户跟进统计</td>                    " & vbcrlf & "    <td align=""left""><strong>"
	...
	Response.write "客户跟进每日走势"
	Response.write m1name
	Response.write m2name
	Response.write sw
	Response.write "</strong></td>" & vbcrlf & "    <td width=""3""><img class=""resetElementHidden"" src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style计条件：" & vbcrlf & "  <select name=""sort_lm"">" & vbcrlf & "    <option value=""0"">跟进分类</option>" & vbcrlf & "        "
	if ZBRuntime.MC(12001) and ZBRuntime.MC(1000) then
	...
	Response.write "" & vbcrlf & "<select name=""time1"">" & vbcrlf & "  <option value=""0"" "
	if time1=0 then
		Response.write "selected"
	end if
	Response.write ">全天</option>" & vbcrlf & "  <option value=""9"" "
	if time1=9 then
		Response.write "selected"
	end if
	Response.write ">上午</option>" & vbcrlf & "  <option value=""15"" "
	if time1=15 then
		Response.write "selected"
	end if
	Response.write ">下午</option>" & vbcrlf & "</select>" & vbcrlf & "<input type=""submit"" name=""Submit422"" value=""统计""  class=""anybutton2""/></td></form>" & vbcrlf & "  </tr></table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30""  align=""right"" class=""toP_btns"">" & vbcrlf & "        <input type=""button"" name=""pillar"" value=""柱状图分析"" onClick=""window.location.href='kh_gj4.asp?type_tj=1&W1="
	Response.write W1
	Response.write "&W2="
	Response.write W2
	Response.write "&W3="
	...
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "          <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""window.location.href='kh_gj4.asp?type_tj=2&W1="
	Response.write W1
	Response.write "&W2="
	Response.write W2
	Response.write "&W3="
	...
	Response.write "&width_tj="
	Response.write width_tj
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""window.location.href='kh_gj4.asp?type_tj=3&W1="
	Response.write W1
	Response.write "&W2="
	Response.write W2
	Response.write "&W3="
	...
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""kh_gj4.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""kh_gj4.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""kh_gj4.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""kh_gj4.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""kh_gj4.asp?type_tj="
	Response.write type_tj
	Response.write "&W1="
	Response.write W1
	Response.write "&W2="
	...
	if open_1_10=1 or open_1_10=3  then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='kh_gj4.asp?type_tj=4&W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
	...
	Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" id=""user2"">" & vbcrlf & "<tr bgcolor=""#FFFFFF""><td height=""39"" align=""left""><div align=""left""><span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt;"">" & vbcrlf & "  <B>正在处理数据...</B></span></div></td></tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & " <table width=""600"" border=""0"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr bgcolor=""#FFFFFF"">  " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage2"" name=""CountImage2"" align=""absmiddle"">" & vbcrlf & "</table>" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr bgcolor=""#FFFFFF""><td><span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" &vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""center"">" & vbcrlf & "</td></tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	dim numcg1,zs1
	tdw=m1
	counts=m
	If counts<0 Then counts = 0
	...
	for j=1 to m
		set rs=server.CreateObject("adodb.recordset")
		if sort_lm=1 then
			powerWhere = " and ((("&open_1_5&"=3 or ("&open_1_5&"=1 AND CHARINDEX(','+CAST(cateid AS VARCHAR(50))+',' , ','+'"&intro_1_5&"'+',') > 0)) and ("&open_1_11&"=3 or ("&open_1_11&"=1 AND CHARINDEX(','+CAST(cateid AS VARCHAR(50))+',' , ','+'"&intro_1_11&"'+',') > 0))) or CHARINDEX(','+CAST("&session("personzbintel2007")&" AS VARCHAR(50))+',' , ','+(case b.replyShare when 0 then '0' else isnull(share,'0') end)+',') > 0  or (case b.replyShare when 0 then '0' else isnull(share,'0') end) = '1')"
'if sort_lm=1 then
			sql="select count(ord) as numcg1 from reply a inner join (select ord as khord, isnull(replyShare,0) replyShare from tel where del=1) b on b.khord=a.ord and sort1=1 and cateid>0  and date7>='"&cdate(tdw)&"' and date7<'"&dateadd("d",1,tdw)&"'  "&Module_Result&" "&powerWhere&" "
		else
			powerWhere = " and (("&open_1_5&"=3 or ("&open_1_5&"=1 AND CHARINDEX(','+CAST(cateid AS VARCHAR(50))+',' , ','+'"&intro_1_5&"'+',') > 0)) or CHARINDEX(','+CAST("&session("personzbintel2007")&" AS VARCHAR(50))+',' , ','+(case b.replyShare when 0 then '0' else isnull(share,'0') end)+',') > 0  or (case b.replyShare when 0 then '0' else isnull(share,'0') end) = '1')"
			sql="select count(ord) as numcg1 from reply a inner join (select ord as khord, isnull(replyShare,0) replyShare from tel where del=1) b on b.khord=a.ord and date7>='"&cdate(tdw)&"' and date7<'"&dateadd("d",1,tdw)&"'  "&Module_Result&"  "&Str_Result2&" "&powerWhere&" "
		end if
		rs.open sql,conn,1,1
		numcg1=zbcdbl(rs("numcg1").value)
		rs.close
		set rs=nothing
		title=month(tdw)&"月"&day(tdw)&"日"
		lburl="../work/genjin.asp?W1="&W1&"&W2="&W2&"&W3="&W3&"&sort="&E&"&sort1="&F&"&khjz="&khjz&"&khhy="&khhy&"&khly="&khly&"&ret="&tdw&"&ret2="&dateadd("d",1,tdw)-1&"&time1="&time1&"&sort_lm="&sort_lm&""
		title=month(tdw)&"月"&day(tdw)&"日"
		total1(j,1)=numcg1
		total1(j,2)=title
		total1(j,0)=lburl
		total2(j,0)=title
		total2(j,1)=numcg1
		total2(0,1)="#FF0000,1.5,1,3,跟进走势"
		total2(j,2)=lburl
		total3(j,0)=title
		total3(j,1)=numcg1
		total3(j,2)=lburl
