...
'If Not rs.Eof Then
	...
	if ord="" or ord=0 then
		if request("sort1")<>"" and request("order1")<>"" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from sortMessage where  sortName='"&request("sort1")&"' "
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "insert into sortMessage(sortName,order1,addtime,addcate) values('"&request("sort1")&"',"&replace(request("order1")," ","")&",'"&now()&"',"&session("personzbintel2007")&")"
				Response.write "" & vbcrlf & "                      <script language=javascript>" & vbcrlf & "                    <!--" & vbcrlf & "                            alert('添加成功');" & vbcrlf & "                              try {" & vbcrlf & "                                   window.opener.location.reload();" & vbcrlf & "                                }" & vbcrlf & "                               catch (e1) { }" & vbcrlf & "                          window.location.href='editSort.asp';" & vbcrlf & "                   " & vbcrlf & "                     -->" & vbcrlf & "                     </script>" & vbcrlf & "                       "
				Response.end
			else
				Response.write"<script language=javascript>alert('该名称已存在！'); history.back();</script>"
				call db_close : Response.end
			end if
		else
			Response.write"<script language=javascript>alert('名称或重要指数不能为空！'); history.back();</script>"
			call db_close : Response.end
		end if
	else
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from sortMessage where   sortName='"&request("sort1")&"'  and ord<>"&ord&" "
		rs.open sql,conn,1,1
		if rs.eof then
			sqltext="Update sortMessage set sortName='"&request("sort1")&"',order1="&request("order1")&",editTime='"&now()&"',editcate="&session("personzbintel2007")&" where  ord="&ord&""
			conn.execute(sqltext)
			Response.write"<script language=javascript>alert('修改成功！');window.opener.location.reload();window.open('','_self');window.close();</script>"
			call db_close : Response.end
		else
			Response.write"<script language=javascript>alert('该名称已存在！'); history.back();</script>"
			call db_close : Response.end
