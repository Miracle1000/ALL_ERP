...
'Private Sub Class_Initialize
	...
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/cr_List_DcarUse_2.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if open_102_1=3 then
		list=""
	elseif open_102_1=1 then
		list=" and b.use_cateid in ("&intro_102_1&")"
	else
		list=" and b.use_cateid=0"
	end if
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and b.use_cateid in ("&W3&") "
	end if
	if D1<>"" then
	...
	if D251<>"" then
		Str_Result=Str_Result&" and a.use_notes like '%"&ml.De(D251)&"%'"
	end if
...
'if D29<>"" and D29<>"点击选择时间" then
	...
	if D30<>"" then
		Str_Result=Str_Result&" and b.use_complete in ("&D30&")"
	end if
	M=ChangeDate(M,1)
...
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	if zdy1<>"" then
		Str_Result=Str_Result&" and b.use_zdy1 like '%"&ml.De(zdy1)&"%'"
	end if
	if zdy2<>"" then
		Str_Result=Str_Result&" and b.use_zdy2 like '%"&ml.De(zdy2)&"%'"
	end if
	if zdy3<>"" then
		Str_Result=Str_Result&" and b.use_zdy3 like '%"&ml.De(zdy3)&"%'"
	end if
	if zdy4<>"" then
		Str_Result=Str_Result&" and b.use_zdy4 like '%"&ml.De(zdy4)&"%'"
	end if
	if zdy5<>"" then
		Str_Result=Str_Result&" and b.use_zdy5 in ("&zdy5&")"
	end if
	if zdy6<>"" then
		Str_Result=Str_Result&" and b.use_zdy6 in ("&zdy6&")"
	end if
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by b.use_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by b.use_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by a.use_startime desc"
	elseif px="4" then
		Ptr_Result=" order by a.use_startime asc"
	elseif px="5" then
		Ptr_Result=" order by a.use_endtime desc"
	elseif px="6" then
		Ptr_Result=" order by a.use_endtime asc"
	elseif px="7" then
		Ptr_Result=" order by a.use_rettime desc"
	elseif px="8" then
		Ptr_Result=" order by a.use_rettime asc"
	end if
	if C<>"" then
		if B="chp" then
			Str_Result=Str_Result&" and d.car_code like '%"&ml.De(C)&"%'"
		elseif B="mc" then
			Str_Result=Str_Result&" and use_mdd like '%"&ml.De(C)&"%'"
		elseif B="xh" then
			Str_Result=Str_Result&" and use_thing like '%"&ml.De(C)&"%'"
		end if
	end if''
	Dim remind,helper
	...
	If request("remind") <> "" Then
		...
		Str_Result= Str_Result & " and b.use_id in (" & helper.listSQL("ids") & ")"
	end if
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_DcarUse.asp"" method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "           <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">申请明细表</td>" & vbcrlf & "                      <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">     " & vbcrlf & "                <tr>" & vbcrlf & "              <td colspan=""11""  background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;"" class=""top_btns"">" & vbcrlf & "          <span id=""kh""><input type=""radio"" name=""sss"" onClick=""window.location='List_carUse.asp?remind="
	Response.write remind
	Response.write "'"">申请列表<input type=""radio"" name=""ddd"" onClick=""window.location='List_DcarUse.asp?remind="
	Response.write remind
	Response.write "'"" checked=""checked"">申请明细表&nbsp;自：<input type=""text"" value="""
	Response.write ret
	Response.write """ name=""ret"" id=""ret"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" readonly=""readonly"">&nbsp;至：<input type=""text"" value="""
	Response.write ret2
	Response.write """ name=""ret2"" id=""ret2"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" readonly=""readonly"">" & vbcrlf & "          <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                               <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""chp"" "
	Response.write ret2
	...
	Response.write ">目的地</option>" & vbcrlf & "                             <option value=""xh"" "
	if B="xh" then Response.write "selected"
	Response.write ">事由</option>" & vbcrlf & "                       </select>" & vbcrlf & "               <input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	Response.write """ size=""10"" style=""width:80px;""><input name=""page_count"" type=""hidden"" value="""
	...
	if open_102_10<>0 then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_DcarUse.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span>                       </td>" & vbcrlf & "           </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""8%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">车牌号</div></td>" & vbcrlf & "                   <td width=""6%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">使用人员</div></td>" & vbcrlf & "                  <td width=""6%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">用车性质</div></td>" & vbcrlf &                  "<td width=""10%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">开始时间</div></td>" & vbcrlf &                  "<td width=""11%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">结束时间</div></td>" & vbcrlf &  "                <td width=""10%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">返还时间</div></td>" & vbcrlf & "                <td width=""13%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">目的地</div></td>" & vbcrlf & "                   <td width=""11%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">事由</div></td>" & vbcrlf &                  "<td width=""7%"" align=""left"" valign=""middle"" class=""name""><div align=""center"">司机</div></td>" & vbcrlf &               "<td width=""12%"" align=""left"" valign=""middle"" class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                     <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.use_id as fid,b.use_complete as complete,b.use_cateid,c.name,d.car_code,e.drivername from O_carUsemx a "&_
	" left join O_carUse b on a.use_fid=b.use_id "&_
	" left join gate c on b.use_cateid=c.ord "&_
	" left join O_carData d on a.use_carid=d.car_id "&_
	" left join O_carDriver e on a.use_driver=e.id "&Str_Result&" "&Ptr_Result
	...
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			Response.write "" & vbcrlf & "                  </div></td>" & vbcrlf & "             <td width=""8%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "         "
			if open_102_14=3 or CheckPurview(intro_102_14,trim(use_cateid))=True then
				Response.write "" & vbcrlf & "               <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_carUse.asp?ord="
			...
			Response.write "</td>" & vbcrlf & "            <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("name")
			Response.write "</div></td>" & vbcrlf & "              <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			if rs7("use_catetype")=1 then
				Response.write "公用"
			else
				Response.write "私用"
			end if
			Response.write "</div></td>" & vbcrlf & "              <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("use_startime")
			Response.write "</div></td>" & vbcrlf & "              <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("use_endtime")
			Response.write "</div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			if rs7("use_rettime")=#1900-01-01# then
			else
				Response.write rs7("use_rettime")
			end if
			Response.write "</div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("use_mdd")
			Response.write "</div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("use_thing")
			Response.write "</div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("drivername")
			Response.write "</div></td>" & vbcrlf & "              <td width=""12%"" height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_102_14=3 or CheckPurview(intro_102_14,trim(use_cateid))=True then
			...
			if ((open_102_2=3 or CheckPurview(intro_102_2,trim(use_cateid))=True)) and rs7("complete")="1" then
			end if
			if open_102_3=3 or CheckPurview(intro_102_3,trim(use_cateid))=True then
			end if
	...
	Response.write "" & vbcrlf & "      </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "  <tr><td height=""30"" colspan=""3"">" & vbcrlf & "      </form>" & vbcrlf & "       <form name=""oneself""action=""List_DcarUse.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&zdy6="
	Response.write zdy6
	Response.write """ method=""post"" id=""oneself"">" & vbcrlf & "         <div align=""right"">" & vbcrlf & "               "
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""200""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""200"">" & vbcrlf & "        <table width=""150"" height=""200"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按开始时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按开始时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按结束时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按结束时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=7')""><font color=""#2F496E"">按返还时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>"& vbcrlf &                "<tr  valign=""middle"">" & vbcrlf &             "<td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl</td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="申请明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & "  window.location=""List_DcarUse.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
