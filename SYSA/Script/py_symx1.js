...
function ask()
{
	...
	if(document.getElementById('allpay').value=="" || document.getElementById('allpay').value==0)
	{
		document.getElementById('allm').innerHTML="*必填"
		return false;
	}
	var mem=document.all('member2');
	membool=false;
	for(i=0;i<mem.length;i++)
	{
		if (mem[i].checked==true)
		{membool=true}
	}
	if(membool==false)
	{
	...
	document.date.action="savesq.asp?num="+arrTr+"&ord="+window.syord;
	return true;
}
var eindex=0;
var arrTr=""
...
tr.appendChild(td2);
td2.align="center";
td2.innerHTML="<input type='text' size='3' id='num_"+eindex+"' name='num_"+sid+"_"+eindex+"' onKeyUp=\" value=value.replace(/[^ \\d]/g,'')\" onpropertychange=\"document.getElementById('allnum').value=getSumNum();\"/>";
}
if(zdyarr[i]=="money1")
{
...
tr.appendChild(td3);
td3.align="center";
td3.innerHTML="<input type='text' size='8' id='money_"+eindex+"' name='money_"+sid+"_"+eindex+"' onKeyUp=\" value=value.replace(/[^\\d*|\\.]/g,'')\" onpropertychange=\"document.getElementById('allpay').value=getSumMoney();\" />";
}
if(zdyarr[i]=="intro")
{
...
tr.appendChild(td5);
td5.align="center";
td5.innerHTML="<input type='hidden' size='10' name='hdj_"+sid+"_"+eindex+"' id='hdj_"+sid+"_"+eindex+"'><span class=red id='dj_"+sid+"_"+eindex+"'></span><span class=\"red\" id='tjdj_"+sid+"_"+eindex+"'><a href='javascript:void(0);' onclick=\"javascript:window.open('resultsq.asp?tyid=dj_"+sid+"_"+eindex+"&ord="+window.syord+"&sort1="+sid+"','eventcom3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');\"><img class='resetElementHidden' style=\"border:0px; width:12px;height:11px;\" src=\"../images/jiantou7.gif\" alt=\"添加\"><img class='resetElementShow' style=\"border:0px; width:12;height:11;display:none;\" src=\"../skin/default/images/MoZihometop/content/lvw_addrow_btn.png\" alt=\"添加\">添加</a></span>&nbsp;|<input type=\"radio\" name=\"moban\" value=\"hdj_"+sid+"_"+eindex+"\" id=\"moban\" />&nbsp;复制&nbsp;|&nbsp;<a href='javascript:void(0);' onclick=\"plupdate('hdj_"+sid+"_"+eindex+"','"+sid+"');\">粘贴</a>";
arrTr=arrTr+','+eindex;
eindex=eindex+1;
}
...
{
	var url="insglsq.asp?ord="+ord+"&sort1="+sid+"&fid="+window.syord+"";
	xmlHttp.open("GET",url,false);
...
function updatepage1(k,sort1)
{
	...
	if(xmlHttp.readyState==4)
	{
		...
		document.getElementById(unameval).innerHTML="<a href=\"javascript:void(0)\" onMouseOut=\"hidelabel()\" onMouseOver=\"xmldata1("+ajaxhtml+")\" onclick=\"javascript:window.open('resultsq.asp?tyid="+tyid+"&valid="+ajaxhtml+"&ord="+window.syord+"&sort1="+window.sySort1+"','eventcom3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');\">修改</a>";
		updatePage3();
	}
}
function plupdate(tid,sid)
{
	...
	{return}
	xmldata2(document.getElementById(mobvalue).value,tid,sid);
}
