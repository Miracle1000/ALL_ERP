...
function showTreeDataReport(box,e){
e=e||window.event;
box.className=this.className.indexOf("_e1")>0?box.className.replace("_e1","_e0"):box.className.replace("_e0","_e1");Report.RefreshTableSizeUI();
 try{
                    e.stopPropagation()
                }catch(e){
                    e.cancelBubble=true;  
...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        setGrid: function (grid)
        {
            ...
            {
                g.grid.bind('SelectRow', function (rowdata,event)
                {
                    selecteds = [rowdata];
                    onGridSelect();
                    g._toggleSelectBox(true);
...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        errorMessage: '发生错误',
        pageStatMessage: '共{total}条   {pagesize}条/页  &nbsp; ',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                <div class='l-bar-group  l-bar-message' style='padding-top: 4px;;'><span class='l-bar-text'></span></div>");

            gridhtmlarr.push("            <div class='l-bar-group l-bar-selectpagesize'></div>");
        //    gridhtmlarr.push("                <div class='l-bar-separator'></div>");
         //   gridhtmlarr.push("                <div class='l-bar-separator'></div>");
         gridhtmlarr.push("                <div class='l-bar-group pagesize'><div class='pcontrol' style='display: inline-block;height:20px;line-height:20px;'><input type='text' uitype='intbox' id='reportrPageNum' size='4' value='1' style='width:31px;text-align:center;height:14px;line-height:14px;float:left;' maxlength='8' /><p style='display: inline-block;margin:0;padding:0 2px;float:left;height:20px;line-height:20px;'>/</p><div style='display: inline-block;float:left;height:20px;line-height:20px;'></div></div></div>");
         //   gridhtmlarr.push("                <div class='l-bar-separator'></div>");
         gridhtmlarr.push("                <div class='l-bar-group'>");
         gridhtmlarr.push("              <div style='padding-top: 4px;float:left'><input id='reportPageJump' type='button' value='跳转' style='margin:0 2px;' class='zb-button'/></div>");

         gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnfirst'><span><input type='button' value='首页' style='margin:0 2px;' class='zb-button'/></span></div>");
         gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprev'><span><input type='button' value='上页' style='margin:0 2px;' class='zb-button'/></span></div>");
         gridhtmlarr.push("                </div>");

         gridhtmlarr.push("                <div class='l-bar-group'>");
         gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnext'><span><input type='button' value='下页' style='margin:0 2px;'/ class='zb-button'></span></div>");
         gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlast'><span><input type='button' value='尾页' style=' margin:0 2px;'/ class='zb-button'></span></div>");
         gridhtmlarr.push("                </div>");
        ...
        _setToolbar: function (value)
        {
            ...
            if (value && $.fn.ligerToolBar)
            {
                ...
                if (value.title)
                {
                    ...
                    if (value.icon)
                    {
                        jtitle.append("< class='l-panel-topbaricon' src='" + value.icon + "'></>");
        ...
        _updateFrozenWidth: function ()
        {
           ...
           //  if (view2width > 0) g.gridview2.css({ width: view2width-100 });
       }
   },
   _setWidth: function (value)
   {
        ...
        */
        ...
        changePage: function (ctype)
        {
            ...
            if (p.dataAction != "local" && g.isDataChanged && !confirm(p.isContinueByDataChanged))
                return false;
            p.pageCount = parseInt($(".pcontrol div", g.toolbar).html());
            function newNum(a) {
               // var tds=$(".l-grid-row-cell-rownumbers");
               // for(var i=0;i<tds.length;i++){
               //  var nowNum=parseInt($(tds[i]).find("div").html());
               //  var newNum=nowNum*a+nowNum;
               //  setTimeout(function (){ $(".l-grid-row-cell-rownumbers").find("div").html(newNum)},100)
            ...
            switch (ctype)
            {
                case 'first': if (p.page == 1) return; p.newPage = 1; newNum(p.newPage);break;
                case 'prev': if (p.page == 1) return; if (p.page > 1) p.newPage = parseInt(p.page) - 1;  newNum(p.newPage);break;
                case 'next': if (p.page >= p.pageCount) return; p.newPage = parseInt(p.page) + 1;   newNum(p.newPage);break;
                case 'last': if (p.page >= p.pageCount) return; p.newPage = p.pageCount;  newNum(p.newPage);break;
                case 'input':
        ...
        demotion: function (rowParm)
        {
            ...
            var rowobjs = [g.getRowObj(rowdata)];
            if (g.enabledFrozen()) rowobjs.push(g.getRowObj(rowdata, true));
            $("> td > div > .l-grid-tree-space:last", rowobjs).removeClass("l-grid-tree-link l-grid-tree-link-open l-grid-tree-link-close none");
        ...
        _showData: function (sourceType)
        {
            ...
            if (p.mouseoverRowCssClass)
            {
                for (var i in g.rows)
                {
                    ...
                    if (g.enabledFrozen())
                        rowobj = rowobj.add(g.getRowObj(g.rows[i], true));
                    rowobj.bind('mouseover.gridrow', function (event)
                    {
                        g._onRowOver(this, true,event);
                          event=event||window.event;
    try{   event.stopPropagation();}catch(e){event.cancelBubble=ture}
                    }).bind('mouseout.gridrow', function (event)
                    {

                        g._onRowOver(this, false,event);
                          event=event||window.event;
    try{   event.stopPropagation();}catch(e){event.cancelBubble=ture}
                    });
                }
        ...
        _getHtmlFromData: function (data, frozen)
        {
            ...
            for (var i = 0, l = data.length; i < l; i++)
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.isrownumber)
                    {//改序号的显示
                        var pageNum=parseInt($("#reportrPageNum").val())-1;
                        var pageSize=/(\d+)条\/页/.exec($(".l-bar-text").html())[1];
                        var newNum=pageSize*pageNum+(parseInt(item['__index'])) + 1
                        ...
                        gridhtmlarr.push('>' + newNum + '</div></td>');
                        return;
                    }
                    //如果是复选框(系统列)
                    if (this.ischeckbox)
                    {
                        ...
                        {
                            gridhtmlarr.push(' class="l-grid-row-cell l-grid-row-cell-detail"><div class="l-grid-row-cell-inner"');
                            if (p.fixedCellHeight)
                                gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" ');
                            else
                                gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
            ...
            html.push("<div class='zt_nd' >");
            if( nd.deepData){
                for (var i = 0; i < nd.deepData.length; i++) {
                    if(nd.deepData.substr(i,1)=="1") {
                        html.push("<div class='l-grid-tree-space line'></div>");
                    }
                    else {
                        html.push("<div class='l-grid-tree-space none'></div>");
                    }
                }
            }
            var existsnext = nd.parent.children.length > (nd.i + 1);
            var expcss = "zty_" + (existsnext ? "2" : "1") + "_e" + ((nd.children||[]).length == 0 ? "2" : isExtend?1: 0);
            html.push("<div class='" + expcss + " l-grid-tree-space l-grid-tree-link'  style='height:27px' isexpnode=1>&nbsp;</div>");
            // if (isExtend && isParent){
             //   html.push("<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>");
            //}
            var stt=nd.childrencount==0?" style='text-decoration:none;cursor:default ;'":""
            html.push("<div class='zt_nd_t' style='padding-top:2px' ><a "+stt+" href='javascript:void(0)' isnt='1'>" + oldContent + "</a></div>");
            html.push("</div>");
            return html.join("");
    ...
    if (column.editor.type && p.editors[column.editor.type])
    {
        ...
        _getTotalSummaryHtml: function (data, classCssName, frozen)
        {
            ...
            $(g.columns).each(function (columnindex, column)
            {
                ...
                if (this.isrownumber)
                {
                    totalsummaryArr.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-rownumbers"><div>&nbsp;</div></td>');
                    return;
                }
                //如果是复选框(系统列)
                if (this.ischeckbox)
                {
                    totalsummaryArr.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-checkbox"><div>&nbsp;</div></td>');
                    return;
                }
                    //如果是明细列(系统列)
                    else if (this.isdetail)
                    {
                        totalsummaryArr.push('<td class="l-grid-totalsummary-cell l-grid-totalsummary-cell-detail"><div>&nbsp;</div></td>');
                        return;
                    }
        ...
        _buildPager: function ()
        {
            ...
            if (!p.pageCount) p.pageCount = 1;
            $('.pcontrol div', g.toolbar).html(p.pageCount);
        ...
        _setEvent: function ()
        {
           ...
           if(nav.indexOf("MSIE")>0){
             ...
             //点击翻页按钮时跳两次，经查属重复绑定事件，遂注释以下代码
            // $(".l-bar-btnprev").click(function (e) {

            //     g._onClick(e)
            // })
            // $(".l-bar-btnnext").click(function (e) {

            //     g._onClick(e)
            // })
            // $(".l-bar-btnfirst").click(function (e) {

            //     g._onClick(e)
            // })
            // $(".l-bar-btnlast").click(function (e) {
            ...
            //工具条 - 切换当前页事件
            $('div.pcontrol :text', g.toolbar).blur(function (e)
            {
                g.changePage('input');
            });
...
_onRowOver: function (rowParm, over,e)
{
    ...
    $(g.getRowObj(rowdata, false))[methodName](p.mouseoverRowCssClass);
      e=e||window.event;
    try{   e.stopPropagation();}catch(e){e.cancelBubble=ture}
...
_onMouseUp: function (e)
{
    ...
    if (l.draggable.dragging)
    {
        ...
        _onClick: function (e)
        {
           var reportTreeHasChildren=""
            var obj = (e.target || e.srcElement);
            var g = this, p = this.options;
            var src = g._getSrcElementByEvent(e);
             var a=$(src.cell).find("a");
             if(a.length>0){
                if(a[0].style.cursor!=="default"){
                   reportTreeHasChildren=true;
                }
             }
            if (src.out)
            {
                ...
                for (var rowid in g.records)
                {
                    if (uncheck){
                      g.unselect(g.records[rowid]);  
                        ...
                        $(".l-grid-body-table .l-grid-row").addClass('l-selected')
                    }
                }
            ...
            else if (src.hcelltext) //排序
            {
                ...
                //树 - 伸展/收缩节点
                 else if (src.treelink||reportTreeHasChildren)
                {
                 var treeClickDiv=$(src.cell).find("div[class*=zty]")
                 if(treeClickDiv.length>0){
                     if(treeClickDiv[0].className.indexOf("e1")>-1){
                        treeClickDiv[0].className=treeClickDiv[0].className.replace("e1","e0");
                    }else{
                     treeClickDiv[0].className= treeClickDiv[0].className.replace("e0","e1")   
                 }
             }
                    g.toggle(src.data);
                    Report.RefreshTableSizeUI()
                }
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if ((src.checkbox || !selectRowButtonOnly) && p.selectable != false)
                {
                   ...
                   if($(".l-grid-row-cell-checkbox").length==$(".l-grid-body1 .l-selected").length){
                     headChek.addClass("l-checked")
                   }
                  if(uncheck){
                    headChek.removeClass("l-checked");//非全选情况下，取消顶部复选框的选中
                    $(src.row).removeClass('l-grid-row-over')
                  }
                }
        ...
        select: function (rowParm)
        {
            ...
            g.trigger('selectRow', [rowdata, rowid, rowobj,event]);
        },
    ...
    $.ligerui.controls.MenuBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addItem: function (item)
        {
            ...
            if (item.menu)
            {
                ...
                //     g.actionMenu = menu;
                //     $(this).addClass(" l-panel-btn-selected").siblings(".l-menubar-item").removeClass("l-panel-btn-selected");
                // }, function ()
                // {
    ...
    $.ligerui.controls.Resizable.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.target.mousemove(function (e)
            {
                ...
                g.dir = g._getDir(e);
                if (g.dir){
                    g.target.css('cursor', 'col-resize');
                }
