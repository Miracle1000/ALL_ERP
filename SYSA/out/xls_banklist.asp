	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				xlApplication.ActiveSheet.Columns(8).ColumnWidth=25
...
'if B<>"0" then
				...
				if E<>"" then
					Str_Result=Str_Result+" and bank in("&E&")"
'if E<>"" then
				end if
				if F<>"" then
...
'function getTotalByBankId(bankId,date1)
	...
	Str_Result=Str_Result&" and bank in (select id  from sortbank where del=1 and (person like '"&session("personzbintel2007")&",%' or person like '%,"&session("personzbintel2007")&",%' or person like '%, "&session("personzbintel2007")&",%'  or person like '%,"&session("personzbintel2007")&"' or person like '%, "&session("personzbintel2007")&"'  or person like '"&session("personzbintel2007")&"' or person like '0') )"
	...
	conn.execute(strs)
	sql="select inx, bank,money1,money2,sort,intro,gl,cateid,date1,date7 ,gl2 from  #BillList "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "银行账户明细_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0  then
		...
		do until rs.eof
			...
			blanance=getTotalByBankId(rs("bank"),m1)
			if px=1 then
				pxsal="select a.inx , "&_
				""&blanance&"+isnull(sum(round(isnull(b.money1,0),"&num_dot_xs&"))-sum(round(isnull(b.money2,0),"&num_dot_xs&")),0) blanance "&_
				"pxsal=""select a.inx , """&_
				"into #data from #BillList a" &_
				" left join #BillList b on b.inx>=a.inx and b.bank="&rs("bank")&" "&_
				" where a.bank="&rs("bank")&"" &_
				" group by a.inx  "
			else
				pxsal="select a.inx , "&_
				""&blanance&"+isnull(sum(round(isnull(b.money1,0),"&num_dot_xs&"))-sum(round(isnull(b.money2,0),"&num_dot_xs&")),0) blanance "&_
			...
			conn.execute(pxsal)
			sql7="select blanance from #data where  inx="&inx&""
			rs7.open sql7,conn,1,1
			if not rs7.eof then
				moneyall=rs7("blanance")
			end if
			rs7.close
			...
			elseif sort=10 then
				...
				rs7.close
			elseif sort=11 then
				sql7="select company from bankout2 where id="&gl&" and del=1"
				rs7.open sql7,conn,1,1
				if rs7.eof then
					gys=0
					bh=0
				...
				rs7.close
				If gys <> 0 Then
					rs7.open "select a.id,a.title,"&_
					...
					"where a.id="&gl&" and a.del=1",conn,1,1
					if rs7.eof then
						title="供应商已被删除"
						bh=0
						hasListPower = False
			...
			xlWorksheet.Cells(1+i,1).Value = dateadd1
			rs7.close
			xlWorksheet.Cells(1+i,1).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,2).Value = k2
			rs7.close
			xlWorksheet.Cells(1+i,2).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,3).Value = Formatnumber(numcg1,num_dot_xs,-1)
			rs7.close
			xlWorksheet.Cells(1+i,3).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,4).Value = Formatnumber(numcg2,num_dot_xs,-1)
			rs7.close
			xlWorksheet.Cells(1+i,4).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,5).Value = Formatnumber(moneyall,num_dot_xs,-1)
			rs7.close
			xlWorksheet.Cells(1+i,5).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,6).Value = intro
			rs7.close
			xlWorksheet.Cells(1+i,6).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,7).Value = cateName
			rs7.close
			xlWorksheet.Cells(1+i,7).font.Size=10
			rs7.close
			xlWorksheet.Cells(1+i,8).NumberFormatLocal = "@"
			rs7.close
			If hasListPower = False Then title=""
			xlWorksheet.Cells(1+i,8).Value = title
			If hasListPower = False Then title=""
			xlWorksheet.Cells(1+i,8).font.Size=10
			If hasListPower = False Then title=""
			Call ClientClosedExit
			Response.Flush
			sumnumcg1=sumnumcg1+numcg1
			Response.Flush
			sumnumcg2=sumnumcg2+numcg2
			Response.Flush
			i=i+1
			Response.Flush
			rs.movenext
...
'Loop
		xlWorksheet.Cells(1+i,1).colspan = 2
'Loop
		xlWorksheet.Cells(1+i,2).Value = Formatnumber(sumnumcg1,num_dot_xs,-1)
'Loop
		xlWorksheet.Cells(1+i,2).font.Size=10
'Loop
		xlWorksheet.Cells(1+i,3).Value = Formatnumber(sumnumcg2,num_dot_xs,-1)
'Loop
		xlWorksheet.Cells(1+i,3).font.Size=10
'Loop
	end if
	...
	set rs=Nothing
	set rs7=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>银行账户明细导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!银行账户明细导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs7=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("银行账户明细_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的银行账户明细</u></strong></font></a></p> " & vbcrlf & ""
