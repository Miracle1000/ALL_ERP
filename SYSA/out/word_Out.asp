...
'If InStr(1,PicUrl,"about:",1) = 1 Then
				If InStr(1,PicUrl,".asp",1) > 0 or instr(1, picUrl, ".ashx", 1)>0 Then
					picUrl = CollapseUrl (PicUrl)
				else
					PicUrl = Replace(server.MapPath(PicUrl),"\","/")
				end if
			end if
			fkz = kzs(ubound(kzs))
			If InStr(1,fkz,"ashx",1) > 0 Then fkz = "jpg"
			...
			xhttp.open "GET", replace(PicUrl,"&amp;","&"), False
			on error resume next
			xhttp.send
			If Err.number <> 0 Then
				data(count) = "R0lGODlhPAANAIAAAO0cJP/yACH5BAAHAP8ALAAAAAA8AA0AAAJkhI+py+0dopzhVAuzuQ8D" & vbcrlf &_
				"unlhcnEWaHZV5HFm+lEwMnXf6eLyeCtlP1vUNhKRjybErGy9GJL3gEmbUZJI5sIWQ7/g0fmk" & vbcrlf &_
				"irmMF5PKGk2tUHLbod2ujWjluJyZZYXLsPcMGNhRAAA7AA=="
				fkz = "gif"
			else
		...
		private function CollapseUrl(byval srcurl)
			dim obj : set obj = server.CreateObject( ZBRLibDLLNameSN & ".HtmlExcelImages")
			obj.init request, server
			obj.SetRuntime ZBRuntime
			CollapseUrl = obj.CollapseUrl(srcurl)
			set obj =  nothing
		end function
...
'Dim i
	...
	Class HtmlWordApplication
		...
		Private Sub Class_Initialize()
			...
			SavePath = server.mappath("../out/HtmlWord")
		end sub
		...
		Public Sub Save(ByVal Path, ByVal body)
			Dim i, attrs, fl
			attrs =  Split("heihgt;colspan;isNumber;rowspan;align;href;style;class;height;width;size;border;bordercolor;cellspacing;cellpadding;bgcolor;src;lang;language;value;valign;face",";")
...
'For i = 0 To ubound(attrs)
			...
			fl.WriteLine ""
			fl.WriteLine "Content-Type: text/html; charset=""GB2312"""
			fl.WriteLine ""
			fl.WriteLine ""
			...
			fl.WriteLine ""
			fl.WriteLine "<head>"
			fl.WriteLine "<meta http-equiv=Content-Type content=""text/html; charset=GB2312"">"
			fl.WriteLine "<head>"
			fl.WriteLine "<meta name=ProgId content=Word.Document>"
			fl.WriteLine "<xml>"
			...
			fl.WriteLine "</xml>"
			fl.WriteLine "<xml>"
			fl.WriteLine "<w:WordDocument>"
			fl.WriteLine " <w:View>Print</w:View>"
			fl.WriteLine " <w:SpellingState>Clean</w:SpellingState>"
			fl.WriteLine " <w:GrammarState>Clean</w:GrammarState>"
			fl.WriteLine " </w:WordDocument>"
			fl.WriteLine " </xml>"
			fl.WriteLine "</head>"
			Err.clear
...
'If sc>0 Then v = v & ";"
	...
	If abs(Err.number) > 0 Then
		Response.write "<meta http-equiv='content-type' content='text/html;charset=UTF-8'><script>alert('导出Word失败，请确认IIS上传大小是否有200K限制。');</script>"
'If abs(Err.number) > 0 Then
		Response.end
	end if
	Dim xDoc
	Set xDoc = New HtmlWordApplication
	relFileName=Server.MapPath(title & "_" & session("name2006chen") & ".doc")
	set docObj=New htmlfile
	docObj.open()
		set body = docObj.createelement("div")
		body.innerHTML = Replace(content,"<SPAN isNumber=""1""></SPAN>","")
		Dim imgUrls
		ReDim imgUrls(0)
		...
		reg.IgnoreCase = True
		reg.Global = True
		reg.Pattern = "<span[^>]*></span>"
		xDoc.save relFileName , reg.replace(body.innerHtml,"")
		Set reg = Nothing
		docObj.close
			Set docObj = Nothing
