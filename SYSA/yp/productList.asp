...
'if strW3<>"" Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/yp_productList.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & ""
	Ptr_Result=""
	Str_Result=""
	if open_101_15=3 then
		list=""
	elseif open_101_15=1 then
		list=" and prod_addcateid in ("&intro_101_15&")"
	else
		list=" and prod_addcateid=0"
	end if
	Str_Result=" where prod_del=1 "&list&""
	...
	if px="1" then
		Ptr_Result=" order by prod_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by prod_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by prod_name desc"
	elseif px="4" then
		Ptr_Result=" order by prod_name asc"
	elseif px="5" then
		Ptr_Result=" order by prod_bh desc"
	elseif px="6" then
		Ptr_Result=" order by prod_bh asc"
	end if
	ret          =request("ret")
	ret2 =request("ret2")
	ret3         =request("ret3")
	...
	W2=tmparr(1)
	if zdy1<>"" then
		Str_Result=Str_Result+" and zdy1 like '%"&zdy1&"%' "
'if zdy1<>"" then
	end if
	if zdy2<>"" then
		Str_Result=Str_Result+" and zdy2 like '%"&zdy2&"%' "
'if zdy2<>"" then
	end if
	if zdy3<>"" then
		Str_Result=Str_Result+" and zdy3 like '%"&zdy3&"%' "
'if zdy3<>"" then
	end if
	if zdy4<>"" then
		Str_Result=Str_Result+" and zdy4 like '%"&zdy4&"%' "
'if zdy4<>"" then
	end if
	if zdy5<>"" then
...
'if zdy6<>"" then
	...
	if D<>"" then
		Str_Result=Str_Result&" and prod_type in("&D&") "
	end if
	if E<>"" then
		Str_Result=Str_Result&" and pro_type in("&E&") "
	end if
	if F<>"" then
		Str_Result=Str_Result&" and prod_name like '%"&F&"%' "
	end if
	if G<>"" then
		Str_Result=Str_Result&" and prod_bh like '%"&G&"%' "
	end if
	if H<>"" then
		Str_Result=Str_Result&" and prod_xh like '%"&H&"%' "
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and prod_less >="&L1&""
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and prod_less <="&L2&""
'if L2<>"" then
	end if
	if L3<>"" then
		Str_Result=Str_Result+" and prod_more >="&L3&""
'if L3<>"" then
	end if
	if L4<>"" then
		Str_Result=Str_Result+" and prod_more <="&L4&""
'if L4<>"" then
	end if
	if ret3 <>"" and not isnull(ret3) then
		Str_Result=Str_Result+" and  datediff(d,'"&ret3&"',prod_addtime)>=0 "
'if ret3 <>"" and not isnull(ret3) then
	end if
	if ret4 <>"" and not isnull(ret4) then
		Str_Result=Str_Result+" and  datediff(d,prod_addtime,'"&ret4&"')>=0 "
'if ret4 <>"" and not isnull(ret4) then
	end if
	if M <>"" and not isnull(M) then
		Str_Result=Str_Result&" and a.prod_note like '%"&M&"%' "
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and prod_addcateid in ("&W3&") "
	end if
	if ret<>"" then
		Str_Result=Str_Result&" and datediff(dd,'"& ret &"',prod_addtime)>=0"
	end if
	if ret2<>"" then
		Str_Result=Str_Result&" and datediff(dd,prod_addtime,'"& ret2 &"')>=0"
	end if
	if A<>"" then
		Str_Result=Str_Result&" and prod_type ="&A&""
	end if
	if C<>"" then
		if B="ypmc" then
			Str_Result=Str_Result&" and prod_name like '%"&C&"%'"
		elseif B="ypbh" then
			Str_Result=Str_Result&" and prod_bh  like '%"&C&"%'"
		elseif B="ypxh" then
			Str_Result=Str_Result&" and prod_xh  like '%"&C&"%'"
		elseif B="zdkc" then
			Str_Result=Str_Result&" and prod_less <= "&cdbl(C)&" "
		elseif B="zgkc" then
			Str_Result=Str_Result&" and prod_more >= "&cdbl(C)&" "
		end if
	end if''
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""productList.asp"" method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "            <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">档案列表</td>" & vbcrlf & "                 <td align=""left""><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""1""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>           " & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">        " & vbcrlf & "                <tr>" & vbcrlf & "              <td colspan=""8"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding-top:0!important;padding-bottom:0!important""><span id=""kh"">自：<input id=""daysOfMonthPos"" value="""
	'currpage=clng(currpage)
	Response.write ret
	Response.write """ onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" style=""width:80px;"" name=""ret"" /><DIV id=daysOfMonth style=""POSITION: absolute""></DIV> 至：<input value="""
	Response.write ret2
	Response.write """ id=""daysOfMonth2Pos"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9"" name=""ret2"" /><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;<input name=""page_count"" type=""hidden""  style=""width:80px;""value="""
	Response.write page_count
	Response.write """><input name=""px"" type=""hidden"" value="""
	Response.write px
	Response.write """><select name=""A"" id=""A"" style=""color:#000000"">" & vbcrlf & "                        <option value="""">--请选择--</option>" & vbcrlf & "                      "
	'Response.write px
	sql8="select * from O_officeType order by pro_gate2 desc"
	...
	rs8.close
	set rs8=nothing
	Response.write "" & vbcrlf & "                     </select>" & vbcrlf & "                       <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                                <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""ypmc"" "
	'set rs8=nothing
	if B="ypmc" then
		Response.write "selected"
	end if
	Response.write ">用品名称</option>" & vbcrlf & "                           <option value=""ypbh"" "
	if B="ypbh" then
		Response.write "selected"
	end if
	Response.write ">用品编号</option>" & vbcrlf & "                           <option value=""ypxh"" "
	if B="ypxh" then
		Response.write "selected"
	end if
	Response.write ">用品型号</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input type=""text"" name=""C"" id=""C"" size=""10"" value="""
	Response.write c
	Response.write """>" & vbcrlf & "                        <input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索"">" & vbcrlf & "                   "
	if open_101_9=1 then
		Response.write "" & vbcrlf & "                     <input name=""submit1"" type=""button""  class=""anybutton2"" value=""导入"" onClick=""window.location.href='../load/UpLoad_product.asp';""/>" & vbcrlf & "                       "
	end if
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_product.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
	...
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" style=""position:relative;*top:-3px"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>" & vbcrlf & "                      </span></td>" & vbcrlf & "              </tr>" & vbcrlf & "          <tr  valign=""center"" class=""top"">" & vbcrlf & "             <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "             <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">用品名称</div></td>" & vbcrlf & "                <td width=""13%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">用品编号</div></td>" & vbcrlf & "                <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">用品型号</div></td>" & vbcrlf & "             <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">用品分类</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">最低库存</div></td>" & vbcrlf & "               <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">最高库存</div></td>" & vbcrlf & "                <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                       <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.pro_type,b.pro_name from O_product a left join O_officeType b on a.prod_type=b.id  "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			delbool=true
			sqltext="SELECT * FROM O_kuinList where prod_id="&rs7("id")
			...
			rstxt.close
			set rstxt=nothing
			if (open_101_3=3 or CheckPurview(intro_101_3,trim(addcateid))=True) and delbool then
				Response.write "" & vbcrlf & "                      <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td width=""15%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "                   "
			...
			Response.write "</td>" & vbcrlf & "                   <td width=""13%"" align=""left"" valign=""middle""  class=""func"">"
			Response.write rs7("prod_bh")
			Response.write "</td>" & vbcrlf & "                   <td width=""14%"" align=""left"" valign=""middle""  class=""func"">"
			Response.write rs7("prod_xh")
			Response.write "</td>" & vbcrlf & "                   <td width=""14%"" align=""left"" valign=""middle""  class=""func"">" & vbcrlf & "             "
			prod_type=rs7("prod_type")
			if prod_type="" or isnull(prod_type) then prod_type=0
			sql8="select  * from O_officeType where id="&prod_type
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				pro_name=rs8("pro_name")
				if rs8("pro_type")=1 then
					pro_type1="易耗品"
				elseif rs8("pro_type")=2 then
					pro_type1="非易耗品"
				end if
			end if
			rs8.close
			set rs8=nothing
			Response.write pro_name&"&nbsp;("&pro_type1&")"
			prod_less=rs7("prod_less")
			prod_more=rs7("prod_more")
			if prod_less="" or isnull(prod_less) then
				prod_less=0
			end if
			if prod_more="" or isnull(prod_more) then
				prod_more=0
			end if
			prod_less=formatnumber(prod_less,num1_dot,-1)
			prod_more=0
			prod_more=formatnumber(prod_more,num1_dot,-1)
			prod_more=0
			Response.write "</td>" & vbcrlf & "                   <td width=""10%"" align=""center"" valign=""middle""  class=""func"">"
			Response.write prod_less
			Response.write "</td>" & vbcrlf & "                   <td width=""10%"" align=""center"" valign=""middle""  class=""func"">"
			Response.write prod_more
			Response.write "</td>" & vbcrlf & "                   <td width=""17%"" height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if rs7("prod_del")=22 then
				Response.write "" & vbcrlf & "                <input name=""button31"" type=""button""  class=""page"" id=""submit3"" onClick=""set_del('1');window.location.reload();"" value=""启用""/>" & vbcrlf & "             "
			elseif rs7("prod_del")=12 then
				Response.write "" & vbcrlf & "                <input name=""button32"" type=""button""  class=""page"" id=""submit3"" onClick=""set_del('2');window.location.reload();"" value=""暂停""/>" & vbcrlf & "             "
			end if
			if open_101_16=3 or CheckPurview(intro_101_16,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('productDetail.asp?ord="
				Response.write rs7("id")
			...
			if open_101_2=3 or CheckPurview(intro_101_2,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('productUpdate.asp?ord="
				Response.write rs7("id")
			...
			if (open_101_3=3 or CheckPurview(intro_101_3,trim(addcateid))=True) and delbool then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('productdel.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write px
	...
	Response.write "" & vbcrlf & "              </div>" & vbcrlf & "          </td></form>" & vbcrlf & "      </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "   <tr>" & vbcrlf & "     <td height=""2000"">" & vbcrlf & "        </td></tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按用品名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按用品名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按用品编号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl""</tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="用品档案列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " document.getElementById('data').action=""productList.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
	...
	Response.write "&ret2="
	Response.write ret2
	Response.write """;" & vbcrlf & " document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "      document.getElementById('data').action=""productdel.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
