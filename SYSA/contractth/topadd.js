...
var ht = 0
var searchHtType = 0 //0 快速检索 1 高级检索
...
function SearchHTKey(ajCode){
	if(ajCode){	
		...
		var r = ajax.send();	
		if(r!=""){
			$("#htTreeHtml")[0].innerHTML=r;
		}
	}	
}

function checkMxFormSubmit(){
	var ret = true;	
	$("input[name^='xlh_'][min=1]").each(function(){
		var mxid = $(this).attr("name").replace("xlh_","");
		var xlh = $.trim($(this).val());
		var inx = 0;
		if(xlh!=""){
			var num1 = Number($("#num1_"+mxid).val().replace(/,/g,""));
			if(num1!=1){
				ret = false;
				inx = $("#mlistvw_ckv_"+mxid).parent().next().html();
				app.Alert("第"+ inx +"行的序列号个数与数量不一致，请重新录入");
				return false;
			}
		}
	});
