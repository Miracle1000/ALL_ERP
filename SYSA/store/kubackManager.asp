...
'if ZBRuntime.MC(35000) = False  And ZBRuntime.MC(18100)=false Then
	...
	Sub MessagePost(msg)
		...
		If app.getint("mtype") = 0 Then
			cn.execute "if not exists(select 1 from ku_back where datediff(d,backUpDate,getdate()) = 0)" & vbcrlf & _
			"begin " & vbcrlf & _
			"    insert into ku_back(ku_id, ord,num1,ck,price1,money1,unit,ph,xlh,datesc,dateyx," &_
			"   bz,js,intro,caigoulist,gys,kuinlist,num2,daterk,Status,num3,date2,caigoulist2,backUpDate,b_cateid, b_ip, b_model) " & vbcrlf & _
			"   select id, ord,num1,ck,price1,money1,unit,ph,xlh,datesc,dateyx,bz,js,intro," &_
			"   caigoulist,gys,kuinlist,num2,daterk,Status,num3,date2,caigoulist2,getdate()," & Info.user & ", '" & app.GetIP() & "', 0  from ku where num2 <> 0 " & vbcrlf & _
			"end", n
			If n > 0 Then
				Response.write "app.Alert('备份完成。');"
			else
				Response.write "app.Alert('您今天已经备份！');"
			end if
		else
			If Len(app.gettext("dat")) = 0 Then
				Response.write "app.Alert('请选择要备份的日期。');"
				Exit sub
			end if
			If cn.execute("select 1 from ku_back where datediff(d,backUpDate,'" & app.getdate("dat") & "')=0").eof = false  Then
				Response.write "app.Alert('【" & app.getdate("dat") & "】已经存在备份记录。');"
				Exit sub
			end if
			cn.execute "exec erp_ku_CBackKuByCurrKu '" & app.getText("dat") & "'," & Info.User & ", '" & app.GetIP & "'", n
			Response.write "app.Alert('执行完成, 共备份" & n & "条记录。');"
		end if
		End select
	end sub
	Sub page_load
		Response.write app.defTopBarHTMl("../","","库存备份管理","")
		Response.write "<body>"
		Response.write "" & vbcrlf & "      <script type=""text/javascript"">" & vbcrlf & "   function cm(m) {" & vbcrlf & "                $ID(""drmodel"").style.display = m==0 ? ""none"" : """"; " & vbcrlf & "   }" & vbcrlf & "       function myconfirm() {" & vbcrlf & "          var msg = ""确定要生成备份记录吗？""" & vbcrlf & "                if($ID(""mx2"").checked) {" & vbcrlf & "                    msg = msg + ""\n\n注意：指定模式下，系统会清除指定时间与当前时间之间的备份数据。""" & vbcrlf & "          }" & vbcrlf & "               return window.confirm(msg);" & vbcrlf & "     }" & vbcrlf & "       </script>" & vbcrlf & "<div>" & vbcrlf & "<style>#comm_itembarbg{height:42px!important;}</style>" & vbcrlf & "<table width=""100%"" bgcolor='#c0ccdd' cellspacing=1 style='border-collapse:separate;'>" & vbcrlf & "<tr><td bgcolor='white' height='40px' width='30%' align='right'>备份信息：</td><td  bgcolor='white' style='color:red'>&nbsp;最近一次备份日期为" & vbcrlf & "    "
		Response.write "<body>"
		Dim rs : Set rs = cn.execute("select convert(varchar(10),max(backUpDate),120) from ku_back")
		If rs.eof = False Then
			Response.write rs(0).value
		end if
		rs.close
		If app.power.existsPower(23,22) Then
			Response.write " &nbsp;&nbsp;<a href='kubacklist.asp'>点击查看详情</a>" & vbcrlf & "        "
		end if
		Response.write "" & vbcrlf & "</td></tr>" & vbcrlf & "<tr><td bgcolor='white' height='40px' width='30%' align='right'>备份模式：</td><td  bgcolor='white'><input onclick='cm(0)' type='radio' id='mx1' name='mt' checked><label for='mx1'>即时</label> &nbsp; <input onclick='cm(1)' type='radio' id='mx2' name='mt'><label for='mx2'>指定</label></td></tr>" & vbcrlf & "<tr id='drmodel' style='display:none' height='30px'>" & vbcrlf & "  <td  bgcolor='white'  align='right'>备份点日期：</td>" & vbcrlf & "   <td  bgcolor='white'>&nbsp;<input type='text' id='dat' onclick='datedlg.show()'></td>" & vbcrlf & "</tr>" & vbcrlf & "<tr height='40px'>" & vbcrlf & "     <td colspan=2  bgcolor='white' style='border-bottom:0;'><div style='float:left;width:30%'>&nbsp;</div><button class='oldbutton' onclick='if(myconfirm()){ajax.regEvent(""dodata"");ajax.addParam(""mtype"",($ID(""mx1"").checked ? 0:1));ajax.addParam(""dat"",$ID(""dat"").value);ajax.exec();}'>备份</button></td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<div class='bottomdiv'></div>" & vbcrlf & "      "
