...
function showColorPanel(obj,txtobj){
    $('#'+obj).click(function(){
		...
		var twid  = $(this).outerWidth();	//控件本身的宽
        var tleft = $(this).offset().left;    //控件的定位点宽
        
		var wWidth = $(window).width();
		var cWidth = $("#colorpanel").width();
		var cHeight = $("#colorpanel").height();

		var l,t;
		if (wWidth > tleft + cWidth + 20)
		{
			l = tleft;
		}
		else
		{
			l = (tleft-cWidth+twid >= 0) ? tleft-cWidth+twid : 0;
		}
		if (dHeight > ttop+thei+5+cHeight)
		{
			t = ttop+thei+5
		}
		else
		...
		var cPosition =  {top:t,left:l};

        $("#colorpanel").css(cPosition);      
