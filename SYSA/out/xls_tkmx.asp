...
'if W4<>"" Then
			...
			if A2<>"0" then
				Str_Result=Str_Result+" and contractth in (select ord from contractth where bz = "&A2&")"
'if A2<>"0" then
			end if
			if C<>""then
				if B="khmc" then
...
'if B="khmc" then
				elseif B="khid" then
					str_Result=str_Result+" and contractth in (select ord from contractth where company in (select ord from tel where khid like '%"& C &"%'))"
'elseif B="khid" then
				elseif B="thzt" then
					str_Result=str_Result+" and contractth in (select ord from contractth where title like '%"& C &"%')"
'elseif B="thzt" then
				elseif B="thid" then
					str_Result=str_Result+" and contractth in (select ord from contractth where thid like '%"& C &"%')"
'elseif B="thid" then
				elseif B="thry" then
					str_Result=str_Result+" and contractth in (select ord from contractth where cateid in (select ord from gate where name like '%"& C &"%'))"
'elseif B="thry" then
				end if
			end if
			if Request("companyname")<>"" then
				str_Result=str_Result+" and contractth in (select ord from contractth where company in (select ord from tel where name like '%"& Request("companyname") &"%'))"
'if Request("companyname")<>"" then
			end if
			if Request("companyid")<>"" then
				str_Result=str_Result+" and contractth in (select ord from contractth where company in (select ord from tel where khid like '%"& Request("companyid") &"%'))"
'if Request("companyid")<>"" then
			end if
			if Request("thdmc")<>"" then
				str_Result=str_Result+" and contractth in (select ord from contractth where title like '%"& Request("thdmc") &"%')"
'if Request("thdmc")<>"" then
			end if
			if Request("thdbh")<>"" then
				str_Result=str_Result+" and contractth in (select ord from contractth where thid like '%"& Request("thdbh") &"%')"
'if Request("thdbh")<>"" then
			end if
...
'if cint(request("bz"))>0 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,"&_
			"(select name from tel where ord in (select company from contractth where ord=payout2.contractth) and del=1) as name,"&_
			"(select isnull(a.money1,0) as money2 from payout2 a where a.ord=payout2.ord and a.complete=2) as money2,"&_
			"(select sort1 from sortonehy where gate2=33 and id=payout2.pay) as payname,"&_
			"(select name from gate where ord in (select cateid from contractth where ord=payout2.contractth)) as cateidname"&_
			" from payout2 "&Str_Result&" and date1>='"&m1&"' and date1<='"&m2&"' "&px_Result&""
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "退款明细表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0  then
				...
				do until rs.eof
					...
					if rs("contractth")<>"" then
						...
						sql7="select ord,name,isnull(cateid,0) as cateid, ISNULL(share,'-222') share ,sort3, del from tel where ord in (select company from contractth where ord="&rs("contractth")&") "
						set rs7=server.CreateObject("adodb.recordset")
					...
					if rs("contractth")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select sort1 from gate1 where ord in (select cateid3 from contractth where ord="&rs("contractth")&") "
						rs7.open sql7,conn,1,1
						if rs7.eof then
							cateid3name=""
						else
					...
					if rs("contractth")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select sort2 from gate2 where ord in (select cateid2 from contractth where ord="&rs("contractth")&") "
						rs7.open sql7,conn,1,1
						if rs7.eof then
							cateid2name=""
						else
					...
					if rs("contractth")<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select * from gate where ord in (select cateid from contractth where ord="&rs("contractth")&") "
						rs7.open sql7,conn,1,1
						if rs7.eof then
							cateidname=""
						else
					...
					if rs("contractth")<>"" then
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select ord,title,cateid from contractth where ord="&rs("contractth")&" and del=1"
						rs1.open sql1,conn,1,1
						if rs1.eof then
							th=0
							thname="关联退货单已被删除"
...
'if cateid_th="" or cateid_th=0 then
					...
					if rs("contractth")<>"" then
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select ord,title,cateid from contract where ord in (select contract from contractthlist where caigou="&rs("contractth")&") and del=1"
						rs1.open sql1,conn,1,1
						if rs1.eof then
							contract=0
							contractname=""
							...
							While rs1.eof = False
								contract=rs1("ord")
								contractname=contractname & rs1("title")
								cateid_ht=rs1("cateid")
								if cateid_ht="" or cateid_ht=0 Then cateid_ht=-1
								cateid_ht=rs1("cateid")
								rs1.movenext
								If rs1.eof = False Then contractname = contractname &  "  "
							wend
						end if
						rs1.close
						set rs1=nothing
					...
					date1=rs("date1")
					date2=rs("date2")
					xlWorksheet.Cells(1+i,1).Value = telname
					date2=rs("date2")
					...
					date2=rs("date2")
					xlWorksheet.Cells(1+i,5).Value = getbzflag(rs("contractth"),"contractthid")&Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
					date2=rs("date2")
					xlWorksheet.Cells(1+i,5).font.Size=10
					date2=rs("date2")
					xlWorksheet.Cells(1+i,6).Value = getbzflag(rs("contractth"),"contractthid")&Formatnumber(money2,num_dot_xs,-1)
					date2=rs("date2")
					...
					date2=rs("date2")
					xlWorksheet.Cells(1+i,11).Value = contractname
					date2=rs("date2")
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>退款明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!退款明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("退款明细表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的退款明细表</u></strong></font></a></p>"
