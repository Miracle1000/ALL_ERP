...
'dim sqlstr
	...
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_1=0
	else
		open_44_1=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=2"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_2=0
	else
		open_44_2=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_3=0
	else
	...
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=4"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_4=0
	else
		open_44_4=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_7=0
	else
		open_44_7=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=11 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_11_7=0
	else
	...
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_10=0
	else
		open_44_10=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=11 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_11_10=0
	else
	...
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=44 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_44_19=0
	else
		open_44_19=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=1101 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1101_19=0
	else
		open_1101_19=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_open from power where ord="&session("personzbintel2007")&" and sort1=1101 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1101_1=0
	else
		open_1101_1=rs1("qx_open")
	end if
	rs1.close
	sql1="select qx_intro from power where ord="&session("personzbintel2007")&" and sort1=1101 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		intro_1101_1=""
	else
		intro_1101_1=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_intro,qx_open from power where ord="&session("personzbintel2007")&" and sort1=1101 and sort2=14"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		intro_1101_14=""
		open_1101_14=0
	else
		intro_1101_14=rs1("qx_intro")
		open_1101_14=rs1("qx_open")
	end if
	rs1.close
	set rs1=nothing
	...
	if ZBRuntime.MC(19000) and ZBRuntime.MC(20000) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=10) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('直接出账','out10',10,1,80,"&session("personzbintel2007")&",8,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(19000) and ZBRuntime.MC(21000) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=7) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('直接入账','in7',7,1,80,"&session("personzbintel2007")&",1,44,1)"
		conn.execute sql
	end if
	if ZBRuntime.MC(19000) and ZBRuntime.MC(22000) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=6) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('账间转入','in6',6,1,80,"&session("personzbintel2007")&",2,44,1)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=5) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('账间转出','out5',5,1,80,"&session("personzbintel2007")&",9,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(23000) and ZBRuntime.MC(23001) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=1) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('合同回款','in1',1,1,80,"&session("personzbintel2007")&",4,44,1)"
		conn.execute sql
	end if
	if ZBRuntime.MC(23000) and ZBRuntime.MC(23002) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=9) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('客户预收款','in9',9,1,90,"&session("personzbintel2007")&",3,44,1)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=13) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('客户退预收款','out13',13,1,100,"&session("personzbintel2007")&",13,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(24000) and ZBRuntime.MC(24002) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=11) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('供应商预付款','out11',11,1,100,"&session("personzbintel2007")&",10,44,0)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=12) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('供应商退预付款','in12',12,1,100,"&session("personzbintel2007")&",6,44,1)"
		conn.execute sql
	end if
	if ZBRuntime.MC(25000) and ZBRuntime.MC(25001) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=8) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('销售退款','out8',8,1,80,"&session("personzbintel2007")&",12,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(25000) and ZBRuntime.MC(25002) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=14) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('采购退款','in14',14,1,80,"&session("personzbintel2007")&",5,44,1)"
		conn.execute sql
	end if
	if ZBRuntime.MC(26000) and ZBRuntime.MC(26001) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=3) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('工资支出','out3',3,1,80,"&session("personzbintel2007")&",16,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(27000) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=4) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('费用返还','in4',4,1,80,"&session("personzbintel2007")&",7,44,1)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=40) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('费用报销','out40',40,1,80,"&session("personzbintel2007")&",15,44,0)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=50) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('费用借款','out50',50,1,80,"&session("personzbintel2007")&",14,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(35000) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=15) "& vbCrLf &_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('委外付款','out15',15,1,80,"&session("personzbintel2007")&",12,44,0)"& vbCrLf &_
		"else if exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=15 and title='整单委外')"& vbCrLf&_
		"update zdymx set title='委外付款' where sort1=44 and kd2="&session("personzbintel2007")&" and sort=15 and title='整单委外'"
		conn.execute sql
	end if
	if ZBRuntime.MC(20001) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=301) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('承兑入账','in301',301,1,80,"&session("personzbintel2007")&",2,44,1)"
		conn.execute sql
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=302) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('承兑出账','out302',302,1,80,"&session("personzbintel2007")&",10,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(23001) or ZBRuntime.MC(24001) or ZBRuntime.MC(20001) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=303) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('手续费','out303',303,1,80,"&session("personzbintel2007")&",17,44,0)"
		conn.execute sql
	end if
	if ZBRuntime.MC(15000) or ZBRuntime.MC(18700) then
		sql="if not exists(select top 1 * from zdymx where sort1=44 and kd2="&session("personzbintel2007")&" and sort=201) "&_
		"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('供应商付款','out201',201,1,80,"&session("personzbintel2007")&",11,44,0)"
		conn.execute sql
	end if
	Sub doSave(title)
		url=Server.MapPath(title &"_"&session("name2006chen")&".xls")
		xlsApp.Save url
	...
	Class AdvanceSearchClass
		...
		Private Sub Class_Initialize()
			ReDim n1(0)   , n2(0), n3(0), n4(0)
			fieldCount = 0
			isCallBack = (Request("asModel")="1")
		end sub
		...
		Public Function  GetGatesOP()
			GetGatesOP =  Replace(request("as_gs_ty") & "" , " ", "")
		end function
		Public Function GetGates(ty)
			...
			if len(request.querystring("w1")  & request.querystring("w2") & request.querystring("w3")) > 0 then
				GetGates = getW_3(request.querystring("w1") & "|" & request.querystring("w2") & "|" & request.querystring("w3"),ty)
			else
				GetGates = ""
			end if
		end function
		private Function getW_3(ByVal Wlist, byval ty)
			Dim rs , r
			Wlist = Split(Replace(Wlist," ",""), "|")
			Set rs =  conn.execute("exec erp_comm_getW3 '" &  Wlist(0) & "','" &  Wlist(1) & "','" &  Wlist(2) &"'," & ty & "," & session("personzbintel2007"))
			while rs.eof = False
		...
		Public Function GetText(keyName)
			GetText = request.querystring("asft_" & keyname)
		end function
		Public Function GetSelect(keyName)
			GetSelect = request.querystring("asearch_f_" & keyname)
		end function
		Public Sub showButton
			Dim i, rs , getCheckBoxTxt,n,getDateItem,selected
			Dim groupdata, gitem, sgrops, lenurl, tmdata
			asing = request.querystring("asing")
			If asing <> "1" Then asing = 0
			asing  = int(asing)
			Response.write "" & vbcrlf & "              <div style='color:#cc0000;position:absolute;top:40px;right:20px;cursor:pointer;"
			If asing = 1 Then Response.write "display:none"
			Response.write "' id='searchitemsbutton' onclick='this.style.display=""none"";document.getElementById(""searchitemspanel"").style.display=""block"";document.getElementById(""as_ing"").value=1'><img src='../images/icon_title.gif'><U><font class=""advanSearch"">高级检索</font></U></div>" & vbcrlf & "             <input type='hidden' name='asing' id='as_ing' value='"
			Response.write asing
			Response.write "'>" & vbcrlf & "            <div id='searchitemspanel' style='"
			If asing = 0 Then Response.write "display:none"
			Response.write "'>" & vbcrlf & "                    <table border='1' bordercolor='#C0CCDD' style='width:100%;border-collapse:collapse;margin-bottom:-3px;'>" & vbcrlf & "                        <tr id=""adv_seach_power_button"">" & vbcrlf & "                          <td align='left' height='26px' colspan='2'>" & vbcrlf & "                             <div style='color:#cc0000;float:left;cursor:pointer' onclick='document.getElementById(""as_ing"").value=0;document.getElementById(""searchitemsbutton"").style.display=""block"";document.getElementById(""searchitemspanel"").style.display=""none""'><img class=""resetElementHidden"" src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><img class=""resetElementShowNoAlign"" src=""../skin/default/images/MoZihometop/leftNav/expand.png"" style=""display:none;"" width=""18"" height=""7"" border=""0""><U class=""advanSearch"">正常状态</U></div>" & vbcrlf & "                          </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   "
			For i = 0 To fieldCount - 1
				Response.write "" & vbcrlf & "                      <tr>" & vbcrlf & "                            "
				If InStr(n2(i),"gates")=1 And Len(n4(i)) > 0 then
					Response.write "<td align='right' style='height:30px;width:120px;' rowspan=2>" & n1(i) & "：</td>"
					Response.write "<td style='padding-top:4px;padding-bottom:4px'>"
					Response.write "<td align='right' style='height:30px;width:120px;' rowspan=2>" & n1(i) & "：</td>"
					groupdata = Split(n4(i),"|")
					sgrops = "," & GetGatesOP() & ","
					For ii = 0 To ubound(groupdata)
						gitem = Split(groupdata(ii) & "", "=")
						Response.write "<input type='checkbox' name='as_gs_ty' "
						If ii = 0 And IsCallBack=false Then
							Response.write "checked "
						else
							If InStr(sgrops, "," & gitem(0) & ",") > 0 Then
								Response.write "checked "
							end if
						end if
						Response.write "value=""" & gitem(0) & """>" & gitem(1) & "&nbsp;"
					next
					Response.write "</td></tr><tr>"
				else
					Response.write "<td align='right' style='height:30px;width:120px;'>" & n1(i) & "：</td>"
				end if
				Response.write "" & vbcrlf & "                              <td>" & vbcrlf & "                            "
				selected=""
				Select Case LCase(n2(i))
				Case "select"
				Response.write "&nbsp;<select id='asearch_f_" & n3(i) & "' advselect='true' name='asearch_f_" & n3(i) & "'>"
				Set rs =  conn.execute(n4(i))
				Response.write "<option value=''>不限</option>"
				While Not rs.eof
					If rs.fields(1).value&""=request.querystring("asearch_f_" & n3(i))&"" Then
						selected=" selected='selected' "
					else
						selected=""
					end if
					Response.write "<option value='" & rs.fields(1).value & "' "& selected &">" & rs.fields(0).value &"</option>"
					rs.movenext
				wend
...
'While Not rs.eof
				...
				Case "checkboxtxt"
				getCheckBoxTxt=n4(i)
				For n=0 To ubound(split(getCheckBoxTxt,","))
					Response.write "<input type='checkbox' advCheckBox='true' name='"&n3(i)&n&"' id='"&n3(i)&n&"' value='"&split(split(getCheckBoxTxt,",")(n),"@")(1)&"'><label for='"&n3(i)&n&"'> " & split(split(getCheckBoxTxt,",")(n),"@")(0) & "</label>"
				next
				Case "gates"
				Call doGateList(1,GetGates(1))
				Case "gates2"
				Call doGateList(2,GetGates(2))
				Case "gates3"
				Call doGateList(3,GetGates(3))
				Case "gates4"
				Call doGateList(4,GetGates(4))
				Case "text"
				Response.write "&nbsp;<input advtext='true' type=text name='" & "asft_" & n3(i) & "' id='" & "asft_" & n3(i) & "' value='" & request.querystring("asft_" & n3(i)) & "'>"
				Case "textdate"
				getDateItem=n4(i)
				Response.write "&nbsp;自<input style='width:86' onpropertychange=formatData(this,'money') advtext='true' readonly type=text name='" & split(n3(i),",")(0) & "' id='"  & split(n3(i),",")(0) & "' value='" & request.querystring(split(n3(i),",")(0)) & "'>"
				Response.write "<DIV id='" & split(n3(i),",")(0) & "_date_show' style='POSITION: absolute;z-index:10'></DIV>"
				Response.write "&nbsp;至<input style='width:86' onpropertychange=formatData(this,'money') advtext='true' readonly type=text name='" & split(n3(i),",")(1) & "' id='"  & split(n3(i),",")(1) & "' value='" & request.querystring(split(n3(i),",")(1)) & "'>"
				Response.write "<DIV id='" & split(n3(i),",")(1) & "_date_show' style='POSITION: absolute;z-index:10'></DIV>"
				Case "khqy"
				Call CreateKhqy
				End Select
				Response.write "" & vbcrlf & "                             </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   "
			next
			Response.write "" & vbcrlf & "                     <tr>" & vbcrlf & "                            <td style='height:30px;'></td>" & vbcrlf & "                          <td style='height:30px;'>" & vbcrlf & "                                       <input type='hidden' name='asModel' value=1>" & vbcrlf & "                                    &nbsp;<input class='page' value='检索' type='submit'>&nbsp;&nbsp;<input bt='reset' class='page' value='重填' type='reset'>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </div>" & vbcrlf & "  "
		end sub
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
		...
		Sub CreateKhqy
			Response.write "" & vbcrlf & "<div id=""khqy"">"
			execute sdk.vbs("../manager/search_area" &  Application("__saas__company")  & ".asp")
			Response.write "</div>" & vbcrlf & "<script language='javascript'>" & vbcrlf & " var data = ["
	...
	Response.write "" & vbcrlf & "<script src= ""../Script/ic_aSearch.js?ver="
	Response.write Application("sys.info.jsver")
...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	End Class
	Function getCashSort()
		Dim ret
		ret = "0"
		if ZBRuntime.MC(19000) and ZBRuntime.MC(21000) And open_11_19<>1 Then
			ret = ret &",7"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(22000) And open_11_19<>1 then
			ret = ret &",6"
		end if
		if ZBRuntime.MC(27000) And open_6_19<>1 then
			ret = ret &",4"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23001) And open_7_19<>1 then
			ret = ret &",1"
			ret = ret &",101"
		end if
		if ZBRuntime.MC(25000) and ZBRuntime.MC(25002) And open_76_19<>1 then
			ret = ret &",14"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23002) And open_7_19<>1 then
			ret = ret &",9"
		end if
		if ZBRuntime.MC(24000) and ZBRuntime.MC(24002) And open_8_19<>1 then
			ret = ret &",12"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(20000) And open_11_19<>1 then
			ret = ret &",10"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(22000) And open_11_19<>1 then
			ret = ret &",5"
		...
		if ZBRuntime.MC(15000) or ZBRuntime.MC(18700) And open_8_19<>1 then
			ret = ret &",201"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23002) And open_7_19<>1 then
			ret = ret &",13"
		...
		if ZBRuntime.MC(26000) and ZBRuntime.MC(26001) And open_10_19<>1 then
			ret = ret &",3"
		end if
		if ZBRuntime.MC(25000) and ZBRuntime.MC(25001) And open_9_19<>1 then
			ret = ret &",8"
		end if
		if ZBRuntime.MC(20001)  And open_1101_19<>1 then
			ret = ret &",301"
			ret = ret &",302"
		end if
		if ZBRuntime.MC(23001) or  ZBRuntime.MC(24001) or ZBRuntime.MC(20001) then
	...
	Function getCashSql()
		Dim sql
		sql = "select 0 id, 0 bank, 0 gl, 0 gl2, 0 sort, 0 money1, 0 money2, cast(null as datetime) date1, 0 cateid "
		if ZBRuntime.MC(19000) and ZBRuntime.MC(21000) And open_11_19<>1 Then
			sql = sql &" " & vbCrLf &_
			"   union all       /*直接入账*/ " & vbCrLf &_
			"   select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a  " & vbCrLf &_
			"   where del=1 and sort=7  " & vbCrLf &_
			"           and ("& open_11_1 &"=3 or ("& open_11_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_11_1 &",')>0))"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(22000) And open_11_19<>1 then
			sql = sql &" " & vbCrLf &_
			"   union all       /*账间转入*/ " & vbCrLf &_
			"   select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a  " & vbCrLf &_
			"   where del=1 and sort=6  " & vbCrLf &_
			"           and ("& open_11_1 &"=3 or ("& open_11_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_11_1 &",')>0))"
		end if
		if ZBRuntime.MC(27000) And open_6_19<>1 then
			sql = sql &" " & vbCrLf &_
			"   union all       /*费用返还*/ " & vbCrLf &_
			"   select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"   inner join pay b on a.gl=b.ord and a.del=1 and a.sort=4  " & vbCrLf &_
			"           and ("& open_6_1 &"=3 or ("& open_6_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_6_1 &",')>0))"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23001) And open_7_19<>1 then
			sql = sql &" " & vbCrLf &_
			"   union all       /*合同回款*/ " & vbCrLf &_
			"   select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"   inner join payback b on a.gl2=b.ord and a.del=1 and a.sort=1  " & vbCrLf &_
			"           and ("& open_7_1 &"=3 or ("& open_7_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_7_1 &",')>0))" & vbCrLf &_
			"   inner join payback b on a.gl2=b.ord and a.del=1 and a.sort=1  " & vbCrLf &_
			"   union all       /*实际回款*/ " & vbCrLf &_
			"   select a.id, a.bank, a.gl, a.gl2, 1 sort, a.money1, 0 money2, a.date1, a.cateid cateid from bank a " & vbCrLf &_
			"   inner join PayBackSureList P on a.gl2=P.ID and a.del=1 and a.sort=101  " & vbCrLf &_
			"   inner join PayBackSure b on p.SureID=b.ID"& vbCrLf &_
			"   inner join tel t on t.ord=b.company "& vbCrLf &_
			"   and ("& open_7_1 &"=3 or ("& open_7_1 &"=1 and charindex(','+cast(t.cateid as varchar)+',',',"& intro_7_1 &",')>0))"
		end if
		if ZBRuntime.MC(25000) and ZBRuntime.MC(25002) And open_76_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*采购退款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join payout3 b on a.gl2=b.ord and a.del=1 and a.sort=14  " & vbCrLf &_
			"          and ("& open_76_1 &"=3 or ("& open_76_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_76_1 &",')>0))"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23002) And open_7_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*预收款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join ( " & vbCrLf &_
			"          select a.id, b.cateid from bankin2 a inner join tel b on a.company=b.ord  " & vbCrLf &_
			"                  and ("& open_7_1 &"=3 or ("& open_7_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_7_1 &",')>0)) " & vbCrLf &_
			"          select a.id, b.cateid from bankin2 a inner join tel b on a.company=b.ord  " & vbCrLf &_
			"  ) b on a.gl=b.id and a.del=1 and a.sort=9"
		end if
		if ZBRuntime.MC(24000) and ZBRuntime.MC(24002) And open_8_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*退预付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join bankout3 b on a.gl=b.id and a.del=1 and a.sort=12  " & vbCrLf &_
			"          and ("& open_8_1 &"=3 or ("& open_8_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_8_1 &",')>0))"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(20000) And open_11_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*直接出账*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a  " & vbCrLf &_
			"  where a.del=1 and a.sort=10  " & vbCrLf &_
			"          and ("& open_11_1 &"=3 or ("& open_11_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_11_1 &",')>0))"
		end if
		if ZBRuntime.MC(19000) and ZBRuntime.MC(22000) And open_11_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*账间转出*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a  " & vbCrLf &_
			"  where del=1 and sort=5  " & vbCrLf &_
			"          and ("& open_11_1 &"=3 or ("& open_11_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_11_1 &",')>0))"
		end if
		if ZBRuntime.MC(27000) And open_6_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*费用报销*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join ( " & vbCrLf &_
			"          select a.id, c.cateid from paybx a  " & vbCrLf &_
			"          inner join paybxlist b on b.bxid=a.id " & vbCrLf &_
			"          inner join pay c on b.payid=c.ord  " & vbCrLf &_
			"                  and ("& open_6_1 &"=3 or ("& open_6_1 &"=1 and charindex(','+cast(c.cateid as varchar)+',',',"& intro_6_1 &",')>0))  " & vbCrLf &_
			"          inner join pay c on b.payid=c.ord  " & vbCrLf &_
			"          group by a.id,c.cateid " & vbCrLf &_
			"  ) b on a.gl=b.id and a.del=1 and a.sort=40 "
		end if
		if ZBRuntime.MC(27000) And open_6_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*费用借款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid cateid from bank a " & vbCrLf &_
			"  inner join payjk b on a.gl=b.id and a.del=1 and a.sort=50  " & vbCrLf &_
			"          and ("& open_6_1 &"=3 or ("& open_6_1 &"=1 and charindex(','+cast(b.sorce2 as varchar)+',',',"& intro_6_1 &",')>0))"
		end if
		if ZBRuntime.MC(24000) and ZBRuntime.MC(24001) And open_8_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*实际付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a. sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"   INNER JOIN PayOutSureList P ON P.ID=a.gl2 and a.del=1 and a.sort=201  "  & vbCrLf &_
			"  inner join PayOutSure b on b.ID=P.SureID   " & vbCrLf &_
			"  and ("& open_8_1 &"=3 or ("& open_8_1 &"=1 and charindex(','+cast(b.Creator as varchar)+',',',"& intro_8_1 &",')>0))"
		end if
		if ZBRuntime.MC(23000) and ZBRuntime.MC(23002) And open_7_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*退预收款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join ( " & vbCrLf &_
			"          select a.id, b.cateid from bankin3 a inner join tel b on a.company=b.ord  " & vbCrLf &_
			"                  and ("& open_7_1 &"=3 or ("& open_7_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_7_1 &",')>0)) " & vbCrLf &_
			"          select a.id, b.cateid from bankin3 a inner join tel b on a.company=b.ord  " & vbCrLf &_
			"  ) b on a.gl=b.id and a.del=1 and a.sort=13"
		end if
		if ZBRuntime.MC(24000) and ZBRuntime.MC(24002) And open_8_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*预付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join bankout2 b on a.gl=b.id and a.del=1 and a.sort=11  " & vbCrLf &_
			"          and ("& open_8_1 &"=3 or ("& open_8_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_8_1 &",')>0))"
		end if
		if ZBRuntime.MC(35000) And open_5025_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*委外付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  where a.del=1 and a.sort=15  " & vbCrLf &_
			"          and ("& open_5025_1 &"=3 or ("& open_5025_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_5025_1 &",')>0))"
		end if
		if ZBRuntime.MC(18700) And open_5025_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*整单委外付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  where a.del=1 and a.sort=19  " & vbCrLf &_
			"          and ("& open_5025_1 &"=3 or ("& open_5025_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_5025_1 &",')>0))"
		end if
		if ZBRuntime.MC(18610) And open_5026_19<>1 Then
			sql = sql &" " & vbCrLf &_
			"  union all       /*工序委外付款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  where a.del=1 and a.sort=18  " & vbCrLf &_
			"          and ("& open_5026_1 &"=3 or ("& open_5026_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_5026_1 &",')>0))"
		end if
		if ZBRuntime.MC(26000) and ZBRuntime.MC(26001) And open_10_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*工资支出*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  where a.del=1 and a.sort=3  " & vbCrLf &_
			"          and ("& open_10_1 &"=3 or ("& open_10_1 &"=1 and charindex(','+cast(a.cateid as varchar)+',',',"& intro_10_1 &",')>0))"
		end if
		if ZBRuntime.MC(25000) and ZBRuntime.MC(25001) And open_9_19<>1 then
			sql = sql &" " & vbCrLf &_
			"  union all       /*销售退款*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join payout2 b on a.gl2=b.ord and a.del=1 and a.sort=8  " & vbCrLf &_
			"          and ("& open_9_1 &"=3 or ("& open_9_1 &"=1 and charindex(','+cast(b.cateid as varchar)+',',',"& intro_9_1 &",')>0))"
		end if
		if ZBRuntime.MC(20001)  And open_1101_19<>1   Then
			sql = sql &" " & vbCrLf &_
			"  union all       /*承兑入账*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, a.money1, 0 money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join AcceptanceDraft b on a.gl=b.ID and a.del=1 and a.sort=301  " & vbCrLf &_
			"          and ("& open_1101_1 &"=3 or ("& open_1101_1 &"=1 and charindex(','+cast(b.Creator as varchar)+',',',"& intro_1101_1 &",')>0))" & vbCrLf &_
			"  inner join AcceptanceDraft b on a.gl=b.ID and a.del=1 and a.sort=301  " & vbCrLf &_
			"   union all      /*承兑出账*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, a.sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join AcceptanceDraft b on a.gl=b.ID and a.del=1 and a.sort=302  " & vbCrLf &_
			"          and ("& open_1101_1 &"=3 or ("& open_1101_1 &"=1 and charindex(','+cast(b.Creator as varchar)+',',',"& intro_1101_1 &",')>0))"
		end if
		if ZBRuntime.MC(20001) or ZBRuntime.MC(23001) or ZBRuntime.MC(24001)  Then
			sql = sql &" " & vbCrLf &_
			"  union all       /*实际收款费用*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, 303 sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join PayBackSureList P on a.gl2=P.ID and a.del=1 and a.sort=102  " & vbCrLf &_
			"   inner join PayBackSure b on p.SureID=b.ID"& vbCrLf &_
			"   inner join tel t on t.ord=b.company "& vbCrLf &_
			"  and ("& open_7_1 &"=3 or ("& open_7_1 &"=1 and charindex(','+cast(t.cateid as varchar)+',',',"& intro_7_1 &",')>0))" & vbCrLf &_
			"   inner join tel t on t.ord=b.company "& vbCrLf &_
			"   union all      /*实际付款费用*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, 303 sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join PayOutSureList P on a.gl2=P.ID and a.del=1 and a.sort=202  " & vbCrLf &_
			"  inner join PayOutSure b on P.SureID=b.ID and a.del=1 and a.sort=202  " & vbCrLf &_
			"   union all      /*承兑出账费用*/ " & vbCrLf &_
			"  select a.id, a.bank, a.gl, a.gl2, 303 sort, 0 money1, a.money2, a.date1, a.cateid from bank a " & vbCrLf &_
			"  inner join AcceptanceDraft b on a.gl=b.ID and a.del=1 and a.sort=303  " & vbCrLf &_
			"          and ("& open_1101_1 &"=3 or ("& open_1101_1 &"=1 and charindex(','+cast(b.Creator as varchar)+',',',"& intro_1101_1 &",')>0))"
		end if
		getCashSql = sql
	end function
	function getQuarter(dateStr)
		ms=month(cDate(dateStr))
		mt=int((ms+2)/3)
		ms=month(cDate(dateStr))
		getQuarter=mt
	end function
	function getQuarterFrom(d)
		q=getQuarter(d)
		if q=1 then
			getQuarterFrom=year(d)&"-1-1"
'if q=1 then
		elseif q=2 then
			getQuarterFrom=year(d)&"-4-1"
'elseif q=2 then
		elseif q=3 then
			getQuarterFrom=year(d)&"-7-1"
'elseif q=3 then
		elseif q=4 then
			getQuarterFrom=year(d)&"-10-1"
'elseif q=4 then
		end if
	end function
	function getQuarterEnd(d)
		...
		q=getQuarter(d)
		if q=1 then
			getQuarterEnd=year(d)&"-3-31"
'if q=1 then
		elseif q=2 then
			getQuarterEnd=year(d)&"-6-30"
'elseif q=2 then
		elseif q=3 then
			getQuarterEnd=year(d)&"-9-30"
'elseif q=3 then
		elseif q=4 then
			getQuarterEnd=year(d)&"-12-31"
'elseif q=4 then
		end if
	end function
	
	Server.ScriptTimeOut=100000000
	t=int(getInt(request("t")))
	if t=1 then
		types="收入"
	elseif t=2 then
		types="支出"
	...
	bname=Conver(request("bname"))
	comecashworker=request("comecashworker")
	Dim advrType,advrType1
	advrType="0"
	advrType1="0"
	Dim getadvrTypeSql
	bname=Request("asft_receipts")
	B=getInt(Request("seachBZ"))
	advrPrice1=getInt(Request("advPrice1"))
	advrPrice2=getInt(Request("advPrice2"))
	advrWorker=asearch.GetGates(1)
	m1=conver(Request("advTime1_"))
	m2=conver(Request("advTime2_"))
	advrType=request("Revenue_")
	If Replace(Replace(Revenue_,"0,",""),",","")<>"" Then advrType=Revenue_
	If advrBz<>"0" and advrBz<>"" Then BzSql1=" and c.id="&advrBz& " "
	If t=1 Then
		If isnumeric(advrPrice1) and advrPrice1>"0" Then MoneySql=" and a.money1>="&advrPrice1&" "
		If isnumeric(advrPrice2) and advrPrice2>"0" Then MoneySql=MoneySql&" and a.money1<="&advrPrice2&" "
	Elseif t=2 Then
		If isnumeric(advrPrice1) and advrPrice1>"0"Then MoneySql=" and a.money2>="&advrPrice1&" "
		If isnumeric(advrPrice2) and advrPrice2>"0" Then MoneySql=MoneySql&" and a.money2<="&advrPrice2&" "
	Elseif t=3 Then
		If isnumeric(advrPrice1) and advrPrice1>"0" Then MoneySql=" (a.money2>="&advrPrice1&" and a.money1>="&advrPrice1&") "
		If isnumeric(advrPrice2) and advrPrice2>"0" Then
			If MoneySql<>"" Then MoneySql=MoneySql&" or "
			MoneySql=MoneySql&" (a.money2<="&advrPrice2&" and a.money1<="&advrPrice2&") "
		end if
		If MoneySql<>"" Then MoneySql=" and ("&MoneySql&")"
	else
		conn.close
		set conn=Nothing
		call db_close : Response.end()''''
	end if
	If isnumeric(advrWorker) and advrWorker>"0" Then WorkerID=advrWorker
	If WorkerID&""<>"" And WorkerID&""<>"0" Then str_Result=str_Result+" and t.cateid="& WorkerID &" "
'If isnumeric(advrWorker) and advrWorker>"0" Then WorkerID=advrWorker '
	If advrType<>"" and advrType<>"0,0,0" and advrType<>"0" Then getadvrTypeSql=" and a.sort in ("&advrType&") "
	If comecashworker&""<>"" And comecashworker&""<>"0" Then str_Result=str_Result+" and t.cateid="& comecashworker &" "
	If not isNumeric(B) Then
		Response.write "<script>alert('没有数据不能导出！');history.go(-1);</script>"
'If not isNumeric(B) Then
		call db_close : Response.end()
	end if
	if isDate(m1) then
...
'if isDate(m1) then
	...
	if isDate(m2) then
		Str_Result=Str_Result+" and  a.date1<='"&m2&"'"
'if isDate(m2) then
	end if
	if B>0 then
		str_Result=str_Result+" and c.id="&B&" "
'if B>0 then
	end if
	if C>0 then
		str_Result=str_Result+" and a.sort="&C&" "
'if C>0 then
	end if
	if t=1 then
		if total1>0 then
...
'if total2>0 then
	...
	elseif t=2 then
		if total1>0 then
			Str_Result=Str_Result+" and  a.money2>="&total1
'if total1>0 then
		end if
		if total2>0 then
			Str_Result=Str_Result+" and  a.money2<="&total2
'if total2>0 then
		end if
		str_sorce=0
	end if
	if total1=0 then total1=""
	if total2=0 then total2=""
	if bname<>"" then
		Str_Result=Str_Result & " and ((a.gl in(select ord from contract where title like'%"&bname&"%') and a.sort=1)" & _
		" or (a.gl in(select ord from caigou where title like'%"&bname&"%') and a.sort=2)" & _
		" or (a.gl in(select id from wages where title like'%"&bname&"%') and a.sort=3)" & _
		" or (a.gl in(select ord from pay where title like'%"&bname&"%') and a.sort=4)" & _
		" or (a.gl in(select id from bankmove where title like'%"&bname&"%') and a.sort in(5,6))" & _
		" or (a.gl in(select id from bankin where title like'%"&bname&"%') and a.sort=7)" & _
		" or (a.gl in(select ord from contractth where title like'%"&bname&"%') and a.sort=8)" & _
		" or (a.gl in(select id from bankin2 where intro like'%"&bname&"%') and a.sort=9)" & _
		" or (a.gl in(select id from bankout where title like'%"&bname&"%') and a.sort=10)" & _
		" or (a.gl in(select id from bankout2 where intro like'%"&bname&"%') and a.sort=11)" & _
		" or (a.gl in(select id from bankout3 where intro like'%"&bname&"%') and a.sort=12)" & _
		" or (a.gl in(select id from bankin3 where intro like'%"&bname&"%') and a.sort=13)" & _
		" or (a.gl in(select ord from caigouth where title like'%"&bname&"%') and a.sort=14)" & _
		" or (a.gl in(select id from paybx where title like'%"&bname&"%') and a.sort=40)" & _
		" or (a.gl in(select id from payjk where title like'%"&bname&"%') and a.sort=50) "&_
		" or (a.gl in (select id from AcceptanceDraft where SN like '%"&bname&"%') and a.sort IN (301,302,303)) " &_
		" or (a.gl in (select id from PayoutSure where title like '%"&bname&"%') and a.sort IN (201,202))  " &_
		" or (a.gl in (select id from PayBackSure where title like '%"&bname&"%') and a.sort IN (101,102)) " &_
		")"
	end if
	px=getInt(request("px"))
	if px=0 then
		px=1
	end if
	if px=1 then
		px_Result=" order by a.date1 desc,a.id desc"
	elseif px=2 then
		px_Result=" order by a.date1 asc,a.id"
	elseif px=3 then
		px_Result=" order by a.intro desc"
	elseif px=4 then
		px_Result=" order by a.intro asc"
	elseif px=5 then
		if t=1 then
			px_Result=" order by a.money1 desc"
		elseif t=2 then
			px_Result=" order by a.money2 desc"
		end if
	elseif px=6 then
		if t=1 then
			px_Result=" order by a.money1 asc"
		elseif t=2 then
	...
	qxSort1 = "5,6,7,8,9,10,11,22,41,75,76,5025,5026"
	arr_qxSort1 = Split(qxSort1,",")
	For i = 0 To ubound(arr_qxSort1)
		execute("open_"& arr_qxSort1(i)  &"_19=1")
		execute("open_"& arr_qxSort1(i)  &"_1=0")
		execute("intro_"& arr_qxSort1(i)  &"_1=""-222"" ")
		execute("open_"& arr_qxSort1(i)  &"_1=0")
	next
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select sort1, sort2, isnull(qx_open,0) qx_open,isnull(qx_intro,'-222') qx_intro from power  where ord="&curCate&" and sort1 in("& qxSort1 &") and sort2 in(10,19)"
	set rs1=server.CreateObject("adodb.recordset")
	rs1.open sql1,conn,1,1
	While rs1.eof = False
		Select Case rs1("sort2")
		Case 19
		execute("open_"& rs1("sort1")  &"_"& rs1("sort2")  &"="& rs1("qx_open"))
		Case 10
		execute("open_"& rs1("sort1")  &"_1="& rs1("qx_open"))
		If Len(Replace(rs1("qx_intro")," ","")&"") = 0 Then
			execute("intro_"& rs1("sort1")  &"_1=""-222""")
'If Len(Replace(rs1("qx_intro")," ","")&"") = 0 Then
		else
			execute("intro_"& rs1("sort1")  &"_1="""& rs1("qx_intro")&"""")
		end if
		End Select
		rs1.movenext
	wend
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(3).NumberFormatLocal = "#,##0."&dotstr&"_ "
			...
			k=7
			if t=1 or t=3 then
				xlApplication.ActiveSheet.Columns(k).ColumnWidth=10
				xlApplication.ActiveSheet.Columns(k).HorizontalAlignment=3
				xlApplication.ActiveSheet.Columns(k).NumberFormatLocal = "#,##0."&dotstr&"_ "
				k=k+1
				xlApplication.ActiveSheet.Columns(k).NumberFormatLocal = "#,##0."&dotstr&"_ "
			end if
			if t=2 or t=3 then
			...
			xlWorksheet.Cells(1,2).Value = "币种"
			xlWorksheet.Cells(1,2).font.Size=10
			xlWorksheet.Cells(1,2).font.bold=true
			xlWorksheet.Cells(1,3).Value = "汇率"
			xlWorksheet.Cells(1,3).font.Size=10
			xlWorksheet.Cells(1,3).font.bold=true
			xlWorksheet.Cells(1,4).Value = "单据类型"
			xlWorksheet.Cells(1,4).font.Size=10
			xlWorksheet.Cells(1,4).font.bold=true
			xlWorksheet.Cells(1,5).Value = "单据名称"
			xlWorksheet.Cells(1,5).font.Size=10
			xlWorksheet.Cells(1,5).font.bold=true
			xlWorksheet.Cells(1,6).Value = "单据编号"
			xlWorksheet.Cells(1,6).font.Size=10
			xlWorksheet.Cells(1,6).font.bold=true
			k=7
			if t=1 or t=3 then
				xlWorksheet.Cells(1,k).Value = "收入金额"
				xlWorksheet.Cells(1,k).font.Size=10
				xlWorksheet.Cells(1,k).font.bold=true
				k=k+1
				xlWorksheet.Cells(1,k).font.bold=true
			end if
			if t=2 or t=3 then
				xlWorksheet.Cells(1,k).Value = "支出金额"
				xlWorksheet.Cells(1,k).font.Size=10
				xlWorksheet.Cells(1,k).font.bold=true
				k=k+1
				xlWorksheet.Cells(1,k).font.bold=true
			end if
			xlWorksheet.Cells(1,k).Value = "银行账号"
			xlWorksheet.Cells(1,k).font.Size=10
			xlWorksheet.Cells(1,k).font.bold=true
			k=k+1
			xlWorksheet.Cells(1,k).font.bold=true
			xlWorksheet.Cells(1,k).Value = "经手人"
			xlWorksheet.Cells(1,k).font.Size=10
			xlWorksheet.Cells(1,k).font.bold=true
			Response.Flush
			Dim BzOpen,BzSql
			BzOpen=false
			set RsBz=conn.execute("select bz from setbz")
			If Not RsBz.eof Then
				If RsBz(0)=1 Then BzOpen=true
			end if
			RsBz.close
			Set RsBz=Nothing
			baseSql = getCashSql()
			conn.execute("if exists(select top 1 1 from tempdb..sysobjects where name='tempdb..#cash') drop table #cash; select distinct * into #cash from ("& baseSql &") t")
			QxSql=""
			If Not BzOpen Then BzSql=" and c.sort1='人民币'"
			strTable = " from bank a WITH(NOLOCK) "&_
			"  inner join #cash t WITH(NOLOCK) on a.id=t.id "&_
			"  inner join sortbank b WITH(NOLOCK) on a.bank=b.id "&_
			"  inner join sortbz c WITH(NOLOCK) on c.id=b.bz "&BzSql&BzSql1&MoneySql&getadvrTypeSql & str_Result &_
			"  inner join zdymx d WITH(NOLOCK) on case a.sort when 101 then 1 when  102 then 303  when 202 then 303 else a.sort end=d.sort and d.sort1=44 and d.kd2="& curCate
			if t<>3 then
				strTable=strTable&" and d.sorce="&str_sorce
			end if
			sql="select isnull(sum(isnull(a.money1,0)),0) as total1,isnull(sum(isnull(a.money2,0)),0) as total2"&strTable
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn
			if not rs.eof then
				totalmoney1=rs(0)
			...
			rs.close
			set rs1=server.CreateObject("adodb.recordset")
			sql="select a.*,(case when replace(cast(b.person as varchar(8000)),' ','')='0' or charindex('," & session("personzbintel2007") & ",',','+cast(b.person as varchar(8000))+',')>0 then b.sort1 else '' end) as bankname,a.intro title,c.sort1,hl=(select hl from hl where bz=b.bz and date1=a.date1) "& _
			",catename=(select top 1 name from gate where a.cateid=ord)" & _
			"&strTable&"" ""&px_Result&"" """
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "公司"&types&"明细_"&session("name2006chen")&".xls",C1
			i=0
			do while not rs.eof
				...
				gl = rs("gl")
				gl2 = rs("gl2")
				If Len(gl&"")=0 Then gl=0
				if sort1=1 then
					strSql="select top 1 title,htid from contract where ord=" & gl
				elseif sort1=2 then
					strSql="select title,cgid from caigou where ord=" & gl
				elseif sort1=3 then
					strSql="select top 1 title,'' from wages where id=" & gl
				elseif sort1=4 then
					strSql="select top 1 title,'' from pay where ord=" & gl
				elseif sort1=5 then
					strSql="select top 1 title,'' from bankmove where id=" & gl
				elseif sort1=6 then
					strSql="select top 1 title,'' from bankmove where id=" & gl
				elseif sort1=7 then
					strSql="select top 1 title,'' from bankin where id=" & gl
				elseif sort1=8 then
					strSql="select top 1 title,thid from contractth where ord=" & gl
				elseif sort1=9 then
					strSql="select top 1 intro,'' from bankin2 where id=" & gl
				elseif sort1=10 then
					strSql="select top 1 title,'' from bankout where id=" & gl
				elseif sort1=11 then
					strSql="select top 1 intro,'' from bankout2 where id=" & gl
				elseif sort1=12 then
					strSql="select top 1 title,'' from bankout3 where id=" & gl
				elseif sort1=13 then
					strSql="select top 1 intro,'' from bankin3 where id=" & gl
				elseif sort1=14 then
					strSql="select top 1 (case when a.fromtype=1 then cgt.title when a.fromtype=2 then cg.title when a.fromtype in (3,4) then moo.title else '' end ) as title, "&_
					"a.BH "&_
					"from payout3 a "&_
					...
					"where a.ord=" & gl2
				elseif sort1=15 then
					strSql="select top 1 title,sn from M2_OutOrder where id=" & gl
				elseif sort1=18 then
					strSql="select top 1 title,sn from M2_OutOrder where id=" & gl
				elseif sort1=40 then
					strSql="select top 1 title,bh from paybx where id=" & gl
				elseif sort1=50 then
					strSql="select top 1 title,'' from payjk where id=" & gl
				elseif sort1=101 or sort1=102 then
					strSql="select top 1 title,BH from PayBackSure where id=" & gl
				elseif sort1=201 or sort1=202 then
					strSql="select top 1 title,SN from PayoutSure where id=" & gl
				elseif sort1=301 or sort1=302 or sort1=303 then
					strSql="select top 1 SN title,'' from AcceptanceDraft where id=" & gl
				end if
				...
				rs1.close
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,1).Value = formatdatetime(rs("date1"),0)
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,1).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,1).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,2).Value = rs("sort1")
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,2).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,2).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,3).Value = hl
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,3).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,3).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,4).Value = rs("title")
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,4).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,4).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,5).Value = title
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,5).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,5).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,6).Value = no
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,6).font.Size=10
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				xlWorksheet.Cells(2+i,6).HorizontalAlignment=3
				if isNull(zbcdbl(rs("hl"))) then hl=1 else hl=zbcdbl(rs("hl"))
				k=7
				if t=1 or t=3 then
					xlWorksheet.Cells(2+i,k).Value = Formatnumber(money1,num_dot_xs,-1)
'if t=1 or t=3 then
					xlWorksheet.Cells(2+i,k).font.Size=10
'if t=1 or t=3 then
					xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
'if t=1 or t=3 then
					k=k+1
'if t=1 or t=3 then
				end if
				if t=2 or t=3 then
					xlWorksheet.Cells(2+i,k).Value = Formatnumber(money2,num_dot_xs,-1)
'if t=2 or t=3 then
					xlWorksheet.Cells(2+i,k).font.Size=10
'if t=2 or t=3 then
					xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
...
'if t=2 or t=3 then
				...
				xlWorksheet.Cells(2+i,k).Value = rs("bankname")
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
'if t=2 or t=3 then
				k=k+1
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).Value = rs("catename")
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
'if t=2 or t=3 then
			...
			rs.close
			set rs=Nothing
			xlWorksheet.Cells(2+i,6).Value = "合计"
			set rs=Nothing
			xlWorksheet.Cells(2+i,6).font.Size=10
			set rs=Nothing
			xlWorksheet.Cells(2+i,6).HorizontalAlignment=3
			...
			k=7
			if t=1 or t=3 then
				xlWorksheet.Cells(2+i,k).Value = Formatnumber(totalmoney1,num_dot_xs,-1)
'if t=1 or t=3 then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if t=1 or t=3 then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
...
'if t=1 or t=3 then
			...
			if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).Value = Formatnumber(totalmoney2,num_dot_xs,-1)
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if t=2 or t=3 then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=3
...
'if t=2 or t=3 then
			...
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>公司"&types&"明细导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!公司"&types&"明细导出成功,共有"&(i)&"条记录!</B>"";</script>"
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("公司"&types&"明细_"&session("name2006chen")&".xls")
			...
			action1="公司"&types&"明细导出"
			call close_list(1)
			Response.write " " & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的公司"
			Response.write types
			Response.write "明细</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
%>
