...
'Private Sub Class_Initialize
    ...
    function MessagePost(msgId)
        ...
        call page_load
        Case "getGateInfo"
        Call App_getGateInfo
        case "onCardDrag"
        call App_onCardDrag
        case "GetSupplierPriceList"
        call App_GetSupplierPriceList
        Case "saveformula"
        Call App_saveformula
        Case "getformula"
        Call App_getformula
        Case "delUrl"
        Response.write request.form("v")
    ...
    sub App_onCardDrag
        dim id1 , id2 , isLast, newCls, pos
        id1 = app.getInt("id1")
        id2 = app.getInt("id2")
        isLast = app.getInt("isLast")
        pos = app.iif(app.getText("pos")="left",0,1)
        cn.execute "exec home_InserCardPost " & id1 & "," & id2 & "," & info.user & "," & isLast & "," & pos
    end sub
    sub page_load
        dim stab , rs, i
        ...
        app.docmodel = "ie=8"
        cn.execute "update setjm set tq1 = 0 where tq1 is null"
        Response.write app.defheadhtml("../","")
        i = app.getint("index")
        if i=0 then i = 1
        Response.write "" & vbcrlf & "      <body class='mainHome me' id='hmainbody' onresize='body_onreisze()' onload='body_onreisze();window.addtipText()'>" & vbcrlf & "       <div id=""hmainbodybtn"">" & vbcrlf & "    "
        if  info.isSupperAdmin then
            Response.write "" & vbcrlf & "            <div class=""sys_guide"" onclick=""window.open('../../SYSN/view/init/guide.ashx','guidewindow','height=800,width=1000,left=100,top=30,z-look=yes,status=no,toolbar=no,menubar=no,location=no,resizable=yes,scrollbars=yes')""><span class=""guide_txt"">启用引导</span></div>" &vbcrlf & "    "
        end if
        Response.write "" & vbcrlf & "              <div class=""system_flow_chart"" onclick=""window.open('../../SYSN/view/init/SysFlowChart.html','flowwindow','height=800,width=1280,left=10,top=30,z-look=yes,status=no,toolbar=no,menubar=no,location=no,resizable=yes,scrollbars=yes')""><span class=""flow_txt"">系统流程图</span></div>"& vbcrlf & "     </div> " & vbcrlf & ""
        If i = 1 Then
            Response.write "" & vbcrlf & "              <style>" & vbcrlf & "                 html{_overflow:hidden;background:#efefef;padding:10px 10px 0px;box-sizing:border-box;}" & vbcrlf & "                  body{_overflow:hidden;position:relative;}" & vbcrlf & "               </style>" & vbcrlf & "                "
'If i = 1 Then
        end if
        ...
        stab.extHeight = -10
        stab.cellspacing = 10
        stab.itemTopHeight = 10
        stab.items.add  "工作台" ,"" , "工作台"
        set rs = cn.execute("exec home_getCardsData 1," & info.user & ",'','" & app.power.models & "'")
        while rs.eof = false
            stab.items.add  rs.fields(0).value ,"" , rs.fields(0).value
            rs.movenext
        wend
        stab.selectindex = cint(i)
        rs.close
        set rs = nothing
        stab.toprighthtml = ""
        stab.writehtml
        set stab = nothing
        Response.write "" & vbcrlf & "    <script src=""../skin/default/js/canvasPanel.js""></script>" & vbcrlf & "   <script type=""text/javascript"">" & vbcrlf & "   //currlinkTime和FirstlinkTime防止点击后退按钮直接退出，关联home.js 606行代码" & vbcrlf & "    if(!top.FirstlinkTime) {" & vbcrlf & "                top.FirstlinkTime = """
        Response.write now
        Response.write """;" & vbcrlf & " }" & vbcrlf & "       top.currlinkTime = """
        Response.write now
        Response.write """;" & vbcrlf & " window.InitNameSpace = function () {" & vbcrlf & "        if (document.documentMode != 9 && document.namespaces) {" & vbcrlf & "            this.DrawApi = 0;" & vbcrlf & "               if (window.ChartImagesInited == true) {" & vbcrlf & "             return;" & vbcrlf & "             }" & vbcrlf & "           if (!document.documentMode || document.documentMode < 8) {" & vbcrlf & "                  document.namespaces.add(""v"", ""urn:schemas-microsoft-com:vml"");" & vbcrlf & "              document.createStyleSheet().addRule(""v\\:*"", ""behavior: url(#default#VML);"");" & vbcrlf & "           } else {" & vbcrlf & "                 document.namespaces.add(""v"", ""urn:schemas-microsoft-com:vml"", ""#default#VML"");" & vbcrlf & "            }" & vbcrlf & "           }" & vbcrlf & "       window.ChartImagesInited = true;" & vbcrlf & "    };" & vbcrlf & "      InitNameSpace()" & vbcrlf & "    </script>" & vbcrlf &"<script src=""../skin/default/js/vmlPanel.js""></script>" & vbcrlf & "      </body>" & vbcrlf & " </html>" & vbcrlf & " "
    end sub
    Sub loadWorkPanel
        Response.write "<iframe id='workframe' style='width:100%;height:100%;*+height:expression((document.body.offsetHeight-41) + ""px"");margin:0px;' frameborder=0 src='homelinks.asp'></iframe>"
'Sub loadWorkPanel
    end sub
    sub App_OnLoadTabItem(item)
        If item.Text = "工作台" Then
            Call loadWorkPanel
            Exit sub
        end if
        select case item.id
        case "tb1"
        call loadMainCardList(item.key, item.index)
        end select
    end sub
    Sub loadMainCardList(cls , i)
        Response.write "<style>div.newskin div.listview {min-width:100%;}</style>"
'Sub loadMainCardList(cls , i)
        dim cview,rs,tb,msql,attrs
        dim t1,t2,powerv,item,rs2,addqx
        set cview = new cardview
        cview.id = "cardlist" & i
        cview.cols = 3
        cview.itemHeight  = ""
        cview.asynModel = true
        set rs = cn.execute("exec home_getCardsData 3," & info.user & ",'" & replace(cls,"'","''") & "','" & app.power.models & "'")
        if rs.eof = true then
            set tb = cview.add("cc", 3)
            tb.isSpace = true
            tb.configId = "card_c_0"
            tb.canmove = true
            tb.colspan = 3
            tb.tag2 = 0
            tb.tag3 = "left"
        end if
        Dim canShow,reminder,isRemind
        while rs.eof = False
            If rs.fields("title").value = "管理驾驶舱" Then
                powerv = ((app.power.existsModel(6000) Or app.power.existsModel(7000)) or app.power.existsModel(15000))
            else
                powerv = true
            end if
            isRemind = rs("id") > 10000 And rs.fields("setjm") > 0
            canShow = True
            If isRemind Then
                Set reminder = CreateReminderHelper(cn,rs("setjm"),0)
                canShow = reminder.hasModule And reminder.isOpened
            end if
            If powerv And canShow  then
                set tb = cview.add(rs.fields("title").value, rs.fields("colspan").value)
                tb.canadd = rs.fields("canadd").value
                tb.canmore= rs.fields("canmore").value
                tb.showdate = rs.fields("monthjs").value
                tb.searchlist = rs.fields("defjs").value
                tb.showAdv   = rs.fields("gjjs").value
                tb.configId = "card_c_" & rs.fields("ID").value
                tb.canmove = true
                tb.colspan = rs.Fields("colspan").Value
                If tb.colspan < 1 Then tb.colspan = 1
                tb.tag2 = rs.fields("ID").value
                tb.tag3 = "left"
                attrs = rs.fields("attrs").value
                if tb.tag2 < 10000 then
                    tb.cansetting = False
                else
                    Set rs2 = cn.execute("select isnull(addqxlb,0) as r from home_maincards_def where id=" & rs.fields("ID").value)
                    If rs2.eof = False Then
                        addqx = rs2.fields(0).value
                    else
                        addqx = 0
                    end if
                    rs2.close
                    tb.addlink = rs.Fields("addurl").Value
                    If addqx > 0 Then
                        Set rs2 = cn.execute("select qx_open from power where qx_open>0 and sort1=" & addqx & " and sort2=13 and ord=" & Info.user)
                        tb.canadd = Not rs2.eof
                        rs2.close
                    end if
                end if
                If Len(attrs) = 0 Then
        ...
        set rs = cn.execute("exec home_getCardsData 2," & info.user & ",'" & replace(cls,"'","''") & "','" & app.power.models & "'")
        if rs.eof = true then
            set tb = cview2.add("cr", 1)
            tb.isSpace = true
            tb.configId = "card_c_r0"
            tb.canmove = true
            tb.colspan = 1
            tb.tag2 = 0
            tb.tag3 = "right"
        ...
        while rs.eof = false
            ...
            If canShow Then
                i = i + 1
'If canShow Then
                Set item = cview2.add(rs.fields("title").value, 1)
                item.canadd = rs.Fields("canadd").Value
                item.addlink = rs.Fields("addurl").Value
                item.colspan = rs.Fields("colspan").Value
                item.tag2 = rs.fields("ID").value
                item.tag3 = "right"
                if item.tag2 < 10000 then
                    item.cansetting = False
                else
                    Set rs2 = cn.execute("select isnull(addqxlb,0) as r from home_maincards_def where id=" & rs.fields("ID").value)
                    If rs2.eof = False Then
                        addqx = rs2.fields(0).value
        ...
        wend
        rs.close
        Response.write "<div id='controlpanelbody" & i & "' class='controlpanelbody' >" & cview.html  & "</div><div id='controlRPenl" & i & "' class='controlRPenl'>" & cview2.html & "</div>"
        set cview = nothing
'set cview = nothing
    end sub
    sub App_onCardloaditem(e)
        dim unit , rs , v , sql , i, colspan , lvw, ii , w , msql , sty, val , val1
        Dim defrow, htmlv, tmpv, fw
        Dim datavar
        dim id , aSearchItems , sitem , aryReturn2, attrs , r
        Dim r_name, r_value, r_count
        if instr(e.id,"cardlist") > 0 then
            select case e.key
            case "管理驾驶舱"
            call LoadDriverPage : exit Sub
            case "综合报表"
            e.obj.addhtml now
            exit sub
            end select
        end if
        If Len(e.t1)=0 Then e.t1 = "1990-01-01"
'end select
        If Len(e.t2)=0 Then e.t2 = Date()
        if instr(e.id,"driverlist") > 0 then
            unit = app.attributes("仪表盘单位_" & e.key)
            if len(unit) = 0 then unit = "万元"
            DIM  formulaIndex : formulaIndex = app.Attributes("lirun_jsfs")
            if formulaIndex&""="" then formulaIndex = 1
            set rs = cn.execute("exec home_getdriverdata '" & e.key & "','" & unit & "','" & e.t1 & "','" & e.t2 & "'," & info.user & "," & formulaIndex)
            if rs.eof  then
                e.obj.addhtml  createCanvas(0,e.index,"万元",0)
            else
                v = rs.fields(0).value
                unit = rs.fields(1).value
                htmlv = rs.fields(2).value
                if isnumeric(v) then
                    e.obj.addhtml createCanvas(v,e.index,unit,htmlv)
                end if
            end if
            exit sub
        end if
        colspan = 1
        set rs = cn.execute("" & _
        "select sql,colspan,x.id,attrs,defrows,isnull(y.fw1,0) fw,x.setjm setjmId " & vbcrlf &_
        "from home_maincards_us x " & vbcrlf &_
        "left join setjm y on x.setjm = y.ord and y.cateid=" & Info.user & " " & vbcrlf &_
        "where title='" & e.Key & "' and uid=" & Info.user)
        If rs.eof = True Then
            rs.close
            set rs = cn.execute("" &_
            "select sql,colspan,x.id,attrs,defrows,isnull(y.fw1,0) fw,x.setjm setjmId " & vbcrlf &_
            "from home_maincards_def x " & vbcrlf &_
            "left join setjm y on x.setjm = y.ord and y.cateid=" & Info.user & " " & vbcrlf &_
            "where title='" & e.Key & "'")
        end if
        Dim reminder,isRemind
        isRemind = False
        if rs.eof = false then
            colspan = rs.fields(1).value
            id = rs.fields("id").value
            If id > 10000 And rs("setjmId") > 0 Then
                isRemind = True
                Set reminder = CreateReminderHelper(cn,rs("setjmId"),0)
                reminder.showStatusField = False
                sql = reminder.listSQL("top")
            else
...
'sql = reminder.listSQL("top")
        ...
        if len(trim(sql)) = 0 then
            e.obj.addhtml "<div class='no_def'>&nbsp;</div>"
            exit sub
        end if
        r_count = 0
        ReDim r_name(0)
        ReDim r_value(0)
        if colspan > 1 then
            e.obj.addhtml  "<div style='padding:0px;' class='newskin'>"
...
'msql = replace(sql,"@uid", info.user)
            r_name(r_count) = "@t1"
            r_value(r_count) = e.t1
            r_name(r_count + 1) = "@t2"
            'r_value(r_count) = e.t1
            r_value(r_count+ 1) = e.t2
            'r_value(r_count) = e.t1
            r_name(r_count+2) = "@t3"
            'r_value(r_count) = e.t1
            r_value(r_count+2) = dateadd("m",-1,e.t1)
            'r_value(r_count) = e.t1
            r_name(r_count+3) = "@t4"
            'r_value(r_count) = e.t1
            r_value(r_count+3) = dateadd("d",datediff("d",e.t1,e.t2),dateadd("m",-1,e.t1))
            'r_value(r_count) = e.t1
            r_name(r_count+4) = "@tweek1"
            'r_value(r_count) = e.t1
            r_value(r_count+4) = dateadd("d",-weekday(e.t1)+2,e.t1)
            'r_value(r_count) = e.t1
            r_name(r_count+5) = "@tweek2"
            'r_value(r_count) = e.t1
            r_value(r_count+5) = dateadd("d",7-(DatePart("w",e.t2)-1),e.t2)
            'r_value(r_count) = e.t1
            r_name(r_count+6) = "@tmonth1"
            'r_value(r_count) = e.t1
            r_value(r_count+6) = dateadd("d",-day(e.t1)+1,e.t1)
            'r_value(r_count) = e.t1
            r_name(r_count+7) = "@tmonth2"
            'r_value(r_count) = e.t1
            r_value(r_count+7) = dateadd("d",-1,dateadd("m",1,dateadd("d",-day(e.t2)+1,e.t2)))
            'r_value(r_count) = e.t1
            r_name(r_count+8) = "@tyear1"
            'r_value(r_count) = e.t1
            r_value(r_count+8) = year(e.t1)&"-1-1" 'dateadd("d",-day(e.t1)+1,e.t1)
            'r_value(r_count) = e.t1
            r_name(r_count+9) = "@tyear2"
            'r_value(r_count) = e.t1
            r_value(r_count+9) = dateadd("d",-1,year(e.t2)+1&"-1-1") 'dateadd("d",-1,dateadd("m",1,dateadd("d",-day(e.t2)+1,e.t2)))
            'r_value(r_count) = e.t1
            r_count = r_count + 9
            'r_value(r_count) = e.t1
            msql = replace(msql ,"@t1", "'" &  e.t1 & "'")
            msql = replace(msql ,"@t2", "'" &  e.t2 & "'")
            msql = replace(msql,"@t3","'" & dateadd("m",-1,e.t1) & "'")
'msql = replace(msql ,"@t2", "'" &  e.t2 & "'")
            msql = replace(msql,"@t4","'" & year(dateadd("m",-1,e.t2))&"-"&month(dateadd("m",-1,e.t2))&"-"&day(date) & "'")
'msql = replace(msql ,"@t2", "'" &  e.t2 & "'")
...
'If instr(attrs(i),"@")=1 and lcase(attrs(i))<>"@t1" and lcase(attrs(i)) <> "@t2" Then
            ...
            Dim link, h
            If len(request.form("lvwpagesize")) > 0 then
                lvw.pagesize = request.form("lvwpagesize")
            else
                lvw.pagesize = 5
            end if
            lvw.border = -1
            lvw.pagesize = 5
            lvw.dataattr = e.Key & "$$$" &  e.t1 & "$$$" & e.t2
            lvw.pageindex = 1
            lvw.mincellwidth = 50
            If e.Key = "客户分布" Then
            ...
            Call setListViewCol(lvw, e.key, r_name, r_value)
            If id = 13 Then
            ...
            r = rs.eof
            If Abs(Err.number) > 0 then
                e.obj.addhtml "<center style='color:#e0e0f0'><br>数据源异常:" & sql & "<br><br><br></center>"
            else
                On Error GoTo 0
                Dim fdname,cellOneWidth
                defrow = CInt(defrow)
                If defrow < 0 Then  defrow = 4
                e.obj.addhtml "<table class='smlist' align='center' style='table-layout:fixed;overflow:hidden;'>"
'If defrow < 0 Then  defrow = 4
                if rs.eof = False Then
                    while rs.eof = False
                        If i < defrow then
                            e.obj.addhtml "<tr class='r" & (i mod 2) & "'>"
                            e.obj.addhtml "<td class='smlist_spc' style='table-layout:fixed;width:1px;'> </td>"
'e.obj.addhtml "<tr class='r" & (i mod 2) & "'>"
                            val=""
                            Dim showCellIndex
                            showCellIndex = 0
                            for ii = 0 to rs.fields.count - 1
                                showCellIndex = 0
                                fdname = rs.fields(ii).name
                                w = replace(fdname,"w","")
                                If (isRemind = True And InStr(1,",title,dt,","," & fdname & ",",1) > 0) Or isRemind <> True Then
                                    val = rs.fields(ii).value & ""
                                    If rs.fields.count > 1 Then
                                        If isRemind Then
                                            val1 = rs.fields("dt").value & ""
                                        else
                                            val1 = rs.fields(1).value & ""
                                        end if
                                    else
                                        val1=""
                                    end if
                                    IF val<>"" then
                                        If showCellIndex=0 then
                                            if isnumeric(val) then
                                                sty=" widl_28p"
                                                cellOneWidth = 28
                                            else
                                                if rs.fields.count>0 then
                                                    if len(cstr(val1))>23 then
                                                        cellOneWidth = 28
                                                    elseif len(cstr(val1))=8 or len(cstr(val1))=9 or len(cstr(val1))=10 then
                                                        cellOneWidth = 63
                                                    elseif len(cstr(val1))<23 and len(cstr(val1))>8 then
                                                        cellOneWidth = 37
                                                    elseif len(cstr(val1))<8 then
                                                        cellOneWidth = 72
                                                    else
                                                        cellOneWidth = 50
                                                    end if
                                                    sty=" widl_" & cellOneWidth & "p"
                                                end if
                                            end if
                                        Elseif showCellIndex=1 then
                                            sty=" widr_" & (100 - cellOneWidth) & "p"
'Elseif showCellIndex=1 then
                                        end if
                                    else
                                        cellOneWidth = 50
                                        sty=app.iif(showCellIndex=0," widl_50p"," widr_50p")
                                    end if
                                    If isRemind Then
                                        Select Case fdname
                                        Case "title" :
                                        val = reminder.getTitleHTML(rs)
                                        Case "dt" :
                                        val = reminder.getDtHTML(rs)
                                        Case Else :
                                        End Select
                                    end if
                                    If showCellIndex=0 then
                                        if isnumeric(w) then
                                            e.obj.addhtml "<td class='"& sty & "' style='width:" & w & "px;word-break:'><span class='rtdline elli'>" & val & "</span></td>"
'if isnumeric(w) then
                                        else
                                            If isRemind Then
                                                e.obj.addhtml "<td class='"& sty & "' style='word-break:break-all'><span class='rtdline elli'>" & val & "</span></td>"
'If isRemind Then
                                            else
                                                e.obj.addhtml "<td class='"& sty & "' style='word-break:break-all'><span class='rtdline elli'>" & val & "</span></td>"
                                            end if
                                        ...
                                        If rs.fields.count = 1 Then
                                            e.obj.addhtml "<td class='widr_28p' style='word-break:break-all'><span class='rtdline elli'></span></td>"
'If rs.fields.count = 1 Then
                                        end if
                                    else
                                        if isnumeric(w) then
                                            e.obj.addhtml "<td align=right class='"& sty & "' style='width:" & w & "px;word-break:'><span class='rtdline'>" & val & "</span></td>"
'if isnumeric(w) then
                                        else
                                            If isRemind Then
                                                e.obj.addhtml "<td align=right class='"& sty & "' style='word-break:break-all'><span class='rtdline'>" & val & "</span></td>"
'If isRemind Then
                                            else
                                                e.obj.addhtml "<td align=right class='"& sty & "' style='word-break:break-all'><span class='rtdline'>" & val & "</span></td>"
'If isRemind Then
                    ...
                    wend
                    if i <=defrow then
                        for ii = i to defrow - 1
'if i <=defrow then
                            e.obj.addhtml "<tr class='r0'>"
                            e.obj.addhtml "<td colspan='" & rs.fields.count & "'>&nbsp;</td>"
                            e.obj.addhtml "</tr>"
                        next
                    end if
                else
                    If isRemind And reminder.num1=0 And reminder.remindCount>0 Then
                        e.obj.addhtml "<tr><td><div class='lvw_nulldata' style='line-height:95px;background:transparent;text-align:center;'>您设置的显示行数为0，无信息可显示</div></td></tr>"
'If isRemind And reminder.num1=0 And reminder.remindCount>0 Then
                    else
                        e.obj.addhtml "<tr><td><div class='lvw_nulldata' style='background-position:center center'>&nbsp;</div></td></tr>"
'If isRemind And reminder.num1=0 And reminder.remindCount>0 Then
                    end if
                end if
                e.obj.addhtml "</table>"
                ...
                dim num
                If isRemind Then
                    e.this.LinkUrl = reminder.moreLinkURL()
                    num = reminder.remindCount
                else
                    e.this.linkurl = getRightCardUrl(e.key, fw)
                    num=rs.nextrecordset.fields(0).value
                end if
                e.this.linkcount = num
                if err.number <> 0 and i>defrow then
                    e.this.linkcount = i
                end if
                err.clear
                on error goto 0
    ...
    Sub setListViewCol(lvw, k, r_name, r_value)
        ...
        For i = 1 To lvw.headers.count
            ...
            elseIf right(h.dbname,1) = "价" Then
                h.dbtype = "money"
            elseif right(h.dbname,1) = "额" Then
                h.dbtype = "money"
            elseif right(h.dbname,1) = "票" Then
                h.dbtype = "money"
            elseif right(h.dbname,1) = "款" Then
                h.dbtype = "money"
            elseif right(h.dbname,1) = "出" Then
                h.dbtype = "money"
            ...
            If k = "销售业绩每月对比" Then
                If instr(1,h.dbname,"_今日成果",1) > 0 Or instr(1,h.dbname,"_本月总计",1) > 0 Or instr(1,h.dbname,"_本月任务",1) > 0 Or instr(1,h.dbname,"_上月同期",1) > 0 Or instr(1,h.dbname,"_同期应完成",1) > 0 Or instr(1,h.dbname,"_同期比较",1) > 0 Then h.dbtype = "money"
            end if
    ...
    Sub App_sys_cardcloseitem
        Dim k
        k = app.gettext("key")
        Dim id , rs, rs1, i
        Set rs = server.CreateObject("adodb.recordset")
        id = app.getInt("id")
        rs.open "select * from home_maincards_us where uid=" & Info.User & " and title='" & Replace(k,"'","") & "'" , cn , 1, 3
        If rs.eof Then
            Set rs1 = cn.execute("select ID,cardClass,title,sql,sql2,colspan,maxspan,sort,qxlb,qxlblist,model,mustadmin,canadd,canset,canmore,canclose,visible,monthjs,defjs,gjjs,attrs,setJM,fw,defRows,canqt from home_maincards_def where title='" & Replace(k,"'","") & "'" )
            If rs1.eof = False Then
                rs.addnew
        ...
        rs.fields("visible").value = 0
        rs.update
        cn.execute "update setjm set intro='0' where ord=" & rs("setjm") & " and cateid=" & Info.user
        rs.close
        set rs = nothing
        Response.write "ok"
    end sub
    sub loadDriverPage
        dim cv  , item , data , i , vis
        set cv = new cardview
        cv.canDrag = false
        cv.asynModel = false
        cv.id = "driverlist"
        cv.cols = 3
        cv.itemheight = 125
        vis = app.attributes("仪表盘显示属性")
        if len(vis) = 0 then
            vis = split("1,1,1,1,1,1",",")
        else
            vis = split(vis & ",1,1,1,1,1,1",",")
        end if
        If Not app.power.existsModel(6000) And not app.power.existsModel(7000) Then
            vis(0) = "0" : vis(1) = "0" : vis(2) = "0"
        end if
        If Not app.power.existsModel(15000) Then
            vis(3) = "0" : vis(4) = "0" : vis(5) = "0"
        end if
        if   app.Attributes("lirun_jsfs")="3" then
            data=split("销售总额,回款总额,费用总额,成本总额,毛利总额,毛利率",",")
        else
            data = split("销售总额,回款总额,费用总额,采购总额,毛利总额,毛利率",",")
        end if
        for i = 0 to ubound(data)
            if vis(i) = 1 then
                set item = cv.add(data(i) , 1)
                item.canadd = false
                item.canmore = False
                If i=5 Then
                    item.cansetting = True
                else
                    item.cansetting = false
                end if
                item.flat = true
    ...
    function  createFlash(currnum, model, unit, htmlv)
        dim url  , html
        dim v
        if currnum*1 < - 40 then
'dim v
            v = - 40
'dim v
        else
        ...
        if v > 150 then  v = 150
        url = "../skin/" & info.skin & "/images/p0" & model & ".swf?num=" & v
        html = ""
        html = html & "<div style='height:100px;overflow:hidden;text-align:center'><OBJECT classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0' WIDTH='171' height='100'>"
        'html = ""
        ...
        html = html & "<EMBED src=""" & url & """ loop=true wmode='transparent' quality='high' swLiveConnect='FALSE'  HEIGHT='100' TYPE='application/x-shockwave-flash' PLUGINSPAGE='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash'></EMBED></OBJECT></div><div class='ybpbottom'>总额：" & htmlv & "&nbsp" & unit & "</div>"
        createFlash = html
    end function
    function  createCanvas(currnum, index,unit, htmlv)
        dim url  , html ,unitPanel , p , v
        unitPanel= unit
        p=""
        if unit = "%" then
            unitPanel = ""
        ...
        if v > 14.5 then  v = 14.5
        html="<div style='display:none'><v:image style='width:100px;height:20px' src='../skin/default/images/yi_gray.png' /><v:image style='width:100px;height:20px' src='../skin/default/images/yi_red.png' /><v:image style='width:100px;height:20px' src='../skin/default/images/yi_blue.png' /></div>"
        html =html&  "<div style='height:100px;overflow:hidden;text-align:center'><canvas class='instruPanel' v="""&v&""" unit="""&unitPanel&""" p="""&p&""" WIDTH='170' height='100' style=''></canvas><div class='IE7_8' v="""&v&""" unit="""&unitPanel&"""  p="""&p&""" style='overflow-y:hidden;position: relative; width: 170px;height:100px;background: #fff;margin: 0 auto;display:none'></div></div>"
        html=html&"<ajaxscript>setTimeout( function () {var canvas = $('canvas');var dom=canvas.eq("&index&");var vmlDiv=$('.IE7_8').eq("&index&");if(app.getIEVer()< 9){dom.css('display','none');vmlDiv.css('display','block');drawVmlIE(vmlDiv,vmlDiv.attr('v'), vmlDiv.attr('unit'),vmlDiv.attr('p'));}else{vmlDiv.css('display','none');dom.css('display','');animate(dom, dom.attr('unit'), dom.attr('v'),dom.attr('p'))} },100)</ajaxscript>"
        html=html&"<div class='ybpbottom'>总额：" & htmlv & "&nbsp" & unit & "</div>"
        createCanvas = html
    end function
    Function getW3(ByVal Wlist)
        Dim rs , r
        Wlist = Split(Replace(Wlist," ",""), "|")
        Set rs =  cn.execute("exec erp_comm_getW3 '" &  Wlist(0) & "','" &  Wlist(1) & "','" &  Wlist(2) &"'")
        while rs.eof = False
    ...
    Sub App_saveformula
        app.Attributes("lirun_jsfs")=app.getint("fs")
        Response.write 1
    end sub
    Sub App_getformula
        Response.write app.Attributes("lirun_jsfs")
    end sub
    Sub App_getGateInfo
        Dim gid, gnm , rs ,sex
        gid = app.getint("ord")
        Set rs = cn.execute("SELECT a.sex ,a.name, a.title, b.sort1, c.sort2  from gate a left join gate1 b on a.sorce = b.ord left join gate2 c on a.sorce2 = c.ord where a.ord= " & gid)
        If rs.eof = true Then  rs.close : Exit Sub
        sex=rs.fields("sex").value
        if IsNumeric(sex)=true then
            if sex=1 then
                sex="男"
            else
                sex="女"
        ...
        Response.write "" & vbcrlf & "<table style='width:310px;height:110px;margin-left:2px;padding:0px;' bordercolor='#b4b6c0' align='center' border='1'>" & vbcrlf & "<tr>" & vbcrlf & "    <td width='70px' align='right'>姓名：</td>" & vbcrlf & "      <td width='80px'>&nbsp;"
'sex="女"
        Response.write rs.fields("name").value
        Response.write "</td>" & vbcrlf & "        <td width='70px' align='right'>性别：</td>" & vbcrlf & "      <td width='80px'>&nbsp;"
        Response.write sex
        Response.write "</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "       <td align='right'>职务：</td>" & vbcrlf & "   <td colspan=3>&nbsp;"
        Response.write rs.fields("title").value
        Response.write "</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "       <td align='right'>部门：</td>" & vbcrlf & "   <td colspan=3>&nbsp;"
        Response.write rs.fields("sort1").value
        Response.write "</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "       <td align='right'>小组：</td>" & vbcrlf & "   <td colspan=3>&nbsp;"
        Response.write rs.fields("sort2").value
        Response.write "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
    end sub
    sub App_GetSupplierPriceList
        dim lvw ,id
        id = request.QueryString("sord")
        If isnumeric(id) = False Then id = app.base64.deurl(id)
        set lvw = new listview
        lvw.checkbox = false
        lvw.addlink = ""
        lvw.pagesize=5
        lvw.sql = "exec home_report_suppriceList " & info.user & "," &  id
        Response.write "<div class='newskin'>" & lvw.HTML & "</div>"
    end sub
    Sub lvw_defCallBack(cmd, l)
        Dim s, k, r_name, r_value, r_count
        Dim e_t1, e_t2, rs, attrs, i, ii
        If cmd <> "lvwHeaderExplan" Then
            Exit sub
        end if
        s = Split(l.dataattr, "$$$")
        ...
        e_t1 = s(1)
        e_t2 = s(2)
        If e_t1&""="" Or IsDate(e_t1&"")=False Then e_t1=Date()
        If e_t2&""="" Or IsDate(e_t2&"")=False Then e_t2=Date()
        r_count = 0
        set rs = cn.execute("select sql, colspan, id, attrs, defrows from home_maincards_us where title='" & k & "' and uid=" & Info.user)
        If rs.eof = True Then
            rs.close
            set rs = cn.execute("select sql, colspan, id, attrs, defrows from home_maincards_def where title='" & k & "'")
        end if
        if rs.eof = false then
            attrs = Split(Replace(Replace(Replace(rs.fields("attrs").value & "",";","="),",","="),"|","="),"=")
        end if
...
'If instr(attrs(i),"@")=1 and lcase(attrs(i))<>"@t1" and lcase(attrs(i)) <> "@t2" Then
        ...
        Call setListViewCol(l, k ,r_name,r_value)
    end sub
