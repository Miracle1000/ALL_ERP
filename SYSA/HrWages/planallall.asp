...
'dim sqlstr
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {      margin-top: 0px;}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton2 {height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "</head>"& vbcrlf & "<body  "
	if open_10_8=0 then
...
'if CDate(m1)>CDate(m2) then m1=m2
	...
	Response.write """>" & vbcrlf & "             </td>" & vbcrlf & "             </form>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "    <td colspan=""2"" align=""center""><div align=""center""><strong>员工编号</strong></div></td>" & vbcrlf & "       <td align=""center""><divalign=""center"">姓名</div></td>" & vbcrlf & "          <td align=""center""><div align=""center""><strong>应发总额</strong></div></td>" & vbcrlf & "         <td width=""13%"" height=""27"" align=""center""><div align=""center"">应扣总额</div></td>" & vbcrlf & "      <td width=""15%"" ><div align=""center""><strong>实发总额</strong></div></td>" & vbcrlf & "       <td width=""15%"" ><div align=""center"">所在部门</div></td>" & vbcrlf & "    <td width=""15%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	person_Result=list
	if W3<>"" then
		person_Result=person_Result+"and  wageslist.cateid in ("&W3&")"
'if W3<>"" then
	end if
	person_Result1=""
	if W2<>"" then
...
'if W1<>"" then
	...
	person_Result=replace(person_Result,"ord","wageslist.cateid")
	Str_Result2=""
	if m1<>"" And isdate(m1) then
		Str_Result2=Str_Result2+" and datediff(d,'"&m1&"',date2)>=0 "
'if m1<>"" And isdate(m1) then
	end if
	if m2<>"" And isdate(m2) then
		Str_Result2=Str_Result2+" and  datediff(d,'"&m2&"',date2)<=0 "
'if m2<>"" And isdate(m2) then
	end if
	If person_type&""<>"" Then
		If InStr(person_type,"0")=0 Then  person_Result1=person_Result1 &" and nowstatus in (2,4) "
		If InStr(person_type,"2")=0 Then  person_Result1=person_Result1 &" and nowstatus<>2 "
		If InStr(person_type,"4")=0 Then  person_Result1=person_Result1 &" and nowstatus<>4 "
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select distinct wageslist.cateid as ord,c.username as name,c.sorce,c.sorce2,c.userbh as ygid from wageslist left join (select b.userid , b.nowstatus,b.username,b.sorce,b.sorce2,b.userbh from gate_person a inner join hr_person b on a.ord=b.userid) c on c.userid=wageslist.cateid where 1=1and wages in (select id from wages where isnull(salaryClass,0)>0 and complete1=1 "& Str_Result2 &" ) "&person_Result1&person_Result&" order by wageslist.cateid asc"
	rs1.open sql1,conn,1,1
	if rs1.RecordCount<=0 then
		...
		do until rs1.eof
			...
			sort1=0
			set rs=server.CreateObject("adodb.recordset")
			sql="select money1,sort1,wages,intro,isNull(sortName,'删除') as sortName,isNull(updown,0) as updown from wageslist where del=1 and cateid="&rs1("ord")&" and wages in (select id from wages where complete1=1 and del=1 and isnull(salaryClass,0)>0 "&Str_Result2&") order by date7 desc"
			...
			if rs.eof then
				...
				do until rs.eof
					...
					wagesid =rs("wages")
					if rs("updown")=1 then
						money1=cdbl(money1)+cdbl(rs("money1"))
'if rs("updown")=1 then
					elseif rs("updown")=-1 then
'if rs("updown")=1 then
						money2=cdbl(money2)+cdbl(rs("money1"))
'if rs("updown")=1 then
					end if
					rs.movenext
				loop
			...
			Response.write getGateBH(UserID)
			Response.write "" & vbcrlf & "       </span></td>" & vbcrlf & "                <td align=""center"" class=""name"">"
			Response.write rs1("name")
			Response.write "</td>" & vbcrlf & "                <td align=""center"" class=""name""><div align=""right"">"
