...
'If isEmpty(m_wxPayKindId) Then
	...
	Sub onReportInit(rpt, lt)
		...
		Dim rs,CategoryList,StatusList,sTypeList,batchOptBtns,ret,ret2
		rpt.title = "数量变动记录表"
		If app.power.existsPower(109,10) Then
			rpt.canexcel = True
		else
			rpt.canexcel = False
		end if
		If app.power.existsPower(109,7) Then
			rpt.canPrint = True
		else
			rpt.canPrint = False
		end if
		...
		ret = app.getText("ret")
		ret2= app.getText("ret2")
		If ret = "" Then ret = Year(Date) & "-" & Month(Date) & "-01"
		ret2= app.getText("ret2")
		If ret2 = "" Then ret2 = Date()
		rpt.addfield "","dates","logDate", "", "", Split(ret&"|"&ret2,"|")
		...
		rpt.sql = "EXEC [ERP_GoodsNumLogList] "&Info.User&",&ReportSort,@logDate_0 ,@logDate_1,@category,@status,@sType,@keywords,"& validTime &",&pagesize,&pageindex,&excelmode"
	...
	Sub app_oncreate(lvw)
		...
		For i = 1 To 5
			...
			col.canSum = False
		next
		Set col = lvw.headers("日期")
		col.width = 150
		lvw.headers("操作类型").width = 100
		Set col = lvw.headers("商品名称")
		col.dbtype = "str"
		col.setlink "商品名称","logGoodsID", "creator" , 109, -15
		col.dbtype = "str"
		col.width = 250
		Set col = lvw.headers("单位")
		col.width = 80
		Set col = lvw.headers("关联单据")
		col.FormatText = "code:PowerContract(@value,@cells[""contractID""],@cells[""contractCateID""],5)"
		Set col = lvw.headers("商品属性")
		col.FormatText = "code:GetGoodsAttr(@cells[""logGoodsID""])"
		col.width = 250
		For i = 7 To 9
			Set col = lvw.headers(i)
			col.dbtype = "number"
		next
		Set col = lvw.headers("期初")
		col.canSum = False
		col.FormatText = "code:GetFirstNum(@cells[""logGoodsID""],@cells[""logID""])"
		Set col = lvw.headers("当前数量")
		col.canSum = False
		col.FormatText = "code:GetEndNum(@cells[""logGoodsID""],@cells[""logID""],@cells[""变动""])"
		beginCol = 11
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		lvw.checkbox = False
		totalCol = lvw.headers.count
		lvw.headers("日期").width = 110
		Set col = lvw.headers("操作类型")
		col.width = 90
		col.dbtype = "str"
		col.excelAlign = "center"
		Set col = lvw.headers("商品编号")
		col.width = 150
		col.dbtype = "str"
		Set col = lvw.headers("单位")
		col.width = 50
		...
		col.excelAlign = "center"
		Set col = lvw.headers("商品名称")
		col.Formula = "PowerCol(@value,@cells[""logGoodsID""],@cells[""creator""],109)"
		col.width = 230
		col.dbtype = "str"
		Set col = lvw.headers("商品属性")
		col.Formula = "GetGoodsAttr(@cells[""logGoodsID""])"
		col.dbtype = "str"
		col.width = 180
		Set col = lvw.headers("关联单据")
		col.Formula = "PowerCol(@value,@cells[""contractID""],@cells[""contractCateID""],5)"
		col.dbtype = "str"
		Set col = lvw.headers("期初")
		col.canSum = False
		col.Formula = "GetFirstNum(@cells[""logGoodsID""],@cells[""logID""])"
		Set col = lvw.headers("当前数量")
		col.canSum = False
		col.Formula = "GetEndNum(@cells[""logGoodsID""],@cells[""logID""],@cells[""变动""])"
		col.width = 110
		For i = 7 To 9
			Set col = lvw.headers(i)
			col.dbtype = "number"
			col.width = 110
		next
		For i = 11 To totalCol
			Set col = lvw.headers(i)
			col.display = "none"
		next
	end sub
	Function PowerContract(text,ID,User,qx)
		Dim str,share,IsShare,rs
		IsShare = False
		If qx = 5 Then
			Set rs = cn.Execute("SELECT TOP 1 ISNULL(share,'-222') share FROM contract WHERE ord = "&ID&" ")
'If qx = 5 Then
			If Not rs.Eof Then
				share = rs("share")
		...
		If app.power.existsPowerIntro(qx,1,User) Or IsShare Then
			If app.power.existsPowerIntro(qx,14,User) Then
				str = "<a href='javascript:void(0)' onClick=""javascript:window.open('" & app.virpath & "../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord=" & app.base64.pwurl(ID) & "','newwin','width=1100,height=500,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & text & "</a>"
			else
				str = text
			end if
		else
	...
	Function PowerCol(text,ID,User,qx)
		Dim str,share,IsShare,rs,tableName
		IsShare = False
		If qx = 5 Then
			tableName = "contract"
			Set rs = cn.Execute("SELECT TOP 1 ISNULL(share,'-222') share FROM "& tableName &" WHERE ord = "&ID&" ")
			tableName = "contract"
			If Not rs.Eof Then
				share = rs("share")
				If InStr(1,","&share&",", ","&info.user&",",1) > 0 Or share = "1" Then
	...
	Function GetOptionCol(gID,AddUser)
		...
		If app.power.existsPowerIntro(109,14,AddUser) Then
			str = str&"<input type='button' value='详情' class='oldbutton2' onclick=""javascript:window.OpenNoUrl('content.asp?ID="& curID &"','goodsContent','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" />&nbsp;"
'If app.power.existsPowerIntro(109,14,AddUser) Then  '
		end if
...
'Function FormatNum(num)
	...
	Function GetGoodsPic(gID)
		...
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
			temp = "<img src="""& rs(0) &""" width=""45"" height=""45"">"
		else
			temp = ""
		end if
		rs.close
	...
	Function GetFirstNum(logGoodsID,logID)
		Dim temp,rs,sql
		sql =       "SELECT ISNULL(SUM(num1),0) FROM Shop_StorageAppendLog " &_
		"WHERE goodsId = "& logGoodsID &" AND id < "& logID &" " &_
		"GROUP BY goodsId"
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
			temp = rs(0)
		...
		rs.close
		set rs = nothing
		GetFirstNum = FormatNum(temp)
	end function
	Function GetEndNum(logGoodsID,logID,changeNum)
		Dim temp,rs,sql
		If IsNumeric(changeNum)=False Then changeNum = 0
		temp = CDbl(GetFirstNum(logGoodsID,logID)) + CDbl(changeNum)
'If IsNumeric(changeNum)=False Then changeNum = 0
		GetEndNum = FormatNum(temp)
	end function
