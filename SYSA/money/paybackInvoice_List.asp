...
'Private Sub Class_Initialize
	...
	Sub App_doAbolish
		Dim ord : ord=request("id")
		cn.execute("update paybackinvoice set isinvoiced=3 where id="&ord)
		Response.write "1"
		call db_close : Response.end()
	end sub
	Sub App_doDel
		Dim ord : ord=request("id")
		if cn.execute("select 1 from paybackinvoice where id="& ord & " and isinvoiced=0").eof=false then
			cn.execute("update paybackinvoice set del=2 , delcate="& Info.User &",deldate='"&now()&"' where id="&ord)
			cn.execute("update paybackinvoice_list set del=2 where paybackinvoice="&ord)
		end if
		Response.write "1"
		call db_close : Response.end()
	end sub
	Sub App_doDelAll
		Dim ordlist, oSQL, pwdel , rs , Scplist
		ordlist = app.GetText("ordlist")
		If Len(ordlist)>0 Then
			pwdel = app.power.GetPowerIntro(7001,3)
			If cn.execute("select count(1) from paybackinvoice where isinvoiced in (0) and id in ( "& ordlist &" ) and ( '" & pwdel & "'='' or charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')>0 )")(0)>0 Then
				pwdel = app.power.GetPowerIntro(7001,3)
				Set rs = cn.execute("select id from paybackinvoice where del=1 and id in ( "& ordlist &" ) and (isinvoiced <>0 or ('" & pwdel & "'<>'' and ((charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')=0 and isnull(cateid,0)<>0) or isnull(cateid,0)=0) ) )")
				pwdel = app.power.GetPowerIntro(7001,3)
				While rs.eof = False
					If Len(Scplist)>0 Then Scplist = Scplist &","
					Scplist = Scplist & rs("id")
					rs.movenext
				wend
				rs.close
				session("Scplist")=Scplist
				oSQL = "update paybackinvoice set del=2 , delcate="& Info.User &",deldate='"&now()&"' Where isinvoiced =0 and id in ( "& ordlist &" ) and ( '" & pwdel & "'='' or charindex(','+ cast(cateid as varchar(10)) +',',',"& pwdel &",')>0 )"
				session("Scplist")=Scplist
				cn.Execute(oSQL)
				oSQL = "update paybackinvoice_list set del=2 where paybackinvoice in ("&ordlist&")"
				If Len(Scplist)>0 Then oSQL = oSQL & " and paybackinvoice not in ("&Scplist &")"
				cn.execute(oSQL)
				Response.write(1)
			else
				session("Scplist")=Replace(ordlist," ","")
				Response.write(0)
			end if
		else
			Response.write(0)
	...
	Dim Scplist, khname, khid
	Sub onReportInit(rpt, lt)
		Dim dat, rs, zdy,SC_level,i ,company0rd,contractOrd , isinvoiced,zt ,dateStr ,dateType,whereSql
		isinvoiced = ""
		dateType = 1
		dateStr = ","
		Dim A  :A = request("A")
		If A<>"" Then
			If A="4" Then
				ret= request("ret")
				ret2= request("ret2")
				if len(ret)>0 or len(ret2)>0 then
					if  len(ret)>0 then retStr = Year(ret)&"-"& Right("0" & Month(ret), 2)&"-"&Right("0" & Day(ret), 2)
'if len(ret)>0 or len(ret2)>0 then
					if  len(ret2)>0 then ret2Str = Year(ret2)&"-"& Right("0" & Month(ret2), 2)&"-"&Right("0" & Day(ret2), 2)
'if len(ret)>0 or len(ret2)>0 then
					dateStr = retStr&","&ret2Str
					B = request("B")
					if B="1" then
						Dim retStr2ForWhere
						if  len(ret2)>0 then retStr2ForWhere =dateadd("d",1,ret2)' Year(ret2)&"-"& Right("0" & Month(ret2)+1, 2)&"-01"
'Dim retStr2ForWhere
						whereSql=" and (p.InvoiceDate>=''"&retStr2ForWhere&"'' or p.InvoiceDate is null) "
			...
			ElseIf A = "5" Then
				isinvoiced = "1"
				zt = "1"
				invdate1 = request("invdate1")
				invdate2 = request("invdate2")
				if len(invdate1)>0 then invdate1Str = Year(invdate1)&"-"& Right("0" & Month(invdate1), 2)&"-"&Right("0" & Day(invdate1), 2)
				'invdate2 = request("invdate2")
				if len(invdate2)>0 then invdate2Str = Year(invdate2)&"-"& Right("0" & Month(invdate2), 2)&"-"&Right("0" & Day(invdate2), 2)
				'invdate2 = request("invdate2")
				dateStr = invdate1Str&","&invdate2Str
				dateType = 2
			elseif A="6" then
				zt = "0,1,2,4,5,11"
				ret= request("ret")
			...
			dateStr =  dateadd("d",1,dateadd("yyyy",-1,date))& "," & date
			dateStr = retStr&","&ret2Str
		end if
		Dim remind,helper,insertSql
		remind = app.getInt("remind")
		If remind > 0 Then
			Set helper = CreateReminderHelper(cn,remind,0)
			insertSql =   helper.listSQL("ids")
			dateStr = "," & date
		end if
		contractthOrd=request("contractthOrd")
		if request("searchkey")&""<>"" then
			v="1"
			ThNo=request("searchTxt")
		end if
		khname=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=1 ")(0)
		If khname="" Then khname="客户名称"
		khid=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=3 ")(0)
		If khid="" Then khid="客户编号"
		company0rd = app.base64.deurl(request("company"))
		If company0rd&"" = "0" Then company0rd = ""
		T2 = request("T2")
		if T2&"" = "1" then dateStr=","
		contractOrd = app.base64.deurl(request("contract"))
		If contractOrd&"" = "0" Then contractOrd = ""
		Call app.addDefaultScript() '加载本文件关联JS ="../skin/default/js/my_paybackInvoice_List.js
		dat = Split(dateStr, ",")'year(now) & "-" & month(now) & "-1," & date
'Call app.addDefaultScript() '加载本文件关联JS ="../skin/default/js/my_paybackInvoice_List.js
		if app.GetText("type")="2" then   dat= split( ""& "," & "" , ",")
		rpt.title = "开票计划列表"
		rpt.showtoppagesize = False
		rpt.pagesize = 10
		rpt.firstSpace = 200
		rpt.addfield "","hidden","contractthOrd","","",contractthOrd
		rpt.addfield "","hidden","companyord","","",company0rd
		rpt.addfield "","hidden","contractord","","",contractOrd
		rpt.addfield "","hidden","W1","","",request("W1")
		rpt.addfield "","hidden","W2","","",request("W2")
		rpt.addfield "","hidden","W3","","",request("W3")
		rpt.addfield "","hidden","zt","","",zt
		rpt.addfield "&nbsp;","select","dateType", "实开日期" & Chr(1) & "2" ,"",dateType
		rpt.SetNullMsg 7,"计划日期=1"
		rpt.addfield "","dates","date1","","", dat
		Dim rptindex,cancelTxt
		rptindex=8
		cancelTxt="已申请已作废"
		If app.power.existsModel(23004)=false  then
			cancelTxt="已申请已废止"
			rptindex=9
			rpt.addfield "&nbsp;","select","invoicemode", "明细开票" & Chr(1) & "2" & Chr(2) & "汇总开票" & Chr(1) & "1" ,"",""
			rpt.SetNullMsg rptindex,"开票类型"
		end if
		rpt.addfield "&nbsp;","select","isinvoiced", "未申请" & Chr(1) & "0" & Chr(2) & "已申请未开票" & Chr(1) & "11"& Chr(2) & "已申请已开票" & Chr(1) & "1"& Chr(2) & cancelTxt & Chr(1) & "3","",isinvoiced
		rpt.SetNullMsg rptindex+1,"开票申请状态"
		rpt.addfield "&nbsp;","select","searchType", khid & Chr(1) & "2" & Chr(2) & "单据主题" & Chr(1) & "3" & Chr(2) & "单据编号" & Chr(1) & "4" & Chr(2) & "开票计划编号" & Chr(1) & "10" & Chr(2) & "销售人员" & Chr(1) & "8" & Chr(2) &"添加人员" & Chr(1) & "9" & Chr(2) & "票据单号" & Chr(1) & "6" & Chr(2) & "开票备注" & Chr(1) & "7","",""&v&""
		rpt.SetNullMsg rptindex+2,khname
',"&v&"
		rpt.addfield "","text","searckKey", "", "", ""&ThNo&""
		rpt.aSearch.addField "销售人员", "gates", "g_xsry",""
		rpt.aSearch.addField "开票申请状态","checks", "zt","select 0 as value, '未申请' as name union all select 11, '已申请未开票'  union all select 1, '已申请已开票'  union all select 3, '" & cancelTxt & "' "
		If app.power.existsModel(23004)=false  then
			rpt.aSearch.addField "开票类型","checks", "md","select 2 as value, '明细开票' as name union all select 1, '汇总开票' "
		end if
		rpt.aSearch.addField khname&"","selectys", "name",""
		rpt.aSearch.addField khid&"","selectys", "khid",""
		rpt.aSearch.addField "单据主题","selectys", "title",""
		rpt.aSearch.addField "单据编号","selectys", "bh",""
		rpt.aSearch.addField "开票计划编号","selectys", "invoicebh",""
		rpt.aSearch.addField "票据单号","selectys", "pjdh",""
		Set rs = cn.execute("select id,title,name,sort,gl from zdy where sort1=5 and set_open=1 and js=1 order by gate1 asc")
		If rs.eof = False Then
			While rs.eof = False
				If rs("sort")="2" Then
					rpt.aSearch.addField rs("title")&"","selectys", rs("name")&"",""
				else
					rpt.aSearch.addField rs("title")&"","checks", rs("name")&"","select sort1,ord from sortonehy where gate2="&rs("gl")&" order by gate1 desc"
				end if
				rs.movenext
			wend
		end if
		...
		If rs.eof= False Then setbz=rs(0).value
		rs.close
		If setbz=0 Then bzsql=" where id=14 "
		rpt.aSearch.addField "计划日期", "dates", "plandate",""
		rpt.aSearch.addField "实开日期", "dates", "invoicedate",""
		rpt.aSearch.addfield "币种","select","bz","select sort1,id from sortbz "&bzsql&" order by gate1 desc "
		rpt.aSearch.addfield "开票总额","moneys","money1",""
		rpt.aSearch.addKzZdyFields 96
		Dim invoiceAbutment
		invoiceAbutment=0
		if app.power.existsModel(23004) then invoiceAbutment=1
		rpt.sql = " exec erp_paybackInvoice_List "& Info.User&",'"& Replace(insertSql,"'","''") &"',@companyord,@contractord,@W1,@W2,@W3,@invoicedate1,@invoicedate2,@g_xsry,@dateType,@date1_0, @date1_1,@plandate_0,@plandate_1,@invoicedate_0,@invoicedate_1,@bz,@isinvoiced, @zt,@invoicemode,@md,@searchType, @searckKey,@name_0,@name_1,@khid_0,@khid_1,@title_0,@title_1,@bh_0,@bh_1,@invoicebh_0,@invoicebh_1,@pjdh_0,@pjdh_1,@zdy1_0,@zdy1_1,@zdy2_0,@zdy2_1,@zdy3_0,@zdy3_1,@zdy4_0,@zdy4_1,@zdy5,@zdy6,@money1_0,@money1_1,@fields_extra,&SearchMode,&excelmode,&ReportSort, &pagesize, &pageindex,@remind,'','','','','','',@contractthOrd ,'"&whereSql&"'," & invoiceAbutment
		rpt.addsort "计划日期", "date1"
		rpt.addsort "实开日期", "invoicedate"
		rpt.addsort "开票总额", "money1"
		rpt.addsort "开票申请状态", "isinvoiced"
		rpt.addsort "客户名称", "name"
		rpt.canexcel = app.power.existsPower(7001,10)
		rpt.canprint = app.power.existsPower(7001,7)
		rpt.onlyadSearchModel = True
		if app.power.existsPower2(7001,13) And app.power.existsPower2(7001,21) then
			rpt.beforeBtnsHTML  = "<button class='oldbutton3' type='button' onclick=""javascript:window.open('begin_invoice.asp','new2win','width=' + 1200 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')"">期初开票</button>"
'if app.power.existsPower2(7001,13) And app.power.existsPower2(7001,21) then
		end if
	end sub
	dim open_7001_2, intro_7001_2, open_7001_3, intro_7001_3, open_7001_14,intro_7001_14, open_7001_16, intro_7001_16
	dim open_41_1,intro_41_1, open_41_14,intro_41_14
	dim open_7_1,open_7_14,intro_7_1, intro_7_14
	dim intro_1_1,intro_1_14,intro_5_1,intro_5_14,intro_26_1,intro_26_14
	dim power_7001_3, power_7001_6,power_7001_13,intro_7001_13
	Sub App_OnListviewExcel(ByRef lvw)
		...
		Set col = lvw.headers("bh")
		col.title = "开票计划编号"
		col.width = 100
		Set col = lvw.headers("name")
		col.title = khname
		col.dbtype = "str"
		col.width = 130
		col.align = "left"
		col.Formula = "GetTitleHtml(@cells[""id""],@value,@cells[""company""],@cells[""cateid_kh""],rs(""share""),rs(""sort3""),"""","""")"
		col.canSum=False
		Set col = lvw.headers("khid")
		col.title = khid
		col.width = 120
		col.Formula = "PowerCol(@value,@cells[""company""],@cells[""cateid_kh""],1)"
		col.canSum=False
		Set col = lvw.headers("title")
		col.title="单据主题"
		col.width = 120
		col.dbtype = "str"
		col.align="left"
		col.Formula = "PowerCol2(@value,@cells[""fromID""],@cells[""fromType""])"
		col.canSum=False
		Set col = lvw.headers("date1")
		col.dbtype = "str"
		col.title ="计划日期"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("invoicedate")
		col.title="实开日期"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("sort1")
		col.title="票据类型"
		col.dbtype = "str"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("invoiceNum")
		col.title="票据单号"
		col.dbtype = "str"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("bzname")
		col.title = "币种"
		col.width = 60
		col.dbType = "str"
		col.excelAlign = "center"
		col.canSum=False
		Set col =lvw.headers("money1")
		col.title="开票总额"
		col.dbtype = "money"
		col.width = 100
		col.canSum=true
		Set col = lvw.headers("status")
		col.title="开票申请状态"
		col.width = 120
		col.canSum=False
		Set col = lvw.headers("cateidName")
		col.title="销售人员"
		col.dbtype = "str"
		col.width = 100
		col.excelAlign = "center"
		col.canSum=False
		Set col = lvw.headers("InvoiceName")
		col.title="开票人员"
		col.dbtype = "str"
		col.excelAlign = "center"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("intro")
		col.title="备注"
		col.dbtype = "str"
		col.width = 100
		col.canSum=False
		Set col = lvw.headers("company")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("moneycolor")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("fromType")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("cateid")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("cateid_kh")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("isinvoiced")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("fromid")
		col.width = 0
		col.display = "none"
		col.canSum=False
		Set col = lvw.headers("bzcode")
		col.display = "none"
		Set col = lvw.headers("url")
		col.display = "none"
		Set col = lvw.headers("share")
		col.display = "none"
		Set col = lvw.headers("sort3")
		col.display = "none"
	end sub
	sub app_oncreate(lvw)
		Dim h, i, col, Czplist
		Czplist= session("Czplist")
		khname=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=1 ")(0)
		If khname="" Then khname="客户名称"
		khid=cn.execute("select (case when isnull(name,'')='' then oldname else name end ) as name from setfields where gate1=3 ")(0)
		If khid="" Then khid="客户编号"
		...
		col.width = 130
		col.dbtype = "str"
		col.formattext = "code:GetTitleHtml(@cells[""id""],""@value"",@cells[""company""],@cells[""cateid_kh""],rs(""share""),rs(""sort3""),"""&Czplist&""",rs(""khid""))"
		col.canSum=False
		lvw.headers("khid").display = "none"
		Set col = lvw.headers("title")
		col.title="单据主题"
		col.width = 120
		col.dbtype = "str"
		col.formattext = "code:GetNameHtml(@cells[""fromid""],""@value"",""@cells[""fromType""]"")"
		col.canSum=False
		Set col = lvw.headers("bh")
		col.title="开票计划编号"
		col.width = 100
		...
		col.title="票据类型"
		col.width = 100
		lvw.headers("date1").display = "none"
		Set col = lvw.headers("bzname")
		col.width = 0
		col.display = "none"
		col.canSum=False
		...
		col.title="开票总额"
		col.dbtype = "money"
		col.ContentStyle="color:@cells[""moneycolor""]"
		col.width = 100
		col.formattext = "code:showCurrMoney(""@value"",""@cells[""moneycolor""]"")"
		col.bz = "bzcode"
		col.canSum=true
		Set col = lvw.headers("invoicedate")
		col.title="日期"
		col.width = 100
		col.align = "left"
		col.formattext = "<div style='margin-left:10px'>计划:@cells[""date1""]<br>实开:@value</div>"
		col.align = "left"
		Set col = lvw.headers("status")
		col.title="开票申请状态"
		col.width = 120
		col.formattext = "code:GetStatus(@cells[""id""],""@value"",@cells[""cateid""],""@cells[""fromType""]"")"
		Set col = lvw.headers("invoiceNum")
		col.title="票据单号"
		col.width = 100
		col.canSum=False
		...
		col.canSum=False
		col.align = "left"
		col.formattext = "<div style='margin-left:10px'>销售:@value<br>开票:@cells[""InvoiceName""]</div>"
		col.align = "left"
		lvw.headers("InvoiceName").display = "none"
		Set col = lvw.headers("company")
		col.width = 180
		col.title = "操作"
		col.formattext = "code:OperateButtons(@cells[""id""],@cells[""cateid""],@cells[""isinvoiced""])"
		col.canSum=False
		Set col = lvw.headers("fromType")
		col.width = 0
		col.display = "none"
		...
		dim btmStr
		btmStr = ""
		intro_7001_13 = app.power.GetPowerIntro(7001,13)
		If Len(power_7001_3) = 0 Then power_7001_3 = app.power.existsPower(7001,3)
		If Len(power_7001_6) = 0 Then power_7001_6 = app.power.existsPower(7001,6)
		If Len(power_7001_13) = 0 Then power_7001_13 = app.power.existsPower(7001,13)
		if power_7001_3 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' type='button' onclick='OperateDeleteAll()'>批量删除</button>"
		if power_7001_6 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' type='button' onclick='OperateHandleAll()'>批量指派</button>"
		if power_7001_13 = true Then btmStr = btmStr& "   &nbsp;<button class='oldbutton3' type='button' onclick='OperateInvoiceAll()'>合并申请</button>"
		btmStr = btmStr& "<a>"
		lvw.addlink = "</a><input type='checkbox' onclick='selectall(this)'>全选"& btmStr
		session("Czplist")=""
	end sub
	function showCurrMoney(money1,color)
		showCurrMoney = "<div style='color:"& color &"'>" & replace(money1,"-","") &"</div>"
'function showCurrMoney(money1,color)
	end function
	Function GetNameHtml(fromid, name , fromType)
		Dim namestr ,rs,share,IsShare
		namestr = name
		If fromType ="CONTRACT" Then
			Set rs = cn.execute("select isnull(cateid,0) as cateid,ISNULL(share,'-222') share from contract where ord="&fromid)
'If fromType ="CONTRACT" Then
			If rs.eof = False Then
				share = rs("share")
				If InStr(1,","&share&",", ","&info.user&",",1) > 0 Or share = "1" Then
					IsShare = True
				end if
				If isEmpty(intro_5_14) Then  intro_5_14 = app.power.GetPowerIntro(5,14)
				If isEmpty(intro_5_1) Then   intro_5_1 = app.power.GetPowerIntro(5,1)
				if intro_5_1="" or (instr(","& intro_5_1 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0") Or IsShare then
					if intro_5_14="" or (instr(","& intro_5_14 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0")  then
						namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?ord=" & app.base64.pwurl(fromid) & "&view=details','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"&name &"</a>"
					else
						namestr = name
					end if
				else
			...
			rs.close
		ElseIf fromType ="PREBACK" Then
			Set rs = cn.execute("select isnull(cateid,0) as cateid from bankin2 where id="&fromid)
			If rs.eof = False Then
				If isEmpty(intro_7_14) Then  intro_7_14 = app.power.GetPowerIntro(7,14)
				If isEmpty(intro_7_1) Then   intro_7_1 = app.power.GetPowerIntro(7,1)
				if intro_7_1="" or (instr(","& intro_7_1 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0") then
					if intro_7_14="" or (instr(","& intro_7_14 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0")  then
						namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../money/contentyfk.asp?ord=" & app.base64.pwurl(fromid) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
					else
						namestr = name
					end if
				else
			...
			rs.close
		ElseIf fromType ="ContractTH" Then
			Set rs = cn.execute("select isnull(cateid,0) as cateid from contractth where ord="&fromid)
			If rs.eof = False Then
				If isEmpty(intro_41_14) Then    intro_41_14 = app.power.GetPowerIntro(41,14)
				If isEmpty(intro_41_1) Then   intro_41_1 = app.power.GetPowerIntro(41,1)
				if intro_41_1="" or (instr(","& intro_41_1 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0") then
					if intro_41_14="" or (instr(","& intro_41_14 &"," , ","& rs("cateid") &",")>0 And rs("cateid")<>"0")  then
						namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../contractth/content.asp?ord=" & app.base64.pwurl(fromid) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
					else
						namestr = name
					end if
				else
		...
		GetNameHtml = namestr '& ("["&fromid&","&name&","&fromType&"]")
	end function
	function GetTitleHtml(invoiceid,title,ord,cateid_kh,share,sort3,Czplist , khid)
		...
		If sort3="2" Then
			if intro_26_1="" or (instr(","& intro_26_1 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
				if intro_26_14="" or (instr(","& intro_26_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
...
'if intro_26_14="" or (instr(","& intro_26_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
				...
				if len(khid)>0 then TitleStr = TitleStr &"<br>"& khid
			else
				TitleStr = ""
			end if
		else
			if intro_1_1="" or (instr(","& intro_1_1 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0) Or InStr(1,","&share&",", ","&Info.user&",",1) > 0 Or share = "1" then
				if intro_1_14="" or (instr(","& intro_1_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
...
'if intro_1_14="" or (instr(","& intro_1_14 &"," , ","& cateid_kh &",")>0 And cateid_kh<>0)  then
		...
		If Len(Scplist)=0 Then
			Scplist= session("Scplist")
			session("Scplist") = ""
		end if
		If Len(Czplist)>0 Then
			If InStr(","&Czplist&",",","& invoiceid &",") Then TitleStr = TitleStr & "&nbsp;<span style='color:red'>不允许指派</span>"
		ElseIf Len(Scplist)>0 Then
			If InStr(","&Scplist&",",","& invoiceid &",") Then TitleStr = TitleStr & "&nbsp;<span style='color:red'>不允许删除</span>"
		end if
		GetTitleHtml = TitleStr
	end function
	function GetStatus(ord,statusStr,cateid,fromType)
		If isEmpty(intro_7001_13) Then       intro_7001_13 = app.power.GetPowerIntro(7001,13)
		If fromType<>"ContractTH" and statusStr = "未申请" And (intro_7001_13="" Or (instr(","& intro_7001_13 &"," , ","& cateid &",")>0 And cateid<>0) ) Then
			if cn.execute("select 1 from paybackinvoice_list where paybackinvoice="& ord &" and isnull(contractlist,0)<=0 " ).eof=true then
				statusStr = statusStr & "<img class='resetElementHidden' src='../images/jiantou.gif'><img class='resetElementShow'  style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png'><a href='javascript:void(0)' onclick=""javascript:window.open('../../SYSN/view/finan/InvoiceManage/MakeOutInvoice/InvoiceApply.ashx?ids=" & app.base64.pwurl(ord) & "','invoice','width=' + 960 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">申请开票</a>"
			elseif cn.execute("select 1 from paybackinvoice_list where paybackinvoice="& ord &" and isnull(contractlist,0)<0" ).eof=false then
				statusStr = statusStr & "<font color='red'>(含退货)</font>"
			else
				statusStr = statusStr & "<font color='red'>(含整单优惠)</font>"
			end if
		end if
		GetStatus = statusStr
	end function
	function OperateButtons(ord,cateid,isinvoiced)
		dim actStr
		if ord&""<>"" then
			If isEmpty(intro_7001_14) Then intro_7001_14 = app.power.GetPowerIntro(7001,14)
			actStr = actStr &"<div style='word-wrap: break-word; word-break: normal;width:100%;'>"
'If isEmpty(intro_7001_14) Then intro_7001_14 = app.power.GetPowerIntro(7001,14)'
			if intro_7001_14="" or (instr(","& intro_7001_14 &"," , ","& cateid &",")>0 And cateid<>"0" ) then
				actStr = actStr & "<button class=""oldbutton2"" type=button onclick=""javascript:window.open('../../SYSN/view/finan/InvoiceManage/InvoicePlan/InvoicePlansDetail.ashx?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 960 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">详情</button>"
			end if
			If isinvoiced="0" Then
				If isEmpty(intro_7001_2) Then        intro_7001_2 = app.power.GetPowerIntro(7001,2)
				if (intro_7001_2="" or (instr(","& intro_7001_2 &"," , ","& cateid &",")>0 and cateid<>"0")) And isinvoiced="0" then
					actStr = actStr & "<button class='oldbutton2 buttons' type=button onclick=""javascript:window.open('../../SYSN/view/finan/InvoiceManage/InvoicePlan/InvoicePlansAdd.ashx?ord=" & app.base64.pwurl(ord) & "','xg','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150');"">修改</button>"
				end if
				If isEmpty(intro_7001_3) Then        intro_7001_3 = app.power.GetPowerIntro(7001,3)
				if intro_7001_3="" or (instr(","& intro_7001_3 &"," , ","& cateid &",")>0 and cateid<>"0") Then
					actStr = actStr & "<button class=""oldbutton2"" type=button onclick=""javascript:OperateDelete("& ord &")"">删除</button>"
				end if
			end if
			actStr = actStr &"</div>"
		end if
		OperateButtons = actStr
	end function
	Function PowerCol(text,ID,User,qx)
		Dim str,share,IsShare,rs
		IsShare = False
		If qx = 1 Then
			Set rs = cn.Execute("SELECT TOP 1 ISNULL(share,'-222') share FROM tel WHERE ord = "&ID&" ")
'If qx = 1 Then
			If Not rs.Eof Then
				share = rs("share")
				If InStr(1,","&share&",", ","&info.user&",",1) > 0 Or share = "1" Then
		...
		If app.power.existsPowerIntro(qx,1,User) Or IsShare Then
			str = text
		else
			str = ""
		end if
	...
	Function PowerCol2(text,fromID,fromType)
		Dim str,share,IsShare,rs,qx,user
		IsShare = False
		qx = 0
		If fromType = "CONTRACT" Then qx = 5
		If fromType = "PREBACK" Then qx = 7
		If qx = 5 Then
			Set rs = cn.Execute("SELECT TOP 1 ISNULL(share,'-222') share,ISNULL(cateid,0) cateid FROM contract WHERE ord = "&fromID&" ")
'If qx = 5 Then
			If Not rs.Eof Then
				share = rs("share")
				user = rs("cateid")
		...
		If app.power.existsPowerIntro(qx,1,User) Or IsShare Or qx = 0 Then
			str = text
		else
			str = ""
		end if
