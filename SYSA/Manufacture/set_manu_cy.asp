...
'If Right(s,1) = "," Then
	...
	Function cw(ByVal txt) : Response.write txt : End Function
	Sub Bill_OnInit(ByVal bill, ByVal t)
		If t = 0 And bill.id=0 And app.gettext("ord")="" Then
			Dim defid : defid = 3
			Dim rs : Set rs = cn.execute("select [id] from M_OrderSettings_flows where used=1")
			If rs.eof = False Then defid = rs(0).value
			rs.close
			Response.redirect "?ord=" & defid
		end if
		bill.maxspan = 2
		bill.colwidth(0) = 9
		bill.colwidth(1) = 10
		bill.colwidth(2) = 11
		bill.colwidth(3) = 70
		bill.sql = "select [title], [id], [used], [mustnodes],[seletedNodes],[canyldj],[mustllhb],[dismorehb] from M_OrderSettings_flows where id=@ID"
		bill.title = "生产流程设置"
		bill.groups(0).title = "生产流程设置（固定）"
		bill.fields.addHtml ("" , "configcard1", 1,4).valign = "top"
		Dim fd : Set fd = bill.fields.add("流程名称", "title")
		fd.edit = False
		Set fd = bill.fields.add("是否启用", "used")
		fd.dbtype = "int"
		fd.uitype = "boolbox"
		bill.fields.addHtml "流程选项" , "configcard2", 1,1
		bill.fields.addHtml "策略选项" , "configcard3", 1,1
	end sub
	Sub configcard1(ByVal fd)
		Response.write "<style> div.tvw_n_ico {padding:9px 2px 0px 2px;} </style>"
		Dim proid : proid = fd.bill.id
		Dim nd , nd1, i, rs
		Dim tvw : Set tvw = New treeview
		tvw.itemheight = 36
		tvw.indent = 26
		...
		nd.ico = "@img/ico16/fd_open.gif"
		nd.ico2 = "@img/ico16/fd_close.gif"
		Set rs = cn.execute("select  [title], [id], [used] from M_OrderSettings_flows ")
		While rs.eof = False
			Set nd1 = nd.nodes.add
			nd1.Text  = rs(0).value
			nd1.ico = "@img/ico16/set.gif"
			nd1.value = rs(1).value
			If rs(2).value Then
				nd1.Text = nd1.Text & "(√)"
			end if
			If  rs(1).value*1 = proid*1 Then
				nd1.selected = True
			else
				nd1.Text = "<a href='?ord=" & rs(1).value & "'>" & nd1.Text & "</a>"
			end if
			rs.movenext
		wend
		rs.close
	...
	Sub configcard2(ByVal fd)
		Dim rs ,ords, mustnodes, seletedNodes, nm, bill, mcode
		Dim i : i = 0
		Set bill = fd.bill
		mustnodes = bill.Data.fields("mustnodes")
		seletedNodes = bill.Data.fields("seletedNodes")
		Response.write "<div style='padding-left:15px;line-height:30px'>"
		seletedNodes = bill.Data.fields("seletedNodes")
		ords = "select 1 as oid, 0 as s " &_
		"union all select 3 ,1" &_
		"union all select 2 ,2" &_
		"union all select 25 ,3" &_
		"union all select 30 ,4" &_
		"union all select 4 ,5" &_
		"union all select 8 ,6" &_
		"union all select 12 ,6" &_
		"union all select 13 ,6.1" &_
		"union all select 14 ,6.2" &_
		"union all select 15 ,6.3" &_
		"union all select 28 ,6.4" &_
		"union all select 11,7" &_
		"union all select 27,8" &_
		"union all select 17,9" &_
		"union all select 19,10" &_
		"union all select 20,11"
		Set rs = cn.execute("select OrderName, ID, Modules from M_OrderSettings x inner join (" & ords & ") y on x.id=y.oid order by y.s")
		while rs.eof = False
			mcode = rs("Modules").value & ""
			If mcode = "" Or app.power.ExistsModel(mcode) Then
				nm = rs(0).value
				If nm = "预测单" Then nm = "生产预测"
				If nm = "质量检验单" Then nm = "生产质检"
				If nm = "生产进度单" Then nm = "进度汇报"
				If nm = "返工进度单" Then nm = "返工汇报"
				If nm = "返工进度单" Then nm = "返工汇报"
				If Len(nm)>4 And Right(nm,1) = "单" Then nm = Replace(nm,"单","")
				If InStr("," & mustnodes & ",","," & rs(1).value & ",") > 0 Then
					Response.write "<input type=checkbox disabled checked><label>" & nm & "</label><input onclick='boxchange(this)' name='mustnodes' type='hidden' value='" & rs(1).value & "'>&nbsp;&nbsp;"
				else
					Dim cked : cked = ""
					If InStr("," & seletedNodes & ",", "," & rs(1).value & ",") > 0 then
						cked = "checked"
					end if
					Response.write "<input type=checkbox id='snodes" & rs(1).value & "' value='" & rs(1).value & "' onclick='boxchange(this)' name='seletednodes' " & cked & "><label>" & nm & "</label>&nbsp;&nbsp;"
				end if
				i=i+1
	...
	Sub configcard3(ByVal fd)
		Dim v1 : v1 = fd.bill.data.fields("canyldj")
		Dim v2 : v2 = fd.bill.data.fields("mustllhb")
		Dim v3 : v3 = fd.bill.data.fields("dismorehb")
		Dim mc1 : mc1 = app.power.existsModel(18570)
		Dim mc2 : mc2 = app.power.existsModel(18540)
		Dim mp11 : mp11 = ExistsManuModel(11)
		Dim mp12 : mp12 = ExistsManuModel(12)
		Response.write "<div style='padding-left:15px'>"
'Dim mp12 : mp12 = ExistsManuModel(12)
		If mc1 Then Response.write "<input name='canyldj' id='canyldj'     value=1 type=checkbox " & app.iif(v1,"checked","") & "><label>生产过程允许登记物料使用</label><br>"
		If mp11 Or mp12 And mc2 Then Response.write "<input name='mustllhb' id='mustllhb'   value=1 type=checkbox " & app.iif(v2,"checked","") & "><label>必须领料才能进行进度汇报</label><br>"
		If mp11 And mc2 Then         Response.write "<input name='dismorehb' id='dismorehb' value=1 type=checkbox " & app.iif(v3,"checked","") & "><label>汇报数量不能大于派工数量</label>"
		Response.write "</div>"
	end sub
	Function ExistsManuModel(ByVal id)
		Dim rs : Set rs = cn.execute("select Modules from M_OrderSettings where ID=" & id)
		ExistsManuModel = app.power.ExistsModel(rs(0).value)
		rs.close
		set rs = nothing
	end function
	Sub bill_onBottomSub(ByVal bill)
		Response.write "" & vbcrlf & "<script>" & vbcrlf & "      function boxchange(box) {" & vbcrlf & "              updateJionEable(box);" & vbcrlf & "    }" & vbcrlf & "       function updateJionEable(box) {" & vbcrlf & "                var sv = new Array();" & vbcrlf & "           var mv = new Array();" & vbcrlf & "           var vs1 = document.getElementsByName(""seletednodes"");" & vbcrlf & "           var vs2 = document.getElementsByName(""mustnodes"");" & vbcrlf & "                for (var i = 0; i < vs1.length ; i++ ) { if(vs1[i].checked) { sv[sv.length] = vs1[i].value; } }" & vbcrlf & "         for (var i = 0; i < vs2.length ; i++ ) { mv[i] = vs2[i].value; }" & vbcrlf & "                if($ID(""mustllhb"")) {" & vbcrlf & "                     $ID(""mustllhb"").disabled = ! ( (sv.indexOf(11)>=0 || mv.indexOf(11)>=0 )  && (sv.indexOf(12)>=0  || mv.indexOf(12)>=0 ) );" & vbcrlf & "                        if($ID(""mustllhb"").disabled) { $ID(""mustllhb"").checked = false; }" & vbcrlf & "           }" & vbcrlf & "               if($ID(""dismorehb"")) { " & vbcrlf & "                       $ID(""dismorehb"").disabled = ! (sv.indexOf(11)>=0 || mv.indexOf(11)>=0 );" & vbcrlf & "                  if($ID(""dismorehb"").disabled) { $ID(""dismorehb"").checked = false; }" & vbcrlf & "         }" & vbcrlf & "               if($ID(""canyldj"")) {" & vbcrlf & "                      $ID(""canyldj"").disabled = ! (sv.indexOf(11)>=0 ||mv.indexOf(11)>=0 );" & vbcrlf & "                       if($ID(""canyldj"").disabled) { $ID(""canyldj"").checked = false; }" & vbcrlf & "             }" & vbcrlf & "               if(box) {" & vbcrlf & "                       if(box.value==19) { $ID(""snodes20"").checked = box.checked;   return; }" & vbcrlf & "                    else if(box.value==20) { $ID(""snodes19"").checked = box.checked;  return;}" & vbcrlf & "                " & vbcrlf & "                        if(box.value==25) { $ID(""snodes27"").checked = box.checked; if(box.checked==false){$ID(""snodes30"").checked=false;};return; }" & vbcrlf & "                 else if(box.value==27) { $ID(""snodes25"").checked = box.checked;if(box.checked==false){$ID(""snodes30"").checked=false;}; return;}" & vbcrlf & "                      else if(box.value==30 && box.checked && $ID(""snodes25"")) { $ID(""snodes25"").checked=true; $ID(""snodes27"").checked=true;return;}" & vbcrlf & "                }" & vbcrlf & "        }" & vbcrlf & "       updateJionEable();" & vbcrlf & "</script>" & vbcrlf & ""
	end sub
	function Bill_OnSave(ByVal Bill)
		Dim used : used = app.getint("used")
		If used = 1 Then
			cn.execute "update M_OrderSettings_flows set used=case id when " & bill.id & " then 1 else 0 end"
		end if
		cn.execute "update M_OrderSettings_flows set mustllhb='" & app.getint("mustllhb") & "'," &_
		"dismorehb='" & app.getint("dismorehb") & "',"&_
		"canyldj='" & app.getint("canyldj") & "',"&_
		"seletedNodes='" & Replace(app.gettext("seletedNodes")," ","") & "'"&_
		" where ID=" & bill.id
		bill.showSaveResultEx "保存成功", false, "leftFrame", ""
		Bill_OnSave =  true
	end function
