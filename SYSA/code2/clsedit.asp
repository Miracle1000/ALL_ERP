...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(byval rpt, byval  state)
		...
		rpt.title = "二维码分类设置"
		rpt.sql = "select title as 二维码分类, (case [stop] when 1 then '<span style=""color:#ff0000"">停用</span>' else '启用' end) as 状态, gate1 as 重要指数, id as 操作 from C2_CodeTypes where isnull(fromSys,0) <> 2 order by gate1 desc, id"
		...
		rpt.canprint = False
		rpt.Settings.clear
		rpt.buttonsHTML = "<button class='anybutton' onclick='myOpenUrl(""clsadd.asp"")'>添加二维码分类</button>"
	end sub
	sub app_oncreate(byval lvw)
		lvw.PageBar = False
		lvw.HeaderPageSizeUI = False
		lvw.pagesize = 100000
		lvw.headers("操作").formattext = "code:cHandleHtml(rs(""操作""))"
	end sub
	function cHandleHtml(ByVal id)
		Dim rss
		cHandleHtml = "<button class='oldbutton2' onclick='myOpenUrl(""clsadd.asp?ord=" & app.base64.pwurl(id) & """)'>修改</button>"
		Set rss =  cn.execute("select id from C2_CodeItems where ctype=" & id)
		If rss.eof = true then
			cHandleHtml = cHandleHtml & "&nbsp;<button class='oldbutton2'  onclick='delcls(""" & (id) & """)'>删除</button>"
		end if
		rss.close
	end function
	Sub App_delcls
		Dim ord
		ord = app.getInt("ord")
		cn.execute "delete C2_CodeTypes where id=" & ord
		cn.execute "delete C2_CodeTypeFields where ctypeid=" & ord
	end sub
