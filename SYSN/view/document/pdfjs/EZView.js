...
$.fn.EZView = function(){
        ...
        if (!$('#EZView').length) {
            ...
            var tools = 
                '<div class="tools-container">'+
                    '<spam class="name"><b style="color:#ff6873;font-size: 22px;line-height: 15px;"></b></spam>' +
                    '<spam class="tools">'+
                    ...
                    '</spam>'+
                    '<a class="closee EZViewHighlight" href="">'+
                    '<img src="'+icons.closee+'"/></a>'+
                '</div>',
                container = '<div class="object"/>'+
                        ...
                        tools,
                template = '<div id="EZView" class="hidee pdf-container" style="display: flex; align-items: center; justify-content: center">' + container + '</div>';
    ...
    self.showOrHideControls = function(imgIndex){
        var href = arIndex[imgIndex].href,       
            name = arIndex[imgIndex].name;  //弹出层标题
    ...
    self.builtObjectTemplate = function(index){
        ...
        if (isPdf) {
            ...
            /*
             陆涛，2020.04.29
             调整加载pdf.js的框架包，调用方式为 viewer.html?file=/abc.pdf
             */
            //alert("pdfsrc:" + src);
            content = '<iframe frameborder="0" index-render="' + index + '" height="' + ($(window).height()-30) * 0.95 + '" width="' + $(window).width() * 0.9 +
               '" src="./pdf-js/viewer.html?file=/' + src + '" ><p>Your browser does not support iframes.</p>' +
               '<script type="text/javascript">iFramesSupported = false; alert(iFramesSupported)</script><iframe/>';
    ...
    self.addEvents = function(){
        ...
        // Add close Event
        .find('.closee>img').click(function(e){
            e.preventDefault();
            e.stopPropagation();
            self.close();
        }).end()
        ...
        // Add prevent default
        .find('.zoomin, .zoomout, .closee').click(function(e){
            e.stopPropagation();
            e.preventDefault();
        }).end()
    ...
    return $(this).each(function(i, el){
        ...
        arIndex[i].name     = $.trim($el.html()).substring(0,30) || $el.attr('alt').substring(0,30) || '';  //弹出层标题
