...
'If sdk.power.existsPowerIntro(5029,14, designer) Then
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "       <style>" & vbcrlf & " .lvwframe2 td{color:#000;}" & vbcrlf & "      #comm_itembarbg{height:64px!important}" & vbcrlf & "  html{padding: 0px 10px 0;background: #efefef;box-sizing:border-box;}" & vbcrlf & "    body{background: #ffffff;}" & vbcrlf & "      </style>" & vbcrlf & "" & vbcrlf & "        "
'Sub Bill_OnPageInit
	end sub
	Sub Bill_OnInit(ByVal bill, ByVal t)
		Dim title ,act : act = app.getText("act")
		title = app.iif(bill.id=0,"设计添加","设计修改")
		If act = "change" Then  title = "设计变更"
		Bill.maxspan = 3
		bill.title = title
		bill.edit=True
		bill.loadJs =True
		bill.loadEasyUI = True
		app.addCSSpath app.virpath & "skin/default/css/add_td_style.css"
		Dim designtype,DSource , formName ,sql , datafiels ,designstatus ,deftype ,fromid
		deftype = app.getInt("designtype")
		fromid = app.base64.deurl(app.getText("fromid"))
		If fromid&""="" Then fromid = 0
		designtype = app.glAttribute("MI_DESIGN_TYPE_2016051601")
		Dim isnet : isnet = IsNetProduce
		If deftype>0 Then
			isnet = false
			If deftype>10000 Then isnet = True
		end if
		DSource = getDesignTypeList("list" , designtype ,isnet)
		If Len(DSource)>0 Then DSource = DSource &";"
		sql = "select d.designtype ,d.title,d.DesignBH,d.Designer,d.sort1,d.level,d.plandate1 , d.plandate2 ,d.zdy1,d.zdy2,d.zdy3,d.zdy4,d.zdy5,d.zdy6,d.intro, "&_
		" case designtype when 1 then c.title when 2 then t.title when 3 then p.title when 4 then m1.title when 5 then m2.title else '' end as formName,d.designstatus , 0 as urlpage " &_
		"   from design d  "&_
		"   left join chance c on c.ord=d.chance "&_
		"   left join contract t on t.ord=d.contract "&_
		"   left join price p on p.ord=d.price "&_
		"   left join M_PredictOrders m1 on m1.id=d.M_PredictOrders "&_
		"   left join M_ManuPlans m2 on m2.id=d.M_ManuPlans "&_
		"   where d.id="& bill.id
		bill.sql = sql
		designstatus = 1
		DSource = DSource & "手动添加=0"
		With Bill.fields.add1("任务来源","designtype","select","varchar",True,bill.id=0,false,False)
		.source = "options:"& DSource
		.defvalue= deftype
		.colspan = 3
		.js = " onchange='setDesignType(this)' "
		If bill.id>0 Then
			If Bill.exists Then
				Set datafiels = bill.DataFields
				designstatus = datafiels("designstatus").value
				If datafiels("designtype").value>0 Then .unit ="："& datafiels("formName").value
			end if
		ElseIf deftype<>0 Then
			.unit ="&nbsp;<input type='hidden' id='fromid' name='fromid' value='"& fromid &"'><input type='text' readonly='true' size='22' id='fromname' onclick='getFromID()' name='fromname' value='"& App_getOrder(deftype , fromid , 0) &"'> <input class='notnull' id='frombt' title='必填' type='button' value='*'>&nbsp;<span id='showbt' style='color:red;display:none;'>必填</span>"
		end if
		End With
		With Bill.fields.add1("设计主题","title","text","varchar",True,True,True,False)
		.colspan = 2
		.width = 202
		.maxlimit = 100
		.defvalue = App_getOrder(deftype , fromid , 1)
		End With
		Dim rs, bh
		If bill.id=0 Then
			bh=""
			set rs = cn.execute("EXEC erp_getdjbh 9029,"& Info.User )
			bh=rs(0).value
			rs.close
			set rs=Nothing
			If bh = "error" Then bh = ""
		end if
		With Bill.fields.add1("设计编号","DesignBH","text","varchar",True,True,True,False)
		.defvalue = bh
		.maxlimit = 50
		End With
		If bill.id=0 Or designstatus=1 Then Bill.fields.add1("设计人员","Designer","gate","int",True,True,false,True).source="design"
		With Bill.fields.add1("设计分类", "sort1", "select", "int",  true, true, true , False)
		.source = "sortonehy:5029"
		.js = " style='max-width:150px;width:120px;' "
		.source = "sortonehy:5029"
		End With
		With Bill.fields.add1("设计等级", "level", "select", "int",  true, true, false , False)
		.source = "sortonehy:5030"
		.js = " style='max-width:150px;width:120px;' "
		.source = "sortonehy:5030"
		End With
		Bill.fields.add1 "计划开始日期", "plandate1", "date", "datetime",  true, true, false , False
		Bill.fields.add1 "计划完成日期", "plandate2", "date", "datetime",  true, true, false , False
		Bill.fields.add1("", "urlpage", "hidden", "int",  true, True, true , False).defvalue = deftype
		Bill.extra = 5029
	end sub
	Sub bill_OnloadAfterExtra(ByVal bill)
		If bill.id = 0 Then
			bill.AddCurrGroup "设计明细","designmx"
			Bill.fields.addListView "", "designlist"
		end if
		With Bill.fields.add1("备注", "intro", "editor", "varchar",  true, true, false , False)
		.colspan = 3
		.inline = false
		.maxlimit = 4000
		End With
		Bill.fields.add1 ("", "@act", "hidden", "varchar",  true, true, false , False).defvalue = app.getText("act")
	end sub
	Sub bill_onListCreate(byref field, byref lvw)
		Dim fromid : fromid = app.base64.deurl(app.getText("fromid"))
		Dim designtype : designtype = app.getInt("designtype")
		If fromid &""="" Then fromid = 0
		Dim dataType : dataType = App_getOrder(designtype , fromid , 2)
		Call App_createListView(lvw , dataType , fromid)
	end sub
	Function getBaseSql(dataType , ord)
		Dim sql : sql = ""
		Select Case dataType
		Case "chance" :
		sql = "select c.ord as ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from chancelist c "&_
		"  inner join product p on p.ord=c.ord and p.del=1 and p.canoutstore=1 "&_
		"  where c.chance = "& ord &" and c.del=1 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=1 and chance="& ord &")  "
		Case "contract" :
		sql = "set nocount on; select top 0 CAST(0 as int) BillType ,  CAST(0 as int) BillId ,  CAST(0 as int) BillListType ,  CAST(0 as int) ListID ,  "&_
		"    CAST('' as nvarchar(2000)) zdy1, CAST('' as nvarchar(2000)) zdy2, CAST('' as nvarchar(2000)) zdy3, CAST('' as nvarchar(2000)) zdy4, "&_
		"    CAST(0 as int) zdy5 , CAST(0 as int) zdy6 into #ZdyListValues; "&_
		" insert into #ZdyListValues(BillType, BillId, BillListType ,ListID ,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6)  "&_
		"  exec ERP_sdk_zdyfieldvalue_list 11001 , "& ord &" , 1 ,'';  "&_
		"  select c.ord as ProductID,p.title,p.order1,p.type1,c.unit, "&_
		"  isnull(zlv.zdy1,c.zdy1) zdy1 ,isnull(zlv.zdy2,c.zdy2) zdy2,isnull(zlv.zdy3,c.zdy3) zdy3,isnull(zlv.zdy4,c.zdy4) zdy4, isnull(zlv.zdy5,c.zdy5) zdy5,isnull(zlv.zdy6,c.zdy6) zdy6, "&_
		"  c.id as listid,p.unit as unitall, "&_
		"   isnull(c.intro,'') + isnull(s1.title + ':' , '') + isnull(v1.title,'') + (case when v1.id>0 and v2.id>0 then ',' else '' end) + isnull(s2.title + ':' , '') + isnull(v2.title,'') as intro "&_
		"  c.id as listid,p.unit as unitall, "&_
		"  from contractlist c "&_
		"  inner join product p on p.ord=c.ord and p.del=1  and p.canoutstore=1"&_
		"   left join shop_GoodsAttr v1 on v1.id = c.ProductAttr1    "&_
		"   left join shop_GoodsAttr s1 on s1.id = v1.PID            "&_
		"   left join shop_GoodsAttr v2 on v2.id = c.ProductAttr2    "&_
		"   left join shop_GoodsAttr s2 on s2.id = v2.PID            "&_
		"   left join #ZdyListValues zlv on zlv.ListID = c.ID        "&_
		"  where c.contract = "& ord &" and c.del=1 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=2 and contract="& ord &") " &_
		"   order by c.id asc;drop table #ZdyListValues;set nocount off; "
		Case "price" :
		sql = "select c.ord as ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from pricelist c "&_
		"  inner join product p on p.ord=c.ord and p.del=1  and p.canoutstore=1"&_
		"  where c.price = "& ord &" and c.del=1 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=3 and price="& ord &")  "
		Case "M_PredictOrders" :
		sql = "select c.ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit, c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from M_PredictOrderlists c "&_
		"  inner join product p on p.ord=c.productid and p.del=1  and p.canoutstore=1"&_
		"  where c.porderid = "& ord &" and c.del=1  and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=4 and M_PredictOrders="& ord &")  "
		Case "M_ManuPlans" :
		sql = "select c.ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,c.zdy1,c.zdy2,c.zdy3,c.zdy4,c.zdy5,c.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from M_ManuPlanlists c "&_
		"  inner join product p on p.ord=c.productid and p.del=1  and p.canoutstore=1"&_
		"  where c.MPSID = "& ord &" and c.del=0 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=5 and M_ManuPlans="& ord &") "
		Case "M2_PrePlans" :
		sql = "select c.ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,P.zdy1,P.zdy2,P.zdy3,P.zdy4,P.zdy5,P.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from M2_ManuPlanListsPre c "&_
		"  inner join product p on p.ord=c.productid and p.del=1  and p.canoutstore=1"&_
		"  where c.MPSID = "& ord &" and c.del=1  and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=52001 and M2_PrePlans="& ord &")  "
		Case "M2_ManuPlans" :
		sql = "select c.ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,P.zdy1,P.zdy2,P.zdy3,P.zdy4,P.zdy5,P.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from M2_ManuPlanlists c "&_
		"  inner join product p on p.ord=c.productid and p.del=1  and p.canoutstore=1"&_
		"  where c.MPSID = "& ord &" and isnull(c.PID,-1)<>0 and c.del=1 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=52002 and M2_ManuPlans="& ord &") "
		Case "M2_WorkAssigns" :
		sql = "select c.ProductID,p.title,p.order1,p.type1, "&_
		"  c.unit,P.zdy1,P.zdy2,P.zdy3,P.zdy4,P.zdy5,P.zdy6,c.intro , c.id as listid,p.unit as unitall "&_
		"  from M2_WorkAssigns c "&_
		"  inner join product p on p.ord=c.productid and p.del=1  and p.canoutstore=1"&_
		"  where c.ID = "& ord &" and c.del=1 and c.id not in (select isnull(listid,0) from designlist l inner join design d on d.del=1 and l.del=1 and d.designType=54002 and M2_WorkAssign="& ord &") "
		Case Else :
		sql = "select top 0 'ProductID' as ProductID,'产品名称' as title,'编号' as order1,'型号' as type1, '单位' as unit,'zdy1' as zdy1,'zdy2' zdy2,'zdy3' zdy3,'zdy4' zdy4,'zdy5' zdy5,'zdy6' zdy6,'intro' intro , 0 as listid, '' as unitall  "
		End Select
		getBaseSql = sql
	end function
	Sub App_getLvwJsonByOrder()
		Dim dataType : dataType = app.getText("dataType")
		Dim ord : ord= app.getInt("ord")
		Dim sql : sql = ""
		Dim lvw : Set lvw = New listview
		lvw.border= 0
		lvw.edit=True
		Call App_createListView(lvw , dataType , ord)
		lvw.iscallback=True
		Response.write lvw.HTML
	end sub
	Sub App_createListView(ByRef lvw , ByVal dataType ,ByVal ord)
		lvw.mxzdyId=5029
		lvw.sql = getBaseSql(dataType , ord)
		lvw.indexbox = True
		lvw.checkbox=False
		lvw.id = "bllst_designlist"
		lvw.jsoneditmodel = True
		lvw.headers("ProductID").display="none"
		lvw.headers("listid").display = "none"
		lvw.headers("unitall").display="none"
		lvw.headers("@@序号").width = 35
		With lvw.headers("title")
		.uitype = "text"
		.notnull = True
		.source = "script:window.open('searchProduct.asp','design_list','width=820,height=560,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100')"
		.EditLock = "code:@cells[""listid""]!=""0"" && @cells[""listid""]!="""" "
		.formattext = "code:ListCellsFormat(""title"",rs) "
		.canBatchInput = 1
		End With
		With lvw.headers("unit")
		.uitype = "select"
		.source = "sortonehy:61;filter:unitall"
		.EditLock = "code:@cells[""listid""]!=""0"" && @cells[""listid""]!="""" "
		End With
		...
		.uitype = "select"
		.source = "sortonehy:2101"
		End With
		With lvw.headers("zdy6")
		.uitype = "select"
		.source = "sortonehy:2102"
		End With
		With lvw.headers("intro")
		.uitype = "text"
		.formula = "app.htmltotext(@value)"
		.maxsize = 4000
		End With
	end sub
	Sub App_getProInfo()
		Dim sql , rs, pid ,rs1 ,json ,unit ,unitall ,SX ,ptype
		pid = app.getText("product")
		If pid&""="" Then pid = 0
		ptype = app.getInt("ptype")
		sql = "select " &_
		" title,order1,type1,unit,unitjb,canOutStore,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6 " &_
		" from product p " &_
		" left join sortonehy z5 on z5.ord = p.zdy5 " &_
		" left join sortonehy z6 on z6.ord = p.zdy6 " &_
		" where p.ord in (" & pid & ") and (user_list = '' or user_list = '0' or user_list = '0,0' or CHARINDEX('," & Info.User & ",',','+replace(cast(User_List as varchar(8000)),' ','')+',') > 0) "
		Set rs = cn.execute(sql)
		If rs.eof Then
			json = "{msg:'false'}"
		else
			...
			While rs.eof=False
				...
				unit = ""
				SX = "虚拟"
				If rs("canOutStore") = "1" Then SX = "实体"
				...
				"UnitJB:'" & rs("unitjb")&"" & "'," &_
				"UnitAll:'" & unitall & "'," &_
				"Unit:'" & unit & "',canOutStore:'"& rs("canOutStore") &"',mxid:0}"
				i = i + 1
				Unit:' & unit & ',canOutStore:'& rs(canOutStore) &
				rs.movenext
			wend
			json = json & "]}"
		end if
	...
	Function Bill_OnSave(ByVal bill)
		Dim Rs ,sql,designtype ,fromid , title , DesignBH , Designer,oldDesigner ,sort1 ,level ,plandate1, plandate2 ,zdyfields ,field ,i,ii , FValue ,intro , ord , listviewjson
		designtype = app.getInt("designtype")
		fromid = app.getInt("fromid")
		If designtype>0 And fromid=0 Then
			bill.showSaveResultEx "请选择设计来源！",false,"none",""
			Exit Function
		end if
		if designtype=3 then
			verifySql="select complete,status from price where del=1 and ord="& fromid
			Set rs = cn.execute(verifySql)
			if rs.eof then
				Response.write"<script language=javascript>alert('此报价单被删除或彻底删除');</script>"
				Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
				Call db_close():Response.end
			else
				status=rs("status")
				complete=rs("complete")
				if (not status=1 and not status=-1 and not complete=4) or complete=5  then
					complete=rs("complete")
					Response.write"<script language=javascript>alert('此报价单状态已变更，不能进行设计');</script>"
					Response.write"<script language=javascript>window.open('','_self');window.close();</script>"
					Call db_close():Response.end
				end if
			end if
		...
		title = app.getText("title")
		DesignBH = app.getText("DesignBH")
		If Len(DesignBH)>0 Then
			If cn.execute("select 1 from design where DesignBH='"& DesignBH &"' and id<>"& bill.id ).eof=False Then
				bill.showSaveResultEx "设计编号重复！",false,"none",""
				Exit Function
			end if
		end if
		Designer =app.getint("Designer")
		sort1 = app.getInt("sort1")
		level = app.getInt("level")
		plandate1 = app.getText("plandate1")
		plandate2 = app.getText("plandate2")
		If Len(plandate1)>0 And Len(plandate2)>0 Then
			If datediff("d",plandate1,plandate2)<0 Then
				bill.showSaveResultEx "计划开始日期不能大于计划完成日期！",false,"none",""
				Exit Function
			end if
		end if
		intro = app.getText("intro")
		If bill.id >0 And app.getText("@act")="change" Then  Call CopyBillBeforChange(cn , -31 , bill.id , "设计变更", info.user)
		intro = app.getText("intro")
		Dim designstatus : designstatus = 1 : oldDesigner=0
		sql = "select * from Design where id= " & bill.id
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.open sql,cn,1,3
		If rs.eof Then
			Rs.addnew
		else
			designstatus = rs("designstatus")
			oldDesigner =rs("Designer")
		end if
		rs("PrefixCode")="DN"
		rs("title")=title
		rs("DesignBH")= DesignBH
		rs("sort1") = sort1
		rs("level") = level
		If Len(plandate1)>0 Then
			rs("plandate1") = plandate1
		else
			rs("plandate1") = null
		end if
		If Len(plandate2)>0 Then
			rs("plandate2") = plandate2
		else
			rs("plandate2") = null
		end if
		If hasOpenZdy(Bill.extra) Then
			Set zdyfields = GetZdyFields(Bill.extra)
			For i = 0 To zdyfields.count-1
				Set zdyfields = GetZdyFields(Bill.extra)
				Set field = zdyfields.item(i)
				If field.show = True Then
					Select Case field.sorttype
					Case 1 :
					FValue=app.getInt(field.dbname)
					Case Else
					FValue=Trim(app.getText(field.dbname))
					End Select
					rs(field.dbname) = FValue
				end if
			next
		end if
		rs("intro") = intro
		If bill.id = 0 Then
			...
			rs("designstatus") = app.iif(Designer>0 , 3 , 1)
			rs("designtype") = designtype
			Select Case designtype
			Case 1 : rs("chance") = fromid
			Case 2 : rs("contract") = fromid
			Case 3 : rs("price") = fromid
			Case 4 : rs("M_PredictOrders") = fromid
			Case 5 : rs("M_ManuPlans") = fromid
			Case 52001 : rs("M2_PrePlans") = fromid
			Case 52002 : rs("M2_ManuPlans") = fromid
			Case 54002 : rs("M2_WorkAssign") = fromid
			End Select
			rs("Creator") = Info.User
			rs("indate") = now()
			rs("del") = 1
			If Designer>0 Then rs("date3") = now()
		else
			If designstatus=1 And Designer>0 Then rs("designstatus") = 3
			If Designer>0 And Designer<>oldDesigner Then
				rs("designer") = designer
				rs("date3") = now()
			end if
		end if
		...
		ord=Rs("id")
		rs.close
		Dim murl : murl = "list.asp"
		If bill.id = 0 Then
			listviewjson = app.getText("listviewjson")
			Dim lvw : Set lvw = app.Eval("var lvw="& listviewjson & "; lvw")
			Dim headers,header : Set headers = lvw.headers
			Dim fields : fields = ""
			Dim keyFieldIndex : keyFieldIndex = 0
			For Each header In headers
				If header.dbname="ProductID" Then keyFieldIndex = header.i
			next
			Dim rows ,row : Set rows = lvw.rows
			If rows.length = 0 Then
				bill.showSaveResultEx "请添加设计明细！",false,"none",""
				Exit Function
			end if
			Dim pid , v , RowCount : pid = 0
			RowCount = 0
			For Each row in rows
				pid = getValueFromJsonArray(row , keyFieldIndex)&""
				If Len(pid)>0 And pid<>"0" Then
					sql = "select * from DesignList where 1=0 "
					Set Rs = server.CreateObject("adodb.recordset")
					Rs.open sql,cn,1,3
					Rs.addnew
					Rs("Design") = ord
					For Each header In headers
						Select Case header.dbname
						Case "zdy1","zdy2","zdy3" , "zdy4":
						v = getValueFromJsonArray(row , header.i) &""
						If len(v)> 200 Then
							bill.showSaveResultEx "第"& (RowCount+1) &"行【"& header.title &"】不能超过200个字！",false,"none",""
'If len(v)> 200 Then
							Exit Function
						end if
						rs(header.dbname)= Left(v,200)
						Case "intro" :
						v = getValueFromJsonArray(row , header.i) &""
						If len(v)> 4000 Then
							bill.showSaveResultEx "第"& (RowCount+1) &"行【"& header.title &"】不能超过4000个字！",false,"none",""
'If len(v)> 4000 Then
							Exit Function
						end if
						rs(header.dbname)= Left(getValueFromJsonArray(row , header.i) &"",4000)
						Case "ProductID","unit","zdy5","zdy6","listid":
						v = getValueFromJsonArray(row , header.i)&""
						rs(header.dbname)= app.iif(Len(v)>0 And isnumeric(v),v,0)
						End Select
					next
					rs("del") = 1
					rs.update
					...
					bill.showSaveResultEx "第"& (RowCount+1) &"行产品明细不能为空！",false,"none",""
					rs("del") = 1
					Exit Function
				end if
			next
			If RowCount = 0 Then
				bill.showSaveResultEx "请重新编辑设计明细！",false,"none",""
				Exit Function
			end if
			If designtype>0 Then
				If cn.execute("select 1 from [erp_bill_extraBills] where oid="& designtype &" and bid = "& fromid &" and gl_id=" & ord).eof= True Then
					cn.execute("insert into erp_bill_extraBills (oid, bid ,gl_id , Creator , indate ) select "& designtype &", "& fromid &","& ord &","& info.user &",getdate() ")
					cn.execute("insert into erp_bill_extraBillDetails (BillType , BillID , ListID ,gl_id,gl_listid, Creator,indate) select "& designtype &", "& fromid &",listid,design,id,"& info.user &", getdate() from designlist where design = " & ord )
				end if
			end if
			If designer=0 Then murl = "pool.asp"
			Dim urlpage : urlpage = app.getInt("urlpage")
			If urlpage>0 Then
				bill.showSaveResultEx "",True,"opener",""
			else
				bill.showSaveResultEx "保存成功",false,"self",murl
			end if
		else
			If app.getText("@act")&""="" Then
				bill.ReportRefresh "保存成功"
			else
				Response.write "<script>window.alert('保存成功');try{top.opener.RefreshWindow();}catch(e){};"
				If app.getText("@act")="change" And ZBRuntime.MC(77000) And app.power.existsPower(5032 , 13) Then
					Response.write "top.location.href='../../SYSN/view/market/notice/notice.ashx?datatype=15001&fromid="& app.base64.pwurl(ord) &"';"
				else
