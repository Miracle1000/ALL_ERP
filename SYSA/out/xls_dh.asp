...
'bigsystemtype = ".mozi"
	...
	Response.Flush
	dim W1,W2,W3,A1,ret1,ret2,s1,s2
	W1=request("W1")
	W2=request("W2")
	W3=request("W3")
	A1=request("A1")
	...
	elseif open_74_10=1 then
		list="and addcate in ("&intro_74_10&")"
	else
		list="and addcate=0"
	end if
...
'elseif G1=6 then
	...
	elseif px=5 then
		px_Result="order by status desc"
	elseif px=6 then
		px_Result="order by status asc"
	elseif px=7 then
		px_Result="order by company desc"
	elseif px=8 then
		px_Result="order by company asc"
	elseif px=9 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from call  "&Str_Result&"  "&px_Result&" "
	...
	do until rs.eof
		...
		cateid=""
		xlWorksheet.Cells(1+i,2).Value = adddate
		cateid=""
		xlWorksheet.Cells(1+i,2).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,3).Value = status
		cateid=""
		xlWorksheet.Cells(1+i,3).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,4).Value = talktime
		cateid=""
		xlWorksheet.Cells(1+i,4).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,5).Value = company
		cateid=""
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户资料导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户资料导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("来电客户资料_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的来电客户资料</u></strong></font></a></p> " & vbcrlf & ""
