...
'function CurrCostDate(cn)
	...
	Response.write Application("sys.info.jsver")
	Response.write "' defer='defer'></script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""100%"" valign=""top"">" & vbcrlf & "                              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                                       <tr>" & vbcrlf & "                                            <td class=""place"">费用报销回收站</td>" & vbcrlf & "                                             <td>&nbsp;</td>" & vbcrlf & "                                         <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=paybx&url=pay1.asp&bank=10'}""/></td>" & vbcrlf & "                                            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                                      </tr>" & vbcrlf & "                           </table> " & vbcrlf & "                               <table width=""100%"" border=""0""cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf &                            "        <tr  class=""top"">" & vbcrlf &             "                       <td width=""8%"" align=""center"" ><div align=""center"">选择</div></td> "& vbcrlf &                                "     <td ><div align=""center"" ><strong>主题</strong></div></td> "& vbcrlf &" <td width=""8%"" height=""23""  ><div align=""center""><strong>金额</strong></div></td>" & vbcrlf & "                                          <td width=""8%"" height=""23""  ><div align=""center""><strong>审批金额</strong></div></td>" & vbcrlf & "                                         <td width=""8%"" ><div align=""center""><strong>使用人</strong></div></td>" & vbcrlf & "<td width=""8%"" ><div align=""center""><strong>币种</strong></div></td>" & vbcrlf &                                    "     <td width=""8%"" ><div align=""center""><strong>删除人</strong></div></td> "& vbcrlf &                    "                   <td width=""12%"" ><div align=""center""><strong>删除日期</strong></div></td> "& vbcrlf &           "                         <td width=""12%"" ><div align=""center""><strong>报销日期</strong></div></td> "& vbcrlf &                            "        <td width=""14%"" ><div align=""center"" ><strong>操作</strong></div></td>" & vbcrlf &                                   "  </tr>"& vbcrlf
	dim n , canDelete
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select a.id,a.bh,a.indate,a.complete,sum(isnull(b.money1,0)) as spmoney,a.title,a.delcate,a.deldate,a.bz,isnull(f.sort1,'币种被删除') as bzname,a.bxdate,d.name as payer,e.name as adder,g.name as deler,sum(isnull(b.money1,0)) as summoney,sum(c.num) as sumnum from paybx a "&_
	"inner join paybxlist b on a.id=b.bxid "&_
	"left join pay c on c.ord=b.payid "&_
	"left join gate d on c.cateid=d.ord "&_
	"left join gate e on c.addcate=e.ord "&_
	"left join gate g on g.ord=a.delcate "&_
	"left join sortbz f on f.id=a.bz "&_
	"where a.del=2 group by a.id,a.bh,a.indate,a.complete,a.title,a.delcate,a.deldate,a.bz,isnull(f.sort1,'币种被删除'),a.bxdate,d.name,e.name,g.name order by a.deldate desc, a.id desc"
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""deletepaybxall.asp?paybxid="
		...
		do until rs.eof
			dim k :k=rs("title")
			if k="" or isnull(k) then k="无标题"
			txtbool=False
			canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs("id") &"',40, 8, '' ").eof
			sqltext="select bx from pay where ord in (select payid from paybxlist where bxid="&rs("id")&")"
			set rstxt=server.CreateObject("adodb.recordset")
			rstxt.open sqltext,conn,1,1
			if not rstxt.eof then
				do while not rstxt.eof
					if rstxt("bx")<>0 and txtbool = false then
						sqltext="select complete from paybx where id="&rs("id")
						set rs9=server.CreateObject("adodb.recordset")
						rs9.open sqltext,conn,1,1
						if not rs9.eof then
							if rs9("complete")=2 then
								txtbool=false
							else
								txtbool=true
							end if
					...
					If txtbool = False Then txtbool = IsCostAnalysis(conn , 41003, rs("id"), 0 ,rs("bxdate"))
			...
			Response.write "" & vbcrlf & "              <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                   <td width=""8%"" align=""center""><span class=""red"">" & vbcrlf & "                              <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
			Response.write rs("id")
			Response.write """></span>" & vbcrlf & "                  </td>" & vbcrlf & "                   <td align=""center"" height=""26"">"
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                                      <a href=""#"" onClick=""javascript:window.open('../pay/paybxcontent.asp?paybxid="
				Response.write rs("id")
				Response.write "','neww3win','width=' + 1200 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs("id")
				Response.write k
			...
			if summoney="" or isnull(summoney) then summoney=0
			Response.write "" & vbcrlf & "                      </td>" & vbcrlf & "                   <td width=""8%"" height=""27""><div align=""center"">"
			Response.write Formatnumber(summoney,num_dot_xs,-1)
			Response.write " </div></td>" & vbcrlf & "                  <td width=""8%""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs("spmoney")),num_dot_xs,-1)
			Response.write " </div></td>" & vbcrlf & "                  <td width=""8%""><div align=""center"">"
			Response.write " </div></td>" & vbcrlf & "                 <td width=""8%""><div align=""center"">"
			Response.write rs("payer")
			Response.write "</div></td>" & vbcrlf & "                  <td width=""8%""><div align=""center"">"
			Response.write rs("bzname")
			Response.write "</div></td>" & vbcrlf & "                  <td width=""8%""><div align=""center"">"
			Response.write rs("deler")
			...
			if txtbool=false then
				Response.write "" & vbcrlf & "                                 <input type=""button"" name=""Submit3c""  class=""anybutton refalse""  value=""恢复""  onClick=""if(confirm('确认恢复？')){window.location.href='setpay1.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>" & vbcrlf & "                "
			...
			if canDelete then
				Response.write "<input  type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletepaybx.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
		...
		loop
		Response.write "" & vbcrlf & "       </table>" & vbcrlf & "      </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td  class=""page"">" & vbcrlf & "  <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "                          <input type=""checkbox"" name=""checkbox2"" value=""Check All"" onClick=""mm()""></div>" & vbcrlf & "                      </td>" & vbcrlf & "                   <td>" & vbcrlf & "                            <input type=""submit"" name=""Submit426"" value=""批量删除""   onClick=""return test();""  class=""anybutton2"">" & vbcrlf & "                            <input type=""button"" name=""Submit426"" value=""批量恢复""  onclick=""ask2();"" class=""anybutton2""/>" & vbcrlf & "                       </td>" & vbcrlf & "                   <td width=""69%""><div align=""right"">" & vbcrlf & "                         "
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='pay1.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='pay1.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='pay1.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='pay1.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "                             </div>" & vbcrlf & "                  </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <script type=""text/Javascript"">window.currask2Url=""setpay1all.asp?currPage="
		Response.write currPage
		Response.write """; </script>" & vbcrlf & "              <script src='../script/bk_comm01.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write "'></script>" & vbcrlf & "          <tr><td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;</p></div></td></tr>" & vbcrlf & "      </table>" & vbcrlf & "</form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	dim actinon1
