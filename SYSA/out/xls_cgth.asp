...
'bigsystemtype = ".mozi"
	...
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=13"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_13=0
		intro_75_13=0
	else
		open_75_13=rs1("qx_open")
		intro_75_13=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=2"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_2=0
		intro_75_2=0
	else
		open_75_2=rs1("qx_open")
		intro_75_2=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_3=0
		intro_75_3=0
	else
		open_75_3=rs1("qx_open")
		intro_75_3=rs1("qx_intro")
	end if
	rs1.close
	...
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=5"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_5=0
		intro_75_5=0
	else
		open_75_5=rs1("qx_open")
		intro_75_5=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=21"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_21=0
		intro_75_21=0
	else
		open_75_21=rs1("qx_open")
		intro_75_21=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_10=0
		intro_75_10=0
	else
		open_75_10=rs1("qx_open")
		intro_75_10=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_7=0
		intro_75_7=0
	else
		open_75_7=rs1("qx_open")
		intro_75_7=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_8=0
		intro_75_8=0
	else
		open_75_8=rs1("qx_open")
		intro_75_8=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_11=0
		intro_75_11=0
	else
		open_75_11=rs1("qx_open")
		intro_75_11=rs1("qx_intro")
	end if
	rs1.close
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=12"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_12=0
		intro_75_12=0
	else
		open_75_12=rs1("qx_open")
		intro_75_12=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=75 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_75_19=0
		intro_75_19=0
	else
		open_75_19=rs1("qx_open")
		intro_75_19=rs1("qx_intro")
	end if
	rs1.close
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=32 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_32_3=0
		intro_32_3=0
	...
	if open_75_1=3 then
		list=""
	elseif open_75_1=1 then
		list="and cateid in ("&intro_75_1&")"
	else
		list="and cateid=0"
	end if
...
'Public Function AddFormValue(name,  value)
	...
	dim W1,W2,W3,A1,A2,B,C,D,E,F,H
	W1=request("W1")
	W2=request("W2")
	W3=request("W3")
	A1=request("A1")
	...
	If caigouid&""<>"" Then
		Str_Result=Str_Result+" and caigou="&caigouid&" "
'If caigouid&""<>"" Then
	end if
	if company_th<>"" then
...
'if B = "thzt" then
		elseif B = "thbh" then
			Str_Result=Str_Result+" and cgthid like '%"& C &"%' "
'elseif B = "thbh" then
		elseif B = "xsry" then
...
'if  A2<>"" then
	...
	elseif px=3 then
		px_Result=" order by cgthid desc,ord desc"
	elseif px=4 then
		px_Result=" order by cgthid asc,ord asc"
	elseif px=5 then
		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					m=13
					set rs1=server.CreateObject("adodb.recordset")
					sql1="select * from zdy where sort1=75 and set_open=1 and dc=1 order by gate1 asc "
					rs1.open sql1,conn,1,1
					colcount=rs1.recordcount
					if not rs1.eof then
						do until rs1.eof
							xlApplication.ActiveSheet.Columns(m).ColumnWidth=15
							xlApplication.ActiveSheet.Columns(m).HorizontalAlignment=3
							m=m+1
							xlApplication.ActiveSheet.Columns(m).HorizontalAlignment=3
							rs1.movenext
						loop
					...
					set rs1=nothing
					set rs1=server.CreateObject("adodb.recordset")
					sql1="select id,title,sort,kd,set_open,gate1,sorce from zdymx where sort1=75 and set_open=1 order by gate1 asc"
					rs1.open sql1,conn,1,1
					if not rs1.eof then
						do until rs1.eof
							xlApplication.ActiveSheet.Columns(m).ColumnWidth=10
							xlApplication.ActiveSheet.Columns(m).HorizontalAlignment=3
							if rs1("sorce")=5 or rs1("sorce")=7 then
								xlApplication.ActiveSheet.Columns(m).NumberFormatLocal = "#,##0."&dotstr&"_ "
							elseif rs1("sorce")=6 then
								xlApplication.ActiveSheet.Columns(m).NumberFormatLocal = "#,##0."&dotstr_num&"_ "
							end if
							m=m+1
							xlApplication.ActiveSheet.Columns(m).NumberFormatLocal = "#,##0."&dotstr_num&"_ "
							rs1.movenext
					...
					do while not rs1.eof
						xlWorksheet.Cells(1,m).Value = rs1("title")
						xlWorksheet.Cells(1,m).font.Size=10
						xlWorksheet.Cells(1,m).font.bold=true
						m=m+1
						xlWorksheet.Cells(1,m).font.bold=true
						rs1.movenext
					loop
					...
					if not rs1.eof then
						do until rs1.eof
							...
							xlWorksheet.Cells(1,m).font.bold=true
							If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+1).Value = "申请"
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+1).font.Size=10
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+1).font.bold=true
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+2).Value = "出库"
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+2).font.Size=10
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+2).font.bold=true
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+3).Value = "申请发货"
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+3).font.Size=10
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+3).font.bold=true
'If rs1("sorce") = 6 Then
								xlWorksheet.Cells(1,m+4).Value = "发货"
'If rs1("sorce") = 6 Then
...
'If rs1("sorce") = 6 Then
							...
							If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+1).Value = "产品属性1"
'If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+1).font.Size=10
'If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+1).font.bold=true
'If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+2).Value = "产品属性2"
'If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+2).font.Size=10
'If rs1("sorce") = 4 and isOpenProductAttr Then
								xlWorksheet.Cells(1,m+2).font.bold=true
...
'If rs1("sorce") = 4 and isOpenProductAttr Then
					...
					set rs=server.CreateObject("adodb.recordset")
					sql="select * " & _
					",bz=(select intro from sortbz where id=a.bz)" & _
					",fl=(select sort1 from sortonehy where ord=a.sort)" & _
					",cate=(select name from gate where ord=a.cateid)" & _
					",addcate=(select name from gate where ord=a.addcate)" & _
					",(select top 1 sort1 from sortonehy where ord = a.complete1) thzt" & _
					",(select top 1 title from caigou where ord=a.caigou) caigouTitle" &_
					",(select top 1 name from person where ord=a.person2) person_otherside" &_
					",case when zt1=0 then '未编辑退货明细' when zt1=1 then '未出库' when zt1=2 then '部分出库' when zt1=3 then '出库完毕' end as ckzt" &_
					" from caigouth a "&Str_Result&" "&px_Result&""
					rs.open sql,conn,1,1
					C1=rs.recordcount
					xlApplication.Init "采购退货列表_"&session("name2006chen")&".xls",C1
					dim i,j
					i=1
					j=1
					do until rs.eof
						...
						ckzt=rs("ckzt")
						company = rs("company")
						person_us = rs("person1")
						person_otherside = rs("person_otherside")
						caigouTitle = rs("caigouTitle")
						companyname = ""
						companybh = ""
						Iscolor = False
						Set gysRs=conn.execute("select name , khid, sort3 ,del , isnull(cateid,0) as cateid from tel where ord="&company)
						If gysRs.eof = False Then
							If gysRs("del")=2 Then
								companyname ="供应商已删除"
								Iscolor =true
							else
								if open_26_1=3 or (open_26_1=1 and CheckPurview(intro_26_1,trim(gysRs("cateid")))=True And gysRs("cateid")<>"0") Then
									companyname = gysRs("name")
									companybh = gysRs("khid")
								else
									companyname = ""
								end if
							end if
						...
						gysRs.close
						if open_22_1=3 or (open_22_1=1 and CheckPurview(intro_22_1,trim(rs("cateid")))=True) Then
							caigouTitle=rs("caigouTitle")
						else
							caigouTitle = ""
						end if
						xlWorksheet.Cells(1+i,1).Value = rs("title")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,1).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,1).HorizontalAlignment=1
						caigouTitle = ""
						xlWorksheet.Cells(1+i,2).Value = rs("cgthid")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,2).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,3).Value = formatdatetime(rs("date7"),0)
						caigouTitle = ""
						xlWorksheet.Cells(1+i,3).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,4).Value = Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
						caigouTitle = ""
						xlWorksheet.Cells(1+i,4).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,5).Value = rs("bz")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,5).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,6).Value = formatdatetime(rs("date3"),0)
						caigouTitle = ""
						xlWorksheet.Cells(1+i,6).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,7).Value = rs("fl")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,7).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,8).Value = thzt
						caigouTitle = ""
						xlWorksheet.Cells(1+i,8).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,9).Value = ckzt
						caigouTitle = ""
						xlWorksheet.Cells(1+i,9).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,10).Value = rs("addcate")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,10).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,11).Value = rs("cate")
						caigouTitle = ""
						xlWorksheet.Cells(1+i,11).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,12).Value = person_us
						caigouTitle = ""
						xlWorksheet.Cells(1+i,12).font.Size=10
						caigouTitle = ""
						xlWorksheet.Cells(1+i,13).Value = person_otherside
						caigouTitle = ""
						xlWorksheet.Cells(1+i,13).font.Size=10
						caigouTitle = ""
						m=14
						set rs88=server.CreateObject("adodb.recordset")
						sql88="select id,name,sort,gl from zdy where sort1=75 and set_open=1 and dc=1 order by gate1 asc "
						...
						if not rs88.eof then
							do until rs88.eof
								if rs88("sort")=1 then
									...
									xlWorksheet.Cells(1+i,m).Value = zdyname
									set rs7=nothing
									xlWorksheet.Cells(1+i,m).font.Size=10
									set rs7=nothing
								else
									xlWorksheet.Cells(1+i,m).Value = rs(""&rs88("name")&"")
									set rs7=nothing
									xlWorksheet.Cells(1+i,m).font.Size=10
									set rs7=nothing
								end if
						...
						rs88.close
						set rs88=nothing
						xlWorksheet.Cells(1+i,m).Value = rs("intro")
						set rs88=nothing
						xlWorksheet.Cells(1+i,m).font.Size=10
						set rs88=nothing
						If Iscolor = True Then
							xlWorksheet.Cells(1+i,m+1).font.color= "white"
'If Iscolor = True Then
							xlWorksheet.Cells(1+i,m+1).font.bgcolor = "red"
'If Iscolor = True Then
						end if
						xlWorksheet.Cells(1+i,m+1).Value = companyname
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+1).HorizontalAlignment=1
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+1).font.Size=10
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+2).Value = companybh
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+2).HorizontalAlignment=1
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+2).font.Size=10
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+3).Value = caigouTitle
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+3).HorizontalAlignment=1
'If Iscolor = True Then
						xlWorksheet.Cells(1+i,m+3).font.Size=10
'If Iscolor = True Then
						sql1 = "select thl.*,sg1.title ProductAttr1,sg2.title as ProductAttr2,"&_
						" isnull((select sum(num2) from kuoutlist2 where sort1=2 and contractlist=thl.id and del=1),0) as numsqfh, "&_
						" isnull((select sum(num3) from kuoutlist2 where sort1=2 and contractlist=thl.id and del=1),0) as numsjfh  "&_
						" from caigouthlist thl left join kuinlist kl on thl.kuinlist=kl.id  "&_
						" left join Shop_GoodsAttr sg1 WITH(NOLOCK) on sg1.id=kl.ProductAttr1  "&_
						" left join Shop_GoodsAttr sg2 WITH(NOLOCK) on sg2.id=kl.ProductAttr2   "&_
						" where caigouth="&rs("ord")&" order by thl.id asc"
						set rs1 = server.CreateObject("adodb.recordset")
						rs1.open sql1,conn,1,1
						if not rs1.eof then
							do until rs1.eof
								...
								ProductAttr2=rs1("ProductAttr2")
								set rs88=server.CreateObject("adodb.recordset")
								sql88="select * from zdymx where sort1=75 and set_open=1 order by gate1 asc "
								rs88.open sql88,conn,1,1
								n = m + 4
								rs88.open sql88,conn,1,1
								do until rs88.eof
									if rs88("sorce")=1 then
										xlWorksheet.Cells(1+i,n).Value = product
'if rs88("sorce")=1 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'if rs88("sorce")=1 then
										n=n+1
'if rs88("sorce")=1 then
									elseif rs88("sorce")=2 then
										xlWorksheet.Cells(1+i,n).Value = order1
'elseif rs88("sorce")=2 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=2 then
										n=n+1
'elseif rs88("sorce")=2 then
									elseif rs88("sorce")=3 then
										xlWorksheet.Cells(1+i,n).Value = type1
'elseif rs88("sorce")=3 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=3 then
										n=n+1
'elseif rs88("sorce")=3 then
									elseif rs88("sorce")=4 then
										xlWorksheet.Cells(1+i,n).Value = unitname
'elseif rs88("sorce")=4 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=4 then
										n=n+1
'elseif rs88("sorce")=4 then
										if isOpenProductAttr then
											xlWorksheet.Cells(1+i,n).Value = ProductAttr1
'if isOpenProductAttr then
											xlWorksheet.Cells(1+i,n).font.Size=10
'if isOpenProductAttr then
											xlWorksheet.Cells(1+i,n+1).Value = ProductAttr2
'if isOpenProductAttr then
...
'if isOpenProductAttr then
									...
									elseif rs88("sorce")=5 then
										xlWorksheet.Cells(1+i,n).Value = Formatnumber(price1,StorePrice_dot_num,-1)
'elseif rs88("sorce")=5 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=5 then
										n=n+1
'elseif rs88("sorce")=5 then
									elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n).Value = Formatnumber(num1,num1_dot,-1)
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+1).Value = Formatnumber(zbcdbl(rs1("num2")),num1_dot,-1)
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+1).font.Size=10
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+2).Value = Formatnumber(zbcdbl(rs1("num3")),num1_dot,-1)
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+2).font.Size=10
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+3).Value = Formatnumber(rs1("numsqfh"),num1_dot,-1)
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+3).font.Size=10
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+4).Value = Formatnumber(rs1("numsjfh"),num1_dot,-1)
'elseif rs88("sorce")=6 then
										xlWorksheet.Cells(1+i,n+4).font.Size=10
'elseif rs88("sorce")=6 then
										n=n+5
'elseif rs88("sorce")=6 then
									elseif rs88("sorce")=7 then
										xlWorksheet.Cells(1+i,n).Value = Formatnumber(money1,num_dot_xs,-1)
'elseif rs88("sorce")=7 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=7 then
										n=n+1
'elseif rs88("sorce")=7 then
									elseif rs88("sorce")=8 then
										xlWorksheet.Cells(1+i,n).Value = ph
'elseif rs88("sorce")=8 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=8 then
										n=n+1
'elseif rs88("sorce")=8 then
									elseif rs88("sorce")=9 then
										xlWorksheet.Cells(1+i,n).Value = xlh
'elseif rs88("sorce")=9 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=9 then
										n=n+1
'elseif rs88("sorce")=9 then
									elseif rs88("sorce")=10 then
										xlWorksheet.Cells(1+i,n).Value = datesc
'elseif rs88("sorce")=10 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=10 then
										n=n+1
'elseif rs88("sorce")=10 then
									elseif rs88("sorce")=11 then
										xlWorksheet.Cells(1+i,n).Value = dateyx
'elseif rs88("sorce")=11 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=11 then
										n=n+1
'elseif rs88("sorce")=11 then
									elseif rs88("sorce")=12 then
										xlWorksheet.Cells(1+i,n).Value = date2
'elseif rs88("sorce")=12 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=12 then
										n=n+1
'elseif rs88("sorce")=12 then
									elseif rs88("sorce")=13 then
										xlWorksheet.Cells(1+i,n).Value = intro
'elseif rs88("sorce")=13 then
										xlWorksheet.Cells(1+i,n).font.Size=10
'elseif rs88("sorce")=13 then
										n=n+1
'elseif rs88("sorce")=13 then
									elseif rs88("sorce")>=14 and rs88("sorce")<=19 then
										if trim(rs88("name"))="zdy5" or trim(rs88("name"))="zdy6" then
											zdy=rs1(""&rs88("name")&"")
											if zdy="" or isnull(zdy) or not isNumeric(zdy) then zdy=0
											set rs7=server.CreateObject("adodb.recordset")
											rs7.open "select * from sortonehy where ord="&zdy&" ",conn,1,1
											if not rs7.eof then
												zdyname=rs7("sort1")
											else
												zdyname=""
											...
											zdyname=rs1(""&rs88("name")&"")
										end if
										xlWorksheet.Cells(1+i,n).Value = zdyname
										zdyname=rs1(""&rs88("name")&"")
										xlWorksheet.Cells(1+i,n).font.Size=10
										zdyname=rs1(""&rs88("name")&"")
										n=n+1
					...
					rs.close
					set rs=Nothing
					Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>采购退货列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!采购退货列表导出成功,共有"&(j-1)&"条记录!</B>"";</script>"
					set rs=Nothing
					Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
					Set fs = CreateObject("Scripting.FileSystemObject")
					tfile=Server.MapPath("采购退货列表_"&session("name2006chen")&".xls")
					...
					Response.write """><font class=""red""><strong><u>下载导出的采购退货列表导出</u></strong></font></a></p> " & vbcrlf & ""
