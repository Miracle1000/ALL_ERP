...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, state)
		app.addScriptPath app.virpath & "skin/" & Info.skin & "/js/kunumberlinks.js"
		Dim dat : dat = Split(year(now) & "-" & month(now) & "-1," & date, ",")
		app.addScriptPath app.virpath & "skin/" & Info.skin & "/js/kunumberlinks.js"
		rpt.title = "产品分类库存变动表"
		rpt.addfield "自：","dates","date1","","", dat
		rpt.addfield "产品系列：","text", "keytxt" , "", "", ""
		rpt.sql = "exec erp_report_kcbdmx_byfgck @date1_0,@date1_1, @keytxt ,''," & Info.user
	...
	Sub app_oncreate(lvw)
		...
		lvw.headers(2).formattext = "<a onclick=opentourl('@encells[1]') href='javascript:void(0)'>@value</a> <a href='../product/event.asp?A2=@cells[1]' ><img src='../images/116.png'></a>"
		lvw.headers(4).splitCell= true
		lvw.headers(3).width = 200
		lvw.headers(4).width = 200
		lvw.currsum = True
		lvw.allsum = True
		if not app.power.existsPower(23,3) Then
			for i = 1 to lvw.headers.count
				set h = lvw.headers(i)
				if instr(h.dbname,"_金额") > 0 then
					h.display = "none"
					h.width = 0
				end if
				if instr(h.dbname, "数量") > 0 then
					h.title =  replace(h.title,"_数量","")
				end if
			next
		end if
		for i = 1 to lvw.headers.count
			set h = lvw.headers(i)
			h.width = 73
			if instr(h.dbname,"_车间剩余") > 0 then
				h.display = "none"
				h.width = 0
			end if
			if instr(h.dbname,"_金额") > 0 Then
