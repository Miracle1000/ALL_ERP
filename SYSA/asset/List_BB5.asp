...
'Sub ShowGatherListHTMLFoot()
	...
	if page_count="" Or IsNumeric(page_count)=False then page_count=50
	if CurrPage="" then CurrPage=1
	if px="" or instr(1,px,"-",1)<=0 then px="0-2"
	if CurrPage="" then CurrPage=1
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=105 and sort2=11 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_105_11=rssort(0)
		intro_105_11=rssort(1)
	else
		open_105_11=0
		intro_105_11=""
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=105 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_105_1=rssort(0)
		intro_105_1=rssort(1)
	else
		open_105_1=0
		intro_105_1=""
	end if
	if intro_105_1="" then intro_105_1="0"
	if open_105_1="0" then
		strCate=strCate&" and 1=2"
	elseif open_105_1="1" then
		strCate=strCate&" and ass_addcateid in ("&intro_105_1&")"
	end if
	if intro_105_11="" then intro_105_11="0"
	if open_105_11="0" then
		strCate=strCate&" and 1=2"
	elseif open_105_11="1" then
		strCate=strCate&" and ass_addcateid in ("&intro_105_11&")"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=105 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_105_14=rssort(0)
		intro_105_14=replace(rssort(1)," ","")
	else
		open_105_14=0
		intro_105_14=""
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=105 and sort2=10 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_105_10=rssort(0)
	else
		open_105_10=0
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=105 and sort2=7 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_105_7=rssort(0)
	else
		open_105_7=0
	...
	rssort.close
	set rssort=nothing
	baseSQL="select PAGE_COUNT_NUM from ( SELECT PAGE_TOP_NUM  "&_
	"主键_ID,资产名称,资产分类,资产编号,资产型号,购置人员,存放地址,卡片类别,资产状态,净残值率,计提周期,折旧方法,资产购价_DONUM_DOSUM, "&_
	"以前是否计提,计提时间,净残值_DONUM_DOSUM,已提周期,已提金额_DONUM_DOSUM,添加时间,添加人员_ID,状态_ID,分类_ID,方法_ID,卡片_ID  "&_
	"from (SELECT a.ass_id 主键_ID,a.ass_name 资产名称,b.sort1 资产分类,a.ass_type as 分类_ID,a.ass_bh 资产编号, "&_
	"a.ass_xh 资产型号,c.name 购置人员,a.ass_cartype 卡片_ID,a.ass_method 方法_ID, "&_
	"a.ass_position 存放地址,d.c_name 卡片类别,g.sort1 资产状态,a.ass_state as 状态_ID,isnull(a.ass_jczl,0) 净残值率,cast(a.ass_cycle as varchar(10)) +' '+ case a.ass_cycle1 when 1 then '年' when 2 then '月' when 3 then '台班' when 4 then '小时' when 5 then '公里' else '' end 计提周期, "&_
	"f.set_name 折旧方法,isnull(a.ass_money,0) 资产购价_DONUM_DOSUM,(case a.ass_isjt when 1 then '是' when 0 then '否' end) 以前是否计提, "&_
	"a.ass_jttime 计提时间,isnull(a.ass_jcz,0) 净残值_DONUM_DOSUM,cast(a.ass_ycycle as varchar(10)) +' '+ case a.ass_cycle1 when 1 then '年' when 2 then '月' when 3 then '台班' when 4 then '小时' when 5 then '公里' else '' end 已提周期,round(isnull(a.ass_money2,0),"& NumDigits &") 已提金额_DONUM_DOSUM, "&_
	"a.ass_addtime 添加时间,ass_addcateid 添加人员_ID  FROM O_asset a "&_
	"LEFT join O_assSetname b ON a.ass_type=b.id "&_
	"LEFT JOIN gate c ON a.ass_cateid=c.ord "&_
	"LEFT JOIN O_assCardType d ON a.ass_cartype=d.id "&_
	"LEFT join O_assSetname e ON a.ass_state=e.id "&_
	"LEFT join O_assSetMethod f ON a.ass_method=f.id "&_
	"left join O_assStatename g on a.ass_state=g.id where ass_del=1 "&strCate&" "&_
	") table1 PAGE_ORDER_STR ) bbb where 1=1 "
	...
	ass_name=request.querystring("ass_name")
	ass_bh=request.querystring("ass_bh")
	ass_xh=request.querystring("ass_xh")
	ass_position=request.querystring("ass_position")
	ass_jczl=request.querystring("ass_jczl")
	ass_jczl1=request.querystring("ass_jczl1")
	ass_cycle=request.querystring("ass_cycle")
	ass_cycle2=request.querystring("ass_cycle2")
	ass_money=request.querystring("ass_money")
	ass_money1=request.querystring("ass_money1")
	ass_jcz=request.querystring("ass_jcz")
	ass_jcz1=request.querystring("ass_jcz1")
	ass_money2=request.querystring("ass_money2")
	ass_money3=request.querystring("ass_money3")
	ass_ycycle=request.querystring("ass_ycycle")
	ass_ycycle2=request.querystring("ass_ycycle2")
	ass_addtime=request.querystring("ass_addtime")
	ass_addtime1=request.querystring("ass_addtime1")
	ass_type=request.querystring("ass_type")
	ass_state=request.querystring("ass_state")
	ass_cartype=request.querystring("ass_cartype")
	ass_method=request.querystring("ass_method")
	strW1=replace(request.querystring("W1")," ","")
	strW2=replace(request.querystring("W2")," ","")
	strW3=replace(request.querystring("W3")," ","")
	if strW1="" then strW1="0"
	...
	for each reqPara in request.querystring
		if instr(1,reqPara,"ass_")>0 then
			execute(reqPara&"="""&request.querystring(reqPara))&""""
		end if
	next
	strCondition=""
	if m1<>"" then strCondition=strCondition&" and datediff(d,'"&m1&"',计提时间)>=0"
	if m2<>"" then strCondition=strCondition&" and datediff(d,'"&m2&"',计提时间)<=0"
	if C<>"" then
		if B=1 then
			strCondition=strCondition&" and 资产名称 like '%"&ml.De(C)&"%'"
		elseif B=2 then
			strCondition=strCondition&" and 资产编号 like '%"&ml.De(C)&"%'"
		elseif B=3 then
			strCondition=strCondition&" and 资产型号 like '%"&ml.De(C)&"%'"
		elseif B=4 then
			strCondition=strCondition&" and 存放地址 like '%"&ml.De(C)&"%'"
		end if
	end if
	if ass_name<>"" then strCondition=strCondition&" and 资产名称 like '%"&ml.De(ass_name)&"%'"
	if ass_bh<>"" then strCondition=strCondition&" and 资产编号 like '%"&ml.De(ass_bh)&"%'"
	if ass_xh<>"" then strCondition=strCondition&" and 资产型号 like '%"&ml.De(ass_xh)&"%'"
	if ass_position<>"" then strCondition=strCondition&" and 存放地址 like '%"&ml.De(ass_position)&"%'"
	if ass_jczl<>"" then strCondition=strCondition&" and 净残值率>="&ml.IsNum(ass_jczl)&""
	if ass_jczl1<>"" then strCondition=strCondition&" and 净残值率<="&ml.IsNum(ass_jczl1)&""
	if ass_cycle<>"" then strCondition=strCondition&" and 计提周期>="&ml.IsNum(ass_cycle)&""
	if ass_cycle2<>"" then strCondition=strCondition&" and 计提周期<="&ml.IsNum(ass_cycle2)&""
	if ass_money<>"" then strCondition=strCondition&" and 资产购价_DONUM_DOSUM>="&ml.IsNum(ass_money)&""
	if ass_money1<>"" then strCondition=strCondition&" and 资产购价_DONUM_DOSUM<="&ml.IsNum(ass_money1)&""
	if ass_jcz<>"" then strCondition=strCondition&" and 净残值_DONUM_DOSUM>="&ml.IsNum(ass_jcz)&""
	if ass_jcz1<>"" then strCondition=strCondition&" and 净残值_DONUM_DOSUM<="&ml.IsNum(ass_jcz1)&""
	if ass_money2<>"" then strCondition=strCondition&" and 已提金额_DONUM_DOSUM>="&ml.IsNum(ass_money2)&""
	if ass_money3<>"" then strCondition=strCondition&" and 已提金额_DONUM_DOSUM<="&ml.IsNum(ass_money3)&""
	if ass_ycycle<>"" then strCondition=strCondition&" and 已提周期>="&ml.IsNum(ass_ycycle)&""
	if ass_ycycle2<>"" then strCondition=strCondition&" and 已提周期<="&ml.IsNum(ass_ycycle2)&""
	if ass_addtime<>"" then strCondition=strCondition&" and datediff(d,'"&ass_addtime&"',添加时间)>=0"
	if ass_addtime1<>"" then strCondition=strCondition&" and datediff(d,'"&ass_addtime1&"',添加时间)<=0"
	if strW3<>"" then strCondition=strCondition&" and 添加人员_ID in ("&strW3&")"
	if ass_type<>"" then strCondition=strCondition&" and 分类_ID in ("&ass_type&")"
	if ass_state<>"" then strCondition=strCondition&" and 状态_ID in ("&ass_state&")"
	if ass_cartype<>"" then strCondition=strCondition&" and 卡片_ID in ("&ass_cartype&")"
	if ass_method<>"" then strCondition=strCondition&" and 方法_ID in ("&ass_method&")"
	dim gather
	...
	function CustomShowFields(idx,rsobj)
		...
		case "资产名称"
		ord=rsobj("主键_ID")
		if open_105_14=3 or (open_105_14=1 and IsInList(intro_105_14,rsobj("添加人员_ID"),",")) then
			Response.write "<a href=""../asset/Detail_assetUse.asp?ord="&ord&""" target=""_blank"" title=""查看资产详情"">"&rsobj("资产名称")&"</a>"
		else
			Response.write rsobj("资产名称")
		end if
		case "净残值率"
		Response.write FormatNumber(rsobj(idx).value,num_dot_xs,-1,0,0)
'case "净残值率"
		case "资产购价_DONUM_DOSUM"
		Response.write FormatNumber(rsobj(idx).value,num_dot_xs,-1,0,0)
'case "资产购价_DONUM_DOSUM"
		case "净残值_DONUM_DOSUM"
		Response.write FormatNumber(rsobj(idx).value,num_dot_xs,-1,0,0)
'case "净残值_DONUM_DOSUM"
		case "已提金额_DONUM_DOSUM"
		Response.write FormatNumber(rsobj(idx).value,num_dot_xs,-1,0,0)
'case "已提金额_DONUM_DOSUM"
		case else
...
'if CanFormatNumber(rsobj(idx).name) then
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<style>" & vbcrlf & ".IE5 .ser_top  input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""display:none;width:700px;height:500px;padding:5px;background: #fafafa;top:0px;""  closed=""true"" >" & vbcrlf & "    <form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc;width:99%;min-height:300"">" & vbcrlf & "               <table width=""100%"" border=""0""cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr onMouseOut=this.style.backgroundColor="" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf &  "                      <td align=""right"" width=""80"">添加人员</td>" & vbcrlf & "                  <td align=""left"" colspan=""3"">" & vbcrlf & "                          "
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
		...
		Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">资产名称</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_name"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ass_name
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">资产编号</td>" & vbcrlf & "             <td width=""177"" align=""left""><input name=""ass_bh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ass_bh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "              </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">资产型号</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_xh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ass_xh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">存放地址</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_position"" type=""text"" class=""searchInput"" id=""ass_position"" style=""width:110px"" onFocus=""this.select();"" value="""
		Response.write ass_position
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">资产分类</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_type1"" type=""text"" id=""ass_type1"" autocomplete=""off"" onClick=""adSearch(this,'1');""/><input name=""ass_type"" type=""hidden"" id=""ass_type"" value="""
		Response.write ass_type
		Response.write """/></td>" & vbcrlf & "                  <td align=""right"">资产状态</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_state1"" type=""text"" id=""ass_state1"" onClick=""adSearch(this,'2');""/><input name=""ass_state"" type=""hidden"" id=""ass_state"" value="""
		Response.write ass_type
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">净残值率</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_jczl"" type=""text"" class=""searchInput"" id=""ass_jczl"" onpropertychange=""formatData(this,'money');""  onFocus=""this.select();"" value="""
		Response.write ass_jczl
		Response.write """ size=""12""/>-<input name=""ass_jczl1"" onpropertychange=""formatData(this,'money');""  type=""text"" class=""searchInput"" id=""ass_jczl1"" onFocus=""this.select();"" size=""12"" value="""
		'Response.write ass_jczl
		Response.write ass_jczl1
		Response.write """/></td>" & vbcrlf & "                  <td align=""right"">计提周期</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_cycle"" type=""text"" class=""searchInput"" id=""ass_cycle"" onpropertychange=""formatData(this,'int');"" size=""12"" onFocus=""this.select();"" value="""
		Response.write ass_cycle
		Response.write """/>-<input name=""ass_cycle2"" type=""text"" class=""searchInput"" onpropertychange=""formatData(this,'int');"" id=""ass_cycle2"" size=""12"" onFocus=""this.select();"" value="""
		'Response.write ass_cycle
		Response.write ass_cycle2
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">资产购价</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_money"" type=""text"" class=""searchInput"" id=""ass_money"" onpropertychange=""formatData(this,'money');""  onFocus=""this.select();"" value="""
		Response.write ass_money
		Response.write """ size=""12""/>-<input name=""ass_money1"" onpropertychange=""formatData(this,'money');""  type=""text"" class=""searchInput"" id=""ass_money1"" onFocus=""this.select();"" size=""12"" value="""
		'Response.write ass_money
		Response.write ass_money1
		Response.write """/></td>" & vbcrlf & "                  <td align=""right"">净残值</td>" & vbcrlf & "                     <td align=""left""><input name=""ass_jcz"" type=""text"" class=""searchInput"" id=""ass_jcz"" onpropertychange=""formatData(this,'money');""  onFocus=""this.select();"" value="""
		Response.write ass_jcz
		Response.write """ size=""12""/>-<input name=""ass_jcz1"" onpropertychange=""formatData(this,'money');""  type=""text"" class=""searchInput"" id=""ass_jcz1"" onFocus=""this.select();"" size=""12"" value="""
		'Response.write ass_jcz
		Response.write ass_jcz1
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">已提金额</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_money2"" type=""text"" class=""searchInput"" id=""ass_money2"" onpropertychange=""formatData(this,'money');""  onFocus=""this.select();"" value="""
		Response.write ass_money2
		Response.write """ size=""12""/>-<input name=""ass_money3"" onpropertychange=""formatData(this,'money');""  type=""text"" class=""searchInput"" id=""ass_money3"" onFocus=""this.select();"" size=""12"" value="""
		'Response.write ass_money2
		Response.write ass_money3
		Response.write """/></td>" & vbcrlf & "                  <td align=""right"">已提周期</td>" & vbcrlf & "                   <td align=""left""><input name=""ass_ycycle"" type=""text"" class=""searchInput"" id=""ass_ycycle"" onpropertychange=""formatData(this,'int');"" size=""12"" onFocus=""this.select();"" value="""
		Response.write ass_ycycle
		Response.write """/>-<input name=""ass_ycycle2"" type=""text"" class=""searchInput"" onpropertychange=""formatData(this,'int');"" id=""ass_ycycle2"" size=""12"" onFocus=""this.select();"" value="""
		'Response.write ass_ycycle
		Response.write ass_ycycle2
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">计提时间</td>" & vbcrlf & "                   <td align=""left"" colspan=""3""><INPUT name=""ret3"" id=""ret3"" Class=""DateTimePick""onClick=""datedlg.showDateTime();"" value="""
		Response.write ret3
		Response.write """ readOnly>-<INPUT name=""ret4"" id=""ret4"" Class=""DateTimePick"" onClick=""datedlg.showDateTime();"" value="""
		'Response.write ret3
		Response.write ret4
		Response.write """ readOnly></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">添加时间</td>" & vbcrlf & "                   <td align=""left"" colspan=""3""><INPUT name=""ass_addtime"" id=""ass_addtime"" Class=""DateTimePick"" onClick=""datedlg.showDateTime();"" value="""
		Response.write ass_addtime
		Response.write """ readOnly>-<INPUT name=""ass_addtime1"" id=""ass_addtime1"" Class=""DateTimePick"" onClick=""datedlg.showDateTime();"" value="""
		'Response.write ass_addtime
		Response.write ass_addtime1
		Response.write """ readOnly></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">折旧方法</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       "
		sql7="select * from O_assSetMethod order by set_sort asc "
		set rs7=conn.execute(sql7)
		if not rs7.eof then
			do while not rs7.eof
				Response.write "" & vbcrlf & "                       <input name=""ass_method"" type=""checkbox"" value="""
		...
		set rs7=nothing
		Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "           </tr>           " & vbcrlf & "                <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">卡片类别</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       "
		sql7="select a.*,b.sort1 from O_assCardType a left join O_assStatename b on a.C_assState=b.id order by a.C_addtime asc "
		set rs7=conn.execute(sql7)
		if not rs7.eof then
			do while not rs7.eof
				Response.write "" & vbcrlf & "                       <input name=""ass_cartype"" type=""checkbox"" value="""
				Response.write rs7("id")
				Response.write """>"
				Response.write rs7("C_Name")
				rs7.movenext
			loop
		end if
		set rs7=nothing
		Response.write "</td>" & vbcrlf & "                </tr>           " & vbcrlf & "" & vbcrlf & "                </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:20px;padding-top:8px; padding-bottom:8px; padding-right:8px"">" & vbcrlf & "               <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf &  "         <input type=""hidden"" name=""page_count"" value="
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """>" & vbcrlf & "                <a class=""easyui-linkbutton"" icon=""icon-search"" href=""javascript:void(0)"" onClick=""document.getElementById('searchform').submit();return false;"">检索</a>" & vbcrlf & "               <a class=""easyui-linkbutton"" icon=""icon-cancel"" href=""javascript:void(0)"" onClick=""$('#dd').window('close');"">取消</a>" & vbcrlf & "                <a class=""easyui-linkbutton"" icon=""icon-undo"" href=""javascript:void(0)"" onClick=""resetForm();"">清空</a>" & vbcrlf & " </div>" & vbcrlf & "  </form>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;z-index:99999;display:none;padding:2px 2px 2px 2px;background-color:lightblue"">" & vbcrlf & "<iframe src="""" name=""adsIF"" id=""adsIF"" width=""195px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<form target=""ssIF"" action=""../asset/Search_All_Search.asp"" id=""ssIFForm"" style=""display:none"" method=""post"">" & vbcrlf & "</form>" & vbcrlf & ""
	end sub
	sub ShowGatherListHTMLFootExtend()
		Response.write "" & vbcrlf & "<script type=""text/Javascript"" src=""../car/Calendar.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """></script>" & vbcrlf & "<script language =""javascript"">" & vbcrlf & "function adSearch(obj,sel_type)" & vbcrlf & "{" & vbcrlf & "  " & vbcrlf & "        var tleft=obj.offsetLeft;" & vbcrlf & "       var ttop=$(obj).offset().top;" & vbcrlf & "   var theight = obj.clientHeight;" & vbcrlf & " while (obj = obj.offsetParent){tleft+=obj.offsetLeft;}" & vbcrlf & "     var sdivobj=document.getElementById(""adsDiv"");" & vbcrlf & "    sdivobj.style.display=""none"";" & vbcrlf & "     sdivobj.style.left=tleft+""px"";" & vbcrlf & "    sdivobj.style.top=ttop+theight+""px"";" & vbcrlf & "      document.getElementById(""ssIFForm"").action=""../asset/Search_All_Search1.asp?sel_type=""+sel_type;" & vbcrlf & " document.getElementById(""ssIFForm"").target=""adsIF"";" & vbcrlf & " document.getElementById(""ssIFForm"").submit();" & vbcrlf & "     sdivobj.style.display=""inline"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "var strW1="","
		'Response.write Application("sys.info.jsver")
...
'sub CurstomButtons()
		...
		Response.write ">每页显示200条</option>" & vbcrlf & "</select>" & vbcrlf & "<!--<INPUT name=""ret"" id=""ret"" style=""COLOR: #cccccc"" onClick=""setday(this,this,true)"" value="""
		Response.write "selected"
		Response.write ret
		Response.write """ size=""20"" readOnly>-<INPUT name=""ret2"" id=""ret2"" style=""COLOR: #cccccc"" onClick=""setday(this,this,true)"" value="""
		'Response.write ret
		Response.write ret2
		Response.write """ size=""20"" readOnly><input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """><input type=""hidden"" name=""sflg"" value=""1""/>-->" & vbcrlf & "<select name=""B"">" & vbcrlf & "<option value=""1"" "
		Response.write px
		if B=1 then
			Response.write "selected"
		end if
		Response.write ">资产名称</option>" & vbcrlf & "<option value=""2"" "
		if B=2 then
			Response.write "selected"
		end if
		Response.write ">资产编号</option>" & vbcrlf & "<option value=""3"" "
		if B=3 then
			Response.write "selected"
		end if
		Response.write ">资产型号</option>" & vbcrlf & "<option value=""4"" "
		if B=4 then
			Response.write "selected"
		end if
		Response.write ">存放地址</option>" & vbcrlf & "</select>" & vbcrlf & "<input type=""text"" name=""C"" value="""
		Response.write C
