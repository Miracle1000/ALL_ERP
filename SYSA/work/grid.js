...
(function(window, document, undefined) {
	...
	var Grid = function(element, options) {
		if ((this.element = (typeof(element) === "string") ? $(element) : element)) {
			...
			this.columns = 0;
			this.columnWidths = [];
			this.cellData = { head : [], body : [], foot : [], initwidths : [] };
	...
	GridProto.convertData = function(data) {
		...
		if ((base = data.Head || data.Body || data.Foot || null)) {
			...
			cols = this.columns;
			if (data.Head) {
				this.convertDataItem(h, data.Head, "<DIV class='tableHeaderbg g_C g_HR g_R", cols, this.options.allowColumnResize);
			} else {
	...
	GridProto.generateGridBody = function() {
		...
		if (this.hasBody) {
			...
			this.bodyStatic.innerHTML = "<DIV class='g_EmptySetMsg'></DIV>";
		}
	};
	...
	//////////////////////////////////////////////////////////////////////////////////
	GridProto.getInitColWidth = function(head) {
		var headHtml = head.toString();
		var i1 = headHtml.indexOf("width=\"");
		var h = 0;
		if(i1==-1) {
			i1 = headHtml.indexOf("width=");
			h = 1;
		}
		if(i1 > 0) {

			var i2 = headHtml.substr(i1+7-h).indexOf(h == 1 ? " " : "\"");
			if(i2 > 0 ) {
				return  headHtml.substr(i1+7-h).substr(0, i2) + "px";
			}
		}
		return "auto";
	}
	...
	GridProto.generateGridSection = function(cols) {
		...
		while (colIdx) {
			if ((--colIdx) < fixedCols) {
				var w = this.cellData.initwidths[colIdx];
				if(!w) {
					w = GridProto.getInitColWidth(this.cellData.head[colIdx]);
					this.cellData.initwidths[colIdx] = w;
				}
	...
	GridProto.alignColumns = function(reAlign, fromInit) {
		...
		for (var i=0, len=this.columns; i<len; i++) {
			...
			if(this.cellData.initwidths[i]!="auto") {
				colWidth = this.cellData.initwidths[i].replace("px","");
				this.cellData.initwidths[i] = "auto";
			} 
	...
	GridProto.resizeColumn = function(event) {
		var clientX = getEventPositions(event || window.event, "client").x, 
		    ...
		    newWidth = Math.max(15, (xDif > 0) ? this.tmp.origWidth*1 + xDif*1 : this.tmp.origWidth - Math.abs(xDif)), 
