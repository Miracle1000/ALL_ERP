...
'if strW3<>"" Then
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>票据来源明细导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "</HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""place"">票据来源明细导出</td>" & vbcrlf & "      <td>&nbsp;</td>" & vbcrlf & "      <td align=""right"">&nbsp;</td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出票据来源明细,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Str_Result = ""
	Str_Result2 = ""
	m1=request("ret")
	m2=request("ret2")
	IsUsePJLY = request("IsUserPJLY")
	GetType=Request("GetType")
	If GetType = "" or IsNull(GetType) Then GetType = 0
	If IsUsePJLY <> "" then
		Str_Result=Str_Result & " where a.invoicely = " & IsUsePJLY
		Str_Result2=Str_Result2 & " where id = " & IsUsePJLY
	end if
	If m1 <> "" and Not IsNull(m1) Then
		If m2 <> "" and Not IsNull(m2) Then
			Select Case GetType
			Case 1
			MyWhereSql = " and p.invoicedate between '" & m1 & "' and '" & m2 & "'"
			MyWhereSql2 = " and 1<>1"
			Str_Result2 = " where 1<>1"
			Case 2
			MyWhereSql = " and 1<>1"
			MyWhereSql2 = " and invoicedate between '" & m1 & "' and '" & m2 & "'"
			Str_Result2 = " where 1<>1"
			Case Else
			MyWhereSql = " and p.invoicedate between '" & m1 & "' and '" & m2 & "'"
			MyWhereSql2 = MyWhereSql
			End Select
		else
			MyWhereSql = " and p.invoicedate < '" & m1 & "'"
			MyWhereSql2 =" and invoicedate < '" & m1 & "'"
		end if
	else
		MyWhereSql = " and 1<>1"
		MyWhereSql2 = MyWhereSql
	...
	if px=1 then
		px_Result=" order by TName desc"
	elseif px=2 then
		px_Result=" order by TName asc"
	elseif px=3 then
		px_Result=" order by  money1 desc"
	elseif px=4 then
		px_Result=" order by money1 asc"
	elseif px=5 then
		px_Result=" order by GName desc"
	elseif px=6 then
		px_Result=" order by GName asc"
	elseif px=7 then
		px_Result=" order by MyType desc"
	elseif px=8 then
		px_Result=" order by MyType asc"
	end if
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(1).ColumnWidth=40
			xlApplication.ActiveSheet.Columns(1).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(2).ColumnWidth=35
			...
			xlWorksheet.Cells(1,1).Value = "客户|供应商|名称"
			xlWorksheet.Cells(1,1).font.Size=10
			xlWorksheet.Cells(1,1).font.bold=true
			xlWorksheet.Cells(1,2).Value = "票据来源"
			xlWorksheet.Cells(1,2).font.Size=10
			xlWorksheet.Cells(1,2).font.bold=true
			xlWorksheet.Cells(1,3).Value = "操作金额"
			xlWorksheet.Cells(1,3).font.Size=10
			xlWorksheet.Cells(1,3).font.bold=true
			xlWorksheet.Cells(1,4).Value = "经手人"
			xlWorksheet.Cells(1,4).font.Size=10
			xlWorksheet.Cells(1,4).font.bold=true
			xlWorksheet.Cells(1,5).Value = "操作类型"
			xlWorksheet.Cells(1,5).font.Size=10
			xlWorksheet.Cells(1,5).font.bold=True
			xlWorksheet.Range("A2:M2").Borders.LineStyle=1
			Response.Flush
			set rs=server.CreateObject("adodb.recordset")
			sql="Select a.*,b.sort1 "&_
			"from ( " & _
			"Select b.Name + '_(供应商)' as TName,p.money1,c.Name as Gname,'收票' as Mytype,isnull(invoicely,0) as invoicely "&_
			"from ( " & _
			"from PayoutInvoice p "&_
			"Left join Tel as b on b.ord = p.company "&_
			"Left join gate as c on c.ord = p.addcate "&_
			"where isnull(invoicely,0)>0 and p.del = 1 and isinvoiced=1 " & MyWhereSql & "  "&_
			"union all " & _
			"Select b.Name + '_(客户)' as TName,a.money1 * -1 as money1,c.Name,'开票' as Mytype,isnull(invoicely,0) as invoicely "&_
			"union all " & _
			"from paybackinvoice as a " & _
			"Left join Tel as b on b.ord = a.company " & _
			"Left join gate as c on c.ord = a.addcate " & _
			"where isnull(invoicely,0)>0 and a.del = 1 and a.isinvoiced=1 " & MyWhereSql2 & " " & _
			") as a  " & _
			"Left join sortonehy as b on b.id = a.invoicely" & Str_Result & _
			"union all " & _
			"Select '期初' as TName,NowMoney,'-' as Gname,'录入' as MyType,id as invoicely,sort1 from sortonehy  " & Str_Result2 & px_Result
			union all  & _
			"rs.open sql,conn,1,1"
			C1=rs.recordcount
			i=1
			while rs.eof = false
				xlWorksheet.Cells(1+i,1).Value =Rs("TName")
'while rs.eof = false
...
'while rs.eof = false
				xlWorksheet.Cells(1+i,4).Value = Rs("Gname")
'while rs.eof = false
				xlWorksheet.Cells(1+i,4).font.Size=10
'while rs.eof = false
				xlWorksheet.Cells(1+i,5).Value = Rs("Mytype")
'while rs.eof = false
...
'while rs.eof = false
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>票据来源明细导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜您!票据来源明细导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("票据来源明细_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的 票据来源明细</u></strong></font></a></p>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
