...
'Dim limitCount : limitCount = ZbRuntime.LimitB
	...
	act = request("act")
	LogFileName = "../manager/setblock.asp"
	select case act
	case "list"
	userid = request("userid")
	Call macBind_list(userid)
	case "add","add2"
	userid = request("userid")
	ord = request("ord")
	user = request("user")
	Call macBind_add(act, userid, ord, user)
	case "save"
	Call macBind_save()
	case "bindUse"
	ord = request("ord")
	useBind = request("useBind")
	Call macBind_bindUse(ord, useBind)
	Case "openBinding"
	ord = request("ord")
	isOpen = request("isOpen")
	Response.write macBind_bindOpen(ord,isOpen)
	case "del"
	ord = request("ord")
	Response.write macBind_del(ord)
	case "getBindNum"
	userid = request("userid")
	Response.write macBind_num(userid)
	case "selectUser"
	Call macBind_selectUser()
	case "noUse"
	userid = request("userid")
	Call macBind_list(userid)
	case "checkAllBind"
	...
	case "checkmacsn"
	macsn = trim(request("macsn"))
	Response.write macBind_chkMacsn(macsn)
	Case "SaveGPS"
	userid = request("userid")
	open = request("open")
	If isnumeric(open&"") = False Then
		open = "0"
	end if
	conn.execute("update gate set GPS_Open=" & open & " where ord="& userid)
	end select
	call db_close
%>
