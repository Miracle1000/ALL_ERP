...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_101_10=1 then
		list=" and Exit Sub_addcateid in ("&intro_101_10&")"
	else
		list=" and Exit Sub_addcateid=0"
	end if
	Str_Result=" where Exit Sub_del=1 "&list&""
	Ptr_Result=""
	if open_101_1=3 then
	elseif open_101_1=1 then
		Str_Result=Str_Result&" and Exit Sub_addcateid in ("&intro_101_1&")"
	else
		Str_Result=Str_Result&" and Exit Sub_addcateid=0"
	end if
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by Exit Sub_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by Exit Sub_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by Exit Sub_time desc"
	elseif px="4" then
		Ptr_Result=" order by Exit Sub_time asc"
	elseif px="5" then
		Ptr_Result=" order by Exit Sub_btime desc"
	elseif px="6" then
		Ptr_Result=" order by Exit Sub_btime asc"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
	...
	If Len(D&"")<>0 and D<>0 Then
		Str_Result=Str_Result&" and ret_state="&D&" "
	end if
	if ret<>"" then
	...
	if C<>"" then
		if B="ffzt" then
			Str_Result=Str_Result&" and Exit Sub_title like '%"&C&"%'"
		elseif B="ffbh" then
			Str_Result=Str_Result&" and Exit Sub_cateid in (select ord from gate where name like '%"&C&"%')"
		elseif B="ffry" then
			Str_Result=Str_Result&" and ret_bcateid in (select ord from gate where name like '%"&C&"%')"
		elseif B="ffbz" then
			Str_Result=Str_Result&" and Exit Sub_note  like '%"&C&"%'"
		end if
	end if''
	If remind <> "" Then
	...
	j=1
	k=1
	sql="select * from O_proReturn "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "返还列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			ret_title=rs("ret_title")
			ret_time=rs("ret_time")
			ret_cateid=rs("ret_cateid")
			...
			rs8.close
			set rs8=nothing
			ret_bcateid=rs("ret_bcateid")
			if ret_bcateid="" or isnull(ret_bcateid) then ret_bcateid=0
			sql8="select  * from gate where ord="&ret_bcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				ret_bcateid=rs8("name")
			end if
			rs8.close
			set rs8=nothing
			ret_btime=rs("ret_btime")
			if rs("ret_state")=2 then
				ret_state="已返还"
			elseif rs("ret_state")=1 then
				ret_state="未返还"
			elseif rs("ret_state")=3 then
				ret_state="已否决"
			end if
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,1).Value =ret_title
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,1).font.Size=10
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,2).Value =ret_time
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,2).font.Size=10
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,3).Value = ret_cateid
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,3).font.Size=10
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,4).Value = ret_bcateid
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,4).font.Size=10
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,5).Value = ret_state
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,5).font.Size=10
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,6).Value = ret_note
			ret_note=rs("ret_note")
			xlWorksheet.Cells(1+i,6).font.Size=10
			ret_note=rs("ret_note")
			sql8="select a.*,f.prod_ph,f.prod_xlh,f.prod_sctime,f.prod_ystime,c.prod_name,c.prod_bh,c.prod_xh,d.sort1,e.pro_name,e.pro_type,h.sto_name from O_proRetList a "&_
			" left join O_proOutList b on a.prod_id=b.id "&_
			" left join O_kuInList f on b.prod_id=f.id "&_
			" left join O_product c on f.prod_id = c.id"&_
			" left join O_store h on h.id=f.sto_id "&_
			" left join O_OfficeType e on c.prod_type=e.id "&_
			" left join sortonehy d on f.prod_unit=d.id where a.ret_fid="&rs("id")
			...
			if not rs8.eof then
				do while not rs8.eof
					...
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,12).Value =prod_ph
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,12).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,13).Value =prod_xlh
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,13).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,14).Value =prod_sctime
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,14).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,15).Value =prod_ystime
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,15).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,16).Value =sto_name
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,16).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,17).Value =pro_name
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,17).font.Size=10
					sto_name=rs8("sto_name")
					xlWorksheet.Cells(2+i,18).Value =ret_intro
					sto_name=rs8("sto_name")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>返还列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!返还列表导出成功,共有"&(i-1)&"条记录!"&(j-1)&"条记录"&(k-1)&"条明细!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("返还列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的返还列表</u></strong></font></a></p>"
