...
'if len(num1 & "") > 0 then
			...
			Response.write id
			Response.write id
			Response.write """" & vbcrlf & "                onblur=""if(!this.value){this.value=this.defaultValue;this.style.color='#000'}""" & vbcrlf & "                onfocus=""if(this.value==this.defaultValue){this.value='';this.style.color='#000'}""" & vbcrlf & "                onkeyup=""this.value=this.value.replac				e(/[^\d\.]/g,'');checkDot('pricejy"
			Response.write id
			Response.write "','"
			Response.write StorePrice_dot_num
			Response.write "');eval('moneyjyall_"
...
'dim listid : listid= contractlistmxid
		...
		Public Sub showWithInputKD()
			...
			Response.write ","
			Response.write jf
			Response.write ",this); " & vbcrlf & "            style=""height:19px;width:40px;solid;font-size:9pt;text-align:right;"" " & vbcrlf & "            dataType=""Range"" msg=""必须填写单价"" min=""-99999999999"" max=""999999999999""" & vbcrlf & "            /> " & vbcrlf & "            <img src=""../images/112.png""  o				nmouseover=callServer2('test"
			Response.write id
			Response.write "','"
			Response.write ord
			Response.write "','"
...
'If Len(Trim(invoiceType)&"")="0" Or invoiceType = 0 then
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/JavaScript"" src=""search.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFiles.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<SCRIPT language=JavaScript1.2>" & vbcrlf & " // 一个简单的测试是否IE浏览器的表达式" & vbcrlf & "   isIE = (document.all ? true : false);" & vbcrlf & "   // 得到IE中各元素真正的位移量，即使这个元素在一个表格中" & vbcrlf & " function getIEPosX(elt) { return getIEPos(elt,""Left""); }" & vbcrlf & "  function getIEPosY(elt) { return getIEPos(elt,""Top""); }" & vbcrlf & "        function getIEPos(elt,which) {" & vbcrlf & "          iPos = 0" & vbcrlf & "                while (elt!=null) {" & vbcrlf & "                     iPos += elt[""offset"" + which]" & vbcrlf & "                     elt = elt.offsetParent" & vbcrlf & "          }" & vbcrlf & "               return iPos" & vbcrlf & "     }" & vbcrlf & "</SCRIPT>" & vbcrlf & "<style>" & vbcrlf & "      .dataCell{" & vbcrlf & "              border-bottom:#ccc 1px solid;" & vbcrlf & "           border-left:#ccc 1px solid;" & vbcrlf & "             border-right:#ccc 1px solid;" & vbcrlf & "    }" & vbcrlf & "       .inputCell{" & vbcrlf & "             overflow-x:hidden" & vbcrlf & "       }" & vbcrlf & "       .proTd{" & vbcrlf & "               text-align:center !important;background-color:#fff;" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "       .sublist-table {" & vbcrlf & "                width: 100%;" & vbcrlf & "            padding: 0;" & vbcrlf & "             border: 1px solid #ccc;" & vbcrlf & "         border-collapse: collapse;" & vbcrlf & "      }" & vbcrlf & "	.sublist-table td {" & vbcrlf & "             border: 1px solid #ccc;" & vbcrlf & "         padding:5px;" & vbcrlf & "    }" & vbcrlf & "</style>" & vbcrlf & "" & vbcrlf & "</head>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	title = "设计明细"
	act =  request("act")
	initact = ""
	extTitle = "选择"
	sql = ""
	If instr(act,"change")>0 Then extTitle = "变更"
	If instr(act,"content")>0 Then
		initact = "content"
		bid = deurl(request("top"))
		sql = "select id ,isnull(Designer,0) as Designer from design where id=" & bid
	ElseIf instr(act,"design")>0 Then
		bid = deurl(request("mxid"))
		initact = "design"
		title = "设计子件"
		sql = "select d.id,isnull(d.Designer,0) as Designer from designlist l inner join design d on d.id=l.design where l.id = "  & bid
	end if
	If bid &""="" Then bid = 0
	billid = 0
	Designer = 0
	...
	canAddNotice = 0
	if ZBRuntime.MC(77000) And sdk.power.existsPowerIntro(5032 , 13, Designer) Then canAddNotice = 1
	Response.write "" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false onmouseover=""jQuery('#info_show_div').hide();"" onload=""try{initProductList('"
	Response.write initact
	Response.write "',"
	Response.write bid
	...
	cols(i)= "<td dbname=""index"" align=""center"" class=""proTd"" style=""width:60px;""><strong>序号</strong></td>"
	i=i+1
	ReDim Preserve cols(i)
	cols(i)= "<td dbname=""title"" align=""center"" class=""proTd""><strong>产品名称</strong></td>"
	i=i+1
	'cols(i)= "<td dbname=""title"" align=""center"" class=""proTd""><strong>产品名称</strong></td>"
	ReDim Preserve cols(i)
	cols(i)= "<td dbname=""order1"" align=""center"" class=""proTd""><strong>产品编号</strong></td>"
	i=i+1
	'cols(i)= "<td dbname=""order1"" align=""center"" class=""proTd""><strong>产品编号</strong></td>"
	ReDim Preserve cols(i)
	cols(i)= "<td dbname=""type1"" align=""center"" class=""proTd""><strong>产品型号</strong></td>"
	i=i+1
	'cols(i)= "<td dbname=""type1"" align=""center"" class=""proTd""><strong>产品型号</strong></td>"
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from zdy where sort1 = 21 and set_open = 1 and 1=0 order by gate1 "
	rs.open sql,conn,1,1
	do until rs.eof
		ReDim Preserve cols(i)
		cols(i)= "<td dbname=""" & rs("name") & """ align=""center"" class=""proTd""><strong>" & rs("title") & "</strong></td>"
		i=i+1
		rs.movenext
	loop
	rs.close
	set rs=Nothing
	ReDim Preserve cols(i)
	cols(i)= "<td dbname=""link"" align=""center"" class=""proTd""><strong>操作</strong></td>"
	i=i+1
	'cols(i)= "<td dbname=""link"" align=""center"" class=""proTd""><strong>操作</strong></td>"
	Dim returnUnit : returnUnit = False
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""5"" >" & vbcrlf & "      <tr>" & vbcrlf & "            <td valign=""center"" style='width:219px;overflow:hidden;'>" & vbcrlf & "         <div class=""leftPageBg"" style="" HEIGHT: 40px; OVERFLOW: hidden"">" & vbcrlf & "                    <div style=""	HEIGHT: 40px; OVERFLOW: hidden""> " & vbcrlf & "                     <div class=""tableTitleLinks"" style=""MARGIN-TOP: 13px; WHITE-SPACE: nowrap; COLOR: #5555aa; MARGIN-LEFT: 1px; FONT-SIZE: 14px; FONT-WEIGHT: bold;padding-left: 20px;"">" & vbcrlf & "                               "
'Dim returnUnit : returnUnit = False '
	Response.write title
	Response.write "产品选择" & vbcrlf & "                          </div>" & vbcrlf & "                      </div>" & vbcrlf & "          </div>" & vbcrlf & "          <div style='border:1px solid #ccc;background:#fff'>" & vbcrlf & "                     <iframe style=""MARGIN-TOP: 0px; WIDTH: 215px; HEIGHT: 500px;"" id=""proSelectFrame"" border=""0"" src=""select_product.asp?hasAdvSearch=1"" frameborder=""0"" scorlling=""no""></iframe>" & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           <td valign=""top"">" & vbcrlf & "                 <form name=""date"" id=""demo"" method=""post"" class='pagefrm' action=""savelistadd.asp"" onSubmit=""BindMxpxData(this);return Validator.Validate(this,2)&&checkValue()&&DelUnusedFilesBeforeSubmit()"">" & vbcrlf & "                   <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "                         <tr>" & vbcrlf & "                                    <td width=""100%"" valign=""center"" >" & vbcrlf & "                                          <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                                                    <tr>" & vbcrlf & "                                                            <td class=""place"">"
	'Response.write title
	Response.write extTitle
	...
	if open_21_13=1  then
		...
		Response.write pwurl(top)
		Response.write "','newwproductin','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" class=""anybutton"" style=""width:65px;""/>"
		'Response.write pwurl(top)
	end if
	Response.write "</td>" & vbcrlf & "                                                                <td align=""right"">&nbsp;</td>" & vbcrlf & "                                                             <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                                                      </tr>" & vbcrlf & "                                           </table>" & vbcrlf & "                                                <table id=""content"" class=""sublist-table"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "                                                    <tr class=""top"" id"
	For i = 0 To ubound(cols)
		Response.write cols(i)
	next
	Response.write "" & vbcrlf & "                                                     </tr>" & vbcrlf & "                                           </table>" & vbcrlf & "" & vbcrlf & "                                                <span id=""trpx0"">" & vbcrlf & "                                                 <table width=""100%"" border=""0""  cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;table-layout: fixed;"">" & vbcrlf & "<tr >" & vbcrlf & "                                                                   <td height=""30"" style=""border-bottom:#ccc 1px solid;border-left:#ccc 1px solid;border-right:#ccc 1px solid;"">无产品明细！</td>" & vbcrlf & "                                                              </tr>" & vbcrlf & "                                                   </table>" & vbcrlf & "                                                </span>" & vbcrlf & "                                 </td>" & vbcrlf & "                           </tr>" & vbcrlf	& "                               <tr>" & vbcrlf & "                                    <td class='page'>" & vbcrlf & "                                               <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                                                 <tr>" & vbcrlf & "                                                            <td height=""50"" colspan=""2"">&nbsp;&nbsp;<span id=""all_num""><input name=""636r"" type=""hidden"" size=""6"" value="""
	Response.write i+1
	Response.write """ id=""alli""></span>" & vbcrlf & "                                                         <input type=""button"" name=""Submit7622"" value=""保存"" class=""page"" onclick=""SaveList('"
	Response.write act
	Response.write "',"
	Response.write bid
	Response.write ",'"
	...
	Response.write "',"
	Response.write canAddNotice
	Response.write ")""/>" & vbcrlf & "                                                              &nbsp;&nbsp;&nbsp;&nbsp;" & vbcrlf & "                                                                <input type=""reset"" value=""重填""  class=""page"" name=""B522"" />" & vbcrlf & "                                                           </td>" & vbcrlf & "                                                           <td width=""65%"">&nbsp;<input name=""top""   type='hidden' value="""
	Response.write top
	Response.write """></td>" & vbcrlf & "                                                   </tr>" & vbcrlf & "                                           </table>" & vbcrlf & "                                        </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                        "
	action1="设计明细选择"
	call close_list(2)
	Response.write "" & vbcrlf & "                     </form>" & vbcrlf & "         </td>" & vbcrlf & "   </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "#pro_tab{" & vbcrlf & "       width:100%;height:22px;padding-top:5px;" & vbcrlf & " background:url(../skin/default/images/tabstrip/bg001.gif) repeat-x left top;" & vbcrlf & "  text-align:center;" & vbcrlf & "}" & vbcrlf & "#pro_tab span{" & vbcrlf & "       width:45%;height:21px;line-height:21px;border:1px solid #A7BBD6;border-bottom:none;display:inline-block;cursor:pointer;" & vbcrlf & " background:url(../skin/default/im	ages/tabstrip/itembg.gif) #c5d6f2 repeat-x;" & vbcrlf & " margin:0px;" & vbcrlf & "     padding:0px;" & vbcrlf & "}" & vbcrlf & "</style>"
	'call close_list(2)
