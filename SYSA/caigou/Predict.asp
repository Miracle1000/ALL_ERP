...
'items = rpt.Settings.item(i)
	...
	Sub OnReportBottom
		Response.write "<style>"
		Response.write "#lvw_mlistvw td.lvw_smceldb {color:#5b7cae;}#fieldsBox{border-bottom:0}#showReportSettings{height:477px!important}" & vbcrlf
		Response.write "<style>"
		Response.write "@media print{" & vbcrlf
		Response.write "       #toolbar1{display:none;}" & vbcrlf
		Response.write "       #comm_itembarbg{display:none;}" & vbcrlf
		Response.write "}" & vbcrlf
		Response.write ".lvwheader TH {padding-left:2px;padding-right:2px;}" & vbcrlf
		Response.write "}" & vbcrlf
		Response.write "</style>"
	end sub
	Sub onReportInit(rpt, lt)
		...
		rpt.addfield "查询日期：","date","d1","","", d1
		rpt.addfield "交货日期：","date","d2","","", d2
		rpt.showtoppagesize = False
		rpt.aSearch.AddField "查询日期","date","date1",""
		rpt.aSearch.AddField "交货日期","date","date2",""
		rpt.aSearch.AddField "产品名称", "text", "cpmc", ""
		rpt.aSearch.AddField "供应商", "text", "gys", ""
		rpt.aSearch.AddField "产品编号", "text", "cpbh", ""
		rpt.aSearch.AddField "产品型号", "text", "cpxh", ""
		rpt.aSearch.AddField "产品系列", "cpfl", "cpfl", ""
		rpt.aSearch.addZdyFields 21
		rpt.onlyadSearchModel = True
		rpt.BillListUIModel=True
		rpt.showtoppagesize = True
		rpt.sql = "exec erp_cg_predict "& Info.User&",@d1,@d2,@date1,@date2,@cpmc,@gys,@cpbh,@cpxh,@cpfl,@zdy21_1,@zdy21_2,@zdy21_3,@zdy21_4,@zdy21_5,@zdy21_6,&SearchMode,&excelmode,&headerconfig, &pagesize, &pageindex,'' "
		rpt.canexcel = app.power.existsPower(25,10)
		rpt.canprint = app.power.existsPower(25,7)
	end sub
	Dim open_21_14
	Sub app_onCreateList(ByVal lvw,rptdata)
		Dim SearchMode : SearchMode = app.getint("asrcm")
		Dim date1, date2 ,i ,ii,n ,rs
		If SearchMode = 0 Then
			date1 = rptdata.GetSearchText("d1")
			date2 = rptdata.GetSearchText("d2")
		else
			date1 = rptdata.GetSearchText("date1")
			date2 = rptdata.GetSearchText("date2")
		end if
		if isdate(date1) = false then date1 = date()
		if isdate(date2) = false then date2 = dateadd("m",1,date)
		If datediff("d",date1,date2)<=0 Then date2=cdate(dateadd("d",1,date1))
		lvw.PreMsg = "执行过程出现问题，公式设置不正确。<a href='predict.asp?__msgId=reset'>还原公式</a>"
		lvw.CanPageSize = True
		lvw.checkbox = false
		lvw.indexbox = false
		lvw.addlink = ""
		lvw.allsum = False
		...
		lvw.HeaderPageSizeUI=False
		lvw.ServerConfig = True
		lvw.isPageReckon = false
		lvw.RowSplitFields = "sort1"
		lvw.excelcallbackproc = "App_OnListviewExcel"
		if app.power.existsPower(25,18) then
			lvw.isshow_anotherName=True
			lvw.isshow_formula=True
		end if
		lvw.FaStr="N="&datediff("d",date1,date2)
		If isEmpty(open_21_14) Then   open_21_14 = app.power.existsPower(21,14)
		Dim arrTitle , other ,formula
		arrTitle = split("产品名称|供应商|编号|型号|单位|库存上限|库存下限|过去一年的出库量|过去一年月均销量|库存下限合理性测试|现库存|采购未交货数量|库存及未交货周转月数|采购预测|预计销售周期|采购订单","|")
		other = Split("A|B|C|D|E|F|G|H|I|T","|")
		formula = Split("A|B|C|C/12|B/C/12|F|G|(F+G)/D|A+C/365*N-F-G|T","|")
		other = Split("A|B|C|D|E|F|G|H|I|T","|")
		lvw.headers(1).cssName = "lvw_index"
		Dim col
		For i=1 To 16
			Set col = lvw.headers(i)
			col.title = arrTitle(i-1)
			Set col = lvw.headers(i)
			if i = 1 And open_21_14 Then col.formatText="<a href='../product/content.asp?ord=@cells[25]' target='_blank'>@value</a>"
			If i = 2 Then col.setLink "company","gysOrd", "gysCateid",  26, -13
			col.cangroupsum = False
			If i>5 And i<16  Then
				col.evalName = other(i-6)
'If i>5 And i<16  Then
				col.evalcode = formula(i-6)
'If i>5 And i<16  Then
				col.dbtype = "number"
				col.cangroupsum = True
				col.width = "120"
			ElseIf i<5 Then
				col.dbtype="str"
			end if
			If i=16 Then lvw.headers(16).evalcode = "getcgnum(I,H,T)" : lvw.headers(16).dbtype = "number" : lvw.headers(16).cangroupsum = True
		next
		i = 17
		Set rs=cn.execute("select id,title,name,sort,set_open,gl,js,dc,dr,tj,gate1,set_open from zdy where sort1=21 order by replace(name,'zdy','') asc")
		While rs.eof = False
			Set col = lvw.headers(i)
		...
		For i=23 To 27
			Set col = lvw.headers(i)
			col.display="none"
			If i = 27 Then col.evalName = "N"
		next
	end sub
	Function getcgnum(I,H,T)
		Dim s
		If I<0 Or H>T Then
			s = 0
		else
	...
	Sub app_reset
		dim path
		path = replace(replace(request.ServerVariables("url"),"/",""),".asp","")
		cn.execute("delete from erp_sys_listviewConfig where path='" & path & "' and (attrn='anotherName' or attrn='formula') and lvwid='yc_mainlist'")
		cn.close
		Response.redirect "predict.asp"
	end sub
...
'If i>5 And i<16  Then
		...
		next
		n=17
		Set rs=cn.execute("select id,title,name,sort,set_open,gl,js,dc,dr,tj,gate1 from zdy where sort1=21 order by replace(name,'zdy','') asc")
		While rs.eof = False
			Set col = lvw.headers(n)
			If rs("dc")=0 or rs("set_open")=0 Then
				col.width = 0
				col.display = "none"
				col.canSum=False
			else
	...
	Sub lvw_onGroupSumCell(ByVal lvw, ByVal cell, ByVal gn , ByVal gv , ByRef value, ByVal cells)
		Dim d
		Select Case cell.dbname
		Case "title"
		value = "分类合计"
		End Select
	end sub
	Sub erp_sys_OnListviewError(ByVal lvw, ByRef ErrNumber, ByRef ErrSourece,  ByRef errDescription, ByVal sql)
		ErrNumber = 0
		Randomize
		session("cg_predict_er") = clng(rnd*10000)
		app.addlog "采购预测表运行出现错误, 数据源：" & sql & ", 错误描述：" & errDescription
		Response.write "<div style='position:absolute;top:70px;left:0px;width:100%'><div style='margin:10px;padding:10px;border:1px solid #e0edf9;color:red'><b>系统信息：</b>这是一张可设置公式报表，运行过程出现错误，您可以尝试<a href='?__msgid=clearerrconfg&n=" & session("cg_predict_er") & "&key=" & lvw.md5key16 & "' style='color:blue'><u>恢复默认公式设置</u></a>，使系统恢复正常。</div></div>"
		cn.close
		Response.end
	end sub
	Sub App_clearerrconfg
		Dim md5key : md5key =  request.querystring("key")
		Dim rs , fs
		If session("cg_predict_er") & "" = request.querystring("n") Then
			session("cg_predict_er")  = ""
			Set rs = cn.execute("select dbname, evalname, formula from erp_sys_LvwColConfig where cfgid in (select id from erp_sys_LvwConfig where lvwid='" & md5key & "' and uid=0)")
			If rs.eof = False Then
				fs = rs.getString(2,-1,vbtab,vbcrlf)
'If rs.eof = False Then
			end if
			rs.close
			app.addlog "[" & Info.username & "（" & Info.user & "）]由于采购预测表报错，清除了其公式配置，原配置数据为：" & fs
			cn.execute "delete erp_sys_LvwColConfig where cfgid in (select id from erp_sys_LvwConfig where lvwid='" & md5key & "' and uid=0)"
			cn.close
			Response.redirect "?"
		else
			Response.write "拒绝访问"
