...
'Sub ShowGatherListHTMLFoot()
	...
	if px="" or instr(1,px,"-",1)<=0 then px="5-1"
	if CurrPage="" then CurrPage=1
	Dim ord,conditions,contexts,time1,time2
	strCondition2=" "
	pageid=80022
	tname=""
	contexts4=request("sort4")
	contexts5=request("sort5")
	If Len(contexts4&"")>0 Then
		Str_Result8=Str_Result8&" and charindex('"&contexts4&"',客户分类)>0 "
	end if
	If Len(contexts5&"")>0 Then
		Str_Result8=Str_Result8&" and charindex('"&contexts5&"',跟进程度)>0 "
	end if
	time1=request("e_ret")
	time2=request("e_ret2")
	If Len(time1&"")>0 Then
		Str_Result8=Str_Result8&" and 修改时间>='"&time1&" 00:00:00'"
	end if
	If Len(time2&"")>0 Then
		Str_Result8=Str_Result8&" and 修改时间<='"&time2&" 23:59:59'"
	end if
	...
	NumDigits=0
	conn.execute("if not exists(select top 1 SettingIndex from GatherRegistration where SettingIndex="&pageid&") insert into GatherRegistration values("&pageid&",'跟进程度修改历史记录','sort5/sort_his_list.asp')")
	baseSQL="select PAGE_COUNT_NUM from ( SELECT PAGE_TOP_NUM ord as 序号,isnull((select sort1 from sort4 where ord=sort5_his.sort1),'') as 客户分类,sort2 as 跟进程度,(select name from gate where ord=sort5_his.op) as 修改人,ip as 修改IP,opdate as 修改时间,'' as 操作 from sort5_his where 1=1 " & strCondition2 & ") bbb where 1=1"
	dim gather
	set gather=new GatherList
	with gather
	.digits=NumDigits
	.title="跟进程度修改历史记录"
	.FieldsSettingIndex=pageid
	...
	.fieldsList="*"
	.canprocpage=false
	.strCondition=strCondition & Str_Result8
	...
	function CustomShowFields(idx,rsobj)
		...
		Case "操作"
		If Len(rsobj(idx).value&"")=0 And rsobj("序号").value>=0 Then
			If rsobj("序号").value>0 Then
				Response.write "<div style='text-align:center'><input type=""button"" name=""Submit322"" value=""详情""  class=""anniu"" onClick=""javascript:window.open('sort5_his.asp?ord=" & rsobj("序号").value & "','newwins','width=' + 1000 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=200,top=150')"" /></div>"
			ElseIf rsobj("序号").value=0 then
				Response.write "<div style='text-align:center'></div>"
'ElseIf rsobj("序号").value=0 then
			end if
		else
			Response.write "<div style='text-align:center'>"&rsobj(idx).value&"</div>"
'ElseIf rsobj("序号").value=0 then
		end if
		case Else
...
'if CanFormatNumber(rsobj(idx).name) then
	...
	sub ShowGatherListHTMLFootExtend()
		...
		Response.write Application("sys.info.jsver")
		Response.write """></script>" & vbcrlf & "<script type=""text/Javascript"">" & vbcrlf & "<!--" & vbcrlf & "function open2()" & vbcrlf & "{" & vbcrlf & "                      var windowWidth=document.all ? document.getElementsByTagName(""html"")[0].offsetWidth : window.innerWidth; " & vbcrlf & "                 $('#dd').dialog('move',{left:document.getElementsByTagName(""body"")[0].scrollWidth-windowWidth+200,top:50});" & vbcrlf & "                   $('#dd').show();" & vbcrlf & "                        $('#dd').dialog('open');" & vbcrlf & "}" & vbcrlf & " -->" & vbcrlf & "</script>" & vbcrlf & ""
		'Response.write Application("sys.info.jsver")
	end sub
	sub CurstomButtons()
		Response.write "" & vbcrlf & "时间：<INPUT readonly name=""e_ret"" size=9  id=daysOfMonthPos onmouseup=toggleDatePicker(""daysOfMonth"",""demo.e_ret"") value="""
		Response.write time1
		Response.write """><DIV id=daysOfMonth style=""POSITION: absolute;z-index:10""></DIV>&nbsp;至：<INPUT readonly name=""e_ret2"" size=9  id=daysOfMonth2Pos onmouseup=toggleDatePicker(""daysOfMonth2"",""demo.e_ret2"") value="""
		'Response.write time1
		Response.write time2
		Response.write """><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>" & vbcrlf & "    &nbsp;" & vbcrlf & "客户分类：" & vbcrlf & " <input name=""sort4"" type=""text"" id=""contexts"" style=""width:100px"" value="""
		Response.write request("sort4")
		Response.write """ />" & vbcrlf & "      &nbsp;" & vbcrlf & "跟进程度：" & vbcrlf & " <input name=""sort5"" type=""text"" id=""contexts2"" style=""width:100px"" value="""
		Response.write request("sort5")
		Response.write """ />" & vbcrlf & "" & vbcrlf & " <input type=""submit"" name=""jiansuo"" value=""检索"" id=""jiansuo"" class=""anybutton"" />" & vbcrlf & "" & vbcrlf & "<select name=""page_count"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl('page_count='+this.value);}"">" & vbcrlf & "<option>-请选择-</option>" & vbcrlf & "<option value=""10"" "
		Response.write request("sort5")
