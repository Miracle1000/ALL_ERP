...
'ElseIf ptype="5" Then
	...
	Sub Bill_Init(Bill)
		...
		.keyField = "ID"
		.needcheck = false
		.SQL = "select ID,ordername,PrefixCode,canReturn from M_OrderSettings where ID = bill_ID"
		.SetUIFields "|单据编号;ID;5;number;0;0;0;单据信息,1,100|单据名称;ordername;2;text;0;;0;单据信息,1,100|标识符;PrefixCode;2;text;0;;0;单据信息,1,100;;;;|是否允许退回;canReturn;1;bit;0;;;单据信息;1;;;;|流程清单;clqdmx;1;detail;0;;;流程信息;3;;|"
		.HasLeftPanel = false
		...
		Detail.tableName = "M_FlowSettings"
		Detail.keyField = "orderid"
		Detail.sql = "select Rank as 审批级别,spName as 审批级别名称,intro as 审批人ID,replace(dbo.erp_bill_GetSpLinkMan(0,intro,3),'|','&L') as 审批人,LinkType as 审批人关联方式 from M_FlowSettings where orderid=@bill_ID order by rank asc"
		detail.setSvFields "rank,spName,intro,LinkType,PrefixCode=@PrefixCode"
		Detail.heads(3).htmlvisible = false
		Detail.heads(3).resize = 0
		Detail.heads(3).edit = 0
		Detail.heads(3).save = 1
		Detail.heads(4).edit = 0
		Detail.heads(4).save = 0
		Detail.heads(5).selId = 36
		Detail.showtool=false
		...
		bill.cmdButtons.items(9).visible = false
		bill.cancopy = false
		Set Bill.GetMainFieldByDBname("clqdmx").defvalue = Detail
		if request("__msgid")="" then
			spid=request("id")
			sql7="SELECT * FROM M_OrderSettings where id="&spid
			set rs7=cn.execute(sql7)
			if not rs7.eof then
				call app.add_log(1,rs7("ordername")&"审批")
			end if
			rs7.close
			set rs7=nothing
		end if
	...
	function Page_OnHead
		Page_OnHead = vbcrlf & "            <script language=javascript src='spconfig.js'></script><!--审批流程自定义js-->" & vbcrlf &_
		"<style>table.edt0tb{table-layout:fixed}</style>"
'function Page_OnHead
	end function
	function App_LastDataTest(svData)
		dim rs , tbname , jb , ml ,  i , ii , x , hs
		v = request.form("MT1")
		if len(v) =  0 or not isnumeric(v) then
			app.alert "参数不对-1"
'if len(v) =  0 or not isnumeric(v) then
		...
		set rs = cn.execute("select Maintable,hasSP from M_OrderSettings where id = " & v)
		if rs.eof then
			app.alert "参数不对-2"
'if rs.eof then
			App_LastDataTest = false
			...
			if abs(rs.fields("hasSp")) = 0 then
				app.alert "配置定义该类型单据无需审批。"
				App_LastDataTest = false
				rs.close
				exit function
			end if
			tbname = rs.fields("Maintable").value
		end if
		rs.close
		dim filterSql : filterSql = ""
		select case v
		case 1001:
		filterSql = " appType=1 and "
		case 1002:
		filterSql = " appType=2 and "
		case 1003:
		filterSql = " appType=3 and "
		case else
		filterSql = ""
		end select
		if ubound(svdata.tables) >= 0 then
			x = svData.tables(0)
			set rs = cn.execute("select distinct cateid_sp,isnull(id_sp,0) as id_sp,(select top 1 b.[name] from gate b where b.ord=a.cateid_sp) as spname,a.id from  " & tbname & " a where " & filterSql & " status <= 1 and cateid_sp> 0 order by id_sp desc")
			while not rs.eof
				ml = rs.fields("cateid_sp").value
				jb = rs.fields("id_sp").value
				for i=0 to ubound(x)
					if x(i)(0) = cstr(jb) and instr( "," & x(i)(2) & ",","," & ml  & "," ) = 0 then
						app.alert "第" & jb & "级中的【" & app.iif(len(rs.fields(2).value)>0,rs.fields(2).value,"用户" & ml)  & "】目前有审批任务，所以无法删除。\n\n关联单据主题为 “" & GetBillInfo(rs.fields("id").value, tbname) & "”。"
						rs.close
						App_LastDataTest = false
						exit function
					end if
			...
			set rs = cn.execute("select distinct isnull(id_sp,0) as id_sp,b.intro,a.id from " & tbname & " a,  M_FlowSettings b where "& filterSql &" a.id_sp=b.rank and b.orderid=" &  v & " and  a.status <= 1")
			if not rs.eof then
				jb = rs.fields(0).value
				app.alert "第" & jb & "级审批任务(" & GetBillInfo(rs.fields("id").value, tbname) & ")正在使用中，所以无法修改。\n\n关联单据主题为 “" & GetBillInfo(rs.fields("id").value, tbname) & "”"
				rs.close
				App_LastDataTest = false
				exit function
			end if
		...
		if ubound(svdata.tables) >= 0 then
			for i = 0 to  ubound(x)
				if Len(RTrim(x(i)(0)))=0 Then
					app.alert "第 " & (i+1) & " 行审批级别不能为空。"
'if Len(RTrim(x(i)(0)))=0 Then
					App_LastDataTest = false
					exit Function
				ElseIf CDbl(x(i)(0))<=0 Then
					app.alert "第 " & (i+1) & " 行审批级别必须大于0。"
'ElseIf CDbl(x(i)(0))<=0 Then
					App_LastDataTest = false
					exit Function
				end if
				if len( rtrim(x(i)(1))) = 0 then
					app.alert "第 " & (i+1) & " 行审批级别名称不能为空。"
'if len( rtrim(x(i)(1))) = 0 then
					App_LastDataTest = false
					exit function
				end if
				if len( rtrim(x(i)(2))) = 0 then
					app.alert "第" & x(i)(0)  & "级审批人不能为空。"
					App_LastDataTest = false
					exit function
				end if
				for ii = i + 1 to ubound(x)
					...
					if x(i)(0) = x(ii)(0) then
						app.alert "第 " & (i+1) & " 行与第 " & (ii+1) & " 行的审批级别重复。"
'if x(i)(0) = x(ii)(0) then
						App_LastDataTest = false
						exit function
					end if
	...
	function GetBillInfo(id, tb)
		on error resume next
		dim rs
		set rs =  cn.execute("select title from " & tb & " where ID=" & id)
		if rs.eof = false then
			GetBillInfo = rs.fields("title").value
		end if
		rs.close
		if len(GetBillInfo) = 0 then
			GetBillInfo =  "ID为" & id
		end if
		set rs = nothing
	end function
