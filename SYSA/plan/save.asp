...
'Private Sub Class_Initialize
	...
	dim intro,num1,gate,ret1,time1,time2,order1,option1
	intro=Request.form("intro3")
	gate=trim(Request.form("gate"))
	ret1=trim(Request.form("ret"))
	time1=trim(Request.form("time1"))
	time2=trim(Request.form("time2"))
	ret2=trim(Request.form("ret2"))
	time11=trim(Request.form("time11"))
	time22=trim(Request.form("time22"))
	option1=Request.form("option1")
	order1=Request("W3")
	company=request("company")
	chance=request("chance")
	If chance&""<>"" Then
		if isNumeric(chance) then
			Set rs=conn.execute("select complete2 from chance where ord="&chance&"")
			If Not rs.eof Then
				lcb=rs(0)
			end if
			rs.close
	...
	person=session("personzbintel")
	session("personzbintel")=""
	isXunhuan=request("isXunhuan")
	If isXunhuan="" Then isXunhuan=0
	if ret2 = "" then
		Response.write"<script language=javascript>alert('请选择开始时间！');history.back()</script>"
		call db_close : Response.end
	end if
	if ret1 = "" then
		Response.write"<script language=javascript>alert('请选择截止时间！');history.back()</script>"
		call db_close : Response.end
	end if
	if isdate(ret2) = false then
		Response.write"<script language=javascript>alert('请选择有效的开始时间！');history.back()</script>"
		call db_close : Response.end
	end if
	if isdate(ret1) = false then
		Response.write"<script language=javascript>alert('请选择有效的截止时间！');history.back()</script>"
		call db_close : Response.end
	end if
	ret1=cdate(ret1)
	ret2=cdate(ret2)
	If ret1<ret2 Then
		Response.write"<script language=javascript>alert('开始时间不能大于截止时间！');history.back()</script>"
		call db_close : Response.end
	ElseIf ret1=ret2 Then
		If (Right("0"&Trim(time1),2)&":"&Right("0"&Trim(time2),2))<=(Right("0"&Trim(time11),2)&":"&Right("0"&Trim(time22),2)) Then
			Response.write"<script language=javascript>alert('开始时间不能大于或等于截止时间！');history.back()</script>"
			call db_close : Response.end
		end if
	end if
	if intro = "" then
		Response.write"<script language=javascript>alert('请填写日程内容！');history.back()</script>"
		call db_close : Response.end
	end if
	W1=replace(request.form("W1")," ","")
	W2=replace(request.form("W2")," ","")
	W3=replace(request.form("W3")," ","")
	W3=replace(getW3(W1,W2,W3)," ","")
	W3=getLimitedW3(W3,2,4,0,session("personzbintel2007"))
	if W3<>"" and W3<>"0" then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select * from gate where  ord in  ("&W3&")"
		rs1.open sql1,conn,1,1
	else
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select * from gate where ord="&session("personzbintel2007")&" "
		rs1.open sql1,conn,1,1
	end if
	col=""
	if rs1.eof then
		...
		do until rs1.eof
			sorce_user=rs1("sorce")
			sorce_user2=rs1("sorce2")
			sql = "select * from plan1 where 1=0"
			Set Rs = server.CreateObject("adodb.recordset")
			...
			If company&""<>"" And isnumeric(company) Then Rs("company")=company
			If chance&""<>""  And isnumeric(chance) Then Rs("chance")=chance
			If person&""<>""  And isnumeric(person) Then Rs("person")=person
			If lcb&""<>""  And isnumeric(lcb) Then Rs("lcb")=lcb
			Rs("option1")=option1
			Rs("date7")=now
			Rs("date4")=date
			Rs("date8")=now
			Rs("startdate1")=ret2
			Rs("starttime1")=time11
			Rs("starttime2")=time22
			Rs("isXunhuan")=isXunhuan
			rs.update
			rs.close
			set rs = nothing
			rs1.movenext
	...
	rs1.close
	set rs1=nothing
	rid = GetIdentity("plan1","ord",col,"")
	Set helper = CreateReminderHelper(conn,1,0)
	Call helper.appendRemind(rid)
	session("companyzbintel")=""
	session("personzbintel")=""
	session("option1zbintel")=""
	session("event1zbintel")=""
	session("chancezbintel")=""
	session("contractzbintel")=""
	If request("Submit42")="增加" Then
		Response.redirect "add.asp?h=1"
	else
		Response.redirect "option.asp?s="&s&""
	end if
	conn.close
	set conn = nothing
