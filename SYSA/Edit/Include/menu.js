if(!window.createPopup) {
	window.createPopup = function() {
		...
		if(!window.createPopupObj.fm){
			...
			window.createPopupObj.fm.src = virpath + "edit/pop.htm";
		...
		window.createPopupObj.document = {
			open: function() {window.createPopupObj.sys_curr_popMMHtml="";},
			close:function() {},
			write: function(html) {
...
var sMenu1="<TABLE border=0 cellpadding=0 cellspacing=0 class=Menu width=170 style='table-layout:fixed'><col width=20><col width=150><tr><td width=18 valign=bottom align=center style='background:url(sysimage/contextmenu.gif);background-position:bottom;'><\/td><td width=132 class=RightBg><TABLE border=0 cellpadding=0 cellspacing=0>";
...
function getMenuRow(s_Disabled, s_Event, s_Image, s_Html) {
	var s_MenuRow = "";
	s_MenuRow = "<tr><td align=center valign=middle><TABLE border=0 cellpadding=0 cellspacing=0 width=152><tr "+s_Disabled+"><td valign=middle height=20 class=MouseOut onMouseOver=this.className='MouseOver'; onMouseOut=this.className='MouseOut';";
	if (s_Disabled==""){
		s_MenuRow += " style='font-size:12px' onclick=\"parent."+s_Event+";parent.oPopupMenu.hide();\"";
	} else {
		s_MenuRow += " style='font-size:12px;color:#aaaaaa' "
	}
	s_MenuRow += ">"
	if (s_Image !=""){
		s_MenuRow += "&nbsp;<img border=0 style='margin:0px' src='ButtonImage/"+config.StyleDir+"/"+s_Image+"' width=20 height=20 align=absmiddle "+s_Disabled+">&nbsp;";
	}else{
		s_MenuRow += "&nbsp;";
	}
...
function getFormatMenuRow(menu, html, image){
	var s_Disabled = "";
	if (!eWebEditor.document.queryCommandEnabled(menu) && window.ActiveXObject){
		s_Disabled = "disabled";
	}
...
function getTableMenuRow(what){
	...
	switch(what){
	case "TableInsert":
		if (!isTableSelected() || !window.ActiveXObject) s_Disabled="";
