...
'Private Sub Class_Initialize
	...
	Sub messagePost(msgid)
		...
		ElseIf msgid="dosearch" Then
			Call App_doSearch
		ElseIf msgid="doDel" Then
			Call App_doDel
		ElseIf msgid="doDelAll" Then
			Call App_doDelAll
		end if
	end sub
	Sub Page_Load
		app.addDefaultScript
		dim pagesizes ,i, sizepageHTML ,pagesize , addHtml
		pagesize=10
		pagesizes = split("10,20,30,50,100,200",",")
		for i = 0 to ubound(pagesizes)
			if pagesizes(i) - pagesize = 0 then
'for i = 0 to ubound(pagesizes)
				sizepageHTML = sizepageHTML & "<option value=" & pagesizes(i)  & " selected>每页显示" & pagesizes(i)  & "条</option>"
			else
				sizepageHTML = sizepageHTML & "<option value=" & pagesizes(i)  & ">每页显示" & pagesizes(i)  & "条</option>"
			end if
		next
		if app.power.existsPower(12,13) Then addHtml="<button class='oldbutton3' style='margin-right:5px' onclick=""javascript:window.open('add.asp','add','width=1000,height=700,fullscreen=no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">预算添加</button>&nbsp;"
		'sizepageHTML = sizepageHTML & "<option value=" & pagesizes(i)  & ">每页显示" & pagesizes(i)  & "条</option>"
		Response.write(app.DefTopBarHTML(app.virPath, "", "预算列表", "<div style='_position:absolute;left:800px'>" & addHtml & "<select name='sizepage' id='sizepage' onchange='doSearch(0)' style='margin-right:5px'>" & sizepageHTML & "</select></div><input type='hidden' id='px' value='11' /><a class='tableTitleLinks sortRule' href='javascript:void();' onclick='showmenu(this);return false' style='position: absolute; top: 20px; left: 197px;'>排序规则<img class='resetElementHidden' src='../images/i10.gif' width='9' height='5' border='0'><img class='resetElementShowNoAlign' style='display:none;' src='../skin/default/images/MoZihometop/content/r_down.png2' width='9' height='5' border='0'></a>")) & vbCrLf
		Dim rs
		Response.write "" & vbcrlf & "    <script type=""text/javascript"">" & vbcrlf & "    function LoadUrl(str) {" & vbcrlf & "        location.href = str;" & vbcrlf & "    }" & vbcrlf & "    function DelSend(ids) {" & vbcrlf & "              ajax.regEvent(""doDel"")" & vbcrlf & "        ajax.addParam('id', ids);" & vbcrlf & "ajax.send(function(r){" & vbcrlf &        "               if (r == ""1"") { "& vbcrlf &                "          lvw_refresh('ids1'); "& vbcrlf &                "     } "& vbcrlf &        "        }); "& vbcrlf & "    } "& vbcrlf &           "    "   & vbcrlf &   "      function DelAllSend() { "& vbcrlf &      "   var ordlist=""""; "& vbcrlf &        "    $("".lvcbox"").each( & vbcrlf & function(){" & vbcrlf & "                          if($(this).attr(""checked"")==true)" & vbcrlf & "                         {" & vbcrlf & "                                       if (ordlist.length!="""")" & vbcrlf & "                                   {" & vbcrlf & "                                               ordlist = ordlist + "","";" & vbcrlf & "                                  }" & vbcrlf & "                                       ordlist = ordlist + $(this).val();" & vbcrlf & "                              }" & vbcrlf & "       }" & vbcrlf & "               )" & vbcrlf & "               if (ordlist.length=="""")" & vbcrlf & "           {" & vbcrlf & "                       app.Alert(""您没有选择任何预算单据,请选择后再删除！"");" & vbcrlf & "                     return ;" & vbcrlf & "                }" & vbcrlf & "               if(confirm('确认删除？')){" & vbcrlf & "                      ajax.regEvent(""doDelAll"")" & vbcrlf & "                 ajax.addParam('ordlist', ordlist);" & vbcrlf & "                        ajax.send(function(r){" & vbcrlf & "                           if (r == ""1"") {" & vbcrlf & "                                  lvw_refresh('ids1');" & vbcrlf & "                             }" & vbcrlf & "                               else" & vbcrlf & "                            {" & vbcrlf & "                                      if (r==""0"")" & vbcrlf & "                                       {               " & vbcrlf & "                                                app.Alert(""没有选择可删除的预算,请重新选择后再删除！"");" & vbcrlf & "                                     }" & vbcrlf & "                                }" & vbcrlf & "                      });" & vbcrlf & "             }" & vbcrlf & "    }" & vbcrlf & "    function doSearch(pIndex) {" & vbcrlf & "           var dtype=0;" & vbcrlf & "            var dtypes=document.getElementsByName(""dtype"");" & vbcrlf & "           for (var i=0;i<dtypes.length ;i++ )" & vbcrlf & "             {" & vbcrlf & "                   if (dtypes[i].checked==true)" & vbcrlf & "                    {" & vbcrlf & "                               dtype=dtypes[i].value;" & vbcrlf & "                          break;" & vbcrlf & "                  }" & vbcrlf & "               }" & vbcrlf & "        ajax.regEvent(""dosearch"")" & vbcrlf & "        ajax.addParam('px', $('#px').val());" & vbcrlf & "            ajax.addParam('dtype',dtype);" & vbcrlf & "        ajax.addParam('indate1', $('#indate1').val());" & vbcrlf & "        ajax.addParam('indate2', $('#indate2').val());" & vbcrlf & "                ajax.addParam('status', $('#status').val());" & vbcrlf & "            ajax.addParam('bz', $('#bz').val());" & vbcrlf & "            ajax.addParam('tbi', $('#tbi').val());" & vbcrlf & "        ajax.addParam('key', $('#key').val());" & vbcrlf & "            ajax.addParam(""Psize"",$('#sizepage').val());" & vbcrlf & "              //ajax.addParam(""Psize"",$('#lvw_pgsize_selids1').val());" & vbcrlf & "          //pIndex==1表示要带上当前的翻页条件" & vbcrlf & "             ajax.addParam(""pageindex"",pIndex==1?document.getElementById(""lvw_pindex_ids1"").value:1);" & vbcrlf & "        ajax.send(function(r){" & vbcrlf & "                     document.getElementById(""lvwbody"").innerHTML = r;" & vbcrlf & "                 MergeCells();" & vbcrlf & "           });" & vbcrlf & "    }" & vbcrlf & "        function doExcel(){var lvw = new Listview('ids1');lvw.cexcel();}" & vbcrlf & "        window.onlistviewRefresh = function(){" & vbcrlf & "          document.getElementById(""toolbar1"").style.width=document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "            MergeCells();" & vbcrlf & "   }" & vbcrlf & "       document.body.onload = function(){" & vbcrlf & "              document.getElementById(""toolbar1"").style.width=document.body.scrollWidth+""px"";//document.getElementById(""lvw_dbtable_ids1"").style.width;" & vbcrlf & "          document.getElementById(""lvwbody"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "              MergeCells();" & vbcrlf & "           window.onresize=function() {" & vbcrlf & "                       document.getElementById(""toolbar1"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "                     document.getElementById(""lvwbody"").style.width=document.body.scrollWidth+""px"";" & vbcrlf & "              }" & vbcrlf & "       }" & vbcrlf & "       function checkall(obj)" & vbcrlf & "  {"& vbcrlf & "               $("".lvcbox"").attr(""checked"", obj.checked);  " & vbcrlf & "        }" & vbcrlf & "       //合并合计列" & vbcrlf & "    function MergeCells()" & vbcrlf & "   {" & vbcrlf & "               var tb= document.getElementById(""lvw_dbtable_ids1"");" & vbcrlf & "              if (tb.rows.length>2)" & vbcrlf & "           {" & vbcrlf & "                       var tr1=tb.rows[tb.rows.length-1];" & vbcrlf & "                 if(tr1.cells[0].colSpan==1){" & vbcrlf & "                            tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.deleteCell(1);" & vbcrlf & "                              tr1.cells[0].colSpan = 5;" & vbcrlf & "                               tr1.cells[0].style.cssText= ""text-align:right;"";" & vbcrlf & "                             tr1.cells[0].innerHTML=tr1.cells[0].innerHTML + ""：&nbsp;&nbsp;""" & vbcrlf & "                  }" & vbcrlf & "                       var tr2=tb.rows[tb.rows.length-2];" & vbcrlf & "                      if(tr2.cells[0].colSpan==1){" & vbcrlf & "                            tr2.deleteCell(1);" & vbcrlf & "                              tr2.deleteCell(1);" &vbcrlf & "                         tr2.deleteCell(1);" & vbcrlf & "                              tr2.deleteCell(1);" & vbcrlf & "                              tr2.cells[0].colSpan = 5;" & vbcrlf & "                               tr2.cells[0].style.cssText= ""text-align:right;"";" & vbcrlf & "                          tr2.cells[0].innerHTML=tr2.cells[0].innerHTML + ""：&nbsp;&nbsp;""" & vbcrlf & "                  }" & vbcrlf & "               }" & vbcrlf & "                " & vbcrlf & "        }" & vbcrlf & "       function showmenu(tag) {" & vbcrlf & "                if (document.getElementById('asasd'))" & vbcrlf & "           {" & vbcrlf & "                       document.getElementById(""asasd"").style.display='';" & vbcrlf & "                }" & vbcrlf & "               else " & vbcrlf & "           {       " & vbcrlf & "                        var obj=document.createElement(""div"");" & vbcrlf & "                    obj.id=""asasd"";" & vbcrlf & "            obj.style.cssText =""Z-INDEX:20000; POSITION: absolute; TOP: 50px; LEFT: 154px;box-shadow:rgb(136 136 170) 1px 1px 7px"";" & vbcrlf & "                  var html;" & vbcrlf & "                       html="""";" & vbcrlf & "                  html=html+""<TABLE border=0 cellSpacing=-2 cellPadding=-2 width=150 height=300>"";" & vbcrlf & "                     html=html+""<TBODY><TR><TD height=139>"";" & vbcrlf & "                   html=html+""<TABLE border=0 width=150 bgColor=#ecf5ff height=115><TBODY>"";" & vbcrlf & "                 html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###'onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(1);doSearch();\""><FONT color=#2f496e>按照预算主题排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                      html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(2);doSearch();\""><FONT color=#2f496e>按照预算主题排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(3);doSearch();\""><FONT color=#2f496e>按照预算编号排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                  html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(4);doSearch();\""><FONT color=#2f496e>按照预算编号排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                  html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(5);doSearch();\""><FONT color=#2f496e>按照预算状态排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(6);doSearch();\""><FONT color=#2f496e>按照预算状态排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                      html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(7);doSearch();\""><FONT color=#2f496e>按照开始日期排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(8);doSearch();\""><FONT color=#2f496e>按照开始日期排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                        html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(9);doSearch();\""><FONT color=#2f496e>按照截止日期排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                      html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(10);doSearch();\""><FONT color=#2f496e>按照截止日期排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "       html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(11);doSearch();\""><FONT color=#2f496e>按照添加时间排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                       html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(12);doSearch();\""><FONT color=#2f496e>按照添加时间排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                       html=html+""<TR vAlign=center><TD height=38 colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(13);doSearch();\""><FONT color=#2f496e>按照添加人员排序(降)</FONT></A></TD></TR>"";" & vbcrlf & "                    html=html+""<TR vAlign=center><TD height=38s colSpan=2>&nbsp;&nbsp;<A href='###' onclick=\""document.getElementById('asasd').style.display='none';$('#px').val(14);doSearch();\""><FONT color=#2f496e>按照添加人员排序(升)</FONT></A></TD></TR>"";" & vbcrlf & "                      html=html+""</TBODY></TABLE>"";" & vbcrlf & "                     html=html+""</TD></TR></TBODY>"";" & vbcrlf & "                   html=html+""</TABLE>"";" & vbcrlf & "                     obj.innerHTML=html;" & vbcrlf & "                     document.body.appendChild(obj);" & vbcrlf & "                 //预算主题、预算编号、预算状态、开始日期、截止日期、添加时间和添加人员的升降序进行排列数据" & vbcrlf & "              }" & vbcrlf & "    }" & vbcrlf & "    </script>" & vbcrlf & "     <div style='background-image:url(../images/112.gif);width:100%;' id='toolbar1' align=""right"">" & vbcrlf & "       <table style='width:auto;table-layout:auto;' cellspacing='3' id=""kh"">" & vbcrlf & "        <tr>" & vbcrlf & "    <td align=""right"" style='border-top:0px;border-bottom:0px;border-left:0px;padding-right:22px;'>" & vbcrlf & "    <input type=""radio"" name=""dtype"" value=""0"" checked>添加时间 " & vbcrlf & "      <input type=""radio"" name=""dtype"" value=""1"">开始日期 " & vbcrlf & "       <input type=""radio"" name=""dtype"" value=""2"">截止日期 " & vbcrlf & "    从 <input type=""text"" id=""indate1"" onclick=""datedlg.showDateTime()"" size=""20"" style=""width:135px"" readonly=""readonly"" /> " & vbcrlf & "    至 <input type=""text"" id=""indate2"" onclick=""datedlg.showDateTime()"" size=""20"" style=""width:135px"" readonly=""readonly"" />　" & vbcrlf & "   <select name=""status"" id=""status"">" & vbcrlf & "  <option value=""10000"">审批状态</option>" & vbcrlf & "   <option value=""1"">未审批</option>" & vbcrlf & " <option value=""2"">审批中</option>" & vbcrlf & " <option value=""0"">审批通过</option>" & vbcrlf & "       <option value=""3"">审批未通过</option>" & vbcrlf & "     </select>" & vbcrlf & "       <select name=""bz"" id=""bz"">" & vbcrlf & "          <option value="""">币种</option>" & vbcrlf & "            "
		Dim bzsql,setbz
		setbz=0
		...
		loop
		set rs=nothing
		Response.write "" & vbcrlf & "      </select>" & vbcrlf & "       <select name=""tbi"" id=""tbi"">" & vbcrlf & "        <option value=""0"">预算主题</option>" & vbcrlf & "       <option value=""1"">预算编号</option>" & vbcrlf & "       <option value=""2"">预算概要</option>" & vbcrlf & "       </select>" & vbcrlf & "       <input type=""text""  id=""key"" maxlength=""20"" style=""width:150px""/> " & vbcrlf & "    <input type=""button"" value="" 检索 "" class=""oldbutton"" onclick=""doSearch();"" /> " & vbcrlf & "    "
		if app.power.existsPower(12,10) then
			Response.write "<input type=""button"" value="" 导出 "" class=""oldbutton"" id=""dcdb"" onclick='doExcel()'/>"
		end if
		if app.power.existsPower(12,7) then
			Response.write "" & vbcrlf & "              <input type=""button"" value="" 打印 "" class=""oldbutton"" id=""dydb"" onclick=""$('#dcdb').hide();$('#dydb').hide();window.print();$('#dcdb').show();$('#dydb').show();"" />" & vbcrlf & "      "
		end if
		Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "        </div>" & vbcrlf & "  <style>" & vbcrlf & "         .lvwpagesizearea {" & vbcrlf & "                      display:none;" & vbcrlf & "           }" & vbcrlf & "               html{padding:0 10px;background:#EFEFEF}" & vbcrlf & " </style>" & vbcrlf & "        <div style=""overflow:hidden;min-width:900px;box-sizing:border-box"" id='lvwbody'>" & vbcrlf & "    "
		App_doSearch
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		...
		lvw.headers(3).canSum =False
		lvw.headers(4).dbtype = "str"
		lvw.headers(4).canSum =False
		lvw.headers(5).canSum =False
		lvw.headers(6).canSum =False
		lvw.headers(7).bz = "bz"
		lvw.headers(8).dbtype = "money"
		lvw.headers(8).canSum =False
		lvw.headers(9).dbtype = "money"
		lvw.headers(9).canSum =False
		lvw.headers(10).dbtype = "money"
		lvw.headers(10).canSum =False
		lvw.headers(11).dbtype = "str"
		lvw.headers(12).dbtype = "str"
		lvw.headers(13).dbtype = "str"
		lvw.headers(14).dbtype = "str"
		lvw.headers(15).dbtype = "str"
	end sub
	Function App_doSearch
		Dim dtype, indate1,indate2,status,bz,lead,tbi, Keystr ,Psize,pageindex,delHtml , px
		dtype         = app.getInt("dtype")
		indate1   = app.GetText("indate1")
		indate2   = app.GetText("indate2")
		status    = app.GetText("status")
		bz            = app.getInt("bz")
		tbi           = app.getInt("tbi")
		Keystr    = app.GetText("key")
		...
		If Len(status) = 0 Then status = "10000"
		If Len(bz) = 0 Then bz = "0"
		If app.power.GetPowerIntro(12,3)<>"0" Then delHtml= "<button class='oldbutton3' onclick=""DelAllSend()""> 批量删除 </button>"
		Dim lvw,i
		...
		lvw.indexbox = False
		lvw.addlink = "</a><div style=''>全选<input type='checkbox' onclick='checkall(this)'>&nbsp;"&delHtml&"</div>"
		lvw.id = "ids1"
		lvw.AllSum=True
		lvw.CurrSum=True
		lvw.cansort = False
		lvw.pagesize= Psize
		lvw.pageindex=pageindex
		lvw.border = 0
		...
		lvw.sql =  "set nocount on "&_
		" create table #id(id int) "&_
		insertSql&_
		"exec erp_budget_List  "& Info.User &",&excelmode,"& tbi &",' "& Keystr &" ',"& dtype &",' "& indate1 &" ',' " & indate2 &" ',"& px &","& status &","& bz &",&pagesize,"&pageindex&","&remind&" "&_
		" drop table #id "&_
		" set nocount off "
		lvw.headers(1).display = "none"
		lvw.headers(2).formattext = "code:BudgetLinkHTML(@cells[2],""@cells[3]"",@cells[15])"
		lvw.headers(2).canSum =False
		lvw.headers(2).dbtype = "str"
		...
		lvw.headers(4).width = 50
		lvw.headers(4).canSum =False
		lvw.headers(5).width = 75
		lvw.headers(5).canSum =False
		lvw.headers(6).dbtype = "money"
		lvw.headers(6).formatText = "code:FormatHTML(@value,""@cells[14]"")"
		lvw.headers(6).bz = ""
		lvw.headers(6).width = 70
		lvw.headers(7).dbtype = "money"
		lvw.headers(7).formatText = "code:LinkHTML(@cells[2],""@cells[8]"")"
		lvw.headers(7).width = 60
		lvw.headers(8).width = 60
		lvw.headers(8).dbtype = "money"
		lvw.headers(8).formatText = "code:FormatHTML(@value ,"""")"
		lvw.headers(9).width = 50
		lvw.headers(9).canSum =False
		lvw.headers(10).width =60
		lvw.headers(10).canSum =False
		lvw.headers(11).width =50
		lvw.headers(11).canSum =False
		lvw.headers(12).width =120
		lvw.headers(12).canSum =False
		lvw.headers(12).formatText = "code:        workButtonHTML(@cells[16],@cells[18],@cells[17],@cells[2],@cells[19])"
		lvw.colbackPost = False
	...
	Function LinkHTML(bgid,money)
		If money>0 Then
			LinkHTML="<a href='budget_paymx.asp?ysid=" & app.base64.pwurl(bgId) & "' target='_blank' title='点击查看使用明细'>" & FormatNumber(money,Info.MoneyNumber,-1) & "</a>"
'If money>0 Then
		else
			LinkHTML=FormatNumber(money,Info.MoneyNumber,-1)
'If money>0 Then
		end if
	end function
	Function FormatHTML(v, strV)
		FormatHTML=strV & "&nbsp;" &FormatNumber(v&"",Info.MoneyNumber,-1)
'Function FormatHTML(v, strV)
	end function
	Function BudgetLinkHTML(bgId, bgtit, creator)
		Dim GetPowerB,GetPower
		If GetPowerB = "" Then
			GetPowerB = True
			GetPower = app.power.GetPowerIntro(12,14)
		end if
		If Len(GetPower) > 0 Then
			If InStr("," & GetPower & ",","," & creator & ",") > 0 And creator<>0 Then
				BudgetLinkHTML = "<a href='content.asp?ord=" & app.base64.pwurl(bgId) & "' target='_blank'>" & bgtit & "</a>"
			else
				BudgetLinkHTML = bgtit
			end if
		else
			BudgetLinkHTML = "<a href='content.asp?ord=" & app.base64.pwurl(bgId) & "' target='_blank'>" & bgtit & "</a>"
		end if
	end function
	Function workButtonHTML(candel, canupdate, canxq, id, cansp)
		Dim html
		remind = app.getInt("remind")
		Dim rid
		If remind > 0 Then
			...
			if helper.canCancelOrder(rid) Then
				html = html & "<img src='../images/alt3.gif' alt='取消提醒' border='0' style='cursor:hand' onclick='sendCancelRequest(" & rid & "," & remind & ",0,null,""ids1"")'/>&nbsp;"
			end if
		end if
		If canxq = 1 Then
			html = html & "<input type=""button"" value=""详情"" class=""oldbutton2"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(Id)  & "','xq','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');""/> "
'If canxq = 1 Then
		end if
		If cansp = 1 Then
			html = html & "<input type=""button"" value=""审批"" class=""oldbutton2"" onclick=""javascript:window.open('setsp.asp?ord=" & app.base64.pwurl(Id)  & "','sp','width=' + 700 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=400,top=200');""/> "
'If cansp = 1 Then
		end if
		If canupdate = 1 Then
			html = html & "<input type=""button"" value=""修改"" class=""oldbutton2"" onclick=""javascript:window.open('add.asp?ord=" & app.base64.pwurl(Id) & "','xg','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');"" /> "
'If canupdate = 1 Then
		end if
		If candel = 1 Then
	...
	Sub App_doDel()
		Dim id, oSQL
		id = app.GetInt("id")
		oSQL = "update budget set del=2,delcate="& Info.user &",deldate='"& Date&"' Where ord = "& id &" "
		cn.Execute(oSQL)
		Response.write(1)
		call db_close : Response.end()
	end sub
	Sub App_doDelAll()
		Dim ordlist, oSQL, pwdel
		ordlist = app.GetText("ordlist")
		If Len(ordlist)>0 Then
			pwdel = app.power.GetPowerIntro(12,3)
			oSQL = "update budget set del=2,delcate="& Info.user &",deldate='"& Date&"' Where ord in ( "& ordlist &" ) and ( '" & pwdel & "'='' or charindex(','+ cast(creator as varchar(10)) +',',',"& pwdel &",')>0 )"
'pwdel = app.power.GetPowerIntro(12,3)
			cn.Execute(oSQL)
			Response.write(1)
		else
