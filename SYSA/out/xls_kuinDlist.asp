...
'if S <>"" and not isnull(S) then
	...
	j=1
	k=1
	sql="select a.*,f.id as aid,b.reg_name,b.reg_addcateid,b.reg_userid,b.reg_time,c.pro_name,d.sort1,e.name,f.prod_name,f.prod_bh,f.prod_xh,k.sto_name from O_kuinList a "&_
	...
	C1=rs.recordcount
	xlApplication.Init "入库明细表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			...
			prod_price=zbcdbl(rs("prod_price"))
			prod_allmoney=zbcdbl(rs("prod_allmoney"))
			if prod_price="" or isnull(prod_price) then prod_price=0
			if prod_allmoney="" or isnull(prod_allmoney) then prod_allmoney=0
			reg_gztime=rs("reg_gztime")
			prod_sctime=rs("prod_sctime")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,1).Value =rs("reg_name")
			prod_ystime=rs("prod_ystime")
			...
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,8).Value =zbcdbl(rs("prod_allnum"))
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,8).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,9).Value =Formatnumber(cdbl(prod_price),CommPrice_dot_num,-1)
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,9).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,10).Value =Formatnumber(cdbl(prod_allmoney),num_dot_xs,-1)
			prod_ystime=rs("prod_ystime")
			...
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,12).Value =reg_gztime
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,12).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,13).Value =rs("prod_gys")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,13).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,14).Value =prod_sctime
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,14).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,15).Value =prod_ystime
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,15).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,16).Value =rs("prod_ph")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,16).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,17).Value =rs("prod_xlh")
			prod_ystime=rs("prod_ystime")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>入库明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!入库明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("入库明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的入库明细表</u></strong></font></a></p>"
