...
'exit function
	...
	if px="" or instr(1,px,"-",1)<=0 then px="3-1"
	if CurrPage="" then CurrPage=1
	...
	If export&"" <> "1" Then
		...
		elseif open_101_1="1" then
			strCate=strCate&" and e.reg_addcateid in ("&intro_101_1&")"
		end if
	end if
	...
	elseif open_101_15="1" then
		strCate=strCate&" and b.prod_addcateid in ("&intro_101_15&")"
	end if
	...
	if not rssort.eof then
		open_101_10=rssort(0) : intro_101_10 = Replace(rssort(1)&""," ","")
	else
		open_101_10=0 : intro_101_10="0"
	end if
	If export&"" = "1" Then
		...
		elseif open_101_10="1" then
			strCate=strCate&" and e.reg_addcateid in ("&intro_101_10&")"
		end if
	end if
	...
	rssort.close
	set rssort=nothing
	Function baseSQL(strCondition)
		baseSQL="select 用品名称,max(主键_ID) as 主键_ID,max(用品_ID) as 用品_ID,max(用品1_ID) as 用品1_ID,max(登记人_ID) as 登记人_ID,max(仓库_ID) as 仓库_ID,用品编号,用品型号,用品单位,sum(库存数量_DONUM_DOSUM) as 库存数量_DONUM_DOSUM,所在仓库 from ( SELECT PAGE_TOP_NUM * from (SELECT b.prod_name as '用品名称',a.id as 主键_ID,a.prod_id as '用品_ID',a.prod_unit as用品单位_ID,a.reg_fid as '用品1_ID',b.prod_addcateid as '登记人_ID',a.sto_id as '仓库_ID',b.prod_bh as '用品编号',b.prod_xh as '用品型号',d.sort1 as '用品单位',"&_
		" isnull(a.prod_num,0) AS '库存数量_DONUM_DOSUM',c.sto_name as '所在仓库', b.prod_type as '用品分类_ID'"&_
		" FROM O_kuinList a"&_
		" LEFT JOIN O_kuin e ON a.reg_fid=e.id"&_
		" LEFT JOIN O_product b ON a.prod_id=b.id"&_
		" LEFT JOIN sortonehy d ON a.prod_unit=d.id"&_
		" LEFT JOIN O_store c ON a.sto_id=c.id where prod_num>0 and ( charindex(',"&session("personzbintel2007")&",',','+replace(cast(c.sto_intro as varchar(8000)),' ','')+',')>0 or c.sto_intro='' or c.sto_intro='0') "&strCate&") aa  PAGE_ORDER_STR ) bbb where 1=1 "&strCondition&" group by 用品名称,用品编号,用品型号,用品单位,所在仓库,用品型号"
	end function
...
'if sflg="" then
	...
	next
	strCondition=""
	getComeseachType=request.QueryString("seachType")
	getComevalues=request.QueryString("value")
	getComepage=request.QueryString("page")
	getComename=request.QueryString("name")
	getComebh=request.QueryString("bh")
	getComexh=request.QueryString("xh")
	getComedw=request.QueryString("dw")
	getComekc=request.QueryString("kc")
	prod_type=request.QueryString("prod_type")
	If getComeseachType<>"" Then
		Select case getComeseachType
		Case 0
		If getComevalues<>"" Then ypdj_title=getComevalues
		case 1
		If getComevalues<>"" Then ypdj_bh=getComevalues
		case 2
		If getComevalues<>"" Then ypdj_company=getComevalues
		End select
	end if
	If getComename<>"" Then ypdj_title=getComename
	If getComebh<>"" Then ypdj_bh=getComebh
	If getComexh<>"" Then ypdj_company=getComexh
	If getComedw<>"" Then ypdj_companybh=getComedw
	If getComekc<>"" Then ypdj_leftcheckpay1=getComekc
	if strW3<>"" then strCondition=strCondition&" and 登记人_ID in ("&strW3&")"
	...
	if ypdj_company<>"" then strCondition=strCondition&" and 用品型号 like '%"&ypdj_company&"%'"
	if ypdj_companybh<>"" then strCondition=strCondition&" and 用品单位_ID in ("&ypdj_companybh&")"
	if ypdj_num1<>"" then strCondition=strCondition&" and 库存数量_DONUM_DOSUM >= " & cdbl(ypdj_num1)
	if ypdj_num2<>"" then strCondition=strCondition&" and 库存数量_DONUM_DOSUM <= " & cdbl(ypdj_num2)
	if ypdj_leftcheckpay1<>"" then strCondition=strCondition&" and 仓库_ID in ("&ypdj_leftcheckpay1&")"
	If prod_type&""<>"" Then strCondition=strCondition&" and 用品分类_ID in ("& prod_type &")"
	...
	.digits=num1_dot
	.title="库存台帐汇总"
	.FieldsSettingIndex=5007
	.PageSize=cint(page_count)
	.CurrPage=clng(CurrPage)
	.px=px
	.baseSQL=baseSQL(strCondition)
	.PKName="主键_ID"
	.fieldsList="*"
	.canprocpage=false
	.strCondition=""
	...
	Set gather = Nothing
	Sub CustomShowTotalSumFields(arrFields,LrsSum)
	end sub
	Sub SumCount(strSql,arrFields)
		if arrFields="库存数量_DONUM_DOSUM" Then Response.write FormatNumber(conn.execute("Select sum(库存数量_DONUM_DOSUM) from ("&strSql&") SumCount")(0),num1_dot, -1, 0, 0)
'Sub SumCount(strSql,arrFields)'
	end sub
	function CustomShowFields(idx,rsobj)
		select case rsobj(idx).name
		case "用品名称"
		if open_101_14=3 or (open_101_14=1 and IsInList(intro_101_14,rsobj("登记人_ID"),",")) then
			If request("export")<>"" Then
				Response.write rsobj(idx).value
			else
				Response.write "<a href=""../yp/productDetail.asp?ord="&rsobj("用品_ID").value&""" target=""_blank"" title=""查看档案单详情"">"&rsobj(idx).value&"</a>"
			end if
		else
			Response.write rsobj(idx).value
		end if
...
'if CanFormatNumber(rsobj(idx).name) then
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<style>.IE5 .ser_top input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}</style>" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""width:410px;height:360px;padding:5px;background: #fafafa;top:0px;display:none""  closed=""true"" >" & vbcrlf & "    <form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc;width:98%;height:auto; margin-bottom:5px;"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td style=""width:60px;"" align=""right"">用品名称</td>" & vbcrlf & "                 <td align=""left"">" & vbcrlf & "         <input name=""ypdj_title"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_title
		Response.write """ style=""width:210px"" class=""searchInput""/></td>" & vbcrlf & "              </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品编号</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_bh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_bh
		Response.write """ style=""width:210px"" class=""searchInput""/></td>" & vbcrlf & "                </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品型号</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_company"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_company
		Response.write """ style=""width:210px"" class=""searchInput""/></td>" & vbcrlf & "                </tr>" & vbcrlf & "" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品单位</td>" & vbcrlf & "                   <td align=""left"">"
		sql8="select id, sort1 from sortonehy where gate2=605 order by gate1 desc"
		set rs8=conn.execute(sql8)
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "              <input name=""ypdj_companybh"" utype=""unit"" type=""checkbox"" value="""
		...
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                  </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">所在仓库</td>" & vbcrlf & "                   <td align=""left"">"
		...
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "              <input name=""ypdj_leftcheckpay1"" type=""checkbox"" utype=""storage"" value="""
				Response.write rs8("id")
				Response.write """>" & vbcrlf & "              "
		...
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                  </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">用品分类</td>" & vbcrlf & "                   <td align=""left"">"
		...
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                  </tr>" & vbcrlf & "         </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "           <input type=""hidden"" name=""page_count"" value="
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
	...
	sub ShowGatherListHTMLFootExtend()
		...
		Response.write ","";" & vbcrlf & "var wobj=document.getElementById(""dd"").getElementsByTagName(""input"");" & vbcrlf & "for(var i=0;i<wobj.length;i++)" & vbcrlf & "{" & vbcrlf & "       if(wobj[i].name)" & vbcrlf & "        {" & vbcrlf & "               if(wobj[i].name==""W1""&&strW1.indexOf(','+wobj[i].value+',')>=0)" & vbcrlf & "           {" & vbcrlf & "                   wobj[i].click();" & vbcrlf & "                }" & vbcrlf & "               else if(wobj[i].name==""W2""&&strW2.indexOf(','+wobj[i].value+',')>=0)" & vbcrlf & "              {" & vbcrlf & "                       wobj[i].click();" & vbcrlf & "                }" & vbcrlf & "               else if(wobj[i].name==""W3"")" & vbcrlf & "               {" & vbcrlf & "                       wobj[i].checked=(strW3.indexOf(','+wobj[i].value+',')>=0)" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "       //if(wobj[i].setAttribute(""uType"")=='unit')alert(wobj[i].value)" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & ""
		'Response.write strW3
	end sub
	sub CurstomButtons()
		Dim rs
		Response.write "" & vbcrlf & "<input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """ >" & vbcrlf & "<input type=""hidden"" name=""sflg"" value=""1""/>" & vbcrlf & "<input name=""selectM"" id=""M1"" type=""radio"" value=""1"" checked><label for=""M1"">汇总模式</label>" & vbcrlf & "<input name=""selectM"" id=""M2"" type=""radio"" value=""2""><label for=""M2"">明细模式</label>" & vbcrlf & "<select name=""page_count"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl('page_count='+this.value);}"">" & vbcrlf & "<option value="""">-请选择-</option>" & vbcrlf & "<option value=""10"" "
		Response.write px
		...
		Response.write ">每页显示200条</option>" & vbcrlf & "</select>" & vbcrlf & "<select name=""prod_type"" id=""prod_type"">" & vbcrlf & "<option value="""">用品分类</option>" & vbcrlf & ""
		prod_type = request.QueryString("prod_type")
		...
		While rs.eof = False
			...
			Response.write """ "
			If prod_type&"" = rs("id")&"" Then Response.write "selected"
			Response.write ">"
		...
		rs.close
		set rs = nothing
		Response.write "" & vbcrlf & "</select>" & vbcrlf & "<select name=""selectOfficePorduct"" id=""selectOfficePorduct"">" & vbcrlf & "<option value=""0"">用品名称</option>" & vbcrlf & "<option value=""1"">用品编号</option>" & vbcrlf & "<option value=""2"">用品型号</option>" & vbcrlf & "</select>" & vbcrlf & "<input type=""text"" sendType=""seachInput"" size=""15"">" & vbcrlf & "<input type=""submit"" value=""查看"" name=""forSeach"" id=""forSeach"" class=""anybutton""/><input type=""button"" class=""anybutton"" value=""高级检索"" onClick=""open1();""/>"
		if open_101_10<>0 Then
	...
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "$(document).ready(function(){" & vbcrlf & "    var getSeachType='"
	if getComeseachType="" Then Response.write request("selectOfficePorduct") else Response.write getComeseachType end if
	Response.write "';" & vbcrlf & "   $('#selectOfficePorduct').find('option[value='+getSeachType.toString()+']').attr('selected',true);" & vbcrlf & "      switch(getSeachType.toString()){" & vbcrlf & "                case '0':" & vbcrlf & "                       $('input[sendType=seachInput]').val('"
'if getComeseachType="" Then Response.write request("selectOfficePorduct") else Response.write getComeseachType end if
	Response.write ypdj_title
	Response.write "');" & vbcrlf & "                  break;" & vbcrlf & "          case '1':" & vbcrlf & "                       $('input[sendType=seachInput]').val('"
	Response.write ypdj_bh
	Response.write "');" & vbcrlf & "                  break;" & vbcrlf & "          case '2':" & vbcrlf & "                       $('input[sendType=seachInput]').val('"
	Response.write ypdj_company
	Response.write "');" & vbcrlf & "                  break;" & vbcrlf & "  }" & vbcrlf & "       " & vbcrlf & "        $('#demo').submit(function(){" & vbcrlf & "           var selectOfficePorduct=$('#selectOfficePorduct').val();" & vbcrlf & "                if(selectOfficePorduct=='0')$('input[sendType=seachInput]').attr('name','ypdj_title');" & vbcrlf & "          if(selectOfficePorduct=='1')$('input[sendType=seachInput]').attr('name','ypdj_bh');" & vbcrlf & "            if(selectOfficePorduct=='2')    $('input[sendType=seachInput]').attr('name','ypdj_company');" & vbcrlf & "    })" & vbcrlf & "      $('input[name=selectM]').click(function(){" & vbcrlf & "              var getType=$(this).val();" & vbcrlf & "           if(getType.toString()=='2'){" & vbcrlf & "                    window.location.href='yptong1.asp?seachType='+$('#selectOfficePorduct').val()+'&value='+$('input[sendType=seachInput]').val()+'&page='+$('select[name=page_count]').val()+'&name="
	'Response.write ypdj_company
	Response.write ypdj_title
	Response.write "&bh="
	Response.write ypdj_bh
	...
	Response.write ypdj_company
	Response.write "&dw="
	Response.write replace(ypdj_companybh," ","")
	Response.write "&kc="
	Response.write replace(ypdj_leftcheckpay1," ","")
	Response.write "';" & vbcrlf & "           }       " & vbcrlf & "        })" & vbcrlf & "      var units='"
	Response.write replace(ypdj_companybh," ","")
	Response.write "';" & vbcrlf & "   for(var i=0;i<units.split(',').length;i++){" & vbcrlf & "             $('input[utype=unit]').each(function(){" & vbcrlf & "                 if(this.value.toString()==units.split(',')[i].toString()){" & vbcrlf & "                              this.checked=true;      " & vbcrlf & "                        }" & vbcrlf & "               })" & vbcrlf & "      }" & vbcrlf & "       var ypdj_leftcheckpay1='"
	Response.write replace(ypdj_leftcheckpay1," ","")
	Response.write "';" & vbcrlf & "   for(var i=0;i<ypdj_leftcheckpay1.split(',').length;i++){" & vbcrlf & "                $('input[utype=storage]').each(function(){" & vbcrlf & "                      if(this.value.toString()==ypdj_leftcheckpay1.split(',')[i].toString()){" & vbcrlf & "                         this.checked=true;      " & vbcrlf & "                        }" & vbcrlf & "               })" & vbcrlf & "       }" & vbcrlf & "       $('#searchform input').keydown(function(e){" & vbcrlf & "             if(e.keyCode==13){" & vbcrlf & "                      document.getElementById('searchform').submit();" & vbcrlf & "         }" & vbcrlf & "       })" & vbcrlf & "})" & vbcrlf & "</script>"
