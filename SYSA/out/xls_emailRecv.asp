...
'elseif G1=6 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_recv_list  where del=1  "&Str_Result2&" order by ord desc "
	...
	do until rs.eof
		...
		'cateid=""
		tmpContent = RemoveHTML(content)
		xlWorksheet.Cells(1+i,5).Value = tmpContent
		'tmpContent = RemoveHTML(content)
