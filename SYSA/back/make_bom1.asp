...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		Dim html
		html = "<input class='oldbutton3 batbtn' type='button' value='全部清空' onclick='ListAction.DoDelAll()' />"
		rpt.beforeBtnsHTML = html
		rpt.sql = "select " &_
		" b.ord, b.title, b.BBH, b.BH,g.name delCate, convert(varchar(10),b.delDate,121) delDate, b.addcate, b.ord id " &_
		" from BOM_Structure_Info b " &_
		" left join gate g on g.ord = b.delcate " &_
		" where b.del = 2 order by b.delDate desc"
	end sub
	Sub app_oncreate(byval lvw)
		...
		.title = "组装清单名称"
		.width = 160
		.formattext = "code: TitleCell(@value,rs)"
		.align = "left"
		End With
		...
		.width = 160
		End With
		With lvw.headers("delCate")
		.title = "删除人"
		.width = 80
		End With
		With lvw.headers("delDate")
		.title = "删除日期"
		.width = 80
		End With
		With lvw.headers("id")
		.title = "操作"
		.width = 120
		.formattext = "code: HandleCell(@value,rs)"
		End With
	...
	Sub App_doBatHandle(cmd, checkvalues, batResult)
		...
		Case "批量恢复":
		Call doHF(checkvalues, batResult)
		End select
	end sub
	Function doDelete(checkvalues, batResult)
		cn.execute("delete from BOM_Structure_list where BOMord in (" & checkvalues & ");delete from BOM_Structure_Info where ord in (" & checkvalues & ")")
	end function
	Function doHF(checkvalues, batResult)
		cn.execute "update BOM_Structure_list set del = 1,delcate = null,delDate = null where BOMord in (" & checkvalues & ");update BOM_Structure_Info set del = 1,delcate = null,delDate = null where ord in (" & checkvalues & ")"
	end function
	Sub App_DoDel
		dim id
		id = request.Form("id")
	...
	Sub App_DoDelAll
		cn.execute("delete from BOM_Structure_list where del = 2;delete from BOM_Structure_Info where del = 2")
		Response.write("true")
	end sub
	Function HandleCell(byval id, ByVal rs)
		Dim buttonEdit, buttonDel
		buttonEdit = "<button class='oldbutton2' onclick=""ListAction.DoHF('" & id & "')"">恢复</button>&nbsp;"
		buttonDel = "<button class='oldbutton3' onclick=""ListAction.DoDel('" & id & "')"">彻底删除</button>"
		HandleCell = buttonEdit & buttonDel
	end function
	Function TitleCell(byval v, ByVal rs)
		if app.power.existsPowerIntro(56, 14, rs("addcate")) Then
			TitleCell = "<a href='javascript:void(0)' onclick=""window.open('../BomList/add.asp?add=1&edit=1&bomord=" & app.base64.pwurl(rs("ord")) & "','bomadd_view001','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">" & v & "</a>"
'if app.power.existsPowerIntro(56, 14, rs("addcate")) Then
		else
			TitleCell = v
		end if
