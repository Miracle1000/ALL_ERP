...
'		end if
	...
	Sub page_load
		Response.write app.deftopbarhtml("../","","预算策略设置","")
		dim rs,strateget
		strateget = 0
		set rs = cn.execute("select sort from strategy where gate2=1")
		if rs.eof = false then
			strateget = rs.fields(0).value
		end if
		rs.close
		set rs = nothing
		Response.write "" & vbcrlf & "      <style>#comm_itembarbg{height:43px!important}</style>" & vbcrlf & "   <table width=""100%"" id=""content"" > " & vbcrlf & " <tr class=""top"">" & vbcrlf & "          <td class="""" colspan=""6""><div class=""reseetTextColor"" align=""left"" style=""color:#2f496e"">&nbsp;<b>预算使用策略设置</b></div></td>" & vbcrlf & "     </tr>" & vbcrlf & "   <tr style=""height:30px;"">" & vbcrlf & " <td style=""color:#2f496e;padding-left:42px;"">" & vbcrlf & "     <input type=""radio"" name=""strateget"" value=""0"" "
		'strateget = rs.fields(0).value
		If strateget=0 Then
			Response.write "checked "
		end if
		Response.write ">不受预算金额控制&nbsp; &nbsp;<input type=""radio"" name=""strateget"" value=""1"" "
		If strateget=1 Then
			Response.write "checked "
		end if
		Response.write ">高于预算金额不能录入&nbsp; &nbsp;<input type=""radio"" name=""strateget"" value=""2"" "
		If strateget=2 Then
			Response.write "checked "
		end if
		Response.write ">高于预算金额需要审批</td></tr>" & vbcrlf & "       <tr style=""height:30px;"">" & vbcrlf & " <td align=center><button class='oldbutton HighLight' onclick='savedata()'>保存</button>&nbsp; &nbsp;<button class='oldbutton' onclick='reset("
		Response.write strateget
		Response.write ")'>重填</button></td>" & vbcrlf & " </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <div class='bottomdiv' style='border-top:0px;'></div>" & vbcrlf & "   <script>" & vbcrlf & "                function savedata() {" & vbcrlf & "                   var objs=document.getElementsByName(""strateget"");" & vbcrlf & "                 var v = 0;" & vbcrlf & "                       for (var i=0;i<objs.length ;i++ )" & vbcrlf & "                       {" & vbcrlf & "                               if (objs[i].checked==true)" & vbcrlf & "                              {" & vbcrlf & "                                       v = objs[i].value;" & vbcrlf & "                                      break;" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf & "                       ajax.regEvent(""onsave"");" & vbcrlf & "                  ajax.addParam(""v"", v);" & vbcrlf & "                        ajax.exec();" & vbcrlf & "            }" & vbcrlf & "               function reset(strateget)" & vbcrlf & "               {" & vbcrlf & "                       var objs=document.getElementsByName(""strateget"");" & vbcrlf & "                 var v = 0;" & vbcrlf & "                      for (var i=0;i<objs.length ;i++ )" & vbcrlf & "                       {" & vbcrlf & "                               if(strateget==objs[i].value)" & vbcrlf & "                           {" & vbcrlf & "                                       objs[i].click();" & vbcrlf & "                                }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "       </script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & " "
	end sub
	Sub app_onsave
		Dim rs, strateget, ishav
		strateget = app.getint("v")
		ishav=True
		set rs = cn.execute("select sort from strategy where gate2=1")
		if rs.eof = True  then
			ishav = False
		...
		set rs = nothing
		If ishav Then
			cn.execute("update strategy set sort=" & strateget & "  where gate2=1 " )
		else
			cn.execute("insert into strategy (sort,gate2) values (" & strateget & " ,1)")
		end if
