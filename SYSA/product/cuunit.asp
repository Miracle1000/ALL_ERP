...
'sqltext = sqltext & " select " & ProductAttrValue & " as Num1 ,'" & attrs & AttrID &"' as Attrs "
	...
	rs1.close
	set rs1=nothing
	if open_21_21<>0 and ZBRuntime.MC(15000) then
		strDisplay21_21=""
	else
		strDisplay21_21="display:none;"
	end if
	if open_21_22<>0 and ZBRuntime.MC(7000) then
		strDisplay21_22=""
	else
		strDisplay21_22="display:none;"
	end if
	phXlhManageShow = ZBRuntime.MC(34000) and ZBRuntime.MC(17002)
	unitgp=request.querystring("unitgp")
	If unitgp&""="" Then unitgp = 0
	isNewAddPriceRule = False
	topid = 1
	If unitgp = 0 Then
		isNewAddPriceRule = True
		topid = 2
		unitgp =GetDefUnitGroup(0 , "select ord from mxpxcp where cateid="&session("personzbintel2007")&" and sort1=1 ")
	end if
	unit = GetDefUnit(unitgp , "select ord from mxpxcp where cateid="&session("personzbintel2007")&" and sort1=1")
	baseUnitName=request.querystring("baseUnitName")
	fromType=request.querystring("fromType")
	unitbl = 1
	Set rs= conn.execute("select bl from ErpUnits where unitgp=" & unitgp &" and ord<>"& unit &" and ord in (select ord from mxpxcp where cateid="&session("personzbintel2007")&" and sort1=1 and topid=1)")
	If rs.eof=False And unit>0 Then
		currunitbl = sdk.getSqlValue("select bl from ErpUnits where ord="& unit)
		jbunitbl = rs("bl").value
		If jbunitbl&""="" Then jbunitbl = 1
		unitbl = cdbl(currunitbl) / cdbl(jbunitbl)
	end if
	rs.close
	unitbl = FormatNumber( unitbl , num1_dot , -1 , 0 ,0)
	'unitbl = cdbl(currunitbl) / cdbl(jbunitbl)
	money1=0
	i=session("num_click2009cp")
	dim id
	If isNewAddPriceRule = False And conn.execute("select 1 from mxpxcp Where cateid="&session("personzbintel2007")&" and topid=1").eof = False Then
		id = sdk.getSqlValue("select id from mxpxcp Where cateid="&session("personzbintel2007")&" and topid=1")
		conn.execute("update mxpxcp set ord=" & unit &" where id="&id &" " )
	else
		sqlStr="Insert Into mxpxcp(ord,cateid,topid,sort1,datepx) values('"
		sqlStr=sqlStr & unit & "','"
		...
		sqlStr=sqlStr & now & "')"
		Conn.execute(sqlStr)
		id = GetIdentity("mxpxcp","id","cateid","")
	end if
	blfontcolor ="color: #828282;border: #CCCCCC 1px solid;"
	SelectGroupHtml = ""
	If isNewAddPriceRule = True Then
		SelectGroupHtml = LoadGroupHtml("select" , id, 0 ,unitgp)
		blfontcolor = ""
	end if
	SelectUnitHtml = LoadUnitHtml("select" , id ,unitgp, unit,"")
...
'if proStore=1 then
	...
	if CGMainUnitTactics&"" = "1" and ZBRuntime.MC(15000) then colLen = colLen + 1
	list=""
	list=list+"<table width='"&(colLen*80+260)&"' id='tpx"&i-1&"' border='0' cellpadding='3' style='table-layout:fixed;word-break:break-all;word-wrap:break-word;border-collapse:collapse;margin-top:-1px;'>"
	list=""
	list=list+"<tr onmouseout=this.style.backgroundColor='' onMouseOver=this.style.backgroundColor='efefef'>"
	list=""
	list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'>"
	list=""
	If isNewAddPriceRule=False Then
		list=list+"<font class='red'>"& sdk.getSqlValue("select name from erp_comm_UnitGroup where id="& unitgp ) &"</font>  "
'If isNewAddPriceRule=False Then
	else
		list=list + SelectGroupHtml
'If isNewAddPriceRule=False Then
		list=list + "<a href='javascript:void(0)' onclick=DelProductPriceRule('trpx"&i-1&"','"&id&"');><img src='../images/del2.gif'  border=0/ alt='删除此单位'></a>"
'If isNewAddPriceRule=False Then
	end if
	list=list+"</div></td>"
'If isNewAddPriceRule=False Then
	if isNewAddPriceRule = False then
		list=list+"<input type='hidden' value='"&id&"' id='baseUnitInput'><td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' onchange=""jQuery('.baseUnitFont').text(' '+jQuery('#unitDiv_0_' + jQuery('#baseUnitInput').val()).find('option:selected').text())"" id='unitDiv_0_"&id&"'>"& SelectUnitHtml &"</div></td>"
	else
		list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='unitDiv_0_"&id&"'>"& SelectUnitHtml &"</div></td>"
	end if
	list=list+"<td width='100' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'>"
	list=list+"<div align='center'><input name='bl_"&id&"' id='bl_"&id&"'  type='text' onKeyUp=value=value.replace(/[^\d\.]/g,'') onpropertychange=formatData(this,""number"") value='"& unitbl &"' size=5  style='width:60px;"& blfontcolor &"' "
	If isNewAddPriceRule = False Then list=list+" readonly='true' "
	if isNewAddPriceRule = False then
		list=list+" dataType='number' maxlength='10'  msg='请输入正确的数据'></div></td>"
'if isNewAddPriceRule = False then
	else
		list=list+" dataType='number' maxlength='10'  msg='请输入正确的数据'><font class='red baseUnitFont'> "&baseUnitName&"</font></div></td>"
'if isNewAddPriceRule = False then
	end if
	If CGMainUnitTactics&"" = "1" and ZBRuntime.MC(15000) Then
		list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='cgMUnit_"&id&"'><input type='radio' name='cgMainUnit' id='cgMainUnit_"&id&"' value="& id &"></div></td>"
'If CGMainUnitTactics&"" = "1" and ZBRuntime.MC(15000) Then
	end if
	If fromType&""<>"add" Then
		list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='isUnitStop_"&id&"'><input type='checkbox' name='isUnitStop' id='isUnitStop_"&id&"' "& iif(isUnitStop=1,"checked","") &" value="& id &"  onclick='ChangeUnitStop(this,"&id&")'></div></td>"
	end if
	If phXlhManageShow Then
		list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='xlhManage_"&id&"'><input type='checkbox' name='xlhManage' id='xlhManage_"&id&"' value="& id &"></div></td>"
'If phXlhManageShow Then
	end if
	If isOpenMoreUnitAttr Then
		UnitAttr = 0
		list=list+"<td width='60' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='unitAttr_0_"&id&"'>"& LoadUnitAttrHtml("select" , id, 0 ,unitgp, UnitAttr) &"</div></td>"
		UnitAttr = 0
		list=list+"<td width='120' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='Parameter_0_"&id&"'  class='UnitFormulaClass'>"& LoadFormulaParameter("input" , id , 0 ,UnitAttr ,num1_dot )  &"</div></td>"
		UnitAttr = 0
	end if
	list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'>基础价格</div></td>"
	UnitAttr = 0
	if proStore=1 then
		list=list+"<td width=""70"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center""><input type=""text"" style=""width:60px"" onclick=""adSearch(this);"" name=""ShowStore_0_"&id&""" datatype=""Limit"" min=""1"" msg=""必填"" readonly><input type=""hidden"" name=""MainStore_0_"&id&"""  id=""MainStore_0_"&id&""" value=""""><span class=""red""></span></td>"
		list=list+"<td width=""70"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center""><input type=""text"" name=""StoreCapacity_0_"&id&""" style=""width:60px;text-align:right"" value=""0"" onKeyUp=value=value.replace(/[^\d\.]/g,'') onfocus='this.select();' onpropertychange=formatData(this,""number"") title=""仓库能存放该单位产品的数量，为0则代表不限容量"" datatype=""Range"" min=""0"" max=""999999999"" msg=""容量必须为数字并且不能超出范围""><span class=""red""></span></td>"
		list=list+"<td width=""50"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center"">"
		list=list+"<span style=""cursor:pointer"" onmouseover=""this.style.textDecoration='underline';"" onmouseout=""this.style.textDecoration='none';"" onclick=""adSearch(this);"">编辑</span>"
		list=list+"<input type=""hidden"" name=""SlaveStore_0_"&id&""" id=""SlaveStore_0_"&id&"""  value="""">"
		list=list+"</td>"
	end if
	list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;' align='center'><input name='txm_0_"&id&"' onKeyDown='unEnterDown()' style='width:60px'  type='text' id='u_nametest_0_"&id&"' onChange=callServer('test_0_"&id&"');  size=7  dataType='Limit' min='0' max='50'  msg='必须在50位之内'><span id='test_0_"&id&"' class='red'></span></td>"
	list=list+"<td width='70' style='"&strDisplay21_21&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price1jy_0_"&id&"' id='price1jy_0_"&id&"'  type='text' onKeyUp=checkDot('price1jy_0_"&id&"','"&StorePrice_dot_num&"')  onpropertychange=formatData(this,""StorePrice"") value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
	if money1=0 then
		list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
	end if
	list=list+"></div></td>"
	if money1=0 then
		list=list+"<td width='70' style='"&strDisplay21_21&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price1_0_"&id&"' id='price1_0_"&id&"' type='text' onKeyUp=checkDot('price1_0_"&id&"','"&StorePrice_dot_num&"')  onpropertychange=formatData(this,""StorePrice"") value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'if money1=0 then
'if money1=0 then
		list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
	...
	list=list+"></div></td>"
	if money1=0 then
		list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price2jy_0_"&id&"' id='price2jy_0_"&id&"' type='text' onKeyUp=checkDot('price2jy_0_"&id&"','"&SalesPrice_dot_num&"')  onpropertychange=formatData(this,""SalesPrice"")   value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'if money1=0 then
'if money1=0 then
		list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
	...
	list=list+"></div></td>"
	if money1=0 then
		list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price2_0_"&id&"' id='price2_0_"&id&"' type='text' onKeyUp=checkDot('price2_0_"&id&"','"&SalesPrice_dot_num&"')  onpropertychange=formatData(this,""SalesPrice"")  value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'if money1=0 then
'if money1=0 then
		list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
...
'if money1=0 then
	set rs2=server.CreateObject("adodb.recordset")
	sql2="select * from sort5 where time1=1  order by sort1 asc,gate2 desc"
	rs2.open sql2,conn,1,1
	j=0
	do until rs2.eof
		list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price3_0_"&rs2("ord")&"_"&id&"' id='price3_0_"&rs2("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price3_0_"&rs2("ord")&"_"&id&"','"&SalesPrice_dot_num&"')  onpropertychange=formatData(this,""SalesPrice"")  value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'do until rs2.eof
		if money1=0 then
			list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
...
'if money1=0 then
	...
	set rs2=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,sort1 from pricegate1 where num1=1 order by gate1 desc"
	rs.open sql,conn,1,1
	if rs.eof =false then
		do until rs.eof
			list=list+"<tr onmouseout=this.style.backgroundColor='' onMouseOver=this.style.backgroundColor='efefef'>"
'if rs.eof =false then
			list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'if rs.eof =false then
			list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'if rs.eof =false then
			list=list+"<td width='60' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'if rs.eof =false then
			If CGMainUnitTactics&"" = "1" and ZBRuntime.MC(15000)  Then
				list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'If CGMainUnitTactics&"" = "1" and ZBRuntime.MC(15000)  Then
			...
			If fromType&""<>"add" Then
				list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center' id='isUnitStop_"&id&"'></div></td>"
'If fromType&""<>"add" Then
			end if
			If phXlhManageShow Then
				list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'If phXlhManageShow Then
			...
			If isOpenMoreUnitAttr Then
				list=list+"<td width='60' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'If isOpenMoreUnitAttr Then
				list=list+"<td width='120' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'></div></td>"
'If isOpenMoreUnitAttr Then
			end if
			list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'>"&rs("sort1")&"<input name='name_"&rs("ord")&"_"&id&"' type='hidden' size='7' value='"&rs("ord")&"'></div></td>"
'If isOpenMoreUnitAttr Then
			if proStore=1 then
				list=list+"<td width=""70"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center""><div></div></td>"
'if proStore=1 then
				list=list+"<td width=""70"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center""><div></div></td>"
'if proStore=1 then
				list=list+"<td width=""50"" style=""border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"" align=""center""><div></div></td>"
'if proStore=1 then
			end if
			list=list+"<td width='70' style='border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;' align='center'><input name='txm_"&rs("ord")&"_"&id&"' onKeyDown='unEnterDown()'  type='hidden' id='u_nametest_"&rs("ord")&"_"&id&"' onChange=callServer('test_"&rs("ord")&"_"&id&"');  size=7  dataType='Limit' min='0' max='50'  msg='必须在50位之内'><span id='test_&rs(ord)&_&id&' class='red"
			list=list+"<td width='70' style='"&strDisplay21_21&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price1jy_"&rs("ord")&"_"&id&"' id='price1jy_"&rs("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price1jy_"&rs("ord")&"_"&id&"','"&StorePrice_dot_num&"')  onpropertychange=formatData(this,""StorePrice"")  value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
			if money1=0 then
				list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
			end if
			list=list+"></div></td>"
			if money1=0 then
				list=list+"<td width='70' style='"&strDisplay21_21&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price1_"&rs("ord")&"_"&id&"' id='price1_"&rs("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price1_"&rs("ord")&"_"&id&"','&StorePrice_dot_num&')  onpropertychange=formatData(this,""StorePrice"")  value="&money1&"' size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px"
'if money1=0 then
'if money1=0 then
				list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
			...
			list=list+"></div></td>"
			if money1=0 then
				list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price2jy_"&rs("ord")&"_"&id&"' id='price2jy_"&rs("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price2jy_"&rs("ord")&"_"&id&"','"&SalesPrice_dot_num&"')   onpropertychange=formatData(this,""SalesPrice"") value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'if money1=0 then
'if money1=0 then
				list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
			...
			list=list+"></div></td>"
			if money1=0 then
				list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price2_"&rs("ord")&"_"&id&"' id='price2_"&rs("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price2_"&rs("ord")&"_"&id&"','"&SalesPrice_dot_num&"')  onpropertychange=formatData(this,""SalesPrice"")  value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px"
'if money1=0 then
'if money1=0 then
				list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
...
'if money1=0 then
			...
			j=0
			do until rs2.eof
				list=list+"<td width='70' style='"&strDisplay21_22&"border-bottom:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;'><div align='center'><input name='price3_"&rs("ord")&"_"&rs2("ord")&"_"&id&"' id='price3_"&rs("ord")&"_"&rs2("ord")&"_"&id&"' type='text' onKeyUp=checkDot('price3_"&rs("ord")&"_"&rs2("ord")&"_"&id&"','"&SalesPrice_dot_num&"')  onpropertychange=formatData(this,""SalesPrice"")  value='"&money1&"'  size=7  dataType='Limit' min='1' max='15'  msg='金额不能为空并在15位之内' style='text-align:right;width:60px'"
'do until rs2.eof
				if money1=0 then
					list=list+"onfocus=if(value==defaultValue){value='';this.style.color='#000'} onBlur=if(!value){value=defaultValue;this.style.color='#000'}"
'if money1=0 then
