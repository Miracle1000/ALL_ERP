...
'Private Sub Class_Initialize
	...
	If export&"" <> "1" Then
		...
		elseif open_101_1="1" then
			strCate=" and reg_addcateid in ("&intro_101_1&")"
		end if
	end if
	...
	elseif open_101_15="1" Then
		strCate1=" and prod_addcateid in ("&intro_101_15&")"
	end if
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=16 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_16=rssort(0)
		intro_101_16=replace(rssort(1)," ","")
	else
		open_101_16=0
		intro_101_16=""
	end if
	...
	If export&"" = "1" Then
		...
		elseif open_101_10="1" then
			strCate=strCate&" and reg_addcateid in ("&intro_101_10&")"
		end if
	end if
	...
	rssort.close
	set rssort=nothing
	baseSQL="select PAGE_COUNT_NUM from (SELECT PAGE_TOP_NUM 用品名称,主键_ID,用品编号,用品型号,用品单位,库存数量_DONUM_DOSUM,库存上限_DONUM_DOSUM,库存下限_DONUM_DOSUM,上限紧急度,下限紧急度,单位_ID,人员_ID,用品_ID,用品分类_ID "&_
	"FROM ( "&_
	"SELECT PAGE_TOP_NUM 用品名称,主键_ID,用品编号,用品型号,用品单位,库存数量_DONUM_DOSUM,库存上限_DONUM_DOSUM,库存下限_DONUM_DOSUM,上限紧急度,下限紧急度,单位_ID,人员_ID,用品_ID,用品分类_ID  "&_
	"FROM ( "&_
	"select 用品名称,主键_ID,用品编号,用品型号,用品单位,库存数量_DONUM_DOSUM,库存上限_DONUM_DOSUM,库存下限_DONUM_DOSUM, "&_
	"(SELECT CASE WHEN 库存数量_DONUM_DOSUM>库存上限_DONUM_DOSUM and 库存上限_DONUM_DOSUM<>0 THEN (CONVERT(decimal,(库存数量_DONUM_DOSUM-库存上限_DONUM_DOSUM))/CONVERT(decimal,库存上限_DONUM_DOSUM))*100 ELSE 0 END) as 上限紧急度, "&_
	"(SELECT CASE WHEN 库存数量_DONUM_DOSUM<库存下限_DONUM_DOSUM and 库存下限_DONUM_DOSUM<>0 THEN (CONVERT(decimal,(库存下限_DONUM_DOSUM-库存数量_DONUM_DOSUM))/CONVERT(decimal,库存下限_DONUM_DOSUM))*100 ELSE 0 END) as 下限紧急度, "&_
	"单位_ID,人员_ID,用品_ID,用品分类_ID from  "&_
	"(SELECT 用品_ID+'_'+单位_ID as 主键_ID,prod_more AS 库存上限_DONUM_DOSUM,prod_less as 库存下限_DONUM_DOSUM,prod_addcateid as 人员_ID,prod_name AS 用品名称,prod_bh as 用品编号,prod_xh as 用品型号, "&_
	"用品_ID,单位_ID,库存数量_DONUM_DOSUM,c.sort1 as 用品单位,a.prod_type 用品分类_ID FROM O_product a LEFT JOIN (SELECT REPLACE(prod_id,' ','') AS 用品_ID, "&_
	"REPLACE(prod_unit,' ','') AS 单位_ID,SUM(prod_num) AS 库存数量_DONUM_DOSUM FROM O_kuinList a left join O_kuin b on a.reg_fid=b.id  where 1=1 "&strCate&" GROUP BY prod_id,prod_unit) b ON a.id=b.用品_ID  "&_
	"LEFT JOIN sortonehy c ON c.id=b.单位_ID where 1=1 "&strCate1&") table1  "&_
	") table2 where 上限紧急度>0 or 下限紧急度>0 "&_
	") table3 PAGE_ORDER_STR ) bbb where 1=1 "
...
'if sflg="" then
	...
	If request("")<>"" Then
		Dim helper
		Set helper = CreateReminderHelper(conn,105,0)
		strCondition = strCondition & " and 用品_ID in (" & helper.listSql("ids") & ")"
	end if
	...
	if ypdj_companybh<>"" then strCondition=strCondition&" and 单位_ID in ("&ypdj_companybh&")"
	...
	if ypdj_kuinnum1<>"" then strCondition=strCondition&" and 库存上限_DONUM_DOSUM >= " & cdbl(ypdj_kuinnum1)
	if ypdj_kuinnum2<>"" then strCondition=strCondition&" and 库存上限_DONUM_DOSUM <= " & cdbl(ypdj_kuinnum2)
	if ypdj_kuinmoney1<>"" then strCondition=strCondition&" and 库存下限_DONUM_DOSUM >= " & cdbl(ypdj_kuinmoney1)
	if ypdj_kuinmoney2<>"" then strCondition=strCondition&" and 库存下限_DONUM_DOSUM <= " & cdbl(ypdj_kuinmoney2)
	if ypdj_kuinnum3<>"" then strCondition=strCondition&" and 上限紧急度 >= " & cdbl(ypdj_kuinnum3)
	if ypdj_kuinnum4<>"" then strCondition=strCondition&" and 上限紧急度 <= " & cdbl(ypdj_kuinnum4)
	if ypdj_kuinmoney3<>"" then strCondition=strCondition&" and 下限紧急度 >= " & cdbl(ypdj_kuinmoney3)
	if ypdj_kuinmoney4<>"" then strCondition=strCondition&" and 下限紧急度 <= " & cdbl(ypdj_kuinmoney4)
	...
	function CustomShowFields(idx,rsobj)
		...
		if rssort.eof=false then
			pbit =zbcdbl( rssort("num1").value)
		end if
		rssort.close
		select case rsobj(idx).name
		case "用品名称"
		if open_101_16=3 or (open_101_16=1 and IsInList(intro_101_16,rsobj("人员_ID"),",")) then
			Response.write "<a href=""../yp/productDetail.asp?ord="&rsobj("用品_ID").value&""" target=""_blank"" title=""查看用品详情"">"&rsobj(idx).value&"</a>"
		else
			Response.write rsobj(idx).value
		end if
		case "上限紧急度","下限紧急度"
		Response.write FormatNumber(rsobj(idx).value,pbit,-1,0,0 )&"%"
		case "上限紧急度","下限紧急度"
		case else
		if CanFormatNumber(rsobj(idx).name) then
			Response.write FormatNumber(rsobj(idx).value,gather.digits,-1,0,0)
'if CanFormatNumber(rsobj(idx).name) then
		else
			Response.write rsobj(idx).value
		end if
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<style>.IE5 .ser_top input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}</style>" & vbcrlf & "<div id=""dd"" class=""easyui-window""  icon=""icon-search"" title=""高级检索""  style=""width:600px;height:330px;padding:5px;background: #fafafa;top:0px;display:none""  closed=""true"" >" & vbcrlf & "   <form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc;width:98%;min-height:200"">" & vbcrlf & "               <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "            <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品名称</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_title"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_title
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">用品编号</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_bh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_bh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "              </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品型号</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_company"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_company
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">库存数量</td>" & vbcrlf & "                   <td align=""left"">" & vbcrlf & "                         <input name=""ypdj_num1"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_num1
		Response.write """ class=""searchInput"" />&nbsp;-&nbsp;<input name=""ypdj_num2"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_num1
		Response.write ypdj_num2
		Response.write """ class=""searchInput"" />" & vbcrlf & "                    </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">库存上限</td>" & vbcrlf & "                   <td align=""left"">" & vbcrlf & "                         <input name=""ypdj_kuinnum1"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_kuinnum1
		Response.write """ class=""searchInput"" />&nbsp;-&nbsp;<input name=""ypdj_kuinnum2"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_kuinnum1
		Response.write ypdj_kuinnum2
		Response.write """ class=""searchInput"" />" & vbcrlf & "                    </td>" & vbcrlf & "                   <td align=""right"">库存下限</td>" & vbcrlf & "                   <td align=""left"">" & vbcrlf & "                         <input name=""ypdj_kuinmoney1"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_kuinmoney1
		Response.write """ class=""searchInput"" />&nbsp;-&nbsp;<input name=""ypdj_kuinmoney2"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_kuinmoney1
		Response.write ypdj_kuinmoney2
		Response.write """ class=""searchInput"" />" & vbcrlf & "                    </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">上限紧急度</td>" & vbcrlf & "                 <td align=""left"">" & vbcrlf & "                         <input name=""ypdj_kuinnum3"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_kuinnum3
		Response.write """ class=""searchInput"" />%&nbsp;-&nbsp;<input name=""ypdj_kuinnum4"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_kuinnum3
		Response.write ypdj_kuinnum4
		Response.write """ class=""searchInput"" />%" & vbcrlf & "                   </td>" & vbcrlf & "                   <td align=""right"">下限紧急度</td>" & vbcrlf & "                 <td align=""left"">" & vbcrlf & "                         <input name=""ypdj_kuinmoney3"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_kuinmoney3
		Response.write """ class=""searchInput"" />%&nbsp;-&nbsp;<input name=""ypdj_kuinmoney4"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_kuinmoney3
		Response.write ypdj_kuinmoney4
		Response.write """ class=""searchInput"" />%" & vbcrlf & "                   </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">用品单位</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "       "
		sql8="select id, sort1 from sortonehy where gate2=605 order by gate1 desc"
		set rs8=conn.execute(sql8)
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "                     <input name=""ypdj_companybh"" type=""checkbox"" value="""
		...
		set rs8=nothing
		Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">用品分类</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">"
		...
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                  </tr>" & vbcrlf & "         </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:20px;padding-top:10px;"">" & vbcrlf & "            <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "           <input type=""hidden"" name=""page_count"" value="
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
	...
	sub CurstomButtons()
		Response.write "" & vbcrlf & "<input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """>" & vbcrlf & "<input type=""hidden"" name=""sflg"" value=""1""/>" & vbcrlf & "<select name=""page_count"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl('page_count='+this.value);}"">" & vbcrlf & "<option>-请选择-</option>" & vbcrlf & "<option value=""10"" "
		'Response.write px
		...
		Response.write ">每页显示200条</option>" & vbcrlf & "</select>" & vbcrlf & "<input type=""button"" class=""anybutton"" value=""高级检索"" onClick=""open1();""/>"
		if open_101_10<>0 Then
