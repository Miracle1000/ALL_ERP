...
<%
	...
	Class ProcChart
		...
		Public Sub draw
			Dim sql , Nodes , DrawNodes, I , II , f1, f2, f3, f4, f5,  x , y , nd
			sql =        "SELECT NodeId, PNodeId, NodeName,isnull(NodeId2,'') as NodeId2, isnull(NodeName2,'') as NodeName2, isnull(posX,0) as PosX, isnull(posy,0) as PosY FROM M_ProcessChart a where isnull(posX,0) > 0 or isnull(posy,0) > 0"
			...
			II = -1
			ReDim DrawNodes(0)
			Set f1 = rs.fields("NodeId")
			Set f2 = rs.fields("PNodeId")
			Set f3 = rs.fields("PosX")
			Set f4 = rs.fields("PosY")
			Set f5 = rs.fields("NodeId2")
			While Not rs.eof
				I = I + 1
'While Not rs.eof
				ReDim preserve Nodes (I)
				Set nd = new ChartNode
				Set Nodes(I) = nd
				nd.NodeId = f1.value
				nd.PNodeId =f2.value
				nd.x = f3.value
				nd.y = f4.value
				nd.NodeId2 = rs.fields("NodeId2").value
				nd.nodeName = rs.fields("NodeName").value
				nd.nodeName2 = rs.fields("NodeName2").value
				hs = False
				For ii = 0 To I-1
					hs = False
					If Nodes(ii).NodeId = f1.value Then
						hs = True
						nd.x =  Nodes(ii).x
						nd.y =  Nodes(ii).y
						Exit for
				...
				If Not hs Then
					II = II + 1
'If Not hs Then
					ReDim preserve DrawNodes (II)
					Set DrawNodes(II) =  nd
					If nd.y = 0 then
						y = abs(Nodes(I).NodeId*(NodeHeight+LineHeight))
'If nd.y = 0 then
					else
						y = (nd.y - 1)  * (NodeHeight+LineHeight) + MarginTop
'If nd.y = 0 then
					end if
					If nd.x = 0 then
						x = NodeWidth*2 + offsetLeft
'If nd.x = 0 then
					else
						x= nd.x * NodeWidth + offsetLeft
'If nd.x = 0 then
					end if
					nd.y = y
					nd.x = x
			...
			rs.close
			Dim curr
			For I = 0 To UBound(DrawNodes)
				If TypeName(DrawNodes(i)) = "ChartNode" then
					Set nd = DrawNodes(i)
					curr = CStr(nd.nodeid) = CStr(me.currnode) Or CStr(nd.nodeid2) = CStr(me.currnode)
					If curr Then
						Response.write "<div orderid=" & me.currnode & " class='currProcNode' style='width:"  & NodeWidth &  "px;left:" & (nd.x-2) & "px;top:" & nd.y & "px;z-index:" & (i+10) & "' id='ProcNode" & i & "'><span class='currProcNodeText'>"
'If curr Then
					else
						Response.write "<div orderid=" & nd.nodeid & "  class='ProcNode' style='width:"  & NodeWidth &  "px;left:" & nd.x & "px;top:" & nd.y & "px;z-index:" & (i+10) & "'><span class='ProcNodeText'>"
'If curr Then
					end if
					If nd.nodeid > 0 And CStr(nd.nodeid) <> CStr(me.currnode)  Then
						Response.write "<span href=""readbill.asp?orderid=" & nd.nodeid & """ target=_blank>" & nd.nodename & "</span>"
					else
						Response.write nd.nodename
					end if
					If Len(nd.nodename2) > 0 Then
						If nd.nodeid2 > 0 And CStr(nd.nodeid2) <> CStr(me.currnode) then
							Response.write "、<span href=""readbill.asp?orderid=" & nd.nodeid2 & """ target=_blank>" & nd.nodename2 & "</span>"
						else
							Response.write "、" & nd.nodename2
						end if
					end if
			...
			For I = 0 To UBound(Nodes)
				For ii=i+1 To UBound(Nodes)
'For I = 0 To UBound(Nodes)
					If nodes(ii).PNodeId = Nodes(i).NodeId Then
						Call DrawJoinLine(Nodes(i),nodes(ii))
					end if
					If nodes(i).PNodeId = Nodes(ii).NodeId Then
						Call DrawJoinLine(Nodes(ii),nodes(i))
					end if
				next
			next
		end sub
		Sub DrawJoinLine(ByRef StartNode ,ByRef  EndNode)
			Dim x , y , w ,h , rotation , x0 , y0 , d0 , flip , jt , opt
			Dim rw
			x = app.iif(StartNode.x > EndNode.x,  EndNode.x , StartNode.x) + NodeWidth / 2
'Dim rw
			y = app.iif(StartNode.y > EndNode.y,  EndNode.y , StartNode.y + (NodeHeight+LineHeight)/ 2)
'Dim rw
			w = abs(StartNode.x -EndNode.x)
'Dim rw
			h = abs(abs(StartNode.y -EndNode.y) -  CInt((NodeHeight+LineHeight)/ 2))
'Dim rw
			jt = "endarrow"
			rotation = 0
			flip = y
			...
			If StartNode.x < EndNode.x And StartNode.y < EndNode.y Then
				...
				w = d0
				flip = "x"
			ElseIf StartNode.x > EndNode.x And StartNode.y < EndNode.y Then
				If w > NodeWidth*2 Then
					opt = 33
					rotation = 90
					x = x + NodeWidth / 2
					rotation = 90
			...
			ElseIf  StartNode.x >= EndNode.x And StartNode.y = EndNode.y Then
				...
				jt = "startarrow"
			ElseIf  StartNode.x >= EndNode.x And StartNode.y > EndNode.y Then
				If w > 2*NodeWidth Then
					opt = 33
					rotation = 90
					d0 =h
					h = w  - nodewidth / 2 -   nodewidth/4
					d0 =h
					w = d0 + nodeheight / 2
					d0 =h
					x = x - (w-h)/2 + nodewidth/3
					d0 =h
					y = y - (h-w)/2
					d0 =h
					y = y + NodeHeight
			...
			w = CInt(w)
			h = CInt(h)
			Response.write "<v:shapetype id=""_x0000_t34"" coordsize=""21600,21600"" o:spt=" & opt & " />"
			Response.write "<v:shape type=""#_x0000_t34""  style='display:block;position:absolute;left:" & x & "px;top:" & y & "px;width:" & w & "px;height:" & h & "px;flip:" & flip & ";rotation:" & rotation & ";z-index:-1'>"
			Response.write "<v:shapetype id=""_x0000_t34"" coordsize=""21600,21600"" o:spt=" & opt & " />"
			Response.write "     <v:stroke  color =""#9B9A99""  FillType=""Pattern"" linestyle=""ThinThin"" startarrowwidth=""narrow"" startarrowlength=""short"" endarrowwidth=""narrow"" endarrowlength=""short"" " & jt & "=""block"" />"
...
'ElseIf ptype="5" Then
	...
	Sub bill_init(bill)
		Dim Id , Code , F , Detail , selconfig , lg , lgsql , colattr , defcol , defid , defrs , defmxsavecol , deffieldbutton
		dim bc
		Id = Request.querystring("OrderId") & ""
		If Len(id) = 0 Then id = request.Form("OrderId") & ""
		If Len(id) = 0 Then  Bill.Cancel = True : Exit Sub
		If Not IsNumeric(id) Then  Bill.Cancel = True : Exit Sub
		if id < 0 then
			set bc = new Base64Class
			select case cstr(id)
			case "-1":
'select case cstr(id)
			cn.close : Response.redirect  "../../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord=" & bc.pwurl(bill.sheetno) & "&sp="
			case "-2":
			cn.close : Response.redirect  "../../store/contentck.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			case "-3"
			'cn.close : Response.redirect  "../../store/contentck.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			cn.close : Response.redirect  "../../store/contentrk.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			case "-4"
			'cn.close : Response.redirect  "../../store/contentrk.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			cn.close : Response.redirect  "../../SYSN/view/store/yugou/YuGou.ashx?view=details&ord=" & bc.pwurl(bill.sheetno) & "&sp="
			case "-5"
			cn.close : Response.redirect  "../../pay/paydetail.asp?ord=" & bill.sheetno
			case "-6"
			cn.close : Response.redirect  "../../pay/paydetail.asp?ord=" & bill.sheetno
			if isnumeric(request.querystring("ParentID")) Then
				cn.execute "exec erp_bill_createwwfk " & app.info.user & "," & request.querystring("ParentID")
				Response.write "<script language=javascript>alert(""添加成功，转到列表查看"");window.location.href=""../../money2/planall2.asp""</script>"
			end if
			case "-7"
			cn.close : Response.redirect  "../report/pieceWageDetail.asp?ord=" & bc.pwurl(bill.sheetno)
			case "-1001"
			'cn.close : Response.redirect  "../report/pieceWageDetail.asp?ord=" & bc.pwurl(bill.sheetno)
			cn.close : Response.redirect  "../../hrm/perform_content.asp?ord=" & bc.pwurl(bill.sheetno) & ""
			case "-8":
			'cn.close : Response.redirect  "../../hrm/perform_content.asp?ord=" & bc.pwurl(bill.sheetno) & ""
			cn.close : Response.redirect  "../../chance/content.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			Case "-9":
			'cn.close : Response.redirect  "../../chance/content.asp?ord=" & bc.pwurl(bill.sheetno) & "&sp="
			cn.close : Response.redirect  "../../work/content.asp?ord=" & bc.pwurl(bill.sheetno) & " "
			Case "-10":
			'cn.close : Response.redirect  "../../work/content.asp?ord=" & bc.pwurl(bill.sheetno) & " "
			cn.close : Response.redirect  "../../person/content.asp?ord=" & bc.pwurl(bill.sheetno) & " "
			case "-11"
			'cn.close : Response.redirect  "../../person/content.asp?ord=" & bc.pwurl(bill.sheetno) & " "
			cn.close : Response.redirect  "../report/pieceWageDetail.asp?ord=" & bc.pwurl(bill.sheetno)
			Case "-82":
			'cn.close : Response.redirect  "../report/pieceWageDetail.asp?ord=" & bc.pwurl(bill.sheetno)
			cn.close : Response.redirect  "../../hrm/personContent.asp?ord=" & bc.pwurl(bill.sheetno) & " "
			end select
			set bc = Nothing
			cn.close
			call db_close : Response.end
			...
			Select Case id
			Case 11:
			If sdk.power.existsManu(11)=False Then
				Dim proid : proid = -1
'If sdk.power.existsManu(11)=False Then
				Set pro_rs = cn.execute("select WAID from M_MaterialOrders where id=" & CLng("0" & request.querystring("id")) & "")
				If pro_rs.eof = False Then
					proid = pro_rs(0).value
				end if
				pro_rs.close
				If proid > 0 Then
					cn.close : Response.redirect  "readbill.asp?orderid=8&id=" & proid
				end if
			else
				bill.canScan = True
			end if
			Case 2:
			Dim ParentOrd : ParentOrd = request("ParentOrd")
			Dim ParentTag : ParentTag = request("ParentTag")
			If ParentOrd&""="3" And Len(ParentTag&"")>0 Then
				If cn.execute("select 1 from M_ManuPlanLists where mpsid="&ParentTag & " and bomid=0 ").eof=False Then
					Response.write "<script language=javascript>$(document).ready(function(){alert('有产品未设置物料清单!');})</script>"
				end if
			end if
			Case 1004:
			cn.close : Response.redirect  "../../../SYSN/view/attendance/attendancemanage/AddApply.ashx?ord=" & bill.sheetno & "&view=details&type=hsz "
			cn.close
			call db_close : Response.end
			End select
		end if
		Bill.OrderId = id
		on error resume next
		cn.execute "update zdybh set sort1 = newsort1 from ( " & vbcrlf &  _
		"select a.id as upid,b.id+5000 as newsort1 from zdybh a, M_OrderSettings b where a.sort1=b.autobh and b.autobh <> b.id+5000 " & vbcrlf &  _
		")  x where upid = id" & vbcrlf & _
		"update M_OrderSettings set autobh = 5000+id where autobh <>  5000+id"
		Set pro_rs = cn.execute("select a.qxlb,a.isGroup,a.OrderType,a.PrefixCode,a.PKColumn,a.OrderName,a.MainTable,a.Sqlstring,a.ColumnSetting,a.Macros,a.leftGroup,a.leftGroupSql,a.HasSP,a.Rules,isnull(a.ParentID,0) as ParentID,a.canCopy,a.pasteAlert,a.disuserdef,len(rtrim(isnull(a.spsql,''))) as hasList ,a.xlsout,a.Reply,a.BHFieldsName,b.exCondition from dbo.M_OrderSettings a left join BHConfigs b on a.autobh = b.id where a.ID= " & Id)
		If abs(Err.number) > 0 Then
			app.showerr "配置问题","<span class=c_r>所需要的配置字段不存在，请确认数据库是否为最新版本。</span>"
			call db_close : Response.end
			Exit Sub
		end if
		On Error goto 0
		If pro_rs.eof Then
			pro_rs.close
			Response.write "<center><br><span style='font-size:12px;color:red'>无法找到对应的配置信息</span></center>"
			pro_rs.close
			Bill.Cancel = True
			Exit Sub
		else
			Bill.qxlb             =  pro_rs.Fields("qxlb").value
			Bill.disuserdef  =  pro_rs.Fields("disuserdef").value
			Bill.POConfigID       =  pro_rs.Fields("ParentID").value
			Bill.showgroup        =      pro_rs.Fields("isGroup").value
			Bill.billName         =  pro_rs.fields("OrderName").value
			Bill.keySign  =      pro_rs.fields("PrefixCode").value & ""
			Bill.tableName       =      pro_rs.fields("MainTable").value & ""
			Bill.keyField        =      pro_rs.fields("PKColumn").value & ""
			Bill.SQL             =      ReplaceMCSql(pro_rs.fields("Sqlstring").value & "")
			Bill.needcheck   =  pro_rs.fields("HasSP").value
			Bill.canCopy         =  pro_rs.fields("CanCopy").value
			Bill.pastealert  =  pro_rs.fields("pastealert").value & ""
			lg                           =  pro_rs.fields("leftGroup").value & ""
			lgsql                        =  pro_rs.fields("leftGroupSql").value & ""
			Bill.Rule        =  pro_rs.fields("Rules").value & ""
			Bill.hasList         =  pro_rs.fields("haslist").value
			Bill.xlsout          =  abs(pro_rs.fields("xlsout").value)
			Bill.canReply    =  abs(abs(pro_rs.fields("Reply").value) = 1 And ZBRuntime.MC(12006))
			Bill.BHFieldName = pro_rs.fields("BHFieldsName") & ""
			Bill.BHexCondition = pro_rs.fields("exCondition") & ""
			bill.cmdbuttons.items(9).visible = true
			If bill.canreply = 1 Then
				If Not app.power.canReplyBill(bill.orderid, bill.sheetno) Then
					bill.canreply = 0
				end if
			end if
			If Len(lg) > 0 And Len(lgsql) > 0 Then
				Bill.HasLeftPanel = True
				Bill.lfData.Add lg
				Bill.lfData.sql = lgsql
			end if
			Call Bill.SetUIFields (pro_rs.fields("ColumnSetting").value & "")
			if App.isSub("Bill_OnLoadField") then
				call Bill_OnloadField(bill)
			end if
			Code = Trim(pro_rs.fields("Macros").value & "")
			If Len(Code) > 0 Then
				if InStr(1,code & "", "@URL:",1) = 1 Then
					execute sdk.vbs(Replace(code,"@URL:","",1,-1,1))
'if InStr(1,code & "", "@URL:",1) = 1 Then
				else
					Execute Code
				end if
		...
		if instr(1,request.servervariables("URL"),"readbill.asp",1)>0 then
			call app.add_log(1,Bill.billName & "详情")
		else
			if len(request.form) = 0 then
				if len(request.QueryString("ID")) = 0 then
					call app.add_log(1,Bill.billName & "添加")
				else
					call app.add_log(1,Bill.billName & "修改")
				end if
			else
				if lcase(request.Form("__msgid")) = "commandhand" then
					if request.Form("commandname")="删除" then
						call app.add_log(2,Bill.billName & "删除")
					end if
				end if
			end if
		end if
		Dim qx_sort1,sql_btn,isModify
		qx_sort1=pro_rs.Fields("qxlb").value
		if qx_sort1<>"" and not isnull(qx_sort1) then
			sql_btn="select top 1 id from " & Bill.tableName & " where " & Bill.keyField & "=" & Bill.sheetno
			set rs_btn=cn.execute(sql_btn)
			if rs_btn.eof then
				isModify=false
				if request.querystring("orderid")="6" Then isModify = true
			else
				isModify=true
			end if
			rs_btn.close
			set rs_btn=nothing
			dim cmd : cmd = request.form("CommandName")
			Dim cateid_qx
			cateid_qx = bill.creator
			Select Case Bill.OrderId&""
			Case "1042" :
			cateid_qx = sdk.getSqlValue("select partB from " & Bill.tableName & " where " & Bill.keyField & "=" & Bill.sheetno , 0)
			Case "8" :
			cateid_qx = sdk.getSqlValue("select cateid_WA from " & Bill.tableName & " where " & Bill.keyField & "=" & Bill.sheetno , 0)
			End Select
			if isnumeric(cateid_qx) and len(cateid_qx) > 0 then
				if cateid_qx*1 > 0 Then
					if not App.Power.CanRead(qx_sort1,cateid_qx) and len(cmd) < 2 then
						app.showerr "系统拒绝了您的访问" , "<span class=c_r>您没有" & Bill.billName & "的查看权限，访问被拒绝。</span><br><span style='float:left;display:block;padding-left:50px'><br>如有疑问，请联系系统管理员。<br>数据：" & qx_sort1 & "-" & cateid_qx & "</span>"
'if not App.Power.CanRead(qx_sort1,cateid_qx) and len(cmd) < 2 then
						call db_close : Response.end
					end if
				end if
			end if
			splogid=request.querystring("splogid")
			if splogid<>"" and isnumeric(splogid) then
				if splogid>0 then
					bill.cmdbuttons.items(5).text="改批"
				end if
			end if
			bill.power_candelete = App.Power.CanDelete(qx_sort1,bill.Creator)
			if not bill.power_candelete and not bill.deleted then bill.cmdbuttons.items(4).visible=False
			If bill.deleted  = -2 Then  bill.cmdbuttons.items(4).visible=False
			if not bill.power_candelete and not bill.deleted then bill.cmdbuttons.items(4).visible=False
			deffieldbutton =  App.Power.CanConfig(qx_sort1)
			bill.cantextcopy  =   App.Power.CanCopy(qx_sort1,bill.Creator)
			bill.power_canadd = App.power.CanAdd(qx_sort1)
			If bill.mustCanAdd then bill.power_canadd=True
			if not bill.power_canadd  then
				if isModify Or zbruntime.mc(39004) = True then
					bill.cmdbuttons.items(1).visible=false
				else
					if instr(lcase(request.ServerVariables("url")),"treeedit.asp") = 0 then
						app.showerr "系统拒绝了您的访问","您没有" & Bill.billName & "的添加权限，访问被拒绝。"
					end if
				end if
			end if
			Dim ExistsUpdatePower
			if bill.power_canupdate then bill.power_canupdate = App.power.canModify(qx_sort1,bill.Creator)
			if not bill.power_canupdate and isModify then
				bill.cmdbuttons.items(2).visible=false
				If bill.orderid<>29 Then bill.cmdbuttons.items(3).visible=False
			end if
			if not App.power.CanPrint(qx_sort1,bill.Creator) then
				bill.cmdbuttons.items(6).visible=false
			end if
			If instr(1,LCase(request.servervariables("URL")),"readbill.asp",1)>0 And  bill.cmdbuttons.items(9).visible = true Then
				if  App.power.CanChange(qx_sort1,bill.Creator) = false  then
					bill.cmdbuttons.items(9).visible = false
				end if
			else
				bill.cmdbuttons.items(9).visible = false
		...
		pro_rs.close
		set pro_rs = cn.execute("select 1 as a from dbo.syscolumns where id= object_id(N'" & bill.tablename & "')  and name='tempsave'")
		if pro_rs.eof then
			bill.cmdbuttons.items(2).visible=false
		end if
		pro_rs.close
		If Bill.HasLeftPanel = False Then
			bill.cmdbuttons.items(2).visible=false
		end if
		set pro_rs = cn.execute("select 1 from [power] where sort1=" & qx_sort1 & " and sort2=10 and ord=" & app.info.user & " and (qx_open=3 or (qx_open=1 and charindex('," & bill.Creator & ",',',' + replace(cast(qx_intro as varchar(4000)),' ','') + ',')>0))")
		bill.cmdbuttons.items(2).visible=false
		bill.cmdbuttons.items(7).visible =  not pro_rs.eof and abs(ID-6) > 0  and abs(Bill.xlsout)>0
		bill.cmdbuttons.items(2).visible=false
		pro_rs.close
		set rsdel=cn.execute("select del from " & bill.tablename & " where del=1 and id=" & bill.sheetno)
		if not rsdel.eof then
			for delidx=1 to bill.cmdbuttons.count
				bill.cmdbuttons.items(delidx).visible=false
			next
		end if
		Set pro_rs = cn.execute("select ID,PKColumnName,SubKeyName,SubTable,AutoComplete,SQLString,SaveSetting,Macros,ColAttrs,allowadd,allowdel,Formula,candr from dbo.M_OrderListSettings where OrderID= " & Id )
		While not pro_rs.eof
			Set F = Bill.GetMainFieldByDBname(pro_rs.fields("PKColumnName"))
			If TypeName(F) = "mField" Then
				defmxsavecol = ""
				Set defrs = cn.execute("select ID,fname,forder from M_CustomFields where oid=" & pro_rs.fields("ID").value &  " and ismaster=0  and IsUsing=1 and del = 0 order by forder")
				While Not defrs.eof
					defmxsavecol = defmxsavecol  & ",{us" & defrs.fields("ID").value & "}" & defrs.fields("fname").value
					defrs.movenext
				wend
				defrs.close
				Set Detail = Bill.CreateDetail()
				Detail.configID = pro_rs.fields("ID").value
				Detail.FieldAttrButton = deffieldbutton
				if instr(1,request.servervariables("URL"),"ReadBill.asp",1)>1 then
					Detail.Sql = replace(pro_rs.Fields("SQLString").value,"@ReadOnly","1")
					detail.candr =  0
				else
					detail.candr =  pro_rs.fields("candr").value
					Detail.Sql = replace(pro_rs.Fields("SQLString").value,"@ReadOnly","0")
				end if
				Detail.tableName = pro_rs.Fields("SubTable").value
				Detail.setSvFields Replace(App.SqlExtension(pro_rs.Fields("SaveSetting").value & defmxsavecol ),vbcrlf,"")
				Detail.keyField = pro_rs.Fields("SubKeyName").value
				detail.canadd = pro_rs.fields("allowadd").value
				detail.candelete = pro_rs.fields("allowdel").value
				detail.Formula = pro_rs.fields("Formula").value
				SelConfig  = Split(Replace(pro_rs.fields("AutoComplete").value & "",",",";"),";")
				colattr = pro_rs.fields("ColAttrs").value & ""
				For I = 1 To Detail.ColCount
					If InStr(Detail.heads(I).dbname ,"{us")=1 then
						defcol= Split(Detail.heads(I).dbname,"}")
						If UBound(defcol) = 1 Then
							Detail.heads(I).oldywname  = Detail.heads(I).ywname
							Detail.heads(I).ywname = defcol(1)
							Detail.heads(I).save = 1
							Detail.heads(I).edit = 1
							defid = Replace(defcol(0),"{us","")
							If IsNumeric(defid) Then
								if defid <> "999999" then
									Set defrs = cn.execute("select isnull(OptionID,0) as OptionID, isnull(canExport,1) as canExport,isnull(mustfillin,0) as notnull from M_CustomFields where ID=" & defid & " and IsUsing=1")
									If Not defrs.eof Then
										Detail.heads(I).canExport =  abs(defrs.fields("canExport").value)
										Detail.heads(I).notnull = defrs.fields("notnull").value
										If defrs.fields("OptionID").value > 0 then
											If defrs.fields(0).value < 0 then
												Detail.heads(I).selid =  abs(defrs.fields(0).value)
											else
												Detail.heads(I).selid =  10000000 + abs(defrs.fields(0).value)
												'Detail.heads(I).selid =  abs(defrs.fields(0).value)
											end if
										end if
									end if
									...
									Detail.heads(I).save = 0
									Detail.heads(I).edit = 0
								end if
							end if
						end if
					end if
				next
				If Len(colattr) > 0 Then
					colattr = Split(colattr,";")
					For I = 0 To UBound(colattr)
						colitem = Split(colattr(i) & "","=")
						If ubound(colitem) = 1 Then
							colitem(0) = Trim(LCase(colitem(0)))
							For II =  1 To  Detail.ColCount
								If LCase(trim(Detail.heads(II).ywname)) =  colitem(0) Then
									Select Case  colitem(1)
									Case "1"
									Detail.heads(II).save = 0
									Detail.heads(II).edit = 0
									Detail.heads(II).selid = 0
									Case "2"
									Detail.heads(II).htmlvisible = false
									Detail.heads(II).resize = 0
									Case "3"
									Detail.heads(II).save = 1
									Detail.heads(II).edit = 0
									Detail.heads(II).selid = 0
									Case "4"
									Detail.heads(II).save = 0
									Detail.heads(II).edit = 1
									case "5"
									Detail.heads(II).save = 0
									Detail.heads(II).edit = 0
									Detail.heads(II).htmlvisible = false
								...
								if Detail.heads(II).ywname = "只读操作" then
									if instr(1,request.ServerVariables("url") , "readbill.asp",1) = 0  then
										Detail.heads(II).htmlvisible = false
									else
										Detail.heads(II).ywname = "操作"
										Bill.addtag "明细添加新单"
									end if
								end if
							next
				...
				dim itemv
				II = 0
				If IsArray(SelConfig) Then
					if instr(join(selconfig,"|") ,"=") > 0 then
						for i = 0 to ubound(selConfig)
							itemv = split(selconfig(i),"=")
							For II = 1 To Detail.colCount
								Set col = Detail.heads(II)
								If InStr(col.oldywname,"{us")=0 and col.ywname = itemv(0) Then
									col.selid = itemv(1)
								end if
							next
						next
						...
						For I = 1 To Detail.colCount
							Set col = Detail.heads(I)
							If InStr(col.oldywname,"{us")=0 Then
								If II <= UBound(selConfig) then
									If Len(selConfig(II)) > 0 Then
										col.selid = selConfig(II)
									end if
									II = II + 1
									'col.selid = selConfig(II)
								end if
				...
				If Len(Code) > 0 Then
					...
					Execute Code
					If  abs(err.number) > 0 then
						Response.write "<error>【" &  pro_rs.fields("PKColumnName").value &  "】中宏定义错误" & vbcrlf & vbcrlf & Err.Description & "</error>"
						call db_close : Response.end
					end if
					On Error Goto 0
				end if
				Dim gl1, gl2, items
				Set defrs = cn.execute("select title, gl, name from zdy  where sort1=21 and gl>0")
				While defrs.eof=False
					If defrs(2).value = "zdy5" Then
						gl1 = defrs(0).value & Chr(1) & defrs(1).value
					elseIf defrs(2).value = "zdy6" Then
						gl2 = defrs(0).value & Chr(1) & defrs(1).value
					end if
					defrs.movenext
				wend
				defrs.close
				Dim haszdy6
				with cn.execute("select 1 from syscolumns where id=object_id('" & Detail.tablename & "') and CHARINDEX(','+name+',',',zdy1,zdy2,zdy3,zdy4,zdy5,zdy6,')>0 having count(1)= 6")
'Dim haszdy6
				haszdy6 = Not .eof
				.close
				End With
				If haszdy6 = False Then
					If LCase(Detail.tablename) = "@dissave" and bill.orderid = 2 Then
				...
				If haszdy6 Then
					For I = 1 To Detail.colCount
						Set col = Detail.heads(I)
						If InStr(col.oldywname,"{us999999")=1 Then
							If InStr(gl1, Replace(col.oldywname,"{us999999}","") & Chr(1)) = 1 then
								col.selid = 1106
								col.align = "center"
							ElseIf InStr(gl2, Replace(col.oldywname,"{us999999}","") & Chr(1))= 1 then
								col.selid = 1107
								col.align = "center"
							else
								col.edit=1
					...
					For I = 1 To Detail.colCount
						Set col = Detail.heads(I)
						If InStr(col.oldywname,"{us999999")=1 Then
							If InStr(gl1, Replace(col.oldywname,"{us999999}","") & Chr(1)) > 0 then
								col.align = "center"
								col.selid = 1106
								col.lockFormat = "true"
							ElseIf InStr(gl2, Replace(col.oldywname,"{us999999}","") & Chr(1)) > 0 then
								col.align = "center"
								col.selid = 1107
								col.lockFormat = "true"
							end if
				...
				Set F.defvalue = Detail
			end if
			pro_rs.movenext
		wend
		pro_rs.close
		If App.isSub("Bill_endConfig") Then
			Call Bill_endConfig(bill)
		end if
		If App.isSub("Bill_DoReadMode") Then
			Call Bill_DoReadMode(bill)
		end if
	end sub
	Function ReplaceMCSql(ByVal sql)
		Dim i : i = 1
		while InStr(1, sql, "@@ExistsMC(", 1) > 0 And i<100
			If InStr(1, sql, "@@ExistsMC(" & i & ")",1) > 0 Then
				sql = Replace(sql, "@@ExistsMC(" & i & ")", Abs(sdk.power.ExistsManu(i)))
			end if
			i = i+1
			sql = Replace(sql, "@@ExistsMC(" & i & ")", Abs(sdk.power.ExistsManu(i)))
		wend
	...
	function Page_onHead
		Id = Request.querystring("OrderId") & ""
		If Len(id) = 0 Then id = request.Form("OrderId") & ""
		If Len(id) = 0  Or Not IsNumeric(id) Then Exit Function
		Set rs = cn.execute("select defJSFile from M_OrderSettings where ID=" & id & " and len(isnull(DefJSFile,''))>0")
		If Not rs.eof Then
			Page_OnHead = vbcrlf & "           <script language=javascript src='" & Replace(rs.fields(0).value,"'","") & "'></script><!--单据自定义JS-->"
'If Not rs.eof Then
		end if
		rs.close
		set rs = nothing
	...
	sub App_ListDrConfig(drdat)
		dim oid , oname, tag
		oid = abs(request.querystring("sys_boid"))
		set rs = cn.execute("select orderName from M_OrderSettings where ID=" & oid)
		if rs.eof = false then
		...
		rs.close
		drdat.title = oname & "列表导入"
		drdat.fileName =oname & "列表数据"
		drdat.filters = "xls|xlsx"
		drdat.smpFilePath = "../../in/bills_" & oid & ".xls"
		drdat.helpFilePath = "../../in/bills_" & oid & ".doc"
		drdat.remark = "请参考示例excel文件，确保导入的文件格式符合要求。"
		drdat.autosave = true
		drdat.allowSize = 25*1024*1024
		drdat.modelCls = oname
	end sub
	sub App_lvw_InExcel
		dim o , smppath , fso, tag
		o = abs(request.querystring("billtype"))
		tag = request.querystring("__tag")
		smppath = server.mappath("../../in/bills_" & o & ".xls")
		set fso = server.createobject("Scripting.FileSystemObject")
		if fso.FileExists(smppath) = false  then
			Response.write "" & vbcrlf & "              <script type=""text/javascript"">alert(""缺少范例文件"");</script>" & vbcrlf & "              "
			set fso = nothing
			exit Sub
		end if
		set fso = nothing
		cols = ""
		set fscn = server.CreateObject("adodb.connection")
		fscn.open "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" & smppath & ";Extended Properties='Excel 8.0;IMEX=1;HDR=NO';"
		set rs =  fscn.execute("select * from [示例$]")
		on error resume next
		for i = 0 to rs.fields.count - 1
			set rs =  fscn.execute("select * from [示例$]")
			cols = cols & rs.fields(i).value
		...
		dim fields , uploader , db , n1 , n2
		set lvw = new listview
		set uploader = lvw.getuploader()
		db  = uploader.dbname
		if not uploader.CheckFields(cols) then
			exit sub
		end if
		...
		Response.write "<div id='data_" & db & "' style='display:none'>"
		set lvw = new listview
		lvw.id = "dr_lvw"
		lvw.sql =  "set nocount on " & vbcrlf &  ";select '" & Replace(Replace(Replace(tag,"'","''"),vbcr,""),vblf,"") & "' as text, " & app.info.user & " as UserID into #tmptagdata;select * into #billlistdata from " & db & vbcrlf & " exec erp_dr_billlist " & o & vbcrlf & " set nocount off"
		lvw.showtool = false
		Response.write lvw.innerHTML
		set lvw = nothing
		if abs(err.number) > 0 then
		...
		Response.write "" & vbcrlf & "      <script language='javascript' src='../../manufacture/inc/base.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write "'></script>" & vbcrlf & "   <script language='javascript' src='../../manufacture/inc/listview.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write "'></script>" & vbcrlf & "   <script language=javascript>" & vbcrlf & "            try" & vbcrlf & "             {" & vbcrlf & "                       var lvwdiv = document.getElementById(""listview_dr_lvw"")" & vbcrlf & "                   lvw.TryCreateHiddenPageDataToArray(lvwdiv);" & vbcrlf & "                     window.parent.parent.lvwexceldrdiv.hdataArray = lvwdiv.hdataArray;" & vbcrlf & "                        window.parent.parent.lvw.Refresh(window.parent.parent.lvwexceldrdiv);" & vbcrlf & "                   window.parent.parent.lvw.UpdateScrollBar(window.parent.parent.lvwexceldrdiv);" & vbcrlf & "           }" & vbcrlf & "               catch(e){" & vbcrlf & "                       alert(""导入过程数据错误:\n\n"" + e.message);" & vbcrlf & "         }" & vbcrlf & "       </script>" & vbcrlf & "       "
		uploader.showReport
	...
	If request("__msgId")="" Then
		Response.write "" & vbcrlf & "<style> " & vbcrlf & "a{" & vbcrlf & "    text-decoration:none;" & vbcrlf & "    color:#f30;    " & vbcrlf & "}" & vbcrlf & "p{" & vbcrlf & "    clear:both;" & vbcrlf & "    margin:0;" & vbcrlf & "    padding:.5em 0;" & vbcrlf & "}" & vbcrlf & "pre{" & vbcrlf & "    display:block;" & vbcrlf & "    font:100% ""Courier New"", Courier, monospace;" & vbcrlf & "    padding:10px;" & vbcrlf & "    border:1px solid #bae2f0;" & vbcrlf & "  " & vbcrlf & "    margin:.5em 0;" & vbcrlf & "    overflow:auto;" & vbcrlf & "    width:800px;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "img{border:none;}" & vbcrlf & "ul,li{" & vbcrlf & "    margin:0;" & vbcrlf & "    padding:0;" & vbcrlf & "}" & vbcrlf & "li{" & vbcrlf & "    list-style:none;" & vbcrlf & "    float:left;" & vbcrlf & "    display:inline;" & vbcrlf & "    margin-right:10px;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "#preview{"& vbcrlf &   "  position:absolute; "& vbcrlf &   "  border:1px solid #ccc; "& vbcrlf &    " background:#999;" & vbcrlf &     "padding:5px;" & vbcrlf &  "   display:none;" & vbcrlf &   "  color:#fff;" & vbcrlf & "    }" & vbcrlf &" </style>" & vbcrlf
	end if
	Function getsort98(ids)
		Dim f_rs
		Set f_rs=cn.execute("select sort1 from sortonehy where ord="&ids)
	...
	Function uploadtoShow(upfile,oldname)
		...
		If InStr(1,allowExt,v,1)>0 Then
			uploadtoShow="&nbsp;<span><a href='../" & upfile & "' class='preview' title='" & oldname & "' target='_blank'><img src='../../images/smico/p_tool_img.gif' alt='" & oldname & "'/></a></span>"
		else
			uploadtoShow="&nbsp;<span><a href='../../out/downfile.asp?fileSpec=" & xlsapp.HexEncode(server.mappath("../"&upfile)) & "&oldfilename="&oldname&"' target='_blank'>" & oldname & "</a></span>"
		end if
		Set xlsApp=Nothing
	end function
	
	Function getNoticeDataType(showType , showExtra)
		Dim s :s = ""
		Select Case showType
		Case "options" :
		If (ZBRuntime.MC("7000") And app.power.existsPower(5,19))                   Then s = s & app.iif(Len(s)>0 , ";","") & "合同=-1"
		'Case "options" :
		If (ZBRuntime.MC("3000") And app.power.existsPower(3,19))                   Then s = s & app.iif(Len(s)>0 , ";","") & "项目=-8"
		'Case "options" :
		If (ZBRuntime.MC("18300") And app.power.existsPower(5029,19))               Then s = s & app.iif(Len(s)>0 , ";","") & "设计=-31"
		'Case "options" :
		if (ZBRuntime.MC("18000+18100") And app.power.existsPower(52,19))   then s = s & app.iif(Len(s)>0 , ";","") & "预测单=1"
		'Case "options" :
		if (ZBRuntime.MC("18000+18200") And app.power.existsPower(50,19))   then s = s & app.iif(Len(s)>0 , ";","") & "生产计划=3"
		'Case "options" :
		if (ZBRuntime.MC("18000") And app.power.existsPower(51,19))                 then s = s & app.iif(Len(s)>0 , ";","") & "生产订单=2"
		if (ZBRuntime.MC("18000+18510") And app.power.existsPower(53,19))  Then s = s & app.iif(Len(s)>0 , ";","") & "生产下达=4"
		if (ZBRuntime.MC("18000|18002") And app.power.existsPower(59,19))  Then s = s & app.iif(Len(s)>0 , ";","") & "工作中心=7"
		if (ZBRuntime.MC("18000") And app.power.existsPower(54,19))                        then s = s & app.iif(Len(s)>0 , ";","") & "生产派工=8"
		if (ZBRuntime.MC("18000+18002") And app.power.existsPower(56,19)) Then s=s&app.iif(Len(s)>0 , ";","") & "物料清单=5"
		if (ZBRuntime.MC("18000+18002") And app.power.existsPower(59,19))  Then s = s & app.iif(Len(s)>0 , ";","") & "工艺流程=10"
		if (ZBRuntime.MC("18000+18530") And app.power.existsPower(57,19))  then s = s & app.iif(Len(s)>0 , ";","") & "生产领料=12;生产补料=13;生产退料=14;生产废料=15"
		if (ZBRuntime.MC("18000+18540") And app.power.existsPower(55,19))  Then s = s & app.iif(Len(s)>0 , ";","") & "生产进度=11;返工进度=19"
		if (ZBRuntime.MC("18000")          And app.power.existsPower(58,19))       then s = s & app.iif(Len(s)>0 , ";","") & "生产质检=17"
		if (ZBRuntime.MC("26002|18000") And app.power.existsPower(60,19))  then s = s & app.iif(Len(s)>0 , ";","") & "工价信息=16"
		if (ZBRuntime.MC("18000+18550") And app.power.existsPower(62,19))  Then s = s & app.iif(Len(s)>0 , ";","") & "生产返工=20"
		if (ZBRuntime.MC("18000") And app.power.existsPower(5025,19))              Then s = s & app.iif(Len(s)>0 , ";","") & "委外加工=25"
		if (ZBRuntime.MC("18000") And app.power.existsPower(5027,19))              Then s = s & app.iif(Len(s)>0 , ";","") & "委外质检=27"
		Case "typeid" :
		Select Case showExtra
		Case "contract" : s = -1
'Select Case showExtra
		Case "chance" :  s = -8
'Select Case showExtra
		Case "M_ProcedureProgres" :  s = -11
'Select Case showExtra
		Case "design" :  s = -31
'Select Case showExtra
		Case "M_PredictOrders": s = 1
		Case "M_ManuOrders" : s = 2
		Case "M_ManuPlans": s = 3
		Case "M_ManuOrderIssueds": s = 4
		Case "M_BOM" : s = 5
		Case "M_WorkingCenters" : s = 7
		Case "M_WorkAssigns" : s = 8
		Case "M_WorkingFlows" : s = 10
		Case "M_MaterialProgres_11" : s = 11
		Case "M_MaterialOrders_12" : s = 12
		Case "M_MaterialOrders_13" : s = 13
		Case "M_MaterialOrders_14" : s = 14
		Case "M_MaterialOrders_15" : s = 15
		Case "M_PieceRateMain" : s = 16
		Case "M_QualityTestings_17" : s = 17
		Case "M_MaterialProgresRaws" : s = 18
		Case "M_MaterialProgres_19" : s = 19
		Case "M_ProgresReturns" : s = 20
		Case "M_OutOrder" : s = 25
		Case "M_QualityTestings_27" : s = 27
		Case "M_MaterialMove" : s = 28
		Case Else : s = showExtra
		End Select
		End Select
		getNoticeDataType = s
	end function
	function App_getOrder(datatype , dataid , showType )
		Dim rs ,sql, url
		If dataid&""="" Then dataid = 0
		If datatype<>0 and dataid<>0 Then
			Dim  title ,cateid ,sort1 ,hasXq ,designTypeName ,designTypeCode
			cateid = 0
			Select Case datatype
			Case -1,11001 :
'Select Case datatype
			sql = "select title , date7 ,cateid from contract where ord = " & dataid
			sort1 = 5
			designTypeName = "合同"
			...
			Case -2 :
			url = "../../SYSN/view/sales/contract/ContractDetails.ashx"
			sql = "select title , date7 ,cateid from kuout where ord = " & dataid
			sort1 = 5
			designTypeName = "领料出库单"
			designTypeCode = "kuout"
			url = "-2"
'designTypeCode = "kuout"
			Case -4 :
'designTypeCode = "kuout"
			sql = "select title , date7 ,cateid from caigou_yg where ord = " & dataid
			sort1 = 5
			designTypeName = "生产预购单"
			designTypeCode = "caigou_yg"
			url = "-2"
'designTypeCode = "caigou_yg"
			Case -8,12001 :
'designTypeCode = "caigou_yg"
			sql = "select title , date7 ,cateid from chance where ord = " & dataid
			sort1 = 3
			designTypeName = "项目"
			...
			Case -11 :
			url = "../chance/content.asp"
			sql = "select title , indate as date7 ,creator as cateid from M_ProcedureProgres where id = " & dataid
			sort1 = 5031
			designTypeName = "工序汇报"
			designTypeCode = "M_ProcedureProgres"
			url = "../ProceProgres/content.asp"
			Case -31 :
			url = "../ProceProgres/content.asp"
			sql = "select title , indate as  date7, designer as cateid from design where id = " & dataid
			sort1 = 5029
			designTypeName = "设计"
			designTypeCode = "design"
			url = "../design/content.asp"
			Case 1
			sql = "select title ,indate date7, creator cateid from M_PredictOrders where id = " & dataid
			sort1 = 52
			designTypeName = "预测单"
			designTypeCode = "M_PredictOrders"
			url = "1"
			Case 2 :
			sql = "select title ,indate date7, creator cateid from M_ManuOrders where id = " & dataid
			sort1 = 51
			designTypeName = "生产订单"
			designTypeCode = "M_ManuOrders"
			url = "2"
			Case 3 :
			sql = "select title ,indate date7, creator cateid from M_ManuPlans where id = " & dataid
			sort1 = 50
			designTypeName = "生产计划"
			designTypeCode = "M_ManuPlans"
			url = "3"
			Case 4 :
			sql = "select title ,indate date7, creator cateid from M_ManuOrderIssueds where id = " & dataid
			sort1 = 53
			designTypeName = "生产下达"
			designTypeCode = "M_ManuOrderIssueds"
			url = "4"
			Case 5 :
			sql = "select title ,indate date7, creator cateid from M_BOM where id = " & dataid
			sort1 = 56
			designTypeName = "物料清单"
			designTypeCode = "M_BOM"
			url = "5"
			Case 51005 :
			sql = "select title ,indate date7, creator cateid from M2_BOM where id = " & dataid
			sort1 = 56
			designTypeName = "物料清单"
			designTypeCode = "M2_BOM"
			url = "5"
			Case 7 :
			sql = "select WCName title ,indate date7, creator cateid from M_WorkingCenters where id = " & dataid
			sort1 = 59
			designTypeName = "工作中心"
			designTypeCode = "M_WorkingCenters"
			url = "7"
			Case 8 :
			sql = "select title ,indate date7,creator cateid from M_WorkAssigns where id = " & dataid
			sort1 = 54
			designTypeName = "生产派工"
			designTypeCode = "M_WorkAssigns"
			url = "8"
			Case 10 :
			sql = "select WFName title ,indate date7,creator cateid from M_WorkingFlows where id = " & dataid
			sort1 = 59
			designTypeName = "工艺流程"
			designTypeCode = "M_WorkingFlows"
			url = "10"
			Case 11 :
			sql = "select title ,indate date7,creator cateid from M_MaterialProgres where id = " & dataid
			sort1 = 55
			designTypeName = "生产进度"
			designTypeCode = "M_MaterialProgres"
			url = "11"
			Case 12,13,14,15 :
			Select Case datatype
			Case 12 :designTypeName = "生产领料"
			Case 13 :designTypeName = "生产补料"
			Case 14 :designTypeName = "生产退料"
			Case 15 :designTypeName = "生产废料"
			End Select
			sql = "select title ,indate date7,creator cateid from M_MaterialOrders where id = " & dataid
			sort1 = 57
			designTypeCode = "M_MaterialOrders"
			url = datatype
			Case 16 :
			sql = "select title ,indate date7,creator cateid from M_PieceRateMain where id = " & dataid
			sort1 = 60
			designTypeName = "工价信息"
			designTypeCode = "M_PieceRateMain"
			url = "16"
			Case 17 :
			sql = "select title ,indate date7,creator cateid from M_QualityTestings where id = " & dataid
			sort1 = 58
			designTypeName = "生产质检"
			designTypeCode = "M_QualityTestings"
			url = "17"
			Case 18 :
			sql = "select title ,indate date7,creator cateid from M_MaterialProgresRaws where id = " & dataid
			sort1 = 61
			designTypeName = "物料使用"
			designTypeCode = "M_MaterialProgresRaws"
			url = "18"
			Case 19 :
			sql = "select title ,indate date7,creator cateid from M_MaterialProgres where id = " & dataid
			sort1 = 55
			...
			url = "19"
			Case 20 :
			sql = "select title ,indate date7,creator cateid from M_ProgresReturns where id = " & dataid
			sort1 = 62
			designTypeName = "生产返工"
			designTypeCode = "M_ProgresReturns"
			url = "20"
			Case 25 :
			sql = "select title ,indate date7,creator cateid from M_OutOrder where id = " & dataid
			sort1 = 5025
			designTypeName = "委外加工"
			designTypeCode = "M_OutOrder"
			url = "25"
			Case 27 :
			sql = "select title ,indate date7,creator cateid from M_QualityTestings where id = " & dataid
			sort1 = 5027
			...
			url = "27"
			Case 28 :
			sql = "select title ,indate date7,creator cateid from M_MaterialMove where id = " & dataid
			sort1 = 5028
			designTypeName = "物料调拨"
			designTypeCode = "M_MaterialMove"
			url = "28"
			End Select
			Select Case  showType
			Case 4 :
			...
			If hasXq Then
				...
				Case -1 ,-8,-11,-31:
'Select Case datatype
				url = "<a href='javaScript:void(0)' title='点击查看详情' onclick=""javascript:window.open('"& url &"?ord=" & app.base64.pwurl(dataid) & "&view=details','designType','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=280,top=180')"">"& title &"</a>&nbsp;"
'Select Case datatype
				Case Else
				url = "<a href='javaScript:void(0)' title='点击查看详情' onclick=""javascript:window.open('../manufacture/inc/Readbill.asp?orderid="& url &"&id=" & dataid & "&SplogId=0','designType','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">"& title&"</a>&nbsp;"
				End Select
			else
				url = title
			end if
			url = designTypeName & app.iif(Len(url)>0 ,"：","") & url
			End Select
		else
			url = "手动添加"
		end if
	...
	Function showGate(share , joinstr)
		Dim rs, s : s= ""
		If share="1" Then
			s = "所有人员"
		ElseIf len(share)>0 Then
			Set rs =  conn.execute("select name from gate where charindex(','+cast(ord as varchar(20))+',',',"& Replace(share," ","")&",')>0")
'ElseIf len(share)>0 Then
			while rs.eof = False
				s = s & rs.fields(0).value
				rs.movenext
				If rs.eof = False Then s = s & joinstr
			wend
			rs.close
		end if
		showGate = s
	end function
	Function AutoCreateNotice(cn , dataType, id , title , intro, share)
		Dim cmdText :cmdText = "insert into notice (title,intro,dataType ,dataid , share,creator,indate,del,NeedHandle) "&_
		" select '"& title &"','" & intro &"','" & dataType &"'," & id &",'"& share &"',"& session("personzbintel2007") &",getdate(),1,1 "
		cn.execute(cmdtext)
	end function
	Function NoticeInfoHandle(cn, dataType, pwid, UIType , handleType)
		Dim dataid : dataid = deurl(pwid)
		Dim sHtml : sHtml = showNoticeButton(cn, dataType ,dataid, true ,pwid , UIType , 0)
		If 1=2 and Len(sHtml)>0 And (handleType="审批退回" or cn.execute("select 1 from M2_ManuPlansPre where (case CreateFrom when 1 then 11001 when 2 then 14001 when 3 then 12001 else 0 end) ="& dataType &" and FromID="& dataid &" and del=1 and status<>0 ").eof=False) Then
			Dim rs , ords
			Set rs = cn.execute("select ord from power where sort1=5032 and sort2=17 and ((qx_open =1 and CHARINDEX(',"&  session("personzbintel2007") &",',','+replace(CAST(qx_intro as varchar(8000)),' ','')+',') >0 ) or qx_open =3)" )
'Dim rs , ords
			If rs.eof = false Then
				ords = ""
				While rs.eof=False
					If Len(ords)>0 Then ords = ords &","
					ords = ords & rs("ord").value
					rs.movenext
				wend
				Dim title : title = App_getOrder(datatype , dataid , 1 )
				Dim intro : intro = "用户" & showGate(session("personzbintel2007")," ") & "对" & title &"进行了" & handleType &"操作，请前往查看。"
				Call AutoCreateNotice(cn ,dataType, dataid, title , intro ,ords)
			end if
			rs.close
		end if
		NoticeInfoHandle = sHtml
	end function
	Function showNoticeButton(cn, dataType ,fromid, canAddNotice ,pwid , UIType , cateid)
		If dataType<10000 Then
			showNoticeButton_ASP  cn, dataType ,fromid, canAddNotice ,pwid , UIType , cateid
			Exit function
		end if
		Dim sHtml : sHtml = ""
		Dim canAdd : canAdd = false
		If ZBRuntime.MC(77000) And canAddNotice And sdk.power.existsPower(5032,19) Then
			If Len(cateid&"")=0 Then cateid = 0
			If sdk.power.existsPower(5032 , 13) Then
				Select case UIType
				Case "bool" : canAdd = True
				Case "url" :
				sHtml = "../../sysn/view/market/notice/notice.ashx?datatype="& dataType &"&fromid="& pwid &"&act=0"
				Case "list" :
				Response.write "<input type=""button"" name=""Submit3"" value=""通知""                                   onClick=""javascript:window.open('../../sysn/view/market/notice/notice.ashx?datatype="
				Response.write dataType
				Response.write "&fromid="
				Response.write pwid
				Response.write "','newwinAddNotice','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" class=""anybutton""/>"
				Response.write pwid
				Case Else
				Response.write "<input type=""button"" name=""Submt3"" value=""通知"" " & vbcrlf & "                                   onClick=""javascript:window.open('../../sysn/view/market/notice/notice.ashx?datatype="
				Response.write dataType
				Response.write "&fromid="
				Response.write pwid
				Response.write "','newwinAddNotice','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" class=""anybutton""/>"
				Response.write pwid
				End Select
			end if
		end if
		If UIType = "bool" Then
			showNoticeButton = canAdd
		else
			showNoticeButton = sHtml
		end if
	end function
	Function showNoticeButton_ASP(cn, dataType ,fromid, canAddNotice ,pwid , UIType , cateid)
		If ZBRuntime.MC(77000) And canAddNotice = True And sdk.power.existsPower(5032,19) Then
			If Len(cateid&"")=0 Then cateid = 0
			If sdk.power.existsPowerIntro(5032 , 13, cateid) Then
				If canAddNotice Then
					If UIType = "list" Then
						Response.write "<input type=""button"" name=""Submit3"" value=""通知""  onClick=""javascript:window.open('../notice/add.asp?datatype="
						Response.write dataType
						Response.write "&fromid="
						Response.write pwid
						Response.write "','newwinAddNotice','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" class=""anybutton""/>"
						...
						Response.write "<input type=""button"" name=""Submt3"" value=""通知"" " & vbcrlf & "                                           onClick=""javascript:window.open('../notice/add.asp?datatype="
						Response.write dataType
						Response.write "&fromid="
						Response.write pwid
						Response.write "','newwinAddNotice','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" class=""anybutton""/>"
	...
	Function showNoticeList(cn, dataType ,fromid ,pwid)
		If ZBRuntime.MC(77000)=False Or Not sdk.power.existsPower(5032,19) Then Exit Function
		Response.write "" & vbcrlf & "     <style>span.link{color:#2F496E;cursor:pointer;}</style>" & vbcrlf & " <script type=""text/javascript"" language=javascript>" & vbcrlf & "       var Bill = {" & vbcrlf & "            showunderline: function (obj, c) {obj.style.textDecoration = ""underline""; if (c) {obj.style.color = c}}," & vbcrlf & "            hideunderline: function (obj, c) {obj.style.textDecoration = ""none"";if (c) {c = c.toLowerCase();if (c==""blue"" || c == ""#0000ff"") {c = ""#2F496E"";}obj.style.color = c}}," & vbcrlf & "         SpShowList: function(OrId,BlId,logId,wName) {var t = new Date();var opener = window.open(""../manufacture/inc/Readbill.asp?orderid="" + OrId + ""&ID="" + BlId + ""&SplogId="" + logId + ""&vTime="" + t.getTime());}" & vbcrlf & "     }" & vbcrlf & "       </script>" & vbcrlf & "       "
'If ZBRuntime.MC(77000)=False Or Not sdk.power.existsPower(5032,19) Then Exit Function
		dim rs ,idx ,sql ,linkHTML
		Response.write "             " & vbcrlf & "        <tr class=""top content-split-bar"">" & vbcrlf & "                <td height=""27"" colspan=""8"">" & vbcrlf & "                        <div style=""float:left;padding-left:2px;height:100%;"">" & vbcrlf & "                            <strong class=""blue2"" >通知信息</strong>" & vbcrlf & "                          "
'dim rs ,idx ,sql ,linkHTML
		If dataType = -1 then
'dim rs ,idx ,sql ,linkHTML
...
'dim rs ,idx ,sql ,linkHTML
		...
		Response.write "" & vbcrlf & "                     </div>" & vbcrlf & "          </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "        <td colspan='8' class='name'>" & vbcrlf & "                   <table class=""detailTable"" width='100%' border='0' cellpadding='3' cellspacing='1' bgcolor='#C0CCDD' style='text-align:center'>" & vbcrlf & "                    <tr class='name tbheader'>" & vbcrlf & "              <td height=25>通知时间</td><td height=25>通知主题</td><td height=25>通知内容</td><td height=25>发布人</td><td height=25>操作</td>" & vbcrlf & "               </tr>" & vbcrlf & "       "
		if not sdk.power.existsPower(5032,1) and cn.execute("select top 1 share,Creator from notice where dataid="&fromid&" and (share='1' or ','+replace(isnull(share,''),' ','')+',' like '%,"&session("personzbintel2007")&"%,')").eof = true  then
			Response.write "<tr><td height=""27"" colSpan=5>暂无通知信息！</td></tr>"
		else
			call shownotice(cn, dataType, fromid)
		end if
		Response.write "   </table>" & vbcrlf & "     </td>" & vbcrlf & "        </tr>"
	end function
	function  shownotice(byval cn,  byval dataType, byval  fromid)
		dim  sql, idx, rs
		sql = "exec [erp_notice_List] "& session("personzbintel2007") &","& dataType &","& fromid &",'' ,'','' ,'' ,'' ,'' , 0 ,0,4 ,1, '-indate' "
'dim  sql, idx, rs
		set rs = cn.execute(sql)
		Set rs = rs.nextrecordset
		If rs.eof = True Then
...
'Do while rs.eof = False
			...
			If rs("url").value = "url" Then
				If dataType < 10000 Then
					linkHTML = "<a href='javascript:void(0)' onclick=""javascript:window.open('../notice/content.asp?ord=" & pwurl(rs("id")) & "','newwincontent','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">查看</a>"
'If dataType < 10000 Then  '
				else
					linkHTML = "<a href='javascript:void(0)' onclick=""javascript:window.open('../../sysn/view/market/notice/details.ashx?ord=" & pwurl(rs("id")) & "','newwincontent','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">查看</a>"
'If dataType < 10000 Then  '
				end if
			end if
			Response.write "<tr style='color:#5B7CAE'>"
			Response.write "<td width='20%'>" & rs.fields("indate").value & "</td>"
			Response.write "<td height=25 width='20%' style='text-align:left;padding-left:5px'>" & rs("title").value  & "</td>"
			Response.write "<td width='20%'>" & rs.fields("indate").value & "</td>"
			Response.write "<td width='20%' align='left' class='ewebeditorImg'>" & rs.fields("intro").value & "</td>"
			Response.write "<td width='20%'>" & rs.fields("addname").value & "</td>"
			Response.write "<td width='20%'>" & linkHTML & "</td>"
			Response.write "</tr>"
			rs.movenext
		Loop
		If idx>3 Then
			If dataType < 10000 Then
				Response.write "" & vbcrlf & "             <tr><td colspan='5' style='height:24px' align='right'> <a href=""javascript:void(0);"" onclick=""javascript:window.open('../notice/list.asp?datatype="
				Response.write dataType
				Response.write "&fromid="
				Response.write pwurl(fromid)
				Response.write "','newwinnotice','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;""><font class=""red"">更多通知信息..&gt;&gt;&gt;</font></a></td></tr>" & vbcrlf & "               "
				Response.write pwurl(fromid)
			else
				Response.write "" & vbcrlf & "             <tr><td colspan='5' style='height:24px' align='right'> <a href=""javascript:void(0);"" onclick=""javascript:window.open('../../sysn/view/market/notice/list.ashx?datatype="
				Response.write dataType
				Response.write "&fromid="
				Response.write pwurl(fromid)
				Response.write "','newwinnotice','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;""><font class=""red"">更多通知信息..&gt;&gt;&gt;</font></a></td></tr>" & vbcrlf & "               "
	...
	sub Page_init
		if trim(request.querystring("orderid")) = "6" then
			Response.redirect "treeedit.asp?PowerReadOnly=1&readmode=1&" & request.querystring()
			call db_close : Response.end
		end if
	end sub
	Sub Bill_DoReadMode(bill)
		Dim I
		For I = 1 To bill.cmdbuttons.count
			If I < 4 then
				Bill.cmdButtons.items(i).visible = false
			end if
		next
		for I = 1 to bill.mainfields.count
			if typename(bill.mainfields.items(i).defvalue) = "DetailFields" then
				bill.mainfields.items(i).defvalue.AutoProductLink = true
			else
				bill.mainfields.items(i).notnull = 0
			end if
		next
		Bill.canRefreshlist  = False
		Bill.HasLeftPanel = False
		Call Bill.SetAppReadState(true)
		If bill.getspStatus() = 3 Then
			Call CancelReminds(bill,"NEW")
		end if
	end sub
	Sub CancelReminds(bill,remindTypes)
		Dim sql,types,rs,i,helper
		types = Split(remindTypes,",")
		For i=0 To ubound(types)
			sql = "select * from reminderConfigs where MOrderSetting=" & Bill.OrderId & " and MBusinessType='" & types(i) & "'"
			...
			If rs.eof = False Then
				Set helper = CreateReminderHelper(cn,rs("id"),0)
				helper.cancelRemind(helper.getRemindIdByOID(Bill.sheetno))
			end if
		next
	end sub
	Sub Bill_AfterLoad(bill)
		Dim sql , rs , hs  , qx
		Response.write "<Script language = ""javascript"">"
		Dim pBillName , pBillConfigID
		If Len(bill.parentsheetno) = 0 Or Not  IsNumeric(bill.parentsheetno) = 0 Then bill.parentsheetno = 0
		If Len(bill.POConfigID) > 0 And IsNumeric(bill.POConfigID)  then
			If bill.POConfigID >  0 then
				Set rs = cn.execute("SELECT top 1 Ordername,ID,Maintable,PKColumn FROM M_OrderSettings where ID = " & bill.POConfigID)
				If Not rs.eof Then
					Response.write "Bill.InfoParent = { orderid: " & rs.fields("ID").value & ", ordername:'" & rs.fields("ordername") & "',billid :" & bill.parentsheetno & "}" & vbcrlf
				end if
				rs.close
			end if
		else
			Response.write "Bill.InfoParent = 0;"
		end if
		hs = False
		Dim noticeCheckGate : noticeCheckGate = bill.creator
		If Bill.orderid= 8 Then noticeCheckGate = cn.execute("select top 1 Cateid_WA from M_WorkAssigns where id=" & Bill.sheetno)(0)
		Response.write "Bill.addSheetButton = new Array("
		If (Bill.getspStatus = 3 or bill.needcheck=false ) and not bill.gettag("明细添加新单") then
			set rs  = app.getdatarecord(cn.execute("exec erp_bill_showaddchillbutton " & bill.orderid & " , " & bill.sheetno & " , " & app.info.user))
			while not rs.eof
				if sdk.power.existsManu(rs.fields(1).value) and (app.power.canadd(rs.fields("qxlb").value) or app.power.CheckPower(rs.fields("qxlb").value,13,noticeCheckGate)) then
					if hs then
						Response.write ","
					else
						hs = true
		...
		Response.write ");" & vbcrlf
		hs = False
		if sdk.Power.ExistsModel(77000)  And sdk.power.existsPower(5032,19) and sdk.power.existsPowerIntro(5032 , 13, noticeCheckGate) then  bill.addButtonsExtra = bill.addButtonsExtra & "notice;"
		Response.write "Bill.otherSheetButton = new Array("
		set rs  = app.getdatarecord(cn.execute("exec erp_bill_ReadPageOuterButtons  " & bill.orderid & "," & bill.sheetno & "," & app.info.user & "," & Bill.getspStatus &",'"& bill.addButtonsExtra & "'" ))
		while not rs.eof
			qx = rs.fields("qxlb").value
			if qx > 0 then
				if app.power.canadd(rs.fields("qxlb").value) then
					if hs then
						Response.write ","
					else
						hs = true
					end if
					Response.write "[""" & replace(rs.fields(0).value & "","""","\""") & """,""" & replace(rs.fields(1).value & "","""","\""") & """]"
				end if
			else
				if hs then
					Response.write ","
		...
		wend
		rs.close
		If bill.canReply Then
			if hs then
				Response.write ","
			else
				hs = true
			end if
			Response.write "[""洽谈进展"",""reply_sale.asp?sort=" & cstr(bill.orderid + 100000) & "&ord=" & app.base64.pwurl(bill.sheetno) & """]"
			hs = true
		end if
		Response.write ");"
		Response.write "</Script>"
	end sub
	sub loadchildlinksheets(bill)
		dim rs, html, gp, fd , tit
		gp = ""
		set rs = cn.execute("exec erp_bill_ChildBills " & bill.orderid & "," & bill.sheetno & "," & app.info.user)
		Set rs2 = cn.execute("exec erp_bill_ParentBills " & bill.orderid & "," & bill.sheetno & "," & app.info.user)
		bill.readModeHasChild = not rs.eof And Not rs2.eof
		If rs.eof = True And rs2.eof = True Then
			html = "<div style='width:100%；text-align:center；line-height:40px；height:40px'>没有信息！<div>"
'If rs.eof = True And rs2.eof = True Then
		else
			html = "<div id='linkOrderDiv' billid='"&bill.orderid&"' orderid='"&bill.sheetno&"' style='padding:5px'/>"
		end if
		rs.close
		rs2.close
		set rs = cn.execute("exec erp_bill_ChildBills_exists " & bill.orderid & "," & bill.sheetno & "," & app.info.user)
		bill.readModeHasChild2 = Not rs.eof
		rs.close
		if len(html) > 0 Then
			Select Case bill.orderid
			Case 2004 , 5 , 10 , 18, 2001 ,2002 :
			Case Else
			Bill.SetUIFields  gp & ";;1;space;0;" + html + ";0;关联单据;3;100"
'Case Else
			End Select
		end if
		If sdk.power.existsPower(5032,19) Then
			Set rs =  cn.execute("exec [erp_notice_List] "& app.info.user &","& bill.orderid &","& bill.sheetno &",'' ,'','' ,'' ,'' ,'' , 0 ,0,4 ,1, '-indate' ")
'If sdk.power.existsPower(5032,19) Then
			If rs.eof = False Then
				If rs("recordcount").value>0 Then Bill.SetUIFields  gp & ";;1;NoticeTable;0;0;0;通知信息;3;100"
			end if
			rs.close
		end if
		html = ""
		Set rs =  cn.execute("select id,indate, (select top 1 name from gate where ord=x.creator) as cname, remark  from erp_bill_ChangeLog x where oid=" & bill.orderid & " and bid=" & bill.sheetno & " order by id desc")
		If rs.eof = False Then
			html = "<div style='padding:5px'><table class='full lvwcss'><tr><th class='lvc'>变更时间</th><th class='lvc' colspan=2>变更原因</th><th class='lvc'>变更人员</th><th class='lvc'>详情</th></tr>"
			While rs.eof = False
				html =  html & "<tr><td class='lvc'  align='center' style='width: 230px；color:red'>" & rs("indate").value & "</td>"&_
				"<td class='lvc' style='color:blue' colspan=2>" & Replace(rs("remark").value & "",";","；") & "</td>"&_
				"<td class='lvc'  align='center' style='width: 60px'>" & Replace(rs("cname").value & "",";","；") & "</td>"&_
				"<td class='lvc' align='center' style='width: 60px'><a href='javascript:void(0)' onclick='Bill.showChangePage(" & rs("id").value & ")'>查看</a></td></tr>"
				rs.movenext
			wend
			html = html & "</table></div>"
			Bill.SetUIFields  gp & ";;1;space;0;" + html + ";0;变更记录;3;100"
			'html = html & "</table></div>"
		end if
		rs.close
		gp = "-x--" : fd = "--x-" : html = ""
		'html = html & "</table></div>"
		If Bill.canReply=1 Then
			Bill.SetUIFields  ";;1;reply;0;;0;洽谈进展;3;100"
		end if
		set rs = app.getdatarecord(cn.execute("exec erp_bill_ReadPageOuterLinks " & bill.orderid & "," & bill.sheetno & "," & app.info.user & "," & Bill.getspStatus))
		while not rs.eof
			if gp = "-x--" then gp = rs.fields(0).value
'while not rs.eof
			if fd = "--x-" then fd = rs.fields(1).value
'while not rs.eof
			if gp <> rs.fields(0).value  or  fd <> rs.fields(1).value then
				Bill.SetUIFields  replace(fd,";","；") & ";;1;space;0;" + html + ";0;" & replace(gp,";","；") & ";3;100"
'if gp <> rs.fields(0).value  or  fd <> rs.fields(1).value then
				html = ""
				gp = rs.fields(0).value
				fd = rs.fields(1).value
			end if
			if len(html) > 0 then html = html & "&nbsp；<img src='../../images/smico/spliter.gif' height=10>&nbsp；"
			if len(rs.fields(3).value) > 0 then
				html = html & "<a class=link target=_blank  href='" & replace(rs.fields(3).value,";","；") & "'>" & replace(rs.fields(2).value,";","；") & "</a>"
			else
				html = html  & replace(rs.fields(2).value,";","；")
			end if
			rs.movenext
		wend
		rs.close
...
'select case  r
			...
			Bill.SetUIFields ";;1;SpTable;0;0;0;审批详情;3;100|审批状态;;1;space;0;" & status  & ";0;基本资料;1"
			If  r = 1 Or r = 0 Then
				Set rs = cn.execute("select [name] from gate a," & bill.tablename & " b where a.ord=b." & bill.checkmanField & " and b.ID=" & bill.sheetno )
				If Not rs.eof then
					Bill.SetUIFields "|当前审批人;;1;text;0;" & rs.fields(0).value  & ";0;基本资料;1;;;;2"
				end if
			end if
		end if
	end sub
	Sub loadDesignList(bill)
		If ZBRuntime.MC(18300) And sdk.power.existsPower(5029,19) Then
			Select Case bill.orderid
			Case 2,3,4,8:
			Bill.SetUIFields ";;1;DesignTable;0;0;0;设计方案;3;100"
			End Select
		end if
	end sub
	Sub Bill_OnloadField(bill)
		dim status
		If request.querystring("changelogid")<> "" Then
		else
			call loadDesignList (bill)
			call loadSpList (bill)
			call loadchildLinkSheets (bill)
		end if
	end sub
	Sub  App_CProcChart
		Set pc = new ProcChart
		pc.MarginTop = 20
		pc.CurrNode = request.Form("orderid")
		pc.draw
		Set pc = nothing
	end sub
	Sub App_OnReadBill_Notice(ByRef bill , ByRef topsyle ,ByRef topDiv)
		Dim rs,sql
		sql = " select top 1 n.id, n.datatype , n.dataid , n.title, n.indate, g.name as addname from noticelist l "&_
		"  inner join notice n on n.id=l.notice and n.del=1 and l.del=1 and l.oid="&bill.orderid &" and l.bid="& bill.sheetno &" " &_
		"  left join gate g on g.ord=n.creator "
		Set rs = cn.execute(sql)
		If rs.eof=False Then
			topsyle = "top:72px;"
			topDiv = "<div id='topNoticeDiv' style='position:absolute;top:32px;height:40px;width:100%;background-color:green;text-align:center;line-height:40px;cursor:pointer' onclick=""Bill.openWindowDialog('../../notice/content.asp?ord="& sdk.base64.pwurl(rs("id").value) &"','newwinNotice' , 1200 , 600);"">"&_
			"<span style='color:white;'>有"& App_getOrder(rs("datatype").value , rs("dataid").value  , 1) &" 的通知【"& rs("title").value &"】，发布人员："& rs("addname").value &"&nbsp;通知时间："& rs("indate").value &"</span>"&_
			"</div>"
		end if
		rs.close
	end sub
