...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>银行汇总导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "<td class=""place"">银行汇总导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出银行汇总,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write Application("sys.info.jsver")
	dotstr=""
...
'if m1="" then
			...
			if a="3" then
				if Str_Result="" then
					str_Result="where year(date3)="&m2&" and month(date3)="&m1&""
				else
			...
			elseif a="4" then
				if Str_Result="" then
					str_Result="where year(date3)="&m2&""
				else
			...
			if b<>"" then
				if Str_Result="" then
					str_Result="where sort='"&b&"'"
				else
					Str_Result=Str_Result+"and sort='"&b&"'"
					str_Result="where sort='"&b&"'"
				end if
			end if
			if c<>"" then
				if Str_Result="" then
					str_Result="where  complete1="&c&""
				else
					Str_Result=Str_Result+"and  complete1="&c&""
					str_Result="where  complete1="&c&""
				end if
			end if
			if d<>"" then
				if Str_Result="" then
					str_Result="where  zt1="&d&""
				else
					Str_Result=Str_Result+"and  zt1="&d&""
					str_Result="where  zt1="&d&""
				end if
			end if
			if e<>"" then
				if Str_Result="" then
					str_Result="where  zt2="&e&""
				else
					Str_Result=Str_Result+"and  zt2="&e&""
					str_Result="where  zt2="&e&""
				end if
			end if
			money1all=0
			...
			"select cast(id as int) as id,identity(int,1,1) as idx into #bz from (" & vbcrlf &_
			"select top 100 percent id from sortbz order by gate1 desc" & vbcrlf &_
			") aa ;" & vbcrlf &_
			"set nocount off " & vbcrlf &_
			"select a.id,a.sort1,a.intro,a.bz from #bz b " & vbcrlf &_
			"inner join sortbank a on a.bz=b.id " & vbcrlf &_
			"where a.del=1 and (" & vbcrlf &_
			"a.person like '"&session("personzbintel2007")&",%' " & vbcrlf &_
			"or a.person like '%,"&session("personzbintel2007")&",%' " & vbcrlf &_
			"or a.person like '%, "&session("personzbintel2007")&",%' " & vbcrlf &_
			"or a.person like '%,"&session("personzbintel2007")&"' " & vbcrlf &_
			"or a.person like '%, "&session("personzbintel2007")&"' " & vbcrlf &_
			"or a.person like '"&session("personzbintel2007")&"' " & vbcrlf &_
			"or a.person like '0'" & vbcrlf &_
			")  order by b.idx,a.gate1 desc"
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "银行汇总_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0  then
				...
				do until rs.eof
					...
					if bz<>"" and  bz<>"0" then
						...
						sql7="select sort1 from sortbz where id="&bz&" "
					...
					if bz<>"" then
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select hl from hl where date1='"&date&"' and bz="&bz&" "
						rs7.open sql7,conn,1,1
						if rs7.eof then
							hl=1
						else
					...
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select sum(round(money1,"&num_dot_xs&")) as num1,sum(round(money2,"&num_dot_xs&")) as num2 from bank where del=1 and bank="&rs("id")&" and date1<'"&m1&"' group by bank "
					rs7.open sql7,conn,1,1
					if rs7.eof then
					else
						num1=zbcdbl(rs7("num1"))
						num2=zbcdbl(rs7("num2"))
					end if
					rs7.close
					set rs7=nothing
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select sum(round(money1,"&num_dot_xs&")) as num3,sum(round(money2,"&num_dot_xs&")) as num4 from bank where del=1 and bank="&rs("id")&" and date1>='"&m1&"' and date1<='"&m2&"' group by bank "
					rs7.open sql7,conn,1,1
					if rs7.eof then
					else
						num3=zbcdbl(rs7("num3"))
						num4=zbcdbl(rs7("num4"))
						money2=num3
						money3=num4
					end if
					rs7.close
					...
					set rs7=nothing
					money1all=money1all+money1*cdbl(hl)
					set rs7=nothing
					money2all=money2all+money2*cdbl(hl)
					set rs7=nothing
					money3all=money3all+money3*cdbl(hl)
					set rs7=nothing
					money4all=money4all+money4*cdbl(hl)
					set rs7=nothing
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,1).Value = rs("sort1")
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,1).font.Size=10
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,1).HorizontalAlignment=1
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,2).Value = rs("intro")
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,2).font.Size=10
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,3).Value = sortbz
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,3).font.Size=10
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,4).Value = Formatnumber(money1,num_dot_xs,-1)
					dblMinus=CDbl(getCanMinus(rs("id")))
					...
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,6).Value = Formatnumber(money3,num_dot_xs,-1)
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,6).font.Size=10
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,7).HorizontalAlignment=1
					dblMinus=CDbl(getCanMinus(rs("id")))
					xlWorksheet.Cells(1+i,7).Value = Formatnumber(dblMinus,num_dot_xs,-1)
					dblMinus=CDbl(getCanMinus(rs("id")))
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>银行汇总导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!银行汇总导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("银行汇总_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的银行汇总</u></strong></font></a></p>"
