...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			rs1.close
			set rs1=nothing
			xlWorksheet.Range("A2:R2").Borders.LineStyle=1
			Response.Flush
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=21 and sort2=10"
			rs1.open sql1,conn,1,1
...
'do until rs1.eof
		...
		date7=rs("date7")
		xlWorksheet.Cells(1+i,7).Value = num_tc
		date7=rs("date7")
		xlWorksheet.Cells(1+i,7).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,8).Value = intro1
		date7=rs("date7")
		xlWorksheet.Cells(1+i,8).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,9).Value = intro2
		date7=rs("date7")
		xlWorksheet.Cells(1+i,9).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,10).Value = intro3
		date7=rs("date7")
