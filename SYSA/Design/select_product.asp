...
'bigsystemtype = ".mozi"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "html{" & vbcrlf & "scrollbar-3dlight-color:#d0d0e8;" & vbcrlf & "scrollbar-highlight-color:#fff;" & vbcrlf & "scrollbar-face-color:#f0f0ff;" & vbcrlf & "scrollbar-arrow-color:#c0c0e8;" & vbcrlf & "scrollbar-shadow-color:#d0d0e8;" & vbcrlf & "scrollbar-darkshadow-color:#fff;" & vbcrlf & "scrollbar-base-color:#ffffff;" & vbcrlf & "scrollbar-track-color:#fff;" & vbcrlf & "}" & vbcrlf & "#cp_search{" & vbcrlf & " width:100% !important;" & vbcrlf & "  padding-top:5px;" & vbcrlf & "}" & vbcrlf & ".ie5 #cp_search{border-right:0;padding-top:5px;}" & vbcrlf & "html,body{height:100%;}" & vbcrlf & ".page2 div{" & vbcrlf & "    width:203px;" & vbcrlf & "}" & vbcrlf & "#productTree{" & vbcrlf & "      overflow-y:hidden !important;" & vbcrlf & "   margin-bottom:-20px;" & vbcrlf & "    padding-bottom:20px;" & vbcrlf & "}" & vbcrlf & ".ie5 #txtKeywords{margin-top:-1px;}" & vbcrlf & " #cpjsdiv{border-right:0!important}" & vbcrlf & " #productTree.pro-menu-wrap{width:215px;height:410px;overflow:auto!important}" & vbcrlf & " body{overflow-y:hidden!important}" & vbcrlf & " " & vbcrlf & ".pro-menu-search #__adv_search_btn{" & vbcrlf & "       border-width:0;" & vbcrlf & "       transition:all 0s;" & vbcrlf & "}" & vbcrlf & "     " & vbcrlf & ".pro-menu-search #__adv_search_btn:hover{" & vbcrlf & "       border-width:1px;" & vbcrlf & "       line-height:27px;" & vbcrlf & "       }" & vbcrlf & "</style>" & vbcrlf & "<body onmouseover=""try{ContinueMovePannel(event,1)	}catch(e){};"" onmouseup=""try{ContinueMovePannel(event,0)}catch(e){};"" style=""OVERFLOW-y:auto;;OVERFLOW-X:HIDDEN"">" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	ShowOnlyCanStoreProduct = True
	hideTreeTitle = True
	hideTreeBorder = True
	ShowOnlyHasBomProduct = (request("ShowOnlyHasBomProduct") & "" = "1")
	hasAdvSearch = (request("hasAdvSearch") & "" = "1")
...
'ElseIf treeType = "TC" Then
		...
		Response.write "&outProductStr="
		Response.write outProductStr
		Response.write "&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "                     }       " & vbcrlf & "                }catch (e){}" & vbcrlf & "            xmlHttp.open(""GET"", url, false);" & vbcrlf & "          xmlHttp.onreadystatechange = function(){" & vbcrlf & "                        if (xmlHttp.readyState < 4) return;" & vbcrlf & "                    updatePage_cp(callBack);" & vbcrlf & "                };" & vbcrlf & "              xmlHttp.send(null);  " & vbcrlf & "   }" & vbcrlf & "" & vbcrlf & "       var xmlHttpNode = GetIE10SafeXmlHttp();" & vbcrlf & " function TxmAjaxSubmit(returnUnit){" & vbcrlf & "             //获取用户输入" & vbcrlf & "          var TxmID=document.txmfrom.txm.value;"& vbcrlf &             "if (TxmID.length ==0){return;}" & vbcrlf &           "var top=document.txmfrom.top.value;" & vbcrlf &              "if (TxmID.indexOf(""："")>=0)" & vbcrlf &                "{" & vbcrlf &                        "//多行文本内容，二维码文本编码.task.2355.binary.2014.12" & vbcrlf &                  "if( TxmID.indexOf(""流水号："")==0) { sendTxmRequest(top, TxmID); }" & vbcrlf & "                 return;" & vbcrlf & "         }" & vbcrlf & "               if (TxmID.toLowerCase().indexOf(""view.asp?v"")>0)" & vbcrlf & "          {" & vbcrlf & "                       //网址信息，可能是二维码URL编码" & vbcrlf & "                 TxmID = ""QrUrl="" + TxmID.split(""view.asp?"")[1];" & vbcrlf & "             }" & vbcrlf & "               sendTxmRequest(top,TxmID,returnUnit); // 常规单		行条码" & vbcrlf & "      }" & vbcrlf & "" & vbcrlf & "       function sendTxmRequest(top,TxmID,returnUnit) {" & vbcrlf & "         returnUnit = returnUnit || '';" & vbcrlf & "          var url = """
		Response.write GetVirPath()
	...
	Call ShowLeftTree
	Response.write "" & vbcrlf & "<script type=""text/JavaScript"">" & vbcrlf & "document.getElementById(""productTree"").style.border = ""none"";" & vbcrlf & "</script>" & vbcrlf & "<script type=""text/JavaScript"" src=""search.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	If hasAdvSearch = True Then
		Response.write "" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "function Left_adSearch(){" & vbcrlf & "        var $dv = parent.$('#adsDiv');" & vbcrlf & "  if($dv.size()==0){" & vbcrlf & "              $dv=$(""#adsDiv"").clone()" & vbcrlf & "          $dv.appendTo(parent.document.body).show();" & vbcrlf & "      }" & vbcrlf & "var frm=parent.$(""#adsIF"").get(0);" & vbcrlf & "        $dv.css({left:218,top:40});" & vbcrlf & "     var h=$dv.attr('_h');" & vbcrlf & "   if($dv.attr('disp')==1){" & vbcrlf & "                $dv.hide()//.attr('disp',0);" & vbcrlf & "            parent.document.getElementById(""adsDiv"").setAttribute(""disp"",'0')" & vbcrlf & "           $('#__adv_search_btn').val(""高级"");" & vbcrlf & "     }else{" & vbcrlf & "          $dv.show()//.attr('disp',1);" & vbcrlf & "            parent.document.getElementById(""adsDiv"").setAttribute(""disp"",'1')" & vbcrlf & "           if(h!='undefined'){" & vbcrlf & "                     $dv.height(h);" & vbcrlf & "                  $(frm).height(h);" & vbcrlf & "               }else{" & vbcrlf & "                       $dv.css({height:$(frm).height()});" & vbcrlf & "              }" & vbcrlf & "               $('#__adv_search_btn').val(""收回"");" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "parent.Left_adClose = function(){" & vbcrlf & "     var $div = parent.$(""#adsDiv"");" & vbcrlf & "   $div.hide().attr({disp:0,_h:$div.height()});" & vbcrlf & "   parent.$('#proSelectFrame').get(0).contentWindow.$('#__adv_search_btn').val(""高级"");" & vbcrlf & "};" & vbcrlf & "function ajaxSubmit_page(sort1,pagenum,callBack, isadsearch){" & vbcrlf & "       $('#productTree').hide();" & vbcrlf & "       $('#cp_search').show();" & vbcrlf & " $('#cp_search').width(210);" & vbcrlf & " //获取用户输入" & vbcrlf & "  var B=document.forms[0].B.value;" & vbcrlf & "        var C=(document.forms[0].C.value==$(""#txtKeywords"").get(0).defaultValue?"""":document.forms[0].C.value);" & vbcrlf & "      var top=document.forms[0].top.value;" & vbcrlf & "    //cstore：表示是否要求显示能出库的产品，即只显示实体产品" & vbcrlf & "   var url = ""../contract/search_cp.asp?cstore=1&lv=1&P=""+pagenum+""&B=""+escape(B)+""&C=""+escape(C) +""&top=""+escape(top) +""&sort1=""+escape(sort1) + ""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "       if(parent.searchModel &&parent.searchModel==1){" & vbcrlf & "             //处理高级搜索" & vbcrlf & "          var ifcobj=parent.document.getElementById(""adsIF"").contentWindow.document;" & vbcrlf & "                var sobj=ifcobj.getElementsByTagName(""input"");" & vbcrlf & "            var txValue="""";" & vbcrlf & "           for(var i=0;i<sobj.length;i++){" & vbcrlf & "         if(sobj[i].getAttribute(""sk"")&&sobj[i].type=='text'&&sobj[i].value!=''){" & vbcrlf & "                          txValue+=(txValue==""""?"""":""&"")+sobj[i].getAttribute(""sk"")+""=""+escape(sobj[i].value);" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               sobj=ifcobj.getElementsByTagName(""select"");" & vbcrlf & "               for(var i=0;i<sobj.length;i++){" & vbcrlf & "                        if(sobj[i].getAttribute(""sk"")&&sobj[i].value!=''){" & vbcrlf & "                                txValue+=(txValue==""""?"""":""&"")+sobj[i].getAttribute(""sk"")+""=""+escape(sobj[i].value);" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               //任务：3573 直接入库明细按照产品分类检索检索出来产品不是分类内的 xieyanhui20150704" & vbcrlf & "               sobj=ifcobj.getElementsByName(""A2"");" & vbcrlf & "              var tmp="""";" & vbcrlf & "               for(var i=0;i<sobj.length;i++){" & vbcrlf & "                 if(sobj[i].checked){" & vbcrlf & "                            tmp+=(tmp==""""?"""":"","")+escape(sobj[i].value);" & vbcrlf & "                  }" & vbcrlf & "               }" & vbcrlf & "               (Math.random()*100);" & vbcrlf & " }" & vbcrlf & "       xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){updatePage_cp(callBack)};" & vbcrlf & "       xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "parent.ajaxSubmit_page = ajaxSubmit_page ;" & vbcrlf & "	//-->" & vbcrlf & "</script>" & vbcrlf & ""
'If hasAdvSearch = True Then
	end if
