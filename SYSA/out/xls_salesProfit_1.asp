...
'Set rs = conn.execute("SELECT top 1 1 FROM (SELECT id,CAST(short_str AS INT) cateid FROM dbo.split('"& intro_33_3 &"',',')) a INNER JOIN (select distinct s.cateid from send s inner join kuout k on k.ord=s.kuout and s.del=1 and k.sort1 in(1,4) and k.order1 in("& htords &")) b on a.cateid=b.cateid AND a.cateid>0")
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>销售人员利润表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">销售人员利润表导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出销售人员利润表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf
	dim MODULES
	MODULES=session("zbintel2010ms")
	set rs=server.CreateObject("adodb.recordset")
	set rs1=server.CreateObject("adodb.recordset")
	strSql="select qx_open,w3 from power2  where cateid="&session("personzbintel2007")&" and sort1=3"
	rs.open strSql,conn
	if rs.eof then
		open_3=0
		intro_3=0
	...
	SearStr = ""
	If request.form("dongjie")=1 Then
		StrDj = 1
		SearStr = SearStr & " or a.del=2"
	else
		StrDj = 0
	end if
	If request.form("huishouzhan")=1 Then
		StrDel = 1
		SearStr = SearStr & " or a.del=5"
	else
		StrDel = 0
	end if
	if open_3=3 then
		Str_Result=" where 1=1 and (a.del=1 "&SearStr&")"
	elseif open_3=1 then
		Str_Result=" where a.ord in("&intro_3&") and a.ord>0 and (a.del=1 "&SearStr&")"
	else
		open_3=" where 1=2 and (a.del=1 "&SearStr&")"
	end if
	set rs1=server.CreateObject("adodb.recordset")
	...
	if m1<>"" and isDate(m1) then
		Str_Result3=Str_Result3+" and e.date3>='"&m1&"' "
'if m1<>"" and isDate(m1) then
	end if
	if m2<>"" and isDate(m2) then
		Str_Result3=Str_Result3+" and e.date3<='"&cdate(m2)&"' "
'if m2<>"" and isDate(m2) then
	end if
	B=Conver(request("B"))
	C=Conver(request("C"))
...
'if B = "xsry" then
	...
	if total1>0 then
		str_Result2=str_Result2&" and contractTotal-productCost-thTotal-payTotal+thCost>="&total1
'if total1>0 then
	end if
	if total2>0 then
		str_Result2=str_Result2&" and contractTotal-productCost-thTotal-payTotal+thCost<="&total2
'if total2>0 then
	end if
	if total1=0 then
		total1=""
	...
	case 3
	Str_Order = " order by "& mlMoneySql("kh_jx7",formulaIdx) &" desc"
	case 4
	Str_Order = " order by "& mlMoneySql("kh_jx7",formulaIdx) &" asc"
	case 5
	Str_Order = " order by contractTotal desc"
	case 6
	Str_Order = " order by contractTotal asc"
	case 7
	Str_Order = " order by sort1 desc"
	case 8
	Str_Order = " order by sort1 asc"
	case 9
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(2).NumberFormatLocal = "#,##0."&dotstr&"_ "
...
'if ZBRuntime.MC(8000) Then
			...
			xlApplication.ActiveSheet.Columns(k).ColumnWidth=10
			xlApplication.ActiveSheet.Columns(k).HorizontalAlignment=3
			xlWorksheet.Cells(1,1).Value = "销售人员"
			xlWorksheet.Cells(1,1).font.Size=10
			xlWorksheet.Cells(1,1).font.bold=true
			xlWorksheet.Cells(1,2).Value = "利润总额"
			...
			xlWorksheet.Cells(1,4).Value = "购买总额"
			...
			if showSkMoney=1 then
				i = i + 1
'if showSkMoney=1 then
				xlWorksheet.Cells(1,i).Value = "到账金额"
				xlWorksheet.Cells(1,i).font.Size=10
				xlWorksheet.Cells(1,i).font.bold=true
			end if
			i = i + 1
			xlWorksheet.Cells(1,i).font.bold=true
			xlWorksheet.Cells(1,i).Value = "税额"
			xlWorksheet.Cells(1,i).font.Size=10
			xlWorksheet.Cells(1,i).font.bold=true
			i = i + 1
			xlWorksheet.Cells(1,i).font.bold=true
			xlWorksheet.Cells(1,i).Value = "产品成本"
			xlWorksheet.Cells(1,i).font.Size=10
			xlWorksheet.Cells(1,i).font.bold=true
			i = i + 1
			xlWorksheet.Cells(1,i).font.bold=true
			xlWorksheet.Cells(1,i).Value = "费用总额"
			xlWorksheet.Cells(1,i).font.Size=10
			xlWorksheet.Cells(1,i).font.bold=True
			if ZBRuntime.MC(8000) Then
				i = i + 1
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(1,i).Value = "退货总额"
				xlWorksheet.Cells(1,i).font.Size=10
				xlWorksheet.Cells(1,i).font.bold=true
				i = i + 1
				xlWorksheet.Cells(1,i).font.bold=true
				xlWorksheet.Cells(1,i).Value = "退货成本"
				xlWorksheet.Cells(1,i).font.Size=10
				xlWorksheet.Cells(1,i).font.bold=True
			end if
			i = i + 1
			xlWorksheet.Cells(1,i).font.bold=True
			xlWorksheet.Cells(1,i).Value = "购买次数"
			xlWorksheet.Cells(1,i).font.Size=10
			xlWorksheet.Cells(1,i).font.bold=true
			i = i + 1
			xlWorksheet.Cells(1,i).font.bold=true
			xlWorksheet.Cells(1,i).Value = "所在部门"
			xlWorksheet.Cells(1,i).font.Size=10
			xlWorksheet.Cells(1,i).font.bold=true
			Response.Flush
			sumContractTotal=0
			...
			sumThTotal=0
			sumThCost=0
			sumhtDzMoney = 0 : sumtaxTotal = 0 : sumthTaxTotal = 0 : sumsthTotal = 0
			pageContractNum=0
			strTable=" from (select a.name,a.ord,b.sort1" & _
			",contractNum=(select count(*) from contract e where del=1 and cateid=a.ord "&str_Result3&")" &_
			",contractTotal=(select isnull(sum(money2),0) from contract e where del=1 and cateid=a.ord "&str_Result3&")" &_
			"thMoneySql(""salesProfit_1"", str_Result3)" & _
			",thCost=(" & vbcrlf &_
			"select isnull(sum( isnull(c.finaMoney,0) *d.num1 / c.num1 ),0) "&_
			"from kuoutlist2 c "&_
			"inner join product pro on pro.ord=c.ord "& vbcrlf &_
			"inner join contractthlist d on c.id=d.kuoutlist2 and d.del=1 " & vbcrlf &_
			"inner join contract e on d.contract=e.ord and e.cateid=a.ord " &_
			"where c.del=1 and (c.sort1=1 or c.sort1=4) "&str_Result3&" " & vbcrlf &_
			")" & vbcrlf & _
			",productCost=(" & vbcrlf &_
			"select isnull(sum(isnull(c.FinaMoney,0)),0) " & vbcrlf &_
			"from kuoutlist2 c " & vbcrlf &_
			"inner join product p on p.ord=c.ord "& vbcrlf &_
			"inner join contract e on c.contract=e.ord and e.cateid=a.ord " &_
			"left join ku s_b on c.ku=s_b.id " & vbcrlf &_
			"where c.del=1 and (c.sort1=1 or c.sort1=4) "& str_Result3 &" " & vbcrlf &_
			")" & vbcrlf & _
			",payTotal=(select isnull(sum(c.money1*dbo.gethl(c.bz,convert(varchar(10),c.date1,120),getdate())),0) from pay c,contract e where c.del=1 and c.complete=3 and c.contract=e.ord and e.cateid=a.ord"&str_Result3&")" & _
			"taxValueSql(""salesProfit_1"", "" where 1=1 """& str_Result3&")" & vbcrlf &_
			"thTaxValueSql(""salesProfit_1"", str_Result3)" & vbcrlf &_
			"sthMoneySql(""salesProfit_1"", str_Result3)" & vbcrlf &_
			"htdzMoneySql(""salesProfit_1"", str_Result3)" & vbcrlf &_
			" from gate a left join gate1 b on a.sorce=b.ord" & Str_Result&") as t where 1=1 "&str_Result2
			strSql="select * "&strTable & Str_Order
			rs.open strSql,conn,1,1
			intCount=rs.recordcount
			xlApplication.Init "销售人员利润表_"&session("name2006chen")&".xls",intCount
			i=0 : k=0
			do while not rs.eof
				i=i+1
'do while not rs.eof
				contractTotal=cdbl(rs("contractTotal"))
				productCost=cdbl(rs("productCost"))
				payTotal=cdbl(rs("payTotal"))
				taxTotal=rs("taxTotal")
				thTaxTotal=rs("thTaxTotal")
				htDzMoney=rs("htDzMoney")
				sthTotal=rs("sthTotal")
				if ZBRuntime.MC(8000) Then
					thTotal=cdbl(rs("thTotal"))
					thCost=cdbl(rs("thCost"))
				else
					thTotal = CDbl(0)
					thCost = CDbl(0)
				end if
				if htDzMoney&"" = "" then htDzMoney = 0 else htDzMoney=cdbl(htDzMoney)
				if taxTotal&"" = "" then taxTotal = 0 else taxTotal=cdbl(taxTotal)
				if thTaxTotal&"" = "" then thTaxTotal = 0 else thTaxTotal=cdbl(thTaxTotal)
				if sthTotal&"" = "" then sthTotal = 0 else sthTotal=cdbl(sthTotal)
				contractNum=clng(rs("contractNum"))
				profitTotal=getHtMlMoney2(formulaIdx,contractTotal,htDzMoney,taxTotal,thTaxTotal,productCost,payTotal,thTotal,sthTotal,thCost)
				profitRate=getHtMlRate3(formulaIdx,profitTotal,contractTotal,htDzMoney,thTotal,sthTotal)
				showTaxValue = tdTaxValueHtml("salesProfit_1",1,formulaIdx,taxTotal,thTaxTotal)
				if showTaxValue&""= "" then showTaxValue = 0 else showTaxValue = cdbl(showTaxValue)
				showThMoneyValue = tdThMoneyHtml("salesProfit_1",1,formulaIdx,thTotal,sthTotal)
				if showThMoneyValue&""= "" then showThMoneyValue = 0 else showThMoneyValue = cdbl(showThMoneyValue)
...
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				...
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					pagehtDzMoney=pagehtDzMoney+htDzMoney
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					pagetaxTotal=pagetaxTotal+showTaxValue
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					pagePayTotal=pagePayTotal+payTotal
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					pageThTotal=pageThTotal+showThMoneyValue
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					pageContractNum=pageContractNum+contractNum
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,1).Value = rs("name")
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
...
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,2).Value = Formatnumber(profitTotal,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,2).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,3).Value = Formatnumber(profitRate,num_dot_xs,-1)&"%"
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
...
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				...
				xlWorksheet.Cells(1+i,4).Value = Formatnumber(contractTotal,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
...
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				...
				if showSkMoney=1 then
					k = k + 1
'if showSkMoney=1 then
					xlWorksheet.Cells(1+i,k).Value = Formatnumber(htDzMoney,num_dot_xs,-1)
'if showSkMoney=1 then
					xlWorksheet.Cells(1+i,k).font.Size=10
'if showSkMoney=1 then
					xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
				end if
				k = k + 1
'if showSkMoney=1 then
				xlWorksheet.Cells(1+i,k).Value = showTaxValue
'if showSkMoney=1 then
				xlWorksheet.Cells(1+i,k).font.Size=10
'if showSkMoney=1 then
				xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
				if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					k = k + 1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,k).Value = Formatnumber(productCost,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,k).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
					xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				...
				k = k + 1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).Value = Formatnumber(payTotal,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				if ZBRuntime.MC(8000) Then
					k = k + 1
'if ZBRuntime.MC(8000) Then
					xlWorksheet.Cells(1+i,k).Value = Formatnumber(showThMoneyValue,num_dot_xs,-1)
'if ZBRuntime.MC(8000) Then
					xlWorksheet.Cells(1+i,k).font.Size=10
'if ZBRuntime.MC(8000) Then
					xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if ZBRuntime.MC(8000) Then
					if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
						k = k + 1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
						xlWorksheet.Cells(1+i,k).Value = Formatnumber(thCost,num_dot_xs,-1)
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
						xlWorksheet.Cells(1+i,k).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
						xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				...
				k = k + 1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).Value = contractNum
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).HorizontalAlignment=1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				k = k + 1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).Value = rs("sort1")
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				xlWorksheet.Cells(1+i,k).font.Size=10
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
				j=j+1
'if open_5_17=3 or (open_5_17=1 and CheckPurview(intro_5_17,trim(rs("ord")))) then
			...
			set rs1=nothing
			xlWorksheet.Cells(2+i,2).Value = Formatnumber(pageProfitTotal,num_dot_xs,-1)
			set rs1=nothing
			xlWorksheet.Cells(2+i,2).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(2+i,2).HorizontalAlignment=1
			set rs1=nothing
			xlWorksheet.Cells(2+i,4).Value = Formatnumber(pageContractTotal,num_dot_xs,-1)
			set rs1=nothing
			xlWorksheet.Cells(2+i,4).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(2+i,4).HorizontalAlignment=1
			set rs1=nothing
			k = 4
			if showSkMoney=1 then
				k = k + 1
'if showSkMoney=1 then
				xlWorksheet.Cells(2+i,k).Value = Formatnumber(pagehtDzMoney,num_dot_xs,-1)
'if showSkMoney=1 then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if showSkMoney=1 then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
			end if
			k = k + 1
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).Value = Formatnumber(pagetaxTotal,num_dot_xs,-1)
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).font.Size=10
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
			k = k + 1
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).Value = Formatnumber(pageproductCost,num_dot_xs,-1)
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).font.Size=10
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
			k = k + 1
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).Value = Formatnumber(pagepayTotal,num_dot_xs,-1)
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).font.Size=10
'if showSkMoney=1 then
			xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if showSkMoney=1 then
			if ZBRuntime.MC(8000) Then
				k = k + 1
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).Value = Formatnumber(pagethTotal,num_dot_xs,-1)
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if ZBRuntime.MC(8000) Then
				k = k + 1
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).Value = Formatnumber(pagethCost,num_dot_xs,-1)
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).font.Size=10
'if ZBRuntime.MC(8000) Then
				xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if ZBRuntime.MC(8000) Then
			...
			xlWorksheet.Cells(2+i,k).Value = pageContractNum
'if ZBRuntime.MC(8000) Then
			xlWorksheet.Cells(2+i,k).font.Size=10
'if ZBRuntime.MC(8000) Then
			xlWorksheet.Cells(2+i,k).HorizontalAlignment=1
'if ZBRuntime.MC(8000) Then
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>销售人员利润表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!销售人员利润表导出成功,共有"&(i)&"条记录!</B>"";</script>"
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("销售人员利润表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的销售人员利润表</u></strong></font></a></p> " & vbcrlf & ""
