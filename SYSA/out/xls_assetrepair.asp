...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_105_10=1 then
		Str_Result=Str_Result&" and R_addcateid in ("&intro_105_10&")"
	else
		Str_Result=Str_Result&" and R_addcateid=0"
	end if
...
'if ret2<>"" then
	...
	i=1
	k=0
	sql="select a.*,b.ass_name,c.set_name from O_assRepair a left join O_asset b on a.R_assID=b.ass_id left join O_assSetRepair c on a.R_type=c.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "维护列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			R_cateid=ml.IsNum(rs("R_cateid"))
			sql8="select * from gate where ord="&R_cateid
			...
			rs8.close
			set rs8=nothing
			R_cateid_sp=ml.IsNum(rs("R_cateid_sp"))
			sql8="select * from gate where ord="&R_cateid_sp
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				R_cateid_sp=rs8("name")
			else
				R_cateid_sp=""
			end if
			rs8.close
			set rs8=nothing
			if rs("R_complete")=1 then
				R_complete="待审批"
			elseif rs("R_complete")=2 then
				R_complete="审批中"
			elseif rs("R_complete")=3 then
				R_complete="审批通过"
			elseif rs("R_complete")=4 then
				R_complete="审批通未通过"
			end if
			if rs("R_sms")=0 then
				R_sms="否"
			elseif rs("R_sms")=1 then
				R_sms="是"
			end if
			if rs("R_state")=2 then
				R_state="维护中"
			elseif rs("R_state")=3 then
				R_state="维护完毕"
			end if
			R_addcateid=ml.IsNum(rs("R_addcateid"))
			sql8="select * from gate where ord="&R_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				R_addcateid=rs8("name")
			else
				R_addcateid=""
			end if
			rs8.close
			set rs8=nothing
			R_addtime=rs("R_addtime")
			R_time=rs("R_time")
			R_alt=rs("R_alt")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>维护列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!维护列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("维护列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的维护列表</u></strong></font></a></p>"
