...
'Private Sub Class_Initialize
	...
	if request("title") = "" then
		Response.write"<script language=javascript>alert('请填写标题！');history.back()</script>"
		call db_close : Response.end
	end if
	if len(request("title")) >50 then
		Response.write"<script language=javascript>alert('标题不要超过50个字！');history.back()</script>"
		call db_close : Response.end
	end if
	if request("content") = "" then
		Response.write"<script language=javascript>alert('请填写具体内容！');history.back()</script>"
		call db_close : Response.end
	end if
	...
	'call db_close : Response.end
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".style10 { font-family: ""楷体_UTF-8"";" & vbcrlf & "        font-size: 14pt;" & vbcrlf & "}" & vbcrlf & ".style14 {font-size: 14px}" & vbcrlf & ".style15 {" & vbcrlf & "   color: #000000;" & vbcrlf & "    font-size: 14px;" & vbcrlf & "        font-weight: bold;" & vbcrlf & "}" & vbcrlf & "body {" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-left: 0px;" & vbcrlf & "       margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "     background-color: #CC99FF;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & ""
	dim sort
...
'if html="1" then
	...
	if request("edit")="" then
		sql = "select  top 0 * from learntz"
		Set rs = server.CreateObject("adodb.recordset")
		rs.Open sql,conn,3,2
		rs.addnew
		rs("date7")=now()
	else
		sql = "select * from learntz where ord="&request("id")
		Set rs = server.CreateObject("adodb.recordset")
	...
	Rs("del")=1
	rs.update
	rid = GetIdentity("learntz","ord","cateid","")
	...
	conn.close
	set conn=nothing
	Response.redirect "edit.asp?sort="&sort&""
%>
