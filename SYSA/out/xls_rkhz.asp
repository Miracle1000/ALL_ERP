...
'If Not rs.Eof Then
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>入库汇总导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">入库汇总导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf &"</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出入库汇总,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" id=""CountTXT_width"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					xApp.init Me, conn
					Set xsheet = xApp.sheets.add("入库汇总表")
					HeaderStr = "产品名称,产品编号,产品型号,产品单位,入库数量"
					if isOpenAssUnit then HeaderStr = HeaderStr & ",辅助单位,辅助数量"
...
'if m2="" then
					...
					Str_Result="where c.ord=a.kuin and a.ku>0 and c.del in (1,99) "
					if m1<>"" then
						Str_Result = Str_Result & " and c.date5>='"&m1&"' "
					end if
					if m2<>"" then
						Str_Result = Str_Result & " and c.date5<='"&m2&" 23:59:59' "
					end if
					...
					if a<>0 then
						Str_Result=Str_Result+"and  a.ku='"&a&"'"
'if a<>0 then
					end if
					if C<>"" then
						if B="cpmc" then
...
'elseif B="cpxh" then
						elseif B="glkh" then
							Str_Result=Str_Result+"and  t.ord in (select ord from tel where name like '%"&C&"%')"
'elseif B="glkh" then
						elseif B="ph" then
...
'elseif B="ckbz" then
					...
					if D<>"" and D<>"33" then
						Str_Result=Str_Result+"and   a.sort1="&D&""
'if D<>"" and D<>"33" then
					end if
...
'if not ZBRuntime.MC(17007) then
					...
					Dim rowIndex, pageCount ,sort1
					pageCount = 1
					sort1 = 0
					i=1
					sql1 ="select title,order1,type1,pricemode,unitname,isnull(AssistUnit,0) AssistUnit,AssistUnitName,sum(num2) as num2,isnull(sum(AssistNum),0) as AssistNum,sum(money1) as money1,sum(money2) as money2 from ("&_
					"select isnull(sum(round(d.num3,"&num1_dot&")),0) as num2,"&_
					"          isnull(sum(isnull(a.AssistNum,0)),0) as AssistNum, "&_
					"          isnull(sum(round(case d.money1 when null then 0 else d.money1 end,"&num_dot_xs&")),0) as money1,"&_
					"          isnull(sum(round(case when c.sort1 in (3,6,7) then cast(isnull(a.pricemonth,0)*d.num3 as decimal(25,12)) else d.money1 end,"&num_dot_xs&")),0) as money2, " &_
					"          isnull(b.title,'产品已经删除') as title,b.order1,b.type1,b.pricemode,"&_
					"          (select top 1 sort1 from sortonehy  where ord=a.unit) as unitname,a.ord,a.unit, "&_
					"          (select top 1 sort1 from sortonehy  where ord=isnull(a.AssistUnit,0)) as AssistUnitName,isnull(a.AssistUnit,0) AssistUnit "&_
					"  from kuinlist a "&_
					"  inner join kuin c WITH(NOLOCK) on a.kuin=c.ord and a.del in (1,99) and c.del in (1,99) "&_
					"   inner join ku d WITH(NOLOCK) on d.kuinlist=a.id "&_
					"   inner join sortck x4 WITH(NOLOCK) on x4.del=1 and x4.ord = d.ck and (cast(x4.intro as varchar(10)) = '0' "&_
					"       or charindex(',' + cast("&session("personzbintel2007")&" as varchar(20)) + ',',','+replace(cast(x4.intro as varchar(8000)),' ','')+',')>0) " &_
					"   inner join sortck x4 WITH(NOLOCK) on x4.del=1 and x4.ord = d.ck and (cast(x4.intro as varchar(10)) = '0' "&_
					"  left join product b on a.ord=b.ord   "&_
					"  left join caigoulist cl on cl.id=a.caigoulist and cl.del=1  "&_
					"  left join caigou e on cl.caigou=e.ord and c.sort1 in (1,4)      "&_
					"  left join contractth  co on co.ord=c.caigou "&_
					"  left join (select distinct oo.gys,mqt.id from M2_QualityTestings mqt LEFT JOIN dbo.M2_QualityTestingLists qtl ON qtl.QTID = mqt.ID and qtl.del=1  "&_
					"  LEFT join  M2_OutOrderlists wa on wa.ID = qtl.bid and wa.del=1  LEFT JOIN  M2_OutOrder oo ON oo.ID=wa.outID and oo.del=1  Where mqt.del=1 "&_
					"  ) sc on sc.id = c.fromid and c.sort1 in (5,13,14,15)     "&_
					"  left join tel t on  (case when c.sort1=2 THEN co.company  when c.sort1=10 THEN 0 when c.sort1=4 THEN c.company when c.sort1 in (5,13,14,15) THEN sc.gys ELSE e.company END )=t.ord and t.del=1  "&_
					"   "&Str_Result&_
					"  group by a.ord,b.title,b.order1,b.type1,b.pricemode,a.unit,isnull(a.AssistUnit,0) "&_
					" union all "&_
					"   select isnull(sum(round(case kh.num1 when null then 0 else kh.num1 end,"&num1_dot&")),0) as num2,"&_
					"          0 as AssistNum, "&_
					"          isnull(sum(round(case a.money1 when null then 0 else cast(cast(kh.num1 * a.money1 as decimal(25,12))/a.num1 as decimal(25,12)) end,"&num_dot_xs&")),0) as money1,"&_
					"          isnull(sum(round(case when c.sort1 in (3,6,7) then cast(isnull(a.pricemonth,0)*kh.num1 as decimal(25,12)) else cast(cast(kh.num1 * a.money1 as decimal(25,12))/a.num1 as decimal(25,12)) end,"&num_dot_xs&")),0) as money2, " &_
					"          isnull(b.title,'产品已经删除') as title,b.order1,b.type1,b.pricemode,"&_
					"          (select top 1 sort1 from sortonehy  where ord=a.unit) as unitname,a.ord,a.unit, "&_
					"          (select top 1 sort1 from sortonehy  where ord=0) as AssistUnitName,0 AssistUnit "&_
					"  from kuinlist a "&_
					"  inner join kuin c WITH(NOLOCK) on a.kuin=c.ord and a.del in (1,99) and c.del in (1,99) "&_
					"   inner join kuhclist kh WITH(NOLOCK) on kh.kuinlist=a.id "&_
					"   INNER JOIN ku WITH(NOLOCK) ON ku.id=kh.kuid "&_
					"   inner join sortck x4 WITH(NOLOCK) on x4.del=1 and x4.ord = ku.ck and (cast(x4.intro as varchar(10)) = '0' "&_
					"       or charindex(',' + cast("&session("personzbintel2007")&" as varchar(20)) + ',',','+replace(cast(x4.intro as varchar(8000)),' ','')+',')>0) " &_
					"   inner join sortck x4 WITH(NOLOCK) on x4.del=1 and x4.ord = ku.ck and (cast(x4.intro as varchar(10)) = '0' "&_
					"  left join product b on a.ord=b.ord "&_
					"  left join caigoulist cl on cl.id=a.caigoulist and cl.del=1  "&_
					...
					"  left join tel t on  (case when c.sort1=2 THEN co.company  when c.sort1=10 THEN 0 when c.sort1=4 THEN c.company when c.sort1 in (5,13,14,15) THEN sc.gys ELSE e.company END )=t.ord and t.del=1  "&_
					"   "&Str_Result&_
					"  group by a.ord,b.title,b.order1,b.type1,b.pricemode,a.unit "&_
					" ) t "&_
					"  group by title,order1,type1,pricemode,unitname,ord,unit,AssistUnitName,isnull(AssistUnit,0) "&_
					"  order by ord asc,unit asc,isnull(AssistUnit,0) asc"
					set rs1 = server.CreateObject("adodb.recordset")
					...
					if rs1.eof then
						...
						do until rs1.eof
							...
							if isOpenAssUnit then
								xsheet.writestr AssistUnitName
							...
							xsheet.writenumber AssistNum
							currProcV = Clng(FormatNumber(i/C1*100,4,-1))
							xsheet.writenumber AssistNum
							...
							If rowIndex > 40000 Then
								If pageCount = 1 then
									xsheet.title = "入库汇总表(" & pageCount & "页)"
								end if
								pageCount = pageCount + 1
								xsheet.title = "入库汇总表(" & pageCount & "页)"
								Set xsheet = xapp.sheets.add("入库汇总表(" & pageCount & "页)")
								rowIndex = 0
								xsheet.showHeader HeaderStr
								xsheet.movenext
							end if
					...
					xsheet.writecell ""
					xsheet.writecell "总计："
					xsheet.writenumber numall
					if isOpenAssUnit then
						xsheet.writecell ""
						xsheet.writenumber AssistNumAll
					end if
					Response.write "<script>CountImage.width=document.all.CountTXT_width.offsetWidth;CountTXT.innerHTML=""<font color=red><b>入库汇总导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!入库汇总导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
					xsheet.writenumber AssistNumAll
					tfile=Server.MapPath(xApp.cFileName("入库汇总表") & ".xls")
					xApp.save tfile
					xApp.dispose
					tfile = xApp.HexEncode(tfile)
					Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
					Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'入库汇总表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
					Set xApp = Nothing
					action1="入库汇总导出"
					call close_list(1)
					Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
					Response.write tfile
					Response.write """><font class=""red""><strong><u>下载导出的入库汇总表</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
