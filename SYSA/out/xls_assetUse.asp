...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			dotstr=dotstr&"0"
			next
			xlApplication.ActiveSheet.Columns(1).ColumnWidth=25
			...
			xlApplication.ActiveSheet.Columns(7).ColumnWidth=20
			...
			xlApplication.ActiveSheet.Columns(9).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(9).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(10).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(10).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(11).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(11).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(12).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(12).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(13).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(13).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(13).NumberFormatLocal = "#,##0."&dotstr&"_ "
			xlApplication.ActiveSheet.Columns(14).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(14).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(15).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(15).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(15).NumberFormatLocal = "#,##0."&dotstr&"_ "
			xlApplication.ActiveSheet.Columns(16).ColumnWidth=20
			xlApplication.ActiveSheet.Columns(16).HorizontalAlignment=3
			xlApplication.ActiveSheet.Columns(17).ColumnWidth=20
			...
			rs1.close
			set rs1=nothing
			if open_105_10=3 then
			elseif open_105_10=1 then
				Str_Result=Str_Result&" and ass_addcateid in ("&intro_105_10&")"
			else
				Str_Result=Str_Result&" and ass_addcateid=0"
			end if
...
'if D12<>"" then
	...
	M=ChangeDate(M,1)
	if M<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&"',ass_lytime)>=0 "
'if M<>"" then
	end if
	K=ChangeDate(K,2)
	if K<>"" then
		Str_Result=Str_Result+" and  datediff(s,ass_lytime,'"&K&"')>=0 "
'if K<>"" then
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&"',ass_time)>=0 "
'if L1<>"" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,ass_time,'"&L2&"')>=0 "
'if L2<>"" then
	end if
	L3=ChangeDate(L3,1)
	if L3<>"" then
...
'if L4<>"" then
	...
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',ass_addtime)>=0 "
'if ret<>"" then
	else
		ret="点击选择时间"
	end if
	...
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,ass_addtime,'"&ret2&"')>=0 "
'if ret2<>"" then
	else
		ret2="点击选择时间"
	end if
...
'if alt="1" then
	...
	i=1
	k=0
	sql="select a.*,b.sort1,c.C_name,d.sort1 as state from O_asset a left join O_assSetname b on a.ass_type=b.id left join O_assCardType c on a.ass_cartype=c.id left join O_assStatename d on a.ass_state=d.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "资产列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			ass_cateid=rs("ass_cateid")
			if ass_cateid="" or isnull(ass_cateid) then ass_cateid=0
			sql8="select * from gate where ord="&ass_cateid
			...
			rs8.close
			set rs8=nothing
			ass_lycateid=rs("ass_lycateid")
			if replace(ml.De(ass_lycateid),",","")<>"" then
				if left(ass_lycateid,1)="," then
					ass_lycateid=right(ass_lycateid,len(ass_lycateid)-1)
'if left(ass_lycateid,1)="," then
				end if
				if right(ass_lycateid,1)="," then
					ass_lycateid=left(ass_lycateid,len(ass_lycateid)-1)
'if right(ass_lycateid,1)="," then
				end if
				sql8="select * from gate where ord in("&ass_lycateid&")"
				set rs8=conn.execute(sql8)
				ass_lycateid=""
				if not rs8.eof then
					n=1
					do while not rs8.eof
						if n=rs8.RecordCount then
							ass_lycateid=ass_lycateid&rs8("name")
						else
							ass_lycateid=ass_lycateid&rs8("name")&","
						end if
						n=n+1
			...
			if rs("ass_cycle1")=1 then
				ass_cycle1="年"
			elseif rs("ass_cycle1")=2 then
				ass_cycle1="月"
			elseif rs("ass_cycle1")=3 then
				ass_cycle1="台班"
			end if
			if rs("ass_isjt")=0 then
				ass_isjt="否"
			elseif rs("ass_isjt")=1 then
				ass_isjt="是"
			end if
			ass_addcateid=rs("ass_addcateid")
			if ass_addcateid="" or isnull(ass_addcateid) then ass_addcateid=0
			sql8="select * from gate where ord="&ass_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				ass_addcateid=rs8("name")
			else
				ass_addcateid=""
			end if
			rs8.close
			set rs8=nothing
			ass_addtime=rs("ass_addtime")
			ass_using=rs("ass_using")
			ass_lytime=rs("ass_lytime")
			ass_time=rs("ass_time")
			ass_jttime=rs("ass_jttime")
			ass_note=rs("ass_note")
			sql7="select * from O_assSetMethod where id="&ml.IsNum(rs("ass_method"))
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				ass_method=rs7("set_name")
			else
				ass_method=""
			end if
			rs7.close
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).Value =rs("ass_name")
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).Value =rs("sort1")
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).Value =rs("ass_bh")
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).Value =rs("ass_xh")
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).Value =ass_cateid
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).font.Size=10
			xlWorksheet.Cells(1+i,5).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,6).Value =rs("ass_position")
			xlWorksheet.Cells(1+i,6).font.Size=10
			xlWorksheet.Cells(1+i,6).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,7).Value =rs("C_name")
			xlWorksheet.Cells(1+i,7).font.Size=10
			xlWorksheet.Cells(1+i,7).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,8).Value =rs("state")
			xlWorksheet.Cells(1+i,8).font.Size=10
			xlWorksheet.Cells(1+i,8).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,9).Value =rs("ass_jczl")
			xlWorksheet.Cells(1+i,9).font.Size=10
			xlWorksheet.Cells(1+i,9).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,10).Value =rs("ass_cycle")&ass_cycle1
			xlWorksheet.Cells(1+i,10).font.Size=10
			xlWorksheet.Cells(1+i,10).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,11).Value =ass_isjt
			xlWorksheet.Cells(1+i,11).font.Size=10
			xlWorksheet.Cells(1+i,11).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,12).Value =ass_jttime
			xlWorksheet.Cells(1+i,12).font.Size=10
			xlWorksheet.Cells(1+i,12).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,13).Value =Formatnumber(ml.IsNum(rs("ass_jcz")),num_dot_xs,-1)
			xlWorksheet.Cells(1+i,13).font.Size=10
			xlWorksheet.Cells(1+i,13).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,14).Value =rs("ass_ycycle")
			xlWorksheet.Cells(1+i,14).font.Size=10
			xlWorksheet.Cells(1+i,14).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,15).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money2"))),num_dot_xs,-1)
			xlWorksheet.Cells(1+i,15).font.Size=10
			xlWorksheet.Cells(1+i,15).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,16).Value =ass_method
			xlWorksheet.Cells(1+i,16).font.Size=10
			xlWorksheet.Cells(1+i,16).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,17).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money"))),num_dot_xs,-1)
			xlWorksheet.Cells(1+i,17).font.Size=10
			xlWorksheet.Cells(1+i,17).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,18).Value =ass_using
			xlWorksheet.Cells(1+i,18).font.Size=10
			xlWorksheet.Cells(1+i,18).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,19).Value =ass_lytime
			xlWorksheet.Cells(1+i,19).font.Size=10
			xlWorksheet.Cells(1+i,19).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,20).Value =ass_time
			xlWorksheet.Cells(1+i,20).font.Size=10
			xlWorksheet.Cells(1+i,20).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,21).Value =ass_lycateid
			xlWorksheet.Cells(1+i,21).font.Size=10
			xlWorksheet.Cells(1+i,21).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,22).Value =ass_addcateid
			xlWorksheet.Cells(1+i,22).font.Size=10
			xlWorksheet.Cells(1+i,22).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,23).Value =ass_addtime
			xlWorksheet.Cells(1+i,23).font.Size=10
			xlWorksheet.Cells(1+i,23).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,24).Value =rs("ass_pj")
			xlWorksheet.Cells(1+i,24).font.Size=10
			xlWorksheet.Cells(1+i,24).HorizontalAlignment=1
			xlWorksheet.Cells(1+i,25).Value =CutStrX(ass_note,len(ass_note))
			xlWorksheet.Cells(1+i,25).font.Size=10
			xlWorksheet.Cells(1+i,25).HorizontalAlignment=1
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>资产列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!资产列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("资产列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的资产列表</u></strong></font></a></p>"
