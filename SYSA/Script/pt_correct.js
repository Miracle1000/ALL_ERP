...
function callServer(nameitr)
{
	...
	var arr_nameitr = nameitr.split("_");
	var cpord = window.cpord;
	var cpunit = arr_nameitr[1];
	var w  = document.all[nameitr];
	if ((u_name == null) || (u_name == "")) return;
	var url = "cu.asp?name=" + escape(u_name)+"&ord="+ cpord +"&unit="+cpunit;
...
function formCheck()
{
	var open_106_13 = window.open_106_13;
	...
	{
		if(open_106_13 == "1" && document.getElementById("ifCode2").value != 1){
			if(confirm("是否同步更新二维码？")){
				document.getElementById("ifCode2").value = 1;
			}else{
				document.getElementById("ifCode2").value = 0;
			}
		}
		return true;
	}
