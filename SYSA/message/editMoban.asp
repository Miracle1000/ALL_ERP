...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN""" & vbcrlf & """http://www.w3.org/TR/html4/loose.dtd"">" & vbcrlf & "<html>" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Language"" content=""zh-cn"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>"
	Response.write title_xtjm
	Response.write "</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script language=""javascript"" src=""http://sms."
	...
	if request("ord")="" or request("ord")="0" then
	else
		ord=deurl(request("ord"))
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from mobanMessage where  del=1  and ord="&ord&""
		...
		if not rs.eof then
			...
			Response.write"<script language=javascript>alert('该模板已删除！'); history.back();</script>"
			call db_close : Response.end
		end if
	end if
	...
	Response.write "短信模板</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "        <table border=""1"" width=""100%"" id=""table2"" cellspacing=""0"" cellpadding=""5"" style=""border-collapse: collapse"" bordercolor=""#DDDDDD"">" & vbcrlf & "                            <form method=""post""   id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "         <tr>" & vbcrlf & "            <td width=""20%""  align=""right"">分类：</td>" & vbcrlf & "            <td width=""80%""  align=""left""><div   id=""mecon"">" & vbcrlf & "              <select name=""sortid"">" & vbcrlf & "                                                   "
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from sortMessage  order by order1 asc ,ord desc"
	rs.open sql,conn,1,1
	if not rs.eof then
		do while not rs.eof
	...
	if open_67_17=1 then
		Response.write "" & vbcrlf & "              <a href=""correctSort.asp"" target=""_blank"" style=""color:#588905"" >添加分类</a> "
	end if
	Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "                                    <tr>" & vbcrlf & "                                    <td width=""14%"" align=""right"" valign=""center"" class=""gray"">共享设置</td>" & vbcrlf & "                                  <td width=""86%"">" & vbcrlf & "                                <table width=""98%""  border=""0"" align=""left""  bgcolor=""#FFFFFF"" >" & vbcrlf & "    <tr>" & vbcrlf & "            <td height=""37"" colspan=""3"" class=""gray""> " & vbcrlf & "              <table width=""80%""  border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "                   <tr>" & vbcrlf & ""
	if W3="1" then
		check_mem1=""
		check_mem2="checked"
	...
	if (len(content) mod 64)>0 then
		count=len(content)\64+1
'if (len(content) mod 64)>0 then
	else
		count=len(content)\64
	end if
	Response.write "" & vbcrlf & "                         <td colspan=""4""><input name=""share"" type=""radio"" value=""0"" onClick=""document.getElementById('sort_member').style.display=''""; " & vbcrlf & "                          "
	if share<>"1" then
		Response.write("checked")
	end if
	Response.write ">" & vbcrlf & "                 <strong>共享给以下用户</strong>（在需要共享用户前面的方框打勾）</td>" & vbcrlf & "                      </tr>" & vbcrlf & "                 <tr id=""sort_member"" style="" "
	if share="1" then
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	str_w3=str_w3&worksharestr
	W1="0"
	W2="0"
	if W3<>"0" and W3<>"" Then
		W3 = sdk.FormatNumList(W3 & "",",","0")
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,sorce,sorce2 from gate where ord in ("&W3&") "&str_w3&" order by cateid asc,ord asc"
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			do until rs1.eof
				If not CheckPurview(W1,trim(rs1("sorce"))) Then
					W1 =W1&","&trim(rs1("sorce"))
				end if
				If not CheckPurview(W2,trim(rs1("sorce2"))) Then
					W2= W2&","&trim(rs1("sorce2"))
				end if
				rs1.movenext
			loop
		end if
	...
	Response.write CBaseUserTreeHtml(basesql,"", "W1","W2","W3", "",  W1 , W2,  W3)
	W1=W1&","
	W2=W2&","
	W3=W3&","
	Response.write "</td>" & vbcrlf & "              </tr>" & vbcrlf & "                  <tr>" & vbcrlf & "                      <td colspan=""4"" height=""16""><strong>" & vbcrlf & "                  <input type=""radio"" name=""share"" value=""1""  onclick=""document.getElementById('sort_member').style.display='none'"";      "& vbcrlf & " "
	if share="1" then
		Response.write("checked")
	end if
	Response.write ">" & vbcrlf & "                  对所有用户公开</strong></td>" & vbcrlf & "              </tr>  " & vbcrlf & "              <tr class=""gray"">" & vbcrlf & "                         <td colspan=""4"" height=""16"">&nbsp;</td>" & vbcrlf & "                  </tr>" & vbcrlf & "       </table></td>" & vbcrlf & "</table>                     </td>" & vbcrlf & "                           </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td  align=""right"">内容：</td>" & vbcrlf & "            <td  align=""left""><div  style=""color:#FF0000"" id=""mecon"">" & vbcrlf & "                <textarea name=""messageContent""  onKeyUp=""getWordsLength();"" style="" color:#5B7CAE;border:#CCCCCC 1px solid;overflow-y:hidden;padding:4px;"" cols=""53"" rows=""5"" id=""messageContent"" datatype=""Limit"" min=""1"" max=""500"" msg=""必填，并且字数不能超过500字"">"
	'Response.write("checked")
	Response.write content
	Response.write "</textarea><a href=""###"" onClick=""document.getElementById('messageContent').value='';document.getElementById('qccon').style.display='none';"" id=""qccon"" style=""display:none"" title=""清除内容"">【清除内容】</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "                                           <!--<tr"
	Response.write content
	if addName<>session("personzbintel2007") and session("top1zbintel2007")<>"1" then
		Response.write " style=""display:none"""
	end if
	Response.write ">" & vbcrlf & "            <td height=""20px"" align=""right"">共享：</td>" & vbcrlf & "            <td align=""left""><input name=""share"" type=""radio"" value=""1""  "
	if share=1 or share="" then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "              共享" & vbcrlf & "                <input type=""radio"" name=""share""  "
	if share=0 then
		Response.write "checked"
	end if
	Response.write " value=""0"">" & vbcrlf & "                不共享</td>" & vbcrlf & "          </tr>-->" & vbcrlf & "          <tr>" & vbcrlf & "            <td height=""20px"" align=""right"">状态：</td>" & vbcrlf & "            <td align=""left"">" & vbcrlf & "                <span id='tip'>(共<em>"
	'Response.write "checked"
	Response.write len(content)
	Response.write "</em>个字,折合"
	Response.write count
	Response.write "条短信的费用),</span><span id='tip2' style='color:#588905'>64个字/条.</span>""                                             " & vbcrlf & "            </td>" & vbcrlf & "          </tr>" & vbcrlf & "                                              <tr>" & vbcrlf & "            <td height=""20px"" align=""right"">&nbsp;</td>" & vbcrlf & "            <td align=""left""><div align=""left"" id=""subBut"" style=""color:#FF0000"">" & vbcrlf & "                <input type=""submit"" style=""cursor:hand""  class=""anybutton"" id=""button""  onclick=""if (messageContent.value.length>0){if(Sensitivewords(messageContent.value)==true){document.all.date.action = 'updateMoban.asp?ord="
	Response.write pwurl(ord)
	Response.write "';}}else{alert('模板内容不能为空!');}""   value=""保  存"" name=""B8"" >" & vbcrlf & "                &nbsp;&nbsp;&nbsp;" & vbcrlf & "<input type=""reset"" style=""cursor:hand""  class=""anybutton"" id=""B8""   value=""取  消"" name=""B82"">" & vbcrlf & "            </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "      <td  class=""page"" colspan=""2""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "          <tr>" & vbcrlf & "            <td height=""30"" ></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "           " & vbcrlf & "      </form>" & vbcrlf & "        </table>" & vbcrlf & "" & vbcrlf & "      " & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "        function getWordsLength()" & vbcrlf & "       {" & vbcrlf & "               var obj=document.getElementById('messageContent');" & vbcrlf & "              var objTip=document.getElementById('tip');" & vbcrlf & "                if(obj.value.length>0){" & vbcrlf & "                 document.getElementById(""qccon"").style.display="""";" & vbcrlf & "                  document.getElementById(""button"").disabled=""disabled"";" & vbcrlf & "              }" & vbcrlf & "               else{" & vbcrlf & "                   document.getElementById(""qccon"").style.display=""none"";" & vbcrlf & "        }" & vbcrlf & "        if (obj.value.length >= 0) {" & vbcrlf & "            objTip.innerHTML = ""(共<em>"" + Math.abs(obj.value.length) + "" </em>个字,折合"" + (obj.value.length == 0 ? 0 : (parseInt(obj.value.length / 64) + (obj.value.length % 64 != 0 ? 1: 0))) + ""条短信费用),"";" & vbcrlf & "            document.getElementById(""button"").disabled = """";" & vbcrlf & "        }" & vbcrlf & "        else" & vbcrlf & "            document.getElementById(""button"").disabled = ""disabled"";" & vbcrlf & " }" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>"
	if request("ord")="" then
		action1="添加短信模板"
	else
		action1="修改短信模板"
