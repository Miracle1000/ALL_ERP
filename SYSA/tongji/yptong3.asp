...
'Sub ShowGatherListHTMLFoot()
	...
	if px="" or instr(1,px,"-",1)<=0 then px="0-1"
	if CurrPage="" then CurrPage=1
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_1=rssort(0)
		intro_101_1=rssort(1)
	else
		open_101_1=0
		intro_101_1=""
	end if
	if intro_101_1="" then intro_101_1="0"
	If export&"" <> "1" Then
		if open_101_1="0" then
			strCate=strCate&" and 1=2"
		elseif open_101_1="1" then
			strCate=strCate&" and a.cateid in ("&intro_101_1&")"
		end if
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=15 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_15=rssort(0)
		intro_101_15=rssort(1)
	else
		open_101_15=0
		intro_101_15=""
	end if
	if intro_101_15="" then intro_101_15="0"
	if open_101_15="0" then
		strCate=strCate&" and 1=2"
	elseif open_101_15="1" then
		strCate=strCate&" and a.cateid in ("&intro_101_15&")"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=5 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_5=rssort(0)
		intro_101_5=rssort(1)
	else
		open_101_5=0
		intro_101_5=""
	end if
	if intro_101_5="" then intro_101_5="0"
	if open_101_5="0" then
		strCate=strCate&" and 1=2"
	elseif open_101_5="1" then
		strCate=strCate&" and a.cateid in ("&intro_101_5&")"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_14=rssort(0)
		intro_101_14=replace(rssort(1)," ","")
	else
		open_101_14=0
		intro_101_14=""
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=10 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_10=rssort(0)
	else
		open_101_10=0
	end if
	If export&"" = "1" Then
		if intro_101_10="" then intro_101_10="0"
		if open_101_10="0" then
			strCate=strCate&" and 1=2"
		elseif open_101_10="1" then
			strCate=strCate&" and a.cateid in ("&intro_101_10&")"
		end if
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=101 and sort2=7 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_101_7=rssort(0)
	else
		open_101_7=0
	...
	rssort.close
	set rssort=nothing
	baseSQL="select PAGE_COUNT_NUM from (SELECT PAGE_TOP_NUM   关联单据,操作时间,主键_ID,操作人员,操作人员_ID,单据类型_ID,单位_ID,仓库_ID,单据类型,用品名称,用品编号,用品型号,用品分类_ID,"&_
	"单位,仓库,批号,序列号,生产日期,有效日期,原库存数量_DONUM_DOSUM,库存变动数量_DONUM_DOSUM,现库存数量_DONUM_DOSUM FROM ("&_
	"select a.djtypeid as 单据类型_ID,a.unitid as 单位_ID,a.storeid as 仓库_ID,a.cztime as 操作时间,a.id as 主键_ID,b.name as 操作人员,a.cateid as 操作人员_ID,"&_
	"a.djtype as 单据类型,a.mxid as 关联单据,a.prod_name as 用品名称,a.prod_bh AS 用品编号,a.prod_xh as 用品型号,"&_
	"a.unit as 单位,a.store as 仓库,a.ph as 批号,a.xlh as 序列号,a.sctime as 生产日期,"&_
	"g.prod_type as 用品分类_ID, "&_
	"a.ystime as 有效日期,isnull(a.ynum,0) as 原库存数量_DONUM_DOSUM,isnull(a.bnum,0) as 库存变动数量_DONUM_DOSUM,isnull(a.xnum,0) as 现库存数量_DONUM_DOSUM from O_kulog a"&_
	" inner JOIN O_product g ON g.prod_name = a.prod_name and a.prod_bh=g.prod_bh and g.prod_del=1"&_
	" LEFT JOIN O_kuin e ON a.mxid=e.id and a.djtypeid in(1,2,4,5) "&_
	" LEFT JOIN O_kuinList f ON f.reg_fid=e.id and f.prod_id = g.id and f.prod_unit = a.unitid "&_
	" LEFT JOIN gate b ON a.cateid=b.ord where 1=1 "&strCate&" "&_
	...
	m7=request.querystring("ret7")
	m8=request.querystring("ret8")
...
'if sflg="" then
	...
	for each reqPara in request.querystring
		if instr(1,reqPara,"ypdj_")>0 then
			execute(reqPara&"="""&request.querystring(reqPara))&""""
		end if
	next
	strCondition=""
	if m1<>"" then strCondition=strCondition&" and datediff(d,'"&m1&"',操作时间)>=0"
	if m2<>"" then strCondition=strCondition&" and datediff(d,'"&m2&"',操作时间)<=0"
	if m5<>"" then strCondition=strCondition&" and datediff(d,'"&m5&"',生产日期)>=0"
	if m6<>"" then strCondition=strCondition&" and datediff(d,'"&m6&"',生产日期)<=0"
	if m7<>"" then strCondition=strCondition&" and datediff(d,'"&m7&"',有效日期)>=0"
	if m8<>"" then strCondition=strCondition&" and datediff(d,'"&m8&"',有效日期)<=0"
	if strW3<>"" then strCondition=strCondition&" and 操作人员_ID in ("&strW3&")"
	if ypdj_title<>"" then strCondition=strCondition&" and 用品名称 like '%"&ypdj_title&"%'"
	if ypdj_bh<>"" then strCondition=strCondition&" and 用品编号 like '%"&ypdj_bh&"%'"
	if ypdj_xh<>"" then strCondition=strCondition&" and 用品型号 like '%"&ypdj_xh&"%'"
	if ypdj_ph<>"" then strCondition=strCondition&" and 批号 like '%"&ypdj_ph&"%'"
	if ypdj_xlh<>"" then strCondition=strCondition&" and 序列号 like '%"&ypdj_xlh&"%'"
	if ypdj_num1<>"" then strCondition=strCondition&" and 原库存数量_DONUM_DOSUM >= " & cdbl(ypdj_num1)
	if ypdj_num2<>"" then strCondition=strCondition&" and 原库存数量_DONUM_DOSUM <= " & cdbl(ypdj_num2)
	if ypdj_num3<>"" then strCondition=strCondition&" and 库存变动数量_DONUM_DOSUM >= " & cdbl(ypdj_num3)
	if ypdj_num4<>"" then strCondition=strCondition&" and 库存变动数量_DONUM_DOSUM <= " & cdbl(ypdj_num4)
	if ypdj_num5<>"" then strCondition=strCondition&" and 现库存数量_DONUM_DOSUM >= " & cdbl(ypdj_num5)
	if ypdj_num6<>"" then strCondition=strCondition&" and 现库存数量_DONUM_DOSUM <= " & cdbl(ypdj_num6)
	if ypdj_unit<>"" then strCondition=strCondition&" and 单位_ID in ("&ypdj_unit&")"
	if ypdj_store<>"" then strCondition=strCondition&" and 仓库_ID in ("&ypdj_store&")"
	if ypdj_kuin<>"" then strCondition=strCondition&" and 单据类型_ID in ("&replace(ypdj_kuin," ","")&")"
	If ypdj_officeType&"" <> "" Then strCondition=strCondition&" and 用品分类_ID in ("& ypdj_officeType &")"
	dim gather
	set gather=new GatherList
	with gather
	.digits=num1_dot
	.title="用品日志"
	.FieldsSettingIndex=5003
	...
	function CustomShowFields(idx,rsobj)
		...
		case "原库存数量_DONUM_DOSUM11"
		value1=cdbl(rsobj("原库存数量_DONUM_DOSUM").value)
		value2=cdbl(rsobj("现库存数量_DONUM_DOSUM").value)
		valuetest=0
		if value1<value2 then
			valuetest="<span class='red'>↑"&rsobj("原库存数量_DONUM_DOSUM").value&"</span>"
		else
			valuetest="<span style='color:green;'>↓"&rsobj("原库存数量_DONUM_DOSUM").value&"</span>"
		end if
		Response.write valuetest
		case "库存变动数量_DONUM_DOSUM"
		value1=cdbl(rsobj("原库存数量_DONUM_DOSUM").value)
		value2=cdbl(rsobj("现库存数量_DONUM_DOSUM").value)
		valuetest=0
		if value1<value2 then
			valuetest="<span class='red'>↑"&rsobj("库存变动数量_DONUM_DOSUM").value&"</span>"
		else
			valuetest="<span style='color:green;'>↓"&rsobj("库存变动数量_DONUM_DOSUM").value&"</span>"
		end if
		Response.write valuetest
		case "现库存数量_DONUM_DOSUM11"
		value1=cdbl(rsobj("原库存数量_DONUM_DOSUM").value)
		value2=cdbl(rsobj("现库存数量_DONUM_DOSUM").value)
		valuetest=0
		if value1<value2 then
			valuetest="<span class='red'>↑"&rsobj("现库存数量_DONUM_DOSUM").value&"</span>"
		else
			valuetest="<span style='color:green;'>↓"&rsobj("现库存数量_DONUM_DOSUM").value&"</span>"
		end if
		Response.write valuetest
		case "关联单据"
		ord=rsobj("关联单据")
		if ord="" or isnull(ord) then ord=0
		if rsobj("单据类型").value="用品入库" or  rsobj("单据类型").value="入库删除" or rsobj("单据类型").value="入库修改" then
			sql="select * from O_kuin where id="&ord
			set rs=conn.execute(sql)
			if not rs.eof then
				if open_101_14=3 or (open_101_14=1 and IsInList(intro_101_14,rsobj("操作人员_ID"),",")) then
					Response.write "<a href=""../yp/kuinDetail.asp?ord="&ord&""" target=""_blank"" title=""查看入库单详情"">"&rs("reg_name")&"</a>"
				else
					Response.write rs("reg_name")
				end if
			else
				Response.write "单据已被删除"
			end if
			set rs=nothing
		elseif rsobj("单据类型").value="用品领用" or rsobj("单据类型").value="领用删除" then
			sql="select * from O_productOut where id="&ord
			set rs=conn.execute(sql)
			if not rs.eof then
				if open_101_14=3 or (open_101_14=1 and IsInList(intro_101_14,rsobj("操作人员_ID"),",")) then
					Response.write "<a href=""../yp/outDetail.asp?ord="&ord&""" target=""_blank"" title=""查看领用单详情"">"&rs("get_title")&"</a>"
				else
			...
			set rs=nothing
		elseif rsobj("单据类型").value="用品返还" or rsobj("单据类型").value="返还删除" then
			sql="select * from O_proReturn where id="&ord
			set rs=conn.execute(sql)
			if not rs.eof then
				if open_101_14=3 or (open_101_14=1 and IsInList(intro_101_14,rsobj("操作人员_ID"),",")) then
					Response.write "<a href=""../yp/returnDetail.asp?ord="&ord&""" target=""_blank"" title=""查看领用单详情"">"&rs("ret_title")&"</a>"
				else
					Response.write rs("ret_title")
				end if
			else
				Response.write "单据已被删除"
			end if
			set rs=nothing
		elseif rsobj("单据类型").value="用品盘点" then
			sql="select * from O_kuCheck where id="&ord
			set rs=conn.execute(sql)
			if not rs.eof then
				if open_101_14=3 or (open_101_14=1 and IsInList(intro_101_14,rsobj("操作人员_ID"),",")) then
					Response.write "<a href=""../yp/checkDetail.asp?ord="&ord&""" target=""_blank"" title=""查看领用单详情"">"&rs("che_title")&"</a>"
				else
					Response.write rs("che_title")
				end if
			else
				Response.write "单据已被删除"
			end if
...
'if CanFormatNumber(rsobj(idx).name) then
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<style>.IE5 #oneself input.anybutton,.IE5 .ser_top input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}</style>" & vbcrlf & "<div id=""dd"" class=""easyui-window""  icon=""icon-search"" title=""高级检索""  style=""width:700px;height:580px;padding:5px;background: #fafafa;top:0px;display:none""  closed=""true"" >" & vbcrlf & "   <form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc;width:98%;min-height:300"">" & vbcrlf & "               <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf &  "               <tr onMouseOut=this.style.backgroundColor="" onMouseOver=this.style.backgroundColor=""ecf5ff""> "& vbcrlf & "                       <td align=""right"" width=""80"">入库人员</td>" & vbcrlf & "                  <td align=""left"" colspan=""3"">" &vbcrlf & "                         "
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
		...
		Response.write "                     </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品名称</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_title"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_title
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">用品编号</td>" & vbcrlf & "             <td width=""177"" align=""left""><input name=""ypdj_bh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_bh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "              </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">用品型号</td>" & vbcrlf & "                   <td align=""left""><input name=""ypdj_xh"" type=""text"" onFocus=""this.select();"" value="""
		Response.write ypdj_xh
		Response.write """ style=""width:110px"" class=""searchInput""/></td>" & vbcrlf & "                      <td align=""right"">批号</td>" & vbcrlf & "                       <td align=""left""><input name=""ypdj_ph"" type=""text"" class=""searchInput"" id=""ypdj_ph"" style=""width:110px"" onFocus=""this.select();"" value="""
		Response.write ypdj_ph
		Response.write """/></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td align=""right"">序列号</td>" & vbcrlf & "             <td align=""left""><input name=""ypdj_xlh"" type=""text"" class=""searchInput"" id=""ypdj_xlh"" style=""width:110px"" onFocus=""this.select();"" value="""
		Response.write ypdj_xlh
		Response.write """/>          </td>" & vbcrlf & "                        <td align=""right"">原库存数量</td>" & vbcrlf & "                 <td align=""left""><input name=""ypdj_num1"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		Response.write ypdj_num1
		Response.write """ class=""searchInput"" />" & vbcrlf & "                      -" & vbcrlf & "                       <input name=""ypdj_num2"" style=""width:50px"" type=""text"" onFocus=""this.select();"" onpropertychange=""formatData(this,'number');"" value="""
		'Response.write ypdj_num1
		Response.write ypdj_num2
		Response.write """ class=""searchInput"" /></td>" & vbcrlf & "               </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td align=""right"">库存变动数量</td>" & vbcrlf & "               <td align=""left""><input name=""ypdj_num3"" type=""text"" class=""searchInput"" id=""ypdj_num3"" style=""width:50px"" onFocus=""this.select();"" value="""
		Response.write ypdj_num3
		Response.write """ onpropertychange=""formatData(this,'number');"" />" & vbcrlf & "              -" & vbcrlf & "               <input name=""ypdj_num4"" type=""text"" class=""searchInput"" id=""ypdj_num4"" style=""width:50px"" onFocus=""this.select();"" value="""
		'Response.write ypdj_num3
		Response.write ypdj_num4
		Response.write """ onpropertychange=""formatData(this,'number');"" /></td>" & vbcrlf & "                     <td align=""right"">现库存数量</td>" & vbcrlf & "                 <td align=""left""><input name=""ypdj_num5"" type=""text"" class=""searchInput"" id=""ypdj_num5"" style=""width:50px"" onFocus=""this.select();"" value="""
		Response.write ypdj_num5
		Response.write """ onpropertychange=""formatData(this,'number');"" />" & vbcrlf & "                    -" & vbcrlf & "                       <input name=""ypdj_num6"" type=""text"" class=""searchInput"" id=""ypdj_num6"" style=""width:50px"" onFocus=""this.select();"" value="""
		'Response.write ypdj_num5
		Response.write ypdj_num6
		Response.write """ onpropertychange=""formatData(this,'number');"" /></td>" & vbcrlf & "             </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">用品单位</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                    "
		sql8="select id, sort1 from sortonehy where gate2=605 order by gate1 desc"
		set rs8=conn.execute(sql8)
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "                     <input name=""ypdj_unit"" type=""checkbox"" value="""
		...
		set rs8=nothing
		Response.write "               </td>" & vbcrlf & "         </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td align=""right"" width=""97"">所在仓库</td>" & vbcrlf & "                  <td align=""left"" colspan=""3"">" & vbcrlf & "                       "
		sql8="select id, sto_name from O_store where ( charindex(',"&session("personzbintel2007")&",',','+replace(cast(sto_intro as varchar(8000)),' ','')+',')>0 or sto_intro='' or sto_intro='0')  order by sto_gate2 desc"
		set rs8=conn.execute(sql8)
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "                     <input name=""ypdj_store"" type=""checkbox"" value="""
				Response.write rs8("id")
				Response.write """>"
		...
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "               <td align=""right"">用品分类</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">"
		sql8="select id, pro_name from O_officeType order by pro_gate2 desc"
		set rs8=conn.execute(sql8)
		if not rs8.eof then
			do while not rs8.eof
				Response.write "" & vbcrlf & "              <input name=""ypdj_officeType"" type=""checkbox"" utype=""officeType"" value="""
				Response.write rs8("id")
				Response.write """>" & vbcrlf & "              "
				Response.write rs8("pro_name")
				rs8.movenext
			loop
		end if
		set rs8=nothing
		Response.write "</td>" & vbcrlf & "                  </tr>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">操作时间</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       <input name=""ret3"" type=""text"" size=""9"" value="""" id=""daysOfMonthPosx"" onmouseup=toggleDatePicker(""daysOfMonthx"",""date.ret3"") readonly=""true""><DIV id=daysOfMonthx style=""POSITION: absolute;z-index:10""></DIV>&nbsp;-&nbsp;<input name=""ret4"" type=""text"" size=""9"" value="""" id=""daysOfMonthPosy"" onmouseup=toggleDatePicker(""daysOfMonthy"",""date.ret4"") readonly=""true""><DIV id=daysOfMonthy style=""POSITION: absolute;z-index:10""></DIV>                   </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">生产时间</td>" & vbcrlf & "                       <td align=""left"" colspan=""3"">" & vbcrlf & "                       <input name=""ret5"" type=""text"" size=""9"" value="""" id=""daysOfMonthPos1"" onmouseup=toggleDatePicker(""daysOfMonth1"",""date.ret5"") readonly=""true""><DIV id=daysOfMonth1 style=""POSITION: absolute;z-index:10""></DIV>&nbsp;-&nbsp;<input name=""ret6"" type=""text"" size=""9"" value="""" id=""daysOfMonthPos9"" onmouseup=toggleDatePicker(""daysOfMonth9"",""date.ret6"") readonly=""true""><DIV id=daysOfMonth9 style=""POSITION: absolute;z-index:10""></DIV>                   </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                     <td align=""right"">有效时间</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                       <input name=""ret7"" type=""text"" size=""9"" value="""" id=""daysOfMonthPos3"" onmouseup=toggleDatePicker(""daysOfMonth3"",""date.ret7"") readonly=""true""><DIV id=daysOfMonth3 style=""POSITION: absolute;z-index:10""></DIV>&nbsp;-&nbsp;<input name=""ret8"" type=""text"" size=""9"" value="""" id=""daysOfMonthPos4"" onmouseup=toggleDatePicker(""daysOfMonth4"",""date.ret8"") readonly=""true""><DIV id=daysOfMonth4 style=""POSITION: absolute;z-index:10""></DIV>                   </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td align=""right"">单据类型</td>" & vbcrlf & "                   <td align=""left"" colspan=""3"">" & vbcrlf & "                      <input name=""ypdj_kuin"" type=""checkbox"" value=""1"">用品入库" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""2"">入库修改" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""3"">入库删除" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""4"">用品领用"& vbcrlf & "                      <input name=""ypdj_kuin"" type=""checkbox"" value=""5"">领用修改" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""6"">领用删除" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""7"">用品返还" & vbcrlf & "                    <input name=""ypdj_kuin"" type=""checkbox"" value=""8"">返还删除" & vbcrlf & "                       <input name=""ypdj_kuin"" type=""checkbox"" value=""9"">用品盘点</td>" & vbcrlf & "               </tr>" & vbcrlf & "           </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <input type=""hidden"" name=""sflg"" value=""1"">" & vbcrlf & "           <input type=""hidden"" name=""page_count"" value="""
		Response.write page_count
		Response.write """>" & vbcrlf & "                <input type=""hidden"" name=""px"" value="""
		Response.write px
	...
	sub CurstomButtons()
		Response.write "" & vbcrlf & "<input type=""hidden"" name=""px"" value="""
		Response.write px
		Response.write """>" & vbcrlf & "<input type=""hidden"" name=""sflg"" value=""1""/>" & vbcrlf & "<select name=""page_count"" onChange=""if(this.selectedIndex && this.selectedIndex!=0 ){gotourl('page_count='+this.value);}"">" & vbcrlf & "<option value='"
		Response.write px
		Response.write page_count
		Response.write "'>-请选择-</option>" & vbcrlf & "<option value=""10"" "
		'Response.write page_count
		...
		Response.write "'>"
		Response.write "" & vbcrlf & "<input type=""submit"" value=""查看"" class=""anybutton""/><input type=""button"" class=""anybutton"" value=""高级检索"" onClick=""open1();""/>"
		if open_101_10<>0 Then
			Response.write "<input type=""button"" value=""导出""  onClick=""if(confirm('确定要导出吗？')){window.location.href = ('"
			Response.write request("script_name")
			Response.write "?'+getUrl('export=1'));}"" class=""anybutton""/>"
			'Response.write request("script_name")
		end if
		if open_101_7<>0 Then
			Response.write "<input type=""button"" value=""打印""  onClick=""window.print();"" class=""anybutton""/>"
		end if
	end sub
	call close_list(1)
