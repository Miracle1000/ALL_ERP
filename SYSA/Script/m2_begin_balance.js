...
function updateTotal(id){
	var v = $("input[name=moneyall1_"+id+"]").val();
...
function batchDel(form){
	...
	{			
		...
		{
		if(chkstring==0) {
			if(window.confirm("确定要批量删除吗？")==false) { return; }
		}
		chkstring=1;
...
function check_kh2(ord,ids){
  ids1=ids;
  var url = "../event/search_kh3.asp?mode=test&ord="+escape(ord)+"&sort3=2&id="+escape(ids)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
...
function resetselect(){
	  var url = "../event/search_kh3.asp?mode=clear&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
...
function check_kh(ord,ids) {
  ...
  var url = "../event/search_kh3.asp?ord="+escape(ord)+"&id="+escape(ids)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
  XMlHttp.open("GET", url, false);
  XMlHttp.onreadystatechange = function(){
...
function updatePage2(ids) {
  ...
  if (XMlHttp.readyState == 4) {
	...
	document.GetElement("company_bh_"+ids).value=document.GetElement("company_khid_"+ids).value;
	document.GetElement("gate_id_"+ids).value=document.GetElement("company_userid_"+ids).value;
	document.GetElement("gate_name_"+ids).value=document.GetElement("company_username_"+ids).value;
	}
	//updatePage3();
  }
}
...
function callServer8(ord,top) {
	...
	var url = "balance_ajax.asp?ord="+escape(ord)+"&top="+escape(top) + "&stamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
	plist.add(url,null);
	resetid();
}
...
window.GetUserVal = function(inputId,val,username){
	$("#"+inputId.replace("_name_","_id_")).val(val);
	$("#"+inputId+"").val(username);
	if (inputId.indexOf("_all")>0){
		gateall();
	}
}
    ...
    for (var i = 1; i < 1000; i++) {
        ...
        if (document.getElementById("moneyall_" + i)) {
			...
			//alert(parseFloat(document.getElementById("moneyall_" + i).value));
			    document.getElementById("moneysum_all").value = FormatNumber(allmoneys + parseFloat(document.getElementById("moneyall_" + i).value), window.sysConfig.moneynumber);
			}
        }
    }
    xmlHttp.abort();
