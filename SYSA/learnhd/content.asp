...
'Private Sub Class_Initialize
	...
	Function BwsBarHTML(mType,billID,UserID)
		Dim isAdmin, rsB, isCreator, sqlR, sqlC, RNum, CNum, urlCode
		urlCode = "T=" & mType & "&B=" & pwurl(billID) & "&U=" & pwurl(UserID)
		isAdmin = False
		Set rsB = conn.execute ("select isnull(qx_open,0) qx_open from power where sort1 = 66 and sort2 = 12 and ord = " & UserID & "")
		If rsB.eof = False And rsB.bof = False Then
			If rsB(0) = 1 Then
				isAdmin = True
			end if
		end if
		...
		isCreator = False
		Select Case mType
		Case "learntz","learnhd":
		Set rsB = conn.execute ("select 1 from " & mType & " where ord = " & billID & " and cateid = " & UserID & "")
		If rsB.eof = False And rsB.bof = False Then
			isCreator = True
		end if
		rsB.close
		Set rsB = Nothing
		sqlR = " select count(distinct ord) as RNum from Browse_Records where sort1 = '" & mType & "' and sorceID = " & billID & " "
		sqlC = " select count(id) as CNum from Browse_Records where sort1 = '" & mType & "' and sorceID = " & billID & " "
		Case Else
		sqlR = " select 0 as RNum "
		sqlC = " select 0 as CNum "
		End Select
		RNum = 0
		CNum = 0
		Set rsB = conn.execute (sqlR)
		If rsB.eof = False And rsB.bof = False Then
			RNum = rsB(0)
		end if
		rsB.close
		Set rsB = Nothing
		Set rsB = conn.execute (sqlC)
		If rsB.eof = False And rsB.bof = False Then
			CNum = rsB(0)
		end if
		rsB.close
		Set rsB = Nothing
		If isAdmin Or isCreator Then
			RNum = "<a href=""javascript:void(0)"" onclick=""javascript:__WinOpen({code:'" & urlCode & "',T:'R'})"">" & RNum & "</a>"
			CNum = "<a href=""javascript:void(0)"" onclick=""javascript:__WinOpen({code:'" & urlCode & "',T:'C'})"">" & CNum & "</a>"
		end if
		BwsBarHTML = "浏览人数：" & RNum & "&nbsp;&nbsp;浏览次数：" & CNum
	end function
	Function AddBrowseRecords(mType,billID,UserID)
		conn.execute("insert into Browse_Records (ord,readDate,sorceID,sort1) values (" & UserID & ",'" & now() & "'," & billID & ",'" & mType & "')")
	end function
	
	dim ord,Products,ProductList
	ord=deurl(request("ord"))
	Dim helper
	Set helper = CreateReminderHelper(conn,8,0)
	helper.cancelRemind(helper.getRemindIdByOID(ord))
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from learnhd  where ord="&ord&""
	rs.open sql,conn,1,1
	if rs.eof then
		Response.write "<script>alert('记录不存在或者已删除');window.close();</script>"
		call db_close : Response.end()
	end if
	Products =session("personzbintel2007")
	ProductList =rs("alt")
	...
	sqltext="Update learnhd set alt='"&ProductList&"' where ord="&ord&""
	conn.execute(sqltext)
	dim share
	share=rs("share")
	cateid=rs("cateid")
	if cateid="" or isnull(cateid) then cateid=0
	strSql="select count(*) from learnhdview where cateid="&session("personzbintel2007")&" and hdid="&ord
	intCount=conn.execute(strSql)(0)
	if intCount=0 then
		strSql="insert into learnhdview(cateid,hdid) values("&session("personzbintel2007")&","&ord&")"
		conn.execute(strSql)
	end if
	Call AddBrowseRecords("learnhd",ord,Products)
	Response.write "" & vbcrlf & "" & vbcrlf & "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN"">" & vbcrlf & "<html>" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>工作互动</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """>" & vbcrlf & "<script type=""text/JavaScript"" src='../Manufacture/inc/base.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script type=""text/JavaScript"" src=""../script/Brw_Rds.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<link rel=""stylesheet"" type=""text/css"" href=""../learntz/BrwStyle.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "  background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & ".style3 {font-size: 16pt}" & vbcrlf & ".style4 {font-size: 18pt}" & vbcrlf & "-->" & vbcrlf & "</style></head>" & vbcrlf & "<body  "
	'Response.write Application("sys.info.jsver")
	if open_73_8=0 or (open_73_8=1 and CheckPurview(intro_73_8,trim(cateid))=False) then
	...
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" height=""27"">" & vbcrlf & "              <tr>" & vbcrlf & "                <td width=""5%"" valign=""center"" height=""27""  style=""padding:0;height:42px;"" background=""../images/contentbg.gif""><div align=""center""><img src=""../images/contenttop.gif""height=""27""> </div></td> " & vbcrlf &          "       <td width=""47%""  background=""../images/contentbg.gif"" style=""height:42px; line-height:42px;""> "& vbcrlf &        "           <strong><font color=""#1445A6"">工作互动</font></strong> </td>" & vbcrlf & "                <td width=""48%""  background=""../images/contentbg.gif""><div align=""right"">" & vbcrlf & "                  <div id=""kh"" class=""hei_42"" style=""height:42px; line-height:42px;"">" & vbcrlf & "                             "
	Response.write "" & vbcrlf & "<script language='javascript'>window.close();</script>" & vbcrlf & ""
	Call ShowPrevOrNext("learnhd","ord",ord,1)
	if open_73_7=3 or CheckPurview(intro_73_7,trim(cateid))=True then
	...
	Response.write "<div>" & vbcrlf & "                </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>  " & vbcrlf & "<table class=""ewebeditorImg"" width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td height=""30"" colspan=""2""><div align=""center"" ><strong>"
	Response.write rs("title")
	Response.write "</strong></div></td>" & vbcrlf & "        </tr>" & vbcrlf & ""
	...
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""30"" colspan=""2""><div align=""center"">发布人："
	Response.write cateidname
	Response.write "&nbsp;&nbsp;发布时间："
	Response.write rs("date7")
	Response.write "&nbsp;&nbsp;"
	Response.write BwsBarHTML("learnhd",ord,Products)
	Response.write "</div></td>" & vbcrlf & "        </tr>" & vbcrlf & "             <tr >" & vbcrlf & "             <td  height=""30"" colspan=""2""  class=""gray"">&nbsp;"
	if share<>"" then
		Response.write "此信息已共享给："
		if share="0" then
			Response.write "所有用户"
		else
			set rs2=server.CreateObject("adodb.recordset")
			sql2="select * from gate order by cateid"
			rs2.open sql2,conn,1,1
			if rs2.RecordCount<=0 then
				Response.write "&nbsp;"
			else
				do until rs2.eof
					if CheckPurview(share,trim(rs2("ord")))=True  then
		...
		Response.write "此信息没有共享给任何人！"
	end if
	Response.write "</td>" & vbcrlf & "            </tr>" & vbcrlf & "               <tr >" & vbcrlf & "             <td  height=""30"" colspan=""2""><div align=""right""><span class=""blue2""><strong><a href=""#"" onClick=""javascript:window.open('reply.asp?ord="
	Response.write pwurl(rs("ord"))
	Response.write "','workreply','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')""><font class=""red"">【发表意见】</font></a></strong></span></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  class=""top"">" & vbcrlf & "       <td  height=""30"" colspan=""2"" >" & vbcrlf &    "      <div align=""left"" class=""blue2""><strong>详细信息</strong></div></td>" & vbcrlf &  "      </tr> "& vbcrlf &     "    <tr>"  & vbcrlf &   "        <td height=""30"" colspan=""2"" class=""ewebeditorImg"">"
	Response.write rs("intro")
	Response.write "</td>" & vbcrlf & "        </tr>" & vbcrlf & "<tr  class=""top"">" & vbcrlf & "       <td  height=""30"" colspan=""2""><div align=""left"" class=""blue2""><strong>互动交流内容</strong></div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  "
	dim k
	k=1
	sql = "select *,ISNULL((SELECT TOP 1 name FROM gate WHERE ord = replyhd.userID ORDER BY ord ASC),name) username from replyhd where ord="& rs("ord") &" order by date7 desc"
	set Rss = server.CreateObject("adodb.recordset")
	Rss.open sql,conn,1,1
	if rss.eof then
		Response.write "" & vbcrlf & "<tr >" & vbcrlf & "       <td height=""25"" colspan=""2"" ><div align=""center"">没有信息！</div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	else
		do until rss.eof
			Response.write "" & vbcrlf & "   <tr >" & vbcrlf & "       <td height=""25"" ><div align=""center"">"
			Response.write year(Rss("date7"))
			Response.write "-"
			Response.write year(Rss("date7"))
			Response.write month(Rss("date7"))
			Response.write "-"
			Response.write month(Rss("date7"))
			Response.write day(Rss("date7"))
			Response.write "</div></td>" & vbcrlf & "       <td>"
			Response.write Rss("intro")
			Response.write "(发言人："
			Response.write Rss("username")
			Response.write "&nbsp;&nbsp;"
			Response.write Rss("date7")
			Response.write ")" & vbcrlf & "         "
			sql3 = "select * from gate where ord="& CLng("0" & rss("userid").value) &""
			set Rss3 = server.CreateObject("adodb.recordset")
			Rss3.open sql3,conn,1,1
			...
			if rss("userid")=session("personzbintel2007") and rss("date7")>dateadd("h",-num_2,now) then
				num_3=0
				Response.write "<input type=""button"" name=""Submit42"" value=""修改""  onClick=""javascript:window.open('correctreply.asp?id="
				Response.write pwurl(rss("id"))
				Response.write "','newwin22','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150')""  class=""anybutton2""/>"
				Response.write pwurl(rss("id"))
			end if
			if rss("userid")=session("personzbintel2007") and rss("date7")>dateadd("h",-num_3,now) then
				Response.write pwurl(rss("id"))
				Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit2"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='delreply1.asp?id="
				Response.write pwurl(rss("id"))
				Response.write "&ord="
			...
			rss3.close
			set rss3=nothing
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit423"" value=""回复""  onClick=""javascript:window.open('huifu.asp?ord="
			Response.write pwurl(rss("id"))
			...
			Response.write pwurl(rss("ord"))
			if session("personzbintel2007")=cateid and rss("alt")=1  then
				Response.write "" & vbcrlf & "             <a href=""setalt2.asp?id="
				Response.write rss("id")
				Response.write "&ord="
				Response.write rss("ord")
				Response.write """ target=""_self"" onClick=""return confirm('确认取消提醒？')"" ><img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0""></a>" & vbcrlf & "            "
			end if
			sql7 = "select id,intro,name,cateid,date7,sort from huifu where ord="& rss("id") &"  order by date7 desc"
			set rs7 = server.CreateObject("adodb.recordset")
			rs7.open sql7,conn,1,1
			if rs7.eof then
			else
				do until rs7.eof
					Response.write "" & vbcrlf & "                     <br><font class=""red"">回复："
					...
					if rs7("cateid")=session("personzbintel2007") and rs7("date7")>dateadd("d",-1,now) then
						Response.write ")</font>" & vbcrlf & "                     "
						Response.write "<input type=""button"" name=""Submit42"" value=""修改""  onClick=""javascript:window.open('correcthuifu.asp?ord="
						Response.write pwurl(rs7("id"))
						Response.write "','newwin22','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150')""  class=""anybutton2""/>" & vbcrlf & "         <input type=""button"" name=""Submit2"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='delhuifu.asp?ord="
						Response.write pwurl(rs7("id"))
						Response.write pwurl(rs7("sort"))
						Response.write "&id="
						Response.write pwurl(rs7("id"))
						Response.write "'}"" class=""anybutton2""/>" & vbcrlf & "         "
					end if
		...
		loop
		if k>3 then
			Response.write "" & vbcrlf & "               <tr >" & vbcrlf & "       <td height=""25""><div align=""center""></div></td>" & vbcrlf & "       <td ><div align=""right""><a href=""#"" onClick=""javascript:window.open('all.asp?ord="
			Response.write pwurl(ord)
			Response.write "','newwins3d3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" title=""点击查看更多进展情况"" ><font class=""red"">查看更多意见..&gt;&gt;&gt;</font></a></div></td>" & vbcrlf & "   </tr>" & vbcrlf & "           "
			Response.write pwurl(ord)
		end if
	end if
	rss.close
	...
	action1="工作互动"
	call close_list(1)
	Response.write "" & vbcrlf & "</table>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "   </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
