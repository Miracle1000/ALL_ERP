...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		...
		app.addDefaultCss
		Dim rs,defDate,StatusList,jjList,ProcessList,sTypeList, khord, slord,contractId
		rpt.title = "维修单列表"
		If app.power.existsPower(46,10) Then
			rpt.canexcel = True
		else
			rpt.canexcel = False
		end if
		If app.power.existsPower(46,7) Then
			rpt.canPrint = True
		else
			rpt.canPrint = False
		end if
		khord = app.base64.deurl(request("company"))
		If khord&""="" Then khord = 0
		slord = app.base64.deurl(request("slord"))
		If slord&""="" Then slord = 0
		contractId = app.base64.deurl(request("contract"))
		If contractId&""="" Then contractId = 0
		Dim fromReminder
		fromReminder = request("fromReminder")
		If fromReminder = "" Then fromReminder = "0"
		Dim datatype, fromid, fromOrds
		fromOrds = ""
		datatype = request("datatype")
		fromid = request("fromid")
		If datatype&""<>"" And fromid&""<>"" Then
			fromid = app.base64.deurl(fromid)
			fromOrds = getRelevanceOrds("repair_deal", datatype, fromid)
		end if
		If khord>0 Or slord>0 Or contractId>0 Or fromReminder="1" Or (datatype&""<>"" And fromid&""<>"") Then
			defDate = Split(",", ",")
		else
			defDate = Split(year(now) & "-" & month(now) & "-1," & date, ",")
			'defDate = Split(",", ",")
		end if
		StatusList = StatusList & "未维修" & Chr(1) & 0 & Chr(2)
		StatusList = StatusList & "维修中" & Chr(1) & 1 & Chr(2)
		StatusList = StatusList & "维修完毕" & Chr(1) & 2
		Set rs = cn.Execute("SELECT id,sort1 FROM sortonehy WHERE gate2 = 46 ORDER BY gate1 DESC")
		While rs.Eof = False
			jjList = jjList & rs("sort1") & Chr(1) & rs("id")
			rs.movenext
			If Not rs.Eof Then
				jjList = jjList & Chr(2)
			end if
		...
		rs.close
		set rs = nothing
		Set rs = cn.Execute("SELECT Id,Title FROM Comm_ProcessSet WHERE Type = 1 ORDER BY Ranking DESC,AddTime DESC")
		While rs.Eof = False
			ProcessList = ProcessList & rs("Title") & Chr(1) & rs("Id")
			rs.movenext
			If Not rs.Eof Then
				ProcessList = ProcessList & Chr(2)
			end if
		wend
		sTypeList = sTypeList & "维修单主题" & Chr(1) & 1 & Chr(2)
		sTypeList = sTypeList & "维修单编号" & Chr(1) & 2 & Chr(2)
		sTypeList = sTypeList & "客户名称" & Chr(1) & 3 & Chr(2)
		sTypeList = sTypeList & "联系人" & Chr(1) & 4 & Chr(2)
		sTypeList = sTypeList & "故障描述" & Chr(1) & 6 & Chr(2)
		sTypeList = sTypeList & "当前处理人" & Chr(1) & 7 & Chr(2)
		sTypeList = sTypeList & "产品名称" & Chr(1) & 11 & Chr(2)
		sTypeList = sTypeList & "产品编号" & Chr(1) & 12 & Chr(2)
		sTypeList = sTypeList & "产品型号" & Chr(1) & 13
		' If ZBRuntime.MC(17002) then
		' 	sTypeList = sTypeList & Chr(2) & "批号" & Chr(1) & 30
		' 	sTypeList = sTypeList & Chr(2) & "序列号" & Chr(1) & 31
		' end if
		rpt.AddField "时间：","dates","AddTime","","",defDate
		rpt.AddField2 "","select","Status",StatusList,"","","维修状态=-1"
		'rpt.AddField "时间：","dates","AddTime","","",defDate
		rpt.AddField "","select","jinji",jjList,"",""
		rpt.SetNullMsg "2","紧急程度"
		rpt.AddField "","select","ProcessID",ProcessList,"",""
		rpt.SetNullMsg "3","维修流程"
		rpt.AddField2 "","select","sType",sTypeList,"","","null"
		rpt.AddField "","text","keywords",,"",""
		rpt.addsort "维修单主题", "1"
		rpt.addsort "维修单编号", "2"
		rpt.addsort "紧急程度", "3"
		rpt.addsort "维修状态", "4"
		...
		If app.power.existsPower(45,6) Then
			rpt.beforeBtnsHTML = "<input type='button' value='维修单添加' class='oldbutton3' onclick=""javascript:window.open('RepairOrder.asp?Referrer=List','RepairOrder','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" />"
'If app.power.existsPower(45,6) Then  '
		end if
		rpt.showtoppagesize = False
		rpt.PageSize = 10
		rpt.sql = "EXEC Erp_Report_RepairOrderList "&Info.User&",&ReportSort,@AddTime_0,@AddTime_1,@Status,@jinji,@ProcessID,@sType,@keywords,&excelmode,"& khord &","& slord & "," & contractId & ",&pagesize, &pageindex, @proUnitID," & fromReminder &",'"& fromOrds &"'"
		rpt.BillListUIModel = True
	end sub
	sub lvw_onCreateCheckBox(lvw, rs, html)
		If Not app.power.existsPowerIntro(46,3,rs("AddUser")) Then
			html = ""
		end if
	end sub
	Sub app_oncreate(lvw)
		Dim col,batchDel,beginCol,totalCol,i
		lvw.checkbox = True
		lvw.checkvalue = "Id"
		lvw.CanSort = False
		lvw.excelcallbackproc = "App_OnListviewExcel"
		lvw.addJoinFeilds "维修单主题,维修单编号"
		lvw.addJoinFeilds "关联客户,关联联系人"
		lvw.CurrSum = True
		lvw.AllSum = True
		For i = 1 To 10
			Set col = lvw.headers(i)
			col.canSum = False
		next
		...
		col.dbtype = "str"
		col.FormatText = "code:setTitle(""@cells[2]"",""@cells[13]"",""@value"")"
		lvw.headers(3).dbtype = "str"
		Set col = lvw.headers(4)
		col.dbtype = "str"
		col.setlink2 "关联客户", "customID" , "customeCateID", 1, -9, "customShare"
		col.dbtype = "str"
		Set col = lvw.headers(5)
		col.dbtype = "str"
		col.setlink  "personList", "personID", "personCateID" , 2, -10
		col.dbtype = "str"
		Set col = lvw.headers(6)
		col.dbtype = "str"
		col.setlink "关联产品","proID", "proCateID" ,  21, -6
		col.dbtype = "str"
		lvw.headers(7).dbtype = "number"
		Set col = lvw.headers(7)
		col.canSum = True
		col.FormatText = "code:FormatNum(""@value"")"
		Set col = lvw.headers(9)
		col.canSum = false
		col.FormatText = "code:setFlowChart(@cells[2],@cells[22],""@value"")"
		beginCol = 12
		totalCol = lvw.headers.count
		For i = beginCol To totalCol
			Set col = lvw.headers(i)
			col.width = 0
			col.display = "none"
		next
		Set col = lvw.headers(10)
		col.FormatText = "code:GetDealPerson(@cells[2],@cells[22],""@value"")"
		Set col = lvw.headers(11)
		col.width = 250
		col.formattext = "code:GetOptionCol(@cells[2],@cells[13],@cells[22],@cells[24],@cells[23])"
		If app.power.existsPower(46,3) Then
			batchDel = "&nbsp;<button class='oldbutton3' id='batchDel'>批量删除</button>"
		end if
		lvw.addlink = "<a></a><label for='selectAll'><input id='selectAll' type='checkbox' /> 全选</label> "&batchDel
	end sub
	Sub App_OnListviewExcel(ByRef lvw)
		dim i, col, coltitle, rs, rs_zdy, rs_slzdy, rs2, rs3, rs4, NodeID, TName, YTName, NodeName, CFID, YCFID, ProcessSet, ProcessName
		set rs = conn.execute("select id,title,name,sort,sorce,set_open from zdymx where sort1=91 order by sorce asc")
		if not rs.eof then
			rs_zdy = rs.GetRows()
		end if
		rs.close
		set rs = nothing
		set rs = conn.execute("select id,title,name,sort,sorce,set_open from zdymx where sort1=45 order by sorce asc")
		if not rs.eof then
			rs_slzdy = rs.GetRows()
		end if
		rs.close
		...
		For i = 1 To lvw.headers.count
			coltitle = ""
			Set col = lvw.headers(i)
			Select Case col.dbname
			Case "维修单主题" :
			coltitle = "维修单信息_维修单主题":col.dbtype = "str"
			col.Formula = "PowerCol(@value,@cells[""proID""],@cells[""rpAddUser""],46)"
			Case "维修单编号" : coltitle = "维修单信息_维修单编号":col.dbtype = "str"
			Case "维修状态" : coltitle = "维修单信息_维修状态":col.dbtype = "str":col.width=60:col.align="center"
			Case "维修流程" : coltitle = "维修单信息_维修流程":col.dbtype = "str":col.width=60
			Case "当前处理人员" :
			coltitle = "维修单信息_当前处理人":col.dbtype = "str":col.width=80:col.align2="center"
			col.Formula = "GetDealPerson(@cells[""rpID""],@cells[""ProcessID""],@value)"
			Case "所有处理人" : coltitle = "维修单信息_所有处理人":col.dbtype = "str":col.width=100
			Case "所有质检人" : coltitle = "维修单信息_所有质检人":col.dbtype = "str":col.width=100
			Case "所有协作人" : coltitle = "维修单信息_所有协作人":col.dbtype = "str":col.width=100
			Case "NodeName" : coltitle = "维修记录_流程节点":col.dbtype = "str":col.width=80:col.align2="center"
			Case "DealDate" : coltitle = "维修记录_处理日期":col.width=80
			Case "DealRemark" : coltitle = "维修记录_处理内容":col.dbtype = "str":col.width=100
			Case "DealName" : coltitle = "维修记录_处理人":col.dbtype = "str":col.width=60:col.align2="center"
			Case "DaiDealName" : coltitle = "维修记录_待处理人":col.dbtype = "str":col.width=60:col.align2="center"
			Case "DealTime" : coltitle = "维修记录_处理时间":col.width=110
			Case "ActorsCateName" : coltitle = "维修记录_协作人":col.dbtype = "str":col.width=100
			Case "PJproName" : coltitle = "新增配件信息_"& rs_zdy(1,0):col.dbtype = "str":col.width=100
			if rs_zdy(5,0)=0 then col.display = "none"
			Case "PJproSN" : coltitle = "新增配件信息_"& rs_zdy(1,1):col.dbtype = "str":col.width=80
			if rs_zdy(5,1)=0 then col.display = "none"
			Case "PJproModel" : coltitle = "新增配件信息_"& rs_zdy(1,2):col.dbtype = "str":col.width=80
			if rs_zdy(5,2)=0 then col.display = "none"
			Case "PJproUnit" : coltitle = "新增配件信息_"& rs_zdy(1,3):col.dbtype = "str":col.width=60
			if rs_zdy(5,3)=0 then col.display = "none"
			Case "PJNum" : coltitle = "新增配件信息_"& rs_zdy(1,4):col.dbtype = "str":col.width=60:col.dbtype = "number"
			if rs_zdy(5,4)=0 then col.display = "none"
			Case "PJscNum" : coltitle = "新增配件信息_生成":col.dbtype = "str":col.width=60:col.dbtype = "number"
			if rs_zdy(5,4)=0 then col.display = "none"
			Case "PJRemark" : coltitle = "新增配件信息_"& rs_zdy(1,5):col.dbtype = "str":col.width=100
			if rs_zdy(5,5)=0 then col.display = "none"
			Case "PJUseDate" : coltitle = "新增配件信息_"& rs_zdy(1,6):col.dbtype = "str":col.width=80
			if rs_zdy(5,6)=0 then col.display = "none"
			Case "PJzdy1" : coltitle = "新增配件信息_"& rs_zdy(1,7):col.dbtype = "str":col.width=100
			if rs_zdy(5,7)=0 then col.display = "none"
			Case "PJzdy2" : coltitle = "新增配件信息_"& rs_zdy(1,8):col.dbtype = "str":col.width=100
			if rs_zdy(5,8)=0 then col.display = "none"
			Case "PJzdy3" : coltitle = "新增配件信息_"& rs_zdy(1,9):col.dbtype = "str":col.width=100
			if rs_zdy(5,9)=0 then col.display = "none"
			Case "PJzdy4" : coltitle = "新增配件信息_"& rs_zdy(1,10):col.dbtype = "str":col.width=100
			if rs_zdy(5,10)=0 then col.display = "none"
			Case "PJzdy5" : coltitle = "新增配件信息_"& rs_zdy(1,11):col.dbtype = "str":col.width=80
			if rs_zdy(5,11)=0 then col.display = "none"
			Case "PJzdy6" : coltitle = "新增配件信息_"& rs_zdy(1,12):col.dbtype = "str":col.width=80
			if rs_zdy(5,12)=0 then col.display = "none"
			Case "PJProID","PJbaoxiu","repair_sl","repair_sl_list","ProID","del" : col.display="none"
			Case "关联客户" :
			coltitle = "客户信息_关联客户":col.dbtype = "str"
			col.Formula = "PowerCol(@value,@cells[""customID""],@cells[""customeCateID""],1)"
			Case "地址" : coltitle = "客户信息_地址":col.dbtype = "str"
			Case "关联联系人" :
			coltitle = "客户信息_关联联系人":col.dbtype = "str"
			col.Formula = "PowerCol(@value,@cells[""personID""],@cells[""personCateID""],2)"
			Case "固定电话" : coltitle = "客户信息_固定电话":col.dbtype = "str"
			Case "手机" : coltitle = "客户信息_手机":col.dbtype = "str"
			Case "维修概要" : coltitle = "维修概要":col.dbtype = "str"
			Case "产品名称" :
			coltitle = "维修产品信息_产品名称":col.dbtype = "str"
			col.Formula = "PowerCol(@value,@cells[""proID""],@cells[""proCateID""],21)"
			Case "产品编号" : coltitle = "维修产品信息_产品编号":col.dbtype = "str"
			Case "产品型号" : coltitle = "维修产品信息_产品型号":col.dbtype = "str"
			Case "单位" : coltitle = "维修产品信息_单位":col.dbtype = "str"
			Case "维修数量" : coltitle = "维修产品信息_维修数量":col.dbtype = "number"
			Case "维修费" : coltitle = "维修产品信息_维修费":col.dbtype = "money"
			Case "故障描述" : coltitle = "维修产品信息_故障描述":col.dbtype = "str"
			Case "备注" : coltitle = "维修产品信息_备注":col.dbtype = "str"
			Case "交付日期" : coltitle = "维修产品信息_交付日期":col.dbtype = "str"
			Case "购买日期" : coltitle = "维修产品信息_购买日期":col.dbtype = "str"
			Case "批号" : coltitle = "维修产品信息_批号":col.dbtype = "str"
			Case "序列号" : coltitle = "维修产品信息_序列号":col.dbtype = "str"
			Case "生产日期" : coltitle = "维修产品信息_生产日期":col.dbtype = "str"
			Case "有效日期" : coltitle = "维修产品信息_有效日期":col.dbtype = "str"
			Case "自定义1" : coltitle = "维修产品信息_"& rs_slzdy(1,18):col.dbtype = "str"
			if rs_slzdy(5,18)=0 then col.display = "none"
			Case "自定义2" : coltitle = "维修产品信息_"& rs_slzdy(1,19):col.dbtype = "str"
			if rs_slzdy(5,19)=0 then col.display = "none"
			Case "自定义3" : coltitle = "维修产品信息_"& rs_slzdy(1,20):col.dbtype = "str"
			if rs_slzdy(5,20)=0 then col.display = "none"
			Case "自定义4" : coltitle = "维修产品信息_"& rs_slzdy(1,21):col.dbtype = "str"
			if rs_slzdy(5,21)=0 then col.display = "none"
			Case "自定义5" : coltitle = "维修产品信息_"& rs_slzdy(1,22):col.dbtype = "str"
			if rs_slzdy(5,22)=0 then col.display = "none"
			Case "自定义6" : coltitle = "维修产品信息_"& rs_slzdy(1,23):col.dbtype = "str"
			if rs_slzdy(5,23)=0 then col.display = "none"
			Case "添加人员" : coltitle = "添加人员":col.dbtype = "str"
			Case "添加时间" : coltitle = "添加时间":col.dbtype = "str"
			Case "WxDealId","rpID","rpAddUser","customID","customeCateID","personID","personCateID","proID","proCateID","ProcessID" : col.display="none"
			End Select
			If Len(coltitle)>0 Then col.title = coltitle
		next
		set rs = conn.execute("select c.ProcessName,a.NodeName,b.FName,b.CFID,b.TName "&_
		"from "&_
		"( "&_
		"  SELECT DISTINCT Id, title as NodeName,ProcessSet "&_
		"  FROM Copy_ProcessNodeSet "&_
		"  where del=1 "&_
		")a        "&_
		"inner join ( "&_
		"  select DISTINCT ID AS CFID,FName,TName "&_
		"  from Copy_CustomFields WITH(NOLOCK) "&_
		"  where isUsing=1 and del=1 "&_
		")b on b.TName=a.Id+300000 "&_
		"left join ( "&_
		"  select DISTINCT title as ProcessName,id "&_
		"  from Copy_ProcessSet WITH(NOLOCK) "&_
		")c on c.Id=a.ProcessSet "&_
		"ORDER BY A.Id ")
		While not rs.eof
			ProcessName = rs("ProcessName") : NodeName = rs("NodeName") : FName = rs("FName") : CFID = rs("CFID") : TName = rs("TName")
			Set col = lvw.headers("extra_"& TName &"_"& CFID &"_"& FName)
			col.title = ProcessName &"—"& NodeName &"_"& FName
			col.dbtype = "str"
			col.formula = "GetExtraValue(rs(""rpID""),rs(""WxDealId""),"& TName &","& CFID &")"
			col.width = 100
			rs.movenext
		wend
		rs.close
	...
	Function setTitle(id , adduser , value)
		dim t,user
		if info.user&""="" then user=0 else user=info.user end if
		if adduser&""="" then adduser=0
		if not (conn.execute("select 1 from power where sort1=46 and sort2=14 and ord="&user&" and (qx_open='3' or CHARINDEX(',"&adduser&",' , ','+CAST(qx_intro AS VARCHAR(max))+',') > 0)")).eof then
'if adduser&""="" then adduser=0
			t = "<a href='javascript:void(0);' onclick='javascript:window.open(""RepairOrderContent.asp?ID="& id &""",""newwin2"",""width="" + 900 + "",height="" + 500 + "",fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150"")'>"& value &"</a>"
'if adduser&""="" then adduser=0
		else
			t=value
		end if
	...
	Function GetExtraValue(OrderID , DealId, TName, fieldsID)
		Dim s : s= ""
		Dim rs, rs2
		Set rs2 = conn.execute("select id from Copy_CustomFields where TName="&TName&" AND RepairOrder = "& OrderID &" and IsUsing=1 and del=1 order by FOrder asc ")
		If rs2.eof = False Then
			if DealId&""<>"0" then 'rs2("id")&"" = fieldsID&"" and
			Set rs = cn.execute("select isNull(FValue,'') from ERP_CustomValues WITH(NOLOCK) where fieldsID=" & fieldsID & " and OrderID=" & DealId)
			If rs.eof = False Then s = rs(0).value
			rs.close
			set rs = nothing
		end if
	end if
	...
	Function setFlowChart(id , pid , value)
		setFlowChart = "<a href='javascript:void(0);' onclick='javascript:window.open(""CopyFlowChart.asp?repID="& ID &"&PID="& pid &""",""newwin2"",""width="" + 900 + "",height="" + 500 + "",fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150"")'>"& value &"</a>"
'Function setFlowChart(id , pid , value)
	end function
	Function PowerCol(text,ID,User,qx)
		...
		If User&"" = "" Then User = 0
		If qx = 1 Then
			Set rs = cn.Execute("SELECT TOP 1 ISNULL(share,'-222') share FROM tel WITH(NOLOCK) WHERE ord = "&ID&" ")
'If qx = 1 Then
			If Not rs.Eof Then
				share = rs("share")
	...
	Function GetOptionCol(repID,AddUser,PID,repSl_list,Status)
		Dim str
		str = "<div class='option-col'>"
'Dim str
		If app.power.existsPowerIntro(46,14,AddUser) Then
			str = str&"<button class='oldbutton2'  onclick=""javascript:window.open('RepairOrderContent.asp?ID="&repID&"','newwin22','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" />详情</button>"
'If app.power.existsPowerIntro(46,14,AddUser) Then   '
		end if
		If app.power.existsPowerIntro(46,2,AddUser) And Status < 2 Then
			str = str&"<button class='oldbutton2'  onclick=""javascript:window.open('RepairOrder.asp?Referrer=List&ID="&repID&"','RepairOrder','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" />修改</button>"
'If app.power.existsPowerIntro(46,2,AddUser) And Status < 2 Then             '
		end if
		If app.power.existsPowerIntro(5,13,AddUser) And ZBRuntime.MC(7000) Then
			str = str&"<button class='oldbutton2'  onclick=""window.open('../../SYSN/view/sales/contract/Contract.ashx?fromId="&repID&"&FromType=14001','newwinRepair','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" />合同</button>"
'If app.power.existsPowerIntro(5,13,AddUser) And ZBRuntime.MC(7000) Then             '
		end if
		If app.power.existsPowerIntro(46,13,AddUser) And Status < 2 Then
			str = str & GetCurrentNodeList(repID,PID,repSl_list)
		end if
		If app.power.existsPowerIntro(46,3,AddUser) Then
			str = str&"<button class='oldbutton2 singelDel-btn' repID="&repID&" />删除</button>"
'If app.power.existsPowerIntro(46,3,AddUser) Then            '
		end if
		str = str&"</div>"
		GetOptionCol = str
	end function
	Function GetDealPerson(repID,PID,DealPerson)
		Dim rs,sql,hasData,str,NodeID,NodeName,dName,boxHTML
		If repID&"" = "" Then repID = 0
		if PID&"" <> "" then
			sql = "SELECT TOP 1 ID FROM RepairDeal WHERE del = 1 AND RepairOrder = "&repID&" AND ProcessID = "&PID&" "
			Set rs = cn.Execute(sql)
			If Not rs.Eof Then
				hasData = True
			else
			...
			Else
				Set rs = server.CreateObject("adodb.recordset")
				sql = "SELECT c.name pName FROM RepairDeal a " &_
				"LEFT JOIN Copy_ProcessNodeSet b ON b.ID = a.NodeID AND b.del = 1 " &_
				"LEFT JOIN gate c ON c.ord = a.DealPerson " &_
				"WHERE a.del = 1 AND a.CurrentStatus = 0 AND a.RepairOrder = "&repID&" AND a.ProcessID = "&PID&" " &_
	...
	Function GetCurrentNodeList(repID,PID,repSl_list)
		Dim rs,sql,hasData,DealPerson,str,NodeID,NodeName,dName,boxHTML,BeforeWhere,listNum,singleButton ,DealID
		sql = "SELECT TOP 1 ID FROM RepairDeal WHERE del = 1 AND RepairOrder = "&repID&" AND ProcessID = "&PID&" "
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
			hasData = True
		...
		set rs = nothing
		If Not hasData Then
			sql = "SELECT TOP 1 ID,(SELECT TOP 1 DealPerson FROM RepairOrder WHERE ID = "&repID&" ) DealPerson FROM Copy_ProcessNodeSet " &_
			"WHERE del = 1 AND ProcessSet = "&PID&" AND RepairOrder = "&repID&" " &_
			...
			If Not rs.Eof Then
				...
				If DealPerson = CInt(info.user) Then
					str = str&"<button class='oldbutton2' onclick=""javascript:window.open('RepairDeal.asp?repID="&repID&"&repSl_list="&repSl_list&"&PID="&PID&"&NID="&NodeID&"','RepairDeal','width=' + 900 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=50')"" />处理</button>"
'If DealPerson = CInt(info.user) Then        '
				else
					str = ""
				end if
			...
			set rs = nothing
		Else
			sql = "SELECT distinct a.id,a.NodeID,b.Title,c.name dName,a.DealPerson,b.BeforeNodeType FROM RepairDeal a " &_
			"LEFT JOIN Copy_ProcessNodeSet b ON b.ID = a.NodeID AND b.del = 1 AND b.ProcessSet = "&PID&" AND b.RepairOrder = "&repID&" " &_
			"LEFT JOIN gate c ON c.ord = a.DealPerson " &_
			"WHERE a.del = 1 AND a.CurrentStatus = 0 AND a.RepairOrder = "&repID&" AND a.ProcessID = "&PID&" " &_
			"AND CHARINDEX(',"&info.user&",' , ','+CAST(a.DealPerson AS VARCHAR(8000))+',') > 0 "
'WHERE a.del = 1 AND a.CurrentStatus = 0 AND a.RepairOrder = &repID& AND a.ProcessID = &PID&  &_
			Set rs = cn.Execute(sql)
			If Not rs.Eof Then
				boxHTML = "<div class='box'>"
				boxHTML = boxHTML &"<table width='100%' border='0' cellspacing='0' cellpadding='0'>"
				boxHTML = boxHTML &"<tr>"
				boxHTML = boxHTML &"       <th width='50%'>节点名称</th><th width='30%'>处理人员</th><th>操作</th>"
				boxHTML = boxHTML &"</tr>"
				listNum = 0
				Do While rs.Eof = False
					NodeID              = rs("NodeID")
					NodeName    = rs("Title")
					dName               = rs("dName")
					BeforeWhere   = rs("BeforeNodeType")
					DealID      = rs("id")
					If IsFinish(repID,PID,NodeID,BeforeWhere) Then
						boxHTML = boxHTML &"<tr onMouseOut=""this.style.backgroundColor=''"" onMouseOver=""this.style.backgroundColor='efefef'"">"
						boxHTML = boxHTML &" <td>"&NodeName&"</td><td>"&dName&"</td>"
						boxHTML = boxHTML &"<td><input type='button' value='处理' class='oldbutton2 boxClose' onclick=""javascript:window.open('RepairDeal.asp?DealID="&DealID&"&repID="&repID&"&repSl_list="&repSl_list&"&PID="&PID&"&NID="&NodeID&"','RepairDeal','width=' + 900 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=50')"" /></td>"
						boxHTML = boxHTML &"</tr>"
						listNum = listNum + 1
						boxHTML = boxHTML &"</tr>"
						singleButton = "<input type='button' value='处理' class='oldbutton2 boxClose' onclick=""javascript:window.open('RepairDeal.asp?DealID="&DealID&"&repID="&repID&"&repSl_list="&repSl_list&"&PID="&PID&"&NID="&NodeID&"','RepairDeal','width=' + 900 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=50')"" />"
					end if
					rs.movenext
				Loop
				boxHTML = boxHTML &"<tr> "
				boxHTML = boxHTML &"<td colspan='3' align='center'><input type='button' name='Submit422' value='关闭' class='oldbutton2 boxClose'/></td> "
				boxHTML = boxHTML &"</tr> "
				boxHTML = boxHTML &"</table>"
				boxHTML = boxHTML &"</div>"
				str = str&boxHTML
				If listNum = 1 Then
					str = str & singleButton & ""
				ElseIf listNum > 1 Then
					str = str & "<input type='button' value='处理' class='oldbutton2' name='dealBtn' />"
				end if
			else
				str = ""
			end if
		...
		GetCurrentNodeList = str
	end function
	Function FormatNum(num)
		FormatNum = FormatNumber(num,info.FloatNumber,-1)
'Function FormatNum(num)
	end function
	Function IsFinish(repID,PID,NID,BeforeWhere)
		Dim rs,sql,IsDeal,DealStatus,IsMust,rs1,result,curBeforeWhere,curID
		...
		"WHERE a.del = 1 AND a.RepairOrder = "&repID&" AND a.ProcessSet = "&PID&" AND a.NextNodeID = "&NID&" order by a.NodeID "
		Set rs = cn.Execute(sql)
		If Not rs.Eof Then
			While rs.Eof = False
				curID = rs("NodeID")
				...
				Set rs1 = cn.Execute("SELECT CurrentStatus FROM RepairDeal WHERE del = 1 AND RepairOrder = "&repID&" AND ProcessID = "&PID&" AND NodeID = "&curID&" ")
				If Not rs1.Eof Then
					IsDeal       = True
