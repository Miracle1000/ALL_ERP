...
'If Quali_rs.eof Then
	...
	caigouType = request("fromType")
	afv_fromType = request("afv_fromType")
	if afv_fromType <>"" then
		fromType="caigou_add"
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=26 and sort2=13"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,sort1,sort2 from power  where ord="&session("personzbintel2007")&" and sort1=1 and sort2=13"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_1_13=0
	else
	...
	Call GetPowerValue(open_1_1, intro_1_1,  1, 1)
	if open_1_1=3 then
		listkh=""
	elseif open_1_1=1 then
		listkh=" and (cateid in ("&intro_1_1&") or share='1') "
	else
		listkh=" and cateid=0"
	end if
	A=request("A")
	Dim zzSql_gys , zzSql_kh
	if A="1" Then
		Str_Result="where ((del=1 and ((sort3=1 "&listkh&") or (sort3=2 "&list&")))  or  ((share like '%"&session("personzbintel2007")&"%' or share='1') and del=1)) and isnull(sp,0)=0"
		Str_Result2="and ((del=1 and ((sort3=1 "&listkh&") or (sort3=2 "&list&"))) or  ((share like '%"&session("personzbintel2007")&"%' or share='1') and del=1)) and isnull(sp,0)=0"
	else
		Str_Result="where sort3=2 and ((del=1 "&list&")  or  ((share like '%"&session("personzbintel2007")&"%' or share='1') and del=1)) and isnull(sp,0)=0"
		Str_Result2="and sort3=2  and ((del=1  "&list&") or  ((share like '%"&session("personzbintel2007")&"%' or share='1') and del=1)) and isnull(sp,0)=0"
	end if
	dim company
	company=request("company")
	session("companyzbintelgys")=company
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "        background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & ".style17 {color: #FF8040}" & vbcrlf & "-->" & vbcrlf & "</style></head>" & vbcrlf & "<body  oncontextmenu=self.event.returnValue=false>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dim clo
	clo=request("clo")
	if clo<>"" then
		Response.write "" & vbcrlf & "<script language='javascript'>" & vbcrlf & "window.open('','_self');window.close();</script>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & ""
	...
	E=request("E")
	if Str_Result="" Then str_Result = " where 1=1 "
	If len(C)>0 Then
		Select Case B
		Case "mc" : Str_Result=Str_Result+"and  name like '%"& C &"%'"
'Select Case B
		Case "pym" : Str_Result=Str_Result+"and  pym like '%"& C &"%'"
'Select Case B
		Case "bh" : Str_Result=Str_Result+"and  khid like '%"& C &"%'"
'Select Case B
		Case "dh" : Str_Result=Str_Result+"and  phone like '%"& C &"%'"
'Select Case B
		Case "cz" : Str_Result=Str_Result+"and  fax like '%"& C &"%'"
'Select Case B
		Case "yj" : Str_Result=Str_Result+"and  email like '%"& C &"%'"
'Select Case B
		Case "dz" : Str_Result=Str_Result+"and  address like '%"& C &"%'"
'Select Case B
		Case "lxrname" : Str_Result=Str_Result+"and  exists(select 1 from person where del=1 and company=c.ord and name like '%"& C &"%') "
'Select Case B
		Case "lxrphone" : Str_Result=Str_Result+"and  exists(select 1 from person where del=1 and company=c.ord and phone like '%"& C &"%') "
'Select Case B
		Case "lxrmobile" : Str_Result=Str_Result+"and exists(select 1 from person where del=1 and company=c.ord and mobile like '%"& C &"%') "
'Select Case B
		Case "lxrmobile2" : Str_Result=Str_Result+"and  exists(select 1 from person where del=1 and company=c.ord and mobile2 like '%"& C &"%') "
'Select Case B
		Case "yb" : Str_Result=Str_Result+"and  zip like '%"& C &"%'"
'Select Case B
		Case "yw" : Str_Result=Str_Result+"and  product like '%"& C &"%'"
'Select Case B
		end Select
	end if
	cc1=request("cc1")
	id=request("id")
	Response.write "" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "            <td class=""place"">供应商检索</td>" & vbcrlf & "            <td><div align=""left""><table width=""98%"" border=""0"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "        <tr>" & vbcrlf & "      <form name=""form"" method=""get"" action=""result5.asp"" > " & vbcrlf & "        <td align=""center""  class=""top"">  " & vbcrlf & "          <div align=""right"">" & vbcrlf & "            <span class=""gray"">" & vbcrlf &"                 "
	list = ""
	list=list+"<option value='mc' "& iif(B="mc" , " selected " ,"") &">---供应商名称---</option>"
	list = ""
	list=list+"<option value='pym' "& iif(B="pym" , " selected " ,"") &">---拼音码---</option>"
	list = ""
	list=list+"<option value='bh' "& iif(B="bh" , " selected " ,"") &">---供应商编号---</option>"
	list = ""
	list=list+"<option value='dh' "& iif(B="dh" , " selected " ,"") &">---联系电话---</option>"
	list = ""
	list=list+"<option value='cz' "& iif(B="cz" , " selected " ,"") &">---联系传真---</option>"
	list = ""
	list=list+"<option value='yj' "& iif(B="yj" , " selected " ,"") &">---电子邮件---</option>"
	list = ""
	list=list+"<option value='dz' "& iif(B="dz" , " selected " ,"") &">---联系地址---</option>"
	list = ""
	list=list+"<option value='lxrname' "& iif(B="lxrname" , " selected " ,"") &">---联系人姓名---</option>"
	list = ""
	list=list+"<option value='lxrphone' "& iif(B="lxrphone" , " selected " ,"") &">---联系人电话---</option>"
	list = ""
	list=list+"<option value='lxrmobile' "& iif(B="lxrmobile" , " selected " ,"") &">---联系人手机---</option>"
	list = ""
	list=list+"<option value='lxrmobile2' "& iif(B="lxrmobile2" , " selected " ,"") &">---联系人手机2---</option>"
	list = ""
	list=list+"<option value='yb' "& iif(B="yb" , " selected " ,"") &">---邮政编码---</option>"
	list = ""
	list=list+"<option value='yw' "& iif(B="yw" , " selected " ,"") &">---供应商简介---</option>"
	list = ""
	Response.write "" & vbcrlf & "            <select name=""B"">"
	Response.write list
	Response.write "</select>" & vbcrlf & "            <input name=""C"" type=""text"" size=""12"" value="""
	Response.write C
	Response.write """/>" & vbcrlf & "            <input name=""from"" type=""hidden"" value="""
	Response.write fromType
	Response.write """ />" & vbcrlf & "            <input name=""fromType"" type=""hidden"" value="""
	Response.write caigouType
	Response.write """ />" & vbcrlf & "                      <input name=""cc1"" type=""hidden"" value="""
	Response.write cc1
	Response.write """ />" & vbcrlf & "                      <input name=""id"" type=""hidden"" value="""
	Response.write id
	Response.write """ />" & vbcrlf & "                      <input name=""dbname"" type=""hidden"" value="""
	Response.write request("dbname")
	Response.write """ />" & vbcrlf & "                      " & vbcrlf & "" & vbcrlf & "                "
	if ZBRuntime.MC(1001) Then
		If request("gyslxr")<>"1" Then
			Response.write "<input type=""checkbox"" name=""A"" value=""1"" "
			If A="1" Then
				Response.write "checked"
			end if
			Response.write "> 含客户"
	...
	Response.write "" & vbcrlf & "            <input type=""submit"" name=""Submit45"" value=""检索"" class=""anybutton""/>" & vbcrlf & "            "
	if open_26_13=1 or CheckPower2010(intro_26_13,trim(cateid))=True then
		Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit42"" value=""添加供应商""  onclick=""javascript:window.open('../work2/add_list.asp?ord="
		Response.write pwurl(89)
		Response.write "&cc1="
		Response.write cc1
		Response.write "&id="
		Response.write id
		Response.write "','new53win','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" class=""anybutton""/>" & vbcrlf & "                         "
		Response.write id
	end if
	Response.write "" & vbcrlf & "          </div></td>" & vbcrlf & "                  <td width=""43"">&nbsp;</td>" & vbcrlf & "      </form>" & vbcrlf & "    </tr>" & vbcrlf & "  </table></div></td>" & vbcrlf & "           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>  " & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "    <tr class=""top"">" & vbcrlf & "        <td align=""center"" width='12%''><div align=""center""><strong>供应商编号</strong></div></td>" & vbcrlf & "<td height=""27"" width=""14%"" class=""gray""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf &         "<td class=""gray"" width=""25%""><div align=""center""><strong>供应商名称</strong></div></td>" & vbcrlf &             "<td class=""gray"" width=""10%""><div align=""center""><strong>联系人姓名</strong></div></td>" & vbcrlf & "            <td class=""gray"" width=""12%""><div align=""center""><strong>供应商分类</strong></div></td>" & vbcrlf & "               <td class=""gray"" width=""19%""><div align=""center""><strong>供应商简介</strong></div></td>" & vbcrlf & "        <td width=""11%"" class=""gray""><div align=""center"">所有者</div></td>" & vbcrlf & "    </tr>" & vbcrlf & ""
	zzSql_kh = returnZzSql(105)
	If Len(zzSql_kh)>0 Then
		zzSql_kh = replace(zzSql_kh,"tel.ord","c.ord")
	end if
	zzSql_gys = returnZzSql(101)
	If Len(zzSql_gys)>0 Then
		zzSql_gys = replace(zzSql_gys,"tel.ord","c.ord")
	end if
	statusFields = "" & _
	"case when sort3=1 and not (1=1 " & zzSql_kh & ") then 0 " & vbcrlf &_
	"when sort3=2 and not (1=1 " & zzSql_gys & " ) then 0 " & vbcrlf &_
	"else 1 end canUse, " & vbcrlf &_
	"case when isnull(status_sp_qualifications,0)<>0 or exists(select top 1 1 from sortFieldsContent where ord = c.ord and del=1 and  DATEDIFF(d,date2,getdate()) > 0) then 0 " & vbcrlf &_
	"else 1 end zzStatus "
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,cateid,share,del,sort3,name,pym,phone,fax,email,address,zip,product,order1,khid,cateid4,(select top 1 name from person a where a.ord = c.person) as lxrname,(select top 1 sort1 from sortonehy a where a.ord=c.ly) as qysname,isnull(isNeedQuali,0) isNeedQuali," & statusFields & " from tel c "&Str_Result&"  order by ord desc,gate desc "
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			if Rs("ord")="" or isnull(Rs("ord")) then
				ord=0
			else
				ord=Rs("ord")
			end if
			dim cateid,cid,cid4
			cateid=""
			if rs("cateid")="" or isnull(rs("cateid")) then
				cid=0
			else
				cid=rs("cateid")
			end if
			if rs("cateid4")="" or isnull(rs("cateid4")) then
				cid4=0
			else
				cid4=rs("cateid4")
			end if
			...
			elseif rs("order1")="3" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select * from gate where ord="&cid4&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
				else
					cateid=rs7("name")
			...
			elseif rs("order1")="2" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select * from gate where ord="&cid&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
				else
					cateid=rs7("name")
			...
			If sort3="1" Then
				if open_1_14=3 or CheckPower2010(intro_1_14,rs("cateid"))=True Then
					Response.write "" & vbcrlf & "                                     <a href=""#"" onclick=""javascript:window.open('../work/content.asp?ord="
					Response.write pwurl(ord)
					Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此客户详情"">" & vbcrlf & "                                   "
					Response.write pwurl(ord)
					Response.write Rs("khid")
				else
					Response.write Rs("khid")
				end if
				Response.write "</a>"
			else
				if open_26_14=3 or CheckPower2010(intro_26_14,rs("cateid"))=True then
					Response.write "" & vbcrlf & "                                     <a href=""#"" onclick=""javascript:window.open('../work2/content.asp?ord="
					Response.write pwurl(ord)
					Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此供应商详情"">" & vbcrlf & "                                 "
					Response.write pwurl(ord)
					Response.write Rs("khid")
			...
			Response.write "</td>" & vbcrlf & "        <td height=""28""><div align=""center"">" & vbcrlf & ""
			If rs("canUse") = 0 and Rs("isNeedQuali")=1 And fromType = "caigou_add" Then
				Response.write "" & vbcrlf & "                             <span style=""color:lightgray"">点击选择此"
				Response.write iif(sort3=2,"供应商","客户")
				Response.write "</span>" & vbcrlf & ""
			ElseIf cc1&""="2" Then
				Response.write "" & vbcrlf & "                               <a  href=""result5.asp?company="
				Response.write rs("ord")
				Response.write "&clo=35&id="
				Response.write request("id")
				Response.write """ target=""_self"" onClick=""if(opener.check_kh) opener.check_kh2("
				Response.write rs("ord")
				Response.write ","
				Response.write request("id")
				Response.write ");return false;"">点击选择此"
				Response.write iif(sort3=2,"供应商","客户")
				Response.write "</a>" & vbcrlf & ""
			else
				telname = Replace(Replace(Replace(Replace(rs("name"),"\","\\"),"'","\'"),"""","&#34")," ","&nbsp;")
				Response.write "" & vbcrlf & "                               <a  href=""result5.asp?company="
				Response.write rs("ord")
				Response.write "&clo=35"" target=""_self"" onClick=""if(opener.OnFieldAutoCompleteCallBack){opener.OnFieldAutoCompleteCallBack({value:'"
				Response.write rs("ord")
				Response.write "',keys:{dbname:'"
				Response.write request.querystring("dbname")
				Response.write "'},text:'"
				Response.write telname
				Response.write "',texts:['"
				Response.write telname
				Response.write "'],tag:null})};if(opener.check_kh) opener.check_kh("
				Response.write rs("ord")
				Response.write ",'"
				Response.write fromType
				Response.write "','"
				Response.write caigouType
				Response.write "')"">点击选择此"
				Response.write iif(sort3=2,"供应商","客户")
				Response.write "</a>" & vbcrlf & ""
			end if
			Response.write "" & vbcrlf & "                       </div></td>           " & vbcrlf & "                </div></td>" & vbcrlf & "        <td><div align=""left"">" & vbcrlf & "                 "
			Response.write rs("name")
			Response.write "" & vbcrlf & "                     <span class='red'>"
			Response.write iif(rs("zzStatus")=1,"",iif(Rs("isNeedQuali")=1,"(资质未通过审核或过期" & iif(rs("canUse") = 0 And fromType="caigou_add",",不允许供货","") & ")",""))
			Response.write "</span>" & vbcrlf & "                      </div></td>" & vbcrlf & "             <td><div align=""center"">"
			Response.write rs("lxrname").value
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">"
			Response.write rs("qysname").value
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">"
			Response.write rs("product").value
			Response.write "</div></td>" & vbcrlf & "        <td><div align=""center"">"
			...
			rs.movenext
		loop
		Response.write " " & vbcrlf & "</table>" & vbcrlf & "    <table width=""80%"" border=""0"" align=""center"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td><div align=""right"">" & vbcrlf & "     <span class=""black"">"
		...
		Response.write "条信息/页</span>&nbsp;&nbsp;" & vbcrlf & "          <input id=""curp1""  maxlength=""8"" onpropertychange=""if(value.replace(/[^\d]/g,'') != value){value=value.replace(/[^\d]/g,'')}""  name=""currpage""  type=text   onkeyup=""value=value.replace(/[^\d\.]/g,'')""  size=3 />" & vbcrlf & "          <input type=""submit"" name=""Submit422"" value=""跳转"" onclick=""gotourl('currPage='+document.getElementById('curp1').value);""  class=""anybutton2""/>" & vbcrlf & "          "
		Response.write page_count
		if currpage=1 then
