...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		Dim rs, dat
		Dim sql, issuperAdmin
		dat = Split(",",",") 'Split(year(now) & "-" & month(now) & "-1," & date, ",")
'Dim sql, issuperAdmin
		rpt.title = "移动登录列表"
		rpt.headHTML = "<script>function delLoginLog(id){if(id!=""""){if(confirm(""确定删除?"")){ajax.regEvent(""delLoginLog"",""loginLog.asp"");ajax.addParam(""id"",id);var r=ajax.send();lvw_refresh(""mlistvw"");}}}</script>"
		rpt.addfield "登录时间 自：","dates","date7","","", dat
		rpt.AddField2 "&nbsp;","select","searchType", "手机型号" & Chr(1) & "2"& Chr(2) & "移动版本" & Chr(1) & "3" & Chr(2) & "手机串号" & Chr(1) & "4","","","姓名=1"
		...
		If issuperAdmin Then strHandle = "批量删除"
		If Len(strHandle)>0 Then rpt.setBatHandle "id", strHandle '"批量删除"
		sql = "exec erp_mobile_loginLog "& Info.User &",@date7_0 ,@date7_1 ,@searchType, @searchKey,&pagesize ,&pageindex, &ReportSort," & clng("0" & Application("sys.info.configindex"))
		rpt.sql = sql
	end sub
	Sub app_onCreateList(lvw,rpt)
		Dim h, i, col, issuperAdmin
		issuperAdmin = info.isAdmin
		lvw.indexbox = False
		lvw.checkbox = app.iif(issuperAdmin,True,False)
		lvw.colresize = False
		lvw.cansort=False
		lvw.HeaderPageSizeUI = False
		...
		col.title = "姓名"
		col.width = 80
		Set col = lvw.headers("MobileModel")
		col.title = "手机型号"
		col.width = 100
		Set col = lvw.headers("AppVersion")
		col.title = "移动版本"
		col.width = 100
		Set col = lvw.headers("macsn")
		col.title = "手机串号"
		col.width = 100
		...
		col.title = "操作"
		col.width = 80
		col.formattext = "code:getColHtml(@value,""操作"",@value)"
		If issuperAdmin = False Then
			col.display = "none"
		end if
	end sub
	Function getColHtml(mxid,colName,colValue)
		Dim tempStr, addcate, cateid_sp, tipStr
		tempStr = ""
		If colValue&"" = "" Then colValue=""
		Select Case colName
		Case "操作"
		tempStr = tempStr & "<button class=""oldbutton2"" onclick=""javascript:delLoginLog("& mxid &")"">删除</button>"
		End Select
		getColHtml = tempStr
	end function
	Sub App_delLoginLog
		Dim id, rs, addcate, noDel, id1
		id = request("id")
		if id&""="" then
			id = "0"
		else
...
'if right(id,1)="," then
		...
		If id&"" = "" Or id="0" Then
			...
			cn.execute("delete from Mob_UserLoginLog where id in("& id &") ")
			Response.write("1")
		end if
	end sub
	Sub App_doBatHandle(cmd, data, result)
		If cmd = "批量删除" Then
			If data&""<>"" Then
				...
				data2 = Replace(data&"",",","")
				If data2&""<>"" Then
					cn.execute("delete from Mob_UserLoginLog where id in("& data &") ")
				end if
			end if
		end if
	end sub
