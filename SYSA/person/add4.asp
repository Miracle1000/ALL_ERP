...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src=""../inc/popup_layer.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "window.yearDate = """
	Response.write year(date)
	Response.write """;" & vbcrlf & "window.monthDate = """
	Response.write month(date)
	Response.write """;" & vbcrlf & "window.dayDate = """
	Response.write day(date)
	Response.write """;" & vbcrlf & "</script>" & vbcrlf & "<script src= ""../Script/pn_add4.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=javascript></script>" & vbcrlf & "</head>" & vbcrlf & "" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & ""
	dim ord,h,id,ty
	id=deurl(request("id"))
	If id="0" Then
	...
	sql="select name from tel where ord="&ord
	clientrs.open sql,conn,1,1
	if clientrs.eof then
		clientname=""
	else
		clientname=clientrs("name")
	end if
	clientrs.close
	set clientrs=nothing
	set rscl=server.CreateObject("adodb.recordset")
	sql="select ts,jz,bt from celue  where  sort1=1 "
	rscl.open sql,conn,1,1
	if rscl.eof then
		ts_kh=""
		jz_kh=""
		bt_kh=""
	...
	set rscl=nothing
	set rscl=server.CreateObject("adodb.recordset")
	sql="select ts from celue  where  sort1=11 "
	rscl.open sql,conn,1,1
	if rscl.eof then
		gs_kh=""
	else
	...
	rscl.close
	set rscl=nothing
	Response.write "" & vbcrlf & "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf &           "<tr>" & vbcrlf &             "<td class=""place"">联系人添加</td>" & vbcrlf &             "<td>&nbsp;</td>" & vbcrlf &             "<td align=""right"">&nbsp;</td>" & vbcrlf &             "<td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <form method=""POST"" action=""save4.asp?company2id="
	Response.write Split(ord,",")(0)
	Response.write "&h="
	Response.write h
	Response.write "&ty="
	Response.write ty
	Response.write """ id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" target=""saveform"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr class=""top"">" & vbcrlf & "        <td height=""27""colspan=""6"" ><div align=""left""><strong>基本情况</strong></div></td>" & vbcrlf & "        </tr>" & vbcrlf & "      <tr>" & vbcrlf & "          <td width=""10%"" align=""right"">供应商名称：</td>" & vbcrlf & "         <td height=""33"" colspan=""3"" class=""gray"">" & vbcrlf & "               <div align=""left""><span id=""khmc"">" & vbcrlf &                          "<input id=""company"" name=""company"" type=""text"" size=""15"" readonly=""true"" maxlength=60  value="""
	Response.write clientname
	Response.write ">" & vbcrlf & "                             <input name=""company2id"" id=""company2id"" type=""hidden"" size=""15"" max=""50"" value="
	Response.write ord
	Response.write "></span>" & vbcrlf & "                <a href=""javascript:void(0)"" onclick=""javascript:window.open('../search/result5.asp?gyslxr=1','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;""><span class=""style1"">查找对应供应商</span></a>" & vbcrlf & "                  <span class=""red"" id=""gys"">*</span>" & vbcrlf & "              </div><input name=""source"" type=""hidden"" size=""15"" max=""50"" value=""2"">             </td>" & vbcrlf & "           <td rowspan=""3""><div align=""right"">照　　片：</div></td>" & vbcrlf & "            <td rowspan=""3""><img id=""imageField"" src=""../person/img/noneperson.jpg"" width=""90"" height=""100"" alt=""头像""><input type=""hidden"" id=""imgurl1"" name=""imgurl1"" value=""""></td>" & vbcrlf & "      </tr>" & vbcrlf & "      <tr>" & vbcrlf & "        <td><div align=""right"">姓　　名：</div></td>" & vbcrlf & "    <td width=""23%"" height=""33"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "            <input name=""name"" type=""text"" onKeyUp=""ff4(this.value,document.getElementById('pym'))"" size=""25"" id=""person1"" onChange=""callServer(3,'person1');"" "
	if  CheckPurview(bt_kh,"3")=True  then
		Response.write " min=""1"" "
	end if
	Response.write " dataType=""Limit"" max=""50""  msg=""必填"">"
	if CheckPurview(bt_kh,"3")=True  then
		Response.write "&nbsp;<span class=""red"">*</span>"
	end if
	Response.write "<span id=""test3"" class=""red""></span>&nbsp;&nbsp;<br>拼音码：<input name=""pym"" type=""text"" id=""pym"" readonly=""true"" size=""22"" style=""margin-top: 2px;""></div></td>" & vbcrlf & "        <td width=""11%""><div align=""right"">性　　别：</div></td>" & vbcrlf & "        <td width=""19%"" class=""gray"">" & vbcrlf & "              <div align=""left"">" & vbcrlf & "                <input name=""sex"" type=""radio"" value=""男"" checked>" & vbcrlf & "              男" & vbcrlf & "              <input type=""radio"" name=""sex"" value=""女"">" & vbcrlf & "            女</div></td>" & vbcrlf & "</tr>" & vbcrlf & "   <tr>" & vbcrlf & "          <td width=""10%""><div align=""right"">年　　龄：</div></td>" & vbcrlf & "        <td width=""27%"" class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "            <input name=""age"" type=""text"" size=""15"" onkeyup=""value=value.replace(/[^\d]/g,'')"" dataType=""Limit"" max=""3""  msg=""年龄不会超过3位数""  >" & vbcrlf & "          </div></td>" & vbcrlf & "            <td><div align=""right"">职　　务：</div></td>" & vbcrlf & "        <td class=""gray""><input name=""job"" type=""text"" size=""15"" dataType=""Limit"" "
	if  CheckPurview(bt_kh,"12")=True  then
		Response.write " min=""1"" "
	end if
	Response.write "  max=""50""  msg=""必填"">"
	if CheckPurview(bt_kh,"12")=True  then
		Response.write "&nbsp;<span class=""red"">*</span>"
	end if
	Response.write "</td>" & vbcrlf & "          </tr>" & vbcrlf & "       <tr>" & vbcrlf & "        <td><div align=""right"">生　　日：</div></td>" & vbcrlf & "        <td height=""33"" class=""gray"">" & vbcrlf & "              <select name=""birthdayType"" id=""birthdayType"" onChange=""getBirthYear()"">" & vbcrlf & "              <option value=""0"">类型</option>" & vbcrlf & "                  <option value=""1"">公历</option>" & vbcrlf & "                   <option value=""2"">农历</option>" & vbcrlf & "                   </select>" & vbcrlf & "               <select name=""birthYear"" id=""birthYear"" onChange=""getBirthMonth()"">" & vbcrlf & "                           "
	...
	next
	Response.write "" & vbcrlf & "                </select>" & vbcrlf & "               <select name=""birthMonth"" id=""birthMonth"" onChange=""getBirthDay()"">" & vbcrlf & "                                <option value="""">选择月</option>" & vbcrlf & "             </select>" & vbcrlf & "               <select name=""birthDay"" id=""birthDay"">" & vbcrlf & "                           <option value="""">选择日</option>" & vbcrlf & "               </select>" & vbcrlf & "        </td>" & vbcrlf & "        <td><div align=""right"">部　　门：</div></td>" & vbcrlf & "        <td class=""gray""><input name=""part1"" type=""text"" size=""15"" dataType=""Limit"" "
	...
	Response.write "</td>" & vbcrlf & "                <td><div align=""right"">上　　传：</div></td>" & vbcrlf & "        <td class=""gray""><div align=""left""><input type=""button"" name=""openbn"" id=""openbn"" value=""上传头像""  class=""anybutton"" onclick=""$('#w').window('open');""/></div></td>" & vbcrlf & "      </tr>" & vbcrlf & "          <tr>" & vbcrlf & "        <td><div align=""right"">办公电话：</div></td>" & vbcrlf & "        <td height=""33"" class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "             "
	if CheckPurview(bt_kh,"4")=True then
		if CheckPurview(gs_kh,"4")=True then
			str1=" dataType='Phone' msg='格式不正确' "
			str11="<span class='red'>*</span>例：01012345678或01012345678-6008"
			'str1=" dataType='Phone' msg='格式不正确' "
		else
			str1=" dataType='Limit' min=1 max=50 msg='必填' "
			str11="<span class='red'>*</span>"
		end if
	else
		if CheckPurview(gs_kh,"4")=True then
			str1=" dataType='PhoneNull' msg='格式不正确' "
			str11="例：01012345678或01012345678-6008"
			'str1=" dataType='PhoneNull' msg='格式不正确' "
		else
			str1="dataType='Limit' max=50  msg='长度不超过50个字'"
			str11=""
		end if
	end if
	Response.write "" & vbcrlf & "            <input name=""phone"" id=""phone1"" type=""text"" onChange=""callServer(4,'phone1');"" size=""15"" "
	Response.write str1
	Response.write ">" & vbcrlf & "                    "
	Response.write str11
	Response.write "<span id=""test4"" class=""red""></span>" & vbcrlf & "          "
	if  tp=1 then
		Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:hand"" onClick=""phoneCall(phone1.value)"">" & vbcrlf & ""
	end if
	Response.write "</div></td>" & vbcrlf & "        <td><div align=""right"">传　　真：</div></td>" & vbcrlf & "        <td class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "                 "
	if CheckPurview(bt_kh,"5")=True then
		if CheckPurview(gs_kh,"5")=True then
			str2=" dataType='Phone' msg='格式不正确' "
			str21="<span class='red'>*</span>例：01012345678或01012345678-6008"
			str2=" dataType='Phone' msg='格式不正确' "
		else
			str2=" dataType='Limit' min=1 max=50 msg='必填' "
			str21="<span class='red'>*</span>"
		end if
	else
		if CheckPurview(gs_kh,"5")=True then
			str2=" dataType='PhoneNull' msg='格式不正确' "
			str21="例：01012345678或01012345678-6008"
			str2=" dataType='PhoneNull' msg='格式不正确' "
		else
			str2="dataType='Limit' max=50  msg='长度不超过50个字'"
			str21=""
		end if
	end if
	Response.write "" & vbcrlf & "            <input name=""fax"" type=""text"" id=""fax"" onChange=""callServer(5,'fax');"" size=""15"" "
	Response.write str2
	Response.write ">" & vbcrlf & "                    "
	Response.write str21
	Response.write "<span id=""test5"" class=""red""></span>" & vbcrlf & "                                             "
	if  tp=1 then
		Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:hand"" onClick=""phoneCall(fax.value)"">" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "          </div></td>" & vbcrlf & "        <td><div align=""right"">手　　机：</div></td>" & vbcrlf & "        <td class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "               "
	if CheckPurview(bt_kh,"6")=True then
		if CheckPurview(gs_kh,"6")=True then
			str3=" dataType='Mobile' msg='格式不正确' "
			str31="<span class='red'>*</span>例：13999999999"
		else
			str3=" dataType='Limit' min=1 max=50 msg='必填' "
			str31="<span class='red'>*</span>"
		end if
	else
		if CheckPurview(gs_kh,"6")=True then
			str3=" dataType='MobileNull' msg='格式不正确' "
			str31="例：13999999999"
		else
			str3="dataType='Limit' max=50  msg='长度不超过50个字'"
			str31=""
		end if
	end if
	Response.write "" & vbcrlf & "            <input name=""mobile"" type=""text"" id=""mobile1"" size=""15"" onChange=""callServer(6,'mobile1');""  "
	Response.write str3
	Response.write ">" & vbcrlf & "                    "
	Response.write str31
	Response.write "<span id=""test6"" class=""red""></span>     "
	if  tp=1 then
		Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:hand"" onClick=""phoneCall(mobile1.value)"">" & vbcrlf & ""
	end if
	...
	Response.write "" & vbcrlf & "</div></td>" & vbcrlf & "      </tr>" & vbcrlf & "      <tr>" & vbcrlf & "        <td><div align=""right"">家庭电话：</div></td>" & vbcrlf & "       <td class=""gray"">" & vbcrlf & "        <div align=""left"">" & vbcrlf & "        "
	if CheckPurview(bt_kh,"19")=True then
		if CheckPurview(gs_kh,"16")=True then
			str4=" dataType='Phone' msg='格式不正确' "
			str41="<span class='red'>*</span>例：01012345678或01012345678"
		else
			str4=" dataType='Limit' min=1 max=50 msg='必填' "
			str41="<span class='red'>*</span>"
		end if
	else
		if CheckPurview(gs_kh,"16")=True then
			str4=" dataType='PhoneNull' msg='格式不正确' "
			str41="例：01012345678或01012345678"
		else
			str4="dataType='Limit' max=50  msg='长度不超过50个字'"
			str41=""
		end if
	end if
	Response.write "" & vbcrlf & "             <input name=""phone2"" type=""text"" size=""15"" id=""phone2""   "
	Response.write str4
	Response.write ">" & vbcrlf & "            "
	Response.write str41
	Response.write "<span id=""test16"" class=""red""></span>"
	if  tp=1 then
		Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:hand"" onClick=""phoneCall(phone2.value)"">" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "        <td><div align=""right"">电子邮件：</div></td>" & vbcrlf & "        <td class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "            <input name=""email"" id=""email"" type=""text"" size=""15"" onChange=""callServer(9,'email');"" dataType="
	if  CheckPurview(gs_kh,"9")=True then
	...
	Response.write "<span id=""test9"" class=""red""></span>" & vbcrlf & "          "
	...
	Response.write "</div></td>" & vbcrlf & "        <td><div align=""right"">手　机2：</div></td>" & vbcrlf & "        <td class=""gray"">" & vbcrlf & "          "
	if CheckPurview(gs_kh,"6")=True then
		str5=" dataType='MobileNull' msg='格式不正确' "
		str51="例：13123456789"
	else
		str5="dataType='Limit' max=50  msg='长度不超过50个字'"
		str51=""
	end if
	Response.write "" & vbcrlf & "             <div align=""left"">" & vbcrlf & "                <input name=""mobile2"" type=""text"" id=""mobile2"" size=""15""  "
	Response.write str5
	Response.write " ><span id=""test30"" class=""red""></span>"
	Response.write str51
	if  tp=1 then
		Response.write "" & vbcrlf & "<img src=""../images/15.gif"" border=""0"" align=""absbottom"" style="" cursor:hand"" onClick=""phoneCall(mobile2.value)"">" & vbcrlf & ""
	end if
	if open_67_13=1  and open_67_19<>1  then
		Response.write "" & vbcrlf & "     <img src=""../images/message.gif"" onClick=""sendSms(mobile.value);function sendSms(phone){if (phone.length==11){" & vbcrlf & "window.open('../message/topadd.asp?phone='+phone+'','newsSendSMS','width=900,height=800,fullscreen=no,scrollbars=0,toolbar=0,status=no,resizable=1,location=no,menubar=no,menubar=no,left=20,top=100');} else{ alert('请填写正确的号码！');}}""  style=""cursor:hand"" border=""0"" alt=""发送短信"" align=""absbottom"">" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "      </tr>" & vbcrlf & "      <tr>" & vbcrlf & "        <td height=""33""><div align=""right"">QQ：</div></td>" & vbcrlf & "        <td height=""33"" class=""gray""><input name=""qq"" type=""text"" size=""15"" id=""qq1"" onChange=""callServer(7,'qq1');value=value.replace(/[^\d]/g,'')"" dataType="
	if  CheckPurview(gs_kh,"7")=True then
		Response.write """QQ"""
	else
		Response.write """Limit"""
	end if
	...
	Response.write "<span id=""test7"" class=""red""></span></td>" & vbcrlf & "        <td height=""33""><div align=""right"">MSN：</div></td>" & vbcrlf & "        <td height=""33"" colspan=""3"" class=""gray""><input name=""msn"" type=""text"" size=""15""  id=""msn"" onChange=""callServer(8,'msn');""  dataType="
	if  CheckPurview(gs_kh,"8")=True then
		Response.write """Email"""
	else
		Response.write """Limit"""
	end if
	...
	Response.write "<span id=""test8"" class=""red""></span></td>" & vbcrlf & "        </tr>" & vbcrlf & "" & vbcrlf & "       <tr>" & vbcrlf & "        <td><div align=""right"">家庭地址：</div></td>" & vbcrlf & "        <td height=""33"" colspan=""3"" class=""gray"">" & vbcrlf & "              <div align=""left"">" & vbcrlf & "                <input name=""address"" type=""text"" size=""40"" dataType=""Limit"" max=""100""  msg=""地址太长了，最多100个字"">" & vbcrlf & "              </div></td>" & vbcrlf & "        <td><div align=""right"">邮　　编：</div></td>" & vbcrlf & "        <td class=""gray"">" & vbcrlf & "          <divalign=""left"">" & vbcrlf & "            <input name=""zip"" type=""text"" size=""15"" id=""zip"" onChange=""callServer(10,'zip');value=value.replace(/[^\d]/g,'')"" dataType="
	if  CheckPurview(gs_kh,"10")=True then
		Response.write """Zip"""
	else
		Response.write """Limit"""
	end if
	...
	Response.write "<span id=""test10"" class=""red""></span>" & vbcrlf & "          </div></td>" & vbcrlf & "      </tr>" & vbcrlf & "      "
	if h=1 then
		Response.write "" & vbcrlf & "      <tr>" & vbcrlf & "        <td><div align=""right"">区　　域：</div></td>" & vbcrlf & "        <td height=""33"" class=""gray""><select name=""area"" class=""gray"">" & vbcrlf & "          "
		dim area,area2
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from sort order  by sort2 "
		...
		do until rs.eof
			...
			Response.write area
			Response.write """>"
			Response.write area2
			Response.write "</option>" & vbcrlf & "          "
			i=i+1
			Response.write "</option>" & vbcrlf & "          "
			rs.movenext
		loop
		Response.write "" & vbcrlf & "        </select></td>" & vbcrlf & "        <td><div align=""right"">行　　业：</div></td>" & vbcrlf & "        <td class=""gray""><select name=""trade"" class=""gray"">" & vbcrlf & "          "
		set rs=server.CreateObject("adodb.recordset")
		...
		if  CheckPurview(bt_kh,"22")=True then
			Response.write " &nbsp;<span class=""red"">*</span>"
		end if
		Response.write "<span class=""red"" id=""test104""></span></td>" & vbcrlf & "        <td><div align=""right""></div></td>" & vbcrlf & "        <td class=""gray"">&nbsp;</td>" & vbcrlf & "      </tr>" & vbcrlf & "   "
	end if
	Response.write "" & vbcrlf & "         <tr class=""top"">" & vbcrlf & "        <td height=""27"" colspan=""6"" ><div align=""left""><strong>详细信息</strong></div></td>" & vbcrlf & "        </tr>" & vbcrlf & "            <tr>" & vbcrlf & "          <td><div align=""right"">学　　历：</div></td>" & vbcrlf & "              <td height=""33""class=""gray""><input name=""xl"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf &               "<td height=""33""><div align=""right"">学　　位：</div></td>" & vbcrlf &             "<td height=""33"" class=""gray""><input name=""xw"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "          <td><div align=""right"">专　　业：</div></td>" & vbcrlf & "              <td class=""gray""><input name=""zy"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "           </tr>" & vbcrlf & "  <tr>" & vbcrlf & "         <td><div align=""right"">毕业院校：</div></td>" & vbcrlf & "         <td height=""33"" class=""gray""><input name=""yx"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "         <td><div align=""right"">籍　　贯：</div></td>" & vbcrlf & "         <td class=""gray""><input name=""jg"" type=""text"" size=""15""  dataType=""Limit"" "
	...
	Response.write "</td>" & vbcrlf & "         <td><div align=""right"">民　　族：</div></td>" & vbcrlf & "         <td class=""gray""><input name=""mz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "       </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">身　　高：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""sg"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "          <td><div align=""right"">体　　重：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""tz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "          <td><div align=""right"">体　　型：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""tx"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">鞋　　码：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""xm"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "          <td><div align=""right"">是否吸烟：</div></td>" & vbcrlf & "          <td class=""gray""><input type=""radio"" name=""xy"" value=""吸烟"">" & vbcrlf & "吸烟" & vbcrlf & "  <input type=""radio"" name=""xy"" value=""不吸烟"">" & vbcrlf & "不吸烟 </td>" & vbcrlf & "          <td><div align=""right"">喜欢烟牌：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""xyname"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">是否饮酒：</div></td>" & vbcrlf & "          <td class=""gray"">" & vbcrlf &             "<div align=""left"">" & vbcrlf &               "<input type=""radio"" name=""yj"" value=""饮酒"">" & vbcrlf &               "饮酒" & vbcrlf &               "<input type=""radio"" name=""yj"" value=""不饮酒"">" & vbcrlf &             "不饮酒</div></td>" & vbcrlf &           "<td><div align=""right"">喜欢酒类：</div></td>" & vbcrlf & "          <td class=""gray""><div align=""left"">" & vbcrlf & "            <input name=""yjsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "          </div></td>" & vbcrlf & "          <td><div align=""right"">喜欢酒牌：</div></td>" & vbcrlf &           "<td class=""gray""><input name=""yjname"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf &             "酒量：<input name=""yjsize"" type=""text"" size=""5"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">是否喝茶：</div></td>" & vbcrlf & "          <td class=""gray""><input type=""radio"" name=""hc"" value=""喝茶"">" & vbcrlf & "喝茶" & vbcrlf & "  <input type=""radio"" name=""hc"" value=""不喝茶"">" & vbcrlf & "不喝茶</td>" & vbcrlf & "          <td><div align=""right"">喜欢茶类：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""hcsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "          <td><div align=""right"">饮食忌口：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""jk"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">疾病情况：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""jb"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf &           "<td><div align=""right"">驾　　照：</div></td>" & vbcrlf &           "<td class=""gray""><div align=""left"">" & vbcrlf &             "<input type=""radio"" name=""jz"" value=""有"">" & vbcrlf &             "有" & vbcrlf &             "<input type=""radio"" name=""jz"" value=""没有"">" & vbcrlf & "没有</div></td>" & vbcrlf &           "<td><div align=""right"">私　　车：</div></td>" & vbcrlf &           "<td class=""gray""><input type=""radio"" name=""sc"" value=""有"">" & vbcrlf &"有" & vbcrlf &               "<input type=""radio"" name=""sc"" value=""无"">" & vbcrlf &               "无" & vbcrlf &             "</td>" & vbcrlf &         "</tr>" & vbcrlf &         "<tr>" & vbcrlf &           "<td><div align=""right"">私车车型：</div></td>" & vbcrlf &           "<td class=""gray""><div align=""left"">" & vbcrlf &             "<input name=""scsort"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf &           "</div></td>" & vbcrlf &           "<td><div align=""right"">私车颜色：</div></td>" & vbcrlf &           "<td class=""gray""><div align=""left"">" & vbcrlf & "            <input name=""scys"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了"">" & vbcrlf & "          </div></td>" & vbcrlf & "          <td><div align=""right"">私车牌照：</div></td>" & vbcrlf & "          <td class=""gray""><input name=""scpz"" type=""text"" size=""15"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td><div align=""right"">明显特征：</div></td>" & vbcrlf & "          <td colspan=""5"" class=""gray""><input name=""tezheng"" type=""text"" size=""60"" dataType=""Limit"" min=""0"" max=""50""  msg=""太长了""></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "        <td><div align=""right"">爱好特长：</div></td>" & vbcrlf & "        <td colspan=""5"" class=""gray"">" & vbcrlf & "          <div align=""left"">" & vbcrlf & "            <textarea name=""joy"" cols=""80"" rows=""4"" dataType=""Limit"" max=""3000""  msg=""最多3000个字""></textarea>" & vbcrlf & "          </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "           <tr>" & vbcrlf & "      <td><div align=""right"">备　　注：</div></td>" & vbcrlf & "      <td colspan=""5"" class=""gray"">" & vbcrlf & "              <div align=""left"">" & vbcrlf & "                  <textarea name=""intro"" cols=""80"" rows=""4"" dataType=""Limit"" max=""5000""  msg=""最多5000个字"" style=""display:none""></textarea>" & vbcrlf & "                  <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME>" & vbcrlf & "              </div>" & vbcrlf & "           </td></tr>" & vbcrlf & "      <tr>" & vbcrlf & "        <td  colspan=""6""class=""gray"">   <p align=""center"">" & vbcrlf & "                <input type=""submit"" name=""Submit42"" value=""保存""  class=""page"" onclick=""return check1()""/>" & vbcrlf & "" & vbcrlf & "                <input type=""reset"" value=""重填"" class=""page"" name=""B2"">" & vbcrlf & "" & vbcrlf &"        </td></tr>" & vbcrlf & "               </form>" & vbcrlf & "    </table>" & vbcrlf & "     </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "          </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <IFRAME style=""display:none"" name=""I5"" id=""I5"" FRAMEBORDER=""0"" SCROLLING=""no"" marginwidth=""1"" marginheight=""1""></IFRAME>" & vbcrlf & "</table>"& vbcrlf & ""
	action1="联系人添加"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=""javascript"" defer=""defer"""">" & vbcrlf & "    getBirthYear();" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""上传头像""  style=""width:500px;height:200px;padding:5px;background: #fafafa;top:60px;left:450px;""  closed=""true"" >" & vbcrlf & "  <div region=""center"" border=""false"" style=""padding:10px;background:#fff;border:0px solid #ccc; width:450px"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <form action=""saveimg.asp"" method=""post"" enctype=""multipart/form-data"" name=""updata"" id=""updata"" target=""I5"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td width=""28%"" height=""45"" valign=""middle""><div align=""right"">路径：</div></td>" & vbcrlf & "          <td width=""72%"" align=""left""valign=""middle""><div align=""left"">" & vbcrlf & "              <input name=""imgurl"" class=""anybutton"" type=""file"" id=""imgurl"" style=""background-color:#FFFFFF;"">" & vbcrlf & "              <input name=""t""  id=""t"" type=""hidden"" value=""2"">" & vbcrlf & "          </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "                               <tr><td colspan=""2"" align=""center"">" & vbcrlf & "                友情提示：请将上传文件大小控制在200KB以内。<br />" & vbcrlf & "                最佳尺寸：90*100(px)  &nbsp;格式要求：JPG/GIF/PNG </td></tr>" & vbcrlf & "      </form>" & vbcrlf & "    </table>" & vbcrlf & "  </div>" & vbcrlf & "  <br />" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:center;height:30px;line-height:30px;"">" & vbcrlf & "    <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('updata').submit()"">确认</a>" & vbcrlf & "    <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a> </div>" & vbcrlf & "</div>" & vbcrlf & "<iframe id=""saveform"" name=""saveform"" style=""width:0px;height:1px;display:none""></iframe>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
