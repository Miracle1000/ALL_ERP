...
'Private Sub Class_Initialize
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>库存预警导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "</HEAD>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class=""place"">库存预警导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出库存预警,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dotstr=""
		for i=1 to num1_dot
			dotstr=dotstr&"0"
			...
			xApp.init Me, conn
			Set xsheet = xApp.sheets.add("库存预警 ")
			HeaderStr = "产品名称,编号,型号,基本单位,分类,可用库存,库存下限,库存上限,待预购数量,下限紧急度,上限紧急度"
...
'elseif px="14" then
	elseif px="15" then
		px_Result=px_Result+" order by per1 desc,date7 desc "
'elseif px="15" then
	elseif px="16" then
		px_Result=px_Result+" order by per1 asc,date7 asc "
'elseif px="16" then
	elseif px="17" then
		px_Result=px_Result+" order by per2 desc,date7 desc "
'elseif px="17" then
	elseif px="18" then
		px_Result=px_Result+" order by per2 asc,date7 asc "
'elseif px="18" then
	end if
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from ("&_
	"select a.ord,addcate,title,order1,a.sort1,a.company,type1,a.unit,unitjb,pym,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6,intro1,intro2,intro3,"&_
	...
	"from product a "&_
	"LEFT JOIN ("&_
	"SELECT ord,SUM(numjb) AS ku_num FROM (Select suba.ord,(CASE WHEN suba.unit=subb.unitjb THEN num2 ELSE num2*ISNULL((SELECT TOP 1 bl FROM jiage WHERE product=suba.ord AND unit=suba.unit),0) END) numjb FROM ku suba INNER JOIN product subb ON suba.ord=subb.ord and exists(select ord from sortck where ord=suba.ck and del=1 and (charindex(',"&session("personzbintel2007")&",',','+replace(cast(intro as varchar(4000)),' ','')+',')>0 or replace(cast(intro as varchar(4000)),' ','') = '0')) ) subaa GROUP BY ord) AS b "&_
	"ON a.ord=b.ord where a.canoutstore=1 and a.del=1 "& showZeroSQL &" and (isnull(ku_num,0)<=aleat1 or isnull(ku_num,0)>aleat2)) as e where 1=1 "&str_Result&" "& showKuLimitZeroSQL  &" "&px_Result&"  --aleat.asp"
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i
	i=1 : PreProcV = -1 : rowIndex = 0 : PageCount = 1
'dim i
...
'Dim iOrd
		...
		sql1="select ord,sum((case when bl<>1 then bl else 1 end)*ydnum)ydnum ,sum((case when bl<>1 then bl else 1 end)*ztnum)ztnum,sum((case when bl<>1 then bl else 1 end)*isnull(locknum1,0))locknum1  from #ZTYD_list group by ord"
...
'if alert2=0 then
			xsheet.writestr k
			xsheet.writehtml "<td width='100' align='center'>"&rs("order1")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&rs("type1")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&rs("unitname")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&trim(rs("sortname"))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&FormatNumber(kynum,num1_dot,-1,-2,-1)&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&trim(rs("sortname"))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&FormatNumber(alert1,num1_dot,-1,-2,-1)&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&trim(rs("sortname"))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&FormatNumber(alert2,num1_dot,-1,-2,-1)&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&trim(rs("sortname"))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&iif(needYgNum>=0,"--",FormatNumber(Abs(needYgNum),num1_dot,-1,-2,-1))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&trim(rs("sortname"))&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&per1&"%"&"</td>"
			xsheet.writehtml "<td width='100' align='center'>&nbsp;"&per2&"%"&"</td>"
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>库存预警导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!库存预警导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	tfile=Server.MapPath("库存预警_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'库存预警_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	Set xApp = Nothing
	action1="库存预警导出"
	call close_list(1)
	Response.write " " & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的库存预警</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
