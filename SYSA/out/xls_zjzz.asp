...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(5).ColumnWidth=20
			...
			xlApplication.ActiveSheet.Columns(7).NumberFormatLocal = "#,##0."&hldotstr&"_ "
			...
			if open_11_10=3 then
				list=""
			elseif open_11_10=1 then
				list="and cateid in ("&intro_11_10&")"
			else
				list="and 1=0"
			end if
			...
			if A<>"0" then
				Str_Result=Str_Result+" and ck2="&A&" "
'if A<>"0" then
			end if
			...
			if D<>"0" then
				Str_Result=Str_Result+" and ck1="&D&" "
'if D<>"0" then
			end if
			if C<>"" then
				if B="rzzt" then
...
'elseif B="rzgy" then
				elseif B="zczh" then
					Str_Result=Str_Result+ " and ck2 in (select id from sortbank where sort1 like '%"&C&"%' and  del=1 AND CHARINDEX(',"&session("personzbintel2007")&",',','+CAST(person AS VARCHAR(8000))+',') > 0) "
'elseif B="zczh" then
				elseif B="zrzh" then
					Str_Result=Str_Result+ " and ck1 in (select id from sortbank where sort1 like '%"&C&"%' and  del=1 AND CHARINDEX(',"&session("personzbintel2007")&",',','+CAST(person AS VARCHAR(8000))+',') > 0) "
'elseif B="zrzh" then
				elseif B="jsr" then
...
'elseif px=8 then
			elseif px=9 then
				px_Result="order by bzmoney desc,datediff(mi,'1970-01-01 00:00:00',date7) desc"
'elseif px=9 then
			elseif px=10 then
				px_Result="order by bzmoney asc,datediff(mi,'1970-01-01 00:00:00',date7) asc"
'elseif px=10 then
			elseif px=11 then
...
'elseif px=12 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from gate where ord=bankmove.cateid) as cateidname from bankmove "&Str_Result&" "&px_Result&" "
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "账间转账单_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				...
				title=rs("title")
				if rs("bzmoney")="" or IsNull(zbcdbl(rs("bzmoney"))) then
					bzmoney=0.00
				else
					bzmoney=zbcdbl(rs("bzmoney"))
				end if
				if bzmoney=0 then
					bzmoney=zbcdbl(rs("money1"))
				end if
				bzmoney=Formatnumber(bzmoney,num_dot_xs,-1)
				bzmoney=zbcdbl(rs("money1"))
				date3=rs("date3")
				date7=rs("date7")
				cateid=rs("cateid")
				intro=RemoveHTML(HTMLDecode(rs("intro")))
				ck2=rs("ck2")
				ck1=rs("ck1")
				if cateid<>"" then
				...
				if ck1<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select a.intro as intro,b.sort1 as sort1,b.person from sortbz as a left join sortbank as b on a.id=b.bz where b.id="&ck1&" "
					rs7.open sql7,conn,1,1
					if rs7.eof then
						sortbz1=""
						ck1name=""
						...
						sortbz1=rs7("intro")
						If instr(","&rs7("person")&",",","&session("personzbintel2007")&",") > 0 Then
							ck1name=rs7("sort1")
						else
							ck1name=""
						end if
				...
				if ck2<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select a.intro as intro,b.sort1 as sort1,b.person from sortbz as a left join sortbank as b on a.id=b.bz where b.id="&ck2&" "
					rs7.open sql7,conn,1,1
					if rs7.eof then
						sortbz2=""
						ck2name=""
				...
				hl=bzmoney/zbcdbl(rs("money1"))
				hl=Formatnumber(hl,hl_dot,-1)
				hl=bzmoney/zbcdbl(rs("money1"))
				money1=sortbz2&money1
				bzmoney=sortbz1&bzmoney
				...
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,3).Value = ck2name
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,3).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,4).Value = ck1name
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,4).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,5).Value = money1
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,5).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,6).Value = bzmoney
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,6).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,7).Value = hl
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,7).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,8).Value = cateidname
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,8).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,9).Value = Replace(Replace(Replace(Replace(intro,"<br>",vbcrlf),"</P>",vbcrlf),"<P>",vbcrlf),"&nbsp;"," ")
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,9).font.Size=10
				bzmoney=sortbz1&bzmoney
				xlWorksheet.Cells(1+i,10).Value = date7
				bzmoney=sortbz1&bzmoney
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>账间转账单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!账间转账单导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("账间转账单_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的账间转账单</u></strong></font></a></p>"
