...
'bigsystemtype = ".mozi"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlWorksheet.Cells(1,3).Value = "费用总额"
			...
			elseif open_1_1=1 then
				list="and x.cateid in ("&intro_1_1&")"
			else
				list="and cateid=0"
			end if
			Str_Result="and x.del=1 and x.sort3=1 "&list&""
			set rs=server.CreateObject("adodb.recordset")
			sql="select x.ord,x.name,x.khid,x.cateid,sum(y.money1) as money1 from tel as x  inner join pay as y on x.ord=y.company and y.del=1 and y.complete=3  "&Str_Result&" group by x.ord,x.name,x.khid,x.cateid order by money1 desc"
			...
			do until rs.eof
				...
				set rs7=nothing
				xlWorksheet.Cells(1+i,4).Value =Formatnumber(pay,num_dot_xs,-1)
				set rs7=nothing
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户费用排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户费用排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("客户费用排行_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的客户费用排行</u></strong></font></a></p> " & vbcrlf & ""
