...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {     margin-top: 0px;}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "#ht1 table{margin-bottom:-1px}" & vbcrlf & "#content.resetLineHei>tbody>tr>td{line-height:1.4!important;padding:9px 4px!important}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src= ""../Script/yp_outDlist.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if open_101_1=3 then
		list=""
	elseif open_101_1=1 then
		list=" and get_addcateid in ("&intro_101_1&")"
	else
		list=" and get_addcateid=0"
	end if
	Str_Result=" where get_del=1 "&list&""
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by get_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by get_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by get_time desc"
	elseif px="4" then
		Ptr_Result=" order by get_time asc"
	elseif px="5" then
		Ptr_Result=" order by c.prod_bh desc"
	elseif px="6" then
		Ptr_Result=" order by c.prod_bh asc"
	end if
	ret  =request("ret")
	ret2=request("ret2")
	A    =request("A")
	...
	W1=tmparr(0)
	W2=tmparr(1)
	ypjs_data=request("ypjs_data")
	If ypjs_data<>"" Then
		ypjs_type=request("ypjs_type")
		Select Case ypjs_type
		Case "ypmc":
		E=ypjs_data
		Case "ypbh":
		F=ypjs_data
	...
	if A<>"" then
		Str_Result=Str_Result&" and b.prod_unit in("&A&") "
	end if
	if B<>"" then
		Str_Result=Str_Result&" and b.sto_id in("&B&") "
	end if
	if C<>"" then
		Str_Result=Str_Result&" and get_title like '%"&C&"%' "
	end if
	if D<>"" then
		Str_Result=Str_Result&" and get_bh like '%"&D&"%' "
	end if
	if E<>"" then
		Str_Result=Str_Result&" and c.prod_name like '%"&E&"%' "
	end if
	if F<>"" then
		Str_Result=Str_Result&" and c.prod_bh like '%"&F&"%' "
	end if
	if G<>"" then
		Str_Result=Str_Result&" and c.prod_xh like '%"&G&"%' "
	end if
	if H<>"" then
		Str_Result=Str_Result&" and b.prod_ph like '%"&H&"%' "
	end if
	if I<>"" then
		Str_Result=Str_Result&" and b.prod_xlh like '%"&I&"%' "
	end if
	if J<>"" then
		Str_Result=Str_Result&" and get_intro like '%"&J&"%' "
	end if
	if M<>"" then
		Str_Result=Str_Result&" and b.prod_gys like '%"&M&"%' "
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',g.get_addtime)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,g.get_addtime,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if ret <>"" and not isnull(ret) then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',g.get_time)>=0 "
'if ret <>"" and not isnull(ret) then
	end if
	if ret2 <>"" and not isnull(ret2) then
		Str_Result=Str_Result+" and  datediff(s,g.get_time,'"&ret2&" 23:59:59')>=0 "
'if ret2 <>"" and not isnull(ret2) then
	end if
	if N<>"" then
		Str_Result=Str_Result&" and g.get_type in("&N&") "
	end if
	If attr<>"" Then
		Str_Result=Str_Result&" and f.pro_type in("&attr&") "
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and  g.get_lycateid in ("&W3&") "
	end if
	...
	alldfhNum = 0
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <form name=""data"" id=""data"" action=""outDList.asp"" method=""get"">" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">领用明细表</td>" & vbcrlf & "                        <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"" class=""top_btns""><div id=""kh"" style='height:22px;line-height:22px;'>" & vbcrlf & "                  <select name=""ypjs_type"" id=""ypjs_type""><option value=""ypmc"" "
	alldfhNum = 0
	if ypjs_type="ypmc" then Response.write "selected"
	Response.write ">用品名称</option><option value=""ypbh"" "
	if ypjs_type="ypbh" then Response.write "selected"
	Response.write ">用品编号</option><option value=""ypxh"" "
	if ypjs_type="ypxh" then Response.write "selected"
	Response.write ">用品型号</option></select> <input type=""text"" size=""12"" name=""ypjs_data"" id=""ypjs_data"" value="""
	Response.write ypjs_data
	Response.write """><input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                 "
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "<input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_outDlist.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "                </select><a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "        <span id=""ht1""></span>" & vbcrlf & "  </form>"& vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" class=""resetLineHei"">   " & vbcrlf & "                 <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""5%"" height=""25"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>领用主题</strong></div></td>" & vbcrlf & "                   <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>领用编号</strong></div></td>" & vbcrlf & "              <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>领用人员</strong></div></td>" & vbcrlf & "                <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>名称</strong></div></td>" & vbcrlf & "                  <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>编号</strong></div></td>" & vbcrlf & "                   <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>型号</strong></div></td>" & vbcrlf & "              <td width=""3%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>单位</strong></div></td>" & vbcrlf & "           <td width=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>领用数量</strong></div></td>" & vbcrlf & "                  <td width=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>返还数量</strong></div></td>" & vbcrlf & "              <tdwidth=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>待返还数量</strong></div></td>" & vbcrlf & "          <td width=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>出库仓库</strong></div></td>" & vbcrlf & "              <td width=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>库存信息</strong></div></td>" & vbcrlf & "         <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>分类</strong></div></td>" & vbcrlf & "              <td width=""4%"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>分类属性</strong></div></td>" & vbcrlf & "                  <td width=""5%"" align=""center"" valign=""middle""  class=""name""><div align=""center"" class=""name""><strong>批号</strong></div></td>" & vbcrlf & "                    <td width=""5%"" height=""27"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>序列号</strong></div></td>" & vbcrlf & "                 <td width=""4%"" height=""27"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>生产日期</strong></div></td>" & vbcrlf & "                    <td width=""4%"" height=""27"" align=""center"" valign=""middle""  class=""name""><div align=""center""><strong>有效日期</strong></div></td>" & vbcrlf & "           <td width=""5%"" align=""center"" valign=""middle"" class=""name""><div align=""center""><strong>备注</strong></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
	currpage=Request("currpage")
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.reg_fid,b.prod_num,b.prod_ph,b.prod_xlh,b.prod_sctime,b.prod_ystime,c.id as cid, "&_
	"c.prod_name,c.prod_bh,c.prod_xh,(case f.pro_type when 1 then '易耗品' when 2 then '非易耗品' end ) as type_attribute,d.sort1,e.sto_name,f.pro_name, "&_
	"g.id as gid, "&_
	"g.get_title, "&_
	"g.get_bh, "&_
	"g.get_addcateid, "&_
	"(select name from gate where ord=g.get_lycateid and del=1) as cateid_Name, "&_
	"isnull(h.fh_num,0) as fh_num,f.pro_type "&_
	"from O_proOutList a  "&_
	"left join O_kuinlist b on a.prod_id=b.id  "&_
	"left join O_productOut g on a.get_fid=g.id  "&_
	"left join O_product c on c.id=b.prod_id  "&_
	"left join sortonehy d on d.id=b.prod_unit  "&_
	"left join O_store e on e.id=a.sto_id  "&_
	"left join O_officeType f on c.prod_type=f.id "&_
	"left join  "&_
	"(select prod_id,sum(prod_num) as fh_num from O_proRetList group by prod_id) h on h.prod_id=a.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=19><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		allnum=0
		allprice=0
		...
		do while not rs7.eof
			addcateid=rs7("get_addcateid")
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "              <td  height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""left"">" & vbcrlf & "              "
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                         <a href=""javascript:void(0);"" onClick=""javascript:window.open('outDetail.asp?ord="
				Response.write rs7("gid")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("gid")
				Response.write rs7("get_title")
				Response.write "</a>" & vbcrlf & "             "
			else
				Response.write rs7("get_title")
			end if
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "                <td  height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("get_bh")
			Response.write "</div></td>" & vbcrlf & "             <td  height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("cateid_Name")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "               "
			if open_101_16=3 or CheckPurview(intro_101_16,trim(addcateid))=True then
				...
				Response.write rs7("cid")
				Response.write "','neww35win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"">"
				Response.write rs7("cid")
			...
			num1=zbcdbl(rs7("prod_num"))
			num=zbcdbl(rs7("get_num"))
			num2=rs7("fh_num")
			if num1="" or isnull(num1) then num1=0
			if num&""="" then num=0 else num = cdbl(num)
			If num2&""="" Then num2=0 else num2 = cdbl(num2)
			if rs7("pro_type") = 2 then
				dfhNum = num - num2
'if rs7("pro_type") = 2 then
			else
				dfhNum = 0
			...
			Response.write "</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(num,num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(num2,num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(dfhNum,num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			...
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("type_attribute")
			...
			allnum=cdbl(allnum)+cdbl(num)
			Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
			allnum1=cdbl(allnum1)+cdbl(num1)
			Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
			allnum2=cdbl(allnum2)+cdbl(num2)
			Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
			alldfhNum = cdbl(alldfhNum) + cdbl(dfhNum)
			Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
			o=o+1
			Response.write "</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
	...
	if rs7.RecordCount>0 then
		Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "              <td height=""25"" colspan=""7"" align=""left"" valign=""middle""  class=""name""><div align=""right"">合计：</div></td>" & vbcrlf & "                 <td width=""45"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
		Response.write Formatnumber(allnum,num1_dot,-1)
		Response.write "</div></td>" & vbcrlf & "             <td width=""45"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
		Response.write Formatnumber(allnum2,num1_dot,-1)
		Response.write "</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
		Response.write Formatnumber(alldfhNum,num1_dot,-1)
		Response.write "</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""name"">&nbsp;</td>" & vbcrlf & "           <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
		Response.write FormatNumber(allnum1,num1_dot,-1)
		Response.write "</div></td>" & vbcrlf & "           <td colspan=""7"" align=""left"" valign=""middle""  class=""name"">&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "          "
	end if
	Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30""></td>" & vbcrlf & "            <td></td>         " & vbcrlf& "       <form name=""oneself"" action=""outDList.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "条信息/页            " & vbcrlf & "                <input name=""CurrPage"" type=""text"" size=""4"" maxlength=""4"" onChange=""this.value=this.value.replace(/[^\d]/g,'')""/>" & vbcrlf & "         <input name=""btntz"" type=""submit""  class=""anybutton2"" id=""btntz"" value=""跳转"" />" & vbcrlf & "          "
	if currpage=1 then
	...
	Response.write "" & vbcrlf & "             </div>" & vbcrlf & "          </td></form>" & vbcrlf & "      </tr>" & vbcrlf & "   <tr><td height=""2000""></td></tr>" & vbcrlf & "        </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按分发时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按分发时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按用品编号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按用品编号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "   </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="领用明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " //return false;" & vbcrlf & " history.back();" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "     document.getElementById('data').action=""kuindel.asp?px="
	Response.write px
	Response.write "&currPage="
	Response.write currpage
	Response.write "&A="
	...
	Response.write "&ret2="
	Response.write ret2
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
