...
'If isEmpty(m_wxPayKindId) Then
	...
	Sub onReportInit(rpt, lt)
		...
		rpt.sql = "EXEC [ERP_GoodsNumSummaryList] "&Info.User&",&ReportSort,@logDate_0 ,@logDate_1,@category,@status,@sType,@keywords,"& validTime &",&pagesize,&pageindex,&excelmode"
	...
	Sub app_oncreate(lvw)
		...
		For i = 6 To 10
			...
			col.canSum = True
		next
		Set col = lvw.headers("变动")
		col.canSum = False
		col.FormatText = "code:GetChangeNum(@cells[""期初""],@cells[""当前数量""],""@value"")"
		beginCol = 11
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		col.width = 180
		Set col = lvw.headers("商品名称")
		col.Formula = "PowerCol(@value,@cells[""ID""],@cells[""creator""],109)"
		col.width = 230
		Set col = lvw.headers("单位")
		col.width = 50
...
'Function FormatNum(num)
	...
	Function GetChangeNum(numFirst,numEnd,num)
		Dim temp
		If numEnd - numFirst < 0 Then
'Dim temp
			temp = "<span class='red'>↓</span>" & num
		ElseIf numEnd - numFirst > 0 Then
			temp = "<span class='red'>↓</span>" & num
			temp = "<span class='red'>↑</span>" & num
		else
			temp = num
