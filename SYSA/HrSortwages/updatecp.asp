...
'If Not rs.Eof Then
	...
	deductible=request("may4")
	If deductible = "" Or IsNumeric(deductible) = false Then deductible = 0
	sort1=request("sort1")
	if instr(sort1,"子女教育")>0 or instr(sort1,"继续教育")>0 or instr(sort1,"大病医疗")>0 or instr(sort1,"住房贷款")>0  or instr(sort1,"住房租金")>0 or instr(sort1,"赡养老人")>0 then
		Response.write"<script language=javascript>alert('此工资项目已存在，请添加其它项目！');history.back()</script>"
		call db_close : Response.end
	end if
	sql = "select * from sortwages where sort1= '"&request("sort1")&"' and id<>"&request("ord")&""
	Set Rs = server.CreateObject("adodb.recordset")
	Rs.open sql,conn,3,2
	if rs.eof then
		sqltext="Update sortwages set deductible="&deductible&", sort1='"&request("sort1")&"',intro="&request("may")&",gate1='"&request("gate1")&"',salaryClass='"&salaryClass&"'  where id="&request("ord")&""
