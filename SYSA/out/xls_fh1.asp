...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			if open_6_1=1 and intro_6_1<>"" then
				Str_Result=Str_Result&" and cateid in ("&intro_6_1&")"
			elseif intro_6_1="" or open_6_1=0 then
				Str_Result=Str_Result&" and cateid=0"
			end if
			if request("ret")<>"" then
				m1=request("ret")
				m2=request("ret2")
			...
			if typ="3333333" or typ="5555555" then
				...
				Str_Result="and (cateid_sp="&session("personzbintel2007")&" and sp>0 and del=1 and complete in (7,11) "&Str_Result&" )  "&noalt1&""
			end if
...
'if m2<>"" then
			...
			i=1
			sqls="select *,isnull((select top 1 sort1 from payjk pa inner join sortbz bz on pa.id=pay.jkid and pa.bz=bz.id),'人民币') bzname from pay where del=1 "&Str_Result&" "&pxResult
			set rss = server.CreateObject("adodb.recordset")
			rss.open sqls,conn,1,1
			C1=rss.recordcount
			xlApplication.Init "费用返还_"&session("name2006chen")&".xls",C1
			if rss.eof then
			else
				do until rss.eof
					title=rss("title")
					datejk=rss("date1")
					allmoney=zbcdbl(rss("money1"))
					spmoney=zbcdbl(rss("money2"))
					perid=rss("cateid")
					addcate=rss("addcate")
					note=rss("intro")
					spstate=rss("complete")
					jkid=rss("jkid")
					...
					member1=setname("gate","ord",addcate,"name")
					member2=setname("gate","ord",perid,"name")
					spname=setname("gate","ord",rss("cateid_sp"),"name")
					if rss("complete")=8 then
						statxt="审批通过"
					elseif rss("complete")=7 then
						statxt="审批中"
					elseif rss("complete")=11 then
						statxt="待审批"
					elseif rss("complete")=12 then
						statxt="审批未通过"
					end if
					bzname=rss("bzname")
					...
					bzname=rss("bzname")
					xlWorksheet.Cells(1+i,3).Value = Formatnumber(allmoney,num_dot_xs,-1)
					bzname=rss("bzname")
					xlWorksheet.Cells(1+i,3).font.Size=10
					bzname=rss("bzname")
					xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
					bzname=rss("bzname")
					xlWorksheet.Cells(1+i,4).Value = Formatnumber(spmoney,num_dot_xs,-1)
					bzname=rss("bzname")
			...
			tfile=Server.MapPath("费用返还_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的费用返还</u></strong></font></a></p> " & vbcrlf & ""
