...
function searchMe(a,b)
{
	...
	var url = "ajax_yp2.asp?a=" + escape(u_name)+"&b=" + escape(u_bh)+"&c=" + escape(u_xh)+"&d=" + escape(u_ph)+"&e=" + escape(u_xlh)+"&t="+u_type+"&sid=" +sid+"&cp="+b+"&stamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
	xmlHttp.open("GET",url,false)
	xmlHttp.send(null);	
...
function addMx(id,prod_name,prod_bh,prod_xh,sort1,prod_ph,prod_xlh,prod_sctime,prod_ystime,pro_name,pro_type,nums)
{
	...
	tr.appendChild(td5);
	td5.align="center";
	var delHref = "<input name=\"prod_num\" value=\"1\" type=\"text\" id=\"prod_num\" onKeyUp=\"value=value.replace(/[^\\d*|\\.]/g,'')\"  onpropertychange=\"formatData(this,'number');\" style=\"width:50px;\">";
	td5.innerHTML=delHref;
	var tds = document.createElement('td');
	tr.appendChild(tds);
	tds.align="center";
	var delHref = parseFloat(nums) + "<input type=\"hidden\" value=\"" + nums + "\" name=\"ret_num\">";
	...
	a = a + 1
	document.getElementById("rscount").innerHTML = parseInt(document.getElementById("rscount").innerHTML) - 1;
}
