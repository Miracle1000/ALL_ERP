		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					xlWorksheet.Cells(1,j).Value = "产品数量"
					xlWorksheet.Cells(1,j).font.Size=10
					xlWorksheet.Cells(1,j).font.bold=true
					j=j+1
					xlWorksheet.Cells(1,j).font.bold=true
...
'if m1="" then
					...
					case 1
					Str_Order = " order by a.date7 desc"
					case 2
					Str_Order = " order by a.date7 asc"
					case 3
					Str_Order = " order by b.title desc"
					case 4
					Str_Order = " order by b.title asc"
					case 5
					Str_Order = " order by b.order1 desc"
					case 6
					Str_Order = " order by b.order1 asc"
					case 7
					Str_Order = " order by b.type1 desc"
					case 8
					Str_Order = " order by b.type1 asc"
					case 9
					Str_Order = " order by a.num1 desc"
					case 10
					Str_Order = " order by a.num1 asc"
					case 11
					Str_Order = " order by caigouTitle desc"
					case 12
					Str_Order = " order by caigouTitle asc"
					case 13
					Str_Order = " order by d.name desc"
					case 14
					Str_Order = " order by d.name asc"
					end select
...
'if companyName<>"" then
					...
					if caigouName<>"" then
						Str_Result=Str_Result+" and c.title like'%" & caigouName &"%'"
'if caigouName<>"" then
					end if
					if intro<>"" then
...
'if intro<>"" then
	...
	next
	Str_Result=Str_Result+" and  b.sort1 in ("&area_list&") and b.sort1<>''"
	menuarea(aryReturn2(i))
	end if
	if m1<>"" then
...
'if m1<>"" then
	...
	if m2<>"" then
		Str_Result=Str_Result+" and a.date7<='"&cdate(m2)&"' "
'if m2<>"" then
	end if
	B=request("B")
	C=request("C")
...
'elseif B="khmc"  then
	...
	sumNum1=0
	sumNum=0
	strTable=" from caigoulist a left join product b" & _
	" on a.ord=b.ord left join caigou c on a.caigou=c.ord left join tel d on c.company=d.ord"
	strSql="select sum(a.num1) as sumNum,sum(a.money1) as sumMoney " & strTable & Str_Result
	rs.open strSql,conn
	if not rs.eof then
		sumNum=rs(0)
		sumMoney=rs(1)
	...
	rs.close
	strSql="select a.ord,a.price1,a.num1,a.money1,a.num3,a.caigou" & _
	",b.title,b.order1,b.type1,c.title as caigouTitle,c.company,d.name" & _
	",unit=(select top 1 sort1 from sortonehy where ord=a.unit)" & _
	"strTable & Str_Result & Str_Order"
...
'do while not rs.eof
		...
		j=j+1
		j=1
		xlWorksheet.Cells(1+i,j).Value = rs("order1")
		j=1
		xlWorksheet.Cells(1+i,j).font.Size=10
		j=1
		j=j+1
		j=1
		xlWorksheet.Cells(1+i,j).Value = rs("type1")
		j=1
		xlWorksheet.Cells(1+i,j).font.Size=10
		j=1
		j=j+1
		...
		j=j+1
		j=1
		if ZBRuntime.MC(12010) and (open_22_14=3 or CheckPurview(intro_22_14,trim(cateid))) then
			if isnull(rs("caigouTitle")) then
				contractTitle="采购已被删除"
			else
	...
	set rs=nothing
	set rs1=nothing
	xlWorksheet.Range("A"&(2+i)&":D"&(2+i)).Merge
	set rs1=nothing
	...
	set rs1=nothing
	xlWorksheet.Cells(2+i,5).Value = sumNum
	set rs1=nothing
	xlWorksheet.Cells(2+i,5).font.Size=10
	set rs1=nothing
	xlWorksheet.Cells(2+i,5).HorizontalAlignment=1
	set rs1=nothing
	xlWorksheet.Cells(2+i,9).Value = Formatnumber(sumMoney,num_dot_xs,-1)
	set rs1=nothing
