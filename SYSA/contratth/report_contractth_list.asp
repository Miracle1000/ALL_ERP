...
'items = rpt.Settings.item(i)
	...
	Sub App_lvw_OnSumProcInit(lvwobj, c, i, ByRef dosum)
		If c.dbname="关联退货" Then
			dosum = true
			end if
		end sub
		Sub App_lvw_OnSumProc(lvwobj, dbname, isPageSum, ByRef sHtml)
			If dbname="关联退货" Then
				If isPageSum = 0 Then
					sHtml = "<div style='width:100%;text-align:left;padding-left:5px'>￥"&FormatNumber(lvwobj.currsumarray(28),Info.moneyNumber,-2)&"</div>"
'If isPageSum = 0 Then
				else
					sHtml = "<div style='width:100%;text-align:left;padding-left:5px'>￥"&FormatNumber(lvwobj.allsumarray(28),Info.moneyNumber,-2)&"</div>"
'If isPageSum = 0 Then
				end if
			end if
		end sub
		Sub onReportInit(rpt, state)
			Dim dat, rs,sql,bz_open
			Call app.addDefaultScript()
			dat = Split(year(now) & "-" & month(now) & "-1," & date, ",")
			'Call app.addDefaultScript()
			dim searchType ,searckKey : searchType = "PRODUCT"
			BKPayModel = Request("BKPayModel")
			if len(BKPayModel)=0 then
				BKPayModel = 0
			end if
			CustomerName=Request("CustomerName")
			if len(CustomerName)>0 then
				searchType = "CUSTOMER"
				searckKey = CustomerName
			end if
			Bz=Request("Bz")
			CKDate0=Request("CKDate0")
			CKDate1=Request("CKDate1")
			if len(CKDate0)>0 or len(CKDate1)>0 then
				dat = split(CKDate0 &"," & CKDate1,",")
			end if
			contractthOrd = app.base64.deurl(request("contractthOrd"))
			IF contractthOrd<>"" THEN
				dat = Split("," & date, ",")
			end if
			rpt.title = "产品销售退货明细表"
			rpt.onlyadSearchModel = true
			rpt.addfield "","hidden","contractthOrd","","",contractthOrd
			rpt.addfield "退货日期：","dates","thDate","","", dat
			rpt.addfield "","select","searchType","产品名称"&Chr(1)&"PRODUCT"&Chr(2)&"产品型号"&Chr(1)&"TYPE"&Chr(2)&"产品编号"&Chr(1)&"ORDER"&Chr(2)&"产品说明"&Chr(1)&"INTRO"&Chr(2)&"产品参数"&Chr(1)&"PARAM"&Chr(2)&"图片与附件"&Chr(1)&"IMG"&Chr(2)&"退货主题"&Chr(1)&"TITLE"&Chr(2)&"客户名称"&Chr(1)&"CUSTOMER","",searchType
			rpt.SetNullMsg 2,"-检索条件-"
			rpt.addfield "","text","searckKey", "", "", searckKey
			rpt.addfield "","hidden","ad_bzs","","",Bz
			rpt.addfield "","hidden","BKPayModel","","",BKPayModel
			rpt.asearch.addfield "产品名称","text","ad_product", ""
			rpt.asearch.addfield "产品分类","cpfl","ad_product_cls", ""
			rpt.asearch.addfield "产品编号","text","ad_product_order", ""
			rpt.asearch.addfield "产品型号","text","ad_product_type", ""
			rpt.asearch.addfield "销售人员","gates","ad_salesman", ""
			rpt.asearch.addfield "总价","moneys","ad_money", ""
			rpt.asearch.addfield "产品数量","numbers","ad_number", ""
			rpt.asearch.addfield "退货日期","dates","ad_thDate",""
			rpt.asearch.addfield "退货次数","numbers","ad_thCount",""
			rpt.asearch.addfield "关联客户","text","ad_customer",""
			rpt.asearch.addfield "关联合同","text","ad_contract",""
			rpt.asearch.addfield "关联退货","text","ad_thTitle",""
			...
			If bz_open = 1 then
				rpt.asearch.addfield "币种","checks","ad_bzs","select sort1,id from sortbz  order by gate1 desc"
			else
				rpt.asearch.addfield "币种","checks","ad_bzs","select sort1,id from sortbz where id=14 order by gate1 desc"
			end if
			rpt.asearch.addfield "图片与附件","text","ad_intro3",""
			rpt.sql = "exec erp_report_ContractTHdetailsReportForms &ReportSort,@thDate_0, @thDate_1,'"&contractthOrd&"',@searchType,@searckKey,"&Info.User&","&_
			"@ad_product,@ad_product_cls,@ad_product_order,@ad_product_type,@ad_salesman,@ad_money_0,@ad_money_1,@ad_number_0,@ad_number_1,"&_
			"@ad_thDate_0,@ad_thDate_1,@ad_thCount_0,@ad_thCount_1,@ad_customer,@ad_contract,@ad_thTitle,@ad_bzs,@ad_intro3,&pagesize,&pageindex,&excelmode,&SearchMode,"&BKPayModel&""
			rpt.canexcel =  app.power.existsPower(41,10)
			rpt.canprint =  app.power.existsPower(41,7)
			rpt.AddSort "日期","1"
			rpt.AddSort "产品名称","2"
			rpt.AddSort "产品编号","3"
			rpt.AddSort "产品型号","4"
		...
		Sub app_oncreate(lvw)
			...
			h.cansum = False
			h.dbtype = "str"
			Set h = lvw.headers("产品单位")
			h.width = 60
			h.cansum = False
			h.dbtype = "str"
			Set h = lvw.headers("币种")
			h.width = 60
			h.cansum = False
			h.dbtype = "str"
			Set h = lvw.headers("数量")
			h.dbtype = "number"
			h.width = 60
			Set h = lvw.headers("未税单价")
			h.dbtype = "salesprice"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("含税单价")
			h.dbtype = "salesprice"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("票据类型")
			h.dbtype = "str"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("税率")
			h.dbtype = "money"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("金额")
			h.dbtype = "money"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("税额")
			h.dbtype = "money"
			h.width = 60
			h.canSum = false
			Set h = lvw.headers("总价")
			h.dbtype = "money"
			h.width = 60
			Set h = lvw.headers("关联退货")
			h.canSum = true
			h.setLink   "", "关联退货单ID", "关联退货单销售人员", 41, -11
			h.canSum = true
			h.dbtype = "str"
			h.width = 130
			Set h = lvw.headers("关联退货单ID")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("关联退货单销售人员")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("关联合同ID")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("关联合同")
			h.setLink   "", "关联合同ID", "销售人员ID", 5, -1
			Set h = lvw.headers("关联合同")
			h.width = 130
			h.canSum = False
			h.dbtype = "str"
			Set h = lvw.headers("销售人员")
			h.width = 130
			h.cansum = False
			h.dbtype = "str"
			Set h = lvw.headers("退货次数")
			h.canSum = false
			h.width = 70
			Set h = lvw.headers("关联客户")
			h.cansum = False
			h.dbtype = "str"
			h.width = 130
			h.formula = "showCustomLink(rs(""关联客户ID""), @value ,rs(""关联客户销售人员""),rs(""sort3""),rs(""share"") )"
			Set h = lvw.headers("关联客户ID")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("关联客户销售人员")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("人民币总价")
			h.dbtype = "money"
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("销售人员ID")
			h.width = 0
			h.display = "none"
			Set rs = cn.execute("select id,title,kd,name,sorce,set_open,(case when sorce in (9,10,11,12,13) then 1 else 0 end) as align,(case sorce when 16 then 1 when 17 then 2 when 18 then 3 when 19 then 4 else sorce end) px from zdymx where sort1=41 and sorce in (16,17,18,19,8,9,10,11,12,13,14,15) order by px")
			While rs.eof = False
				Set h = lvw.headers(rs("name"))
				h.title = rs("title")
				h.canSum = false
				If rs("align") = 1 Then h.dbtype = "str"
			...
			wend
			rs.close
			Set h = lvw.headers("sort3")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("share")
			h.width = 0
			h.display = "none"
			Set h = lvw.headers("inx")
			h.width = 0
			h.display = "none"
		end sub
		Dim getPower , intro_1_1 ,intro_1_14 , intro_26_1 , intro_26_14
		getPower = False
		Function showCustomLink(ord, name , cateid , sort3 , share )
			Dim s : s= ""
			If getPower = False Then
				intro_1_1 = app.power.GetPowerIntro(1,1)
				intro_1_14 = app.power.GetPowerIntro(1,14)
				intro_26_1 = app.power.GetPowerIntro(26,1)
				intro_26_14 = app.power.GetPowerIntro(26,14)
				getPower = True
			end if
			If sort3 = 2 Then
				if Len(intro_26_1) = 0 Or InStr(","& intro_26_1 &",",","& cateid &",")>0 Then
					if Len(intro_26_14) = 0 Or InStr(","& intro_26_14 &",",","& cateid &",")>0 Then
						s = "<a href='javascript:void(0)' onClick=""javascript:window.open('" & app.virpath & "work2/content.asp?ord=" & app.base64.pwurl(ord) & "','newwin_gys','width=900,height=500,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & name & "</a>"
					else
						s = name
					end if
				end if
			else
				if Len(intro_1_1) = 0 Or InStr(","& intro_1_1 &",",","& cateid &",")>0  Or  InStr(1,","&Replace(share," ","")&",", ","&info.user&",",1) > 0 Or share = "1"  Then
					if Len(intro_1_14) = 0 Or InStr(","& intro_1_14 &",",","& cateid &",")>0 Then
						s = "<a href='javascript:void(0)' onClick=""javascript:window.open('" & app.virpath & "work/content.asp?ord=" & app.base64.pwurl(ord) & "','newwin_gys','width=900,height=500,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & name & "</a>"
					else
						s = name
					end if
				end if
