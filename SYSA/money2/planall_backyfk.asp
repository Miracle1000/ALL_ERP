...
'if B="khmc" then
		elseif B="yfkgy" then
			Str_Result=Str_Result+ " and intro2 like '%"&C&"%' "
'elseif B="yfkgy" then
		elseif B="jsr" then
...
'elseif px=10 then
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "        </select>" & vbcrlf & "      </div></td>" & vbcrlf & "      <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "             <tr height=""35"">" & vbcrlf & "                        <td class='ser_btn2 resetHeadBg' colspan=""3"" height=""55"" valign=""middle"" align=""right"">" & vbcrlf & "                       "
	...
	if open_bz=1 Then
		sql7="select * from sortbank where del=1 and ( charindex(',"& session("personzbintel2007") &",',','+replace(cast(person as varchar(8000)),' ','')+',')>0 or person like '0') order by gate1 desc"
'if open_bz=1 Then
	else
		sql7="select * from sortbank where del=1 and bz=14 order by gate1 desc"
	end if
	rs88.open sql7,conn,1,1
	...
	Response.write ">供应商名称</option>" & vbcrlf & "                         <option value=""jsr"" "
	if B="jsr" then
		Response.write "selected"
	end if
	...
	if open_8_10=1 or open_8_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""EXCEL导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_gystyfk.asp'})}"" class=""anybutton""/>" & vbcrlf & "            "
	end if
	...
	Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "             </tr>" & vbcrlf & "      </table> " & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-top:-3px;"">" & vbcrlf & "      </form>" & vbcrlf & " <tr class=""top resetGroupTableBg"">" & vbcrlf &  "  <td width=""40"" align=""center""><div align=""center""><strong>选择</strong></div></td> "& vbcrlf &   "  <td width=""28%""><div align=""center"">供应商名称</div></td>" & vbcrlf &  "  <td width=""12%""><div align=""center""><strong>退款日期</strong></div></td> "& vbcrlf &  "   <td width=""12%""><div align=""center""><strong>退款金额</strong></div></td> "& vbcrlf &   "   <td width=""12%""><div align=""center""><strong>入账账户</strong></div></td> "& vbcrlf &  "   <td width=""8%""><div align=""center""><strong>经手人</strong></div></td> "& vbcrlf &      "  <td width=""10%""><div align=""center""><strong>概要</strong></div></td>" & vbcrlf & "   <td width=""12%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "        </tr>" & vbcrlf & ""
	dim n
	n=0
	summoney1=0
	money1_all=0
	set rs1=conn.execute("select isnull(sum(money1),0) from bankout3 "&Str_Result&" ")
	money1_all=cdbl(rs1(0).value)
	set rs1=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select (select name from tel where ord=bankout3.company) as companyname,(select sort3 from tel where ord=bankout3.company) as sort3,(select name from gate where ord=bankout3.cateid) as cateidname,bankout3.* from bankout3 "&Str_Result&" "&px_Result&" "
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""deleteallbackyf.asp?ord="
		...
		do until rs.eof
			...
			bank=rs("bank")
			sort3=rs("sort3")
			if bank="" or Isnull(bank) Then bank=0
			bankname = sdk.getSqlValue("select sort1 from sortbank where id="&bank&" and del=1 and ( charindex(',"& session("personzbintel2007") &",',','+replace(cast(person as varchar(8000)),' ','')+',')>0 or person like '0')","")
'if bank="" or Isnull(bank) Then bank=0
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td align=""center""><span class=""red"">" & vbcrlf & "        <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
			Response.write rs("id")
			Response.write """>" & vbcrlf & "          </span></td> " & vbcrlf & "      <td class=""name"">" & vbcrlf & "         "
			if (open_26_14=3 or CheckPurview(intro_26_14,trim(cateid))=True) and sort3="2" Then
				...
				Response.write "</a>"
				Response.write GetPrintInfo(conn, 44002 , id ,1)
			Elseif (open_1_14=3 or CheckPurview(intro_1_14,trim(cateid))=True) and sort3="1" Then
			...
			Response.write "" & vbcrlf & "      </td>" & vbcrlf & "   <td class=""name"" ><div align=""center"">"
			Response.write rs("date3")
			Response.write "</div></td>" & vbcrlf & "   <td class=""name""><div align=""right"">"
			Response.write  getbzflag(bank,"bankid")
			...
			Response.write "</div></td>" & vbcrlf & "           <td class=""func""> " & vbcrlf & "                        <div align=""center""><input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('contentbackyfk.asp?ord="
			...
			Response.write pwurl(rs("id"))
			if open_8_3=3 or (open_8_3=1 and CheckPurview(intro_8_3,trim(cateid)))=True then
				Response.write "" & vbcrlf & "                        <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Deletebackyf.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "&A="
		...
		loop
		m=n
		Response.write "       " & vbcrlf & "      <tr height=""27"">" & vbcrlf & "          <td colspan=""3"" align=""right"">本页合计：</td>" & vbcrlf & "               <td align=""right"">"
		Response.write Formatnumber(summoney1,num_dot_xs,-1)
		Response.write "</td>" & vbcrlf & "                <td colspan=""4"">&nbsp;</td>" & vbcrlf & "       </tr>" & vbcrlf & "   <tr height=""27"">" & vbcrlf & "          <td colspan=""3"" align=""right"">所有合计：</td>" & vbcrlf & "               <td align=""right"">"
		...
		if open_8_3=3 or open_8_3=1 then
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "        <input name=""currpage"" id=""currpage"" type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "      <input type=""button"" onClick=""gotourl('currPage='+document.getElementById('currpage').value);"" name=""Submit422"" value=""跳转""  class=""anybutton2""/>" & vbcrlf & "    "
		if currpage=1 then
	...
	action1="供应商退预付款列表"
	call close_list(1)
	Response.write "   " & vbcrlf & "  <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "    <tr> " & vbcrlf & "      <td height=""10"" colspan=""2""><img src=""../image/pixel.gif"" width=""1"" height=""1""></td>" & vbcrlf & "    </tr> " & vbcrlf & "        <tr>" & vbcrlf & "      <tdwidth=""16%"" height=""10""><div align=""right""></div></td>" & vbcrlf & "      <td width=""84%"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "     <tr>" & vbcrlf & "      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "     <tr>" & vbcrlf & "      <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen(divID){" & vbcrlf & " if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "  }else{" & vbcrlf &   "        divID.style.display="""" "& vbcrlf &  "   } "& vbcrlf &    "    divID.style.left=300; "& vbcrlf & "   divID.style.top=0; "& vbcrlf &" } "& vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:400;display:none;"">" & vbcrlf &" <table width=""150"" height="""
	if D<>"0" then
		Response.write "300"
	else
		Response.write "250"
	end if
	Response.write """  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按退款日期排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按退款日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">"& vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按供应商名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按供应商名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & ""
	if D<>"0" then
