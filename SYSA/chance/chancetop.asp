...
'bigsystemtype = ".mozi"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=3 and sort2=13"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_3_13=0
		intro_3_13=0
	else
		open_3_13=rs1("qx_open")
		intro_3_13=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=4 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_4_19=0
		intro_4_19=0
	else
		open_4_19=rs1("qx_open")
		intro_4_19=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=5 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_5_19=0
		intro_5_19=0
	else
		open_5_19=rs1("qx_open")
		intro_5_19=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=25 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_25_19=0
		intro_25_19=0
	else
		open_25_19=rs1("qx_open")
		intro_25_19=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=42 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_42_19=0
		intro_42_19=0
	else
		open_42_19=rs1("qx_open")
		intro_42_19=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=6 and sort2=19"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_6_19=0
		intro_6_19=0
	else
		open_6_19=rs1("qx_open")
		intro_6_19=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	set rs1=nothing
	ischancepool = InStr(1, request.ServerVariables("url"), "chancetop.asp", 1)  > 0
	if open_3_1=3 then
		list1=""
	elseif open_3_1=1 Then
		If ischancepool Then
			list1="and (cateid in ("&intro_3_1&") or isnull(cateid,0)=0)"
		else
			list1="and cateid in ("&intro_3_1&") and cateid>0"
		end if
	else
		If ischancepool then
			list1="and isnull(cateid,0)=0"
		else
			list1="and 1=2"
		end if
	end if
	if open_3_1=3 then
		Str_Result="where ((del=1 "&list1&") or (isnull(cateid,0)=0 and del=1) or ((share like '%"&session("personzbintel2007")&"%' or share='1') and cateid <>"&session("personzbintel2007")&" and del=1))"
		Str_Result2="and ((del=1 "&list1&")  or (isnull(cateid,0)=0 and del=1) or ((share like '%"&session("personzbintel2007")&"%' or share='1') and cateid <>"&session("personzbintel2007")&" and del=1))"
	else
		Str_Result="where ((del=1 "&list1&") or ((share like '%"&session("personzbintel2007")&"%' or share='1') and cateid <>"&session("personzbintel2007")&" and del=1))"
		Str_Result2="and ((del=1 "&list1&") or ((share like '%"&session("personzbintel2007")&"%' or share='1') and cateid <>"&session("personzbintel2007")&" and del=1))"
	end if
...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<!--[if IE 6]>  " & vbcrlf & "<style>" & vbcrlf & "html{_background-image:url(about:blank);_background-attachment:fixed;}" & vbcrlf & "</style>" & vbcrlf & "<![endif]-->" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "       /*#content tr.tophead {background-color:#C0CCDD;}*/" & vbcrlf & "    body {margin-top: 0px;}" & vbcrlf & " /*自定义固定行标题*/ " & vbcrlf & "   tr.tophead {left:0px;}" & vbcrlf & "  tr.tophead td {" & vbcrlf & "         border-right:0px solid #C0CCDD;" & vbcrlf & "         border-bottom:0px solid #C0CCDD;" & vbcrlf & "                border-top:0px solid #C0CCDD ;" & vbcrlf & "             padding:0px;" & vbcrlf & "    }" & vbcrlf & "       table.xiang td {background-color:transparent;color:#5b7cae;}" & vbcrlf & "    #content tr.top td{padding-bottom:8px!important}" & vbcrlf & "        .advanSearch{" & vbcrlf & "           background-color:#FFF!important;" & vbcrlf & "        }" & vbcrlf & "</style>" & vbcrlf & "" & vbcrlf & "<script src= ""../Script/ce_chancetop.js?ver="
	Response.write Application("sys.info.jsver")
	...
	dim W1,W2,W3,A1,A2,B,C,D,E,F,H,sort1,sort2
	A1=request("A1")
	A2=request("A2")
	B=request("B")
	D=request("D")
	...
	zdy5=request("zdy5")
	zdy6=request("zdy6")
	if (open_3_6=3 or open_3_6=1) or (B2<>"" and C2<>"") then
		Str_Result="where del=1"
	else
		Str_Result="where (del=1 and (isnull(cateid,0)=0 or isnull(cateid,0)=" & sdk.user & ") )"
	end if
	if G<>"" then
...
'case 1:
		case 2:
		Str_Result=Str_Result+"and  cateidfq in  ("&W3&")"
'case 2:
		case 3:
		Str_Result=Str_Result+"and  addcate in  ("&W3&")"
'case 3:
		case 4:
		Str_Result=Str_Result+"and  (cateid in ("&W3&") and cateid>0 and addcate in ("&W3&"))"
'case 4:
		case 5:
		Str_Result=Str_Result+"and  (cateidfq in  ("&W3&") and addcate in ("&W3&"))"
'case 5:
		end select
	end if
	area_list=-1
...
'if E<>"" then
	...
	if F<>"" then
		Str_Result=Str_Result+"and  complete2 in ("&F&")"
'if F<>"" then
	end if
	if A1<>"" then
		Str_Result=Str_Result+"and  sorce in ("&A1&")"
'if A1<>"" then
	end if
	if m3<>"" then
	...
	if m4<>"" then
		Str_Result=Str_Result &" and  date1<='"&cdate(m4)&" 23:59:59'"
	end if
	if m5<>"" then
		Str_Result=Str_Result &" and  date2>='"&m5&"'"
	end if
	if m6<>"" then
		Str_Result=Str_Result &" and  date2<='"&cdate(m6)&" 23:59:59'"
	end if
	if m1<>"" then
		Str_Result=Str_Result &" and  date7>='"&m1&"'"
	end if
	if m2<>"" then
		Str_Result=Str_Result &" and  date7<='"&cdate(m2)&" 23:59:59'"
	end if
	khid=request("khid")
	If khid&""<>"" Then
		Str_Result=Str_Result &" and (charindex(','+CAST ("&khid&" AS VARCHAR)+',',','+a.company+',')>0 or charindex(CAST ("&khid&" AS VARCHAR)+',',','+a.company+',')>0)"
'If khid&""<>"" Then
	end if
	kh_js=""
	If khmc<>"" Then
...
'if zdy6<>"" then
	...
	if C2<>"" then
		if B2="xmzt" then
			Str_Result=Str_Result+"and  title like '%"&C2&"%'"
'if B2="xmzt" then
		elseif B2="xmbh" then
			Str_Result=Str_Result+"and  xmid like '%"&C2&"%'"
'elseif B2="xmbh" then
		elseif B2="xmgy" then
			Str_Result=Str_Result+"and  intro like '%"&C2&"%'"
'elseif B2="xmgy" then
		elseif B2="zdy1" then
			Str_Result=Str_Result+"and  zdy1 like '%"&C2&"%'"
'elseif B2="zdy1" then
		elseif B2="zdy2" then
			Str_Result=Str_Result+"and  zdy2 like '%"&C2&"%'"
'elseif B2="zdy2" then
		elseif B2="zdy3" then
			Str_Result=Str_Result+"and  zdy3 like '%"&C2&"%'"
'elseif B2="zdy3" then
		elseif B2="zdy4" then
			Str_Result=Str_Result+"and  zdy4 like '%"&C2&"%'"
'elseif B2="zdy4" then
		end if
	end if
	Call searchExtended(3,"ord")
	if D<>"" then
		set rs=server.CreateObject("adodb.recordset")
	...
	sql="if not exists(select top 1 * from zdymx WITH(NOLOCK) where sort1=1011 and kd2="&session("personzbintel2007")&" and sort=1) "&_
	"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('申请人','lie_1',1,1,80,"&session("personzbintel2007")&",8,1011,1)"
	conn.execute sql
	sql="if not exists(select top 1 * from zdymx WITH(NOLOCK) where sort1=1011 and kd2="&session("personzbintel2007")&" and sort=2) "&_
	"insert into zdymx(title,name,sort,set_open,kd,kd2,gate1,sort1,sorce) values('每页显示','lie_2',2,1,80,"&session("personzbintel2007")&",8,1011,1)"
	conn.execute sql
	function get_Items(sid,tid)
		set rs8=conn.execute("select sorce from zdymx WITH(NOLOCK) where sort1="&sid&" and kd2="&session("personzbintel2007")&" and sort="&tid&" ")
		if not rs8.eof then
	...
	s_it1=get_Items(1011,1)
	s_it2=get_Items(1011,2)
	...
	if (slie_1=cdbl(s_it1) or lie_1=s_it1) and (slie_2=cdbl(s_it2) or lie_2=s_it2) then
		Flag_LockItems=true
	else
	...
	elseif px=3 then
		px_Result="order by xmid desc"
	elseif px=4 then
		px_Result="order by xmid asc"
	elseif px=5 then
		px_Result="order by money1 desc"
	elseif px=6 then
		px_Result="order by money1 asc"
	elseif px=7 then
		px_Result="order by money2 desc"
	elseif px=8 then
		px_Result="order by money2 asc"
	elseif px=9 then
		px_Result="order by date7 desc"
	elseif px=10 then
		px_Result="order by date7 asc"
	elseif px=11 then
		px_Result="order by date1 desc"
	elseif px=12 then
		px_Result="order by date1 asc"
	end if
	Response.write "" & vbcrlf & "<form action=""chancetop.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"" id=""gd1"" class=""zdy"" style=""z-index:9999"">" & vbcrlf & "<tr>" & vbcrlf & "      <td class=""place place42"">项目池</td>" & vbcrlf & "      <td>&nbsp;</td>" & vbcrlf & "      <td align=""right"" ><div id=""kh"" style='height:30px;line-height:30px;*padding-top:3px'>" & vbcrlf & "      <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();return false;""><u><font class=""advanSearch"">高级检索</font></u></a></div></td><td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>    " & vbcrlf & "  </table>" & vbcrlf & "      <span id=""ht1"" style=""padding:20px""></span>" & vbcrlf & " </form>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""0"" bgcolor=""transparent"" id=""content"" style=""margin-left:20px"">" & vbcrlf & "   <tr id=""gd2"" class=""zdy1 top tophead"">" & vbcrlf & "        <td align=""center"" width=""5%""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "     <td align=""center"" width=""10%""><div align=""center""><strong>项目编号</strong></div></td>" & vbcrlf & "       <td height=""27"" width=""18%""><div align=""center""><strong>项目名称</strong></div></td>" & vbcrlf & "          <td width=""8%""><div align=""center""><strong>领用情况</strong></div></td>" & vbcrlf & "      <td width=""8%""><div align=""center""><strong>领用人</strong></div></td>" & vbcrlf & "       <td width=""8%""><div align=""center""><strong>指派人</strong></div></td>" & vbcrlf & "       <td width=""7%""><div align=""center""><strong>领用日期</strong></div></td>" & vbcrlf & "    <td width=""15%""><div align=""center""><strong>" & vbcrlf & "            <select name=""select"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"" >" & vbcrlf & "            <option>-请选择-</option>" & vbcrlf & "            <option value=""lie_1=1"" "
	px_Result="order by date1 asc"
	if lie_1=1 then
		Response.write "selected"
	end if
	Response.write ">申请人</option>" & vbcrlf & "            <option value=""lie_1=2"" "
	if lie_1=2 then
		Response.write "selected"
	end if
	Response.write ">放弃人</option>" & vbcrlf & "            <option value=""lie_1=3"" "
	if lie_1=3 then
		Response.write "selected"
	end if
	Response.write ">放弃日期</option>" & vbcrlf & "            <option value=""lie_1=4"" "
	if lie_1=4 then
		Response.write "selected"
	end if
	Response.write ">放弃原因</option>            " & vbcrlf & "          </select>" & vbcrlf & "        </strong></div></td>" & vbcrlf & "          <td width=""20%""><div align=""center"" style=""white-space:nowrap;"">" & vbcrlf & "        <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "                          <option value=""lie_2=1"" "
	Response.write "selected"
	if lie_2=1 then
		Response.write "selected"
	end if
	Response.write ">每页显示10条</option>" & vbcrlf & "                         <option value=""lie_2=2"" "
	if lie_2=2 then
		Response.write "selected"
	end if
	Response.write ">每页显示20条</option>" & vbcrlf & "                         <option value=""lie_2=3"" "
	if lie_2=3  then
		Response.write "selected"
	end if
	Response.write ">每页显示30条</option>" & vbcrlf & "                         <option value=""lie_2=4"" "
	if lie_2=4  then
		Response.write "selected"
	end if
	Response.write ">每页显示50条</option>" & vbcrlf & "                         <option value=""lie_2=5"" "
	if lie_2=5  then
		Response.write "selected"
	end if
	Response.write ">每页显示100条</option>" & vbcrlf & "                        <option value=""lie_2=6"" "
	if lie_2=6  then
		Response.write "selected"
	end if
	Response.write ">每页显示200条</option>" & vbcrlf & "        </select>" & vbcrlf & "             &nbsp;<input type=""checkbox"" id=""LockItems"" onClick=""SaveItems(1011,"
	...
	sql2="select intro  from setchance  "
	rs2.open sql2,conn,1,1
	if rs2.eof then
		share=0
	else
		share=trim(rs2("intro"))
	end if
	rs2.close
	set rs2=nothing
	if share<>"" then
	...
	Str_Result = Str_Result & " and complete2 in ("&share&") "
	If request("spinfo") = "6" Then
		Dim cpower
		Set rs = conn.execute("select (case qx_open when 3 then '' when 0 then '-222' else qx_intro end) as r from power WITH(NOLOCK) where ord=" & session("personzbintel2007") & " and sort1=3 and sort2=1")
'Dim cpower
		If rs.eof = False Then
			cpower = rs("r").value
		else
		...
		rs.close
		Str_Result = " where exists(select 1 from chance x WITH(NOLOCK) where x.del=1 and x.ord=a.ord and isnull(x.cateid,0) = 0 and ((len('" & cpower & "')=0 and x.cateidfq > 0) or charindex(',' + cast(x.cateidfq as varchar(12)) +',','," & cpower & ",')>0))"
		cpower = rs("r").value
		If Len(W3) > 0 And Len(W3)<12 Then
			Str_Result = Str_Result & " and a.cateidfq in (" & W3 & ") "
		end if
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select *,(select color from sortjh2 where ord=a.complete2) as color from chance a WITH(NOLOCK) "&Str_Result&"  order by date7 desc"
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		Response.write "<table><tr><td  align='center' class='name' height='1138' valign='top'>没有信息!</td></tr></table>"
	else
		i=0
		...
		do until rs.eof
			...
			if open_3_3=3 or CheckPurview(intro_3_3,trim(cateid))=True or open_3_6=3 or CheckPurview(intro_3_6,trim(cateid))=True then
			...
			Response.write rs("xmid")
			Response.write "</td>" & vbcrlf & "    <td height=""27""><div align=""left"">&nbsp;"
			if open_3_14=3 or CheckPurview(intro_3_14,trim(cateid))=True then
				...
				Response.write "','newttwin','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,resizable=1,toolbar=0,left=100,top=100');return false;"">"
				Response.write pwurl(rs("ord"))
			end if
			...
			Response.write """>"
			Response.write""&k&""
			Response.write "</span></a></div></td>" & vbcrlf & "    <td>" & vbcrlf & "         <div align=""center"">" & vbcrlf & ""
			if rs("order1")="3" then
				Response.write "<font class=""red"">待审批</font>"
			elseif rs("order1")="2" or rs("cateid")<>"" then
				Response.write "<font class=""name"">已领用</font>"
			elseif rs("order1")="1" then
				Response.write "<font class=""name"">全公开</font>"
			else
				Response.write "未领用"
			end if
			...
			Response.write "</div></td>" & vbcrlf & "       <td class=""name""><div align='"
			if lie_1=4 then
				Response.write("left")
			else
			...
			if lie_1=1 then
				if cateid4&""<>"" then
					if cateid4>0 then
						set rs7 = conn.execute("select name from gate WITH(NOLOCK) where ord="& cateid4)
			...
			elseif lie_1=2 then
				if rs("cateidfq").value=-50 then
				elseif lie_1=2 then
					Response.write "系统"
				elseif not isnull(rs("cateidfq").value) then
					set rsfqname=conn.execute("select name from gate WITH(NOLOCK) where ord="&rs("cateidfq").value)
					if rsfqname.eof then
						fqname=""
					else
						fqname=rsfqname(0)
					end if
					rsfqname.close
					set rsfqname=nothing
					Response.write fqname
				end if
			elseif lie_1=3 then
				Response.write rs("datefq").value
			elseif lie_1=4 then
				if rs("sortfq")="1" then
					Response.write "领用未联系"
				elseif rs("sortfq")="2" then
					Response.write "间隔未联系"
				elseif rs("sortfq")="3" then
					Response.write "跟进未成功"
				elseif rs("sortfq")="6" then
					Response.write "主管收回"
				end if
			end if
			Response.write "</div>" & vbcrlf & "                </td>" & vbcrlf & "                <td class=""func""><div align=""center"">" & vbcrlf & "                          "
			if rs("order1")="3" then
				if open_3_6=3 or CheckPurview(intro_3_6,trim(cateid))=True or CheckPurview(intro_3_6,trim(cateid4))=True then
				...
				if open_3_6=3 or CheckPurview(intro_3_6,trim(cateid))=True then
					...
					Response.write pwurl(rs("ord"))
					if rs("order1")="2" or rs("cateid")<>"" then
						Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit"" value=""收回"" onClick=""if(confirm('确认收回？')){window.location.href='saveback.asp?"
						Response.write sdk.ReturnURL("ord=" & pwurl(rs("ord")) &"")
						Response.write "'}""  />" & vbcrlf & "        "
					end if
				end if
				if open_3_3=3 or CheckPurview(intro_3_3,trim(cateid))=True then
					Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Deletetop.asp?"
					Response.write sdk.ReturnURL("ord=" & rs("ord") &"")
					Response.write "'}""/>" & vbcrlf & "                             "
				end if
			end if
			...
			rs.movenext
		loop
		Response.write "" & vbcrlf & "      </table>" & vbcrlf & "         </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""8%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""checkbox2"" value=""Check All"" onClick=""mm();"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td width=""34%"" >        "
		...
		if open_3_6=1 or open_3_6=3 then
			Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit422"" value=""批量指派""  onclick=""ask();"" class=""anybutton2""/>" & vbcrlf & "                     <input type=""button"" name=""Submit426"" value=""批量收回""  onclick=""ask2();"" class=""anybutton2""/>" & vbcrlf & "                    <input type=""button"" name=""Submit452"" value=""批量审批""  onclick=""ask4();"" class=""anybutton2""/>" & vbcrlf & "             "
		end if
		...
		Response.write "条信息/页 "
		Response.write currpage
		Response.write "/"
		Response.write PageCount
		Response.write "页</span>&nbsp;" & vbcrlf & "     <input  size=3  type=text   name=""currPage"" id=""currPage"" maxlength=8 onkeyup=""checkDot('currpage',0,8);formatData(this,'int');"" >&nbsp;<input type=""submit"" name=""Submit422"" value=""跳转""  class=""anybutton2"" onclick=""gotourl('currPage='+document.getElementById('currPage').value);"" />" & vbcrlf & "          "
		if currpage=1 then
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " function ask() {" & vbcrlf & "document.all.form1.action = ""orderall.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """;" & vbcrlf & "document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function ask2() {" & vbcrlf & "document.all.form1.action = ""savebackall.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """;" & vbcrlf & "document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function ask3() {" & vbcrlf & "  document.all.form1.action = ""lingyong.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """;" & vbcrlf & "        document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function ask4() {" & vbcrlf & "  if($("":checkbox:checked[name=selectid]"").size()==0){alert(""您没有选择任何项目，请选择后再审批！"");return false;}" & vbcrlf & "    document.all.form1.action = ""shenpi.asp?"
		Response.write sdk.ReturnURL("")
		Response.write """;" & vbcrlf & "        document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "     var a = document.getElementsByTagName(""input"");" & vbcrlf & "   if(a[1].checked==true){" & vbcrlf & "         for (var i=1; i<a.length; i++){ " & vbcrlf & "                        if (a[i].type == ""checkbox""){ a[i].checked = false;}" & vbcrlf & "            }" & vbcrlf & "       }" & vbcrlf & "       else" & vbcrlf & "    {" & vbcrlf & "               for (var i=1; i<a.length; i++){" & vbcrlf & "                 if (a[i].type == ""checkbox""){ a[i].checked = true;}" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & " "
	end if
	rs.close
	set rs=nothing
	action1="项目池"
