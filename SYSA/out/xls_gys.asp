...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>供应商资料导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "       background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & " margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf &"    <td class=""place"">供应商资料导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出供应商资料,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "  "
	Response.write Application("sys.info.jsver")
	dim bh,W1,W2,W3,A1,A2,B,C,D,E,F,H
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	set rs1=nothing
	if open_26_10=3 then
		list=""
	elseif open_26_10=1 then
		list="and t.cateid in ("&intro_26_10&") "
	else
		list="and 1=0"
	end if
	Str_Result="where t.del=1 and t.sort3=2 "&list&""
	W1=request("W1")
	W2=request("W2")
	W3=request("W3")
	A2=request("A2")
	...
	W4=replace(W3,"0","")
	W4=replace(W4,",","")
	Str_Result=Str_Result+" and isnull(t.order1,0)<>4 "
	W4=replace(W4,",","")
	if W4<>"" then
		Str_Result=Str_Result+"and  t.cateid in  ("&W3&")"
'if W4<>"" then
	ElseIf request("w3")="0" Then
		Str_Result=Str_Result+" and isnull(t.cateid,0)=0 "
'ElseIf request("w3")="0" Then
	ElseIf request("w2")="0" Then
		Str_Result=Str_Result+" and isnull(t.cateid3,0)=0 "
'ElseIf request("w2")="0" Then
	ElseIf request("w1")="0" Then
		Str_Result=Str_Result+" and isnull(t.cateid2,0)=0 "
'ElseIf request("w1")="0" Then
	end if
	area_list=-1
'ElseIf request("w1")="0" Then
...
'ElseIf request("w1")="0" Then
	...
	if  A2<>"" Then
		If A2 = "-1" Then
'if  A2<>"" Then
			Str_Result=Str_Result+"and  isnull(t.area,0) not in  (select id from menuarea)"
'if  A2<>"" Then
		else
			Str_Result=Str_Result+"and  t.area in ("&area_list&")"
'if  A2<>"" Then
		end if
	end if
	if D<>"" then
		Str_Result=Str_Result+"and  isnull(t.trade,0) in  ("&D&")"
'if D<>"" then
	end if
	if A3<>"" then
		Str_Result=Str_Result+"and  t.jz in  ("&A3&")"
'if A3<>"" then
	end if
	if E<>"" then
		Str_Result=Str_Result+"and  t.ly in  ("&E&")"
'if E<>"" then
	end if
	if F<>"" then
		Str_Result=Str_Result+"and  t.sort1 in  ("&F&")"
'if F<>"" then
	end if
	if m1<>"" then
		Str_Result=Str_Result+"and  t.date1>='"&m1&" 00:00:00'"
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+"and  t.date1<='"&m2&" 23:59:59'"
'if m2<>"" then
	end if
	if m3<>"" then
		Str_Result=Str_Result+"and  t.date2>='"&m3&" 00:00:00'"
'if m3<>"" then
	end if
	if m4<>"" then
		Str_Result=Str_Result+"and  t.date2<='"&m4&" 23:59:59'"
'if m4<>"" then
	end if
	if H="3" then
		if Str_Result="" then
			str_Result="where  t.x='2'"
		else
			Str_Result=Str_Result+"and  t.x='2'"
			str_Result="where  t.x='2'"
		end if
	elseif H="4" then
		if Str_Result="" then
			str_Result="where  t.h='2'"
		else
			Str_Result=Str_Result+"and  t.h='2'"
			str_Result="where  t.h='2'"
		end if
	elseif H="5" then
		if Str_Result="" then
			str_Result="where t.datealt<>''"
		else
			Str_Result=Str_Result+"and  t.datealt<>''"
			str_Result="where t.datealt<>''"
		end if
	elseif H="6" then
		if Str_Result="" then
			str_Result="where  t.profect1=1"
		else
			Str_Result=Str_Result+"and  t.profect1=1"
			str_Result="where  t.profect1=1"
		end if
	elseif H="7" then
		str_Result="where  t.share<>''  and t.cateid ="&session("personzbintel2007")&" and t.del=1"
	elseif H="8" then
		str_Result="where  (t.share like '%"&session("personzbintel2007")&"%' or t.share='1') and t.cateid <>"&session("personzbintel2007")&" and t.del=1"
	end if
	Response.write("[F2="&F2&"]")
	if F2<>"" then
		if F1=1 then
			str_Result=str_Result+"and t.name like '%"& F2 &"%'"
'if F1=1 then
		elseif F1=2 then
			str_Result=str_Result+"and t.name not like '%"& F2 &"%'"
'elseif F1=2 then
		elseif F1=3 then
			str_Result=str_Result+"and t.name='"&F2&"'"
'elseif F1=3 then
		elseif F1=4 then
			str_Result=str_Result+"and t.name<>'"&F2&"'"
'elseif F1=4 then
...
'elseif F1=6 then
	...
	if S2<>"" then
		if S1=1 then
			str_Result=str_Result+"and t.pym like '%"& S2 &"%'"
'if S1=1 then
		elseif S1=2 then
			str_Result=str_Result+"and t.pym not like '%"& S2 &"%'"
'elseif S1=2 then
		elseif S1=3 then
			str_Result=str_Result+"and t.pym='"&S2&"'"
'elseif S1=3 then
		elseif S1=4 then
			str_Result=str_Result+"and t.pym<>'"&S2&"'"
'elseif S1=4 then
...
'elseif S1=6 then
	...
	if G2<>"" then
		if G1=1 then
			str_Result=str_Result+"and t.phone like '%"& G2 &"%'"
'if G1=1 then
		elseif G1=2 then
			str_Result=str_Result+"and t.phone not like '%"& G2 &"%'"
'elseif G1=2 then
		elseif G1=3 then
			str_Result=str_Result+"and t.phone='"& G2 &"'"
'elseif G1=3 then
		elseif G1=4 then
			str_Result=str_Result+"and t.phone<>'"& G2 &"'"
'elseif G1=4 then
...
'elseif G1=6 then
	...
	if P2<>"" then
		if P1=1 then
			str_Result=str_Result+"and t.fax like '%"& P2 &"%'"
'if P1=1 then
		elseif P1=2 then
			str_Result=str_Result+"and t.fax not like '%"& P2 &"%'"
'elseif P1=2 then
		elseif P1=3 then
			str_Result=str_Result+"and t.fax='"& P2 &"'"
'elseif P1=3 then
		elseif P1=4 then
			str_Result=str_Result+"and t.fax<>'"& P2 &"'"
'elseif P1=4 then
...
'elseif P1=6 then
	...
	if I2<>"" then
		if I1=1 then
			str_Result=str_Result+"and t.url like '%"& I2 &"%'"
'if I1=1 then
		elseif I1=2 then
			str_Result=str_Result+"and t.url not like '%"& I2 &"%'"
'elseif I1=2 then
		elseif I1=3 then
			str_Result=str_Result+"and t.url='"& I2 &"'"
'elseif I1=3 then
		elseif I1=4 then
			str_Result=str_Result+"and t.url<>'"& I2 &"'"
'elseif I1=4 then
...
'elseif I1=6 then
	...
	if J2<>"" then
		if J1=1 then
			str_Result=str_Result+"and t.address like '%"& J2 &"%'"
'if J1=1 then
		elseif J1=2 then
			str_Result=str_Result+"and t.address not like '%"& J2 &"%'"
'elseif J1=2 then
		elseif J1=3 then
			str_Result=str_Result+"and t.address='"& J2 &"'"
'elseif J1=3 then
		elseif J1=4 then
			str_Result=str_Result+"and t.address<>'"& J2 &"'"
'elseif J1=4 then
...
'elseif J1=6 then
	...
	if K2<>"" then
		if K1=1 then
			str_Result=str_Result+"and t.zip like '%"& K2 &"%'"
'if K1=1 then
		elseif K1=2 then
			str_Result=str_Result+"and t.zip not like '%"& K2 &"%'"
'elseif K1=2 then
		elseif K1=3 then
			str_Result=str_Result+"and t.zip='"& K2 &"'"
'elseif K1=3 then
		elseif K1=4 then
			str_Result=str_Result+"and t.zip<>'"& K2 &"'"
'elseif K1=4 then
...
'elseif K1=6 then
	...
	if T2<>"" then
		if T1=1 then
			str_Result=str_Result+"and t.intro like '%"& T2 &"%'"
'if T1=1 then
		elseif T1=2 then
			str_Result=str_Result+"and t.intro not like '%"& T2 &"%'"
'elseif T1=2 then
		elseif T1=3 then
			str_Result=str_Result+"and t.intro='"& T2 &"'"
'elseif T1=3 then
		elseif T1=4 then
			str_Result=str_Result+"and t.intro<>'"& T2 &"'"
'elseif T1=4 then
...
'elseif T1=6 then
	...
	if H2<>"" then
		if H1=1 Then
			str_Result=str_Result+" and t.khid like '%"& H2 &"%'"
'if H1=1 Then
		end if
	end if
	if H_2<>"" then
		if H_1=1 Then
			str_Result=str_Result+" and t.email like '%"& H_2 &"%'"
'if H_1=1 Then
		end if
	end if
	if H_4<>"" then
		if H_3=1 Then
			str_Result=str_Result+" and t.ord in (select company from person where sort3=2 and name like '%"& H_4 &"%')"
'if H_3=1 Then
		end if
	end if
	if H_6<>"" then
		if H_5=1 Then
			str_Result=str_Result+" and t.ord in (select company from person where sort3=2 and phone like '%"& H_6 &"%' or phone2 like '%"& H_6 &"%')"
'if H_5=1 Then
		end if
	end if
	if H_8<>"" then
		if H_7=1 Then
			str_Result=str_Result+" and t.ord in (select company from person where sort3=2 and mobile like '%"& H_8 &"%' or mobile2 like '%"& H_8 &"%')"
'if H_7=1 Then
		end if
	end if
	if X2<>"" then
		if X1=1 then
			str_Result=str_Result+"and t.product like '%"& X2 &"%'"
'if X1=1 then
		elseif X1=2 then
			str_Result=str_Result+"and t.product not like '%"& X2 &"%'"
'elseif X1=2 then
		elseif X1=3 then
			str_Result=str_Result+"and t.product='"& X2 &"'"
'elseif X1=3 then
		elseif X1=4 then
			str_Result=str_Result+"and t.product<>'"& X2 &"'"
'elseif X1=4 then
...
'elseif X1=6 then
	...
	if X4<>"" then
		if X3=1 then
			str_Result=str_Result+"and t.c2 like '%"& X4 &"%'"
'if X3=1 then
		elseif X3=2 then
			str_Result=str_Result+"and t.c2 not like '%"& X4 &"%'"
'elseif X3=2 then
		elseif X3=3 then
			str_Result=str_Result+"and t.c2='"& X4 &"'"
'elseif X3=3 then
		elseif X3=4 then
			str_Result=str_Result+"and t.c2<>'"& X4 &"'"
'elseif X3=4 then
...
'elseif X3=6 then
	...
	if X6<>"" then
		if X5=1 then
			str_Result=str_Result+"and t.c3 like '%"& X6 &"%'"
'if X5=1 then
		elseif X5=2 then
			str_Result=str_Result+"and t.c3 not like '%"& X6 &"%'"
'elseif X5=2 then
		elseif X5=3 then
			str_Result=str_Result+"and t.c3='"& X6 &"'"
'elseif X5=3 then
		elseif X5=4 then
			str_Result=str_Result+"and t.c3<>'"& X6 &"'"
'elseif X5=4 then
...
'elseif X5=6 then
	...
	if X8<>"" then
		if X7=1 then
			str_Result=str_Result+"and t.c4 like '%"& X8 &"%'"
'if X7=1 then
		elseif X7=2 then
			str_Result=str_Result+"and t.c4 not like '%"& X8 &"%'"
'elseif X7=2 then
		elseif X7=3 then
			str_Result=str_Result+"and t.c4='"& X8 &"'"
'elseif X7=3 then
		elseif X7=4 then
			str_Result=str_Result+"and t.c4<>'"& X8 &"'"
'elseif X7=4 then
...
'elseif X7=6 then
	...
	Str_Result=Str_Result+"and  t.sort3=2"
'elseif X7=6 then
	...
	If aboutSupplier_<>"" Then Str_Result=Str_Result&" and t.product like '%"&aboutSupplier_&"%' "
	If cooperation_<>"" Then Str_Result=Str_Result&" and t.c2 like '%"&cooperation_&"%' "
	If prospect_<>"" Then Str_Result=Str_Result&" and t.c3 like '%"&prospect_&"%' "
	If strategy_<>"" Then Str_Result=Str_Result&" and t.c4 like '%"&strategy_&"%' "
	If  ZBRuntime.MC(201203) Then Call searchExtended(26,"t.ord")
	...
	if px=1 then
		px_Result="order by t.date1 desc"
	elseif px=2 then
		px_Result="order by t.date1 asc"
	elseif px=3 then
		px_Result="order by t.name desc"
	elseif px=4 then
		px_Result="order by t.name asc"
	elseif px=5 then
		px_Result="order by t.khid desc"
	elseif px=6 then
		px_Result="order by t.khid asc"
	elseif px=7 then
		px_Result="order by t.jz desc"
	elseif px=8 then
		px_Result="order by t.jz asc"
	elseif px=9 then
		px_Result="order by t.datealt desc"
	elseif px=10 then
		px_Result="order by t.datealt asc"
	end if
	Chtml=""&_
	"<td style='font-weight:bold'>供应商编号</td>"&_
	"<td style='font-weight:bold'>供应商名称</td>"&_
	"<td style='font-weight:bold'>供应商分类</td>"&_
	"<td style='font-weight:bold'>供应商级别</td>"&_
	"<td style='font-weight:bold'>采购人员</td>"&_
	"<td style='font-weight:bold'>添加时间</td>"&_
	"<td style='font-weight:bold'>电话</td>"&_
	"<td style='font-weight:bold'>传真</td>"&_
	"<td style='font-weight:bold'>电子邮件</td>"&_
	"<td style='font-weight:bold'>邮编</td>"&_
	"<td style='font-weight:bold'>所在区域</td>"&_
	"<td style='font-weight:bold'>所在行业</td>"&_
	"<td style='font-weight:bold'>供应商网址</td>"&_
	"<td style='font-weight:bold'>供应商地址</td>"&_
	"<td style='font-weight:bold'>法人代表</td>"&_
	"<td style='font-weight:bold'>注册资本</td>"&_
	"<td style='font-weight:bold'>开户银行1</td>"&_
	"<td style='font-weight:bold'>开户名称1</td>"&_
	"<td style='font-weight:bold'>银行行号1</td>"&_
	"<td style='font-weight:bold'>银行账号1</td>"&_
	"<td style='font-weight:bold'>税号1</td>"&_
	"<td style='font-weight:bold'>地址1</td>"&_
	"<td style='font-weight:bold'>电话1</td>"&_
	"<td style='font-weight:bold'>开户银行2</td>"&_
	"<td style='font-weight:bold'>开户名称2</td>"&_
	"<td style='font-weight:bold'>银行行号2</td>"&_
	"<td style='font-weight:bold'>银行账号2</td>"&_
	"<td style='font-weight:bold'>税号2</td>"&_
	"<td style='font-weight:bold'>地址2</td>"&_
	"<td style='font-weight:bold'>电话2</td>"&_
	"<td style='font-weight:bold'>账期</td>"&_
	"<td style='font-weight:bold'>结算日期</td>"&_
	"<td style='font-weight:bold'>主联系人</td>"&_
	"<td style='font-weight:bold'>性别</td>"&_
	"<td style='font-weight:bold'>部门</td>"&_
	"<td style='font-weight:bold'>职务</td>"&_
	"<td style='font-weight:bold'>办公电话</td>"&_
	"<td style='font-weight:bold'>手机</td>"&_
	"<td style='font-weight:bold'>电子邮件</td>"&_
	"<td style='font-weight:bold'>QQ</td>"&_
	"<td style='font-weight:bold'>MSN</td>"&_
	"<td style='font-weight:bold'>洽谈进展</td>"&_
	"<td style='font-weight:bold'>供应商简介</td>"&_
	"<td style='font-weight:bold'>合作现状</td>"&_
	"<td style='font-weight:bold'>合作前景</td>"&_
	"<td style='font-weight:bold'>跟进策略</td>"&_
	"<td style='font-weight:bold'>备注</td>"
	If ZBRuntime.MC(201203) Then
		csql="select fname from ERP_CustomFields where isUsing=1 and canexport=1 and tname=26 and del=1 order by id"
		Set crs=conn.execute(csql)
		do until crs.eof=True
			Chtml=Chtml&"<td style='font-weight:bold'>"&crs("fname").value&"</td>"
'do until crs.eof=True
			crs.movenext
		loop
		crs.close
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("供应商资料")
	response.Flush()
	xsheet.writeHTML Chtml
	set rs=server.CreateObject("adodb.recordset")
	sql="select t.*,s1.sort1 as jzname,s2.sort1 as lyname,"&_
	"m.menuname as menuname,s3.sort1 as tradename,"&_
	"p.name as pname,p.sex,p.part1,p.job,p.phone as phonenum,"&_
	"p.fax as faxnum,p.mobile,p.email pemail,p.qq,p.msn,g.name as cateidname "&_
	"from tel t "&_
	"left join sortonehy s1 on s1.ord=t.jz "&_
	"left join sortonehy s2 on s2.ord=t.ly "&_
	"left join menuarea m on m.id=t.area "&_
	"left join sortonehy s3 on s3.ord=t.trade "&_
	"left join person p on p.ord=t.person "&_
	"left join gate g on g.ord=t.cateid "&Str_Result&"  "&px_Result&" "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim n
	n=0
	do until rs.eof
		dim name,khid,sort1,sort2,jz,ly,area,trade,preson,sex,part1,job,phone,fax,mobile,email,url,address,zip,product,c2,c3,c4,intro,intro2,cateid,date7,fkdays,fkdate
		name=rs("name")
		fkdays=rs("fkdays")
		fkdate=rs("fkdate")
		khid=rs("khid")
		...
		qq=rs("qq")
		msn=rs("msn")
		If phone&""="" Then  phone=rs("phone")
		If fax&""="" Then fax=rs("fax")
		url=rs("url")
		address=rs("address")
		zip=rs("zip")
		faren=rs("faren")
		...
		intro=rs("intro")
		intro2=""
		set rs1=conn.execute("select top 1 intro,date7 from reply  where ord="&rs("ord")&" and id1 is null and del=1 order by date7 asc")
		do until rs1.eof
			intro_tmp=rs1("intro")&""
			intro_tmp=replace(intro_tmp,"<br>","{vbcrlf}")
			intro_tmp=HTMLDecode(intro_tmp)
			intro_tmp=replace(intro_tmp,"{vbcrlf}","<br>")
			if intro2="" then
				intro2=intro2&intro_tmp&"："&rs1("date7")
			else
				intro2=intro2&"<BR>"&intro_tmp&"："&rs1("date7")
			end if
			rs1.movenext
		loop
		rs1.close
		...
		date7=rs("date1")
		If intro&""<>"" Then intro=replaceTab(intro)
		If product&""<>"" Then product=replaceTab(product)
		If c2&""<>"" Then c2=replaceTab(c2)
		If c3&""<>"" Then c3=replaceTab(c3)
		If c4&""<>"" Then c4=replaceTab(c4)
		rowHTML=""&_
		"<td>"&khid&"&nbsp;</td>"&_
		"<td>"&name&"</td>"&_
		"<td>"&ly&"</td>"&_
		...
		"<td>"&c3&"</td>"&_
		"<td>"&c4&"</td>"&_
		"<td>"&Replacehtml(HTMLDecode(intro))&"</td>"
		If  ZBRuntime.MC(201203) Then
			set rs2=conn.execute("select * from ERP_CustomFields f left join ERP_CustomValues v on v.FieldsID=f.id and v.OrderID="& rs("ord") &" where TName=26 and IsUsing=1 and CanExport=1 order by F.id ")
			do until rs2.eof
				c_Value=rs2("FValue")
				rowHTML=rowHTML&"<td>"&c_Value&"</td>"
				rs2.movenext
			loop
			rs2.close
		end if
		...
		If rowIndex > 40000 Then
			If pageCount = 1 then
				xsheet.title = "供应商资料(" & pageCount & "页)"
			end if
			pageCount = pageCount + 1
			xsheet.title = "供应商资料(" & pageCount & "页)"
			Set xsheet = xApp.sheets.add("供应商资料(" & pageCount & "页)")
			rowIndex = 0
			xsheet.writeHTML Chtml
		end if
	...
	rs.close
	set rs=Nothing
	tfile=Server.MapPath("供应商资料_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'供应商资料_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="供应商资料导出"
	call close_list(1)
	...
	Public Function replaceTab(introStr)
		replaceTab=Replace(Replace(Replace(Replace(Replace(Replace(Replace(introStr,"<DIV","<span"),"</DIV>","</span>"),"<BR>"," "),"<P>",""),"<IMG","<span"),"<br>"," "),"</P>","")
	end function
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的供应商资料</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
