...
'If Not rs.Eof Then
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"" src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "</head>" & vbcrlf & "<body  >" & vbcrlf & ""
	if action="update" then
		title=trim(request.Form("title"))
		sortid=trim(request.Form("sortid"))
		content=trim(request.Form("content"))
		isdefault=trim(request.Form("isdefault"))
		Dtype=trim(request.QueryString("type"))
		if title="" then call jsBack(inName1&"不能为空")
		if sortid="" then call jsBack(inName2&"不能为空")
		if isdefault="" then call jsBack(inName3&"不能为空")
		if content=""  then call jsBack(inName4&"不能为空")
		if Dtype="add" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from sms_temp  where title='"&title&"' and del=1"
			rs.open sql,conn,1,1
			if rs.eof then
				if isdefault="1" then
					conn.execute "update sms_temp set isdefault=0 where sortid="&sortid&""
				end if
				conn.execute "insert into sms_temp (title,sortid,isdefault,content,del,addcate) values('"&title&"',"&sortid&","&isdefault&",'"&content&"',1,"&session("personzbintel2007")&")"
				call jsLocat("添加成功","sms_temp_list.asp")
			else
				call jsBack(title&"已存在")
			end if
			rs.close
			set rs=nothing
		elseif Dtype="save" and isnumeric(ord) then
			...
			conn.execute "update sms_temp set title='"&title&"',sortid="&sortid&",isdefault="&isdefault&",content='"&content&"' where ord="&ord&" and del=1"
			call jsLocat("修改成功","sms_temp_list.asp")
		end if
	elseif action="edit" and isnumeric(ord) then
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from sms_temp  where ord="&ord&" and del=1"
		rs.open sql,conn,1,1
		if not rs.eof then
			title=rs("title")
			sortid=rs("sortid")
		...
		rs.close
		set rs=nothing
	elseif action="del" and isnumeric(ord) and ord<>"" then
		conn.execute "delete sms_temp  where ord="&ord&""
		call jsLocat("删除成功","sms_temp_list.asp")
	end if
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">短信提醒模板修改</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">短信提醒模板设置</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <form method=""post"" name=""demo""  action=""sms_temp_edit.asp?action=update"" id=""demo"" >" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
	Response.write inName1
	Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input name=""title"""
	if action="edit" then
		Response.write " readonly"
	end if
	Response.write "  type=""text"" id=""title""  value="""
	Response.write title
	Response.write """ size=""23"" maxlength=""50""   dataType=""Limit""  onkeyup=""value=value.replace(/[\=\']/g,'')"" min=""1"" max=""15""  msg=""必填"">" & vbcrlf & "              （标题不能修改）" & vbcrlf & "              <span class=""red"">*</span>" & vbcrlf & "              </td>" & vbcrlf & "          </tr>" & vbcrlf & "                                                               <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
	Response.write inName3
	Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input type=""radio"" name=""isdefault"" value=""1"" "
	if  isdefault="1" then
		Response.write " checked"
	end if
	Response.write ">是" & vbcrlf & "              " & vbcrlf & "                <input name=""isdefault"" type=""radio"" value=""0"" "
	if isdefault="" or isdefault="0" then
		Response.write " checked"
	end if
	Response.write ">否" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
	Response.write inName2
	Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><select name=""sortid"" onChange=""getReplaceStr(this.value)"" size=""1"" id=""sortid"">" & vbcrlf & "              <option value="""" "
	if sortid="" or isnumeric(sortid)=false then
		Response.write "selected"
	end if
	Response.write ">请选择</option>" & vbcrlf & "                      "
	Dim wxGoods,gdzc
	wxGoods = ZBRuntime.MC(76000)
	gdzc = ZBRuntime.MC(51004)
	set rs3=server.CreateObject("adodb.recordset")
	sql3="select * from sms_temp_sort where del=1 AND (ord <> 9 OR (ord = 9 AND '"& wxGoods &"' = 'true') ) AND (ord <> 10 OR (ord = 10 AND '"& gdzc &"' = 'true') )"
	...
	rs3.close
	set rs3=nothing
	Response.write "" & vbcrlf & "            </select>" & vbcrlf & "            <span class=""red""> *</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "                                                 <tr id=""replace"">" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
	Response.write inName5
	Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name"">" & vbcrlf & "                                             <div id=""strList"">" & vbcrlf & "                                        "
	Response.write replaceStr(sortid)
	Response.write "" & vbcrlf & "                                             </div>" & vbcrlf & "                          <script language=""javascript"" type=""text/javascript"">" & vbcrlf & "                               <!--" & vbcrlf & "                                function getReplaceStr(sortid) {" & vbcrlf & "                                    $.ajax({" & vbcrlf & "                                            url: ""ajax_sms_replace.asp?sortid="" + sortid + ""&r="" + Math.random()," & vbcrlf & "                                          type: ""get""," & vbcrlf & "                                      dataType: ""html""," & vbcrlf & "                                         success: function (data) {" & vbcrlf & "                                          document.getElementById(""strList"").innerHTML = data;" & vbcrlf & "                                              //$(""#strList"").html(data);" & vbcrlf & "                                          }" & vbcrlf & "                                   });" & vbcrlf & "                                 }" & vbcrlf & "                           -->" & vbcrlf & "                             </script>" & vbcrlf & "                                               </td>" & vbcrlf & "          </tr>" & vbcrlf & "            " & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
	Response.write inName4
	Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><textarea name=""content"" cols=""60"" rows=""15"" id=""content"">"
	Response.write content
	Response.write "</textarea>" & vbcrlf & "            <span class=""red"">*</span>&nbsp;&nbsp;<br>(合同待审批提醒，短信回复格式是“Y/N,下级审批人id,审批意见”,如没有下级审批人，可直接回复“Y/N”)</td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">&nbsp;</td>" & vbcrlf & "            <td  height=""26""  class=""name"">"
	if action="edit" then
		Response.write "" & vbcrlf & "              <input name=""button22"" class=""anybutton2"" onClick=""javascript:document.all.demo.action='sms_temp_edit.asp?action=update&type=save&ord="
		Response.write pwurl(ord)
		Response.write "'""  value=""保  存"" type=""submit"">" & vbcrlf & "              "
	else
		Response.write "" & vbcrlf & "              <input name=""button23"" class=""anybutton2"" value=""保  存""  onClick=""javascript:document.all.demo.action='sms_temp_edit.asp?action=update&type=add'"" type=""submit"">" & vbcrlf & "              "
	end if
	Response.write "" & vbcrlf & "              &nbsp;&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "        </form>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""1130"" ><div align=""center""></div></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "        "
	function replaceStr(sortid)
		if sortid<>"" and isnumeric(sortid) then
			set rs3=server.CreateObject("adodb.recordset")
			sql3="select * from sms_replace_str where del=1 and (sortid="&sortid&" or sortid=0)"
			rs3.open sql3,conn,1,1
			if not rs3.eof then
				replacelist=""
				do while not rs3.eof
					replacelist=replacelist&"<span class=""replace"" title=""单击复制参数"" id=""replace"&rs3("ord")&""" onclick=""copyClick(this.id)"">"&rs3("title")&"="&rs3("replaceStr")&"</span>&nbsp;&nbsp;"
					rs3.movenext
				loop
			end if
			rs3.close
			...
			Dim rs5
			If sortid="5" Then
				Set rs5=conn.execute("select title,name from zdy where sort1=33 and set_open=1")
				Do While rs5.eof=False
					replacelist=replacelist&"<span class=""replace"" title=""单击复制参数"" id=""send_"&rs5("name")&""" onclick=""copyClick(this.id)"">"&rs5("title")&"=$send_"&rs5("name")&"</span>&nbsp;&nbsp;"
					rs5.movenext
				Loop
				rs5.close
				set rs5=Nothing
