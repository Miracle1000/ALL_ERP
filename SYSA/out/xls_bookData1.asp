...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlApplication.ActiveSheet.Columns(26).ColumnWidth=20
	xlApplication.ActiveSheet.Columns(26).HorizontalAlignment=3
	xlWorksheet.Cells(1,1).Value = "书名"
	...
	set rs1=nothing
	Str_Result=""
	if open_103_10=3 then
		list=""
	elseif open_103_10=1 then
		list=" and bk_addcateid in ("&intro_103_10&")"
	else
		list=" and bk_addcateid=0"
	end if
	Str_Result=" where 1=1 "&list&""
	...
	if open_103_1=3 then
	elseif open_103_1=1 then
		Str_Result=Str_Result&" and bk_addcateid in ("&intro_103_1&")"
	else
		Str_Result=Str_Result&" and bk_addcateid=0"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
...
'if Exit Sub2<>"" then
	...
	i=1
	sql="select a.*,b.set_name,c.name from O_regbook_his a left join O_bookset b on a.bk_type=b.id left join gate c on a.bk_updateid=c.ord "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "图书修改记录_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			bk_pubtime=rs("bk_pubtime")
			addtime=rs("bk_addtime")
			bk_updatetime=rs("bk_updatetime")
			sql8="select name from gate where ord="&ml.isNum(rs("bk_addcateid"))
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>图书修改记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!图书修改记录导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("图书修改记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的图书修改记录</u></strong></font></a></p>"
