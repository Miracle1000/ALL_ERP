...
'elseif B="thry" then
			...
			elseif open_76_10=1 then
				str_Result11=str_Result11&" and a.cateid in ("&intro_76_10&")"
			else
			...
			"(select name from tel where del=1 and ord=a.company) as name, "&_
			"(select intro from sortbz where id=a.bz) as sortbz, "&_
			...
			"from payout3 as a left join caigouth as b on a.caigouth=b.ord where a.del=1 "&Str_Result11&" group by a.company,a.bz "&px_Result&""
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "采购退款汇总表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
...
'if cateid="" or cateid=0 then
					...
					elseif sort3=2 and (open_26_14=3 or CheckPurview(intro_26_1,trim(cateid))=True) then
					else
						companyname=""
					end if
					...
					xlWorksheet.Cells(1+i,1).Value = companyname & delname
					cateidname=""
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>采购退款汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!采购退款汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("采购退款汇总表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的采购退款汇总表</u></strong></font></a></p>"
