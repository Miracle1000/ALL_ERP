...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	j=9
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,name,sort,gl from zdy where sort1=802 and set_open=1 and dc=1 order by gate1 asc "
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
	...
	j=9
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,title,sort,gl from zdy where sort1=802 and set_open=1 and dc=1 order by gate1 asc "
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
	...
	rs1.close
	set rs1=nothing
	xlWorksheet.Cells(1,j).Value = "备注"
	xlWorksheet.Cells(1,j).font.Size=10
	xlWorksheet.Cells(1,j).font.bold=true
	...
	elseif open_101_10=1 then
		list=" and prod_addcateid in ("&intro_101_10&")"
	else
		list=" and prod_addcateid=0"
	end if
	Str_Result=" where prod_del=1 "&list&""
	...
	elseif open_101_15=1 then
		Str_Result=Str_Result&" and prod_addcateid in ("&intro_101_15&")"
	else
		Str_Result=Str_Result&" and prod_addcateid=0"
	end if
	Ptr_Result=""
	px=request("px")
...
'if Exit Sub4 <>"" and not isnull(ret4) then
	...
	i=1
	sql="select * from O_product a left join O_officeType b on a.prod_type=b.id  "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "档案列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			prod_type=rs("prod_type")
			if prod_type="" or isnull(prod_type) then prod_type=0
			sql7="select * from O_officeType where id="&prod_type
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				pro_type1=rs7("pro_name")
			end if
			set rs7=nothing
			sql7="select * from gate where ord="&rs("prod_addcateid")
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				addname=rs7("name")
			end if
			set rs7=nothing
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,1).Value = rs("prod_name")
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,1).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,2).Value = rs("prod_bh")
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,2).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,3).Value = rs("prod_xh")
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,3).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,4).Value = pro_type1
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,4).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,5).Value = rs("prod_less")
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,5).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,6).Value = rs("prod_more")
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,6).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,7).Value = addname
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,7).font.Size=10
			prod_addtime=rs("prod_addtime")
			xlWorksheet.Cells(1+i,8).Value =prod_addtime
			prod_addtime=rs("prod_addtime")
			...
			rs1.close
			set rs1=nothing
			xlWorksheet.Cells(1+i,j).Value = RegReplace(rs("prod_note"),"<[^>]+>","")
			set rs1=nothing
			xlWorksheet.Cells(1+i,j).font.Size=10
			set rs1=nothing
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>档案列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!档案列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("档案列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的档案列表</u></strong></font></a></p>"
