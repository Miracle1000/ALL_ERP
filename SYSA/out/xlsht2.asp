...
'Private Sub Class_Initialize
		...
		Sub page_load
			Set conn = cn
			Dim open_5_1,intro_5_1,open_5_21,intro_5_21,open_5_10,intro_5_10,open_5_11,intro_5_11
			Call fillPowerInfo(5,1,open_5_1,intro_5_1)
			Call fillPowerInfo(5,21,open_5_21,intro_5_21)
			Call fillPowerInfo(5,10,open_5_10,intro_5_10)
			Call fillPowerInfo(5,11,open_5_11,intro_5_11)
			Dim list,fromTJ
			if open_5_1=3 then
				list=""
			elseif open_5_1=1 then
				list=" and a.cateid<>0 and a.cateid in ("&intro_5_1&")"
			else
				list=" and 1=0"
			end if
			Dim interval,sql
			sql = "SELECT tq1 FROM setjm WHERE ord = 21 AND cateid = "&session("personzbintel2007")&" "
			set rs = conn.execute(sql)
			If Not rs.Eof Then
				interval = rs(0)
			else
			...
			If fromTJ="1" Then
				...
				ElseIf open_5_11 = 1 Then
					conditionTJ = " and cateid in ("&intro_5_11&")"
				else
					conditionTJ = " and 1=2"
				end if
				Str_Result=" where del=1 " & list & conditionTJ & " "
			else
				Str_Result=" where a.del=1 and ((a.del=1 "&list&") or (charindex(',"&Info.User&",',','+replace(cast(a.share as varchar(8000)),' ','')+',')>0 or a.share='1')) "
				'Str_Result=" where del=1 " & list & conditionTJ & " "
			end if
			dim W1,W2,W3,A1,A2,A3,B,C,D,E,F,H,MinContractMoney,MaxContractMoney,m3,m4,bz,m1,m2,jhdate,jhdate2,SH,sp1,F1,F2,G1,G2,P1,P2,I1,I2,S1,S2,ZT
			Dim ksjs,ksjs2,zdy1_1,zdy1_2,zdy2_1,zdy2_2,zdy3_1,zdy3_2,zdy4_1,zdy4_2,zdy5,zdy6,W4,tmp,ghs,gateTy,area_list,jhCondition,px,px_Result
			A1=request("A1")
			A2=request("A2")
			A3=request("A3")
			B=request("B")
			...
			zdy5=request("zdy5")
			zdy6=request("zdy6")
			Dim fst,htTerminated,searchType,searchValue
			fst=request("fst")
			...
			If searchValue<>"" Then
				...
				Case "htTitle" : Str_Result = Str_Result & " and a.title like '%" & searchValue & "%'"
				Case "htBh" : Str_Result = Str_Result & " and htid like '%" & searchValue & "%'"
				Case "khname" : Str_Result = Str_Result & " and a.company in (select ord from tel where name like '%" & searchValue & "%')"
				Case "htIntro" : Str_Result = Str_Result & " and a.intro like '%" & searchValue & "%'"
				Case Else :
				If InStr(searchType,"zdy")>0 Then
					Str_Result = Str_Result & " and a."&searchType&" like '%" & searchValue & "%'"
				else
					Str_Result = Str_Result & " and "&searchType&" like '%" & searchValue & "%'"
				end if
				End Select
			end if
			If fst<>"" And m1 = "" And m2 = "" Then
				m1 = year(date)&"-1-1"
'If fst<>"" And m1 = "" And m2 = "" Then '
				m2 = year(date)&"-12-31"
'If fst<>"" And m1 = "" And m2 = "" Then '
			...
			if sp1<>"" then
				if sp1="1" then
					Str_Result="where a.del=3 and a.sp>0 "&list&""
				elseif sp1="2" then
					Str_Result="where a.del=3 and a.sp=-1 "&list&""
'elseif sp1="2" then
				elseif sp1="3" then
					Str_Result="where a.del=3 or a.del=1 "&list&""
				end if
			else
			end if
			if SH<>"" then
				if SH="1" then
					Str_Result=" where a.share<>'0' and a.share<>'-1' and (a.share='1' or a.addshare="&info.user&") "
'if SH="1" then
				elseif SH="2" then
					Str_Result=" where (a.share='1' or a.share like '%"&info.user&"%') "
				end if
			end if
			dim link
			link=trim(request("link"))
			dim newstate,Str_Result_21,sql9,rs9,tq21
			newstate=trim(request("newstate"))
			if newstate="1" then
				set rs9=server.CreateObject("adodb.recordset")
				sql9="select ord,intro,num1,tq1,fw1 from setjm2  where ord=21  and cateid="&session("personzbintel2007")&""
				...
				if rs9.eof then
					...
					if open_5_1=3 then
						Str_Result_21=""
					elseif open_5_1=1 then
						Str_Result_21=" and a.cateid<>0 and (a.cateid in ("&intro_5_1&") or (charindex(',"&session("personzbintel2007")&",',','+replace(cast(a.share as varchar(8000)),' ','')+',')>0 or a.share='1'))"
'elseif open_5_1=1 then
					else
						Str_Result_21=" and 1=0"
					end if
					if rs9("fw1")="1" then
						Str_Result_21=" and (a.cateid="&session("personzbintel2007")&" or (charindex(',"&session("personzbintel2007")&",',','+replace(cast(a.share as varchar(8000)),' ','')+',')>0 or a.share='1'))"
'if rs9("fw1")="1" then
					end if
					tq21=rs9("tq1")
				end if
				rs9.close
				set rs9=nothing
				Str_Result=Str_Result&" and a.date2<='"&dateadd("d",tq21,date)&"' and a.alt2 is NULL "
			end if
			Dim helper
			Set helper = CreateReminderHelper(conn,21,0)
			If b<>"3333333" Then
				Str_Result = Str_Result & " and a.ord in (" & helper.listSQL("ids") & ")"
			end if
			Dim new_list,timelg,noalt1
			if b="3333333" or b="5555555" then
				...
				set rs9=server.CreateObject("adodb.recordset")
				sql9="select ord,intro,num1,tq1,fw1 from setjm  where ord=21  and cateid="&session("personzbintel2007")&""
				rs9.open sql9,conn,1,1
				if rs9.eof then
					tq21=0
				else
...
'elseif open_5_1=1 then
				...
				rs9.close
				set rs9=nothing
				if link="yes" and W3<>"" then
					Str_Result=Str_Result&" and a.cateid="&W3&""
				end if
				If b="3333333" Then
					Str_Result=Str_Result&" and a.date2<='"&dateadd("d",tq21,date)&"' and a.alt2 is NULL "&noalt1&""
				end if
			end if
			...
			if W4<>"" Then
				...
				If InStr(gatety,",1,") > 0 Then
					gateCondition = gateCondition & " or (a.cateid in("& W3 &") and a.cateid<>0) "
				end if
				If InStr(gatety,",2,") > 0 Then
					gateCondition = gateCondition & " or (a.addcate in("& W3 &") and a.addcate<>0) "
				end if
				If InStr(gatety,",3,") > 0 Then
					gateCondition = gateCondition & " or (isnull(a.cateid,0) = 0 or a.cateid not in (select ord from gate)) "
				end if
				...
				If InStr(gatety,",3,") > 0 Then
					Str_Result = Str_Result & " and (isnull(a.cateid,0) = 0 or a.cateid not in (select ord from gate)) "
				end if
			end if
			...
			if HtType<>"" then
				select case HtType
				case 0
				Str_Result=Str_Result&" and (a.paybacktype=0 or a.paybacktype is null) AND ISNULL(a.sort1,0) = 0"
				titlename="销售"
				case 2
				Str_Result=Str_Result&" and a.paybacktype=1 AND ISNULL(a.sort1,0) = 0"
				titlename="直接出库"
				case 3
				Str_Result=Str_Result&" AND a.sort1 = 1"
				titlename="销售开单"
				case 4
				Str_Result=Str_Result&" AND a.repairOrderId is not null"
				titlename="维修"
				case 6
				Str_Result=Str_Result+" AND exists(select 1 from contractlist where contract = a.ord and isnull(treeord,0)>0)  "
'case 6      '
				titlename="组装"
				Case 5
...
'Case 5 '
			...
			Str_Result=Str_Result
			if A2<>"" then
				Str_Result=Str_Result&" and a.area in ("&selects(A2)&")"
			end if
			if A1<>"" then
				Str_Result=Str_Result&" and a.zt1="&A1&""
			end if
			if A3<>"" then
				Str_Result=Str_Result&" and a.zt2="&A3&""
			end if
			if D<>"" then
			...
			if F<>"" then
				Str_Result=Str_Result&" and a.sort in  ("&F&")"
			end if
			if zdy5<>"" then
			...
			if F2<>"" then
				if F1=1 then
					Str_Result=Str_Result&" and a.company in (select ord from tel where name like '%"& F2 &"%')"
				elseif F1=2 then
					Str_Result=Str_Result&" and a.company in (select ord from tel where name not like '%"& F2 &"%')"
				elseif F1=3 then
					Str_Result=Str_Result&" and a.company in (select ord from tel where name='"&F2&"')"
				elseif F1=4 then
					Str_Result=Str_Result&" and a.company in (select ord from tel where name<>'"&F2&"')"
				elseif F1=5 then
					Str_Result=Str_Result&" and a.company in (select ord from tel where name like '"& F2 &"%')"
			...
			if G2<>"" then
				if G1=1 then
					Str_Result=Str_Result&" and a.title like '%"& G2 &"%'"
				elseif G1=2 then
					Str_Result=Str_Result&" and a.title not like '%"& G2 &"%'"
				elseif G1=3 then
					Str_Result=Str_Result&" and a.title='"& G2 &"'"
				elseif G1=4 then
					Str_Result=Str_Result&" and a.title<>'"& G2 &"'"
				elseif G1=5 then
					Str_Result=Str_Result&" and a.title like '"& G2 &"%'"
			...
			if S2<>"" then
				if S1=1 then
					Str_Result=Str_Result&" and a.person1 like '%"& S2 &"%'"
				elseif S1=2 then
					Str_Result=Str_Result&" and a.person1 not like '%"& S2 &"%'"
				elseif S1=3 then
					Str_Result=Str_Result&" and a.person1='"& S2 &"'"
				elseif S1=4 then
					Str_Result=Str_Result&" and a.person1<>'"& S2 &"'"
				elseif S1=5 then
					Str_Result=Str_Result&" and a.person1 like '"& S2 &"%'"
			...
			if P2<>"" then
				if P1=1 then
					Str_Result=Str_Result&" and a.htid like '%"& P2 &"%'"
				elseif P1=2 then
					Str_Result=Str_Result&" and a.htid not like '%"& P2 &"%'"
				elseif P1=3 then
					Str_Result=Str_Result&" and a.htid='"& P2 &"'"
				elseif P1=4 then
					Str_Result=Str_Result&" and a.htid<>'"& P2 &"'"
				elseif P1=5 then
					Str_Result=Str_Result&" and a.htid like '"& P2 &"%'"
			...
			if I2<>"" then
				if I1=1 then
					Str_Result=Str_Result&" and a.intro like '%"& I2 &"%'"
				elseif I1=2 then
					Str_Result=Str_Result&" and a.intro not like '%"& I2 &"%'"
				elseif I1=3 then
					Str_Result=Str_Result&" and a.intro like '%"& I2 &"%'"
				elseif I1=4 then
					Str_Result=Str_Result&" and a.intro not like '%"& I2 &"%'"
			...
			if zdy1_2<>"" then
				if zdy1_1=1 then
					Str_Result=Str_Result&" and a.zdy1 like '%"& zdy1_2 &"%'"
				elseif zdy1_1=2 then
					Str_Result=Str_Result&" and a.zdy1 not like '%"& zdy1_2 &"%'"
				elseif zdy1_1=3 then
					Str_Result=Str_Result&" and a.zdy1='"& zdy1_2 &"'"
			...
			if zdy2_2<>"" then
				if zdy2_1=1 then
					Str_Result=Str_Result&" and a.zdy2 like '%"& zdy2_2 &"%'"
				elseif zdy2_1=2 then
					Str_Result=Str_Result&" and a.zdy2 not like '%"& zdy2_2 &"%'"
				elseif zdy2_1=3 then
					Str_Result=Str_Result&" and a.zdy2='"& zdy2_2 &"'"
			...
			if zdy3_2<>"" then
				if zdy3_1=1 then
					Str_Result=Str_Result&" and a.zdy3 like '%"& zdy3_2 &"%'"
				elseif zdy3_1=2 then
					Str_Result=Str_Result&" and a.zdy3 not like '%"& zdy3_2 &"%'"
				elseif zdy3_1=3 then
					Str_Result=Str_Result&" and a.zdy3='"& zdy3_2 &"'"
			...
			if zdy4_2<>"" then
				if zdy4_1=1 then
					Str_Result=Str_Result&" and a.zdy4 like '%"& zdy4_2 &"%'"
				elseif zdy4_1=2 then
					Str_Result=Str_Result&" and a.zdy4 not like '%"& zdy4_2 &"%'"
				elseif zdy4_1=3 then
					Str_Result=Str_Result&" and a.zdy4='"& zdy4_2 &"'"
			...
			if m1<>"" then
				Str_Result=Str_Result&" and a.date3>='"&m1&"' "
			end if
			if m2<>"" then
				Str_Result=Str_Result&" and a.date3<='"&m2&"' "
			end if
			if m3<>"" then
				Str_Result=Str_Result&" and a.date2>='"&m3&"' "
			end if
			if m4<>"" then
				Str_Result=Str_Result&" and a.date2<='"&m4&"' "
			end if
			jhCondition = ""
			If jhdate<>"" Then
			...
			If jhdate2<>"" Then
				jhCondition = jhCondition & " and date2<='" & jhdate2 & "' "
			end if
			If jhCondition <> "" Then
				Str_Result = Str_Result & " and a.ord in (select contract from contractlist where del=1 "&jhCondition&")"
			end if
			if ZT<>"" then
				if zt="1" then
			...
			If Not (b="3333333" or b="5555555" or newstate="1") then
				Str_Result = Str_Result + " AND DATEADD(D,-"&interval&",a.date2) <= GETDATE() "
'If Not (b="3333333" or b="5555555" or newstate="1") then
			end if
			px=request("px")
			...
			elseif px="3" then
				px_Result="order by a.title desc,a.date7 desc"
			elseif px="4" then
				px_Result="order by a.title asc,a.date7 asc"
			elseif px="5" then
				px_Result="order by a.htid desc,a.date7 desc"
			elseif px="6" then
				px_Result="order by a.htid asc,a.date7 asc"
			elseif px="7" then
				px_Result="order by a.date3 desc,a.ord desc,a.date7 desc"
			elseif px="8" then
				px_Result="order by a.date3 asc,a.ord asc,a.date7 asc"
			elseif px="9" then
				px_Result="order by a.date2 desc,a.ord desc,a.date7 desc"
			elseif px="10" then
				px_Result="order by a.date2 asc,a.ord asc,a.date7 asc"
			end if
			if open_5_10=3 then
				list=""
			elseif open_5_10=1 then
				list=" and a.cateid in ("&intro_5_10&") and a.cateid>0 "
			else
				list=" and 1=2 "
			end if
			Str_Result=Str_Result&""&list&"" & Str_Result_21
			Dim Export_Fields,CantExport_Fields(255),cfidx,fields
			cfidx=0
			fields= "合同主题,合同编号,客户名称,客户编号,币种,合同总额,合同优惠金额,优惠后总额,到账金额,运杂费,签订日期,开始日期,截止日期,我方代表,对方代表,合同分类,合同状态,收款状态,开票状态,出库状态,发货状态"
			If open_5_21 = 0 Then
				CantExport_Fields(cfidx) = "合同总额" : cfidx=cfidx+1
'If open_5_21 = 0 Then
				CantExport_Fields(cfidx) = "合同优惠金额" : cfidx=cfidx+1
'If open_5_21 = 0 Then
				CantExport_Fields(cfidx) = "优惠后总额" : cfidx=cfidx+1
'If open_5_21 = 0 Then
				CantExport_Fields(cfidx) = "到账金额" : cfidx=cfidx+1
'If open_5_21 = 0 Then
			end if
			if ZBRuntime.MC(17000) = False then
				CantExport_Fields(cfidx) = "出库状态" : cfidx=cfidx+1
'if ZBRuntime.MC(17000) = False then
				CantExport_Fields(cfidx) = "发货状态" : cfidx=cfidx+1
'if ZBRuntime.MC(17000) = False then
			end if
			Dim rs,zdyFields(),j
			ReDim zdyFields(1,0)
			set rs=server.CreateObject("adodb.recordset")
			rs.open "select title,name,set_open,dc from zdy where sort1=5 order by gate1 asc ",conn,1,1
			j=0
			do while rs.eof=False
...
'If rs("set_open") <> 1 Or rs("dc") <> 1 Then
			...
			rs.close
			set rs=nothing
			fields=fields&",合同概要,洽谈进展,销售人员,创建人,创建时间"
			if ZBRuntime.MC(12006) = False then
				CantExport_Fields(cfidx) = "洽谈进展" : cfidx=cfidx+1
'if ZBRuntime.MC(12006) = False then
			end if
			Dim exZdyFields(),exJoinTables,exZdy1,exZdy2
			ReDim exZdyFields(1,0)
			sql="select ID,FName from ERP_CustomFields where isUsing=1 and canexport=1 and tname=5 and del=1 order by id"
			j=0
			...
			sql = "SELECT name,title,sorce,set_open FROM zdymx WHERE sort1 = 5 and sorce not in (16,17) ORDER BY gate1"
			Set rs = cn.execute(sql)
			j = 0
...
'If rs("set_open") <> 1 Then
				...
				If sorce = 6 Then
					fields=fields&",退货数量,申请出库,已经出库,已经发货"
				end if
				rs.movenext
				j = j + 1
				'fields=fields&",退货数量,申请出库,已经出库,已经发货"
			wend
			if ZBRuntime.MC(17000) = False Then
				CantExport_Fields(cfidx) = "申请出库" : cfidx=cfidx+1
'if ZBRuntime.MC(17000) = False Then '
				CantExport_Fields(cfidx) = "已经出库" : cfidx=cfidx+1
'if ZBRuntime.MC(17000) = False Then '
				CantExport_Fields(cfidx) = "已经发货" : cfidx=cfidx+1
'if ZBRuntime.MC(17000) = False Then '
			end if
			if ZBRuntime.MC(8000) = False Then
				CantExport_Fields(cfidx) = "退货数量" : cfidx=cfidx+1
'if ZBRuntime.MC(8000) = False Then '
			end if
			if open_5_21 = 0 Then
				Dim tmpArr
				tmpArr = Split("未税单价,折扣,折后单价,含税单价,含税折后单价,税额,税前总价,税后总价,优惠金额,产品总价",",")
				For j=0 To ubound(tmpArr)
					CantExport_Fields(cfidx) = fDBName(tmpArr(j)) : cfidx=cfidx+1
'For j=0 To ubound(tmpArr)
				next
			end if
			Dim baseSql
			baseSql = " exec erp_xls_ht '"& replace(Str_Result,"'","''") &"','"& replace(px_Result,"'","''") &"','"& replace(exZdy1,"'","''") &"','"& replace(exZdy2,"'","''") &"','"& replace(exJoinTables,"'","''") &"','"& replace(fields,"'","''")&"','','','','' "
			Dim adapter
			Dim sqls,sheetSetting
			Set adapter = New ExcelExportAdapter
			adapter.fileName = "合同资料"
			adapter.canSplitFormula_And = "canSplit(rs)"
			Dim hd,fds,i,fd,sname
			Set sheetSetting = New SqlSoruce
			sheetSetting.sql = baseSql
			sheetSetting.title = "合同资料"
			fds = Split(fields,",")
			For i = 0 To ubound(fds)
				...
				Select Case sname
				Case "合同总额","合同优惠金额","优惠后总额","到账金额","折扣","产品总价","税额","税前总价","税后总价","优惠金额","运杂费","建议总价":
				hd.dbtype = "money"
				...
				hd.ignoreNonnumeric = True
				hd.excelAlign = "right"
				Case "折后单价","未税单价","含税折后单价","含税单价":
				hd.dbtype = "salesprice"
				hd.width = EX_WIDTH_DIGIT
				hd.ignoreNonnumeric = True
				hd.excelAlign = "right"
				Case "购买数量","申请出库","已经出库","已经发货","退货数量":
				hd.dbtype = "number"
				hd.width = EX_WIDTH_DIGIT
				hd.ignoreNonnumeric = True
				...
				hd.width = 60
				hd.excelAlign = "center"
				If sname="税率" Then hd.Formula = "myFormatNumber(@value,"&info.FloatNumber&",""%"")"
				Case "合同主题","合同编号","客户名称","客户编号","zdy1","zdy2","zdy3","zdy4","产品名称","产品编号","产品型号","明细自定义1","明细自定义2","明细自定义3","明细自定义4":
				hd.width = EX_WIDTH_TEXT
				Case "签订日期","开始日期","截止日期","创建时间","交货日期":
				hd.width = EX_WIDTH_DATE
				Case "我方代表","对方代表","销售人员","创建人":
				hd.width = EX_WIDTH_PERSON
				Case "合同分类","合同状态","出库状态","发货状态","zdy5","zdy6","产品单位","明细自定义5","明细自定义6":
				...
				If InStr(sname,"折扣")>0 Then
					hd.canSum = False
					hd.dbtype="str"
					hd.Formula="myFormatNumber(@value,"&info.discountNumber&","""")"
					hd.ignoreNonnumeric = True
					hd.excelAlign="center"
				end if
			...
			hd.title = "深度"
			hd.width = 40
			If zbruntime.mc(17010) = False Then
				hd.display = "none"
			end if
			sheetSetting.headerSettings.add(hd)
		...
		Function canSplit(rs)
			canSplit = instr(rs("合同编号")&"","【产品明细】")<=0
		end function
		Function myFormatNumber(v,n,c)
			If v&""<>"" Then
				myFormatNumber = FormatNumber(v,n,-1,0,-1) & c
'If v&""<>"" Then
			else
				myFormatNumber = ""
			end if
		...
		Function fShowName(fname)
			Select Case fname
			Case "P0":fShowName="深度"
			Case "P1":fShowName="产品名称"
			Case "P2":fShowName="产品编号"
			Case "P3":fShowName="产品型号"
			Case "P4":fShowName="产品单位"
			Case "P6":fShowName="购买数量"
			Case "P5":fShowName="未税单价"
			Case "P7":fShowName="产品总价"
			Case "P18":fShowName="折扣"
			Case "P19":fShowName="折后单价"
			Case "P20":fShowName="票据类型"
			Case "P21":fShowName="税率"
			Case "P22":fShowName="含税折后单价"
			Case "P23":fShowName="税额"
			Case "P24":fShowName="税前总价"
			Case "P25":fShowName="税后总价"
			Case "P26":fShowName="优惠金额"
			Case "P27":fShowName="含税单价"
			Case "P8":fShowName="交货日期"
		...
		Function fDBName(fname)
			...
			Case "产品单位":fDBName="P4"
			Case "购买数量":fDBName="P6"
			Case "未税单价":fDBName="P5"
			Case "产品总价":fDBName="P7"
			Case "折扣":fDBName="P18"
			Case "折后单价":fDBName="P19"
			Case "票据类型":fDBName="P20"
			Case "税率":fDBName="P21"
			Case "含税折后单价":fDBName="P22"
			Case "税额":fDBName="P23"
			Case "税前总价":fDBName="P24"
			Case "税后总价":fDBName="P25"
			Case "优惠金额":fDBName="P26"
			Case "含税单价":fDBName="P27"
			Case "交货日期":fDBName="P8"
			Case "备注":fDBName="P9"
