...
'dim sqlstr
	...
	rs1.close
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=8 and sort2=11"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_8_11=0
		intro_8_11=0
	else
		open_8_11=rs1("qx_open")
		intro_8_11=rs1("qx_intro")
	end if
	rs1.close
	set rs1=Nothing
	companyPowerCondition = " and (" & open_8_11 & "=3 or (" & open_8_11 & "=1 and charindex(','+cast(a.cateid as varchar)+',','," & intro_8_11 & ",')>0) ) "
'set rs1=Nothing
	A=request("A")
	B=request("B")
	C=request("C")
	...
	if D<>"0" then
		Str_Result3=Str_Result3+" and bz="&D&" "
'if D<>"0" then
	end if
	if C<>"" then
		if B="khmc" then
			Str_Result3=Str_Result3+ " and name like '%"&C&"%' "
'if B="khmc" then
		end if
	end if
	Str_Result4 = ""
	...
	elseif px=2 then
		px_Result="order by name asc,ord asc"
	elseif px=3 then
	...
	elseif px=9 then
		px_Result="order by money4 desc,ord desc"
	elseif px=10 then
		px_Result="order by money4 asc,ord asc"
	end if
	...
	Response.write page_count
	Response.write """ id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""place"">供应商预付款统计分析</td>" & vbcrlf & "      <td align=""left"">&nbsp;<a class='px_btn' href=""#"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "      <td width=""60%""><div align=""right"">" & vbcrlf & "        <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""page_count=10"" "
	...
	Response.write ">供应商名称</option>" & vbcrlf & "                 </select>" & vbcrlf & "                       <input type=""text"" name=""C"" value="""
	Response.write C
	...
	if open_8_10=1 or open_8_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""EXCEL导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_gysyfktj.asp'})}"" class=""anybutton""/>" & vbcrlf & "            "
	end if
	...
	Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "             </tr>" & vbcrlf & "      </table> " & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-top:-3px;"">" & vbcrlf & "      </form>" & vbcrlf & " <tr class=""top resetGroupTableBg"">" & vbcrlf &   " <td height=""26""><div align=""center"">供应商名称</div></td> "& vbcrlf &  " <td width=""8%""><div align=""center"">币种</div></td> "& vbcrlf &          " <td width=""12%""><div align=""center"">预付款总额</div></td>" & vbcrlf & "   <td width=""12%""><div align=""center"">使用预付款</div></td>"
'& "         <td width=""12%""><div align=""center"">退预付款</div></td>" & vbcrlf & "     <td width=""12%""><div align=""center"">预付款余额</div></td>" & vbcrlf & "   <td width=""20%""><div align=""center"">退预付款</div></td>" & vbcrlf & "     </tr>" & vbcrlf & ""
	...
	"  from ( "&_
	"  select s.company,s.bz,isnull(sum(s.money1),0) as money1,0 as money2 , 0 as money3 ,isnull(sum(s.money_left),0) as money4 from bankout2 s WITH(NOLOCK) where s.del=1 and isnull(s.status_sp,1) in (-1,1) and isnull(s.BankinStatus,0) in (2,3) "& Str_Result4 &" group by s.company,s.bz " &_
	"  union all " &_
	"  select b.company,b.bz,0,isnull(sum(b.money1),0) as money2,0,0 from bankout4_list b WITH(NOLOCK) inner join bankout2 s WITH(NOLOCK) on b.bankout2=s.id where b.del=1 and b.money1>0 "& Str_Result4 &"  group by b.company,b.bz " &_
	"  union all " &_
	"  select b.company,k.bz,0,0,isnull(sum(b.money1),0) as money3,0 from bankout3 b WITH(NOLOCK) inner join bankout2 s WITH(NOLOCK) on b.bankout2=s.id inner join sortbank k on k.id =b.bank where b.del=1  "& Str_Result4 &"  group by b.company,k.bz" &_
	") m group by company,bz " &_
	") m1 on m1.company = a.ord and m1.bz=z.id "&_
	"where a.del =1 and (m1.money1<>0 or money2<>0 or money3<>0 or money4<>0) " & Str_Result3 & companyPowerCondition & " "&px_Result&" "
	rs.open sql,conn,1,1
	if not rs.eof then
		do while not rs.eof
...
'sort3=rs("sort3")
			...
			if sort3="1"  then
				if open_1_14=3 or  CheckPurview(intro_1_14,trim(rs("cateid")))=True  Then
				...
				If open_26_1=3 or  CheckPurview(intro_26_1,trim(rs("cateid")))=True  Then
					if open_26_14=3 or  CheckPurview(intro_26_14,trim(rs("cateid")))=True  Then
			...
			Response.write "</td>" & vbcrlf & "                <td align=""right"">"
			Response.write "</td>" & vbcrlf & "                <td align=""right""><a href=""javascript:void(0);"" onclick=""javascript:window.open('../../SYSN/view/finan/payout/payoutpre/AdvanceChargeLogList.ashx?company="
			Response.write pwurl(rs("ord"))
			Response.write "&bz="
			Response.write pwurl(rs("bz"))
			Response.write "','newww56insd3','width=' + 1200 + ',height=' + 800 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;"" title=""查看操作日志"">"
			Response.write pwurl(rs("bz"))
			Response.write Formatnumber(zbcdbl(rs("money4")),num_dot_xs,-1)
			Response.write pwurl(rs("bz"))
			Response.write "</a></td>" & vbcrlf & "            <td align=""center""><div align=""center""><a href=""javascript:void(0);"" onclick=""javascript:window.open('../money2/back_yfk.asp?company="
			Response.write pwurl(rs("ord"))
			Response.write "&bz="
			Response.write pwurl(rs("bz"))
			Response.write "','newwwinsd3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;"" title=""点击退供应商预付款""><font class=""red"">退预付款</font></a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "           "
			Response.write pwurl(rs("bz"))
			n=n+1
			Response.write pwurl(rs("bz"))
			rs.movenext
		...
		loop
		Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "       <td class=""name"" colspan=""2"" align=""center""><div align=""right"">本页合计：</div></td>" & vbcrlf & "            <td class=""name"" align=""right"">"
		Response.write  Formatnumber(summoney1,num_dot_xs,-1)
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""right"">"
		...
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""right"">"
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""center"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "       <td class=""name"" colspan=""2"" align=""center""><div align=""right"">所有合计：</div></td>" & vbcrlf & "           <td class=""name"" align=""right"">"
		Response.write  Formatnumber(money1_all,num_dot_xs,-1)
'ght"">所有合计：</div></td>" & vbcrlf & "           <td class=""name"" align=""right"">"
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""right"">"
		...
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""right"">"
		Response.write "</td>" & vbcrlf & "            <td class=""name"" align=""center"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "      </table>" & vbcrlf & "  </td>" & vbcrlf & "  </tr>" & vbcrlf & "          <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30"">&nbsp;</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "   </form>" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "     <span class=""black"">"
	...
	action1="供应商预付款统计分析"
	call close_list(1)
	Dim userwidth : userwidth = 400
	Dim tbHeight :  tbHeight = 280
	if D="0" Then userwidth = 20 : tbHeight = 70
	Response.write "  " & vbcrlf & "   </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen(divID){" & vbcrlf & " if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display="""" "& vbcrlf &  "   }" & vbcrlf &     "   divID.style.left=308;" & vbcrlf &  "  divID.style.top=0; "& vbcrlf &" } "& vbcrlf &" </script> "& vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:"
	Response.write userwidth
	Response.write ";display:none;"" >" & vbcrlf & "<table width=""150"" height="""
	Response.write tbHeight
	Response.write """  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td>" & vbcrlf & "          <table width=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "                     <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按供应商名称排序(降)</font></a></td></tr>" & vbcrlf & "                    <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按供应商名称排序(升)</font></a> </td></tr>" & vbcrlf & "                        <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按预付款总额排序(降)</font></a> </td></tr>" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按预付款总额排序(升)</font></a> </td></tr>" & vbcrlf & "                    <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按使用预付款排序(降)</font></a> </td></tr>" & vbcrlf & "                        <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按使用预付款排序(升)</font></a> </td></tr>" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按退预付款排序(降)</font></a> </td></tr>" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按退预付款排序(升)</font></a> </td></tr>" & vbcrlf & "                  <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=9')""><font color=""#2F496E"">按预付款余额排序(降)</font></a> </td></tr> "& vbcrlf &                 " <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0);"" onclick=""gotourl('px=10')""><font color=""#2F496E"">按预付款余额排序(升)</font></a> </td></tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "</body>" & vbcrlf & "</html>"
