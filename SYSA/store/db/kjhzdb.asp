...
'If True  Then '(canAdd Or PageType = "database")  Then
					AddHtml htmls,("<td align='left'> " & app.iif(canadd And showAddButton, "<table id='lvw_add_" & id & "_tb' style='margin-left:15px' onclick='lvw.addRow(this.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.children[0])'><tr><td><img class='resetElementHidden' src='../../images/jiantou.gif'><img class='resetElementShow' style='display:none;vertical-align:-3px;' width='14' height='14' src='../../skin/default/images/MoZihometop/content/lvw_addrow_btn.png'></td><td class='ctl_lvwaddrowlk' onmousemove='Bill.showunderline(this)' onmouseout='Bill.hideunderline(this)' nowrap>添加新行</td></tr></table>","") & "</td>")
					Dim PageSizeConst
...
'Public Function AddFormValue(name,  value)
		...
		sub page_load
			cn.execute  "if not exists(select * from syscolumns where id=object_id('kuoutlist') and name='JoinDBListID') " & vbcrlf & _
			"begin " & vbcrlf & _
			"   alter table kuoutlist add JoinDBListID int  " & vbcrlf & _
			"end" & vbcrlf & _
			"if not exists(select * from syscolumns where id=object_id('kuoutlist2') and name='JoinDBListID') " & vbcrlf & _
			"begin " & vbcrlf & _
			"   alter table kuoutlist2 add JoinDBListID int  " & vbcrlf & _
			"end" & vbcrlf & _
			"if not exists(select * from syscolumns where id=object_id('kuinlist') and name='JoinDBListID') " & vbcrlf & _
			"begin " & vbcrlf & _
			"   alter table kuinlist add JoinDBListID int  " & vbcrlf & _
			"end" & vbcrlf
			cn.execute   "if not exists(select ID from M_CustomSQLStrings where ID=74)" & vbcrlf & _
			"begin" & vbcrlf & _
			"   insert into M_CustomSQLStrings (ID,SQLString,sColumns,hColumns,GroupString,LinkURL,Title,Description)" & vbcrlf & _
			"   values (74,'sql=select ''随机'' as billselectname , 0 as id      union all      select ''指定'' as billselectname , 1 as id','','','','','汇总调拨方式','')" & vbcrlf & _
			"end"
			Response.write "" & vbcrlf & "<script language=""javascript"" src='/inc/dateid.js?ver="
			Response.write Application("sys.info.jsver")
			Response.write "'></script>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "      function fMove(obj){" & vbcrlf & "            if(obj.offsetWidth<197){" & vbcrlf & "                        obj.style.width = 197;" & vbcrlf & "          }" & vbcrlf & "       }" & vbcrlf & "</script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "html{overflow:auto;padding:0;}" & vbcrlf & "" & vbcrlf & "</style>" & vbcrlf & "<frameset id='iframebody' cols='197,*' border=""10"" frameSpacing=10 frameborder=1 style='border:4px solid white'>" & vbcrlf & "              <frame src='addpd.asp?__msgId=ckcptreelist&musthasnum=1' frameborder=0 bordercolor=""#fefeff"" style='min-width:197px' onresizex='return fMove(this)'></frame>" & vbcrlf & "              <frame src='?__msgId=addpdPage&"
			'Response.write Application("sys.info.jsver")
...
'if not isnumeric(top) or len(top) = 0 then
			...
			set rs = cn.execute("select * from kupd where ord=" & top)
			if not rs.eof then
				tit = rs.fields("title").value
				pdno = rs.fields("pdbh").value
				pddate = rs.fields("date3").value
				bz = rs.fields("intro").value
			else
				tit = ""
				set rss = app.getdatarecord(cn.execute("EXEC erp_getdjbh 36,"&session("personzbintel2007")))
				...
				If cn.execute("select 1 from inventoryCost WHERE datediff(mm,date1,'"& date &"')=0 and complete1 >= 1").eof=False Then
					pddate = cn.execute("select convert(varchar(10),dateadd(mm,1,max(date1)), 120) from inventoryCost where complete1 >= 1")(0)
				end if
				bz = ""
			end if
			rs.close
			Response.write app.headhtml
			Response.write "<style>html{padding:0px}textarea{ border:1px solid #b6c0c9; }textarea:focus{ outline:none;box-shadow: 0px 0px 2px 1px #4fa1e5; } table.edtfocustb{height:12px !important;width:auto !important;float:right;} .edt1tb{height:50%;}</style>"
			'Response.write app.headhtml
			Response.write "" & vbcrlf & "<body style='margin-top:0px;padding-top:0px;overflow:auto;'>" & vbcrlf & "  <script language=javascript src='../../Manufacture/inc/bill.js?ver="
			'Response.write app.headhtml
			Response.write Application("sys.info.jsver")
			...
			Response.write Application("sys.info.jsver")
			'Response.write Application("sys.info.jsver")
			Response.write "'></script><!-- office/.net样式菜单 -->" & vbcrlf & "       <div id='head' class=""resetHeadBgImg"" style='margin:0px;padding:0px;background-image:url(../images/m_mpbg.gif)'>" & vbcrlf & "          <div class=""resetTransparent"" style='float:left;background-image:url(../../images/content_tab.png);width:277px;height:32px'>" & vbcrlf & "                 <div class=""resetTransparent resetTextColor333 billTitle"" style='margin-top:10px;*margin-top:14px;margin-left:65px;font-weight:bold;font-size:14px;color:#444499;letter-spacing:3px;'>库间汇总移货</div>" & vbcrlf & "                  "
			'Response.write Application("sys.info.jsver")
			action1="库间汇总移货"
			...
			Response.write top
			Response.write "' id='pdID'>" & vbcrlf & "  <table class=""resetBorderColor"" style='table-layout:fixed;border:1px solid #A7BBD7;border-top:0px;width:100%'>" & vbcrlf & "    <tr>" & vbcrlf & "            <td class='billfieldleft resetBorderColor' style='width:70px;border-left:1px solid #ccc'><pre style='display:inline'>  调拨主题： </pre></td>" & vbcrlf & "         <td class='billfieldright'>" & vbcrlf & "                     <table>" & vbcrlf & "                 <tr>" & vbcrlf & "                            <td><input type=text class=text style='height:17px;width:120px' id='MT1' value='"
			Response.write tit
			Response.write "' maxlength=40></td>" & vbcrlf & "                          <td>&nbsp;<span class=c_red>*</span></td>" & vbcrlf & "                               <td  id='MT1_msg'></td>" & vbcrlf & "                 </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "           <td class='billfieldleft'  style='width:70px'><pre style='display:inline'>  调拨编号： </pre></td>" & vbcrlf & "          <td class='billfieldright'>" & vbcrlf & "                     <table>" & vbcrlf & "                 <tr>" & vbcrlf & "                            <td><input type=text class=text style='height:17px' value="""
			Response.write pdno
			Response.write """  id='MT2' maxlength=40></td>" & vbcrlf & "                             <td>&nbsp;<span class=c_red>*</span></td>" & vbcrlf & "                               <td  id='MT2_msg'></td>" & vbcrlf & "                 </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "           <td class='billfieldleft'  style='width:70px'><pre style='display:inline'>  调拨日期： </pre></td>" & vbcrlf & "             <td class='billfieldright'>" & vbcrlf & "                     <table class=textitemtable><tr><td>" & vbcrlf & "                     <input  id='MT3' class=text style= 'border-right:0px;height:17px' type=text readonly value='"
			'Response.write pdno
			Response.write pddate
			Response.write "'></td><td><button class=InselButton   value='0' onclick='Bill.showDateDlg()'><img class=""resetElementHidden"" src='../../images/datePicker.gif'><img class=""resetElementShow"" style=""display:none;"" width=""12"" height=""14"" src='../../skin/default/images/MoZihometop/content/datePicker.png'></button></td><td>&nbsp;<span class=c_red>*</span></td><td  id='MT3_msg'></td></tr></table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class='billfieldleft'><pre style='display:inline'>  调拨备注： </td>" & vbcrlf & "                <td class='billfieldright' colspan=5  style='padding:5px'>" & vbcrlf & "                   <textarea name=""intro""  type=""text"" id='MT4' class='text' rows=""4"" onpropertychange=""this.style.posHeight=this.scrollHeight>64?this.scrollHeight:64;"" style=""width:80%;height:64px;"">"
			Response.write HTMLDecode(bz)
			Response.write "</textarea><span id=""MT4_msg""></span>" & vbcrlf & "           </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class='billfieldleft'><pre style='display:inline'>  调拨明细： </td>" & vbcrlf & "                <td class='billfieldright' colspan=5>" & vbcrlf & "                   <div style='width:100%;overflow-y:hidden;overflow-x:auto' onchange='return listviewChange()'>" & vbcrlf & "                  "
			'Response.write HTMLDecode(bz)
			...
			lvw.canadd = false
			lvw.autosum = false
			lvw.sql =   "exec erp_ck_khhzdb_list " & top
			sttrInx = 2
			set rss = cn.execute("select title,name,sorce,set_open from zdymx where sort1=1003 order by gate1 asc")
			If Not rss.eof Then
				While Not rss.eof
					sorce=rss("sorce")
					title=rss("title")
					set_open=rss("set_open")
					Select Case sorce
					Case 1:
					lvw.cols.items(1).edit = 0
					lvw.cols.items(1).save = 0
					lvw.cols.items(1).ywname = title
					lvw.cols.items(2).htmlvisible = false
					lvw.cols.items(2).save = 1
					lvw.cols.items(2).resize = False
					Case 2:
					If  set_open<>1 Then
						lvw.cols.items(3).htmlvisible = false
						lvw.cols.items(3).resize = False
					end if
					lvw.cols.items(3).edit = 0
					lvw.cols.items(3).save = 0
					lvw.cols.items(3).ywname = title
					Case 3:
					If  set_open<>1 Then
						lvw.cols.items(4).htmlvisible = false
						lvw.cols.items(4).resize = False
					end if
					lvw.cols.items(4).edit = 0
					lvw.cols.items(4).save = 0
					lvw.cols.items(4).ywname = title
					Case 4:
					If  set_open<>1 Then
						lvw.cols.items(5).htmlvisible = false
						lvw.cols.items(5).resize = False
					end if
					lvw.cols.items(5).edit = 0
					lvw.cols.items(5).save = 0
					lvw.cols.items(5).ywname = title
					lvw.cols.items(6).htmlvisible = false
					lvw.cols.items(6).save = 1
					lvw.cols.items(6).resize = False
					Case 5 :
					lvw.cols.items(7).resize = false
					lvw.cols.items(7).save = 1
					lvw.cols.items(7).edit = 0
					lvw.cols.items(7).align ="center"
					lvw.cols.items(7).htmlvisible = isOpenProductAttr
					lvw.cols.items(8).resize = false
					lvw.cols.items(8).save = 1
					lvw.cols.items(8).edit = 0
					lvw.cols.items(8).align ="center"
					lvw.cols.items(8).htmlvisible = isOpenProductAttr
					lvw.cols.items(7+sttrInx).edit = 1
'lvw.cols.items(8).htmlvisible = isOpenProductAttr
					lvw.cols.items(7+sttrInx).save = 1
'lvw.cols.items(8).htmlvisible = isOpenProductAttr
					lvw.cols.items(7+sttrInx).ywname = title
'lvw.cols.items(8).htmlvisible = isOpenProductAttr
					dbnumName=title
					lvw.cols.items(8+sttrInx).edit = 0
'dbnumName=title
					lvw.cols.items(8+sttrInx).save = 0
'dbnumName=title
					lvw.cols.items(9+sttrInx).htmlvisible = false
'dbnumName=title
					lvw.cols.items(9+sttrInx).save = 1
'dbnumName=title
					lvw.cols.items(9+sttrInx).resize = false
'dbnumName=title
					lvw.cols.items(10+sttrInx).edit = 1
'dbnumName=title
					lvw.cols.items(10+sttrInx).save = 1
'dbnumName=title
					lvw.cols.items(10+sttrInx).selid =  74
'dbnumName=title
					lvw.cols.items(10+sttrInx).disztlr = 1
'dbnumName=title
					lvw.cols.items(11+sttrInx).selid = 0
'dbnumName=title
					lvw.cols.items(11+sttrInx).save = 1
'dbnumName=title
					lvw.cols.items(11+sttrInx).htmlvisible = false
'dbnumName=title
					lvw.cols.items(11+sttrInx).resize = false
'dbnumName=title
					lvw.cols.items(12+sttrInx).save = 1
'dbnumName=title
					lvw.cols.items(12+sttrInx).edit = 1
'dbnumName=title
					lvw.cols.items(12+sttrInx).selid = 22
'dbnumName=title
					If cn.execute("select count(1) from setjm3 where ord=5430 and  num1=1").fields(0).value = 1 then
						lvw.cols.items(12+sttrInx).disztlr = 1
'If cn.execute("select count(1) from setjm3 where ord=5430 and  num1=1").fields(0).value = 1 then
					else
						lvw.cols.items(12+sttrInx).disztlr = 0
'If cn.execute("select count(1) from setjm3 where ord=5430 and  num1=1").fields(0).value = 1 then
					end if
					Case 6:
					If  set_open<>1 Then
						lvw.cols.items(13+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(13+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(13+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(13+sttrInx).disztlr=0
'If  set_open<>1 Then
					end if
					lvw.cols.items(13+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(13+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(13+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 7:
					If  set_open<>1 Then
						lvw.cols.items(14+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(14+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(14+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(14+sttrInx).disztlr=0
'If  set_open<>1 Then
					end if
					lvw.cols.items(14+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(14+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(14+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 8:
					If  set_open<>1 Then
						lvw.cols.items(15+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(15+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(15+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(15+sttrInx).disztlr=0
'If  set_open<>1 Then
					end if
					lvw.cols.items(15+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(15+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(15+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 9:
					If  set_open<>1 Then
						lvw.cols.items(16+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(16+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(16+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(16+sttrInx).disztlr=0
'If  set_open<>1 Then
					end if
					lvw.cols.items(16+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(16+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(16+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 10 :
					If  set_open<>1 Then
						lvw.cols.items(17+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(17+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(17+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(17+sttrInx).disztlr=0
'If  set_open<>1 Then
					end if
					lvw.cols.items(17+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(17+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(17+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 11:
					If  set_open<>1 Then
						lvw.cols.items(18+sttrInx).htmlvisible = false
'If  set_open<>1 Then
						lvw.cols.items(18+sttrInx).resize = False
'If  set_open<>1 Then
						lvw.cols.items(18+sttrInx).disztlr=1
'If  set_open<>1 Then
					else
						lvw.cols.items(18+sttrInx).disztlr=01
'If  set_open<>1 Then
					end if
					lvw.cols.items(18+sttrInx).selid = 1062
'If  set_open<>1 Then
					lvw.cols.items(18+sttrInx).edit = 1
'If  set_open<>1 Then
					lvw.cols.items(18+sttrInx).save = 1
'If  set_open<>1 Then
					lvw.cols.items(18+sttrInx).ywname = title
'If  set_open<>1 Then
					Case 12:
					If set_open<>1 Then
						lvw.cols.items(19+sttrInx).htmlvisible = false
'If set_open<>1 Then
						lvw.cols.items(19+sttrInx).resize = False
'If set_open<>1 Then
						lvw.cols.items(19+sttrInx).disztlr=1
'If set_open<>1 Then
					else
						lvw.cols.items(19+sttrInx).disztlr=0
'If set_open<>1 Then
					end if
					lvw.cols.items(19+sttrInx).selid = 1063
'If set_open<>1 Then
					lvw.cols.items(19+sttrInx).edit = 1
'If set_open<>1 Then
					lvw.cols.items(19+sttrInx).save = 1
'If set_open<>1 Then
					lvw.cols.items(19+sttrInx).ywname = title
'If set_open<>1 Then
					lvw.cols.items(20+sttrInx).edit = 0
'If set_open<>1 Then
					lvw.cols.items(20+sttrInx).save = 0
'If set_open<>1 Then
					lvw.cols.items(20+sttrInx).disztlr = 0
'If set_open<>1 Then
					lvw.cols.items(20+sttrInx).htmlvisible = false
'If set_open<>1 Then
					lvw.cols.items(20+sttrInx).resize = false
'If set_open<>1 Then
					lvw.cols.items(21+sttrInx).edit = 0
'If set_open<>1 Then
					lvw.cols.items(21+sttrInx).save = 0
'If set_open<>1 Then
					End Select
					rss.movenext
				wend
			...
			rss.close
			lvw.candr = not cn.execute("select 1 from [power] where sort1 = 36 and sort2 = 20 and ord = " & app.info.user & " and qx_open =1").eof
			lvw.formula = "if(〖"&dbnumName&"〗*1<0){【"&dbnumName&"】=(0)};if(〖现有库存〗-〖"&dbnumName&"〗< 0){【"&dbnumName&"】=(〖现有库存〗)}"
			Response.write lvw.innerHTML
			Response.write "<br>" & vbcrlf & "                 </div>" & vbcrlf & "          </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <table style='table-layout:fixed;width:100%'>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class=""resetTransparent"" style='background-image:url(../../images/m_table_b.jpg);padding-top:5px'>" & vbcrlf & "                 <table align=center style='width:100%;text-align:center;'>" & vbcrlf & "                      <tr>" & vbcrlf & "                            <td><button class=button onclick='savedb(1)'>确认调拨</button></td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <script language=javascript src='../../Manufacture/inc/dateCalender.js?ver="
			Response.write Application("sys.info.jsver")
			Response.write "'></script><!-- 日期选择组件 -->" & vbcrlf & "    <script language=javascript src='../../inc/UrlEncode.js?ver="
			'Response.write Application("sys.info.jsver")
			Response.write Application("sys.info.jsver")
			Response.write "'></script>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "        //判断是不是辅助数量触发的格式" & vbcrlf & "          function checkDBNname(colname,InputDBName)" & vbcrlf & "      {" & vbcrlf & "           if(colname==InputDBName && InputDBName==""指定辅助数量"") return false;" & vbcrlf & "             return true;"& vbcrlf & "          }" & vbcrlf & "       function GetDataCellIndexByTD(obj)" & vbcrlf & "      {           " & vbcrlf & "                return 1;" & vbcrlf & "           }" & vbcrlf & "           ListViewInit();" & vbcrlf & "         var menu_returnResult = menu.returnResult       //重新函数" & vbcrlf & "              //var lvw_editfocus = lvw.editfocus     //重写函数" & vbcrlf & "" & vbcrlf & "          window.lvwoldfocus = lvw.editfocus" & vbcrlf & "          lvw.editfocus = function(CurrTd){" & vbcrlf & "                       if(!CurrTd){return false}" & vbcrlf & "                       var cellBody = CurrTd" & vbcrlf & "                   if (CurrTd.className.length==0){return false}" & vbcrlf & "" & vbcrlf & "                   if(CurrTd.className!=""full""){" & vbcrlf & "                            if(CurrTd.children.length==0){return false;}" & vbcrlf & "                            if(CurrTd.children[0].tagName!=""TABLE""){return false;}" & vbcrlf & "                            var cellBody = CurrTd.children[0].rows[0].cells[0];" & vbcrlf & "                     }" & vbcrlf & "                       var td = cellBody.parentElement.parentElement.parentElement.parentElement;" & vbcrlf & "                  if(td.cellIndex == 8 && td.parentElement.cells.length>13){  // 表示是调拨数量编辑格子" & vbcrlf & "                           var tr = td.parentElement;" & vbcrlf & "                              var hdIndex = lvw.getDataRowIndexByTR(tr);" & vbcrlf & "                              if(tr.cells[13].outerHTML.indexOf(""随机"")<0){"& vbcrlf &"                                     return false; "& vbcrlf &"                            } "& vbcrlf &"                        } "& vbcrlf & vbcrlf &"                        return window.lvwoldfocus(CurrTd); "& vbcrlf &"               } "& vbcrlf & vbcrlf &"                //lvw.editfocus = function(td){ "& vbcrlf &"          //      lvw_editfocus(td) "& vbcrlf &"                //} "& vbcrlf &"              menu.returnResult= function(dat,box){" & vbcrlf & "                       var txt = menu.handText" & vbcrlf & "                 if(txt.className==""ctllvwCurrInput"" && dat[0]==""指定"" && dat[1]==1){" & vbcrlf & "                                var div = window.DivOpen(""xsaadax"",""精确指定调拨"",980,500,'aa','aa',1,20)" & vbcrlf & "                           var tr = window.getParent(txt,8);" & vbcrlf &"                         if(tr.tagName==""TABLE"") tr =  window.getParent(txt,6);" & vbcrlf & "                            var sn = tr.cells[tr.cells.length-3].innerText;" & vbcrlf & "                         var attr1 = lvw.getCellValue(tr.cells[8]);" & vbcrlf & "                              var attr2 = lvw.getCellValue(tr.cells[9]);" & vbcrlf & "                              var ProductAttr1 = 0 ,ProductAttr2 = 0 " & vbcrlf & "                                if(attr1.length>0) ProductAttr1 = attr1.split(""~"")[1];" & vbcrlf & "                            if(attr2.length>0) ProductAttr2 = attr2.split(""~"")[1];" & vbcrlf & "                            sn +=""_"" + ProductAttr1 + ""_"" + ProductAttr2;" & vbcrlf & "                               var nm = tr.cells[10].innerText;" & vbcrlf & "                                var config = tr.cells[14].innerText; //已保存的指定信息" & vbcrlf & "                          ajax.regEvent(""getzzdbpage"")" & vbcrlf & "                              ajax.addParam(""sn"",sn) //标示产品的标识" & vbcrlf & "                           ajax.addParam(""nm"",nm) //指定调拨数量" & vbcrlf & "                             ajax.addParam(""config"",config) //指定调拨数量" & vbcrlf & "                             div.innerHTML = ajax.send();" & vbcrlf& "                               lvw.UpdateAllScroll();" & vbcrlf & "                          window.currHandRow = tr;" & vbcrlf & "                                return;" & vbcrlf & "                 }" & vbcrlf & "                       menu_returnResult(dat,box);" & vbcrlf & "             }" & vbcrlf & "" & vbcrlf & "               window.getdddbdata = function(){" & vbcrlf & "                        var nRow = new Array()" & vbcrlf & "                  var div = document.getElementById(""listview_ddcf"");" & vbcrlf & "                 var dat = lvw.GetSaveDetailData(div); //1454#oc15.000#oc11#oc0.000#oc55#oc#oc" & vbcrlf & "                   var rows = dat.split(""#or"")" & vbcrlf & "                       var sumNumber = 0" & vbcrlf & "                       for(var i = 0 ; i < rows.length ; i++ ){" & vbcrlf & "                                var cells = rows[i].split(""#oc"")" & vbcrlf & "                          if(cells[2].length==0){cells[2] = 0 ;}" & vbcrlf & "" & vbcrlf & "                          if(isNaN(cells[2])) {" & vbcrlf & "                                   alert(""第 "" + (i+1) + ""行，调拨数不正确。"")" & vbcrlf & "                                 return;" & vbcrlf & "                         }" & vbcrlf & "                               if(cells[1]-cells[2]<0) {" & vbcrlf & "                       alert(""第 "" + (i+1) + ""行，调拨数不能大于库存。"")" & vbcrlf & "                                   return;" & vbcrlf & "                         }" & vbcrlf & "                               if(cells[1]<0) {" & vbcrlf & "                                        alert(""第 "" + (i+1) + ""行，库存不足，无法调拨。"")" & vbcrlf & "                                   return;" & vbcrlf & "                         }" & vbcrlf & "                               if(cells[2]>0){" & vbcrlf & "                                 nRow[nRow.length] = cells.join(""_"")" & vbcrlf & "                                  sumNumber = sumNumber*1 + cells[2]*1" & vbcrlf & "                            }" & vbcrlf & "                       }" & vbcrlf & "                       lvw.updateDataCell(window.currHandRow.cells[10],sumNumber.toFixed(window.floatnumber));" & vbcrlf & "                 lvw.updateDataCell(window.currHandRow.cells[13],'指定' + lvw.sBoxSpr + '1');" & vbcrlf & "                   lvw.updateDataCell(window.currHandRow.cells[14],nRow.join(""|""));" & vbcrlf & "                  lvw.Refresh(document.getElementById(""listview_pdlist""));" & vbcrlf & "                  window.DivClose(window.event.srcElement);" & vbcrlf & "                       window.currHandRow = null;" & vbcrlf & "              }" & vbcrlf & "" & vbcrlf & "              function savedb(){" & vbcrlf & "                      var $editor = jQuery('.ctllvwCurrInput');" & vbcrlf & "                       if ($editor.size()>0){" & vbcrlf & "                          if($editor.val().length>200){" & vbcrlf & "                                   $editor[0].select();" & vbcrlf & "                                    alert('字段长度不能超过200！');" & vbcrlf & "                                 return;" & vbcrlf & "                         }" & vbcrlf & "                   }" & vbcrlf & "" & vbcrlf & "                       //Task.1213.KILLER.2013.12.16 调拨保存提示顺序需要调整 ,输入无效仓库提示" & vbcrlf & "                        var dat = UrlEncode(lvw.GetSaveDetailData(document.getElementById('listview_pdlist')));         " & vbcrlf & "                        var rows = dat.split(""#or"");" & vbcrlf & "                      var str = '';" & vbcrlf & "                   var ii = 0;" & vbcrlf & "                      for (i=0;i<rows.length ;i++ )" & vbcrlf & "                   {" & vbcrlf & "                               rows[i] = rows[i].split(""#oc"");" & vbcrlf & "                           if(rows[i][8] == 0 || rows[i][8] == 'invalid')" & vbcrlf & "                          {                                       " & vbcrlf & "                                        if(ii>10) {str = str + '…………\r\n';break;}" & vbcrlf & "                                   ii = ii + 1;"& vbcrlf &"                                     str = str +'第'+(i+1)+'行，请选择正确的入库仓库。\r\n';                         " & vbcrlf & "                                }                               " & vbcrlf & "                        }" & vbcrlf & "                       " & vbcrlf & "                        if(str.length > 0 )" & vbcrlf & "                     {" & vbcrlf & "                               alert(str);" & vbcrlf & "                             return false;" & vbcrlf & "                   }" & vbcrlf & "" & vbcrlf & "                       if (!window.confirm(""确定要调拨吗，该操作将立即生效？"")) { return false }" & vbcrlf & "                    if (!window.ajaxbaseurl) { window.ajaxbaseurl = ajax.url; }" & vbcrlf & "            ajax.url = window.ajaxbaseurl.split(""/SYSA/"")[0] + ""/SYSN/view/store/kumove/kjhzdb.save.ashx"";" & vbcrlf & "                       ajax.regEvent("""");" & vbcrlf & "                        for(var i=1;i<5;i++){" & vbcrlf & "                         if(i<4){document.getElementById(""MT"" + i + ""_msg"").innerHTML = """"}" & vbcrlf & "                            ajax.addParam(""MT"" + i,document.getElementById(""MT"" + i).value)" & vbcrlf & "                     }" & vbcrlf & "                       ajax.addParam(""dat"",dat);" & vbcrlf & "                 ajax.addParam(""ID"","
			'Response.write Application("sys.info.jsver")
			Response.write top
			Response.write ");" & vbcrlf & "                   ajax.exec();" & vbcrlf & "                    ajax.url = window.ajaxbaseurl;" & vbcrlf & "          }" & vbcrlf & "" & vbcrlf & "               window.top.TreeDataSelect = function(dat,dr){" & vbcrlf & "                   //产品名称,出库仓库,产品单位,调拨数量,入库仓库 0-3-1-2-4" & vbcrlf & "                        var dats = dat.join(""$"")" & vbcrlf & "                  ajax.regEvent(""addChild"");" & vbcrlf & "                       ajax.addParam(""datArray"",dats);" & vbcrlf & "                   ajax.addParam(""hasKuArray"",GetHasKuIdArray());" & vbcrlf & "                    ajax.addParam(""dr"",dr ? 1 : 0);" & vbcrlf & "                   ajax.send(getnewlist)" & vbcrlf & "                   ajax.showprocc();" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "       window.lvwCellValue = function(v){" & vbcrlf & "                      v = v.split(lvw.sBoxSpr)" & vbcrlf & "                        if(v.length==2){" & vbcrlf & "                                return v[1].length>0 ? v[1] : v[0]" & vbcrlf & "                      }" & vbcrlf & "                       else{" & vbcrlf & "                           return v[0];" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "               window.isRepeat = function(div,cells){ //判断是否充分" & vbcrlf & "                   for(var i=0;i<div.hdataArray.length;i++){" & vbcrlf & "                               var row = div.hdataArray[i]" & vbcrlf & "                             var ku = row[row.length-1] //ku" & vbcrlf & "" & vbcrlf & "                         var ok1 = (cells[cells.length-1] == lvwCellValue(ku));" & vbcrlf & "                          if(ok1 == true){" & vbcrlf & "                                     return i;" & vbcrlf & "                               }" & vbcrlf & "                       }" & vbcrlf & "                       return -1;" & vbcrlf & "              }" & vbcrlf & "               function GetHasKuIdArray(){" & vbcrlf & "                     var r = """"" & vbcrlf & "                        var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    for(var i=0;i<div.hdataArray.length;i++){" & vbcrlf & "                             var row = div.hdataArray[i]" & vbcrlf & "                             var hsEdit = row.toString()" & vbcrlf & "                             r = r + "","" + row[row.length - 2].replace(lvw.sBoxSpr,"""") //ku" & vbcrlf & "                      }" & vbcrlf & "                       return r" & vbcrlf & "                }" & vbcrlf & "" & vbcrlf & "               window.getnewlist = function(r){" & vbcrlf & "                 var rowIndex = 0" & vbcrlf & "                        var hsCount = 0 , addCount = 0" & vbcrlf & "                  var div = document.getElementById('listview_pdlist')" & vbcrlf & "                    var rows = r.split(""$="")" & vbcrlf & "                  for(var i = 0 ; i < rows.length ; i++){" & vbcrlf & "                         if(rows[i].length>1){" & vbcrlf & "                                      var cells = rows[i].split(""|-"")" & vbcrlf & "                                   cells.splice(cells.length-1,1)" & vbcrlf & "                                  addCount = addCount + 1" & vbcrlf & "                                 div.hdataArray[div.hdataArray.length] = (""+%;$++%;$+"" + cells.join(""+%;$+"")).split(""+%;$+"") //lvw.addDataRow(div,cells);" & vbcrlf & "                           }" & vbcrlf & "                       }" & vbcrlf & "                       div.PageStartIndex = div.hdataArray.length - div.PageSize + 1" & vbcrlf & "                   div.PageStartIndex = div.PageStartIndex > 0 ? div.PageStartIndex : 1" & vbcrlf & "                    div.PageEndIndex = div.hdataArray.length" & vbcrlf & "                        lvw.Refresh(div);" & vbcrlf & "       lvw.UpdateScrollBar(div);" & vbcrlf & "                       if(hsCount > 0){" & vbcrlf & "                                alert(""有"" + hsCount + ""条记录已经存在，没有重复添加。"")" & vbcrlf & "                    }" & vbcrlf & "               }" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "               window.formulCallBack = lvw.formulaApply" & vbcrlf & "                lvw.formulaApply = function(tb,tr){" & vbcrlf & "                     window.formulCallBack(tb,tr);" & vbcrlf & "                   var currKUID = tr.cells[tr.cells.length-2].innerText;" & vbcrlf & "           }" & vbcrlf & "" & vbcrlf & "               function HideButton(){" & vbcrlf & "                  if(!document.getElementById(""listtoolbar_pdlist""))return;" & vbcrlf & "                 var buttons = document.getElementById(""listtoolbar_pdlist"").getElementsByTagName(""button"")" & vbcrlf & "                    for(var i = 0 ; i < buttons.length ; i ++){" & vbcrlf & "                             if(buttons[i].title==""导出表格(Excel)"" || buttons[i].title==""统计图示""){" & vbcrlf & "                                    buttons[i].style.display = ""none""" & vbcrlf & "                         }" & vbcrlf & "                       }" & vbcrlf & "           }" & vbcrlf & "               HideButton();" & vbcrlf & "" & vbcrlf & "           jQuery(function(){" & vbcrlf & "                      var remark = document.getElementById(""MT4"");" & vbcrlf & "                      remark.posHeight=remark.scrollHeight>34?remark.scrollHeight:34;" & vbcrlf & "         });" & vbcrlf & "" & vbcrlf & "             $(document.getElementById(""MT4"")).trigger(""change"");" & vbcrlf & "    </script>" & vbcrlf & "</body>" & vbcrlf & ""
		end sub
...
's = s & c
		...
		sub App_addChild
			dim dat , dats , cell , sql , dr
			dat = request.form("datArray")
			dr = abs(request.form("dr"))
			cn.execute("select 10000 as cpID , 10000 as ckID,cast(null as int) as d_unit, cast(null as int) as d_attr1, cast(null as int) as d_attr2, cast(null as float) as d_num, cast(null as int) as d_rkid,cast('' as ntext) as d_remark,cast(null as varchar(50)) as zdy1,cast(null as varchar(50)) as zdy2,cast(null as varchar(50)) as zdy3,cast(null as varchar(50)) as zdy4,cast(null as int) as zdy5,cast(null as int) as zdy6 into #ttt;delete from #ttt;select cast('' as varchar(24)) as sn into #ttt2;delete from #ttt2")
			set rs = server.CreateObject("adodb.recordset")
			dats = split(dat,"$")
			rs.open "select * from #ttt" , cn , 1 ,3
			set rss = server.CreateObject("adodb.recordset")
			set rss = cn.execute("select title,name,set_open from zdymx where sort1=1003 and name like 'zdy%' order by gate1 asc")
			for i = 0 to ubound(dats)
				...
				if dr = 1 then
					rs.fields(2).value = cell(2)
					rs.fields(3).value = cell(3)
					rs.fields(4).value = cell(4)
					rs.fields(5).value = cell(5)
					rs.fields(6).value = cell(6)
					rs.fields(7).value = replace(replace(cell(7),"#-x01","|"),"#-x02","$")
'rs.fields(6).value = cell(6)
					On Error Resume Next
					m=8
					n=0
					do until rss.eof
						if rss("set_open")=1 then
							rs.fields(m).value = replace(replace(cell(8+n),"#-x01","|"),"#-x02","$")
'if rss("set_open")=1 then
							n=n+1
'if rss("set_open")=1 then
						end if
...
'if rss("set_open")=1 then
			...
			dats = split(dat,",")
			rs.open "select sn from #ttt2" , cn , 1 ,3
			for i = 1 to ubound(dats)
				rs.addnew
				rs.fields(0).value = dats(i)
				rs.update
			next
			rs.close
			If dr<>1 Then dr=0
			sql =       "exec erp_ck_kjhzdb_list " & app.info.User & "," & dr
			set rs = cn.execute(sql)
			Dim sValue
			while not rs.eof
				for i = 0 to rs.fields.count-1
'while not rs.eof
					...
					If i=8 Or i= rs.fields.count-1 Then
						sValue=rs.fields(i).value
		...
		sub App_getzzdbpage
			dim sn , nm , cp , unit , ck , ckname , config, item , ProductAttr1,ProductAttr2
			sn = split(request.form("sn") , "_")
			cp = sn(0) : unit = sn(1) : ck = sn(2) : ProductAttr1 = sn(3) : ProductAttr2 = sn(4)
			nm = request.form("nm")
			config = request.form("config")
			set rs = cn.execute("select a.title , a.order1 , a.type1 , (select top 1 sort1 from  sortck  where ord=" & ck & ") as ck,(select top 1 sort1 from sortonehy c where gate2=61 and c.ord=" & unit & ") as 单位   from product a where  a.del=1 and  ord = " & cp)
			if rs.eof then
				rs.close
				exit sub
			end if
			Response.write "" & vbcrlf & "      <table style='color:#444455;height:100%;table-layout:fixed;width:100%;word-break: keep-all'>" & vbcrlf & "    <tr>" & vbcrlf & "           <td rowspan=2 style='width:10px;height:16px'></td>" & vbcrlf & "              <td style='height:16px;width:12%;text-align:right'>产品编号：</td>" & vbcrlf & "              <td style='height:16px;width:22%'><div style='height:16px;overflow:hidden'>"
			Response.write rs.fields("order1").value
			Response.write "</div></td>" & vbcrlf & "          <td style='height:16px;width:12%;text-align:right'>产品名称：</td>" & vbcrlf & "              <td style='height:16px;width:23%'><div style='height:16px;overflow:hidden'>"
			Response.write rs.fields("order1").value
			Response.write rs.fields("title").value
			Response.write "</div></td>" & vbcrlf & "          <td style='height:16px;width:11%;text-align:right'>型号：</td>" & vbcrlf & "          <td style='height:16px;width:18%'><div style='height:16px;overflow:hidden'>"
			Response.write rs.fields("title").value
			Response.write rs.fields("type1").value
			Response.write "</div></td>" & vbcrlf & "   </tr>" & vbcrlf & "    <tr>" & vbcrlf & "          <td style='height:16px;text-align:right'>仓库名称：</td>" & vbcrlf & "                <td style='height:16px' title='"
			Response.write rs.fields("type1").value
			Response.write rs.fields("ck").value
			Response.write "'><div style='height:16px;overflow:hidden'>"
			Response.write rs.fields("ck").value
			Response.write "</div></td>" & vbcrlf & "          <td style='height:16px;text-align:right'>调拨总数：</td>" & vbcrlf & "                <td style='height:16px;color:red'>"
			Response.write rs.fields("ck").value
			Response.write nm
			Response.write "</td>" & vbcrlf & "                <td style='height:16px;text-align:right'>单位：</td>" & vbcrlf & "            <td style='height:16px;'>"
			Response.write nm
			Response.write rs.fields("单位").value
			Response.write "</td>" & vbcrlf & "         </tr>" & vbcrlf & "   <tr>" & vbcrlf & "           <td colspan=8 valign=top style='height:380px'>" & vbcrlf & "          "
			fzunit=GetAssistUnitTactics()
			ConversionUnitTactics=GetConversionUnitTactics()
			fzunitShow=fzunit=1
			rs.close
			cn.execute("select cast(0 as int) as ckid , cast(0 as money) as  nm,cast(0 as decimal(25,12)) as  fznum  into #ttt " & vbcrlf & " delete from #ttt")
			config = request.form("config")
			if len(config)>0 then
				config = split(config,"|")
				set rs = server.CreateObject("adodb.recordset")
				rs.open "select ckid,nm,fznum from #ttt" , cn , 1,3
				for I = 0 to ubound(config)
					item = split(config(i),"_")
					rs.addnew
					rs.fields(0).value = item(0)
					rs.fields(1).value = item(2)
					if item(3)&""=""then
						rs.fields(2).value=0
					else
						rs.fields(2).value = item(3)
					end if
					rs.update
				next
				rs.close
			end if
			dim mbit: mbit = sdk.getSqlvalue("select num1 from setjm3 where ord = 2019042803",4)
			dim numdot: numdot = sdk.getSqlvalue("select num1 from setjm3 where ord = 88",4)
			dim attrfields , inxAttr
			inxAttr = 0
			if isOpenProductAttr then
				set rs = ProductAttrsByOrd(cp)
				while rs.eof=false
					if rs("isTiled") = 1 then
						attrfields = attrfields & " s1.title ["& rs("title")&"],"
					else
						attrfields = attrfields & " s2.title ["& rs("title")&"],"
					end if
					inxAttr = inxAttr + 1
					attrfields = attrfields & " s2.title ["& rs("title")&"],"
					rs.movenext
			...
			dim hasCBPower : hasCBPower = sdk.getsqlValue("select isnull(qx_open,0) from power where sort1=23 and sort2=2 and ord = "& app.info.User  , 0 )
			set lvw = new listview
			dim cmdText
			cmdText ="  select " & vbcrlf & _
			"      a.id ," & attrfields & " ph as 批号,xlh as 序号,convert(varchar(10),daterk,120) as 入库日期," & vbcrlf & _
			"dbo.formatnumber(price1,& mbit &,0) as 单价, num2 as  现有库存,cast(dbo.formatnumber(a.num2*a.AssistNum/a.num3,& numdot &,0) as nvarchar(max))+(select sort1 from sortonehy where gate2=61 and id=a.AssistUnit) as 现有辅助库存, cast(b.nm as int) as 指定数量,  "& vbcrlf & _
			"       b.fznum as 指定辅助数量,a.num3 初始库存,a.AssistNum 辅助数量,(select sort1 from sortonehy where gate2=61 and id=a.AssistUnit)as 辅助单位 "  & vbcrlf & _
			"   from ku a "&_
			"   left join #ttt b on a.id=b.ckid  "&_
			"   left join Shop_GoodsAttr s1 on s1.id = a.ProductAttr1 "&_
			"   left join Shop_GoodsAttr s2 on s2.id = a.ProductAttr2 "&_
			"   where num2>0 and ord = " & cp & " and ck=" & ck  & " and unit=" & unit
			if ProductAttr1&""="" then ProductAttr1 = 0
			if ProductAttr2&""="" then ProductAttr2 = 0
			cmdText = cmdText & " and isnull(a.ProductAttr1,0) = " & ProductAttr1
			cmdText = cmdText & " and isnull(a.ProductAttr2,0) = " & ProductAttr2
			lvw.sql = cmdText & "   order by daterk"
			lvw.pagetype = "script"
			lvw.showtool = 0
			lvw.border = 0
			lvw.autosum = 0
			...
			lvw.canadd = false
			lvw.id = "ddcf"
			lvw.cols.items(1).htmlvisible = false
			for n = 2 to 7 + inxAttr
				lvw.cols.items(1).htmlvisible = false
				lvw.cols.items(n).edit = false
				lvw.cols.items(n).save = false
				lvw.cols.items(n).htmlvisible = true
				if n = 5+inxAttr and hasCBPower&"" <>"1" then
					lvw.cols.items(n).htmlvisible = true
					lvw.cols.items(n).htmlvisible = false
				end if
			next
			lvw.cols.items(6 + inxAttr).save = true
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(7 + inxAttr).htmlvisible = fzunitShow
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(9 + inxAttr).htmlvisible = fzunitShow
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(9 + inxAttr).save = true
			lvw.cols.items(n).htmlvisible = false
			if ConversionUnitTactics=1 then lvw.cols.items(9 + inxAttr).edit = false
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(10 + inxAttr).htmlvisible = false
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(11 + inxAttr).htmlvisible = false
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(12 + inxAttr).htmlvisible = fzunitShow
			lvw.cols.items(n).htmlvisible = false
			lvw.cols.items(12 + inxAttr).edit = false
			lvw.cols.items(n).htmlvisible = false
			lvw.formula = "【指定辅助数量】=【指定数量】*【辅助数量】/【初始库存】"
			Response.write lvw.innerHTML
			Response.write "" & vbcrlf & "              <td>" & vbcrlf & "     </tr>" & vbcrlf & "   <tr>" & vbcrlf & "           <td style='height:30px' colspan=8 align='center'><button class='button' style='width:70px' onclick='getdddbdata()'>确定</button>&nbsp;&nbsp;<button class='button'  style='width:70px' onclick='window.DivClose(this)'>取消</button></td>" & vbcrlf & "  </tr>" & vbcrlf & "   </table>" & vbcrlf & "" & vbcrlf & "       "
		end sub
		sub App_ListDrConfig(drdat)
			dim mFilePath
			mFilePath = "../../in/hzdb.xls"
			if isOpenProductAttr then mFilePath = "../../in/hzdb1.xls"
			drdat.title = "汇总调拨导入"
			drdat.fileName = "汇总调拨数据"
			drdat.filters = "xls|xlsx"
			drdat.smpFilePath = mFilePath
			drdat.helpFilePath = "../../in/hzdb.doc"
			drdat.remark = "请参考示例excel文件，确保导入的文件格式符合要求。"
			drdat.autosave = true
			drdat.allowSize = 25*1024*1024
			drdat.modelCls = "调拨信息"
		...
		sub App_lvw_InExcel
			...
			dim zdys,zdy5tit, zdy6tit, zdynames, zdyfields, i , zdycount,zdystr
			redim zdynames(0)
			redim zdyfields(0)
			i = 0
			zdystr=""
			set rs = cn.execute("select title,name from zdymx where sort1=1003 and set_open=1 and name like 'zdy%' order by gate1 asc")
			while rs.eof = false
				zdys = zdys & "," & rs.fields("title").value
				if rs.fields("name").value = "zdy5" then zdy5tit = "[" & rs.fields("title").value & "]"
				if rs.fields("name").value = "zdy6" then zdy6tit = "[" & rs.fields("title").value & "]"
				redim preserve zdynames(i)
				redim preserve zdyfields(i)
				zdynames(i) = "[" & rs.fields("title").value & "]"
				zdyfields(i) = rs.fields("name").value
				zdystr=zdystr&"+'|'+ cells["&i+6&"]"
'zdyfields(i) = rs.fields("name").value
				i = i + 1
'zdyfields(i) = rs.fields("name").value
				rs.movenext
			...
			db  = uploader.dbname
			if not uploader.CheckFields("产品名称,产品编号,产品型号,产品单位,"& mFields &"调拨数量,出库仓库,入库仓库,备注"&zdys) then
				exit sub
			end if
			...
			cn.execute "select up_index, ('产品名称:<b style=""color:#000088"">' + isnull(产品名称,'') + '</b>;产品型号:<b style=""color:#000088"">' + isnull(产品型号,'') + '</b>;产品编号:<b style=""color:#000088"">'+ isnull(产品编号,'') + '</b>') as 产品信息 into #pname from " & db
			cn.execute "create table #k_all (序号 int  IDENTITY(1,1) not null,内容 varchar(300), 说明 varchar(300))"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单位错误' from " & db & " where isnumeric(isnull(产品单位,0))=1"
			cn.execute "delete from " & db & " where isnumeric(isnull(产品单位,0))=1" , n
			cn.execute "update " & db & " set 产品单位 = u.ord from sortonehy u where u.gate2=61 and u.sort1 = 产品单位"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '单位错误' from " & db & " where isnumeric(产品单位)=0"
			cn.execute "delete from " & db & " where isnumeric(产品单位)=0" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测单位正确性','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & (n1+n) & "</b>条记录单位不正确。","") & "')"
			cn.execute "delete from " & db & " where isnumeric(产品单位)=0" , n1
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '产品名称不能为空' from " & db & " where len(rtrim(isnull(产品名称,''))) = 0"
			cn.execute "delete from " & db & " where len(rtrim(isnull(产品名称,''))) = 0" , n
			cn.execute "update " & db & " set 产品名称=ord, 产品型号='#s_xx_tok' from product x where  x.del=1 and  x.title = isnull(产品名称,'') and x.order1 = isnull(产品编号,'') and x.type1=isnull(产品型号,'')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '产品资料不存在' from " & db & " where isnull(产品型号,'') <> '#s_xx_tok'"
			cn.execute "delete from " & db & " where isnull(产品型号,'') <> '#s_xx_tok'" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测产品正确性','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & (n1+n) & "</b>条记录产品资料不正确。","") & "')"
			cn.execute "delete from " & db & " where isnull(产品型号,'') <> '#s_xx_tok'" , n1
			if len(zdy5tit) > 0 then
				cn.execute "update " & db & " set " & zdy5tit & "= cast(a.ord as varchar(12)) from sortonehy a where a.gate2=2101 and a.sort1=" & db & "." & zdy5tit
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '" & zdy5tit & "不匹配' from " & db & " where isnumeric(isnull(" & zdy5tit & ",'c'))=0 or cast(" & zdy5tit & " as float)<=0"
				cn.execute "delete from " & db & "  where isnumeric(isnull(" & zdy5tit & ",'c'))=0 or cast(" & zdy5tit & " as float)<=0" , n1
				cn.execute "insert into #k_all (内容,说明) values ('" & zdy5tit & "检测','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录" & zdy5tit & "不匹配。","") & "')"
			end if
			if len(zdy6tit) > 0 then
				cn.execute "update " & db & " set " & zdy6tit & "= cast(a.ord as varchar(12)) from sortonehy a where a.gate2=2102 and a.sort1=" & db & "." & zdy6tit
				cn.execute "insert into #k_fail (行号,失败原因) select up_index, '" & zdy6tit & "不匹配' from " & db & " where isnumeric(isnull(" & zdy6tit & ",'c'))=0 or cast(" & zdy6tit & " as float)<=0"
				cn.execute "delete from " & db & "  where isnumeric(isnull(" & zdy6tit & ",'c'))=0 or cast(" & zdy6tit & " as float)<=0" , n1
				cn.execute "insert into #k_all (内容,说明) values ('" & zdy6tit & "检测','" & app.iif(n1 > 0 ,"共<b style=""color:red"">" & (n1) & "</b>条记录" & zdy6tit & "不匹配。","") & "')"
			end if
			cn.execute  "declare @I int" & vbcrlf & _
			"set @I = 1 " & vbcrlf & _
			"select ord ,sort1 ,sort , cast(sort1 as varchar(7000)) as ckname into  #tmp from sortck d where charindex( '," & app.info.user & ",',','+replace(cast(d.intro as varchar(4000)),' ','')+',')>0 or replace(cast(d.intro as varchar(10)),' ','')='0'" & vbcrlf & _
			"set @I = 1 " & vbcrlf & _
			...
			cn.execute "update " & db & " set 出库仓库=ord , 产品型号='#s_ckck_tok'  from #tmp x where x.ckname = isnull(出库仓库,'')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '出库仓库不存在或无查看权限' from " & db & " where isnull(产品型号,'') <> '#s_ckck_tok'"
			cn.execute "delete from " & db & " where  产品型号<>'#s_ckck_tok'" , n
			cn.execute "update " & db & " set 入库仓库=ord , 产品型号='#s_rkck_tok'  from #tmp x where x.ckname = isnull(入库仓库,'')"
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '入库仓库不存在或无查看权限' from " & db & " where isnull(产品型号,'') <> '#s_rkck_tok'"
			cn.execute "delete from " & db & " where  产品型号<>'#s_rkck_tok'" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测仓库名称正确性','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & n & "</b>条记录出库仓库不正确，<b style=""color:red"">" & n1 & "</b>条记录入库仓库不正确","") & "')"
			cn.execute "delete from " & db & " where  产品型号<>'#s_rkck_tok'" , n1
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '调拨数量不正确' from " & db & " where isnumeric(isnull(调拨数量,'xx')) = 0"
			cn.execute "update " & db & " set 调拨数量=replace(cast(调拨数量 as nvarchar(15)),',','') where 调拨数量 is not null "
			cn.execute "delete from " & db & " where isnumeric(isnull(调拨数量,'xx')) = 0" , n
			cn.execute "delete from " & db & " where cast(调拨数量 as float) < 0" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测调拨数量正确性','" & app.iif((n +n1)> 0 ,"共<b style=""color:red"">" & (n+n1) & "</b>条记录调拨数量不正确","") & "')"
			cn.execute "delete from " & db & " where cast(调拨数量 as float) < 0" , n1
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '调拨数量小数位数不正确' from " & db & " where  (cast(调拨数量 as numeric(20,8))-cast(调拨数量 as numeric(20,"&num_dot_xs&")) > 0)"
			cn.execute "delete from " & db & " where cast(调拨数量 as float) < 0" , n1
			cn.execute "delete from " & db & " where  (cast(调拨数量 as numeric(20,8))-cast(调拨数量 as numeric(20,"&num_dot_xs&")) > 0)" , n
			cn.execute "delete from " & db & " where cast(调拨数量 as float) < 0" , n1
			cn.execute "insert into #k_all (内容,说明) values ('检测调拨数量小数位数','" & app.iif( n> 0 ,"共<b style=""color:red"">" & n & "</b>条记录调拨数量小数位数不正确","") & "')"
			dim attrText,attrInx,attrText2 , attrText3,attrText4 , showfields
			attrText = ""
			attrInx = 2
			attrText2 = ""
			attrText3 = ""
			attrText4 = ""
			showfields = " 0 产品属性1 , 0 产品属性2, "
			if isOpenProductAttr then
				showfields = " 产品属性1 ,产品属性2, "
			...
			"select distinct e.up_index , '没有库存可供调拨.' from " & db & " e left join ku a on a.ord=e.产品名称 and a.unit = e.产品单位 and a.ck=e.出库仓库 "& attrText3 &" group by a.ord , a.ck , a.unit, e.up_index  having sum(isnull(a.num2,0)) <=0 " , n
			cn.execute "insert into #k_fail (行号,失败原因)" & vbcrlf & _
			"select distinct e.up_index , '库存不足' from " & db & " e "&_
			"left join ku a on a.ord=b.产品名称 and a.unit = e.产品单位 and a.ck=e.出库仓库  "& attrText3 &_
			"group by a.ord ,e.调拨数量, a.ck , a.unit, e.up_index  "&_
			"having sum(isnull(a.num2,0)) >0 and sum(isnull(a.num2,0)) <e.调拨数量 " , n1
			cn.execute "delete from " & db & " where up_index in (select 行号 from #k_fail where 失败原因='库存不足')"
			cn.execute "insert into #k_all (内容,说明) values ('检测库存','" & app.iif(n1+n > 0 ,"共<b style=""color:red"">" & (n+n1) & "</b>条记录没有库存或库存不足。","") & "')"
			cn.execute "delete from " & db & " where up_index in (select 行号 from #k_fail where 失败原因='库存不足')"
			cn.execute "delete from " & db & " where up_index in (select 行号 from #k_fail where 失败原因='没有库存可供调拨.')"
			cn.execute "select x.up_index, y.up_index as cup_index into #overlist "&_
			"from "&db&" x "&_
			"left join "&db&" y on cast(x.产品名称 as nvarchar(500)) = cast(y.产品名称 as nvarchar(500)) "&_
			"                       and isnull(cast(x.出库仓库 as nvarchar(500)),'') = isnull(cast(y.出库仓库 as nvarchar(500)),'') "&_
			"                       and cast(x.产品单位 as nvarchar(500)) = cast(y.产品单位 as nvarchar(500)) "& attrText &" and x.up_index<>y.up_index "&_
			"where  y.up_index>0 "
			cn.execute "insert into #k_fail (行号,失败原因)  select up_index , '产品信息与出库仓库重复，无法精确匹配库存' as x from #overlist "
			cn.execute "insert into #k_fail (行号,失败原因) select up_index, '与第' + cast(cup_index as varchar(10))+ '行数据重复' from #overlist "
			cn.execute "insert into #k_fail (行号,失败原因)  select up_index , '产品信息与出库仓库重复，无法精确匹配库存' as x from #overlist "
			cn.execute "delete from " & db & " where up_index in (select up_index from #overlist)" , n
			cn.execute "insert into #k_all (内容,说明) values ('检测重复数据','" & app.iif(n > 0 ,"共<b style=""color:red"">" & n & "</b>条数据重复","") & "')"
			...
			set rs = cn.execute("select 产品名称,产品单位,"& showfields &"调拨数量,出库仓库,入库仓库,备注"&zdys&" from " & db)
			while not rs.eof
				for i = 0 to rs.fields.count -1
'while not rs.eof
			...
			rs.close
			Response.write "</div>"
			Response.write "" & vbcrlf & "     <script language=javascript>" & vbcrlf & "            function cdata()" & vbcrlf & "                {" & vbcrlf & "                       var div =  window.parent.parent.lvwexceldrdiv;" & vbcrlf & "                  var lvw =  window.parent.parent.lvw;" & vbcrlf & "                    if(!div){alert(""无法找到相关联的列表"");return;}" & vbcrlf & "                   lvw.clear(div);" & vbcrlf & "                  var rows = document.getElementById(""data_"
			Response.write db
			Response.write """).innerHTML.split(""$~#"")" & vbcrlf & "                   var dat = new Array();" & vbcrlf & "                  for(var i=0; i< rows.length;i++){" & vbcrlf & "                               if(rows[i].length>0){" & vbcrlf & "                                   var cells = rows[i].split(""^*#"")" & vbcrlf & "                                  dat[dat.length] = cells[0] + ""|"" + cells[3] + ""|"" + cells[1] +""|"" + cells[2] + ""|"" + cells[4] + ""|"" + cells[5].replace(""|"",""#-x01"").replace(""$"",""#-x02"")"
			Response.write db
			Response.write zdystr
			Response.write ";" & vbcrlf & "                            }" & vbcrlf & "                       }" & vbcrlf & "                       window.top.TreeDataSelect(dat,1);" & vbcrlf & "               }" & vbcrlf & "               setTimeout(""cdata();"",1000);" & vbcrlf & "      </script>" & vbcrlf & "       "
			uploader.showReport
