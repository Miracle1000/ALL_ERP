...
'Private Sub Class_Initialize
	...
	Response.write "" & vbcrlf & "" & vbcrlf & "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN""" & vbcrlf & """http://www.w3.org/TR/html4/loose.dtd"">" & vbcrlf & "<html>" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>"
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body{" & vbcrlf & "      margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".ser_btn input[type='text']{height:20px;line-height:20px;}" & vbcrlf & ".ser_btn select{position:relative;top:1px;*top:0}" & vbcrlf & "</style></head>" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & ""
	...
	if typ="3333333" or typ="5555555" then
		...
		if typ="3333333" and timelg<>"" then
			noalt1=" and date6>'"&timelg&"'"
		end if
		if open_6_1=3 then
			Str_Result_43=""
		elseif open_6_1=1 then
			Str_Result_43=" and cateid in ("&intro_6_1&")"
		else
			Str_Result_43=" and cateid=0"
		end if
		Str_Result="and (cateid_sp="&session("personzbintel2007")&" and sp>0 and del=1 and complete in (7,11) "&Str_Result_43&" )  "&noalt1&""
	end if
...
'if W4<>"" and not isnull(W4) then
	...
	if typ="0" then
		Str_Result=Str_Result&" and (complete=11 or complete=8 or complete=7 or complete=12)"
	elseif typ="1" then
		str_Result = str_Result + " and complete=11"
'elseif typ="1" then
	elseif typ="2" then
		str_Result = str_Result + " and complete=7"
'elseif typ="2" then
	elseif typ="3" then
		str_Result = str_Result + " and complete=8 "
'elseif typ="3" then
	elseif typ="4" then
		str_Result = str_Result + " and complete=12 "
'elseif typ="4" then
	elseif typ="711" then
		str_Result = str_Result + " and complete in (7,11)"
'elseif typ="711" then
	elseif typ<>"3333333" and typ<>"5555555" then
		str_Result = str_Result + " and cateid_sp="&typ&" and complete in (7,11) "
'elseif typ<>"3333333" and typ<>"5555555" then
	end if
	if title<>"" then
...
'if intro<>"" then
	...
	if money1<>"" then
		if isnumeric(money1)=false then
			Response.write("<script>alert('请输入合法金额');window.location=('fhlist.asp');</script>")
			call db_close : Response.end
		else
	...
	if money2<>"" then
		if isnumeric(money2)=false then
			...
			str_Result = str_Result + "and  money2 >="&money2&" "
			call db_close : Response.end
		end if
	end if
	if money22<>"" then
		if isnumeric(money22)=false then
			...
			str_Result = str_Result + "and  money2 <="&money22&" "
			call db_close : Response.end
		end if
	end if
	if tjr<>"" then
		...
		str_Result = str_Result + "and  addcate in ("&perid&") "
'if perid="" or isnull(perid) then perid=0
	end if
	if jkr<>"" then
...
'if perid="" or isnull(perid) then perid=0
	...
	if D<>"" then
		if A="zt" then
			Str_Result=Str_Result&" and  title like '%"&D&"%' "
		elseif A="gy" then
			str_Result = str_Result + "and intro like '%"&D&"%' "
'elseif A="gy" then
		elseif A="hkr" then
...
'if trim(perid)<>"" then
				...
				str_Result = str_Result + "and cateid=-1 "
'if trim(perid)<>"" then
			end if
		elseif A="spyj" then
			tr_Result=Str_Result+"and intro2 like "&D&" "
'elseif A="spyj" then
		end if
	end if
	if m1<>"" then
		str_Result = str_Result + "and  date1>='"&cdate(m1)&"'"
'if m1<>"" then
	end if
	if m2<>"" then
		str_Result = str_Result + "and  date1<='"&cdate(m2)&"'"
'if m2<>"" then
	end if
	If remind <> "" Then
	...
	elseif px=3 then
		pxResult=" order by date6 desc"
	elseif px=4 then
		pxResult=" order by date6 asc"
	elseif px=5 then
		pxResult=" order by date1 desc"
	elseif px=6 then
		pxResult=" order by date1 asc"
	elseif px=7 then
		pxResult=" order by date2 desc"
	elseif px=8 then
		pxResult=" order by date2 asc"
	end if
	Response.write "" & vbcrlf & "     <form name=""da"" id=""da"" action=""fhlist.asp?px="
	Response.write px
	Response.write "&page_count="
	Response.write page_count
	Response.write """ method=""get"">" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">         " & vbcrlf & "      <tr>" & vbcrlf & "        <td class=""place"">费用返还列表</td>" & vbcrlf & "        <td><A class='px_btn' onclick=""Myopen(User);return false;"" href=""javascript:void(0)"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "        <td align=""right"">" & vbcrlf & "                              <font id=""kh"">" & vbcrlf & "          <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" >" & vbcrlf & "            <option>-请选择-</option>" & vbcrlf & "            <option value=""page_count=10"" "
	Response.write page_count
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "          </select>" & vbcrlf & "        </font>" & vbcrlf & "        </td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32""/></td>" & vbcrlf & "      </tr>            " & vbcrlf & "    </table>  " & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "     <tr><td colspan=""12"" class='ser_btn' align=""right"" style=""border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding-top:0;padding-bottom:0;height:50px;"">" & vbcrlf & "                    "
	...
	Response.write ">全部</option>" & vbcrlf & "                               <option value=""3"" "
	if typ="3" then Response.write "selected"
	Response.write ">审批通过</option>" & vbcrlf & "                       <option value=""1"" "
	if typ="1" then Response.write "selected"
	Response.write ">待审批</option>" & vbcrlf & "                         <option value=""2"" "
	if typ="2" then Response.write "selected"
	Response.write ">审批中</option>                                             <option value=""4"" "
	...
	Response.write ">返还主题</option>" & vbcrlf & "                        <option value=""gy"" "
	if A="gy" then Response.write "selected"
	Response.write ">返还概要</option>" & vbcrlf & "                        <option value=""hkr"" "
	if A="hkr" then Response.write "selected"
	Response.write ">返还人</option>" & vbcrlf & "                          <!--<option value=""spyj"" "
'if A="hkr" then Response.write "selected"
	Response.write ">审批意见</option>-->" & vbcrlf & "                  </select>" & vbcrlf & "                       <input name=""D"" type=""text"" size=""10""  value="""
'if A="hkr" then Response.write "selected"
	Response.write D
	...
	if open_6_10=1 or open_6_10=3 then
		Response.write "" & vbcrlf & "                  <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_fh1.asp'})}"" class=""anybutton2""/>" & vbcrlf & "                  "
	end if
	...
	Response.write "</td>" & vbcrlf & "                        </tr>" & vbcrlf & "</form>" & vbcrlf & "             <form name=""form1"" method=""post"" action=""deleteall6.asp?"
	Response.write sdk.ReturnURL("")
	Response.write """>" & vbcrlf & "                        <tr  class=""top"">" & vbcrlf & "                         <td width=""5%"" align=""center"" height=""23"" ><div align=""center"">选择</div></td>" & vbcrlf & "                          <td width=""10%"" ><div align=""center"" ><strong>返还主题</strong></div></td>" & vbcrlf & "                          <td width=""10%"" ><div align=""center""><strong>返还金额</strong></div></td>" & vbcrlf & "                          <td width=""10%"" ><div align=""center"" ><strong>审批金额</strong></div></td>" & vbcrlf & "                          <td width=""6%"" ><div align=""center""><strong>返还人</strong></div></td>" & vbcrlf & "                              <td width=""6%"" ><div align=""center""><strong>审批人</strong></div></td>" & vbcrlf & "                <td width=""8%"" ><div align=""center""><strong>添加人</strong></div></td>" & vbcrlf & "                          <td width=""8%"" ><div align=""center""><strong>返还状态</strong></div></td>" & vbcrlf & "                            <td width=""8%"" ><div align=""center""><strong>返还日期</strong></div></td>" & vbcrlf & "                            <td width=""6%"" ><div align=""center""><strong>入账状态</strong></div></td>" & vbcrlf & "                         <td width=""6%"" ><div align=""center""><strong>入账人员</strong></div></td>" & vbcrlf & "                            <td width=""12%"" ><div align=""center"" ><strong>操作</strong></div></td>" & vbcrlf & "                        </tr>" & vbcrlf & "                 "
	dim n
	n=0
	all1=0
	all2=0
	...
	allm2=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from pay WITH(NOLOCK) where del=1 "&Str_Result&" "&pxResult
	rs.open sql,conn,1,1
	if not rs.eof then
		do while not rs.eof
			mo1=zbcdbl(rs("money1"))
			mo2=zbcdbl(rs("money2"))
			if mo1="" or isnull(mo1) then mo1=0
			if mo2="" or isnull(mo2) then mo2=0
			allm1=cdbl(mo1)+cdbl(allm1)
'if mo2="" or isnull(mo2) then mo2=0
			allm2=cdbl(mo2)+cdbl(allm2)
'if mo2="" or isnull(mo2) then mo2=0
			rs.movenext
		loop
	end if
	set rs=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select *,case when exists(select top 1 1 from  PrinterInfo a where a.sort = 20  and a.formid = pay.ord) then '<span style=''color:#009900''>[已打印]</span>' else '<span  style=''color:#FF0000''>[未打印]</span>' end  as printStatus from pay WITH(NOLOCK) where del=1 "&Str_Result&" "&pxResult
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		Response.write "<table><tr><td  align='center'>没有信息!</td></tr></table>"
	else
		...
		do until rs.eof
			dim k,k2,title
			title1=rs("title")
			if isnull(title1) then
				title1=""
			end if
			money11=zbcdbl(rs("money1"))
			money21=zbcdbl(rs("money2"))
			if isnull(money11) or money11="" then money11=0
			if isnull(money21) or money21="" then money21=0
			all1=cdbl(money11)+cdbl(all1)
'if isnull(money21) or money21="" then money21=0
			all2=cdbl(money21)+cdbl(all2)
'if isnull(money21) or money21="" then money21=0
			catename=setname("gate","ord",rs("cateid"),"name")
			spname=setname("gate","ord",rs("cateid_sp"),"name")
			addname=setname("gate","ord",rs("addcate"),"name")
			if isnull(spname) then spname=""
			if isnull(addname) then addname=""
			date1=rs("date1")
			jkid=rs("jkid")
			if jkid="" or isnull(jkid) then jkid=0
			bz=setname("payjk","id",rs("jkid"),"bz")
			if bz="" or isnull(bz) then bz=14
			trade8=selbz(bz)
			if trade8="" or isnull(trade8) then trade8="RMB"
			cateid=rs("cateid")
			if timelg<>"" and datediff("s",timelg,rs("date6"))>0 then
				title1=title1&new_list
			end if
			Response.write "" & vbcrlf & "                     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "       <td width=""5%"" align=""center""><span class=""red"">" & vbcrlf & "      "
			...
			Response.write "" & vbcrlf & "       </span></td> " & vbcrlf & "        <td height=""23"" align=""left""><div align=""left"">" & vbcrlf & "                "
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('content3.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','neww34win','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write pwurl(rs("ord"))
			...
			Response.write "<span style=""color:red""></span>        " & vbcrlf & "                </div></td>" & vbcrlf & "     <td ><div align=""right"">"
			Response.write trade8
			Response.write "&nbsp;"
			Response.write Formatnumber(money11,num_dot_xs,-1)
			Response.write "&nbsp;"
			Response.write "</div></td>" & vbcrlf & "          <td ><div align=""right"">"
			Response.write trade8
			Response.write "&nbsp;"
			Response.write Formatnumber(money21,num_dot_xs,-1)
			Response.write "&nbsp;"
			...
			rzStr = "未入账"
			rzname = ""
			sqltext="select * from bank WITH(NOLOCK) where sort=4 and gl2="&rs("ord")
			set rstxt=server.CreateObject("adodb.recordset")
			rstxt.open sqltext,conn,1,1
			if rstxt.eof = False  then
				fhbool=True
				rzStr = "已入账"
				rzname=setname("gate","ord",rstxt("cateid"),"name")
			end if
			complete = rs("complete")
			If complete&""="" Then complete = 0
			Select Case complete
			Case "7" : Response.write "审批中"
			Case "8" : Response.write "审批通过"
			Case "11": Response.write "待审批"
			Case "12": Response.write "审批未通过"
			End Select
			Response.write "</div></td>" & vbcrlf & "          <td ><div align=""center"">"
			Response.write date1
			Response.write "</div></td>" & vbcrlf & "          <td ><div align=""center"">"
			Response.write rzStr
			Response.write "</div></td>" & vbcrlf & "          <td ><div align=""center"">"
			Response.write rzname
			...
			if open_6_14=3 or CheckPurview(intro_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit53"" value=""详情""  onClick=""javascript:window.open('content3.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','neww34win','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "              "
				Response.write pwurl(rs("ord"))
			end if
			sqls="select * from sortbank where bz="&bz&" and person like '%"&session("personzbintel2007")&"%'"
			set rsbank=conn.execute(sqls)
			if not rsbank.eof then
				hasbank=true
			else
				hasbank=false
			end if
			if (open_6_21=3 or CheckPurview(intro_6_21,trim(cateid))=True) and fhbool=false and complete=8 and hasbank then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit33"" value=""入账""  onClick=""javascript:window.location=('fh1.asp?"
				Response.write sdk.ReturnURL("ord=" & rs("ord") &"")
				Response.write "')""/>" & vbcrlf & "             "
			end if
			if (open_6_16=3 or CheckPurview(intro_6_16,trim(cateid))=True) and trim(rs("cateid_sp"))=trim(session("personzbintel2007")) and (complete="7" or complete="11") then
				Response.write "" & vbcrlf & "        <input type=""button"" name=""Submit33"" value=""审批""  onClick=""javascript:window.location=('setfh.asp?ord="
				Response.write rs("ord")
				Response.write "&reflush=1&typ="
				Response.write typ
			...
			if open_6_3=3 or CheckPurview(intro_6_3,trim(cateid))=True then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete3.asp?"
				Response.write sdk.ReturnURL("ord=" & rs("ord") &"")
				Response.write "'}""/>" & vbcrlf & "             "
			end if
		...
		Response.write "&nbsp;"
		Response.write "</div>" & vbcrlf & "                               </td>" & vbcrlf & "                           <td colspan=""8"">&nbsp;</td>" & vbcrlf & "                       </tr>" & vbcrlf & "                   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "                     <td height=""27"" colspan=""2""><div align=""right"">所有合计：</div></td> " & vbcrlf & "                         <td  class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		...
		Response.write "&nbsp;"
		Response.write "</div>" & vbcrlf & "                         </td>" & vbcrlf & "                           <td colspan=""8"">&nbsp;</td>" & vbcrlf & "                       </tr>            " & vbcrlf & "      </table>" & vbcrlf & "  </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "       <td  class=""page"">        " & vbcrlf & "                <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "                       <tr>" & vbcrlf & "                          <td width=""10%"" height=""30"">" & vbcrlf & "                                        <div align=""center"">全选" & vbcrlf & "                                    <input name=""chkall"" type=""checkbox"" id=""chkall"" onClick=""mm()"" value=""chkall"">" & vbcrlf & "                                 </div>" & vbcrlf & "                          </td>" & vbcrlf & "                           <td >" & vbcrlf & "                          "
		...
		Response.write "" & vbcrlf & "                              </td>" & vbcrlf & "                           <td width=""79%"">" & vbcrlf & "                          <div align=""right"">" & vbcrlf & "                               "
		...
		if currpage=1 then
			Response.write "" & vbcrlf & "                                <input type=""button"" name=""Submit47"" value=""首页""  class=""page""/> <input type=""button"" name=""Submit42"" value=""上一页""  class=""page""/>" & vbcrlf & "                           "
		else
			Response.write "" & vbcrlf & "                                <input type=""button"" name=""Submit47"" value=""首页""   class=""page"" onClick=""gotourl('currPage=1')""/>" & vbcrlf & "                                <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""gotourl('currPage="
		...
		Response.write "" & vbcrlf & "                                </div>" & vbcrlf & "                                </td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   <script language=javascript>" & vbcrlf & "                  function test()" & vbcrlf & "                 {" & vbcrlf & "                         if(!confirm('确认删除吗？')) return false;" & vbcrlf & "                    }" & vbcrlf & "                        " & vbcrlf & "                       function mm()" & vbcrlf & "                 {" & vbcrlf & "                          var a = document.getElementsByTagName(""input"");" & vbcrlf & "                           var b=document.getElementById(""chkall"");" & vbcrlf & "                          if(b.checked==true)" & vbcrlf & "                          {" & vbcrlf & "                                       for (var i=0; i<a.length; i++)" & vbcrlf & "                                  {" & vbcrlf & "                                               if (a[i].type == ""checkbox"")" & vbcrlf & "                                          { a[i].checked = true;}" & vbcrlf & "                                 }" & vbcrlf & "                          }" & vbcrlf & "                       else" & vbcrlf & "                    {" & vbcrlf & "                                    for (var i=0; i<a.length; i++)" & vbcrlf & "                                  {" & vbcrlf & "                                               if (a[i].type == ""checkbox"")" & vbcrlf & "                                              {a[i].checked = false;}" & vbcrlf & "                                       }" & vbcrlf & "                          }" & vbcrlf & "                    }" & vbcrlf & "                       </script>" & vbcrlf & "                         <tr>" & vbcrlf & "                          <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;</p></td>" & vbcrlf & "                       </tr>" & vbcrlf & "          </table>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	Response.write "              " & vbcrlf & "        </td>  " & vbcrlf & "</tr>" & vbcrlf & "  </form>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""200""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""192"">" & vbcrlf & "        <table width=""150"" height=""192"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按返还主题排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按返还主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按返还日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按返还日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按审批日期排序(降)</font></a> </td>" & vbcrlf & "  </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按审批日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>   " & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<div id=""w"" style=""width:565px;height:450px;padding:5px;background: #fafafa;top:60px;display:none;"" class=""easyui-window"" title=""高级检索""   closed=""true"" >" & vbcrlf & "<form method=""get"" action=""fhlist.asp""id=""date1"" onSubmit=""return Validator.Validate(this,2)"" name=""date1"">" & vbcrlf & "<div id=""hideDiv"" style=""border-style: none; border-width: 0px; border-color: inherit; margin: 0px; display:none; padding:0px;"">" & vbcrlf & " <div region=""north"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "          <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "         <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "   </div>" & vbcrlf & "  <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:515px"">" & vbcrlf & "               <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                      <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                               <td width=""20%"" class=""func""><div align=""right"">返还标题</div></td>" & vbcrlf & "                           <td width=""80%""><label><input name=""title"" type=""text"" id=""title""></label></td>" & vbcrlf & "                 </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                               <td width=""20%""><div align=""right"">返还概要</div></td>" & vbcrlf & "                              <td width=""80%""><input name=""intro"" type=""text"" id=""intro""></td>" & vbcrlf & "                        </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td width=""20%""><div align=""right"">返还金额</div></td>" & vbcrlf & "                              <td width=""80%"">" & vbcrlf & "                                  <input name=""money1"" type=""text""  class=""easyui-numberbox"" id=""money1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision=""2""  value=""0"">" & vbcrlf & "                                        -" & vbcrlf & "                                      <input name=""money12"" type=""text"" id=""money12"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision=""2""  value=""999999999"">" & vbcrlf & "                          </td>" & vbcrlf & "                  </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                               <td width=""20%""><div align=""right"">审批金额</div></td>" & vbcrlf & "                              <td width=""80%"">" & vbcrlf & "                                  <input name=""money2"" type=""text""  class=""easyui-numberbox"" id=""money2"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision=""2""  value=""0"">" & vbcrlf & "                                 -" & vbcrlf & "                                       <input name=""money22"" type=""text"" id=""money22"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision=""2""  value=""999999999"">" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                               <td width=""20%""><div align=""right"">返还时间</div></td>" & vbcrlf & "                              <td width=""80%"">" & vbcrlf & "                               <INPUT name=""rett"" size=""9"" readonly id=""duepaydate1divPos"" onmouseup=toggleDatePicker(""duepaydate1div"",""date1.rett"") value="""
	Response.write m1
	Response.write """>" & vbcrlf & "                         <DIV id=""duepaydate1div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate1div""></DIV>-" & vbcrlf & "                         <INPUT name=""rett2"" size=""9"" readonly id=""duepaydate2divPos"" onmouseup=toggleDatePicker(""duepaydate2div"",""date1.rett2"") value="""
	'Response.write m1
	Response.write m2
	Response.write """>" & vbcrlf & "                         <DIV id=""duepaydate2div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate2div""></DIV>                           </td>" & vbcrlf & "                  </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                               <td width=""20%""><div align=""right"">返还状态</div></td>" & vbcrlf & "                               <td width=""80%"">" & vbcrlf & "                                  <select name=""typ"" id=""typ"">" & vbcrlf & "                                                <option value=""0"" "
	if typ="0" then Response.write "selected"
	Response.write ">全部</option>" & vbcrlf & "                                                <option value=""3"" "
	if typ="3" then Response.write "selected"
	Response.write ">审批通过</option>" & vbcrlf & "                                            <option value=""1"" "
	...
	Response.write ">待审批</option>" & vbcrlf & "                                              <option value=""2"" "
	if typ="2" then Response.write "selected"
	Response.write ">审批中</option>                          " & vbcrlf & "                                            <option value=""4"" "
	if typ="4" then Response.write "selected"
	Response.write ">审批未通过</option>" & vbcrlf & "                                  </select>" & vbcrlf & "                               </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td width=""20%""><div align=""right"">币种</div></td>" & vbcrlf & "                  <td width=""80%"">" & vbcrlf & "                              "
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	set rs8=nothing
	...
	set rs8=nothing
	Response.write "" & vbcrlf & "                     </select>" & vbcrlf & "                                </td>" & vbcrlf & "               </tr>" & vbcrlf & "                         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><div align=""right"">返还人员</div></td>" & vbcrlf & "                 <td width=""80%"">"
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	Response.write "</td>" & vbcrlf & "                           </tr>                           " & vbcrlf & "        </table>" & vbcrlf & "     </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "            <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit();"">确认</a>" & vbcrlf & "              <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "   </div>" & vbcrlf & "</div>" & vbcrlf & "</form>" & vbcrlf & "</div>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "    if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=20;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & ""
	if remind<>"" then
