...
'If Len(exists_Pics) = 0 Then
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		If remind <> "" Then
			Set helper = CreateReminderHelper(cn,57,0)
			remindSql = helper.listSQL("ids_withoutOrderBy")
		end if
		rpt.addfield "", "radio", "dtype","options:1=添加时间;2=开始时间","","2"
		rpt.addfield "自：", "datetimes", "date3", "", "", Split(",", ",")
		rpt.addfield "执行人：", "text", "catetxt", "", "", ""
		dim keytype : keytype = "洽谈进展" & Chr(1) & "1"
		keytype = keytype & Chr(2) & "日程总结" & Chr(1) & "2"
		If ZBRuntime.MC(1000) And ZBRuntime.MC(1001) Then keytype = keytype & Chr(2) & "关联客户" & Chr(1) & "4"
		If ZBRuntime.MC(3000) Then keytype = keytype & Chr(2) & "关联项目" & Chr(1) & "5"
		If ZBRuntime.MC(2000) Then keytype = keytype & Chr(2) & "联系人" & Chr(1) & "6"
		rpt.addfield2 "","select", "keytype" , keytype, "", "", "日程内容=0"
		rpt.addfield "", "text", "keytxt", "", "", ""
		rpt.aSearch.addField "人员列表", "gates", "ry",""
		If s <> "1" And s <> "2" And s <> "5" Then
			rpt.aSearch.addField2 "日程状态", "checks", "zt","options:1=待完成;2=已延误;3=已完成" ,""
			if s="6" or s="7" then
				dim chance : chance = request("chance")
				if s="6" then
					s = 10000000+cdbl(s) + cdbl(chance)
'if s="6" then
				else
					s = -1 * (cdbl(s) + cdbl(chance))
'if s="6" then
				end if
			end if
		end if
		rpt.aSearch.addField "日程内容", "text", "nr",""
		rpt.aSearch.addField "洽谈进展", "text", "jz",""
		rpt.aSearch.addField "日程总结", "text", "zj",""
		If ZBRuntime.MC(1000) And ZBRuntime.MC(1001) Then rpt.aSearch.addField "关联客户", "text", "company",""
		If ZBRuntime.MC(3000) Then rpt.aSearch.addField "关联项目", "text", "xm",""
		If ZBRuntime.MC(2000) Then rpt.aSearch.addField "联系人", "text", "lxr",""
		rpt.aSearch.addfield "添加时间","datetime","date7",""
		rpt.aSearch.addfield "开始时间","datetime","date1",""
		rpt.aSearch.addfield "截止时间","datetime","date2",""
		rpt.beforeBtnsHTML = "<style>#sfields_dtype table{*margin-top:2px} .ser_btn input.oldbutton{height:28px;line-height:26px;}</style>"
'rpt.aSearch.addfield "截止时间","datetime","date2",""
		rpt.sql = "exec erp_plan_list "&info.user&",@dtype, @date3_0,@date3_1,@catetxt,@keytype,@keytxt,@D,@ry,@zt,@nr,@jz,@zj,@company,@xm,@lxr,@date7_0,@date7_1,@date1_0,@date1_1,@date2_0,@date2_1," & s & ",@page,@v2,'" & Replace(remindSql,"'","''") & "','" & session("timezbintel2007") & "',@newstate,@propm,&SearchMode,&excelmode,&PageSize,&PageIndex"
		dim bats
		if app.power.existsPower(71,3) then
			bats = "批量删除"
		end if
		rpt.setBatHandle "ord", bats
		rpt.canexcel = app.power.existsPower(71,10)
		rpt.canprint = app.power.existsPower(71,7)
	end sub
	Sub app_onCreateList(lvw, rptdata)
		Dim s, timelg
		timelg = session("timezbintel2007")
		s = rptdata.GetInt("s")
		lvw.checkboxwidth=5
		lvw.headers("stime").title = "开始时间"
		lvw.headers("etime").title = "截止时间"
		lvw.headers("intro").title = "日程安排"
		lvw.headers("intro").dbtype = "str"
		lvw.headers("intro").cansort = false
		lvw.headers("intro2").title = "洽谈进展"
		lvw.headers("intro2").cansort = false
		lvw.headers("intro2").dbtype = "str"
		lvw.headers("dianping").title = "上级点评"
		lvw.headers("dianping").cansort = false
		lvw.headers("dianping").dbtype = "str"
		lvw.headers("complete").title = "日程状态"
		If s = 3 Then
			lvw.headers("zxr").display = "none"
			lvw.headers("zpr").title = "指派人"
		else
			lvw.headers("zpr").display = "none"
			lvw.headers("zxr").title = "执行人"
		end if
		lvw.headers("id").title = "操作"
		lvw.headers("stime").width = 100
		lvw.headers("etime").width = 100
		lvw.headers("intro").width = 280
		lvw.headers("zxr").width = 70
		lvw.headers("complete").width = 70
		lvw.headers("intro").formattext = "code: IntroCell(@value,rs(""id""),rs(""canView""),rs(""order1""),rs(""date8""),""" & s & """,""" & timelg & """)"
		lvw.headers("intro2").formattext = "code: Intro2Cell(@value,rs(""canAdd""),rs(""canJZCorrect""),rs(""id""),rs(""numdel""))"
		lvw.headers("dianping").formattext = "code: dianpingCell(rs(""jtdate""),rs(""cateid""),rs(""canView""))"
		lvw.headers("complete").formula = "ZTCell(@value,rs(""etime""))"
		lvw.headers("id").formattext = "code: HandleCell(@value,rs(""canView""),rs(""canDel""),rs(""canCorrect""),rs(""cateid""),rs(""jtdate""))"
		lvw.headers("ord").display = "none"
		lvw.headers("cateid").display = "none"
		lvw.headers("order1").display = "none"
		lvw.headers("numdel").display = "none"
		lvw.headers("date1").display = "none"
		lvw.headers("canView").display = "none"
		lvw.headers("canDel").display = "none"
		lvw.headers("canCorrect").display = "none"
		lvw.headers("canJZCorrect").display = "none"
		lvw.headers("canAdd").display = "none"
		lvw.headers("jtdate").display = "none"
		lvw.headers("date8").display = "none"
		lvw.excelcallbackproc= "OnExcelDC"
	end sub
	sub OnExcelDC(byval lvw)
		Dim rs
		lvw.headers("intro").title = "日程内容"
		lvw.headers("intro").width = 230
		lvw.headers("intro").dbtype = "str"
		lvw.headers("stime").title = "开始时间"
		lvw.headers("stime").width = 110
		lvw.headers("etime").title = "截止时间"
		lvw.headers("etime").width = 110
		lvw.headers("intro2").title = "洽谈进展"
		lvw.headers("intro2").dbtype = "str"
		lvw.headers("complete").title = "日程状态"
		lvw.headers("complete").width = 70
		lvw.headers("zxr").title = "执行人"
		lvw.headers("zxr").width = 70
		lvw.headers("zxr").dbtype = "str"
		lvw.headers("当天总结").dbtype = "str"
		Set rs = cn.execute("select charName from sp_schedule ORDER BY intLevel, id")
		While rs.eof = False
			lvw.headers("AAA"& rs("charName") &"").dbtype = "str"
			lvw.headers("AAA"& rs("charName") &"").title = rs("charName")
			rs.movenext
		wend
		rs.close
		set rs = nothing
	...
	Sub doDelete(values, batResult)
		dim rs, sql, intro_old, picUrl, picUrlArray ,x
		intro_old = ""
		set rs=server.CreateObject("adodb.recordset")
		sql="select intro from plan1 Where ord in (" & values & ") "
		rs.open sql,cn,1,1
		while rs.eof = False
		...
		cn.execute("Delete plan1 Where ord in (" & values & ") ")
	end sub
	Sub App_DoDel
		dim id, sql, rs, intro_old, picUrl, picUrlArray ,x
		id = request.Form("id")
		set rs=server.CreateObject("adodb.recordset")
		sql="select intro from plan1 Where ord=" & id & " "
		rs.open sql,cn,1,1
		if rs.eof then
			intro_old=""
		else
		...
		cn.execute("Delete plan1 Where ord=" & id & " ")
		Response.write("true")
	end sub
	Function IntroCell(ByVal intro, ByVal id, ByVal canView,ByVal order1, ByVal date8, ByVal s, ByVal timelg)
		IntroCell = "<div class='ewebeditorImg_plan' style='text-align: left;'>"
'Function IntroCell(ByVal intro, ByVal id, ByVal canView,ByVal order1, ByVal date8, ByVal s, ByVal timelg)
		If canView = "1" Then
			IntroCell = IntroCell & "<a href=""javascript:;"" onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(id) & "','newwin321','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此日程详情"">"
'If canView = "1" Then
		end if
		If order1 & "" <> "" And order1 & "" <> "0" Then
			IntroCell = IntroCell &"<span class=""red"">"
		else
			IntroCell = IntroCell &"<span class=""name"">"
		end if
		If s = "3333333" And timelg <> "" Then
			If datediff("s", timelg, date8) > 0 Then
				IntroCell = IntroCell & "<span style='background: url(../images/hot-new.gif) no-repeat;height: 6px;cursor: pointer;padding-left: 22px'>&nbsp;</span>"
'If datediff("s", timelg, date8) > 0 Then
			end if
		end if
		IntroCell = IntroCell & intro & "</span>"
		If canView = "1" Then IntroCell = IntroCell & "</a>"
		IntroCell = IntroCell & "</div>"
	end function
	Function Intro2Cell(ByVal intro2, ByVal canAdd, ByVal canJZCorrect,ByVal id, ByVal num_del)
		Intro2Cell = "<div "
		Select Case canAdd
		Case "-1"
'Select Case canAdd
		Intro2Cell = Intro2Cell & "title='您没有操作权限，只允许执行人和指派人操作！'>"
		Case "1"
		Intro2Cell = Intro2Cell & "><img src='../images/jiantou.gif' complete='complete'/><a title=""点击添加洽谈进展"" href='javascript:void(0)' onclick=""javascript:window.open('replyhome.asp?ord=" & app.base64.pwurl(id) & "','plancor1235','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" style='cursor:pointer;'>添加</a>"
		Case "2"
		If canJZCorrect&""="1" Then
			Intro2Cell = Intro2Cell & ">"
		else
			Intro2Cell = Intro2Cell & "title='" & num_del & "小时后不能再对以前的日程进行任何修改！'>"
		end if
		Case Else
		Intro2Cell = Intro2Cell & ">"
		End Select
		If canJZCorrect Then
			Intro2Cell = Intro2Cell &  "<a href='javascript:;' onClick=""javascript:window.open('../plan/correctreply.asp?ord="& id &"','newwinxg','width=' + 760 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;"">"& intro2 &"</a>"
'If canJZCorrect Then
		else
			Intro2Cell = Intro2Cell &  intro2
		end if
		Intro2Cell = Intro2Cell &"&nbsp;</div>"
	end function
	Dim intro_71_16
	Function dianpingCell(ByVal jtdate, ByVal cateid, ByVal canView)
		Dim ret, sql, rs, date1, charSpId
		ret = "<div class='ewebeditorImg_plan'>" : date1 = jtdate'dateadd("d",1,jtdate)
		sql = "select top 1 c.ord, c.intro from sp_schedule a "&_
		"          inner join ( "&_
		"                  select lcb,spid,date1,cateid,MAX(ord) ord from plan2 where type=17 and datediff(dd, date1,'"& date1 &"')=0 and cateid= " & cateid &_
		"                  group by lcb,spid,date1,cateid "&_
		"          ) b on b.lcb=a.intLevel and CHARINDEX(','+cast(b.spid as varchar(10))+',',','+a.charSpId+',')>0 "&_
		"                  group by lcb,spid,date1,cateid "&_
		"          inner join plan2 c on c.ord=b.ord "&_
		"          order by a.intLevel desc"
		Set rs = cn.execute(sql)
		If rs.eof = False Then
			If canView = "1" Then
				ret = ret & "<a href=""javascript:;"" title=""查看评定""  onClick=""javascript:window.open('../china/tophome1.asp?cateid=" & cateId & "&jtdate=" & jtdate & "','newwin666','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& rs("intro") &"</a>"
			else
				ret = ret & rs("intro")
			end if
		end if
		rs.close
		set rs = nothing
		If isEmpty(intro_71_16) Then
			intro_71_16 = app.power.GetPowerIntro(71,16)
		end if
		if (intro_71_16="" or instr(","& intro_71_16 &"," , ","& cateid &",")>0) Then
			sql = "select isnull(b.ord,0) dpord, a.intLevel, a.charSpId from sp_schedule a "&_
			"          left join ( "&_
			"                  select lcb,spid,date1,cateid,MAX(ord) ord from plan2 where type=17 and datediff(dd, date1,'"& date1 &"')=0 and cateid= " & cateid  &_
			"                  group by lcb,spid,date1,cateid "&_
			"          ) b on b.lcb=a.intLevel and CHARINDEX(','+cast(b.spid as varchar(10))+',',','+a.charSpId+',')>0 order by a.intLevel"
			Set rs = cn.execute(sql)
			Do While rs.eof = False
				If rs("dpord")=0 Then
					charSpId = rs("charSpId")&""
					charSpId = Replace(charSpId," ","")
					If InStr(","& charSpId &",", ","& Info.User &",")>0 Then
						ret = ret & "<img src='../images/jiantou.gif' complete='complete'/><a title=""点击添加评定"" href=""javascript:;"" onClick=""javascript:window.open('../plan/addhome2.asp?date1="& date1 &"&cateid="& cateid &"&spid="& Info.User &"&lcb="& rs("intLevel") &"&H=17&time1=11&divID=mlistvw','_blank','width=' + 800 + ',height=' + 500 + ',fullscreen=no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">评定</a>"
'If InStr(","& charSpId &",", ","& Info.User &",")>0 Then
						Exit Do
					end if
				end if
	...
	Function ZTCell(byval zt, ByVal etime)
		Select Case zt
		Case "1"
		If isdate(etime) then
			If CDate(etime) > now() Then
				ZTCell = "<font color=""#CC3333"">待完成</font>"
			else
				ZTCell = "<font color=""#CC3333"">已延误</font>"
			end if
		else
			ZTCell = ""
		end if
	...
	Function HandleCell(ByVal id, ByVal canView, ByVal canDel, ByVal canCorrect, ByVal cateId, ByVal jtdate)
		HandleCell = ""
		If canView = "1" Then
			HandleCell = HandleCell & "<input type=""button"" class=""oldbutton2"" name=""Submit3"" onClick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(id) & "','newwin777','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""详情"">"
			HandleCell = HandleCell & "<input type=""button"" class=""oldbutton2"" name=""Submit3"" title=""查看评定""  onClick=""javascript:window.open('../china/tophome1.asp?cateid=" & cateId & "&jtdate=" & jtdate & "','newwin666','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""查看"">"
		end if
		If canCorrect = "1" Then
			HandleCell = HandleCell & "<input type=""button"" class=""oldbutton2"" name=""Submit3"" onClick=""javascript:window.open('correct.asp?ord=" & app.base64.pwurl(id) & "','newwin21','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value=""修改"">"
		end if
		If canDel = "1" Then
			HandleCell = HandleCell & "<input type=""button"" class=""oldbutton2"" onclick=""ListAction.DoDel('" & id & "')"" value=""删除"">"
		end if
		If Len(HandleCell) = 0 Then
			HandleCell = " "
		else
			HandleCell = Replace(HandleCell,"><","> <")
		end if
	end function
