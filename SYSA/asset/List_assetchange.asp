...
'function IsTime(values)
	...
	if open_105_1=3 then
		list=""
	elseif open_105_1=1 then
		list=" and H_addcateid in ("&intro_105_1&")"
	else
		list=" and H_addcateid=0"
	end if
	Str_Result=" where H_del=1 "&list&""
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and H_addcateid in ("&W3&") "
	end if
	...
	if D1<>"" then
		Str_Result=Str_Result&" and H_type in ("&D1&")"
	end if
	if D2<>"" then
	...
	if D10<>"" then
		Str_Result=Str_Result&" and H_title like '%"&ml.De(D10)&"%'"
	end if
...
'if D12<>"" then
	...
	if D18<>"" then
		Str_Result=Str_Result&" and W_pz in ("&D18&")"
	end if
	if D19<>"" then
		Str_Result=Str_Result&" and W_code like '%"&ml.De(D19)&"%'"
	end if
	if D20<>"" then
	...
	if D21<>"" then
		Str_Result=Str_Result&" and W_zy like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and W_note like '%"&ml.De(D22)&"%'"
	end if
	M=ChangeDate(M,1)
	if M<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&"',H_time)>=0 "
'if M<>"" then
	end if
	K=ChangeDate(K,2)
	if K<>"" then
		Str_Result=Str_Result+" and  datediff(s,H_time,'"&K&"')>=0 "
'if K<>"" then
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&"',H_time)>=0 "
'if L1<>"" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,H_time,'"&L2&"')>=0 "
'if L2<>"" then
	end if
	L3=ChangeDate(L3,1)
	if L3<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L3&"',H_addtime)>=0 "
'if L3<>"" then
	end if
	L4=ChangeDate(L4,2)
	if L4<>"" then
		Str_Result=Str_Result+" and  datediff(s,H_addtime,'"&L4&"')>=0 "
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	if px="1" then
		Ptr_Result=" order by H_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by H_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by H_time desc"
	elseif px="4" then
		Ptr_Result=" order by H_time asc"
	elseif px="5" then
		Ptr_Result=" order by H_title desc"
	elseif px="6" then
		Ptr_Result=" order by H_title asc"
	end if
	ret=ChangeDate(ret,1)
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',H_addtime)>=0 "
'if ret<>"" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,H_addtime,'"&ret2&"')>=0 "
'if ret2<>"" then
	end if
	if C<>"" then
		if B="name" then
		...
		elseif B="zt" then
			Str_Result=Str_Result&" and H_title like '%"&ml.De(C)&"%'"
		end if
	end if''
	...
	Response.write ">存放地址</option>" & vbcrlf & "                           <option value=""zt"" "
	if B="zt" then Response.write "selected"
	Response.write ">变动主题</option>" & vbcrlf & "                   </select>&nbsp;<input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	...
	if open_105_10=3 or (open_105_10=1 and CheckPurview(intro_105_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_assetchange.asp?px="
		Response.write px
	...
	Response.write "" & vbcrlf & "                     <a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动主题</div></td>" & vbcrlf & "                  <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动资产</div></td>" & vbcrlf & "                <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动时间</div></td>" & vbcrlf & "            <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动类型</div></td>" & vbcrlf & "                <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动代码</div></td>" & vbcrlf & "                <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">变动摘要</div></td>" & vbcrlf & "               <td width=""18%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}""  class=""reseetTextColor"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                          <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.ass_name,c.W_name,W_code,W_zy from O_assChange a left join O_asset b on a.H_assID=b.ass_id left join O_assChangeWay c on a.H_type=c.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			addcateid=rs7("H_addcateid")
			H_cateid_sp=rs7("H_cateid_sp")
			H_complete=rs7("H_complete")
			...
			if open_105_14=3 or CheckPurview(intro_105_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_assetchange.asp?ord="
				Response.write rs7("id")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("id")
				Response.write rs7("H_title")
			...
			Response.write rs7("H_time")
			Response.write "</div></td>" & vbcrlf & "             <td width=""14%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("W_name")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("W_code")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("W_zy")
			Response.write "</div></td>" & vbcrlf & "             <td width=""18%"" height=""30"" align=""left"" valign=""middle""><div align=""center"">" & vbcrlf & "             "
			if (open_105_16=3 or CheckPurview(intro_105_16,trim(addcateid))=True) and (H_cateid_sp=session("personzbintel2007")) and (H_complete="1" or H_complete="2") then
				Response.write "" & vbcrlf & "                <input name=""Submit33"" type=""button"" class=""anybutton2""  onClick=""javascript:window.location=('Set_sp2.asp?ord="
				Response.write pwurl(rs7("id"))
				Response.write "')"" value=""审批""/>" & vbcrlf & "             "
			end if
			if open_105_14=3 or CheckPurview(intro_105_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""anybutton2"" id=""submit3"" onClick=""javascript:window.open('Detail_assetchange.asp?ord="
				Response.write rs7("id")
			...
			if (open_105_2=3 or CheckPurview(intro_105_2,trim(addcateid))=True)  and H_complete="1" then
				Response.write "" & vbcrlf & "                     <input name=""button4"" type=""button""  class=""anybutton2"" id=""submit4"" onClick=""javascript:window.open('Update_assetchange.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
			...
			if (open_105_3=3 or CheckPurview(intro_105_3,trim(addcateid))=True) then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""anybutton2"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_assetchange.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write ml.Dep(px)
				Response.write "&A="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按变动时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按变动时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按变动主题排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按变动主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="变动列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;z-index:2001;display:none;padding:2px 2px 2px 2px;background-color:lightblue"">" & vbcrlf & "<iframe src="""" name=""adsIF"" id=""adsIF"" width=""195px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<form target=""ssIF"" action=""../asset/Search_All_Search.asp"" id=""ssIFForm"" style=""display:none"" method=""post"">" & vbcrlf & "</form>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function adSearch(obj,sel_type)" & vbcrlf & "{" & vbcrlf & "   " & vbcrlf & "        var tleft=obj.offsetLeft;" & vbcrlf & "     var ttop=obj.offsetTop;" & vbcrlf & " var theight=obj.clientHeight;" & vbcrlf & "   while (obj = obj.offsetParent){ttop+=obj.offsetTop; tleft+=obj.offsetLeft;}" & vbcrlf & "     var sdivobj=document.getElementById(""adsDiv"");" & vbcrlf & "    sdivobj.style.display=""none"";" & vbcrlf &" sdivobj.style.left=tleft+""px"";" & vbcrlf & "    sdivobj.style.top=ttop+theight+""px"";" & vbcrlf & "      document.getElementById(""ssIFForm"").action=""../asset/Search_All_Search.asp?sel_type=""+sel_type;" & vbcrlf & "     document.getElementById(""ssIFForm"").target=""adsIF"";" & vbcrlf & " document.getElementById(""ssIFForm"").submit();" & vbcrlf & "      sdivobj.style.display=""inline"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "  }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_assetchange.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action=""Del_assetchange.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
