...
'If Not rs.Eof Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "</head>" & vbcrlf & "<body >" & vbcrlf & ""
	if action="update" then
		ordLi=trim(request.Form("ordLi"))
		typeid=trim(request.Form("typeid"))
		if typeid="" then call jsBack(inName2&"不能为空")
		if ordLi="" then call jsBack("序号丢失")
		ordLi=replace(ordLi," ","")
		typeid=replace(typeid," ","")
		ordArr=split(ordLi,",")
		typeArr=split(typeid,",")
		for i=0 to ubound(ordArr)
			if isnumeric(ordArr(i)) and isnumeric(typeArr(i)) then
				conn.execute "update sms_temp_sort set isopen="&typeArr(i)&" where ord="&ordArr(i)&" and del=1"
			end if
		next
		call jsLocat("保存成功","sms_temp_sort.asp")
	end if
	Response.write "" & vbcrlf & "<style>" & vbcrlf & "    #content>tbody>tr>td{border:0!important;}" & vbcrlf & "    #content table.exOrder{border-collapse:collapse;}" & vbcrlf & "    #content table.exOrder>tbody tr>td{border:1px solid #c0ccdd;}" & vbcrlf & "</style>" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">短信提醒设置</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "</table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""0"" bgcolor=""transparent"" id=""content""> " & vbcrlf & "        "
	Dim wxGoods,gdzc,invoice
	wxGoods = ZBRuntime.MC(76000)
	gdzc = ZBRuntime.MC(51004)
	invoice=ZBRuntime.MC(23004)
	set rs=server.CreateObject("adodb.recordset")
	sql="SELECT * from sms_temp_sort WHERE del = 1 AND (ord <> 9 OR (ord = 9 AND '"& wxGoods &"' = 'true') ) AND (ord <> 10 OR (ord = 10 AND '"& gdzc &"' = 'true')) and  (ord <> 11 OR (ord = 11 AND '"& invoice &"' = 'true') )"
	rs.open sql,conn,1,1
	if not rs.eof then
		k=1
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td width=""100%""  height=""26"" class=""name"">短信提醒分类列表</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td  height=""26"" class=""name""><table width=""100%"" border=""0"" cellpadding=""4"" bgcolor=""#C0CCDD"" class=""exOrder"">" & vbcrlf & "              <tr class=""top"">" & vbcrlf & "                <td width=""16%"" align=""center"" style=""""><div align=""center"">序号</div></td>" & vbcrlf & "                <td width=""31%"" align=""center"" style=""""><div align=""center"">"
		Response.write inName1
		Response.write "</div></td>" & vbcrlf & "                <td width=""25%"" align=""center"" style=""""><div align=""center"">"
		Response.write inName2
		Response.write "</div></td>" & vbcrlf & "                <td width=""28%"" align=""center"" style=""""><div align=""center"">"
		Response.write inName3
		Response.write "</div></td>" & vbcrlf & "              </tr>" & vbcrlf & "              <form method=""post"" name=""demo""  action=""sms_temp_sort.asp?action=update"" id=""demo"" >" & vbcrlf & "                "
		do while not rs.eof
			T_title=rs("title")
			T_isopen=rs("isopen")
			T_addTime=rs("addTime")
			T_ord=rs("ord")
			Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                  <td align=""center"">"
			Response.write k
			Response.write "</td>" & vbcrlf & "                  <td align=""left"">"
			Response.write T_title
			Response.write "</td>" & vbcrlf & "                  <td align=""center""><select name=""typeid"">" & vbcrlf & "                      <option "
			if T_isopen="" then
				Response.write "selected"
			end if
			Response.write ">请选择</option>" & vbcrlf & "                      <option value=""0"" "
			if T_isopen="0" then
				Response.write "selected"
			end if
			Response.write ">关闭</option>" & vbcrlf & "                                          "
			If T_ord <> 10 Then
				Response.write "" & vbcrlf & "                      <option value=""1"" "
				if T_isopen="1" then
					Response.write "selected"
				end if
				Response.write ">自动</option>" & vbcrlf & "                                          "
			end if
			If T_ord <> 9 Then
				...
				Response.write ">手动</option>" & vbcrlf & "                                          "
			end if
			Response.write "" & vbcrlf & "                    </select>" & vbcrlf & "                    <input type=""hidden"" name=""ordLi"" value="""
			Response.write T_ord
			Response.write """></td>" & vbcrlf & "                  <td align=""center"">&nbsp;"
			Response.write T_addTime
			Response.write "&nbsp;</td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
			k=k+1
			Response.write "&nbsp;</td>" & vbcrlf & "                </tr>" & vbcrlf & "                "
			rs.movenext
		loop
		Response.write "" & vbcrlf & "                <tr>" & vbcrlf & "                  <td colspan=""4"" align=""center""><input type=""submit"" class=""anybutton2"" name=""Submit"" value=""保 存"">" & vbcrlf & "                    &nbsp;&nbsp;&nbsp;" & vbcrlf & "                    <input type=""reset"" class=""anybutton2"" name=""Submit2"" value=""重 置""></td>" & vbcrlf & "                </tr>" & vbcrlf & "              </form>" & vbcrlf & "            </table></td>" & vbcrlf & "        </tr>" & vbcrlf & "        "
	end if
