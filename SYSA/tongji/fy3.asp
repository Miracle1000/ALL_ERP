...
'bigsystemtype = ".mozi"
	...
	if open_6_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_6_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "#echarts{margin-top:125px;}" & vbcrlf & "</style>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf &" <!-- "& vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0 "& vbcrlf & "eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf & "if (restore) selObj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "//-->" & vbcrlf & "</script>"& vbcrlf & "</head>" & vbcrlf & vbcrlf & "<body oncontextmenu=self.event.returnValue=false  onMouseOver=""window.status='none';return true;""  scroll=AUTO>" & vbcrlf & "" & vbCrLf
	Str_Result2=""
	Str_Result=""
...
'if m1<>"" then
		Str_Result2=Str_Result2+"and a.date1>='"&m1&"' "
'if m1<>"" then
		Str_Result3=Str_Result3+"and a.datejk>='"&m1&" 00:00:00' "
'if m1<>"" then
		Str_Result4=Str_Result4+"and c.bxdate >='"&m1&" 00:00:00' "
'if m1<>"" then
		Str_Result5=Str_Result5+"and a.sp_date>='"&m1&"' "
'if m1<>"" then
		m1name="  起始："&m1
	end if
	if m2<>"" then
...
'if m2<>"" then
		Str_Result2=Str_Result2+"and a.date1<='"&cdate(m2)&"' "
'if m2<>"" then
		Str_Result3=Str_Result3+"and a.datejk<='"&cdate(m2)&" 23:59:59' "
'if m2<>"" then
		Str_Result4=Str_Result4+"and a.datepay <='"&cdate(m2)&" 23:59:59' "
'if m2<>"" then
		Str_Result5=Str_Result5+"and a.sp_date <='"&cdate(m2)&"' "
'if m2<>"" then
		m2name="  截止："&m2
	end if
	if open_6_11=3 then
	elseif open_6_11=1 then
		Str_Result=Str_Result+"and  a.cateid in ("&intro_6_11&")"
'elseif open_6_11=1 then
		Str_Result2=Str_Result2+"and  a.cateid in ("&intro_6_11&")"
'elseif open_6_11=1 then
		Str_Result4=Str_Result4+"and  b.cateid in ("&intro_6_11&")"
'elseif open_6_11=1 then
		Str_Result3=Str_Result3+"and  a.sorce2 in ("&intro_6_11&")"
'elseif open_6_11=1 then
		Str_Result5=Str_Result5+"and  a.cateid in ("&intro_6_11&")"
'elseif open_6_11=1 then
	else
		Str_Result=Str_Result+"and  a.cateid=0"
'elseif open_6_11=1 then
		Str_Result2=Str_Result2+"and  a.cateid=0"
'elseif open_6_11=1 then
		Str_Result4=Str_Result4+"and  b.cateid=0"
'elseif open_6_11=1 then
		Str_Result3=Str_Result3+"and  a.sorce2=0"
'elseif open_6_11=1 then
		Str_Result5=Str_Result5+"and  a.cateid=0"
'elseif open_6_11=1 then
	end if
	...
	Dim SysTongJiJoinPageToTop : SysTongJiJoinPageToTop = "1"
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">费用统计</td>" & vbcrlf & "                       " & vbcrlf & "            <td  align=""left""><strong>"
	Response.write sort
	Response.write sort1
	Response.write "人员分布"
	Response.write m1name
	Response.write m2name
	Response.write "" & vbcrlf & "    </strong>" & vbcrlf & "                 </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><form method=""get"" action=""fy3.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"" style=""display:inline"">" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "<td>选择统计条件：" & vbcrlf & "<input type=""button"" name=""selectfy"" value=""费用分类""  class=""anybutton2"" id=""selectfy"" onClick=""document.getElementById('w').style.display='';$('#w').window('open');""/>" & vbcrlf & ""
	...
	Response.write "'>"
	Response.write "<input name=""k5"" type=""checkbox"" value=""5""  "
	if k5<>"0" then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "申请" & vbcrlf & "<input name=""k2"" type=""checkbox"" value=""2""  "
	if k2<>"0" then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "使用" & vbcrlf & "<input name=""k1"" type=""checkbox"" value=""1"" "
	if k1<>"0" then
		Response.write "checked"
	end if
	Response.write ">" & vbcrlf & "报销" & vbcrlf & ""
	if t<>1 then
		Response.write "" & vbcrlf & "<input name=""k4"" type=""checkbox"" value=""4""  "
		if k4<>"0" then
			Response.write "checked"
		end if
		Response.write ">" & vbcrlf & "借款" & vbcrlf & "<input name=""k3"" type=""checkbox"" value=""3""  "
		if k3<>"0" then
			Response.write "checked"
		end if
		Response.write ">" & vbcrlf & "返还" & vbcrlf & ""
	end if
	
	Response.write "" & vbcrlf & "<input name=""search""  type=""hidden"" size=""1"" value=""1"">" & vbcrlf & "<input name=""width_tj""  type=""hidden"" size=""0"" value="""
	Response.write width_tj
	Response.write """>" & vbcrlf & "<input type=""submit"" name=""Submit422"" value=""统计""  class=""anybutton2""/></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr><td>"
	Response.write "<input type=""hidden"" id=""sorttypes"" name=""sort1"" value="""
	Response.write request("sort1")
	Response.write """>" & vbcrlf & ""
	Dim rss ,rs1 ,typeStr ,typeArr, selStr ,typHtml, selHtml , typelist
	sql_sort = ""
	If ZBRuntime.MC(18500) = False Then
		sql_sort = sql_sort &" and isnull(id1,0) not in(5,7)"
	end if
	If ZBRuntime.MC(18900) = False Then
		sql_sort = sql_sort &" and isnull(id1,0)<>6"
	end if
	If ZBRuntime.MC(18700) = False Then
		sql_sort = sql_sort &" and isnull(id1,0)<>8"
	end if
	If ZBRuntime.MC(18610) = False Then
		sql_sort = sql_sort &" and isnull(id1,0)<>9"
	end if
	typeArr = Replace(request("sort1")," ","")
	count=conn.execute("select count(ord) from sortonehy where gate2=41 and del=1 "&sql_sort&"")(0)
	Dim ss() , Trs()
	Set rss=conn.execute("select ord,sort1 from sortonehy where gate2=41 and del=1 "&sql_sort&" order by gate1 desc")
	If rss.eof= False Then
		Dim i , trhtml
		i=0
		While rss.eof= False
			reDim Preserve ss(i)
			reDim Preserve Trs(i)
			selStr =""
			typHtml=""
			trhtml = ""
			If InStr(","&typeArr&",",","&rss("ord")&",")>0 Then typHtml=" checked='checked' "
			typeStr = Replace(request("sort1_"& rss("ord") &"")," ","")
			If typeStr&""<>"" Then
				If Len(typelist)>0 Then typelist= typelist &","
				typelist = typelist & typeStr
			end if
			Response.write "" & vbcrlf & "              <input type=""hidden"" id=""sort1_"
			Response.write rss("ord")
			Response.write """ name=""sort1_"
			Response.write rss("ord")
			Response.write """ value="""
			Response.write typeStr
			Response.write """>" & vbcrlf & "         "
			trhtml = "<tr class='top' onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"&_
			"<td height='27'><input type='checkbox' name='sort1sort' "&typHtml&" value='"& rss("ord")& "' onclick='selectall(this)'>"& rss("sort1") &"</td></tr>"
			Set rs1= conn.execute("select id,sort1 from paytype where sort2="& rss("ord") &" order by gate2 desc")
			If rs1.eof = False Then
				trhtml = trhtml &  "<tr class='' onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"&_
				"<td height='27'>"
				While rs1.eof = False
					If typeStr&""="" And Len(typHtml)>0 Then
						If Len(typelist)>0 Then typelist= typelist &","
						typelist = typelist & rs1("id")
					end if
					If InStr(","& typeStr &",",","&rs1("id")&",")>0 Or (typeStr&""="" And Len(typHtml)>0) Then
						selHtml=" checked='checked'"
						selStr =  selStr & rs1("sort1")&"&nbsp;"
					else
						selHtml=""
					end if
					trhtml = trhtml &  "&nbsp;<input type='checkbox' name='sort1type_"& rss("ord") &"' "&selHtml&" value='"& rs1("id")& "' title='"& rs1("sort1") &"' onClick=""selectType(this)"">"& rs1("sort1")
					rs1.movenext
				wend
				trhtml = trhtml & "</td></tr>"
			end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""费用分类选择""  style=""width:500px;height:300px;padding:5px;background: #fafafa;top:0px;display:none;"" collapsible=""false"" minimizable=""false"" maximizable=""false"" closed=""true"" >" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:0px;background:#fff;border:0px solid #ccc; width:100%"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">"
	Response.write Join(Trs,"")
	Response.write "" & vbcrlf & "              </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;padding:4px"">" & vbcrlf & "         <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('demo').submit()"">确认</a>" & vbcrlf & "             <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "   </div>" & vbcrlf & "</div>" & vbcrlf & "<script src= ""../Script/ti_select_fy.js?ver="
	'Response.write Join(Trs,"")
	Response.write Application("sys.info.jsver")
	...
	Sub area_Fytype
		...
		While rss.eof= False
			...
			Response.write rss("sort1")&"：<font id='typelist_"&rss("ord")&"' style='color:#000000'>"& ss(i) & "</font>"
			i=i+1
			'Response.write rss("sort1")&"：<font id='typelist_"&rss("ord")&"' style='color:#000000'>"& ss(i) & "</font>"
			rss.movenext
		wend
	...
	Call area_Fytype
	Response.write "</td></tr>" & vbcrlf & "  </table></form>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30""  align=""right"">" & vbcrlf & "          <input type=""button"" name=""pillar"" value=""柱状图分析"" onClick=""gotourl('type_tj=1');"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""gotourl('type_tj=2');"" class=""anybutton2"">" & vbcrlf & "            <select name=""select"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "              <option value=""width_tj=550"" "
	'Call area_Fytype
	if width_tj=550 then
		Response.write "selected"
	end if
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""width_tj=700"" "
	if width_tj=700 then
		Response.write "selected"
	end if
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""width_tj=1000"" "
	if width_tj=1000 then
		Response.write "selected"
	end if
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""width_tj=1500"" "
	if width_tj=1500 then
		Response.write "selected"
	end if
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""width_tj=2000"" "
	if width_tj=2000 then
		Response.write "selected"
	end if
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""width_tj=3000"" "
	...
	if open_6_10=1 or open_6_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='fy3.asp?type_tj=4&sort="
		Response.write E
	...
	Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" id=""user2"">" & vbcrlf & "<tr bgcolor=""#FFFFFF""><td align=""left""><div align=""left""><span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt;"">" & vbcrlf & "  <B>正在处理数据...</B></span></div></td></tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & " <table width=""600"" border=""0"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr bgcolor=""#FFFFFF"">  " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage2"" name=""CountImage2"" align=""absmiddle"">" & vbcrlf & "</table>" & vbcrlf & "  </td>" & vbcrlf &"</tr> "& vbcrlf &" <tr bgcolor=""#FFFFFF""><td><span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf
	Response.write "'}"" class=""anybutton2""/>"
	If SysTongJiJoinPageToTop = "1" Then Call DoSysTongJiJoinPageProc(1)
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1""  id=""VMLGraphicsDiv"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""center"">" & vbcrlf & ""
	If request("sortlist")<>"" Then
		F=request("sortlist")
	else
		F=typelist
	end if
	if F<>"" then
		Str_Result2=Str_Result2+"and  a.sort in ("&F&") "
'if F<>"" then
		Str_Result4=Str_Result4+"and a.sort in ("&F&") "
'if F<>"" then
		Str_Result5=Str_Result5+"and b.sort1 in ("&F&") "
'if F<>"" then
		Str_Result=Str_Result+" and  del=3"
'if F<>"" then
		Str_Result3=Str_Result3+" and del=3"
'if F<>"" then
	end if
	dim numcg1,zs1
	set rs1=server.CreateObject("adodb.recordset")
	sql = "exec Tongji_fyryfb @uid,'@f','@m1','@m2',@k1,@k2,@k3,@k4,@k5"
	sql = Replace(sql,"@uid",session("personzbintel2007"))
	sql = Replace(sql,"@m1",m1)
	sql = Replace(sql,"@m2",m2)
	sql = Replace(sql,"@k1",k1)
	sql = Replace(sql,"@k2",k2)
	sql = Replace(sql,"@k3",k3)
	sql = Replace(sql,"@k4",k4)
	sql = Replace(sql,"@k5",k5)
	sql = Replace(sql,"@f",f)
	conn.CursorLocation = 3
	...
	if counts<=0 Then
		...
		zs6=0
		While rs1.eof = false
			Select Case rs1("fy_type")
			Case 1
			numcg5=Formatnumber(rs1("fy_money"),num_dot_xs,-1)
'Case 1
			sortfy="申请"
			title=sortfy&"<br>"&"<br>"&rs1("name")
			lburl="../pay/paysq1.asp?W3="&rs1("ord")&"&F="&F&"&ret="&m1&"&ret2="&m2&""
			total1(j,1)=numcg5
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			total2(j,1)=numcg5
			total2(0,1)="#FF0000,1.5,1,2,费用分布"
			total2(j,2)=lburl
			total3(j,0)=title
			total3(j,1)=numcg5
			'total3(j,2)=lburl
			...
			'total3(j,2)=lburl
			Case 2
			numcg2=Formatnumber(rs1("fy_money"),num_dot_xs,-1)
'Case 2
			E=1
			sortfy="使用"
			if  k5="0" then
			...
			lburl="../pay/paytj.asp?W3="&rs1("ord")&"&E="&E&"&F="&F&"&ret="&m1&"&ret2="&m2&""
			total1(j,1)=numcg2
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			...
			'total3(j,2)=lburl
			Case 3
			numcg6=Formatnumber(rs1("fy_money"),num_dot_xs,-1)
'Case 3
			sortfy="报销"
			if  k5="0" and k2="0" then
				title=sortfy&"<br>"&"<br>"&rs1("name")
			else
				title=sortfy
			end if
			lburl="../pay/paybx1.asp?W3="&rs1("ord")&"&E="&E&"&F="&F&"&ret="&m1&"&ret2="&m2&""
			total1(j,1)=numcg6
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			...
			'total3(j,2)=lburl
			Case 4
			numcg4=Formatnumber(rs1("fy_money"),num_dot_xs,-1)
'Case 4
			E=""
			sortfy="借款"
			if  k5="0" and k2="0" and k1="0" then
				title=sortfy&"<br>"&"<br>"&rs1("name")
			else
				title=sortfy
			end if
			lburl="../pay/jklist1.asp?W3="&rs1("ord")&"&F="&F&"&ret="&m1&"&ret2="&m2&""
			total1(j,1)=numcg4
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			...
			'total3(j,2)=lburl
			Case 5
			numcg3=Formatnumber(rs1("fy_money"),num_dot_xs,-1)
'Case 5
			E=2
			sortfy="返还"
			if  k5="0" and k2="0" and k1="0" and k4="0" then
				title=sortfy&"<br>"&"<br>"&rs1("name")
			else
				title=sortfy
			end if
...
'End Select
	...
	rs1.close
	Set rs1 = nothing
	zs6=Formatnumber(zs6,num_dot_xs,-1)
	Set rs1 = nothing
	zs2=Formatnumber(zs2,num_dot_xs,-1)
	Set rs1 = nothing
	zs3=Formatnumber(zs3,num_dot_xs,-1)
	Set rs1 = nothing
	zs4=Formatnumber(zs4,num_dot_xs,-1)
	Set rs1 = nothing
	zs5=Formatnumber(zs5,num_dot_xs,-1)
	Set rs1 = nothing
	zs1=Formatnumber(cdbl(zs6)+cdbl(zs2)+cdbl(zs3)+cdbl(zs4)+cdbl(zs5),num_dot_xs,-1)
	Set rs1 = nothing
	if k5<>"0" then
		zs1=zs1&" 申请:￥"&zs5
	end if
