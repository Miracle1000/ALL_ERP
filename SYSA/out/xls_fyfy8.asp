	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				xlApplication.ActiveSheet.Columns(11).NumberFormatLocal = "#,##0"&dotstr&"_ "
				xlApplication.ActiveSheet.Columns(12).ColumnWidth=15
				xlApplication.ActiveSheet.Columns(12).HorizontalAlignment=3
				xlApplication.ActiveSheet.Columns(12).NumberFormatLocal = "#,##0"&dotstr&"_ "
...
'elseif open_6_11=1 then
				...
				if m1<>"" then
					str_Result11=str_Result11+ " and sp_date>='"&m1&"' "
'if m1<>"" then
					str_Result22=str_Result22+ " and datejk>='"&m1&"' "
'if m1<>"" then
					str_Result33=str_Result33+ " and bxdate>='"&m1&"' "
'if m1<>"" then
				end if
				if m2<>"" then
					str_Result11=str_Result11+ " and sp_date<='"&m2&"' "
'if m2<>"" then
					str_Result22=str_Result22+ " and datejk<='"&m2&"' "
'if m2<>"" then
					str_Result33=str_Result33+ " and bxdate<='"&m2&"' "
'if m2<>"" then
				end if
				...
				if A3<>"0" then
					str_Result=str_Result+" and sortbzid="&A3&" "
'if A3<>"0" then
				end if
				...
				W4=replace(W3,"0","")
				W4=replace(W4,",","")
				if W4<>"" then    str_Result=str_Result+"and cateid in ("&W3&")"
				W4=replace(W4,",","")
				if C<>"" then
					if B="xmmc" then
...
'if B="xmmc" then
					elseif B="xmid" then
						str_Result=str_Result+" and xmid like '%"& C &"%' "
'elseif B="xmid" then
					elseif B="khmc" then
...
'elseif B="zfzr" then
				if FK1>"" then Str_Result=Str_Result+" and  id in  ("&FK1&") "
'elseif B="zfzr" then
...
'if duemoney5<>"" and duemoney6<>"" then
				...
				if bz<>"0" and bz<>"" then
					str_Result=str_Result+" and sortbzid="&bz&" "
'if bz<>"0" and bz<>"" then
				end if
				if khmc<>"" then
...
'if khmc<>"" then
				...
				if xmid<>"" then
					str_Result=str_Result+" and xmid like '%"&xmid&"%' "
'if xmid<>"" then
				end if
				if xmmc<>"" then
					str_Result=str_Result+" and title like '%"&xmmc&"%' "
'if xmmc<>"" then
				end if
				if D<>"" then Str_Result=Str_Result+" and  sorce in  ("&D&") "
'if xmmc<>"" then
				if E<>"" then Str_Result=Str_Result+" and  trade in  ("&E&") "
'if xmmc<>"" then
				area_list=-1
'if xmmc<>"" then
				if A2<>"" then
	...
	n=0
	sql="select * from("&_
	"select chance.ord,chance.title,chance.area,chance.xmid,chance.company,chance.cateid,chance.trade,chance.complete2,chance.sorce,"&_
	"sortbz.id as sortbzid,sortbz.sort1 as sortbzname, "&_
	"paytype.id,paytype.sort1,paytype.sort2,"&_
	"(select sort1 from sortonehy where ord=paytype.sort2) as sortname,"&_
	"(select isnull(sum(money1),0) from paysqlist where del=1 and chance=chance.ord and sort1=paytype.id and fid in  "&_
	"(select id from paysq where bz=sortbz.id "&str_Result11&" ) ) as money1,"&_
	"(select isnull(sum(money1),0) from pay where del=1 and complete=3 and sort=paytype.id and  chance=chance.ord and fid in "&_
	"(select id from f_pay where bz=sortbz.id "&str_Result22&" ) ) as money2,"&_
	"(select isnull(sum(money1),0) from paybxlist where del=1 and sort=paytype.id and payid in  "&_
	"(select ord from pay where chance=chance.ord "&str_Result44&") and bxid in (select id from paybx where bz=sortbz.id "&str_Result33&" ) ) as money3 "&_
	"from chance,paytype,sortbz "&_
	"where chance.del=1 and "&_
	"("&_
	"chance.ord in (select chance from paysqlist where del=1) or chance.ord in (select chance from pay where del=1 and complete=3)"&_
	")"&_
	"and "&_
	"("&_
	"sortbz.id in (select bz from paysq where del=1) or sortbz.id in (select bz from f_pay where del=1) or sortbz.id in (select bz from paybx where del=1)"&_
	") "&_
	") as fy6 "&_
	""&Str_Result&" "&_
	"and "&_
	"("&_
	"money1>0 or money2>0 or money3>0"&_
	") "&px_Result&""
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "项目费用明细表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0 then
		Response.write "<table><tr><td>没有信息!</td></tr><tr><td height='1300'></td></tr></table>"
	else
		do until rs.eof
			...
			if rs("cateid")<>"" and rs("cateid")<>0 and (not isnull(rs("cateid"))) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			if rs("trade")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select * from sortonehy where ord="&rs("trade")&""
				rs7.open sql7,conn,1,1
				dim trade
				if rs7.eof then
					trade=""
			...
			if rs("complete2")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1,may from sortjh2 where ord="&rs("complete2")&""
				rs7.open sql7,conn,1,1
				if not rs7.eof then
					complete4=rs7("sort1")
				end if
			...
			if rs("sorce")<>0 and rs("sorce")<>"" and (not isnull(rs("sorce"))) then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1 from sortonehy where ord="&rs("sorce")&""
				rs7.open sql7,conn,1,1
				dim ly
				if rs7.eof then
					ly=""
			...
			if rs("area")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select id1,menuname from menuarea where id="&rs("area")&""
				rs7.open sql7,conn,1,1
				if not rs7.eof then
					area=rs7("menuname")
				else
			...
			xlWorksheet.Cells(1+i,1).Value = rs("sortname")
			area=""
			...
			area=""
			xlWorksheet.Cells(1+i,3).Value = rs("title")
			area=""
			xlWorksheet.Cells(1+i,3).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,4).Value = rs("xmid")
			area=""
			xlWorksheet.Cells(1+i,4).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,5).Value = sort67
			area=""
			xlWorksheet.Cells(1+i,5).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,6).Value = trade
			area=""
			xlWorksheet.Cells(1+i,6).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,7).Value = complete4
			area=""
			xlWorksheet.Cells(1+i,7).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,8).Value = ly
			area=""
			xlWorksheet.Cells(1+i,8).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,9).Value = area
			area=""
			xlWorksheet.Cells(1+i,9).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,10).Value = rs("sortbzname")
			area=""
			xlWorksheet.Cells(1+i,10).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,11).Value = Formatnumber(money1,num_dot_xs,-1)
			area=""
			xlWorksheet.Cells(1+i,11).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,12).Value = Formatnumber(money2,num_dot_xs,-1)
			area=""
			xlWorksheet.Cells(1+i,12).font.Size=10
			area=""
			xlWorksheet.Cells(1+i,13).Value = Formatnumber(money3,num_dot_xs,-1)
			area=""
			...
			rs.movenext
		loop
		xlWorksheet.Cells(1+i,1).Value = "总计："
'loop
		xlWorksheet.Cells(1+i,1).font.Size=10
'loop
		xlWorksheet.Cells(1+i,1).HorizontalAlignment=4
'loop
		xlWorksheet.Cells(1+i,1).colspan = 10
'loop
		xlWorksheet.Cells(1+i,2).Value = Formatnumber(summoney1,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,2).font.Size=10
'loop
		xlWorksheet.Cells(1+i,3).Value = Formatnumber(summoney2,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,3).font.Size=10
'loop
		xlWorksheet.Cells(1+i,4).Value = Formatnumber(summoney3,num_dot_xs,-1)
'loop
		xlWorksheet.Cells(1+i,4).font.Size=10
'loop
	end if
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>项目费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!项目费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("项目费用明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的项目费用明细表</u></strong></font></a></p>"
