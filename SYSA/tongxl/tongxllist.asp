...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns select{margin-top:-0.5px;}" & vbcrlf & ".IE5 .top_btns input.anybutton," & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "#ht1 table{margin-bottom:-1px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="""
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src=""../inc/loadingBar.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/tl_tongxllist.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "<script src=""../inc/popup_layer.js?ver="
	Response.write Application("sys.info.jsver")
	...
	if px="1" then
		Ptr_Result=" order by date7 desc"
	elseif px="2" then
		Ptr_Result=" order by date7 asc"
	elseif px="3" then
		Ptr_Result=" order by name desc"
	elseif px="4" then
		Ptr_Result=" order by name asc"
	elseif px="5" then
		Ptr_Result=" order by birthday desc"
	elseif px="6" then
		Ptr_Result=" order by birthday asc"
	elseif px="7" then
		Ptr_Result=" order by familyadd desc"
	elseif px="8" then
		Ptr_Result=" order by familyadd asc"
	end if
	ret          =request("ret")
	ret2 =request("ret2")
	content      =request("content")
	conkey       =request("conkey")
	if ret<>"" then
		Str_Result=Str_Result&" and datediff(dd,'"& ret &"',date7)>=0"
	end if
	if ret2<>"" then
		Str_Result=Str_Result&" and datediff(dd,date7,'"& ret2 &"')>=0"
	end if
	if conkey<>"" then
		if content="yhxm" then
			Str_Result=Str_Result&" and name like '%"&conkey&"%'"
		elseif content="jtzz" then
			Str_Result=Str_Result&" and familyadd like '%"&conkey&"%'"
		elseif content="jtdh" then
			Str_Result=Str_Result&" and phone1 like '%"&conkey&"%'"
		elseif content="srsj" then
			Str_Result=Str_Result&" and mobile1 like '%"&conkey&"%'"
		elseif content="gsmc" then
			Str_Result=Str_Result&" and company like '%"&conkey&"%'"
		end if
	end if'
	names=request("name")
	if names<>"" then
		Str_Result=Str_Result&" and name like '%"&names&"%'"
	end if
	email=request("email")
	if email<>"" then
		Str_Result=Str_Result&" and email like '%"&email&"%'"
	end if
	sex=request("sex")
	if sex<>"" then
		Str_Result=Str_Result&" and sex ='"&sex&"'"
	end if
	qq=request("qq")
	if qq<>"" then
		Str_Result=Str_Result&" and qq like '%"&qq&"%'"
	end if
	sorts=request("sort")
	if sorts<>"" then
		Str_Result=Str_Result&" and sort in ("&sorts&")"
	end if
	msn=request("msn")
	if msn<>"" then
		Str_Result=Str_Result&" and msn like '%"&msn&"%'"
	end if
	phone1=request("phone1")
	if phone1<>"" then
		Str_Result=Str_Result&" and phone1 like '%"&phone1&"%'"
	end if
	ret5=request("ret5")
	if ret5<>"" then
		Str_Result=Str_Result&" and month(birthday) >="&month(ret5)&" and day(birthday) >="&day(ret5)
	end if
	ret6=request("ret6")
	if ret6<>"" then
		Str_Result=Str_Result&" and month(birthday) <="&month(ret6)&" and day(birthday) <="&day(ret6)
	end if
	phone2=request("phone2")
	if phone2<>"" then
		Str_Result=Str_Result&" and phone2 like '%"&phone2&"%'"
	end if
	age1=request("age1")
	if age1<>"" then
		Str_Result=Str_Result&" and age>="&age1
	end if
	age2=request("age2")
	if age2<>"" then
		Str_Result=Str_Result&" and age<="&age2
	end if
	fax=request("fax")
	if fax<>"" then
		Str_Result=Str_Result&" and fax like '%"&fax&"%'"
	end if
	familyadd=request("familyadd")
	if familyadd<>"" then
		Str_Result=Str_Result&" and familyadd like '%"&familyadd&"%'"
	end if
	mobile1=request("mobile1")
	if mobile1<>"" then
		Str_Result=Str_Result&" and mobile1 like '%"&mobile1&"%'"
	end if
	company=request("company")
	if company<>"" then
		Str_Result=Str_Result&" and company like '%"&company&"%'"
	end if
	comtel = request("comtel")
	If comtel <> "" Then
		Str_Result=Str_Result&" and comtel like '%"&comtel&"%'"
	end if
	comadd1=request("comadd1")
	if comadd1<>"" then
		Str_Result=Str_Result&" and comadd1 like '%"&comadd1&"%'"
	end if
	comadd=request("comadd")
	if comadd<>"" then
		Str_Result=Str_Result&" and comadd like '%"&comadd&"%'"
	end if
	xl=request("xl")
	if xl<>"" then
		Str_Result=Str_Result&" and xl like '%"&xl&"%'"
	end if
	zy=request("zy")
	if zy<>"" then
		Str_Result=Str_Result&" and zy like '%"&zy&"%'"
	end if
	xw=request("xw")
	if xw<>"" then
		Str_Result=Str_Result&" and xw like '%"&xw&"%'"
	end if
	shool=request("shool")
	if shool<>"" then
		Str_Result=Str_Result&" and shool like '%"&shool&"%'"
	end if
	mz=request("mz")
	if mz<>"" then
		Str_Result=Str_Result&" and mz like '%"&mz&"%'"
	end if
	jg=request("jg")
	if jg<>"" then
		Str_Result=Str_Result&" and jg like '%"&jg&"%'"
	end if
	smoke=request("smoke")
	if smoke<>"" then
		Str_Result=Str_Result&" and smoke='"&smoke&"'"
	end if
	wine=request("wine")
	if wine<>"" then
		Str_Result=Str_Result&" and wine='"&wine&"'"
	end if
	tea=request("tea")
	if tea<>"" then
		Str_Result=Str_Result&" and tea='"&tea&"'"
	end if
	food=request("food")
	if food<>"" then
		Str_Result=Str_Result&" and food like '%"&food&"%'"
	end if
	drive=request("drive")
	if drive<>"" then
		Str_Result=Str_Result&" and drive='"&drive&"'"
	end if
	car=request("car")
	if car<>"" then
		Str_Result=Str_Result&" and car='"&car&"'"
	end if
	note=request("note")
	if note<>"" then
		Str_Result=Str_Result&" and note like '%"&note&"%'"
	end if
	ret3=request("ret3")
	time1=request("time1")
	time2=request("time2")
	if ret3<>"" then
		Str_Result=Str_Result&" and date7>='"&ret3 &" " & time1 &":"&time2&":00'"
	end if
	ret4=request("ret4")
	time3=request("time3")
	time4=request("time4")
	if ret4<>"" then
		Str_Result=Str_Result&" and date7<'"&ret4 &" " & time3 &":"&time4&":00'"
	end if
	share=request("share")
	if share<>"" then
		if share="1" then
			Str_Result=Str_Result&" and (share<>'1' and (share is not null))"
		elseif share="0" then
			Str_Result=Str_Result&" and (share='1' or share is null)"
		end if
	end if
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <form name=""data1"" id=""data1"" action=""tongxllist.asp"" method=""get"">" & vbcrlf & "             <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf &           "<tr>" & vbcrlf &             "<td class=""place"">通讯录列表</td>" & vbcrlf &                    "<td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "           <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32""  /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "         <span id=""ht1""></span>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "       <tr id=""kh""><td colspan=""9"" background=""../images/112.gif"" class=""top_btns""  height=""30"" align=""right""><div align=""right"">" & vbcrlf & "自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" readonly=""readonly"" size=""9"" name=""ret"" /><DIV id=daysOfMonth style=""POSITION: absolute""></DIV>&nbsp;" & vbcrlf & "                     至：<input id=""daysOfMonth2Pos"" onmouseup=""toggleDatePicker('daysOfMonth2','date.ret2')"" size=""9""  readonly=""readonly"" name=""ret2"" /><DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>&nbsp;" & vbcrlf & "                     <select name=""content"" id=""content"" style=""color:#000000"">" & vbcrlf & "                      <option value=""yhxm"">用户姓名</option>" & vbcrlf & "                        <option value=""jtzz"">家庭住址</option>" & vbcrlf & "                        <option value=""jtdh"">家庭电话</option>" & vbcrlf & "                            <option value=""srsj"">私人手机</option>" & vbcrlf & "                            <option value=""gsmc"">所在单位</option>" & vbcrlf & "                            </select>" & vbcrlf & "                     <input type=""text"" name=""conkey"" id=""conkey"" size=""10"">" & vbcrlf & "<input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索"">" & vbcrlf &                    "<input name=""submit1"" type=""button""  class=""anybutton"" value=""导入"" onClick=""window.location.hrefort.ashx';""/>" & vbcrlf & "                    <input name=""submit2"" type=""button""  class=""anybutton"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xlstxl.asp?qx='"
	Response.write qx
	Response.write "&ret="
	Response.write ret
	Response.write "&ret2="
	...
	Response.write "&fax="
	Response.write fax
	Response.write "&familyadd="
	Response.write familyadd
	Response.write "&mobile1="
	Response.write mobile1
	Response.write "&comadd1="
	Response.write comadd1
	Response.write "&comadd="
	Response.write comadd
	Response.write "&xl="
	Response.write xl
	Response.write "&zy="
	...
	Response.write share
	Response.write "'});}""/>" & vbcrlf & "<a href=""###"" class=""AfterQuickSearch"" onClick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>" & vbcrlf & "</div></td></tr>" & vbcrlf & "       </form>" & vbcrlf & " <form name=""data"" id=""data"" action=""tongxllist.asp"" method=""post"">" & vbcrlf & "               <tr  valign=""center"" class=""top"">" & vbcrlf & "             <td width=""5%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                 <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">姓名</div></td>" & vbcrlf & "            <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">家庭电话</div></td>" & vbcrlf & "                   <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">传真</div></td>" & vbcrlf & "            <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">手机</div></td>" & vbcrlf & "            <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">邮件</div></td>" & vbcrlf &                "<td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">生日</div></td>" & vbcrlf &             "<td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">添加人</div></td>" & vbcrlf & "              <td width=""17%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "            "
	...
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from tongxunlu  where del=1 "&Str_Result&" and (addcateid="&session("personzbintel2007")&" or share like '%"&session("personzbintel2007")&"%' or share='0' ) "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do until rs7.eof
			...
			if rs7("addcateid")=session("personzbintel2007") then
				Response.write "" & vbcrlf & "                      <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs7("id")
				Response.write """>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "                </div></td>" & vbcrlf & "             <td height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center""><a href=""javascript:;"" onClick=""detail("""
			Response.write rs7("id")
			Response.write ");"">"
			Response.write rs7("name")
			Response.write "</a></div></td>" & vbcrlf & "                 <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("phone1")
			if ZBRuntime.MC(32000) And rs7("phone1")<>"" and tp=1 then
				Response.write " <img  src='../images/15.gif' onClick=""getCall('../china/bh.asp?ord=" & rs7("id") & "&ordtype=txl&phone="&rs7("phone1")&"');return false;"" style='cursor:hand' border='0' align='absbottom'>"
			end if
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("fax")
			if ZBRuntime.MC(32000) And rs7("fax")<>"" and tp=1 then
				Response.write " <img src='../images/15.gif' onClick=""getCall('../china/bh.asp?ord=" & rs7("id") & "&ordtype=txl&phone="&rs7("fax")&"');return false;"" style='cursor:hand' border='0' align='absbottom'>"
			end if
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("mobile1")
			if ZBRuntime.MC(32000) And isMobile(rs7("mobile1")) and tp=1 then
				Response.write " <img src='../images/sj.gif' onClick=""getCall('../china/bh.asp?ord=" & rs7("id") & "&ordtype=txl&phone="&rs7("mobile1")&"');return false;"" style='cursor:hand' border='0' align='absbottom'>"
			end if
			if open_67_13=1  and open_67_19<>1 and isMobile(rs7("mobile1")) then
				Response.write "" & vbcrlf & "<img src=""../images/message.gif"" onClick=""javascript:window.open('../message/topadd.asp?phone="
				Response.write trim(rs7("mobile1"))
			...
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("email")
			if open_77_13=1 and open_77_19<>1 and isemail(rs7("email")) then
				Response.write "" & vbcrlf & "<img src=""../images/email.gif"" onClick=""javascript:window.open('../email/index.asp?email="
				Response.write trim(rs7("email"))
			...
			Response.write rs7("birthday")
			Response.write "</div></td>" & vbcrlf & ""
			if rs7("addcateid")<>"" and not isnull(rs7("addcateid")) then
				set rs88=conn.execute("select name from gate where ord="&rs7("addcateid")&" ")
				if not rs88.eof then
					addcatename=rs88("name")
				else
					addcatename=""
				end if
				set rs88=nothing
			...
			Response.write "" & vbcrlf & "                <td align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write addcatename
			Response.write "</div></td>" & vbcrlf & "             <td height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                    "
			Response.write "" & vbcrlf & "                     <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""detail("
			Response.write rs7("id")
			Response.write ");"" value=""详情""/>" & vbcrlf & "              "
			if rs7("addcateid")=session("personzbintel2007") then
				Response.write "" & vbcrlf & "                      <input name=""button32"" type=""button""  class=""page"" id=""button3"" onClick=""share("
				Response.write rs7("id")
				Response.write ");"" value=""共享""/>" & vbcrlf & "                  <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""edit("""
				Response.write rs7("id")
				Response.write ");"" value=""修改""/>" & vbcrlf & "                  <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""test("""
				Response.write rs7("id")
			...
			j7=j7+1
			Response.write "" & vbcrlf & "                 </div></td>" & vbcrlf & "         </tr>" & vbcrlf & "            "
			if j7>=rs7.PageSize then exit do
			rs7.movenext
		loop
		Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "    <tr>" & vbcrlf & "          <td width=""60"" height=""30"">全选<input name=""chkall"" type=""checkbox"" id=""chkall"" value=""chkall"" onClick=""mm(this.form)""></td>" & vbcrlf & "          <td><input name=""submit6"" type=""button"" class=""anybutton"" id=""submit6""  style=""cursor:hand""  value=""批量删除"" onClick=""testall()""/><input name=""submit61"" type=""button""  class=""anybutton"" id=""submit61"" value=""批量共享"" onClick=""shareall()""/>"
		if open_67_13=1 and open_67_19<>1  then
			Response.write "<input  name=""submit22"" type=""submit"" class=""anybutton"" style=""cursor:hand""   onClick=""this.form.action='../message/sendPersonAll2.asp?type=1'"" value=""批量短信群发"">" & vbcrlf & "            "
		end if
		Response.write "" & vbcrlf & "             </td></form>    " & vbcrlf & "                <td>            " & vbcrlf & "                <div align=""right"">" & vbcrlf & "               "
		...
		Response.write "条信息/页            " & vbcrlf & "                <input name=""CurrPage"" type=""text"" size=""4"" onpropertychange=""formatData(this,'int');"" maxlength=8 onkeyup=""checkDot('currpage',0,8)""/>" & vbcrlf & "           <input name=""btntz"" type=""submit""  class=""anybutton2"" id=""btntz"" value=""跳转""  onClick=""gotourl('currPage='+document.getElementById('CurrPage').value);"" />" & vbcrlf & "           "
		Response.write rs7.pagesize
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "                       <input name=""Submit7"" type=""button""   class=""anybutton2"" id=""Submit7"" onClick=""gotourl('CurrPage=1')"" value=""首页""/> " & vbcrlf & "                       <input name=""Submit9"" type=""button"" class=""anybutton2"" id=""Submit9""  onClick=""gotourl('CurrPage="
	...
	Response.write "" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""200""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""192"">" & vbcrlf & "    <table width=""150"" height=""192"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按用户姓名排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按用户姓名排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按用户生日排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按用户生日排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按用户地址排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按用户地址排序(升)</font></a> </td>" & vbcrlf & "        </tr>                " & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="通讯录列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script src= ""../Script/tl_tongxllist_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=javascript></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function share(sid)" & vbcrlf & "{" & vbcrlf & "document.getElementById('data').action=""share.asp?sid=""+sid+""&tp=1&currPage="
	Response.write Application("sys.info.jsver")
	Response.write currpage
	Response.write "&qx="
	Response.write qx
	...
	Response.write "&share="
	Response.write share
	Response.write """;" & vbcrlf & "document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function test(sid)" & vbcrlf & "{" & vbcrlf & " if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & " return false;" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "     document.getElementById('data').action=""delete.asp?sid=""+sid+""&tp=1&currPage="
	Response.write share
	Response.write currpage
	Response.write "&qx="
	Response.write qx
	...
	Response.write "&share="
	Response.write share
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {//BUG 6673 Sword 2014-12-25 批量删除后，点击取消，报网页已过时 " & vbcrlf & "  return false;" & vbcrlf & " // history.back();" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "         document.getElementById('data').action=""delete.asp?tp=2&currPage="
	Response.write currpage
	Response.write "&qx="
	Response.write qx
	Response.write "&ret="
