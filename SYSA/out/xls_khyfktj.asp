	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			elseif open_7_10=1 then
				str_Result3=" and a.cateid in ("&intro_7_10&") and a.cateid<>0 "
			else
				str_Result3=" and 1=0 "
			end if
			A=request("A")
...
'if B="khmc" then
			...
			n=0
			set rs=server.CreateObject("adodb.recordset")
			sql="select a.ord,a.name,a.cateid,z.id as bz ,"&_
			"z.sort1 as sortbz,"&_
			...
			C1=rs.recordcount
			xlApplication.Init "客户预收款统计表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				company=rs("ord")
				...
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,2).Value = rs("sortbz")
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,2).font.Size=10
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,3).Value = Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,3).font.Size=10
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,4).Value = Formatnumber(zbcdbl(rs("money2")),num_dot_xs,-1)
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,4).font.Size=10
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,5).Value = Formatnumber(zbcdbl(rs("money3")),num_dot_xs,-1)
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,5).font.Size=10
				companyname = " 关联客户已被删除"
				xlWorksheet.Cells(1+i,6).Value = Formatnumber(zbcdbl(rs("money4")),num_dot_xs,-1)
				companyname = " 关联客户已被删除"
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户预收款统计表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户预收款统计表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("客户预收款统计表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的客户预收款统计表</u></strong></font></a></p>"
