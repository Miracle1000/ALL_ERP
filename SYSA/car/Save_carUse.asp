...
'bigsystemtype = ".mozi"
	...
	Class CommonSP
		...
		public UseRollBack
		public isModify
		public spOrderTypeID
		public spOrderTableName
		public spOrderFieldNameSPID
		public spOrderFieldNameSPUID
		public spOrderID
		public spOrderSPID
		public spOrderSPGate1
		public spOrderSPCateID
		public spOrderAddCate
		public spOrderExtendCate
		public spOrderTotalMoney
		public spCurrentUser
		public spSettingTableName
		public spSettingFieldNamePK
		public spSettingFieldNameType
		public spSettingFieldNameSort
		public spSettingFieldNameSortType
		public spSettingFieldNameLinkType
		public spSettingFieldNameMustIn
		public spSettingFieldNameIntro
		public spSettingFieldNameMoney1
		public spSettingFieldNameMoney2
		public spResultTableName
		public spResultFieldNameJG
		public spResultFieldNameSPDate
		public spResultFieldNameSPintro
		public spResultFieldNameSPType
		public spResultFieldNameOrderID
		public spResultFieldNameSPName
		public spResultFieldNameSPMoney
		private Function spCanSkipOver(spintro)
			dim extcate
			extcate="-1"
'dim extcate
			if not me.isModify then
				if me.spOrderAddCate<>"" and me.spOrderAddCate<>"0" then extcate=extcate&","&me.spOrderAddCate
				if me.spCurrentUser<>"" and me.spCurrentUser<>"0" then extcate=extcate&","&me.spCurrentUser
				if me.spOrderExtendCate<>"" and me.spOrderExtendCate<>"0" then extcate=extcate&","&me.spOrderExtendCate
			end if
			spCanSkipOver=ExtendCateInIntro(extcate,spintro,",")
		end function
		private Function ExtendCateInIntro(extcate,spintro,strSplit)
			dim extArr,fi,rValue
			extArr=split(extcate,strSplit)
			rValue=false
			for fi=0 to ubound(extArr)
				if IsInList(spintro,extArr(fi),strSplit) then
					rValue=true
					exit for
				end if
			next
			ExtendCateInIntro=rValue
		end function
		public Function getLastSP()
			dim cateid_sp,sqlsp,rssp,lastSP , jqsql
			dim sp_money1,sp_money2,sp_bt
			dim rsbz,sqlbz
			set rssp=server.CreateObject("adodb.recordset")
			jqsql = ""
			If isnumeric(Me.spOrderID)  Then
				If Me.spOrderID > 0 Then
					jqsql =  Me.spResultFieldNameOrderID & " not in (select sp_id from sp_intro where sort1=" & Me.spOrderTypeID & " and ord=" & Me.spOrderID & ") and "
				end if
			end if
			sqlsp="select * from " & me.spSettingTableName & " where " & jqsql & me.spSettingFieldNameType & "=" & spOrderTypeID & " and " & me.spSettingFieldNameSort & ">" & me.spOrderSPGate1 & " order by " & me.spSettingFieldNameSort & " " & me.spSettingFieldNameSortType
			rssp.open sqlsp ,conn, 2, 3
			lastSP=0
			do until rssp.eof
				gate1_sp=replace(rssp(me.spSettingFieldNameIntro)," ","")
				gate3=rssp(me.spSettingFieldNameLinkType)
				gate1=rssp(me.spSettingFieldNameSort)
				if me.UseMoneyLimit then
					sp_money1=rssp(me.spSettingFieldNameMoney1)
					sp_money2=rssp(me.spSettingFieldNameMoney2)
					if me.UseHL then
						sqlbz="select hl from hl where bz="&me.BZ&" and year(date1)="&year(me.HLdate)&" and month(date1)="&month(me.HLdate)&" and day(date1)="&day(me.HLdate)
						set rsbz=conn.execute(sqlbz)
						if not rsbz.eof then me.spOrderTotalMoney=zbcdbl(me.spOrderTotalMoney)*zbcdbl(rsbz(0))
						rsbz.close
					end if
					if me.UseMustIn then
						sp_bt=cstr(rssp(spSettingFieldNameMustIn))
					end if
				end if
				jp=0
				if me.UseMoneyLimit then
					if me.UseMustIn and sp_bt="1" then
						if not spCanSkipOver(gate1_sp) then
							lastSP=rssp(me.spSettingFieldNamePK)
							exit do
						else
						end if
					else
						if zbcdbl(me.spOrderTotalMoney)>=zbcdbl(sp_money1) then
							if zbcdbl(me.spOrderTotalMoney)<zbcdbl(sp_money2) Or zbcdbl(sp_money2)=0 then
								set rsMustIn=conn.execute("select * from " & me.spSettingTableName & " where " & me.spSettingFieldNameType & "=" & spOrderTypeID & " and " & me.spSettingFieldNameSort & ">" & rssp(Me.spSettingFieldNameSort) & " and "&me.spSettingFieldNameMustIn&"=1 order by " & me.spSettingFieldNameSort & " "& me.spSettingFieldNameSortType)
								if rsMustIn.eof then
									if spCanSkipOver(gate1_sp) then
										lastSP=0
										exit do
									else
										lastSP=rssp(me.spSettingFieldNamePK)
									...
									if spCanSkipOver(gate1_sp) then
										Do until rssp.eof
											If rssp(me.spSettingFieldNamePK)<>rsMustIn(me.spSettingFieldNamePK) Then
												jp=jp+1
'If rssp(me.spSettingFieldNamePK)<>rsMustIn(me.spSettingFieldNamePK) Then
												rssp.movenext
											else
		...
		public Sub ShowSPUsers()
			dim lastSP
			dim sqlsp,rssp,sorce_user,sorce_user2,sort_cateid
			lastSP=getLastSP()
			sqlsp="select sorce,sorce2,cateid from gate where ord="&me.spOrderExtendCate&" and del=1  "
			set rssp=conn.execute(sqlsp)
			if not rssp.eof then
				sorce_user=rssp("sorce")
				sorce_user2=rssp("sorce2")
				sort_cateid=rssp("cateid")
			end if
			sqlsp="select * from " & me.spSettingTableName & " where " & me.spSettingFieldNamePK & "=" & lastSP
			set rssp=conn.execute(sqlsp)
			if rssp.eof then
				gate1_sp="0"
				gate3=0
			...
			gate3list = " and ord in (select  ord from dbo.GetAvlRelPersons(" & gate3 & "," & me.spOrderExtendCate & ",'" & gate1_sp & "'," & session("personzbintel2007") & ") t) "
			if gate1_sp="" then gate1_sp="0"
			if me.isModify then
				sqlsp="select ord,name from gate where 1=1 "&gate3list&" and ord in ("&gate1_sp&") and del=1  order by cateid asc"
			else
				sqlsp="select ord,name from gate where ord<>"&me.spCurrentUser&" "&gate3list&" and ord in ("&gate1_sp&") and del=1  order by cateid asc"
			end if
			set rssp=conn.execute(sqlsp)
			if rssp.eof then
				Response.write "#*,@~无审批人"
				...
				while not rssp.eof
					if i=0 then
						Response.write rssp("ord")&"#*,@~"&rssp("name")
					else
						Response.write "@*;!&"&rssp("ord")&"#*,@~"&rssp("name")
					end if
					i=i+1
		...
		Public Sub Class_Initialize()
			me.UseMoneyLimit=true
			me.UseMustIn=true
			me.UseRollBack=false
			me.isModify=false
			me.UseHL=false
			me.spCurrentUser=session("personzbintel2007")
			me.spSettingTableName="sp"
			me.spSettingFieldNamePK="ID"
			me.spSettingFieldNameType="gate2"
			me.spSettingFieldNameSort="gate1"
			me.spSettingFieldNameSortType="asc"
			me.spSettingFieldNameLinkType="gate3"
			me.spSettingFieldNameMustIn="bt"
			me.spSettingFieldNameIntro="intro"
			me.spSettingFieldNameMoney1="money1"
			me.spSettingFieldNameMoney2="money2"
			me.spResultTableName="sp_intro"
			me.spResultFieldNameJG="jg"
			me.spResultFieldNameSPDate="date1"
			me.spResultFieldNameSPintro="intro"
			me.spResultFieldNameSPType="sort1"
			me.spResultFieldNameOrderID="ord"
			me.spResultFieldNameSPName="sp"
			me.spResultFieldNameSPMoney="money1"
		end sub
		private function IsInList(StrList,StrFind,CharSplit)
...
'Private Sub Class_Initialize
	...
	SMS=request("SMS")
	sort1=request("sort1")
	use_cateid=request("member2")
	use_zdy5=request("zdy5")
	use_zdy6=request("zdy6")
	use_zdy1=trim(replace(replace(request("zdy1"),"^#$6a",", "),"$^&1&*$",""))
	use_zdy2=trim(replace(replace(request("zdy2"),"^#$6a",", "),"$^&1&*$",""))
	use_zdy3=trim(replace(replace(request("zdy3"),"^#$6a",", "),"$^&1&*$",""))
	use_zdy4=trim(replace(replace(request("zdy4"),"^#$6a",", "),"$^&1&*$",""))
	use_note=trim(replace(replace(request("use_note"),"^#$6a",", "),"$^&1&*$",""))
	if use_zdy5="" or isnull(use_zdy5) then use_zdy5=0
	if use_zdy6="" or isnull(use_zdy6) then use_zdy6=0
	if sort1=1 Then
		spid = request("_spid")
		spuid=request("_spuid")
		if spuid="" then spuid=0
		if spid="" then spid=0
		if spid=0 then
			use_complete=3
		else
	...
	use_sms_driver=0
	use_sms_cateid=0
	if instr(SMS,"2")>0 then
		use_sms_sp=1
	end if
	if instr(SMS,"3")>0 then
		use_sms_driver=1
	end if
	if instr(SMS,"1")>0 then
		use_sms_cateid=1
	end if
	on error resume next
	conn.BeginTrans
	sql="select * from O_carUse where 1=2"
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,3,2
	rs.addnew
	rs("use_cateid")=use_cateid
	rs("use_type")=sort1
	rs("use_complete")=use_complete
	rs("use_id_sp")=use_id_sp
	rs("use_cateid_sp")=use_cateid_sp
	rs("use_sms_sp")=use_sms_sp
	rs("use_sms_driver")=use_sms_driver
	rs("use_sms_cateid")=use_sms_cateid
	rs("use_zdy1")=use_zdy1
	rs("use_zdy2")=use_zdy2
	rs("use_zdy3")=use_zdy3
	rs("use_zdy4")=use_zdy4
	rs("use_zdy5")=use_zdy5
	rs("use_zdy6")=use_zdy6
	rs("use_note")=use_note
	rs("use_addtime")=now
	rs("use_addcateid")=session("personzbintel2007")
	rs("use_del")=1
	rs.update
	rs.close
	set rs=nothing
	ord = GetIdentity("O_carUse","use_id","use_addcateid","")
	if ord="0" Or ord&"" = "" Then
		conn.RollbackTrans
		Response.write "<script>alert('数据添加失败，请重新添加。');history.back();</script>"
		call db_close : Response.end()
	end if
	use_carid=IIF(trim(request("use_carid"))="","$^&1&*$",request("use_carid"))
	use_driver=IIF(trim(request("use_driver"))="","$^&1&*$",request("use_driver"))
	use_catetype=IIF(trim(request("use_catetype"))="","$^&1&*$",request("use_catetype"))
	use_pctime=IIF(trim(request("use_pctime"))="",",",request("use_pctime"))
	use_startime=IIF(trim(request("use_startime"))="","$^&1&*$",request("use_startime"))
	use_endtime=IIF(trim(request("use_endtime"))="","$^&1&*$",request("use_endtime"))
	use_mdd=IIF(trim(request("use_mdd"))="","$^&1&*$",request("use_mdd"))
	use_lc=IIF(trim(request("use_lc"))="","$^&1&*$",request("use_lc"))
	use_thing=IIF(trim(request("use_thing"))="","$^&1&*$",request("use_thing"))
	use_notes=IIF(trim(request("use_notes"))="","$^&1&*$",request("use_notes"))
	use_caridarr=split(use_carid,",")
	use_driverarr=split(use_driver,",")
	use_catetypearr=split(use_catetype,",")
	use_pctimearr=split(use_pctime,",")
	use_startimearr=split(use_startime,",")
	use_endtimearr=split(use_endtime,",")
	use_mddarr=split(use_mdd,",")
	use_lcarr=split(use_lc,",")
	use_thingarr=split(use_thing,",")
	use_notesarr=split(use_notes,",")
	i=0
	if use_driver="" Then
		conn.RollbackTrans
		Response.write "<script>alert('请选择司机');window.history.back()</script>"
		call db_close : Response.end()
	end if
	for each a in use_caridarr
		if trim(use_caridarr(i))="" or trim(use_startimearr(i))="" or trim(use_startimearr(i))="点击选择时间" or trim(use_endtimearr(i))="点击选择时间" or trim(use_endtimearr(i))="" then
		else
			sql="select * from O_carUsemx where 1=2 "
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,3,2
			rs.addnew
			rs("use_fid")=ord
			rs("use_carid")=replace(replace(replace(CStr(use_caridarr(i)),"^#$6a",", "),"$^&1&*$","")," ","")
			if replace(use_driver," ","")<>"" then
				if isnumeric(replace(replace(replace(CStr(use_driverarr(i)),"^#$6a",", "),"$^&1&*$","")," ","")) and replace(replace(replace(CStr(use_driverarr(i)),"^#$6a",", "),"$^&1&*$","")," ","")<>"" then
					rs("use_driver")=replace(replace(replace(CStr(use_driverarr(i)),"^#$6a",", "),"$^&1&*$","")," ","")
				end if
			end if
			rs("use_catetype")=replace(replace(replace(CStr(use_catetypearr(i)),"^#$6a",", "),"$^&1&*$","")," ","")
			if trim(use_pctimearr(i))<>"" and replace(replace(CStr(use_pctimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"点击选择时间" and replace(replace(CStr(use_pctimearr(i)),"^#$6a",", "),"$^&1&*$","")<>" 点击选择时间" then
				rs("use_pctime")=use_pctimearr(i)
			end if
			if replace(replace(CStr(use_startimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"" and replace(replace(CStr(use_startimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"点击选择时间" and replace(replace(CStr(use_startimearr(i)),"^#$6a",", "),"$^&1&*$","")<>" 点击选择时间" then
				rs("use_startime")=replace(replace(CStr(use_startimearr(i)),"^#$6a",", "),"$^&1&*$","")
			end if
			if replace(replace(CStr(use_endtimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"" and replace(replace(CStr(use_endtimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"点击选择时间" and replace(replace(CStr(use_endtimearr(i)),"^#$6a",", "),"$^&1&*$","")<>" 点击选择时间" then
				rs("use_endtime")=replace(replace(CStr(use_endtimearr(i)),"^#$6a",", "),"$^&1&*$","")
			end if
			rs("use_mdd")=replace(replace(CStr(use_mddarr(i)),"^#$6a",", "),"$^&1&*$","")
			rs("use_lc")=replace(replace(CStr(use_lcarr(i)),"^#$6a",", "),"$^&1&*$","")
			rs("use_thing")=replace(replace(CStr(use_thingarr(i)),"^#$6a",", "),"$^&1&*$","")
			rs("use_notes")=replace(replace(CStr(use_notesarr(i)),"^#$6a",", "),"$^&1&*$","")
			rs.update
			set rs=nothing
			carid=replace(replace(replace(CStr(use_caridarr(i)),"^#$6a",", "),"$^&1&*$","")," ","")
			cztype=sort1
			if trim(use_pctimearr(i))<>"" and replace(replace(CStr(use_pctimearr(i)),"^#$6a",", "),"$^&1&*$","")<>"点击选择时间" and replace(replace(CStr(use_pctimearr(i)),"^#$6a",", "),"$^&1&*$","")<>" 点击选择时间" then
				cztime=replace(replace(CStr(use_pctimearr(i)),"^#$6a",", "),"$^&1&*$","")
			end if
			czname=use_cateid
			if carid="" or isnull(carid) then carid=0
			sql7="select a.*,b.setName from O_carData a left join O_carSet b on a.car_type=b.id where car_id="&carid
			set rs7=conn.execute(sql7)
			if not rs7.eof then
				carcode=rs7("car_code")
				cartype=rs7("setName")
				carname=rs7("car_name")
				carxh=rs7("car_xh")
				fdjh=rs7("car_fdjh")
				dph=rs7("car_dph")
				...
				free1=carid
				free2=ord
				if use_complete=3 then
					ba=ml.addCarLog(carcode,cztype,cztime,czname,cartype,carname,carxh,fdjh,dph,addcateid,free1,free2)
				end if
			end if
			set rs7=nothing
		end if
	...
	Dim helper
	Set helper = CreateReminderHelper(conn,70,0)
	Call helper.appendRemind(ord)
	Response.redirect "List_carUse.asp"
