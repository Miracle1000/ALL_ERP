...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	rtype=getInt(request("reportType"))
	if rtype=3 then
		reportTypeName="本年"
		type1=7
		type2=8
	...
	Response.write "报导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write reportTypeName
	Response.write "报导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出"
	Response.write reportTypeName
	Response.write reportTypeName
	Response.write "报,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	set rs1=server.CreateObject("adodb.recordset")
...
'if intro3<>"" then
	...
	xlApplication.ActiveSheet.Columns(2).ColumnWidth=60
	xlApplication.ActiveSheet.Columns(2).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(3).ColumnWidth=60
	xlApplication.ActiveSheet.Columns(3).HorizontalAlignment=3
	xlApplication.ActiveSheet.Columns(4).ColumnWidth=60
	...
	xlWorksheet.Cells(1,1).Value = "执行日期"
	xlWorksheet.Cells(1,1).font.Size=10
	xlWorksheet.Cells(1,1).font.bold=true
	xlWorksheet.Cells(1,2).Value = reportTypeName&"计划"
	xlWorksheet.Cells(1,2).font.Size=10
	xlWorksheet.Cells(1,2).font.bold=true
	xlWorksheet.Cells(1,3).Value = reportTypeName&"总结"
	xlWorksheet.Cells(1,3).font.Size=10
	xlWorksheet.Cells(1,3).font.bold=True
	Dim colNum
	colNum = 4
	sql = "SELECT charSpId, charName, intLevel FROM sp_schedule ORDER BY intLevel"
	set rs = conn.execute(sql)
	If Not rs.Eof Then
		Do While Not rs.Eof
			xlWorksheet.Cells(1,colNum).Value = rs("charName")
	...
	rs.close
	set rs = nothing
	xlWorksheet.Cells(1,colNum).Value = "执行人"
	xlWorksheet.Cells(1,colNum).font.Size=10
	xlWorksheet.Cells(1,colNum).font.bold=true
	Response.Flush
	set rs=server.CreateObject("adodb.recordset")
	set rs1=server.CreateObject("adodb.recordset")
	sql="select a.date1,a.cateid,b.name from plan2 a,gate b where a.type in("&type1&","&type2&","&type3&") and a.cateid=b.ord "&Str_Result&" group by a.date1,a.cateid,b.name "&pxResult
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init reportTypeName & "报_" & session("name2006chen")&".xls",C1
	i=0
	do while not rs.eof
		intro1=""
		sql1="select intro,ord from plan2 a where date1='"&rs("date1")&"' and cateid="&rs("cateid")&" and type="&type1
		rs1.open sql1,conn
		if not rs1.eof then
			intro1=rs1(0)
			ord1=rs1(1)
		...
		rs1.close
		intro2=""
		sql1="select intro,ord from plan2 a where date1='"&rs("date1")&"' and cateid="&rs("cateid")&" and type="&type2
		rs1.open sql1,conn
		if not rs1.eof then
			intro2=rs1(0)
			ord2=rs1(1)
		...
		intro3=""
		if intro1<>"" then
			intro1=HTMLDecode(intro1)
		end if
		if intro2<>"" then
			intro2=HTMLDecode(intro2)
		end if
		xlWorksheet.Cells(2+i,1).Value = formatdatetime(rs("date1"))
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,1).font.Size=10
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,1).HorizontalAlignment=3
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,2).Value = RemoveHTML(intro1)
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,2).font.Size=10
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,2).HorizontalAlignment=1
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,3).Value = RemoveHTML(intro2)
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,3).font.Size=10
		intro2=HTMLDecode(intro2)
		xlWorksheet.Cells(2+i,3).HorizontalAlignment=1
		intro2=HTMLDecode(intro2)
		colNum = 4
		sqlx = "SELECT charSpId, charName, intLevel FROM sp_schedule ORDER BY intLevel"
		Set rsx = conn.Execute(sqlx)
		If Not rsx.Eof Then
			Do While Not rsx.Eof
				intro3 = ""
				set rsy=server.CreateObject("adodb.recordset")
				sqly="select intro,spname,date7 from plan2 where lcb = "& rsx("intLevel") &" And date1='"&rs("date1")&"' and cateid="&rs("cateid")&" and type="&type3
				rsy.open sqly,conn
				Do While Not rsy.Eof
					intro3 = intro3&rsy("intro") &"（"&rsy("spname")&" "&rsy("date7")&"）"&chr(10)&chr(13)
					rsy.movenext
				Loop
				rsy.Close
				Set rsy = Nothing
				xlWorksheet.Cells(2+i,colNum).Value = RemoveHTML(HTMLDecode(intro3))
				Set rsy = Nothing
		...
		rsx.Close
		Set rsx = Nothing
		xlWorksheet.Cells(2+i,colNum).Value = rs("name")
		Set rsx = Nothing
		xlWorksheet.Cells(2+i,colNum).font.Size=10
		Set rsx = Nothing
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>"&reportTypeName&"报导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!"&reportTypeName&"报导出成功,共有"&(i)&"条记录!</B>"";</script>"
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath(reportTypeName&"报_"&session("name2006chen")&".xls")
	...
	Response.write reportTypeName
	Response.write "报</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>"
