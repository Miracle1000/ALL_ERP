...
'm_ids(count) = keyid
	...
	Function GetExtraFields(sort)
		...
		if sort = 21 then
			...
			" where billtype=16001 and ListType='0' order by Showindex " &_
...
'If ycgnum1&"" = "" Then ycgnum1 = 0 Else ycgnum1 = CDBL(ycgnum1)
	...
	Dim cplistPower, cpInfoPower, cglistPower, cgInfoPower, qclistPower, qcInfoPower, gyslistPower, gysInfoPower
	Dim rs1,open_28_21,num1_dot
	open_28_21 = ""
	function priceShow()
		set rs1=cn.execute("select qx_open,qx_intro from power where ord="&session("personzbintel2007")&" and sort1=28 and sort2=21")
		if rs1.eof then
			open_28_21=0
		else
	...
	Function onReportInit(rpt, state)
		app.adddefaultCss
		Dim sort,sort1,ret,ret2,dat,cls
		Dim jhr, C
		sort=request("sort")
		sort1=request("sort1")
		ret=request("ret")
		ret2=request("ret2")
		cls = app.base64.deurl(Trim(request.QueryString("cls")))
		if isnumeric(cls) = true and len(cls) > 0 Then
		else
			cls =0
		end if
		...
		rpt.pagesize = 10
		rpt.addfield "","html","","","",""
		rpt.addfield "申请日期：","dates","date1","","", dat
		rpt.addfield "","select", "E" , "待质检" & Chr(1) & "0" & Chr(2) & "质检中" & Chr(1) & "1" & Chr(2) & "待审批" & Chr(1) & "2" & Chr(2) & "质检完毕" & Chr(1) & "3" ,"", ""
		rpt.addfield "","select", "B" , "产品名称" & Chr(1) & "0" & Chr(2) & "产品编号" & Chr(1) & "1" & Chr(2) & "产品型号" & Chr(1) & "2" & Chr(2) & "关联采购" & Chr(1) & "3" & Chr(2) & "关联供应商" & Chr(1) & "4" & Chr(2) & "质检主题" & Chr(1) & "5" & Chr(2) & "质检编号" & Chr(1) & "6" & Chr(2) & "质检备注" & Chr(1) & "7","", ""
		rpt.addfield "","text", "C" , C, "", ""
		rpt.sql = "exec erp_report_zjmxbz "& session("personzbintel2007") &",@date1_0, @date1_1,@E,@B,@C, &pagesize, &pageindex, &excelmode, &sortSql"
		rpt.canexcel =  app.power.existsPower(28,10)
		rpt.canprint =  app.power.existsPower(28,7)
		rpt.headHTML = "<style>.ser_btn input.oldbutton{float:left;margin-top:1px}</style>"
		rpt.canprint =  app.power.existsPower(28,7)
		If state = 1 Then
			If sort<>"" Then
				rpt.headHTML = rpt.headHTML & "<script defer='defer' language='javascript'>window.onjoinFieldUpdate=function(){document.getElementById('fsitem_ckitem').value='"&sort1&"';"&_
				"ReportSubmit();window.onjoinFieldUpdate=null;};document.getElementById('fsitem_ckcls').value='"&sort&"';"&_
				"document.getElementById('fsitem_ckcls').fireEvent('onchange');var txts = document.getElementById('sfields_date1').getElementsByTagName('input');"&_
				"for(var i=0;i<txts.length;i++){if(txts[i].type=='text'&&i==0){txts[i].value ='"&ret&"'; }if(txts[i].type=='text'&&i==1){txts[i].value='"&ret2&"';}}</script>"
			end if
		end if
	end function
	function formatPer(str)
		formatPer = str & "%"
	end function
	Function app_oncreate(lvw)
		Dim col
		if open_28_21="" then
			open_28_21 = 0
			call priceShow()
		end if
		If isEmpty(num1_dot) Then
			Dim rsf, sqlStr
			set rsf=cn.execute("select num1 from setjm3 where ord=88")
			if rsf.eof then
				num1_dot=2
		...
		col.formattext = "code:CaigouLinkHTML(@cells[41],@cells[2],""@value"")"
		col.width = 180
		col.dbtype = "str"
		col.canSum = False
		...
		col.width = 0
		col.display = "none"
		Set col = lvw.headers(3)
		col.formattext = "code:ProductLinkHTML2(@cells[43],@cells[37],""@value"")"
		col.width = 120
		col.dbtype = "str"
		col.canSum = False
		...
		if open_28_21<>0 then
			col.width = 70
			col.dbtype = "storeprice"
			col.canSum = False
		else
			col.width = 0
			col.display = "none"
		end if
		Set col = lvw.headers(8)
		col.title = "总价"
		if open_28_21<>0 then
			col.width = 70
			col.dbtype = "money"
		...
		col.width = 70
		col.dbtype = "number"
		Set col = lvw.headers(12)
		col.title = "质检数量"
		col.width = 70
		col.dbtype = "number"
		Set col = lvw.headers(13)
		col.title = "入库数量"
		col.width = 70
		col.dbtype = "number"
		if ShowRKInfo() = false then
			col.display = "none"
		end if
		Set col = lvw.headers(14)
		col.title = "合格数量"
		col.width = 70
		col.dbtype = "number"
		Set col = lvw.headers(15)
		col.title = "不合格数量"
		col.width = 70
		col.dbtype = "number"
		Set col = lvw.headers(16)
		col.title = "合格率"
		col.width = 60
		col.Formula = "cstr(formatNum(@value,"& num1_dot &")) + ""%"""
		col.width = 60
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(17)
		col.title = "拒收"
		col.width = 70
		col.dbtype = "number"
		Set col = lvw.headers(18)
		col.title = "质检方案"
		col.width = 60
		col.dbtype = "str"
		col.align2 = "center"
		col.canSum = False
		Set col = lvw.headers(19)
		col.title = "批号"
		col.width = 60
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(20)
		col.title = "序列号"
		col.width = 80
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(21)
		col.title = "生产日期"
		col.width = 70
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(22)
		col.title = "有效日期"
		col.width = 70
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(23)
		col.title = "到货日期"
		col.width = 70
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(24)
		col.title = "备注"
		col.width = 100
		col.dbtype = "str"
		col.canSum = False
		setZjmxZdyCol lvw
		Set col = lvw.headers(31)
		col.title = "包装"
		col.width = 50
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(32)
		col.title = "质检人员"
		col.width = 70
		col.dbtype = "str"
		col.align2 = "center"
		col.canSum = False
		Set col = lvw.headers(33)
		col.width = 70
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(34)
		col.formattext = "code:QCLinkHTML(@cells[40],@cells[38],""@value"")"
		col.width = 200
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(35)
		col.width = 100
		col.align2 = "center"
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers(36)
		col.formattext = "code:GysLinkHTML(@cells[42],@cells[39],""@value"")"
		col.width = 120
		col.dbtype = "str"
		col.canSum = False
		Set col = lvw.headers("cpord")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("qcord")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("company")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("addcate")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("cgcate")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("telcate")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("procate")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("date7")
		col.width = 0
		col.display = "none"
		Set col = lvw.headers("inx")
		col.width = 0
		col.display = "none"
		lvw.currsum = true
		lvw.AllSum = true
	end function
	Public Function ProductLinkHTML2(cateid,ProductId, ProductName)
		If isEmpty(cplistPower) Then
			cplistPower = app.power.GetPowerIntro(21,1)
		end if
		if cplistPower="" or instr(","& cplistPower &"," , ","& cateid &",")>0 then
			If isEmpty(cpInfoPower) Then
				cpInfoPower = powerIntro(21,14)
			end if
			if cpInfoPower="" then
				ProductLinkHTML2 = "<a href=""#"" onclick=""javascript:window.open('../product/content.asp?ord=" & app.base64.pwurl(ProductId) & "','productcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  ProductName & "</a>"
'if cpInfoPower="" then
			else
				ProductLinkHTML2 = ProductName
			end if
		else
			ProductLinkHTML2=""
		end if
	end function
	Public Function ProductBH(cateid, ProductStr)
		If isEmpty(cplistPower) Then
			cplistPower = app.power.GetPowerIntro(21,1)
		end if
		if cplistPower="" or instr(","& cplistPower &"," , ","& cateid &",")>0 then
			ProductBH = ProductStr
		else
			ProductBH=""
		end if
	end function
	Public Function ProductXH(cateid, ProductStr)
		If isEmpty(cplistPower) Then
			cplistPower = app.power.GetPowerIntro(21,1)
		end if
		if cplistPower="" or instr(","& cplistPower &"," , ","& cateid &",")>0 then
			ProductXH = ProductStr
		else
			ProductXH=""
		end if
	end function
	Public Function CaigouLinkHTML(cateid,cgord,cgtitle)
		If isEmpty(cglistPower) Then
			cglistPower = app.power.GetPowerIntro(22,1)
		end if
		if cglistPower="" or instr(","& cglistPower &"," , ","& cateid &",")>0 then
			If isEmpty(cgInfoPower) Then
				cgInfoPower = powerIntro(22,14)
			end if
			if cgInfoPower="" or instr(","& cgInfoPower &"," , ","& cateid &",")>0 then
				CaigouLinkHTML = "<a href=""#"" onclick=""javascript:window.open('../../SYSN/view/store/caigou/caigoudetails.ashx?view=details&ord=" & app.base64.pwurl(cgord) & "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  cgtitle & "</a>"
			else
				CaigouLinkHTML = cgtitle
			end if
		else
			CaigouLinkHTML=""
		end if
	end function
	Public Function QCLinkHTML(cateid,qcord,qctitle)
		If isEmpty(qclistPower) Then
			qclistPower = app.power.GetPowerIntro(28,1)
		end if
		if qclistPower="" or instr(","& qclistPower &"," , ","& cateid &",")>0 then
			If isEmpty(qcInfoPower) Then
				qcInfoPower = powerIntro(28,14)
			end if
			if qcInfoPower="" or instr(","& qcInfoPower &"," , ","& cateid &",")>0 then
				QCLinkHTML = "<a href=""#"" onclick=""javascript:window.open('../caigouQC/content.asp?ord=" & app.base64.pwurl(qcord) & "','qccon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  qctitle & "</a>"
'if qcInfoPower="" or instr(","& qcInfoPower &"," , ","& cateid &",")>0 then
			else
				QCLinkHTML = qctitle
			end if
	...
	Public Function GysLinkHTML(cateid,gysord,gystitle)
		If isEmpty(gyslistPower) Then
			gyslistPower = app.power.GetPowerIntro(26,1)
		end if
		if gyslistPower="" or instr(","& gyslistPower &"," , ","& cateid &",")>0 then
			If isEmpty(gysInfoPower) Then
				gysInfoPower = powerIntro(26,14)
			end if
			if gysInfoPower="" or instr(","& gysInfoPower &"," , ","& cateid &",")>0 then
				GysLinkHTML = "<a href=""#"" onclick=""javascript:window.open('../work2/content.asp?ord=" & app.base64.pwurl(gysord) & "','qccon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" &  gystitle & "</a>"
'if gysInfoPower="" or instr(","& gysInfoPower &"," , ","& cateid &",")>0 then
			else
				GysLinkHTML = gystitle
			end if
		else
			GysLinkHTML=""
		end if
	end function
	Public Function setZjmxZdyCol(lvw)
		Dim rsf, rsf2, k, h, db, len_rsf, arr_rsf, lvalue
		Set rsf = cn.execute("select title,name,sort,set_open,kd,sorce,gate1 from zdymx where sort1=28 and name like 'zdy%' order by gate1 asc")
		If Not rsf.eof Then
			arr_rsf = rsf.GetRows()
		end if
		rsf.close
		Set rsf = Nothing
		If isArray(arr_rsf) Then
			len_rsf = ubound(arr_rsf,2)
		else
			len_rsf = -1
			len_rsf = ubound(arr_rsf,2)
		end if
		For k=0 to len_rsf
			db = "p_"& arr_rsf(1,k)
			Set h = lvw.headers.getItemByDbname(db)
			If arr_rsf(3,k) = 1 Then
				h.title = arr_rsf(0,k)
				h.width = arr_rsf(4,k)
				if arr_rsf(1,k)="zdy5" or arr_rsf(1,k)="zdy6" then
					h.align2 = "center"
				end if
			else
				h.width = 0
	...
	Public Function powerIntro(sort1,sort2)
		Dim rsf, qx_open, qx_intro, qlsort, qxtmp, r
		Set rsf = cn.execute("select (case QL.sort when 3 then (case P.qx_open when 3 then '' when 1 then P.qx_intro else '0' end) when 1 then (case P.qx_open when 1 then '' else '0' end) else '0' end) from qxlb Q,qxlblist QL,power P where QL.sort1 = Q.sort1 and P.sort1 = QL.sort1 and P.sort2=QL.sort2 and P.ord="& session("personzbintel2007") &" and QL.sort1="& sort1 &" and QL.sort2="& sort2)
		if not rsf.eof then
			r = rsf.fields(0).value
			if len(r) > 0 then
				r =  replace("" & r & ""," ","")
				while instr(r,",,") > 0
					r = replace(r,",,",",")
	...
	Public function formatNum(numf,dot_num)
		Dim numf2
		if numf&""<>"" then
			if numf>1 then
				formatNum = round(numf,dot_num)
			elseif numf>0 and numf<1 then
				numf2 = cstr(round(numf,dot_num))
				if left(numf2,1)="." then
					formatNum = "0"& round(numf,dot_num)
