...
'If isEmpty(m_wxPayKindId) Then
	...
	Sub onReportInit(rpt, lt)
		...
		rpt.sql = "EXEC [ERP_GoodsSaleNumList] "&Info.User&",&ReportSort,@category,@status,@sType,@keywords,"& validTime &",&pagesize,&pageindex,&excelmode"
...
'Function FormatNum(num)
	...
	Sub App_saveGoodsNum
		...
		If goodsNum <> "" And IsNumeric(goodsNum) Then
			conn.CursorLocation = 3
			conn.begintrans
			cn.Execute("INSERT INTO Shop_StorageAppendLog (goodsId,num1,creator,createtime) VALUES ("& goodsID &","& goodsNum &","& Info.User &",'"& Now() &"')")
			If Err.number <> 0 Then
				conn.rollbacktrans
	...
	Sub App_addNumPage
		...
		Response.write "</div>" & vbcrlf & "       <form id=""editForm"" name=""editForm"" method=""post"" action=""?"">" & vbcrlf & "           <table id=""content"">" & vbcrlf & "                      <tbody>" & vbcrlf & "                         <tr>" & vbcrlf & "                                    <th>商品名称：</th>" & vbcrlf & "                                     <td>"
		...
		Response.write info.floatnumber
		Response.write "')"" dataType=""number"" min=""-99999999.99999999"" max=""99999999.99999999"" msg=""必填"" value="""">" & vbcrlf & "                                 <span class=""red"">*</span>" & vbcrlf & "                                </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </tbody>" & vbcrlf & "                </table>                " & vbcrlf & "        </form>" & vbcrlf & " <div class=""description"">说明：追加数量可以是正数、也可以是负数。负数不能大于可售数量</div>" & vbcrlf & "</div>" & vbcrlf & ""
	end sub
