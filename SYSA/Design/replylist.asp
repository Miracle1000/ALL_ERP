...
'Private Sub Class_Initialize
	...
	Sub onReportInit(rpt, lt)
		Dim title,remindSql,remind
		title = "设计进展列表"
		remind = app.getint("remind")
		If remind > 0 Then
			Set helper = CreateReminderHelper(cn, remind ,0)
			remindSql = Replace(helper.listSQL("ids"),"'","''")
			title = helper.name &"提醒"
		end if
		rpt.title = title
		rpt.addsort "添加时间","date7"
		rpt.AddField2 "添加时间","dates","date7", "", "", Split(",",","),""
		rpt.addfield "进展内容","text","intro", "", "", ""
		rpt.AddField2 "","select","B", "提醒人员"&Chr(1)&"reminder"&Chr(2)&"添加人员"&Chr(1)&"catename", "", "","进展主题=#title"
		rpt.addfield "","text","C", "", "", ""
		...
		rpt.canExcel = false
		rpt.canPrint = false
		Dim sql : sql = "exec [erp_design_replyList] "& Info.User &",@date7_0,@date7_1,@intro,@B,@C,'"& remindSql &_
		"',&SearchMode,&excelmode,&pagesize,&pageindex,&ReportSort"
		rpt.sql = sql
	end sub
	Sub App_doBatHandle(cmd, data, result)
		...
		If cmd = "批量删除" Or  cmd = "删除" Then
			...
			While rs.eof =False
				If rs("url") = "url" Then
					cn.execute("update design set del = 2 where id = "& rs("id"))
				else
	...
	Sub app_onCreateList(lvw,rpd)
		...
		.title = "进展内容"
		.dbtype="str"
		.formatText = "code:replyhtml(rs(""id""),@value ,rs(""ord2"") ,rs(""uploadfile"") )"
		.width = "200"
		End With
		With lvw.headers("dp")
		.title = "最新点评"
		.dbtype="str"
		.formatText = "code:getDpHtml(rs(""id""))"
		.width = "200"
		End With
		With lvw.headers("reminders")
		.title = "提醒人员"
		.formatText = "code:getRemindersName(@value)"
		.width = "120"
		End With
		With lvw.headers("name2")
		.title = "添加人员"
		.width = "80"
		End With
		With lvw.headers("date7")
		.title = "添加时间"
		.width = "120"
		End With
		With lvw.headers("cateid")
		.width = "170"
		.formatText = "code:buttonhtml(@value,rs(""id"") ,rs(""date7"") , rs(""ord2"") ,"& remind &")"
		End With
		lvw.headers("uploadfile").display="none"
		lvw.headers("ord2").display="none"
	end sub
	Function getRemindersName(Reminders)
		Dim s : s = ""
		If Len(Reminders&"")>0 Then
			If Reminders <> "0" Then
				Dim rs : Set rs = cn.execute("select name from gate where del=1 and ord in ("& Reminders &")")
				While rs.eof=False
					s= s & app.iif(Len(s)>0 ," " ,"") & rs("name")
					rs.movenext
				wend
				rs.close
			end if
	...
	Function titlehtml(value ,ord, url , remind)
		Dim s : s = value
		If url="url" And ord>0 Then
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(ord) & "','newwin','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">"& value &"</a>"
'If url="url" And ord>0 Then
		end if
		titlehtml = s
	end function
	Function buttonhtml(cateid, id ,date7 , ord2, remind)
		Dim s,rid : s = ""
		If remind>0 Then
			set helper =  CreateReminderHelper(cn,remind,0)
		...
		if session("cateidzbintel")<>"4" Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('dianpin.asp?ord=" & app.base64.pwurl(id) & "&sortreply="& app.base64.pwurl(5029) &"','newwin22','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">点评</button> "
'if session("cateidzbintel")<>"4" Then
		end if
		If sdk.setup.isAllowHandle(CLng(cateid),date7,0) Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:window.open('reply.asp?action=listAdd&replyid=" & app.base64.pwurl(id) & "','newwin22','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">修改</button> "
'If sdk.setup.isAllowHandle(CLng(cateid),date7,0) Then
		end if
		If sdk.setup.isAllowHandle(CLng(cateid),date7,1) Then
			s = s & "<button class=""oldbutton2"" onclick=""javascript:if(confirm('确认删除?')){window.open('delreply.asp?act=list&ord="&app.base64.pwurl(ord2)&"&id=" & app.base64.pwurl(id) & "&sortreply=5029','newwindel','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150');}"">删除</button> "
		end if
		buttonhtml = s
	end function
	Function getDpHtml(id)
		Dim rs7 , s : s = ""
		set rs7 = cn.execute("select top 1 id,intro,name,cateid,date7 from dianping where ord="& id &"  and sort=5029 order by date7 desc")
		If rs7.eof=false Then
			s = "<span style=""color:#CC3333"">"&rs7("intro")&" <br>("&rs7("name")&"&nbsp;"&rs7("date7")&")</span>"
			rs7.movenext
		end if
		rs7.close
		getDpHtml = s
	end function
	Function replyhtml(id,intro ,ord ,uploadfile)
		Dim s :s = ""
		If ord>0 Then
			If Len(uploadfile&"")>0 Then uploadfile = Getuploadfile(uploadfile)
			if intro<>"" Or uploadfile&""<>"" Then
				s = "<a href='#' onClick=""javascript:window.open('all.asp?ord="&app.base64.pwurl(ord)&"&sortreply="&app.base64.pwurl(5029)&"&replyid="& app.base64.pwurl(id)  &"','newwinsd3','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;""title='点击查看该设计全部进展' >"&intro&"</a>"& uploadfile &" "
			end if
		end if
		replyhtml = s
	end function
