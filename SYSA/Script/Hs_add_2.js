...
//将字符串str1中所有从字符char1到char2的子字符串转换为字符串str2
function jisuangongsi(str1,char1,char2,str2)
{
	var n1,n2,str;
	n1=str1.indexOf(char1);
	n2=str1.indexOf(char2);
	if ((n1>-1) && n2>n1)
	{
		str=str1.substring(n1,n2+1);
		str1=str1.replace(str,str2);
		return jisuangongsi(str1,char1,char2,str2);
	}
	else
	{	
		return str1;
...
//字符串str1中所有从字符char1到char2的子字符串在str2中是否出现
function compareStr(str1,char1,char2,str2){
	var n1,n2,str;
	n1=str1.indexOf(char1);
	n2=str1.indexOf(char2);
	if ((n1>-1) && n2>n1)
	{
		...
		{
			str1=str1.substring(n2+1,str1.length);
			compareStr(str1,char1,char2,str2);
		}
		else
		{
			str4="对不起,以下可选项中,没有"+str;
			alert(str4);
			return false;
		}
	}
...
function compare(){
	str1=document.getElementById("salaryClass").value;
	str2=document.getElementById("strlist").innerHTML;
	jisuan=jisuangongsi(str1,"{","}",1);
	if (jisuan || jisuan=="")
	{
		try
		{
...
function onsubm(obj)
{
	...
	{
        if(!document.getElementById('may3').checked){
		return compare();
        }
        return true;        
	}
