...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,1).Value = "地址"
	...
	if Session("SSExportOrdList")="" then
		sql="select * from email_Person  "&Str_Result&" "&str_sql_checkid&" order by ord desc"
	else
		sql="select * from email_person where ord in (" & Session("SSExportOrdList") & ") " & Str_Result & " order by ord desc"
	end if
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "邮件地址_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof
		name=rs("clictName")
		...
		xlWorksheet.Cells(1+i,1).Value = email
		'cateid=""
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>邮件地址导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!邮件地址导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("邮件地址_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的邮件地址</u></strong></font></a></p> " & vbcrlf & ""
