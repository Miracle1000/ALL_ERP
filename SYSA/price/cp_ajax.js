...
function callServer2(nameitr,ord,id) {
   ...
   if ((u_name == null) || (u_name == "")) return;
  var url = "cu_lishi.asp?unit=" + escape(u_name)+"&ord="+escape(ord)+"&id="+escape(id)+"&nameitr="+escape(nameitr) + "&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
  xmlHttp.open("GET", url, false);
  xmlHttp.onreadystatechange = function(){
...
var money1=price.value.replace(/\,/g,'') * num.value.replace(/\,/g,'') * zhekou.value.replace(/\,/g,''); 
