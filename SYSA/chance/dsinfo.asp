...
'If ii<Len(formula) then
	...
	Function ChanceLinkHTML(chId, chtit, chadd)
		If GetChancePowerB = "" Then
			...
			GetChancePower = app.power.GetPowerIntro(3,14)
		end if
		If Len(GetChancePower) > 0 Then
			If InStr("," & GetChancePower & ",","," & chadd & ",") > 0 Then
	...
	Sub PageLoad()
		...
		Response.write "" & vbcrlf & "<script src= ""../Script/ce_dsinfo.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<style>" & vbcrlf & "    body.defcomm{background-image:none;}" & vbcrlf & "   #content .label{" & vbcrlf & "                width:140px;" & vbcrlf & "            text-align:right;" & vbcrlf & "               padding-right:8px;" & vbcrlf & "      }" & vbcrlf & "       #content td{height:32px;}" & vbcrlf & "    .oldbutton{" & vbcrlf & "     background-color:#FFF;" & vbcrlf & "    border:1px solid #FFF;" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "<form method=""post"" name=""myform"" action=""?__msgId=doSave"" onsubmit=""return CheckForm();"">" & vbcrlf & "  <input type=""hidden"" name=""itemId"" id=""itemId"" value="
		Response.write id
		Response.write """ />" & vbcrlf & "  <input type=""hidden"" name=""userId"" id=""userId"" value=""0"" />" & vbcrlf & "  <table id=""content"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'style='border-top:0px'>关联对手：</td>" & vbcrlf & "      <td align=""left"" style='border-top:0px;padding-left:5px;'><input type=""text"" style=""width:280px;color:#bbb"" readonly=""readonly"" value=""选择关联竞争对手"" name=""dsinfo"" id=""dsinfo"" onclick=""javascript:window.open('?__msgId=Search','eventcom','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=150,top=150');return false;"" />" & vbcrlf & "        <span id=""msgInfo"" style='color:red'></span></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手报价：</td>" & vbcrlf & "      <td align=""left"" style='padding-left:5px;'><input type=""text"" name=""dsMoney"" id=""dsMoney"" maxlength=""15"" value=""0"" onpropertychange='formatData(this,""money"")'/>" & vbcrlf & "        (元) <span id=""msgprice"" style='color:red'>*</span></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手优势：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""dsYs"" id=""dsYs""></textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手劣势：</td>" & vbcrlf & "      <td align=""left"" style='padding:5px;'><textarea style=""width:400px;height:50px;"" name=""dsLs"" id=""dsLs""></textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    "
		Dim fieldTextId,oRs,oSQL
		fieldTextId = ""
		Set oRs = server.CreateObject("adodb.recordset")
		oSQL = "Select id,fieldName,fieldtype,fieldemun,"
		oSQL = oSQL & "sort1 From sys_billfieldconfig Where OrderID = 1 And IsDel = 1 order by sort1 asc"
		oRs.Open oSQL, cn, 1, 1
		If Not oRs.EOF Then
			Do While Not oRs.EOF
				fieldTextId = fieldTextId & oRs("id") & ","
				Response.write "" & vbcrlf & "              <!-- #region -->" & vbcrlf & "                <tr>" & vbcrlf & "              <td align=""right"">"
				fieldTextId = fieldTextId & oRs("id") & ","
				Response.write oRs("fieldName")
				Response.write "：</td>" & vbcrlf & "                <td align=""left"">"
				Response.write ZdyFun(oRs("fieldtype"),"dszdy_" & oRs("id"),oRs("fieldemun"),"")
				Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "           <!-- #endregion -->"
				Response.write ZdyFun(oRs("fieldtype"),"dszdy_" & oRs("id"),oRs("fieldemun"),"")
				oRs.MoveNext
			Loop
		end if
		...
		Set oRs = Nothing
		If fieldTextId <> "" Then
			fieldTextId = Left(fieldTextId,Len(fieldTextId) - 1)
'If fieldTextId <> "" Then
		end if
		Response.write "" & vbcrlf & "    <tr>" & vbcrlf & "      <td align=""right"">备注：<input type=""hidden"" name=""zdyCount"" value="""
		Response.write fieldTextId
		Response.write """ /></td>" & vbcrlf & "      <td align=""left""><textarea class=""textbox"" style=""display:none;"" name=""intro"" id=""intro""></textarea>" & vbcrlf & "        <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=intro&style=news&isNew=0"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""98%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME></td> "& vbcrlf &"     </tr> "& vbcrlf &"     <tr>" & vbcrlf & "      <td colspan=""2"" align=""center""><input type=""submit"" value=""保存"" idet"" value=""重填"" class=""button"" id='sb2' /></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & "</form>" & vbcrlf & ""
		Response.write("</body>") & vbCrLf
		Response.write("</html>") & vbCrLf
	end sub
	Sub doSave()
		Dim itemId, userId, dsMoney, dsYs, dsLs, intro, SQL, Rs, zdyCount, arrSp, BillID, strValue, i, oSQL, oRs
		...
		SQL = "Select * From zjds where 1=0"
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.Open SQL, cn, 1, 3
		Rs.AddNew
		...
		rs.close
		set rs = nothing
		zdyCount = app.GetText("zdyCount")
		If zdyCount <> "" Then
			BillID = ChkNumEric(Cn.Execute("Select Top 1 Max(id) From zjds")(0))
			If InStr(zdyCount, ",") > 0 Then
				arrSp = Split(zdyCount, ",")
				For i = 0 To UBound(arrSp)
					strValue = app.GetText("dszdy_" & arrSp(i))
					oSQL = "Select * From sys_billfieldsdata where 1=0"
					Set oRs = server.CreateObject("adodb.recordset")
					oRs.Open oSQL, Cn, 1, 3
					oRs.AddNew
					oRs("fieldid") = arrSp(i)
					oRs("BillID") = BillID
					oRs("BillListID") = 0
					if len(strValue) > 200 then strValue = left(strValue,200)
					oRs("value") = strValue
					oRs.Update
					oRs.Close
					Set oRs = Nothing
				...
				strValue = app.GetText("dszdy_" & zdyCount)
				oSQL = "Select * From sys_billfieldsdata where 1=0"
				Set oRs = server.CreateObject("adodb.recordset")
				oRs.Open oSQL, Cn, 1, 3
				oRs.AddNew
				oRs("fieldid") = zdyCount
				oRs("BillID") = BillID
				oRs("BillListID") = 0
				oRs("value") = strValue
				oRs.Update
...
'if len(powers) > 0 then
	...
	Sub Search()
		...
		Response.write("    document.getElementById('lvw_dbtable_ids').style.borderTop='1px solid #c0ccdd';") & vbCrLf
...
'if len(powers) > 0 then
		...
		lvw.headers(1).display = "none"
		lvw.headers(2).formatText = "<a href=""javascript:void(0);"" onclick=""doSub('@value','@cells[1]')"">点击选择此对手</a>"
		Response.write("<div style='position:absolute;right:20px;top:3px;'>对手名称：<input type=text onkeypress='if(window.event.keyCode==13){dosearch(this.value)}'></div><div style='overflow:hidden;border-top:0px solid #red;' id='listbody'>")
		lvw.headers(2).width = "200"
		lvw.headers(3).width = "300"
		lvw.headers(4).width = "200"
		lvw.width = "100%"
	...
	Sub ShowInfo()
		Dim strFilePath
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery.js""></script>"
		Dim id, oSQL, oRs, dsName, dsMoney, dsYs, dsLs, intro, userId, addUserId,addUser, addTime, dscateid
		id = ChkNumEric(request("id"))
		oSQL = "Select Top 1 (Select Top 1 name From jzds where jzds.id = zjds.userId and jzds.del=1) As dsName,addTime,dsMoney,dsYs,"
		oSQL = oSQL & "dsLs,intro,userId,addUser,addTime,"
		oSQL = oSQL & "(Select Top 1 name From gate Where ord = zjds.addUser) As AddUserName,(Select Top 1 cateid From jzds where jzds.id = zjds.userId and jzds.del=1) As cateid From zjds Where id = "&id&" "
		Set oRs = server.CreateObject("adodb.recordset")
		oRs.Open oSQL, cn, 1, 1
		...
		qx_print = app.power.GetPowerIntro(3,7)
		hspower = true
		html = "<div id='ccc' style='padding-top:5px'><input type=""button"" value="" 修改 "" class=""oldbutton"" onclick=""window.location.href = '?__msgId=Edit&id=" & id & "';""/>"
		hspower = true
		...
		Response.write "" & vbcrlf & "<style>" & vbcrlf & "      #content .label{" & vbcrlf & "                width:140px;" & vbcrlf & "            text-align:right;" & vbcrlf & "               padding-right:8px;" & vbcrlf & "      }" & vbcrlf & "       #content td{" & vbcrlf & "            height:26px;" & vbcrlf & "    }" & vbcrlf & "       #content{background:;}" & vbcrlf & "  #comm_itembarbg{height:43px!important;}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/ce_dsinfo_4.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<input type=""hidden"" id=""userid"" name=""userid"" value="""
		Response.write id
		Response.write """ />" & vbcrlf & "<table id=""content"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"" width=""15%"" style=""border-top:0px;"">关联对手：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""35%"" style=""border-top:0px;"">" & vbcrlf & "  "
		Response.write id
		If dsName&""<>"" Then
...
'If InStr("," & app.power.GetPowerIntro(43,14) & ",","," & dscateid & ",") > 0  Then
		...
		Response.write "" & vbcrlf & "     " & vbcrlf & "        </td>" & vbcrlf & "    <td align=""right"" width=""15%"" style=""border-top:0px;"">对手报价：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""35%"" style=""border-top:0px;""><span style=""margin-left:10px;"">"
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write Formatnumber(dsMoney,Info.moneynumber,-1)
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write "　(元)</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"" width=""15%"">添加人员：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""35%""><span style=""margin-left:10px;"">"
		Response.write "</a></span>" & vbcrlf & "                                  "
		Response.write addUser
		Response.write "</span></td>" & vbcrlf & "    <td align=""right"" width=""15%"">添加时间：&nbsp;</td>" & vbcrlf & "    <td align=""left"" width=""35%""><span style=""margin-left:10px;"">"
		Response.write addUser
		...
		Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  "
		cn.execute "delete sys_billfieldsdata where BillID = "&id&" and fieldid in (select id from sys_billfieldconfig where orderid=1 and orderlistid=0) and id not in (    select max(id) from sys_billfieldsdata  where BillID = "&id&" and fieldid in (select id from sys_billfieldconfig where orderid=1 and orderlistid=0)   group by  fieldid, billid, billlistID)"
		Set oRs = server.CreateObject("adodb.recordset")
		oSQL = "Select a.fieldName,b.value,a.fieldtype From sys_billfieldconfig a left join sys_billfieldsdata b "
		oSQL = oSQL & "on a.OrderID = 1 and a.orderlistid=0  And a.id = b.fieldid And b.BillID = "&id&" where a.IsDel = 1"
		oSQL = oSQL & "order by a.sort1 asc"
		oRs.Open oSQL, cn, 1, 1
		If Not oRs.EOF Then
			Do While Not oRs.EOF
				Response.write "" & vbcrlf & "  <!-- #region -->" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">"
'Do While Not oRs.EOF
				...
				Response.write "：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"">"
				Response.write oRs(0)
				Response.write HtmlBr(ShowMsgBox(oRs(1),oRs(2)))
				Response.write "</span></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <!-- #endregion -->"
				Response.write HtmlBr(ShowMsgBox(oRs(1),oRs(2)))
				oRs.MoveNext
			Loop
		end if
		oRs.Close
		Set oRs = Nothing
		Response.write "" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""right"">备注：&nbsp;</td>" & vbcrlf & "    <td align=""left"" colspan=""3""><span style=""margin-left:10px;"" class=""ewebeditorImg"">"
		Set oRs = Nothing
	...
	Sub Edit()
		...
		Dim dsName,userId,dsMoney,dsYs,dsLs,intro
		strFilePath = "<script type=""text/javascript"" src=""../inc/jquery.js""></script>"
		...
		id = ChkNumEric(request("id"))
		oSQL = "Select Top 1 (Select Top 1 name From jzds where jzds.id = zjds.userId) As dsName,userId,dsMoney,dsYs,dsLs,intro From zjds Where id = "&id&" "
		Set oRs = server.CreateObject("adodb.recordset")
		oRs.Open oSQL, cn, 1, 1
		If (oRs.EOF And oRs.BOF) Then
		else
		...
		oRs.Close
		Set oRs = Nothing
		Response.write "" & vbcrlf & "<script src= ""../Script/ce_dsinfo_5.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<style>" & vbcrlf & "  #content .label{" & vbcrlf & "                width:140px;" & vbcrlf & "            text-align:right;" & vbcrlf & "               padding-right:8px;" & vbcrlf & "      }" & vbcrlf & "       #content td{" & vbcrlf & "            height:24px;" & vbcrlf & "    }" & vbcrlf & "       #content{background:;}" & vbcrlf & "</style>" & vbcrlf & "<form method=""post"" name=""myform"" action=""?__msgId=doEdit"" onsubmit=""return AjaxSend();"">" & vbcrlf & "  <input type=""hidden"" name=""id"" id=""id"" value="""
		Response.write id
		...
		Response.write """  onpropertychange='formatData(this,""money"")'/>" & vbcrlf & "        (元)</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "     <td class='label'>对手优势：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px'><textarea style=""width:400px;height:50px;"" name=""dsYs"" id=""dsYs"">"
		Response.write dsYs
		Response.write "</textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td class='label'>对手劣势：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px'><textarea style=""width:400px;height:50px;"" name=""dsLs"" id=""dsLs"">"
		Response.write dsLs
		Response.write "</textarea></td>" & vbcrlf & "    </tr>" & vbcrlf & "    "
		Dim DataTextId,DataTextType,sId
		DataTextId = ""
		DataTextType = ""
		dim i
		i = 0
		cn.execute "delete sys_billfieldsdata where BillID = "&id&" and fieldid in (select id from sys_billfieldconfig where orderid=1 and orderlistid=0) and id not in (    select max(id) from sys_billfieldsdata  where BillID = "&id&" and fieldid in (select id from sys_billfieldconfig where orderid=1 and orderlistid=0)   group by  fieldid, billid, billlistID)"
		Set oRs = server.CreateObject("adodb.recordset")
		oSQL = "Select b.id,a.fieldName,a.fieldtype,a.fieldemun,b.value,a.id From sys_billfieldconfig a left join sys_billfieldsdata b "
		oSQL = oSQL & "on a.OrderID = 1 and a.orderlistid=0  And a.id = b.fieldid And b.BillID = "&id&" where a.IsDel = 1 order by a.sort1 asc"
		oRs.Open oSQL, cn, 1, 1
		If Not oRs.EOF Then
			sId = 0
			Do While Not oRs.EOF
				If (oRs(0) = "" Or Not IsNumeric(oRs(0))) Then
					i = i - 1
'If (oRs(0) = "" Or Not IsNumeric(oRs(0))) Then
					sId = i
				else
				...
				DataTextId = DataTextId & sId & ","
				DataTextType = DataTextType & oRs(5) & ","
				Response.write "" & vbcrlf & "    <!-- #region -->" & vbcrlf & "    <tr>" & vbcrlf & "      <td align=""right"">"
				DataTextType = DataTextType & oRs(5) & ","
				Response.write oRs("fieldName")
				Response.write "：</td>" & vbcrlf & "      <td align=""left"" style='padding:4px'>"
				Response.write ZdyFun(oRs("fieldtype"),"dszdy_" & sId,oRs("fieldemun"),oRs("value"))
				Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <!-- #endregion -->"
				Response.write ZdyFun(oRs("fieldtype"),"dszdy_" & sId,oRs("fieldemun"),oRs("value"))
				oRs.MoveNext
			Loop
		...
		Set oRs = Nothing
		If DataTextId <> "" Then
			DataTextId = Left(DataTextId,Len(DataTextId) - 1)
'If DataTextId <> "" Then
		end if
		If DataTextType <> "" Then
			DataTextType = Left(DataTextType,Len(DataTextType) - 1)
'If DataTextType <> "" Then
		end if
		Response.write "" & vbcrlf & "    <tr>" & vbcrlf & "      <td align=""right"">备注：<input type=""hidden"" name=""DataTextType"" value="""
		Response.write DataTextType
		Response.write """ />" & vbcrlf & "    <input type=""hidden"" name=""DataCount"" value="""
		Response.write DataTextId
		Response.write """ /></td>" & vbcrlf & "      <td align=""left""><textarea class=""textbox"" style=""display:none;"" name=""intro"" id=""intro"">"
	...
	Sub doEdit()
		Dim id, userId, dsMoney, dsYs, dsLs, intro, SQL, Rs, strId, strValue, arrSp, i, oSQL, oRs, DataTextType, arrDataType
		id = app.GetInt("id")
		...
		intro = app.GetText("intro")
		SQL = "Select * From zjds Where id = "& id &" "
		Set Rs = server.CreateObject("adodb.recordset")
		Rs.Open SQL, cn, 1, 3
		Rs("userId") = userId
		...
		rs.close
		set rs = nothing
		strId = app.GetText("DataCount")
		DataTextType = app.GetText("DataTextType")
		If strId <> "" Then
			If InStr(strId, ",") > 0 Then
				arrDataType = Split(DataTextType, ",")
				arrSp = Split(strId, ",")
				For i = 0 To UBound(arrSp)
					strValue = app.GetText("dszdy_" & arrSp(i))
					oSQL = "Select * From sys_billfieldsdata Where id = " & arrSp(i) & " "
					Set oRs = server.CreateObject("adodb.recordset")
					oRs.Open oSQL, Cn, 1, 3
					If (oRs.EOF And oRs.BOF) Then
						oRs.AddNew
						oRs("fieldid") = ChkNumEric(arrDataType(i))
						oRs("BillID") = ChkNumEric(id)
					end if
					if len(strValue) > 200 then strValue = left(strValue,200)
					oRs("value") = strValue
					oRs.Update
				...
				strValue = app.GetText("dszdy_" & strId)
				oSQL = "Select * From sys_billfieldsdata Where id = " & strId & " "
				Set oRs = server.CreateObject("adodb.recordset")
				oRs.Open oSQL, Cn, 1, 3
				If (oRs.EOF And oRs.BOF) Then
					oRs.AddNew
					oRs("fieldid") = DataTextType
					oRs("BillID") = id
				end if
				oRs("value") = strValue
				oRs.Update
		...
		Response.write("<script>")
		Response.write("try{opener.window.DoRefresh();}catch(e){}")
		Response.write("window.close();")
		Response.write("</script>")
		call db_close : Response.end()
	end sub
	Sub doDel()
		...
		oSQL = "Delete From zjds Where id = "& id &" "
		cn.Execute(oSQL)
		Response.write(1)
		call db_close : Response.end()
	end sub
	Sub List()
		Dim oSQL, strHtml,ItemsId, i
		ItemsId = app.getint("ItemsId")
		Response.write(app.DefTopBarHTML("../", "", "关联对手列表", "<span id='tmv'></span>")) & vbCrLf
		Response.write "" & vbcrlf & "      <style>" & vbcrlf & " </style>" & vbcrlf & "    <script type=""text/javascript"">" & vbcrlf & "    function LoadUrl(str) {" & vbcrlf & "        location.href = str;" & vbcrlf & "    }" & vbcrlf & "    function showmenu(tag) {" & vbcrlf & "        var m = new ContextMenuClass();"& vbcrlf & "        m.id=""asasd"";" & vbcrlf & "        m.menus.add('按照对手名称升序', '1');" & vbcrlf & "        m.menus.add('按照对手名称降序', '2');" & vbcrlf & "        m.menus.add('按照对手报价升序', '3');" & vbcrlf & "        m.menus.add('按照对手报价降序', '4');" & vbcrlf & "        m.menus.add('按照添加时间升序', '5');" & vbcrlf & "        m.menus.add('按照添加时间降序', '6');" & vbcrlf & "        m.onitemclick = function(li){" & vbcrlf & "            var v = li.getAttribute('value');" & vbcrlf & "            $('#px').val(v);" & vbcrlf & "            doSearch();" & vbcrlf & "        }" & vbcrlf & "        m.show();" & vbcrlf & " m.BindElement(tag, 0, tag.offsetHeight);" & vbcrlf & "        return false;" & vbcrlf & "    }" & vbcrlf & "    function DelSend(ids) {" & vbcrlf & "        $.get('?__msgId=doDel', {" & vbcrlf & "            id: ids" & vbcrlf & "        }," & vbcrlf & "        function(data) {" & vbcrlf & "  var msg = parseInt(data);" & vbcrlf & "            if (msg == 1) {" & vbcrlf & "                lvw_refresh('ids1');" & vbcrlf & "            }" & vbcrlf & "        })" & vbcrlf & "    }" & vbcrlf & "    function doSearch(pIndex) {" & vbcrlf & "        ajax.regEvent(""dosearch"")" & vbcrlf& "        ajax.addParam('px', $('#px').val());" & vbcrlf & "        ajax.addParam('stratTime', $('#stratTime').val());" & vbcrlf & "        ajax.addParam('endTime', $('#endTime').val());" & vbcrlf & "        ajax.addParam('stype', $('#stype').val());" & vbcrlf & "        ajax.addParam('key', $('#key').val());" & vbcrlf & "         ajax.addParam(""ItemsId"","
		Response.write ItemsId
		Response.write ");" & vbcrlf & "            ajax.addParam(""Psize"",$('#lvw_pgsize_selids1').val());" & vbcrlf & "            //pIndex==1表示要带上当前的翻页条件" & vbcrlf & "             ajax.addParam(""pageindex"",pIndex==1?document.getElementById(""lvw_pindex_ids1"").value:1);" & vbcrlf & "        ajax.send(function(r){" & vbcrlf & "                      document.getElementById(""lvwbody"").innerHTML = r;" & vbcrlf & "          });" & vbcrlf & "    }" & vbcrlf & "        function doExcel(){var lvw = new Listview('ids1');lvw.cexcel();}" & vbcrlf & "        window.onlistviewRefresh=function() {" & vbcrlf & "           document.getElementById('lvw_dbtable_ids1').style.borderTop='0px solid red';"& vbcrlf & "     } "& vbcrlf &"        window.onload=function() {window.onlistviewRefresh(); } "& vbcrlf &"     </script> "& vbcrlf &  "   <style> " & vbcrlf &"                 #searchitemspanel {margin-top:-1px} "& vbcrlf &  "            #lvw_dbtable_ids1 th {border-top:0px solid #cdcde0}" & vbcrlf &  "    </style> "& vbcrlf & "    <table id=""content"" style='background-image:url();border:0px;background-color:transparent;margin-top:2px'>" & vbcrlf & "    <tr>" & vbcrlf & "    <td align=""right"" style='border-top:0px;border-bottom:0px;border-left:0px'>" & vbcrlf & "    <input type=""hidden"" id=""px"" value=""0"" />" & vbcrlf & " <a href=""javascript:void();"" onclick=""showmenu(this);return false"" style=""color:#000;font-weight:bold"">排序</a>　" & vbcrlf & "    添加时间：" & vbcrlf & "    <input type=""text"" id=""stratTime"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" /> " & vbcrlf & "    - <input type=""text"" id=""endTime"" onclick=""datedlg.show()"" size=""10"" readonly=""readonly"" />　" & vbcrlf &"     <select id=""stype"">　" & vbcrlf &"       <option value=""0"" selected=""selected"">对手名称</option> "& vbcrlf & "      <option value=""1"">项目名称</option> "& vbcrlf &"       <option value=""2"">对手报价</option>" & vbcrlf & "    <option value=""3"">对手优势</option>" & vbcrlf & "      <option value=""4"">对手劣势</option>" & vbcrlf & "      <option value=""5"">备注</option>" & vbcrlf & "    </select>" & vbcrlf & "    <input type=""text""  id=""key"" maxlength=""20"" /> " & vbcrlf & "    <input type=""button"" value=""检索"" class=""oldbutton"" onclick=""doSearch();"" /> " & vbcrlf & "    "
		if app.power.existsPower(3,10) then
			Response.write "<input type=""button"" value="" 导出 "" class=""oldbutton"" id=""dcdb"" onclick='doExcel()'/>&nbsp;"
		end if
		if app.power.existsPower(3,7) then
		...
		Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    </tr>" & vbcrlf & "    </table>" & vbcrlf & "        <script type=""text/javascript"">" & vbcrlf & "   document.getElementById(""content"").swapNode(document.getElementById(""tmv""));" & vbcrlf & "        </script>" & vbcrlf & "       <div style=""overflow:hidden;min-width:900px;border-top:0px solid #cdcde0"" id='lvwbody'>" & vbcrlf & "  "
		Dim lvw
		Set lvw = New listview
		...
		lvw.cansort = False
		lvw.border = 0
		lvw.sql = "exec erp_chance_gldsList " & Info.User & "," & ItemsId & ",&excelmode,0,'','','',0"
		lvw.headers(1).display = "none"
		lvw.headers(2).formattext = "code:ChanceLinkHTML(@cells[14],""@cells[2]"",@cells[13])"
		lvw.headers(3).formattext = "code:  jzdsLinkHTML(@cells[16],""@cells[3]"",@cells[15])"
		lvw.headers(9).formatText = "code:workButtonHTML(@cells[10],@cells[12],@cells[11],@cells[1])"
		lvw.headers(2).dbtype = "str"
		lvw.headers(2).width = 120
		lvw.headers(3).dbtype = "str"
		lvw.headers(3).width = 120
		lvw.headers(4).dbtype = "money"
		lvw.headers(4).width = 60
		lvw.headers(5).dbtype = "str"
		lvw.headers(5).width = 100
		lvw.headers(6).dbtype = "str"
		lvw.headers(6).width = 100
		lvw.headers(7).width = 60
		lvw.headers(8).width = 60
		lvw.headers(9).width = 110
		For i=10 To 16
	...
	Sub SearchResult
		...
		lvw.cansort = False
		lvw.sql = "exec erp_chance_gldsList " & Info.User & "," & ItemsId & ",&excelmode," & app.GetInt("stype") & ",'" & k & "','" & d1 & "','" & d2 & "'," & app.GetInt("px")
		lvw.headers(1).display = "none"
		lvw.headers(2).formattext = "code:ChanceLinkHTML(@cells[14],""@cells[2]"",@cells[13])"
		lvw.headers(3).formattext = "code:  jzdsLinkHTML(@cells[16],""@cells[3]"",@cells[15])"
		lvw.headers(9).formatText = "code:workButtonHTML(@cells[10],@cells[12],@cells[11],@cells[1])"
		...
		Response.write Replace(lvw.html,"border-top:1px solid #cccddc;","border-top:0px solid #cccddc;")
		lvw.pagesize = app.getint("Psize")
	end sub
