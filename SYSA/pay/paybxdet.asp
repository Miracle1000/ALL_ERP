...
'If Not rs.Eof Then
	...
	m3=request("indate_ret1")
	m4=request("indate_ret2")
	B=request("B")
	C=request("C")
	px=request("px")
	typ=request("typ")
	...
	payid=request("payid")
	stel=request("stel")
	scontract=request("scontract")
	sperson=request("sperson")
	sricheng=request("sricheng")
	sshouhou=request("sshouhou")
	schance=request("schance")
	scaigou=request("scaigou")
	sfahuo=request("sfahuo")
	swork=request("swork")
	...
	if stel<>"" then
		Str_Result=Str_Result&" and c.company in (select ord from tel where name like '%"&stel&"%') "
	end if
	if sperson<>"" then
		Str_Result=Str_Result&" and c.person in (select ord from person where name like '%"&sperson&"%') "
	end if
	if scontract<>"" then
		Str_Result=Str_Result&" and c.contract in (select ord from contract where title like '%"&scontract&"%') "
	end if
	if sshouhou<>"" then
		Str_Result=Str_Result&" and c.shouhou in (select ord from tousu where title like '%"&sshouhou&"%') "
	end if
	if scaigou<>"" then
		Str_Result=Str_Result&" and c.caigou in (select ord from caigou where title like '%"&scaigou&"%') "
	end if
	if swork<>"" then
		Str_Result=Str_Result&" and c.iwork in (select ord from tel where name like '%"&swork&"%') "
	end if
	if schance<>"" then
		Str_Result=Str_Result&" and c.chance in (select ord from chance where title like '%"&schance&"%') "
	end if
	if sfahuo<>"" then
		Str_Result=Str_Result&" and c.fahuo in (select ord from send where title like '%"&sfahuo&"%') "
	end if
	if sricheng<>"" then
		Str_Result=Str_Result&" and c.richeng in (select ord from plan1 where intro like '%"&sricheng&"%') "
	end if
	if f<>"" then
		Str_Result=Str_Result&" and b.sort in("&f&") "
	end if
	if payid<>"" then
		Str_Result=Str_Result&" and a.bh like '%"&payid&"%' "
	end if
	if listbz<>"" then
		listbz=cdbl(listbz)
	...
	if e<>"" then
		Str_Result=Str_Result&" and a.complete in ("&e&") "
	end if
	if title<>"" then
		Str_Result=Str_Result&" and  a.title like '%"&title&"%' "
	end if
	if intro<>"" then
		Str_Result = Str_Result & " and  c.intro like '%"&intro&"%' "
	end if
	if money3<>""and money3>0   then
		if isnumeric(money3)=false then
			Response.write("<script>alert('请输入合法金额');window.location=('paybx.asp');</script>")
			call db_close : Response.end
		else
			if str="" then
				str=str+" where smoney>="&money3&" "
'if str="" then
			else
				str=str+" and smoney>="&money3&" "
'if str="" then
			end if
		end if
	end if
	if money12<>"" then
		if isnumeric(money12)=false then
			...
			if str="" then
				str=str+" where smoney<="&money12&" "
'if str="" then
			else
				str=str+" and smoney<="&money12&" "
'if str="" then
			end if
		end if
	end if
	if request("ret_sp1")<>"" and request("ret_sp2")<>"" then
		Str_Result=Str_Result&" and a.id in(select ord from (SELECT MAX(date1) AS date1,ord FROM sp_intro WHERE sort1=4 GROUP BY ord) a where date1>='"&cdate(request("ret_sp1"))&" 00:00:00' and date1<='"&cdate(request("ret_sp2"))&" 23:59:59' ) "
	end if
	if request("ret_sp3")<>"" and request("ret_sp4")<>"" then
		Str_Result=Str_Result&" and a.id in(select gl from bank where sort=40 and date1>='"&request("ret_sp3")&" 00:00:00' and date1<='"&cdate(request("ret_sp4"))&" 23:59:59' ) "
	end if
	if tjr<>"" then
	...
	elseif typ="10" then
		Str_Result=Str_Result&" and a.complete in (0,1)"
	end if
	if B="1" then
	...
	if px=1 then
		pxResult=" order by title desc,indate desc"
	elseif px=2 then
		pxResult=" order by title asc,indate asc"
	elseif px=5 then
		pxResult=" order by bxdate desc,indate desc"
	elseif px=6 then
		pxResult=" order by bxdate asc,indate asc"
	elseif px=3 then
		pxResult=" order by indate desc"
	elseif px=4 then
		pxResult=" order by indate asc"
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & ".ser_btn input[type='text']{height:20px;line-height:20px;}" & vbcrlf & ".ser_btn select{position:relative;top:1px;*top:0}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  "
	Response.write Application("sys.info.jsver")
	if open_6_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "<td width=""100%"" valign=""top"">" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <form action=""paybxdet.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" >" & vbcrlf & "    <tr>" & vbcrlf & "            <td class=""place"">费用报销明细表</td>" & vbcrlf & "             <td height=""32""><div align=""left""><A class='px_btn' onclick=""Myopen(User);return false;"" href=""javascript:void(0)"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></div></td>" & vbcrlf & "               <td><div align=""right"">" & vbcrlf & "                 <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value)}"" >" & vbcrlf & "                    <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	Response.write """>" & vbcrlf & "                        </div></td>" & vbcrlf & "       </tr>" & vbcrlf & " </table> " & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr height=""50px"">" & vbcrlf & "                        <td colspan=""11"" align=""left"" class='ser_btn' style='height:50px;line-height:50px;padding-top:0;padding-bottom:0;'><div align=""right"">" & vbcrlf & "                          <input type=""radio"" name=""rad1"" onClick=""window.location='paybx.asp?"
	Response.write page_count
	Response.write sdk.ReturnURL("sflg=")
	Response.write "';"" checked=""checked"">" & vbcrlf & "                        列表" & vbcrlf & "                          <input type=""radio"" name=""rad1"" onClick="""" checked=""checked"">" & vbcrlf & "                           明细 " & vbcrlf & "                           "
	...
	set rs8=nothing
	Response.write "" & vbcrlf & "     </select>" & vbcrlf & "                               <span id=""ss1"">                                                           " & vbcrlf & "                                <select name=""typ"" id=""typ"">" & vbcrlf & "        <option value=""0"" "
	...
	Response.write "                                     " & vbcrlf & "                                <select name=""B"" id=""B"">" & vbcrlf & "                              <option value=""1"" "
	if B="1" then
		Response.write "selected"
	end if
	...
	Response.write ">添加人员</option>" & vbcrlf & "                           </select>" & vbcrlf & "                               <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	...
	if open_6_10=1 or open_6_10=3 then
		Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_fylist1.asp'});}"" class=""anybutton2""/>" & vbcrlf & "                                "
	end if
	if open_6_7=1 or open_6_7=3 then
		Response.write "" & vbcrlf & "                             <input type=""button""  name=""print"" onclick=""javascript:ss1.style.display='none';window.print();return   false;"" value=""打印""  class=""anybutton2""/>" & vbcrlf & "                                "
	end if
	Response.write "" & vbcrlf & "                             </span>                           " & vbcrlf & "                        </div></td>" & vbcrlf & "             </tr>" & vbcrlf & "         </form>" & vbcrlf & "         <tr class=""top"">" & vbcrlf & "           <td width=""10%""><div align=""center"">报销编号</div></td>" & vbcrlf & "              <td><div align=""center"">报销主题</div></td>" & vbcrlf & "               <td width=""9%""><div align=""center"">费用分类</div></td>" & vbcrlf & "                <td width=""9%""><div align=""center"">报销金额</div></td>      " & vbcrlf & "                <td width=""9%""><div align=""center"">审批金额</div></td>" & vbcrlf & "              <td width=""6%""><div align=""center"">报销人员</div></td>                             " & vbcrlf & "                 <td width=""6%""><div align=""center"">添加人员</div></td>" & vbcrlf & "            <td width=""7%""><div align=""center"">报销日期</div></td>" & vbcrlf & "              <td width=""7%""><div align=""center"">报销状态</div></td>" & vbcrlf & "              <td width=""10%""><div align=""center"">报销备注</div></td>" & vbcrlf & "             <td><div align=""center"">关联使用单</div></td>" & vbcrlf & "              </tr>" & vbcrlf & "           "
	dim n ,tableSQL ,hsaTempTable
	hsaTempTable = False
	Dim ExtID : ExtID = 0
	...
	If Len(afv_existssql)>0 Then
		...
		If Len(existssql)>0 Then
			ExtID = deurl(request("ExtID"))
			date1 = request("date1")
			If Len(date1)=0 Then date1 = date
			conn.execute("select top 0 cast(0 as money) as id into #existssqltable; insert into #existssqltable(id) " & Replace(Replace(existssql,"@ExtID",ExtID),"@date1" , date1))
			Str_Result = Str_Result & " and b.id in (select id from #existssqltable) "
			hsaTempTable = True
		end if
	end if
	n=0
	tableSQL = "select a.id,a.bh,a.indate,a.complete,a.title,a.bz,a.bxdate,a.cateid_sp,b.id as paybxlistid,"&_
	"c.fid as fid,b.money1 as money1,isnull(c.money1,0) as money2,fy.title as paytitle,c.cateid as paycate,d.name as payer,"&_
	"e.name as adder,b.money1 as summoney,isnull(c.money1,0) as smoney,c.num as sumnum, " &_
	"isnull(s.intro,'RMB') as trade8,b.intro,p.sort1 as ftype,y.sort1 as fftype  "&_
	"from paybx a WITH(NOLOCK) "&_
	"inner join paybxlist b WITH(NOLOCK) on a.id=b.bxid "&_
	"left join pay c WITH(NOLOCK) on c.ord=b.payid "&_
	"left join f_pay fy WITH(NOLOCK) on fy.id=c.fid "&_
	"left join gate d WITH(NOLOCK) on c.cateid=d.ord "&_
	"left join gate e WITH(NOLOCK) on a.addcate=e.ord "&_
	"left join sortbz s on s.id = a.bz "&_
	"left join paytype p on p.id = b.sort "&_
	"left join sortonehy y on y.ord = p.sort2 "&_
	"where a.del=1  and b.del=1 "&Str_Result&" "
	sqltext="select isnull(sum(summoney),0) summoney,isnull(sum(smoney),0) smoney from ("& tableSQL & ") aa "& str &" "
	Set rss = conn.execute(sqltext)
	money2_all = CDbl(rss("smoney"))
	money1_all = CDbl(rss("summoney"))
	rss.close
	Set rss=Nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from ( "&  tableSQL & ") aa "&str&" " & pxResult
	...
	if rs.eof then
		...
		Response.write "<form name=""form1"" method=""post"" action=""deletebxall.asp?"
		Response.write sdk.ReturnURL("sflg=")
		Response.write """>"
		smoney_all=0
		money_all=0
		do while not rs.eof
			...
			cateid=rs("paycate")
			paytitle=rs("paytitle")
			if paytitle="" or isnull(paytitle) then paytitle="无主题"
			complete=rs("complete")
			trade8=rs("trade8")
			ftype=rs("ftype")
			fftype=rs("fftype")
			bxintro=rs("intro")
			If complete&"" = "" Then complete = 0
			...
			if open_6_14=3 or (open_6_14=1 and CheckPurview(intro_6_14,trim(cateid))=True) then
				Response.write "" & vbcrlf & "                                             <a href=""###"" onClick=""javascript:window.open('paybxcontent.asp?paybxid="
				Response.write ord
				Response.write "','neww3win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write ord
				Response.write k
			...
			if ftype<>"" then Response.write fftype&"→"&ftype
			...
			Response.write bxintro
			Response.write "</div></td>" & vbcrlf & "                                  <td><div align=""left"">" & vbcrlf & "                                    "
			if open_6_14=3 or CheckPurview(open_6_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                                             <a href=""###"" onClick=""javascript:window.open('paydetail.asp?ord="
				Response.write rs("fid")
				Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=200');"">" & vbcrlf & "                                             "
				Response.write rs("fid")
				Response.write paytitle
				Response.write "</a> " & vbcrlf & "                                        "
			else
				Response.write paytitle
			end if
			Response.write "</div>" & vbcrlf & "                                       </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           "
			money_all=cdbl(money1)+cdbl(money_all)
			smoney_all=cdbl(money2)+cdbl(smoney_all)
			n=n+1
			rs.movenext
			if rs.eof or n>=rs.PageSize then exit do
		loop
		Response.write "" & vbcrlf & "                     <tr>" & vbcrlf & "                    <td height=""25"" colspan=""3""><div align=""right"">本页合计：</div></td>" & vbcrlf & "                  <td class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		Response.write Formatnumber(smoney_all,num_dot_xs,-1)
		Response.write "&nbsp;"
		...
		Response.write Formatnumber(money_all,num_dot_xs,-1)
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "                  <td colspan=""6""></td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   <tr>" & vbcrlf & "                    <td height=""25"" colspan=""3""><div align=""right"">所有合计：</div></td>" & vbcrlf & "                  <td class=""red""><div align=""right"">"
		if listbz<>"" then
			Response.write trade8
			Response.write "&nbsp;"
		end if
		...
		Response.write "&nbsp;"
		Response.write "</div></td>" & vbcrlf & "                  <td colspan=""6""></td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                        </td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   <tr>" & vbcrlf & "                    <td  class=""page"">" & vbcrlf & "                       <table width=""100%"" cellpadding=""3"" border=""0"" align=""left"" >"& vbcrlf & "                      <tr>" & vbcrlf & "                    <td height=""30"" colspan=""4""><div align=""right"">" & vbcrlf & "                         "
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "                    <input  name=""cpage"" id=""cpage"" type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')"" size=""3""  >" & vbcrlf & "                        &nbsp;<input type=""button"" name=""btntz"" value=""跳转""  class=""anybutton2"" onClick=""gotourl('currpage='+document.getElementById('cpage').value);""/>" & vbcrlf & "                        "
		if currpage=1 then
	...
	rs.close
	set rs=Nothing
	If hsaTempTable = True Then
		conn.execute("drop table #existssqltable ")
	end if
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:152;display:none;"">" & vbcrlf & "<table width=""150"" height=""160""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""152"">" & vbcrlf & "        <table width=""150"" height=""152"" bgcolor=""#ecf5ff"" border=""0"">" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按报销主题排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按报销主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <tdheight=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按报销日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6')""><font color=""#2F496E"">按报销日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	Response.write "" & vbcrlf & "<div id=""w""  title=""高级检索""  style=""width:700px;height:520px;padding:5px;background: #fafafa;top:60px;display:none;"" class=""easyui-window"" closed=""true"" >" & vbcrlf & "<form method=""get"" action=""paybxdet.asp"" id=""date1"" onSubmit=""return Validator.Validate(this,2)""name=""date22"">" & vbcrlf & "<div id=""hideDiv"" style=""border-style: none; border-width: 0px; border-color: inherit; margin: 0px; display:none; padding:0px;"">" & vbcrlf & "    <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:650px"">" & vbcrlf & "               <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "                            <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                         <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                  </div>" & vbcrlf & "                  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf& "          <td width=""14%"" class=""func""><div align=""right"">报销主题</div></td>" & vbcrlf & "          <td width=""36%""><label><input name=""title"" type=""text"" id=""title""></label></td>" & vbcrlf & "          <td width=""13%""><div align=""right"">报销编号</div></td>" & vbcrlf & "          <td width=""37%""><input name=""payid"" type=""text"" id=""payid""></td>" & vbcrlf & "         </tr>" & vbcrlf & "                     <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td><div align=""right"">报销备注</div></td>" & vbcrlf & "                        <td><input name=""intro"" type=""text"" id=""intro""></td>" & vbcrlf & "                <td><div align=""right"">报销金额</div></td>" & vbcrlf & "                <td><input name=""money1"" type=""text""  class=""easyui-numberbox"" id=""money1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision="""
	Response.write num_dot_xs
	Response.write """ onpropertychange=""formatData(this,'money');"" value=""0"">" & vbcrlf & "                  -" & vbcrlf & "                  <input name=""money12"" type=""text"" id=""money12"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision="""
	Response.write num_dot_xs
	Response.write num_dot_xs
	Response.write """ onpropertychange=""formatData(this,'money');"" value=""999999999""></td>" & vbcrlf & "                   </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">费用分类</div></td>" & vbcrlf & "                 <td colspan=""3"">"
	sql_sort = ""
	If ZBRuntime.MC(18500) = False Then
		sql_sort = sql_sort &" and isnull(b.id1,0) not in(5,7)"
	end if
	If ZBRuntime.MC(18900) = False Then
		sql_sort = sql_sort &" and isnull(b.id1,0)<>6"
	end if
	If ZBRuntime.MC(18700) = False Then
		sql_sort = sql_sort &" and isnull(b.id1,0)<>8"
	end if
	If ZBRuntime.MC(18610) = False Then
		sql_sort = sql_sort &" and isnull(b.id1,0)<>9"
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select a.* from paytype a inner join sortonehy b on b.ord = a.sort2 and b.gate2=41 and b.del=1 "&sql_sort&" where a.del=1 order by a.gate2 desc"
	rs1.open sql1,conn,1,1
	if not rs1.eof then
		do until rs1.eof
			Response.write "" & vbcrlf & "                     <input name=""f"" type=""checkbox"" id=""f""  value="""
	...
	set rs1=nothing
	Response.write "</td>" & vbcrlf & "               </tr>" & vbcrlf & "                        <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                         <td><div align=""right"">审批状态</div></td>" & vbcrlf & "                        <td colspan=""3"">" & vbcrlf & "                         <input name=""e"" type=""checkbox"" id=""e""  value=""3"">审批通过&nbsp;" & vbcrlf & "                     <input name=""e"" type=""checkbox"" id=""e""  value=""1"">审批中&nbsp;" & vbcrlf & "                  <input name=""e"" type=""checkbox"" id=""e""  value=""0"">待审批&nbsp;" & vbcrlf & "                      <input name=""e"" type=""checkbox"" id=""e""  value=""2"">审批未通过" & vbcrlf & "                            </td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">报销日期</div></td>" & vbcrlf & "                <td><INPUT name=""rett"" size=""9"" readonly=""true"" id=""duepaydate1divPos"" onmouseup=toggleDatePicker(""duepaydate1div"",""date.rett"") value="""
	Response.write m1
	Response.write """>" & vbcrlf & "                     <DIV id=""duepaydate1div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate1div""></DIV>" & vbcrlf & "                   -" & vbcrlf & "                   <INPUT name=""rett2"" size=""9"" readonly=""true"" id=""duepaydate2divPos"" onmouseup=toggleDatePicker(""duepaydate2div"",""date.rett2"") value="""
	Response.write m2
	Response.write """>" & vbcrlf & "                     <DIV id=""duepaydate2div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate2div""></DIV></td>" & vbcrlf & "                 <td><div align=""right"">币种</div></td>" & vbcrlf & "                 <td><select name=""listbz"">" & vbcrlf & "                            "
	Response.write m2
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	...
	sql9="select id,sort1 from sortbz order by gate1 desc"
	set rs9=server.CreateObject("adodb.recordset")
	...
	if bz<>0 then
		if not rs9.eof then
			do while not rs9.eof
				Response.write "" & vbcrlf & "                     <option value="""
				Response.write rs9("id")
				Response.write """ "
				if cdbl(rs9("id"))=listbz then Response.write "selected"
				Response.write ">"
				Response.write rs9("sort1")
				Response.write "</option>" & vbcrlf & "                     "
				rs9.movenext
			loop
		end if
	...
	set rs9=nothing
	Response.write "" & vbcrlf & "                 </select></td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">添加日期</div></td>" & vbcrlf & "    <td colspan=""3"">" & vbcrlf & "                                 <INPUT name=""indate_ret1"" size=""9"" readonly=""true"" id=""paybxindate1Pos"" onmouseup=toggleDatePicker(""paybxindate1"",""date.indate_ret1"") value="""
	Response.write m3
	Response.write """>" & vbcrlf & "                                        <DIV id=""paybxindate1"" style=""POSITION: absolute;z-index:10"" name=""paybxindate1""></DIV>-" & vbcrlf & "      <INPUT name=""indate_ret2"" size=""9"" readonly=""true"" id=""paybxindate2Pos"" onmouseup=toggleDatePicker(""paybxindate2"",""date.indate_ret2"") value="""
	Response.write m3
	Response.write m4
	Response.write """>" & vbcrlf & "        <DIV id=""paybxindate2"" style=""POSITION: absolute;z-index:10"" name =""paybxindate2""></DIV>                               </td>" & vbcrlf & "               </tr>" & vbcrlf & "                         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "      <td><div align=""right"">审批日期</div></td>" & vbcrlf & "                 <td colspan=""3"">" & vbcrlf & "                                  <INPUT name=""ret_sp1"" size=""9"" readonly=""true"" id=""ret_sp1divPos"" onmouseup=toggleDatePicker(""ret_sp1div"",""date.ret_sp1"")>" & vbcrlf & "                                  <DIV id=""ret_sp1div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp1div""></DIV>-" & vbcrlf & "   <INPUT name=""ret_sp2"" size=""9"" readonly=""true"" id=""ret_sp2divPos"" onmouseup=toggleDatePicker(""ret_sp2div"",""date.ret_sp2"")>" & vbcrlf & "  <DIV id=""ret_sp2div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp2div""></DIV></td>" & vbcrlf & "               </tr>" & vbcrlf & "                     <tr onMouseOut=this.style.backgroundColor="" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td><div align=""right"">出账日期</div></td>" & vbcrlf & "                 <td colspan=""3"">" & vbcrlf & "                                        <INPUT name=""ret_sp3"" size=""9"" readonly=""true"" id=""ret_sp3divPos"" onmouseup=toggleDatePicker(""ret_sp3div"",""date.ret_sp3"")>" & vbcrlf & "                                  <DIV id=""ret_sp3div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp3div""></DIV>-" & vbcrlf & "         <INPUT name=""ret_sp4"" size=""9"" readonly=""true"" id=""ret_sp4divPos"" onmouseup=toggleDatePicker(""ret_sp4div"",""date.ret_sp4"")>" & vbcrlf & "       <DIV id=""ret_sp4div"" style=""POSITION: absolute;z-index:10"" name =""ret_sp4div""></DIV></td>" & vbcrlf & "               </tr>" & vbcrlf & "                    <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                       <td><div align=""right"">报销人员</div></td>" & vbcrlf & "                        <td  colspan=""3"">"
...
'd_at(44) = "               If nodes.items(i).orgstype = 0 Then" & vbCrLf
	...
	if ZBRuntime.MC(1001) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联客户</div></td>"
			Response.write "<td><input name=""stel"" type=""text"" id=""stel""></td>"
		else
			Response.write "<td ><div align=""right"">关联客户</div></td>"
			Response.write "<td><input name=""stel"" type=""text"" id=""stel""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(2000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联联系人</div></td>"
			Response.write "<td><input name=""sperson"" type=""text"" id=""sperson""></td>"
		else
			Response.write "<td ><div align=""right"">关联联系人</div></td>"
			Response.write "<td><input name=""sperson"" type=""text"" id=""sperson""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(3000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联项目</div></td>"
			Response.write "<td><input name=""schance"" type=""text"" id=""schance""></td>"
		else
			Response.write "<td ><div align=""right"">关联项目</div></td>"
			Response.write "<td><input name=""schance"" type=""text"" id=""schance""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(7000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联合同</div></td>"
			Response.write "<td><input name=""scontract"" type=""text"" id=""scontract""></td>"
		else
			Response.write "<td ><div align=""right"">关联合同</div></td>"
			Response.write "<td><input name=""scontract"" type=""text"" id=""scontract""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(9000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联售后</div></td>"
			Response.write "<td><input name=""sshouhou"" type=""text"" id=""sshouhou""></td>"
		else
			Response.write "<td ><div align=""right"">关联售后</div></td>"
			Response.write "<td><input name=""sshouhou"" type=""text"" id=""sshouhou""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(31000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联日程</div></td>"
			Response.write "<td><input name=""sricheng"" type=""text"" id=""sricheng""></td>"
		else
			Response.write "<td ><div align=""right"">关联日程</div></td>"
			Response.write "<td><input name=""sricheng"" type=""text"" id=""sricheng""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(17008) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联发货</div></td>"
			Response.write "<td><input name=""sfahuo"" type=""text"" id=""sfahuo""></td>"
		else
			Response.write "<td ><div align=""right"">关联发货</div></td>"
			Response.write "<td><input name=""sfahuo"" type=""text"" id=""sfahuo""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(15000) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联采购</div></td>"
			Response.write "<td><input name=""scaigou"" type=""text"" id=""scaigou""></td>"
		else
			Response.write "<td ><div align=""right"">关联采购</div></td>"
			Response.write "<td><input name=""scaigou"" type=""text"" id=""scaigou""></td>"
			Response.write "</tr>"
	...
	if ZBRuntime.MC(1002) then
		if g mod 2=1 then
			Response.write "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">"
			Response.write "<td ><div align=""right"">关联供应商</div></td>"
			Response.write "<td><input name=""swork"" type=""text"" id=""swork""></td>"
		else
			Response.write "<td ><div align=""right"">关联供应商</div></td>"
			Response.write "<td><input name=""swork"" type=""text"" id=""swork""></td>"
			Response.write "</tr>"
	...
	if g mod 2<>1 then
		Response.write "<td ><div align=""right""></div></td>"
		Response.write "<td></td>"
		Response.write "</tr>"
	end if
	Response.write "                        " & vbcrlf & "                        " & vbcrlf & "              </table>" & vbcrlf & "       </div>" & vbcrlf & "                  <div region=""south"" border=""false"" style=""padding-top:5px;text-align:right;height:27px;line-height:27px;"">" & vbcrlf & "        <input type=""hidden"" name=""px"" value="""
	Response.write "</tr>"
	Response.write px
	Response.write """>" & vbcrlf & "        <input type=""hidden"" name=""page_count"" value="""
	Response.write page_count
	Response.write """>" & vbcrlf & "                                <a class=""easyui-linkbutton""  href=""###"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                         <a class=""easyui-linkbutton"" href=""###"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                   </div>" & vbcrlf & "  </div>" & vbcrlf & "</form>" & vbcrlf & "  </div>" & vbcrlf & ""
	action1="费用报销明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "" & vbcrlf & "<script src= ""../Script/py_paybxdet.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
