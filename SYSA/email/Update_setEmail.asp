...
'If Not rs.Eof Then
	...
	if open_77_19<>1 then
		EmailPasswd=trim(SaveRequestForm("EmailPasswd"))
		Emailsmtp=trim(SaveRequestForm("Emailsmtp"))
		EmailPop3=trim(SaveRequestForm("EmailPop3"))
		EmailUrl=trim(SaveRequestForm("EmailName"))
		EmailOrd=trim(SaveRequestForm("EmailOrd"))
		SignatureName=trim(SaveRequestForm("SignatureName"))
		SignatureCon=trim(SaveRequestForm("SignatureCon"))
		SignatureOrd=trim(SaveRequestForm("SignatureOrd"))
		delMail=trim(SaveRequestForm("delMail"))
		SmtpObj=trim(SaveRequestForm("SmtpObj"))
		ssl=trim(SaveRequestForm("ssl"))
		smtpPort=trim(SaveRequestForm("smtpPort"))
		receiveTotal=trim(SaveRequestForm("receiveTotal"))
		if EmailUrl="" then
			Response.write"<script language=javascript>alert('Email地址不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		EmailName=(split(EmailUrl,"@"))(0)
		if EmailName="" then
			Response.write"<script language=javascript>alert('Email用记名不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if EmailPasswd="" then
			Response.write"<script language=javascript>alert('Email密码不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if Emailsmtp="" then
			Response.write"<script language=javascript>alert('smtp服务器不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if EmailPop3="" then
			Response.write"<script language=javascript>alert('pop3服务器不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if smtpPort="" or isnumeric(smtpPort)=false then
			Response.write"<script language=javascript>alert('端口号只能为数字！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if SignatureName<>"" and SignatureCon="" then
			Response.write"<script language=javascript>alert('您设置了签名名称，但内容为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if SignatureName="" and SignatureCon<>"" then
			Response.write"<script language=javascript>alert('您设置了签名内容，但名称为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if SignatureOrd<>"" and  (SignatureName="" or SignatureCon="") then
			Response.write"<script language=javascript>alert('签名内容或名称不能为空！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if SmtpObj="" then
			Response.write"<script language=javascript>alert('您还没有选择邮件组件！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		if delMail="" then
			Response.write"<script language=javascript>alert('请选择是否删除邮件！'); window.history.back();</script>"
			call db_close : Response.end
		end if
		strConfig = "<?xml version=""1.0"" encoding=""utf-8"" ?>"&vbCrLf
		call db_close : Response.end
		strConfig =strConfig&"<configuration>"&vbCrLf
		strConfig = strConfig&" <appSettings>"&vbCrLf
		strConfig = strConfig&" <add key=""sendcount"" value=""3""/>"&vbCrLf
		strConfig =strConfig&"  </appSettings>"&vbCrLf
		strConfig =strConfig&"<connectionStrings>"&vbCrLf
		strConfig =strConfig&"  <add name=""connstring"" connectionString="""&zbintelEmailEncode("Server="&server_1&";DataBase="&sql_1&";User ID="&user_1&";Pwd="&pw_1&"",1,7)&"""/>"&vbCrLf
		strConfig =strConfig&" </connectionStrings>"&vbCrLf
		strConfig =strConfig&"</configuration>"&vbCrLf
		filename ="zbintelWindowsSendEmail.exe.config"
		checkFolder("zbintelSendMail")
		filePath="zbintelSendMail/zbintelWindowsSendEmail.exe.config"
		filtCon=strConfig
		set fso = server.createobject("scripting.filesystemobject")
		thePath=server.mappath(filePath)
		on error resume next
		set thefile = fso.createtextfile(thePath)
		If Err.number <> 0 Then
			conn.close
			errtext = "无法创建文件“" &  filePath & "”， 请确认IIS是否对目录“" & server.mappath("/email/zbintelSendMail") & "”有可写权限。"
			On Error GoTo 0
			Err.raise -1, "运行过程出错", errtext
			On Error GoTo 0
		end if
		On Error GoTo 0
		thefile.writeline filtCon
		thefile.close
		set fso = Nothing
		if SignatureOrd<>"" and SignatureName<>"" and SignatureCon<>""  then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from email_Signature where del=1 and addcate="&session("personzbintel2007")&" and SignatureName='"&SignatureName&"' and ord<>"&SignatureOrd&""
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "update email_Signature set SignatureName='"&SignatureName&"',SignatureCon='"&SignatureCon&"' where del=1 and addcate="&session("personzbintel2007")&" and ord="&SignatureOrd&""
			else
				Response.write"<script language=javascript>alert('该签名已存在！'); window.history.back();</script>"
				call db_close : Response.end
			end if
			rs.close
			set rs=nothing
		elseif SignatureOrd="" and SignatureName<>"" and SignatureCon<>"" then
			conn.execute "insert into email_Signature(SignatureName,SignatureCon,addTime,addcate,SignatureDefault,del) values('"&(SignatureName)&"','"&SignatureCon&"','"&now()&"',"&session("personzbintel2007")&",1,1)"
		end if
		Dim y_rs
		if Len(EmailOrd&"")=0 Then
			Set y_rs=server.CreateObject("adodb.recordset")
			y_rs.open "select top 0 * from email_sender",conn,3,2
			y_rs.addnew
			y_rs("EmailName")=EmailName&""
			y_rs("EmailPasswd")=EnCrypt(EmailPasswd)&""
			y_rs("Emailsmtp")=Emailsmtp&""
			y_rs("EmailPop3")=EmailPop3&""
			y_rs("EmailUrl")=EmailUrl&""
			y_rs("EmailDefault")=1
			y_rs("addTime")=now()
			y_rs("gate")=session("personzbintel2007")
			y_rs("del")=1
			y_rs("SmtpObj")=SmtpObj
			y_rs("delMail")=delMail
			y_rs("ssl")=ssl
			y_rs("port")=smtpPort
			y_rs("ReceiveTotal")=receiveTotal
			y_rs.update
			y_rs.close
			Set y_rs=nothing
'conn.execute "insert into email_sender(EmailName,EmailPasswd,Emailsmtp,EmailPop3,EmailUrl,EmailDefault,addTime,gate,del,SmtpObj,delMail,ssl,port,ReceiveTotal) values('"&(EmailName)&"','"&EnCrypt(EmailPasswd)&"','"&(Emailsmtp)&"','"&(EmailPop3)&"','"&(EmailUrl)&"',1,'"&now()&"',"&session("personzbintel2007")&",1,'"&SmtpObj&"',"&delMail&","&ssl&","&smtpPort&","&receiveTotal&")"
			Response.write"<script language=javascript>alert('添加成功！');window.location.href='setEmail.asp';</script>"
			call db_close : Response.end
		else
			set rs=server.CreateObject("adodb.recordset")
			sql="select top 1 1 from email_sender where del=1 and gate="&session("personzbintel2007")&" and ord<>"&EmailOrd&" and EmailUrl='"&(EmailUrl)&"'"
			rs.open sql,conn,1,1
			if rs.eof=true Then
				rs.close
				Set rs=nothing
				Set y_rs=server.CreateObject("adodb.recordset")
				y_rs.open "select * from email_sender where del=1 and gate="&session("personzbintel2007")&" and ord="&EmailOrd&"",conn,3,2
				If y_rs.eof=False then
					y_rs("EmailName")=EmailName&""
					y_rs("EmailPasswd")=EnCrypt(EmailPasswd)&""
					y_rs("Emailsmtp")=Emailsmtp&""
					y_rs("EmailPop3")=EmailPop3&""
				...
				y_rs.close
				Set y_rs=nothing
'conn.execute "update email_sender set EmailName='"&(EmailName)&"',delMail="&delMail&",EmailPasswd='"&EnCrypt(EmailPasswd)&"',Emailsmtp='"&(Emailsmtp)&"',EmailPop3='"&(EmailPop3)&"',EmailUrl='"&(EmailUrl)&"',EmailDefault=1,SmtpObj='"&SmtpObj&"',ssl="&ssl&",port="&smtpPort&",ReceiveTotal="&receiveTotal&"  where del=1 and gate="&session("personzbintel2007")&" and ord="&EmailOrd&""
				Response.write"<script language=javascript>alert('修改成功！');window.location.href='setEmail.asp';</script>"
				call db_close : Response.end
			else
				Response.write"<script language=javascript>alert('该邮箱账号已存在！'); window.history.back();</script>"
				call db_close : Response.end
			end if
			rs.close
			set rs=nothing
