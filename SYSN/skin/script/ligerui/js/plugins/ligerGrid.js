...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        width: 'auto',                          //宽度值
        height: 'auto',                          //宽度值
        rowNumberContinual: true,                //行序号是否连续
    ...
    $.ligerDefaults.GridString = {
        errorMessage: LanguageGlobal.LigerGrid.ErrorMessage,
        pageStatMessage: LanguageGlobal.LigerGrid.PageStatMessage,
        pageTextMessage: LanguageGlobal.LigerGrid.PageTextMessage,
        loadingMessage: LanguageGlobal.LigerGrid.LoadingMessage,
        findTextMessage: LanguageGlobal.LigerGrid.FindTextMessage,
        noRecordMessage: LanguageGlobal.LigerGrid.NoRecordMessage,
        isContinueByDataChanged: LanguageGlobal.LigerGrid.IsContinueByDataChanged,
        cancelMessage: LanguageGlobal.LigerGrid.CancelMessage,
        saveMessage: LanguageGlobal.LigerGrid.SaveMessage,
        applyMessage: LanguageGlobal.LigerGrid.ApplyMessage,
        draggingMessage: LanguageGlobal.LigerGrid.DraggingMessage
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        //获取父节点
        //如果columnLevel不为空，获取叶节点集合
        getColumns2: function (columnLevel) {
            var g = this, p = this.options;
            var columns = [];
        ...
        toggleCol: function (columnparm, visible, toggleByPopup) {
            ...
            //GuoCheng.Chen Added
            if (typeof (RefactorHeader) != 'undefined') {
                RefactorHeader(g.id);
            }
        },
        //设置列宽
        ...
        _initColumns: function () {
            ...
            //父级别列的信息
            $(g.getColumns2()).each(function (i, column) {
                column.columnname = column.name;
                column.columnindex = i;
                column.type = column.type || "string";
                column.islast = i == g.columns.length - 1;
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    if (this.isrownumber) {
                        ...
                        if (p.rowNumberContinual) {
                            gridhtmlarr.push('>' + ((p.page - 1) * p.pageSize + parseInt(item['__index']) + 1) + '</div></td>');
                        }
                        else {
        ...
        _getCellHtml: function (rowdata, column) {
           ...
           // htmlarr.push('min-height:' + p.rowHeight + 'px; ');加了行高导致最右侧的按钮过于靠上，遂注释之
        ...
        _getTotalInfo: function (column, data) {
            ...
            try {
                ...
                if (data && data.length) {
                    if ($(data[0][column.name]).find("input").length > 0) {
                        max = parseFloat($(data[0][column.name]).val());
                        min = parseFloat($(data[0][column.name]).val());
                    }
                    else {
        ...
        _onClick: function (e) {
            ...
            else if (src.hcelltext) //排序
            {
                ...
                g.trigger('SysGridHeightChanged');
                $(".l-grid-body-table").css({width:"100%"})
            }
                //树 - 伸展/收缩节点
        ...
        showFilter: function () {
            ...
            return g.winfilter = $.ligerDialog.open({
                ...
                target: filtercontainer, isResize: true, top: getScrollTop(),
                buttons: [
            ...
            function getFields() {
                ...
                $(g.columns).each(function () {
                    if (this.name != 'PhotoAddress' && this.name != 'Operate') {
            ...
            function loadClientData(data) {
                ...
                var clause = $.ligerFilter.getFilterFunction(data);
                g.filteredData = $.extend(true, {}, p.data || g.currentData);
                var data = g._searchData(g.filteredData[p.root], clause);
                if (data != null)
                {
                    for (var i = 0; i < data.length; i++)
                    {
