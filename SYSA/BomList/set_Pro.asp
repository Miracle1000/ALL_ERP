...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		rpt.title = "组装清单虚拟名称自定义"
		rpt.canExcel = False
		rpt.canPrint = False
		rpt.BillListUIModel = True
		rpt.oldReportUI = true
		rpt.nosearchBar = True
		rpt.buttonsHTML = "<button class='anybutton' onclick=""window.open('set_pro_edit.asp','set_pro_edit','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">添加虚拟名称</button>"
		rpt.nosearchBar = True
		rpt.sql = "select name,gate1,ord from bom_proname order by gate1 desc,ord desc"
	end sub
	Sub app_oncreate(byval lvw)
		lvw.headers("name").title = "虚拟名称"
		lvw.headers("gate1").title = "重要指数"
		lvw.headers("ord").title = "操作"
		lvw.headers("ord").formattext = "code: HandleCell(@value)"
	end sub
	Sub App_dodel
		Dim ord : ord = request.form("id")
		cn.execute("delete from bom_proName where ord = " & ord)
		Response.write("true")
	end sub
	Function HandleCell(ByVal ord)
		Dim buttonEdit, buttonDel, rs, disPlay
		disPlay = ""
		Set rs = cn.execute("select count(ord) from BOM_Structure_List where ProType = 0 and ProOrd = " & ord)
		If rs(0) > 0 Then
			disPlay = " disabled='disabled' title='虚拟名称已使用，不可删除！' "
		end if
		rs.close
		set rs = nothing
		buttonEdit = "<button class='oldbutton2' onclick=""window.open('set_pro_edit.asp?ord=" & app.base64.pwurl(ord) & "','set_pro_edit','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"">修改</button>"
		disPlay = " disabled='disabled' title='虚拟名称已使用，不可删除！' "
		buttonDel = "<button class='oldbutton2' " & disPlay & " onclick=""ListAction.DoDel('" & ord & "')"">删除</button>"
