...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(byval rpt, byval  state)
		dim table, tabName, keyField, sql, formid, tplid, tiaojian, [sort], titleField,account
		formid = app.base64.deurl(request.QueryString("formid"))
		If request.QueryString("tplid")&""<>"" Then
			tplid = app.base64.deurl(request.QueryString("tplid"))
		end if
		[sort] = request.QueryString("sort")
		account = request.QueryString("account")
		accountyear = request.QueryString("accountyear")
		rpt.title = "打印记录"
		tabName = "" : keyField = "ord" : titleField = "b.title"
		Select Case [sort]
		Case 1 : tabname = "contract"
		Case 11001 : tabname = "contract"
		Case 2 : tabname = "price"
		Case 3 : tabname = "caigou"
		Case 4 : tabName = "send"
		Case 18001 : tabName = "tousu"
		Case 6 : tabname = "kuin"
		Case 44009 : tabname = "payout"
		Case 10,62001 : tabName = "kuout"
		case 43002:tabName = "BankIn3"
		keyField = "id"
		titleField = "b.intro"
		Case 44002:tabName= "bankout3"
		keyField="id"
		titleField="b.intro"
		Case 19:
		tabName = "f_pay"
		...
		keyField = "id"
		Case 127
		tabName = "caigou_yg" : keyField = "id"
		Case 2001
		tabName = "hr_person" : keyField = "id" : titleField = "b.username"
		Case 2002
		tabName = "gate" : titleField = "b.name"
		Case 2003
		tabName = "product" : titleField = "b.title"
		Case 16
		tabName = "paybackinvoice" : keyField = "id"
		titleField = " (case b.fromType when 'CONTRACT' then (select title from contract where ord = b.fromid) " &_
		"when 'PREBACK' then '预收款' " &_
		"when 'BEGINVOICE' then '期初开票' else '' end) "
		Case 54007
		tabName = request.QueryString("tabName")
		keyField = request.QueryString("keyField")
		titleField = "( SELECT p.WPName FROM dbo.M2_WFP_Assigns a INNER JOIN dbo.M2_WorkingProcedures p ON a.WPID = p.ID WHERE a.ID = b.ID)"
		Case 76
		tabName = "f_AccountSubject"
		keyField = "ord"
		titleField = "'总账打印'"
		Case 72001
		tabName = "caigou_yg" : keyField = "id"
		...
		tabName = "MCostLog"
		keyField = "ID"
		titleField = " (case when b.RootChgType=1 or b.RootChgType=2 then (select title from MCostChgForKuin where ID=b.JoinBillId) "&_
		"       when b.RootChgType=20 then (select title from MCostChgForException where ID=b.JoinBillId ) end ) "
		Case Else
		tabName = request.QueryString("tabName")
		keyField = request.QueryString("keyField")
		titleField = "b."&request.QueryString("titleField")
		End Select
		If Trim(titlefield & "") = "b." Then titleField = "b.title"
		table = " PrinterInfo a left join ["& tabName &"] b on a.formID = b.["& keyField &"] left join gate c on a.addCate = c.ord left join PrintTemplates d on a.templateID = d.id left join contract_out e on a.templateID = e.id "
		tiaojian =  ""
		if len([sort]) <> 0 then
			Select Case [sort]
			Case 10,62001 :
			tiaojian = tiaojian & " and a.sort in (10,62001) "
			Case Else
			tiaojian = tiaojian & " and a.sort = " & [sort] & " "
			End Select
		end if
		if len(formid) <> 0 then
			tiaojian = tiaojian & " and a.formid = " & formid & " "
		end if
		if len(tplid) <> 0 then
			tiaojian = tiaojian & " and a.templateid = " & tplid & " "
		end if
		if len(account) <> 0 then
			tiaojian = tiaojian & " and a.AccountSys = " & account & " "
			if accountyear<>"0" and accountyear<>"" then tiaojian = tiaojian & " and a.AccountYear = " & accountyear & " "
		end if
		sql = "select " & titleField & " as SendTitle, (case isnull(a.isOld,0) when 1 then e.title when 2 then e.title else d.title end) as TemplateTitle,(case a.device when 2 then '移动' else 'PC' end) device, c.name, a.adddate , a.id  as 操作,a.sort,isnull(a.isSum,0) as isSum,a.templateid ,a.formid, isnull(a.isOld,0) as isOld from " + table + " where 1 = 1 " & tiaojian & " order by a.ord desc"
		rpt.sql = sql
	...
	sub app_oncreate(byval lvw)
		...
		lvw.currsum = false
		lvw.indexbox = true
		lvw.headers(1).title = "单据主题"
		lvw.headers(1).dbtype = "str"
		'lvw.headers(2).title = "所用模板"
		lvw.headers(3).title = "类型"
		lvw.headers(4).title = "打印人"
		lvw.headers(5).title = "打印时间"
		lvw.headers(6).formattext = "code: CHandleCell(@value,rs(""sort""),rs(""isSum""),rs(""templateid""),rs(""formid""),rs(""isold""))"
	...
	function CHandleCell(byval id , ByVal sort , ByVal isSum ,ByVal templateid ,ByVal formid, ByVal isold)
		If isold&"" = "2" then
			CHandleCell = ""
		elseIf isold&"" = "1" And sort <> 5 then
			CHandleCell = "<button class='oldbutton2' onclick=""window.open('" & app.virpath & "contract/moban_dy.asp?id="& templateid &"&sort="& sort &"&ord="&app.base64.pwurl(formid)  &"&out=print&isSum="& isSum &"','PrinterRecordInfo1','width=' + (screen.availWidth-20) + ',height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=0,top=0');"">详情</button>"
'elseIf isold&"" = "1" And sort <> 5 then
		elseIf isold&"" = "1" And sort = 5 then
			CHandleCell = "<button class='oldbutton2' onclick=""window.open('" & app.virpath & "contract/content_out3.asp?id="& templateid &"&sort="& sort &"&ord="&app.base64.pwurl(formid)  &"&out=print&isSum="& isSum &"','PrinterRecordInfo1','width=' + (screen.availWidth-20) + ',height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=0,top=0');"">详情</button>"
'elseIf isold&"" = "1" And sort = 5 then
		else
			CHandleCell = "<button class='oldbutton2' onclick=""window.open('PrinterRecordInfo.asp?id=" & app.base64.pwurl(id) & "','PrinterRecordInfo1','width=' + (screen.availWidth-20) + ',height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=0,top=0');"">详情</button>"
		end if
	end function
