...
'if m2="" then
	...
	m=int((datediff("m",getQuarterFrom(m1),getQuarterEnd(m2))+1)/3)
'if m2="" then
	if (datediff("m",getQuarterFrom(m1),getQuarterEnd(m2))+1) mod 3<>0 then
'if m2="" then
		m=m+1
'if m2="" then
	end if
...
'if B>0 then
	...
	if m>4 then
		Response.write "<script>alert('显示数据最多不能超过4个季度');window.location.href='cash_j.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
...
'Case 10
	...
	if intCols1=0 or intCols2=0 then
		Response.write "<script>alert('收入项与支出项至少各保留一项');window.location.href='../tongji/cash_j.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
...
'do while not rs.eof
				...
				For k=0 to m-1
					tempSort1 = ""
					date1=dateadd("m",k*3,m1)
					if k>0 then
						date1=getQuarterFrom(date1)
					end if
					date3=year(date1)&"-"&month(date1)&"-1"
					date1=getQuarterFrom(date1)
					date2=getQuarterEnd(date1)
					if cDate(date2)>cDate(m2) then date2=m2
					str2=year(date1)&"年第"&getQuarter(date1)&"季度"
					hl=1
					sort1 =  rs("sort1")
...
'For Each key In colsTotal
			...
			doSave("现金流量每季度统计")
				...
				Response.write """><font class=""red""><strong><u>下载导出的现金流量每季度统计</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
