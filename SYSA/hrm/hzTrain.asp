...
'if request("hiddenflag")="1" then
	...
	for each reqPara in Request.QueryString
		if instr(1,reqPara,"hrP_")>0 then
			execute(reqPara&"="""&SaveRequestUrl(reqPara))&""""
		end if
	next
	if hrP_ret<>"" and hrP_ret2<>"" and isdate(hrP_ret) and isdate(hrP_ret2) then
		baseSQL="select PAGE_COUNT_NUM from (select PAGE_TOP_NUM * from (select * from dbo.hrTrainPlanHz('"&hrP_ret&"','"&hrP_ret2&"',"&session("personzbintel2007")&")) aaaavv PAGE_ORDER_STR) bbb  where 1=1"
	else
		baseSQL="select PAGE_COUNT_NUM from (select PAGE_TOP_NUM * from (select * from dbo.hrTrainPlanHz('"&startDate&"','"&endDate&"',"&session("personzbintel2007")&")) aaaavv PAGE_ORDER_STR) bbb  where 1=1"
	end if
	dim gather
	...
	function CustomShowFields(idx,rsobj)
		...
		case "培训完成率"
		Response.write "<span class='proport' title='"&FormatNumber(trim(rsobj("培训完成率")),percentage,-1,0,0)&"%'><span style='width:"&trim(rsobj("培训完成率"))&"%;' class='ratio'><span class='rationText'>"&FormatNumber(trim(rsobj("培训完成率")),percentage,-1,0,0)&"%</span></span></span>"
		'case "培训完成率"
		case else
	...
	sub ShowSearchDiv()
		Response.write "" & vbcrlf & "<form name=""form2"" method=""get"" id=""searchform"">" & vbcrlf & "<div id=""dd"" class=""easyui-window"" icon=""icon-search"" title=""高级检索""  style=""width:550px;height:270px;padding:5px;background: #fafafa;top:0px;""  closed=""true"" >" & vbcrlf & "      <div region=""center"" border=""false"" style=""padding:2%;background:#fff;border:1px solid #ccc;width:96%;height:150px"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <style>.detailTable {table-layout:auto!important;}</style>" & vbcrlf & "         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "          <td><div align=""right"">时间段：</div></td>" & vbcrlf & "          <td>&nbsp;自：" & vbcrlf & "            <INPUT name=""hrP_ret"" size=9  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" value="""">&nbsp;至:<INPUT name=""hrP_ret2"" size=9  Class=""DatePick""  onclick=""datedlg.show();"" readonly=""readonly"" value=""""></td>" & vbcrlf & "        </tr>" & vbcrlf & "            </table>" & vbcrlf & "        </div>" & vbcrlf & "  <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "         <input type=""hidden"" name=""jtdate"" value="""
'sub ShowSearchDiv()
