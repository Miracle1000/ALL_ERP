...
'Private Sub Class_Initialize
	...
	If msgid &"" <> "" Then
		...
		response.clear
		Select Case msgid
		Case "getCompany" : Call getCompany()
		End Select
		call db_close : Response.end
	end if
	...
	Sub savealt()
		company=getInt(request("company"))
		sort1=getInt(request("sort1"))
		title=request("title")
		ret3=request("ret3")
		introts=request("introts")
		gate_name=request("gate_name")
		gate_ord=getInt(request("gate_ord"))
		iord=getInt(request.form("iord"))
		If introts&""<>"" Then
			If InStr(introts,"'")>0 Then introts = Replace(introts,"'","''")
		end if
		If sort1=0 Then
			Response.write "<script>alert('请选择要事分类！');history.back(-1);</script>"
'If sort1=0 Then
			Call db_close() : Response.end
		end if
		If gate_ord=0 Then
			Response.write "<script>alert('请选择执行人！');history.back(-1);</script>"
'If gate_ord=0 Then
			Call db_close() : Response.end
		end if
		If company>0 And Len(title&"")>0 Then
			Dim helper
			If iord>0 Then
				Set rs =  conn.Execute("SELECT metype FROM importantMsg WHERE id = "& iord &"")
				If Not rs.Eof Then
					Set helper = CreateReminderHelper(conn,4,rs(0))
					helper.dropRemindByOID(iord)
				end if
				rs.close
				set rs = nothing
				conn.execute("update importantMsg set title='" & title & "',content='" & introts & "',stime='" & ret3 & "',metype='" & sort1 & "',ecateid='" & gate_ord & "' where id="&iord)
				Set helper = CreateReminderHelper(conn,4,sort1)
				helper.appendRemind(iord)
			else
				conn.execute("insert into importantMsg(title,content,stime,metype,cateid,ecateid,t_ord,state,date7,del) values('" & title & "','" & introts & "','" & ret3 & "','" & sort1 & "','" & session("personzbintel2007") & "','" & gate_ord & "','" & company & "',1,getdate(),1)")
				Set rs = conn.Execute("SELECT ISNULL(SCOPE_IDENTITY(),0)")
				If Not rs.Eof Then
					iord = rs(0)
				else
					iord = 0
			...
			Response.write "<script>alert('请输入提醒主题！');history.back(-1);</script>"
			helper.appendRemind(iord)
			Call db_close() : Response.end
		end if
		Response.write "" & vbcrlf & "   <script src= ""../Script/wk_alt_hy.js?ver="
		Response.write Application("sys.info.jsver")
...
'Sub showpage()
		...
		Response.write Application("sys.info.jsver")
		Response.write """ charset=""utf-8""></script>" & vbcrlf & "<script src= ""../Script/wk_alt_hy_1.js?ver="
		'Response.write Application("sys.info.jsver")
		Response.write Application("sys.info.jsver")
		Response.write """  type=""text/javascript""></script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "   margin-top: 0px;" & vbcrlf & "        background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrslf & "}" & vbcrlf & ".STYLE8 {color: #E4433F}" & vbcrlf & "" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<body>" & vbcrlf & "" & vbcrlf & ""
		'Response.write Application("sys.info.jsver")
		Dim metype,ord,t_name
		metype=request("metype")
		If request("ord")&"" = "" Then
			ord = 0
		else
			ord=deurl(request("ord"))
		end if
		If Len(metype&"")=0 Or Not isnumeric(metype) Then metype=0
		If Len(ord&"")=0 Then ord=0
		Dim rs
		Set rs=conn.execute("select top 1 name from tel where ord="&ord)
		If rs.eof=False Then
			t_name=rs(0).value
		end if
		rs.close : Set rs=Nothing
		Dim iord
		If request("iord")&"" = "" Then
			iord = 0
		else
			iord=request("iord")
		end if
		If isnumeric(iord)=False Or Len(iord)=0 Then iord=0
		If iord>0 Then
			Set rs=conn.execute("select top 1 a.*,(select name from gate where ord=a.ecateid) as ename from importantMsg a where a.id="&iord)
			If rs.eof=False Then
				title=rs("title")
				intro=rs("content")
				stime=rs("stime")
		...
		Response.write "" & vbcrlf & "" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "      <tr>" & vbcrlf & "            <td width=""100%"" valign=""top"">" & vbcrlf & "                      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                               <tr>" & vbcrlf & "                                    <td class=""place"">添加要事提醒</td>" & vbcrlf & "                                       <td>&nbsp;</td>" & vbcrlf & "                                 <td align=""right"">&nbsp;</td>" & vbcrlf & "                                     <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                              </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                 <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD""" & vbcrlf & "                  id=""content"">" & vbcrlf & "                             <form method=""POST"" action=""?act=save"" id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "                                        <input type=""hidden"" name=""metype"" id=""metype1"" value="""
		Response.write metype
		Response.write """>" & vbcrlf & "                                        <input type=""hidden"" name=""iord"" id=""iord1"" value="""
		Response.write iord
		Response.write """>" & vbcrlf & "                                        <tr>" & vbcrlf & "                                            <td width=""12%"" height=""27"">" & vbcrlf & "                                                        <div align=""right"">要事分类：</div>" & vbcrlf & "                                               </td>" & vbcrlf & "                                           <td width=""35%"" height=""27"">" & vbcrlf & "                                                        <select name=""sort1"" dataType=""Limit"" min=""1"" max=""50"" msg=""必填"">" & vbcrlf & "                                                             <option value="""">" & vbcrlf & "                                                         </option>" & vbcrlf & "                                                               "
		Set rs=conn.execute( "select ord,sort1 from sort10 where del=1")
		Do While Not rs.eof
			Response.write "<option value='" & rs( "ord") & "'"
			If metype& ""=rs( "ord")& "" Then Response.write " selected='selected' "
			Response.write " > " & rs( "sort1") & "</option>"
			rs.movenext
		Loop
		rs.close
		Response.write "" & vbcrlf & "                                                     </select>" & vbcrlf & "                                                       <span class=""red"">*</span>" & vbcrlf & "                                                </td>" & vbcrlf & "                                           <td width=""12%"" height=""27"">" & vbcrlf & "                                                        <div align=""right"" >"
		Response.write arrName(1)
		Response.write "：</div>" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td width=""41%"" height=""27"" class=""gray"">" & vbcrlf & "                                             "
		If ord = 0 Then
			Response.write "" & vbcrlf & "                                             <INPUT style=""TEXT-ALIGN: center"" id=""telName"" onclick=""window.open('../search/result4_onlykh.asp?ly=alt','newwinskh','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');"" datatype=""Limit""  min=""1"" msg=""请选择客户"" > <span class=""red"">*</span> "& vbcrlf &" "
		else
			Response.write t_name
		end if
		Response.write "" & vbcrlf & "                                                             <input name=""company"" id=""company"" type=""hidden"" value="""
		Response.write ord
		Response.write """ size=""25"">" & vbcrlf & "                                                </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td height=""27"">" & vbcrlf & "                                                  <div align=""right"">要事主题：</div>" & vbcrlf & "                                               </td>" & vbcrlf & "                                           <td height=""27"">" & vbcrlf & "                                                  <input name=""title"" type=""text"" value="""
		Response.write title
		Response.write """ size=""20"" datatype=""Limit"" min=""1"" max=""50"" msg=""长度必须在1个至50个字之间"">" & vbcrlf & "                                                      <span class=""red"">*</span>" & vbcrlf & "                                                </td>" & vbcrlf & "                                           <td height=""27"">" & vbcrlf & "                                                  <div align=""right"">预计时间：</div>" & vbcrlf & "                                               </td>" & vbcrlf & "                                           <td height=""27"">" & vbcrlf & "                                                <input name=ret3 onclick='datedlg.showDateTime()' minDate="""
		Response.write Format_Time(dateStr,2)
		Response.write """ value="""
		Response.write now()
		Response.write """ id=""ret31"" size=20 readonly dataType=""Limit"" min=""1"" max=""50"" msg=""请选择预计时间"">" & vbcrlf & "                                                       <span class=""red"">*</span>" & vbcrlf & "                                                </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td height=""27"">" & vbcrlf & "                                                  <div align=""right"">要事备注：</div>" & vbcrlf & "                                            </td>" & vbcrlf & "                                           <td height=""27"" colspan=""3"">" & vbcrlf & "                                                        <textarea name=""introts"" cols=""60"" rows=""8"" dataType=""Limit"" min=""0"" max=""500""  msg=""必须在500个字之内"">"
		Response.write intro
		Response.write "</textarea>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td height=""27"">" & vbcrlf & "                                                  <div align=""right"">执行人：</div>" & vbcrlf & "                                         </td>" & vbcrlf & "                                           <td height=""27"" colspan=""3"">" & vbcrlf & " <INPUT style=""TEXT-ALIGN: center"" id=gate_name_1 onclick=""showGatePersonDiv(this.name,this.id,'','../inc/Search_GateCheck.asp','','');""  value="""
		If iord>0 Then
			Response.write ename
		else
			Response.write session("name2006chen")
		end if
		Response.write """ readOnly size=10 name=""gate_name"" msg="""" max=""100"" dataType=""Limit"" min=""1"">" & vbcrlf & "                                                      <input type=""hidden"" name=""gate_ord"" id=""gate_ord_1"" value="""
		If iord>0 Then
			Response.write ecateid
		else
			Response.write session("personzbintel2007")
		end if
		Response.write """ />" & vbcrlf & "                                                      <span class=""red"">*</span>" & vbcrlf & "                                                </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td class=""page"">" & vbcrlf & "                 <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "                         <tr>" & vbcrlf & "                                    <td height=""30"">" & vbcrlf & "                                          <div align=""center"">" & vbcrlf & "                                                      <input type=""submit"" name=""Submit42"" value=""保存"" class=""anybutton"" />" & vbcrlf & "                                                  <span id=""an"">" & vbcrlf & "                                                            　" & vbcrlf & "                                                      </span>" & vbcrlf & "                         　" & vbcrlf & "                                                      <input type=""reset"" value=""重填"" class=""anybutton"" name=""B2"">" & vbcrlf & "                                           </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td height=""80"">" & vbcrlf & "                                          <div align=""center"">" & vbcrlf & "                                              </div>" & vbcrlf & "                                     </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </form>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	end sub
	Function arrName(ByVal no)
		...
		wend
		rs.close
		Set rs=conn.execute("select id,title,name,sort,gl from zdy where (name='zdy5' or name='zdy6') and sort1=1 and set_open=1 order by gate1 asc")
		While Not rs.eof
	...
	Sub getCompany()
		...
		tempStr = ""
		ord = request("ord")
		Set rs = conn.execute("select name from tel where ord="& ord)
		If rs.eof = False Then
			tempStr = rs("name")
		end if
		rs.close
