...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "<style>        " & vbcrlf & "        html {" & vbcrlf & "          background: #EFEFEF;" & vbcrlf & "            padding: 0 10px" & vbcrlf & " }" & vbcrlf & "</style>" & vbcrlf & ""
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		bill.title = "树形结构（报价或合同配置BOM时）设置"
		bill.ahonegp = False
		Dim f, v, rs, sql, i, intro, BHChecked, XHChecked, zdyChecked
		Dim SLChecked, DJChecked, ZJChecked, BZDJChecked, KCXXChecked, JHRQChecked, BZChecked, ZGYSChecked
		Set rs = cn.execute("select intro from setjm3 where ord = 8040")
		If rs.bof = True And rs.eof = True Then
			cn.execute("insert into setjm3 (ord,cateid,intro) values (8040,63,'name')")
			intro = "name"
		else
			intro = rs(0)
		end if
		...
		set rs = nothing
		intro = "," & Replace(intro," ","") &","
		If InStr(intro,",BH,") > 0 Then
			BHChecked = "checked='chedked'"
		end if
		intro = "," & Replace(intro," ","") &","
		If InStr(intro,",XH,") > 0 Then
			XHChecked = "checked='chedked'"
		end if
		If InStr(intro,",SL,") > 0 Then
			SLChecked = "checked='chedked'"
		end if
		If InStr(intro,",DJ,") > 0 Then
			DJChecked = "checked='chedked'"
		end if
		If InStr(intro,",ZJ,") > 0 Then
			ZJChecked = "checked='chedked'"
		end if
		If InStr(intro,",BZDJ,") > 0 Then
			BZDJChecked = "checked='chedked'"
		end if
		If InStr(intro,",KCXX,") > 0 Then
			KCXXChecked = "checked='chedked'"
		end if
		If InStr(intro,",JHRQ,") > 0 Then
			JHRQChecked = "checked='chedked'"
		end if
		If InStr(intro,",BZ,") > 0 Then
			BZChecked = "checked='chedked'"
		end if
		If InStr(intro,",ZGYS,") > 0 Then
			ZGYSChecked = "checked='chedked'"
		end if
		v = "<input id='s_name' name='setjm3_8040' type='checkbox' value='name' disabled='disabled' checked='chedked' />" &_
		"<input id='s_name_h' name='setjm3_8040' type='hidden' value='name' />" &_
		"<label for='s_name'>产品名称</label>" &_
		"<span style='padding-right:40px'></span>" &_
		"<input id='s_BH' name='setjm3_8040' type='checkbox' value='BH' " & BHChecked & " />" &_
		"<label for='s_BH'>产品编号</label>" &_
		"<span style='padding-right:40px'></span>" &_
		"<input id='s_XH' name='setjm3_8040' type='checkbox' value='XH' " & XHChecked & " />" &_
		"<label for='s_XH'>产品型号</label>"
		bill.Groups(0).title = "显示内容"
		Set f = Bill.fields.add("", "title")
		f.value = v
		f.colspan = 3
		f.uitype = "html"
		v = "<input id='s_SL' name='setjm3_8040' type='checkbox' value='SL' " & SLChecked & " />" &_
		"<label for='s_SL'>数量</label>"&_
		"<span style='padding-right:63px'></span>" &_
		"<input id='s_DJ' name='setjm3_8040' type='checkbox' value='DJ' " & DJChecked & " />" &_
		"<label for='s_DJ'>单价</label>"&_
		"<span style='padding-right:64px'></span>" &_
		"<input id='s_ZJ' name='setjm3_8040' type='checkbox' value='ZJ' " & ZJChecked & " />" &_
		"<label for='s_ZJ'>总价</label>"&_
		"<span style='padding-right:48px'></span>" &_
		"<input id='s_BZDJ' name='setjm3_8040' type='checkbox' value='BZDJ' " & BZDJChecked & " />" &_
		"<label for='s_BZDJ'>标准单价</label>"&_
		"<span style='padding-right:33px'></span>" &_
		"<input id='s_KCXX' name='setjm3_8040' type='checkbox' value='KCXX' " & KCXXChecked & " />" &_
		"<label for='s_KCXX'>库存信息</label>"&_
		"<span style='padding-right:34px'></span>" &_
		"<input id='s_JHRQ' name='setjm3_8040' type='checkbox' value='JHRQ' " & JHRQChecked & " />" &_
		"<label for='s_JHRQ'>交货日期</label>"&_
		"<span style='padding-right:40px'></span>" &_
		"<input id='s_BZ' name='setjm3_8040' type='checkbox' value='BZ' " & BZChecked & " />" &_
		"<label for='s_BZ'>备注</label>" &_
		"<span style='padding-right:40px'></span>" &_
		"<input id='s_BZ' name='setjm3_8040' type='checkbox' value='ZGYS' " & ZGYSChecked & " />" &_
		"<label for='s_BZ'>主供应商</label>"
		Set f = Bill.fields.add("", "title")
		f.value = v
		f.colspan = 3
		f.uitype = "html"
		v = ""
		i = 0
		Set rs = cn.execute("select title,name from zdy where sort1 = 21 and set_open = 1 order by gate1")
		While rs.eof = False
			i = i + 1
'While rs.eof = False
			If i > 1 Then
				v = v & "<span style='padding-right:40px'></span>"
'If i > 1 Then
			end if
			If InStr(intro,"," & rs("name") & ",") > 0 Then
				zdyChecked = "checked='chedked'"
			else
				zdyChecked = ""
			end if
			v = v & "<input id='s_" & rs("name") & "' name='setjm3_8040' type='checkbox' value='" & rs("name") & "' " & zdyChecked & " />" &_
			"<label for='s_" & rs("name") & "'>" & rs("title") & "</label>"
			rs.movenext
		wend
		rs.close
		set rs = nothing
	...
	Function Bill_OnSave(ByVal bill)
		Dim v
		v = request.form("setjm3_8040")
		v = Replace(v," ","")
		If Len(v & "") = 0 Then
			v = "name"
		end if
		cn.execute("update setjm3 set intro = '" & v & "' where ord = 8040")
		bill.alert("组装清单显示内容设置成功！")
		Bill_OnSave = true
	end function
