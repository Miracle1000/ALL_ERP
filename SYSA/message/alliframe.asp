		...
		if opentype="1" then
			...
			while not rs.eof
				...
				if ChildCount=0 then
					...
					sql7="select * from MessagePerson where sort="&rs("id")& " " & Str_Result
					rs7.open sql7,conn,1,1
					...
					while not rs7.eof and k<RecPageSize
						...
						if isMobile(mobile) then
							mobile1="<a href='###' title='"&trim(rs7("clictName"))&":"&trim(rs7("clictPhone"))&"' onClick=""changeMobile('"&rs7("ord")&"','"&mobile&"',true);try{window.frames['cFF2'].document.getElementById('date').submit();}catch(e1){}"">"
...
'if C<>"" and C<>"检索关键字" then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql7="select * from MessagePerson where 1=1 " &strsort&str_Result
			...
			while not rs.eof and k<RecPageSize
				...
				mobile1=""
				if isMobile(mobile) then
					mobile1="<a href='###' onClick=""changeMobile('"&rs("ord")&"','"&mobile&"',true);try{window.frames['cFF2'].document.getElementById('date').submit();}catch(e1){}"">"
					mobile2=mobile&"</a><br/>"&chr(13)
				else
					mobile1=""
					mobile2="<br/>"&chr(13)
...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>短信发送</title>" & vbcrlf & "<script src= ""../Script/me_alliframe.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write "<script src= ""../Script/me_leftadd.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "button.wavbutton {" & vbcrlf & "     color: #000;" & vbcrlf & "    border: 1px solid #ccc;" & vbcrlf & " text-align:center;" & vbcrlf & "      font-size:14px;" & vbcrlf & " cursor:pointer;" & vbcrlf & " " & vbcrlf & "padding: 0 4px;" & vbcrlf & " padding-top:0;" & vbcrlf & "  transition:all 0.2s ease-in-out;" & vbcrlf & "        -moz-transition:all 0.2s ease-in-out;" & vbcrlf & "   -webkit-transition:all 0.2s ease-in-out;" & vbcrlf & "       -o-transition:all 0.2s ease-in-out;"& "}" & "tr.hei_37 td,tr.hei_37 td.name{height:35px;}" & vbcrlf & "</style>" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;zindex:2000;display:none"">" & vbcrlf & "       <iframe src=""MessageADSearch.asp"" id=""adsIF"" width=""400px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf &"<table border=""0"" cellpadding=""0"" width=""195""  cellspacing=""0"" id=""placeInfo"" style=""margin:0;"">" & vbcrlf & "<tr><td  id=""placeInfo"">&nbsp;短信地址库</td></tr>" & vbcrlf & "</table>" & vbcrlf & "<table height=""100%"" border=""0"" width=""193""  cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "<tr><td valign=""top"" background=""../images/m_leftbg.gif"">" & vbcrlf & "        <table width=""193"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" height=""100px"" style=""table-layout:fixed"">" & vbcrlf & "               <tr>" & vbcrlf & "                    <td class=""resetHeadBg"" height=""27"" valign=""middle"" style=""height:55px;background:#f4fafe;"">" & vbcrlf & "                        <div align=""center"">" & vbcrlf & "                      <form name=""form"" method=""post"">" & vbcrlf & "                    <select name=""B"" id=""B"" style=""width:55px;"">" & vbcrlf & "                  <option value=""name"" "
	if B="name" then
	...
	Response.write ">手机号码</option>" & vbcrlf & "                    </select>   " & vbcrlf & "                    <input name=""C"" id=""C8"" type=""text"" size=""10"" value=""检索关键字"" style=""width:70px;font-size: 9pt;color: #999999;text-align:center;"" onFocus=""if(value==defaultValue){value='';this.style.color='#000'}""  onkeyup=""ajaxSubmit_page(0,1,3)"" onKeyPress=""if(event.keyCode==13) {ajaxSubmit_page(0,1,3);return false;}"" />" & vbcrlf & "                     <input name=""top"" type=""hidden"" size=""2""  value="""
	Response.write top
	Response.write """>" & vbcrlf & "                 </form>" & vbcrlf & "                 <button class='wavbutton' style='width:44px;' onclick=""Left_adSearch(this);"" >高级</button>" & vbcrlf & "                       </div>" & vbcrlf & "                  </td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "                  <td valign=""center"" class=""menutab resetTableBgColor resetBorderColor"" style=""height:37px;background:#f4fafe;border-bottom:1px solid #ccc;border-top:1px solid #ccc;padding:0;"">" & vbcrlf & "                     <table class=""resetGroupTableBg"" width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" style=""background-image:url(../../../sysa/images/smico/lmeuntab_bg_c.gif);"">" & vbcrlf & "                   <tr class=""hei_37"">" & vbcrlf & "                       <td width=""80px""  height=""22px""  class=""name"" style=""background:url(../../../sysa/images/smico/jt1.gif) no-repeat left center;""><b>联系人列表</b></td>" & vbcrlf & "                  <td class=""name""  align=""right"">" & vbcrlf & "                     <a href=""###"" onclick=""ajaxSubmit_page(0,1,1);"">显示分类</a>" & vbcrlf & "                        "
	if open_67_12=1 or  open_67_12=3  then
		Response.write "<a href=""../sortsms/add_cp.asp"" target=""_blank"">添加分组</a>"
	end if
	Response.write "</td>" & vbcrlf & "                 </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                        </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr><td valign=""top"">" & vbcrlf & "                     <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""3"" id=""leftmenu1"">" & vbcrlf & "                    <tr><td>" & vbcrlf & "                                        <span id=""cp_search"">" & vbcrlf & "                                       "
	menu 0,1
	Response.write "" & vbcrlf & "                                      </span>" & vbcrlf & "                         </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   </table></td>" & vbcrlf & "           </tr>" & vbcrlf & "   </table>" & vbcrlf & "        </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr><td><div style='width:195px;height:8px'></div></td></tr>" & vbcrlf & "</table>" & vbcrlf & ""
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    <td valign=""top""><iframe name=""cFF2"" id=""cFF2""  autocomplete=""off"" src=""../message/all.asp?time="
	Response.write now()
	Response.write """ width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self""  > 浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</iframe></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "</html>"
