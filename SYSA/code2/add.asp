...
'function IsTime(values)
	...
	Sub  Bill_OnPageInit
		Response.write "" & vbcrlf & "<style> .sub-field{word-wrap:break-word}" & vbcrlf & "       html{padding: 0px 10px;background: #efefef;box-sizing:border-box;}" & vbcrlf & "      body{background: #ffffff;}</style>" & vbcrlf & "<script>window.__ImgBigToSmall();</script>" & vbcrlf & ""
'Sub  Bill_OnPageInit
	end sub
	Sub App_usernologin
		if app.getInt("onlyview")<>1 then
			if info.user = 0 then
				Response.write "<center style='font-size:40px'><br><br><br><br><br> 400 . Bad Request </center>"
'if info.user = 0 then
				Response.end
			end if
		else
			response.Cookies("QRCodeViewSign") = StrReverse( datediff("s", "2010-1-1", now) & "" )
			Response.end
		end if
	end sub
	Sub Bill_OnInit(Byval bill, ByVal t)
		Dim rs, defcls, i, fd, drs, onlyview, onlyread, addcate, fromSys, isupdate, clsstop
		onlyview = app.getInt("onlyview")=1
		onlyread = app.getInt("onlyread")=1
		defcls = app.getint("ctype")
		If t = 0 And onlyview = False Then
			if defcls=0 then
				Call bill.deleteTempRes("qrcode.")
			end if
			app.adddefaultScript
		end if
		Set drs = cn.execute("select *, isnull((select top 1 fromSys from C2_CodeTypes where C2_CodeTypes.id=C2_CodeItems.ctype),0) as fromSys from dbo.C2_CodeItems where id=" & bill.id)
		If drs.eof = False Then
			If defcls = 0 then
				defcls = drs("ctype").value
			end if
			addcate = drs("addcate").value
			fromSys = drs("fromSys").value
			isupdate = 1
		else
			addcate = Info.User
			fromSys = 0
		...
		drs.close
		If defcls = 0 Then
			Set rs = cn.execute("select top 1 id from C2_CodeTypes where isnull(fromSys,0) <> 2 and stop=0 order by gate1 desc, id")
			If rs.eof = False Then
				defcls = rs(0).value
			else
				defcls = 0
		...
		bill.groups(0).title = "手动录入信息"
		bill.ahonegp = False
		bill.maxspan =  3 - app.getInt("onlyview")
		bill.ahonegp = False
		bill.buttons.remove "reset"
		If onlyread Then
			bill.buttons.getItemByDbname("save").topvisible = False
			bill.buttons.getItemByDbname("save").bottomvisible = False
			If app.power.existsPowerIntro(106,2,addcate) then
				bill.buttons.add "修改", "correct", "window.location.href=""add.asp?ord=" & app.base64.pwurl(bill.id) & """", true, False
			end if
			If app.power.existsPowerIntro(106,3,addcate) then
				bill.buttons.add "删除", "delete1", "cBtnAjax(""delItem""," & bill.id & ",""确定要删除吗？"");", true, false
			end if
		end if
		If app.power.existsPowerIntro(106,7,addcate) Then
			bill.buttons.add "打印", "printer", "window.print()", true, False
			If onlyread And bill.id>0 then
				bill.buttons.add "模板打印", "newprinter", "NewPrint("& bill.id &")", true, False
			end if
		end if
		With bill.fields.add("选择分类", "ctype")
		.uitype = "select"
		If fromSys = 2 Then
			.source = "sql:select title, id from C2_CodeTypes where id =" & defcls & " and (stop=0 or id=" & defcls & ")"
		else
			.source = "sql:select title, id from C2_CodeTypes where isnull(fromSys,0) <> 2 and (stop=0 or id=" & defcls & ")  order by gate1 desc, id"
		end if
		.colspan = 3
		If isupdate = 0 Then
			.defvalue = defcls
		...
		If cn.execute("select 1 from C2_CodeTypes where id=" & defcls & " and stop=1").eof = False Then
			.unit = "<span style='color:red'>（注：该分类已经停用，不允许修改该分类下信息。）</span>"
			clsstop = True
			bill.buttons.getItemByDbname("save").topvisible = False
			bill.buttons.getItemByDbname("save").bottomvisible = false
		end if
		.edit = Not (onlyview Or onlyread)
		.js = " onchange=""window.location.href='?" & app.iif(bill.id=0, "", "ord=" & app.base64.pwurl(bill.id) & "&") & "ctype=' + this.value "" "
		.edit = Not (onlyview Or onlyread)
		End With
		Dim fdcount, cpan ,sql
		fdcount = cn.execute("select count(1) from C2_CodeTypeFields where cTypeId=" & defcls & "").fields(0).value
		cpan = 0
		if fromSys=2 then
			sql="select x.*, uName as funame,"&_
			"CASE WHEN x.uName='流水号' AND ci.billType IS NOT NULL AND  ci.billType !=0 THEN ('B'+cast (ci.billType AS NVARCHAR(max))+ x.uvalue)ELSE  x.uvalue end as nvalue,"&_
			"utype from C2_CodeItemsFields x INNER JOIN dbo.C2_CodeItems ci ON ci.id = x.codeId  where codeid= & bill.id &  ORDER BY x.gate1,x.id"
		else
			sql = "select a.*, "&_
			"  isnull((select uname from C2_CodeItemsFields x where x.codeid=" & bill.id & " and x.ftypeId=a.id),a.uname) as funame  ,"&_
			"  isnull((select uvalue from C2_CodeItemsFields x where x.codeid=" & bill.id & " and x.ftypeId=a.id),'') as nvalue "&_
			"  from C2_CodeTypeFields a "&_
			"  where a.stop=0 and a.cTypeId=" & defcls & " "&_
			"  order by a.gate1 desc , id "
		end if
		Set rs = cn.execute(sql)
		fieldType= "number"
		If onlyread or onlyview then fieldType="text"
		set ml=New myClass
		While rs.eof = False
			Set fd =  bill.fields.add(rs("funame").value, "qrf_" & rs("id").value)
			nvalue = rs("nvalue").value
			Select Case rs("uType").value
			Case 0 : fd.uitype = "text" : fd.maxlimit = 100
			Case 11 : fd.uitype = "picture" :
			fd.colspan= 2 : fd.inline = False  : fd.remark = "提示：最佳图片大小200*200（PS），只支持GIF、JPG、JPEG、PNG。"
			Case 12 : fd.uitype = "image" :
			Case 36 : fd.uitype = fieldType:
			Case 4 : fd.uitype = "radio" : fd.source = "options:是=1,否=0"
			Case 1 : fd.uitype = "date"
			Case 2 : nvalue = FormatNumber(ml.IsNum(nvalue),sdk.Info.floatNumber,-1)
			Case 1 : fd.uitype = "date"
			Case 3 : nvalue = FormatNumber(ml.IsNum(nvalue),sdk.Info.moneyNumber,-1)
			Case 1 : fd.uitype = "date"
			Case 3000 : nvalue = FormatNumber(ml.IsNum(nvalue),sdk.Info.StorePriceDotNum,-1)
			Case 1 : fd.uitype = "date"
			Case 8 :
			Dim opdata : opdata = getOptionData(rs("zdys").value)
			If opdata = "" Then opdata = "未设置=0"
			fd.uitype = "select"
			fd.source = "options:" & opdata
			case 13: fd.uitype = "editor" : fd.colspan = 2 : fd.inline = False
			End Select
			If Len(rs("nvalue").value & "") > 0 Then
				fd.value = nvalue
			end if
			fd.edit = (Not (onlyview Or onlyread)) And (clsstop=false)
			If fd.edit Then
				fd.title =  rs("uName").value
			end if
			i = i + 1
			fd.title =  rs("uName").value
			rs.movenext
			If cpan >=0 Then
				cpan = cpan + fd.colspan
'If cpan >=0 Then
				If i = 1 And fd.colspan = 1 Then
					If rs.eof = False then
						Dim ui : ui = rs.fields("uType").value
						If ui = 1 Or ui = 2 Or ui = 3 Then
							fd.colspan = 2
							cpan = cpan + 1
							fd.colspan = 2
						end if
			...
			If cpan >= 2 And Not onlyview Then
				With bill.fields.addHtml("" , "cqrcodePage",  1,  13)
				.dbname = "@cqrcodePage"
				.valign = "top"
				End with
				cpan = -1
'End with
		...
		cpan = 0
		For i = 1  To bill.fields.count  -1
			cpan = 0
			Set fd = bill.fields.item(i)
			If fd.dbname = "@cqrcodePage" Then Exit For
			cpan = cpan+fd.colspan
'If fd.dbname = "@cqrcodePage" Then Exit For
			If cpan > 2 Then
				cpan = cpan-fd.colspan
'If cpan > 2 Then
				fd.SwapField "@cqrcodePage"
'If cpan<2 then
				Set fd = bill.fields.item(i-1)
'If cpan<2 then
				fd.colspan = 2
			end if
			Exit for
'end if
	...
	Function getOptionData(ByVal data)
		If Len(data) = 0 Then Exit function
		Dim i , ii,  items1, items2, rows : rows = Split(data, Chr(7))
		Dim c ,ck , v: c =  ubound(rows)
		ck=true
		do Until ck = false
			ck=false
			For i = 0 to c -1
				ck=false
				items1 = Split(rows(i), Chr(6))
				items2 = Split(rows(i+1), Chr(6))
				items1 = Split(rows(i), Chr(6))
				if clng(items1(1)) > clng(items2(1)) then
					v = rows(i)
					rows(i) = rows(i+1)
					v = rows(i)
					rows(i+1) = v
	...
	Sub cqrcodePage(ByRef field)
		Dim sizev, color, bgcolor, bll, rst, logo, onlyread
		onlyread = app.getInt("onlyread")=1
		sizev = 280
		color = "#000000"
		bgcolor = "#ffffff"
		Set bll = field.parentgroup.bill
		If bll.id > 0 Then
			Set rst = cn.execute("select logo, color, bgcolor,errhandle, width from dbo.C2_CodeItems where id=" & bll.id)
			If rst.eof = False then
				color = rst.fields("color").value
				bgcolor = rst.fields("bgcolor").value
				logo = rst.fields("logo").value
				sizev = rst.fields("width").value
				errhandle = rst.fields("errhandle").value
				If errhandle&""="" then errhandle=3
			end if
			rst.close
			Set rst = nothing
		end if
		Response.write "" & vbcrlf & "     <table align='center' style='width:100%;height:450px' valign='top' border=0>" & vbcrlf & "    <tr>" & vbcrlf & "            <td style='height:320px; border:0px' align='center' valign='center'>" & vbcrlf & "                    <table style='border:1px solid #c2c3df;width:300px;height:300px;text-align:center' id='qrcodebody'>" & vbcrlf & "                    <tr>" & vbcrlf & "                            <td style='width:300px;height:300px;display:block'>" & vbcrlf & "                                     "
		If onlyread Then
			Response.write GetQrCodeHtml(bll.id)
		else
			Response.write "" & vbcrlf & "                                     <img align='center' src='' style='margin:5px' onerror='this.style.display=""none"";$ID(""viewcodespan"").style.display=""""' onclick='' id='qrcodeView'><span id='viewcodespan' style='color:#ccc;display:none;font-size:16px'>无法预览二维码</span>" & vbcrlf & "                                        "
			Response.write GetQrCodeHtml(bll.id)
		end if
		Response.write "" & vbcrlf & "                             </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   "
		If onlyread = False Then
			Response.write "" & vbcrlf & "     <tr>" & vbcrlf & "            <td valign='top'  style=""height:20px;"">" & vbcrlf & "                   <table align='center' style='color:#000;width:60%;text-align:center'>" & vbcrlf & "                   <tr>" & vbcrlf & "                            <td style='border-bottom:2px solid #eee;cursor:pointer' id='spk1' onmouseover='swpColor(this,0)' onclick='sSetItem(this)' onmouseout='swpColor(this,1)'>基本设置</td>" & vbcrlf & "                            <td style='border-bottom:2px solid #eee;cursor:pointer' id='spk2' onmouseover='swpColor(this,0)' onclick='sSetItem(this)' onmouseout='swpColor(this,1)'>颜色</td>" & vbcrlf & "                               <td style='border-bottom:2px solid #eee;cursor:pointer' id='spk3' onmouseover='swpColor(this,0)' onclick='sSetItem(this)' onmouseout='swpColor(this,1)'>嵌入LOGO</td>" & vbcrlf & "                    </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "            <td  style='height:70px;padding-top:5px' valign='top'>" & vbcrlf & "                 <table align='center' style='width:320px;height:50px;text-align:left;cursor:default' border=0>" & vbcrlf & "                  <tr  id='spk_b_1'>" & vbcrlf & "                              <td style='line-height:20px;padding-left:20px;'>" & vbcrlf & "                                容错率：" & vbcrlf & "                                <input name='errorh' type='radio' id='errorh_1' onclick='errhchange()' value='0' "
			if errhandle=0 then
				Response.write "checked"
			end if
			Response.write "><label for='errorh_1'>7%</label> &nbsp; " & vbcrlf & "                            <input name='errorh' type='radio' id='errorh_2' onclick='errhchange()' value='1' "
			if errhandle=1 then
				Response.write "checked"
			end if
			Response.write "><label for='errorh_2'>15%</label> &nbsp;" & vbcrlf & "                            <input name='errorh' type='radio' id='errorh_3' onclick='errhchange()' value='2' "
			if errhandle=2 then
				Response.write "checked"
			end if
			Response.write "><label for='errorh_3'>25%</label> &nbsp;" & vbcrlf & "                            <input name='errorh' type='radio' id='errorh_4' onclick='errhchange()' value='3' "
			if errhandle=3 then
				Response.write "checked"
			end if
			Response.write "><label for='errorh_4'>30%</label><br>" & vbcrlf & "                               <table border=0>" & vbcrlf & "                                <tr><td style='width:70px;'>图案大小：</td><td>" & vbcrlf & "                                 <input id='esrl' name='imagesize' class=""easyui-slider"" value='"
			Response.write "checked"
			Response.write sizev
			Response.write "' min='52' max='804' style=""width:160px"" data-options=""showTip:false"">" & vbcrlf & "                           </td>" & vbcrlf & "                           <td style='width:60px;padding-left:10px;'><input id='esrvTxt' onfocus='this.blur()' type='text' value='"
			Response.write sizev
			'Response.write sizev
			Response.write "px' style='width:35px;padding:2px;font-size:12px' onkeydown='window.event.keyCode=0;return false;' readonly>" & vbcrlf & "                         </td>" & vbcrlf & "                           </tr></table>" & vbcrlf & "                           </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   <tr  id='spk_b_2' >" & vbcrlf & "                             <td style='line-height:22px'>" & vbcrlf & "                                 <table border=0>" & vbcrlf & "                                        <tr>" & vbcrlf & "                                            <td rowspan=2 style='width:20px'>&nbsp;</td>" & vbcrlf & "                                            <td align='right'>前景色：</td>" & vbcrlf & "                                         <td align='left'>" & vbcrlf & "                                                       "
			Dim cui : Set cui = New BillUIClass
			Response.write cui.GetColorPickerHTML("color", "box", 0, color)
			Response.write "" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td align='right'>背景色：</td>" & vbcrlf & "                                         <td align='left'>" & vbcrlf & "                                                       "
			Response.write cui.GetColorPickerHTML("bgcolor", "box", 0, bgcolor)
			Set cui = nothing
			Response.write "" & vbcrlf & "                                             </td>" & vbcrlf & "                                           <td rowspan=2>&nbsp;</td>" & vbcrlf & "                                       </tr>" & vbcrlf & "                                   <tr><td colspan = 4 style='padding-top:5px'>提示:前景色要比背景色深</td> " & vbcrlf & "                                       </table>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   <tr  id='spk_b_3'>" & vbcrlf &"                         <td style='line-height:22px'>" & vbcrlf & "                                   <input type='hidden' name='codelogo' rsaid="""
			Set cui = nothing
			If Logo <> "" And isnumeric(logo) then
				Response.write app.base64.rsaencode(logo)
			end if
			Response.write """ value="""
			Response.write logo
			Response.write """  id='codelogo_0' onchange='olddata=""\2"";if(isNaN(this.value)){$ID(""codelogo_m2"").value=this.value}else{$ID(""codelogo_m2"").value=""../sdk/bill.upload.asp?v"" + this.getAttribute(""rsaid"")};updateQrCodeView()'>" & vbcrlf & "                                     <table style='width:88%' align='center'>" & vbcrlf & "                        <col style='width:70px'>" & vbcrlf & "                                        <tr>" & vbcrlf & "                                            <td>网络图片：</td>" & vbcrlf & "                                             <td><input type='text' value="""
			Response.write app.iif(isnumeric(logo), "../sdk/bill.upload.asp?V" + app.base64.rsaencode(logo), logo)
			Response.write """ style='width:97%' id='codelogo_m2' onblur='setimgValue(this);app.fireEvent($ID(""codelogo_0""),""onchange"")'></td>" & vbcrlf & "                                     </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td>本地上传：</td>" & vbcrlf & "                                             <td>" & vbcrlf & "                                                    "
			Dim bui : Set bui = New BillUIClass
			Response.write bui.UploaderHtml("image", "选择logo图", "codelogo_0", bll.dbname & ".codelogo", bll.id, 0, 0, "png|gif|jpg|jpeg",true)
			Set bui = nothing
			Response.write "" & vbcrlf & "                                             </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   </table>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                        " & vbcrlf & "                        <script type=""text/JavaScript"">" & vbcrlf & "                           var showUploadDlg = bill.showUploadDlg;" & vbcrlf & "                         bill.showUploadDlg = function(ebox, vboxid, source, id1, id2, id3, filter){" & vbcrlf & "                                       var dHeight = $(document).height();" & vbcrlf & "                                     var wHeight = $(window).height();" & vbcrlf & "                                       if (wHeight == dHeight)" & vbcrlf & "                                 {" & vbcrlf & "                                               $(""html"").css({""overflow"":""hidden""});"& vbcrlf &  "                                   }" & vbcrlf & "                                       showUploadDlg(ebox, vboxid, source, id1, id2, id3, filter); "& vbcrlf & "                                     var cWin = $(""#RoundWin_bll_uploadRes_"" + vboxid);" & vbcrlf &  "                                       var cHeight = cWin.height();" & vbcrlf &  "                                   var cTop = cWin.offset().top; "& vbcrlf &  "                                  if (dHeight < cHeight + cTop + 20)" & vbcrlf & "                                   {" & vbcrlf & "                                               var t = cTop - cHeight - 60;" & vbcrlf & "                                            cWin.css({top:t})" & vbcrlf & "                                       }" & vbcrlf & "                                       if (wHeight == dHeight)" & vbcrlf & "                                 {" & vbcrlf & "                                               $(""html"").css({""overflow"":""auto""});" & vbcrlf & "                                   }" & vbcrlf & "       }" & vbcrlf & "                       </script>" & vbcrlf & "               </td>" & vbcrlf & "   </tr>" & vbcrlf & "   "
		end if
		Response.write "" & vbcrlf & "     </table>" & vbcrlf & ""
	end sub
	function Bill_onSave(ByVal bill)
		Dim item, itemid, itemvalue,  rs, fm,fromSys
		Dim billId : billId = bill.id
		Dim obillid : obillid = billId
		If cn.execute("select 1 from C2_CodeTypes where id=" & app.getInt("ctype") & " and stop=0").eof Then
			Bill_onSave = False
			bill.alert "分类已停用，不允许修改该分类下信息。"
			Exit function
		end if
		Set rs = server.CreateObject("adodb.recordset")
		rs.open "select * from C2_CodeItems where ID=" & billId, cn, 1, 3
		If rs.eof Then
			rs.addnew
			rs.fields("addcate").value = Info.user
			rs.fields("addtime").value = now
			rs.fields("source").value = 0
			rs.fields("sourceID").value = 0
		end if
		rs.fields("ctype").value = app.getInt("ctype")
		rs.fields("logo").value = app.getText("codelogo")
		rs.fields("bgcolor").value = app.getText("bgcolor")
		rs.fields("color").value = app.getText("color")
		rs.fields("errhandle").value = app.getint("errorh")
		rs.fields("width").value = app.getint("imagesize")
		rs.fields("height").value = app.getint("imagesize")
		rs.fields("intro").value = ""
		rs.fields("del").value = 1
		rs.update
		billId = rs.fields("id").value
		rs.close
		rs.open "select TOP 1 ISNULL(ct.fromSys,2) fromSys from C2_CodeItemsFields a left join C2_CodeTypeFields b on a.ftypeid=b.id  LEFT JOIN dbo.C2_CodeTypes ct ON b.cTypeId=ct.id where codeId=" & billId,cn,3,1
		fromSys=""
		if rs.EOF =false then fromSys=rs("fromSys")&""
		if fromSys="2" then
			For Each item In request.form
				If InStr(item, "qrf_")=1 Then
					itemid = clng(Replace(item, "qrf_", ""))
					itemvalue = app.gettext(item)
					If Len(itemvalue) > 0 Then
						cn.execute "update C2_CodeItemsFields set uValue ='"& itemvalue &"' where Id=" & itemid
					end if
				end if
			next
		else
			cn.execute "delete C2_CodeItemsFields where codeId=" & billId
			Set rs = server.CreateObject("adodb.recordset")
			rs.open "select * from C2_CodeItemsFields where 1=0", cn, 1, 3
			For Each item In request.form
				If InStr(item, "qrf_")=1 Then
					itemid = clng(Replace(item, "qrf_", ""))
					itemvalue = app.gettext(item)
					If Len(itemvalue) > 0 Then
						rs.addnew
						rs("codeId").value = billId
						rs("ftypeID").value = itemid
						rs("uValue").value = itemvalue
						rs("uName").value  = ""
						rs("gate1").value = 0
						rs.update
					end if
		...
		rs.close
		cn.execute "update a set a.utype= (Case when ct.fromSys = 2 then a.utype ELSE  b.utype END),a.uname= (Case when ct.fromSys = 2 then a.uName ELSE b.uName END), a.gate1= (Case when ISNULL(ct.fromSys,2) = 2 then a.gate1 ELSE  b.gate1 END) from C2_CodeItemsFields a inner join C2_CodeTypeFields b on a.codeId=" & billId & " and a.ftypeid = b.id  LEFT JOIN dbo.C2_CodeTypes ct ON b.cTypeId=ct.id "
		cn.execute "update a set id1=" & billid & " from sys_upload_res a where a.id1=0 and addcate=" & Info.user & " and charindex('qrcode.',a.source)=1"
		Bill_onSave = True
		Dim txtlong :  txtlong = False
		If cn.execute("select entype from C2_CodeTypes where entype = 0 and id=" & app.getInt("ctype")).eof = False Then
			If cn.execute("select 1 from C2_CodeItemsFields a where codeId = " & billId & " having sum(LEN(uName) + LEN(uvalue) + 1) >= 300").eof = False Then
				txtlong = true
			end if
		end if
		If obillid = 0 then
			bill.showSaveResult2 app.iif(txtlong, "文本模式生成二维码信息超过300个字符时，存在无法扫描的风险，建议修改为网址模式。", "保存成功"), "list.asp"
		else
			bill.ReportRefresh app.iif(txtlong, "文本模式生成二维码信息超过300个字符时，存在无法扫描的风险，建议修改为网址模式。", "保存成功")
		end if
	end function
	Sub App_delItem
		Dim id : id = app.getInt("id")
		cn.execute "update C2_CodeItems  set del=2 , delcate=" & Info.user & ", deltime=getdate() where id=" & id
	end sub
