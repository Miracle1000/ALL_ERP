...
function add(ord,id,contractlist,kuout,kuoutlist,ckid) {
  ...
  var url = "cu_kuin_ck.asp?ord="+escape(ord)+"&num1="+escape(num1)+"&num1old="+escape(num1old)+"&num3="+escape(num3)+"&id="+escape(id)+"&contractlist="+escape(contractlist)+"&kuout="+escape(kuout)+"&kuoutlist="+escape(kuoutlist)+"&ckid="+escape(ckid)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
...
function lockKuCP(idx,status , rid){
	if(idx!=""){
		...
		if(confirm("确定要"+lockStr+"吗？")){
		    var url = "lockKuCP.asp?idx=" + idx + "&rid=" + rid + "&status=" + status + "&timestamp=" + new Date().getTime() + "&date1=" + Math.round(Math.random() * 100);
			xmlHttp.open("GET", url, false);
			xmlHttp.onreadystatechange = function(){				
				if (xmlHttp.readyState == 4) {
					var response = xmlHttp.responseText;
