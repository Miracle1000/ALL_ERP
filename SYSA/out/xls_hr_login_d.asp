...
'bigsystemtype = ".mozi"
	...
	rs1.close
	set rs1=nothing
	if open_77_10=3 then
		list_tj=""
	elseif open_77_10=1 then
		list_tj="and addcate in ("&intro_77_10&")"
	else
	...
	sorce2=0
	sorce3=0
	if open_77_10="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_77_10&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'elseif S1="6" then
	...
	if companyid<>"" then
		Str_Result2=Str_Result2&" and( recv_email in(select email from tel where ord="& companyid &") or person in(select ord from person where company="& companyid &"))"
	end if
	if S2<>"" and S1="" then
	end if
	if G2<>"" then
...
'if T1<>"" and T2<>"" and isnumeric(T1) and isnumeric(T2) then
	...
	elseif px=3 then
		px_Result="order by recv_email desc"
	elseif px=4 then
		px_Result="order by recv_email asc"
	elseif px=5 then
		px_Result="order by stact desc"
	elseif px=6 then
		px_Result="order by stact asc"
	elseif px=7 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_log  where del=1  "&Str_Result2&" order by ord desc "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i
	i=1
	do until rs.eof
		recv_email=rs("recv_email")
		adddate=rs("addtime")
		stact= rs("stact")
		...
		xlWorksheet.Cells(1+i,1).Value = recv_email
		stact="发送失败"
		...
		stact="发送失败"
		xlWorksheet.Cells(1+i,3).Value = csEmail
		stact="发送失败"
		xlWorksheet.Cells(1+i,3).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,4).Value = msEmail
		stact="发送失败"
		xlWorksheet.Cells(1+i,4).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,5).Value = RemoveHTML(content)
		stact="发送失败"
		xlWorksheet.Cells(1+i,5).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,6).Value = stact
		stact="发送失败"
		xlWorksheet.Cells(1+i,6).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,7).Value = person
		stact="发送失败"
		xlWorksheet.Cells(1+i,7).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,8).Value = companyName
		stact="发送失败"
		xlWorksheet.Cells(1+i,8).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,9).Value = adddate
		stact="发送失败"
		xlWorksheet.Cells(1+i,9).font.Size=10
		stact="发送失败"
		xlWorksheet.Cells(1+i,10).Value = cateid
		stact="发送失败"
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>邮件记录导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!邮件记录导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("邮件记录_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的邮件记录</u></strong></font></a></p> " & vbcrlf & ""
