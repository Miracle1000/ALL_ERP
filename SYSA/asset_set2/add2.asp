...
'if pid="" then pid="0"
		...
		if rd<>"" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select parentid,sort1,gate1,StoreCode from O_assAddressType Where id="&rd&" "
		...
		Response.write "" & vbcrlf & "      <table width=""100%""  border=""0"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "            <tr>" & vbcrlf & "                    <td width=""100%"" valign=""top"">" & vbcrlf & "                              <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" height=""27"">" & vbcrlf & "                                      <tr>" & vbcrlf & "                                            <td width=""5%"" height=""27""  background=""../images/contentbg.gif""><div align=""center""><img src=""../images/contenttop.gif""height=""27""> </div></td>" & vbcrlf & "                                            <td width=""95%""  background=""../images/contentbg.gif""><strong><font color=""#1445A6"">目录设置</font></strong></td>" & vbcrlf & "                                        </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                                <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                                      <form name=""date"" method=""post"" action=""add2.asp"" onsubmit=""return Validator.Validate(this,2)&&CheckSort()"" id=""demo"">" & vbcrlf & "                                   <span style=""font-size: 9pt""><input type=hidden name=menu1 value=addto></span>" & vbcrlf & "                                    <tr class=""top"">" & vbcrlf & "                                          <td  colspan=""2""><div align=""center"">添加目录</div></td>" & vbcrlf & "                                    </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                            <td width=""120px""><div align=""right"">目录名称：</div></td>" & vbcrlf & "                                          <td>" & vbcrlf & "                                                    <div align=""left"">" & vbcrlf & "                                                                <input name=""sort1"" type=""text"" id=""sort1"" size=""20""  dataType=""Limit"" min=""1"" max=""50""  msg=""长度必须在1个至50个字之间""  value="""
		...
		Response.write ">根地址" & vbcrlf & "                                                               <input type=""radio"" name=""cktp"" id=""sttp"" value=""0"" onclick=""document.getElementById('leftmenuall').style.display='block';"" "
		if pid<>"0" then Response.write " checked"
		Response.write ">子地址" & vbcrlf & "                                                               <a href=""javascript:void(0);"" onclick=""ExpandAll(this);"">全部收缩</a>" & vbcrlf & "                                                       </div>" & vbcrlf & "                                                  <div align=""left"" id=""leftmenuall"" "
        ...
        sql="select isnull(max(depth),0) from O_assAddressType where del=1"
        set rs=conn.execute(sql)
        Depth=rs(0)
        rs.close
        set rs=nothing
        for i=0 to Depth
            sql="select id,sort1,gate1,parentid,depth,storecode,isLeef,isnull((select count(*) from O_assAddressname where sort=a.id),0) as StoreCount from O_assAddressType a where del=1 and Depth="&i&" order by gate1 desc"
            set rs=conn.execute(sql)
            while not rs.eof
...
'else '
    ...
    sub addto()
        ...
        sql = "select id from O_assAddressType where sort1='"&ml.De(sort1)&"'"
        Set Rs = server.CreateObject("adodb.recordset")
        ...
        if cktp=1 then
            ...
            sql="select * from O_assAddressType where del=1 and id="&cksort
            set rs=conn.execute(sql)
            if rs.eof then
                Response.write "<script>alert('父地址不存在或者已被删除！');window.history.back();</script>"
                call db_close : Response.end
            else
                ParentID=cksort
                ...
                isLeef=1
                if rs("isLeef") then
                    conn.execute("update O_assAddressType set isLeef=0 where id="&cksort)
                end if
            end if
        end if
        sqlStr="Insert Into O_assAddressType(sort1,gate1,ParentID,Depth,isLeef,RootID,StoreCode) values("&_
        "'" & ml.De(sort1) & "'," &_
        gate1 & "," &_
        ParentID & "," &_
        Depth & "," &_
        ...
        rd=rs(0)
        rs.close
        if cktp=1 then conn.execute "update O_assAddressType set RootID=id where id="&rd
        if sort=1 then
