...
'if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
	...
	if action="update" then
		...
		if Dtype="add" then
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from hr_perform_sp  where title='"&title&"' and del=0"
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "insert into hr_perform_sp (title,creator,inDate,del) values('"&title&"',"&session("personzbintel2007")&",'"&now()&"',0)"
				call jsLocat("添加成功","perform_sp_class.asp")
			else
		...
		elseif Dtype="save" and isnumeric(ord) then
			conn.execute "update hr_perform_sp set title='"&title&"'  where id="&ord&" and del=0"
			call jsLocat("修改成功","perform_sp_class.asp")
		end if
	elseif action="edit" and isnumeric(ord) then
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from hr_perform_sp  where id="&ord&" and del=0"
		rs.open sql,conn,1,1
		if not rs.eof then
			title=rs("title")
			ord=rs("id")
	...
	elseif action="del" and isnumeric(ord) then
		conn.execute "delete hr_perform_sp  where id="&ord&""
		call jsLocat("删除成功","perform_sp_class.asp")
	end if
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place""><span class=""name"">绩效审批分类设置</span></td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & ""
	if open_81_13=1 or ((open_81_2=3 or CheckPurview(intro_81_2,trim(creator))=True) and ord<>"") then
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">审批分类基本设置</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <form method=""post"" name=""demo""  action=""perform_sp_class.asp?action=update"" id=""demo"" onsubmit=""return Validator.Validate(this,2)"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""14%""  height=""26"" align=""right"" class=""name"">"
		...
		Response.write """ size=""30"" dataType=""Limit"" min=""1"" max=""50""  msg=""长度必须在1至50个字之间"">" & vbcrlf & "              <span id=""ulist1"" class=""red"">*&nbsp;</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""14%""  height=""26"" align=""right"" class=""name"">&nbsp;</td>" & vbcrlf & "            <td  height=""26""  class=""name"">"
		if action="edit" then
			Response.write "" & vbcrlf & "              <input name=""button22"" class=""anybutton2"" onClick=""javascript:document.all.demo.action='perform_sp_class.asp?action=update&type=save&ord="
			Response.write pwurl(ord)
			Response.write "'""  value=""保  存"" type=""submit"">" & vbcrlf & "              "
		else
			Response.write "" & vbcrlf & "              <input name=""button23"" class=""anybutton2"" value=""保  存""  onClick=""javascript:document.all.demo.action='perform_sp_class.asp?action=update&type=add'"" type=""submit"">" & vbcrlf & "              "
		end if
	...
	rs.open sql,conn,1,1
	if not rs.eof then
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">审批分类列表</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name""><table width=""100%"" border=""0"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#C0CCDD"" >" & vbcrlf & "              <tr class=""top"">" & vbcrlf & "                <td width=""10%"" align=""center""><div align=""center"">序号</div></td>" & vbcrlf & "                <td width=""19%"" align=""center""><div align=""center"">"
		Response.write inName1
		Response.write "</div></td>" & vbcrlf & "                <td width=""13%"" align=""center""><div align=""center"">添加人</div></td>" & vbcrlf & "                <td width=""14%"" align=""center""><div align=""center"">添加时间</div></td>" & vbcrlf & "                <td width=""18%"" align=""center""><div align=""center"">操作</div></td>" & vbcrlf & "              </tr>" & vbcrlf & "              "
		k=1
		do while not rs.eof
			D_title=rs("title")
			D_indate=rs("indate")
			D_cate=getGateName(rs("creator"))
			ord=rs("id")
			Set rss=conn.execute("select id from hr_perform_sp_list where del=0 and sortid="&ord)
			If rss.eof Then
				delsp=0
			else
				delsp=1
			...
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write D_cate
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write D_indate
			...
			if open_81_2=3 or CheckPurview(intro_81_2,trim(rs("creator")))=True then
				Response.write "" & vbcrlf & "                                                             <input name=""button3"" class=""anybutton2"" value=""修改"" onClick=""javascript:window.location.href='perform_sp_class.asp?action=edit&ord="
				Response.write pwurl(ord)
				Response.write "'"" type=""button"">"
			end if
			if (open_81_3=3 or CheckPurview(intro_81_3,trim(rs("creator")))=True) And delsp=0 then
				Response.write "" & vbcrlf & "                                                             <input name=""button32"" class=""anybutton2"" value=""删除"" onClick=""if(confirm('确认删除？')){javascript:window.location.href='perform_sp_class.asp?action=del&ord="
				Response.write pwurl(ord)
	...
	Response.write "" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""430"" ><div align=""center""></div></td>" & vbcrlf & "        </tr>"& vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	rs.close
	conn.close
	set conn=nothing
