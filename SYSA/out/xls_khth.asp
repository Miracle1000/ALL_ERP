...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_1_10=1 then
		list_tj="and x.cateid in ("&intro_1_10&")"
	else
		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					xlApplication.ActiveSheet.Columns(3).NumberFormatLocal = "#,##0."&dotstr_num&"_ "
					...
					zdy5=request("zdy5")
					zdy6=request("zdy6")
					Str_Result=Str_Result & "and x.del=1 and x.sort3=1 "
					px2=request("px2")
					if px2="1" then
						Str_Order=" order by money1 desc"
					else
						Str_Order=" order by num2 desc"
					end if
					set rs=server.CreateObject("adodb.recordset")
					sql=""&_
					"select x.ord,x.name,x.khid,x.cateid,sum(y.num1) as num2 ,count(distinct z.ord) as num1,isnull(sum(y.money1),0) as money1 "&_
					"from tel as x "&_
					"inner join contractthlist as y WITH(NOLOCK) on x.ord=y.company and y.del=1 and x.sort3=1" &_
					"inner join contractth as z WITH(NOLOCK) on z.ord=y.caigou and z.del=1 " &_
					""&Str_Result & " and x.del=1 "&_
					"group by x.ord,x.name,x.khid,x.cateid" & Str_Order
					rs.open sql,conn,1,1
					C1=rs.recordcount
					xlApplication.Init "客户退货排行_"&session("name2006chen")&".xls",C1
					dim i
					i=1
					do until rs.eof
						k=rs("name")
						...
						sql7="select isnull(sum(money1),0) as pay ,count(ord) as numht from contract where company="&ord&" and del=1"
						...
						set rs7=nothing
						xlWorksheet.Cells(1+i,3).Value =Formatnumber(num2,num1_dot,-1)
						set rs7=nothing
						...
						set rs7=nothing
						xlWorksheet.Cells(1+i,5).Value =num1
						set rs7=nothing
						...
						set rs7=nothing
						xlWorksheet.Cells(1+i,7).Value = numht
						set rs7=nothing
						xlWorksheet.Cells(1+i,7).font.Size=10
						set rs7=nothing
						xlWorksheet.Cells(1+i,8).Value = catename
						set rs7=nothing
					...
					rs.close
					set rs=Nothing
					Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户退货排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户退货排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
					set rs=Nothing
					Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
					Set fs = CreateObject("Scripting.FileSystemObject")
					tfile=Server.MapPath("客户退货排行_"&session("name2006chen")&".xls")
					...
					Response.write """><font class=""red""><strong><u>下载导出的客户退货排行</u></strong></font></a></p> " & vbcrlf & ""
