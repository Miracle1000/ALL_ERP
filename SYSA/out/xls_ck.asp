...
'Private Sub Class_Initialize
	...
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>出库单导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "</HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">出库单导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出出库单,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "  "
	dim MODULES
	...
	dotstr_num=""
		for i=1 to num1_dot
			...
			xApp.init Me, conn
			Set xsheet = xApp.sheets.add("出库单")
			HeaderStr =  "出库编号,出库主题,出库类别,出库状态,库管,申请日期,申请人员,审批日期,审批人员,客户名称"
			m=10
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select * from zdy where sort1=32 and set_open=1 and dc=1 order by gate1 asc "
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				do until rs1.eof
					HeaderStr = HeaderStr  & "," & rs1("title")
					rs1.movenext
				loop
			end if
			rs1.close
			set rs1=nothing
			HeaderStr = HeaderStr & ",审批意见,关联合同"
			n=m
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select id,title,sort,kd,set_open,gate1,sorce from zdymx where sort1=32 and set_open=1 order by gate1 asc"
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				do until rs1.eof
					If rs1("sorce")=4 Then
						HeaderStr = HeaderStr  & "," &  rs1("title") & ",仓库,批号,序列号"
					elseIf rs1("sorce")=5  Then
						HeaderStr = HeaderStr  & "," &  rs1("title") & ",出库数量,申请发货,实际发货"
						If open_23_4 = 1 Or open_23_4 = 3 Then
							HeaderStr = HeaderStr  & "," & "单价,总价"
						end if
						If open_23_3 = 1 Or open_23_3 = 3 Then
							HeaderStr = HeaderStr  & "," & "成本单价,成本总价"
						end if
					elseif rs1("sorce")=6 Then
						HeaderStr = HeaderStr  & ",生产日期,有效日期,到货日期"
					elseif rs1("sorce")=7 Then
						HeaderStr = HeaderStr  & "," &  rs1("title")
					elseif rs1("sorce")=13 Then
						HeaderStr = HeaderStr  & "," &  rs1("title") & ",包装,件数"
					else
						HeaderStr = HeaderStr  & "," &  rs1("title")
					end if
					rs1.movenext
			...
			rs1.close
			set rs1=Nothing
			Set rs1= conn.execute("select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=32 and sort2=16")
			if rs1.eof then
				open_32_16=0
				intro_32_16=0
			else
				...
				If intro_32_16&""<>"" Then intro_32_16 = Replace(intro_32_16," ","")
			end if
			rs1.close
			Set rs1 = Nothing
			...
			if newstate="1" Then
				Str_Result= Str_Result + " and ((kg="&session("personzbintel2007")&" and complete1=1 and ("& open_32_16 &"=3 or ("& open_32_16 &"=1 and charindex(','+cast(cateid as varchar(10))+',',',"& intro_32_16 &",')>0))) or (complete1=3 and alt is null and cateid="&session("personzbintel2007")&"))"
'if newstate="1" Then
			end if
			if a="" then
				a=7
...
'elseif a=3 then
			elseif a=4 then
				Str_Result=Str_Result+"and   (fh1=0 or fh1=1) and complete1=3 and sort1<>8"
'elseif a=4 then
			elseif a=5 then
				Str_Result=Str_Result+"and   fh1=1 and complete1=3"
'elseif a=5 then
			elseif a=6 then
				Str_Result=Str_Result+"and   fh1=2"
'elseif a=6 then
			end if
			if C<>"" then
				if B="ckzt" then
...
'if B="ckzt" then
				elseif B="ckbh" then
					Str_Result=Str_Result+"and  ckbh like '%"&C&"%'"
'elseif B="ckbh" then
				elseif B="sqry" then
...
'elseif B="spry" then
				elseif B="glht" then
					Str_Result=Str_Result+"and  order1 in (select ord from contract where title like '%"&C&"%')"
'elseif B="glht" then
				elseif B="glkh" then
					Str_Result=Str_Result+"and  order1 in (select ord from contract where company in (select ord from tel where name like '%"&C&"%'))"
'elseif B="glkh" then
				elseif B="zdy1" then
...
'elseif B="zdy4" then
				elseif B="zdy5" then
					Str_Result = Str_Result + " and  zdy5 in (select ord from sortonehy where gate2=4501 and sort1 like '%"&C&"%')"
'elseif B="zdy5" then
				elseif B="zdy6" then
					Str_Result = Str_Result + " and  zdy6 in (select ord from sortonehy where gate2=4502 and sort1 like '%"&C&"%')"
'elseif B="zdy6" then
				ElseIf B="cpmc" Then
					conn.execute("select a.kuout into #temp from kuoutlist a inner join kuout c on a.kuout = c.ord and c.complete1 = 1 inner join product b on a.ord = b.ord and b.del=1 and a.del=1 where b.title like '%"&C&"%' group by a.kuout")
					Str_Result = Str_Result + " and  ord in (select kuout from #temp)"
				end if
			end if
			if D<>"" and D<>"33" then
...
'if not ZBRuntime.MC(17007) then
			...
			if not ZBRuntime.MC(18000) then
				Str_Result=Str_Result+" and  sort1<>3 and sort1<>5 and sort1<>12 "
'if not ZBRuntime.MC(18000) then
			end if
			if m1<>"" then
...
'if m2<>"" then
			...
			If remind <> "" Then
				...
				Str_Result=Str_Result &" and ddd.ord in (" & helper.listSQL("ids") & ")"
			end if
			...
			if px=1 then
				px_Result="order by title desc,date3 desc,date7 desc"
			elseif px=2 then
				px_Result="order by title asc,date3 desc,date7 asc"
			elseif px=3 then
				px_Result="order by ckbh desc,date3 desc,date7 desc"
			elseif px=4 then
				px_Result="order by ckbh asc,date3 desc,date7 asc"
			elseif px=5 then
				px_Result="order by num1 desc,date3 desc,date7 desc"
			elseif px=6 then
				px_Result="order by num1 asc,date3 desc,date7 asc"
			elseif px=7 then
				px_Result="order by complete1 desc,date3 desc,date7 desc"
			elseif px=8 then
				px_Result="order by complete1 asc,date3 desc,date7 asc"
			elseif px=9 then
				px_Result="order by date3 desc, date7 desc"
			elseif px=10 then
				px_Result="order by date3 asc,date7 asc"
			elseif px=11 then
				px_Result="order by date5 desc,date3 desc,date7 desc"
			elseif px=12 then
				px_Result="order by date5 asc,date3 desc,date7 asc"
			elseif px=13 then
				px_Result="order by date3 desc,date7 desc"
			elseif px=14 then
				px_Result="order by date3 desc,date7 asc"
			ElseIf px = 15 Then
				px_Result = "order by cateidname desc,date3 desc,date7 desc"
			ElseIf px = 16 Then
				px_Result = "order by cateidname asc,date3 desc,date7 asc"
			end if
			set rs1=server.CreateObject("adodb.recordset")
			...
			elseif open_32_10=1 then
				list="and cateid in ("&intro_32_10&")"
			else
				list="and cateid=0"
			end if
...
'Dim rsmxcount, rsmxname, rsmxsort
			...
			rsmx.open "select name,sort from zdy where sort1=32 and set_open=1 and dc=1 order by gate1 asc" , conn, 3, 1
...
'While rsmx.eof = False
				...
				rsmxsort(rsmxcount) = rsmx(1).value
				rsmx.movenext
			wend
			rsmx.close
			Dim rsmxcount2, rsmxname2, rsmxsort2
			rsmxcount2 = -1
'Dim rsmxcount2, rsmxname2, rsmxsort2
			ReDim rsmxname2(0)
			ReDim rsmxsort2(0)
			set rsmx=server.CreateObject("adodb.recordset")
			rsmx.open "select name,sorce from zdymx where sort1=32 and set_open=1 order by gate1 asc" , conn, 3, 1
			While rsmx.eof = False
				rsmxcount2 = rsmxcount2 + 1
'While rsmx.eof = False
				ReDim Preserve rsmxname2(rsmxcount2)
				ReDim Preserve rsmxsort2(rsmxcount2)
				rsmxname2(rsmxcount2) = rsmx(0).value
				rsmxsort2(rsmxcount2) = rsmx(1).value
				rsmx.movenext
			wend
			rsmx.close
			dim i, PreProcV : i=0 : j=1 : PreProcV=-1 : currProcV = 0
			rsmx.close
			dim title,name,khid,sort1,complete1,jz,ly,area,trade,preson,sex,part1,job
			dim phone,fax,mobile,email,url,address,zip,product,c2,c3,c4,intro,intro2,cateid,date7
			Dim complete,rowIndex,pageCount
			pageCount = 1
			set rs=server.CreateObject("adodb.recordset")
			sql="select ord,fh1,sort1,complete1,ckbh,title,CONVERT(VARCHAR(20),date3,120) date3,CONVERT(VARCHAR(20),date5,120) date5,zdy1,zdy2,zdy3,zdy4,zdy5,zdy6," & _
			"(select top 1 name from gate where ord=ddd.cateid) as cateidname," & _
			"(select top 1 name from gate where ord=ddd.kg) as kgname," & _
			"(select top 1 name from gate where ord=ddd.cateout) as cateoutname," & _
			"(case when sort1=1 or sort1=4 then (select top 1 title from contract cc where cc.ord=ddd.order1) else '' end) as contractTitle," & _
			"(case when sort1=1 or sort1=4 then (select top 1 x.name from tel x inner join contract y  on x.sort3=1 and  x.ord=y.company and y.ord=ddd.order1) when sort1=2 then (select top 1 x.name from tel x inner join caigouth y  on x.sort3=2 and  x.ord=y.company and y.ord=ddd.order1) else '' end) as telname," &_
			"(select count(1) from kuoutlist2 where kuout=ddd.ord and num2<num1) as fhcount,intro2 from kuout ddd  "&Str_Result&" "&px_Result
			...
			For j = 1 To C1
				...
				Select case A
				case "4" : if fh1="0" or fh1="1" Then zt="待发货"
				case "5" : if fh1="1" Then zt="部分发货"
				case "6" : if fh1="2" Then zt="已发货"
				case else
				...
				End select
				End Select
				xsheet.writestr rs("ckbh")
				xsheet.writestr rs("title")
				xsheet.writestr cklb
				xsheet.writecell zt
				xsheet.writecell rs("kgname")
				xsheet.writecell rs("date3")
				xsheet.writecell rs("cateidname")
				xsheet.writecell rs("date5")
				xsheet.writecell rs("cateoutname")
				xsheet.writestr rs("telname")
				For i = 0 To rsmxcount
					zdy=rs(rsmxname(i))
					if rsmxsort(i)=1 then
						zdyname=""
						if Len(zdy & "") > 0 then
							Set rs7 = conn.execute("select top 1 sort1 from sortonehy where ord=" & zdy)
							if rs7.eof = false Then zdyname= rs7("sort1")
							rs7.close
						end if
						xsheet.writecell zdyname
					else
						xsheet.writecell zdy
					end if
				next
				xsheet.writecell rs("intro2")
				xsheet.writestr contractTitle
				i = 0
				set rs1 = conn.execute("exec erp_xlsout_kuoutlist " & complete & "," & id & "," & sort)
				While rs1.eof = false
					i=i+1
'While rs1.eof = false
					if complete="3" then
						...
						money4 =zbcdbl( rs1("money4"))
					else
						intro=zbcdbl(rs1("price1"))
						num1=zbcdbl(rs1("num1"))
						num2=zbcdbl(rs1("num2"))
						num3=zbcdbl(rs1("num3"))
						money1 =zbcdbl( rs1("money1"))
					...
					If money3 = "" Then money3 = 0
					If money4 = "" Then money4 = 0
					unit=rs1("unit")
					product=rs1("ptitle")
					order1=rs1("porder1")
					type1=rs1("ptype1")
					...
					xsheet.writespace 11+rsmxcount
					xsheet.writecell "【产品明细】"
					For ii = 0 To rsmxcount2
						sortIndex = rsmxsort2(ii)
						fname = rsmxname2(ii)
						Select Case rsmxsort2(ii)
						...
						If open_23_4 = 1 Or open_23_4 = 3 Then
							xsheet.writenumber money1
							xsheet.writenumber money2
						end if
						If open_23_3 = 1 Or open_23_3 = 3 Then
							xsheet.writenumber money3
							xsheet.writenumber money4
						end if
						Case 6:
						xsheet.writecell datesc
						xsheet.writecell dateyx
						xsheet.writecell rs1(fname)
						Case 7:
						xsheet.writecell rs1(fname)
						Case Else
						If sortIndex>=8 And sortIndex<=13 then
							if fname="zdy5" or fname="zdy6" Then fname = fname & "name"
							xsheet.writecell rs1(fname)
							If sortIndex=13 Then
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>出库单导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!出库单导出成功,共有"&(j-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			tfile=Server.MapPath(xApp.cFileName("出库单") & ".xls")
			xApp.save tfile
			xApp.dispose
			tfile = xApp.HexEncode(tfile)
			Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
			Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'出库单_"&session("name2006chen")&".xls',fileCnt:1})</script>"
			Set xApp = Nothing
			action1="出库单导出"
			call close_list(1)
			Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的出库单</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
