...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_btns input.anybutton2{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "#ht1 table{margin-bottom:-1px}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	if open_101_1=3 then
		list=""
	elseif open_101_1=1 then
		list=" and che_addcateid in ("&intro_101_1&")"
	else
		list=" and che_addcateid=0"
	end if
	...
	if px="1" then
		Ptr_Result=" order by che_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by che_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by che_time desc"
	elseif px="4" then
		Ptr_Result=" order by che_time asc"
	elseif px="5" then
		Ptr_Result=" order by che_bh desc"
	elseif px="6" then
		Ptr_Result=" order by che_bh asc"
	end if
	ret  =request("ret")
	ret2 =request("ret2")
	A    =request("A")
	...
	W2=tmparr(1)
	if A<>"" then
		Str_Result=Str_Result&" and g.prod_unit in("&A&") "
	end if
	if B<>"" then
		Str_Result=Str_Result&" and g.sto_id in("&B&") "
	end if
	if C<>"" then
		Str_Result=Str_Result&" and f.prod_xh like '%"&C&"%' "
	end if
	if D<>"" then
		Str_Result=Str_Result&" and che_title like '%"&D&"%' "
	end if
	if E<>"" then
		Str_Result=Str_Result&" and che_bh like '%"&E&"%' "
	end if
	if F<>"" then
		Str_Result=Str_Result&" and f.prod_name like '%"&F&"%' "
	end if
	if G<>"" then
		Str_Result=Str_Result&" and f.prod_bh like '%"&G&"%' "
	end if
	if H<>"" then
		Str_Result=Str_Result&" and g.prod_ph like '%"&H&"%' "
	end if
	if I<>"" then
		Str_Result=Str_Result&" and g.prod_xlh like '%"&I&"%' "
	end if
	if J<>"" then
		Str_Result=Str_Result&" and b.che_note like '%"&J&"%' "
	end if
	if M<>"" then
		Str_Result=Str_Result&" and g.prod_gys like '%"&M&"%' "
	end if
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',che_addtime)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,che_addtime,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if ret <>"" and not isnull(ret) then
		Str_Result=Str_Result+" and  datediff(d,'"&ret&"',b.che_time)>=0 "
'if ret <>"" and not isnull(ret) then
	end if
	if ret2 <>"" and not isnull(ret2) then
		Str_Result=Str_Result+" and  datediff(d,b.che_time,'"&ret2&"')>=0 "
'if ret2 <>"" and not isnull(ret2) then
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and  b.che_cateid in ("&W3&") "
	end if
	...
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <form name=""data"" id=""data"" action=""checkDList.asp"" method=""get"">" & vbcrlf & "               <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">盘点明细表</td>" & vbcrlf & "                      <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"" class=""top_btns""><div id=""kh"" style='height:22px;line-height:22px;'>" & vbcrlf & "                        "
	currpage=clng(currpage)
	if open_101_10=3 or (open_101_10=1 and CheckPurview(intro_101_10,trim(session("personzbintel2007"))))=True then
		Response.write "<input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_checkDlist.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "                </select><a href=""###"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "        <span id=""ht1""></span>" & vbcrlf & "    </form>" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">      " & vbcrlf & "                 <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点主题</div></td>" & vbcrlf & "                  <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点编号</div></td>" & vbcrlf & "                 <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盘点人员</div></td>" & vbcrlf & "                 <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">名称</div></td>" & vbcrlf & "                   <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">编号</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">型号</div></td>" & vbcrlf & "               <td width=""5%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">单位</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">账面数量</div></td>" & vbcrlf & "         <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">实际数量</div></td>" & vbcrlf & "            <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">盈亏数量</div></td>" & vbcrlf & "         <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">仓库</div></td>" & vbcrlf & "           <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">批号</div></td>" & vbcrlf & "           <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">序列号</div></td>" &_
	"                  <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">生产日期</div></td>" & vbcrlf & "                 <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">有效日期</div></td>" & vbcrlf & "               </tr>" & vbcrlf & "            "
	currpage=Request("currpage")
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.che_title,b.che_cateid,b.che_cateid,b.che_addcateid,b.che_addtime,b.che_bh,b.che_time,c.pro_name,d.sort1,e.name,f.prod_name,f.id as fid,f.prod_bh,f.prod_xh,k.sto_name,g.reg_fid,g.prod_ph,g.prod_xlh,g.prod_sctime,g.prod_ystime from O_kuCheList a "&_
	" left join O_kuCheck b on a.che_fid=b.id "&_
	" left join O_kuInList g on a.prod_id=g.id "&_
	" left join O_product f on g.prod_id=f.id "&_
	" left join O_OfficeType c on f.prod_type=c.id "&_
	" left join sortonehy d on g.prod_unit=d.id "&_
	" left join O_store k on g.sto_id=k.id "&_
	" left join gate e on b.che_cateid=e.ord "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=18><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		allnum=0
		allprice=0
		...
		do while not rs7.eof
			addcateid=rs7("che_addcateid")
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "              <td width=""7%"" height=""25"" align=""left"" valign=""middle""  class=""name"">" & vbcrlf & "            "
			if open_101_14=3 or CheckPurview(intro_101_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('checkDetail.asp?ord="
				Response.write rs7("che_fid")
				Response.write "','neww34win','width=' + 1000 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"
				Response.write rs7("che_fid")
				Response.write rs7("che_title")
				Response.write "</a>" & vbcrlf & "         "
			else
				Response.write rs7("che_title")
			end if
			Response.write "</td>" & vbcrlf & "                   <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write rs7("che_bh")
			Response.write "</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "              "
			che_cateid=rs7("che_cateid")
			if che_cateid="" or isnull(che_cateid) then che_cateid=0
			sql8="select name from gate where ord="&che_cateid
			...
			rs8.close
			set rs8=nothing
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "                <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			...
			Response.write "</div></td>" & vbcrlf & "         <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs7("che_zmnum")),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "         <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs7("che_sjnum")),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "         <td width=""7%"" align=""left"" valign=""middle""  class=""func""><div align=""center"">"
			Response.write Formatnumber(zbcdbl(rs7("che_yknum")),num1_dot,-1)
			...
			che_zmnum=zbcdbl(rs7("che_zmnum"))
			che_sjnum=zbcdbl(rs7("che_sjnum"))
			che_yknum=zbcdbl(rs7("che_yknum"))
			if che_zmnum="" or isnull(che_zmnum) then che_zmnum=0
			if che_sjnum="" or isnull(che_sjnum) then che_sjnum=0
			if che_yknum="" or isnull(che_yknum) then che_yknum=0
			allnum=cdbl(allnum)+cdbl(che_zmnum)
			if che_yknum="" or isnull(che_yknum) then che_yknum=0
			allprice=cdbl(allprice)+cdbl(che_sjnum)
			if che_yknum="" or isnull(che_yknum) then che_yknum=0
			allmoney=cdbl(allmoney)+cdbl(che_yknum)
			if che_yknum="" or isnull(che_yknum) then che_yknum=0
			o=o+1
			if che_yknum="" or isnull(che_yknum) then che_yknum=0
	...
	Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "              <td height=""25"" colspan=""7"" align=""left"" valign=""middle""  class=""name""><div align=""right"">合计：</div></td>" & vbcrlf & "                 <td width=""61"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write Formatnumber(allnum,num1_dot,-1)
	Response.write "</div></td>" & vbcrlf & "         <td width=""65"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write Formatnumber(allprice,num1_dot,-1)
	Response.write "</div></td>" & vbcrlf & "         <td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
	Response.write Formatnumber(allmoney,num1_dot,-1)
	Response.write "</div></td>" & vbcrlf & "         <td colspan=""5"" align=""left"" valign=""middle""  class=""name"">&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "    </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & " <table width=""100%""border=""0"" align=""left"">" & vbcrlf & "         <tr>" & vbcrlf & "          <td width=""60"" height=""30"" class=""page""></td>" & vbcrlf & "         <td></td>         " & vbcrlf & "      <form name=""oneself"" action=""checkDList.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td></form>" & vbcrlf & "      </tr>" & vbcrlf & "   <tr><td height=""2000""></td></tr>" & vbcrlf & "        </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="盘点明细表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:300; height:200;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按盘点时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按盘点时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按盘点编号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:15px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按盘点编号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "</body>" & vbcrlf & "</html>"& vbcrlf
