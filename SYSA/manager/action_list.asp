...
'function IsTime(values)
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "body {  margin-top: 0px;}" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src= ""../Script/mr_action_list.js?ver="
	Response.write Application("sys.info.jsver")
	...
	D =request("D")
	tname=request("tname")
	if tname="" or isnull(tname) then tname=1
	if tname=1 then
		ttable="action_list"
	else
		ttable="action_list1"
	...
	W1=tmparr(0)
	W2=tmparr(1)
	Str_Result=" where a.action1<>'RegOK' and a.action1 not like '%userid%' "
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and a.username in ("&W3&") "
	end if
	...
	if D<>"" then
		Str_Result=Str_Result&" and action1 like '%"&ml.De(D)&"%'"
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" then
		Str_Result=Str_Result+" and  '"&L1&"'<=(case when isnull(a.time_login,'1900-01-01')='' then '1900-01-01' else a.time_login end) "
'if L1<>"" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" Then
		Str_Result=Str_Result+" and  (case when isnull(a.time_login,'1900-01-01')='' then '1900-01-01' else a.time_login end) <= '"&L2&"' "
'if L2<>"" Then
	end if
	Ptr_Result=""
	Ptr_Result2= ""
	...
	if px="1" then
		Ptr_Result=" order by a.time_login desc"
		Ptr_Result2=" order by time_login asc"
		Ptr_Result3=" order by time_login desc"
	elseif px="2" then
		Ptr_Result=" order by a.time_login asc"
		Ptr_Result2=" order by time_login desc"
		Ptr_Result3=" order by time_login asc"
	elseif px="3" then
		Ptr_Result=" order by action1 desc"
		Ptr_Result2=" order by action1 asc"
		Ptr_Result3=" order by action1 desc"
	elseif px="4" then
		Ptr_Result=" order by action1 asc"
		Ptr_Result2=" order by action1 desc"
		Ptr_Result3=" order by action1 asc"
	end if
	ret=ChangeDate(ret,1)
	if ret<>"" Then
		Str_Result=Str_Result+" and  '"&ret&"' <= (case when isnull(a.time_login,'1900-01-01')='' then '1900-01-01' else a.time_login end) "
'if Exit Sub<>"" Then
	else
		ret="点击选择时间"
	end if
	...
	if ret2<>"" Then
		Str_Result=Str_Result+" and  (case when isnull(a.time_login,'1900-01-01')='' then '1900-01-01' else a.time_login end) <= '"&ret2&"' "
'if Exit Sub2<>"" Then
	else
		ret2="点击选择时间"
	end if
	if C<>"" then
		if B="name" then
			Str_Result=Str_Result&" and case when a.wxUserId is null then b.username else '[微信用户]' end like '%"&ml.De(C)&"%' "
		elseif B="bh" then
			Str_Result=Str_Result&" and case when a.wxUserId is null then b.name else m.nickname end like '%"&ml.De(C)&"%'"
		elseif B="xh" then
			Str_Result=Str_Result&" and action1 like '%"&ml.De(C)&"%'"
		elseif B="browser" then
			Str_Result=Str_Result&" and type_brower like '%"&ml.De(C)&"%'"
		elseif B="os" then
			Str_Result=Str_Result&" and type_sys like '%"&ml.De(C)&"%'"
		elseif B="ip" then
			Str_Result=Str_Result&" and ip like '%"&ml.De(C)&"%'"
		end if
	end if'
	...
	sql8="SELECT * FROM power WHERE sort1=66 AND sort2=15 and ord="&session("personzbintel2007")
	set rs8=conn.execute(sql8)
	if rs8.eof then
		open_66_15=0
		intro_66_15=""
	else
		open_66_15=rs8("qx_open")
		intro_66_15=rs8("qx_intro")
	end if
	rs8.close
	set rs8=nothing
	if open_66_15=3 then
		list=""
	elseif open_66_15=1 then
		list=" and a.username in ("&intro_66_15&")"
	else
		list=" and a.username=0 "
	end if
	Str_Result=Str_Result&list
	open_rz=request("open_rz")
	if open_rz<>"" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select id from setjm where ord=802"
		rs.open sql,conn,1,1
		if rs.eof then
			sqlStr="Insert Into setjm(ord,intro,num1,tq1,fw1,cateid,gate1) values('"
			sqlStr=sqlStr & 802 & "','"
			sqlStr=sqlStr & open_rz & "','"
			sqlStr=sqlStr & 0 & "','"
			sqlStr=sqlStr & 0 & "','"
			sqlStr=sqlStr & 0 & "','"
			...
			sqltext="Update setjm set intro="&open_rz&" where ord=802"
	...
	if Application("_open_rz_system")<>open_rz then
		Application("_open_rz_system")=open_rz
	end if
	Response.write "<script>" & vbcrlf & "     function shp(box)" & vbcrlf & "       {" & vbcrlf & "               var dv=box.parentNode.getElementsByTagName(""div"")[0];" & vbcrlf & "             dv.style.display=dv.style.display==""none""?""block"":""none"";" & vbcrlf & "     }" & vbcrlf & "</script>" & vbcrlf & "<table width=""100%""  border=""0""align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">        " & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"" style=""height:44px;"">" & vbcrlf & "          <tr>"& vbcrlf & "            <td class=""place"">操作日志管理</td>" & vbcrlf & "                 <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "            <td align=""right"">" & vbcrlf & "                     <input name=""radiobutton"" type=""radio""  value=""1"" "
	if open_rz="1" then Response.write "checked"
	Response.write " onclick=""if(confirm('开启日志会增加数据库空间，消耗部分服务器资源，启用后，请定期“归档”。确认开启操作日志？')){javascript:window.location='action_list.asp?open_rz=1'}"">" & vbcrlf & "                      开启日志 " & vbcrlf & "                       <input type=""radio"" name=""radiobutton"" value=""0"" "
	if open_rz="0" then Response.write "checked"
	Response.write " onclick=""javascript:window.location='action_list.asp?open_rz=0'"">" & vbcrlf & "                       关闭日志　　&nbsp;&nbsp;" & vbcrlf & "                        <input type=""button"" class=""anybutton2"" id=""btnmit1"" name=""btnmit1"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'url',page:'../out/xls_action_list.asp'});}"">" & vbcrlf & "                    <input type=""button"" name=""Submit43"" value=""打印"" onClick=""javascript:kh.style.display='none';window.print();return  false;""   class=""anybutton2""/>" & vbcrlf & "                       <input type=""button"" class=""anybutton2"" id=""btnmit6756"" name=""btnmit6756"" value=""删除"" onClick=""if(confirm('确认删除查看的记录？')){window.location.href="
	Response.write px
	...
	Response.write "&D="
	Response.write ml.Dep(D)
	Response.write "'}"">" & vbcrlf & "                      <input type=""button"" class=""anybutton2"" id=""btnmit423"" name=""btnmit423"" value=""清空"" onClick=""if(confirm('确认清空所有记录？')){window.location.href='../manager/action_list_clear.asp?px="
	Response.write px
	Response.write "&tname="
	Response.write tname
	Response.write "&A="
	...
	if tname=1 then
		Response.write "" & vbcrlf & "                     <input type=""button"" class=""anybutton2"" id=""btnmit65"" name=""btnmit65"" value=""归档"" onClick=""if(confirm('确认归档？')){window.location.href='../manager/action_list_fromto.asp?px="
		Response.write px
		Response.write "&tname="
		Response.write tname
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""font-size:12px;font-weight: bold;color:#2F496E; overflow:hidden"">" & vbcrlf & "                        <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	'Response.write "'}"">" & vbcrlf & "                      "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "                </select></td>" & vbcrlf & "          <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table>" & vbcrlf & "        <form name=""data"" id=""data"" action=""action_list.asp?tname="
	Response.write tname
	Response.write """ method=""get"">" & vbcrlf & "      <span id=""ht1""></span>" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""table-layout:fixed"">     " & vbcrlf & "                <tr>" & vbcrlf & "              <td colspan=""8""  background=""../images/112.gif""  height=""30"" align=""right"" style=""box-sizing:border-box;border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "             <span id=""kh"">" & vbcrlf & "            <label><input  type=""radio""name=""tname"" value=""1"" "
	Response.write tname
	if tname=1 then Response.write "checked"
	Response.write " onclick=""javascript:window.location='action_list.asp?tname=1'"">日志列表</label>" & vbcrlf & "               <label><input type=""radio"" name=""tname"" value=""2"" "
	if tname=2 then Response.write "checked"
	Response.write " onclick=""javascript:window.location='action_list1.asp?tname=2'"">归档列表</label>" & vbcrlf & "              自：<input type=""text"" name=""ret"" id=""ret"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" value="""
	Response.write ret
	Response.write """ readonly=""readonly"">&nbsp;至：<INPUT type=""text"" name=""ret2"" id=""ret2"" Class=""DateTimePick"" onclick=""datedlg.showDateTime();"" value="""
	Response.write ret2
	Response.write """ readonly=""readonly"">" & vbcrlf & "              <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                                <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""name"" "
	Response.write ret2
	if B="name" then Response.write "selected"
	Response.write ">用户名</option>" & vbcrlf & "                             <option value=""bh"" "
	if B="bh" then Response.write "selected"
	Response.write ">姓名</option>" & vbcrlf & "                               <option value=""xh"" "
	if B="xh" then Response.write "selected"
	Response.write ">操作类型</option>" & vbcrlf & "                           <option value=""browser"" "
	if B="browser" then Response.write "selected"
	Response.write ">浏览器</option>" & vbcrlf & "                             <option value=""os"" "
	if B="os" then Response.write "selected"
	Response.write ">操作系统</option>" & vbcrlf & "                           <option value=""ip"" "
	if B="ip" then Response.write "selected"
	Response.write ">操作IP</option>" & vbcrlf & "                     </select>&nbsp;<input type=""text"" name=""C"" id=""C"" value="""
	Response.write C
	Response.write """ size=""10"" maxlength=""200"">" & vbcrlf & "                  <input type=""submit"" class=""anybutton2"" id=""btnmit"" value=""检索"" onclick=""chkForm()""><input name=""page_count"" type=""hidden"" value="""
	Response.write page_count
	Response.write """><input name=""px"" type=""hidden"" value="""
	Response.write px
	Response.write """>" & vbcrlf & "                        <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "              </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""12%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">用户名</div></td>" & vbcrlf & "                <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">姓名</div></td>" & vbcrlf & "            <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作时间</div></td>" & vbcrlf & "                <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作类型</div></td>" & vbcrlf &   "             <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">浏览器</div></td>" & vbcrlf &  "             <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作系统</div></td>" & vbcrlf &"<td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作IP</div></td>" & vbcrlf & "          <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">操作地址</div></td>" & vbcrlf & "          </tr>" & vbcrlf &""
	Dim numtop
	currpage=Request("currpage")
	...
	currpage=clng(currpage)
	Set rs7 = conn.execute("select count(1) from "&ttable&" a WITH (NOLOCK) " & _
	"left join gate b on a.username=b.ord " & _
	"left join MMsg_User m on m.id=a.wxUserId " &_
	Str_Result)
	BookNum = rs7(0).value
	Set rs7 =  Nothing
	...
	numtop = page_count
	If CurrPage = PageCount And BookNum Mod page_count <> 0 Then numtop = (BookNum Mod page_count)
	If BookNum = 0 Then
		Response.write "<tr><td colspan=8 height=30><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
	else
		set rs7=server.CreateObject("adodb.recordset")
		sql7= "select * from (" & vbcrlf &_
		"select top " & numtop & " * from (" & vbcrlf &_
		"select top " & CurrPage*page_count & " a.*," & vbcrlf &_
		"case when isnull(a.wxUserId,0) = 0 then 0 else 1 end isWxLog," & vbcrlf &_
		"case when isnull(a.wxUserId,0) = 0 then b.username else '[微信用户]' end as uname," & vbcrlf &_
		"case when isnull(a.wxUserId,0) = 0 then b.name else m.nickname end as gname " & vbcrlf &_
		"from " & ttable & "  a WITH (NOLOCK) " & vbcrlf &_
		"left join gate b on a.username=b.ord " & vbcrlf &_
		"left join MMsg_User m on m.id=a.wxUserId " & vbcrlf &_
		Str_Result & Ptr_Result & vbcrlf &_
		") xx"  & Ptr_Result2 & vbcrlf &_
		") xy " & Ptr_Result3
		rs7.open sql7,conn,1,1
		while rs7.eof = false
			type_login=rs7("type_login").value
			keyvalue=rs7("keyvalue").value & ""
			if type_login="" or isnull(type_login) then type_login=2
			Response.write "" & vbcrlf & "<tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "<td height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("uname")
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("gname")
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("time_login")
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("action1").value & ""
			if len(keyvalue)>0 and type_login=2 then
				Response.write "<img src='../images/116.png' style='margin-left:2px;cursor:pointer' onclick='shp(this)'>"
'if len(keyvalue)>0 and type_login=2 then
				Response.write "<div style='display:none;color:#999;padding-top:7px'><span style='border:1px solid #aaa;padding:3px 5px;'>" & keyvalue & "</span></div>"
'if len(keyvalue)>0 and type_login=2 then
			end if
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("type_brower")
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("type_sys")
			Response.write "</div></td>" & vbcrlf & "<td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("ip")
			Response.write "</div></td>" & vbcrlf & "<td valign=""middle"" class=""func""><div align=""center"">" & vbcrlf & "                           "
			Domain_Name = LCase(Request.ServerVariables("Server_Name"))
				pageurl = rs7("page1").value & ""
				if type_login=1 And Len(pageurl)>0 And InStr(1,pageurl,"/mobilephone/",1)=0 And rs7("isWxLog")<>1 And InStr(1,pageurl,"/MicroMsg/mobile/",1)=0 then
					if instr(rs7("page1"),"http://")>0 then
						Response.write "<a href='javascript:void(0)' onclick=""javascript:window.open('"&ml.Dep(pageurl)&"','neww34win','width=1000,height=600,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">查看</a>"
					else
						Response.write "<a href='javascript:void(0)' onclick=""javascript:window.open('"&ml.Dep(pageurl)&"','neww34win','width=1000,height=600,fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">查看</a>"
					end if
				else
		...
		Response.write "" & vbcrlf & "     </table>" & vbcrlf & "        </form>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & "         <form name=""oneself"" action=""action_list.asp?px="
		Response.write px
		Response.write "&A="
		Response.write A
		Response.write "&B="
		...
		Response.write "&tname="
		Response.write tname
		Response.write """ method=""post"" id=""oneself"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""60"" height=""30"">&nbsp;</td>" & vbcrlf & "              <td>&nbsp;</td>" & vbcrlf & "                 <td>" & vbcrlf & "                    <div align=""right"">" & vbcrlf & "                       "
		...
		Response.write "             " & vbcrlf & "                        </div>" & vbcrlf & "                  </td>" & vbcrlf & "                   " & vbcrlf & "                  </tr>" & vbcrlf & "           <tr>" & vbcrlf & "            <td colspan=""3"" height=""300""></td>" & vbcrlf & "                  </tr>" & vbcrlf & "           </table>" & vbcrlf & "              </form>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:140;display:none;"">" & vbcrlf & "<table width=""150"" height=""140""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""140"">" & vbcrlf & "        <table width=""150"" height=""132"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按操作时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按操作时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按操作类型排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl"
		conn.close
