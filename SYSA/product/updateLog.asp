...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script src= ""../Script/pt_updateLog.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "<script language=""javascript"" src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "</head>" & vbcrlf & "<body onload=""txmFocus();"" onclick=""TexTxmFocus(event);"" "
	if open_21_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">"
	dim MODULES
	...
	if  A2<>"" then
		if Str_Result="" then
			Str_Result="where sort1 in ("&area_list&") and sort1<>''"
		else
	...
	if m1<>"" then
		Str_Result=Str_Result+"and  xgTime>='"&m1&"'"
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+"and  xgTime<='"&cdate(m2)+1&"'"
'if m2<>"" then
	end if
	if zdy5<>"" then
...
'elseif P1=6 then
	...
	if I2<>"" then
		if I1=1 then
			str_Result=str_Result+"and aleat1>"&I2&""
'if I1=1 then
		elseif I1=2 then
			str_Result=str_Result+"and aleat1>="&I2&""
'elseif I1=2 then
...
'if H1=1 then
	...
	ord = 0
	PriceModeChangeLog = false
	if request("ord")&""<>"" then
		ord = deurl(request("ord"))
		if ord>0 then
			str_Result=str_Result+" and  ord = " & ord &"   and (not exists(select 1 from product_log where ord = a.ord and mark = 1 and ex_pid =a.ex_pid ) or exists(select 1 from product_log where ord = a.ord and mark = 1 and ex_pid =a.ex_pid and priceMode<> a.priceMode) ) "
'if ord>0 then
			PriceModeChangeLog = (request("PriceModeChangeLog")&""="1")
		end if
	end if
	if J2<>"" then
		if J1=1 then
			str_Result=str_Result+"and aleat2>"&J2&""
'if J1=1 then
		elseif J1=2 then
			str_Result=str_Result+"and aleat2>="&J2&""
'elseif J1=2 then
...
'elseif zdy4_1=6 then
	...
	W4=replace(W3,"0","")
	W4=replace(W4,",","")
	if W4<>"" then    str_Result=str_Result+"and xgord in  ("&W3&")"
	W4=replace(W4,",","")
	...
	if px=1 then
		px_Result="order by xgtime desc,ord desc"
	elseif px=2 then
		px_Result="order by xgtime asc,ord desc"
	elseif px=3 then
		px_Result="order by title desc,ord desc"
	elseif px=4 then
		px_Result="order by title asc,ord desc"
	elseif px=5 then
		px_Result="order by order1 desc,ord desc"
	elseif px=6 then
		px_Result="order by order1 asc,ord desc"
	elseif px=7 then
		px_Result="order by type1 desc,ord desc"
	elseif px=8 then
		px_Result="order by type1 asc,ord desc"
	end if
	xgord=Trim(request("xgord"))
	If len(xgord&"")>0 Then
		Str_Result=Str_Result& " and  xgord in (select ord from gate where del=1 and name like '%"&xgord&"%')"
	end if
	unit=Trim(request("unit"))
	If Len(unit&"")>0 Then
		Str_Result=Str_Result& " and  unitjb in ("&unit&")"
	end if
	Response.write "        <form action=""updatelog.asp?page_count="
	Response.write page_count
	Response.write "&px="
	Response.write px
	Response.write "&H=1"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "<td class=""place"">"
	if PriceModeChangeLog then
		Response.write "计价方式变更记录"
	else
		Response.write "产品日志列表"
	end if
	Response.write "</td>" & vbcrlf & "          <td>&nbsp;"
	if ord=0 then
		Response.write "<a href=""###"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a>"
	end if
	Response.write "</td>" & vbcrlf & "          <td align=""right"">"
	if ord=0 then
		Response.write "<div id=""kh"">" & vbcrlf & "                    操作员： <input type=""text"" name=""xgord"" id=""xgord"" value="""
		Response.write xgord
		Response.write """ size=""10"">" & vbcrlf & "                          修改日期"
		...
		Response.write "" & vbcrlf & "                       <input type=""hidden"" id=""bzhi"" name=""bzhi"" value=""1"">" & vbcrlf & "                   <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "              <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden;"">" & vbcrlf & "                <option>-请选择-</option>" & vbcrlf & "                <option value=""page_count=10"" "
		...
		Response.write ">每页显示200条</option>" & vbcrlf & "              </select>" & vbcrlf & "              <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div>"
	end if
	Response.write "</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <span id=""ht1""></span>" & vbcrlf & "       </form>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""5"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        "
	if PriceModeChangeLog then
		Response.write "" & vbcrlf & "            <tr class=""top"">" & vbcrlf & "              <td align=""center"" ><div align=""center""><strong>操作时间</strong></div></td>" & vbcrlf & "              <td height=""23""  ><div align=""center"">变更前</div></td>" & vbcrlf & "              <td height=""26"" align=""center""><div align=""center"">变更后</div></td>" & vbcrlf & "              <td ><div align=""center"" ><strong>用户名</strong></div></td>" & vbcrlf & "            </tr>" & vbcrlf & "        "
	else
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td align=""center"" ><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "          <td height=""23""  ><div align=""center"">产品名称</div></td>" & vbcrlf & "          <td height=""26"" align=""center"" ><div align=""center"">编号</div></td>" & vbcrlf & "          <td ><div align=""center"" ><strong>产品型号</strong></div></td>" & vbcrlf & "          <td ><div align=""center"">分类</div></td>" & vbcrlf & "          <td ><div align=""center"" ><strong>单位</strong></div></td>" & vbcrlf & "              <td ><div align=""center"" >修改人</div></td>" & vbcrlf & "            <td ><div align=""center"" >修改日期</div></td>" & vbcrlf & "             <td ><div align=""center"" >操作</div></td>" & vbcrlf & "        </tr>" & vbcrlf & "         "
	end if
	dim n
	n=0
	pricemodes = split("先进先出法,个别计价法,全月平均法,移动加权平均法",",")
	set rs=server.CreateObject("adodb.recordset")
	if PriceModeChangeLog=false then
		sql="select * , isnull((select top 1 pricemode from product_log where ord=a.ord and ex_pid=a.ex_pid and mark=1),-1) newPriceMode from product_log a "&Str_Result&" and mark=0 "&px_Result&""
'if PriceModeChangeLog=false then
	else
		sql="select newpricemode newPriceMode,oldpricemode PriceMode,'' title, 0 addcate,modifyuser xgOrd,modifytime xgTime from ProductPriceMode_log where productid="&ord&" "
	end if
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		request("ord")
		Response.write "&action=log&CurrPage="
		...
		do until rs.eof
			...
			k=rs("title")
			cateid=rs("addcate")
			nPriceMode = rs("newPriceMode").value
			if nPriceMode&""="-1" then
				nPriceMode = rs("newPriceMode").value
				nPriceMode = rs("PriceMode").value
				oPriceMode = -1
				nPriceMode = rs("PriceMode").value
			else
				oPriceMode = rs("PriceMode").value
			end if
			if nPriceMode&""="" then nPriceMode = 0
			if oPriceMode &""="" then oPriceMode = 0
			oldPriceMode= ""
			if oPriceMode>=0 then oldPriceMode = pricemodes(oPriceMode)
			newPriceMode = pricemodes(nPriceMode)
			Set rs1=conn.execute("select name from gate where ord="&rs("xgOrd"))
			If rs1.eof Then
				name=""
			else
				name=rs1("name")
			...
			if PriceModeChangeLog=false then
				Response.write "" & vbcrlf & "          <td width=""6%"" align=""center"" class=""red"">"
				if open_21_3=3 or (open_21_3=1 and CheckPurview(intro_21_3,trim(cateid))=True) or open_21_2=3 or (open_21_2=1 AND CheckPurview(intro_21_2,trim(cateid))=True)  then
					Response.write "" & vbcrlf & "            <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
					Response.write rs("id")
					Response.write """>" & vbcrlf & "            "
				end if
				Response.write "</td>" & vbcrlf & "          <td width=""20%"" height=""27"" class=""name"">&nbsp;" & vbcrlf & "            "
				if open_21_14=1 then
					Response.write "" & vbcrlf & "            <a href=""javascript:void(0)"" onclick=""javascript:window.open('content_log.asp?ord="
				...
				Response.write""&k&""
				Response.write "" & vbcrlf & "            </a> </td>" & vbcrlf & "          <td width=""9%"" align=""center"" height=""27"">"
				Response.write rs("order1")
				Response.write "</td>" & vbcrlf & "          <td width=""9%""><div align=""center"">"
				Response.write rs("type1")
				Response.write "</div></td>" & vbcrlf & ""
				if rs("sort1")<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select * from menu where id="&rs("sort1")&""
					rs7.open sql7,conn,1,1
					dim cateid
					if rs7.eof then
						menuname=""
					else
						menuname=rs7("menuname")
					end if
					rs7.close
					set rs7=nothing
				else
				...
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select * from sortonehy where ord in ("&unit&") order by gate1 desc"
				rs1.open sql1,conn,1,1
				dim trade5
				if rs1.eof then
					trade5=""
...
'do until rs1.eof
				...
				rs1.close
				set rs1=Nothing
				Response.write "" & vbcrlf & "          <td width=""7%""><div align=""center""> "
				Response.write trade5
				Response.write " </div></td>" & vbcrlf & "          <td width=""11%""><div align=""center"">"
				Response.write name
				Response.write "</div></td>" & vbcrlf & "          <td width=""11%""><div align=""center"">"
				Response.write rs("xgTime")
				Response.write "</div></td>" & vbcrlf & "          <td class=""func""><div align=""center"">" & vbcrlf & "                 "
				if open_21_14=1 then
					Response.write "" & vbcrlf & "               <input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('content_log.asp?ord="
					Response.write pwurl(rs("id"))
					Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "            "
					Response.write pwurl(rs("id"))
				end if
				isupdate=False
				Set rss=conn.execute("select * from power where qx_open=1 and sort1=21 and sort2=15 and ord="&session("personzbintel2007")&"")
				If Not rss.eof Then
					isupdate=True
				end if
				rss.close
				If isupdate Then
					Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3c"" value=""恢复""  onClick=""if(confirm('确认恢复？')){window.open('correct_reset.asp?ord="
					Response.write pwurl(rs("id"))
					Response.write "','newwincor','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=50,top=50')}""/>" & vbcrlf & "                                <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete.asp?action=log&ord="
					Response.write pwurl(rs("id"))
					Response.write rs("id")
					Response.write "&CurrPage="
					Response.write CurrPage
				...
				Response.write rs("xgTime")
				Response.write "</div></td>" & vbcrlf & "                <td width=""11%""><div align=""center"">"
				Response.write oldPriceMode
				Response.write "</div></td>" & vbcrlf & "                <td width=""11%""><div align=""center"">"
				Response.write newPriceMode
				Response.write "</div></td>" & vbcrlf & "                <td width=""11%""><div align=""center"">"
				Response.write name
				Response.write "</div></td>" & vbcrlf & "            "
			end if
		...
		loop
		Response.write "" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>"
		if PriceModeChangeLog=false then
			Response.write "" & vbcrlf & "          <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "              <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "            </div></td>" & vbcrlf & "          <td width=""40%"">" & vbcrlf & "                       "
			If isupdate Then
				Response.write "" & vbcrlf & "                     <input type=""submit"" value=""批量删除"" class=""anybutton2"" onClick=""return test();""  name=submit1>" & vbcrlf & "                        <input type=""button"" value=""批量恢复"" class=""anybutton2"" onClick=""shareAll();""  name=submit2>" & vbcrlf & "                   "
			end if
			Response.write "" & vbcrlf & "          </td>" & vbcrlf & "            "
		end if
		Response.write "" & vbcrlf & "          </form>" & vbcrlf & "        <script language=""javascript"" type=""text/javascript"">" & vbcrlf & "        function shareAll()" & vbcrlf & "        {" & vbcrlf & "               if(confirm('确认恢复？'))" & vbcrlf & "               {" & vbcrlf & "                       document.all.form1.action = ""correct_reset.asp?action=pl&CurrPage="
		Response.write CurrPage
		Response.write "&Z1="
		Response.write Z1
		Response.write "&Z2="
		...
		Response.write zdy6
		Response.write str33
		Response.write """;" & vbcrlf & "                        document.all.form1.submit();" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        </script>" & vbcrlf & "    <td width=""79%""><div align=""right""> "
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "      <input name=""currpage"" id=""currpage"" type=""text""  onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "      &nbsp;" & vbcrlf & "      <input type=""button"" name=""Submit422"" value=""跳转""  onClick=""gotourl('currPage='+document.getElementById('currpage').value);"" class=""anybutton2""/>" & vbcrlf & "      "
		if currpage=1 then
	...
	action1="产品列表"
	call close_list(1)
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script src= ""../Script/pt_updateLog_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:200;display:none;"">" & vbcrlf & "  <table width=""150"" height=""200""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td height=""139""><table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "               <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按修改时间排序(降)</font></a></td>" & vbcrlf & "    </tr>" & vbcrlf & "          <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按修改时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr  valign=""middle"">"& vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按产品名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按产品名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=5')""><fontcolor=""#2F496E"">按产品编号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按产品编号排序(升)</font></a> </td>" & vbcrlf & "</tr>" & vbcrlf &           "<tr  valign=""middle"">" & vbcrlf &             "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=7')""><font color=""#2F496E"">按产品型号排序(降)</font></a> </td>" & vbcrlf &           "</tr>" & vbcrlf &           "<tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=8')""><font color=""#2F496E"">按产品型号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "  </table>" & vbcrlf & "</div>"& vbcrlf & "</html>"
