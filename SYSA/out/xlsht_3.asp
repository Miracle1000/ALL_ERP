		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					xlApplication.ActiveSheet.Columns(j0).HorizontalAlignment=3
					xlApplication.ActiveSheet.Columns(j0+8).NumberFormatLocal = "#,##0."&saledotstr_num&"_ "
					xlApplication.ActiveSheet.Columns(j0).HorizontalAlignment=3
					xlApplication.ActiveSheet.Columns(j0+9).ColumnWidth=10
					xlApplication.ActiveSheet.Columns(j0).HorizontalAlignment=3
					xlApplication.ActiveSheet.Columns(j0+9).HorizontalAlignment=3
					xlApplication.ActiveSheet.Columns(j0).HorizontalAlignment=3
					xlApplication.ActiveSheet.Columns(j0+9).NumberFormatLocal = "#,##0."&dotstr&"_ "
					xlApplication.ActiveSheet.Columns(j0).HorizontalAlignment=3
...
'if not rs77.eof then
		...
		if rs1.eof then
			...
			do until rs1.eof
				...
				money1=zbcdbl(rs1("money1"))
				if open_5_21<>0 then
					xlWorksheet.Cells(1+i,j2+8).Value = price1
'if open_5_21<>0 then
				else
					xlWorksheet.Cells(1+i,j2+8).Value = ""
'if open_5_21<>0 then
				end if
				xlWorksheet.Cells(1+i,j2+8).font.Size=10
				if open_5_21<>0 then
'if open_5_21<>0 then
					xlWorksheet.Cells(1+i,j2+9).Value = money1
'if open_5_21<>0 then
				else
