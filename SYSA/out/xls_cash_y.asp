...
'if B>0 then
	...
	if m>10 then
		Response.write "<script>alert('显示数据最多不能超过10年');window.location.href='cash_y.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	...
	if intCols1=0 or intCols2=0 then
		Response.write "<script>alert('收入项与支出项至少各保留一项');window.location.href='../tongji/cash_y.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
...
'do while not rs.eof
				...
				For k=0 to m-1
					tempSort1 = ""
					date1=dateadd("yyyy",k,m1)
					if i>0 then
						date1=year(date1)&"-"&month(date1)&"-1"
'if i>0 then
					end if
					date2=year(date1)&"-12-31"
'if i>0 then
...
'For Each key In colsTotal
			...
			doSave("现金流量每年度统计")
				...
				Response.write """><font class=""red""><strong><u>下载导出的现金流量每年度统计</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
