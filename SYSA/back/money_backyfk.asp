...
'If typ=2 Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""setbackyfkall.asp?currPage="
	Response.write currPage
	Response.write """; </script>" & vbcrlf & "<script src='../script/bk_comm01.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write "'></script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">客户退预收款回收站</td>" & vbcrlf & "            <td align=""left"">&nbsp;</td>" & vbcrlf & "                       <td align=""right""><input type=""button"" name=""Submitdel""class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=bankin3&url=money_backyfk.asp&bank=13'}""/></td> "& vbcrlf &         "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td> "& vbcrlf &         "  </tr>"& vbcrlf &    "   </table>  "& vbcrlf &       " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content""> "& vbcrlf &    "  <tr class=""top"">" & vbcrlf &   "  <td width=""40"" align=""center""><div align=""center""><strong>选择</strong></div></td> "& vbcrlf &"<td width=""40"" align=""center"" height=""24""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf &      "     <td><div align=""center"">客户名称</div></td> "& vbcrlf &     "   <td width=""10%""><div align=""center""><strong>退款日期</strong></div></td>" & vbcrlf &    " <td width=""10%""><div align=""center""><strong>退款金额</strong></div></td>" & vbcrlf & "         <td width=""8%""><div align=""center""><strong>负责人</strong></div></td>" & vbcrlf & "       <td><div align=""center""><strong>删除人</strong></div></td>" & vbcrlf & "        <td width=""10%""><div align=""center""><strong>删除日期</strong></div></td>" & vbcrlf& "          <td width=""15%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	dim n , canDelete
	n=0
	canDelete = True
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from bankin3  where del=2 order by deldate desc "
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "       <form name=""form1"" method=""post"" action=""deletebackyfkall.asp?ord="
		...
		do until rs.eof
			...
			if conn.execute("select 1 from bankin2 where id="& bankin2 &" and company="& company &" and del=1 and isnull(money_left,0)>="& money1 ).eof = true then
				str="disabled"
			else
				str=""
			end if
			canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs("id") &"',13, 6, '6002' ").eof
			...
			Response.write "" & vbcrlf & "       </span></td> " & vbcrlf & "    <td align=""center"" height=""24"">"
			Response.write "</td>" & vbcrlf & "    <td width=""28%"" height=""27"" class=""name"" >"
			...
			Response.write "</div></td>" & vbcrlf & ""
			delcate=rs("delcate")
			if delcate="" or Isnull("delcate") then
				delcate=0
			end if
			if delcate<>0 then
				set rs7=server.CreateObject("adodb.recordset")
			...
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">" & vbcrlf & "     <input type=""button"" name=""Submit3c""  class=""anybutton"" "
			Response.write str
			Response.write " value=""恢复""  onClick=""if(confirm('确认恢复？')){window.location.href='setbackyfk.asp?ord="
			Response.write rs("id")
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>" & vbcrlf & "       "
			if canDelete then
				Response.write "<input  type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletebackyfk.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "    <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='money_backyfk.asp?currPage="
			Response.write  1
			Response.write "&A="
			Response.write A
			Response.write "&B="
			...
			Response.write "&page_count="
			Response.write page_count
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='money_backyfk.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "     <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='money_backyfk.asp?currPage="
			Response.write  currpage + 1
			Response.write "&A="
			Response.write A
			Response.write "&B="
			...
			Response.write "&page_count="
			Response.write page_count
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='money_backyfk.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&A="
			Response.write A
			Response.write "&B="
