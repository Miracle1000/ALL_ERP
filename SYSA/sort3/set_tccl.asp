...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Dim sort111, num1
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "<script>" & vbcrlf & "function checkRadio(radio){" & vbcrlf & "    if(radio.id == ""intro2"" && radio.checked == true){" & vbcrlf & "                $(""#hkbl"").css(""display"",""block"");" & vbcrlf & "            $(""#num1"").focus();" & vbcrlf & "       }else{" & vbcrlf & "          $(""#hkbl"").css(""display"",""none"");" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & ""
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		bill.title = "提成发放规则设置"
		bill.ahonegp = False
		bill.groups(0).title = "提成发放规则"
		bill.buttons.getItemByDbName("save").topvisible = false
		bill.buttons.getItemByDbName("reset").topvisible = False
		Dim rs
		sort111 = ""
		Set rs = cn.execute("select intro from setopen where sort1 = 111")
		If rs.eof = False Then
			sort111 = rs("intro")&""
		end if
		rs.close
		set rs = nothing
		num1 = "0"
		Set rs = cn.execute("select num1 from tcbl where sort1 = 10")
		If rs.eof = False Then
			num1 = rs("num1")&""
		end if
		rs.close
	...
	Sub Ahtml(ByVal field)
		sort111 = sort111 &""
		Response.write "" & vbcrlf & "      <div class='sub-field'><div>" & vbcrlf & "    <input type='radio' name='intro' id='intro1' value='1' "
		sort111 = sort111 &""
		If sort111="" Or sort111="1" Then Response.write "checked"
		Response.write " onclick=""checkRadio(this)""><label for='intro1'>按回款额发放</label>" & vbcrlf & "    <span style='margin-left:30px;'><input type='radio' name='intro' id='intro2' value='2' "
		If sort111="" Or sort111="1" Then Response.write "checked"
		If sort111="2" Then Response.write "checked"
		Response.write " onclick=""checkRadio(this)""><label for='intro2'>按回款比例发放</label></span>" & vbcrlf & "   <span style='margin-left:30px;'><input type='radio' name='intro' id='intro3' value='3' "
		If sort111="2" Then Response.write "checked"
		If sort111="3" Then Response.write "checked"
		Response.write " onclick=""checkRadio(this)""><label for='intro3'>按销售目标发放</label></span>" & vbcrlf & "   </div>" & vbcrlf & "  <div id='hkbl' style=""display:"
		If sort111="2" Then Response.write "block" Else Response.write "none"
		Response.write "; height:30x; line-height:30px;"">" & vbcrlf & "          回款比例：<input type=""input"" name=""num1"" id=""num1"" value="""
		If sort111="2" Then Response.write "block" Else Response.write "none"
		Response.write num1
		Response.write """ size=""8"" onkeyup=""checkNumDot('num1',"
		Response.write info.MoneyNumber
		Response.write ")"" onkeypress=""return checkOnlyNum(1)"" onblur='if(this.value==""""){this.value=0}'> % " & vbcrlf & "   </div></div>" & vbcrlf & ""
	end sub
	Function Bill_OnSave(ByVal bill)
		Dim rs, intro, num1
		intro = app.getint("intro")         : num1 = app.getText("num1")
		If num1&""="" Then num1 = 0
		Set rs = cn.execute("select sort1 from setopen where sort1 = 111")
		If rs.eof = False Then
			cn.execute("update setopen set intro = "& intro &" where sort1 = 111")
		else
			cn.execute("insert into setopen (intro, sort1) values("& intro &",111)")
		end if
		rs.close
		set rs = nothing
		If intro = 2 Then
			Set rs = cn.execute("select sort1 from tcbl where sort1 = 10")
			If rs.eof = False Then
				cn.execute("update tcbl set num1 = "& num1 &" where sort1 = 10")
			else
				cn.execute("insert into tcbl (num1,sort1) values("& num1 &",10)")
			end if
			rs.close
			set rs = nothing
		end if
		bill.alert "保存成功"
		bill.showSaveResult2 "","set_tccl.asp"
		Bill_OnSave = True
	end function
