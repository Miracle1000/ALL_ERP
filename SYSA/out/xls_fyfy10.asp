...
'if B="cgmc" then
					elseif B="cgid" then
						str_Result=str_Result+" and cgid like '%"& C &"%' "
'elseif B="cgid" then
					elseif B="khmc" then
...
'if khmc<>"" then
				...
				if cgid<>"" then
					str_Result=str_Result+" and cgid like '%"&cgid&"%' "
'if cgid<>"" then
				end if
				if cgmc<>"" then
					str_Result=str_Result+" and title like '%"&cgmc&"%' "
'if cgmc<>"" then
				end if
...
'if cgmc<>"" then
	...
	n=0
	sql="select * from("&_
	"select caigou.ord,caigou.title,caigou.cgid,caigou.company,caigou.cateid,caigou.sort,"&_
	...
	"(select isnull(sum(money1),0) from paysqlist where del=1 and caigou=caigou.ord and sort1=paytype.id and fid in  "&_
	"(select id from paysq where bz=sortbz.id "&str_Result11&" ) ) as money1,"&_
	"(select isnull(sum(money1),0) from pay where del=1 and complete=3 and sort=paytype.id and  caigou=caigou.ord and fid in "&_
	...
	"(select ord from pay where caigou=caigou.ord "&str_Result44&") and bxid in (select id from paybx where bz=sortbz.id "&str_Result33&" ) ) as money3 "&_
	"from caigou,paytype,sortbz "&_
	"where caigou.del=1 and "&_
	"("&_
	"caigou.ord in (select caigou from paysqlist where del=1) or caigou.ord in (select caigou from pay where del=1 and complete=3)"&_
	")"&_
	"and "&_
	"("&_
	...
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "采购费用明细表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			if rs("company")<>0 then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select *  from tel Where ord="&rs("company")&" "
				rs7.open sql7,conn,1,1
				if not rs7.eof then
			...
			companyname=""
			xlWorksheet.Cells(1+i,4).Value = rs("cgid")
			companyname=""
			...
			companyname=""
			xlWorksheet.Cells(1+i,6).Value = so
			companyname=""
			xlWorksheet.Cells(1+i,6).font.Size=10
			companyname=""
			j=0
			if ZBRuntime.MC(1002) then
				xlWorksheet.Cells(1+i,6+j).Value = companyname
'if ZBRuntime.MC(1002) then
...
'loop
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>采购费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!采购费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("采购费用明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的采购费用明细表</u></strong></font></a></p>" & vbcrlf & ""
