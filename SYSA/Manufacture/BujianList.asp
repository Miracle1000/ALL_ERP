...
'If Right(s,1) = "," Then
	...
	class PrintDBInfoClass
		public RowSql
	end class
	function GetPrintDBInfo(byval keytitle)
		dim obj
		set obj = new  PrintDBInfoClass
		select case lcase(keytitle)
		case "prtyugoudate_showlist" :
		obj.rowSql = Get_YuGou_RowSql
		case "prtyugoucgdate_showlist" :
		obj.rowSql = Get_YuGouCG_RowSql
		Case "prt_ck_info_list" :
		obj.rowSql = Get_StoreOut_RowSql
		Case "prt_product_info_list"
		obj.rowSql = Get_Product_RowSql
		case else
		obj.rowSql = "exec " & keytitle
		end select
		set GetPrintDBInfo = obj
	end function
	function Get_YuGou_RowSql
		Get_YuGou_RowSql = "set nocount on;" & vbcrlf &_
		"if exists(select 1 from tempdb..sysobjects where id=object_id('tempdb..#MyTable')) drop table #MyTable;" & vbcrlf &_
		"create table #MyTable(RowName nvarchar(100),ShowTitle nvarchar(100),showType nvarchar(100),Showorder int);" & vbcrlf &_
		"insert into #MyTable exec PrtYuGouDate_ShowList;"
		if app.power.existsModel(5000) = false then
			Get_YuGou_RowSql = Get_YuGou_RowSql & " delete #MyTable where showType = '关联询价' "
		end if
		if app.power.existsModel(15000) = false then
			Get_YuGou_RowSql = Get_YuGou_RowSql & " delete #MyTable where showType = '关联采购' "
		end if
		if app.power.existsModel(18000) = false or app.power.existsModel(35000) = false then
			Get_YuGou_RowSql = Get_YuGou_RowSql & " delete #MyTable where showType = '关联生产订单' "
		end if
		Get_YuGou_RowSql = Get_YuGou_RowSql & " select * from #MyTable order by Showorder; set nocount off"
	end function
	function Get_YuGouCG_RowSql
		Get_YuGouCG_RowSql = "set nocount on;"&vbcrlf &_
		"if exists(select 1 from tempdb..sysobjects where id=object_id('tempdb..#MyTable')) drop table #MyTable;" & vbcrlf &_
		"create table #MyTable(RowName nvarchar(100),ShowTitle nvarchar(100),showType nvarchar(100),Showorder int);"&vbcrlf &_
		"insert into #MyTable exec PrtYuGouCGDate_ShowList;"
		if app.power.existsModel(18000) = false or app.power.existsModel(35000) = false then
			Get_YuGouCG_RowSql = Get_YuGouCG_RowSql & " delete #MyTable where showType = '关联生产订单' "
		end if
		Get_YuGouCG_RowSql = Get_YuGouCG_RowSql & " select * from #MyTable order by Showorder; set nocount off"
	end function
	Function Get_StoreOut_RowSql
		Get_StoreOut_RowSql = "set nocount on;"&vbcrlf &_
		"if exists(select 1 from tempdb..sysobjects where id=object_id('tempdb..#MyTable')) drop table #MyTable;" & vbcrlf &_
		"create table #MyTable(RowName nvarchar(100),ShowTitle nvarchar(100),showType nvarchar(100),Showorder int);"&vbcrlf &_
		"insert into #MyTable exec prt_ck_info_list;"
		if app.power.existsModel(18000) = false or app.power.existsModel(35000) = false then
			Get_StoreOut_RowSql = Get_StoreOut_RowSql & " delete #MyTable where showType in ('关联补料单','关联领料单') "
		end if
		if app.power.existsModel(16000) = false then
			Get_StoreOut_RowSql = Get_StoreOut_RowSql & " delete #MyTable where showType = '关联采购退货' "
		end if
		if app.power.existsModel(7000) = false then
			Get_StoreOut_RowSql = Get_StoreOut_RowSql & " delete #MyTable where showType = '关联合同' "
		end if
		Get_StoreOut_RowSql = Get_StoreOut_RowSql & " select * from #MyTable order by Showorder; set nocount off"
	end function
	Function Get_Product_RowSql
		Get_Product_RowSql = "set nocount on;"&vbcrlf &_
		"if exists(select 1 from tempdb..sysobjects where id=object_id('tempdb..#MyTable')) drop table #MyTable;" & vbcrlf &_
		"create table #MyTable(RowName nvarchar(100),ShowTitle nvarchar(100),showType nvarchar(100),Showorder int);"&vbcrlf &_
		"insert into #MyTable exec prt_product_info_list;"
		if app.power.existsModel(1002) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '主供应商' "
		end if
		if app.power.existsModel(7000) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '建议售价' or showTitle = '最低售价' "
		end if
		if app.power.existsModel(15000) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '建议进价' or showTitle = '最高进价' "
		end if
		if app.power.existsModel(17000) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '库存上限' or showTitle = '库存下限' "
		end if
		if app.power.existsModel(17009) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '养护周期' "
		end if
		if app.power.existsModel(18000) = false Then
			Get_Product_RowSql = Get_Product_RowSql & " delete #MyTable where showTitle = '生产周期' "
		end if
		Get_Product_RowSql = Get_Product_RowSql & " select * from #MyTable order by Showorder; set nocount off"
	end function
	sub page_load
		Response.write(app.DefHeadHTML(app.virPath,"<LINK rel=stylesheet type=text/css href='"&app.virPath&"skin/default/css/leftmenu.css'>"))
		dim [sort]
		[sort] = request.QueryString("sort")
		Response.write "" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "body{background:none;background-color:#eee;}" & vbcrlf & "</style>" & vbcrlf & "<body class=""menumbody"">" & vbcrlf & "<DIV class=""resetHeadBg"" style=""position: absolute; background: white; height: 100%; overflow: auto; right: 0px; left:0px;background-color:#eee;"" id=menu_mainFrame>" & vbcrlf & "    <div style=""_position:relative; _overflow:auto; width:200px;"">" & vbcrlf & "            <div class=""treeRoot anybutton"" style=""padding-bottom:3px;margin: 10px 0px 8px 10px;line-height:24px;""><img class=""resetElementHidden"" src="""
		[sort] = request.QueryString("sort")
		Response.write app.virPath
		Response.write "skin/default/images/elbow-end-minus.gif"" /><img class=""resetElementShowNoAlign"" style=""display:none;"" src="""
		Response.write app.virPath
		Response.write app.virPath
		Response.write "skin/default/images/MoZihometop/leftNav/expand.png"" /><strong class=""dhtreetop"">打印参数说明</strong></div>" & vbcrlf & "          "
		dim rs, trv, nd0, nd1, rst, formType, dataTitle
		set rs = cn.execute("select a.* from PrintTemplate_Datas a left join PrintTemplate_Type b on a.TemplateType = b.id where b.ord = " & [sort])
		if rs.eof = true then
			Response.write("无私有部件！")
		else
			set trv = new TreeView
			trv.indent = 30
			while rs.eof <> true
				dim PrtDBInfo
				set PrtDBInfo = GetPrintDBInfo(rs("RowsSql"))
				set rst = cn.execute (PrtDBInfo.rowSql)
				if rst.bof =false and rst.eof = false then
					dataTitle = ""
					while rst.eof = false
						if dataTitle <> rst("showType") then
							dataTitle = rst("showType")
							set nd0 = trv.nodes.add()
							nd0.text = dataTitle
							nd0.expand = false
							nd0.canSelect = false
							nd0.ico = app.virPath & "skin/default/images/ico_menu_02.gif"
							nd0.ico2 = app.virPath & "skin/default/images/ico_menu_04.gif"
						end if
						set nd1 = nd0.nodes.add()
						nd1.text = rst("showTitle")
						nd1.value = rst("rowName") & "&|&" & rst("showTitle") & "&|&" & rs("id") & "&|&" & rs("ResolveType")
						rst.movenext
					wend
				end if
				set rst = nothing
			...
			rs.close
			set rs = nothing
			Response.write(trv.html)
			set trv = nothing
		end if
		Response.write "" & vbcrlf & "      </div>" & vbcrlf & "</DIV>" & vbcrlf & "" & vbcrlf & "<script src= ""../Script/Me_BujianList.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """ ></script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "" & vbcrlf & ""
	end sub
