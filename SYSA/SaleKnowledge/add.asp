...
'Set extrafields = GetExtraFields(bill.extra)
	...
	function add_logs(args,action1)
		dim open_rz_system, rs3, sql3, sqlStr, type_login
	...
	Sub Bill_OnPageInit
		Response.write "" & vbcrlf & "       <style>" & vbcrlf & "         body{overflow-x:initial;}" & vbcrlf & "               .edit-body .sub-thead td.fcell{height:40px!important;}/*售后知识库添加-保存按钮*/" & vbcrlf & "               html{padding: 10px 10px 0;background: #efefef;box-sizing:border-box;}" & vbcrlf & "           body{background: #ffffff;}" & vbcrlf & "      </style>" & vbcrlf & " "
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		bill.title = "知识库添加"
		app.addScriptPath("inc/SaleKnowledge.js")
		dim rs, bh
		set rs = cn.execute("EXEC erp_getdjbh 8004,"&Info.user&" ")
		bh=rs(0).value
		set rs=nothing
		dim f
		Set f = Bill.fields.add("标题", "title")
		f.notnull = true
		f.colspan = 2
		f.width = 360
		f.maxlimit = 200
		Set f = Bill.fields.add("编号", "bh")
		f.defvalue = bh
		f.maxlimit = 200
		with Bill.fields.addHtml("知识库分类", "sortID", 1, 1)
		.value = ""
		.dbname = "sortID"
		end with
		Set f = Bill.fields.add("知识库级别", "modeID")
		f.uitype = "select"
		f.source = "sql:select sort1,ord from sortonehy where gate2 = 8004 and del = 1 order by gate1 desc"
		Set f = Bill.fields.add("网址", "url1")
		f.defvalue = "http://"
		f.maxlimit = 200
		with Bill.fields.addHtml("关联客户", "Company", 2, 1)
		.value = ""
		.maxlimit = 100
		.dbname = "Company"
		end with
		with Bill.fields.addHtml("关联售后", "ServiceID", 1, 1)
		.value = ""
		.dbname = "ServiceID"
		end with
		with Bill.fields.addHtml("共享人员", "user_list", 3, 1)
		.value = ""
		.width = 600
		.dbname = "user_list"
		end with
		Dim i : i = 0
		set rs = cn.execute ("select title,name,sort,gl from zdy where sort1 = 8004 and set_open = 1 order by gate1")
		while rs.eof = false
			select case rs("sort")
			case "2"
			...
			f.uitype = "select"
			f.value = ""
			f.source = "sql:select sort1,ord from sortonehy where gate2 = " & rs("gl") & " and del = 1 order by gate1 desc"
			end select
			rs.movenext
			i = i + 1
'end select
			If (i Mod 3) > 0 And rs.eof = True Then
				f.colspan = 3 - (i Mod 3) + 1
'If (i Mod 3) > 0 And rs.eof = True Then
			end if
		wend
		rs.close
		set rs = nothing
		Set f = Bill.fields.add("关键字", "KeyWord")
		f.uitype = "textarea"
		f.colspan = 3
		f.maxlimit = 4000
		Set f = Bill.fields.add("知识库内容", "content")
		f.uitype = "textarea"
		f.colspan = 3
		f.notnull = true
		f.maxlimit = 4000
		Set f = Bill.fields.add("解决方法", "solution")
		f.uitype = "textarea"
		f.colspan = 3
		f.maxlimit = 4000
		Set f = Bill.fields.add("备注", "remark")
		f.uitype = "editor"
		f.colspan = 3
	end sub
	Function Bill_OnSave(ByVal bill)
		dim title, bh, sortID, modeID, Company, ServiceID, user_list, KeyWord, content, solution, remark, url1
		dim zdy1, zdy2, zdy3, zdy4, zdy5, zdy6
		title = app.getText("title")
		bh = app.getText("bh")
		sortID = app.getInt("sortID")
		modeID = app.getInt("modeID")
		Company = app.getInt("Company")
		ServiceID = app.getInt("ServiceID")
		user_list = app.getText("user_list")
		KeyWord = app.getText("KeyWord")
		content = app.getText("content")
		solution = app.getText("solution")
		remark = app.getText("remark")
		url1 = app.getText("url1")
		zdy1 = app.getText("zdy1")
		zdy2 = app.getText("zdy2")
		zdy3 = app.getText("zdy3")
		zdy4 = app.getText("zdy4")
		zdy5 = app.getInt("zdy5")
		zdy6 = app.getInt("zdy6")
		dim rs
		set rs = cn.execute("select COUNT(1) from sale_knowledge where del = 1 and title = '" & replace(title,"'","''") & "'")
		if rs(0) <> 0 and isnull(rs(0)) = false then
			bill.alert "标题【"& title &"】已存在！请重试"
			Bill_OnSave = False
			Exit function
		end if
		rs.close
		set rs = nothing
		if len(bh) > 0 then
			set rs = cn.execute("select COUNT(1) from sale_knowledge where del = 1 and bh = '" & replace(bh,"'","''") & "'")
			if rs(0) <> 0 and isnull(rs(0)) = false then
		...
		set rs = server.CreateObject("adodb.recordset")
		rs.open "select * from sale_knowledge where id = 0 ",cn,3,3
		rs.addnew
		rs("title") = title
		rs("bh") = replace(bh,"'","''")
		rs("modeID") = modeID
		rs("sortID") = sortID
		rs("user_list") = user_list
		rs("content") = [content]
		...
		if isnull(zdy1) = false then
			rs("zdy1") = zdy1
		end if
		if isnull(zdy2) = false then
			rs("zdy2") = zdy2
		end if
		if isnull(zdy3) = false then
			rs("zdy3") = zdy3
		end if
		if isnull(zdy4) = false then
			rs("zdy4") = zdy4
		end if
		if isnull(zdy5) = false then
			rs("zdy5") = zdy5
		end if
		if isnull(zdy6) = false then
			rs("zdy6") = zdy6
		end if
		rs("url1") = url1
		rs("indate") = now()
		...
		rs.close
		set rs = nothing
		call add_logs(1,bill.title)
		bill.showSaveResult2 "知识库添加完成","planall.asp"
		Bill_OnSave = True
	end function
	Function sortID(Byval field)
		Response.write("<div class='sub-field'>")
'Function sortID(Byval field)
		Response.write("<input name='s_" & field.dbname & "_txt' id='s_" & field.dbname & "_txt' type='text' isfield='1' readonly />")
		Response.write("<input name='" & field.dbname & "' type='hidden' isfield='1' />")
		Response.write("&nbsp;<A onclick=""javascript:window.open('inc/TreeSelect.asp?objname=" & field.dbname & "','even3tcom','width=' + 195 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;"" href=""#""><SPAN class=blue2>更改<IMG class=""resetElementHidden"" border=0 src=""../images/jiantou.gif""><IMG class=""resetElementShow"" style=""display:none;"" border=0 src=""../skin/default/images/MoZihometop/content/lvw_addrow_btn.png""></SPAN></A>")
		Response.write("</div>")
	end function
	Function Company(Byval field)
		Response.write("<div class='sub-field'>")
'Function Company(Byval field)
		Response.write("<input name='s_" & field.dbname & "_txt' id='s_" & field.dbname & "_txt' type='text' isfield='1' readonly value='点击选择客户'"_
		&" onclick=""javascript:window.open('inc/SearchCompany.asp?objname=" & field.dbname & "','newwin','width=1000,height=600,fullscreen=no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100');return false;"""_
		&" style='text-align:center;color:#999' />")
		Response.write("<input name='" & field.dbname & "' type='hidden' isfield='1' />")
		Response.write("</div>")
	end function
	Function ServiceID(Byval field)
		Response.write("<div class='sub-field'>")
'Function ServiceID(Byval field)
		Response.write("<input name='s_" & field.dbname & "_txt' id='s_" & field.dbname & "_txt' type='text' isfield='1' readonly value='点击选择售后'"_
		&" onclick=""javascript:window.open('inc/SearchService.asp?ord='+document.getElementsByName('Company')[0].value+'&objname=" & field.dbname & "','newwin121','width=1000,height=600,fullscreen=no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100');return false;"""_
		&" style='text-align:center;color:#999' />")
		Response.write("<input name='" & field.dbname & "' type='hidden' isfield='1' />")
		Response.write("</div>")
	end function
	function user_list(Byval field)
		Response.write("<div class='sub-field'>")
'function user_list(Byval field)
		Response.write("<input name='s_" & field.dbname & "_txt' id='s_" & field.dbname & "_txt' type='text' isfield='1' readonly value='点击选择人员'"_
		&" onclick=""javascript:window.open('inc/SearchShare.asp?objname=" & field.dbname & "','newwin121','width=1000,height=600,fullscreen=no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100');return false;"""_
		&" style='text-align:center;width:600px;;color:#999' />")
		Response.write("<input name='" & field.dbname & "' type='hidden' isfield='1' />")
		Response.write("</div>")
	end function
%>
