...
'if hasNumStr(j)<>"" and hasNumStr(j)<>"0" then
	...
	Response.write "不能为空"");};} );" & vbcrlf & "  " & vbcrlf & "});" & vbcrlf & "//BUG 6634 Sword 2014-12-17 年终奖个税设置，开始区间的值大于结束区间时，建议给出提示，不让保存 " & vbcrlf & "function checkMoney(){" & vbcrlf & "        var lower = $(""#lower"").val();" & vbcrlf & "    var limit = $(""#limit"").val();" & vbcrlf & "    if (lower.length>0 && limit.length>0)" & vbcrlf & "   {" & vbcrlf & "               if (parseFloat(lower)>parseFloat(limit))" & vbcrlf & "                {" & vbcrlf & "                       alert(""结束区间金额不能小于开始区间金额！"");" & vbcrlf & "                      return false;" & vbcrlf & "           }       " & vbcrlf & "        }" & vbcrlf & "       return true ;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function openDisplay(id)" & vbcrlf & "{" & vbcrlf & "        if(id==1)" & vbcrlf & "       {" & vbcrlf & "       $("".ksSet"").show();" & vbcrlf & "       }" & vbcrlf & "       else{" & vbcrlf & "   $("".ksSet"").hide();" & vbcrlf & "       }" & vbcrlf & "" & vbcrlf & "}" & vbcrlf & "function GetTaxQuick(w_sortid,w_lv,w_taxRate)" & vbcrlf & "{" & vbcrlf & "//alert(w_taxRate);" & vbcrlf & "  if(w_sortid!=""""&&w_lv!=""""&&w_taxRate!="""")" & vbcrlf & "     {" & vbcrlf & "               $.get(""GetTaxQuick.asp"",{action:""get"",sortid:w_sortid,lv:w_lv,taxRate:w_taxRate},function(data,html){" & vbcrlf & "               $(""#cut"").val(data.split(""</noscript>"")[1]);" & vbcrlf & "        });"& vbcrlf &" } "& vbcrlf & "else "& vbcrlf & "{" & vbcrlf & "       //alert(""速算扣除数=上一级最高所得额×（本级税率-上一级税率）+上一级速算扣除数,三者缺一不可，否则无法自动计算！"");" & vbcrlf & "}" & vbcrlf &" }" & vbcrlf & "-->" & vbcrlf & "</script> "& vbcrlf & "<style type=""text/css"">" & vbcrlf &" <!--" & vbcrlf & ".open{ display:inline}" & vbcrlf & ".close{ display:none}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body   "
	Response.write inName1
	if open_10_8=0 then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select intro from setopen  where sort1=20190308 "
	rs.open sql,conn,1,1
	if rs.eof then
		sort20190308=""
	else
		sort20190308=rs("intro")
	end if
	rs.close
	set rs=nothing
	if action="update" then
		sortid=trim(SaveRequestForm("sortid"))
		lv=trim(SaveRequestForm("lv"))
		limit=trim(SaveRequestForm("limit"))
		lower=trim(SaveRequestForm("lower"))
		taxRate=trim(SaveRequestForm("taxRate"))
		cut=trim(SaveRequestForm("cut"))
		Dtype=trim(SaveRequestUrl("type"))
		if cut&""="" then cut = 0
		if sortid=""  then call jsBack(inName7&"不能为空")
		if lv="" then call jsBack(inName1&"不能为空")
		if lower="" then call jsBack(inName2&"必须大于0")
		if limit=""  then call jsBack(inName3&"不能为空")
		if taxRate=""  then call jsBack(inName4&"不能为空")
		if cut=""  then call jsBack(inName5&"不能为空")
		set rs=server.CreateObject("adodb.recordset")
		sql="select isnull(IsEnabled,0) as IsEnabled from hr_PersonTaxSort where del=0 and id ="&sortid
		rs.open sql,conn,1,1
		if not rs.eof then
		...
		if Dtype="add" then
			...
			call add_logs(1)
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from hr_PersonTax  where lv="&lv&" and sortid="&sortid&" and del=0"
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "insert into hr_PersonTax (sortid,lv,Limit,[Lower],taxRate,cut,creator,inDate,del,IsEnabled) values("&sortid&","&lv&","&limit&","&lower&","&taxRate&","&cut&","&session("personzbintel2007")&",'"&now()&"',0,"&IsEnabled&")"
				call jsLocat("添加成功","gz_PersonTax.asp?sort1="& sort1)
			else
				call jsBack(TaxLvName(lv)&"已存在")
			end if
			rs.close
			set rs=nothing
		elseif Dtype="save" and isnumeric(ord) then
			...
			call add_logs(1)
			set rs=server.CreateObject("adodb.recordset")
			sql="select * from hr_PersonTax  where lv="&lv&" and sortid="&sortid&" and  id<>"&ord&"  and del=0"
			rs.open sql,conn,1,1
			if rs.eof then
				conn.execute "update hr_PersonTax set sortid="&sortid&",lv="&lv&",Limit="&limit&",[Lower]="&lower&",taxRate="&taxRate&",cut="&cut&",editTime='"&now()&"',IsEnabled = "&IsEnabled&" where id="&ord&" and del=0"
				call jsLocat("修改成功","gz_PersonTax.asp?sort1="& sort1)
			else
				call jsBack(TaxLvName(lv)&"已存在")
			end if
			rs.close
	...
	elseif action="edit" and isnumeric(ord) then
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from hr_PersonTax  where id="&ord&" and del=0"
		rs.open sql,conn,1,1
		if not rs.eof then
			lv=rs("lv")
			Limit=rs("Limit")
	...
	elseif action="del" and isnumeric(ord) then
		action1=taxTitle &"删除"
		call add_logs(2)
		conn.execute "delete hr_PersonTax  where id="&ord&""
		call jsLocat("删除成功","gz_PersonTax.asp?sort1="& sort1)
	end if
	if cut="" or isnull(cut) then cut=0
	if limit="" or isnull(limit) then limit=0
	if lower="" or isnull(lower) then lower=0
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">"
	Response.write action1
	Response.write "</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & ""
	if open_10_13=3 or (open_10_13=1 or CheckPurview(intro_10_1,trim(creator))=True)  or ((open_10_2=3 or CheckPurview(intro_10_2,trim(creator))=True) and ord<>"") then
		Response.write "" & vbcrlf & "            <form method=""post"" name=""demo1""  action=""gz_setopen.asp?"""" >" & vbcrlf & "                <tr class=""top"">" & vbcrlf & "            <td  height=""26"" colspan=""2"" class=""name"">个税累计规则设置</td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr>" & vbcrlf & "            <td colspan=""2"" class=""name"">" & vbcrlf & "                <input name=""sort20190308""  type=""radio""    value=""1"" "
		if sort20190308="1" then
			Response.write "checked"
		end if
		Response.write ">按工资月份累计" & vbcrlf & "                          <input type=""radio""   name=""sort20190308"" "
		if sort20190308="2" then
			Response.write "checked"
		end if
		Response.write "  value=""2"">按实际发放月份累计" & vbcrlf & "             </td>" & vbcrlf & "           </tr> " & vbcrlf & "            <tr>" & vbcrlf & "                <td colspan=""2"" class=""name"">" & vbcrlf & "                              <input name=""button"" class=""anybutton"" value=""保  存""  onClick=""javascript:document.all.demo.action=''"" type=""submit"">" & vbcrlf & "                </td>" & vbcrlf & "            </tr>" & vbcrlf & "" & vbcrlf & "            </form>" & vbcrlf & "" & vbcrlf & "        <form method=""post"" name=""demo""  action=""gz_PersonTax.asp?action=update"" id=""demo"" onsubmit=""return Validator.Validate(this,2) && checkMoney()"" >" & vbcrlf & "          " & vbcrlf & "            <tr class=""top"">" & vbcrlf & "            <td  height=""26"" colspan=""2"" class=""name"">基本设置</td>" & vbcrlf & "          </tr>" & vbcrlf & "                "
		If inName7 = "" Then
			Response.write "" & vbcrlf & "               <input type=""hidden"" name=""sortid"" value="""
			Response.write sort1
			Response.write """><input type=""hidden"" name=""sort1"" value="""
			Response.write sort1
			Response.write """>" & vbcrlf & "                  "
		else
			Response.write "" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
			Response.write inName7
			Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><select name=""sortid"" size=""1"" onchange=""GetTaxQuick(sortid.value,lv.value,taxRate.value)""  id=""sortid"" min=""1"" msg=""必选""  datatype=""Limit"">" & vbcrlf & "                <option value="""">请选择</option>" & vbcrlf & "                "
			sql="select id, title from hr_PersonTaxSort where del=0 order by id asc"
			set rs=conn.execute(sql)
			if not rs.eof then
				do until rs.eof
			...
			rs.close
			set rs=nothing
			Response.write "" & vbcrlf & "              </select>" & vbcrlf & "              <span class=""red""> *</span>       <a href=""perform_tax_class.asp"" style=""color:#FF0000"">点击这里</a>添加分类</td>" & vbcrlf & "          </tr>" & vbcrlf & "                "
		end if
		Response.write "" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName1
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><select name=""lv"" id=""lv"" min=""1"" msg=""必选""  datatype=""Limit"" onchange=""GetTaxQuick(sortid.value,lv.value,taxRate.value)"" >" & vbcrlf & "                <option value="""" >请选择</option>" & vbcrlf & "   "
		for i=1 to lvMax
			Response.write "" & vbcrlf & "                <option value="""
			Response.write i
			Response.write """ "
			...
			Response.write " >"
			Response.write TaxLvName(i)
			Response.write "</option>" & vbcrlf & "                "
		next
		Response.write "" & vbcrlf & "              </select>" & vbcrlf & "              <span class=""red""> *</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "             "
		Response.write "" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName2
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input name=""lower"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('lower','"
		Response.write num_dot_xs
		Response.write "')""  type=""text"" id=""lower""  value="""
		Response.write lower
		Response.write """ size=""23"" maxlength=""14""   dataType=""number"" min=""0""   msg=""必填"">" & vbcrlf & "              (一级下限为0) <span class=""red""> </span> <span class=""red""> *</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName3
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input name=""limit""  type=""text"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('limit','"
		Response.write num_dot_xs
		Response.write "')"" id=""limit""  value="""
		Response.write limit
		Response.write """ size=""23"" maxlength=""14""   dataType=""number"" min=""1""  msg=""必填"">" & vbcrlf & "              (无上限时请填写一个较大的数) <span class=""red""> *</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName4
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><input name=""taxRate"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('limit','"
		Response.write num_dot_xs
		Response.write "');GetTaxQuick(sortid.value,lv.value,taxRate.value)""  type=""text"" id=""taxRate""  value="""
		Response.write taxRate
		Response.write """ size=""15"" maxlength=""50"" onBlur=""GetTaxQuick(sortid.value,lv.value,taxRate.value)""  dataType=""number"" min=""0"" max=""100""  msg=""必填"">" & vbcrlf & "              (%)<span class=""red"">*</span></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">"
		Response.write inName5
		Response.write "：</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name""><p>" & vbcrlf & "                <input name=""cut""  type=""text"" onpropertychange=""formatData(this,'money');"" onKeyUp=""checkDot('limit','"
		Response.write num_dot_xs
		Response.write "')"" id=""cut""  value="""
		Response.write cut
		Response.write """ size=""23"" maxlength=""50""   dataType=""number"" min=""0"" msg=""必填"">" & vbcrlf & "                (元)<span class=""red"">*</span></p></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""16%""  height=""26"" align=""right"" class=""name"">&nbsp;</td>" & vbcrlf & "            <td width=""84%""  height=""26""  class=""name"">"
		if action="edit" then
			Response.write "" & vbcrlf & "              <input name=""button22"" class=""anybutton2"" onClick=""javascript:document.all.demo.action='gz_PersonTax.asp?action=update&type=save&ord="
			Response.write pwurl(ord)
			Response.write "'""  value=""保  存"" type=""submit"">" & vbcrlf & "              "
		else
			Response.write "" & vbcrlf & "              <input name=""button23"" class=""anybutton2"" value=""保  存""  onClick=""javascript:document.all.demo.action='gz_PersonTax.asp?action=update&type=add'"" type=""submit"">" & vbcrlf & "              "
		end if
		Response.write "" & vbcrlf & "              &nbsp;&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "        </form>" & vbcrlf & ""
	end if
	If sort1 = "" Then
		sql="select isnull(IsEnabled,0)isshow,* from hr_PersonTax  where del=0 and sortid>0 order by sortid desc,lv asc"
	ElseIf sort1 = "0" Then
		sql="select isnull(IsEnabled,0)isshow,* from hr_PersonTax  where del=0 and sortid=0 order by lv asc"
	end if
	set rs=server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	if not rs.eof then
		Response.write "" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name"">"
		Response.write taxTitle
		Response.write "列表</td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "          <td  height=""26"" colspan=""2"" class=""name""><table width=""100%"" border=""0"" cellpadding=""4"" cellspacing=""1"" bgcolor=""#C0CCDD"" >" & vbcrlf & "              <tr class=""top"">" & vbcrlf & "                         "
		If inName7&"" <> "" Then
			Response.write "" & vbcrlf & "                <td width=""15%"" align=""center""><div align=""center"">"
			Response.write inName7
			Response.write "</div></td>" & vbcrlf & "                          "
		end if
		Response.write "" & vbcrlf & "                <td width=""13%"" align=""center""><div align=""center"">"
		Response.write inName1
		Response.write "</div></td>" & vbcrlf & "                <td width=""13%"" align=""center""><div align=""center"">"
		Response.write inName2
		Response.write "</div></td>" & vbcrlf & "                <td width=""12%"" align=""center""><div align=""center"">"
		Response.write inName3
		Response.write "</div></td>" & vbcrlf & "                <td width=""12%"" align=""center""><div align=""center"">"
		Response.write inName4
		Response.write "</div></td>" & vbcrlf & "                <td width=""16%"" align=""center""><div align=""center"">"
		Response.write inName5
		Response.write "</div></td>" & vbcrlf & "                <td width=""19%"" align=""center""><div align=""center"">操作</div></td>" & vbcrlf & "              </tr>" & vbcrlf & "              "
		do while not rs.eof
			P_sortid=rs("sortid")
			P_lv=TaxLvName(rs("lv"))
			P_Limit=rs("Limit")
			P_Lower=rs("Lower")
			P_taxRate=rs("taxRate")
			P_cut=rs("cut")
			isshow=rs("isshow")
			if P_cut&""="" then P_cut = 0
			ord=rs("id")
			Response.write "" & vbcrlf & "              <tr>" & vbcrlf & "                "
			If inName7&"" <> "" Then
				Response.write "" & vbcrlf & "                             <td align=""center""><a href=""perform_tax_class.asp?action=edit&ord="
				Response.write pwurl(P_sortid)
				Response.write """ title=""点击查看分类"">"
				Response.write taxSortName(P_sortid)
				Response.write "</a></td>" & vbcrlf & "                            "
			end if
			Response.write "" & vbcrlf & "                <td align=""center"">"
			Response.write P_lv
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write FormatNumber(P_Lower,num_dot_xs,-1,0,0)
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write FormatNumber(P_Limit,num_dot_xs,-1,0,0)
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write FormatNumber(P_taxRate,num_dot_xs,-1,0,0)
			Response.write "</td>" & vbcrlf & "                <td align=""center"">"
			Response.write "%</td>" & vbcrlf & "                <td align=""center"">"
			Response.write FormatNumber(P_cut,num_dot_xs,-1,0,0)
			Response.write "%</td>" & vbcrlf & "                <td align=""center"">"
			...
			if open_10_2=3 or CheckPurview(intro_10_2,trim(rs("creator")))=True then
				Response.write "" & vbcrlf & "<input name=""button3""  value=""修改"" onClick=""javascript:window.location.href='gz_PersonTax.asp?action=edit&ord="
				Response.write pwurl(ord)
				Response.write "&sort1="
				Response.write sort1
				Response.write "'"" type=""button"">"
			end if
			Response.write "&nbsp;"
			if open_10_3=3 and isshow=0  or CheckPurview(intro_10_3,trim(rs("creator")))=True then
				Response.write "<input name=""button32"" class=""anybutton2"" value=""删除"" onClick=""if(confirm('确认删除？')){javascript:window.location.href='gz_PersonTax.asp?action=del&ord="
				Response.write pwurl(ord)
				Response.write "&sort1="
				Response.write sort1
	...
	rs.close
	Set rs=Nothing
	Response.write "" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td height=""30"" ><div align=""center""></div></td>" & vbcrlf & "        </tr>"& vbcrlf &   "    </table></td> "& vbcrlf & "  </tr>" & vbcrlf & "</table> "& vbcrlf &" <script language=""javascript"" type=""text/javascript"">" & vbcrlf &" <!-- "& vbcrlf &" var needWorkClass=parseInt("
	'Set rs=Nothing
