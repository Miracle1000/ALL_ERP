...
function call_Server2(nameitr,ord,id,i) {
   var w  = "tttest" +id;
   w=document.all[w]
   
   var u_name = document.getElementById(nameitr+i).value;
   
   if ((u_name == null) || (u_name == "")) return;
  var url = "../price/cu_lishi.asp?unit=" + escape(u_name)+"&ord="+escape(ord)+"&id="+escape(id)+"&nameitr="+escape("test"+id) + "&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
  xmlHttp.open("GET", url, false);
  xmlHttp.onreadystatechange = function(){
  //alert(w);
  updatePage2(w);
...
function updatePage4(ord,top) {
 ...
 w=document.all[w]
  ...
  var url = "addlistadd_ck.asp?ord="+escape(ord)+"&top="+escape(top);
  xmlHttp.open("GET", url, false);
  xmlHttp.onreadystatechange = function(){
  updatePage5(w);
  };
...
function ck_total(id,num_dot_xs,i) 
{ 
var price= document.getElementById("pricetest"+i); 
var num= document.getElementById("num"+i); 
var moneyall= document.getElementById("moneyall"+i);
var money1=price.value.replace(/\,/g,'') * num.value.replace(/\,/g,''); 
moneyall.value=FormatNumber(money1,num_dot_xs);
}
...
function callServer5(s,nameitr,ord,id) {
  var w  =s ;
   ...
   var u_name = document.getElementById(nameitr).value;
