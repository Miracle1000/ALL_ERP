...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(byval rpt, byval  state)
		...
		ReDim dat(0)
		dat(c) = ""
		Set rs = cn.execute("select * from (select distinct convert(varchar(20),backUpDate,120) as n,convert(varchar(20),backUpDate,120) v from ku_back union all  select '不限' as n, '' as v ) t order by v desc")
		While rs.eof = False
			If fd = "" Then
				fd = rs.fields(0).value  & "=#" & rs.fields(0).value
			else
				ReDim Preserve dat(c)
				dat(c) = rs.fields(0).value  & Chr(1) & rs.fields(0).value
				c = c + 1
				dat(c) = rs.fields(0).value  & Chr(1) & rs.fields(0).value
			end if
			rs.movenext
		...
		Call rpt.addfield2("备份时间：","select", "backtime" , s, "","",fd)
		rpt.addfield "","stores", "cks" , "", "", ""
		Call rpt.addfield2("","select", "keytype" , "产品编号" & Chr(1) & "1" & Chr(2) & "产品型号" & Chr(1) & "2","", "","产品名称=0")
		rpt.addfield "","text", "keytxt" , "", "", ""
		rpt.sql = "exec erp_report_kubacklist @keytype, @keytxt, @cks, @backtime, " & Info.user
	end sub
	sub app_oncreate(byval lvw)
		lvw.currsum = true
		lvw.currsum = true
		lvw.headers(1).dbtype = "str"
		lvw.headers(1).formattext = "code:ProductLinkHTML(@cells[11],""@value"")"
		lvw.headers(2).dbtype = "str"
		lvw.headers(3).dbtype = "str"
		lvw.headers(5).dbtype = "number"
		lvw.headers(6).dbtype = "money"
		lvw.headers(11).display = "none"
		Dim cbPower : cbPower = app.power.existspower(23,2)
		If cbpower = False Then lvw.headers(6).display = "none"
	end sub
