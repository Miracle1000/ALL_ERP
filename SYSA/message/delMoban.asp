...
'If Not rs.Eof Then
	...
	if ord="" or ord=0 then
		...
		if not rs.eof Then
			sqltext="update mobanMessage set del=2 where  ord="&ord&""
			conn.execute(sqltext)
			Response.write"<script language=javascript>alert('删除成功！');window.location.href='listMoban.asp?currpage="&currpage&"';</script>"
			call db_close : Response.end
		else
			Response.write"<script language=javascript>alert('该数据不存在！'); history.back();</script>"
			call db_close : Response.end
		end if
		rs.close
		set rs=nothing
