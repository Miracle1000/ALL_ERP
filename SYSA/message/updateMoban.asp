...
'if strW3<>"" Then
	...
	sharecontact=request("share")
	if ord="" or ord=0 then
		if request("sortid")<>"" and request("messageContent")<>"" then
			conn.execute "insert into mobanMessage(content,sort,addtime,addcate,del,shareinfo,share) values('"&request("messageContent")&"',"&request("sortid")&",'"&now()&"',"&session("personzbintel2007")&",1,'"&W3&"',"&sharecontact&")"
		...
		addcate=""
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select addcate from mobanMessage where del=1 and ord="&ord&" "
		rs1.open sql1,conn,1,1
		if not rs1.eof then
			addcate=rs1("addcate")
		end if
		...
		set rs1=nothing
		set rs=server.CreateObject("adodb.recordset")
		sql="select * from mobanMessage where del=1 and content='"&request("messageContent")&"'  and ord<>"&ord&" "
		rs.open sql,conn,1,1
		if  rs.eof then
			if addcate<>session("personzbintel2007") and session("top1zbintel2007")<>1 then
				sqltext="Update mobanMessage set shareinfo='"&W3&"',share="&sharecontact&", content='"&request("messageContent")&"',sort="&request("sortid")&",editTime='"&now()&"',editcate="&session("personzbintel2007")&" where del=1 and  ord="&ord&""
			else
				sqltext="Update mobanMessage set shareinfo='"&W3&"',share="&sharecontact&", content='"&request("messageContent")&"',sort="&request("sortid")&",editTime='"&now()&"',editcate="&session("personzbintel2007")&" where del=1 and  ord="&ord&""
			end if
