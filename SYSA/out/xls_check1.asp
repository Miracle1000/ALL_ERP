...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=String(num1_dot,"0")
		...
		xlWorksheet.Cells(1,1).Value = "盘点主题"
		xlWorksheet.Cells(1,1).font.Size=10
		xlWorksheet.Cells(1,1).font.bold=true
		xlWorksheet.Cells(1,2).Value = "盘点编号"
		...
		xlWorksheet.Cells(1,19).Value = "明细备注"
		...
		elseif open_101_10=1 then
			list=" and che_addcateid in ("&intro_101_10&")"
		else
		...
		if open_101_1=3 then
		elseif open_101_1=1 then
			Str_Result=Str_Result&" and che_addcateid in ("&intro_101_1&")"
		else
			Str_Result=Str_Result&" and che_addcateid=0"
		end if
		px=request("px")
		...
		j=1
		k=1
		sql="select * from O_kuCheck_his "&Str_Result&" "&Ptr_Result
		set rs = server.CreateObject("adodb.recordset")
		rs.open sql,conn,1,1
		C1=rs.recordcount
		xlApplication.Init "盘点历史列表_"&session("name2006chen")&".xls",C1
		if not rs.eof then
			do until rs.eof
				che_title=rs("che_title")
				che_bh=rs("che_bh")
				che_cateid=rs("che_cateid")
				if che_cateid="" or isnull(che_cateid) then che_cateid=0
				sql8="select  * from gate where ord="&che_cateid
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					che_cateid=rs8("name")
				end if
				set rs8=nothing
				che_time=rs("che_time")
				if rs("che_del")=1 then
					che_del="已盘点"
				elseif rs("che_del")=2 then
					che_del="未盘点"
				end if
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,1).Value =che_title
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,1).font.Size=10
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,2).Value =che_bh
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,2).font.Size=10
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,3).Value = che_cateid
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,3).font.Size=10
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,4).Value = che_time
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,4).font.Size=10
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,5).Value = che_del
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,5).font.Size=10
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,6).Value = che_note
				che_note=rs("che_note")
				xlWorksheet.Cells(1+i,6).font.Size=10
				che_note=rs("che_note")
				sql8="select a.*,b.che_title,b.che_bh,b.che_cateid,b.che_cateid,b.che_addcateid,b.che_addtime,b.che_bh,b.che_time,g.prod_ph,g.prod_xlh,g.prod_sctime,g.prod_ystime,c.pro_name,d.sort1,e.name,f.prod_name,f.prod_bh,f.prod_xh,k.sto_name from O_kuCheList_his a "&_
				...
				" left join gate e on b.che_cateid=e.ord where che_fid="&rs("che_id")
				...
				if not rs8.eof then
					do while not rs8.eof
						...
						xlWorksheet.Cells(2+i,1).Value = "【盘点明细】"
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,2).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,7).Value = rs8("prod_name")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,7).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,8).Value =rs8("prod_bh")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,8).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,9).Value = rs8("prod_xh")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,9).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,10).Value =rs8("sort1")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,10).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,11).Value =zbcdbl(rs8("che_zmnum"))
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,11).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,12).Value =zbcdbl(rs8("che_sjnum"))
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,12).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,13).Value =zbcdbl(rs8("che_yknum"))
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,13).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,14).Value =rs8("sto_name")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,14).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,15).Value =rs8("prod_ph")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,15).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,16).Value =rs8("prod_xlh")
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,16).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,17).Value =prod_sctime
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,17).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,18).Value =prod_ystime
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,18).font.Size=10
						prod_ystime=rs8("prod_ystime")
						xlWorksheet.Cells(2+i,19).Value =rs8("che_intro")
						prod_ystime=rs8("prod_ystime")
		...
		rs.close
		set rs=nothing
		Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>盘点历史列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!盘点历史列表导出成功,共有"&(i-1)&"条记录!"&(j-1)&"条记录"&(k-1)&"条明细!</B>"";</script>"
		set rs=nothing
		Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
		Set fs = CreateObject("Scripting.FileSystemObject")
		tfile=Server.MapPath("盘点历史列表_"&session("name2006chen")&".xls")
		...
		Response.write """><font class=""red""><strong><u>下载导出的盘点历史列表</u></strong></font></a></p>"
