...
function selectCK(imgobj,batflg)
{
	...
	if(batflg==true)
	{
		div.innerHTML = "<iframe src='StoreDlg.asp' frameborder='0' scrolling='no' style='width:100%;height:100%'></iframe>"
	}
	else
	{
		var pid=document.getElementById('ord_'+mi).value;
		var uid=document.getElementById('unit'+mi).value;
		div.innerHTML = "<iframe src='StoreDlg.asp?pid=" + pid  + "&unit=" +  uid +"' frameborder='0' scrolling='no' style='width:100%;height:100%'></iframe>"
	}
	...
	window.currStore =
	{
		text : "" , value : "" , change : function()
		{
			...
			imgobj.parentElement.getElementsByTagName("label")[0].style.cssText="width:80px;height:20px;overflow:hidden;float:left;white-space:nowrap;text-overflow:ellipsis;";
			imgobj.parentElement.getElementsByTagName("label")[0].title=cktext;
			...
			if(batflg==true) {
				$("input[id^=ck]").val(ckvalue);
				var ckLabel = $("input[id^=ck]").siblings("label");
				ckLabel.html(cktext);			
			};
			showBG(0);
		}
