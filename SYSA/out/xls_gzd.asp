...
'dim sqlstr
	...
	Response.clear
	Response.write "" & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>工资表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">工资表导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>" & vbcrlf &  "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出工资表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr>" & vbcrlf & "<td>" & vbcrlf & "<table width=""100%""border=""0"" cellspacing=""0"" cellpadding=""1"" style='border:1px solid #ccc'>" & vbcrlf & "<tr>" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf& "</table>" & vbcrlf & ""
	dim MODULES,ord,i
	...
	j=2
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from sortwages order by gate1 desc,sort1 asc"
	rs7.open sql7,conn,1,1
	...
	xlApplication.ActiveSheet.Columns(j).HorizontalAlignment=3
	set rs1=server.CreateObject("adodb.recordset")
	rs1.open "select title,bz from wages where id="&ord&"",conn,1,1
	title=rs1("title")
	bz=rs1("bz")
	rs1.close
	set rs1=nothing
	...
	rs1.close
	If jf=0 Then
		rs1.open "select id from wages_jj where wages2="&ord&" ",conn,1,1
		if rs1.eof then
			jf=0
		else
			jf=1
	...
	num1=0
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select sort1 from sortwages order by gate1 desc,sort1 asc"
	rs7.open sql7,conn,1,1
	num1=rs7.recordCount
...
'if ZBRuntime.MC(26002) and jf=1 then
	...
	xlWorksheet.Range(xlWorksheet.Cells(1,1),xlWorksheet.Cells(1,num1)).Merge
	...
	xlWorksheet.Cells(2,j).Value = "应发工资"
	xlWorksheet.Cells(2,j).font.Size=10
	xlWorksheet.Cells(2,j).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+1).Value = "应扣工资"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	...
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+2).Value = "实发工资"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	...
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).Value = "备注"
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).font.Size=10
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Cells(2,j+3).font.bold=true
	xlApplication.ActiveSheet.Columns(j).NumberFormatLocal = "#,##0.00_ "
	xlWorksheet.Range(xlWorksheet.Cells(2,1),xlWorksheet.Cells(2,num1)).Borders.LineStyle=1
	Response.Flush
	set rs=server.CreateObject("adodb.recordset")
	If ZBRuntime.MC(39000) Then
	end if
	If open_10_13 = 3 Then
		sqlGate="SELECT ord, name, cateid FROM gate_person WHERE del = 1  "& hrmsql &" "
	else
		If Cstr(intro_10_13) = "0" Then
			sqlGate="SELECT ord, name, cateid FROM gate_person WHERE 1 = 0 "
		else
			sqlGate="SELECT ord, name, cateid FROM gate_person WHERE ord in ("&intro_10_13&") AND  del=1 "& hrmsql &" "
		end if
	end if
	If isOffer Then
		sqlGate = "SELECT ord, name, cateid FROM gate_person "
	end if
	sqlGate = sqlGate & " and ord in (SELECT cateid FROM wageslist WHERE wages = "&ord&" UNION ALL SELECT cateid FROM wageslist_jj WHERE wages2 = "&ord&") "
	sqlGate = sqlGate &" ORDER BY cateid ASC,sorce asc,sorce2 asc,ord asc"
	rs.open sqlGate,conn,1,1
	C1=rs.recordcount
	i=1
	do until rs.eof
		...
		sql2="select * from sortwages order by gate1 desc,sort1 asc"
		rs2.open sql2,conn,1,1
		j=2
		do until rs2.eof
			sql88="select money1 from wageslist Where wages="&ord&" and cateid="&rs("ord")&"  and sort1="&rs2("id")&""
			sql88 = sql88& "AND ("&open_10_10&" = 3 OR ("&open_10_10&" = 1 AND CHARINDEX(','+CAST(cateid AS VARCHAR(20))+',',',"&intro_10_10&",') > 0 ) ) "
			Set rs88 = conn.execute(sql88)
			if rs88.eof then
				money1=0
			else
...
'if rs2("intro")=1 then
			...
			xlWorksheet.Cells(2+i,j).Value = sortbz&Formatnumber(money1,2,-1)
'if rs2("intro")=1 then
...
'if rs2("intro")=1 then
		...
		set rs2=Nothing
		intro = ""
		Set rs2=conn.execute("select MAX(CAST(intro AS VARCHAR(1000))) as intro from wageslist Where wages="&ord&" and cateid="&rs("ord")&" ")
		If rs2.eof=False Then
			intro = rs2("intro").value
		end if
		rs2.close
		...
		if ZBRuntime.MC(26002) and jf=1 then
			...
			sql8="select isnull(sum(money1),0) as money1 from wageslist_jj where wages2="&ord&" and cateid="&rs("ord")&" "
			...
			set rs8=nothing
			xlWorksheet.Cells(2+i,j).value = sortbz&Formatnumber(money_jj,2,-1)
			set rs8=nothing
			xlWorksheet.Cells(2+i,j).font.Size=10
			set rs8=nothing
			j=j+1
		...
		xlWorksheet.Cells(2+i,j).Value = sortbz&Formatnumber(money2,2,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+1).Value = sortbz&Formatnumber(money3,2,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+1).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+2).Value = sortbz&Formatnumber(money4,2,-1)
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+2).font.Size=10
		set rs8=nothing
		xlWorksheet.Cells(2+i,j+3).Value = intro
		set rs8=nothing
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>工资表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!工资表导出成功,共有"&(i-1)&"条工资记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("工资表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的工资表</u></strong></font></a></p>" & vbcrlf & ""
