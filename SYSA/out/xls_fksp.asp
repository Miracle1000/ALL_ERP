...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>收票明细表导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "</HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td class=""place"">收票明细表导出</td>" & vbcrlf & "    <td>&nbsp;</td>" & vbcrlf & "    <td align=""right"">&nbsp;</td>" & vbcrlf & "    <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出收票明细表,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%""border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "  "
	Response.write Application("sys.info.jsver")
	dotstr=""
		for i=1 to num_dot_xs
			...
			if open_8_1=3 then
				list_tj=""
			elseif open_8_1=1 then
				list_tj="and cateid in ("&intro_8_1&") and cateid<>0 "
			else
			...
			xlApplication.ActiveSheet.Columns(13).ColumnWidth=22
			...
			xlWorksheet.Cells(1,4).Value = "采购人员"
...
'if m2="" then
			...
			If A="100" Then  m1=Request("invdate1")
			If A="100" Then  m2=Request("invdate2")
			if Request("invdate11")<>"" then
				str_Result=str_Result+" and date1>='"&Request("invdate11")&"'"
'if Request("invdate11")<>"" then
			end if
			if Request("invdate22")<>"" then
				str_Result=str_Result+" and date1<='"&Request("invdate22")&"'"
'if Request("invdate22")<>"" then
			end if
			if m1<>"" then
				if A="1" then
...
'elseif A="5" then
			...
			If D<>"10" And Len(D)>0 And isnumeric(D) Then
				str_Result=str_Result+" and isnull(tik,0) in ("& D &")"
'If D<>"10" And Len(D)>0 And isnumeric(D) Then
			end if
...
'if W4<>"" Then
	...
	function menuarea2(id1)
		set rsarea=server.CreateObject("adodb.recordset")
		sqlarea="select id1 from menuarea where id="&id1&" "
		rsarea.open sqlarea,conn,1,1
		if rsarea.eof then
			gateord=""
		else
			do until rsarea.eof
				gateord=rsarea("id1")
				gateord=gateord&","
				A2=replace(A2,""&gateord&"","")
				menuarea2(rsarea("id1"))
				rsarea.movenext
			loop
		end if
		rsarea.close
...
'if  A2<>"" then
	...
	if C<>""then
		if B="gysmc" then
			str_Result=str_Result+"and contract in (select ord from PayCaigouProxy where company in (select ord from tel where name like '%"& C &"%'))"
'if B="gysmc" then
		elseif B="cgzt" then
			str_Result=str_Result+"and contract in (select ord from PayCaigouProxy where title like '%"& C &"%')"
'elseif B="cgzt" then
		elseif B="cgid" then
			str_Result=str_Result+"and contract in (select ord from PayCaigouProxy where cgid like '%"& C &"%')"
'elseif B="cgid" then
		ElseIf B="pjtitle" Then
			str_Result=str_Result+" and tikname like '%"& C &"%'"
'ElseIf B="pjtitle" Then
		end if
	end if
	...
	if Request("companyname")<>"" then
		str_Result=str_Result+" and contract in (select ord from PayCaigouProxy where company in (select ord from tel where name like '%"& request("companyname") &"%'))"
'if Request("companyname")<>"" then
	end if
...
'if request("tikname")<>"" then
	...
	if request("contractname")<>"" then
		str_Result=str_Result+" and contract in (select ord from PayCaigouProxy where title like '%"& request("contractname") &"%')"
'if request("contractname")<>"" then
	end if
...
'if request("intro")<>"" then
	...
	if request("bz")<>"" then
		if cint(request("bz"))>0 then
			str_Result=str_Result+" and contract in (select ord from PayCaigouProxy where bz = "&cint(request("bz"))&")"
'if cint(request("bz"))>0 then
		end if
	end if
	Set rst=conn.execute("select qx_open,isnull(qx_intro,'0') as qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=8 and sort2=11 ")
	If Not rst.eof Then
		open_8_11=rst("qx_open")
		intro_8_11=rst("qx_intro")
	else
		open_8_11=0
		intro_8_11=0
	end if
	...
	ElseIf open_8_11=1 Then
		str_Result = str_Result&" and cateid in ("&intro_8_11&")"
	end if
	...
	rst.close:Set rst=Nothing
	set rs=server.CreateObject("adodb.recordset")
	if Request("paydate1")<>"" or Request("paydate2")<>"" or Request("invdate1")<>"" or Request("invdate2")<>"" then
		sql="select * from PayoutPayInv "&Str_Result&" "&px_Result&""
	else
		sql="select * from PayoutPayInv "&Str_Result&" "&px_Result&""
	end if
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "收票明细表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0  then
		...
		do until rs.eof
			if rs("contract")<>"" then
				If rs("cls")="2" Then
					sql7="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=5025 and sort2=14"
					sql8="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=5025 and sort2=1"
					set rs1=server.CreateObject("adodb.recordset")
					sql1="select id,title,gys,14 as bz,fzr from M_OutOrder  where id="&rs("contract")&" and del=0"
					rs1.open sql1,conn,1,1
					if rs1.eof then
						contract=0
						cateid_cg=-1
						contract=0
						contractname="委外单已被删除"
						company=0
						bz = 0
					else
						cateid_cg=rs1("fzr")
						contract=rs1("id")
						contractname="委外："&rs1("title")
						company=rs1("gys")
						bz=rs1("bz")
					end if
					rs1.close
					...
					sql7="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=22 and sort2=14"
					sql8="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=22 and sort2=1"
					set rs1=server.CreateObject("adodb.recordset")
					sql1="select ord,title,company,bz,cateid from caigou  where ord="&rs("contract")&" and del=1"
					rs1.open sql1,conn,1,1
					if rs1.eof then
						contract=0
						cateid_cg=-1
						contract=0
						contractname="采购单已被删除"
						company=0
						bz = 0
					else
						cateid_cg=rs1("cateid")
						contract=rs1("ord")
			...
			set rs1=server.CreateObject("adodb.recordset")
			rs1.open sql7,conn,1,1
			if rs1.eof then
				open_22_5025_14=0
				intro_22_5025_14=0
			else
				open_22_5025_14=rs1("qx_open")
				intro_22_5025_14=rs1("qx_intro")
			end if
			rs1.close:Set rs1=Nothing
			set rs1=server.CreateObject("adodb.recordset")
			rs1.open sql8,conn,1,1
			if rs1.eof then
				open_22_5025_1=0
				intro_22_5025_1=0
			else
				open_22_5025_1=rs1("qx_open")
				intro_22_5025_1=rs1("qx_intro")
			end if
			rs1.close:Set rs1=nothing
			if isnul(bz) then
				bz = 0
			...
			if bz<>0 then
				bzstr = getrsval("select  top 1 sort1 from  sortbz  where id =  "&bz)
			end if
			if company<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select ord,name,cateid from tel where ord="&company&" and del=1"
				rs1.open sql1,conn,1,1
				if rs1.eof then
					company=0
					cateid_gys=-1
			...
			date3=rs("date3")
			dim gatearray
			gatearray = getgatearray(rs("cateid"))
			if isarray(gatearray) then
				catename = gatearray(0,0)
				sorce1cn = gatearray(1,0)
				sorce2cn = gatearray(2,0)
			end if
			sql7="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=26 and sort2=14"
			sql8="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=26 and sort2=1"
			set rs1=server.CreateObject("adodb.recordset")
			rs1.open sql7,conn,1,1
			if rs1.eof then
				open_26_14=0
...
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,2).Value = sorce1cn
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,2).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,3).Value = sorce2cn
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,3).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,4).Value = catename
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,4).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,5).Value = bzstr
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,5).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,6).Value = Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
...
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,8).Value = tik
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,8).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,9).Value = rs("tikname")
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,9).font.Size=10
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			xlWorksheet.Cells(1+i,9).HorizontalAlignment=1
'If open_26_1=3 Or (open_26_1=1 And CheckPurview(intro_26_1,trim(cateid_gys))=True) Then
			If open_22_5025_1=3 Or (open_22_5025_1=1 And CheckPurview(intro_22_5025_1,trim(cateid_cg))=True) Then
				xlWorksheet.Cells(1+i,10).Value = contractname
'If open_22_5025_1=3 Or (open_22_5025_1=1 And CheckPurview(intro_22_5025_1,trim(cateid_cg))=True) Then
			else
				xlWorksheet.Cells(1+i,10).Value = ""
'If open_22_5025_1=3 Or (open_22_5025_1=1 And CheckPurview(intro_22_5025_1,trim(cateid_cg))=True) Then
			end if
...
'If open_22_5025_1=3 Or (open_22_5025_1=1 And CheckPurview(intro_22_5025_1,trim(cateid_cg))=True) Then
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>收票明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!收票明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("收票明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的收票明细表</u></strong></font></a></p> " & vbcrlf & ""
