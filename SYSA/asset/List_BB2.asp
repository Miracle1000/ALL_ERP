...
'function IsTime(values)
	...
	Str_Result=" where D_del=1 "
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by convert(varchar(10),a.D_addtime,120) desc,a.ass_jttime desc"
	elseif px="2" then
		Ptr_Result=" order by convert(varchar(10),a.D_addtime,120) asc,a.ass_jttime asc"
	elseif px="3" then
		Ptr_Result=" order by a.D_time desc"
	elseif px="4" then
		Ptr_Result=" order by a.D_time asc"
	elseif px="5" then
		Ptr_Result=" order by b.ass_lytime desc"
	elseif px="6" then
		Ptr_Result=" order by b.ass_lytime asc"
	end if
	ret=ChangeDate(ret,1)
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',D_addtime)>=0 "
'if ret<>"" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,D_addtime,'"&ret2&"')>=0 "
'if ret2<>"" then
	end if
	Str_Result2 = ""
	qjret = request("qjret")
	if qjret<>"" then
		Str_Result2=Str_Result2+" and  datediff(m,'"& qjret & "-01" &"',a.ass_jttime)>=0 "
'if qjret<>"" then
	end if
	qjret2 = request("qjret2")
	if qjret2<>"" then
		Str_Result2=Str_Result2+" and datediff(m,a.ass_jttime,'"& qjret2 &"-01" &"')>=0 "
'if qjret2<>"" then
	end if
	Str_Result=Str_Result+Str_Result2
'if qjret2<>"" then
	if C<>"" then
		if B="name" then
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_BB2.asp"" method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "               <table width=""100%""border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf &  "           <td class=""place"">计提折旧列表</td>" & vbcrlf &   "               <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">        " & vbcrlf & "                <tr>" & vbcrlf & "              <td colspan=""9"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "           <span id=""kh"">计提期间：<input type=""text"" name=""qjret"" id=""qjret"" Class=""DatePick""  style=""width:70px"" value="""
	currpage=clng(currpage)
	Response.write qjret
	Response.write """ onclick=""setmonth(this)"" readonly=""readonly"">&nbsp;至：<input type=""text"" name=""qjret2"" id=""qjret2"" Class=""DatePick""  value="""
	Response.write qjret2
	Response.write """ style=""width:70px"" onclick=""setmonth(this)""  readonly=""readonly"">" & vbcrlf & "             <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                                <option value=""name"" "
	...
	if open_105_10=3 or (open_105_10=1 and CheckPurview(intro_105_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if (confirm('确认导出为EXCEL文档？')) { exportExcel({ from: 'form_with_page_action', page: '../out/xls_List_BB2.asp?px="
		Response.write px
	...
	if open_105_13<>0 then
		Response.write "<input type=""button"" name=""Submit43"" value=""折旧核算"" onClick=""if (confirm('确认折旧核算？')) { window.location.href = '../asset/zj.asp?px="
		Response.write px
		Response.write "&lie_1="
		Response.write lie_1
		Response.write "&lie_2="
	...
	Response.write "" & vbcrlf & "                     </span></td>" & vbcrlf & "            </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产名称</div></td>" & vbcrlf & "             <td width=""14%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产分类</div></td>" & vbcrlf & "                <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">资产购价</div></td>" & vbcrlf & "                  <td width=""12%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">已提折旧</div></td>" & vbcrlf & "                <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">净残值</div></td>" & vbcrlf & "           <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">计提期间</div></td>" & vbcrlf & "             <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">本期折旧</div></td>" & vbcrlf & "                <td width=""18%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                        <option value=""page_count=10"" "
	...
	currpage=Request("currpage")
	if currpage<="0" or currpage="" or isnull(currpage) then currpage=1
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select (case when d.ass_jttime>a.ass_jttime then 0 else 1 end) as isdel,(case when jtzjs.zjs=b.ass_cycle then '0' else '1' end) as isZj,"&_
	"   (case when e.ass_jttime=a.ass_jttime then 1 else 0 end) as isold,a.*,"&_
	"   convert(varchar(7),a.ass_jttime,120) as jttime,"&_
	"   a.D_assID,b.ass_name,b.ass_cycle,b.ass_money,b.ass_money2,b.ass_jcz,b.ass_ycycle,b.ass_addcateid,c.sort1 , a.IsCostCollect "&_
	"from O_assDeprect a "&_
	"left join O_asset b on a.D_assID=b.ass_id "&_
	"left join O_assSetname c on b.ass_type=c.id "&_
	"left join (select MAX(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del=1 group by d_assid) d on d.D_assID = a.D_assID "&_
	"left join (select count(1) as zjs,D_assID from O_assDeprect where D_del=2 group by D_assID) as jtzjs on jtzjs.D_assId=b.ass_id "&_
	"left join (select min(ass_jttime) as ass_jttime,d_assid from O_assDeprect where D_del=1 group by d_assid) e on e.D_assID = a.D_assID "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		...
		do while not rs7.eof
			...
			if open_105_3=3 or CheckPurview(intro_105_3,trim(addcateid))=True then
				...
				Response.write """>" & vbcrlf & "                         <input name=""no_del"" type=""hidden"" value="""
				If rs7("isdel") = 0 Then:Response.write rs7("id"):Else:Response.write 0:End if
			...
			Response.write "</div></td>" & vbcrlf & "             <td width=""12%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write Formatnumber(ml.IsNum(rs7("ass_money")),num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "             <td width=""12%"" align=""left"" valign=""middle""><div align=""center"">"
			Response.write Formatnumber(ml.IsNum(rs7("ass_money2")),num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write Formatnumber(ml.IsNum(rs7("ass_jcz")),num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "" & vbcrlf & "           <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write rs7("jttime")
			Response.write "</div></td>" & vbcrlf & "" & vbcrlf & "             <td align=""left"" valign=""middle""><div align=""center"">"
			Response.write Formatnumber(rs7("D_ymoney"),num_dot_xs,-1)
			Response.write "</div></td>" & vbcrlf & "             <td width=""18%"" height=""30"" align=""left"" valign=""middle""><div align=""center"">" & vbcrlf & "             "
			canDepreciation = true
			if rs7("jttime")&""<>"" then
				ADID = sdk.getSqlValue("select id from O_AssDpt where date1='" & rs7("jttime") & "-01'")
'if rs7("jttime")&""<>"" then
				canDepreciation = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & ADID &"',0, 14, '14001' ").eof
			end if
			if (open_105_2=3 or CheckPurview(intro_105_2,trim(addcateid))=True) And rs7("isold")=1 And rs7("isZj")=1 and canDepreciation then
				Response.write "" & vbcrlf & "                <input name=""button5"" type=""button""  class=""anybutton2"" id=""submit5"" onClick=""if(confirm('确认折旧吗？')){window.location=('Dep_assetBB2.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				...
				Response.write "&zdy9="
				Response.write ml.Dep(zdy9)
				Response.write "')}"" value=""折旧""/>" & vbcrlf & "                  "
			end if
			if open_105_14=3 or CheckPurview(intro_105_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""button3"" type=""button""  class=""anybutton2"" id=""submit3"" onClick=""javascript:window.open('Detail_assetBB2.asp?ord="
				Response.write rs7("id")
			...
			canDelete = conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs7("id") &"',105, 14, '' ").eof
			if (open_105_3=3 or CheckPurview(intro_105_3,trim(addcateid))=True) And rs7("isdel")=1 and canDelete and rs7("IsCostCollect").value=0  then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""anybutton2"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_assetBB2.asp?ord="
				Response.write rs7("id")
				Response.write "&px="
				Response.write ml.Dep(px)
				Response.write "&A="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200px; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "     <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "      <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按折旧时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按折旧时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按领用时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按领用时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="计提折旧列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<div id=""adsDiv"" style=""position:absolute;z-index:2001;display:none;padding:2px 2px 2px 2px;background-color:lightblue"">" & vbcrlf & "<iframe src="""" name=""adsIF"" id=""adsIF"" width=""195px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & "</div>" & vbcrlf & "<form target=""ssIF"" action=""../asset/Search_All_Search.asp"" id=""ssIFForm"" style=""display:none"" method=""post"">" & vbcrlf & "</form>" & vbcrlf & "<script language=javascript>" & vbcrlf & "function adSearch(obj,sel_type)" & vbcrlf & "{" & vbcrlf & "   " & vbcrlf & "        var tleft=obj.offsetLeft;" & vbcrlf & "     var ttop=obj.offsetTop;" & vbcrlf & " var theight=obj.clientHeight;" & vbcrlf & "   while (obj = obj.offsetParent){ttop+=obj.offsetTop; tleft+=obj.offsetLeft;}" & vbcrlf & "     var sdivobj=document.getElementById(""adsDiv"");" & vbcrlf & "    sdivobj.style.display=""none"";" & vbcrlf &" sdivobj.style.left=tleft+""px"";" & vbcrlf & "    sdivobj.style.top=ttop+theight+""px"";" & vbcrlf & "      document.getElementById(""ssIFForm"").action=""../asset/Search_All_Search.asp?sel_type=""+sel_type;" & vbcrlf & "     document.getElementById(""ssIFForm"").target=""adsIF"";" & vbcrlf & " document.getElementById(""ssIFForm"").submit();" & vbcrlf & "      sdivobj.style.display=""inline"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "  }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & " if(!confirm('确认删除吗？')) return;" & vbcrlf & "    document.getElementById('data').action=""Del_assetBB2.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认折旧吗？')) return;" & vbcrlf & "    document.getElementById('data').action=""Dep_assetBB2.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
