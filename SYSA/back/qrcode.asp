...
'items = rpt.Settings.item(i)
	...
	sub onReportInit(byval rpt, byval  state)
		rpt.title = "二维码回收站"
		rpt.sql = "exec erp_qrcode_backlist " & Info.User & ", 0, 0, '', 0"
		rpt.BillListUIModel = True
		rpt.nosearchBar = true
		rpt.canprint = False
		rpt.canexcel = false
		rpt.buttonsHTML = "<button class=oldbutton3 onclick='if(window.confirm(""确定要全部清空吗？"")){ajax.regEvent(""cdisposeallItem"");ajax.exec()}'>全部清空</button>"
		rpt.batbuttons = "批量删除,批量恢复"
	end sub
	...
	Function handleHtml(data)
		...
		html = html & "<button " & disable & " class='oldbutton' onclick='if(window.confirm(""确定要恢复吗？"")){ajax.regEvent(""rollbackItem"");ajax.addParam(""id""," & id & ");ajax.exec()}'>恢复</button>&nbsp;"
		html = html & "<button class='oldbutton3' onclick='if(window.confirm(""确定要彻底删除吗？"")){ajax.regEvent(""delItem"");ajax.addParam(""id""," & id & ");ajax.exec()}'>彻底删除</button>"
		handleHtml = html
	end function
	Sub App_delItem
		Dim id : id = app.getInt("id")
		cn.execute "delete C2_CodeItemsFields where codeid in (" & id & ")"
		cn.execute "delete C2_CodeItems  where id=" & id & " and del=2"
		Response.write "window.DoRefresh()"
	end sub
	Sub App_cdisposeallItem
		cn.execute "delete C2_CodeItems  where del=2; delete C2_CodeItemsFields where codeid not in (select id from C2_CodeItems)"
		Response.write "window.DoRefresh()"
	end sub
	Sub App_rollbackItem
		Dim id : id = app.getInt("id")
		cn.execute "update C2_CodeItems set del=1  where id=" & id
		Response.write "window.DoRefresh()"
	end sub
	Sub App_doBatHandle(cmd, checkvalues, batResult)
		...
		Case "批量恢复"
		Call doBatRollback(checkvalues, batResult)
		End select
	end sub
	Sub doBatDelete(ByVal checkvalues, ByVal batResult)
		cn.execute "delete from C2_CodeItems where id in (" & checkvalues & ") and del=2; delete C2_CodeItemsFields where codeid not in (select id from C2_CodeItems)"
	end sub
	Sub doBatRollback(ByVal checkvalues, ByVal batResult)
		cn.execute   " update a set del = 1 from C2_CodeItems a " &_
		" inner join ( " &_
		" select MAX(id) as id from " &_
		" (select id,ctype,source,sourceID,del from C2_CodeItems y where y.id in (" & checkvalues & ") " &_
		"  and not exists(select 1 from C2_CodeItems z where y.ctype = z.ctype and y.source = z.source and y.sourceID = z.sourceID and z.sourceID >0 and z.del = 1) " &_
		" and del = 2) x " &_
		" group by ctype,source,case when sourceID > 0 then sourceID else id end,del " &_
		" ) b on a.id = b.id "
	end sub
