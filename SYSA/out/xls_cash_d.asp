...
'elseif q=4 then
	...
	Response.write " " & vbcrlf & "<HTML>" & vbcrlf & "<HEAD>" & vbcrlf & "<TITLE>现金流量每日统计导出</TITLE>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8""><style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      background-color: #FFFFFF;" & vbcrlf & "      margin-left: 0px;" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        margin-right: 0px;" & vbcrlf & "      margin-bottom: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style></HEAD>" & vbcrlf & "<body>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">现金流量每日统计导出</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "</table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1>&nbsp;</td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & "<span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt; color:#008040"">" & vbcrlf & "<B>正在导出现金流量每日统计,请稍后...</B></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""1"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=000000>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""1"">" & vbcrlf & "<tr> " & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/tiao.jpg"" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""></td></tr></table>" & vbcrlf & "</td></tr></table> <span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "<tr class=""top"">" & vbcrlf & "  <td colspan=2 class=tablebody1 height=""40"">" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	m1=Conver(request("ret"))
	m2=Conver(request("ret2"))
	B=getInt(request("B"))
	getAdv=request("setBRow")
	If getAdv="true" Then
		B= Request("asearch_f_seachBZSelect")
		m1= Conver(Request("advTime1"))
		m2= Conver(Request("advTime2"))
		Worker_ = asearch.GetGates(1)
		If Worker_<>""  Then getCate=Worker_
		If B<>"" and B<>"0"  Then getBzSql=" and c.id="&B
		If getCate&""<>"" And getCate&""<>"0" Then str_Result=str_Result+" and a.cateid="& getCate &" "
'If B<>"" and B<>"0"  Then getBzSql=" and c.id="&B
	end if
...
'If not isNumeric(B) Then
	...
	w=weekday(date)
	mon=DateAdd("ww",0,date)
	if m1="" then
		m1=DateAdd("d",1-w,mon)
'if m1="" then
	end if
	if m2="" then
		m2=DateAdd("d",6,m1)
	end if
	m=datediff("d",m1,m2)+1
	m2=DateAdd("d",6,m1)
	if B>0 then
...
'if B>0 then
	...
	if m>32 then
		Response.write "<script>alert('日报表最多允许单个自然月的天数!');window.location.href='../tongji/cash_d.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	...
	qxSort1 = "6,7,8,9,10,11,76,5025,5026"
...
'Case 10
	...
	strSql="select count(*) as incount from zdymx where sort1=44 and set_open=1 and kd2="&curCate&" and sorce=1 and sort in("&sort_result&")"
	intCols1=conn.execute(strSql)(0)
	strSql="select count(*) as incount from zdymx where sort1=44 and set_open=1 and kd2="&curCate&" and sorce=0 and sort in("&sort_result&")"
	intCols2=conn.execute(strSql)(0)
	if intCols1=0 or intCols2=0 then
		Response.write "<script>alert('收入项与支出项至少各保留一项');window.location.href='../tongji/cash_d.asp';</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	dotstr=""
		for i=1 to num_dot_xs
			...
			Set xlWorksheet = xlsApp.sheets.Add("sheet1")
			xlWorksheet.WriteHtmlCell "<center><b>币种</b></center>" , "rowspan=2 width=80 "
			xlWorksheet.WriteHtmlCell "<center><b>汇率</b></center>" , "rowspan=2 width=60 "
			xlWorksheet.WriteHtmlCell "<center><b>日期</b></center>" , "rowspan=2 width=120 "
			xlWorksheet.WriteHtmlCell  "<center><b>收入项</b></center>", "colspan=" & intCols1
			xlWorksheet.WriteHtmlCell  "<center><b>支出项</b></center>", "colspan=" & intCols2
			xlWorksheet.WriteHtmlCell  "<center><b>合计</b></center>", "rowspan=2 width=80"
			xlWorksheet.movenext
			i=4
			strSql= "select * from zdymx where sort1=44 and set_open=1 and kd2="&curCate&" and sort in("&sort_result&") order by gate1 asc"
			set rs=server.CreateObject("adodb.recordset")
			rs.open strSql,conn
			do while not rs.eof
				xlWorksheet.WriteHtmlCell "<center><b>"& rs("title") &"</b></center>" , "width=120 "
				i=i+1
				xlWorksheet.WriteHtmlCell "<center><b>"& rs("title") &"</b></center>" , "width=120 "
				rs.movenext
			loop
			rs.close
			xlWorksheet.movenext
			Set colsTotal=Server.createObject("Scripting.Dictionary")
			set rs1=server.CreateObject("adodb.recordset")
			allExpensesTotal=0
			allIncomeTotal=0
			Dim BzOpen,BzSql
			...
			RsBz.close
			Set RsBz=Nothing
			strSql = "select c.id, c.sort1 from #cash a WITH(NOLOCK) "&_
			"  inner join sortbank b WITH(NOLOCK) on a.bank=b.id "&_
			"  inner join sortbz c WITH(NOLOCK) on c.id=b.bz " & str_Result & getBzSql &_
			"  group by c.id,c.sort1 order by c.id"
			If Not BzOpen Then strSql="select id,sort1 from sortbz where sort1='人民币'"
			rs.open strSql,conn,1,1
			C1=rs.recordcount*(m+1)
			rs.open strSql,conn,1,1
			i=1
			r=2
			t=0
			do while not rs.eof
				rowspanNum = (2+t+(t+1)*m) - (3+t+t*m) + 1
'do while not rs.eof
				tempSort1 = ""
				t=t+1
				tempSort1 = ""
				For k=0 to m-1
					tempSort1 = ""
					date1=dateadd("w",k,m1)
					rs1.open "select hl from hl where bz="&rs("id")&" and date1='"&date1&"' ",conn,1,1
					if not rs1.eof then
						hl=rs1(0)
					else
						hl=1
					...
					rs1.close
					sort1 =  rs("sort1")
					If tempSort1 <> sort1 Then
						tempSort1 = sort1
						xlWorksheet.WriteHtmlCell "<center>"& rs("sort1") &"</center>" , "rowspan=" & rowspanNum
					end if
					xlWorksheet.WriteNumber hl
					xlWorksheet.WriteDate date1
					strSql = "select isnull(money1,0) money1,isnull(money2,0) money2,c.sorce,c.sort "&_
					"  from zdymx c  "&_
					"  left join( "&_
					"          select sum(a.money1) money1,sum(a.money2) money2,a.sort from #cash a  "&_
					"          inner join sortbank b on a.bank=b.id and a.sort>0 and b.bz="&rs("id") &"  and datediff(dd,a.date1,'"&date1&"')=0 "&_
					"          inner join sortbz c WITH(NOLOCK) on c.id=b.bz " & str_Result &_
					"          group by a.sort "&_
					"  ) t on c.sort=t.sort where c.sort1=44 and c.set_open=1 and c.kd2="&curCate &_
					"  and c.sort in("&sort_result&")  order by c.gate1"
					...
					do while not rs1.eof
						if rs1("sorce")=1 then
							...
							allIncomeTotal=allIncomeTotal+cdbl(money1)*cdbl(hl)
							money1=zbcdbl(rs1("money1"))
						elseif rs1("sorce")=0 then
							money1=-zbcdbl(rs1("money2"))
'elseif rs1("sorce")=0 then
							allExpensesTotal=allExpensesTotal+abs(cdbl(money1))*cdbl(hl)
'elseif rs1("sorce")=0 then
						end if
						xlWorksheet.writemoney abs(money1)
						p=p+1
						xlWorksheet.writemoney abs(money1)
						Call ClientClosedExit
						Response.write "<script>CountImage.width=" & Fix((i/C1) * 710) & ";" & VbCrLf
						Response.write "CountTXT.innerHTML=""共有<font color=red><b>"&C1&"</b></font>条数据!导出进度:<font color=red><b>" & Clng(FormatNumber(i/C1*100,4,-1)) & "</b></font>"";" & VbCrLf
						Response.write "<script>CountImage.width=" & Fix((i/C1) * 710) & ";" & VbCrLf
						Response.write "CountImage.title=""正在处理数据,请稍后..."";</script>" & VbCrLf
						Response.Flush
						sumRowsMoney=sumRowsMoney+money1
						Response.Flush
						if colsTotal.Exists(cstr(rs1("sort"))) then
							val=colsTotal(cstr(rs1("sort")))
							colsTotal(cstr(rs1("sort")))=cdbl(val)+cdbl(money1)
							val=colsTotal(cstr(rs1("sort")))
						else
							colsTotal.Add cstr(rs1("sort")),cstr(money1)
						end if
						rs1.movenext
					loop
					rs1.close
					xlWorksheet.writemoney sumRowsMoney
					xlWorksheet.movenext
					i=i+1
					xlWorksheet.movenext
				next
				xlWorksheet.writehtmlcell "<center><b>列合计</b></center>", "colspan=3"
				sumColsMoney=0
				p=4
				For Each key In colsTotal
					sumColsMoney=sumColsMoney+colsTotal(cstr(key))
'For Each key In colsTotal
					xlWorksheet.writemoney abs(colsTotal(cstr(key)))
					p=p+1
					xlWorksheet.writemoney abs(colsTotal(cstr(key)))
				next
				xlWorksheet.writemoney sumColsMoney
				Call ClientClosedExit
			...
			set rs=Nothing
			set rs1=Nothing
			xlWorksheet.writehtmlcell "<center><b>收入合计</b></center>", ""
			xlWorksheet.writemoney allIncomeTotal
			xlWorksheet.movenext
			xlWorksheet.writehtmlcell "<center><b>支出合计</b></center>", ""
			xlWorksheet.writemoney allExpensesTotal
			xlWorksheet.movenext
			reMains=allIncomeTotal-allExpensesTotal
			xlWorksheet.movenext
			xlWorksheet.writehtmlcell "<center><b>盈亏合计</b></center>", ""
			xlWorksheet.writemoney reMains
			doSave("现金流量每日统计")
				set colsTotal=nothing
				action1="现金流量每日统计导出"
				call close_list(1)
				Response.write " " & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
				Response.write tfile
				Response.write """><font class=""red""><strong><u>下载导出的现金流量每日统计</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
