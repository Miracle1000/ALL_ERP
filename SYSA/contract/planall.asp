...
'Call GetCode128(Mid(Data, i, 2), ID, CodinBin, EAN128)
	...
	Function getDesignTypeList(sourceType , designtype , isNet)
		Dim DSource : DSource = ""
		Dim islist : islist = false
		if sourceType = "list" Then islist = true
		Dim hasChance : hasChance = (ZBRuntime.MC(3000) And app.power.existsPower(3,19))
		If hasChance And (islist = False Or (islist = True And InStr(","&designtype&",",",1,")>0)) Then DSource = "项目=1"
		Dim hasPrice:hasPrice = (ZBRuntime.MC(4000) And app.power.existsPower(4,19))
		If hasPrice And (islist = False Or (islist = True And InStr(","&designtype&",",",3,")>0)) Then DSource = DSource & app.iif(Len(DSource)>0 ,";" , "") & "报价=3"
		Dim hasContract:hasContract = (ZBRuntime.MC(7000) And app.power.existsPower(5,19))
		If hasContract And (islist = False Or (islist = True And InStr(","&designtype&",",",2,")>0)) Then DSource = DSource & app.iif(Len(DSource)>0 ,";" , "")& "合同=2"
		Dim hasYcd:hasYcd = (ZBRuntime.MC(18100) And app.power.existsPower(52,19))
		If hasYcd And (islist = False Or (isNet=False And islist = True And InStr(","&designtype&",",",4,")>0)) Then
			DSource = DSource& app.iif(Len(DSource)>0 ,";" , "") & "预测单=4"
		end if
		If hasYcd And (islist = False Or ( isNet=true And islist = True And InStr(","&designtype&",",",52001,")>0)) Then
			DSource = DSource& app.iif(Len(DSource)>0 ,";" , "") & "预生产计划=52001"
		end if
		Dim hasSCPlan:hasSCPlan = (ZBRuntime.MC(18200) And app.power.existsPower(50,19))
		If  Application("sys.info.systemtype")<>3 and hasSCPlan And (islist = False Or (isNet=False And islist = True And InStr(","&designtype&",",",5,")>0)) Then
			DSource = DSource& app.iif(Len(DSource)>0 ,";" , "")&"生产计划（老版）=5"
		end if
		If hasSCPlan And (islist = False Or (isNet=True And islist = True And InStr(","&designtype&",",",52002,")>0)) Then
			DSource = DSource& app.iif(Len(DSource)>0 ,";" , "")&"生产计划=52002"
		end if
		Dim hasSCPG:hasSCPG = (ZBRuntime.MC(18600) And app.power.existsPower(54,19))
		If hasSCPG And (isNet=True And islist = False Or (islist = True And InStr(","&designtype&",",",54002,")>0)) Then DSource = DSource& app.iif(Len(DSource)>0 ,";" , "")&"生产派工=54002"
		getDesignTypeList = DSource
	end function
	Function showDesignStatus(designStatus)
		Dim DStr : DStr = ""
		Select Case designStatus
		Case 1 : DStr = "未领用"
		Case 2 : DStr = "申请中"
		Case 3 : DStr = "已领用"
		Case 4 : DStr = "待设计"
		...
		Case 10: DStr = "作废"
		End Select
		showDesignStatus = DStr
	end function
	Function bill_AjaxWindow_setAbandon(byref ajaxpage)
		Dim ord : ord = app.getInt("ord")
		ajaxpage.title = "放弃原因"
		ajaxpage.width = 410
		ajaxpage.height = 140
		Response.write "" & vbcrlf & "      <div  style=""border:0px;width:100%;height:98.4%"">" & vbcrlf & " <textarea name=""reson"" id = ""reson"" style=""width:98.4%;height:70px"" rows=""3""></textarea>" & vbcrlf & "        <div style=""width:100%;padding-top:2px;"" align=""center"">" & vbcrlf & "            <button class=""oldbutton2"" onclick=""javascript:saveAbandon("
		Response.write ord
		Response.write ")"">确定</button>&nbsp;" & vbcrlf & "             <button class=""oldbutton2"" onclick=""javascript:app.easyui.closeWindow('setAbandon');"">取消</button>" & vbcrlf & " <div>" & vbcrlf & "   </div>" & vbcrlf & "  "
	end function
	function App_getOrder(designtype , fromid , showType)
		Dim rs ,sql, url
		If fromid&""="" Then fromid = 0
		If designtype<>0 and fromid<>0 Then
			Dim  title ,cateid ,sort1 ,hasXq ,designTypeName ,designTypeCode
			cateid = 0
			Select Case designtype
			Case 1 :
			sql = "select title , date7 ,cateid from chance where ord = " & fromid
			sort1 = 3
			designTypeName = "项目"
			designTypeCode = "chance"
			url = "../chance/content.asp"
			Case 2 :
			sql = "select title , date7 ,cateid from contract where ord = " & fromid
			sort1 = 5
			designTypeName = "合同"
			designTypeCode = "contract"
			url = "../../SYSN/view/sales/contract/ContractDetails.ashx"
			Case 3 :
			sql = "select title , date7 ,cateid from price where ord = " & fromid
			sort1 = 4
			designTypeName = "报价"
			designTypeCode = "price"
			url = "../../SYSN/view/sales/price/price.ashx"
			Case 4
			sql = "select title ,indate date7, creator cateid from M_PredictOrders where id = " & fromid
			sort1 = 52
			designTypeName = "预测单"
			designTypeCode = "M_PredictOrders"
			url = "1"
			Case 5 :
			sql = "select title ,indate date7,creator cateid from M_ManuPlans where id = " & fromid
			sort1 = 50
			designTypeName = "生产计划"
			designTypeCode = "M_ManuPlans"
			url = "3"
			Case 52001
			sql = "select title ,indate date7, creator cateid from M2_ManuPlansPre where id = " & fromid
			sort1 = 52
			designTypeName = "预生产计划"
			designTypeCode = "M2_PrePlans"
			url = "../../SYSN/view/produceV2/ManuPlansPre/ManuPlansPreAdd.ashx"
			Case 52002 :
			sql = "select title ,indate date7,creator cateid from M2_ManuPlans where id = " & fromid
			sort1 = 50
			designTypeName = "生产计划"
			designTypeCode = "M2_ManuPlans"
			url = "../../SYSN/view/produceV2/ManuPlans/ManuPlansAdd.ashx"
			Case 54002 :
			sql = "select title ,indate date7,creator cateid from M2_WorkAssigns where id = " & fromid
			sort1 = 54
			designTypeName = "生产派工"
			designTypeCode = "M2_WorkAssigns"
			url = "../../SYSN/view/produceV2/WorkAssign/WorkAssignDetail.ashx"
			End Select
			Set rs = cn.execute(sql)
			If rs.eof=False Then
				title = rs("title")
			...
			rs.close
			If Len(cateid&"") = 0 Then cateid = 0
			Select Case  showType
			Case 0 :
			url = title
			Case 1 :
			url = "转自" & designTypeName &"：" & title
			Case 2 :
			url = designTypeCode
			case 3 :
			hasXq = app.power.existsPowerIntro(sort1 ,14 , cateid)
			If hasXq Then
				If designtype = 4 Or designtype = 5 Then
					url = "<a href='javaScript:void(0)' title='点击查看详情' onclick=""javascript:window.open('../manufacture/inc/Readbill.asp?orderid="& url &"&id=" & fromid & "&SplogId=0','designType','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=250,top=150')"">"& title&"</a>&nbsp;"
				else
					url = "<a href='javaScript:void(0)' title='点击查看详情' onclick=""javascript:window.open('"& url &"?ord=" & app.base64.pwurl(fromid) & "&view=details','designType','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=280,top=180')"">"& title &"</a>&nbsp;"
'&"</a>&nbsp;"
				end if
			else
				url = title
			end if
			url = app.iif(Len(url)>0 ,"：","") & url
			End Select
		end if
		App_getOrder = url
	end function
	Function showDesignButton(cn, designType ,fromid, canAddDesign ,pwid , UIType)
		If UIType = "list" Then Exit Function
		If ZBRuntime.MC(18300) And canAddDesign = True And InStr(","& Replace(sdk.glAttribute("MI_DESIGN_TYPE_2016051601")," ","")&",",","&designType&",")>0 Then
			If cn.execute("select 1 from power where ord="& session("personzbintel2007") &" and sort1=5029 and sort2=13 and qx_open=1 ").eof=False Then
				Select Case designType
				Case 1 :
				canAddDesign = (cn.execute("select 1 from chancelist a where a.chance="& fromid &"  and a.del=1 and exists(select 1 from product where del=1 and canoutstore=1 and ord=a.ord) and a.id not in (select isnull(listid,0) from designlist l inner join design d on d.id=l.design and l.del=1 and d.del=1 and d.designType=1 and chance="& fromid &")").eof=False)
				Case 2 :
				canAddDesign = (cn.execute("select 1 from contractlist a where a.contract="& fromid &"  and a.del=1  and exists(select 1 from product where del=1 and canoutstore=1 and ord=a.ord) and a.id not in (select isnull(listid,0) from designlist l inner join design d on d.id=l.design and l.del=1 and d.del=1 and d.designType=2 and contract="& fromid &")").eof=False)
				Case 3 :
				canAddDesign = (cn.execute("select 1 from pricelist a where a.price="& fromid &" and a.del=1  and exists(select 1 from product where del=1 and canoutstore=1 and ord=a.ord) and a.id not in (select isnull(listid,0) from designlist l inner join design d on d.id=l.design and l.del=1 and d.del=1 and d.designType=3 and price="& fromid &")").eof=False)
				End Select
				If canAddDesign Then
					If UIType = "list" Then
						Response.write "<input type=""button"" name=""Submit3"" value=""设计""  onClick=""javascript:window.open('../design/add.asp?designtype="
						Response.write designType
						Response.write "&fromid="
						Response.write pwid
						Response.write "','newwinAddDesign','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" class=""anybutton""/>"
						Response.write pwid
					else
						Response.write "<input type=""button"" name=""Submt3"" value=""设计"" " & vbcrlf & "                                           onClick=""javascript:window.open('../design/add.asp?designtype="
						Response.write designType
						Response.write "&fromid="
						Response.write pwid
						Response.write "','newwinAddDesign','width=' + 1200 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"" class=""anybutton""/>"
						Response.write pwid
					end if
				end if
			end if
	...
	Function showDesignList(cn, designType ,fromid ,pwid)
		If ZBRuntime.MC(18300)=False Or Not sdk.power.existsPower(5029,19) Then Exit Function
		If InStr(sdk.glAttribute("MI_DESIGN_TYPE_2016051601"),designType)=0 Then
			If cn.execute("select 1 from design d where d.del=1 and d.designType="& designType &" and (case "& designType &" when 1 then d.chance when 2 then d.contract  when 3 then d.price when 4 then d.M_PredictOrders when 5 then d.M_ManuPlans else 0 end)  = "& fromid &" ").eof=True Then
				Exit Function
			end if
		end if
		Response.write "<style>span.link{color:#2F496E;cursor:pointer;}</style>"
		Response.write "" & vbcrlf & "     <script type=""text/javascript"" language=javascript>" & vbcrlf & "       var Bill = {" & vbcrlf & "            showunderline: function (obj, c) {obj.style.textDecoration = ""underline""; if (c) {obj.style.color = c}}," & vbcrlf & "          hideunderline: function (obj, c) {obj.style.textDecoration = ""none"";if (c) {c = c.toLowerCase();if (c==""blue"" || c == ""#0000ff"") {c = ""#2F496E"";}obj.style.color = c}}, "& vbcrlf &"           SpShowList: function(OrId,BlId,logId,wName) {var t = new Date();var opener = window.open(""../manufacture/inc/Readbill.asp?orderid="" + OrId + ""&ID="" + BlId + ""&SplogId="" + logId + ""&vTime="" + t.getTime());}" & vbcrlf & "   }" & vbcrlf & "       </script>" & vbcrlf & "       "
		Response.write "<style>span.link{color:#2F496E;cursor:pointer;}</style>"
		dim rs , qxopen , qxintro , uid , iii ,idx ,sql ,linkHTML ,designer
		uid = session("personzbintel2007")
		scllqxopen = ""
		sdk.setup.getpowerattr 5029, 1, qxopen, qxintro
		if qxopen = "1" or qxopen = "3" then
			Response.write "             " & vbcrlf & "                <tr class=""top content-split-bar"">" & vbcrlf & "                        <td height=""27"" colspan=""8"">" & vbcrlf & "                                <div style=""float:left;padding-left:2px;height:100%;"">" & vbcrlf & "                                    <strong class=""blue2"" >关联设计任务</strong>" & vbcrlf & "                                      "
'if qxopen = "1" or qxopen = "3" then
			If designType = 2 then
				Response.write "<img class=""content-split-icon"" src=""../images/r_down.png"" style=""border:0px;width:14px;height:14px;""/>"
'If designType = 2 then
			end if
			Response.write "" & vbcrlf & "                             </div>" & vbcrlf & "                  </td>" & vbcrlf & "           </tr>" & vbcrlf & "           <tr><td colspan='8' class='name'>" & vbcrlf & "                       <table class=""detailTable"" width='100%' border='0' cellpadding='3' cellspacing='1' bgcolor='#C0CCDD' style='text-align:center'>" & vbcrlf & "                   <tr class='nametbheader'>" & vbcrlf & "         "
			sql = "select designBH as 设计编号,title 设计主题 , "&_
			"  ISNULL((SELECT TOP 1 name FROM gate WHERE ord = d.creator), '用户' + CAST(d.Creator AS varchar(10)) + '【已删】')  AS 添加人, "&_
			"convert(varchar(16),indate,120) as 添加时间, designstatus as 设计状态 ,designer, id "&_
			"  from design d "&_
			"  where designType="& designType &" and (case "& designType &" when 1 then d.chance when 2 then d.contract  when 3 then d.price when 4 then d.M_PredictOrders when 5 then d.M_ManuPlans else 0 end)  = "& fromid &" and del=1 "&_
			"  and ("& qxopen &"=3 or ("& qxopen &"=1 and charindex(','+cast(isnull(designer,0) as varchar(12))+',' , ',"& qxintro &",')>0))"
			set rs = cn.execute(sql)
			for iii = 0 to rs.fields.count - 3
				set rs = cn.execute(sql)
				Response.write "<td height=25>" & rs.fields(iii).name & "</td>"
			next
			Response.write "</tr>"
			If rs.eof = True Then
				Response.write "<tr><td height=""27"" colSpan="&rs.fields.count&">无关联设计任务！</td></tr>"
			end if
			idx = 0
			Do while rs.eof = False
				idx=idx+1
'Do while rs.eof = False
				...
				linkHTML = ""
				designer = rs("designer").value
				If Len(designer&"")=0 Then designer = 0
				If sdk.power.existsPowerIntro(5029,14, designer) Then
					linkHTML = "<a href='javascript:void(0);' onclick=""javascript:window.open('../design/content.asp?ord="& pwurl(rs("id")) &"','newwincontent','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;"">"& rs.fields(1).value &"</a>"
'If sdk.power.existsPowerIntro(5029,14, designer) Then
				else
					linkHTML= rs.fields(1).value
				end if
				Response.write "<tr style='color:#5B7CAE'>"
				Response.write "<td width='20%'>" & rs.fields(0).value& "</td>"
				Response.write "<td height=25 width='20%' style='text-align:left;padding-left:5px'>" & linkHTML  & "</td>"
				Response.write "<td width='20%'>" & rs.fields(0).value& "</td>"
				Response.write "<td width='20%'>" & rs.fields(2).value & "</td>"
				Response.write "<td width='20%'>" & rs.fields(3).value & "</td>"
				Response.write "<td width='20%'>" & showDesignStatus(rs.fields(4).value) & "</td>"
				Response.write "</tr>"
				rs.movenext
			Loop
			If idx>3 then
				Response.write "" & vbcrlf & "                     <tr><td colspan='5' style='height:24px' align='right'> <a href=""javascript:void(0);"" onclick=""javascript:window.open('../design/list.asp?designtype="
				Response.write designType
				Response.write "&fromid="
				Response.write pwurl(fromid)
				Response.write "','newwindesign','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150');return false;""><font class=""red"">更多设计任务..&gt;&gt;&gt;</font></a></td></tr>" & vbcrlf & "                       "
				Response.write pwurl(fromid)
			end if
			rs.close
	...
	Function ShowBOMInfo(cn , model , typ , ord , extraCss)
		Dim canAdd : canAdd = False
		if ZBRuntime.MC(18002) Then
			Dim canShow : canShow = True
			Select Case typ
			Case 3 : canShow = (cn.execute("select 1 from designlist dl inner join design d on d.id=dl.Design and d.designstatus=8 where dl.id = "& ord).eof=false)
			End Select
			if sdk.power.existsPower(56,13) And canShow And model="bool" Then canAdd = True
		end if
		ShowBOMInfo = canAdd
	end function
	Function getRelevanceOrds(source, datatype, dataid)
		Dim rs, ret, sql
		ret = ""
		sql = "SET NOCOUNT ON; select top 0 CAST(0 as int) as datatype , CAST(0 as int) as dataid into #templist "& vbCrLf &_
		"    insert into #templist(datatype , dataid)  "& vbCrLf &_
		"    exec [erp_bill_Relevance] "& datatype &" , "& dataid &" ; "& vbCrLf
		Select Case source&""
		Case "chance"
		sql = sql &"select a.ord from chance a  "& vbCrLf &_
		"    inner join #templist tp on tp.datatype=12001 and tp.dataid = a.ord "& vbCrLf &_
		"   where (a.del=1 or a.del=3)  "
		Case "contract"
		sql = sql &"select a.ord from contract a  "& vbCrLf &_
		"   inner join #templist tp on tp.datatype=11001 and tp.dataid = a.ord "& vbCrLf &_
		"   where (a.del=1 or a.del=3)  "
		Case "repair_deal"
		sql = sql &"select a.id from RepairOrder a  "& vbCrLf &_
		"   inner join #templist tp on tp.datatype=14001 and tp.dataid = a.id "& vbCrLf &_
		"   where a.del=1  "
		End Select
		sql = sql & vbCrLf &"drop table #templist; SET NOCOUNT OFF; "
		set rs = conn.execute(sql)
		While rs.eof = False
			If ret&"" <> "" Then ret = ret&","
			ret = ret & rs(0)
...
'Set rs = conn.execute("SELECT top 1 1 FROM (SELECT id,CAST(short_str AS INT) cateid FROM dbo.split('"& intro_33_3 &"',',')) a INNER JOIN (select distinct s.cateid from send s inner join kuout k on k.ord=s.kuout and s.del=1 and k.sort1 in(1,4) and k.order1 in("& htords &")) b on a.cateid=b.cateid AND a.cateid>0")
	...
	If fromTJ="1" OR request("TJ")="1" Then
		If open_5_11 = 3 Then
			conditionTJ = ""
		ElseIf open_5_11 = 1 Then
			conditionTJ = " and isnull(cateid,-1) in ("&intro_5_11&")"
'ElseIf open_5_11 = 1 Then
		else
			conditionTJ = " and 1=2"
		end if
		If list_1<>"" Then
			Str_Result=" where del=1 and isnull(status,-1) in (-1,1) and " & list_1 & conditionTJ & " "
'If list_1<>"" Then
		else
			Str_Result=" where del=1 and isnull(status,-1) in (-1,1) " & conditionTJ & " "
'If list_1<>"" Then
		end if
	end if
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=5 and sort2=13")
	if rs1.eof then
		open_5_13=0
	else
		open_5_13=rs1("qx_open").value
	end if
	rs1.close
	set rs1 = nothing
	dim MODULES
	MODULES=session("zbintel2010ms")
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=5 and sort2=21")
	if rs1.eof then
		open_5_21=0
	else
		open_5_21=rs1("qx_open").value
	end if
	rs1.close
	set rs1 = nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=7 and sort2=19")
	if rs1.eof then
		open_7_19=1
	else
		open_7_19=rs1("qx_open").value
	end if
	rs1.close
	set rs1 = nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=32 and sort2=19")
	if rs1.eof then
		open_32_19=1
	else
		open_32_19=rs1("qx_open").value
	end if
	rs1.close
	set rs1 = nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=33 and sort2=19")
	if rs1.eof then
		open_33_19=1
	else
		open_33_19=rs1("qx_open").value
	end if
	rs1.close
	set rs1 = nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=7 and sort2=3")
	if rs1.eof then
		open_7_3=0
		intro_7_3=0
	else
	...
	rs1.close
	set rs1=nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=32 and sort2=3")
	if rs1.eof then
		open_32_3=0
		intro_32_3=0
	else
		open_32_3=rs1("qx_open")
		intro_32_3=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1 = conn.execute("select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=33 and sort2=3")
	if rs1.eof then
		open_33_3=0
		intro_33_3=0
	else
		open_33_3=rs1("qx_open")
		intro_33_3=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql="select  1 from sp_ApprovalRules where gate2=62001  and sptype<>0"
	rs.open sql,conn,1,1
	if rs.eof then
		plck=0
	else
	...
	set rs=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power WITH(NOLOCK) where ord="&nowCate&" and sort1=5 and sort2=26"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_5_26=0
	else
		open_5_26=rs1("qx_open")
	end if
	rs1.close
	set rs1=nothing
	hasStoreModel = ZBRuntime.MC(17000)
	hasKuoutModel = ZBRuntime.MC(17003)
	hasSendModel = ZBRuntime.MC(17008)
	Call GetPowerValue(open_5_24, intro_5_24,  5, 24)
	if ZBRuntime.MC(15000)=false then
		open_5_24=0
	end if
	dim W1,W2,W3,A1,A2,B,C,E,F,H,MinContractMoney,MaxContractMoney,m3,m4,bz
	A1=request("A1")
	A2=request("A2")
	A3=request("A3")
	B=request("B")
	...
	If Len(request("bz"))>0 then
		bz=request("bz")
	else
		bz=0
	end if
	MinContractMoney=request("MinContractMoney")
	MaxContractMoney=request("MaxContractMoney")
	m3=request("ret3")
	m4=request("ret4")
	jhdate=request("jhdate")
	jhdate2=request("jhdate2")
	SH=request("SH")
	sp1=request("sp")
	F1=request("F1")
	F2=request("F2")
	...
	zdy5=request("zdy5")
	zdy6=request("zdy6")
	if instr(zdy5,"@")>0 then zdy5=""
	if instr(zdy6,"@")>0 then zdy6=""
	remind = Request("remind")
	dateType = Request("dateType")
	date1_0 = Request("date1_0")
	date1_1 = Request("date1_1")
	companyid=deurl(Request("companyid"))
	if sp1<>"" then
		if sp1="1" then
			Str_Result="where del=1 and isnull(status,-1) not in (-1,0,1) and "&list_1&""
'if sp1="1" then
		elseif sp1="2" then
			Str_Result="where del=1 and isnull(status,-1) =0 and "&list_1&""
'elseif sp1="2" then
		elseif sp1="3" then
			Str_Result="where del=1 and isnull(status,-1)<>0 and "&list_1&""
'elseif sp1="3" then
	...
	elseif request("type")&""="1" then
		Str_Result="where del=1 and isnull(status,-1) in (-1,1) and "&list_1&""
'elseif request("type")&""="1" then
	end if
	If remind <> "" Then
	...
	if SH<>"" Then
		if SH="1" then
			Str_Result=" where del=1 and share<>'0' and share<>'-1' and (share='1' or addshare="&nowCate&") "
'if SH="1" then
		elseif SH="2" then
			Str_Result=" where del=1 and (share='1' or share like '%"&nowCate&"%') "
			If remind <> "" Then
				Set helper = CreateReminderHelper(conn,remind,0)
	...
	If datatype&""<>"" And fromid&""<>"" Then
		fromid = deurl(fromid)
		Str_Result=Replace(Str_Result&"","del=1 ","del in(1,3) ")
		Str_Result = Str_Result & " and ord in("& getRelevanceOrds("contract", datatype, fromid) &") "
	end if
	fst=request("fst")
	htTerminated = request("htTerminated")
	searchType = request("searchType")
	searchValue = request("searchValue")
	If htTerminated <> "" Then Str_Result = Str_Result & " and isTerminated=" & htTerminated
	Dim paybackState : paybackState = request("paybackState")
	Dim invoiceState : invoiceState = request("invoiceState")
	Select Case paybackState
	Case "1"
	paybackStateCondition = " and isnull(paybackMoney,(case isnull(importPayback,0) when 0 then 0 else money1 end)) = 0 and (case when round(money1,"&num_dot_xs&")=0 then 1 else round(money1,"&num_dot_xs&")-isnull(round(thmoney,"&num_dot_xs&"),0)end)>0 "
'Case "1" '
	Case "2"
	paybackStateCondition = " and isnull(round(paybackMoney,"&num_dot_xs&"),(case isnull(importPayback,0) when 0 then 0 else round(money1,"&num_dot_xs&")-isnull(round(thmoney,"&num_dot_xs&"),0) end)) < round(money1,"&num_dot_xs&")-isnull(round(thmoney,"&num_dot_xs&"),0) and isnull(paybackMoney,0) > 0 "
'Case "2" '
	Case "3"
	paybackStateCondition = " and isnull(round(paybackMoney,"&num_dot_xs&"),(case isnull(importPayback,0) when 0 then 0 else round(money1,"&num_dot_xs&")-isnull(round(thmoney,"&num_dot_xs&"),0) end)) >= round(money1,"&num_dot_xs&")-isnull(round(thmoney,"&num_dot_xs&"),0) "
'Case "3" '
	Case Else
	paybackStateCondition = ""
	End Select
	Select Case invoiceState
	Case "1"
	Str_Result = Str_Result & " and (invoiceMode = 0 or (invoiceMode = 2 and not exists (select top 1 1 from contractlist WITH(NOLOCK) where contract = a.ord and invoiceType > 0) ) )"
	Case "2"
	invoiceStateCondition = " and invoiceMode <> 0 " & vbcrlf &_
	" and((invoiceMode=1 and sa.InvoicePlanType>0) or exists (select top 1 1 from contractlist WITH(NOLOCK) where contract = a.ord and invoiceType > 0)) " & vbcrlf &_
	" and isnull(invoiceMoney,(case isnull(importInvoice,0) when 0 then 0 else isnull(sf.needInvoiceMoney,0) end)) = 0 and (isnull(sf.needInvoiceMoney,0)-isnull(sa.yhmoney,0) >= 0) "
	Case "3"
	invoiceStateCondition = " and isnull(invoiceMoney,(case isnull(importInvoice,0) when 0 then 0 else isnull(sf.needInvoiceMoney,0)-isnull(sa.yhmoney,0) end)) < isnull(sf.needInvoiceMoney,0)-isnull(sa.yhmoney,0) and isnull(invoiceMoney,0) > 0 "
'Case "3" '
	Case "4"
	invoiceStateCondition = " and isnull(invoiceMoney,(case isnull(importInvoice,0) when 0 then 0 else isnull(sf.needInvoiceMoney,0) end)) >= isnull(sf.needInvoiceMoney,0)-isnull(sa.yhmoney,0) and isnull(sf.needInvoiceMoney,0)-isnull(sa.yhmoney,0) > 0 "
'Case "4" '
	Case Else
	invoiceStateCondition = ""
	End Select
	If paybackStateCondition <> "" Or invoiceStateCondition <> "" Then
		Str_Result = Str_Result & " and ord in (" & vbcrlf &_
		"select ord from contract sa WITH(NOLOCK) " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select sum(money1) paybackMoney,contract " & vbcrlf &_
		"from payback WITH(NOLOCK) " & vbcrlf &_
		"where complete>=3 and del=1 " & vbcrlf &_
		"group by contract" & vbcrlf &_
		") sd on sd.contract = sa.ord " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select sum(money1) invoiceMoney,fromId contract " & vbcrlf &_
		"from paybackInvoice WITH(NOLOCK) " & vbcrlf &_
		"where (isInvoiced=1 or isInvoiced=2) and del=1 and fromType = 'CONTRACT' " & vbcrlf &_
		"group by fromId " & vbcrlf &_
		") se on se.contract = sa.ord " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select case when t.invoiceMode=0 then 0 when t.invoiceMode=1 then t.money1 else sum(case when ct.invoiceType > 0 then ct.money1 else 0 end) end needInvoiceMoney,t.ord contract " & vbcrlf &_
		" from contract t  "&_
		" left join contractlist ct WITH(NOLOCK) on ct.del<>2 and ct.contract=t.ord " & vbcrlf &_
		"group by t.ord,t.invoiceMode,t.money1 " & vbcrlf &_
		") sf on sf.contract=sa.ord " & vbcrlf &_
		"left join ( " & vbcrlf &_
		"    select a.contract, " & vbcrlf &_
		"        isnull(sum(a.num1),0) as num_1, " & vbcrlf &_
		"        sum(case when a.num2 =0 then 0 else isnull(a.num2,0) end) as num_2, " & vbcrlf &_
		"        sum(case when a.num3 =0 then 0 else isnull(a.num3,0) end) as num_3, " & vbcrlf &_
		"        sum(case when a.num4 =0 then 0 else isnull(a.num4,0) end) as num_4, " & vbcrlf &_
		"        isnull(sum(c.numth),0) as numth, isnull(sum(c.thmoney),0) as thmoney  " & vbcrlf &_
		"        from contractlist a WITH(NOLOCK) " & vbcrlf &_
		"        inner join product b WITH(NOLOCK) on a.ord=b.ord and b.canoutstore=1 " & vbcrlf &_
		"        left join ( " & vbcrlf &_
		"            select l.contractlist, isnull(sum(l.money1),0) as thmoney, " & vbcrlf &_
		"                isnull(sum(case when l.unit=cl.unit then l.num1 else cast(l.num1*isnull(cc.bl,1) as decimal(25,12))/isnull(dd.bl,1) end),0) as numth " & vbcrlf &_
		"            from contractthlist l WITH(NOLOCK)  " & vbcrlf &_
		"            inner join contractth t WITH(NOLOCK) on t.ord=l.caigou and (t.del<>2 and t.sp<>-1) " & vbcrlf &_
		"            from contractthlist l WITH(NOLOCK)  " & vbcrlf &_
		"            inner join contractlist cl WITH(NOLOCK) on cl.id = l.contractlist " & vbcrlf &_
		"            left join ( " & vbcrlf &_
		"                select distinct unit,product,bl from jiage WITH(NOLOCK) where bm=0 " & vbcrlf &_
		"            ) cc on cc.product=l.ord and cc.unit = l.unit " & vbcrlf &_
		"            left join ( " & vbcrlf &_
		"                select distinct unit,product,bl from jiage WITH(NOLOCK) where bm=0 " & vbcrlf &_
		"            ) dd on dd.product=l.ord and dd.unit = cl.unit " & vbcrlf &_
		"            where isnull(l.kuoutlist2,0)=0 and t.del=1 and t.sp=0  " & vbcrlf &_
		"            group by l.contractlist " & vbcrlf &_
		"        ) c on isnull(c.contractlist,0)=a.id " & vbcrlf &_
		"        group by a.contract " & vbcrlf &_
		"    ) sc on sc.contract = sa.ord  " & vbcrlf &_
		"where 1=1  & paybackStateCondition & invoiceStateCondition "& vbcrlf &_
		") "
	end if
	If searchValue<>"" Then
		Select Case searchType
		Case "htTitle" : Str_Result = Str_Result & " and title like '%" & searchValue & "%'"
		Case "htBh" : Str_Result = Str_Result & " and htid like '%" & searchValue & "%'"
		Case "khname" : Str_Result = Str_Result & " and exists (select top 1 1 from tel WITH(NOLOCK) where ord=a.company and name like '%" & searchValue & "%')"
		Case "htIntro" : Str_Result = Str_Result & " and intro like '%" & searchValue & "%'"
		Case "spcate" : Str_Result = Str_Result & " and exists (select top 1 1 from gate WITH(NOLOCK) where ord=a.cateid_sp and name like '%" & searchValue & "%')"
		Case "xscate" : Str_Result = Str_Result & " and exists (select top 1 1 from gate WITH(NOLOCK) where ord=a.cateid and name like '%" & searchValue & "%')"
		Case "cpname" : Str_Result = Str_Result & " and exists(select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.title like '%" & searchValue & "%')"
		Case "cporder1" : Str_Result = Str_Result & " and exists (select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.order1 like '%" & searchValue & "%')"
		Case "cptype1" : Str_Result = Str_Result & " and exists (select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.type1 like '%" & searchValue & "%')"
		Case Else : Str_Result = Str_Result & " and "&searchType&" like '%" & searchValue & "%'"
		End Select
	end if
	If fst<>"" And m1 = "" And m2 = "" and request("Type")<>"所有" Then
...
'If fst<>"" And m1 = "" And m2 = "" and request("Type")<>"所有" Then '
	...
	if link="dhk" then
		if m1<>"" then
			sqlcon=" and date3>='"&m1&" 00:00:00'"
		end if
		if m2<>"" then
			sqlcon=sqlcon &" and datediff(dd,'"&m2&"', date3)<=0"
		end if
		if trim(request("w3"))<>"" then
			sqlcon=sqlcon & " and cateid in (" & trim(request("w3")) & ") and cateid<>0"
		end if
		w3=""
		m1=""
		m2=""
	end if
	if SH="3333333" or SH="5555555" then
		new_list="<span class='alt_tx'></span>"
		timelg=session("timezbintel2007")
		if SH="3333333" and timelg<>"" then
			noalt1=" and date7>'"&timelg&"'"
		end if
		if not (open_5_1=1 or open_5_1=3) then
			Str_Result_14="and (Charindex(',"&nowCate&",', ',' + Replace(Cast(share AS VARCHAR(8000)), ' ', '') + ',') > 0 or share='1') "
'if not (open_5_1=1 or open_5_1=3) then
		end if
		if link="dsp" then
			Str_Result=" where (cateid_sp="& uid &" or cateid=" & uid & ") and del=1 and isnull(status,-1) in (3,4,5)"&noalt1
'if link="dsp" then
		else
			Str_Result="where ((cateid_sp="&nowCate&"  and del=1 and isnull(status,-1) in (3,4,5)) or (addcate="&nowCate&" and alt=1 and del=1)) "&Str_Result_14&" "&noalt1&""
'if link="dsp" then
		end if
	end if
	dim newstate
	...
	if newstate="1" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select ord,intro,num1,tq1,fw1 from setjm2 WITH(NOLOCK) where ord=35 and cateid="&nowCate&""
		rs.open sql,conn,1,1
		if not rs.eof then
			if rs("fw1")="0" then
				Str_Result = Str_Result & ""
			else
				Str_Result = Str_Result & " and cateid="&nowCate&" "
			end if
		end if
		rs.close
		set rs=nothing
	...
	W3=sdk.GetGlistUrl(request("W3"))
	If request("tj")=1 Or request("FromTJ")&""="1" Then
		if  clng("0" & request.QueryString("IsOther"))= 0 then
			W3=getW3(W1,W2,W3)
		else
			W3 = ""
			set strW3rs = conn.execute("select ord  from gate where orgsid=" & clng(W1))
			while strW3rs.eof = false
				if len(W3 & "")>0 then  W3=W3 & ","
				W3 = W3 & strW3rs(0).value
				strW3rs.movenext
			wend
			strW3rs.close
		end if
		If w1="999999999" Then
			Str_Result=Str_Result+" and isnull(cateid,0)= 0 and not exists (select ord from gate WITH(NOLOCK) where ord=a.cateid)"
'If w1="999999999" Then
		else
			If w2<>"" Then Str_Result=Str_Result+" and isnull(cateid3,0) in ("& W2 &")"
'If w1="999999999" Then
			If w3<>""and W3&""<>"0" Then Str_Result=Str_Result+" and isnull(cateid,0) in ("& W3 &")"
'If w1="999999999" Then
		end if
	else
		if W1="" then W1=0
		...
		W4=replace(W3,"0","")
		W4=replace(W4,",","")
		gateTy = "," & Replace(request.querystring("gatety")," ","") & ","
		if W4<>"" Then
			...
			Dim gateCondition
			gateCondition = ""
			If InStr(gatety,",1,") > 0 Then
				gateCondition = gateCondition & " or (cateid in ("& W3 &") and cateid<>0) "
			end if
			If InStr(gatety,",2,") > 0 Then
				gateCondition = gateCondition & " or (addcate in ("& W3 &") and addcate<>0) "
			end if
			If InStr(gatety,",3,") > 0 Then
				gateCondition = gateCondition & " or (isnull(cateid,0) = 0 or not exists (select ord from gate WITH(NOLOCK) where ord=a.cateid)) "
			end if
			If InStr(gatety,",4,") > 0 Then
				gateCondition = gateCondition & " or (cateid_sp in ("& W3 &") and cateid_sp<>0) "
			end if
			If len(gateCondition)>0 Then
				Str_Result=Str_Result & " and (1=2 "&gateCondition&") "
			end if
		else
			If InStr(gatety,",3,") > 0 Then
				Str_Result = Str_Result & " and (isnull(cateid,0) = 0 or not exists (select top 1 1 from gate WITH(NOLOCK) where ord=a.cateid)) "
			end if
		end if
	end if
	dim ContactLx
	ContactLx=request("ContactLx")
	if ContactLx="planall.asp" then
		Str_Result = Str_Result & " and isnull(status,-1) in (-1,1) "
'if ContactLx="planall.asp" then
	end if
	...
	if HtType<>"" then
		select case HtType
		case 0
		Str_Result=Str_Result+" and (paybacktype=0 or paybacktype is null) AND (sort1 is null or sort1=0) and isnull(repairOrderId,0)=0 and not exists(select top 1 1 from contractlist where contract = a.ord and isnull(treeord,0)>0) and  isnull(sort1,0) not in (1,7) "
'case 0   '
		titlename="销售"
		case 2
		Str_Result=Str_Result+" and paybacktype=1 AND (sort1 is null or sort1=0) "
'case 2   '
		titlename="直接出库"
		case 3
		Str_Result=Str_Result+" AND sort1 = 1 "
'case 3      '
		titlename="销售开单"
		case 4
		Str_Result=Str_Result+" AND isnull(repairOrderId,0)>0 "
		titlename="维修"
		case 5
		Str_Result=Str_Result+" AND wxUserId is not null "
'case 5        '
		titlename="微信"
		Case 6
		Str_Result=Str_Result+" AND isnull(repairOrderId,0)=0   AND exists(select top 1 1 from contractlist where contract = a.ord and isnull(treeord,0)>0) "
'Case 6 '
		titlename="组装"
		Case 7
		Str_Result=Str_Result+" AND sort1=7 "
'Case 7 '
		titlename="导入"
		case else
		Str_Result=Str_Result
	...
	adhtTerminated = request("adhtTerminated")
	adCpName = request("adCpName")
	adCpOrder1 = request("adCpOrder1")
	adCpType1 = request("adCpType1")
	if adhtTerminated&""<>"" then adhtTerminated = Replace(adhtTerminated&""," ","") else adhtTerminated = ""
	if adhtTerminated&""<>"" then
		Str_Result = Str_Result & " and isnull(isTerminated,0) in(" & adhtTerminated &") "
	end if
	if adCpName&""<>"" then Str_Result = Str_Result & " and exists(select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.title like '%" & adCpName & "%') "
	if adCpOrder1&""<>"" then Str_Result = Str_Result & " and exists (select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.order1 like '%" & adCpOrder1 & "%')"
	if adCpType1&""<>"" then Str_Result = Str_Result & " and exists (select top 1 1 from contractlist ctl WITH(NOLOCK) inner join product cp WITH(NOLOCK) on cp.ord=ctl.ord and ctl.del="&iif(sp1<>"","3","1")&" and ctl.contract=a.ord and cp.type1 like '%" & adCpType1 & "%')"
	area_list=-1
	if A2<>"" Then
		Function selects(id)
			Dim rs,strid
	...
	Str_Result=Str_Result+"and  area in ("&selects(A2)&")"
	selects=strid
	end if
	if A1<>"" then
...
'if A3<>"" then
	...
	if D<>"" then
		Str_Result=Str_Result+"and  (select isnull(trade,0)trade from tel where ord=a.company) in  ("&D&")"
'if D<>"" then
	end if
	if E<>"" then
...
'if zdy6<>"" then
	...
	if F2<>"" then
		if F1=1 then
			str_Result=str_Result+" and exists (select top 1 1 from tel WITH(NOLOCK) where ord=a.company and name like '%"& F2 &"%')"
'if F1=1 then
		elseif F1=2 then
			str_Result=str_Result+" and exists (select top 1 1 from tel WITH(NOLOCK) where ord=a.company and name not like '%"& F2 &"%')"
'elseif F1=2 then
		elseif F1=3 then
			str_Result=str_Result+" and exists (select top 1 1 from tel WITH(NOLOCK) where ord=a.company and name='"&F2&"')"
'elseif F1=3 then
		elseif F1=4 then
			str_Result=str_Result+" and exists (select top 1 1 from tel WITH(NOLOCK) where ord=a.company and name<>'"&F2&"')"
'elseif F1=4 then
...
'elseif zdy4_1=6 then
	...
	if bz<>"" then
		if bz<>0 then
			Str_Result=Str_Result+"and bz="&bz&" "
'if bz<>0 then
		end if
	end if
	if MinContractMoney<>"" then
		Str_Result=Str_Result+"and a.money1>="&MinContractMoney&" "
'if MinContractMoney<>"" then
	end if
	if MaxContractMoney<>"" then
		Str_Result=Str_Result+"and a.money1<="&MaxContractMoney&" "
'if MaxContractMoney<>"" then
	end if
	if dateType&""="2" then
		date1_0 = m1 : date1_1 = m2
		m1 = "" : m2 = ""
	end if
	if m1<>"" then
...
'if m1<>"" then
	...
	if m2<>"" then
		Str_Result=Str_Result+"and datediff(dd,'"&m2&"', date3)<=0 "
'if m2<>"" then
	end if
	if date1_0<>"" then
		Str_Result=Str_Result+"and date1>='"&date1_0&"' "
'if date1_0<>"" then
	end if
	if date1_1<>"" then
		Str_Result=Str_Result+"and datediff(dd,'"&date1_1&"', date1)<=0 "
'if date1_1<>"" then
	end if
	if m3<>"" then
		Str_Result=Str_Result+"and date2>='"&m3&"' "
'if m3<>"" then
	end if
	if m4<>"" then
		Str_Result=Str_Result+"and datediff(dd,'"&m4&"', date2)<=0 "
'if m4<>"" then
	end if
	jhCondition = ""
	If jhdate<>"" Then
		jhCondition = jhCondition & " and date2>='" & jhdate & "' "
	end if
	If jhdate2<>"" Then
		jhCondition = jhCondition & " and datediff(dd,'"&jhdate2&"', date2)<=0 "
	end if
	If jhCondition <> "" Then
		Str_Result = Str_Result & " and ord in (select distinct contract from contractlist WITH(NOLOCK) where del=1 "&jhCondition&")"
	end if
	If request("remind") <> "" Then
		Set helper = CreateReminderHelper(conn,request("remind"),0)
		str_Result1 = " and a.ord in (" & helper.listSQL("ids") & ")"
	end if
	if ZT<>"" then
		dim ztSql : ztSql = "select sum(num1) num1,sum(num2) num2,SUM(isnull(numth,0)) numth  from (select contractlist.contract,contractlist.num1,contractlist.num2,isnull(ct.num1,0) numth  from contractlist WITH(NOLOCK) inner join product p on p.ord=contractlist.ord and p.canoutstore=1 left join contractthlist ct on isnull(ct.kuoutlist2,0)=0 and ct.contractlist=contractlist.id and ct.del=1) y where y.contract=ord"
		if zt="1" then
			Str_Result=Str_Result+" and zt1=1 and zt2=0 and not exists(select 1 from ( "& ztSql &") x where  x.num1<=x.num2+x.numth and x.num2>0) "
'if zt="1" then
		elseif zt="2" then
			Str_Result=Str_Result+" and zt1=1 and zt2=0 and exists(select 1 from ( "& ztSql &") x where x.num1<=x.num2+x.numth and x.num2>0) "
'elseif zt="2" then
		elseif zt="8"     then
			Str_Result=Str_Result+" and zt1=2 and zt2=0 and exists(select 1 from ( "& ztSql &") x where x.num1<=x.num2+x.numth and x.num2>0) "
'elseif zt="8"     then
		elseif zt="9"     then
			Str_Result=Str_Result+" and zt1=2 and zt2=1 and exists(select 1 from ( "& ztSql &") x where x.num1<=x.num2+x.numth and x.num2>0) "
'elseif zt="9"     then
		elseif zt="3" then
			Str_Result=Str_Result+" and (zt1=2 or zt1=5) and zt2=0 and exists(select 1 from ("& ztSql &") x where  x.num1>x.num2+x.numth) "
'elseif zt="3" then
		elseif zt="1,3" then
			Str_Result=Str_Result+" and  ((zt1=1 and zt2=0 and not exists(select 1 from ( "& ztSql &") x where  x.num1<=x.num2+x.numth and x.num2>0)) OR  (zt1=2 or zt1=5) and zt2=0 and exists(select 1 from ("& ztSql &") x where  x.num1>x.num2+x.numth) or (zt1=2 and zt2=1 and exists(select 1 from ("& ztSql&") x where x.num1>x.num2+x.numth and x.num2>0))) "
'elseif zt="1,3" then
		elseif zt="4" then
...
'elseif zt="6" then
		elseif zt="7" then
			Str_Result=Str_Result+" and (zt1=3 or zt1=6) and (zt2=2 or zt2=3) "
'elseif zt="7" then
		elseif zt = "100" then
			Str_Result = Str_Result + " and (zt1=1 or zt1=2)"
'elseif zt = "100" then
		elseif zt = "101" then
			Str_Result = Str_Result + " and (zt2=0 or zt2=1)"
'elseif zt = "101" then
		end if
	end if
	Call searchExtended(5,"a.ord")
	repID = Request("repID")
	If repID <> "" Then Str_Result = Str_Result + " AND RepairOrderID = "&repID&" "
	repID = Request("repID")
	currpage=Request("currpage")
	If Len(currpage) > 8 Then currpage = Left(currpage,8)
	If currpage & "" = "" Or isnumeric(currpage & "") = False Then currpage=1
	if CLng(currpage)<=0 or currpage="" Then currpage=1
	...
	if lie_2="" Then lie_2=1
	lie_3=request.QueryString("lie_3")
	if hasStoreModel then
		if lie_3="" Then lie_3=1
	else
		if lie_3="" Then lie_3=2
	end if
	lie_4=request.QueryString("lie_4")
	if lie_4="" Then lie_4=1
	lie_7=request.QueryString("lie_7")
	if lie_7="" Then lie_7=1
	Dim hslie, liedata, liedataarr
	Dim lieOpen
	lieOpen = getattr("dislieOpen")
	If lieOpen = "0" Then
		lieOpen = 0
	else
		lieOpen = 1
	...
	hslie =  false
	liedata = getattr("ctractcols") & ""
	liedataarr = Split(liedata & ";;;;;;;",";")
	If Len(request.QueryString("lie_1")) = 0 or isnumeric(request.QueryString("lie_1")) = false  Then
		If Len(liedataarr(0)) > 0 Then lie_1 = liedataarr(0)
	else
		hslie = true
	end if
	If Len(request.QueryString("lie_2")) = 0 or isnumeric(request.QueryString("lie_2")) = false Then
		If Len(liedataarr(1)) > 0 Then lie_2 = liedataarr(1)
	else
		hslie = true
	end if
	If Len(request.QueryString("lie_3")) = 0 or isnumeric(request.QueryString("lie_3")) = false Then
		If Len(liedataarr(2)) > 0 Then
			If sp1<>"1" And liedataarr(2)="9" Then
			else
				lie_3 = liedataarr(2)
			end if
		end if
	...
	If Len(request.QueryString("lie_4")) = 0 or isnumeric(request.QueryString("lie_4")) = false Then
		If Len(liedataarr(3)) > 0 Then lie_4 = liedataarr(3)
	else
		hslie = true
	end if
	If Len(request.QueryString("lie_5")) = 0 or isnumeric(request.QueryString("lie_5")) = false Then
		If Len(liedataarr(4)) > 0 Then lie_5 = liedataarr(4)
	else
		hslie = true
	end if
	If Len(request.QueryString("lie_6")) = 0 or isnumeric(request.QueryString("lie_6")) = false Then
		If Len(liedataarr(5)) > 0 Then lie_6 = liedataarr(5)
	else
		hslie = true
	end if
	If Len(request.QueryString("lie_7")) = 0 or isnumeric(request.QueryString("lie_7")) = false Then
		If Len(liedataarr(6)) > 0 Then lie_7 = liedataarr(6)
	else
		hslie = true
	end if
	...
	If Len(request.QueryString("px")) = 0 or isnumeric(request.QueryString("px")) = false Then
		If Len(liedataarr(7)) > 0 Then px = liedataarr(7)
	else
		hslie = true
	end if
	liedata = lie_1 & ";" & lie_2 & ";" & lie_3 & ";" & lie_4 & ";" & lie_5 & ";" & lie_6 & ";" & lie_7& ";" & px
	If lieOpen = 1 Then
		If hslie Then
			setAttr "ctractcols" , liedata
		end if
	end if
	...
	elseif px=5 then
		px_Result="order by htid desc,date7 desc"
	elseif px=6 then
		px_Result="order by htid asc,date7 asc"
	elseif px=7 then
		px_Result="order by date3 desc,ord desc,date7 desc"
	elseif px=8 then
		px_Result="order by date3 asc,ord asc,date7 asc"
	elseif px=9 then
		px_Result="order by date2 desc,ord desc,date7 desc"
	elseif px=10 then
		px_Result="order by date2 asc,ord asc,date7 asc"
	elseif px=11 then
		px_Result="order by catespname desc,ord desc,date7 desc"
	elseif px=12 then
		px_Result="order by catespname asc,ord asc,date7 asc"
	elseif px=13 then
		px_Result="order by sptime desc,ord desc,date7 desc"
	elseif px=14 then
		px_Result="order by sptime asc,ord asc,date7 asc"
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & " margin-top: 0px;" & vbcrlf & "}" & vbcrlf & ".dlgbackdiv{" & vbcrlf & "   margin:-5px;" & vbcrlf & "    position:absolute;" & vbcrlf & "      text-align:center;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "/*body {" & vbcrlf & "       margin-top: 0px;" & vbcrlf & "        overflow-x:hidden;" & vbcrlf & "}*/" & vbcrlf & "#content tr.top td{padding-bottom:8px!important}" & vbcrlf & "#ht1 #content{margin-bottom:-1px}" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf &"    function plkuout(){" & vbcrlf & "        var selectid = """";" & vbcrlf & "        var id = """";" & vbcrlf & "        var canRK = ""1"";" & vbcrlf & "        $(""input[name='selectid']:checked"").each(function(){" & vbcrlf & "            selectid += (selectid==""""?"""":"","") + $(this).val();" & vbcrlf & "        });     " & vbcrlf & "        if(selectid == """"){" & vbcrlf & "            alert(""您没有选择任何合同，请选择后再出库！"");" & vbcrlf & "        }else{" & vbcrlf & "            $(""input[name='selectid'][canck=0]:checked"").each(function(){" & vbcrlf & "                id = $(this).val();" & vbcrlf &"                canRK = ""0"";" & vbcrlf & "                $(""#ci""+id).html(""不允许出库！"");" & vbcrlf & "            });   " & vbcrlf & "            var id1 = ""0""; var i = ""0"";" & vbcrlf & "            var cg;       " & vbcrlf & "            if(canRK == 0){return;}  " & vbcrlf & "            $('#w').html('<iframe src=""contentck.asp?ords='+selectid+'"" style=""width:100%;height:100%"" frameborder=""0""></iframe>').window({" & vbcrlf & "                title:'库管选择'," & vbcrlf & "                top:200+$(document.body).scrollTop()," & vbcrlf & "                width:340," & vbcrlf & "                height:220," & vbcrlf & "                closeable:true," & vbcrlf & "                collapsible:false," & vbcrlf & "                minimizable:false," & vbcrlf & "                maximizable:false," & vbcrlf & "                modal:true" & vbcrlf & "            }).window('open');" & vbcrlf & "    }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function ChooseKg(date){       " & vbcrlf & "        var url = ""contractplck.asp?ords=""+date.ords+""&kg=""+date.kg+""&sp=""+date.sp+""&sp_ApprovalRules=""+date.sp_ApprovalRules+""&spid=""+date.spid+""&gate3=""+date.gate3+""""" & vbcrlf & "   xmlHttp.open(""GET"", url, false);" & vbcrlf & "        xmlHttp.onreadystatechange = function(){" & vbcrlf & "            if(xmlHttp.readyState==4){" & vbcrlf & "                alert('出库申请完毕！');" & vbcrlf & "                window.location.reload(); " & vbcrlf & "            }" & vbcrlf & "   };" & vbcrlf & "        xmlHttp.send(null);" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "function callServer2(sp1) {" & vbcrlf & "  var url = ""liebiao_tj.asp?gategroup=1&sp=""+sp1+""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  xmlHttp.open(""GET"", url, false); "& vbcrlf &"   xmlHttp.onreadystatechange = function(){ "& vbcrlf &"  updatePage2(); "& vbcrlf &"   }; "& vbcrlf &"   xmlHttp.send(null); "& vbcrlf &" } "& vbcrlf &" function updatePage2() { "& vbcrlf &"         var test7=""ht1"" "& vbcrlf &"    if (xmlHttp.readyState < 4) { "& vbcrlf &"         ht1.innerHTML=""loading...;" & vbcrlf &"       }"& vbcrlf &"       if (xmlHttp.readyState == 4) {" & vbcrlf & "          var response = xmlHttp.responseText;" & vbcrlf & "            ht1.innerHTML=response;" & vbcrlf & "         $(""#daysOfMonthPos"").val($(""input[name=ret]"").val());" & vbcrlf & "               $(""#daysOfMonth2Pos"").va1:""0"") + "&ld=" + escape(ldata) + "&date1="+ Math.round(Math.random()*100); "& vbcrlf &"  xmlHttp.open(""GET"", url, false); "& vbcrlf &"   xmlHttp.send(); "& vbcrlf &"  var r = xmlHttp.responseText; "& vbcrlf &"    if(r!=""ok""){alert(r)} "& vbcrlf &" } "& vbcrlf & vbcrlf &" function terminateHtConfirm(ord){" & vbcrlf & "       var yord = jQuery(""#zzHtord"").val();" & vbcrlf & "      if(yord!=ord){" & vbcrlf & "          if(jQuery(""#delpeyback"").size()>0){" & vbcrlf & "                       jQuery(""#delpeyback"").attr(""checked"",false);" & vbcrlf & "                }" & vbcrlf & "               if(jQuery(""#delInvoice"").size()>0){" & vbcrlf & "                      jQuery(""#delInvoice"").attr(""checked"",false);" & vbcrlf & "                }" & vbcrlf & "               if(jQuery(""#delkuout"").size()>0){" & vbcrlf & "                 jQuery(""#delkuout"").attr(""checked"",false);" & vbcrlf & "          }" & vbcrlf & "               if(jQuery(""#delsend"").size()>0){" & vbcrlf & "                  jQuery(""#delsend"").attr(""checked"",false);" & vbcrlf & "          }               " & vbcrlf & "        }" & vbcrlf & "       jQuery.ajax({" & vbcrlf & "           url:'AjaxReturn.asp'," & vbcrlf & "           data:{act:'terminateCheck',id:ord}," & vbcrlf & "             cache:false," & vbcrlf & "            async:false," & vbcrlf & "            dataType:'json'," & vbcrlf & "                success:function(r){" & vbcrlf & "                      if(r.success){" & vbcrlf & "                          var canDelSk = r.canDelSk" & vbcrlf & "                               var canDelKp = r.canDelKp" & vbcrlf & "                               var canDelCk = r.canDelCk" & vbcrlf & "                               var canDelFh = r.canDelFh" & vbcrlf & "                               if(jQuery(""#delpeyback"").size()>0){" & vbcrlf & "                                       jQuery(""#delpeyback"").attr(""disabled"",(canDelSk==0?true:false));" & vbcrlf & "                              }" & vbcrlf & "                               if(jQuery(""#delInvoice"").size()>0){" & vbcrlf & "                                       jQuery(""#delInvoice"").attr(""disabled"",(canDelKp==0?true:false));" & vbcrlf & "                            }" & vbcrlf & "                               if(jQuery(""#delkuout"").size()>0){" & vbcrlf & "                                 jQuery(""#delkuout"").attr(""disabled"",(canDelCk==0?true:false));" & vbcrlf & "                                }" & vbcrlf & "                               if(jQuery(""#delsend"").size()>0){" & vbcrlf & "                                  jQuery(""#delsend"").attr(""disabled"",(canDelFh==0?true:false));" & vbcrlf & "                               }" & vbcrlf & "                       }else{" & vbcrlf & "                          alert(r.msg);" & vbcrlf & "                   }"& vbcrlf &"             }, "& vbcrlf &"               error:function(req, textStatus, errorThrown){ "& vbcrlf &"                    if(confirmuery('#ZZDiv').window('resize',{top:$(document).scrollTop() + ($(window).height()-260) * 0.5});" & vbcrlf & "     jQuery(""#zzHtord"").val(ord);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function kuoutCheck(){" & vbcrlf & "       if(jQuery(""#delkuout"").attr(""checked"")){" & vbcrlf & "            if(jQuery(""#delsend"").size()>0){ "& vbcrlf &"                     if(!jQuery(""#delsend"").attr(""disabled"")){ "& vbcrlf &"                            jQuery(""#delsend"").attr(""checked"",true); "& vbcrlf &"                     } "& vbcrlf &"                } "& vbcrlf &"        } "& vbcrlf &" } "& vbcrlf & vbcrlf &" function sendCheck(){  "& vbcrlf & "        if(!jQuery(""#delsend"").attr(checked)){ "& vbcrlf &"             if(jQuery(""#delkuout"").size()>0){ "& vbcrlf &"                  if(!jQuery(""#delkuout"").attr(""disabled"")){ "& vbcrlf &"                           jQuery(""#delkuout"").attr(""checked"",false); "& vbcrlf &"                   } "& vbcrlf &"                } "& vbcrlf &"        } "& vbcrlf &" } "& vbcrlf & vbcrlf &" function terminateSave(){" & vbcrlf & "  var ord = jQuery(""#zzHtord"").val();       " & vbcrlf & "        terminateContract(ord, 1)" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function terminateContract(ord,flg){" & vbcrlf & "  var s = """";" & vbcrlf & "       if(flg==""1""){" & vbcrlf & "             s = """"" & vbcrlf & "    }else{" & vbcrlf & "  s = ""确定要取消合同的终止状态吗？""" & vbcrlf & "        }" & vbcrlf & "       var delpeyback,delInvoice,delkuout,delsend;" & vbcrlf & "     if(s == """"){" & vbcrlf & "              delpeyback=0;delInvoice=0;delkuout=0;delsend=0;" & vbcrlf & "         if(jQuery(""#delpeyback"").size()>0){" & vbcrlf & "                       if(jQuery(""#delpeyback"").attr(""checked"")){delpeyback=1;} "& vbcrlf &"                } "& vbcrlf &"                if(jQuery(""#delInvoice"").size()>0){ "& vbcrlf &"                        if(jQuery(""#delInvoice"").attr(""checked"")){delInvoice=1;} "& vbcrlf &"             } "& vbcrlf &"                if(jQuery(""#delkuout"").size()>0){ "& vbcrlf &"                  if(jQuery(""#delkuout"").attr(""checked"")){delkuout=1;}" & vbcrlf & "             }" & vbcrlf & "               if(jQuery(""#delsend"").size()>0){" & vbcrlf & "                  if(jQuery(""#delsend"").attr(""checked"")){delsend=1;}" & vbcrlf & "          }" & vbcrlf & "       }       " & vbcrlf & "        if(s=="""" || (s!="""" && confirm(s))){" & vbcrlf & "         jQuery.ajax({" & vbcrlf & "                   url:'AjaxReturn.asp',"& vbcrlf &"                     data:{actrlf &                                     if (r.addnoticeurl!=""""){" & vbcrlf & "                                          window.open(r.addnoticeurl,'newwin_add','width=' + 1250 + ',height=' + 600 + ',fullscreen =""no"",scrollbars=""1"",toolbar=""0"",resizable=""1"",left=""250"",top=""150""')" & vbcrlf & "                                     }" & vbcrlf & "                                       gotourl("""");" & vbcrlf & "                              }else{" & vbcrlf & "                          alert(r.msg);" & vbcrlf & "                           }" & vbcrlf & "                       }," & vbcrlf & "                      error:function(req, textStatus, errorThrown){" & vbcrlf & "                           if(confirm('提交数据时发生错误，需要查看错误信息吗？')){" & vbcrlf & "                                        alert(req.responseText);" & vbcrlf & "                                }" & vbcrlf & "                               jQuery('#terminateConfirm').dialog('close');" & vbcrlf & "                     }" & vbcrlf & "               });" & vbcrlf & "     }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script language=""javascript1.2"">" & vbcrlf & "function ChecklhbType(values,nums)" & vbcrlf & "{" & vbcrlf & "  document.location.href=""planall.asp?HtType="" + nums+'&'+values;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "$(function(){" & vbcrlf & " $('.commonSelect option').each(function(){" & vbcrlf & "              $(this).attr('title',$(this).text());" & vbcrlf & "   });" & vbcrlf & "});" & vbcrlf & "</script>" & vbcrlf & "<script type=""text/JavaScript"" src=""../inc/jquery.easyui.min.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<style>" & vbcrlf & ".commonSelect{width:79px}" & vbcrlf & ".btn_visible ." & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  "
	if open_5_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"" onload='window.maxSboxWidth(document.getElementById(""quick_search""),98);' >" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" class='btn_visible'>" & vbcrlf & "<tr>" & vbcrlf & "      <td width=""100%"" valign=""top"">" & vbcrlf & "            <table class=""resetTableBg"" width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"" style='table-layout:fixed'>" & vbcrlf & "                   <tr>" & vbcrlf & "                            <td class=""place"" style='width:300px;height:64px'>" & vbcrlf & ""
	if H="1" and sp1="" Then
		Response.write "合同检索"
	elseif sp1="1" Then
		Response.write "审批中合同"
	elseif sp1="2" Then
		Response.write "未通过合同"
	elseif SH="1" Then
		Response.write "我共享给他人合同"
	elseif SH="2" Then
		Response.write "他人共享给我合同"
	else
		Response.write titlename & "合同列表"
	end if
	Response.write "" & vbcrlf & "                             </td>" & vbcrlf & "                           <td style='width:100px;'>&nbsp;<a class=""tableTitleLinks sortRule"" class='px_btn' href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>"& vbcrlf & "                              <td align=""right"" style='width:auto;padding-left:0;padding-right:0;line-height:32px;'>" & vbcrlf & ""
	Response.write titlename & "合同列表"
	Dim urlParams
	urlParams="gatety="&request("gatety")&"&W1="&W1&"&W2="&W2&"&W3="&W3&"&W31="&request.querystring("w31")&"&A1="&A1&"&A2="&A2&"&A3="&A3&"&B="&B&"&C="&Server.UrlEncode(C)&"&D="&D&"&E="&E&"&F="&F&"&H="&H&"&lie_1="&lie_1&"&lie_3="&lie_3&"&lie_4="&lie_4&"&lie_7="&lie_7&"&page_count="&page_count&"&px="&px&"&bz="&bz&"&MinContractMoney="&MinContractMoney&"&MaxContractMoney="&MaxContractMoney&"&ret="&m1&"&ret2="&m2&"&ret3="&m3&"&ret4="&m4&"&F1="&F1&"&F2="&F2&"&G1="&G1&"&G2="&G2&"&P1="&P1&"&P2="&P2&"&I1="&I1&"&I2="&I2&"&zdy1_1="&zdy1_1&"&zdy1_2="&zdy1_2&"&zdy2_1="&zdy2_1&"&zdy2_2="&zdy2_2&"&zdy3_1="&zdy3_1&"&zdy3_2="&zdy3_2&"&zdy4_1="&zdy4_1&"&zdy4_2="&zdy4_2&"&zdy5="&zdy5&"&zdy6="&zdy6&"&S1="&S1&"&S2="&S2&"&sp="&sp1&"&ZT="&ZT&"&SH="&SH&"&fst="&fst&"&dt_0="&dt_0&"&dt_1="&dt_1&"&htCate="&htCate&"&htStatus="&htStatus&"&htTerminated="&htTerminated&"&fromTJ="&fromTJ&"&searchType="&searchType&"&searchValue="&searchValue&"&price="&request.querystring("price")&"&Telord="&request.querystring("Telord") & "&remind=" & remind &"&paybackState="& paybackState &"&invoiceState="& invoiceState &"&adhtTerminated="& adhtTerminated &"&adCpName="& adCpName &"&adCpOrder1="& adCpOrder1 &"&adCpType1="& adCpType1
	if hasKuoutModel and hasStoreModel Then
		Response.write "" & vbcrlf & "                                     <select name=""ContactLx"" id=""ContactLx"" onchange=""gotourl('httype='+this.value)"">" & vbcrlf & "                                             <option value="""" "
'if hasKuoutModel and hasStoreModel Then
		If HtType="" Then Response.write("selected")
		Response.write ">所有合同</option>" & vbcrlf & "                                             <option value=""3"" "
		If HtType="3" Then Response.write("selected")
		Response.write ">销售开单</option>" & vbcrlf & "                                             <option value=""0"" "
		If HtType="0" Then Response.write("selected")
		Response.write ">销售合同</option>" & vbcrlf & "                                             <option value=""2"" "
		If HtType="2" Then Response.write("selected")
		Response.write ">直接出库合同</option>" & vbcrlf & "                                         "
		if ZBRuntime.MC(71000) Then
			Response.write "<option value=""4"" "
			If HtType="4" Then Response.write("selected")
			Response.write ">维修合同</option>"
		end if
		if ZBRuntime.MC(76000) Then
			Response.write "<option value=""5"" "
			If HtType="5" Then Response.write("selected")
			Response.write ">微信合同</option>"
		end if
		if ZBRuntime.MC(17010) Then
			Response.write "<option value=""6"" "
			If HtType="6" Then Response.write("selected")
			Response.write ">组装合同</option>"
		end if
		Response.write "" & vbcrlf & "                                              <option value=""7"" "
		If HtType="7" Then Response.write("selected")
		Response.write ">导入合同</option>" & vbcrlf & "                                    </select>" & vbcrlf & ""
	end if
	if open_5_25=1 or open_5_25=3 Then
		Response.write "" & vbcrlf & "                                      <input type=""button"" name=""Submitdel2"" value=""导入"" onClick=""window.location.href='../../SYSN/view/sales/contract/ContractImport.ashx'"" class=""anybutton""/>" & vbcrlf & ""
	end if
	if open_5_10=1 or open_5_10=3 Then
		Response.write "" & vbcrlf & "                                      <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xlsht.asp',postParams:[{n:'px_v',v:document.getElementById('px_1').value},{n:'Str_Result_v',v:document.getElementById('ReportOrds_1').value},{n:'join_Str_Result_v',v:document.getElementById('join_Str_Result_1').value},{n:'persons_result_v',v:document.getElementById('persons_result_1').value}]});}"" class=""anybutton""/>" & vbcrlf & ""
	end if
	if open_5_7=1 or open_5_7=3 Then
		Response.write "" & vbcrlf & "                                      <input type=""button""  name=""print"" onclick=""javascript:kh.style.display='none';window.print();return  false;"" value=""打印"" class=""anybutton""/>" & vbcrlf & ""
	end if
	isSuperAdmin = (conn.execute("select  a.ord from gate a inner  join power b on a.top1 = 1 and a.ord = b.ord and b.sort1 = 66 and b.sort2 = 12 and b.qx_open = 1 and a.del = 1 and a.ord="& session("personzbintel2007")).eof=false)
	tempCount = 0
	if isSuperAdmin then
		tempCount = conn.execute("select count(1) from contract where del=5")(0)
	else
		tempCount = conn.execute("select count(1) from contract where del=5 and addcate=" & session("personzbintel2007"))(0)
	end if
	if open_5_13 then
		Response.write "                  " & vbcrlf & "                        <input type=""button""  name=""templist"" onclick=""window.location='../../sysn/view/sales/contract/ContractTempList.ashx';"" value=""草稿箱("
		Response.write tempCount
		Response.write ")"" class=""anybutton""/>" & vbcrlf & "                    "
	end if
	Response.write "" & vbcrlf & "                    <span id=""kh"" style='display:inline'>" & vbcrlf & "                                       <a href=""javascript:void(0)"" class=""AfterQuickSearch"" style='display:inline-block;width:66px;margin-bottom:6px' onclick=""jQuery('#adSearchForm').show();callServer2('"
	'Response.write ")"" class=""anybutton""/>" & vbcrlf & "                    "
	Response.write sp1
	Response.write "');document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a>&nbsp;</span>" & vbcrlf & "                          </td>" & vbcrlf &" <td style=""width:0px;""></td> "& vbcrlf &"                       </tr> "& vbcrlf &"                    <tr style=""height:50px""> "& vbcrlf &"                           <td colspan=""4""  class='page_top_btn ser_btn2 resetHeadBg resetBorderColor'  style=""text-align:right;padding-left:20px;height:50px;line-height:50px"" > "& vbcrlf & "                                      <form action=""planall.asp?HtType="
	Response.write HtType
	Response.write """ method=""get"" id=""quick_search"" name=""date2"" style=""margin:0"">" & vbcrlf & "                                            <INPUT TYPE=""hidden"" NAME=""H"" VALUE="""
	Response.write H
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""v2"" VALUE="""
	Response.write request.querystring("v2")
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""w31"" VALUE="""
	Response.write request.querystring("w31")
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""px"" VALUE="""
	Response.write px
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""sp"" VALUE="""
	Response.write sp1
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""SH"" VALUE="""
	Response.write SH
	Response.write """>" & vbcrlf & "                                         <input type=""hidden"" name=""HtType"" value="""
	Response.write HtType
	...
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""price"" VALUE="""
	Response.write request.querystring("price")
	Response.write """>" & vbcrlf & "                                         <INPUT TYPE=""hidden"" NAME=""Telord"" VALUE="""
	Response.write request.querystring("Telord")
	Response.write """>                                         " & vbcrlf & "                                                <select name=""dateType"">" & vbcrlf & "                                                  <option value=""1"" "
	Response.write iif(dateType&""="" or dateType&""="1","selected","")
	Response.write ">签订日期</option>" & vbcrlf & "                                                    <option value=""2"" "
	Response.write iif(dateType&""="2","selected","")
	Response.write ">开始日期</option>" & vbcrlf & "                                            </select>" & vbcrlf & "                                               自：<INPUT name=""ret"" onmousedown=""datedlg.show()"" readOnly maxLength=""10"" size=""10"" style=""width:80px;"" value="""
	Response.write iif(dateType&""="2",date1_0,m1)
	Response.write """/>" & vbcrlf & "                                                至：<INPUT name=""ret2"" onmousedown=""datedlg.show()"" readOnly maxLength=""10"" size=""10"" style=""width:80px;"" value="""
	Response.write iif(dateType&""="2",date1_1,m2)
	Response.write """/>" & vbcrlf & "                                                <select name=""F"" class=""commonSelect"">" & vbcrlf & "                                                      <option value="""">合同分类</option>" & vbcrlf & ""
	Set rsSearch = conn.execute("select sort1,ord from sortonehy WITH(NOLOCK) where gate2=31")
	While rsSearch.eof = False
		Response.write "" & vbcrlf & "                                                      <option value="""
		Response.write rsSearch(1)
		Response.write """ "
		If F=rsSearch(1)&"" Then Response.write " selected"
		Response.write ">"
		Response.write rsSearch(0)
		Response.write "</option>" & vbcrlf & ""
		rsSearch.movenext
	wend
	rsSearch.close
	Response.write "" & vbcrlf & "                                              </select>" & vbcrlf & "                                               <select name=""E"" class=""commonSelect"">" & vbcrlf & "                                                      <option value="""">合同状态</option>" & vbcrlf & ""
	Set rsSearch = conn.execute("select sort1,ord from sortonehy WITH(NOLOCK) where gate2=32")
	While rsSearch.eof = False
		Response.write "" & vbcrlf & "                                                     <option value="""
		Response.write rsSearch(1)
		Response.write """ "
		If E=rsSearch(1)&"" Then Response.write " selected"
		Response.write ">"
		Response.write rsSearch(0)
		Response.write "</option>" & vbcrlf & ""
		rsSearch.movenext
	wend
	rsSearch.close
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               <select name=""htTerminated"" class=""commonSelect"">" & vbcrlf & "                                                   <option value="""">终止状态</option>" & vbcrlf & "                                                        <option value=""0"""
	If htTerminated="0" Then Response.write " selected"
	Response.write ">正常</option>" & vbcrlf & "                                                       <option value=""1"""
	If htTerminated="1" Then Response.write " selected"
	Response.write ">已终止</option>" & vbcrlf & "                                             </select>" & vbcrlf & ""
	if ZBRuntime.MC(23000) then
		Response.write "" & vbcrlf & "                                             <select name=""paybackState"" class=""commonSelect"">" & vbcrlf & "                                                   <option value="""">收款状态</option>" & vbcrlf & "                                                        <option value=""1"""
		If paybackState="1" Then Response.write " selected"
		Response.write ">未收款</option>" & vbcrlf & "                                                     <option value=""2"""
		If paybackState="2" Then Response.write " selected"
		Response.write ">部分收款</option>" & vbcrlf & "                                                   <option value=""3"""
		If paybackState="3" Then Response.write " selected"
		Response.write ">收款完毕</option>" & vbcrlf & "                                           </select>" & vbcrlf & "                                               <select name=""invoiceState"" class=""commonSelect"">" & vbcrlf & "                                                   <option value="""">开票状态</option>" & vbcrlf & "                                                        <option value=""1"""
		If invoiceState="1" Then Response.write " selected"
		Response.write ">不开票</option>" & vbcrlf & "                                                     <option value=""2"""
		If invoiceState="2" Then Response.write " selected"
		Response.write ">未开票</option>" & vbcrlf & "                                                     <option value=""3"""
		If invoiceState="3" Then Response.write " selected"
		Response.write ">部分开票</option>" & vbcrlf & "                                                   <option value=""4"""
		If invoiceState="4" Then Response.write " selected"
		Response.write ">开票完毕</option>" & vbcrlf & "                                           </select>" & vbcrlf & ""
	end if
	if hasStoreModel and  hasSendModel then
		Response.write "" & vbcrlf & "                                             <select name=""ZT"" id=""zt"" class=""commonSelect"">" & vbcrlf & "                                                       <option value=""0"">发货进展</option>" & vbcrlf & "                                                       <option value=""1"""
		If ZT="1" Then Response.write " selected"
		Response.write ">未出库未发货</option>" & vbcrlf & "                                                       <option value=""2"""
		If ZT="2" Then Response.write " selected"
		Response.write ">申请完毕未出库未发货</option>" & vbcrlf & "                                                       <option value=""8"""
		If ZT="8" Then Response.write " selected"
		Response.write ">申请完毕部分出库未发货</option>" & vbcrlf & "                                                     <option value=""9"""
		If ZT="9" Then Response.write " selected"
		Response.write ">申请完毕部分出库部分发货</option>" & vbcrlf & "                                                   <option value=""3"""
		If ZT="3" Then Response.write " selected"
		Response.write ">部分出库未发货</option>" & vbcrlf & "                                                     <option value=""4"""
		If ZT="4" Then Response.write " selected"
		Response.write ">出库完毕未发货</option>" & vbcrlf & "                                                     <option value=""5"""
		If ZT="5" Then Response.write " selected"
		Response.write ">部分出库部分发货</option>" & vbcrlf & "                                                   <option value=""6"""
		If ZT="6" Then Response.write " selected"
		Response.write ">出库完毕部分发货</option>" & vbcrlf & "                                                   <option value=""7"""
		If ZT="7" Then Response.write " selected"
		Response.write ">出库完毕发货完毕</option>" & vbcrlf & "                                           </select>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "                                             <select name=""searchType"" class=""commonSelect"">" & vbcrlf & "                                                     <option value=""htTitle"" "
	If searchType="htTitle" Then Response.write " selected"
	Response.write ">合同主题</option>" & vbcrlf & "                                                   <option value=""htBh"" "
	If searchType="htBh" Then Response.write " selected"
	Response.write ">合同编号</option>" & vbcrlf & "                                                   <option value=""khname"" "
	If searchType="khname" Then Response.write " selected"
	Response.write ">关联客户</option>" & vbcrlf & "                                                   <option value=""htIntro"" "
	If searchType="htIntro" Then Response.write " selected"
	Response.write ">合同概要</option>" & vbcrlf & "                                                   "
	If sp1="1" Then
		Response.write "" & vbcrlf & "                                                             <option value=""spcate"" "
		If searchType="spcate" Then Response.write " selected"
		Response.write ">审批人员</option>" & vbcrlf & "                                                           "
	end if
	Response.write "" & vbcrlf & "                                                     <option value=""xscate"" "
	If searchType="xscate" Then Response.write " selected"
	Response.write ">销售人员</option>" & vbcrlf & "                                                   "
	if ZBRuntime.MC(13000) Then
		Response.write "" & vbcrlf & "                                                     <option value=""cpname"" "
		If searchType="cpname" Then Response.write " selected"
		Response.write ">产品名称</option>" & vbcrlf & "                                                   <option value=""cporder1"" "
		If searchType="cporder1" Then Response.write " selected"
		Response.write ">产品编号</option>" & vbcrlf & "                                                   <option value=""cptype1"" "
		If searchType="cptype1" Then Response.write " selected"
		Response.write ">产品型号</option>                                                   " & vbcrlf & "                                                        "
	end if
	Set rsSearch = conn.execute("select title,name from zdy WITH(NOLOCK) where sort1=5 and sort=2 and set_open=1 and js=1")
	While rsSearch.eof = False
		Response.write "" & vbcrlf & "                                                             <option value="""
		Response.write rsSearch(1)
		Response.write """ "
		If searchType=rsSearch(1) Then Response.write " selected"
		Response.write ">"
		Response.write rsSearch(0)
		Response.write "</option>" & vbcrlf & "                                                            "
		rsSearch.movenext
	wend
	rsSearch.close
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               <input type=""text"" name=""searchValue"" size=""10"" style=""width:120px"" value="""
	Response.write searchValue
	Response.write """/>" & vbcrlf & "                                               <input type=""submit"" class=""anybutton"" value=""检索""/>" & vbcrlf & "                                 </form>" & vbcrlf & "                         </td>" & vbcrlf & "                   </tr>" & vbcrlf & "           </table>" & vbcrlf & "                <form id=""adSearchForm"" action=""planall.asp"" method=""get"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "                        <INPUT TYPE=""hidden"" NAME=""H"" VALUE="""
	Response.write H
	Response.write """>" & vbcrlf & "                        <INPUT TYPE=""hidden"" NAME=""v2"" VALUE="""
	Response.write request.querystring("v2")
	Response.write """>" & vbcrlf & "                        <INPUT TYPE=""hidden"" NAME=""w31"" VALUE="""
	...
	Response.write """>" & vbcrlf & "                        <INPUT TYPE=""hidden"" NAME=""Telord"" VALUE="""
	Response.write request.querystring("Telord")
	Response.write """>" & vbcrlf & "                        <span id=""ht1""></span>" & vbcrlf & "            </form>" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & ""
	...
	Response.write "" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "    <td width=""5%"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "    <td width=""25%"" height=""27"" align=""center""><div align=""center"">" & vbcrlf & "           <select name=""select"" class=""tableTitleLinks"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""font-size:12px;font-weight: bold;color:#2F496E;"" >" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""lie_1=1"" "
'If khname="" Then khname="客户名称"
	if lie_1=1 then
		Response.write "selected"
	end if
	Response.write "><strong>合同主题</strong></option>" & vbcrlf & "          <option value=""lie_1=2"" "
	if lie_1=2 then
		Response.write "selected"
	end if
	Response.write ">合同编号</option>" & vbcrlf & "             <option value=""lie_1=3"" "
	if lie_1=3 then
		Response.write "selected"
	end if
	Response.write ">"
	Response.write khname
	Response.write "</option>" & vbcrlf & "        </select>" & vbcrlf & "     </div></td>" & vbcrlf & "     <td width=""12%"" ><div align=""center"">" & vbcrlf & "         <select name=""select2"" class=""tableTitleLinks"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""font-size:12px;font-weight: bold;color:#2F496E;"" >" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""lie_2=1"" "
	Response.write khname
	if lie_2=1 then
		Response.write "selected"
	end if
	Response.write ">合同金额</option>" & vbcrlf & "             <option value=""lie_2=5"" "
	if lie_2=5 then
		Response.write "selected"
	end if
	Response.write ">退货后总额</option>" & vbcrlf & "          <option value=""lie_2=2"" "
	if lie_2=2 then
		Response.write "selected"
	end if
	Response.write ">签订日期</option>" & vbcrlf & "          <option value=""lie_2=3"" "
	if lie_2=3 then
		Response.write "selected"
	end if
	Response.write ">开始日期</option>" & vbcrlf & "          <option value=""lie_2=4"" "
	if lie_2=4 then
		Response.write "selected"
	end if
	Response.write ">截止日期</option>" & vbcrlf & "        </select>" & vbcrlf & "    </div></td>" & vbcrlf & "     <td width=""10%"" ><div align=""center"">" & vbcrlf & "         <select name=""select4"" class=""tableTitleLinks"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""font-size:12px;font-weight: bold;color:#2F496E;"" >" & vbcrlf & "       <option>-请选择-</option>" & vbcrlf & "       <option value=""lie_4=1"" "
	Response.write "selected"
	if lie_4=1 then
		Response.write "selected"
	end if
	Response.write ">到账金额</option>" & vbcrlf & "         <option value=""lie_4=2"" "
	if lie_4=2 then
		Response.write "selected"
	end if
	Response.write ">费用总额</option>" & vbcrlf & "         <option value=""lie_4=3"" "
	if lie_4=3 then
		Response.write "selected"
	end if
	Response.write ">销售毛利</option>" & vbcrlf & "         <option value=""lie_4=4"" "
	if lie_4=4 then
		Response.write "selected"
	end if
	Response.write ">毛利率</option>" & vbcrlf & "               "
	if open_5_24=1 Then
		Response.write "" & vbcrlf & "               <option value=""lie_4=5"" "
		if lie_4=5 then
			Response.write "selected"
		end if
		Response.write ">销售毛利（进价）</option>" & vbcrlf & "         <option value=""lie_4=6"" "
		if lie_4=6 then
			Response.write "selected"
		end if
		Response.write ">毛利率（进价）</option>" & vbcrlf & "               "
	end if
	Response.write "" & vbcrlf & "               </select>" & vbcrlf & "         </div></td>" & vbcrlf & "   <td width=""16%"" ><div align=""center"">" & vbcrlf & "         <select name=""select3"" class=""tableTitleLinks"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}""  style=""font-size:12px;font-weight: bold;color:#2F496E;"" >" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "                     "
	Response.write ">毛利率（进价）</option>" & vbcrlf & "               "
	if hasStoreModel and hasSendModel then
		...
		Response.write ">发货进展</option>" & vbcrlf & "                   "
	end if
	Response.write "" & vbcrlf & "                     <option value=""lie_3=2"" "
	if lie_3=2 then
		Response.write "selected"
	end if
	Response.write ">合同状态</option>" & vbcrlf & "                   <option value=""lie_3=8"" "
	if lie_3=8 then
		Response.write "selected"
	end if
	Response.write ">执行状态</option>" & vbcrlf & "                   <option value=""lie_3=3"" "
	if lie_3=3 then
		Response.write "selected"
	end if
	Response.write ">合同分类</option>" & vbcrlf & "                   "
	If sp1="1" Then
		Response.write "" & vbcrlf & "                     <option value=""lie_3=9"" "
		if lie_3=9 then
			Response.write "selected"
		end if
		Response.write ">审批人员</option>" & vbcrlf & "                   "
	end if
	Response.write "" & vbcrlf & "                     <option value=""lie_3=4"" "
	if lie_3=4 then
		Response.write "selected"
	end if
	Response.write ">销售人员</option>" & vbcrlf & "                   <option value=""lie_3=5"" "
	if lie_3=5 then
		Response.write "selected"
	end if
	Response.write ">创建人员</option>" & vbcrlf & "                   <option value=""lie_3=7"" "
	if lie_3=7 then
		Response.write "selected"
	end if
	Response.write ">我方代表</option>" & vbcrlf & "                   <option value=""lie_3=6"" "
	if lie_3=6 then
		Response.write "selected"
	end if
	Response.write ">创建日期</option>" & vbcrlf & "        </select>" & vbcrlf & "      </div></td>" & vbcrlf & "   <td width=""32%"" ><div align=""center"">" & vbcrlf & "         <select name=""select5"" class=""tableTitleLinks"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""font-size:12px;font-weight: bold;color:#2F496E;"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""page_count=10&lie_7=1"" "
	Response.write "selected"
	if page_count=10 Or lie_7=1 then
		Response.write "selected"
	end if
	Response.write ">每页显示10条</option>" & vbcrlf & "          <option value=""page_count=20&lie_7=2"" "
	if page_count=20 Or lie_7=2 then
		Response.write "selected"
	end if
	Response.write ">每页显示20条</option>" & vbcrlf & "          <option value=""page_count=30&lie_7=3"" "
	if page_count=30 Or lie_7=3 then
		Response.write "selected"
	end if
	Response.write ">每页显示30条</option>" & vbcrlf & "          <option value=""page_count=50&lie_7=4"" "
	if page_count=50 Or lie_7=4 then
		Response.write "selected"
	end if
	Response.write ">每页显示50条</option>" & vbcrlf & "          <option value=""page_count=100&lie_7=5"" "
	if page_count=100 Or lie_7=5 then
		Response.write "selected"
	end if
	Response.write ">每页显示100条</option>" & vbcrlf & "          <option value=""page_count=200&lie_7=6"" "
	if page_count=200 Or lie_7=6 then
		Response.write "selected"
	end if
	Response.write ">每页显示200条</option>" & vbcrlf & "        </select>" & vbcrlf & "             &nbsp;<input type=""checkbox"" id=""LockItems"" onClick=""setLockItems(this,'"
	...
	dim fromContent,fromChance, fromPrice, fromTel
	fromChance=trim(Request.QueryString("chance"))
	if fromChance<>"" then
		Str_Result = Replace(Str_Result," del=1 "," (del=1 or del=3) ")
		fromContent=" and chance="&fromChance&""
	end if
	fromPrice=trim(Request.QueryString("price"))
	if fromPrice<>"" then
		fromContent=" and event1="&fromPrice&""
	end if
	fromTel=trim(Request.QueryString("Telord"))
	if fromTel<>"" then
		Str_Result = Replace(Str_Result," del=1 "," del=1 and isnull(status,-1) in (-1,1,4,5) ")
'if fromTel<>"" then
		fromContent=" and company="&fromTel&""
	end if
	if companyid&""<>"" and companyid>0 then
		Str_Result = Str_Result & " and a.company in (" & companyid &  ")"
	end if
	Dim payKind : payKind = request("payKind")
	If payKind <> "" Then Str_Result = Str_Result & " and payKind in (" & payKind &  ")"
	If H="31" Then
		Dim v2
		v2=request("v2")
		w31=sdk.GetGlistUrl(request("w31"))
		If Len(w31)=0 Then w31=0
		If isdate(v2)=True Then Str_Result=Str_Result&" and (" & iif(isnumeric(request("w31")),1,open_5_1) & "=3 or isnull(cateid,0) in("&w31&")) and datediff(d,date3,'"&v2&"')>=0 "
	end if
	dim n
	n=0
	summoney1_ht=0
	...
	summoney1_cpall=0
	summoney1_thAfter=0
	summoney1_mlJinjia=0
	summoney1_mlallJinjia=0
	If link="dhk" Then
		Str_Result = Str_Result & " and isnull(paybackMoney,0) < money1"
		conn.execute("select * into #temp from (" & vbcrlf &_
		"select *," & vbcrlf &_
		"num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=a.ord and del=1)," & vbcrlf &_
		"num2=(select isnull(sum(num2),0) from contractlist WITH(NOLOCK) where contract=a.ord and del=1) " & vbcrlf &_
		"from (" & vbcrlf &_
		"select sa.*,isnull(paybackMoney,(case isnull(sa.importPayback,0) when 0 then 0 else sa.money1 end)) paybackMoney " & vbcrlf &_
		"from contract sa WITH(NOLOCK) " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select sum(money1) paybackMoney,contract " & vbcrlf &_
		"from payback WITH(NOLOCK) " & vbcrlf &_
		"where complete>=3 and del=1 " & vbcrlf &_
		"group by contract" & vbcrlf &_
		") sd on sd.contract = sa.ord " & vbcrlf &_
		"where del = 1" & sqlcon &"" & vbcrlf &_
		") as a " & Str_Result & fromContent & " and del = 1 and cateid <> 0 " & vbcrlf &_
		")xx1 ")
		sql8="select isnull(sum(round(money1,"&num_dot_xs&")),0),isnull(sum(round(money2,"&num_dot_xs&")),0) from #temp "
		sql9="select isnull(sum(y.money1),0),isnull(sum(y.money2),0) from contractth x WITH(NOLOCK) inner join (select a.caigou,sum(a.money1) as money1,sum(a.money1*isnull(h.hl,1)) as money2 from contractthlist a WITH(NOLOCK) inner join #temp b on a.contract = b.ord LEFT JOIN hl h ON h.bz = b.bz AND CONVERT(VARCHAR(10),h.date1,120) = CONVERT(VARCHAR(10),b.date3,120) where a.del=1 group by a.caigou) y on x.ord = y.caigou where x.del=1"
		summoney1_hkall=conn.execute( vbcrlf &_
		"select isnull(sum(money1),0) from payback WITH(NOLOCK) " & vbcrlf &_
		"where del=1 and complete>=3 " & vbcrlf &_
		"and contract in (" & vbcrlf &_
		"select ord from (" & vbcrlf &_
		"select *," & vbcrlf &_
		"num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=a.ord and del=1)," & vbcrlf &_
		"num2=(select isnull(sum(num2),0) from  contractlist WITH(NOLOCK) where contract=a.ord and del=1) " & vbcrlf &_
		...
		"select sum(money1) paybackMoney,contract " & vbcrlf &_
		"from payback WITH(NOLOCK) " & vbcrlf &_
		"where complete>=3 and del=1 and CompleteType<>7 " & vbcrlf &_
		"group by contract" & vbcrlf &_
		") sd on sd.contract = sa.ord " & vbcrlf &_
		"where del = 1" & sqlcon &"" & vbcrlf &_
		") as a " & Str_Result & fromContent & " and del = 1 and cateid <> 0 " & vbcrlf &_
		")xx1" & vbcrlf &_
		")")(0).value
		summoney1_fyall=conn.execute( vbcrlf &_
		"select isnull(sum(money1),0) as money1 " & vbcrlf &_
		"from pay WITH(NOLOCK) where del=1 and complete=3 and exists(select 1 from f_pay c with(nolock) where c.id = pay.fid and c.del=1) and contract in (" & vbcrlf &_
		"select ord from (" & vbcrlf &_
		"select *,num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=a.ord and del=1)," & vbcrlf &_
		"num2=(select isnull(sum(num2),0) from contractlist WITH(NOLOCK) where contract=a.ord and del=1) " & vbcrlf &_
		"from (" & vbcrlf &_
		"select sa.*,isnull(paybackMoney,(case isnull(sa.importPayback,0) when 0 then 0 else sa.money1 end)) paybackMoney " & vbcrlf &_
		"from contract sa WITH(NOLOCK) " & vbcrlf &_
		...
		conn.execute("select * into #temp from (" & vbcrlf &_
		"select *," & vbcrlf &_
		"num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1)," & vbcrlf &_
		"num2=(select isnull(sum(num2),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1) " & vbcrlf &_
		"from contract WITH(NOLOCK) " & vbcrlf &_
		"where 1=1 " & sqlcon & vbcrlf &_
		") as a " & Str_Result & fromContent & " " & querystr & " ")
		sql8="select isnull(sum(round(money1,"&num_dot_xs&")),0),isnull(sum(round(money2,"&num_dot_xs&")),0) from #temp "
		sql9="select isnull(sum(y.money1),0),isnull(sum(y.money2),0) from contractth x WITH(NOLOCK) inner join (select a.caigou,sum(a.money1) as money1,sum(a.money1*isnull(h.hl,1)) as money2 from contractthlist a inner join #temp b on a.contract = b.ord LEFT JOIN hl h ON h.bz = b.bz AND CONVERT(VARCHAR(10),h.date1,120) = CONVERT(VARCHAR(10),b.date3,120) where a.del=1 group by a.caigou) y on x.ord = y.caigou where x.del=1"
		summoney1_hkall=conn.execute( vbcrlf &_
		"  select isnull((" & vbcrlf &_
		"  select sum((case isnull(a.importPayback,0) when 1 then round(isnull(a.money1,0),"& num_dot_xs &") else round(isnull(k.money2,0),"& num_dot_xs &") end)) money1 " & vbcrlf &_
		"  from (" & vbcrlf &_
		"          select *," & vbcrlf &_
		"                  num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1)," & vbcrlf &_
		"                  num2=(select isnull(sum(num2),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1) " & vbcrlf &_
		"          from contract WITH(NOLOCK) where 1=1 " & sqlcon & vbcrlf &_
		"  ) a " & vbcrlf &_
		"  left join (" & vbcrlf &_
		"          select p.contract, isnull(sum(p.money1),0) as money2 " & vbcrlf &_
		"          from payback p WITH(NOLOCK) " & vbcrlf &_
		"          inner join contract c WITH(NOLOCK) on p.contract=c.ord and p.del=1 and p.complete>=3 and p.CompleteType<>7  " & vbcrlf &_
		"          group by p.contract" & vbcrlf &_
		"  ) k on k.contract=a.ord  "& Str_Result & querystr & fromContent & vbcrlf &_
		"),0)")(0).value
		summoney1_fyall=conn.execute( vbcrlf &_
		"select isnull(sum(a.money1 * isnull(b.hl,1)),0) as money1 from pay a WITH(NOLOCK) inner join f_pay c with(nolock) on c.id = a.fid and c.del=1  "&_
		" left join hl b WITH(NOLOCK) on a.bz = b.bz and convert(varchar(10),a.date1,120)=convert(varchar(10),b.date1,120)  "&_
		" where a.del=1 and a.complete=3 and a.contract in (" & vbcrlf &_
		"select ord from (" & vbcrlf &_
		"select *, "& vbcrlf &_
		"num1=(select isnull(sum(num1),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1)," & vbcrlf &_
		"num2=(select isnull(sum(num2),0) from contractlist WITH(NOLOCK) where contract=contract.ord and del=1) " & vbcrlf &_
		"from contract WITH(NOLOCK) where 1=1 " & sqlcon & vbcrlf &_
		") as a " & Str_Result &  querystr & fromContent  & vbcrlf &_
		")")(0).value
	end if
	set rs8=conn.execute(sql8)
	if bz<>14 and bz>0 then
		summoney2_htall=rs8(0).value
	else
		summoney2_htall=rs8(1).value
	end if
	rs8.close
	set rs8=Nothing
	set rs9=conn.execute(sql9)
	if bz<>14 and bz>0 then
		summoney1_thall=rs9(0).value
	else
		summoney1_thall=rs9(1).value
	end if
	rs9.close
	set rs9=nothing
	set rs=server.CreateObject("adodb.recordset")
	if link = "dhk" Then
		sql="select [__top__fields__] " & _
		"from("& vbcrlf&_
		"select sa.* "& vbcrlf&_
		",zt_1 = (" & vbcrlf &_
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then " & vbcrlf &_
		"case when isnull(num_2,0) = 0 then 1 when isnull(num_3,0) = 0 then 5 else 2 end "& vbcrlf &_
		"when isnull(num_1,0)<=isnull(num_2,0)+isnull(numth,0) then " & vbcrlf &_
		"case when isnull(num_3,0) = isnull(num_2,0) then 3 " & vbcrlf &_
		"when isnull(num_3,0)=0 then 1 " & vbcrlf &_
		"when isnull(num_3,0) < isnull(num_2,0) then 2 " & vbcrlf &_
		"else 1 end " & vbcrlf &_
		"when sc.contract is null then 3 " & vbcrlf &_
		"else 1 end )" & vbcrlf &_
		",zt_2 = (" & vbcrlf &_
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then  " & vbcrlf &_
		"case when isnull(num_4,0) = 0 then 0 else 1 end " & vbcrlf &_
		"when isnull(num_1,0)<=isnull(num_2,0)+isnull(numth,0) then " & vbcrlf &_
		"case when isnull(num_4,0) = isnull(num_2,0) then 2 " & vbcrlf &_
		"when isnull(num_4,0)=0 then 0 " & vbcrlf &_
		"when isnull(num_4,0) < isnull(num_2,0) then 1 " & vbcrlf &_
		"else 0 end "&_
		"when sc.contract is null then 2 " & vbcrlf &_
		"else 0 end )"&_
		",zt_3 = (" & vbcrlf &_
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then 1 " & vbcrlf &_
		"when isnull(num_1,0)<=isnull(num_2,0)+isnull(numth,0)or sc.contract is null  then 2 " & vbcrlf &_
		"else 0 " & vbcrlf &_
		"end )" & vbcrlf &_
		",num_1 as num1,num_2 as num2,num_3 as num3,num_4 as num4,numth "&_
		",isnull(sd.paybackMoney,(case isnull(sa.importPayback,0) when 0 then 0 else sa.money1 end)) paybackMoney" & vbcrlf &_
		",isnull(se.invoiceMoney,0) invoiceMoney , g.catespname , isnull(sp.sptime , sa.date7) as sptime,thmoney  [__px__tempcontract__]   " & vbcrlf &_
		"from contract sa "& vbcrlf&_
		" [__top__tempcontract__]  " & vbcrlf &_
		"left join ("&_
		"select a.contract," & vbcrlf &_
		"isnull(sum(a.num1),0) as num_1," & vbcrlf &_
		"sum(case when a.num2 =0 then 0 else isnull(a.num2,0) end) as num_2," & vbcrlf &_
		"sum(case when a.num3 =0 then 0 else isnull(a.num3,0) end) as num_3," & vbcrlf &_
		"sum(case when a.num4 =0 then 0 else isnull(a.num4,0) end) as num_4," & vbcrlf &_
		"isnull(sum(c.numth),0) as numth ,isnull(sum(c.thmoney),0) as thmoney "&_
		"from contractlist a WITH(NOLOCK) " & vbcrlf &_
		"inner join product b WITH(NOLOCK) on a.ord=b.ord and b.canoutstore=1 " &_
		"left join ( "&_
		"select l.contractlist,isnull(sum(l.money1),0) as thmoney," & vbcrlf &_
		"isnull(sum(case when l.unit=cl.unit then l.num1 else cast(l.num1*isnull(cc.bl,1) as decimal(25,12))/isnull(dd.bl,1) end),0) as numth "& vbcrlf &_
		"from contractthlist l WITH(NOLOCK) "& vbcrlf &_
		...
		"select distinct unit,product,bl from jiage WITH(NOLOCK) where bm=0" & vbcrlf &_
		") dd on dd.product=l.ord and dd.unit = cl.unit "& vbcrlf &_
		"where isnull(l.kuoutlist2,0)=0 " & vbcrlf &_
		"group by l.contractlist "& vbcrlf &_
		") c on isnull(c.contractlist,0)=a.id " & vbcrlf &_
		"group by a.contract " & vbcrlf &_
		") sc on sc.contract = sa.ord " & vbcrlf &_
		...
		"group by fromId " & vbcrlf &_
		") se on se.contract = sa.ord " & vbcrlf &_
		"left join (select ord gord ,name catespname from gate WITH(NOLOCK) where del=1) g on g.gord = sa.cateid_Sp " & vbcrlf &_
		"left join (SELECT MAX(date1) sptime ,ord as spord FROM sp_intro WITH(NOLOCK) WHERE sort1=2 GROUP BY ord ) sp on sp.spord = sa.ord " & vacrlf &_
		"where sa.del = 1 " & sqlcon & vbcrlf &_
		") as a " & Str_Result & fromContent & " and del = 1 and cateid <> 0"
	else
		sql= "select [__top__fields__] from(" & vbcrlf &_
		"select sa.*" & vbcrlf &_
		",isnull(sd.paybackMoney,(case isnull(sa.importPayback,0) when 0 then 0 else sa.money1 end)) paybackMoney" & vbcrlf &_
		",isnull(se.invoiceMoney,0) invoiceMoney" & vbcrlf &_
		",zt_1 = (" & vbcrlf &_
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then  " & vbcrlf &_
		"case when isnull(num_2,0) = 0 then 1 when isnull(num_3,0) = 0 then 5 else 2 end " & vbcrlf &_
		"when isnull(num_1,0)<=isnull(num_2,0)+isnull(cast(numth as decimal(25,12)),0) then " & vbcrlf &_
		"case when isnull(num_3,0) = isnull(num_2,0) then 3 " & vbcrlf &_
		"when isnull(num_3,0)=0 then 1 " & vbcrlf &_
		"when isnull(num_3,0) < isnull(num_2,0) then 2 " & vbcrlf &_
		"else 1 end " & vbcrlf &_
		...
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then  " & vbcrlf &_
		"case when isnull(num_4,0) = 0 then 0 else 1 end " & vbcrlf &_
		"when isnull(num_1,0)<=isnull(num_2,0)+isnull(cast(numth as decimal(25,12)),0) then "&_
		"case when isnull(cast(num_4 as decimal(25,12)),0) = isnull(num_2,0) then 2 " & vbcrlf &_
		"when isnull(num_4,0)=0 then 0 " & vbcrlf &_
		"when isnull(num_4,0) < isnull(num_2,0) then 1 " & vbcrlf &_
		"else 0 end " & vbcrlf &_
		"else 0 end )" & vbcrlf &_
		",zt_3 = (" & vbcrlf &_
		"case when isnull(num_1,0)>isnull(num_2,0)+isnull(numth,0) then 1 " & vbcrlf &_
		"when isnull(cast(num_1 as decimal(25,12)),0)<=isnull(cast(num_2 as decimal(25,12)),0)+isnull(cast(numth as decimal(25,12)),0) then 2 " & vbcrlf &_
		"else 0 end )" & vbcrlf &_
		",num_1 as num1,num_2 as num2,num_3 as num3,num_4 as num4,numth,g.catespname , isnull(sp.sptime , sa.date7) as sptime,thmoney  [__px__tempcontract__]   " & vbcrlf &_
		"from contract sa WITH(NOLOCK) "& vbcrlf&_
		" [__top__tempcontract__]  " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select a.contract," & vbcrlf &_
		...
		"select l.contractlist,isnull(sum(l.money1),0) as thmoney," & vbcrlf &_
		"isnull(sum(case when l.unit=cl.unit then l.num1 else cast(l.num1*isnull(cc.bl,1) as decimal(25,12))/isnull(dd.bl,1) end),0) as numth " & vbcrlf &_
		"from contractthlist l WITH(NOLOCK) "&_
		"inner join contractth t WITH(NOLOCK) on t.ord=l.caigou and (t.del<>2 and t.sp<>-1) " & vbcrlf &_
		"inner join contractlist cl WITH(NOLOCK) on cl.id = l.contractlist " & vbcrlf &_
		"left join (" & vbcrlf &_
		"select distinct unit,product,bl from jiage WITH(NOLOCK) where bm=0 " & vbcrlf &_
		...
		"select distinct unit,product,bl from jiage WITH(NOLOCK) where bm=0" & vbcrlf &_
		") dd on dd.product=l.ord and dd.unit = cl.unit " & vbcrlf &_
		"where isnull(l.kuoutlist2,0)=0 and t.del=1 and t.sp=0 group by l.contractlist " & vbcrlf &_
		") c on isnull(c.contractlist,0)=a.id " & vbcrlf &_
		"group by a.contract " & vbcrlf &_
		") sc on sc.contract = sa.ord " & vbcrlf &_
		"left join (" & vbcrlf &_
	...
	if lie_4=3 or lie_4=5 then
		formulaIdx = GetSetopenValue(2018030701, 2)
		summTaxValue=conn.execute("select isnull((" & vbcrlf &_
		"            ""select isnull(sum(taxValue),0) as taxValue from contractlist ctl where del=1 and exists(select top 1 1 from contract a WITH(NOLOCK) "" & iif(Str_Result&""""<>"""",Replace(Str_Result,""and isnull(paybackMoney,0) < money1"","""") &"" and a.ord=ctl.contract "",""where a.ord=ctl.contract"") & fromContent & "") ),0"")")(0).value
		summThTaxValue=conn.execute("select isnull((" & vbcrlf &_
		"select SUM(isnull(thl.taxValue,0)) taxValue "&_
		"  FROM contractth th WITH(NOLOCK)  "&_
		"  INNER JOIN contractthlist thl WITH(NOLOCK) ON thl.caigou=th.ord AND th.del=1 and thl.del=1 "&_
		"  INNER JOIN contractlist ctl WITH(NOLOCK) ON thl.contractlist=ctl.id AND thl.contract=ctl.contract "&_
		"          and exists (select top 1 1 from contract a WITH(NOLOCK) ""& iif(Str_Result&""<>"",Replace(Str_Result,""and isnull(paybackMoney,0) < money1"","") &"" and a.ord=ctl.contract "",""where a.ord=ctl.contract"") & "" & fromContent & "") "&_
		"),0)")(0).value
		if lie_4=3 then
			summTproCb=conn.execute("select isnull((SELECT SUM(money1) FROM (" & vbcrlf &_
			"SELECT ISNULL((CASE ISNULL(paybacktype,0) WHEN 0 THEN ( " & vbcrlf &_
			"          select isnull(sum(d.FinaMoney),0) as money1 from kuoutlist2 d WITH(NOLOCK) " & vbcrlf &_
			"          INNER JOIN kuout k WITH(NOLOCK) ON k.ord=d.kuout " & vbcrlf &_
			"          where k.sort1 in (1,4) and d.contract=a.ord and d.del=1)  " & vbcrlf &_
			"  WHEN 1 THEN (" & vbcrlf &_
			"          select isnull(sum(k.FinaMoney),0) money1 " & vbcrlf &_
			"          from kuoutlist2 k WITH(NOLOCK) " & vbcrlf &_
			"       INNER JOIN kuout kt WITH(NOLOCK) ON kt.ord=k.kuout " &_
			"          where (kt.sort1=1 or kt.sort1=4) and k.contract=a.ord and k.del=1)" & vbcrlf &_
			"  ELSE 0 END),0) money1 " & vbcrlf &_
			"   FROM contract a ""& Replace(Str_Result,""and isnull(paybackMoney,0) < money1"","")" & fromContent  & vbcrlf &_
			") a  ),0)")(0).value
		else
			summTproCb = conn.execute("select isnull((select sum(ctl.tpricejy) tpricejy from (select contract,sum(tpricejy) tpricejy from contractlist WITH(NOLOCK) where del=1 GROUP BY contract) ctl inner join contract a WITH(NOLOCK) on ctl.contract=a.ord " & Str_Result & " " & fromContent & " ),0)")(0).value
		end if
		summThMoney=conn.execute("select isnull((" & vbcrlf &_
		"            ""select SUM(thl.money1*dbo.gethl(thl.bz,thl.date1,getdate())) from contractth th WITH(NOLOCK) INNER JOIN contractthlist thl WITH(NOLOCK) ON thl.caigou=th.ord AND th.del=1 and thl.del=1 and exists (select top 1 1 from contract a WITH(NOLOCK) "" & iif(Str_Result&""""<>"""",Replace(Str_Result,""and isnull(""paybackMoney,0) < money1"","") &""and a.ord=thl.contract "",""where a.ord=thl.contract"")" & fromContent & ") " & vbcrlf &_
		"),0)")(0).value
		summSthMoney=conn.execute("select isnull((SELECT SUM(money1) FROM (" & vbcrlf &_
		"select (isnull((select SUM(thl.money1*dbo.gethl(thl.bz,thl.date1,getdate())) from contractth th WITH(NOLOCK) INNER JOIN contractthlist thl WITH(NOLOCK) ON thl.caigou=th.ord AND th.del=1 and thl.del=1 and thl.contract=a.ord ),0) - "&_
		"summSthMoney=conn.execute(""select isnull((SELECT SUM(money1) FROM (" & vbcrlf &_
		"isnull((select isnull(sum(thld.money2*dbo.gethl(th2.bz,th2.date3,getdate())),0) from contractthListDetail thld WITH(NOLOCK) "&_
		"inner join contractth th2 WITH(NOLOCK) on thld.contractth = th2.ord and th2.del=1 and th2.sp=0 "&_
		"and thld.thtype='GOODS' and thld.contract=a.ord),0) ) money1 " & vbcrlf &_
		"from contract a WITH(NOLOCK) " & Replace(Str_Result,"and isnull(paybackMoney,0) < money1","")& " " & fromContent & " " & vbcrlf &_
		") t  ),0)")(0).value
		summThCost=conn.execute("select isnull((SELECT SUM(money1) FROM (" & vbcrlf &_
		"select (case a.paybacktype when 1 then  " & vbcrlf &_
		"  (select isnull(sum(c.price1*d.num1),0) from kuoutlist2 c WITH(NOLOCK),contractthlist d WITH(NOLOCK),contractth e WITH(NOLOCK) where e.del=1 and d.caigou=e.ord and c.del=1 and d.del=1 and c.id=d.kuoutlist2 and d.contract=a.ord) " & vbcrlf &_
		"else   " & vbcrlf &_
		"  (select SUM(ISNULL((CASE ISNULL(c.kuinlist,0) WHEN 0 THEN c.price1*d.num1 ELSE convert(decimal(25,12),ROUND(REPLACE(r.price1,',',''),12))*d.num1 END),c.price1*d.num1)) " & vbcrlf &_
		"  FROM kuoutlist2 c WITH(NOLOCK) INNER JOIN contractthlist d WITH(NOLOCK) ON c.id=d.kuoutlist2 and c.del=1 and d.del=1 " & vbcrlf &_
		"  INNER JOIN contractth e WITH(NOLOCK) ON e.del=1 and e.ord=d.caigou and d.contract=a.ord " & vbcrlf &_
		"  LEFT JOIN kuinlist r WITH(NOLOCK) ON r.id=c.kuinlist AND r.del=1) " & vbcrlf &_
		"end) money1 from contract a WITH(NOLOCK) "& Replace(Str_Result,"and isnull(paybackMoney,0) < money1","") &" " & fromContent &""&_
		") t  ),0)")(0).value
		summThJinjiaCost=conn.execute("select sum(money1) "&_
		" from( "&_
		" select ( select isnull(sum(c.pricejy*d.num1),0) "&_
		" from contract b WITH(NOLOCK) "&_
		" inner join contractthlist d WITH(NOLOCK) on d.contract=b.ord and d.del=1 "&_
		" inner join contractth e WITH(NOLOCK) on d.caigou=e.ord and e.del=1 "&_
		" inner join contractlist c WITH(NOLOCK) on c.id=d.contractlist and c.del=1 "&_
		" where a.ord=b.ord ) money1 "&_
		" from contract a WITH(NOLOCK) " & Replace(Str_Result,"and isnull(paybackMoney,0) < money1","") & " " & fromContent & ""&_
		" ) t ")(0).value
		summoney1_mlall = getHtMlMoney2(formulaIdx,summoney2_htall,summoney1_hkall,summTaxValue,summThTaxValue,summTproCb,summoney1_fyall,summThMoney,summSthMoney,summThCost)
		summoney1_mlallJinjia = getHtMlMoney2(formulaIdx,summoney2_htall,summoney1_hkall,summTaxValue,summThTaxValue,summTproCb,summoney1_fyall,summThMoney,summSthMoney,summThJinjiaCost)
	end if
	dim px_sql : px_sql =  Replace(sql, "[__top__fields__]", " ord ,ROW_NUMBER()over("& px_Result &") inx into #htlist ")
	px_sql = replace(px_sql , "[__top__tempcontract__]" , "")
	px_sql = replace(px_sql , "[__px__tempcontract__]" , "")
	conn.execute(px_sql)
	Dim countsql : countsql = " select count(1) as recordcount from #htlist  "
	Set rs = conn.execute(countsql)
	If rs.eof = False Then
		rs_RecordCount =  rs(0).value
	end if
	rs.close
	Dim cols, outercols
	outercols = Split("cateid2,cateid3,sort1,repairOrderId,area,trade,zdy5,zdy6,zdy1,zdy2,zdy3,zdy4,intro,chance", ",")
	cols = "ord,title,htid,money1,money2,complete1,sort,date1,date2,date3,zt1,zt2,isnull(status,-1) status,cateid,addcate,cateid_sp,alt," &_
	"date7,isnull(company,0) as company,person1,paybacktype,sp,paybackMode,invoiceMode,isTerminated,bz,del,share,zt_1,zt_2,zt_3,"&_
	"paybackMoney,invoiceMoney,wxUserId,payKind,payStatus,catespname"
	For i = 0 To ubound(outercols)
		If InStr(1,Str_Result & fromContent & px_Result , outercols(i), 1) > 0 Then
			cols = cols & "," & outercols(i)
		end if
	next
	htbz="" : yhtbz = "" : bzintro = ""
	PageCount=rs_RecordCount \ page_count
	if rs_RecordCount mod page_count then PageCount=PageCount+1
	PageCount=rs_RecordCount \ page_count
	if currpage>=PageCount Then currpage=PageCount
	If CLng(currpage) <= 0 Then currpage = 1
	sql =  Replace(sql, "[__top__fields__]", cols , 1, 1, 1)
	sql = replace(sql , "[__top__tempcontract__]" , " inner join #htlist a on a.ord = sa.ord and inx>"& ((currpage - 1) * page_count) &" and inx<=" & (page_count * currpage) )   & " order by inx "
	'sql =  Replace(sql, "[__top__fields__]", cols , 1, 1, 1)
	sql = replace(sql , "[__px__tempcontract__]" , ", a.inx ")
	...
	if rs_RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "      <form name=""form1"" method=""post"" action=""delhtconfirm.asp?method=0&CurrPage="
		Response.write CurrPage
		Response.write "&gatety="
		Response.write (request("gatety")&"")
		Response.write "&W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		...
		Response.write "&I2="
		Response.write I2
		Response.write "&zdy1_1="
		Response.write zdy1_1
		Response.write "&zdy1_2="
		Response.write zdy1_2
		Response.write "&zdy2_1="
		Response.write zdy2_1
		Response.write "&zdy2_2="
		Response.write zdy2_2
		Response.write "&zdy3_1="
		Response.write zdy3_1
		Response.write "&zdy3_2="
		Response.write zdy3_2
		Response.write "&zdy4_1="
		Response.write zdy4_1
		Response.write "&zdy4_2="
		Response.write zdy4_2
		Response.write "&zdy5="
		Response.write zdy5
		Response.write "&zdy6="
		...
		Response.write fromPrice
		Response.write "&Telord="
		Response.write fromTel
		Response.write "&htTerminated="
		Response.write htTerminated
		Response.write "&paybackState="
		Response.write paybackState
		Response.write "&invoiceState="
		Response.write invoiceState
		Response.write "&adhtTerminated="
		Response.write adhtTerminated
		Response.write "&adCpName="
		Response.write adCpName
		Response.write "&adCpOrder1="
		Response.write adCpOrder1
		Response.write "&adCpType1="
		Response.write adCpType1
		...
		do until rs.eof
			If remind <> "" Then
				rid = helper.getRemindIdByOID(rs("ord"))
			end if
			k=rs("title") : contractTitle = k
			ord=rs("ord") : contractOrd = ord : contractOrd2 = pwurl(ord)
			htid=rs("htid")
			...
			zt1=rs("zt1") : yzt1=zt1
			zt2=rs("zt2") : yzt2=zt2
			cateid=rs("cateid") : ycateid = ""
			if cateid="" or isnull(cateid) then
				cateid=-1
'if cateid="" or isnull(cateid) then
			elseif cstr(cateid)="0" then
				cateid=-1
'elseif cstr(cateid)="0" then
			end if
			addcate=rs("addcate") : yaddcate = ""
			date7=rs("date7")
			company=rs("company")
			person1=rs("person1")
			paybacktype=rs("paybacktype")
			if conn.execute("select 1 from contract where ord="& ord & " and importPayback=1").eof=false then
				money_hkall=CDbl(rs("money1").value)
			else
				money_hkall = conn.execute("select isnull(sum(money1),0) as money1 from payback WITH(NOLOCK) where del=1 and complete>=3 and isnull(completeType,0)<>7 and contract="&ord)(0)
				If money_hkall&"" = "" Then money_hkall = 0 Else money_hkall = CDbl(money_hkall)
				money_hkml = conn.execute("select isnull(sum(money1),0) as money1 from payback WITH(NOLOCK) where del=1 and complete>=3 and isnull(completeType,0)=7 and contract="&ord)(0)
				If money_hkml&"" = "" Then money_hkml = 0 Else money_hkml = CDbl(money_hkml)
				money_hkall=money_hkall+money_hkml
'If money_hkml&"" = "" Then money_hkml = 0 Else money_hkml = CDbl(money_hkml)
			end if
			money_th = conn.execute("select isnull((select top 1 isnull(sum(d.money2),0) as thmoney from contractthListDetail d WITH(NOLOCK) inner join contractlist l WITH(NOLOCK) on l.id=d.contractlist inner join contractthlist tl WITH(NOLOCK) on tl.id=d.contractthlist inner join contractth ct WITH(NOLOCK) on ct.ord=tl.caigou and ct.del=1 and ct.sp=0 where d.del=1 and d.thtype = 'GOODS' and l.contract ="&ord&"),0) ")(0)
			paybackMode = rs("paybackMode")
			invoiceMode = rs("invoiceMode")
			isTerminated = rs("isTerminated") & ""
			spid=rs("sp")
			fromWx = CLng("0" & rs("wxUserId")) > 0
			if money1=0 then
				hk_bl=1
			else
				If money1-CDbl(money_th) <>0 Then
					hk_bl=1
					hk_bl=abs(money_hkall/(money1-CDbl(money_th)))
					hk_bl=1
				else
					hk_bl=1
				end if
			end if
			sql1 = "select hk_xz,name,sort3,share from tel WITH(NOLOCK) where ord="&company&""
			set rs1 = server.CreateObject("adodb.recordset")
			rs1.open sql1,conn,1,1
			if rs1.eof then
				hk_xz=0
				...
				if rs1("hk_xz")<>"" then
					hk_xz=cdbl(rs1("hk_xz"))/100
				else
					hk_xz=0
				end if
				companyname=rs1("name")
				companysort3 =rs1("sort3")
				share = rs1("share")
			end if
			rs1.close
			set rs1=nothing
			if CDbl(hk_bl)*100>=CDbl(hk_xz)*100 then
				ck_xz=1
			else
				ck_xz=0
			end if
			money_fyall=getHtFyMoney(ord)
			total_th=getHtThMoney(ord)
			total_th_ybz=getHtThYbzMoney(ord)
			if (lie_4=3 or lie_4=4) and (open_5_17=3 Or (open_5_17=1 and CheckPurview(intro_5_17,trim(cateid))=True)) then
				money_mlall=getHtMlMoney(ord)
				money_ll = getHtMlRate2(ord, money_mlall)
			else
				money_mlall = 0
				money_ll = 0
			end if
			if lie_4 = 5 or lie_4 = 6 then
				money_mlallJinjia = getHtMlMoneyX(ord, "jinjia")
				money_llJinjia = getHtMlRate2(ord, money_mlallJinjia)
			else
				money_mlallJinjia = 0 : money_llJinjia = 0
			end if
			zt1 = rs("zt_1")
			zt2 = rs("zt_2")
			status = rs("status")
			if yzt1=6 then zt1 = 3 : zt2 = 2
			zt3 = 0
			if (status = -1 or status=1) and  yzt1<>6 Then
				zt3 = rs("zt_3")
			end if
			if bz<>14 and bz>0 then
				summoney1_ht=summoney1_ht+cdbl(formatNumber(cdbl(money1),num_dot_xs,-1))
'if bz<>14 and bz>0 then
				summoney1_thAfter = summoney1_thAfter + cdbl(formatNumber(cdbl(money1),num_dot_xs,-1)) -  cdbl(formatNumber(cdbl(total_th_ybz),num_dot_xs,-1))
'if bz<>14 and bz>0 then
			else
				summoney1_ht=summoney1_ht+cdbl(formatNumber(cdbl(money2),num_dot_xs,-1))
'if bz<>14 and bz>0 then
				summoney1_thAfter = summoney1_thAfter + cdbl(formatNumber(cdbl(money2),num_dot_xs,-1)) -  cdbl(formatNumber(cdbl(total_th),num_dot_xs,-1))
'if bz<>14 and bz>0 then
			end if
			summoney1_hk=summoney1_hk+cdbl(formatnumber(cdbl(money_hkall),num_dot_xs,-1))
'if bz<>14 and bz>0 then
			summoney1_fy=summoney1_fy+cdbl(money_fyall)
'if bz<>14 and bz>0 then
			summoney1_ml=summoney1_ml+cdbl(formatnumber(cdbl(money_mlall),num_dot_xs,-1))
'if bz<>14 and bz>0 then
			summoney1_mll=summoney1_mll+cdbl(money_ll)
'if bz<>14 and bz>0 then
			summoney1_mlJinjia=summoney1_mlJinjia+cdbl(money_mlallJinjia)
'if bz<>14 and bz>0 then
			if  SH="3333333" and timelg<>"" and datediff("s",timelg,rs("date7"))>0 then
				k=k&new_list
			end if
			htbz = rs("bz")
			if htbz&""<>yhtbz&"" then
				set rs2=server.CreateObject("adodb.recordset")
				rs2.open "select intro from sortbz WITH(NOLOCK) where id="&htbz&"",conn,1,1
				if not rs2.eof Then
					If rs2("intro")="14" Then
						bzintro = "RMB"
					else
						bzintro = rs2("intro")
					end if
			...
			yhtbz = htbz
			Response.write "" & vbcrlf & "      <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td align=""center""><span class=""red"">" & vbcrlf & "         <input name=""selectid"" canck="""
			Response.write iif((zt1="1" or zt1="2" Or zt1 = "5") and zt3="1" and (status=-1 or status=1)  and ck_xz=1 and open_32_13 <> 0 and (open_32_13=3 or CheckPurview(intro_32_13,trim(cateid))=True) And isTerminated<>"1" ,"1","0")
			Response.write """  type=""checkbox"" id=""selectid"" value="""
			Response.write rs("ord")
			Response.write """>" & vbcrlf & "   </span></td>" & vbcrlf & "                <td align=""center"">" & vbcrlf & "                           <div align=""left"">" & vbcrlf & "                              "
			if lie_1=1 then
				if open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True then
					Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?ord="
					Response.write contractOrd2
					Response.write "&view=details','contractcon','width=' + 1000 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100').focus();return false;"" title="""
					'Response.write contractOrd2
					Response.write contractTitle
					Response.write """>"
				end if
				Response.write""&k&""
				Response.write "" & vbcrlf & "                                    </a><span id=""ci"
				Response.write rs("ord")
				Response.write """ class=""red""></span>" & vbcrlf & "                                                         "
			elseif lie_1=2 then
				if open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True then
					Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?ord="
					Response.write contractOrd2
			...
			elseif lie_1=3 then
				If companysort3 = "2" Then
					if open_26_1=3 or CheckPurview(intro_26_1,trim(cateid))=True then
						if open_26_14=3 or CheckPurview(intro_26_14,trim(cateid))=True then
							Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('../work2/content.asp?ord="
							Response.write pwurl(company)
							Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100').focus();return false;"" title="""
							'Response.write pwurl(company)
							Response.write contractTitle
							Response.write """>"
					...
					if open_1_1=3 or CheckPurview(intro_1_1,trim(cateid))=True or share="1" or CheckPurview(share&"",session("personzbintel2007")&"")=True then
						if open_1_14=3 or CheckPurview(intro_1_14,trim(cateid))=True then
							Response.write "" & vbcrlf & "                                                              <a href=""javascript:void(0)"" onclick=""javascript:window.open('../work/content.asp?ord="
							Response.write pwurl(company)
							Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100').focus();return false;"" title="""
							'Response.write pwurl(company)
							Response.write contractTitle
			...
			Response.write GetPrintInfo(conn, 11001 , contractOrd ,1)
			if sp1="1" then
				if conn.execute("select 1 from contract_his where ord="& contractOrd &" and del=1").eof=false then
					Response.write "<font color=red>(修改审批)</font>"
				end if
			end if
			Response.write "" & vbcrlf & "                     </div></td>" & vbcrlf & "      <td align="""
			If lie_2=1 Or lie_2=5 Then Response.write "right" Else Response.write "center" End If
			Response.write """  class=""name"">" & vbcrlf & ""
			if lie_2=1 Then
				if open_5_21<>0 Then
					Response.write bzintro & Formatnumber(money1,num_dot_xs,-1)
'if open_5_21<>0 Then
				else
					Response.write "&nbsp;"
				end if
			elseif lie_2=5 Then
				if open_5_21<>0 Then
					Response.write bzintro & Formatnumber(CDbl(money1)-CDbl(total_th_ybz),num_dot_xs,-1)
'if open_5_21<>0 Then
				else
					Response.write "&nbsp;"
				end if
			...
			Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "        <td align=""right""  class=""name"">" & vbcrlf & ""
			if lie_4=1 Then
				if open_5_21<>0 and (open_7_1=3 or (open_7_1=1 and CheckPurview(intro_7_1,trim(cateid))=True) ) Then
					Response.write Formatnumber(money_hkall,num_dot_xs,-1)
'if open_5_21<>0 and (open_7_1=3 or (open_7_1=1 and CheckPurview(intro_7_1,trim(cateid))=True) ) Then
				else
					Response.write "&nbsp;"
				end if
			elseif lie_4=2 Then
				Response.write Formatnumber(money_fyall,num_dot_xs,-1)
'elseif lie_4=2 Then
			elseif lie_4=3 Then
				if open_5_17=3 Or (open_5_17=1 and CheckPurview(intro_5_17,trim(cateid))=True) Then
					Response.write "RMB"& Formatnumber(money_mlall,num_dot_xs,-1)
'if open_5_17=3 Or (open_5_17=1 and CheckPurview(intro_5_17,trim(cateid))=True) Then
				end if
			elseif lie_4=4 Then
				if open_5_17=3 Or (open_5_17=1 and CheckPurview(intro_5_17,trim(cateid))=True) Then
					Response.write Formatnumber(money_ll,num_dot_xs,-1)
'if open_5_17=3 Or (open_5_17=1 and CheckPurview(intro_5_17,trim(cateid))=True) Then
				end if
			elseif lie_4=5 Then
				if open_5_24=1 Then
					Response.write "RMB"& Formatnumber(money_mlallJinjia,num_dot_xs,-1)
'if open_5_24=1 Then
				end if
			elseif lie_4=6 Then
'if open_5_24=1 Then
				Response.write Formatnumber(money_llJinjia,num_dot_xs,-1)
'if open_5_24=1 Then
			end if
'end if
			Response.write "" & vbcrlf & "             </td>" & vbcrlf & "        <td class=""name"" ><div align=""center"">" & vbcrlf & ""
			if lie_3=1 then
				if hasStoreModel and  hasKuoutModel then
					if zt1="0" then
						Response.write "未编辑明细"
					elseif zt1="1" then
						if zt3=2 then
							Response.write "申请完毕"
						end if
			...
			elseif lie_3=2 then
				...
				if complete1<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select sort1 from sortonehy WITH(NOLOCK) where ord="&complete1&""
					rs7.open sql7,conn,1,1
				...
				Response.write ""&completename&""
			elseif lie_3=3 Then
				sortname=""
				if complete1<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select sort1 from sortonehy WITH(NOLOCK) where ord="&sort&""
					rs7.open sql7,conn,1,1
					if rs7.eof = False then
						sortname=rs7("sort1")
					end if
			...
			elseif lie_3=4 Then
				if cateid&""<>"" and cateid&""<>ycateid&"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select name from gate WITH(NOLOCK) where ord="&cateid&""
					rs7.open sql7,conn,1,1
					if rs7.eof = False then
						catename=rs7("name")
					else
			...
			elseif lie_3=5 then
				if addcate<>"" and addcate&""<>yaddcate&"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select name from gate WITH(NOLOCK) where ord="&addcate&""
					rs7.open sql7,conn,1,1
					if rs7.eof = False Then
			...
			elseif lie_3=6 Then
				Response.write year(date7)&"-"&month(date7)&"-"&day(date7)
'elseif lie_3=6 Then
			elseif lie_3=7 Then
				Response.write ""&person1&""
			elseif lie_3=8 Then
				If rs("isTerminated") = 1 Then
					Response.write "已终止"
				else
					If zt1 = "3" And CDbl(rs("paybackMoney")) >= CDbl(rs("money1")) And (CDbl(rs("invoiceMoney")) >= CDbl(rs("money1")) Or invoiceMode = 0) Then
						Response.write "执行完毕"
					else
						Response.write "执行中"
					end if
			...
			elseif lie_3=9 Then
				catespname=rs("catespname")
				Response.write ""&catespname&""
			end if
			Response.write "" & vbcrlf & "               </div></td>" & vbcrlf & "           <td class=""func"">" & vbcrlf & "               <div align=""center"">" & vbcrlf & "                                  "
			If remind <> "" Then
				...
				if nowCate=rs("addcate") and rs("alt")=1  then
					Response.write "" & vbcrlf & "                               <a href=""../contract/setalt2.asp?ord="
					Response.write contractOrd
					Response.write "&W1="
					Response.write W1
					Response.write "&W2="
					Response.write W2
					...
					Response.write "&searchValue="
					Response.write searchValue
					Response.write """ target=""_self"" onclick=""return confirm('确认取消提醒？')"" ><img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0""></a>" & vbcrlf & "                             "
				end if
			end if
			if open_5_14=3 or CheckPurview(intro_5_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                               <input type=""button"" name=""Submit3"" value=""详情""  onClick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?ord="
				Response.write contractOrd2
				Response.write "&view=details','newwinxq','width=' + 1100 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100').focus()""/>" & vbcrlf & "                            "
				'Response.write contractOrd2
			end if
			canAddDesign = ((zt1="1" or zt1="2" Or zt1 = "5") and (status=-1 or status=1) And isTerminated<>"1")
			Response.write contractOrd2
			Call showDesignButton(conn, 2 ,contractOrd, canAddDesign , contractOrd2 , "list")
			if hasStoreModel and hasKuoutModel then
				if (zt1="1" or zt1="2" Or zt1 = "5") and zt3="1"  and (status=-1 or status=1) and ck_xz=1 and open_32_13 <> 0 and (open_32_13=3 or CheckPurview(intro_32_13,trim(cateid))=True) And isTerminated<>"1" then
'if hasStoreModel and hasKuoutModel then
					Response.write "" & vbcrlf & "                                                      <input type=""button"" name=""Submit3"" value=""出库""  onClick=""javascript:window.open('addlistck.asp?top="
					Response.write contractOrd2
					Response.write "','newwin','width=' + 1000 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100').focus();""/>"
					'Response.write contractOrd2
				end if
			end if
			if (open_5_4=3 or (open_5_4=1 and  CheckPurview(intro_5_4,trim(cateid))=True))  and (status=-1 or status=1) then
				Response.write contractOrd2
				Response.write "" & vbcrlf & "                  <input type=""button"" name=""Submit3c"" value=""共享"" onClick=""javascript:window.open('share.asp?ord="
				Response.write pwurl(ord)
				Response.write "&k=1','newwin77S','width=' + 800 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=120,top=120').focus();"" />" & vbcrlf & "                           "
				'Response.write pwurl(ord)
			end if
			if 1>2 and  (open_5_2=3 or (open_5_2=1 and  CheckPurview(intro_5_2,trim(cateid))=True)) then
				Response.write "" & vbcrlf & "                  <input type=""button"" name=""Submit3c"" value=""修改""  onClick=""javascript:if(!hwndxg||hwndxg.closed){hwndxg=window.open('../../SYSN/view/sales/contract/contract.ashx?ord="
				Response.write contractOrd2
				Response.write "','newwincor','width=' + 1024 + ',height=' + 768 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');}else{hwndxg.focus();}""/>" & vbcrlf & "                                 "
				'Response.write contractOrd2
			end if
			if 1>2 and (open_5_3=3 or (open_5_3=1 and  CheckPurview(intro_5_3,trim(cateid))=True)) then
				Response.write "" & vbcrlf & "                               <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""window.location.href='delhtconfirm.asp?method=0&ord="
				Response.write contractOrd
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "&gatety="
				...
				Response.write "&invoiceState="
				Response.write invoiceState
				Response.write "&rebackurl=planall.asp&adhtTerminated="
				Response.write adhtTerminated
				Response.write "&adCpName="
				Response.write adCpName
				Response.write "&adCpOrder1="
			...
			If open_5_26 = 1 And (spid=0 Or spid=-1) Then
				Response.write "'""/>" & vbcrlf & "              "
				If rs("isTerminated")=1 Then
					Response.write "" & vbcrlf & "                  <input type=""button"" value=""取消"" title=""取消终止"" onClick=""terminateContract("
					Response.write ord
					Response.write ",0);""/>" & vbcrlf & ""
				else
					Response.write "" & vbcrlf & "                  <input type=""button"" value=""终止"" title=""终止合同"" onClick=""terminateHtConfirm("
					Response.write ord
					Response.write ")""/>" & vbcrlf & ""
				end if
			end if
		...
		loop
		m=n
		Response.write "" & vbcrlf & "   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "               <td height=""27"" colspan=""2"" class=""name"" align=""right"">&nbsp;本页合计：</td>" & vbcrlf & "            <td align=""right"" class=""red"">"
		if lie_2=1 and open_5_21<>0 Then
			If bz>0 Then
				Response.write bzintro & Formatnumber(summoney1_ht,num_dot_xs,-1)
'If bz>0 Then
			else
				Response.write "RMB"& Formatnumber(summoney1_ht,num_dot_xs,-1)
'If bz>0 Then
			end if
		elseIf lie_2=5 and open_5_21<>0 Then
'If bz>0 Then
			Response.write bzintro & Formatnumber(summoney1_thAfter,num_dot_xs,-1)
'If bz>0 Then
		else
			Response.write "RMB"& Formatnumber(summoney1_thAfter,num_dot_xs,-1)
'If bz>0 Then
		end if
	end if
	Response.write "</td>" & vbcrlf & "         <td align=""right"" class=""red"">" & vbcrlf & "              "
	if lie_4=1 and open_5_21<>0 then
		If bz>0 Then
			Response.write bzintro
		else
			Response.write ""
		end if
		Response.write Formatnumber(summoney1_hk,num_dot_xs,-1)
		Response.write ""
	end if
	if lie_4=2 then
		Response.write Formatnumber(summoney1_fy,num_dot_xs,-1)
'if lie_4=2 then
	end if
	if lie_4=3 and (open_5_17=3 or open_5_17=1) then
		Response.write "RMB"
		Response.write Formatnumber(summoney1_ml,num_dot_xs,-1)
		'Response.write "RMB"
	end if
	if lie_4=5 and open_5_24=1 then
		Response.write "RMB"
		Response.write Formatnumber(summoney1_mlJinjia,num_dot_xs,-1)
		'Response.write "RMB"
	end if
	if lie_4=40 then
		If bz>0 Then
		...
		Response.write Formatnumber(summoney1_mll,num_dot_xs,-1)
		Response.write "RMB"
	end if
	Response.write "</td>" & vbcrlf & "         <td colspan=""2"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "         <td height=""27"" colspan=""2"" class=""name"" align=""right"">&nbsp;所有合计：</td>" & vbcrlf & "           <td align=""right"" class=""red"">"
	if lie_2=1 and open_5_21<>0 Then
		If bz>0 Then
			Response.write bzintro & Formatnumber(summoney2_htall,num_dot_xs,-1)
'If bz>0 Then
		else
			Response.write "RMB" & Formatnumber(summoney2_htall,num_dot_xs,-1)
'If bz>0 Then
		end if
	ElseIf lie_2=5 and open_5_21<>0 Then
		If bz>0 Then
			Response.write bzintro & Formatnumber(CDbl(summoney2_htall)-CDbl(summoney1_thall),num_dot_xs,-1)
'If bz>0 Then
		else
			Response.write "RMB" & Formatnumber(CDbl(summoney2_htall)-CDbl(summoney1_thall),num_dot_xs,-1)
'If bz>0 Then
		end if
	end if
	Response.write "</td>" & vbcrlf & "         <td align=""right"" class=""red"">" & vbcrlf & "              "
	if lie_4=1 and open_5_21<>0 then
		...
		Response.write Formatnumber(summoney1_hkall,num_dot_xs,-1)
		Response.write ""
	end if
	if lie_4=2 then
		Response.write Formatnumber(summoney1_fyall,num_dot_xs,-1)
'if lie_4=2 then
	end if
	if lie_4=3 and (open_5_17=3 or open_5_17=1) then
		Response.write "RMB"
		Response.write Formatnumber(summoney1_mlall,num_dot_xs,-1)
		'Response.write "RMB"
	end if
	if lie_4=5 and open_5_24=1 then
		Response.write "RMB"
		Response.write Formatnumber(summoney1_mlallJinjia,num_dot_xs,-1)
		'Response.write "RMB"
	end if
	if lie_4=40 then
		If bz>0 Then
		...
		Response.write Formatnumber(summoney1_mllall,num_dot_xs,-1)
		Response.write "RMB"
	end if
	Response.write "</td>" & vbcrlf & "         <td colspan=""2"">&nbsp;</td>" & vbcrlf & "         </tr>" & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "     </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" &vbcrlf & "    <td width=""8%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "    </div></td>" & vbcrlf & "    <td width=""40%"" >" & vbcrlf & "       "
	if open_5_6=1 or open_5_6=3 then
		Response.write "" & vbcrlf & "        <input type=""submit"" name=""Submit422"" value=""批量指派""  onclick=""ask();"" class=""anybutton2""/>" & vbcrlf & "   "
	end if
	if 1>2 and (open_5_3=1 or open_5_3=3) then
		Response.write "" & vbcrlf & "      <input type=""submit"" name=""Submit2"" value=""批量删除""  class=""anybutton2""/>" & vbcrlf & "    "
	end if
	if (open_5_4=1 or open_5_4=3) and sp1="" then
		Response.write "" & vbcrlf & "     <input type=""submit"" name=""Submit422"" value=""批量共享""  onclick=""ask5();"" class=""anybutton2""/>" & vbcrlf & "    "
	end if
	if ZBRuntime.MC(17003)then
		if (open_32_13=1 or open_32_13=3) and sp1=""  then
			Response.write "" & vbcrlf & "         <input type=""button""  value=""批量出库申请""  onclick=""plkuout();"" class=""anybutton2""/>" & vbcrlf & "        "
		end if
	end if
	Response.write "" & vbcrlf & "     </td>" & vbcrlf & "    <td width=""50%""><div align=""right"">" & vbcrlf & "    共"
	...
	Response.write "/"
	Response.write pagecount
	Response.write "页&nbsp;" & vbcrlf & "     <input name=""currpage"" id=""currpage"" type=text maxlength=""8"" onkeyup=""value=value.replace(/[^\d]/g,'')"" onpropertychange=""if(value.replace(/[^\d]/g,'') != value){value=value.replace(/[^\d]/g,'')}"" size=""3"">" & vbcrlf & "     &nbsp;<input type=""button"" name=""Submit422"" value=""跳转"" onclick=""gotourl('currPage='+document.getElementById('currpage').value);""  class=""anybutton2""/>" & vbcrlf & "          "
	Response.write pagecount
	if currpage=1 then
		...
		Response.write 1
		Response.write "');""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""gotourl('currPage="
	...
	Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  </form>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "var hwndsp=null,hwndcq=null,hwndxg=null;hwndtk=null;" & vbcrlf & "" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;"& vbcrlf & "}" & vbcrlf & "function ask() {" & vbcrlf & "document.all.form1.action = ""orderallhy.asp?currPage="
	Response.write currPage
	Response.write "&gatety="
	Response.write (request("gatety")&"")
	Response.write "&W1="
	...
	Response.write "&searchValue="
	Response.write searchValue
	Response.write """;" & vbcrlf & "document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function ask5() {" & vbcrlf & "document.all.form1.action = ""shareall.asp?currPage="
	Response.write currPage
	Response.write "&gatety="
	Response.write (request("gatety")&"")
	Response.write "&W1="
	...
	Response.write "&searchValue="
	Response.write searchValue
	Response.write """;" & vbcrlf & "document.all.form1.submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & " for (var i=0;i<form.elements.length;i++)" & vbcrlf & "        {" & vbcrlf & "       var e = form.elements[i];" & vbcrlf & "       if (e.name != 'chkall')" & vbcrlf & " e.checked = form.chkall.checked;" & vbcrlf & "    }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & "</table>" & vbcrlf & ""
'end if
	rs.close
	set rs=nothing
	action1="合同列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=374+""px"";/*原来的300不知道怎么来的，导致合同列表的排序规则显示的位置不对；*/" & vbcrlf & "       divID.style.top=0;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute; height:400;display:none;"">" & vbcrlf & "<table width=""150"" height=""300""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "              <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1');"">按添加时间排序(降)</a></td></tr>" & vbcrlf & "         <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2');"">按添加时间排序(升)</a> </td></tr>" & vbcrlf & "            <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3');"">按合同主题排序(降)</a> </td></tr>" & vbcrlf & "             <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4');"">按合同主题排序(升)</a> </td></tr>" & vbcrlf & "            <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5');"">按合同编号排序(降)</a></td></tr>" & vbcrlf & "         <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6');"">按合同编号排序(升)</a> </td></tr>" & vbcrlf & "            <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7');"">按签订日期排序(降)</a> </td></tr>" & vbcrlf & "             <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8');"">按签订日期排序(升)</a> </td></tr>" & vbcrlf & "            <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=9');"">按截止日期排序(降)</a> </td></tr>" & vbcrlf & "               <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=10');"">按截止日期排序(升)</a> </td></tr>" & vbcrlf & "           "
	If sp1="1" then
		Response.write "" & vbcrlf & "                     <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=11');"">按审批人员排序(降)</a> </td></tr>" & vbcrlf & "                   <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=12');"">按审批人员排序(升)</a> </td></tr>" & vbcrlf & "              "
	ElseIf sp1<>"1" And sp1<>"2" Then
		Response.write "" & vbcrlf & "                     <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=13');"">按审批时间排序(降)</a> </td></tr>" & vbcrlf & "                   <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=14');"">按审批时间排序(升)</a> </td></tr>" & vbcrlf & "              "
	end if
	Response.write "" & vbcrlf & "             </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<div id=""ZZDiv"" class=""easyui-window"" title=""温馨提示""  style=""width:490px;height:310px;padding:5px;background: #fafafa; display:none;""  closed=""true"" closable=""true""  collapsible=""false"" modal=""true""> " & vbcrlf & "      <div region=""center"" border=""false"" style=""width:465px; margin-top:5px;""> " & vbcrlf & "    <table id=""content"" width=""100%"" border=""0"" cellpadding=""5"" cellspacing=""1"" bgcolor=""#C0CCDD"">" & vbcrlf & "              <tr>" & vbcrlf & "                    <tdstyle=""padding:8px;line-height:20px;"">" & vbcrlf & "                       /a> </td></tr>" & vbcrlf & "              "
	htMStr = ""
	if hasKuoutModel Then htMStr = htMStr &"出库"
	if ZBRuntime.MC(23000) Then
		if htMStr&""<>"" then htMStr = htMStr &"、"
		htMStr = htMStr &"收款和开票"
	end if
	Response.write "" & vbcrlf & "                     终止合同后，此合同相关单据"
	Response.write iif(htMStr&""<>"","，如："& htMStr &"等","")
	Response.write "，均无法再生成新的单据，只能操作终止前生成的单据，您确定终止吗？" & vbcrlf & "                     <input type=""hidden"" id=""zzHtord"" value=""0"">" & vbcrlf & "                  </td>" & vbcrlf & "           </tr>" & vbcrlf & "           "
	if ZBRuntime.MC(23000) and open_7_19=0 Then
		Response.write "" & vbcrlf & "             <tr>" & vbcrlf & "                    <td><label><input type=""checkbox"" name=""delpeyback"" id=""delpeyback"" value=""1"" style=""vertical-align:middle;"">同时删除已生成的收款单</label></td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr>" & vbcrlf & "                    <td><label><input type=""checkbox"" name=""delInvoice"" id=""delInvoice"" value=""1"" style=""vertical-align:middle;"">同时删除已生成的开票单</label></td>" & vbcrlf & "              </tr>" & vbcrlf & "           "
'if ZBRuntime.MC(23000) and open_7_19=0 Then
	end if
	if hasKuoutModel and open_32_19=0 Then
		Response.write "" & vbcrlf & "             <tr>" & vbcrlf & "                    <td><label><input type=""checkbox"" name=""delkuout"" id=""delkuout"" value=""1"" style=""vertical-align:middle;"" onclick=""kuoutCheck()"">同时删除已生成的出库单</label></td>" & vbcrlf & "         </tr>" & vbcrlf & "           "
'if hasKuoutModel and open_32_19=0 Then
	end if
	if hasSendModel and open_33_19=0 Then
		Response.write "" & vbcrlf & "             <tr>" & vbcrlf & "                    <td><label><input type=""checkbox"" name=""delsend"" id=""delsend"" value=""1"" style=""vertical-align:middle;"" onclick=""sendCheck()"">同时删除已生成的发货单</label></td>" & vbcrlf & "            </tr>" & vbcrlf & "           "
'if hasSendModel and open_33_19=0 Then
	end if
	Response.write "" & vbcrlf & "     </table>        " & vbcrlf & "        </div> " & vbcrlf & " <div region=""south"" border=""false"" style=""text-align:right;height:25px;line-height:25px; margin-top:8px;""> " & vbcrlf & "           <a class=""easyui-linkbutton""  href=""javascript:;"" icon=""icon-ok"" onClick=""terminateSave();"">确定</a> " & vbcrlf & "         <a class=""easyui-linkbutton""  href=""javascript:;"" icon=""icon-cancel"" onClick=""jQuery('#ZZDiv').window('close')"">取消</a> " & vbcrlf & "       </div> " & vbcrlf & "</div>" & vbcrlf & ""
'if hasSendModel and open_33_19=0 Then
	Set BC = server.createobject(ZBRLibDLLNameSN & ".Base64Class")
	Response.write "" & vbcrlf & "<form target='_blank' action='' id='' method='post'>" & vbcrlf & "<input type='hidden' name=""sort"" value=7>" & vbcrlf & "<input type='hidden' name=""main"" value=""1"">" & vbcrlf & "<input type='hidden' name=""px"" id='px_1' value="""
	Response.write BC.SafeEnCode(px_Result)
	Response.write """>" & vbcrlf & "<input type='hidden' name=""ReportOrds"" id='ReportOrds_1' value='"
	Response.write BC.SafeEnCode(Str_Result&fromContent)
	...
	Response.write BC.SafeEnCode(sqlcon)
	Response.write "'>" & vbcrlf & ""
	Set BC =Nothing
	Response.write "" & vbcrlf & "</form>" & vbcrlf & "          <div id=""w""></div>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "    function checkAll(str){" & vbcrlf & "        var a=document.getElementById(""d""+str).getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""t""+str);" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            a[i].checked=b.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function fixChk(str){" & vbcrlf & "        var a=document.getElementById(""t1"").getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""d1"");" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                b.checked=false;" & vbcrlf & "                return ;" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        b.checked=true;" & vbcrlf & "    }" & vbcrlf & "  </script>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "    function checkAll2(str){" & vbcrlf & "        var a=document.getElementById(""u""+str).getElementsByTagName(""input""); "& vbcrlf &"         var b=document.getElementById(""e""+str); "& vbcrlf &"         for(var i=0;i<a.length;i++){ "& vbcrlf &"             a[i].checked=b.checked; "& vbcrlf &"         } "& vbcrlf &"     } "& vbcrlf & vbcrlf &"     function fixChk2(str){ "& vbcrlf &"         var a=document.getElementById(""u1"").getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""e1"");" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                b.checked=false;" & vbcrlf & "                return ;" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        b.checked=true;" & vbcrlf & "    }" & vbcrlf & "  </script>" & vbcrlf & "  <script type=""text/javascript"">" & vbcrlf & "    function checkAll3(str){" & vbcrlf & "        var a=document.getElementById(""h""+str).getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""i""+str);" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            a[i].checked=b.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function fixChk3(str){" & vbcrlf & "        var a=document.getElementById(""h1"").getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""i1"");" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                b.checked=false;" & vbcrlf & "               return ;" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        b.checked=true;" & vbcrlf & "    }" & vbcrlf & "  </script>" & vbcrlf & "  <script type=""text/javascript"">" & vbcrlf & "    function checkAll7(str){" & vbcrlf & "        var a=document.getElementById(""Wt""+str).getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""Wd""+str);" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            a[i].checked=b.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function fixChk7(str){" & vbcrlf & "var a=document.getElementById(""Wd1"").getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""Wt1"");" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                b.checked=false;" & vbcrlf & "return ;" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        b.checked=true;" & vbcrlf & "    }" & vbcrlf & "  </script>" & vbcrlf & "  <script type=""text/javascript"">" & vbcrlf & "    function checkAll4(str){" & vbcrlf & "        var a=document.getElementById(""j""+str).getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""k""+str);" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            a[i].checked=b.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "    function fixChk4(str){"& vbcrlf & "        var a=document.getElementById(""j1"").getElementsByTagName(""input"");" & vbcrlf & "        var b=document.getElementById(""k1"");" & vbcrlf & "        for(var i=0;i<a.length;i++){" & vbcrlf & "            if(a[i].checked==false){" & vbcrlf & "                b.checked=false;" &vbcrlf & "                return ;" & vbcrlf & "            }" & vbcrlf & "        }" & vbcrlf & "        b.checked=true;" & vbcrlf & "    }" & vbcrlf & "  </script>" & vbcrlf & "" & vbcrlf & "<script language=""javascript"">" & vbcrlf & ""
	if remind<>"" then
