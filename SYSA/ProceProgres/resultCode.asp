...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		Dim sql
		rpt.title= "工序质检结果条码"
		sql ="select 1 as 序号 , '通过' as 工序质检结果 ,'Y' as 质检结果代码 , '1' as 一维条形码 , '1' as 二维条形码 " &_
		"    UNION ALL " &_
		"    select 2 as 序号 , '返工' as 工序质检结果 ,'N' as 质检结果代码 , '2' as 一维条形码 , '2' as 二维条形码 " &_
		"    UNION ALL " &_
		"    select 3 as 序号 , '作废' as 工序质检结果 ,'S' as 质检结果代码 , '3' as 一维条形码 , '3' as 二维条形码 "
		rpt.sql = sql
		rpt.canExcel = False
		rpt.canPrint = False
		rpt.showtoppagesize = False
		rpt.nosearchBar = True
		rpt.buttonsHTML = "<input class='anybutton' style='top:7px;' type='button' onclick=""window.open('resultCodePrint.asp?codeType=128','imgurl1','width=' + 320 + ',height=' + 520 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150');"" value='一维码批量打印' id='subtn2'>&nbsp;"
	end sub
	Sub app_oncreate(lvw)
		lvw.cansort= False
		lvw.PageBar = False
		lvw.headers("一维条形码").formatText = "code:showImageCode(1 , @value , rs(""质检结果代码""))"
		lvw.headers("二维条形码").formatText = "code:showImageCode(2 , @value , rs(""质检结果代码""))"
	end sub
	Function showImageCode(stype ,v , code)
		Dim s ,imgurl
		If stype=2 Then
			imgurl = "../code2/view.asp?sn=view&ct=46&data=ZJJG:"& v &"&width=120&errorh=3"
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('../code2/viewImage.asp?title="& server.urlencode(code) &"&imgurl=" & server.urlencode(imgurl) & "','imgurl_2','width=' + 320 + ',height=' + 320 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')""><img width='30' title='质检结果二维码' src='"& imgurl &"' style='padding-top:10px;cursor:pointer'></a>"
'imgurl = "../code2/view.asp?sn=view&ct=46&data=ZJJG:"& v &"&width=120&errorh=3"
		else
			imgurl = "../code2/viewCode.asp?codeType=128&data=ZJJG:"& v &"&height=60"
			s = "<a href='javascript:void(0)' onclick=""javascript:window.open('../code2/viewImage.asp?codeType=128&title="& server.urlencode(code) &"&imgurl=" & server.urlencode(imgurl) & "&t="&now()&"','imgurl_1','width=' + 320 + ',height=' + 320 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')""><img width='100' height='30' title='质检结果一维码' src='"& imgurl &"' style='padding-top:10px;cursor:pointer'></a>"
'imgurl = "../code2/viewCode.asp?codeType=128&data=ZJJG:"& v &"&height=60"
		end if
		showImageCode = s
	end function
