...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		...
		Dim rs, dat
		Dim jinjiStr, jiedaiStr, width2 ,company,sTypeList
		width2 = 15
		...
		sTypeList = sTypeList & "联系人" & Chr(1) & 5 & Chr(2)
		sTypeList = sTypeList & "固定电话" & Chr(1) & 6 & Chr(2)
		sTypeList = sTypeList & "手机" & Chr(1) & 7 & Chr(2)
		sTypeList = sTypeList & "地址" & Chr(1) & 8 & Chr(2)
		...
		Dim topButtonHTML
		topButtonHTML = ""
		if app.power.existsPower(45,13) then
			topButtonHTML = topButtonHTML &"<input type='button' class='anybutton' style='width:80px;' onclick=""javascript:window.open('addsl.asp?ly=list','new2win','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value='受理单添加'>"
'if app.power.existsPower(45,13) then
		end if
		If app.power.existsPower(45,10) Then
			topButtonHTML = topButtonHTML &"<button class='oldbutton3' onclick='(new Listview(""mlistvw"")).cexcel(""维修受理单列表"")'>Excel导出</button>"
		end if
		...
		rpt.sql = "exec erp_report_wxsl_list "& Info.User &", &excelmode,&pagesize, &pageindex,&reportsort,@date1_0, @date1_1, @jinji, @jiedai, @zt1, @zt2, @searchType, @searckKey, "& company &""
	end sub
	Dim intro_45_2, intro_45_3, intro_45_8, intro_45_14, intro_1_1, intro_1_14,  intro_2_1,  intro_2_14, intro_21_1, intro_45_6
	...
	Sub app_oncreate(lvw)
		...
		lvw.addJoinFeilds "受理单主题,受理单编号"
		lvw.addJoinFeilds "派工状态,维修状态"
		lvw.addJoinFeilds "受理人员,受理时间"
		lvw.cansort = False
		Set col = lvw.headers(1)
		col.title = "选择"
		col.width = 40
		col.formattext = "code:GetCheckboxHtml(@value,""@cells[8]"",@cells[14])"
		Set col = lvw.headers(2)
		col.width = 220
		col.dbtype = "str"
		col.formattext = "code:GetSLTitleHtml(@cells[1],""@value"",@cells[14],""@cells[15]"")"
		Set col = lvw.headers(3)
		col.width = 220
		col.dbtype = "str"
		col.formattext = "code:GetSLidHtml(@cells[1],""@value"",@cells[14],""@cells[15]"")"
		Set col = lvw.headers(4)
		col.width = 240
		col.dbtype = "str"
		col.formattext = "code:GetCompanyHtml(""@value"",@cells[13],@cells[16])"
		Set col = lvw.headers(5)
		col.width = 180
		col.dbtype = "str"
		col.formattext = "code:GetPersonHtml(""@value"",@cells[17],@cells[18],""@cells[20]"",""@cells[21]"")"
		...
		col.width = 180
		col.formattext = "code:GetSLActHTML(@cells[1],@cells[14],""@cells[8]"")"
		...
		col.display = "none"
		btmStr = ""
		if app.power.existsPower(45,3)= true then
	...
	Sub App_OnListviewExcel(ByRef lvw)
		...
		re.Pattern="(\<img[^\<]*\>)"
		kzzdyStr = ""
		kzzdyStr = getKzzdyFileds(45)
		lvw.checkbox = False
		lvw.checkvalue="id"
		lvw.distinctSpaceCol = "受理单主题,受理单编号,派工状态,维修状态,关联客户,客户地址,关联联系人,固定电话,手机,紧急程度,接待方式,受理概要"& kzzdyStr &",受理人员,受理时间"
		For i =1 To lvw.headers.count
			lvw.headers(i).ignoreHTMLTag = True
		next
		lvw.headers(12).dbType = "str"
		If kzzdyStr&""<>"" Then
			arr_zdyStr = Split(kzzdyStr,",")
			For i = 1 To ubound(arr_zdyStr)
				If arr_zdyStr(i)&""<>"" Then
					lvw.headers.getItemByDbname(arr_zdyStr(i)).dbType = "str"
				end if
			next
		end if
		lvw.headers.getItemByDbname("故障描述").dbType = "str"
		lvw.headers.getItemByDbname("备注").dbType = "str"
	end sub
	Function getColHtml(mxid,colName,colValue,revID)
		Dim tempStr
		tempStr = ""
		If colValue&"" = "" Then colValue=""
		If colName="接件数量" Or colName="派工数量" Or colName="剩余数量" Or colName="维修费" Then
		...
		Case "接件数量", "派工数量", "剩余数量"
		If colValue&""<>"" Then
			tempStr = FormatNumber(colValue , num1_dot ,true,0,0)
		end if
		Case "维修费"
		If colValue&""<>"" Then
			tempStr = FormatNumber(colValue,num_dot_xs,true,0,0)
		end if
		End Select
		getColHtml = tempStr
	end function
	Function GetCheckboxHtml(slord,pgStatus,addcate)
		Dim tempStr : tempStr = ""
		If pgStatus = "未派工" Then
			If isEmpty(intro_45_3) Then
				intro_45_3 = app.power.GetPowerIntro(45,3)
			end if
			if intro_45_3="" or instr(","& intro_45_3 &"," , ","& addcate &",")>0 Then
				tempStr = "<input type=checkbox class='lvcbox' name='sys_lvw_ckbox' value='" & slord & "'>"
			end if
		end if
...
'if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
				If titleColor&""<>"" Then
					titleStr = titleStr &" style=''"
				end if
				titleStr = titleStr &">"& slTitle &"</a>"
			else
				If titleColor&""<>"" Then
					titleStr = "<font>"& slTitle &"</font>"
				else
					titleStr = slTitle
				end if
			end if
	...
	Function GetSLidHtml(slord,slTitle,addcate,titleColor)
		If slord&""<>"" And addcate&""<>"" Then
			Dim titleStr, rs
			titleStr = ""
			if slTitle&"" = "" then slTitle=""
	...
	Function GetPersonHtml(title1,lxrOrd,lxrCateid,phone,mobile)
		Dim titleStr
		titleStr = ""
...
'if intro_2_14="" or instr(","& intro_2_14 &"," , ","& lxrCateid &",")>0 then
			...
			If phone&""<>"" Then
				titleStr = titleStr &"<br>"& phone
			end if
			If mobile&""<>"" Then
				titleStr = titleStr &"<br>"& mobile
			end if
	...
	function GetSLActHTML(slord, addcate,pgStatus)
		dim actStr
		actStr = ""
		if slord&""<>"" then
...
'if intro_45_14="" or instr(","& intro_45_14 &"," , ","& addcate &",")>0 then
			...
			If (intro_45_3="" or instr(","& intro_45_3 &"," , ","& addcate &",")>0) And pgStatus&""="未派工" then
				if actStr&""<>"" then actStr = actStr & ""
