...
'if strW3<>"" Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/cr_List_repair_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & ""
	if open_102_1=3 then
		list=""
	elseif open_102_1=1 then
		list=" and rep_addcateid in ("&intro_102_1&")"
	else
		list=" and rep_addcateid=0"
	end if
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and rep_cateid in ("&W3&") "
	end if
	if D1<>"" then
		Str_Result=Str_Result&" and rep_type in ("&D1&")"
	end if
	if D2<>"" then
		Str_Result=Str_Result&" and car_code like '%"&ml.De(D2)&"%'"
	end if
	if D3<>"" then
		Str_Result=Str_Result&" and rep_company like '%"&ml.De(D3)&"%'"
	end if
	if D4<>"" then
		Str_Result=Str_Result&" and rep_reason like '%"&ml.De(D4)&"%'"
	end if
	if D5<>"" then
		Str_Result=Str_Result&" and rep_money>="&D5&""
	end if
	if D6<>"" then
		Str_Result=Str_Result&" and rep_money<="&D6&""
	end if
	if D7<>"" then
		Str_Result=Str_Result&" and rep_note like '%"&ml.De(D7)&"%'"
	end if
	if D8<>"" then
...
'if D27<>"" and D27<>"点击选择时间" then
	...
	if D28<>"" and D28<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&D28&"',rep_addtime)>=0 "
'if D28<>"" and D28<>"点击选择时间" then
	end if
	D29=ChangeDate(D29,2)
	if D29<>"" and D29<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,rep_addtime,'"&D29&"')>=0 "
'if D29<>"" and D29<>"点击选择时间" then
	end if
	if D30<>"" then
	...
	if M<>"" and M<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&M&"',a.rep_time)>=0 "
'if M<>"" and M<>"点击选择时间" then
	end if
	K=ChangeDate(K,2)
	if K<>"" and K<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,a.rep_time,'"&K&"')>=0 "
'if K<>"" and K<>"点击选择时间" then
	end if
	L1=ChangeDate(L1,1)
	if L1<>"" and L1<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&"',a.rep_time)>=0 "
'if L1<>"" and L1<>"点击选择时间" then
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" and L2<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,a.rep_time,'"&L2&"')>=0 "
'if L2<>"" and L2<>"点击选择时间" then
	end if
	ret=ChangeDate(ret,1)
	if ret<>"" and ret<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',rep_addtime)>=0 "
'if Exit Sub<>"" and Exit Sub<>"点击选择时间" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" and ret2<>"点击选择时间" then
'Str_Result=Str_Result+" and  datediff(s,rep_addtime,'"&ret2&"')>=0 "
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	end if
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="3"
	if px="1" then
		Ptr_Result=" order by rep_addcateid desc"
	elseif px="2" then
		Ptr_Result=" order by rep_addcateid asc"
	elseif px="3" then
		Ptr_Result=" order by rep_time desc"
	elseif px="4" then
		Ptr_Result=" order by rep_time asc"
	elseif px="5" then
		Ptr_Result=" order by b.car_code desc"
	elseif px="6" then
		Ptr_Result=" order by b.car_code asc"
	end if
	if C<>"" then
		if B="cph" then
			Str_Result=Str_Result&" and b.car_code like '%"&ml.De(C)&"%'"
		elseif B="yy" then
			Str_Result=Str_Result&" and rep_reason like '%"&ml.De(C)&"%'"
		elseif B="not" then
			Str_Result=Str_Result&" and rep_note like '%"&ml.De(C)&"%'"
		end if
	end if
	...
	Response.write ">车牌号</option>" & vbcrlf & "                             <option value=""yy"" "
	if B="yy" then Response.write "selected"
	Response.write ">维护原因</option>" & vbcrlf & "                           <option value=""not"" "
	if B="not" then Response.write "selected"
	Response.write ">维护备注</option>" & vbcrlf & "                   </select>" & vbcrlf & "                       <input type=""text"" name=""C"" value="""
	Response.write C
	...
	if open_102_10<>0 then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_repair.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span>                      </td>" & vbcrlf & "           </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""7%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">车牌号</div></td>" & vbcrlf &               "<td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">经办人员</div></td>" & vbcrlf &                  "<td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">维护日期</div></td>" & vbcrlf & "            <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">维护类型</div></td>" & vbcrlf & "                <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">维护费用</div></td>" & vbcrlf & "                 <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">维护原因</div></td>" & vbcrlf & "                 <td width=""16%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">维护备注</div></td>" & vbcrlf & "                <td width=""13%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">" & vbcrlf & "                <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">"& vbcrlf & "                        <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	allmoney=0
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.*,b.car_code,c.setName,d.name from O_repair a"&_
	" left join O_carData b on a.rep_carid=b.car_id "&_
	" left join O_carSet c on a.rep_type=c.id and setType=4"&_
	" left join gate d on a.rep_cateid=d.ord "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if not rs7.eof then
		do while not rs7.eof
			allmoney=allmoney+cdbl(rs7("rep_money"))
'do while not rs7.eof
			rs7.movenext
		loop
	end if
	...
	if rs7.RecordCount<=0 then
		...
		o=0
		do while not rs7.eof
			rep_addcateid=rs7("rep_addcateid")
			money1=zbcdbl(rs7("rep_money"))
			...
			rep_cateid=rs7("rep_cateid")
			if rep_cateid="" or isnull(rep_cateid) then rep_cateid=0
			sql8="select * from gate where ord="&rep_cateid
			...
			Response.write rs7("rep_time")
			Response.write "</div></td>" & vbcrlf & "              <td width=""11%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			rep_type=rs7("rep_type")
			if rep_type="" or isnull(rep_type) then rep_type=0
			sql8="select * from O_carSet where setType=4 and id="&rep_type
			set rs8=conn.execute(sql8)
			if not rs8.eof then
			...
			if ((open_102_2=3 or CheckPurview(intro_102_2,trim(rep_addcateid))=True)) then
				Response.write "" & vbcrlf & "                       <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_repair.asp?ord="
				Response.write rs7("rep_id")
				Response.write "&px="
				Response.write px
			...
			if open_102_3=3 or CheckPurview(intro_102_3,trim(rep_addcateid))=True then
				Response.write "" & vbcrlf & "                      <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_repair.asp?ord="
				Response.write rs7("rep_id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	Response.write "" & vbcrlf & "              <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "             <td height=""30"" colspan=""5"" align=""left"" valign=""middle""  class=""name""><div align=""right"">本页合计：</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
	Response.write Formatnumber(pagemoney,num_dot_xs,-1)
'align=""middle"" class=""func""><div align=""center"">"
	Response.write "</div></td>" & vbcrlf & "             <td height=""30"" colspan=""5"" align=""left"" valign=""middle"" class=""func"">&nbsp;</td>" & vbcrlf & "            </tr>" & vbcrlf & "              <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "             <td height=""30"" colspan=""5"" align=""left"" valign=""middle""  class=""name""><div align=""right"">全部合计：</div></td>" & vbcrlf & "              <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
	Response.write Formatnumber(allmoney,num_dot_xs,-1)
	Response.write "</div></td>" & vbcrlf & "             <td height=""30"" colspan=""5"" align=""left"" valign=""middle"" class=""func"">&nbsp;</td>" & vbcrlf & "            </tr>" & vbcrlf & "     </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "      <td class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf &      "<tr>" & vbcrlf &           "<td width=""60"" height=""30"">全选<input name=""chkall"" type=""checkbox"" id=""chkall"" value=""chkall"" onClick=""mm(this.form)""></td>" & vbcrlf &           "<td><input name=""submit6"" type=""button"" IsShowProcessBar=""True""  class=""anybutton"" id=""submit6""  style=""cursor:hand""  value=""批量删除"" onClick=""testall()""/></td>" & vbcrlf & "   </form>" & vbcrlf & "       <form name=""oneself"" action=""List_repair.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "             " & vbcrlf & "                </div>" & vbcrlf & "          </td>" & vbcrlf & "           </form>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td colspan=""3"" height=""3000""></td>" & vbcrlf & "         </tr>" & vbcrlf & " </table>" & vbcrlf & "        </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:200; height:150;display:none;"">" & vbcrlf & "<table width=""150"" height=""150""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""150"">" & vbcrlf & "        <table width=""150"" height=""150"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按添加时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "        <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按维护时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按维护时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按车牌号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" style=""padding-left:20px;""><a href=""###"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按车牌号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	rs7.close
	set rs7=nothing
	action1="车辆维护列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？'))" & vbcrlf & " {" & vbcrlf & "  window.location=""List_repair.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&zdy6="
	Response.write zdy6
	Response.write """;" & vbcrlf & " }" & vbcrlf & " else" & vbcrlf & " {" & vbcrlf & "       document.getElementById('data').action=""Del_repair.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
