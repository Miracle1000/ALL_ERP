...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	Str_Result=Str_Result&" and D_del=1 "
	Ptr_Result=""
	px=request("px")
	if px="" or isnull(px) then px="1"
	if px="1" then
		Ptr_Result=" order by D_addtime desc"
	elseif px="2" then
		Ptr_Result=" order by D_addtime asc"
	elseif px="3" then
		Ptr_Result=" order by D_time desc"
	elseif px="4" then
		Ptr_Result=" order by D_time asc"
	elseif px="5" then
...
'if qjret2<>"" then
	...
	i=1
	k=0
	sql="select a.*,convert(varchar(7),a.ass_jttime,120) as jttime,b.ass_name,b.ass_cycle,b.ass_money,b.ass_money2,b.ass_jcz,b.ass_ycycle,c.sort1 from O_assDeprect a left join O_asset b on a.D_assID=b.ass_id left join O_assSetname c on b.ass_type=c.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "计提折旧列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
...
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money"))),num_dot_xs,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).Value =Formatnumber(ml.IsNum(zbcdbl(rs("ass_money2"))),num_dot_xs,-1)
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,4).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).Value =rs("ass_jcz")
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,5).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).Value =rs("jttime")
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).font.Size=10
'if not rs.eof then
			xlWorksheet.Cells(1+i,6).HorizontalAlignment=1
'if not rs.eof then
			xlWorksheet.Cells(1+i,7).Value =Formatnumber(ml.IsNum(zbcdbl(rs("D_ymoney"))),num_dot_xs,-1)
'if not rs.eof then
...
'if not rs.eof then
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>计提折旧列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!计提折旧列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("计提折旧列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的计提折旧列表</u></strong></font></a></p>"
