...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	xlWorksheet.Cells(1,2).Value = "名称"
	...
	if open_67_10= 3 then
		Str_Result=""
	elseif open_67_10= 1 then
		Str_Result=" and (addcate in("&intro_67_10&")) "
	else
		Str_Result=" and 1=2"
	end if
	if strid<>"" then
	...
	C1=rs.recordcount
	xlApplication.Init "短信地址_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof Or response.IsClientConnected = False
		...
		if addcate<>"" then
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select * from gate where ord="&addcate&""
			rs1.open sql1,conn,1,1
			dim cateid
			if rs1.eof then
				cateid=""
		...
		cateid=""
		xlWorksheet.Cells(1+i,2).Value = name
		cateid=""
		xlWorksheet.Cells(1+i,2).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,3).Value = sex
		cateid=""
		xlWorksheet.Cells(1+i,3).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,4).Value = job
		cateid=""
		xlWorksheet.Cells(1+i,4).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,5).Value = sortTitle
		cateid=""
		xlWorksheet.Cells(1+i,5).font.Size=10
		cateid=""
		xlWorksheet.Cells(1+i,6).Value = name2
		cateid=""
		...
		cateid=""
		xlWorksheet.Cells(1+i,8).Value = addtime
		cateid=""
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>短信地址导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!短信地址导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("短信地址_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的短信地址</u></strong></font></a></p> " & vbcrlf & ""
