...
'Public Function AddFormValue(name,  value)
	...
	dotstr=""
		for i=1 to num_dot_xs
			dotstr=dotstr&"0"
			next
			s = "发货主题,发货编号,发货状态,发货方式,"
			If ZBRuntime.MC(217008) Then s = s & "快递公司,快递单号,快递费用,"
			s = s & "发货人,应发日期,收货人,实发日期,客户地址,联系电话,手机,关联客户/供应商,关联合同/采购退货"
			arrFields = Split(s,",")
			For i = 0 To ubound(arrFields)
				Width = 10
				HAment = 1
				Select Case arrFields(i)
				Case "发货主题" : Width = 25
				Case "发货状态,发货方式","发货人" ,"收货人": HAment = 3
				Case "发货编号" : Width = 15
				Case "应发日期","实发日期" ,"联系电话","手机","快递单号","快递公司" : Width = 12
				Case "客户地址","关联客户/供应商","关联合同/采购退货":  Width = 20
				Case "快递费用" : xlApplication.ActiveSheet.Columns(i+1).NumberFormatLocal = "#,##0."&dotstr&"_ "
				Case "客户地址","关联客户/供应商","关联合同/采购退货":  Width = 20
				End Select
				xlApplication.ActiveSheet.Columns(i+1).ColumnWidth=Width
'End Select
				xlApplication.ActiveSheet.Columns(i+1).HorizontalAlignment=1
'End Select
				xlWorksheet.Cells(1,i+1).Value = arrFields(i)
'End Select
...
'End Select
			next
			m=ubound(arrFields)+1
'End Select
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select title from zdy where sort1=33 and set_open=1 and dc=1 order by gate1 asc "
			rs1.open sql1,conn,1,1
			if not rs1.eof then
				do until rs1.eof
					xlApplication.ActiveSheet.Columns(m+1).ColumnWidth=15
'do until rs1.eof
					xlApplication.ActiveSheet.Columns(m+1).HorizontalAlignment=3
'do until rs1.eof
					xlWorksheet.Cells(1,m+1).Value = rs1("title")
'do until rs1.eof
					xlWorksheet.Cells(1,m+1).font.Size=10
'do until rs1.eof
					xlWorksheet.Cells(1,m+1).font.bold=true
'do until rs1.eof
					m=m+1
'do until rs1.eof
					rs1.movenext
			...
			rs1.close
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+1).ColumnWidth=20
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+1).HorizontalAlignment=1
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+2).ColumnWidth=10
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+2).HorizontalAlignment=1
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+3).ColumnWidth=10
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+3).HorizontalAlignment=1
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+4).ColumnWidth=12
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+4).HorizontalAlignment=1
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+5).ColumnWidth=15
			set rs1=nothing
			xlApplication.ActiveSheet.Columns(m+5).HorizontalAlignment=1
			set rs1=nothing
			xlWorksheet.Cells(1,m+1).Value = "交货地址"
			set rs1=nothing
			xlWorksheet.Cells(1,m+1).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(1,m+1).font.bold=true
			set rs1=nothing
			xlWorksheet.Cells(1,m+2).Value = "付款方式"
			set rs1=nothing
			xlWorksheet.Cells(1,m+2).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(1,m+2).font.bold=True
			set rs1=nothing
			xlWorksheet.Cells(1,m+3).Value = "交货方式"
			set rs1=nothing
			xlWorksheet.Cells(1,m+3).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(1,m+3).font.bold=True
			set rs1=nothing
			xlWorksheet.Cells(1,m+4).Value = "交货时间"
			set rs1=nothing
			xlWorksheet.Cells(1,m+4).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(1,m+4).font.bold=true
			set rs1=nothing
			xlWorksheet.Cells(1,m+5).Value = "配件"
			set rs1=nothing
			xlWorksheet.Cells(1,m+5).font.Size=10
			set rs1=nothing
			xlWorksheet.Cells(1,m+5).font.bold=true
			set rs1=nothing
			Response.Flush
			...
			receiver=request("shouhuoperson")
			phone=request("shouhuotel")
			mobile=request("shouhuomobile")
			SearchType = request("SearchType")
			ksjs3=request("ksjs3")
			ksjs4=request("ksjs4")
			ksjs5=request("ksjs5")
			if len(SearchType&"")=0 then SearchType = 0
			if SearchType = "0" then
				datetype=request("datetype")
				if(request("Type"))<>"1" then
				if len(ksjs3&"") = 0 then ksjs3 = year(now)&"-"&month(now)&"-01"
'if(request("Type"))<>"1" then
				if len(ksjs4&"") = 0 then ksjs4 = date()
			end if
			if len(ksjs3) > 0 and isdate(ksjs3) then m1 = ksjs3
			if len(ksjs4) > 0 and isdate(ksjs4) then m2 = ksjs4
		end if
		if(request("Type"))="2" then
		m1=""
		m2=""
	end if
	if len(ksjs5) > 0 Then Prt = ksjs5
	...
	W4=replace(W3,"0","")
	W4=replace(W4,",","")
	if receiver<>"" then Str_Result=Str_Result + " and sh in(select ord from person where del=1 and name like '%"&receiver&"%')"
	W4=replace(W4,",","")
	if phone<>"" then Str_Result=Str_Result + " and sh in(select ord from person where del=1 and phone like '%"&phone&"%')"
	W4=replace(W4,",","")
	if mobile<>"" then Str_Result=Str_Result + " and sh in(select ord from person where del=1 and mobile like '%"&mobile&"%')"
	W4=replace(W4,",","")
	if datetype&""="sfrq" then
		if m1<>"" then Str_Result=Str_Result + " and date2>='"&m1&" 00:00:00'"
'if datetype&""="sfrq" then
		if m2<>"" then Str_Result=Str_Result + " and date2<='"&m2&" 23:59:59'"
'if datetype&""="sfrq" then
	else
		if m1<>"" then Str_Result=Str_Result + " and date1>='"&m1&" 00:00:00'"
'if datetype&""="sfrq" then
		if m2<>"" then Str_Result=Str_Result + " and date1<='"&m2&" 23:59:59'"
'if datetype&""="sfrq" then
	end if
	if W4<>"" then
...
'case 2:
		case 3:
		Str_Result=Str_Result+"and  (cateid in  ("&W3&") or addcate in ("&W3&"))"
'case 3:
		case 4:
		Str_Result=Str_Result+"and  (cateid in  ("&W3&") and addcate in ("&W3&"))"
'case 4:
		end select
	end if
	if companyid&""<>"" and companyid>0 then
		str_Result=str_Result+" and company in ("&companyid&") "
'if companyid&""<>"" and companyid>0 then
	end if
	area_list=-1
'if companyid&""<>"" and companyid>0 then
...
'elseif G1=6 then
	...
	if P2<>"" then
		if P1=1 then
			str_Result=str_Result+" and code like '%"& P2 &"%'"
'if P1=1 then
		elseif P1=2 then
			str_Result=str_Result+" and code not like '%"& P2 &"%'"
'elseif P1=2 then
		elseif P1=3 then
			str_Result=str_Result+" and code='"& P2 &"'"
'elseif P1=3 then
		elseif P1=4 then
			str_Result=str_Result+" and code<>'"& P2 &"'"
'elseif P1=4 then
...
'elseif P1=6 then
	...
	if H2<>"" then
		if H1=1 then
			str_Result=str_Result+" and order1 in (select ord from contract where title like '%"& H2 &"%')"
'if H1=1 then
		end if
	end if
	If K2 <> "" Then
		If K1 = 1 Then
			str_Result=str_Result+" AND sh IN (SELECT ord FROM person WHERE name LIKE '%"& K2 &"%') "
'If K1 = 1 Then
		end if
	end if
	if zdy12<>"" then
		if zdy1=1 then
			str_Result=str_Result+" and zdy1 like '%"& zdy12 &"%'"
'if zdy1=1 then
		elseif zdy1=2 then
			str_Result=str_Result+" and zdy1 not like '%"& zdy12 &"%'"
'elseif zdy1=2 then
		elseif zdy1=3 then
			str_Result=str_Result+" and zdy1='"& zdy12 &"'"
'elseif zdy1=3 then
		elseif zdy1=4 then
			str_Result=str_Result+" and zdy1<>'"& zdy12 &"'"
'elseif zdy1=4 then
...
'elseif zdy1=6 then
	...
	if zdy22<>"" then
		if zdy2=1 then
			str_Result=str_Result+" and zdy2 like '%"& zdy22 &"%'"
'if zdy2=1 then
		elseif zdy2=2 then
			str_Result=str_Result+" and zdy2 not like '%"& zdy22 &"%'"
'elseif zdy2=2 then
		elseif zdy2=3 then
			str_Result=str_Result+" and zdy2='"& zdy22 &"'"
'elseif zdy2=3 then
		elseif zdy2=4 then
			str_Result=str_Result+" and zdy2<>'"& zdy22 &"'"
'elseif zdy2=4 then
...
'elseif zdy2=6 then
	...
	if zdy32<>"" then
		if zdy3=1 then
			str_Result=str_Result+" and zdy3 like '%"& zdy32 &"%'"
'if zdy3=1 then
		elseif zdy3=2 then
			str_Result=str_Result+" and zdy3 not like '%"& zdy32 &"%'"
'elseif zdy3=2 then
		elseif zdy3=3 then
			str_Result=str_Result+" and zdy3='"& zdy32 &"'"
'elseif zdy3=3 then
		elseif zdy3=4 then
			str_Result=str_Result+" and zdy3<>'"& zdy32 &"'"
'elseif zdy3=4 then
...
'elseif zdy3=6 then
	...
	if zdy42<>"" then
		if zdy4=1 then
			str_Result=str_Result+" and zdy4 like '%"& zdy42 &"%'"
'if zdy4=1 then
		elseif zdy4=2 then
			str_Result=str_Result+" and zdy4 not like '%"& zdy42 &"%'"
'elseif zdy4=2 then
		elseif zdy4=3 then
			str_Result=str_Result+" and zdy4='"& zdy42 &"'"
'elseif zdy4=3 then
		elseif zdy4=4 then
			str_Result=str_Result+" and zdy4<>'"& zdy42 &"'"
'elseif zdy4=4 then
...
'if zdy6<>"" then
	...
	if a<>"" then
		If propm="1" Then
			...
			Str_Result= Str_Result + " and ((complete1=0 and ("& open_33_16 &"=3 or ("& open_33_16 &"=1 and charindex(','+cast(cateid as varchar(10))+',',',"& intro_33_16 &",')>0))) or (complete1=1 and alt is null and cateid="&session("personzbintel2007")&"))"
			If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
		else
			Str_Result=Str_Result + " and charindex(','+cast( isnull(complete1,0) as varchar(10))+',',',"&Replace(A," ","")&",')>0 "
			If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
		end if
	end if
	if B<>"" then Str_Result=Str_Result+" and  sort in  ("&sdk.FormatNumList(B)&") "
	If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
	If remind <> "" Then
	...
	"(select sort1 from sortonehy  where ord=send.sort) fhfs," &_
	...
	" from send  "&Str_Result&" "&px_Result&" "
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "发货列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			dim k,ord,id,sorttype,fhfs
			id=rs("ord")
			ord=rs("company")
			k=rs("title")
			cateid=rs("cateid")
			...
			WayMoney =zbcdbl( rs("WayMoney"))
			If WayMoney&""="" Then WayMoney = 0
			ecname = ""
			if ecid<>"" then
				set rs7=conn.execute("select sort1 from sortonehy  where ord="&ecid&"")
				if rs7.eof then
					ecname=""
				else
					ecname=rs7("sort1")
			...
			If sh<>"0" Or sh<>"" Then
				Set rs7=server.CreateObject("adodb.recordset")
				rs7.open "select name,phone,mobile from person where ord="&sh&" ",conn,1,1
				If Not rs7.eof Then
					name_sh=rs7("name")
					phone_sh=rs7("phone")
					mobile_sh=rs7("mobile")
				else
					name_sh=""
					phone_sh=""
					mobile_sh=""
			...
			if rs("order1")<>"0" and rs("order1")<>"" Then
				If sorttype="2" Then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select * from caigouth where ord="&rs("order1")&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						order1="关联采购退货已被删除"
						order1ord=0
					...
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select * from contract where ord="&rs("order1")&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						order1="关联合同已被删除"
						order1ord=0
			...
			if ord<>"0" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select * from tel where ord="&ord&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
					If sorttype="2" Then
						company="关联供应商已被删除"
			...
			if sorttype="2" Then
				sql_qx=open_75_1
				sql_qx_intro=intro_75_1
			else
				sql_qx=open_5_1
				sql_qx_intro=intro_5_1
			end if
			hasPower = True
			if sort3="2" Then
				sql_qx2=open_26_1
				sql_qx_intro2=intro_26_1
				hasPower = ( sql_qx2=3 or (sql_qx2=1 And CheckPurview(sql_qx_intro2,trim(cateid_kh))=True))
			else
				sql_qx2=open_1_1
				sql_qx_intro2=intro_1_1
				hasPower = ( sql_qx2=3 or (sql_qx2=1 And CheckPurview(sql_qx_intro2,trim(cateid_kh))=True) Or share = "1" Or CheckPurview(share&"",trim(session("personzbintel2007")))=True)
			end if
			if hasPower = False Then company=""
			if sql_qx=3 Or (CheckPurview(sql_qx_intro,Trim(cateid))=True And sql_qx=1) Then
			else
				order1=""
			end if
			addsendid=rs("cateid")
			senderid=rs("addcate")
			if isnull(addsendid) or addsendid="" then
				addsendname=""
			else
				sql="select name from gate where ord=" & addsendid
				set rssender=conn.execute(sql)
				if rssender.eof then
					addsendname=""
				else
					addsendname=rssender(0)
				end if
				rssender.close
			end if
			senderid=rs("addcate")
			if isnull(senderid) or senderid="" then
				sendname=""
			else
				sql="select name from gate where ord=" & senderid
				set rssender=conn.execute(sql)
				if rssender.eof then
					sendname=""
				else
			...
			For n = 0 To ubound(arrFields)
				v = ""
				Select Case arrFields(n)
				Case "发货主题" : v = k
				Case "发货编号" : v = code
				Case "发货状态" : v = fhzt
				Case "发货方式" : v = fhfs
				Case "快递公司" : v = ecname
				Case "快递单号" : v = WaybillNumber
				Case "快递费用" : v = Formatnumber(WayMoney,num_dot_xs,-1)
				Case "快递单号" : v = WaybillNumber
				Case "发货人" : v = sendname
				Case "应发日期" : v = date1
				Case "收货人" : v = name_sh
				Case "实发日期" : v = date7
				Case "客户地址" : v = address
				Case "联系电话" : v = phone_sh
				Case "手机" : v = mobile_sh
				Case "关联客户/供应商" : v = company
				Case "关联合同/采购退货": v = order1
				End Select
				xlWorksheet.Cells(1+i,n+1).Value = v
'End Select
				xlWorksheet.Cells(1+i,n+1).font.Size=10
'End Select
			next
			m = ubound(arrFields)+1
'End Select
			set rs88=server.CreateObject("adodb.recordset")
			sql88="select id,name,sort,gl from zdy where sort1=33 and set_open=1 and dc=1 order by gate1 asc "
			rs88.open sql88,conn,3,1
			if not rs88.eof then
				do until rs88.eof
					if rs88("sort")=1 then
						zdy=rs(""&rs88("name")&"")
						if IsNull(zdy) or zdy="" then
							zdyname=""
						else
							set rs7=server.CreateObject("adodb.recordset")
							sql7="select ord,sort1 from sortonehy where ord="&zdy&""
							rs7.open sql7,conn,1,1
							if rs7.eof then
								zdyname=""
							else
						...
						xlWorksheet.Cells(1+i,m+1).Value = zdyname
						set rs7=nothing
						xlWorksheet.Cells(1+i,m+1).font.Size=10
						set rs7=nothing
					else
						xlWorksheet.Cells(1+i,m+1).Value = rs(""&rs88("name")&"")
						set rs7=nothing
						xlWorksheet.Cells(1+i,m+1).font.Size=10
						set rs7=nothing
					end if
			...
			rs88.close
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+1).Value = intro4
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+1).font.Size=10
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+2).Value = intro3
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+2).font.Size=10
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+2).HorizontalAlignment=1
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+3).Value = intro5
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+3).font.Size=10
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+3).HorizontalAlignment=1
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+4).Value = intro6
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+4).font.Size=10
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+5).Value = intro1
			set rs88=nothing
			xlWorksheet.Cells(1+i,m+5).font.Size=10
			set rs88=nothing
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>发货列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!发货列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("发货列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的发货列表</u></strong></font></a></p>"
