...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body,td,th {" & vbcrlf & "       color: #000000;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" src=""../Inc/DelUnusedFilesForMoban.js?ver="""
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script>" & vbcrlf & "function hideEditorPlus(){" & vbcrlf & "      $(window.frames[""wfasdg""].document).find(""#SizePlus"").hide();" & vbcrlf & "       $(window.frames[""wfasdg""].document).find(""#SizeMinus"").hide();" & vbcrlf & "      $(window.frames[""wfasdg""].document).find(""#StatusBarbg"").find(""td[align='right']"").hide();" & vbcrlf & "}" & vbcrlf & vbcrlf & "$(function(){"     & vbcrlf &         "setTimeout(""hideEditorPlus()"",500)"         & vbcrlf & "})" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td width=""100%"" valign=""top"">" & vbcrlf & ""
...
'if id<>"" then
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select top 1 *  from contract_out Where  main<>1 and sort="&sort&" "&S_result&" order by main desc"
	...
	rs1.close
	set rs1=nothing
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=2 and sort2=13 and ord="&session("personzbintel2007"))
	...
	Set BC = nothing
	Str_Result = reportords
	Dim pageindex, pagesize, pagecount, recordcount
	...
	recordcount = conn.execute("select count(*) as r from person as a left join tel on tel.ord=a.company where 1=1 "&Str_Result).fields(0).value
	pagecount = Int(recordcount/pagesize) + abs(recordcount mod pagesize > 0)
	If pageindex*1 > pagecount*1 Then pageindex = pagecount*1
	set rs=server.CreateObject("adodb.recordset")
	sql="select top "&pagesize&" a.* from person as a left join tel on tel.ord=a.company where a.ord not in (select top "&pagesize*abs(pageindex-1)&" a.ord from person as a left join tel on tel.ord=a.company where 1=1 "&replace(Str_Result,"'","'")&" order by a.date7 desc,a.name asc) "&replace(Str_Result,"'","'")&" order by a.date7 desc,a.name asc"
	rs.open sql,conn,1,1
	if rs.eof then
	else
		j=1
		...
		do until rs.eof
			company=rs("company")
			If company="" Or isnull(company) Then company=0
			set rst=server.CreateObject("adodb.recordset")
			rst.open "select * from tel where ord="&company&"",conn,3,1
			If Not rst.eof then
				kh_companyname=rst("name")
				kh_order1=rst("khid")
				kh_phone=rst("phone")
				kh_fax=rst("fax")
				kh_address=rst("address")
				kh_zip=rst("zip")
				kh_email=rst("email")
				areaid=rst("area")
				...
				if not rsa.eof then
					...
					areaid=rsa(0)
					do
						set rst1=conn.execute("select id1,menuname from menuarea where id=" & areaid)
						if rst1.eof then exit do
						kh_area=rst1(1) & " " & kh_area
						areaid=rst1(0)
						rst1.close
						set rst1=nothing
					loop
				...
				kh_zdy5=rst("zdy5")
				kh_zdy6=rst("zdy6")
				sharecontect=rst("sharecontact")
				kh_cateid=rst("cateid")
			end if
			rst.close:Set rst=nothing
			if kh_cateid<>"" and kh_cateid<>"0" and kh_cateid<>0  And not isnull(kh_cateid) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			if kh_zdy5<>"" And not isnull(kh_zdy5) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			if kh_zdy6<>"" And not isnull(kh_zdy6) then
				set rs7=server.CreateObject("adodb.recordset")
			...
			intro=replace(intro,"<H2></H2>", ss33)
	...
	Response.write sort
	Response.write """ id=""demo"" onsubmit=""return Validator.Validate(this,2)&&DelUnusedFilesBeforeSubmit()"" name=""date"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td height=""30"" colspan=""2""><div style='float:right'>" & vbcrlf & "          <textarea name=""content"" style=""display:none"" cols=""1"" rows=""1"">"
	if content<>"" then Response.write content
	Response.write "" & vbcrlf & "            </textarea>" & vbcrlf & "              <select name=""select5"" id=""sid"" onChange=""check_moban_dy(this)"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "        <option value='0'>-选择打印模板-</option>" & vbcrlf & "                           "
	if content<>"" then Response.write content
	If id="" Then id=0
	...
	loop
	rs7.close
	Response.write "" & vbcrlf & "                             </select>" & vbcrlf & "          <input type=""submit"" name=""Submit42"" value=""确认打印"" class=""anybutton2""/>" & vbcrlf & "        </div><div>" & vbcrlf & "                        <table>" & vbcrlf & "                 <tr>" & vbcrlf & "                            <td></td>" & vbcrlf & "                               <td>每页打印信封数量：</td>" & vbcrlf & "                             <td>" & vbcrlf & "                                       <select onchange='psize()' id='scount'>" & vbcrlf & "                                 "
	...
	Response.write "</td>" & vbcrlf & "                                </td>" & vbcrlf & "                           <td>页</td>" & vbcrlf & "                     </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                </div></td>" & vbcrlf & "        </tr>" & vbcrlf & "        <tr>" & vbcrlf & "            <td colspan=""2"" height=""29"" align=""right"" style=""color:#5b7cae"">" & vbcrlf & "                        纸张类型："
...
'If Len(exists_Pics) = 0 Then
	...
	conn.close
	set conn=nothing
	Response.write "" & vbcrlf & " </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr>" & vbcrlf & "<td class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30""><div align=""center""></div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table >" & vbcrlf & ""
	Set BC= server.createobject(ZBRLibDLLNameSN & ".Base64Class")
	Response.write "" & vbcrlf & "<form action='moban_dy.asp' id='mailprint' method='post'>" & vbcrlf & "<input type='hidden' id=""id"" name=""id"" value="""">" & vbcrlf & "<input type='hidden' name=""sort"" value=7>" & vbcrlf & "<input type='hidden' name=""main"" value=1>" & vbcrlf & "<input type='hidden' name=""ReportOrds"" id='ReportOrds_1' value="""
	...
	Set BC = nothing
	Response.write "" & vbcrlf & "<script>" & vbcrlf & "function psize()" & vbcrlf & "{" & vbcrlf & "    document.getElementById(""pageindex"").value=document.getElementById(""sindex"").value;" & vbcrlf & " document.getElementById(""pagesize"").value=document.getElementById(""scount"").value;" & vbcrlf & "  document.getElementById(""id"").value=document.getElementById(""sid"").value;" & vbcrlf & "        document.getElementById(""mailprint"").submit();" & vbcrlf & "}" & vbcrlf & "function check_moban_dy(obj){" & vbcrlf & "      if(obj.selectedIndex && obj.selectedIndex!=0){" & vbcrlf & "          document.getElementById(""id"").value=obj.value;" & vbcrlf & "              document.getElementById(""mailprint"").submit();" & vbcrlf & "    }" & vbcrlf & "       obj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>"
