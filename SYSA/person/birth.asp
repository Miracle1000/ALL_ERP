...
'bigsystemtype = ".mozi"
	...
	elseif open_2_1=1 then
		list="and cateid in ("&intro_2_1&")"
	else
		list="and cateid=0"
	end if
	dim rs,sql,list,Str_Result,Str_Result2
...
'Exit For
	...
	End Class
	ZBRLibDLLNameSN = "ZBRLib3205"
	Sub toNongli(days,GongliStr,WeekdayStr,NongliStr,NongliDayStr)
		If not isdate(days) Then
			days="1921-2-8"
'If not isdate(days) Then
		end if
		Dim obj : Set obj = server.createobject(ZBRLibDLLNameSN & ".NCalendar")
		Call obj.toNongli(days,GongliStr,WeekdayStr,NongliStr,NongliDayStr)
		Set obj = nothing
	end sub
	Response.Charset="UTF-8"
	Response.ExpiresAbsolute = Now() - 1
...
'Private Sub Class_Initialize
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<script src=""../inc/system.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	s3=request("s3")
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="& uid &" and sort1=2 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_2_1=0
		intro_2_1=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="& uid &" and sort1=26 and sort2=14"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_26_14=0
		intro_26_14=0
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "       <tr>" & vbcrlf & "            <td class=""place"">生日提醒        </td>" & vbcrlf & "           <td>&nbsp;<a href=""#"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img class=""resetElementHidden"" src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""><img class=""resetElementShowNoAlign"" style=""display:none"" src=""../skin/default/images/MoZihometop/content/r_down2.png"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "         <td align=""right"">"
	If conn.execute("select count(1) from nldata").fields(0).value <> 73050 Then
		Response.write "<span style='color:red'>没有找到农历和公历对应数据，请执行数据库升级程序解决!</span>&nbsp;"
	end if
	Response.write "</td>" & vbcrlf & "         <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "</table>  " & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "       <tr class=""top no-padding-bottom"">" & vbcrlf & "     <td align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "     <td height=""27"" align=""center""><div align=""center""><strong>编号</strong></div></td>" & vbcrlf & "   <td width=""8%"" height=""27""><div align=""center""><strong>姓名</strong></div>            <div align=""center""></div></td>" & vbcrlf & "         <td width=""9%""><div align=""center"">生日</div></td>" & vbcrlf & "          <td ><div align=""center""><strong>距离生日</strong></div></td>" & vbcrlf & "     <td height=""27"" ><div align=""center""><strong>来源</strong></div></td>" & vbcrlf & "      <td width=""12%"" height=""27""><div align=""center""><strong>电话</strong></div></td>" & vbcrlf & "    <td height=""27"" ><div align=""center""><strong>所有者</strong></div></td>" & vbcrlf & "     <td width=""18%"" height=""27"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "   </tr>" & vbcrlf & ""
	tq7=0
	ordCondition = ""
	Dim helper
	Set helper = CreateReminderHelper(conn,7,0)
	If request("remind")<>"" Then
		ordCondition = " and ord in (" & helper.listSQL("ids") & ")"
	end if
	if s=1 or s=2 then
		sql9="select * from setjm  where ord=7  and cateid="& uid
	else
		sql9="select * from setjm2  where ord=7  and cateid="& uid
	end if
	...
	rs9.close
	set rs9=Nothing
	If Len(intro_2_1 & "")= 0 Then intro_2_1 = "-222"
	set rs9=Nothing
	if alert_op=1 Then
		if fw1=0 then
			if open_2_1=3 then
				Str_Result_7=""
			elseif open_2_1=1 then
				Str_Result_7=" and a.company in (select ord from tel where cateid in ("&intro_2_1&") and cateid>0 or share='1' or charindex(',"&uid&",',','+share+',')>0)"
'elseif open_2_1=1 then
			else
				Str_Result_7=" and a.company in (select ord from tel where share='1' or charindex(',"&uid&",',','+share+',')>0)"
'elseif open_2_1=1 then
			end if
		else
			Str_Result_7=" and a.company in (select ord from tel where cateid="&uid&" or share='1' or charindex(',"&uid&",',','+share+',')>0)"
'elseif open_2_1=1 then
		end if
	else
		if open_2_1=3 then
...
'elseif open_2_1=1 then
	...
	Dim nyear,nyear2
	conn.execute("if object_id('Tempdb..#person"& uid &"') is not null drop table #person"& uid &"")
	nyear=conn.execute("select top 1 ny from nldata where yl=CONVERT(varchar(10),GETDATE(),23)")(0)
	nyear2=conn.execute("select top 1 ny from nldata where yl=CONVERT(varchar(10),GETDATE()+180,23)")(0)
	'nyear=conn.execute("select top 1 ny from nldata where yl=CONVERT(varchar(10),GETDATE(),23)")(0)
	conn.execute("update person set alt=0 where ord in (select ord from (select ord,(case when birthdayType=2 or birthdayType=3 then (select top 1 yl from nldata where ny="&nyear&" and nm=month(person.year1) and nd=day(person.year1)) else case when isdate(cast(year(GETDATE()) as varchar(4))+N'-'+cast(MONTH(person.year1) as varchar(2))+N'-'+cast(day(person.year1) as varchar(2)))=1 then cast(cast(year(GETDATE()) as varchar(4))+N'-'+cast(MONTH(person.year1) as varchar(2))+N'-'+cast(day(person.year1) as varchar(2)) as datetime) else cast('2000-1-1' as datetime) end end) as birth from person where isdate(year1)=1 and isnull(birthdaytype,1) in (1,2,3)) PersonBirthYL where isdate(birth)=1 and isnull(datediff(d,getdate(),birth),-1)<0)")
	'nyear=conn.execute("select top 1 ny from nldata where yl=CONVERT(varchar(10),GETDATE(),23)")(0)
	conn.execute ("select *,isnull(datediff(d,getdate(),birth),-1) as days into #person"& uid &" from ("&_
	"select ord,(case when isnull(datediff(d,getdate(),birth),-1)>=0 then birth else case when birthdayType=2 or birthdayType=3 then (select top 1 yl from nldata where ny="&nyear2&" and nm=month(PersonBirthYL.year1) and nd=day(PersonBirthYL.year1)) else case when isdate(cast(year(GETDATE()+180) as varchar(4))+N'-'+cast(MONTH(year1) as varchar(2))+N'-'+cast(day(year1) as varchar(2)))=1 then cast(cast(year(GETDATE()+180) as varchar(4))+N'-'+cast(MONTH(year1) as varchar(2))+N'-'+cast(day(year1) as varchar(2)) as datetime) else cast('2000-1-1' as datetime) end end end ) as birth,birthdaytype from ("&_
	"select ord,(case when birthdayType=2 or birthdayType=3 then (select top 1 yl from nldata where ny="&nyear&" and nm=month(person.year1) and nd=day(person.year1)) else case when isdate(cast(year(GETDATE()) as varchar(4))+N'-'+cast(MONTH(person.year1) as varchar(2))+N'-'+cast(day(person.year1) as varchar(2)))=1 then cast(cast(year(GETDATE()) as varchar(4))+N'-'+cast(MONTH(person.year1) as varchar(2))+N'-'+cast(day(person.year1) as varchar(2)) as datetime) else cast('2000-1-1' as datetime) end end) as birth ,isnull(birthdaytype,1) as birthdaytype,year1 from person where isdate(year1)=1 and isnull(birthdaytype,1) in (1,2,3) " & ordCondition & ""&_
	") PersonBirthYL"&_
	") PersonBirth where isdate(birth)=1 and isnull(datediff(d,getdate(),birth),-1)>=0")
	...
	elseif px=5 Then
		px_Result1="order by #person"& uid &".birth desc,a.ord asc"
		px_Result2="order by #person"& uid &".birth asc,a.ord desc"
	elseif px=6 Then
		px_Result1="order by #person"& uid &".birth asc,a.ord asc"
		px_Result2="order by #person"& uid &".birth desc,a.ord desc"
	elseif px=7 Then
		px_Result1="order by #person"& uid &".days desc,a.ord asc"
		px_Result2="order by #person"& uid &".days asc,a.ord desc"
	elseif px=8 Then
		px_Result1="order by #person"& uid &".days asc,a.ord asc"
		px_Result2="order by #person"& uid &".days desc,a.ord desc"
	elseif px=9 then
	...
	date2 = trim(replace(hlxNongGong.GongToNong(date()),"闰",""))
	if instr(hlxNongGong.GongToNong(date()),"闰")>0 then
		flag=1
	else
		flag=0
	end if
	date_alt=dateadd("d",tq7,date)
	date_alt2=hlxNongGong.dayAddNongli(date2,tq7,flag)
	dim n7
	n7=0
	t = date
	propm = request("propm")
	if propm = "" then propm = 0
	If propm = 1 Then
		Str_Result_7 = Str_Result_7 & " and #person" &  uid  & ".days>=0 and #person" &  uid  & ".days<=" & tq7
	else
		If s=1 Or s=3 Then
			Str_Result_7 = Str_Result_7 & " and year(getdate())=year(#person"& uid &".birth) and month(getdate())=month(#person"& uid &".birth) and day(getdate())=day(#person"& uid &".birth)"
		Else
			Str_Result_7 = Str_Result_7 & " and year(getdate())=year(#person"& uid &".birth) and month(getdate())=month(#person"& uid &".birth) and day(getdate())<=day(#person"& uid &".birth)"
		end if
	end if
	if s3<>"" then
		Str_Result_7=Str_Result_7 & " and a.sort3="&s3&" "
	end if
	sqlcount="select count(*) from person a " &_
	"left join #person"& uid &" on a.ord=#person"& uid &".ord "&_
	"left join tel on tel.ord=a.company " &_
	"where a.del=1 and a.year1 is not null "&Str_Result_7
...
'if currpage=pagecount then
	...
	sql="select * from ("&_
	"select top "&page_count&" person.*,isnull((select top 1 cateid from tel where ord=person.company),0) as addcate,"&_
	"#person"& uid &".days "&_
	"from person "&_
	"left join tel on person.company=tel.ord "&_
	"left join #person"& uid &" on #person"& uid &".ord=person.ord "&_
	"where person.ord in ("&_
	"select top "&intcount&" person.ord from person "&_
	"left join tel on person.company=tel.ord "&_
	"left join #person"& uid &" on #person"& uid &".ord=person.ord "&_
	"where person.ord in ("&_
	"select top "&page_count*CurrPage&" a.ord from person a "&_
	"left join tel on a.company=tel.ord "&_
	"left join #person"& uid &" on a.ord=#person"& uid &".ord "&_
	"where a.del=1 and a.year1 is not null "&replace(Str_Result_7,"'","'")&" "&px_Result1&_
	") " & Replace(px_Result2,"a.","person.") &_
	")) aa "
	...
	if rs.eof=true then
		...
		i=0
		Response.write "" & vbcrlf & "   <form name=""form1"" id=""data"" method=""post"" action=""deleteall2.asp?CurrPage="
		...
		do until rs.eof
			...
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select name,cateid from tel where ord="&rs("company")&" "
			rs1.open sql1,conn,1,1
			dim zmr_sort  : zmr_sort=0
			if rs("sort")&""<>"" then zmr_sort=rs("sort")
			if  zmr_sort=7 and isnull(rs("sort1")) then
				name2="备注：私人好友"
			else
				if rs1.eof  then
					name2="此联系人对应公司已被删除"
			...
			year1=rs("year1")
			if year1>"1900" then
				set hlxNongGong2 = New hlxNongLiGongLi
				if rs("birthdayType")=3 then
					year1=hlxNongGong2.getYearStr(year1)&"年闰"&hlxNongGong2.NongliMonth(hlxNongGong2.getMonthStr(year1))&"月"&hlxNongGong2.NongliDay(hlxNongGong2.getDayStr(year1))
				elseif rs("birthdayType")=2 then
					year1=hlxNongGong2.getYearStr(year1)&"年"&hlxNongGong2.NongliMonth(hlxNongGong2.getMonthStr(year1))&"月"&hlxNongGong2.NongliDay(hlxNongGong2.getDayStr(year1))
				end if
				set hlxNongGong2 = nothing
			end if
			depDay=rs("days")
			...
			if open_2_3=3 or CheckPurview(intro_2_3,trim(cateid))=True then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>"
			end if
			Response.write "</td> " & vbcrlf & "      <td width=""6%"" align=""center"">"
			Response.write recCount-page_count*(currpage-1)-i
			'Response.write "</td> " & vbcrlf & "      <td width=""6%"" align=""center"">"
			Response.write "</td>" & vbcrlf & "      <td height=""28"" class=""name""><div align=""center"">"
			if open_2_14=3 or CheckPurview(intro_2_14,trim(cateid))=True then
				Response.write "<a href=""#"" onclick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=100,top=100');return false;"" title=""点击可查看此联系人详情"">"
				'Response.write pwurl(rs("ord"))
			end if
			Response.write "<font class=""name"">"
			Response.write rs("name")
			Response.write "</font></a>" & vbcrlf & ""
			if helper.canCancelOrder(rId) And request("remind")<>"" Then
				Response.write "" & vbcrlf & "                     <a href=""javascript:void(0);"" class=""cancelBtn"" rid="""
				Response.write rId
				Response.write """><img src=""../images/alt3.gif"" alt=""取消提醒"" border=""0""></a>" & vbcrlf & ""
			end if
			Response.write "" & vbcrlf & "                     </div></td>" & vbcrlf & "      <td height=""28"" class=""name""><div align=""center"">"
			Response.write year1
			Response.write "</div></td>" & vbcrlf & "      <td width=""14%""><div align=""center"">" & vbcrlf & "        "
			if depDay=0 then
				Response.write "" & vbcrlf & "        今日生日" & vbcrlf & "        "
			else
				Response.write "" & vbcrlf & "        还差"
				Response.write depDay
				Response.write "天" & vbcrlf & "        "
			end if
			Response.write "" & vbcrlf & "      </div></td>" & vbcrlf & "      <td width=""18%""><div align=""left"">" & vbcrlf & "      "
			if rs("sort3")=1 then
				if open_1_14=3 or (open_1_14=1 and CheckPurview(intro_1_14,trim(cateid))=True) then
					...
					Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此客户详情"">"
					'Response.write pwurl(rs("company"))
				end if
			...
			Response.write "</div></td>" & vbcrlf & "                  <td width=""9%"" class=""name""><div align=""center"">"
			...
			if open_2_2=3 or CheckPurview(intro_2_2,trim(cateid))=True then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3c"" value=""修改""  onClick=""javascript:window.open('correct.asp?cc=2&ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwincor','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "                       "
				'Response.write pwurl(rs("ord"))
			end if
			if open_2_3=3 or CheckPurview(intro_2_3,trim(cateid))=True then
				Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete2.asp?ord="
				Response.write rs("ord")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "&propm="
			...
			rs.movenext
		loop
		Response.write "  " & vbcrlf & "      </table>" & vbcrlf & "</td>" & vbcrlf & "          </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""8%"" height=""30"" ><div align=""center"">全选" & vbcrlf & "          <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "    </div></td>" & vbcrlf & "    <td width=""8%"" align=""left"" > <input class=""resetElementHidden"" type=image src=""../images/m_an_del.gif""   onClick=""return test();""  name=submit> <input class=""resetElementShowNoAlign anybutton"" type=""submit"" onClick=""return test();"" name=submit value=""批量删除""/>  </td>" & vbcrlf & "    <td width=""18%"" >"
		if open_67_13=1 and open_67_19<>1  then
			Response.write "&nbsp;<input  name=""submit22"" type=""submit"" class=""anybutton"" style=""cursor:hand""   onClick=""submit3();"" value=""批量短信群发"">" & vbcrlf & "     "
		end if
		Response.write "</td>" & vbcrlf & "    <td width=""66%""><div align=""right"">" & vbcrlf & "     <span class=""black"">"
		Response.write recCount
		Response.write "个 | "
		Response.write currpage
		Response.write "/"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "      <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='birth.asp?currPage="
			Response.write  1
			Response.write "&q="
			Response.write q
			Response.write "&s="
			...
			Response.write "&propm="
			Response.write propm
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='birth.asp?currPage="
		...
		if currpage=PageCount then
			...
			Response.write "" & vbcrlf & "     <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='birth.asp?currPage="
			Response.write  currpage + 1
			Response.write "&q="
			Response.write q
			Response.write "&s="
			...
			Response.write "&propm="
			Response.write propm
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='birth.asp?currPage="
			Response.write  PageCount
			Response.write "&q="
			Response.write q
			Response.write "&s="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "<script language=javascript>" & vbcrlf & "    function test() {" & vbcrlf & "        if (!confirm('确认删除吗？')) return false;" & vbcrlf & "    }" & vbcrlf & "    function mm(form) {" & vbcrlf & "        for (var i = 0; i < form.elements.length; i++) {" & vbcrlf & "            var e = form.elements[i];" & vbcrlf & "            if (e.name != 'chkall')" & vbcrlf & "                e.checked = form.chkall.checked;" & vbcrlf & "        }" & vbcrlf & "    }" & vbcrlf & "    function submit3() {" & vbcrlf & "        document.getElementById('data').action = ""../message/sendPersonAll.asp?type=1&q=1"";" & vbcrlf & "        document.getElementById('data').submit();" & vbcrlf & "    }" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""4""><div align=""right""><p>&nbsp;" & vbcrlf & "</p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "  </form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	Set hlxNongGong=nothing
	action1="联系人生日提醒列表"
	call close_list(1)
	Response.write "           " & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:230;display:none;"">" & vbcrlf & "<table width=""150"" height=""230""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按姓名排序(降)</font></a></td>" & vbcrlf &           "</tr>" & vbcrlf &                    "<tr  valign=""middle"">" & vbcrlf &             "<td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按姓名排序(升)</font></a> </td>" & vbcrlf & "</tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按来源排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按来源排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=5');""><font color=""#2F496E"">按生日排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按生日排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=7');""><font color=""#2F496E"">按距离生日排序(降)</font></a> </td>" & vbcrlf & "          </tr>" &vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=8');""><font color=""#2F496E"">按距离生日排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "         <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=9');""><font color=""#2F496E"">按添加时间排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=10');""><font color=""#2F496E"">按添加时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & " " & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "<script language=""javascript""> " & vbcrlf & "function Myopen(divID){" & vbcrlf & "   if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=0;" & vbcrlf & "}" & vbcrlf & "jQuery(function(){" & vbcrlf & "      BindCancelEvents({cfgId:7});" & vbcrlf & "});" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>"
