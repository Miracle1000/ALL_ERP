...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, lt)
		...
		Dim rs, sql, sort, title, oldurl, topButtonHTML, tiaojian, paixu
		sort = app.getInt("sort") : title = "" : oldurl = "" : topButtonHTML = ""
		Set rs = cn.execute("select title, oldurl from PrintTemplate_Type where ord="& sort)
		If rs.eof = False Then
			title = rs("title") : oldurl = rs("oldurl")
		end if
		rs.close
		set rs = nothing
		rpt.title = title & "打印模板"
		rpt.showtoppagesize = False
		rpt.nosearchBar = True
		rpt.pagesize = 10
		rpt.canexcel = False
		rpt.canprint = False
		If oldurl&"" <> "" Then
			...
			topButtonHTML = "<input type='radio' id='newprint' onclick=""javascript:window.location.href='"&oldurl&"isnew=1';""><lable for='newprint'>切换至最新版</lable>&nbsp;" &_
			"   <input type='radio' id='oldprint' onclick=""javascript:window.location.href='../manufacture/printerlist.asp?sort="& sort &"';"" checked><lable for='oldprint'>切换至标准版</lable>"
			If sort<>2003 And sort<>150 Then
				topButtonHTML = topButtonHTML & "   &nbsp;<input type='radio' id='oldprint' onclick=""javascript:window.location.href='"&oldurl&"isnew=0';""><lable for='oldprint'>切换至怀旧版</lable>"
			end if
			Case Else
			topButtonHTML = topButtonHTML &"<input type='button' class='anybutton' style='width:80px;' onclick=""javascript:window.location.href='"&oldurl&"';"" value='切换至旧版'>"
			End Select
		end if
		topButtonHTML = topButtonHTML &"&nbsp;<input type='button' class='oldbutton3'  onclick=""javascript:window.open('inc/PrinterCreator.asp?sort="& sort &"','learnconzs2','width=1124,height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=' + (screen.availWidth-1124)/2 + ',top=0');"" value='添加模板'>"
'End Select
		rpt.buttonsHTML = topButtonHTML
		If sort&"" <> "" Then tiaojian = " and c.ord = "&sort&" "
		paixu = "order by t.ismain desc,t.gate1 desc,t.id desc"
		sql = "select t.id,(t.title+(case t.isMain when 1 then '<font style=''color:#CC3333''>（主模板）</font>' else '' end)) title,  "&_
		"t.isMain, isnull(t.PrintType,'未知类型') as PrintType,g.name,isnull(t.gate1,1) 重要指数," &_
		"           isnull(t.[stop],0) [stop], (case isnull(t.[stop],0) when 0 then '启用' else '<div style=''color:red''>停用</div>' end) 状态,t.addid as 操作 "&_
		"           from PrintTemplates t "&_
		"           left join PrintTemplate_Type c on t.templatetype = c.id "&_
		"           left join  ["& Application("_sys_sql_db") &"]..gate g on g.ord= t.addid "&_
		"           where t.del = 1" & tiaojian & paixu
		rpt.sql = sql
	end sub
	sub app_onCreateList(lvw, rpt)
		Dim h, i, col, sort, showGate1, showStatus
		sort = app.getInt("sort") : showGate1 = False : showStatus = False
		lvw.oldPageSizeUI = True
		lvw.HeaderPageSizeUI = True
		lvw.checkbox=False
		lvw.checkvalue = "id"
		...
		Select Case sort
		Case 150
		showGate1 = True : showStatus = True
		End Select
		...
		col.width = 80
		col.align="center"
		col.display = app.iif(showGate1=False,"none","")
		lvw.headers(7).display = "none"
		Set col = lvw.headers(8)
		col.width = 80
		col.align="center"
		col.display = app.iif(showStatus=False,"none","")
		Set col = lvw.headers(9)
		col.formattext = "code:OperateButtons("& sort &",@cells[1],@cells[7],@cells[9]) "
		col.width = 100
		dim btmStr : btmStr = ""
		lvw.addlink = "</a>"& btmStr
	end sub
	Function showPersonName(ord)
		Dim rs1 , s
		Set rs1 = cn.execute("select name from ["& Application("_sys_sql_db") &"]..gate where ord="&ord)
		If rs1.eof = False Then
			s = rs1("name")
		end if
		rs1.close
	...
	function OperateButtons(sort,ord,status,addcate)
		dim actStr , pwxq , pwdel ,pwsh,pwjz ,pwup, showStop
		showStop = False
		Select Case sort
		Case 150 : showStop = True
		End Select
		actStr = actStr &"<div style='word-wrap: break-word; word-break: normal;'>"
'End Select
		actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:window.open('inc/PrinterCreator.asp?id="& ord &"','learnconzs2','width=' + 1150 + ',height=' + (screen.availHeight-80) + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=' + (screen.availWidth-1150)/2  + ',top=0');"">"& app.iif(sort=150,"修改","详情") &"</button>"
		If showStop = True Then
			if actStr&""<>"" then actStr = actStr & "&nbsp;"
			If status = 1 Then
				actStr = actStr & "<button class=""oldbutton2"" onclick=""doSetUse(0,"&ord&")"">启用</button>"
			else
				actStr = actStr & "<button class=""oldbutton2"" onclick=""doSetUse(1,"&ord&")"">停用</button>"
			end if
		end if
		If addcate = Info.User Or  info.isAdmin = True Then
			if actStr&""<>"" then actStr = actStr & "&nbsp;"
			actStr = actStr & "<button class=""oldbutton2"" onclick=""javascript:doDel("& ord &")"">删除</button>"
		end if
		actStr = actStr &"</div>"
		OperateButtons = actStr
	end function
	Sub App_doDel
		Dim id, oSQL,templatetype ,rs
		templatetype = 0
		id = app.GetText("id")
		If Len(id)>0 Then
			oSQL ="update PrintTemplates set del=2, delid=" & Info.user & ", deldate='" & now() & "' where id in(" & id &")"
			cn.Execute(oSQL)
			Response.write(1)
			Set rs = cn.execute("select top 1 templatetype from PrintTemplates where id in ("& id &")")
			If rs.eof = False Then
				templatetype = rs(0)
			end if
			rs.close
		...
		Case 150 : typename = "凭证"
		End Select
		app.Log.remark =typename & "打印模板删除"
		app.Log.href=""
	end sub
	Sub App_doSet
		Dim id,status, oSQL , rs , Scplist ,templatetype
		id = app.GetText("id")
		templatetype = 0
		status = app.getInt("status")
		If Len(id)>0 Then
			oSQL = "update PrintTemplates set stop="& status &" Where id in ( "& id &" ) "
			cn.Execute(oSQL)
			Response.write(1)
			Set rs = cn.execute("select top 1 templatetype from PrintTemplates where id in ("& id &")")
			If rs.eof = False Then
		...
		Case 150 : typename = "凭证"
		End Select
		app.Log.remark =typename & "打印模板" & app.iif(status=0 ,"启用","停用")
		app.Log.href=""
	end sub
