...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_102_10=1 then
		list=" and a.rep_cateid in ("&intro_102_10&")"
	else
		list=" and a.rep_cateid=0"
	end if
	Str_Result=" where 1=1 "&list&""
	Ptr_Result=""
	if open_102_1=3 then
	...
	if open_102_1=3 then
	elseif open_102_1=1 then
		Str_Result=Str_Result&" and rep_cateid in ("&intro_102_1&")"
	else
		Str_Result=Str_Result&" and rep_cateid=0"
	end if
	if open_102_1=3 then
		list=""
	elseif open_102_1=1 then
...
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	i=1
	sql="select a.*,b.car_code,c.setName,d.name from O_repair a"&_
	...
	C1=rs.recordcount
	xlApplication.Init "申请明细_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			rep_time=rs("rep_time")
			rep_addtime=rs("rep_addtime")
			rep_addcateid=rs("rep_addcateid")
			rep_addcateid=rs("rep_addcateid")
			if rep_addcateid="" or isnull(rep_addcateid) then rep_addcateid=0
			sql8="select * from gate where ord="&rep_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				addname=rs8("name")
			end if
			set rs8=nothing
			money1=zbcdbl(rs("rep_money"))
			...
			if money1="" or isnull(money1) then money1=0
			xlWorksheet.Cells(1+i,6).Value =rep_addtime
			if money1="" or isnull(money1) then money1=0
			...
			if money1="" or isnull(money1) then money1=0
			xlWorksheet.Cells(1+i,8).Value =rs("rep_reason")
			if money1="" or isnull(money1) then money1=0
			xlWorksheet.Cells(1+i,8).font.Size=10
			if money1="" or isnull(money1) then money1=0
			xlWorksheet.Cells(1+i,8).HorizontalAlignment=1
			if money1="" or isnull(money1) then money1=0
			xlWorksheet.Cells(1+i,9).Value =CutStrX(rs("rep_note"),1)
			if money1="" or isnull(money1) then money1=0
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>维护列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!维护列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("申请明细_"&session("name2006chen")&".xls")
