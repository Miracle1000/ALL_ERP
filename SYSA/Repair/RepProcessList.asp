...
'Dim sql
	...
	pType = Request("pType")
	If pType = "" Or Not IsNumeric(pType) Then
		Response.write("<script>alert('提示：流程分类参数值不能为空或不是数字！');history.back();</script>")
		Response.end()
	end if
	Response.write "" & vbcrlf & "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN""" & vbcrlf & """http://www.w3.org/TR/html4/loose.dtd"">" & vbcrlf & "<html>" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>维修处理流程设置</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "#content .top td.th {" & vbcrlf & "  text-align: center;" & vbcrlf & "}" & vbcrlf & "#content .text-center {" & vbcrlf& "        text-align: center;" & vbcrlf & "}" & vbcrlf & ".break{word-break:break-all;word-wrap:break-word;width:300px;}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "<body bgcolor=""#ebebeb"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">维修处理流程设置</td>" & vbcrlf & "          <td></td>" & vbcrlf & "          <td align=""right""><input type=""button"" name=""add"" value=""添加维修流程""  class=""anybutton"" onClick=""javascript:window.open('RepProcessSet.asp?pType="
	Response.write pType
	Response.write "','newadd','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100')""/></td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr border=1 valign=""center"" class=""top"">" & vbcrlf & "          <td class=""th"">流程名称</td>" & vbcrlf & "          <td class=""th"">是否启用</td>" & vbcrlf & "<td class=""th"">重要指数</td>" & vbcrlf & "          <td class=""th"">添加人员</td>" & vbcrlf & "          <td class=""th"">操作</td>" & vbcrlf & "        </tr>" & vbcrlf & ""
	CurrPage=clng(Request("CurrPage"))
	Set rs = server.CreateObject("adodb.recordset")
	sql = "SELECT ID,Title,(CASE IsUse WHEN 1 THEN '启用' ELSE '停用' END) IsUse,Ranking," &_
	"(SELECT NAME FROM gate WHERE ord = Comm_ProcessSet.AddUser) AddUser " &_
	"FROM Comm_ProcessSet WHERE Type = "&pType&" " &_
	"ORDER BY Ranking DESC,AddTime DESC "
	...
	If rs.RecordCount<=0 Then
		...
		Do While rs.Eof = False
			...
			Ranking = rs("Ranking")
			AddUser = rs("AddUser")
			Response.write "        " & vbcrlf & "        <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "          <td height=""28"" class=""text-center break"">"
			AddUser = rs("AddUser")
			Response.write Title
			Response.write "</td>" & vbcrlf & "          <td class=""text-center "
			'Response.write Title
			If IsUse = "停用" Then Response.write("red")
			Response.write """>"
			Response.write IsUse
			Response.write "</td>" & vbcrlf & "          <td class=""text-center"">"
			'Response.write IsUse
			...
			Response.write "</td>" & vbcrlf & "          <td class=""text-center"">" & vbcrlf & "          <input type=""button"" name=""content_btn"" value=""预览""  onClick=""javascript:window.open('FlowChart.asp?PID="
			'Response.write AddUser
			Response.write ID
			Response.write "','FlowChart','width=' + 850 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100')"" class=""anybutton""/><input type=""button"" name=""content_btn"" value=""详情""  onClick=""javascript:window.open('RepProcessSetContent.asp?pType="
			'Response.write ID
			Response.write pType
			Response.write "&ID="
			Response.write ID
			Response.write "','newadd','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100')"" class=""anybutton""/>"
			'Response.write ID
			If IsUse = "停用" Then
				Response.write "<input type=""button"" name=""stop"" value=""启用""  onClick=""javascript:if(!confirm('您确定要启用此维修流程吗？')){return false;};window.location.href='RepSaveProcessSet.asp?u="
				Response.write server.urlencode("RepProcessList.asp?CurrPage=" & CurrPage & "&pType=" & pType)
				Response.write "&pType="
				Response.write pType
				Response.write "&ID="
				Response.write ID
				Response.write "&Use=1';"" class=""anybutton""/>"
			else
				Response.write "<input type=""button"" name=""stop"" value=""停用""  onClick=""javascript:if(!confirm('您确定要停用此维修流程吗？')){return false;};window.location.href='RepSaveProcessSet.asp?u="
				Response.write server.urlencode("RepProcessList.asp?CurrPage=" & CurrPage & "&pType=" & pType)
				Response.write "&pType="
				Response.write pType
				Response.write "&ID="
				Response.write ID
				Response.write "&Use=0';"" class=""anybutton""/>"
			end if
			Response.write "<input type=""button"" name=""edit_btn"" value=""修改""  onClick=""javascript:window.open('RepProcessSet.asp?pType="
			Response.write pType
			Response.write "&ID="
			Response.write ID
			Response.write "','newadd','width=' + 900 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=100')"" class=""anybutton""/><input type=""button"" name=""del_btn"" value=""删除""  onClick=""javascript:if(!confirm('您确定要删除此维修流程吗？')){return false;};window.location.href='RepSaveProcessSet.asp?pType="
			Response.write pType
			Response.write "&action=del&ID="
			Response.write ID
			Response.write "';"" class=""anybutton""/>" & vbcrlf & "          </td>" & vbcrlf & "        </tr>" & vbcrlf & ""
			i=i+1
			If i>=rs.PageSize Then Exit Do
			rs.movenext
		Loop
		Response.write "        " & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "        <tr>" & vbcrlf & "          <td width=""10%"" height=""30""><div align=""center""></div></td>" & vbcrlf & "          <td >&nbsp;</td>" & vbcrlf & "          <td width=""79%"">" & vbcrlf & "            <div align=""right"">" & vbcrlf & "            "
		...
		If currpage=1 Then
			...
			Response.write "" & vbcrlf & "            <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='?pType="
			Response.write pType
			Response.write "&currPage="
			Response.write  1
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='?pType="
			Response.write pType
			Response.write "&currPage="
		...
		If currpage=rs.pagecount Then
			...
			Response.write "" & vbcrlf & "            <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='?pType="
			Response.write pType
			Response.write "&currPage="
			Response.write  currpage + 1
			Response.write "&currPage="
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='?pType="
			Response.write pType
			Response.write "&currPage="
			Response.write  rs.PageCount
			Response.write "'"" class=""page""/>" & vbcrlf & "            "
		end if
		Response.write "" & vbcrlf & "            </div>" & vbcrlf & "           </td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & ""
	end if
	rs.close
	set rs = nothing
	conn.Close
