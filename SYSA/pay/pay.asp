...
') c on a.fromType=c.fromType and a.id=c.fromid &_
	...
	if open_6_1=3 then
		Str_Result=""
	elseif open_6_1=1 then
		Str_Result=" and sy.sorce2 in ("&intro_6_1&")"
	else
		Str_Result="and sy.sorce2=0"
	end if
	title=request("title")
	intro=request("intro")
	...
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result+" and sy.sorce2 in ("&W3&")  "
'if W4<>"" and not isnull(W4) then
	end if
	IF sqid <> "" Then
		Str_Result=Str_Result+" and sy.sqid="&sqid&" "
'IF sqid <> "" Then
	end if
	if listbz<>"" and listbz<>"0" then
		listbz=cdbl(listbz)
		Str_Result=Str_Result+" and sy.bz="&listbz&" "
		listbz=cdbl(listbz)
	else
		listbz=0
	end if
	if title<>"" then
		Str_Result=Str_Result&" and  sy.title like '%"&title&"%' "
	end if
	if intro<>"" then
		Str_Result=Str_Result+"and  sy.intro like '%"&intro&"%' "
'if intro<>"" then
	end if
	if money3<>"" then
		if isnumeric(money3)=false then
			Response.write("<script>alert('请输入合法金额');window.location=('pay.asp');</script>")
			Response.end
		else
			Str_Result=Str_Result+" and  sy.money1>="&money3&""
			Response.end
		end if
	end if
	if money12<>"" then
		if isnumeric(money12)=false then
			...
			Str_Result=Str_Result+" and  sy.money1<="&money12&""
			Response.end
		end if
	end if
	if c<>"" then
		if B="1" then
			Str_Result=Str_Result+" and  sy.title like '%"&C&"%' "
'if B="1" then
		elseif B="2" then
			perid=selname("gate","name",C,"ord")
			if trim(perid)="" or isnull(perid) then    perid=0
			if trim(perid)<>"" then
				Str_Result=Str_Result+" and sy.sorce2 in ("&perid&") "
'if trim(perid)<>"" then
			end if
		elseif B="3" then
			perid=selname("gate","name",C,"ord")
			if trim(perid)="" or isnull(perid) then    perid=0
			if trim(perid)<>"" then
				Str_Result=Str_Result+" and sy.addcate in ("&perid&") "
'if trim(perid)<>"" then
			end if
		elseif B="4" then
			Str_Result=Str_Result+" and sy.intro like '%"&C&"%' "
'elseif B="4" then
		end if
	end if
	if m1<>"" then
		If request("link")="1" Then
			Str_Result=Str_Result+" and sy.id in(select fid from pay where date1>='"&m1&" 00:00:00 ') "
'If request("link")="1" Then '
		else
			Str_Result=Str_Result+" and sy.datejk>='"&m1&" 00:00:00 '"
'If request("link")="1" Then '
		end if
	end if
	if m2<>"" then
		If request("link")="1" Then
			Str_Result=Str_Result+" and sy.id in(select fid from pay where date1<='"&m2&" 23:59:59 ') "
'If request("link")="1" Then '
		else
			Str_Result=Str_Result+" and sy.datejk<='"&m2&" 23:59:59 ' "
'If request("link")="1" Then '
		end if
	end if
	dim chance,contract,fromContract,fromChance,fromContract1,fromChance1
	chance=trim(Request.QueryString("chance"))
	contract=trim(Request.QueryString("contract"))
	if chance<>"" then
		fromChance = " and syl.chance=" & chance&""
		fromChance1 = " and  sy.id in (select fid from pay where chance=" & chance & ")"
	end if
	if contract<>"" then
		fromContract = " and syl.contract=" & contract&""
		fromContract1 = " and sy.id in (select fid from pay where contract=" & contract & ")"
	end if
	...
	if px=1 then
		pxResult1=" order by sy.title desc"
		pxResult2=" order by x.title desc"
	elseif px=2 then
		pxResult1=" order by sy.title asc"
		pxResult2=" order by x.title asc"
	elseif px=3 then
		pxResult1=" order by sy.date7 desc"
		pxResult2=" order by x.date7 desc"
	elseif px=4 then
		pxResult1=" order by sy.date7 asc"
		pxResult2=" order by x.date7 asc"
	elseif px=5 then
		pxResult1=" order by sy.datejk desc"
		pxResult2=" order by x.datejk desc"
	elseif px=6 then
		pxResult1=" order by sy.datejk asc"
		pxResult2=" order by x.datejk asc"
	end if
	UUrl=ReturnUrl()
	isShowFyFt = showFyFt()
	function paybxNum(pord)
		set rsf = conn.execute("select count(a.id) from paybxlist a left join paybx b on a.bxid=b.id where a.payid in(select P.ord from pay P left join f_pay F on P.fid=F.id where F.id="& pord &" and P.del=1 and F.del=1) and a.del=1 and b.del=1 ")
		if not rsf.eof then
			if rsf(0)>0 then paybxNum = rsf(0) else paybxNum = 0
		else
			paybxNum = 0
		end if
		rsf.close
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & ".ser_btn input[type='text']{height:20px;line-height:20px;}" & vbcrlf & ".ser_btn select{position:relative;top:1px;*top:0}" & vbcrlf & "</style>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " " & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "   var a = document.getElementsByTagName(""input"");" & vbcrlf & "   var b=document.getElementById(""chkall"");" & vbcrlf &"   if(b.checked==true)" & vbcrlf & "   {" & vbcrlf & "               for (var i=0; i<a.length; i++)" & vbcrlf & "          {" & vbcrlf & "               if (a[i].type == ""checkbox"")" & vbcrlf & "                      { a[i].checked = true;}" & vbcrlf & "         }" & vbcrlf & "   }" & vbcrlf & "   else" & vbcrlf & "   {" & vbcrlf & "                for (var i=0; i<a.length; i++)" & vbcrlf & "               {" & vbcrlf & "               if (a[i].type == ""checkbox"")" & vbcrlf & "                      {a[i].checked = false;}" & vbcrlf & "         }" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> " & vbcrlf & "     <form action=""pay.asp"" method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" >" & vbcrlf & "               <tr>                                  " & vbcrlf & "         <td class=""place"">费用使用列表</td>" & vbcrlf & "        <td><div align=""left""><A class='px_btn' onclick=""Myopen(User);return false;"" href=""javascript:void(0)"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></div></td>" & vbcrlf & "        <td align=""right""><div id=""kh"" style" & vbcrlf & "       <option>-请选择-</option>" & vbcrlf & "       <option value=""page_count=10"" "
	'Response.write Application("sys.info.jsver")
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "      </select></div></td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "  </table> " & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "   <tr>" & vbcrlf & "            <td colspan=""8"" class='ser_btn' style='height:50px;line-height:50px;padding-top:0;padding-bottom:0;' align=""right""><a href=""addrs.asp"">费用管理</a><input type=""radio"" name=""rad1"" checked=""checked"">列表<input type=""radio""  onClick=""javascript:window.location.href"
	Response.write UUrl
	Response.write "'"" name=""rad1"">明细"
	...
	if bz<>0 then
		if not rs8.eof then
			do while not rs8.eof
				...
				Response.write """ "
				if cdbl(rs8("id"))=cdbl(listbz) then Response.write "selected"
				Response.write ">"
	...
	Response.write ">使用主题</option>" & vbcrlf & "                  <option value=""2"" "
	if B="2" then
		Response.write "selected"
	end if
	Response.write ">使用人员</option>" & vbcrlf & "                  <option value=""3"" "
	if B="3" then
		Response.write "selected"
	end if
	Response.write ">添加人员</option>" & vbcrlf & "                      <option value=""4"" "
	if B="4" then
		Response.write "selected"
	end if
	Response.write ">使用概要</option>" & vbcrlf & "             </select>" & vbcrlf & "                  <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "                    <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                 <input type=""button"" name=""openbn"" id=""openbn"" value=""高级检索""  class=""anybutton"" onclick=""$('#w').show();$('#w').window('open');$('#hideDiv').show();""/>" & vbcrlf & "                   "
	if open_6_10=1 or open_6_10=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_sy.asp?B="
		Response.write B
		Response.write "&C="
		Response.write C
	...
	if open_6_7=1 or open_6_7=3 then
		Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit43"" value=""打印""  onClick=""javascript:window.print();return  false;""   class=""anybutton""/>" & vbcrlf & "                       "
	end if
	Response.write "" & vbcrlf & "                             </td></tr>" & vbcrlf & "              </form>" & vbcrlf & " <tr class=""top"">" & vbcrlf & "    <td width=""6%"" align=""center"" ><div align=""center"">选择</div></td>" & vbcrlf & "    <td><div align=""center"">费用主题</div></td>       " & vbcrlf & "         <td width=""10%"" ><div align=""center"" >使用总额</div></td>" & vbcrlf & "        <td width=""7%"" ><div align=""center"" >单据张数</div></td>" & vbcrlf & "   <td width=""9%"" ><div align=""center"" >使用人员</div></td>" & vbcrlf & "    <td width=""9%"" ><div align=""center"" >添加人员</div></td>" & vbcrlf & "   <td width=""11%"" ><div align=""center"" >使用日期</div></td>" & vbcrlf & "        <td width=""20%"" ><div align=""center"">操作</div></td>" & vbcrlf & "      </tr>" & vbcrlf & "           "
	conn.execute("  update p set p.bx =(case when isnull(pb.payid,0) = 0 or isnull(pb.del,0) = 2 or "&_
	" isnull(pb.complete,0)=2 then 0 when isnull(pb.complete,0) in (3) then 3 else 1 end)  "&_
	" from pay p   left join ( select ISNULL(payid,0) AS payid,isnull(max(pb.Del),0) as del,isnull(max(pb.complete),0) "&_
	" as complete from paybxlist pbl inner join paybx pb on pb.id = pbl.bxid and pb.del=1 and pbl.del=1 group by payid) "&_
	" as pb on pb.payid = p.ord" )
	Dim sql_sy, Flds
	Flds = " x.title,x.money1,x.money2,x.syNum,syr.name as syrName,tjr.name as tjrName,x.datejk,x.date7,x.datejk,x.id,x.num2,x.paybxCount,bz.intro BzIntro, x.sorce2 cateid,case when exists(select top 1 1 from  PrinterInfo a where a.sort = 19  and a.formid = x.id) then '<span style=''color:#009900''>[已打印]</span>' else '<span  style=''color:#FF0000''>[未打印]</span>' end  as printStatus "
	sql_sy = " select @Fields "&_
	" from ( "&_
	" select @Top sy.title,SUM(syl.money1) money1, "&_
	" sum(case when syl.complete = 3 then isnull(syl.money1,0) else 0 end) as money2, "&_
	" SUM(syl.num) as syNum, "&_
	" sy.sorce2,sy.addcate,sy.datejk,sy.date7,sy.bz,sy.id, "&_
	" sum(1) num2, "&_
	" SUM(isnull(tt.num1,0)) paybxCount  "&_
	" from f_pay sy "&_
	" left join pay syl on sy.id = syl.fid "&_
	" left join ("&_
	" select distinct bxl.payid,1 num1 "&_
	" from  paybxlist bxl  "&_
	" left join paybx bx on bx.id = bxl.bxid and bx.del = 1 "&_
	" where bxl.del=1 and  isnull(bx.complete,0)<>2 "&_
	" group by bxl.payid "&_
	") tt on tt.payid=syl.ord "&_
	" where 1 = 1 and isnull(syl.del,1) = 1  and sy.del = 1 "&_
	" @Where "&_
	" group by sy.id,sy.title,sy.sorce2,sy.addcate,sy.datejk,sy.date7,sy.bz"&_
	" @OrderBy "&_
	" ) x "&_
	" left join gate syr on syr.ord = x.sorce2 "&_
	" left join gate tjr on tjr.ord = x.addcate "&_
	" left join sortbz bz on bz.id = x.bz "
	dim n
	n=0
	sqlcount="select count(*) from f_pay sy where del=1 "&replace(str_Result,"sy.","") & fromContract1 & fromChance1
...
'if currpage=pagecount then
	...
	Dim rMin, rMax
	rMin = (CInt(currpage) - 1) * CInt(page_count)
'Dim rMin, rMax
	rMax = CInt(currpage) * CInt(page_count)
	set rs=server.CreateObject("adodb.recordset")
	sql = Replace(sql_sy,"@Fields","isnull(sum(isnull(money1,0)),0) as money1")
	sql = Replace(sql,"@Top","")
	sql = Replace(sql,"@Where",replace(str_Result,"'","'") & fromContract & fromChance )
	sql = Replace(sql,"@OrderBy","")
	rs.open  sql,conn,3,1
	If not rs.eof then
		almoney=cdbl(rs("money1"))
	end if
	set rs=nothing
	set rs=server.CreateObject("adodb.recordset")
	sql = Replace(sql_sy,"@Fields",Flds)
	sql = Replace(sql,"@Top"," top " & rMax & " ")
	sql = Replace(sql,"@Where",replace(str_Result & fromContract & fromChance,"'","'"))
	sql = Replace(sql,"@OrderBy",pxResult1)
	sql1 = Replace(sql_sy,"@Fields"," x.id ")
	sql1 = Replace(sql1,"@Top"," top " & rMin & " ")
	sql1 = Replace(sql1,"@Where",replace(str_Result & fromContract & fromChance,"'","'"))
	sql1 = Replace(sql1,"@OrderBy",pxResult1)
	sql = sql & " where x.id not in (" & sql1 & ") " & Replace(pxResult1,"sy.","x.")
	amoney = 0
	rs.open sql,conn,3,1
	if rs.eof=true then
		...
		do until rs.eof
			...
			if money1="" or isnull(money1) then money1=zbcdbl(rs("money2"))
			amoney=cdbl(money1)+cdbl(amoney)
'if money1="" or isnull(money1) then money1=zbcdbl(rs("money2"))
			num=zbcdbl(rs("syNum"))
			cateid = rs("cateid")
			Response.write "" & vbcrlf & "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td width=""6%"" align=""center"" height=""27""><div align=""center""><span class=""red"">" & vbcrlf & "            "
			...
			Response.write "</span></div></td> " & vbcrlf & "          <td>            " & vbcrlf & "                "
			...
			Response.write "<span style=""color:red""></span>        " & vbcrlf & "                </td>" & vbcrlf & "    <td width=""13%""><div align=""right"">"
			Response.write rs("BzIntro")
			Response.write "&nbsp;"
			...
			Response.write "</div></td>" & vbcrlf & "          <td><div align=""center"">"
			if open_6_14=3 or (open_6_14=1 and CheckPurview(intro_6_14,trim(cateid))=True) then
				Response.write "" & vbcrlf & "               <input type=""button"" name=""Submit53"" value=""详情"" class=""anybutton2""  onClick=""javascript:window.open('paydetail.asp?ord="
				Response.write ord
				Response.write "','neww3win','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "                 "
				Response.write ord
			end if
			num2=zbcdbl(rs("num2"))
			paybxCount=rs("paybxCount")
			if open_6_17=3 or CheckPurview(intro_6_17,trim(cateid))=True then
				if (open_6_17=3 or CheckPurview(intro_6_17,trim(cateid))=True) and cdbl(paybxCount)=0 then
					Response.write "" & vbcrlf & "               <input type=""button"" name=""Submit53"" value=""修改"" class=""anybutton2"" onClick=""javascript:window.location=('add2.asp?payid="
					Response.write ord
					Response.write "&up=1')"""
					if paybxCount>0 or (isShowFyFt=true and glFyFtNum("pay",ord)>0) then Response.write(" disabled=""disabled""")
					Response.write "/>" & vbcrlf & "             "
				else
					Response.write "" & vbcrlf & "               <input name=""Submit523"" type=""button"" class=""anybutton2"" onClick=""javascript:window.location=('add22.asp?payid="
					Response.write ord
					Response.write "')"" value=""修改""/>" & vbcrlf & "            "
				end if
			end if
			if cdbl(num2)>cdbl(paybxCount) and (open_6_13=3 or (open_6_13=1 and CheckPurview(intro_6_13,trim(cateid))=True)) then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit"" value=""报销"" class=""anybutton2"" onClick=""window.location.href='add.asp?ord="
				Response.write ord
				Response.write "'""/>" & vbcrlf & "              "
			end if
			if open_6_3=3 or (open_6_3=1 and CheckPurview(intro_6_3,trim(cateid))=True) then
				Response.write "" & vbcrlf & "                 <input type=""button"" name=""Submit"" class=""anybutton2"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete.asp?ord="
				Response.write ord
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "&B="
			...
			rs.movenext
		loop
		Response.write "      " & vbcrlf & "                         <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "               <td height=""27"" colspan=""2""><div align=""right"">本页合计：</div></td>" & vbcrlf & "                  <td class=""red""><div align=""right"">"
		Response.write Formatnumber(amoney,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "          <td colspan=""5"" class=""func"">&nbsp;</td>" & vbcrlf & "          </tr>" & vbcrlf & "                       <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "        <td height=""27"" colspan=""2""><div align=""right"">所有合计：</div></td>" & vbcrlf & "              <td class=""red""><div align=""right"">"
		Response.write Formatnumber(almoney,num_dot_xs,-1)
		Response.write "</div></td>" & vbcrlf & "         <td colspan=""5"" class=""func"">&nbsp;</td>" & vbcrlf & "       </tr>" & vbcrlf & "            </table>" & vbcrlf & "    </td>" & vbcrlf & "       </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""chkall""  id=""chkall"" value=""Check All"" onClick=""mm()"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >"
		...
		Response.write "" & vbcrlf & "      </td>" & vbcrlf & "     </form>" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "    "
		...
		Response.write "页 | &nbsp;"
		Response.write page_count
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & " <input  name=""cpage"" id=""cpage"" type=text   onkeyup=""value=value.replace(/[^\d]/g,'')"" size=3  >" & vbcrlf & "     &nbsp;<input type=""button"" name=""btntz"" value=""跳转"" onClick=""gotourl('currPage='+document.getElementById('cpage').value);"" class=""anybutton2"" />" & vbcrlf & "    "
		if currpage=1 then
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:150; height:152;display:none;"">" & vbcrlf & "<table width=""150"" height=""160""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""152"">" & vbcrlf & "        <table width=""150"" height=""152"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=1')""><font color=""#2F496E"">按使用主题排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=2')""><font color=""#2F496E"">按使用主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=3')""><font color=""#2F496E"">按添加日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "            <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=4')""><font color=""#2F496E"">按添加日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=5')""><font color=""#2F496E"">按使用日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""javascript:void(0)"" onClick=""gotourl('px=6')""><font color=""#2F496E"">按使用日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function Myopen(divID){" & vbcrlf & "      if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & "   divID.style.top=20;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""w"" style=""width:525px;height:425px;padding:5px;background: #fafafa;top:60px;display:none;"" class=""easyui-window"" title=""高级检索""  closed=""true"">" & vbcrlf & "<form method=""get"" action=""pay.asp"" id=""date1"" onSubmit=""return Validator.Validate(this,2)"" name=""date22"">" & vbcrlf & "<div id=""hideDiv"" style=""width:480px;height:425px;border-style: none; border-width: 0px; border-color: inherit; margin: 0px; display:none; padding:0px;"">" & vbcrlf & "                    <div region=""center"" border=""false"" style=""padding:5px;background:#fff;border:1px solid #ccc; width:98%"">" & vbcrlf & "                              <div region=""south"" border=""false"" style=""text-align:right;height:30px;line-height:30px;"">" & vbcrlf & "                            <a class=""easyui-linkbutton""  href=""javascript:void(0)"" onClick=""document.getElementById('date1').submit()"">确认</a>" & vbcrlf & "                          <a class=""easyui-linkbutton"" href=""javascript:void(0)"" onClick=""$('#w').window('close');"">取消</a>" & vbcrlf & "                  </div>" & vbcrlf & "                  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "" & vbcrlf & "          <td width=""20%"" class=""func""><div align=""right"">使用主题</div></td>" & vbcrlf & "                  <td width=""80%""><label>" & vbcrlf & "                  <input name=""title"" type=""text"" id=""title>" & vbcrlf & "          </label></td>" & vbcrlf & "              </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><div align=""right"">使用概要</div></td>" & vbcrlf & "<td width=""80%""><input name=""intro"" type=""text"" id=""intro""></td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><divalign=""right"">使用金额</div></td>" & vbcrlf & "                 <td width=""80%""><input name=""money1"" type=""text""  class=""easyui-numberbox"" id=""money1"" size=""10"" maxlength=""10"" min=""0"" max=""999999999"" precision=""2""  value=""0"">" & vbcrlf & "                   -" & vbcrlf & "      <input name=""money12"" type=""text"" id=""money12"" size=""10"" maxlength=""10"" class=""easyui-numberbox"" min=""0"" max=""999999999"" precision=""2""  value=""999999999""></td>" & vbcrlf & "               </tr>                      " & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><div align=""right"">使用时间</div></td>" & vbcrlf & "                 <td width=""80%"">" & vbcrlf & "                                   <INPUT name=""rett"" size=""9"" readonly=""true"" id=""duepaydate1divPos"" onmouseup=toggleDatePicker(""duepaydate1div"",""date.rett"") value="""
	Response.write m1
	Response.write """>" & vbcrlf & "                                 <DIV id=""duepaydate1div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate1div""></DIV>-" & vbcrlf & "         <INPUT name=""rett2"" size=""9"" readonly=""true"" id=""duepaydate2divPos"" onmouseup=toggleDatePicker(""duepaydate2div"",""date.rett2"") value="""
	Response.write m1
	Response.write m2
	Response.write """>" & vbcrlf & "        <DIV id=""duepaydate2div"" style=""POSITION: absolute;z-index:10"" name =""duepaydate2div""></DIV>                           </td>" & vbcrlf & "               </tr>" & vbcrlf & "               <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><div align=""right"">币种</div></td>" & vbcrlf & "                 <td width=""80%"">" & vbcrlf & "                               "
	sql8="select bz from setbz"
	set rs8=conn.execute(sql8)
	bz=rs8("bz")
	set rs8=nothing
	...
	set rs8=nothing
	Response.write "" & vbcrlf & "                     </select></td>" & vbcrlf & "               </tr>" & vbcrlf & "                         <tr onMouseOut=this.style.backgroundColor="""" onMouseOver=this.style.backgroundColor=""ecf5ff"">" & vbcrlf & "                 <td width=""20%""><div align=""right"">使用人员</div></td>" & vbcrlf & "             <td width=""80%"">"
