...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	if W1<>"0" then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord from gate1  where ord in  ("&W1&") order by gate1 asc"
		rs1.open sql1,conn,3,1
		if rs1.eof then
		else
			do until rs1.eof
				...
				sql2="select ord from gate2  where sort1="&rs1("ord")&" order by gate2 asc"
				rs2.open sql2,conn,3,1
				if rs2.eof then
				else
					do until rs2.eof
						Products=rs2("ord")
						If CheckPurview(W2,trim(Products))=True Then
							W2list="0"
							exit do
						else
				...
				set rs2=nothing
				set rs2=server.CreateObject("adodb.recordset")
				sql2="select ord from gate  where sorce="&rs1("ord")&" and del=1 order by ord asc"
				rs2.open sql2,conn,3,1
				if rs2.eof then
				else
					do until rs2.eof
						Products=rs2("ord")
						If CheckPurview(W3,trim(Products))=True Then
							W2list="0"
							exit do
						end if
						rs2.movenext
				...
				if W2list<>"0" then
					set rs2=server.CreateObject("adodb.recordset")
					sql2="select ord from gate  where sorce="&rs1("ord")&" and cateid=2 order by ord asc"
					rs2.open sql2,conn,3,1
					if rs2.eof then
					else
						do until rs2.eof
	...
	if W2<>"" and W2<>"0" then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord from gate2  where ord in  ("&W2&") order by gate2 desc"
		rs1.open sql1,conn,3,1
		if rs1.eof then
		else
			do until rs1.eof
				W3list="0"
				set rs2=server.CreateObject("adodb.recordset")
				sql2="select ord from gate  where sorce2="&rs1("ord")&" and del=1 order by ord asc"
				rs2.open sql2,conn,3,1
				if rs2.eof then
				else
					do until rs2.eof
...
'if W4<>"" then
	...
	if A1<>"" then
		Str_Result=Str_Result+"and  status="&A1
'if A1<>"" then
	end if
	if m1<>"" then
		Str_Result=Str_Result+"and  adddate>='"&m1&"'"
'if m1<>"" then
	end if
	if m2<>"" then
		Str_Result=Str_Result+"and  adddate<='"&cdate(m2)+1&"'"
'if m2<>"" then
	end if
	if E<>"" then
		Str_Result=Str_Result+"and  gj1 in  ("&E&")"
'if E<>"" then
	end if
	if F<>"" then
		Str_Result=Str_Result+"and  gj2 in  ("&F&")"
'if F<>"" then
	end if
	if S2<>"" then
		if S1=1 then
...
'elseif F1=6 then
	...
	if G2<>"" then
		if G1=1 then
			str_Result=str_Result+"and person in (select ord from person where name like '%"& G2 &"%')"
'if G1=1 then
		elseif G1=2 then
			str_Result=str_Result+"and person in (select ord from person where name not like '%"& G2 &"%')"
'elseif G1=2 then
		elseif G1=3 then
			str_Result=str_Result+"and person in (select ord from person where name='"&G2&"')"
'elseif G1=3 then
		elseif G1=4 then
			str_Result=str_Result+"and person in (select ord from person where name<>'"&G2&"')"
'elseif G1=4 then
...
'elseif G1=6 then
	...
	if px=1 then
		px_Result="order by adddate desc"
	elseif px=2 then
		px_Result="order by adddate asc"
	elseif px=3 then
		px_Result="order by RefuseTel desc"
	elseif px=4 then
		px_Result="order by RefuseTel asc"
	elseif px=9 then
		px_Result="order by addcate desc"
	elseif px=10 then
		px_Result="order by addcate asc"
	end if
	Response.write "" & vbcrlf & "     <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "      <form action=""refuseList.asp?px="
	Response.write px
	Response.write """ method=""post""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "                 <tr>" & vbcrlf & "            <td class=""place"">黑名单列表</td>" & vbcrlf & "            <td>&nbsp;<a href=""#"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td> "& vbcrlf &  "           <td align=""right"">" & vbcrlf &  "                      <div id=""kh"">" & vbcrlf &" <input type=""button"" name=""Submit43"" value=""打印""  onClick=""javascript:kh.style.displayfalse;"" style='margin-top:10px;'  class=""anybutton""/>" & vbcrlf & "            </div>" & vbcrlf & "            </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "<span id=""ht1""></span>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""5"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "</form>" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "    <td align=""center"" ><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "    <td width=""24%"" height=""23""  ><div align=""center"">拒接号码</div></td>" & vbcrlf & "     <td width=""15%"" height=""26"" align=""center"" ><div align=""center"">号码主人</div></td>" & vbcrlf & "     <td width=""16%"" ><div align=""center"">备注</div></td>" & vbcrlf & "        <td width=""11%"" ><div align=""center"">添加时间</div></td>" & vbcrlf & "       <td width=""8%"" ><div align=""center"" ><strong>登记者</strong></div></td>" & vbcrlf & "     <td width=""18%"" ><div align=""center"" >" & vbcrlf & "        <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){window.open(this.value,'_self');}this.selectedIndex=0;"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "          <option>-请选择-</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=10&CurrPage="
	Response.write px
	...
	Response.write ">每页显示10条</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=20&CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&px="
	...
	Response.write ">每页显示20条</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=30&CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&px="
	...
	Response.write ">每页显示30条</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=50&CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&px="
	...
	Response.write ">每页显示50条</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=100&CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&px="
	...
	Response.write ">每页显示100条</option>" & vbcrlf & "          <option value=""refuseList.asp?page_count=200&CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&px="
	...
	n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from callRefuse  "&Str_Result&"   "&px_Result&""
	rs.open sql,conn,3,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "   <form name=""form1"" method=""post"" action=""refuseEdit.asp?action=delAll&ord="
		...
		do until rs.eof
			dim k
			k=rs("RefuseTel")
			Response.write "" & vbcrlf & "      <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "     <td width=""8%"" align=""center"" class=""red"">"
			if open_74_3=3 or open_74_3=1 then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>"
			end if
			Response.write "</td> " & vbcrlf & "                <td height=""27"" class=""name"">"
			Response.write""&k&""
			if rs("RefuseTel")<>"" and tp=1 then
				Response.write "" & vbcrlf & "<!--<a href=""#"" onclick=""javascript:window.open('../china/bh.asp?phone="
'if rs("RefuseTel")<>"" and tp=1 then
				Response.write rs("RefuseTel")
				Response.write "','newwin2','width=' + 400 + ',height=' + 300 + ',fullscreen =no,scrollbars=0,toolbar=0,resizable=0,left=200,top=200');return false;"" title=呼出><img src=""../images/15.gif"" border=""0"" align=""absbottom""></a>-->" & vbcrlf & "<img src=""../images/15.gif"" onClick=""getCall('../china/bh.asp?phone="
				Response.write rs("RefuseTel")
				Response.write "')"" style=""cursor:hand"" border=""0"" align=""absbottom"">" & vbcrlf & ""
			end if
			if open_67_13=1 and open_67_19<>1  and isMobile(trim(rs("RefuseTel")))then
				Response.write "" & vbcrlf & "<img src=""../images/message.gif"" onClick=""javascript:window.open('../message/topadd.asp?phone="
				Response.write trim(rs("RefuseTel"))
			...
			Response.write "</td>" & vbcrlf & "                         <td height=""27"" ><div align=""center"">" & vbcrlf & "                               "
			if open_74_12=1 or open_74_12=3 or CheckPurview(intro_74_12,trim(cateid))=True then
				Response.write "" & vbcrlf & "                                <a href=""#"" onClick=""javascript:window.open('refuseEdit.asp?action=edit&ord="
				Response.write pwurl(rs("ord"))
			...
			Response.write rs("RefuseName")
			Response.write "</a></div></td> " & vbcrlf & "              " & vbcrlf & "                <td><div align=""center"">"
			if open_74_12=1 or open_74_12=3 or CheckPurview(intro_74_12,trim(cateid))=True then
				Response.write "" & vbcrlf & "                                <a href=""#"" onClick=""javascript:window.open('refuseEdit.asp?action=edit&ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >"
			...
			Response.write rs("RefuseContent")
			Response.write "</a></div></td>" & vbcrlf & "" & vbcrlf & "        <td><div align=""center"">"
			Response.write rs("adddate")
			Response.write "</div></td>" & vbcrlf & ""
			if rs("addcate")<>"" then
				set rs1=server.CreateObject("adodb.recordset")
				sql1="select * from gate where ord="&rs("addcate")&""
				rs1.open sql1,conn,3,1
				dim addcateName
				if rs1.eof then
					addcateName=""
				else
					addcateName=rs1("name")
				end if
				rs1.close
				set rs1=nothing
			else
			...
			if open_74_3=3 or CheckPurview(intro_74_3,trim(cateid))=True then
				Response.write "" & vbcrlf & "                    <input type=""button"" name=""Submitdel"" style=""cursor:hand"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='refuseEdit.asp?action=del&ord="
				Response.write pwurl(rs("ord"))
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "&A1="
		...
		loop
		Response.write "     " & vbcrlf & "          </table>" & vbcrlf & "  </td>" & vbcrlf & "         </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "       <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30"">"
		if open_74_3=3 or open_74_3=1 then
			Response.write "<div align=""center"">全选" & vbcrlf & "          <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "    </div>"
		end if
		Response.write "</td>" & vbcrlf & "    <td >"
		if open_74_3=1 or open_74_3=3 then
		...
		Response.write "</td>" & vbcrlf & "        </form>" & vbcrlf & " <form   name=""form2""   method=""post""   action=""refuseList.asp?W1="
		Response.write W1
		Response.write "&W2="
		Response.write W2
		Response.write "&W3="
		...
		Response.write "&F="
		Response.write F
		Response.write """> " & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "    "
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "         <input   name=""currpage""  type=text   onkeyup=""value=value.replace(/[^\d\.]/g,'')""  size=3  >" & vbcrlf & "     &nbsp;<input type=""submit"" name=""Submit422"" value=""跳转""  class=""anybutton2""/>" & vbcrlf & "             "
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='refuseList.asp?currPage="
			Response.write  1
			Response.write "&A1="
			Response.write A1
			Response.write "&page_count="
			...
			Response.write "&F="
			Response.write F
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='refuseList.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='refuseList.asp?currPage="
			Response.write  currpage + 1
			Response.write "&A1="
			Response.write A1
			Response.write "&page_count="
			...
			Response.write "&F="
			Response.write F
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='refuseList.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&A1="
			Response.write A1
			Response.write "&page_count="
		...
		Response.write "" & vbcrlf & "    </div></td></form>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " " & vbcrlf & "function mm(form) " & vbcrlf & "{ ///定义函数checkall,参数为form " & vbcrlf & "" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "" & vbcrlf & "///把表单里的内容依依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf & "if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "} " & vbcrlf & "} " & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""1138"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>         " & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	dim actinon1
	action1="黑名单列表"
	call close_list(1)
	Response.write "  " & vbcrlf & "   </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script language=""javascript"">" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "    if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=210;" & vbcrlf & "   divID.style.top=-40;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:400;display:none;"">" & vbcrlf & "<table width=""150"" height=""300""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf &  " <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "        <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" > "& vbcrlf &  "         <tr valign=""middle""> "& vbcrlf &   "          <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
	...
	Response.write "&F="
	Response.write F
	Response.write "&px=1""><font color=""#2F496E"">按来电时间排序(降)</font></a></td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
	...
	Response.write "&F="
	Response.write F
	Response.write "&px=2""><font color=""#2F496E"">按来电时间排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
	...
	Response.write "&F="
	Response.write F
	Response.write "&px=3""><font color=""#2F496E"">按来电号码排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
	...
	Response.write "&F="
	Response.write F
	Response.write "&px=4""><font color=""#2F496E"">按来电号码排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
	...
	Response.write "&F="
	Response.write F
	Response.write "&px=9""><font color=""#2F496E"">按添加人排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""refuseList.asp?CurrPage="
	Response.write CurrPage
	Response.write "&A1="
	Response.write A1
	Response.write "&page_count="
