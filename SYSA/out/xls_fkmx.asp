	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			if open_8_1=3 then
				list_tj=""
			elseif open_8_1=1 then
				list_tj="and payout.cateid in ("&intro_8_1&") and payout.cateid<>0 "
			else
				list_tj="and payout.cateid=0"
			end if
			Str_Result="where payout.del=1 "&list_tj&""
			...
			xlWorksheet.Cells(1,4).Value = "单据人员"
			...
			xlWorksheet.Cells(1,11).Value = "备注"
			...
			If istype=0 then
				if m1="" and Request("Ismode")<>1 then
					m1=cdate(year(date)&"-"&month(date)&"-1")
'if m1="" and Request("Ismode")<>1 then
				end if
				if m2="" and Request("Ismode")<>1 then
					m2=dateadd("m",1,m1)-1
'if m2="" and Request("Ismode")<>1 then
				end if
			end if
			if m1<>"" then
				if A="1" or A="4" Or A="6" Or A="7"  Or A="10" then
					Str_Result=Str_Result+" and  payout.date1>='"&m1&"'"
'if A="1" or A="4" Or A="6" Or A="7"  Or A="10" then
				elseif A="2" then
...
'elseif A="3" then
				elseif A="5" Or A="6" then
					Str_Result=Str_Result+" and  payout.date3>='"&m1&"'"
'elseif A="5" Or A="6" then
				ElseIf A="10" then
					Str_Result=Str_Result+" and  (payout.date1>='"&m1&"' or payout.date2>='" & m1 & "')"
'ElseIf A="10" then
				end if
			end if
			if m2<>"" then
				if A="1" or A="4" Or A="6" Or A="7"  Or A="10"  then
					Str_Result=Str_Result+" and  payout.date1<='"&m2&" 23:59:59'"
'if A="1" or A="4" Or A="6" Or A="7"  Or A="10"  then
				elseif A="2" then
					Str_Result=Str_Result+" and  payout.date2<='"&m2&" 23:59:59'"
'elseif A="2" then
				elseif A="3" then
					Str_Result=Str_Result+" and  payout.date2<='"&m2&" 23:59:59'"
'elseif A="3" then
				elseif A="5" Or A="6" then
					Str_Result=Str_Result+" and  payout.date3<='"&m2&" 23:59:59'"
'elseif A="5" Or A="6" then
				ElseIf A="10" then
					Str_Result=Str_Result+" and  (payout.date1<='"&m2&" 23:59:59' or payout.date2<='" & m2 & " 23:59:59')"
'ElseIf A="10" then
				end if
			end if
			if A="1" then
...
'elseif A="5" then
			elseif A="6" then
				Str_Result=Str_Result+" and  complete2=1"
'elseif A="6" then
			ElseIf A="7" Then
				Str_Result=Str_Result+" and  complete2=4"
'ElseIf A="7" Then
			end if
			str_Result = str_Result & searchExtended_Simple(8, "ord")
...
'if  A2<>"" then
	...
	if C<>""then
		if B="gysmc" then
			str_Result=str_Result+"and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.company in (select ord from tel where name like '%"& C &"%'))"
'if B="gysmc" then
		elseif B="cgzt" then
			str_Result=str_Result+"and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.title like '%"& C &"%')"
'elseif B="cgzt" then
		elseif B="cgid" then
			str_Result=str_Result+"and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.cgid like '%"& C &"%')"
'elseif B="cgid" then
		end if
	end if
	...
	elseif px=7 then
		px_Result="order by payout.title desc,payout.date7 desc"
	elseif px=8 then
		px_Result="order by payout.title asc,payout.date7 asc"
	elseif px=9 then
		px_Result="order by payout.complete desc,payout.date7 desc"
	elseif px=10 then
		px_Result="order by payout.complete asc,payout.date7 asc"
	elseif px=13 then
	...
	if Request("companyname")<>"" then
		str_Result=str_Result+" and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.company in (select ord from tel where name like '%"& request("companyname") &"%'))"
'if Request("companyname")<>"" then
	end if
...
'if  Request("duemoney1")<>"" and Request("duemoney2")<>"" then
	...
	if  Request("duepaydate1")<>"" and Request("duepaydate2")<>"" then
		...
		str_Result=str_Result+" and payout.date1  between '"&Request("duepaydate1")&"' and '"&Request("duepaydate2")&"'"
		m2=Request("duepaydate2")
	end if
	if request("paytype")<>"" then
		if cint(request("paytype"))>0 then
			If CStr(request("paytype"))="999" Then
				str_Result = str_Result + " and title='余额付款'"
'If CStr(request("paytype"))="999" Then
			else
...
'If CStr(request("paytype"))="999" Then
	...
	if Request("paydate1")<>"" then
		str_Result=str_Result+" and payout.date2>='"&Request("paydate1")&"'"
'if Request("paydate1")<>"" then
	end if
	if Request("paydate2")<>"" then
		str_Result=str_Result+" and payout.date2<='"&Request("paydate2")&"'"
'if Request("paydate2")<>"" then
	end if
...
'if request("tikname")<>"" then
	...
	if request("contractname")<>"" then
		str_Result=str_Result+" and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.title like '%"& request("contractname") &"%')"
'if request("contractname")<>"" then
	end if
...
'if request("intro")<>"" then
	...
	if request("bz")<>"" then
		if cint(request("bz"))>0 then
			str_Result=str_Result+" and payout.ord in (select p.ord from PayCaigouProxy a inner join payout p on p.contract = a.ord and p.cls = a.cls where a.bz = "&cint(request("bz"))&")"
'if cint(request("bz"))>0 then
		end if
	end if
	...
	set rs=server.CreateObject("adodb.recordset")
	if IsType=0 then
		sql="select payout.*,CONVERT(VARCHAR(10),payout.date1,120) date10 "&_
		"from payout "&_
		...
		If Len(m1)>0 Then Str_Result = Str_Result & " and payout.date1>='"&m1&"' "
		If Len(m2)>0 Then Str_Result = Str_Result & " and payout.date1<='"&m2&" 23:59:59' "
		sql="select payout.*,CONVERT(VARCHAR(10),payout.date1,120) date10 from payout left join tel t on t.ord=payout.company "&Str_Result &px_Result&""
	end if
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "付款明细表_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	if rs.RecordCount<=0  then
		...
		do until rs.eof
			...
			if contract&""<>"" then
				...
				sql2="select a.title,isnull(a.creator,0) cateid ,a.del,isnull((select sort1 from sortbz where id=14),'人民币') bzname from M_OutOrder a where a.id="& contract &"  "
				Case "4" :
...
'End Select
			...
			if rs("complete2")="2" then
				complete2="未收票"
			elseif rs("complete2")="3" then
				complete2="已收票"
			end if
			...
			date5=rs("date2")
			dim gatearray
			gatearray = getgatearray(djcateid)
			...
			pay=""
			if not isnull(rs("pay")) then
				pay = getrsval("select sort1 from sortonehy where ord = "&rs("pay"))
			end if
			if isnull(pay) then
				pay = rs("title")
			ElseIf rs("title")="余额付款" Then
				pay="余额付款"
			end if
			...
			pay="余额付款"
			xlWorksheet.Cells(1+i,10).Value = caigouname
			pay="余额付款"
	...
	set rs=Nothing
	Set rs1 = Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>付款明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!付款明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	Set rs1 = Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("付款明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的付款明细表</u></strong></font></a></p> " & vbcrlf & ""
