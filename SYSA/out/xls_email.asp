...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	if companyid<>"" then
		Str_Result2=Str_Result2&" and( recv_email in(select email from tel where ord="& companyid &") or person in(select ord from person where company="& companyid &")  or (sort_action=1 and ord_action in (select ord from contract where company = "& companyid&")) or (sort_action=2 and ord_action in (select ord from price where company = "& companyid&")) ) "
	end if
	if S2<>"" and S1="" then
	'end if
		if G2<>"" then
	...
	C1=rs.recordcount
	xlApplication.Init "邮件记录_"&session("name2006chen")&".xls",C1
	dim i
	i=1
	do until rs.eof
