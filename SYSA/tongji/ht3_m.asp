...
'bigsystemtype = ".mozi"
	...
	elseif open_5_11=1 then
		list_tj="and cateid in ("&intro_5_11&")"
	else
	...
	if open_5_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_5_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "      margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "#echarts{margin-top:115px;}" & vbcrlf & "#content tr.top td{line-height:22px;background:#f4fafe}" & vbcrlf & "</style>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf & "<!--" & vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0" & vbcrlf & "eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf & "if (restore) selObj.selectedIndex=0;" & vbcrlf & "}" & vbcrlf & "//-->" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "" & vbcrlf & "<body  oncontextmenu=self.event.returnValue=false  onMouseOver=""window.status='none';return true;""  scroll=AUTO>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
...
'if m1<>"" then
		Str_Result2=Str_Result2+"and date3>='"&m1&"' "
'if m1<>"" then
		m1name="  起始："&m1
	end if
	if m2<>"" then
...
'if m2<>"" then
		Str_Result2=Str_Result2+"and date3<='"&cdate(m2)&"' "
'if m2<>"" then
		m2name="  截止："&m2
	end if
	if E<>"" then
...
'if E<>"" then
		Str_Result2=Str_Result2+"and  complete1='"&E&"'"
'if E<>"" then
	end if
	if F<>"" then
		Str_Result=Str_Result+"and  sort='"&F&"'"
'if F<>"" then
		Str_Result2=Str_Result2+"and  sort='"&F&"'"
'if F<>"" then
	end if
	dim HtType
	...
	if HtType<>"" then
		select case HtType
		case 0
		Str_Result=Str_Result+" and (paybacktype=0 or paybacktype is null) AND (sort1 is null or sort1=0)"
'case 0   '
		Str_Result2=Str_Result2+" and (paybacktype=0 or paybacktype is null) AND (sort1 is null or sort1=0)"
'case 0   '
		case 2
		Str_Result=Str_Result+" and paybacktype=1 AND (sort1 is null or sort1=0) "
'case 2   '
		Str_Result2=Str_Result2+" and paybacktype=1 "
'case 2   '
		case 3
...
'case 4       '
		Str_Result2=Str_Result2+" AND repairOrderId is not null "
'case 4       '
		Case 6
		Str_Result=Str_Result+" AND exists(select 1 from contractlist where contract = a.ord and isnull(treeord,0)>0)  "
'Case 6 '
		Str_Result2=Str_Result2+" AND exists(select 1 from contractlist where contract = a.ord and isnull(treeord,0)>0)  "
'Case 6 '
		Case 5
		Str_Result=Str_Result+" AND wxUserId is not null "
'Case 5 '
		Str_Result2=Str_Result2+" AND wxUserId is not null "
'Case 5 '
		Case 7
		Str_Result=Str_Result+" AND sort1=7 "
'Case 7 '
		Str_Result2=Str_Result2+" AND sort1=7 "
'Case 7 '
		end select
	end if
	dim sort,sort1
	...
	if F<>"" then
		set rs=server.CreateObject("adodb.recordset")
		sql="select sort1 from sortonehy where ord="&F&""
		rs.open sql,conn,1,1
		sort1=rs("sort1")
		sort1=sort1&">>"
		rs.close
	...
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">合同金额统计</td>" & vbcrlf & "                   " & vbcrlf & "            <td  align=""left""><strong>"
	Response.write sort
	Response.write sort1
	Response.write "人员分布"
	Response.write m1name
	Response.write m2name
	Response.write "" & vbcrlf & "    </strong>" & vbcrlf & "                 </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & " <form method=""get"" action=""ht3_m.asp?width_tj="
	Response.write width_tj
	Response.write """ id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "<td>选择统计条件：" & vbcrlf & ""
	Response.write "<select name=""sort"">" & vbcrlf & ""
	if E<>"" then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select id,sort1 from sortonehy where  ord="&E&""
		...
		set rss=conn.execute("select * from sortonehy  where gate2=32 and  ord<>"&rs7("id")&" order by gate1 desc")
		while not rss.eof
			sele=""
			if request("sort")&""=rss("id")&"" then
				sele="selected"
			end if
		...
		rs7.close
		set rs7=nothing
		Response.write "" & vbcrlf & "<option value="""" >合同状态</option>" & vbcrlf & ""
	else
		Response.write "" & vbcrlf & "<option value="""" >合同状态</option>" & vbcrlf & ""
		set rss=conn.execute("select * from sortonehy where gate2=32 order by gate1 desc")
		while not rss.eof
			sele=""
			if request("sort")&""=rss("id")&"" then
				sele="selected"
	...
	Response.write "" & vbcrlf & "            </select>" & vbcrlf & "                 <select name=""sort1""  datatype=""Limit"" >" & vbcrlf & ""
	if F<>"" then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select id,sort1 from sortonehy where  ord="&F&""
		rs7.open sql7,conn,1,1
		Response.write " " & vbcrlf & "<option value="""
		Response.write rs7("id")
		Response.write """ >"
		Response.write rs7("sort1")
		Response.write "</option> " & vbcrlf & ""
		set rss=conn.execute("select id ,sort1 from sortonehy where  gate2=31 and ord<>"&rs7("id")&"  order by gate1 desc")
		while not rss.eof
			sele=""
			if request("sort1")&""=rss("id")&"" then
				sele="selected"
			end if
			Response.write "<option value="""&rss("id")&""" "&sele&">"&rss("sort1")&"</option>"
			rss.movenext
		...
		rs7.close
		set rs7=nothing
		Response.write "" & vbcrlf & "<option value="""" >合同阶段</option>" & vbcrlf & ""
	else
		Response.write "" & vbcrlf & "<option value="""" >合同分类</option>" & vbcrlf & ""
		set rss=conn.execute("select id ,sort1 from sortonehy where gate2=31  order by gate1 desc")
		while not rss.eof
			Response.write "<option value="""&rss("id")&""">"&rss("sort1")&"</option>"
			rss.movenext
		wend
		rss.close
		set rss=nothing
	end if
	Response.write "" & vbcrlf & "" & vbcrlf & "            </select>"
	...
	Response.write "" & vbcrlf & "<select name=""HtType"" id=""HtType"" datatype=""Limit"">" & vbcrlf & ""
	if ZBRuntime.MC(6000) Then
		Response.write "" & vbcrlf & "       <option value=""3"" "
		if HtType="3" Then Response.write " selected='selected'"
		Response.write ">销售开单</option>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "       <option value=""0"" "
	if HtType="0" Then Response.write " selected='selected'"
	Response.write ">销售合同</option>" & vbcrlf & "    <!--option value=""1"" "
	'if HtType="0" Then Response.write " selected='selected'"
	if HtType="1" Then Response.write " selected='selected'"
	Response.write ">租赁合同</option-->"
	'if HtType="1" Then Response.write " selected='selected'"
	if ZBRuntime.MC(17000) and ZBRuntime.MC(17003) Then
		Response.write "" & vbcrlf & "      <option value=""2"" "
		if HtType="2" Then Response.write " selected='selected'"
		Response.write ">直接出库合同</option>" & vbcrlf & ""
	end if
	if ZBRuntime.MC(71000) Then
		Response.write "" & vbcrlf & "      <option value=""4"" "
		if HtType="4" Then Response.write " selected='selected'"
		Response.write ">维修合同</option>" & vbcrlf & ""
	end if
	if ZBRuntime.MC(17010) Then
		Response.write "" & vbcrlf & "      <option value=""6"" "
		if HtType="6" Then Response.write " selected='selected'"
		Response.write ">组装合同</option>" & vbcrlf & ""
	end if
	if ZBRuntime.MC(76000) Then
		Response.write "" & vbcrlf & "      <option value=""5"" "
		if HtType="5" Then Response.write " selected='selected'"
		Response.write ">微信合同</option>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "      <option value=""7"" "
	if HtType="7" Then Response.write " selected='selected'"
	Response.write ">导入合同</option>" & vbcrlf & "    <option value="""" "
	if HtType="" Then Response.write " selected='selected'"
	Response.write ">合同类型</option>" & vbcrlf & "</select>" & vbcrlf & "" & vbcrlf & ""
	...
	Response.write "" & vbcrlf & "<input type=""submit"" name=""Submit422"" value=""统计""  class=""anybutton2""/></td>" & vbcrlf & "  </tr></table>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""30""  align=""right"">" & vbcrlf & "          <input type=""button"" name=""pillar"" value=""柱状图分析"" onClick=""gotourl('type_tj=1')"" class=""anybutton2"">" & vbcrlf & "          <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""gotourl('type_tj=2')"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""gotourl96E;border:0px   solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "              <option value=""550"" "
	if width_tj=550 then
		Response.write "selected"
	end if
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""700"" "
	if width_tj=700 then
		Response.write "selected"
	end if
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""1000"" "
	if width_tj=1000 then
		Response.write "selected"
	end if
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""1500"" "
	if width_tj=1500 then
		Response.write "selected"
	end if
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""2000"" "
	if width_tj=2000 then
		Response.write "selected"
	end if
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""3000"" "
	...
	if open_5_10=1 or open_5_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='ht3_m.asp?type_tj=4&sort="
		Response.write E
		Response.write "&sort1="
		Response.write F
		Response.write "&ret="
	...
	Response.write "</td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" id=""user2"">" & vbcrlf & "<tr bgcolor=""#FFFFFF""><td align=""left""><div align=""left""><span id=""CountTXTok"" name=""CountTXTok"" style=""font-size:10pt;"">" & vbcrlf & "  <B>正在处理数据...</B></span></div></td></tr>" & vbcrlf & "<tr>" & vbcrlf & "  <td colspan=2 class=tablebody1>" & vbcrlf & " <table width=""600"" border=""0"" cellpadding=""1"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr bgcolor=""#FFFFFF"">" & vbcrlf & "<td bgcolor=ffffff height=9><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage"" name=""CountImage"" align=""absmiddle""><img src=""../images/bj_tiao2.gif"" style="""" width=""0"" height=""16"" id=""CountImage2"" name=""CountImage2"" align=""absmiddle"">" & vbcrlf & "</table>" & vbcrlf & "  </td>" & vbcrlf & "</tr>" & vbcrlf & "<tr bgcolor=""#FFFFFF""><td><span id=""CountTXT"" name=""CountTXT"" style=""font-size:9pt; color:#008040"">0</span><span style=""font-size:9pt; color:#008040"">%</span></td></tr>" & vbcrlf & "</table>" & vbcrlf & "</	div>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td align=""center"">" & vbcrlf & ""
	dim numcg1,zs1
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select * from (select ord,name,sorce,cateid,sorce2 from gate "&Str_power33&" union all select 0,'无销售人员',9999,10,9999) t order by sorce asc,cateid asc,sorce2 asc"
	rs1.open sql1,conn,1,1
	...
	if counts<=0 then
		...
		do until rs1.eof
			set rs=server.CreateObject("adodb.recordset")
			sql="select isnull(sum(money2),0) as money1 from contract a where isnull(a.status,-1) in (-1,1) and isnull(cateid,0)="&rs1("ord")&"  "&Str_Result2&" "
			'set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			If rs.eof = False Then
				numcg1=Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
'If rs.eof = False Then
			end if
			rs.close
			set rs=nothing
			If numcg1&"" = "" Then numcg1 = 0
			title=rs1("name")
			lburl="../contract/planall.asp?fromTJ=1&W3="&rs1("ord")&"&E="&E&"&F="&F&"&ret="&m1&"&ret2="&m2&"&HtType="&HtType&"&tj=1"
			total1(j,1)=numcg1
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			total2(j,1)=numcg1
			total2(0,1)="#FF0000,1.5,1,2,合同分布"
			total2(j,2)=lburl
			total3(j,0)=title
			total3(j,1)=numcg1
			total3(j,2)=lburl
...
'If tempCategoryName <> "" Then tempCategoryName = Replace(tempCategoryName,"<br>","")
						Response.write "<v:shape id='_x0000_s1025' type='#_x0000_t202' title='"& tempCategoryName &"："&total2(i,j * 3 -2) & colTipHxt&" "&VbCrLf&"点击可查看详细列表' style='position:absolute;left:"&x1&"px;top:"&y1-15&"px;width:100px;height:18px;z-index:-1'><v:textbox inset='0px,0px,0px,0px'><table cellspacing='0' cellpad			ding='0' width='100%' height='100%'><tr><td align='left'><a href='"&total2(i,j * 3 -1)&"'>"&total2(i,j * 3 -2) & colTipHxt&"</a></td></tr></table></v:textbox></v:shape>"
'If tempCategoryName <> "" Then tempCategoryName = Replace(tempCategoryName,"<br>","")
					else
...
'x4=x3
				Response.write "<v:textbox  title='点击可查看详细列表' inset='0px,0px,0px,0px'><table cellspacing='0' cellpadding='0' width='100%' style='position:relative' height='100%'><tr><td align='left'><a href='"&total3(i,2)&"'>"&total3(i,0)&" "&total3(i,1)&" 占"&formatnumber(total3(i,1)*100/totalpie,2,-1)&"%</a></td></tr></t			able></v:textbox></v:shape>"
