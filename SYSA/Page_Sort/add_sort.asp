...
'If Not rs.Eof Then
	...
	Ord=SaveRequestUrlNum("Ord")
	TableID=SaveRequestUrlNum("TableID")
	TypeID=SaveRequestUrlNum("TypeID")
	copyOrd=SaveRequestUrlNum("copyOrd")
	If typeID="" then typeID=1
	If typeID=2 Then
		PageTitle="方式"
		If tableID=3 Then PageTitle="形式"
	...
	IF TableID>0 Then
		set  Rs=conn.execute("select NameUI from page_table where Ord="& TableID &"")
		If Not Rs.eof Then
			OrderName=Rs(0)
		else
			OrderName=""
	...
	Class GetSort
		...
		Public Sub Initial()
			dim SortNum
			SortArr=""
			If IsNum(TableID) Then
				If TableID>0 Then
					SortNum=conn.execute("Select count(*) from Page_sort where  TableID="& TableID &" and TypeID="& TypeID &"  and del=1")(0)
					SortArr="<script language=""javascript"" type=""text/javascript"">"&vbCrLf
					If SortNum>0 Then
						SortArr=SortArr&"var UserSortArr = new Array("& SortNum-1 &");"&vbCrLf
'If SortNum>0 Then
						Set Rs=server.CreateObject("adodb.recordset")
						Sql="Select * from Page_sort where  TableID="& TableID &" and TypeID="& TypeID &" and Del=1 order by px Desc"
						rs.open sql,conn,1,1
						If Not Rs.eof Then
							Dim i
							i=0
							Do While Not Rs.eof
								SortArr=SortArr&"UserSortArr["& i &"] = ["""& Rs("Ord") &""", """& Rs("Title") &""", """& Rs("SortID") &"""];"&vbCrLf
								i=i+1
								rs.movenext
							Loop
						end if
	...
	title=""
	sortID=""
	If Action="edit" and Ord>0 and TableID>0 Then
		Set Rs=server.CreateObject("adodb.recordset")
		SQL="select top 1 *  from Page_sort Where Ord="&Ord&" and TableID="&TableID&" and TypeID="& TypeID &" and del=1"
		rs.open sql,conn,1,1
		If Not Rs.Eof then
			title=Rs("title")
			sortID=Rs("sortID")
	...
	ElseIf Action="add" Then
		...
		If copyOrd > 0 Then
			Set Rs=server.CreateObject("adodb.recordset")
			SQL="select top 1 *  from Page_sort Where Ord="&copyOrd&" and TableID="&TableID&" and TypeID="& TypeID &" and del=1"
			rs.open sql,conn,1,1
			If Not Rs.Eof then
				title=Rs("title")
				sortID=Rs("sortID")
	...
	If IsNum(sortID)=False Then sortID=0
	If IsNum(px)=False Then px=0
	dim CanChild
	if typeID=2 or typeID=3 or typeID=4   then
		CanChild=False
	else
		CanChild=true
	end if
	...
	'Response.write Application("sys.info.jsver")
	set Get_Sort=New GetSort
	Call Get_Sort.Initial()
	Response.write Get_Sort.SortArr
	Set Get_Sort=Nothing
	Response.write "" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "/**" & vbcrlf & " * 无限级联动选择菜单类" & vbcrlf & " * _storeValueObjName:存放选择项值的页面元素名称" & vbcrlf & " * _showSelectObjName:显示该菜单的页面元素名称" & vbcrlf & " * _sortArr:显示菜单所需的数组，格式如下" & vbcrlf & " * arrSorts[0] = [""类别ID1"",""类别一"", ""父类ID1""];" & vbcrlf & " * arrSorts[1] = [""类别ID2"", ""类别二"", ""父类ID2""];" & vbcrlf & " *" & vbcrlf & " */" & vbcrlf & "function sortMenu(_storeValueObjName, _showSelectObjName, _sortArr)" & vbcrlf & "{" & vbcrlf & "     this.storeValueObj=document.getElementById(_storeValueObjName);" & vbcrlf & "      this.showSelectObj=document.getElementById(_showSelectObjName);" & vbcrlf & " this.sortArr=_sortArr;" & vbcrlf & "" & vbcrlf & "  /**" & vbcrlf & "     * 获取第一层分类，并显示在showSelectObj中" & vbcrlf & "       * _sortMenuObj:sortMenu的实例对象，指向自己" & vbcrlf & "     */" & vbcrlf & "      this.initSorts=function(_sortMenuObj){"& vbcrlf & "              this.storeValueObj.value=0;" & vbcrlf & "             _select=document.createElement(""select"");" & vbcrlf & "         this.showSelectObj.insertAdjacentElement(""afterBegin"",_select);" & vbcrlf & "           _select.sortMenuObj=_sortMenuObj;" & vbcrlf & "               _select.onchange=function(){" & vbcrlf & "                    this.sortMenuObj.setSorts(this,this.sortMenuObj);" & vbcrlf & "           }" & vbcrlf & "               _select.add(new Option(""根目录"",""""));" & vbcrlf & "               for (var i = 0; i < this.sortArr.length; i++){" & vbcrlf & "                  if (this.sortArr[i][2] == 0){" & vbcrlf & "                           _select.add(new Option(this.sortArr[i][1],this.sortArr[i][0]));" & vbcrlf & "                   }" & vbcrlf & "               }  " & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "       /**" & vbcrlf & "     * 下拉框联动" & vbcrlf & "    * _curSelect:当前选择的下拉框" & vbcrlf & "    * _sortMenuObj:sortMenu的实例对象，指向自己" & vbcrlf & "    */" & vbcrlf & "      this.setSorts=function(_curSelect,_sortMenuObj){" & vbcrlf & "                //若当前下拉框后面还有下拉框，即有下级下拉框时，清除下级下拉框，在后面会重新生成下级部分" & vbcrlf & "                //下级下拉框与当前下拉框由于都是显示在showSelectObj中，故它们是兄弟关系，所以用nextSibling获取" & vbcrlf & "          while (_curSelect.nextSibling){" & vbcrlf & "                 _curSelect.parentNode.removeChild(_curSelect.nextSibling);" & vbcrlf & "              }" & vbcrlf & "  " & vbcrlf & "             //获取当前选项的值" & vbcrlf & "              _iValue = _curSelect.options[_curSelect.selectedIndex].value;" & vbcrlf & "           //如果选择的是下拉框第一项(第一项的值为"""")" & vbcrlf & "                if (_iValue == """"){" & vbcrlf & "                       //若存在上级下拉框" & vbcrlf & "                      if (_curSelect.previousSibling){" & vbcrlf & "                                //取值为上级下拉框选中值" & vbcrlf & "                                this.storeValueObj.value = _curSelect.previousSibling.options[_curSelect.previousSibling.selectedIndex].value;" & vbcrlf & "                  }" & vbcrlf & "                       else{" & vbcrlf & "                           //没上级则取值为0" & vbcrlf & "                               this.storeValueObj.value = 0;" & vbcrlf & "                   }" & vbcrlf & "                       //选择第一项(根目录),没有下级选项,所以要返回" & vbcrlf & "                    return false;" & vbcrlf &" } "& vbcrlf &   "             //选择的不是第一项 "& vbcrlf &  "             this.storeValueObj.value = _iValue; "& vbcrlf &  "     "  & vbcrlf &  "             //去掉当前下拉框原来的选择状态 "& vbcrlf &"           //将选中的选项对应代码更改为 selected "& vbcrlf & "           for (i=0;i<_curSelect.options.length;i++){ "& vbcrlf &  "                     if (_curSelect.options[i].selected==""selected""){ "& vbcrlf & "                          _curSelect.options[i].removeAttribute(""selected"");" & vbcrlf & "                        }" & vbcrlf & "                       if (_curSelect.options[i].value==_iValue){" & vbcrlf & "                              _curSelect.options[i].selected=""selected"";" & vbcrlf & "                        }" & vbcrlf & "               }" & vbcrlf & "               //新生成的下级下拉列表" & vbcrlf & "          _hasChild=false;"& vbcrlf & "              for (var i = 0; i < this.sortArr.length; i++){" & vbcrlf & "                  if (this.sortArr[i][2] == _iValue){" & vbcrlf & "                             if (_hasChild==false){" & vbcrlf & "                                  _siblingSelect=document.createElement(""select"");" & vbcrlf & "                                  this.showSelectObj.insertAdjacentElement(""beforeEnd"",_siblingSelect);" & vbcrlf & "                                        _siblingSelect.sortMenuObj=_sortMenuObj;" & vbcrlf & "                                        _siblingSelect.onchange=function(){" & vbcrlf & "                                             this.sortMenuObj.setSorts(this,this.sortMenuObj);" & vbcrlf & "                                       }" & vbcrlf & "                                       _siblingSelect.add(new Option(""根目录"",""""));" & vbcrlf & "                                        _siblingSelect.add(new Option(this.sortArr[i][1],this.sortArr[i][0]));" & vbcrlf & "                                 _hasChild=true;" & vbcrlf & "                         }" & vbcrlf & "                               else{                   " & vbcrlf & "                                        _siblingSelect.add(new Option(this.sortArr[i][1],this.sortArr[i][0]));" & vbcrlf & "                          }" & vbcrlf & "                       }" & vbcrlf &"         }" & vbcrlf & "        }" & vbcrlf & "" & vbcrlf & "      /**" & vbcrlf & "     * 根据最小类选取值生成整个联动菜单,由后往前递归完成" & vbcrlf & "     * _minCataValue:最小类的取值" & vbcrlf & "    * _sortMenuObj:sortMenu的实例对象，指向自己" & vbcrlf & "     */" & vbcrlf & "      this.newInit=function(_minCataValue,_sortMenuObj){" & vbcrlf & "              if (this.storeValueObj.value==""undefined"" || this.storeValueObj.value==""""){" & vbcrlf & "                  this.storeValueObj.value=_minCataValue;" & vbcrlf & "         }" & vbcrlf & "               if (_minCataValue == 0){" & vbcrlf & "                        //minCataValue为0，也就是初始化了" & vbcrlf & "                       this.initSorts(_sortMenuObj);" & vbcrlf & "                   //初始化完成后，退出函数" & vbcrlf &"                 return false;" & vbcrlf & "           }" & vbcrlf & "               //父级ID" & vbcrlf & "                _parentID=null;" & vbcrlf & "         _select=document.createElement(""select"");" & vbcrlf & "         _select.sortMenuObj=_sortMenuObj;" & vbcrlf & "               _select.onchange=function(){" & vbcrlf & "                    this.sortMenuObj.setSorts(this,this.sortMenuObj);" & vbcrlf & "              }" & vbcrlf & "               this.showSelectObj.insertAdjacentElement(""afterBegin"",_select);" & vbcrlf & "           _select.add(new Option(""根目录"","""")); " & vbcrlf & "              for (var i = 0; i < this.sortArr.length; i++){" & vbcrlf & "                  if (_minCataValue == this.sortArr[i][0]){" & vbcrlf & "                               _parentID = this.sortArr[i][2];" & vbcrlf & "                                break;" & vbcrlf & "                  }" & vbcrlf & "               }" & vbcrlf & "               for (var i = 0; i < this.sortArr.length; i++){" & vbcrlf & "                  if (this.sortArr[i][2] == _parentID){" & vbcrlf & "                           if (this.sortArr[i][0] == _minCataValue){" & vbcrlf & "                                       _opt=new Option(this.sortArr[i][1],this.sortArr[i][0]); " & vbcrlf & "                                       _select.add(_opt);" & vbcrlf & "                                      _opt.selected=""selected"";" & vbcrlf & "                         }" & vbcrlf & "                               else{" & vbcrlf & "                                   _select.add(new Option(this.sortArr[i][1],this.sortArr[i][0]));" & vbcrlf & "                         }" & vbcrlf & "                       }" & vbcrlf & "               }  "& vbcrlf &  "           if (_parentID > 0){ "& vbcrlf &  "                    this.newInit(_parentID,_sortMenuObj); "& vbcrlf &  "          } "& vbcrlf &  "      }" & vbcrlf & "}" & vbcrlf & "function DoSave(saveType){ "& vbcrlf & "    var hid = document.getElementById(""saveType"");" & vbcrlf &  "   hid.value = saveType; "& vbcrlf & "   var form= document.forms[""date""];" & vbcrlf & "     if (Validator.Validate(form,2)){" & vbcrlf & "                form.submit()" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%""  border=""0"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" height=""27"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""resetElementHidden"" width=""5%"" height=""27""  background=""../images/contentbg.gIf""><div align=""center""><img src=""../images/contenttop.gIf""height=""27""> </div></td>" & vbcrlf & "          <td class=""resetUniqueTabtleTitle"" width=""95%""  background=""../images/contentbg.gIf""><strong><font color=""#1445A6"">"
	Response.write OrderName
	Response.write PageTitle
	Response.write "设置</font></strong> </td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <form name=""date"" method=""post"" action=""Save_sort.asp?action="
	...
	Response.write "&Ord="
	Response.write Ord
	Response.write """ onsubmit=""return Validator.Validate(this,2)"" id=""demo"">" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td  colspan=""2""><div align=""center"">"
	Response.write OrderName
	Response.write PageTitle
	Response.write "设置</div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""15%"" align=""right"" style='*height:20px'>上级"
	'Response.write PageTitle
	Response.write "：</td>" & vbcrlf & "            <td width=""85%"" style='*height:20px;'>" & vbcrlf & "                              "
	If CanChild Then
		Response.write "<p id=""SortP""  style='margin:0px;'>&nbsp;</P><input name=""sortid""   type=""hidden"" id=""sortid""  value="""& sortID &""">"&_
		"<script language=""javascript"">"&_
		"var SortClass=new sortMenu(""sortid"",""SortP"",UserSortArr);"&_
		"SortClass.newInit("& sortID &",SortClass);"&_
		"</script>"
	else
		Response.write "<select Name=""sortid"">"&_
		"<option value=0 selected=selected>根目录</option>"&_
		"</select>"
	end if
	Response.write "" & vbcrlf & "                     </td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""15%"" align=""right"" height=""33"">"
	Response.write PageTitle
	Response.write "名称：</td>" & vbcrlf & "            <td width=""85%""><input name=""title"" type=""text"" id=""title"" value="""
	Response.write  title
	Response.write """ size=""20"" dataType=""Limit"" min=""1"" max=""50""  msg=""长度必须在1至50个字之间"" ></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td width=""15%"" align=""right"" height=""33"">重要指数：</td>" & vbcrlf & "            <td width=""85%""><input name=""px"" type=""text"" id=""px"" value="
	Response.write  px
	Response.write """  onkeyup=""value=value.replace(/[^\d\.]/g,'')"" dataType=""Limit"" min=""1"" max=""50""  msg=""必填""  size=""10"">" & vbcrlf & "              （排序框里只能输入数字，排序时将按照数字从大到小排序）</td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""resetHeadBg"" height=""35"" bordercolorlight=""#000000"" bordercolordark=""#000000"" bordercolor=""#CCFFFF"" colspan=""2""  style='border:0; background:#fff url(""../images/tb_top_td_bg.gif"") repeat-x' ><div align=""center"">" & vbcrlf & "                <input type=""submit"" name=""Submit422"" value=""保存""  class=""page""/>" & vbcrlf & "                             "
	If Action="add" Then
		Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit423"" value=""增加"" onclick=""DoSave('add')""  class=""page""/>" & vbcrlf & "                                <input type=""button"" name=""Submit424"" value=""复制"" onclick=""DoSave('copy')""  class=""page""/><input name=""saveType"" id=""saveType"" type=""hidden"" value=""save""/>" & vbcrlf & "                "
	end if
	Response.write "" & vbcrlf & "                             <input type=""reset"" value=""重填"" class=""page"" name=""B2"" />" & vbcrlf & "            </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </form>" & vbcrlf & "      </table></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td ><table width=""100%""border=""0"" align=""left"" > "& vbcrlf &  "       <tr> "& vbcrlf & "          <td height=""30"" ><div align=""center""></div></td> "& vbcrlf &   "      </tr> "& vbcrlf &   "    </table></td> "& vbcrlf & "  </tr> "& vbcrlf & "</table> "& vbcrlf & "</center>" & vbcrlf & "</div>" & vbcrlf
	conn.Close
	Set conn = Nothing
