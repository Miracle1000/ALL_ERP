...
'if rs_kz_zdy("MustFillin") Or Len(KZ_LIMITID&"")>0  Then retStr = retStr & " min=""1"""
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			j=14
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select id,name,sort,gl from zdy where sort1=21 and set_open=1 and dc=1 order by gate1 asc "
			rs1.open sql1,conn,1,1
			if rs1.eof then
			else
				do until rs1.eof
			...
			sql2="select * from ERP_CustomFields where TName=21 and IsUsing=1 and CanExport=1 order by FOrder desc "
			...
			set rs1=Nothing
			set rs2=server.CreateObject("adodb.recordset")
			sql2="select FName from ERP_CustomFields where TName=21 and IsUsing=1 and CanExport=1 order by FOrder desc "
			rs2.open sql2,conn,1,1
			If rs2.eof then
			else
				do until rs2.eof
					xlWorksheet.Cells(1,j).Value = rs2("FName")
					xlWorksheet.Cells(1,j).font.Size=10
					xlWorksheet.Cells(1,j).font.bold=true
					j=j+1
					xlWorksheet.Cells(1,j).font.bold=true
			...
			j=j-1
			set rs2=Nothing
			xlWorksheet.Cells(1,j+1).Value = "单位"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+1).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+1).font.bold=true
			set rs2=Nothing
			xlWorksheet.Cells(1,j+2).Value = "比例"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+2).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+2).font.bold=true
			set rs2=Nothing
			xlWorksheet.Cells(1,j+3).Value = "部门"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+3).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+3).font.bold=true
			set rs2=Nothing
			xlWorksheet.Cells(1,j+4).Value = "条形码"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+4).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+4).font.bold=true
			set rs2=Nothing
			xlWorksheet.Cells(1,j+5).Value = "建议进价"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+5).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+5).font.bold=true
			set rs2=Nothing
			xlApplication.ActiveSheet.Columns(j+5).NumberFormatLocal = "#,##0."&dotstr&"_ "
			set rs2=Nothing
			xlWorksheet.Cells(1,j+6).Value = "最高进价"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+6).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+6).font.bold=true
			set rs2=Nothing
			xlApplication.ActiveSheet.Columns(j+6).NumberFormatLocal = "#,##0."&dotstr&"_ "
			set rs2=Nothing
			xlWorksheet.Cells(1,j+7).Value = "建议售价"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+7).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+7).font.bold=true
			set rs2=Nothing
			xlApplication.ActiveSheet.Columns(j+7).NumberFormatLocal = "#,##0."&dotstr&"_ "
			set rs2=Nothing
			xlWorksheet.Cells(1,j+8).Value = "最低售价"
			set rs2=Nothing
			xlWorksheet.Cells(1,j+8).font.Size=10
			set rs2=Nothing
			xlWorksheet.Cells(1,j+8).font.bold=true
			set rs2=Nothing
			xlApplication.ActiveSheet.Columns(j+8).NumberFormatLocal = "#,##0."&dotstr&"_ "
			set rs2=Nothing
			j=j+9
			set rs2=Nothing
			set rs1=server.CreateObject("adodb.recordset")
			sql1="select id,sort2 from sort5 where time1=1  order by sort1 asc,gate2 desc"
			rs1.open sql1,conn,1,1
			if rs1.eof then
			else
				do until rs1.eof
					xlWorksheet.Cells(1,j).Value = rs1("sort2")
					xlWorksheet.Cells(1,j).font.Size=10
					xlWorksheet.Cells(1,j).font.bold=true
					j=j+1
					xlWorksheet.Cells(1,j).font.bold=true
			...
			elseif open_21_10=1 then
				list="and addcate in ("&intro_21_10&")"
			else
				list="and addcate=0"
			end if
			Str_Result="where del=1 "&list&""
	...
	if  A2<>"" then
		if Str_Result="" then
			Str_Result="where sort1 in ("&area_list&")"
		else
			Str_Result=Str_Result+"and  sort1 in ("&area_list&")"
			Str_Result="where sort1 in ("&area_list&")"
		end if
	end if
	if m1<>"" then
...
'elseif zdy4_1=6 then
	...
	elseif px=2 then
		px_Result="order by date7 asc,ord desc"
	elseif px=3 then
	...
	elseif px=9 then
		px_Result="order by aleat1 desc,ord desc"
	elseif px=10 then
		px_Result="order by aleat1 asc,ord desc"
	elseif px=11 then
		px_Result="order by aleat2 desc,ord desc"
	elseif px=12 then
		px_Result="order by aleat2 asc,ord desc"
	end if
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from product  "&Str_Result&"  "&px_Result&" "
	rs.open sql,conn,1,1
	C1=rs.recordcount
	dim i,j
	i=1
...
'do until rs1.eof
		...
		set rs1=nothing
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select sort1 from sortonehy where id="&unitjb&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			unitjbname=""
		else
			unitjbname=rs7("sort1")
		end if
		rs7.close
		set rs7=nothing
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select menuname from menu where id="&sort&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			sortname=""
		else
			sortname=rs7("menuname")
		end if
		rs7.close
		set rs7=nothing
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select name from tel where ord="&company&""
		rs7.open sql7,conn,1,1
		if rs7.eof then
			companyname=""
		else
		...
		date7=rs("date7")
		xlWorksheet.Cells(1+i,4).Value = sortname
		date7=rs("date7")
		xlWorksheet.Cells(1+i,4).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,5).Value = unitjbname
		date7=rs("date7")
		...
		date7=rs("date7")
		xlWorksheet.Cells(1+i,7).Value = aleat1
		date7=rs("date7")
		xlWorksheet.Cells(1+i,7).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,8).Value = aleat2
		date7=rs("date7")
		xlWorksheet.Cells(1+i,8).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,9).Value = companyname
		date7=rs("date7")
		xlWorksheet.Cells(1+i,9).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,10).Value = num_tc
		date7=rs("date7")
		xlWorksheet.Cells(1+i,10).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,11).Value = intro1
		date7=rs("date7")
		xlWorksheet.Cells(1+i,11).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,12).Value = intro2
		date7=rs("date7")
		xlWorksheet.Cells(1+i,12).font.Size=10
		date7=rs("date7")
		xlWorksheet.Cells(1+i,13).Value = intro3
		date7=rs("date7")
		...
		if rs1.eof then
			...
			do until rs1.eof
				if rs1("sort")=1 and rs(""&rs1("name")&"")<>"" then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select sort1 from sortonehy where ord="&rs(""&rs1("name")&"")&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						zdy=""
					else
				...
				xlWorksheet.Cells(1+i,j).Value = zdy
				zdy=rs(""&rs1("name")&"")
		...
		if rs2.eof then
			...
			do until rs2.eof
				...
				rs88.open "select FValue from ERP_CustomValues where FieldsID="&rs2("id")&" and OrderID="&ord&" ",conn,1,1
				...
				rs88.close
				Set rs88=nothing
				xlWorksheet.Cells(1+i,j).Value = c_Value
				Set rs88=nothing
				xlWorksheet.Cells(1+i,j).font.Size=10
				Set rs88=nothing
				j=j+1
		...
		set rs2=nothing
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,sort1 from sortonehy where ord="&unitjb&" order by gate1 desc "
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select bl,txm,price1,price1jy,price2jy,price2 from jiage where bm=0 and unit="&rs1("ord")&" and product="&ord&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
					bl=1
					txm=""
				...
				price2jy=0
				k=j
				xlWorksheet.Cells(1+i,1).Value = "【价格策略】"
				k=j
				xlWorksheet.Cells(1+i,1).font.Size=10
				k=j
				xlWorksheet.Cells(1+i,1).HorizontalAlignment=4
				k=j
				xlWorksheet.Cells(1+i,k).Value = rs1("sort1")
				k=j
				xlWorksheet.Cells(1+i,k).font.Size=10
				k=j
				xlWorksheet.Cells(1+i,k+1).Value = bl
				k=j
				xlWorksheet.Cells(1+i,k+1).font.Size=10
				k=j
				xlWorksheet.Cells(1+i,k+2).Value = "基础价格"
				k=j
				xlWorksheet.Cells(1+i,k+2).font.Size=10
				k=j
				xlWorksheet.Cells(1+i,k+3).Value = txm
				k=j
				xlWorksheet.Cells(1+i,k+3).font.Size=10
				k=j
				if open_21_21<>0 then
					xlWorksheet.Cells(1+i,k+4).Value = Formatnumber(price1jy,num_dot_xs,-1)
'if open_21_21<>0 then
				else
					xlWorksheet.Cells(1+i,k+4).Value = ""
'if open_21_21<>0 then
				end if
				xlWorksheet.Cells(1+i,k+4).font.Size=10
				if open_21_21<>0 then
'if open_21_21<>0 then
					xlWorksheet.Cells(1+i,k+5).Value = Formatnumber(price1,num_dot_xs,-1)
'if open_21_21<>0 then
				else
					xlWorksheet.Cells(1+i,k+5).Value = ""
'if open_21_21<>0 then
				end if
				xlWorksheet.Cells(1+i,k+5).font.Size=10
				if open_21_21<>0 then
'if open_21_22<>0 then
					xlWorksheet.Cells(1+i,k+6).Value = Formatnumber(price2jy,num_dot_xs,-1)
'if open_21_22<>0 then
				else
					xlWorksheet.Cells(1+i,k+6).Value = ""
'if open_21_22<>0 then
				end if
				xlWorksheet.Cells(1+i,k+6).font.Size=10
				if open_21_22<>0 then
'if open_21_22<>0 then
					xlWorksheet.Cells(1+i,k+7).Value = Formatnumber(price2,num_dot_xs,-1)
'if open_21_22<>0 then
				else
					xlWorksheet.Cells(1+i,k+7).Value = ""
'if open_21_22<>0 then
				end if
				xlWorksheet.Cells(1+i,k+7).font.Size=10
'if open_21_22<>0 then
				x=1
				set rs2=server.CreateObject("adodb.recordset")
				...
				do until rs2.eof
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select isnull(price3,0) as price3 from jiage where bm=0 and unit="&rs1("ord")&" and sort="&rs2("id")&" and product="&ord&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						price3=0
					else
					...
					set rs7=nothing
					if open_21_22<>0 then
						xlWorksheet.Cells(1+i,k+7+x).Value = Formatnumber(price3,num_dot_xs,-1)
'if open_21_22<>0 then
					else
						xlWorksheet.Cells(1+i,k+7+x).Value = ""
'if open_21_22<>0 then
					end if
					xlWorksheet.Cells(1+i,k+7+x).font.Size=10
'if open_21_22<>0 then
					x=x+1
'if open_21_22<>0 then
					rs2.movenext
				...
				sql3="select ord,sort1 from pricegate1 where num1=1 and ord in (select bm from jiage where product="&ord&") order by gate1 desc"
				rs3.open sql3,conn,1,1
				if rs3.eof then
				else
					do until rs3.eof
						set rs7=server.CreateObject("adodb.recordset")
						sql7="select txm,price1jy,price1,price2jy,price2 from jiage where bm="&rs3("ord")&" and unit="&rs1("ord")&" and product="&ord&""
						rs7.open sql7,conn,1,1
						if rs7.eof then
							txm=""
							price1jy=0
						...
						xlWorksheet.Cells(1+i,k+1).font.Size=10
						k=j
						xlWorksheet.Cells(1+i,k+2).Value = rs3("sort1")
						k=j
						xlWorksheet.Cells(1+i,k+2).font.Size=10
						k=j
						xlWorksheet.Cells(1+i,k+3).Value = txm
...
'if open_21_22<>0 then
						...
						do until rs2.eof
							set rs7=server.CreateObject("adodb.recordset")
							sql7="select price3 from jiage where bm="&rs3("ord")&" and unit="&rs1("ord")&" and sort="&rs2("id")&" and product="&ord&""
							rs7.open sql7,conn,1,1
							if rs7.eof then
								price3=0
							else
...
'if open_21_22<>0 then
		...
		set rs1=nothing
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,sort1 from sortonehy where ord in (select unit from jiage where product="&ord&") and ord<>"&unitjb&" order by gate1 desc "
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'if open_21_22<>0 then
				...
				do until rs2.eof
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select price3 from jiage where bm=0 and unit="&rs1("ord")&" and sort="&rs2("id")&" and product="&ord&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						price3=0
					else
...
'if open_21_22<>0 then
		...
		Response.write "<script>CountImage.width=" & Fix((z/C1) * 710) & ";" & VbCrLf
		Response.write "CountTXT.innerHTML=""共有<font color=red><b>"&C1&"</b></font>条数据!导出进度:<font color=red><b>" & Clng(FormatNumber(z/C1*100,4,-1)) & "</b></font>"";" & VbCrLf
		Response.write "<script>CountImage.width=" & Fix((z/C1) * 710) & ";" & VbCrLf
	...
	rs.close
	set rs=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>产品资料导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!产品资料导出成功,共有"&(z-1)&"条记录!</B>"";</script>"
	set rs=Nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("产品资料_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的产品资料</u></strong></font></a></p>" & vbcrlf & ""
