...
'If Not rs.Eof Then
	...
	function menu(id,opentype)
		...
		elseif open_77_1= 1 then
			Str_Result=" and (addcate in("&intro_77_1&") or cateid in("&intro_77_1&")) "
		else
			Str_Result=" and 1=2"
		end if
			...
			while not rs.eof
				...
				if ChildCount=0 then
					...
					while not rs7.eof and k<RecPageSize
						...
						mobile=trim(rs7("clictEmail"))
						if len(title_1&"")=0 then title_1=mobile
						mobile1=""
						if isEmail(mobile) then
							mobile1="<a href='###' title='"&trim(rs7("clictName"))&" <"&mobile&">' onClick=""changeemail('"&rs7("ord")&"','"&mobile&"',true);try{window.frames['cFF2'].document.getElementById('date').submit();}catch(e1){}"">"
							mobile2="</a><br/>"&chr(13)
						else
							mobile1=""
							mobile2="<br/>"&chr(13)
						end if
						Response.write("<img id='IMG"&rs7("ord")&"_"&mobile&"' src='../images/155.gif'>"&mobile1)
						Response.write(""&zh_ck&" "&title_1&mobile2)
...
'if C<>"" and C<>"检索关键字" then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql7="select * from email_Person where  (sort is null or sort=0 or sort in(select id from email_Person_class where (CHARINDEX(',"&session("personzbintel2007")&",',','+user_list+',')>0 or user_list='0,0' or user_list='0' or isnull(user_list,'')='') and (del=1 or del is null))) and " &strsort&str_Result
			...
			while not rs.eof and k<RecPageSize
				title_1=rs("clictName")
				mobile=trim(rs("clictEmail"))
				if len(title_1&"")=0 then title_1=mobile
				mobile1=""
				if isEmail(mobile) then
					mobile1="<a href='###' title='"&trim(rs("clictName"))&" <"&mobile&">'  onClick=""changeemail('"&rs("ord")&"','"&mobile&"',true);try{window.frames['cFF2'].document.getElementById('date').submit();}catch(e1){}"">"
					mobile2=mobile&"</a><br/>"&chr(13)
				else
					mobile1=""
					mobile2="<br/>"&chr(13)
				end if
				Response.write("<img id='IMG"&rs("ord")&"_"&mobile&"' src='../images/155.gif'>"&mobile1)
				Response.write(""&zh_ck&" "&title_1&mobile2)
...
'if Pagination=true then
			...
			if Pagination and recCount>0 then
				...
				Response.write "<input id='txtGoToPage_"&id&"' value='"&P&"' style='width:30px;height:18px'  maxlength=8 onkeyup=value=value.replace(/[^\d]/g,'') type=text style='width:30px'>"&chr(13)
	...
	Response.write "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>邮件发送</title>" & vbcrlf & "<style>" & vbcrlf & "   html, body{height:100%;margin:0px;}" & vbcrlf & "   iframe[name=""I2""]{" & vbcrlf & "     width:100%!important;" & vbcrlf & "   }" & vbcrlf & "</style>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "function frameResize(){" & vbcrlf & "     try{" & vbcrlf & "        document.getElementById(""cFF2"").style.height = I2.document.documentElement.scrollHeight + 0 + ""px"";" & vbcrlf & "     }catch(e){}" & vbcrlf & "}" & vbcrlf & "function $(name){" & vbcrlf & "   return document.getElementById(name);" & vbcrlf & "}" &vbcrlf & "function showmenu(self,showname){" & vbcrlf & " var tableObj=self.parentNode.parentNode;" & vbcrlf & "        for(var i=0;i<tableObj.rows.length;i++){" & vbcrlf & "                var tdObj=tableObj.rows(i).cells(0);" & vbcrlf & "            if(tdObj==self){" & vbcrlf & "                        var old=tdObj.className;" & vbcrlf & "                        tdObj.className=(old==""menutab""?""menutab_hover"":""menutab"");" & vbcrlf & "             }else if(tdObj==$(showname)){" & vbcrlf & "                   var old=tdObj.className;" & vbcrlf & "                        tdObj.className=(old==""menulist""?""menulist_hover"":""menulist"");" & vbcrlf & "                }else{" & vbcrlf & "                  if(tdObj.id){" & vbcrlf & "           tdObj.className=""menulist"";" & vbcrlf & "                       }else{" & vbcrlf & "                          tdObj.className=""menutab"";" & vbcrlf & "                        }" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "function switchMenu(self){" & vbcrlf & "    var subobj=self.getElementsByTagName(""ul"")[0];" & vbcrlf & "    if(self.className==""hover""){" & vbcrlf & "           self.className="""";" & vbcrlf & "                subobj.style.display=""none"";" & vbcrlf & "      }else{" & vbcrlf & "          self.className=""hover"";" & vbcrlf & "           subobj.style.display=""block"";" & vbcrlf & "     }" & vbcrlf & "}" & vbcrlf & "function showHidaLeftMenu(self){" & vbcrlf & "      varleftObj=$(""leftmenuall"");" & vbcrlf & "     if(leftObj.style.display==""none""){" & vbcrlf & "                leftObj.style.display=""block"";" & vbcrlf & "            self.src=""../images/hideleft.gif"";" & vbcrlf & "        }else{" & vbcrlf & "          leftObj.style.display=""none"";" & vbcrlf & "             self.src=""../images/showleft.gif"";"& vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "function ajaxSubmit(sort1){" & vbcrlf & "    //获取用户输入" & vbcrlf & "    var B=document.getElementById(""B"").value;" & vbcrlf & "    var C=document.getElementById(""C8"").value;" & vbcrlf & "    var url = ""../message/search_yh.asp?B=""+escape(B)+""&C=""+escape(C) +""&sort1=""+escape(sort1) + ""&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  xmlHttp.open(""GET"", url, false);" & vbcrlf& "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "  updatePage_cp();" & vbcrlf & "  };" & vbcrlf & "  xmlHttp.send(null);  " & vbcrlf & "}" & vbcrlf & "function updatePage_cp() {" & vbcrlf & "  if (xmlHttp.readyState < 4) {" & vbcrlf & "       cp_search.innerHTML=""loading..."";" & vbcrlf & "  }"& vbcrlf & "  if (xmlHttp.readyState == 4) { "& vbcrlf &    " var response = xmlHttp.responseText; "& vbcrlf &    "  cp_search.innerHTML=response; "& vbcrlf &  "  xmlHttp.abort();" & vbcrlf &  " }" & vbcrlf & "}" & vbcrlf & vbcrlf &" function ajaxSubmit2(sort1){" & vbcrlf &     "setTimeout(""ajaxSubmit(2);"",500);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "</script>" & vbcrlf & "<script type=""text/JavaScript"">" & vbcrlf & "<!--" & vbcrlf & "function MM_findObj(n, d) { //v4.01" & vbcrlf & "  var p,i,x;  if(!d) d=document; if((p=n.indexOf(""?""))>0&&parent.frames.length) {" & vbcrlf & "    d=parent.frames[n.substring(p+1)].document; n=n.substring(0,p);}" & vbcrlf & "  if(!(x=d[n])&&d.all) x=d.all[n]; for (i=0;!x&&i<d.forms.length;i++) x=d.forms[i][n];" & vbcrlf & "  for(i=0;!x&&d.layers&&i<d.layers.length;i++) x=MM_findObj(n,d.layers[i].document);" & vbcrlf & "  if(!x && d.getElementById) x=d.getElementById(n); return x;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function MM_preloadImages() { //v3.0" & vbcrlf & "  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();" & vbcrlf & "    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)" & vbcrlf & " if (a[i].indexOf(""#"")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function MM_swapImgRestore() { //v3.0" & vbcrlf & "  var i,x,a=document.MM_sr; for(i=0;a&&i<a.length&&(x=a[i])&&x.oSrc;i++) x.src=x.oSrc;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function MM_swapImage() { //v3.0" & vbcrlf & "  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)" & vbcrlf & "   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}" & vbcrlf & "}" & vbcrlf & "//-->" & vbcrlf & "</script>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" height=""100%"" border=""0"" cellpadding=""0"" cellspacing=""1"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td valign=""top"" id='rightmenuall'>"
	Response.write "<script language=""javascript"">" & vbcrlf & "function changeemail(recid,mobilenum,flg){" & vbcrlf & " try" & vbcrlf & "     {" & vbcrlf & "               var obj = window.ActiveXObject?window.frames['cFF2']:document.getElementById('cFF2').contentWindow;" & vbcrlf & "             var tmpstr=obj.document.getElementById('takeName').value;" & vbcrlf & "              if(("",""+tmpstr+"","").indexOf("",""+recid+"","")<0){" & vbcrlf & "                  if(tmpstr==""""){" & vbcrlf & "                           tmpstr=recid;" & vbcrlf & "                   }else{" & vbcrlf & "                          tmpstr+="",""+recid;" & vbcrlf & "                        }" & vbcrlf & "                       if(document.getElementById(""IMG""+recid+""_""+mobilenum)){document.getElementById(""IMG""+recid+""_""+mobilenum).src=""../images/d14.gif"";}" & vbcrlf & "" & vbcrlf & "          }" & vbcrlf & "               else{" & vbcrlf & "                   if(flg){" & vbcrlf & "                                tmpstr=("",""+tmpstr+"","").replace("",""+recid+"","","","");" & vbcrlf & "                               if(tmpstr!=""""){" & vbcrlf & "                                   if(tmpstr=="",""){" & vbcrlf & "                                          tmpstr="""";" & vbcrlf & "                                        }else{" & vbcrlf & "                                          tmpstr=tmpstr.substring(1,tmpstr.length-1);" & vbcrlf & "                                     }" & vbcrlf & "                               }" & vbcrlf & "                               if(document.getElementById(""IMG""+recid+""_""+mobilenum)){document.getElementById(""IMG""+recid+""_""+mobilenum).src=""../images/155.gif"";}" & vbcrlf & "                        }" & vbcrlf & "               }" & vbcrlf & "               obj.document.getElementById('takeName').value=tmpstr;" & vbcrlf & "           if (flg){obj.document.getElementById('date').submit();}" & vbcrlf & " }catch(e1){}" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function getAll(cid){"& vbcrlf & "      var xmlhttp=window.ActiveXObject? new ActiveXObject(""Microsoft.XMLHTTP""):new XMLHttpRequest();" & vbcrlf &    " if(xmlhttp){ "& vbcrlf &      "       xmlhttp.onreadystatechange=function(){" & vbcrlf &            "       if(xmlhttp.readyState==4){ "& vbcrlf &                   "            var yy=unescape(xmlhttp.responseText).split(""</noscript>"")[1];" & vbcrlf & "                           var zz=yy.split("","");" & vbcrlf & "                             for(var m=0;m<zz.length;m++){" & vbcrlf & "                                   var kk=zz[m].split("";"");" & vbcrlf & "                                  if(kk[0]&&kk[1]){" & vbcrlf & "                                               changeemail(kk[0],kk[1],false);" & vbcrlf & "                                 }" & vbcrlf & "                               }" & vbcrlf & "                               try{    "& vbcrlf & "                                        var obj = window.ActiveXObject?window.frames['cFF2']:document.getElementById('cFF2').contentWindow;" & vbcrlf & "                                     obj.document.getElementById('date').submit();" & vbcrlf & "                           }catch(e1){" & vbcrlf & "                                     window.frames['cFF2'].src=""EmailList.asp"";" & vbcrlf & "                                }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               xmlhttp.open(""get"",""getPerson.asp?cid=""+cid+""&r=""+Math.round(Math.random()*100),false);" & vbcrlf & "               xmlhttp.send(null);" & vbcrlf & "     }" & vbcrlf & "}" & vbcrlf & "function chageIframe(url){" & vbcrlf & "    var urlValue = url;" & vbcrlf & "     if(urlValue!=""""){" & vbcrlf & "                document.getElementById(""cFF2"").src = urlValue;" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "function ajaxSubmit_page(pid,pagenum,ftype){" & vbcrlf & "  var xmlhttp=window.ActiveXObject? new ActiveXObject(""Microsoft.XMLHTTP""):new XMLHttpRequest();" & vbcrlf & "  if(xmlhttp)" & vbcrlf & "     {" & vbcrlf & "               xmlhttp.onreadystatechange=function(){" & vbcrlf & "                  if(xmlhttp.readyState==4)" & vbcrlf & "                       {" & vbcrlf & "                               var yy=unescape(xmlhttp.responseText).split(""</noscript>"")[1];" & vbcrlf & "                            if(ftype==3||!document.getElementById(""a""+pid)){ "& vbcrlf &             "                      document.getElementById(""cp_search"").innerHTML=yy; "& vbcrlf &                "                 } "& vbcrlf &                        "        else{ "& vbcrlf &                             "       document.getElementById(""a""+pid).cells[0].innerHTML=yy; "& vbcrlf &                 "           } "& vbcrlf &              "                  //top.document.getElementById(""cFF2"").style.height=top.document.getElementById(""cFF2"").document.body.scrollHeight+0+""px"";" & vbcrlf & "                               var imgobj=document.getElementsByTagName(""img"");" & vbcrlf & "                          var allmobile="""";" & vbcrlf & "                         try{" & vbcrlf & "                                    var obj = window.ActiveXObject?window.frames['cFF2']:document.getElementById('cFF2').contentWindow;" & vbcrlf & "                                      allmobile=obj.document.getElementById('takeName').value;" & vbcrlf & "                                }catch(e1){" & vbcrlf & "                                     document.getElementById('cFF2').src=""EmailList.asp"";" & vbcrlf & "                              }" & vbcrlf & "                               for(var j=0;j<imgobj.length;j++){" & vbcrlf & "                                       var tmpid=imgobj[j].id;" & vbcrlf & "                                  if(tmpid&&tmpid.substring(0,3)==""IMG""){" & vbcrlf & "                                           var tmpmobile=tmpid.substring(tmpid.indexOf(""_"")+1);" & vbcrlf & "                                              if(("",""+allmobile+"","").indexOf("",""+tmpmobile+"","")>=0){" & vbcrlf & "                                                  imgobj[j].src=""../images/d14.gif"";" & vbcrlf & "                                                }" & vbcrlf& "                                        }" & vbcrlf & "                               }" & vbcrlf & "                       }" & vbcrlf & "               }" & vbcrlf & "               xmlhttp.open(""get"",""search_person.asp?B=""+document.getElementById(""B"").value+""&C=""+document.getElementById(""C8"").value+""&P=""+pagenum+""&sort=""+pid+""&ftype=""+ftype+""&r=""+Math.round(Math.random()*100),false);" & vbcrlf & "          xmlhttp.send(null);" & vbcrlf & "     }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function goPage(pageid,ftype,recPageCount){" & vbcrlf & "   var pagenum=document.getElementById('txtGoToPage_'+pageid).value;" & vbcrlf & "       if(pagenum&&!isNaN(pagenum)){" & vbcrlf & "           if(parseInt(pagenum)>recPageCount){" & vbcrlf & "                      ajaxSubmit_page(pageid,parseInt(recPageCount),ftype);" & vbcrlf & "           }else{" & vbcrlf & "                  ajaxSubmit_page(pageid,parseInt(pagenum),ftype);" & vbcrlf & "                }" & vbcrlf & "       }" & vbcrlf & "       else{" & vbcrlf & "           alert('请输入数字');" & vbcrlf & "            document.getElementById('txtGoToPage_'+pageid).value=1;" & vbcrlf & "          document.getElementById('txtGoToPage_'+pageid).select();" & vbcrlf & "        }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "//高级搜索" & vbcrlf & "function Left_adSearch(obj){" & vbcrlf & "        var sdivobj=document.getElementById(""adsDiv"");" & vbcrlf & "    if(sdivobj.style.display!=""none""){" & vbcrlf & "               Left_adClose();" & vbcrlf & " }else{" & vbcrlf & "          var x=obj.offsetLeft,y=obj.offsetTop;" & vbcrlf & "           var obj2=obj;" & vbcrlf & "           var offsetx=0;" & vbcrlf & "          while(obj2=obj2.offsetParent){" & vbcrlf & "                  x+=obj2.offsetLeft;" & vbcrlf & "                     y+=obj2.offsetTop;" & vbcrlf & "              }" & vbcrlf & "               sdivobj.style.left=x+33+""px"";" & vbcrlf & "             sdivobj.style.top=y+""px"";" & vbcrlf & "         sdivobj.style.display=""inline"";" & vbcrlf & "   }" & vbcrlf & "       document.getElementById('adsIF').style.height=document.getElementById('adsIF').contentWindow.document.getElementsByTagName('table')[1].offsetHeight+30+'px';" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function Left_adClose(){" & vbcrlf & "  document.getElementById('adsDiv').style.display=""none"";" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function advance(result){" & vbcrlf & " document.getElementById(""cp_search"").innerHTML = result;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "button.wavbutton {" & vbcrlf & "  " & vbcrlf & "        text-align:36px;" & vbcrlf & "        font-size:12px;" & vbcrlf & " cursor:pointer;" & vbcrlf & " border:0;" & vbcrlf & "" & vbcrlf & "       padding: 0 3px;" & vbcrlf & " padding-top:0;" & vbcrlf & "  height: 28px;" & vbcrlf & "   line-height:26px;" & vbcrlf & "       *line-height:25px;" & vbcrlf & "}" & vbcrlf & "tr.hei_37 td,tr.hei_37 td.name{height:35px;}" & vbcrlf & "</style>" & vbcrlf & " <div id=""adsDiv"" style=""position:absolute;zindex:2000;display:none"">" & vbcrlf & "  <iframe src=""EmailAddressADSearch.asp"" id=""adsIF"" width=""400px"" frameborder=""0"" border=""0"" scrolling=""no""></iframe>" & vbcrlf & " </div>" & vbcrlf & "     <table border=""0"" cellpadding=""0"" width=""195px""  cellspacing=""0"" id=""placeInfo"">" &vbcrlf & "      <tr>" & vbcrlf & "        <td  id=""placeInfo"">&nbsp;邮件地址库</td>" & vbcrlf & "      </tr>" & vbcrlf & "    </table>" & vbcrlf & " <table height=""100%"" border=""0"" width=""193""  cellpadding=""0"" cellspacing=""0"" style=""border:1px solid #ccc;"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td valign=""top"" background=""../images/m_leftbg.gif"">" & vbcrlf & "                  <table width=""193px"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" height=""100px"" >" & vbcrlf & "                               <tr>" & vbcrlf & "                   <td class=""resetHeadBg"" height=""27"" valign=""middle"" style=""height:55px;background:#f4fafe;"">" & vbcrlf & "                    <div align=""center"">" & vbcrlf & "                                        <form name=""form"" method=""post"" style=""margin:0"">" & vbcrlf & "                    <select name=""B"" id=""B""  >" & vbcrlf & "                        <option value=""name"" "
	if B="name" then
	...
	Response.write ">邮箱</option>" & vbcrlf & "                                        </select>" & vbcrlf & "                    <input name=""C"" id=""C8"" type=""text"" size=""10"" value=""检索关键字"" style=""width:70px;font-size: 9pt;color: #999999;text-align:center;"" onFocus=""if(value==defaultValue){value='';this.style.color='#000'}""  onkeyup=""ajaxSubmit_page(0,1,3)"" onKeyPress=""if(event.keyCode==13) {ajaxSubmit_page(0,1,3);return false;}""/>" & vbcrlf &                       "                 <input name=""top"" type=""hidden"" size=""2""  value="""
	Response.write top
	Response.write """>" & vbcrlf & "                                 </form>" & vbcrlf & "                    <button class='wavbutton anybutton' style='width:44px;' onclick=""Left_adSearch(this);"" >高级</button>" & vbcrlf & "                    </div>" & vbcrlf & "                    </td>" & vbcrlf & "                               </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                             <td valign=""center"" class=""menutab resetTableBgColor resetBorderColor"" style=""height:37px;background:#f4fafe;border-bottom:1px solid #ccc;border-top:1px solid #ccc;padding:0;"">" & vbcrlf & "                          <table class=""resetGroupTableBg"" width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"" style=""background-image:url(../../../sysa/images/smico/lmeuntab_bg_c.gif);"">" & vbcrlf & "                                                 <tr class=""hei_37"">" & vbcrlf & "                                                     <td width=""35%""  height=""27"" class=""name""><b>邮箱列表</b></td>" & vbcrlf & "                                                        <td class=""name"" align=""right""><a href=""###"" onclick=""ajaxSubmit_page(0,1,1);"">显示分类</a>&nbsp;"
	if open_77_12=1 or  open_77_12=3  then
		Response.write "<a href=""../email_sort/add_cp.asp"" target=""_blank"">添加分组</a>"
	end if
	Response.write "</td>" & vbcrlf & "                                           </tr>" & vbcrlf & "                                   </table>" & vbcrlf & "                                </td>" & vbcrlf & "                         </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                              <td valign=""top"">" & vbcrlf & "                                         <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""3"" id=""leftmenu1"">" & vbcrlf & "  <tr>" & vbcrlf & "                                              <td>" & vbcrlf & "                                                          <span id=""cp_search"">" & vbcrlf & "                                                             "
	menu 0,1
	Response.write "" & vbcrlf & "                                                              </span>" & vbcrlf & "                                           </td>" & vbcrlf & "                                         </tr>" & vbcrlf & "                                     </table>" & vbcrlf & "                                </td>" & vbcrlf & "                         </tr>" & vbcrlf & "          </table>" & vbcrlf & "          </td>" & vbcrlf & "       </tr>" & vbcrlf & "       <tr>" & vbcrlf & "          <td></td>" & vbcrlf & "        </tr>" & vbcrlf & "    </table>" & vbcrlf & ""
	Response.write "" & vbcrlf & "    </td>" & vbcrlf & "    <td valign=""top"" width='100%' ><iframe name=""I2"" id=""cFF2"" src=""../email/EmailList.asp?time="
	Response.write now()
	Response.write """ width=""100%""  onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self""  > 浏览器不支持嵌入式框架，或被配置为不显示嵌入式框架。</iframe></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</body>" & vbcrlf & "" & vbcrlf & "</html>"
