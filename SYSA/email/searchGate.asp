...
'if strW3<>"" Then
	...
	loadNum2=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from gate  where del=1 and email is not null"
		...
		do until rs.eof
			...
			if email<>"" then
				...
				if  rs2.eof then
					...
					conn.execute"insert into email_Person (clictName,clictEmail,clictSex,clictJob,complete,addtime,del,addcate,sort) values('"&rs("name")&"','"&email&"','"&rs("sex")&"','"&rs("title")&"',1,'"&now()&"',1,"&session("personzbintel2007")&","&classType&")"
					loadNum1=loadNum1+1
				else
					loadNum2=loadNum2+1
				end if
