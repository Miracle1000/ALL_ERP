		...
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				elseif open_6_11=1 then
					str_Result11=str_Result11+ " and sorce2 in ("&intro_6_11&") "
'elseif open_6_11=1 then
					str_Result22=str_Result22+ " and b.cateid in ("&intro_6_11&") "
'elseif open_6_11=1 then
					str_Result33=str_Result33+ " and cateid in ("&intro_6_11&") "
'elseif open_6_11=1 then
					list=" ord in ("&intro_6_11&") and "
				else
					str_Result11=str_Result11+ " and sorce2 =0 "
					list=" ord in ("&intro_6_11&") and "
					str_Result22=str_Result22+ " and b.cateid =0 "
					list=" ord in ("&intro_6_11&") and "
					str_Result33=str_Result33+ " and cateid =0 "
					list=" ord in ("&intro_6_11&") and "
					list=" ord in (0) and "
				end if
				...
				duemoney7=request("duemoney7")
				duemoney8=request("duemoney8")
				bz=request("bz")
				duepaydate1=request("duepaydate1")
				duepaydate2=request("duepaydate2")
				str_Result12="where 1=1 "
...
'if m2="" then
				...
				if m1<>"" then
					str_Result11=str_Result11+ " and datejk>='"&m1&"' "
'if m1<>"" then
					str_Result22=str_Result22+ " and date1>='"&m1&"' "
'if m1<>"" then
					str_Result33=str_Result33+ " and date1>='"&m1&"' "
'if m1<>"" then
				end if
				if m2<>"" then
					str_Result11=str_Result11+ " and datejk<='"&m2&"' "
'if m2<>"" then
					str_Result22=str_Result22+ " and date1<='"&m2&"' "
'if m2<>"" then
					str_Result33=str_Result33+ " and date1<='"&m2&"' "
'if m2<>"" then
				end if
				...
				if A2<>"0" then
					str_Result12=str_Result12+" and sortbzid="&A2&" "
'if A2<>"0" then
				end if
				W3=getW3(W1,W2,W3)
				W4=replace(W3,"0","")
				W4=replace(W4,",","")
				if W4<>"" then    str_Result12=str_Result12+"and ord in  ("&W3&")"
				W4=replace(W4,",","")
				if duemoney1<>"" and duemoney2<>"" then
					str_Result12=str_Result12+ " and money1 between "&duemoney1&" and "&duemoney2&" "
'if duemoney1<>"" and duemoney2<>"" then
				end if
				if duemoney3<>"" and duemoney4<>"" then
					str_Result12=str_Result12+ " and money2 between "&duemoney3&" and "&duemoney4&" "
'if duemoney3<>"" and duemoney4<>"" then
				end if
				if duemoney5<>"" and duemoney6<>"" then
					str_Result12=str_Result12+ " and money3 between "&duemoney5&" and "&duemoney6&" "
'if duemoney5<>"" and duemoney6<>"" then
				end if
				str_Result122="where 1=1 "
				if duemoney7<>"" and duemoney8<>"" then
					str_Result122=str_Result122+ " and money4 between "&duemoney7&" and "&duemoney8&" "
'if duemoney7<>"" and duemoney8<>"" then
				end if
				if bz<>"0" and bz<>"" then
					str_Result12=str_Result12+" and sortbzid="&bz&" "
'if bz<>"0" and bz<>"" then
				end if
				...
				if px=1 then
					px_Result="order by cateidname desc"
				elseif px=2 then
					px_Result="order by cateidname asc"
				elseif px=3 then
				...
				elseif px=8 then
					px_Result="order by money3 asc"
				elseif px=9 then
					px_Result="order by money4 desc"
				elseif px=10 then
					px_Result="order by money4 asc"
				end if
				...
				money4_all=0
				n=0
				sql="select * from (select *,(money1-money5-money3) as money4 "&_
				"from (select gate.ord,gate.name as cateidname,sortbz.id as sortbzid,sortbz.sort1 as sortbz,sortbz.intro,"&_
				"(select isnull(sum(spmoney),0) from payjk where del=1 "&str_Result11&" and (spstate=1 or spstate=4 or spstate is null) and isnull(bz,14)=sortbz.id and sorce2=gate.ord) as money1,"&_
				"(select isnull(sum(a.money1),0) from paybxlist a left join pay b on a.payid=b.ord left join paybx c on a.bxid=c.id where c.complete=3 and a.del=1 and b.del=1  "&str_Result22&" and  b.cateid=gate.ord and isnull(b.bz,14)=sortbz.id) as money2,"&_
				"(select isnull(sum(money1),0) from pay where del=1 "&str_Result33&" and cateid=gate.ord  and  jkid in (select id from payjk where del=1 and isnull(bz,14)=sortbz.id)) as money3,"&_
				"    (select isnull(sum(dkmoney),0) as money5 from dk where del=1 ""&str_Result33&"" and bxid in(select id from paybx where del=1 and complete=3 and isnull(bz,14)=sortbz.id and id in (select bxid from paybxlist where del=1 and payid in (select ord from pay where cateid=gate.ord and del=1 and complete=3)))) as money5 "&_
				"    from gate,sortbz where ""&list&"" gate.del=1 and (gate.ord in (select sorce2 from payjk where del=1) or gate.ord in (select cateid from pay where del=1)) and (sortbz.id in (select bz from payjk where del=1 ) or sortbz.id in (select bz from paybx where del=1))) as fy1 ""&str_Result12&"" and (money1<>0 or money2<>0 or money3<>0) ) as fyy "&str_Result122&" "&px_Result&" "
				set rs=server.CreateObject("adodb.recordset")
				rs.open sql,conn,1,1
				C1=rs.recordcount
				xlApplication.Init "费用汇总表_"&session("name2006chen")&".xls",C1
				dim i
				i=1
				if rs.RecordCount<=0 then
					...
					do until rs.eof
						...
						if rs("money5")="" or IsNull(zbcdbl(rs("money5"))) then
							money5=0
						else
							money5=zbcdbl(rs("money5"))
						end if
						...
						if rs("money4")="" or IsNull(zbcdbl(rs("money4"))) then
							money4=0
						else
							money4=zbcdbl(rs("money4"))
						end if
						dim gatearray
						gatearray = getgatearray(rs("ord"))
						if isarray(gatearray) then
							catename = gatearray(0,0)
							sorce1cn = gatearray(1,0)
							sorce2cn = gatearray(2,0)
						end if
						xlWorksheet.Cells(1+i,1).Value = sorce1cn
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,1).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,2).Value = sorce2cn
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,2).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,3).Value = catename
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,3).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,4).Value = sortbz
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,4).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,5).Value = Formatnumber(money1,num_dot_xs,-1)
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,5).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,6).Value = Formatnumber(money5,num_dot_xs,-1)
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,6).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,7).Value = Formatnumber(money3,num_dot_xs,-1)
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,7).font.Size=10
						sorce2cn = gatearray(2,0)
						xlWorksheet.Cells(1+i,8).Value = Formatnumber(money4,num_dot_xs,-1)
						sorce2cn = gatearray(2,0)
...
'loop
					xlWorksheet.Cells(1+i,1).colspan = 4
'loop
					xlWorksheet.Cells(1+i,2).Value = Formatnumber(summoney1,num_dot_xs,-1)
'loop
					xlWorksheet.Cells(1+i,2).font.Size=10
'loop
					xlWorksheet.Cells(1+i,3).Value = Formatnumber(summoney5,num_dot_xs,-1)
'loop
					xlWorksheet.Cells(1+i,3).font.Size=10
'loop
...
'loop
					xlWorksheet.Cells(1+i,4).font.Size=10
'loop
					xlWorksheet.Cells(1+i,5).Value = Formatnumber(summoney4,num_dot_xs,-1)
'loop
					xlWorksheet.Cells(1+i,5).font.Size=10
'loop
				end if
				rs.close
				set rs=Nothing
				Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>费用汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!费用汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
				set rs=Nothing
				Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
				Set fs = CreateObject("Scripting.FileSystemObject")
				tfile=Server.MapPath("费用汇总表_"&session("name2006chen")&".xls")
				...
				Response.write """><font class=""red""><strong><u>下载导出的费用汇总表</u></strong></font></a></p>"
