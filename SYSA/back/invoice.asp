...
'Set rs = conn.execute("SELECT top 1 1 FROM (SELECT id,CAST(short_str AS INT) cateid FROM dbo.split('"& intro_33_3 &"',',')) a INNER JOIN (select distinct s.cateid from send s inner join kuout k on k.ord=s.kuout and s.del=1 and k.sort1 in(1,4) and k.order1 in("& htords &")) b on a.cateid=b.cateid AND a.cateid>0")
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false>" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "              <tr>" & vbcrlf & "            <td class=""place"">开票计划回收站</td>" & vbcrlf & "             <td>&nbsp;</td>" & vbcrlf & "         <td align=""right""><input type=""button"" name=""Submitdel"" class=""anybutton"" value=""全部清空"" onClick=""if(confirm('确认清空本回收站里的所有内容？')){window.location.href='delall.asp?ord=paybackinvoice&url=invoice.asp&bank=1'}""/></td>" & vbcrlf & "               <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "              </tr>" & vbcrlf & "   </table>  " & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "              <tr class=""top"">" & vbcrlf & "          <td width=""40""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "         <td width=""40""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "               <td width=""10%""><div align=""center""><strong>开票总额</strong></div></td>" & vbcrlf & "            <td width=""15%""><div align=""center""><strong>开票进展</strong></div></td>" & vbcrlf & "            <td width=""15%""><div align=""center""><strong>单据主题</strong></div></td>" & vbcrlf & "            <td width=""10%""><div align=""center""><strong>开票人员</strong></div></td>" & vbcrlf & "               <td width=""10%""><div align=""center"">删除人员</div></td>" & vbcrlf & "             <td width=""15%""><div align=""center"">删除日期</div></td>" & vbcrlf & "             <td width=""15%""><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "            </tr>" & vbcrlf & "           "
	dim n , canDelete
	n=0
	canDelete = True
	set rs=server.CreateObject("adodb.recordset")
	sql="select *,(select intro from sortbz where id=paybackinvoice.bz) as bzname,(select name from gate where ord=paybackinvoice.invoicecate) as invoicecateName,(select name from gate where ord=paybackinvoice.delcate) as delcateName  from paybackinvoice  where del=2 order by deldate desc"
	...
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "                      <form name=""form1"" method=""post"" action=""deletekpall.asp?ord="
		...
		do until rs.eof
			isinvoiced= rs("isinvoiced") : kpbz=rs("bz") : kpInvoiceType=rs("invoiceType")
			If kpbz&"" = "" Then kpbz = 0
			If kpInvoiceType&"" = "" Then kpInvoiceType = 0
			...
			Response.write rs("id")
			Response.write """></span></td> " & vbcrlf & "                                    <td align=""center"" height=""24""><span class=""gray"">"
			Response.write Rs.recordcount-Rs.pagesize*(currpage-1)-n
			Response.write "</span></td>" & vbcrlf & "                                  <td height=""27"" ><div align=""center""><span class=""red"">"
			Response.write rs("bzname")
			Response.write Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1)
			Response.write rs("bzname")
			Response.write "</span></div></td>" & vbcrlf & "                                    <td><div align=""center""><span class=""gray"">" & vbcrlf & "                                         "
			if isinvoiced="0" Then
				Response.write "未申请"
			elseif isinvoiced="3" then
				Response.write "已申请已废止"
			elseif isinvoiced="11" then
				Response.write "已申请未开票"
			elseif isinvoiced="1" then
				Response.write "已申请已开票"
			elseif isinvoiced="2" then
				Response.write "预收款开票"
			end if
			Response.write "" & vbcrlf & "                                              </span></div></td>" & vbcrlf & "                                      "
			fromType = rs("fromType")
			fromid = rs("fromid")
			isRenew = True
			If fromType ="CONTRACT" Or fromType ="PREBACK" or fromType ="ContractTH" Then
				If conn.execute("select count(1) from paybackinvoice where del=1 and fromtype='"&fromType&"' and fromid="&fromid&"")(0)>0 Then isRenew = False
			end if
			if isRenew And fromType ="CONTRACT" then
				htbz = GetContractBz(fromid)
				if kpbz <> htbz then isRenew = False
			end if
			if isRenew And fromType ="CONTRACT" And kpInvoiceType>0 then
				htInvoiceType = GetHzKpHtInvoiceType(fromid)
				if kpInvoiceType <> htInvoiceType and htInvoiceType>0 then isRenew = False
			end if
			namestr = ""
			If fromType ="CONTRACT" Then
				Set rs1 = conn.execute("select isnull(cateid,0) as cateid,title from contract where del=1 and ord="&fromid)
				If rs1.eof = False Then
					name = rs1("title")
					if open_5_1=3 or (open_5_1=1 And CheckPurview(intro_5_1,trim(rs1("cateid")))=True And rs1("cateid")<>"0") then
						if open_5_14=3 or (open_5_14=1 And CheckPurview(intro_5_14,trim(rs1("cateid")))=True And rs1("cateid")<>"0")  then
							namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord=" & pwurl(fromid) & "','contract','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
						else
							namestr = name
						end if
					end if
				else
					namestr = "<font color='red'>合同已删除</font>"
					isRenew = false
				end if
				rs1.close
				kuoutID = rs("kuoutID").value
				if isRenew and kuoutID&""<>"" and kuoutID&""<>"0" then
					if conn.execute("select 1 from kuout where ord=" & kuoutID & " and del=1").eof = true then isRenew = false
				end if
				SendID = rs("SendID").value
				if isRenew and SendID&""<>"" and SendID&""<>"0" then
					if conn.execute("select 1 from send where ord=" & SendID & " and del=1").eof = true then isRenew = false
				end if
			ElseIf fromType ="PREBACK" Then
				Set rs1 = conn.execute("select isnull(cateid,0) as cateid from bankin2 where del=1 and id="&fromid)
				If rs1.eof = False Then
					name = "预收款"
					if open_7_1=3 or (open_7_1=1 And CheckPurview(intro_7_1,trim(rs1("cateid")))=True And rs1("cateid")<>"0") then
						if open_7_14=3 or (open_7_14=1 And CheckPurview(intro_7_14,trim(rs1("cateid")))=True And rs1("cateid")<>"0")  then
							namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../money/contentyfk.asp?ord=" & pwurl(fromid) & "','pre','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
						else
							namestr = name
						end if
					else
					...
					namestr ="<font color='red'>预收款已删除</font>"
					isRenew =false
				end if
				rs1.close
			ElseIf fromType ="ContractTH" Then
				Set rs1 = conn.execute("select title, isnull(cateid,0) as cateid from contractth where del=1 and ord="&fromid)
				If rs1.eof = False Then
					name = rs1("title")
					if open_41_1=3 or (open_41_1=1 And CheckPurview(intro_41_1,trim(rs1("cateid")))=True And rs1("cateid")<>"0") then
						if open_7_14=3 or  (open_41_14=1 And CheckPurview(intro_41_14,trim(rs1("cateid")))=True And rs1("cateid")<>"0")  then
							namestr = "<a href=""javascript:;"" onclick=""javascript:window.open('../contractth/content.asp?ord=" & pwurl(fromid) & "','newwin','width=' + 1200 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">"& name &"</a>"
						else
							namestr = name
						end if
					else
					...
					namestr ="<font color='red'>退货已删除</font>"
					isRenew =false
				end if
				rs1.close
			else
				namestr ="<font color='name'>期初开票</font>"
			end if
			canDelete = true 'conn.execute("exec [erp_rela_finance_GetCollectionIDs] "& session("personzbintel2007") &", 1, '" & rs("id") &"',6, 6, '6005,6007' ").eof
			Response.write "" & vbcrlf & "                                     <td><div align=""left"">&nbsp;"
			Response.write namestr
			Response.write "</div></td>" & vbcrlf & "                                  <td><div align=""center""><font class=""name"">"
			Response.write rs("invoicecateName")
			Response.write "</font></div></td>" & vbcrlf & "                                   <td><div align=""center""><font class=""name"">"
			Response.write rs("delcateName")
			Response.write "</font></div></td>" & vbcrlf & "                                   <td><div align=""center""><span class=""name"">"
			Response.write rs("deldate")
			Response.write "</span></div></td> " & vbcrlf & "                                  <td><div align=""center"">" & vbcrlf & "                                          <input type=""button"" name=""Submit3c"" class=""anybutton"" value=""恢复"" "
			If isRenew=False Then
				Response.write "disabled "
			end if
			Response.write " onClick=""if(confirm('确认恢复？')){window.location.href='setkp.asp?ord="
			Response.write rs("id")
			Response.write "&CurrPage="
			Response.write CurrPage
			Response.write "'}""/>"
			if canDelete then
				Response.write "" & vbcrlf & "                                             <input   type=""button"" name=""Submitdel"" class=""anybutton"" value=""彻底删除"" onClick=""if(confirm('您选择的是彻底删除，删除后不能再恢复，确认删除？')){window.location.href='deletekp.asp?ord="
				Response.write rs("id")
				Response.write "&CurrPage="
				Response.write CurrPage
				Response.write "'}""/>"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='invoice.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='invoice.asp?currPage="
		...
		if currpage=rs.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='invoice.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='invoice.asp?currPage="
			Response.write  rs.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script type=""text/Javascript"">window.currask2Url=""setkpall.asp?currPage="
		Response.write currPage
