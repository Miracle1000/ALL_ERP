...
'if H1=1 then
	...
	if I2<>"" then
		if I1=1 Then
			str_Result=str_Result+" and send.ECID in (select ord from sortonehy where gate2=83 and del=1 and isStop=0 and sort1 like '%"& I2 &"%')"
'if I1=1 Then
		end if
	end if
	if J2<>"" then
...
'if zdy6<>"" then
	...
	if a<>"" then
		If propm="1" Then
			...
			Str_Result= Str_Result + " and ((send.complete1=0 and ("& open_33_16 &"=3 or ("& open_33_16 &"=1 and charindex(','+cast(send.cateid as varchar(10))+',',',"& intro_33_16 &",')>0))) or (send.complete1=1 and send.alt is null and send.cateid="&session("personzbintel2007")&"))"
			'If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
		else
	...
	rssort.close
	i=1
	conn.execute(" declare @cnt int "&vbcrlf&_
	" set @cnt = 1 "&vbcrlf&_
	" select a.id,a.id1 as pid,cast(a.menuname as varchar(8000)) as fullPath,cast(a.menuname as varchar(8000)) as fullPathWithoutSpace into #area "&vbcrlf&_
	" from menuarea a  with(nolock) "&vbcrlf&_
	" while exists(select 1 from #area where pid<>0) and @cnt < 100 "&vbcrlf&_
	" begin "&vbcrlf&_
	"  update #area set fullPath = b.menuname + ' ' + fullPath , pid=b.id1,fullPathWithoutSpace = b.menuname + fullPathWithoutSpace  from menuarea b where b.id=#area.pid "&vbcrlf&_
	...
	" left join #area bb on aa.areaId=bb.id "&Str_Result&" "&px_Result&" "
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	...
	if not rs.eof then
		do until rs.eof
			...
			xlWorksheet.Cells(1+i,1).Value = date7
			set rs7=nothing
			xlWorksheet.Cells(1+i,1).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).Value = ecname
			set rs7=nothing
			xlWorksheet.Cells(1+i,2).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).Value = WaybillNumber
			set rs7=nothing
			xlWorksheet.Cells(1+i,3).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).Value = Formatnumber(WayMoney,num_dot_xs,-1)
			set rs7=nothing
			xlWorksheet.Cells(1+i,4).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).Value = code
			set rs7=nothing
			xlWorksheet.Cells(1+i,5).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,6).Value = k
			set rs7=nothing
			xlWorksheet.Cells(1+i,6).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,7).Value = AllAddress
			set rs7=nothing
			xlWorksheet.Cells(1+i,7).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,8).Value = company
			set rs7=nothing
			xlWorksheet.Cells(1+i,8).font.Size=10
			set rs7=nothing
			xlWorksheet.Cells(1+i,9).Value = order1
			set rs7=nothing
