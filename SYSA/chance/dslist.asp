...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(byval rpt, byval  state)
		...
		rpt.addfield "添加时间 自：", "dates", "date3", "", "", Split(",", ",")
		dim keytype : keytype = "项目名称" & Chr(1) & "1"
		keytype = keytype & Chr(2) & "对手报价" & Chr(1) & "2"
		keytype = keytype & Chr(2) & "对手优势" & Chr(1) & "3"
		keytype = keytype & Chr(2) & "对手劣势" & Chr(1) & "4"
		keytype = keytype & Chr(2) & "备注" & Chr(1) & "5"
		rpt.addfield2 "","select", "keytype" , keytype, "", "", "对手名称=0"
		rpt.addfield "", "text", "keytxt", "", "", ""
		rpt.sql = "exec erp_chance_gldsList " & Info.User & ", @ItemsId,&excelmode,@keytype,@keytxt,@date3_0,@date3_1,0"
	...
	Function ChanceLinkHTML(chId, chtit, chadd)
		Dim GetChancePowerB, GetChancePower
		...
		If Len(GetChancePower) > 0 Then
			If InStr("," & GetChancePower & ",","," & chadd & ",") > 0 Then
				ChanceLinkHTML = "<a title='点击可查看此项目详情' href='javascript:void(0)' onclick=""javascript:window.open('content.asp?ord=" & app.base64.pwurl(chId) & "','newwin_xm','width=' + 1180 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & chtit & "</a>"
			else
	...
	Function jzdsLinkHTML(zsId, zstit, zsadd)
		Dim GetJzdsPowerB, GetJzdsPower
		...
		If zsId>0 Then
			If Len(GetJzdsPower) > 0 Then
				If InStr("," & GetJzdsPower & ",","," & zsadd & ",") > 0 Then
					JzdsLinkHTML = "<a href='javascript:void(0)' onclick=""javascript:window.open('../work/contentds.asp?ord=" & app.base64.pwurl(zsId) & "','newwin_xmds','width=' + 930 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">" & zstit & "</a>"
'If InStr("," & GetJzdsPower & ",","," & zsadd & ",") > 0 Then
				else
					JzdsLinkHTML = zstit
				end if
	...
	Function workButtonHTML(candel, canupdate, canxq, id)
		Dim html
		html = "<input type=""button"" value=""详情"" class=""oldbutton2"" onclick=""javascript:window.open('dsinfo.asp?__msgId=ShowInfo&id=" & id & "','newwin_xmds','width=' + 930 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;""/> "
'Dim html
		If canupdate = 1 Then
			html = html & "<input type=""button"" value=""修改"" class=""oldbutton2"" onclick=""javascript:window.open('dsinfo.asp?__msgId=Edit&id=" & id & "','newwin_xmds','width=' + 930 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;""/> "
'If canupdate = 1 Then
		end if
		If candel = 1 Then
			html = html & "<input type=""button"" value=""删除"" class=""oldbutton2"" onclick=""ListAction.DoDel('" & id & "')"" />"
		end if
		workButtonHTML = html
	end function
	Sub App_DoDel
