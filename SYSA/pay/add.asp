...
'If Not rs.Eof Then
	...
	dim rs,strateget
	strateget = 0
	set rs = conn.execute("select sort from strategy where gate2=1")
...
'Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script src=""fun.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "<script language=javascript>" & vbcrlf & "var strateget="
	Response.write strateget
	Response.write ";" & vbcrlf & "</script>" & vbcrlf & "<script src= ""../Script/py_add.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=javascript></script>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & ".as {width:60px;" & vbcrlf & "height:20px;}" & vbcrlf & "-->" & vbcrlf & ".panel-body.panel-body-noborder.window-body{height:150px!important}" & vbcrlf & "body{height:100px;}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body  onunload=""if(window.opener) window.opener.location.reload();"">" & vbcrlf & "<div style=""position:absolute; display:none;width: 500px;top:0px; left:0px;height:320px;border: 0px;padding: 1px;z-index: 100;"" id=""dhtml""></div>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
	dim MODULES
	MODULES=session("zbintel2010ms")
	dim ordbool,sorce2,bz,spmoney,title
	if request("paybxid")="" then
		conn.execute "delete from paybx where del=7 and addcate=" & session("personzbintel2007")
		conn.execute "update pay set bx=0 where ord in (select payid from paybxlist where del=7 and (addcate=" & session("personzbintel2007") & " or datediff(hh,indate,getdate())>=12))  and ord not in(select distinct a.payid from paybxlist a inner join pay b on a.payid=b.ord and a.del=1 and b.del=1)"
		conn.execute "delete from paybxlist where del=7 and (addcate=" & session("personzbintel2007") & " or datediff(hh,indate,getdate())>=12)"
		set rsbh = conn.execute("EXEC erp_getdjbh 80,"&session("personzbintel2007"))
		khid=rsbh(0).value
		rsbh.close
		set rsbh=nothing
		conn.execute "insert into paybx(complete,title,intro,bh,bz,indate,bxdate,addcate,sp_id,cateid_sp,del,ModifyStamp) values(0,null,null,'"&khid&"',null,getdate(),null,"&session("personzbintel2007")&",null,null,7,getdate())"
		paybxid=GetIdentity("paybx","id","addcate","")
	else
		paybxid=getInt(request("paybxid"))
	end if
	ord=getInt(request("ord"))
	if ord=0 then
		ordbool=false
	else
		ordbool=true
	end if
	if ordbool=true then
		sql="select * from f_pay where id="&ord
		set rs=server.CreateObject("adodb.recordset")
		...
		if rs.RecordCount<=0 then
			...
			bz=rs("bz")
			sql="select * from pay where complete=3 and del=1 and isnull(bx,0)<>1 and isnull(bx,0)<>3 and fid=" & ord &_
			" and ord not in (select payid from paybxlist where del=7 and addcate=" & session("personzbintel2007") & ") "&_
			" and ord not in (select distinct b.payid from paybx a inner join paybxlist b on a.id=b.bxid where a.del=1 and  a.complete<>2) "&_
			" order by ord asc"
			set rs=conn.execute(sql)
			while not rs.eof
				bxuid=rs("cateid")
				datesy=setname("f_pay","id",ord,"datejk")
				if datesy="" or isnull(datesy) then datesy=now
				intro=rs("intro")
				...
				"values("&paybxid&","&rs("ord")&",0,'"&Replace(intro,"'","’")&"',"&rs("sort")&",'"&datesy&"',7,"&session("personzbintel2007")&",getdate(),null,null)"
				conn.execute sql
				rs.movenext
			wend
		end if
	...
	Response.write "" & vbcrlf & "<form method=""POST"" action=""savepaybx.asp"" id=""demo"" onsubmit=""return false"" name=""date"">" & vbcrlf & "       <input type=""hidden"" name=""sptype"" id=""sptype"" value=""0"">" & vbcrlf & "       <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & " <tr>" & vbcrlf & "                    <td width=""100%"" valign=""top"">" & vbcrlf & "                              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">费用报销添加</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right""></td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "                                <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "                                 <tr class=""top"">" & vbcrlf & "                                          <td colspan=""2"">基本信息</td>" & vbcrlf & "                                             <td colspan=""2""><div align=""right""><input type=""button"" class=""anniu"" value=""保存"" onclick=""beforeSave();""></div></td>" & vbcrlf & "                                      </tr>" & vbcrlf & "                                 <tr>" & vbcrlf & "                                      <td width=""10%"" height=""37""><div align=""right"">报销主题：</div></td>" & vbcrlf & "                                                <td width=""30%"" height=""37"" ><div align=""left"">" & vbcrlf & "                                               <input name=""title""  type=""text"" id=""title"" datatype=""Limit"" min=""1"" max=""200""msg=""字数不能超过200字"">" & vbcrlf & "                                              <font id=""tit"" style=""color:red"">*</font>" & vbcrlf & "                                     </div></td>" & vbcrlf & "                                           <td width=""15%"" height=""37""><div align=""right"">报销编号：</div></td>" & vbcrlf & "                                          <td width=""35%"" height=""37"" ><div align=""left"">" & vbcrlf & "                                               <input name=""payid""  type=""text"" id=""payid"" value="""
	Response.write khid
	Response.write """ size=""30"" class='jquery-auto-bh' autobh-options='cfgId:80,recId:"
'Response.write khid
	Response.write paybxid
	Response.write ",autoCreate:false' datatype=""Limit"" min=""1"" max=""200"" msg=""字数不能超过200字"">" & vbcrlf & "                                                </div></td>" & vbcrlf & "                                     </tr> " & vbcrlf & "                                  <tr>" & vbcrlf & "                                      <td width=""10%"" height=""37""><div align=""right"">报销人员：</div></td>" & vbcrlf & "                                                <td width=""30%"" height=""37""><div align=""left""> "& vbcrlf & "                                                    <span id=""catenamebx"">"
	Response.write ry
	Response.write "</span>" & vbcrlf & "                                                       <font id=""ry""></font>" & vbcrlf & "                                                     <input type=""hidden"" name=""bxuid"" id=""bxuid"" value="""
	Response.write bxuid
	Response.write """>" & vbcrlf & "                                                        </div>" & vbcrlf & "                                    </td>" & vbcrlf & "                                   <td width=""15%"" height=""37""><div align=""right"">报销日期：</div></td>" & vbcrlf & "                                          <td width=""35%"" height=""37"" style=""padding-top:0!important;padding-bottom:0!important"">" & vbcrlf & "                                             <TABLE border=0 align=""left"" >" & vbcrlf & "                            <TBODY>" & vbcrlf & "                                                                 <TR>" & vbcrlf & "                    <TD>" & vbcrlf & "                            <INPUT name=ret onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" value="""
	Response.write date
	Response.write """ readonly size=10 >" & vbcrlf & "                                                                                      <input type=""button"" class=""anybutton"" style=""margin-bottom:-0.5px;"" value=""选择日期"" align=""absMiddle""  border=""0""  id=""daysOfMonthPos"" name=""daysOfMonthPos""  onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" />" & vbcrlf & "                 <span class=""red"">*</span>" & vbcrlf & "                                                                                        <DIV id=daysOfMonth style=""POSITION: absolute""></DIV>" & vbcrlf & "                                                                       </TD>" & vbcrlf & "                  </TR>" & vbcrlf & "                </TBODY>" & vbcrlf & "              </TABLE>" & vbcrlf & "                                      </td>" & vbcrlf & "                 </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                              <td width=""10%"" height=""37""><div align=""right"">报销分类：</div></td>" & vbcrlf & "                                        <td width=""30%"" height=""37""><div align=""left"">" & vbcrlf & "                                                <select name=""bxtype"" id=""bxtype"" datatype=""Limit"" min=""1"" msg=""请选择报销分类"">" & vbcrlf & "                                                   <option value="""">选择报销分类</option>" & vbcrlf & "                                                    "
	Dim sql_bxtype, rs_bxtype
	sql_bxtype = "select * from sortonehy where gate2='10001' order by gate1 desc"
	...
	Response.write "" & vbcrlf & "                                             </select>" & vbcrlf & "                                               <span class=""red"">*</span>" & vbcrlf & "                                        </div>" & vbcrlf & "                                  </td>" & vbcrlf & "                                           <td width=""15%"" height=""37""><div align=""right"">总张数：</div></td>" & vbcrlf & "                                            <td height=""37""><div align=""left""><span id=""sumnum""></span></div></td>" & vbcrlf & "                              </tr>" & vbcrlf & "                                 <tr>" & vbcrlf & "                                      <td width=""10%"" height=""37""><div align=""right"">币种：</div></td>" & vbcrlf & "                                            <td width=""30%"" height=""37""><div align=""left"">" & vbcrlf & ""
	if ordbool=true then
		bzname=setname("sortbz","id",bz,"sort1")
		if bzname="" or isnull(bzname) then
			bzname = "人民币"
		end if
	end if
	Response.write "" & vbcrlf & "                                                             <span id=""bzname"">"
	Response.write bzname
	Response.write "</span>" & vbcrlf & "                                                              <input type=""hidden"" name=""bz"" id=""bz"" value="""
	Response.write bz
	Response.write """>" & vbcrlf & "                                                        </div>" & vbcrlf & "                            </td>" & vbcrlf & "                                         <td width=""15%"" height=""37""><div align=""right"">总金额：</div></td>" & vbcrlf & "                                            <td height=""37""><div align=""left"">" & vbcrlf & ""
	if ordbool=true then
		money1=getrsval("select sum(money1) as money1 from pay where del=1 and fid="&ord)
		if money1="" or isnull(money1) then money1=0
	end if
	Response.write "" & vbcrlf & "                                                     <span id=""summoney"">"
	Response.write Formatnumber(money1,num_dot_xs,-1)
'Response.write "" & vbcrlf & "                                                     <span id=""summoney"">"
	Response.write "</span> </div>" & vbcrlf & "                                                       <input type=""hidden"" name=""bxmoney"" id=""bxmoney"" value="""
	Response.write money1
	Response.write """>" & vbcrlf & "                                                </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                      <td width=""10%"" height=""27""><div align=""right"">费用明细：</div></td>" & vbcrlf & "                                                <td height=""27"" colspan=""5"" class=""red""><IFRAME name=""I3"" SRC=""../event/bxmx.asp?paybxid="
	Response.write pwurl(cstr(paybxid))
	Response.write """ id=""mxlist"" width=""100%"" height=""60px"" onload=""frameResize()"" scrolling=no BORDER=0 marginheight=0 marginwidth=0 frameborder=0 target=""_self"" ></IFRAME></td>" & vbcrlf & "                                     </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                      <td width=""10%""><div align=""right"">报销概要：</div></td>  " & vbcrlf & "                                                <td  colspan=""5""><textarea name=""note"" style=""display:none"" cols=""1"" rows=""1"">"
	if note<>"" then Response.write note
	Response.write "</textarea>" & vbcrlf & "                                          <IFRAME ID=""eWebEditor1"" SRC=""../edit/ewebeditor.asp?id=note&style=news"" FRAMEBORDER=""0"" SCROLLING=""no"" WIDTH=""100%"" HEIGHT=""300"" marginwidth=""1"" marginheight=""1"" name=""wfasdg""></IFRAME></td>              " & vbcrlf & "                                     </tr>" & vbcrlf & "                                   <tr>" & vbcrlf & "                                               <td height=""50"" colspan=""6"">" & vbcrlf & "                                                        <div align=""center""><input type=""button"" class=""anniu"" value=""保存"" onclick=""beforeSave()"">" & vbcrlf & "                                                               <input type=""hidden"" name=""paybxid"" value="""
	Response.write paybxid
	Response.write """>" & vbcrlf & "                                                                <input name=""Submit4"" type=""reset"" class=""anniu"" value=""重置"">" & vbcrlf & "                                                  </div>" & vbcrlf & "                                          </td>" & vbcrlf & "                                   </tr> " & vbcrlf & "                  </table>" & vbcrlf & "                        </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "              <td class=""page"">" & vbcrlf & "                            <table width=""100%"" border=""0"" height=""100"" align=""left"" >" & vbcrlf & "                                      <tr>" & vbcrlf & "                                            <td>" & vbcrlf & "                                            </td>" & vbcrlf & "                                   </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & ""
	action1="费用报销添加"
	call close_list(1)
