...
'bigsystemtype = ".mozi"
	...
	dim open_72_1,intro_72_1,kh_list
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_1=0
		intro_72_1=0
	else
		open_72_1=rs1("qx_open")
		intro_72_1=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=14"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_14=0
		intro_72_14=0
	else
		open_72_14=rs1("qx_open")
		intro_72_14=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=2"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_2=0
		intro_72_2=0
	else
		open_72_2=rs1("qx_open")
		intro_72_2=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_3=0
		intro_72_3=0
	else
		open_72_3=rs1("qx_open")
		intro_72_3=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=6"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_6=0
		intro_72_6=0
	else
		open_72_6=rs1("qx_open")
		intro_72_6=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_7=0
		intro_72_7=0
	else
		open_72_7=rs1("qx_open")
		intro_72_7=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=72 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_72_8=0
		intro_72_8=0
	else
		open_72_8=rs1("qx_open")
		intro_72_8=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	if open_72_1=3 then
		list=""
	elseif open_72_1=1 then
		list="and cateid in ("&intro_72_1&")"
	else
		list="and cateid=0"
	end if
	dim rs,sql,Str_Result,Str_Result2
	Str_Result="where ord>0 "&list&""
	Str_Result2="and ord>0 "&list&""
	Str_power=""
	Str_power22=""
	Str_power33=""
	sorce=0
	...
	if open_72_1="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_72_1&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'Private Sub Class_Initialize
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & "td.top_search{border-bottom:0;background:#f4fafe;padding:0px;} " & vbcrlf & "#content td{padding:8px 4px ;} #content td.name table tr td{padding:0 ;}" & vbcrlf & ".IE5 .page .page," & vbcrlf & ".IE5 .page input.anybutton2{height:16px;line-height:14px;margin-bottom:-0.5px;}" & vbcrlf & ".IE5 .top_search input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/lz_event.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "" & vbcrlf & "<body  "
	if open_72_8=0 then
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & ""
	Str_Result=Str_Result & " and del=1"
	dim sort,result,sort2,page_count
	...
	elseif sort<>"3333333" and sort<>"5555555" then
		Str_Result=Str_Result + "and sort2="&request("sort")&" and del=1"
	elseif sort<>"3333333" and sort<>"5555555" then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select sort1 from sortonehy where ord="&sort&""
		rs1.open sql1,conn,1,1
	...
	select case stype
	case 0
	Str_Result = Str_Result & " and cateid in (select ord from gate where name like '%" & replace(skey,"'","''") & "%')"
	case 1
	Str_Result = Str_Result & " and title like '%" & replace(skey,"'","''") & "%'"
	case 2
	Str_Result = Str_Result & " and intro like '%" & replace(skey,"'","''") & "%'"
	end Select
	m1 = request("ret")
	m2 = request("ret2")
	If m1 <> "" Then Str_Result = Str_Result & " and DATEDIFF(D,'" & m1 & "',date7) >= 0 "
	If m2 <> "" Then Str_Result = Str_Result & " and DATEDIFF(D,'" & m2 & "',date7) <= 0 "
	Dim helper
	Set helper = CreateReminderHelper(conn,2,0)
	dim newstate
	...
	if sort="3333333" or sort="5555555" or newstate="1" then
		new_list="<span class='alt_tx'></span>"
		timelg=session("timezbintel2007")
		if sort="3333333" and timelg<>"" then
			noalt1=" and date7>'"&timelg&"'"
		end if
		set rs9=server.CreateObject("adodb.recordset")
		if newstate="1" then
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where ord=2 and cateid="&session("personzbintel2007")&""
		else
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where ord=2 and cateid="&session("personzbintel2007")&""
		end if
		...
		if rs9.eof then
			...
			if rs9("fw1")="0" then
				if open_72_1=3 then
					Str_Result_alt=""
				elseif open_72_1=1 then
					Str_Result_alt="and cateid in ("&intro_72_1&")"
				else
					Str_Result_alt="and cateid=0"
				end if
			else
				Str_Result_alt="and cateid="&session("personzbintel2007")&""
			end if
			tq1=rs9("tq1")
		end if
		rs9.close
	...
	Response.write "" & vbcrlf & "      <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td class=""place""><b>"
	if sort="3333333" or sort="5555555" Then
		Response.write("公告提醒")
	else
		if sort<>"" then
			Response.write("公司公告>>><font class='red tableTitleLinks'>" & sort2 & "</font>")
		else
			Response.write("所有公告列表")
		end if
	end if
	Response.write "</b></td>" & vbcrlf & "        <td><div style=""width:200px;left:286px;top:13px;padding-top:20px""><a href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></div>&nbsp;</td>" & vbcrlf &"        <td align=""right""><div id=""kh"" style='height:22px;line-height:22px;'>&nbsp;"
	Response.write("所有公告列表")
	if open_72_7=1 or open_72_7=3 then
		Response.write "<input type=""button"" name=""Submit43"" value=""打印""  onClick=""javascript:kh.style.display='none';window.print();return  false;""   class=""anybutton""/>"
	end if
	Response.write "</div></td>" & vbcrlf & "        <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "    </table> " & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "         <form action="""" method=""get""　id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "          <TR height=""50"">" & vbcrlf & "                <td class=""top_search resetHeadBg""  height=""50"" valign=""middle"" align=""right"" >"& vbcrlf & "                      添加时间"
	...
	Response.write "" & vbcrlf & "                     <select name=""sort"">" & vbcrlf & "                              <option value="""">公告分类</option>" & vbcrlf & "                                "
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from sortonehy where gate2=91 order by gate1 desc"
	...
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "                     </select>" & vbcrlf & "                       <select name=""stype"">" & vbcrlf & "                             <option value=""0"" "
	if cstr(stype)="0" then
		Response.write "selected"
	end if
	Response.write ">发布人</option>" & vbcrlf & "                             <option value=""1"" "
	if cstr(stype)="1" then
		Response.write "selected"
	end if
	Response.write ">标题</option>" & vbcrlf & "                               <option value=""2"" "
	if cstr(stype)="2" then
		Response.write "selected"
	end if
	Response.write ">内容</option>" & vbcrlf & "                       </select>" & vbcrlf & "                       <input value="""
	Response.write skey
	Response.write """ type=""text"" name=""skey"" />" & vbcrlf & "                  <input value="""
	Response.write px
	Response.write """ type=""hidden"" name=""px"" />" & vbcrlf & "                  <input value="""
	Response.write newstate
	Response.write """ type=""hidden"" name=""newstate"" />" & vbcrlf & "                    <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "         </td>" & vbcrlf & "     </tr>" & vbcrlf & "   </form>" & vbcrlf & "       </table>" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr class=""top resetGroupTableBg"">" & vbcrlf & "         <td height=""27"" width=""5%"" ><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "      <td width=""30%""><div align=""center""><strong>标题</strong></div></td>" & vbcrlf & "       <td width=""15%""><div align=""center""><strong>分类</strong></div></td>" & vbcrlf & "        <td width=""15%""><div align=""center""><strong>发布人</strong></div></td>" & vbcrlf & "      <td width=""15%""><div align=""center"" ><strong>发布时间</strong></div></td>" & vbcrlf & "   <td width=""20%""><div align=""center"" >" & vbcrlf & "         <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid #FFFFFF;overflow:hidden"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "                  <option value=""page_count=10"" "
	Response.write newstate
	...
	dim n
	n=0
	sql="   select * from learntz  "&Str_Result&"  order by date7 desc,ord DESC "
	CALL InitTempTable("learntz",sql,"ord","cateid",72)
	set rs=server.CreateObject("adodb.recordset")
	sql="select count(1) from learntz " & Str_Result
	rs.open sql,conn,1,1
	RCount = CLng(rs(0))
	rs.close
	set rs = nothing
	...
	If RCount Mod PageSize = 0 Then
		PageCount = CLng(RCount/PageSize)
	else
		PageCount = (RCount - (RCount Mod PageSize)) / PageSize + 1
		PageCount = CLng(RCount/PageSize)
	end if
	CurrPage = Request("CurrPage")
	if CurrPage="" Or CurrPage="0" Or isnumeric(CurrPage)=False Then CurrPage=1
	...
	BookNum=RCount
	rMin = (CurrPage - 1) * PageSize
	BookNum=RCount
	rMax = CurrPage * PageSize
	If rMin < 0 Then
		rMin = 0
	end if
	pxStr = " tz.date7 desc "
	...
	End Select
	set rs=server.CreateObject("adodb.recordset")
	sql =       " select top " & PageSize & " tz.ord,tz.cateid,tz.title,s.sort1 as sort3,tz.date7,g.name as cateidname " &_
	" from " &_
	" (select * from learntz " & Str_Result & ") tz " &_
	" left join sortonehy s on s.ord = tz.sort2 " &_
	" left join gate g on g.ord = tz.cateid " &_
	" where tz.ord not in ( " &_
	"  select top " & rMin & " tz.ord " &_
	"  from" &_
	"  (select * from learntz " & Str_Result & ") tz " &_
	"  left join sortonehy s on s.ord = tz.sort2 " &_
	"  left join gate g on g.ord = tz.cateid " &_
	"  order by " & pxStr & " ,tz.ord desc " &_
	" ) " &_
	" order by " & pxStr & " ,tz.ord desc "
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			k=rs("title")
			cateid=rs("cateid")
			if  sort="3333333" and timelg<>"" and datediff("s",timelg,rs("date7"))>0 then
				k=k&new_list
			end if
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "      <td align=""center"" height=""27"">"
			Response.write RCount - rMin - n
			Response.write "</td>" & vbcrlf & "      <td class=""name"">"
			if open_72_14=3 or (open_72_14=1 and CheckPurview(intro_72_14,trim(cateid))=True) then
				Response.write "<a href=""javascript:;"" onclick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','learncon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title="""
				Response.write pwurl(rs("ord"))
				Response.write rs("title")
				Response.write """>"
			end if
			Response.write HtmlArea(k)
			Response.write "</a></td>" & vbcrlf & ""
			sort3=rs("sort3")
			Response.write "" & vbcrlf & "     <td class=""name""><div align=""center"">"
			Response.write sort3
			Response.write "</div></td>" & vbcrlf & ""
			if rs("cateid")<>"" and not isnull(rs("cateid")) then
				cateid=rs("cateid")
			...
			if open_72_2=3 or (open_72_2=1 and CheckPurview(intro_72_2,trim(cateid))=True)  then
				Response.write " <input type=""button"" name=""Submit3"" value=""修改""  onClick=""javascript:window.open('correct.asp?id="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>"
				'Response.write pwurl(rs("ord"))
			end if
			if open_72_3=3 or (open_72_3=1 and CheckPurview(intro_72_3,trim(cateid))=True) then
				Response.write " <input type=""button"" name=""Submit"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete.asp?"
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "      <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='edit.asp?"
			Response.write sdk.ReturnURL("currPage=" & 1)
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='edit.asp?"
			Response.write sdk.ReturnURL("currPage=" & currpage - 1)
		...
		if currpage=pagecount then
			...
			Response.write "" & vbcrlf & "     <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='edit.asp?"
			Response.write sdk.ReturnURL("currPage=" & currpage + 1)
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='edit.asp?"
			Response.write sdk.ReturnURL("currPage=" & PageCount)
			Response.write "'"" class=""page""/>" & vbcrlf & "      "
		end if
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & " " & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "   var a = document.getElementsByTagName(""input"");" & vbcrlf & "   if(a[0].checked==true){" & vbcrlf & "   for (var i=0; i<a.length; i++)" & vbcrlf & "      if (a[i].type == ""checkbox"") a[i].checked = false;" & vbcrlf & "   }" & vbcrlf & "   else" & vbcrlf & "   {" & vbcrlf & "   for (var i=0; i<a.length; i++)" &vbcrlf & "      if (a[i].type == ""checkbox"") a[i].checked = true;" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	action1="公司公告"
	call close_list(1)
	Response.write "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "" & vbcrlf & "<div id=""User"" style=""position:absolute;height:100;display:none;padding-top:20px;"">" & vbcrlf & "    <table width=""150"" id = ""pxtab""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "    <tr>" & vbcrlf & "                <td height=""139"">" & vbcrlf & "         <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "                  <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按发布时间排序(降)</font></a></td></tr>" & vbcrlf & "                    <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按发布时间排序(升)</font></a></td></tr>" & vbcrlf & "                  <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按标题排序(降)</font></a></td></tr>" & vbcrlf & "                       <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按标题排序(升)</font></a></td></tr>" & vbcrlf & "                      "
	...
	Response.write "" & vbcrlf & "                     <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=7');""><font color=""#2F496E"">按发布人排序(降)</font></a></td></tr>" & vbcrlf & "                    <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=8');""><font color=""#2F496E"">按发布人排序(升)</font></a></td></tr>" & vbcrlf & "         </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "</html>"
