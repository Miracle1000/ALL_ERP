...
'n1 = rs("num1").value  & ""  :  If Len(n1) = 0 Then  n1 = 0
	...
	Sub onReportInit(rpt, lt)
		...
		rpt.PageSize = 10
		rpt.sql = "exec erp_report_zzhz "& Info.User &", '"& topUrl &"', &pagesize, &pageindex,@date1_0, @date1_1, @searchType, @searckKey,'','',&headerconfig"
		rpt.BillListUIModel = True
		rpt.showtoppagesize = True
		rpt.addgroup "呆滞库存TOP10", "exec erp_report_zzhz "& Info.User &", '"& topUrl &"', 10000000,1,@date1_0, @date1_1, @searchType, @searckKey,'','BAD5',''", "count",  "row",  "产品名称"
		rpt.addgroup "周转最快TOP10", "exec erp_report_zzhz "& Info.User &", '"& topUrl &"', 10000000,1,@date1_0, @date1_1, @searchType, @searckKey,'','TOP5',''", "count",  "row",  "产品名称"
	end sub
	Sub app_oncreate(lvw)
		...
		next
		lvw.headers(lenf+8).display = "none"
		col.width = 90
		For i = 0 To len_rszdy
			If rs_zdy(0,i) = 0 Then
				lvw.headers(16+i).display = "none"
'If rs_zdy(0,i) = 0 Then
			else
				lvw.headers(16+i).title = rs_zdy(1,i)
'If rs_zdy(0,i) = 0 Then
			end if
		next
	end sub
