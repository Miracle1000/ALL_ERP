...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Dim yhord, addcate, cateid_sp
	Dim trTopbtn
	Sub Bill_OnPageInit
		app.docmodel = "IE=8"
		Response.write "" & vbcrlf & "       <style>" & vbcrlf & " .sub-field{color:#585858;}" & vbcrlf & "      table.approval td{border:1px solid #CCC;}" & vbcrlf & "       table#editbody>tbody>tr.top.resetTableBg{       /*库存-产品养护-详情-审批记录*/" & vbcrlf & "         position:relative;      " & vbcrlf & "        }" & vbcrlf & "       table#editbody>tbody>tr.top.resetTableBg>td.fcell>div.group-title{" & vbcrlf & "             margin:60px 0 0 32px!important;" & vbcrlf & " }" & vbcrlf & "       .edit-body .sub-thead td.fcell:after{" & vbcrlf & "           left:32px!important;" & vbcrlf & "    }" & vbcrlf & "       /*库存-产品养护-详情-基本信息*/" & vbcrlf & " .sub-thead[dbname='divBody2'] td{" & vbcrlf & "               height:80px!important;" & vbcrlf & "          padding:0 0 0 32px!important;" & vbcrlf & "   }" & vbcrlf & "       .sub-thead[dbname='divBody2'] td:before{" & vbcrlf & "                border-top:0!important" & vbcrlf & "  }" & vbcrlf & "       .sub-thead[dbname='divBody2'] td:after{" & vbcrlf & "         top:60px!important;" & vbcrlf & "     }" & vbcrlf & "    </style>" & vbcrlf & "        <script>" & vbcrlf & "        function frameResize(){" & vbcrlf & "         document.getElementById(""mxlist"").style.height=(I3.document.getElementById(""mxPos"").offsetTop+28)+""px"";" & vbcrlf & "       }" & vbcrlf & "" & vbcrlf & "       window.onload=function(){" & vbcrlf & "               var grayImg = document.getElementsByTagName(""img"");" & vbcrlf & "              if(grayImg.length>0){" & vbcrlf & "                   for(i=0; i<grayImg.length; i++){" & vbcrlf & "                                if(grayImg[i].width>550){" & vbcrlf & "                                       grayImg[i].width = 550;" & vbcrlf & "                         }" & vbcrlf & "                               if(grayImg[i].height>150){" & vbcrlf & "                                      grayImg[i].height = 150;" & vbcrlf & "                         }" & vbcrlf & "                       }" & vbcrlf & "               } " & vbcrlf & "              " & vbcrlf & "        }" & vbcrlf & "       </script>" & vbcrlf & "       <script type=""text/JavaScript"" src=""../skin/default/js/mn_planall.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """></script>" & vbcrlf & "        <body>" & vbcrlf & "  "
	end sub
	Sub Bill_OnInit(ByVal Bill, ByVal initType)
		...
		Dim mButtons,rs, rs1, sql, sort , typeTitle ,secTitle ,f
		Dim title, bh, date1, reason, status, intro, zdy1, zdy2, zdy3, zdy4, zdy5, zdy6 ,statusname ,addname ,spname
		Dim sqlStr
		yhord = app.base64.deurl(request("ord"))
		If yhord&""<>"" Then
			Set rs = cn.execute("select a.*,(case isnull(a.status,0) when 0 then '审批通过' when 1 then '待审批' when 2  then '审批中' when 4 then '审批未通过' end) statusname,(case when isnull(a.status,0) in (1,2) then g1.name else '' end ) as spname,g.name addname from maintain a left join gate g on a.addcate = g.ord left join gate g1 on g1.ord= a.cateid_sp  where a.ord="& yhord)
			If rs.eof = True Then
				bill.showSaveResultEx "该养护单已不存在",true,"self",""
			ElseIf rs.eof = False Then
				title = rs("title") : bh = rs("bh") : date1 = rs("date1") : reason = rs("reason") : status = rs("status")  : intro = rs("intro")
				cateid_sp = rs("cateid_sp") : addcate = rs("addcate")
				zdy1 = rs("zdy1") : zdy2 = rs("zdy2") : zdy3 = rs("zdy3") : zdy4 = rs("zdy4") : zdy5 = rs("zdy5") : zdy6 = rs("zdy6")
				statusname = rs("statusname") : addname = rs("addname") : spname = rs("spname")
			end if
			rs.close
			set rs = nothing
		end if
		trTopbtn = checkTopbtn()
		bill.title = "养护详情"
		If trTopbtn = True Then
			bill.groups(0).title = ""
			bill.groups(0).showbar = False
			Bill.fields.addHtml "", "Ahtml", 3, 1
			Bill.AddCurrGroup "基本信息", "divBody2"
		end if
		...
		f.value = date1
		f.edit = False
		Set f = Bill.fields.add("审批人员", "spname")
		f.value = spname
		f.edit = False
		Set f = Bill.fields.add("审批状态", "statusname")
		f.value = statusname
		f.edit = False
		Set f = Bill.fields.add("添加人员", "addname")
		f.value = addname
		f.edit = False
		Dim fValue , n
		Set rs = cn.execute("select id,title,name,sort from zdy where sort1=9001 and set_open=1 order by gate1")
		While rs.eof = False
			Set f = Bill.fields.add(rs("title"), rs("name"))
			f.edit = False
			Select Case rs("name")
			Case "zdy1" : fValue = zdy1
			Case "zdy2" : fValue = zdy2
			Case "zdy3" : fValue = zdy3
			Case "zdy4" : fValue = zdy4
			Case "zdy5" , "zdy6"
			If rs("name") = "zdy5" Then
				sql = "select sort1 from sortonehy where ord="& zdy5 &""
			ElseIf rs("name") = "zdy6" Then
				sql = "select sort1 from sortonehy where ord="& zdy6 &""
			end if
			...
			rs.movenext
			If rs.eof Then
				If n /3 >0 Then f.colspan = 3 - n/3 +1
'If rs.eof Then
			end if
		wend
		rs.close
		...
		If intro&""<>"" Then
			Dim arr_img
			arr_img = split(intro,"<img",-1,1)
'Dim arr_img
			if ubound(arr_img)>0 Then
				intro = "<a onMouseOver=""window.status='none';return true;"" title=""放大查看"">"& intro &"</a>"
			end if
		end if
		...
		f.colspan = 3
		f.edit = False
		with Bill.AddCurrGroup("产品明细", "divBody3")
		.fields.addHtml "", "Bhtml", 3, 1
		End With
		with Bill.AddCurrGroup("", "divBody3")
		.visible = False
		.fields.addHtml "", "Chtml", 3, 1
		End With
	end sub
	Function checkTopbtn()
		Dim intro_79_2, intro_79_3, intro_79_7, intro_79_10, intro_79_16, tempStr
		tempStr = False
		...
		intro_79_10 = app.power.GetPowerIntro(79,10)
		If intro_79_10="" or instr(","& intro_79_10 &"," , ","& addcate &",")>0 then
			tempStr = True
			checkTopbtn = tempStr
			Exit Function
		end if
		intro_79_7 = app.power.GetPowerIntro(79,7)
		If intro_79_7="" or instr(","& intro_79_7 &"," , ","& addcate &",")>0 then
			tempStr = True
			checkTopbtn = tempStr
			Exit Function
		end if
	...
	Sub Ahtml(ByVal field)
		Dim intro_79_2, intro_79_3, intro_79_7, intro_79_10, intro_79_16, tempStr
		Response.write "<div id='dydb' align='right' style='margin-top:0;'>"
'Dim intro_79_2, intro_79_3, intro_79_7, intro_79_10, intro_79_16, tempStr
		Response.write "<script>$('#dydb').parent().attr('class','');</script>"
		intro_79_2 = app.power.GetPowerIntro(79,2)
		intro_79_3 = app.power.GetPowerIntro(79,3)
		intro_79_7 = app.power.GetPowerIntro(79,7)
		intro_79_10 = app.power.GetPowerIntro(79,10)
		...
		if (intro_79_16="" or instr(","& intro_79_16 &"," , ","& addcate &",")>0) And cateid_sp&"" = Info.User&"" then
			...
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:window.open('../inc/CommSPSet.asp?ord=" & app.base64.pwurl(yhord) & "&sort1=91&lvw=mlistvw','new6win','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">审批</button>"
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
		end if
		If intro_79_2="" or instr(","& intro_79_2 &"," , ","& addcate &",")>0 then
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:window.open('correct.asp?ord=" & app.base64.pwurl(yhord) & "','new2win','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">修改</button>"
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
		end if
		If intro_79_3="" or instr(","& intro_79_3 &"," , ","& addcate &",")>0 then
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:delSLOrders("& yhord &",'content')"">删除</button>"
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
		end if
		If intro_79_10="" or instr(","& intro_79_10 &"," , ","& addcate &",")>0 then
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:window.open('../out/word_Out1.asp?ord="& app.base64.pwurl(yhord)&"&sort=91&main=1&isSum=1','newwinword','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"">导出Word</button>"
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:window.open('../out/excel_Out1.asp?ord="& app.base64.pwurl(yhord)&"&sort=91&main=1','newwinexcel','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=152,top=152')"">导出Excel</button>"
		end if
		If intro_79_7="" or instr(","& intro_79_7 &"," , ","& addcate &",")>0 then
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
			tempStr = tempStr & "<button class=""zb-button""  onclick=""$('#dydb').parent().parent().hide();window.print();$('#dydb').parent().parent().show();"">打印</button>"
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
			tempStr = tempStr & "<button class=""zb-button"" onclick=""javascript:window.open('../contract/moban_dy.asp?ord="& app.base64.pwurl(yhord)&"&sort=91&main=1','newwin45','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=150,top=150')"">模板打印</button>"
			if tempStr&""<>"" then tempStr = tempStr & "&nbsp;"
		end if
		if tempStr&""<>"" then
			Response.write "<div class='resetHeadBg' style='padding:17px;' >" & tempStr & "</div>"
		end if
		Response.write "</div>"
	end sub
	Sub BHtml(ByVal field)
		Response.write "<div id=""mxdiv"" class='sub-field' style='position:relative'><IFRAME name=""I3"" id=""mxlist"" SRC=""?__msgID=yhmxFrame&ord="& request("ord") &""" FRAMEBORDER=""0"" SCROLLING=""auto"" width=""100%"" HEIGHT=""63"" marginwidth=""1"" marginheight=""1"" style=""margin:0; padding:0;"" onload=""frameResize()""></IFRAME></div>"
	end sub
	Sub CHtml(ByVal field)
		Response.write "<div class='sub-field'>"
'Sub CHtml(ByVal field)
		Call show_sp_intro(91,yhord,"")
		Response.write "</div>"
	end sub
	Sub App_yhmxFrame
		app.docmodel = "IE=8"
...
'If app.isIE7 Or app.isIE6 Then
		...
		Response.write "<style> #lvw_mxistvw table td a:hover{ text-decoration:underline} #lvw_mxistvw{border-bottom:none;border-right:none; _border-top:none;} #lvw_pagebar_mxistvw {display:none;} #lvw_tablebg_mxistvw{_border-bottom:none;_border-top:none;}</style>"
'If app.isIE7 Or app.isIE6 Then
		Response.write "" & vbcrlf & "<script>" & vbcrlf & "function lockKuCP(idx,status){" & vbcrlf & "       if(idx!=""""){" & vbcrlf & "              var lockStr = ""冻结"";" & vbcrlf & "             if(status==0){" & vbcrlf & "                  lockStr = ""冻结""" & vbcrlf & "          }else if(status==1){" & vbcrlf & "                    lockStr = ""解冻""" & vbcrlf & "          }" & vbcrlf & "if(confirm(""确定要""+lockStr+""吗？"")){ "& vbcrlf &  "                      ajax.regEvent("""",""../store/lockKuCP.asp""); "& vbcrlf &"                   ajax.addParam(""idx"", idx); "& vbcrlf &   "                      ajax.addParam(""status"", status); "& vbcrlf &   "                var r = ajax.send(); "& vbcrlf &   "                  if(r!=""""){ "& vbcrlf &      "                           var arr_res = r.split(""""|"""");" & vbcrlf & "                             if(arr_res[0]==""1""){" & vbcrlf & "                                      if (arr_res[1]==""1""){" & vbcrlf & "                                             $ID(""Lock_""+idx).innerHTML = ""<input type='button' class='oldbutton2' value='解冻' onClick='lockKuCP(""+idx+"",""+arr_res[1]+"")'>""" & vbcrlf & "                                 }else if(arr_res[1]==""0""){" & vbcrlf & "                                            $ID(""Lock_""+idx).innerHTML = ""<input type='button' class='oldbutton2' value='冻结' onClick='lockKuCP(""+idx+"",""+arr_res[1]+"")'>""" & vbcrlf & "                                 }" & vbcrlf & "                               }" & vbcrlf & "                       }" & vbcrlf & "" & vbcrlf & "               }" & vbcrlf & "       }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf &" "
...
'Call showMxItems
	...
	sub showMxItems
		dim mxMaxNum, num1_dot, num_dot_xs, rs, sql
		yhord = app.base64.deurl(request("ord"))
		If yhord&""="" Then yhord = 0
		addcate = 0
		Set rs = cn.execute("select addcate from maintain where ord="& yhord)
		If rs.eof = False Then
			addcate = rs("addcate")
		end if
		rs.close
		...
		"dbo.formatNumber(a.num1,"& num1_dot &",0) num1, k.ph, case when isnull(sr2.id,0)=0 then k.xlh else nl.serinum end xlh,k.datesc, k.dateyx, a.share, " &_
		...
		"isnull(a.ord,0) cpord,p.addcate cpAddcate, isnull(k.locked,0) kuLocked, ck.intro ckIntro, k.id 操作 " &_
		"from maintainlist a  " &_
		"left join product p on a.ord=p.ord and p.del=1 " &_
		"left join ku k on a.ku=k.id and a.ord=k.ord  " &_
		"left join sortck ck on k.ck=ck.ord " &_
		"left join kuinlist n on k.kuinlist=n.id and n.del=1 " &_
		" left join S2_SerialNumberRelation sr2 on sr2.Billtype = 61001 and sr2.billid=n.kuin and sr2.Listid = n.id and sr2.serialID= a.serialID "&_
		...
		"where a.del=1 and a.maintain="& yhord &" order by a.id"
		dim lvw : set lvw = new listview
		lvw.id = "mxistvw"
		lvw.IsAbsWidth = True
		lvw.IsaccWidth = True
		...
		lvw.AllSum = true
		lvw.toolbar = False
		lvw.headers.getItemByDbname("title").setLink "title","cpord", "cpAddcate",  21, -6
		lvw.toolbar = False
		lvw.headers.getItemByDbname("title").cansum = False
		...
		lvw.headers.getItemByDbname("kuLocked").display = "none"
		lvw.headers.getItemByDbname("ckIntro").display = "none"
		lvw.headers.getItemByDbname("操作").formattext = "code:getColHtml(@value,""操作"",rs(""ckIntro""),rs(""kuLocked""))"
		lvw.headers.getItemByDbname("操作").width = 60
		lvw.headers.getItemByDbname("操作").cansum = False
		If Not app.power.existsPower(23,21) Then
			lvw.headers.getItemByDbname("操作").display = "none"
		end if
		lvw.addlink = ""
		dim html : html = lvw.HTML
		Response.write "<div style='position:relative;width:" & (lvw.datawidth + 1 + Abs(app.isIE7 Or app.isIE6)*50) & "px'>" & html & "</div>"
'dim html : html = lvw.HTML
	end sub
	Sub App_lvw_onSumProc(lvw, db,  mtype, html)
		Dim tempStr, intro_79_2, sumkd, sumkd2, rs
		If db = "@label" Then
			tempStr = ""
...
'If sumkd<=730 Then
			...
			intro_79_2 = app.power.GetPowerIntro(79,2)
			If intro_79_2="" or instr(","& intro_79_2 &"," , ","& addcate &",")>0 then
				tempStr = "&nbsp;<img src='../images/jiantou.gif'><a href='javascript:;' onClick=""window.open('../maintain/topadd.asp?top="& app.base64.pwurl(yhord) &"&act=edit&f=101','editslmx8','width=' + "&(sumkd2+306)&"+ ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"" >编辑明细</a>"
			end if
			html = "<div style='height:23px; padding-top:5px;word-wrap:break-word; word-break:normal;'>合计"& tempStr &"</div>"
');"" >编辑明细</a>"
		else
			html = "<span style='word-wrap:break-word; word-break:normal;'>" & html & "</span>"
');"" >编辑明细</a>"
		end if
	end sub
	Sub App_lvw_onnullData(lvw)
		Dim tempStr, sumkd, sumkd2, intro_79_2
		sumkd = lvw.datawidth - 260
'Dim tempStr, sumkd, sumkd2, intro_79_2
		If sumkd<=730 Then
			sumkd2 = sumkd + 180
'If sumkd<=730 Then
		...
		tempStr = "<div class='nulltr' style='margin-left:500px; height:28px; line-height:28px;'>暂无记录！"
		sumkd2 = 800
		intro_79_2 = app.power.GetPowerIntro(79,2)
		If intro_79_2="" or instr(","& intro_79_2 &"," , ","& addcate &",")>0 then
			tempStr = tempStr &"&nbsp;<img src='../images/jiantou.gif'><a href='javascript:;' onClick=""window.open('../maintain/topadd.asp?top="& app.base64.pwurl(yhord) &"&act=edit&f=101','editslmx8','width=' + "&(sumkd2+306)&"+ ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"" style='color:#5b7cae'>添加产品明细</a>"
		end if
		tempStr = tempStr &"</div>"
		lvw.addhtml tempStr
	end sub
	Function getColHtml(mxid,colName,colValue,revID)
		...
		Case "操作"
		tempStr = ""
		Dim kuid, kuLocked, ckIntro, lockStr
		kuid = mxid : ckIntro = colValue : kuLocked = revID
		If ckIntro = "0" Or InStr(","& ckIntro &",",","& Info.User &",")>0 Then
			lockStr = "冻结"
			If kuLocked = 1 Then lockStr = "解冻"
			tempStr = "<div style='text-align:center' id=""Lock_"& kuid &"""><button class=""oldbutton2""  onclick=""lockKuCP("& kuid &","& kuLocked &")"">"& lockStr &"</button></div>"
'If kuLocked = 1 Then lockStr = "解冻"
		end if
		End Select
		getColHtml = tempStr
