...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(4).ColumnWidth=7
			...
			xlWorksheet.Cells(1,1).Value = "签订日期"
			...
			xlWorksheet.Cells(1,3).Value = "合同主题"
...
'if m1&""="" and m2&""="" then
			...
			if dateType="1" then
				if m1<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date7, 120)>=CONVERT(varchar(10), '"&m1&"', 120) "
'if dateType="1" then
				if m2<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date7, 120)<=CONVERT(varchar(10), '"&m2&"', 120) "
'if dateType="1" then
			else
				if m1<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date3, 120)>=CONVERT(varchar(10),'"&m1&"' , 120)"
'if dateType="1" then
				if m2<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date3, 120)<=CONVERT(varchar(10),'"&m2&"', 120) "
'if dateType="1" then
			end if
			if m3<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date7, 120)>=CONVERT(varchar(10),'"&m3&"', 120) "
'if dateType="1" then
			if m4<>"" then Str_Result=Str_Result+"and CONVERT(varchar(10), c.date7, 120)<=CONVERT(varchar(10),'"&m4&"', 120) "
'if dateType="1" then
...
'if hkjz<>"" and hkjz<>"10" then
			...
			"   c.money1-isnull(dkh.thMoney3,0) as money1,isnull(p.allmoney,0) as allmoney,isnull(th.thmoney,0) as thmoney,isnull(p.ysmoney,0) as ysmoney "&_
			...
			" where isnull(c.importPayback,0)<>1 and c.del=1 AND ISNULL(c.[status],-1) IN (-1,1) and isnull(c.isTerminated,0)<>1 and c.money1>isnull(p.allmoney,0)+ isnull(th.thmoney,0) "& Str_Result & px_Result
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "待建立应收账款合同列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0  then
				...
				do until rs.eof
					...
					sort3 = rs("sort3").value
					date3 = rs("date3").value
					leftmoney = cdbl(money1) - cdbl(allmoney)
					date3 = rs("date3").value
					companyname = ""
					If sort3="2" Then
						if open_26_1=3 or (open_26_1=1 and CheckPurview(intro_26_1,trim(cateid_kh))=True And cateid_kh<>0 ) Then companyname = rs("companyname")
					else
						if open_1_1=3 or (open_1_1=1 and CheckPurview(intro_1_1,trim(cateid_kh))=True And cateid_kh<>0 ) Or InStr(1,","&khshare&",", ","&session("personzbintel2007")&",",1) > 0 Or khshare = "1" Then companyname = rs("companyname")
					end if
					str = ""
					if allmoney=0 Then
						str="未生成"
					elseif allmoney<money1 Then
						str="部分生成"
					elseif allmoney>money1 Then
					end if
...
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,2).Value = companyname
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,2).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,2).HorizontalAlignment=1
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,3).Value = contracttitle
'elseif allmoney>money1 Then
...
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,5).Value = Formatnumber(money1,num_dot_xs,-1,0,-1)
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,5).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,6).Value = Formatnumber(allmoney,num_dot_xs,-1,0,-1)
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,6).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,7).Value = Formatnumber(ysmoney,num_dot_xs,-1,0,-1)
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,7).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,8).Value = Formatnumber(leftmoney,num_dot_xs,-1,0,-1)
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,8).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,9).Value = str
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,9).font.Size=10
'elseif allmoney>money1 Then
					xlWorksheet.Cells(1+i,10).Value = cateidname
'elseif allmoney>money1 Then
...
'elseif allmoney>money1 Then
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>待建立应收账款合同列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!待建立应收账款合同列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("待建立应收账款合同列表_"&session("name2006chen")&".xls")
			...
			action1="待建立应收账款合同列表导出"
			call close_list(1)
			Response.write " " & vbcrlf & "" & vbcrlf & "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的待建立应收账款合同列表</u></strong></font></a></p> " & vbcrlf & ""
