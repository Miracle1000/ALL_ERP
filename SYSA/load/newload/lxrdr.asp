...
'If InStr(1,","&mustcon&",",",97,",1)>0 Then
		...
		Sub GetDefFieldName(ByRef arrShow, ByRef arrName,ByRef arrRequired)
			Set rs=cn.execute("select * from (select (case when isnull(name,'')='' then oldname else name end ) as name, (case when show>0 then 1 else 0 end) as show,required,gate1 from setfields union all select title,1,1,gate1+70 from zdy where sort1=1 and set_open=1 and dr=1 and bt=1 ) a order by gate1 asc ")
			ReDim arrShow(0)
			ReDim arrName(0)
			ReDim arrRequired(0)
			While Not rs.eof
		...
		sub Page_loadConfig(uploadatt)
			...
			.fileName = "联系人资料"
			.filters = "xls|xlsx"
			.smpFilePath = replace("../../in/example_lxr.asp","_0","")
			.helpFilePath = "../../in/caption_lxr.doc"
		...
		Sub Page_OnCreateTempTable(uploader)
			...
			If arrShow(13)=1 Then
				strcols = strcols & "|" & arrName(13)& "=zip=0=tel"
				strcols = strcols & "|" & arrName(13)& "=zip=0=person"
			end if
			strcols = strcols & "|家庭地址=address=0=person|跟进方式=sort98=1=reply|洽谈进展=intro=0=reply|销售人员=cateid=1=tel|添加时间=date1=0=tel|up_index=tel_excel_drSign=0=tel" & CHK_Fields
			uploader.baseCols = strcols
		...
		sub Page_LinkFieldHand(tmp , ywname , dbname , gl , UserDefCol, dbtable)
			...
			case ""&arrName(5)&"" : cn.execute "update " & tmp & " set ["&arrName(5)&"] = cast(a.ord as varchar(10)) from sort5 a where a.sort2=["&arrName(5)&"] and ["&arrName(4)&"]=a.sort1 and isnumeric(["&arrName(4)&"])=1 "
		...
		function Page_InsertDataBase(dbname , uploader)
			...
			For i = 0 To kxrs.fields.count - 1
				...
				If cn.execute("select count(1) from "&dbname&" where isnumeric(cast(["&fname&"] as varchar(4000)))=1 and charindex('e+',cast(["&fname&"] as varchar(4000)))>0")(0)>0 Then
					fname = kxrs.fields(i).name
			...
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'空数据' from " & dbname & " where  datalength(isnull(rtrim(["&arrName(1)&"]),'')) = 0 and datalength(isnull(["&arrName(17)&"],''))=0"
			cn.execute "delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) = 0 and datalength(isnull(["&arrName(17)&"],''))=0" , n
			cn.execute "insert into #k_all  (内容)  values ('从导入文件删除" & n & "条空数据')"
			uploader.ShowProc "写入客户资料：删除无效扩展客户自定义数据                进度：" , 100
			set rs_kz_zdy=server.CreateObject("adodb.recordset")
			...
			do while not rs_kz_zdy.eof
				...
				Ins_Kong_Sql="insert into #k_fail(行号,失败原因) select up_index,'客户扩展自定义字段【"&kz_fname&"】为空,必填' from " & dbname & " " _
				& " where  datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and datalength(isnull(rtrim(["&kz_fname&"]),''))=0 "
				Del_Kong_Sql="delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and datalength(isnull(rtrim(["&kz_fname&"]),''))=0 "
				If kz_Ftype=1 or kz_Ftype=2 then
					if kz_mustfillin=true then
						cn.execute(Ins_Kong_Sql)
						cn.execute(Del_Kong_Sql)
					end if
				ElseIf kz_Ftype=3 then
					if kz_mustfillin=true then
						cn.execute("insert into #k_fail(行号,失败原因) select up_index,'客户扩展自定义字段【"&kz_fname&"】不是日期,需日期类型' from " & dbname & " where  datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and ISDATE(["&kz_fname&"])=0")
						cn.execute("delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and (["&kz_fname&"]='' or ISDATE(["&kz_fname&"])=0)")
						cn.execute(Ins_Kong_Sql)
						cn.execute(Del_Kong_Sql)
					else
						cn.execute("update " & dbname & " set ["&kz_fname&"]='' where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and (["&kz_fname&"]='' or ISDATE(["&kz_fname&"])=0)")
					end if
				ElseIf kz_Ftype=4 then
					if kz_mustfillin=true Then
						Ins_Kong_Sql="insert into #k_fail(行号,失败原因) select up_index,'客户扩展自定义字段【"&kz_fname&"】不是数字,需数字类型' from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and ISNUMERIC(["&kz_fname&"])=0"
						Del_Kong_Sql="delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and (["&kz_fname&"]='' or ISNUMERIC(["&kz_fname&"])=0)"
						cn.execute(Ins_Kong_Sql)
						cn.execute(Del_Kong_Sql)
					else
						cn.execute("update " & dbname & " set ["&kz_fname&"]=0 where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and (["&kz_fname&"]='' or ISNUMERIC(["&kz_fname&"])=0)")
					end if
				ElseIf kz_Ftype=6 then
					Ins_Sql="insert into #k_fail(行号,失败原因) select up_index,'客户扩展自定义字段【"&kz_fname&"】,必需为是或否' from " & dbname & "  where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and ["&kz_fname&"] not in ('是','否') "
					uploader.ShowProc Ins_Sql&"：" , 160
					Del_Sql="delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and ["&kz_fname&"] not in ('是','否')"
					cn.execute(Ins_Sql)
					cn.execute(Del_Sql)
				ElseIf kz_Ftype=7 then'必须必填
					Ins_Sql="insert into #k_fail(行号,失败原因) select up_index,'客户扩展自定义字段【"&kz_fname&"】,必需是自定义字段里的一项' from " & dbname & "  where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and not exists (select * from erp_customOptions where cvalue=["&kz_fname&"]) and ["&kz_fname&"]<>'' "
					Del_Sql="delete from " & dbname & " where datalength(isnull(rtrim(["&arrName(1)&"]),'')) > 0 and not exists (select * from erp_customOptions where cvalue=["&kz_fname&"]) and ["&kz_fname&"]<>'' "
					cn.execute(Ins_Sql)
					cn.execute(Del_Sql)
				end if
				rs_kz_zdy.movenext
			...
			For countR=0 To ubound(arrRequired)
				If arrShow(countR)>0 And arrRequired(countR)>0 And countR<>11 And countR<>18 And countR<>25 Then
					If countR>=17 And countR<=32 Then
						cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(countR)&" 是必填字段,不能为空,无法导入。' from " & dbname & " where datalength(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 "
						cn.execute "delete from " & dbname & " where datalength(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 ", i  :  n = n + i
						cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & i & "条"&arrName(countR)&" 为空数据,请参见字段策略设置')"
					else
						cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(countR)&" 是必填字段,不能为空,无法导入。' from " & dbname & " where datalength(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 and datalength(isnull(["&arrName(1)&"],'')) > 0"
						cn.execute "delete from " & dbname & " where datalength(isnull(cast(["&arrName(countR)&"] as varchar(500)),''))=0 and datalength(isnull(["&arrName(1)&"],'')) > 0", i  :  n = n + i
						cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & i & "条"&arrName(countR)&" 为空数据,请参见字段策略设置')"
					end if
				end if
			next
			uploader.ShowProc "写入客户资料：分离非主联系人资料                进度：" , 100
			sql =       "select ["&arrName(17)&"],"
			...
			If arrShow(19)=1 Then sql = sql & "[" & arrName(19) &"],"
			If arrShow(21)=1 Then sql = sql & "[" & arrName(21) &"],"
			If arrShow(20)=1 Then sql = sql & "[" & arrName(20) &"],"
			...
			If arrShow(13)=1 Then sql = sql & "[" & arrName(13) &"],"
			sql =sql & "家庭地址,销售人员,添加时间,bb.kfID  as 客户ID ,aa.up_index as 联系人ID,bb.跟进方式,bb.洽谈进展 into  #lxrtb from " & dbname & " aa ,( " & vbcrlf & _
			"select  a.up_index as lxrID , max(b.kfID) as kfID,a.跟进方式,cast(a.洽谈进展 as varchar(8000)) 洽谈进展 from " & dbname & " a , (select up_index as kfID from " & dbname & " b where datalength(isnull(["&arrName(1)&"],'')) > 0 )  b  " & vbcrlf & _
			" where datalength(isnull(["&arrName(1)&"],'')) = 0 and a.up_index > b.kfID " & vbcrlf & _
			"group by a.up_index,a.跟进方式,cast(a.洽谈进展 as varchar(8000))  " & vbcrlf & _
			") bb where aa.up_index = bb.lxrID and datalength(isnull(["&arrName(17)&"],'')) > 0"
			cn.execute sql , n
			cn.execute "update  #lxrtb set 销售人员 = cast(a.ord as varchar(10)) from gate a where a.name = 销售人员"
			cn.execute "delete from " & dbname & " where datalength(isnull(["&arrName(1)&"],''))=0"
			cn.execute "select ["&arrName(1)&"],up_index as kid into #kftb from " & dbname
			cn.execute "insert into #k_all  (内容)  values ('从导入文件获取" & n & "条非主联系人资料')"
			uploader.ShowProc "写入客户资料：去重复记录                进度：" , 200
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(1)&"与第' + cast(isnull((select min(up_index) as id from " & dbname & " b where isnull(a.["&arrName(1)&"],'')=isnull(b.["&arrName(1)&"],'')),0) as varchar(10)) + '行重复' from " & dbname & " a where  up_index not in (select min(up_index) as id from " & dbname & " group by ["&arrName(1)&"])"
			cn.execute "delete " & dbname & " where up_index not in (select min(up_index) as id from " & dbname & " group by ["&arrName(1)&"])" , n1
			If arrShow(20)=1 Then
				cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(20)&"与第' + cast(isnull((select min(up_index) as id from " & dbname & " b where datalength(isnull(["&arrName(20)&"],''))>0 and isnull(a.["&arrName(17)&"],'')=isnull(b.["&arrName(17)&"],'') and isnull(a.["&arrName(20)&"],'')=isnull(b.["&arrName(20)&"],'')),0) as varchar(10)) + '行重复' from " & dbname & " a  where  up_index not in (select min(up_index) as id from " & dbname & " where datalength(isnull(["&arrName(20)&"],''))>0  group by ["&arrName(17)&"],["&arrName(20)&"]) and datalength(isnull(["&arrName(20)&"],''))>0"
'If arrShow(20)=1 Then
				cn.execute "delete " & dbname & " where up_index not in (select min(up_index) as id from " & dbname & " where datalength(isnull(["&arrName(20)&"],''))>0  group by ["&arrName(17)&"],["&arrName(20)&"]) and datalength(isnull(["&arrName(20)&"],''))>0" , n2
				cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(17)&"已经存在' from " & dbname & " where  up_index in (select b.up_index from person a , " & dbname & " b where datalength(isnull(a.mobile,'')) > 0 and b.["&arrName(17)&"]=a.name and b.["&arrName(20)&"]=a.mobile) and datalength(isnull(["&arrName(20)&"],''))>0"
				cn.execute "delete " & dbname & " where up_index in (select b.up_index from person a , " & dbname & " b where datalength(isnull(a.mobile,'')) > 0 and b.["&arrName(17)&"]=a.name and b.["&arrName(20)&"]=a.mobile) and datalength(isnull(["&arrName(20)&"],''))>0" , n3
				cn.execute "insert into #k_all (内容,说明) values ('从导入文件删除" & n1 & "条客户重复记录," & n2 & "条【"&arrName(20)&"】与【"&arrName(17)&"】重复记录," & n3 & "条【"&arrName(20)&"】与【"&arrName(17)&"】已存在的记录','当"&arrName(1)&"重复算重复')"
			end if
			uploader.ShowProc "写入客户资料：删除无效关联空数据                进度：" ,300
			sql= "select ["&arrName(4)&"] as 无效数据, '"&arrName(4)&"' as 无效数据列, count(["&arrName(4)&"]) as 存在记录数量,'未导入' as 导入结果 from " & dbname & " kl where isnumeric(isnull(["&arrName(4)&"],'0'))=0 group by ["&arrName(4)&"]" & vbcrlf
			sql=sql & "union all" & vbcrlf
			sql=sql & "select ["&arrName(5)&"] , '"&arrName(5)&"' , count(["&arrName(5)&"]),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(5)&"],'0'))=0   group  by  ["&arrName(5)&"]" & vbcrlf
			sql=sql & "union all" & vbcrlf
			sql=sql & "select ["&arrName(7)&"] , '"&arrName(7)&"' , count(["&arrName(7)&"]),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(7)&"],'0'))=0   group  by  ["&arrName(7)&"]" & vbcrlf
			sql=sql & "union all" & vbcrlf
			sql=sql & "select ["&arrName(8)&"] , '"&arrName(8)&"' , count(["&arrName(8)&"]),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(8)&"],'0'))=0   group  by  ["&arrName(8)&"]" & vbcrlf
			sql=sql & "union all" & vbcrlf
			sql=sql & "select ["&arrName(9)&"] , '"&arrName(9)&"' , count(["&arrName(9)&"] ),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(9)&"] ,'0'))=0   group  by  ["&arrName(9)&"] " & vbcrlf
			If ZBRuntime.MC(207101) Then
				sql=sql & "union all" & vbcrlf
				sql=sql & "select ["&arrName(52)&"] , '"&arrName(52)&"' , count(["&arrName(52)&"] ),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(52)&"] ,'0'))=0   group  by  ["&arrName(52)&"] " & vbcrlf
			end if
			sql=sql & "union all" & vbcrlf
			sql=sql & "select ["&arrName(6)&"] , '"&arrName(6)&"' , count(["&arrName(6)&"]),'未导入' from " & dbname & " kl where isnumeric(isnull(["&arrName(6)&"],'0'))=0   group  by  ["&arrName(6)&"]" & vbcrlf
			sql=sql & "union all" & vbcrlf
			sql=sql & "select 销售人员 , '销售人员' , count(销售人员),'未导入' from " & dbname & " kl where isnumeric(isnull(销售人员,'0'))=0   group  by  销售人员" & vbcrlf
			Set rs_kz_zdy = cn.execute("select title,gl from zdy where sort1=1 and set_open=1 and dr=1 and bt=1 order by gate1")
			While rs_kz_zdy.eof = False
				kz_fname=rs_kz_zdy("title")
				sql=sql & "union all" & vbcrlf
				sql=sql & "select ["& kz_fname &"], '"& kz_fname &"' , count(["& kz_fname &"]) ,'未导入' from " & dbname & " kl where (isnumeric(isnull(["& kz_fname &"],'0'))=0 or (isnumeric(isnull(["& kz_fname &"],'0'))=1 and not exists(select top 1 1 from sortonehy where gate2="& rs_kz_zdy("gl") &" and ord=(case when isnumeric(isnull(["& kz_fname &"],'0'))=0 then 0 else ["&kz_fname&"] end)))) group by ["& kz_fname &"]" & vbcrlf
				rs_kz_zdy.movenext
			wend
			rs_kz_zdy.close
			Set rs_kz_zdy = Nothing
			...
			n = 0
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(4)&"无效,不识别“' + cast(["&arrName(4)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(4)&"],'0'))=0"
			n = 0
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(4)&"],'0'))=0", i  :  n = n + i
			n = 0
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(5)&"无效,不识别“' + cast(["&arrName(5)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(5)&"],'0'))=0"
			n = 0
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(5)&"],'0'))=0", i  :  n = n + i
			n = 0
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(7)&"无效,不识别“' + cast(["&arrName(7)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(7)&"] ,'0'))=0"
			n = 0
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(7)&"],'0'))=0", i  :  n = n + i
			n = 0
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(8)&"无效,不识别“' + cast(["&arrName(8)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(8)&"],'0'))=0"
			n = 0
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(8)&"],'0'))=0", i  :  n = n + i
			n = 0
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(9)&"无效,不识别“' + cast(["&arrName(9)&"]  as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(9)&"] ,'0'))=0"
			n = 0
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(9)&"] ,'0'))=0", i  :  n = n + i
			n = 0
			If ZBRuntime.MC(207101) Then
				cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(52)&"无效,不识别“' + cast(["&arrName(52)&"]  as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(52)&"] ,'0'))=0"
'If ZBRuntime.MC(207101) Then
				cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(52)&"] ,'0'))=0", i  :  n = n + i
'If ZBRuntime.MC(207101) Then
			end if
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(6)&"无效,不识别“' + cast(["&arrName(6)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(6)&"],'0'))=0"
'If ZBRuntime.MC(207101) Then
			cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(6)&"],'0'))=0", i  :  n = n + i
'If ZBRuntime.MC(207101) Then
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'销售人员无效,不识别“' + cast(销售人员 as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(销售人员,'0'))=0"
'If ZBRuntime.MC(207101) Then
			cn.execute "delete from " & dbname & " where isnumeric(isnull(销售人员,'0'))=0", i  :  n = n + i
'If ZBRuntime.MC(207101) Then
			If arrShow(29)=1 Then
				cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"&arrName(29)&"无效,不识别“' + cast(["&arrName(29)&"] as varchar(255)) + '”。' from " & dbname & " where isnumeric(isnull(["&arrName(29)&"],'0'))=0"
'If arrShow(29)=1 Then
				cn.execute "delete from " & dbname & " where isnumeric(isnull(["&arrName(29)&"],'0'))=0", i  :  n = n + i
'If arrShow(29)=1 Then
			end if
			Set rs_kz_zdy = cn.execute("select title,gl from zdy where sort1=1 and set_open=1 and dr=1 and bt=1 order by gate1")
			While rs_kz_zdy.eof = False
				kz_fname=rs_kz_zdy("title")
				cn.execute "insert into #k_fail(行号,失败原因) select up_index,'"& kz_fname &"无效,不识别第'+cast(up_index as varchar(10))+ '行"& kz_fname &"。' from " & dbname & " where (isnumeric(isnull(["& kz_fname &"],'0'))=0 or (isnumeric(isnull(["& kz_fname &"],'0'))=1 and not exists(select top 1 1 from sortonehy where gate2="& rs_kz_zdy("gl") &" and ord=(case when isnumeric(isnull(["& kz_fname &"],'0'))=0 then 0 else ["&kz_fname&"] end))))"
				cn.execute "delete from " & dbname & " where (isnumeric(isnull(["& kz_fname &"],'0'))=0 or (isnumeric(isnull(["& kz_fname &"],'0'))=1 and not exists(select top 1 1 from sortonehy where gate2="& rs_kz_zdy("gl") &" and ord=(case when isnumeric(isnull(["& kz_fname &"],'0'))=0 then 0 else ["&kz_fname&"] end)))), i  :  n = n + i"
				rs_kz_zdy.movenext
			wend
			rs_kz_zdy.close
			Set rs_kz_zdy = Nothing
			cn.execute "insert into #k_all  (内容) values ('从导入文件删除" & n & "条无效关联空数据,请参见无效关联数据附表')"
			...
			If arrShow(13)=1 Then sql=sql & "zip,"
			sql=sql & "sort,sort1,area,trade,address,del,cateid,company,numc1,date4,date8)" & vbcrlf
			sql=sql & "select distinct getdate(),"
			sql=sql & "cast(["&arrName(17)&"] as varchar(40)),"
			If arrShow(30)=1 Then sql=sql & "["&arrName(30)&"],"
			If arrShow(27)=1 Then sql=sql & "["&arrName(27)&"],"
			If arrShow(31)=1 Then sql=sql & "["&arrName(31)&"],(case when datalength(["& arrName(31) &"])>0 then datediff(YY,isnull(["& arrName(31) &"],'1900-1-1'),getdate()) end),"
			If arrShow(27)=1 Then sql=sql & "["&arrName(27)&"],"
			If arrShow(31)=1 Then
				sql=sql & "(case when datalength(["& arrName(31) &"])>0 then 1 else 0 end),"
			else
				sql=sql & "0,"
			end if
			Dim datestr : datestr = ZBRuntime.SDK.VBL.Format(date, "yyyy-MM-dd")
			sql=sql & "0,"
			If arrShow(28)=1 Then sql=sql & "["&arrName(28)&"],"
			If arrShow(29)=1 Then sql=sql & "["&arrName(29)&"],"
			If arrShow(19)=1 Then sql=sql & "["&arrName(19)&"],"
			...
			sql=sql & "["&arrName(5)&"],"
			sql=sql & "["&arrName(7)&"],"
			sql=sql & "isnull(["&arrName(8)&"],0),"
			sql=sql & "家庭地址,1," & app.info.user & " ,-up_index,1,'" & datestr & "',getdate() from " & dbname & " where datalength(rtrim(["&arrName(17)&"]))>0 and up_index not in (" & vbcrlf
			sql=sql & "isnull(["&arrName(8)&"],0),"
			sql=sql & " select up_index from " & dbname & " a ,  person  b where b.del=1 and   isnull(a.["&arrName(17)&"],'')=isnull(b.name,'') "
			If arrShow(20)=1 Then sql=sql & " and  datalength(isnull(a.["&arrName(20)&"],''))>=0 and isnull(a.["&arrName(20)&"],'')=isnull(b.mobile,'') " & vbcrlf
			sql=sql & ")" & vbcrlf
			cn.execute sql , n
			cn.execute "insert into #k_all  (内容) values ('写入" & n & "位新主联系人资料。')"
			uploader.ShowProc "写入客户资料：转换主联系人关联          进度：" ,500
			sql =       "update " & dbname & " set ["&arrName(17)&"]=n1 from ( " & vbcrlf & _
			"select a.ord as n1,b.up_index as n2 from person a," & dbname & " b where a.name=b.["&arrName(17)&"] "
			If arrShow(20)=1 Then sql=sql & " and  isnull(b.["&arrName(20)&"],'')=isnull(a.mobile,'') "
			If arrShow(19)=1 Then sql=sql & " and isnull(b.["&arrName(19)&"],'') = isnull(a.phone,'') "
			If arrShow(22)=1 Then sql=sql & " and isnull(b.["&arrName(22)&"],'')=isnull(a.email,'') "
			sql=sql & " and a.ord > " & maxord2 & " " & vbcrlf
			If arrShow(23)=1 Then sql=sql & " and isnull(b.["&arrName(23)&"],'')=isnull(a.QQ,'')" & vbcrlf
			sql=sql & ") t  where up_index = n2"
			cn.execute sql , n
			cn.execute "insert into #k_all  (内容) values ('共转换" & n & "条主联系人关联数据')"
			cn.execute "select  b.up_index , a.ord  into #hasTel  from tel a, " & dbname & " b where isnull(a.name,'') = isnull(b.["&arrName(1)&"],'')"
			uploader.ShowProc "写入客户资料：写入主联系人洽谈进展              进度：" ,550
			cn.execute "insert into reply (ord,ord2,sort1,gj1,gj2,name,sort98,intro,cateid,date7,cateid2,cateid3,del)" & vbcrlf & _
			"select a.ord,a.ord,1,9,a.ord,(select name from gate aa  where aa.ord=a.cateid),b.跟进方式,b.洽谈进展,a.cateid,getdate(),-1,0,1 from tel a, " & dbname & " b where a.[name]=b.["&arrName(1)&"] and datalength(isnull(b.洽谈进展,''))>0 and isnull(b.跟进方式,0)>0 "
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'跟进方式为空,洽谈进展无法导入' from " & dbname & " where datalength(isnull(洽谈进展,''))>0 and isnull(跟进方式,0)=0 "
			n = 0
			uploader.ShowProc "写入客户资料：删除数据库中已存在的客户资料              进度：" ,600
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'数据库中已经存在该客户' from " & dbname & " where up_index in (" & vbcrlf & _
			"select  b.up_index as n from tel a, " & dbname & " b where a.del=1 and a.name = b.["&arrName(1)&"]" & vbcrlf & _
			")"
			sql= "delete from " & dbname & " where up_index in (" & vbcrlf
			sql=sql & "select  b.up_index as n from tel a, " & dbname & " b where a.name = b.["&arrName(1)&"]" & vbcrlf
			sql=sql & ")"
			cn.execute sql , n
			cn.execute "insert into #k_all (内容) values ('从导入文件删除" & n & "条已经存在于数据库中的客户资料')"
			uploader.ShowProc "写入客户资料：数据类型检测匹配处理              进度：" ,700
			n = uploader.autoHandFieldType(dbname)
			cn.execute "insert into #k_all (内容,说明) values ('共自动处理" & n & "处数据类型不匹配状况','文本超长自动截断,数字不正确设置默认为0，日期不正确默认为当前日期')"
			...
			cn.execute uploader.getInsertTableSql(dbname,"tel") , n
			cn.execute "update tel set tel.phone=a.phone , tel.fax = a.fax , tel.email = a.email from person a where a.ord = tel.person"
			cn.execute "insert into #k_all (内容) values ('成功导入" & n & "条客户资料记录')"
			cn.execute "update tel set del = 1 where del is null and ord > " & maxord
			cn.execute "update tel set cateadd = " & app.info.user & " where cateadd is null and ord >" & maxord
			cn.execute "update tel set cateid = " & app.info.user & " where (cateid is null or cateid = 0 ) and ord >" & maxord
			cn.execute "update tel set date2 = getdate() where ord >" & maxord
			cn.execute "update tel set date1 = '" & now & "' where datepart(hour,date1) =0 and datediff(d,date1,'" & datestr & "')=0 and ord > " & maxord
			cn.execute "update tel set order1=2, cateid4 = " & app.info.user & " where ord > " & maxord
			cn.execute "update tel set cateid2 = sorce , cateid3 = sorce2  from gate a where tel.cateid=a.ord and a.ord>" & maxord
			cn.execute "update person set company= a.ord, cateid2=a.cateid2 , cateid3 = a.cateid3 from tel a where a.person=person.ord and person.ord >" &  maxord2
			cn.execute "update person set company=a.ord from #hasTel a where -person.company = a.up_index and person.ord >" & maxord2
			cn.execute "update tel set tel.person =  a.ord from person a where a.ord > " & maxord & " and a.company = tel.ord and isnull(tel.person,0) not in (select ord from person) "
			cn.execute("insert into [tel_sales_change_log](tord,sort3,sort,sort1,precateid,newcateid,cateid,date2,date7,reason,reasonchildren,replynum,intro) select ord,sort3,sort,sort1,0,cateid,"& session("personzbintel2007") &",getdate(),getdate(),2,202,0,'联系人导入' from tel where ord > " & maxord  & " and ord < " & t_ord)
			uploader.ShowProc "写入客户资料：写入主联系人洽谈进展              进度：" ,870
			cn.execute "insert into reply (ord,ord2,sort1,gj1,gj2,name,sort98,intro,cateid,date7,cateid2,cateid3,del)" & vbcrlf & _
			"select a.ord,a.ord,1,9,a.ord,(select name from gate aa  where aa.ord=a.cateid),b.跟进方式,b.洽谈进展,a.cateid,getdate(),-1,0,1 from tel a, " & dbname & " b where a.[name]=b.["&arrName(1)&"] and datalength(isnull(b.洽谈进展,''))>0  and isnull(b.跟进方式,0)>0 "
			cn.execute "insert into #k_fail(行号,失败原因) select up_index,'跟进方式为空,洽谈进展无法导入' from " & dbname & " where datalength(isnull(洽谈进展,''))>0  and isnull(跟进方式,0)=0 "
...
'select sorce as bm, sorce2 as xz , ord  from gate & vbcrlf & _
			If arrShow(20)=1 Then
				cn.execute "insert into #k_fail(行号,失败原因) select 联系人ID,'"&arrName(17)&"与第' + cast(isnull((select min(联系人ID) from #lxrtb b where isnull(b.["&arrName(20)&"],'')=isnull(a.["&arrName(20)&"],'')),0) as varchar(10)) +'行重复' from #lxrtb a where datalength(isnull(["&arrName(20)&"],''))>0 and 联系人ID not in (select min(联系人ID) from #lxrtb  group by ["&arrName(20)&"])"
				cn.execute "delete from #lxrtb where datalength(isnull(["&arrName(20)&"],''))>0 and 联系人ID not in (select min(联系人ID) from #lxrtb group by ["&arrName(20)&"])" , n1
				cn.execute "insert into #k_fail(行号,失败原因) select 联系人ID,'"&arrName(17)&"与第' + cast(isnull((select min(联系人ID) from #lxrtb b where datalength(isnull(["&arrName(20)&"],''))=0 and isnull(a.["&arrName(17)&"],'')=isnull(b.["&arrName(17)&"],'') and isnull(a.客户ID,0)=isnull(b.客户ID,0)),0) as varchar(10)) +'行重复' from #lxrtb a where datalength(isnull(["&arrName(20)&"],''))=0 and 联系人ID not in (select min(联系人ID) from #lxrtb where datalength(isnull(["&arrName(20)&"],''))=0 group by ["&arrName(17)&"],客户ID)"
				cn.execute "delete from #lxrtb where datalength(isnull(["&arrName(20)&"],''))=0 and 联系人ID not in (select min(联系人ID) from #lxrtb where datalength(isnull(["&arrName(20)&"],''))=0 group by ["&arrName(17)&"],客户ID)", n2
				cn.execute "insert into #k_all (内容) values ('从导入文件删除" & (n1+n2) & "条重复的非主联系人记录')"
				cn.execute "insert into #k_fail(行号,失败原因) select 联系人ID,'手机号码已存在' from #lxrtb b where datalength(isnull(["&arrName(20)&"],''))>0 and ["&arrName(20)&"] in (select mobile from person) "
				cn.execute "delete from #lxrtb where datalength(isnull(["&arrName(20)&"],''))>0 and ["&arrName(20)&"] in (select mobile from person)" , n1
				cn.execute "insert into #k_fail(行号,失败原因) select 联系人ID,'联系人已存在' from #lxrtb b where 联系人ID in (select a.联系人ID from #lxrtb a, #kftb b , tel c , person d where a.客户ID=b.kid and c.name=b.["&arrName(1)&"] and d.company =c.ord and d.name=a.["&arrName(17)&"] and datalength(d.mobile) = 0 ) and datalength(isnull(["&arrName(20)&"],''))=0 "
				cn.execute "delete from #lxrtb where  联系人ID in (select a.联系人ID from #lxrtb a, #kftb b , tel c , person d where a.客户ID=b.kid and c.name=b.["&arrName(1)&"] and d.company =c.ord and d.name=a.["&arrName(17)&"] and datalength(d.mobile) = 0 ) and datalength(isnull(["&arrName(20)&"],''))=0" , n2
				cn.execute "insert into #k_all (内容) values ('从导入文件删除" & n1 & "条手机号码已存在的非主联系人记录，删除" & n2 & "条联系人已存在的非主联系人记录')"
			end if
			If arrShow(29)=1 Then
				cn.execute "insert into #k_fail(行号,失败原因) select 联系人ID,'"&arrName(29)&"无效,不识别“' + cast(["&arrName(29)&"] as varchar(255)) + '”。' from #lxrtb where isnumeric(isnull(["&arrName(29)&"],'0'))=0"
'If arrShow(29)=1 Then
				cn.execute "delete from #lxrtb where isnumeric(isnull(["&arrName(29)&"],'0'))=0", n3
			...
			If arrShow(100)=1 Then sql=sql & "weixinAcc,"
			If arrShow(13)=1 Then sql=sql & "zip,"
			sql=sql & "sort,sort1,area,trade,address,del,cateid,date7,date4,company,numc1,date8)" & vbcrlf
			sql = sql & "select distinct a.["&arrName(17)&"],"
			If arrShow(30)=1 Then sql=sql & "a.["&arrName(30)&"],"
			If arrShow(27)=1 Then sql=sql & "a.["&arrName(27)&"],"
			If arrShow(31)=1 Then sql=sql & "a.["&arrName(31)&"],(case when datalength(["& arrName(31) &"])>0 then datediff(YY,isnull(["& arrName(31) &"],'1900-1-1'),getdate()) end),"
			If arrShow(27)=1 Then sql=sql & "a.["&arrName(27)&"],"
			If arrShow(31)=1 Then
				sql=sql & "(case when datalength(["& arrName(31) &"])>0 then 1 else 0 end),"
			else
			...
			If arrShow(28)=1 Then sql=sql & "a.["&arrName(28)&"],"
			If arrShow(29)=1 Then sql=sql & "a.["&arrName(29)&"],"
			If arrShow(19)=1 Then sql=sql & "a.["&arrName(19)&"],"
			If arrShow(21)=1 Then sql=sql & "a.["&arrName(21)&"],"
			If arrShow(20)=1 Then sql=sql & "a.["&arrName(20)&"],"
			If arrShow(22)=1 Then sql=sql & "a.["&arrName(22)&"],"
			If arrShow(23)=1 Then sql=sql & "a.["&arrName(23)&"],"
			If arrShow(24)=1 Then sql=sql & "a.["&arrName(24)&"],"
			If arrShow(100)=1 Then sql=sql & "a.["&arrName(100)&"],"
			If arrShow(13)=1 Then sql=sql & "a.["&arrName(13)&"],"
			sql=sql & "c.sort,c.sort1,c.area,c.trade,a.家庭地址,1," & app.info.user & " ,'" & now & "','" & datestr & "',c.ord,1,getdate()  from #lxrtb a , #kftb b , tel c where a.客户ID = b.kid and c.name = b.["&arrName(1)&"] "
			cn.execute sql , n
			...
			uploader.ShowProc "写入客户扩展自定义： 写入数据            进度：" ,1000
			kz_sql=""
			set rs_kz_zdy=server.CreateObject("adodb.recordset")
			kz_sql="select id,FName from erp_customFields  where TName=1  and IsUsing=1 and canInport=1 order by FOrder desc "
			rs_kz_zdy.open kz_sql,cn,1,1
			do while not rs_kz_zdy.eof
				...
				& " select "&rs_kz_zdy("id")&",a.ord,isnull(b.["&rs_kz_zdy("FName")&"],'') from tel a ," & dbname & " b where a.tel_excel_drSign=b.up_index and a.ord > " &  maxord
			...
			sql =  "update person set age='' where age is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set jg='' where jg is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set part1='' where part1 is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set job='' where job is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set phone='' where phone is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set phone2='' where phone2 is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set fax='' where fax is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set mobile='' where mobile is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set email='' where email is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set msn='' where msn is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set qq='' where qq is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set zip='' where zip is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set address='' where address is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set year1='' where year1 is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set joy='' where joy is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set sort='' where sort is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set intro='' where intro is null and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set year1='' where isdate(year1)=0 and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set zip='' where isnumeric(zip)=0 and ord>" & maxord2 & vbcrlf
			sql = sql & "update person set pym=dbo.getPinYin(name), xl='',xw='',zy='',yx='',mz='',mobile2='',sg='',tz='',tx='',xm=''," & vbcrlf & _
			"xy='',xyname='',yj='',yjsort='',yjname='',yjsize='',jz='',hc='',hcsort='',jk='',jb='',sc='',scsort='',scys='',scpz='',tezheng=''" & vbcrlf & _
			...
			sql=sql & "update tel set gate='',product='', c2='',c3='',c4='',bank_1='',bank_2='',bank_3='',bank_4='',bank_5='',bank_6=''," & vbcrlf
			sql=sql & "bank2_1='',bank2_2='',bank2_3='',bank2_4='',bank2_5='',bank2_6=''" & vbcrlf
			...
			For vi=0 To arr_i-1
				...
				If Len(sort1arr(vi))>0 And isnumeric(sort1arr(vi)) Then
					call autoSkipSort(ordarr(vi),sortarr(vi),sort1arr(vi),11,replyarr(vi),True,"联系人导入")
				end if
			next
