	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			If dotstr<>"" Then
				...
				xlWorksheet.Cells(1,6+i).Value = "销售人员"
				xlWorksheet.Cells(1,6).font.bold=true
...
'if khmc<>"" then
				...
				if htid<>"" then
					str_Result=str_Result+" and htid like '%"&htid&"%' "
'if htid<>"" then
				end if
				if htmc<>"" then
					str_Result=str_Result+" and title like '%"&htmc&"%' "
'if htmc<>"" then
				end if
				if sort<>"" then
...
'if sort<>"" then
				...
				money3_all=0
				n=0
				conn.execute("select l.contract ,l.id , l.sort , l.bz , sum(l.money1) as money1 into #paysqlist " &_
				" From ( select distinct a.contract ,a.id , a.sort1 as sort , isnull(b.bz,14) as bz, a.money1 from paysqlist a inner join paysq b on a.fid = b.id and a.del=1 "&str_Result11&" ) l group by l.contract ,l.id , l.sort , l.bz ")
				conn.execute("select l.contract ,l.id , l.sort , l.bz , sum(l.money1) as money1 into #pay from " &_
				" ( select distinct a.contract ,a.ord as id , a.sort as sort , isnull(b.bz,14) as bz, a.money1 from pay a inner join f_pay b on a.fid = b.id and a.del=1 and a.complete=3 "&str_Result22&" ) l group by l.contract ,l.id , l.sort , l.bz")
				conn.execute(""&_
				"  select l.contract ,l.id , l.sort , l.bz , sum(l.money1) as money1,l.payid into #paybxlist"&_
				"  from ("&_
				"          select distinct b.contract ,a.id , a.sort , isnull(c.bz,14) as bz, a.money1,a.payid "&_
				"          from paybxlist a "&_
				"          inner join pay b on b.ord = a.payid "&_
				"          inner join paybx c on c.id = a.bxid "&str_Result33&" " &_
				"          where a.del=1 and c.complete=3" &_
				"  ) l " &_
				"  group by l.contract ,l.id , l.sort , l.bz ,l.payid")
				conn.execute("select distinct contract into #temp1 from (select contract from paysqlist WITH(NOLOCK) where del=1 and ISNULL(contract,0)>0 group by contract union all select contract from pay WITH(NOLOCK) where del=1 and complete=3 and ISNULL(contract,0)>0 group by contract) x")
				conn.execute("select distinct bz into #temp2 from (select bz from paysq WITH(NOLOCK) where del=1 group by bz union all select bz from f_pay WITH(NOLOCK) where del=1 group by bz union all select bz from paybx WITH(NOLOCK) where del=1 group by bz ) x")
				sql="select * from("&_
				"select c.ord,c.title,c.sort,c.htid,c.company,c.cateid,s.id as sortbzid,s.sort1 as sortbzname, "&_
				"p.id,p.sort1,p.sort2,"&_
				"(select sort1 from sortonehy where ord=p.sort2) as sortname,"&_
				"isnull(pq.money1,0) as money1,"&_
				"isnull(py.money1,0) as money2,"&_
				"isnull(b.money1,0) as money3 "&_
				"from contract c WITH(NOLOCK) "&_
				" inner join #temp1 pl on pl.contract=c.ord "&_
				" inner join paytype p WITH(NOLOCK) on 1=1 "&_
				" inner join sortbz s WITH(NOLOCK) on 1=1 "&_
				" inner join #temp2 tp2 WITH(NOLOCK) on tp2.bz=s.id "&_
				" left join #paybxlist b on b.contract = c.ord and b.bz = s.id and b.sort = p.id "&_
				" left join #paysqlist pq on pq.contract = c.ord and pq.bz = s.id and pq.sort = p.id "&_
				" left join #pay py on py.contract = c.ord and py.bz = s.id and py.sort = p.id and (b.payid is null or (b.payid is not null and b.payid=py.id)) "&_
				"where c.del=1 "&_
				") as fy6 "&_
				""&Str_Result&" "&_
				"and "&_
				...
				rs.open sql,conn,1,1
				C1=rs.recordcount
				xlApplication.Init "合同费用明细表_"&session("name2006chen")&".xls",C1
				dim i
				i=1
				if rs.RecordCount<=0 then
					...
					do until rs.eof
						...
						set rs7=nothing
						xlWorksheet.Cells(1+i,4).Value = rs("htid")
						set rs7=nothing
...
'loop
				...
				rs.close
				set rs=Nothing
				Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>合同费用明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!合同费用明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
				set rs=Nothing
				Response.write "" & vbcrlf & "</BODY>" & vbcrlf & ""
				Set fs = CreateObject("Scripting.FileSystemObject")
				tfile=Server.MapPath("合同费用明细表_"&session("name2006chen")&".xls")
				...
				Response.write """><font class=""red""><strong><u>下载导出的合同费用明细表</u></strong></font></a></p>" & vbcrlf & ""
