...
if(w2){
  ...
  var url = "cu.asp?sort3=2&name=" + escape(u_name)+"&ord="+escape(m)+"&telid="+escape(telid)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
...
function check_kh(ord) {
  ...
  var url = "../event/search_gys2.asp?ord="+escape(ord)+"&timestamp=" + new Date().getTime() + "&date1="+ Math.round(Math.random()*100);
  XMlHttp.open("GET", url, false);
  XMlHttp.onreadystatechange = function(){
...
function check1()
{
	...
	{
		document.getElementById("gys").innerHTML="* 请选择供应商！"
		return false;
	}
	return true;
}
