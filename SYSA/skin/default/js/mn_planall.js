...
function batDel() {	//批量删除
	...
	if(selectid == ""){
		app.Alert("您没有选择任何产品养护单，请选择后再删除！");
	}else{		
		if(confirm("确定要删除吗？")){
			ajax.regEvent("delYHOrders");
...
function delSLOrders(slord,ly){
	if(slord!=""){
		if(confirm("确定删除?")){
			ajax.regEvent("delYHOrders","planall.asp");
			...
			if(arr_res[0]=="-1"){	//没有删除权限
				...
				if (ly == 'list'){
					...
					try{
						...
						if(openHref.indexOf("/maintain/planall.asp")>0){
