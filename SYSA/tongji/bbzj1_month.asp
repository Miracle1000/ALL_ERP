...
'elseif qx_open="0" then
	...
	if request("selday")="" or isnull(request("selday")) then
		selday=day(now)
	else
	...
	if request("hiddendate")="" then
		...
		if request("jtdate")<>"" then
			if day(request("jtdate"))="" then
				tdate=year(request("jtdate"))+month(request("jtdate"))+day(request("jtdate"))
'if day(request("jtdate"))="" then
			else
...
'if day(request("jtdate"))="" then
	...
	if len(selyear)>0 and len(selmonth)>0 then tdate=selyear & "-" & selmonth & "-" & selday
	tdate=request("jtdate")
	tdmonth=month(tdate)
	tdyear=year(tdate)
	tdday=day(tdate)
	...
	SearStr = ""
	If request("dongjie")=1 Then
		StrDj = 1
		SearStr = SearStr & " or g.del=2"
	else
		StrDj = 0
	end if
	If request("huishouzhan")=1 Then
		StrDel = 1
...
'if C <> "" then
	...
	Response.write "" & vbcrlf & "<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.01 Transitional//EN""" & vbcrlf & """http://www.w3.org/TR/html4/loose.dtd"">" & vbcrlf & "<html>" & vbcrlf & "" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<title>"
	gate_result=" where g.name<>'' and (g.del=1 "& SearStr &")"
	Response.write title_xtjm
	Response.write "</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "var IsOpen = false;  //判断是否执行callServer2()函数，未执行过为假，执行过为真" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "//新增传值冻结帐号和回收站账号的勾选状态xieyanhui2013-3-6" & vbcrlf & "function callServer2() {" & vbcrlf & "      IsOpen = true;" & vbcrlf & "  //var url = ""liebiao_tj2.asp?timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  var dongjie = document.getElementById(""dongjie"");" & vbcrlf & "  var huishouzhan = document.getElementById(""huishouzhan"");" & vbcrlf & "  var djzh = "";" & vbcrlf & "  var hszzh = "";" & vbcrlf & "  if (dongjie.checked)" & vbcrlf & "  {" & vbcrlf & "                djzh=1;" & vbcrlf & "  }" & vbcrlf & "  if (huishouzhan.checked)" & vbcrlf & "  {" & vbcrlf & "         hszzh=1;" & vbcrlf &  " }" & vbcrlf & "  var url = ""namelist.asp?tdate="
	Response.write tdate
	Response.write "&sort_zjjg=3&dongjie=""+ djzh +""&huishouzhan=""+ hszzh +""&url=bbzj1_month.asp&timestamp="" + new Date().getTime() + ""&date1=""+ Math.round(Math.random()*100);" & vbcrlf & "  xmlHttp.open(""GET"", url, false);" & vbcrlf & "  xmlHttp.onreadystatechange = function(){" & vbcrlf & "  updatePage2();" & vbcrlf & "  };" & vbcrlf & "  xmlHttp.send(null);" & vbcrlf & "}" & vbcrlf & "function updatePage2() {" & vbcrlf & "var test7=""ht1""" & vbcrlf & "  if (xmlHttp.readyState < 4) {" & vbcrlf & "   ht1.innerHTML=""loading..."";" & vbcrlf & "  }" & vbcrlf & "  if (xmlHttp.readyState == 4) {" & vbcrlf & "    var response = xmlHttp.responseText;" & vbcrlf & "     ht1.innerHTML=response;" & vbcrlf & " xmlHttp.abort();" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & "<SCRIPT language=JavaScript1.2>" & vbcrlf & "" & vbcrlf & "// 一个简单的测试是否IE浏览器的表达式" & vbcrlf & "isIE = (document.all ? true : false);" & vbcrlf & "" & vbcrlf & "// 得到IE中各元素真正的位移量，即使这个元素在一个表格中" & vbcrlf & "function getIEPosX(elt) { return getIEPos(elt,""Left""); }" & vbcrlf & "function getIEPosY(elt) { return getIEPos(elt,""Top""); }" & vbcrlf & "function getIEPos(elt,which) {" & vbcrlf & " iPos = 0" &vbcrlf & " while (elt!=null) {" & vbcrlf & "  iPos += elt[""offset"" + which]" & vbcrlf & "  elt = elt.offsetParent" & vbcrlf & " }" & vbcrlf & " return iPos" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function getXBrowserRef(eltname) {" & vbcrlf & "// return (isIE ? document.all[eltname].style : document.layers[eltname]);" & vbcrlf & "     return document.getElementById(eltname);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function hideElement(eltname) { getXBrowserRef(eltname).style.visibility = 'hidden'; }" & vbcrlf & "" & vbcrlf & "// 按不同的浏览器进行处理元件的位置" & vbcrlf & "function moveBy(elt,deltaX,deltaY) {" & vbcrlf & " if (isIE) {" & vbcrlf & "  elt.left = elt.pixelLeft + deltaX;" & vbcrlf & "  elt.top = elt.pixelTop + deltaY;" & vbcrlf & " } else {" & vbcrlf & "  elt.left += deltaX;" & vbcrlf & "  elt.top += deltaY;" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function toggleVisible(eltname) {" & vbcrlf & " elt = getXBrowserRef(eltname);" & vbcrlf & " if (elt.style.visibility == 'visible' || elt.style.visibility == 'show') {" & vbcrlf & "   elt.style.visibility = 'hidden';" & vbcrlf & " } else {" & vbcrlf & "   fixPosition(eltname);" & vbcrlf & "   elt.style.visibility = 'visible';" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function setPosition(elt,positionername,isPlacedUnder) {" & vbcrlf & " positioner = null;" & vbcrlf & " if (isIE) {" & vbcrlf & "  positioner = document.all[positionername];" & vbcrlf & "  elt.left = getIEPosX(positioner);" & vbcrlf & "elt.top = getIEPosY(positioner);" & vbcrlf & " } else {" & vbcrlf &"   positioner = document.images[positionername];" & vbcrlf & "  elt.left = positioner.x; "& vbcrlf & "  elt.top = positioner.y;" & vbcrlf & " }" & vbcrlf & " if (isPlacedUnder) { moveBy(elt,0,positioner.height); }" & vbcrlf & "}"& vbcrlf & "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "//——————————————————————————————————————" & vbcrlf & "" & vbcrlf & "         // 判断浏览器" & vbcrlf & "         isIE = (document.all ? true : false);" & vbcrlf & "" & vbcrlf & "         // 初始月份及各月份天数数组" & vbcrlf & "         var months = new Array(""Month();" & vbcrlf & "     var displayYear = new Date().getFullYear();" & vbcrlf & "     var displayDivName;" & vbcrlf & "     var displayElement;" & vbcrlf & "" & vbcrlf & "         function getDays(month, year) {" & vbcrlf & "            //测试选择的年份是否是润年？" & vbcrlf & "            if (1 == month)" & vbcrlf & "               return ((0 == year % 4) && (0 != (year % 100))) ||" & vbcrlf & "                  (0 == year % 400) ? 29 : 28;" & vbcrlf & "            else" & vbcrlf & "               return daysInMonth[month];" & vbcrlf & "         }" & vbcrlf & "" & vbcrlf & "         function getToday(){" & vbcrlf & "            // 得到今天的日期" & vbcrlf & "            this.now = new Date();" & vbcrlf & "            this.year = this.now.getFullYear();" & vbcrlf & "            this.month = this.now.getMonth();" & vbcrlf & "            this.day = this.now.getDate();" & vbcrlf & "         }" & vbcrlf & ""& vbcrlf & "         // 并显示今天这个月份的日历" & vbcrlf & "         today = new getToday();" & vbcrlf & vbcrlf & "         function newCalendar(eltName,attachedElement) {" & vbcrlf & "        if (attachedElement) {" & vbcrlf & "           if (displayDivName && displayDivName != eltName) hideElement(displayDivName);" & vbcrlf & "           displayElement = attachedElement;" & vbcrlf & "        }" & vbcrlf & "        displayDivName = eltName;" & vbcrlf & "            today = new getToday();" & vbcrlf & "            var parseYear = parseInt(displayYear + '');" & vbcrlf & "            var newCal = new Date(parseYear,displayMonth,1);" & vbcrlf & "            var day = -1;" & vbcrlf & "            var startDayOfWeek = newCal.getDay();" & vbcrlf & "            if ((today.year == newCal.getFullYear()) &&" & vbcrlf & "                  (today.month == newCal.getMonth()))" & vbcrlf & "        {" & vbcrlf & "               day = today.day;" & vbcrlf & "            }" & vbcrlf & "            var intDaysInMonth =" & vbcrlf & "               getDays(newCal.getMonth(), newCal.getFullYear());" & vbcrlf & "            var daysGrid = makeDaysGrid(startDayOfWeek,day,intDaysInMonth,newCal,eltName)" & vbcrlf & "" & vbcrlf & "                   var elt = document.getElementById(eltName);" & vbcrlf & "                     elt.innerHTML = daysGrid;" & vbcrlf & "//        if (isIE) {" & vbcrlf & "//           var elt = document.all[eltName];" & vbcrlf & "//           elt.innerHTML = daysGrid;" & vbcrlf & "//        } else {" & vbcrlf &"//           var elt = document.layers[eltName].document;" & vbcrlf & "//           elt.open();" & vbcrlf & "//           elt.write(daysGrid);" & vbcrlf & "//           elt.close();" & vbcrlf & "//        }" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "     function incMonth(delta,eltName) {" & vbcrlf & "       displayMonth += delta;" & vbcrlf & "       if (displayMonth >= 12) {" & vbcrlf & "         displayMonth = 0;" & vbcrlf & "         incYear(1,eltName);" & vbcrlf & "       } else if (displayMonth <= -1) {" & vbcrlf & "         displayMonth = 11;" & vbcrlf & "         incYear(-1,eltName);" & vbcrlf & "       } else {" & vbcrlf & "         newCalendar(eltName);" & vbcrlf & "       }" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "     function incYear(delta,eltName) {" & vbcrlf & "       displayYear = parseInt(displayYear + '') + delta;" & vbcrlf & "       newCalendar(eltName);" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "     function makeDaysGrid(startDay,day,intDaysInMonth,newCal,eltName) {" & vbcrlf & "        var daysGrid;" & vbcrlf & "        var month = newCal.getMonth();" & vbcrlf & "        var year = newCal.getFullYear();" & vbcrlf & "        var isThisYear = (year == new Date().getFullYear());" & vbcrlf & "        var isThisMonth = (day > -1)" & vbcrlf & "        daysGrid = '<table border=1 cellspacing=0 cellpadding=2><tr><td bgcolor=#ffffff nowrap>';" & vbcrlf & "        daysGrid += '<font face=""courier new, courier"" size=2>';" & vbcrlf & "        daysGrid += '<a href=""javascript:hideElement(\'' + eltName + '\')"">x</a>';" & vbcrlf & "        daysGrid += '  ';" & vbcrlf & "        daysGrid += '<a href=""javascript:incMonth(-1,\'' + eltName + '\')"">&laquo; </a>';" & vbcrlf & "" & vbcrlf & "        daysGrid += '<b>';" & vbcrlf & "        if (isThisMonth) { daysGrid += '<a href=""javascript:setDay(';" & vbcrlf & "           daysGrid += dayOfMonth + ',\'' + eltName + '\')""> <font color=red>' + months[month] + '</font></a>'; }" & vbcrlf & "        else { daysGrid += '<a href=""javascript:setDay(';" & vbcrlf & "           daysGrid += dayOfMonth + ',\'' + eltName + '\')""> ' +months[month]+ '</a>'; }" & vbcrlf & "        daysGrid += '</b>';" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "        daysGrid += '<a href=""javascript:incMonth(1,\'' + eltName + '\')""> &raquo;</a>';" & vbcrlf & "        daysGrid += '   ';" & vbcrlf & "    daysGrid += '<a href=""javascript:incYear(-1,\'' + eltName + '\')"">&laquo; </a>';" & vbcrlf & "" & vbcrlf & "        daysGrid += '<b>';" & vbcrlf & "        if (isThisYear) { daysGrid += '<font color=red>' + year + '</font>'; }" & vbcrlf & "        else { daysGrid += ''+year; }" & vbcrlf & "   daysGrid += '</b>';" & vbcrlf & "" & vbcrlf & "        daysGrid += '<a href=""javascript:incYear(1,\'' + eltName + '\')""> &raquo;</a>';" & vbcrlf & "" & vbcrlf & "        var dayOfMonthOfFirstSunday = (7 - startDay + 1);" & vbcrlf & "        for (var intWeek = 0; intWeek < 6; intWeek++) {" & vbcrlf & "           var dayOfMonth;" & vbcrlf & "           for (var intDay = 0; intDay < 7; intDay++) {" & vbcrlf & "             dayOfMonth = (intWeek * 7) + intDay + dayOfMonthOfFirstSunday - 7;" & vbcrlf & "         if (dayOfMonth <= 0) {" & vbcrlf & "         } else if (dayOfMonth <= intDaysInMonth) {" & vbcrlf & "          var color = ""blue"";" & vbcrlf & "           if (day > 0 && day == dayOfMonth) color=""red"";" & vbcrlf & "           daysGrid += '<a href=""javascript:setDay(';" & vbcrlf & "           daysGrid += dayOfMonth + ',\'' + eltName + '\')"" '" & vbcrlf & "           daysGrid += 'style=""color:' + color + '"">';" & vbcrlf & "           var dayString = dayOfMonth + ""</a> "";" & vbcrlf & "           if (dayString.length == 6) dayString = '0' + dayString;" & vbcrlf & "         }" & vbcrlf & "           }" & vbcrlf & "" & vbcrlf & "        }" & vbcrlf & "        return daysGrid + ""</td></tr></table>"";" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "      function setDay(day,eltName) {" & vbcrlf & "       var myDate = new Date();" & vbcrlf & "       displayElement.value =displayYear+""-""+(displayMonth + 1)+ ""-"" +day;" & vbcrlf & "       hideElement(eltName);" & vbcrlf & "        document.location.href=""bbzj1_month.asp?px="
	Response.write px
	Response.write "&C="
	Response.write C
	...
	Response.write "&W3="
	Response.write W3
	Response.write "&W4=1&dongjie="
	Response.write StrDj
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write "&type=1&jtdate=""+displayYear+""-""+(displayMonth + 1)+""-""+myDate.getDate();" & vbcrlf & "     }" & vbcrlf & "" & vbcrlf & "//——————————————————————————————————————" & vbcrlf & "" & vbcrlf & "<!--" & vbcrlf & "// fixPosition() 这个函数和前面所讲的那个函数一样" & vbcrlf & "//" & vbcrlf & "function fixPosition(eltname) {" & vbcrlf & " elt = getXBrowserRef(eltname);" & vbcrlf & " positionerImgName = eltname + 'Pos';" & vbcrlf & " isPlacedUnder = false;" & vbcrlf & " if (isPlacedUnder) {" & vbcrlf & "  setPosition(elt,positionerImgName,true);" & vbcrlf & " } else {" & vbcrlf & "  setPosition(elt,positionerImgName)" & vbcrlf & " }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function toggleDatePicker(eltName,formElt) {" & vbcrlf & "  var x = formElt.indexOf('.');" & vbcrlf & "  var formName = formElt.substring(0,x);" & vbcrlf & "  var formEltName = formElt.substring(x+1);" & vbcrlf & "  newCalendar(eltName,document.forms[formName].elements[formEltName]);" & vbcrlf & "  toggleVisible(eltName);" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "// fixPositions() 这个函数前面也讲过" & vbcrlf & "function fixPositions()" & vbcrlf & "{" & vbcrlf & " fixPosition('daysOfMonth');" & vbcrlf & " fixPosition('daysOfMonth2');" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "// -->" & vbcrlf & "</SCRIPT>" & vbcrlf & "<script language=""javascript1.2"">" & vbcrlf & "function ChecklhbType(values)" & vbcrlf & "{" & vbcrlf & "  document.location.href=values + ""?px="
	'Response.write StrDel
	Response.write px
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "<script language=""JavaScript"" >" & vbcrlf & "function dh()" & vbcrlf & "{" & vbcrlf & "    checkaction();" & vbcrlf & "  if (IsOpen)" & vbcrlf & "     {" & vbcrlf & "               callServer2();" & vbcrlf & "  }" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function checkaction(){" & vbcrlf & "  var strdongjie = document.getElementById(""dongjie"");" & vbcrlf & "      var strhuishouzhan = document.getElementById(""huishouzhan"");" & vbcrlf & "      var strdjzh="""";" & vbcrlf & "   var strhszzh="""";" & vbcrlf & "  if (strdongjie.checked)" & vbcrlf & " {" & vbcrlf & "               strdjzh=1;" & vbcrlf & "   }" & vbcrlf & "       else" & vbcrlf & "    {" & vbcrlf & "               strdjzh=0;" & vbcrlf & "      }" & vbcrlf & "" & vbcrlf & "       if (strhuishouzhan.checked)" & vbcrlf & "     {" & vbcrlf & "               strhszzh=1;" & vbcrlf & "     }" & vbcrlf & "       else" & vbcrlf & "    {" & vbcrlf & "               strhszzh=0;" & vbcrlf & "     }" & vbcrlf & "  //var strurl =""bbzj1_month.asp?px="
	Response.write px
	Response.write "&jtdate="
	Response.write tdate
	Response.write "&C="
	...
	Response.write "&x=1&lhbtype="
	Response.write lhbtype
	Response.write "&Type=1&dongjie=""+ strdjzh +""&huishouzhan="" + strhszzh" & vbcrlf & "      var strurl =""bbzj1_month.asp?px="
	'Response.write lhbtype
	Response.write px
	Response.write "&jtdate="
	Response.write tdate
	...
	Response.write "&W3="
	Response.write W3
	Response.write "&W4=1&type=1&dongjie=""+ strdjzh +""&huishouzhan="" + strhszzh" & vbcrlf & " document.location.href=strurl;" & vbcrlf & "  //document.date.action=strurl;" & vbcrlf & "  //date.submit();" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body oncontextmenu=self.event.returnValue=false >" & vbcrlf & ""
	...
	Call InitReportBar("每月龙虎榜")
	action1="每月龙虎榜（业绩）"
	Response.write "" & vbcrlf & "<span id=""contentdiv"">" & vbcrlf & ""
	lhbtype=request.QueryString("lhbtype")
	Response.write "" & vbcrlf & "<table width=""99.9%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & " <tr>" & vbcrlf & "          <td> " & vbcrlf & "          <form action=""bbzj1_month.asp?px="
	Response.write px
	Response.write "&jtdate="
	Response.write tdate
	Response.write "&C="
	...
	Response.write "&lhbtype="
	Response.write lhbtype
	Response.write """ method=""get""　id=""demo"" onsubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"" >" & vbcrlf & "          <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> " & vbcrlf & "                       <tr>" & vbcrlf & "              <td  class=""place2"" width=""30%"">每月龙虎榜（业绩）</td>" & vbcrlf & "                <td width=""70%"">" & vbcrlf & "                         <table width=""350"" cellspacing=""2"">" & vbcrlf & "           <tr height=""25"">" & vbcrlf & "            <td align=""center""><a href=""#"" onClick=""date.hiddenflag.value=1;date.submit();""><img src=""../images/main_2.gif"" width=""8"" height=""8"" border=""0"" /> 前一月</a></td>" & vbcrlf & "              <td colspan=""3"" align=""center"">"
	Response.write tdyear
	Response.write "年"
	Response.write tdmonth
	Response.write "月</td>" & vbcrlf & "              <td align=""center""><a href=""#"" onClick=""date.hiddenflag.value=2;date.submit();"">后一月 <img src=""../images/main_1.gif"" width=""8"" height=""8"" border=""0"" /></a></td>" & vbcrlf & "                       <td align=""center""><input type=""button"" class=""anybutton"" value=""选择月份"" align=""absMiddle""  border=""0""  id=""daysOfMonth1Pos"" name=""daysOfMonth1Pos""  onmouseup=""toggleDatePicker('daysOfMonth1','date.ret1')"" />" & vbcrlf & "            <DIV id=daysOfMonth1 style=""POSITION: absolute""></DIV>" & vbcrlf & "            <INPUT name=""ret1"" type=""hidden"" size=10 > </td>"& vbcrlf & "            </tr>" & vbcrlf & "                     </table></td>" & vbcrlf & "                            <td  width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "              </tr>" & vbcrlf & "" & vbcrlf & "         <tr>" & vbcrlf & "                    <td width=""100%"" valign=""center"" colspan=""4"" >" & vbcrlf & "                       <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" >" & vbcrlf & "                                  <tr>" & vbcrlf & "                                  <td>&nbsp;</td>" & vbcrlf & "                                 <td align=""right"">" & vbcrlf & "                                        <!--增加冻结账号和回收站账号复选框xieyanhui2013-3-6-->" & vbcrlf & "                                   <input type=""checkbox"" name=""dongjie"" id=""dongjie"" onClick=""dh()"" value=""1"" "
	If StrDj=1 then
		Response.write "checked"
	end if
	...
	Response.write " /><label for=""huishouzhan"">含回收站</label>" & vbcrlf & "                                    人员姓名：<input name=""C1"" type=""text"" size=""10""  value=""""/>" & vbcrlf & "                           "
	...
	Response.write "" & vbcrlf & "                                     <input type=""button"" name=""Submit45"" value=""高级"" class=""anybutton2"" onClick=""callServer2();document.getElementById('ht1').style.display='';return false;""/>" & vbcrlf & "                                      </td>" & vbcrlf & "                                   <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                            </tr>" & vbcrlf & "                   </table>"  & vbcrlf & "                     </td>" & vbcrlf &  "          </tr>" & vbcrlf & vbcrlf &  "   </table><span id=""ht1""></span>" & vbcrlf & "    " & vbcrlf & "    <input type=""hidden"" name=""hiddendate"" value="
	Response.write tdate
	...
	Response.write """>" & vbcrlf & "        </form>" & vbcrlf & "    </td>" & vbcrlf & "     </tr>          " & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "        <tr class=""top"">" & vbcrlf & "      <td height=""26"" ><div align=""center"">姓名</div></td>" & vbcrlf & "        <td colspan=""8"" ><div align=""center"">本月龙虎榜</div></td>" & vbcrlf & "          <td ><div align=""center"">                                       <SELECT style=""BORDER-BOTTOM: #ffffff 0px solid; BORDER-LEFT: #ffffff 0px solid; BACKGROUND: #ffffff; HEIGHT: 20px; COLOR: #2f496e; FONT-SIZE: 12px; OVERFLOW: hidden; BORDER-TOP: #ffffff 0px solid; FONT-WEIGHT: bold; BORDER-RIGHT: #ffffff 0px solid"" onchange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.name+'='+this.value);}"" name=""pz"">" & vbcrlf & "                                             <OPTION>-请选择-</OPTION>" & vbcrlf & "                                         <OPTION value=""10"""
	...
	Response.write ">每页显示200条</OPTION>" & vbcrlf & "                                      </SELECT></div></td>" & vbcrlf & "      </tr>" & vbcrlf & " <tr>" & vbcrlf & "      <td height=""27"" >&nbsp;</td>" & vbcrlf & "      <td colspan=""4""  align=""center"">销售龙虎榜</td>" & vbcrlf & "     <td colspan=""4""  align=""center"">到账龙虎榜</td>" & vbcrlf & "     <td   align=""center"">&nbsp;</td>" & vbcrlf & "      </tr>" & vbcrlf & " <tr>" & vbcrlf & "      <td height=""26"" ></td>" & vbcrlf & "    <td  colspan=""2""  align=""center"">本月</td>" & vbcrlf & "          <td  colspan=""2""  align=""center"">累计</td>" & vbcrlf & "          <td  colspan=""2""  align=""center"">本月</td>" & vbcrlf & "      <td  colspan=""2""  align=""center"">累计</td>" & vbcrlf & "          <td align=""center"">&nbsp;</td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "            <td align=""center"">&nbsp;</td>" & vbcrlf & "      <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=1&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>数量</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=m1&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>金额</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=2&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>数量</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=m2&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>金额</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=3&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>数量</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=m3&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>金额</u></a></td>" & vbcrlf & "        <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=4&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	Response.write "&huishouzhan="
	Response.write StrDel
	Response.write """><u>数量</u></a></td>" & vbcrlf & "         <td  width=""9%"" align=""center""><a href=""bbzj1_month.asp?px=m4&jtdate="
	Response.write tdate
	Response.write "&C="
	Response.write C
	Response.write "&x=1&lhbtype="
	...
	"left join ( "&_
	"  select cateid,count(ord) as numcg1,isnull(sum(money2),0) as numcg1m,0 as numcg2,0 as numcg2m,0 as numcg3,0 as numcg3m, "&_
	"  0 as numcg4,0 as numcg4m "&_
	"  from contract WITH(NOLOCK) "&_
	"  Where year(date3)="&tdyear&" and month(date3)="&tdmonth&" and del=1 and isnull(status,-1) in (-1,1)  group by cateid "&_
	"  union all "&_
	...
	"  inner join contract b WITH(NOLOCK) on a.contract = b.ord and b.del=1 "&_
	"  left join hl h on h.bz = b.bz and h.date1 = convert(varchar(10),b.date3,120) "&_
	"  where  year(a.date5)="&tdyear&" and month(a.date5)="&tdmonth&" and a.del=1 and isnull(b.status,-1) in (-1,1)  and a.contract=b.ord group by a.cateid "&_
	"  union all "&_
	...
	rs1.close
	set rs1=nothing
	Response.write "" & vbcrlf & "  <script language=javascript>" & vbcrlf & "function test()" & vbcrlf & "{" & vbcrlf & "  if(!confirm('确认删除吗？')) return false;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function mm()" & vbcrlf & "{" & vbcrlf & "   var a = document.getElementsByTagName(""input"");" & vbcrlf & "   if(a[0].checked==true){" & vbcrlf & "   for (var i=0; i<a.length; i++)" & vbcrlf & "      if (a[i].type == ""checkbox"") a[i].checked = false;" & vbcrlf & "   }" & vbcrlf & "   else" & vbcrlf & "   {" & vbcrlf & "   for (var i=0; i<a.length; i++)" & vbcrlf & "      if (a[i].type == ""checkbox"") a[i].checked = true;" & vbcrlf & "   }" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf& " "
	call close_list(1)
	Response.write "" & vbcrlf & "     </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</span>" & vbcrlf & ""
	Call closeReportBar()
