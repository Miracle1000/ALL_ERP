...
'If Not rs.Eof Then
	...
	Response.write "" & vbcrlf & "<!DOCTYPE html PUBLIC ""-//W3C//DTD XHTML 1.0 Transitional//EN"" ""http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd"">" & vbcrlf & "<html xmlns=""http://www.w3.org/1999/xhtml"">" & vbcrlf & "<head>" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"" />" & vbcrlf & "<title>邮件群发</title>" & vbcrlf & "<link href=""../inc/cskt.css?ver="
	'Server.ScriptTimeOut=100000000
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script language=""javascript"" src=""../inc/system.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript""></script>" & vbcrlf & "<script src=""../inc/loadingBar.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write "" & vbcrlf & "<table width=""100%""  border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <form id=""date"" name=""date"" method=""post"" action=""index.asp?qf=1&sendType="
	Response.write sendType
	Response.write """ target=""_parent"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">群发邮件</td>" & vbcrlf & "            <td></td>"& vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td align=""center""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" >" & vbcrlf & " <tr>" & vbcrlf & "                  <td width=""19%"" align=""right"" valign=""middle"">状态：</td>" & vbcrlf & "                  <td width=""81%"" align=""left"">"
	...
	if typeid=1 then
		...
		if selectid="" then
			Response.write"<script language=javascript>alert('您没有选择任何记录，请选择后再重试！');history.back()</script>"
			call db_close : Response.end
		else
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from email_Person where del=1 "&Str_Result&" "&str_sql_checkid&" and (sort is null or sort=0 or sort in(select id from email_Person_class where (CHARINDEX(',"&session("personzbintel2007")&",',','+user_list+',')>0 or user_list='0,0' or user_list='0' or user_list is null) and (del=1 or del is null))) order by sort,ord desc"
	...
	strPerson=""
	do until rs.eof
		personName=trim(rs("clictName"))
		personMobile=trim(rs("clictEmail"))
		emailId=rs("ord")
		if personMobile<>"" then
			if isEmail(personMobile)=false then
				Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的邮箱号码("&personMobile&")格式不符，不能发送</font><br/>")
				errNum=errNum+1
'Response.write ("<font  color=#ff0000>第【"&i&"】条:“"&personName&"”的邮箱号码("&personMobile&")格式不符，不能发送</font><br/>")
			else
				if sendType<>"1" then
					...
					if strEmailIds="" then
						...
						strEmailIds=strEmailIds&","&emailId
					end if
				end if
			end if
		else
			Response.write ("第【"&i&"】条:“"&personName&"”的email为空，不能发送<br/>")
			errNum=errNum+1
			'Response.write ("第【"&i&"】条:“"&personName&"”的email为空，不能发送<br/>")
		end if
		rs.movenext
...
'loop
	...
	action1="邮件群发"
	call close_list(1)
	Response.write "</td>" & vbcrlf & "                </tr>" & vbcrlf & "            " & vbcrlf & "                <tr>" & vbcrlf & "                  <td align=""right""></td>" & vbcrlf & "                  <td align=""left"">" & vbcrlf & "                    "
	if sendType="1" then
		Response.write "" & vbcrlf & "                  <input type=""hidden"" value="""
		Response.write strid
		Response.write """ name=""emailIds"" />" & vbcrlf & "                  "
	end if
	if sendType="0" then
		Response.write "" & vbcrlf & "                  <input type=""hidden"" value="""
		Response.write strEmailIds
		Response.write """ name=""emailIds"" />" & vbcrlf & "                  "
	end if
	if sendType="" then
		Response.write "" & vbcrlf & "                  <input type=""hidden"" value="""
		Response.write strPhone
		Response.write """ name=""email"" />" & vbcrlf & "                  "
	end if
	Response.write "" & vbcrlf & "                    <input type=""submit"" "
	if (C1-errNum)>0 then
		Response.write "" & vbcrlf & "                    <input type=""submit"" "
