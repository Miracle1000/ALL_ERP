...
'i1 = InStr(1, data, "l.id=""", 1)
	...
	sub app_sys_lvw_callback
		...
		response.write backcode : response.end
		execute backcode
...
'Response.write "</div></form>"
	...
	Sub onReportInit(rpt, lt)
		...
		rpt.title = "提成规则设置"
		rpt.headHTML = "<style> #lvw_dbtable_mlistvw tr td table{width:100%} #lvw_dbtable_mlistvw table.formulaTab td.td1{width:70%; text-align:left;vertical-align:top; padding-top:1px;} #lvw_dbtable_mlistvw table.formulaTab td.td2{width:30%; text-align:right;}</style>"
'rpt.title = "提成规则设置"
		rpt.addfield "&nbsp;","select","rule", "统一规则" & Chr(1) & "1" & Chr(2) & "例外规则" & Chr(1) & "2","",""
		rpt.SetNullMsg 0,"规则"
		rpt.addfield "&nbsp;","select","radix", "销售额" & Chr(1) & "1" & Chr(2) & "毛利" & Chr(1) & "3"& Chr(2) & "实际销售价" & Chr(1) & "5"& Chr(2) & "毛利（进价）" & Chr(1) & "7","",""
		rpt.SetNullMsg 1,"基数"
		rpt.addfield "&nbsp;","select","scale", "同一比例" & Chr(1) & "1" & Chr(2) & "累加比例" & Chr(1) & "2"& Chr(2) & "产品比例" & Chr(1) & "3","",""
		rpt.SetNullMsg 2,"类型"
		rpt.addfield "&nbsp;","select","searchType", "产品编号" & Chr(1) & "2"& Chr(2) & "产品型号" & Chr(1) & "3"& Chr(2) & "拼音码" & Chr(1) & "4"& Chr(2) & "条形码" & Chr(1) & "5","",""
		rpt.SetNullMsg 3,"产品名称"
		rpt.addfield "","text","searchKey", "", "", ""
		rpt.showtoppagesize = False
		rpt.canexcel = False
		rpt.canprint = False
		Dim topButtonHTML
		topButtonHTML = "<input type='button' class='anybutton' style='width:90px;' onclick=""javascript:window.open('set_ht.asp','new3win','width=' + 850 + ',height=' + 670 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" value='添加提成规则'>"
'Dim topButtonHTML
		rpt.buttonsHTML = topButtonHTML
		rpt.sql = "select p.ord, p.addcate, p.title 产品名称, p.sort1 分类, dw.sort1 单位, p.type1 型号, (case when isnull(p.tcsort2,0)=0 then '统一规则' else '例外规则' end) 规则,(case isnull(p.tcsort1,0) when 1 then '销售额' when 3 then '毛利' when 5 then '实际销售价' when 7 then '毛利（进价）' else (case ISNULL(s1.intro,0) when 1 then '销售额' when 3 then '毛利' when 5 then '实际销售价' when 7 then '毛利（进价）' else '' end) end) 基数, (case isnull(p.tcsort2,0) when 1 then '同一比例' when 2 then '累加比例' when 3 then '产品比例' when 4 then '累加比例' else (case ISNULL(s2.intro,0) when 1 then '同一比例' when 2 then '累加比例' when 3 then '产品比例' when 4 then '累加比例' else '' end) end) 类型, 0 计算公式, isnull(p.tcsort2,0) tcsort2, ISNULL(s2.intro,0) intro2, isnull(p.num_tc,0) num_tc, 0 操作 from product p left join sortonehy dw on p.unitjb=dw.ord left join setopen s1 on s1.sort1 = 11 left join setopen s2 on s2.sort1 = 12 where p.del=1 and (isnull(p.tcsort1,0)>0 or ISNULL(s1.intro,0)>0) and (p.user_list is null or replace(replace(replace(p.user_list,' ',''),',',''),'0','')='' or charindex(',"&session("personzbintel2007")&",',','+replace(p.user_list,' ','')+',')>0) and (len(@rule)=0 or (len(@rule)>0 and ((@rule='1' and isnull(p.tcsort2,0)=0) or (@rule='2' and isnull(p.tcsort2,0)>0)))) and (len(@radix)=0 or (len(@radix)>0 and (p.tcsort1>0 and p.tcsort1=cast(@radix as int) or (isnull(p.tcsort1,0)=0 and s1.intro=cast(@radix as int))))) and (len(@scale)=0 or (len(@scale)>0 and (p.tcsort2>0 and (@scale='2' and p.tcsort2 in(2,4) or (p.tcsort2=cast(@scale as int)))) or (isnull(p.tcsort2,0)=0 and (@scale='2' and s2.intro in(2,4) or (s2.intro=cast(@scale as int)))))) and (len(@searchKey)=0 or (len(@searchKey)>0 and ((@searchType='' and p.title like '%'+@searchKey+'%') or (@searchType='2' and p.order1 like '%'+@searchKey+'%') or (@searchType='3' and p.type1 like '%'+@searchKey+'%') or (@searchType='4' and p.pym like '%'+@searchKey+'%') or (@searchType='5' and p.ord in(select product from jiage where txm like '%'+@searchKey+'%'))))) order by p.date7 desc, p.ord desc"
'rpt.buttonsHTML = topButtonHTML
	end sub
	Dim num1_dot
	Sub app_oncreate(lvw)
		Dim h, i, col, btmStr, rs2, intro12
		intro12 = 0
		Set rs2 = cn.execute("select isnull(intro,0) intro12 from setopen where sort1 = 12")
		...
		col.width = 40
		col.formattext = "code:getColHtml(@cells[1],""选择"",""@value"","& intro12 &",""@cells[7]"")"
...
'col.dbtype = "str"
		...
		col.width = 130
		col.dbtype = "str"
		col.formattext = "code:getColHtml(@cells[1],""分类"",""@value"",0,0)"
		...
		col.width = 280
		col.formattext = "code:getColHtml(@cells[1],""计算公式"",@cells[11],@cells[12],@cells[13])"
		lvw.headers(11).display = "none"
		lvw.headers(12).display = "none"
		lvw.headers(13).display = "none"
		Set col = lvw.headers(14)
		col.width = 90
		col.formattext = "code:getColHtml(@cells[1],""操作"",@cells[2],"& intro12 &",""@cells[7]"")"
		btmStr = "   &nbsp;<button class='oldbutton3' onclick=""batDel('删除')"">批量删除</button>"
		If intro12>0 Then
			btmStr = "   &nbsp;<button class='oldbutton3' onclick=""batDel('重置')"" title='批量重置成统一规则'>批量重置</button>"
		end if
		btmStr = btmStr& "<a>"
		lvw.addlink = "</a><input type='checkbox' onclick='selectall(this)'>全选"& btmStr
	end sub
	Sub App_lvw_onnullData(lvw)
		Dim tempStr, intro_45_2, sumkd, sumkd2
		tempStr = "<div style='height:28px; line-height:28px; text-align:center'>没有信息！</div>"
'Dim tempStr, intro_45_2, sumkd, sumkd2
		lvw.addhtml tempStr
	end sub
	Function getColHtml(mxid,colName,colValue,revID1,revID2)
		Dim tempStr
		tempStr = ""
		...
		Case "选择"
		If revID2 = "例外规则" Then
			tempStr = "<input type=checkbox class='lvcbox' name='sys_lvw_ckbox' value='" & mxid & "'>"
		end if
		Case "分类"
		If colValue&""<>"" Then
			tempStr = GetFullSort("menu",colValue,"","menuname","","->")
'If colValue&""<>"" Then
		end if
		Case "计算公式"
		Dim cpord, tcsort2, intro2, num_tc, num_dot_xs, rs, num1, sort1
		cpord = mxid : tcsort2 = colValue : intro2 = revID1 : num_tc = revID2
		num_dot_xs = info.MoneyNumber
		sort1 = 0
		If tcsort2>0 Then
			sort1 = tcsort2
		ElseIf intro2>0 Then
			sort1 = intro2
		end if
		Select Case tcsort2
		Case 1, 2, 4
		tempStr = getTCFormula(sort1, cpord)
		Case 3 : tempStr = FormatNumber(num_tc,num_dot_xs,True) &"%"
		Case 0
		Select Case intro2
		Case 1, 2, 4
		tempStr = getTCFormula(sort1, 0)
		Case 3 : tempStr = FormatNumber(num_tc,num_dot_xs,True) &"%"
		End Select
		End Select
		Case "操作"
		tempStr = tempStr & "<button class=""oldbutton2"" onclick=""javascript:window.open('set_ht.asp?ord=" & app.base64.pwurl(mxid) & "','new5win','width=' + 850 + ',height=' + 670 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"">修改</button>&nbsp;"
'Case "操作"
		If revID1 = 0 Then
			tempStr = tempStr & "<button class=""oldbutton2"" onclick=""javascript:delCPTCSet("& mxid &",'删除')"">删除</button>"
		ElseIf revID1>0 Then
			If revID2 = "例外规则" Then
				tempStr = tempStr & "<button class=""oldbutton2"" onclick=""javascript:delCPTCSet("& mxid &",'重置')"" title='重置成统一规则'>重置</button>"
			end if
		end if
		End Select
		getColHtml = tempStr
...
'if filed_keyId&"" = "" then filed_keyId = "id"
			...
			sortStr=""
			Set rsf = cn.execute("select "& filed_id1 &","& filed_sort1 &" from "& theTable &" where "& filed_keyId &"=" & sortID)
			If rsf.Eof = False Then
				...
				Dim sort_i
				For sort_i = 1 To 20
					Set rst=cn.execute("select "& filed_id1 &","& filed_sort1 &" from "& theTable &" where "& filed_keyId &"=" & id1)
	...
	Function getTCFormula(sort1, cpord)
		Dim rs, num1, money1, money2, formula1, formula2, num_dot_xs, tempStr
		num_dot_xs = info.MoneyNumber : tempStr = ""
		Select Case sort1
		Case 1
		Set rs = cn.execute("select isnull(num1,0) num1 from tcbl where sort1 = "& sort1 &" and isnull(ord,0)="& cpord &"")
		If rs.eof = False Then
			tempStr = FormatNumber(CDbl(rs("num1")),num_dot_xs,True) &"%"
		end if
		rs.close
		set rs = nothing
		Case 2, 4
		Set rs = cn.execute("select isnull(num1,0) num1, isnull(money1,0) money1, isnull(money2,0) money2, isnull(tc_formula1,'') formula1, isnull(tc_formula2,'') formula2 from tcbl where sort1 = "& sort1 &" and isnull(ord,0)="& cpord &" order by gate1")
		If rs.eof = False Then
			tempStr = tempStr & "<table class='formulaTab' border=0 cellpadding=0 cellspacing=0>"
			While rs.eof = False
	...
	Sub App_delCPTCSet()
		Dim rs, rs2, ord, tcsort1, tcsort2, intro11, intro12
		ord = request("ord")
		if ord&""="" then
			ord = "0"
		else
		...
		If ord&"" = "" Or ord="0" Then
			...
			cn.execute("update product set num_tc=0, tcsort1=null, tcsort2=null where ord in("& ord &")")
			cn.execute("delete from tcbl where ord in("& ord &")")
			Response.write "1"
			Response.end
		end if
	end sub
