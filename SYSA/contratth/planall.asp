...
'elseif sp1="2" then
	...
	if  clng("0" & request.QueryString("IsOther"))= 0 then
		...
		W3 = ""
		set strW3rs = conn.execute("select ord  from gate with(nolock) where orgsid=" & clng(W1))
	...
	If w1="999999999" Then
		Str_Result=Str_Result+" and isnull(cateid,0)= 0 and not exists (select ord from gate WITH(NOLOCK) where ord=cateid)"
'If w1="999999999" Then
	end if
...
'elseif B = "thbh" then
		elseif B = "xsry" then
			Str_Result=Str_Result+" and cateid in (select ord from gate WITH(NOLOCK) where name like '%"& C &"%' ) "
'elseif B = "xsry" then
		elseif B = "cjry" then
...
'if m2<>"" And gjjs<>"1" then
	...
	if A2<>"" then
		function menuarea(id1)
			...
			sqlarea="select id from menuarea WITH(NOLOCK) where id1="&id1&" "
...
'if company_th<>"" then
	...
	if request("khmc")<>"" then
		Str_Result=Str_Result+" and company in (select ord from tel WITH(NOLOCK) where name like '%"&request("khmc")&"%') "
'if request("khmc")<>"" then
	end if
	if request("productname")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where ord in (select ord from product WITH(NOLOCK) where title like '%"&request("productname")&"%')) "
'if request("productname")<>"" then
	end if
	if request("cpbh")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where ord in (select ord from product WITH(NOLOCK) where order1 like '%"&request("cpbh")&"%')) "
'if request("cpbh")<>"" then
	end if
	if request("cpxh")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where ord in (select ord from product WITH(NOLOCK) where type1 like '%"&request("cpxh")&"%')) "
'if request("cpxh")<>"" then
	end if
	if request("cpph")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where ph like '%"&request("cpph")&"%') "
'if request("cpph")<>"" then
	end if
	if request("cpxlh")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where xlh like '%"&request("cpxlh")&"%') "
'if request("cpxlh")<>"" then
	end if
	if  Request("duepaydate1")<>"" and Request("duepaydate2")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where datesc between '"&Request("duepaydate1")&"' and '"&Request("duepaydate2")&"') "
'if  Request("duepaydate1")<>"" and Request("duepaydate2")<>"" then
...
'ElseIf  Request("duepaydate2")<>"" Then
	...
	if  Request("paydate1")<>"" and Request("paydate2")<>"" then
		Str_Result=Str_Result+" and ord in (select caigou from contractthlist WITH(NOLOCK) where dateyx between '"&Request("paydate1")&"' and '"&Request("paydate2")&"') "
'if  Request("paydate1")<>"" and Request("paydate2")<>"" then
...
'ElseIf Request("duemoney2")<>"" Then
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "    margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".ser_btn .anybutton{*height:21px;*line-height:13px;*margin-bottom:0;*padding-top:2px}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/ch_planall.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write "" & vbcrlf & "      <form method=""get"" action=""planall.asp"" id=""demo"" onkeyup=""if(event.keyCode==13){this.submit();}"" onSubmit=""return Validator.Validate(this,2)"" name=""date"" style=""margin:0"">" & vbcrlf & "  <input type=""hidden"" name=""company"" value="""
	Response.write company_th
	Response.write """>" & vbcrlf & "         <input type=""hidden"" name=""sp"" value="""
	Response.write sp1
	Response.write """> " & vbcrlf & "       <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td class=""place"" style=""width:300px;"">" & vbcrlf & "        "
	if H="1" then
		Response.write "退货检索"
	elseif sp1="1" then
		Response.write "审批中退货"
	elseif sp1="2" then
		Response.write "未通过退货"
	else
		Response.write "退货列表"
	end if
	Response.write "</td>" & vbcrlf & "      <td style=""width:100px"">&nbsp;<a class=""sortRule"" href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "      <td style=""width:auto"" height=""20"" align=""right"">" & vbcrlf & "            <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-weight: bold;color:#2F496E; overflow:hidden"">" & vbcrlf & "        <option>-请选择-</option>" & vbcrlf & "        <option value=""page_count=10"" "
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "      </select>&nbsp;" & vbcrlf & "         </td>" & vbcrlf & "           <td style=""width:3px""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "    </tr>" & vbcrlf & "      </table>" & vbcrlf & "         <div id=""khh"" style=""margin-top:-1px;overflow:hidden;"">" & vbcrlf & "   <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "      <tr>" & vbcrlf & "        <td  height=""55"" class='ser_btn' align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"">"
	'Response.write "selected"
	...
	Response.write "" & vbcrlf & "<select name=""F"">" & vbcrlf & "      <option value=""0"" "
	if F=0 then
		Response.write "selected"
	end if
	Response.write ">退货分类</option>" & vbcrlf & ""
	set rs2=server.CreateObject("adodb.recordset")
	sql2="select ord,sort1 from sortonehy WITH(NOLOCK) where gate2=35 order by gate1 desc"
	rs2.open sql2,conn,1,1
	do until rs2.eof
		Response.write "" & vbcrlf & "     <option value="""
		Response.write rs2("ord")
		Response.write """ "
		if clng(F)=rs2("ord") then
			Response.write "selected"
		end if
		Response.write ">"
	...
	if ZBRuntime.MC(17000) and ZBRuntime.MC(17002) then
		Response.write "" & vbcrlf & "<select name=""A1"">" & vbcrlf & "  <option value=""10"" "
		if A1=10 then
			Response.write "selected"
		end if
		Response.write ">入库进展</option>" & vbcrlf & "  <option value=""1"" "
		if A1=1 then
			Response.write "selected"
		end if
		Response.write ">未入库</option>" & vbcrlf & "  <option value=""2"" "
		if A1=2 then
			Response.write "selected"
		end if
		Response.write ">部分入库</option>" & vbcrlf & "  <option value=""3"" "
		if A1=3 then
			Response.write "selected"
		end if
		Response.write ">入库完毕</option>" & vbcrlf & "  <option value=""4"" "
		if A1=4 then
			Response.write "selected"
		end if
		Response.write ">无需入库</option>" & vbcrlf & "</select>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "<select name=""B"">" & vbcrlf & "  <option value=""thzt"" "
	if B="thzt" then
		Response.write "selected"
	end if
	Response.write ">退货主题</option>" & vbcrlf & "  <option value=""thbh"" "
	if B="thbh" then
		Response.write "selected"
	end if
	Response.write ">退货编号</option>" & vbcrlf & "  <option value=""xsry"" "
	if B="xsry" then
		Response.write "selected"
	end if
	Response.write ">销售人员</option>" & vbcrlf & "  <option value=""cjry"" "
	if B="cjry" then
		Response.write "selected"
	end if
	Response.write ">添加人员</option>" & vbcrlf & "</select>" & vbcrlf & "<input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/>" & vbcrlf & "<input type=""button"" name=""Submit422"" value=""检索"" onclick=""javascript:document.getElementById('demo').submit();""  class=""anybutton""/>" & vbcrlf & ""
	if open_41_10=1 or open_41_10=3 then
		Response.write "" & vbcrlf & "<input type=""button"" name=""Submit3"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_th.asp?"
		Response.write sdk.ReturnURL("")
		Response.write "'});}"" class=""anybutton""/>"
	end if
	Response.write "" & vbcrlf & "<input type=""hidden"" id=""th_sp"" name=""th_sp"" value="""
	Response.write sp1
	Response.write """ /><input type=""hidden"" id=""th_company"" name=""th_company"" value="""
	Response.write company_th
	Response.write """ />" & vbcrlf & "<a href=""javascript:void(0)"" class=""AfterQuickSearch"" style='display:inline-block;width:108px;*height:30px;*line-height:30px;*padding:0;padding-top:4px;margin:0;*margin-left:-5px' onclick=""callServer2();document.getElementById('khh').style.display='none';document.getElementById('ht1').style.display='block';return false;""><u><font class=""advanSearch"" style=""background:##efefef;"">高级检索</font></u></a>" & vbcrlf & "           </td>" & vbcrlf & "        </tr>" & vbcrlf & "      </table>" & vbcrlf & "      </div>" & vbcrlf & "          <span id=""ht1"" style=""padding:20px;display:none""></span>" & vbcrlf & "      </form>" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style=""margin-left:20px"">" & vbcrlf & "      <tr class=""top"">" & vbcrlf & "    <td align=""center"" width=""4%""><div align=""center"">选择</div></td> "& vbcrlf &   "  <td align=""center"" width=""20%""><div align=""center""> "& vbcrlf &    "<select name=""select"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E; overflow:hidden"" >" & vbcrlf & "      <option>-请选择-</option>" & vbcrlf & "      <option value=""lie_1=1"" "
	Response.write company_th
	if lie_1=1 then
		Response.write "selected"
	end if
	Response.write "><strong>退货主题</strong></option>" & vbcrlf & "      <option value=""lie_1=2"" "
	if lie_1=2 then
		Response.write "selected"
	end if
	Response.write ">退货编号</option>" & vbcrlf & "    </select>" & vbcrlf & "        </div></td>" & vbcrlf & "     <td align=""center"" width=""20%""><div align=""center"">" & vbcrlf & "   <select name=""select"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E; overflow:hidden"" >" & vbcrlf & "      <option>-请选择-</option>" & vbcrlf & "      <option value=""lie_4=1"" "
	'Response.write "selected"
	if lie_4=1 then
		Response.write "selected"
	end if
	Response.write "><strong>关联客户</strong></option>" & vbcrlf & "      <option value=""lie_4=2"" "
	if lie_4=2 then
		Response.write "selected"
	end if
	Response.write ">客户编号</option>" & vbcrlf & "    </select>" & vbcrlf & "        </div></td>" & vbcrlf & "     <td align=""center"" width=""20%""><div align=""center"">" & vbcrlf & "         <select name=""select2"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND:#FFFFFF;HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;overflow:hidden"">" & vbcrlf & "        <option>-请选择-</option>" & vbcrlf & "        <option value=""lie_2=1"" "
	'Response.write "selected"
	if lie_2=1 then
		Response.write "selected"
	end if
	Response.write ">退货金额</option>" & vbcrlf & "        <option value=""lie_2=2"" "
	if lie_2=2 then
		Response.write "selected"
	end if
	Response.write ">退货阶段</option>" & vbcrlf & "        <option value=""lie_2=3"" "
	if lie_2=3 then
		Response.write "selected"
	end if
	Response.write ">退货分类</option>" & vbcrlf & "        <option value=""lie_2=4"" "
	if lie_2=4 then
		Response.write "selected"
	end if
	Response.write ">退货状态</option>" & vbcrlf & "      </select>" & vbcrlf & "      </div></td>" & vbcrlf & "     <td align=""center"" width=""20%""><div align=""center"">" & vbcrlf & "         <select name=""select4"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND:#FFFFFF;HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;overflow:hidden"">" & vbcrlf & "              <option>-请选择-</option>" & vbcrlf & "       <option value=""lie_3=1"" "
	'Response.write "selected"
	if lie_3=1 then
		Response.write "selected"
	end if
	Response.write ">入库进展</option>" & vbcrlf & "         <option value=""lie_3=2"" "
	if lie_3=2 then
		Response.write "selected"
	end if
	Response.write ">退货日期</option>" & vbcrlf & "                     <option value=""lie_3=3"" "
	if lie_3=3 then
		Response.write "selected"
	end if
	Response.write ">销售人员</option>" & vbcrlf & "           <option value=""lie_3=4"" "
	if lie_3=4 then
		Response.write "selected"
	end if
	Response.write ">创建人员</option>" & vbcrlf & "                     <option value=""lie_3=5"" "
	if lie_3=5 then
		Response.write "selected"
	end if
	Response.write ">创建日期</option>" & vbcrlf & "              </select>" & vbcrlf & "       </div></td>" & vbcrlf & "     <td align=""center"" width=""22%""><div align=""center"">操作</div></td>" & vbcrlf & "    </tr>" & vbcrlf & ""
	fromContract=trim(Request.QueryString("contract"))
	if fromContract<>"" then
		fromContent=" and ord in (select caigou from contractthlist WITH(NOLOCK) where contract = "&fromContract&")"
	end if
	If request("remind") <> "" Then
	...
	dim n
	n=0
	Set rss=conn.execute("select isnull(sum(money1),0),isnull(sum(money2),0) from (select money1,money2 from (select a.ord,a.title,a.thid,a.money1,a.complete1,a.sort,a.date3,a.sp,a.contract,a.money2,a.cateid,a.addcate,a.date7,a.cateid_sp,a.company,a.bz,a.zdy1,a.zdy2,a.zdy3,a.zdy4,a.zdy5,a.zdy6,a.intro,a.area,a.trade,a.del,a.alt,(case when isnull(b.num1,0) = 0 then 4 when isnull(b.num3,0)=0 then 1 when isnull(b.num3,0)>0 and isnull(b.num3,0)<isnull(b.num1,0) then 2  else 3 end) zt1 from contractth a WITH(NOLOCK) left join (select caigou,isnull(sum(case when NoNeedInKu=1 then a.num1 else 0 end),0) as num1,isnull(sum(a.num2),0) as num2,isnull(sum(a.num3),0) as num3 from contractthlist a WITH(NOLOCK) LEFT JOIN product p WITH(NOLOCK) ON a.ord=p.ord where (kuoutlist2<>0 or (isnull(kuoutlist2,0)=0 and isnull(a.contract,0)=0 AND ISNULL(p.canOutStore,1)=1)) and a.del=1 group by a.caigou) b on b.caigou = a.ord) x "&Str_Result&fromContent&") s ")
	If Err.number<>0 Then
		summoney2_thall_rmb=0
		summoney2_thall=0
	else
		summoney2_thall_rmb=rss(1).value
		summoney2_thall=rss(0).value
	end if
	rss.close
	dim telrss : set telrss =conn.execute("select ord, isnull(cateid,0) as cateid ,name,khid ,ISNULL(share,'-222') share , sort3 from tel WITH(NOLOCK) where del=1")
	'rss.close
	dim bzrss : set bzrss = conn.execute("select id,intro from sortbz WITH(NOLOCK)")
	dim onerss : set onerss = conn.execute("select sort1,ord from sortonehy WITH(NOLOCK) where gate2 in (35,36)")
	dim gaterss : set gaterss = conn.execute("select ord, name from gate WITH(NOLOCK)")
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from (select a.ord,a.title,a.thid,isnull(a.money1,0) money1,a.complete1,a.sort,a.date3,a.sp,a.contract,isnull(a.money2,0) money2,a.cateid,a.addcate,a.date7,a.cateid_sp,a.company,a.bz,a.zdy1,a.zdy2,a.zdy3,a.zdy4,a.zdy5,a.zdy6,a.intro,a.area,a.trade,a.del,a.alt,(case when isnull(b.num1,0) = 0 then 4 when isnull(b.num3,0)=0 then 1 when isnull(b.num3,0)>0 and isnull(b.num3,0)<isnull(b.num1,0) then 2  else 3 end) zt1,isnull(b.num1,0) thrknum1,isnull(b.num2,0) thsqnum2,isnull(b.num3,0) thyrnum3 from contractth a WITH(NOLOCK) left join (select caigou,isnull(sum(case when NoNeedInKu=1 then a.num1 else 0 end),0) as num1,isnull(sum(a.num2),0) as num2,isnull(sum(a.num3),0) as num3 from contractthlist a WITH(NOLOCK) LEFT JOIN product p WITH(NOLOCK) ON a.ord=p.ord where (kuoutlist2<>0 or (isnull(kuoutlist2,0)=0 and isnull(a.contract,0)=0 AND ISNULL(p.canOutStore,1)=1))  group by a.caigou) b on b.caigou = a.ord) x "&Str_Result&fromContent&" "&px_Result&""
	rs.open sql,conn,1,1
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "       <form name=""form1"" method=""post"" action=""delhtconfirm.asp?"
		Response.write sdk.ReturnURL("method=0")
		Response.write """>" & vbcrlf & ""
		Dim rid
		do While not rs.eof
			If remind <> "" Then
			...
			summoney1_th_rmb=summoney1_th_rmb+cdbl(getInt(zbcdbl(rs("money2"))))
			contract=rs("contract")
			summoney1_th=summoney1_th+cdbl(getInt(zbcdbl(rs("money1"))))
			contract=rs("contract")
			if rs("cateid")=0 or isnull(rs("cateid")) then
				cateid=-1
'if rs("cateid")=0 or isnull(rs("cateid")) then
			else
				cateid=rs("cateid")
			...
			candel = True
			If rs("sp") = 0 Then
				If conn.execute("select count(1) from contractthlistdetail L WITH(NOLOCK) inner join paybacklist p WITH(NOLOCK) on p.del=1 and p.contractlist<0 and abs(p.contractlist) = l.id inner join payback b WITH(NOLOCK) on b.del=1 and b.complete=3  and p.payback=b.ord where L.contractth ="&ord&" and L.thType='GOODS' and L.del=1")(0)>0 Then
					candel = False
				else
					If conn.execute("select count(1) from contractthlistdetail L WITH(NOLOCK) inner join paybackinvoice_list p WITH(NOLOCK) on p.del=1 and p.contractlist<0 and abs(p.contractlist) = l.id inner join paybackinvoice b on b.del=1 and b.isInvoiced in (1,2)  and b.id=p.paybackinvoice where L.contractth ="&ord&" and L.thType='GOODS' and L.del=1")(0)>0 Then candel = False
				end if
			end if
			...
			if open_41_3=3 or CheckPurview(intro_41_3,trim(cateid))=True then
				Response.write "<input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>"
			end if
			Response.write "</span></td>" & vbcrlf & "   <td align=""left"">" & vbcrlf & ""
			if lie_1=1 Then
				if open_41_14=3 or CheckPurview(intro_41_14,trim(cateid))=True Then
					Response.write "" & vbcrlf & "             <a href=""javascript:void(0)"" onclick=""javascript:window.open('content.asp?ord="
			...
			Response.write "" & vbcrlf & "       </td>" & vbcrlf & "   <td "
			If lie_4=1 Then
			...
			share = ""
			sort3 = 1
			telrss.filter = " ord = " & company
			If telrss.eof = False Then
				cateid_kh = telrss("cateid")
				companyTitle = telrss("name")
				companybh = telrss("khid")
				share = telrss("share")
				sort3 = telrss("sort3")
			end if
			if lie_4=1 Then
				If sort3 = "2" Then
					if open_26_1=3 or ( open_26_1 = 1 And CheckPurview(intro_26_1,trim(cateid_kh))=True  And cateid_kh<>0) Then
						if open_26_14=3 or ( open_26_14 = 1 And CheckPurview(intro_26_14,trim(cateid_kh))=True  And cateid_kh<>0) Then
							...
							Response.write "','telcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title="""
							Response.write pwurl(company)
							Response.write companyTitle
							Response.write """>"
							Response.write companyTitle
							Response.write "</a>" & vbcrlf & "                                         "
						else
			...
			elseif lie_4=2 Then
				Response.write companybh
			end if
			Response.write "" & vbcrlf & "       </td>" & vbcrlf & "   <td align=""center"">" & vbcrlf & "             "
			if lie_2=1 Then
				if open_5_21<>0 Then
					bzrss.filter = " id = " & rs("bz")
					if not bzrss.eof Then
						bzintro = bzrss("intro")
					else
						bzintro = ""
					end if
			...
			elseif lie_2=3 Then
				sortname=""
				if complete1<>"" then
					onerss.filter = " ord = " & sort
					if onerss.eof = False then
						sortname=onerss("sort1")
					end if
				else
					sortname=""
				end if
			...
			elseif lie_2=4 Then
				completename=""
				if complete1<>"" then
					onerss.filter = " ord = " & complete1
					if onerss.eof = False then
						completename=onerss("sort1")
					end if
				else
					completename=""
				end if
			...
			if lie_3=1 Then
				...
				Response.write ztStr
			elseif lie_3=2 then
				Response.write date3
			elseif lie_3=3 Then
				catename=""
				if cateid<>"" then
					gaterss.filter = " ord = " & cateid
					if gaterss.eof = False then
						catename=gaterss("name")
					else
						catename=""
					end if
				else
			...
			elseif lie_3=4 Then
				addcatename=""
				if addcate<>"" then
					gaterss.filter = " ord = " & addcate
					if gaterss.eof = False then
						addcatename=gaterss("name")
					else
						addcatename=""
					end if
				else
			...
			if open_41_14=3 or CheckPurview(intro_41_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit32"" value=""详情""  onClick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin','width=' + 1000 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "         "
				Response.write pwurl(rs("ord"))
			end if
			if session("personzbintel2007")=cateid_sp and rs("sp")<>"0" and rs("sp")<>"-1" and (open_41_16=3 or CheckPurview(intro_41_16,trim(cateid))=True) then
				Response.write pwurl(rs("ord"))
				Response.write "" & vbcrlf & "                     <input type=""button"" name=""Submit33"" value=""审批""  onClick=""javascript:if(!hwndsp||hwndsp.closed){hwndsp=window.open('../inc/commSPSet.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "&sort1=41&remind=69','neww3win','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');}else{hwndsp.focus();}""/>" & vbcrlf & "              "
				Response.write pwurl(rs("ord"))
			end if
			if ZBRuntime.MC(17000) and ZBRuntime.MC(17002) then
				if (zt3=1 Or zt3=2) And Rs("sp")&""="0" and  (open_31_13=3 or (open_31_13=1 And CheckPurview(intro_31_13,trim(cateid))=True And cateid&""<>"0" )) then
					Response.write "" & vbcrlf & "                             <input type=""button"" name=""Submit3"" value=""入库""  onClick=""javascript:if(!hwndrk||hwndrk.closed){hwndrk=window.open('addlistrk2.asp?top="
					Response.write pwurl(rs("ord"))
					Response.write "','newwin','width=' + 1000 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');}else{hwndrk.focus();}""/>" & vbcrlf & "                  "
					Response.write pwurl(rs("ord"))
				end if
			end if
			if open_41_2=3 or CheckPurview(intro_41_2,trim(cateid))=True then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submit3c"" value=""修改""  onClick=""javascript:window.open('correct.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','newwincor','width=' + 1000 + ',height=' + 700 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "              "
				Response.write pwurl(rs("ord"))
			end if
			if (open_41_3=3 or CheckPurview(intro_41_3,trim(cateid))=True) And candel = true then
				Response.write "" & vbcrlf & "             <input type=""button"" name=""Submitdel"" value=""删除"" onClick=""window.location.href='delhtconfirm.asp?"
				Response.write sdk.ReturnURL("method=0&ord=" & rs("ord") &"")
				Response.write "'""/>" & vbcrlf & "              "
			end if
		...
		if lie_2=1 then
			...
			If open_5_21<>0 then
				Response.write Formatnumber(summoney1_th,num_dot_xs,-1)
'If open_5_21<>0 then
			end if
			Response.write "</td>" & vbcrlf & "                        <td align=""right"" class=""red"">"
'If open_5_21<>0 then
			Response.write "￥"
			Response.write Formatnumber(summoney1_th_rmb,num_dot_xs,-1)
			Response.write "￥"
		end if
		...
		if  open_5_21<>0 then
			Response.write Formatnumber(summoney2_thall,num_dot_xs,-1)
'if  open_5_21<>0 then
		end if
		Response.write "</td>" & vbcrlf & "                        <td align=""right"" class=""red"">"
'if  open_5_21<>0 then
		Response.write "￥"
		Response.write Formatnumber(summoney2_thall_rmb,num_dot_xs,-1)
		Response.write "￥"
	end if
	Response.write "</td>" & vbcrlf & "                        <td colspan=""2"">&nbsp;</td>" & vbcrlf & "                 </tr>" & vbcrlf & ""
'end if
	Response.write "" & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "  </tr>" & vbcrlf & "          <tr>" & vbcrlf & "    <td  class=""page"" style=""padding:0 20px;background:#fff;"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""6%"" height=""30""><div align=""center"">全选" & vbcrlf & "       <input name=""chkall"" type=""checkbox"" id=""chkall"" value=""all"" onclick=""mm(this.form)"" />" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >"
	if open_41_3=1 or open_41_3=3 then
		Response.write "" & vbcrlf & "    " & vbcrlf & "    <input class=""resetElementHidden"" type=""image"" src=""../images/m_an_del.gif"" onClick=""return test();""  name=""submit"">" & vbcrlf & "    <input class=""resetElementShow anybutton"" value=""批量删除"" style=""display:none;"" type=""submit"" src=""../images/m_an_del.gif"" onClick=""return test();""  name=""submit"">" & vbcrlf & "    "
	end if
	Response.write "</td>" & vbcrlf & "        </form>" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "    共"
	...
	Response.write "页&nbsp;" & vbcrlf & "    <input name=""currpage"" id=""currpage"" type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "     &nbsp;<input type=""button"" name=""Submit422"" value=""跳转"" onClick=""gotourl('currPage='+document.getElementById('currpage').value);"" class=""anybutton2""/>" & vbcrlf & "     "
	if currpage=1 then
	...
	Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script src= ""../Script/ch_planall_1.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=javascript></script>" & vbcrlf & "  " & vbcrlf & "</table>" & vbcrlf & ""
'end if
	rs.close
	set rs=nothing
	Response.write "" & vbcrlf & "" & vbcrlf & "     </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "<script src= ""../Script/ch_planall_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript""></script>" & vbcrlf & "<div id=""User"" style=""position:absolute;height:400;display:none;"">" & vbcrlf & "<table width=""150"" height=""300""  border=""0"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "  <table width=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1')""><font color=""#2F496E"">按退货主题排序(降)</font></a></td>" & vbcrlf & "</tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2')""><font color=""#2F496E"">按退货主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3')""><font color=""#2F496E"">按退货编号排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4')""><font color=""#2F496E"">按退货编号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5')""><font color=""#2F496E"">按退货金额排序(降)</font></a> </td>" & vbcrlf &    "      </tr> "& vbcrlf &            "     <tr  valign=""middle""> "& vbcrlf &         "    <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourlbcrlf & ""           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7')""><font color=""#2F496E"">按退货日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "       <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8')""><font color=""#2F496E"">按退货日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "             <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###""onclick=""gotourl('px=9')""><font color=""#2F496E"">按创建日期排序(降)</font></a> </td> "& vbcrlf &    "       </tr> "& vbcrlf &         "      <tr  valign=""middle""> "& vbcrlf &        "     <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=10')""><font color=""#2F496E"">按创建日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" & vbcrlf & ""
	action1="退货列表"
	call close_list(1)
	...
	Response.write "" & vbcrlf & "</script>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "<script type=""text/javascript"">" & vbcrlf & "//验证费用文本框不允许填非数字并且不允许小于0" & vbcrlf & "function yanZheng(src) {" & vbcrlf & "    isNaN(src.value) ? src.value = 0 : src.value;" & vbcrlf & "    src.value.indexOf(""-"") > -1 ? src.value = 0 : src.value;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</html>" & vbcrlf & ""
