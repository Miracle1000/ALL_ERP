...
'bigsystemtype = ".mozi"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=2"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_2=0
		intro_33_2=0
	else
		open_33_2=rs1("qx_open")
		intro_33_2=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_3=0
		intro_33_3=0
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=5"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_5=0
		intro_33_5=0
	else
		open_33_5=rs1("qx_open")
		intro_33_5=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_7=0
		intro_33_7=0
	else
		open_33_7=rs1("qx_open")
		intro_33_7=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_8=0
		intro_33_8=0
	else
		open_33_8=rs1("qx_open")
		intro_33_8=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=10"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_10=0
		intro_33_10=0
	else
		open_33_10=rs1("qx_open")
		intro_33_10=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=13"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_13=0
		intro_33_13=0
	else
		open_33_13=rs1("qx_open")
		intro_33_13=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=33 and sort2=16"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_33_16=0
		intro_33_16=0
	else
		open_33_16=rs1("qx_open")
		intro_33_16=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	rs1.close
	set rs1=nothing
	if open_33_1=3 or Request.QueryString("fromid")="contract" then
		list=""
	elseif open_33_1=1 then
		list="and cateid in ("&intro_33_1&")"
	else
		list="and cateid=-1"
		list="and cateid in ("&intro_33_1&")"
	end if
...
'Private Sub Class_Initialize
	...
	if ksjs<>"" and ksjs2<>"" then
		...
		elseif ksjs="htid" then
			P1=1
			P2=ksjs2
		ElseIf ksjs="glht" Then
			H1=1
			H2=ksjs2
		ElseIf ksjs = "consignee" Then
			K1 = 1
			K2 = ksjs2
		ElseIf ksjs = "kdgs" Then
			I1 = 1
			I2 = ksjs2
		ElseIf ksjs = "kddh" Then
			J1 = 1
			J2 = ksjs2
		ElseIf ksjs = "shadr" Then
			A1 = 1
			Aadr = ksjs2
		end if
	end if
	...
	if SearchType = "0" then
		if len(ksjs3) > 0 and isdate(ksjs3) then
			m1 = ksjs3
		end if
		if len(ksjs4) > 0 and isdate(ksjs4) then
			m2 = ksjs4
		end if
		if len(ksjs5) > 0 then
			Prt = ksjs5
	...
	if len(A) > 0 Then Str_Result=Str_Result + " and charindex(','+cast( isnull(send.complete1,0) as varchar(10))+',',',"&Replace(A," ","")&",')>0 "
	Prt = ksjs5
	...
	W4=replace(W3,"0","")
	W4=replace(W4,",","")
	if m1<>"" then Str_Result=Str_Result + " and send.date2>='"&m1&" 00:00:00'"
	W4=replace(W4,",","")
	if m2<>"" then Str_Result=Str_Result + " and send.date2<='"&m2&" 23:59:59'"
	W4=replace(W4,",","")
	if len(Prt) > 0 then
		Str_Result=Str_Result + " and case (select count(PrinterInfo.ord) from PrinterInfo where PrinterInfo.sort=4 and PrinterInfo.formID = send.ord) when 0 then 0 else 1 end = "&Prt
'if len(Prt) > 0 then
	end if
	if W4<>"" then
		...
		case 1:
		Str_Result=Str_Result+"and  send.cateid in  ("&W3&")"
'case 1:
		case 2:
		Str_Result=Str_Result+"and  send.addcate in  ("&W3&")"
'case 2:
		case 3:
		Str_Result=Str_Result+"and  (send.cateid in  ("&W3&") or send.addcate in ("&W3&"))"
'case 3:
		case 4:
		Str_Result=Str_Result+"and  (send.cateid in  ("&W3&") and send.addcate in ("&W3&"))"
'case 4:
		end select
	end if
	area_list=-1
'end select
	...
	if A2<>"" then Str_Result=Str_Result+" and  send.area in ("&area_list&")"
	set rsarea=nothing
	if F2<>"" then
		if F1=1 then
			str_Result=str_Result+" and  send.company in (select ord from tel where name like '%"& F2 &"%')"
'if F1=1 then
		elseif F1=2 then
			str_Result=str_Result+" and send.company in (select ord from tel where name not like '%"& F2 &"%')"
'elseif F1=2 then
		elseif F1=3 then
			str_Result=str_Result+" and send.company in (select ord from tel where name='"&F2&"')"
'elseif F1=3 then
		elseif F1=4 then
			str_Result=str_Result+" and send.company in (select ord from tel where name<>'"&F2&"')"
'elseif F1=4 then
...
'elseif F1=6 then
	...
	if G2<>"" then
		if G1=1 then
			str_Result=str_Result+" and send.title like '%"& G2 &"%'"
'if G1=1 then
		elseif G1=2 then
			str_Result=str_Result+" and send.title not like '%"& G2 &"%'"
'elseif G1=2 then
		elseif G1=3 then
			str_Result=str_Result+" and send.title='"& G2 &"'"
'elseif G1=3 then
		elseif G1=4 then
			str_Result=str_Result+" and send.title<>'"& G2 &"'"
'elseif G1=4 then
...
'elseif G1=6 then
	...
	if P2<>"" then
		if P1=1 then
			str_Result=str_Result+" and send.code like '%"& P2 &"%'"
'if P1=1 then
		elseif P1=2 then
			str_Result=str_Result+" and send.code not like '%"& P2 &"%'"
'elseif P1=2 then
		elseif P1=3 then
			str_Result=str_Result+" and send.code='"& P2 &"'"
'elseif P1=3 then
		elseif P1=4 then
			str_Result=str_Result+" and send.code<>'"& P2 &"'"
'elseif P1=4 then
...
'elseif P1=6 then
	...
	if H2<>"" then
		if H1=1 Then
			str_Result=str_Result+" and send.order1 in (select ord from contract where title like '%"& H2 &"%')"
'if H1=1 Then
		end if
	end if
	if I2<>"" then
		if I1=1 Then
			str_Result=str_Result+" and send.ECID in (select ord from sortonehy where gate2=83 and del=1 and sort1 like '%"& I2 &"%')"
'if I1=1 Then
		end if
	end if
	if J2<>"" then
		if J1=1 Then
			str_Result=str_Result+" and send.WaybillNumber like '%"& J2 &"%' "
'if J1=1 Then
		end if
	end if
	if Aadr<>"" then
		if A1=1 then
			str_Result=str_Result+" and (isnull(bb.fullPathWithoutSpace,'') + isnull(aa.[address],'')) like '%"& Aadr &"%' "
'if A1=1 then
		end if
	end if
	If K2 <> "" Then
		If K1 = 1 Then
			str_Result=str_Result+" AND send.sh IN (SELECT ord FROM person WHERE name LIKE '%"& K2 &"%') "
'If K1 = 1 Then
		end if
	end if
	if zdy12<>"" then
		if zdy1=1 then
			str_Result=str_Result+" and send.zdy1 like '%"& zdy12 &"%'"
'if zdy1=1 then
		elseif zdy1=2 then
			str_Result=str_Result+" and send.zdy1 not like '%"& zdy12 &"%'"
'elseif zdy1=2 then
		elseif zdy1=3 then
			str_Result=str_Result+" and send.zdy1='"& zdy12 &"'"
'elseif zdy1=3 then
		elseif zdy1=4 then
			str_Result=str_Result+" and send.zdy1<>'"& zdy12 &"'"
'elseif zdy1=4 then
...
'elseif zdy1=6 then
	...
	if zdy22<>"" then
		if zdy1=1 then
			str_Result=str_Result+" and send.zdy2 like '%"& zdy22 &"%'"
'if zdy1=1 then
		elseif zdy2=2 then
			str_Result=str_Result+" and send.zdy2 not like '%"& zdy22 &"%'"
'elseif zdy2=2 then
		elseif zdy2=3 then
			str_Result=str_Result+" and send.zdy2='"& zdy22 &"'"
'elseif zdy2=3 then
		elseif zdy2=4 then
			str_Result=str_Result+" and send.zdy2<>'"& zdy22 &"'"
'elseif zdy2=4 then
...
'elseif zdy2=6 then
	...
	if zdy32<>"" then
		if zdy3=1 then
			str_Result=str_Result+" and send.zdy3 like '%"& zdy32 &"%'"
'if zdy3=1 then
		elseif zdy3=2 then
			str_Result=str_Result+" and send.zdy3 not like '%"& zdy32 &"%'"
'elseif zdy3=2 then
		elseif zdy3=3 then
			str_Result=str_Result+" and send.zdy3='"& zdy32 &"'"
'elseif zdy3=3 then
		elseif zdy3=4 then
			str_Result=str_Result+" and send.zdy3<>'"& zdy32 &"'"
'elseif zdy3=4 then
...
'elseif zdy3=6 then
	...
	if zdy42<>"" then
		if zdy4=1 then
			str_Result=str_Result+" and send.zdy4 like '%"& zdy42 &"%'"
'if zdy4=1 then
		elseif zdy4=2 then
			str_Result=str_Result+" and send.zdy4 not like '%"& zdy42 &"%'"
'elseif zdy4=2 then
		elseif zdy4=3 then
			str_Result=str_Result+" and send.zdy4='"& zdy42 &"'"
'elseif zdy4=3 then
		elseif zdy4=4 then
			str_Result=str_Result+" and send.zdy4<>'"& zdy42 &"'"
'elseif zdy4=4 then
...
'elseif zdy4=6 then
	...
	if zdy5<>"" then
		Str_Result=Str_Result+" and  send.zdy5 in  ("&zdy5&")"
'if zdy5<>"" then
	end if
	if zdy6<>"" then
		Str_Result=Str_Result+" and  send.zdy6 in  ("&zdy6&")"
'if zdy6<>"" then
	end if
	if a<>"" then
		If propm&""="1" Then
			If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
			Str_Result= Str_Result + " and ((send.complete1=0 and ("& open_33_16 &"=3 or ("& open_33_16 &"=1 and charindex(','+cast(send.cateid as varchar(10))+',',',"& intro_33_16 &",')>0))) or (send.complete1=1 and alt is null and send.cateid="&session("personzbintel2007")&"))"
'If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
		else
			Str_Result=Str_Result + " and charindex(','+cast( isnull(send.complete1,0) as varchar(10))+',',',"&Replace(A," ","")&",')>0 "
'If intro_33_16&""<>"" Then intro_33_16 = Replace(intro_33_16," ","")
		end if
	end if
	If Len(b)>0 Then
		If Left(Trim(b),1)="," Then b=Right(Trim(b),Len(Trim(b))-1)
'If Len(b)>0 Then
		If Right(Trim(b),1) ="," Then b = Left(Trim(b),Len(Trim(b))-1)
'If Len(b)>0 Then
	end if
	if B<>"" then Str_Result=Str_Result+" and  send.sort in  ("&B&") "
'If Len(b)>0 Then
	If remind <> "" Then
		Set helper = CreateReminderHelper(conn,remind,0)
		Str_Result=Str_Result &" and send.ord in (" & helper.listSQL("ids") & ")"
	end if
	...
	if px=1 then
		px_Result="order by code desc,date7 desc"
	elseif px=2 then
		px_Result="order by code asc,date7 asc"
	elseif px=3 then
	...
	elseif px=7 then
		px_Result="order by addcatename desc,date7 desc"
	elseif px=8 then
		px_Result="order by addcatename asc,date7 asc"
	elseif px=9 then
		px_Result="order by telname desc,date7 desc"
	elseif px=10 then
		px_Result="order by telname asc,date7 asc"
	elseif px=11 then
		px_Result="order by contractname desc,date7 desc"
	elseif px=12 then
		px_Result="order by contractname asc,date7 asc"
	elseif px=13 then
		px_Result="order by date2 desc,date7 desc"
	elseif px=14 then
		px_Result="order by date2 asc,date7 asc"
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=1 and sort2=1 and ord="&session("personzbintel2007"))
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=1 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_1_14=rssort(0)
		intro_1_14=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=26 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_26_1=rssort(0)
		intro_26_1=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=26 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_26_14=rssort(0)
		intro_26_14=rssort(1)
	else
		open_26_14=0
		intro_26_14=""
	end if
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=5 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_5_1=rssort(0)
		intro_5_1=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=5 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_5_14=rssort(0)
		intro_5_14=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=75 and sort2=1 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_75_1=rssort(0)
		intro_75_1=rssort(1)
	else
	...
	set rssort=conn.execute("select qx_open,qx_intro from power where sort1=75 and sort2=14 and ord="&session("personzbintel2007"))
	if not rssort.eof then
		open_75_14=rssort(0)
		intro_75_14=rssort(1)
	else
		open_75_14=0
		intro_75_14=""
	end if
	dim link,userid,sqlcon
	link=request("link")
	userid=trim(request("userid"))
	if link = "sent" and userid<>"" then
		sqlcon=" where b.ord =" & userid
	end if
...
'currpage=clng(currpage)
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style>" & vbcrlf & ".IE5 .top_btns input.anybutton{height:18px;line-height:17px;margin-bottom:-0.5px;}" & vbcrlf & ".echand{color:red}" & vbcrlf & "@media print {" & vbcrlf & "  .echand{display:none;}" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/st_planall.js?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write " onMouseOver=""window.status='none';return true;"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "<td width=""100%"" valign=""top"">" & vbcrlf & " <form method=""get"" action=""logisticsStatement.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date""> "& vbcrlf &"   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> "& vbcrlf &"  <tr> "& vbcrlf &"     <td class=""place"" >快递对账单 "& vbcrlf &"     "
	if b<>"" then
		set rs7=server.CreateObject("adodb.recordset")
		sql7="select sort1 from sortonehy where ord in ("&b&") "
		rs7.open sql7,conn,1,1
		if rs7.eof = false then
			sortname=rs7("sort1").value
		else
			sortname= "未知方式"
		end if
		rs7.close
	...
	Response.write "" & vbcrlf & "     </td>" & vbcrlf & "   <td>&nbsp;<a href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></td>" & vbcrlf & "   <td align=""right"">" & vbcrlf & "        <select name=""select2""  onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"">" & vbcrlf & "       <option>-请选择-</option>" & vbcrlf & "       <option value=""page_count=10"" "
	Response.write sortname
	...
	Response.write ">每页显示200条</option>" & vbcrlf & "      </select>" & vbcrlf & "       </td>" & vbcrlf & "   <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "      </tr>" & vbcrlf & "   </table>" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;"">" & vbcrlf & "    <tr>" & vbcrlf & "    <td  background=""../images/112.gif"" class='top_btns'  height=""30"" colspan=""3"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;""><div id=""kh"">" & vbcrlf & "      <input type=""hidden"" id=""SearchType"" name=""SearchType"" value=""0"" />" & vbcrlf & "     发货日期&nbsp;自：<INPUT readonly name=ksjs3 size=9  id=daysOfMonth3Pos onmouseup=""toggleDatePicker('daysOfMonth3','date.ksjs3')"" value="""
	Response.write m1
	Response.write """>" & vbcrlf & "  <DIV id=daysOfMonth3 style=""POSITION: absolute;z-index:10""></DIV>" & vbcrlf &" &nbsp;至：<INPUT name=ksjs4 readonly size=9  id=daysOfMonth4Pos onmouseup=""toggleDatePicker('daysOfMonth4','date.ksjs4')"" value="""
'Response.write m1
	Response.write m2
	Response.write """>" & vbcrlf & "  <DIV id=daysOfMonth4 style=""POSITION: absolute""></DIV>" & vbcrlf & "" & vbcrlf & "    <select name=""ksjs5"">" & vbcrlf & "     <option value="""" "
	if Prt="" then
		Response.write "selected"
	end if
	...
	Response.write ">--已打印--</option>" & vbcrlf & "  </select>" & vbcrlf & "" & vbcrlf & "       <select name=""ksjs"">" & vbcrlf & "      <option value=""htzt"" "
	Response.write "selected"
	if ksjs="htzt" then
		Response.write "selected"
	end if
	Response.write ">发货主题</option>" & vbcrlf & "    <option value=""htid"" "
	if ksjs="htid" then
		Response.write "selected"
	end if
	Response.write ">发货编号</option>" & vbcrlf & "    <option value=""khmc"" "
	if ksjs="khmc" then
		Response.write "selected"
	end if
	Response.write ">客户/供应商名称</option>" & vbcrlf & "     <option value=""kdgs"" "
	if ksjs="kdgs" then
		Response.write "selected"
	end if
	Response.write ">快递公司</option>" & vbcrlf & "    <option value=""kddh"" "
	if ksjs="kddh" then
		Response.write "selected"
	end if
	Response.write ">快递单号</option>" & vbcrlf & "    <option value=""shadr"" "
	if ksjs="shadr" then
		Response.write "selected"
	end if
	Response.write ">收货地址</option>" & vbcrlf & "    </select>" & vbcrlf & "       <input name=""ksjs2"" type=""text"" size=""15""  value="""
	Response.write sdk.htmlconvert(ksjs2)
	Response.write """/>" & vbcrlf & "        <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/><input type=""hidden"" value="""
	Response.write a
	Response.write """ name=""a"">" & vbcrlf & "  <input type=""hidden"" value="""
	Response.write b
	Response.write """ name=""b"">" & vbcrlf & "  "
	if open_33_10=1 or open_33_10=3 then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({debug:false,page:'../out/xls_fh_statement.asp'})}"" class=""anybutton""/>"
	end if
	Response.write "" & vbcrlf & "      <font id=""kh3"">"
	if open_33_7=1 or open_33_7=3 then
		Response.write "<input type=""button""  name=""print"" onclick=""window.print();"" value=""打印"" class=""anybutton""/>"
	end if
	Response.write "" & vbcrlf & "      </font></div></td>" & vbcrlf & "      </tr>" & vbcrlf & "   </table><span id=""ht1""></span>" & vbcrlf & "    </form>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr class=""top"">" & vbcrlf & "          <td width=""7%""><div align=""center"">发货时间</div></td>" & vbcrlf & "             <td width=""7%""><div align=""center"">快递公司</div></td>" & vbcrlf & "              <td width=""7%""><div align=""center"">快递单号</div></td>" & vbcrlf & "              <td width=""7%""><div align=""center"">快递费用</div></td>" & vbcrlf & "              <td width=""10%""><divalign=""center"">发货编号</div></td>" & vbcrlf & "            <td width=""15%""><div align=""center"">发货主题</div></td>" & vbcrlf & "             <td width=""13%""><div align=""center"">收货地址</div></td>" & vbcrlf & "             <td width=""12%""><div align=""center"">关联客户/供应商</div></td>" & vbcrlf & "              <td width=""12%""><div align=""center"">关联合同/采购退货</div></td>" & vbcrlf & "   </tr>" & vbcrlf & ""
	fromContract=trim(Request.QueryString("contract"))
	if fromContract<>"" Then fromContent=" and order1="&fromContract&""
	conn.execute(" declare @cnt int "&vbcLf&_
	" set @cnt = 1 "&vbcLf&_
	" select a.id,a.id1 as pid,cast(a.menuname as varchar(8000)) as fullPath,cast(a.menuname as varchar(8000)) as fullPathWithoutSpace into #area "&vbcLf&_
	" from menuarea a  with(nolock) "&vbcLf&_
	" while exists(select 1 from #area where pid<>0) and @cnt < 100 "&vbcLf&_
	" begin "&vbcLf&_
	"   update #area set fullPath = b.menuname + ' ' + fullPath , pid=b.id1,fullPathWithoutSpace = b.menuname + fullPathWithoutSpace  from menuarea b where b.id=#area.pid "&vbcLf&_
	"   set @cnt = @cnt + 1 "&vbcLf&_
	" end ")
	dim n : n=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select ord,send.company,title,cateid,complete1,code,order1,kuout,addcate,date2,alt,date1,del,"&_
	"ECID, WaybillNumber , WayMoney ,"&_
	"(select name from gate where ord=send.cateid) as cateidname, "&_
	"(select name from gate where ord=send.addcate) as addcatename, "&_
	"(select name from tel where del=1 and ord=send.company) as telname, "&_
	"(select title from contract where ord=send.order1) as contractname, "&_
	"case (select count(PrinterInfo.ord) from PrinterInfo where PrinterInfo.sort=4 and PrinterInfo.formID = send.ord) when 0 then '未打印' else '已打印' end as IsPrt, "&_
	"(select sort1 from kuout where ord=send.kuout) as sorttype, "&_
	" aa.receiver+' '+aa.mobile+' '+aa.phone+' '+isnull(bb.fullPath,'') + isnull(aa.[address],'') as AllAddress "&_
	" from send WITH(NOLOCK) "&_
	" left join DeliveryAddress aa  with(nolock) on aa.company=send.company and aa.receiver=send.Receiver and aa.phone=send.phone and aa.mobile=send.mobile and aa.address=send.address2 "&_
	" left join #area bb on aa.areaId=bb.id "&Str_Result & fromContent
	if link="sent" then
		sql = "select a.ord,company,title,cateid,complete1,code,order1,kuout,addcate,date2,alt,IsPrt,sorttype from (" & sql & ") as a inner join(select ord , ',' + replace((case qx_open when 1 then cast(qx_intro as varchar(8000)) when 0 then '0' else '' end),' ','') + ',' as qxfw from power b where b.sort1 = 33 and b.sort2=1 and ord in (select distinct cateid  from send WITH(NOLOCK) where ( len('') =0 or charindex(',' + cast(cateid as nvarchar(10)) + ',','') > 0) and del = 1 and cateid<>0 and complete1 = 0)) as b on (b.qxfw=',,' or  charindex(',' + cast(a.cateid as nvarchar(10)) + ',',b.qxfw) > 0)and a.del = 1 and a.cateid<>0 and a.complete1 = 0" & sqlcon
		sql=replace(sql,"addcate,date2","YYYYYY")
		sql=replace(sql,"date2","date1")
		sql=replace(sql,"YYYYYY","addcate,date2")
	else
		sql = sql + " "&px_Result&" "
		sql=replace(sql,"YYYYYY","addcate,date2")
	end if
	allsum = conn.execute("select isnull(sum(isnull(WayMoney,0)),0) from send WITH(NOLOCK) "&_
	" left join DeliveryAddress aa  with(nolock) on aa.company=send.company and aa.receiver=send.Receiver and aa.phone=send.phone and aa.mobile=send.mobile and aa.address=send.address "&_
	" left join #area bb on aa.areaId=bb.id "&Str_Result & fromContent )(0)
	...
	if rs.RecordCount<=0 then
		...
		do until rs.eof
			...
			WayMoney = rs("WayMoney")
			If WayMoney&""="" Then WayMoney = 0 else WayMoney=cdbl(WayMoney)
			currSum = currSum + WayMoney
			'If WayMoney&""="" Then WayMoney = 0 else WayMoney=cdbl(WayMoney)
			dim company,order1
			company=""
			order1=""
			orderord=0
			companyord=0
			if rs("order1")<>"0" and rs("order1")<>"" Then
				If sorttype="2" Then
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select ord,title,company from caigouth where ord="&rs("order1")&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						order1ord=0
					else
					...
					set rs7=server.CreateObject("adodb.recordset")
					sql7="select ord,title,company from contract where ord="&rs("order1")&""
					rs7.open sql7,conn,1,1
					if rs7.eof then
						order1ord=0
					else
			...
			if ord<>"0" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select ord,name ,sort3,isnull(cateid,0) as cateid,share from tel where ord="&ord&""
				rs7.open sql7,conn,1,1
				if rs7.eof Then
					If sorttype="2" Then
						company="关联供应商已被删除"
					...
					company=rs7("name")
					companyord=rs7("ord")
					cateid_kh = rs7("cateid")
					share = rs7("share")
				end if
				rs7.close
				set rs7=nothing
			end if
			set rs8=conn.execute("select sort1 from kuout where ord="&rs("kuout")&" ")
			if not rs8.eof then
				kuout_sort1=rs8("sort1").value
			else
				kuout_sort1=0
			end if
			set rs8=Nothing
			ecname = ""
			if ecid<>"" then
				set rs7=conn.execute("select sort1,isnull(color,'') color from sortonehy where ord="&ecid&"")
				if rs7.eof then
					ecname=""
				else
					ecname=rs7("sort1") & iif(rs7("color")&""="","<span class='echand'>（手动）</span>","")
				end if
				rs7.close
				set rs7=nothing
			end if
			Response.write "" & vbcrlf & "             <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "           <td width=""7%""><div align=""center"">"
			Response.write rs("date2")
			Response.write "</div></td>" & vbcrlf & "          <td width=""7%""><div align=""center"">"
			Response.write ecname
			Response.write "</div></td>" & vbcrlf & "          <td width=""7%""><div align=""center"">"
			Response.write WaybillNumber
			Response.write "</div></td>" & vbcrlf & "          <td width=""7%""><div align=""right"">"
			Response.write FormatNumber(WayMoney ,num_dot_xs,-1,0,-1)
			Response.write "</div></td>" & vbcrlf & "          <td width=""7%""><div align=""right"">"
			Response.write "</div></td>" & vbcrlf & "          <td width=""10%"" height=""27""><div align=""center"">"
			Response.write rs("code")
			Response.write "</div></td>" & vbcrlf & "          <td width=""15%"" class=""name"">" & vbcrlf & "                       "
			if open_33_14=3 or CheckPurview(intro_33_14,trim(cateid))=True then
				Response.write "" & vbcrlf & "                             <a href=""javascript:void(0)"" onclick=""javascript:window.open('../../SYSN/view/store/sent/Sent.ashx?view=details&ord="
				Response.write pwurl(rs("ord"))
				Response.write "','contractcon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title="""
				'Response.write pwurl(rs("ord"))
			...
			Response.write "" & vbcrlf & "                     <span style=""color:"
			If p = "未打印" Then
				Response.write "red"
			else
				Response.write "green"
			end if
			Response.write "; text-wrap:none"">["
			Response.write "green"
			Response.write p
			Response.write "]</span></td>" & vbcrlf & "                <td width=""13%""><div align=""left"">"
			Response.write rs("AllAddress")
			Response.write "</div></td>" & vbcrlf & "          <td width=""12%""><div align=""left"">" & vbcrlf & "          "
			If sort3="2" Then
				If open_26_1=3 Or (CheckPurview(intro_26_1,Trim(cateid_kh))=True And open_26_1=1) Then
					If open_26_14=3 Or (CheckPurview(intro_26_14,Trim(cateid_kh))=True And open_26_14=1) Then
						...
						Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此供应商详情"">"
						Response.write pwurl(companyord)
				...
				if open_1_1=3 or (open_1_1=1 And CheckPurview(intro_1_1,trim(cateid_kh))=True) Or share="1" Or  CheckPurview(share&"",trim(session("personzbintel2007")))=True then
					if open_1_14=3 or (CheckPurview(intro_1_14,trim(cateid_kh))=True And open_1_14=1) Then
						Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('../work/content.asp?ord="
						Response.write pwurl(companyord)
						Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此客户详情"">"
						Response.write pwurl(companyord)
						Response.write company
						Response.write "</a>"
					else
			...
			Response.write "" & vbcrlf & "             </div></td>" & vbcrlf & "             <td width=""12%""><div align=""left"">" & vbcrlf & "          "
			If sorttype="2" Then
				If open_75_1=3 Or (CheckPurview(intro_75_1,Trim(cateid))=True And open_75_1=1) Then
					If open_75_14=3 Or (CheckPurview(intro_75_14,Trim(cateid))=True And open_75_14=1) Then
						Response.write "" & vbcrlf & "                                     <a href=""javascript:void(0)"" onclick=""javascript:window.open('../caigouth/content.asp?ord="
						Response.write pwurl(order1ord)
						Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此采购退货单详情"">"
'Response.write pwurl(order1ord)
				...
				if open_5_1=3 or (CheckPurview(intro_5_1,trim(cateid))=True And open_5_1=1) then
					if open_5_14=3 or (CheckPurview(intro_5_14,trim(cateid))=True And open_5_14=1) Then
						Response.write "<a href=""javascript:void(0)"" onclick=""javascript:window.open('../../SYSN/view/sales/contract/ContractDetails.ashx?view=details&ord="
						Response.write pwurl(order1ord)
						Response.write "','newwin','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此合同详情""> "
'Response.write pwurl(order1ord)
						Response.write order1
						Response.write "</a>"
					else
		...
		loop
		m=n
		Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "   <td height=""27"" colspan=""3"" class=""name"" align=""right"">&nbsp;本页合计：</td>" & vbcrlf & "    <td align=""right"" class=""red"">"
		Response.write Formatnumber(currSum,num_dot_xs,-1 ,0, -1)
		Response.write "</td>" & vbcrlf & "        <td height=""27"" colspan=""5"" class=""name"" align=""right"">&nbsp;</td>" & vbcrlf & "      </tr>" & vbcrlf & "   <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "   <td height=""27"" colspan=""3"" class=""name"" align=""right"">&nbsp;所有合计：</td>" & vbcrlf & "     <td align=""right"" class=""red"">"
		Response.write Formatnumber(allsum,num_dot_xs,-1)
		Response.write "</td>" & vbcrlf & "        <td height=""27"" colspan=""5"" class=""name"" align=""right"">&nbsp;</td>" & vbcrlf & "      </tr>" & vbcrlf & "   </table>" & vbcrlf & "        </td>" & vbcrlf & "   </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"">" & vbcrlf & "                <tr>" & vbcrlf & "            <td width=""10%"" height=""30""></td>" & vbcrlf & "           <td></td>" & vbcrlf & "               <td width=""85%""><div align=""right"">" & vbcrlf & "         <script>" & vbcrlf & "                $(function(){" & vbcrlf & "           var currpage = $(""#currpage"");" & vbcrlf & "            currpage.bind(""keyup"",function(){" & vbcrlf & "            this.value = this.value.replace(/[^\d]/g,'')" & vbcrlf & "            });" & vbcrlf & "             });" & vbcrlf & "             </script>" & vbcrlf & "               "
		...
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "                <input id=""currpage"" name=""currpage"" type=""text"" onkeyup=""value=value.replace(/[^\d]/g,'')""  size=""3"">" & vbcrlf & "            <input type=""button"" name=""Submit422"" value=""跳转"" onclick=""gotourl('currPage='+document.getElementById('currpage').value);""  class=""anybutton2""/>" & vbcrlf & "           "
		if currpage=1 then
	...
	rs.close
	set rs=nothing
	Response.write "   " & vbcrlf & "  <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & " <tr> " & vbcrlf & "   <td height=""10"" colspan=""2""></td>" & vbcrlf & "   </tr> " & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""16%"" height=""10""><div align=""right""></div></td>" & vbcrlf & "<td width=""84%"">&nbsp;</td>" & vbcrlf & "       </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "     </tr>" & vbcrlf & "   <tr>" & vbcrlf & "    <td height=""10"" colspan=""2"">&nbsp;</td>" & vbcrlf & "     </tr>" & vbcrlf & "   </table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & "<div id=""User"" style=""position:absolute;width:100%; height:400;display:none;"">" & vbcrlf & "<table width=""200"" height=""400""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""139"">" & vbcrlf & "    <table width=""100%"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "          <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=1');""><font color=""#2F496E"">按发货编号排序(降)</font></a></td>" & vbcrlf & "</tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=2');""><font color=""#2F496E"">按发货编号排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                  <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=3');""><font color=""#2F496E"">按发货主题排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=4');""><font color=""#2F496E"">按发货主题排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=5');""><font color=""#2F496E"">按申请人排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=6');""><font color=""#2F496E"">按申请人排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "           <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=7');""><font color=""#2F496E"">按发货人排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                   <tr  valign=""middle"">" & vbcrlf & "    <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=8');""><font color=""#2F496E"">按发货人排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"" >&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=9');""><font color=""#2F496E"" style=""width:160px; cursor:pointer;"">按客户/供应商名称排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "               <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=10');""><font color=""#2F496E"" style=""width:160px; cursor:pointer;"">按客户/供应商名称排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=11');""><font color=""#2F496E"">按合同主题排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                <tr valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=12');""><font color=""#2F496E"">按合同主题排序(升)</font></a> </td>" & vbcrlf & "       </tr>" & vbcrlf & "              <tr  valign=""middle"">" & vbcrlf & "            <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=13');""><font color=""#2F496E"">按发货日期排序(降)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "                 <tr valign=""middle"">" & vbcrlf & " <td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onclick=""gotourl('px=14');""><font color=""#2F496E"">按发货日期排序(升)</font></a> </td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "    </td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</div>" &vbcrlf & ""
	action1="所有发货单"
	call close_list(1)
	...
	Response.write "" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & "<script src= ""../Script/st_planall_1.js?ver="
	Response.write Application("sys.info.jsver")
