		...
		for i=1 to num_dot_xs
			...
			dotstr_num=""
				for i=1 to num1_dot
					...
					xlApplication.ActiveSheet.Columns(4).NumberFormatLocal = "#,##0."&dotstr_num&"_ "
					...
					xlApplication.ActiveSheet.Columns(9).ColumnWidth=25
					...
					if C<>"" then
						if B = "mc" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  name like '%"& C &"%')"
'if B = "mc" then
						elseif B = "dh" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  phone like '%"& C &"%')"
'elseif B = "dh" then
						elseif B = "cz" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  fax like '%"& C &"%')"
'elseif B = "cz" then
						elseif B = "yj" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  email like '%"& C &"%')"
'elseif B = "yj" then
						elseif B = "dz" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  address like '%"& C &"%')"
'elseif B = "dz" then
						elseif B = "yb" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  zip like '%"& C &"%')"
'elseif B = "yb" then
						elseif B = "yw" then
							searchSql=searchSql+" and company in (select ord from tel where del=1 and  product like '%"& C &"%')"
'elseif B = "yw" then
						end if
					end if
					if m1<>"" then
						searchSql=searchSql+" and date1>='"&m1&"' "
'if m1<>"" then
					end if
					if m2<>"" then
						searchSql=searchSql+" and date1<='"&m2&"' "
'if m2<>"" then
					end if
					set rs1=server.CreateObject("adodb.recordset")
					sql1="select ord,isnull(price1,0) as price1,num1,num3,contract,date1,isnull(money1,0) as money1,company from contractlist where del=1 "&searchSql&" order by ord "
					rs1.open sql1,conn,1,1
					if rs1.RecordCount<=0 then
						...
						do until rs1.eof
							...
							set rs7=nothing
							set rs7=server.CreateObject("adodb.recordset")
							sql7="select ord,title,htid from contract where  ord="&rs1("contract")&" and del=1 "
							rs7.open sql7,conn,1,1
							if rs7.eof then
								title3="合同已被删除"
								k3="合同已被删除"
							...
							set rs7=nothing
							k3=k3&"["&htid&"]"
							numcg1=Cdbl(rs1("num1"))
							numcg2=Cdbl(rs1("num3"))
							numcg1m=Cdbl(rs1("price1"))
							numcg3=Cdbl(rs1("money1"))
							if numcg1<>0 then
								numcg3m=numcg3/numcg1
							else
								numcg3m=0
							end if
							if numcg3m<>0 then
							...
							date1=rs1("date1")
							set rs=server.CreateObject("adodb.recordset")
							sql="select ord,name from tel  where ord="&rs1("company")&""
							...
							set rs=nothing
							xlWorksheet.Cells(1+i,2).Value = date1
							set rs=nothing
							xlWorksheet.Cells(1+i,2).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,3).Value =k2
							set rs=nothing
							xlWorksheet.Cells(1+i,3).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,4).Value =numcg1
							set rs=nothing
							xlWorksheet.Cells(1+i,4).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,5).Value = Formatnumber(numcg1m,num_dot_xs,-1)
							set rs=nothing
							xlWorksheet.Cells(1+i,5).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,6).Value = Formatnumber(numcg3m,num_dot_xs,-1)
							set rs=nothing
							xlWorksheet.Cells(1+i,6).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,7).Value = Formatnumber(numcg2m,num_dot_xs,-1)
							set rs=nothing
							xlWorksheet.Cells(1+i,7).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,8).Value = numcg2
							set rs=nothing
							xlWorksheet.Cells(1+i,8).font.Size=10
							set rs=nothing
							xlWorksheet.Cells(1+i,9).Value = k3
							set rs=nothing
					...
					rs1.close
					set rs1=Nothing
					Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户购买明细导出导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户购买明细导出导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
					set rs1=Nothing
					Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
					Set fs = CreateObject("Scripting.FileSystemObject")
					tfile=Server.MapPath("客户购买明细导出_"&session("name2006chen")&".xls")
					...
					Response.write """><font class=""red""><strong><u>下载导出的客户购买明细导出</u></strong></font></a></p> "
