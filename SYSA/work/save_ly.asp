...
'Private Sub Class_Initialize
	...
	num1=getInt(request.form("num1"))
	num2=getInt(request.form("num2"))
	num3=getInt(request.form("num3"))
	if cdbl(num1)>cdbl(num3) then
		Response.write"<script language=javascript>alert('您输入的领用数量超过客户池可领用数量！');history.back()</script>"
		call db_close : Response.end
	end if
	num4=0
	set rs=server.CreateObject("adodb.recordset")
	sql="select num_4 from gate  where  ord="&session("personzbintel2007")&""
	rs.open sql,conn,1,1
	if not rs.eof then
		num4=rs("num_4")
		if isnull(num4) or num4="" then
			num4=0
		else
			num4=clng(num4)
		end if
	...
	if not rs1.eof then
		...
		numall=zbcdbl(rs1("num1"))
	end if
	rs1.close
	set  rs1=nothing
	if sorce_user&""="" then sorce_user = 0
	if sorce_user2&""="" then sorce_user2 = 0
	Call check_tel_applynum(session("personzbintel2007"),num1,0)
	conn.CursorLocation = 3
	conn.begintrans
	set rs=server.CreateObject("adodb.recordset")
	sql="exec tactics_tel "& session("personzbintel2007") &",0,"& num1
	rs.open sql,conn,1,1
	Count=rs.RecordCount
	Item=num1
	if Count=0 Then
		conn.rollbacktrans
		Response.write "<script>alert('客户池暂无可领用客户');history.back();</script>"
		conn.close
		set conn=nothing
		call db_close : Response.end()
	end if
	...
	do while not rs.eof
		Call salesChangeLog(rs("ord"), session("personzbintel2007"), 5, 501, "客户批量领用")
		sqltext="Update tel set order1='2' , datelast=null, cateid="&session("personzbintel2007")&",cateid2="&sorce_user&" ,cateid3="&sorce_user2&",cateorder1="&session("personzbintel2007")&" ,date2=getdate()  where ord="&rs("ord")&""
		Call helper.appendRemind(rs("ord"))
	...
	action1="客户批量领用"
	call close_list(1)
	Response.write "<script>alert('成功领用"&i&"个客户');window.location.href='telcb.asp'</script>"
