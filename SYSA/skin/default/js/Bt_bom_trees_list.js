...
bomTree.saveTree = function(id){
	if (bomTree.beforeSaveTree(id))
	{
        ...
        var mxid, SL, JHRQ, BZ, arr_mxid, arr_Pid, arr_ProductAttr1, arr_ProductAttr2;
		for (var i = 0; i < rows.length; i++)
		{
			if (mark == "") {
			...
			if (!data[treeord])
			{
                ...
                data[treeord].pid = [];		//pid
				data[treeord].includeTax = [];		//includeTax
				data[treeord].PriceXS = [];		//PriceXS
                data[treeord].ProductAttr1 = [];	
                data[treeord].ProductAttr2 = [];		
				rows[i][nnIndex] = "1";
				rows[i][stypeIndex] = "0";
            ...
            data[treeord].pid.push(String(rows[i][pidIndex]).replace(/,/g, ""));
            data[treeord].ProductAttr1.push(String(rows[i][productAttr1Index]).replace(/,/g, ""));
			data[treeord].ProductAttr2.push(String(rows[i][productAttr2Index]).replace(/,/g, ""));
		...
		for (var i in data)
		{
            ...
            arrPid.push(data[i].pid.join(","))
            arrProductAttr1.push(data[i].ProductAttr1.join(","))
			arrProductAttr2.push(data[i].ProductAttr2.join(","))
		...
		json.arrPid = arrPid.join(String.fromCharCode(1));
		json.mark = mark;
        var aj = $.ajax({
            type: 'post',
            url: '../bomlist/Bom_Trees_List.asp',
...
//                    try {
                        ...
                        for (var i = 0; i < arrmxid.length; i++) {
                            ...
                            arr_ProductAttr1 = arrProductAttr1[i].split(",");
                            arr_ProductAttr2 = arrProductAttr2[i].split(",");
                            ymxid = "-1";
                            for (var ii = 0; ii < arr_mxid.length; ii++) {
                                mxid = arr_mxid[ii]
                                //if (ymxid != mxid && mxid != "" && mxid != "0") {$("#numAttrsBatch_Attr1_" + mxid + "_" + arr_ProductAttr1[ii] + "_0_0", window.parent.opener.document).val(arr_SL[ii]);}
                                if (!jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).length) {
                                    mxid = arr_Pid[ii];
                                    if (ymxid != mxid && mxid != "" && mxid != "0") { $("input[id^='numAttrsBatch_Attr1_" + mxid + "_" + arr_ProductAttr1[ii] + "_']", window.parent.opener.document).val(arr_SL[ii]); }
                                    var subTotal = 0;
                                    var subTotalDom = $("input[id^='numAttrsBatch_Attr1_" + mxid + "']", window.parent.opener.document);
                                    if (subTotalDom.length > 0) {
                                        for (var iii = 0; iii < subTotalDom.length; iii++) {
                                            subTotal += isNaN(subTotalDom[iii].value) ? 0 : Number(subTotalDom[iii].value)
                                        }
                                        jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).val(subTotal);
                                    } else {
                                        jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).val(SL);
                                    }
                                    if (!jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).length) { continue; }
                                }
                                if (ymxid != mxid && mxid != "" && mxid != "0") {
                                    ...
                                    ProductAttr1 = arr_ProductAttr1[ii];
                                    //console.log("mxid="+mxid+"][PriceXS="+PriceXS);
                                    $("input[id^='numAttrsBatch_Attr1_" + mxid + "_" + arr_ProductAttr1[ii] + "_']", window.parent.opener.document).val(SL);
                                    if (!isNaN(SL) && Number(SL) > 0) { slSum += Number(SL); }
                                    var subTotalDom = $("input[id^='numAttrsBatch_Attr1_" + mxid + "']", window.parent.opener.document);
                                    var subTotal = 0;
                                    if (subTotalDom.length > 0) {
                                        for (var iii = 0; iii < subTotalDom.length; iii++) {
                                        ...
                                        if (subTotal == 0) { jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).val(SL) }
                                        else { jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).val(subTotal); }
                                    } else {
                                        jQuery("input[name='num1_" + mxid + "']", window.parent.opener.document).val(SL);
                                    }
									if(treeType+""=="2"){
