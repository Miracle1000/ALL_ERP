...
'bigsystemtype = ".mozi"
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=9 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_9_8=0
		intro_9_8=0
	else
		open_9_8=rs1("qx_open")
		intro_9_8=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	...
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=9 and sort2=16"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_9_16=0
		intro_9_16=0
	else
		open_9_16=rs1("qx_open")
		intro_9_16=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
...
'dim sqlstr
	...
	If request("__msgid") = "pview" Then
		'Response.clear
		Response.write "" & vbcrlf & "<meta http-equiv=""Content-Type"" content=""text/html; charset=UTF-8"">" & vbcrlf & "<body onload='ff()'><div style='visibility:hidden'>" & vbcrlf & "     <script src= ""../Script/ce_ProcModelsAdd.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """ ></script>" & vbcrlf & "       </div>" & vbcrlf & "</body>" & vbcrlf & "</html>"
		conn.close
		Response.end
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<script src= ""../Script/ce_ProcModelsAdd_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""JavaScript""></script>" & vbcrlf & "<script language=""JavaScript"" type=""text/JavaScript"">" & vbcrlf & "<!--" & vbcrlf & "function MM_jumpMenu(targ,selObj,restore){ //v3.0" & vbcrlf & "eval(targ+"".location=\'""+selObj.options[selObj.selectedIndex].value+""\'"");" & vbcrlf &" if (restore) selObj.selectedIndex=0; "& vbcrlf &" }" & vbcrlf &" //--> "& vbcrlf & vbcrlf &" //去子页面写入子页面名字？值 "& vbcrlf &" function editFields(obj) { "& vbcrlf &"    var valueDates = document.getElementsByName(""jh""); "& vbcrlf &"         var mpp=""""; "& vbcrlf &"        var q1=""""; "& vbcrlf &"         var q2="""";         "& vbcrlf &"         for(i=0;i<valueDates.length;i++)   "& vbcrlf &"       { "& vbcrlf & "               mppLim=""""; "& vbcrlf &"                 if (valueDates[i].checked){ "& vbcrlf &"                      mpp = document.getElementById(""nextid""+valueDates[i].value).value; "& vbcrlf &"                         if (mppLim==""""){ "& vbcrlf &"                           mppLim=mpp;"& "                 }" & vbcrlf & "                       q1=valueDates[i].value+"":""+mppLim+"";"";" & vbcrlf & "                      q2=q2+q1;" & vbcrlf & "               }" & vbcrlf & "       } " & vbcrlf & "" & vbcrlf & "      document.getElementById(""q2"").value=q2.substring(0,q2.length-1);" & vbcrlf & "  var ords=obj.getAttribute(""dataid"");" & vbcrlf & "      window.open(""ProcMNURL.asp?mbID="
	Response.write request("mbID")
	Response.write "&dataid="" +ords,""jdSet22"",""width=800,height=600,fullscreen=no,toolbar=0,resizable=1,left=150,top=50,scrollbars=yes"");" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "function congtian(){" & vbcrlf & "     var valueDates = document.getElementsByName(""jh"");" & vbcrlf & "        for(i=0;i<valueDates.length;i++)  " & vbcrlf & "        {" & vbcrlf & "               var c=valueDates[i].value;" & vbcrlf & "              if (valueDates[i].checked){" & vbcrlf & "             document.getElementById(""execorder""+c).value="""";" & vbcrlf & "            document.getElementById(""timeproject""+c).value="""";" & vbcrlf & "          document.getElementById(""budgetmoney""+c).value=""""; "& vbcrlf &"                document.getElementById(""executors""+c).value=""""; "& vbcrlf &"             document.getElementById(""actors""+c).value=""""; "& vbcrlf &"                document.getElementById(""intro""+c).value=""""; "& vbcrlf &"                 document.getElementById(""mustat""+c).value=document.getElementById(""mustat""+c).defaultValue;" & vbcrlf & "           document.getElementById(""allOKModel""+c).value=document.getElementById(""allOKModel""+c).defaultValue;" & vbcrlf & "         document.getElementById(""commFields""+c).value="""";" & vbcrlf & "           document.getElementById(""linkFields""+c).value="""";" & vbcrlf & "           document.getElementById(""zdyFields""+c).value="""";" & vbcrlf & "            document.getElementById(""splinktype""+c).value="""";" & vbcrlf & "           document.getElementById(""nextid""+c).value=document.getElementById(""nextid""+c).defaultValue;" & vbcrlf & "         document.getElementById(""NodeMEX_""+c).innerHTML="""";" & vbcrlf & "           }" & vbcrlf & "       }  " & vbcrlf & "}  " & vbcrlf & "</script>" & vbcrlf & "<style>" & vbcrlf & ".zb-button{display:inline-block;height:20px; color: #486D9E;font-size:12px; margin:0 3px; padding:0 5px; display:inline-block; overflow:visible; line-height:20px; *line-height:15px;}" & vbcrlf & ".top.billgrouptitlebar td{" & vbcrlf & "     padding-left:48px;" & vbcrlf & "      line-height:34px;" & vbcrlf & "}" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & ""
	Response.write request("mbID")
	dim clo
	clo=request("clo")
	mbID=request("mbID")
	act=request("act")
	type1=1
	if act="update" or act="openuCopy" Then
		set rs=server.CreateObject("adodb.recordset")
		sql="select *  from ProcModels where id='"&mbID&"'"
		rs.open sql,conn,1,1
		IF not rs.eof then
			type1=rs("type")
			name1=rs("name")
		...
		set rs=nothing
		set rs=server.CreateObject("adodb.recordset")
		sql="select sortid  from ProcModelsNodes where chancePMid='"&mbID&"'"
		rs.open sql,conn,1,1
		IF not rs.eof then
			do until rs.eof
				chancePMidv=rs("sortid")&","&chancePMidv
				rs.movenext
			loop
		else
			chancePMidv=0
		...
		rs.close
		set rs=nothing
		sortidx=left(chancePMidv,len(chancePMidv)-1)
		set rs=nothing
		onld="showsortjh2('jh');"
	end if
	Select Case act
	...
	titlename = "流程模板添加"
	End Select
	Response.write "" & vbcrlf & "<body  onunload=""window.opener.location.reload();"" onLoad="""
	Response.write onld
	Response.write """>" & vbcrlf & ""
	if clo<>"" then
		Response.write "" & vbcrlf & "<script language='javascript'>window.close();</script>" & vbcrlf & ""
	end if
	Response.write "" & vbcrlf & "  <form method=""post"" id=""demo"" onsubmit=""return false;"" name=""date"">" & vbcrlf & "   <input type=hidden name=""q2"" id=""q2"">" & vbcrlf & "   <input type=hidden name=""ordsv"" id=""ordsv"">" & vbcrlf & "   " & vbcrlf & "<table width=""100%""  border=""0""  cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1""> "& vbcrlf & "  <tr> "& vbcrlf &"     <td width=""100%"" valign=""top""> "& vbcrlf & vbcrlf &        "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif""> "& vbcrlf &"           <tr> "& vbcrlf &" <td class=""place"">"
	Response.write titlename
	Response.write "</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">" & vbcrlf & "                  &nbsp;" & vbcrlf & "                  </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "" & vbcrlf & " " & vbcrlf & "" & vbcrlf & "  <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"" style='table-layout:fixed'>" & vbcrlf & "   <input type=hidden name=""act"" id=""act""  value="""
	Response.write titlename
	Response.write act
	Response.write """>" & vbcrlf & "   <input type=hidden name=""mbID"" id=""mbID""  value="""
	Response.write mbID
	Response.write """>" & vbcrlf & " <col style='width:15%'><col style='width:35%'>" & vbcrlf & "  <col style='width:15%'><col style='width:35%'>" & vbcrlf & "  <tr class=top>" & vbcrlf & "  <td colspan=4>" & vbcrlf & "          <div style='float:right'>" & vbcrlf & "                               <input type=""button"" name=""Submit422"" value=""保存""  class=""zb-button"" onClick=""ddd(1)"" id基本信息 "& vbcrlf &"                 <input type=""hidden"" name=""jhlist"" value='jhlist' id=""jhlist""> "& vbcrlf &"         </td> "& vbcrlf &"    </tr> "& vbcrlf &"     <tr> "& vbcrlf &"       <td height='32'><div align=""right"">模板名称：</div></td> "& vbcrlf &"       <td colspan=3><div align=""left""><input type=""text""name=""title"" id=""title"" value='"
	Response.write name1
	Response.write "'   size=""30"" class=""text""  dataType=""Limit"" min=""1"" max=""50""  msg=""长度在1至50个字之间"">" & vbcrlf & "        <span class=""red"">*</span></div></td>" & vbcrlf & "     </tr><tr>" & vbcrlf & "      <td  height='32'><div align=""right"" >模板类型：</div></td>" & vbcrlf & "      <td colspan=3><div align=""left""><select name=""type1"" id=""type1"" style=""font-size:12px;"">" & vbcrlf & "        <option value=""1"" "
	Response.write name1
	IF type1 = 1 Then:response.Write " selected":End IF
	Response.write " >主模板</option>" & vbcrlf & "        <option value=""0"""
	IF type1 = 0 Then:response.Write " selected":End IF
	Response.write ">副模板</option>" & vbcrlf & "      </select></div></td>" & vbcrlf & "    </tr>  " & vbcrlf & "    <tr>" & vbcrlf & "     <td  height='32'><div align=""right"">是否启用：</div></td>" & vbcrlf & "      <td colspan=3>"
	IF Flag = 1 or Flag="" Then
		bu = ""
		lu="checked"
	ELSEIF  Flag = 0 Then
		lu = ""
	...
	Response.write "<input type=""radio"" name=""Flag"" id=""Flag"" value=""1"" "
	Response.write lu
	Response.write "> 启用&nbsp;&nbsp;<input type=""radio"" name=""Flag"" id=""Flag"" value=""0"" "
	Response.write bu
	Response.write "> 停用</td>" & vbcrlf & "    </tr><tr>" & vbcrlf & "       <td height='32'><div align=""right"">重要指数：</div></td>" & vbcrlf & "      <td colspan=3><input type=""text"" name=""gate1"" id=""gate1"" value="""
	Response.write iif(Len(gate1&"")=0,1,gate1)
	Response.write """ size=""6"" maxlength=""4"" onfocus=""this.select();"" onkeyup=""value=value.replace(/[^\d]/g,'');"" dataType=""Number"" min=""1"" max=""9999""></td>" & vbcrlf & "    </tr>" & vbcrlf & "       <tr class=top>" & vbcrlf & "  <td colspan=4>" & vbcrlf & "          <div style='float:right'>" & vbcrlf & "                       <input type=""button"" name=""Submit32"" value=""添加""  class=""zb-button"" onClick=""document.getElementById('w').style.display='block';$('#w').window('open');""/>" & vbcrlf & "                       <input type=""button"" name=""Submit32"" value=""重置""  title='重置会清除项目阶段的执行人、协作人、执行顺序等信息，重置后需要点击保存才能生效。' class=""zb-button"" onClick=""congtian();""/>" & vbcrlf & "                       <input type=""button"" value=""预览"" onClick=""FnPview('"
	Response.write mbID
	Response.write "')"" class=""zb-button"" name=""B2"" />" & vbcrlf & "            </div>" & vbcrlf & "          阶段设置" & vbcrlf & "        </td>" & vbcrlf & "   </tr>" & vbcrlf & "" & vbcrlf & "    <tr>" & vbcrlf & "       <td colspan=""4"" style='padding:4px'>" & vbcrlf & "<table width=""100%"" border=""0""  cellpadding=""4"" cellspacing=""1"" bgcolor=""#c0ccdd""> " & vbcrlf & "<tr class='top' style='font-weight:normal'>" & vbcrlf & "<td height='30' width=""25%"" style='text-align:center;font-weight:normal'>阶段名称</td>" & vbcrlf & "<td height='30' width=""10%"" style='text-align:center;font-weight:normal'>阶段类型</td>" & vbcrlf & "<td height='30' width=""30%"" style='text-align:center;font-weight:normal'>执行人</td>" & vbcrlf & "<td height='30' width=""30%"" style='text-align:center;font-weight:normal'>操作</td></tr>" & vbcrlf & ""
	Response.write mbID
	conn.cursorlocation =  3
	Set rs=conn.execute("select a.id1,case when b.[name] is null then a.sort1 else b.[name] end as sort1,a.ord,gate1,flag,b.execorder from sortjh2 a left join (select DISTINCT [name],sortid,execorder from ProcModelsNodes where chancepmid='"&mbID&"') b on a.ord=b.[sortid] where a.id1=0 order by b.execorder desc, a.gate1 desc")
	do while rs.eof = False
		...
		rs.moveprevious
		Response.write "" & vbcrlf & " <tr style=""display:none"" id="""
		Response.write rsord
		Response.write """> " & vbcrlf & "    <td height=""27"" align=""center"" valign=""middle""><div align=""left"">"
		Response.write rs("sort1")
		Response.write "</div></td>" & vbcrlf & ""
		jdtype = 1
		IF act="update" or act="openuCopy" Then
			Set rs1 = server.CreateObject("adodb.recordset")
			sql1 = "select * from ProcModelsNodes where chancePMid='"&mbID&"' and sortid='"&rsord&"'"
			rs1.Open sql1, conn, 1, 1
			If Not rs1.EOF Then
				jdtype = rs1("jdtype")
				execorder = rs1("execorder")
				timeproject = rs1("timeproject")
				budgetmoney =zbcdbl( rs1("budgetmoney"))
				executors = rs1("executors")
				actors = rs1("actors")
				intro = rs1("intro")
				mustat = rs1("mustat")
				allOKModel = rs1("allOKModel")
				linkFields = rs1("linkFields")
				commFields = rs1("commFields")
				zdyFields = rs1("zdyFields")
				splinktype = rs1("splinktype")
				ids = rs1("id")
			else
				jdtype =""
				execorder = ""
			...
			sql="select sortid from ProcModelsNodes where id in (select nextid from ProcNextNodes where nodeid in (select id from ProcModelsNodes where chancePMid='"&mbID&"' and id='"&ids&"'))"
...
'loop
			...
			Set rscld = conn.execute("select ord from sortjh2 x where x.id1=" & rsord)
			If rscld.eof = False then
				sortidm = Replace(rscld.GetString & "", Chr(13), ",")
				sortidm = Replace(Replace(sortidm & "A", ",A", ""), "A", "")
			else
				sortidm = rsordnext
			end if
			rscld.close
		end if
		Response.write "" & vbcrlf & "<td height=""27"" align=""center"" valign=""middle""><div align=""center"" id='NodeType_"
		Response.write rsord
		Response.write "'>"
		If jdtype & "" = "0" Then
			Response.write "审核"
		...
		Response.write "</div></td>" & vbcrlf & "  <td height=""27"" align=""center"" valign=""middle""><div align=""center"" id='NodeMEX_"
		Response.write rsord
		Response.write "'>"
		If Len(executors & "") > 0 Then
			Set rsss =  conn.execute("select name from gate where ord in (" & executors & ")")
			While rsss.eof = False
				Response.write rsss(0).value & " "
				rsss.movenext
			wend
			rsss.close
		end if
		Response.write "</div></td>" & vbcrlf & "    <td align=""center"" valign=""middle"" nowrap><div align=""center""  id=""setboj"">&nbsp;<a href=""javascript:void(0)"" onclick='editFields(this)' dataid='"
		Response.write rsord
		Response.write "'>阶段工作</a>&nbsp;<a href=""javascript:void(0)"" onClick='if(confirm(""删除节点会影响后面的阶段工作，确认删除？"")==true){document.getElementById(""jdtype"
		Response.write rsord
		Response.write """).value="""";document.getElementById("""
		Response.write rsord
		Response.write """).style.display=""none"";var a_Values = document.all[""a_"
		Response.write rsord
		Response.write """]; if(a_Values.length){ for(var i=0;i<a_Values.length;i++) { a_Values[i].click(); }}else{a_Values.click();};var code_Values = document.all[""jh"
		Response.write rsord
		Response.write rsord
		Response.write """]; if(code_Values.length){ for(var i=0;i<code_Values.length;i++) { code_Values[i].checked = false; }}else{code_Values.checked = false;};document.getElementById(""v_"
		Response.write rsord
		Response.write rsord
		Response.write """).style.display=""none"";}' id=""a_"
		Response.write rsord
		Response.write """>删除</a>" & vbcrlf & "" & vbcrlf & ""
		If intro <> "" Then intro = Replace(intro,"""","&quot;")
		Response.write "" & vbcrlf & "<input " & vbcrlf & "value="""
		Response.write jdtype
		Response.write """ type=""hidden"" id='jdtype"
		Response.write rsord
		Response.write "' name=""jdtype"
		Response.write rsord
		Response.write """  class=""jdty""><input " & vbcrlf & "value="""
		Response.write execorder
		Response.write """ type='hidden' id='execorder"
		Response.write rsord
		Response.write "' name=""execorder"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write timeproject
		Response.write """ type='hidden' id='timeproject"
		Response.write rsord
		Response.write "' name=""timeproject"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write budgetmoney
		Response.write """ type='hidden' id='budgetmoney"
		Response.write rsord
		Response.write "' name=""budgetmoney"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write executors
		Response.write """ type='hidden' id='executors"
		Response.write rsord
		Response.write "' name=""executors"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write actors
		Response.write """ type='hidden' id='actors"
		Response.write rsord
		Response.write "' name=""actors"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write sdk.HtmlConvert(intro)
		Response.write """ type='hidden' id='intro"
		Response.write rsord
		Response.write "' name=""intro"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write sortidm
		Response.write """ type='hidden' id='nextid"
		Response.write rsord
		Response.write "' name=""nextid"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write mustat
		Response.write """ type='hidden' id='mustat"
		Response.write rsord
		Response.write "' name=""mustat"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write allOKModel
		Response.write """ type='hidden' id='allOKModel"
		Response.write rsord
		Response.write "' name=""allOKModel"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write commFields
		Response.write """ type='hidden' id='commFields"
		Response.write rsord
		Response.write "' name=""commFields"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write linkFields
		Response.write """ type='hidden' id='linkFields"
		Response.write rsord
		Response.write "' name=""linkFields"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write zdyFields
		Response.write """ type='hidden' id='zdyFields"
		Response.write rsord
		Response.write "' name=""zdyFields"
		Response.write rsord
		Response.write """><input " & vbcrlf & "value="""
		Response.write splinktype
		Response.write """ type='hidden' id='splinktype"
		Response.write rsord
		Response.write "' name=""splinktype"
		Response.write rsord
		Response.write """><input " & vbcrlf & "type='hidden' id='id1"
		Response.write rsord
		Response.write "' name=""id1"
		Response.write rsord
		Response.write """ value="""
		Response.write rs("id1")
		Response.write """><input " & vbcrlf & "type='hidden' id='sort1"
		Response.write rsord
		Response.write "' name=""sort1"
		Response.write rsord
		Response.write """ value="""
		Response.write rs("sort1")
		Response.write """><input " & vbcrlf & "type='hidden' id='ids"
		Response.write rsord
		Response.write "' name=""ids"
		Response.write rsord
		Response.write """ value="""
		Response.write ids
		Response.write """>      " & vbcrlf & "" & vbcrlf & "    </div></td>" & vbcrlf & "    </tr>" & vbcrlf & "" & vbcrlf & "  "
		Set rsv=conn.execute("select a.id1,case when b.[name] is null then a.sort1 else b.[name] end as sort1,a.ord,gate1,flag,b.execorder from sortjh2 a left join (select DISTINCT [name],sortid,execorder from ProcModelsNodes where chancepmid='"&mbID&"') b on a.ord=b.[sortid] where a.id1='"&rsord&"' order by b.execorder desc, a.gate1 desc")
		do while rsv.eof = False
			rsvord=rsv("ord")
			IF act="update" or act="openuCopy" Then
				Set rs1 = server.CreateObject("adodb.recordset")
				sql1 = "select * from ProcModelsNodes where chancePMid='"&mbID&"' and sortid='"&rsvord&"'"
				rs1.Open sql1, conn, 1, 1
				If Not rs1.EOF Then
					jdtype = rs1("jdtype")
					execorder = rs1("execorder")
...
'loop
				...
				set rss=nothing
				sortidxv=ssv
				if sortidxv = "" then sortidxv = "0"
			else
				sortidxv = rsordnext
			end if
			Response.write "" & vbcrlf & "     <tr style=""display:none"" id="""
			Response.write rsvord
			Response.write """> " & vbcrlf & "        <td width=""25%"" height=""27"" align=""center"" valign=""middle"" style='padding-left:40px'><div align=""left"">"
			Response.write rsvord
			Response.write rsv("sort1")
			Response.write "</div></td>" & vbcrlf & "          <td height=""27"" align=""center"" valign=""middle""><div align=""center"" id='NodeType_"
			Response.write rsvord
			Response.write "'>"
			If jdtype & "" = "0" Then
				Response.write "审核"
			...
			Response.write "</div></td>" & vbcrlf & "        <td width=""25%"" align=""center"" valign=""middle"" nowrap><div align=""center"" id=""setboj1"">&nbsp;<a href=""javascript:void(0)"" onclick='editFields(this)' dataid='"
			Response.write rsvord
			Response.write "'>阶段工作</a>&nbsp;<a href=""javascript:void(0)"" onClick='var code_Values = document.all[""jh"
			Response.write rsord
			Response.write """]; if(code_Values.length){for(var i=0;i<code_Values.length;i++) {if(code_Values[i].value == """
			Response.write rsord
			Response.write rsvord
			Response.write """){code_Values[i].checked = false;}}};document.getElementById(""jdtype"
			Response.write rsvord
			Response.write """).value="""";document.getElementById("""
			Response.write rsvord
			Response.write """).style.display=""none"";' id=""a_"
			Response.write rsord
			Response.write """>删除</a>" & vbcrlf & "<input " & vbcrlf & "value="""
			Response.write jdtype
			Response.write """ type='hidden' id='jdtype"
			Response.write rsvord
			Response.write "' name=""jdtype"
	...
	rs.close
	set rs=nothing
	Response.write " " & vbcrlf & "</table>" & vbcrlf & "  " & vbcrlf & "       </td>" & vbcrlf & "      </tr>    " & vbcrlf & "    " & vbcrlf & "    <tr>" & vbcrlf & "        <td colspan=""4"" height='32'><div align=""center"">" & vbcrlf & "            <input type=""button"" name=""Submit42"" value=""保存""  class=""zb-button"" onClick=""ddd(1)"" id='dsce'/>" & vbcrlf & "                     <input type=""button"" name=""Submit422"" value=""增加""  class=""zb-button"" onClick=""ddd(2)"" id='dsce'/>" & vbcrlf & "            <input type=""reset"" value=""重填"" class=""zb-button"" name=""B2"" />" & vbcrlf & "        </div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & " </td>" & vbcrlf & "  </tr>" & vbcrlf & "  <tr>" & vbcrlf & "  <td  class=""page"">" & vbcrlf & "   <table width=""100%"" border=""0"" align=""left"" ><tr>" & vbcrlf & "    <td height=""80"" ><div align=""center""></div></td>" & vbcrlf & "    </tr>"& vbcrlf &" </table> "& vbcrlf &" </td></tr></table> "& vbcrlf & vbcrlf & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""阶段选择""  collapsible=""false"" style=""display:none;width:540px;height:500px;padding:5px;background: #fafafa;top:60px; background-image:../images/contenttop.gif""  closed=""true"" >" & vbcrlf & "<div region=""center"" border=""false"" style=""padding:10px;background:#fff;border:1px solid #ccc;"">" & vbcrlf & "<div align=""left""><input type=""radio"" name=""sortjhr"" id=""sortjhr"" value=""1"" onClick='check(document.getElementsByName(""jh""));document.getElementById(""divsortjhr"").style.display=""""' checked><b>选择以下阶段</b>（在需要选择的阶段前面的方框打勾）</div>" & vbcrlf & "<div id=""divsortjhr"" align=""left"" style='height:350px;overflow:auto'>" & vbcrlf & ""
	sortidxU = Split(sortidx, ",")
	Set rs=conn.execute("select ord,sort1,gate1 from sortjh2 where id1=0 and flag=1 order by gate1 desc")
	do while rs.eof = False
		ckd2 = ""
		For s = 0 To UBound(sortidxU)
			If int(sortidxU(s)) = int(rs("ord")) Then
				ckd2 = "checked"
				exit for
			end if
		next
		Response.write "" & vbcrlf & "<table border=""0"" cellpadding=""0"" cellspacing=""0""> " & vbcrlf & " <tr> " & vbcrlf & "    <td height=""20"" align=""center"" valign=""middle""><div align=""left""><input onClick='if(this.checked==true){ document.getElementById(""v_"
		Response.write rs("ord")
		Response.write """).style.display=""block""; }else{ document.getElementById(""v_"
		Response.write rs("ord")
		Response.write """).style.display=""none""; var code_Values = document.all[""jh"
		Response.write rs("ord")
		Response.write """]; if(code_Values.length){ for(var i=0;i<code_Values.length;i++) { code_Values[i].checked = false; }}else{code_Values.checked = false;}}' name=""jh"" type=""checkbox"" id=""jh"
		Response.write rs("ord")
		Response.write rs("ord")
		Response.write """ value="""
		Response.write rs("ord")
		...
		Response.write ">"
		Response.write rs("sort1")
		Response.write "</div></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>  " & vbcrlf & "        <table border=""0"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "     <tr> <td align=""center"" valign=""middle"" style=""display:none;"" id=""v_"
		Response.write rs("ord")
		Response.write """><div align=""left"">" & vbcrlf & "         "
		Set rsv=conn.execute("select ord,id1,sort1,gate1,id1 Flag from sortjh2 where id1='"&rs("ord")&"'  and flag=1 order by gate1 desc")
		do while rsv.eof = False
			ckd3=""
			For s = 0 To UBound(sortidxU)
				If int(sortidxU(s)) = int(rsv("ord")) Then
					ckd3 = "checked"
					exit for
				end if
			next
			Response.write "" & vbcrlf & "        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=""checkbox"" id=""jh"
			Response.write rs("ord")
			Response.write """ name=""jh"" value="""
			Response.write rsv("ord")
			Response.write """ "
			Response.write ckd3
			Response.write ">"
			Response.write rsv("sort1")
		...
		rsv.close
		set rsv=nothing
		Response.write "  " & vbcrlf & "      </div></td>  </tr>" & vbcrlf & "   </table>" & vbcrlf & "    " & vbcrlf & ""
		rs.movenext
	Loop
	rs.close
	set rs=nothing
	Response.write " " & vbcrlf & "</div>" & vbcrlf & "<div align=""left""><input type=""radio"" name=""sortjhr"" id=""sortjhr"" value=""0""  onClick='uncheck(document.getElementsByName(""jh""));document.getElementById(""divsortjhr"").style.display=""none""'><b>选择全部阶段</b></div>" & vbcrlf & "                 </div>" & vbcrlf & "<div region=""south"" border=""false"" style=""text-align:right;height:27px;line-height:27px;padding-right:20px;padding-top:5px""> "& vbcrlf &"                           <input type=""button"" class=""page"" value=""确认"" onClick=""showsortjh2('jh');"">"  & vbcrlf & "                           <input type=""button"" class=""page"" value=""重填"" onClick='check(document.getElementsByName(""jh""));'>" & vbcrlf & "                    </div>" & vbcrlf & "  </div>" & vbcrlf & ""
	conn.close
	set conn=Nothing
	If session("procmsaveok") = "1" Then
		session("procmsaveok") = ""
		Response.write "<script>setTimeout(""alert('操作成功!')"",500);</script>"
	end if
	Response.write "" & vbcrlf & "</form><iframe src='' name='aaax' style='position:absolute;left:-100px;width:1px;height:1px;'></frame>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	Response.write "<script>setTimeout(""alert('操作成功!')"",500);</script>"
