...
'elseif B="thry" then
			...
			str_Result11 = str_Result11 & " and (" & open_9_10 & "=3 or (" & open_9_10 & "=1 and a.cateid in (" & intro_9_10 & ")))"
			...
			"(select name from tel where sort3=1 and del=1 and ord=b.company) as name,"&_
			"(select intro from sortbz where id=b.bz) as sortbz,"&_
			...
			"(select name from gate where del=1 and ord in (select max(cateid) from contractth where company=b.company and bz=b.bz)) as cateidname "&_
			"from payout2 as a left join contractth as b on a.contractth=b.ord "&_
			"where a.del=1 "&Str_Result11&" and a.date1>='"&m1&"' and a.date1<='"&m2&" 23:59:59' group by b.company,b.bz "&px_Result&""
			set rs=server.CreateObject("adodb.recordset")
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "退款汇总表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			if rs.RecordCount<=0 then
				...
				do until rs.eof
					...
					if rs("company")<>"" then
						...
						sql7="select ord,name,isnull(cateid,0) as cateid, ISNULL(share,'-222') share ,sort3, del from tel where ord="& rs("company") &" "
						set rs7=server.CreateObject("adodb.recordset")
...
'if cateid="" or cateid=0 then
					...
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,2).Value = rs("sortbz")&Formatnumber(money1,num_dot_xs,-1)
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,2).font.Size=10
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,3).Value = rs("sortbz")&Formatnumber(money2,num_dot_xs,-1)
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,3).font.Size=10
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,4).Value = rs("sortbz")&Formatnumber(money3,num_dot_xs,-1)
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,4).font.Size=10
					money3=zbcdbl(rs("money3"))
					xlWorksheet.Cells(1+i,5).Value = rs("cateidname")
					money3=zbcdbl(rs("money3"))
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>退款汇总表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!退款汇总表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("退款汇总表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的退款汇总表</u></strong></font></a></p>"
