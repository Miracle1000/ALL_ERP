...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlWorksheet.Cells(1,2).Value = "预付日期"
			xlWorksheet.Cells(1,2).font.Size=10
			xlWorksheet.Cells(1,2).font.bold=true
			xlWorksheet.Cells(1,3).Value = "预付金额"
			xlWorksheet.Cells(1,3).font.Size=10
			xlWorksheet.Cells(1,3).font.bold=true
			xlWorksheet.Cells(1,4).Value = "出账账户"
			xlWorksheet.Cells(1,4).font.Size=10
			xlWorksheet.Cells(1,4).font.bold=true
			Dim pos : pos = 4
			If ZBRuntime.MC(223100) Then
				xlWorksheet.Cells(1,5).Value = "审批状态"
				xlWorksheet.Cells(1,5).font.Size=10
				xlWorksheet.Cells(1,5).font.bold=true
				pos = 5
			End if
			xlWorksheet.Cells(1,pos+1).Value = "经手人"
'End  if
			xlWorksheet.Cells(1,pos+1).font.Size=10
'End  if
			xlWorksheet.Cells(1,pos+1).font.bold=True
'End  if
			xlWorksheet.Cells(1,pos+2).Value = "是否收票"
'End  if
			xlWorksheet.Cells(1,pos+2).font.Size=10
'End  if
			xlWorksheet.Cells(1,pos+2).font.bold=true
'End  if
			xlWorksheet.Cells(1,pos+3).Value = "预付概要"
'End  if
			xlWorksheet.Cells(1,pos+3).font.Size=10
'End  if
			xlWorksheet.Cells(1,pos+3).font.bold=true
'End  if
			xlWorksheet.Cells(1,pos+4).Value = "添加时间"
'End  if
			xlWorksheet.Cells(1,pos+4).font.Size=10
'End  if
			xlWorksheet.Cells(1,pos+4).font.bold=true
'End  if
			...
			companyid=deurl(request("company"))
			If open_8_10 =3 Then
				Str_Result = Str_Result &" "
			ElseIf  open_8_10 =1 Then
				Str_Result = Str_Result & " and cateid in ("& intro_8_10 &") and cateid>0 "
			else
...
'if m2<>"" then
			...
			If status_sp<>"" And status_sp<>10 Then
				Str_Result=Str_Result+"and  isnull(status_sp,1)="&status_sp&""
'If status_sp<>"" And status_sp<>10 Then
			end if
			If remind <> "" Then
...
'elseif px=10 then
			...
			set rs=server.CreateObject("adodb.recordset")
			sql="select *,(select name from tel where ord=bankout2.company) as companyname,"&_
			"(select name from gate where ord=bankout2.cateid) as cateidname"&_
			" from bankout2 "&Str_Result&" "&px_Result&" "
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "供应商预付款列表_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				...
				bankname=""
				set rs7=server.CreateObject("adodb.recordset")
				rs7.open "select sort1 from sortbank where id= "& bank &" and del=1 and ( charindex(',"& session("personzbintel2007") &",',','+replace(cast(person as varchar(8000)),' ','')+',')>0 or person like '0')",conn,1,1
				set rs7=server.CreateObject("adodb.recordset")
				...
				money1=bzname&money1
				InvoiceMode = rs("InvoiceMode")
				If InvoiceMode&""="" Then InvoiceMode=0
				InvoiceModeName=""
				Select Case InvoiceMode
				Case 0: InvoiceModeName = "采购时收票"
				Case 1: InvoiceModeName = "不收票"
				Case 2: InvoiceModeName = "预付款收票"
				End Select
...
'End Select
				If ZBRuntime.MC(223100) Then
					xlWorksheet.Cells(1+i,5).Value = spstatus
'If ZBRuntime.MC(223100) Then
...
'If ZBRuntime.MC(223100) Then
				...
				xlWorksheet.Cells(1+i,pos+1).Value = cateidname
				pos = 4
				xlWorksheet.Cells(1+i,pos+1).font.Size=10
				pos = 4
				xlWorksheet.Cells(1+i,pos+2).Value = InvoiceModeName
				pos = 4
				xlWorksheet.Cells(1+i,pos+2).font.Size=10
				pos = 4
				xlWorksheet.Cells(1+i,pos+3).Value = Replace(Replace(Replace(Replace(intro,"<br>",vbcrlf),"</P>",vbcrlf),"<P>",vbcrlf),"&nbsp;"," ")
				pos = 4
				xlWorksheet.Cells(1+i,pos+3).font.Size=10
				pos = 4
				xlWorksheet.Cells(1+i,pos+3).HorizontalAlignment=1
				pos = 4
				xlWorksheet.Cells(1+i,pos+4).Value = date7
				pos = 4
				xlWorksheet.Cells(1+i,pos+4).font.Size=10
				pos = 4
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>供应商预付款列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!供应商预付款列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("供应商预付款列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的供应商预付款列表</u></strong></font></a></p>"
