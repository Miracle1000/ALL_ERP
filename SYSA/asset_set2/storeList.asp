...
'If sc>0 Then v = v & ";"
	...
	Response.write "" & vbcrlf & "<style>" & vbcrlf & "pre{display:inline;}" & vbcrlf & "html{scrollbar-3dlight-color:#d0d0e8;scrollbar-highlight-color:#fff;scrollbar-face-color:#f0f0ff;scrollbar-arrow-color:#c0c0e8;" & vbcrlf & "scrollbar-shadow-color:#d0d0e8;scrollbar-darkshadow-color:#fff;scrollbar-base-color:#ffffff;scrollbar-track-color:#fff;}" & vbcrlf & "</style>" & vbcrlf & "<script type=""text/JavaScript"" src=""../inc/menu.js?ver="
	'Call addDefaultScript()
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<script language=""javascript"" src=""../sortcp/function.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & "<table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""1"">" & vbcrlf & "    <tr>" & vbcrlf & "            <td width=""195"" valign=""center"" id='leftmenuall'>" & vbcrlf & "                   <table border=""0"" cellpadding=""0"" cellspacing=""0"" id=""placeInfo"" style=""margin:0"">" & vbcrlf &        "                  <tr>" & vbcrlf &                 "                    <td id=""placeInfo"">&nbsp;地址管理</td> "& vbcrlf &                    "         </tr> "& vbcrlf &                "    </table> "& vbcrlf &                       "  <table width=""195"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"" style=""border-right:1px solid#ccc;margin:0"">     " & vbcrlf & "                             <tr>" & vbcrlf & "                                    <td class=""resetHeadBg"" height=""55"" valign=""middle"" style=""background:#f4fafe;"" colspan=""2"" >" & vbcrlf & "                                             <table border=""0"" cellpadding=""0"" cellspacing=""0"" width=""100%"">" & vbcrlf & "                                                 <tr>" & vbcrlf & "                            <td height=""27"" align=""left"" style=""padding-left:5px"" valign=""middle"">" & vbcrlf & "                                                                  地址检索:" & vbcrlf & "                                                                         <input name=""C"" id=""txtKeywords"" type=""text"" value=""输入地址名检索"" style=""height: 19px;width:100px; solid;font-size: 9pt;color: #999999;text-align:center;"" onFocus=""if(value==defaultValue){value='';this.style.color='#000'}"" onBlur=""if(value=='')value=defaultValue;"" onpropertychange=""if(value.indexOf('\'')>=0){value=value.replace('\'','');}else{if(value!=defaultValue) searchSort(this.value);}"" onKeyPress=""if(event.keyCode==13) {searchSort(this.value);return false;}""/>" & vbcrlf & "                                                   </td>" & vbcrlf & "                                                 </tr>" & vbcrlf & "                                           </table>" & vbcrlf & "                                        </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           <tr>" & vbcrlf & "                                    <td class=""menutab"" style=""height:38px;padding:0 2px;"">" & vbcrlf & "                                             <table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf &                                       "                <tr>" & vbcrlf &                         "                                    <td width=""31%""  height=""27""><a href=""#"" onClick=""return false;"" class=""m_l"">地址名称</a></td>" & vbcrlf &               "                                              <td width=""69%"" align=""right"">" & vbcrlf &                                "                                       <a href=""javascript:void(0);"" onClick=""searchSort('');document.getElementById('cFF2').contentWindow.document.location='edit_ck.asp';"">所有地址</a>" & vbcrlf & "                                                                       <a href=""javascript:void(0);"" onClick=""ExpandAll(this);"">全部收缩</a>" & vbcrlf & "                                                               </td>" & vbcrlf & "                                                   </tr>" & vbcrlf & "                                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                   </table>" & vbcrlf & "                        <div id=""allStore"" style=""width:195px;height:550px;overflow:auto"">" & vbcrlf & ""
	set doc= New HtmlFile
	doc.open()
    set divobj=doc.createelement("div")
    doc.appendChild(divobj)
    sql="select isnull(max(depth),0) from O_assAddressType"
    set rs=conn.execute(sql)
    Depth=rs(0)
    rs.close
    set rs=nothing
    for i=0 to Depth
        sql="select id,sort1,gate1,parentid,depth,storecode,isLeef,isnull((select count(*) from O_assAddressType where depth=a.depth and parentid=a.parentid and id>a.id),0) as f from O_assAddressType a where Depth="&i&" order by a.gate1 desc"
        set rs=server.CreateObject("adodb.recordset")
        j=1
        ...
        while not rs.eof
            ...
            if i=0 then
                ...
                if rs("isLeef").value=true then
                    sql="select id,sort1 from O_assAddressname where sort="&rs("id")&" order by gate1 desc"
