...
'dim sqlstr
	...
	Response.write "" & vbcrlf & "      <div id=""user"" name=""user"" style=""position:absolute;left:0; top:0;width:100%; height:10;"">" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">工资统计</td>" & vbcrlf & "" & vbcrlf & "            <td  align=""left""><strong>人员分布"
	Response.write m1name
	Response.write m2name
	Response.write "" & vbcrlf & "    </strong>" & vbcrlf & "                 </td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & " <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "<tr class=""top"">" & vbcrlf & "<form method=""get"" action=""hrgz3.asp"" id=""demo"" onSubmit=""return Validator.Validate(this,2)"" name=""date"">" & vbcrlf & "<td class=""top_btns""><input type=""hidden"" name=""width_tj"" value="""
	Response.write width_tj
	Response.write """>" & vbcrlf & "选择统计条件："
	...
	If InStr(person_type,"0")=0 Then
		Str_power33 = Str_power33 &" and nowstatus in (2,4) "
		Str_Result2=Str_Result2 &" and nowstatus in (2,4) "
	end if
	If InStr(person_type,"2")=0 Then
		Str_power33 = Str_power33 &" and nowstatus<>2 "
		Str_Result2=Str_Result2 &" and nowstatus<>2 "
	end if
	If InStr(person_type,"4")=0 Then
		Str_power33 = Str_power33 &" and nowstatus<>4 "
		Str_Result2=Str_Result2 &" and nowstatus<>4 "
	end if
...
'if CDate(m1)>CDate(m2) then m1=m2
	...
	Response.write person_type
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "           <input type=""button"" name=""zxt"" value=""折线图分析"" onClick=""window.location.href='hrgz3.asp?type_tj=2&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write "&person_type="
	Response.write person_type
	Response.write "'"" class=""anybutton2"">" & vbcrlf & "        <input type=""button"" name=""line"" value=""饼图分析"" onClick=""window.location.href='hrgz3.asp?type_tj=3&sort="
	Response.write E
	Response.write "&sort1="
	Response.write F
	Response.write "&time1="
	...
	Response.write ">图宽=550</option>" & vbcrlf & "              <option value=""hrgz3.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=700</option>" & vbcrlf & "              <option value=""hrgz3.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=1000</option>" & vbcrlf & "              <option value=""hrgz3.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=1500</option>" & vbcrlf & "              <option value=""hrgz3.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	Response.write ">图宽=2000</option>" & vbcrlf & "              <option value=""hrgz3.asp?type_tj="
	Response.write type_tj
	Response.write "&sort="
	Response.write E
	Response.write "&sort1="
	...
	if open_10_10=1 or open_10_10=3  then
		Response.write "<input type=""button"" name=""Submitdel2"" value=""导出"" onClick=""if(confirm('确认将统计结果导出为EXCEL文档？')){window.location.href='hrgz3.asp?type_tj=4&sort="
		Response.write E
		Response.write "&sort1="
		Response.write F
		Response.write "&ret="
	...
	Str_Result4=""
	if m1<>"" and isdate(m1) then
		Str_Result4=Str_Result4+"and datediff(d,'"&m1&"',date2)>=0 "
'if m1<>"" and isdate(m1) then
	end if
	if m2<>"" and isdate(m2) then
		Str_Result4=Str_Result4+"and  datediff(d,'"&m2&"',date2)<=0 "
'if m2<>"" and isdate(m2) then
	end if
	dim numcg1,zs1
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select a.ord,a.name from (select ord,name,sorce,sorce2 from gate_person) a left join (select userid,nowstatus from hr_person) b on a.ord=b.userid "&Str_power33&" order by sorce asc,sorce2 asc"
	rs1.open sql1,conn,1,1
	...
	if counts<=0 then
		...
		do until rs1.eof
			...
			money2=0
			set rs=server.CreateObject("adodb.recordset")
			sql="select money1,sort1,wages  from  wageslist a inner join (select id from wages where del=1 and complete1=1 and isnull(salaryClass,0)>0 "&Str_Result4&" ) b on a.wages=b.id inner join (select b.userid , b.nowstatus from gate_person a inner join hr_person b on a.ord=b.userid) c on c.userid =a.cateid  where del=1  and cateid="&rs1("ord")&"  "&Str_Result2&" "
...
'if sort1=1 then
			...
			set rs=nothing
			title=rs1("name")
			lburl="../hrwages/planallall.asp?W3="&rs1("ord")&"&a=1&F="&F&"&ret="&m1&"&ret2="&m2&"&person_type="& person_type &""
			total1(j,1)=numcg1
			total1(j,2)=title
			total1(j,0)=lburl
			total2(j,0)=title
			total2(j,1)=numcg1
			total2(0,1)="#FF0000,1.5,1,2,人员分布"
			total2(j,2)=lburl
			total3(j,0)=title
			total3(j,1)=numcg1
			total3(j,2)=lburl
