...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_104_10=1 then
		list=" and sum_addcateid in ("&intro_104_10&")"
	else
	...
	if open_104_1=3 then
	elseif open_104_1=1 then
		Str_Result=Str_Result&" and sum_addcateid in ("&intro_104_1&")"
	else
		Str_Result=Str_Result&" and sum_addcateid=0"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
	...
	if ret<>"" and ret<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&"',sum_addtime)>=0 "
'if ret<>"" and Exit Sub<>"点击选择时间" then
	end if
	ret2=ChangeDate(ret2,2)
	if ret2<>"" and ret2<>"点击选择时间" then
		Str_Result=Str_Result+" and  datediff(s,sum_addtime,'"&ret2&"')>=0 "
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	i=1
	k=0
	sql="select a.*,b.use_startime,b.use_endtime,b.use_meetingcycle,b.use_title,b.use_state,b.use_jyy from O_MeetingSummary a left join O_MeetingUse b on a.sum_metid=b.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "纪要列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			if rs("use_meetingcycle")=1 then
				MeetingTime=rs("use_startime")&"—"&rs("use_endtime")
			else
				sql7="select * from O_MeetingUseList where lst_fid="&rs("sum_metid")
				set rs7=conn.execute(sql7)
				if not rs7.eof then
					do while not rs7.eof
						MeetingTime=MeetingTime&rs7("lst_starttime")&"—"&rs7("lst_endtime")&",&nbsp;"
						rs7.movenext
					loop
				end if
				rs7.close
			...
			sum_sendcateid=ml.IsNum(rs("sum_sendcateid"))
			sql8="select * from gate where ord="&sum_sendcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				sum_sendcateid=rs8("name")
			else
				sum_sendcateid=""
			end if
			rs8.close
			set rs8=nothing
			sum_addcateid=ml.IsNum(rs("sum_addcateid"))
			sql8="select * from gate where ord="&sum_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				addname=rs8("name")
			else
			...
			set rs8=nothing
			sum_sms=rs("sum_sms")
			if isnull(sum_sms) then sum_sms=""
			if replace(sum_sms,",","")<>"" then
				sql8="select * from gate where ord in("&sum_sms&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					sum_sms=""
					do while not rs8.eof
						sum_sms=sum_sms&rs8("name")&","
						rs8.movenext
					loop
				else
					sum_sms=""
			...
			sum_email=rs("sum_email")
			if isnull(sum_email) then sum_email=""
			if replace(sum_email,",","")<>"" then
				sql8="select * from gate where ord in("&sum_email&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					sum_email=""
					do while not rs8.eof
						sum_email=sum_email&rs8("name")&","
						rs8.movenext
					loop
				else
					sum_email=""
			...
			sum_index=rs("sum_index")
			if isnull(sum_index) then sum_index=""
			if replace(sum_index,",","")<>"" then
				sql8="select * from gate where ord in("&sum_index&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					sum_index=""
					do while not rs8.eof
						sum_index=sum_index&rs8("name")&","
						rs8.movenext
					loop
				else
					sum_index=""
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>纪要列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!纪要列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("纪要列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的纪要列表</u></strong></font></a></p>"
