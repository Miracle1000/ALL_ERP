...
'items = rpt.Settings.item(i)
	...
	Sub onReportInit(rpt, state)
		...
		Call app.addDefaultScript()
		rpt.title = "生产下达明细表"
		rpt.addfield "派工状态：","select","state", "全部" & Chr(1) & "0" & Chr(2) & "未完成" & Chr(1) & "1" & Chr(2) & "已完成" & Chr(1) & "2","",""
		rpt.addfield "&nbsp;添加时间：","dates","date1","","", dat
		rpt.addfield "订单：","text","OrderTitle","","", ""
		rpt.addfield "&nbsp;","select","searchType", "产品名称" & Chr(1) & "0" & Chr(2) & "产品型号" & Chr(1) & "1" & Chr(2) & "产品编号" & Chr(1) & "2","",""
		rpt.addfield "","text","searckKey", "", "", ""
		rpt.aSearch.addField "下达人员", "gates", "G_xdry",""
		rpt.aSearch.addField "开始日期", "dates", "g_btime",""
		rpt.aSearch.addField "截止日期", "dates", "g_etime",""
		rpt.aSearch.addField "工艺流程", "text", "g_wproc",""
		rpt.aSearch.addField "下达主题", "text", "g_xdzt",""
		rpt.aSearch.addField "下达编号", "text", "g_xdbh",""
		rpt.aSearch.addField "添加人员", "gates", "g_tjry",""
		rpt.aSearch.addField "备注", "text", "remark",""
		rpt.aSearch.addZdyFields 21
		rpt.sql = "exec erp_editReport_WholeWorkAssigns @state, @date1_0, @date1_1, @OrderTitle, @searchType, @searckKey, @G_xdry, @g_btime_0, @g_btime_1," & _
		"@g_etime_0, @g_etime_1, @g_wproc, @g_xdzt, @g_xdbh, @g_tjry, @remark, @zdy21_1, @zdy21_2, @zdy21_3, @zdy21_4, @zdy21_5, @zdy21_6," & Info.User
		rpt.canexcel =  true
		rpt.canprint =  app.power.existsPower(53,7)
		rpt.BillListUIModel = True
	end sub
	Function showselectbox(state, mxid, xdCateid, ddnostate)
		If Len(xdCateid & "") = 0 Then xdCateid = 0
		If state = "派工完毕" Or Info.User*1 <> xdCateid*1 Or ddnostate>0 Then
			showselectbox = "&nbsp;"
		else
			showselectbox = "<input type=checkbox name='mxid' value='" & mxid & "'>"
		end if
	end function
	Sub app_oncreate(lvw)
		...
		lvw.IsaccWidth = True
		Set h = lvw.headers(1)
		h.formatText = "code:showselectBox(""@cells[19]"",@cells[31],@cells[28],@cells[34])"
		h.width = 50
		Set h = lvw.headers(2)
		h.display = "none"
		h.width = 0
		...
		Set h = lvw.headers(6)
		h.width = 140
		h.formattext = "code:ProductLinkHTML(@cells[5],""@value"")"
		h.dbtype = "str"
		Set h = lvw.headers(7)
		h.width = 100
		h.dbtype = "str"
		...
		h.setLink   "", "工艺流程ID", "工艺添加人ID", 59, 24
		...
		h.width = 0
		h.display = "none"
		Set h = lvw.headers(29)
		h.setLink   "", "", "下达人ID", 53, 4
		Set h = lvw.headers(31)
		h.width = 0
		h.display = "none"
		lvw.headers(32).width = 100
		lvw.headers(33).width = 150
		lvw.headers(34).display = "none"
		lvw.headers(34).width = 0
	end sub
	Sub OnReportBottom
		Response.write "" & vbcrlf & "<div style='position:relative;top:-30px;'>" & vbcrlf & "   <table style='table-layout:auto;width:auto'>" & vbcrlf & "    <tr>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "         <td><input type='checkbox' id='ckall' onclick='selectall(this)'> </td>" & vbcrlf & "          <td><label for='ckall'>全选</label></td>" & vbcrlf & "         <td width='20'>&nbsp;</td>" & vbcrlf & "              <td>" & vbcrlf & "                    <form action='../apply/WholeWorkAssignsHandle.asp' target='_blank' id='mfrm' method='post' style='display:inline'>" & vbcrlf & "                              <input type='hidden' name='handle' id='handle' value=1>" & vbcrlf & "                         <input type='hidden' name='selectid' id='selectid'>" & vbcrlf & "                      </form>" & vbcrlf & "                 "
		If  app.power.existsPower(54,13) then
			Response.write "" & vbcrlf & "                     <button class='oldbutton3' onclick='dosubmit()'>批量派工</button>" & vbcrlf & "                       "
		end if
		Response.write "" & vbcrlf & "             </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "</div>" & vbcrlf & ""
	end sub
