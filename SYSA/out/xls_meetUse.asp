...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	set rs1=nothing
	Str_Result=""
	if open_104_10=3 then
		list=""
	elseif open_104_10=1 then
		list=" and use_cateid in ("&intro_104_10&")"
	else
		list=" and use_cateid=0"
	end if
	Str_Result=" where 1=1 "&list&""
	...
	if open_104_1=3 then
	elseif open_104_1=1 then
		Str_Result=Str_Result&" and use_cateid in ("&intro_104_1&")"
	else
		Str_Result=Str_Result&" and use_cateid=0"
	end if
	ret         =request("ret")
	ret2        =request("ret2")
	A   =request("A")
...
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	i=1
	k=0
	sql="select a.*,b.met_name from O_MeetingUse a left join O_Meeting b on a.use_meeting=b.id "&Str_Result&" "&Ptr_Result
	set rs = server.CreateObject("adodb.recordset")
	rs.open sql,conn,1,1
	C1=rs.recordcount
	xlApplication.Init "申请列表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			use_cateid=ml.IsNum(rs("use_cateid"))
			...
			rs8.close
			set rs8=nothing
			if rs("use_complete")=1 then
				use_complete="待审批"
			elseif rs("use_complete")=2 then
				use_complete="审批中"
			elseif rs("use_complete")=3 then
				use_complete="审批通过"
			elseif rs("use_complete")=4 then
				use_complete="审批未通过"
			end if
			use_cateid_sp=ml.IsNum(rs("use_cateid_sp"))
			sql8="select * from gate where ord="&use_cateid_sp
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				use_cateid_sp=rs8("name")
			else
				use_cateid_sp=""
			end if
			rs8.close
			set rs8=nothing
			use_addcateid=ml.IsNum(rs("use_addcateid"))
			if use_addcateid="" or isnull(use_addcateid) then use_addcateid=0
			sql8="select * from gate where ord="&use_addcateid
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				use_addcateid=rs8("name")
			else
				use_addcateid=""
			end if
			rs8.close
			set rs8=nothing
			use_property=ml.IsNum(rs("use_property"))
			sql8="select * from O_MeetingSet where set_type=1 and id="&use_property
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				use_property=rs8("set_name")
			else
				use_property=""
			end if
			rs8.close
			set rs8=nothing
			use_important=ml.IsNum(rs("use_important"))
			sql8="select * from O_MeetingSet where set_type=2 and id="&use_important
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				use_important=rs8("set_name")
			else
				use_important=""
			end if
			rs8.close
			set rs8=nothing
			use_jyy=ml.IsNum(rs("use_jyy"))
			sql8="select * from gate where ord="&use_jyy
			set rs8=conn.execute(sql8)
			if not rs8.eof then
				use_jyy=rs8("name")
			else
			...
			set rs8=nothing
			use_zcr=rs("use_zcr")
			if replace(use_zcr,",","")<>"" then
				sql8="select * from gate where ord in("&use_zcr&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					use_zcr=""
					do while not rs8.eof
						use_zcr=use_zcr&rs8("name")&","
						rs8.movenext
					loop
				end if
				rs8.close
			...
			use_device=rs("use_device")
			if replace(use_device,",","")<>"" then
				sql8="select * from O_MettingDevice where id in("&use_device&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					use_device=""
					do while not rs8.eof
						use_device=use_device&rs8("dev_name")&","
						rs8.movenext
					loop
				else
					use_device=""
			...
			use_cxry=rs("use_cxry")
			if replace(use_cxry,",","")<>"" then
				if left(use_cxry,1)="," then
					use_cxry=right(use_cxry,len(use_cxry)-1)
'if left(use_cxry,1)="," then
				end if
				if right(use_cxry,1)="," then
					use_cxry=left(use_cxry,len(use_cxry)-1)
'if right(use_cxry,1)="," then
				end if
				sql8="select * from gate where ord in("&use_cxry&")"
				set rs8=conn.execute(sql8)
				if not rs8.eof then
					use_cxry=""
					do while not rs8.eof
						use_cxry=use_cxry&rs8("name")&","
						rs8.movenext
					loop
				end if
				rs8.close
			...
			use_addtime=rs("use_addtime")
			if rs("use_meetingcycle")=1 then
				xlWorksheet.Cells(1+i,13).Value =use_startime
'if rs("use_meetingcycle")=1 then
				xlWorksheet.Cells(1+i,13).font.Size=10
'if rs("use_meetingcycle")=1 then
				xlWorksheet.Cells(1+i,13).HorizontalAlignment=1
'if rs("use_meetingcycle")=1 then
				xlWorksheet.Cells(1+i,14).Value =use_endtime
'if rs("use_meetingcycle")=1 then
...
'if rs("use_meetingcycle")=1 then
				...
				sql7="select * from O_MeetingUseList where lst_fid="&rs("id")&" order by lst_starttime asc"
				set rs7=conn.execute(sql7)
				if not rs7.eof then
					do while not rs7.eof
						lst_starttime=rs7("lst_starttime")
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,2).Value ="【明细信息】"
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,2).font.Size=10
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,13).Value =lst_starttime
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,13).font.Size=10
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,13).HorizontalAlignment=1
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,14).Value =lst_endtime
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,14).font.Size=10
						lst_endtime=rs7("lst_endtime")
						xlWorksheet.Cells(2+i,14).HorizontalAlignment=1
						lst_endtime=rs7("lst_endtime")
						i=i+1
						lst_endtime=rs7("lst_endtime")
						rs7.movenext
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>申请列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!申请列表导出成功,共有"&(k)&"条记录!</B>"";</script>"
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("申请列表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的申请列表</u></strong></font></a></p>"
