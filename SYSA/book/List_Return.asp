...
'if isnull(values) then IsTime = null : exit function
	...
	if open_103_1=3 then
	elseif open_103_1=1 then
		Str_Result=Str_Result&" and b.addcateid in ("&intro_103_1&")"
	else
		Str_Result=Str_Result&" and b.addcateid=0"
	end if
	if W4<>"" and not isnull(W4) then
		Str_Result=Str_Result&" and b.addcateid in ("&W3&") "
	end if
	if D1<>"" then
	...
	if D20<>"" then
		Str_Result=Str_Result&" and Ret_num >="&ml.isNum(D20)&""
	end if
	if D201<>"" then
		Str_Result=Str_Result&" and Ret_num <="&ml.isNum(D201)&""
	end if
	if D21<>"" then
		Str_Result=Str_Result&" and bk_postion like '%"&ml.De(D21)&"%'"
	end if
	if D22<>"" then
		Str_Result=Str_Result&" and Ret_note like '%"&ml.De(D22)&"%'"
	end if
	if M<>"" then
...
'if K<>"" then
	...
	if L1<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L1&" 00:00:00',Ret_time)>=0 "
'if L1<>"" then
	end if
	if L2<>"" then
		Str_Result=Str_Result+" and  datediff(s,Ret_time,'"&L2&" 23:59:59')>=0 "
'if L2<>"" then
	end if
	if L3<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&L3&" 00:00:00',b.addtime)>=0 "
'if L3<>"" then
	end if
	if L4<>"" then
		Str_Result=Str_Result+" and  datediff(s,b.addtime,'"&L4&" 23:59:59')>=0 "
'if L4<>"" then
	end if
	Ptr_Result=""
	px=request("px")
	...
	elseif px="3" then
		Ptr_Result=" order by Ret_time desc"
	elseif px="4" then
		Ptr_Result=" order by Ret_time asc"
	elseif px="5" then
		Ptr_Result=" order by b.addtime desc"
	elseif px="6" then
		Ptr_Result=" order by b.addtime asc"
	end if
	if ret<>"" then
		Str_Result=Str_Result+" and  datediff(s,'"&ret&" 00:00:00',b.addtime)>=0 "
'if Exit Sub<>"" then
	end if
	if ret2<>"" then
		Str_Result=Str_Result+" and  datediff(s,b.addtime,'"&ret2&" 23:59:59')>=0 "
'if Exit Sub2<>"" then
	end if
	if C<>"" then
		if B="name" then
	...
	Response.write "" & vbcrlf & "<form name=""data"" id=""data"" action=""List_Return.asp?ret="
	Response.write ret
	Response.write "&ret2="
	Response.write ret2
	Response.write "&B="
	...
	Response.write "&C="
	Response.write C
	Response.write """ method=""get"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "              <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">归还列表</td>" & vbcrlf & "                       <td><A onclick=""Myopen(User);return false;"" href=""javascript:;"" class=""sortRule"">排序规则<IMG src=""../images/i10.gif"" alt=""排序规则"" width=""9"" height=""5"" border=""0""></A></td>" & vbcrlf & "           <td width=""2""><img src=""../images/m_mpr.gif"" width=""1"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table><span id=""ht1""></span>" & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""3"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">   "    & vbcrlf &       "          <tr>" & vbcrlf &         "      <td colspan=""12"" class=""top_btns"" background=""../images/112.gif""  height=""30"" align=""right"" style=""border-bottom:0px;border-top:#C0CCDD 0px solid;border-left:#C0CCDD 0px solid;border-right:#C0CCDD 1px solid;padding:0!important"">" & "               <span id=""kh"">自：<input id=""daysOfMonthPos"" onmouseup=""toggleDatePicker('daysOfMonth','date.ret')"" size=""9"" name=""ret"" readonly=""true"" value="""
	Response.write ret
	...
	Response.write ret2
	Response.write """/>" & vbcrlf & "                       <DIV id=daysOfMonth2 style=""POSITION: absolute""></DIV>" & vbcrlf & "                    <select name=""B"" id=""B"" style=""color:#000000"">" & vbcrlf & "                                <option value="""">--请选择--</option>" & vbcrlf & "                              <option value=""name"" "
	Response.write ret2
	...
	Response.write """>" & vbcrlf & "                        <input name=""px"" type=""hidden"" value="""
	Response.write px
	Response.write """><input type=""submit"" class=""anybutton2"" id=""btnmit"" name=""btnmit"" value=""检索"">                   " & vbcrlf & "                        "
	if open_103_10=3 or (open_103_10=1 and CheckPurview(intro_103_10,trim(session("personzbintel2007"))))=True then
		Response.write "" & vbcrlf & "                     <input name=""submit2"" type=""button""  class=""anybutton2"" value=""导出"" onClick=""if(confirm('确认导出为EXCEL文档？')){exportExcel({from:'form_with_page_action',page:'../out/xls_ReturnBook.asp?px="
		Response.write px
		Response.write "&A="
	...
	Response.write "" & vbcrlf & "                     <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onclick=""callServer2();document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></span></td>" & vbcrlf & "         </tr>" & vbcrlf & "            <tr valign=""center"" class=""top"">" & vbcrlf & "              <td width=""6%"" height=""30"" align=""left"" valign=""middle""  class=""name""><div align=""center"">选择</div></td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">书名</div></td>" & vbcrlf & "                   <td width=""10%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">编号</div></td>" & vbcrlf & "            <td width=""9%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">ISBN</div></td>" & vbcrlf & "            <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">作者</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">借阅人员</div></td>" & vbcrlf & "                 <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">借阅时间</div></td>" & vbcrlf & "                <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">借阅数量</div></td>" & vbcrlf & "                 <td width=""6%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">归还数量</div></td> "& vbcrlf &          "      <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">实还时间</div></td> "& vbcrlf &           "       <td width=""8%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">应还时间</div></td>" & vbcrlf & "<td width=""15%"" align=""left"" valign=""middle""  class=""name""><div align=""center""><select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid   #FFFFFF; overflow:hidden"">" & vbcrlf & "                        <option>-请选择-</option>" & vbcrlf & "                       <option value=""page_count=10"" "
	...
	currpage=clng(currpage)
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select a.id as Ret_id,a.Ret_fid,a.Ret_bkid,isnull(a.Ret_num,0) as Ret_num,a.Ret_time,a.Ret_note,b.addcateid,b.addtime, "&_
	"e.set_name,c.bk_name,c.bk_bh,c.bk_isbn,c.bk_auther,c.bk_publishing,d.Ld_time,isnull(d.Ld_num,0) as Ld_num,d.Ld_rettime,h.name from O_RetBookmx a "&_
	"left join O_RetBook b on a.Ret_fid=b.id "&_
	"left join O_Lendbookmx d on a.Ret_bkid=d.id "&_
	"left join O_Lendbook g on d.Ld_fid=g.id "&_
	"left join gate h on g.addcateid=h.ord "&_
	"left join O_RegBook c on d.Ld_bkid=c.id "&_
	"left join O_bookset e on c.bk_type=e.id "&Str_Result&" "&Ptr_Result
	rs7.open sql7,conn,1,1
	if rs7.RecordCount<=0 then
		Response.write "<tr><td colspan=12><table align='center'><tr><td>没有信息!</td></tr></table></td></tr>"
		currpage=0
	else
		...
		do while not rs7.eof
			...
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "             <a href=""javascript:void(0);"" onClick=""javascript:window.open('Detail_Return.asp?ord="
				Response.write rs7("Ret_fid")
			...
			Response.write "</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("name")
			Response.write "</div></td>" & vbcrlf & "             <td width=""7%"" align=""left"" valign=""middle""  class=""name""><div align=""center"">"
			Response.write rs7("Ld_time")
			...
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write FormatNumber(rs7("Ret_num"),num1_dot,-1)
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("Ret_time")
			Response.write "</div></td>" & vbcrlf & "             <td align=""left"" valign=""middle"" class=""func""><div align=""center"">"
			Response.write rs7("Ld_rettime")
			Response.write "</div></td>" & vbcrlf & "             <td width=""15%"" height=""25"" align=""left"" valign=""middle""  class=""func""><div align=""center"">" & vbcrlf & "                 "
			if open_103_14=3 or CheckPurview(intro_103_14,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                  <input name=""button3"" type=""button""  class=""page"" id=""submit3"" onClick=""javascript:window.open('Detail_Return.asp?ord="
				Response.write rs7("Ret_fid")
			...
			if (open_103_2=3 or CheckPurview(intro_103_2,trim(addcateid))=True) then
				Response.write "" & vbcrlf & "                      <input name=""button4"" type=""button""  class=""page"" id=""submit4"" onClick=""javascript:window.open('Update_Return.asp?ord="
				Response.write rs7("Ret_id")
				Response.write "&px="
				Response.write px
			...
			if open_103_3=3 or CheckPurview(intro_103_3,trim(addcateid))=True then
				Response.write "" & vbcrlf & "                     <input name=""button5"" type=""button""  class=""page"" id=""submit5"" onClick=""if(confirm('确认删除吗？')){window.location=('Del_Return.asp?ord="
				Response.write rs7("Ret_id")
				Response.write "&px="
				Response.write px
				Response.write "&A="
	...
	action1="返还列表"
	call close_list(1)
	Response.write "" & vbcrlf & "<script language=javascript>" & vbcrlf & "function Myopen(divID){ //根据传递的参数确定显示的层" & vbcrlf & "     if(divID.style.display==""""){" & vbcrlf & "              divID.style.display=""none""" & vbcrlf & "        }else{" & vbcrlf & "          divID.style.display=""""" & vbcrlf & "    }" & vbcrlf & "       divID.style.left=300;" & vbcrlf & " divID.style.top=10;" & vbcrlf & "}" & vbcrlf & "" & vbcrlf & "function testall()" & vbcrlf & "{" & vbcrlf & "if(!confirm('确认删除吗？'))" & vbcrlf & "{" & vbcrlf & "     window.location=""List_Return.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "}" & vbcrlf & "else" & vbcrlf & "{" & vbcrlf & "  document.getElementById('data').action=""Del_Return.asp?px="
	Response.write px
	Response.write "&A="
	Response.write A
	Response.write "&B="
	...
	Response.write "&CurrPage="
	Response.write CurrPage
	Response.write """;" & vbcrlf & "        document.getElementById('data').method=""post""" & vbcrlf & "     document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "function testall1()" & vbcrlf & "{" & vbcrlf & "         document.getElementById('data').action=""Del_Return.asp"";" & vbcrlf & "  document.getElementById('data').submit();" & vbcrlf & "}" & vbcrlf & "function mm(form)" & vbcrlf & "{" & vbcrlf & "for (var i=0;i<form.elements.length;i++)" & vbcrlf & "///循环,form.elements.length得到表单里的控件个数" & vbcrlf & "{" & vbcrlf & "///把表单里的内容依付给e这个变量 " & vbcrlf & "var e = form.elements[i]; " & vbcrlf &"if (e.name != 'chkall') " & vbcrlf & "e.checked = form.chkall.checked; " & vbcrlf & "}" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
