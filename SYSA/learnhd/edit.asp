...
'if strW3<>"" Then
	...
	dim open_73_1,intro_73_1,kh_list
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=1"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_1=0
		intro_73_1=0
	else
		open_73_1=rs1("qx_open")
		intro_73_1=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=14"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_14=0
		intro_73_14=0
	else
		open_73_14=rs1("qx_open")
		intro_73_14=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=2"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_2=0
		intro_73_2=0
	else
		open_73_2=rs1("qx_open")
		intro_73_2=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=3"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_3=0
		intro_73_3=0
	else
		open_73_3=rs1("qx_open")
		intro_73_3=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=6"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_6=0
		intro_73_6=0
	else
		open_73_6=rs1("qx_open")
		intro_73_6=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=7"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_7=0
		intro_73_7=0
	else
		open_73_7=rs1("qx_open")
		intro_73_7=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=73 and sort2=8"
	rs1.open sql1,conn,1,1
	if rs1.eof then
		open_73_8=0
		intro_73_8=0
	else
		open_73_8=rs1("qx_open")
		intro_73_8=rs1("qx_intro")
	end if
	rs1.close
	set rs1=nothing
	if open_73_1=3 then
		list=""
	elseif open_73_1=1 then
		list="and cateid in ("&intro_73_1&")"
	else
		list="and cateid=0"
	end if
	dim rs,sql,Str_Result,Str_Result2
	Str_Result="where ((ord>0 "&list&")  or  share like '%"&session("personzbintel2007")&"%' or share='0')"
	Str_Result2="and ((ord>0 "&list&")  or  share like '%"&session("personzbintel2007")&"%' or share='0')"
	Str_power=""
	Str_power22=""
	Str_power33=""
	sorce=0
	...
	if open_73_1="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_73_1&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
...
'Private Sub Class_Initialize
	...
	if open_73_1=3 then
		list=""
	elseif open_73_1=1 then
		list="and a.cateid in ("&intro_73_1&")"
	else
		list="and a.cateid=0"
	end if
	Str_Result="where ((a.ord>0 "&list&")  or  CHARINDEX(',"&session("personzbintel2007")&",',','+replace(a.share,' ','')+',')>0  or a.share='0')"
	list="and a.cateid=0"
	If request("Submit45") <> "" Then
		if time1<>"" Then
			if(Ubound(split(time1," "))<1) then time1=time1&" 00:00:00"
			time1=CDate(time1)
			Str_Result=Str_Result+"and  a.date7>='"& time1 &"' "
			time1=CDate(time1)
		end if
		if time2<>"" Then
			if(Ubound(split(time2," "))<1) then time2=time2&" 23:59:59"
			time2=CDate(time2)
			Str_Result=Str_Result+"and  a.date7<='"& time2 &"' "
			time2=CDate(time2)
		end if
		if title<>"" then
...
'if title<>"" then
		...
		if content<>"" then
			Str_Result=Str_Result+"and  a.intro like '%"& content &"%' "
'if content<>"" then
		end if
		if cktype<>"" then
			Str_Result=Str_Result+"and  a.sort2 in ("& cktype &") "
'if cktype<>"" then
		end if
		W3=getW3(W1,W2,W3)
	...
	if sort="" then
		...
		Str_Result=Str_Result + "and a.sort2="&request("sort")&" and a.del=1"
		Str_Result=Str_Result
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select * from sortonehy where ord="&sort&""
		rs1.open sql1,conn,1,1
		if rs1.RecordCount<=0 then
			Response.write "<tr><td ><span class='red'>没有分类信息!</span></td></tr>"
		else
			sort2=rs1("sort1")
		end if
		rs1.close
	...
	Str_Result=Str_Result & " and a.del=1"
	if A="3333333" or A="5555555" then
	...
	If remind <> "" Then
		...
		Str_Result = Str_Result &" and a.ord in (" & helper.listSQL("ids") & ")"
	end if
	If request("Submit45") = "" Then
		A=request("A")
		C=trim(request("C"))
		...
		ret2=request("ret2")
		if A="1" then
			Str_Result=Str_Result+"and  a.cateid in (select ord from gate where  name like '%"& C &"%')"
'if A="1" then
		end if
		if A="2" then
...
'if A="3" then
		...
		if ret<>"" Then
			m1 = ret
			if(Ubound(split(ret," "))<1) then ret=ret&" 00:00:00"
			ret=CDate(ret)
			Str_Result=Str_Result+"and  a.date7>='"& ret &"' "
			ret=CDate(ret)
		end if
		if ret2<>"" Then
			m2 = ret2
			if(Ubound(split(ret2," "))<1) then ret2=ret2&" 23:59:59"
			ret2=CDate(ret2)
			Str_Result=Str_Result+"and  a.date7<='"& ret2 &"' "
			ret2=CDate(ret2)
		end if
	end if
	dim newstate
	newstate=trim(request("newstate"))
	if A="3333333" or A="5555555" or A="cylm" then
		new_list="<span class='alt_tx'></span>"
		timelg=session("timezbintel2007")
		if A="3333333" and timelg<>"" then
			noalt1=" and a.date7>'"&timelg&"'"
		...
		set rs9=server.CreateObject("adodb.recordset")
		if newstate="1" then
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where ord=8 and cateid="&session("personzbintel2007")&""
		else
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm  where ord=8 and cateid="&session("personzbintel2007")&""
		end if
		rs9.open sql9,conn,1,1
		if rs9.eof then
			tq1=0
			sql = "and ("&open_73_1&" = 3 OR ("&open_73_1&" = 1 AND CHARINDEX(’,’+ CAST(learnhd.cateid AS VARCHAR(12)) +’,’ , ’,"&intro_73_1&",’) > 0)" &_
			"OR share = ’0’ OR CHARINDEX(’,’+CAST(&session(personzbintel2007)& AS VARCHAR(12))+’,’ , ’,’+ share +’,’) > 0)"
		else
			if open_73_1=3 then
				Str_Result=Str_Result&""
			elseif open_73_1=1 then
				Str_Result=Str_Result&"and (a.cateid in ("&intro_73_1&") or  CHARINDEX(',"&session("personzbintel2007")&",',','+replace(a.share,' ','')+',')>0  or a.share='0')"
'elseif open_73_1=1 then
			else
				Str_Result=Str_Result&"and (a.cateid=0 or  CHARINDEX(',"&session("personzbintel2007")&",',','+replace(a.share,' ','')+',')>0  or a.share='0')"
'elseif open_73_1=1 then
			end if
			if rs9("fw1")="1" then
		...
		rs9.close
		set rs9=nothing
		Str_Result=Str_Result+"and CHARINDEX(',"&session("personzbintel2007")&",',','+replace(a.alt,' ','')+',')=0  "&noalt1&" "
		set rs9=nothing
	end if
	...
	currpage1=Request.form("currpage1")
	if currpage1<>"" then
		currpage=currpage1
	else
	...
	px = request("px")
	If px & "" = "" Or isnumeric(px & "") = False Then
		px = 1
	end if
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "    margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & ".IE5 .top_search input.anybutton{height:18px;line-height:16px;margin-bottom:-0.5px;}" & vbcrlf & "td.top_search{padding-top:0!important;padding-bottom:0!important}" & vbcrlf & "#ht1 table{margin-bottom:-1px}" & vbcrlf & "</style>" & vbcrlf & "<script src= ""../Script/ld_edit.js?ver="
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	...
	Response.write "" & vbcrlf & "<form method=""get"" action=""edit.asp"" id=""demo"" name=""date"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0""  bgcolor=""#FFFFFF"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "            <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "                  <tr>" & vbcrlf & "        <td class=""place"">"
	if sort="" then Response.write("所有")
	Response.write "互动列表"
	if sort<>"" then
	...
	Response.write "</td>" & vbcrlf & "                                <td><div style=""position:absolute;width:200px;left:286px;top:15px""><a href=""javascript:void(0)"" onClick=""Myopen(User);return false;"" class=""sortRule"">排序规则<img src=""../images/i10.gif"" width=""9"" height=""5"" border=""0""></a></div>&nbsp;</td>" & vbcrlf & "     <td align=""right"" valign=""top"">" & vbcrlf & "                              &nbsp;"
	if open_73_7=1 or open_73_7=3 then
		Response.write "<input type=""button"" name=""Submit43"" value=""打印""  onClick=""javascript:kh.style.display='none';window.print();return  false;""   class=""anybutton"" style=""padding-top:1px;margin-top:5px;""/>"
'if open_73_7=1 or open_73_7=3 then
	end if
	Response.write "</td>" & vbcrlf & "                <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "                      </tr>" & vbcrlf & "           </table>" & vbcrlf & "                <table id=""kh"" width=""100%"" border=""0"" cellpadding=""3"" style=""word-break:break-all;word-wrap:break-word;border-collapse:collapse;border-top:1px solid #ccc"">" & vbcrlf & "                  <TR height=""50"">" & vbcrlf & "                          <td class=""top_search"" background=""../images/112.gif""  height=""50"" valign=""middle"" align=""right"" style=""border-top:#C0CCDD 1px solid;border-left:#C0CCDD 1px solid;border-right:#C0CCDD 1px solid;"">" & vbcrlf & "                         <div>" & vbcrlf & "                           发布时间"
	...
	Response.write "" & vbcrlf & "                             <select name=""sort"">" & vbcrlf & "                                      <option value="""">互动分类</option>" & vbcrlf & "                                        "
	set rs7=server.CreateObject("adodb.recordset")
	sql7="select * from sortonehy where gate2=93 order by gate1 desc"
	rs7.open sql7,conn,1,1
	do until rs7.eof
		Response.write "" & vbcrlf & "                                     <option value="""
		Response.write rs7("ord")
		Response.write """ "
		if cstr(Trim(sort))=CStr(rs7("ord")) then
	...
	rs7.close
	set rs7=nothing
	Response.write "" & vbcrlf & "                             </select>" & vbcrlf & "                               <select name=""A"">" & vbcrlf & "                                 <option value=""1"" "
	if A="1" then Response.write "selected"
	Response.write ">发布人</option>" & vbcrlf & "                             <option value=""2"" "
	if A="2" then Response.write "selected"
	Response.write ">标题</option>" & vbcrlf & "                               <option value=""3"" "
	if A="3" then Response.write "selected"
	Response.write ">内容关键字</option>" & vbcrlf & "                         </select>" & vbcrlf & "                               <input name=""C"" type=""text"" size=""10""  value="""
	Response.write C
	Response.write """/><input value="""
	Response.write px
	Response.write """ type=""hidden"" name=""px"" />" & vbcrlf & "                          <input type=""submit"" name=""Submit422"" value=""检索""  class=""anybutton""/>" & vbcrlf & "                         <a href=""javascript:void(0)"" class=""AfterQuickSearch"" onClick=""callServer2('"
	Response.write sort
	Response.write "');document.getElementById('kh').style.display='none';document.getElementById('ht1').style.display='';return false;""><img src=""../images/icon_title.gif"" width=""18"" height=""7"" border=""0""><u><font class=""advanSearch"">高级检索</font></u></a></div>" & vbcrlf & "                                </td>" & vbcrlf & "                   </tr>" & vbcrlf & "          </table>" & vbcrlf & "                <span id=""ht1""></span>" & vbcrlf & "    </form>" & vbcrlf & " <table class=""ewebeditorImg_plan"" width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "      <tr class=""top"">" & vbcrlf & "        <td align=""center"" width=""5%"" ><div align=""center"">选择</div></td>" & vbcrlf & "        <td height=""27"" width=""5%""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "        <td height=""23"" width=""10%""><div align=""center""><strong>标题</strong></div></td>" & vbcrlf & "       <td width=""8%""><div align=""center""><strong>分类</strong></div></td>" & vbcrlf & "        "
	if A="7777777" or A="9999999" then
		Response.write "<td width=""15%""><div align=""center""><strong>回复</strong></div></td>"
	else
		Response.write "<td width=""15%""><div align=""center""><strong>互动交流</strong></div></td>"
	end if
	Response.write "" & vbcrlf & "        <td width=""8%""><div align=""center""><strong>交流/回复</strong></div></td>" & vbcrlf & "        <td width=""8%""><div align=""center""><strong>发布人</strong></div></td>" & vbcrlf & "        "
	if A="7777777" or A="9999999" then
		Response.write "<td width=""8%""><div align=""center""><strong>回复人</strong></div></td>"
	end if
	Response.write "" & vbcrlf & "        <td width=""13%""><div align=""center"" ><strong>发布时间</strong></div></td>" & vbcrlf & "        <td width=""20%""><div align=""center"" >" & vbcrlf & "          <select name=""select5"" onChange=""if(this.selectedIndex && this.selectedIndex!=0){gotourl(this.value);}"" style=""BACKGROUND: #FFFFFF; HEIGHT: 20px;font-size:12px;font-weight: bold;color:#2F496E;border:0px solid #FFFFFF; overflow:hidden"">" & vbcrlf & "            <option>-请选择-</option>" & vbcrlf & "            <option value=""page_count=10"" "
	'Response.write "<td width=""8%""><div align=""center""><strong>回复人</strong></div></td>"
	...
	sql = ""
	set rs=server.CreateObject("adodb.recordset")
	pxStr = " replyhd.date7 desc "
	Select Case px
	Case 1
	pxStr = " replyhd.date7 desc "
	Case 2
	pxStr = " replyhd.date7 asc "
	Case 3
	pxStr = " replyhd.title desc "
	Case 4
	pxStr = " replyhd.title asc "
	Case 5
	pxStr = " s.sort1 desc "
	Case 6
	pxStr = " s.sort1 asc "
	...
	if A="7777777" or A="9999999" then
		set rs9=server.CreateObject("adodb.recordset")
		if newstate="1" then
			sql9="select id,ord,intro,num1,tq1,fw1,gate1 from setjm2  where ord=39 and cateid="&session("personzbintel2007")&""
			rs9.open sql9,conn,1,1
			if rs9.eof then
				tq1=0
				sql = "and ("&open_73_1&" = 3 OR ("&open_73_1&" = 1 AND CHARINDEX(','+ CAST(learnhd.cateid AS VARCHAR(12)) +',' , ',"&intro_73_1&",') > 0)" &_
				"OR share = '0' OR CHARINDEX(','+CAST(&session(personzbintel2007)& AS VARCHAR(12))+',' , ',"
			else
				if rs9("fw1")="0" then
					if open_73_1=3 then
						sql=""
					elseif open_73_1=1 then
						sql=" and learnhd.cateid in ("&intro_73_1&") OR (share = '0' OR CHARINDEX(','+CAST("&session("personzbintel2007")&" AS VARCHAR(12))+',' , ','+ share +',') > 0)"
'elseif open_73_1=1 then
					else
						sql=" and 1=2 "
					end if
				else
					sql=" and learnhd.cateid="&session("personzbintel2007")
				end if
				tq1=rs9("tq1")
			end if
			rs9.close
		...
		if A="7777777" or (A="9999999" And newstate<>"1") then
			sql=sql&" and learnhd.cateid="&session("personzbintel2007")
		end if
		sql="SELECT replyhd.id as id,replyhd.alt as alt,learnhd.sort2 as sort2, learnhd.clicknum as clicknum, learnhd.cateid as cateid,learnhd.title as title,learnhd.ord as ord,replyhd.date7,replyhd.date7 as replydate,ISNULL(h.name,replyhd.name) as replyname,cast(replyhd.intro as varchar(4000)) as replytitle,s.sort1 as sortname,g.name as cateName " &_
		" FROM replyhd " &_
		" LEFT JOIN learnhd ON replyhd.ord = learnhd.ord " &_
		" Left Join sortonehy s on learnhd.sort2 = s.ord " &_
		" Left Join gate g on learnhd.cateid = g.ord " &_
		"LEFT JOIN gate h ON h.ord = replyhd.userID " &_
		" where learnhd.del=1 and replyhd.alt=1 "&sql&" "&noalt1&" order by " & pxStr
	else
		sql = "SELECT a.[ord],a.[title],a.[share],a.[cateid],a.[date7],a.[alt],a.[sort2],a.[del],a.[delcate],a.[deldate], " &_
		"clicknum=b.vTotal,replytitle=c.intro,replydate=c.date7,ISNULL(h.name,c.name) AS replyname,s.sort1 as sortname,g.name as cateName " &_
		"FROM learnhd a " &_
		"LEFT JOIN (SELECT count(1) AS vTotal,hdid FROM learnhdview group by hdid) b ON b.hdid = a.ord " &_
		"LEFT JOIN (SELECT max(id) as id , ord FROM replyhd group by ord) e ON e.ord = a.ord "&_
		"left join replyhd c on c.id = e.id "&_
		" Left Join sortonehy s on a.sort2 = s.ord " &_
		" Left Join gate g on a.cateid = g.ord " &_
		"LEFT JOIN gate h ON h.ord = c.userID " &_
		" "&Str_Result&" " &_
		"ORDER BY " & Replace(pxStr,"replyhd.","a.")
	end if
	rs.open sql,conn,1,1
	CALL InitTempTable("learnhd",sql,"ord","cateid",73)
	if rs.RecordCount<=0 then
		...
		Response.write "" & vbcrlf & "     <form name=""form1"" method=""post"" id=""deleteFor"" action=""deleteall.asp?"
		...
		do until rs.eof
			...
			if clicknum="" or Isnull(clicknum) then
				clicknum=0
			end if
			if  A="3333333" or A="7777777" and timelg<>"" and datediff("s",timelg,rs("date7"))>0 then
				k=k&new_list
			end if
			...
			if open_73_3=3 or (open_73_3=1 and CheckPurview(intro_73_3,trim(cateid))=True) then
				Response.write "" & vbcrlf & "        <input name=""selectid"" type=""checkbox"" id=""selectid"" value="""
				Response.write rs("ord")
				Response.write """>" & vbcrlf & "      "
			end if
			Response.write "</td>" & vbcrlf & "      <td align=""center"" height=""27"">"
			Response.write n+1
			Response.write "</td>" & vbcrlf & "      <td align=""center"" height=""27"">"
			Response.write "</td>" & vbcrlf & "      <td class=""name"">&nbsp;" & vbcrlf & "          "
			if open_73_14=3 or (open_73_14=1 and CheckPurview(intro_73_14,trim(cateid))=True) then
				Response.write "" & vbcrlf & "        <a href=""javascript:void(0)"" onClick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs("ord"))
				Response.write "','learncon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" title=""点击可查看此信息详情"">" & vbcrlf & "" & vbcrlf & "          "
				'Response.write pwurl(rs("ord"))
				Response.write""&k&""
			...
			if A="7777777" or A="9999999" then
				if open_73_14=3 or (open_73_14=1 and CheckPurview(intro_73_14,trim(cateid))=True) then
					Response.write "<td class=""name""><div align=""left""><a href=""#"" onClick=""javascript:window.open('content.asp?ord="&pwurl(rs("ord"))&"','learncon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">"&rs("replytitle")&"</a>"
'if open_73_14=3 or (open_73_14=1 and CheckPurview(intro_73_14,trim(cateid))=True) then
					if rs("replydate")<>"" then  Response.write "<font class='red'>(<font class=""blue2"">"&rs("replyname")&"</font>&nbsp;"&rs("replydate")&")</font>"
					Response.write "&nbsp;<img src=""../images/jiantou.gif""> <a href='reply.asp?ord="&pwurl(rs("ord"))&"' onClick=""javascript:window.open('reply.asp?ord="&pwurl(rs("ord"))&"','learncon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;>交流</a>"
					Response.write "</div></td>"
				else
					Response.write "<td class=""name""><div align=""left""></div></td>"
				end if
			else
				if open_73_14=3 or (open_73_14=1 and CheckPurview(intro_73_14,trim(cateid))=True) then
					Response.write "<td class=""name""><div align=""left""><a href=""#"" onClick=""javascript:window.open('content.asp?ord="&pwurl(rs("ord"))&"','learncon','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">"&rs("replytitle")&"</a>"
'if open_73_14=3 or (open_73_14=1 and CheckPurview(intro_73_14,trim(cateid))=True) then
			...
			count = 0
			count1 = 0
			sql = "select count(1) from replyhd where ord="& rs("ord")
			set Rss = server.CreateObject("adodb.recordset")
			Rss.open sql,conn,1,1
			count=rss(0)
			rss.close
			set rss=Nothing
			sql = "select count(1) from huifu where sort="& rs("ord")
			set Rss = server.CreateObject("adodb.recordset")
			Rss.open sql,conn,1,1
			count1=rss(0)
			rss.close
			set rss=nothing
			Response.write "" & vbcrlf & "          <td  class=""name""><div align=""center""><span style=""cursor:pointer"" title=""交流数"">"
			Response.write count
			Response.write "</span>/<strong style=""cursor:pointer"" title=""回复数"">"
			Response.write CLng(count1)
			Response.write "</strong></div></td>" & vbcrlf & ""
			cateidname=rs("cateName")
			Response.write "" & vbcrlf & "    <td class=""name""><div align=""center"">"
			Response.write cateidname
			Response.write "</div></td>" & vbcrlf & "      "
			if A="7777777" or A="9999999" then
				Response.write "<td class=""name""><div align=""center"">"&rs("replyname")&"</div></td>"
			end if
			Response.write "" & vbcrlf & "    <td class=""name""><div align=""center""> "
			...
			if open_73_2=3 or CheckPurview(intro_73_2,trim(cateid))=True  then
				Response.write "" & vbcrlf & "    <input type=""button"" name=""Submit3"" value=""修改""  onClick=""javascript:window.open('correct.asp?id="
				Response.write pwurl(rs("ord"))
				Response.write "','newwin33','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')""/>" & vbcrlf & "    "
				'Response.write pwurl(rs("ord"))
			end if
			if (open_73_3=3 or CheckPurview(intro_73_3,trim(cateid))=True) And A<>"7777777" and A<>"9999999" then
				Response.write "" & vbcrlf & "    <input type=""button"" name=""Submit"" value=""删除"" onClick=""if(confirm('确认删除？')){window.location.href='Delete.asp?"
		...
		if (open_73_3=1 or open_73_3=3) And A<>"7777777" and A<>"9999999" then
			Response.write "<input class=""anybutton"" value=""批量删除"" width=""75"" height=""22"" border=""0"" onClick=""return test();"" type=""button"">"
		end if
		
		Response.write "</td>" & vbcrlf & "   </form>" & vbcrlf & "    <td width=""79%""><div align=""right"">" & vbcrlf & "     <span class=""black"">"
		...
		Response.write "条信息/页</span>&nbsp;" & vbcrlf & "    <input name=""currpage1"" id=""currpage1"" type=""text"" onpropertychange=""formatData(this,'int');"" maxlength=8 onkeyup=""checkDot('currpage1',0,8)"" size=""3"">" & vbcrlf & "     &nbsp;<input type=""button"" name=""Submit422"" value=""跳转"" class=""anybutton2"" onClick=""gotourl('currPage='+document.getElementById('currpage1').value);""/>" & vbcrlf & "      "
		Response.write rs.pagesize
		if currpage=1 then
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "          <tr>" & vbcrlf & "          <td height=""1038"" colspan=""3""></td>" & vbcrlf & "   </tr>" & vbcrlf & " </table>" & vbcrlf & "</td>" & vbcrlf & "</tr>" & vbcrlf & "</table>" & vbcrlf & ""
	end if
	rs.close
	set rs=nothing
	action1="互动列表"
	call close_list(1)
	Response.write "" & vbcrlf & "" & vbcrlf & "<div id=""User"" style=""position:absolute;height:100;display:none;padding-top:20px;"">" & vbcrlf & "       <table width=""150"" id = ""pxtab""  border=""0"" cellpadding=""-2"" cellspacing=""-2"">" & vbcrlf & "    <tr>" & vbcrlf & "            <td height=""139"">" & vbcrlf & "         <tablewidth=""150"" height=""115"" bgcolor=""#ecf5ff"" border=""0"" >" & vbcrlf & "                     <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=3');""><font color=""#2F496E"">按标题排序(降)</font></a></td></tr>" & vbcrlf & "                      <tr valign=""middle""><td height=""24""colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=4');""><font color=""#2F496E"">按标题排序(升)</font></a></td></tr> "& vbcrlf &     "                    "
	If sort = "" Or sort = "5555555" Or sort = "5555555" Then
		Response.write "" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=5');""><font color=""#2F496E"">按分类排序(降)</font></a></td></tr>" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=6');""><font color=""#2F496E"">按分类排序(升)</font></a></td></tr>" & vbcrlf & "                     "
	end if
	Response.write "" & vbcrlf & "                      <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=7');""><font color=""#2F496E"">按发布人排序(降)</font></a></td></tr>" & vbcrlf & "                    <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=8');""><font color=""#2F496E"">按发布人排序(升)</font></a></td></tr>" & vbcrlf & "                  <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=1');""><font color=""#2F496E"">按发布时间排序(降)</font></a></td></tr>" & vbcrlf & "                  <tr valign=""middle""><td height=""24"" colspan=""2"">&nbsp;&nbsp;<a href=""###"" onClick=""gotourl('px=2');""><font color=""#2F496E"">按发布时间排序(升)</font></a></td></tr>" & vbcrlf & "         </table>" & vbcrlf & "                </td>" & vbcrlf & "   </tr>" & vbcrlf & "   </table>" & vbcrlf & "</div>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "" & vbcrlf & "</html>"
%>
