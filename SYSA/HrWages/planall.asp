...
'If Not rs.Eof Then
	...
	rs1.close
	set rs1=nothing
	if open_10_11=3 then
		list_tj=""
	elseif open_10_11=1 then
		list_tj="and cateid in ("&intro_10_11&")"
	else
	...
	sorce2=0
	sorce3=0
	if open_10_11="1"  then
		set rs1=server.CreateObject("adodb.recordset")
		sql1="select ord,name,sorce,sorce2 from gate  where ord in ("&intro_10_11&") and del=1 order by sorce asc,sorce2 asc ,cateid asc ,ord asc"
		rs1.open sql1,conn,1,1
		if rs1.eof then
		else
			do until rs1.eof
		...
		Str_power3="and ord in ("&sorce3&")  "
		Str_power33="where ord in ("&sorce3&") "
	elseif open_10_11="3" then
...
'function CurrCostDate(cn)
	...
	if open_10_13=3 then
		list_tj=""
	elseif open_10_13=1 then
		list_tj=" and cateid in ("&intro_10_13&") "
	else
		list_tj=" and cateid=0 "
	end if
	Str_Result2=" and del=1 "&list_tj&" "
	...
	if m1<>"" then
		m1= Replace(m1,"/","-")
'if m1<>"" then
		if InstrRev(m1,"-")<7 then  m1=m1&"-1"
'if m1<>"" then
	end if
	if m2<>"" then
		m2= Replace(m2,"/","-")
'if m2<>"" then
		if InstrRev(m2,"-")<7 then  m2=m2&"-1"
'if m2<>"" then
	end if
	if m2<m1 Then m1=m2
	if m1<>"" and m2<>"" Then str_Result2=" and date2>='"&m1&"' and date2<='"&m2&"'"
	...
	f=request("f")
	if a="" Then a=1
	if a = "1" or a="3" or a="4" or a="5" then
		order="order by date3 desc,date7 desc"
	elseif a = "2" then
		order="order by money1 desc"
	else
		order="order by date7 desc"
	end if
	if Str_Result="" Then Str_Result = " where 1=1 "
	if a="3" then
		Str_Result=Str_Result+"and  year(date3)="&m2&" and month(date3)="&m1&""
'if a="3" then
	elseif a="4" then
		Str_Result=Str_Result+"and  year(date3)="&m2&""
'elseif a="4" then
	elseif a="5" then
		Str_Result=Str_Result+"and  date3='"&date&"'"
'elseif a="5" then
	end if
	if b<>"" Then Str_Result=Str_Result+" and  sort='"&b&"'"
'elseif a="5" then
	if c<>"" Then Str_Result=Str_Result+" and  complete1="&c&""
'elseif a="5" then
	if d<>"" Then Str_Result=Str_Result+" and  zt1="&d&""
'elseif a="5" then
	if e<>"" Then Str_Result=Str_Result+" and  zt2="&e&""
'elseif a="5" then
	Dim remindId,helper
	remindId = request("remindId")
	Set helper = CreateReminderHelper(conn,110,0)
	...
	Response.write Application("sys.info.jsver")
	Response.write """ rel=""stylesheet"" type=""text/css"">" & vbcrlf & "<style type=""text/css"">" & vbcrlf & "<!--" & vbcrlf & "body {" & vbcrlf & "     margin-top: 0px;" & vbcrlf & "}" & vbcrlf & "-->" & vbcrlf & "</style>" & vbcrlf & "</head>" & vbcrlf & "<body class=""ReportUI"">" & vbcrlf & "<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">" & vbcrlf & "<tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "         <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">"
	if a=0 then
		Response.write "未发放"
	elseif a=1 then
		Response.write "已发放"
	end if
	Response.write "工资表</td>" & vbcrlf & "             <td  align=""left"">&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "      </table> " & vbcrlf & "    <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "     <tr class=""top"">" & vbcrlf & "    <td width=""5%"" align=""center""><div align=""center""><strong>选择</strong></div></td>" & vbcrlf & "    <td width=""6%"" align=""center""><div align=""center""><strong>序号</strong></div></td>" & vbcrlf & "       <td width=""13%"" align=""center""><div align=""center""><strong>工资主题</strong></div></td>" & vbcrlf & "       <td width=""10%"" align=""center""><div align=""center""><strong>工资月份</strong></div></td>" & vbcrlf & "       <td width=""11%"" height=""27"" align=""center""><div align=""center""><strong>工资总额</strong></div></td>" & vbcrlf & "     <td width=""13%"" ><div align=""center""><strong>发放状态</strong></div></td>" & vbcrlf &  "  "
	If a=0 then
		Response.write "" & vbcrlf & "      <td width=""8%"" ><div align=""center""><strong>审批人员</strong></div></td>" & vbcrlf & "      "
	end if
	If a=1 Then
		Response.write "" & vbcrlf & "        <td width=""13%""><div align=""center"">支付账户</div></td>" & vbcrlf & "     "
	end if
	Response.write "" & vbcrlf & "        <td width=""8%""><div align=""center""><strong>经手人</strong></div></td>" & vbcrlf & "     "
	If a=1 then
		Response.write "" & vbcrlf & "        <td width=""9%""><div align=""center"">发放日期</div></td>" & vbcrlf & "      "
	end if
	Response.write "" & vbcrlf & "        <td width=""23%"" ><div align=""center""><strong>操作</strong></div></td>" & vbcrlf & "       </tr>" & vbcrlf & ""
	If a = 0 Then
		hrmsql=""
		if ZBRuntime.MC(39000) then
			hrmsql=" and y.nowStatus<>4 and y.contractEnd>='"&date&"' and y.contractStart<='"&date&"'"
		end if
		sql1 = ""
		if open_10_13 <> 3 then
			if cstr(open_10_13) = "0" then
				sql1="and 1=0"
			else
				sql1="and x.ord in (" & intro_10_13 & ") "
			end if
		end if
		sql1 = "isnull((select sum(y.money1*y.updown) from gate_person x inner join wageslist y on y.wages=wages.id and x.ord=y.cateid " & sql1 & "),0) as money1"
	else
		sql1 = "money1"
	end if
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id,title," & sql1 & ",bank,complete1,cateid,del,delcate,deldate,date1,date2,date3,date7,bz,salaryClass,ModifyStamp,complete2,cateid_sp ,  "&_
	" (select count(1) cnt from wageslist where wages=wages.id and iscostcollect=1) cnt "&_
	"  from wages where del=1 and isnull(salaryClass,0)>0 " & iif(remindId&""<>"",""," and complete1="&a) & str_result2&" order by date7 desc,date3 desc"
	rs1.open sql1,conn,1,1
	if rs1.RecordCount<=0 then
		Response.write "<tr><td  colspan='11' height=30>没有信息!</td></tr>"
	else
		n=0
		rs1.PageSize=15
		PageCount=clng(rs1.PageCount)
		...
		Dim rid,canCancel
		do until rs1.eof
			if not isnull(rs1("bz")) and rs1("bz")<>0  then
				bzflag = getrsval("select top 1 intro from sortbz where id="&rs1("bz"))
			else
				bzflag = ""
			end if
			rid = helper.getRemindIdByOID(rs1("id"))
			canCancel = helper.canCancelOrder(rid)
			id=rs1("id")
			money1=0
			complete1 = rs1("complete1")
			complete2 = rs1("complete2")
			if isnull(complete2) then complete2=1
			cateid_sp = rs1("cateid_sp")
			date2=Replace(rs1("date2"),"/","-")
			cateid_sp = rs1("cateid_sp")
			Response.write "" & vbcrlf & "     <tr onmouseout=""this.style.backgroundColor=''"" onmouseover=""this.style.backgroundColor='efefef'"">" & vbcrlf & "   <td align=""center"" class=""name"">"
			if open_10_3=3 or CheckPurview(intro_10_3,trim(rs1("cateid")))=True then
			...
			Response.write "</td> " & vbcrlf & "       <td align=""center"" class=""name"">"
			Response.write Rs1.recordcount-Rs1.pagesize*(currpage-1)-n
			'Response.write "</td> " & vbcrlf & "       <td align=""center"" class=""name"">"
			Response.write "</td>" & vbcrlf & "        <td align=""center"" class=""name"">" & vbcrlf & "            "
			Response.write rs1("title")
			If canCancel Then
				Response.write "" & vbcrlf & "             <img src=""../images/alt3.gif"" class=""cancelBtn"" style=""cursor:hand"" alt=""取消提醒"" rid="""
				Response.write rid
				Response.write """ border=""0""/>" & vbcrlf & ""
			end if
			Response.write "" & vbcrlf & "     </td>" & vbcrlf & "   <td align=""center"" class=""name"">"
			Response.write Split(date2,"-")(0)&"年"&Split(date2,"-")(1)&"月"
			Response.write "" & vbcrlf & "     </td>" & vbcrlf & "   <td align=""center"" class=""name"">"
			Response.write "</td>" & vbcrlf & "        <td class=""name""  align=""center""><div align=""right"">"
			Response.write  bzflag&"  "&Formatnumber(rs1("money1"),num_dot_xs,-1)
			'Response.write "</td>" & vbcrlf & "        <td class=""name""  align=""center""><div align=""right"">"
			...
			if complete2=6 Then
				...
				if complete2=2 then
					Response.write("待审批")
				elseif complete2=3 then
					Response.write("审批中")
				elseif complete2=-1 then
					Response.write("审批中")
				...
				if ZBRuntime.MC(226100) then
					Response.write "" & vbcrlf & "    <img src=""../images/jiantou.gif""><a href=""#"" class=""blue2"" onClick=""loadSPmx("
					Response.write rs1("id")
					Response.write ");"">审批记录</a>" & vbcrlf & "  "
				end if
			end if
			...
			If a=0 then
				...
				if cateid_sp&""<>"" then
					set rs2 =conn.execute("select name from gate where ord="&cateid_sp&"")
					if rs2.eof=false then
			...
			if rs1("bank")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select sort1 from sortbank where id="&rs1("bank")&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
				else
					bank=rs7("sort1")
			...
			if rs1("cateid")<>"" then
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select name from gate where ord="&rs1("cateid")&""
				rs7.open sql7,conn,1,1
				if rs7.eof then
				else
					cateid=rs7("name")
			...
			If a=1 then
				Response.write "" & vbcrlf & "             <td class=""name"" ><div align=""center"">"
				Response.write rs1("date1")
				Response.write "</div></td>" & vbcrlf & "          "
			end if
			Response.write "" & vbcrlf & "             <td class=""func""> " & vbcrlf & "                             <div align=""center"">" & vbcrlf & "                            "
			if open_10_14=3 or CheckPurview(intro_10_14,trim(rs1("cateid")))=True then
				Response.write " " & vbcrlf & "                           <input type=""button"" name=""Submit32"" value=""详情""  onClick=""javascript:window.open('content.asp?ord="
				Response.write pwurl(rs1("id"))
				Response.write "&ret="
				Response.write m1
				Response.write "&ret2="
				Response.write m2
				Response.write "','newgzwin1','width=' + 1100 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" >" & vbcrlf & "                                 "
				'Response.write m2
			end if
			if complete2 = 6 And (open_10_13=3 or CheckPurview(intro_10_13,trim(rs1("cateid")))=True) then
				Response.write " " & vbcrlf & "                           <input type=""button"" name=""Submit32"" value=""编辑""  onClick=""javascript:window.open('edit.asp?ord="
				Response.write pwurl(rs1("id"))
				Response.write "&ret="
				Response.write m1
				Response.write "&ret2="
				Response.write m2
				Response.write "','newgzwin2','width=' + 1150 + ',height=' + 600 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100')"" >" & vbcrlf & "                               "
				'Response.write m2
			end if
			if complete2<>6 And (open_10_16=3 or CheckPurview(intro_10_16,trim(rs1("cateid")))=True) and cateid_sp=session("personzbintel2007") then
				Response.write "" & vbcrlf & "                   <input type=""button"" name=""Submit3"" value=""审批""  onClick=""javascript:window.open('../wages/set.asp?ord="
				Response.write pwurl(rs1("id"))
				Response.write "','neww3win','width=' + 600 + ',height=' + 350 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');"" >" & vbcrlf & "                                  "
				'Response.write pwurl(rs1("id"))
			end if
			if complete2=2 or complete2=-1 then
				Response.write pwurl(rs1("id"))
				Response.write "<input type=""button"" name=""Submit3"" value=""修改""  onClick=""javascript:window.open('correct.asp?ord="
				'Response.write pwurl(rs1("id"))
				Response.write "&ret="
				Response.write m1
				Response.write "&ret2="
				Response.write m2
				Response.write "','newgzwin3','width=' + 960 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')"" >" & vbcrlf & "                   "
				Response.write m2
			end if
			if complete2=1 And complete1=0 then
				Response.write "" & vbcrlf & "                   <input type=""button"" name=""Submit3"" value=""发放""  onClick=""javascript:window.open('zf.asp?id="
				Response.write pwurl(rs1("id"))
				Response.write "&ret="
				Response.write m1
				Response.write "&ret2="
				Response.write m2
				Response.write "','newgzwin4','width=' + 600 + ',height=' + 400 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=200,top=150')"" >" & vbcrlf & "                                  "
				Response.write m2
			end if
			if (open_10_3=3 or  (open_10_3=1 and CheckPurview(intro_10_3,trim(rs1("cateid")))=True) ) and rs1("cnt").value=0  then
			...
			n=n+1
			rs1.movenext
			if rs1.eof or n>=rs1.PageSize then exit do
		loop
		Response.write "       " & vbcrlf & "      </table>" & vbcrlf & "   </td>" & vbcrlf & "          </tr>" & vbcrlf & "   <tr>" & vbcrlf & "     <td  class=""page"">" & vbcrlf & "      <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""10%"" height=""30""><div align=""center"">全选" & vbcrlf & "          <input type=""checkbox"" name=""checkbox2"" value=""Check All"" onClick=""mm()"">" & vbcrlf & "    </div></td>" & vbcrlf & "    <td >" & vbcrlf & ""
		if open_10_3=3 or CheckPurview(intro_10_3,trim(cateid1))=True Then
		...
		Response.write "页 | &nbsp;"
		Response.write rs1.pagesize
		Response.write "条信息/页&nbsp;&nbsp;" & vbcrlf & "        <input id=""curp1"" name=""currpage""  type=text  onkeyup=""value=value.replace(/[^\d]/g,'')""  size=3  maxlength=8/>&nbsp;" & vbcrlf & "                 <input type=""button"" name=""Submit422"" value=""跳转"" onClick=""window.location.href='planall.asp?currPage='+document.getElementById('curp1').value+'&a="
		'Response.write rs1.pagesize
		Response.write a
		Response.write "&b="
		Response.write b
		...
		if currpage=1 then
			...
			Response.write "" & vbcrlf & "          <input type=""button"" name=""Submit4"" value=""首页""   class=""page"" onClick=""window.location.href='planall.asp?currPage="
			Response.write  1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'""/> <input type=""button"" name=""Submit42"" value=""上一页""  onClick=""window.location.href='planall.asp?currPage="
		...
		if currpage=rs1.pagecount then
			...
			Response.write "" & vbcrlf & "         <input type=""button"" name=""Submit43"" value=""下一页""  onClick=""window.location.href='planall.asp?currPage="
			Response.write  currpage + 1
			Response.write "&a="
			Response.write a
			Response.write "&b="
			...
			Response.write "&c="
			Response.write c
			Response.write "'"" class=""page""/> <input type=""button"" name=""Submit43"" value=""尾页""  onClick=""window.location.href='planall.asp?currPage="
			Response.write  rs1.PageCount
			Response.write "&a="
			Response.write a
			Response.write "&b="
		...
		Response.write "" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "  <script src= ""../Script/Hs_planall.js?ver="
		Response.write Application("sys.info.jsver")
		Response.write """  language=javascript></script>" & vbcrlf & "  <tr>" & vbcrlf & "    <td height=""38"" colspan=""3""><div align=""right""><p>&nbsp;" & vbcrlf & "      </p>" & vbcrlf & "    </div></td>" & vbcrlf & "  </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>                    " & vbcrlf & ""
	end if
	rs1.close
	set rs1=nothing
	action1="工资表"
	call close_list(1)
	Response.write "" & vbcrlf & "" & vbcrlf & "<link rel=""stylesheet"" type=""text/css"" href=""../inc/themes/default/easyui.css?ver="
	Response.write Application("sys.info.jsver")
	...
	Response.write Application("sys.info.jsver")
	Response.write """></script>" & vbcrlf & "<div id=""w"" class=""easyui-window"" title=""工资表审批记录"" style=""width:700px;height:300px;padding:5px;background: #fafafa;top:200px; display:none""  closed=""true"" collapsible=""false"" minimizable=""false"" maximizable=""false"">" & vbcrlf & "    <div id=""dhtml"" region=""center"" border=""false"" style=""padding:5px;background:#fff;border:0px solid #ccc; width:98%; margin-top:5px;"">" & vbcrlf & "    </div>" & vbcrlf & "</div>   " & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
	'Response.write Application("sys.info.jsver")
