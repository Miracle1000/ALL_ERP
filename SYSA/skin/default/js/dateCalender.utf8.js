...
function fPopCalendar(evt, popCtrl, dateCtrl, showtime, showMonth, box) {
    ...
    with (ddlgGetElement("calendardiv").style) {
        ...
        if (showMonth) {
            width = "180px";
            point.left = ((point.left + 228 > clientWidth) ? (clientWidth - 235) : point.left)
        } else {
            point.left = (((point.left + 228) > clientWidth) ? (clientWidth - 235) : point.left)
        };
        var _s = 0;
...
function fHideCalendar() {
    ...
    if ((typeof Bill == 'object') && Bill.EnterTabKeyHandle) {
...
function getDateDiv(isMonth, a) {
    ...
    for (var i = 0; i < 60; i++) {
        ...
        if (i < 24) { optionsText2 += "<option value=" + v + ">" + v + "</option>" }
    }
    var dateDiv = "";
    dateDiv += "<table border='0' cellpadding='2' style='table-layout:fixed;width:100%;' cellspacing='0'>";
    ...
    dateDiv += "<tr>";
    dateDiv += "<td align='center'><span class='leftarrow img'  src='" + window.VirPath + "SYSN/skin/default/img/dateleft.gif' id='PrevMonth'  onclick='fPrevMonth(" + isMonth + ")'/><</span>";
    dateDiv += "</td><td align=center><div style='overflow:hidden; '><input id='tbSelYear' type='button' style='display:inline-block;box-sizing:border-box;width:70px;height:24px;line-height: 24px;background-postion:55px center;' onclick='selectionBoxscroll()' onchange='fUpdateCal(ddlgGetElementV(\"tbSelYear\"," + isMonth + "),ddlgGetElementV(\"tbSelMonth\"," + isMonth + ")," + isMonth + ")'>"
    ...
    dateDiv += "<span class='rightarrow img' id='NextMonth' src='" + window.VirPath + "SYSN/skin/default/img/dateright.gif'  onclick='fNextMonth(" + isMonth + ")'/>></span>";
    dateDiv += "</td>";
    dateDiv += "</tr><tr>";
    ...
    dateDiv += "<span class='todayDate' style='padding-left:5px;padding-right:5px;cursor:pointer;text-decoration: underline' onclick='fSetDate(giYear,giMonth,giDay," + isMonth + ",1)'>" + giYear + strYear + giMonth + strMonth + "</span>";
    if (!a) {
    ...
    dateDiv += "<span class='todayDate' style='padding-left:5px;padding-right:5px;cursor:pointer;text-decoration: underline' onclick='fSetDate(giYear,giMonth,giDay,null,1)'>" + strToday + ":" + giYear + strYear + giMonth + strMonth + giDay + strDay + "</span>";
    if (!a) {
...
function BindPageScrollVisible() {
    $('body').bind("scroll", function () {//因为在鼠标滚动的时候，日历框不会跟着动，但是又不能在body上加“position：relative”，所以滚动的时候隐藏；
        clearSelDate();
    })
    if ($("#rptframebody")) {
        $("#rptframebody").bind("scroll", function () {
            clearSelDate();
        })
    }
}
    ...
    cdiv: function (isMonth, a) {
        ...
        if (!d || oldIsMonth != isMonth) {
            ...
            if (app && app.IeVer && app.IeVer < 100) {
                $(document).bind("focus", "textarea", function () {
