...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	if L1<>"" Then
		v = "s"
		If Abs(datediff("y",L1,date))>40 Then
			v = "n"
		End If
		Str_Result=Str_Result+" and  datediff("& v &",'"&L1&"',a.time_login)>=0 "
'End If
	end if
	L2=ChangeDate(L2,2)
	if L2<>"" Then
		v = "s"
		If Abs(datediff("y",L2,date))>40 Then
			v = "n"
		End If
		Str_Result=Str_Result+" and  datediff("& v &",a.time_login,'"&L2&"')>=0 "
'End If
	end if
	Ptr_Result=""
	px=request("px")
	...
	if ret<>"" Then
		v = "s"
		If Abs(datediff("y",ret,date))>40 Then
			v = "n"
		End If
		Str_Result=Str_Result+" and  datediff("& v &",'"&ret&"',a.time_login)>=0 "
'End  If
	else
		ret="点击选择时间"
	end if
	...
	if ret2<>"" Then
		v = "s"
		If Abs(datediff("y",ret2,date))>40 Then
			v = "n"
		End If
		Str_Result=Str_Result+" and  datediff("& v &",a.time_login,'"&ret2&"')>=0 "
'End  If
	else
		ret2="点击选择时间"
	end if
	if C<>"" then
		if B="name" then
			Str_Result=Str_Result&" and b.username like '%"&ml.De(C)&"%'"
		elseif B="bh" then
			Str_Result=Str_Result&" and b.name like '%"&ml.De(C)&"%'"
		elseif B="xh" then
	...
	if not rs.eof then
		...
		rs.close
		set rs=nothing
		Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>日志管理导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!日志管理导出成功,共有"&(k)&"条记录!</B>"";</script>"
		Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
		Set fs = CreateObject("Scripting.FileSystemObject")
		tfile=Server.MapPath("日志管理_"&session("name2006chen")&".xls")
		...
		Response.write """><font class=""red""><strong><u>下载导出的日志管理</u></strong></font></a></p>"
