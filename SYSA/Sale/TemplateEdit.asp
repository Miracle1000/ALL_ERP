...
'If Not rs.Eof Then
	...
	List=SaveRequestForm("quest_list")
	If Action="save" Then
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select * from hf_Template where   title='"&title&"' and del=1"
		...
		rs.close
		Set Rs=Nothing
		MainID = GetIdentity("hf_Template","id","creator","")
		if List<>"" And IsNum(MainID) then
			ArrList=Split(List,",")
			For i=0 To Ubound(ArrList)
				Set Rs=server.CreateObject("adodb.recordset")
				Sql="select * from hf_TemplateList where  MainID="&MainID&" And QuestionID="&ArrList(i)&"  and del=1"
				Rs.Open Sql,conn,1,3
				If Rs.Eof Then
					Rs.AddNew
					Rs("MainID")=MainID
					Rs("QuestionID")=ArrList(i)
					Rs("InDate")=now
					Rs("Creator")=session("personzbintel2007")
					Rs("del")=1
					rs.update
		...
		Response.write "<script language='javascript'>if(window.parent.opener){ window.parent.opener.location.href='TempLateList.asp';window.parent.close();}else {window.parent.location.href='TempLateList.asp';}</script>"
		call db_close : Response.end
	ElseIf Action="edit" And IsNum(Ord) Then
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select * from hf_Template where id="&Ord&" and del=1"
		...
		Set Rs=Nothing
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select a.id as tmid, a.QuestionID,b.* from hf_TemplateList a inner join hf_Question b on a.QuestionID=b.id where a.MainID="&Ord&" and a.del=1 order by a.id"
		rs.open sql,conn,1,1
		If Not Rs.Eof Then
			Do While Not Rs.Eof
				Qtype=Rs("Qtype")
				...
				SQL="Select * from hf_QuestionLists where MainID="&Rs("id")&" And del=1 order by id desc"
				Rs1.Open SQL,Conn,1,1
				If Not Rs1.Eof Then
					i=1
					j=65
					Do While Not Rs1.Eof
						ItemList=ItemList&chr(j)&":"&Rs1("answer")&"("&Rs1("Point")&")"&"<br>"
						j=j+1
						ItemList=ItemList&chr(j)&":"&Rs1("answer")&"("&Rs1("Point")&")"&"<br>"
						rs1.movenext
					loop
				...
				Rs1.Close
				Set Rs1=Nothing
				TempList=TempList&"<tr id='quest_list_temp"&Rs("tmid")&"'><td><input name='quest_list' type='hidden' value='"&Rs("QuestionID")&"' /><div align='center'>"&Rs("title")&"</div></td><td><div align='center'>"&QtypeName&"</div></td><td><div align='left'>"&ItemList&"</div></td><td><div align='center'><input name='button' class='anybutton2' type='button' value='删除' onClick='delClassItem(this,"&Rs("tmid")&")'></div></td></tr>"
				rs.movenext
			Loop
		end if
		rs.close
	...
	ElseIf Action="update"  And IsNum(Ord) Then
		If conn.execute("select 1 from hf_Template where id<>"&Ord&" and Title = '"&Title&"'").eof = False Then
			Call jsAlert("回访模板标题重复！")
			call db_close : Response.end
		end if
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select * from hf_Template where   id="&Ord&" and del=1"
		...
		Response.write "<script language='javascript'>if(window.parent.opener){ window.parent.opener.location.href='TempLateList.asp?CurrPage="&CurrPage&"';window.parent.close();}else {window.parent.location.href='TempLateList.asp?CurrPage="&CurrPage&"';}</script>"
		call db_close : Response.end
	ElseIf Action="del"  And IsNum(Ord) Then
		CurrPage=request("CurrPage")
		Set rs=conn.execute ("select * from hf_configlist where template="& ord &" and del=1")
		If rs.eof Then
			Conn.execute("Delete hf_Template Where id="&Ord&"")
			Conn.execute("Delete hf_TemplateList Where MainID="&Ord&"")
			Call jsLocat("删除成功！","TempLateList.asp?CurrPage="&CurrPage)
		else
			Call jsLocat("该模板已被流程使用,不允许删除！","TempLateList.asp?CurrPage="&CurrPage)
		end if
		rs.close
		conn.close
		Set conn=Nothing
		call db_close : Response.end
	ElseIf Action="dellist" Then
		CurrPage=request("CurrPage")
		If OrdList<>"" Then
			Olist=""
			Set rs=conn.execute("select id from hf_Template where id not in (select template from hf_configlist where template in ("& OrdList &") and del=1) and id in ("& OrdList &")")
			If Not rs.eof Then
				While Not rs.eof
					If Olist="" Then
						Olist=rs("id")
					...
					rs.movenext
				wend
				Conn.execute("Delete hf_Template Where id in("&Olist&")")
				Conn.execute("Delete hf_TemplateList Where MainID in("&Olist&")")
				Call jsLocat("批量删除成功！","TempLateList.asp?CurrPage="&CurrPage)
			else
				Call jsLocat("所选择的模板不允许删除！","TempLateList.asp?CurrPage="&CurrPage)
			end if
			rs.close
		else
			Call jsLocat("请选择模板后再删除！","TempLateList.asp?CurrPage="&CurrPage)
		end if
		call db_close : Response.end
	end if
	...
	'Response.write Application("sys.info.jsver")
	Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script src= ""../Script/Se_TemplateEdit_2.js?ver="
	Response.write Application("sys.info.jsver")
	Response.write """  language=""javascript"" type=""text/javascript""></script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	If  Used="" Then
		Used=true
	elseif isnull(Used)  then
		Used=false
	end if
	Response.write "" & vbcrlf & "<form method=""post"" name=""demo""  action=""?action=update"" id=""demo"" onsubmit=""return Validator.Validate(this,2)"" target=""saveFrom"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "    <tr>" & vbcrlf & "      <td width=""100%"" valign=""top""><table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "          <tr>" & vbcrlf & "            <td class=""place"">回访模板"
	if action="edit" then
		Response.write "修改"
	else
		Response.write "添加"
	end if
	Response.write "</td>" & vbcrlf & "            <td>&nbsp;</td>" & vbcrlf & "            <td align=""right"">&nbsp;</td>" & vbcrlf & "            <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table>" & vbcrlf & "        <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr class=""top"">" & vbcrlf & "            <td height=""27"" colspan=""6"" ><div align=""left""><img src=""../images/jiantou.gif"" border=""0"" width=""16px"" height=""11px"" /><a href=""javascript:void(0);"" onClick=""TrToggle('tr_1')"">基本信息设置</a></div></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""15%""><div align=""right"">标题：</div></td>" & vbcrlf & "            <td width=""35%"" class=""gray""><input type=""text"" name=""title""  dataType=""Limit"" value="""
	Response.write title
	Response.write """ max=""50"" min=""1"" msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td width=""15%""><div align=""right"">是否启用：</div></td>" & vbcrlf & "            <td width=""35%"" ><input name=""Used"" type=""radio"" value=""1""  "
	If Used=true then
	...
	Response.write ">" & vbcrlf & "              停用</td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td align=""right"" width=""15%"" >回访内容:</td>        " & vbcrlf & "            <td colspan=""3""><table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1""bgcolor=""#C0CCDD"" class=""home"">" & vbcrlf & "                <tr class=""top"">" & vbcrlf & "                  <td width=""26%""><div align=""center"">题目</div></td>" & vbcrlf & "                  <td width=""25%""><div align=""center"">类型</div></td>" & vbcrlf & "                  <td width=""33%""><div align=""center"">选项</div></td>" & vbcrlf & "                  <td width=""20%""><div align=""center"">操作</div></td>" & vbcrlf & "                </tr>" & vbcrlf & ""
	if Action="edit" Then Response.write(TempList)
	Response.write "" & vbcrlf & "                <tr id=""StageList"">" & vbcrlf & "                  <td colspan=""5""><div align=""center""><img class=""resetElementHidden"" src=""../images/jiantou.gif"" /><img class='resetElementShow' style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png' /><a title=""点击添加明细"" onClick=""javascript:window.open('Search_Question.asp','newsquest','width=' + 900 + ',height=' + 800 + ',fullscreen =no,scrollbars=1,toolbar=0, status=no,resizable=1,location=no,menubar=no,menubar=no,left=200,top=100')"" href=""javaScript:void(0)"">添加明细</a></div></td>" & vbcrlf & "                </tr>" & vbcrlf & "              </table></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "    <tr>" & vbcrlf & "      <td  class=""page""><table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "     <tr>" & vbcrlf & "            <td height=""30"" ><div align=""center"">"
	if action="edit" then
		Response.write "" & vbcrlf & "                             <input name=""button22"" class=""anybutton"" onClick=""javascript:document.all.demo.action='?action=update&type=save&ord="
		Response.write pwurl(ord)
		Response.write "&CurrPage="
		Response.write request("CurrPage")
		Response.write "'""  value="" 保存 "" type=""submit"">" & vbcrlf & "              "
	else
		Response.write "" & vbcrlf & "                             <input name=""button23"" class=""anybutton"" value="" 保存 ""  onClick=""javascript:document.all.demo.action='?action=save'"" type=""submit"">" & vbcrlf & "              "
	end if
	Response.write "" & vbcrlf & "                       <input name=""reset"" class=""anybutton""  value="" 重填 "" type=""reset"">" & vbcrlf & "                     </div></td>" & vbcrlf & "          </tr>" & vbcrlf & "        </table></td>" & vbcrlf & "    </tr>" & vbcrlf & "</table>" & vbcrlf & "</form>" & vbcrlf & "<iframe name=""saveFrom"" id=""saveFrom"" style=""width:0;height:0""></iframe>" & vbcrlf & "</body>" & vbcrlf & "</html>"
