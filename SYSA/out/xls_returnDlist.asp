...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	elseif open_101_10=1 then
		list=" and ret_addcateid in ("&intro_101_10&")"
	else
	...
	if open_101_1=3 then
	elseif open_101_1=1 then
		Str_Result=Str_Result&" and ret_addcateid in ("&intro_101_1&")"
	else
		Str_Result=Str_Result&" and ret_addcateid=0"
	end if
	px=request("px")
...
'if Exit Sub2<>"" then
	...
	j=1
	k=1
	sql="select a.*,b.reg_fid,b.prod_ph,b.prod_xlh,b.prod_sctime,b.prod_ystime,c.prod_name,c.prod_bh,c.prod_xh,d.sort1,f.pro_name,h.sto_name,r.ret_addcateid,r.ret_title,r.ret_time from O_proRetList a "&_
	...
	C1=rs.recordcount
	xlApplication.Init "返还明细表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			ret_time=rs("ret_time")
			prod_sctime=rs("prod_sctime")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,1).Value =rs("ret_title")
			prod_ystime=rs("prod_ystime")
			...
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,3).Value = rs("prod_name")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,3).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,3).HorizontalAlignment=1
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,4).Value = rs("prod_bh")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,4).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,5).Value = rs("prod_xh")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,5).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,6).Value = rs("sort1")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,6).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,7).Value =zbcdbl(rs("prod_num"))
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,7).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,8).Value =rs("prod_ph")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,8).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,9).Value =rs("prod_xlh")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,9).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,10).Value =prod_sctime
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,10).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,11).Value =prod_ystime
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,11).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,12).Value =rs("pro_name")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,12).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,13).Value =rs("sto_name")
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,13).font.Size=10
			prod_ystime=rs("prod_ystime")
			xlWorksheet.Cells(1+i,14).Value =rs("ret_intro")
			prod_ystime=rs("prod_ystime")
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>返还明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!返还明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("返还明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的返还明细表</u></strong></font></a></p>"
