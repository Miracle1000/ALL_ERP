...
'if Exit Sub2<>"" and Exit Sub2<>"点击选择时间" then
	...
	i=1
	sql="select a.*,b.use_id as fid,b.use_complete as complete,b.use_cateid,c.name,d.car_code,e.drivername from O_carUsemx a "&_
	...
	C1=rs.recordcount
	xlApplication.Init "申请明细表_"&session("name2006chen")&".xls",C1
	if not rs.eof then
		do until rs.eof
			if rs("use_catetype")=1 then
				use_catetype="公用"
			else
				use_catetype="私用"
			end if
			use_startime=rs("use_startime")
			use_endtime=rs("use_endtime")
			use_rettime=rs("use_rettime")
			if use_rettime=#1900-01-01# then
				use_rettime=rs("use_rettime")
				use_rettime=""
			end if
			...
			use_retcateid=rs("use_retcateid")
			if use_retcateid="" or isnull(use_retcateid) then use_retcateid=0
			sqlx="select * from gate where ord="&use_retcateid
			set rsx=conn.execute(sqlx)
			if not rsx.eof then
				use_retcateid=rsx("name")
			else
				use_retcateid=""
			end if
			set rsx=nothing
	...
	rs.close
	set rs=nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>申请明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!申请明细表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
	set rs=nothing
	Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
	Set fs = CreateObject("Scripting.FileSystemObject")
	tfile=Server.MapPath("申请明细表_"&session("name2006chen")&".xls")
	...
	Response.write """><font class=""red""><strong><u>下载导出的申请/派车明细</u></strong></font></a></p>"
