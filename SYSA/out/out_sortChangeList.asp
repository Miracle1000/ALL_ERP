...
'if strW3<>"" Then
	...
	elseif px=5 then
		px_Result1=" order by (case datediff(d,'1900-01-01 00:00:00',isnull(a.date2,'1900-01-01 00:00:00')) when 0 then datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord anddate2<a.date7),a.date7)),a.date7) else datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) end) desc,id desc"
		px_Result2=" order by (case datediff(d,'1900-01-01 00:00:00',isnull(a.date2,'1900-01-01 00:00:00')) when 0 then datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord anddate2<a.date7),a.date7)),a.date7) else datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) end) asc,id asc"
	elseif px=6 then
		px_Result1=" order by (case datediff(d,'1900-01-01 00:00:00',isnull(a.date2,'1900-01-01 00:00:00')) when 0 then datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord anddate2<a.date7),a.date7)),a.date7) else datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) end) asc,id asc"
		px_Result2=" order by (case datediff(d,'1900-01-01 00:00:00',isnull(a.date2,'1900-01-01 00:00:00')) when 0 then datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord anddate2<a.date7),a.date7)),a.date7) else datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) end) desc,id desc"
	end if
	Dim ord,ordvalue,states,cateid,sort,sort1,sort2,sort2_1,sort3,sort3_1
	sort=request("sort")
	sort1=request("sort1")
	...
	If states="1" Or states="-1" Or states="0" then
		cateid=request("cateid")
		states= " and state="&states
	else
		states = " and a.presort1<>a.newsort1"
	end if
	If Len(cateid)>0 Then
		states= states & " and cateid in("&cateid&")"
	end if
	If Len(sort&"")>0 And Len(sort1&"")>0 Then
		If states="1" then
			states=states & " and newsort1=" & sort1 &" and newsort="&sort
		ElseIf states="-1" Then
			states=states & " and newsort1=" & sort1 &" and newsort="&sort
			states=states & " and presort1=" & sort1 &" and presort="&sort
		end if
	end if
	...
	If Len(contexts&"")>0 then
		...
		Case 1:
		Str_Result8=Str_Result8&" and cateadd in(select ord from gate where charindex('" & contexts & "',name)>0) "
		Case 2:
	...
	Const tourusql="(case datediff(d,'1900-01-01 00:00:00',isnull(a.date2,'1900-01-01 00:00:00')) when 0 then datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) else datediff(d,isnull((select top 1 date7 from tel_sort_change_log where tord=a.tord and presort1<>a.presort1 and id<a.id order by id desc),isnull((select date2 from tel where ord=a.tord and date2<a.date7),a.date7)),a.date7) end)"
	Dim sflg
	sflg=request("sflg")
	If sflg="1" Then
		Str_Result8=""
		...
		If sort2>0 Then
			states= " and presort="&sort2
			If sort2_1>0 Then
				states= states & " and presort1="&sort2_1
			end if
		end if
		If sort3>0 Then
			states= states & " and newsort="&sort3
			If sort3_1>0 Then
				states= states & " and newsort1="&sort3_1
			end if
		end if
		If Len(sday&"")>0 And isnumeric(sday)=True Then
			Str_Result8=Str_Result8 & " and " & tourusql &">="&sday
		end if
		If Len(eday&"")>0 And isnumeric(eday)=True Then
			Str_Result8=Str_Result8 & " and " & tourusql &"<="&eday
		end if
	end if
	Dim H
	H=request("H")
	If H="35" Then
		Dim tel_jz
		Dim tel_trade
		Dim tel_ly,zdy5,zdy6
		Dim areaall , areaname
		str_Result=""
		tel_jz=getint(request("jz"))
		If tel_jz>0 Then str_Result=str_Result & " and b.jz in(" & tel_jz & ")"
		tel_trade=getint(request("trade"))
		If tel_trade>0 Then str_Result=str_Result & " and b.trade in(" & tel_trade & ")"
		If Len(Trim(request("area")))>0 Or Trim(request("area"))&""<>"" Then
			areaall = Trim(request("area"))
			str_Result=str_Result & " and b.area in(" & areaall & ")"
			areaname = ""
			Set rs2 = conn.execute("select menuname from menuarea where id in(" & areaall & ")")
			While Not rs2.eof
				areaname = areaname & rs2("menuname") &","
				rs2.movenext
			wend
			rs2.close
			Set rs2 = Nothing
...
'If areaname<>"" Then
		...
		tel_ly=getint(request("ly"))
		If tel_ly>0 Then str_Result=str_Result & " and b.ly in(" & tel_ly & ")"
		zdy5=getint(request("zdy5"))
		zdy6=getint(request("zdy6"))
		If zdy5>0 Then str_Result=str_Result & " and b.zdy5 in(" & zdy5 & ")"
		If zdy6>0 Then str_Result=str_Result & " and b.zdy6 in(" & zdy6 & ")"
		states=request("state")
		If Len(sort&"")>0 And Len(sort1&"")>0 Then
			If states="1" then
				Str_Result8 = " and datediff(d,date7,'"&time1&"')=0 and cateid="& session("personzbintel2007") &" and newsort=" & sort & " and newsort1=" & sort1 & " and isnull(state,0)<>0 and tord in(select b.ord from tel b where b.ord=a.tord " & str_Result & ")"
			ElseIf states="-1" Then
				Str_Result8 = " and datediff(d,date7,'"&time1&"')=0 and cateid="& session("personzbintel2007") &" and presort=" & sort & " and presort1=" & sort1 & " and isnull(state,0)<>0 and tord in(select b.ord from tel b where b.ord=a.tord " & str_Result & ")"
			end if
		elseIf Len(sort&"")>0 then
			If states="1" then
				Str_Result8 = " and datediff(d,date7,'"&time1&"')=0 and cateid="& session("personzbintel2007") &" and newsort=" & sort & " and isnull(state,0)<>0 and tord in(select b.ord from tel b where b.ord=a.tord " & str_Result & ")"
			ElseIf states="-1" Then
				Str_Result8 = " and datediff(d,date7,'"&time1&"')=0 and cateid="& session("personzbintel2007") &" and presort=" & sort & " and isnull(state,0)<>0 and tord in(select b.ord from tel b where b.ord=a.tord " & str_Result & ")"
			end if
		end if
		states=""
	end if
...
'ElseIf open_1_10=1 Then
	...
	xApp.init Me, conn
	Set xsheet = xApp.sheets.add("客户阶段变更表")
	If ord>0 Then
		xsheet.showHeader "操作时间,变更前阶段,变更后阶段,变更方式,销售人员,投入时间,操作人,领用时间,添加时间"
	else
		xsheet.showHeader "操作时间,变更前阶段,变更后阶段,变更方式,销售人员,投入时间,操作人,关联客户,领用时间,添加时间"
	end if
	xsheet.movenext
	i=0
	set rs1=server.CreateObject("adodb.recordset")
	sql1="select id as ID_ID,tord as tord_ID, date7 as 操作时间," & _
	" isnull((select sort1 from sort4 where ord=a.presort),'')+'/'+isnull((select sort2 from sort5 where ord=a.presort1),'') as 变更前阶段," & _
	"isnull((select sort1 from sort4 where ord=a.newsort),'')+'/'+isnull((select sort2 from sort5 where ord=a.newsort1),'') as 变更后阶段," & _
	"isnull(reason,0) as 变更方式," & _
	"  isnull((select name from gate where ord=a.cateid),'')  as 销售人员," & _
	" " & tourusql & " as 投入时间, " & _
	"isnull((select name from gate where ord=a.cateadd),'') as 操作人," & _
	"  date2 as 领用时间," & _
	"  isnull((select date1 from tel where ord=a.tord),'') as 添加时间" & _
	"  from tel_sort_change_log a where 1=1 " &states&"" & strCondition2 & px_Result1
	rs1.open sql1,conn,1,1
	C1 = rs1.recordCount
	if rs1.eof=True Then
		...
		Do While Not rs1.eof
			xsheet.writedate rs1("操作时间")
			xsheet.writehtml "<td width='150' align='center'>"&rs1("变更前阶段")&"</td>"
			xsheet.writehtml "<td width='150' align='center'>"&rs1("变更后阶段")&"</td>"
			Select Case rs1("变更方式")
...
'Select Case rs1("变更方式")
			End Select
			xsheet.writehtml "<td width='100' align='center'>"&rs1("销售人员")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&"<div style='text-align:center'>"&rs1("投入时间").value&" 天</div>"&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&rs1("销售人员")&"</td>"
			xsheet.writehtml "<td width='100' align='center'>"&rs1("操作人").value&"</td>"
			If ord>0 Then
			else
	...
	rs1.close
	set rs1=Nothing
	Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户阶段变更表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户阶段变更表"&i&"条导出成功!</B>"";</script>"
	tfile=Server.MapPath("客户阶段变更表_"&session("name2006chen")&".xls")
	xApp.save tfile
	xApp.Dispose
	tfile = xApp.HexEncode(tfile)
	Set xApp = Nothing
	Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
	Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'客户阶段变更表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
	action1="客户阶段变更表导出"
	call close_list(1)
	Response.write "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
	Response.write tfile
	Response.write """><font class=""red""><strong><u>下载导出的客户阶段变更表</u></strong></font></a></p>" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
