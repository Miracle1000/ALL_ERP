	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			if rs1.eof then
				...
				If intro_41_1&""<>"" Then intro_41_1 = Replace(intro_41_1," ","") Else intro_41_1 = "-222"
				intro_41_1=rs1("qx_intro")
			end if
			rs1.close
			sql1="select qx_open,qx_intro from power  where ord="&session("personzbintel2007")&" and sort1=9 and sort2=10"
			...
			if rs1.eof then
				...
				open_9_10=rs1("qx_open")
				intro_9_10=rs1("qx_intro")
				If intro_9_10&""<>"" Then intro_9_10 = Replace(intro_9_10," ","") Else intro_9_10 = "-222"
				intro_9_10=rs1("qx_intro")
			end if
			rs1.close
			set rs1=nothing
			...
			xApp.init Me, conn
			HeaderStr =  khname &",类型,回款状态,币种,应收金额,已收金额,应收余额,应收日期,底单编号,底单日期,付款方式,到账日期,备注,销售人员,关联合同,合同类型"
			jn = 0
			mainTitle = "收款明细表"
			...
			dim w,a ,b,c,d,e,f,sort1,sort2,order,m1,m2, fst
			fst = request("fst")
			m1=request("ret")
			m2=request("ret2")
			A=request("A")
			...
			If istype=1 Then
				if  Trim(Request("duepaydate1"))<>"" then
					m1=Trim(Request("duepaydate1"))
				else
					m1=""
				end if
				if  Trim(Request("duepaydate2"))<>"" then
					m2=Trim(Request("duepaydate2"))
				else
					m2=""
				end if
			end if
...
'if A="1" then
				elseif A="2" then
					Str_Result=Str_Result+"and  date2<='"&m2&" 23:59:59'"
'elseif A="2" then
				elseif A="3" or A="7" or A="8"  then
					Str_Result=Str_Result+"and  date5<='"&m2&" 23:59:59'"
'elseif A="3" or A="7" or A="8"  then
				elseif A="5" then
					Str_Result=Str_Result+"and  date3<='"&m2&" 23:59:59'"
'elseif A="5" then
				ElseIf A="10" then
					Str_Result=Str_Result+"and  date1<='"&m2&" 23:59:59'"
'ElseIf A="10" then
...
'elseif A="2" then
			elseif A="3" then
				Str_Result=Str_Result+"and  complete=3"
'elseif A="3" then
			elseif A="4" then
				Str_Result=Str_Result+"and  complete2=2"
'elseif A="4" then
			elseif A="5" then
				Str_Result=Str_Result+"and  complete2=3"
'elseif A="5" then
			elseif A="7" then
				Str_Result=Str_Result+"and  tc=0 and complete=3"
'elseif A="7" then
			elseif A="8" then
				Str_Result=Str_Result+"and  tc=1 and complete=3"
'elseif A="8" then
			end if
			hklx = request("hklx")
			If hklx&""<>"" Then
				str_Result = str_Result + " and  flag in ("& hklx &") "
'If hklx&""<>"" Then
			end if
			...
			if FK<>"10" then
				str_Result=str_Result+ " and contract in (select ord from contract where fqhk="&FK&" ) and flag=1"
'if FK<>"10" then
			end if
			if C<>""then
				if B="khmc" then
...
'if B="khmc" then
				elseif B="htzt" then
					str_Result=str_Result+"and contract in (select ord from contract where  title like '%"& C &"%') and flag=1"
'elseif B="htzt" then
				elseif B="htid" then
					str_Result=str_Result+"and contract in (select ord from contract where  htid like '%"& C &"%') and flag=1"
'elseif B="htid" then
				end if
			end if
			zmr=request("zmr")
			...
			if zmr<>"10" then
				str_Result=str_Result+ " and paybacktype="&zmr&" "
'if zmr<>"10" then
			end if
			...
			elseif px=3 then
				px_Result="order by date5 desc,date7 desc"
			elseif px=4 then
				px_Result="order by date5 asc,date7 asc"
			elseif px=5 then
				px_Result="order by  abs(money1) desc,date7 desc"
			elseif px=6 then
				px_Result="order by abs(money1) asc,date7 asc"
			elseif px=7 then
			...
			if request("hkzt")<>"" and request("hkzt")<>"10" then
				...
				if hkztStr<>"" then
					...
					next
					if trim(hkztStr2)<>"" then
						str_Result = str_Result + " and complete in("& replace(trim(hkztStr2)," ",",") &") and flag=1"
'if trim(hkztStr2)<>"" then
					end if
					if trim(hkztStr3)<>"" then
						str_Result = str_Result + " and complete2 in("& replace(trim(hkztStr3)," ",",") &") and flag=1"
'if trim(hkztStr3)<>"" then
					end if
				end if
			end if
			if hkjh="" or IsNull(hkjh) then
				hkjh=10
			end if
			if request("hkjh")<>"" and request("hkjh")<>"10" then
				hkjhStr = trim(request("hkjh"))
				hkjhStr = replace(replace(replace(hkjhStr," ",""),Chr(10),""),Chr(9),"")
				str_Result = str_Result + " and  contract in (select ord from contract where del=1 and isnull(status,-1) in (1,-1) and fqhk in("& hkjhStr &")) and flag=1 "
				hkjhStr = replace(replace(replace(hkjhStr," ",""),Chr(10),""),Chr(9),"")
			end if
...
'if Request("companyname")<>"" then
			...
			if  Request("duemoney1")<>"" and Request("duemoney2")<>"" then
				str_Result=str_Result+" and (abs(money1)  between "&Request("duemoney1")&" and "&Request("duemoney2")&")  "
'if  Request("duemoney1")<>"" and Request("duemoney2")<>"" then
			end if
			if request("paytype")<>"" then
				if cint(request("paytype"))>0 then
					str_Result=str_Result+" and  isnull(pay,0) ="&cint(request("paytype")) &" and flag=1"
'if cint(request("paytype"))>0 then
				end if
			end if
			if Request("paydate1")<>"" then
				str_Result=str_Result+" and date5>='"&Request("paydate1")&"'"
'if Request("paydate1")<>"" then
			end if
			if Request("paydate2")<>"" then
				str_Result=str_Result+" and date5<='"&Request("paydate2")&"'"
'if Request("paydate2")<>"" then
			end if
			if Request("invdate1")<>"" then
				str_Result=str_Result+" and date3>='"&Request("invdate1")&"'"
'if Request("invdate1")<>"" then
			end if
			if Request("invdate2")<>"" then
				str_Result=str_Result+" and date3<='"&Request("invdate2")&"'"
'if Request("invdate2")<>"" then
			end if
			if trim(request("invtype"))<>"" then
				if trim(request("invtype"))<>"0" then
					str_Result=str_Result+" and  isnull(tik,0) in("& replace(trim(request("invtype"))," ",",") &")"
'if trim(request("invtype"))<>"0" then
				end if
			end if
			if request("tikname")<>"" then
				str_Result=str_Result+" and tikname  like '%"& request("tikname") &"%'"
'if request("tikname")<>"" then
			end if
			if request("contractname")<>"" then
				str_Result=str_Result+" and contract in (select ord from contract where title and isnull(status,-1) in (1,-1) ) and flag=1"
'if request("contractname")<>"" then
			end if
			if request("intro")<>"" then
				str_Result=str_Result+" and t.intro like '%"& request("intro") &"%' "
'if request("intro")<>"" then
			end if
			if request("bz")<>"" then
				if cint(request("bz"))>0 then
					str_Result=str_Result+" and bz = "&cint(request("bz"))&" "
'if cint(request("bz"))>0 then
				end if
			end if
			if hkjh="" or IsNull("hkjh") then
			...
			if hkjh<>"10" then
				hkjhStr = trim(request("hkjh"))
				hkjhStr = replace(replace(replace(hkjhStr," ",""),Chr(10),""),Chr(9),"")
				str_Result = str_Result + " and  contract in (select ord from contract where del=1 and fqhk in("& hkjhStr &"))  and flag=1"
				hkjhStr = replace(replace(replace(hkjhStr," ",""),Chr(10),""),Chr(9),"")
			end if
			str_Result = str_Result & searchExtended_Simple(95, " flag=1 and t.ord")
			dim zmrstr1,zmrstr2,zmrstr3,zmrstr4,zmrstr5,zmrstr
			zmr_zj=request("zmr_zj")
			zmr_zc=request("zmr_zc")
			zmr_qc=request("zmr_qc")
			if zmr_zj<>"" then zmrstr1=zmrstr1 + " paybacktype="&zmr_zj&""
			zmr_qc=request("zmr_qc")
			if zmr_zc<>"" then zmrstr2=zmrstr2 + " paybacktype="&zmr_zc&" or paybacktype is null"
			zmr_qc=request("zmr_qc")
			if zmr_qc<>"" then zmrstr5=zmrstr5 + " paybacktype="&zmr_qc&""
			zmr_qc=request("zmr_qc")
...
'if zmrstr<>"" then
				str_Result=str_Result+ " and (" + zmrstr + ")"
'if zmrstr<>"" then
			end if
			Dim rowIndex,pageCount, sheetPage
			pageCount = 1
			rowIndex = 1
			sheetPage = 40000
			summoney1=0
			...
			set rs=server.CreateObject("adodb.recordset")
			if IsType=1 then
				sql = "select flag,company,contract,complete,complete2,money1,date1,date2,date3,date5,date7,title,cateid,paybacktype,del,t.intro,tik,tikname,pay,bz,s.bzname from ( "&_
				"          select a.ord,1 flag,a.company,contract,complete,complete2,isnull(a.money1,0) money1, "&_
				"          a.date1,a.date2,a.date3,date5,a.date7,title,a.cateid,paybacktype,a.del,a.intro,tik,tikname,pay,"&_
				"       (select bz from contract where ord =a.contract) as bz,t.name   "&_
				"          from payback a left join tel t on t.ord=a.company where a.del=1 "&_
				"          union all "&_
				"          select a.ord,2 flag,b.company,isnull(b.ord,0),complete,null,-isnull(b.money1,0) money1,a.date1,a.date2,null,null,a.date7,null,a.cateid,-1,a.del,a.intro,null,null,null,b.bz,t.name  "&_
				"          union all "&_
				"          from payout2 a "&_
				"          left join (select ord,company,money1,bz from contractth where del=1) b on a.contractth=b.ord "&_
				"       left join tel t on t.ord=b.company"&_
				"          where a.del=1"&_
				"  ) t left join (select intro as bzname,id as bzid from sortbz ) s on s.bzid=t.bz "& Str_Result
			else
				sql = "select flag,company,contract,complete,complete2,money1,date1,date2,date3,date5,date7,title,cateid,paybacktype,del,t.intro,tik,tikname,pay,bz,s.bzname from ( "&_
				"          select a.ord,1 flag,a.company,contract,complete,complete2,isnull(a.money1,0) money1, "&_
				"          a.date1,a.date2,date3,date5,a.date7,title,a.cateid,paybacktype,a.del,a.intro,tik,tikname,pay,"&_
				"       (select bz from contract where ord =a.contract) as bz,t.name  "&_
				"          from payback a left join tel t on t.ord=a.company where a.del=1 and ("& open_7_10 &"=3 or ("& open_7_10 &"=1 and charindex(','+cast(a.cateid as varchar(15))+',', ',"& intro_7_10 &",')>0)) "&_
				"       (select bz from contract where ord =a.contract) as bz,t.name  "&_
				"          union all "&_
				"          select a.ord,2 flag,b.company,isnull(b.ord,0),complete,null,-isnull(b.money1,0) money1,a.date1,a.date2,null,null,a.date7,null,a.cateid,-1,a.del,a.intro,null,null,null ,b.bz,t.name  "&_
				"          union all "&_
				...
				"          left join (select ord,company,money1,bz from contractth where del=1) b on a.contractth=b.ord "&_
				"       left join tel t on t.ord=b.company"&_
				"          where a.del=1 and "& MSellReturn &"=1  and ("& open_9_10 &"=3 or ("& open_9_10 &"=1 and charindex(','+cast(a.cateid as varchar(15))+',', ',"& intro_9_10 &",')>0)) "&_
				"       left join tel t on t.ord=b.company"&_
				"  ) t left join (select intro as bzname,id as bzid from sortbz ) s on s.bzid=t.bz  "& Str_Result &""
				If fst&"" = "" Then sql = sql & " and date1>='"&m1&"' and date1<='"&m2&" 23:59:59'"
			end if
			sql2 = sql
			sql = sql &" "&px_Result&""
			rs.open sql,conn,1,1
			C1=rs.recordcount
			If C1>sheetPage Then
				Set rs2 = conn.execute("select sum(应收金额) 应收金额,sum(应收金额_rmb) 应收金额_rmb, sum(已收金额) 已收金额,sum(已收金额_rmb) 已收金额_rmb from (select isnull(sum(t2.money1),0) 应收金额,isnull(sum(t2.money1),0)*ISNULL(h.hl,1) 应收金额_rmb,isnull(sum((case when t2.complete=3 or (t2.flag = 2 And t2.complete = 2) then t2.money1 else 0 end)),0) 已收金额,isnull(sum((case when t2.complete=3 or (t2.flag = 2 And t2.complete = 2) then t2.money1 else 0 end)),0)*ISNULL(h.hl,1) 已收金额_rmb from ("& sql2 &") t2 left join contract c on t2.contract=c.ord and t2.flag=1 left join contractth d on t2.contract=d.ord and t2.flag=2 left join hl h on (t2.flag=1 and h.bz=c.bz and h.date1=c.date3) or (t2.flag=2 and h.bz=d.bz and h.date1=d.date3) group by t2.contract,h.hl) t1")
				If rs2.eof=False Then
					money1_all=cdbl(rs2("应收金额"))
					money2_all=cdbl(rs2("已收金额"))
					money3_all = money1_all - money2_all
					money2_all=cdbl(rs2("已收金额"))
					Rmb_Money_All1 = cdbl(rs2("应收金额_rmb"))
					Rmb_Money_All2 = cdbl(rs2("已收金额_rmb"))
					Rmb_Money_All3 = Rmb_Money_All1 - Rmb_Money_All2
					Rmb_Money_All2 = cdbl(rs2("已收金额_rmb"))
				end if
				rs2.close
				Set rs2 = Nothing
			...
			if rs.RecordCount<=0  then
				...
				do until rs.eof
					i=i+1 : flag = rs("flag") : cateid=rs("cateid") : intro=rs("intro") : htord = rs("contract")
					xsheet.writecell "没有信息!"
					If htord&"" = "" Then htord = 0
					companyname = ""
					if htord&""<>"" And htord&""<>"0" then
						set rs1=server.CreateObject("adodb.recordset")
						sql1="select ord,name,ISNULL(share,'-222') CustomShare,cateid,sort3,del from tel where ord="& iif(rs("company")&""<>"",rs("company"),0)&""
						set rs1=server.CreateObject("adodb.recordset")
						rs1.open sql1,conn,1,1
						if rs1.eof then
							companyname="客户已删除"
							...
							If rs1("del") = 1 Then
								If khSort3 = 2 Then
									If (open_26_1=3 or CheckPurview(intro_26_1,trim(khcateid))=True) Then companyname=rs1("name")
								else
									...
									If (open_1_1=3 or CheckPurview(intro_1_1,trim(khcateid))=True) Or IsShare Then companyname=rs1("name")
								end if
							else
								If khSort3 = 2 Then
									companyname="供应商已删除"
						...
						If flag = 1 Then
							set rs1=server.CreateObject("adodb.recordset")
							sql1="select title,ISNULL(share,'-222') ContractShare,cateid from contract where ord="& htord &" and del=1"
							set rs1=server.CreateObject("adodb.recordset")
							rs1.open sql1,conn,1,1
							if rs1.eof then
								contractname="合同已删除"
								...
								ContractShare = rs1("ContractShare") : htcateid = rs1("cateid")
								share = ContractShare
								If InStr(1,","&share&",", ","&session("personzbintel2007")&",",1) > 0 Or share = "1" Then
									IsShare = True
								else
								...
								If (open_5_1=3 or CheckPurview(intro_5_1,trim(htcateid))=True) Or IsShare then
									contractname=rs1("title")
								else
									contractname=""
								end if
							end if
						...
						ElseIf flag = 2 Then
							set rs1=server.CreateObject("adodb.recordset")
							sql1="select title,cateid from contractth where ord="& htord &" and del=1"
							rs1.open sql1,conn,1,1
							if rs1.eof then
								contractname="销售退货单已删除"
							else
								thcateid = rs1("cateid")
								If (open_41_1=3 or CheckPurview(intro_41_1,trim(thcateid))=True) then
									contractname=rs1("title")
								else
									contractname=""
								end if
					...
					if rs("tik")<>"" then
						sql7="select sort1 from sortonehy where ord="&rs("tik")&""
						rs7.open sql7,conn,1,1
						if rs7.eof then
							tic=""
						else
					...
					If flag = 1 Then
						sql7="select hl.hl from hl inner join contract on contract.bz=hl.bz and contract.date3=hl.date1  where contract.ord="& htord
					ElseIf flag = 2 Then
						sql7="select hl.hl from hl inner join contractth on contractth.bz=hl.bz and contractth.date3=hl.date1  where contractth.ord="& htord
					end if
					rs7.open sql7,conn,1,1
					if not rs7.eof then
						if rs7("hl")&""<>"" then
					...
					money1=zbcdbl(rs("money1"))
					Rmb_SumMoney1 = Rmb_SumMoney1 + money1 * nowhl
					money1=zbcdbl(rs("money1"))
					complete=""
					money2 = 0
					...
					elseif rs("complete")="2" then
						...
						ElseIf flag = 2 Then
							money2 = money1
							complete="已退款"
							summoney2=summoney2+money1
							complete="已退款"
							Rmb_SumMoney2 = Rmb_SumMoney2 + money1 * nowhl
							complete="已退款"
						end if
					...
					xsheet.WriteHtmlCell typeStr ,"align='center'"
					xsheet.writecell complete
					xsheet.WriteHtmlCell sortbz ,"align='center'"
					xsheet.WriteHtmlCell Formatnumber(zbcdbl(rs("money1")),num_dot_xs,-1),"class='moneyAc'"
					xsheet.WriteHtmlCell sortbz ,"align='center'"
					xsheet.WriteHtmlCell money2,"class='moneyAc'"
					xsheet.WriteHtmlCell money1-money2,"class='moneyAc'"
					xsheet.WriteHtmlCell money2,"class='moneyAc'"
					xsheet.WriteHtmlCell date1,"align='center'"
					xsheet.WriteHtmlCell rs("title") ,"align='center'"
					...
					xsheet.writecell cateidname
					xsheet.writecell contractname
					xsheet.writecell payback_type
					...
					Response.Flush
					summoney3 = summoney1 - summoney2
					Response.Flush
					Rmb_SumMoney3 = Rmb_SumMoney1 - Rmb_SumMoney2
					Response.Flush
					rowIndex=rowIndex+1
					Response.Flush
					xsheet.movenext
					rs.movenext
					If rowIndex > sheetPage And rs.eof=false  Then
						xsheet.WriteHtmlCell "本页合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(summoney1,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "本页合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(summoney2,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "本页合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(summoney3,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "本页合计：","colspan='4' align='right'"
						xsheet.movenext
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_SumMoney1,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_SumMoney2,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_SumMoney3,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.movenext
						xsheet.WriteHtmlCell "所有合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(money1_all,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "所有合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(money2_all,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "所有合计：","colspan='4' align='right'"
						xsheet.WriteHtmlCell Formatnumber(money3_all,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "所有合计：","colspan='4' align='right'"
						xsheet.movenext
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_Money_All1,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_Money_All2,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.WriteHtmlCell "￥"&Formatnumber(Rmb_Money_All3,num_dot_xs,-1)&"","class='moneyAc'"
						xsheet.WriteHtmlCell "","colspan='4'"
						xsheet.writeHTML "</table>"
						If pageCount = 1 then
							xsheet.title = "收款明细(" & pageCount & "页)"
						end if
						pageCount = pageCount + 1
						xsheet.title = "收款明细(" & pageCount & "页)"
						Set xsheet = xApp.sheets.add("收款明细(" & pageCount & "页)")
						xsheet.showHeader HeaderStr
						xsheet.movenext
						rowIndex = 1
					end if
				loop
				hjStr1="" : hjStr2=""
				If rowIndex >sheetPage Then
					hjStr1="本页" : hjStr2="所有"
					xsheet.WriteHtmlCell hjStr1 &"合计：","colspan='4' align='right'"
					xsheet.WriteHtmlCell Formatnumber(summoney1,num_dot_xs,-1)&"","class='moneyAc'"
					xsheet.WriteHtmlCell hjStr1 &"合计：","colspan='4' align='right'"
					xsheet.WriteHtmlCell Formatnumber(summoney2,num_dot_xs,-1)&"","class='moneyAc'"
					xsheet.WriteHtmlCell hjStr1 &"合计：","colspan='4' align='right'"
					...
					xsheet.WriteHtmlCell "","colspan='4'"
					xsheet.movenext
					xsheet.WriteHtmlCell hjStr2 &"合计：","colspan='4' align='right'"
					xsheet.WriteHtmlCell Formatnumber(money1_all,num_dot_xs,-1)&"","class='moneyAc'"
					xsheet.WriteHtmlCell hjStr2 &"合计：","colspan='4' align='right'"
					xsheet.WriteHtmlCell Formatnumber(money2_all,num_dot_xs,-1)&"","class='moneyAc'"
					xsheet.WriteHtmlCell hjStr2 &"合计：","colspan='4' align='right'"
					...
					money1_all=summoney1
					money2_all=summoney2
					money3_all = summoney1 - summoney2
					money2_all=summoney2
					Rmb_Money_All1=Rmb_SumMoney1
					Rmb_Money_All2=Rmb_SumMoney2
					Rmb_Money_All3 = Rmb_SumMoney1 - Rmb_SumMoney2
					Rmb_Money_All2=Rmb_SumMoney2
					xsheet.WriteHtmlCell hjStr2 &"合计：","colspan='4' align='right'"
					xsheet.WriteHtmlCell Formatnumber(money1_all,num_dot_xs,-1)&"","class='moneyAc'"
					xsheet.WriteHtmlCell hjStr2 &"合计：","colspan='4' align='right'"
			...
			set rs=Nothing
			set rs7=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>收款明细表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!收款明细表导出成功,共有"&(i)&"条记录!</B>"";</script>"
			tfile=Server.MapPath("收款明细表_"&session("name2006chen") & ".xls")
			xApp.save tfile
			xApp.dispose
			tfile = xApp.HexEncode(tfile)
			Set xApp = Nothing
			Response.write "<script>exportProcBar.showExcelProgress(100," & C1 & "," & C1 & ")</script>"
			Response.write "<script>exportProcBar.addFileLink({fileUrl:'" & tfile & "',fileName:'收款明细表_"&session("name2006chen")&".xls',fileCnt:1})</script>"
			action1="收款明细表导出"
			call close_list(1)
			Response.write "" & vbcrlf & "" & vbcrlf & "<p align=""center""><a href=""downfile.asp?fileSpec="
			Response.write tfile
			Response.write """><font class=""red""><strong><u>下载导出的收款明细表</u></strong></font></a></p> " & vbcrlf & "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & "" & vbcrlf & "" & vbcrlf & "" & vbcrlf & ""
