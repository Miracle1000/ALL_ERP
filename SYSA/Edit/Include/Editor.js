...
window.getIEVer = function () {
	...
	if (window.HasActiveXObject) {
		...
		if (window.navigator.userAgent.indexOf("rv:11.0")>-1) { return 11; }
		else { return 10 }
	}
	else {
		return 100;
...
function setHTML(html) {
	...
	if (window.HasActiveXObject || window.navigator.userAgent.indexOf("rv:11.0") > -1) {
...
function IE11InsertHtml(html) {
	var docobj = eWebEditor.document;
	if (docobj.selection) {
		var s = docobj.selection;
		var r = s.createRange();
		if (r.length>0 && r.item) {
			r.item(0).outerHTML = html;
		} else {
			r.pasteHTML(html);
		}
	} else {
...
function isControlSelected(tag) {
	var doc = eWebEditor.document;
	var selection = doc.getSelection ? doc.getSelection() : doc.selection;
	if (selection && selection.type == "Control" && window.NotIE == false) {
		var oControlRange = eWebEditor.document.selection.createRange();
		if (oControlRange.item(0).tagName.toUpperCase() == tag) {
			return true;
		}
	} else {
		var win = doc.parentWindow;
		if (win && win.event && win.event.srcElement) {
			var obj = win.event.srcElement;
			if (obj.tagName.toUpperCase() == tag) {
				return true;
			}
		}
	}
...
function mapEdit(){
	...
	if (eWebEditor.document.selection.type == "Control") {
		var oControlRange = eWebEditor.document.selection.createRange();
		if (oControlRange.item(0).tagName.toUpperCase() == "IMG") {
			b = true;
		}
	}
	if (!b){
...
// 修正Undo/Redo
var historySelf = new Object; //ie9下编辑框显示异常:此处使用history变量名造成变量污染（window.history）,因此修改变量名
historySelf.data = [];
historySelf.position = 0;
historySelf.bookmark = [];

// 保存历史
function savehistorySelf() {
	if (bEditMode){
		if (historySelf.data[historySelf.position] != eWebEditor.document.body.innerHTML){
			var nBeginLen = historySelf.data.length;
			var nPopLen = historySelf.data.length - historySelf.position;
			for (var i=1; i<nPopLen; i++){
				historySelf.data.pop();
				historySelf.bookmark.pop();
			}

			historySelf.data[historySelf.data.length] = eWebEditor.document.body.innerHTML;
			if(eWebEditor.document.selection) {
				if (eWebEditor.document.selection.type != "Control"){
					historySelf.bookmark[historySelf.bookmark.length] = eWebEditor.document.selection.createRange().getBookmark();
				} else {
					var oControl = eWebEditor.document.selection.createRange();
					historySelf.bookmark[historySelf.bookmark.length] = oControl[0];
				}
			}
			if (nBeginLen!=0){
				historySelf.position++;
			}
		}
	}
}
...
// 初始历史
function inithistorySelf() {
	historySelf.data.length = 0;
	historySelf.bookmark.length = 0;
	historySelf.position = 0;
}

// 返回历史
function gohistorySelf(value) {
	savehistorySelf();
	// undo
	if (value == -1){
		if (historySelf.position > 0){
			eWebEditor.document.body.innerHTML = historySelf.data[--historySelf.position];
			sethistorySelfCursor();
		}
	// redo
	} else {
		if (historySelf.position < historySelf.data.length -1){
			eWebEditor.document.body.innerHTML = historySelf.data[++historySelf.position];
			sethistorySelfCursor();
		}
	}
...
// 设置当前书签
function sethistorySelfCursor() {
	if (historySelf.bookmark[historySelf.position]){
		r = eWebEditor.document.body.createTextRange()
		if (historySelf.bookmark[historySelf.position] != "[object]"){
			if (r.moveToBookmark(historySelf.bookmark[historySelf.position])){
				r.collapse(false);
				r.select();
			}
		}
...
var winbodysize = function () {
	...
	if (!tb) { return; }
    try { document.getElementById("disupload0").style.height = document.getElementById("disupload0").children[0].offsetHeight; } catch (e) { }//打印模板在改变高度时工具栏上面出现空白
	...
	if (window.IEVer == 9) {//ie9下编辑框显示异常,高度丢失
	    var td = document.getElementById("disupload0");
	    var div = document.getElementsByClassName("yToolbar");
	    td.style.height = "72px";
	    td.style.borderBottom = "1px solid #c0ccdd";
