...
'Set extrafields = GetExtraFields(bill.extra)
	...
	Function setLogisticsFields(cn , ecid , waybillnumber , WayMoney , num_dot_xs)
		Dim rs7 , so , rs2 , i
		If ZBRuntime.MC(217008) Then
			Response.write "" & vbcrlf & "               <tr>    " & vbcrlf & "                <td height=""30""><div align=""right"">快递公司：</div></td>" & vbcrlf & "            <td height=""30"">" & vbcrlf & "          <select name=""ecid"">" & vbcrlf & "                      "
			if ecid<>"" Then
				so = ""
				set rs7=cn.execute("select sort1,isnull(color,'') color,tagdata from sortonehy where ord="&ecid&"")
				if rs7.eof=false Then so=rs7("sort1")& iif(rs7("color")&""="","（手动）",iif(rs7("tagdata")&""="0","（未对接）",""))
				rs7.close
				...
				ecid = 0
				Response.write "" & vbcrlf & "                              <option value=""0""></option>" & vbcrlf & "                               "
			end if
			set rs2=cn.execute("select ord,sort1,isnull(color,'') color,tagdata from sortonehy where gate2=83 and del=1 and isStop=0 and ord<>"&ecid&" order by gate1 desc")
			if not rs2.eof then
				do until rs2.eof
					Response.write "" & vbcrlf & "                                      <option value="""
					Response.write rs2("ord")
					Response.write """>"
					Response.write rs2("sort1")& iif(rs2("color")&""="","（手动）",iif(rs2("tagdata")&""="0","（未对接）",""))
					Response.write "</option>" & vbcrlf & "                                     "
					i=i+1
					Response.write "</option>" & vbcrlf & "                                     "
					rs2.movenext
			...
			rs2.close
			set rs2=nothing
			Response.write "" & vbcrlf & "              </select>" & vbcrlf & "               </td>" & vbcrlf & "            <td height=""30""><div align=""right"">快递单号：</div></td>" & vbcrlf & "            <td height=""30""><input name=""waybillnumber"" id=""waybillnumber"" type=""text"" value="""
			Response.write waybillnumber
			Response.write """ size=""25"" style=""width:150px"" dataType=""Limit"" min=""0"" max=""50"" msg=""长度不要超过50个字""></td>" & vbcrlf & "                <td height=""30""><div align=""right"">快递费用：</div></td>" & vbcrlf & "            <td height=""30""><input name=""waymoney"" id=""waymoney"" type=""text"" value="""
			Response.write FormatNumber(WayMoney ,num_dot_xs,-1,0,0)
			Response.write """ size=""10"" " & vbcrlf & "         onkeyup=""value=value.replace(/[^\d\.]/g,'');checkDot('waymoney','"
			Response.write num_dot_xs
			Response.write "');"" " & vbcrlf & "              onfocus=""this.select();"" dataType=""LimitB"" min=""0"" max=""15""  msg=""金额长度必须在15位之内"">元</td>" & vbcrlf & "         </tr>" & vbcrlf & "           "
		end if
	end function
	Function showLogisticsFields(cn , ecid , waybillnumber , WayMoney , num_dot_xs)
		Dim rs7 , ecname
		If ZBRuntime.MC(217008) Then
			Response.write "" & vbcrlf & "              <style>" & vbcrlf & "         .echand{color:red}" & vbcrlf & "              @media print {" & vbcrlf & "                  .echand{display:none;}" & vbcrlf & "          }" & vbcrlf & "               </style>" & vbcrlf & "                <tr>" & vbcrlf & "            <td><div align=""right"">快递公司：</div></td>" & vbcrlf & "              "
			if ecid<>"" then
				set rs7=cn.execute("select sort1,isnull(color,'') color,tagdata from sortonehy  where ord="&ecid&"")
				if rs7.eof then
					ecname=""
				else
					ecname=rs7("sort1")& iif(rs7("color")&""="","<span class='echand'>（手动）</span>",iif(rs7("tagdata")&""="0","<span style='color:gray'>（未对接）</span>",""))
				end if
				rs7.close
				set rs7=nothing
			end if
			Response.write "" & vbcrlf & "              <td  class=""gray""><font class=""gray"">&nbsp;"
			Response.write ecname
			Response.write "</font></td>" & vbcrlf & "          <td ><div align=""right"">快递单号：</div></td>" & vbcrlf & "             <td width=""16%"" class=""gray""><font class=""gray"">&nbsp;"
			Response.write WaybillNumber
			Response.write "</font></td>" & vbcrlf & "          <td ><div align=""right"">快递费用：</div></td>" & vbcrlf & "             <td width=""16%"" class=""gray""><font class=""gray"">&nbsp;"
			Response.write FormatNumber(WayMoney ,num_dot_xs,-1,0,-1)
			Response.write "元</font></td>" & vbcrlf & "                </tr>" & vbcrlf & "           "
		end if
	end function
	Function showLogisticsList(cn ,fhord, complete1 , ecid , waybillnumber , WayTime)
		Dim ecname ,rs, rs7 , n , AcceptTime ,  State ,AcceptStation, date1 , oldDate1 ,weekdayStr, s1,s2, s3,s4,toppoint1,toppoint2 , toppoint3, toppoint4
		If ZBRuntime.MC(217008) Then
			If ecid&""="" Then ecid = 0
			set rs7=cn.execute("select sort1 from sortonehy where ord="&ecid&"")
			if rs7.eof then
				ecname=""
			else
				ecname=rs7("sort1")
			...
			rs7.close
			set rs7=Nothing
			Response.write "" & vbcrlf & "              <tr class=""top""><td colspan=""6"" ID=""wltop"" height=""30"" background=""../images/m_table_top.jpg""><strong class=""blue2 tableTitleLinks"">物流详情 "
			If Len(ecname)>0 Or Len(WaybillNumber)>0 Then
				Response.write "("
				Response.write ecname &"&nbsp;" & WaybillNumber
				Response.write ")"
			end if
			Response.write "</strong></td></tr>" & vbcrlf & "           "
			If fhord = 0 Then
				Response.write "<tr><td height=""30"" colspan=""6"" align=""center"" style='padding:20px'>抱歉，该快速单号没有关联的发货单，系统暂不支持其快递信息查询。</td></tr>"
			ElseIf Len(ecname)=0 Or Len(WaybillNumber)=0 Then
				Response.write "<tr><td height=""30"" colspan=""6"" align=""center"">暂无记录！</td></tr>"
			else
				If cn.execute("select isnull((select top 1 LogisticCode from Store_Logistics where OrderCode = " & fhord &" order by AcceptTime desc),'')")(0)&""<>waybillnumber&"" Then
					cn.execute("delete from Store_Logistics where OrderCode = " & fhord &" ")
				end if
				Call checkSendLogistics( cn, fhord  , WayTime , 0)
				Set rs = cn.execute("select * from Store_Logistics where OrderCode = " & fhord &" order by AcceptTime desc")
				If rs.eof=False Then
					n = 0
					Response.write "" & vbcrlf & "                              <tr>" & vbcrlf & "                            <td height=""30"" colspan=""6"" align=""center"">" & vbcrlf & "                                   <div style=""width:100%;padding-top:10px;padding-bottom:20px;"" align=""center"">" & vbcrlf & "                                               <table id=""logisticslist"" style=""border:0;border-spacing:0;width:80%;padding-top:10px;padding-right:50px;padding-bottom:10px;padding-left:50px;border-collapse:collapse"">" & vbcrlf & "                                           "
					n = 0
					While rs.eof=False
						AcceptTime = rs("AcceptTime").value
						State = rs("State").value
						AcceptStation =rs("AcceptStation").value
						date1 = FormatDateTime(AcceptTime , 2)
						weekdayStr = ""
						select case weekday(AcceptTime)
						case 1 : weekdayStr="周日"
						case 2 : weekdayStr="周一"
						case 3 : weekdayStr="周二"
						case 4 : weekdayStr="周三"
						case 5 : weekdayStr="周四"
						case 6 : weekdayStr="周五"
						case 7 : weekdayStr="周六"
						End Select
						If oldDate1 = date1 Then
							date1 = ""
							weekdayStr = ""
						else
							oldDate1 = date1
						...
						If n = 0 Then
							s1 = "_s" : s2 = "_s" : s3 = "_s" : s4 = "_s" : toppoint1 = "" : toppoint2 = "" : toppoint3 = "" : toppoint4=""
							Select Case State
							Case 1 : s1 = "" : toppoint1 = "toppoint"
							Case 2 :
							s1 = "" : s2 = "" : s3 = "" : toppoint3 = "toppoint"
							Case 5 :s1 = "" : s2 = "" : toppoint2 = "toppoint"
							Case 3 : s1 = "" : s2 = "" : s3 = "" : s4 = "" : toppoint4 = "toppoint"
							Case Else
							End Select
							Response.write "" & vbcrlf & "                                                             <tr><td colspan=""5"" align=""center"" style=""margin:0;padding:0"">" & vbcrlf & "                                                                                <table style=""border:0;border-spacing:0;width:100%;border-collapse:collapse;"">" & vbcrlf & "                                                                            <tr><td align=""center""><div style=""width:100%;height:110px;""><img src=""../images/proc_p1"
							Response.write s1
							Response.write ".png""></div><strong>揽件</strong></td>" & vbcrlf & "                                                                                    <td align=""center""><div style=""width:100%;height:110px;""><img src=""../images/proc_p2"
							Response.write s2
							Response.write ".png""></div><strong>运输</strong></td>" & vbcrlf & "                                                                                    <td align=""center""><div style=""width:100%;height:110px;""><img src=""../images/proc_p3"
							Response.write s3
							Response.write ".png""></div><strong>派件</strong></td>" & vbcrlf & "                                                                                    <td align=""center""><div style=""width:100%;height:110px;""><img src=""../images/proc_p4"
							Response.write s4
							Response.write ".png""></div><strong>签收</strong></td>" & vbcrlf & "                                                                            </tr>" & vbcrlf & "                                                                           <tr><td class=""top"" width=""25%"" align=""center""><div class="""
							Response.write toppoint1
							Response.write """></div></td>" & vbcrlf & "                                                                                     <td class=""top"" width=""25%"" align=""center""><div class="""
							Response.write toppoint2
							Response.write """></div></td>" & vbcrlf & "                                                                                     <td class=""top"" width=""25%"" align=""center""><div class="""
							Response.write toppoint3
							Response.write """></div></td>" & vbcrlf & "                                                                                     <td class=""top"" width=""25%"" align=""center""><div class="""
							Response.write toppoint4
							Response.write """></div></td>" & vbcrlf & "                                                                             </tr>" & vbcrlf & "                                                                           </table>" & vbcrlf & "                                                                        </td>" & vbcrlf & "                                                           </tr>" & vbcrlf & "                                                           <tr><td class=""proc top1"" style=""width:10%;"" align=""right"">" & vbcrlf & "                                                                           <div class=""leftline1"" align=""center""></div></td>"& vbcrlf &"                                                                     <td class=""proc top1"" style=""width:80px;"" align=""center"">"
							Response.write date1
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc top1"" style=""width:50px;"" align=""center"">"
							Response.write weekdayStr
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc top1"" style=""width:80px;"" align=""center"">"
							Response.write FormatDateTime(AcceptTime , 3)
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc top1"" align=""left"">"
							Response.write AcceptStation
							Response.write "</td></tr>" & vbcrlf & "                                                           "
						else
							Response.write "" & vbcrlf & "                                                             <tr><td class=""proc gray"" style=""width:10%;"" align=""right"">" & vbcrlf & "                                                                           <div class=""leftline2"" align=""center""></div></td>" & vbcrlf & "                                                                   <td class=""proc gray"" style=""width:70px;"" align=""center"">"
							Response.write date1
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc gray"" style=""width:50px;"" align=""center"">"
							Response.write weekdayStr
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc gray"" style=""width:70px;"" align=""center"">"
							Response.write FormatDateTime(AcceptTime , 3)
							Response.write "</td>" & vbcrlf & "                                                                        <td class=""proc gray"" align=""left"">"
							Response.write AcceptStation
							Response.write "</td></tr>" & vbcrlf & "                                                           "
						end if
						n = n + 1
						...
						rs.movenext
					wend
					Response.write "" & vbcrlf & "                                             </table>" & vbcrlf & "                                        </div>" & vbcrlf & "                          </td></tr>"
				else
					Response.write "<tr><td height=""30"" colspan=""6"" align=""center"">暂无记录！</td></tr>"
				end if
				rs.close
	...
	Function checkSendLogistics(cn ,fhord , WayTime , fromType)
		If WayTime&"" = "" Or cn.execute("select 1 from Store_Logistics where OrderCode = " & fhord &"").eof Then
			Call getLogisticsList(fhord ,fromType)
		ElseIf datediff("n", WayTime , now())>30 Then
			If cn.execute("select top 1 State from Store_Logistics where OrderCode = " & fhord &" order by AcceptTime desc")(0)<>3 Then
				Call getLogisticsList(fhord , fromType)
			end if
		end if
	end function
	Function getLogisticsList(ByVal ord , ByVal fromType)
		on error resume next
		Dim page, Http ,getLogisticsURL
		Set page = server.createobject(ZBRLibDLLNameSN & ".pageclass")
		getLogisticsURL= Replace(LCase(page.HomeUrl)&"****/****","sysa****/****", "sysn") & "/view/store/Logistics.ashx?ord="& ord &"&fromType=1"
		Set Http=server.createobject("ZBXml.XMLHTTP")
		Http.open "post",getLogisticsURL,False
		Http.setRequestHeader "Content-Type","application/x-www-form-urlencoded"
		Http.open "post",getLogisticsURL,False
		Http.Send()
		Set Http = Nothing
		If Err.number<>0 Then
			sdk.file.writealltext "C:\LogisticsError.txt", getLogisticsURL & "【"& Err.number &"="& Err.description&"】"
			Set Http=server.createobject("ZBXml.XMLHTTP")
			Http.open "post",getLogisticsURL,False
			Http.Send()
			Set Http = Nothing
	...
	Function DeleteSend(fhord, updateHtStatus)
		dim rs, rss, sql, sqltext, ContractID, numlist2, numlist4, fh1
		If fhord&"" = "" Then fhord = 0
		If updateHtStatus&"" = "" Then updateHtStatus = 0
		sql="select order1,kuout,isnull(complete1,0) as complete1,isnull((select sort1 from kuout where ord=send.kuout),0) as sort1 from send Where del=1 and ord="&fhord&" "
		set rs = conn.execute(sql)
		If rs.eof = False Then
			order1=rs("order1")
			kuout=rs("kuout")
		...
		rs.close
		set rs = nothing
		if order1<>"" and order1&""<>"0" Then
			If sorttype="2" Then
			else
				ContractID =order1
			end if
			if complete1&""="1" Then
				conn.execute("update c set num3 = isnull(num3,0)-s.num1,num2 = isnull(num2,0)- s.num1 from kuoutlist2 c inner join (select isnull(sum(num1),0) as num1,kuoutlist from sendlist where send="& fhord &" group by kuoutlist ) s on s.kuoutlist= c.id  ")
'if complete1&""="1" Then  '
			Else
				conn.execute("update c set num2 = isnull(num2,0)- s.num1 from kuoutlist2 c inner join (select isnull(sum(num1),0) as num1,kuoutlist from sendlist where send="& fhord &" group by kuoutlist ) s on s.kuoutlist= c.id  ")
'Else  '
			end if
			sqltext="Update send set del=2 ,delcate='"&session("personzbintel2007")&"',deldate=getdate() where ord="& fhord &""
			conn.Execute(sqltext)
			sqltext="Update sendlist set del=2 where send="& fhord &""
			conn.Execute(sqltext)
			sqltext="Update telalt set del=2 where fh1="& fhord &""
			conn.Execute(sqltext)
			numlist2=0
			numlist4=0
			fh1=0
			Set rss = conn.execute("select isnull(sum(num1),0) as num1,isnull(sum(num3),0) as num3 from kuoutlist2 where kuout="&kuout&" ")
			If rss.eof = False Then
				numlist2 = cdbl(rss("num1"))
				numlist4 = cdbl(rss("num3"))
			end if
			rss.close
			If numlist4 = 0 Then
				fh1=0
			elseif numlist4<numlist2 Then
				fh1=1
			elseif numlist4=numlist2 Then
				fh1=2
			end if
			sqltext="Update kuout set fh1="&fh1&"  where ord="&kuout&" "
			conn.execute(sqltext)
			If Len(ContractID & "")> 0 and updateHtStatus&""="1" Then
				conn.execute "exec [erp_contract_UpdateStatus] " & ContractID
			end if
		end if
	end function
	
	Sub Bill_OnPageInit
		Response.write "<style>" & vbcrlf & "                body{height:auto;position:initial;}" & vbcrlf & "             #search_cel {border-top:0px;}" & vbcrlf & "           #searchTD {height:100px;}" & vbcrlf & "               #LogisticsListDIV table {width:100%;}" & vbcrlf & "           #LogisticsListDIV table {border:#c0ccdd 1px solid;height:24px;padding:3px;table-layout:auto;}" & vbcrlf & "          #logisticslist .proc {background-color:#f1f1f1;height:30px;}" & vbcrlf & "            #logisticslist .top1 {color:#25ae5f;font-weight:bold;}" & vbcrlf & "          #logisticslist .top {background-image:url(../images/proc_normal.png);height:30px;}" & vbcrlf & "              #logisticslist .toppoint {width:30px;height:30px;background:url(../images/proc_point.png) no-repeat}" & vbcrlf & "              #logisticslist .leftline1 {width:28px;height:32px;background:url(../images/proc_green.png) no-repeat;}" & vbcrlf & "          #logisticslist .leftline2 {width:28px;height:32px;background:url(../images/proc_gray.png) no-repeat;}" & vbcrlf & "               #wltop{border-bottom:1px solid #c0ccdd}" & vbcrlf & " </style>"
'Sub Bill_OnPageInit
	end sub
	Sub Bill_OnInit(ByVal bill, ByVal t)
		...
		bill.edit=False
		bill.loadJs =True
		bill.groups(0).showbar = False
		Bill.fields.addHtml("", "bill_onLogisticsList" , 3 , 1).dbname="search"
	...
	Sub bill_onLogisticsList(byref field)
		Dim rs2
		Response.write "" & vbcrlf & "      <table style=""width:100%;height:100%"">" & vbcrlf & "            <tr>" & vbcrlf & "                    <td id=""searchTD"" align=""right"">快递公司：" & vbcrlf & "                          <select name=""ecid"" id=""ecid"">" & vbcrlf & "                                      <option value=""""></option>" & vbcrlf & "                                        "
		set rs2=cn.execute("select ord , sort1 from sortonehy where gate2=83 and del=1 and isStop=0 and tagdata='1' and color<>'' order by gate1 desc")
		if not rs2.eof then
			do until rs2.eof
				Response.write "" & vbcrlf & "                                                      <option value="""
				Response.write rs2("ord")
		...
		rs2.close
		set rs2=nothing
		Response.write "" & vbcrlf & "                              </select>&nbsp;" & vbcrlf & "                         请输入您要查询单号：<input name=""waybillnumber"" id=""waybillnumber"" type=""text"" value="""" size=""25"" style=""width:150px"" dataType=""Limit"" min=""0"" max=""50"" msg=""长度不要超过50个字"">&nbsp;<button class=""oldbutton4"" onclick=""searchLogisticsList()"">检索</button>" & vbcrlf & "                 </td>" & vbcrlf & "                   <td width=""30%"" align=""left"">&nbsp;<span id=""showerr"" style=""color:red""></span></td>" & vbcrlf & "            </tr>" & vbcrlf & "           <tr><td colspan=""2""><div id=""LogisticsListDIV""></div></td></tr>" & vbcrlf & "     </table>" & vbcrlf & "        "
	end sub
	Sub App_onSearchLogistics
		Dim Logistics_Company : Logistics_Company = app.getInt("Logistics_Company")
		Dim Logistics_BillNumber : Logistics_BillNumber = Trim(app.getText("Logistics_BillNumber")&"")
		Response.write "<table>"
		Dim rs,sql
		If Logistics_Company = "" Or Logistics_Company = 0 Then
			sql = "select top 1 ord, complete1,ecid ,waybillnumber,  WayTime from send where WaybillNumber like '%"& Logistics_BillNumber &"%' order by date2 desc"
		else
			sql = "select top 1 ord, complete1,ecid ,waybillnumber,  WayTime from send where ecid="& Logistics_Company &" and WaybillNumber like '%"& Logistics_BillNumber &"%' order by date2 desc"
		end if
		Set rs = cn.execute(sql)
		If rs.eof=False Then
			Call showLogisticsList(cn ,rs("ord").value, rs("complete1").value , rs("ecid").value , rs("waybillnumber").value , rs("WayTime").value)
		else
			Call showLogisticsList(cn ,0, 1 , -Logistics_Company , Logistics_BillNumber , "")
		end if
		rs.close
		Response.write "" & vbcrlf & "      </table>" & vbcrlf & ""
	end sub
	Sub App_onGetLogisticsUrl
		on error resume next
		Dim page, getLogisticsURL,Ord,rs,WayTime,sql
		Dim Logistics_Company : Logistics_Company = app.getInt("Logistics_Company")
		Dim Logistics_BillNumber : Logistics_BillNumber = Trim(app.getText("Logistics_BillNumber")&"")
		Set page = server.createobject(ZBRLibDLLNameSN & ".pageclass")
		If Logistics_Company = "" Or Logistics_Company = 0 Then
			sql = "select top 1 ord,WayTime,ecid from send where WaybillNumber like '%"& Logistics_BillNumber &"%' order by date2 desc"
		else
			sql = "select top 1 ord,WayTime,ecid from send where ecid="& Logistics_Company &" and WaybillNumber like '%"& Logistics_BillNumber &"%' order by date2 desc"
		end if
		Set rs = cn.execute(sql)
		If rs.eof=False Then
			Ord = rs("ord").value
			WayTime = rs("WayTime").value
			Logistics_Company = rs("ecid").value
		else
			Ord = 0
			WayTime = ""
			Logistics_Company = 0
		end if
		getLogisticsURL = Logistics_Company&"|OK"
		If WayTime&"" = "" Or cn.execute("select 1 from Store_Logistics where OrderCode = " & Ord &"").eof Then
			getLogisticsURL = Logistics_Company&"|"&Replace(LCase(page.HomeUrl)&"****/****","sysa****/****", "sysn") & "/view/store/Logistics.ashx?ord="& ord &"&fromType=1"
		ElseIf datediff("n", WayTime , now())>30 Then
			If cn.execute("select top 1 State from Store_Logistics where OrderCode = " & Ord &" order by AcceptTime desc")(0)<>3 Then
				getLogisticsURL = Logistics_Company&"|"&Replace(LCase(page.HomeUrl)&"****/****","sysa****/****", "sysn") & "/view/store/Logistics.ashx?ord="& ord &"&fromType=1"
			end if
		end if
		Response.clear()
		Response.write getLogisticsURL
	end sub
