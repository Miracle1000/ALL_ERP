...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	C=Request("C")
	if m1="" then
		m1 = Cstr(DateAdd("d",(day(Date)-1)*-1,Date))
'if m1="" then
	end if
	if m2="" then
		m2 = Cstr(Date)
	...
	if C<>"" then
		if A="1" then
			str_Result=str_Result+"and a.sort1 like '%"& C &"%'"
'if A="1" then
		end if
	end if
	...
	if px=1 then
		px_Result=" order by a.sort1 desc"
	elseif px=2 then
		px_Result=" order by a.sort1 asc"
	elseif px=3 then
		px_Result=" order by agoMoney desc"
	elseif px=4 then
		px_Result=" order by agoMoney asc"
	elseif px=5 then
		px_Result=" order by NowMoney desc"
	elseif px=6 then
		px_Result=" order by NowMoney asc"
	end if
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(1).ColumnWidth=35
			...
			Response.Flush
			set rs=server.CreateObject("adodb.recordset")
			sql="Select a.sort1,a.Id,isNull(b.money1,0) + IsNull(a.NowMoney,0) as agoMoney,IsNull(c.money1,0) as InserMoney," & _
			"IsNull(d.money1,0) as BackMoney,isNull(b.money1,0) + IsNull(a.NowMoney,0) + IsNull(c.money1,0) - IsNull(d.money1,0) as NowMoney"  & _
			"from sortonehy as a"  & _
			"Left join ("&_
			"   Select IsNull(b.money1,0) - IsNull(a.money1,0) as money1,IsNull(a.invoicely,b.invoicely) as invoicely "&_
			"Left join ("&_
			"   from (" & _
			"           Select Sum(money1) as money1,invoicely "&_
			"           from paybackinvoice where isnull(invoicely,0)>0 and del = 1 and isinvoiced=1 and invoicedate <'" & m1 & "' "&_
			"           Group by invoicely"&_
			"   ) as a " & _
			"   full join ( " & _
			"           select Sum(money1) as money1,invoicely "&_
			"           from payoutinvoice where isnull(invoicely,0)<>0 and del = 1 and isinvoiced=1 and invoicedate<'" & m1 & "' "&_
			"           Group by invoicely "&_
			"  ) as b on a.invoicely = b.invoicely "&_
			") as b on a.id = b.invoicely " & _
			"Left join ( " & _
			"  select Sum(money1) as money1,invoicely "&_
			"  from payoutinvoice where isnull(invoicely,0)<>0 and del=1 and isinvoiced=1 and invoicedate between  '" & m1 & "' and '" & m2 & "' "&_
			"  Group by invoicely "&_
			") as c on a.id = c.invoicely " & _
			"Left join ( " & _
			"  Select Sum(money1) as money1,invoicely "&_
			"  from paybackinvoice where isnull(invoicely,0)>0 and del = 1 and isinvoiced=1 and invoicedate between  '" & m1 & "' and '" & m2 & "'"&_
			"  Group by invoicely "&_
			") as d on a.id = d.invoicely " & _
			"where a.gate2 = 37 and a.del = 1 " & Str_Result & px_Result
			rs.open sql,conn,1,1
			C1=rs.recordcount
			xlApplication.Init "票据来源对账列表_"&session("name2006chen")&".xls",C1
			i=1
			while rs.EOF = false
...
'while rs.EOF = false
				xlWorksheet.Cells(1+i,2).Value = Formatnumber(Rs("agoMoney"),num_dot_xs,-1)
'while rs.EOF = false
				xlWorksheet.Cells(1+i,2).font.Size=10
'while rs.EOF = false
				xlWorksheet.Cells(1+i,3).Value = Formatnumber(Rs("InserMoney"),num_dot_xs,-1)
'while rs.EOF = false
				xlWorksheet.Cells(1+i,3).font.Size=10
'while rs.EOF = false
				xlWorksheet.Cells(1+i,4).Value = Formatnumber(Rs("BackMoney"),num_dot_xs,-1)
'while rs.EOF = false
				xlWorksheet.Cells(1+i,4).font.Size=10
'while rs.EOF = false
				xlWorksheet.Cells(1+i,5).Value = Formatnumber(zbcdbl(Rs("NowMoney")),num_dot_xs,-1)
'while rs.EOF = false
...
'while rs.EOF = false
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>票据来源对账列表导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!票据来源对账列表导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("票据来源对账列表_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的票据来源对账列表</u></strong></font></a></p>" & vbcrlf & "" & vbcrlf & "</body>" & vbcrlf & "</html>" & vbcrlf & ""
