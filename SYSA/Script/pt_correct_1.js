...
function changeForm(sindex)
{
	...
	{//ROP
		...
		objLock("M_MRPTitle",false,'1','50');//主题
...
function MRPModify(obj,mrpid,showflg)
{
	...
	showobj.innerHTML="";
	hcdivobj.style.left=offsetx+x;
	hcdivobj.style.top=y-5;
...
function MRPajaxSave()
{
	...
	if(Validator.Validate(formobj,2)&&checkSelection())
	{
		...
		ajaxurl="MRPajaxSave.asp?"+ajaxurl+"&ProductID="+window.cpord+"&stamp=" + (Math.random()*10).toString().replace(".","");
