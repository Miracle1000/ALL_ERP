...
'Private Sub Class_Initialize
	...
	Sub onReportInit(rpt,state)
		cn.CursorLocation = 3
		Dim metype,tord,tdate,complete,ecateid,cateid,stype,scontent,open,t1,t2,findex
		t1=request("t1")
		...
		open=request("open")
		If isnumeric(open)=False Or Len(open)=0 Then open=0
		metype=request("metype")
		If isnumeric(metype)=False Or Len(metype)=0 Then metype=0
		tord=app.base64.deurl(Trim(request.queryString("ord")))
		If isnumeric(tord)=False Or Len(tord)=0 Then tord=0
		complete=request("complete")
		If isnumeric(complete)=False Or Len(complete)=0 Then complete=0
		ecateid=app.sdk.GetGlistUrl(request("ecateid"))
		If Len(ecateid)=0 Then ecateid=GateGroupOrd(3)
		cateid=request("cateid")
		If isnumeric(cateid)=False Or Len(cateid)=0 Then cateid=0
		stype=request("stype")
		If isnumeric(stype)=False Or Len(stype)=0 Then stype=0
		tdate=Split(t1&","&t2, ",")
		scontent=request("scontent")
		rpt.title="要事提醒"
		rpt.addfield "","html","","<style><!--input.anybutton{background-image: url(../images/anybutton_bg.gif);background-repeat: repeat-x;border: 1px solid #9FA3BC;        height: 20px;color: #486D9E;font-size: 12px;margin:0 5px;padding-top: 3px;}--></style>","",""
		'rpt.title="要事提醒"
		rpt.addfield "","select","datetype","预计时间" & Chr(1) & "0" & Chr(2) & "完成时间" & Chr(1) & "1","",""
		rpt.addfield "自：","datetime","date1","","",tdate
		rpt.addfield "","select","metype",getmetypes(state),"",metype
		rpt.addfield "","select","complete", "未完成" & Chr(1) & "1" & Chr(2) & "已完成" & Chr(1) & "2" ,"",complete
		findex = rpt.addfield("","select","ecateid",getcates("执行人员",state,3),"",ecateid)
		rpt.setNullMsg findex, "==执行人员=="
		findex = rpt.addfield("","select","cateid",getcates("添加人员",state,3),"",cateid)
		'rpt.setNullMsg findex, "==添加人员=="
		rpt.addfield "","select","stype",arrName(1) & Chr(1) & "0" & Chr(2) & "要事主题" & Chr(1) & "1" & Chr(2) & "要事备注" & Chr(1) & "2", "",stype
		rpt.addfield "","text", "keytxt" , "", "", scontent
		Dim remind,helper,insertSql
		remind = app.getInt("remind")
		metype = app.getInt("metype")
		If remind > 0 Then
			Set helper = CreateReminderHelper(cn,remind,metype)
			insertSql =  " INSERT INTO #ReminderQueue SELECT * FROM (" & helper.listSQL("ids") & ") a  "
		end if
		rpt.sql =    " SET NOCOUNT ON;" &_
		" select top 0 10000 as id into #ReminderQueue;" & insertSql &_
		" exec erp_report_importtantmsg @datetype,@date1_0,@date1_1,@metype,@complete,@ecateid,@cateid,@stype,@keytxt," & tord & ","& info.User&","&open&",@remind ;" &_
		" DROP TABLE #ReminderQueue;" &_
		" SET NOCOUNT OFF;/*必须静态游标*/"
		rpt.canexcel =  app.power.existsPower(1,10)
		rpt.canprint =  app.power.existsPower(1,7)
		rpt.firstSpace = 0
		...
		Dim topButtonHTML
		topButtonHTML = ""
		If app.power.existsPower(1,5) then
			topButtonHTML = topButtonHTML &"<button class='oldbutton3' onClick=""javascript:window.open('../work/alt_hy.asp?metype=" & metype & "','newwins','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">添加提醒</button>"
'If app.power.existsPower(1,5) then
		end if
		topButtonHTML = topButtonHTML &"&nbsp;<button class='oldbutton' onclick='(new Listview(""mlistvw"")).cexcel(""要事提醒"")'>导出</button>&nbsp;<button class='oldbutton' onclick='window.print()'>打印</button>"
		If app.power.existsPower(1,12) Then
			topButtonHTML = topButtonHTML &"&nbsp;<INPUT style=""MARGIN-LEFT: 0px;padding-top:0px;"" class='oldbutton3' onclick=""javascript:window.open('../work/setMsgtype.asp','newwins','width=' + 1100 + ',height=' + 800 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=0,left=200,top=1')"" value=分类设置 type=button name=Submit32>"
		end if
		rpt.buttonsHTML =topButtonHTML
		rpt.toolminWidth = 1200
		rpt.fullWidthMode = True
	end sub
	Private Function GateGroupOrd(ByVal sType)
		Dim rs,v,open1_1,intro1_1
		Set rs=cn.execute("select qx_open,qx_intro from power  where ord=" & info.user & " and sort1=1 and sort2=11")
		If Not rs.eof Then
			open1_1=rs(0).value
			intro1_1=rs(1).value
		else
		...
		rs.close
		If open1_1=3  Then
			Set rs=cn.execute("select ord from gate  where del=1")
			Do While Not rs.eof
				If v="" Then
					v=rs(0).value
				else
	...
	Function getmetypes(state)
		Dim rs, r, i
		If state = 1 Then
			i = 0
			ReDim r(0)
			r(0) = ""
			Set rs = cn.execute("select ord,sort1 from sort10 where del=1 order by gate2 desc")
			While rs.eof = False
				ReDim Preserve r(i)
				r(i) = rs.fields(1).value & Chr(1) & rs.fields(0).value
				i = i + 1
				'r(i) = rs.fields(1).value & Chr(1) & rs.fields(0).value
				rs.movenext
			wend
			rs.close
			getmetypes = Join(r, Chr(2))
		end if
	end function
	Function getcates(title,state,part)
		Dim Rs, r, i, open_1_1, w1_list, w2_list, w3_list
		Dim str_w1, str_w2, str_w3, W3
		If isnumeric(part)=False Or Len(part)=0 Then part=0
		If state=1 then
			Set Rs = cn.execute("select sort1,qx_open,w1,w2,w3 from power2  where cateid="& Info.User &" and sort1="&part)
			If Not Rs.eof then
				open_1_1=Rs("qx_open") : w1_list=Rs("w1") : w2_list=Rs("w2") : w3_list=Rs("w3")
			else
				open_1_1=0
			end if
			...
			i = 0
			ReDim r(0)
			Set Rs = cn.execute("select ord,name from gate where del=1 "&str_w3&" order by ord asc")
			If Not Rs.eof Then
				Do while Not Rs.eof
					ReDim Preserve r(i)
					r(i) = Rs.fields(1).value & Chr(1) & Rs.fields(0).value
	...
	Sub app_oncreate(lvw)
		...
		lvw.headers(4).title=cn.execute("select isnull((select (case when isnull(Name,'')='' then oldName else Name end) from setfields where gate1=1),'')")(0).value
		col.Formattext = "code:needcheckbox(@cells[1],@value)"
		Set col = lvw.headers(4)
		col.width=150
		col.Formattext = "code:showtellink(""@cells[2]"",""@cells[2]"")"
		col.dbtype="str"
		...
		col.width=120
		col.formattext = "code:getcomplete(""@cells[1]"",""@cells[2]"",""@cells[3]"",""@value"")"
		Set col = lvw.headers(8)
		col.width=120
		col.formattext = "code:formatdate(""@value"")"
	...
	Function formatdate(datestr)
		...
		If Isdate(v) Then
			If InStr(1,v," ",1)>0 Then
				formatdate=CStr(datestr)
			else
				formatdate=CStr(datestr)&" 00:00:00"
			end if
		end if
	end function
	Function getcomplete(iord,tord,metype,v)
		Dim rs,ecateid,cateid
		If Len(tord&"")=0 Then getcomplete="" : Exit Function
		Set rs=cn.execute("select ecateid,cateid from importantMsg where id="&iord)
		If rs.eof=False Then
			ecateid=rs(0).value
			cateid=rs(1).value
		else
		...
		rs.close : Set rs=nothing
		If v&""="未完成" Then
			If ecateid&""=Info.user&"" Or cateid&""=Info.user&"" Then
				getcomplete="<span class='red' style='float:left;margin-left:2px'>未完成</span><br><img src='../images/jiantou.gif'><a href=""#"" onClick=""javascript:window.open('../work/reply_s.asp?ord=" & app.base64.pwurl(tord) & "&metype=" & metype & "&iord=" & iord & "','newwins','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"">解决</a> <img src='../images/jiantou.gif'><a href=""javascript:;"" onClick=""javascript:window.open('../work/alt_hy.asp?ord=" & app.base64.pwurl(tord) & "&metype=" & metype & "&iord=" & iord & "','newwins','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" >修改</a>"
'If ecateid&""=Info.user&"" Or cateid&""=Info.user&"" Then
			else
				getcomplete="<span class='red'>未完成</span>"
			end if
		else
			getcomplete="已完成"
		end if
	end function
	Function showtellink(ord,v)
		Dim rs,s
		If v="0" Or Len(v&"")=0 Then showtellink="" : Exit Function
		If ord="0" Or Len(ord&"")=0 Then showtellink="" : Exit Function
		Set rs=cn.execute("select top 1 name,cateid from tel where ord=" & v)
		If rs.eof=False Then
			s=app.power.GetPowerIntro(1,14)
			If s="" Or InStr(1,"," & s & ",", "," & rs("cateid") & ",",1)>0 Then
				showtellink="<a href=""#"" onClick=""javascript:window.open('../work/content.asp?ord=" & app.base64.pwurl(v) & "','newwins','width=' + 900 + ',height=' + 500 + ',fullscreen =no,scrollbars=1,toolbar=0,resizable=1,left=100,top=100');return false;"" onMouseOver=""window.status='none';return true;""  title=""点击可查看此客户详情""> & rs.fields(0).value & </a>"
			else
				showtellink=rs.fields(0).value
			end if
		else
			showtellink=""
		end if
	...
	Function needcheckbox(ord,v)
		needcheckbox = "<input type='checkbox' name='ord' value='" & v & "'>"
	end function
	Function gatename(ord,v)
		Dim rs
		If v=0 Then getmetype="" : Exit Function
		Set rs=cn.execute("select top 1 name from gate where ord=" & v)
		If rs.eof=False Then
			gatename=rs.fields(0).value
		else
			gatename=""
		end if
		rs.close : Set rs=Nothing
