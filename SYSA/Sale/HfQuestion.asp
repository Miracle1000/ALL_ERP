...
'If Not rs.Eof Then
	...
	Qtype=SaveRequestForm("Qtype")
	gate1 = SaveRequestForm("gate1")
	If gate1 &"" = "" Then gate1=0
	liNum = 0
	For Each RegPara in Request.Form
		If Instr(RegPara,"ListFiled$hf_QuestionLists$answer")>0 Then
			if j=0 then
				ListFiledanswer=SaveRequestForm(RegPara)
			else
				ListFiledanswer=ListFiledanswer&","&SaveRequestForm(RegPara)
			end if
			j=j+1
			ListFiledanswer=ListFiledanswer&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"ListFiled$hf_QuestionLists$point")>0 Then
			if k=0 then
				ListFiledpoint=SaveRequestForm(RegPara)
			else
				ListFiledpoint=ListFiledpoint&","&SaveRequestForm(RegPara)
			end if
			k=k+1
			ListFiledpoint=ListFiledpoint&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"ListFiled$hf_QuestionLists$id__iskey")>0 Then
			if l=0 then
				ListFiledID=SaveRequestForm(RegPara)
			else
				ListFiledID=ListFiledID&","&SaveRequestForm(RegPara)
			end if
			l=l+1
			ListFiledID=ListFiledID&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"ListFiled$hf_QuestionLists$indate")>0 Then
			if m=0 then
				ListFiledIndate=SaveRequestForm(RegPara)
			else
				ListFiledIndate=ListFiledIndate&","&SaveRequestForm(RegPara)
			end if
			m=m+1
			ListFiledIndate=ListFiledIndate&","&SaveRequestForm(RegPara)
		end if
		If Instr(RegPara,"ListFiled$hf_QuestionLists$creator")>0 Then
			if n=0 then
				ListFiledCreator=SaveRequestForm(RegPara)
			else
				ListFiledCreator=ListFiledCreator&","&SaveRequestForm(RegPara)
			end if
			n=n+1
			ListFiledCreator=ListFiledCreator&","&SaveRequestForm(RegPara)
		end if
	...
	If action="save" Then
		If Qtype&""="" Then
			Call jsAlert("请选择题目类型！")
			call db_close : Response.end
		end if
		Set Rs=server.CreateObject("adodb.recordset")
		SQL="Select Top 1 * from hf_Question Where title='"&Title&"'"
		...
		rs.close
		Set Rs=Nothing
		MainID = GetIdentity("hf_Question","id","creator","")
		If ListFiledanswer<>"" Then
			ID = Split(ListFiledID,",")
			answer=Split(ListFiledanswer,",")
			point=Split(ListFiledpoint,",")
			Indate=Split(ListFiledIndate,",")
			Creator=Split(ListFiledCreator,",")
			For i=0 To Ubound(ID)
				Set Rs=server.CreateObject("adodb.recordset")
				SQL="Select Top 1 * from hf_Questionlists Where MainID="&MainID&" And  id="&ID(i)&""
				Rs.Open Sql,conn,1,3
				If Rs.Eof Then
					Rs.AddNew
					Rs("filedid")="104"
					Rs("MainID")=trim(MainID)
					Rs("answer")=trim(answer(i))
					Rs("point")=trim(point(i))
					Rs("InDate")=now
					Rs("Creator")=session("personzbintel2007")
					Rs("del")=1
					rs.update
		...
		Response.write "<script language=''javascript>if(window.parent.opener){window.parent.opener.location.href='HfQuestionList.asp';window.parent.close();}else{window.parent.location.href='HfQuestionList.asp';}</script>"
		call db_close : Response.end
	ElseIf Action="edit" And IsNum(Ord) Then
		Set Rs=server.CreateObject("adodb.recordset")
		SQL="Select Top 1 * from hf_Question Where id="&Ord&""
		rs.open sql,conn,1,1
		If Not Rs.Eof Then
			QuestionTitle=Rs("Title")
			Questiongate1=Rs("gate1")
			QuestionQtype=Rs("Qtype")
		end if
		rs.close
		Set Rs=Nothing
		Set Rs=server.CreateObject("adodb.recordset")
		Sql="select a.* from hf_Questionlists a  where a.MainID="&Ord&" and a.del=1"
		rs.open sql,conn,1,1
		If Not Rs.Eof Then
			liNum = 0
			Do While Not Rs.Eof
				liNum = liNum+ 1
'Do While Not Rs.Eof
				TempList=TempList&"<tr class='filedList_"&Rs("id")&"'><td><div align='center' class='liNum'>"&liNum&"</div></td>"
				TempList=TempList&"<td align='center'><input name='ListFiled$hf_QuestionLists$answer' type='text'   id='ListFiled_answer'  value='"&rs("answer")&"' size='30'  datatype='Limit' min='1' maaax='100'  Msg='必填'>&nbsp;<span class='red'>*</span></td><td align='center'><input name='ListFiled$hf_QuestionLists$point' onKeyUp=""value=value.replace(/[^\d]/g,'')"" type='text' id='ListFiled_point'  value='"&rs("point")&"' size='20'   datatype='Limit' min='1' maaax='100'  Msg='必填'>&nbsp;<span class='red'>*</span></td>"
				TempList=TempList&"<td><div align='center'><input name='ListFiled$hf_QuestionLists$id__iskey' type='hidden'   id='ListFiled_id__iskey'  value='"&rs("id")&"' ><input name='ListFiled$hf_QuestionLists$MainID__mainid' type='hidden'   id='ListFiled_MainID__mainid'  value='"&Ord&"' ><input name='ListFiled$hf_QuestionLists$creator' type='hidden'   id='ListFiled_creator'  value='"&rs("creator")&"' ><input name='ListFiled$hf_QuestionLists$indate' type='hidden'   id='ListFiled_indate'  value='"&rs("indate")&"' '><input name='ListFiled$hf_QuestionLists$del' type='hidden'   id='ListFiled_del'  value='1' ><input name='ListFiled$hf_QuestionLists$FiledID' type='hidden'   id='ListFiled_FiledID' value='104' ><input type='button' name='button' value='删除' onClick='delClassItem(this,104,0);'  class='anybutton2'/></div></td>"
	...
	ElseIf Action="update" And IsNum(Ord) Then
		If conn.execute("select 1 from hf_Question where id<>"&Ord&" and Title = '"&Title&"'").eof = False Then
			Call jsAlert("题目内容重复！")
			call db_close : Response.end
		end if
		Set Rs=server.CreateObject("adodb.recordset")
		...
		If ListFiledanswer<>"" Then
			...
			For i=0 To Ubound(ID)
				...
				Set Rs=Nothing
				If isadd=True Then
					listid = GetIdentity("hf_Questionlists","id","creator","")
					If Len(ListFiledID)>0 Then ListFiledID = ListFiledID &","
					ListFiledID = ListFiledID & listid
				end if
			next
		end if
		If Len(ListFiledID)=0 Then ListFiledID= "0"
		CurrPage = request("CurrPage")
		conn.execute("delete from hf_Questionlists where id not in ("&ListFiledID&") and MainID="&MainID&" ")
		Response.write "<script language=''javascript>alert('修改成功！');if(window.parent.opener){window.parent.opener.location.href='HfQuestionList.asp?CurrPage="&CurrPage&"';window.parent.close();}else{window.parent.location.href='HfQuestionList.asp?CurrPage="&CurrPage&"';}</script>"
		call db_close : Response.end
	ElseIf Action="del"  And IsNum(Ord) Then
		Conn.execute("Delete hf_Question Where id="&Ord&"")
		Conn.execute("Delete hf_Questionlists Where MainID="&Ord&"")
		CurrPage=request("CurrPage")
		Call jsLocat("删除成功！","hfQuestionlist.asp?CurrPage="&CurrPage)
		call db_close : Response.end
	ElseIf Action="dellist"  And OrdList<>"" Then
		If conn.execute("select count(1) from hf_Template where id in (select mainid from hf_TemplateList where questionid in ("&OrdList&") and del=1) and del=1")(0)>0 Then
			Call jsLocat("您选择了已被模板使用的题目，无法删除！","hfQuestionlist.asp")
		else
			CurrPage = request("CurrPage")
			Conn.execute("Delete hf_Question Where id in("&OrdList&")")
			Conn.execute("Delete hf_Questionlists Where MainID in("&OrdList&")")
			Call jsLocat("批量删除成功！","hfQuestionlist.asp?CurrPage="&CurrPage)
		end if
		call db_close : Response.end
	end if
	...
	Response.write Application("sys.info.jsver")
	'Response.write Application("sys.info.jsver")
	Response.write """ type=""text/javascript"" language=""javascript""></script>" & vbcrlf & "<script language=""javascript"" type=""text/javascript"">" & vbcrlf & "function delClassItem(cid,TableListID,listid)" & vbcrlf & "{" & vbcrlf & "       $("".""+cid.parentElement.parentElement.parentElement.className).remove();" & vbcrlf & "           try" & vbcrlf & "     { parent.frameResize();}" & vbcrlf & "        catch(e){}" & vbcrlf & "}" & vbcrlf & "function checkQuestType()" & vbcrlf & "{" & vbcrlf & "   if ($(""input[name=Qtype]:checked"").size()==0)" & vbcrlf & "     {" & vbcrlf & "               alert(""请选择题目类型！"");" & vbcrlf & "                return false;" & vbcrlf & " }" & vbcrlf & "       return true;" & vbcrlf & "}" & vbcrlf & "</script>" & vbcrlf & "</head>" & vbcrlf & "<body>" & vbcrlf & ""
	If Base_ListID="" or isnull(Base_ListID)  then Base_ListID=0
	If EffectiveTime="" or isnull(EffectiveTime) then EffectiveTime=date()
	Response.write "       " & vbcrlf & "<form method=""post"" name=""demo""  action=""?action=save"" id=""demo"" onsubmit=""return checkQuestType()&&Validator.Validate(this,2)"" target=""saveFrom"">" & vbcrlf & "<table width=""100%""  border=""0"" align=""left"" cellpadding=""0"" cellspacing=""0"" bordercolor=""#E2E2E1"" bgcolor=""#FFFFFF"">" & vbcrlf & "  <tr>" & vbcrlf & "    <td width=""100%"" valign=""top"">" & vbcrlf & "                <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" background=""../images/m_mpbg.gif"">" & vbcrlf & "        <tr>" & vbcrlf & "          <td class=""place"">回访题目"
	If action = "edit" Then
		Response.write "修改"
	else
		Response.write "添加"
	end if
	Response.write "</td>" & vbcrlf & "          <td>&nbsp;</td>" & vbcrlf & "          <td align=""right"">&nbsp;</td>" & vbcrlf & "          <td width=""3""><img src=""../images/m_mpr.gif"" width=""3"" height=""32"" /></td>" & vbcrlf & "        </tr>" & vbcrlf & "         </table> " & vbcrlf & "               <table width=""100%""border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" id=""content"">" & vbcrlf & "          <tr class=""tr_1"">" & vbcrlf & "            <td width=""12%""><div align=""right"">题目内容：</div></td>" & vbcrlf & "            <td width=""20%"" class=""gray""><input type=""text"" name=""title"" size=""25"" value="""
	Response.write QuestionTitle
	Response.write """ dataType=""Limit"" min=""1"" max=""50""  msg=""必填"">" & vbcrlf & "              <span class=""red"">*</span></td>" & vbcrlf & "            <td width=""12%""><div align=""right"">题目类型：</div></td>" & vbcrlf & "            <td width=""20%""><input type=""radio"" name=""Qtype"" value=""1"" "
	If QuestionQtype = "1" Then
		Response.write "checked"
	end if
	Response.write "/>单选题&nbsp;<input type=""radio"" name=""Qtype"" value=""2""  "
	If QuestionQtype = "2" Then
		Response.write "checked"
	end if
	Response.write "/>多选题&nbsp;<input type=""radio"" name=""Qtype"" value=""3"" "
	If QuestionQtype = "3" Then
		Response.write "checked"
	end if
	Response.write "/>简答题&nbsp;<span class=""red"">*</span></td>" & vbcrlf & "            <td width=""12%""><div align=""right"">重要指数：</div></td>" & vbcrlf & "            <td ><input name=""Gate1"" type=""text""  onKeyUp=""value=value.replace(/[^\d]/g,'')""  id=""Gate1""  value="""
	Response.write Questiongate1
	Response.write """ size=""0""  maxlength=15  datatype=""Limit"" min=""0"" max=""100""  Msg=""格式不符""></td>" & vbcrlf & "          </tr>" & vbcrlf & "          <tr class=""tr_2"">" & vbcrlf & "                  <td width=""12%"" height=""33"" ><div align=""right"">答案及分值：</div></td>" & vbcrlf & "            <td colspan=""5"">"& vbcrlf & "                              <table width=""100%"" border=""0"" cellpadding=""6"" cellspacing=""1"" bgcolor=""#C0CCDD"" class=""home"" >" & vbcrlf & "                             <tr class=""top"">" & vbcrlf & "                          <td width=60><div align=""center"">序号</div></td>" & vbcrlf & "                          <td><div align='center'>答案</div></td>" & vbcrlf & "                         <td><div align='center'>分数</div></td>" & vbcrlf & "                              <td width=80><div align=""center"">操作</div></td>" & vbcrlf & "                          </tr >" & vbcrlf & "                          "
	if action="edit" then Response.write TempList
	Response.write "" & vbcrlf & "                             <tr  id=""FieldList"" height=""33"">" & vbcrlf & "                            <td colspan=""4"">" & vbcrlf & "                          <script language=""javascript"" type=""text/javascript""> " & vbcrlf & "                              var liNum="
	Response.write liNum
	Response.write ";" & vbcrlf & "                            function addFiledList()" & vbcrlf & "                         {" & vbcrlf & "                               liNum=liNum+1;" & vbcrlf & "                          var date=new Date();" & vbcrlf & "                            var trID=date.getHours()+""-""+date.getMinutes()+""-""+date.getSeconds()+""-""+date.getMilliseconds();" & vbcrlf & "                              var tempStr=""<tr  class='filedList_""+trID+""'>""" & vbcrlf & "                             +""<td><div align='center' class='liNum'>""+liNum+""</div></td>""" & vbcrlf & "                               +""<td align='center'><input name='ListFiled$hf_QuestionLists$answer' type='text'   id='ListFiled_answer'  value='' size='30'   datatype='Limit' min='1' maaax='100'  Msg='必填'>&nbsp;<span class='red'>*</span></td><td align='center'><input name='ListFiled$hf_QuestionLists$point' onKeyUp=\""value=value.replace(/[^\\d]/g,'')\"" type='text'   id='ListFiled_point'  value='' size='20'   datatype='Limit' min='1' maaax='100'  Msg='必填'>&nbsp;<span class='red'>*</span></td>""" & vbcrlf & "                          +""<td><div align='center'><input name='ListFiled$hf_QuestionLists$id__iskey' type='hidden'   id='ListFiled_id__iskey'  value='0' ><input name='ListFiled$hf_QuestionLists$MainID__mainid' type='hidden'   id='ListFiled_MainID__mainid'  value='0' ><input name='ListFiled$hf_QuestionLists$creator' type='hidden'   id='ListFiled_creator'  value='"
	Response.write session("personzbintel2007")
	Response.write "' ><input name='ListFiled$hf_QuestionLists$indate' type='hidden'   id='ListFiled_indate'  value='2014-05-26 17:17:06' ><input name='ListFiled$hf_QuestionLists$del' type='hidden'   id='ListFiled_del'  value='1' ><input name='ListFiled$hf_QuestionLists$FiledID' type='hidden'   id='ListFiled_FiledID'  value='104' ><input type='button' name='button' value='删除' onClick='delClassItem(this,104,0);'  class='anybutton2'/></div></td>""" & vbcrlf & "                         +""</tr>"";" & vbcrlf & "                         $(""#FieldList"").before(tempStr);" & vbcrlf & "                          try" & vbcrlf & "                             { parent.frameResize();}" & vbcrlf & "                                catch(e){}" & vbcrlf & "                             }" & vbcrlf & "                               </script>" & vbcrlf & "                               <div align=""center""><img class=""resetElementHidden"" src=""../images/jiantou.gif"" /><img class='resetElementShow' style='display:none;' src='../skin/default/images/MoZihometop/content/lvw_addrow_btn.png' /><a title=""点击添加明细""onclick=""addFiledList()"" href=""javaScript:void(0)"">添加明细</a></div>" & vbcrlf & "                          </td>" & vbcrlf & "                           </tr>" & vbcrlf & "                           </table>" & vbcrlf & "                        </td>" & vbcrlf & "          </tr>" & vbcrlf & "            </table>" & vbcrlf & "        </td>" & vbcrlf & "    </tr>" & vbcrlf & "  <tr>" & vbcrlf & "    <td  class=""page"">" & vbcrlf & "         <table width=""100%"" border=""0"" align=""left"" >" & vbcrlf & "           <tr>" & vbcrlf & "                  <td  colspan=""6"" class=""gray""><p align=""center"">" & vbcrlf & "                              "
	if action="edit" then
	...
	Response.write "" & vbcrlf & "                             <input type=""reset"" value="" 重填 "" class=""page"" name=""B2"">" & vbcrlf & "                      </td>" & vbcrlf & "             </tr>" & vbcrlf & "          </table></td></tr>    " & vbcrlf & " </td></tr>" & vbcrlf & "</table>" & vbcrlf & "</form> " & vbcrlf & "<iframe name=""saveFrom"" id=""saveFrom"" style=""width:0;height:0""></iframe>" & vbcrlf & "</body>" & vbcrlf & "</html>"
