...
'If Len(intro_67_1&"") = 0 Then intro_67_1 = 0
	...
	if sortid<>"" and isnumeric(sortid) then
		...
		sql5="select * from sms_replace_str where del=1 and (sortid="&sortid&" or sortid=0)"
		rs5.open sql5,conn,1,1
		if not rs5.eof then
			replacelist=""
			do while not rs5.eof
				replacelist=replacelist&"<span class=""replace"" title=""单击复制参数"" id=""replace"&rs5("ord")&""" onclick=""copyClick(this.id)"">"&rs5("title")&"="&rs5("replaceStr")&"</span>&nbsp;&nbsp;"
				rs5.movenext
			loop
		end if
		rs5.close
