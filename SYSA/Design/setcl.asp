...
'If sdk.power.existsPowerIntro(5029,14, designer) Then
	...
	Sub Bill_OnPageinit
		Response.write "" & vbcrlf & " <style>" & vbcrlf & " #comm_itembarbg{height:43px!important;}" & vbcrlf & " #editbody{border-top:0!important;margin-top:-1px}" & vbcrlf & " </style>" & vbcrlf & ""
'Sub Bill_OnPageinit
	end sub
	Sub Bill_OnInit(ByVal bill, ByVal t)
		...
		Dim DSource : DSource = getDesignTypeList("source","", IsNetProduce)
		If len(DSource)>0 Then
			Dim designtype : designtype = app.glAttribute("MI_DESIGN_TYPE_2016051601")
			With Bill.fields.add1("允许生成设计任务的来源","designtype","checkbox","varchar",True,True,false,False)
			.source = "options:"& DSource
			.defvalue= designtype
			.colspan = 2
			End With
		end if
		Dim designstatus : designstatus = app.glAttribute("MI_DESIGN_STATUS_2016051602")
		With Bill.fields.add1("设计池可查看设计状态","designstatus","checkbox","varchar",True,True,false,False)
		.source = "options:未领用=1;申请中=2;已领用=3;待设计=4;设计中=5;设计完毕=6;设计待审批=7;审批通过=8;审批未通过=9;以上所有状态=0"
		.defvalue= designstatus
		.js = " onclick='setChecks(this)' "
		.colspan = 2
		End With
	...
	Function Bill_OnSave(ByVal bill)
		Dim designtype : designtype = replace(app.getText("designtype")," ","")
		Dim designstatus : designstatus = replace(app.getText("designstatus")," ","")
		app.glAttribute("MI_DESIGN_TYPE_2016051601")=designtype
		app.glAttribute("MI_DESIGN_STATUS_2016051602") = designstatus
		bill.showSaveResultEx "保存成功",false,"self",""
		Bill_OnSave = True
	end function
