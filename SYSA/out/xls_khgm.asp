...
'If font.color & "" <> "" Then cssText = cssText & "color:" & font.color & ";"
	...
	dotstr=""
		for i=1 to num_dot_xs
			...
			xlApplication.ActiveSheet.Columns(8).NumberFormatLocal = "#0."&dotstr&"% "
			...
			zdy5=request("zdy5")
			zdy6=request("zdy6")
			Str_Result2=Str_Result2 + "and x.del=1 and x.sort3=1"
			zdy6=request("zdy6")
			ksjs=request("B")
			ksjs2=request("C")
			m1=request("ret")
			...
			if F2<>"" then
				str_Result2=str_Result2+"and x.name like '%"& F2 &"%'"
'if F2<>"" then
			end if
			if S2<>"" then
				str_Result2=str_Result2+"and x.pym like '%"& S2 &"%'"
'if S2<>"" then
			end if
			if Y2<>"" then
				str_Result2=str_Result2+"and x.khid like '%"& Y2 &"%'"
'if Y2<>"" then
			end if
			if G2<>"" then
				str_Result2=str_Result2+"and x.phone like '%"& G2 &"%'"
'if G2<>"" then
			end if
			if P2<>"" then
				str_Result2=str_Result2+"and x.fax like '%"& P2 &"%'"
'if P2<>"" then
			end if
			if I2<>"" then
				str_Result2=str_Result2+"and x.url like '%"& I2 &"%'"
'if I2<>"" then
			end if
			if J2<>"" then
				str_Result2=str_Result2+"and x.address like '%"& J2 &"%'"
'if J2<>"" then
			end if
			if K2<>"" then
				str_Result2=str_Result2+"and x.zip like '%"& K2 &"%'"
'if K2<>"" then
			end if
			if T2<>"" then
				str_Result2=str_Result2+"and x.intro like '%"& T2 &"%'"
'if T2<>"" then
			end if
			dim HtType
			...
			if HtType<>"" then
				select case HtType
				case 0
				Str_Result=Str_Result+" and (paybacktype=0 or paybacktype is null) AND (y.sort1 is null or y.sort1=0) "
'case 0   '
				Str_Result2=Str_Result2+" and (paybacktype=0 or paybacktype is null) AND (y.sort1 is null or y.sort1=0) "
'case 0   '
				case 2
				Str_Result=Str_Result+" and paybacktype=1 AND (y.sort1 is null or y.sort1=0) "
'case 2   '
				Str_Result2=Str_Result2+" and paybacktype=1 AND (y.sort1 is null or y.sort1=0) "
'case 2   '
				Case 3
				Str_Result=Str_Result+" AND y.sort1 = 1 "
'Case 3   '
				Str_Result2=Str_Result2+" AND y.sort1 = 1 "
'Case 3   '
				Case 4
...
'Case 4   '
				Case 6
				Str_Result=Str_Result+" AND exists(select 1 from contractlist where contract = y.ord and isnull(treeord,0)>0)  "
'Case 6 '
				Str_Result2=Str_Result2+" AND exists(select 1 from contractlist where contract = y.ord and isnull(treeord,0)>0)  "
'Case 6 '
				end select
			end if
			if m1<>"" then
				Str_Result2=Str_Result2+"and  y.date1>='"&m1&"'"
'if m1<>"" then
				Str_Result_fy=Str_Result_fy+"and  date1>='"&m1&"'"
'if m1<>"" then
				Str_Result_cp=Str_Result_cp+"and  date1>='"&m1&"'"
'if m1<>"" then
			end if
			if m2<>"" then
				Str_Result2=Str_Result2+"and  y.date1<='"&m2&" 23:59:59'"
'if m2<>"" then
				Str_Result_fy=Str_Result_fy+"and  date1<='"&m2&" 23:59:59'"
'if m2<>"" then
				Str_Result_cp=Str_Result_cp+"and  date1<='"&m2&" 23:59:59'"
'if m2<>"" then
			end if
			set rs=server.CreateObject("adodb.recordset")
			sql="select x.ord,x.name,x.khid,x.cateid,isnull(sum(y.money2),0) as money1,count(y.ord) as numht,  " & vbcrlf & _
			"( select isnull(sum(a.FinaMoney),0) " & vbcrlf & _
			"  from kuoutlist2 a WITH(NOLOCK) "&_
			"   where a.sort1 in (1,4) and a.company=x.ord and a.del=1 "&Str_Result_cp & _
			") as money_cpall " & vbcrlf & _
			"from tel as x  inner join contract as y on x.ord=y.company and y.del=1 and isnull(y.status,-1) in (-1,1) "&Str_Result2&" group by x.ord,x.name,x.khid,x.cateid order by money1 desc" &_
			"rs.open sql,conn,1,1"
			C1=rs.recordcount
			xlApplication.Init "客户购买总额排行_"&session("name2006chen")&".xls",C1
			dim i
			i=1
			do until rs.eof
				k=rs("name")
				...
				set rs7=server.CreateObject("adodb.recordset")
				sql7="select isnull(sum(money1),0) as pay from pay where company="&ord&" and del=1 and complete=3 "&Str_Result_fy&""
				rs7.open sql7,conn,1,1
				pay=rs7("pay")
				rs7.close
				set rs7=nothing
				money_mlall=cdbl(money1)-cdbl(money_cpall)-cdbl(pay)
				set rs7=nothing
				money_ll=0
				if money1<>0 then
					money_ll=money_mlall*100/cdbl(money1)
				end if
				xlWorksheet.Cells(1+i,1).Value = khid
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,1).font.Size=10
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,2).Value = k
				money_ll=money_mlall*100/cdbl(money1)
				...
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,4).Value =numht
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,4).font.Size=10
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,5).Value =Formatnumber(pay,num_dot_xs,-1)
				money_ll=money_mlall*100/cdbl(money1)
				xlWorksheet.Cells(1+i,5).font.Size=10
				money_ll=money_mlall*100/cdbl(money1)
				if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,6).Value =Formatnumber(money_cpall,num_dot_xs,-1)
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,6).font.Size=10
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,7).Value =Formatnumber(money_mlall,num_dot_xs,-1)
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,7).font.Size=10
'if open_5_17=1 or open_5_17=3 then
					xlWorksheet.Cells(1+i,8).Value =Formatnumber(money_ll,num_dot_xs,-1)&"%"
'if open_5_17=1 or open_5_17=3 then
...
'if open_5_17=1 or open_5_17=3 then
			...
			rs.close
			set rs=Nothing
			Response.write "<script>CountImage.width=710;CountTXT.innerHTML=""<font color=red><b>客户购买总额排行导出全部完成!</b></font>  100"";CountTXTok.innerHTML=""<B>恭喜!客户购买总额排行导出成功,共有"&(i-1)&"条记录!</B>"";</script>"
			set rs=Nothing
			Response.write "" & vbcrlf & "</BODY>" & vbcrlf & "</HTML>" & vbcrlf & ""
			Set fs = CreateObject("Scripting.FileSystemObject")
			tfile=Server.MapPath("客户购买总额排行_"&session("name2006chen")&".xls")
			...
			Response.write """><font class=""red""><strong><u>下载导出的客户购买总额排行</u></strong></font></a></p> "
